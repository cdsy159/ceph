...
struct AuthTicket {
  ...
  void
  init_timestamps(utime_t now, double ttl)
  {
    created = now;
    expires = now;
    expires += ttl;
...
struct AuthAuthorizer {
  ...
  virtual bool verify_reply(
      ceph::buffer::list::const_iterator& reply,
      std::string* connection_secret) = 0;
  virtual bool add_challenge(
      CephContext* cct,
...
struct AuthConnectionMeta {
  ...
  size_t
  get_connection_secret_length() const
  {
    switch (con_mode) {
...
struct RotatingSecrets {
  ...
  bool
  need_new_secrets() const
  {
    return secrets.size() < KEY_ROTATE_NUM;
  }

  bool
  need_new_secrets(const utime_t& now) const
  {
...
inline bool
auth_principal_needs_rotating_keys(EntityName& name)
{
  uint32_t ty(name.get_type());
  return (
      (ty == CEPH_ENTITY_TYPE_OSD) || (ty == CEPH_ENTITY_TYPE_MDS) ||
