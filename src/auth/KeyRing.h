...
public:
  ...
  // accessors
  bool
  exists(const EntityName& name) const
  {
  ...
  bool
  get_auth(const EntityName& name, EntityAuth& a) const
  {
  ...
  bool
  get_secret(const EntityName& name, CryptoKey& secret) const override
  {
    std::map<EntityName, EntityAuth>::const_iterator k = keys.find(name);
    if (k == keys.end())
      return false;
  ...
  bool
  get_service_secret(uint32_t service_id, uint64_t secret_id, CryptoKey& secret)
      const override
  {
    return false;
  }
  ...
  bool
  get_caps(const EntityName& name, const std::string& type, AuthCapsInfo& caps)
      const
  {
    std::map<EntityName, EntityAuth>::const_iterator k = keys.find(name);
    if (k == keys.end())
      return false;
    std::map<std::string, ceph::buffer::list>::const_iterator i =
        k->second.caps.find(type);
  ...
  // modifiers
  void
  add(const EntityName& name, const EntityAuth& a)
  {
    keys[name] = a;
  }

  void
  add(const EntityName& name, const CryptoKey& k)
  {
    EntityAuth a;
    a.key = k;
    keys[name] = a;
  ...
  void
  add(const EntityName& name, const CryptoKey& k, const CryptoKey& pk)
  {
    EntityAuth a;
    a.key = k;
    a.pending_key = pk;
  ...
  void
  set_key(EntityName& ename, CryptoKey& key)
  {
    keys[ename].key = key;
  }
...
static inline void
decode(KeyRing& kr, ceph::buffer::list::const_iterator& p)
{
  kr.decode(p);
}
