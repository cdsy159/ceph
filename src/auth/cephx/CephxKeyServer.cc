...
void
KeyServerData::encode(ceph::buffer::list& bl) const
{
  __u8 struct_v = 1;
  using ceph::encode;
  encode(struct_v, bl);
...
void
KeyServerData::decode(ceph::buffer::list::const_iterator& bl)
{
  using ceph::decode;
  __u8 struct_v;
  decode(struct_v, bl);
...
void
KeyServerData::encode_rotating(ceph::buffer::list& bl) const
{
  using ceph::encode;
  __u8 struct_v = 1;
  encode(struct_v, bl);
...
void
KeyServerData::decode_rotating(ceph::buffer::list& rotating_bl)
{
  using ceph::decode;
...
void
KeyServerData::dump(ceph::Formatter* f) const
{
...
bool
KeyServerData::get_service_secret(
    CephContext* cct,
    uint32_t service_id,
    CryptoKey& secret,
    uint64_t& secret_id,
    double& ttl) const
{
  ...
  if (iter == rotating_secrets.end()) {
    ldout(cct, 10) << "get_service_secret service "
                   << ceph_entity_type_name(service_id) << " not found "
                   << dendl;
    return false;
  }
  ...
  ttl = std::min(
      ttl,
      static_cast<double>(secrets.secrets.rbegin()->second.expiration - now));

  ldout(cct, 30) << __func__ << " service " << ceph_entity_type_name(service_id)
                 << " secret_id " << secret_id << " " << riter->second
                 << " ttl " << ttl << dendl;
  return true;
}
    ...
    CryptoKey& secret) const
{
  auto iter = rotating_secrets.find(service_id);
  if (iter == rotating_secrets.end()) {
    ldout(cct, 10) << __func__ << " no rotating_secrets for service "
                   << service_id << " " << ceph_entity_type_name(service_id)
                   << dendl;
    return false;
  }
  ...
  if (riter == secrets.secrets.end()) {
    ldout(cct, 10) << "get_service_secret service "
                   << ceph_entity_type_name(service_id) << " secret "
                   << secret_id << " not found" << dendl;
    ldout(cct, 30) << " I have:" << dendl;
    for (auto iter = secrets.secrets.begin(); iter != secrets.secrets.end();
         ++iter)
...
bool
KeyServerData::get_auth(const EntityName& name, EntityAuth& auth) const
{
...
bool
KeyServerData::get_secret(const EntityName& name, CryptoKey& secret) const
{
  auto iter = secrets.find(name);
  if (iter != secrets.end()) {
    ...
    AuthCapsInfo& caps_info) const
{
  ...
  auto iter = secrets.find(name);
  if (iter != secrets.end()) {
    ldout(cct, 10) << "get_caps: num of caps=" << iter->second.caps.size()
                   << dendl;
...
void
KeyServerData::Incremental::encode(ceph::buffer::list& bl) const
{
  using ceph::encode;
  __u8 struct_v = 1;
  encode(struct_v, bl);
...
void
KeyServerData::Incremental::decode(ceph::buffer::list::const_iterator& bl)
{
  using ceph::decode;
  __u8 struct_v;
  decode(struct_v, bl);
...
void
KeyServerData::Incremental::dump(ceph::Formatter* f) const
{
...
KeyServer::KeyServer(CephContext* cct_, KeyRing* extra_secrets) :
  cct(cct_), data(extra_secrets), lock{ceph::make_mutex("KeyServer::lock")}
{}
...
void
KeyServer::_dump_rotating_secrets()
{
  ldout(cct, 30) << "_dump_rotating_secrets" << dendl;
  for (auto iter = data.rotating_secrets.begin();
       iter != data.rotating_secrets.end(); ++iter) {
    RotatingSecrets& key = iter->second;
    for (auto mapiter = key.secrets.begin(); mapiter != key.secrets.end();
         ++mapiter)
      ldout(cct, 30) << "service " << ceph_entity_type_name(iter->first)
                     << " id " << mapiter->first << " key " << mapiter->second
                     << dendl;
  }
}

int
KeyServer::_rotate_secret(uint32_t service_id, KeyServerData& pending_data)
{
  ...
  while (r.need_new_secrets(now)) {
    ...
    ldout(cct, 10) << "_rotate_secret adding "
                   << ceph_entity_type_name(service_id) << dendl;
    ldout(cct, 30) << "_rotate_secret adding "
                   << ceph_entity_type_name(service_id) << " id " << secret_id
                   << " " << ek << dendl;
    added++;
  }
  return added;
...
bool
KeyServer::get_secret(const EntityName& name, CryptoKey& secret) const
{
  std::scoped_lock l{lock};
...
bool
KeyServer::get_auth(const EntityName& name, EntityAuth& auth) const
{
  std::scoped_lock l{lock};
...
void
KeyServer::note_used_pending_key(const EntityName& name, const CryptoKey& key)
{
  std::scoped_lock l(lock);
...
void
KeyServer::clear_used_pending_keys()
{
  std::scoped_lock l(lock);
  used_pending_keys.clear();
}

std::map<EntityName, CryptoKey>
...
void
KeyServer::dump(Formatter* f) const
{
  f->dump_object("data", data);
}

std::list<KeyServer>
KeyServer::generate_test_instances()
{
...
bool
KeyServer::generate_secret(CryptoKey& secret)
{
  bufferptr bp;
...
bool
KeyServer::generate_secret(EntityName& name, CryptoKey& secret)
{
...
void
KeyServer::encode(ceph::buffer::list& bl) const
{
  using ceph::encode;
  encode(data, bl);
}
...
void
KeyServer::decode(ceph::buffer::list::const_iterator& bl)
{
  std::scoped_lock l{lock};
  using ceph::decode;
  decode(data, bl);
...
bool
KeyServer::contains(const EntityName& name) const
{
  std::scoped_lock l{lock};
...
int
KeyServer::encode_secrets(Formatter* f, stringstream* ds) const
{
  std::scoped_lock l{lock};
...
void
KeyServer::encode_formatted(string label, Formatter* f, bufferlist& bl)
{
  ceph_assert(f != NULL);
...
void
KeyServer::encode_plaintext(bufferlist& bl)
{
  stringstream os;
...
bool
KeyServer::prepare_rotating_update(bufferlist& rotating_bl)
{
  std::scoped_lock l{lock};
  ldout(cct, 20) << __func__
                 << " before: data.rotating_ver=" << data.rotating_ver << dendl;
  ...
  ldout(cct, 20) << __func__ << " after: pending_data.rotating_ver="
                 << pending_data.rotating_ver << dendl;
  pending_data.encode_rotating(rotating_bl);
  return true;
}

bool
KeyServer::get_rotating_encrypted(const EntityName& name, bufferlist& enc_bl) const
{
  std::scoped_lock l{lock};
...
int
KeyServer::_build_session_auth_info(
    uint32_t service_id,
    ...
    CephXSessionAuthInfo& info)
{
  ...
  return _build_session_auth_info(
      service_id, parent_ticket, info, cct->_conf->auth_service_ticket_ttl);
}
