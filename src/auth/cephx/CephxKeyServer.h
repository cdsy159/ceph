...
struct KeyServerData {
  ...
  explicit KeyServerData(KeyRing* extra) :
    version(0), extra_secrets(extra), rotating_ver(0)
  {}
  ...
  bool
  contains(const EntityName& name) const
  {
  ...
  void
  add_auth(const EntityName& name, EntityAuth& auth)
  {
    secrets[name] = auth;
  }

  void
  remove_secret(const EntityName& name)
  {
  ...
  std::map<EntityName, EntityAuth>::iterator
  secrets_begin()
  {
    return secrets.begin();
  }

  std::map<EntityName, EntityAuth>::const_iterator
  secrets_begin() const
  {
    return secrets.begin();
  }
  ...
  std::map<EntityName, EntityAuth>::iterator
  find_name(const EntityName& name)
  {
    return secrets.find(name);
  }

  std::map<EntityName, EntityAuth>::const_iterator
  find_name(const EntityName& name) const
  {
    return secrets.find(name);
  }
  ...
  void
  apply_incremental(Incremental& inc)
  {
    switch (inc.op) {
    case AUTH_INC_ADD:
...
class KeyServer : public KeyStore {
  ...
  int _build_session_auth_info(
      uint32_t service_id,
      const AuthTicket& parent_ticket,
      CephXSessionAuthInfo& info,
      double ttl);
  bool _get_service_caps(
      const EntityName& name,
      uint32_t service_id,
      AuthCapsInfo& caps) const;
...
public:
  ...
  bool get_caps(
      ...
      const std::string& type,
      AuthCapsInfo& caps) const;
  bool get_active_rotating_secret(
      const EntityName& name,
      CryptoKey& secret) const;
  ...
  int
  list_secrets(std::stringstream& ds) const
  {
  ...
  void
  apply_data_incremental(KeyServerData::Incremental& inc)
  {
    std::scoped_lock l{lock};
  ...
  void
  clone_to(KeyServerData& dst) const
  {
    std::scoped_lock l{lock};
    dst = data;
  }
  ...
  void
  export_keyring(KeyRing& keyring)
  {
    std::scoped_lock l{lock};
