    ...
    CephXTicketBlob& blob)
{
  ...
  ldout(cct, 10) << "build_service_ticket service "
                 << ceph_entity_type_name(info.service_id) << " secret_id "
                 << info.secret_id << " ticket_info.ticket.name="
                 << ticket_info.ticket.name.to_str() << " ticket.global_id "
                 << info.ticket.global_id << dendl;
  blob.secret_id = info.secret_id;
  std::string error;
  if (!info.service_secret.get_secret().length())
    error = "invalid key"; // Bad key?
  else
    encode_encrypt_enc_bl(
        cct, ticket_info, info.service_secret, blob.blob, error);
  if (!error.empty()) {
    ...
    bufferlist& reply)
{
  ...
  for (auto ticket_iter = ticket_info_vec.begin();
       ticket_iter != ticket_info_vec.end(); ++ticket_iter) {
  ...
  try {
    ...
    if (ticket_enc) {
      ...
      if (decode_decrypt(cct, service_ticket_bl, session_key, indata, error)) {
        ldout(cct, 10) << __func__ << " decode_decrypt failed " << "with "
                       << error << dendl;
        return false;
      }
    } else {
    ...
    ldout(cct, 10) << __func__ << " service "
                   ...
                   << ticket.secret_id << " session_key " << msg_a.session_key
...
bool
CephXTicketHandler::need_key() const
{
  if (have_key_flag) {
...
bool
CephXTicketManager::have_key(uint32_t service_id)
{
...
bool
CephXTicketManager::need_key(uint32_t service_id) const
{
  auto iter = tickets_map.find(service_id);
  if (iter == tickets_map.end())
    return true;
...
void
CephXTicketManager::invalidate_ticket(uint32_t service_id)
{
  auto iter = tickets_map.find(service_id);
  if (iter != tickets_map.end())
    ...
    bufferlist::const_iterator& indata)
{
  ...
  try {
    ...
    ldout(cct, 10) << __func__
                   << " failed to decode ticket v or count: " << e.what()
                   << dendl;
  }
  ldout(cct, 10) << "verify_service_ticket_reply got " << num << " keys"
                 << dendl;
  ...
  for (int i = 0; i < (int)num; i++) {
    ...
    try {
      ...
      ldout(cct, 10) << __func__
                     << " failed to decode ticket type: " << e.what() << dendl;
    }
    ldout(cct, 10) << "got key for service_id " << ceph_entity_type_name(type)
                   << dendl;
 ...
 */
CephXAuthorizer*
CephXTicketHandler::build_authorizer(uint64_t global_id) const
{
 ...
 */
CephXAuthorizer*
CephXTicketManager::build_authorizer(uint32_t service_id) const
{
  auto iter = tickets_map.find(service_id);
  if (iter == tickets_map.end()) {
    ...
    CephXServiceTicketInfo& ticket_info)
{
  ...
  if (secret_id == (uint64_t)-1) {
    if (!keys->get_secret(cct->_conf->name, service_secret)) {
      ldout(cct, 0) << "ceph_decode_ticket could not get general service "
                       "secret for service_id="
                    << ceph_entity_type_name(service_id)
                    << " secret_id=" << secret_id << dendl;
      return false;
    }
  } else {
    if (!keys->get_service_secret(service_id, secret_id, service_secret)) {
      ldout(cct, 0)
          << "ceph_decode_ticket could not get service secret for service_id="
          << ceph_entity_type_name(service_id) << " secret_id=" << secret_id
          << dendl;
      return false;
    }
  }
  ...
  std::string error;
  decode_decrypt_enc_bl(
      cct, ticket_info, service_secret, ticket_blob.blob, error);
  if (!error.empty()) {
    ...
    bufferlist* reply_bl)
{
  ...
  if (ticket.secret_id == (uint64_t)-1) {
    ...
    if (!keys.get_secret(name, service_secret)) {
      ldout(cct, 0) << "verify_authorizer could not get general service secret "
                       "for service "
                    << ceph_entity_type_name(service_id)
                    << " secret_id=" << ticket.secret_id << dendl;
      return false;
  ...
  if (ticket_info.ticket.global_id != global_id) {
    ldout(cct, 0) << "verify_authorizer global_id mismatch: declared id="
                  << global_id << " ticket_id=" << ticket_info.ticket.global_id
                  << dendl;
    return false;
  }
  ...
  if (decode_decrypt(cct, auth_msg, ticket_info.session_key, indata, error)) {
    ldout(cct, 0)
        << "verify_authorizercould not decrypt authorize request with error: "
        << error << dendl;
    return false;
  }
  ...
  if (challenge) {
    ...
    if (!auth_msg.have_challenge || !c) {
      ...
      challenge->reset(c);
      cct->random()->get_bytes(
          (char*)&c->server_challenge, sizeof(c->server_challenge));
      ldout(cct, 10) << __func__ << " adding server_challenge "
                     << c->server_challenge << dendl;

      encode_encrypt_enc_bl(cct, *c, ticket_info.session_key, *reply_bl, error);
      if (!error.empty()) {
        ldout(cct, 10) << "verify_authorizer: encode_encrypt error: " << error
                       << dendl;
        return false;
      }
      return false;
    ...
    ldout(cct, 10) << __func__ << " got server_challenge+1 "
                   << auth_msg.server_challenge_plus_one << " expecting "
  ...
  if (connection_secret) {
    ...
    if (connection_secret_required_len) {
      ...
      std::generate_n(
          connection_secret->data(), connection_secret_required_len,
  ...
  ldout(cct, 10) << "verify_authorizer ok nonce " << hex << auth_msg.nonce
                 << dec << " reply_bl.length()=" << reply_bl->length() << dendl;
  return true;
}
    ...
    std::string* connection_secret)
{
  ...
  if (decode_decrypt(cct, reply, session_key, indata, error)) {
    ldout(cct, 0) << "verify_reply couldn't decrypt with error: " << error
                  << dendl;
    return false;
  }
  ...
  if (expect != reply.nonce_plus_one) {
    ldout(cct, 0) << "verify_authorizer_reply bad nonce got "
                  << reply.nonce_plus_one << " expected " << expect << " sent "
                  << nonce << dendl;
    return false;
  }
...
bool
CephXAuthorizer::add_challenge(CephContext* cct, const bufferlist& challenge)
{
  bl = base_bl;
  ...
  if (!p.end()) {
    ...
    if (!error.empty()) {
      ldout(cct, 0) << "failed to decrypt challenge (" << challenge.length()
                    << " bytes): " << error << dendl;
      return false;
    }
  ...
  if (encode_encrypt(cct, msg, session_key, bl, error)) {
    ldout(cct, 0) << __func__ << " failed to encrypt authorizer: " << error
                  << dendl;
    return false;
  }
  return true;
