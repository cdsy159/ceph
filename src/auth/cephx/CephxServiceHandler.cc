...
int
CephxServiceHandler::do_start_session(
    ...
    AuthCapsInfo* caps)
{
  ...
  ldout(cct, 10) << "start_session server_challenge " << hex << server_challenge
                 << dec << dendl;
...
int
CephxServiceHandler::verify_old_ticket(
    ...
    bool& should_enc_ticket)
{
  ...
  if (!req.old_ticket.blob.length()) {
    ...
    if (cct->_conf->auth_allow_insecure_global_id_reclaim &&
      ...
      ldout(cct, 10) << " allowing reclaim of global_id " << global_id
                     << " with no ticket presented (legacy client, "
  ...
  if (!cephx_decode_ticket(
          cct, key_server, CEPH_ENTITY_TYPE_AUTH, req.old_ticket,
          old_ticket_info)) {
    if (cct->_conf->auth_allow_insecure_global_id_reclaim &&
        req.old_ticket_may_be_omitted) {
      ldout(cct, 10) << " allowing reclaim of global_id " << global_id
                     << " using bad ticket (legacy client, "
                        "auth_allow_insecure_global_id_reclaim=true)"
                     << dendl;
      global_id_status = global_id_status_t::RECLAIM_INSECURE;
      return 0;
  ...
  if (global_id != old_ticket_info.ticket.global_id) {
    if (cct->_conf->auth_allow_insecure_global_id_reclaim &&
        req.old_ticket_may_be_omitted) {
      ldout(cct, 10) << " allowing reclaim of global_id " << global_id
                     << " using mismatching ticket (legacy client, "
                        "auth_allow_insecure_global_id_reclaim=true)"
                     << dendl;
      global_id_status = global_id_status_t::RECLAIM_INSECURE;
      return 0;
    ...
    std::string* pconnection_secret)
{
  ...
  try {
    ...
    ldout(cct, 0) << __func__
                  << " failed to decode CephXRequestHeader: " << e.what()
                  << dendl;
    return -EPERM;
  }
  ...
  case CEPHX_GET_AUTH_SESSION_KEY: {
    ldout(cct, 10) << "handle_request get_auth_session_key for " << entity_name
                   << dendl;
    ...
    cephx_calc_client_server_challenge(
        cct, eauth.key, server_challenge, req.client_challenge, &expected_key,
        error);
    if ((!error.empty() || req.key != expected_key) &&
      ...
      error.clear();
      cephx_calc_client_server_challenge(
          cct, eauth.pending_key, server_challenge, req.client_challenge,
          &expected_key, error);
      if (error.empty()) {
    ...
    if (!error.empty()) {
      ldout(cct, 0) << " cephx_calc_client_server_challenge error: " << error
                    << dendl;
      ret = -EACCES;
      break;
    }
    ...
    double ttl;
    if (!key_server->get_service_secret(
            CEPH_ENTITY_TYPE_AUTH, info.service_secret, info.secret_id, ttl)) {
      ldout(cct, 0) << " could not get service secret for auth subsystem"
                    << dendl;
      ret = -EIO;
      break;
    }
        ...
        if (pconnection_secret) {
          ...
          if (connection_secret_required_len) {
            cct->random()->get_bytes(
                pconnection_secret->data(), connection_secret_required_len);
          }
          std::string err;
        ...
        for (uint32_t service_id = 1; service_id <= req.other_keys;
          ...
          if ((req.other_keys & service_id) &&
            ...
            key_server->build_session_auth_info(
                service_id, info.ticket, svc_info);
  ...
  case CEPHX_GET_PRINCIPAL_SESSION_KEY: {
    ...
    if (!cephx_verify_authorizer(
            cct, *key_server, indata, 0, auth_ticket_info, nullptr, nullptr,
            &tmp_bl)) {
      ret = -EACCES;
      break;
    }
    ...
    cephx_build_service_ticket_reply(
        cct, auth_ticket_info.session_key, info_vec, false, no_key, *result_bl);
  } break;
  ...
  default:
    ldout(cct, 10) << "handle_request unknown op " << cephx_header.request_type
                   << dendl;
    return -EINVAL;
  }
  return ret;
...
void
CephxServiceHandler::build_cephx_response_header(
    int request_type,
    int status,
    bufferlist& bl)
{
