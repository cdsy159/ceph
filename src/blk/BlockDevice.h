...
private:
  ...
  ceph::mutex stalled_read_event_queue_lock =
      ceph::make_mutex("BlockDevice::stalled_read_event_queue_lock");
  ...
  static BlockDevice* create_with_type(
...
public:
  ...
  virtual uint64_t
  get_conventional_region_size() const
  {
    ceph_assert(is_smr());
  ...
  uint64_t
  get_optimal_io_size() const
  {
    return optimal_io_size;
  }

  bool
  is_discard_supported() const
  {
    return support_discard;
  }

  /// hook to provide utilization of thinly-provisioned device
  virtual int
  get_ebd_state(ExtBlkDevState& state) const
  {
    return -ENOENT;
  }

  virtual int
  get_ebd_id(std::string& id) const
  {
    return -ENOENT;
  }
  ...
  virtual int
  get_devname(std::string* out) const
  {
    return -ENOENT;
  }

  virtual int
  get_devices(std::set<std::string>* ls) const
  {
    std::string s;
    if (get_devname(&s) == 0) {
      ls->insert(s);
  ...
  virtual int read(
      ...
      bool buffered) = 0;
  virtual int
  read_random(uint64_t off, uint64_t len, char* buf, bool buffered) = 0;
  virtual int write(
      uint64_t off,
      ceph::buffer::list& bl,
      bool buffered,
  ...
  virtual void
  swap_discard_queued(interval_set<uint64_t>& other)
  {
    other.clear();
  }
