...
struct data_cache_buf
  : public bi::slist_base_hook<bi::link_mode<bi::normal_link>> {};
...
public:
  ...
  bool
  is_equal(const spdk_nvme_transport_id& trid2) const
  {
  ...
  void
  register_device(NVMEDevice* device)
  {
    registered_devices.push_back(device);
  }

  void
  remove_device(NVMEDevice* device)
  {
...
struct Task {
  ...
  Task(
      ...
      int64_t rc = 0,
      Task* p = nullptr) :
    device(dev), command(c), offset(off), len(l), return_code(rc), primary(p)
  {
    if (primary) {
      primary->ref++;
  ...
  void
  release_segs(SharedDriverQueueData* queue_data)
  {
  ...
  void
  copy_to_buf(char* buf, uint64_t off, uint64_t len)
  {
    uint64_t copied = 0;
    uint64_t left = len;
    void** segs = io_request.extra_segs ? io_request.extra_segs
                                        : io_request.inline_segs;
    uint16_t i = 0;
    while (left > 0) {
...
static void
data_buf_reset_sgl(void* cb_arg, uint32_t sgl_offset)
{
...
static int
data_buf_next_sge(void* cb_arg, void** address, uint32_t* length)
{
  uint32_t size;
  void* addr;
  Task* t = static_cast<Task*>(cb_arg);
...
int
SharedDriverQueueData::alloc_buf_from_pool(Task* t, bool write)
{
...
void
SharedDriverQueueData::_aio_handle(Task* t, IOContext* ioc)
{
...
public:
      ...
      SharedDriverData** driver)
  {
    ...
    if (num_ns > 1) {
      dout(0)
          << __func__ << " namespace count larger than 1, currently only use the first namespace"
          << dendl;
    }
    ...
    struct spdk_nvme_ctrlr_opts* opts)
{
  ...
  if (do_attach) {
    dout(0) << __func__ << " found device at: " << "trtype="
...
static int
find_first_bitset(const string& s)
{
  auto e = s.rend();
    ...
    dpdk_thread = std::thread([this, coremask_arg, m_core_arg, mem_size_arg,
      ...
      while (!stopping) {
        if (!probe_queue.empty()) {
          ...
          r = spdk_nvme_probe(
              local_pci_device ? NULL : &trid, ctxt, probe_cb, attach_cb, NULL);
          if (r < 0) {
...
void
io_complete(void* t, const struct spdk_nvme_cpl* completion)
{
...
bool
NVMEDevice::support(const std::string& path)
{
  char buf[PATH_MAX + 1];
...
int
NVMEDevice::open(const string& p)
{
...
int
NVMEDevice::collect_metadata(const string& prefix, map<string, string>* pm) const
{
...
void
NVMEDevice::aio_submit(IOContext* ioc)
{
  ...
  if (pending && t) {
    ...
    thread_local SharedDriverQueueData queue_t =
        SharedDriverQueueData(this, driver);
    queue_t._aio_handle(t, ioc);
  }
}

static void
ioc_append_task(IOContext* ioc, Task* t)
{
  Task *first, *last;
...
static void
write_split(NVMEDevice* dev, uint64_t off, bufferlist& bl, IOContext* ioc)
{
...
int
NVMEDevice::write(uint64_t off, bufferlist& bl, bool buffered, int write_hint)
{
  uint64_t len = bl.length();
...
int
NVMEDevice::aio_read(uint64_t off, uint64_t len, bufferlist* pbl, IOContext* ioc)
{
...
int
NVMEDevice::read_random(uint64_t off, uint64_t len, char* buf, bool buffered)
{
  ceph_assert(len > 0);
  ceph_assert(off < size);
  ceph_assert(off + len <= size);
...
int
NVMEDevice::invalidate_cache(uint64_t off, uint64_t len)
{
  dout(5) << __func__ << " " << off << "~" << len << dendl;
  return 0;
}
