...
public:
  ...
  explicit MDSCommandOp(ceph_tid_t t, ceph_tid_t multi_id) :
    CommandOp(t, multi_id)
  {}
};

/* error code for ceph_fuse */
#define CEPH_FUSE_NO_MDS_UP \
  -((1 << 16) + 0) /* no mds up deteced in ceph_fuse */
...
struct dir_result_t {
  ...
  struct dentry_off_lt {
    bool
    operator()(const dentry& d, int64_t off) const
    {
  ...
  static uint64_t
  make_fpos(unsigned h, unsigned l, bool hash)
  {
  ...
  static int
  fpos_cmp(uint64_t l, uint64_t r)
  {
...
public:
  ...
  static UserPerm
  pick_my_perms(CephContext* c)
  {
    uid_t uid = c->_conf->client_mount_uid >= 0 ? c->_conf->client_mount_uid
                                                : -1;
    gid_t gid = c->_conf->client_mount_gid >= 0 ? c->_conf->client_mount_gid
                                                : -1;
...
#if defined(__linux__)
  int
  add_fscrypt_key(const char* key_data, int key_len, char* keyid, int user = 0);
...
#endif
  int
  fcopyfile(const char* sname, const char* dname, UserPerm& perms, mode_t mode);
  ...
  int readdir_r_cb(
      ...
      void* p,
      unsigned want = 0,
      unsigned flags = AT_STATX_DONT_SYNC,
      bool getref = false);
  ...
  int _getdents(
      ...
      char* buf,
      int buflen,
      bool ful); // get a bunch of dentries at once

  int
  getdents(dir_result_t* dirp, char* buf, int buflen)
  {
    return _getdents(dirp, buf, buflen, true);
  }

  int
  getdnames(dir_result_t* dirp, char* buf, int buflen)
  {
  ...
  int
  link(const char* oldpath, const char* newpath, const UserPerm& perm)
  {
  ...
  int
  rename(const char* from, const char* to, const UserPerm& perm)
  {
  ...
  // dirs
  int
  mkdir(const char* path, mode_t mode, const UserPerm& perm)
  {
    return mkdirat(CEPHFS_AT_FDCWD, path, mode, perm);
  }

  int
  mkdirat(int dirfd, const char* path, mode_t mode, const UserPerm& perm)
  {
  ...
  int
  symlink(const char* target, const char* linkpath, const UserPerm& perms)
  {
  ...
  int
  chown(const char* path, uid_t new_uid, gid_t new_gid, const UserPerm& perms);
  int fchown(int fd, uid_t new_uid, gid_t new_gid, const UserPerm& perms);
  int
  lchown(const char* path, uid_t new_uid, gid_t new_gid, const UserPerm& perms);
  int chownat(
      int dirfd,
      const char* relpath,
      uid_t new_uid,
  ...
  int
  open(const char* path, int flags, const UserPerm& perms, mode_t mode = 0)
  {
  ...
  {
    return openat(
        CEPHFS_AT_FDCWD, path, flags, perms, mode, stripe_unit, stripe_count,
  ...
  {
    return do_openat(
        dirfd, path, flags, perms, mode, stripe_unit, stripe_count, object_size,
        data_pool);
  }
  ...
  int
  llistxattr(const char* path, char* list, size_t size, const UserPerm& perms);
  ...
  int get_file_stripe_address(
      int fd,
      loff_t offset,
      std::vector<entity_addr_t>& address);
  int
  get_file_extent_osds(int fd, loff_t off, loff_t* len, std::vector<int>& osds);
  ...
  int
  ll_opendir(Inode* in, int flags, dir_result_t** dirpp, const UserPerm& perms);
  ...
  int64_t
  ll_readv(struct Fh* fh, const struct iovec* iov, int iovcnt, int64_t off);
  int64_t
  ll_writev(struct Fh* fh, const struct iovec* iov, int iovcnt, int64_t off);
  int64_t ll_preadv_pwritev(
      struct Fh* fh,
  ...
  void ll_register_callbacks(
      struct ceph_client_callback_args* args); // deprecated
  ...
  int
  cancel_commands_if(
      std::regular_invocable<MDSCommandOp const&> auto&& error_for_op)
  {
  ...
  void adjust_session_flushing_caps(
      Inode* in,
      MetaSession* old_s,
      MetaSession* new_s);
  ...
  void handle_cap_import(
      MetaSession* session,
      Inode* in,
      const MConstRef<MClientCaps>& m);
  void handle_cap_export(
      MetaSession* session,
      Inode* in,
      const MConstRef<MClientCaps>& m);
  void handle_cap_trunc(
      MetaSession* session,
      Inode* in,
      const MConstRef<MClientCaps>& m);
  void handle_cap_flush_ack(
      MetaSession* session,
      Inode* in,
      Cap* cap,
      const MConstRef<MClientCaps>& m);
  void handle_cap_flushsnap_ack(
      MetaSession* session,
      Inode* in,
      const MConstRef<MClientCaps>& m);
  void handle_cap_grant(
      MetaSession* session,
      Inode* in,
      Cap* cap,
      const MConstRef<MClientCaps>& m);
  ...
  void insert_readdir_results(
      MetaRequest* request,
      MetaSession* session,
      Inode* diri,
      Inode* diri_other);
  Inode* insert_trace(MetaRequest* request, MetaSession* session);
  void update_inode_file_size(
      Inode* in,
      int issued,
      uint64_t size,
      uint64_t truncate_seq,
      uint64_t truncate_size);
  void update_inode_file_time(
      Inode* in,
      int issued,
  ...
  Inode* add_update_inode(
      InodeStat* st,
      utime_t ttl,
      MetaSession* session,
      const UserPerm& request_perms);
  Dentry* insert_dentry_inode(
      Dir* dir,
  ...
  void
  update_read_io_size(size_t size)
  {
    total_read_ops++;
  ...
  void
  update_write_io_size(size_t size)
  {
    total_write_ops++;
  ...
  void
  dec_pinned_icaps(uint64_t nr = 1)
  {
    pinned_icaps -= nr;
  }
  ...
  bool
  get_fuse_default_permissions() const
  {
    return fuse_default_permissions;
  }

  bool
  get_fscrypt_as() const
  {
    return fscrypt_as;
  }
...
protected:
  ...
  MetaSessionRef _get_mds_session(
      mds_rank_t mds,
  ...
  int walk(
      ...
      struct walk_dentry_result* result,
      const UserPerm& perms,
      bool followsym = true);
  ...
  void handle_client_request_forward(
      const MConstRef<MClientRequestForward>& reply);
  ...
  int path_walk(
      ...
      std::string trimmed_path = std::string());
  int path_walk(
      InodeRef dirinode,
      const filepath& fp,
      InodeRef* end,
  ...
  void update_snap_trace(
      MetaSession* session,
      const bufferlist& bl,
      SnapRealm** realm_ret,
      bool must_flush = true);
  ...
  void
  signal_context_list(std::vector<Context*>& ls)
  {
  ...
  void
  fill_statx(const InodeRef& in, unsigned int mask, struct ceph_statx* stx)
  {
  ...
  public:
    bool
    is_valid_state(state_t state) const override
    {
      switch (state) {
    ...
    int
    check_reader_state(state_t require) const override
    {
    ...
    int
    check_writer_state(state_t require) const override
    {
      if (require == Client::CLIENT_MOUNTING &&
          state == Client::CLIENT_UNMOUNTED)
        return true;
      else if (
          require == Client::CLIENT_MOUNTED && state == Client::CLIENT_MOUNTING)
        return true;
      else if (
          require == Client::CLIENT_UNMOUNTING &&
          state == Client::CLIENT_MOUNTED)
        return true;
      else if (
          require == Client::CLIENT_UNMOUNTED &&
    ...
    check_writer_state(state_t require) const override
    {
      if (require == Client::CLIENT_INITIALIZING &&
        ...
        return true;
          ...
          require == Client::CLIENT_INITIALIZED &&
          (state == Client::CLIENT_INITIALIZING))
        return true;
      else if (
          require == Client::CLIENT_NEW && (state == Client::CLIENT_INITIALIZED))
        return true;
      else
        return false;
    }
  ...
  int
  get_injected_write_delay_secs() const
  {
...
private:
  ...
  public:
    ...
    void
    get_write_params(int64_t* poffset, uint64_t* psize, bufferlist** ppbl) const
    {
      *poffset = offset;
      *psize = size;
      *ppbl = pbl;
  ...
  public:
    WriteEncMgr_Buffered(
        ...
        bufferlist& bl,
        bool async) :
      WriteEncMgr(clnt, f, offset, size, bl, async)
    {}
  ...
  public:
    ...
    void
    update_write_params(int64_t _ofs, uint64_t _size)
    {
      offset = _ofs;
      size = _size;
    }
  ...
  struct C_nonblocking_fsync_state_advancer : Context {
    ...
    C_nonblocking_fsync_state_advancer(
        Client* clnt,
        Client::C_nonblocking_fsync_state* state) :
  ...
  struct C_nonblocking_fsync_flush_finisher : Context {
    ...
    C_nonblocking_fsync_flush_finisher(
        Client* clnt,
        Client::C_nonblocking_fsync_state* state) :
      clnt(clnt), state(state)
    {}
  ...
  typedef std::function<void(dir_result_t*, MetaRequest*, InodeRef&, frag_t)>
  ...
  int
  _opendir(Inode* in, dir_result_t** dirpp, const UserPerm& perms, int fd = -1);
  ...
  int _readdir_get_frag(
      int op,
      dir_result_t* dirp,
      fill_readdir_args_cb_t fill_req_cb);
  int _readdir_cache_cb(
      dir_result_t* dirp,
      add_dirent_cb_t cb,
      void* p,
      int caps,
  ...
  int
  _read_sync(Fh* f, uint64_t off, uint64_t len, bufferlist* bl, bool* checkeof);
  int _read_async(
      Fh* f,
      uint64_t off,
      uint64_t len,
  ...
  int _setxattr(
      ...
      int flags,
      const UserPerm& perms);
  int _setxattr_check_data_pool(
      std::string& name,
      std::string& value,
      const OSDMap* osdmap);
  void _setxattr_maybe_wait_for_osdmap(
      const char* name,
      const void* value,
      size_t len);
  ...
  int64_t _write(
      ...
      bool do_fsync = false,
      bool syncdataonly = false);
  int64_t _preadv_pwritev_locked(
      Fh* fh,
      const struct iovec* iov,
      int iovcnt,
      int64_t offset,
  ...
  int _vxattrcb_fscrypt_auth_set(
      Inode* in,
      const void* val,
      size_t size,
      const UserPerm& perms);
  ...
  int _vxattrcb_fscrypt_file_set(
      Inode* in,
      const void* val,
      size_t size,
      const UserPerm& perms);
  ...
  bool
  should_check_perms() const
  {
    return (is_fuse && !fuse_default_permissions) ||
           (!is_fuse && client_permissions);
  }
  ...
  std::set<mds_rank_t>
      mds_ranks_closing; // mds ranks currently tearing down sessions
