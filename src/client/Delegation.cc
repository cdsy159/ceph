...
public:
  ...
  finish(int r) override
  {
    ...
    lsubdout(client->cct, client, 0)
        << __func__ << ": delegation return timeout for inode 0x" << std::hex
        << in->ino << ". Forcibly unmounting client. " << client << std::dec
        << dendl;
 ...
 */
int
ceph_deleg_caps_for_type(unsigned type)
{
...
void
Delegation::reinit(unsigned _type, ceph_deleg_cb_t _recall_cb, void* _priv)
{
...
void
Delegation::recall(bool skip_read)
{
