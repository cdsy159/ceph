...
public:
  ...
  Cap(Inode& i, MetaSession* s) :
    inode(i), session(s), gen(s->cap_gen), cap_item(this)
  {
...
struct Inode : RefCountedObject {
  ...
  is_encrypted() const
  {
    return (i_flags & S_ENCRYPTED) == S_ENCRYPTED;
  }

  bool
  has_dir_layout() const
  {
    return layout != file_layout_t();
  }

  __u32
  hash_dentry_name(const std::string& dn)
  {
  ...
  has_any_filelocks()
  {
    return (fcntl_locks && !fcntl_locks->empty()) ||
  ...
  struct Compare {
    bool
    operator()(Inode* const& left, Inode* const& right)
    {
...
private:
  ...
  open_count()
  {
    return (long)std::accumulate(
        open_by_mode.begin(), open_by_mode.end(), 0,
