...
#undef dout_prefix
#define dout_prefix \
  *_dout << "client." << (whoami >= 0 ? whoami : client->get_nodeid()) << " "
...
void
parse_syn_options(vector<const char*>& args)
{
...
void*
synthetic_client_thread_entry(void* ptr)
{
  ...
  for (list<int>::iterator it = modes.begin(); it != modes.end(); ++it) {
    ...
    case SYNCLIENT_MODE_ONLYRANGE: {
      ...
      if (first <= client->get_nodeid() && last > client->get_nodeid()) {
        ...
        dout(2) << "onlyrange [" << first << ", " << last << ") includes me"
                << dendl;
      } else
    ...
    case SYNCLIENT_MODE_MAKEDIRS: {
      ...
      iargs.pop_front();
      if (run_me()) {
        dout(2) << "makedirs " << sarg1 << " " << iarg1 << " " << iarg2 << " "
                << iarg3 << dendl;
    ...
    case SYNCLIENT_MODE_STATDIRS: {
      ...
      iargs.pop_front();
      if (run_me()) {
        dout(2) << "statdirs " << sarg1 << " " << iarg1 << " " << iarg2 << " "
                << iarg3 << dendl;
    ...
    case SYNCLIENT_MODE_READDIRS: {
      ...
      iargs.pop_front();
      if (run_me()) {
        dout(2) << "readdirs " << sarg1 << " " << iarg1 << " " << iarg2 << " "
                << iarg3 << dendl;
    ...
    case SYNCLIENT_MODE_THRASHLINKS: {
      ...
      iargs.pop_front();
      if (run_me()) {
        dout(2) << "thrashlinks " << sarg1 << " " << iarg1 << " " << iarg2
                << " " << iarg3 << dendl;
    ...
    case SYNCLIENT_MODE_TRACE: {
      ...
      char realtfile[100];
      snprintf(
          realtfile, sizeof(realtfile), tfile.c_str(),
          (int)client->get_nodeid().v);

      if (run_me()) {
        dout(0) << "trace " << tfile << " prefix=" << prefix
                << " count=" << iarg1 << " data=" << playdata << dendl;

        Trace t(realtfile);

        if (iarg1 == 0)
          iarg1 = 1; // play trace at least once!
        ...
        for (int i = 0; i < iarg1; i++) {
          ...
          if (time_to_stop())
            break;
          if (iarg1 > 1)
            clean_dir(prefix); // clean only if repeat

          utime_t lat = ceph_clock_now();
          lat -= start;

          dout(0) << " trace " << tfile << " loop " << (i + 1) << "/" << iarg1
                  << " done in " << (double)lat << " seconds" << dendl;
    ...
    case SYNCLIENT_MODE_OPENTEST: {
      ...
      if (run_me()) {
        for (int i = 0; i < count; i++) {
          int fd = client->open(
              "test", (rand() % 2) ? (O_WRONLY | O_CREAT) : O_RDONLY, perms);
          if (fd > 0)
            client->close(fd);
        }
      }
...
int
SyntheticClient::play_trace(Trace& t, string& prefix, bool metadata_only)
{
  ...
  if (prefix.length()) {
    ...
    if (client->ll_lookupx(
            i1, prefix.c_str(), &i2, &stx, CEPH_STATX_INO, 0, perms) == 0) {
  ...
  while (!t.end()) {
    ...
    // low level ops ---------------------
      ...
      if (ll_inos.count(i)) {
        ...
        if (client->ll_lookupx(i1, name, &i2, &stx, CEPH_STATX_INO, 0, perms) ==
            0)
      ...
      int64_t f = t.get_int();
      if (!metadata_only && ll_files.count(f))
    ...
    // object-level traces
      ...
      client->objecter->stat(
          oid, oloc, CEPH_NOSNAP, &size, &mtime, 0,
      ...
      std::unique_lock locker{lock};
      bufferlist bl;
      client->objecter->read(
          oid, oloc, off, len, CEPH_NOSNAP, &bl, 0,
          new C_SafeCond(lock, cond, &ack));
      cond.wait(locker, [&ack] { return ack; });
      ...
      client->objecter->write(
          oid, oloc, off, len, snapc, bl, ceph::real_clock::now(), 0,
          new C_SafeCond(lock, cond, &ack));
      cond.wait(locker, [&ack] { return ack; });
      ...
      client->objecter->zero(
          oid, oloc, off, len, snapc, ceph::real_clock::now(), 0,
          new C_SafeCond(lock, cond, &ack));
      cond.wait(locker, [&ack] { return ack; });
    }
...
int
SyntheticClient::clean_dir(string& basedir)
{
  // read dir
  list<string> contents;
  UserPerm perms = client->pick_my_perms();
  ...
  for (list<string>::iterator it = contents.begin(); it != contents.end();
       ++it) {
    if (*it == ".")
      continue;
    if (*it == "..")
...
int
SyntheticClient::full_walk(string& basedir)
{
  if (time_to_stop())
    return -1;
  ...
  while (!dirq.empty()) {
    ...
    for (list<string>::iterator it = contents.begin(); it != contents.end();
      ...
      tm[strlen(tm) - 1] = 0;
      printf(
          "%llx %c%c%c%c%c%c%c%c%c%c %2d %5d %5d %8llu %12s %s\n",
          (long long)st.st_ino, S_ISDIR(st.st_mode) ? 'd' : '-',
          (st.st_mode & 0400) ? 'r' : '-', (st.st_mode & 0200) ? 'w' : '-',
          (st.st_mode & 0100) ? 'x' : '-', (st.st_mode & 040) ? 'r' : '-',
          (st.st_mode & 020) ? 'w' : '-', (st.st_mode & 010) ? 'x' : '-',
          (st.st_mode & 04) ? 'r' : '-', (st.st_mode & 02) ? 'w' : '-',
          (st.st_mode & 01) ? 'x' : '-', (int)st.st_nlink, (int)st.st_uid,
          (int)st.st_gid, (long long unsigned)st.st_size, tm, file.c_str());
    ...
    if (dir != "" && (actual.nsubdirs != expect.nsubdirs ||
  ...
  for (auto p = nlink.begin(); p != nlink.end(); ++p) {
    if (nlink_seen[p->first] != p->second)
      dout(0) << p->first << " nlink " << p->second
              << " != " << nlink_seen[p->first] << "seen" << dendl;
  }
...
int
SyntheticClient::dump_placement(string& fn)
{
  ...
  for (const auto& x : extents) {
    ...
    for (const auto& be : x.buffer_extents)
      dout(0) << "OSD " << osd << ", offset " << be.first << ", length "
              << be.second << dendl;
  }
  return 0;
}
...
int
SyntheticClient::make_dirs(const char* basedir, int dirs, int files, int depth)
{
  if (time_to_stop())
    return 0;
  ...
  // children
  char d[500];
  dout(3) << "make_dirs " << basedir << " dirs " << dirs << " files " << files
          << " depth " << depth << dendl;
...
int
SyntheticClient::stat_dirs(const char* basedir, int dirs, int files, int depth)
{
  if (time_to_stop())
    return 0;
  ...
  // children
  char d[500];
  dout(3) << "stat_dirs " << basedir << " dirs " << dirs << " files " << files
          << " depth " << depth << dendl;
  for (int i = 0; i < files; i++) {
    snprintf(d, sizeof(d), "%s/file.%d", basedir, i);
...
int
SyntheticClient::read_dirs(const char* basedir, int dirs, int files, int depth)
{
  if (time_to_stop())
    return 0;
  ...
  // children
  char d[500];
  dout(3) << "read_dirs " << basedir << " dirs " << dirs << " files " << files
          << " depth " << depth << dendl;
  ...
  if (depth > 0)
    for (int i = 0; i < dirs; i++) {
      snprintf(d, sizeof(d), "%s/dir.%d", basedir, i);
      if (read_dirs(d, dirs, files, depth - 1) < 0)
        return -1;
    }
...
int
SyntheticClient::make_files(int num, int count, int priv, bool more)
{
  ...
  for (int c = 0; c < count; c++) {
    for (int n = 0; n < num; n++) {
      snprintf(
          d, sizeof(d), "dir.%d.run%d/file.client%d.%d", priv ? whoami : 0, c,
          whoami, n);
  ...
  dout(0) << "makefiles time is " << end << " or "
          << ((double)end / (double)num) << " per file" << dendl;

  return 0;
}

int
SyntheticClient::link_test()
{
  char d[255];
  char e[255];
...
int
SyntheticClient::create_shared(int num)
{
  // files
  UserPerm perms = client->pick_my_perms();
  char d[255];
...
int
SyntheticClient::open_shared(int num, int count)
{
  // files
  char d[255];
  UserPerm perms = client->pick_my_perms();
...
int
SyntheticClient::overload_osd_0(int n, int size, int wrsize)
{
  UserPerm perms = client->pick_my_perms();
...
int
SyntheticClient::check_first_primary(int fh)
{
  vector<ObjectExtent> extents;
...
int
SyntheticClient::rm_file(string& fn)
{
  UserPerm perms = client->pick_my_perms();
...
int
SyntheticClient::write_file(
    string& fn,
    int size,
    loff_t wrsize) // size is in MB, wrsize in bytes
{
  ...
  dout(0) << "write total " << (total / el / 1048576.0) << " MB/sec (" << total
          << " bytes in " << el << " seconds)" << dendl;
...
int
SyntheticClient::write_fd(
    int fd,
    int size,
    int wrsize) // size is in MB, wrsize in bytes
{
  //uint64_t wrsize = 1024*256;
  char* buf = new char[wrsize + 100]; // 1 MB
  memset(buf, 7, wrsize);
...
int
SyntheticClient::write_batch(int nfile, int size, int wrsize)
{
    ...
    bool ignoreprint)
{
  ...
  for (unsigned i = 0; i < chunks; i++) {
    ...
    while ((char*)p + 32 < buf + rdsize) {
      uint64_t readoff = *p;
      uint64_t wantoff = (uint64_t)i * (uint64_t)rdsize +
                         (uint64_t)((char*)p - buf);
      p++;
      ...
      if (readoff != wantoff || readclient != client->get_nodeid()) {
        if (!bad && !ignoreprint)
          dout(0) << "WARNING: wrong data from OSD, block says fileoffset="
                  << readoff << " client=" << readclient
                  << ", should be offset " << wantoff << " client "
                  << client->get_nodeid() << dendl;
        bad++;
      }
    }
    if (bad && !ignoreprint)
      dout(0) << " + " << (bad - 1) << " other bad 16-byte bits in this block"
              << dendl;
  }

  utime_t stop = ceph_clock_now();
  double el = stop - start;
  dout(0) << "read total " << (total / el / 1048576.0) << " MB/sec (" << total
          << " bytes in " << el << " seconds)" << dendl;
...
int
SyntheticClient::create_objects(int nobj, int osize, int inflight)
{
  // divy up
  ...
  dout(5) << "create_objects " << nobj << " size=" << osize << " .. doing ["
          << start << "," << end << ") inc " << inc << dendl;
  ...
  for (int i = start; i < end; i += inc) {
    ...
    starts.push_back(ceph_clock_now());
    {
      ...
      client->objecter->write(
          oid, oloc, 0, osize, snapc, bl, ceph::real_clock::now(), 0,
...
SyntheticClient::object_rw(
    ...
    double wskew)
{
  dout(5) << "object_rw " << nobj << " size=" << osize << " with " << wrpc
          << "% writes" << ", " << overlappc << "% overlap"
  ...
  while (1) {
    ...
    if (write) {
      o = (long)trunc(
          pow(r, wskew) * (double)nobj); // exponentially skew towards 0
      int pnoremap = (long)(r * 100.0);
      if (pnoremap >= overlappc)
        o = (o * prime) % nobj; // remap
    } else {
      o = (long)trunc(
          pow(r, rskew) * (double)nobj); // exponentially skew towards 0
    }
    ...
    if (write) {
      ...
      op.indata = bl;
      m.ops.push_back(op);
      client->objecter->mutate(
          oid, oloc, m, snapc, ceph::real_clock::now(), 0,
          new C_Ref(lock, cond, &unack));
    } else {
      dout(10) << "read from " << oid << dendl;
      bufferlist inbl;
      client->objecter->read(
          oid, oloc, 0, osize, CEPH_NOSNAP, &inbl, 0,
          new C_Ref(lock, cond, &unack));
    }
...
int
SyntheticClient::read_random(
    string& fn,
    int size,
    int rdsize) // size is in MB, wrsize in bytes
{
  UserPerm perms = client->pick_my_perms();
  uint64_t chunks = (uint64_t)size * (uint64_t)(1024 * 1024) / (uint64_t)rdsize;
  ...
  for (unsigned i = 0; i < 2000; i++) {
    ...
    if (read) {
      ...
      if (r < rdsize) {
        dout(1) << "read_file got r = " << r << ", probably end of file"
                << dendl;
      }
    } else {
...
int
normdist(int min, int max, int stdev) /* specifies input values */
{
...
int
SyntheticClient::read_random_ex(
    string& fn,
    int size,
    int rdsize) // size is in MB, wrsize in bytes
{
...
int
SyntheticClient::random_walk(int num_req)
{
  int left = num_req;
  ...
  while (left > 0) {
	...
	*/
    ...
    if (r < 0) {
      ...
      dout(DBL) << "r = " << r << ", client may not have " << cwd << ", cd .."
                << dendl;
      up();
      //}
    }
...
void
SyntheticClient::make_dir_mess(const char* basedir, int n)
{
  UserPerm perms = client->pick_my_perms();
  vector<string> dirs;
...
SyntheticClient::foo()
{
  ...
  if (1) {
    ...
    handle = client->open(
        fn, O_RDWR,
        perms); // open the same  file, it must have some data already
    ceph_assert(handle >= 0);
...
SyntheticClient::thrash_links(
    ...
    int n)
{
  dout(1) << "thrash_links " << basedir << " " << dirs << " " << files << " "
          << depth << " links " << n << dendl;
...
void
SyntheticClient::import_find(const char* base, const char* find, bool data)
{
  dout(1) << "import_find " << base << " from " << find << " data=" << data
          << dendl;
  ...
  while (!f.eof()) {
    ...
    if (S_ISLNK(mode)) {
      ...
      dout(10) << "symlink from '" << link << "' -> '" << target << "'"
               << dendl;
    ...
    const UserPerm& perms)
{
  ...
  dout(0) << "lookup_hash(" << ino << ", #" << dirino << "/" << name
          << ") = " << r << dendl;
  return r;
}

int
SyntheticClient::lookup_ino(inodeno_t ino, const UserPerm& perms)
{
...
int
SyntheticClient::chunk_file(string& filename)
{
  UserPerm perms = client->pick_my_perms();
  ...
  while (pos < size) {
    ...
    bufferlist bl;
    {
      ...
      client->filer->read(
          inode.ino, &inode.layout, CEPH_NOSNAP, pos, get, &bl, 0, onfinish);
...
void
SyntheticClient::mksnap(const char* base, const char* name, const UserPerm& perms)
{
  client->mksnap(base, name, perms);
}

void
SyntheticClient::rmsnap(const char* base, const char* name, const UserPerm& perms)
{
  client->rmsnap(base, name, perms);
}

void
SyntheticClient::mksnapfile(const char* dir)
{
  UserPerm perms = client->pick_my_perms();
  client->mkdir(dir, 0755, perms);
