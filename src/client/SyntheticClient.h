...
class SyntheticClient {
  ...
  get_random_subdir()
  {
    ...
    int r = ((rand() % subdirs.size()) + (rand() % subdirs.size())) /
            2; // non-uniform distn
  ...
  get_random_sub()
  {
    ...
    int r = ((rand() % contents.size()) + (rand() % contents.size())) /
            2; // non-uniform distn
...
public:
  ...
  time_to_stop()
  {
    ...
    if (0)
      std::cout << "time_to_stop .. now " << now << " until " << run_until
                << " start " << run_start << std::endl;
  ...
  std::string
  compose_path(std::string& prefix, char* rest)
  {
    return prefix + rest;
  }
  ...
  int
  read_file(const std::string& fn, int mb, int chunk, bool ignoreprint = false);
