...
#define cldout(cl, v) \
  dout_impl((cl)->cct, dout_subsys, v) * _dout << "client." << cl->whoami << " "
...
void
C_Block_Sync::finish(int r)
{
  cldout(cl, 1) << "C_Block_Sync::finish() for " << ino << " " << iv
                << " r==" << r << dendl;
  if (rval)
    *rval = r;
...
void
BarrierContext::write_nobarrier(C_Block_Sync& cbs)
{
...
void
BarrierContext::write_barrier(C_Block_Sync& cbs)
{
  ...
  { /* find blocking commit--intrusive no help here */
    ...
    bool done = false;
    for (iter = active_commits.begin(); !done && (iter != active_commits.end());
         ++iter) {
...
void
BarrierContext::commit_barrier(barrier_interval& civ)
{
  std::unique_lock locker(lock);
...
void
BarrierContext::complete(C_Block_Sync& cbs)
{
  std::lock_guard locker(lock);
