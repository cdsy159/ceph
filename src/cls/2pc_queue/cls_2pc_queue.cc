...
static uint64_t
calc_reservations_size(const cls_2pc_reservations& reservations)
{
  uint64_t total = 0;
...
static int
cls_2pc_queue_init(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  ...
  try {
    ...
    CLS_LOG(
        1, "ERROR: cls_2pc_queue_init: failed to decode entry: %s", err.what());
    return -EINVAL;
  }
  ...
  CLS_LOG(
      20, "INFO: cls_2pc_queue_init: max size is %lu (bytes)", op.queue_size);

  init_op.queue_size = op.queue_size;
  init_op.max_urgent_data_size =
      23552; // overall head is 24KB ~ pending 1K reservations ops
    ...
    bufferlist* out)
{
  ...
  const auto remaining_size = (head.tail.offset >= head.front.offset)
                                  ? (head.queue_size - head.tail.offset) +
  ...
  try {
    ...
    CLS_LOG(
        1,
        "ERROR: cls_2pc_queue_get_topic_stats: failed to decode header of "
        "queue: %s",
        err.what());
    return -EINVAL;
  }
...
static int
cls_2pc_queue_reserve(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  ...
  try {
    ...
    CLS_LOG(
        1, "ERROR: cls_2pc_queue_reserve: failed to decode entry: %s",
        err.what());
    return -EINVAL;
  }
  ...
  if (urgent_data.decoded_struct_v < 3) {
    urgent_data.reserved_size = calc_reservations_size(urgent_data.reservations);
    ...
    if (urgent_data.has_xattrs) {
      ...
      if (ret < 0 && (ret != -ENOENT && ret != -ENODATA)) {
        CLS_LOG(
            1, "ERROR: cls_2pc_queue_reserve: failed to read xattrs with: %d",
            ret);
        return ret;
      }
      if (ret >= 0) {
    ...
    CLS_LOG(
        1,
        "INFO: cls_2pc_queue_reserve: re-calculated urgent_data.reserved_size, "
        "reserved_size=%lu",
  ...
  if (res_op.size + urgent_data.reserved_size + overhead > remaining_size) {
    CLS_LOG(
        1,
        "ERROR: cls_2pc_queue_reserve: reservations exceeded maximum capacity");
    CLS_LOG(
        10, "INFO: cls_2pc_queue_reserve: remaining size: %lu (bytes)",
        remaining_size);
    CLS_LOG(
        10, "INFO: cls_2pc_queue_reserve: current reservations: %lu (bytes)",
        urgent_data.reserved_size);
    CLS_LOG(
        10, "INFO: cls_2pc_queue_reserve: requested size: %lu (bytes)",
        res_op.size);
    return -ENOSPC;
  }
  ...
  std::tie(last_reservation, result) = urgent_data.reservations.emplace(
      std::piecewise_construct, std::forward_as_tuple(urgent_data.last_id),
      std::forward_as_tuple(
          res_op.size, ceph::coarse_real_clock::now(), res_op.entries));
  if (!result) {
    // an old reservation that was never committed or aborted is in the map
    // caller should try again assuming other IDs are ok
    CLS_LOG(
        1,
        "ERROR: cls_2pc_queue_reserve: reservation id conflict after rollover: "
        "%u",
        urgent_data.last_id);
    return -EAGAIN;
  }
  ...
  if (head.max_urgent_data_size < urgent_data_length) {
    CLS_LOG(
        10,
        "INFO: cls_2pc_queue_reserve: urgent data size: %lu exceeded maximum: "
        "%lu using xattrs",
    ...
    if (ret >= 0) {
      ...
      try {
        ...
        CLS_LOG(
            1,
            "ERROR: cls_2pc_queue_reserve: failed to decode xattrs urgent data "
            "map");
        return -EINVAL;
      } //end - catch
    }
    std::tie(std::ignore, result) = xattr_reservations.emplace(
        std::piecewise_construct, std::forward_as_tuple(urgent_data.last_id),
        std::forward_as_tuple(
            res_op.size, ceph::coarse_real_clock::now(), res_op.entries));
    if (!result) {
      ...
      CLS_LOG(
          1,
          "ERROR: cls_2pc_queue_reserve: reservation id conflict inside xattrs "
          "after rollover: %u",
          urgent_data.last_id);
      return -EAGAIN;
    }
    ...
    if (ret < 0) {
      CLS_LOG(
          1, "ERROR: cls_2pc_queue_reserve: failed to write xattrs with: %d",
          ret);
      return ret;
    }
  ...
  CLS_LOG(
      20, "INFO: cls_2pc_queue_reserve: remaining size: %lu (bytes)",
      remaining_size);
  CLS_LOG(
      20, "INFO: cls_2pc_queue_reserve: current reservations: %lu (bytes)",
      urgent_data.reserved_size);
  CLS_LOG(
      20, "INFO: cls_2pc_queue_reserve: requested size: %lu (bytes)",
      res_op.size);
  CLS_LOG(
      20, "INFO: cls_2pc_queue_reserve: urgent data size: %lu (bytes)",
      urgent_data_length);
...
static int
cls_2pc_queue_commit(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  ...
  try {
    ...
    CLS_LOG(
        1, "ERROR: cls_2pc_queue_commit: failed to decode entry: %s",
        err.what());
    return -EINVAL;
  }
  ...
  if (it == urgent_data.reservations.end()) {
    if (!urgent_data.has_xattrs) {
      CLS_LOG(
          1, "ERROR: cls_2pc_queue_commit: reservation does not exist: %u",
          commit_op.id);
      return -ENOENT;
    }
    ...
    if (ret < 0) {
      ...
      CLS_LOG(
          1, "ERROR: cls_2pc_queue_commit: failed to read xattrs with: %d", ret);
      return ret;
    }
    auto iter = bl_xattrs.cbegin();
    try {
      ...
      CLS_LOG(
          1,
          "ERROR: cls_2pc_queue_commit: failed to decode xattrs urgent data "
          "map");
      return -EINVAL;
    } //end - catch
  ...
  const auto actual_size = std::accumulate(
      commit_op.bl_data_vec.begin(), commit_op.bl_data_vec.end(), 0UL,
      [](uint64_t sum, const bufferlist& bl) { return sum + bl.length(); });

  if (res.size < actual_size) {
    CLS_LOG(
        1,
        "ERROR: cls_2pc_queue_commit: trying to commit %lu bytes to a %lu "
        "bytes reservation",
  ...
  if (xattr_reservations.empty()) {
    ...
    if (ret < 0) {
      CLS_LOG(
          1, "ERROR: cls_2pc_queue_commit: failed to write xattrs with: %d",
          ret);
      return ret;
    }
  }
  ...
  CLS_LOG(
      20, "INFO: cls_2pc_queue_commit: current reservations: %lu (bytes)",
      urgent_data.reserved_size);
  CLS_LOG(
      20, "INFO: cls_2pc_queue_commit: current reservation entries: %lu",
...
static int
cls_2pc_queue_abort(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  ...
  try {
    ...
    CLS_LOG(
        1, "ERROR: cls_2pc_queue_abort: failed to decode entry: %s", err.what());
    return -EINVAL;
  }
  ...
  if (it == urgent_data.reservations.end()) {
    if (!urgent_data.has_xattrs) {
      CLS_LOG(
          20, "INFO: cls_2pc_queue_abort: reservation does not exist: %u",
          abort_op.id);
      return 0;
    }
    // try to look for the reservation in xattrs
    ...
    if (ret < 0) {
      ...
      CLS_LOG(
          1, "ERROR: cls_2pc_queue_abort: failed to read xattrs with: %d", ret);
      return ret;
    }
    auto iter = bl_xattrs.cbegin();
    cls_2pc_reservations xattr_reservations;
    try {
      ...
      CLS_LOG(
          1,
          "ERROR: cls_2pc_queue_abort: failed to decode xattrs urgent data "
          "map");
      return -EINVAL;
    } //end - catch
    ...
    const auto reservation_overhead = it->second.entries * QUEUE_ENTRY_OVERHEAD;
    ...
    if (ret < 0) {
      CLS_LOG(
          1, "ERROR: cls_2pc_queue_abort: failed to write xattrs with: %d", ret);
      return ret;
    }
  } else {
    const auto reservation_overhead = it->second.entries * QUEUE_ENTRY_OVERHEAD;
  ...
  CLS_LOG(
      20, "INFO: cls_2pc_queue_abort: current reservations: %lu (bytes)",
      urgent_data.reserved_size);
    ...
    bufferlist* out)
{
  ...
  try {
    ...
    CLS_LOG(
        1, "ERROR: cls_2pc_queue_list_reservations: failed to decode entry: %s",
        err.what());
    return -EINVAL;
  }
  ...
  CLS_LOG(
      20,
      "INFO: cls_2pc_queue_list_reservations: %lu reservation entries found",
      urgent_data.reservations.size());
  ...
  if (urgent_data.has_xattrs) {
    ...
    if (ret < 0 && (ret != -ENOENT && ret != -ENODATA)) {
      CLS_LOG(
          1,
          "ERROR: cls_2pc_queue_list_reservations: failed to read xattrs with: "
          "%d",
          ret);
      return ret;
    }
    if (ret >= 0) {
      ...
      try {
        ...
        CLS_LOG(
            1,
            "ERROR: cls_2pc_queue_list_reservations: failed to decode xattrs "
            "urgent data map");
        return -EINVAL;
      } //end - catch
      CLS_LOG(
          20,
          "INFO: cls_2pc_queue_list_reservations: %lu reservation entries "
          "found in xatts",
    ...
    bufferlist* out)
{
  ...
  CLS_LOG(
      20,
      "INFO: cls_2pc_queue_expire_reservations: %lu reservation entries found",
      urgent_data.reservations.size());
  CLS_LOG(
      20,
      "INFO: cls_2pc_queue_expire_reservations: current reservations: %lu "
      "(bytes)",
      urgent_data.reserved_size);
  ...
  for (auto it = urgent_data.reservations.begin();
       it != urgent_data.reservations.end();) {
    if (it->second.timestamp < expire_op.stale_time) {
      CLS_LOG(
          5,
          "WARNING: cls_2pc_queue_expire_reservations: stale reservation %u "
          "will be removed",
          it->first);
  ...
  if (urgent_data.has_xattrs) {
    ...
    if (ret < 0 && (ret != -ENOENT && ret != -ENODATA)) {
      CLS_LOG(
          1,
          "ERROR: cls_2pc_queue_expire_reservations: failed to read xattrs "
          "with: %d",
          ret);
      return ret;
    }
    if (ret >= 0) {
      ...
      try {
        ...
        CLS_LOG(
            1,
            "ERROR: cls_2pc_queue_expire_reservations: failed to decode xattrs "
            "urgent data map");
        return -EINVAL;
      } //end - catch
      CLS_LOG(
          20,
          "INFO: cls_2pc_queue_expire_reservations: %lu reservation entries "
          "found in xatts",
          xattr_reservations.size());
      for (auto it = xattr_reservations.begin();
           it != xattr_reservations.end();) {
        if (it->second.timestamp < expire_op.stale_time) {
          CLS_LOG(
              5,
              "WARNING: cls_2pc_queue_expire_reservations: stale reservation "
              "%u will be removed",
              it->first);
          const auto reservation_overhead = it->second.entries *
                                            QUEUE_ENTRY_OVERHEAD;
      ...
      if (xattr_stale_found) {
        ...
        bl_xattrs.clear();
        encode(xattr_reservations, bl_xattrs);
        ret = cls_cxx_setxattr(
            hctx, CLS_QUEUE_URGENT_DATA_XATTR_NAME, &bl_xattrs);
        if (ret < 0) {
          CLS_LOG(
              1,
              "ERROR: cls_2pc_queue_expire_reservations: failed to write "
              "xattrs with: %d",
              ret);
          return ret;
        }
  ...
  if (stale_found || xattr_stale_found) {
    ...
    CLS_LOG(
        20,
        "INFO: cls_2pc_queue_expire_reservations: reservations after cleanup: "
        "%lu (bytes)",
        urgent_data.reserved_size);
    ...
    bufferlist* out)
{
  ...
  try {
    ...
    CLS_LOG(
        1, "ERROR: cls_2pc_queue_list_entries: failed to decode entry: %s",
        err.what());
    return -EINVAL;
  }
    ...
    bufferlist* out)
{
  ...
  try {
    ...
    CLS_LOG(
        1, "ERROR: cls_2pc_queue_remove_entries: failed to decode entry: %s",
        err.what());
    return -EINVAL;
  }
  ...
  if (rem_2pc_op.entries_to_remove == 0) {
    CLS_LOG(
        10,
        "INFO: cls_2pc_queue_remove_entries: incompatible RGW with rados, "
        "counting entries to remove...");
    cls_queue_list_op list_op;
    list_op.max = std::numeric_limits<uint64_t>::
        max(); // max length because endmarker is the stopping condition.
    list_op.end_marker = rem_2pc_op.end_marker;
    ret = cls_2pc_queue_count_entries(
        hctx, list_op, head, rem_2pc_op.entries_to_remove);
    if (ret < 0) {
      CLS_LOG(1, "ERROR: cls_2pc_queue_remove_entries: returned: %d", ret);
      return ret;
    }
    CLS_LOG(
        10, "INFO: cls_2pc_queue_remove_entries: counted: %u",
        rem_2pc_op.entries_to_remove);
  }
  ...
  try {
    ...
    CLS_LOG(
        1,
        "ERROR: cls_2pc_queue_remove_entries: failed to decode header of "
        "queue: %s",
        err.what());
    return -EINVAL;
  }
...
CLS_INIT(2pc_queue)
{
  ...
  cls_register_cxx_method(
      h_class, TPC_QUEUE_INIT, CLS_METHOD_RD | CLS_METHOD_WR,
      cls_2pc_queue_init, &h_2pc_queue_init);
  cls_register_cxx_method(
      h_class, TPC_QUEUE_GET_CAPACITY, CLS_METHOD_RD,
      cls_2pc_queue_get_capacity, &h_2pc_queue_get_capacity);
  cls_register_cxx_method(
      h_class, TPC_QUEUE_GET_TOPIC_STATS, CLS_METHOD_RD,
      cls_2pc_queue_get_topic_stats, &h_2pc_queue_get_topic_stats);
  cls_register_cxx_method(
      h_class, TPC_QUEUE_RESERVE, CLS_METHOD_RD | CLS_METHOD_WR,
      cls_2pc_queue_reserve, &h_2pc_queue_reserve);
  cls_register_cxx_method(
      h_class, TPC_QUEUE_COMMIT, CLS_METHOD_RD | CLS_METHOD_WR,
      cls_2pc_queue_commit, &h_2pc_queue_commit);
  cls_register_cxx_method(
      h_class, TPC_QUEUE_ABORT, CLS_METHOD_RD | CLS_METHOD_WR,
      cls_2pc_queue_abort, &h_2pc_queue_abort);
  cls_register_cxx_method(
      h_class, TPC_QUEUE_LIST_RESERVATIONS, CLS_METHOD_RD,
      cls_2pc_queue_list_reservations, &h_2pc_queue_list_reservations);
  cls_register_cxx_method(
      h_class, TPC_QUEUE_LIST_ENTRIES, CLS_METHOD_RD,
      cls_2pc_queue_list_entries, &h_2pc_queue_list_entries);
  cls_register_cxx_method(
      h_class, TPC_QUEUE_REMOVE_ENTRIES, CLS_METHOD_RD | CLS_METHOD_WR,
      cls_2pc_queue_remove_entries, &h_2pc_queue_remove_entries);
  cls_register_cxx_method(
      h_class, TPC_QUEUE_EXPIRE_RESERVATIONS, CLS_METHOD_RD | CLS_METHOD_WR,
