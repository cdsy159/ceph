...
int cls_2pc_queue_get_capacity(
    librados::IoCtx& io_ctx,
    const std::string& queue_name,
    uint64_t& size);
// return the number of committed entries and size (bytes)
int cls_2pc_queue_get_topic_stats(
    librados::IoCtx& io_ctx,
    const std::string& queue_name,
...
int cls_2pc_queue_list_reservations(
    librados::IoCtx& io_ctx,
    const std::string& queue_name,
...
void cls_2pc_queue_get_capacity(
...
void cls_2pc_queue_get_topic_stats(
    librados::ObjectReadOperation& op,
    bufferlist* obl,
    int* prval);
...
void cls_2pc_queue_list_entries(
    librados::ObjectReadOperation& op,
    const std::string& marker,
    uint32_t max,
    bufferlist* obl,
...
int cls_2pc_queue_list_entries_result(
    const bufferlist& bl,
    std::vector<cls_queue_entry>& entries,
    bool* truncated,
    std::string& next_marker);
...
void cls_2pc_queue_list_reservations(
    librados::ObjectReadOperation& op,
    bufferlist* obl,
    int* prval);

int cls_2pc_queue_list_reservations_result(
    const librados::bufferlist& bl,
    cls_2pc_reservations& reservations);

// expire stale reservations (older than the given time)
void cls_2pc_queue_expire_reservations(
    librados::ObjectWriteOperation& op,
...
void cls_2pc_queue_remove_entries(
    librados::ObjectWriteOperation& op,
