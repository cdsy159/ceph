...
std::string
key_from_client_id(const std::string& client_id)
{
...
uint64_t
tag_tid_from_key(const std::string& key)
{
...
int
expire_tags(cls_method_context_t hctx, const std::string* skip_client_id)
{
  ...
  do {
    ...
    int r = cls_cxx_map_get_vals(
        hctx, last_read, HEADER_KEY_CLIENT_PREFIX, MAX_KEYS_READ, &vals, &more);
    if (r < 0 && r != -ENOENT) {
      CLS_ERR(
          "failed to retrieve registered clients: %s", cpp_strerror(r).c_str());
      return r;
    }
  ...
  do {
    std::map<std::string, bufferlist> vals;
    int r = cls_cxx_map_get_vals(
        hctx, last_read, HEADER_KEY_TAG_PREFIX, MAX_KEYS_READ, &vals, &more);
    if (r < 0 && r != -ENOENT) {
    ...
    uint64_t max_return)
{
  ...
  bool more;
  int r = cls_cxx_map_get_vals(
      hctx, last_read, HEADER_KEY_CLIENT_PREFIX, max_return, &vals, &more);
  if (r < 0) {
    ...
    cls::journal::ObjectSetPosition* minset)
{
  ...
  while (true) {
    ...
    r = get_client_list_range(
        hctx, &batch, start_after, cls::journal::JOURNAL_MAX_RETURN);
    if ((r < 0) || batch.empty()) {
      break;
    }
    ...
    for (const auto& client : batch) {
      ...
      if (!valid || (tag_tid > first.tag_tid) ||
          ((tag_tid == first.tag_tid) && (entry_tid > first.entry_tid))) {
 ...
 */
int
journal_create(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  uint8_t order;
  uint8_t splay_width;
  int64_t pool_id;
 ...
 */
int
journal_get_order(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  uint8_t order;
 ...
 */
int
journal_get_pool_id(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  int64_t pool_id = 0;
    ...
    bufferlist* out)
{
  ...
  if (current_active_set < object_set) {
    CLS_LOG(
        10, "active object set earlier than minimum: %" PRIu64 " < %" PRIu64,
        current_active_set, object_set);
    return -EINVAL;
  }
  ...
  if (object_set == current_minimum_set) {
    ...
    CLS_ERR(
        "object number earlier than current object: %" PRIu64 " < %" PRIu64,
 ...
 */
int
journal_get_active_set(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  uint64_t active_set;
 ...
 */
int
journal_set_active_set(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  uint64_t object_set;
  try {
    auto iter = in->cbegin();
  ...
  if (current_minimum_set > object_set) {
    CLS_ERR(
        "minimum object set later than active: %" PRIu64 " > %" PRIu64,
        current_minimum_set, object_set);
    return -EINVAL;
  }
 ...
 */
int
journal_get_client(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  std::string id;
  try {
    auto iter = in->cbegin();
 ...
 */
int
journal_client_commit(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  std::string id;
 ...
 */
int
journal_client_list(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  std::string start_after;
  uint64_t max_return;
  try {
 ...
 */
int
journal_get_tag(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  uint64_t tag_tid;
  try {
    auto iter = in->cbegin();
 ...
 */
int
journal_tag_create(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  uint64_t tag_tid;
  uint64_t tag_class;
  bufferlist data;
 ...
 */
int
journal_tag_list(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  ...
  int tag_pass =
      (minimum_tag_tid == std::numeric_limits<uint64_t>::max()
           ? TAG_PASS_LIST
           : TAG_PASS_CALCULATE_MINIMUMS);
    ...
    bufferlist* out)
{
  ...
  if (size >= soft_max_size) {
    CLS_LOG(
        5, "journal object full: %" PRIu64 " >= %" PRIu64, size, soft_max_size);
    return -EOVERFLOW;
  }
  return 0;
}
 ...
 */
int
journal_object_append(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  uint64_t soft_max_size;
  bufferlist data;
  try {
  ...
  auto offset = size;
  r = cls_cxx_write2(
      hctx, offset, data.length(), &data, CEPH_OSD_OP_FLAG_FADVISE_DONTNEED);
  if (r < 0) {
...
CLS_INIT(journal)
{
  ...
  cls_register_cxx_method(
      h_class, "create", CLS_METHOD_RD | CLS_METHOD_WR, journal_create,
      &h_journal_create);
  cls_register_cxx_method(
      h_class, "get_order", CLS_METHOD_RD, journal_get_order,
      &h_journal_get_order);
  cls_register_cxx_method(
      h_class, "get_splay_width", CLS_METHOD_RD, journal_get_splay_width,
      &h_journal_get_splay_width);
  cls_register_cxx_method(
      h_class, "get_pool_id", CLS_METHOD_RD, journal_get_pool_id,
      &h_journal_get_pool_id);
  cls_register_cxx_method(
      h_class, "get_minimum_set", CLS_METHOD_RD, journal_get_minimum_set,
      &h_journal_get_minimum_set);
  cls_register_cxx_method(
      h_class, "set_minimum_set", CLS_METHOD_RD | CLS_METHOD_WR,
      journal_set_minimum_set, &h_journal_set_minimum_set);
  cls_register_cxx_method(
      h_class, "get_active_set", CLS_METHOD_RD, journal_get_active_set,
      &h_journal_get_active_set);
  cls_register_cxx_method(
      h_class, "set_active_set", CLS_METHOD_RD | CLS_METHOD_WR,
      journal_set_active_set, &h_journal_set_active_set);

  cls_register_cxx_method(
      h_class, "get_client", CLS_METHOD_RD, journal_get_client,
      &h_journal_get_client);
  cls_register_cxx_method(
      h_class, "client_register", CLS_METHOD_RD | CLS_METHOD_WR,
      journal_client_register, &h_journal_client_register);
  cls_register_cxx_method(
      h_class, "client_update_data", CLS_METHOD_RD | CLS_METHOD_WR,
      journal_client_update_data, &h_journal_client_update_data);
  cls_register_cxx_method(
      h_class, "client_update_state", CLS_METHOD_RD | CLS_METHOD_WR,
      journal_client_update_state, &h_journal_client_update_state);
  cls_register_cxx_method(
      h_class, "client_unregister", CLS_METHOD_RD | CLS_METHOD_WR,
      journal_client_unregister, &h_journal_client_unregister);
  cls_register_cxx_method(
      h_class, "client_commit", CLS_METHOD_RD | CLS_METHOD_WR,
      journal_client_commit, &h_journal_client_commit);
  cls_register_cxx_method(
      h_class, "client_list", CLS_METHOD_RD, journal_client_list,
      &h_journal_client_list);

  cls_register_cxx_method(
      h_class, "get_next_tag_tid", CLS_METHOD_RD, journal_get_next_tag_tid,
      &h_journal_get_next_tag_tid);
  cls_register_cxx_method(
      h_class, "get_tag", CLS_METHOD_RD, journal_get_tag, &h_journal_get_tag);
  cls_register_cxx_method(
      h_class, "tag_create", CLS_METHOD_RD | CLS_METHOD_WR, journal_tag_create,
      &h_journal_tag_create);
  cls_register_cxx_method(
      h_class, "tag_list", CLS_METHOD_RD, journal_tag_list, &h_journal_tag_list);

  /// methods for journal_data.$journal_id.$object_id objects
  cls_register_cxx_method(
      h_class, "guard_append", CLS_METHOD_RD | CLS_METHOD_WR,
      journal_object_guard_append, &h_journal_object_guard_append);
  cls_register_cxx_method(
      h_class, "append", CLS_METHOD_RD | CLS_METHOD_WR, journal_object_append,
