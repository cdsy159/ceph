...
struct C_AioExec : public Context {
  ...
  static void
  rados_callback(rados_completion_t c, void* arg)
  {
...
struct C_ClientList : public C_AioExec {
  ...
  void
  send(const std::string& start_after)
  {
    bufferlist inbl;
...
get_immutable_metadata(
    ...
    Context* on_finish)
{
  C_ImmutableMetadata* metadata = new C_ImmutableMetadata(
      ioctx, oid, order, splay_width, pool_id, on_finish);
  metadata->send();
}
    ...
    Context* on_finish)
{
  ...
  C_MutableMetadata* metadata =
      new C_MutableMetadata(ioctx, oid, minimum_set, active_set, client_list);
  metadata->send();
}

void
set_minimum_set(librados::ObjectWriteOperation* op, uint64_t object_set)
{
  bufferlist bl;
...
void
set_active_set(librados::ObjectWriteOperation* op, uint64_t object_set)
{
  bufferlist bl;
  encode(object_set, bl);
...
void
get_client_start(librados::ObjectReadOperation* op, const std::string& id)
{
  bufferlist bl;
  encode(id, bl);
...
int
get_client_finish(bufferlist::const_iterator* iter, cls::journal::Client* client)
{
  try {
    decode(*client, *iter);
  } catch (const buffer::error& err) {
...
void
client_unregister(librados::ObjectWriteOperation* op, const std::string& id)
{
...
void
get_next_tag_tid_start(librados::ObjectReadOperation* op)
{
  bufferlist bl;
...
int
get_next_tag_tid_finish(bufferlist::const_iterator* iter, uint64_t* tag_tid)
{
  try {
    decode(*tag_tid, *iter);
  } catch (const buffer::error& err) {
...
void
get_tag_start(librados::ObjectReadOperation* op, uint64_t tag_tid)
{
  bufferlist bl;
  encode(tag_tid, bl);
...
int
get_tag_finish(bufferlist::const_iterator* iter, cls::journal::Tag* tag)
{
  try {
    decode(*tag, *iter);
  } catch (const buffer::error& err) {
    ...
    std::set<cls::journal::Tag>* tags)
{
  ...
  while (true) {
    librados::ObjectReadOperation op;
    tag_list_start(
        &op, start_after_tag_tid, JOURNAL_MAX_RETURN, client_id, tag_class);
...
void
guard_append(librados::ObjectWriteOperation* op, uint64_t soft_max_size)
{
  bufferlist bl;
