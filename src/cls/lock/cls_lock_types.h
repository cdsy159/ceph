...
inline const char*
cls_lock_type_str(ClsLockType type)
{
  switch (type) {
...
inline bool
cls_lock_is_exclusive(ClsLockType type)
{
  return ClsLockType::EXCLUSIVE == type ||
         ClsLockType::EXCLUSIVE_EPHEMERAL == type;
}

inline bool
cls_lock_is_ephemeral(ClsLockType type)
{
  return ClsLockType::EXCLUSIVE_EPHEMERAL == type;
}

inline bool
cls_lock_is_valid(ClsLockType type)
{
  return ClsLockType::SHARED == type || ClsLockType::EXCLUSIVE == type ||
         ClsLockType::EXCLUSIVE_EPHEMERAL == type;
}
...
struct locker_id_t {
  ...
  bool
  operator<(const locker_id_t& rhs) const
  {
  ...
  friend std::ostream&
  operator<<(std::ostream& out, const locker_id_t& data)
  {
    out << data.locker;
    return out;
  }
...
struct locker_info_t {
  ...
  friend std::ostream&
  operator<<(std::ostream& out, const locker_info_t& data)
  {
    using ceph::operator<<;
