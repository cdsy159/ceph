...
void
otp_instance::trim_expired(const ceph::real_time& now)
{
...
void
otp_instance::check(const string& token, const string& val, bool* update)
{
...
bool
otp_instance::verify(const ceph::real_time& timestamp, const string& val)
{
  uint64_t index;
  uint32_t secs = (uint32_t)ceph::real_clock::to_time_t(timestamp);
  int result = oath_totp_validate2(
      otp.seed_bin.c_str(), otp.seed_bin.length(), secs, otp.step_size,
      otp.time_ofs, otp.window, nullptr /* otp pos */, val.c_str());
  ...
  if (index <= last_success) { /* already used value */
    CLS_LOG(
        20, "otp, use of old token: index=%lld last_success=%lld",
        (long long)index, (long long)last_success);
    return false;
  }
...
void
otp_instance::find(const string& token, otp_check_t* result)
{
...
static int
write_otp_instance(cls_method_context_t hctx, const otp_instance& instance)
{
  ...
  if (r < 0) {
    CLS_ERR(
        "ERROR: %s(): failed to store key (otp id=%s, r=%d)", __func__,
        instance.otp.id.c_str(), r);
    return r;
  }
...
static int
remove_otp_instance(cls_method_context_t hctx, const string& id)
{
  string key = otp_key_prefix + id;
  ...
  if (r < 0) {
    CLS_ERR(
        "ERROR: %s(): failed to remove key (otp id=%s, r=%d)", __func__,
        id.c_str(), r);
    return r;
  }
...
static int
read_header(cls_method_context_t hctx, otp_header* h)
{
  bufferlist bl;
  encode(h, bl);
...
static int
write_header(cls_method_context_t hctx, const otp_header& h)
{
  bufferlist bl;
  encode(h, bl);
...
static int
parse_seed(const string& seed, SeedType seed_type, bufferlist* seed_bin)
{
...
static int
otp_set_op(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  ...
  for (auto entry : op.entries) {
    ...
    r = parse_seed(
        instance.otp.seed, instance.otp.seed_type, &instance.otp.seed_bin);
    if (r < 0) {
      return r;
    }
...
static int
otp_remove_op(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  CLS_LOG(20, "%s", __func__);
...
static int
otp_get_op(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  CLS_LOG(20, "%s", __func__);
...
static int
otp_check_op(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  CLS_LOG(20, "%s", __func__);
...
static int
otp_get_result(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  CLS_LOG(20, "%s", __func__);
  cls_otp_check_otp_op op;
  try {
...
CLS_INIT(otp)
{
  ...
  cls_register_cxx_method(
      h_class, "otp_set", CLS_METHOD_RD | CLS_METHOD_WR, otp_set_op,
      &h_set_otp_op);
  cls_register_cxx_method(
      h_class, "otp_get", CLS_METHOD_RD, otp_get_op, &h_get_otp_op);
  cls_register_cxx_method(
      h_class, "otp_check", CLS_METHOD_RD | CLS_METHOD_WR, otp_check_op,
      &h_check_otp_op);
  cls_register_cxx_method(
      h_class, "otp_get_result", CLS_METHOD_RD, otp_get_result,
      &h_get_result_op);
  cls_register_cxx_method(
      h_class, "otp_remove", CLS_METHOD_RD | CLS_METHOD_WR, otp_remove_op,
      &h_remove_otp_op);
  cls_register_cxx_method(
