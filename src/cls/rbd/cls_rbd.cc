...
uint64_t
get_encode_features(cls_method_context_t hctx)
{
  uint64_t features = 0;
...
static int
snap_read_header(cls_method_context_t hctx, bufferlist& bl)
{
...
static void
key_from_snap_id(snapid_t snap_id, string* out)
{
  ostringstream oss;
  oss << RBD_SNAP_KEY_PREFIX << std::setw(16) << std::setfill('0') << std::hex
      << snap_id;
  *out = oss.str();
}

static snapid_t
snap_id_from_key(const string& key)
{
...
template <typename T>
static int
read_key(cls_method_context_t hctx, const string& key, T* out)
{
  bufferlist bl;
  ...
  if (r < 0) {
    if (r != -ENOENT) {
      CLS_ERR(
          "error reading omap key %s: %s", key.c_str(), cpp_strerror(r).c_str());
    }
    return r;
  }
...
template <typename T>
static int
write_key(cls_method_context_t hctx, const string& key, const T& t)
{
  bufferlist bl;
  encode(t, bl);
...
static int
remove_key(cls_method_context_t hctx, const string& key)
{
 ...
 */
static int
check_exists(cls_method_context_t hctx)
{
  uint64_t size;
  time_t mtime;
 ...
 */
int
require_feature(cls_method_context_t hctx, uint64_t need)
{
  uint64_t features;
  ...
  if ((features & need) != need) {
    CLS_LOG(
        10, "require_feature missing feature %llx, have %llx",
...
{
  ostringstream oss;
  oss << RBD_SNAP_CHILDREN_KEY_PREFIX << std::setw(16) << std::setfill('0')
      << std::hex << snap_id;
  return oss.str();
}

int
set_op_features(cls_method_context_t hctx, uint64_t op_features, uint64_t mask)
{
  ...
  CLS_LOG(
      10, "op_features=%" PRIu64 " orig_op_features=%" PRIu64, op_features,
      orig_op_features);
  ...
  if (init) {
    ...
    if (r == -ENOENT) {
      ...
      if (memcmp(RBD_HEADER_TEXT, header.c_str(), header.length()) != 0) {
        if (memcmp(RBD_MIGRATE_HEADER_TEXT, header.c_str(), header.length()) ==
            0) {
          CLS_LOG(10, "migration already set");
          return -EEXIST;
        } else {
...
int
read_migration(cls_method_context_t hctx, cls::rbd::MigrationSpec* migration_spec)
{
  uint64_t features = 0;
  int r = read_key(hctx, "features", &features);
  if (r == -ENOENT) {
...
int
remove_migration(cls_method_context_t hctx)
{
...
template <typename L>
int
iterate(cls_method_context_t hctx, L& lambda)
{
  ...
  do {
    ...
    int r = cls_cxx_map_get_vals(
        hctx, last_read, RBD_SNAP_KEY_PREFIX, max_read, &vals, &more);
    if (r < 0) {
      return r;
    }
    ...
    for (auto& val : vals) {
      ...
      try {
        ...
        CLS_ERR(
            "error decoding snapshot metadata for snap : %s", val.first.c_str());
        return -EIO;
      }
...
int
write(cls_method_context_t hctx, const std::string& snap_key, cls_rbd_snap&& snap)
{
  int r;
...
int
attach(cls_method_context_t hctx, cls_rbd_parent parent, bool reattach)
{
  ...
  CLS_LOG(
      20,
      "cls_rbd::image::parent::attach: pool=%" PRIi64
      ", ns=%s, id=%s, "
      "snapid=%" PRIu64 ", size=%" PRIu64,
      parent.pool_id, parent.pool_namespace.c_str(), parent.image_id.c_str(),
      parent.snap_id.val, parent.head_overlap.value_or(0ULL));
...
int
detach(cls_method_context_t hctx, bool legacy_api)
{
  int r = check_exists(hctx);
  if (r < 0) {
 ...
 */
int
create(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  ...
  CLS_LOG(
      20, "create object_prefix=%s size=%llu order=%u features=%llu",
 ...
 */
int
get_features(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  ...
  uint64_t incompatible =
      (read_only ? features & RBD_FEATURES_INCOMPATIBLE
 ...
 */
int
set_features(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  uint64_t features;
  uint64_t mask;
  auto iter = in->cbegin();
  ...
  if (r < 0 && r != -ENOENT) {
    CLS_ERR(
        "Could not read image's features off disk: %s", cpp_strerror(r).c_str());
    return r;
  }

  if ((mask & RBD_FEATURES_INTERNAL) != 0ULL) {
    CLS_ERR(
        "Attempting to set internal feature: %" PRIu64,
  ...
  if ((enabled_features & RBD_FEATURES_MUTABLE) != enabled_features) {
    CLS_ERR(
        "Attempting to enable immutable feature: %" PRIu64,
  ...
  if ((disabled_features & disable_mask) != disabled_features) {
    CLS_ERR(
        "Attempting to disable immutable feature: %" PRIu64,
  ...
  CLS_LOG(
      10, "set_features features=%" PRIu64 " orig_features=%" PRIu64, features,
      orig_features);
 ...
 */
int
get_size(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  ...
  if (snap_id == CEPH_NOSNAP) {
    ...
    if (r < 0) {
      CLS_ERR(
          "failed to read the image's size off of disk: %s",
 ...
 */
int
set_size(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  uint64_t size;
  ...
  CLS_LOG(
      20, "set_size size=%llu orig_size=%llu", (unsigned long long)size,
 ...
 */
int
get_protection_status(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  snapid_t snap_id;
  ...
  CLS_LOG(
      20, "get_protection_status snap_id=%llu", (unsigned long long)snap_id.val);
  ...
  if (snap.protection_status >= RBD_PROTECTION_STATUS_LAST) {
    CLS_ERR(
        "invalid protection status for snap id %llu: %u",
 ...
 */
int
set_protection_status(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  snapid_t snap_id;
  uint8_t status;
  ...
  CLS_LOG(
      20, "set_protection_status snapid=%llu status=%u",
  ...
  if (status >= RBD_PROTECTION_STATUS_LAST) {
    CLS_LOG(
        10, "invalid protection status for snap id %llu: %u",
        (unsigned long long)snap_id.val, status);
    return -EINVAL;
  }
 ...
 */
int
get_stripe_unit_count(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  int r = check_exists(hctx);
  if (r < 0)
    return r;
  ...
  if (r == -ENOENT) {
    ...
    if (r < 0) {
      CLS_ERR(
          "failed to read the order off of disk: %s", cpp_strerror(r).c_str());
      return -EIO;
    }
 ...
 */
int
set_stripe_unit_count(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  ...
  if ((1ull << order) % stripe_unit || stripe_unit > (1ull << order)) {
    CLS_ERR(
        "stripe unit %llu is not a factor of the object size %llu",
...
int
get_create_timestamp(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
 ...
 */
int
get_access_timestamp(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
 ...
 */
int
get_modify_timestamp(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
 ...
 */
int
get_flags(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  uint64_t snap_id;
  auto iter = in->cbegin();
  try {
 ...
 */
int
set_flags(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  uint64_t flags;
  uint64_t mask;
  ...
  if (snap_id == CEPH_NOSNAP) {
    ...
    if (r < 0 && r != -ENOENT) {
      CLS_ERR(
          "Could not read image's flags off disk: %s", cpp_strerror(r).c_str());
      return r;
    }
  } else {
    ...
    if (r < 0) {
      CLS_ERR(
          "Could not read snapshot: snap_id=%" PRIu64 ": %s", snap_id,
          cpp_strerror(r).c_str());
      return r;
    }
 ...
 */
int
op_features_get(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
 ...
 */
int
op_features_set(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
 ...
 */
int
get_parent(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  uint64_t snap_id;
  ...
  if (r == 0) {
    ...
    if (snap_id != CEPH_NOSNAP) {
      ...
      if (snap.parent.exists()) {
        ...
        if (!parent.exists()) {
          CLS_ERR(
              "get_parent: snap_id=%" PRIu64 ": invalid parent spec", snap_id);
          return -EINVAL;
        }
 ...
 */
int
set_parent(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  cls_rbd_parent parent;
  auto iter = in->cbegin();
  try {
 ...
 */
int
remove_parent(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
 ...
 */
int
parent_get(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  int r = check_exists(hctx);
  if (r < 0) {
    return r;
  ...
  cls::rbd::ParentImageSpec parent_image_spec{
      parent.pool_id, parent.pool_namespace, parent.image_id, parent.snap_id};
 ...
 */
int
parent_overlap_get(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  uint64_t snap_id;
  auto iter = in->cbegin();
  try {
 ...
 */
int
parent_attach(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  cls::rbd::ParentImageSpec parent_image_spec;
  ...
  int r = image::parent::attach(
      hctx, {parent_image_spec, parent_overlap}, reattach);
  if (r < 0) {
    return r;
  }
 ...
 */
int
parent_detach(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
...
static string
parent_key(uint64_t pool_id, string image_id, snapid_t snap_id)
{
  bufferlist key_bl;
...
int
add_child(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  int r;
  ...
  r = decode_parent_and_child(
      in, &p_pool_id, &p_image_id, &p_snap_id, &c_image_id);
  if (r < 0)
    return r;

  CLS_LOG(
      20, "add_child %s to (%" PRIu64 ", %s, %" PRIu64 ")", c_image_id.c_str(),
...
int
remove_child(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  int r;
  ...
  CLS_LOG(
      20, "remove_child %s from (%" PRIu64 ", %s, %" PRIu64 ")",
      c_image_id.c_str(), p_pool_id, p_image_id.c_str(), p_snap_id.val);
  ...
  if (children.empty()) {
    ...
    if (r < 0)
      CLS_LOG(
          20, "remove_child: remove key failed: %s", cpp_strerror(r).c_str());
  } else {
    ...
    if (r < 0)
      CLS_LOG(
          20, "remove_child: write omap failed: %s", cpp_strerror(r).c_str());
  }
  return r;
}
 ...
 */
int
get_children(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  int r;
  uint64_t p_pool_id;
  snapid_t p_snap_id;
  ...
  CLS_LOG(
      20, "get_children of (%" PRIu64 ", %s, %" PRIu64 ")", p_pool_id,
      p_image_id.c_str(), p_snap_id.val);
 ...
 */
int
get_snapcontext(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  ...
  if (r < 0) {
    CLS_ERR(
        "could not read the image's snap_seq off disk: %s",
        cpp_strerror(r).c_str());
    return r;
  }
 ...
 */
int
get_object_prefix(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  ...
  if (r < 0) {
    CLS_ERR(
        "failed to read the image's object prefix off of disk: %s",
        cpp_strerror(r).c_str());
    return r;
  }
 ...
 */
int
get_data_pool(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
 ...
 */
int
get_snapshot_name(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  uint64_t snap_id;
 ...
 */
int
get_snapshot_timestamp(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  uint64_t snap_id;
  ...
  CLS_LOG(
      20, "get_snapshot_timestamp snap_id=%llu", (unsigned long long)snap_id);
 ...
 */
int
snapshot_get(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  uint64_t snap_id;
 ...
 */
int
snapshot_add(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  ...
  CLS_LOG(
      20, "snapshot_add name=%s id=%llu", snap_meta.name.c_str(),
  ...
  if (r < 0) {
    CLS_ERR(
        "Could not read image's snap_seq off disk: %s", cpp_strerror(r).c_str());
    return r;
  }
  ...
  if (r == -ENOENT) {
    ...
    CLS_ERR(
        "Could not read snapshot limit off disk: %s", cpp_strerror(r).c_str());
    return r;
  }
  ...
  auto pre_check_lambda = [&snap_meta, &total_read,
                           snap_limit](const cls_rbd_snap& old_meta) {
    ++total_read;
    ...
    if ((snap_meta.name == old_meta.name &&
      ...
      CLS_LOG(
          20,
          "snap_name %s or snap_id %" PRIu64
          " matches existing snap "
          "%s %" PRIu64,
          snap_meta.name.c_str(), snap_meta.id.val, old_meta.name.c_str(),
          old_meta.id.val);
      return -EEXIST;
    }
    return 0;
  ...
  if (cls::rbd::get_snap_namespace_type(snap_meta.snapshot_namespace) ==
    ...
    r = image::set_op_features(
        hctx, RBD_OPERATION_FEATURE_SNAP_TRASH,
 ...
 */
int
snapshot_rename(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  bufferlist snap_namebl, snap_idbl;
  ...
  CLS_LOG(
      20, "snapshot_rename id=%" PRIu64 ", dst_name=%s", src_snap_id.val,
      dst_snap_name.c_str());
  ...
  auto duplicate_name_lambda = [&dst_snap_name](const cls_rbd_snap& snap_meta) {
    if (cls::rbd::get_snap_namespace_type(snap_meta.snapshot_namespace) ==
      ...
      CLS_LOG(
          20, "snap_name %s matches existing snap with snap id %" PRIu64,
  ...
  if (r == -ENOENT) {
    CLS_LOG(
        20, "cannot find existing snap with snap id = %" PRIu64,
        src_snap_id.val);
    return r;
  }
 ...
 */
int
snapshot_remove(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  snapid_t snap_id;
  ...
  auto remove_lambda = [snap_id, &has_child_snaps,
                        &has_trash_snaps](const cls_rbd_snap& snap_meta) {
 ...
 */
int
snapshot_trash_add(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  snapid_t snap_id;
  ...
  // add snap_trash feature bit if not already enabled
  r = image::set_op_features(
      hctx, RBD_OPERATION_FEATURE_SNAP_TRASH, RBD_OPERATION_FEATURE_SNAP_TRASH);
  if (r < 0) {
    return r;
  }

  snap.snapshot_namespace =
      cls::rbd::TrashSnapshotNamespace{snap_type, snap.name};
  uuid_d uuid_gen;
 ...
 */
int
get_all_features(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
...
int
copyup(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
...
int
sparse_copyup(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  ...
  for (auto& it : extent_map) {
    ...
    CLS_LOG(
        20, "sparse_copyup: writing extent %" PRIu64 "~%" PRIu64 "\n", off, len);
    int r = cls_cxx_write(hctx, off, len, &tmpbl);
    if (r < 0) {
      CLS_ERR(
          "sparse_copyup: error writing extent %" PRIu64 "~%" PRIu64 ": %s",
 ...
 */
int
get_id(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  uint64_t size;
 ...
 */
int
set_id(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  int r = check_exists(hctx);
  if (r < 0)
    return r;
 ...
 */
int
set_access_timestamp(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  int r = check_exists(hctx);
  if (r < 0)
    return r;
...
int
set_modify_timestamp(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  int r = check_exists(hctx);
  if (r < 0)
    return r;
...
static const string
dir_key_for_id(const string& id)
{
  return RBD_DIR_ID_KEY_PREFIX + id;
}

static const string
dir_key_for_name(const string& name)
{
  return RBD_DIR_NAME_KEY_PREFIX + name;
}

static const string
dir_name_from_key(const string& key)
{
  return key.substr(strlen(RBD_DIR_NAME_KEY_PREFIX));
}

static int
dir_add_image_helper(
    cls_method_context_t hctx,
    const string& name,
    const string& id,
    bool check_for_unique_id)
{
  if (!name.size() || !is_valid_id(id)) {
    CLS_ERR(
        "dir_add_image_helper: invalid name '%s' or id '%s'", name.c_str(),
        id.c_str());
    return -EINVAL;
  }
    ...
    const string& id)
{
  ...
  if (stored_name != name || stored_id != id) {
    CLS_ERR(
        "stored name '%s' and id '%s' do not match args '%s' and '%s'",
 ...
 */
int
dir_rename_image(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  string src, dest, id;
  try {
    auto iter = in->cbegin();
 ...
 */
int
dir_get_id(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  string name;
  ...
  if (r < 0) {
    if (r != -ENOENT)
      CLS_ERR(
          "error reading id for name '%s': %s", name.c_str(),
          cpp_strerror(r).c_str());
    return r;
  }
  encode(id, *out);
 ...
 */
int
dir_get_name(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  string id;
  ...
  if (r < 0) {
    if (r != -ENOENT) {
      CLS_ERR(
          "error reading name for id '%s': %s", id.c_str(),
          cpp_strerror(r).c_str());
    }
    return r;
  }
 ...
 */
int
dir_list(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  string start_after;
  uint64_t max_return;
  ...
  while (more && images.size() < max_return) {
    ...
    int r = cls_cxx_map_get_vals(
        hctx, last_read, RBD_DIR_NAME_KEY_PREFIX, max_read, &vals, &more);
    if (r < 0) {
      if (r != -ENOENT) {
    ...
    for (auto it = vals.begin(); it != vals.end(); ++it) {
      ...
      CLS_LOG(
          20, "adding '%s' -> '%s'", dir_name_from_key(it->first).c_str(),
          id.c_str());
 ...
 */
int
dir_add_image(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
 ...
 */
int
dir_remove_image(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  string name, id;
  try {
    auto iter = in->cbegin();
 ...
 */
int
dir_state_assert(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
 ...
 */
int
dir_state_set(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  ...
  case cls::rbd::DIRECTORY_STATE_ADD_DISABLED: {
    ...
    r = cls_cxx_map_get_vals(
        hctx, RBD_DIR_NAME_KEY_PREFIX, RBD_DIR_NAME_KEY_PREFIX, 1, &vals, &more);
    if (r < 0) {
      return r;
    } else if (!vals.empty()) {
      return -EBUSY;
...
int
object_map_read(cls_method_context_t hctx, BitVector<2>& object_map)
{
  uint64_t size;
  int r = cls_cxx_stat(hctx, &size, NULL);
  if (r < 0) {
 ...
 */
int
object_map_load(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
 ...
 */
int
object_map_save(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  BitVector<2> object_map;
  try {
    auto iter = in->cbegin();
  ...
  CLS_LOG(
      20, "object_map_save: object size=%" PRIu64 ", byte size=%u",
 ...
 */
int
object_map_resize(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  ...
  CLS_LOG(
      20, "object_map_resize: object size=%" PRIu64 ", byte size=%u",
 ...
 */
int
object_map_update(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  ...
  r = cls_cxx_read2(
      hctx, 0, object_map.get_header_length(), &header_bl,
  ...
  r = cls_cxx_read2(
      hctx, object_byte_offset, byte_length, &footer_bl,
      CEPH_OSD_OP_FLAG_FADVISE_WILLNEED);
  if (r < 0) {
  ...
  object_map.get_data_crcs_extents(
      start_object_no, object_count, &object_byte_offset, &byte_length);
  ...
  object_map.get_data_extents(
      start_object_no, object_count, &data_byte_offset, &object_byte_offset,
      &byte_length);

  bufferlist data_bl;
  r = cls_cxx_read2(
      hctx, object_byte_offset, byte_length, &data_bl,
      CEPH_OSD_OP_FLAG_FADVISE_WILLNEED);
  if (r < 0) {
  ...
  try {
    ...
    CLS_ERR(
        "failed to decode data chunk [%" PRIu64 "]: %s", data_byte_offset,
        err.what());
    return -EINVAL;
  }
  ...
  if (updated) {
    ...
    r = cls_cxx_write2(
        hctx, footer_object_offset, footer_bl.length(), &footer_bl,
        CEPH_OSD_OP_FLAG_FADVISE_WILLNEED);
    if (r < 0) {
 ...
 */
int
object_map_snap_add(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  BitVector<2> object_map;
  int r = object_map_read(hctx, object_map);
  if (r < 0) {
 ...
 */
int
object_map_snap_remove(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
...
static const string
metadata_key_for_name(const string& name)
{
  return RBD_METADATA_KEY_PREFIX + name;
}

static const string
metadata_name_from_key(const string& key)
{
 ...
 */
int
metadata_list(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  string start_after;
  uint64_t max_return;
  ...
  while (more && data.size() < max_return) {
    ...
    int max_read =
        std::min<uint64_t>(RBD_MAX_KEYS_READ, max_return - data.size());
    int r = cls_cxx_map_get_vals(
        hctx, last_read, RBD_METADATA_KEY_PREFIX, max_read, &raw_data, &more);
    if (r < 0) {
      if (r != -ENOENT) {
        CLS_ERR(
            "failed to read the vals off of disk: %s", cpp_strerror(r).c_str());
      }
      return r;
    }
 ...
 */
int
metadata_set(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  ...
  for (auto it = data.begin(); it != data.end(); ++it) {
    CLS_LOG(
        20, "metadata_set key=%s value=%.*s", it->first.c_str(),
 ...
 */
int
metadata_remove(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  string key;
 ...
 */
int
metadata_get(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  string key;
  bufferlist value;
...
int
snapshot_get_limit(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  uint64_t snap_limit;
...
int
snapshot_set_limit(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  int rc;
  uint64_t new_limit;
  bufferlist bl;
 ...
 */
int
child_attach(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  uint64_t snap_id;
  ...
  CLS_LOG(
      ...
      ", "
      "child_image_id=%s",
      snap_id, child_image.pool_id, child_image.image_id.c_str());
  ...
  r = image::set_op_features(
      hctx, RBD_OPERATION_FEATURE_CLONE_PARENT,
 ...
 */
int
child_detach(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  uint64_t snap_id;
  cls::rbd::ChildImageSpec child_image;
  try {
 ...
 */
int
children_list(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  uint64_t snap_id;
  try {
    auto it = in->cbegin();
 ...
 */
int
migration_set(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
 ...
 */
int
migration_set_state(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
 ...
 */
int
migration_get(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  cls::rbd::MigrationSpec migration_spec;
  int r = image::read_migration(hctx, &migration_spec);
  if (r < 0) {
 ...
 */
int
migration_remove(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
 ...
 */
int
assert_snapc_seq(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  uint64_t snapc_seq;
...
int
old_snapshots_list(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  bufferlist bl;
  struct rbd_obj_header_ondisk* header;
  ...
  char* name = buf + sizeof(*header) +
               header->snap_count * sizeof(struct rbd_obj_snap_ondisk);
...
int
old_snapshot_add(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  bufferlist bl;
  struct rbd_obj_header_ondisk* header;
  bufferlist newbl;
...
int
old_snapshot_remove(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  bufferlist bl;
  struct rbd_obj_header_ondisk* header;
  bufferlist newbl;
  ...
  int snaps_id_ofs = sizeof(*header);
  int names_ofs = snaps_id_ofs +
                  sizeof(struct rbd_obj_snap_ondisk) * header->snap_count;
  const char* snap_name;
  const char* snap_names = ((char*)header) + names_ofs;
  ...
  if (header->snap_count) {
    ...
    if (i < header->snap_count) {
      memcpy(
          new_snaps_bp.c_str() + snaps_len, header->snaps + i + 1,
...
*/
int
old_snapshot_rename(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  bufferlist bl;
  struct rbd_obj_header_ondisk* header;
  bufferlist newbl;
  ...
  int snaps_id_ofs = sizeof(*header);
  int names_ofs = snaps_id_ofs +
                  sizeof(rbd_obj_snap_ondisk) * header->snap_count;
  const char* snap_names = ((char*)header) + names_ofs;
  const char* orig_names = snap_names;
  const char* end = snap_names + header->snap_names_len;
  auto iter = in->cbegin();
  ...
  CLS_LOG(
      20, "rename snap with snap id %llu to dest name %s",
      (unsigned long long)src_snap_id.val, dst_snap_name);
  header->snap_names_len = header->snap_names_len - strlen(snap_names) +
                           dst.length();
  ...
  if (header->snap_count) {
    ...
    memcpy(
        new_snaps_bp.c_str(), header->snaps,
        sizeof(header->snaps[0]) * header->snap_count);
  }
...
static const std::string REMOTE_STATUS_GLOBAL_KEY_PREFIX(
    "remote_status_global_");
...
std::string
peer_key(const std::string& uuid)
{
  return PEER_KEY_PREFIX + uuid;
}

std::string
image_key(const string& image_id)
{
  return IMAGE_KEY_PREFIX + image_id;
}

std::string
global_key(const string& global_id)
{
  return GLOBAL_KEY_PREFIX + global_id;
}

std::string
remote_status_global_key(
...
std::string
status_global_key(const std::string& global_id, const std::string& mirror_uuid)
{
...
std::string
instance_key(const string& instance_id)
{
  return INSTANCE_KEY_PREFIX + instance_id;
}

std::string
mirror_image_map_key(const string& global_image_id)
{
  return MIRROR_IMAGE_MAP_KEY_PREFIX + global_image_id;
}

int
uuid_get(cls_method_context_t hctx, std::string* mirror_uuid)
{
...
int
list_watchers(cls_method_context_t hctx, std::set<entity_inst_t>* entities)
{
...
int
read_peers(cls_method_context_t hctx, std::vector<cls::rbd::MirrorPeer>* peers)
{
  ...
  while (more) {
    std::map<std::string, bufferlist> vals;
    int r = cls_cxx_map_get_vals(
        hctx, last_read, PEER_KEY_PREFIX.c_str(), max_read, &vals, &more);
    if (r < 0) {
      if (r != -ENOENT) {
...
int
write_peer(cls_method_context_t hctx, const cls::rbd::MirrorPeer& peer)
{
  bufferlist bl;
  encode(peer, bl);
  ...
  if (r < 0) {
    CLS_ERR(
        "error writing peer '%s': %s", peer.uuid.c_str(),
        cpp_strerror(r).c_str());
    return r;
  }
  return 0;
...
int
check_mirroring_enabled(cls_method_context_t hctx)
{
  ...
  if (r < 0 && r != -ENOENT) {
    return r;
      ...
      r == -ENOENT || mirror_mode_decode == cls::rbd::MIRROR_MODE_DISABLED) {
    ...
    const std::string& mirror_uuid)
{
  ...
  auto site_it =
      std::find_if(peers.begin(), peers.end(), [&site_name](auto& peer) {
  ...
  if (site_it == peers.end() ||
    ...
    mirror_uuid_it =
        std::find_if(peers.begin(), peers.end(), [&mirror_uuid](auto& peer) {
...
int
peer_add(cls_method_context_t hctx, cls::rbd::MirrorPeer mirror_peer)
{
  int r = check_mirroring_enabled(hctx);
  if (r < 0) {
    return r;
  ...
  if (r < 0) {
    ...
    return -EINVAL;
      ...
      mirror_peer.mirror_peer_direction == cls::rbd::MIRROR_PEER_DIRECTION_TX) {
    CLS_ERR(
        "peer uuid '%s' cannot use TX-only direction", mirror_peer.uuid.c_str());
    return -EINVAL;
  }
...
int
peer_remove(cls_method_context_t hctx, const std::string& uuid)
{
    ...
    cls::rbd::MirrorImage* mirror_image)
{
  ...
  if (r < 0) {
    if (r != -ENOENT) {
      CLS_ERR(
          "error reading mirrored image '%s': '%s'", image_id.c_str(),
          cpp_strerror(r).c_str());
    }
    return r;
  }
    ...
    const cls::rbd::MirrorImage& mirror_image)
{
  ...
  if (r == -ENOENT) {
    ...
    if (r >= 0) {
      ...
      CLS_ERR(
          "error reading global image id: '%s': '%s'", image_id.c_str(),
          cpp_strerror(r).c_str());
      return r;
    }
  ...
  if (r < 0) {
    CLS_ERR(
        "error adding mirrored image '%s': %s", image_id.c_str(),
        cpp_strerror(r).c_str());
    return r;
  }
  ...
  r = cls_cxx_map_set_val(
      hctx, global_key(mirror_image.global_image_id), &image_id_bl);
  if (r < 0) {
    CLS_ERR(
        "error adding global id for image '%s': %s", image_id.c_str(),
        cpp_strerror(r).c_str());
    return r;
  }
  return 0;
...
int
image_remove(cls_method_context_t hctx, const string& image_id)
{
  bufferlist bl;
  cls::rbd::MirrorImage mirror_image;
  ...
  if (r < 0) {
    CLS_ERR(
        "error removing mirrored image '%s': %s", image_id.c_str(),
        cpp_strerror(r).c_str());
    return r;
  }
  ...
  if (r < 0 && r != -ENOENT) {
    CLS_ERR(
        "error removing global id for image '%s': %s", image_id.c_str(),
        cpp_strerror(r).c_str());
    return r;
  }
    ...
    const cls::rbd::MirrorImageSiteStatus& status)
{
  ...
  r = cls_cxx_map_set_val(
      hctx, status_global_key(global_image_id, status.mirror_uuid), &bl);
  if (r < 0) {
    CLS_ERR(
        "error setting status for mirrored image, global id '%s', "
        "site '%s': %s",
        global_image_id.c_str(), status.mirror_uuid.c_str(),
        cpp_strerror(r).c_str());
    return r;
  }
  return 0;
...
int
get_remote_image_status_mirror_uuids(
    cls_method_context_t hctx,
...
int
image_status_remove(cls_method_context_t hctx, const string& global_image_id)
{
  // remove all local/remote image statuses
  std::set<std::string> mirror_uuids;
  int r = get_remote_image_status_mirror_uuids(
      hctx, global_image_id, &mirror_uuids);
  if (r < 0 && r != -ENOENT) {
    return r;
  }
  ...
  for (auto& mirror_uuid : mirror_uuids) {
    CLS_LOG(
        20, "removing status object for mirror_uuid %s", mirror_uuid.c_str());
    auto key = status_global_key(global_image_id, mirror_uuid);
    r = cls_cxx_map_remove_key(hctx, key);
    if (r < 0 && r != -ENOENT) {
      CLS_ERR(
          "error removing stale status for key '%s': %s", key.c_str(),
          cpp_strerror(r).c_str());
      return r;
    }
  }
    ...
    cls::rbd::MirrorImageStatus* status)
{
  ...
  try {
    ...
    CLS_ERR(
        "could not decode status for mirrored image, global id '%s', "
        "site '%s'",
    ...
    cls::rbd::MirrorImageStatus* status)
{
  ...
  int r = cls_cxx_map_get_val(
      hctx,
      status_global_key(
          global_image_id, cls::rbd::MirrorImageSiteStatus::LOCAL_MIRROR_UUID),
      &bl);
  if (r == -ENOENT) {
    return 0;
  } else if (r < 0) {
    CLS_ERR(
        "error reading status for mirrored image, global id '%s', "
        "site '%s': '%s'",
  ...
  return image_status_get(
      hctx, global_image_id, cls::rbd::MirrorImageSiteStatus::LOCAL_MIRROR_UUID,
      bl, watchers, status);
}

int
image_status_get_remote(
    cls_method_context_t hctx,
    const string& global_image_id,
    const std::set<entity_inst_t>& watchers,
    cls::rbd::MirrorImageStatus* status)
{
  ...
  do {
    ...
    CLS_LOG(20, "last_read = '%s'", last_read.c_str());
    int r =
        cls_cxx_map_get_vals(hctx, last_read, filter, max_read, &vals, &more);
    if (r == -ENOENT) {
      return 0;
    } else if (r < 0) {
      return r;
    ...
    for (auto& it : vals) {
      ...
      r = image_status_get(
          hctx, global_image_id, mirror_uuid, it.second, watchers, status);
      if (r < 0) {
        return r;
      }
    }
    ...
    map<std::string, cls::rbd::MirrorImageStatus>* mirror_statuses)
{
  ...
  while (more && mirror_images->size() < max_return) {
    ...
    CLS_LOG(20, "last_read = '%s'", last_read.c_str());
    r = cls_cxx_map_get_vals(
        hctx, last_read, IMAGE_KEY_PREFIX, max_read, &vals, &more);
    if (r < 0) {
      if (r != -ENOENT) {
        CLS_ERR(
            "error reading mirror image directory by name: %s",
            cpp_strerror(r).c_str());
      }
      return r;
    }
    ...
    for (auto it = vals.begin();
         it != vals.end() && mirror_images->size() < max_return; ++it) {
      ...
      try {
        ...
        CLS_ERR(
            "could not decode mirror image payload of image '%s'",
            image_id.c_str());
        return -EIO;
      }
      ...
      cls::rbd::MirrorImageStatus status;
      int r1 = image_status_get(
          hctx, mirror_image.global_image_id, watchers, &status);
      if (r1 < 0) {
        continue;
      }
    ...
    const cls::rbd::MirrorImageStatus& status)
{
  ...
  case cls::rbd::MIRROR_PEER_DIRECTION_TX:
    ...
    for (auto& remote_status : status.mirror_image_site_statuses) {
      ...
      auto remote_state =
          (remote_status.up ? remote_status.state
  ...
  while (more) {
    ...
    for (auto& list_it : vals) {
      ...
      try {
        ...
        CLS_ERR(
            "could not decode mirror image payload for key '%s'", key.c_str());
        return -EIO;
      }
...
int
image_status_remove_down(cls_method_context_t hctx)
{
  std::set<entity_inst_t> watchers;
  int r = list_watchers(hctx, &watchers);
  if (r < 0) {
    ...
    while (more) {
      ...
      for (auto& list_it : vals) {
        ...
        try {
          ...
          CLS_ERR(
              "could not decode status metadata for mirrored image '%s'",
              key.c_str());
          return -EIO;
        }
    ...
    entity_inst_t* instance)
{
  ...
  if (r < 0) {
    if (r != -ENOENT) {
      CLS_ERR(
          "error reading status for mirrored image, global id '%s': '%s'",
    ...
    map<std::string, entity_inst_t>* instances)
{
  ...
  while (more && instances->size() < max_return) {
    ...
    for (auto it = vals.begin();
         it != vals.end() && instances->size() < max_return; ++it) {
      const std::string& image_id = it->first.substr(IMAGE_KEY_PREFIX.size());
      cls::rbd::MirrorImage mirror_image;
      auto iter = it->second.cbegin();
      try {
      ...
      entity_inst_t instance;
      r = image_instance_get(
          hctx, mirror_image.global_image_id, watchers, &instance);
      if (r < 0) {
        continue;
      }
...
int
instances_list(cls_method_context_t hctx, std::vector<std::string>* instance_ids)
{
  ...
  while (more) {
    std::map<std::string, bufferlist> vals;
    int r = cls_cxx_map_get_vals(
        hctx, last_read, INSTANCE_KEY_PREFIX.c_str(), max_read, &vals, &more);
    if (r < 0) {
      if (r != -ENOENT) {
...
int
instances_add(cls_method_context_t hctx, const string& instance_id)
{
  bufferlist bl;
  ...
  if (r < 0) {
    CLS_ERR(
        "error setting mirror instance %s: %s", instance_id.c_str(),
        cpp_strerror(r).c_str());
    return r;
  }
  return 0;
...
int
instances_remove(cls_method_context_t hctx, const string& instance_id)
{

  int r = cls_cxx_map_remove_key(hctx, instance_key(instance_id));
  if (r < 0) {
    CLS_ERR(
        "error removing mirror instance %s: %s", instance_id.c_str(),
        cpp_strerror(r).c_str());
    return r;
  }
  return 0;
    ...
    std::map<std::string, cls::rbd::MirrorImageMap>* image_mapping)
{
  ...
  while (more && image_mapping->size() < max_return) {
    ...
    int max_read = std::min<uint64_t>(
        RBD_MAX_KEYS_READ, max_return - image_mapping->size());
    int r = cls_cxx_map_get_vals(
        hctx, last_read, MIRROR_IMAGE_MAP_KEY_PREFIX, max_read, &vals, &more);
    if (r < 0) {
    ...
    for (auto it = vals.begin(); it != vals.end(); ++it) {
      ...
      try {
        ...
        CLS_ERR(
            "could not decode image map payload: %s", cpp_strerror(r).c_str());
        return -EINVAL;
      }
    ...
    std::string mirror_peer_uuid)
{
  ...
  if (r < 0) {
    if (r != -ENOENT) {
      CLS_ERR(
          "Could not read snapshot meta off disk: %s", cpp_strerror(r).c_str());
    }
    return r;
  }

  auto mirror_ns =
      std::get_if<cls::rbd::MirrorSnapshotNamespace>(&snap.snapshot_namespace);
...
int
image_snapshot_set_copy_progress(
    cls_method_context_t hctx,
    uint64_t snap_id,
    bool complete,
 ...
 */
int
mirror_uuid_get(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  std::string mirror_uuid;
 ...
 */
int
mirror_uuid_set(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  std::string mirror_uuid;
  try {
 ...
 */
int
mirror_mode_get(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  uint32_t mirror_mode_decode;
  int r = read_key(hctx, mirror::MODE, &mirror_mode_decode);
  if (r < 0) {
 ...
 */
int
mirror_mode_set(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  uint32_t mirror_mode_decode;
  try {
    auto bl_it = in->cbegin();
    ...
    bufferlist* out)
{
  ...
  if (r < 0 && r != -ENOENT) {
    return r;
      ...
      r == 0 && (mirror_mode != cls::rbd::MIRROR_MODE_DISABLED &&
  ...
  if (r < 0) {
    CLS_ERR(
        "error setting mirror remote namespace: %s", cpp_strerror(r).c_str());
    return r;
  }
  return 0;
}
 ...
 */
int
mirror_peer_ping(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  ...
  try {
    ...
    mirror_peer_direction =
        static_cast<cls::rbd::MirrorPeerDirection>(direction);
  } catch (const ceph::buffer::error& err) {
    return -EINVAL;
  }
 ...
 */
int
mirror_peer_list(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  std::vector<cls::rbd::MirrorPeer> peers;
 ...
 */
int
mirror_peer_add(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  cls::rbd::MirrorPeer mirror_peer;
  try {
    auto it = in->cbegin();
 ...
 */
int
mirror_peer_remove(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  std::string uuid;
  try {
    auto it = in->cbegin();
 ...
 */
int
mirror_peer_set_client(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  std::string uuid;
 ...
 */
int
mirror_image_list(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  ...
  while (more && mirror_images.size() < max_return) {
    ...
    CLS_LOG(20, "last_read = '%s'", last_read.c_str());
    int r = cls_cxx_map_get_vals(
        hctx, last_read, mirror::IMAGE_KEY_PREFIX, max_read, &vals, &more);
    if (r < 0) {
      if (r != -ENOENT) {
        CLS_ERR(
            "error reading mirror image directory by name: %s",
    ...
    bufferlist* out)
{
  ...
  if (r < 0) {
    if (r != -ENOENT) {
      CLS_ERR(
          "error retrieving image id for global id '%s': %s", global_id.c_str(),
          cpp_strerror(r).c_str());
    }
    return r;
  }
 ...
 */
int
mirror_image_get(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  string image_id;
  try {
    auto it = in->cbegin();
 ...
 */
int
mirror_image_set(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  string image_id;
  cls::rbd::MirrorImage mirror_image;
  try {
 ...
 */
int
mirror_image_remove(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  string image_id;
  try {
    auto it = in->cbegin();
    ...
    bufferlist* out)
{
  ...
  int r = mirror::image_status_list(
      hctx, start_after, max_return, &images, &statuses);
  if (r < 0) {
    return r;
  }
    ...
    bufferlist* out)
{
  ...
  std::map<cls::rbd::MirrorImageStatusState, int32_t> states;
  int r = mirror::image_status_get_summary(
      hctx, mirror_peer_direction, tx_peer_mirror_uuids, &states);
  if (r < 0) {
    return r;
  }
 ...
 */
int
mirror_instances_list(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
 ...
 */
int
mirror_instances_add(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
 ...
 */
int
mirror_image_map_list(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  std::string start_after;
  uint64_t max_return;
  try {
  ...
  std::map<std::string, cls::rbd::MirrorImageMap> image_mapping;
  int r = mirror::mirror_image_map_list(
      hctx, start_after, max_return, &image_mapping);
  if (r < 0) {
    return r;
  }
    ...
    bufferlist* out)
{
  ...
  if (r < 0) {
    CLS_ERR(
        "error updating image map %s: %s", key.c_str(), cpp_strerror(r).c_str());
    return r;
  }
    ...
    bufferlist* out)
{
  ...
  if (r < 0 && r != -ENOENT) {
    CLS_ERR(
        "error removing image map %s: %s", key.c_str(), cpp_strerror(r).c_str());
    return r;
  }
    ...
    bufferlist* out)
{
  ...
  CLS_LOG(
      20, "mirror_image_snapshot_unlink_peer snap_id=%" PRIu64 " peer_uuid=%s",
    ...
    bufferlist* out)
{
  ...
  int r = mirror::image_snapshot_set_copy_progress(
      hctx, snap_id, complete, last_copied_object_number);
  if (r < 0) {
    return r;
  }
  return 0;
...
int
dir_remove(cls_method_context_t hctx, const string& name, const string& id)
{
...
std::string
snap_key(const std::string& snap_id)
{
  ostringstream oss;
...
std::string
snap_order_key(const std::string& snap_id)
{
  ostringstream oss;
...
std::string
snap_id_from_order_key(const string& key)
{
    ...
    std::vector<cls::rbd::GroupSnapshot>* group_snaps)
{
  ...
  do {
    int r = cls_cxx_map_get_vals(
        hctx, last_read, RBD_GROUP_SNAP_KEY_PREFIX, max_read, &vals, &more);
    if (r < 0)
      return r;
    ...
    for (auto it = vals.begin();
      ...
      CLS_LOG(
          20, "Discovered snapshot %s %s", snap.name.c_str(), snap.id.c_str());
 ...
 */
int
group_dir_list(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  string start_after;
  uint64_t max_return;
  ...
  while (more && groups.size() < max_return) {
    ...
    for (auto val : vals) {
      ...
      CLS_LOG(
          20, "adding '%s' -> '%s'", dir_name_from_key(val.first).c_str(),
          id.c_str());
 ...
 */
int
group_dir_add(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  int r = cls_cxx_create(hctx, false);
 ...
 */
int
group_dir_rename(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  string src, dest, id;
  try {
    auto iter = in->cbegin();
 ...
 */
int
group_dir_remove(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  string name, id;
  try {
    auto iter = in->cbegin();
 ...
 */
int
group_image_set(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
 ...
 */
int
group_image_remove(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
 ...
 */
int
group_image_list(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  ...
  do {
    int r = cls_cxx_map_get_vals(
        hctx, last_read, cls::rbd::RBD_GROUP_IMAGE_KEY_PREFIX, max_read, &vals,
        &more);
    if (r < 0)
      return r;

    for (auto it = vals.begin(); it != vals.end() && res.size() < max_return;
         ++it) {
      ...
      CLS_LOG(
          20, "Discovered image %s %" PRId64 " %d", spec.image_id.c_str(),
          spec.pool_id, (int)state);
 ...
 */
int
image_group_add(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  ...
  r = image::set_op_features(
      hctx, RBD_OPERATION_FEATURE_GROUP, RBD_OPERATION_FEATURE_GROUP);
  if (r < 0) {
    return r;
  }
 ...
 */
int
image_group_remove(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
 ...
 */
int
image_group_get(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
 ...
 */
int
group_snap_set(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  ...
  if (group_snap.state == cls::rbd::GROUP_SNAPSHOT_STATE_INCOMPLETE) {
    ...
    if (r < 0) {
      CLS_ERR(
          ...
          group::RBD_GROUP_SNAP_MAX_ORDER_KEY.c_str(), cpp_strerror(r).c_str());
      return r;
    }
    ...
    if (r < 0) {
      CLS_ERR(
          "error setting key: %s : %s", order_key.c_str(),
          cpp_strerror(r).c_str());
      return r;
    }
  }
 ...
 */
int
group_snap_remove(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  ...
  if (r < 0) {
    CLS_ERR(
        "error removing snapshot with key %s : %s", snap_key.c_str(),
        cpp_strerror(r).c_str());
    return r;
  }
  ...
  if (r < 0 && r != -ENOENT) {
    CLS_ERR(
        "error removing snapshot order key %s : %s", snap_order_key.c_str(),
        cpp_strerror(r).c_str());
    return r;
  }
 ...
 */
int
group_snap_get_by_id(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
 ...
 */
int
group_snap_list(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
...
int
group_snap_list_order(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  ...
  do {
    int r = cls_cxx_map_get_vals(
        hctx, last_read, group::RBD_GROUP_SNAP_ORDER_KEY_PREFIX, max_read,
        &vals, &more);
    if (r < 0) {
...
std::string
image_key(const std::string& image_id)
{
  return IMAGE_KEY_PREFIX + image_id;
}

std::string
image_id_from_key(const std::string& key)
{
 ...
 */
int
trash_add(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  int r = cls_cxx_create(hctx, false);
  if (r < 0) {
  ...
  if (r < 0 && r != -ENOENT) {
    CLS_ERR(
        "could not read key %s entry from trash: %s", key.c_str(),
        cpp_strerror(r).c_str());
    return r;
  } else if (r == 0) {
    CLS_LOG(10, "id already exists");
 ...
 */
int
trash_remove(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  string id;
  try {
    auto iter = in->cbegin();
  ...
  if (r < 0) {
    if (r != -ENOENT) {
      CLS_ERR(
          "error reading entry key %s: %s", key.c_str(),
          cpp_strerror(r).c_str());
    }
    return r;
  }
 ...
 */
int
trash_list(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  string start_after;
  uint64_t max_return;
  ...
  while (data.size() < max_return) {
    map<string, bufferlist> raw_data;
    int max_read =
        std::min<int32_t>(RBD_MAX_KEYS_READ, max_return - data.size());
    int r = cls_cxx_map_get_vals(
        hctx, last_read, trash::IMAGE_KEY_PREFIX, max_read, &raw_data, &more);
    if (r < 0) {
      if (r != -ENOENT) {
        CLS_ERR(
            "failed to read the vals off of disk: %s", cpp_strerror(r).c_str());
 ...
 */
int
trash_get(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  string id;
  try {
    auto iter = in->cbegin();
  ...
  if (r < 0 && r != -ENOENT) {
    CLS_ERR(
        "error reading image from trash '%s': '%s'", id.c_str(),
        cpp_strerror(r).c_str());
  }
  return r;
}
 ...
 */
int
trash_state_set(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  string id;
  ...
  if (r < 0) {
    if (r != -ENOENT) {
      CLS_ERR(
          "Could not read trash image spec off disk: %s",
          cpp_strerror(r).c_str());
    }
    return r;
  }
  ...
  if (trash_spec.state == expect_state) {
    ...
    CLS_ERR(
        "Current trash state: %d do not match expected: %d or set: %d",
...
std::string
key_for_name(const std::string& name)
{
  return NAME_KEY_PREFIX + name;
}

std::string
name_from_key(const std::string& key)
{
 ...
 */
int
namespace_add(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  std::string name;
  try {
    auto iter = in->cbegin();
 ...
 */
int
namespace_remove(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  std::string name;
  try {
    auto iter = in->cbegin();
 ...
 */
int
namespace_list(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  string start_after;
  uint64_t max_return;
  try {
  ...
  while (data.size() < max_return) {
    ...
    int max_read =
        std::min<int32_t>(RBD_MAX_KEYS_READ, max_return - data.size());
    int r = cls_cxx_map_get_vals(
        hctx, last_read, nspace::NAME_KEY_PREFIX, max_read, &raw_data, &more);
    if (r < 0) {
      if (r != -ENOENT) {
        CLS_ERR(
            "failed to read the vals off of disk: %s", cpp_strerror(r).c_str());
 ...
 */
int
sparsify(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
  ...
  bool replace = true;
  while (offset < length) {
    if (calc_sparse_extent(
            ptr, sparse_size, length, &write_offset, &write_length, &offset)) {
      if (write_offset == 0 && write_length == length) {
        CLS_LOG(20, "nothing to do");
        return 0;
      }
      CLS_LOG(
          20, "write%s %" PRIu64 "~%" PRIu64, (replace ? "(replace)" : ""),
          write_offset, write_length);
      bufferlist write_bl;
      write_bl.push_back(
          ceph::buffer::ptr_node::create(ptr, write_offset, write_length));
      if (replace) {
...
CLS_INIT(rbd)
{
  ...
  cls_register_cxx_method(
      h_class, "create", CLS_METHOD_RD | CLS_METHOD_WR, create, &h_create);
  cls_register_cxx_method(
      h_class, "get_features", CLS_METHOD_RD, get_features, &h_get_features);
  cls_register_cxx_method(
      h_class, "set_features", CLS_METHOD_RD | CLS_METHOD_WR, set_features,
      &h_set_features);
  cls_register_cxx_method(
      h_class, "get_size", CLS_METHOD_RD, get_size, &h_get_size);
  cls_register_cxx_method(
      h_class, "set_size", CLS_METHOD_RD | CLS_METHOD_WR, set_size, &h_set_size);
  cls_register_cxx_method(
      h_class, "get_snapcontext", CLS_METHOD_RD, get_snapcontext,
      &h_get_snapcontext);
  cls_register_cxx_method(
      h_class, "get_object_prefix", CLS_METHOD_RD, get_object_prefix,
      &h_get_object_prefix);
  cls_register_cxx_method(
      h_class, "get_data_pool", CLS_METHOD_RD, get_data_pool, &h_get_data_pool);
  cls_register_cxx_method(
      h_class, "get_snapshot_name", CLS_METHOD_RD, get_snapshot_name,
      &h_get_snapshot_name);
  cls_register_cxx_method(
      h_class, "get_snapshot_timestamp", CLS_METHOD_RD, get_snapshot_timestamp,
      &h_get_snapshot_timestamp);
  cls_register_cxx_method(
      h_class, "snapshot_get", CLS_METHOD_RD, snapshot_get, &h_snapshot_get);
  cls_register_cxx_method(
      h_class, "snapshot_add", CLS_METHOD_RD | CLS_METHOD_WR, snapshot_add,
      &h_snapshot_add);
  cls_register_cxx_method(
      h_class, "snapshot_remove", CLS_METHOD_RD | CLS_METHOD_WR,
      snapshot_remove, &h_snapshot_remove);
  cls_register_cxx_method(
      h_class, "snapshot_rename", CLS_METHOD_RD | CLS_METHOD_WR,
      snapshot_rename, &h_snapshot_rename);
  cls_register_cxx_method(
      h_class, "snapshot_trash_add", CLS_METHOD_RD | CLS_METHOD_WR,
      snapshot_trash_add, &h_snapshot_trash_add);
  cls_register_cxx_method(
      h_class, "get_all_features", CLS_METHOD_RD, get_all_features,
      &h_get_all_features);

  // NOTE: deprecate v1 parent APIs after mimic EOLed
  cls_register_cxx_method(
      h_class, "get_parent", CLS_METHOD_RD, get_parent, &h_get_parent);
  cls_register_cxx_method(
      h_class, "set_parent", CLS_METHOD_RD | CLS_METHOD_WR, set_parent,
      &h_set_parent);
  cls_register_cxx_method(
      h_class, "remove_parent", CLS_METHOD_RD | CLS_METHOD_WR, remove_parent,
      &h_remove_parent);

  cls_register_cxx_method(
      h_class, "parent_get", CLS_METHOD_RD, parent_get, &h_parent_get);
  cls_register_cxx_method(
      h_class, "parent_overlap_get", CLS_METHOD_RD, parent_overlap_get,
      &h_parent_overlap_get);
  cls_register_cxx_method(
      h_class, "parent_attach", CLS_METHOD_RD | CLS_METHOD_WR, parent_attach,
      &h_parent_attach);
  cls_register_cxx_method(
      h_class, "parent_detach", CLS_METHOD_RD | CLS_METHOD_WR, parent_detach,
      &h_parent_detach);

  cls_register_cxx_method(
      h_class, "set_protection_status", CLS_METHOD_RD | CLS_METHOD_WR,
      set_protection_status, &h_set_protection_status);
  cls_register_cxx_method(
      h_class, "get_protection_status", CLS_METHOD_RD, get_protection_status,
      &h_get_protection_status);
  cls_register_cxx_method(
      h_class, "get_stripe_unit_count", CLS_METHOD_RD, get_stripe_unit_count,
      &h_get_stripe_unit_count);
  cls_register_cxx_method(
      h_class, "set_stripe_unit_count", CLS_METHOD_RD | CLS_METHOD_WR,
      set_stripe_unit_count, &h_set_stripe_unit_count);
  cls_register_cxx_method(
      h_class, "get_create_timestamp", CLS_METHOD_RD, get_create_timestamp,
      &h_get_create_timestamp);
  cls_register_cxx_method(
      h_class, "get_access_timestamp", CLS_METHOD_RD, get_access_timestamp,
      &h_get_access_timestamp);
  cls_register_cxx_method(
      h_class, "get_modify_timestamp", CLS_METHOD_RD, get_modify_timestamp,
      &h_get_modify_timestamp);
  cls_register_cxx_method(
      h_class, "get_flags", CLS_METHOD_RD, get_flags, &h_get_flags);
  cls_register_cxx_method(
      h_class, "set_flags", CLS_METHOD_RD | CLS_METHOD_WR, set_flags,
      &h_set_flags);
  cls_register_cxx_method(
      h_class, "op_features_get", CLS_METHOD_RD, op_features_get,
      &h_op_features_get);
  cls_register_cxx_method(
      h_class, "op_features_set", CLS_METHOD_RD | CLS_METHOD_WR,
      op_features_set, &h_op_features_set);
  cls_register_cxx_method(
      h_class, "metadata_list", CLS_METHOD_RD, metadata_list, &h_metadata_list);
  cls_register_cxx_method(
      h_class, "metadata_set", CLS_METHOD_RD | CLS_METHOD_WR, metadata_set,
      &h_metadata_set);
  cls_register_cxx_method(
      h_class, "metadata_remove", CLS_METHOD_RD | CLS_METHOD_WR,
      metadata_remove, &h_metadata_remove);
  cls_register_cxx_method(
      h_class, "metadata_get", CLS_METHOD_RD, metadata_get, &h_metadata_get);
  cls_register_cxx_method(
      h_class, "snapshot_get_limit", CLS_METHOD_RD, snapshot_get_limit,
      &h_snapshot_get_limit);
  cls_register_cxx_method(
      h_class, "snapshot_set_limit", CLS_METHOD_RD | CLS_METHOD_WR,
      snapshot_set_limit, &h_snapshot_set_limit);
  cls_register_cxx_method(
      h_class, "child_attach", CLS_METHOD_RD | CLS_METHOD_WR, child_attach,
      &h_child_attach);
  cls_register_cxx_method(
      h_class, "child_detach", CLS_METHOD_RD | CLS_METHOD_WR, child_detach,
      &h_child_detach);
  cls_register_cxx_method(
      h_class, "children_list", CLS_METHOD_RD, children_list, &h_children_list);
  cls_register_cxx_method(
      h_class, "migration_set", CLS_METHOD_RD | CLS_METHOD_WR, migration_set,
      &h_migration_set);
  cls_register_cxx_method(
      h_class, "migration_set_state", CLS_METHOD_RD | CLS_METHOD_WR,
      migration_set_state, &h_migration_set_state);
  cls_register_cxx_method(
      h_class, "migration_get", CLS_METHOD_RD, migration_get, &h_migration_get);
  cls_register_cxx_method(
      h_class, "migration_remove", CLS_METHOD_RD | CLS_METHOD_WR,
      migration_remove, &h_migration_remove);

  cls_register_cxx_method(
      h_class, "set_modify_timestamp", CLS_METHOD_RD | CLS_METHOD_WR,
      set_modify_timestamp, &h_set_modify_timestamp);

  cls_register_cxx_method(
      h_class, "set_access_timestamp", CLS_METHOD_RD | CLS_METHOD_WR,
      set_access_timestamp, &h_set_access_timestamp);

  /* methods for the rbd_children object */
  cls_register_cxx_method(
      h_class, "add_child", CLS_METHOD_RD | CLS_METHOD_WR, add_child,
      &h_add_child);
  cls_register_cxx_method(
      h_class, "remove_child", CLS_METHOD_RD | CLS_METHOD_WR, remove_child,
      &h_remove_child);
  cls_register_cxx_method(
      h_class, "get_children", CLS_METHOD_RD, get_children, &h_get_children);

  /* methods for the rbd_id.$image_name objects */
  cls_register_cxx_method(h_class, "get_id", CLS_METHOD_RD, get_id, &h_get_id);
  cls_register_cxx_method(
      h_class, "set_id", CLS_METHOD_RD | CLS_METHOD_WR, set_id, &h_set_id);

  /* methods for the rbd_directory object */
  cls_register_cxx_method(
      h_class, "dir_get_id", CLS_METHOD_RD, dir_get_id, &h_dir_get_id);
  cls_register_cxx_method(
      h_class, "dir_get_name", CLS_METHOD_RD, dir_get_name, &h_dir_get_name);
  cls_register_cxx_method(
      h_class, "dir_list", CLS_METHOD_RD, dir_list, &h_dir_list);
  cls_register_cxx_method(
      h_class, "dir_add_image", CLS_METHOD_RD | CLS_METHOD_WR, dir_add_image,
      &h_dir_add_image);
  cls_register_cxx_method(
      h_class, "dir_remove_image", CLS_METHOD_RD | CLS_METHOD_WR,
      dir_remove_image, &h_dir_remove_image);
  cls_register_cxx_method(
      h_class, "dir_rename_image", CLS_METHOD_RD | CLS_METHOD_WR,
      dir_rename_image, &h_dir_rename_image);
  cls_register_cxx_method(
      h_class, "dir_state_assert", CLS_METHOD_RD, dir_state_assert,
      &h_dir_state_assert);
  cls_register_cxx_method(
      h_class, "dir_state_set", CLS_METHOD_RD | CLS_METHOD_WR, dir_state_set,
      &h_dir_state_set);

  /* methods for the rbd_object_map.$image_id object */
  cls_register_cxx_method(
      h_class, "object_map_load", CLS_METHOD_RD, object_map_load,
      &h_object_map_load);
  cls_register_cxx_method(
      h_class, "object_map_save", CLS_METHOD_RD | CLS_METHOD_WR,
      object_map_save, &h_object_map_save);
  cls_register_cxx_method(
      h_class, "object_map_resize", CLS_METHOD_RD | CLS_METHOD_WR,
      object_map_resize, &h_object_map_resize);
  cls_register_cxx_method(
      h_class, "object_map_update", CLS_METHOD_RD | CLS_METHOD_WR,
      object_map_update, &h_object_map_update);
  cls_register_cxx_method(
      h_class, "object_map_snap_add", CLS_METHOD_RD | CLS_METHOD_WR,
      object_map_snap_add, &h_object_map_snap_add);
  cls_register_cxx_method(
      h_class, "object_map_snap_remove", CLS_METHOD_RD | CLS_METHOD_WR,
  ...
  cls_register_cxx_method(
      h_class, "snap_add", CLS_METHOD_RD | CLS_METHOD_WR, old_snapshot_add,
      &h_old_snapshot_add);
  cls_register_cxx_method(
      h_class, "snap_remove", CLS_METHOD_RD | CLS_METHOD_WR,
      old_snapshot_remove, &h_old_snapshot_remove);
  cls_register_cxx_method(
      h_class, "snap_rename", CLS_METHOD_RD | CLS_METHOD_WR,
      old_snapshot_rename, &h_old_snapshot_rename);

  /* methods for the rbd_mirroring object */
  cls_register_cxx_method(
      h_class, "mirror_uuid_get", CLS_METHOD_RD, mirror_uuid_get,
      &h_mirror_uuid_get);
  cls_register_cxx_method(
      h_class, "mirror_uuid_set", CLS_METHOD_RD | CLS_METHOD_WR,
      mirror_uuid_set, &h_mirror_uuid_set);
  cls_register_cxx_method(
      h_class, "mirror_mode_get", CLS_METHOD_RD, mirror_mode_get,
      &h_mirror_mode_get);
  cls_register_cxx_method(
      h_class, "mirror_mode_set", CLS_METHOD_RD | CLS_METHOD_WR,
      mirror_mode_set, &h_mirror_mode_set);
  cls_register_cxx_method(
      h_class, "mirror_remote_namespace_get", CLS_METHOD_RD,
      mirror_remote_namespace_get, &h_mirror_remote_namespace_get);
  cls_register_cxx_method(
      h_class, "mirror_remote_namespace_set", CLS_METHOD_RD | CLS_METHOD_WR,
      mirror_remote_namespace_set, &h_mirror_remote_namespace_set);
  cls_register_cxx_method(
      h_class, "mirror_peer_ping", CLS_METHOD_RD | CLS_METHOD_WR,
      mirror_peer_ping, &h_mirror_peer_ping);
  cls_register_cxx_method(
      h_class, "mirror_peer_list", CLS_METHOD_RD, mirror_peer_list,
      &h_mirror_peer_list);
  cls_register_cxx_method(
      h_class, "mirror_peer_add", CLS_METHOD_RD | CLS_METHOD_WR,
      mirror_peer_add, &h_mirror_peer_add);
  cls_register_cxx_method(
      h_class, "mirror_peer_remove", CLS_METHOD_RD | CLS_METHOD_WR,
      mirror_peer_remove, &h_mirror_peer_remove);
  cls_register_cxx_method(
      h_class, "mirror_peer_set_client", CLS_METHOD_RD | CLS_METHOD_WR,
      mirror_peer_set_client, &h_mirror_peer_set_client);
  cls_register_cxx_method(
      h_class, "mirror_peer_set_cluster", CLS_METHOD_RD | CLS_METHOD_WR,
      mirror_peer_set_cluster, &h_mirror_peer_set_cluster);
  cls_register_cxx_method(
      h_class, "mirror_peer_set_direction", CLS_METHOD_RD | CLS_METHOD_WR,
      mirror_peer_set_direction, &h_mirror_peer_set_direction);
  cls_register_cxx_method(
      h_class, "mirror_image_list", CLS_METHOD_RD, mirror_image_list,
      &h_mirror_image_list);
  cls_register_cxx_method(
      h_class, "mirror_image_get_image_id", CLS_METHOD_RD,
      mirror_image_get_image_id, &h_mirror_image_get_image_id);
  cls_register_cxx_method(
      h_class, "mirror_image_get", CLS_METHOD_RD, mirror_image_get,
      &h_mirror_image_get);
  cls_register_cxx_method(
      h_class, "mirror_image_set", CLS_METHOD_RD | CLS_METHOD_WR,
      mirror_image_set, &h_mirror_image_set);
  cls_register_cxx_method(
      h_class, "mirror_image_remove", CLS_METHOD_RD | CLS_METHOD_WR,
      mirror_image_remove, &h_mirror_image_remove);
  cls_register_cxx_method(
      h_class, "mirror_image_status_set",
      CLS_METHOD_RD | CLS_METHOD_WR | CLS_METHOD_PROMOTE,
      mirror_image_status_set, &h_mirror_image_status_set);
  cls_register_cxx_method(
      h_class, "mirror_image_status_remove", CLS_METHOD_RD | CLS_METHOD_WR,
      mirror_image_status_remove, &h_mirror_image_status_remove);
  cls_register_cxx_method(
      h_class, "mirror_image_status_get", CLS_METHOD_RD,
      mirror_image_status_get, &h_mirror_image_status_get);
  cls_register_cxx_method(
      h_class, "mirror_image_status_list", CLS_METHOD_RD,
      mirror_image_status_list, &h_mirror_image_status_list);
  cls_register_cxx_method(
      h_class, "mirror_image_status_get_summary", CLS_METHOD_RD,
      mirror_image_status_get_summary, &h_mirror_image_status_get_summary);
  cls_register_cxx_method(
      h_class, "mirror_image_status_remove_down", CLS_METHOD_RD | CLS_METHOD_WR,
      mirror_image_status_remove_down, &h_mirror_image_status_remove_down);
  cls_register_cxx_method(
      h_class, "mirror_image_instance_get", CLS_METHOD_RD,
      mirror_image_instance_get, &h_mirror_image_instance_get);
  cls_register_cxx_method(
      h_class, "mirror_image_instance_list", CLS_METHOD_RD,
      mirror_image_instance_list, &h_mirror_image_instance_list);
  cls_register_cxx_method(
      h_class, "mirror_instances_list", CLS_METHOD_RD, mirror_instances_list,
      &h_mirror_instances_list);
  cls_register_cxx_method(
      h_class, "mirror_instances_add",
      CLS_METHOD_RD | CLS_METHOD_WR | CLS_METHOD_PROMOTE, mirror_instances_add,
      &h_mirror_instances_add);
  cls_register_cxx_method(
      h_class, "mirror_instances_remove", CLS_METHOD_RD | CLS_METHOD_WR,
      mirror_instances_remove, &h_mirror_instances_remove);
  cls_register_cxx_method(
      h_class, "mirror_image_map_list", CLS_METHOD_RD, mirror_image_map_list,
      &h_mirror_image_map_list);
  cls_register_cxx_method(
      h_class, "mirror_image_map_update", CLS_METHOD_WR,
      mirror_image_map_update, &h_mirror_image_map_update);
  cls_register_cxx_method(
      h_class, "mirror_image_map_remove", CLS_METHOD_WR,
      mirror_image_map_remove, &h_mirror_image_map_remove);
  cls_register_cxx_method(
      h_class, "mirror_image_snapshot_unlink_peer",
      CLS_METHOD_RD | CLS_METHOD_WR, mirror_image_snapshot_unlink_peer,
      &h_mirror_image_snapshot_unlink_peer);
  cls_register_cxx_method(
      h_class, "mirror_image_snapshot_set_copy_progress",
      CLS_METHOD_RD | CLS_METHOD_WR, mirror_image_snapshot_set_copy_progress,
      &h_mirror_image_snapshot_set_copy_progress);

  /* methods for the groups feature */
  cls_register_cxx_method(
      h_class, "group_dir_list", CLS_METHOD_RD, group_dir_list,
      &h_group_dir_list);
  cls_register_cxx_method(
      h_class, "group_dir_add", CLS_METHOD_RD | CLS_METHOD_WR, group_dir_add,
      &h_group_dir_add);
  cls_register_cxx_method(
      h_class, "group_dir_remove", CLS_METHOD_RD | CLS_METHOD_WR,
      group_dir_remove, &h_group_dir_remove);
  cls_register_cxx_method(
      h_class, "group_dir_rename", CLS_METHOD_RD | CLS_METHOD_WR,
      group_dir_rename, &h_group_dir_rename);
  cls_register_cxx_method(
      h_class, "group_image_remove", CLS_METHOD_RD | CLS_METHOD_WR,
      group_image_remove, &h_group_image_remove);
  cls_register_cxx_method(
      h_class, "group_image_list", CLS_METHOD_RD, group_image_list,
      &h_group_image_list);
  cls_register_cxx_method(
      h_class, "group_image_set", CLS_METHOD_RD | CLS_METHOD_WR,
      group_image_set, &h_group_image_set);
  cls_register_cxx_method(
      h_class, "image_group_add", CLS_METHOD_RD | CLS_METHOD_WR,
      image_group_add, &h_image_group_add);
  cls_register_cxx_method(
      h_class, "image_group_remove", CLS_METHOD_RD | CLS_METHOD_WR,
      image_group_remove, &h_image_group_remove);
  cls_register_cxx_method(
      h_class, "image_group_get", CLS_METHOD_RD, image_group_get,
      &h_image_group_get);
  cls_register_cxx_method(
      h_class, "group_snap_set", CLS_METHOD_RD | CLS_METHOD_WR, group_snap_set,
      &h_group_snap_set);
  cls_register_cxx_method(
      h_class, "group_snap_remove", CLS_METHOD_RD | CLS_METHOD_WR,
      group_snap_remove, &h_group_snap_remove);
  cls_register_cxx_method(
      h_class, "group_snap_get_by_id", CLS_METHOD_RD, group_snap_get_by_id,
      &h_group_snap_get_by_id);
  cls_register_cxx_method(
      h_class, "group_snap_list", CLS_METHOD_RD, group_snap_list,
      &h_group_snap_list);
  cls_register_cxx_method(
      h_class, "group_snap_list_order", CLS_METHOD_RD, group_snap_list_order,
      &h_group_snap_list_order);

  /* rbd_trash object methods */
  cls_register_cxx_method(
      h_class, "trash_add", CLS_METHOD_RD | CLS_METHOD_WR, trash_add,
      &h_trash_add);
  cls_register_cxx_method(
      h_class, "trash_remove", CLS_METHOD_RD | CLS_METHOD_WR, trash_remove,
      &h_trash_remove);
  cls_register_cxx_method(
      h_class, "trash_list", CLS_METHOD_RD, trash_list, &h_trash_list);
  cls_register_cxx_method(
      h_class, "trash_get", CLS_METHOD_RD, trash_get, &h_trash_get);
  cls_register_cxx_method(
      h_class, "trash_state_set", CLS_METHOD_RD | CLS_METHOD_WR,
      trash_state_set, &h_trash_state_set);

  /* rbd_namespace object methods */
  cls_register_cxx_method(
      h_class, "namespace_add", CLS_METHOD_RD | CLS_METHOD_WR, namespace_add,
      &h_namespace_add);
  cls_register_cxx_method(
      h_class, "namespace_remove", CLS_METHOD_RD | CLS_METHOD_WR,
      namespace_remove, &h_namespace_remove);
  cls_register_cxx_method(
      h_class, "namespace_list", CLS_METHOD_RD, namespace_list,
      &h_namespace_list);

  /* data object methods */
  cls_register_cxx_method(
      h_class, "copyup", CLS_METHOD_RD | CLS_METHOD_WR, copyup, &h_copyup);
  cls_register_cxx_method(
      h_class, "sparse_copyup", CLS_METHOD_RD | CLS_METHOD_WR, sparse_copyup,
      &h_sparse_copyup);
  cls_register_cxx_method(
      h_class, "assert_snapc_seq", CLS_METHOD_RD | CLS_METHOD_WR,
      assert_snapc_seq, &h_assert_snapc_seq);
  cls_register_cxx_method(
      h_class, "sparsify", CLS_METHOD_RD | CLS_METHOD_WR, sparsify, &h_sparsify);
}
