...
class RGWGetDirHeader_CB
  : public boost::intrusive_ref_counter<RGWGetDirHeader_CB> {
public:
...
public:
  ...
  void
  add(int shard, const std::string& value)
  {
    value_by_shards[shard] = value;
  }

  const std::string&
  get(int shard, const std::string& default_value) const
  {
  ...
  void
  to_string(std::string* out) const
  {
    if (!out) {
      return;
    }
    out->clear();
    for (auto iter = value_by_shards.begin(); iter != value_by_shards.end();
         ++iter) {
      if (out->length()) {
  ...
  static bool
  is_shards_marker(const std::string& marker)
  {
   ...
   */
  int
  from_string(std::string_view composed_marker, int shard_id)
  {
  ...
  static std::string
  get_shard_marker(const std::string& marker)
  {
...
void cls_rgw_bucket_update_stats(
...
void cls_rgw_bucket_prepare_op(
    librados::ObjectWriteOperation& o,
    RGWModifyOp op,
...
void cls_rgw_bucket_complete_op(
    librados::ObjectWriteOperation& o,
    RGWModifyOp op,
    const std::string& tag,
...
void cls_rgw_remove_obj(
    librados::ObjectWriteOperation& o,
    std::list<std::string>& keep_attr_prefixes);
void cls_rgw_obj_store_pg_ver(
    librados::ObjectWriteOperation& o,
    const std::string& attr);
void cls_rgw_obj_check_attrs_prefix(
    librados::ObjectOperation& o,
    const std::string& prefix,
    bool fail_if_exist);
void cls_rgw_obj_check_mtime(
    librados::ObjectOperation& o,
...
void cls_rgw_bucket_link_olh(
    ...
    bool log_op,
    const rgw_zone_set& zones_trace);
void cls_rgw_bucket_unlink_instance(
    librados::ObjectWriteOperation& op,
    const cls_rgw_obj_key& key,
    const std::string& op_tag,
...
int cls_rgw_bucket_link_olh(
    ...
    bool log_op,
    const rgw_zone_set& zones_trace);
int cls_rgw_bucket_unlink_instance(
    librados::IoCtx& io_ctx,
    const std::string& oid,
    const cls_rgw_obj_key& key,
    const std::string& op_tag,
...
int cls_rgw_get_olh_log(
    ...
    const std::string& olh_tag,
    rgw_cls_read_olh_log_ret& log_ret);
int cls_rgw_clear_olh(
    librados::IoCtx& io_ctx,
    std::string& oid,
    const cls_rgw_obj_key& olh,
    const std::string& olh_tag);
int cls_rgw_usage_log_trim(
    librados::IoCtx& io_ctx,
    const std::string& oid,
...
void cls_rgw_bilog_trim(
    librados::ObjectWriteOperation& op,
    const std::string& start_marker,
    const std::string& end_marker);

void cls_rgw_bucket_check_index(
    librados::ObjectReadOperation& op,
    bufferlist& out);
// decode the response; may throw buffer::error
void cls_rgw_bucket_check_index_decode(
...
int cls_rgw_get_dir_header_async(
    librados::IoCtx& io_ctx,
    const std::string& oid,
    boost::intrusive_ptr<RGWGetDirHeader_CB> cb);

void cls_rgw_encode_suggestion(
    char op,
    rgw_bucket_dir_entry& dirent,
    ceph::buffer::list& updates);

void cls_rgw_suggest_changes(
    librados::ObjectWriteOperation& o,
    ceph::buffer::list& updates);
...
void cls_rgw_usage_log_add(
    librados::ObjectWriteOperation& op,
    rgw_usage_log_info& info);

/* garbage collection */
void cls_rgw_gc_set_entry(
    librados::ObjectWriteOperation& op,
    uint32_t expiration_secs,
    cls_rgw_gc_obj_info& info);
void cls_rgw_gc_defer_entry(
    librados::ObjectWriteOperation& op,
    uint32_t expiration_secs,
    const std::string& tag);
void cls_rgw_gc_remove(
    librados::ObjectWriteOperation& op,
    const std::vector<std::string>& tags);
void cls_rgw_gc_list(
    librados::ObjectReadOperation& op,
    const std::string& marker,
    uint32_t max,
    bool expired_only,
    bufferlist& bl);
int cls_rgw_gc_list_decode(
    const bufferlist& bl,
...
void cls_rgw_lc_put_head(
    librados::ObjectWriteOperation& op,
    const cls_rgw_lc_obj_head& head);
void cls_rgw_lc_get_next_entry(
    librados::ObjectReadOperation& op,
    const std::string& marker,
    bufferlist& bl);
int cls_rgw_lc_get_next_entry_decode(
    const bufferlist& bl,
    cls_rgw_lc_entry& entry);
void cls_rgw_lc_rm_entry(
    librados::ObjectWriteOperation& op,
    const cls_rgw_lc_entry& entry);
void cls_rgw_lc_set_entry(
    librados::ObjectWriteOperation& op,
    const cls_rgw_lc_entry& entry);
void cls_rgw_lc_get_entry(
    librados::ObjectReadOperation& op,
    const std::string& marker,
    bufferlist& bl);
int cls_rgw_lc_get_entry_decode(const bufferlist& bl, cls_rgw_lc_entry& entry);
void cls_rgw_lc_list(
    librados::ObjectReadOperation& op,
    const std::string& marker,
    uint32_t max_entries,
    bufferlist& bl);
int cls_rgw_lc_list_decode(
    const bufferlist& bl,
    std::vector<cls_rgw_lc_entry>& entries);

/* multipart */
void cls_rgw_mp_upload_part_info_update(
    librados::ObjectWriteOperation& op,
    const std::string& part_key,
    const RGWUploadPartInfo& info);

/* resharding */
void cls_rgw_reshard_add(
    librados::ObjectWriteOperation& op,
    const cls_rgw_reshard_entry& entry,
    const bool create_only);
void cls_rgw_reshard_remove(
    librados::ObjectWriteOperation& op,
    const cls_rgw_reshard_entry& entry);
// these overloads which call io_ctx.operate() should not be called in the rgw.
// rgw_rados_operate() should be called after the overloads w/o calls to io_ctx.operate()
#ifndef CLS_CLIENT_HIDE_IOCTX
int cls_rgw_reshard_list(
    librados::IoCtx& io_ctx,
    const std::string& oid,
    std::string& marker,
    uint32_t max,
