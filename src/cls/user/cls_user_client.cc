...
void
cls_user_complete_stats_sync(librados::ObjectWriteOperation& op)
{
  bufferlist in;
...
public:
  ClsUserListCtx(
      list<cls_user_bucket_entry>* _entries,
      string* _marker,
      bool* _truncated,
      int* _pret) :
...
public:
  ClsUserGetHeaderCtx(cls_user_header* _h, RGWGetUserHeader_CB* _ctx, int* _pret) :
    header(_h), ret_ctx(_ctx), pret(_pret)
  {}
    ...
    int* pret)
{
  ...
  op.exec(
      "user", "get_header", inbl, new ClsUserGetHeaderCtx(header, NULL, pret));
}

void
cls_user_reset_stats(librados::ObjectWriteOperation& op)
{
  bufferlist inbl;
...
int
cls_user_get_header_async(IoCtx& io_ctx, string& oid, RGWGetUserHeader_CB* ctx)
{
  bufferlist in, out;
  cls_user_get_header_op call;
  encode(call, in);
    ...
    int* pret)
{
  ...
  bufferlist inbl;
  encode(call, inbl);
  op.exec("user", "account_resource_get", inbl, new ResourceGetCB(&entry, pret));
}

void
cls_user_account_resource_rm(
    librados::ObjectWriteOperation& op,
    std::string_view name)
{
...
public:
  ResourceListCB(
      ...
      std::string* next_marker,
      int* pret) :
    entries(entries), truncated(truncated), next_marker(next_marker), pret(pret)
  {}
    ...
    int* pret)
{
  ...
  encode(call, inbl);
  op.exec(
      "user", "account_resource_list", inbl,
