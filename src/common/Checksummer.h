...
public:
  ...
  static const char*
  get_csum_type_string(unsigned t)
  {
    switch (t) {
    case CSUM_NONE:
      return "none";
  ...
  static int
  get_csum_string_type(const std::string& s)
  {
    if (s == "none")
      return CSUM_NONE;
    if (s == "xxhash32")
  ...
  static size_t
  get_csum_init_value_size(int csum_type)
  {
    switch (csum_type) {
    case CSUM_NONE:
      return 0;
  ...
  static size_t
  get_csum_value_size(int csum_type)
  {
    switch (csum_type) {
    case CSUM_NONE:
      return 0;
      ...
      ceph::buffer::ptr* csum_data)
  {
    ...
    ceph_assert(
        csum_data->length() >=
        (offset + length) / csum_block_size * sizeof(typename Alg::value_t));
