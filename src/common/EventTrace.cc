...
TracepointProvider::Traits event_tracepoint_traits(
    "libeventtrace_tp.so",
    "event_tracing");
bool EventTrace::tpinit = false;

void
EventTrace::init_tp(CephContext* _ctx)
{
  if (unlikely(!_ctx))
    return;
    ...
    bool incl_oid)
{
  ...
  if (m && (m->get_type() == CEPH_MSG_OSD_OP ||
    ...
    ostringstream buf;
    buf << m->get_source() << "!" << m->get_source_addr() << "!" << m->get_tid()
        << "!" << m->get_seq() << "!" << m->get_type();
    context = buf.str();
  }
}
...
EventTrace::EventTrace(
    ...
    const char* _func,
    int _line) :
  ctx(_ctx), file(_file), func(_func), line(_line)
{
  if (unlikely(!ctx))
    return;
  ...
  lsubdout(ctx, eventtrace, LOG_LEVEL)
      << "ENTRY (" << func << ") " << file << ":" << line << dendl;
...
EventTrace::~EventTrace()
{
  if (unlikely(!ctx))
    return;
  lsubdout(ctx, eventtrace, LOG_LEVEL)
      << "EXIT (" << func << ") " << file << dendl;
  tracepoint(eventtrace, func_exit, file.c_str(), func.c_str());
}

void
EventTrace::log_event_latency(const char* event)
{
    ...
    int line)
{
  ...
  init_tp(g_ceph_context);
  tracepoint(
      eventtrace, oid_elapsed, oid, event, context, elapsed, file, func, line);
}
    ...
    bool incl_oid)
{
  ...
  set_message_attrs(m, oid, context, incl_oid);
  trace_oid_elapsed(
      oid.c_str(), event, context.c_str(), elapsed, file, func, line);
}
