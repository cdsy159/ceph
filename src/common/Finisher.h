...
class Finisher {
  ...
  ceph::condition_variable
      finisher_cond; ///< Signaled when there is something to process.
  ceph::condition_variable
      finisher_empty_cond; ///< Signaled when the finisher has nothing more to process.
  bool finisher_stop = false; ///< Set when the finisher should stop.
  bool finisher_running =
      false; ///< True when the finisher is currently executing contexts.
  ...
  struct FinisherThread : public Thread {
    ...
    entry() override
    {
      return fin->finisher_thread_entry();
    }
  } finisher_thread;
...
public:
  ...
  template <typename T>
  auto
  queue(T& ls) -> decltype(std::distance(ls.begin(), ls.end()), void())
  {
    {
      const std::lock_guard l{finisher_lock};
      const bool should_notify = finisher_queue.empty() && !finisher_running;
  ...
  std::string_view
  get_thread_name() const noexcept
  {
    return thread_name;
  }
