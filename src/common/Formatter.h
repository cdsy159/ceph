...
public:
  ...
  void
  with_array_section(std::string_view txt, const M& m, FN&& fn)
  {
    Formatter::ArraySection as(*this, txt);
    for (const auto& [k, v] : m) {
      if constexpr (std::is_invocable_v<
                        FN, Formatter&, const K&, const V&, std::string_view>) {
  ...
  template <
    ...
    requires(
        ...
        (std::is_invocable_v<FN, Formatter&, const K&, const V&, std::string_view> ||
         std::is_invocable_v<FN, Formatter&, const K&, const V&>))
  void
  with_obj_array_section(std::string_view txt, const M& m, FN&& fn)
  {
    Formatter::ArraySection as(*this, txt);
    for (const auto& [k, v] : m) {
  ...
  static Formatter*
  create(std::string_view type, std::string_view default_type)
  {
  ...
  virtual void open_array_section_in_ns(
      std::string_view name,
      const char* ns) = 0;
  virtual void open_object_section(std::string_view name) = 0;
  virtual void open_object_section_in_ns(
      std::string_view name,
      const char* ns) = 0;
  ...
  virtual void
  dump_bool(std::string_view name, bool b)
  {
  ...
  template <typename T>
  void
  dump_object(std::string_view name, const T& foo)
  {
  ...
  virtual void
  dump_format_ns(std::string_view name, const char* ns, const char* fmt, ...);
  ...
  virtual void*
  get_external_feature_handler(const std::string& feature)
  {
    return nullptr;
  }
