...
void
Graylog::set_destination(const std::string& host, int port)
{
  try {
...
void
Graylog::set_hostname(const std::string& host)
{
  assert(!host.empty());
  m_hostname = host;
}
...
void
Graylog::set_fsid(const uuid_d& fsid)
{
...
void
Graylog::log_entry(const Entry& e)
{
  if (m_log_dst_valid) {
    auto s = e.strv();
    ...
    try {
      ...
      socket.send_to(
          boost::asio::buffer(m_ostream_compressed.str()), m_endpoint);
    } catch (boost::system::system_error const& e) {
...
void
Graylog::log_log_entry(LogEntry const* const e)
{
  if (m_log_dst_valid) {
    m_formatter->open_object_section("");
    m_formatter->dump_string("version", "1.1");
    ...
    m_formatter->dump_float(
        "timestamp", e->stamp.sec() + (e->stamp.usec() / 1000000.0));
    m_formatter->dump_string("_app", "ceph");
