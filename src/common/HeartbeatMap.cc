...
HeartbeatMap::HeartbeatMap(CephContext* cct) :
  m_cct(cct), m_unhealthy_workers(0), m_total_workers(0)
{}

HeartbeatMap::~HeartbeatMap() { ceph_assert(m_workers.empty()); }

heartbeat_handle_d*
HeartbeatMap::add_worker(string&& name, pthread_t thread_id)
{
  ...
  ANNOTATE_BENIGN_RACE_SIZED(
      &h->timeout, sizeof(h->timeout), "heartbeat_handle_d timeout");
  ANNOTATE_BENIGN_RACE_SIZED(
      &h->suicide_timeout, sizeof(h->suicide_timeout),
...
void
HeartbeatMap::remove_worker(const heartbeat_handle_d* h)
{
  std::unique_lock locker{m_rwlock};
    ...
    ceph::coarse_mono_time now)
{
  ...
  if (auto was = h->timeout.load(std::memory_order_relaxed);
    ...
    ldout(m_cct, 1) << who << " '" << h->name << "'" << " had timed out after "
                    << h->grace << dendl;
    healthy = false;
  }
  if (auto was = h->suicide_timeout.load(std::memory_order_relaxed);
    ...
    ldout(m_cct, 1) << who << " '" << h->name << "'"
                    << " had suicide timed out after " << h->suicide_grace
                    << dendl;
...
void
HeartbeatMap::clear_timeout(heartbeat_handle_d* h)
{
...
HeartbeatMap::is_healthy()
{
  ...
  if (m_cct->_conf->heartbeat_inject_failure) {
    ...
    m_inject_unhealthy_until =
        now + std::chrono::seconds(m_cct->_conf->heartbeat_inject_failure);
  ...
  if (now < m_inject_unhealthy_until) {
    auto sec = std::chrono::duration_cast<std::chrono::seconds>(
                   m_inject_unhealthy_until - now)
                   .count();
    ldout(m_cct, 0) << "is_healthy = false, injected failure for next " << sec
                    << " seconds" << dendl;
    healthy = false;
  }
