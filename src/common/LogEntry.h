...
public:
  ...
  friend bool
  operator==(const LogEntryKey& l, const LogEntryKey& r)
  {
...
namespace std {
template <>
struct hash<LogEntryKey> {
  size_t
  operator()(const LogEntryKey& r) const
  {
    return r.get_hash();
  }
};
...
struct LogEntry {
  ...
  LogEntry() :
    seq(0), prio(CLOG_DEBUG)
  {}

  LogEntryKey
  key() const
  {
    return LogEntryKey(rank, stamp, seq);
  }
...
struct LogSummary {
  ...
  std::map<std::string, std::pair<uint64_t, uint64_t>>
      channel_info; // channel -> [begin, end)
  ...
  bool
  contains(const LogEntryKey& k) const
  {
...
inline std::ostream&
operator<<(std::ostream& out, const clog_type t)
{
  switch (t) {
  case CLOG_DEBUG:
    return out << "[DBG]";
...
inline std::ostream&
operator<<(std::ostream& out, const LogEntry& e)
{
  return out << e.stamp << " " << e.name << " (" << e.rank << ") " << e.seq
...
struct fmt::formatter<EntityName> : fmt::formatter<std::string_view> {
  ...
  auto
  format(const EntityName& e, FormatContext& ctx) const
  {
...
template <>
struct fmt::formatter<LogEntry> : fmt::formatter<std::string_view> {
  template <typename FormatContext>
  auto
  format(const LogEntry& e, FormatContext& ctx) const
  {
    return fmt::format_to(
        ctx.out(), "{} {} ({}) {} : {} [{}] {}", e.stamp, e.name, e.rank, e.seq,
        e.channel, LogEntry::level_to_str(e.prio), e.msg);
  }
};
