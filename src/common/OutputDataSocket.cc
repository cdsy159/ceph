...
static void
remove_cleanup_file(const char* file)
{
...
static void
add_cleanup_file(const char* file)
{
...
OutputDataSocket::~OutputDataSocket() { shutdown(); }
...
std::string
OutputDataSocket::create_shutdown_pipe(int* pipe_rd, int* pipe_wr)
{
  int pipefd[2];
...
std::string
OutputDataSocket::bind_and_listen(const std::string& sock_path, int* fd)
{
  ...
  if (sock_fd < 0) {
    int err = errno;
    ostringstream oss;
    oss << "OutputDataSocket::bind_and_listen: " << "failed to create socket: "
        << cpp_strerror(err);
    return oss.str();
  }
...
OutputDataSocket::entry()
{
  ...
  while (true) {
    ...
    if (ret < 0) {
      ...
      lderr(m_cct) << "OutputDataSocket: poll(2) error: '" << cpp_strerror(err)
                   << dendl;
      return PFL_FAIL;
    }
...
OutputDataSocket::do_accept()
{
  ...
  if (connection_fd < 0) {
    int err = errno;
    lderr(m_cct) << "OutputDataSocket: do_accept error: '" << cpp_strerror(err)
                 << dendl;
    return false;
  }
...
void
OutputDataSocket::handle_connection(int fd)
{
  ceph::buffer::list bl;
...
int
OutputDataSocket::dump_data(int fd)
{
  m_lock.lock();
...
void
OutputDataSocket::close_connection(int fd)
{
  VOID_TEMP_FAILURE_RETRY(close(fd));
}

bool
OutputDataSocket::init(const std::string& path)
{
...
void
OutputDataSocket::append_output(ceph::buffer::list& bl)
{
