...
PluginRegistry::PluginRegistry(CephContext* cct) :
  cct(cct), loading(false), disable_dlclose(false)
{}
...
int
PluginRegistry::remove(const std::string& type, const std::string& name)
{
    ...
    Plugin* plugin)
{
  ...
  ldout(cct, 1) << __func__ << " " << type << " " << name << " " << plugin
                << dendl;
...
Plugin*
PluginRegistry::get_with_load(const std::string& type, const std::string& name)
{
...
Plugin*
PluginRegistry::get(const std::string& type, const std::string& name)
{
  ceph_assert(ceph_mutex_is_locked(lock));
  Plugin* ret = 0;
...
out:
  ldout(cct, 1) << __func__ << " " << type << " " << name << " = " << ret
                << dendl;
  return ret;
}

int
PluginRegistry::load(const std::string& type, const std::string& name)
{
  ceph_assert(ceph_mutex_is_locked(lock));
  ldout(cct, 1) << __func__ << " " << type << " " << name << dendl;

  std::string fname = cct->_conf.get_val<std::string>("plugin_dir") + "/" +
                      type + "/" + PLUGIN_PREFIX + name + PLUGIN_SUFFIX;
  void* library = dlopen(fname.c_str(), RTLD_NOW);
  if (!library) {
    string err1(dlerror());
    // fall back to plugin_dir
    fname = cct->_conf.get_val<std::string>("plugin_dir") + "/" +
            PLUGIN_PREFIX + name + PLUGIN_SUFFIX;
  ...
  if (code_version() != string(CEPH_GIT_NICE_VER)) {
    lderr(cct) << __func__ << " plugin " << fname << " version "
               << code_version() << " != expected " << CEPH_GIT_NICE_VER
               << dendl;
    dlclose(library);
    return -EXDEV;
  }
  ...
  int (*code_init)(
      CephContext*, const std::string& type, const std::string& name) =
      (int (*)(CephContext*, const std::string& type, const std::string& name))
          dlsym(library, PLUGIN_INIT_FUNCTION);
  if (code_init) {
  ...
  if (plugin == 0) {
    lderr(cct) << __func__ << " " << fname << " " << PLUGIN_INIT_FUNCTION
               << "() " << "did not register plugin type " << type << " name "
               << name << dendl;
    dlclose(library);
    return -EBADF;
  }
