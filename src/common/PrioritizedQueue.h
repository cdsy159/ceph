...
class PrioritizedQueue : public OpQueue<T, K> {
  ...
  public:
    ...
    void
    set_max_tokens(unsigned mt)
    {
      max_tokens = mt;
    }

    unsigned
    get_max_tokens() const
    {
      return max_tokens;
    }
    ...
    void
    enqueue(K cl, unsigned cost, T&& item)
    {
    ...
    void
    enqueue_front(K cl, unsigned cost, T&& item)
    {
    ...
    void
    remove_by_class(K k, std::list<T>* out)
    {
      ...
      if (out) {
        for (typename ListPairs::reverse_iterator j = i->second.rbegin();
             j != i->second.rend(); ++j) {
  ...
  void
  remove_queue(unsigned priority)
  {
  ...
  void
  distribute_tokens(unsigned cost)
  {
    if (total_priority == 0) {
      return;
    }
...
public:
  ...
  {
    unsigned total = 0;
    for (typename SubQueues::const_iterator i = queue.begin(); i != queue.end();
         ++i) {
  ...
  void
  remove_by_class(K k, std::list<T>* out = 0) final
  {
  ...
  void
  enqueue_strict(K cl, unsigned priority, T&& item) final
  {
    high_queue[priority].enqueue(cl, 0, std::move(item));
  }

  void
  enqueue_strict_front(K cl, unsigned priority, T&& item) final
  {
    high_queue[priority].enqueue_front(cl, 0, std::move(item));
  }

  void
  enqueue(K cl, unsigned priority, unsigned cost, T&& item) final
  {
    if (cost < min_cost)
      cost = min_cost;
  ...
  void
  enqueue_front(K cl, unsigned priority, unsigned cost, T&& item) final
  {
    if (cost < min_cost)
      cost = min_cost;
    if (cost > max_tokens_per_subqueue)
  ...
  dump(ceph::Formatter* f) const final
  {
    ...
    for (typename SubQueues::const_iterator p = queue.begin(); p != queue.end();
         ++p) {
      f->open_object_section("subqueue");
      f->dump_int("priority", p->first);
      p->second.dump(f);
