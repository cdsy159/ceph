...
Readahead::extent_t
Readahead::update(const vector<extent_t>& extents, uint64_t limit)
{
  m_lock.lock();
  for (vector<extent_t>::const_iterator p = extents.begin(); p != extents.end();
       ++p) {
...
Readahead::extent_t
Readahead::update(uint64_t offset, uint64_t length, uint64_t limit)
{
  m_lock.lock();
...
void
Readahead::_observe_read(uint64_t offset, uint64_t length)
{
...
Readahead::_compute_readahead(uint64_t limit)
{
  ...
  if (m_nr_consec_read >= m_trigger_requests) {
    ...
    if (m_last_pos >= m_readahead_trigger_pos) {
      ...
      for (vector<uint64_t>::iterator p = m_alignments.begin();
           p != m_alignments.end(); ++p) {
...
void
Readahead::inc_pending(int count)
{
  ceph_assert(count > 0);
  m_pending_lock.lock();
  m_pending += count;
...
void
Readahead::dec_pending(int count)
{
  ceph_assert(count > 0);
  m_pending_lock.lock();
...
void
Readahead::wait_for_pending(Context* ctx)
{
  m_pending_lock.lock();
  if (m_pending > 0) {
    m_pending_lock.unlock();
...
void
Readahead::set_trigger_requests(int trigger_requests)
{
  m_lock.lock();
...
uint64_t
Readahead::get_min_readahead_size(void)
{
...
uint64_t
Readahead::get_max_readahead_size(void)
{
  std::lock_guard lock(m_lock);
...
void
Readahead::set_min_readahead_size(uint64_t min_readahead_size)
{
  m_lock.lock();
...
void
Readahead::set_max_readahead_size(uint64_t max_readahead_size)
{
  m_lock.lock();
...
void
Readahead::set_alignments(const vector<uint64_t>& alignments)
{
  m_lock.lock();
