...
public:
  bool
  operator()(const S i, const T& k) const
  {
    return i < k.key;
  }

  bool
  operator()(const T& k, const S i) const
  {
    return k.key < i;
  }
};
...
private:
  ...
  public:
    ...
    friend bool
    operator<(const Klass& a, const Klass& b)
    {
      return a.key < b.key;
    }
    ...
    void
    insert(unsigned cost, T&& item, bool front)
    {
      if (front) {
  ...
  public:
    ...
    SubQueue(unsigned& p) :
      key(p), next(klasses.begin())
    {}

    ~SubQueue() { klasses.clear_and_dispose(DelItem<Klass>()); }

    friend bool
    operator<(const SubQueue& a, const SubQueue& b)
    {
      return a.key < b.key;
    }
    ...
    void
    insert(K cl, unsigned cost, T&& item, bool front = false)
    {
    ...
    void
    filter_class(K& cl, std::list<T>* out)
    {
  ...
  public:
    ...
    void
    insert(unsigned p, K cl, unsigned cost, T&& item, bool front = false)
    {
      typename SubQueues::insert_commit_data insert_data;
      std::pair<typename SubQueues::iterator, bool> ret =
          queues.insert_unique_check(
              p, MapKey<SubQueue, unsigned>(), insert_data);
      if (ret.second) {
    ...
    pop(bool strict = false)
    {
      ...
      if (queues.size() > 1) {
        while (true) {
          ...
          if (max_cost == 0 ||
              rand() % max_cost <= (max_cost - ((i->get_cost() * 9) / 10))) {
            break;
          }
          i = --queues.end();
        }
...
public:
  ...
  void
  remove_by_class(K cl, std::list<T>* removed = 0) final
  {
  ...
  void
  enqueue_strict(K cl, unsigned p, T&& item) final
  {
    strict.insert(p, cl, 0, std::move(item));
  }

  void
  enqueue_strict_front(K cl, unsigned p, T&& item) final
  {
    strict.insert(p, cl, 0, std::move(item), true);
  }

  void
  enqueue(K cl, unsigned p, unsigned cost, T&& item) final
  {
    normal.insert(p, cl, cost, std::move(item));
  }

  void
  enqueue_front(K cl, unsigned p, unsigned cost, T&& item) final
  {
