...
static void
remove_cleanup_file(std::string_view file)
{
...
static void
add_cleanup_file(std::string file)
{
  std::unique_lock l(cleanup_lock);
...
AdminSocket::~AdminSocket() { shutdown(); }
...
std::string
AdminSocket::create_wakeup_pipe(int* pipe_rd, int* pipe_wr)
{
  int pipefd[2];
#ifdef _WIN32
...
std::string
AdminSocket::bind_and_listen(const std::string& sock_path, int* fd)
{
  ...
  if (sock_path.size() > sizeof(address.sun_path) - 1) {
    ostringstream oss;
    oss << "AdminSocket::bind_and_listen: " << "The UNIX domain socket path "
        << sock_path << " is too long! The "
  ...
  if (sock_fd < 0) {
    ...
    ostringstream oss;
    oss << "AdminSocket::bind_and_listen: " << "failed to create socket: "
        << cpp_strerror(err);
    return oss.str();
  }
  ...
  if (::bind(sock_fd, (struct sockaddr*)&address, sizeof(struct sockaddr_un)) !=
      0) {
    int err = ceph_sock_errno();
    if (err == EADDRINUSE) {
  ...
  if (listen(sock_fd, 5) != 0) {
    int err = ceph_sock_errno();
    ostringstream oss;
    oss << "AdminSocket::bind_and_listen: " << "failed to listen to socket: "
        << cpp_strerror(err);
    close(sock_fd);
    retry_sys_call(::unlink, sock_path.c_str());
    return oss.str();
...
AdminSocket::entry() noexcept
{
  ...
  while (true) {
    ...
    if (ret < 0) {
      ...
      lderr(m_cct) << "AdminSocket: poll(2) error: '" << cpp_strerror(err)
                   << dendl;
      return;
    }
...
void
AdminSocket::chown(uid_t uid, gid_t gid)
{
  if (m_sock_fd >= 0) {
    int r = ::chown(m_path.c_str(), uid, gid);
    if (r < 0) {
      r = -errno;
      lderr(m_cct) << "AdminSocket: failed to chown socket: " << cpp_strerror(r)
                   << dendl;
    }
  }
}
...
void
AdminSocket::chmod(mode_t mode)
{
  if (m_sock_fd >= 0) {
    int r = ::chmod(m_path.c_str(), mode);
    if (r < 0) {
      r = -errno;
      lderr(m_cct) << "AdminSocket: failed to chmod socket: " << cpp_strerror(r)
                   << dendl;
    }
  }
}
...
AdminSocket::do_accept()
{
  ...
  int connection_fd =
      accept_cloexec(m_sock_fd, (struct sockaddr*)&address, &address_length);
  if (connection_fd < 0) {
    int err = ceph_sock_errno();
    lderr(m_cct) << "AdminSocket: do_accept error: '" << cpp_strerror(err)
                 << dendl;
    return;
  }
    ...
    asok_finisher on_finish)
{
  ...
  try {
    ...
    return on_finish(
        -EINVAL, "invalid json, missing format and/or prefix", empty);
  }

  ldout(m_cct, 20) << __func__ << ": format is " << format << " prefix is "
                   << prefix << dendl;
  ...
  if (output == ":tmp:") {
    ...
    if (int fd = mkstemp(path.data()); fd >= 0) {
      close(fd);
      output = path;
      ldout(m_cct, 20) << __func__ << ": output file created in tmp_dir is "
                       << output << dendl;
    } else {
      return on_finish(
          -errno, "temporary output file could not be opened", empty);
    }
  }
  ...
  if (!output.empty()) {
    ...
    ldout(m_cct, 10) << __func__ << ": opening file for json output: " << output
                     << dendl;
  ...
  switch (retval) {
  case ENOENT:
    lderr(m_cct) << "AdminSocket: request '" << cmdvec << "' not defined"
                 << dendl;
    delete f;
  ...
  hook->call_async(
      ...
      [f, output, on_finish,
       m_cct = m_cct](int r, std::string_view err, bufferlist& out) {
        // handle either existing output in bufferlist *or* via formatter
        ldout(m_cct, 10) << __func__ << ": command completed with result " << r
                         << dendl;
        if (auto* jff = dynamic_cast<JSONFormatterFile*>(f); jff != nullptr) {
          ...
          ldout(m_cct, 25) << __func__ << ": out is empty, dumping formatter"
                           << dendl;
          f->flush(out);
        }
        delete f;
...
AdminSocket::find_matched_hook(std::string& prefix, const cmdmap_t& cmdmap)
{
  std::unique_lock l(lock);
...
void
AdminSocket::queue_tell_command(cref_t<MCommand> m)
{
...
void
AdminSocket::queue_tell_command(cref_t<MMonCommand> m)
{
  ldout(m_cct, 10) << __func__ << " " << *m << dendl;
  std::lock_guard l(tell_lock);
    ...
    std::string_view help)
{
  ...
  if (i != hooks.cend() && i->second.desc == cmddesc) {
    ldout(m_cct, 5) << "register_command " << prefix << " cmddesc " << cmddesc
                    << " hook " << hook << " EEXIST" << dendl;
    ret = -EEXIST;
  } else {
    ldout(m_cct, 5) << "register_command " << prefix << " hook " << hook
                    << dendl;
    hooks.emplace_hint(
        i, std::piecewise_construct, std::forward_as_tuple(prefix),
...
void
AdminSocket::unregister_commands(const AdminSocketHook* hook)
{
  std::unique_lock l(lock);
  auto i = hooks.begin();
  while (i != hooks.end()) {
      ...
      bufferlist& out) override
  {
    ...
    for (const auto& [command, info] : m_as->hooks) {
      ...
      dump_cmd_and_help_to_json(
          f, CEPH_FEATURES_ALL, secname.str().c_str(), info.desc, info.help);
      cmdnum++;
    }
...
static const std::map<std::string, int> known_signals = {
    ...
    ADD_SIGNAL(SIGABRT), ADD_SIGNAL(SIGFPE),  ADD_SIGNAL(SIGILL),
    ADD_SIGNAL(SIGINT),  ADD_SIGNAL(SIGSEGV), ADD_SIGNAL(SIGTERM),
#ifndef WIN32
    ADD_SIGNAL(SIGTRAP), ADD_SIGNAL(SIGHUP),  ADD_SIGNAL(SIGBUS),
    ADD_SIGNAL(SIGQUIT), ADD_SIGNAL(SIGKILL), ADD_SIGNAL(SIGUSR1),
    ADD_SIGNAL(SIGUSR2), ADD_SIGNAL(SIGPIPE), ADD_SIGNAL(SIGALRM),
    ADD_SIGNAL(SIGCHLD), ADD_SIGNAL(SIGCONT), ADD_SIGNAL(SIGSTOP),
...
class RaiseHook : public AdminSocketHook {
  ...
  struct Killer {
    ...
    describe()
    {
      ...
      return fmt::format(
          "pending signal ({}) due in {}", strsignal_compat(signal),
    ...
    cancel()
    {
      ...
      if (0 == (status = waitpid(pid, &wstatus, WNOHANG))) {
        ...
        if (status) {
          ldout(m_cct, 5) << __func__ << "couldn't kill the killer. Error: "
      ...
      if (status < 0) {
        ldout(m_cct, 5) << __func__ << "waitpid(killer, NOHANG) returned "
                        << status << "; " << strerror(errno) << dendl;
      } else {
        ldout(m_cct, 20) << __func__ << "killer process " << pid << "\""
                         << describe() << "\" reaped. "
                         << "WIFEXITED: " << WIFEXITED(wstatus)
                         << "WIFSIGNALED: " << WIFSIGNALED(wstatus) << dendl;
      }
#endif
      return false;
    }

    static std::optional<Killer>
    fork(CephContext* m_cct, int signal_to_send, double delay)
    {
#ifndef WIN32
      pid_t victim = getpid();
      ...
      int fresult = ::fork();
      if (fresult < 0) {
        ldout(m_cct, 5) << __func__ << "couldn't fork the killer. Error: "
                        << strerror(errno) << dendl;
        return std::nullopt;
      }
      ...
      while (getppid() == victim) {
        ...
        if (remaining.count() > 0) {
          ...
          std::this_thread::sleep_for(
              duration_cast<nanoseconds>(std::min(remaining, poll_interval)));
        } else {
          break;
        }
      }
      ...
      if (getppid() != victim) {
        ...
        ldout(m_cct, 5) << __func__
                        << "my parent isn't what it used to be, i'm out"
                        << strerror(errno) << dendl;
        _exit(1);
      }
      ...
      if (0 != status) {
        ldout(m_cct, 5) << __func__
                        << "couldn't kill the victim: " << strerror(errno)
                        << dendl;
      }
      _exit(status);
#endif
  ...
  int
  parse_signal(std::string&& sigdesc, Formatter* f, std::ostream& errss)
  {
    int result = 0;
    std::transform(
        sigdesc.begin(), sigdesc.end(), sigdesc.begin(),
    ...
    if (sigdesc == "L") {
      ...
      try {
        ...
        if (result < 1 || result > 64) {
          errss << "signal number should be an integer in the range [1..64]"
                << std::endl;
          return -EINVAL;
        }
        ...
        if (sig_it == known_signals.end()) {
          errss << "unknown signal name; use -l to see recognized names"
                << std::endl;
          return -EINVAL;
        }
...
public:
  ...
  get_help()
  {
    return "deliver the <signal> to the daemon process, optionally delaying "
           "<after> seconds; "
           "when --after is used, the program will fork before sleeping, which "
           "allows to "
           "schedule signal delivery to a stopped daemon; it's possible to "
           "--cancel a pending signal delivery. "
           "<signal> can be in the forms '9', '-9', 'kill', '-KILL'. Use "
           "`raise -l` to list known signal names.";
  }
      ...
      bufferlist& out) override
  {
    ...
    if (cmd_getval(cmdmap, "after", delay)) {
      ...
      if (killer) {
        ...
        ldout(m_cct, 20) << __func__ << "scheduled " << killer->describe()
                         << dendl;
      } else {
      ...
      if (0 == status) {
        errss << "raised signal " << " (" << strsignal_compat(signal_to_send)
              << ")" << endl;
      } else {
...
AdminSocket::init(const std::string& path)
{
  ...
  register_command(
      "get_command_descriptions", getdescs_hook.get(),
      "list available commands");

  raise_hook = std::make_unique<RaiseHook>(m_cct);
  register_command(
      RaiseHook::get_cmddesc(), raise_hook.get(), RaiseHook::get_help());
