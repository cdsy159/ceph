...
get_rand_socket_path()
{
  ...
  if (g_socket_path == NULL) {
    ...
    snprintf(
        buf, sizeof(((struct sockaddr_un*)0)->sun_path),
        "%s/perfcounters_test_socket.%ld.%ld", tdir, (long int)getpid(),
        time(NULL));
...
static std::string
asok_connect(const std::string& path, int* fd)
{
  ...
  if (::connect(
          socket_fd, (struct sockaddr*)&address, sizeof(struct sockaddr_un)) !=
      0) {
    int err = ceph_sock_errno();
    ostringstream oss;
  ...
  timer.tv_usec = 0;
  if (::setsockopt(
          socket_fd, SOL_SOCKET, SO_RCVTIMEO, (SOCKOPT_VAL_TYPE)&timer,
          sizeof(timer))) {
    int err = ceph_sock_errno();
    ostringstream oss;
    oss << "setsockopt(" << socket_fd
        << ", SO_RCVTIMEO) failed: " << cpp_strerror(err);
    compat_closesocket(socket_fd);
    return oss.str();
  }
  timer.tv_sec = 10;
  timer.tv_usec = 0;
  if (::setsockopt(
          socket_fd, SOL_SOCKET, SO_SNDTIMEO, (SOCKOPT_VAL_TYPE)&timer,
          sizeof(timer))) {
    int err = ceph_sock_errno();
    ostringstream oss;
    oss << "setsockopt(" << socket_fd
        << ", SO_SNDTIMEO) failed: " << cpp_strerror(err);
    compat_closesocket(socket_fd);
    return oss.str();
  }
...
static std::string
asok_request(int socket_fd, std::string request)
{
  ssize_t res = safe_send(socket_fd, request.c_str(), request.length() + 1);
  if (res < 0) {
    int err = res;
    ostringstream oss;
    oss << "safe_write(" << socket_fd
        << ") failed to write request code: " << cpp_strerror(err);
    return oss.str();
  }
  return "";
}
...
std::string
AdminSocketClient::do_request(std::string request, std::string* result)
{
  int socket_fd = 0, res;
  std::string buffer;
  ...
  if (res < 0) {
    int e = res;
    ostringstream oss;
    oss << "safe_recv(" << socket_fd
        << ") failed to read message size: " << cpp_strerror(e);
    err = oss.str();
    goto done;
  }
