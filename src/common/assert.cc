...
/* If you register an assert context, ceph_assert() will try to lock the dout
   ...
   */
void
register_assert_context(CephContext* cct)
{
  ...
  if (!supressions.empty()) {
    lderr(g_assert_context)
        << "WARNING: supressions for ceph_assert present: " << supressions
        << dendl;
  }
}
    ...
    const char* func)
{
  ...
  snprintf(
      g_assert_msg, sizeof(g_assert_msg),
      "%s: In function '%s' thread %llx time %s\n"
      "%s: %d: FAILED ceph_assert(%s)\n",
      file, func, (unsigned long long)pthread_self(), tss.str().c_str(), file,
      line, assertion);
  ...
  if (should_abort) {
    ...
    dout_emergency(
        "WARNING: ceph_assert() does NOT abort() due "
        "to ceph_assert_supresssions");
  }
}

[[gnu::cold]] void
__ceph_assert_fail(const assert_data& ctx)
{
    ...
    ...)
{
  ...
  if (should_abort) {
    ...
    dout_emergency(
        "WARNING: ceph_assertf() does NOT abort() due to"
        "to ceph_assert_supresssions");
  }
}

[[gnu::cold]] void
__ceph_abort(const char* file, int line, const char* func, const std::string& msg)
{
  ostringstream tss;
  tss << ceph_clock_now();
...
[[gnu::cold]] void
__ceph_abortf(const char* file, int line, const char* func, const char* msg, ...)
{
  ostringstream tss;
  tss << ceph_clock_now();
    ...
    const char* func)
{
  char buf[8096];
  snprintf(
      buf, sizeof(buf), "WARNING: ceph_assert(%s) at: %s: %d: %s()\n",
