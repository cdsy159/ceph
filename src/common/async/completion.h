...
public:
  ...
  static std::unique_ptr<Completion> create(
      const Executor1& ex1,
      Handler&& handler,
      TArgs&&... args);
...
class CompletionImpl final : public Completion<void(Args...), T> {
  ...
  static void*
  operator new(size_t, RebindAlloc2 alloc2)
  {
  ...
  static void
  operator delete(void* p, RebindAlloc2 alloc2)
  {
    RebindTraits2::deallocate(alloc2, static_cast<CompletionImpl*>(p), 1);
  }

  static auto
  bind_and_forward(const Executor2& ex, Handler&& h, std::tuple<Args...>&& args)
  {
  ...
  void
  destroy_defer(std::tuple<Args...>&& args) override
  {
    auto w = std::move(work);
    auto ex2 = w.second.get_executor();
    RebindAlloc2 alloc2 =
        boost::asio::get_associated_allocator(handler, DefaultAlloc{});
  ...
  void
  destroy_dispatch(std::tuple<Args...>&& args) override
  {
    auto w = std::move(work);
    auto ex2 = w.second.get_executor();
    RebindAlloc2 alloc2 =
  ...
  void
  destroy_post(std::tuple<Args...>&& args) override
  {
    auto w = std::move(work);
    auto ex2 = w.second.get_executor();
    RebindAlloc2 alloc2 =
...
public:
  ...
  create(const Executor1& ex, Handler&& handler, TArgs&&... args)
  {
    ...
    return Ptr{new (alloc2) CompletionImpl(
        ex, std::move(handler), std::forward<TArgs>(args)...)};
  }
  ...
  static void
  operator delete(CompletionImpl* ptr, std::destroying_delete_t) noexcept
  {
    ptr->destroy();
  }
};
    ...
    TArgs&&... args)
{
  ...
  return Impl::create(
      ex, std::forward<Handler>(handler), std::forward<TArgs>(args)...);
}

template <typename T, typename... Args>
template <typename... Args2>
void
Completion<void(Args...), T>::defer(
    std::unique_ptr<Completion>&& ptr,
    Args2&&... args)
{
  auto c = ptr.release();
...
template <typename... Args2>
void
Completion<void(Args...), T>::dispatch(
    std::unique_ptr<Completion>&& ptr,
    Args2&&... args)
{
  auto c = ptr.release();
...
{
  return Completion<Signature, T>::create(
      ex, std::forward<Handler>(handler), std::forward<TArgs>(args)...);
}
...
void
defer(std::unique_ptr<Completion<Signature, T>>&& ptr, Args&&... args)
{
...
template <typename Signature, typename T, typename... Args>
void
dispatch(std::unique_ptr<Completion<Signature, T>>&& ptr, Args&&... args)
{
  Completion<Signature, T>::dispatch(
      std::move(ptr), std::forward<Args>(args)...);
}
...
template <typename Signature, typename T, typename... Args>
void
post(std::unique_ptr<Completion<Signature, T>>&& ptr, Args&&... args)
{
