...
public:
  ...
  io_context_pool(std::int64_t threadcnt) noexcept { start(threadcnt); }
  ...
  void
  start(std::int16_t threadcnt) noexcept
  {
    ...
    if (threadvec.empty()) {
      ...
      for (std::int16_t i = 0; i < threadcnt; ++i) {
        threadvec.emplace_back(make_named_thread("io_context_pool", [this] {
          ioctx.run();
        }));
      }
    }
  ...
  template <std::invocable<> Init>
  void
  start(std::int16_t threadcnt, Init&& init) noexcept
  {
    auto l = std::scoped_lock(m);
    if (threadvec.empty()) {
      guard.emplace(boost::asio::make_work_guard(ioctx));
      ...
      for (std::int16_t i = 0; i < threadcnt; ++i) {
        threadvec.emplace_back(make_named_thread(
            "io_context_pool", [this, init = std::move(init)] {
              std::move(init)();
              ioctx.run();
            }));
      }
