...
template <boost::asio::execution::executor Executor>
class spawn_group_impl : public boost::intrusive_ref_counter<
                             spawn_group_impl<Executor>,
                             boost::thread_unsafe_counter>,
                         public service_list_base_hook {
public:
  using size_type = uint16_t;

  spawn_group_impl(Executor ex, size_type limit, cancel_on_error on_error) :
  ...
  ~spawn_group_impl() { svc.remove(*this); }
  ...
  spawn_group_handler<executor_type>
  completion()
  {
    if (spawned >= limit) {
