...
template <typename CompletionToken, boost::asio::disposition Disposition>
[[nodiscard]] inline auto
redirect_error(CompletionToken&& token, Disposition& d)
{
  return redirect_error_t<
      std::decay_t<CompletionToken>, std::decay_t<Disposition>>{
...
public:
  ...
  redirect_error_handler(
      redirect_error_t<std::decay_t<CompletionToken>, std::decay_t<Disposition>>
          re) :
  ...
  redirect_error_handler(Disposition& disposition, RedirectedHandler&& handler) :
    disposition(disposition), handler(std::forward<RedirectedHandler>(handler))
  {}
  ...
  std::enable_if_t<!std::is_same_v<std::decay_t<Arg0>, Disposition>>
...
class initiation_base<Initiation, std::enable_if_t<!std::is_class_v<Initiation>>> {
public:
  template <typename I>
  explicit initiation_base(I&& initiation) :
    initiation(std::forward<I>(initiation))
...
template <
    ...
    typename Signature>
struct async_result<
    ::ceph::async::redirect_error_t<CompletionToken, Disposition>,
    Signature>
  : async_result<
        CompletionToken,
  ...
  struct init_wrapper : ::ceph::async::detail::initiation_base<Initiation> {
    ...
    void
    operator()(Handler&& handler, Disposition* d, Args&&... args) &&
    {
      static_cast<Initiation&&>(*this)(
          ::ceph::async::detail::redirect_error_handler<
              decay_t<Handler>, Disposition>(*d, std::forward<Handler>(handler)),
          std::forward<Args>(args)...);
    }

    template <typename Handler, typename... Args>
    void
    operator()(Handler&& handler, Disposition* d, Args&&... args) const&
    {
...
struct associator<
    Associator,
    ::ceph::async::detail::redirect_error_handler<Handler, Disposition>,
    DefaultCandidate> : Associator<Handler, DefaultCandidate> {
  static auto
  get(const ::ceph::async::detail::redirect_error_handler<Handler, Disposition>&
          h) noexcept
  {
...
struct async_result<
    ::ceph::async::partial_redirect_error<Disposition>,
    Signatures...> {
  template <typename Initiation, typename RawCompletionToken, typename... Args>
  static auto
  initiate(Initiation&& initiation, RawCompletionToken&& token, Args&&... args)
