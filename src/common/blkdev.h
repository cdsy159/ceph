...
extern std::string _decode_model_enc(
    const std::string& in); // helper, exported only so we can unit test
...
extern std::string get_device_path(
    const std::string& devname,
    std::string* err = 0);
...
extern int block_device_get_metrics(
    const std::string& devname,
    int timeout,
