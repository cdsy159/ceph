...
void
buffer::track_cached_crc(bool b)
{
  buffer_track_crc = b;
}
...
public:
  ...
  alloc_data_n_controlblock(unsigned len, unsigned align)
  {
    // posix_memalign() requires a multiple of sizeof(void *)
    align = std::max<unsigned>(align, sizeof(void*));
    size_t rawlen = round_up_to(
        sizeof(buffer::raw_combined), alignof(buffer::raw_combined));
  ...
  create(unsigned len, unsigned align, int mempool = mempool::mempool_buffer_anon)
  {
  ...
  static void
  operator delete(raw_combined* raw, std::destroying_delete_t)
  {
...
public:
    ...
    raw(l)
  {
    ...
    bdout << "raw_malloc " << this << " alloc " << (void*)data << " " << l
          << bendl;
  }
...
public:
  ...
  explicit raw_claimed_char(unsigned l, char* b) :
    raw(b, l)
  {
    bdout << "raw_claimed_char " << this << " alloc " << (void*)data << " " << l
          << bendl;
  }

  ~raw_claimed_char() override
  {
    bdout << "raw_claimed_char " << this << " free " << (void*)data << bendl;
  }
};
...
buffer::copy(const char* c, unsigned len)
{
...
ceph::unique_leakable_ptr<buffer::raw>
buffer::create(unsigned len, char c)
{
...
ceph::unique_leakable_ptr<buffer::raw>
buffer::create_aligned_in_mempool(unsigned len, unsigned align, int mempool)
{
...
ceph::unique_leakable_ptr<buffer::raw>
buffer::create_aligned(unsigned len, unsigned align)
{
...
buffer::ptr::ptr(ceph::unique_leakable_ptr<raw> r) :
  _raw(r.release()), _off(0), _len(_raw->get_len())
{
...
buffer::ptr::ptr(const ptr& p, ceph::unique_leakable_ptr<raw> r) :
  _raw(r.release()), _off(p._off), _len(p._len)
{
  _raw->nref.store(1, std::memory_order_release);
  bdout << "ptr " << this << " get " << _raw << bendl;
}
...
buffer::ptr&
buffer::ptr::operator=(const ptr& p)
{
  if (p._raw) {
    p._raw->nref++;
    bdout << "ptr " << this << " get " << _raw << bendl;
...
buffer::ptr&
buffer::ptr::operator=(ptr&& p) noexcept
{
  release();
  buffer::raw* raw = p._raw;
  if (raw) {
...
void
buffer::ptr::swap(ptr& other) noexcept
{
  raw* r = _raw;
  unsigned o = _off;
  unsigned l = _len;
...
buffer::ptr::release()
{
  ...
  if (auto* const cached_raw = std::exchange(_raw, nullptr); cached_raw) {
    ...
    if (likely(last_one) || --cached_raw->nref == 0) {
      bdout << "deleting raw " << static_cast<void*>(cached_raw) << " len "
            << cached_raw->get_len() << bendl;
...
void
buffer::ptr::reassign_to_mempool(int pool)
{
  if (_raw) {
...
void
buffer::ptr::try_assign_to_mempool(int pool)
{
  if (_raw) {
...
unsigned
buffer::ptr::unused_tail_length() const
{
  return _raw ? _raw->get_len() - (_off + _len) : 0;
}

const char&
buffer::ptr::operator[](unsigned n) const
{
  ceph_assert(_raw);
  ceph_assert(n < _len);
...
char&
buffer::ptr::operator[](unsigned n)
{
  ceph_assert(_raw);
  ceph_assert(n < _len);
  return _raw->get_data()[_off + n];
...
void
buffer::ptr::copy_out(unsigned o, unsigned l, char* dest) const
{
  ceph_assert(_raw);
  if (o + l > _len)
    throw end_of_buffer();
...
unsigned
buffer::ptr::wasted() const
{
  return _raw->get_len() - _len;
}

int
buffer::ptr::cmp(const ptr& o) const
{
...
void
buffer::ptr::copy_in(unsigned o, unsigned l, const char* src, bool crc_reset)
{
  ceph_assert(_raw);
  ceph_assert(o <= _len);
...
void
buffer::ptr::zero(bool crc_reset)
{
  if (crc_reset)
    _raw->invalidate_crc();
  // FIPS zeroization audit 20191115: this memset is not security related.
...
void
buffer::ptr::zero(unsigned o, unsigned l, bool crc_reset)
{
  ceph_assert(o + l <= _len);
  if (crc_reset)
    _raw->invalidate_crc();
...
template <bool B>
buffer::ptr::iterator_impl<B>&
buffer::ptr::iterator_impl<B>::operator+=(size_t len)
{
  pos += len;
  if (pos > end_ptr)
    throw end_of_buffer();
...
buffer::list::iterator_impl<is_const>::iterator_impl(
    const buffer::list::iterator& i) :
  iterator_impl<is_const>(i.bl, i.off, i.p, i.p_off)
{}

template <bool is_const>
auto
buffer::list::iterator_impl<is_const>::operator+=(unsigned o) -> iterator_impl&
{
...
template <bool is_const>
void
buffer::list::iterator_impl<is_const>::seek(unsigned o)
{
  p = ls->begin();
  off = p_off = 0;
  *this += o;
...
template <bool is_const>
char
buffer::list::iterator_impl<is_const>::operator*() const
{
  if (p == ls->end())
    throw end_of_buffer();
  return (*p)[p_off];
...
template <bool is_const>
buffer::ptr
buffer::list::iterator_impl<is_const>::get_current_ptr() const
{
  if (p == ls->end())
    throw end_of_buffer();
...
template <bool is_const>
bool
buffer::list::iterator_impl<is_const>::is_pointing_same_raw(
...
template <bool is_const>
void
buffer::list::iterator_impl<is_const>::copy(unsigned len, char* dest)
{
  if (p == ls->end())
    seek(off);
  while (len > 0) {
...
template <bool is_const>
void
buffer::list::iterator_impl<is_const>::copy(unsigned len, ptr& dest)
{
  copy_deep(len, dest);
}
...
template <bool is_const>
void
buffer::list::iterator_impl<is_const>::copy_deep(unsigned len, ptr& dest)
{
  if (!len) {
    return;
  }
...
template <bool is_const>
void
buffer::list::iterator_impl<is_const>::copy_shallow(unsigned len, ptr& dest)
{
  if (!len) {
    return;
  }
...
template <bool is_const>
void
buffer::list::iterator_impl<is_const>::copy(unsigned len, list& dest)
{
  if (p == ls->end())
    seek(off);
  while (len > 0) {
...
template <bool is_const>
void
buffer::list::iterator_impl<is_const>::copy(unsigned len, std::string& dest)
{
  if (p == ls->end())
    seek(off);
  while (len > 0) {
...
template <bool is_const>
void
buffer::list::iterator_impl<is_const>::copy_all(list& dest)
{
  if (p == ls->end())
    seek(off);
  while (1) {
...
template <bool is_const>
size_t
buffer::list::iterator_impl<is_const>::get_ptr_and_advance(
    size_t want,
    const char** data)
{
  if (p == ls->end()) {
...
template <bool is_const>
uint32_t
buffer::list::iterator_impl<is_const>::crc32c(size_t length, uint32_t crc)
{
...
// copy data in
void
buffer::list::iterator::copy_in(unsigned len, const char* src, bool crc_reset)
{
  // copy
  if (p == ls->end())
    seek(off);
...
void
buffer::list::iterator::copy_in(unsigned len, const list& otherl)
{
  if (p == ls->end())
    seek(off);
  unsigned left = len;
...
void
buffer::list::swap(list& other) noexcept
{
...
bool
buffer::list::contents_equal(const ceph::buffer::list& other) const
{
...
bool
buffer::list::contents_equal(const void* const other, size_t length) const
{
...
bool
buffer::list::is_provided_buffer(const char* const dst) const
{
  if (_buffers.empty()) {
    return false;
  }
...
bool
buffer::list::is_aligned(const unsigned align) const
{
...
bool
buffer::list::is_n_align_sized(const unsigned align) const
{
  for (const auto& node : _buffers) {
...
bool
buffer::list::is_aligned_size_and_memory(
...
void
buffer::list::zero(const unsigned o, const unsigned l)
{
  ceph_assert(o + l <= _len);
  unsigned p = 0;
  for (auto& node : _buffers) {
...
bool
buffer::list::is_contiguous() const
{
  return _num <= 1;
}

bool
buffer::list::is_n_page_sized() const
{
  return is_n_align_sized(CEPH_PAGE_SIZE);
}

bool
buffer::list::is_page_aligned() const
{
  return is_aligned(CEPH_PAGE_SIZE);
}

int
buffer::list::get_mempool() const
{
  if (_buffers.empty()) {
    return mempool::mempool_buffer_anon;
  }
...
void
buffer::list::reassign_to_mempool(int pool)
{
  for (auto& p : _buffers) {
...
void
buffer::list::try_assign_to_mempool(int pool)
{
  for (auto& p : _buffers) {
...
uint64_t
buffer::list::get_wasted_space() const
{
  if (_num == 1)
...
bool
buffer::list::rebuild_aligned(unsigned align)
{
  return rebuild_aligned_size_and_memory(align, align);
}

bool
buffer::list::rebuild_aligned_size_and_memory(
    unsigned align_size,
    ...
    unsigned max_buffers)
{
  ...
  while (p != std::end(_buffers)) {
    ...
    if (!(unaligned.is_contiguous() &&
      ...
      unaligned.rebuild(ptr_node::create(
          buffer::create_aligned(unaligned._len, align_memory)));
      had_to_rebuild = true;
    }
    if (unaligned.get_num_buffers()) {
      _buffers.insert_after(
          p_prev, *ptr_node::create(unaligned._buffers.front()).release());
      _num += 1;
    } else {
...
void
buffer::list::reserve(size_t prealloc)
{
...
void
buffer::list::claim_append(list& bl)
{
  // check overflow
...
buffer::list::refill_append_space(const unsigned len)
{
  ...
  if (_carriage == &_buffers.back()) {
    size_t nlen = round_up_to(_carriage->raw_length(), CEPH_BUFFER_ALLOC_UNIT) *
                  2;
...
buffer::ptr
buffer::list::always_zeroed_bptr()
{
...
void
buffer::list::append(const char* data, unsigned len)
{
  _len += len;
...
buffer::list::reserve_t
buffer::list::obtain_contiguous_space(const unsigned len)
{
...
void
buffer::list::append(const ptr& bp)
{
  push_back(bp);
}

void
buffer::list::append(ptr&& bp)
{
  push_back(std::move(bp));
}

void
buffer::list::append(const ptr& bp, unsigned off, unsigned len)
{
...
void
buffer::list::append(const list& bl)
{
  _len += bl._len;
  _num += bl._num;
...
void
buffer::list::append(std::istream& in)
{
  while (!in.eof()) {
    std::string s;
    getline(in, s);
...
void
buffer::list::prepend_zero(unsigned len)
{
...
void
buffer::list::append_zero2(unsigned len)
{
  _len += len;
  while (len > 0) {
...
void
buffer::list::append_zero(unsigned len)
{
  _len += len;
...
/*
   ...
   */
const char&
buffer::list::operator[](unsigned n) const
{
  if (n >= _len)
    throw end_of_buffer();
...
void
buffer::list::substr_of(const list& other, unsigned off, unsigned len)
{
...
buffer::list::splice(
    unsigned off,
    unsigned len,
    list* claim_by /*, bufferlist& replace_with */)
{ // fixme?
  if (len == 0)
    return;
  ...
  if (off) {
    ...
    _buffers.insert_after(
        curbuf_prev, *ptr_node::create(*curbuf, 0, off).release());
    _len += off;
    _num += 1;
    ++curbuf_prev;
  }
...
void
buffer::list::write(int off, int len, std::ostream& out) const
{
  list s;
...
void
buffer::list::encode_base64(buffer::list& o)
{
  bufferptr bp(length() * 4 / 3 + 3);
  int l = ceph_armor(
      bp.c_str(), bp.c_str() + bp.length(), c_str(), c_str() + length());
  bp.set_length(l);
  o.push_back(std::move(bp));
}

void
buffer::list::decode_base64(buffer::list& e)
{
  bufferptr bp(4 + ((e.length() * 3) / 4));
  int l = ceph_unarmor(
      bp.c_str(), bp.c_str() + bp.length(), e.c_str(), e.c_str() + e.length());
  if (l < 0) {
...
int
buffer::list::read_file(const char* fn, std::string* error)
{
  int fd = TEMP_FAILURE_RETRY(::open(fn, O_RDONLY | O_CLOEXEC | O_BINARY));
  if (fd < 0) {
    int err = errno;
...
ssize_t
buffer::list::read_fd(int fd, size_t len)
{
...
ssize_t
buffer::list::recv_fd(int fd, size_t len)
{
  auto bp = ptr_node::create(buffer::create(len));
...
int
buffer::list::write_file(const char* fn, int mode)
{
  int fd = TEMP_FAILURE_RETRY(
      ::open(fn, O_WRONLY | O_CREAT | O_TRUNC | O_CLOEXEC | O_BINARY, mode));
  if (fd < 0) {
    int err = errno;
    cerr << "bufferlist::write_file(" << fn
         << "): failed to open file: " << cpp_strerror(err) << std::endl;
    return -err;
  }
  int ret = write_fd(fd);
  if (ret) {
    cerr << "bufferlist::write_fd(" << fn
...
#ifndef _WIN32
int
buffer::list::write_fd(int fd) const
{
  // use writev!
  iovec iov[IOV_MAX];
  int iovlen = 0;
...
int
buffer::list::send_fd(int fd) const
{
  return buffer::list::write_fd(fd);
}

int
buffer::list::write_fd(int fd, uint64_t offset) const
{
  iovec iov[IOV_MAX];
...
__u32
buffer::list::crc32c(__u32 crc) const
{
  int cache_misses = 0;
  int cache_hits = 0;
  int cache_adjusts = 0;
  ...
  for (const auto& node : _buffers) {
    if (node.length()) {
	   ...
	   */
          crc = ccrc.second ^
                ceph_crc32c(ccrc.first ^ crc, NULL, node.length());
          cache_adjusts++;
        }
      } else {
        cache_misses++;
 ...
 */
void
buffer::list::write_stream(std::ostream& out) const
{
  for (const auto& node : _buffers) {
    if (node.length() > 0) {
      out.write(node.c_str(), node.length());
...
void
buffer::list::hexdump(std::ostream& out, bool trailing_newline) const
{
  if (!length())
    return;
...
buffer::list
buffer::list::static_from_mem(char* c, size_t l)
{
  list bl;
...
buffer::list
buffer::list::static_from_string(string& s)
{
...
bool
buffer::ptr_node::dispose_if_hypercombined(buffer::ptr_node* const delete_this)
{
  ...
  const bool is_hypercombined = reinterpret_cast<std::uintptr_t>(delete_this) ==
                                bptr;
  if (is_hypercombined) {
...
buffer::ptr_node*
buffer::ptr_node::cloner::operator()(const buffer::ptr_node& clone_this)
{
  return new ptr_node(clone_this);
}

std::ostream&
buffer::operator<<(std::ostream& out, const buffer::raw& r)
{
  return out << "buffer::raw(" << (void*)r.get_data() << " len " << r.get_len()
             << " nref " << r.nref.load() << ")";
}

std::ostream&
buffer::operator<<(std::ostream& out, const buffer::ptr& bp)
{
  if (bp.have_raw())
    out << "buffer::ptr(" << bp.offset() << "~" << bp.length() << " "
        << (void*)bp.c_str() << " in raw " << (void*)bp.raw_c_str() << " len "
        << bp.raw_length() << " nref " << bp.raw_nref() << ")";
  else
...
std::ostream&
buffer::operator<<(std::ostream& out, const buffer::list& bl)
{
...
MEMPOOL_DEFINE_OBJECT_FACTORY(
    ...
    buffer_raw_posix_aligned,
    buffer_meta);
MEMPOOL_DEFINE_OBJECT_FACTORY(
    buffer::raw_claimed_char,
    buffer_raw_claimed_char,
    buffer_meta);
MEMPOOL_DEFINE_OBJECT_FACTORY(buffer::raw_static, buffer_raw_static, buffer_meta);

void
ceph::buffer::list::page_aligned_appender::_refill(size_t len)
{
  const unsigned alloc = std::max(
      min_alloc,
      shift_round_up(
          static_cast<unsigned>(len), static_cast<unsigned>(CEPH_PAGE_SHIFT)));
...
const char*
buffer_error_category::name() const noexcept
{
  return "buffer";
}
...
std::string
buffer_error_category::message(int ev) const
{
...
bool
buffer_error_category::equivalent(
    int ev,
...
int
buffer_error_category::from_code(int ev) const noexcept
{
  using ceph::buffer::errc;
  switch (static_cast<errc>(ev)) {
  case errc::bad_alloc:
