...
void
string_to_vec(std::vector<std::string>& args, std::string argstr)
{
...
void
env_to_vec(std::vector<const char*>& args, const char* name)
{
  if (!name)
    name = "CEPH_ARGS";
...
argv_to_vec(int argc, const char* const* argv)
{
  assert(argc > 0);
...
void
ceph_arg_value_type(const char* nextargstr, bool* bool_option, bool* bool_numeric)
{
...
bool
parse_ip_port_vec(const char* s, std::vector<entity_addrvec_t>& vec, int type)
{
...
static bool
check_bool_str(const char* val, int* ret)
{
...
template bool ceph_argparse_witharg<int>(
    std::vector<const char*>& args,
    std::vector<const char*>::iterator& i,
    int* ret,
    std::ostream& oss,
...
template bool ceph_argparse_witharg<long long>(
    std::vector<const char*>& args,
    std::vector<const char*>::iterator& i,
    long long* ret,
    std::ostream& oss,
...
template bool ceph_argparse_witharg<float>(
    std::vector<const char*>& args,
    std::vector<const char*>::iterator& i,
    float* ret,
    std::ostream& oss,
  ...
  for (std::vector<const char*>::iterator i = args.begin(); i != args.end();) {
    if (strcmp(*i, "--") == 0) {
      ...
      _exit(0);
        ...
        ceph_argparse_witharg(args, i, &val, "--conf", "-c", (char*)NULL)) {
      *conf_file_list = val;
      ...
      *cluster = val;
        ...
        (module_type != CEPH_ENTITY_TYPE_CLIENT) &&
        (ceph_argparse_witharg(args, i, &val, "-i", (char*)NULL))) {
      iparams.name.set_id(val);
    } else if (
        ceph_argparse_witharg(args, i, &val, "--id", "--user", (char*)NULL)) {
      iparams.name.set_id(val);
    } else if (
        ceph_argparse_witharg(args, i, &val, "--name", "-n", (char*)NULL)) {
      if (!iparams.name.from_str(val)) {
        std::cerr << "error parsing '" << val
      ...
      std::cout << "args: ";
      for (std::vector<const char*>::iterator ci = orig_args.begin();
           ci != orig_args.end(); ++ci) {
...
static void
generic_usage(bool is_server)
{
  std::cout
      << "  --conf/-c FILE    read configuration from the given configuration "
         "file"
      << std::endl
...
bool
ceph_argparse_need_usage(const std::vector<const char*>& args)
{
  if (args.empty()) {
    return true;
  }
