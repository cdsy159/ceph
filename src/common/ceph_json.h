...
public:
  ...
  void set(
      ...
      const JSONObjIter::map_iter_t& _end);
...
inline std::ostream&
operator<<(std::ostream& out, const JSONObj::data_val& dv)
{
...
public:
  ...
  template <class T>
  static bool
  decode_json(const char* name, T& val, JSONObj* obj, bool mandatory = false);

  template <class C>
  static bool decode_json(
      const char* name,
      C& container,
      void (*cb)(C&, JSONObj* obj),
      JSONObj* obj,
      bool mandatory = false);

  template <class T>
  static void
  decode_json(const char* name, T& val, const T& default_val, JSONObj* obj);
...
template <class T>
void
decode_json_obj(T& val, JSONObj* obj)
{
  val.decode_json(obj);
}

inline void
decode_json_obj(std::string& val, JSONObj* obj)
{
  val = obj->get_data();
}

static inline void
decode_json_obj(JSONObj::data_val& val, JSONObj* obj)
{
...
template <class T>
void
decode_json_obj(std::list<T>& l, JSONObj* obj)
{
  l.clear();
...
template <class T>
void
decode_json_obj(std::deque<T>& l, JSONObj* obj)
{
  l.clear();
...
template <class T>
void
decode_json_obj(std::set<T>& l, JSONObj* obj)
{
  l.clear();
...
void
decode_json_obj(boost::container::flat_set<T, Compare, Alloc>& l, JSONObj* obj)
{
  l.clear();
...
template <class T>
void
decode_json_obj(std::vector<T>& l, JSONObj* obj)
{
  l.clear();
...
void
decode_json_obj(std::map<K, V, C>& m, JSONObj* obj)
{
  m.clear();
...
template <class K, class V, class C = std::less<K>>
void
decode_json_obj(boost::container::flat_map<K, V, C>& m, JSONObj* obj)
{
  m.clear();
...
void
decode_json_obj(std::multimap<K, V>& m, JSONObj* obj)
{
  m.clear();
...
template <class K, class V>
void
decode_json_obj(boost::container::flat_map<K, V>& m, JSONObj* obj)
{
  m.clear();
...
template <class C>
void
decode_json_obj(C& container, void (*cb)(C&, JSONObj* obj), JSONObj* obj)
{
  container.clear();
...
template <class T>
bool
JSONDecoder::decode_json(const char* name, T& val, JSONObj* obj, bool mandatory)
{
...
public:
  ...
  public:
    ...
    virtual void encode_json(
        const char* name,
        const void* pval,
        ceph::Formatter*) const = 0;
  };
...
public:
  ...
  template <class T>
  bool
  encode_json(const char* name, const T& val, ceph::Formatter* f)
  {
...
template <class T>
static void
encode_json_impl(const char* name, const T& val, ceph::Formatter* f)
{
...
encode_json(const char* name, const T& val, ceph::Formatter* f)
{
  JSONEncodeFilter* filter = static_cast<JSONEncodeFilter*>(
      f->get_external_feature_handler("JSONEncodeFilter"));
...
void encode_json(
    const char* name,
    const ceph::real_time& val,
    ceph::Formatter* f);
void encode_json(
    const char* name,
    const ceph::coarse_real_time& val,
    ceph::Formatter* f);

template <class T>
static void
encode_json(const char* name, const std::list<T>& l, ceph::Formatter* f)
{
...
template <class T>
static void
encode_json(const char* name, const std::deque<T>& l, ceph::Formatter* f)
{
  f->open_array_section(name);
  for (auto iter = l.cbegin(); iter != l.cend(); ++iter) {
    encode_json("obj", *iter, f);
...
static void
encode_json(const char* name, const std::set<T, Compare>& l, ceph::Formatter* f)
{
  f->open_array_section(name);
  for (auto iter = l.cbegin(); iter != l.cend(); ++iter) {
    encode_json("obj", *iter, f);
...
template <class T>
static void
encode_json(const char* name, const std::vector<T>& l, ceph::Formatter* f)
{
  f->open_array_section(name);
  for (auto iter = l.cbegin(); iter != l.cend(); ++iter) {
    encode_json("obj", *iter, f);
...
static void
encode_json(const char* name, const std::array<T, N>& l, ceph::Formatter* f)
{
  f->open_array_section(name);
  for (auto iter = l.cbegin(); iter != l.cend(); ++iter) {
    encode_json("obj", *iter, f);
...
template <class K, class V, class C = std::less<K>>
static void
encode_json(const char* name, const std::map<K, V, C>& m, ceph::Formatter* f)
{
  f->open_array_section(name);
...
encode_json(
    const char* name,
    const boost::container::flat_map<K, V, C>& m,
    ceph::Formatter* f)
{
  f->open_array_section(name);
  for (auto i = m.cbegin(); i != m.cend(); ++i) {
...
template <class K, class V>
static void
encode_json(const char* name, const std::multimap<K, V>& m, ceph::Formatter* f)
{
  f->open_array_section(name);
...
template <class K, class V>
void
encode_json_map(const char* name, const std::map<K, V>& m, ceph::Formatter* f)
{
  f->open_array_section(name);
...
encode_json_map(
    ...
    void (*cb)(const char*, const V&, ceph::Formatter*, void*),
    void* parent,
    const std::map<K, V>& m,
    ceph::Formatter* f)
{
...
{
  encode_json_map<K, V>(
      name, index_name, object_name, value_name, NULL, NULL, m, f);
}
...
template <class T>
static void
encode_json(const char* name, const std::optional<T>& o, ceph::Formatter* f)
{
  if (!o) {
    return;
  }
...
protected:
  bool handle_value(std::string_view name, std::string_view s, bool quoted)
      override;
  bool handle_open_section(
      std::string_view name,
      const char* ns,
      bool section_is_array) override;
...
public:
  ...
  static std::list<JSONFormattable>
  ...
  const std::map<std::string, JSONFormattable>
  object() const
  {
    return obj;
  }

  const std::vector<JSONFormattable>&
  array() const
  {
    return arr;
  }
  ...
  std::string
  operator()(const char* def_val) const
  {
    return def(std::string(def_val));
  }

  int
  operator()(int def_val) const
  {
    return def(def_val);
  }

  bool
  operator()(bool def_val) const
  {
    return def(def_val);
  }
