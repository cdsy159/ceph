...
#define FUT_DEBUG(FMT_MSG, ...) \
  crimson::get_logger(ceph_subsys_).trace(FMT_MSG, ##__VA_ARGS__)
#else
#define FUT_DEBUG(FMT_MSG, ...)
#endif
...
template <typename... Args>
recursive_mutex
make_recursive_mutex(Args&&... args)
{
  return {};
}
...
ceph::containers::tiny_vector<LockT>
make_lock_container(const std::size_t num_instances, LockFactoryT&& lock_factory)
{
