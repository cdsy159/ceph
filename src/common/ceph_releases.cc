...
std::ostream&
operator<<(std::ostream& os, const ceph_release_t r)
{
    ...
    std::ostream& err)
{
  ...
  const ceph_release_t cutoff{
      static_cast<uint8_t>(static_cast<uint8_t>(from_release) + 2)};
