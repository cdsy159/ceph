...
const char*
ceph_entity_type_name(int type)
{
  switch (type) {
...
const char*
ceph_con_mode_name(int con_mode)
{
  switch (con_mode) {
...
const char*
ceph_osd_op_name(int op)
{
  switch (op) {
#define GENERATE_CASE(op, opcode, str) \
  case CEPH_OSD_OP_##op:               \
    return (str);
...
ceph_release_features(int r)
{
  ...
  req |= CEPH_FEATURE_CRUSH_TUNABLES3 | CEPH_FEATURE_OSD_PRIMARY_AFFINITY |
...
int
ceph_release_from_features(uint64_t features)
{
  int r = 1;
  while (true) {
...
const char*
ceph_osd_alloc_hint_flag_name(int f)
{
  switch (f) {
...
const char*
ceph_osd_backoff_op_name(int op)
{
  switch (op) {
  case CEPH_OSD_BACKOFF_OP_BLOCK:
    return "block";
  case CEPH_OSD_BACKOFF_OP_ACK_BLOCK:
    return "ack-block";
