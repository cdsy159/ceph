...
void dump_cmd_to_json(
    ceph::Formatter* f,
    uint64_t features,
    const std::string& cmd);
void dump_cmd_and_help_to_json(
    ceph::Formatter* f,
    uint64_t features,
    const std::string& secname,
    const std::string& cmd,
    const std::string& helptext);
void dump_cmddesc_to_json(
    ceph::Formatter* jf,
    uint64_t features,
    const std::string& secname,
...
template <typename T>
bool
cmd_getval(const cmdmap_t& cmdmap, std::string_view k, T& val)
{
...
template <typename T>
std::optional<T>
cmd_getval(const cmdmap_t& cmdmap, std::string_view k)
{
  T ret;
...
T
cmd_getval_or(const cmdmap_t& cmdmap, std::string_view k, const V& defval)
{
  auto found = cmdmap.find(k);
...
T
cmd_getval_cast_or(const cmdmap_t& cmdmap, std::string_view k, T defval)
{
  auto found = cmdmap.find(k);
  if (found == cmdmap.cend()) {
    return defval;
