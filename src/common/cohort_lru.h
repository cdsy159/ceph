...
private:
  ...
  typedef bi::list<
      Object,
      bi::member_hook<Object, bi::list_member_hook<link_mode>, &Object::lru_hook>,
  ...
  typedef bi::slist<
      Object,
      bi::member_hook<Object, bi::slist_member_hook<link_mode>, &Object::q2_hook>,
      bi::constant_time_size<true>>
      Queue2;

public:
  Object() :
    lru_flags(FLAG_NONE), lru_refcnt(0), lru_adj(0)
  {}
...
private:
  ...
  template <typename T, typename TTree, typename CLT, typename CEQ, typename K, typename LK>
  friend class TreeX;
};
...
private:
  ...
  bool
  can_reclaim(Object* o)
  {
    return (
        (o->lru_refcnt == SENTINEL_REFCNT) && (!(o->lru_flags & FLAG_EVICTING)));
  }

  Object*
  evict_block(const ObjectFactory* newobj_fac)
  {
...
public:
  ...
  Object*
  insert(ObjectFactory* fac, Edge edge, uint32_t& flags)
  {
...
public:
  ...
  T*
  find(uint64_t hk, const K& k, uint32_t flags)
  {
    T* v;
    Latch lat;
    uint32_t slot = 0;
  ...
  T*
  find_latch(uint64_t hk, const K& k, Latch& lat, uint32_t flags)
  {
    uint32_t slot = 0;
    T* v;
    lat.p = &(partition_of_scalar(hk));
  ...
  bool
  is_same_partition(uint64_t lhs, uint64_t rhs)
  {
    return ((lhs % n_part) == (rhs % n_part));
  }

  void
  insert_latched(T* v, Latch& lat, uint32_t flags)
  {
  ...
  void
  insert(uint64_t hk, T* v, uint32_t flags)
  {
  ...
  void
  remove(uint64_t hk, T* v, uint32_t flags)
  {
    Partition& p = partition_of_scalar(hk);
  ...
  void
  drain(std::function<void(T*)> uref, uint32_t flags = FLAG_NONE)
  {
