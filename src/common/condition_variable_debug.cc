...
void
condition_variable_debug::wait(std::unique_lock<mutex_debug>& lock)
{
...
void
condition_variable_debug::notify_one()
{
...
void
condition_variable_debug::notify_all(bool sloppy)
{
  if (!sloppy) {
    // make sure signaler is holding the waiter's lock.
...
std::cv_status
condition_variable_debug::_wait_until(mutex_debug* mutex, timespec* ts)
{
  // make sure this cond is used with one mutex only
