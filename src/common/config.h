...
public:
  ...
  template <typename T>
  const T get_val(const ConfigValues& values, const std::string_view key) const;
  ...
  with_val(
      ...
      const std::string_view key,
      Callback&& cb,
      Args&&... args) const -> std::invoke_result_t<Callback, const T&, Args...>
  {
...
private:
  ...
  typedef boost::container::
      small_vector<std::pair<const Option*, const Option::value_t*>, 4>
          expand_stack_t;

  void validate_schema();
  void validate_default_settings();

  Option::value_t get_val_generic(
      const ConfigValues& values,
  ...
  Option::value_t _get_val_nometa(
      const ConfigValues& values,
...
public: // for global_init
  ...
  std::list<std::string> get_conffile_paths(
      const ConfigValues& values,
      const char* conf_files,
      std::ostream* warnings,
      int flags) const;
...
public:
  unsigned
  get_osd_pool_default_min_size(const ConfigValues& values, uint8_t size) const
  {
...
template <typename T>
const T
md_config_t::get_val(const ConfigValues& values, const std::string_view key) const
{
  return std::get<T>(this->get_val_generic(values, key));
}

inline std::ostream&
operator<<(std::ostream& o, const std::monostate&)
{
  return o << "INVALID_CONFIG_VALUE";
}

int ceph_resolve_file_search(
    const std::string& filename_list,
    std::string& result);
