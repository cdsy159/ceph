...
public:
  ...
  typedef std::function<void(config_obs_ptr, const std::string&)>
      config_gather_cb;
  ...
  void for_each_change(
      const std::map<std::string, bool>& changes,
      config_gather_cb callback,
      std::ostream* oss);
...
template <class ConfigObs>
void
ObserverMgr<ConfigObs>::add_observer(ConfigObs* observer)
{
...
template <class ConfigObs>
typename ObserverMgr<ConfigObs>::config_obs_wptr
ObserverMgr<ConfigObs>::remove_observer(ConfigObs* observer)
{
...
template <class ConfigObs>
void
ObserverMgr<ConfigObs>::for_each_observer(config_gather_cb callback)
{
...
template <class ConfigObs>
void
ObserverMgr<ConfigObs>::for_each_change(
    const std::map<std::string, bool>& changes,
    config_gather_cb callback,
    std::ostream* oss)
{
...
template <class ConfigObs>
bool
ObserverMgr<ConfigObs>::is_tracking(const std::string& name) const
{
