...
class ConfigProxy {
  ...
  void
  _call_observers(rev_obs_map_t& rev_obs)
  {
  ...
  void
  _map_observer_changes(
      ObsMgr::config_obs_ptr obs,
...
public:
  ...
  void
  set_config_values(const ConfigValues& val)
  {
#ifndef WITH_CRIMSON
    std::lock_guard l{lock};
#endif
  ...
  int
  get_val(const std::string_view key, char** buf, int len) const
  {
    std::lock_guard l{lock};
  ...
  int
  get_val(const std::string_view key, std::string* val) const
  {
    std::lock_guard l{lock};
  ...
  template <typename T>
  const T
  get_val(const std::string_view key) const
  {
    std::lock_guard l{lock};
  ...
  auto
  with_val(const std::string_view key, Callback&& cb, Args&&... args) const
  {
    std::lock_guard l{lock};
    return config.template with_val<T>(
        values, key, std::forward<Callback>(cb), std::forward<Args>(args)...);
  }

  void
  config_options(ceph::Formatter* f) const
  {
    std::lock_guard l{lock};
    config.config_options(f);
  }

  const decltype(md_config_t::schema)&
  get_schema() const
  {
    std::lock_guard l{lock};
  ...
  const Option*
  get_schema(const std::string_view key) const
  {
    std::lock_guard l{lock};
  ...
  const Option*
  find_option(const std::string& name) const
  {
    std::lock_guard l{lock};
  ...
  void
  diff(ceph::Formatter* f, const std::string& name = {}) const
  {
    std::lock_guard l{lock};
  ...
  unsigned
  get_osd_pool_default_min_size(uint8_t size) const
  {
    std::lock_guard l{lock};
  ...
  void
  early_expand_meta(std::string& val, std::ostream* oss) const
  {
    std::lock_guard l{lock};
  ...
  void
  add_observer(md_config_obs_t* obs)
  {
    std::lock_guard l(lock);
  ...
  void
  remove_observer(md_config_obs_t* obs)
  {
  ...
  call_all_observers()
  {
    ...
    {
      std::lock_guard locker(lock);
      obs_mgr.for_each_observer([this,
  ...
  void
  config_options(ceph::Formatter* f)
  {
    std::lock_guard l{lock};
    config.config_options(f);
  }
  ...
  int
  rm_val(const std::string_view key)
  {
    std::lock_guard l{lock};
  ...
  void
  set_val_default(const std::string_view key, const std::string& val)
  {
    std::lock_guard l{lock};
  ...
  void
  set_val_or_die(const std::string_view key, const std::string& val)
  {
    std::lock_guard l{lock};
  ...
  int
  injectargs(const std::string& s, std::ostream* oss)
  {
    int ret;
    rev_obs_map_t rev_obs;
    {
  ...
  void
  parse_env(unsigned entity_type, const char* env_var = "CEPH_ARGS")
  {
    std::lock_guard l{lock};
  ...
  int
  parse_argv(std::vector<const char*>& args, int level = CONF_CMDLINE)
  {
    std::lock_guard l{lock};
  ...
  int
  parse_config_files(const char* conf_files, std::ostream* warnings, int flags)
  {
    std::lock_guard l{lock};
    return config.parse_config_files(
        values, obs_mgr, conf_files, warnings, flags);
  }

  bool
  has_parse_error() const
  {
    std::lock_guard l(lock);
  ...
  void
  complain_about_parse_error(CephContext* cct)
  {
    std::lock_guard l(lock);
  ...
  void
  get_defaults_bl(ceph::buffer::list* bl)
  {
    std::lock_guard l{lock};
