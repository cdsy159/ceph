...
#ifdef HAVE_RES_NQUERY
int
DNSResolver::get_state(CephContext* cct, res_state* ps)
{
  lock.lock();
  if (!states.empty()) {
...
void
DNSResolver::put_state(res_state s)
{
...
#ifdef HAVE_RES_NQUERY
  len = resolv_h->res_nquery(
      res, origname, ns_c_in, ns_t_cname, buf, sizeof(buf));
#else
  {
...
#ifdef HAVE_RES_NQUERY
  len = resolv_h->res_nquery(
      *res, hostname.c_str(), ns_c_in, type, nsbuf->data(), nsbuf->size());
#else
  {
#ifndef HAVE_THREAD_SAFE_RES_QUERY
    std::lock_guard l(lock);
#endif
    len = resolv_h->res_query(
        hostname.c_str(), ns_c_in, type, nsbuf->data(), nsbuf->size());
  }
#endif
  if (len < 0) {
    lderr(cct) << "res_query() failed" << dendl;
...
int
DNSResolver::resolve_srv_hosts(
    CephContext* cct,
    const string& service_name,
    const SRV_Protocol trans_protocol,
    map<string, DNSResolver::Record>* srv_hosts)
{
  return this->resolve_srv_hosts(
      cct, service_name, trans_protocol, "", srv_hosts);
}
    ...
    map<string, DNSResolver::Record>* srv_hosts)
{
  ...
  string query_str = "_" + service_name + "._" + proto_str +
                     (domain.empty() ? "" : "." + domain);
  int len;

#ifdef HAVE_RES_NQUERY
  len = resolv_h->res_nsearch(
      res, query_str.c_str(), ns_c_in, ns_t_srv, nsbuf->data(), nsbuf->size());
#else
  {
#ifndef HAVE_THREAD_SAFE_RES_QUERY
    std::lock_guard l(lock);
#endif
    len = resolv_h->res_search(
        query_str.c_str(), ns_c_in, ns_t_srv, nsbuf->data(), nsbuf->size());
  }
#endif
  if (len < 0) {
  ...
  for (int i = 0; i < num_hosts; i++) {
    ...
    string srv_domain =
        full_srv_name.substr(full_srv_name.find(protocol) + protocol.length());
    ...
    ns_name_uncompress(
        ns_msg_base(handle), ns_msg_end(handle), rdata, full_target,
        sizeof(full_target));
    ...
    if (r == 0) {
      ...
      if (end == target.npos) {
        lderr(cct) << "resolved target not in search domain: " << target
                   << " / " << srv_domain << dendl;
        return -EINVAL;
      }
