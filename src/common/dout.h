...
inline std::ostream&
operator<<(std::ostream& out, _bad_endl_use_dendl_t)
{
...
inline std::ostream&
operator<<(std::ostream& lhs, const DoutPrefixProvider& dpp)
{
  return dpp.gen_prefix(lhs);
}
#if FMT_VERSION >= 90000
template <>
struct fmt::formatter<DoutPrefixProvider> : fmt::ostream_formatter {};
#endif
...
public:
  ...
  std::ostream&
  gen_prefix(std::ostream& out) const override final
  {
    dpp.gen_prefix(out);
    add_prefix(out);
    return out;
...
#define dout_impl(cct, sub, v)                                              \
  do {                                                                      \
     ...
     * GCC prior to v14 was not enforcing these restrictions. */ \
      if constexpr (                                                        \
          ceph::dout::is_dynamic<decltype(sub_)>::value ||                  \
...
#define lgeneric_subdout(cct, sub, v) \
  dout_impl(cct, ceph_subsys_##sub, v) * _dout
