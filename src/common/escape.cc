...
size_t
escape_xml_attr_len(const char* buf)
{
  const char* b;
  size_t ret = 0;
  for (b = buf; *b; ++b) {
...
void
escape_xml_attr(const char* buf, char* out)
{
  char* o = out;
  const char* b;
  for (b = buf; *b; ++b) {
...
std::ostream&
operator<<(std::ostream& out, const xml_stream_escaper& e)
{
...
size_t
escape_json_attr_len(const char* buf, size_t src_len)
{
  const char* b;
  size_t i, ret = 0;
...
void
escape_json_attr(const char* buf, size_t src_len, char* out)
{
  char* o = out;
  const char* b;
  size_t i;
...
std::ostream&
operator<<(std::ostream& out, const json_stream_escaper& e)
{
  boost::optional<hex_formatter> fmt;
