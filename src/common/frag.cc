...
void
frag_t::encode(ceph::buffer::list& bl) const
{
  ceph::encode(_enc, bl);
}

void
frag_t::decode(ceph::buffer::list::const_iterator& p)
{
  ceph::decode(_enc, p);
  if (!is_frag_valid()) {
    ...
    if (nfg.is_frag_valid()) {
      std::cerr << "correcting byte swapped frag_t(0x" << std::hex
                << std::setfill('0') << std::setw(8) << _enc << ") to frag_t(0x"
                << std::hex << std::setfill('0') << std::setw(8) << nfg._enc
                << ")" << " aka " << nfg << std::endl;
      _enc = nfg._enc;
    } else {
      CachedStackStringStream css;
      *css << "Invalid frag_t(0x" << std::hex << std::setfill('0')
           << std::setw(8) << _enc << ")";
...
void
frag_t::dump(ceph::Formatter* f) const
{
...
std::list<frag_t>
frag_t::generate_test_instances()
{
  std::list<frag_t> ls;
  ls.emplace_back();
  ls.push_back(frag_t((1 << 22), 2));
  ls.push_back(frag_t((1 << 21), 3));
  return ls;
}

std::ostream&
operator<<(std::ostream& out, const frag_t& hb)
{
...
bool
fragtree_t::force_to_leaf(CephContext* cct, frag_t x)
{
  if (is_leaf(x))
    return false;
  ...
  if (parent.bits() < x.bits()) {
    ...
    lgeneric_dout(cct, 10) << "spread " << spread << ", parent splits by " << nb
                           << dendl;
    if (nb == 0) {
      // easy: split parent (a leaf) by the difference
      lgeneric_dout(cct, 10)
          << "splitting parent " << parent << " by spread " << spread << dendl;
      split(parent, spread);
    ...
    for (auto& frag : subs) {
      lgeneric_dout(cct, 10) << "splitting intermediate " << frag << " by "
                             << (nb - spread) << dendl;
...
void
fragtree_t::encode(ceph::buffer::list& bl) const
{
  using ceph::encode;
  encode(_splits, bl);
}
...
void
fragtree_t::decode(ceph::buffer::list::const_iterator& p)
{
  using ceph::decode;
  decode(_splits, p);
}
...
void
fragtree_t::encode_nohead(ceph::buffer::list& bl) const
{
  using ceph::encode;
...
void
fragtree_t::decode_nohead(int n, ceph::buffer::list::const_iterator& p)
{
  using ceph::decode;
  _splits.clear();
  while (n-- > 0) {
...
void
fragtree_t::print(std::ostream& out)
{
  out << "fragtree_t(";
  frag_vec_t s;
  s.push_back(frag_t());
...
void
fragtree_t::dump(ceph::Formatter* f) const
{
...
std::list<fragtree_t>
fragtree_t::generate_test_instances()
{
...
std::ostream&
operator<<(std::ostream& out, const fragtree_t& ft)
{
  out << "fragtree_t(";
...
void
fragset_t::encode(ceph::buffer::list& bl) const
{
  ceph::encode(_set, bl);
}

void
fragset_t::decode(ceph::buffer::list::const_iterator& p)
{
  ceph::decode(_set, p);
}

std::ostream&
operator<<(std::ostream& out, const fragset_t& fs)
{
