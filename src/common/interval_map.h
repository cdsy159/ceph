...
class interval_map {
  ...
  std::pair<Mapiter, Mapiter>
  get_range(K off, K len)
  {
  ...
  std::pair<Cmapiter, Cmapiter>
  get_range(K off, K len) const
  {
    // fst is first iterator with end after off (may be end)
  ...
  try_merge(Mapiter niter)
  {
    if (niter != m.begin()) {
      ...
      if (prev->first + prev->second.first == niter->first &&
        ...
        V n = s.merge(
            std::move(prev->second.second), std::move(niter->second.second));
        K off = prev->first;
        K len = niter->first + niter->second.first - off;
        niter++;
        m.erase(prev, niter);
        auto p =
            m.insert(std::make_pair(off, std::make_pair(len, std::move(n))));
    ...
    if (next != m.end() && niter->first + niter->second.first == next->first &&
      ...
      V n = s.merge(
          std::move(niter->second.second), std::move(next->second.second));
      K off = niter->first;
...
public:
    ...
    for (auto i = range.first; i != range.second; ++i) {
      if (i->first < off) {
        to_insert.emplace_back(std::make_pair(
            i->first,
            std::make_pair(
                off - i->first, s.split(0, off - i->first, i->second.second))));
      }
      if ((off + len) < (i->first + i->second.first)) {
        ...
        to_insert.emplace_back(std::make_pair(
            off + len,
            std::make_pair(
                nlen, s.split(i->second.first - nlen, nlen, i->second.second))));
      }
    }
  ...
  void
  insert(const interval_map& other)
  {
    for (auto&& i : other) {
  ...
  template <
      template <typename, typename, typename...> class ISC = std::map,
      bool strict = true>
  void
  to_interval_set(interval_set<K, ISC, strict>& set) const
  {
    for (auto&& i : *this) {
  ...
  class iterator {
    ...
    iterator(Mapiter&& it, bool end) :
      it(std::move(it)), end(end)
    {}
  ...
  get_containing_range(K off, K len) const
  {
  ...
  const_iterator
  get_lower_range(K off, K len) const
  {
  ...
  bool
  operator==(const interval_map& rhs) const
  {
    return m == rhs.m;
  }
  ...
  print(std::ostream& os) const
  {
    ...
    for (auto&& i : *this) {
      ...
      os << i.get_off() << "~" << i.get_len() << "(" << s.length(i.get_val())
         << ")";
    }
    os << "}";
  }
    ...
    requires fmt::formattable<K>
  {
    ...
    for (auto&& i : *this) {
      ...
      str += fmt::format(
          "{}~{}({})", i.get_off(), i.get_len(), s.length(i.get_val()));
    }
    str += "}";
    return str;
  }
