...
class intrusive_lru {
  ...
  using lru_set_option_t = boost::intrusive::
      member_hook<base_t, boost::intrusive::set_member_hook<>, &base_t::set_hook>;
  ...
  struct VToKWrapped {
    ...
    const type&
    operator()(const base_t& obc)
    {
      return VToK()(static_cast<const T&>(obc));
    }
  };

  using lru_set_t = boost::intrusive::
      set<base_t, lru_set_option_t, boost::intrusive::key_of_value<VToKWrapped>>;
  lru_set_t lru_set;
  ...
  void
  evict(unsigned target_size)
  {
    while (!unreferenced_list.empty() && lru_set.size() > target_size) {
      auto& evict_target = unreferenced_list.front();
      assert(evict_target.is_unreferenced());
      unreferenced_list.pop_front();
      lru_set.erase_and_dispose(lru_set.iterator_to(evict_target), [](auto* p) {
        delete p;
      });
    }
  }
  ...
  void
  clear_range(
      typename lru_set_t::iterator from,
      typename lru_set_t::iterator to,
      F&& f)
  {
    while (from != to) {
      if (!(*from).lru) {
        ...
        from = lru_set.erase_and_dispose(from, [](auto* p) {
...
public:
  ...
  template <typename F>
  void
  clear_range(const K& from, const K& to, F&& f)
  {
  ...
  void
  set_target_size(size_t target_size)
  {
...
template <typename Config>
void
intrusive_ptr_add_ref(intrusive_lru_base<Config>* p)
{
  assert(p);
  p->use_count++;
...
template <typename Config>
void
intrusive_ptr_release(intrusive_lru_base<Config>* p)
{
