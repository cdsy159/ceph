...
DataGenerator::validate(
    ...
    ceph::io_exerciser::Sequence seq,
    int step)
{
...
SeededRandomGenerator::generate_block(uint64_t block_offset)
{
  ...
  if (seed != 0) {
    ...
    if (remainingBytes > generation_length) {
      ...
      std::memcpy(
          buffer + block_size - remainingBytes, &rand1, generation_length);
      std::memcpy(
          buffer + block_size - remainingBytes2, &rand2, remainingBytes2);
...
ceph::bufferptr
SeededRandomGenerator::generate_wrong_block(uint64_t block_offset)
{
  uint64_t block_size = m_model.get_block_size();
  char buffer[block_size];
...
bufferlist
SeededRandomGenerator::generate_data(uint64_t offset, uint64_t length)
{
  bufferlist retlist;
...
bufferlist
SeededRandomGenerator::generate_wrong_data(uint64_t offset, uint64_t length)
{
  bufferlist retlist;
...
uint64_t
HeaderedSeededRandomGenerator::generate_unique_run_id()
{
...
HeaderedSeededRandomGenerator::generate_block(uint64_t block_offset)
{
  ...
  if (seed != 0) {
    std::memcpy(
        bufferptr.c_str() + uniqueIdStart(), &unique_run_id, uniqueIdLength());
...
ceph::bufferptr
HeaderedSeededRandomGenerator::generate_wrong_block(uint64_t block_offset)
{
  return HeaderedSeededRandomGenerator::generate_block(block_offset % 8);
}

const HeaderedSeededRandomGenerator::UniqueIdBytes
HeaderedSeededRandomGenerator::readUniqueRunId(
...
HeaderedSeededRandomGenerator::readSeed(
    uint64_t block_offset,
    const bufferlist& bufferlist)
{
...
HeaderedSeededRandomGenerator::readDateTime(
    uint64_t block_offset,
    const bufferlist& bufferlist)
{
...
bool
HeaderedSeededRandomGenerator::validate(
    bufferlist& bufferlist,
    uint64_t offset,
    uint64_t length,
    std::string_view pool,
...
bool
HeaderedSeededRandomGenerator::validate_block(
    uint64_t block_offset,
    const char* buffer_start)
{
  ...
  valid = valid ? strncmp(
                      bufferptr.c_str() + timeEnd(), buffer_start + timeEnd(),
...
void
HeaderedSeededRandomGenerator ::printDebugInformationForBlock(
    uint64_t read_offset,
    uint64_t block_offset,
    const bufferlist& bufferlist)
{
  ...
  if (blockError == ErrorType::DATA_MISMATCH ||
    ...
    std::memcpy(
        &generated_bytes, generate_block(block_offset).c_str(),
        m_model.get_block_size() - bodyStart());
  }
  ...
  case ErrorType::DATA_MISMATCH: {
    error_string = fmt::format(
        ...
        " Data written at {}\nExpected data: \n{:02x}\nRead data:{:02x}",
        block_offset, block_offset * m_model.get_block_size(), std::ctime(&ttp),
        fmt::join(
            generated_bytes, generated_bytes + m_model.get_block_size(), ""),
...
void
HeaderedSeededRandomGenerator ::printDebugInformationForRange(
    uint64_t read_offset,
...
void
HeaderedSeededRandomGenerator ::printDebugInformationForRunIdMismatchRange(
    uint64_t read_offset,
    uint64_t start_block_offset,
    uint64_t range_length_in_blocks,
    const bufferlist& bufferlist)
{
  ...
  for (uint64_t i = start_block_offset;
    ...
    ceph_assert(
        getErrorTypeForBlock(read_offset, i, bufferlist) ==
...
void
HeaderedSeededRandomGenerator ::printDebugInformationForSeedMismatchRange(
    uint64_t read_offset,
    uint64_t start_block_offset,
    uint64_t range_length_in_blocks,
    const bufferlist& bufferlist)
{
  ...
  ceph_assert(
      m_model.get_block_size() < (std::numeric_limits<uint64_t>::max() / 2));
...
void
HeaderedSeededRandomGenerator ::printDebugInformationDataBodyMismatchRange(
    uint64_t read_offset,
    uint64_t start_block_offset,
    uint64_t range_length_in_blocks,
    const bufferlist& bufferlist)
...
void
HeaderedSeededRandomGenerator ::printDebugInformationCorruptRange(
    uint64_t read_offset,
    uint64_t start_block_offset,
    uint64_t range_length_in_blocks,
    const bufferlist& bufferlist)
...
void
HeaderedSeededRandomGenerator ::printDebugInformationDataNotFoundRange(
    uint64_t read_offset,
    uint64_t start_block_offset,
    uint64_t range_length_in_blocks,
    const bufferlist& bufferlist)
...
void
HeaderedSeededRandomGenerator ::printDebugInformationForOffsets(
    uint64_t read_offset,
  ...
  for (const uint64_t& block_offset : offsets) {
    ...
    if (range_start == 0 && range_length == 0) {
      ...
      rangeError = blockError;
        ...
        blockError == rangeError && range_start + range_length == block_offset) {
      range_length++;
    } else {
      if (range_length == 1) {
        printDebugInformationForBlock(read_offset, range_start, bufferlist);
      } else if (range_length > 1) {
        printDebugInformationForRange(
            read_offset, range_start, range_length, rangeError, bufferlist);
      }
