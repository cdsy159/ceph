...
bool
EcIoSequence::is_supported(Sequence sequence) const
{
  return true;
}

std::unique_ptr<IoSequence>
EcIoSequence::generate_sequence(
    Sequence sequence,
    ...
    bool check_consistency)
{
  ...
  case Sequence::SEQUENCE_SEQ14:
    return std::make_unique<ReadInjectSequence>(
        obj_size_range, seed, sequence, km, mappinglayers, check_consistency);
  case Sequence::SEQUENCE_SEQ10:
    return std::make_unique<Seq10>(
        obj_size_range, seed, km, mappinglayers, check_consistency);
  default:
    ceph_abort_msg("Unrecognised sequence");
  }
}

EcIoSequence::EcIoSequence(
    std::pair<int, int> obj_size_range,
    int seed,
    bool check_consistency) :
...
void
EcIoSequence::select_random_data_shard_to_inject_read_error(
    std::optional<std::pair<int, int>> km,
    std::optional<std::pair<std::string_view, std::string_view>> mappinglayers)
{
  if (km) {
  ...
  if (mappinglayers) {
    ...
    for (int i = 0; i < data_shard_position; i++) {
      shard_to_inject = std::distance(
          std::find(
              mappinglayers->first.begin(), mappinglayers->first.end(),
              *shard_to_inject),
...
void
EcIoSequence::select_random_data_shard_to_inject_write_error(
    std::optional<std::pair<int, int>> km,
    std::optional<std::pair<std::string_view, std::string_view>> mappinglayers)
{
...
void
EcIoSequence::select_random_shard_to_inject_read_error(
    std::optional<std::pair<int, int>> km,
    std::optional<std::pair<std::string_view, std::string_view>> mappinglayers)
{
  if (km) {
...
void
EcIoSequence::select_random_shard_to_inject_write_error(
    std::optional<std::pair<int, int>> km,
    std::optional<std::pair<std::string_view, std::string_view>> mappinglayers)
{
  // Write errors do not support injecting to the primary OSD
...
void
EcIoSequence::generate_random_read_inject_type()
{
...
void
EcIoSequence::generate_random_write_inject_type()
{
  inject_op_type = static_cast<InjectOpType>(
...
{
  child_sequence =
      IoSequence::generate_sequence(s, obj_size_range, seed, check_consistency);
  select_random_data_shard_to_inject_read_error(km, mappinglayers);
  generate_random_read_inject_type();
}

Sequence
ceph::io_exerciser::ReadInjectSequence::get_id() const
{
  return child_sequence->get_id();
}

std::string
ceph::io_exerciser::ReadInjectSequence::get_name() const
{
...
Sequence
ceph::io_exerciser::Seq10::get_id() const
{
  return Sequence::SEQUENCE_SEQ10;
}

std::string
ceph::io_exerciser::Seq10::get_name() const
{
...
std::unique_ptr<ceph::io_exerciser::IoOp>
ceph::io_exerciser::Seq10::_next()
{
  if (!inject_error_done) {
    inject_error_done = true;
    barrier = true;
    return InjectWriteErrorOp::generate(
        *shard_to_inject, 0, 0, std::numeric_limits<uint64_t>::max());
