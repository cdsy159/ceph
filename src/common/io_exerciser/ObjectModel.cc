...
int
ObjectModel::get_seed(uint64_t offset) const
{
...
std::vector<int>
ObjectModel::get_seed_offsets(int seed) const
{
...
std::string
ObjectModel::to_string(int mask) const
{
  if (!primary_created) {
...
bool
ObjectModel::readyForIoOp(IoOp& op)
{
  return true;
}

void
ObjectModel::applyIoOp(IoOp& op)
{
  ...
  auto verify_and_record_read_op =
      [&primary_contents = primary_contents, &primary_created = primary_created,
       &num_io = num_io, &reads = reads,
       &writes = writes]<OpType opType, int N>(ReadWriteOp<opType, N>& readOp) {
        ceph_assert(primary_created);
        for (int i = 0; i < N; i++) {
          ceph_assert(
              readOp.offset[i] + readOp.length[i] <= primary_contents.size());
      ...
      [&primary_contents = primary_contents, &primary_created = primary_created,
       ...
       &writes = writes]<OpType opType, int N>(ReadWriteOp<opType, N> writeOp) {
        ...
        for (int i = 0; i < N; i++) {
          ...
          writes.union_insert(writeOp.offset[i], writeOp.length[i]);
          ceph_assert(
              writeOp.offset[i] + writeOp.length[i] <= primary_contents.size());
        }
        num_io++;
      };
  ...
  case OpType::Copy:
    ...
    std::copy(
        primary_contents.begin(), primary_contents.end(),
        secondary_contents.begin());
    break;
  ...
  case OpType::Create:
    ...
    std::generate(
        std::execution::seq, primary_contents.begin(), primary_contents.end(),
        generate_random);
    break;
  ...
  case OpType::Truncate: {
    ...
    if (expand) {
      std::generate(
          std::execution::seq, primary_contents.begin() + new_size,
