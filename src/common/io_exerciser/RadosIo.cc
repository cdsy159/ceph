...
RadosIo::RadosIo(
  ...
  rados(rados),
  asio(asio),
  om(std::make_unique<ObjectModel>(
      primary_oid,
      secondary_oid,
      block_size,
      seed,
      delete_objects)),
  db(data_generation::DataGenerator::create_generator(data_generation_type, *om)),
  pool(pool),
  threads(threads),
  lock(lock),
  cond(cond),
...
template <int N>
RadosIo::AsyncOpInfo<N>::AsyncOpInfo(
...
RadosIo::applyIoOp(IoOp& op)
{
  ...
  case OpType::Create: {
    ...
    std::shared_ptr<AsyncOpInfo<1>> op_info = std::make_shared<AsyncOpInfo<1>>(
        std::array<uint64_t, 1>{0}, std::array<uint64_t, 1>{opSize});
    ...
    librados::async_operate(
        asio.get_executor(), io, primary_oid, std::move(wop), 0, nullptr,
        create_cb);
    break;
  }
  ...
  case OpType::Remove: {
    if (!delete_objects) {
      const std::string new_primary_oid = primary_oid_base + "_" +
  ...
  case OpType::Copy: {
    ...
    librados::async_operate(
        asio.get_executor(), io, secondary_oid, std::move(wop), 0, nullptr,
        write_cb);
    break;
  }
...
void
RadosIo::applyReadWriteOp(IoOp& op)
{
  auto applyReadOp = [this]<OpType opType, int N>(ReadWriteOp<opType, N> readOp) {
    ...
    for (int i = 0; i < N; i++) {
      rop.read(
          readOp.offset[i] * block_size, readOp.length[i] * block_size,
          &op_info->bufferlist[i], nullptr);
    }
    auto read_cb =
        [this,
         op_info](boost::system::error_code ec, version_t ver, bufferlist bl) {
          ceph_assert(ec == boost::system::errc::success);
          for (int i = 0; i < N; i++) {
            ceph_assert(db->validate(
                op_info->bufferlist[i], op_info->offset[i], op_info->length[i],
                pool, seq ? seq->get_id() : Sequence::SEQUENCE_END,
                seq ? seq->get_step() : -1));
          }
          finish_io();
        };
    librados::async_operate(
        asio.get_executor(), io, primary_oid, std::move(rop), 0, nullptr,
        read_cb);
    num_io++;
  };

  auto applyWriteOp =
      [this]<OpType opType, int N>(ReadWriteOp<opType, N> writeOp) {
        auto op_info =
        ...
        for (int i = 0; i < N; i++) {
          ...
          wop.write(writeOp.offset[i] * block_size, op_info->bufferlist[i]);
        }
        auto write_cb = [this](boost::system::error_code ec, version_t ver) {
          ceph_assert(ec == boost::system::errc::success);
          finish_io();
  ...
  auto applyFailedWriteOp =
      [this]<OpType opType, int N>(ReadWriteOp<opType, N> writeOp) {
        ...
        auto write_cb = [this,
                         writeOp](boost::system::error_code ec, version_t ver) {
          ceph_assert(ec != boost::system::errc::success);
          finish_io();
        };
        librados::async_operate(
...
RadosIo::applyInjectOp(IoOp& op)
{
  ...
  int rc = send_mon_command(
      osdMapRequest, rados, "OSDMapRequest", {}, &osdmap_outbl, formatter.get());
  ceph_assert(rc == 0);
  ...
  case OpType::InjectReadError: {
    ...
    if (errorOp.type == 0) {
      ...
      int rc = send_osd_command(
          osd, injectErrorRequest, rados, "InjectECErrorRequest", {},
          &inject_outbl, formatter.get());
      ceph_assert(rc == 0);
    } else if (errorOp.type == 1) {
      ceph::messaging::osd::InjectECErrorRequest<InjectOpType::ReadMissingShard>
          injectErrorRequest{pool,         primary_oid,  errorOp.shard,
                             errorOp.type, errorOp.when, errorOp.duration};
      int rc = send_osd_command(
          osd, injectErrorRequest, rados, "InjectECErrorRequest", {},
  ...
  case OpType::ClearReadErrorInject: {
    ...
    if (errorOp.type == 0) {
      ...
      int rc = send_osd_command(
          osd, clearErrorInject, rados, "InjectECClearErrorRequest", {},
          &inject_outbl, formatter.get());
      ceph_assert(rc == 0);
    } else if (errorOp.type == 1) {
  ...
  case OpType::ClearWriteErrorInject: {
    ...
    if (errorOp.type == 0) {
      ...
      ceph::messaging::osd::InjectECClearErrorRequest<InjectOpType::WriteOSDAbort>
          clearErrorInject{pool, primary_oid, errorOp.shard, errorOp.type};
      int rc = send_osd_command(
          osd, clearErrorInject, rados, "InjectECClearErrorRequest", {},
          &inject_outbl, formatter.get());
