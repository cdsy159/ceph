...
bool
matches_ipv4_in_subnet(
    const struct ifaddrs& addrs,
    ...
    struct in6_addr* out)
{
  ...
  if (prefix_len < 128)
    out->s6_addr[prefix_len / 8] = addr->s6_addr[prefix_len / 8] &
                                   ~(0xFF >> (prefix_len % 8));
  if (prefix_len / 8 < 15)
...
bool
parse_network(const char* s, entity_addr_t* network, unsigned int* prefix_len)
{
  sockaddr_storage ss;
    ...
    const struct entity_addr_t& addr)
{
  ...
  case AF_INET6: {
    struct in6_addr a, b;
    netmask_ipv6(
        &((const sockaddr_in6*)network.get_sockaddr())->sin6_addr, prefix_len,
        &a);
    netmask_ipv6(
