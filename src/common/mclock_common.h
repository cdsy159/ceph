 ...
 */
constexpr profile_t HIGH_CLIENT_OPS{{.6, 2, 0}, {.4, 1, 0}, {0, 1, .7}};
 ...
 */
constexpr profile_t HIGH_RECOVERY_OPS{{.3, 1, 0}, {.7, 2, 0}, {0, 1, 0}};
 ...
 */
constexpr profile_t BALANCED{{.5, 1, 0}, {.5, 1, 0}, {0, 1, .9}};
...
struct client_profile_id_t {
  ...
  client_profile_id_t(uint64_t _client_id, uint64_t _profile_id) :
    client_id(_client_id), profile_id(_profile_id)
  {}
...
class ClientRegistry {
  ...
  std::map<client_profile_id_t, crimson::dmclock::ClientInfo>
...
public:
  ...
  get_tracked_keys() const noexcept final
  {
