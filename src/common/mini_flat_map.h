...
class mini_flat_map {
  ...
  void
  range_check(KeyT const k) const
  {
...
public:
  ...
  public:
    ...
    using value_type = std::conditional_t<
        is_const,
        const std::pair<const KeyT&, const ValueT&>,
  ...
  private:
    using mini_flat_map_p =
        std::conditional_t<is_const, const mini_flat_map*, mini_flat_map*>;
    mini_flat_map_p map;
  ...
  public:
    _iterator(mini_flat_map_p map) :
      map(map), key(0)
    {
      progress();
    }

    _iterator(mini_flat_map_p map, KeyT key) :
      map(map), key(key)
    {
      if (unsigned_cast(key) < map->data.size()) {
        value.emplace(key, *map->_at(key));
    ...
    _iterator(mini_flat_map_p map, size_t map_size) :
      map(map), key(map_size)
    {
    ...
    _iterator&
    operator=(const _iterator& other)
    {
      if (this != &other) {
        key = other.key;
...
public:
  ...
  mini_flat_map(size_t max_size) :
    data(max_size), _end(this, max_size), _const_end(this, max_size), _size(0)
  {}
   ...
   */
  mini_flat_map(size_t max_size, const std::map<KeyT, ValueT>&& other) :
    data(max_size), _end(this, max_size), _const_end(this, max_size), _size(0)
  {
    for (auto&& [k, t] : other) {
   ...
   */
  void
  swap(mini_flat_map& other) noexcept
  {
  ...
  mini_flat_map&
  operator=(mini_flat_map&& other) noexcept
  {
    data = std::move(other.data);
    _size = other._size;
    return *this;
  ...
  mini_flat_map&
  operator=(const mini_flat_map& other)
  {
  ...
  /** Equality operator */
  bool
  operator==(mini_flat_map const& other) const
  {
  ...
  template <class... Args>
  bool
  emplace(const KeyT& k, Args&&... args)
  {
    if (!contains(k)) {
      _size++;
  ...
  bool
  insert(const KeyT& k, const ValueT& value)
  {
  ...
  template <size_t N>
  void
  populate_bitset_set(bitset_set<N, KeyT>& set) const
  {
  ...
  friend std::ostream&
  operator<<(std::ostream& lhs, const mini_flat_map<KeyT, ValueT>& rhs)
  {
    unsigned int c = 0;
    lhs << "{";
    for (auto&& [k, v] : rhs) {
