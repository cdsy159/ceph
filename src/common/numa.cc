...
int
parse_cpu_set_list(const char* s, size_t* cpu_set_size, cpu_set_t* cpu_set)
{
  CPU_ZERO(cpu_set);
  while (*s) {
    char* end;
...
std::string
cpu_set_to_str_list(size_t cpu_set_size, const cpu_set_t* cpu_set)
{
  std::string r;
  unsigned a = 0;
  while (true) {
...
std::set<int>
cpu_set_to_set(size_t cpu_set_size, const cpu_set_t* cpu_set)
{
  set<int> r;
  unsigned a = 0;
  while (true) {
...
int
get_numa_node_cpu_set(int node, size_t* cpu_set_size, cpu_set_t* cpu_set)
{
...
int
set_cpu_affinity_all_threads(size_t cpu_set_size, cpu_set_t* cpu_set)
{
