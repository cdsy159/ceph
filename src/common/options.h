...
struct Option {
  ...
  static int
  str_to_type(const std::string& s)
  {
    if (s == "uint") {
      return TYPE_UINT;
    }
  ...
  static level_t
  str_to_level(std::string_view s)
  {
    if (s == "basic") {
      return LEVEL_BASIC;
    } else if (s == "advanced") {
  ...
  struct size_t {
    ...
    bool
    operator==(const size_t& rhs) const
    {
  ...
  void dump_value(const char* field_name, const value_t& v, ceph::Formatter* f)
      const;
  ...
  Option&
  set_value(value_t& v, const char* new_value)
  {
  ...
  Option&
  set_value(value_t& v, const T& new_value)
  {
    v = new_value;
    return *this;
  }
  ...
  Option&
  set_value(value_t& v, T new_value)
  {
    switch (type) {
    case TYPE_INT:
      v = int64_t(new_value);
    ...
    case TYPE_BOOL:
      ...
      break;
    case TYPE_SIZE:
      v = size_t{static_cast<std::uint64_t>(new_value)};
      break;
    case TYPE_SECS:
      v = std::chrono::seconds{new_value};
      break;
    case TYPE_MILLISECS:
      v = std::chrono::milliseconds{new_value};
      break;
    default:
      std::cerr << "Bad type in set_value: " << name << ": " << typeid(T).name()
                << std::endl;
      ceph_abort();
    }
    return *this;
  ...
  Option&
  add_tag(const std::initializer_list<const char*>& ts)
  {
  ...
  Option&
  add_service(const std::initializer_list<const char*>& ss)
  {
  ...
  Option&
  add_see_also(const std::initializer_list<const char*>& ts)
  {
  ...
  template <typename T>
  Option&
  set_min_max(const T& mi, const T& ma)
  {
    set_value(min, mi);
    set_value(max, ma);
    return *this;
  ...
  Option&
  set_enum_allowed(const std::vector<const char*>& allowed)
  {
  ...
  Option&
  set_validator(const validator_fn_t& validator_)
  {
   ...
   */
  bool
  can_update_at_runtime() const
  {
    return (has_flag(FLAG_RUNTIME) ||
            (!has_flag(FLAG_MGR) &&
             (type == TYPE_BOOL || type == TYPE_INT || type == TYPE_UINT ||
              type == TYPE_FLOAT || type == TYPE_SIZE || type == TYPE_SECS ||
              type == TYPE_MILLISECS))) &&
           !has_flag(FLAG_STARTUP) && !has_flag(FLAG_CLUSTER_CREATE) &&
...
constexpr unsigned long long
operator""_hr(unsigned long long hr)
{
  return hr * 60 * 60;
}

constexpr unsigned long long
operator""_day(unsigned long long day)
{
  return day * 24 * 60 * 60;
}
...
constexpr unsigned long long
operator""_G(unsigned long long n)
{
  return n << 30;
}

constexpr unsigned long long
operator""_T(unsigned long long n)
{
  return n << 40;
}
