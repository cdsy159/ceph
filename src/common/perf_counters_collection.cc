...
PerfCountersCollection::PerfCountersCollection(CephContext* cct) :
  m_cct(cct), m_lock(ceph::make_mutex("PerfCountersCollection"))
{}

PerfCountersCollection::~PerfCountersCollection() { clear(); }

void
PerfCountersCollection::add(PerfCounters* l)
{
...
void
PerfCountersCollection::remove(PerfCounters* l)
{
  std::lock_guard lck(m_lock);
  perf_impl.remove(l);
}
...
bool
PerfCountersCollection::reset(const std::string& name)
{
  std::lock_guard lck(m_lock);
...
void
PerfCountersCollection::dump_formatted(
    ceph::Formatter* f,
    bool schema,
...
void
PerfCountersCollection::dump_formatted_histograms(
    ceph::Formatter* f,
    bool schema,
    const std::string& logger,
    const std::string& counter)
...
void
PerfCountersCollection::with_counters(
...
void
PerfCountersDeleter::operator()(PerfCounters* p) noexcept
{
  if (cct)
