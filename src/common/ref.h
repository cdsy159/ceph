...
template <typename T>
using cref_t = boost::intrusive_ptr<const T>;

template <class T, class U>
ref_t<T>
ref_cast(const ref_t<U>& r) noexcept
{
...
template <class T, class U>
cref_t<T>
ref_cast(const cref_t<U>& r) noexcept
{
...
#define FRIEND_MAKE_REF(C)             \
  template <class T, typename... Args> \
  friend ceph::ref_t<T> ceph::make_ref(Args&&... args)
