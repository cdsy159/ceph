...
namespace librados {
inline void
decode(object_id_t& obj, ceph::buffer::list::const_iterator& bp)
{
...
namespace librados {
inline void
decode(librados::osd_shard_t& shard, ceph::buffer::list::const_iterator& bp)
{
...
public:
  ...
  bool
  only_data_digest_mismatch_info() const
  {
...
namespace librados {
inline void
decode(librados::shard_info_t& shard, ceph::buffer::list::const_iterator& bp)
{
...
inline void
decode(
    librados::inconsistent_snapset_t& snapset,
...
struct fmt::formatter<librados::object_id_t> {
  ...
  auto
  format(const auto& oid, FormatContext& ctx) const
  {
    return fmt::format_to(
        ctx.out(), "{}/{}/{}", oid.locator, oid.nspace, oid.name);
  }
};
...
struct fmt::formatter<librados::err_t> {
  ...
  template <typename FormatContext>
  auto
  format(const auto& err, FormatContext& ctx) const
  {
    bool first = true;
#define F(FLAG_NAME)                             \
...
struct fmt::formatter<librados::shard_info_t> {
  ...
  format(const auto& err, FormatContext& ctx) const
  {
    return fmt::format_to(
        ...
        "selected_io: {}, "
        "primary: {})",
        static_cast<librados::err_t>(err), err.size, err.omap_digest_present,
        err.omap_digest, err.data_digest_present, err.data_digest,
        err.selected_oi, err.primary);
  }
};
...
struct fmt::formatter<librados::osd_shard_t> {
  ...
  template <typename FormatContext>
  auto
  format(const auto& shard, FormatContext& ctx) const
  {
    return fmt::format_to(
        ctx.out(), "osd_shard_t(osd: {}, shard: {})", shard.osd, shard.shard);
  }
};
...
struct fmt::formatter<librados::inconsistent_obj_t> {
  ...
  format(const auto& err, FormatContext& ctx) const
  {
    return fmt::format_to(
        ...
        "shards: {}, "
        "union_shards: {})",
        static_cast<librados::obj_err_t>(err), err.object, err.version,
        err.shards, err.union_shards);
  }
};
...
#undef F
    return fmt::format_to(
        ctx.out(), ", object: {}, clones: {}, missing: {}", err.object,
        err.clones, err.missing);
  }
};
