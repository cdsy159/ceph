...
struct SnapRealmInfoNew {
  ...
  ino() const
  {
    return inodeno_t(info.h.ino);
  }

  inodeno_t
  parent() const
  {
    return inodeno_t(info.h.parent);
  }

  snapid_t
  seq() const
  {
    return snapid_t(info.h.seq);
  }

  snapid_t
  parent_since() const
  {
    return snapid_t(info.h.parent_since);
  }

  snapid_t
  created() const
  {
    return snapid_t(info.h.created);
  }
...
#if FMT_VERSION >= 90000
template <>
struct fmt::formatter<SnapContext> : fmt::ostream_formatter {};
#endif
