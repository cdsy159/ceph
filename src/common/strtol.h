...
template <typename T>
auto
parse(std::string_view s, int base = 10)
...
template <typename T>
auto
consume(std::string_view& s, int base = 10)
    -> std::enable_if_t<std::is_integral_v<T>, std::optional<T>>
{
  T t;
  auto r = std::from_chars(s.data(), s.data() + s.size(), t, base);
...
unsigned long long strict_strtoull(
    std::string_view str,
    int base,
    std::string* err);
...
ritoa(T u, char* buf)
{
  ...
  static_assert(
      base <= 16, "extend character map below to support higher bases");
  unsigned digits = 0;
  while (u) {
