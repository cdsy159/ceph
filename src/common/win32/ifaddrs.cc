...
getifaddrs(struct ifaddrs** ifap)
{
  ...
  res = GetAdaptersAddresses(
      AF_UNSPEC, GAA_FLAG_INCLUDE_PREFIX, NULL, adapter_addrs, &size);
  ...
  for (PIP_ADAPTER_ADDRESSES curr_addrs = adapter_addrs; curr_addrs != NULL;
       curr_addrs = curr_addrs->Next) {
    if (curr_addrs->OperStatus != 1)
      continue;

    for (PIP_ADAPTER_UNICAST_ADDRESS unicast_addrs =
             curr_addrs->FirstUnicastAddress;
         unicast_addrs != NULL; unicast_addrs = unicast_addrs->Next) {
      ...
      if (unicast_sockaddr->sa_family == AF_INET) {
        ...
        if (ConvertLengthToIpv4Mask(
                unicast_addrs->OnLinkPrefixLength, &subnet_mask)) {
          errno = ENODATA;
          ret = -1;
          goto out;
        }
        struct sockaddr_in* addr4 =
            (struct sockaddr_in*)&out_list_curr->in_addrs;
        struct sockaddr_in* netmask4 =
            (struct sockaddr_in*)&out_list_curr->in_netmasks;
      ...
      sprintf_s(
          out_list_curr->ad_name, sizeof(out_list_curr->ad_name),
