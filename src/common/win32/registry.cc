  ...
  cct(cct_)
{
  ...
  if (status == ERROR_FILE_NOT_FOUND && create_value) {
    ...
    status = RegCreateKeyEx(
        hRootKey, strKey, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS,
        NULL, &hKey, NULL);
  }
...
int
RegistryKey::remove(CephContext* cct_, HKEY hRootKey, LPCTSTR strKey)
{
  DWORD status = RegDeleteKeyEx(hRootKey, strKey, KEY_WOW64_64KEY, 0);

  if (status == ERROR_FILE_NOT_FOUND) {
    ldout(cct_, 20) << "Registry key : " << strKey << " does not exist."
                    << dendl;
    return 0;
  }
...
int
RegistryKey::set(LPCTSTR lpValue, DWORD data)
{
  DWORD status =
      RegSetValueEx(hKey, lpValue, 0, REG_DWORD, (LPBYTE)&data, sizeof(DWORD));
  if (ERROR_SUCCESS != status) {
    derr << "Error: " << win32_strerror(status)
...
int
RegistryKey::set(LPCTSTR lpValue, std::string data)
{
  DWORD status = RegSetValueEx(
      hKey, lpValue, 0, REG_SZ, (LPBYTE)data.c_str(), data.length());
  if (ERROR_SUCCESS != status) {
    derr << "Error: " << win32_strerror(status)
         << ". Could not set registry value: " << (char*)lpValue << dendl;
    return -EINVAL;
...
int
RegistryKey::get(LPCTSTR lpValue, bool& value)
{
  DWORD value_dw = 0;
...
int
RegistryKey::get(LPCTSTR lpValue, DWORD& value)
{
  DWORD data;
...
int
RegistryKey::get(LPCTSTR lpValue, std::string& value)
{
  std::string data{""};
  DWORD size = 0;
  DWORD type = REG_SZ;
  DWORD status =
      RegQueryValueEx(hKey, lpValue, NULL, &type, (LPBYTE)data.c_str(), &size);
  if (ERROR_MORE_DATA == status) {
    data.resize(size);
    status = RegQueryValueEx(
        hKey, lpValue, NULL, &type, (LPBYTE)data.c_str(), &size);
  }
