      ...
      ceph::bufferlist&& input) const final
  {
    ...
    if (!cmd_getval(cmdmap, "pgid", pgid_str)) {
      ...
      return seastar::make_ready_future<tell_result_t>(tell_result_t{
          -EINVAL, fmt::format("couldn't parse pgid '{}'", pgid_str)});
    }
    ...
    return osd.get_pg_shard_manager().with_pg(
        spg_id,
        [this, spg_id, cmdmap = std::move(cmdmap), format = std::move(format),
         input = std::move(input)](auto&& pg) mutable {
          if (!pg) {
            return seastar::make_ready_future<tell_result_t>(tell_result_t{
...
private:
  virtual seastar::future<tell_result_t> do_command(
      Ref<PG> pg,
      const cmdmap_t& cmdmap,
      std::string_view format,
...
template std::unique_ptr<AdminSocketHook>
make_asok_hook<crimson::admin::pg::MarkUnfoundLostCommand>(
    crimson::osd::OSD& osd);
