...
public:
  ...
  std::vector<uint32_t>
  get_supported_con_modes(int peer_type, uint32_t auth_method) final
  {
      ...
      const std::vector<uint32_t>& preferred_modes) final
  {
    ...
    ceph_assert(
        preferred_modes.size() && preferred_modes[0] == CEPH_CON_MODE_CRC);
    return CEPH_CON_MODE_CRC;
  }

  AuthAuthorizeHandler*
  get_auth_authorize_handler(int peer_type, int auth_method) final
  {
    return nullptr;
  }

  AuthClient::auth_request_t
  get_auth_request(
