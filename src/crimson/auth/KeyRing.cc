...
seastar::future<KeyRing*>
load_from_keyring(KeyRing* keyring)
{
  std::vector<std::string> paths;
  boost::split(
      paths, crimson::common::local_conf()->keyring, boost::is_any_of(",;"));
  std::pair<bool, std::string> found;
  return seastar::map_reduce(
             paths,
             [](auto path) {
               return seastar::engine().file_exists(path).then(
                   [path](bool file_exists) {
...
seastar::future<KeyRing*>
load_from_keyfile(KeyRing* keyring)
{
