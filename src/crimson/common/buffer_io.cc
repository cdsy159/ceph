    ...
    seastar::file_permissions permissions)
{
  ...
  return seastar::open_file_dma(fn, flags, foo)
      .then([bl = std::move(bl)](seastar::file f) {
        return seastar::make_file_output_stream(f).then(
            [bl = std::move(bl),
             f = std::move(f)](seastar::output_stream<char> out) {
              return seastar::do_with(
                  std::move(out), std::move(f), std::move(bl),
                  [](seastar::output_stream<char>& out, seastar::file& f,
                     ceph::buffer::list& bl) {
...
{
  return seastar::open_file_dma(fn, seastar::open_flags::ro)
      .then([](seastar::file f) {
