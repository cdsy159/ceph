...
class ConfigProxy : public seastar::peering_sharded_service<ConfigProxy> {
  ...
  do_change(Func&& func)
  {
    return container().invoke_on(
        values.get_owner_shard(), [func = std::move(func)](ConfigProxy& owner) {
          ...
          return seastar::parallel_for_each(
                     std::views::iota(1u, seastar::smp::count),
                     [&owner, new_values](auto cpu) {
                       return owner.container().invoke_on(
                           cpu, [foreign_values = seastar::make_foreign(
                                     new_values)](ConfigProxy& proxy) mutable {
                             proxy.values.reset();
                             ...
                             for (const auto& change : proxy.values->changed) {
                               std::string dummy;
                               changes_present[change] =
                                   proxy.get_val(change, &dummy);
                             }
...
public:
  ...
  {
    get_config().get_config_bl(
        get_config_values(), have_version, bl, got_version);
  }
  ...
  void
  add_observer(ConfigObserver* obs)
  {
    obs_mgr.add_observer(obs);
  }

  void
  remove_observer(ConfigObserver* obs)
  {
    obs_mgr.remove_observer(obs);
  }

  seastar::future<>
  rm_val(const std::string& key)
  {
  ...
  seastar::future<>
  set_val(const std::string& key, const std::string& val)
  {
  ...
  int
  get_val(std::string_view key, std::string* val) const
  {
  ...
  template <typename T>
  const T
  get_val(std::string_view key) const
  {
  ...
  {
    return get_config().get_val_from_conf_file(
        *values, sections, key, out, expand_meta);
  }
  ...
  seastar::future<>
  inject_args(const std::string& s)
  {
  ...
  seastar::future<>
  parse_argv(std::vector<const char*>& argv)
  {
  ...
  parse_env()
  {
    return do_change([this](ConfigValues& values) {
      get_config().parse_env(CEPH_ENTITY_TYPE_OSD, values, obs_mgr);
    });
  }
...
inline ConfigProxy::ShardedConfig&
sharded_conf()
{
