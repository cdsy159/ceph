...
public:
  ...
  struct iter_t {
    ...
    friend bool
    operator==(const iter_t<is_const>& lhs, const iter_t<!is_const>& rhs)
    {
      assert(lhs.node == rhs.node);
      return lhs.offset == rhs.offset;
    }
  ...
  private:
    ...
    get_key_ptr() const
    {
      return reinterpret_cast<typename maybe_const_t<char, is_const>::type>(
  ...
  struct delta_t {
    ...
    bool
    operator==(const delta_t& rhs) const
    {
...
public:
  ...
  public:
    ...
    void
    update(const K& key, const V& val)
    {
      KINT k;
      k = key;
    ...
    void
    copy_in(const char* out, size_t len)
    {
      assert(empty());
    ...
    bool
    operator==(const delta_buffer_t& rhs) const
    {
  ...
  void
  journal_update(const_iterator _iter, const V& val, delta_buffer_t* recorder)
  {
    auto iter = iterator(this, _iter.offset);
    if (recorder) {
  ...
  void
  journal_remove(const_iterator _iter, delta_buffer_t* recorder)
  {
    auto iter = iterator(this, _iter.offset);
    if (recorder) {
      recorder->remove(iter->get_key());
  ...
  const_iterator
  lower_bound(K l) const
  {
    auto it = std::lower_bound(
        boost::make_counting_iterator<uint16_t>(0),
        boost::make_counting_iterator<uint16_t>(get_size()), l,
  ...
  bool
  operator==(const FixedKVNodeLayout& rhs) const
  {
   ...
   */
  K
  split_into(FixedKVNodeLayout& left, FixedKVNodeLayout& right) const
  {
   ...
   */
  void
  merge_from(const FixedKVNodeLayout& left, const FixedKVNodeLayout& right)
  {
      ...
      const FixedKVNodeLayout& right)
  {
    ...
    assert(
        pivot_idx > std::min(l_size, r_size) &&
      ...
      FixedKVNodeLayout& replacement_right)
  {
    ...
    if (pivot_idx < left.get_size()) {
      replacement_left.copy_from_foreign(
          replacement_left.end(), left.begin(), left.iter_idx(pivot_idx));
      replacement_left.set_size(pivot_idx);

      replacement_right.copy_from_foreign(
          replacement_right.end(), left.iter_idx(pivot_idx), left.end());

      replacement_right.set_size(left.get_size() - pivot_idx);
      replacement_right.copy_from_foreign(
          replacement_right.end(), right.begin(), right.end());
      replacement_right.set_size(total - pivot_idx);
    } else {
      replacement_left.copy_from_foreign(
          replacement_left.end(), left.begin(), left.end());
      replacement_left.set_size(left.get_size());

      replacement_left.copy_from_foreign(
          replacement_left.end(), right.begin(),
          right.iter_idx(pivot_idx - left.get_size()));
      replacement_left.set_size(pivot_idx);

      replacement_right.copy_from_foreign(
          replacement_right.end(), right.iter_idx(pivot_idx - left.get_size()),
          right.end());
      replacement_right.set_size(total - pivot_idx);
    }

    auto [lmeta, rmeta] =
        Meta::rebalance(left.get_meta(), right.get_meta(), replacement_pivot);
...
private:
  void
  insert(iterator iter, const K& key, const V& val)
  {
  ...
  void
  replace(iterator iter, const K& key, const V& val)
  {
    assert(iter != end());
    if (VALIDATE_INVARIANTS) {
      if (iter != begin()) {
  ...
  template <size_t N>
  uint32_t
  calc_phy_checksum_iteratively(uint32_t crc) const
  {
    if constexpr (N == 0) {
      return crc;
    } else {
      uint32_t r = ceph_crc32c(
          crc, (unsigned char const*)layout.template Pointer<N>(buf),
   ...
   */
  virtual void
  node_resolve_vals(iterator from, iterator to) const
  {}

  virtual void
  node_unresolve_vals(iterator from, iterator to) const
  {}
   ...
   */
  static void
  copy_from_local(iterator tgt, iterator from_src, iterator to_src)
  {
    assert(tgt->node == from_src->node);
    assert(to_src->node == from_src->node);
    memmove(
