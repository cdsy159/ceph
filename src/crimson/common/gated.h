...
public:
  ...
  Gated(const seastar::shard_id sid) :
    sid(sid)
  {}
  ...
  inline void
  dispatch_in_background(const char* what, T& who, Func&& func)
  {
  ...
  dispatch(const char* what, T& who, Func&& func)
  {
    ceph_assert(seastar::this_shard_id() == sid);
    return seastar::with_gate(pending_dispatch, std::forward<Func>(func))
        .handle_exception([what, &who](std::exception_ptr eptr) {
          if (*eptr.__cxa_exception_type() ==
              typeid(system_shutdown_exception)) {
  ...
  auto
  simple_dispatch(const char* what, Func&& func)
  {
    ceph_assert(seastar::this_shard_id() == sid);
    return seastar::with_gate(pending_dispatch, std::forward<Func>(func));
  }
...
public:
  ...
  dispatch(const char* what, T& who, Func&& func)
  {
    return gates[seastar::this_shard_id()]->dispatch(
        what, who, std::forward<Func>(func));
  }
  ...
  simple_dispatch(const char* what, Func&& func)
  {
    return gates[seastar::this_shard_id()]->simple_dispatch(
        what, std::forward<Func>(func));
  }
  ...
  close_all()
  {
    ...
    return seastar::parallel_for_each(
        gates.begin(), gates.end(), [](std::unique_ptr<Gated>& gate_ptr) {
          return seastar::smp::submit_to(
              gate_ptr->get_shard_id(),
              [gate = gate_ptr.get()] { return gate->close(); });
        });
  }
