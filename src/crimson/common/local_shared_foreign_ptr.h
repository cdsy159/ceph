...
public:
  ...
  seastar::future<seastar::foreign_ptr<PtrType>>
  get_foreign() const noexcept
  {
    assert(!ptr || (ptr && *ptr));
    return ptr ? ptr->copy()
               : seastar::make_ready_future<seastar::foreign_ptr<PtrType>>(
                     nullptr);
  }
  ...
  unsigned
  get_owner_shard() const noexcept
  {
    assert(!ptr || (ptr && *ptr));
  ...
  local_shared_foreign_ptr&
  operator=(local_shared_foreign_ptr&& other) noexcept
  {
  ...
  local_shared_foreign_ptr&
  operator=(const local_shared_foreign_ptr& other) noexcept
  {
    ptr = other.ptr;
    return *this;
  }
...
template <typename T>
local_shared_foreign_ptr<T>
make_local_shared_foreign(seastar::foreign_ptr<T>&& ptr)
{
...
template <typename T>
local_shared_foreign_ptr<T>
make_local_shared_foreign(T&& ptr)
{
...
inline bool
operator==(
    const local_shared_foreign_ptr<T>& x,
...
template <typename T>
inline bool
operator==(const local_shared_foreign_ptr<T>& x, std::nullptr_t)
{
...
template <typename T>
inline bool
operator==(std::nullptr_t, const local_shared_foreign_ptr<T>& y)
{
...
struct hash<crimson::local_shared_foreign_ptr<T>>
  ...
  size_t
  operator()(const crimson::local_shared_foreign_ptr<T>& p) const
  {
    return hash<typename std::pointer_traits<T>::element_type*>::operator()(
        p.get());
  }
};
