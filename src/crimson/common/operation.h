...
namespace detail {
void dump_time_event(
    const char* name,
    const utime_t& timestamp,
    ceph::Formatter* f);
void dump_blocking_event(
    const char* name,
    const utime_t& timestamp,
...
struct Event {
  ...
  void
  trigger(OpT&& op, Args&&... args)
  {
    that()->internal_backend.handle(
        *that(), std::forward<OpT>(op), std::forward<Args>(args)...);
...
struct TimeEvent : Event<T> {
  ...
  struct InternalBackend final : Backend {
    void
    handle(T&, const Operation&) override
    {
  ...
  dump(ceph::Formatter* f) const
  {
    ...
    detail::dump_time_event(
        demangled_name.c_str(), internal_backend.timestamp, f);
  }
...
public:
  struct BlockingEvent : Event<BlockingEvent>,
    ...
    struct InternalBackend : Backend {
      void
      handle(BlockingEvent&, const Operation&, const T& blocker) override
      {
    ...
    struct TriggerI {
      ...
      auto
      maybe_record_blocking(FutureT&& fut, T& blocker)
      {
        if (!fut.available()) {
          ...
          return std::forward<FutureT>(fut).finally([&event = this->event,
                                                     &blocker]() mutable {
    ...
    protected:
      ...
      static void
      record_unblocking(BlockingEvent& event, T& blocker)
      {
    ...
    struct Trigger : TriggerI {
      ...
      template <class FutureT>
      decltype(auto)
      maybe_record_exit_barrier(FutureT&& fut)
      {
        if (!fut.available()) {
    ...
    protected:
      void
      record_blocking(T& blocker) override
      {
    ...
    dump(ceph::Formatter* f) const
    {
      ...
      detail::dump_blocking_event(
          demangled_name.c_str(), internal_backend.timestamp,
          internal_backend.blocker, f);
  ...
  decltype(auto)
  track_blocking(TriggerT&& trigger, Args&&... args)
  {
...
private:
  ...
  void
  add_event(BlockingEvent& event)
  {
    event_list.push_back(event);
  }

  void
  delete_event(BlockingEvent& event)
  {
...
struct AggregateBlockingEvent {
  ...
  public:
    template <class FutureT>
    auto
    maybe_record_blocking(FutureT&& fut, typename T::Blocker& blocker)
    {
  ...
  protected:
    typename TriggerI::TriggerContainerIRef
...
class OperationRegistryI {
  using op_list_member_option = boost::intrusive::
      member_hook<Operation, registry_hook_t, &Operation::registry_hook>;
...
protected:
  void
  do_register(Operation* op) final
  {
  ...
  bool
  registries_empty() const final
  {
    return std::all_of(registries.begin(), registries.end(), [](auto& opl) {
      return opl.empty();
    });
  }
...
public:
  ...
  void
  remove_from_registry(Operation& op)
  {
    const auto op_type = op.get_type();
...
class PipelineHandle {
  ...
  enter_maybe_sync(T& stage, typename T::BlockingEvent::template Trigger<OpT>&& t)
  {
    ...
    if (wait_fut.has_value()) {
      return wait_fut.value().then(
          [this, &stage, t = std::move(t),
...
class OrderedExclusivePhaseT : public PipelineStageIT<T> {
  void
  dump_detail(ceph::Formatter* f) const final
  {
...
public:
  ...
  seastar::future<PipelineExitBarrierI::Ref>
  enter(TriggerT& t)
  {
    waiting++;
...
private:
  ...
  void
  clear_held_by(Operation::id_t id)
  {
...
private:
  ...
  public:
    ExitBarrier(
        ...
        TriggerT& trigger) :
      phase(&phase), barrier(std::move(barrier)), trigger(trigger)
    {}
    ...
    ~ExitBarrier() final
    {
      ...
      if (barrier) {
        ...
        std::ignore = std::move(*barrier).then([phase = this->phase] {
          phase->mutex.unlock();
        });
      } else {
...
template <>
struct fmt::formatter<crimson::Operation> : fmt::ostream_formatter {};
#endif
