...
void
CrushLocation::_parse(const std::string& s)
{
  ...
  if (r < 0) {
    logger().error(
        "CrushWrapper::parse_loc_multimap error, keeping original\
      crush_location {}",
        *this);
    return;
  }
    ...
    return seastar::file_accessible(
        ...
        .then([this](bool result) {
          ...
          seastar::experimental::spawn_parameters params = {
              .argv = {
                  local_conf().get_val<std::string>("crush_location_hook"),
                  "--cluster", local_conf()->cluster, "--id",
                  local_conf()->name.get_id(), "--type",
...
std::ostream&
operator<<(std::ostream& os, const CrushLocation& loc)
{
  bool first = true;
