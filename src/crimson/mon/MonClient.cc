...
public:
  ...
  auth::AuthClient::auth_request_t get_auth_request(
      const EntityName& name,
      uint32_t want_keys);
  using secret_t = string;
  tuple<CryptoKey, secret_t, bufferlist> handle_auth_reply_more(
  ...
  tuple<CryptoKey, secret_t, int> handle_auth_done(
...
private:
  std::unique_ptr<AuthClientHandler> create_auth(
      crimson::auth::method_t,
      uint64_t global_id,
      const EntityName& name,
      uint32_t want_keys);
...
Connection::Connection(
  ...
  conn{conn},
  rotating_keyring{
      std::make_unique<RotatingKeyRing>(nullptr, CEPH_ENTITY_TYPE_OSD, keyring)}
{}

seastar::future<>
Connection::handle_auth_reply(Ref<MAuthReply> m)
{
...
seastar::future<>
Connection::renew_rotating_keyring()
{
  ...
  if (!rotating_keyring->need_new_secrets(cutoff)) {
    logger().debug(
        "renew_rotating_keyring secrets are up-to-date "
        "(they expire after {})",
        cutoff);
    return seastar::now();
  } else {
    logger().info(
        "renew_rotating_keyring renewing rotating keys "
        " (they expired before {})",
        cutoff);
  }
  if ((now > last_rotating_renew_sent) &&
    ...
    logger().info(
        "renew_rotating_keyring called too often (last: {})",
...
AuthAuthorizer*
Connection::get_authorizer(entity_type_t peer) const
{
  if (auth) {
...
Connection::create_auth(
    crimson::auth::method_t protocol,
    uint64_t global_id,
    const EntityName& name,
    uint32_t want_keys)
{
...
Connection::do_auth_single(Connection::request_t what)
{
  ...
  return conn->send(std::move(m))
      ...
      .then([this, life_extender = shared_from_this()](Ref<MAuthReply> m) {
        ...
        logger().info(
            "do_auth_single: {} returns {}: {}", *conn, *m, (int)m->result);
        ...
        default:
          ...
          logger().error(
              "do_auth_single: got error {} on mon {}", ret,
              conn->get_peer_addr());
          break;
        }
        return auth_result;
...
{
  return seastar::repeat_until_value([this, life_extender = shared_from_this(),
                                      what]() { return do_auth_single(what); });
}
...
Connection::get_auth_request(const EntityName& entity_name, uint32_t want_keys)
{
  // choose method
  auth_method = [&] {
...
int
Connection::handle_auth_bad_method(
    uint32_t old_auth_method,
    int result,
    const std::vector<uint32_t>& allowed_methods,
    const std::vector<uint32_t>& allowed_modes)
{
  logger().info(
      "old_auth_method {} result {} allowed_methods {}", old_auth_method,
      cpp_strerror(result), allowed_methods);
  ...
  if (p == auth_supported.end()) {
    logger().error(
        "server allowed_methods {} but i only support {}", allowed_methods,
        auth_supported);
    assert(auth_done);
    auth_done->set_exception(std::system_error(
        make_error_code(crimson::net::error::negotiation_failure)));
    return -EACCES;
  }
  auth_method = *p;
...
bool
Connection::is_my_peer(const entity_addr_t& addr) const
{
  ceph_assert(conn);
  ...
  gates.dispatch_in_background(__func__, *this, [this] {
    if (active_con) {
      return seastar::when_all_succeed(
                 wait_for_send_log(), active_con->get_conn()->send_keepalive(),
...
seastar::future<>
Client::send_log(log_flushing_t flush_flag)
{
  if (log_client) {
...
Client::ms_dispatch(crimson::net::ConnectionRef conn, MessageRef m)
{
  ...
  gates.dispatch_in_background(__func__, *this, [this, conn, &m, &dispatched] {
    ...
    case CEPH_MSG_AUTH_REPLY:
      return handle_auth_reply(*conn, boost::static_pointer_cast<MAuthReply>(m));
    ...
    case MSG_LOGACK:
      return handle_log_ack(boost::static_pointer_cast<MLogAck>(m));
    case MSG_CONFIG:
      return handle_config(boost::static_pointer_cast<MConfig>(m));
    default:
      dispatched = false;
      return seastar::now();
    }
...
void
Client::ms_handle_reset(crimson::net::ConnectionRef conn, bool /* is_replace */)
{
...
std::vector<uint32_t>
Client::get_supported_con_modes(int peer_type, uint32_t auth_method)
{
  std::vector<uint32_t> modes;
...
Client::get_auth_authorize_handler(int peer_type, int auth_method)
{
  return auth_registry.get_handler(peer_type, auth_method);
}

int
Client::handle_auth_request(
    ...
    ceph::bufferlist* reply)
{
  ...
  AuthAuthorizeHandler* ah =
      get_auth_authorize_handler(conn.get_peer_type(), auth_method);
  if (!ah) {
    logger().error(
        "no AuthAuthorizeHandler found for auth method: {}", auth_method);
    return -EOPNOTSUPP;
  }
  ...
  bool is_valid = ah->verify_authorizer(
      &cct, active_con->get_keys(), payload,
      auth_meta.get_connection_secret_length(), reply, &name, p_peer_global_id,
      &caps_info, &auth_meta.session_key, &auth_meta.connection_secret,
...
auth::AuthClient::auth_request_t
Client::get_auth_request(
    crimson::net::Connection& conn,
    AuthConnectionMeta& auth_meta)
{
  logger().info(
      "get_auth_request(conn={}, auth_method={})", conn, auth_meta.auth_method);
  // connection to mon?
  if (conn.get_peer_type() == CEPH_ENTITY_TYPE_MON) {
    ...
    auth_registry.get_supported_modes(
        conn.get_peer_type(), auth_meta.auth_method, &modes);
    return {authorizer->protocol, modes, authorizer->bl};
  }
}

ceph::bufferlist
Client::handle_auth_reply_more(
    crimson::net::Connection& conn,
    AuthConnectionMeta& auth_meta,
    const bufferlist& bl)
{
  if (conn.get_peer_type() == CEPH_ENTITY_TYPE_MON) {
    ...
    bufferlist reply;
    tie(auth_meta.session_key, auth_meta.connection_secret,
        reply) = (*found)->handle_auth_reply_more(bl);
    return reply;
  } else {
    ...
    const bufferlist& bl)
{
  if (conn.get_peer_type() == CEPH_ENTITY_TYPE_MON) {
    ...
    int r = 0;
    tie(auth_meta.session_key, auth_meta.connection_secret,
        r) = (*found)->handle_auth_done(global_id, bl);
    return r;
  } else {
    ...
    const std::vector<uint32_t>& allowed_modes)
{
  if (conn.get_peer_type() == CEPH_ENTITY_TYPE_MON) {
    ...
    // huh...
    logger().info(
        "hmm, they didn't like {} result {}", old_auth_method,
        cpp_strerror(result));
    return -EACCES;
  }
}
...
seastar::future<>
Client::handle_monmap(crimson::net::Connection& conn, Ref<MMonMap> m)
{
  monmap.decode(m->monmapbl);
  const auto peer_addr = conn.get_peer_addr();
  auto cur_mon = monmap.get_name(peer_addr);
  logger().info(
      "got monmap {}, mon.{}, is now rank {}", monmap.epoch, cur_mon,
...
seastar::future<>
Client::handle_auth_reply(crimson::net::Connection& conn, Ref<MAuthReply> m)
{
  logger().info("handle_auth_reply {} returns {}: {}", conn, *m, (int)m->result);
  auto found = std::find_if(
      pending_conns.begin(), pending_conns.end(),
      [peer_addr = conn.get_peer_addr()](auto& mc) {
        return mc->is_my_peer(peer_addr);
...
seastar::future<>
Client::handle_subscribe_ack(Ref<MMonSubscribeAck> m)
{
  sub.acked(m->interval);
  return seastar::now();
}

Client::get_version_t
Client::get_version(const std::string& map)
{
...
Client::handle_get_version_reply(Ref<MMonGetVersionReply> m)
{
  if (auto found = version_reqs.find(m->handle); found != version_reqs.end()) {
    ...
    logger().warn(
        "{}: version request with handle {} not found", __func__, m->handle);
  }
  return seastar::now();
}

seastar::future<>
Client::handle_mon_command_ack(Ref<MMonCommandAck> m)
{
  const auto tid = m->get_tid();
  if (auto found = std::find_if(
          mon_commands.begin(), mon_commands.end(),
          [tid](auto& cmd) { return cmd.req->get_tid() == tid; });
      found != mon_commands.end()) {
    auto& command = *found;
    logger().trace("{} {}", __func__, tid);
    command.result.set_value(
        std::make_tuple(m->r, m->rs, std::move(m->get_data())));
...
seastar::future<>
Client::handle_log_ack(Ref<MLogAck> m)
{
  if (log_client) {
...
std::vector<unsigned>
Client::get_random_mons(unsigned n) const
{
...
seastar::future<bool>
Client::reopen_session(int rank)
{
  ...
  if (rank >= 0) {
    ...
    const auto parallel = crimson::common::local_conf().get_val<uint64_t>(
  ...
  return seastar::parallel_for_each(
             mons,
             [this](auto rank) {
               auto peer = choose_client_addr(
                   msgr.get_myaddrs(), monmap.get_addrs(rank));
               if (peer == entity_addr_t{}) {
                 // crimson msgr only uses the first bound addr
                 logger().warn(
                     "mon.{} does not have an addr compatible with me", rank);
                 return seastar::now();
               }
               logger().info("connecting to mon.{}", rank);
               auto conn = msgr.connect(peer, CEPH_ENTITY_TYPE_MON);
               auto& mc =
                   pending_conns.emplace_back(seastar::make_shared<Connection>(
                       auth_registry, conn, &keyring));
               ...
               return mc->authenticate_v2()
                   .then([peer, this](auto result) {
                     ...
                     logger().debug(
                         "reopen_session mon connection attempts complete");
                   })
...
void
Client::_finish_auth(const entity_addr_t& peer)
{
  if (!is_hunting()) {
    return;
  }
  ...
  auto found = std::find_if(
      pending_conns.begin(), pending_conns.end(),
      [peer](auto& conn) { return conn->is_my_peer(peer); });
  if (found == pending_conns.end()) {
    // Happens if another connection has won the race
    ceph_assert(active_con && pending_conns.empty());
    logger().info(
        "no pending connection for mon.{}, peer {}", monmap.get_name(peer),
        peer);
    return;
  }
...
Client::run_command(std::string&& cmd, bufferlist&& bl)
{
  ...
  auto& command =
      mon_commands.emplace_back(crimson::make_message<MMonCommand>(*m));
...
bool
Client::sub_want(const std::string& what, version_t start, unsigned flags)
{
  return sub.want(what, start, flags);
}

void
Client::sub_got(const std::string& what, version_t have)
{
  sub.got(what, have);
}

void
Client::sub_unwant(const std::string& what)
{
  sub.unwant(what);
}

bool
Client::sub_want_increment(
...
void
Client::print(std::ostream& out) const
{
  out << "mon." << entity_name;
}
