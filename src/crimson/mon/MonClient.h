...
private:
  ...
  std::vector<uint32_t> get_supported_con_modes(
      int peer_type,
      uint32_t auth_method) final;
  uint32_t pick_con_mode(
      int peer_type,
  ...
  // AuthClient methods
  crimson::auth::AuthClient::auth_request_t get_auth_request(
      crimson::net::Connection& conn,
...
template <>
struct fmt::formatter<crimson::mon::Client> : fmt::ostream_formatter {};
#endif
