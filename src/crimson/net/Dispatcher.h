...
public:
  ...
  virtual std::optional<seastar::future<>> ms_dispatch(
      ConnectionRef,
      MessageRef) = 0;
  ...
  virtual void
  ms_handle_connect(ConnectionRef conn, seastar::shard_id prv_shard)
  {}
  ...
  // happens under lossy policy when both sides wish to connect to each other.
  virtual void
  ms_handle_reset(ConnectionRef conn, bool is_replace)
  {}

  virtual void
  ms_handle_remote_reset(ConnectionRef conn)
  {}
};
