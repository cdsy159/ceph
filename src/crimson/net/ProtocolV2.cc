...
{
  throw std::system_error(
      make_error_code(crimson::net::error::protocol_aborted));
}
    ...
    const char* where)
{
  if (actual != expected) {
    logger().warn(
        "{} {} received wrong tag: {}, expected {}", conn, where,
...
{
  logger().warn(
      "{} {} received unexpected tag: {}", conn, where,
...
ProtocolV2::Timer::backoff(double seconds)
{
  ...
  return seastar::sleep_abortable(dur, *as).handle_exception_type(
      [this](const seastar::sleep_aborted& e) {
    ...
    const entity_name_t& _peer_name)
{
  ...
  logger().info(
      "{} ProtocolV2::start_connect(): peer_addr={}, peer_name={}, "
      "client_cookie=0x{:x}"
      " policy(lossy={}, server={}, standby={}, resetcheck={})",
      conn, _peer_addr, _peer_name, client_cookie, conn.policy.lossy,
...
void
ProtocolV2::start_accept(SocketFRef&& new_socket, const entity_addr_t& _peer_addr)
{
  assert(seastar::this_shard_id() == conn.get_messenger_shard_id());
  ceph_assert(state == state_t::NONE);
  ...
  logger().info(
      "{} ProtocolV2::start_accept(): target_addr={}", conn, _peer_addr);
  ...
  gate.dispatch_in_background("set_accepted_sid", conn, [this, cc_seq] {
    return io_handler.set_accepted_sid(
        cc_seq, frame_assembler->get_socket_shard_id(),
...
void
ProtocolV2::trigger_state_phase1(state_t new_state)
{
  ceph_assert_always(!gate.is_closed());
  if (new_state == state) {
    logger().error(
        "{} is not allowed to re-trigger state {}", conn, get_state_name(state));
    ceph_abort();
  }
  if (state == state_t::CLOSING) {
    logger().error(
        "{} CLOSING is not allowed to trigger state {}", conn,
        get_state_name(new_state));
    ceph_abort();
  }
  logger().debug(
      "{} TRIGGER {}, was {}", conn, get_state_name(new_state),
...
void
ProtocolV2::trigger_state_phase2(state_t new_state, io_state_t new_io_state)
{
  ...
  auto cc_seq = crosscore.prepare_submit();
  logger().debug(
      "{} send {} IOHandler::set_io_state(): new_state={}, new_io_state={}, "
      "fa={}, set_notify_out={}",
  ...
  gate.dispatch_in_background(
      ...
      [this, cc_seq, new_io_state, fa = std::move(fa)]() mutable {
        return seastar::smp::submit_to(
            ...
            [this, cc_seq, new_io_state, fa = std::move(fa),
             set_notify_out = need_notify_out]() mutable {
  ...
  if (need_exit_io) {
    ...
    auto cc_seq = crosscore.prepare_submit();
    logger().debug(
        "{} send {} IOHandler::wait_io_exit_dispatching() ...", conn, cc_seq);
    ...
    gate.dispatch_in_background("exit_io", conn, [this, cc_seq] {
      return seastar::smp::submit_to(
          ...
          .then([this, cc_seq](auto ret) {
            logger().debug(
                "{} finish {} IOHandler::wait_io_exit_dispatching(), {}", conn,
                cc_seq, ret.io_states);
...
{
  assert(
      expected_state == state_t::CONNECTING ||
  ...
  if (state != expected_state) {
    logger().info(
        "{} protocol {} {} is aborted at inconsistent {} -- {}", conn,
        get_state_name(expected_state), where, get_state_name(state), e_what);
#ifndef NDEBUG
    if (expected_state == state_t::REPLACING) {
      assert(state == state_t::CLOSING);
    } else if (expected_state == state_t::READY) {
      assert(
          state == state_t::CLOSING || state == state_t::REPLACING ||
          state == state_t::CONNECTING || state == state_t::STANDBY);
    } else {
  ...
  if (state != state_t::CONNECTING && conn.policy.lossy) {
    ...
    logger().info(
        "{} protocol {} {} fault on lossy channel, going to CLOSING -- {}",
  ...
  if (conn.policy.server ||
    ...
    if (conn.policy.server) {
      logger().info(
          "{} protocol {} {} fault as server, going to STANDBY {} -- {}", conn,
          get_state_name(state), where, io_states, e_what);
    } else {
      logger().info(
          "{} protocol {} {} fault with nothing to send, going to STANDBY {} "
          "-- {}",
          conn, get_state_name(state), where, io_states, e_what);
    }
    execute_standby();
  } else if (state == state_t::CONNECTING || state == state_t::REPLACING) {
    logger().info(
        "{} protocol {} {} fault, going to WAIT {} -- {}", conn,
        get_state_name(state), where, io_states, e_what);
    execute_wait(false);
  } else {
    assert(state == state_t::READY || state == state_t::ESTABLISHING);
    logger().info(
        "{} protocol {} {} fault, going to CONNECTING {} -- {}", conn,
        get_state_name(state), where, io_states, e_what);
    execute_connecting();
  }
}
...
void
ProtocolV2::reset_session(bool full)
{
  server_cookie = 0;
  connect_seq = 0;
  if (full) {
...
ProtocolV2::banner_exchange(bool is_connect)
{
  ...
  logger().debug(
      "{} SEND({}) banner: len_payload={}, supported=0x{:x}, "
      "required=0x{:x}, banner=\"{}\"",
      conn, bl.length(), len_payload, CRIMSON_MSGR2_SUPPORTED_FEATURES,
      CEPH_MSGR2_REQUIRED_FEATURES, CEPH_BANNER_V2_PREFIX);
  ...
  return frame_assembler
      ...
      .then([this](auto bptr) {
        ...
        logger().debug(
            "{} RECV({}) banner: \"{}\"", conn, bptr.length(),
            std::string(bptr.c_str(), banner_prefix_len));

        if (memcmp(bptr.c_str(), CEPH_BANNER_V2_PREFIX, banner_prefix_len) !=
            0) {
...
#ifdef UNIT_TESTS_BUILT
        return frame_assembler
            ->intercept_frame(custom_bp_t::BANNER_PAYLOAD_READ, false)
      ...
      .then([this, is_connect](bufferlist bl) {
        ...
        logger().debug(
            "{} RECV({}) banner features: supported=0x{:x} required=0x{:x}",
            conn, bl.length(), _peer_supported_features,
        ...
        if ((required_features & _peer_supported_features) !=
            required_features) {
          logger().error(
        ...
        if ((supported_features & _peer_required_features) !=
            _peer_required_features) {
          logger().error(
        ...
        auto hello =
            HelloFrame::Encode(messenger.get_mytype(), conn.target_addr);
        logger().debug(
            "{} WRITE HelloFrame: my_type={}, peer_addr={}", conn,
            ceph_entity_type_name(messenger.get_mytype()), conn.target_addr);
      ...
      .then([this](auto payload) {
        ...
        logger().debug(
            "{} GOT HelloFrame: my_type={} peer_addr={}", conn,
            ceph_entity_type_name(hello.entity_type()), hello.peer_addr());
        return seastar::make_ready_future<
            std::tuple<entity_type_t, entity_addr_t>>(
  ...
  return frame_assembler->read_main_preamble().then([this](auto ret) {
    ...
    case Tag::AUTH_BAD_METHOD:
      return frame_assembler->read_frame_payload().then([this](auto payload) {
        ...
        logger().warn(
            "{} GOT AuthBadMethodFrame: method={} result={}, "
        ...
        int r = messenger.get_auth_client()->handle_auth_bad_method(
            conn, *auth_meta, bad_method.method(), bad_method.result(),
            bad_method.allowed_methods(), bad_method.allowed_modes());
        if (r < 0) {
          logger().warn(
              "{} auth_client handle_auth_bad_method returned {}", conn, r);
          abort_in_fault();
        }
    ...
    case Tag::AUTH_REPLY_MORE:
      return frame_assembler->read_frame_payload()
          .then([this](auto payload) {
            ...
            logger().debug(
                "{} GOT AuthReplyMoreFrame: payload_len={}", conn,
                auth_more.auth_payload().length());
            ceph_assert(messenger.get_auth_client());
            ...
            logger().debug(
                "{} WRITE AuthRequestMoreFrame: payload_len={}", conn,
                reply.length());
    ...
    case Tag::AUTH_DONE:
      return frame_assembler->read_frame_payload().then([this](auto payload) {
        ...
        logger().debug(
            "{} GOT AuthDoneFrame: gid={}, con_mode={}, payload_len={}", conn,
            auth_done.global_id(), ceph_con_mode_name(auth_done.con_mode()),
            auth_done.auth_payload().length());
        ceph_assert(messenger.get_auth_client());
        int r = messenger.get_auth_client()->handle_auth_done(
            conn, *auth_meta, auth_done.global_id(), auth_done.con_mode(),
...
seastar::future<>
ProtocolV2::client_auth(std::vector<uint32_t>& allowed_methods)
{
...
ProtocolV2::client_connect()
{
  ...
  auto client_ident = ClientIdentFrame::Encode(
      messenger.get_myaddrs(), conn.target_addr, messenger.get_myname().num(),
      global_seq, conn.policy.features_supported,
      conn.policy.features_required | msgr2_required, flags, client_cookie);

  logger().debug(
      "{} WRITE ClientIdentFrame: addrs={}, target={}, gid={},"
      ...
      messenger.get_myname().num(), global_seq, conn.policy.features_supported,
      conn.policy.features_required | msgr2_required, flags, client_cookie);
      ...
      .then([this](auto ret) {
        ...
        case Tag::IDENT_MISSING_FEATURES:
          return frame_assembler->read_frame_payload().then([this](auto payload) {
            ...
            auto ident_missing =
                IdentMissingFeaturesFrame::Decode(payload->back());
            logger().warn(
                "{} GOT IdentMissingFeaturesFrame: features=0x{:x}"
        ...
        case Tag::SERVER_IDENT:
          return frame_assembler->read_frame_payload().then([this](auto payload) {
            if (unlikely(state != state_t::CONNECTING)) {
              logger().debug(
                  "{} triggered {} at receiving SERVER_IDENT", conn,
                  get_state_name(state));
              abort_protocol();
            }
            ...
            auto cc_seq = crosscore.prepare_submit();
            logger().debug(
                "{} send {} IOHandler::requeue_out_sent()", conn, cc_seq);
            ...
            logger().debug(
                "{} GOT ServerIdentFrame:"
                " addrs={}, gid={}, gs={},"
                " features_supported=0x{:x}, features_required=0x{:x},"
                " flags=0x{:x}, server_cookie=0x{:x}",
                conn, server_ident.addrs(), server_ident.gid(),
                server_ident.global_seq(), server_ident.supported_features(),
                server_ident.required_features(), server_ident.flags(),
                server_ident.cookie());
            ...
            if (!server_ident.addrs().contains(conn.target_addr)) {
              logger().warn(
                  "{} peer identifies as {}, does not include {}", conn,
                  server_ident.addrs(), conn.target_addr);
              throw std::system_error(
            ...
            if (server_ident.addrs().front() != conn.peer_addr) {
              logger().warn(
                  "{} peer advertises as {}, does not match {}", conn,
                  server_ident.addrs(), conn.peer_addr);
              throw std::system_error(
                  make_error_code(crimson::net::error::bad_peer_address));
            }
            if (conn.get_peer_id() != entity_name_t::NEW &&
              ...
              logger().error(
                  "{} connection peer id ({}) does not match "
...
ProtocolV2::client_reconnect()
{
  ...
  auto reconnect = ReconnectFrame::Encode(
      messenger.get_myaddrs(), client_cookie, server_cookie, global_seq,
      connect_seq, io_states.in_seq);
  logger().debug(
      "{} WRITE ReconnectFrame: addrs={}, client_cookie=0x{:x},"
      " server_cookie=0x{:x}, gs={}, cs={}, in_seq={}",
      conn, messenger.get_myaddrs(), client_cookie, server_cookie, global_seq,
      connect_seq, io_states.in_seq);
      ...
      .then([this](auto ret) {
        ...
        case Tag::SESSION_RETRY_GLOBAL:
          return frame_assembler->read_frame_payload().then([this](auto payload) {
            ...
            logger().warn(
                "{} GOT RetryGlobalFrame: gs={}", conn, retry.global_seq());
        ...
        case Tag::SESSION_RESET:
          return frame_assembler->read_frame_payload().then([this](auto payload) {
            if (unlikely(state != state_t::CONNECTING)) {
              logger().debug(
                  "{} triggered {} before reset_session()", conn,
                  get_state_name(state));
              abort_protocol();
            }
        ...
        case Tag::SESSION_RECONNECT_OK:
          return frame_assembler->read_frame_payload().then([this](auto payload) {
            if (unlikely(state != state_t::CONNECTING)) {
              logger().debug(
                  "{} triggered {} at receiving RECONNECT_OK", conn,
                  get_state_name(state));
              abort_protocol();
            }
                ...
                "requeue_out_reconnecting", conn, [this, cc_seq, msg_seq] {
                  return seastar::smp::submit_to(
                      io_handler.get_shard_id(), [this, cc_seq, msg_seq] {
                        return io_handler.requeue_out_sent_up_to(
                            cc_seq, msg_seq);
                      });
                });
...
ProtocolV2::execute_connecting()
{
  ...
  gated_execute("execute_connecting", conn, [this] {
    ...
    if (!conn.policy.lossy && server_cookie != 0) {
      ++connect_seq;
      logger().debug(
          "{} UPDATE: gs={}, cs={} for reconnect", conn, global_seq,
          connect_seq);
    ...
    return wait_exit_io()
        .then([this] {
          ...
          return conn.interceptor
              ->intercept(conn, {Breakpoint{custom_bp_t::SOCKET_CONNECTING}})
        ...
        .then([this] {
          ...
          if (unlikely(state != state_t::CONNECTING)) {
            logger().debug(
                "{} triggered {} before Socket::connect()", conn,
                get_state_name(state));
            abort_protocol();
          }
        ...
        .then([this](SocketRef _new_socket) {
          ...
          if (unlikely(state != state_t::CONNECTING)) {
            logger().debug(
                "{} triggered {} during Socket::connect()", conn,
                get_state_name(state));
          ...
          if (!has_socket) {
            ...
            gate.dispatch_in_background(
                "replace_socket_connecting", conn,
                [this, new_socket = std::move(new_socket)]() mutable {
                  return frame_assembler->replace_shutdown_socket(
                      std::move(new_socket));
                });
          }
          is_socket_valid = true;
        ...
        .then([this](auto&& ret) {
          ...
          if (conn.get_peer_type() != _peer_type) {
            logger().warn(
                "{} connection peer type does not match what peer advertises "
                "{} != {}",
          ...
          if (unlikely(state != state_t::CONNECTING)) {
            logger().debug(
                "{} triggered {} during banner_exchange(), abort", conn,
                get_state_name(state));
            abort_protocol();
          }
          ...
          if (unlikely(_my_addr_from_peer.is_legacy())) {
            logger().warn(
                "{} peer sent a legacy address for me: {}", conn,
                _my_addr_from_peer);
            throw std::system_error(
                make_error_code(crimson::net::error::bad_peer_address));
          }
        ...
        .then([this](next_step_t next) {
          if (unlikely(state != state_t::CONNECTING)) {
            logger().debug(
                "{} triggered {} at the end of execute_connecting()", conn,
                get_state_name(state));
            abort_protocol();
          }
          switch (next) {
          case next_step_t::ready: {
            if (unlikely(state != state_t::CONNECTING)) {
              logger().debug(
                  "{} triggered {} before dispatch_connect(), abort", conn,
                  get_state_name(state));
              abort_protocol();
            }
            ...
            logger().info(
                ...
                conn, global_seq, peer_global_seq, connect_seq, client_cookie,
                server_cookie, io_states,
            ...
            return seastar::smp::submit_to(
                ...
                .then([this, new_io_shard] {
                  ...
                  if (unlikely(state != state_t::CONNECTING)) {
                    logger().debug(
                        "{} triggered {} after dispatch_connect(), abort", conn,
                        get_state_name(state));
                    abort_protocol();
                  }
                  execute_ready();
          ...
          case next_step_t::wait: {
            logger().info(
                "{} execute_connecting(): going to WAIT(max-backoff)", conn);
            ceph_assert_always(is_socket_valid);
            frame_assembler->shutdown_socket<true>(&gate);
            is_socket_valid = false;
            execute_wait(true);
...
ProtocolV2::_auth_bad_method(int r)
{
  ...
  ceph_assert(r < 0);
  auto allowed_methods = messenger.get_auth_server()->get_supported_auth_methods(
      conn.get_peer_type());
  auto allowed_modes = messenger.get_auth_server()->get_supported_con_modes(
  ...
  logger().warn(
      "{} WRITE AuthBadMethodFrame: method={}, result={}, "
      "allowed_methods={}, allowed_modes={})",
      conn, auth_meta->auth_method, cpp_strerror(r), allowed_methods,
      allowed_modes);
...
seastar::future<>
ProtocolV2::_handle_auth_request(bufferlist& auth_payload, bool more)
{
  // _handle_auth_request() logic
  ceph_assert(messenger.get_auth_server());
  bufferlist reply;
  int r = messenger.get_auth_server()->handle_auth_request(
      conn, *auth_meta, more, auth_meta->auth_method, auth_payload,
      &conn.peer_global_id, &reply);
  switch (r) {
  // successful
  case 1: {
    auto auth_done =
        AuthDoneFrame::Encode(conn.peer_global_id, auth_meta->con_mode, reply);
    logger().debug(
        "{} WRITE AuthDoneFrame: gid={}, con_mode={}, payload_len={}", conn,
        conn.peer_global_id, ceph_con_mode_name(auth_meta->con_mode),
        reply.length());
  ...
  case 0: {
    ...
    logger().debug(
        "{} WRITE AuthReplyMoreFrame: payload_len={}", conn, reply.length());
    return frame_assembler->write_flush_frame(more)
        .then([this] { return frame_assembler->read_main_preamble(); })
        .then([this](auto ret) {
          expect_tag(
              Tag::AUTH_REQUEST_MORE, ret.tag, conn, "read_auth_request_more");
          return frame_assembler->read_frame_payload();
        })
        .then([this](auto payload) {
          auto auth_more = AuthRequestMoreFrame::Decode(payload->back());
          logger().debug(
              "{} GOT AuthRequestMoreFrame: payload_len={}", conn,
              auth_more.auth_payload().length());
...
ProtocolV2::server_auth()
{
  return frame_assembler->read_main_preamble()
      ...
      .then([this](auto payload) {
        ...
        logger().debug(
            "{} GOT AuthRequestFrame: method={}, preferred_modes={},"
            " payload_len={}",
        ...
        if (auth_meta->con_mode == CEPH_CON_MODE_UNKNOWN) {
          logger().warn(
              "{} auth_server pick_con_mode returned mode "
...
bool
ProtocolV2::validate_peer_name(const entity_name_t& peer_name) const
{
    ...
    uint64_t msg_seq)
{
  if (unlikely(state != state_t::ACCEPTING)) {
    logger().debug(
        "{} triggered {} before trigger_replacing()", conn,
        get_state_name(state));
    abort_protocol();
  }

  existing_proto->trigger_replacing(
      reconnect, do_reset, frame_assembler->to_replace(), std::move(auth_meta),
      peer_global_seq, client_cookie, conn.get_peer_name(), conn.get_features(),
      peer_supported_features, conn_seq, msg_seq);
...
ProtocolV2::handle_existing_connection(SocketConnectionRef existing_conn)
{
  ...
  ProtocolV2* existing_proto =
      dynamic_cast<ProtocolV2*>(existing_conn->protocol.get());
  ceph_assert(existing_proto);
  logger().debug(
      "{}(gs={}, pgs={}, cs={}, client_cookie=0x{:x}, server_cookie=0x{:x}) "
      "connecting,"
      " found existing {}(state={}, gs={}, pgs={}, cs={}, "
      "client_cookie=0x{:x}, server_cookie=0x{:x})",
      conn, global_seq, peer_global_seq, connect_seq, client_cookie,
      server_cookie, fmt::ptr(existing_conn.get()),
      get_state_name(existing_proto->state), existing_proto->global_seq,
      existing_proto->peer_global_seq, existing_proto->connect_seq,
      existing_proto->client_cookie, existing_proto->server_cookie);

  if (!validate_peer_name(existing_conn->get_peer_name())) {
    logger().error(
        "{} server_connect: my peer_name doesn't match"
        " the existing connection {}, abort",
        conn, fmt::ptr(existing_conn.get()));
    abort_in_fault();
  }

  if (existing_proto->state == state_t::REPLACING) {
    logger().warn(
        "{} server_connect: racing replace happened while"
  ...
  if (existing_proto->peer_global_seq > peer_global_seq) {
    logger().warn(
        ...
        conn, peer_global_seq, existing_proto->peer_global_seq, *existing_conn);
    abort_in_fault();
  }
  ...
  if (existing_conn->policy.lossy) {
    ...
    if (unlikely(state != state_t::ACCEPTING)) {
      logger().debug(
          "{} triggered {} before execute_establishing()", conn,
          get_state_name(state));
      abort_protocol();
    }
  ...
  if (existing_proto->server_cookie != 0) {
    if (existing_proto->client_cookie != client_cookie) {
      ...
      logger().warn(
          ...
          " when existing {} {} is with stale session (client_cookie=0x{:x}, "
          "server_cookie=0x{:x}),"
          " peer must have reset",
          conn, client_cookie, get_state_name(existing_proto->state),
      ...
      // continuing...
      logger().warn(
          "{} server_connect: found client session with existing {} {}"
          " matched (client_cookie=0x{:x}, e_server_cookie=0x{:x}), continuing "
          "session establishment",
          conn, get_state_name(existing_proto->state), *existing_conn,
          client_cookie, existing_proto->server_cookie);
    ...
    if (existing_proto->client_cookie != client_cookie) {
      if (existing_conn->peer_wins()) {
        ...
        logger().warn(
            "{} server_connect: connection race detected"
            " (client_cookie=0x{:x}, e_client_cookie=0x{:x}, server_cookie=0x0)"
            " and win, reusing existing {} {}",
            conn, client_cookie, existing_proto->client_cookie,
            get_state_name(existing_proto->state), *existing_conn);
        return reuse_connection(existing_proto);
      } else {
...
ProtocolV2::server_connect()
{
  return frame_assembler->read_frame_payload().then([this](auto payload) {
    ...
    logger().debug(
        "{} GOT ClientIdentFrame: addrs={}, target={},"
        ...
        client_ident.supported_features(), client_ident.required_features(),
    ...
    if (!messenger.get_myaddrs().contains(client_ident.target_addr())) {
      logger().warn(
          "{} peer is trying to reach {} which is not us ({})", conn,
          client_ident.target_addr(), messenger.get_myaddrs());
      throw std::system_error(
          make_error_code(crimson::net::error::bad_peer_address));
    }
    ...
    if (!conn.policy.lossy && !conn.policy.server &&
        conn.target_addr.get_port() <= 0) {
      logger().warn(
          "{} we don't know how to reconnect to peer {}", conn,
          conn.target_addr);
      throw std::system_error(
          make_error_code(crimson::net::error::bad_peer_address));
    }
    ...
    if (conn.get_peer_id() != entity_name_t::NEW &&
      ...
      logger().error(
          "{} client_ident peer_id ({}) does not match"
    ...
    uint64_t feat_missing = (conn.policy.features_required | msgr2_required) &
                            ~(uint64_t)client_ident.supported_features();
    if (feat_missing) {
      auto ident_missing_features =
          IdentMissingFeaturesFrame::Encode(feat_missing);
      logger().warn(
          "{} WRITE IdentMissingFeaturesFrame: features=0x{:x} (peer missing)",
          conn, feat_missing);
    ...
    if (lossy != conn.policy.lossy) {
      logger().warn(
          "{} my lossy policy {} doesn't match client {}, ignore", conn,
          conn.policy.lossy, lossy);
    }
...
ProtocolV2::server_reconnect()
{
  return frame_assembler->read_frame_payload().then([this](auto payload) {
    ...
    logger().debug(
        "{} GOT ReconnectFrame: addrs={}, client_cookie=0x{:x},"
        " server_cookie=0x{:x}, gs={}, cs={}, msg_seq={}",
        conn, reconnect.addrs(), reconnect.client_cookie(),
        reconnect.server_cookie(), reconnect.global_seq(),
        reconnect.connect_seq(), reconnect.msg_seq());
    ...
    if (conn.peer_addr == entity_addr_t()) {
      ...
      logger().error(
          "{} peer identifies as {}, while conn.peer_addr={},"
          " reconnect failed",
    ...
    if (!existing_conn) {
      ...
      // session
      logger().warn(
          "{} server_reconnect: no existing connection from address {},"
          " reseting client",
          conn, conn.peer_addr);
    ...
    logger().debug(
        ...
        existing_proto->global_seq, existing_proto->peer_global_seq,
        existing_proto->connect_seq, existing_proto->client_cookie,
        existing_proto->server_cookie);

    if (!validate_peer_name(existing_conn->get_peer_name())) {
      logger().error(
          "{} server_reconnect: my peer_name doesn't match"
          " the existing connection {}, abort",
          conn, fmt::ptr(existing_conn.get()));
      abort_in_fault();
    }
    ...
    if (existing_proto->state == state_t::REPLACING) {
      logger().warn(
          "{} server_reconnect: racing replace happened while "
    ...
    if (existing_proto->client_cookie != reconnect.client_cookie()) {
      logger().warn(
          ...
          " e_client_cookie=0x{:x} r_client_cookie=0x{:x}. I must have reset, "
          "reseting client.",
          conn, *existing_conn, existing_proto->client_cookie,
      ...
      //   - a has cookie==0
      logger().warn(
          "{} server_reconnect: I was a client (e_client_cookie=0x{:x}) and "
          "didn't received the"
    ...
    if (existing_proto->peer_global_seq > reconnect.global_seq()) {
      logger().warn(
          "{} server_reconnect: stale global_seq: exist_pgs({}) > peer_gs({}),"
          " with existing connection {},"
          " ask client to retry global",
          conn, existing_proto->peer_global_seq, reconnect.global_seq(),
          *existing_conn);
      return send_retry_global(existing_proto->peer_global_seq);
    }
    ...
    if (existing_proto->connect_seq > reconnect.connect_seq()) {
      logger().warn(
          "{} server_reconnect: stale peer connect_seq peer_cs({}) < "
          "exist_cs({}),"
          " with existing connection {}, ask client to retry",
          conn, reconnect.connect_seq(), existing_proto->connect_seq,
          *existing_conn);
      ...
      if (existing_conn->peer_wins()) {
        // acceptor (this connection, the peer) wins
        logger().warn(
            "{} server_reconnect: reconnect race detected (cs={})"
            " and win, reusing existing {} {}",
            conn, reconnect.connect_seq(),
            get_state_name(existing_proto->state), *existing_conn);
        return reuse_connection(
            existing_proto, false, true, reconnect.connect_seq(),
            reconnect.msg_seq());
      } else {
        // acceptor (this connection, the peer) loses
        logger().warn(
      ...
      logger().warn(
          "{} server_reconnect: stale exsiting connect_seq exist_cs({}) < "
          "peer_cs({}),"
          " reusing existing {} {}",
    ...
    return seastar::futurize_invoke([this] {
        ...
        .then([this](auto&& ret) {
          ...
          logger().info(
              "{} UPDATE: peer_type={},"
              " policy(lossy={} server={} standby={} resetcheck={})",
              conn, ceph_entity_type_name(_peer_type), conn.policy.lossy,
              conn.policy.server, conn.policy.standby, conn.policy.resetcheck);
          if (!messenger.get_myaddr().is_blank_ip() &&
              (messenger.get_myaddr().get_port() !=
                   _my_addr_from_peer.get_port() ||
               messenger.get_myaddr().get_nonce() !=
                   _my_addr_from_peer.get_nonce())) {
            logger().warn(
                "{} my_addr_from_peer {} port/nonce doesn't match myaddr {}",
        ...
        .then([this](next_step_t next) {
          ...
          case next_step_t::wait:
            if (unlikely(state != state_t::ACCEPTING)) {
              logger().debug(
                  "{} triggered {} at the end of execute_accepting()", conn,
                  get_state_name(state));
              abort_protocol();
            }
        ...
        .handle_exception([this](std::exception_ptr eptr) {
          ...
          logger().info(
              "{} execute_accepting(): fault at {}, going to CLOSING -- {}",
...
ProtocolV2::finish_auth()
{
  ...
  return frame_assembler->write_flush_frame(sig_frame)
      ...
      .then([this, txbuf = std::move(records.txbuf)](auto payload) {
        ...
        logger().debug(
            "{} GOT AuthSignatureFrame: signature={}", conn,
            sig_frame.signature());

        const auto actual_tx_sig =
            auth_meta->session_key.empty()
                ? sha256_digest_t()
                : auth_meta->session_key.hmac_sha256(nullptr, txbuf);
        if (sig_frame.signature() != actual_tx_sig) {
          logger().warn(
              "{} pre-auth signature mismatch actual_tx_sig={}"
              " sig_frame.signature()={}",
...
void
ProtocolV2::execute_establishing(SocketConnectionRef existing_conn)
{
  auto accept_me = [this] {
    messenger.register_conn(seastar::static_pointer_cast<SocketConnection>(
  ...
  if (existing_conn) {
    logger().info(
        "{} start establishing: gs={}, pgs={}, cs={}, "
        "client_cookie=0x{:x}, server_cookie=0x{:x}, {}, new_sid={}, "
        "close existing {}",
        conn, global_seq, peer_global_seq, connect_seq, client_cookie,
        server_cookie, io_states, frame_assembler->get_socket_shard_id(),
        *existing_conn);
    is_replace = true;
    ProtocolV2* existing_proto =
        dynamic_cast<ProtocolV2*>(existing_conn->protocol.get());
    ...
    if (unlikely(state != state_t::ESTABLISHING)) {
      logger().warn(
          "{} triggered {} during execute_establishing(), "
  ...
  gated_execute("execute_establishing", conn, [this, is_replace] {
    ...
    auto new_io_shard = frame_assembler->get_socket_shard_id();
    logger().debug(
        "{} send {} IOHandler::dispatch_accept({})", conn, cc_seq, new_io_shard);
    ...
    return seastar::smp::submit_to(
        ...
        .then([this, new_io_shard] {
          ...
          if (unlikely(state != state_t::ESTABLISHING)) {
            logger().debug(
                "{} triggered {} after dispatch_accept() during "
        ...
        .then([this] {
          if (unlikely(state != state_t::ESTABLISHING)) {
            logger().debug(
                "{} triggered {} at the end of execute_establishing()", conn,
                get_state_name(state));
            abort_protocol();
          }
...
{
  ceph_assert_always(
      state == state_t::ESTABLISHING || state == state_t::REPLACING);
  ...
  gate.dispatch_in_background("reset_peer_state", conn, [this, cc_seq] {
    return seastar::smp::submit_to(io_handler.get_shard_id(), [this, cc_seq] {
  ...
  auto server_ident = ServerIdentFrame::Encode(
      messenger.get_myaddrs(), messenger.get_myname().num(), global_seq,
      conn.policy.features_supported,
      conn.policy.features_required | msgr2_required, flags, server_cookie);

  logger().debug(
      "{} WRITE ServerIdentFrame: addrs={}, gid={},"
      " gs={}, features_supported=0x{:x}, features_required=0x{:x},"
      " flags=0x{:x}, server_cookie=0x{:x}",
      conn, messenger.get_myaddrs(), messenger.get_myname().num(), global_seq,
      conn.policy.features_supported,
      conn.policy.features_required | msgr2_required, flags, server_cookie);
    ...
    uint64_t new_msg_seq)
{
  ...
  logger().info(
      "{} start replacing ({}): pgs was {}, cs was {}, "
      "client_cookie was 0x{:x}, {}, new_sid={}",
      conn, reconnect ? "reconnected" : "connected", peer_global_seq,
      connect_seq, client_cookie, io_states, mover.socket->get_shard_id());
  if (is_socket_valid) {
    frame_assembler->shutdown_socket<true>(&gate);
    is_socket_valid = false;
  }
  ...
  gate.dispatch_in_background(
      "trigger_replacing", conn,
      [this, reconnect, do_reset, mover = std::move(mover),
       new_auth_meta = std::move(new_auth_meta), new_client_cookie,
       new_peer_name, new_conn_features, new_peer_supported_features,
       new_peer_global_seq, new_connect_seq, new_msg_seq]() mutable {
        ...
        return wait_switch_io_shard()
            ...
            .then([this, new_io_shard] {
              ...
              crosscore.prepare_submit();
              logger().debug(
                  "{} send {} IOHandler::dispatch_accept({})", conn, cc_seq,
                  new_io_shard);
              ConnectionFRef conn_fref =
                  seastar::make_foreign(conn.shared_from_this());
              ceph_assert_always(!pr_switch_io_shard.has_value());
              ...
              return seastar::smp::submit_to(
                  ...
                  .then([this, new_io_shard] {
                    ceph_assert_always(
                        io_handler.get_shard_id() == new_io_shard);
                    pr_switch_io_shard->set_value();
                    pr_switch_io_shard = std::nullopt;
                    // user can make changes
                  });
            })
            .then([this, reconnect, do_reset, mover = std::move(mover),
                   new_auth_meta = std::move(new_auth_meta), new_client_cookie,
                   new_peer_name, new_conn_features,
                   new_peer_supported_features, new_peer_global_seq,
              ...
              if (unlikely(state != state_t::REPLACING)) {
                logger().debug(
                    "{} triggered {} in the middle of trigger_replacing(), "
                    "abort",
                    conn, get_state_name(state));
                ceph_assert_always(state == state_t::CLOSING);
                return mover.socket->close().then(
                    [sock = std::move(mover.socket)] { abort_protocol(); });
              }

              auth_meta = std::move(new_auth_meta);
              peer_global_seq = new_peer_global_seq;
              gate.dispatch_in_background(
                  "replace_frame_assembler", conn,
              ...
              if (reconnect) {
                ...
                auto cc_seq = crosscore.prepare_submit();
                logger().debug(
                    "{} send {} IOHandler::requeue_out_sent_up_to({})", conn,
                    cc_seq, new_msg_seq);
                io_states.requeue_out_sent_up_to();
                gate.dispatch_in_background(
                ...
                logger().debug(
                    "{} WRITE ReconnectOkFrame: msg_seq={}", conn,
                    io_states.in_seq);
                ...
                bool is_rev1 =
                    HAVE_MSGR2_FEATURE(peer_supported_features, REVISION_1);
                frame_assembler->set_is_rev1(is_rev1);
                return send_server_ident();
              }
            })
            .then([this, reconnect] {
              if (unlikely(state != state_t::REPLACING)) {
                logger().debug(
                    "{} triggered {} at the end of trigger_replacing(), abort",
                    conn, get_state_name(state));
                ceph_assert_always(state == state_t::CLOSING);
                abort_protocol();
              }
              logger().info(
                  "{} replaced ({}), going to ready: "
                  "gs={}, pgs={}, cs={}, "
                  "client_cookie=0x{:x}, server_cookie=0x{:x}, {}",
                  conn, reconnect ? "reconnected" : "connected", global_seq,
                  peer_global_seq, connect_seq, client_cookie, server_cookie,
                  io_states);
              execute_ready();
            })
            .handle_exception([this](std::exception_ptr eptr) {
    ...
    io_handler_state _io_states)
{
  ...
  if (!crosscore.proceed_or_wait(cc_seq)) {
    logger().debug(
        "{} got {} notify_out_fault(), wait at {}", conn, cc_seq,
        crosscore.get_in_seq());
    return crosscore.wait(cc_seq).then([this, cc_seq, where, eptr, _io_states] {
  ...
  logger().debug(
      "{} got {} notify_out_fault(): io_states={}", conn, cc_seq, io_states);
  fault(state_t::READY, where, eptr);
  return seastar::now();
}

void
ProtocolV2::execute_ready()
{
...
seastar::future<>
ProtocolV2::notify_out(cc_seq_t cc_seq)
{
  assert(seastar::this_shard_id() == conn.get_messenger_shard_id());
  if (!crosscore.proceed_or_wait(cc_seq)) {
    logger().debug(
        "{} got {} notify_out(), wait at {}", conn, cc_seq,
        crosscore.get_in_seq());
    return crosscore.wait(cc_seq).then([this, cc_seq] {
      return notify_out(cc_seq);
    });
  }

  logger().debug(
      "{} got {} notify_out(): at {}", conn, cc_seq, get_state_name(state));
  io_states.is_out_queued = true;
  if (unlikely(state == state_t::STANDBY && !conn.policy.server)) {
    logger().info(
        "{} notify_out(): at {}, going to CONNECTING", conn,
        get_state_name(state));
    execute_connecting();
  }
  return seastar::now();
...
void
ProtocolV2::execute_wait(bool max_backoff)
{
  ceph_assert_always(!is_socket_valid);
  ...
  gated_execute("execute_wait", conn, [this, max_backoff] {
    ...
    if (max_backoff) {
      ...
      backoff =
          std::min(local_conf().get_val<double>("ms_max_backoff"), 2 * backoff);
    } else {
    ...
    return protocol_timer.backoff(backoff)
        .then([this] {
          if (unlikely(state != state_t::WAIT)) {
            logger().debug(
                "{} triggered {} at the end of execute_wait()", conn,
                get_state_name(state));
            abort_protocol();
          }
        ...
        .handle_exception([this](std::exception_ptr eptr) {
          ...
          logger().info(
              "{} execute_wait(): protocol aborted at {} -- {}", conn,
              get_state_name(state), e_what);
...
void
ProtocolV2::execute_server_wait()
{
  ceph_assert_always(is_socket_valid);
  ...
  gated_execute("execute_server_wait", conn, [this] {
    return frame_assembler->read_exactly(1)
        ...
        .handle_exception([this](std::exception_ptr eptr) {
          ...
          logger().info(
              "{} execute_server_wait(): fault at {}, going to CLOSING -- {}",
              conn, get_state_name(state), e_what);
          do_close(false);
        });
  });
...
seastar::future<>
ProtocolV2::notify_mark_down(cc_seq_t cc_seq)
{
  assert(seastar::this_shard_id() == conn.get_messenger_shard_id());
  if (!crosscore.proceed_or_wait(cc_seq)) {
    logger().debug(
        "{} got {} notify_mark_down(), wait at {}", conn, cc_seq,
        crosscore.get_in_seq());
    return crosscore.wait(cc_seq).then([this, cc_seq] {
    ...
    std::optional<std::function<void()>> f_accept_new)
{
  ...
  logger().info(
      "{} closing: reset {}, replace {}", conn,
      is_dispatch_reset ? "yes" : "no", is_replace ? "yes" : "no");
  ...
  gate.dispatch_in_background(
      "close_io", conn, [this, is_dispatch_reset, is_replace] {
        // this is preemptive
        return wait_switch_io_shard().then([this, is_dispatch_reset,
                                            is_replace] {
          trigger_state_phase2(state_t::CLOSING, io_state_t::drop);
          auto cc_seq = crosscore.prepare_submit();
          logger().debug(
              "{} send {} IOHandler::close_io(reset={}, replace={})", conn,
              cc_seq, is_dispatch_reset, is_replace);
          ...
          std::ignore =
              gate.close()
                  ...
                  .handle_exception([conn_ref = conn.shared_from_this(),
                                     this](auto eptr) {
                    logger().error(
                        "{} closing got unexpected exception {}", conn, eptr);
                    ceph_abort();
                  });
