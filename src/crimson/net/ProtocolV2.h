...
private:
  seastar::future<> notify_out(cc_seq_t cc_seq) final;
  ...
  seastar::future<> notify_mark_down(cc_seq_t cc_seq) final;
...
private:
  ...
  get_state_name(state_t state)
  {
    const char* const statenames[] = {
        "NONE",  "ACCEPTING", "SERVER_WAIT", "ESTABLISHING", "CONNECTING",
        "READY", "STANDBY",   "WAIT",        "REPLACING",    "CLOSING"};
  ...
  void
  trigger_state(state_t new_state, io_state_t new_io_state)
  {
  ...
  gated_execute(const char* what, T& who, Func&& func)
  {
    gate.dispatch_in_background(what, who, [this, &who, &func] {
      ...
      return seastar::futurize_invoke(std::forward<Func>(func))
          .finally([pr = std::move(pr)]() mutable { pr.set_value(); });
    });
  }
  ...
  seastar::future<std::tuple<entity_type_t, entity_addr_t>> banner_exchange(
      bool is_connect);
  ...
  seastar::future<next_step_t> reuse_connection(
  ...
  seastar::future<next_step_t> handle_existing_connection(
...
inline create_handlers_ret
create_handlers(ChainedDispatchers& dispatchers, SocketConnection& conn)
{
  std::unique_ptr<ConnectionHandler> io_handler =
      std::make_unique<IOHandler>(dispatchers, conn);
...
template <>
struct fmt::formatter<crimson::net::ProtocolV2> : fmt::ostream_formatter {};
#endif
