  ...
  return try_trap_pre(next_trap_read)
      ...
      .then([this](auto buf) {
        return try_trap_post(next_trap_read)
            .then([buf = std::move(buf)]() mutable { return std::move(buf); });
      });
#endif
}
  ...
  return try_trap_pre(next_trap_read)
      .then([bytes, this] {
        ...
        return in.read_exactly(bytes).then([bytes](auto buf) {
          ...
          inject_failure();
          return inject_delay().then([ptr = std::move(ptr)]() mutable {
      ...
      .then([this](auto ptr) {
        return try_trap_post(next_trap_read)
            .then([ptr = std::move(ptr)]() mutable { return std::move(ptr); });
      });
#endif
}
...
Socket::close()
{
  ...
  return seastar::when_all_succeed(
             inject_delay(), in.close(), close_and_handle_errors(out))
...
Socket::connect(const entity_addr_t& peer_addr)
{
  ...
  return inject_delay()
      ...
      .then([peer_addr](seastar::connected_socket socket) {
        ...
        logger().debug(
            "Socket::connect(): connected to {}, socket {}", peer_addr,
            fmt::ptr(ret.get()));
        return ret;
      });
}
...
#ifdef UNIT_TESTS_BUILT
void
Socket::set_trap(bp_type_t type, bp_action_t action, socket_blocker* blocker_)
{
  assert(seastar::this_shard_id() == sid);
  blocker = blocker_;
  if (type == bp_type_t::READ) {
    ...
    if (next_trap_write == bp_action_t::CONTINUE) {
      ...
      ceph_assert_always(
          action == bp_action_t::FAULT || action == bp_action_t::CONTINUE);
    } else {
      ceph_abort();
    }
  }
  ...
  return this->container().invoke_on_all([_fn_accept](auto& ss) {
    ...
    std::ignore = seastar::with_gate(ss.shutdown_gate, [&ss] {
      return seastar::keep_doing([&ss] {
               return ss.listener->accept().then([&ss](seastar::accept_result
                                                           accept_result) {
#ifndef NDEBUG
                 if (ss.dispatch_only_on_primary_sid) {
                   // see seastar::listen_options::set_fixed_cpu()
                   ceph_assert_always(
                       seastar::this_shard_id() == ss.primary_sid);
                 }
#endif
                 ...
                 logger().debug(
                     "ShardedServerSocket({})::accept(): accepted peer {}, "
          ...
          .handle_exception_type([&ss](const std::system_error& e) {
            if (e.code() == std::errc::connection_aborted ||
              ...
              logger().debug(
                  "ShardedServerSocket({})::accept(): stopped ({})",
