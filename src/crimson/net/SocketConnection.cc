...
bool
SocketConnection::is_connected() const
{
...
#ifdef UNIT_TESTS_BUILT
bool
SocketConnection::is_protocol_ready() const
{
...
bool
SocketConnection::is_protocol_standby() const
{
  assert(seastar::this_shard_id() == msgr_sid);
...
bool
SocketConnection::is_protocol_closed() const
{
  assert(seastar::this_shard_id() == msgr_sid);
...
bool
SocketConnection::is_protocol_closed_clean() const
{
  assert(seastar::this_shard_id() == msgr_sid);
...
#endif
bool
SocketConnection::peer_wins() const
{
  assert(seastar::this_shard_id() == msgr_sid);
...
seastar::future<>
SocketConnection::send(MessageURef msg)
{
...
void
SocketConnection::set_last_keepalive_ack(clock_t::time_point when)
{
  io_handler->set_last_keepalive_ack(when);
}

void
SocketConnection::mark_down()
{
...
void
SocketConnection::start_accept(SocketFRef&& sock, const entity_addr_t& _peer_addr)
{
  assert(seastar::this_shard_id() == msgr_sid);
...
void
SocketConnection::set_peer_type(entity_type_t peer_type)
{
  assert(seastar::this_shard_id() == msgr_sid);
  ...
  assert(!(
      peer_type != 0 && peer_name.type() != 0 && peer_type != peer_name.type()));
  peer_name._type = peer_type;
}

void
SocketConnection::set_peer_id(int64_t peer_id)
{
  assert(seastar::this_shard_id() == msgr_sid);
  // it is not allowed to assign an unknown value when the current
  // value is known
  assert(!(
      peer_id == entity_name_t::NEW && peer_name.num() != entity_name_t::NEW));
  // it is not allowed to assign a different known value when the
  // current value is also known.
  assert(
      !(peer_id != entity_name_t::NEW &&
        peer_name.num() != entity_name_t::NEW && peer_id != peer_name.num()));
  peer_name._num = peer_id;
}

void
SocketConnection::set_features(uint64_t f)
{
  assert(seastar::this_shard_id() == msgr_sid);
  features = f;
}
...
void
SocketConnection::set_socket(Socket* s)
{
  assert(seastar::this_shard_id() == msgr_sid);
  socket = s;
}
...
void
SocketConnection::print(ostream& out) const
{
  out << (void*)this << " ";
  messenger.print(out);
  if (seastar::this_shard_id() != msgr_sid) {
    ...
    out << "@" << socket->get_ephemeral_port() << " >> " << get_peer_name()
        << " " << peer_addr;
  }
}
