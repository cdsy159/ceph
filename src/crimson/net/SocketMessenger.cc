...
bool
SocketMessenger::set_addr_unknowns(const entity_addrvec_t& addrs)
{
...
void
SocketMessenger::set_myaddrs(const entity_addrvec_t& addrs)
{
  assert(seastar::this_shard_id() == sid);
  my_addrs = addrs;
  ...
  return seastar::futurize_invoke([this] {
           if (!listener) {
             return ShardedServerSocket::create(dispatch_only_on_sid)
                 .then([this](auto _listener) { listener = _listener; });
           } else {
             return seastar::now();
           }
         })
                 ...
                 .safe_then(
                     [this]() -> seastar::future<std::optional<std::error_code>> {
                       ...
                       return seastar::make_ready_future<
                           ...
                           std::make_optional<std::error_code>(std::error_code{
                               /* success! */}));
                     },
                     listen_ertr::all_same_way(
                         [this, max_port,
                          &port](const std::error_code& e) mutable
                         -> seastar::future<std::optional<std::error_code>> {
                           logger().trace(
                               "{} try_bind: {} got error {}", *this, port, e);
                           if (port == max_port) {
                             return seastar::make_ready_future<
                                 std::optional<std::error_code>>(
...
SocketMessenger::bind(const entity_addrvec_t& addrs)
{
  ...
  return seastar::do_with(
      int64_t{local_conf()->ms_bind_retry_count}, [this, addrs](auto& count) {
        return seastar::repeat_until_value([this, addrs, &count] {
                 assert(count >= 0);
                 return try_bind(
                            addrs, local_conf()->ms_bind_port_min,
                     ...
                     .safe_then(
                         [this] {
                           ...
                           return seastar::make_ready_future<
                               std::optional<std::error_code>>(
                               std::make_optional<std::error_code>(
                                   std::error_code{/* success! */}));
                         },
                         bind_ertr::all_same_way([this, &count](
                           ...
                           if (count-- > 0) {
                             ...
                             logger().info(
                                 "{} was unable to bind after {} attempts: {}",
                                 *this, local_conf()->ms_bind_retry_count,
                                 error);
                             return seastar::make_ready_future<
                                 std::optional<std::error_code>>(
...
seastar::future<>
SocketMessenger::accept(SocketFRef&& socket, const entity_addr_t& peer_addr)
{
  assert(seastar::this_shard_id() == sid);
...
seastar::future<>
SocketMessenger::start(const dispatchers_t& _dispatchers)
{
  assert(seastar::this_shard_id() == sid);
    ...
    return listener->accept([this](SocketRef _socket, entity_addr_t peer_addr) {
      ...
      if (listener->is_fixed_shard_dispatching()) {
        ...
        return seastar::smp::submit_to(
            sid, [this, peer_addr, socket = std::move(socket)]() mutable {
              return accept(std::move(socket), peer_addr);
            });
      }
    });
...
choose_addr(const entity_addr_t& peer_addr_for_me, const SocketConnection& conn)
{
  using crimson::common::local_conf;
  ...
  if (const auto local_addr = conn.get_local_address();
    ...
    logger().info(
        "{} peer {} says I am {} (socket says {})", conn,
        conn.get_peer_socket_addr(), peer_addr_for_me, local_addr);
    return peer_addr_for_me;
  } else {
    const auto local_addr_for_me = conn.get_local_address();
    logger().info(
        "{} socket to {} says I am {} (peer says {})", conn,
    ...
    const SocketConnection& conn)
{
  ...
  if (!need_addr) {
    if ((!get_myaddr().is_any() &&
      ...
      logger().warn(
          "{} peer_addr_for_me {} type/family/IP doesn't match myaddr {}", conn,
          peer_addr_for_me, get_myaddr());
  ...
  if (get_myaddr().get_type() == entity_addr_t::TYPE_NONE) {
    ...
    if (!get_myaddr().is_any() &&
        get_myaddr().get_type() != peer_addr_for_me.get_type()) {
      logger().warn(
          "{} peer_addr_for_me {} type doesn't match myaddr {}", conn,
          peer_addr_for_me, get_myaddr());
      throw std::system_error(
          make_error_code(crimson::net::error::bad_peer_address));
    }
    if (get_myaddr().get_family() != peer_addr_for_me.get_family()) {
      logger().warn(
          "{} peer_addr_for_me {} family doesn't match myaddr {}", conn,
          peer_addr_for_me, get_myaddr());
      throw std::system_error(
          make_error_code(crimson::net::error::bad_peer_address));
    }
    if (get_myaddr().is_blank_ip()) {
      ...
      logger().warn(
          "{} peer_addr_for_me {} IP doesn't match myaddr {}", conn,
          peer_addr_for_me, get_myaddr());
      throw std::system_error(
          make_error_code(crimson::net::error::bad_peer_address));
    } else {
...
SocketPolicy
SocketMessenger::get_policy(entity_type_t peer_type) const
{
  assert(seastar::this_shard_id() == sid);
...
void
SocketMessenger::set_default_policy(const SocketPolicy& p)
{
  assert(seastar::this_shard_id() == sid);
...
void
SocketMessenger::set_policy(entity_type_t peer_type, const SocketPolicy& p)
{
  assert(seastar::this_shard_id() == sid);
...
void
SocketMessenger::set_policy_throttler(entity_type_t peer_type, Throttle* throttle)
{
  assert(seastar::this_shard_id() == sid);
...
void
SocketMessenger::accept_conn(SocketConnectionRef conn)
{
  assert(seastar::this_shard_id() == sid);
...
void
SocketMessenger::unaccept_conn(SocketConnectionRef conn)
{
  assert(seastar::this_shard_id() == sid);
...
void
SocketMessenger::register_conn(SocketConnectionRef conn)
{
  assert(seastar::this_shard_id() == sid);
...
void
SocketMessenger::unregister_conn(SocketConnectionRef conn)
{
  assert(seastar::this_shard_id() == sid);
  ceph_assert(conn);
...
void
SocketMessenger::closing_conn(SocketConnectionRef conn)
{
  assert(seastar::this_shard_id() == sid);
...
void
SocketMessenger::closed_conn(SocketConnectionRef conn)
{
  assert(seastar::this_shard_id() == sid);
