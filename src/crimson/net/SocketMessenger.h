...
public:
  ...
  const entity_name_t&
  get_myname() const override
  {
    return my_name;
  }

  const entity_addrvec_t&
  get_myaddrs() const override
  {
    return my_addrs;
  }
  ...
  void
  set_auth_client(crimson::auth::AuthClient* ac) override
  {
  ...
  void
  set_auth_server(crimson::auth::AuthServer* as) override
  {
    assert(seastar::this_shard_id() == sid);
    auth_server = as;
  }
  ...
  bool
  owns_connection(Connection& conn) const override
  {
    assert(seastar::this_shard_id() == sid);
  ...
  {
    out << get_myname() << "(" << logic_name << ") " << get_myaddr();
  }
...
public:
  crimson::auth::AuthClient*
  get_auth_client() const
  {
    assert(seastar::this_shard_id() == sid);
    return auth_client;
  }
  ...
  void
  set_interceptor(Interceptor* i) override
  {
    interceptor = i;
  }
  ...
  seastar::future<>
  mark_down(const entity_addr_t& a) final
  {
...
template <>
struct fmt::formatter<crimson::net::SocketMessenger> : fmt::ostream_formatter {
};
#endif
