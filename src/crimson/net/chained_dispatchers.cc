...
seastar::future<>
ChainedDispatchers::ms_dispatch(ConnectionRef conn, MessageRef m)
{
  try {
      ...
      if (dispatched.has_value()) {
        return std::move(*dispatched)
            .handle_exception([conn](std::exception_ptr eptr) {
              logger().error(
                  "{} got unexpected exception in ms_dispatch() throttling {}",
                  *conn, eptr);
              ceph_abort();
            });
      }
    ...
    logger().error(
        "{} got unexpected exception in ms_dispatch() {}", *conn,
    ...
    bool ac)
{
  try {
    ...
    logger().error(
        "{} got unexpected exception in ms_handle_shard_change() {}", *conn,
        std::current_exception());
    ceph_abort();
  }
}
    ...
    bool is_replace)
{
  try {
    ...
    logger().error(
        "{} got unexpected exception in ms_handle_accept() {}", *conn,
        std::current_exception());
    ceph_abort();
  }
}
    ...
    seastar::shard_id prv_shard)
{
  try {
    ...
    logger().error(
        "{} got unexpected exception in ms_handle_connect() {}", *conn,
        std::current_exception());
    ceph_abort();
  }
}
...
ChainedDispatchers::ms_handle_reset(ConnectionRef conn, bool is_replace)
{
  try {
    ...
    logger().error(
        "{} got unexpected exception in ms_handle_reset() {}", *conn,
        std::current_exception());
    ceph_abort();
  }
}
...
ChainedDispatchers::ms_handle_remote_reset(ConnectionRef conn)
{
  try {
    ...
    logger().error(
        "{} got unexpected exception in ms_handle_remote_reset() {}", *conn,
        std::current_exception());
    ceph_abort();
  }
}
