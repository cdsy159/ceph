...
std::size_t
get_msg_size(const FrameAssembler& rx_frame_asm)
{
...
IOHandler::IOHandler(ChainedDispatchers& dispatchers, SocketConnection& conn) :
  shard_states(
      shard_states_t::create(seastar::this_shard_id(), io_state_t::delay)),
    ...
    bool require_ack)
{
  ...
  std::for_each(
      out_pending_msgs.begin(), out_pending_msgs.begin() + num_msgs,
      [this, &bl
#ifdef UNIT_TESTS_BUILT
       ,
       &tags
        ...
        ceph_msg_header2 header2{header.seq,      header.tid,
                                 ...
                                 header.data_off, ceph_le64(in_seq),
                                 footer.flags,    header.compat_version,
                                 header.reserved};

        auto message = MessageFrame::Encode(
            header2, msg->get_payload(), msg->get_middle(), msg->get_data());
        logger().debug(
            "{} --> #{} === {} ({})", conn, msg->get_seq(), *msg,
            msg->get_type());
  ...
  if (!conn.policy.lossy) {
    out_sent_msgs.insert(
        out_sent_msgs.end(), std::make_move_iterator(out_pending_msgs.begin()),
...
seastar::future<>
IOHandler::send(MessageURef _msg)
{
  ...
  logger().debug(
      "{} send {} send() to core {} -- {}", conn, cc_seq, get_shard_id(), *msg);
...
seastar::future<>
IOHandler::do_send(cc_seq_t cc_seq, core_id_t source_core, MessageFRef msg)
{
  assert(seastar::this_shard_id() == get_shard_id());
  if (io_crosscore.proceed_or_wait(cc_seq, source_core)) {
    logger().debug(
        "{} got {} do_send() from core {} -- {}", conn, cc_seq, source_core,
        *msg);
    ...
    logger().debug(
        "{} got {} do_send() from core {}, wait at {} -- {}", conn, cc_seq,
        source_core, io_crosscore.get_in_seq(source_core), *msg);
...
IOHandler::send_keepalive()
{
  ...
  auto source_core = seastar::this_shard_id();
  logger().debug(
      "{} send {} send_keepalive() to core {}", conn, cc_seq, get_shard_id());
  // sid may be changed on-the-fly during the submission
  if (source_core == get_shard_id()) {
    return do_send_keepalive(cc_seq, source_core);
  } else {
    return seastar::smp::submit_to(get_shard_id(), [this, cc_seq, source_core] {
...
seastar::future<>
IOHandler::send_keepalive_recheck_shard(cc_seq_t cc_seq, core_id_t source_core)
{
  // sid may be changed on-the-fly during the submission
  if (seastar::this_shard_id() == get_shard_id()) {
    return do_send_keepalive(cc_seq, source_core);
    ...
    logger().debug(
        "{} send {} send_keepalive_recheck_shard() "
        "redirected from core {} to {}",
...
seastar::future<>
IOHandler::do_send_keepalive(cc_seq_t cc_seq, core_id_t source_core)
{
  assert(seastar::this_shard_id() == get_shard_id());
  if (io_crosscore.proceed_or_wait(cc_seq, source_core)) {
    logger().debug(
        "{} got {} do_send_keeplive() from core {}: need_keepalive={}", conn,
        cc_seq, source_core, need_keepalive);
    if (!need_keepalive) {
      need_keepalive = true;
      notify_out_dispatch();
    }
    ...
    logger().debug(
        "{} got {} do_send_keepalive() from core {}, wait at {}", conn, cc_seq,
        source_core, io_crosscore.get_in_seq(source_core));
    return io_crosscore.wait(cc_seq, source_core)
...
IOHandler::mark_down()
{
  ...
  logger().info(
      "{} mark_down() at {}, send {} notify_mark_down()", conn,
      io_stat_printer{*this}, cc_seq);
  do_set_io_state(io_state_t::drop);
  shard_states->dispatch_in_background("notify_mark_down", conn, [this, cc_seq] {
    return seastar::smp::submit_to(conn.get_messenger_shard_id(), [this, cc_seq] {
...
void
IOHandler::print_io_stat(std::ostream& out) const
{
  assert(seastar::this_shard_id() == get_shard_id());
  out << "io_stat(" << "io_state=" << fmt::format("{}", get_io_state())
      << ", in_seq=" << in_seq << ", out_seq=" << out_seq
...
void
IOHandler::assign_frame_assembler(FrameAssemblerV2Ref fa)
{
  assert(fa != nullptr);
    ...
    bool set_notify_out)
{
  ...
  logger().debug(
      "{} got {} do_set_io_state(): prv_state={}, new_state={}, "
      "fa={}, set_notify_out={}, at {}",
      conn, cc_seq.has_value() ? fmt::format("{} ", *cc_seq) : "", prv_state,
      new_state, fa ? "present" : "N/A", set_notify_out,
    ...
    bool set_notify_out)
{
  ...
  if (!proto_crosscore.proceed_or_wait(cc_seq)) {
    logger().debug(
        "{} got {} set_io_state(), wait at {}", conn, cc_seq,
        proto_crosscore.get_in_seq());
    return proto_crosscore.wait(cc_seq).then([this, cc_seq, new_state,
                                              fa = std::move(fa),
                                              set_notify_out]() mutable {
...
IOHandler::wait_io_exit_dispatching(cc_seq_t cc_seq)
{
  assert(seastar::this_shard_id() == get_shard_id());
  if (!proto_crosscore.proceed_or_wait(cc_seq)) {
    logger().debug(
        "{} got {} wait_io_exit_dispatching(), wait at {}", conn, cc_seq,
        proto_crosscore.get_in_seq());
    return proto_crosscore.wait(cc_seq).then([this, cc_seq] {
  ...
  return seastar::futurize_invoke([this] {
           ...
           if (maybe_dropped_sid.has_value()) {
             ...
             return seastar::smp::submit_to(prv_sid, [this] {
               logger().debug(
                   "{} got wait_io_exit_dispatching from prv_sid", conn);
      ...
      .then([this] {
        logger().debug(
            "{} finish wait_io_exit_dispatching at {}", conn,
            io_stat_printer{*this});
        ceph_assert_always(frame_assembler != nullptr);
        ceph_assert_always(!frame_assembler->is_socket_valid());
...
seastar::future<>
IOHandler::reset_session(cc_seq_t cc_seq, bool full)
{
  assert(seastar::this_shard_id() == get_shard_id());
  if (!proto_crosscore.proceed_or_wait(cc_seq)) {
    logger().debug(
        "{} got {} reset_session(), wait at {}", conn, cc_seq,
        proto_crosscore.get_in_seq());
    return proto_crosscore.wait(cc_seq).then([this, cc_seq, full] {
...
seastar::future<>
IOHandler::reset_peer_state(cc_seq_t cc_seq)
{
  assert(seastar::this_shard_id() == get_shard_id());
  if (!proto_crosscore.proceed_or_wait(cc_seq)) {
    logger().debug(
        "{} got {} reset_peer_state(), wait at {}", conn, cc_seq,
        proto_crosscore.get_in_seq());
    return proto_crosscore.wait(cc_seq).then([this, cc_seq] {
...
seastar::future<>
IOHandler::requeue_out_sent(cc_seq_t cc_seq)
{
  assert(seastar::this_shard_id() == get_shard_id());
  if (!proto_crosscore.proceed_or_wait(cc_seq)) {
    logger().debug(
        "{} got {} requeue_out_sent(), wait at {}", conn, cc_seq,
        proto_crosscore.get_in_seq());
    return proto_crosscore.wait(cc_seq).then([this, cc_seq] {
...
IOHandler::do_requeue_out_sent()
{
  ...
  logger().debug(
      "{} requeue {} items, revert out_seq to {}", conn, out_sent_msgs.size(),
      out_seq);
  ...
  out_pending_msgs.insert(
      out_pending_msgs.begin(), std::make_move_iterator(out_sent_msgs.begin()),
...
seastar::future<>
IOHandler::requeue_out_sent_up_to(cc_seq_t cc_seq, seq_num_t msg_seq)
{
  assert(seastar::this_shard_id() == get_shard_id());
  if (!proto_crosscore.proceed_or_wait(cc_seq)) {
    logger().debug(
        "{} got {} requeue_out_sent_up_to(), wait at {}", conn, cc_seq,
        proto_crosscore.get_in_seq());
    return proto_crosscore.wait(cc_seq).then([this, cc_seq, msg_seq] {
...
void
IOHandler::do_requeue_out_sent_up_to(seq_num_t seq)
{
  assert(get_io_state() != io_state_t::open);
  if (out_sent_msgs.empty() && out_pending_msgs.empty()) {
    logger().debug(
        "{} nothing to requeue, reset out_seq from {} to seq {}", conn, out_seq,
        seq);
    out_seq = seq;
    return;
  }
  logger().debug(
      "{} discarding sent msgs by seq {} (sent_len={}, out_seq={})", conn, seq,
      out_sent_msgs.size(), out_seq);
...
IOHandler::cleanup_prv_shard(seastar::shard_id prv_sid)
{
  ...
  return seastar::smp::submit_to(
             ...
             [this] {
               ...
               return prv_states.close().then([ref_prv_states =
    ...
    std::optional<bool> is_replace)
{
  ...
  if (!proto_crosscore.proceed_or_wait(cc_seq)) {
    logger().debug(
        "{} got {} to_new_sid(), wait at {}", conn, cc_seq,
        proto_crosscore.get_in_seq());
    return proto_crosscore.wait(cc_seq).then(
        [this, cc_seq, new_sid, is_replace,
  ...
  logger().debug(
      "{} got {} to_new_sid_1(new_sid={}, {}) at {}", conn, cc_seq, new_sid,
      fmt::format(
  ...
  if (get_io_state() != io_state_t::drop) {
    ...
    if (new_sid != seastar::this_shard_id()) {
      dispatchers.ms_handle_shard_change(
          conn_ref, new_sid, is_accept_or_connect);
  ...
  shard_states =
      shard_states_t::create_from_previous(*maybe_prv_shard_states, new_sid);
  assert(new_sid == get_shard_id());
  // broadcast shard change to all the io waiters, atomically.
  io_crosscore.reset_wait();

  return seastar::smp::submit_to(
      new_sid, [this, next_cc_seq, is_dropped, prv_sid, is_replace,
                conn_fref = std::move(conn_fref)]() mutable {
        logger().debug(
            "{} got {} to_new_sid_2(prv_sid={}, is_dropped={}, {}) at {}", conn,
            next_cc_seq, prv_sid, is_dropped,
            fmt::format(
                "{}",
        ...
        if (is_dropped) {
          ...
          shard_states->dispatch_in_background(
              "cleanup_prv_sid", conn,
              [this, prv_sid] { return cleanup_prv_shard(prv_sid); });
          maybe_notify_out_dispatch();
        }
    ...
    ConnectionFRef conn_fref)
{
  ...
  return seastar::smp::submit_to(
      sid, [this, cc_seq, conn_fref = std::move(conn_fref)]() mutable {
...
void
IOHandler::dispatch_reset(bool is_replace)
{
  ceph_assert_always(get_io_state() == io_state_t::drop);
...
void
IOHandler::dispatch_remote_reset()
{
  if (get_io_state() == io_state_t::drop) {
    return;
  }
...
void
IOHandler::ack_out_sent(seq_num_t seq)
{
  ...
  while (!out_sent_msgs.empty() && out_sent_msgs.front()->get_seq() <= seq) {
    logger().trace(
        "{} got ack seq {} >= {}, pop {}", conn, seq,
        out_sent_msgs.front()->get_seq(), *out_sent_msgs.front());
    out_sent_msgs.pop_front();
  }
}
  ...
  return seastar::repeat([this, &ctx] {
      ...
      .handle_exception_type([this, &ctx](const std::system_error& e) {
        ...
        if (e.code() != std::errc::broken_pipe &&
          ...
          logger().error(
              "{} do_out_dispatch(): unexpected error at {} -- {}", conn,
              io_state, e.what());
          ceph_abort();
        }
        ...
        if (io_state == io_state_t::open) {
          auto cc_seq = proto_crosscore.prepare_submit();
          logger().info(
              "{} do_out_dispatch(): fault at {}, {}, going to delay -- {}, "
          ...
          if (io_state != io_state_t::switched) {
            logger().info(
                "{} do_out_dispatch(): fault at {}, {} -- {}", conn, io_state,
                io_stat_printer{*this}, e.what());
          } else {
            logger().info(
                "{} do_out_dispatch(): fault at {} -- {}", conn, io_state,
                e.what());
...
void
IOHandler::maybe_notify_out_dispatch()
{
  ceph_assert_always(seastar::this_shard_id() == get_shard_id());
  if (is_out_queued()) {
    notify_out_dispatch();
...
IOHandler::read_message(
    ...
    std::size_t msg_size)
{
  return frame_assembler->read_frame_payload<false>().then([this, throttle_stamp,
                                                            msg_size,
                                                            &ctx](auto payload) {
    if (unlikely(ctx.get_io_state() != io_state_t::open)) {
      logger().debug(
          "{} triggered {} during read_message()", conn, ctx.get_io_state());
      abort_protocol();
    }
    ...
    logger().trace(
        ...
        msg_frame.data_len(), (uint16_t)current_header.type,
        conn.get_peer_name(), (uint16_t)current_header.data_off,
    ...
    Message* message = decode_message(
        nullptr, 0, header, footer, msg_frame.front(), msg_frame.middle(),
        msg_frame.data(), nullptr);
    if (!message) {
    ...
    if (message->get_seq() <= cur_seq) {
      logger().error(
          "{} got old message {} <= {} {}, discarding", conn,
          message->get_seq(), cur_seq, *message);
      ...
      logger().error(
          "{} missed message? skipped from seq {} to {}", conn, cur_seq,
          message->get_seq());
    ...
    if (conn.policy.lossy) {
      logger().debug(
          "{} <== #{} === {} ({})", conn, message->get_seq(), *message,
          message->get_type());
    } else {
      logger().debug(
          "{} <== #{},{} === {} ({})", conn, message->get_seq(),
          (uint32_t)current_header.ack_seq, *message, message->get_type());
    }
...
IOHandler::do_in_dispatch()
{
  ...
  shard_states->dispatch_in_background("do_in_dispatch", conn, [this, &ctx = *shard_states] {
    return seastar::keep_doing([this, &ctx] {
             return frame_assembler->read_main_preamble<false>().then(
                 [this, &ctx](auto ret) {
                   switch (ret.tag) {
                   case Tag::MESSAGE: {
                     ...
                     return seastar::futurize_invoke([this] {
                         ...
                         .then([this, msg_size] {
                           ...
                           logger().trace(
                               "{} wants {} bytes from policy throttler {}/{}",
                               conn, msg_size,
                   ...
                   case Tag::ACK:
                     return frame_assembler->read_frame_payload<false>().then(
                         [this](auto payload) {
                           // handle_message_ack() logic
                           auto ack = AckFrame::Decode(payload->back());
                           logger().debug(
                               "{} GOT AckFrame: seq={}", conn, ack.seq());
                     ...
                     return frame_assembler->read_frame_payload<false>().then(
                         [this](auto payload) {
                           ...
                           auto keepalive_frame =
                               KeepAliveFrame::Decode(payload->back());
                           logger().debug(
                               "{} GOT KeepAliveFrame: timestamp={}", conn,
                               keepalive_frame.timestamp());
                         ...
                         [this](auto payload) {
                           ...
                           auto _last_keepalive_ack =
                               seastar::lowres_system_clock::time_point{
                                   keepalive_ack_frame.timestamp()};
                           set_last_keepalive_ack(_last_keepalive_ack);
                           logger().debug(
                               "{} GOT KeepAliveFrameAck: timestamp={}", conn,
                               _last_keepalive_ack);
                         });
                   default: {
                     logger().warn(
                         "{} do_in_dispatch() received unexpected tag: {}",
        ...
        .handle_exception([this, &ctx](std::exception_ptr eptr) {
          ...
          if (io_state == io_state_t::open) {
            auto cc_seq = proto_crosscore.prepare_submit();
            logger().info(
                "{} do_in_dispatch(): fault at {}, {}, going to delay -- {}, "
                "send {} notify_out_fault()",
            ...
            if (io_state != io_state_t::switched) {
              logger().info(
                  "{} do_in_dispatch(): fault at {}, {} -- {}", conn, io_state,
                  io_stat_printer{*this}, e_what);
            } else {
              logger().info(
                  "{} do_in_dispatch(): fault at {} -- {}", conn, io_state,
                  e_what);
...
seastar::future<>
IOHandler::close_io(cc_seq_t cc_seq, bool is_dispatch_reset, bool is_replace)
{
  ceph_assert_always(seastar::this_shard_id() == get_shard_id());
  if (!proto_crosscore.proceed_or_wait(cc_seq)) {
    logger().debug(
        "{} got {} close_io(), wait at {}", conn, cc_seq,
        proto_crosscore.get_in_seq());
    return proto_crosscore.wait(cc_seq).then([this, cc_seq, is_dispatch_reset,
                                              is_replace] {
  ...
  logger().debug(
      "{} got {} close_io(reset={}, replace={})", conn, cc_seq,
  ...
  if (maybe_dropped_sid.has_value()) {
    ...
    return shard_states->close().then([this] {
      assert(shard_states->assert_closed_and_exit());
    });
  }
}
    ...
    SocketConnection& conn)
{
  ...
  if (unlikely(out_exit_dispatching.has_value())) {
    ...
    logger().info(
        "{} do_out_dispatch: stop({}) at {}, set out_exit_dispatching", conn,
        what, io_state);
  } else {
    if (unlikely(io_state != io_state_t::open)) {
      logger().info(
          "{} do_out_dispatch: stop({}) at {}, no out_exit_dispatching", conn,
          what, io_state);
    }
  }
}
