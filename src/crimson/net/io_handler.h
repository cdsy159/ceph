...
public:
  using proto_crosscore_ordering_t =
      smp_crosscore_ordering_t<crosscore_type_t::ONE>;
  ...
  virtual seastar::future<> notify_out(cc_seq_t cc_seq) = 0;
  ...
  virtual seastar::future<> notify_mark_down(cc_seq_t cc_seq) = 0;
...
public:
  using io_crosscore_ordering_t =
      smp_crosscore_ordering_t<crosscore_type_t::N_ONE>;
  using proto_crosscore_ordering_t =
      smp_crosscore_ordering_t<crosscore_type_t::ONE>;
  using cc_seq_t = proto_crosscore_ordering_t::seq_t;
...
public:
  seastar::shard_id
  get_shard_id() const final
  {
    return shard_states->get_shard_id();
  }

  bool
  is_connected() const final
  {
  ...
  clock_t::time_point
  get_last_keepalive() const final
  {
    ceph_assert_always(seastar::this_shard_id() == get_shard_id());
  ...
  clock_t::time_point
  get_last_keepalive_ack() const final
  {
    ceph_assert_always(seastar::this_shard_id() == get_shard_id());
  ...
  void
  set_last_keepalive_ack(clock_t::time_point when) final
  {
    ceph_assert_always(seastar::this_shard_id() == get_shard_id());
...
public:
  ...
  void
  set_handshake_listener(HandshakeListener& hl)
  {
  ...
  seastar::future<> reset_session(cc_seq_t cc_seq, bool full);

  seastar::future<> reset_peer_state(cc_seq_t cc_seq);

  seastar::future<> requeue_out_sent_up_to(cc_seq_t cc_seq, seq_num_t msg_seq);

  seastar::future<> requeue_out_sent(cc_seq_t cc_seq);
...
private:
  ...
  public:
    ...
    void
    set_io_state(io_state_t new_state)
    {
      assert(seastar::this_shard_id() == sid);
    ...
    void
    dispatch_in_background(const char* what, SocketConnection& who, Func&& func)
    {
      assert(seastar::this_shard_id() == sid);
    ...
    bool
    try_enter_out_dispatching(SocketConnection& conn)
    {
      assert(seastar::this_shard_id() == sid);
      if (out_dispatching) {
      ...
      case io_state_t::switched:
        ...
        return false;
      default:
        crimson::get_logger(ceph_subsys_ms)
            .error(
                "{} try_enter_out_dispatching() got wrong io_state {}", conn,
                io_state);
    ...
    void
    exit_out_dispatching(const char* what, SocketConnection& conn)
    {
      assert(seastar::this_shard_id() == sid);
    ...
    bool
    assert_closed_and_exit() const
    {
      assert(seastar::this_shard_id() == sid);
      if (gate.is_closed()) {
        ceph_assert_always(
            io_state == io_state_t::drop || io_state == io_state_t::switched);
        ceph_assert_always(!out_dispatching);
    ...
    static shard_states_ref_t
    create(seastar::shard_id sid, io_state_t state)
    {
  ...
  {
    return (
        !out_pending_msgs.empty() || ack_left > 0 || need_keepalive ||
...
inline std::ostream&
operator<<(std::ostream& out, IOHandler::io_stat_printer stat)
{
...
struct fmt::formatter<crimson::net::io_handler_state> {
  ...
  auto
  format(crimson::net::io_handler_state state, FormatContext& ctx) const
  {
    return fmt::format_to(
        ctx.out(), "io(in_seq={}, is_out_queued={}, has_out_sent={})",
        state.in_seq, state.is_out_queued, state.has_out_sent);
  }
};
...
struct fmt::formatter<crimson::net::IOHandler::io_state_t>
  ...
  template <typename FormatContext>
  auto
  format(crimson::net::IOHandler::io_state_t state, FormatContext& ctx) const
  {
...
template <>
struct fmt::formatter<crimson::net::IOHandler::io_stat_printer>
