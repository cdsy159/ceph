...
struct Task final : WorkItem {
  ...
  using future_stored_type_t = std::conditional_t<
      std::is_void_v<T>,
...
public:
  WorkItem*
  pop_front(std::chrono::milliseconds& queue_max_wait)
  {
...
public:
  ...
  auto
  submit(int shard, Func&& func, Args&&... args)
  {
    ...
    return seastar::with_gate(
        ...
        [packaged = std::move(packaged), shard, this] {
          return local_free_slots().wait().then([packaged = std::move(packaged),
                                                 shard, this] {
