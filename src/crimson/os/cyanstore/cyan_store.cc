...
private:
  struct this_error_category : std::error_category {
    ...
    std::string
    message([[maybe_unused]] const int ev) const final
    {
      assert(ev == 42);
      return MsgV;
    }
...
CyanStore::start()
{
  ...
  return get_shard_nums()
      ...
      .then([this] {
        logger().debug(
            "CyanStore started with {} shard stores", store_shard_nums);
...
CyanStore::umount()
{
  ...
  return shard_stores.invoke_on_all([](auto& local_store) {
    return seastar::do_for_each(
        ...
        [](auto& mshard_store) { return mshard_store->umount(); });
  });
}
...
CyanStore::stat() const
{
  ...
  return shard_stores
      ...
      .then([](uint64_t used_bytes) {
        store_statfs_t st;
        st.total = crimson::common::local_conf().get_val<Option::size_t>(
...
seastar::future<store_statfs_t>
CyanStore::pool_statfs(int64_t pool_id) const
{
  return stat();
}

CyanStore::mkfs_ertr::future<>
  ...
  return read_meta("fsid")
      .then([=, this](auto&& ret) -> mkfs_ertr::future<> {
        ...
        if (r == -ENOENT) {
          ...
          if (!osd_fsid.parse(fsid_str.c_str())) {
            ...
            logger().error(
                "on-disk fsid {} != provided {}", osd_fsid, new_osd_fsid);
      ...
      .safe_then([this] {
        return shard_stores.invoke_on_all([](auto& local_store) {
          return seastar::do_for_each(
              local_store.mshard_stores,
              [](auto& mshard_store) { return mshard_store->mkfs(); });
        });
      });
}
...
CyanStore::Shard::Shard(
  ...
  path(path), store_index(store_index)
{
    ...
    uint32_t op_flags) const
{
  ...
  for (const auto& [oid, obj] : boost::make_iterator_range(
           c->object_map.lower_bound(start), c->object_map.end())) {
    std::ignore = obj;
...
CyanStore::Shard::list_collections()
{
  ...
  for (auto& coll : coll_map) {
    collections.push_back(std::make_pair(
        coll.first, std::make_pair(seastar::this_shard_id(), store_index)));
  }
  return seastar::make_ready_future<std::vector<coll_core_t>>(
        ...
        return crimson::do_for_each(
                   ...
                   [this, ch, oid, op_flags, &bl](auto& p) {
                     return read(ch, oid, p.first, p.second, op_flags)
                         .safe_then([&bl](auto ret) { bl.claim_append(ret); });
                   })
            .safe_then([&bl] {
              return read_errorator::make_ready_future<ceph::bufferlist>(
                  std::move(bl));
...
auto
CyanStore::Shard::omap_get_values(
    CollectionRef ch,
    const ghobject_t& oid,
...
auto
CyanStore::Shard::omap_get_header(
    CollectionRef ch,
    const ghobject_t& oid,
    uint32_t op_flags)
    -> CyanStore::Shard::get_attr_errorator::future<ceph::bufferlist>
...
int
CyanStore::Shard::_remove(const coll_t& cid, const ghobject_t& oid)
{
...
int
CyanStore::Shard::_touch(const coll_t& cid, const ghobject_t& oid)
{
  logger().debug("{} cid={} oid={}", __func__, cid, oid);
  auto c = _get_collection(cid);
  if (!c)
...
int
CyanStore::Shard::_omap_clear(const coll_t& cid, const ghobject_t& oid)
{
...
int
CyanStore::Shard::_omap_set_values(
    const coll_t& cid,
    const ghobject_t& oid,
    std::map<std::string, ceph::bufferlist>&& aset)
{
...
int
CyanStore::Shard::_omap_set_header(
    const coll_t& cid,
    const ghobject_t& oid,
...
int
CyanStore::Shard::_omap_rmkeyrange(
    const coll_t& cid,
    const ghobject_t& oid,
    ...
    const std::string& last)
{
  ...
  for (auto i = o->omap.lower_bound(first);
       i != o->omap.end() && i->first < last; o->omap.erase(i++))
    ;
  return 0;
}

int
CyanStore::Shard::_truncate(
    const coll_t& cid,
    const ghobject_t& oid,
    uint64_t size)
{
...
int
CyanStore::Shard::_rm_attrs(const coll_t& cid, const ghobject_t& oid)
{
  logger().debug("{} cid={} oid={}", __func__, cid, oid);
  auto c = _get_collection(cid);
  if (!c) {
...
int
CyanStore::Shard::_create_collection(const coll_t& cid, int bits)
{
...
int
CyanStore::Shard::_remove_collection(const coll_t& cid)
{
...
seastar::future<>
CyanStore::write_meta(const std::string& key, const std::string& value)
{
  ceph_assert(seastar::this_shard_id() == primary_core);
  std::string v = value;
  v += "\n";
  if (int r = safe_write_file(
          path.c_str(), key.c_str(), v.c_str(), v.length(), 0600);
      r < 0) {
...
CyanStore::read_meta(const std::string& key)
{
  ...
  if (r > 0) {
    ...
    boost::algorithm::trim_right_if(fsid, [](unsigned char c) {
      return isspace(c);
    });
  } else {
    fsid.clear();
...
unsigned
CyanStore::Shard::get_max_attr_name_length() const
{
...
CyanStore::Shard::stat(CollectionRef ch, const ghobject_t& oid, uint32_t op_flags)
{
  assert(store_active);
  auto c = static_cast<Collection*>(ch.get());
  auto o = c->get_object(oid);
