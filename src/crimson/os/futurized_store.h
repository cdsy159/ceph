...
struct BackendStore {
  FuturizedStore&
      f_store; // indicate alienstore/seastore/cyanstore, not shard store
  ...
  BackendStore(
      FuturizedStore& f_store,
      store_shard_t shard_id,
...
public:
  ...
  public:
    ...
    bool
    is_shard_store_active(store_index_t store_index, uint32_t store_shard_nums)
    {
    ...
    using get_attr_errorator =
        crimson::errorator<crimson::ct_error::enoent, crimson::ct_error::enodata>;
     ...
     */
    using omap_iterate_cb_t = std::function<
    ...
    virtual read_errorator::future<ObjectStore::omap_iter_ret_t> omap_iterate(
        ...
        uint32_t op_flags = 0,
        omap_iterate_conf_t on_conflict = nullptr) = 0;
    ...
    virtual seastar::future<std::tuple<std::vector<ghobject_t>, ghobject_t>>
    list_objects(
        CollectionRef c,
    ...
    virtual seastar::future<CollectionRef> create_new_collection(
        const coll_t& cid) = 0;

    virtual seastar::future<CollectionRef> open_collection(
        const coll_t& cid) = 0;

    virtual seastar::future<> set_collection_opts(
        CollectionRef c,
  ...
  public:
    seastar::future<>
    do_transaction(CollectionRef ch, ceph::os::Transaction&& txn)
    {
    ...
    // error injection
    virtual seastar::future<>
    inject_data_error(const ghobject_t& o)
    {
      return seastar::now();
    }

    virtual seastar::future<>
    inject_mdata_error(const ghobject_t& o)
    {
      return seastar::now();
    }
...
public:
  static std::unique_ptr<FuturizedStore> create(
  ...
  virtual seastar::future<> write_meta(
...
auto
with_store(BackendStore store, Args&&... args)
{
  using raw_return_type =
      decltype((std::declval<crimson::os::FuturizedStore::Shard>().*MemberFunc)(
          std::forward<Args>(args)...));

  constexpr bool is_errorator = is_errorated_future_v<raw_return_type>;
  constexpr bool is_seastar_future =
      seastar::is_future<raw_return_type>::value && !is_errorator;
  ...
  if (store.shard_id == seastar::this_shard_id() ||
      store.shard_id == GLOBAL_STORE) {
    if constexpr (is_plain) {
      return seastar::make_ready_future<raw_return_type>(
          (store.f_store.get_sharded_store(store.store_index).*
           MemberFunc)(std::forward<Args>(args)...));
    } else {
          ...
          seastar::smp::submit_to(
              ...
              [store,
               args = std::make_tuple(std::forward<Args>(args)...)]() mutable {
                return std::apply(
                    [store](auto&&... args) {
                      return (store.f_store.get_sharded_store(store.store_index).*
                              MemberFunc)(std::forward<decltype(args)>(args)...)
                          .to_base();
                    },
                    std::move(args));
              })
