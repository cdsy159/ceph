...
void
segment_info_t::set_open(
    segment_seq_t _seq,
    segment_type_t _type,
    data_category_t _category,
...
std::ostream&
operator<<(std::ostream& out, const segment_info_t& info)
{
  out << "seg_info_t(" << "state=" << info.state << ", " << info.id;
  if (info.is_empty()) {
    // pass
  } else { // open or closed
    out << " " << info.type << " " << segment_seq_printer_t{info.seq} << " "
        << info.category << " " << rewrite_gen_printer_t{info.generation}
...
void
segments_info_t::add_segment_manager(SegmentManager& segment_manager)
{
  ...
  INFO(
      "adding segment manager {}, size=0x{:x}, segment size=0x{:x}, "
      "segments={}",
    ...
    rewrite_gen_t generation)
{
  ...
  DEBUG(
      ...
      segment, type, segment_seq_printer_t{seq}, category,
      rewrite_gen_printer_t{generation}, segment_info, num_empty, num_open,
      num_closed);
...
void
segments_info_t::mark_empty(segment_id_t segment)
{
  LOG_PREFIX(segments_info_t::mark_empty);
  auto& segment_info = segments[segment];
  INFO(
      "releasing {}, {}, num_segments(empty={}, opened={}, closed={})", segment,
...
void
segments_info_t::mark_closed(segment_id_t segment)
{
  LOG_PREFIX(segments_info_t::mark_closed);
  auto& segment_info = segments[segment];
  INFO(
      "closing {}, {}, num_segments(empty={}, opened={}, closed={})", segment,
      segment_info, num_empty, num_open, num_closed);
...
void
segments_info_t::update_written_to(segment_type_t type, paddr_t offset)
{
  ...
  if (!segment_info.is_open()) {
    ERROR(
        "segment is not open, not updating, type={}, offset={}, {}", type,
        offset, segment_info);
    ceph_abort();
  }
  ...
  if (segment_info.written_to > new_written_to) {
    ERROR(
        "written_to should not decrease! type={}, offset={}, {}", type, offset,
        segment_info);
    ceph_abort();
  }
...
std::ostream&
operator<<(std::ostream& os, const segments_info_t& infos)
{
  return os << "segments(" << "empty=" << infos.get_num_empty()
            << ", open=" << infos.get_num_open()
            << ", closed=" << infos.get_num_closed()
            << ", type_journal=" << infos.get_num_type_journal()
            << ", type_ool=" << infos.get_num_type_ool() << ", total=0x"
            << std::hex << infos.get_total_bytes() << "B" << ", available=0x"
            << infos.get_available_bytes() << "B" << ", unavailable=0x"
            << infos.get_unavailable_bytes() << "B" << std::dec
            << ", available_ratio=" << infos.get_available_ratio()
            << ", submitted_head=" << infos.get_submitted_journal_head()
            << ", time_bound="
            << sea_time_point_printer_t{infos.get_time_bound()} << ")";
}

void
JournalTrimmerImpl::config_t::validate() const
{
  ...
  ceph_assert(rewrite_dirty_bytes_per_cycle >= rewrite_dirty_bytes_per_trans);
...
JournalTrimmerImpl::config_t
JournalTrimmerImpl::config_t::get_test(std::size_t roll_size, backend_type_t type)
{
  assert(roll_size);
  std::size_t target_dirty_bytes = 0;
  std::size_t target_alloc_bytes = 0;
...
void
JournalTrimmerImpl::set_journal_head(journal_seq_t head)
{
  ...
  ceph_assert(
      journal_alloc_tail == JOURNAL_SEQ_NULL || head >= journal_alloc_tail);
  ceph_assert(
      journal_dirty_tail == JOURNAL_SEQ_NULL || head >= journal_dirty_tail);

  std::swap(journal_head, head);
  if (journal_head.segment_seq == head.segment_seq) {
    DEBUG(
        "journal_head {} => {}, {}", head, journal_head,
        stat_printer_t{*this, false});
  } else {
    INFO(
        "journal_head {} => {}, {}", head, journal_head,
        stat_printer_t{*this, false});
...
void
JournalTrimmerImpl::set_journal_head_sequence(segment_seq_t seq)
{
...
void
JournalTrimmerImpl::update_journal_tails(
    ...
    journal_seq_t alloc_tail)
{
  ...
  if (dirty_tail != JOURNAL_SEQ_NULL) {
    ...
    if (journal_dirty_tail != JOURNAL_SEQ_NULL &&
      ...
      ERROR(
          "journal_dirty_tail {} => {} is backwards!", journal_dirty_tail,
          dirty_tail);
      ceph_abort();
    }
    ...
    if (journal_dirty_tail.segment_seq == dirty_tail.segment_seq) {
      DEBUG(
          "journal_dirty_tail {} => {}, {}", dirty_tail, journal_dirty_tail,
          stat_printer_t{*this, false});
    } else {
      INFO(
          "journal_dirty_tail {} => {}, {}", dirty_tail, journal_dirty_tail,
  ...
  if (tail_include_alloc && alloc_tail != JOURNAL_SEQ_NULL) {
    ...
    if (journal_alloc_tail != JOURNAL_SEQ_NULL &&
      ...
      ERROR(
          "journal_alloc_tail {} => {} is backwards!", journal_alloc_tail,
          alloc_tail);
      ceph_abort();
    }
    ...
    if (journal_alloc_tail.segment_seq == alloc_tail.segment_seq) {
      DEBUG(
          "journal_alloc_tail {} => {}, {}", alloc_tail, journal_alloc_tail,
          stat_printer_t{*this, false});
    } else {
      INFO(
          "journal_alloc_tail {} => {}, {}", alloc_tail, journal_alloc_tail,
...
JournalTrimmerImpl::get_tail_limit() const
{
  ...
  auto ret = journal_head.add_offset(
      backend_type, -static_cast<device_off_t>(config.max_journal_bytes),
      roll_start, roll_size);
  return ret;
}

journal_seq_t
JournalTrimmerImpl::get_dirty_tail_min_target() const
{
  assert(background_callback->is_ready());
  auto ret = journal_head.add_offset(
      backend_type, -static_cast<device_off_t>(config.min_journal_dirty_bytes),
      roll_start, roll_size);
  return ret;
}

journal_seq_t
JournalTrimmerImpl::get_dirty_tail_target_per_cycle() const
{
  assert(background_callback->is_ready());
...
JournalTrimmerImpl::get_dirty_tail_target() const
{
  ...
  auto ret = journal_head.add_offset(
      backend_type,
      -static_cast<device_off_t>(config.target_journal_dirty_bytes), roll_start,
      roll_size);
  return ret;
}
...
JournalTrimmerImpl::get_alloc_tail_target() const
{
  ...
  auto ret = journal_head.add_offset(
      backend_type,
      -static_cast<device_off_t>(config.target_journal_alloc_bytes), roll_start,
      roll_size);
  return ret;
}

std::size_t
JournalTrimmerImpl::get_dirty_journal_size() const
{
  if (!background_callback->is_ready()) {
    return 0;
  }
  auto ret = journal_head.relative_to(
      backend_type, journal_dirty_tail, roll_start, roll_size);
  ceph_assert(ret >= 0);
  return static_cast<std::size_t>(ret);
}

std::size_t
JournalTrimmerImpl::get_alloc_journal_size() const
{
  if (!background_callback->is_ready() || !tail_include_alloc) {
    return 0;
  }
  auto ret = journal_head.relative_to(
      backend_type, journal_alloc_tail, roll_start, roll_size);
  ceph_assert(ret >= 0);
  return static_cast<std::size_t>(ret);
}
  ...
  return seastar::when_all(
             [this] {
               if (should_trim_alloc()) {
                 return trim_alloc().handle_error(crimson::ct_error::assert_all{
             ...
             [this] {
               if (should_start_trim_dirty()) {
                 return trim_dirty().handle_error(crimson::ct_error::assert_all{
...
JournalTrimmerImpl::trim_alloc()
{
  ...
  return repeat_eagain([this, FNAME, &shard_stats] {
           ...
           return extent_callback->with_transaction_intr(
               Transaction::src_t::TRIM_ALLOC, "trim_alloc", CACHE_HINT_NOCACHE,
               [this, FNAME](auto& t) {
                 auto target = get_alloc_tail_target();
                 DEBUGT(
                     "start, alloc_tail={}, target={}", t, journal_alloc_tail,
                     target);
                 return backref_manager
                     .merge_cached_backrefs(
                         t, target, config.max_backref_bytes_per_cycle)
                     .si_then(
                         [this, FNAME, &t](auto trim_alloc_to)
                                   ...
                                   if (trim_alloc_to != JOURNAL_SEQ_NULL) {
                                     return extent_callback
                                         ->submit_transaction_direct(
                                             t,
                                             std::make_optional<journal_seq_t>(
                                                 trim_alloc_to));
                                   }
                                   return seastar::now();
                                 });
...
JournalTrimmerImpl::trim_dirty()
{
  ...
  return ::crimson::repeat([this, FNAME, &shard_stats, target] {
    if (should_stop_trim_dirty(target)) {
      return trim_ertr::make_ready_future<seastar::stop_iteration>(
    ...
    return repeat_eagain([this, FNAME, &shard_stats, target] {
             ...
             return extent_callback->with_transaction_intr(
                 Transaction::src_t::TRIM_DIRTY, "trim_dirty",
                 CACHE_HINT_NOCACHE, [this, FNAME, target](auto& t) {
                   DEBUGT(
                       "start, dirty_tail={}, target={}", t, journal_dirty_tail,
                       target);
                   return extent_callback
                       ->get_next_dirty_extents(
                           t, target, config.rewrite_dirty_bytes_per_trans)
                       .si_then([this, FNAME, &t](auto dirty_list) {
                         DEBUGT(
                             "rewrite {} dirty extents", t, dirty_list.size());
                         return seastar::do_with(
                             std::move(dirty_list),
                             [this, &t](auto& dirty_list) {
                               return trans_intr::do_for_each(
                                   dirty_list, [this, &t](auto& e) {
        ...
        .safe_then([] { return seastar::stop_iteration::no; })
        .finally([this, FNAME, &shard_stats] {
...
void
JournalTrimmerImpl::register_metrics(store_index_t store_index)
{
  ...
  metrics.add_group(
      ...
      {
          sm::make_counter(
              "dirty_journal_bytes", [this] { return get_dirty_journal_size(); },
              sm::description("the size of the journal for dirty extents"),
              {sm::label_instance(
                  "shard_store_index", std::to_string(store_index))}),
          sm::make_counter(
              "alloc_journal_bytes", [this] { return get_alloc_journal_size(); },
...
std::ostream&
operator<<(std::ostream& os, const JournalTrimmerImpl::stat_printer_t& stats)
{
  os << "JournalTrimmer(";
  ...
  if (stats.detailed) {
    ...
    if (stats.trimmer.background_callback->is_ready()) {
      os << ", alloc_tail_target=" << stats.trimmer.get_alloc_tail_target()
         << ", dirty_tail_min_target="
         << stats.trimmer.get_dirty_tail_min_target()
...
bool
SpaceTrackerSimple::equals(const SpaceTrackerI& _other) const
{
  ...
  bool all_match = true;
  for (auto i = live_bytes_by_segment.begin(),
            j = other.live_bytes_by_segment.begin();
       i != live_bytes_by_segment.end(); ++i, ++j) {
    if (i->second.live_bytes != j->second.live_bytes) {
      all_match = false;
      DEBUG(
          "segment_id {} live bytes mismatch *this: {}, other: {}", i->first,
          i->second.live_bytes, j->second.live_bytes);
    }
  }
  return all_match;
}
...
int64_t
SpaceTrackerDetailed::SegmentMap::allocate(
...
int64_t
SpaceTrackerDetailed::SegmentMap::release(
    device_segment_id_t segment,
    segment_off_t offset,
    extent_len_t len,
    const extent_len_t block_size)
...
bool
SpaceTrackerDetailed::equals(const SpaceTrackerI& _other) const
{
  ...
  for (auto i = segment_usage.begin(), j = other.segment_usage.begin();
    ...
    if (i->second.get_usage() != j->second.get_usage()) {
      ...
      ERROR(
          "segment_id {} live bytes mismatch *this: {}, other: {}", i->first,
          i->second.get_usage(), j->second.get_usage());
    }
  }
  return all_match;
}
...
void
SpaceTrackerDetailed::SegmentMap::dump_usage(extent_len_t block_size) const
{
...
void
SpaceTrackerDetailed::dump_usage(segment_id_t id) const
{
...
void
SpaceTrackerSimple::dump_usage(segment_id_t id) const
{
  LOG_PREFIX(SpaceTrackerSimple::dump_usage);
  INFO("id: {}, live_bytes: 0x{:x}", id, live_bytes_by_segment[id].live_bytes);
}

std::ostream&
operator<<(std::ostream& os, const AsyncCleaner::stat_printer_t& stats)
{
  ...
  max_rewrite_generation(max_rewrite_generation)
{
  ...
  INFO(
      "gc_formula={}, max_rewrite_generation={}", formula,
...
void
SegmentCleaner::register_metrics()
{
  namespace sm = seastar::metrics;
  ...
  metrics.add_group(
      prefix,
      {sm::make_counter(
           "segments_number", [this] { return segments.get_num_segments(); },
           sm::description("the number of segments"),
           {sm::label_instance(
               "shard_store_index", std::to_string(store_index))}),
       sm::make_counter(
           "segment_size", [this] { return segments.get_segment_size(); },
           sm::description("the bytes of a segment"),
           {sm::label_instance(
               "shard_store_index", std::to_string(store_index))}),
       sm::make_counter(
           "segments_in_journal", [this] { return get_segments_in_journal(); },
       ...
       sm::make_counter(
           ...
           {sm::label_instance(
               "shard_store_index", std::to_string(store_index))}),
       sm::make_counter(
           "segments_type_ool", [this] { return segments.get_num_type_ool(); },
           sm::description("the number of segments typed out-of-line"),
           {sm::label_instance(
               "shard_store_index", std::to_string(store_index))}),
       sm::make_counter(
           "segments_open", [this] { return segments.get_num_open(); },
           sm::description("the number of open segments"),
           {sm::label_instance(
               "shard_store_index", std::to_string(store_index))}),
       sm::make_counter(
           "segments_empty", [this] { return segments.get_num_empty(); },
           sm::description("the number of empty segments"),
           {sm::label_instance(
               "shard_store_index", std::to_string(store_index))}),
       sm::make_counter(
           "segments_closed", [this] { return segments.get_num_closed(); },
       ...
       sm::make_counter(
           "total_bytes", [this] { return segments.get_total_bytes(); },
           sm::description("the size of the space"),
           {sm::label_instance(
               "shard_store_index", std::to_string(store_index))}),
       sm::make_counter(
           "available_bytes", [this] { return segments.get_available_bytes(); },
       ...
       sm::make_counter(
           ...
           sm::description(
               "the size of the space is unavailable and unreclaimable"),
           {sm::label_instance(
               "shard_store_index", std::to_string(store_index))}),
       sm::make_counter(
           ...
           sm::description(
               "the size of the space is unavailable and reclaimable"),
           {sm::label_instance(
               "shard_store_index", std::to_string(store_index))}),
       sm::make_counter(
       ...
       sm::make_counter(
           ...
           sm::description(
               "the size of the space is unavailable and not alive"),
           {sm::label_instance(
               "shard_store_index", std::to_string(store_index))}),
       ...
       sm::make_gauge(
           "available_ratio", [this] { return segments.get_available_ratio(); },
           sm::description("ratio of available space to total space"),
           {sm::label_instance(
               "shard_store_index", std::to_string(store_index))}),
       sm::make_gauge(
           "reclaim_ratio", [this] { return get_reclaim_ratio(); },
    ...
    rewrite_gen_t generation)
{
  ...
  ERROR(
      "out of space with {} {} {} {}", type, segment_seq_printer_t{seq},
...
void
SegmentCleaner::close_segment(segment_id_t segment)
{
...
double
SegmentCleaner::calc_gc_benefit_cost(
    segment_id_t id,
    ...
    const sea_time_point& bound_time) const
{
  ...
  if (likely(
          bound_time != NULL_TIME && modify_time != NULL_TIME &&
    ...
    rewrite_gen_t target_generation)
{
  ...
  return repeat_eagain([&extent_callback, &backref_extents, &shard_stats,
                        &pin_list, &reclaimed, &runs, is_cold, &backref_manager,
           ...
           return extent_callback.with_transaction_intr(
               src, "clean_reclaim_space", CACHE_HINT_NOCACHE,
               [&extent_callback, &backref_extents, &pin_list, modify_time,
                ...
                target_generation](auto& t) {
                 return seastar::do_with(
                            ...
                            [&extent_callback, &t, &reclaimed, &pin_list,
                             modify_time, &backref_manager, start_pos, end_pos,
                             target_generation](auto& extents) {
                              ...
                              for (auto& pin : pin_list) {
                                ...
                                backref_entries.emplace(
                                    pin.get_key(), pin.get_val(),
                                    pin.get_length(), pin.get_type());
                              }
                              for (auto& cached_backref :
                                   cached_backref_entries) {
                                if (cached_backref.laddr == L_ADDR_NULL) {
                                  auto it = backref_entries.find(
                                      cached_backref.paddr);
                              ...
                              DEBUGT(
                                  "start, backref_entries={}, "
                                  "backref_extents={}",
                                  t, backref_entries.size(), extents.size());
                              return seastar::do_with(
                                         std::move(backref_entries),
                                         [&extent_callback, &extents,
                                          &t](auto& backref_entries) {
                                                ...
                                                &t](auto& ent) {
                                                 LOG_PREFIX(
                                                     SegmentCleaner::
                                                         do_reclaim_space);
                                                 TRACET(
                                                     "getting extent of type "
                                                     "{} at {}~0x{:x}",
                                                     t, ent.type, ent.paddr,
                                                     ent.len);
                                                 return extent_callback
                                                     .get_extents_if_live(
                                                         t, ent.type, ent.paddr,
                                                         ent.laddr, ent.len)
                                                     .si_then([FNAME, &extents,
                                                               &ent,
                                                               &t](auto list) {
                                                       if (list.empty()) {
                                                         TRACET(
                                  ...
                                  .si_then([FNAME, &extents, &extent_callback,
                                            &reclaimed, &t, modify_time,
                                            target_generation] {
                                    DEBUGT(
                                        "reclaim {} extents", t, extents.size());
                                    // rewrite live extents
                                    return trans_intr::do_for_each(
                                        extents, [&extent_callback, modify_time,
                                                  &t, &reclaimed,
                                                  target_generation](auto ext) {
                                          reclaimed += ext->get_length();
                                          return extent_callback.rewrite_extent(
                                              t, ext, target_generation,
                                              modify_time);
                                        });
                                  });
                            })
...
SegmentCleaner::clean_space()
{
  ...
  if (!reclaim_state) {
    ...
    INFO(
        "reclaim start... {} {}, usage={}, time_bound={}", seg_id, segment_info,
        space_tracker->calc_utilization(seg_id),
        sea_time_point_printer_t{segments.get_time_bound()});
    ceph_assert(segment_info.is_closed());
    ceph_assert(
        is_rewrite_generation(segment_info.generation, max_rewrite_generation));
  ...
  DEBUG(
      "reclaiming {} {}~{}", rewrite_gen_printer_t{reclaim_state->generation},
  ...
  return seastar::do_with(
               ...
               return repeat_eagain([this, &weak_read_ret] {
                        ...
                        return extent_callback->with_transaction_intr(
                            ...
                            [this, &weak_read_ret](auto& t) {
                              return backref_manager
                                  .get_mappings(
                                      t, reclaim_state->start_pos,
                                      reclaim_state->end_pos)
                                  .si_then([this, &t,
                                            &weak_read_ret](auto pin_list) {
                                    if (!pin_list.empty()) {
                                      auto it = pin_list.begin();
                                      auto& first_pin = *it;
                                      if (first_pin.get_key() <
      ...
      .safe_then([this, FNAME, pavail_ratio, start](auto weak_read_ret) {
        return seastar::do_with(
            std::move(weak_read_ret.first), std::move(weak_read_ret.second),
            (size_t)0, (size_t)0,
            [this, FNAME, pavail_ratio, start](
                auto& backref_extents, auto& pin_list, auto& reclaimed,
                auto& runs) {
              return do_reclaim_space(
                         backref_extents, pin_list, reclaimed, runs,
                         *extent_callback, is_cold, backref_manager,
                         segments[reclaim_state->get_segment_id()].modify_time,
                         reclaim_state->start_pos, reclaim_state->end_pos,
                         reclaim_state->target_generation)
                  .safe_then([this, FNAME, pavail_ratio, start, &reclaimed,
                              &runs] {
                    stats.reclaiming_bytes += reclaimed;
                    auto d = seastar::lowres_system_clock::now() - start;
                    DEBUG(
                        "duration: {}, pavail_ratio before: {}, repeats: {}", d,
                        pavail_ratio, runs);
                    if (reclaim_state->is_complete()) {
                      auto segment_to_release = reclaim_state->get_segment_id();
                      INFO(
                          "reclaim finish {}, reclaimed alive/total={}",
                          segment_to_release,
                          stats.reclaiming_bytes /
                              (double)segments.get_segment_size());
                      ...
                      return sm_group->release_segment(segment_to_release)
                          .handle_error(
                              ...
                              crimson::ct_error::assert_all{
                                  "SegmentCleaner::clean_space encountered "
                                  "invalid error in release_segment"})
                          .safe_then([this, FNAME, segment_to_release] {
                            ...
                            adjust_segment_util(old_usage, new_usage);
                            INFO(
                                "reclaim released {}, {}", segment_to_release,
                                stat_printer_t{*this, false});
                            background_callback->maybe_wake_blocked_io();
                          });
                    } else {
...
SegmentCleaner::mount()
{
  ...
  space_tracker.reset(
      detailed ? (SpaceTrackerI*)new SpaceTrackerDetailed(sms)
               ...
               return sm_group->read_segment_header(segment_id)
                   .safe_then([segment_id, this, FNAME](auto header) {
                     ...
                     if (s_type == segment_type_t::NULL_SEG) {
                       ERROR(
                           "got null segment, segment_id={} -- {}", segment_id,
                           header);
                       ceph_abort();
                     }
                     return sm_group->read_segment_tail(segment_id)
                         .safe_then(
                             [this, FNAME, segment_id, header](
                                 auto tail) -> scan_extents_ertr::future<> {
                               bool tail_valid =
                                   (tail.segment_nonce == header.segment_nonce);
                               if (!tail_valid &&
                                   header.type == segment_type_t::JOURNAL) {
                                 return scan_no_tail_segment(header, segment_id);
                               }
                               ceph_assert(
                                   header.get_type() == tail.get_type() ||
                                   !tail_valid);

                               sea_time_point header_time =
                                   mod_to_timepoint(header.modify_time);
                               if (tail_valid) {
                                 sea_time_point tail_time =
                                     mod_to_timepoint(tail.modify_time);
                                 std::size_t num_extents = tail.num_extents;
                                 DEBUG(
                                     "updating modify time for segment {}, "
                                     "mod time {}-{}, num_extents {}",
                                     segment_id, header_time, tail_time,
                                     num_extents);
                                 if (num_extents == 0) {
                                   ...
                                   segments.update_modify_time(
                                       segment_id, avg_time, num_extents);
                                 }
                               } else {
                                 DEBUG(
                                     "updating modify time for segment {}, mod "
                                     "time {}, without tail",
                                     segment_id, header_time);
                                 segments.init_modify_time(
                                     segment_id, header_time);
                               }

                               init_mark_segment_closed(
                                   segment_id, header.segment_seq, header.type,
                                   header.category, header.generation);
                               return seastar::now();
                             })
                         .handle_error(
...
SegmentCleaner::scan_extents_ret
  ...
  return seastar::do_with(
             ...
             SegmentManagerGroup::found_record_handler_t(
                 ...
                 -> SegmentManagerGroup::scan_valid_records_ertr::future<> {
                   DEBUG(
                       "{} {}, decoding {} records", segment_id,
                       segment_header.get_type(), record_group_header.records);

                   auto maybe_headers =
                       try_decode_record_headers(record_group_header, mdbuf);
                   if (!maybe_headers) {
                     // This should be impossible, we did check the crc on the mdbuf
                     ERROR(
                         "unable to decode record headers for record group {}",
                   ...
                   for (auto& record_header : *maybe_headers) {
                     auto modify_time =
                         mod_to_timepoint(record_header.modify_time);
                     if (record_header.extents == 0 ||
                         modify_time != NULL_TIME) {
                       segments.update_modify_time(
             ...
             [this, segment_header](auto& cursor, auto& handler) {
               return sm_group->scan_valid_records(
                   cursor, segment_header.segment_nonce,
                   segments.get_segment_size(), handler);
             })
      .safe_then([this, segment_id, segment_header] {
        init_mark_segment_closed(
            segment_id, segment_header.segment_seq, segment_header.type,
            segment_header.category, segment_header.generation);
      });
}

bool
SegmentCleaner::check_usage(bool)
{
  SpaceTrackerIRef tracker(space_tracker->make_empty());
  extent_callback
      ->with_transaction_weak(
          "check_usage", CACHE_HINT_NOCACHE,
          [this, &tracker](auto& t) {
            return backref_manager.scan_mapped_space(
                t, [&tracker](
                       paddr_t paddr, paddr_t backref_key, extent_len_t len,
                       extent_types_t type, laddr_t laddr) {
                  if (paddr.is_absolute_segmented()) {
                    if (is_backref_node(type)) {
                      assert(laddr == L_ADDR_NULL);
                      assert(
                          backref_key.is_absolute_segmented() ||
                          backref_key == P_ADDR_MIN);
                      tracker->allocate(
                          paddr.as_seg_paddr().get_segment_id(),
                          paddr.as_seg_paddr().get_segment_off(), len);
...
void
SegmentCleaner::mark_space_used(paddr_t addr, extent_len_t len)
{
  ...
  [[maybe_unused]] auto ret = space_tracker->allocate(
      seg_addr.get_segment_id(), seg_addr.get_segment_off(), len);
  ...
  assert(ret > 0);
  DEBUG(
      "segment {} new len: {}~0x{:x}, live_bytes: 0x{:x}",
      seg_addr.get_segment_id(), addr, len,
      space_tracker->get_usage(seg_addr.get_segment_id()));
}

void
SegmentCleaner::mark_space_free(paddr_t addr, extent_len_t len)
{
  ...
  DEBUG("segment {} free len: {}~0x{:x}", seg_addr.get_segment_id(), addr, len);
  auto old_usage = calc_utilization(seg_addr.get_segment_id());
  ...
  assert(ret >= 0);
  DEBUG(
      "segment {} free len: {}~0x{:x}, live_bytes: 0x{:x}",
      seg_addr.get_segment_id(), addr, len,
      space_tracker->get_usage(seg_addr.get_segment_id()));
}
...
bool
SegmentCleaner::try_reserve_projected_usage(std::size_t projected_usage)
{
  assert(background_callback->is_ready());
...
void
SegmentCleaner::release_projected_usage(std::size_t projected_usage)
{
  assert(background_callback->is_ready());
...
void
SegmentCleaner::print(std::ostream& os, bool is_detailed) const
{
  os << "SegmentCleaner(";
  if (background_callback->is_ready()) {
  ...
  if (is_detailed) {
    os << ", unavailable_unreclaimable=0x" << std::hex
       ...
       << ", unavailable_reclaimble=0x" << get_unavailable_reclaimable_bytes()
       << "B" << ", alive=0x" << stats.used_bytes << "B" << std::dec << ", "
       << segments;
  }
  os << ")";
}
...
void
RBMCleaner::print(std::ostream& os, bool is_detailed) const
{
  // TODO
  return;
}
...
void
RBMCleaner::mark_space_used(paddr_t addr, extent_len_t len)
{
...
void
RBMCleaner::mark_space_free(paddr_t addr, extent_len_t len)
{
...
void
RBMCleaner::commit_space_used(paddr_t addr, extent_len_t len)
{
  auto rbms = rb_group->get_rb_managers();
  for (auto rbm : rbms) {
    if (addr.get_device_id() == rbm->get_device_id()) {
...
bool
RBMCleaner::try_reserve_projected_usage(std::size_t projected_usage)
{
  assert(background_callback->is_ready());
  stats.projected_used_bytes += projected_usage;
  return true;
...
void
RBMCleaner::release_projected_usage(std::size_t projected_usage)
{
  assert(background_callback->is_ready());
  ceph_assert(stats.projected_used_bytes >= projected_usage);
  stats.projected_used_bytes -= projected_usage;
...
bool
RBMCleaner::check_usage(bool has_cold_tier)
{
  assert(detailed);
          ...
          [this, &tracker, &rbms, has_cold_tier](auto& t) {
            if (has_cold_tier) {
                    ...
                    for (auto rbm : rbms) {
                      if (rbm->get_device_id() == paddr.get_device_id()) {
                        if (is_backref_node(type)) {
                          assert(laddr == L_ADDR_NULL);
                          assert(
                              backref_key.is_absolute_random_block() ||
                              backref_key == P_ADDR_MIN);
              ...
              return lba_manager.scan_mapped_space(
                  t, [&tracker, &rbms](
                         paddr_t paddr, extent_len_t len, extent_types_t type,
                         laddr_t laddr) {
                    for (auto rbm : rbms) {
                      if (rbm->get_device_id() == paddr.get_device_id()) {
                        tracker.allocate(paddr, len);
...
bool
RBMCleaner::equals(const RBMSpaceTracker& _other) const
{
...
RBMCleaner::register_metrics()
{
  ...
  metrics.add_group(
      "rbm_cleaner",
      {sm::make_counter(
           "total_bytes", [this] { return get_total_bytes(); },
           sm::description("the size of the space"),
           {sm::label_instance(
               "shard_store_index", std::to_string(store_index))}),
       sm::make_counter(
           "available_bytes",
           [this] {
             return get_total_bytes() - get_journal_bytes() - stats.used_bytes;
           },
           sm::description("the size of the space is available"),
           {sm::label_instance(
               "shard_store_index", std::to_string(store_index))}),
