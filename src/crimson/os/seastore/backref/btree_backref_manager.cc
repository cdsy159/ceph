...
get_tree_stats<crimson::os::seastore::backref::BackrefBtree>(Transaction& t)
{
...
template <>
phy_tree_root_t&
get_phy_tree_root<crimson::os::seastore::backref::BackrefBtree>(root_t& r)
{
...
get_phy_tree_root_node<crimson::os::seastore::backref::BackrefBtree>(
    ...
    op_context_t c)
{
  ...
  if (backref_root) {
    ...
    return {true, c.cache.get_extent_viewable_by_trans(c.trans, backref_root)};
...
public:
  static void
  link_root(RootBlockRef& root_block, RootT* backref_root)
  {
...
BtreeBackrefManager::mkfs(Transaction& t)
{
  ...
  return cache.get_root(t)
      .si_then([this, &t](auto croot) {
        ...
        croot->get_root().backref_root =
            BackrefBtree::mkfs(croot, get_context(t));
...
BtreeBackrefManager::get_mapping(Transaction& t, paddr_t offset)
{
  LOG_PREFIX(BtreeBackrefManager::get_mapping);
  TRACET("{}", t, offset);
  auto c = get_context(t);
  return with_btree<BackrefBtree>(cache, c, [c, offset](auto& btree) {
    return btree.lower_bound(c, offset).si_then(
        [offset, c](auto iter) -> get_mapping_ret {
          LOG_PREFIX(BtreeBackrefManager::get_mapping);
          if (iter.is_end() || iter.get_key() != offset) {
            ...
            TRACET(
                "{} got {}, {}", c.trans, offset, iter.get_key(),
                iter.get_val());
      ...
      cache, c, [c, offset, end](auto& btree, auto& ret) {
        return BackrefBtree::iterate_repeat(
            ...
            [&ret, offset, end, c](auto& pos) {
              ...
              TRACET(
                  "{}~{} got {}, {}, repeat ...", c.trans, offset, end,
                  pos.get_key(), pos.get_val());
              ceph_assert(
                  (pos.get_key().add_offset(pos.get_val().len)) > offset);
...
BtreeBackrefManager::new_mapping(
    ...
    extent_types_t type)
{
  ceph_assert(is_aligned(
      key.is_absolute_segmented() ? key.as_seg_paddr().get_segment_off()
  ...
  return crimson::os::seastore::with_btree_state<BackrefBtree, state_t>(
             cache, c, key,
             [val, c, key, len, addr,
              /*lookup_attempts,*/ &t](auto& btree, auto& state) {
               return BackrefBtree::iterate_repeat(
                          c, btree.upper_bound_right(c, key),
                          [&state, len, addr, &t,
                           key /*, lookup_attempts*/](auto& pos) {
                            LOG_PREFIX(BtreeBackrefManager::new_mapping);
                            //++stats.num_alloc_extents_iter_nexts;
                            if (pos.is_end()) {
                              DEBUGT(
                                  "{}~{}, paddr={}, state: end, insert at {}",
                                  t, addr, len, key,
                              ...
                              return BackrefBtree::iterate_repeat_ret_inner(
                                ...
                                pos.get_key() >=
                                (state.last_end.add_offset(len))) {
                              DEBUGT(
                                  "{}~{}, paddr={}, state: {}~{}, "
                                  "insert at {} -- {}",
                                  t, addr, len, key, pos.get_key(),
                                  pos.get_val().len,
                                  //stats.num_alloc_extents_iter_nexts - lookup_attempts,
                                  state.last_end, pos.get_val());
                              state.insert_iter = pos;
                              return BackrefBtree::iterate_repeat_ret_inner(
                                  interruptible::ready_future_marker{},
                                  seastar::stop_iteration::yes);
                              ...
                              ERRORT(
                                  ...
                                  "-- {}",
                                  t, addr, len, key, pos.get_key(),
                                  pos.get_val().len, pos.get_val());
                              ceph_abort_msg(
                                  "not possible for the backref tree");
                              return BackrefBtree::iterate_repeat_ret_inner(
                                  interruptible::ready_future_marker{},
                                  seastar::stop_iteration::no);
                            }
                   ...
                   .si_then([c, addr, len, key, &btree, &state, val] {
                     return btree
                         .insert(
                             c, *state.insert_iter, state.last_end, val, nullptr)
                         .si_then([&state, c, addr, len, key](auto&& p) {
                           LOG_PREFIX(BtreeBackrefManager::new_mapping);
                           auto [iter, inserted] = std::move(p);
                           TRACET(
                               "{}~{}, paddr={}, inserted at {}, leaf {}",
    ...
    const uint64_t max)
{
  ...
  return seastar::do_with(
      limit, JOURNAL_SEQ_NULL, [this, &t, max](auto& limit, auto& inserted_to) {
        auto& backref_entryrefs_by_seq = cache.get_backref_entryrefs_by_seq();
        return seastar::do_with(
            backref_entryrefs_by_seq.begin(), JOURNAL_SEQ_NULL,
            [this, &t, &limit, &backref_entryrefs_by_seq,
             max](auto& iter, auto& inserted_to) {
              return trans_intr::
                  repeat([&iter, this, &t, &limit, &backref_entryrefs_by_seq,
                       ...
                       LOG_PREFIX(BtreeBackrefManager::merge_cached_backrefs);
                       DEBUGT(
                           "seq {}, limit {}, num_fresh_backref {}", t, seq,
                           limit, t.get_num_fresh_backref());
                       if (seq <= limit &&
                           t.get_num_fresh_backref() * BACKREF_NODE_SIZE < max) {
                         inserted_to = seq;
                         return trans_intr::do_for_each(
                                    backref_entry_refs,
                                    [this, &t](auto& backref_entry_ref) {
                                      LOG_PREFIX(BtreeBackrefManager::
  ...
  return seastar::do_with(std::move(f), [this, c, FNAME](auto& scan_visitor) {
    ...
    return with_btree<BackrefBtree>(
               ...
               [c, &scan_visitor, block_size, FNAME](auto& btree) {
                 return BackrefBtree::iterate_repeat(
                     c, btree.lower_bound(c, P_ADDR_MIN),
                     [c, &scan_visitor, block_size, FNAME](auto& pos) {
                       ...
                       TRACET(
                           "tree value {}~{} {}~{} {} used", c.trans,
                           pos.get_key(), pos.get_val().len,
                           pos.get_val().laddr, pos.get_val().len,
                           pos.get_val().type);
                       ...
                       scan_visitor(
                           pos.get_key(), P_ADDR_NULL, pos.get_val().len,
                           pos.get_val().type, pos.get_val().laddr);
                       return BackrefBtree::iterate_repeat_ret_inner(
                           interruptible::ready_future_marker{},
                           seastar::stop_iteration::no);
                     });
        ...
        .si_then([this, &scan_visitor, c, FNAME, block_size] {
          ...
          for (auto& [seq, refs] : backref_entryrefs) {
            ...
            for (auto& backref_entry : refs) {
              if (backref_entry->laddr == L_ADDR_NULL) {
                TRACET(
                    "backref entry {}~{} {} free", c.trans,
                    backref_entry->paddr, backref_entry->len,
                    backref_entry->type);
              } else {
                TRACET(
                    "backref entry {}~{} {}~{} {} used", c.trans,
                    backref_entry->paddr, backref_entry->len,
                    backref_entry->laddr, backref_entry->len,
                    backref_entry->type);
              }
              ...
              scan_visitor(
                  backref_entry->paddr, P_ADDR_NULL, backref_entry->len,
                  backref_entry->type, backref_entry->laddr);
            }
          }
        })
        .si_then([this, &scan_visitor, block_size, c, FNAME] {
          BackrefBtree::mapped_space_visitor_t f =
              [&scan_visitor, block_size, FNAME,
               c](paddr_t paddr, paddr_t key, extent_len_t len, depth_t depth,
                  extent_types_t type, BackrefBtree::iterator&) {
                TRACET(
                    "tree node {}~{} {}, depth={} used", c.trans, paddr, len,
                    type, depth);
          ...
          return seastar::do_with(std::move(f), [this, c](auto& tree_visitor) {
            ...
            return with_btree<BackrefBtree>(
                cache, c, [c, &tree_visitor](auto& btree) {
                  return BackrefBtree::iterate_repeat(
                      c, btree.lower_bound(c, P_ADDR_MIN, &tree_visitor),
                      [](auto& pos) {
                        if (pos.is_end()) {
                          return BackrefBtree::iterate_repeat_ret_inner(
                              interruptible::ready_future_marker{},
...
BtreeBackrefManager::init_cached_extent_ret
BtreeBackrefManager::init_cached_extent(Transaction& t, CachedExtentRef e)
{
...
BtreeBackrefManager::rewrite_extent(Transaction& t, CachedExtentRef extent)
{
  auto c = get_context(t);
  return with_btree<BackrefBtree>(cache, c, [c, extent](auto& btree) mutable {
    return btree.rewrite_extent(c, extent);
  });
}

BtreeBackrefManager::remove_mapping_ret
BtreeBackrefManager::remove_mapping(Transaction& t, paddr_t addr)
{
  auto c = get_context(t);
  return with_btree<BackrefBtree>(cache, c, [c, addr](auto& btree) mutable {
    return btree.lower_bound(c, addr).si_then(
        [&btree, c, addr](auto iter) -> remove_mapping_ret {
          if (iter.is_end() || iter.get_key() != addr) {
            LOG_PREFIX(BtreeBackrefManager::remove_mapping);
            WARNT(
                "paddr={} doesn't exist, state: {}, leaf {}", c.trans, addr,
                iter.get_key(), *iter.get_leaf_node());
          ...
          auto ret = remove_mapping_result_t{
              iter.get_key(), iter.get_val().len, iter.get_val().laddr};
          return btree.remove(c, iter).si_then([ret](auto) { return ret; });
        });
  });
}
...
void
BtreeBackrefManager::cache_new_backref_extent(
    paddr_t paddr,
    paddr_t key,
    extent_types_t type)
{
    ...
    paddr_t end)
{
  ...
  return seastar::do_with(
      std::vector<CachedExtentRef>(), std::move(backref_extents),
        ...
        return trans_intr::parallel_for_each(
                   ...
                   [this, &extents, &t](auto& ent) {
                     ...
                     LOG_PREFIX(
                         BtreeBackrefManager::retrieve_backref_extents_in_range);
                     DEBUGT(
                         "getting backref extent of type {} at {}, key {}", t,
                         ent.type, ent.paddr, ent.key);
