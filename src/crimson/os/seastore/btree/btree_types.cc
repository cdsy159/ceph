...
std::ostream&
operator<<(std::ostream& out, const lba_map_val_t& v)
{
  return out << "lba_map_val_t(" << v.pladdr << "~0x" << std::hex << v.len
             << ", type=" << (extent_types_t)v.type << ", checksum=0x"
             << v.checksum << ", refcount=" << std::dec << v.refcount << ")";
}
...
std::ostream&
operator<<(std::ostream& out, const backref_map_val_t& val)
{
  return out << "backref_map_val_t(" << val.laddr << "~0x" << std::hex
             << val.len << std::dec << ")";
}
...
bool
modified_since(T&& extent, uint64_t iter_modifications)
{
...
bool
BtreeCursor<key_t, val_t, ParentT>::is_viewable() const
{
  LOG_PREFIX(BtreeCursor::is_viewable());
  if (!parent->is_valid() || modified_since<key_t>(parent, modifications)) {
    return false;
  }

  auto [viewable, state] = parent->is_viewable_by_trans(ctx.trans);
  SUBTRACET(
      seastore_cache, "{} with viewable state {}", ctx.trans, *parent, state);
  return viewable;
}

template struct BtreeCursor<laddr_t, lba::lba_map_val_t, lba::LBALeafNode>;
template struct BtreeCursor<
    paddr_t,
