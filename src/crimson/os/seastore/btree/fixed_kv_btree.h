...
public:
  ...
  public:
    ...
    iterator_fut
    next(op_context_t c, mapped_space_visitor_t* visitor = nullptr) const
    {
#ifndef NDEBUG
      assert_valid();
#endif
      ...
      if (ret.at_boundary()) {
        return seastar::do_with(ret, [c, visitor](auto& ret) mutable {
          return ret.handle_boundary(c, visitor).si_then([&ret] {
            return std::move(ret);
          });
        });
      } else {
          ...
          [c](auto& depth_with_space, auto& ret, auto& li, auto& ll) {
            return ret
                ...
                .si_then([&ret, c, &li, &ll](auto depth_with_space) {
                  assert(
                      depth_with_space <=
                      ret.get_depth()); // must not be begin()
    ...
    const auto&
    get_internal(depth_t depth) const
    {
      assert(depth > 1);
      assert((depth - 2) < internal.size());
      return internal[depth - 2];
    ...
    ensure_internal_ret
    ensure_internal(op_context_t c, depth_t depth)
    {
      ...
      if (i.node.get()) {
        ...
        assert(c.trans.is_weak() || i.node->is_viewable_by_trans(c.trans).first);
      ...
      auto fut = (depth == 2) ? get_parent(leaf.node)
    ...
    get_val() const
    {
      ...
      if constexpr (
          std::is_same_v<crimson::os::seastore::lba::lba_map_val_t, node_val_t>) {
        if (ret.pladdr.is_paddr()) {
          ret.pladdr =
              ret.pladdr.get_paddr().maybe_relative_to(leaf.node->get_paddr());
        }
      }
      return ret;
    }
    ...
    {
      return leaf.pos == 0 &&
             leaf.node->get_node_meta().begin == min_max_t<node_key_t>::min;
    }

    // handle_boundary() must be called before get_cursor
    boost::intrusive_ptr<cursor_t>
    get_cursor(op_context_t ctx) const
    {
      return new cursor_t(
          ctx, leaf.node, leaf.node->modifications,
  ...
  private:
    ...
    boost::container::static_vector<node_position_t<internal_node_t>, MAX_DEPTH>
        internal;
    ...
    ensure_internal_bottom_up_ret
    ensure_internal_bottom_up(op_context_t c, depth_t start_from, Func&& stop_f)
    {
      return seastar::do_with(
          start_from, std::move(stop_f),
          [c, this](auto& start_from, auto& stop_f) {
            return trans_intr::repeat([this, c, &stop_f, &start_from] {
                ...
                .si_then([&start_from] { return start_from; });
          });
    }
    ...
    handle_boundary_ret
    handle_boundary(op_context_t c, mapped_space_visitor_t* visitor)
    {
      assert(at_boundary());
      return seastar::do_with(
          (depth_t)2, [c, this, visitor](auto& depth_with_space) {
            return ensure_internal_bottom_up(
                       c, depth_with_space,
                       [this](auto depth_with_space) {
                         return this->get_internal(depth_with_space).pos + 1 <
                                this->get_internal(depth_with_space)
                                    .node->get_size();
                       })
                .si_then([c, this, visitor](auto depth_with_space) {
                  if (depth_with_space <= get_depth()) {
                    return seastar::do_with(
                        ...
                        [this, c, depth_with_space,
                         visitor](auto& li, auto& ll) {
                          for (depth_t depth = 2; depth < depth_with_space;
                               ++depth) {
                          ...
                          // note, cannot result in at_boundary() by construction
                          return lookup_depth_range(
                              c, *this, depth_with_space - 1, 0, li, ll,
                              visitor);
                        });
                  } else {
                    // end
  ...
  template <typename T>
  void
  set_root_node(const TCachedExtentRef<T>& root_node)
  {
  ...
  auto
  get_root_node(op_context_t c) const
  {
  ...
  static mkfs_ret
  mkfs(RootBlockRef& root_block, op_context_t c)
  {
    ...
    fixed_kv_node_meta_t<node_key_t> meta{
        min_max_t<node_key_t>::min, min_max_t<node_key_t>::max, 1};
    root_leaf->set_meta(meta);
    root_leaf->range = meta;
    get_tree_stats<self_type>(c.trans).depth = 1u;
    get_tree_stats<self_type>(c.trans).extents_num_delta++;
    TreeRootLinker<RootBlock, leaf_node_t>::link_root(
        root_block, root_leaf.get());
    return phy_tree_root_t{root_leaf->get_paddr(), 1u};
  }

  iterator
  make_partial_iter(op_context_t c, cursor_t& cursor)
  {
    return make_partial_iter(
        c, cursor.parent->template cast<leaf_node_t>(), cursor.get_key(),
        cursor.get_pos());
  }

  boost::intrusive_ptr<cursor_t>
  get_cursor(op_context_t c, TCachedExtentRef<leaf_node_t> leaf, node_key_t key)
  {
    ...
    return lookup(
               ...
               [FNAME, c, addr](const leaf_node_t& leaf) {
                 ...
                 SUBTRACET(
                     ...
                     c.trans, addr, ret.get_offset(), leaf.get_size(),
                     ret == leaf.end());
                 return ret;
               },
               min_depth, visitor)
        .si_then([FNAME, c, min_depth](auto&& ret) {
          SUBTRACET(
              seastore_fixedkv_tree, "ret.leaf.pos {}", c.trans, ret.leaf.pos);
#ifndef NDEBUG
          if (min_depth == 1) {
            ret.assert_valid();
          }
   ...
   */
  iterator_fut
  upper_bound(op_context_t c, node_key_t addr) const
  {
   ...
   */
  iterator_fut
  upper_bound_right(op_context_t c, node_key_t addr) const
  {
    return lower_bound(c, addr).si_then([c, addr](auto iter) {
      if (iter.is_begin()) {
        return iterator_fut(interruptible::ready_future_marker{}, iter);
  ...
  iterator_fut
  begin(op_context_t c) const
  {
    return lower_bound(c, min_max_t<node_key_t>::min);
  }

  iterator_fut
  end(op_context_t c) const
  {
  ...
  template <
      typename child_node_t,
      typename node_t,
      bool lhc = leaf_has_children,
      typename std::enable_if<lhc, int>::type = 0>
  void
  check_node(op_context_t c, TCachedExtentRef<node_t> node)
  {
    for (auto i : *node) {
      ...
      if constexpr (
        ...
        ret = c.trans.get_extent(
            i->get_val().maybe_relative_to(node->get_paddr()), &child_node);
      } else {
        if (i->get_val().pladdr.is_laddr()) {
          assert(
              !node->children[i->get_offset()] ||
      ...
      if (ret == Transaction::get_extent_ret::PRESENT) {
          ...
          if (child_node->is_mutation_pending()) {
            ...
            if (node->is_pending()) {
              ...
              [[maybe_unused]] auto pos =
                  n.lower_bound(i->get_key()).get_offset();
              assert(pos < n.get_size());
        ...
        BaseChildNode<std::remove_reference_t<decltype(*node)>, node_key_t>*
            child = nullptr;
        if (node->is_pending()) {
          auto& n = node->get_stable_for_key(i->get_key());
        ...
        if (!is_valid_child_ptr(child)) {
          ...
          if (is_reserved_ptr(child)) {
            if constexpr (!std::is_base_of_v<
                              typename internal_node_t::base_t, child_node_t>) {
          ...
          assert(
              c->peek_parent_node().get() == node.get() ||
              (node->is_pending() && c->is_stable() &&
               c->peek_parent_node().get() ==
                   &node->get_stable_for_key(i->get_key())));
        }
      } else {
        ceph_abort_msg("impossible");
      }
  ...
  template <
      bool lhc = leaf_has_children,
      typename std::enable_if<lhc, int>::type = 0>
  check_child_trackers_ret
  check_child_trackers(op_context_t c)
  {
    mapped_space_visitor_t checker = [c, this](
                                         paddr_t, node_key_t, extent_len_t,
                                         depth_t depth, extent_types_t,
                                         iterator& iter) {
      if (depth == 1) {
        return seastar::now();
      }
    ...
    return seastar::do_with(std::move(checker), [this, c](auto& checker) {
      return iterate_repeat(
          c, lower_bound(c, min_max_t<node_key_t>::min, &checker),
          [](auto& pos) {
            if (pos.is_end()) {
  ...
  iterate_repeat(
      ...
      mapped_space_visitor_t* visitor = nullptr)
  {
    return std::move(iter_fut).si_then([c, visitor,
                                        f = std::forward<F>(f)](auto iter) {
      return seastar::do_with(
          iter, std::move(f), [c, visitor](auto& pos, auto& f) {
      ...
      BaseChildNode<leaf_node_t, node_key_t>* child)
  {
    ...
    SUBTRACET(
        seastore_fixedkv_tree, "inserting laddr {} at iter {}", c.trans, laddr,
        iter.is_end() ? min_max_t<node_key_t>::max : iter.get_key());
    return seastar::do_with(iter, [this, c, laddr, val, child](auto& ret) {
      return find_insertion(c, laddr, ret)
          .si_then([this, c, laddr, val, &ret, child] {
            if (!ret.at_boundary() && ret.get_key() == laddr) {
              ...
              return handle_split(c, ret).si_then([c, laddr, val, &ret, child] {
                if (!ret.leaf.node->is_mutable()) {
                  CachedExtentRef mut =
                      c.cache.duplicate_for_write(c.trans, ret.leaf.node);
  ...
  insert(
      ...
      BaseChildNode<leaf_node_t, node_key_t>* child)
  {
    return lower_bound(c, laddr).si_then([child, this, c, laddr,
                                          val](auto iter) {
      ...
      BaseChildNode<leaf_node_t, node_key_t>* child)
  {
    ...
    SUBTRACET(
        seastore_fixedkv_tree, "update element at {}", c.trans,
        iter.is_end() ? min_max_t<node_key_t>::max : iter.get_key());
    ...
    iter.leaf.node->update(iter.leaf.node->iter_idx(iter.leaf.pos), val);
    if constexpr (
        std::is_base_of_v<ParentNode<leaf_node_t, node_key_t>, leaf_node_t>) {
      if (child) {
  ...
  remove_ret
  remove(op_context_t c, iterator iter)
  {
    LOG_PREFIX(FixedKVBtree::remove);
    SUBTRACET(
        seastore_fixedkv_tree, "remove element at {}", c.trans,
        iter.is_end() ? min_max_t<node_key_t>::max : iter.get_key());
    assert(!iter.is_end());
  ...
  init_cached_extent_ret
  init_cached_extent(op_context_t c, CachedExtentRef e)
  {
    assert(!e->is_logical());
    ...
    if (e->get_type() == internal_node_t::TYPE) {
      ...
      return lower_bound(c, eint->get_node_meta().begin)
          .si_then([e, c, eint](auto iter) {
            ...
            if (cand_depth <= iter.get_depth() &&
              ...
              SUBTRACET(
                  seastore_fixedkv_tree, "extent {} is live", c.trans, *eint);
              return true;
            } else {
              SUBTRACET(
                  seastore_fixedkv_tree, "extent {} is not live", c.trans,
                  *eint);
              return false;
            }
          });
      ...
      return lower_bound(c, eleaf->get_node_meta().begin)
          .si_then([c, e, eleaf](auto iter) {
            // Note, this check is valid even if iter.is_end()
            LOG_PREFIX(FixedKVTree::init_cached_extent);
            if (iter.leaf.node == &*eleaf) {
              SUBTRACET(
                  seastore_fixedkv_tree, "extent {} is live", c.trans, *eleaf);
              return true;
            } else {
              SUBTRACET(
                  seastore_fixedkv_tree, "extent {} is not live", c.trans,
      ...
      SUBTRACET(
          seastore_fixedkv_tree, "found other extent {} type {}", c.trans, *e,
          e->get_type());
      ...
      extent_len_t len)
  {
    ...
    return lower_bound(c, laddr).si_then([FNAME, c, addr, laddr,
                                          len](auto iter) {
      if (iter.leaf.node->get_paddr() == addr) {
        SUBTRACET(
            seastore_fixedkv_tree, "extent laddr {} addr {}~{} found: {}",
            c.trans, laddr, addr, len, *iter.leaf.node);
  ...
  using get_internal_if_live_ret =
      get_internal_if_live_iertr::future<CachedExtentRef>;

  get_internal_if_live_ret
  get_internal_if_live(
      op_context_t c,
      paddr_t addr,
      node_key_t laddr,
      extent_len_t len)
    ...
    return lower_bound(c, laddr).si_then([FNAME, c, addr, laddr,
      ...
      for (depth_t d = 2; d <= iter.get_depth(); ++d) {
        ...
        if (internal_node->get_paddr() == addr) {
          SUBTRACET(
              seastore_fixedkv_tree, "extent laddr {} addr {}~{} found: {}",
              c.trans, laddr, addr, len, *internal_node);
  ...
  rewrite_extent_ret
  rewrite_extent(op_context_t c, CachedExtentRef e)
  {
    ...
    auto do_rewrite = [&](auto& fixed_kv_extent) {
      auto n_fixed_kv_extent = c.cache.template alloc_new_non_data_extent<
          ...
          c.trans, fixed_kv_extent.get_length(),
      ...
      SUBTRACET(
          seastore_fixedkv_tree, "rewriting {} into {}", c.trans,
          fixed_kv_extent, *n_fixed_kv_extent);

      return update_internal_mapping(
                 c, n_fixed_kv_extent->get_node_meta().depth,
                 n_fixed_kv_extent->get_node_meta().begin, e->get_paddr(),
                 n_fixed_kv_extent->get_paddr(), n_fixed_kv_extent)
  ...
  template <typename T>
    requires std::is_same_v<internal_node_t, T> ||
             std::is_same_v<leaf_node_t, T>
      ...
      TCachedExtentRef<T> nextent)
  {
    ...
    SUBTRACET(
        seastore_fixedkv_tree,
        "updating laddr {} at depth {} from {} to {}, nextent {}", c.trans,
        laddr, depth, old_addr, new_addr, *nextent);
      ...
      if (depth == iter.get_depth()) {
        ...
        if (laddr != min_max_t<node_key_t>::min) {
          SUBERRORT(
              ...
              "laddr is not 0",
              c.trans, laddr, depth, old_addr, new_addr,
        ...
        root_block = c.cache.duplicate_for_write(c.trans, root_block)
        ...
        if (piter->get_key() != laddr) {
          SUBERRORT(
              ...
              c.trans, laddr, depth, old_addr, new_addr, *(parent.node),
              parent.pos, piter->get_key());
          ceph_assert(0 == "impossible");
        }
...
private:
      ...
      uint16_t pos)
  {
    ...
    auto ret = iterator(
        depth,
        depth == 1 ? iterator::state_t::FULL : iterator::state_t::PARTIAL);
#else
      ...
      typename std::optional<node_position_t<internal_node_t>> parent_pos)
  {
    ...
    SUBTRACET(
        seastore_fixedkv_tree,
        "reading internal at offset {}, depth {}, begin {}, end {}", c.trans,
        offset, depth, begin, end);
    assert(depth > 1);
    ...
    return c.cache
        .template maybe_get_absent_extent<internal_node_t>(
            c.trans, offset, node_size, init_internal)
        .si_then([FNAME, c, offset, init_internal, depth, begin,
                  end](typename internal_node_t::Ref ret) {
          if (unlikely(
            ...
            SUBERRORT(
                ...
                "recorded: {}, actually: {}",
                c.trans, ret->get_in_extent_checksum(),
                ret->get_last_committed_crc());
            ceph_abort();
          }
          SUBTRACET(
              seastore_fixedkv_tree, "read internal at offset {} {}", c.trans,
              offset, *ret);
          ...
          return get_internal_node_ret(
              interruptible::ready_future_marker{}, ret);
        });
  }
      ...
      typename std::optional<node_position_t<internal_node_t>> parent_pos)
  {
    ...
    SUBTRACET(
        seastore_fixedkv_tree, "reading leaf at offset {}, begin {}, end {}",
        c.trans, offset, begin, end);
    auto init_leaf = [c, begin, end,
                      parent_pos = std::move(parent_pos)](leaf_node_t& node) {
    ...
    return c.cache
        .template maybe_get_absent_extent<leaf_node_t>(
            c.trans, offset, node_size, init_leaf)
        .si_then([FNAME, c, offset, init_leaf, begin,
                  end](typename leaf_node_t::Ref ret) {
          if (unlikely(
                  ret->get_in_extent_checksum() !=
                  ret->get_last_committed_crc())) {
            SUBERRORT(
                seastore_fixedkv_tree,
                "leaf fixedkv extent checksum inconsistent, recorded: {}, "
                "actually: {}",
                c.trans, ret->get_in_extent_checksum(),
                ret->get_last_committed_crc());
            ceph_abort();
          ...
          SUBTRACET(
              seastore_fixedkv_tree, "read leaf at offset {} {}", c.trans,
              offset, *ret);
          // This can only happen during init_cached_extent
          // or when backref extent being rewritten by gc space reclaiming
          if (ret->is_stable() && !ret->is_linked()) {
  ...
  lookup_root_ret
  lookup_root(op_context_t c, iterator& iter, mapped_space_visitor_t* visitor)
      const
  {
    LOG_PREFIX(FixedKVBtree::lookup_root);
    SUBTRACET(
        seastore_fixedkv_tree, "looking up root on {}", c.trans, *root_block);
    ...
    auto on_found_internal = [this, visitor, &iter](InternalNodeRef& root_node) {
      ...
      if (visitor)
        (*visitor)(
            root_node->get_paddr(), root_node->get_node_meta().begin,
            root_node->get_length(), get_root().get_depth(),
            internal_node_t::TYPE, iter);
    ...
    auto on_found_leaf = [visitor, &iter, this](LeafNodeRef root_node) {
      ...
      if (visitor)
        (*visitor)(
            root_node->get_paddr(), root_node->get_node_meta().begin,
            root_node->get_length(), get_root().get_depth(), leaf_node_t::TYPE,
            iter);
      return lookup_root_iertr::now();
    };

    if (found) {
      return fut.si_then([this, c,
                          on_found_internal = std::move(on_found_internal),
                          on_found_leaf = std::move(on_found_leaf)](auto root) {
        LOG_PREFIX(FixedKVBtree::lookup_root);
        ceph_assert(root);
        SUBTRACET(
            seastore_fixedkv_tree, "got root node on {}, res: {}", c.trans,
            *root_block, *root);
      ...
      if (get_root().get_depth() > 1) {
        return get_internal_node(
                   c, get_root().get_depth(), get_root().get_location(),
                   min_max_t<node_key_t>::min, min_max_t<node_key_t>::max,
                   std::nullopt)
            .si_then([on_found = std::move(on_found_internal)](
                         InternalNodeRef root_node) {
              return on_found(root_node);
            });
      } else {
        return get_leaf_node(
                   c, get_root().get_location(), min_max_t<node_key_t>::min,
                   min_max_t<node_key_t>::max, std::nullopt)
            .si_then([on_found = std::move(on_found_leaf)](
                         LeafNodeRef root_node) { return on_found(root_node); });
      }
    }
  }
  ...
  template <typename F>
  static lookup_internal_level_ret
  lookup_internal_level(
      op_context_t c,
      depth_t depth,
      iterator& iter,
      F& f,
      mapped_space_visitor_t* visitor)
  {
    ...
    auto on_found = [depth, visitor, &iter, &f](InternalNodeRef node) {
      ...
      if (visitor)
        (*visitor)(
            node->get_paddr(), node->get_node_meta().begin, node->get_length(),
            depth, node->get_type(), iter);
      return seastar::now();
    };
    ...
    if (v.has_child()) {
      return std::move(v.get_child_fut())
          .si_then([on_found = std::move(on_found), node_iter, c,
            ...
            SUBTRACET(
                seastore_fixedkv_tree, "got child on {}, pos: {}, res: {}",
                c.trans, *parent_entry.node, parent_entry.pos, *child);
            [[maybe_unused]] auto& cnode =
                (typename internal_node_t::base_t&)*child;
    ...
    auto end = next_iter == parent->end() ? parent->get_node_meta().end
                                          : next_iter->get_key();
    return get_internal_node(
               c, depth,
               ...
               std::make_optional<node_position_t<internal_node_t>>(
                   child_pos.get_parent(), child_pos.get_pos()))
      ...
      mapped_space_visitor_t* visitor)
  {
    ...
    return get_leaf_node(
               c, node_iter->get_val().maybe_relative_to(parent->get_paddr()),
               begin, end,
               std::make_optional<node_position_t<internal_node_t>>(
                   child_pos.get_parent(), child_pos.get_pos()))
...
#endif
        std::forward<LI>(lookup_internal), std::forward<LL>(lookup_leaf),
        [FNAME, this, visitor, c, min_depth](auto& iter, auto& li, auto& ll) {
          return lookup_root(c, iter, visitor)
              .si_then([FNAME, this, visitor, c, &iter, &li, &ll, min_depth] {
                ...
                SUBTRACET(
                    seastore_fixedkv_tree, "got root, depth {}", c.trans,
                    get_root().get_depth());
                return lookup_depth_range(
                           c, iter, get_root().get_depth() - 1, min_depth - 1,
                           li, ll, visitor)
  ...
  static find_insertion_ret
  find_insertion(op_context_t c, node_key_t laddr, iterator& iter)
  {
  ...
  handle_split_ret
  handle_split(op_context_t c, iterator& iter)
  {
    LOG_PREFIX(FixedKVBtree::handle_split);

    return iter.check_split(c).si_then([FNAME, this, c, &iter](auto split_from) {
      SUBTRACET(
          seastore_fixedkv_tree, "split_from {}, depth {}", c.trans, split_from,
          iter.get_depth());
      ...
      if (split_from == iter.get_depth()) {
        ...
        nroot->journal_insert(
            nroot->begin(), min_max_t<node_key_t>::min,
            get_root().get_location(), nullptr);
      ...
      auto split_level = [&, c, FNAME](auto& parent_pos, auto& pos) {
        ...
        parent_node->insert(
            parent_iter + 1, pivot, right->get_paddr(), right.get());

        SUBTRACET(
            seastore_fixedkv_tree, "splitted {} into left: {}, right: {}",
      ...
      for (; split_from > 0; --split_from) {
        ...
        if (!parent_pos.node->is_mutable()) {
          parent_pos.node = c.cache
                                .duplicate_for_write(c.trans, parent_pos.node)
        ...
        if (split_from > 1) {
          ...
          SUBTRACET(
              ...
              c.trans, *pos.node, split_from, *parent_pos.node, parent_pos.pos);
          ...
          auto& pos = iter.leaf;
          SUBTRACET(
              seastore_fixedkv_tree, "splitting leaf {}, parent: {} at pos: {}",
              c.trans, *pos.node, *parent_pos.node, parent_pos.pos);
          auto [left, right] = split_level(parent_pos, pos);
  ...
  handle_merge_ret
  handle_merge(op_context_t c, iterator& iter)
  {
    LOG_PREFIX(FixedKVBtree::handle_merge);
    if (iter.get_depth() == 1 || !iter.leaf.node->below_min_capacity()) {
      SUBTRACET(
          seastore_fixedkv_tree,
          "no need to merge leaf, leaf size {}, depth {}", c.trans,
          iter.leaf.node->get_size(), iter.get_depth());
      return seastar::now();
    }
              ...
              return merge_fut.si_then([FNAME, this, c, &iter, &to_merge] {
                ...
                if (to_merge == iter.get_depth()) {
                  ...
                  if (pos.node->get_size() == 1) {
                    ...
                    root_block = c.cache
                                     .duplicate_for_write(c.trans, root_block)
                                     ->template cast<RootBlock>();
                    get_root().set_location(
                        node_iter->get_val().maybe_relative_to(
                            pos.node->get_paddr()));
                    get_root().set_depth(iter.get_depth());
                    if (iter.get_depth() > 1) {
                    ...
                    SUBTRACET(
                        seastore_fixedkv_tree, "no need to collapse root",
                        c.trans);
                  }
                  return seastar::stop_iteration::yes;
                  ...
                  SUBTRACET(
                      seastore_fixedkv_tree,
                      "continuing, next node {} depth {} at min", c.trans,
                      *pos.node, to_merge);
                  return seastar::stop_iteration::no;
                } else {
                  SUBTRACET(
                      seastore_fixedkv_tree,
                      "complete, next node {} depth {} not min", c.trans,
                      *pos.node, to_merge);
                  return seastar::stop_iteration::yes;
                }
              });
      ...
      node_position_t<NodeType>& pos)
  {
    ...
    bool donor_is_left =
        ((iter.get_offset() + 1) == parent_pos.node->get_size());
    ...
    SUBTRACET(
        seastore_fixedkv_tree, "parent: {}, node: {}", c.trans,
        *parent_pos.node, *pos.node);
    auto do_merge = [c, iter, donor_iter, donor_is_left, &parent_pos,
                     &pos](typename NodeType::Ref donor) {
      LOG_PREFIX(FixedKVBtree::merge_level);
      auto [l, r] = donor_is_left ? std::make_pair(donor, pos.node)
                                  : std::make_pair(pos.node, donor);

      auto [liter, riter] = donor_is_left ? std::make_pair(donor_iter, iter)
                                          : std::make_pair(iter, donor_iter);

      if (donor->at_min_capacity()) {
        auto replacement = l->make_full_merge(c, r);

        parent_pos.node->update(
            liter, replacement->get_paddr(), replacement.get());
        ...
        SUBTRACET(
            seastore_fixedkv_tree, "l: {}, r: {}, replacement: {}", c.trans, *l,
            *r, *replacement);
        ...
        auto [replacement_l, replacement_r, pivot] =
            l->make_balanced(c, r, pivot_idx);

        parent_pos.node->update(
            liter, replacement_l->get_paddr(), replacement_l.get());
        parent_pos.node->replace(
            riter, pivot, replacement_r->get_paddr(), replacement_r.get());
        ...
        SUBTRACET(
            seastore_fixedkv_tree,
            "l: {}, r: {}, replacement_l: {}, replacement_r: {}", c.trans, *l,
            *r, *replacement_l, *replacement_r);
        c.cache.retire_extent(c.trans, l);
        c.cache.retire_extent(c.trans, r);
      }
    ...
    if (v.has_child()) {
      return std::move(v.get_child_fut())
          .si_then([do_merge = std::move(do_merge), &pos, donor_iter,
                    donor_is_left, c, parent_pos](auto child) {
            LOG_PREFIX(FixedKVBtree::merge_level);
            SUBTRACET(
                seastore_fixedkv_tree, "got child on {}, pos: {}, res: {}",
                c.trans, *parent_pos.node, donor_iter.get_offset(), *child);
            [[maybe_unused]] auto& node =
                (typename internal_node_t::base_t&)*child;
            assert(
                donor_is_left ? node.get_node_meta().end ==
                                    pos.node->get_node_meta().begin
                              : node.get_node_meta().begin ==
                                    pos.node->get_node_meta().end);
    ...
    return get_node<NodeType>(
               c, depth,
               donor_iter.get_val().maybe_relative_to(
                   parent_pos.node->get_paddr()),
               begin, end,
               std::make_optional<node_position_t<internal_node_t>>(
                   child_pos.get_parent(), child_pos.get_pos()))
        .si_then([do_merge = std::move(do_merge)](
                     typename NodeType::Ref donor) { return do_merge(donor); });
  }
};
...
struct is_fixed_kv_tree<
    FixedKVBtree<node_key_t, node_val_t, internal_node_t, leaf_node_t, cursor_t, node_size>>
  : std::true_type {};
...
template <
    ...
    std::enable_if_t<is_fixed_kv_tree<tree_type_t>::value, int> = 0>
auto
with_btree(Cache& cache, op_context_t c, F&& f)
{
  return cache.get_root(c.trans).si_then(
      [f = std::forward<F>(f)](RootBlockRef croot) mutable {
        return seastar::do_with(
            tree_type_t(croot),
            [f = std::move(f)](auto& btree) mutable { return f(btree); });
      });
}
...
template <
    ...
    std::enable_if_t<is_fixed_kv_tree<tree_type_t>::value, int> = 0>
auto
with_btree_state(Cache& cache, op_context_t c, State&& init, F&& f)
{
  return seastar::do_with(
...
template <
    ...
    std::enable_if_t<is_fixed_kv_tree<tree_type_t>::value, int> = 0>
auto
with_btree_state(Cache& cache, op_context_t c, F&& f)
{
