...
struct FixedKVNode : CachedExtent {
  ...
  FixedKVNode(const FixedKVNode& rhs) :
    CachedExtent(rhs), range(rhs.range)
  {}
  ...
  bool
  is_in_range(const node_key_t key) const
  {
...
struct FixedKVInternalNode : FixedKVNode<NODE_KEY>,
  ...
  using node_layout_t = common::FixedKVNodeLayout<
      CAPACITY,
      fixed_kv_node_meta_t<NODE_KEY>,
      fixed_kv_node_meta_le_t<NODE_KEY_LE>,
      NODE_KEY,
  ...
  using this_type_t =
      FixedKVInternalNode<CAPACITY, NODE_KEY, NODE_KEY_LE, node_size, node_type_t>;
  ...
  explicit FixedKVInternalNode(extent_len_t length) :
    FixedKVNode<NODE_KEY>(length), ParentNode<node_type_t, NODE_KEY>(CAPACITY)
  {}

  FixedKVInternalNode(const FixedKVInternalNode& rhs) :
    FixedKVNode<NODE_KEY>(rhs), ParentNode<node_type_t, NODE_KEY>(rhs)
  {
    this->set_layout_buf(this->get_bptr().c_str());
  }

  uint16_t
  get_node_split_pivot() const final
  {
  ...
  void
  on_data_commit() final
  {
    this->set_layout_buf(this->get_bptr().c_str());
  }

  void
  on_invalidated(Transaction& t) final
  {
  ...
  uint32_t
  calc_crc32c() const final
  {
    return this->calc_phy_checksum();
  }

  void
  update_in_extent_chksum_field(uint32_t crc) final
  {
    this->set_phy_checksum(crc);
  }

  uint32_t
  get_in_extent_checksum() const
  {
  ...
  typename node_layout_t::delta_buffer_t*
  ...
  void
  on_replace_prior(Transaction& t) final
  {
    if (!is_rewrite_transaction(t.get_src())) {
  ...
  void
  update(internal_const_iterator_t iter, paddr_t addr, base_child_t* nextent)
  {
    LOG_PREFIX(FixedKVInternalNode::update);
    SUBTRACE(
        seastore_fixedkv_tree, "trans.{}, pos {}, {}",
        this->pending_for_transaction, iter.get_offset(), (void*)nextent);
    this->update_child_ptr(iter.get_offset(), nextent);
    return this->journal_update(
        iter, this->maybe_generate_relative(addr), maybe_get_delta_buffer());
  }
      ...
      base_child_t* nextent)
  {
    ...
    SUBTRACE(
        ...
        this->pending_for_transaction, iter.get_offset(), pivot,
        (void*)nextent);
    this->insert_child_ptr(iter.get_offset(), nextent);
    return this->journal_insert(
        iter, pivot, this->maybe_generate_relative(addr),
        maybe_get_delta_buffer());
  }

  void
  remove(internal_const_iterator_t iter)
  {
      ...
      base_child_t* nextent)
  {
    ...
    SUBTRACE(
        seastore_fixedkv_tree, "trans.{}, pos {}, old key {}, key {}, {}",
        this->pending_for_transaction, iter.get_offset(), iter.get_key(), pivot,
        (void*)nextent);
    this->update_child_ptr(iter.get_offset(), nextent);
  ...
  Ref
  make_full_merge(op_context_t c, Ref& right)
  {
  ...
  std::tuple<Ref, Ref, NODE_KEY>
  make_balanced(op_context_t c, Ref& _right, uint32_t pivot_idx)
  {
    ceph_assert(_right->get_type() == this->get_type());
    auto& right = *_right->template cast<node_type_t>();
    auto replacement_left =
        c.cache.template alloc_new_non_data_extent<node_type_t>(
            c.trans, node_size, placement_hint_t::HOT, INIT_GENERATION);
    ...
    this->balance_child_ptrs(
        c.trans, static_cast<node_type_t&>(*this), right, pivot_idx,
        *replacement_left, *replacement_right);
    auto pivot = this->balance_into_new_nodes(
        *this, right, pivot_idx, *replacement_left, *replacement_right);
  ...
  void
  node_resolve_vals(internal_iterator_t from, internal_iterator_t to) const
  {
  ...
  void
  node_unresolve_vals(internal_iterator_t from, internal_iterator_t to) const
  {
    if (this->is_initial_pending()) {
      for (auto i = from; i != to; ++i) {
        if (i->get_val().is_relative()) {
  ...
  print_detail(std::ostream& out) const
  {
    ...
    if (this->my_tracker) {
      out << ", my_tracker->parent="
          << (void*)this->my_tracker->get_parent().get();
    }
  ...
  void
  apply_delta_and_adjust_crc(paddr_t base, const ceph::bufferlist& _bl)
  {
    assert(_bl.length());
...
struct FixedKVLeafNode
  ...
  FixedKVLeafNode(const FixedKVLeafNode& rhs) :
    FixedKVNode<NODE_KEY>(rhs), modifications(rhs.modifications)
  {
    this->set_layout_buf(this->get_bptr().c_str());
  }
  ...
  bool
  modified_since(uint64_t v) const
  {
  ...
  virtual typename node_layout_t::delta_buffer_t*
  maybe_get_delta_buffer()
  {
    return this->is_mutation_pending() ? &delta_buffer : nullptr;
  }
  ...
  virtual void update(internal_const_iterator_t iter, VAL val) = 0;
...
struct fmt::formatter<
    crimson::os::seastore::FixedKVNode<crimson::os::seastore::laddr_t>>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<
    crimson::os::seastore::FixedKVNode<crimson::os::seastore::paddr_t>>
  : fmt::ostream_formatter {};
#endif
