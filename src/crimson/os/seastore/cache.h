...
public:
      ...
      bool is_weak)
  {
    ...
    auto ret = std::make_unique<Transaction>(
        get_dummy_ordering_handle(), is_weak, src,
        [this](Transaction& t) { return on_transaction_destruct(t); },
        ++next_id, cache_hint);
    SUBDEBUGT(
        seastore_t, "created name={}, source={}, is_weak={}", *ret, name, src,
        is_weak);
  ...
  void
  retire_extent(Transaction& t, CachedExtentRef ref)
  {
      ...
      extent_types_t type)
  {
    ...
    if (result == Transaction::get_extent_ret::RETIRED) {
      SUBDEBUGT(
          seastore_cache, "{} {}~0x{:x} is retired on t", t, type, paddr, len);
      ...
      if (!ret->is_fully_loaded()) {
        SUBDEBUGT(
            seastore_cache,
            "{} {}~0x{:x} is present on t without fully loaded -- {}", t, type,
            paddr, len, *ret);
      ...
      SUBTRACET(
          seastore_cache, "{} {}~0x{:x} is present on t -- {}", t, type, paddr,
          len, *ret);
    ...
    if (!ret) {
      SUBDEBUGT(
          seastore_cache, "{} {}~0x{:x} is absent in cache", t, type, paddr,
          len);
      return get_extent_if_cached_iertr::make_ready_future<CachedExtentRef>();
    }
    ...
    if (is_retired_placeholder_type(ret->get_type())) {
      ...
      SUBDEBUGT(
          seastore_cache,
          "{} {}~0x{:x} ~0x{:x} is absent(placeholder) in cache", t, type,
          paddr, len, ret->get_length());
      return get_extent_if_cached_iertr::make_ready_future<CachedExtentRef>();
    }
    ...
    if (!ret->is_fully_loaded()) {
      SUBDEBUGT(
          seastore_cache,
          "{} {}~0x{:x} is present without fully loaded in cache -- {}", t,
          type, paddr, len, *ret);
      return trans_intr::make_interruptible(
          do_read_extent_maybe_partial<CachedExtent>(
              ret->cast<CachedExtent>(), 0, ret->get_length(), &t_src));
    ...
    SUBDEBUGT(
        seastore_cache, "{} {}~0x{:x} is present in cache -- {}", t, type,
        paddr, len, *ret);
    return ret->wait_io().then([ret] {
  ...
  get_caching_extent(Transaction& t, paddr_t offset, extent_len_t length)
  {
    CachedExtentRef ret;
    ...
    if (result == Transaction::get_extent_ret::RETIRED) {
      SUBERRORT(
          seastore_cache, "{} {}~0x{:x} is retired on t -- {}", t, T::TYPE,
          offset, length, *ret);
      ...
      if (ret->is_fully_loaded()) {
        SUBTRACET(
            seastore_cache, "{} {}~0x{:x} is present on t -- {}", t, T::TYPE,
            offset, length, *ret);
        return ret->wait_io().then([ret] {
          return seastar::make_ready_future<TCachedExtentRef<T>>(ret->cast<T>());
        });
      } else {
        SUBDEBUGT(
            seastore_cache,
        ...
        return do_read_extent_maybe_partial<T>(
            ret->cast<T>(), 0, length, &t_src);
      }
    } else {
      SUBTRACET(
          seastore_cache, "{} {}~0x{:x} is absent on t, query cache ...", t,
      ...
      return trans_intr::make_interruptible(do_get_caching_extent<T>(
          offset, length, [](T&) {}, std::move(f), &t_src));
    }
  }

  void
  invalidate_retired_placeholder(
      Transaction& t,
      ...
      Func&& extent_init_func)
  {
    ...
    cache_access_stats_t& access_stats =
        get_by_ext(get_by_src(stats.access_by_src_ext, t_src), T::TYPE);
    ++access_stats.load_absent;
    ++stats.access.load_absent;
    t.add_to_read_set(CachedExtentRef(ret));
    touch_extent_by_range(
        *ret, &t_src, t.get_cache_hint(), partial_off, partial_len);
    return ret;
  }
      ...
      uint32_t pin_crc = CRC_NULL)
  {
    ...
    const auto t_src = t.get_src();
    return trans_intr::make_interruptible(read_extent<T>(
        std::move(ret), partial_off, partial_len, &t_src, pin_crc));
  }
  ...
  template <typename T>
  get_extent_iertr::future<TCachedExtentRef<T>>
  get_absent_extent(Transaction& t, paddr_t offset, extent_len_t length)
  {
  ...
  get_absent_extent(
      ...
      Func&& extent_init_func)
  {
    return get_absent_extent<T>(
        t, offset, length, 0, length, std::forward<Func>(extent_init_func));
  }

  template <typename T, typename Func>
  get_extent_iertr::future<TCachedExtentRef<T>>
  maybe_get_absent_extent(
      Transaction& t,
      paddr_t offset,
      extent_len_t length,
      Func&& extent_init_func)
  {
    ...
    if (unlikely(booting)) {
      ...
      return trans_intr::make_interruptible(do_get_caching_extent<T>(
          offset, length, 0, length, std::forward<Func>(extent_init_func),
          std::move(f), &t_src));
    }
    return get_absent_extent<T>(
        t, offset, length, 0, length, std::forward<Func>(extent_init_func));
  ...
  CachedExtentRef
  peek_extent_viewable_by_trans(Transaction& t, CachedExtentRef extent) final
  {
    assert(extent);
  ...
  get_extent_iertr::future<>
  maybe_wait_accessible(Transaction& t, CachedExtent& extent) final
  {
    ...
    cache_access_stats_t& access_stats =
        get_by_ext(get_by_src(stats.access_by_src_ext, t_src), ext_type);
    if (extent.is_stable()) {
      ...
      return trans_intr::make_interruptible(extent.wait_io())
          .then_interruptible([target_extent, needs_touch, needs_step_2, &t,
                               this, t_src] {
            if (needs_step_2) {
  ...
  get_extent_viewable_by_trans(Transaction& t, CachedExtentRef extent) final
  {
    assert(extent->is_valid());
    ...
    assert(is_logical_type(p_extent->get_type()) || p_extent->is_fully_loaded());

    auto target_extent = CachedExtentRef(p_extent);
    return trans_intr::make_interruptible(p_extent->wait_io())
        .then_interruptible([target_extent, needs_touch, needs_step_2, &t, this,
                             t_src] {
      ...
      extent_len_t partial_len)
  {
    ...
    if (!extent->is_range_loaded(partial_off, partial_len)) {
      ...
      cache_access_stats_t& access_stats = get_by_ext(
          get_by_src(stats.access_by_src_ext, t_src), extent->get_type());
      ++access_stats.load_present;
      ++stats.access.load_present;
      return trans_intr::make_interruptible(do_read_extent_maybe_partial(
...
public:
  CachedExtentRef
  test_query_cache(paddr_t offset)
  {
...
private:
      ...
      const Transaction::src_t* p_src)
  {
    ...
    if (extent->is_pending_io()) {
      ...
      return p_extent->wait_io().then(
          [extent = std::move(extent), partial_off, partial_len, this, FNAME,
           src]() mutable -> read_extent_ret<T> {
            if (extent->is_range_loaded(partial_off, partial_len)) {
              SUBDEBUG(
                  seastore_cache,
              ...
              Transaction::src_t* p_src =
                  (src.has_value() ? &src.value() : nullptr);
      ...
      SUBDEBUG(
          ...
          extent->get_type(), extent->get_paddr(), extent->get_length(),
          partial_off, partial_len, *extent);
      return read_extent<T>(std::move(extent), partial_off, partial_len, p_src);
    }
  }
      ...
      const Transaction::src_t* p_src)
  {
    ...
    if (!cached) {
      ...
      on_cache(*ret);
      return read_extent<T>(std::move(ret), partial_off, partial_len, p_src);
    }
    ...
    if (is_retired_placeholder_type(cached->get_type())) {
      ...
      SUBDEBUG(
          seastore_cache,
          "{} {}~0x{:x} is absent(placeholder), add extent and reading range "
          "0x{:x}~0x{:x} ... -- {}",
          T::TYPE, offset, length, partial_off, partial_len, *ret);
      extent_init_func(*ret);
      on_cache(*ret);
    ...
    if (ret->is_range_loaded(partial_off, partial_len)) {
      SUBTRACE(
          seastore_cache,
          "{} {}~0x{:x} is present with range 0x{:x}~0x{:x} ... -- {}", T::TYPE,
          offset, length, partial_off, partial_len, *ret);
      return ret->wait_io().then([ret] {
  ...
  do_get_caching_extent(
      ...
      const Transaction::src_t* p_src)
  {
    return do_get_caching_extent<T>(
        offset, length, 0, length, std::forward<Func>(extent_init_func),
        std::forward<OnCache>(on_cache), p_src);
  }
  ...
  class extent_init_func_t {
    ...
    struct callable_wrapper final : callable_i {
      ...
      void
      operator()(CachedExtent& extent) final
      {
        return func(extent);
      }
  ...
  public:
    ...
    void
    operator()(CachedExtent& extent)
    {
      return (*wrapped)(extent);
    }
  };

  get_extent_ertr::future<CachedExtentRef> do_get_caching_extent_by_type(
      extent_types_t type,
      paddr_t offset,
      laddr_t laddr,
      extent_len_t length,
  ...
  using get_extent_by_type_ret =
      get_extent_by_type_iertr::future<CachedExtentRef>;
      ...
      extent_init_func_t&& extent_init_func)
  {
    ...
    if (status == Transaction::get_extent_ret::RETIRED) {
      SUBERRORT(
          seastore_cache, "{} {}~0x{:x} {} is retired on t -- {}", t, type,
          offset, length, laddr, *ret);
      ceph_abort_msg("impossible");
    } else if (status == Transaction::get_extent_ret::PRESENT) {
      assert(ret->get_length() == length);
      if (ret->is_fully_loaded()) {
        SUBTRACET(
            seastore_cache, "{} {}~0x{:x} {} is present on t -- {}", t, type,
            offset, length, laddr, *ret);
        return ret->wait_io().then([ret] {
      ...
      SUBTRACET(
          seastore_cache, "{} {}~0x{:x} {} is absent on t, query cache ...", t,
          type, offset, length, laddr);
      auto f = [&t, this, t_src](CachedExtent& ext) {
        t.add_to_read_set(CachedExtentRef(&ext));
        touch_extent_fully(ext, &t_src, t.get_cache_hint());
      };
      return trans_intr::make_interruptible(do_get_caching_extent_by_type(
          type, offset, laddr, length, std::move(extent_init_func),
          std::move(f), &t_src));
    }
  }
  ...
  using backref_entry_query_mset_t =
      std::multiset<backref_entry_t, backref_entry_t::cmp_t>;

  backref_entry_query_mset_t
  get_backref_entries_in_range(paddr_t start, paddr_t end)
  {
    auto start_iter =
        backref_entry_mset.lower_bound(start, backref_entry_t::cmp_t());
    auto end_iter =
        backref_entry_mset.lower_bound(end, backref_entry_t::cmp_t());
  ...
  const backref_entry_mset_t&
  get_backref_entry_mset()
  {
    return backref_entry_mset;
  }

  backref_entryrefs_by_seq_t&
  get_backref_entryrefs_by_seq()
  {
    return backref_entryrefs_by_seq;
  }

  const segment_info_t*
  get_segment_info(segment_id_t sid)
  {
...
public:
  ...
  )
  {
    return _get_absent_extent_by_type(
        ...
        extent_init_func_t(std::forward<Func>(extent_init_func)), pin_crc);
  }
  ...
  )
  {
    ...
    SUBTRACET(
        seastore_cache, "allocate {} 0x{:x}B, hint={}, gen={}", t, T::TYPE,
        length, hint, rewrite_gen_printer_t{gen});
#ifdef UNIT_TESTS_BUILT
    auto result =
        epm.alloc_new_non_data_extent(t, T::TYPE, length, hint, gen, epaddr);
#else
    ...
    assert(
        is_rewrite_generation(result->gen, epm.dynamic_max_rewrite_generation));
    ret->init(
        CachedExtent::extent_state_t::INITIAL_WRITE_PENDING, result->paddr,
        hint, result->gen, t.get_trans_id());
    t.add_fresh_extent(ret);
    SUBDEBUGT(
        seastore_cache,
        "allocated {} 0x{:x}B extent at {}, hint={}, gen={} -- {}", t, T::TYPE,
        length, result->paddr, hint, rewrite_gen_printer_t{result->gen}, *ret);
    return ret;
  }
  /**
  ...
  )
  {
...
#ifdef UNIT_TESTS_BUILT
    auto results =
        epm.alloc_new_data_extents(t, T::TYPE, length, hint, gen, epaddr);
#else
    ...
    for (auto& result : results) {
      ...
      assert(is_rewrite_generation(
          result.gen, epm.dynamic_max_rewrite_generation));
      ret->init(
          CachedExtent::extent_state_t::INITIAL_WRITE_PENDING, result.paddr,
          hint, result.gen, t.get_trans_id());
      t.add_fresh_extent(ret);
      SUBDEBUGT(
          seastore_cache,
          "allocated {} 0x{:x}B extent at {}, hint={}, gen={} -- {}", t,
          T::TYPE, length, result.paddr, hint,
          rewrite_gen_printer_t{result.gen}, *ret);
      ...
      std::optional<ceph::bufferptr>& original_bptr)
  {
    ...
    if (original_bptr.has_value()) {
      ...
      auto remap_offset =
          remap_laddr.get_byte_distance<extent_len_t>(original_laddr);
    ...
    ext->init(
        CachedExtent::extent_state_t::EXIST_CLEAN, remap_paddr,
    ...
    SUBTRACET(
        seastore_cache, "allocated {} 0x{:x}B, hint={}, has ptr? {} -- {}", t,
        T::TYPE, remap_length, remap_laddr, original_bptr.has_value(), *extent);
    return extent;
  }
   ...
   */
  void
  set_segment_providers(std::vector<SegmentProvider*>&& providers)
  {
  ...
  using replay_delta_ret =
      replay_delta_ertr::future<std::pair<bool, CachedExtentRef>>;
  ...
  template <typename F>
  init_cached_extents_ret
  init_cached_extents(Transaction& t, F&& f)
  {
    LOG_PREFIX(Cache::init_cached_extents);
    SUBINFOT(
        seastore_cache,
        "start with {}(0x{:x}B) extents, {} dirty, dirty_from={}, "
        "alloc_from={}",
        t, extents_index.size(), extents_index.get_bytes(), dirty.size(),
    ...
    return seastar::do_with(
               std::forward<F>(f), std::move(_dirty),
               [this, FNAME, &t](auto& f, auto& refs) mutable {
                 return trans_intr::do_for_each(
                     refs, [this, FNAME, &t, &f](auto& e) {
                       SUBTRACET(
                           seastore_cache, "inspecting extent ... -- {}", t, *e);
                       return f(t, e).si_then([this, FNAME, &t,
                                               e](bool is_alive) {
                         if (!is_alive) {
                           SUBDEBUGT(
                               seastore_cache,
                               "extent is not alive, remove extent -- {}", t,
                               *e);
        ...
        .si_then([this, FNAME, &t] {
          SUBINFOT(
              seastore_cache,
              "finish with {}(0x{:x}B) extents, {} dirty, dirty_from={}, "
              "alloc_from={}",
              t, extents_index.size(), extents_index.get_bytes(), dirty.size(),
              get_oldest_dirty_from().value_or(JOURNAL_SEQ_NULL),
              get_oldest_backref_dirty_from().value_or(JOURNAL_SEQ_NULL));
   ...
   */
  CachedExtentRef
  update_extent_from_transaction(Transaction& t, CachedExtentRef extent)
  {
  ...
  using get_next_dirty_extents_ret =
      get_next_dirty_extents_iertr::future<std::vector<CachedExtentRef>>;
  ...
  get_oldest_backref_dirty_from() const
  {
  ...
  struct backref_extent_entry_t {
    ...
    struct cmp_t {
      ...
      bool
      operator()(const paddr_t& l, const backref_extent_entry_t& r) const
      {
        return l < r.paddr;
      }

      bool
      operator()(const backref_extent_entry_t& l, const paddr_t& r) const
      {
        return l.paddr < r;
      }
    };
  ...
  void
  update_tree_extents_num(extent_types_t type, int64_t delta)
  {
    switch (type) {
  ...
  template <typename T>
  get_extent_iertr::future<>
  read_extents_maybe_partial(Transaction& t, std::vector<read_extent_t<T>>&& exts)
  {
    ...
    for (auto& ext : extents) {
      ...
      SUBDEBUGT(
          seastore_cache, "reading extent {} 0x{:x}~0x{:x} ...", t, *extent,
      ...
      if (extent->is_range_loaded(ext.offset, ext.length)) {
        ...
        read_extent_futs.emplace_back(
            trans_intr::make_interruptible(extent->wait_io())
    ...
    co_await trans_intr::parallel_for_each(read_extent_futs, [](auto& fut) {
      return std::move(fut);
    });
  }
...
private:
  ...
  using backref_extent_entry_query_set_t =
      std::set<backref_extent_entry_t, backref_extent_entry_t::cmp_t>;
  backref_extent_entry_query_set_t backref_extents;

  void
  add_backref_extent(paddr_t paddr, paddr_t key, extent_types_t type)
  {
    assert(paddr.is_absolute());
  ...
  void
  remove_backref_extent(paddr_t paddr)
  {
  ...
  backref_extent_entry_query_set_t
  get_backref_extents_in_range(paddr_t start, paddr_t end)
  {
  ...
  struct commit_trans_efforts_t {
    ...
    counter_by_extent_t<io_stat_t>
        fresh_invalid_by_ext; // inline but is already invalid (retired)
    ...
    uint64_t inline_record_metadata_bytes =
  ...
  struct tree_efforts_t {
    ...
    void
    increment(const Transaction::tree_stats_t& incremental)
    {
  ...
  static constexpr std::size_t NUM_SRC_COMB = TRANSACTION_TYPE_MAX *
                                              (TRANSACTION_TYPE_MAX + 1) / 2;
  ...
  struct {
    ...
    counter_by_src_t<counter_by_extent_t<dirty_io_stats_t>> dirty_io_by_src_ext;

    cache_access_stats_t access;
    counter_by_src_t<uint64_t> cache_absent_by_src;
    counter_by_src_t<counter_by_extent_t<cache_access_stats_t>> access_by_src_ext;
  ...
  void
  account_conflict(Transaction::src_t src1, Transaction::src_t src2)
  {
    ...
    assert(!(
        src1 == Transaction::src_t::READ && src2 == Transaction::src_t::READ));
    assert(
  ...
  void
  apply_backref_mset(backref_entry_refs_t& backref_entries)
  {
  ...
  void add_to_dirty(CachedExtentRef ref, const Transaction::src_t* p_src);
  ...
  void remove_from_dirty(CachedExtentRef ref, const Transaction::src_t* p_src);

  void clear_dirty();

  /// Remove extent from extents handling dirty and refcounting
  void remove_extent(CachedExtentRef ref, const Transaction::src_t* p_src);

  /// Retire extent
  void commit_retire_extent(Transaction& t, CachedExtentRef ref);

  /// Replace prev with next
  void commit_replace_extent(
      Transaction& t,
      CachedExtentRef next,
      CachedExtentRef prev);
  ...
  static void check_full_extent_integrity(uint32_t ref_crc, uint32_t pin_crc);
      ...
      uint32_t pin_crc)
  {
    ...
    return seastar::do_with(
               to_read.ranges,
               [extent, this, FNAME](auto& read_ranges) {
                 return ExtentPlacementManager::read_ertr::parallel_for_each(
                     read_ranges, [extent, this, FNAME](auto& read_range) {
                       SUBDEBUG(
                           seastore_cache,
                           "reading extent {} 0x{:x}~0x{:x} ...",
                           extent->get_paddr(), read_range.offset,
                           read_range.get_length());
                       assert(is_aligned(read_range.offset, get_block_size()));
                       assert(is_aligned(
                           read_range.get_length(), get_block_size()));
                       return epm.read(
                           extent->get_paddr() + read_range.offset,
                           read_range.get_length(), read_range.ptr);
                     });
               })
        .safe_then(
            [this, FNAME, extent = std::move(extent), offset, length, pin_crc,
             extent_fully_loaded]() mutable {
              ceph_assert(
                  extent->state == CachedExtent::extent_state_t::EXIST_CLEAN ||
                  extent->state == CachedExtent::extent_state_t::CLEAN ||
                  !extent->is_valid());
              if (extent->is_valid()) {
                if (extent_fully_loaded) {
                  ...
                  SUBDEBUG(
                      seastore_cache, "read extent 0x{:x}~0x{:x} done -- {}",
                      offset, length, *extent);

                  if (pin_crc != CRC_NULL) {
                    SUBDEBUG(
                        seastore_cache,
                        "read extent 0x{:x}~0x{:x} veryfing integrity -- {}",
                        offset, length, *extent);
                    ...
                    assert(extent->is_fully_loaded());
                    check_full_extent_integrity(
                        extent->last_committed_crc, pin_crc);
                  }
                } else {
                  extent->last_committed_crc = CRC_NULL;
                  SUBDEBUG(
                      seastore_cache,
                      "read extent 0x{:x}~0x{:x} done (partial) -- {}", offset,
                      length, *extent);
                }
              } else {
                SUBDEBUG(
                    seastore_cache,
                    "read extent 0x{:x}~0x{:x} done (invalidated) -- {}",
                    offset, length, *extent);
              }
              extent->complete_io();
            ...
            crimson::ct_error::assert_all{"Cache::read_extent: invalid error"});
  }
      ...
      uint32_t pin_crc = CRC_NULL)
  {
    return _read_extent(std::move(extent), offset, length, p_src, pin_crc)
        .safe_then([](auto extent) { return extent->template cast<T>(); });
  }

  template <typename T>
  get_extent_ertr::future<>
  read_absent_extents_maybe_partial(
      Transaction& t,
      std::vector<read_extent_t<T>>&& exts)
  {
    ...
    for (auto& ext : extents) {
      ...
      assert(
          extent->state == CachedExtent::extent_state_t::EXIST_CLEAN ||
          extent->state == CachedExtent::extent_state_t::CLEAN);
      if (extents_read.empty() || extents_read.back().extent != extent) {
    ...
    for (auto& ext : extents_read) {
      ...
      if (ext.fully_loaded) {
        if (extent->is_fully_loaded()) {
          ...
          extent->last_committed_crc = CRC_NULL;
          SUBDEBUGT(
              seastore_cache, "read extent done (partial) -- {}", t, *extent);
        }
      } else {
        SUBDEBUGT(
            seastore_cache, "read extent done (invalidated) -- {}", t, *extent);
      }
      if (extent->is_pending_io()) {
        extent->complete_io();
      }
