...
inline ceph::bufferptr
create_extent_ptr_rand(extent_len_t len)
{
...
inline ceph::bufferptr
create_extent_ptr_zero(extent_len_t len)
{
...
class read_set_item_t {
  ...
  using trans_hook_options = boost::intrusive::
      member_hook<read_set_item_t, set_hook_t, &read_set_item_t::trans_hook>;
  using extent_hook_options = boost::intrusive::
      member_hook<read_set_item_t, set_hook_t, &read_set_item_t::extent_hook>;

public:
  struct extent_cmp_t {
    using is_transparent = paddr_t;
    bool operator()(
        const read_set_item_t<T>& lhs,
        const read_set_item_t& rhs) const;
  ...
  bool
  is_extent_attached_to_trans() const
  {
    return extent_hook.is_linked();
  }

  bool
  is_trans_attached_to_extent() const
  {
...
struct trans_spec_view_t {
  ...
  trans_spec_view_t(transaction_id_t id) :
  ...
  struct cmp_t {
    bool
    operator()(const trans_spec_view_t& lhs, const trans_spec_view_t& rhs) const
    {
      return lhs.pending_for_transaction < rhs.pending_for_transaction;
    }

    bool
    operator()(const transaction_id_t& lhs, const trans_spec_view_t& rhs) const
    {
      return lhs < rhs.pending_for_transaction;
    }

    bool
    operator()(const trans_spec_view_t& lhs, const transaction_id_t& rhs) const
    {
      return lhs.pending_for_transaction < rhs;
    }
  };

  using trans_view_hook_t = boost::intrusive::set_member_hook<
      boost::intrusive::link_mode<boost::intrusive::auto_unlink>>;
...
struct load_ranges_t {
  ...
  void
  push_back(extent_len_t offset, ceph::bufferptr ptr)
  {
    assert(ranges.empty() || (ranges.back().get_end() < offset));
    assert(ptr.length());
    length += ptr.length();
...
private:
      ...
      const map_t::const_iterator& next_it)
  {
    ...
    auto it = buffer_map.insert(next_it, std::pair{hole_offset, std::move(bl)});
...
class ExtentCommitter
  : public boost::
        intrusive_ref_counter<ExtentCommitter, boost::thread_unsafe_counter> {
public:
...
class CachedExtent
  : public boost::intrusive_ref_counter<CachedExtent, boost::thread_unsafe_counter>,
...
public:
  ...
  void
  set_modify_time(sea_time_point t)
  {
    modify_time = t;
  }
   ...
   */
  virtual void
  on_delta_write(paddr_t record_block_offset)
  {}
  ...
  void
  rewrite(Transaction& t, CachedExtent& e, extent_len_t o)
  {
    ...
    e.get_bptr().copy_out(o, get_length(), get_bptr().c_str());
  ...
  virtual std::ostream&
  print_detail(std::ostream& out) const
  {
    return out;
  }
  ...
  print(std::ostream& out) const
  {
    std::string prior_poffset_str = prior_poffset
                                        ...
                                        : "nullopt";
    out << "CachedExtent(addr=" << this << ", type=" << get_type()
        << ", trans=" << pending_for_transaction << ", version=" << version
        << ", dirty_from=" << dirty_from
        << ", modify_time=" << sea_time_point_printer_t{modify_time}
        << ", paddr=" << get_paddr() << ", prior_paddr=" << prior_poffset_str
        << std::hex << ", length=0x" << get_length() << ", loaded=0x"
        << get_loaded_length() << std::dec << ", state=" << state
        << ", last_committed_crc=" << last_committed_crc
        << ", refcount=" << use_count() << ", user_hint=" << user_hint
  ...
  virtual complete_load_ertr::future<>
  complete_load()
  {
  ...
  bool
  is_exist_mutation_pending() const
  {
  ...
  is_stable() const
  {
    return state == extent_state_t::CLEAN || state == extent_state_t::DIRTY;
  }

  /// Returns true iff extent is stable and not io-pending
  bool
  is_stable_ready() const
  {
  ...
  bool
  is_range_loaded(extent_len_t offset, extent_len_t _length)
  {
  ...
  ceph::bufferlist
  get_range(extent_len_t offset, extent_len_t _length)
  {
  ...
  {
    return ceph_crc32c(
        1, reinterpret_cast<const unsigned char*>(get_bptr().c_str()),
        get_length());
  }
  ...
  /// Compare by paddr
  friend bool
  operator<(const CachedExtent& a, const CachedExtent& b)
  {
  ...
  void
  set_target_rewrite_generation(rewrite_gen_t gen)
  {
  ...
  void
  set_inplace_rewrite_generation()
  {
    user_hint = placement_hint_t::REWRITE;
  ...
  CachedExtentRef
  get_prior_instance() const
  {
    return prior_instance;
  }

  uint32_t
  get_last_committed_crc() const
  {
  ...
  bool
  is_pending_in_trans(transaction_id_t id) const
  {
   ...
   */
  std::pair<bool, viewable_state_t> is_viewable_by_trans(Transaction& t);
  ...
  get_2q_state() const
  {
    assert(
        "2Q" ==
        crimson::common::get_conf<std::string>("seastore_cachepin_type"));
    return cache_state;
  }

  void
  set_2q_state(extent_2q_state_t state)
  {
    assert(
        "2Q" ==
        crimson::common::get_conf<std::string>("seastore_cachepin_type"));
  ...
  extent_len_t
  get_last_touch_end() const
  {
    return last_touch_end;
  }

  void
  set_last_touch_end(extent_len_t touch_end)
  {
    assert(touch_end != 0);
...
private:
  ...
  using primary_ref_list =
      boost::intrusive::list<CachedExtent, primary_ref_list_member_options>;
  ...
  void
  set_io_wait(extent_state_t new_state, bool rewriting)
  {
    ceph_assert(!io_wait);
...
protected:
  ...
  explicit CachedExtent(ceph::bufferptr&& _ptr) :
    length(_ptr.length()), loaded_length(_ptr.length())
  {
    ptr = std::move(_ptr);
  ...
  explicit CachedExtent(extent_len_t _length) :
    length(_length), loaded_length(0), buffer_space(std::in_place)
  {
  ...
  CachedExtent(root_construct_t) :
    ptr(ceph::bufferptr(0)), length(0), loaded_length(0)
  {
    assert(is_fully_loaded());
    // must call init() to fully initialize
  }
  ...
  static TCachedExtentRef<T>
  make_cached_extent_ref(Args&&... args)
  {
   ...
   */
  virtual void
  update_in_extent_chksum_field(uint32_t)
  {}

  void
  set_prior_instance(CachedExtentRef p)
  {
  ...
  void
  set_last_committed_crc(uint32_t crc)
  {
    last_committed_crc = crc;
  }

  void
  set_paddr(paddr_t offset, bool need_update_mapping = false)
  {
  ...
  load_ranges_t
  load_ranges(extent_len_t offset, extent_len_t _length)
  {
    assert(is_aligned(offset, CEPH_PAGE_SIZE));
    assert(is_aligned(_length, CEPH_PAGE_SIZE));
    assert(_length > 0);
...
struct paddr_cmp {
  bool
  operator()(paddr_t lhs, const CachedExtent& rhs) const
  {
    return lhs < rhs.poffset;
  }

  bool
  operator()(const CachedExtent& lhs, paddr_t rhs) const
  {
...
struct ref_paddr_cmp {
  ...
  bool
  operator()(const paddr_t& lhs, const trans_retired_extent_link_t& rhs) const
  {
    return lhs < rhs.extent->poffset;
  }

  bool
  operator()(const trans_retired_extent_link_t& lhs, const paddr_t& rhs) const
  {
...
class addr_extent_list_base_t : public std::list<std::pair<T, C>> {};

using pextent_list_t = addr_extent_list_base_t<paddr_t, CachedExtentRef>;

template <typename T, typename C, typename Cmp>
class addr_extent_set_base_t : public std::set<C, Cmp> {};

using retired_extent_set_t =
    addr_extent_set_base_t<paddr_t, trans_retired_extent_link_t, ref_paddr_cmp>;
...
public:
  auto
  get_overlap(paddr_t addr, extent_len_t len)
  {
  ...
  void
  erase(CachedExtent::index::iterator& it)
  {
    auto& extent = *it;
  ...
  void
  replace(CachedExtent& to, CachedExtent& from)
  {
  ...
  auto
  find_offset(paddr_t offset)
  {
    return extent_index.find(offset, paddr_cmp());
  }

  bool
  exists(CachedExtent& extent) const
  {
...
public:
  ...
  LogicalCachedExtent(const LogicalCachedExtent& other) :
    CachedExtent(other), seen_by_users(other.seen_by_users)
  {}

  LogicalCachedExtent(const LogicalCachedExtent& other, share_buffer_t s) :
    CachedExtent(other, s), seen_by_users(other.seen_by_users)
  {}

  void
  on_rewrite(Transaction& t, CachedExtent& extent, extent_len_t off) final
  {
  ...
  virtual void
  do_on_rewrite(Transaction& t, LogicalCachedExtent& extent)
  {}

  bool
  is_seen_by_users() const
  {
  ...
  void
  set_laddr(laddr_t nladdr)
  {
    laddr = nladdr;
  }

  void
  apply_delta_and_adjust_crc(paddr_t base, const ceph::bufferlist& bl) final
  {
    apply_delta(bl);
  ...
  virtual std::optional<modified_region_t>
  get_modified_region()
  {
...
protected:
  ...
  virtual std::ostream&
  print_detail_l(std::ostream& out) const
  {
    return out;
  }
  ...
  void
  on_delta_write(paddr_t record_block_offset) final
  {
    logical_on_delta_write();
  }

  void
  on_state_commit() final
  {
...
struct ref_laddr_cmp {
  ...
  bool
  operator()(
      const LogicalCachedExtentRef& lhs,
  ...
  bool
  operator()(const laddr_t& lhs, const LogicalCachedExtentRef& rhs) const
  {
    return lhs < rhs->get_laddr();
  }

  bool
  operator()(const LogicalCachedExtentRef& lhs, const laddr_t& rhs) const
  {
...
template <typename T>
inline bool
read_set_item_t<T>::extent_cmp_t::operator()(
    const read_set_item_t<T>& lhs,
    const read_set_item_t<T>& rhs) const
{
...
using lextent_set_t =
    addr_extent_set_base_t<laddr_t, LogicalCachedExtentRef, ref_laddr_cmp>;

template <typename T>
using lextent_list_t = addr_extent_list_base_t<laddr_t, TCachedExtentRef<T>>;

} // namespace crimson::os::seastore

#if FMT_VERSION >= 90000
template <>
struct fmt::formatter<crimson::os::seastore::CachedExtent>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::os::seastore::CachedExtent::viewable_state_t>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::os::seastore::LogicalCachedExtent>
  : fmt::ostream_formatter {};
#endif
...
struct fmt::formatter<crimson::os::seastore::extent_2q_state_t>
  ...
  auto
  format(const State& s, auto& ctx) const
  {
    switch (s) {
    case State::Fresh:
