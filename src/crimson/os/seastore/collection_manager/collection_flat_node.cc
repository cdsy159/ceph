...
void
delta_t::replay(coll_map_t& l) const
{
  switch (op) {
  case op_t::INSERT: {
    l.insert(coll, bits);
...
std::ostream&
CollectionNode::print_detail_l(std::ostream& out) const
{
  return out;
}
...
CollectionNode::create(coll_context_t cc, coll_t coll, unsigned bits)
{
  ...
  if (encoded_sizeof((base_coll_map_t&)decoded) > get_bptr().length()) {
    decoded.erase(iter);
    return create_ret(
        interruptible::ready_future_marker{}, create_result_t::OVERFLOW);
  } else {
...
{
  logger().debug(
      "trans.{} CollectionNode:{} {} {}", cc.t.get_trans_id(), __func__, coll,
      bits);
  if (!is_mutable()) {
    auto mut = cc.tm.get_mutable_extent(cc.t, this)->cast<CollectionNode>();
