...
FlatCollectionManager::mkfs(Transaction& t)
{
  ...
  return tm
      .alloc_non_data_extent<CollectionNode>(t, L_ADDR_MIN, MIN_FLAT_BLOCK_SIZE)
      .si_then([](auto&& root_extent) {
        coll_root_t coll_root =
            coll_root_t(root_extent->get_laddr(), MIN_FLAT_BLOCK_SIZE);
...
FlatCollectionManager::get_coll_root(const coll_root_t& coll_root, Transaction& t)
{
  ...
  return cc.tm
      .read_extent<CollectionNode>(
          cc.t, coll_root.get_location(), coll_root.get_size())
        ...
        return extent->create(get_coll_context(t), cid, info.split_bits)
            .si_then([=, this, &coll_root, &t](auto ret) {
              ...
              case CollectionNode::create_result_t::OVERFLOW: {
                ...
                return tm
                    .alloc_non_data_extent<CollectionNode>(
                        t, L_ADDR_MIN, new_size)
                    .si_then([=, this, &coll_root, &t](auto&& root_extent) {
                      coll_root.update(
                          root_extent->get_laddr(), root_extent->get_length());

                      root_extent->decoded = extent->decoded;
                      return root_extent
                          ->create(get_coll_context(t), cid, info.split_bits)
...
{
  logger().debug("FlatCollectionManager: {}", __func__);
  return get_coll_root(coll_root, t).si_then([](auto extent) {
    return extent->list();
  });
}

FlatCollectionManager::update_ret
FlatCollectionManager::update(
