...
std::ostream&
operator<<(std::ostream& out, const device_spec_t& ds)
{
  return out << "device_spec(" << "magic=0x" << std::hex << ds.magic << std::dec
             << ", dtype=" << ds.dtype << ", " << device_id_printer_t{ds.id}
             << ")";
}

std::ostream&
operator<<(std::ostream& out, const device_config_t& conf)
{
  out << "device_config_t(" << "major_dev=" << conf.major_dev
      << ", spec=" << conf.spec << ", meta=" << conf.meta << ", secondary(";
...
check_create_device(const std::string path, size_t size)
{
  LOG_PREFIX(block_check_create_device);
  INFO("path={}, size=0x{:x}", path, size);
  return seastar::open_file_dma(
             path, seastar::open_flags::exclusive | seastar::open_flags::rw |
                       seastar::open_flags::create)
      .then([size, FNAME, path](auto file) {
        return seastar::do_with(
            file, [size, FNAME, path](auto& f) -> seastar::future<> {
              DEBUG("path={} created, truncating to 0x{:x}", path, size);
              ceph_assert(f);
              return f.truncate(size)
                  .then([&f, size] { return f.allocate(0, size); })
