...
struct device_config_t {
      ...
      secondary_device_set_t sds)
  {
    return device_config_t{
        true, device_spec_t{(magic_t)std::rand(), d_type, id},
        seastore_meta_t{new_osd_fsid}, sds};
  }
  ...
  {
    return device_config_t{
        false, device_spec_t{magic, d_type, id}, seastore_meta_t{new_osd_fsid},
...
public:
  ...
  virtual Device&
  get_sharded_device(store_index_t store_index = 0)
  {
    return *this;
  }
...
public:
  ...
  virtual bool
  is_end_to_end_data_protection() const
  {
    return false;
  }
  ...
  virtual read_ertr::future<> readv(
      paddr_t addr,
      std::vector<bufferptr> vecs) = 0;

  read_ertr::future<ceph::bufferptr>
  read(paddr_t addr, size_t len)
  {
    auto ptrref =
        std::make_unique<ceph::bufferptr>(buffer::create_page_aligned(len));
  ...
  virtual read_ertr::future<uint32_t>
  get_shard_nums()
  {
...
check_create_device_ret check_create_device(const std::string path, size_t size);
} // namespace crimson::os::seastore
...
template <>
struct fmt::formatter<crimson::os::seastore::device_config_t>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::os::seastore::device_spec_t>
  : fmt::ostream_formatter {};
#endif
