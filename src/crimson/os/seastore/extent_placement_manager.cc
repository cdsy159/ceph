...
SegmentedOolWriter::SegmentedOolWriter(
  ...
  record_submitter(
      crimson::common::get_conf<uint64_t>("seastore_journal_iodepth_limit"),
    ...
    bool with_atomic_roll_segment)
{
  ...
  auto ret =
      record_submitter.submit(std::move(record), with_atomic_roll_segment);
  DEBUGT(
      "{} start at {} with {} extents ...", t, segment_allocator.get_name(),
      ret.record_base_regardless_md, extents.size());
  paddr_t extent_addr = ret.record_base_regardless_md.offset;
  for (auto& extent : extents) {
    TRACET(
        "{} extent will be written at {} -- {}", t,
        segment_allocator.get_name(), extent_addr, *extent);
  ...
  return std::move(ret.future)
      .safe_then([this, FNAME, &t, record_base = ret.record_base_regardless_md](
                     record_locator_t ret) {
        TRACET(
            "{} finish {}=={}", t, segment_allocator.get_name(), ret,
            record_base);
...
SegmentedOolWriter::do_write(Transaction& t, std::list<CachedExtentRef>& extents)
{
  LOG_PREFIX(SegmentedOolWriter::do_write);
  assert(!extents.empty());
  if (!record_submitter.is_available()) {
    DEBUGT(
        "{} extents={} wait ...", t, segment_allocator.get_name(),
        extents.size());
    return trans_intr::make_interruptible(record_submitter.wait_available())
  ...
  for (auto it = extents.begin(); it != extents.end();) {
    ...
    if (action == action_t::ROLL) {
      ...
      DEBUGT(
          "{} extents={} submit {} extents and roll, unavailable ...", t,
          segment_allocator.get_name(), extents.size(), num_extents);
      ...
      return trans_intr::make_interruptible(
                 record_submitter.roll_segment().safe_then(
                     [fut_write = std::move(fut_write)]() mutable {
    ...
    TRACET(
        "{} extents={} add extent to record -- {}", t,
        segment_allocator.get_name(), extents.size(), *extent);
    ceph::bufferlist bl;
    ...
    record.push_back(
        extent_t{extent->get_type(), extent->get_laddr(), std::move(bl)},
        modify_time);
    ...
    if (action == action_t::SUBMIT_FULL) {
      DEBUGT(
          "{} extents={} submit {} extents ...", t,
          segment_allocator.get_name(), extents.size(), pending_extents.size());
      return trans_intr::make_interruptible(
  ...
  auto num_extents = pending_extents.size();
  DEBUGT(
      "{} submit the rest {} extents ...", t, segment_allocator.get_name(),
      num_extents);
    ...
    AsyncCleanerRef&& cold_cleaner)
{
  ...
  if (cold_segment_cleaner) {
    dynamic_max_rewrite_generation = hot_tier_generations +
                                     cold_tier_generations - 1;
  }
  DEBUG(
      "dynamic_max_rewrite_generation: {}, "
  ...
  if (trimmer->get_backend_type() == backend_type_t::SEGMENTED) {
    ...
    for (rewrite_gen_t gen = OOL_GENERATION; gen < hot_tier_generations; ++gen) {
      writer_refs.emplace_back(std::make_unique<SegmentedOolWriter>(
          store_index, data_category_t::DATA, gen, *segment_cleaner,
    ...
    for (rewrite_gen_t gen = OOL_GENERATION; gen < hot_tier_generations; ++gen) {
      writer_refs.emplace_back(std::make_unique<SegmentedOolWriter>(
          store_index, data_category_t::METADATA, gen, *segment_cleaner,
          *ool_segment_seq_allocator));
      md_writers_by_gen[generation_to_writer(gen)] = writer_refs.back().get();
    }

    for (auto* device :
         segment_cleaner->get_segment_manager_group()->get_segment_managers()) {
      add_device(device);
    }
  } else {
    ...
    data_writers_by_gen[generation_to_writer(OOL_GENERATION)] =
        writer_refs.back().get();
    md_writers_by_gen[generation_to_writer(OOL_GENERATION)] =
        writer_refs.back().get();
  ...
  if (cold_segment_cleaner) {
    ...
    for (rewrite_gen_t gen = hot_tier_generations;
         gen <= dynamic_max_rewrite_generation; ++gen) {
      writer_refs.emplace_back(std::make_unique<SegmentedOolWriter>(
          store_index, data_category_t::DATA, gen, *cold_segment_cleaner,
          *ool_segment_seq_allocator));
      data_writers_by_gen[generation_to_writer(gen)] = writer_refs.back().get();
    }
    for (rewrite_gen_t gen = hot_tier_generations;
      ...
      writer_refs.emplace_back(std::make_unique<SegmentedOolWriter>(
          store_index, data_category_t::METADATA, gen, *cold_segment_cleaner,
          *ool_segment_seq_allocator));
      md_writers_by_gen[generation_to_writer(gen)] = writer_refs.back().get();
    }
  ...
  background_process.init(
      std::move(trimmer), std::move(cleaner), std::move(cold_cleaner),
...
void
ExtentPlacementManager::set_primary_device(Device* device)
{
    ...
    double seconds) const
{
  ...
  if (get_main_backend_type() == backend_type_t::SEGMENTED) {
    ...
    // 2. mainmdat
    main_writer_stats.emplace_back();
    for (rewrite_gen_t gen = MIN_REWRITE_GENERATION; gen < hot_tier_generations;
         ++gen) {
  ...
  if (report_detail && seconds != 0) {
    ...
    auto report_writer_stats =
        [seconds, &oss](const char* name, const writer_stats_t& stats) {
    ...
    auto report_by_src = [seconds, has_cold_tier, &oss, &journal_stats,
                          &main_writer_stats,
      ...
      oss << "\n" << src << ": " << tw_stats_printer_t{seconds, t_stats};

      auto report_tw_stats =
          [seconds, src, &oss](const char* name, const writer_stats_t& stats) {
  ...
  return {
      ...
      main_stats.io_depth_stats.num_io_grouped, main_stats.get_total_bytes()};
}
...
ExtentPlacementManager::open_for_write()
{
  ...
  auto total_writers_num = data_writers_by_gen.size() +
                           md_writers_by_gen.size();
  ...
  for (auto& extent : res.delayed_extents) {
    if (dispatch_delayed_extent(extent)) {
      ...
      auto writer_ptr = get_writer(
          extent->get_user_hint(), get_extent_category(extent->get_type()),
...
#ifndef NDEBUG
    std::for_each(extents.begin(), extents.end(), [](auto& extent) {
...
void
ExtentPlacementManager::BackgroundProcess::log_state(const char* caller) const
{
  ...
  if (has_cold_tier()) {
    DEBUG(
        "caller {}, cold_cleaner: {}", caller,
        AsyncCleaner::stat_printer_t{*cold_cleaner, true});
  }
}

ExtentPlacementManager::mount_ret
ExtentPlacementManager::BackgroundProcess::mount(store_index_t store_index)
{
...
void
ExtentPlacementManager::BackgroundProcess::start_background()
{
...
ExtentPlacementManager::BackgroundProcess::stop_background()
{
  ...
  return seastar::futurize_invoke([this, FNAME] {
      ...
      .then([this, FNAME] {
        INFO(
            "done, {}, {}", JournalTrimmerImpl::stat_printer_t{*trimmer, true},
            AsyncCleaner::stat_printer_t{*main_cleaner, true});
        if (has_cold_tier()) {
          INFO(
              "done, cold_cleaner: {}",
    ...
    io_usage_t usage)
{
  ...
  if (res.is_successful()) {
    ...
    DEBUG(
        "blocked: inline={}, main={}, cold={}, usage={}",
        res.reserve_inline_success, res.cleaner_result.reserve_main_success,
        res.cleaner_result.reserve_cold_success, usage);
    ...
    do {
      ...
      if (res.is_successful()) {
        ...
        stats.io_blocked_time +=
            std::chrono::duration_cast<std::chrono::milliseconds>(duration)
                .count();
      } else {
        DEBUG(
            "blocked again: inline={}, main={}, cold={}, usage={}",
            res.reserve_inline_success, res.cleaner_result.reserve_main_success,
            res.cleaner_result.reserve_cold_success, usage);
        abort_io_usage(usage, res);
        if (!res.reserve_inline_success) {
 ...
 */
bool
ExtentPlacementManager::BackgroundProcess::try_reserve_cold(std::size_t usage)
{
  if (has_cold_tier()) {
...
void
ExtentPlacementManager::BackgroundProcess::abort_cold_usage(
    std::size_t usage,
    bool success)
{
...
void
ExtentPlacementManager::BackgroundProcess::abort_cleaner_usage(
    const cleaner_usage_t& usage,
...
void
ExtentPlacementManager::BackgroundProcess::abort_io_usage(
    const io_usage_t& usage,
...
ExtentPlacementManager::BackgroundProcess::do_background_cycle()
{
  ...
  if (proceed_trim) {
    ...
    return trimmer->trim().finally([this, trim_usage, FNAME] {
      DEBUG("finished trimming");
      abort_cleaner_usage(trim_usage, {true, true});
    });
  } else {
    assert(!proceed_trim);
    bool should_clean_main_for_trim = should_trim &&
    ...
    bool should_clean_cold_for_trim = should_trim &&
                                      !trim_reserve_res.reserve_cold_success;
    bool should_clean_cold_for_main = should_clean_main && !proceed_clean_main;
    bool proceed_clean_cold = false;
    if (has_cold_tier() &&
        (cold_cleaner->should_clean_space() || should_clean_cold_for_trim ||
    ...
    return seastar::when_all(
               [this, FNAME, proceed_clean_main, should_clean_main_for_trim,
                main_cold_usage] {
                 ...
                 return main_cleaner->clean_space()
                     .handle_error(crimson::ct_error::assert_all{
                         "do_background_cycle encountered invalid error in "
                         "main clean_space"})
               ...
               [this, FNAME, proceed_clean_cold, should_clean_cold_for_trim,
                should_clean_cold_for_main] {
                 ...
                 return cold_cleaner->clean_space()
                     .handle_error(crimson::ct_error::assert_all{
                         "do_background_cycle encountered invalid error in "
                         "cold clean_space"})
                     .finally([FNAME] { DEBUG("finished clean cold"); });
               })
        .discard_result();
  }
}
...
void
ExtentPlacementManager::BackgroundProcess::register_metrics(
    store_index_t store_index)
{
  ...
  metrics.add_group(
      "background_process",
      {sm::make_counter(
           "io_count", stats.io_count, sm::description("the sum of IOs"),
           {sm::label_instance(
       ...
       sm::make_counter(
           ...
           {sm::label_instance(
               "shard_store_index", std::to_string(store_index))}),
       sm::make_counter(
           "io_retried_blocked_count_clean", stats.io_blocked_count_clean,
           sm::description("Retried IOs that are blocked by cleaning"),
           {sm::label_instance(
               "shard_store_index", std::to_string(store_index))}),
       sm::make_counter(
           "io_retried_blocked_count_trim", stats.io_blocked_count_trim,
           sm::description("Retried IOs that are blocked by trimming"),
           {sm::label_instance(
               "shard_store_index", std::to_string(store_index))}),
       sm::make_counter(
           "io_blocked_count_clean", stats.io_blocked_count_clean,
       ...
       sm::make_counter(
           ...
           {sm::label_instance(
               "shard_store_index", std::to_string(store_index))}),
       sm::make_counter(
           "io_blocked_time", stats.io_blocked_time,
    ...
    std::list<CachedExtentRef>& extents)
{
  ...
  return seastar::with_gate(write_guard, [this, &t, &extents] {
    ...
    return do_write(t, extents).finally([this, ptr = ptr] {
    ...
    std::list<CachedExtentRef>& extents)
{
  ...
  for (auto& ex : extents) {
    ...
    assert(rbm);
    TRACE("write extent {}, paddr {} ...", fmt::ptr(ex.get()), paddr);
    auto& stats = t.get_ool_write_stats();
    stats.extents.num += 1;
    ...
    if (can_inplace_rewrite(t, ex)) {
      ...
      extent_len_t len = p2roundup(r->offset + r->len, rbm->get_block_size()) -
                         offset;
    ...
    TRACE(
      ...
      maybe-merged current extent: {}~0x{:x}",
        paddr, ex->get_length(), writes.back().offset,
        writes.back().bp.length());
  }
  ...
  return trans_intr::make_interruptible(
      seastar::do_with(std::move(writes), [&t, this](auto& writes) {
        auto& stats = t.get_ool_write_stats();
        stats.num_records += writes.size();
        auto& trans_stats = get_by_src(w_stats.stats_by_src, t.get_src());
        trans_stats.num_records += writes.size();
        return alloc_write_ertr::parallel_for_each(writes, [](auto& info) {
