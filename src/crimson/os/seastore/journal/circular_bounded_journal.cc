...
{
  return record_submitter.open(store_index, true).safe_then([this](auto ret) {
    return open_for_mkfs_ret(
        open_for_mkfs_ertr::ready_future_marker{}, get_written_to());
  });
}
...
{
  return record_submitter.open(store_index, false).safe_then([this](auto ret) {
    return open_for_mount_ret(
        open_for_mount_ertr::ready_future_marker{}, get_written_to());
  });
}

CircularBoundedJournal::close_ertr::future<>
CircularBoundedJournal::close()
{
    ...
    on_submission_func_t&& on_submission)
{
  ...
  while (true) {
    if (!record_submitter.is_available()) {
      ...
      stats.submit_record_wait_latency_total += ceph::mono_clock::now() -
                                                wait_start;
      continue;
    }
    ...
    if (action == RecordSubmitter::action_t::ROLL) {
      ...
      stats.submit_record_roll_latency_total += ceph::mono_clock::now() -
                                                roll_start;
      continue;
    }
    break;
  ...
  if (is_trim_transaction(t_src)) {
    co_await update_journal_tail(
        trimmer.get_dirty_tail(), trimmer.get_alloc_tail());
  }
}

Journal::replay_ret
CircularBoundedJournal::replay_segment(
    ...
    scan_valid_records_cursor& cursor)
{
  ...
  return seastar::do_with(
      RecordScanner::found_record_handler_t(
          [this, &handler, FNAME](
              record_locator_t locator, const record_group_header_t& r_header,
              const bufferlist& mdbuf)
              -> RecordScanner::scan_valid_records_ertr::future<> {
            auto maybe_record_deltas_list =
                try_decode_deltas(r_header, mdbuf, locator.record_block_base);
            if (!maybe_record_deltas_list) {
              // This should be impossible, we did check the crc on the mdbuf
              ERROR(
                  "unable to decode deltas for record {} at {}", r_header,
                  locator.record_block_base);
              return crimson::ct_error::input_output_error::make();
            }
            assert(locator.write_result.start_seq != JOURNAL_SEQ_NULL);
            auto cursor_addr = convert_paddr_to_abs_addr(
                locator.write_result.start_seq.offset);
            DEBUG("{} at {}", r_header, cursor_addr);
            journal_seq_t start_seq = locator.write_result.start_seq;
            auto write_result =
                write_result_t{start_seq, r_header.mdlength + r_header.dlength};
            ...
            if (cursor_addr >= get_journal_end()) {
              ...
              ++expected_seq;
              paddr_t addr =
                  convert_abs_addr_to_paddr(cursor_addr, get_device_id());
            ...
            return seastar::do_with(
                ...
                [write_result, &handler, FNAME](auto& record_deltas_list) {
                  return crimson::do_for_each(
                      record_deltas_list, [write_result, &handler, FNAME](
                                              record_deltas_t& record_deltas) {
                        auto locator = record_locator_t{
                            record_deltas.record_block_base, write_result};
                        DEBUG(
                            "processing {} deltas at block_base {}",
                            record_deltas.deltas.size(), locator);
                        return crimson::do_for_each(
                            record_deltas.deltas, [locator, &handler](auto& p) {
                              auto& modify_time = p.first;
                              auto& delta = p.second;
                              return handler(locator, delta, modify_time)
                                  .discard_result();
                            });
                      });
                });
      ...
      [=, this, &cursor](auto& dhandler) {
        return scan_valid_records(
                   cursor, cjs.get_cbj_header().magic,
                   std::numeric_limits<size_t>::max(), dhandler)
            .handle_error(
...
Journal::replay_ret
CircularBoundedJournal::scan_valid_record_delta(
    cbj_delta_handler_t&& handler,
    journal_seq_t tail)
{
  ...
  return seastar::do_with(
      scan_valid_records_cursor(tail), std::move(handler), bool(false),
      [this](auto& cursor, auto& handler, auto& rolled) {
        return crimson::repeat(
            [this, &handler, &cursor,
             &rolled]() -> replay_ertr::future<seastar::stop_iteration> {
...
RecordScanner::read_ret
CircularBoundedJournal::read(paddr_t start, size_t len)
{
  ...
  return cjs.read(addr, bptr).safe_then([bptr = std::move(bptr)]() {
    return read_ret(
        RecordScanner::read_ertr::ready_future_marker{}, std::move(bptr));
  });
}

bool
CircularBoundedJournal::is_record_segment_seq_invalid(
    scan_valid_records_cursor& cursor,
    record_group_header_t& r_header)
{
  ...
  if (cursor.seq.offset ==
    ...
    if ((r_header.committed_to.offset >= cursor.last_committed.offset &&
         r_header.committed_to.segment_seq ==
             cursor.last_committed.segment_seq) &&
        r_header.committed_to.segment_seq == cursor.seq.segment_seq - 1) {
      return false;
    }
    return print_invalid(r_header);
...
Journal::replay_ret
CircularBoundedJournal::replay(delta_handler_t&& delta_handler)
{
  /*
  ...
  return cjs.read_header()
      ...
      .safe_then([this, FNAME,
                  delta_handler = std::move(delta_handler)](auto p) mutable {
        auto& [head, bl] = *p;
        cjs.set_cbj_header(head);
        DEBUG("header : {}", cjs.get_cbj_header());
        cjs.set_initialized(true);
        return seastar::do_with(
                   std::move(delta_handler), std::map<paddr_t, journal_seq_t>(),
                         ...
                         [&map](
                           ...
                           if (e.type == extent_types_t::ALLOC_INFO) {
                             ...
                             if (alloc_delta.op ==
                               ...
                               for (auto& alloc_blk :
                                    alloc_delta.alloc_blk_ranges) {
                                 map[alloc_blk.paddr] =
                                     offsets.write_result.start_seq;
                               }
                             }
                           }
                               ...
                               [this, &map, &d_handler, &crc_info](
                                 ...
                                 if (map.find(e.paddr) == map.end() ||
                                     map[e.paddr] <=
                                         offsets.write_result.start_seq) {
                                   return d_handler(
                                              offsets, e, get_dirty_tail(),
                                              get_alloc_tail(), modify_time)
                                 ...
                                 return replay_ertr::make_ready_future<bool>(
                                     true);
                               };
                           // The second pass to replay deltas
                           return scan_valid_record_delta(
                                      std::move(call_d_handler_if_valid), tail)
                               .safe_then([&crc_info]() {
                                 for (auto p : crc_info) {
                                   ceph_assert_always(
                                       p.second.first->get_last_committed_crc() ==
                                       p.second.second);
                                 }
                                 crc_info.clear();
...
void
CircularBoundedJournal::register_metrics()
{
  namespace sm = seastar::metrics;
  metrics.add_group(
      "seastore_cbj",
      {sm::make_gauge(
           "submit_record_count", [this] { return stats.submit_record_count; }),
       sm::make_gauge(
           "submit_record_size", [this] { return stats.submit_record_size; }),
       sm::make_gauge(
           "submit_record_latency_total",
           [this] { return stats.submit_record_latency_total.count(); }),
       sm::make_gauge(
       ...
       sm::make_gauge(
           ...
           [this] {
             return stats.submit_record_size / stats.submit_record_count;
           }),
       sm::make_gauge(
           "submit_record_roll_count",
           [this] { return stats.submit_record_roll_count; }),
       sm::make_gauge(
           "submit_record_roll_latency_total",
           [this] { return stats.submit_record_roll_latency_total.count(); }),
       sm::make_gauge(
       ...
       sm::make_gauge(
           ...
           [this] { return stats.submit_record_wait_count; }),
       sm::make_gauge(
           "submit_record_wait_latency_total",
           [this] { return stats.submit_record_wait_latency_total.count(); }),
       sm::make_gauge("submit_record_wait_latency_average", [this] {
