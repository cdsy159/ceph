...
std::ostream&
operator<<(std::ostream& out, const CircularJournalSpace::cbj_header_t& header)
{
  return out << "cbj_header_t(" << "dirty_tail=" << header.dirty_tail
...
bool
CircularJournalSpace::needs_roll(std::size_t length) const
{
...
CircularJournalSpace::roll_ertr::future<>
...
CircularJournalSpace::write(ceph::bufferlist&& to_write)
{
  ...
  DEBUG(
      "length {}, commit target {}, used_size {}", encoded_size, target,
...
segment_nonce_t
calc_new_nonce(uint32_t crc, unsigned char const* data, unsigned length)
{
  crc &= std::numeric_limits<uint32_t>::max() >> 1;
  return ceph_crc32c(crc, data, length);
}

CircularJournalSpace::open_ret
CircularJournalSpace::open(bool is_mkfs)
{
  ...
  if (is_mkfs) {
    ...
    head.dirty_tail = journal_seq_t{
        0,
        convert_abs_addr_to_paddr(get_records_start(), device->get_device_id())};
...
CircularJournalSpace::encode_header()
{
  ...
  if (!device->is_end_to_end_data_protection()) {
    ...
    auto header_crc =
        bliter.crc32c(ceph::encoded_sizeof_bounded<cbj_header_t>(), -1);
    ceph_le32 header_crc_le;
    header_crc_le = header_crc;
    header_crc_filler.copy_in(
        sizeof(checksum_t), reinterpret_cast<const char*>(&header_crc_le));
  }
  return bl;
}

CircularJournalSpace::submit_ertr::future<>
CircularJournalSpace::device_write_bl(rbm_abs_addr offset, bufferlist& bl)
{
  ...
  DEBUG(
      "overwrite in CircularJournalSpace, offset {}, length {}", offset, length);
...
CircularJournalSpace::read_header()
{
  ...
  assert(device);
  auto bptr =
      bufferptr(ceph::buffer::create_page_aligned(device->get_block_size()));
  DEBUG("reading {}", device->get_shard_journal_start());
  return device->read(device->get_shard_journal_start(), bptr)
      .safe_then([bptr, FNAME, this]() mutable -> read_header_ret {
        bufferlist bl;
        bl.append(bptr);
        auto bp = bl.cbegin();
        ...
        if (!device->is_end_to_end_data_protection()) {
          ...
          if (test_crc != recorded_crc) {
            ...
            return read_header_ret(
                read_header_ertr::ready_future_marker{}, std::nullopt);
          }
        }
        return read_header_ret(
