...
public:
   ...
   *
   */
  submit_ertr::future<> device_write_bl(
      rbm_abs_addr offset,
      ceph::bufferlist& bl);
  ...
  */
  ...
  size_t
  get_records_available_size() const
  {
  ...
  is_available_size(uint64_t size)
  {
    ...
    if (rbm_written_to > rbm_tail &&
        ...
        size >
            (get_records_used_size() - (get_journal_end() - rbm_written_to))) {
      return false;
    }
  ...
  read_ertr::future<>
  read(uint64_t offset, bufferptr& bptr)
  {
    assert(device);
  ...
  update_journal_tail(journal_seq_t dirty, journal_seq_t alloc)
  {
    ...
    return write_header().handle_error(crimson::ct_error::assert_all{
        "encountered invalid error in update_journal_tail"});
  }

  void
  set_initialized(bool init)
  {
    initialized = init;
  }

  void
  set_cbj_header(cbj_header_t& head)
  {
    header = head;
  }
