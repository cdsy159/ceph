  ...
  return sm_group.open(new_segment_id)
      ...
      .safe_then([this, is_mkfs, FNAME, new_segment_seq](auto sref) {
        ...
        if (type == segment_type_t::JOURNAL) {
          ...
          if (is_mkfs) {
            ...
            auto mkfs_seq = journal_seq_t{
                new_segment_seq, paddr_t::make_seg_paddr(segment_id, 0)};
            dirty_tail = mkfs_seq;
            alloc_tail = mkfs_seq;
          } else {
            ceph_assert(dirty_tail != JOURNAL_SEQ_NULL);
        ...
        auto new_journal_seq = journal_seq_t{
            new_segment_seq, paddr_t::make_seg_paddr(segment_id, written_to)};
...
SegmentAllocator::close_segment()
{
  ...
  ceph::bufferlist bl;
  encode(tail, bl);
  INFO("{} close segment {}, written_to=0x{:x}", print_name, tail, written_to);
