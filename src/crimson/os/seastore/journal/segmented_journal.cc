...
SegmentedJournal::close_ertr::future<>
    ...
    std::vector<std::pair<segment_id_t, segment_header_t>> segments)
{
  ...
  return scan_last_segment(last_segment_id, last_header)
      .safe_then([this, FNAME, segments = std::move(segments)] {
        INFO(
            "dirty_tail={}, alloc_tail={}", trimmer.get_dirty_tail(),
    ...
    const segment_header_t& segment_header)
{
  ...
  auto seq = journal_seq_t{
      segment_header.segment_seq, paddr_t::make_seg_paddr(segment_id, 0)};
  ...
  return seastar::do_with(
      ...
      SegmentManagerGroup::found_record_handler_t(
          [FNAME, this](
            ...
            if (!maybe_headers) {
              ...
              ERROR(
                  "unable to decode headers from {} at {}", record_group_header,
                  locator);
              ceph_abort();
            }
            ...
            if (has_tail_delta) {
              ...
              if (!maybe_record_deltas_list) {
                ERROR(
                    "unable to decode deltas from {} at {}",
      ...
      [this, nonce = segment_header.segment_nonce](auto& cursor, auto& handler) {
        return sm_group.scan_valid_records(
            cursor, nonce, std::numeric_limits<std::size_t>::max(), handler);
      });
}
    ...
    replay_stats_t& stats)
{
  ...
  return seastar::do_with(
      ...
      SegmentManagerGroup::found_record_handler_t(
          [&handler, this, &stats](
              record_locator_t locator, const record_group_header_t& header,
              const bufferlist& mdbuf)
              -> SegmentManagerGroup::scan_valid_records_ertr::future<> {
            LOG_PREFIX(Journal::replay_segment);
            ++stats.num_record_groups;
            auto maybe_record_deltas_list =
                try_decode_deltas(header, mdbuf, locator.record_block_base);
            if (!maybe_record_deltas_list) {
              // This should be impossible, we did check the crc on the mdbuf
              ERROR(
                  "unable to decode deltas for record {} at {}", header,
                  locator);
              return crimson::ct_error::input_output_error::make();
            }
            ...
            return seastar::do_with(
                ...
                [write_result = locator.write_result, this, FNAME, &handler,
                 &stats](auto& record_deltas_list) {
                  return crimson::do_for_each(
                      record_deltas_list,
                      [write_result, this, FNAME, &handler,
                       &stats](record_deltas_t& record_deltas) {
                        ...
                        return crimson::do_for_each(
                            ...
                            [locator, this, &handler, &stats](auto& p) {
                              auto& modify_time = p.first;
                              auto& delta = p.second;
                              return handler(
                                         locator, delta,
                                         trimmer.get_dirty_tail(),
                                         trimmer.get_alloc_tail(), modify_time)
                                  .safe_then([&stats, delta_type = delta.type](
                                                 auto ret) {
...
SegmentedJournal::replay(delta_handler_t&& delta_handler)
{
  LOG_PREFIX(Journal::replay);
  return sm_group.find_journal_segment_headers().safe_then(
      [this, FNAME, delta_handler = std::move(delta_handler)](
          auto&& segment_headers) mutable -> replay_ret {
        INFO("got {} segments", segment_headers.size());
        return seastar::do_with(
            std::move(delta_handler), replay_segments_t(), replay_stats_t(),
            [this, segment_headers = std::move(segment_headers), FNAME](
                auto& handler, auto& segments,
                auto& stats) mutable -> replay_ret {
              return prep_replay_segments(std::move(segment_headers))
                  .safe_then([this, &handler, &segments,
                              &stats](auto replay_segs) mutable {
                    segments = std::move(replay_segs);
                    return crimson::do_for_each(
                        segments, [this, &handler, &stats](auto i) mutable {
                          return replay_segment(
                              i.first, i.second, handler, stats);
                        });
                  })
                  .safe_then([&stats, FNAME] {
                    INFO(
                        "replay done, record_groups={}, records={}, "
                        "alloc_deltas={}, dirty_deltas={}",
                        stats.num_record_groups, stats.num_records,
                        stats.num_alloc_deltas, stats.num_dirty_deltas);
                  });
            });
      });
}
...
seastar::future<>
SegmentedJournal::flush(OrderingHandle& handle)
{
    ...
    on_submission_func_t&& on_submission)
{
  ...
  if (!record_submitter.is_available()) {
    ...
    return record_submitter.wait_available().safe_then(
        [this, record = std::move(record), &handle,
  ...
  if (action == RecordSubmitter::action_t::ROLL) {
    ...
    return record_submitter.roll_segment().safe_then(
        [this, record = std::move(record), &handle,
         on_submission = std::move(on_submission)]() mutable {
          return do_submit_record(
              std::move(record), handle, std::move(on_submission));
    ...
    on_submission_func_t&& on_submission)
{
  ...
  if (expected_size > max_record_length) {
    ERROR(
        "H{} {} exceeds max record size 0x{:x}", (void*)&handle, record,
        max_record_length);
    return crimson::ct_error::erange::make();
  }

  return do_submit_record(std::move(record), handle, std::move(on_submission));
}
