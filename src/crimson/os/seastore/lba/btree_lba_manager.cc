...
template <>
struct fmt::formatter<crimson::os::seastore::lba::LBABtree::iterator>
  : public fmt::formatter<std::string_view> {
  using Iter = crimson::os::seastore::lba::LBABtree::iterator;

  template <typename FmtCtx>
  auto
  format(const Iter& iter, FmtCtx& ctx) const -> decltype(ctx.out())
  {
    if (iter.is_end()) {
...
template <typename T>
Transaction::tree_stats_t&
get_tree_stats(Transaction& t)
{
  return t.get_lba_tree_stats();
}

template Transaction::tree_stats_t&
get_tree_stats<crimson::os::seastore::lba::LBABtree>(Transaction& t);

template <typename T>
phy_tree_root_t&
get_phy_tree_root(root_t& r)
{
  return r.lba_root;
}

template phy_tree_root_t&
get_phy_tree_root<crimson::os::seastore::lba::LBABtree>(root_t& r);

template <>
const get_phy_tree_root_node_ret
get_phy_tree_root_node<crimson::os::seastore::lba::LBABtree>(
    ...
    op_context_t c)
{
  ...
  if (lba_root) {
    ...
    return {true, c.cache.get_extent_viewable_by_trans(c.trans, lba_root)};
    ...
    if (lba_root) {
      ...
      return {
          false, Cache::get_extent_iertr::make_ready_future<CachedExtentRef>()};
    }
  } else {
    return {
        false, Cache::get_extent_iertr::make_ready_future<CachedExtentRef>()};
...
public:
  static void
  link_root(RootBlockRef& root_block, RootT* lba_root)
  {
  ...
  static void
  unlink_root(RootBlockRef& root_block)
  {
...
BtreeLBAManager::mkfs(Transaction& t)
{
...
BtreeLBAManager::get_cursors(Transaction& t, laddr_t laddr, extent_len_t length)
{
...
BtreeLBAManager::get_cursor(Transaction& t, laddr_t laddr, bool search_containing)
{
...
BtreeLBAManager::get_cursor_ret
BtreeLBAManager::get_cursor(Transaction& t, LogicalChildNode& extent)
{
  LOG_PREFIX(BtreeLBAManager::get_cursor);
  TRACET("{}", t, extent);
#ifndef NDEBUG
  if (extent.is_mutation_pending()) {
    auto& prior = static_cast<LogicalChildNode&>(*extent.get_prior_instance());
  ...
  if (leaf->is_pending()) {
    TRACET("find pending extent {} for {}", c.trans, (void*)leaf.get(), extent);
  }
#ifndef NDEBUG
    ...
    extent_len_t length)
{
  ...
  while (true) {
    if (pos.is_end() || pos.get_key() >= (laddr + length)) {
      TRACET(
          "{}~0x{:x} done with {} results, stop at {}", c.trans, laddr, length,
          ret.size(), pos);
      break;
    }
    TRACET("{}~0x{:x} got {}, repeat ...", c.trans, laddr, length, pos);
    ...
    const LBACursor& indirect_cursor)
{
  ...
  return get_cursors(
             c, btree, indirect_cursor.get_intermediate_key(),
      ...
      .si_then([&indirect_cursor](auto cursors) {
        ...
        assert(
            direct_cursor->get_laddr() + direct_cursor->get_length() >=
    ...
    extent_len_t len)
{
  ...
  lba_map_val_t val{
      len, P_ADDR_ZERO, EXTENT_DEFAULT_REF_COUNT, 0, extent_types_t::NONE};
    ...
    std::vector<LogicalChildNodeRef> extents)
{
  ...
  for (auto eiter = extents.rbegin(); eiter != extents.rend(); ++eiter) {
    ...
    auto p = co_await btree.insert(
        c, iter, ext->get_laddr(),
        lba_map_val_t{
            ext->get_length(), ext->get_paddr(), EXTENT_DEFAULT_REF_COUNT,
            ext->get_last_committed_crc(), ext->get_type()},
        ext.get());
    auto& [it, inserted] = p;
    ...
    bool updateref)
{
  ...
  DEBUGT(
      "pos={}, mapping={}, laddr={}~{}, inter_key={} updateref={}", t, *pos,
      *mapping, laddr, len, inter_key, updateref);
  assert(
      inter_key.get_byte_distance<extent_len_t>(mapping->get_laddr()) + len <=
      mapping->get_length());
  auto c = get_context(t);
  if (updateref) {
  ...
  auto p = co_await btree.insert(
      c, btree.make_partial_iter(c, *pos), laddr,
      lba_map_val_t{
          len, inter_key, EXTENT_DEFAULT_REF_COUNT, 0, extent_types_t::NONE},
      get_reserved_ptr<LBALeafNode, laddr_t>());
  auto& [iter, inserted] = p;
  co_await mapping->refresh();
  co_return clone_mapping_ret_t{iter.get_cursor(c), mapping};
}

BtreeLBAManager::get_cursor_ret
BtreeLBAManager::get_cursor(op_context_t c, LBABtree& btree, laddr_t laddr)
{
  LOG_PREFIX(BtreeLBAManager::get_cursor);
  TRACET("{} ...", c.trans, laddr);
  return btree.lower_bound(c, laddr).si_then(
      [FNAME, c, laddr](auto iter) -> get_cursor_ret {
        ...
        return get_cursor_ret(
            interruptible::ready_future_marker{}, iter.get_cursor(c));
      });
}
    ...
    alloc_policy_t policy)
{
  ...
  return seastar::do_with(
      ...
      [this, c, &btree, hint, length, lookup_attempts, policy,
       FNAME](laddr_t& last_end, OptIter& insert_iter) {
        return LBABtree::iterate_repeat(
                   c, btree.upper_bound_right(c, hint),
                   [this, c, hint, length, lookup_attempts, policy, &last_end,
                    &insert_iter, FNAME](auto& iter) {
                     ...
                     if (iter.is_end() ||
                       ...
                       DEBUGT(
                           "hint: {}~0x{:x}, allocated laddr: {}, insert "
                           "position: {}, "
                           "done with {} attempts",
                     ...
                     last_end = (iter.get_key() + iter.get_val().len)
                                    .checked_to_laddr();
                     TRACET(
                         "hint: {}~0x{:x}, current iter: {}, repeat ...",
    ...
    alloc_policy_t policy)
{
  ...
  return with_btree<LBABtree>(
      cache, c, [this, c, hint, &alloc_infos, total_len, policy](auto& btree) {
    ...
    std::vector<alloc_mapping_info_t>& alloc_infos)
{
  return seastar::do_with(
      ...
      [c, &btree,
       &alloc_infos](LBABtree::iterator& iter, std::list<LBACursorRef>& ret) {
        return trans_intr::do_for_each(
                   alloc_infos.begin(), alloc_infos.end(),
                   [c, &btree, &iter](auto& info) {
                     assert(info.key != L_ADDR_NULL);
                     bool need_reserved_ptr = info.is_indirect_mapping() ||
                                              info.is_zero_mapping();
                     return btree
                         .insert(
                             ...
                             need_reserved_ptr
                                 ? get_reserved_ptr<LBALeafNode, laddr_t>()
                                 : static_cast<
                                       BaseChildNode<LBALeafNode, laddr_t>*>(
                                       info.extent))
...
static bool
is_lba_node(const CachedExtent& e)
{
    ...
    bool& ret)
{
  if (e->is_logical()) {
    ...
    return btree.lower_bound(c, logn->get_laddr())
        .si_then([e, c, logn, &ret](auto iter) {
          LOG_PREFIX(BtreeLBAManager::init_cached_extent);
          if (!iter.is_end() && iter.get_key() == logn->get_laddr() &&
    ...
    return btree.init_cached_extent(c, e).si_then([&ret](bool is_alive) {
      ret = is_alive;
    });
  }
}
...
BtreeLBAManager::init_cached_extent(Transaction& t, CachedExtentRef e)
{
  LOG_PREFIX(BtreeLBAManager::init_cached_extent);
  TRACET("{}", t, *e);
...
BtreeLBAManager::check_child_trackers(Transaction& t)
{
  auto c = get_context(t);
  return with_btree<LBABtree>(cache, c, [c](auto& btree) {
    ...
    scan_mappings_func_t&& f)
{
  ...
  auto c = get_context(t);
  return with_btree<LBABtree>(
      cache, c, [c, f = std::move(f), begin, end](auto& btree) mutable {
        return LBABtree::iterate_repeat(
            c, btree.upper_bound_right(c, begin),
...
BtreeLBAManager::rewrite_extent(Transaction& t, CachedExtentRef extent)
{
  ...
  if (is_lba_node(*extent)) {
    ...
    auto c = get_context(t);
    return with_btree<LBABtree>(cache, c, [c, extent](auto& btree) mutable {
    ...
    LogicalChildNode& nextent)
{
  ...
  TRACET(
      "laddr={}, paddr {}~0x{:x} => {}~0x{:x}, crc=0x{:x}", t, laddr, prev_addr,
      prev_len, addr, len, checksum);
  assert(laddr == nextent.get_laddr());
  assert(!addr.is_null());
  auto res = co_await _update_mapping(
                 t, *cursor,
                 [prev_addr, addr, prev_len, len,
                  checksum](const lba_map_val_t& in) {
                   assert(!addr.is_null());
  ...
  DEBUGT(
      "laddr={}, paddr {}~0x{:x} => {}~0x{:x}, crc=0x{:x} done -- {}", t, laddr,
      prev_addr, prev_len, addr, len, checksum, *cursor);
    ...
    const std::list<LogicalChildNodeRef>& extents)
{
  ...
  auto c = get_context(t);
  return with_btree<LBABtree>(cache, c, [c, &extents, FNAME, this](auto& btree) {
    return trans_intr::do_for_each(extents, [this, FNAME, c, &btree](auto& extent) {
      return extent->get_parent_node(c.trans, c.cache)
          .si_then([c, &extent, FNAME, &btree, this](auto leaf) {
            if (leaf->is_pending()) {
              TRACET(
                  "find pending extent {} for {}", c.trans, (void*)leaf.get(),
                  *extent);
            }
            return seastar::do_with(
                ...
                [this, c, &extent, FNAME](auto& cursor) {
                  ...
                  TRACET(
                      "cursor={}, paddr {}~0x{:x} => {}, crc=0x{:x}", c.trans,
                      *cursor, prev_addr, len, addr, checksum);
                  assert(!addr.is_null());
                  return this
                      ->_update_mapping(
                          c.trans, *cursor,
                      ...
                      .si_then(
                          [c, prev_addr, len, addr, checksum, FNAME](auto res) {
                            DEBUGT(
                                "paddr {}~0x{:x} => {}, crc=0x{:x} done -- {}",
                                c.trans, prev_addr, len, addr, checksum, *res);
    ...
    extent_len_t len)
{
  ...
  auto c = get_context(t);
  return with_btree<LBABtree>(cache, c, [c, type, addr, laddr, len](auto& btree) {
...
void
BtreeLBAManager::register_metrics(store_index_t store_index)
{
  ...
  metrics.add_group(
      ...
      {
          sm::make_counter(
              "alloc_extents", stats.num_alloc_extents,
              sm::description("total number of lba alloc_extent operations"),
              {sm::label_instance(
                  "shard_store_index", std::to_string(store_index))}),
          sm::make_counter(
              "alloc_extents_iter_nexts", stats.num_alloc_extents_iter_nexts,
              sm::description("total number of iterator next operations during "
                              "extent allocation"),
              {sm::label_instance(
                  "shard_store_index", std::to_string(store_index))}),
      });
    ...
    LogicalChildNode* nextent)
{
  ...
  if (ret.refcount == 0) {
    ...
    iter = co_await btree.update(
        ...
        nextent && !nextent->has_parent_tracker() ? nextent : nullptr);
    assert(
        !nextent ||
    ...
    BtreeLBAManager::scan_mapped_space_func_t&& f)
{
  ...
  while (!pos.is_end()) {
    ...
    TRACET(
        "tree value {}~{} {}~{} used, type {}", c.trans, pos.get_key(),
        pos.get_val().len, pos.get_val().pladdr.get_paddr(), pos.get_val().len,
        pos.get_val().type);
    ceph_assert(pos.get_val().len > 0 && pos.get_val().len % block_size == 0);
    ceph_assert(pos.get_val().pladdr != L_ADDR_NULL);
    scan_visitor(
        pos.get_val().pladdr.get_paddr(), pos.get_val().len, pos.get_val().type,
        pos.get_key());
    pos = co_await pos.next(c);
  }

  LBABtree::mapped_space_visitor_t tree_visitor =
      [&scan_visitor, block_size, FNAME,
       c](paddr_t paddr, laddr_t key, extent_len_t len, depth_t depth,
          extent_types_t type, LBABtree::iterator&) {
        TRACET(
            "tree node {}~{} {}, depth={} used", c.trans, paddr, len, type,
            depth);
    ...
    laddr_t laddr)
{
  ...
  TRACET("{}", c.trans, laddr);
  return btree.upper_bound_right(c, laddr).si_then(
      [c, laddr, FNAME](LBABtree::iterator iter) -> get_cursor_ret {
        if (iter.is_end() || iter.get_key() > laddr ||
            iter.get_key() + iter.get_val().len <= laddr) {
          ERRORT("laddr={} doesn't exist", c.trans, laddr);
          return crimson::ct_error::enoent::make();
        }
        TRACET("{} got {}, {}", c.trans, laddr, iter.get_key(), iter.get_val());
        return get_cursor_iertr::make_ready_future<LBACursorRef>(
            iter.get_cursor(c));
      });
}
...
BtreeLBAManager::get_end_mapping(Transaction& t)
{
    ...
    std::vector<remap_entry_t> remaps)
{
  ...
  assert(
      orig_indirect || (orig_val.pladdr.is_paddr() &&
                        orig_val.pladdr.get_paddr().is_absolute()));
  cursor = co_await update_mapping_refcount(c.trans, cursor, -1);
  ...
  for (auto& remap : remaps) {
    ...
    auto p = co_await btree.insert(
        ...
        orig_indirect ? get_reserved_ptr<LBALeafNode, laddr_t>() : remap.extent);
    auto& [it, inserted] = p;
    ceph_assert(inserted);
    ret.push_back(it.get_cursor(c));
    iter = co_await it.next(c);
  }
  co_await trans_intr::parallel_for_each(ret, [](auto& cursor) {
