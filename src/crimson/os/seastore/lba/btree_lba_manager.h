...
public:
  ...
  get_cursor_ret get_cursor(Transaction& t, LogicalChildNode& extent) final;
  ...
  alloc_extent_ret
  reserve_region(Transaction& t, laddr_t hint, extent_len_t len) final
  {
      ...
      extent_ref_count_t refcount) final
  {
    ...
    std::vector<alloc_mapping_info_t> alloc_infos = {
        alloc_mapping_info_t::create_direct(
            L_ADDR_NULL, ext.get_length(), ext.get_paddr(), refcount,
            ext.get_last_committed_crc(), ext)};
    auto cursors = co_await alloc_contiguous_mappings(
        t, hint, alloc_infos, alloc_policy_t::linear_search);
    assert(cursors.size() == 1);
    co_return std::move(cursors.front());
      ...
      extent_ref_count_t refcount) final
  {
    ...
    for (auto& extent : extents) {
      ...
      alloc_infos.emplace_back(alloc_mapping_info_t::create_direct(
          ...
          extent->get_length(), extent->get_paddr(), refcount,
          extent->get_last_committed_crc(), *extent));
    }
  ...
  update_mapping_refcount(Transaction& t, LBACursorRef cursor, int delta) final
  {
    co_return co_await _update_mapping(
        ...
        .handle_error_interruptible(
            base_iertr::pass_further{}, crimson::ct_error::assert_all{});
  }
...
private:
  ...
  struct alloc_mapping_info_t {
    ...
    bool
    is_zero_mapping() const
    {
      return value.pladdr.is_paddr() && value.pladdr.get_paddr().is_zero();
    }

    bool
    is_indirect_mapping() const
    {
      return value.pladdr.is_laddr();
    }

    static alloc_mapping_info_t
    create_zero(extent_len_t len)
    {
      return {
          L_ADDR_NULL,
          {len, pladdr_t(P_ADDR_ZERO), EXTENT_DEFAULT_REF_COUNT, 0,
           extent_types_t::NONE}};
    }

    static alloc_mapping_info_t
    create_indirect(laddr_t laddr, extent_len_t len, laddr_t intermediate_key)
    {
      return {
          laddr,
          {len, pladdr_t(intermediate_key), EXTENT_DEFAULT_REF_COUNT,
  ...
  get_cursor_ret get_cursor(op_context_t c, LBABtree& btree, laddr_t offset);
  ...
  resolve_indirect_cursor_ret
  resolve_indirect_cursor(op_context_t c, const LBACursor& indirect_cursor)
  {
    assert(indirect_cursor.is_indirect());
    return with_btree<LBABtree>(
        cache, c, [c, &indirect_cursor, this](auto& btree) {
