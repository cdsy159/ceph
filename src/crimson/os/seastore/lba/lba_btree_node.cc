...
std::ostream&
LBALeafNode::print_detail(std::ostream& out) const
{
...
void
LBALeafNode::resolve_relative_addrs(paddr_t base)
{
...
void
LBALeafNode::update(internal_const_iterator_t iter, lba_map_val_t val)
{
  LOG_PREFIX(LBALeafNode::update);
  SUBTRACE(
      seastore_fixedkv_tree, "trans.{}, pos {}", this->pending_for_transaction,
      iter.get_offset());
  this->on_modify();
...
LBALeafNode::internal_const_iterator_t
LBALeafNode::insert(
    ...
    lba_map_val_t val)
{
  ...
  SUBTRACE(
      ...
      this->pending_for_transaction, iter.get_offset(), addr);
  this->on_modify();
  if (val.pladdr.is_paddr()) {
    val.pladdr = maybe_generate_relative(val.pladdr.get_paddr());
  }
...
LBACursor::refresh()
{
  ...
  return with_btree<lba::LBABtree>(
      ctx.cache, ctx, [this, FNAME, c = ctx](auto& btree) {
        ...
        if (!parent->is_valid()) {
          ...
          SUBTRACET(
              seastore_lba,
              "cursor {} parent is invalid, re-search from scratch", c.trans,
              *this);
          return btree.lower_bound(c, this->get_laddr())
              .si_then([this](lba::LBABtree::iterator it) {
                ...
                iter = LBALeafNode::iterator(
                    it.get_leaf_node().get(), it.get_leaf_pos());
        ...
        if (leaf->is_pending_in_trans(c.trans.get_trans_id())) {
          ...
          SUBTRACET(
              seastore_lba, "cursor: {} viewable: {}", c.trans, *this, viewable);
          if (!viewable) {
            leaf = l;
