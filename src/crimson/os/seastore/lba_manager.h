...
public:
  ...
  using get_cursor_iertr = base_iertr::extend<crimson::ct_error::enoent>;
  using get_cursor_ret = get_cursor_iertr::future<LBACursorRef>;
  virtual get_cursor_ret get_cursor(
      Transaction& t,
      laddr_t offset,
      bool search_containing = false) = 0;
  virtual get_cursor_ret get_cursor(Transaction& t, LogicalChildNode& extent) = 0;
  ...
  using alloc_extents_ret =
      alloc_extent_iertr::future<std::vector<LBACursorRef>>;
  ...
  ref_iertr::future<>
  update_mapping_refcount(Transaction& t, laddr_t addr, int delta)
  {
  ...
  virtual remap_ret remap_mappings(
      Transaction& t,
      LBACursorRef orig_mapping,
      std::vector<remap_entry_t> remaps) = 0;
  ...
  using scan_mappings_func_t =
      std::function<void(laddr_t, paddr_t, extent_len_t)>;
  ...
  using scan_mapped_space_func_t =
      std::function<void(paddr_t, extent_len_t, extent_types_t, laddr_t)>;
...
inline std::ostream&
operator<<(std::ostream& lhs, const LBAManager::remap_entry_t& rhs)
{
  return lhs << "remap_entry_t(" << "offset=0x" << std::hex << rhs.offset
             << ", len=0x" << rhs.len << std::dec << ", extent=" << rhs.extent
             << ")";
}
...
template <>
struct fmt::formatter<crimson::os::seastore::LBAManager::remap_entry_t>
