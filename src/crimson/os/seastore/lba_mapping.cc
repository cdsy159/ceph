...
std::ostream&
operator<<(std::ostream& out, const LBAMapping& rhs)
{
  if (rhs.is_end()) {
    return out << "LBAMapping(END)";
  }
  out << "LBAMapping(" << rhs.get_key() << "~0x" << std::hex
      << rhs.get_length();
  if (rhs.is_complete()) {
    out << std::dec << "->" << rhs.get_val();
  } else {
    out << std::dec << "->" << rhs.indirect_cursor->iter.get_val();
  }
  if (rhs.is_complete_indirect()) {
    out << ",indirect(" << rhs.get_intermediate_base() << "~0x" << std::hex
        << rhs.get_intermediate_length() << "@0x"
        << rhs.get_intermediate_offset() << std::dec << ")";
  }
  out << ")";
  return out;
}
...
std::ostream&
operator<<(std::ostream& out, const lba_mapping_list_t& rhs)
{
  bool first = true;
  out << '[';
...
LBAMapping::get_logical_extent(Transaction& t) const
{
  ...
  ceph_assert(direct_cursor->ctx.trans.get_trans_id() == t.get_trans_id());
...
LBAMapping::is_stable() const
{
  ...
  return leaf->is_child_stable(
      direct_cursor->ctx, direct_cursor->get_pos(), direct_cursor->get_laddr());
}

bool
LBAMapping::is_data_stable() const
{
  assert(is_linked_direct());
  ceph_assert(direct_cursor->is_viewable());
  assert(!direct_cursor->is_end());
...
LBAMapping::next()
{
  ...
  mapping = co_await with_btree_state<lba::LBABtree, LBAMapping>(
      ctx.cache, ctx, std::move(mapping),
...
bool
LBAMapping::is_initial_pending() const
{
  assert(is_linked_direct());
  ceph_assert(direct_cursor->is_viewable());
  assert(!direct_cursor->is_end());
