...
public:
  ...
  child_pos_t(
    ...
    stable_parent(stable_parent), pos(pos), retired_placeholder(placeholder)
  {
    assert(retired_placeholder->is_placeholder());
  }

  TCachedExtentRef<ParentT>
  get_parent()
  {
...
using get_child_iertr =
    trans_iertr<crimson::errorator<crimson::ct_error::input_output_error>>;
template <typename T>
...
struct node_cmp_t {
  ...
  bool
  operator()(const TCachedExtentRef<T>& l, const TCachedExtentRef<T>& r) const
  {
    assert(
        l->get_end() <= r->get_begin() || r->get_end() <= l->get_begin() ||
        (l->get_begin() == r->get_begin() && l->get_end() == r->get_end()));
    return l->get_begin() < r->get_begin();
  }

  bool
  operator()(const key_t& l, const TCachedExtentRef<T>& r) const
  {
    return l < r->get_begin();
  }

  bool
  operator()(const TCachedExtentRef<T>& l, const key_t& r) const
  {
...
template <typename ParentT>
class parent_tracker_t : public boost::intrusive_ref_counter<
                             parent_tracker_t<ParentT>,
...
public:
  ...
  void
  reset_parent(TCachedExtentRef<ParentT> p)
  {
    parent = p;
  }
...
public:
  bool
  has_parent_tracker() const
  {
    return (bool)parent_tracker;
  }

  void
  reset_parent_tracker(parent_tracker_t<ParentT>* p = nullptr)
  {
    parent_tracker.reset(p);
  }

  bool
  is_parent_valid() const
  {
...
inline BaseChildNode<T, node_key_t>*
get_reserved_ptr()
{
...
template <typename T, typename node_key_t>
bool
is_reserved_ptr(BaseChildNode<T, node_key_t>* child)
{
...
template <typename T, typename node_key_t>
bool
is_valid_child_ptr(BaseChildNode<T, node_key_t>* child)
{
...
public:
  template <typename T>
  get_child_ifut<T>
  get_extent_viewable_by_trans(Transaction& t, TCachedExtentRef<T> ext)
  {
    return get_extent_viewable_by_trans(t, CachedExtentRef(ext.get()))
        .si_then([](auto ext) { return ext->template cast<T>(); });
  }
...
template <typename T, typename node_key_t, typename Comparator>
class ParentNode {
  /*
   *
...
public:
  std::pair<bool, TCachedExtentRef<T>>
  resolve_transaction(Transaction& t, node_key_t key)
  {
    auto& me = down_cast();
      ...
      node_key_t key)
  {
    ...
    if (is_valid_child_ptr(child)) {
      if (child->is_retired_placeholder()) {
        assert(me.is_stable());
        return child_pos_t<T>(&me, pos, dynamic_cast<CachedExtent*>(child));
      } else {
  ...
  void
  link_child(BaseChildNode<T, node_key_t>* child, btreenode_pos_t pos)
  {
    auto& me = down_cast();
      ...
      btreenode_pos_t size = 0)
  {
    ...
    std::memmove(
        &raw_children[offset + 1], &raw_children[offset],
  ...
  void
  update_child_ptr(btreenode_pos_t pos, BaseChildNode<T, node_key_t>* child)
  {
    children[pos] = child;
    set_child_ptracker(child);
  }
  ...
  struct copy_dests_t : trans_spec_view_t {
    ...
    copy_dests_t(Transaction& t) :
      trans_spec_view_t{t.get_trans_id()}
    {}
  ...
  const copy_dests_t*
  get_copy_dests(Transaction& t)
  {
    auto iter =
        copy_dests_by_trans.find(t.get_trans_id(), trans_spec_view_t::cmp_t());
...
protected:
  ...
  find_pending_version(
      Transaction& t,
      node_key_t key,
      CachedExtent::viewable_state_t& hint)
  {
    auto& me = down_cast();
    assert(me.is_stable());
    if (hint == CachedExtent::viewable_state_t::stable_become_pending) {
      ...
      assert(
          copy_dests_by_trans.find(t.get_trans_id()) ==
  ...
  void
  for_each_copy_dest_set(Transaction& t, Func&& f)
  {
  ...
  void
  add_copy_dest(Transaction& t, TCachedExtentRef<T> dest)
  {
    ceph_assert(down_cast().is_stable());
    ceph_assert(dest->is_pending());
    auto tid = t.get_trans_id();
    auto iter = copy_dests_by_trans.lower_bound(tid, trans_spec_view_t::cmp_t());
    if (iter == copy_dests_by_trans.end() ||
    ...
    auto& copy_dests = static_cast<copy_dests_t&>(*iter);
    [[maybe_unused]] auto [it, inserted] = copy_dests.dests_by_key.insert(dest);
    assert(inserted || it->get() == dest.get());
  }

  void
  del_copy_dest(Transaction& t, TCachedExtentRef<T> dest)
  {
    auto iter =
        copy_dests_by_trans.find(t.get_trans_id(), trans_spec_view_t::cmp_t());
    ceph_assert(iter != copy_dests_by_trans.end());
  ...
  void
  set_child_ptracker(BaseChildNode<T, node_key_t>* child)
  {
    if (!this->my_tracker) {
      auto& me = down_cast();
  ...
  void
  remove_child_ptr(btreenode_pos_t offset)
  {
    auto& me = down_cast();
    LOG_PREFIX(ParentNode::remove_child_ptr);
    auto raw_children = children.data();
    SUBTRACE(
        seastore_fixedkv_tree, "trans.{}, pos {}, total size {}, extent {}",
        me.pending_for_transaction, offset, me.get_size(),
    ...
    // reset it here
    std::memmove(
        &raw_children[offset], &raw_children[offset + 1],
        (me.get_size() - offset - 1) * sizeof(BaseChildNode<T, node_key_t>*));
    maybe_shink_children();
  }

  void
  on_rewrite(Transaction& t, T& foreign_extent)
  {
    auto& me = down_cast();
    if (foreign_extent.is_stable()) {
      ...
      auto copy_source = foreign_extent.get_prior_instance()->template cast<T>();
  ...
  T&
  get_stable_for_key(node_key_t key) const
  {
    auto& me = down_cast();
  ...
  static void
  push_copy_sources(Transaction& t, T& dest, T& src)
  {
    ...
    if (src.is_stable()) {
      ...
      auto copy_src = src.get_prior_instance()->template cast<T>();
  ...
  {
    std::memmove(
        dest.children.data() + dest_start, src.children.data() + src_start,
    ...
    for (auto it = src.children.begin() + src_start;
         it != src.children.begin() + src_end; it++) {
      auto child = *it;
      if (is_valid_child_ptr(child)) {
  ...
  void
  split_child_ptrs(Transaction& t, T& left, T& right)
  {
    auto& me = down_cast();
  ...
  void
  adjust_copy_src_dest_on_split(Transaction& t, T& left, T& right)
  {
    auto& me = down_cast();
  ...
  void
  merge_child_ptrs(Transaction& t, T& left, T& right)
  {
    auto& me = down_cast();
  ...
  void
  adjust_copy_src_dest_on_merge(Transaction& t, T& left, T& right)
  {
    auto& me = down_cast();
  ...
  is_children_empty() const
  {
    for (auto it = children.begin();
         it != children.begin() + down_cast().get_size(); it++) {
  ...
  void
  set_children_from_prior_instance()
  {
    auto& me = down_cast();
    ...
    if (!copy_sources.empty()) {
      ...
      for (; it != copy_sources.end(); it++) {
        ...
        local_next_pos = copy_children_from_stable_source(
            *copy_source, foreign_start_iter, foreign_end_iter,
            local_start_iter);
  ...
  validate_stable_children()
  {
    ...
    for (auto i : me) {
      ...
      if (is_valid_child_ptr(child) && child->node_begin() != i.get_key()) {
        SUBERROR(
            seastore_fixedkv_tree, "stable child not valid: child {}, key {}",
            *dynamic_cast<CachedExtent*>(child), i.get_key());
        ceph_abort();
        return false;
      }
    }
...
private:
  ...
  void
  maybe_expand_children(size_t size)
  {
  ...
  maybe_shink_children()
  {
    if constexpr (T::CHILD_VEC_UNIT) {
      ...
      if (children.capacity() > T::CHILD_VEC_UNIT &&
          me.get_size() <
              (children.capacity() / 3 /*should be parameterized*/)) {
...
public:
  ...
  get_parent_node_ret
  get_parent_node(Transaction& t, ExtentTransViewRetriever& etvr)
  {
    auto& me = down_cast();
  ...
  bool
  is_retired_placeholder() const final
  {
    auto& me = down_cast();
...
private:
  ...
  get_parent_node_ret
  _get_parent_node(Transaction& t, ExtentTransViewRetriever& etvr, key_t key)
  {
    return etvr.maybe_wait_accessible(t, *this->peek_parent_node())
        .si_then([&t, key, this] {
          auto parent = this->peek_parent_node();
...
template <typename T>
std::ostream&
operator<<(std::ostream& out, const parent_tracker_t<T>& tracker)
{
