...
public:
  ...
  void
  on_invalidated(Transaction&) final
  {
  ...
  std::ostream&
  print_detail(std::ostream& out) const final
  {
...
class LogicalChildNode
  : public LogicalCachedExtent,
    public ChildNode<lba::LBALeafNode, LogicalChildNode, laddr_t> {
  using lba_child_node_t =
      ChildNode<lba::LBALeafNode, LogicalChildNode, laddr_t>;

public:
  template <typename... T>
  LogicalChildNode(T&&... t) :
    LogicalCachedExtent(std::forward<T>(t)...)
  {}

  virtual void
  lcn_on_invalidated(Transaction& t)
  {}
...
protected:
  ...
  virtual void
  do_on_replace_prior(Transaction& t)
  {}
...
template <>
struct fmt::formatter<crimson::os::seastore::LogicalChildNode>
