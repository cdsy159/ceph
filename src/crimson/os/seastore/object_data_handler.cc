...
void
ObjectDataBlock::apply_delta(const ceph::bufferlist& bl)
{
  auto biter = bl.begin();
...
guarded_object_data_t
guard_object_data(context_t ctx)
{
...
template <typename F>
auto
with_object_data(ObjectDataHandler::context_t ctx, F&& f)
{
  return seastar::do_with(
      ctx.onode.get_layout().object_data.get(), std::forward<F>(f),
...
template <typename F>
auto
with_objects_data(ObjectDataHandler::context_t ctx, F&& f)
{
  ceph_assert(ctx.d_onode);
  return seastar::do_with(
      ctx.onode.get_layout().object_data.get(),
      ctx.d_onode->get_layout().object_data.get(), std::forward<F>(f),
    ...
    extent_len_t size)
{
  ...
  if (!object_data.is_null()) {
    ...
    DEBUGT(
        "reservation present: {}~0x{:x}", ctx.t,
        object_data.get_reserved_data_base(),
        object_data.get_reserved_data_len());
    return write_iertr::make_ready_future<std::optional<LBAMapping>>();
  } else {
    DEBUGT(
        "reserving: {}~0x{:x}", ctx.t, ctx.onode.get_data_hint(),
        max_object_size);
    return ctx.tm
        .reserve_region(ctx.t, ctx.onode.get_data_hint(), max_object_size)
...
ObjectDataHandler::read_iertr::future<std::optional<bufferlist>>
read_mapping(
    ...
    bool for_zero /* whether this is for zero overwrite*/)
{
  ...
  TRACET(
      "{}~{} {} zero?{}", ctx.t, unaligned_offset, unaligned_len, read_pos,
      for_zero);
  ...
  if (read_pos.is_zero_reserved()) {
    ...
    auto aligned_len =
        p2roundup(unaligned_offset + unaligned_len, ctx.tm.get_block_size()) -
        aligned_offset;
    return ctx.tm
        .read_pin<ObjectDataBlock>(ctx.t, read_pos, aligned_offset, aligned_len)
        .si_then([unaligned_offset, unaligned_len, aligned_offset,
                  aligned_len](auto maybe_indirect_left_extent) {
          auto read_bl =
              maybe_indirect_left_extent.get_range(aligned_offset, aligned_len);
...
std::ostream&
operator<<(std::ostream& out, const clone_range_t& clonerange_info)
{
  return out << "clone_range_t{"
             << "first_src_mapping=" << clonerange_info.first_src_mapping
             << ", src_base=" << std::hex << clonerange_info.src_base
             << std::dec << ", dest_base=" << std::hex
             << clonerange_info.dest_base << std::dec << ", offset=" << std::hex
             << clonerange_info.offset << std::dec << ", len=" << std::hex
             << clonerange_info.len << std::dec << "}";
}

std::ostream&
operator<<(std::ostream& out, const overwrite_range_t& overwrite_range)
{
  out << "overwrite_range_t{" << std::hex << "unaligned_len=0x"
      << overwrite_range.unaligned_len << ", unaligned_begin=0x"
      << overwrite_range.unaligned_begin << ", aligned_begin=0x"
      << overwrite_range.aligned_begin << ", unaligned_end=0x"
      << overwrite_range.unaligned_end << ", aligned_end=0x"
      << overwrite_range.aligned_end << ", aligned_len=0x"
      << overwrite_range.aligned_len << std::dec;
...
{
  return out << "data_t{" << std::hex << "headbl=0x"
             << (data.headbl ? data.headbl->length() : 0) << ", head_padding=0x"
             << (data.head_padding ? data.head_padding->length() : 0)
             << ", bl=0x" << (data.bl ? data.bl->length() : 0) << ", tailbl=0x"
             << (data.tailbl ? data.tailbl->length() : 0) << ", tail_padding=0x"
             << (data.tail_padding ? data.tail_padding->length() : 0)
             << std::dec << "}";
}
    ...
    std::optional<bufferlist> data)
{
  ...
  DEBUGT(
      "0x{:x}~0x{:x} {} zero={}", ctx.t, unaligned_offset, unaligned_len,
      overwrite_mapping, !data.has_value());
  // delta based overwrite
  return ctx.tm.read_pin<ObjectDataBlock>(ctx.t, overwrite_mapping)
      .handle_error_interruptible(
      ...
      .si_then([overwrite_mapping, unaligned_offset, unaligned_len,
                data = std::move(data)](auto extent) {
        bufferlist bl;
        if (data) {
          bl.append(*data);
        } else {
    ...
    data_t& data)
{
  ...
  if (data.tailbl) {
    ...
    data.tailbl->prepend_zero(ctx.tm.get_block_size() - data.tailbl->length());
    auto extents =
        co_await ctx.tm
            .alloc_data_extents<ObjectDataBlock>(
                ctx.t,
                (overwrite_range.aligned_end - ctx.tm.get_block_size())
                    .checked_to_laddr(),
                ctx.tm.get_block_size(), std::move(zero_pos))
            .handle_error_interruptible(
                crimson::ct_error::enospc::assert_failure{"unexpected enospc"},
    ...
    zero_pos =
        co_await ctx.tm.get_pin(ctx.t, *extent)
            .handle_error_interruptible(
                crimson::ct_error::enospc::assert_failure{"unexpected enospc"},
                TransactionManager::get_pin_iertr::pass_further{})
            .handle_error_interruptible(
  ...
  auto laddr = (overwrite_range.aligned_begin +
                (data.headbl ? ctx.tm.get_block_size() : 0))
                   .checked_to_laddr();
  auto end = (overwrite_range.aligned_end -
              (data.tailbl ? ctx.tm.get_block_size() : 0))
                 .checked_to_laddr();
  auto len = end.get_byte_distance<extent_len_t>(laddr);
  if (len != 0) {
    zero_pos =
        co_await ctx.tm.reserve_region(ctx.t, std::move(zero_pos), laddr, len)
            .handle_error_interruptible(
                crimson::ct_error::enospc::assert_failure{"unexpected enospc"},
                TransactionManager::get_pin_iertr::pass_further{})
            .handle_error_interruptible(
  ...
  if (data.headbl) {
    ...
    data.headbl->append_zero(ctx.tm.get_block_size() - data.headbl->length());
    auto extents =
        co_await ctx.tm
            .alloc_data_extents<ObjectDataBlock>(
                ctx.t, overwrite_range.aligned_begin, ctx.tm.get_block_size(),
                std::move(zero_pos))
            .handle_error_interruptible(
                crimson::ct_error::enospc::assert_failure{"unexpected enospc"},
                TransactionManager::get_pin_iertr::pass_further{})
    ...
    data_t& data)
{
  ...
  assert(
      write_pos.is_end() || write_pos.get_key() >= overwrite_range.aligned_end);
  ...
  auto aligned_len = p2align(offset + len, ctx.tm.get_block_size()) -
                     aligned_off;
    ...
    auto extents =
        co_await ctx.tm
            .alloc_data_extents<ObjectDataBlock>(
                ...
                (overwrite_range.aligned_end - ctx.tm.get_block_size())
                    .checked_to_laddr(),
                ctx.tm.get_block_size(), std::move(write_pos))
            .handle_error_interruptible(
                crimson::ct_error::enospc::assert_failure{"unexpected enospc"},
                TransactionManager::get_pin_iertr::pass_further{});
    assert(extents.size() == 1);
    ...
    data_t& data)
{
  ...
  return ctx.tm
      .alloc_data_extents<ObjectDataBlock>(
          ...
          overwrite_range.aligned_end.template get_byte_distance<extent_len_t>(
...
std::ostream&
operator<<(std::ostream& out, const edge_t& edge)
{
  out << "edge_t{";
  switch (edge) {
  case edge_t::NONE:
    ...
    edge_t edge)
{
  ...
  if (edge & edge_t::LEFT) {
    auto unaligned_off =
        read_pos.get_key().template get_byte_distance<extent_len_t>(
            overwrite_range.aligned_begin);
    auto unaligned_length =
        overwrite_range.unaligned_begin.template get_byte_distance<extent_len_t>(
            overwrite_range.aligned_begin);
    futs.emplace_back(
        read_mapping(ctx, read_pos, unaligned_off, unaligned_length, !data.bl)
            .si_then([&data](auto bl) { data.headbl = std::move(bl); }));
  }

  if (edge & edge_t::RIGHT) {
    auto unaligned_off =
        overwrite_range.unaligned_end.template get_byte_distance<extent_len_t>(
            read_pos.get_key());
    auto unaligned_length =
        overwrite_range.aligned_end.template get_byte_distance<extent_len_t>(
            overwrite_range.unaligned_end);
    futs.emplace_back(
        read_mapping(ctx, read_pos, unaligned_off, unaligned_length, !data.bl)
            .si_then([&data](auto bl) { data.tailbl = std::move(bl); }));
  }
  ...
  return trans_intr::parallel_for_each(futs, [](auto& fut) {
    return std::move(fut);
  });
}
    ...
    edge_t edge)
{
  ...
  if (edge & edge_t::LEFT) {
    auto unaligned_length =
        edge_mapping.get_key().template get_byte_distance<extent_len_t>(
            overwrite_range.unaligned_begin);
    if (unaligned_length != 0) {
      overwrite_range.expand_begin(edge_mapping.get_key());
      futs.emplace_back(
          read_mapping(ctx, edge_mapping, 0, unaligned_length, !data.bl)
              .si_then([&data](auto bl) { data.headbl = std::move(bl); }));
    }
  }

  if (edge & edge_t::RIGHT) {
    auto unaligned_offset =
        overwrite_range.unaligned_end.template get_byte_distance<extent_len_t>(
            edge_mapping.get_key());
    ...
    if (len != 0) {
      ...
      futs.emplace_back(
          read_mapping(ctx, edge_mapping, unaligned_offset, len, !data.bl)
              .si_then([&data](auto bl) { data.tailbl = std::move(bl); }));
    }
  }
    ...
    edge_t edge)
{
  ...
  if (data.bl) {
    extent_len_t unaligned_len =
        (edge == edge_t::LEFT)
            ? overwrite_range.unaligned_begin
                  .template get_byte_distance<extent_len_t>(
                      edge_mapping.get_key() + edge_mapping.get_length())
            : overwrite_range.unaligned_end
                  .template get_byte_distance<extent_len_t>(
                      edge_mapping.get_key());
    extent_len_t unaligned_offset = (edge == edge_t::LEFT)
                                        ? 0
                                        : data.bl->length() - unaligned_len;
  ...
  extent_len_t unaligned_overlapped_offset =
      (edge == edge_t::LEFT)
          ? overwrite_range.unaligned_begin
                .template get_byte_distance<extent_len_t>(edge_mapping.get_key())
          : 0;
  ...
  return delta_based_overwrite(
             ctx, unaligned_overlapped_offset, unaligned_overlapped_len,
      ...
      .si_then([edge_mapping, &overwrite_range, edge]() mutable {
        if (edge == edge_t::LEFT) {
          ...
          return base_iertr::make_ready_future<LBAMapping>(
    ...
    LBAMapping edge_mapping)
{
  ...
  return ctx.tm.read_pin<ObjectDataBlock>(ctx.t, edge_mapping)
      .si_then([&overwrite_range, &data,
                edge_mapping](auto maybe_indirect_extent) {
        assert(!maybe_indirect_extent.is_indirect());
        assert(maybe_indirect_extent.extent);
        assert(maybe_indirect_extent.extent->is_initial_pending());
        auto offset = overwrite_range.unaligned_begin
                          .template get_byte_distance<extent_len_t>(
                              edge_mapping.get_key());
        bufferlist bl;
        if (data.bl) {
    ...
    edge_t edge)
{
  ...
  return ctx.tm.read_pin<ObjectDataBlock>(ctx.t, edge_mapping)
      .si_then([bl = std::move(bl), &overwrite_range, edge_mapping,
                edge](auto maybe_indirect_extent) mutable {
        assert(!maybe_indirect_extent.is_indirect());
        assert(maybe_indirect_extent.extent);
        assert(maybe_indirect_extent.extent->is_initial_pending());
        extent_len_t offset =
  ...
  return fut.si_then([ctx, edge_mapping, &overwrite_range, edge] {
    if (edge == edge_t::LEFT) {
      if (overwrite_range.aligned_begin > edge_mapping.get_key()) {
        return ctx.tm
            .cut_mapping<ObjectDataBlock>(
                ctx.t, overwrite_range.aligned_begin, std::move(edge_mapping),
                true)
            .si_then([](auto mapping) { return mapping.next(); });
      } else {
    ...
    op_type_t op_type)
{
  ...
  laddr_t aligned_overlapped_start = (edge == edge_t::LEFT)
                                         ? overwrite_range.aligned_begin
                                         : edge_key;
  extent_len_t aligned_overlapped_len =
      (edge == edge_t::LEFT)
          ? overwrite_range.aligned_begin
                .template get_byte_distance<extent_len_t>(edge_key + edge_length)
          : overwrite_range.aligned_end.template get_byte_distance<extent_len_t>(
                edge_key);
  auto ehpolicy = get_edge_handle_policy(
      edge_mapping, aligned_overlapped_start, aligned_overlapped_len, op_type);
  switch (ehpolicy) {
    ...
    op_type_t op_type)
{
  if (overwrite_range.unaligned_begin > left_mapping.get_key()) {
    ...
    return punch_mapping_on_edge(
        ctx, overwrite_range, overwrite_data, std::move(left_mapping),
        edge_t::LEFT, op_type);
  }
  return base_iertr::make_ready_future<LBAMapping>(std::move(left_mapping));
}
    ...
    LBAMapping first_mapping)
{
  ...
  return ctx.tm.remove_mappings_in_range(
      ctx.t, overwrite_range.aligned_begin, unaligned_len,
    ...
    op_type_t op_type)
{
  if (right_mapping.is_end() ||
    ...
    return base_iertr::make_ready_future<LBAMapping>(std::move(right_mapping));
  }
  return punch_mapping_on_edge(
      ctx, overwrite_range, overwrite_data, std::move(right_mapping),
      edge_t::RIGHT, op_type);
}
    ...
    op_type_t op_type)
{
  ...
  mapping =
      co_await punch_inner_mappings(ctx, overwrite_range, std::move(mapping));
  if (overwrite_range.clonerange_info.has_value()) {
    co_await overwrite_range.clonerange_info->refresh();
  }
...
{
  auto ehpolicy = get_edge_handle_policy(
      mapping, overwrite_range.aligned_begin, overwrite_range.aligned_len,
      op_type);
  auto do_overwrite = [ctx, &overwrite_range, &data, op_type](auto pos) {
    ...
    if (overwrite_range.aligned_end.template get_byte_distance<extent_len_t>(
            overwrite_range.aligned_begin) == ctx.tm.get_block_size() &&
  ...
  case edge_handle_policy_t::DELTA_BASED_PUNCH: {
    auto unaligned_offset =
        mapping.get_key().template get_byte_distance<extent_len_t>(
            overwrite_range.unaligned_begin);
  ...
  case edge_handle_policy_t::MERGE_INPLACE: {
    return merge_into_mapping(ctx, overwrite_range, data, std::move(mapping));
  }
  case edge_handle_policy_t::REMAP: {
    auto fut = base_iertr::now();
    ...
    if (edge != edge_t::NONE) {
      fut = read_unaligned_edge_data(ctx, overwrite_range, data, mapping, edge);
    }
    return fut
        .si_then([ctx, &overwrite_range, mapping] {
          return ctx.tm.punch_hole_in_mapping<ObjectDataBlock>(
              ctx.t, overwrite_range.aligned_begin, overwrite_range.aligned_len,
              std::move(mapping));
        })
    ...
    LBAMapping first_mapping)
{
  ...
  return seastar::do_with(
      data_t{std::move(bl)},
      overwrite_range_t{
          len, unaligned_begin, unaligned_end, ctx.tm.get_block_size()},
      [first_mapping = std::move(first_mapping), this,
       ctx](auto& data, auto& overwrite_range) {
    ...
    data_t& data)
{
  ...
  TRACET(
      "object_data: {}~{} range: {}~{}", ctx.t, base,
      object_data.get_reserved_data_len(), offset, len);
  auto block_size = ctx.tm.get_block_size();
  if (is_aligned(offset, block_size) && is_aligned(offset + len, block_size)) {
    return read_iertr::now();
  }
  std::vector<TransactionManager::get_pin_iertr::future<>> read_paddings;
  ...
  if (aligned_offset == aligned_end) {
    ...
    return ctx.tm.get_containing_pin(ctx.t, aligned_offset)
        .si_then([begin, ctx, len](auto read_pos) {
          auto unaligned_offset =
              begin.template get_byte_distance<extent_len_t>(read_pos.get_key());
          return read_mapping(
              ctx, std::move(read_pos), unaligned_offset, len, false);
        })
        .si_then([&data](auto bl) { data.bl = std::move(bl); })
        .handle_error_interruptible(
    ...
    read_paddings.emplace_back(
        ctx.tm.get_containing_pin(ctx.t, aligned_offset)
            .si_then([begin, ctx, block_size](auto read_pos) {
              auto unaligned_offset =
                  begin.template get_byte_distance<extent_len_t>(
                      read_pos.get_key());
              auto rounded_begin = begin.get_roundup_laddr(block_size);
              auto len =
                  rounded_begin.template get_byte_distance<extent_len_t>(begin);
              return read_mapping(
                  ctx, std::move(read_pos), unaligned_offset, len, false);
            })
    ...
    read_paddings.emplace_back(
        ctx.tm.get_containing_pin(ctx.t, aligned_end)
            .si_then([aligned_end, end, ctx](auto read_pos) {
              auto aligned_offset =
                  aligned_end.template get_byte_distance<extent_len_t>(
                      read_pos.get_key());
              auto len =
                  end.template get_byte_distance<extent_len_t>(aligned_end);
              return read_mapping(
                  ctx, std::move(read_pos), aligned_offset, len, false);
            })
    ...
    extent_len_t destoff)
{
  ...
  ceph_assert(ctx.d_onode);
  DEBUGT(
      "{}->{}, {}~{}", ctx.t, ctx.onode.get_hobj(), ctx.d_onode->get_hobj(),
      srcoff, len);
  ...
  return with_objects_data(
      ctx,
      [ctx, this, srcoff,
       len](auto& object_data, auto& d_object_data) -> clone_ret {
        ...
        if (!dest_mapping) {
          ...
          auto laddr =
              (d_base + srcoff).get_aligned_laddr(ctx.tm.get_block_size());
          dest_mapping =
              co_await ctx.tm.get_containing_pin(ctx.t, laddr)
                  .handle_error_interruptible(
        ...
        co_await read_edge_for_clone_range(ctx, object_data, srcoff, len, data);
        auto base = object_data.get_reserved_data_base();
        auto begin = base + srcoff;
        auto block_size = ctx.tm.get_block_size();
        auto src_mapping =
            co_await ctx.tm
                .get_containing_pin(ctx.t, begin.get_aligned_laddr(block_size))
                .handle_error_interruptible(
                    clone_iertr::pass_further{},
                    crimson::ct_error::assert_all{"unexpected enoent"});
        auto d_base = d_object_data.get_reserved_data_base();
...
ObjectDataHandler::zero_ret
ObjectDataHandler::zero(context_t ctx, objaddr_t offset, extent_len_t len)
{
  return with_object_data(ctx, [this, ctx, offset, len](auto& object_data) {
    LOG_PREFIX(ObjectDataHandler::zero);
    DEBUGT(
        "zero to 0x{:x}~0x{:x}, object_data: {}~0x{:x}, is_null {}", ctx.t,
        offset, len, object_data.get_reserved_data_base(),
        object_data.get_reserved_data_len(), object_data.is_null());
    return prepare_data_reservation(
        ...
        .si_then([this, ctx, offset, len, &object_data](auto mapping) {
          ...
          if (mapping) {
            return overwrite(
                ctx, data_base, offset, len, std::nullopt, std::move(*mapping));
          }
          ...
          return ctx.tm
              ...
              .si_then([this, ctx, data_base, offset, len](auto pin) {
                return overwrite(
                    ctx, data_base, offset, len, std::nullopt, std::move(pin));
              })
              .handle_error_interruptible(
                  write_iertr::pass_further{},
                  crimson::ct_error::assert_all("unexpected enoent"));
...
ObjectDataHandler::write_ret
ObjectDataHandler::write(context_t ctx, objaddr_t offset, const bufferlist& bl)
{
  return with_object_data(ctx, [this, ctx, offset, &bl](auto& object_data) {
    LOG_PREFIX(ObjectDataHandler::write);
    DEBUGT(
        "writing to 0x{:x}~0x{:x}, object_data: {}~0x{:x}, is_null {}", ctx.t,
        offset, bl.length(), object_data.get_reserved_data_base(),
        object_data.get_reserved_data_len(), object_data.is_null());
    return prepare_data_reservation(
               ctx, object_data,
               p2roundup(offset + bl.length(), ctx.tm.get_block_size()))
        .si_then(
            [this, ctx, offset, &object_data, &bl](auto mapping) -> write_ret {
              auto data_base = object_data.get_reserved_data_base();
              if (mapping) {
                return overwrite(
                    ctx, data_base, offset, bl.length(), bufferlist(bl),
                    std::move(*mapping));
              }
              laddr_offset_t l_start = data_base + offset;
              return ctx.tm
    ...
    extent_len_t size)
{
  ...
  DEBUGT(
      "0x{:x}~0x{:x}, 0x{:x}", ctx.t, object_data.get_reserved_data_base(),
  ...
  return ctx.tm
      ...
      .si_then([ctx, data_base, size, this, unaligned_begin,
                &object_data](auto mapping) {
        assert(
            mapping.get_key() <= unaligned_begin &&
            mapping.get_key() + mapping.get_length() > unaligned_begin);
        auto data_len = object_data.get_reserved_data_len();
        return overwrite(
            ctx, data_base, size, data_len - size, std::nullopt,
            std::move(mapping));
      })
      .handle_error_interruptible(
...
ObjectDataHandler::read(context_t ctx, objaddr_t obj_offset, extent_len_t len)
{
  ...
  DEBUGT(
      "reading {}~0x{:x}", ctx.t, object_data.get_reserved_data_base(),
      object_data.get_reserved_data_len());
  ...
  ceph_assert(len > 0);
  laddr_offset_t l_start = object_data.get_reserved_data_base() + obj_offset;
  laddr_offset_t l_end = l_start + len;
  laddr_t aligned_start = l_start.get_aligned_laddr(ctx.tm.get_block_size());
  loffset_t aligned_length = l_end.get_roundup_laddr(ctx.tm.get_block_size())
                                 .get_byte_distance<loffset_t>(aligned_start);
  auto _pins = co_await ctx.tm.get_pins(ctx.t, aligned_start, aligned_length);
  ...
  for (auto& pin : _pins) {
    ...
    if (l_current == l_start) { // first pin may skip head
      ceph_assert(
          l_current.get_aligned_laddr(ctx.tm.get_block_size()) >= pin_start);
      read_start = l_current.template get_byte_distance<extent_len_t>(pin_start);
    ...
    if (pin.is_zero_reserved()) {
      DEBUGT(
          "got {}~0x{:x} from zero-pin {}~0x{:x}", ctx.t, l_current, read_len,
          pin_start, pin_len);
    ...
    DEBUGT(
        "reading {}~0x{:x} from pin {}~0x{:x}", ctx.t, l_current, read_len,
        pin_start, pin_len);
    rpins.emplace_back(
        pin, read_start_aligned, read_len_aligned, unalign_start_offset,
        read_len);
    l_current = l_current_end;
  }
  ...
  for (auto& pin : rpins) {
    ...
    auto aligned_bl =
        maybe_indirect_extent.get_range(pin.partial_off, pin.partial_len);
...
ObjectDataHandler::fiemap_ret
ObjectDataHandler::fiemap(context_t ctx, objaddr_t obj_offset, extent_len_t len)
{
  return seastar::do_with(
      std::map<uint64_t, uint64_t>(), [ctx, obj_offset, len](auto& ret) {
        return with_object_data(
                   ctx,
                   [ctx, obj_offset, len, &ret](const auto& object_data) {
                     LOG_PREFIX(ObjectDataHandler::fiemap);
                     DEBUGT(
                         "0x{:x}~0x{:x}, reservation {}~0x{:x}", ctx.t,
                         obj_offset, len, object_data.get_reserved_data_base(),
                         object_data.get_reserved_data_len());
                     /* Assumption: callers ensure that onode size is <= reserved
       * size and that len is adjusted here prior to call */
                     ceph_assert(!object_data.is_null());
                     ...
                     laddr_offset_t l_end = l_start + len;
                     laddr_t aligned_start =
                         l_start.get_aligned_laddr(ctx.tm.get_block_size());
                     loffset_t aligned_length =
                         l_end.get_roundup_laddr(ctx.tm.get_block_size())
                             .get_byte_distance<loffset_t>(aligned_start);
                     return ctx.tm
                         .get_pins(ctx.t, aligned_start, aligned_length)
                         .si_then([l_start, len, &object_data,
                                   &ret](auto&& pins) {
                           ...
                           for (auto&& i : pins) {
                             if (!i.is_zero_reserved()) {
                               laddr_offset_t ret_left = std::max(
                                   laddr_offset_t(i.get_key(), 0), l_start);
                               laddr_offset_t ret_right = std::min(
                                   i.get_key() + i.get_length(), l_start + len);
                               assert(ret_right > ret_left);
                               if (off == ret_left && !ret.empty()) {
                                 ret.rbegin()->second +=
                                     ret_right.get_byte_distance<uint64_t>(
                                         ret_left);
                               } else {
...
ObjectDataHandler::truncate_ret
ObjectDataHandler::truncate(context_t ctx, objaddr_t offset)
{
  return with_object_data(ctx, [this, ctx, offset](auto& object_data) {
    LOG_PREFIX(ObjectDataHandler::truncate);
    DEBUGT(
        "truncating {}~0x{:x} offset: 0x{:x}", ctx.t,
        object_data.get_reserved_data_base(),
        object_data.get_reserved_data_len(), offset);
    if (offset < object_data.get_reserved_data_len()) {
      ...
      return prepare_data_reservation(
                 ctx, object_data, p2roundup(offset, ctx.tm.get_block_size()))
          .discard_result();
    } else {
...
ObjectDataHandler::clone_ret
ObjectDataHandler::copy_on_write(context_t ctx)
{
  return with_object_data(ctx, [ctx, this](auto& object_data) -> clone_ret {
    auto mapping = co_await ctx.tm
                       .get_pin(ctx.t, object_data.get_reserved_data_base())
                       .handle_error_interruptible(
                           clone_iertr::pass_further{},
                           crimson::ct_error::assert_all{"unexpected enoent"});
    ...
    bool updateref)
{
  ...
  auto old_base = object_data.get_reserved_data_base();
  auto old_len = object_data.get_reserved_data_len();
  auto mapping = co_await prepare_data_reservation(ctx, d_object_data, old_len);
  ceph_assert(mapping.has_value());
  DEBUGT(
      "new obj reserve_data_base: {}, len 0x{:x}", ctx.t,
      d_object_data.get_reserved_data_base(),
      d_object_data.get_reserved_data_len());
  ...
  auto cr_ret = co_await ctx.tm.clone_range(
      ctx.t, old_base, base, 0, len, std::move(pos), std::move(first_mapping),
      updateref);
  if (cr_ret.shared_direct_mapping) {
    ctx.onode.set_need_cow(ctx.t);
  }
...
ObjectDataHandler::clone_ret
ObjectDataHandler::clone(context_t ctx)
{
  return with_objects_data(
      ctx, [ctx, this](auto& object_data, auto& d_object_data) {
        ...
        return ctx.tm.get_pin(ctx.t, object_data.get_reserved_data_base())
            .si_then([this, &object_data, &d_object_data, ctx](auto mapping) {
              ceph_assert(ctx.d_onode);
              return do_clone(
                  ctx, object_data, d_object_data, std::move(mapping), true);
            })
            .handle_error_interruptible(
                clone_iertr::pass_further{},
                crimson::ct_error::assert_all{"unexpected enoent"});
...
template <>
struct fmt::formatter<crimson::os::seastore::overwrite_range_t>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::os::seastore::data_t> : fmt::ostream_formatter {
};

template <>
struct fmt::formatter<crimson::os::seastore::edge_t> : fmt::ostream_formatter {
};
#endif
