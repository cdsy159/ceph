...
public:
  ...
  bool
  has_cached_bptr() const
  {
    return ptr.has_value();
  }

  void
  add(const block_delta_t& b)
  {
    changes.push_back(b);
  }

  void
  apply_changes_to(bufferptr& b) const
  {
  ...
  const bufferptr&
  get_cached_bptr(const bufferptr& _ptr) const
  {
  ...
  bufferptr&
  get_cached_bptr(const bufferptr& _ptr)
  {
    apply_changes_to_cache(_ptr);
    return *ptr;
  }
...
private:
  void
  apply_changes_to_cache(const bufferptr& _ptr) const
  {
    assert(!is_empty());
    if (!has_cached_bptr()) {
...
struct overwrite_range_t {
  ...
  overwrite_range_t(
    ...
    aligned_len(
        aligned_end.template get_byte_distance<extent_len_t>(aligned_begin))
  {}
  ...
  bool
  is_range_in_mapping(const LBAMapping& mapping) const
  {
  ...
  bool
  is_begin_aligned(size_t alignment) const
  {
    return unaligned_begin.is_aligned(alignment);
  }

  bool
  is_end_aligned(size_t alignment) const
  {
    return unaligned_end.is_aligned(alignment);
  }
#ifndef NDEBUG
  bool
  is_begin_in_mapping(const LBAMapping& mapping) const
  {
    return unaligned_begin > mapping.get_key() &&
  ...
  bool
  is_end_in_mapping(const LBAMapping& mapping) const
  {
...
#endif
  void
  expand_begin(laddr_t new_begin)
  {
    assert(new_begin <= aligned_begin);
    unaligned_len +=
        new_begin.template get_byte_distance<extent_len_t>(unaligned_begin);
    aligned_len +=
        new_begin.template get_byte_distance<extent_len_t>(aligned_begin);
    aligned_begin = new_begin;
    unaligned_begin = laddr_offset_t{new_begin};
  }

  void
  expand_end(laddr_t new_end)
  {
    assert(new_end >= aligned_end);
    unaligned_len +=
        new_end.template get_byte_distance<extent_len_t>(unaligned_end);
    aligned_len += new_end.template get_byte_distance<extent_len_t>(aligned_end);
    aligned_end = new_end;
    unaligned_end = laddr_offset_t{new_end};
  }

  void
  shrink_begin(laddr_t new_begin)
  {
    assert(new_begin >= aligned_begin);
    unaligned_len -=
        new_begin.template get_byte_distance<extent_len_t>(unaligned_begin);
...
struct data_t {
  ...
  data_t(std::optional<bufferlist>&& _bl) :
    bl(std::move(_bl))
  {}

  void
  merge_head(extent_len_t block_size)
  {
  ...
  void
  merge_tail(extent_len_t block_size)
  {
...
struct ObjectDataBlock : crimson::os::seastore::LogicalChildNode {
  ...
  void
  overwrite(extent_len_t offset, bufferlist bl)
  {
  ...
  std::optional<modified_region_t>
  get_modified_region() final
  {
    if (modified_region.empty()) {
      return std::nullopt;
    }
    return modified_region_t{
        modified_region.range_start(),
        modified_region.range_end() - modified_region.range_start()};
  }

  void
  clear_modified_region() final
  {
...
private:
  ...
  write_iertr::future<std::optional<LBAMapping>> prepare_data_reservation(
      context_t ctx,
      object_data_t& object_data,
      extent_len_t size);
  ...
  edge_handle_policy_t
  get_edge_handle_policy(
      ...
      op_type_t op_type) const
  {
    ...
    if (op_type == op_type_t::TRIM || op_type == op_type_t::ZERO ||
        len > delta_based_overwrite_max_extent_size ||
        edge_mapping.is_zero_reserved() || edge_mapping.is_indirect()) {
  ...
  base_iertr::future<LBAMapping> punch_mapping_on_edge(
      context_t ctx,
      overwrite_range_t& overwrite_range,
      data_t& data,
      LBAMapping edge_mapping,
...
template <>
struct fmt::formatter<crimson::os::seastore::ObjectDataBlock>
