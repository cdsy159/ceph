...
struct fmt::formatter<ObjectStore::omap_iter_seek_t> {
  ...
  auto
  format(const ObjectStore::omap_iter_seek_t& seek, format_context& ctx) const
  {
    return fmt::format_to(
        ctx.out(), "omap_iter_seek_t{{seek_position='{}', seek_type={}}}",
        seek.seek_position,
        seek.seek_type == ObjectStore::omap_iter_seek_t::LOWER_BOUND
            ? "LOWER_BOUND"
            : "UPPER_BOUND");
  }
};
...
public:
  ...
  using omap_get_value_ret =
      omap_get_value_iertr::future<std::optional<bufferlist>>;
  ...
  using omap_iterate_ret =
      omap_iterate_iertr::future<ObjectStore::omap_iter_ret_t>;
  ...
  struct omap_list_config_t {
    ...
    auto
    with_inclusive(bool first_inclusive, bool last_inclusive)
    {
    ...
    auto
    with_reduced_max(size_t reduced_by) const
    {
      assert(reduced_by <= max_result_size);
      return omap_list_config_t(
          max_result_size - reduced_by, first_inclusive, last_inclusive);
    }
  };

  using omap_list_iertr = base_iertr;
  using omap_list_bare_ret =
      std::tuple<bool, std::map<std::string, bufferlist, std::less<>>>;
