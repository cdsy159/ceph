...
BtreeOMapManager::initialize_omap(Transaction& t, laddr_t hint, omap_type_t type)
{
    ...
    const OMapNode::mutation_result_t& mresult)
{
  ...
  return oc.tm
      .alloc_non_data_extent<OMapInnerNode>(
          oc.t, omap_root.hint, OMAP_INNER_BLOCK_SIZE)
      .si_then(
          [&omap_root, mresult, oc,
           FNAME](auto&& nroot) -> handle_root_split_ret {
            ...
            nroot->insert_child_ptr(
                ...
                dynamic_cast<BaseChildNode<OMapInnerNode, std::string>*>(
                    left.get()));
            nroot->journal_inner_insert(
                nroot->iter_begin(), left->get_laddr(), BEGIN_KEY,
            ...
            nroot->insert_child_ptr(
                ...
                dynamic_cast<BaseChildNode<OMapInnerNode, std::string>*>(
                    right.get()));
            nroot->journal_inner_insert(
                nroot->iter_begin() + 1, right->get_laddr(), pivot,
                nroot->maybe_get_delta_buffer());
            omap_root.update(
                nroot->get_laddr(), omap_root.get_depth() + 1, omap_root.hint,
                omap_root.get_type());
    ...
    OMapNode::mutation_result_t mresult)
{
  ...
  omap_root.update(
      iter->get_val(), omap_root.depth -= 1, omap_root.hint,
      omap_root.get_type());
  oc.t.get_omap_tree_stats().depth = omap_root.depth;
  oc.t.get_omap_tree_stats().extents_num_delta--;
    ...
    const std::string& key)
{
  ...
  DEBUGT("key={}", t, key);
  return get_omap_root(get_omap_context(t, omap_root), omap_root)
      .si_then([this, &t, &key, &omap_root](auto&& extent) {
        return extent->get_value(get_omap_context(t, omap_root), key);
      })
      .si_then([](auto&& e) {
        return omap_get_value_ret(
            interruptible::ready_future_marker{}, std::move(e));
      });
}
    ...
    std::map<std::string, ceph::bufferlist>&& keys)
{
  return seastar::do_with(std::move(keys), [&, this](auto& keys) {
    return trans_intr::do_for_each(keys.begin(), keys.end(), [&, this](auto& p) {
    ...
    const ceph::bufferlist& value)
{
  ...
  return get_omap_root(get_omap_context(t, omap_root), omap_root)
      .si_then([this, &t, &key, &value, &omap_root](auto root) {
        return root->insert(get_omap_context(t, omap_root), key, value);
      })
    ...
    std::set<std::string>& keys)
{
  ...
  return trans_intr::do_for_each(
      keys.begin(), keys.end(), [&t, &omap_root, type, this](auto& p) {
    ...
    const std::string& last)
{
  ...
  while (key_range.total_complete == false) {
    auto root =
        co_await get_omap_root(get_omap_context(t, omap_root), omap_root);
    key_range.root_depth = omap_root.depth;
    auto mresult =
        co_await root->rm_key_range(get_omap_context(t, omap_root), key_range);
    if (mresult.status == mutation_status_t::SUCCESS) {
      continue;
    } else if (mresult.status == mutation_status_t::WAS_SPLIT) {
      co_await handle_root_split(
          get_omap_context(t, omap_root), omap_root, mresult);
    } else if (mresult.status == mutation_status_t::NEED_MERGE) {
      auto root = *(mresult.need_merge);
      if (root->get_node_size() == 1 && omap_root.depth != 1) {
        co_await handle_root_merge(
            get_omap_context(t, omap_root), omap_root, mresult);
      }
    }
  }
  ...
  return get_omap_root(get_omap_context(t, omap_root), omap_root)
      .si_then([this, &t, &start_from, callback, &omap_root](auto extent) {
        return extent->iterate(
            get_omap_context(t, omap_root), start_from, callback);
      });
}
    ...
    omap_list_config_t config)
{
  ...
  return get_omap_root(get_omap_context(t, omap_root), omap_root)
      .si_then([this, config, &t, &first, &last, &omap_root](auto extent) {
        return extent->list(get_omap_context(t, omap_root), first, last, config);
      });
}
...
BtreeOMapManager::omap_clear(omap_root_t& omap_root, Transaction& t)
{
  ...
  return get_omap_root(get_omap_context(t, omap_root), omap_root)
      ...
      .si_then([this, &omap_root, &t] {
        return tm.remove(t, omap_root.get_location())
            .si_then([&omap_root](auto ret) {
              omap_root.update(L_ADDR_NULL, 0, L_ADDR_MIN, omap_root.get_type());
