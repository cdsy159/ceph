...
struct OMapNode : LogicalChildNode {
  ...
  struct mutation_result_t {
    ...
    mutation_result_t(
      ...
      status(s), split_tuple(tuple), need_merge(n_merge)
    {}
  };

  explicit OMapNode(ceph::bufferptr&& ptr) :
    LogicalChildNode(std::move(ptr))
  {}

  explicit OMapNode(extent_len_t length) :
    LogicalChildNode(length)
  {}

  OMapNode(const OMapNode& other) :
    LogicalChildNode(other), root(other.root), begin(other.begin), end(other.end)
  {}
  ...
  using insert_iertr = base_iertr::extend<crimson::ct_error::value_too_large>;
...
template <>
struct fmt::formatter<crimson::os::seastore::omap_manager::OMapNode>
