...
std::ostream&
operator<<(std::ostream& out, const omap_inner_key_t& rhs)
{
...
std::ostream&
operator<<(std::ostream& out, const omap_leaf_key_t& rhs)
{
...
std::ostream&
OMapInnerNode::print_detail_l(std::ostream& out) const
{
  out << ", size=" << get_size() << ", depth=" << get_meta().depth
...
template <typename T>
dec_ref_ret
dec_ref(omap_context_t oc, T&& addr)
{
  return oc.tm.remove(oc.t, std::forward<T>(addr))
    ...
    OMapNodeRef& node)
{
  ...
  return make_split_children(oc).si_then([FNAME, oc, iter = std::move(iter),
                                          key = std::move(key), node,
                                          this](auto tuple) {
    ...
    if (pivot > key) {
      ...
      left->insert_child_ptr(
          liter.get_offset(), dynamic_cast<base_child_t*>(node.get()));
      left->journal_inner_insert(
          liter, node->get_laddr(), key, left->maybe_get_delta_buffer());
    } else { //right
      auto riter = right->iter_idx(iter.get_offset() - left->get_node_size());
      right->insert_child_ptr(
          riter.get_offset(), dynamic_cast<base_child_t*>(node.get()));
      right->journal_inner_insert(
          riter, node->get_laddr(), key, right->maybe_get_delta_buffer());
    }
    ...
    mutation_result_t mresult)
{
  ...
  this->update_child_ptr(
      iter.get_offset(), dynamic_cast<base_child_t*>(left.get()));
  ...
  if (!overflow) {
    this->insert_child_ptr(
        right_iter.get_offset(), dynamic_cast<base_child_t*>(right.get()));
    journal_inner_insert(
        right_iter, right->get_laddr(), pivot, maybe_get_delta_buffer());
    return insert_ret(
        interruptible::ready_future_marker{},
        mutation_result_t(
            mutation_status_t::SUCCESS, std::nullopt, std::nullopt));
  } else {
    return make_split_insert(oc, right_iter, pivot, right)
        .si_then([this, oc](auto m_result) {
          return dec_ref(oc, get_laddr())
              .si_then([m_result = std::move(m_result)] {
                return insert_ret(
                    interruptible::ready_future_marker{}, m_result);
              });
        });
  }
}
...
OMapInnerNode::get_value(omap_context_t oc, const std::string& key)
{
  ...
  return get_child_node(oc, key)
      .si_then([oc, &key](auto extent) {
    ...
    const ceph::bufferlist& value)
{
  ...
  return get_child_node(oc, child_pt)
      .si_then([oc, &key, &value](auto extent) {
        ceph_assert(!extent->is_btree_root());
...
OMapInnerNode::rm_key(omap_context_t oc, const std::string& key)
{
  ...
  auto child_pt = get_containing_child(key);
  return get_child_node(oc, child_pt)
      .si_then([this, oc, &key, child_pt](auto extent) {
        ceph_assert(!extent->is_btree_root());
        return extent->rm_key(oc, key).si_then([this, oc, child_pt,
...
OMapInnerNode::rm_key_range(omap_context_t oc, key_range_t& key_range)
{
  ...
  DEBUGT(
      "key range={}-{}, this: {}", oc.t, key_range.first, key_range.last, *this);
  ...
  DEBUGT(
      "key range after rm_key_range: {}-{}, get_next: {}, total_complete: {}",
      oc.t, key_range.first, key_range.last, key_range.get_next,
      key_range.total_complete);
  switch (mresult.status) {
  case mutation_status_t::SUCCESS:
  case mutation_status_t::FAIL:
    co_return mresult;
    ...
    omap_iterate_cb_t callback)
{
  ...
  return seastar::do_with(
      ...
      [this, &start_from, oc, callback](auto& iter, auto& ret) {
        return trans_intr::repeat(
                   [&start_from, &iter, &ret, oc, callback,
                    this]() -> iterate_iertr::future<seastar::stop_iteration> {
                     if (iter == iter_cend()) {
                       return iterate_iertr::make_ready_future<
                           seastar::stop_iteration>(
                           seastar::stop_iteration::yes);
                     }
                     return get_child_node(oc, iter).si_then(
                         [&start_from, &iter, &ret, callback,
                          oc](auto&& extent) {
    ...
    omap_list_config_t config)
{
  ...
  return seastar::do_with(
      first_iter, last_iter, iter_t(first_iter), list_bare_ret(false, {}),
      [this, &first, &last, oc,
       config](auto& fiter, auto& liter, auto& iter, auto& ret) {
        auto& complete = std::get<0>(ret);
        auto& result = std::get<1>(ret);
        return trans_intr::
            repeat([&, config, oc,
                    this]() -> list_iertr::future<seastar::stop_iteration> {
                 if (iter == liter) {
                   complete = true;
                 ...
                 return get_child_node(oc, iter).si_then([&, config,
                                                          oc](auto&& extent) {
                   ceph_assert(!extent->is_btree_root());
                   return seastar::do_with(
                              iter == fiter
                              ...
                              iter == liter - 1
                                  ? last
                                  : std::optional<std::string>(std::nullopt),
                              [&result, extent = std::move(extent), config,
                               oc](auto& nfirst, auto& nlast) {
                                return extent->list(
                                    oc, nfirst, nlast,
...
OMapInnerNode::clear(omap_context_t oc)
{
  ...
  return trans_intr::do_for_each(iter_begin(), iter_end(), [oc, this](auto iter) {
    ...
    if (ndepth > 1) {
      ...
      assert(ndepth == 1);
      return dec_ref(oc, laddr).si_then([ref = OMapNodeRef(this)] {
...
OMapInnerNode::make_split_children(omap_context_t oc)
{
  ...
  DEBUGT("this: {}", oc.t, *this);
  return oc.tm
      .alloc_extents<OMapInnerNode>(oc.t, oc.hint, OMAP_INNER_BLOCK_SIZE, 2)
...
OMapInnerNode::make_full_merge(omap_context_t oc, OMapNodeRef right)
{
  ...
  DEBUGT("", oc.t);
  return oc.tm
      .alloc_non_data_extent<OMapInnerNode>(oc.t, oc.hint, OMAP_INNER_BLOCK_SIZE)
      .si_then([this, right, oc](auto&& replacement) {
        ...
        return full_merge_ret(
            interruptible::ready_future_marker{}, std::move(replacement));
      })
      .handle_error_interruptible(
          crimson::ct_error::enospc::assert_failure{"unexpected enospc"},
  ...
  return oc.tm
      ...
      .si_then([this, &right, pivot_idx, oc](auto&& replacement_pair) {
        ...
        this->balance_child_ptrs(
            oc.t, *this, right, pivot_idx, *replacement_left,
            *replacement_right);
        return make_balanced_ret(
            ...
            std::make_tuple(
                ...
                balance_into_new_nodes(
                    *this, right, pivot_idx, *replacement_left,
                    *replacement_right)));
      })
      .handle_error_interruptible(
          crimson::ct_error::enospc::assert_failure{"unexpected enospc"},
    ...
    OMapNodeRef r)
{
  ...
  DEBUGT(
      "make_full_merge l {} r {} liter {} riter {}", oc.t, *l, *r,
      liter->get_key(), riter->get_key());
  return l->make_full_merge(oc, r).si_then([liter = liter, riter = riter, l = l,
                                            r = r, oc, this,
                                            FNAME](auto&& replacement) {
    DEBUGT("to update parent: {}", oc.t, *this);
    this->update_child_ptr(
        liter.get_offset(), dynamic_cast<base_child_t*>(replacement.get()));
    journal_inner_update(
        liter, replacement->get_laddr(), maybe_get_delta_buffer());
    ...
    if (get_meta().depth > 2) { // replacement is an inner node
      ...
      rep.adjust_copy_src_dest_on_merge(
          oc.t, *l->template cast<OMapInnerNode>(),
        ...
        .si_then([this, oc, r = std::move(replacement)] {
          ...
          if (extent_is_below_min()) {
            return merge_entry_ret(
                interruptible::ready_future_marker{},
                mutation_result_t(
                    mutation_status_t::NEED_MERGE, std::nullopt, this));
          } else {
            return merge_entry_ret(
                interruptible::ready_future_marker{},
                mutation_result_t(
    ...
    OMapNodeRef r)
{
  ...
  DEBUGT(
      "balanced l {} r {} liter {} riter {}", oc.t, *l, *r, liter->get_key(),
      riter->get_key());
  return l->make_balanced(oc, r, *pivot_idx)
      .si_then([FNAME, pivot_idx = *pivot_idx, liter = liter, riter = riter,
        ...
        DEBUGT(
            "to update parent: {} {} {}", oc.t, *this, *replacement_l,
            *replacement_r);
        ...
        journal_inner_update(
            liter, replacement_l->get_laddr(), maybe_get_delta_buffer());
        bool overflow =
            extent_will_overflow(replacement_pivot.size(), std::nullopt);
        if (!overflow) {
          this->update_child_ptr(
              riter.get_offset(),
          ...
          journal_inner_remove(riter, maybe_get_delta_buffer());
          journal_inner_insert(
              riter, replacement_r->get_laddr(), replacement_pivot,
    ...
    OMapNodeRef entry)
{
  ...
  return get_child_node(oc, donor_iter).si_then([=, this](auto&& donor) mutable {
    ceph_assert(!donor->is_btree_root());
    auto [l, r] = is_left ? std::make_pair(donor, entry)
                          : std::make_pair(entry, donor);
    auto [liter, riter] = is_left ? std::make_pair(donor_iter, iter)
...
std::ostream&
OMapLeafNode::print_detail_l(std::ostream& out) const
{
  out << ", size=" << get_size() << ", depth=" << get_meta().depth
      << ", is_root=" << is_btree_root();
  if (get_size() > 0) {
  ...
  if (this->child_node_t::is_parent_valid())
    return out << ", parent="
               << (void*)this->child_node_t::peek_parent_node().get();
  else
    return out;
}
    ...
    return make_split_children(oc).si_then([this, oc, &key, &value](auto tuple) {
      ...
      if (replace_pt != iter_end()) {
        ...
        if (key < pivot) { //left
          ...
          left->journal_leaf_update(
              mut_iter, key, value, left->maybe_get_delta_buffer());
        } else if (key >= pivot) { //right
          auto mut_iter =
              right->iter_idx(replace_pt->get_offset() - left->get_node_size());
          right->journal_leaf_update(
              mut_iter, key, value, right->maybe_get_delta_buffer());
        }
      } else {
        ++(oc.t.get_omap_tree_stats().num_inserts);
        auto insert_pt = string_lower_bound(key);
        if (key < pivot) { //left
          ...
          left->journal_leaf_insert(
              mut_iter, key, value, left->maybe_get_delta_buffer());
        } else {
          auto mut_iter =
              right->iter_idx(insert_pt->get_offset() - left->get_node_size());
          right->journal_leaf_insert(
              mut_iter, key, value, right->maybe_get_delta_buffer());
        }
      }
      ++(oc.t.get_omap_tree_stats().extents_num_delta);
      return dec_ref(oc, get_laddr()).si_then([tuple = std::move(tuple)] {
        return insert_ret(
            interruptible::ready_future_marker{},
            mutation_result_t(
                mutation_status_t::WAS_SPLIT, tuple, std::nullopt));
      });
    });
  }
}
  ...
  if (rm_pt != iter_end()) {
    ...
    if (extent_is_below_min()) {
      return rm_key_ret(
          interruptible::ready_future_marker{},
          mutation_result_t(
              mutation_status_t::NEED_MERGE, std::nullopt,
              this->cast<OMapNode>()));
    } else {
      return rm_key_ret(
          interruptible::ready_future_marker{},
...
OMapLeafNode::rm_key_range(omap_context_t oc, key_range_t& key_range)
{
  LOG_PREFIX(OMapLeafNode::rm_key_range);
  DEBUGT(
      "remove kyes: {}--{}, this: {}", oc.t, key_range.first, key_range.last,
      *this);

  auto fiter = key_range.first.empty()
                   ? iter_begin()
                   : string_lower_bound(key_range.first); //include the first key
  auto liter =
      key_range.last.empty()
          ? iter_end()
          : string_lower_bound(key_range.last); //not include the last key

  DEBUGT(
      "list from {} to {}, begin off {} to end off {}, this: {}", oc.t,
      fiter != iter_end() ? fiter->get_key() : "<end>",
      liter != iter_end() ? liter->get_key() : "<end>",
      fiter != iter_end() ? fiter->get_offset() : get_node_size(),
      liter != iter_end() ? liter->get_offset() : get_node_size(), *this);
  if (fiter == iter_end() ||
      fiter == liter) { //out of range or needn't remove anything
    key_range.total_complete = true;
    ...
    omap_iterate_cb_t callback)
{
  ...
  return iterate_iertr::make_ready_future<ObjectStore::omap_iter_ret_t>(
      std::move(ret));
}
    ...
    omap_list_config_t config)
{
  ...
  DEBUGT(
    ...
    last_inclusive {}, this: {}",
      oc.t, first ? first->c_str() : "", last ? last->c_str() : "",
      config.max_result_size, config.first_inclusive, config.last_inclusive,
      *this);
  auto ret = list_bare_ret(false, {});
  auto& [complete, result] = ret;
  auto iter = first ? (config.first_inclusive ? string_lower_bound(*first)
                                              : string_upper_bound(*first))
                    : iter_begin();
  auto liter = last ? (config.last_inclusive ? string_upper_bound(*last)
    ...
    internal_const_iterator_t child_pt)
{
  ...
  if (get_meta().depth == 2) {
    ...
    if (ret.has_child()) {
      return ret.get_child_fut().si_then([](auto extent) {
    ...
    return omap_load_extent<OMapLeafNode>(
               oc, laddr, get_meta().depth - 1, child_pt->get_key(),
               next == iter_cend() ? this->get_end() : next->get_key(),
               std::move(child_pos))
        .si_then([](auto extent) { return extent->template cast<OMapNode>(); });
  } else {
...
extent_len_t
get_leaf_size(omap_type_t type)
{
  if (type == omap_type_t::LOG) {
    return LOG_LEAF_BLOCK_SIZE;
  }
  ceph_assert(type == omap_type_t::OMAP || type == omap_type_t::XATTR);
