...
struct OMapInnerNode : OMapNode,
  ...
  explicit OMapInnerNode(extent_len_t length) :
    OMapNode(length), StringKVInnerNodeLayout(nullptr), parent_node_t(0)
  {}

  OMapInnerNode(const OMapInnerNode& rhs) :
    OMapNode(rhs), StringKVInnerNodeLayout(get_bptr().c_str()), parent_node_t(0)
  {
    this->parent_node_t::sync_children_capacity();
  }
  ...
  void
  do_on_rewrite(Transaction& t, LogicalCachedExtent& extent) final
  {
  ...
  void
  do_on_replace_prior(Transaction& t) final
  {
    if (is_rewrite_transaction(t.get_src())) {
      return;
    }
  ...
  void
  lcn_on_invalidated(Transaction& t) final
  {
  ...
  bool
  extent_will_overflow(size_t ksize, std::optional<size_t> vsize) const
  {
    return is_overflow(ksize);
  }

  bool
  can_merge(OMapNodeRef right) const
  {
    return !is_overflow(*right->cast<OMapInnerNode>());
  }

  bool
  extent_is_below_min() const
  {
    return below_min();
  }
  ...
  rm_key_range_ret rm_key_range(omap_context_t oc, key_range_t& key_range) final;
  ...
  using make_split_insert_ret =
      make_split_insert_iertr::future<mutation_result_t>;
  ...
  internal_iterator_t
  lower_bound(const std::string& key)
  {
    return string_lower_bound(key);
  }

  internal_iterator_t
  upper_bound(const std::string& key)
  {
    return string_upper_bound(key);
  }

  bool
  is_in_range(const std::string& key) const
  {
...
private:
  ...
  get_child_node_ret
  get_child_node(omap_context_t oc, const std::string& key)
  {
...
struct OMapLeafNode : OMapNode,
  ...
  void
  apply_delta(const ceph::bufferlist& _bl) final
  {
    assert(_bl.length());
    ...
    std::optional<child_pos_t<OMapInnerNode>> chp = std::nullopt)
{
  ...
  assert(end <= END_KEY);
  auto size = std::is_same_v<OMapInnerNode, T> ? OMAP_INNER_BLOCK_SIZE
                                               : get_leaf_size(oc.type);
  return oc.tm
      .read_extent<T>(
          oc.t, laddr, size,
          [begin = std::move(begin), end = std::move(end), FNAME, oc,
           chp = std::move(chp)](T& extent) mutable {
            ...
            SUBDEBUGT(
                seastore_omap, "linking {} to {}", oc.t, extent,
                *chp->get_parent());
            assert(chp);
...
template <>
struct fmt::formatter<crimson::os::seastore::omap_manager::OMapInnerNode>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::os::seastore::omap_manager::OMapLeafNode>
  : fmt::ostream_formatter {};
#endif
