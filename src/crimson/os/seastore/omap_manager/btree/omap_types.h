...
struct omap_node_meta_t {
  ...
  std::pair<omap_node_meta_t, omap_node_meta_t>
  split_into() const
  {
    return std::make_pair(omap_node_meta_t{depth}, omap_node_meta_t{depth});
  }

  static omap_node_meta_t
  merge_from(const omap_node_meta_t& lhs, const omap_node_meta_t& rhs)
  {
  ...
  rebalance(const omap_node_meta_t& lhs, const omap_node_meta_t& rhs)
  {
    ...
    return std::make_pair(
        omap_node_meta_t{lhs.depth}, omap_node_meta_t{lhs.depth});
  }
};
...
struct omap_inner_key_t {
  ...
  inline bool
  operator==(const omap_inner_key_t b) const
  {
...
struct omap_inner_key_le_t {
  ...
  explicit omap_inner_key_le_t(const omap_inner_key_t& key) :
    key_off(key.key_off), key_len(key.key_len), laddr(key.laddr)
  {}

  operator omap_inner_key_t() const
  {
    return omap_inner_key_t{
        uint16_t(key_off), uint16_t(key_len), laddr_t(laddr)};
  }

  omap_inner_key_le_t&
  operator=(omap_inner_key_t key)
  {
    key_off = key.key_off;
    key_len = key.key_len;
  ...
  inline bool
  operator==(const omap_inner_key_le_t b) const
  {
    return key_off == b.key_off && key_len == b.key_len && laddr == b.laddr;
  }
};
...
struct omap_leaf_key_t {
  ...
  inline bool
  operator==(const omap_leaf_key_t b) const
  {
...
struct omap_leaf_key_le_t {
  ...
  explicit omap_leaf_key_le_t(const omap_leaf_key_t& key) :
    key_off(key.key_off), key_len(key.key_len), val_len(key.val_len)
  {}

  operator omap_leaf_key_t() const
  {
    return omap_leaf_key_t{
        uint16_t(key_off), uint16_t(key_len), uint16_t(val_len)};
  }

  omap_leaf_key_le_t&
  operator=(omap_leaf_key_t key)
  {
    key_off = key.key_off;
    key_len = key.key_len;
    val_len = key.val_len;
  ...
  inline bool
  operator==(const omap_leaf_key_le_t b) const
  {
    return key_off == b.key_off && key_len == b.key_len && val_len == b.val_len;
  }
};
