...
static void
copy_from_foreign(iterator tgt, const_iterator from_src, const_iterator to_src)
{
  ...
  assert(to_copy > 0);
  memcpy(
      tgt->get_right_ptr_end() - to_copy, to_src->get_right_ptr_end(), to_copy);
  memcpy(
      tgt->get_node_key_ptr(), from_src->get_node_key_ptr(),
      to_src->get_node_key_ptr() - from_src->get_node_key_ptr());

  auto offset_diff = tgt->get_right_offset_end() -
                     from_src->get_right_offset_end();
...
static void
copy_from_local(unsigned len, iterator tgt, iterator from_src, iterator to_src)
{
  assert(tgt->node == from_src->node);
  assert(to_src->node == from_src->node);
  ...
  memmove(
      to_src->get_right_ptr_end() + adjust_offset, to_src->get_right_ptr_end(),
      to_copy);

  if (from_src >
      tgt) { //keep same content for rm_key and rm_keyrange in case replay has crc error
    memset(end, 0, len);
  }
...
struct delta_inner_t {
  ...
  bool
  operator==(const delta_inner_t& rhs) const
  {
    return op == rhs.op && key == rhs.key && addr == rhs.addr;
  }
};
} // namespace crimson::os::seastore::omap_manager
...
struct delta_leaf_t {
  ...
  bool
  operator==(const delta_leaf_t& rhs) const
  {
...
public:
  ...
  void
  insert(const std::string& key, laddr_t addr)
  {
    buffer.push_back(delta_inner_t{delta_inner_t::op_t::INSERT, key, addr});
  }

  void
  update(const std::string& key, laddr_t addr)
  {
  ...
  {
    buffer.push_back(
        delta_inner_t{delta_inner_t::op_t::REMOVE, key, L_ADDR_NULL});
  }

  void
  replay(StringKVInnerNodeLayout& node)
  {
    for (auto& i : buffer) {
      i.replay(node);
    }
  ...
  bool
  operator==(const delta_inner_buffer_t& rhs) const
  {
...
public:
  ...
  void
  insert(const std::string& key, const ceph::bufferlist& val)
  {
    buffer.push_back(delta_leaf_t{delta_leaf_t::op_t::INSERT, key, val});
  }

  void
  update(const std::string& key, const ceph::bufferlist& val)
  {
  ...
  {
    buffer.push_back(
        delta_leaf_t{delta_leaf_t::op_t::REMOVE, key, bufferlist()});
  }

  void
  replay(StringKVLeafNodeLayout& node)
  {
    for (auto& i : buffer) {
      i.replay(node);
    }
  ...
  bool
  operator==(const delta_leaf_buffer_t& rhs) const
  {
    return buffer == rhs.buffer;
  }
};
...
class StringKVInnerNodeLayout {
  ...
  using L = absl::container_internal::
      Layout<ceph_le32, omap_node_meta_le_t, omap_inner_key_le_t>;
...
public:
  ...
  class iter_t {
    ...
    using parent_t = typename crimson::common::
        maybe_const_t<StringKVInnerNodeLayout, is_const>::type;
  ...
  public:
    ...
    friend bool
    operator==(const iter_t& lhs, const iter_t& rhs)
    {
  ...
  private:
    ...
    void
    set_node_key(omap_inner_key_t _lb)
    {
      static_assert(!is_const);
    ...
    void
    set_node_val(const std::string& str)
    {
      static_assert(!is_const);
...
public:
  ...
  void
  journal_inner_remove(const_iterator _iter, delta_inner_buffer_t* recorder)
  {
    auto iter = iterator(this, _iter.index);
    if (recorder) {
  ...
  const_iterator
  iter_idx(uint16_t off) const
  {
    return const_iterator(this, off);
  }

  const_iterator
  string_lower_bound(std::string_view str) const
  {
    auto it = std::lower_bound(
        boost::make_counting_iterator<uint16_t>(0),
        boost::make_counting_iterator<uint16_t>(get_size()), str,
  ...
  const_iterator
  string_upper_bound(std::string_view str) const
  {
  ...
  const_iterator
  find_string_key(std::string_view str) const
  {
  ...
  {
    uint32_t total_size =
        omap_inner_key_t(get_node_key_ptr()[get_size() - 1]).key_off;
  ...
  void
  set_meta(const omap_node_meta_t& meta)
  {
  ...
  used_space() const
  {
    ...
    if (count) {
      omap_inner_key_t last_key =
          omap_inner_key_t(get_node_key_ptr()[count - 1]);
  ...
  bool
  is_overflow(size_t ksize) const
  {
    return free_space() < (sizeof(omap_inner_key_le_t) + ksize);
  }

  bool
  is_overflow(const StringKVInnerNodeLayout& rhs) const
  {
  ...
  bool
  operator==(const StringKVInnerNodeLayout& rhs) const
  {
   ...
   */
  std::string
  split_into(StringKVInnerNodeLayout& left, StringKVInnerNodeLayout& right) const
  {
  ...
  {
    uint32_t left_size =
        omap_inner_key_t(left.get_node_key_ptr()[left.get_size() - 1]).key_off;
    uint32_t right_size =
        omap_inner_key_t(right.get_node_key_ptr()[right.get_size() - 1]).key_off;
      ...
      StringKVInnerNodeLayout& replacement_right)
  {
    ...
    auto replacement_pivot =
        pivot_idx >= left.get_size()
            ? right.iter_idx(pivot_idx - left.get_size())->get_key()
            : left.iter_idx(pivot_idx)->get_key();

    if (pivot_size < left_size) {
      copy_from_foreign(
          replacement_left.iter_end(), left.iter_begin(),
          left.iter_idx(pivot_idx));
      replacement_left.set_size(pivot_idx);

      copy_from_foreign(
          replacement_right.iter_end(), left.iter_idx(pivot_idx),
          left.iter_end());
      replacement_right.set_size(left.get_size() - pivot_idx);

      copy_from_foreign(
          replacement_right.iter_end(), right.iter_begin(), right.iter_end());
      replacement_right.set_size(right.get_size() + left.get_size() - pivot_idx);
    } else {
      copy_from_foreign(
          replacement_left.iter_end(), left.iter_begin(), left.iter_end());
      replacement_left.set_size(left.get_size());

      copy_from_foreign(
          replacement_left.iter_end(), right.iter_begin(),
          right.iter_idx(pivot_idx - left.get_size()));
      replacement_left.set_size(pivot_idx);

      copy_from_foreign(
          replacement_right.iter_end(),
          right.iter_idx(pivot_idx - left.get_size()), right.iter_end());
      replacement_right.set_size(right.get_size() + left.get_size() - pivot_idx);
    }

    auto [lmeta, rmeta] =
        omap_node_meta_t::rebalance(left.get_meta(), right.get_meta());
...
private:
  void
  inner_insert(iterator iter, const std::string& key, laddr_t val)
  {
    if (iter != iter_begin()) {
  ...
  void
  inner_update(iterator iter, laddr_t addr)
  {
...
class StringKVLeafNodeLayout {
  ...
  using L = absl::container_internal::
      Layout<ceph_le32, omap_node_meta_le_t, omap_leaf_key_le_t>;
  static constexpr L layout{1, 1, 1}; // = L::Partial(1, 1, 1);
...
public:
  ...
  class iter_t {
    ...
    using parent_t = typename crimson::common::
        maybe_const_t<StringKVLeafNodeLayout, is_const>::type;
  ...
  private:
    ...
    void
    set_node_key(omap_leaf_key_t _lb) const
    {
      static_assert(!is_const);
  ...
  public:
    ...
    get_str_val() const
    {
      auto node_key = get_node_key();
      return std::string(
          get_node_val_ptr() + node_key.key_len, get_node_key().val_len);
    }
...
public:
  ...
  void
  journal_leaf_remove(const_iterator _iter, delta_leaf_buffer_t* recorder)
  {
    auto iter = iterator(this, _iter.index);
    if (recorder) {
      recorder->remove(iter->get_key());
  ...
  capacity() const
  {
    return len - (reinterpret_cast<char*>(layout.template Pointer<2>(buf)) -
                  reinterpret_cast<char*>(layout.template Pointer<0>(buf)));
  }
  ...
  bool
  is_overflow(size_t ksize, size_t vsize) const
  {
    return free_space() < (sizeof(omap_leaf_key_le_t) + ksize + vsize);
  }

  bool
  is_overflow(const StringKVLeafNodeLayout& rhs) const
  {
    return free_space() < rhs.used_space();
  }
  ...
  bool
  operator==(const StringKVLeafNodeLayout& rhs) const
  {
    if (get_size() != rhs.get_size()) {
      return false;
    }
   ...
   */
  std::string
  split_into(StringKVLeafNodeLayout& left, StringKVLeafNodeLayout& right) const
  {
    auto piviter = get_split_pivot();
    assert(piviter != iter_end());
  ...
  {
    uint32_t left_size =
        omap_leaf_key_t(left.get_node_key_ptr()[left.get_size() - 1]).key_off;
    uint32_t right_size =
        omap_leaf_key_t(right.get_node_key_ptr()[right.get_size() - 1]).key_off;
    uint32_t total = left_size + right_size;
    uint32_t pivot_size = total / 2;
    uint32_t pivot_idx = 0;
    if (pivot_size < left_size) {
...
private:
  void
  leaf_insert(iterator iter, const std::string& key, const bufferlist& val)
  {
    if (iter != iter_begin()) {
      assert((iter - 1)->get_key() < key);
    }
    ...
    if (get_size() != 0 && iter != iter_end())
      copy_from_local(
          node_key.key_len + node_key.val_len, iter + 1, iter, iter_end());
  ...
  void
  leaf_update(iterator iter, const std::string& key, const ceph::bufferlist& val)
  {
    assert(iter != iter_end());
    leaf_remove(iter);
    assert(!is_overflow(key.size(), val.length()));
  ...
  void
  leaf_remove_range(iterator fiter, iterator liter)
  {
...
inline void
delta_inner_t::replay(StringKVInnerNodeLayout& l)
{
  switch (op) {
  case op_t::INSERT: {
...
inline void
delta_leaf_t::replay(StringKVLeafNodeLayout& l)
{
  switch (op) {
  case op_t::INSERT: {
