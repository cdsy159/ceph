...
template <typename T>
const void*
ptr(const ::boost::intrusive_ptr<T>& p)
{
  return p.get();
}
} // namespace fmt
...
void
tree_cursor_t::assert_next_to(const tree_cursor_t& prv, value_magic_t magic) const
{
#ifndef NDEBUG
  assert(!prv.is_end());
  if (is_end()) {
...
template eagain_ifuture<Ref<tree_cursor_t>> tree_cursor_t::erase<true>(
    context_t,
    bool);
template eagain_ifuture<Ref<tree_cursor_t>> tree_cursor_t::erase<false>(
    context_t,
    bool);

std::strong_ordering
tree_cursor_t::compare_to(const tree_cursor_t& o, value_magic_t magic) const
{
...
void
tree_cursor_t::update_track(Ref<LeafNode> node, const search_position_t& pos)
{
...
template void tree_cursor_t::update_track<true>(
    Ref<LeafNode>,
    const search_position_t&);
template void tree_cursor_t::update_track<false>(
    Ref<LeafNode>,
    const search_position_t&);

void
tree_cursor_t::update_cache_same_node(
    const key_view_t& key_view,
...
void
tree_cursor_t::Cache::update_all(
    const node_version_t& current_version,
    ...
    const value_header_t* _p_value_header)
{
  ...
  assert(
      (const char*)p_value_header - p_node_base <
...
void
tree_cursor_t::Cache::maybe_duplicate(const node_version_t& current_version)
{
...
void
tree_cursor_t::Cache::make_latest(
    value_magic_t magic,
    const search_position_t& pos)
{
...
void
tree_cursor_t::Cache::validate_is_latest(const search_position_t& pos) const
{
#ifndef NDEBUG
  assert(!needs_update_all);
...
{
  return seastar::do_with(MatchHistory(), [this, c, &key](auto& history) {
    return lower_bound_tracked(c, key, history);
  });
}

eagain_ifuture<std::pair<Ref<tree_cursor_t>, bool>>
Node::insert(
    context_t c,
        ...
        return lower_bound_tracked(c, key, history)
            .si_then([c, &key, vconf, &history,
              ...
              if (result.match() == MatchKindBS::EQ) {
                ...
                return leaf_node
                    ->insert_value(
                        c, key, vconf, result.p_cursor->get_position(), history,
                        result.mstat)
...
Node::erase(context_t c, const key_hobj_t& key, Ref<Node>&& this_ref)
{
  return lower_bound(c, key).si_then([c, this_ref = std::move(this_ref)](
                                         auto result) mutable {
    // the cursor in the result should already hold the root node upwards
    this_ref.reset();
    if (result.match() != MatchKindBS::EQ) {
...
eagain_ifuture<tree_stats_t>
Node::get_tree_stats(context_t c)
{
...
std::ostream&
Node::dump(std::ostream& os) const
{
  return impl->dump(os);
}

std::ostream&
Node::dump_brief(std::ostream& os) const
{
...
eagain_ifuture<>
Node::mkfs(context_t c, RootNodeTracker& root_tracker)
{
  ...
  return c.nm.get_super(c.t, root_tracker)
      .handle_error_interruptible(
          ...
          crimson::ct_error::input_output_error::assert_failure(
              fmt::format("{} EIO during get_super()", FNAME).c_str()))
      .si_then([c, &root_tracker, FNAME](auto&& _super) {
        ...
        return Node::load(c, root_addr, true)
            .si_then([c, _super = std::move(_super), &root_tracker,
                      FNAME](auto root) mutable {
...
void
Node::make_root(context_t c, Super::URef&& _super)
{
...
void
Node::as_root(Super::URef&& _super)
{
  assert(!is_tracked());
...
eagain_ifuture<>
Node::upgrade_root(context_t c, laddr_t hint)
{
...
template <bool VALIDATE>
void
Node::as_child(const search_position_t& pos, Ref<InternalNode> parent_node)
{
    ...
    Ref<Node>&& this_ref)
{
  ...
  return parent_info()
      ...
      .si_then(
          [c, this_ref = std::move(this_ref), this, FNAME,
           update_parent_index](auto lr_nodes) mutable -> eagain_ifuture<> {
            ...
            if (left_for_merge) {
              ...
              auto [merge_stage, merge_size] =
                  left_for_merge->impl->evaluate_merge(*right_for_merge->impl);
              if (merge_size <= left_for_merge->impl->total_size()) {
                ...
                DEBUGT(
                    "merge {} and {} at merge_stage={}, merge_size={}B, "
                    "update_index={}, is_left={} ...",
                    c.t, left_for_merge->get_name(),
                    right_for_merge->get_name(), merge_stage, merge_size,
                    update_index_after_merge, is_left);
                ...
                return left_for_merge->rebuild_extent(c).si_then(
                    [c, update_index_after_merge, left_addr,
                     merge_stage = merge_stage, merge_size = merge_size,
                     left_for_merge = std::move(left_for_merge),
                     right_for_merge = std::move(right_for_merge)](
                        auto left_mut) mutable {
                      if (left_for_merge->impl->node_type() ==
                          node_type_t::LEAF) {
                        auto& left =
                            *static_cast<LeafNode*>(left_for_merge.get());
                        left.on_layout_change();
                      }
                      search_position_t left_last_pos =
                          left_for_merge->impl->merge(
                              left_mut, *right_for_merge->impl, merge_stage,
                              merge_size);
                      left_for_merge->track_merge(
                          right_for_merge, merge_stage, left_last_pos);
                      --(c.t.get_onode_tree_stats().extents_num_delta);
                      return left_for_merge->parent_info()
                          .ptr->apply_children_merge(
...
template eagain_ifuture<> Node::try_merge_adjacent<true>(
    context_t,
    bool,
    Ref<Node>&&);
template eagain_ifuture<> Node::try_merge_adjacent<false>(
    context_t,
    bool,
    Ref<Node>&&);

eagain_ifuture<>
Node::erase_node(context_t c, Ref<Node>&& this_ref)
{
  // To erase a node:
...
template <bool FORCE_MERGE>
eagain_ifuture<>
Node::fix_parent_index(context_t c, Ref<Node>&& this_ref, bool check_downgrade)
{
  assert(!is_root());
  assert(this == this_ref.get());
...
template eagain_ifuture<> Node::fix_parent_index<true>(
    context_t,
    Ref<Node>&&,
    bool);
template eagain_ifuture<> Node::fix_parent_index<false>(
    context_t,
    Ref<Node>&&,
    bool);
  ...
  return c.nm.read_extent(c.t, addr)
      .handle_error_interruptible(
          ...
          crimson::ct_error::assert_all(fmt::format(
                                            "{} -- addr={}, is_level_tail={}",
                                            FNAME, addr, expect_is_level_tail)
                                            .c_str()))
      .si_then(
          [FNAME, c, addr,
           expect_is_level_tail](auto extent) -> eagain_ifuture<Ref<Node>> {
            assert(extent);
            auto header = extent->get_header();
            auto field_type = header.get_field_type();
            if (!field_type) {
              ERRORT(
                  "load addr={}, is_level_tail={} error, "
                  "got invalid header -- {}",
...
Node::rebuild_extent(context_t c)
{
...
eagain_ifuture<>
Node::retire(context_t c, Ref<Node>&& this_ref)
{
  ...
  return impl->retire_extent(c).si_then(
      [this_ref = std::move(this_ref)] { /* deallocate node */ });
}

void
Node::make_tail(context_t c)
{
...
InternalNode::get_next_cursor(context_t c, const search_position_t& pos)
{
  ...
  if (next_pos.is_end() && !impl->is_level_tail()) {
    ...
    return get_or_track_child(c, next_pos, p_child_addr->value)
        .si_then([c](auto child) { return child->lookup_smallest(c); });
  }
}

eagain_ifuture<>
InternalNode::apply_child_split(
    context_t c,
    Ref<Node>&& left_child,
    ...
    bool update_right_index)
{
  ...
  DEBUGT(
      "apply {}'s child {} to split to {}, update_index={} ...", c.t,
      get_name(), left_child->get_name(), right_child->get_name(),
      update_right_index);
...
eagain_ifuture<>
InternalNode::erase_child(context_t c, Ref<Node>&& child_ref)
{
  ...
  bool fix_tail =
      (child_ref->parent_info().position.is_end() && !impl->is_keys_empty());
  return eagain_iertr::now()
      ...
      .si_then([c, this, child_ref = std::move(child_ref),
                FNAME](auto&& new_tail_child) mutable {
        auto child_pos = child_ref->parent_info().position;
        if (new_tail_child) {
          DEBUGT(
              "erase {}'s child {} at pos({}), "
          ...
          DEBUGT(
              "erase {}'s child {} at pos({}) ...", c.t, get_name(),
              child_ref->get_name(), child_pos);
        }
            ...
            .si_then([c, this, child_pos, FNAME,
              ...
              if (impl->has_single_value()) {
                ...
                if (is_root()) {
                  ...
                  ceph_abort_msg(
                      "trying to erase the last item from the internal root "
                      "node");
                }
...
template <bool FORCE_MERGE>
eagain_ifuture<>
InternalNode::fix_index(context_t c, Ref<Node>&& child, bool check_downgrade)
{
  ...
  DEBUGT(
      "fix {}'s index of child {} at pos({}), new_key={} ...", c.t, get_name(),
      child->get_name(), child_pos, new_key);
  ...
  return insert_or_split(c, next_pos, new_key, child)
      .si_then([this, c, update_parent_index, check_downgrade,
        ...
        if (split_right) {
          ...
          return apply_split_to_parent(
              c, std::move(this_ref), std::move(split_right),
              update_parent_index);
        } else {
          // no split path
          if (is_root()) {
...
template <bool FORCE_MERGE>
eagain_ifuture<>
InternalNode::apply_children_merge(
    context_t c,
    Ref<Node>&& left_child,
    laddr_t origin_left_addr,
    Ref<Node>&& right_child,
    bool update_index)
{
  ...
  return right_child->retire(c, std::move(right_child))
      .si_then([c, this, update_index,
        ...
        if (update_index) {
          ...
          return left_child->fix_parent_index<FORCE_MERGE>(
              c, std::move(left_child), true);
        } else {
          validate_tracked_children();
          Ref<Node> this_ref = this;
          left_child.reset();
...
eagain_ifuture<std::pair<Ref<Node>, Ref<Node>>>
InternalNode::get_child_peers(context_t c, const search_position_t& pos)
{
  // assume I'm already ref counted by caller
        ...
        if (next_p_child_addr != nullptr) {
          return get_or_track_child(c, next_pos, next_p_child_addr->value)
              .si_then([lnode](Ref<Node> rnode) {
                return seastar::make_ready_future<
                    std::pair<Ref<Node>, Ref<Node>>>(lnode, rnode);
              });
        } else {
...
eagain_ifuture<Ref<InternalNode>>
InternalNode::allocate_root(
    context_t c,
    laddr_t hint,
    level_t old_root_level,
    laddr_t old_root_addr,
    Super::URef&& super)
{
  // support tree height up to 256
  ceph_assert(old_root_level < MAX_LEVEL);
  return InternalNode::allocate(
             c, hint, field_type_t::N0, true, old_root_level + 1)
...
InternalNode::lookup_largest(context_t c)
{
  ...
  return get_or_track_child(c, search_position_t::end(), p_child_addr->value)
      .si_then([c](auto child) { return child->lookup_largest(c); });
}
...
eagain_ifuture<>
InternalNode::do_get_tree_stats(context_t c, tree_stats_t& stats)
{
  impl->validate_non_empty();
  ...
  return seastar::do_with(
      ...
      [this, this_ref, c, &stats](auto& pos, auto& p_child_addr) {
        ...
        return trans_intr::repeat(
            [this, this_ref, c, &stats, &pos,
             &p_child_addr]() -> eagain_ifuture<seastar::stop_iteration> {
...
void
InternalNode::track_merge(
    Ref<Node> _right_node,
    match_stage_t stage,
    search_position_t& left_last_pos)
{
...
eagain_ifuture<>
InternalNode::test_clone_root(
    context_t c_other,
    RootNodeTracker& tracker_other) const
{
  assert(is_root());
  assert(impl->is_level_tail());
  assert(impl->field_type() == field_type_t::N0);
  ...
  return InternalNode::allocate(
             c_other, L_ADDR_MIN, field_type_t::N0, true, impl->level())
        ...
        return c_other.nm.get_super(c_other.t, tracker_other)
            .handle_error_interruptible(
                ...
                crimson::ct_error::assert_all{
                    "Invalid error during test clone"})
      ...
      .si_then([this_ref, this, c_other](auto cloned_root) {
        ...
        return trans_intr::do_for_each(
            tracked_child_nodes.begin(), tracked_child_nodes.end(),
...
eagain_ifuture<>
InternalNode::try_downgrade_root(context_t c, Ref<Node>&& this_ref)
{
    ...
    Ref<Node> outdated_child)
{
  ...
  auto insert_pos = pos;
  DEBUGT(
      "insert {} with insert_key={}, insert_child={}, insert_pos({}), "
      "outdated_child={} ...",
      c.t, get_name(), insert_key, insert_child->get_name(), insert_pos,
      (outdated_child ? "True" : "False"));
  auto [insert_stage, insert_size] =
      impl->evaluate_insert(insert_key, insert_value, insert_pos);
  ...
  return (is_root() ? upgrade_root(c, left_hint) : eagain_iertr::now())
      ...
      .si_then([this, insert_key, insert_child, insert_pos,
        ...
        DEBUGT(
            "proceed split {} to fresh {} with insert_child={},"
            " outdated_child={} ...",
            c.t, get_name(), right_node->get_name(), insert_child->get_name(),
        ...
        if (outdated_child) {
          if (is_insert_left) {
            ...
            right_node->template track_insert<false>(
                insert_pos, insert_stage, insert_child);
          }
#ifndef NDEBUG
...
eagain_ifuture<Ref<Node>>
InternalNode::get_or_track_child(
    context_t c,
    ...
    laddr_t child_addr)
{
  ...
  return [this, position, child_addr, c, FNAME] {
    ...
    if (found != tracked_child_nodes.end()) {
      TRACET(
          "loaded child tracked {} at pos({}) addr={}", c.t,
          found->second->get_name(), position, child_addr);
      return eagain_iertr::make_ready_future<Ref<Node>>(found->second);
    }
    // the child is not loaded yet
    TRACET("loading child at pos({}) addr={} ...", c.t, position, child_addr);
    ...
    return Node::load(c, child_addr, level_tail)
        .si_then([this, position, c, FNAME](auto child) {
          ...
          if (child->level() + 1 != level()) {
            ERRORT(
                "loaded child {} error from parent {} at pos({}), level "
                "mismatch",
...
template <bool VALIDATE>
void
InternalNode::track_insert(
    const search_position_t& insert_pos,
...
template void InternalNode::track_insert<true>(
    const search_position_t&,
    match_stage_t,
    Ref<Node>,
    Ref<Node>);
template void InternalNode::track_insert<false>(
    const search_position_t&,
    match_stage_t,
    Ref<Node>,
    Ref<Node>);
...
void
InternalNode::replace_track(
    Ref<Node> new_child,
    Ref<Node> old_child,
    bool is_new_child_outdated)
{
...
void
InternalNode::track_split(
    const search_position_t& split_pos,
    Ref<InternalNode> right_node)
{
...
template <bool VALIDATE>
void
InternalNode::track_erase(
    const search_position_t& erase_pos,
    match_stage_t erase_stage)
{
...
template void InternalNode::track_erase<true>(
    const search_position_t&,
    match_stage_t);
template void InternalNode::track_erase<false>(
    const search_position_t&,
    match_stage_t);

void
InternalNode::track_make_tail(const search_position_t& last_pos)
{
  // assume I'm ref counted by the caller.
  assert(impl->is_level_tail());
  assert(!last_pos.is_end());
  assert(
      tracked_child_nodes.find(search_position_t::end()) ==
  ...
  if (last_it != tracked_child_nodes.end()) {
    ...
    assert(
        tracked_child_nodes.lower_bound(last_pos) == tracked_child_nodes.end());
  }
}

void
InternalNode::validate_child(const Node& child) const
{
#ifndef NDEBUG
...
void
InternalNode::validate_child_inconsistent(const Node& child) const
{
#ifndef NDEBUG
  assert(impl->level() - 1 == child.impl->level());
...
eagain_ifuture<InternalNode::fresh_node_t>
InternalNode::allocate(
    context_t c,
    laddr_t hint,
    ...
    level_t level)
{
  return InternalNodeImpl::allocate(c, hint, field_type, is_level_tail, level)
      .si_then([](auto&& fresh_impl) {
        ...
        auto node = Ref<InternalNode>(
            new InternalNode(derived_ptr, std::move(fresh_impl.impl)));
...
bool
LeafNode::is_level_tail() const
{
  return impl->is_level_tail();
}

node_version_t
LeafNode::get_version() const
{
...
LeafNode::erase(context_t c, const search_position_t& pos, bool get_next)
{
  ...
  return eagain_iertr::now()
      ...
      .si_then([c, &pos, this_ref = std::move(this_ref), this,
                FNAME](Ref<tree_cursor_t> next_cursor) mutable {
...
template eagain_ifuture<Ref<tree_cursor_t>> LeafNode::erase<true>(
    context_t,
    const search_position_t&,
    bool);
template eagain_ifuture<Ref<tree_cursor_t>> LeafNode::erase<false>(
    context_t,
    const search_position_t&,
    bool);

eagain_ifuture<>
LeafNode::extend_value(
    context_t c,
    const search_position_t& pos,
    value_size_t extend_size)
{
...
eagain_ifuture<>
LeafNode::do_get_tree_stats(context_t, tree_stats_t& stats)
{
  auto nstats = impl->get_stats();
...
eagain_ifuture<>
LeafNode::test_clone_root(context_t c_other, RootNodeTracker& tracker_other) const
{
  assert(is_root());
  assert(impl->is_level_tail());
  assert(impl->field_type() == field_type_t::N0);
  ...
  return (is_root() ? upgrade_root(c, left_hint) : eagain_iertr::now())
      ...
      .si_then([this_ref = std::move(this_ref), this, c, &key, vconf, FNAME,
        ...
        if (is_insert_left) {
          ...
          ret = right_node->track_insert(
              insert_pos, insert_stage, p_value_header);
        }
        validate_tracked_cursors();
...
eagain_ifuture<Ref<LeafNode>>
LeafNode::allocate_root(context_t c, RootNodeTracker& root_tracker)
{
    ...
    const value_header_t* p_value_header)
{
  ...
  if (found == tracked_cursors.end()) {
    p_cursor =
        tree_cursor_t::create_tracked(this, position, key, p_value_header);
  } else {
...
void
LeafNode::validate_cursor(const tree_cursor_t& cursor) const
{
#ifndef NDEBUG
...
void
LeafNode::track_split(
    const search_position_t& split_pos,
    Ref<LeafNode> right_node)
{
...
eagain_ifuture<LeafNode::fresh_node_t>
LeafNode::allocate(
    context_t c,
    laddr_t hint,
    field_type_t field_type,
    bool is_level_tail)
{
  return LeafNodeImpl::allocate(c, hint, field_type, is_level_tail)
      .si_then([](auto&& fresh_impl) {
        auto* derived_ptr = fresh_impl.impl.get();
        auto node = Ref<LeafNode>(
            new LeafNode(derived_ptr, std::move(fresh_impl.impl)));
        return fresh_node_t{node, fresh_impl.mut};
      });
}
