...
public:
  ...
  void
  encode_split(const StagedIterator& split_at, const char* p_node_start)
  {
  ...
  void
  encode_erase(const position_t& erase_pos)
  {
  ...
  create_for_encode(const ValueBuilder& v_builder)
  {
...
protected:
      ...
      const NodeExtent& node) override
  {
    ...
    assert(is_empty());
    node_stage_t stage(
        reinterpret_cast<const FieldType*>(mut.get_read()), mut.get_length());
    node_delta_op_t op;
    try {
      ...
      case node_delta_op_t::SPLIT: {
        ...
        auto split_at =
            StagedIterator::decode(mut.get_read(), mut.get_length(), delta);
      ...
      case node_delta_op_t::SPLIT_INSERT: {
        ...
        layout_t::template split_insert<KeyT::HOBJ>(
            mut, stage, split_at, key, value, insert_pos, insert_stage,
            insert_size);
        break;
      }
      case node_delta_op_t::UPDATE_CHILD_ADDR: {
        ...
        auto p_addr =
            reinterpret_cast<laddr_packed_t*>(mut.get_write() + update_offset);
        SUBDEBUG(
            seastore_onode, "apply {} to offset {:#x} ...", new_addr,
            update_offset);
      ...
      case node_delta_op_t::SUBOP_UPDATE_VALUE: {
        ...
        SUBDEBUG(
            seastore_onode, "update {} at {:#x} ...", *p_header_,
            value_header_offset);
        auto payload_mut = p_header_->get_payload_mutable(mut);
        auto value_addr = node.get_laddr() + payload_mut.get_node_offset();
        get_value_replayer(p_header_->magic)
            ->apply_value_delta(delta, payload_mut, value_addr);
        break;
      }
      default:
        SUBERROR(seastore_onode, "got unknown op {} when replay {}", op, node);
        ceph_abort_msg("fatal error");
      }
    } catch (buffer::error& e) {
      SUBERROR(
          seastore_onode, "got decode error {} when replay {}", e.what(), node);
      ceph_abort_msg("fatal error");
    }
  }
...
private:
  ValueDeltaRecorder*
  get_value_replayer(value_magic_t magic)
  {
    ...
    if (value_replayer) {
      if (value_replayer->get_header_magic() != magic) {
        ceph_abort_msgf(
            "OTree::Extent::Replay: value magic mismatch %x != %x",
      ...
      if (!value_replayer) {
        ceph_abort_msgf(
            "OTree::Extent::Replay: got unexpected value magic = %x", magic);
      }
    }
  ...
  void
  encode_value(const value_input_t& value, ceph::bufferlist& encoded) const
  {
  ...
  value_input_t
  decode_value(ceph::bufferlist::const_iterator& delta) const
  {
    if constexpr (std::is_same_v<value_input_t, laddr_t>) {
      // NODE_TYPE == node_type_t::INTERNAL
      laddr_t value;
...
public:
        ...
        extent->get_length()}
  {
    ...
    test_extent =
        TestReplayExtent::create(get_length(), std::move(ref_recorder));
#endif
  }
  ...
  prepare_mutate(context_t c)
  {
    ...
    if (state == nextent_state_t::READ_ONLY) {
      ...
      node_stage = node_stage_t(
          reinterpret_cast<const FieldType*>(extent->get_read()), get_length());
      ...
      node_offset_t& insert_size)
  {
    ...
    auto ret = layout_t::template insert<KT>(
        *mut, read(), key, value, insert_pos, insert_stage, insert_size);
#ifndef NDEBUG
  ...
  void
  split_replayable(StagedIterator& split_at)
  {
    assert(extent->is_mutable());
    assert(state != nextent_state_t::READ_ONLY);
    if (state == nextent_state_t::MUTATION_PENDING) {
      ...
      node_offset_t& insert_size)
  {
    ...
    auto ret = layout_t::template split_insert<KT>(
        *mut, read(), split_at, key, value, insert_pos, insert_stage,
        insert_size);
#ifndef NDEBUG
    test_extent->replay_and_verify(extent);
#endif
  ...
  void
  update_child_addr_replayable(const laddr_t new_addr, laddr_packed_t* p_addr)
  {
    assert(extent->is_mutable());
    assert(state != nextent_state_t::READ_ONLY);
    if (state == nextent_state_t::MUTATION_PENDING) {
  ...
  std::tuple<match_stage_t, position_t>
  erase_replayable(const position_t& pos)
  {
    assert(extent->is_mutable());
    assert(state != nextent_state_t::READ_ONLY);
    if (state == nextent_state_t::MUTATION_PENDING) {
  ...
  void
  test_copy_to(NodeExtentMutable& to) const
  {
  ...
  rebuild(context_t c, laddr_t hint)
  {
    ...
    return c.nm.alloc_extent(c.t, hint, alloc_size)
        .handle_error_interruptible(
            ...
            crimson::ct_error::input_output_error::assert_failure(
                fmt::format(
                    "{} during allocate -- node_size={}, to_discard={}", FNAME,
                    alloc_size, extent->get_laddr())
                    .c_str()))
        .si_then([this, c, FNAME](auto fresh_extent) {
          SUBDEBUGT(
              seastore_onode, "update addr from {} to {} ...", c.t,
          ...
          return c.nm.retire_extent(c.t, to_discard)
              .handle_error_interruptible(
                  ...
                  crimson::ct_error::assert_all(
                      fmt::format(
                          "{} during retire -- to_disgard={}, fresh={}", FNAME,
                          to_discard->get_laddr(), fresh_extent->get_laddr())
                          .c_str()));
        })
        .si_then([this, c] {
  ...
  retire(context_t c)
  {
    ...
    return c.nm.retire_extent(c.t, std::move(extent))
        .handle_error_interruptible(
            eagain_iertr::pass_further{},
            crimson::ct_error::assert_all(
                fmt::format("{} addr={}", FNAME, addr).c_str())
#ifndef NDEBUG
                )
