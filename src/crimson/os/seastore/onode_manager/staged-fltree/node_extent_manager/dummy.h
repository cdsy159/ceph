...
protected:
  ...
  retire_iertr::future<>
  retire_extent(Transaction& t, NodeExtentRef extent) override
  {
    SUBTRACET(
        seastore_onode, "retiring {}B at {} -- {} ...", t, extent->get_length(),
...
private:
  read_iertr::future<NodeExtentRef>
  read_extent_sync(Transaction& t, laddr_t addr)
  {
    auto iter = allocate_map.find(addr);
    assert(iter != allocate_map.end());
    auto extent = iter->second;
    SUBTRACET(
        seastore_onode, "read {}B at {} -- {}", t, extent->get_length(),
        extent->get_laddr(), *extent);
  ...
  alloc_iertr::future<NodeExtentRef>
  alloc_extent_sync(Transaction& t, extent_len_t len)
  {
    ...
    SUBDEBUGT(
        seastore_onode, "allocated {}B at {} -- {}", t, extent->get_length(),
        extent->get_laddr(), *extent);
  ...
  retire_iertr::future<>
  retire_extent_sync(Transaction& t, NodeExtentRef _extent)
  {
  ...
  getsuper_iertr::future<Super::URef>
  get_super_sync(Transaction& t, RootNodeTracker& tracker)
  {
...
template <>
struct fmt::formatter<crimson::os::seastore::onode::DummyNodeExtent>
