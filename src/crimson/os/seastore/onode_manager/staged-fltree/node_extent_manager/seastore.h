...
protected:
  laddr_t
  get_root_laddr() const override
  {
    return root_addr;
  }

  void
  write_root_laddr(context_t c, laddr_t addr) override
  {
...
public:
  ...
  void
  set_generate_eagain(bool enable)
  {
...
protected:
  bool
  is_read_isolated() const override
  {
    return true;
  }

  read_iertr::future<NodeExtentRef>
  read_extent(Transaction& t, laddr_t addr) override
  {
    ...
    return tm.read_extent<SeastoreNodeExtent>(t, addr).si_then(
        [addr,
         &t](auto maybe_indirect_extent) -> read_iertr::future<NodeExtentRef> {
          auto e = maybe_indirect_extent.extent;
          SUBTRACET(
              seastore_onode, "read {}B at {} -- {}", t, e->get_length(),
              e->get_laddr(), *e);
  ...
  alloc_iertr::future<NodeExtentRef>
  alloc_extent(Transaction& t, laddr_t hint, extent_len_t len) override
  {
    ...
    return tm.alloc_non_data_extent<SeastoreNodeExtent>(t, hint, len)
        .si_then([len, &t](auto extent) {
          SUBDEBUGT(
              seastore_onode, "allocated {}B at {} -- {}", t,
              extent->get_length(), extent->get_laddr(), *extent);
          if (!extent->is_initial_pending()) {
            SUBERRORT(
                seastore_onode, "allocated {}B but got invalid extent: {}", t,
                len, *extent);
  ...
  retire_iertr::future<>
  retire_extent(Transaction& t, NodeExtentRef _extent) override
  {
    LogicalChildNodeRef extent = _extent;
    auto addr = extent->get_laddr();
    auto len = extent->get_length();
    SUBDEBUGT(
        seastore_onode, "retiring {}B at {} -- {} ...", t, len, addr, *extent);
    if constexpr (INJECT_EAGAIN) {
      if (trigger_eagain()) {
        SUBDEBUGT(
            seastore_onode, "retiring {}B at {} -- {} : trigger eagain", t, len,
            addr, *extent);
        t.test_set_conflict();
  ...
  getsuper_iertr::future<Super::URef>
  get_super(Transaction& t, RootNodeTracker& tracker) override
  {
...
template <>
struct fmt::formatter<crimson::os::seastore::onode::SeastoreNodeExtent>
