...
struct last_split_info_t {
  ...
  bool
  match(const split_expectation_t& e) const
  {
    match_stage_t split_stage;
    ...
    return split_stage == e.split_stage && insert_stage == e.insert_stage &&
           is_insert_left == e.is_insert_left && insert_type == e.insert_type;
  }

  bool
  match_split_pos(const search_position_t& pos) const
  {
...
public:
  ...
  virtual std::tuple<match_stage_t, search_position_t> erase(
      const search_position_t&) = 0;
  virtual std::tuple<match_stage_t, std::size_t> evaluate_merge(NodeImpl&) = 0;
  virtual search_position_t
  merge(NodeExtentMutable&, NodeImpl&, match_stage_t, extent_len_t) = 0;
...
#pragma GCC diagnostic ignored "-Woverloaded-virtual"
  ...
  virtual lookup_result_t<node_type_t::INTERNAL>
  lower_bound(
      const key_hobj_t&,
      MatchHistory&,
      key_view_t* = nullptr,
...
#pragma GCC diagnostic ignored "-Woverloaded-virtual"
  ...
  virtual std::tuple<search_position_t, bool, const laddr_packed_t*>
  split_insert(
      NodeExtentMutable&,
      NodeImpl&,
      const key_view_t&,
  ...
  virtual void replace_child_addr(
  ...
  struct fresh_impl_t {
    ...
    std::pair<NodeImplURef, NodeExtentMutable>
    make_pair()
    {
  ...
  static eagain_ifuture<fresh_impl_t>
...
#pragma GCC diagnostic ignored "-Woverloaded-virtual"
  ...
  virtual lookup_result_t<node_type_t::LEAF>
  lower_bound(
      const key_hobj_t&,
      MatchHistory&,
      key_view_t* = nullptr,
...
#pragma GCC diagnostic ignored "-Woverloaded-virtual"
  ...
  virtual std::tuple<search_position_t, bool, const value_header_t*>
  split_insert(
      NodeExtentMutable&,
      NodeImpl&,
      const key_hobj_t&,
