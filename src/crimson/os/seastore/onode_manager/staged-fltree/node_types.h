...
inline std::ostream&
operator<<(std::ostream& os, field_type_t type)
{
...
inline std::ostream&
operator<<(std::ostream& os, const node_type_t& type)
{
...
inline std::ostream&
operator<<(std::ostream& os, const laddr_packed_t& laddr)
{
...
constexpr match_stat_t MSTAT_LT0 =
    0; // key == index [pool/shard crush ns/oid]; key < index [snap/gen]
constexpr match_stat_t MSTAT_LT1 =
    1; // key == index [pool/shard crush]; key < index [ns/oid]
...
template <>
struct fmt::formatter<crimson::os::seastore::onode::node_delta_op_t>
  ...
  auto
  format(node_delta_op_t op, FormatContext& ctx) const
  {
