...
public:
  ...
  p_start() const
  {
    return item_range.p_start;
  }

  const char*
  p_end() const
  {
    return item_range.p_end + sizeof(node_offset_t);
  }

  const memory_range_t&
  get_item_range() const
  {
    return item_range;
  }

  node_offset_t
  get_back_offset() const
  {
    return back_offset;
  }
  ...
  const item_iterator_t<NODE_TYPE>&
  operator++() const
  {
    assert(has_next());
  ...
  static node_offset_t
  estimate_insert(const Key& key, const value_input_t&)
  {
...
private:
  void
  next_item_range(const char* p_end) const
  {
    auto p_item_end = p_end - sizeof(node_offset_t);
    assert(p_items_start < p_item_end);
    back_offset =
        reinterpret_cast<const node_offset_packed_t*>(p_item_end)->value;
    assert(back_offset);
