...
static_assert(
    sizeof(crush_hash_t) == sizeof(ghobject_t().hobj.get_bitwise_key_u32()));
...
template <>
struct _full_key_type<KeyT::VIEW> {
  using type = key_view_t;
};

template <>
struct _full_key_type<KeyT::HOBJ> {
...
static laddr_t
get_lba_hint(shard_t shard, pool_t pool, crush_hash_t crush)
{
  ...
  // conflicts.
  return laddr_t::from_raw_uint(
      (uint64_t)(shard & 0xFF) << 56 | (uint64_t)(pool & 0xFF) << 48 |
      (uint64_t)(crush) << 16);
}
...
struct string_key_view_t {
  ...
  static constexpr auto MARKER_MIN = std::numeric_limits<string_size_t>::max() -
                                     1;
  static constexpr auto VALID_UPPER_BOUND =
      std::numeric_limits<string_size_t>::max() - 2;

  static bool
  is_valid_size(size_t size)
  {
  ...
  bool
  operator==(const string_key_view_t& x) const
  {
  ...
  void
  reset_to(const char* origin_base, const char* new_base, extent_len_t node_size)
  {
  ...
  static void
  test_append_str(std::string_view str, char*& p_append)
  {
  ...
  static void
  test_append_dedup(const Type& dedup_type, char*& p_append)
  {
    p_append -= sizeof(string_size_t);
    string_size_t len;
...
public:
  ...
  bool
  operator==(const string_view_masked_t& x) const
  {
  ...
  min()
  {
    return string_view_masked_t{Type::MIN};
  }

  static auto
  max()
  {
    return string_view_masked_t{Type::MAX};
  }

  static string_view_masked_t
  decode(std::string& str_storage, ceph::bufferlist::const_iterator& delta)
  {
    string_size_t size;
...
inline auto
operator<=>(const string_view_masked_t& l, const string_view_masked_t& r)
{
  using Type = string_view_masked_t::Type;
...
inline std::ostream&
operator<<(std::ostream& os, const string_view_masked_t& masked)
{
  using Type = string_view_masked_t::Type;
...
struct ns_oid_view_t {
  ...
  ns_oid_view_t(const char* p_end) :
    nspace(p_end), oid(nspace.p_next_end())
  {}
  ...
  bool
  operator==(const ns_oid_view_t& x) const
  {
    return (
        string_view_masked_t{nspace} == string_view_masked_t{x.nspace} &&
  ...
  template <typename Key>
  static void append(NodeExtentMutable&, const Key& key, char*& p_append);

  static void
  append(NodeExtentMutable& mut, const ns_oid_view_t& view, char*& p_append)
  {
...
inline std::ostream&
operator<<(std::ostream& os, const ns_oid_view_t& ns_oid)
{
...
inline auto
operator<=>(const ns_oid_view_t& l, const ns_oid_view_t& r)
{
...
{
  auto ret = ghobject_t(
      shard_id_t(MAX_SHARD), MAX_POOL, MAX_CRUSH, "MAX", "MAX", MAX_SNAP,
      MAX_GEN);
...
public:
  ...
  {
    os << "key_hobj(" << (int)shard() << "," << pool() << ",0x" << std::hex
       << crush() << std::dec << "; " << string_view_masked_t{nspace()} << ","
       << string_view_masked_t{oid()} << "; " << snap() << "," << gen() << ")";
    return os;
  }
  ...
  decode(ceph::bufferlist::const_iterator& delta)
  {
    ...
    std::string nspace;
    [[maybe_unused]] auto nspace_masked =
        string_view_masked_t::decode(nspace, delta);
    // TODO(cross-node string dedup)
    ...
    return key_hobj_t(
        ghobject_t(shard_id_t(shard), pool, crush, nspace, oid, snap, gen));
  }
...
inline std::ostream&
operator<<(std::ostream& os, const key_hobj_t& key)
{
  return key.dump(os);
}
...
public:
  ...
  to_ghobj() const
  {
    ...
    return ghobject_t(
        shard_id_t(shard()), pool(), crush(), std::string(nspace()),
        std::string(oid()), snap(), gen());
  }

  void
  replace(const crush_t& key)
  {
    p_crush = &key;
  }

  void
  set(const crush_t& key)
  {
    assert(!has_crush());
    replace(key);
  }
  ...
  void
  replace(const ns_oid_view_t& key)
  {
    p_ns_oid = key;
  }

  void
  set(const ns_oid_view_t& key)
  {
    assert(!has_ns_oid());
    replace(key);
  }
  ...
  void
  replace(const snap_gen_t& key)
  {
    p_snap_gen = &key;
  }

  void
  set(const snap_gen_t& key)
  {
    assert(!has_snap_gen());
    replace(key);
  }
...
inline std::ostream&
operator<<(std::ostream& os, const shard_pool_t& sp)
{
...
inline std::ostream&
operator<<(std::ostream& os, const crush_t& c)
{
...
inline std::ostream&
operator<<(std::ostream& os, const shard_pool_crush_t& spc)
{
...
inline std::ostream&
operator<<(std::ostream& os, const snap_gen_t& sg)
{
  return os << sg.snap << "," << sg.gen;
}

shard_t
key_view_t::shard() const
{
...
crush_hash_t
key_view_t::crush() const
{
  return crush_packed().crush;
}

snap_t
key_view_t::snap() const
{
  return snap_gen_packed().snap;
}

gen_t
key_view_t::gen() const
{
  return snap_gen_packed().gen;
}

void
key_view_t::replace(const shard_pool_crush_t& key)
{
  p_shard_pool = &key.shard_pool;
}

void
key_view_t::set(const shard_pool_crush_t& key)
{
  set(key.crush);
  assert(!has_shard_pool());
  replace(key);
...
template <IsFullKey Key>
void
encode_key(const Key& key, ceph::bufferlist& bl)
{
...
template <typename Key>
bool
is_valid_key(const Key& key)
{
  static_assert(IsFullKey<Key>);
  return (
      key > key_hobj_t(ghobject_t()) && key < key_hobj_t(ghobject_t::get_max()));
}

inline std::ostream&
operator<<(std::ostream& os, const key_view_t& key)
{
  return key.dump(os);
}
...
auto
operator<=>(const T& key, const shard_pool_t& target)
{
...
template <IsFullKey T>
auto
operator<=>(const T& key, const crush_t& target)
{
...
template <IsFullKey T>
auto
operator<=>(const T& key, const shard_pool_crush_t& target)
{
...
template <IsFullKey T>
auto
operator<=>(const T& key, const ns_oid_view_t& target)
{
...
template <IsFullKey T>
auto
operator<=>(const T& key, const snap_gen_t& target)
{
...
template <typename Key>
node_offset_t
ns_oid_view_t::estimate_size(const Key& key)
{
  static_assert(IsFullKey<Key>);
  if constexpr (std::same_as<Key, key_view_t>) {
...
template <typename Key>
void
ns_oid_view_t::append(NodeExtentMutable& mut, const Key& key, char*& p_append)
{
  static_assert(IsFullKey<Key>);
  if (key.dedup_type() == Type::STR) {
...
template <typename Key>
void
ns_oid_view_t::test_append(const Key& key, char*& p_append)
{
  static_assert(IsFullKey<Key>);
  if (key.dedup_type() == Type::STR) {
...
template <>
struct fmt::formatter<crimson::os::seastore::onode::key_hobj_t>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::os::seastore::onode::key_view_t>
  : fmt::ostream_formatter {};
#endif
