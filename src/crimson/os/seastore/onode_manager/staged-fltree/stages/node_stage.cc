...
template <typename FieldType, node_type_t NODE_TYPE>
node_offset_t
NODE_T::size_to_nxt_at(index_t index) const
{
  assert(index < keys());
  if constexpr (
      FIELD_TYPE == field_type_t::N0 || FIELD_TYPE == field_type_t::N1) {
...
template <typename FieldType, node_type_t NODE_TYPE>
container_range_t
NODE_T::get_nxt_container(index_t index) const
{
  if constexpr (std::is_same_v<FieldType, internal_fields_3_t>) {
...
template <typename FieldType, node_type_t NODE_TYPE>
void
NODE_T::update_is_level_tail(
    NodeExtentMutable& mut,
    const node_extent_t& extent,
    bool value)
{
    ...
    const char* p_left_bound)
{
  ...
  if constexpr (
    ...
    const char* p_insert = node.p_start() + node.fields().get_item_end_offset(
                                                index, mut.get_length());
...
#define IPA_TEMPLATE(FT, NT, Key)                                    \
  template memory_range_t NODE_INST(FT, NT)::insert_prefix_at<Key>(  \
      NodeExtentMutable&, const node_extent_t&, const Key&, index_t, \
    ...
    node_offset_t trimmed)
{
  ...
  if constexpr (std::is_same_v<FieldType, internal_fields_3_t>) {
    ...
    node_offset_t offset =
        node.p_fields->get_item_start_offset(index, node_size);
  ...
  p_mut{p_mut}, p_start{p_mut->get_write()}
{
  ...
  if (open) {
    ...
    if constexpr (
      ...
      p_append_right = p_start + node.fields().get_item_end_offset(
                                     node.keys() - 1, node_size);
    } else if constexpr (FIELD_TYPE == field_type_t::N2) {
      ceph_abort_msg("not implemented");
    } else {
    ...
    if constexpr (std::is_same_v<FieldType, internal_fields_3_t>) {
      ...
      p_append_left =
          p_start + node.fields().get_key_start_offset(node.keys(), node_size);
      p_append_right =
          p_start + node.fields().get_item_end_offset(node.keys(), node_size);
    }
    num_keys = node.keys();
  }
}
...
template <KeyT KT>
void
APPEND_T::append(const node_extent_t& src, index_t from, index_t items)
{
  ...
  if constexpr (std::is_same_v<FieldType, internal_fields_3_t>) {
    ...
    node_offset_t offset_left_start =
        src.fields().get_key_start_offset(from, node_size);
    node_offset_t offset_left_end =
        src.fields().get_key_start_offset(from + items, node_size);
    ...
    if (num_keys == 0) {
      ...
      p_mut->copy_in_absolute(
          p_append_left, src.p_start() + offset_left_start, left_size);
    } else {
      node_offset_t step_size = FieldType::estimate_insert_one();
      extent_len_t offset_base =
          src.fields().get_item_end_offset(from, node_size);
      ...
      assert(
          p_append_left + left_size + sizeof(typename FieldType::key_t) ==
          p_offset_dst);
    }
    p_append_left += left_size;

    // append right part backwards
    auto offset_right_start =
        src.fields().get_item_end_offset(from + items, node_size);
    ...
    p_mut->copy_in_absolute(
        p_append_right, src.p_start() + offset_right_start,
...
#define APPEND_TEMPLATE(FT, NT, KT) \
  template class node_extent_t<FT, NT>::Appender<KT>
