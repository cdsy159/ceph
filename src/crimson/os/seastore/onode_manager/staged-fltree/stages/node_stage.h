...
public:
  ...
  p_start() const
  {
    return fields_start(*p_fields);
  }

  bool
  is_level_tail() const
  {
    return p_fields->is_level_tail();
  }

  level_t
  level() const
  {
    return p_fields->header.level;
  }

  node_offset_t
  free_size() const
  {
    return p_fields->template free_size_before<NODE_TYPE>(keys(), node_size);
  }
  ...
  {
    auto free_size =
        p_fields->template free_size_before<NODE_TYPE>(index, node_size);
      ...
      ceph::bufferlist::const_iterator& delta)
  {
    ...
    return node_extent_t(
        reinterpret_cast<const FieldType*>(p_node_start), node_size);
  }

  static void
  validate(const FieldType& fields)
  {
#ifndef NDEBUG
  ...
  estimate_insert(const Key& key, const value_input_t& value)
  {
    ...
    if constexpr (FIELD_TYPE == field_type_t::N2) {
      size += ns_oid_view_t::estimate_size(key);
        ...
        FIELD_TYPE == field_type_t::N3 && NODE_TYPE == node_type_t::LEAF) {
