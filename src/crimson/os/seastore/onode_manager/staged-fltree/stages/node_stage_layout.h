...
struct node_header_t {
  ...
  static void update_is_level_tail(
      NodeExtentMutable&,
      const node_header_t&,
      bool);
...
private:
  void
  set_field_type(field_type_t type)
  {
    field_type = static_cast<uint8_t>(type);
  }

  void
  set_node_type(node_type_t type)
  {
    node_type = static_cast<uint8_t>(type);
  }

  void
  set_is_level_tail(bool value)
  {
...
inline std::ostream&
operator<<(std::ostream& os, const node_header_t& header)
{
...
const char*
fields_start(const FieldType& node)
{
...
struct _node_fields_013_t {
  ...
  static constexpr node_offset_t HEADER_SIZE = sizeof(node_header_t) +
                                               sizeof(num_keys_t);
  ...
  is_level_tail() const
  {
    return header.get_is_level_tail();
  }

  extent_len_t
  total_size(extent_len_t node_size) const
  {
    return node_size;
  }

  key_get_type
  get_key(index_t index, extent_len_t node_size) const
  {
  ...
  node_offset_t
  get_key_start_offset(index_t index, extent_len_t node_size) const
  {
    assert(index <= num_keys);
  ...
  node_offset_t
  get_item_start_offset(index_t index, extent_len_t node_size) const
  {
    assert(index < num_keys);
  ...
  extent_len_t
  get_item_end_offset(index_t index, extent_len_t node_size) const
  {
  ...
  static node_offset_t
  erase_at(NodeExtentMutable&, const me_t&, index_t, const char*);
  ...
  template <IsFullKey Key>
  static void
  append_key(NodeExtentMutable& mut, const Key& key, char*& p_append)
  {
...
struct node_fields_2_t {
  ...
  static void
  append_key(NodeExtentMutable& mut, const key_t& key, char*& p_append)
  {
...
struct internal_fields_3_t {
  ...
  static constexpr node_offset_t HEADER_SIZE = sizeof(node_header_t) +
                                               sizeof(num_keys_t);
  static constexpr node_offset_t ITEM_SIZE = sizeof(snap_gen_t) +
                                             sizeof(laddr_t);
  ...
  get_p_child_addr(index_t index, extent_len_t node_size) const
  {
#ifndef NDEBUG
    if (is_level_tail()) {
      assert(index <= num_keys);
...
private:
  num_keys_t
  get_max_num_keys(extent_len_t node_size) const
  {
