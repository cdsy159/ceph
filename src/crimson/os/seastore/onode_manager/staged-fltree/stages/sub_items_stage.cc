...
internal_sub_items_t::insert_at(
    ...
    const internal_sub_items_t& sub_items,
    const Key& key,
    const laddr_t& value,
    index_t index,
    node_offset_t size,
    const char* p_left_bound)
{
  ...
  const char* p_shift_end =
      reinterpret_cast<const char*>(sub_items.p_first_item + 1 - index);
    ...
    const char* p_left_bound)
{
  ...
  const char* p_shift_start = p_left_bound;
  const char* p_shift_end =
      reinterpret_cast<const char*>(sub_items.p_first_item - index);
...
template <KeyT KT>
void
internal_sub_items_t::Appender<KT>::append(
    const internal_sub_items_t& src,
    index_t from,
    index_t items)
{
...
leaf_sub_items_t::insert_at(
    NodeExtentMutable& mut,
    const leaf_sub_items_t& sub_items,
    const Key& key,
    ...
    const char* p_left_bound)
{
  ...
  for (auto i = index; i < sub_items.keys(); ++i) {
    ...
    mut.copy_in_absolute(
        (void*)&offset_i, node_offset_t(offset_i.value + item_size));
  }
  ...
  mut.shift_absolute(
      p_shift_start, p_shift_end - p_shift_start, -(int)sizeof(node_offset_t));

  // e. insert offset
  node_offset_t offset_to_item_start = item_size +
                                       sub_items.get_offset_to_end(index);
  mut.copy_in_absolute(
  ...
  mut.copy_in_absolute(
      (void*)sub_items.p_num_keys, num_keys_t(sub_items.keys() + 1));
    ...
    index_t index)
{
  ...
  mut.shift_absolute(
      p_shift_start, p_shift_end - p_shift_start, size_trim_offsets);
    ...
    const char* p_left_bound)
{
  ...
  for (auto i = index + 1; i < sub_items.keys(); ++i) {
    const node_offset_packed_t& offset_i = sub_items.get_offset(i);
    mut.copy_in_absolute(
        (void*)&offset_i, node_offset_t(offset_i.value - item_erase_size));
  }

  // b. kv[index-1] ... kv[0] ... offset[index+1] >> sizeof(node_offset_t)
  mut.shift_absolute(
      p_item_end, p_offset_end - p_item_end, sizeof(node_offset_t));
...
template <KeyT KT>
void
leaf_sub_items_t::Appender<KT>::append(
    const leaf_sub_items_t& src,
    index_t from,
    index_t items)
{
  if (p_append) {
    ...
    p_mut->shift_absolute(
        p_items_start, p_items_end - p_items_start, -(int)src_offsets_size);
    ...
    p_appended = const_cast<char*>(p_items_start) - src_offsets_size -
                 src_items_size;
...
template <KeyT KT>
char*
leaf_sub_items_t::Appender<KT>::wrap()
{
  if (op_dst.has_value()) {
    // append from existing
    assert(p_appended);
    ...
    std::visit(
        overloaded{
            [&](const range_items_t& arg) {
              int compensate =
                  (last_offset - op_src->get_offset_to_end(arg.from));
              node_offset_t offset;
            ...
            [&](const kv_item_t& arg) {
              last_offset += sizeof(snap_gen_t) +
                             arg.value_config.allocation_size();
              p_cur -= sizeof(node_offset_t);
