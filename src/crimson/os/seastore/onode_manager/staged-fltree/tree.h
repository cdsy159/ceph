...
public:
  ...
  public:
    ...
    get_ghobj() const
    {
      assert(!is_end());
      auto view =
          p_cursor->get_key_view(p_tree->value_builder.get_header_magic());
    ...
    bool
    operator==(const Cursor& o) const
    {
      return operator<=>(o) == 0;
    }

    eagain_ifuture<Cursor>
    get_next(Transaction& t)
    {
      assert(!is_end());
      auto this_obj = *this;
      return p_cursor->get_next(p_tree->get_context(t))
          .si_then([this_obj](Ref<tree_cursor_t> next_cursor) {
            next_cursor->assert_next_to(
                *this_obj.p_cursor,
                this_obj.p_tree->value_builder.get_header_magic());
  ...
  eagain_ifuture<bool>
  contains(Transaction& t, const ghobject_t& obj)
  {
    return seastar::do_with(
        key_hobj_t{obj}, [this, &t](auto& key) -> eagain_ifuture<bool> {
          return get_root(t)
  ...
  eagain_ifuture<Cursor>
  find(Transaction& t, const ghobject_t& obj)
  {
    return seastar::do_with(
        key_hobj_t{obj}, [this, &t](auto& key) -> eagain_ifuture<Cursor> {
          return get_root(t)
              .si_then([this, &t, &key](auto root) {
                // TODO: improve lower_bound()
                return root->lower_bound(get_context(t), key);
   ...
   */
  eagain_ifuture<Cursor>
  lower_bound(Transaction& t, const ghobject_t& obj)
  {
    return seastar::do_with(
        key_hobj_t{obj}, [this, &t](auto& key) -> eagain_ifuture<Cursor> {
          return get_root(t)
  ...
  eagain_ifuture<Cursor>
  get_next(Transaction& t, Cursor& cursor)
  {
  ...
  insert(Transaction& t, const ghobject_t& obj, tree_value_config_t _vconf)
  {
    ...
    if (_vconf.payload_size > value_builder.get_max_value_payload_size()) {
      SUBERRORT(
          seastore_onode, "value payload size {} too large to insert {}", t,
          _vconf.payload_size, key_hobj_t{obj});
      return crimson::ct_error::value_too_large::make();
    }
    if (obj.hobj.nspace.size() > value_builder.get_max_ns_size()) {
      SUBERRORT(
          seastore_onode, "namespace size {} too large to insert {}", t,
          obj.hobj.nspace.size(), key_hobj_t{obj});
      return crimson::ct_error::value_too_large::make();
    }
    if (obj.hobj.oid.name.size() > value_builder.get_max_oid_size()) {
      SUBERRORT(
          seastore_onode, "oid size {} too large to insert {}", t,
          obj.hobj.oid.name.size(), key_hobj_t{obj});
      return crimson::ct_error::value_too_large::make();
    }
  ...
  erase(Transaction& t, const ghobject_t& obj)
  {
    return seastar::do_with(
        key_hobj_t{obj}, [this, &t](auto& key) -> eagain_ifuture<std::size_t> {
  ...
  eagain_ifuture<Cursor>
  erase(Transaction& t, Cursor& pos)
  {
    return pos.erase(t);
  }

  eagain_ifuture<>
  erase(Transaction& t, Value& value)
  {
  ...
  std::ostream&
  dump(Transaction& t, std::ostream& os)
  {
  ...
  eagain_ifuture<>
  test_clone_from(Transaction& t, Transaction& t_from, Btree& from)
  {
    ...
    return from.get_root(t_from).si_then([this, &t](auto root_from) {
...
template <typename ValueImpl>
inline std::ostream&
operator<<(std::ostream& os, const Btree<ValueImpl>& tree)
{
