...
inline std::ostream&
operator<<(std::ostream& os, const value_magic_t& magic)
{
  switch (magic) {
...
inline std::ostream&
operator<<(std::ostream& os, const value_config_t& conf)
{
...
struct value_header_t {
  ...
  bool
  operator==(const value_header_t& rhs) const
  {
  ...
  NodeExtentMutable
  get_payload_mutable(NodeExtentMutable& node) const
  {
  ...
  void
  initiate(NodeExtentMutable& mut, const value_config_t& config)
  {
  ...
  static value_size_t
  estimate_allocation_size(value_size_t payload_size)
  {
    return payload_size + sizeof(value_header_t);
  }
...
inline std::ostream&
operator<<(std::ostream& os, const value_header_t& header)
{
  return os << "Value(" << header.magic << ", " << header.payload_size << "B)";
}

inline value_size_t
value_config_t::allocation_size() const
{
...
public:
  ...
  virtual void apply_value_delta(
...
private:
  ...
  std::pair<NodeExtentMutable&, ValueDeltaRecorder*> do_prepare_mutate_payload(
      Transaction&);
...
struct ValueBuilder {
  ...
  virtual std::unique_ptr<ValueDeltaRecorder> build_value_recorder(
      ceph::bufferlist&) const = 0;
};
 ...
 * unavailable.
 */
std::unique_ptr<ValueDeltaRecorder> build_value_recorder_by_type(
    ceph::bufferlist& encoded,
...
template <>
struct fmt::formatter<crimson::os::seastore::onode::value_config_t>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::os::seastore::onode::value_header_t>
  : fmt::ostream_formatter {};
#endif
