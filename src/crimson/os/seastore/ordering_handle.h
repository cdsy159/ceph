...
struct WritePipeline {
  ...
  struct OolWritesAndLBAUpdates : UnorderedStageT<OolWritesAndLBAUpdates> {
    constexpr static auto type_name =
        "UnorderedStage::ool_writes_and_update_lba_stage";
...
class PlaceholderOperation
  : public crimson::osd::PhasedOperationT<PlaceholderOperation> {
public:
  constexpr static auto type = 0U;
  constexpr static auto type_name =
      "crimson::os::seastore::PlaceholderOperation";

  static PlaceholderOperation::IRef
  create()
  {
...
struct OperationProxyT : OperationProxy {
  ...
  seastar::future<>
  enter(WritePipeline::ReserveProjectedUsage& s) final
  {
    return that()->enter_stage(s);
  }

  seastar::future<>
  enter(WritePipeline::OolWritesAndLBAUpdates& s) final
  {
    return that()->enter_stage(s);
  }

  seastar::future<>
  enter(WritePipeline::Prepare& s) final
  {
    return that()->enter_stage(s);
  }

  seastar::future<>
  enter(WritePipeline::DeviceSubmission& s) final
  {
    return that()->enter_stage(s);
  }

  seastar::future<>
  enter(WritePipeline::Finalize& s) final
  {
    return that()->enter_stage(s);
  }
...
struct OrderingHandle {
  ...
  seastar::future<>
  take_collection_lock(seastar::shared_mutex& mutex)
  {
