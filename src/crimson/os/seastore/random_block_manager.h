...
struct rbm_superblock_t {
  ...
  validate() const
  {
    ...
    for (unsigned int i = 0; i < seastar::smp::count; i++) {
      ...
      ceph_assert(
          shard_infos[i].start_offset < size &&
          shard_infos[i].start_offset % block_size == 0);
    }
    ceph_assert(config.spec.magic != 0);
    ceph_assert(
        get_default_backend_of_device(config.spec.dtype) ==
  ...
  void
  set_end_to_end_data_protection()
  {
...
inline rbm_abs_addr
convert_paddr_to_abs_addr(const paddr_t& paddr)
{
...
inline paddr_t
convert_abs_addr_to_paddr(rbm_abs_addr addr, device_id_t d_id)
{
...
seastar::future<std::unique_ptr<random_block_device::RBMDevice>> get_rb_device(
...
WRITE_CLASS_DENC_BOUNDED(crimson::os::seastore::rbm_superblock_t)

#if FMT_VERSION >= 90000
template <>
struct fmt::formatter<crimson::os::seastore::rbm_superblock_t>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::os::seastore::rbm_shard_info_t>
  : fmt::ostream_formatter {};
#endif
