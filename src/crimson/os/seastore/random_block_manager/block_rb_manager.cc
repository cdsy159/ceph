...
device_config_t
get_rbm_ephemeral_device_config(std::size_t index, std::size_t num_devices)
{
  ...
  return {
      is_major_device, device_spec_t{magic, type, id}, meta, secondary_devices};
}
...
BlockRBManager::alloc_extent(size_t size)
{
  ...
  paddr_t paddr =
      convert_abs_addr_to_paddr(extent.get_start(), device->get_device_id());
  DEBUG(
      "allocated addr: {}, size: {}, requested size: {}", paddr,
      extent.get_len(), size);
  return paddr;
}
...
BlockRBManager::alloc_extents(size_t size)
{
  ...
  size_t len = 0;
  for (auto extent = (*alloc).begin(); extent != (*alloc).end(); extent++) {
    len += extent.get_len();
    paddr_t paddr =
        convert_abs_addr_to_paddr(extent.get_start(), device->get_device_id());
    DEBUG(
        ...
        extent.get_len(), size);
    ret.push_back(
        {std::move(paddr), static_cast<extent_len_t>(extent.get_len())});
  }
...
void
BlockRBManager::complete_allocation(paddr_t paddr, size_t size)
{
  assert(allocator);
...
BlockRBManager::open_ertr::future<>
BlockRBManager::open()
{
  assert(device);
  ...
  allocator->init(
      ool_start, device->get_shard_end() - ool_start, device->get_block_size());
  return open_ertr::now();
}

bool
BlockRBManager::check_valid_range(rbm_abs_addr addr, bufferptr& bptr)
{
  ...
  if (addr < start || addr + bptr.length() > end) {
    ERROR(
        "out of range: start {}, end {}, addr {}, length {}", start, end, addr,
        bptr.length());
    return false;
  }
  return true;
...
BlockRBManager::write_ertr::future<>
BlockRBManager::write(paddr_t paddr, bufferptr bptr)
{
  ceph_assert(device);
  ceph_assert(bptr.is_page_aligned());
  rbm_abs_addr addr = convert_paddr_to_abs_addr(paddr);
  if (!check_valid_range(addr, bptr)) {
    co_return co_await write_ertr::future<>(crimson::ct_error::erange::make());
  }
  co_return co_await device->write(addr, bptr);
}

BlockRBManager::read_ertr::future<>
BlockRBManager::read(paddr_t paddr, bufferptr& bptr)
{
  ceph_assert(device);
  ceph_assert(bptr.is_page_aligned());
  rbm_abs_addr addr = convert_paddr_to_abs_addr(paddr);
  if (!check_valid_range(addr, bptr)) {
    co_return co_await read_ertr::future<>(crimson::ct_error::erange::make());
  }
  co_return co_await device->read(addr, bptr);
}

BlockRBManager::close_ertr::future<>
...
BlockRBManager::write_ertr::future<>
BlockRBManager::write(rbm_abs_addr addr, bufferlist& bl)
{
...
void
BlockRBManager::prefill_fragmented_device()
{
  ...
  for (size_t block = get_block_size() * 3;
    ...
    DEBUG("marking {}~{} used", get_start_rbm_addr() + block, get_block_size());
    allocator->mark_extent_used(get_start_rbm_addr() + block, get_block_size());
  }
}
#endif

std::ostream&
operator<<(std::ostream& out, const rbm_superblock_t& header)
{
  out << " rbm_superblock_t(size=" << header.size
      << ", block_size=" << header.block_size << ", feature=" << header.feature
      << ", journal_size=" << header.journal_size << ", crc=" << header.crc
      << ", config=" << header.config << ", shard_num=" << header.shard_num
      << ", end_to_end_data_protection="
      << header.is_end_to_end_data_protection()
...
std::ostream&
operator<<(std::ostream& out, const rbm_shard_info_t& shard)
{
