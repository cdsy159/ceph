...
public:
  ...
  allocate_ret_bare alloc_extents(
      size_t size) override; // allocator, return blocks

  void complete_allocation(paddr_t addr, size_t size) override;

  size_t
  get_start_rbm_addr() const
  {
  ...
  uint64_t
  get_free_blocks() const override
  {
  ...
  void
  mark_space_used(paddr_t paddr, size_t len) override
  {
    assert(allocator);
    rbm_abs_addr addr = convert_paddr_to_abs_addr(paddr);
    assert(
        addr >= get_start_rbm_addr() && addr + len <= device->get_shard_end());
    allocator->mark_extent_used(addr, len);
  }

  void
  mark_space_free(paddr_t paddr, size_t len) override
  {
    assert(allocator);
    rbm_abs_addr addr = convert_paddr_to_abs_addr(paddr);
    assert(
  ...
  get_start() override
  {
    return convert_abs_addr_to_paddr(
        get_start_rbm_addr(), device->get_device_id());
  }

  rbm_extent_state_t
  get_extent_state(paddr_t paddr, size_t size) override
  {
    assert(allocator);
    rbm_abs_addr addr = convert_paddr_to_abs_addr(paddr);
    assert(
        addr >= get_start_rbm_addr() && addr + size <= device->get_shard_end());
    return allocator->get_extent_state(addr, size);
  }

  size_t
  get_journal_size() const override
  {
