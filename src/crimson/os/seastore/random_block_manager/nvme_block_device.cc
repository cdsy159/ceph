...
seastar::future<>
NVMeBlockDevice::start(uint32_t shard_nums)
{
...
seastar::future<>
NVMeBlockDevice::stop()
{
  return shard_devices.stop();
}

Device&
NVMeBlockDevice::get_sharded_device(store_index_t store_index)
{
  assert(store_index < shard_devices.local().mshard_devices.size());
  return *shard_devices.local().mshard_devices[store_index];
}

NVMeBlockDevice::mkfs_ret
NVMeBlockDevice::mkfs(device_config_t config)
{
  using crimson::common::get_conf;
  co_await shard_devices.local().mshard_devices[0]->do_primary_mkfs(
      config, seastar::smp::count,
      get_conf<Option::size_t>("seastore_cbjournal_size"));
}

open_ertr::future<>
NVMeBlockDevice::open(const std::string& in_path, seastar::open_flags mode)
{
...
open_ertr::future<>
NVMeBlockDevice::open_for_io(const std::string& in_path, seastar::open_flags mode)
{
...
NVMeBlockDevice::mount()
{
  ...
  co_await shard_devices.invoke_on_all([](auto& local_device) {
    return seastar::do_for_each(
        local_device.mshard_devices, [](auto& mshard_device) {
          return mshard_device->do_shard_mount().handle_error(
  ...
  if (is_end_to_end_data_protection()) {
    ...
    if (id_namespace_data.dps.protection_type !=
      ...
      ERROR(
          "seastore was formated with end-to-end-data-protection \
...
write_ertr::future<>
NVMeBlockDevice::write(uint64_t offset, bufferptr bptr, uint16_t stream)
{
  ...
  auto ret =
      co_await io_device[supported_stream]
  ...
  if (ret != length) {
    ...
    co_await write_ertr::future<>(crimson::ct_error::input_output_error::make());
  }
}

read_ertr::future<>
NVMeBlockDevice::read(uint64_t offset, bufferptr& bptr)
{
  LOG_PREFIX(NVMeBlockDevice::read);
  DEBUG("block: read offset {} len {}", offset, bptr.length());
  auto length = bptr.length();
  if (length == 0) {
    co_return;
  }
  ...
  if (ret != length) {
    ...
    co_await read_ertr::future<>(crimson::ct_error::input_output_error::make());
  }
}

read_ertr::future<>
NVMeBlockDevice::_readv(uint64_t offset, std::vector<bufferptr> ptrs)
{
...
write_ertr::future<>
NVMeBlockDevice::writev(uint64_t offset, ceph::bufferlist bl, uint16_t stream)
{
  ...
  co_await seastar::coroutine::parallel_for_each(
      iovs, [this, supported_stream, offset, has_error, FNAME](auto& p) {
        ...
        return io_device[supported_stream]
            ...
            .handle_exception(
                [this, off, len, has_error,
                 FNAME](auto e) -> seastar::future<size_t> {
                  ERROR(
                      "{} poffset={}~{} dma_write got error -- {}",
            ...
            .then([this, off, len, has_error, FNAME](size_t written) {
              if (written != len) {
                ERROR(
                    "{} poffset={}~{} dma_write len={} inconsistent",
...
discard_ertr::future<>
NVMeBlockDevice::discard(uint64_t offset, uint64_t len)
{
...
NVMeBlockDevice::identify_namespace(seastar::file f)
{
  ...
  auto nsid = co_await get_nsid(f).handle_error(
...
nvme_command_ertr::future<int>
NVMeBlockDevice::get_nsid(seastar::file f)
{
...
nvme_command_ertr::future<int>
NVMeBlockDevice::pass_admin(nvme_admin_command_t& admin_cmd, seastar::file f)
{
...
nvme_command_ertr::future<int>
NVMeBlockDevice::pass_through_io(nvme_io_command_t& io_cmd)
{
...
NVMeBlockDevice::try_enable_end_to_end_protection()
{
  LOG_PREFIX(NVMeBlockDevice::try_enable_end_to_end_protection);
  auto id_ns_data = co_await identify_namespace(device);
  if (!id_ns_data) {
    INFO(
        "the device does not support end to end data protection,\
  ...
  if (ret != 0) {
    ERROR("formt nvm command to use end-to-end-protection fails : {}", ret);
    ceph_abort();
  }
  ...
  ceph_assert(
      id_namespace_data.dps.protection_type ==
      nvme_format_nvm_command_t::PROTECT_INFORMATION_TYPE_2);
  super.set_end_to_end_data_protection();
}

nvme_command_ertr::future<>
NVMeBlockDevice::initialize_nvme_features()
{
...
write_ertr::future<>
NVMeBlockDevice::nvme_write(uint64_t offset, size_t len, void* buffer_ptr)
{
...
read_ertr::future<>
NVMeBlockDevice::nvme_read(uint64_t offset, size_t len, void* buffer_ptr)
{
...
read_ertr::future<>
NVMeBlockDevice::nvme_readv(uint64_t offset, std::vector<bufferptr> ptrs)
{
  struct io_t {
    uint64_t offset = 0;
    bufferptr ptr;
  ...
  return seastar::do_with(std::move(iov), [this](auto& iov) {
    return read_ertr::parallel_for_each(iov, [this](auto& io) {
