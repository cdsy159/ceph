    ...
    size_t journal_size)
{
  ...
  for (int i = 0; i < shard_num; i++) {
    uint64_t aligned_size = (super.size / shard_num) -
  ...
  // write super block
  co_await open(
      get_device_path(), seastar::open_flags::rw | seastar::open_flags::dsync)
      .handle_error(
          mkfs_ertr::pass_further{},
          crimson::ct_error::assert_all{
              "Invalid error open in RBMDevice::do_primary_mkfs"});
  co_await initialize_nvme_features();
  co_await write_rbm_superblock().handle_error(
      mkfs_ertr::pass_further{},
      crimson::ct_error::assert_all{
...
write_ertr::future<>
RBMDevice::write_rbm_superblock()
{
...
RBMDevice::read_rbm_superblock(rbm_abs_addr addr)
{
  ...
  try {
    ...
    DEBUG("read_rbm_superblock: unable to decode rbm super block {}", e.what());
    err = true;
  }
  if (err) {
  ...
  assert(
      ceph::encoded_sizeof<rbm_superblock_t>(super_block) <
  ...
  if (super_block.is_end_to_end_data_protection() == false) {
    if (meta_b_header.crc32c(-1) != crc) {
      DEBUG(
          "bad crc on super block, expected {} != actual {} ",
  ...
  DEBUG("got {} ", super);
  co_return co_await read_ertr::future<rbm_superblock_t>(
      read_ertr::ready_future_marker{}, super_block);
}
...
RBMDevice::do_shard_mount()
{
  ...
  super.block_size = (*st).block_size;
  auto s =
      co_await read_rbm_superblock(RBM_START_ADDRESS)
          .handle_error(
              mount_ertr::pass_further{},
              crimson::ct_error::assert_all{"Invalid error read_rbm_superblock "
                                            "in RBMDevice::do_shard_mount"});
  LOG_PREFIX(RBMDevice::do_shard_mount);
  if (seastar::this_shard_id() + seastar::smp::count * store_index >=
      s.shard_num) {
    INFO(
        "{} shard_id {} out of range {}", device_id_printer_t{get_device_id()},
  ...
  shard_info =
      s.shard_infos[seastar::this_shard_id() + seastar::smp::count * store_index];
  INFO("{} read {}", device_id_printer_t{get_device_id()}, shard_info);
  s.validate();
}

read_ertr::future<uint32_t>
RBMDevice::get_shard_nums()
{
  co_await open(
      get_device_path(), seastar::open_flags::rw | seastar::open_flags::dsync)
  ...
  auto st = co_await stat_device().handle_error(crimson::ct_error::assert_all{
...
EphemeralRBMDeviceRef
create_test_ephemeral(uint64_t journal_size, uint64_t data_size)
{
  return EphemeralRBMDeviceRef(new EphemeralRBMDevice(
      journal_size + data_size +
          random_block_device::RBMDevice::get_shard_reserved_size(),
      EphemeralRBMDevice::TEST_BLOCK_SIZE));
}

open_ertr::future<>
EphemeralRBMDevice::open(const std::string& in_path, seastar::open_flags mode)
{
...
write_ertr::future<>
EphemeralRBMDevice::write(uint64_t offset, bufferptr bptr, uint16_t stream)
{
...
read_ertr::future<>
EphemeralRBMDevice::_readv(uint64_t offset, std::vector<bufferptr> ptrs)
{
  LOG_PREFIX(EphemeralRBMDevice::_readv);
  ceph_assert(buf);
  DEBUG("EphemeralRBMDevice: read offset {} {} buffers", offset, ptrs.size());
...
read_ertr::future<>
EphemeralRBMDevice::read(uint64_t offset, bufferptr& bptr)
{
...
write_ertr::future<>
EphemeralRBMDevice::writev(uint64_t offset, ceph::bufferlist bl, uint16_t stream)
{
...
EphemeralRBMDevice::mount_ret
EphemeralRBMDevice::mount()
{
  return do_shard_mount();
}

EphemeralRBMDevice::mkfs_ret
