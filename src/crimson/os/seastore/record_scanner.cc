    ...
    found_record_handler_t& handler)
{
  ...
  return crimson::
      repeat([=, &cursor, &budget_used, &handler, this]() mutable
             ...
             if (!cursor.last_valid_header_found) {
               return read_validate_record_metadata(cursor, nonce)
                   .safe_then([=, &cursor](auto md) {
                     if (!md) {
                       ...
                       if (cursor.is_complete()) {
                         INFO(
                             "complete at {}, invalid record group metadata",
                             cursor);
                       } else {
                         DEBUG(
                             "found invalid record group metadata at {}, "
                             "processing {} pending record groups",
                             cursor.seq, cursor.pending_record_groups.size());
                       }
                   ...
                   .safe_then([=, &cursor, &budget_used, &handler, this] {
                     DEBUG(
                         "processing committed record groups until {}, {} "
                         "pending",
                         cursor.last_committed,
                         cursor.pending_record_groups.size());
                     return crimson::repeat([=, &budget_used, &cursor, &handler,
                                             this] {
		   ...
		   * read record must always fall after cursor.last_committed */
                       ...
                       journal_seq_t next_seq = {
                           cursor.seq.segment_seq, next.offset};
               ...
               return read_validate_data(next.offset, next.header)
                   .safe_then([this, FNAME, &budget_used, &cursor, &handler,
                               &next](auto valid) {
                     if (!valid) {
                       INFO(
                           "complete at {}, invalid record group data at {}, "
                           "{}",
                     ...
                     return consume_next_records(cursor, handler, budget_used);
                      ...
                      .safe_then([=, &budget_used, &cursor] {
                        if (cursor.is_complete() || budget_used >= budget) {
                          DEBUG(
                              "finish at {}, budget_used=0x{:x}, budget=0x{:x}",
    ...
    segment_nonce_t nonce)
{
  ...
  if (get_segment_off(cursor.seq.offset) + block_size >
      get_segment_end_offset(cursor.seq.offset)) {
    DEBUG(
        "failed -- record group header block {}~0x{:x} > segment_size 0x{:x}",
        start, block_size, get_segment_end_offset(cursor.seq.offset));
    return read_validate_record_metadata_ret(
        read_validate_record_metadata_ertr::ready_future_marker{}, std::nullopt);
  }
  TRACE("reading record group header block {}~0x{:x}", start, block_size);
  return read(start, block_size)
      .safe_then(
          [this, FNAME, nonce, block_size,
           &cursor](bufferptr bptr) -> read_validate_record_metadata_ret {
            bufferlist bl;
            bl.append(bptr);
            ...
            if (header.mdlength < block_size ||
                ...
                (header.committed_to != JOURNAL_SEQ_NULL &&
                 get_segment_off(header.committed_to.offset) %
                         cursor.get_block_size() !=
                     0) ||
                (get_segment_off(cursor.seq.offset) + header.mdlength +
                     header.dlength >
                 get_segment_end_offset(cursor.seq.offset))) {
            ...
            TRACE(
                "reading record group header rest {}~0x{:x}", rest_start,
                rest_len);
            return read(rest_start, rest_len)
                .safe_then([header = std::move(header),
                            bl = std::move(bl)](auto&& bptail) mutable {
                  bl.push_back(bptail);
                  return read_validate_record_metadata_ret(
                      read_validate_record_metadata_ertr::ready_future_marker{},
                      std::make_pair(std::move(header), std::move(bl)));
...
RecordScanner::read_validate_data_ret
RecordScanner::read_validate_data(
    paddr_t record_base,
    ...
    std::size_t& budget_used)
{
  ...
  auto locator = record_locator_t{
      ...
      write_result_t{
          journal_seq_t{cursor.seq.segment_seq, next.offset}, total_length}};
  DEBUG(
      "processing {} at {}, budget_used=0x{:x}", next.header, locator,
      budget_used);
