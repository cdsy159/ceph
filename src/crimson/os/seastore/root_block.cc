...
void
RootBlock::on_replace_prior(Transaction& t)
{
  if (!lba_root_node ||
    ...
    if (prior.lba_root_node) {
      ...
      auto lba_root = static_cast<lba::LBANode*>(prior.lba_root_node);
      if (likely(lba_root->range.depth > 1)) {
        TreeRootLinker<RootBlock, lba::LBAInternalNode>::link_root(
            this_ref, static_cast<lba::LBAInternalNode*>(prior.lba_root_node));
      } else {
        assert(lba_root->range.depth == 1);
        TreeRootLinker<RootBlock, lba::LBALeafNode>::link_root(
            this_ref, static_cast<lba::LBALeafNode*>(prior.lba_root_node));
      }
    }
  }
