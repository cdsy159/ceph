...
template <>
struct fmt::formatter<crimson::os::seastore::op_type_t>
  ...
  auto
  format(op_type_t op, FormatContext& ctx) const
  {
...
public:
  ...
  write_meta_ret
  write_meta(const std::string& key, const std::string& value) final
  {
...
{
  if (store_active = is_shard_store_active(store_index, store_shard_nums);
      !store_active) {
    LOG_PREFIX(SeaStore::Shard::Shard);
    INFO(
        "store_index {} is out of range - inactivating this store shard, "
        "store_shard_nums {}",
        store_index, store_shard_nums);
  }
...
SeaStore::SeaStore(const std::string& root, MDStoreRef mdstore) :
  root(root), mdstore(std::move(mdstore))
{
...
void
SeaStore::Shard::register_metrics(store_index_t store_index)
{
  if (!store_active) {
    return;
  }
  ...
  std::pair<op_type_t, sm::label_instance> labels_by_op_type[] = {
      {op_type_t::DO_TRANSACTION,
       sm::label_instance("latency", "DO_TRANSACTION")},
    ...
    metrics.add_group(
        "seastore",
        {
            sm::make_histogram(
                ...
                sm::description(desc),
                {label, sm::label_instance(
                            "shard_store_index", std::to_string(store_index))}),
        });
  }
  ...
  metrics.add_group(
      "seastore",
      {sm::make_gauge(
           "concurrent_transactions", [this] { return throttler.get_current(); },
           sm::description("transactions that are running inside seastore"),
           {sm::label_instance(
               "shard_store_index", std::to_string(store_index))}),
       sm::make_gauge(
           "pending_transactions", [this] { return throttler.get_pending(); },
...
SeaStore::get_shard_nums()
{
  ...
  if (done == -1) {
    ...
    auto shard_nums = co_await device->get_shard_nums().handle_error(
    ...
    if (crimson::common::get_conf<bool>(
      ...
      if (store_shard_nums != seastar::smp::count) {
        INFO(
            "seastore shards {} do not match seastar::smp {}", store_shard_nums,
            seastar::smp::count);
        ceph_abort_msg(
            "seastore_require_partition_count_match_reactor_count is true, "
            "seastore shards do not match seastar::smp");
      }
    }
    co_return;
  }
...
seastar::future<>
SeaStore::shard_stores_start(bool is_test)
{
  LOG_PREFIX(SeaStore::shard_stores_start);
  auto num_shard_services = (store_shard_nums + seastar::smp::count - 1) /
                            seastar::smp::count;
  INFO(
      "store_shard_nums={} seastar::smp={}, num_shard_services={}",
      store_shard_nums, seastar::smp::count, num_shard_services);
  return shard_stores.start(
      num_shard_services, root, device.get(), is_test, store_shard_nums);
}

seastar::future<>
SeaStore::shard_stores_stop()
{
...
SeaStore::start()
{
  ...
  assert(
      d_type == device_type_t::SSD || d_type == device_type_t::RANDOM_BLOCK_SSD);
...
SeaStore::test_start(DeviceRef device_obj)
{
  ...
  co_await shard_stores.start_single(
      1, root, device.get(), true, seastar::smp::count);
  INFO("done");
}
...
SeaStore::test_mount()
{
  ...
  co_await seastar::do_for_each(
      ...
      [](auto& mshard_store) { return mshard_store->mount_managers(); });
  INFO("done");
}

Device::access_ertr::future<>
SeaStore::_mount()
{
  ...
  ceph_assert(
      device->get_sharded_device(0).get_block_size() >= laddr_t::UNIT_SIZE);
  ...
  for (auto& device_entry : sec_devices) {
    ...
    ceph_assert(
        sec_dev->get_sharded_device(0).get_block_size() >= laddr_t::UNIT_SIZE);
...
SeaStore::Shard::mount_managers()
{
  ...
  INFO("start");
  init_managers();
  return transaction_manager->mount().handle_error(
      crimson::ct_error::assert_all{"Invalid error in mount_managers"});
}
...
SeaStore::umount()
{
  ...
  co_await shard_stores.invoke_on_all([](auto& local_store) {
    return seastar::do_for_each(local_store.mshard_stores, [](auto& mshard_store) {
      return mshard_store->umount().handle_error(crimson::ct_error::assert_all{
...
SeaStore::Shard::mkfs_managers()
{
  ...
  co_await with_repeat_trans_intr(
      *t, seastar::coroutine::lambda(
              [&](auto& tr) -> TransactionManager::alloc_extent_iertr::future<> {
...
SeaStore::set_secondaries()
{
  ...
  return shard_stores.invoke_on_all([sec_dev](auto& local_store) {
    return seastar::do_for_each(
        local_store.mshard_stores, [sec_dev](auto& mshard_store) {
...
SeaStore::mkfs_ertr::future<>
SeaStore::test_mkfs(uuid_d new_osd_fsid)
{
...
Device::access_ertr::future<>
SeaStore::_mkfs(uuid_d new_osd_fsid)
{
  ...
  if (!root.empty()) {
    ...
    while (auto de = co_await lister()) {
      ...
      if (entry.name.find("block.") == 0 && entry.name.length() > 6) {
        ...
        sds.emplace(
            (device_id_t)id, device_spec_t{magic, dtype, (device_id_t)id});
        co_await p_sec_dev
  ...
  co_await shard_stores.invoke_on_all([](auto& local_store) {
    return seastar::do_for_each(
        local_store.mshard_stores, [](auto& mshard_store) {
          return mshard_store->mkfs_managers().handle_error(
              crimson::ct_error::assert_all{
                  "Invalid error in SeaStoreS::mkfs_managers"});
        });
  });
  co_await prepare_meta(new_osd_fsid);
  co_await umount();
  ...
  return shard_stores
      ...
      .then([FNAME](auto all_collections) {
        ...
        return seastar::make_ready_future<std::vector<coll_core_t>>(
            std::move(all_collections));
      });
}

seastar::future<store_statfs_t>
SeaStore::Shard::stat() const
{
  if (!store_active) {
...
seastar::future<store_statfs_t>
SeaStore::pool_statfs(int64_t pool_id) const
{
  LOG_PREFIX(SeaStore::pool_statfs);
  DEBUG("pool_id={} ...", pool_id);
  ceph_assert(seastar::this_shard_id() == primary_core);
  //TODO
  return SeaStore::stat().then([FNAME, pool_id](store_statfs_t ss) {
...
SeaStore::report_stats()
{
  ...
  return shard_stores
      .invoke_on_all([this](auto& local_store) {
        return seastar::do_for_each(
            local_store.mshard_stores, [this](auto& mshard_store) {
              bool report_detail = false;
              double seconds = 0;
              if (seastar::this_shard_id() == 0 &&
                  mshard_store->get_store_index() == 0) {
              ...
              shard_device_stats
                  [seastar::this_shard_id() +
                   seastar::smp::count * mshard_store->get_store_index()] =
      ...
      .then([this, FNAME] {
        ...
        INFO("{}", oss_bd.str());
        INFO(
            "device IO depth per writer: {:.2f}",
            device_total.total_depth / device_total_num_io);
        INFO(
            "device bytes per write: {:.2f}",
        ...
        INFO(
            "trans IOPS: {:.2f},{:.2f},{:.2f},{:.2f} per-shard: "
            "{:.2f},{:.2f},{:.2f},{:.2f}",
            io_total.io_num / seconds, io_total.read_num / seconds,
            io_total.get_bg_num() / seconds, io_total.flush_num / seconds,
        ...
        INFO(
            ...
            io_total.pending_io_num, io_total.pending_read_num,
            io_total.pending_bg_num, io_total.pending_flush_num,
        ...
        for (const auto& s : shard_io_stats) {
          oss_pending << s.pending_io_num << "(" << s.starting_io_num << ","
        ...
        INFO(
            "cache pinboard: total{} {}; per-shard: total{} {}",
            cache_total.pinboard_sizes,
            cache_io_stats_printer_t{seconds, cache_total.pinboard_io},
            queue_sizes_ps, cache_io_stats_printer_t{seconds, queue_io_ps});
    ...
    uint32_t op_flags) const
{
  ...
  return seastar::do_with(
             RetType(std::vector<ghobject_t>(), start), std::move(limit),
               ...
               return repeat_eagain([this, ch, start, end, &limit, &ret,
                        ...
                        return transaction_manager
                            ->with_transaction_intr(
                                Transaction::src_t::READ, "list_objects",
                                op_flags,
                                [this, ch, start, end, &limit, &ret](auto& t) {
                                  LOG_PREFIX(SeaStoreS::list_objects);
                                  DEBUGT(
                                      "cid={} start={} end={} limit={} ...", t,
                                      ch->get_cid(), start, end, limit);
                                  return get_coll_bits(ch, t).si_then([FNAME,
                                                                       this, ch,
                                                                       &t, start,
                                                                       end,
                                                                       &limit,
                                    ...
                                    if (!bits) {
                                      ...
                                      DEBUGT("bits={} ...", t, *bits);
                                      auto filter =
                                          SeaStore::get_objs_range(ch, *bits);
                                      using list_iertr =
                                          OnodeManager::list_onodes_iertr;
                                      using repeat_ret = list_iertr::future<
                                          seastar::stop_iteration>;
                                      return trans_intr::
                                          repeat([this, FNAME, &t, &ret, &limit,
                                                  end, filter,
                                                  ranges = get_ranges(
                                                      ch, start, end,
                                                      filter)]() mutable
                                               ...
                                               DEBUGT(
                                                   ...
                                                   "{} ...",
                                                   t, pstart, pend, limit);
                                               return onode_manager
                                                   ->list_onodes(
                                                       t, pstart, pend, limit)
                                                   .si_then([&limit, &ret, pend,
                                                             &t,
                                                             last =
                                                                 ranges.empty(),
                                                             end, FNAME](
...
SeaStore::Shard::open_collection(const coll_t& cid)
{
  ...
  DEBUG("cid={} ...", cid);
  return list_collections().then([cid, this, FNAME](auto colls_cores) {
    if (auto found = std::find(
            colls_cores.begin(), colls_cores.end(),
            std::make_pair(
                cid, std::make_pair(seastar::this_shard_id(), store_index)));
             ...
             [this](auto& ret) {
               return repeat_eagain([this, &ret] {
                        ...
                        return transaction_manager->with_transaction_intr(
                            Transaction::src_t::READ, "list_collections",
                            CACHE_HINT_TOUCH, [this, &ret](auto& t) {
    ...
    uint32_t op_flags)
{
  ...
  if (offset >= size) {
    DEBUGT(
        "0x{:x}~0x{:x} onode-size=0x{:x} flags=0x{:x}, got none", t, offset,
        len, size, op_flags);
    return seastar::make_ready_future<ceph::bufferlist>();
  }

  DEBUGT(
      "0x{:x}~0x{:x} onode-size=0x{:x} flags=0x{:x} ...", t, offset, len, size,
      op_flags);
  size_t corrected_len = (len == 0) ? size - offset
    ...
    uint32_t op_flags)
{
  ...
  return repeat_with_onode<ceph::bufferlist>(
             ch, oid, Transaction::src_t::READ, "read", op_type_t::READ,
             op_flags,
...
SeaStore::Shard::exists(CollectionRef c, const ghobject_t& oid, uint32_t op_flags)
{
  assert(store_active);
  ...
  return repeat_with_onode<bool>(
             c, oid, Transaction::src_t::READ, "exists", op_type_t::READ,
             op_flags,
             [FNAME](auto& t, auto&) {
               DEBUGT("exists", t);
    ...
    uint32_t op_flags)
{
  ...
  DEBUG(
      "cid={} oid={} op_flags=0x{:x} {} intervals", ch->get_cid(), _oid,
      op_flags, m.num_intervals());

  return seastar::do_with(
      _oid, ceph::bufferlist{},
        ...
        return crimson::do_for_each(
                   ...
                   [ch, op_flags, this, &oid, &ret](auto& p) {
                     return read(ch, oid, p.first, p.second, op_flags)
                         .safe_then([&ret](auto bl) { ret.claim_append(bl); });
                   })
            .safe_then([&ret, FNAME] {
...
SeaStore::Shard::_get_attr(Transaction& t, Onode& onode, std::string_view name)
    const
{
  assert(store_active);
    ...
    uint32_t op_flags) const
{
  ...
  return repeat_with_onode<ceph::bufferlist>(
             ch, oid, Transaction::src_t::READ, "get_attr", op_type_t::GET_ATTR,
             op_flags,
...
SeaStore::Shard::_get_attrs(Transaction& t, Onode& onode)
{
  assert(store_active);
  auto& layout = onode.get_layout();
  return omaptree_get_values(
      ...
      .si_then([&layout, &t](auto p) {
        ...
        DEBUGT(
            "got OI length=0x{:x}, SS length=0x{:x}", t,
            (uint32_t)layout.oi_size, (uint32_t)layout.ss_size);
        ceph::bufferlist bl;
        if (layout.oi_size) {
          bl.append(ceph::bufferptr(&layout.oi[0], layout.oi_size));
    ...
    uint32_t op_flags)
{
  ...
  return repeat_with_onode<attrs_t>(
             ...
             [this](auto& t, auto& onode) { return _get_attrs(t, onode); })
      .handle_error(
          crimson::ct_error::input_output_error::assert_failure{
              "EIO when getting attrs"},
          crimson::ct_error::pass_further_all{})
...
SeaStore::Shard::_stat(Transaction& t, Onode& onode, const ghobject_t& oid)
{
  ...
  st.st_nlink = 1;
  DEBUGT(
      "oid={}, size=0x{:x}, blksize=0x{:x}", t, oid, st.st_size, st.st_blksize);
  return seastar::make_ready_future<struct stat>(st);
}

seastar::future<struct stat>
SeaStore::Shard::stat(CollectionRef c, const ghobject_t& oid, uint32_t op_flags)
{
  if (!store_active) {
  ...
  return repeat_with_onode<struct stat>(
             c, oid, Transaction::src_t::READ, "stat", op_type_t::STAT,
             op_flags,
             [this, oid](auto& t, auto& onode) { return _stat(t, onode, oid); })
      .handle_error(crimson::ct_error::assert_all{
...
omap_root_t
SeaStore::Shard::select_log_omap_root(Onode& onode) const
{
  assert(store_active);
    ...
    uint32_t op_flags)
{
  ...
  return repeat_with_onode<omap_values_t>(
             ch, oid, Transaction::src_t::READ, "omap_get_values",
             op_type_t::OMAP_GET_VALUES, op_flags,
             [this, keys](auto& t, auto& onode) {
               auto root = select_log_omap_root(onode);
               return omaptree_get_values(t, std::move(root), keys);
             })
      .finally([this] {
        assert(shard_stats.pending_read_num);
        --(shard_stats.pending_read_num);
    ...
    omap_iterate_conf_t on_conflict)
{
  ...
  ++(shard_stats.pending_read_num);
  return seastar::do_with(
      std::move(start_from), uint32_t(0),
      [this, ch, &oid, callback, op_flags,
       on_conflict](auto& start_from, auto& conflict_counter) {
        return repeat_with_onode<ObjectStore::omap_iter_ret_t>(
                   ch, oid, Transaction::src_t::READ, "omap_iterate",
                   op_type_t::OMAP_ITERATE, op_flags,
                   [this, &start_from, callback, on_conflict,
                    &conflict_counter](auto& t, auto& onode) {
                     ceph_assert(
                         conflict_counter <
    ...
    uint64_t len) const
{
  ...
  size_t size = onode.get_layout().size;
  if (off >= size) {
    DEBUGT("0x{:x}~0x{:x} onode-size=0x{:x}, got none", t, off, len, size);
    return seastar::make_ready_future<std::map<uint64_t, uint64_t>>();
  }
  DEBUGT("0x{:x}~0x{:x} onode-size=0x{:x} ...", t, off, len, size);
    ...
    uint32_t op_flags)
{
  ...
  return repeat_with_onode<fiemap_ret_t>(
             ch, oid, Transaction::src_t::READ, "fiemap", op_type_t::READ,
             op_flags,
...
void
SeaStore::Shard::transaction_dump(ceph::os::Transaction& t)
{
    ...
    ceph::os::Transaction&& _t)
{
  ...
  internal_context_t ctx{
      ...
      transaction_manager->create_transaction(
          Transaction::src_t::MUTATE, "do_transaction", flags)};
  ...
  co_await with_repeat_trans_intr(
      *ctx.transaction,
      seastar::coroutine::lambda([&ctx, this, FNAME](auto& t) -> tm_ret {
        ++(shard_stats.repeat_io_num);
#ifndef NDEBUG
        ...
        DEBUGT(
            "cid={}, {} operations, 0x{:x} bytes, {} colls, {} objects ...", t,
            ctx.ch->get_cid(), ctx.ext_transaction.get_num_ops(),
            ctx.ext_transaction.get_num_bytes(), ctx.iter.colls.size(),
        ...
        while (ctx.iter.have_op()) {
          ...
          DEBUGT(
              "processing op {} of {} for cid={}", t, current_op, total_ops,
              ctx.ch->get_cid());
          co_await _do_transaction_step(ctx, ctx.ch, onodes, ctx.iter);
        }

        DEBUGT(
            "completed all {} ops for cid={}", t, total_ops, ctx.ch->get_cid());
        co_await transaction_manager->submit_transaction(*ctx.transaction);
      }))
      .handle_error(crimson::ct_error::all_same_way([FNAME, &ctx](auto e) {
...
seastar::future<>
SeaStore::Shard::flush(CollectionRef ch)
{
  assert(store_active);
    ...
    ceph::os::Transaction::iterator& i)
{
  ...
  case Transaction::OP_SPLIT_COLLECTION2: {
    ...
    DEBUGT(
        "op OP_SPLIT_COLLECTION2, cid={}, dest_cid={}, bits={}",
  ...
  using onode_iertr =
      OnodeManager::get_onode_iertr::extend<crimson::ct_error::value_too_large>;
  ...
  if (!onodes[op->oid]) {
    ...
    if (!create) {
      DEBUGT("op {}, get oid={} ...", *ctx.transaction, (uint32_t)op->op, oid);
      fut = onode_manager->get_onode(*ctx.transaction, oid);
    } else {
      DEBUGT(
          "op {}, get_or_create oid={} ...", *ctx.transaction, (uint32_t)op->op,
          oid);
      fut = onode_manager->get_or_create_onode(*ctx.transaction, oid);
    }
  }
  return fut
      .si_then([&, op, this, FNAME](auto get_onode) {
        ...
        if ((op->op == Transaction::OP_CLONE ||
          ...
          DEBUGT(
              "op {}, get_or_create dest oid={} ...", *ctx.transaction,
              (uint32_t)op->op, dest_oid);
      ...
      .si_then([&ctx, &i, &onodes, op, this, FNAME]() -> tm_ret {
        ...
        try {
          ...
          case Transaction::OP_REMOVE: {
            ...
            return _remove(ctx, onode).si_then([&onode] { onode.reset(); });
          }
          case Transaction::OP_CREATE:
          case Transaction::OP_TOUCH: {
            DEBUGT(
                "op {}, oid={} ...", *ctx.transaction,
                op->op == Transaction::OP_CREATE ? "CREATE" : "TOUCH", oid);
          ...
          case Transaction::OP_WRITE: {
            ...
            i.decode_bl(bl);
            DEBUGT(
                "op WRITE, oid={}, 0x{:x}~0x{:x}, flags=0x{:x} ...",
                *ctx.transaction, oid, off, len, fadvise_flags);
            return _write(ctx, *onode, off, len, std::move(bl), fadvise_flags);
          }
          case Transaction::OP_TRUNCATE: {
            uint64_t off = op->off;
            DEBUGT(
                "op TRUNCATE, oid={}, 0x{:x} ...", *ctx.transaction, oid, off);
          ...
          case Transaction::OP_SETATTR: {
            ...
            i.decode_bl(bl);
            DEBUGT(
                "op SETATTR, oid={}, attr name={}, value length=0x{:x} ...",
          ...
          case Transaction::OP_SETATTRS: {
            ...
            DEBUGT(
                "op SETATTRS, oid={}, attrs size={} ...", *ctx.transaction, oid,
                to_set.size());
            return _setattrs(ctx, *onode, std::move(to_set));
          }
          case Transaction::OP_RMATTR: {
            std::string name = i.decode_string();
            DEBUGT(
                "op RMATTR, oid={}, attr name={} ...", *ctx.transaction, oid,
                name);
          ...
          case Transaction::OP_OMAP_SETKEYS: {
            ...
            auto root = select_log_omap_root(*onode);
            DEBUGT(
                "op OMAP_SETKEYS, oid={}, omap size={}, type={} ...",
                *ctx.transaction, oid, aset.size(), root.get_type());
            return omaptree_set_keys(
                *ctx.transaction, std::move(root), *onode, std::move(aset));
          }
          case Transaction::OP_OMAP_SETHEADER: {
            ceph::bufferlist bl;
            i.decode_bl(bl);
            DEBUGT(
                "op OMAP_SETHEADER, oid={}, length=0x{:x} ...",
          ...
          case Transaction::OP_OMAP_RMKEYS: {
            ...
            auto root = select_log_omap_root(*onode);
            DEBUGT(
                "op OMAP_RMKEYS, oid={}, omap size={}, type={} ...",
                *ctx.transaction, oid, keys.size(), root.get_type());
            return omaptree_rm_keys(
                *ctx.transaction, std::move(root), *onode, std::move(keys));
          }
          case Transaction::OP_OMAP_RMKEYRANGE: {
            ...
            auto root = select_log_omap_root(*onode);
            DEBUGT(
                "op OMAP_RMKEYRANGE, oid={}, first={}, last={}, type={}...",
                *ctx.transaction, oid, first, last, root.get_type());
            return omaptree_rm_keyrange(
                *ctx.transaction, std::move(root), *onode, std::move(first),
                std::move(last));
          }
          ...
          case Transaction::OP_ZERO: {
            ...
            DEBUGT(
                "op ZERO, oid={}, 0x{:x}~0x{:x} ...", *ctx.transaction, oid,
                off, len);
          ...
          case Transaction::OP_SETALLOCHINT: {
            // TODO
            DEBUGT(
                "op SETALLOCHINT, oid={}, not implemented", *ctx.transaction,
                oid);
            if (op->hint & CEPH_OSD_ALLOC_HINT_FLAG_LOG) {
              ...
              return seastar::do_with(
                  crimson::os::seastore::log_manager::LogManager(
                      *transaction_manager),
          ...
          case Transaction::OP_CLONE: {
            DEBUGT(
                "op CLONE, oid={}, dest oid={} ...", *ctx.transaction, oid,
          ...
          case Transaction::OP_CLONERANGE2: {
            ...
            return _clone_range(
                ctx, onode, onodes[op->dest_oid], srcoff, len, dstoff);
          }
          case Transaction::OP_COLL_MOVE_RENAME: {
            DEBUGT(
                "op COLL_MOVE_RENAME, oid={}, dest oid={} ...",
                *ctx.transaction, oid, i.get_oid(op->dest_oid));
            ceph_assert(op->cid == op->dest_cid);
            return _rename(ctx, onode, onodes[op->dest_oid]).si_then([&onode] {
              onode.reset();
            });
          }
          default:
      ...
      .handle_error_interruptible(
          tm_iertr::pass_further{}, crimson::ct_error::enoent::handle([op] {
    ...
    OnodeRef& d_onode)
{
  ...
  auto oi_bl = ceph::bufferlist::static_from_mem(
      &olayout.oi[0], (uint32_t)olayout.oi_size);
  auto ss_bl = ceph::bufferlist::static_from_mem(
      &olayout.ss[0], (uint32_t)olayout.ss_size);
  ...
  return onode_manager->erase_onode(*ctx.transaction, onode)
      .handle_error_interruptible(
          ...
          crimson::ct_error::assert_all{"Invalid error in SeaStoreS::_rename"});
}

SeaStore::Shard::tm_ret
SeaStore::Shard::_remove(internal_context_t& ctx, OnodeRef& onode)
{
  return omaptree_clear_no_onode(
             *ctx.transaction, get_omap_root(omap_type_t::OMAP, *onode))
      .si_then([this, &ctx, &onode] {
        return omaptree_clear_no_onode(
            *ctx.transaction, get_omap_root(omap_type_t::XATTR, *onode));
      })
      .si_then([this, &ctx, &onode] {
        return seastar::do_with(
            ObjectDataHandler(max_object_size),
      ...
      .handle_error_interruptible(
          crimson::ct_error::input_output_error::pass_further(),
          crimson::ct_error::assert_all("Invalid error in SeaStoreS::_remove"));
}

SeaStore::Shard::tm_ret
SeaStore::Shard::_touch(internal_context_t& ctx, Onode& onode)
{
  return tm_iertr::now();
}
    ...
    uint32_t fadvise_flags)
{
  ...
  if (offset + len > object_size) {
    onode.update_onode_size(
        *ctx.transaction, std::max<uint64_t>(offset + len, object_size));
  }
  return seastar::do_with(
      std::move(_bl), ObjectDataHandler(max_object_size),
...
SeaStore::Shard::tm_ret
SeaStore::Shard::_clone(internal_context_t& ctx, Onode& onode, Onode& d_onode)
{
  return seastar::do_with(
             ObjectDataHandler(max_object_size),
       ...
       */
                 ...
                 return objHandler.clone(ObjectDataHandler::context_t{
                     *transaction_manager, *ctx.transaction, d_onode, &onode});
               } else { // OP_ROLLBACK
                 assert(d_onode.is_head());
                 return objHandler.clone(ObjectDataHandler::context_t{
                     *transaction_manager, *ctx.transaction, onode, &d_onode});
               }
             })
      .si_then([&ctx, &onode, &d_onode, this] {
        return omaptree_clone(
            *ctx.transaction, omap_type_t::XATTR,
            onode.is_head() ? d_onode : onode,
            onode.is_head() ? onode : d_onode);
      })
      .si_then([&ctx, &onode, &d_onode, this] {
        return omaptree_clone(
            *ctx.transaction, omap_type_t::OMAP,
            onode.is_head() ? d_onode : onode,
            onode.is_head() ? onode : d_onode);
      });
}
    ...
    extent_len_t dstoff)
{
  ...
  DEBUGT(
      "src_onode={}, dst_onode={}, src {}~{}, dst {}", *ctx.transaction,
      *src_onode, *dst_onode, srcoff, length, dstoff);
  const auto& d_object_size = dst_onode->get_layout().size;
  if (srcoff + length > d_object_size) {
    dst_onode->update_onode_size(
        *ctx.transaction, std::max<uint64_t>(srcoff + length, d_object_size));
  }
  return seastar::do_with(
      ObjectDataHandler(max_object_size), [=, this, &ctx](auto& objHandler) {
        return objHandler.clone_range(
            ObjectDataHandler::context_t{
                *transaction_manager, *ctx.transaction, *src_onode,
                dst_onode.get()},
            srcoff, length, dstoff);
      });
}
    ...
    extent_len_t len)
{
  if (offset + len >= max_object_size) {
    ...
    ERRORT(
        "0x{:x}~0x{:x} >= 0x{:x}", *ctx.transaction, offset, len,
        max_object_size);
    return crimson::ct_error::input_output_error::make();
  }
  const auto& object_size = onode.get_layout().size;
...
SeaStore::Shard::tm_ret
SeaStore::Shard::_omap_clear(internal_context_t& ctx, Onode& onode)
{
  return omaptree_rm_key(
             *ctx.transaction, get_omap_root(omap_type_t::XATTR, onode), onode,
             std::string(OMAP_HEADER_XATTR_KEY))
      .si_then([this, &ctx, &onode] {
        return omaptree_clear(
            *ctx.transaction, get_omap_root(omap_type_t::OMAP, onode), onode);
      });
}

SeaStore::Shard::tm_ret
SeaStore::Shard::_truncate(internal_context_t& ctx, Onode& onode, uint64_t size)
{
    ...
    std::map<std::string, bufferlist>&& aset)
{
  ...
  return std::move(fut).si_then([this, &onode, &ctx,
...
SeaStore::Shard::tm_ret
SeaStore::Shard::_rmattr(internal_context_t& ctx, Onode& onode, std::string name)
{
  auto& layout = onode.get_layout();
...
SeaStore::Shard::tm_ret
SeaStore::Shard::_rmattrs(internal_context_t& ctx, Onode& onode)
{
  onode.clear_object_info(*ctx.transaction);
  onode.clear_snapset(*ctx.transaction);
  return omaptree_clear(
  ...
  return transaction_manager->read_collection_root(*ctx.transaction)
      .si_then([=, this, &ctx](auto _cmroot) {
        return seastar::do_with(_cmroot, [=, this, &ctx](auto& cmroot) {
          return collection_manager->update(cmroot, *ctx.transaction, cid, bits)
              .si_then([this, &ctx, &cmroot] {
                if (cmroot.must_update()) {
                  transaction_manager->write_collection_root(
                      *ctx.transaction, cmroot);
                }
              });
        });
      })
    ...
    int bits)
{
  return transaction_manager->read_collection_root(*ctx.transaction)
      .si_then([=, this, &ctx](auto _cmroot) {
        return seastar::do_with(_cmroot, [=, this, &ctx](auto& cmroot) {
          return collection_manager->create(cmroot, *ctx.transaction, cid, bits)
              .si_then([this, &ctx, &cmroot] {
                if (cmroot.must_update()) {
                  transaction_manager->write_collection_root(
                      *ctx.transaction, cmroot);
...
SeaStore::Shard::tm_ret
SeaStore::Shard::_remove_collection(internal_context_t& ctx, const coll_t& cid)
{
  return transaction_manager->read_collection_root(*ctx.transaction)
      .si_then([=, this, &ctx](auto _cmroot) {
        return seastar::do_with(_cmroot, [=, this, &ctx](auto& cmroot) {
          return collection_manager->remove(cmroot, *ctx.transaction, cid)
              .si_then([this, &ctx, &cmroot] {
...
seastar::future<>
SeaStore::write_meta(const std::string& key, const std::string& value)
{
  LOG_PREFIX(SeaStore::write_meta);
  DEBUG("key={} value={} ...", key, value);

  ceph_assert(seastar::this_shard_id() == primary_core);
  return seastar::do_with(key, value, [this, FNAME](auto& key, auto& value) {
...
seastar::future<>
SeaStore::Shard::write_meta(const std::string& key, const std::string& value)
{
  assert(store_active);
  ++(shard_stats.io_num);
  ++(shard_stats.pending_io_num);
  ...
  return repeat_eagain([this, &key, &value] {
           ...
           return transaction_manager->with_transaction_intr(
               Transaction::src_t::MUTATE, "write_meta", CACHE_HINT_NOCACHE,
               [this, &key, &value](auto& t) {
                 LOG_PREFIX(SeaStoreS::write_meta);
                 DEBUGT("key={} value={} ...", t, key, value);
                 return transaction_manager->update_root_meta(t, key, value)
                     .si_then([this, &t] {
      ...
      .handle_error(crimson::ct_error::assert_all{
          "Invalid error in SeaStoreS::write_meta"})
      .finally([this] {
        assert(shard_stats.pending_io_num);
        --(shard_stats.pending_io_num);
        // XXX: it's wrong to assume no failure,
...
SeaStore::get_default_device_class()
{
  using crimson::common::get_conf;
  std::string type = get_conf<std::string>("seastore_main_device_type");
...
uuid_d
SeaStore::Shard::get_fsid() const
{
  return device->get_meta().seastore_id;
}

void
SeaStore::Shard::init_managers()
{
  assert(store_active);
  ...
  collection_manager =
      std::make_unique<collection_manager::FlatCollectionManager>(
          *transaction_manager);
  onode_manager =
...
double
SeaStore::Shard::reset_report_interval() const
{
  if (!store_active) {
    return 0;
  }
...
device_stats_t
SeaStore::Shard::get_device_stats(bool report_detail, double seconds) const
{
  if (!store_active) {
...
shard_stats_t
SeaStore::Shard::get_io_stats(bool report_detail, double seconds) const
{
  if (!store_active) {
  ...
  if (report_detail && seconds != 0) {
    ...
    INFO(
        "iops={:.2f},{:.2f},{:.2f}({:.2f},{:.2f},{:.2f},{:.2f}),{:.2f} "
        "conflicts={:.2f},{:.2f},{:.2f}({:.2f},{:.2f},{:.2f},{:.2f}) "
        "outstanding={}({},{},{},{},{}),{},{},{}",
        // iops
        ret.io_num / seconds, ret.read_num / seconds,
        ret.get_bg_num() / seconds, ret.trim_alloc_num / seconds,
        ret.trim_dirty_num / seconds, ret.cleaner_main_num / seconds,
        ret.cleaner_cold_num / seconds, ret.flush_num / seconds,
        // conflicts
        ...
        // outstanding
        ret.pending_io_num, ret.starting_io_num, ret.waiting_collock_io_num,
        ret.waiting_throttler_io_num, ret.processing_inlock_io_num,
        ret.processing_postlock_io_num, ret.pending_read_num,
        ret.pending_bg_num, ret.pending_flush_num);
  }
...
cache_stats_t
SeaStore::Shard::get_cache_stats(bool report_detail, double seconds) const
{
  if (!store_active) {
    return cache_stats_t();
  }
  return transaction_manager->get_cache_stats(report_detail, seconds);
}

std::unique_ptr<SeaStore>
make_seastore(const std::string& device)
{
...
std::unique_ptr<SeaStore>
make_test_seastore(SeaStore::MDStoreRef mdstore)
{
    ...
    std::string_view key) const
{
  ...
  auto run = [&](auto manager) -> omaptree_get_value_ret {
    return seastar::do_with(
        std::move(manager), std::move(root), std::string(key),
        [&t](auto& manager, auto& root, auto& key) -> omaptree_get_value_ret {
          ...
          return manager.omap_get_value(root, t, key)
              .si_then(
                  [&key, &t, FNAME, type](auto opt) -> omaptree_get_value_ret {
                    if (!opt) {
                      DEBUGT("{} key={} is absent", t, type, key);
                      return crimson::ct_error::enodata::make();
                    }
                    DEBUGT(
                        "{} key={}, value length=0x{:x}", t, type, key,
                        opt->length());
                    return seastar::make_ready_future<ceph::bufferlist>(
                        std::move(*opt));
                  });
    ...
    const omap_keys_t& keys) const
{
  ...
  auto run = [&](auto manager) {
    return seastar::do_with(
        std::move(manager), std::move(root), omap_values_t(),
        [&t, &keys, type, FNAME](auto& manager, auto& root, auto& ret) {
          return trans_intr::do_for_each(
              ...
              .si_then([&t, &ret, &keys, type, FNAME] {
                DEBUGT(
                    "{} {} keys got {} values", t, type, keys.size(),
                    ret.size());
                return std::move(ret);
              });
        });
    ...
    omap_iterate_cb_t callback)
{
  ...
  if (root.is_null()) {
    ...
    return seastar::make_ready_future<ObjectStore::omap_iter_ret_t>(
        ObjectStore::omap_iter_ret_t::NEXT);
  }
  auto run = [&](auto manager) {
    return seastar::do_with(
        std::move(manager), std::move(root),
    ...
    OMapManager::omap_list_config_t config) const
{
  ...
  auto run = [&](auto manager) {
    return seastar::do_with(
        std::move(manager), std::move(root), start,
    ...
    const std::optional<std::string>& start) const
{
  ...
  return omaptree_list(t, std::move(root), start, config)
      .si_then([FNAME, type, &t](omap_values_paged_t ret) {
        DEBUGT(
            "{} got {} values, complete={}", t, type, std::get<1>(ret).size(),
            std::get<0>(ret));
        return ret;
      });
}
...
SeaStore::Shard::omaptree_do_clear(Transaction& t, omap_root_t&& root)
{
  assert(store_active);
  assert(!root.is_null());
  auto run = [&](auto manager) {
    return seastar::do_with(
        std::move(manager), std::move(root), [&t](auto& manager, auto& root) {
          return manager.omap_clear(root, t).si_then([&root] {
            assert(root.is_null());
            return root;
          });
        });
...
base_iertr::future<>
SeaStore::Shard::omaptree_clear_no_onode(Transaction& t, omap_root_t&& root)
{
  assert(store_active);
  ...
  return omaptree_do_clear(t, std::move(root)).si_then([FNAME, &t](auto root) {
...
void
omaptree_update_root(Transaction& t, omap_root_t& root, Onode& onode)
{
...
base_iertr::future<>
SeaStore::Shard::omaptree_clear(Transaction& t, omap_root_t&& root, Onode& onode)
{
  assert(store_active);
        ...
        [&t, &onode, &d_onode, this, type, FNAME](auto& start) {
          ...
          return omaptree_list(t, get_omap_root(type, onode), start, config)
              .si_then([&t, &d_onode, this, type, FNAME, &start](auto p) {
                ...
                return omaptree_set_keys(
                           t, get_omap_root(type, d_onode), d_onode,
                           std::map<std::string, ceph::bufferlist>(
                               attrs.begin(), attrs.end()))
                    .si_then([complete, nstart = std::move(nstart), &start,
                              FNAME, type, &t]() mutable {
                      if (complete) {
    ...
    std::map<std::string, ceph::bufferlist>&& kvs)
{
  ...
  auto run = [&](auto manager) {
    return seastar::do_with(
               std::move(manager), std::move(root),
               [this, &t, &onode,
                kvs = std::move(kvs)](auto& manager, auto& root) mutable {
                 ...
                 return std::move(maybe_create_root)
                     .si_then([&t, &root, &manager,
                               kvs = std::move(kvs)]() mutable {
                       return manager.omap_set_keys(root, t, std::move(kvs));
                     })
                     .si_then([&root] {
                       return base_iertr::make_ready_future<omap_root_t>(
                           std::move(root));
                     });
               })
    ...
    omap_keys_t&& keys)
{
  ...
  auto run = [&](auto manager) {
    return seastar::do_with(
        std::move(manager), std::move(root), std::move(keys),
        [&t, &onode](auto& manager, auto& root, auto& keys) {
          return manager.omap_rm_keys(root, t, keys).si_then([&t, &root, &onode] {
            if (root.must_update()) {
              omaptree_update_root(t, root, onode);
            }
          });
    ...
    std::string last)
{
  ...
  auto run = [&](auto manager) {
    return seastar::do_with(
        std::move(manager), std::move(root), std::move(first), std::move(last),
        [&t, &onode, FNAME](auto& manager, auto& root, auto& first, auto& last) {
          return manager.omap_rm_key_range(root, t, first, last)
    ...
    std::string&& name)
{
  ...
  auto run = [&](auto manager) {
    return seastar::do_with(
        std::move(manager), std::move(root), std::move(name),
