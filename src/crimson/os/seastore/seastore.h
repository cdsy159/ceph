...
public:
  ...
  public:
    ...
    seastar::future<CollectionRef> create_new_collection(
        const coll_t& cid) override final;
    ...
    unsigned
    get_max_attr_name_length() const override final
    {
      return 256;
    }
  ...
  public:
    ...
    void
    set_secondaries(Device& sec_dev)
    {
  ...
  private:
    struct internal_context_t {
      ...
      std::chrono::steady_clock::time_point begin_timestamp =
          std::chrono::steady_clock::now();

      void
      reset_preserve_handle(TransactionManager& tm)
      {
        ...
        F&& f) const
    {
      ...
      return seastar::do_with(
          ...
          [this, ch, src, op_type, begin_time, tname,
           cache_hint_flags](auto& oid, auto& ret, auto& f) {
            return repeat_eagain([&, this, ch, src, tname, cache_hint_flags] {
                ...
                .safe_then([&ret, op_type, begin_time, this] {
                  const_cast<Shard*>(this)->add_latency_sample(
                      op_type, std::chrono::steady_clock::now() - begin_time);
    ...
    using omaptree_get_value_iertr =
        base_iertr::extend<crimson::ct_error::enodata>;
    using omaptree_get_value_ret =
        omaptree_get_value_iertr::future<ceph::bufferlist>;
    omaptree_get_value_ret _get_attr(
        Transaction& t,
        Onode& onode,
        std::string_view name) const;

    base_iertr::future<attrs_t> _get_attrs(Transaction& t, Onode& onode);
    ...
    base_iertr::future<fiemap_ret_t>
    _fiemap(Transaction& t, Onode& onode, uint64_t off, uint64_t len) const;

    using tm_iertr = base_iertr::extend<crimson::ct_error::value_too_large>;
    ...
    tm_ret _touch(internal_context_t& ctx, Onode& onode);
    tm_ret _write(
        internal_context_t& ctx,
        Onode& onode,
        uint64_t offset,
    ...
    tm_ret _clone(internal_context_t& ctx, Onode& onode, Onode& d_onode);
    tm_ret _rename(internal_context_t& ctx, OnodeRef& onode, OnodeRef& d_onode);
    tm_ret _clone_range(
        internal_context_t& ctx,
        OnodeRef& src_onode,
        OnodeRef& dst_onode,
    ...
    tm_ret _omap_clear(internal_context_t& ctx, Onode& onode);
    tm_ret _truncate(internal_context_t& ctx, Onode& onode, uint64_t size);
    tm_ret _setattrs(
        internal_context_t& ctx,
        Onode& onode,
        std::map<std::string, bufferlist>&& aset);
    tm_ret _rmattr(internal_context_t& ctx, Onode& onode, std::string name);
    tm_ret _rmattrs(internal_context_t& ctx, Onode& onode);
    ...
    void
    add_latency_sample(op_type_t op_type, std::chrono::steady_clock::duration dur)
    {
    ...
    omap_root_t
    get_omap_root(omap_type_t type, Onode& onode) const
    {
    ...
    omap_root_t
    rename_omap_root(omap_type_t type, Onode& onode, Onode& d_onode) const
    {
      return onode.get_root(type).get(
    ...
    using omaptree_set_keys_iertr =
        base_iertr::extend<crimson::ct_error::value_too_large>;
...
public:
  ...
  crimson::os::FuturizedStore::mount_ertr::future<>
  mount() override
  {
    return _mount().handle_error(Device::access_ertr::all_same_way(
        [](auto& code) { return crimson::stateful_ec{code}; }));
  }
  ...
  crimson::os::FuturizedStore::mkfs_ertr::future<>
  mkfs(uuid_d new_osd_fsid) override
  {
    return _mkfs(new_osd_fsid)
        .handle_error(Device::access_ertr::all_same_way([](auto& code) {
...
private:
  ...
  public:
    ...
    ~MultiShardStores() { mshard_stores.clear(); }
  };
