...
bool
is_aligned(uint64_t offset, uint64_t alignment)
{
  return (offset % alignment) == 0;
}

std::ostream&
operator<<(std::ostream& out, const omap_root_t& root)
{
  return out << "omap_root{addr=" << root.addr << ", depth=" << root.depth
             << ", hint=" << root.hint << ", mutated=" << root.mutated << "}";
}

std::ostream&
operator<<(std::ostream& out, const seastore_meta_t& meta)
{
  return out << meta.seastore_id;
}

std::ostream&
operator<<(std::ostream& out, const device_id_printer_t& id)
{
  auto _id = id.id;
  if (_id == DEVICE_ID_NULL) {
    ...
    return out << "Dev(0x" << std::hex << (unsigned)_id << std::dec << ")";
  }
}

std::ostream&
operator<<(std::ostream& out, const segment_id_t& segment)
{
  if (segment == NULL_SEG_ID) {
    return out << "Seg[NULL]";
  } else {
    return out << "Seg[" << device_id_printer_t{segment.device_id()} << ",0x"
               << std::hex << segment.device_segment_id() << std::dec << "]";
  }
}

std::ostream&
operator<<(std::ostream& out, segment_type_t t)
{
  switch (t) {
...
std::ostream&
operator<<(std::ostream& out, segment_seq_printer_t seq)
{
...
std::ostream&
operator<<(std::ostream& out, const laddr_t& laddr)
{
  return out << "L0x" << std::hex << laddr.value << std::dec;
}

std::ostream&
operator<<(std::ostream& out, const laddr_offset_t& laddr_offset)
{
  return out << laddr_offset.get_laddr() << "+0x" << std::hex
             << laddr_offset.get_offset() << std::dec;
}

std::ostream&
operator<<(std::ostream& out, const pladdr_t& pladdr)
{
  if (pladdr.is_laddr()) {
...
std::ostream&
operator<<(std::ostream& out, const paddr_t& rhs)
{
  ...
  if (rhs == P_ADDR_NULL) {
    ...
    out << device_id_printer_t{id} << ",0x" << std::hex << s.get_device_off()
        << std::dec;
  } else if (rhs.is_absolute_segmented()) {
    auto& s = rhs.as_seg_paddr();
    out << s.get_segment_id() << ",0x" << std::hex << s.get_segment_off()
        << std::dec;
  } else if (rhs.is_absolute_random_block()) {
    auto& s = rhs.as_blk_paddr();
    out << device_id_printer_t{s.get_device_id()} << ",0x" << std::hex
        << s.get_device_off() << std::dec;
  } else {
    out << "INVALID!";
  }
  return out << ">";
    ...
    device_off_t roll_size) const
{
  ...
  if (type == backend_type_t::SEGMENTED) {
    ret +=
        (static_cast<device_off_t>(offset.as_seg_paddr().get_segment_off()) -
...
std::ostream&
operator<<(std::ostream& out, const journal_seq_t& seq)
{
  if (seq == JOURNAL_SEQ_NULL) {
    ...
    return out << "jseq(" << segment_seq_printer_t{seq.segment_seq} << ", "
               << seq.offset << ")";
  }
}

std::ostream&
operator<<(std::ostream& out, extent_types_t t)
{
  switch (t) {
...
std::ostream&
operator<<(std::ostream& out, rewrite_gen_printer_t gen)
{
...
std::ostream&
operator<<(std::ostream& out, data_category_t c)
{
  switch (c) {
...
bool
can_inplace_rewrite(extent_types_t type)
{
  return is_data_type(type);
}

std::ostream&
operator<<(std::ostream& out, sea_time_point_printer_t tp)
{
  if (tp.tp == NULL_TIME) {
    return out << "tp(NULL)";
  }
...
std::ostream&
operator<<(std::ostream& out, mod_time_point_printer_t tp)
{
...
std::ostream&
operator<<(std::ostream& out, const laddr_list_t& rhs)
{
  bool first = false;
  for (auto& i : rhs) {
...
std::ostream&
operator<<(std::ostream& out, const paddr_list_t& rhs)
{
  bool first = false;
  for (auto& i : rhs) {
    out << (first ? '[' : ',') << '(' << i.first << ',' << i.second << ')';
...
std::ostream&
operator<<(std::ostream& out, const delta_info_t& delta)
{
  return out << "delta_info_t(" << "type: " << delta.type
             << ", paddr: " << delta.paddr << ", laddr: " << delta.laddr
             << ", prev_crc: 0x" << std::hex << delta.prev_crc
             << ", final_crc: 0x" << delta.final_crc << ", length: 0x"
             << delta.length << std::dec << ", pversion: " << delta.pversion
             << ", ext_seq: " << segment_seq_printer_t{delta.ext_seq}
             << ", seg_type: " << delta.seg_type << ")";
}

std::ostream&
operator<<(std::ostream& out, const journal_tail_delta_t& delta)
{
  return out << "journal_tail_delta_t(" << "alloc_tail=" << delta.alloc_tail
             << ", dirty_tail=" << delta.dirty_tail << ")";
}

std::ostream&
operator<<(std::ostream& out, const extent_info_t& info)
{
  return out << "extent_info_t(" << "type: " << info.type
             << ", addr: " << info.addr << ", len: 0x" << std::hex << info.len
             << std::dec << ")";
}

std::ostream&
operator<<(std::ostream& out, const segment_header_t& header)
{
  return out << "segment_header_t(" << header.physical_segment_id << " "
             << header.type << " " << segment_seq_printer_t{header.segment_seq}
             << " " << header.category << " "
             << rewrite_gen_printer_t{header.generation}
             << ", dirty_tail=" << header.dirty_tail
             << ", alloc_tail=" << header.alloc_tail << ", segment_nonce=0x"
             << std::hex << header.segment_nonce << std::dec
             << ", modify_time=" << mod_time_point_printer_t{header.modify_time}
             << ")";
}

std::ostream&
operator<<(std::ostream& out, const segment_tail_t& tail)
{
  return out << "segment_tail_t(" << tail.physical_segment_id << " "
             << tail.type << " " << segment_seq_printer_t{tail.segment_seq}
             << ", segment_nonce=0x" << std::hex << tail.segment_nonce
             << std::dec
...
void
record_size_t::account_extent(extent_len_t extent_len)
{
  assert(extent_len);
  if (record_type == record_type_t::JOURNAL) {
...
void
record_size_t::account(const delta_info_t& delta)
{
...
std::ostream&
operator<<(std::ostream& os, transaction_type_t type)
{
  switch (type) {
  ...
  default:
    return os << "INVALID_TRANS_TYPE(" << static_cast<std::size_t>(type) << ")";
  }
}

std::ostream&
operator<<(std::ostream& out, const record_size_t& rsize)
{
  return out << "record_size_t(" << "record_type=" << rsize.record_type
             << "raw_md=0x" << std::hex << rsize.get_raw_mdlength()
             << ", data=0x" << rsize.dlength << std::dec << ")";
}

std::ostream&
operator<<(std::ostream& out, const record_type_t& type)
{
  switch (type) {
  ...
  case record_type_t::MAX:
    return out << "NULL";
  default:
    return out << "INVALID_RECORD_TYPE(" << static_cast<std::size_t>(type)
               << ")";
  }
}

std::ostream&
operator<<(std::ostream& out, const record_t& r)
{
  return out << "record_t(" << "trans_type=" << r.trans_type
...
std::ostream&
operator<<(std::ostream& out, const record_header_t& r)
{
  return out << "record_header_t(" << "type=" << r.type
             << ", num_extents=" << r.extents << ", num_deltas=" << r.deltas
             << ", modify_time=" << mod_time_point_printer_t{r.modify_time}
             << ")";
}

std::ostream&
operator<<(std::ostream& out, const record_group_header_t& h)
{
  return out << "record_group_header_t(" << "num_records=" << h.records
             << ", mdlength=0x" << std::hex << h.mdlength << ", dlength=0x"
             << h.dlength << ", segment_nonce=0x" << h.segment_nonce << std::dec
             << ", committed_to=" << h.committed_to << ", data_crc=0x"
             << std::hex << h.data_crc << std::dec << ")";
}
...
void
record_group_size_t::account(const record_size_t& rsize, extent_len_t _block_size)
{
  // empty record is allowed to submit
  assert(_block_size > 0);
  assert(rsize.dlength % _block_size == 0);
  assert(block_size == 0 || block_size == _block_size);
  assert(record_type == RECORD_TYPE_NULL || record_type == rsize.record_type);
...
std::ostream&
operator<<(std::ostream& out, const record_group_size_t& size)
{
  return out << "record_group_size_t(" << "record_type=" << size.record_type
             << "raw_md=0x" << std::hex << size.get_raw_mdlength()
             << ", data=0x" << size.dlength << ", block_size=0x"
             << size.block_size << std::dec
             << ", fullness=" << size.get_fullness() << ")";
}

std::ostream&
operator<<(std::ostream& out, const record_group_t& rg)
{
  return out << "record_group_t(" << "num_records=" << rg.records.size() << ", "
             << rg.size << ")";
}
    ...
    segment_nonce_t current_segment_nonce)
{
  ...
  if (record_group.size.record_type == record_type_t::OOL) {
    ...
    ceph_assert(data_bl.length() == record_group.size.get_encoded_length());
    record_group.clear();
    return data_bl;
  }
  // JOURNAL
  ...
  for (auto& r : record_group.records) {
    record_header_t rheader{
        r.trans_type, (extent_len_t)r.deltas.size(),
        (extent_len_t)r.extents.size(), timepoint_to_mod(r.modify_time)};
    encode(rheader, bl);
  }
  for (auto& r : record_group.records) {
  ...
  auto metadata_crc =
      bliter.crc32c(ceph::encoded_sizeof_bounded<record_group_header_t>(), -1);
  ...
  metadata_crc_filler.copy_in(
      sizeof(checksum_t), reinterpret_cast<const char*>(&metadata_crc_le));
    ...
    segment_nonce_t expected_nonce)
{
  ...
  if (header.segment_nonce != expected_nonce) {
    journal_logger().debug(
        "try_decode_records_header: failed, record_group_header nonce "
        "mismatch, "
        "read {}, expected {}!",
        header.segment_nonce, expected_nonce);
    return std::nullopt;
  }
  return header;
}
...
bool
validate_records_metadata(const ceph::bufferlist& md_bl)
{
    ...
    const ceph::bufferlist& md_bl)
{
  ...
  bliter += sizeof(checksum_t); /* metadata crc hole */
  bliter +=
      (ceph::encoded_sizeof_bounded<record_header_t>() * maybe_headers->size());
    ...
    paddr_t record_block_base)
{
  ...
  for (auto& r : *maybe_record_extent_infos) {
    bliter +=
        (ceph::encoded_sizeof_bounded<extent_info_t>() * r.extent_infos.size());
  }
...
std::ostream&
operator<<(std::ostream& out, placement_hint_t h)
{
  switch (h) {
...
bool
can_delay_allocation(device_type_t type)
{
...
std::ostream&
operator<<(std::ostream& out, device_type_t t)
{
  switch (t) {
...
std::ostream&
operator<<(std::ostream& out, backend_type_t btype)
{
...
std::ostream&
operator<<(std::ostream& out, const write_result_t& w)
{
  return out << "write_result_t(" << "start=" << w.start_seq << ", length=0x"
             << std::hex << w.length << std::dec << ")";
}

std::ostream&
operator<<(std::ostream& out, const record_locator_t& l)
{
  return out << "record_locator_t(" << "block_base=" << l.record_block_base
             << ", " << l.write_result << ")";
}

void
scan_valid_records_cursor::emplace_record_group(
    const record_group_header_t& header,
    ceph::bufferlist&& md_bl)
{
  auto new_committed_to = header.committed_to;
  ceph_assert(
      last_committed == JOURNAL_SEQ_NULL || last_committed <= new_committed_to);
...
std::ostream&
operator<<(std::ostream& out, const writer_stats_printer_t& p)
{
  constexpr const char* dfmt = "{:.2f}";
  auto d_num_io = static_cast<double>(p.stats.io_depth_stats.num_io);
  out << "iops=" << fmt::format(dfmt, d_num_io / p.seconds)
      << ",depth=" << fmt::format(dfmt, p.stats.io_depth_stats.average())
      << ",batch=" << fmt::format(dfmt, p.stats.record_batch_stats.average())
      << ",bwMiB="
      << fmt::format(dfmt, p.stats.get_total_bytes() / p.seconds / (1 << 20))
      << ",sizeB=" << fmt::format(dfmt, p.stats.get_total_bytes() / d_num_io)
...
std::ostream&
operator<<(std::ostream& out, const cache_size_stats_t& p)
{
  constexpr const char* dfmt = "{:.2f}";
  out << "(" << fmt::format(dfmt, p.get_mb()) << "MiB,"
      << fmt::format(dfmt, p.get_avg_kb()) << "KiB," << p.num_extents << ")";
  return out;
}

std::ostream&
operator<<(std::ostream& out, const cache_size_stats_printer_t& p)
{
  constexpr const char* dfmt = "{:.2f}";
  out << "(" << fmt::format(dfmt, p.stats.get_mb() / p.seconds) << "MiB/s,"
      << fmt::format(dfmt, p.stats.get_avg_kb()) << "KiB,"
      << fmt::format(dfmt, p.stats.num_extents / p.seconds) << "ps)";
  return out;
}

std::ostream&
operator<<(std::ostream& out, const cache_io_stats_printer_t& p)
{
  out << "in" << cache_size_stats_printer_t{p.seconds, p.stats.in_sizes}
      << " out" << cache_size_stats_printer_t{p.seconds, p.stats.out_sizes};
  return out;
}

std::ostream&
operator<<(std::ostream& out, const dirty_io_stats_printer_t& p)
{
  constexpr const char* dfmt = "{:.2f}";
  out << "in" << cache_size_stats_printer_t{p.seconds, p.stats.in_sizes}
      << " replaces=" << fmt::format(dfmt, p.stats.num_replace / p.seconds)
      << "ps out" << cache_size_stats_printer_t{p.seconds, p.stats.out_sizes}
      << " outv=" << fmt::format(dfmt, p.stats.get_avg_out_version());
  return out;
}

std::ostream&
operator<<(std::ostream& out, const cache_access_stats_printer_t& p)
{
  constexpr const char* dfmt = "{:.2f}";
  double total_access = static_cast<double>(p.stats.get_total_access());
  out << "(";
  if (total_access > 1000000) {
    out << fmt::format(dfmt, total_access / 1000000) << "M, ";
  } else {
    out << fmt::format(dfmt, total_access / 1000) << "K, ";
  }
  ...
  out << "trans-hit=" << fmt::format(dfmt, trans_hit / total_access * 100)
      << "%(pend" << fmt::format(dfmt, p.stats.trans_pending / trans_hit)
      << ",dirt" << fmt::format(dfmt, p.stats.trans_dirty / trans_hit) << ",lru"
      << fmt::format(dfmt, p.stats.trans_lru / trans_hit)
      << "), cache-hit=" << fmt::format(dfmt, cache_hit / cache_access * 100)
      << "%(dirt" << fmt::format(dfmt, p.stats.cache_dirty / cache_hit)
      << ",lru" << fmt::format(dfmt, p.stats.cache_lru / cache_hit)
      << "), load-present/absent="
...
std::ostream&
operator<<(std::ostream& out, const omap_type_t& t)
{
  switch (t) {
