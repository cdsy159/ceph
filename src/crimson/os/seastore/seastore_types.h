...
public:
  ...
  bool
  operator==(const cache_hint_t& other) const
  {
...
private:
  constexpr cache_hint_t(hint_t hint) :
    hint(hint)
  {}
...
constexpr std::size_t SEASTORE_SUPERBLOCK_SIGN_LEN =
    sizeof(SEASTORE_SUPERBLOCK_SIGN) - 1;
...
constexpr bool
has_device_off(device_id_t id)
{
  return id == DEVICE_ID_RECORD_RELATIVE || id == DEVICE_ID_BLOCK_RELATIVE ||
         id == DEVICE_ID_DELAYED || id == DEVICE_ID_FAKE ||
         id == DEVICE_ID_ROOT;
}
...
constexpr auto SEGMENT_ID_BITS =
    std::numeric_limits<internal_segment_id_t>::digits;
...
public:
  ...
  {
    constexpr internal_segment_id_t _SEGMENT_ID_MASK =
        (1u << DEVICE_SEGMENT_ID_BITS) - 1;
    return segment & _SEGMENT_ID_MASK;
  }

  bool
  operator==(const segment_id_t& other) const
  {
  ...
  static constexpr segment_id_t
  create_const(device_id_t id, device_segment_id_t segment)
  {
...
private:
  ...
  constexpr static inline internal_segment_id_t
  make_internal(device_id_t d_id, device_segment_id_t s_id)
  {
...
// segment_id_t() == MAX_SEG_ID == NULL_SEG_ID
constexpr segment_id_t MAX_SEG_ID = segment_id_t::create_const(
    DEVICE_ID_MAX_VALID_SEGMENT,
...
public:
  ...
  void
  add_device(device_id_t device, std::size_t segments, const T& init)
  {
  ...
  const T&
  operator[](segment_id_t id) const
  {
    assert(id.device_segment_id() < device_to_segments[id.device_id()].size());
    return device_to_segments[id.device_id()][id.device_segment_id()];
  }
...
private:
  ...
  class iterator {
    ...
    std::optional<
        std::pair<const segment_id_t, std::conditional_t<is_const, const T&, T&>>>
        current;
    ...
    iterator(
        ...
        device_id_t device_id,
        device_segment_id_t device_segment_id) :
      parent(parent), device_id(device_id), device_segment_id(device_segment_id)
    {}
  ...
  public:
    ...
    static iterator
    end_iterator(decltype(parent)& parent)
    {
    ...
    bool
    operator==(iterator<is_const> rit)
    {
      return (
          device_id == rit.device_id &&
    ...
    const std::pair<const segment_id_t, const T&>*
    operator->()
    {
      assert(!is_end());
      return &*current;
...
constexpr auto DEVICE_OFF_MAX = std::numeric_limits<device_off_t>::max() >>
                                DEVICE_ID_BITS;
...
constexpr auto DEVICE_ID_MASK = ((internal_paddr_t(1) << DEVICE_ID_BITS) - 1)
                                << DEVICE_OFF_BITS;
constexpr auto DEVICE_OFF_MASK = std::numeric_limits<u_device_off_t>::max() >>
                                 DEVICE_ID_BITS;
constexpr auto SEGMENT_ID_MASK = ((internal_paddr_t(1) << SEGMENT_ID_BITS) - 1)
                                 << SEGMENT_OFF_BITS;
constexpr auto SEGMENT_OFF_MASK = (internal_paddr_t(1) << SEGMENT_OFF_BITS) - 1;

constexpr internal_paddr_t
encode_device_off(device_off_t off)
{
  return static_cast<internal_paddr_t>(off) & DEVICE_OFF_MASK;
}

constexpr device_off_t
...
public:
  ...
  static paddr_t
  make_seg_paddr(segment_id_t seg, segment_off_t offset)
  {
  ...
  static paddr_t
  make_blk_paddr(device_id_t device, device_off_t offset)
  {
  ...
  static paddr_t
  make_res_paddr(device_id_t device, device_off_t offset)
  {
  ...
  paddr_t
  add_block_relative(paddr_t o) const
  {
  ...
  paddr_t
  add_record_relative(paddr_t o) const
  {
    // special version mainly for documentation purposes
   ...
   */
  paddr_t
  maybe_relative_to(paddr_t base) const
  {
  ...
  bool
  is_block_relative() const
  {
    return get_device_id() == DEVICE_ID_BLOCK_RELATIVE;
  }

  bool
  is_record_relative() const
  {
  ...
  is_absolute() const
  {
    ...
    return get_addr_type() != paddr_types_t::RESERVED || is_fake();
#else
  ...
  bool
  is_absolute_random_block() const
  {
    return get_addr_type() == paddr_types_t::RANDOM_BLOCK;
  }

  bool
  is_absolute_segmented() const
  {
  ...
  constexpr static paddr_t
  create_const(device_id_t d_id, device_off_t offset)
  {
...
private:
  ...
  paddr_t(segment_id_t seg, segment_off_t offset) :
    paddr_t(
        (static_cast<internal_paddr_t>(seg.segment) << SEGMENT_OFF_BITS) |
  ...
  paddr_t(device_id_t d_id, device_off_t offset) :
    paddr_t(
        (static_cast<internal_paddr_t>(d_id) << DEVICE_OFF_BITS) |
...
struct seg_paddr_t : public paddr_t {
  ...
  {
    return segment_id_t(
        static_cast<internal_segment_id_t>(internal_paddr >> SEGMENT_OFF_BITS));
  }
  ...
  void
  set_segment_off(segment_off_t off)
  {
    assert(off >= 0);
  ...
  paddr_t
  add_offset(device_off_t o) const
  {
...
struct blk_paddr_t : public paddr_t {
  ...
  void
  set_device_off(device_off_t off)
  {
    assert(off >= 0);
...
struct res_paddr_t : public paddr_t {
  ...
  paddr_t
  block_relative_to(const res_paddr_t& rhs) const
  {
...
struct journal_seq_t {
  ...
  bool
  operator==(const journal_seq_t& o) const
  {
    return cmp(o) == 0;
  }
...
private:
  int
  cmp(const journal_seq_t& other) const
  {
...
public:
  ...
  static constexpr Unsigned RAW_VALUE_MAX = std::numeric_limits<Unsigned>::max();

  constexpr laddr_t() :
    laddr_t(RAW_VALUE_MAX)
  {}
  ...
  static laddr_t
  from_byte_offset(Unsigned value)
  {
    assert((value & UNIT_MASK) == 0);
    return laddr_t(value >> UNIT_SHIFT);
  }

  static constexpr laddr_t
  from_raw_uint(Unsigned v)
  {
    return laddr_t(v);
  }
  ...
  {
    assert(
        static_cast<std::size_t>(p.get_end() - p.get_pos()) >= sizeof(Unsigned));
  ...
  struct laddr_offset_t {
    ...
    laddr_t
    get_roundup_laddr(size_t alignment) const
    {
    ...
    laddr_t
    get_aligned_laddr(size_t alignment) const
    {
      ceph_assert(alignment % laddr_t::UNIT_SIZE == 0);
    ...
    bool
    is_aligned(size_t alignment) const
    {
      assert(alignment % laddr_t::UNIT_SIZE == 0);
      return !has_offset() && base % (alignment >> UNIT_SHIFT) == 0;
    }

    laddr_t
    checked_to_laddr() const
    {
      assert(offset == 0);
      return laddr_t(base);
    }
    ...
    U
    get_byte_distance(const laddr_t& l) const
    {
    ...
    template <std::unsigned_integral U>
    U
    get_byte_distance(const laddr_offset_t& l) const
    {
      assert(offset < UNIT_SIZE);
      if (*this >= l) {
    ...
    friend bool operator==(const laddr_offset_t&, const laddr_offset_t&) =
        default;
    friend auto operator<=>(const laddr_offset_t&, const laddr_offset_t&) =
        default;
    friend std::ostream& operator<<(std::ostream&, const laddr_offset_t&);

    friend laddr_offset_t
    operator+(const laddr_offset_t& laddr_offset, const loffset_t& offset)
    {
    ...
    friend laddr_offset_t
    operator+(const loffset_t& offset, const laddr_offset_t& loffset)
    {
    ...
    operator-(const laddr_offset_t& laddr_offset, const loffset_t& offset)
    {
      if (laddr_offset.get_offset() >= offset) {
        return laddr_offset_t(
            laddr_offset.get_laddr(), laddr_offset.get_offset() - offset);
      } else {
        // laddr_offset_t could access (laddr_t - loffset_t) overload.
        return laddr_offset.get_laddr() - (offset - laddr_offset.get_offset());
      }
    }
  ...
  get_byte_distance(const laddr_t& l) const
  {
    Unsigned diff = value > l.value ? value - l.value : l.value - value;
  ...
  friend bool
  operator==(const laddr_t& laddr, const laddr_offset_t& laddr_offset)
  {
    return laddr == laddr_offset.get_laddr() && 0 == laddr_offset.get_offset();
  }

  friend bool
  operator==(const laddr_offset_t& laddr_offset, const laddr_t& laddr)
  {
    return laddr_offset.get_laddr() == laddr && laddr_offset.get_offset() == 0;
  }
  ...
  friend laddr_offset_t
  operator+(const laddr_t& laddr, const loffset_t& offset)
  {
    auto base = laddr;
  ...
  friend laddr_offset_t
  operator+(const loffset_t& offset, const laddr_t& laddr)
  {
    return laddr + offset;
  }

  friend laddr_offset_t
  operator-(const laddr_t& laddr, loffset_t offset)
  {
    auto base = laddr;
  ...
  struct laddr_hash_t {
    std::size_t
    operator()(const laddr_t& laddr) const
    {
...
private:
  ...
  constexpr explicit laddr_t(Unsigned value) :
    value(value)
  {}
...
struct __attribute__((packed)) pladdr_le_t {
  ...
  explicit pladdr_le_t(const pladdr_t& addr) :
    pladdr(ceph_le64(
        addr.is_laddr() ? std::get<0>(addr.pladdr).value
...
constexpr bool
is_data_type(extent_types_t type)
{
...
constexpr bool
is_logical_metadata_type(extent_types_t type)
{
...
constexpr bool
is_logical_type(extent_types_t type)
{
  if ((type >= extent_types_t::ROOT_META &&
       type <= extent_types_t::OBJECT_DATA_BLOCK) ||
      type == extent_types_t::TEST_BLOCK || type == extent_types_t::LOG_NODE) {
    assert(is_logical_metadata_type(type) || is_data_type(type));
    return true;
  } else {
    assert(!is_logical_metadata_type(type) && !is_data_type(type));
    return false;
...
constexpr bool
is_retired_placeholder_type(extent_types_t type)
{
  return type == extent_types_t::RETIRED_PLACEHOLDER;
}

constexpr bool
is_root_type(extent_types_t type)
{
  return type == extent_types_t::ROOT;
}

constexpr bool
is_lba_node(extent_types_t type)
{
...
constexpr bool
is_backref_node(extent_types_t type)
{
...
constexpr bool
is_backref_mapped_type(extent_types_t type)
{
  if ((type >= extent_types_t::LADDR_INTERNAL &&
      ...
      type == extent_types_t::LOG_NODE) {
    assert(
        is_logical_type(type) || is_lba_node(type) ||
        type == extent_types_t::TEST_BLOCK_PHYSICAL);
    return true;
  } else {
    assert(
...
constexpr bool
is_real_type(extent_types_t type)
{
  if (type <= extent_types_t::OBJECT_DATA_BLOCK ||
      (type >= extent_types_t::TEST_BLOCK && type <= extent_types_t::LOG_NODE)) {
    assert(is_logical_type(type) || is_physical_type(type));
    return true;
  } else {
    assert(!is_logical_type(type) && !is_physical_type(type));
    return false;
...
generation_to_writer(rewrite_gen_t gen)
{
...
// before EPM decision
constexpr bool
is_target_rewrite_generation(rewrite_gen_t gen, rewrite_gen_t max_gen)
{
  return gen == INIT_GENERATION ||
         (gen >= MIN_REWRITE_GENERATION && gen <= max_gen + 1);
}

// after EPM decision
constexpr bool
is_rewrite_generation(rewrite_gen_t gen, rewrite_gen_t max_gen)
{
  return gen >= INLINE_GENERATION && gen <= max_gen;
}
...
{
  return sea_time_point(
      std::chrono::duration_cast<sea_duration>(std::chrono::milliseconds(t)));
}
...
constexpr sea_time_point
get_average_time(
    const sea_time_point& t1,
    std::size_t n1,
    const sea_time_point& t2,
    std::size_t n2)
{
  assert(t1 != NULL_TIME);
  assert(t2 != NULL_TIME);
...
struct delta_info_t {
  ...
  bool
  operator==(const delta_info_t& rhs) const
  {
    return (
        type == rhs.type && paddr == rhs.paddr && laddr == rhs.laddr &&
        prev_crc == rhs.prev_crc && final_crc == rhs.final_crc &&
        length == rhs.length && pversion == rhs.pversion &&
...
public:
  object_data_t(laddr_t reserved_data_base, extent_len_t reserved_data_len) :
    reserved_data_base(reserved_data_base), reserved_data_len(reserved_data_len)
  {}
  ...
  void
  update_reserved(laddr_t base, extent_len_t len)
  {
    dirty = true;
...
constexpr object_data_t
get_null_object_data()
{
...
struct __attribute__((packed)) object_data_le_t {
  ...
  void
  update(const object_data_t& nroot)
  {
  ...
  get() const
  {
    return object_data_t(reserved_data_base, reserved_data_len);
  }
};
...
struct omap_root_t {
  ...
  omap_root_t(laddr_t addr, depth_t depth, laddr_t addr_min, omap_type_t type) :
    addr(addr), depth(depth), hint(addr_min), type(type)
  {}
  ...
  void
  update(laddr_t _addr, depth_t _depth, laddr_t _hint, omap_type_t _type)
  {
    mutated = true;
    addr = _addr;
    depth = _depth;
...
public:
  ...
  omap_root_le_t(omap_type_t type) :
    type(type)
  {}
  ...
  void
  update(const omap_root_t& nroot)
  {
...
public:
  ...
  void
  set_depth(depth_t ndepth)
  {
    depth = ndepth;
  }

  void
  adjust_addrs_from_base(paddr_t base)
  {
...
public:
  ...
  void
  update(laddr_t _addr, extent_len_t _s)
  {
    mutated = true;
    addr = _addr;
    size = _s;
...
public:
  ...
  void
  update(const coll_root_t& nroot)
  {
    addr = nroot.get_location();
...
struct alloc_blk_t {
  ...
  static alloc_blk_t
  create_retire(const paddr_t& paddr, extent_len_t len, extent_types_t type)
  {
    assert(
        is_backref_mapped_type(type) || is_backref_node(type) ||
...
static constexpr auto TRANSACTION_TYPE_MAX =
    static_cast<std::size_t>(transaction_type_t::MAX);

std::ostream& operator<<(std::ostream& os, transaction_type_t type);

constexpr bool
is_valid_transaction(transaction_type_t type)
{
  return type < transaction_type_t::MAX;
}

constexpr bool
is_user_transaction(transaction_type_t type)
{
  return type <= transaction_type_t::READ;
}

constexpr bool
is_background_transaction(transaction_type_t type)
{
  return (
      type >= transaction_type_t::TRIM_DIRTY && type < transaction_type_t::MAX);
}

constexpr bool
is_rewrite_transaction(transaction_type_t type)
{
...
constexpr bool
is_trim_transaction(transaction_type_t type)
{
  return (
      type == transaction_type_t::TRIM_DIRTY ||
      type == transaction_type_t::TRIM_ALLOC);
}

constexpr bool
is_modify_transaction(transaction_type_t type)
{
  return (type == transaction_type_t::MUTATE || is_background_transaction(type));
}
...
struct record_size_t {
  ...
  is_empty() const
  {
    return plain_mdlength == 0 && dlength == 0;
  }

  void account_extent(extent_len_t extent_len);

  void
  account(const extent_t& extent)
  {
...
struct record_t {
  ...
  record_t(record_type_t r_type, transaction_type_t t_type) :
    trans_type{t_type}
  {
  ...
  is_empty() const
  {
    return extents.size() == 0 && deltas.size() == 0;
  }

  std::size_t
  get_delta_size() const
  {
    ...
    auto delta_size = std::accumulate(
        ...
        [](uint64_t sum, auto& delta) { return sum + delta.bl.length(); });
    return delta_size;
  }

  void
  push_back(extent_t&& extent, sea_time_point& t)
  {
...
struct record_group_size_t {
  ...
  record_group_size_t(const record_size_t& rsize, extent_len_t block_size)
  {
...
struct record_group_t {
  ...
  record_group_t(record_t&& record, extent_len_t block_size)
  {
  ...
  void
  push_back(record_t&& record, extent_len_t block_size)
  {
...
std::optional<std::vector<record_extent_infos_t>> try_decode_extent_infos(
    const record_group_header_t& header,
    const ceph::bufferlist& md_bl);
std::optional<std::vector<record_header_t>> try_decode_record_headers(
    const record_group_header_t& header,
    const ceph::bufferlist& md_bl);
...
std::optional<std::vector<record_deltas_t>> try_decode_deltas(
    const record_group_header_t& header,
    const ceph::bufferlist& md_bl,
    paddr_t record_block_base);
...
struct write_result_t {
  ...
  get_end_seq() const
  {
    return journal_seq_t{
        start_seq.segment_seq, start_seq.offset.add_offset(length)};
  }
};
...
struct scan_valid_records_cursor {
  ...
  void
  increment_seq(segment_off_t off)
  {
...
template <typename CounterT>
CounterT&
get_by_src(counter_by_src_t<CounterT>& counters_by_src, transaction_type_t src)
{
...
template <typename CounterT>
void
add_srcs(counter_by_src_t<CounterT>& base, const counter_by_src_t<CounterT>& by)
{
...
template <typename CounterT>
void
minus_srcs(counter_by_src_t<CounterT>& base, const counter_by_src_t<CounterT>& by)
{
  for (std::size_t i = 0; i < TRANSACTION_TYPE_MAX; ++i) {
    base[i] -= by[i];
  }
...
template <typename CounterT>
CounterT&
get_by_ext(counter_by_extent_t<CounterT>& counters_by_ext, extent_types_t ext)
{
...
struct grouped_io_stats {
  ...
  void
  minus(const grouped_io_stats& o)
  {
    num_io -= o.num_io;
    num_io_grouped -= o.num_io_grouped;
  }
  ...
  void
  increment(uint64_t num_grouped_io)
  {
...
struct device_stats_t {
  ...
  void
  add(const device_stats_t& other)
  {
    num_io += other.num_io;
...
struct writer_stats_t {
  ...
  get_total_bytes() const
  {
    return record_group_padding_bytes + record_group_metadata_bytes +
           data_bytes;
  }

  void
  add(const writer_stats_t& o)
  {
...
struct shard_stats_t {
  ...
  get_bg_num() const
  {
    return trim_alloc_num + trim_dirty_num + cleaner_main_num +
           cleaner_cold_num;
  }

  uint64_t
  get_repeat_bg_num() const
  {
    return repeat_trim_alloc_num + repeat_trim_dirty_num +
           repeat_cleaner_main_num + repeat_cleaner_cold_num;
  }

  void
  add(const shard_stats_t& o)
  {
    io_num += o.io_num;
...
struct cache_size_stats_t {
  ...
  void
  account_parital_in(extent_len_t sz)
  {
    size += sz;
  }

  void
  account_out(extent_len_t sz)
  {
    assert(size >= sz);
  ...
  void
  add(const cache_size_stats_t& o)
  {
    size += o.size;
  ...
  void
  minus(const cache_size_stats_t& o)
  {
    size -= o.size;
    num_extents -= o.num_extents;
  }
...
struct cache_io_stats_t {
  ...
  void
  minus(const cache_io_stats_t& o)
  {
...
struct dirty_io_stats_t {
  ...
  is_empty() const
  {
    return in_sizes.is_empty() && num_replace == 0 && out_sizes.is_empty();
  }

  void
  add(const dirty_io_stats_t& o)
  {
    in_sizes.add(o.in_sizes);
  ...
  void
  minus(const dirty_io_stats_t& o)
  {
    in_sizes.minus(o.in_sizes);
    num_replace -= o.num_replace;
    out_sizes.minus(o.out_sizes);
...
struct cache_access_stats_t {
  ...
  uint64_t
  get_cache_access() const
  {
    return get_cache_hit() + load_absent;
  }

  uint64_t
  get_total_access() const
  {
  ...
  void
  add(const cache_access_stats_t& o)
  {
  ...
  void
  minus(const cache_access_stats_t& o)
  {
    trans_pending -= o.trans_pending;
    trans_dirty -= o.trans_dirty;
    trans_lru -= o.trans_lru;
...
template <>
struct fmt::formatter<crimson::os::seastore::cache_access_stats_printer_t>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::os::seastore::cache_io_stats_printer_t>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::os::seastore::cache_size_stats_t>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::os::seastore::cache_size_stats_printer_t>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::os::seastore::data_category_t>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::os::seastore::delta_info_t>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::os::seastore::device_id_printer_t>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::os::seastore::dirty_io_stats_printer_t>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::os::seastore::extent_types_t>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::os::seastore::journal_seq_t>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::os::seastore::backend_type_t>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::os::seastore::journal_tail_delta_t>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::os::seastore::laddr_t> : fmt::ostream_formatter {
};

template <>
struct fmt::formatter<crimson::os::seastore::laddr_offset_t>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::os::seastore::laddr_list_t>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::os::seastore::omap_root_t>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::os::seastore::paddr_list_t>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::os::seastore::paddr_t> : fmt::ostream_formatter {
};

template <>
struct fmt::formatter<crimson::os::seastore::pladdr_t>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::os::seastore::placement_hint_t>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::os::seastore::device_type_t>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::os::seastore::record_group_header_t>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::os::seastore::record_group_size_t>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::os::seastore::record_header_t>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::os::seastore::record_locator_t>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::os::seastore::record_type_t>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::os::seastore::record_t>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::os::seastore::rewrite_gen_printer_t>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::os::seastore::scan_valid_records_cursor>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::os::seastore::sea_time_point_printer_t>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::os::seastore::segment_header_t>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::os::seastore::segment_id_t>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::os::seastore::segment_seq_printer_t>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::os::seastore::segment_tail_t>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::os::seastore::segment_type_t>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::os::seastore::transaction_type_t>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::os::seastore::write_result_t>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::os::seastore::omap_type_t>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<ceph::buffer::list> : fmt::ostream_formatter {};
#endif
...
struct std::hash<crimson::os::seastore::laddr_t> {
  ...
  std::size_t
  operator()(const Laddr& laddr) const
  {
