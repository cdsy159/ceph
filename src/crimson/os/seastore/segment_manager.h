...
struct block_sm_superblock_t {
  ...
  validate() const
  {
    ...
    for (unsigned int i = 0; i < shard_num; i++) {
      ceph_assert(
          shard_infos[i].size > segment_size &&
      ...
      ceph_assert(
          shard_infos[i].tracker_offset > 0 &&
          shard_infos[i].tracker_offset % block_size == 0);
      ceph_assert(
          shard_infos[i].first_segment_offset > shard_infos[i].tracker_offset &&
...
class Segment
  : public boost::intrusive_ref_counter<Segment, boost::thread_unsafe_counter> {
public:
...
public:
  ...
  static seastar::future<SegmentManagerRef> get_segment_manager(
...
template <>
struct fmt::formatter<crimson::os::seastore::block_shard_info_t>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::os::seastore::block_sm_superblock_t>
  : fmt::ostream_formatter {};
#endif
