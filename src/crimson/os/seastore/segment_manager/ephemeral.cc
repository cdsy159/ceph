...
std::ostream&
operator<<(std::ostream& lhs, const ephemeral_config_t& c)
{
  return lhs << "ephemeral_config_t(size=0x" << std::hex << c.size
             << ", block_size=0x" << c.block_size << ", segment_size=0x"
             << c.segment_size << std::dec << ")";
}
    ...
    std::size_t num_cold_devices)
{
  ...
  if (index == 0) {
    ...
    for (std::size_t secondary_index = index + 1; secondary_index < num_devices;
         ++secondary_index) {
  ...
  return {
      is_major_device, device_spec_t{magic, get_sec_dtype(index), id}, meta,
      secondary_devices};
}
...
segment_off_t
EphemeralSegment::get_write_capacity() const
{
  return manager.get_segment_size();
}

Segment::close_ertr::future<>
EphemeralSegment::close()
{
  return manager.segment_close(id).safe_then([] { return seastar::yield(); });
}

Segment::write_ertr::future<>
EphemeralSegment::write(segment_off_t offset, ceph::bufferlist bl)
{
...
Segment::write_ertr::future<>
EphemeralSegment::advance_wp(segment_off_t offset)
{
  return write_ertr::now();
}

Segment::close_ertr::future<>
EphemeralSegmentManager::segment_close(segment_id_t id)
{
  ...
  return Segment::close_ertr::now().safe_then([] { return seastar::yield(); });
}
...
Segment::write_ertr::future<>
EphemeralSegmentManager::segment_write(
    paddr_t addr,
    ceph::bufferlist bl,
    bool ignore_check)
{
  ...
  logger().debug(
      ...
      bl.length(), bl.crc32c(1));
  if (!ignore_check &&
      segment_state[seg_addr.get_segment_id().device_segment_id()] !=
          segment_state_t::OPEN)
    return crimson::ct_error::invarg::make();

  bl.begin().copy(bl.length(), buffer + get_offset(addr));
  return Segment::write_ertr::now().safe_then([] { return seastar::yield(); });
}

EphemeralSegmentManager::init_ertr::future<>
EphemeralSegmentManager::init()
{
  ...
  void* addr = ::mmap(
      nullptr, config.size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS,
      -1, 0);

  segment_state.resize(
      config.size / config.segment_size, segment_state_t::EMPTY);
  ...
  return init_ertr::now().safe_then([] { return seastar::yield(); });
}
...
void
EphemeralSegmentManager::remount()
{
...
EphemeralSegmentManager::open(segment_id_t id)
{
  auto s_id = id.device_segment_id();
  ...
  return open_ertr::make_ready_future<SegmentRef>(
      new EphemeralSegment(*this, id));
}

SegmentManager::release_ertr::future<>
EphemeralSegmentManager::release(segment_id_t id)
{
  auto s_id = id.device_segment_id();
  ...
  if (segment_state[s_id] != segment_state_t::CLOSED) {
    logger().error(
        "EphemeralSegmentManager::release: segment id {} not closed", id);
    return crimson::ct_error::invarg::make();
  }

  ::memset(
      buffer + get_offset(paddr_t::make_seg_paddr(id, 0)), 0,
      config.segment_size);
  segment_state[s_id] = segment_state_t::EMPTY;
  return release_ertr::now().safe_then([] { return seastar::yield(); });
}

SegmentManager::read_ertr::future<>
EphemeralSegmentManager::read(paddr_t addr, size_t len, ceph::bufferptr& out)
{
  auto& seg_addr = addr.as_seg_paddr();
  ...
  if (seg_addr.get_segment_off() + len > config.segment_size) {
    logger().error(
        "EphemeralSegmentManager::read: invalid offset {}~0x{:x}!", addr, len);
    return crimson::ct_error::invarg::make();
  }
  ...
  logger().debug(
      ...
      seg_addr.get_segment_id().device_segment_id(), seg_addr.get_segment_off(),
      get_offset(addr), len, bl.begin().crc32c(len, 1));

  return read_ertr::now().safe_then([] { return seastar::yield(); });
}

SegmentManager::read_ertr::future<>
EphemeralSegmentManager::readv(paddr_t addr, std::vector<bufferptr> ptrs)
{
  size_t len = 0;
  for (auto& ptr : ptrs) {
    len += ptr.length();
  ...
  auto& seg_addr = addr.as_seg_paddr();
  if (seg_addr.get_segment_id().device_segment_id() >= get_num_segments()) {
    logger().error("EphemeralSegmentManager::readv: invalid segment {}", addr);
    return crimson::ct_error::invarg::make();
  }
