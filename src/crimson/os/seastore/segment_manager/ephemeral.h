...
class EphemeralSegmentManager final : public SegmentManager {
  ...
  get_offset(paddr_t addr)
  {
    ...
    return (seg_addr.get_segment_id().device_segment_id() *
            config.segment_size) +
...
public:
  ...
  read_ertr::future<> readv(paddr_t addr, std::vector<bufferptr> ptr) override;
  ...
  segment_off_t
  get_segment_size() const override
  {
  ...
  using init_ertr =
      crimson::errorator<crimson::ct_error::enospc, crimson::ct_error::invarg>;
...
template <>
struct fmt::formatter<crimson::os::seastore::segment_manager::ephemeral_config_t>
