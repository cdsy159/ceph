...
template <>
struct fmt::formatter<z_op> : fmt::formatter<std::string_view> {
  template <typename FormatContext>
  auto
  format(z_op s, FormatContext& ctx) const
  {
...
seastar::future<>
ZBDSegmentManager::start(uint32_t shard_nums)
{
...
seastar::future<>
ZBDSegmentManager::stop()
{
  return shard_devices.stop();
}

Device&
ZBDSegmentManager::get_sharded_device(store_index_t store_index)
{
...
open_device(const std::string& path, seastar::open_flags mode)
{
  ...
  return seastar::file_stat(path, seastar::follow_symlink::yes)
      .then([FNAME, mode, &path](auto stat) mutable {
        return seastar::open_file_dma(path, mode).then([=](auto file) {
          DEBUG("open of device {} successful, size 0x{:x}", path, stat.size);
    ...
    size_t num_zones)
{
  ...
  INFO(
      ...
      total_size, available_size, data.block_size, data.allocated_size,
      ...
      per_shard_segments, per_shard_available_size);
  ...
  for (unsigned int i = 0; i < seastar::smp::count; i++) {
    ...
    INFO(
        "First segment offset for shard {} is: 0x{:x}", i,
        shard_infos[i].first_segment_offset);
  }

  zbd_sm_metadata_t ret = zbd_sm_metadata_t{
      seastar::smp::count, segment_size,  zone_capacity * zones_per_segment,
      zones_per_segment,   zone_capacity, data.block_size,
      zone_size,           shard_infos,   meta};
  ret.validate();
  return ret;
}
...
get_blk_dev_size(seastar::file& device)
{
...
reset_device(seastar::file& device, uint64_t zone_size_sects, uint64_t nr_zones)
{
  return seastar::do_with(
      blk_zone_range{}, [&, nr_zones, zone_size_sects](auto& range) {
        range.sector = 0;
        range.nr_sectors = zone_size_sects * nr_zones;
        return device.ioctl(BLKRESETZONE, &range).then([&](int ret) {
          return seastar::now();
        });
      });
}
...
static seastar::future<size_t>
get_zone_capacity(seastar::file& device, uint32_t nr_zones)
{
...
static seastar::future<size_t>
get_nr_cnv_zones(seastar::file& device, uint32_t nr_zones)
{
  return seastar::do_with(ZoneReport(nr_zones), [&](auto& zr) {
    zr.hdr->sector = 0;
    zr.hdr->nr_zones = nr_zones;
...
do_write(seastar::file& device, uint64_t offset, bufferptr& bptr)
{
  LOG_PREFIX(ZBDSegmentManager::do_write);
  DEBUG("offset 0x{:x} len 0x{:x}", offset, bptr.length());
    ...
    size_t block_size)
{
  ...
  DEBUG(
      "{} offset 0x{:x} len 0x{:x}", device_id_printer_t{device_id}, offset,
      bl.length());
  ...
  return seastar::do_with(
      ...
      [&device, device_id, offset, FNAME](auto& iovs, auto& bl) {
        return write_ertr::parallel_for_each(
            iovs, [&device, device_id, offset, FNAME](auto& p) {
...
do_read(seastar::file& device, uint64_t offset, size_t len, bufferptr& bptr)
{
  LOG_PREFIX(ZBDSegmentManager::do_read);
  assert(len <= bptr.length());
  DEBUG("offset 0x{:x} len 0x{:x}", offset, len);
...
static ZBDSegmentManager::access_ertr::future<zbd_sm_metadata_t>
...
ZBDSegmentManager::read_ertr::future<uint32_t>
ZBDSegmentManager::get_shard_nums()
{
...
ZBDSegmentManager::mkfs_ret
ZBDSegmentManager::mkfs(device_config_t config)
{
  return shard_devices.local().mshard_devices[0]->primary_mkfs(config).safe_then(
      [this] {
        return shard_devices.invoke_on_all([](auto& local_device) {
          return seastar::do_for_each(
              local_device.mshard_devices, [](auto& mshard_device) {
...
ZBDSegmentManager::mkfs_ret
ZBDSegmentManager::primary_mkfs(device_config_t config)
{
  LOG_PREFIX(ZBDSegmentManager::primary_mkfs);
  INFO("starting, device_path {}", device_path);
  return seastar::do_with(
      seastar::file{}, seastar::stat_data{}, zbd_sm_metadata_t{}, size_t(),
      size_t(), size_t(), size_t(),
      [=, this](
          auto& device, auto& stat, auto& sb, auto& zone_size_sects,
          auto& nr_zones, auto& size, auto& nr_cnv_zones) {
        return open_device(device_path, seastar::open_flags::rw)
            .safe_then([=, this, &device, &stat, &sb, &zone_size_sects,
                        &nr_zones, &size, &nr_cnv_zones](auto p) {
              device = p.first;
              stat = p.second;
              return device.ioctl(BLKGETNRZONES, (void*)&nr_zones)
                  ...
                  .then([&, FNAME, config](auto zone_capacity_sects) {
                    ...
                    DEBUG(
                        "zone_size in sectors {}, zone_capacity in sectors {}",
                        zone_size_sects, zone_capacity_sects);
                    sb = make_metadata(
                        size, config.meta, stat, zone_size_sects,
                        zone_capacity_sects, nr_cnv_zones, nr_zones);
                    metadata = sb;
...
make_range(segment_id_t id, size_t segment_size, size_t first_segment_offset)
{
...
blk_zone_op(seastar::file& device, blk_zone_range& range, zone_op op)
{
  ...
  return device.ioctl(ioctl_op, &range)
...
ZBDSegmentManager::open_ertr::future<SegmentRef>
ZBDSegmentManager::open(segment_id_t id)
{
  ...
  return seastar::do_with(
             ...
             [=, this](auto& range) {
               range = make_range(
                   id, metadata.segment_size, shard_info.first_segment_offset);
...
ZBDSegmentManager::release_ertr::future<>
ZBDSegmentManager::release(segment_id_t id)
{
...
ZBDSegmentManager::readv(paddr_t addr, std::vector<bufferptr> ptrs)
{
  ...
  if (seg_addr.get_segment_off() + len > metadata.segment_capacity) {
    ERROR("invalid read offset {}, len 0x{:x}", addr, len);
    return crimson::ct_error::invarg::make();
  }
  return do_readv(get_device_id(), device, get_offset(addr), std::move(ptrs));
}

SegmentManager::read_ertr::future<>
ZBDSegmentManager::read(paddr_t addr, size_t len, ceph::bufferptr& out)
{
...
ZBDSegmentManager::segment_close(segment_id_t id, segment_off_t write_pointer)
{
    ...
    bool ignore_check)
{
  ...
  auto& seg_addr = addr.as_seg_paddr();
  DEBUG(
      "write to segment {} at offset 0x{:x}, physical offset 0x{:x}, len "
      "0x{:x}",
      seg_addr.get_segment_id(), seg_addr.get_segment_off(), get_offset(addr),
      bl.length());
...
Segment::write_ertr::future<>
ZBDSegment::write(segment_off_t offset, ceph::bufferlist bl)
{
  LOG_PREFIX(ZBDSegment::write);
  if (offset != write_pointer || offset % manager.metadata.block_size != 0) {
    ERROR(
        "Segment offset and zone write pointer mismatch. "
...
ZBDSegment::write_padding_bytes(size_t padding_bytes)
{
  ...
  DEBUG(
      "Writing 0x{:x} padding bytes to segment {} at wp 0x{:x}", padding_bytes,
      id, write_pointer);
  ...
  return crimson::repeat([padding_bytes, this]() mutable {
    ...
    return write(write_pointer, padd_bl).safe_then([padding_bytes]() {
      if (padding_bytes == 0) {
        return write_ertr::make_ready_future<seastar::stop_iteration>(
...
Segment::write_ertr::future<>
ZBDSegment::advance_wp(segment_off_t offset)
{
