...
SegmentManagerGroup::read_segment_tail(segment_id_t segment)
{
  ...
  return segment_manager
      ...
      .safe_then([=, &segment_manager](bufferptr bptr) -> read_segment_tail_ret {
        ...
        DEBUG(
            "segment {} block crc 0x{:x}", segment,
        ...
        try {
          ...
          DEBUG(
              "segment {} unable to decode tail, skipping -- {}", segment,
              e.what());
          return crimson::ct_error::enodata::make();
        }
        DEBUG("segment {} tail {}", segment, tail);
        return read_segment_tail_ret(
            read_segment_tail_ertr::ready_future_marker{}, tail);
      });
}
...
SegmentManagerGroup::read_segment_header(segment_id_t segment)
{
  ...
  auto& segment_manager = *segment_managers[segment.device_id()];
  return segment_manager
      .read(paddr_t::make_seg_paddr(segment, 0), get_rounded_header_length())
      .handle_error(
          read_segment_header_ertr::pass_further{},
          crimson::ct_error::assert_all{
              "Invalid error in SegmentManagerGroup::read_segment_header"})
      .safe_then(
          [=, &segment_manager](bufferptr bptr) -> read_segment_header_ret {
            ...
            try {
              ...
              DEBUG(
                  "segment {} unable to decode header, skipping -- {}", segment,
                  e.what());
              return crimson::ct_error::enodata::make();
            }
            DEBUG("segment {} header {}", segment, header);
            return read_segment_header_ret(
                read_segment_header_ertr::ready_future_marker{}, header);
          });
}

void
SegmentManagerGroup::initialize_cursor(scan_valid_records_cursor& cursor)
{
...
SegmentManagerGroup::find_journal_segment_headers()
{
  return seastar::do_with(
      get_segment_managers(), find_journal_segment_headers_ret_bare{},
                   ...
                   [this, &ret](SegmentManager* sm) {
                     ...
                     return crimson::do_for_each(
                         ...
                         boost::counting_iterator<device_segment_id_t>(
                             num_segments),
                         [this, &ret,
                          device_id](device_segment_id_t d_segment_id) {
                               ...
                               .safe_then([segment_id, &ret](auto&& header) {
                                 if (header.get_type() ==
                                   ...
                                   ret.emplace_back(std::make_pair(
                                       segment_id, std::move(header)));
                                 }
                               })
                               .handle_error(
                                   crimson::ct_error::enoent::handle([](auto) {
                                     return find_journal_segment_headers_ertr::
                                         now();
                                   }),
