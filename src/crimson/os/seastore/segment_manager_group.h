...
public:
  ...
  void
  add_segment_manager(SegmentManager* segment_manager)
  {
    ...
    if (!device_ids.empty()) {
      ...
      ceph_assert(
          segment_managers[existing_id]->get_device_type() ==
  ...
  std::size_t
  get_rounded_header_length() const
  {
    return p2roundup(
  ...
  using read_segment_header_ret =
      read_segment_header_ertr::future<segment_header_t>;
   ...
   */
  using find_journal_segment_headers_ertr =
      crimson::errorator<crimson::ct_error::input_output_error>;
  using find_journal_segment_headers_ret_bare =
      std::vector<std::pair<segment_id_t, segment_header_t>>;
  using find_journal_segment_headers_ret =
      find_journal_segment_headers_ertr::future<
  ...
  release_ertr::future<>
  release_segment(segment_id_t id)
  {
    assert(has_device(id.device_id()));
...
private:
  ...
  get_segment_end_offset(paddr_t addr) final
  {
    ...
    auto& segment_manager =
        *segment_managers[seg_addr.get_segment_id().device_id()];
