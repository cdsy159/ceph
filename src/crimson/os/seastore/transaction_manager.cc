...
TransactionManager::mkfs_ertr::future<>
  ...
  return epm->mount()
      ...
      .safe_then([this, FNAME]() {
        ...
        return with_transaction_intr(
                   Transaction::src_t::MUTATE, "mkfs_tm", CACHE_HINT_TOUCH,
                   [this, FNAME](auto& t) {
                     cache->init();
                     return cache->mkfs(t)
                         ...
                         .si_then([this, &t] { return init_root_meta(t); })
...
TransactionManager::mount()
{
  ...
  return epm->mount()
      .safe_then([this] {
        return journal->replay([this](
                                   const auto& offsets, const auto& e,
                                   const journal_seq_t& dirty_tail,
                                   const journal_seq_t& alloc_tail,
                                   sea_time_point modify_time) {
          auto start_seq = offsets.write_result.start_seq;
          return cache->replay_delta(
              start_seq, offsets.record_block_base, e, dirty_tail, alloc_tail,
              modify_time);
        });
      })
      ...
      .safe_then([this](auto start_seq) {
        journal->get_trimmer().set_journal_head(start_seq);
        return with_transaction_weak("mount", CACHE_HINT_TOUCH, [this](auto& t) {
          return cache
              ->init_cached_extents(
                  t,
                  [this](auto& t, auto& e) {
...
TransactionManager::ref_ret
TransactionManager::remove(Transaction& t, LogicalChildNodeRef ref)
{
  ...
  assert(refcount > 0);
  --refcount;
  co_await lba_manager->update_mapping_refcount(t, std::move(cursor), -1);
  if (refcount == 0) {
    cache->retire_extent(t, ref);
  }
  DEBUGT(
      "removed {}~0x{:x} refcount={} -- {}", t, laddr, length, refcount, *ref);
  co_return refcount;
}

TransactionManager::ref_ret
TransactionManager::remove(Transaction& t, laddr_t offset)
{
  LOG_PREFIX(TransactionManager::remove);
  DEBUGT("{} ...", t, offset);
...
TransactionManager::remove(Transaction& t, LBAMapping mapping)
{
    ...
    LBAMapping mapping)
{
  ...
  co_return co_await resolve_cursor_to_mapping(t, ret_cursor);
}

TransactionManager::ref_iertr::future<LBAMapping>
TransactionManager::_remove(Transaction& t, LBAMapping mapping)
{
  ...
  if (!mapping.is_zero_reserved() &&
    ...
    if (maybe_mapped_extent.has_child()) {
      ...
      auto extent =
          co_await maybe_mapped_extent.get_child_fut_as<LogicalChildNode>();
      ceph_assert(extent);
      ...
      auto retired_placeholder =
          cache
              ->retire_absent_extent_addr(
                  t, mapping.get_intermediate_base(), mapping.get_val(),
  ...
  if (mapping.is_indirect()) {
    DEBUGT(
        "removing indirect mapping {}~0x{:x} refcount={} -- offset={}", t,
  ...
  DEBUGT(
      "removing direct mapping {}~0x{:x} refcount={} -- offset={}", t,
      mapping.direct_cursor->get_paddr(), mapping.direct_cursor->get_length(),
...
TransactionManager::resolve_cursor_to_mapping(Transaction& t, LBACursorRef cursor)
{
  ...
  auto direct_cursors = co_await lba_manager->get_cursors(
      t, cursor->get_intermediate_key(), cursor->get_length());
  ...
  auto ret =
      co_await LBAMapping::create_indirect(std::move(direct_cursor), cursor)
          .refresh();
  DEBUGT("{} -> {}", t, *cursor, ret);
  co_return ret;
}

TransactionManager::refs_ret
TransactionManager::remove(Transaction& t, std::vector<laddr_t> offsets)
{
  LOG_PREFIX(TransactionManager::remove);
  DEBUGT("{} offsets ...", t, offsets.size());
  return seastar::do_with(
      std::move(offsets), std::vector<unsigned>(),
      [this, &t, FNAME](auto&& offsets, auto& refcnts) {
        return trans_intr::do_for_each(
                   offsets.begin(), offsets.end(),
                   [this, &t, &refcnts](auto& laddr) {
                     return this->remove(t, laddr).si_then([&refcnts](auto ref) {
            ...
            .si_then([&refcnts, &t, FNAME] {
              ...
              return ref_iertr::make_ready_future<std::vector<unsigned>>(
                  std::move(refcnts));
            });
      });
}
...
TransactionManager::submit_transaction(Transaction& t)
{
    ...
    std::optional<journal_seq_t> trim_alloc_to)
{
  return do_submit_transaction(
      tref, epm->dispatch_delayed_extents(tref), trim_alloc_to);
}
    ...
    std::list<CachedExtentRef>& pre_allocated_extents)
{
  ...
  return seastar::do_with(
      ...
      [this, &t, &pre_allocated_extents](auto& lextents, auto& pextents) {
        ...
        std::for_each(
            pre_allocated_extents.begin(), pre_allocated_extents.end(),
            chksum_func);

        return lba_manager->update_mappings(t, lextents)
    ...
    std::optional<journal_seq_t> trim_alloc_to)
{
  ...
  co_await epm->write_delayed_ool_extents(tref, dispatch_result.alloc_map);
  ...
  auto record = cache->prepare_record(
      tref, journal->get_trimmer().get_journal_head(),
  ...
  co_await journal
      ->submit_record(
          std::move(record), tref.get_handle(), tref.get_src(),
  ...
  co_await trans_intr::make_interruptible(tref.get_handle().complete());
}

seastar::future<>
TransactionManager::flush(OrderingHandle& handle)
{
  ...
  if (get_extent_category(extent->get_type()) == data_category_t::METADATA) {
    ...
    auto nextent = cache
                       ->alloc_new_non_data_extent_by_type(
                           t, extent->get_type(), extent->get_length(),
    ...
    co_await lba_manager->update_mapping(
        t, std::move(cursor), extent->get_length(), extent->get_paddr(),
        *nextent);
  } else {
    ...
    auto extents = cache->alloc_new_data_extents_by_type(
        t, extent->get_type(), extent->get_length(), extent->get_user_hint(),
        // get target rewrite generation
        extent->get_rewrite_generation());
    extent_len_t off = 0;
    ...
    for (auto& _nextent : extents) {
      ...
      if (first_extent) {
        assert(off == 0);
        auto cursor =
            co_await lba_manager->get_cursor(t, *extent)
                .handle_error_interruptible(
                    rewrite_extent_iertr::pass_further{},
                    crimson::ct_error::assert_all{"unexpected enoent"});
        refcount = co_await lba_manager
                       ->update_mapping(
                           t, std::move(cursor), extent->get_length(),
        ...
        auto cursor = co_await lba_manager->alloc_extent(
            t, (extent->get_laddr() + off).checked_to_laddr(), *nextent,
            refcount);
    ...
    sea_time_point modify_time)
{
...

  {
    ...
    if (!updated) {
      DEBUGT(
          "target={} {} already retired, skipping -- {}", t,
          rewrite_gen_printer_t{target_generation},
          sea_time_point_printer_t{modify_time}, *extent);
      return rewrite_extent_iertr::now();
    }

    extent = updated;
    DEBUGT(
        "target={} {} -- {} ...", t, rewrite_gen_printer_t{target_generation},
        sea_time_point_printer_t{modify_time}, *extent);
  ...
  if (extent->has_mutation() || extent->is_stable_dirty()) {
    ...
    if (extent->is_stable_dirty() && epm->can_inplace_rewrite(t, extent)) {
  ...
  return fut.si_then([FNAME, &t] { DEBUGT("rewritten", t); });
}
    ...
    extent_len_t len)
{
  ...
  if (extent) {
    DEBUGT(
        "{} {}~0x{:x} {} is cached and alive -- {}", t, type, laddr, len, paddr,
        *extent);
    assert(extent->get_length() == len);
    std::list<CachedExtentRef> res;
    res.emplace_back(std::move(extent));
  } else if (is_logical_type(type)) {
    auto pin_list = co_await lba_manager->get_cursors(t, laddr, len);
    ...
    DEBUGT(
        "{} {}~0x{:x} {} is alive as {} extents", t, type, laddr, len, paddr,
        res.size());
  } else {
    ...
    if (ext) {
      DEBUGT(
          "{} {}~0x{:x} {} is absent and alive as physical extent -- {}", t,
          type, laddr, len, paddr, *ext);
      res.emplace_back(std::move(ext));
    } else {
      DEBUGT(
          "{} {}~0x{:x} {} is not alive as physical extent", t, type, laddr,
          len, paddr);
    }
  }
  co_return res;
    ...
    bool is_test)
{
  ...
  if (p_backend_type == backend_type_t::SEGMENTED) {
    ...
    ceph_assert(
        dtype != device_type_t::HDD && dtype != device_type_t::EPHEMERAL_COLD);
  ...
  if (backend_type == backend_type_t::SEGMENTED) {
    ...
    ceph_assert_always(
        (std::size_t)roll_size + roll_start <=
  ...
  if (is_test) {
    ...
    trimmer_config =
        JournalTrimmerImpl::config_t::get_test(roll_size, backend_type);
  } else {
    cleaner_is_detailed = false;
    cleaner_config = SegmentCleaner::config_t::get_default();
    trimmer_config =
        JournalTrimmerImpl::config_t::get_default(roll_size, backend_type);
  }

  bool pure_rbm_backend = (p_backend_type == backend_type_t::RANDOM_BLOCK) &&
                          !cold_sms;
  auto journal_trimmer = JournalTrimmerImpl::create(
      store_index, *backref_manager, trimmer_config, backend_type, roll_start,
      roll_size, !pure_rbm_backend);
  ...
  if (cold_sms) {
    cold_segment_cleaner = SegmentCleaner::create(
        store_index, cleaner_config, std::move(cold_sms), *backref_manager,
        epm->get_ool_segment_seq_allocator(),
        hot_tier_generations + cold_tier_generations - 1, cleaner_is_detailed,
        /* is_cold = */ true);
    if (backend_type == backend_type_t::SEGMENTED) {
  ...
  if (backend_type == backend_type_t::SEGMENTED) {
    cleaner = SegmentCleaner::create(
        store_index, cleaner_config, std::move(sms), *backref_manager,
        epm->get_ool_segment_seq_allocator(), hot_tier_generations - 1,
        cleaner_is_detailed);
    ...
    journal = journal::make_segmented(
        store_index, *segment_cleaner, *journal_trimmer);
  } else {
    cleaner = RBMCleaner::create(
        store_index, std::move(rbs), *backref_manager, *lba_manager,
        cleaner_is_detailed);
    journal = journal::make_circularbounded(
        store_index, *journal_trimmer,
  ...
  epm->init(
      std::move(journal_trimmer), std::move(cleaner),
      std::move(cold_segment_cleaner));
  epm->set_primary_device(primary_device);

  INFO(
      "main backend type: {}, cold tier: {}", epm->get_main_backend_type(),
      (bool)cold_sms);
  return std::make_unique<TransactionManager>(
      std::move(journal), std::move(cache), std::move(lba_manager),
      std::move(epm), std::move(backref_manager), shard_stats);
}
