  ...
  my_base(ctx)
{
  ...
  for (const auto& bt : peering_state().get_backfill_targets()) {
    DEBUGDPP(
        "{}: target shard {} from {}", pg(), __func__, bt,
...
BackfillState::Initial::react(const BackfillState::Triggered& evt)
{
  ...
  if (Enqueuing::all_enqueued(
          peering_state(), backfill_state().backfill_info,
...
BackfillState::Enqueuing::maybe_update_range()
{
  ...
  if (auto& primary_bi = backfill_state().backfill_info;
    ...
    DEBUGDPP(
        "bi is old, ({}) can be updated with log to {}", pg(),
        primary_bi.version, pg().get_projected_last_update());
    auto func = [&](const pg_log_entry_t& e) {
      DEBUGDPP(
          "maybe_update_range(lambda): updating from version {}", pg(),
          e.version);
      if (e.soid >= primary_bi.begin && e.soid < primary_bi.end) {
        if (e.is_update()) {
          DEBUGDPP(
              "maybe_update_range(lambda): {} updated to ver {}", pg(), e.soid,
              e.version);
          if (e.written_shards.empty()) {
            // Log entry updates all shards, replace all entries for e.soid
            primary_bi.objects.erase(e.soid);
            primary_bi.objects.insert(std::make_pair(
                e.soid, std::make_pair(shard_id_t::NO_SHARD, e.version)));
          } else {
            ...
            for (auto& [shard, version] : versions) {
              primary_bi.objects.insert(
                  std::make_pair(e.soid, std::make_pair(shard, version)));
            }
          }
        } else if (e.is_delete()) {
          DEBUGDPP("maybe_update_range(lambda): {} removed", pg(), e.soid);
...
void
BackfillState::Enqueuing::trim_backfill_infos()
{
  for (const auto& bt : peering_state().get_backfill_targets()) {
    backfill_state().peer_backfill_info[bt].trim_to(std::max(
        peering_state().get_peer_last_backfill(bt),
        backfill_state().last_backfill_started));
  }
...
/* static */ bool
BackfillState::Enqueuing::all_enqueued(
...
{
  const bool all_local_enqueued = backfill_info.extends_to_end() &&
                                  backfill_info.empty();
  const bool all_peer_enqueued = std::all_of(
      std::begin(peer_backfill_info), std::end(peer_backfill_info),
      [](const auto& kv) {
        [[maybe_unused]] const auto& [shard, peer_backfill_info] = kv;
        return peer_backfill_info.extends_to_end() &&
               peer_backfill_info.empty();
      });
  return all_local_enqueued && all_peer_enqueued;
}

hobject_t
BackfillState::Enqueuing::earliest_peer_backfill(
    const std::map<pg_shard_t, ReplicaBackfillInterval>& peer_backfill_info) const
{
...
bool
BackfillState::Enqueuing::should_rescan_replicas(
    const std::map<pg_shard_t, ReplicaBackfillInterval>& peer_backfill_info,
    const PrimaryBackfillInterval& backfill_info) const
{
  ...
  return std::any_of(std::begin(targets), std::end(targets), [&](const auto& bt) {
    return ReplicasScanning::replica_needs_scan(
        peer_backfill_info.at(bt), backfill_info);
  });
}

bool
BackfillState::Enqueuing::should_rescan_primary(
    const std::map<pg_shard_t, ReplicaBackfillInterval>& peer_backfill_info,
    const PrimaryBackfillInterval& backfill_info) const
{
...
void
BackfillState::Enqueuing::trim_backfilled_object_from_intervals(
    BackfillState::Enqueuing::result_t&& result,
    hobject_t& last_backfill_started,
    std::map<pg_shard_t, ReplicaBackfillInterval>& peer_backfill_info)
{
  std::for_each(
      std::begin(result.pbi_targets), std::end(result.pbi_targets),
...
BackfillState::Enqueuing::remove_on_peers(const hobject_t& check)
{
  ...
  DEBUGDPP(
      "BACKFILL removing {} from peers {}", pg(), check, result.pbi_targets);
...
bool
BackfillState::Enqueuing::Enqueuing::all_emptied(
    const PrimaryBackfillInterval& local_backfill_info,
    const std::map<pg_shard_t, ReplicaBackfillInterval>& peer_backfill_info) const
{
  const auto& targets = peering_state().get_backfill_targets();
  const auto replicas_emptied =
      std::all_of(std::begin(targets), std::end(targets), [&](const auto& bt) {
  ...
  my_base(ctx)
{
  ...
  do {
    if (!backfill_listener().budget_available()) {
      ...
      return;
    } else if (should_rescan_replicas(
                   backfill_state().peer_backfill_info, primary_bi)) {
    ...
    if (const hobject_t check =
      ...
      trim_backfilled_object_from_intervals(
          std::move(result), backfill_state().last_backfill_started,
          backfill_state().peer_backfill_info);
  ...
  if (should_rescan_primary(backfill_state().peer_backfill_info, primary_bi)) {
    ...
    DEBUGDPP(
        "reached end for both local and all peers "
...
BackfillState::PrimaryScanning::react(ObjectPushed evt)
{
  ...
  DEBUGDPP(
      "PrimaryScanning::react() on ObjectPushed; evt.object={}", pg(),
      evt.object);
...
// -- ReplicasScanning
bool
BackfillState::ReplicasScanning::replica_needs_scan(
...
BackfillState::ReplicasScanning::react(ReplicaScanned evt)
{
  ...
  DEBUGDPP("got scan result from osd={}, result={}", pg(), evt.from, evt.result);
  ...
  if (waiting_on_backfill.erase(evt.from)) {
    ...
    if (waiting_on_backfill.empty()) {
      ceph_assert(
          backfill_state().peer_backfill_info.size() ==
          peering_state().get_backfill_targets().size());
      if (!backfill_state().is_suspended()) {
        return transit<Enqueuing>();
      } else {
...
BackfillState::ReplicasScanning::react(ObjectPushed evt)
{
  ...
  DEBUGDPP(
      "ReplicasScanning::react() on ObjectPushed; evt.object={}", pg(),
      evt.object);
  backfill_state().progress_tracker->complete_to(evt.object, evt.stat, true);
  return discard_event();
}
...
// updates.
bool
BackfillState::ProgressTracker::tracked_objects_completed() const
{
  return registry.empty();
}

bool
BackfillState::ProgressTracker::enqueue_push(const hobject_t& obj)
{
...
void
BackfillState::ProgressTracker::enqueue_drop(const hobject_t& obj)
{
...
void
BackfillState::ProgressTracker::complete_to(
    const hobject_t& obj,
    ...
    bool may_push_to_max)
{
  ...
  for (auto it = std::begin(registry);
       it != std::end(registry) && it->second.stage != op_stage_t::enqueued_push;
...
void
BackfillState::enqueue_standalone_push(
    const hobject_t& obj,
    const eversion_t& v,
...
std::ostream&
operator<<(std::ostream& out, const BackfillState::PGFacade& pg)
{
  return pg.print(out);
}
