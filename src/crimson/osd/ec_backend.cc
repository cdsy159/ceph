...
ECBackend::create_ec_impl(const ec_profile_t& ec_profile)
{
  using crimson::common::local_conf;
  ceph::ErasureCodeInterfaceRef ec_impl;
  std::stringstream ss;
  ceph::ErasureCodePluginRegistry::instance().factory(
      ec_profile.find("plugin")->second, local_conf()->erasure_code_dir,
      const_cast<ec_profile_t&>(ec_profile), &ec_impl, &ss);
  return ec_impl;
}
...
ECBackend::ECBackend(
  ...
  rmw_pipeline{
      ...
      &eclistener,
      *this,
      shard_services.lookup_ec_extent_cache_lru()}
{}
    ...
    const uint32_t flags)
{
  ...
  reads[hoid].emplace_back(ec_align_t{aligned_off, aligned_len, flags});
  seastar::promise<ceph::bufferlist> promise;
  auto ret = promise.get_future();
  objects_read_and_reconstruct(
      reads, fast_read, object_size,
      make_gen_lambda_context<ec_extents_t&&>([hoid, off, len,
                                               promise = std::move(promise),
                                               FNAME](auto&& results) mutable {
        ...
        ceph_assert(
            (off + len) <= (range.first.get_off() + range.first.get_len()));
        ceph::bufferlist clients_bl;
        clients_bl.substr_of(
            range.first.get_val(), off - range.first.get_off(), len);
...
struct ECCrimsonOp : ECCommon::RMWPipeline::Op {
  ...
  static PGTransactionUPtr
  transate_transaction(
      ...
      crimson::osd::ObjectContextRef&& obc)
  {
    ...
    while (i.have_op()) {
      auto* op = i.decode_op();
      logger().debug(
          "ECBackend::{} decoded op={} oid={} dest_oid={}", __func__,
          static_cast<uint32_t>(op->op), i.get_oid(op->oid),
      ...
      case ceph::os::Transaction::OP_SETATTR: {
        ...
        ceph::bufferlist bl;
        i.decode_bl(bl);
        t_pg->setattr(i.get_oid(op->oid).hobj, name, bl);
      } break;
      ...
      case ceph::os::Transaction::OP_WRITE: {
        ...
        i.decode_bl(bl);
        t_pg->write(
            i.get_oid(op->oid).hobj, op->off, op->len, bl,
            i.get_fadvise_flags());
      } break;
      ...
      case ceph::os::Transaction::OP_SETALLOCHINT:
        t_pg->set_alloc_hint(
            i.get_oid(op->oid).hobj, op->expected_object_size,
            op->expected_write_size, op->hint);
        break;
      case ceph::os::Transaction::OP_CLONERANGE2:
        t_pg->clone_range(
            i.get_oid(op->oid).hobj, i.get_oid(op->dest_oid).hobj, op->off,
            op->len, op->dest_off);
      ...
      case ceph::os::Transaction::OP_MERGE_COLLECTION:
        ceph_abort_msg(
            "Coll-related ops shouldn't be associated with PGTransaction");
      default:
  ...
  ECCrimsonOp(
    ...
    Op(rmw_pipeline), t(transate_transaction(std::move(t), std::move(obc)))
  {}
    ...
    std::vector<pg_log_entry_t>&& log_entries)
{
  ...
  logger().debug(
      "{} hoid={} obc->attr_cache={}", __func__, hoid, obc->attr_cache);
  auto op = std::make_unique<ECCrimsonOp>(
      std::move(txn), std::move(obc), rmw_pipeline);
  op->hoid = hoid;
  ...
  op->plan = ECCommon::get_write_plan(
      sinfo, *(op->t), read_pipeline, rmw_pipeline, &dpp);
  ...
  return make_ready_future<rep_op_ret_t>(
      seastar::now(), on_all_commit->get_future().then_interruptible([] {
        logger().debug(
            "{}: op {} finished completely", "_submit_transaction", "");
        return seastar::now();
      }));
}

ECBackend::write_iertr::future<>
ECBackend::handle_sub_write(pg_shard_t from, ECSubWrite&& op, ECListener& pg)
{
  ...
  if (op.backfill_or_async_recovery) {
    for (const auto& obj : op.temp_removed) {
      logger().info(
          "{}: removing object {} since we won't get the transaction", __func__,
          obj);
      txn.remove(
          coll->get_cid(), ghobject_t{obj, ghobject_t::NO_GEN, get_shard()});
    }
  }
  ...
  if (pg.is_missing_object(op.soid)) {
    ...
    for (const auto& e : op.log_entries) {
      logger().debug(
          "{}: add_next_event entry {}, is_delete {}", __func__, e,
          e.is_delete());
  ...
  logger().debug("{}:{}", __func__, __LINE__);
  return crimson::os::with_store_do_transaction(store, coll, std::move(txn))
      .then([FNAME] {
...
ECBackend::write_iertr::future<>
ECBackend::handle_rep_write_op(Ref<MOSDECSubOpWrite> m, crimson::osd::PG& pg)
{
...
ECBackend::handle_rep_write_reply(ECSubWriteReply&& op)
{
  ...
  if (op.committed) {
    ...
    logger().debug(
        "ECBackend::{} from {} pending_commits {}", __func__, from,
        wop.pending_commits);
...
bool
ECBackend::is_single_chunk(const hobject_t& obj, const ECSubRead& op)
{
    ...
    const uint32_t flags)
{
  ...
  if (is_single_chunk(obj, op)) {
    return crimson::os::with_store<&crimson::os::FuturizedStore::Shard::read>(
        store, coll, ghobject_t{obj, ghobject_t::NO_GEN, get_shard()}, off,
        size, flags);
  } else {
    return seastar::do_with(ceph::bufferlist{}, [=, this](auto&& result_bl) {
      ...
      return crimson::do_for_each(
                 ...
                 boost::make_counting_iterator(
                     1 + (size - 1) / sinfo.get_chunk_size()),
                 [off, flags, subchunk_size, &obj, &op, &result_bl,
                  this](const auto m) {
                   const auto& sub_spec = op.subchunks.find(obj)->second;
                   return crimson::do_for_each(
                       std::begin(sub_spec), std::end(sub_spec),
                       [&obj, off, flags, subchunk_size, m, &result_bl,
                        this](const auto& subchunk) {
                         const auto [sub_off_count, sub_size_count] = subchunk;
                         return crimson::os::with_store<
                                    &crimson::os::FuturizedStore::Shard::read>(
                                    store, coll,
                                    ghobject_t{
                                        obj, ghobject_t::NO_GEN, get_shard()},
                                    off + m * sinfo.get_chunk_size() +
                                        sub_off_count * subchunk_size,
                                    sub_size_count * subchunk_size, flags)
...
ECBackend::handle_rep_read_op(ECSubRead& op)
{
  ...
  return seastar::do_with(ECSubReadReply{}, [&op, FNAME, this](auto&& reply) {
    reply.from = whoami;
    reply.tid = op.tid;
    ...
    return interruptor::do_for_each(
               op.to_read,
               [FNAME, &op, &reply, this](auto read_item) {
                 ...
                 return interruptor::do_for_each(
                     op_list,
                     [FNAME, &op, &reply, obj = obj, this](auto op_spec) {
                       const auto& [off, size, flags] = op_spec;
                       return maybe_chunked_read(obj, op, off, size, flags)
                           .safe_then([&reply, obj, off = off, size = size,
                                       FNAME](auto&& result_bl) {
                             DEBUG(
                                 "read requested={} len={}", size,
                                 result_bl.length());
                             reply.buffers_read[obj].emplace_back(
                                 off, std::move(result_bl));
                             return read_ertr::now();
                           })
                           .handle_error(read_ertr::all_same_way([&reply, obj,
                                                                  FNAME, this](
                                                                     const auto&
                                                                         e) {
                             assert(e.value() > 0);
                             if (e.value() == ENOENT && fast_read) {
                               INFO(
                                   "ENOENT reading {}, fast, read, probably ok",
                                   obj);
                             } else {
        ...
        .si_then([&op, &reply, FNAME, this] {
          ...
          return interruptor::do_for_each(
              op.attrs_to_read, [&reply, FNAME, this](auto obj_attr) {
                ...
                return crimson::os::with_store<
                           ...
                           store, coll,
                           ghobject_t{obj_attr, ghobject_t::NO_GEN, get_shard()},
                           0)
                    .safe_then(
                        [&reply, obj_attr](auto&& attrs) {
                          reply.attrs_read[obj_attr] = std::move(attrs);
                          return read_ertr::now();
                        },
                        read_ertr::all_same_way([&reply,
                                                 obj_attr](const auto& e) {
                          assert(e.value() > 0);
...
ECBackend::handle_rep_read_reply(ECSubReadReply& mop)
{
  ...
  for (auto&& [hoid, req] : rop.to_read) {
    ...
    for (auto&& [shard, read] : std::as_const(req.shard_reads)) {
      if (complete.errors.contains(read.pg_shard))
        continue;
      ...
      if (!rop.complete.contains(hoid) ||
        ...
        if (!read.extents.empty())
          continue; // Complete the actual read first.
      ...
      if (!err) {
        ...
        if (!rop.complete.at(oid).errors.empty()) {
          ...
          if (local_conf()->osd_read_ec_check_for_errors) {
            logger().info(
                "{}: not ignoring errors, use one shard err={}", __func__, err);
            err = rop.complete.at(oid).errors.begin()->second;
            rop.complete.at(oid).r = err;
          } else {
            logger().info(
                "{}: error(s) ignored for {} enough copies available", __func__,
                oid);
...
ECBackend::getxattr(const hobject_t& soid, std::string&& key) const
{
