...
make_keyring()
{
  ...
  return seastar::file_exists(path)
      .then([path](bool exists) {
        ...
        EntityAuth auth;
        if (exists && keyring.load(nullptr, path) == 0 &&
      ...
      .handle_exception_type([path](const std::filesystem::filesystem_error& e) {
        fmt::print(
            std::cerr, "FATAL: writing new keyring to {}: {}\n", path, e.what());
        throw e;
      });
}
        ...
        const_cast<char**>(seastar_n_early_args.data()), [&] {
          ...
          return seastar::async([&] {
            try {
              ...
              sharded_conf()
                  .start(
                      early_config.init_params.name, early_config.cluster_name)
                  .get();
              ...
              if (const auto ret = pidfile_write(local_conf()->pid_file);
                ...
                ceph_abort_msg(
                    "likely there is another crimson-osd instance with the "
                    "same id");
              } else if (ret < 0) {
                ceph_abort_msg(fmt::format(
                    "pidfile_write failed with {} {}", ret, cpp_strerror(-ret)));
              }
              ...
              if (uint16_t prom_port = config["prometheus_port"].as<uint16_t>();
                ...
                prom_config.prefix =
                    config["prometheus_prefix"].as<std::string>();
                seastar::prometheus::start(prom_server, prom_config).get();
                seastar::net::inet_address prom_addr(
                    config["prometheus_address"].as<std::string>());
                prom_server
                    .listen(seastar::socket_address{prom_addr, prom_port})
              ...
              INFO(
                  "passed objectstore is {}",
                  local_conf().get_val<std::string>("osd_objectstore"));

              crimson::osd::OSD osd(
                  whoami, nonce, std::ref(should_stop.abort_source()),
                  std::ref(*store), cluster_msgr, client_msgr, hb_front_msgr,
                  hb_back_msgr);
