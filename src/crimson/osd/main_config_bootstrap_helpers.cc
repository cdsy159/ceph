...
{
  std::cout << "crimson osd usage: " << prog << " -i <ID> [flags...]"
            << std::endl;
...
populate_config_from_mon()
{
  ...
  public:
    void
    handle_authentication(const EntityName& name, const AuthCapsInfo& caps)
    {}
  };
  ...
  return seastar::async([] {
    ...
    auto msgr = crimson::net::Messenger::create(
        entity_name_t::CLIENT(), "temp_mon_client", get_nonce(), true);
...
const std::vector<SeastarOption> seastar_options = {
    ...
    {"--io-latency-goal-ms", "crimson_reactor_io_latency_goal_ms",
     Option::TYPE_FLOAT},
    {"--idle-poll-time-us", "crimson_reactor_idle_poll_time_us",
     Option::TYPE_UINT},
    {"--poll-mode", "crimson_poll_mode", Option::TYPE_BOOL},
    {"--reactor-backend", "crimson_reactor_backend", Option::TYPE_STR}};

std::optional<std::string>
get_option_value(const SeastarOption& option)
{
...
_get_early_config(int argc, const char* argv[])
{
  ...
  ret.init_params = ceph_argparse_early_args(
      early_args, CEPH_ENTITY_TYPE_OSD, &ret.cluster_name, &ret.conf_file_list);
  ...
  int r = app.run(
      ...
      const_cast<char**>(bootstrap_args), [argc, argv, &ret, &early_args] {
        return seastar::async([argc, argv, &ret, &early_args] {
          ...
          // get ceph configs
          std::set_difference(
              argv, argv + argc, std::begin(early_args), std::end(early_args),
              std::back_inserter(ret.ceph_args));

          ret.early_args.insert(
              std::end(ret.early_args), std::begin(early_args),
              std::end(early_args));
          ...
          for (const auto& option : seastar_options) {
            ...
            if (option_value) {
              logger().info(
                  "configure {} with value: {}", option.option_name,
                  option_value);
          ...
          if (auto found = std::find_if(
            ...
            auto cpu_cores =
                crimson::common::get_conf<std::string>("crimson_cpu_set");
            if (!cpu_cores.empty()) {
              ...
              logger().info(
                  "get_early_config: set --thread-affinity 1 --cpuset {}",
                  cpu_cores);
            } else {
              auto reactor_num =
                  crimson::common::get_conf<uint64_t>("crimson_cpu_num");
              if (!reactor_num) {
                // We would like to avoid seastar using all available cores.
                logger().error(
                    "get_early_config: crimson_cpu_set"
              ...
              logger().info(
                  "get_early_config: set --thread-affinity 0 --smp {}",
                  reactor_num);
            }
          } else {
            logger().error(
                "get_early_config: --cpuset can be "
...
get_early_config(int argc, const char* argv[])
{
  ...
  if (r < 0) {
    std::cerr << "get_early_config: failed to create pipes: " << -errno
              << std::endl;
  ...
  if (worker < 0) {
    ...
    if (ret.has_value()) {
      ...
      close(pipes[1]);
      if (r < 0) {
        std::cerr << "get_early_config: child failed to write_fd: " << r
                  << std::endl;
        exit(-r);
      } else {
        exit(0);
      ...
      std::cerr << "get_early_config: child failed: " << -ret.error()
                << std::endl;
      exit(-ret.error());
    }
    ...
    if (r < 0) {
      std::cerr << "get_early_config: parent failed to read from pipe: " << r
                << std::endl;
      return tl::unexpected(r);
    }
    try {
