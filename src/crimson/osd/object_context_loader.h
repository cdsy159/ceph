...
public:
  using obc_accessing_list_t =
      boost::intrusive::list<ObjectContext, ObjectContext::obc_accessing_option_t>;

  ObjectContextLoader(
      ObjectContextRegistry& _obc_services,
      PGBackend& _backend,
      DoutPrefixProvider& dpp) :
    obc_registry{_obc_services}, backend{_backend}, dpp{dpp}
  {}
  ...
  class Manager {
    ...
    struct state_t {
      ...
      void
      demote_excl_to(RWState::State lock_type)
      {
      ...
      auto
      lock_to(RWState::State lock_type)
      {
        assert(state == RWState::RWNONE);
        switch (lock_type) {
        case RWState::RWWRITE:
          return interruptor::make_interruptible(obc->lock.lock_for_write().then(
              [this] { state = RWState::RWWRITE; }));
        case RWState::RWREAD:
          return interruptor::make_interruptible(obc->lock.lock_for_read().then(
              [this] { state = RWState::RWREAD; }));
        case RWState::RWNONE:
          // noop
          return interruptor::now();
        case RWState::RWEXCL:
          return interruptor::make_interruptible(obc->lock.lock_for_excl().then(
              [this] { state = RWState::RWEXCL; }));
        default:
          ceph_assert(0 == "impossible");
          return interruptor::now();
        }
    ...
    void
    set_state_obc(state_t& s, ObjectContextRef _obc)
    {
  ...
  public:
    ...
    get_releaser()
    {
      return seastar::defer([this] { release(); });
    }
   ...
   */
  ObjectContextRef
  create_cached_obc_from_push_data(const object_info_t& oi, SnapSetContextRef ssc)
  {
  ...
  Orderer
  get_obc_orderer(const hobject_t& oid)
  {
    Orderer ret;
  ...
  Manager
  get_obc_manager(const hobject_t& oid, bool resolve_clone = true)
  {
  ...
  Manager
  get_obc_manager(ObjectContextRef obc)
  {
  ...
  using load_and_lock_iertr = interruptible::
      interruptible_errorator<IOInterruptCondition, load_and_lock_ertr>;
  using load_and_lock_fut = load_and_lock_iertr::future<>;

private:
  load_and_lock_fut load_and_lock_head(Manager&, RWState::State);
  load_and_lock_fut load_and_lock_clone(
      Manager&,
      RWState::State,
      bool lock_head = true);
...
public:
  ...
  with_obc(hobject_t oid, with_obc_func_t func, bool resolve_clone = true)
  {
    auto manager = get_obc_manager(oid, resolve_clone);
    co_await load_and_lock(manager, State);
    co_await std::invoke(func, manager.get_head_obc(), manager.get_obc());
  }
  ...
  void load_obc(ObjectContextRef obc, PGBackend::loaded_object_md_t::ref);
