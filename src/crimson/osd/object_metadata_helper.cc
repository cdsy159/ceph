    ...
    const hobject_t& last_backfill)
{
  subsets_t subsets;
  logger().debug(
      "{}: {} clone_overlap {} ", __func__, soid, snapset.clone_overlap);
  ...
  if (dirty_regions.empty()) {
    logger().debug(
        "{} {} not touched, no need to recover, skipping", __func__, soid);
    return subsets;
  }
  ...
  logger().debug("{} {} data_subset {}", __func__, soid, subsets.data_subset);
  ...
  if (!crimson::common::local_conf()->osd_recover_clone_overlap) {
    logger().debug(
        "{} {} -- osd_recover_clone_overlap is disabled", __func__, soid);
    ;
    return subsets;
  }
#endif
  ...
  auto soid_snap_iter =
      find(snapset.clones.begin(), snapset.clones.end(), soid.snap);
  ...
  for (int i = soid_snap_index - 1; i >= 0; i--) {
    ...
    if (!missing.is_missing(clone) && clone < last_backfill) {
      logger().debug(
          "{} {} has prev {} overlap {}", __func__, soid, clone, prev);
      subsets.clone_subsets[clone] = prev;
      cloning.union_of(prev);
      break;
    }
    logger().debug(
        "{} {} does not have prev {} overlap {}", __func__, soid, clone, prev);
  }
  ...
  for (unsigned i = soid_snap_index + 1; i < snapset.clones.size(); i++) {
    hobject_t clone = soid;
    clone.snap = snapset.clones[i];
    ...
    if (!missing.is_missing(clone) && clone < last_backfill) {
      logger().debug(
          "{} {} has next {} overlap {}", __func__, soid, clone, next);
      subsets.clone_subsets[clone] = next;
      cloning.union_of(next);
      break;
    }
    logger().debug(
        "{} {} does not have next {} overlap {}", __func__, soid, clone, next);
  }

#ifndef UNIT_TESTS_BUILT
  if (cloning.num_intervals() > crimson::common::local_conf().get_val<uint64_t>(
...
{
  logger().debug(
      "{}: {} clone_overlap {} ", __func__, head, snapset.clone_overlap);
  ...
  // let data_subset store only the modified content of the object.
  subsets.data_subset.intersection_of(
      missing_item.clean_regions.get_dirty_regions());
  logger().debug("{} {} data_subset {}", __func__, head, subsets.data_subset);
  ...
  if (!crimson::common::local_conf()->osd_recover_clone_overlap) {
    logger().debug(
        "{} {} -- osd_recover_clone_overlap is disabled", __func__, head);
    return subsets;
  }
#endif
  ...
  for (int i = snapset.clones.size() - 1; i >= 0; i--) {
    ...
    if (!missing.is_missing(clone) && clone < last_backfill) {
      logger().debug(
          "{} {} has prev {} overlap {}", __func__, head, clone, prev);
      cloning = prev;
      break;
    }
    logger().debug(
        "{} {} does not have prev {} overlap {}", __func__, head, clone, prev);
  }
...
void
set_subsets(const subsets_t& subsets, ObjectRecoveryInfo& recovery_info)
{
