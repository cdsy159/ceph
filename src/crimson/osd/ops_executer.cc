...
OpsExecuter::call_ierrorator::future<>
OpsExecuter::do_op_call(OSDOp& osd_op)
{
  ...
  logger().debug(
      "calling method {}.{}, num_read={}, num_write={}", cname, mname, num_read,
      num_write);
  const auto prev_rd = num_read;
  const auto prev_wr = num_write;
  return interruptor::async([this, method,
                             indata = std::move(indata)]() mutable {
      ...
      .then_interruptible(
          [this, prev_rd, prev_wr, &osd_op,
           flags](auto outcome) -> call_errorator::future<> {
            auto& [ret, outdata] = outcome;
            osd_op.rval = ret;

            logger().debug(
                "do_op_call: method returned ret={}, outdata.length()={}"
                " while num_read={}, num_write={}",
                ret, outdata.length(), num_read, num_write);
            if (num_read > prev_rd && !(flags & CLS_METHOD_RD)) {
              logger().error(
                  "method tried to read object but is not marked RD");
              osd_op.rval = -EIO;
              return crimson::ct_error::input_output_error::make();
            }
            if (num_write > prev_wr && !(flags & CLS_METHOD_WR)) {
              logger().error(
                  "method tried to update object but is not marked WR");
              osd_op.rval = -EIO;
              return crimson::ct_error::input_output_error::make();
            }
            ...
            if (op_info.allows_returnvec() && op_info.may_write() && ret >= 0 &&
              ...
              logger().error(
                  "outdata overflow due to .length()={}, limit={}",
                  outdata.length(), local_conf()->osd_max_write_op_reply_len);
...
{
  using crimson::common::local_conf;
  const uint32_t timeout = osd_op.op.watch.timeout == 0
                               ? local_conf()->osd_client_watch_timeout
                               : osd_op.op.watch.timeout;
  return {osd_op.op.watch.cookie, timeout, peer_addr};
}

OpsExecuter::watch_ierrorator::future<>
  ...
  return with_effect_on_obc(
      ...
      [](auto&& ctx, ObjectContextRef obc, Ref<PG> pg) {
        ...
        if (emplaced) {
          ...
          it->second =
              crimson::osd::Watch::create(obc, ctx.info, entity, std::move(pg));
  ...
  return with_effect_on_obc(
      ...
      [](auto&& ctx, ObjectContextRef obc, Ref<PG>) {
        if (auto nh = obc->watchers.extract(ctx.key); !nh.empty()) {
          ...
          logger().info(
              "op_effect: disconnect failed to find watcher {}", ctx.key);
          return seastar::now();
        }
      });
}
...
OpsExecuter::watch_ierrorator::future<>
OpsExecuter::do_op_notify(OSDOp& osd_op, const ObjectState& os)
{
  ...
  return with_effect_on_obc(
      ...
      [](auto&& ctx, ObjectContextRef obc, Ref<PG>) {
        ...
        return crimson::osd::Notify::create_n_propagate(
            std::begin(alive_watchers), std::end(alive_watchers),
            std::move(ctx.conn), ctx.ninfo, ctx.client_gid,
            obc->obs.oi.user_version);
      });
}

OpsExecuter::watch_ierrorator::future<>
OpsExecuter::do_op_list_watchers(OSDOp& osd_op, const ObjectState& os)
{
  logger().debug("{}", __func__);
  ...
  for (const auto& [key, info] : os.oi.watchers) {
    logger().debug(
        "{}: key cookie={}, entity={}", __func__, key.first, key.second);
    assert(key.first == info.cookie);
    assert(key.second.is_client());
    response.entries.emplace_back(
        watch_item_t{key.second, info.cookie, info.timeout_seconds, info.addr});
  }
...
OpsExecuter::watch_ierrorator::future<>
OpsExecuter::do_op_notify_ack(OSDOp& osd_op, const ObjectState& os)
{
  logger().debug("{}", __func__);
  ...
  return with_effect_on_obc(
      ...
      [](auto&& ctx, ObjectContextRef obc, Ref<PG>) {
        logger().info(
            "notify_ack watch_cookie={}, notify_id={}", ctx.watch_cookie,
            ctx.notify_id);
        return seastar::do_for_each(
            obc->watchers, [ctx = std::move(ctx)](auto& kv) {
              const auto& [key, watchp] = kv;
              static_assert(std::is_same_v<
                            std::decay_t<decltype(watchp)>,
                            seastar::shared_ptr<crimson::osd::Watch>>);
              auto& [cookie, entity] = key;
              if (ctx.entity != entity) {
                logger().debug(
                    "skipping watch {}; entity name {} != {}", key, entity,
                    ctx.entity);
                return seastar::now();
              }
              if (ctx.watch_cookie != cookie) {
                logger().debug(
                    "skipping watch {}; cookie {} != {}", key, ctx.watch_cookie,
                    cookie);
                return seastar::now();
              }
...
template <class Func>
auto
OpsExecuter::do_const_op(Func&& f)
{
...
template <class Func>
auto
OpsExecuter::do_read_attr_cache(Func&& f)
{
  ++num_read;
  // TODO: pass backend as read-only
  return std::invoke(
      std::forward<Func>(f), pg->get_backend(), std::as_const(obc->attr_cache),
...
template <class Func>
auto
OpsExecuter::do_write_op(Func&& f, OpsExecuter::modified_by m)
{
  ++num_write;
...
template <class Func>
auto
OpsExecuter::do_write_op_attr_cache(Func&& f, OpsExecuter::modified_by m)
{
  ++num_write;
  check_init_op_params(m);
  return std::forward<Func>(f)(
      pg->get_backend(), obc->obs, txn, obc->attr_cache);
}

OpsExecuter::call_errorator::future<>
OpsExecuter::do_assert_ver(OSDOp& osd_op, const ObjectState& os)
{
...
OpsExecuter::list_snaps_iertr::future<>
OpsExecuter::do_list_snaps(
    OSDOp& osd_op,
    ...
    const SnapSet& ss)
{
  ...
  resp.seq = ss.seq;
  logger().error(
      "OpsExecuter::do_list_snaps: {}, resp.clones.size(): {}", os.oi.soid,
      resp.clones.size());
...
OpsExecuter::execute_op(OSDOp& osd_op)
{
  return do_execute_op(osd_op).handle_error_interruptible(
      osd_op_errorator::all_same_way(
          [&osd_op](
              auto e, auto&& e_raw) -> OpsExecuter::osd_op_errorator::future<> {
...
OpsExecuter::do_execute_op(OSDOp& osd_op)
{
  ...
  // of each op.
  logger().debug(
      "handling op {} on object {}", ceph_osd_op_name(osd_op.op.op),
      get_target());
  ...
  case CEPH_OSD_OP_GETXATTR:
    return do_read_attr_cache(
        [this, &osd_op](auto& backend, const auto& attr_cache, const auto& os) {
  ...
  case CEPH_OSD_OP_RMXATTR:
    return do_write_op_attr_cache(
        [&osd_op](auto& backend, auto& os, auto& txn, auto& attr_cache) {
  ...
  case CEPH_OSD_OP_ROLLBACK:
    return do_write_op([this, &head = obc,
      ...
      return backend.rollback(
          os, obc->ssc->snapset, osd_op, txn, *osd_op_params, delta_stats, head,
          pg->obc_loader, snapc);
    });
  ...
  case CEPH_OSD_OP_SETXATTR:
    return do_write_op_attr_cache(
        [this, &osd_op](auto& backend, auto& os, auto& txn, auto& attr_cache) {
  ...
  case CEPH_OSD_OP_DELETE: {
    ...
    if (should_whiteout(obc->ssc->snapset, snapc)) { // existing obj is old
      logger().debug(
          "{} has or will have clones, will whiteout {}", __func__,
          obc->obs.oi.soid);
      whiteout = true;
    }
      ...
      return with_effect_on_obc(
          ...
          [&](auto& ctx) {
            ...
            if (obc->obs.oi.soid.is_snap()) {
              ceph_assert(
                  obc->ssc->snapset.clone_overlap.count(obc->obs.oi.soid.snap));
              num_bytes =
                  obc->ssc->snapset.get_clone_bytes(obc->obs.oi.soid.snap);
            } else {
              num_bytes = obc->obs.oi.size;
            }
            return backend.remove(
                os, txn, *osd_op_params, delta_stats, whiteout, num_bytes);
          },
          [](auto&& ctx, ObjectContextRef obc, Ref<PG>) {
            return seastar::do_for_each(
                       obc->watchers, [](auto& p) { return p.second->remove(); })
                .then([obc] {
                  obc->watchers.clear();
                  return seastar::now();
                });
  ...
  case CEPH_OSD_OP_STAT:
    ...
    return do_const_op([this,
                        &osd_op](/* const */ auto& backend, const auto& os) {
...
#endif
    return do_write_op([this, &osd_op](auto& backend, auto& os, auto& txn) {
      return backend.omap_set_header(
          os, osd_op, txn, *osd_op_params, delta_stats);
    });
    ...
    OSDriver& osdriver)
{
  ...
  if (want_mutate) {
    ...
    auto [_submitted, _all_completed] = co_await pg->submit_transaction(
        ...
        std::move(txn), std::move(*osd_op_params), std::move(log_entries));
    ...
    all_completed =
        std::move(all_completed).then_interruptible([this, pg = this->pg] {
          ...
          return interruptor::do_for_each(
              op_effects, [pg = std::move(pg)](auto& op_effect) {
    ...
    ceph::os::Transaction& txn)
{
  ...
  pg_log_entry_t ret{
      obc->obs.exists ? pg_log_entry_t::MODIFY : pg_log_entry_t::DELETE,
      obc->obs.oi.soid,
  ...
  if (obc->obs.exists) {
    ...
    pg->get_backend().set_metadata(
        obc->obs.oi.soid, obc->obs.oi,
        obc->obs.oi.soid.is_head() ? &(obc->ssc->snapset) : nullptr, txn);
  } else {
...
// Defined here because there is a circular dependency between OpsExecuter and PG
uint32_t
OpsExecuter::get_pool_stripe_width() const
{
...
void
OpsExecuter::prepare_cloning_ctx(
    ...
    ceph::os::Transaction& txn)
{
  ...
  logger().debug(
      "{} {} snapset={} snapc={}", __func__, soid, initial_snapset, snapc);
  ...
  cloning_ctx->new_snapset.clone_size[cloning_ctx->coid.snap] =
      initial_obs.oi.size;
  ...
  // log clone
  DEBUGDPP(
      "cloning v {} to {} v {} snaps={} snapset={}", *pg,
      initial_obs.oi.version, cloning_ctx->coid, osd_op_params->at_version,
      cloned_snaps, cloning_ctx->new_snapset);
}

pg_log_entry_t
OpsExecuter::complete_cloning_ctx()
{
  ...
  pg->get_backend().set_metadata(
      cloning_ctx->coid, cloning_ctx->clone_obc->obs.oi, nullptr /* snapset */,
      txn);
  ...
  ret.clean_regions.mark_data_region_dirty(
      0, cloning_ctx->clone_obc->obs.oi.size);
  ret.mtime = cloning_ctx->clone_obc->obs.oi.mtime;
  return ret;
}

void
OpsExecuter::update_clone_overlap()
{
  interval_set<uint64_t>* newest_overlap;
  if (cloning_ctx) {
    newest_overlap = &cloning_ctx->new_snapset.clone_overlap.rbegin()->second;
  } else if (
      op_info.may_write() && obc->obs.exists &&
      !obc->ssc->snapset.clones.empty()) {
    newest_overlap = &obc->ssc->snapset.clone_overlap.rbegin()->second;
  } else {
    return;
  }
...
ObjectContextRef
OpsExecuter::prepare_clone(const hobject_t& coid, const ObjectState& initial_obs)
{
  ...
  snapc(_snapc)
{
  if (op_info.may_write() && should_clone(*obc, snapc)) {
    do_write_op([this](auto& backend, auto& os, auto& txn) {
      prepare_cloning_ctx(
          std::as_const(snapc), std::as_const(obc->obs),
          std::as_const(obc->ssc->snapset), backend, txn);
    });
  }
}

static inline std::unique_ptr<const PGLSFilter>
get_pgls_filter(const std::string& type, bufferlist::const_iterator& iter)
{
  ...
  if (type.compare("plain") == 0) {
    ...
    if (!filter) {
      ...
      logger().warn(
          "buggy class {} failed to construct filter {}", class_name,
          filter_name);
      throw crimson::osd::invalid_argument{};
    }
  }
...
static PG::interruptible_future<hobject_t>
pgls_filter(
    ...
    const hobject_t& sobj)
{
  if (std::string xattr = filter.get_xattr(); !xattr.empty()) {
    logger().debug(
        "pgls_filter: filter is interested in xattr={} for obj={}", xattr, sobj);
...
static PG::interruptible_future<ceph::bufferlist>
do_pgnls_common(
...
{
  if (!(lower_bound.is_min() || lower_bound.is_max() ||
  ...
  return backend.list_objects(lower_bound, limit)
      ...
      .then_interruptible([pg_end](auto&& ret) {
        ...
        boost::push_back(
            response.entries, items | boost::adaptors::filtered(is_matched) |
                                  boost::adaptors::transformed(to_entry));
        response.handle = next.is_max() ? pg_end : next;
        ceph::bufferlist out;
        encode(response, out);
        logger().debug(
            "do_pgnls_common: response.entries.size()= {}",
            response.entries.size());
        return seastar::make_ready_future<ceph::bufferlist>(std::move(out));
      });
}

static PG::interruptible_future<>
do_pgnls(const PG& pg, const std::string& nspace, OSDOp& osd_op)
{
  ...
  return do_pgnls_common(
             pg_start, pg_end, pg.get_backend(), lower_bound, nspace,
             osd_op.op.pgls.count, nullptr /* no filter */)
...
static PG::interruptible_future<>
do_pgnls_filtered(const PG& pg, const std::string& nspace, OSDOp& osd_op)
{
  ...
  logger().debug(
      "{}: cname={}, mname={}, type={}, lower_bound={}, filter={}", __func__,
      cname, mname, type, lower_bound, static_cast<const void*>(filter.get()));
    ...
    const PGLSFilter* const filter)
{
  ...
  return backend.list_objects(lower_bound, limit)
      ...
      .then_interruptible([pg_end](auto&& ret) {
        ...
        encode(response, out);
        logger().debug(
            "{}: response.entries.size()=", __func__, response.entries.size());
        return seastar::make_ready_future<ceph::bufferlist>(std::move(out));
      });
}

static PG::interruptible_future<>
do_pgls(const PG& pg, const std::string& nspace, OSDOp& osd_op)
{
  hobject_t lower_bound;
  auto bp = osd_op.indata.cbegin();
  try {
...
static PG::interruptible_future<>
do_pgls_filtered(const PG& pg, const std::string& nspace, OSDOp& osd_op)
{
  std::string cname, mname, type;
  auto bp = osd_op.indata.cbegin();
  try {
