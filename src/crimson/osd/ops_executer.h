...
class OpsExecuter {
  ...
  using call_ierrorator = ::crimson::interruptible::
      interruptible_errorator<IOInterruptCondition, call_errorator>;
  using read_ierrorator = ::crimson::interruptible::
      interruptible_errorator<IOInterruptCondition, read_errorator>;
  using write_iertr = ::crimson::interruptible::
      interruptible_errorator<IOInterruptCondition, write_ertr>;
  using get_attr_ierrorator = ::crimson::interruptible::
      interruptible_errorator<IOInterruptCondition, get_attr_errorator>;
  using watch_ierrorator = ::crimson::interruptible::
      interruptible_errorator<IOInterruptCondition, watch_errorator>;

  template <typename Errorator, typename T = void>
  using interruptible_errorated_future = ::crimson::interruptible::
      interruptible_errorated_future<IOInterruptCondition, Errorator, T>;
  using interruptor =
      ::crimson::interruptible::interruptor<IOInterruptCondition>;
  template <typename T = void>
  using interruptible_future =
      ::crimson::interruptible::interruptible_future<IOInterruptCondition, T>;
...
public:
  ...
  public:
    ...
    bool
    has_flag(uint32_t flag) const final
    {
    ...
    entity_name_t
    get_source() const final
    {
      return pimpl->get_source();
    }

    uint64_t
    get_features() const final
    {
      return pimpl->get_features();
    }

    snapid_t
    get_snapid() const final
    {
  ...
  using osd_op_ierrorator = ::crimson::interruptible::
      interruptible_errorator<IOInterruptCondition, osd_op_errorator>;
...
private:
  ...
  using abstracted_msg_t =
      ceph::static_ptr<ExecutableMessage, sizeof(ExecutableMessagePimpl<void>)>;
   ...
   */
  static bool
  should_clone(const ObjectContext& initial_obc, const SnapContext& snapc)
  {
    // clone?
...
private:
  ...
  template <class Func>
  auto
  do_snapset_op(Func&& f)
  {
    ++num_read;
    return std::invoke(
        std::forward<Func>(f), std::as_const(obc->obs),
...
public:
  ...
  OpsExecuter(
    ...
    OpsExecuter(
        ...
        op_info,
        abstracted_msg_t{
            std::in_place_type_t<ExecutableMessagePimpl<MsgT>>{}, &msg, conn},
        conn,
        snapc)
  {}
  ...
  void
  check_init_op_params(OpsExecuter::modified_by m)
  {
    if (!osd_op_params) {
  ...
  size_t
  get_processed_rw_ops_num() const
  {
  ...
  bool
  has_cloning_ctx() const
  {
    return cloning_ctx != nullptr;
  }

  const hobject_t&
  get_cloning_coid() const
  {
...
template <class Context, class MainFunc, class EffectFunc>
auto
OpsExecuter::with_effect_on_obc(
    Context&& ctx,
    MainFunc&& main_func,
    EffectFunc&& effect_func)
{
  ...
  static_assert(
      std::is_convertible_v<EffectFunc, allowed_effect_func_t>,
  ...
  struct task_t final : effect_t {
    ...
    seastar::future<>
    execute(Ref<PG> pg) final
    {
      return std::move(effect_func)(
          std::move(ctx), std::move(obc), std::move(pg));
    }
  };
...
OpsExecuter::RollbackHelper<Func>::rollback_obc_if_modified()
{
  ...
  crimson::get_logger(ceph_subsys_osd)
      .debug(
          "{}: object {} got error, need_rollback={}", __func__,
          ox->obc->get_oid(), need_rollback);
  if (need_rollback) {
    func(ox->obc);
  }
}
