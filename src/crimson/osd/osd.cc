  ...
  clog(log_client.create_channel())
{
  ...
  for (auto msgr :
       {std::ref(cluster_msgr), std::ref(public_msgr), std::ref(hb_front_msgr),
  ...
  if (local_conf()->osd_open_classes_on_start) {
    ...
    if (r) {
      WARN(
          "warning: got an error loading one or more classes: {}",
          cpp_strerror(r));
    }
  }
  INFO("nonce is {}", nonce);
...
OSD::open_meta_coll()
{
  ...
  return store.get_sharded_store()
      .open_collection(coll_t::meta())
      .then([this, FNAME](auto ch) {
        DEBUG("registering metadata collection");
        pg_shard_manager.init_meta_coll(
            ch, store.get_backend_store(META_STORE_INDEX));
        return seastar::now();
      });
}
...
OSD::set_perf_queries(const ConfigPayload& config_payload)
{
  ...
  return shard_services.invoke_on_all([supported_queries,
                                       queries](auto& local_service) {
...
seastar::future<OSDMeta>
OSD::open_or_create_meta_coll(FuturizedStore& store)
{
  LOG_PREFIX(OSD::open_or_create_meta_coll);
  DEBUG("");
  return store.get_sharded_store()
      .open_collection(coll_t::meta())
      .then([&store, FNAME](auto ch) {
        if (!ch) {
          ...
          return seastar::make_ready_future<OSDMeta>(
              ch, store.get_backend_store(META_STORE_INDEX));
        }
      });
}
    ...
    std::string osdspec_affinity)
{
  ...
  co_await store.mkfs(osd_uuid).handle_error(
      crimson::stateful_ec::assert_failure(
          fmt::format(
              "{} error creating empty object store in {}", FNAME,
              local_conf().get_val<std::string>("osd_data"))
              .c_str()));

  DEBUG("mounting store mkfs");
  co_await store.mount().handle_error(crimson::stateful_ec::assert_failure(
      fmt::format(
          "{} error mounting object store in {}", FNAME,
          local_conf().get_val<std::string>("osd_data"))
          .c_str()));
  ...
  INFO(
      "created object store {} for osd.{} fsid {}\n",
    ...
    OSDSuperblock superblock)
{
  ...
  return seastar::do_with(
      std::move(meta_coll), std::move(superblock),
...
static std::string
to_string(const seastar::temporary_buffer<char>& temp_buf)
{
  return {temp_buf.get(), temp_buf.size()};
}

seastar::future<>
OSD::_write_key_meta(FuturizedStore& store)
{
  ...
  if (auto key = local_conf().get_val<std::string>("key"); !std::empty(key)) {
    ...
    return read_file(keyfile)
        ...
        .handle_exception([FNAME, keyfile](auto ep) {
          ERROR("_write_key_meta: failed to handle keyfile {}: {}", keyfile, ep);
          ceph_abort();
        });
  } else {
    return seastar::now();
...
replace_unknown_addrs(
    entity_addrvec_t maybe_unknowns,
    const entity_addrvec_t& knowns)
{
  ...
  std::transform(
      maybe_unknowns.v.begin(), maybe_unknowns.v.end(),
      std::back_inserter(replaced.v), maybe_replace);
...
OSD::start()
{
  ...
  if (auto cpu_cores = local_conf().get_val<std::string>("crimson_cpu_set");
      cpu_cores.empty()) {
  ...
  return store.start()
      .then([this](auto store_shard_nums) {
        return pg_to_shard_mappings
            .start(0, seastar::smp::count, store_shard_nums)
            .then([this] {
            ...
            .then([this, store_shard_nums] {
              ...
              return shard_services.start(
                  std::ref(osd_singleton_state), std::ref(pg_to_shard_mappings),
                  store_shard_nums, whoami, startup_time,
                  osd_singleton_state.local().perf,
                  osd_singleton_state.local().recoverystate_perf,
                  std::ref(store), std::ref(osd_states));
            });
      })
      .then([this, FNAME] {
        heartbeat.reset(new Heartbeat{
            whoami, get_shard_services(), *monc, *hb_front_msgr, *hb_back_msgr});
        DEBUG("mounting store");
        return store.mount().handle_error(crimson::stateful_ec::assert_failure(
            fmt::format(
                "{} error mounting object store in {}", FNAME,
                local_conf().get_val<std::string>("osd_data"))
                .c_str()));
      ...
      .then([this, FNAME] {
        auto stats_seconds =
            local_conf().get_val<int64_t>("crimson_osd_stat_interval");
        if (stats_seconds > 0) {
          ...
          stats_timer.set_callback([this, FNAME] {
            gate.dispatch_in_background("stats_osd", *this, [this, FNAME] {
              return shard_services
                  .invoke_on_all([this](auto& local_service) {
                  ...
                  .then([this, FNAME] {
                    ...
                    INFO(
                        "reactor_utilizations: {}({})", int(agg_ru / cnt),
                        oss.str());
                  });
            });
      ...
      .then([this](OSDMapService::local_cached_map_t&& map) {
        osdmap =
            make_local_shared_foreign(OSDMapService::local_cached_map_t(map));
      ...
      .then([this, FNAME] {
        uint64_t osd_required = CEPH_FEATURE_UID | CEPH_FEATURE_PGID64 |
                                CEPH_FEATURE_OSDENC;
        using crimson::net::SocketPolicy;

        public_msgr->set_default_policy(SocketPolicy::stateless_server(0));
        public_msgr->set_policy(
            entity_name_t::TYPE_MON, SocketPolicy::lossy_client(osd_required));
        public_msgr->set_policy(
            entity_name_t::TYPE_MGR, SocketPolicy::lossy_client(osd_required));
        public_msgr->set_policy(
            entity_name_t::TYPE_OSD, SocketPolicy::stateless_server(0));

        cluster_msgr->set_default_policy(SocketPolicy::stateless_server(0));
        cluster_msgr->set_policy(
            entity_name_t::TYPE_MON, SocketPolicy::lossy_client(0));
        cluster_msgr->set_policy(
            entity_name_t::TYPE_OSD, SocketPolicy::lossless_peer(osd_required));
        cluster_msgr->set_policy(
            entity_name_t::TYPE_CLIENT, SocketPolicy::stateless_server(0));
      ...
      .then_unpack([this, FNAME] {
        ...
        return seastar::when_all_succeed(monc->start(), mgrc->start());
      })
      .then_unpack([this, FNAME] {
        DEBUG("adding to crush");
        return _add_me_to_crush();
      })
      .then([this] { return _add_device_class(); })
      .then([this] {
        if (is_rotational.has_value()) {
          return shard_services.invoke_on_all([this](auto& local_service) {
            local_service.local_state.initialize_scheduler(
                local_service.get_cct(), *is_rotational);
          });
        } else {
...
seastar::future<>
OSD::_preboot(version_t oldest, version_t newest)
{
...
OSD::_send_boot()
{
  ...
  auto m = crimson::make_message<MOSDBoot>(
      superblock, osdmap->get_epoch(), boot_epoch, hb_back_addrs,
  ...
  auto get_weight = [this] {
    if (auto w = local_conf().get_val<double>("osd_crush_initial_weight");
      ...
      return store.stat().then([](auto st) {
        auto total = st.total;
        return seastar::make_ready_future<double>(std::max(
            .00001,
...
seastar::future<>
OSD::handle_command(crimson::net::ConnectionRef conn, Ref<MCommand> m)
{
  ceph_assert(seastar::this_shard_id() == PRIMARY_CORE);
...
OSD::start_asok_admin()
{
  ...
  return asok->start(asok_path).then([this] {
    ...
    asok->register_command(
        make_asok_hook<InjectDataErrorHook>(get_shard_services()));
    asok->register_command(
        make_asok_hook<InjectMDataErrorHook>(get_shard_services()));
    // PG commands
    ...
    // ops commands
    asok->register_command(
        make_asok_hook<DumpInFlightOpsHook>(std::as_const(pg_shard_manager)));
    ...
    asok->register_command(make_asok_hook<DumpReactorBackendHook>());
    asok->register_command(
...
OSD::stop()
{
  ...
  return prepare_to_stop()
      .then([this] { return pg_shard_manager.set_stopping(); })
      .then([FNAME, this] {
        ...
        return asok->stop()
            ...
            .then([this] {
              return when_all_succeed(
                         public_msgr->shutdown(), cluster_msgr->shutdown())
                  .discard_result();
            })
...
void
OSD::dump_status(Formatter* f) const
{
...
void
OSD::print(std::ostream& out) const
{
  out << "{osd." << superblock.whoami << " " << superblock.osd_fsid << " maps "
      << superblock.get_maps()
...
std::optional<seastar::future<>>
OSD::do_ms_dispatch(crimson::net::ConnectionRef conn, MessageRef m)
{
  ...
  case MSG_OSD_MARK_ME_DOWN:
    return handle_mark_me_down(
        conn, boost::static_pointer_cast<MOSDMarkMeDown>(m));
  ...
  case MSG_OSD_REPOPREPLY:
    return handle_rep_op_reply(
        conn, boost::static_pointer_cast<MOSDRepOpReply>(m));
  case MSG_OSD_SCRUB2:
    return handle_scrub_command(conn, boost::static_pointer_cast<MOSDScrub2>(m));
  case MSG_OSD_REP_SCRUB:
  case MSG_OSD_REP_SCRUBMAP:
    return handle_scrub_message(
        conn, boost::static_pointer_cast<MOSDFastDispatchOp>(m));
  case MSG_OSD_PG_UPDATE_LOG_MISSING:
    return handle_update_log_missing(
        conn, boost::static_pointer_cast<MOSDPGUpdateLogMissing>(m));
  case MSG_OSD_PG_UPDATE_LOG_MISSING_REPLY:
    return handle_update_log_missing_reply(
        conn, boost::static_pointer_cast<MOSDPGUpdateLogMissingReply>(m));
  case MSG_OSD_PG_PCT:
    return handle_pg_pct(conn, boost::static_pointer_cast<MOSDPGPCT>(m));
  case MSG_OSD_EC_WRITE:
    return handle_some_ec_messages(
        conn, boost::static_pointer_cast<MOSDECSubOpWrite>(m));
  case MSG_OSD_EC_WRITE_REPLY:
    return handle_some_ec_messages(
        conn, boost::static_pointer_cast<MOSDECSubOpWriteReply>(m));
  case MSG_OSD_EC_READ:
    return handle_some_ec_messages(
        conn, boost::static_pointer_cast<MOSDECSubOpRead>(m));
  case MSG_OSD_EC_READ_REPLY:
    return handle_some_ec_messages(
        conn, boost::static_pointer_cast<MOSDECSubOpReadReply>(m));
#if 0
  case MSG_OSD_PG_PUSH:
    [[fallthrough]];
  case MSG_OSD_PG_PUSH_REPLY:
...
void
OSD::ms_handle_reset(crimson::net::ConnectionRef conn, bool is_replace)
{
...
void
OSD::handle_authentication(const EntityName& name, const AuthCapsInfo& caps_info)
{
  ...
  gate.dispatch_in_background("statfs", *this, [this] {
    return store.stat().then([this](store_statfs_t&& st) {
      return get_shard_services().update_osd_stat(
          get_shard_services().get_up_epoch(), heartbeat->get_peers(), st);
    });
  });
}

seastar::future<MessageURef>
OSD::get_stats()
{
  // MPGStats::had_map_for is not used since PGMonitor was removed
  auto m =
      crimson::make_message<MPGStats>(monc->get_fsid(), osdmap->get_epoch());
  return get_shard_services()
      .get_osd_stat()
      .then([this, m = m.get()](auto&& osd_stat) {
      ...
      .then([this, m = std::move(m)](auto&& stats) mutable {
        ...
        for (auto [pgid, stat] : stats) {
          min_last_epoch_clean = std::min(
              min_last_epoch_clean, stat.get_effective_last_epoch_clean());
      ...
      .then([this](auto message) mutable {
        ...
        return seastar::do_with(
            std::move(m), pool_stat, pool_set,
            [this](auto&& msg, auto& pool_stat, auto& pool_set) {
              return seastar::do_for_each(
                         pool_set,
                         [this, &pool_stat](auto& pool_id) {
                           return store.pool_statfs(pool_id).then(
                               [pool_id, &pool_stat](store_statfs_t st) mutable {
                                 pool_stat[pool_id] = st;
                               });
                         })
                  .then([&pool_stat, msg = std::move(msg)]() mutable {
                    msg->pool_stat = std::move(pool_stat);
                    return seastar::make_ready_future<MessageURef>(
                        std::move(msg));
                  });
            });
      });
...
seastar::future<>
OSD::handle_osd_map(Ref<MOSDMap> m)
{
  ...
  */
  ...
  return handle_osd_map_lock.lock()
      .then([this, m] { return _handle_osd_map(m); })
      .finally([this] { return handle_osd_map_lock.unlock(); });
}

seastar::future<>
OSD::_handle_osd_map(Ref<MOSDMap> m)
{
  ...
  INFO(
      " epochs [{}..{}], i have {}, src has [{}..{}]", first, last,
      superblock.get_newest_map(), m->cluster_osdmap_trim_lower_bound,
      m->newest_map);
  ...
  if (superblock.cluster_osdmap_trim_lower_bound <
    ...
    superblock.cluster_osdmap_trim_lower_bound =
        m->cluster_osdmap_trim_lower_bound;
    DEBUG(
        "superblock cluster_osdmap_trim_lower_bound new epoch is: {}",
        superblock.cluster_osdmap_trim_lower_bound);
    ceph_assert(
        superblock.cluster_osdmap_trim_lower_bound >=
        superblock.get_oldest_map());
  ...
  if (first > start) {
    INFO("message skips epochs {}..{}", start, first - 1);
...
seastar::future<>
OSD::committed_osd_maps(epoch_t first, epoch_t last, Ref<MOSDMap> m)
{
  ...
  for (epoch_t cur = first; cur <= last; cur++) {
    OSDMapService::local_cached_map_t&& o =
        co_await pg_shard_manager.get_local_map(cur);
    ...
    co_await seastar::coroutine::parallel_for_each(
        old_osds, [this, FNAME, old_map](auto& osd_id) -> seastar::future<> {
          DEBUG(
              "osd.{}: whoami ? {}, old up ? {} , now down ? {}", osd_id,
              osd_id != whoami, old_map->is_up(osd_id),
              osdmap->is_down(osd_id));
          if (osd_id != whoami && old_map->is_up(osd_id) &&
              osdmap->is_down(osd_id)) {
  ...
  if (osdmap->is_up(whoami)) {
    ...
    INFO(
        "osd.{}: map e {} marked me up: up_from {}, bind_epoch {}, state {}",
    ...
    // yay!
    INFO(
        "osd.{}: committed_osd_maps: broadcasting osdmaps up"
        " to {} epoch to pgs",
        whoami, osdmap->get_epoch());
  ...
  if (!pg_shard_manager.is_stopping()) {
    ...
    */
    co_await get_shard_services().osdmap_subscribe(
...
seastar::future<>
OSD::handle_osd_op(crimson::net::ConnectionRef conn, Ref<MOSDOp> m)
{
  return pg_shard_manager
      .start_pg_operation<ClientRequest>(
          get_shard_services(), conn, std::move(m))
      .second;
}

seastar::future<>
OSD::handle_pg_create(crimson::net::ConnectionRef conn, Ref<MOSDPGCreate2> m)
{
...
seastar::future<>
OSD::handle_pg_pct(crimson::net::ConnectionRef conn, Ref<MOSDPGPCT> m)
{
...
seastar::future<>
OSD::handle_rep_op(crimson::net::ConnectionRef conn, Ref<MOSDRepOp> m)
{
...
seastar::future<>
OSD::handle_rep_op_reply(crimson::net::ConnectionRef conn, Ref<MOSDRepOpReply> m)
{
...
seastar::future<>
OSD::handle_scrub_command(crimson::net::ConnectionRef conn, Ref<MOSDScrub2> m)
{
  ...
  return seastar::parallel_for_each(
      std::move(m->scrub_pgs), [m, conn, this](spg_t pgid) {
        return pg_shard_manager
            .start_pg_operation<crimson::osd::ScrubRequested>(
...
seastar::future<>
OSD::handle_scrub_message(
    crimson::net::ConnectionRef conn,
    Ref<MOSDFastDispatchOp> m)
{
  return pg_shard_manager.start_pg_operation_active<crimson::osd::ScrubMessage>(
      m, conn, m->get_min_epoch(), m->get_spg());
}

seastar::future<>
OSD::handle_mark_me_down(crimson::net::ConnectionRef conn, Ref<MOSDMarkMeDown> m)
{
  ceph_assert(seastar::this_shard_id() == PRIMARY_CORE);
  if (pg_shard_manager.is_prestop()) {
    got_stop_ack();
  ...
  if (slow) {
    ...
    if (log_aggregated_slow_op && !slow_op_pools.empty()) {
      ...
      if (osdmap->get_pools().find(slow_pool_it->first) !=
        ...
        ss << "slow requests (most affected pool [ '" << pool_name
           << "' : " << slow_pool_it->second << " ])";
      }
      WARN("{}", ss.str());
      clog->warn() << ss.str();
...
bool
OSD::should_restart() const
{
  LOG_PREFIX(OSD::should_restart);
  if (!osdmap->is_up(whoami)) {
    INFO("map e {} marked osd.{} down", osdmap->get_epoch(), whoami);
    return true;
  } else if (osdmap->get_addrs(whoami) != public_msgr->get_myaddrs()) {
    ERROR(
        "map e {} had wrong client addr ({} != my {})", osdmap->get_epoch(),
        osdmap->get_addrs(whoami), public_msgr->get_myaddrs());
    return true;
  } else if (osdmap->get_cluster_addrs(whoami) != cluster_msgr->get_myaddrs()) {
    ERROR(
        "map e {} had wrong cluster addr ({} != my {})", osdmap->get_epoch(),
        osdmap->get_cluster_addrs(whoami), cluster_msgr->get_myaddrs());
    return true;
  } else {
    return false;
  }
...
OSD::handle_some_ec_messages(crimson::net::ConnectionRef conn, MessageRefT&& m)
{
  ...
  (void)pg_shard_manager.start_pg_operation<ECRepRequest>(
      std::move(conn), std::forward<MessageRefT>(m));
  return seastar::now();
}
...
OSD::send_beacon()
{
  ...
  auto beacon = crimson::make_message<MOSDBeacon>(
      osdmap->get_epoch(), min_last_epoch_clean,
...
seastar::future<double>
OSD::run_bench(int64_t count, int64_t bsize, int64_t osize, int64_t onum)
{
  ...
  if (osize && onum) {
    ...
    for (int i = 0; i < onum; ++i) {
      ...
      ghobject_t oid(
          hobject_t(sobject_t(object_t(oid_str), 0)), ghobject_t::NO_GEN,
          shard_id_t::NO_SHARD);
  ...
  for (int i = 0; i < count / bsize; ++i) {
    ...
    futures_bench.push_back(
        store.get_sharded_store().do_transaction(collection_ref, std::move(t)));
  ...
  co_await seastar::when_all_succeed(
      cleanup_futures.begin(), cleanup_futures.end());
...
seastar::future<>
OSD::handle_peering_op(crimson::net::ConnectionRef conn, Ref<MOSDPeeringOp> m)
{
  ...
  return pg_shard_manager
      .start_pg_operation<RemotePeeringEvent>(
          conn, pg_shard_t{from, m->get_spg().shard}, m->get_spg(),
          std::move(*evt))
      .second;
}

seastar::future<>
OSD::handle_pg_remove(crimson::net::ConnectionRef conn, Ref<MOSDPGRemove> m)
{
  ...
  for (auto& pg : m->pg_list) {
    DEBUG("{} from {}", pg, from);
    futs.emplace_back(pg_shard_manager
                          .start_pg_operation<RemotePeeringEvent>(
                              conn, pg_shard_t{from, pg.shard}, pg,
...
OSD::check_osdmap_features()
{
  ...
  if (osdmap->require_osd_release != last_require_osd_release) {
    DEBUG(
        "updating require_osd_release from {} to {}",
    ...
    return seastar::with_timeout(
               ...
               monc
                   ->send_message(crimson::make_message<MOSDMarkMeDown>(
                       monc->get_fsid(), whoami, osdmap->get_addrs(whoami),
                       osdmap->get_epoch(), true))
