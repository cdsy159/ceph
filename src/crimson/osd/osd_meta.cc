...
void
OSDMeta::create(ceph::os::Transaction& t)
{
  t.create_collection(coll->get_cid(), 0);
}

void
OSDMeta::store_map(ceph::os::Transaction& t, epoch_t e, const bufferlist& m)
{
  t.write(coll->get_cid(), osdmap_oid(e), 0, m.length(), m);
}

void
OSDMeta::store_inc_map(ceph::os::Transaction& t, epoch_t e, const bufferlist& m)
{
  t.write(coll->get_cid(), inc_osdmap_oid(e), 0, m.length(), m);
}

void
OSDMeta::remove_map(ceph::os::Transaction& t, epoch_t e)
{
  t.remove(coll->get_cid(), osdmap_oid(e));
}

void
OSDMeta::remove_inc_map(ceph::os::Transaction& t, epoch_t e)
{
...
OSDMeta::load_map(epoch_t e)
{
  return crimson::os::with_store<&crimson::os::FuturizedStore::Shard::read>(
             store, coll, osdmap_oid(e), 0, 0, CEPH_OSD_OP_FLAG_FADVISE_WILLNEED)
      .handle_error(read_errorator::assert_all_func([e](const auto&) {
        ceph_abort_msg(
            fmt::format("{} read gave enoent on {}", __func__, osdmap_oid(e)));
      }));
}

read_errorator::future<ceph::bufferlist>
OSDMeta::load_inc_map(epoch_t e)
{
  return crimson::os::with_store<&crimson::os::FuturizedStore::Shard::read>(
      store, coll, inc_osdmap_oid(e), 0, 0, CEPH_OSD_OP_FLAG_FADVISE_WILLNEED);
}

void
OSDMeta::store_superblock(
...
OSDMeta::load_superblock_ret
OSDMeta::load_superblock()
{
  LOG_PREFIX(OSDMeta::load_superblock);
  DEBUG("");
  return crimson::os::with_store<&crimson::os::FuturizedStore::Shard::read>(
             store, coll, superblock_oid(), 0, 0, 0)
...
seastar::future<std::tuple<pg_pool_t, std::string, OSDMeta::ec_profile_t>>
OSDMeta::load_final_pool_info(int64_t pool)
{
  return crimson::os::with_store<&crimson::os::FuturizedStore::Shard::read>(
             store, coll, final_pool_info_oid(pool), 0, 0, 0)
      .safe_then(
          [](bufferlist&& bl) {
            auto p = bl.cbegin();
            pg_pool_t pi;
            ...
            return seastar::make_ready_future<
                std::tuple<pg_pool_t, string, ec_profile_t>>(std::make_tuple(
                std::move(pi), std::move(name), std::move(ec_profile)));
          },
          read_errorator::assert_all_func([pool](const auto&) {
            throw std::runtime_error(fmt::format(
                "read gave enoent on {}", final_pool_info_oid(pool)));
          }));
}

void
OSDMeta::store_final_pool_info(
    ceph::os::Transaction& t,
  ...
  for (auto [e, map] : added_map) {
    ...
    for (auto& [pool_id, pool] : previous->get_pools()) {
      if (!map->have_pg_pool(pool_id)) {
        ...
        if (pool.is_erasure()) {
          profile =
              previous->get_erasure_code_profile(pool.erasure_code_profile);
        }
        encode(profile, bl);
...
ghobject_t
OSDMeta::inc_osdmap_oid(epoch_t epoch)
{
...
{
  return ghobject_t(
      hobject_t(sobject_t(object_t("osd_superblock"), CEPH_NOSNAP)));
}
