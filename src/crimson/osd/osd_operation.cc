...
OSDOperationRegistry::do_stop()
{
  ...
  std::transform(
      std::begin(historic_registry), std::end(historic_registry),
      std::back_inserter(to_ref_down), [](const Operation& op) {
...
static auto
get_duration(const ClientRequest& client_request)
{
...
void
OSDOperationRegistry::put_historic(const ClientRequest& op)
{
  ...
  if (get_duration(op) > local_conf()->osd_op_complaint_time) {
    ...
    _put_historic(
        slow_historic_registry, op, local_conf()->osd_op_history_slow_op_size);
  } else {
    auto& historic_registry = get_registry<historic_reg_index>();
    _put_historic(historic_registry, op, local_conf()->osd_op_history_size);
  }
}

void
OSDOperationRegistry::_put_historic(
    op_list& list,
    ...
    uint64_t max)
{
  ...
  ClientRequest::ICRef(&op, /* add_ref= */ true)
      .detach(); // yes, "leak" it for now!

  if (list.size() >= max) {
    auto old_op_ptr = &list.front();
    list.pop_front();
    const auto& old_op = static_cast<const ClientRequest&>(*old_op_ptr);
...
size_t
OSDOperationRegistry::dump_historic_client_requests(ceph::Formatter* f) const
{
  const auto& historic_client_registry = get_registry<static_cast<size_t>(
      OperationTypeCode::historic_client_request)>(); //ClientRequest::type)>();
...
size_t
OSDOperationRegistry::dump_slowest_historic_client_requests(
    ceph::Formatter* f) const
{
  const auto& slow_historic_client_registry = get_registry<static_cast<size_t>(
      OperationTypeCode::historic_slow_client_request)>(); //ClientRequest::type)>();
  f->open_object_section("op_history");
...
void
OSDOperationRegistry::visit_ops_in_flight(
    std::function<void(const ClientRequest&)>&& visit)
{
...
void
OperationThrottler::initialize_scheduler(
    CephContext* cct,
    ConfigProxy& conf,
    bool is_rotational,
    int whoami)
{
  scheduler = crimson::osd::scheduler::make_scheduler(
      cct, conf, whoami, seastar::smp::count, seastar::this_shard_id(),
      is_rotational, true);
  update_from_config(conf);
}

seastar::future<>
OperationThrottler::background_task()
{
  ...
  while (!stopped) {
    ...
    while (available() && !scheduler->empty() && !stopped) {
      ...
      if (auto when_ready = std::get_if<double>(&work_item)) {
        ceph::real_clock::time_point future_time =
      ...
      if (auto* item = std::get_if<crimson::osd::scheduler::item_t>(&work_item)) {
        ...
        --pending;
        DEBUG(
            "Updated counters during background_task: in_progress={}, "
            "pending={}",
            in_progress, pending);
      } else {
...
void
OperationThrottler::release_throttle()
{
  LOG_PREFIX(OperationThrottler::release_throttle);
  ceph_assert(in_progress > 0);
  --in_progress;
  DEBUG(
      "Updated counters during release_throttle: in_progress={}, pending={}",
      in_progress, pending);
  wake();
}

seastar::future<>
OperationThrottler::acquire_throttle(crimson::osd::scheduler::params_t params)
{
...
void
OperationThrottler::dump_detail(Formatter* f) const
{
...
void
OperationThrottler::update_from_config(const ConfigProxy& conf)
{
...
OperationThrottler::get_tracked_keys() const noexcept
{
  return {"crimson_osd_scheduler_concurrency"s};
}

void
OperationThrottler::handle_conf_change(
