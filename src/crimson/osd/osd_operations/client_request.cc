...
void
ClientRequest::Orderer::requeue(Ref<PG> pg)
{
...
void
ClientRequest::Orderer::clear_and_cancel(PG& pg)
{
...
void
ClientRequest::complete_request(PG& pg)
{
...
void
ClientRequest::print(std::ostream& lhs) const
{
  lhs << "m=[" << *m << "]";
}

void
ClientRequest::dump_detail(Formatter* f) const
{
  LOG_PREFIX(ClientRequest::dump_detail);
  TRACE("{}: dumping", *this);
  std::apply([f](auto... event) { (..., event.dump(f)); }, tracking_events);
  std::apply(
      [f](auto... event) { (..., event.dump(f)); },
...
{
  return get_osd_priv(&get_local_connection()).client_request_conn_pipeline;
}

PerShardPipeline&
ClientRequest::get_pershard_pipeline(ShardServices& shard_services)
{
  return shard_services.get_client_request_pipeline();
}

CommonPGPipeline&
ClientRequest::client_pp(PG& pg)
{
...
{
  return std::any_of(begin(m->ops), end(m->ops), [](auto& op) {
    return ceph_osd_op_type_pg(op.op.op);
  });
}
...
ClientRequest::interruptible_future<>
ClientRequest::with_pg_process_interruptible(
    Ref<PG> pgref,
    const unsigned this_instance_id,
    instance_handle_t& ihref)
{
  LOG_PREFIX(ClientRequest::with_pg_process);
  DEBUGDPP(
      "{}: same_interval_since: {}", *pgref, *this,
      pgref->get_interval_start_epoch());

  DEBUGDPP("{} start", *pgref, *this);
  PG& pg = *pgref;

  DEBUGDPP(
      "{}.{}: entering wait_pg_ready stage", *pgref, *this, this_instance_id);
  ...
  auto map_epoch = co_await interruptor::make_interruptible(ihref.enter_blocker(
      *this, pg.osdmap_gate, &decltype(pg.osdmap_gate)::wait_for_map,
      m->get_min_epoch(), nullptr));

  DEBUGDPP(
      "{}.{}: waited for epoch {}, waiting for active", pg, *this,
      this_instance_id, map_epoch);
  co_await interruptor::make_interruptible(ihref.enter_blocker(
      *this, pg.wait_for_active_blocker,
      &decltype(pg.wait_for_active_blocker)::wait));

  DEBUGDPP(
      "{}.{}: waited for active, entering get_obc stage ", pg, *this,
      this_instance_id);
  ...
  if (int res = op_info.set_from_op(&*m, *pg.get_osdmap()); res != 0) {
  ...
  if (!pg.is_primary()) {
    ...
    if (is_misdirected_replica_read(pg)) {
      DEBUGDPP("{}.{}: dropping misdirected op", pg, *this, this_instance_id);
      co_return;
    }
    ...
    if (pg.is_missing_head_and_clones(m->get_hobj())) {
      DEBUGDPP(
          "{}.{}: {} possibly missing head or clone object on replica,"
          " bouncing to primary",
      ...
      DEBUGDPP(
          "{}.{}: unstable write on replica, bouncing to primary", pg, *this,
          this_instance_id);
      ...
      DEBUGDPP(
          "{}.{}: serving replica read on oid {}", pg, *this, this_instance_id,
          m->get_hobj());
  ...
  DEBUGDPP(
      "{}.{}: pg active, entering process[_pg]_op", *pgref, *this,
      this_instance_id);

  co_await (
      is_pg_op() ? process_pg_op(pgref)
                 : process_op(ihref, pgref, this_instance_id));

  DEBUGDPP(
      "{}.{}: process[_pg]_op complete, completing handle", *pgref, *this,
      this_instance_id);
  co_await interruptor::make_interruptible(ihref.handle.complete());
}

seastar::future<>
ClientRequest::with_pg_process(Ref<PG> pgref)
{
  ...
  return interruptor::with_interruption(
             [FNAME, this, pgref, this_instance_id, &ihref]() mutable {
               return with_pg_process_interruptible(
                   ...
                   .then_interruptible([FNAME, this, this_instance_id, pgref] {
                     DEBUGDPP(
                         "{}.{}: with_pg_process_interruptible complete,"
                         " completing request",
             ...
             [FNAME, this, this_instance_id, pgref](std::exception_ptr eptr) {
               DEBUGDPP(
                   "{}.{}: interrupted due to {}", *pgref, *this,
                   this_instance_id, eptr);
             },
             pgref, pgref->get_osdmap_epoch())
      .finally([this, FNAME, opref = std::move(opref), pgref, this_instance_id,
                instance_handle = std::move(instance_handle), &ihref]() mutable {
        DEBUGDPP("{}.{}: exit", *pgref, *this, this_instance_id);
        return ihref.handle.complete().finally(
            [instance_handle = std::move(instance_handle)] {});
      });
}

seastar::future<>
ClientRequest::with_pg(ShardServices& _shard_services, Ref<PG> pgref)
{
  shard_services = &_shard_services;
...
ClientRequest::interruptible_future<>
ClientRequest::process_pg_op(Ref<PG> pg)
{
...
ClientRequest::interruptible_future<>
ClientRequest::recover_missing_snaps(Ref<PG> pg, std::set<snapid_t>& snaps)
{
  ...
  auto resolve_oids =
       ...
       * See https://tracker.ceph.com/issues/63821 */
          ...
          .handle_error_interruptible(crimson::ct_error::assert_all(
              fmt::format("{} {} error", *pg, FNAME).c_str()));
    ...
    unsigned this_instance_id)
{
  ...
  auto obc_manager =
      pg->obc_loader.get_obc_manager(*(ihref.obc_orderer), m->get_hobj());
  co_await ihref.enter_stage<interruptor>(
      ihref.obc_orderer->obc_pp().process, *this);

  if (!pg->is_primary()) {
    DEBUGDPP(
        "Skipping recover_missings on non primary pg for soid {}", *pg,
        m->get_hobj());
  } else {
  ...
  DEBUGDPP("{}.{}: checking already_complete", *pg, *this, this_instance_id);
  auto completed = co_await pg->already_complete(m->get_reqid());

  if (completed) {
    DEBUGDPP(
        "{}.{}: already completed, sending reply", *pg, *this, this_instance_id);
    auto reply = crimson::make_message<MOSDOpReply>(
  ...
  DEBUGDPP(
      "{}.{}: not completed, about to wait_scrub", *pg, *this, this_instance_id);
  co_await ihref.enter_blocker(
      *this, pg->scrubber, &decltype(pg->scrubber)::wait_scrub, m->get_hobj());

  DEBUGDPP(
      "{}.{}: past scrub blocker, getting obc", *pg, *this, this_instance_id);

  int load_err =
      co_await pg->obc_loader
          .load_and_lock(obc_manager, pg->get_lock_type(op_info))
          .si_then([]() -> int { return 0; })
          .handle_error_interruptible(PG::load_obc_ertr::all_same_way(
              [](const auto& code) -> int { return -code.value(); }));
  if (load_err) {
    DEBUGDPP(
        "{}.{}: saw error code loading obc {}", *pg, *this, this_instance_id,
        load_err);
  ...
  DEBUGDPP(
      "{}.{}: obc {} loaded and locked, calling do_process", *pg, *this,
      this_instance_id, obc_manager.get_obc()->obs);
  co_await do_process(ihref, pg, obc_manager.get_obc(), this_instance_id);
}
    ...
    unsigned this_instance_id)
{
  ...
  if (pool.has_flag(pg_pool_t::FLAG_EIO)) {
    ...
    if (m->has_flag(CEPH_OSD_FLAG_SUPPORTSPOOLEIO)) {
      DEBUGDPP(
          "{}.{}: discarding op due to pool EIO flag", *pg, *this,
          this_instance_id);
      co_return;
    } else {
      DEBUGDPP(
          "{}.{}: replying EIO due to pool EIO flag", *pg, *this,
          this_instance_id);
  ...
  if (m->get_oid().name.size() >
    ...
    co_return;
      ...
      m->get_hobj().get_key().size() >
      crimson::common::local_conf()->osd_max_object_name_len) {
    co_await reply_op_error(pg, -ENAMETOOLONG);
    co_return;
  } else if (
      m->get_hobj().nspace.size() >
    ...
    DEBUGDPP(
        "{}.{}: {} is blocklisted", *pg, *this, this_instance_id,
        get_foreign_connection().get_peer_addr());
  ...
  if ((m->has_flag(CEPH_OSD_FLAG_ORDERSNAP)) &&
    ...
    DEBUGDPP(
        ...
        *pg, *this, this_instance_id, snapc.seq, obc->ssc->snapset.seq,
        obc->obs.oi.soid);
  ...
  size_t inb = 0, outb = 0;
  {
    ...
    if (ret) {
      ...
      if (op_info.may_write()) {
        ...
        auto version =
            co_await pg->submit_error_log(m, op_info, obc, *ret, rep_tid);

        all_completed = pg->complete_error_log(rep_tid, version);
      }
      // simply return the error below, leaving all_completed alone
    } else {
      auto submitted = interruptor::now();
      inb = ox.get_bytes_written();
      std::tie(submitted, all_completed) =
  ...
  if (ret) {
    ...
    auto reply = crimson::make_message<MOSDOpReply>(
        m.get(), result, pg->get_osdmap_epoch(), 0, false);
    reply->add_flags(CEPH_OSD_FLAG_ACK | CEPH_OSD_FLAG_ONDISK);
    if (obc->obs.exists) {
      reply->set_reply_versions(
          pg->peering_state.get_info().last_update, obc->obs.oi.user_version);
    } else {
      reply->set_reply_versions(
          pg->peering_state.get_info().last_update,
          pg->peering_state.get_info().last_user_version);
    ...
    DEBUGDPP("{}.{}: sending response {}", *pg, *this, this_instance_id, *m);
    // TODO: gate the crosscore sending
    co_await interruptor::make_interruptible(
        get_foreign_connection().send_with_throttling(std::move(reply)));
  }
...
bool
ClientRequest::is_misdirected_replica_read(const PG& pg) const
{
  LOG_PREFIX(ClientRequest::is_misdirected_replica_read);
  // otherwise take a closer look
  if (const int flags = m->get_flags(); flags & CEPH_OSD_FLAG_BALANCE_READS ||
                                        flags & CEPH_OSD_FLAG_LOCALIZE_READS) {
    if (op_info.rwordered()) {
      DEBUGDPP(
          "{}: dropping - rwoedered with balanced/localize read {}", pg, *this);
      return true;
    }
    if (!op_info.may_read()) {
      DEBUGDPP(
          "{}: dropping - no read found with balanced/localize read", pg, *this);
      return true;
    }
...
void
ClientRequest::put_historic() const
{
  ceph_assert_always(shard_services);
...
const SnapContext
ClientRequest::get_snapc(PG& pg, crimson::osd::ObjectContextRef obc) const
{
  ...
  if (op_info.may_write() || op_info.may_cache()) {
    ...
    if (pg.get_pgpool().info.is_pool_snaps_mode()) {
      ...
      DEBUGDPP("{} using pool's snapc snaps={}", pg, *this, snapc.snaps);
    } else {
      // client specified snapc
      snapc.seq = m->get_snap_seq();
      snapc.snaps = m->get_snaps();
      DEBUGDPP(
          "{}: client specified snapc seq={} snaps={}", pg, *this, snapc.seq,
          snapc.snaps);
    }
  }
  return snapc;
