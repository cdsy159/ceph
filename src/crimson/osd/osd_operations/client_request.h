...
public:
  ...
  may_write() const
  {
    return op_info.may_write();
  }

  bool
  may_cache() const
  {
    return op_info.may_cache();
  }

  bool
  may_read() const
  {
    return op_info.may_read();
  }
  ...
  class instance_handle_t
    : public boost::intrusive_ref_counter<instance_handle_t> {
  public:
    ...
    auto
    with_blocking_event(F&& f, ClientRequest& op)
    {
      ...
      if constexpr (std::is_same_v<InterruptorT, void>) {
        ...
        return
            typename InterruptorT::template futurize_t<ret_t>{std::move(ret)};
      }
    }

    template <typename InterruptorT = void, typename StageT>
    auto
    enter_stage(StageT& stage, ClientRequest& op)
    {
      return this->template with_blocking_event<
          typename StageT::BlockingEvent, InterruptorT>(
    ...
    void
    enter_stage_sync(StageT& stage, ClientRequest& op)
    {
    ...
    template <
        typename InterruptorT = void,
        typename BlockingObj,
        typename Method,
        typename... Args>
    auto
    enter_blocker(
    ...
    {
      return this->template with_blocking_event<
          typename BlockingObj::Blocker::BlockingEvent, InterruptorT>(
          [&obj, method, args = std::forward_as_tuple(std::move(args)...)](
              auto&& trigger) mutable {
            return apply_method_to_tuple(
                obj, method,
                std::tuple_cat(
                    std::forward_as_tuple(std::move(trigger)), std::move(args)));
          },
          op);
    }
  };
  ...
  public:
    void
    add_request(ClientRequest& request)
    {
    ...
    void
    remove_request(ClientRequest& request)
    {
      assert(request.ordering_hook.is_linked());
...
private:
  ...
  process_pg_op(Ref<PG> pg);
  interruptible_future<> recover_missing_snaps(
      Ref<PG> pg,
      std::set<snapid_t>& snaps);
  ::crimson::interruptible::interruptible_future<
      ::crimson::osd::IOInterruptCondition>
  process_op(instance_handle_t& ihref, Ref<PG> pg, unsigned this_instance_id);
  bool is_pg_op() const;

  CommonPGPipeline& client_pp(PG& pg);

  template <typename Errorator>
  using interruptible_errorator = ::crimson::interruptible::
      interruptible_errorator<::crimson::osd::IOInterruptCondition, Errorator>;

  bool is_misdirected_replica_read(const PG& pg) const;

  const SnapContext get_snapc(PG& pg, crimson::osd::ObjectContextRef obc) const;
...
template <>
struct fmt::formatter<crimson::osd::ClientRequest> : fmt::ostream_formatter {};
#endif
