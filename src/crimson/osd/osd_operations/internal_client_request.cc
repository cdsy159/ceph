...
void
InternalClientRequest::print(std::ostream&) const
{}

void
InternalClientRequest::dump_detail(Formatter* f) const
{}

CommonPGPipeline&
InternalClientRequest::client_pp()
{
...
InternalClientRequest::with_interruption()
{
  ...
  auto obc_manager =
      pg->obc_loader.get_obc_manager(*obc_orderer, get_target_oid());

  co_await enter_stage<interruptor>(obc_orderer->obc_pp().process);

  bool unfound =
      co_await pg->do_recover_missing(get_target_oid(), osd_reqid_t());
  ...
  DEBUGI("InternalClientRequest: got {} OSDOps to execute", std::size(osd_ops));
  [[maybe_unused]] const int ret = op_info.set_from_op(
      std::as_const(osd_ops), pg->get_pgid().pgid, *pg->get_osdmap());
  assert(ret == 0);

  co_await pg->obc_loader.load_and_lock(obc_manager, pg->get_lock_type(op_info))
      .handle_error_interruptible(
          crimson::ct_error::assert_all(fmt::format(
                                            "{} {} {} error when loading {}",
                                            *pg, FNAME, *this, get_target_oid())
                                            .c_str()));
  ...
  OpsExecuter ox(
      ...
      SnapContext{});
  co_await pg->run_executer(ox, obc_manager.get_obc(), op_info, osd_ops)
      .handle_error_interruptible(
          crimson::ct_error::assert_all(fmt::format(
  ...
  auto [submitted, completed] =
      co_await pg->submit_executer(std::move(ox), osd_ops);
...
InternalClientRequest::start()
{
  ...
  return interruptor::with_interruption(
             [this]() mutable { return with_interruption(); },
             [](std::exception_ptr eptr) { return seastar::now(); }, pg,
             start_epoch)
