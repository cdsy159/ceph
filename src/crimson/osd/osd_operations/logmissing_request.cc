...
LogMissingRequest::LogMissingRequest(
...
void
LogMissingRequest::print(std::ostream& os) const
{
  os << "LogMissingRequest(" << "from=" << req->from << " req=" << *req << ")";
}

void
LogMissingRequest::dump_detail(Formatter* f) const
{
...
{
  return get_osd_priv(&get_local_connection()).replicated_request_conn_pipeline;
}

PerShardPipeline&
LogMissingRequest::get_pershard_pipeline(ShardServices& shard_services)
{
  return shard_services.get_replicated_request_pipeline();
}

PGRepopPipeline&
LogMissingRequest::repop_pipeline(PG& pg)
{
  return pg.repop_pipeline;
}

seastar::future<>
LogMissingRequest::with_pg(ShardServices& shard_services, Ref<PG> pg)
{
             ...
             [this, pg] {
               ...
               return this
                   ...
                   .then_interruptible([this, pg](auto) {
                     return pg->do_update_log_missing(
                         req, get_remote_connection());
                   })
