...
void
LogMissingRequestReply::print(std::ostream& os) const
{
  os << "LogMissingRequestReply(" << "from=" << req->from << " req=" << *req
     << ")";
}

void
LogMissingRequestReply::dump_detail(Formatter* f) const
{
...
ConnectionPipeline&
LogMissingRequestReply::get_connection_pipeline()
{
  return get_osd_priv(&get_local_connection()).replicated_request_conn_pipeline;
}

PerShardPipeline&
LogMissingRequestReply::get_pershard_pipeline(ShardServices& shard_services)
{
  return shard_services.get_replicated_request_pipeline();
}

seastar::future<>
LogMissingRequestReply::with_pg(ShardServices& shard_services, Ref<PG> pg)
{
  ...
  return interruptor::with_interruption(
             [this, pg] {
               return pg->do_update_log_missing_reply(req).then_interruptible(
                   [this] {
