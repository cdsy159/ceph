...
template <class T>
void
PeeringEvent<T>::print(std::ostream& lhs) const
{
  lhs << "PeeringEvent(" << "from=" << from << " pgid=" << pgid
...
template <class T>
void
PeeringEvent<T>::dump_detail(Formatter* f) const
{
...
template <class T>
PGPeeringPipeline&
PeeringEvent<T>::peering_pp(PG& pg)
{
...
template <class T>
seastar::future<>
PeeringEvent<T>::with_pg(ShardServices& shard_services, Ref<PG> pg)
{
  ...
  return interruptor::with_interruption(
             [this, pg, &shard_services] {
               LOG_PREFIX(PeeringEvent<T>::with_pg);
               DEBUGI(
                   "{} {}: pg present", interruptor::get_interrupt_cond(),
                   *this);
               return this
                   ->template enter_stage<interruptor>(peering_pp(*pg).await_map)
...
template <class T>
void
PeeringEvent<T>::on_pg_absent(ShardServices&)
{
  using interruptor = typename T::interruptor;
...
ConnectionPipeline&
RemotePeeringEvent::get_connection_pipeline()
{
  return get_osd_priv(&get_connection()).peering_request_conn_pipeline;
}

PerShardPipeline&
RemotePeeringEvent::get_pershard_pipeline(ShardServices& shard_services)
{
  return shard_services.get_peering_request_pipeline();
}

void
RemotePeeringEvent::on_pg_absent(ShardServices& shard_services)
{
  if (auto& e = get_event().get_event();
    ...
    if (q.query.type == q.query.LOG || q.query.type == q.query.FULLLOG) {
      auto m = crimson::make_message<MOSDPGLog>(
          q.query.from, q.query.to, map_epoch, empty, q.query.epoch_sent);
      ctx.send_osd_message(q.from.osd, std::move(m));
    } else {
      ctx.send_notify(
          q.from.osd, {q.query.from, q.query.to, q.query.epoch_sent, map_epoch,
                       empty, PastIntervals{}, PG_FEATURE_CRIMSON_ALL});
    }
  }
}

RemotePeeringEvent::interruptible_future<>
RemotePeeringEvent::complete_rctx(ShardServices& shard_services, Ref<PG> pg)
{
  if (pg) {
...
seastar::future<>
RemotePeeringEvent::complete_rctx_no_pg(ShardServices& shard_services)
{
  return shard_services.dispatch_context_messages(std::move(ctx));
}

seastar::future<>
LocalPeeringEvent::start()
{
  ...
  return maybe_delay
      .then([this] { return with_pg(pg->get_shard_services(), pg); })
