...
protected:
  ...
  virtual typename PeeringEvent::template interruptible_future<> complete_rctx(
      ShardServices&,
      Ref<PG>);

  virtual seastar::future<>
  complete_rctx_no_pg(ShardServices& shard_services)
  {
    return seastar::now();
  }
...
public:
  ...
  PeeringEvent(const pg_shard_t& from, const spg_t& pgid, Args&&... args) :
    from(from), pgid(pgid), evt(std::forward<Args>(args)...)
  {}
  ...
  PeeringEvent(
      ...
      float delay,
      Args&&... args) :
    from(from), pgid(pgid), delay(delay), evt(std::forward<Args>(args)...)
  {}

  bool
  requires_pg() const final
  {
...
public:
  ...
  RemotePeeringEvent(crimson::net::ConnectionRef conn, Args&&... args) :
    PeeringEvent(std::forward<Args>(args)...), RemoteOperation(std::move(conn))
  {}
...
public:
  ...
  LocalPeeringEvent(Ref<PG> pg, Args&&... args) :
    PeeringEvent(std::forward<Args>(args)...), pg(pg)
  {}
...
template <>
struct fmt::formatter<crimson::osd::LocalPeeringEvent>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::osd::RemotePeeringEvent>
  : fmt::ostream_formatter {};

template <class T>
struct fmt::formatter<crimson::osd::PeeringEvent<T>> : fmt::ostream_formatter {
};
#endif
