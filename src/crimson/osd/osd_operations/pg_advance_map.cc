...
void
PGAdvanceMap::print(std::ostream& lhs) const
{
  lhs << "PGAdvanceMap(" << "pg=" << pg->get_pgid()
      << " from=" << (from ? *from : -1) << " to=" << to;
  if (do_init) {
    lhs << " do_init";
  }
  lhs << ")";
...
void
PGAdvanceMap::dump_detail(Formatter* f) const
{
     ...
     */
        ...
        return seastar::do_for_each(
                   ...
                   [this, FNAME](epoch_t next_epoch) {
                     ...
                     return shard_services.get_map(next_epoch)
                         .then([this, FNAME](cached_map_t&& next_map) {
                           DEBUG(
                               "{}: advancing map to {}", *this,
                               next_map->get_epoch());
                           ...
                           DEBUG(
                               "{}: checking for splits between {} and {}",
...
seastar::future<>
PGAdvanceMap::check_for_splits(epoch_t old_epoch, cached_map_t next_map)
{
...
seastar::future<>
PGAdvanceMap::split_pg(std::set<spg_t> split_children, cached_map_t next_map)
{
  ...
  for (auto child_pgid : split_children) {
    ...
    auto core = co_await shard_services.create_split_pg_mapping(
        child_pgid, seastar::this_shard_id(), pg->get_store_index());
    DEBUG(" PG {} mapped to {}", child_pgid.pgid, core);
    DEBUG(" {} map epoch: {}", child_pgid.pgid, pg_epoch);
    auto map = next_map;
    auto child_pg = co_await shard_services.make_pg(
        std::move(map), child_pgid, pg->get_store_index(), true);
    ...
    DEBUG(
        " pg num is {}, m_seed is {}, split bits is {}", new_pg_num,
        child_pg->get_pgid().ps(), split_bits);

    co_await pg->split_colls(
        child_pg->get_pgid(), split_bits, child_pg->get_pgid().ps(),
    ...
    rctx.transaction.touch(
        child_coll_ref->get_cid(), child_pg->get_pgid().make_snapmapper_oid());
...
seastar::future<>
PGAdvanceMap::handle_split_pg_creation(Ref<PG> child_pg, cached_map_t next_map)
{
  ...
  auto fut =
      shard_services.create_split_pg(std::move(trigger), child_pg->get_pgid())
          .handle_error(crimson::ct_error::ecanceled::handle([FNAME](auto) {
            DEBUG("PG creation canceled");
            return seastar::make_ready_future<Ref<PG>>();
          }));
  co_await shard_services
      .start_operation<PGAdvanceMap>(
          child_pg, shard_services, next_map->get_epoch(), std::move(rctx), true)
      .second;
  co_await std::move(fut);
}

void
PGAdvanceMap::split_stats(
    std::set<Ref<PG>> children_pgs,
    const std::set<spg_t>& children_pgids)
{
  ...
  for (std::set<Ref<PG>>::const_iterator iter = children_pgs.begin();
       iter != children_pgs.end(); ++iter, ++stat_iter) {
