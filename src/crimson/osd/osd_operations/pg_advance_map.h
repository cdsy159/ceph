...
public:
  ...
  seastar::future<> check_for_splits(epoch_t old_epoch, cached_map_t next_map);
  seastar::future<> split_pg(
      std::set<spg_t> split_children,
      cached_map_t next_map);
  void split_stats(
      std::set<Ref<PG>> child_pgs,
      const std::set<spg_t>& child_pgids);

  std::tuple<PGPeeringPipeline::Process::BlockingEvent, PGMap::PGCreationBlockingEvent>
      tracking_events;
...
template <>
struct fmt::formatter<crimson::osd::PGAdvanceMap> : fmt::ostream_formatter {};
#endif
