...
void
PGPCTRequest::print(std::ostream& os) const
{
  os << "PGPCTRequest(" << " req=" << *req << ")";
}

void
PGPCTRequest::dump_detail(Formatter* f) const
{
...
ConnectionPipeline&
PGPCTRequest::get_connection_pipeline()
{
  return get_osd_priv(&get_local_connection()).client_request_conn_pipeline;
}

PerShardPipeline&
PGPCTRequest::get_pershard_pipeline(ShardServices& shard_services)
{
  return shard_services.get_replicated_request_pipeline();
}

PGPCTRequest::interruptible_future<>
PGPCTRequest::with_pg_interruptible(PG& pg)
{
...
seastar::future<>
PGPCTRequest::with_pg(ShardServices& shard_services, Ref<PG> pgref)
{
  ...
  return interruptor::with_interruption(
      ...
      .finally([FNAME, this, ref = std::move(ref),
                pgref = std::move(pgref)]() mutable {
        DEBUGDPP("exit", *pgref, *this);
        return handle.complete().then([ref = std::move(ref),
                                       pgref = std::move(pgref)] {});
      });
}
