...
template <class T>
PGPeeringPipeline&
RemoteScrubEventBaseT<T>::get_peering_pipeline(PG& pg)
{
...
template <class T>
ConnectionPipeline&
RemoteScrubEventBaseT<T>::get_connection_pipeline()
{
  return get_osd_priv(&get_local_connection()).peering_request_conn_pipeline;
}

template <class T>
PerShardPipeline&
RemoteScrubEventBaseT<T>::get_pershard_pipeline(ShardServices& shard_services)
{
...
template <class T>
seastar::future<>
RemoteScrubEventBaseT<T>::with_pg(ShardServices& shard_services, Ref<PG> pg)
{
  ...
  return interruptor::with_interruption(
      [FNAME, this, pg] {
        ...
        return this
            ->template enter_stage<interruptor>(
            ...
            .then_interruptible([this, pg] { return handle_event(*pg); });
      },
...
ScrubRequested::ifut<>
ScrubRequested::handle_event(PG& pg)
{
...
ScrubMessage::ifut<>
ScrubMessage::handle_event(PG& pg)
{
...
template <typename T>
ScrubAsyncOpT<T>::ScrubAsyncOpT(Ref<PG> pg) :
  pg(pg)
{}

template <typename T>
typename ScrubAsyncOpT<T>::template ifut<>
ScrubAsyncOpT<T>::start()
{
...
ScrubReserveRange::ifut<>
ScrubReserveRange::run(PG& pg)
{
  ...
  return pg.background_process_lock.lock()
      .then_interruptible([FNAME, this, &pg] {
        ...
        auto p =
            find_if(log.crbegin(), log.crend(), [this](const auto& e) -> bool {
...
ScrubScan::run(PG& pg)
{
  ...
  for (const auto& object : objects) {
    co_await scan_object(
        pg, ghobject_t(object, ghobject_t::NO_GEN, pg.get_pgid().shard));
  }

  if (local) {
    DEBUGDPP("complete, submitting local event", pg);
    pg.scrubber.handle_event(scrub::ScrubContext::scan_range_complete_t(
        pg.get_pg_whoami(), std::move(ret)));
  } else {
    ...
    auto m = crimson::make_message<MOSDRepScrubMap>(
        ...
        pg.get_osdmap_epoch(), pg.get_pg_whoami());
    encode(ret, m->get_data());
    pg.scrubber.handle_event(
        scrub::ScrubContext::generate_and_submit_chunk_result_complete_t{});
    co_await interruptor::make_interruptible(pg.shard_services.send_to_osd(
        pg.get_primary().osd, std::move(m), pg.get_osdmap_epoch()));
  }
}

ScrubScan::ifut<>
ScrubScan::scan_object(PG& pg, const ghobject_t& obj)
{
  ...
  return interruptor::make_interruptible(
      ...
      .handle_error_interruptible(ct_error::all_same_way([FNAME, &pg, &obj,
                                                          &entry](auto e) {
...
ScrubScan::ifut<>
ScrubScan::deep_scan_object(PG& pg, const ghobject_t& obj)
{
  ...
  co_await interruptor::repeat(
      [FNAME, this, &progress, &obj, &entry,
       &pg]() -> interruptible_future<seastar::stop_iteration> {
        auto store_read =
            [FNAME, this, &progress, &obj, &entry,
             &pg]() -> interruptible_future<seastar::stop_iteration> {
          DEBUGDPP(
              "op: {}, obj: {}, progress: {} scanning data", pg, *this, obj,
              progress);
          const auto stride =
              local_conf().get_val<Option::size_t>("osd_deep_scrub_stride");
          return crimson::os::with_store<
                     &crimson::os::FuturizedStore::Shard::read>(
                     pg.shard_services.get_store(pg.get_store_index()),
                     pg.get_collection_ref(), obj, *(progress.offset), stride,
                     0)
              .safe_then([this, FNAME, stride, &obj, &progress, &entry,
                          &pg](auto bl) {
                size_t offset = *progress.offset;
                DEBUGDPP(
                    "op: {}, obj: {}, progress: {} got offset {}", pg, *this,
                    obj, progress, offset);
              ...
              .handle_error(ct_error::all_same_way([&progress, &entry](auto e) {
        ...
        auto get_header =
            [FNAME, this, &progress, &obj, &entry,
             &pg]() -> interruptible_future<seastar::stop_iteration> {
          DEBUGDPP(
              "op: {}, obj: {}, progress: {} scanning omap header", pg, *this,
              obj, progress);
          return crimson::os::with_store<
                     &crimson::os::FuturizedStore::Shard::omap_get_header>(
                     pg.shard_services.get_store(pg.get_store_index()),
                     pg.get_collection_ref(), obj, 0)
              .safe_then([&progress](auto bl) { progress.omap_hash << bl; })
              .handle_error(
        ...
        auto get_keys =
            [FNAME, this, &progress, &obj, &entry, &pg, start_from,
             callback]() -> interruptible_future<seastar::stop_iteration> {
          DEBUGDPP(
              "op: {}, obj: {}, progress: {} scanning omap keys", pg, *this,
              obj, progress);
          return crimson::os::with_store<
                     &crimson::os::FuturizedStore::Shard::omap_iterate>(
                     pg.shard_services.get_store(pg.get_store_index()),
                     pg.get_collection_ref(), obj, start_from, callback, 0,
                     nullptr)
              .safe_then([FNAME, this, &obj, &progress, &entry,
                          &pg](auto result) {
                assert(result == ObjectStore::omap_iter_ret_t::NEXT);
                DEBUGDPP(
                    "op: {}, obj: {}, progress: {} omap done", pg, *this, obj,
                    progress);
                ...
                if ((entry.object_omap_keys >
                    ...
                    (entry.object_omap_bytes >
                     local_conf().get_val<Option::size_t>(
                         "osd_deep_scrub_large_omap_object_value_sum_"
                         "threshold"))) {
              ...
              .handle_error(ct_error::all_same_way(
                  [FNAME, this, &obj, &progress, &entry, &pg](auto e) {
                    DEBUGDPP(
                        "op: {}, obj: {}, progress: {} error reading omap {}",
        ...
        if (progress.offset) {
          ...
          DEBUGDPP(
              "op: {}, obj: {}, progress: {} done", pg, *this, obj, progress);
