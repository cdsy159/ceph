...
{
  return st_event.enter_stage<interruptor>(wait).then_interruptible([this] {
    return mutex.lock();
  });
}

void
SnapTrimEvent::print(std::ostream& lhs) const
{
  lhs << "SnapTrimEvent(" << "pgid=" << pg->get_pgid() << " snapid=" << snapid
      << " needs_pause=" << needs_pause << ")";
}

void
SnapTrimEvent::dump_detail(Formatter* f) const
{
...
SnapTrimEvent::start()
{
  ...
  ShardServices& shard_services = pg->get_shard_services();
  {
    ...
    auto to_trim_fut = interruptor::async([this] {
      ...
      auto to_trim = snap_mapper.get_next_objects_to_trim(snapid, max);
    ...
    for (const auto& object : to_trim) {
      ...
      subop_blocker.emplace_back(
          shard_services
              .start_operation_may_interrupt<interruptor, SnapTrimObjSubEvent>(
                  pg, object, snapid));
    }
  ...
  if (needs_pause) {
    ...
    co_await interruptor::make_interruptible(seastar::sleep(
    ...
    ceph::os::Transaction& txn)
{
  const auto p = std::find(
      ...
      head_obc->ssc->snapset.clones.end(), coid.snap);
  ...
  add_log_entry(
      pg_log_entry_t::DELETE, coid, obc->obs.oi.version, 0, osd_reqid_t(),
...
void
SnapTrimObjSubEvent::remove_head_whiteout(
    ObjectContextRef obc,
    ObjectContextRef head_obc,
    ceph::os::Transaction& txn)
{
  ...
  add_log_entry(
      pg_log_entry_t::DELETE, head_oid, head_obc->obs.oi.version, 0,
      osd_reqid_t(),
      obc->obs.oi.mtime, // will be replaced in `apply_to()`
      0);
    ...
    ceph::os::Transaction& txn)
{
  ...
  ceph::bufferlist bl;
  obc->obs.oi.encode(
      bl, pg->get_osdmap()->get_features(CEPH_ENTITY_TYPE_OSD, nullptr));
  txn.setattr(
      pg->get_collection_ref()->get_cid(),
      ghobject_t{coid, ghobject_t::NO_GEN, shard_id_t::NO_SHARD}, OI_ATTR, bl);
  auto& loge = add_log_entry(
      pg_log_entry_t::MODIFY, coid, obc->obs.oi.prior_version, 0, osd_reqid_t(),
      obc->obs.oi.mtime, 0);
  bufferlist snapsbl;
...
void
SnapTrimObjSubEvent::update_head(
    ObjectContextRef obc,
    ObjectContextRef head_obc,
    ceph::os::Transaction& txn)
{
  const auto head_oid = coid.get_head();
  logger().info(
      "{}: writing updated snapset on {}, snapset is {}", *this, head_oid,
      head_obc->ssc->snapset);
  add_log_entry(
      pg_log_entry_t::MODIFY, head_oid, head_obc->obs.oi.version, 0,
      osd_reqid_t(), obc->obs.oi.mtime, 0);
  ...
  bl.clear();
  head_obc->obs.oi.encode(
      bl, pg->get_osdmap()->get_features(CEPH_ENTITY_TYPE_OSD, nullptr));
  attrs[OI_ATTR] = std::move(bl);
  txn.setattrs(
      pg->get_collection_ref()->get_cid(),
      ghobject_t{head_oid, ghobject_t::NO_GEN, shard_id_t::NO_SHARD}, attrs);
}
    ...
    ObjectContextRef head_obc)
{
  ...
  if (citer == head_obc->ssc->snapset.clone_snaps.end()) {
    logger().error(
        "{}: No clone_snaps in snapset {} for object {}", *this,
        head_obc->ssc->snapset, coid);
    return crimson::ct_error::enoent::make();
  }
  ...
  for (const auto& old_snap : old_snaps) {
    if (!osdmap->in_removed_snaps_queue(
            pg->get_info().pgid.pgid.pool(), old_snap) &&
  ...
  return seastar::do_with(ceph::os::Transaction{}, [=, this](auto& txn) {
    ...
    if (new_snaps.empty()) {
      ...
      logger().info(
          "{}: {} snaps {} -> {} ... deleting", *this, coid, old_snaps,
          new_snaps);
    ...
    return std::move(ret)
        .si_then([&txn, obc, head_obc = std::move(head_obc), this]() mutable {
          // save head snapset
          logger().debug(
              "{}: {} new snapset {} on {}", *this, coid,
              head_obc->ssc->snapset, head_obc->obs.oi);
          osd_op_p.at_version.version++;
          if (head_obc->ssc->snapset.clones.empty() &&
        ...
        .si_then([&txn]() mutable { return std::move(txn); });
  });
}
...
{
  obc_orderer = pg->obc_loader.get_obc_orderer(coid);
  ...
  auto obc_manager = pg->obc_loader.get_obc_manager(
      *obc_orderer, coid, false /* resolve_oid */);

  co_await pg->obc_loader.load_and_lock(obc_manager, RWState::RWWRITE)
      .handle_error_interruptible(
          remove_or_update_iertr::pass_further{},
          crimson::ct_error::assert_all{
              fmt::format(
                  "{} error SnapTrimObjSubEvent::snap_trim_obj_subevent_ret_t "
                  "with {}",
                  *this, coid)
                  .c_str()});
  ...
  {
    auto unlocker = seastar::defer([this] { pg->submit_lock.unlock(); });
    ...
    logger().debug(
        "{}: calling remove_or_update obc={}", *this,
    ...
    std::tie(submitted, all_completed) = co_await pg->submit_transaction(
        ObjectContextRef(obc_manager.get_obc()), nullptr, std::move(txn),
        std::move(osd_op_p), std::move(log_entries));
...
void
SnapTrimObjSubEvent::print(std::ostream& lhs) const
{
  lhs << "SnapTrimObjSubEvent(" << "coid=" << coid << " snapid=" << snap_to_trim
      << ")";
}

void
SnapTrimObjSubEvent::dump_detail(Formatter* f) const
{
