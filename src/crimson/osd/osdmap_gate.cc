...
void
OSDMapGate<OSDMapGateTypeV>::OSDMapBlocker::dump_detail(Formatter* f) const
{
...
OSDMapGate<OSDMapGateTypeV>::wait_for_map(
    ...
    ShardServices* shard_services)
{
  ...
  if (current >= epoch) {
    ...
    auto& blocker = waiting_peering
                        .emplace(epoch, std::make_pair(blocker_type, epoch))
                        .first->second;
    auto fut = blocker.promise.get_shared_future();
    if (shard_services) {
      return trigger.maybe_record_blocking(
          shard_services->osdmap_subscribe(current, true)
...
template <OSDMapGateType OSDMapGateTypeV>
void
OSDMapGate<OSDMapGateTypeV>::got_map(epoch_t epoch)
{
  if (epoch == 0) {
    return;
  }
...
template <OSDMapGateType OSDMapGateTypeV>
seastar::future<>
OSDMapGate<OSDMapGateTypeV>::stop()
{
