...
std::ostream&
operator<<(std::ostream& out, const signedspan& d)
{
...
PG::PG(
  ...
  backend(PGBackend::create(
      ...
      *this,
      *this)),
  recovery_backend(RecoveryBackend::create(
      pool,
      *this,
      shard_services,
      coll_ref,
  ...
  peering_state(
      ...
      pg_shard,
      pgid,
      PGPool(osdmap, pgid.pool(), pool, name),
      osdmap,
  ...
  wait_for_active_blocker(this)
{
  ...
  peering_state.set_backend_predicates(
      new ReadablePredicate(pg_whoami), new RecoverablePredicate());
...
PG::check_blocklisted_watchers()
{
  ...
  obc_registry.for_each([this](ObjectContextRef obc) {
    ...
    for (const auto& [key, watch] : obc->watchers) {
      ...
      logger().debug(
          "watch: Found {} cookie {}. Checking entity_add_t {}",
...
PG::try_flush_or_schedule_async()
{
  ...
  (void)crimson::os::with_store_do_transaction(
      shard_services.get_store(store_index), coll_ref, ceph::os::Transaction{})
...
PG::publish_stats_to_osd()
{
  ...
  if (auto new_pg_stats = peering_state.prepare_stats_for_publish(
          pg_stats, object_stat_collection_t());
...
void
PG::apply_stats(const hobject_t& soid, const object_stat_sum_t& delta_stats)
{
...
void
PG::queue_check_readable(epoch_t last_peering_reset, ceph::timespan delay)
{
  // handle the peering event in the background
  logger().debug(
      "{}: PG::queue_check_readable lpr: {}, delay: {}", *this,
      last_peering_reset, delay);
  check_readable_timer.cancel();
  check_readable_timer.set_callback([last_peering_reset, this] {
    logger().debug(
        "{}: PG::queue_check_readable callback lpr: {}", *this,
        last_peering_reset);
    (void)shard_services.start_operation<LocalPeeringEvent>(
        this, pg_whoami, pgid, last_peering_reset, last_peering_reset,
...
PG::find_unfound(epoch_t epoch_started)
{
  if (!have_unfound()) {
  ...
  if (!peering_state.discover_all_missing(rctx)) {
    if (peering_state.state_test(PG_STATE_BACKFILLING)) {
      logger().debug(
          "{} {} no luck, giving up on this pg for now (in backfill)", *this,
          __func__);
      std::ignore = get_shard_services().start_operation<LocalPeeringEvent>(
          this, get_pg_whoami(), get_pgid(), epoch_started, epoch_started,
          PeeringState::UnfoundBackfill());
    } else if (peering_state.state_test(PG_STATE_RECOVERING)) {
      logger().debug(
          "{} {} no luck, giving up on this pg for now (in recovery)", *this,
          __func__);
      std::ignore = get_shard_services().start_operation<LocalPeeringEvent>(
          this, get_pg_whoami(), get_pgid(), epoch_started, epoch_started,
  ...
  return get_shard_services().dispatch_context(
      store_index, get_collection_ref(), std::move(rctx));
}
...
PG::recheck_readable()
{
  ...
  if (peering_state.state_test(PG_STATE_WAIT)) {
    ...
    if (mnow < prior_readable_until_ub) {
      logger().info(
          "{}: {} will wait (mnow {} < prior_readable_until_ub {})", *this,
      ...
      logger().info(
          "{}:{} no longer wait (mnow {} >= prior_readable_until_ub {})", *this,
          __func__, mnow, prior_readable_until_ub);
  ...
  if (peering_state.state_test(PG_STATE_LAGGY)) {
    ...
    if (readable_until == readable_until.zero()) {
      logger().info(
          "{}:{} still laggy (mnow {}, readable_until zero)", *this, __func__,
          mnow);
    } else if (mnow >= readable_until) {
      logger().info(
          "{}:{} still laggy (mnow {} >= readable_until {})", *this, __func__,
          mnow, readable_until);
    } else {
      logger().info(
          "{}:{} no longer laggy (mnow {} < readable_until {})", *this,
...
bool
PG::should_send_op(pg_shard_t peer, const hobject_t& hoid)
{
  if (peer == get_primary()) {
    return true;
  }
  ...
  if (!should_send) {
    ...
    logger().debug(
        "{}: {} shipping empty opt to osd.{}, object {}"
...
unsigned
PG::get_target_pg_log_entries() const
{
  ...
  if (local_num_pgs > 0 && local_target > 0) {
    ...
    return std::clamp(
        local_target / local_num_pgs, min_pg_log_entries, max_pg_log_entries);
  } else {
...
void
PG::on_removal(ceph::os::Transaction& t)
{
...
void
PG::clear_log_entry_maps()
{
  log_entry_update_waiting_on.clear();
}

void
PG::on_activate(interval_set<snapid_t> snaps)
{
...
PG::on_activate_complete()
{
  ...
  if (peering_state.needs_recovery()) {
    ...
    (void)shard_services.start_operation<LocalPeeringEvent>(
        this, pg_whoami, pgid, float(0.001), get_osdmap_epoch(),
        get_osdmap_epoch(), PeeringState::RequestBackfill{});
  } else {
    logger().debug(
        "{}: no need to recover or backfill, AllReplicasRecovered",
    ...
    ceph::os::Transaction& t)
{
  ...
  if (dirty_big_info || dirty_info) {
    int ret = prepare_info_keymap(
        shard_services.get_cct(), &km, &key_to_remove, get_osdmap_epoch(), info,
        last_written_info, past_intervals, dirty_big_info, need_write_epoch,
        true, nullptr, this);
    ceph_assert(ret == 0);
  }
...
PG::do_delete_work(ceph::os::Transaction& t, ghobject_t _next)
{
  ...
  auto fut = interruptor::make_interruptible(
      crimson::os::with_store<&crimson::os::FuturizedStore::Shard::list_objects>(
          shard_services.get_store(store_index), coll_ref, _next,
          ghobject_t::get_max(), local_conf()->osd_target_transaction_size, 0));
  ...
  if (objs_to_rm.empty()) {
    ...
    (void)crimson::os::with_store_do_transaction(
        ...
        .then([this] { return shard_services.remove_pg(pgid); });
    return {next, false};
  } else {
    DEBUGDPP(
        "pg {}, removing {} objects in this batch", *this, pgid,
        objs_to_rm.size());
    for (auto& obj : objs_to_rm) {
      ...
      if (obj.is_pgmeta()) {
        WARNDPP(
            "pg {} found stray pgmeta-like {} during PG removal", *this, pgid,
            obj);
      }
      TRACEDPP("pg {}, removing obj {}", *this, pgid, obj);
      t.remove(coll_ref->get_cid(), obj);
    }
    t.register_on_commit(new LambdaContext([this, FNAME](int r) {
      ceph_assert(r == 0);
...
PG::clear_temp_objects()
{
  ...
  while (true) {
    auto [objs, next] = co_await crimson::os::with_store<
        &crimson::os::FuturizedStore::Shard::list_objects>(
        shard_services.get_store(store_index), coll_ref, _next,
        ghobject_t::get_max(), max_size, 0);
    if (objs.empty()) {
      if (!t.empty()) {
        co_await crimson::os::with_store_do_transaction(
            shard_services.get_store(store_index), coll_ref, std::move(t));
      }
      break;
    }
    for (auto& obj : objs) {
...
PG::interruptible_future<seastar::stop_iteration>
PG::trim_snap(snapid_t to_trim, bool needs_pause)
{
  return interruptor::repeat([this, to_trim, needs_pause] {
           logger().debug(
               "{}: going to start SnapTrimEvent, to_trim={}", *this, to_trim);
           return shard_services
               .start_operation_may_interrupt<interruptor, SnapTrimEvent>(
                   this, snap_mapper, to_trim, needs_pause)
            ...
            [this] {
              return interruptor::repeat(
                  ...
                  .then_interruptible([this] {
                    logger().debug(
                        "{}: PG::on_active_actmap() finished trimming", *this);
...
void
PG::on_active_advmap(const OSDMapRef& osdmap)
{
  ...
  if (auto it = new_removed_snaps.find(get_pgid().pool());
    ...
    for (auto j : it->second) {
      if (snap_trimq.intersects(j.first, j.second)) {
        ...
        logger().error(
            "{}: {} removed_snaps already contains {}", *this, __func__,
            overlap);
        bad = true;
    ...
    logger().info(
        "{}: {} new removed snaps {}, snap_trimq now{}", *this, __func__,
        it->second, snap_trimq);
    assert(!bad || !local_conf().get_val<bool>("osd_debug_verify_cached_snaps"));
  }
}

PG::interruptible_future<bool>
PG::do_recover_missing(const hobject_t& soid, const osd_reqid_t& reqid)
{
  LOG_PREFIX(PG::do_recover_missing);
  DEBUGDPP("reqid {} check for recovery, {}", *this, reqid, soid);
  assert(is_primary());
  eversion_t ver;
  ...
  if (!needs_recovery_or_backfill) {
    DEBUGDPP("reqid {} nothing to recover {}", *this, reqid, soid);
    co_return false;
  }
  ...
  DEBUGDPP(
      "reqid {} need to wait for recovery, {} version {}", *this, reqid, soid);
  if (recovery_backend->is_recovering(soid)) {
    DEBUGDPP(
        "reqid {} object {} version {}, already recovering", *this, reqid, soid,
        ver);
    co_await PG::interruptor::make_interruptible(
        recovery_backend->get_recovering(soid).wait_for_recovered());
    co_return false;
  } else {
    DEBUGDPP(
        "reqid {} object {} version {}, starting recovery", *this, reqid, soid,
        ver);
    auto [op, fut] = shard_services.start_operation<UrgentRecovery>(
...
void
PG::scrub_requested(scrub_level_t scrub_level, scrub_type_t scrub_type)
{
...
void
PG::log_state_enter(const char* state)
{
...
{
  logger().info(
      "Exiting state: {}, entered at {}, {} spent on {} events", state_name,
      enter_time, event_dur, events);
}
...
void
PG::schedule_renew_lease(epoch_t last_peering_reset, ceph::timespan delay)
{
  // handle the peering event in the background
...
{
  peering_state.init(
      role, newup, new_up_primary, newacting, new_acting_primary, history, pi,
      t);
  assert(coll_ref);
  return crimson::os::with_store<&crimson::os::FuturizedStore::Shard::exists>(
             shard_services.get_store(store_index), get_collection_ref(),
             pgid.make_snapmapper_oid(), 0)
      .safe_then(
          [&t, this](bool existed) {
            if (!existed) {
          ...
          ::crimson::ct_error::assert_all{
              fmt::format("{} {} unexpected eio", *this, __func__).c_str()});
}

seastar::future<>
PG::read_state(crimson::os::BackendStore store)
{
  ...
  return seastar::do_with(
      ...
      .then([this, store]() {
        ...
        return crimson::os::with_store<
            &crimson::os::FuturizedStore::Shard::set_collection_opts>(
            store, coll_ref, get_pgpool().info.opts);
      });
}

void
PG::do_peering_event(PGPeeringEvent& evt, PeeringCtx& rctx)
{
...
void
PG::handle_advance_map(cached_map_t next_map, PeeringCtx& rctx)
{
  vector<int> newup, newacting;
  int up_primary, acting_primary;
  next_map->pg_to_up_acting_osds(
      pgid.pgid, &newup, &up_primary, &newacting, &acting_primary);
  peering_state.advance_map(
      next_map, peering_state.get_osdmap(), newup, up_primary, newacting,
      acting_primary, rctx);
  osdmap_gate.got_map(next_map->get_epoch());
}

void
PG::handle_activate_map(PeeringCtx& rctx)
{
  peering_state.activate_map(rctx);
}

void
PG::handle_initialize(PeeringCtx& rctx)
{
...
{
  std::ignore = crimson::os::with_store<
      &crimson::os::FuturizedStore::Shard::set_collection_opts>(
...
void
PG::print(ostream& out) const
{
  out << peering_state << " ";
}

void
PG::dump_primary(Formatter* f)
{
...
std::ostream&
operator<<(std::ostream& os, const PG& pg)
{
...
void
PG::enqueue_delete_for_backfill(
    const hobject_t& obj,
    const eversion_t& v,
    const std::vector<pg_shard_t>& peers)
{
    ...
    std::vector<pg_log_entry_t>&& log_entries)
{
  ...
  auto [submitted, all_completed] = co_await backend->submit_transaction(
      peering_state.get_acting_recovery_backfill(), std::move(obc),
      std::move(new_clone), std::move(txn), std::move(osd_op_p),
      peering_state.get_last_peering_reset(), map_epoch,
      std::move(log_entries));
  co_return std::make_tuple(std::move(submitted), std::move(all_completed));
}

PG::interruptible_future<>
PG::repair_object(const hobject_t& oid, eversion_t& v)
{
  // see also PrimaryLogPG::rep_repair_primary_object()
  assert(is_primary());
  logger().debug(
      "{}: {} peers osd.{}", __func__, oid, get_acting_recovery_backfill());
...
PG::interruptible_future<>
PG::complete_error_log(const ceph_tid_t& rep_tid, const eversion_t& version)
{
  ...
  if (log_update.waiting_on.empty()) {
    ...
    logger().debug(
        "complete_error_log: write complete,"
        " erasing rep_tid {}",
        rep_tid);
  } else {
    logger().debug(
        "complete_error_log: rep_tid {} awaiting update from {}", rep_tid,
        log_update.waiting_on);
    result = interruptor::make_interruptible(
                 ...
                 .then_interruptible([this, last_complete, rep_tid, version] {
                   logger().debug(
                       "complete_error_log: rep_tid {} awaited ", rep_tid);
                   peering_state.complete_write(version, last_complete);
                   ceph_assert(!log_entry_update_waiting_on.contains(rep_tid));
                   return seastar::now();
                 });
...
PG::interruptible_future<eversion_t>
PG::submit_error_log(
    Ref<MOSDOp> m,
    ...
    ceph_tid_t rep_tid)
{
  auto unlocker = seastar::defer([this] { submit_lock.unlock(); });
  ...
  DEBUGDPP("{} rep_tid: {} error: {}", *this, *m, rep_tid, e);
  const osd_reqid_t& reqid = m->get_reqid();
  mempool::osd_pglog::list<pg_log_entry_t> log_entries;
  log_entries.push_back(pg_log_entry_t(
      pg_log_entry_t::ERROR, obc->obs.oi.soid, get_next_version(), eversion_t(),
      0, reqid, utime_t(), -e.value()));
  ...
  for (const auto& peer : get_acting_recovery_backfill()) {
    ...
    auto log_m = crimson::make_message<MOSDPGUpdateLogMissing>(
        log_entries, spg_t(peering_state.get_info().pgid.pgid, peer.shard),
        pg_whoami.shard, get_osdmap_epoch(), get_last_peering_reset(), rep_tid,
        peering_state.get_pg_trim_to(), peering_state.get_pg_committed_to());

    DEBUGDPP(
        "sending log missing_request (rep_tid: {} entries: {}) to osd {}",
        *this, rep_tid, log_entries, peer.osd);
    co_await interruptor::make_interruptible(shard_services.send_to_osd(
        peer.osd, std::move(log_m), get_osdmap_epoch()));
  }
    ...
    std::vector<OSDOp>& ops)
{
  ...
  auto rollbacker = ox.create_rollbacker([FNAME,
                                          obc_data = duplicate_obc_data(obc),
                                          &ox, this](auto& obc) mutable {
    ...
    if (ox.has_cloning_ctx()) {
      ...
      DEBUGDPP(
          "cleaning up clone OBC for {} reqid={}", FNAME, coid,
          ox.get_message().get_reqid());
  ...
  if ((ox.delta_stats.num_bytes > 0 || ox.delta_stats.num_objects > 0) &&
    ...
    if (m.get_reqid().name.is_mds() || // FIXME: ignore MDS for now
      ...
      if (get_pgpool().info.has_flag(pg_pool_t::FLAG_FULL_QUOTA)) {
        co_await run_executer_fut(crimson::ct_error::edquot::make());
      } else {
        co_await run_executer_fut(crimson::ct_error::enospc::make());
      }
    } else {
      // drop request
      INFODPP("full, dropping request (bad client)", *this);
      co_await run_executer_fut(crimson::ct_error::eagain::make());
    }
  }
  rollback_on_error.cancel();
}

PG::submit_executer_fut
PG::submit_executer(OpsExecuter&& ox, const std::vector<OSDOp>& ops)
{
  ...
  auto [submitted, completed] = co_await std::move(ox).flush_changes_and_submit(
      ops, snap_mapper, osdriver);
  co_return std::make_tuple(
...
PG::interruptible_future<MURef<MOSDOpReply>>
PG::do_pg_ops(Ref<MOSDOp> m)
{
  if (__builtin_expect(stopping, false)) {
  ...
  auto ox =
      std::make_unique<PgOpsExecuter>(std::as_const(*this), std::as_const(*m));
  return interruptor::do_for_each(
             m->ops,
             [ox = ox.get()](OSDOp& osd_op) {
               logger().debug(
                   "will be handling pg op {}", ceph_osd_op_name(osd_op.op.op));
               return ox->execute_op(osd_op);
             })
      .then_interruptible([m, this, ox = std::move(ox)] {
        auto reply = crimson::make_message<MOSDOpReply>(
            m.get(), 0, get_osdmap_epoch(),
            CEPH_OSD_FLAG_ACK | CEPH_OSD_FLAG_ONDISK, false);
        reply->claim_op_out_data(m->ops);
        reply->set_reply_versions(
            peering_state.get_info().last_update,
            peering_state.get_info().last_user_version);
        return seastar::make_ready_future<MURef<MOSDOpReply>>(std::move(reply));
      })
      .handle_exception_type_interruptible([=, this](
                                               const crimson::osd::error& e) {
        auto reply = crimson::make_message<MOSDOpReply>(
...
hobject_t
PG::get_oid(const hobject_t& hobj)
{
  return hobj.snap == CEPH_SNAPDIR ? hobj.get_head() : hobj;
}

RWState::State
PG::get_lock_type(const OpInfo& op_info)
{
...
void
PG::check_blocklisted_obc_watchers(ObjectContextRef& obc)
{
  if (obc->watchers.empty()) {
    for (auto& [src, winfo] : obc->obs.oi.watchers) {
      ...
      assert(emplaced);
      logger().debug(
          "added watch for obj {}, client {}", obc->get_oid(), src.second);
    }
  }
}

void
PG::update_stats(const pg_stat_t& stat)
{
  peering_state.update_stats([&stat](auto& history, auto& stats) {
    stats = stat;
    return false;
  });
}

PG::handle_rep_op_fut
PG::handle_rep_op(Ref<MOSDRepOp> req)
{
  ...
  txn.decode(
      req->get_middle().length() != 0 ? encoded_txn_p : encoded_txn_d,
      encoded_txn_d);
  ...
  co_await update_snap_map(log_entries, txn);

  log_operation(
      std::move(log_entries), std::nullopt, req->pg_trim_to, req->version,
      req->pg_committed_to, !txn.empty(), txn, false);
  DEBUGDPP("{} do_transaction", *this, *req);

  auto commit_fut =
      interruptor::make_interruptible(crimson::os::with_store_do_transaction(
          shard_services.get_store(store_index), coll_ref, std::move(txn)));
  ...
  auto reply = crimson::make_message<MOSDRepOpReply>(
      req.get(), pg_whoami, 0, map_epoch, req->get_min_epoch(),
    ...
    ObjectStore::Transaction& t)
{
  ...
  DEBUGDPP("", *this);
  return interruptor::do_for_each(
      log_entries, [this, &t](const auto& entry) mutable {
    ...
    bool async)
{
  ...
  if (is_primary() && last != logv.rend()) {
    ...
    projected_log.trim(
        shard_services.get_cct(), last->version, nullptr, nullptr, nullptr);
  }
  ...
  peering_state.append_log(
      std::move(logv), trim_to, roll_forward_to, pg_committed_to, txn,
      !txn.empty(), false);
}

void
PG::clear_repop_obc(const std::vector<pg_log_entry_t>& logv)
{
  LOG_PREFIX(PG::clear_repop_obc);
  DEBUGDPP("clearing obc for {} log entries", *this, logv.size());
  for (auto&& e : logv) {
    DEBUGDPP(
        "clearing entry for {} from: {} to: {}", *this, e.soid,
...
void
PG::handle_rep_op_reply(const MOSDRepOpReply& m)
{
...
PG::interruptible_future<>
PG::handle_rep_write_op(Ref<MOSDECSubOpWrite> m)
{
  logger().debug("{}", __func__);
  if (!is_primary()) {
  ...
  return ec_backend->handle_rep_write_op(std::move(m), *this)
      .si_then([this, then_lcod = peering_state.get_info().last_complete, tid] {
        ...
        return shard_services.send_to_osd(
            get_primary().osd, std::move(r), get_osdmap_epoch());
      })
      .handle_error_interruptible(crimson::ct_error::assert_all{});
}

PG::interruptible_future<>
PG::handle_rep_write_reply(Ref<MOSDECSubOpWriteReply> m)
{
  ...
  auto* ec_backend = dynamic_cast<crimson::osd::ECBackend*>(&get_backend());
  assert(ec_backend);
  return ec_backend->handle_rep_write_reply(std::move(m->op))
      .handle_error_interruptible(crimson::ct_error::assert_all{});
}

PG::interruptible_future<>
PG::handle_rep_read_op(Ref<MOSDECSubOpRead> m)
{
  auto* ec_backend = dynamic_cast<crimson::osd::ECBackend*>(&get_backend());
  assert(ec_backend);
  return ec_backend->handle_rep_read_op(std::move(m))
    ...
    crimson::net::ConnectionXcoreRef conn)
{
  ...
  return interruptor::make_interruptible(
             crimson::os::with_store_do_transaction(
                 shard_services.get_store(store_index), coll_ref, std::move(t)))
      .then_interruptible([m, conn,
                           lcod = peering_state.get_info().last_complete, this] {
        if (!peering_state.pg_has_reset_since(m->get_epoch())) {
          peering_state.update_last_complete_ondisk(lcod);
          auto reply = crimson::make_message<MOSDPGUpdateLogMissingReply>(
              spg_t(peering_state.get_info().pgid.pgid, get_primary().shard),
              pg_whoami.shard, m->get_epoch(), m->min_epoch, m->get_tid(),
              lcod);
...
PG::interruptible_future<>
PG::do_update_log_missing_reply(Ref<MOSDPGUpdateLogMissingReply> m)
{
  ...
  if (it != log_entry_update_waiting_on.end()) {
    if (it->second.waiting_on.count(m->get_from())) {
      ...
      logger().error(
          "{} : {} got reply {} from shard we are not waiting for ", __func__,
          peering_state.get_info().pgid, *m, m->get_from());
    }

    if (it->second.waiting_on.empty()) {
      it->second.all_committed.set_value();
      it->second.all_committed = {};
      logger().debug("{}: erasing rep_tid {}", __func__, m->get_tid());
      log_entry_update_waiting_on.erase(it);
    }
  } else {
    ceph_abort_msg(fmt::format(
        "{} : {} got reply {} on unknown tid {}", __func__,
        peering_state.get_info().pgid, *m, m->get_tid()));
  }
  return seastar::now();
}

bool
PG::old_peering_msg(const epoch_t reply_epoch, const epoch_t query_epoch) const
{
  if (const epoch_t lpr = peering_state.get_last_peering_reset();
      lpr > reply_epoch || lpr > query_epoch) {
    logger().debug(
        "{}: pg changed {} lpr {}, reply_epoch {}, query_epoch {}", __func__,
        get_info().history, lpr, reply_epoch, query_epoch);
    return true;
  }
  return false;
}
...
bool
PG::can_discard_replica_op(const Message& m, epoch_t m_map_epoch) const
{
...
void
PG::on_change(ceph::os::Transaction& t)
{
  logger().debug("{} {}:", *this, __func__);
...
bool
PG::is_missing_head_and_clones(const hobject_t& hoid)
{
  return peering_state.is_missing_any_head_or_clone_of(hoid);
}

bool
PG::can_discard_op(const MOSDOp& m) const
{
  if (m.get_map_epoch() < peering_state.get_info().history.same_primary_since) {
    logger().debug(
        "{} changed after {} dropping {} ", __func__, m.get_map_epoch(), m);
    return true;
  }

  if ((m.get_flags() & CEPH_OSD_FLAGS_DIRECT_READ) && !is_primary() &&
      (m.get_map_epoch() <
  ...
  if (m.get_map_epoch() < peering_state.get_info().history.last_epoch_split) {
    logger().debug(
        "{} pg split in {} dropping!", __func__,
        peering_state.get_info().history.last_epoch_split);
    return true;
  }
  return __builtin_expect(
      m.get_map_epoch() < peering_state.get_info().history.same_primary_since,
      false);
}

bool
PG::is_degraded_or_backfilling_object(const hobject_t& soid) const
{
...
bool
PG::should_send_op(pg_shard_t peer, const hobject_t& hoid) const
{
  if (peer == get_primary())
    return true;
  bool should_send =
  ...
  if (!should_send) {
    ceph_assert(is_backfill_target(peer));
    logger().debug(
        "{} issue_repop shipping empty opt to osd."
        "{}, object {} beyond std::max(last_backfill_started, "
        "peer_info[peer].last_backfill {})",
        __func__, peer, hoid, peering_state.get_peer_info(peer).last_backfill);
  }
  return should_send;
...
void
PG::op_applied(const eversion_t& applied_version)
{
...
PG::already_complete(const osd_reqid_t& reqid)
{
  ...
  if (check_in_progress_op(reqid, &version, &user_version, &ret, &op_returns)) {
    ...
    return seastar::make_ready_future<std::optional<complete_op_t>>(
        std::nullopt);
  }
}

void
PG::remove_maybe_snapmapped_object(
    ceph::os::Transaction& t,
...
void
PG::PGLogEntryHandler::remove(const hobject_t& soid)
{
...
void
PG::set_pglog_based_recovery_op(PglogBasedRecovery* op)
{
...
void
PG::cancel_pglog_based_recovery_op()
{
...
void
PG::C_PG_FinishRecovery::finish(int r)
{
...
void
PG::send_message_osd_cluster(int osd, MOSDPGPush* msg, epoch_t from_epoch)
{
  logger().debug(
      "{}: MOSDPGPush from_epoch {} to osd.{}", __func__, from_epoch, osd);
  if (whoami_shard().osd == osd) {
    std::ignore =
        static_cast<ECRecoveryBackend&>(*recovery_backend).handle_push(msg);
  } else {
    std::vector wrapped_msg{std::make_pair(osd, static_cast<Message*>(msg))};
    send_message_osd_cluster(wrapped_msg, from_epoch);
  }
}

void
PG::PGLogEntryHandler::partial_write(
    pg_info_t* info,
    ...
    const pg_log_entry_t& entry)
{
  ...
  logger().debug(
      "{}: version version={} written_shards={}"
      " pwlc={} previous_version={}",
      __func__, entry.version, entry.written_shards,
      info->partial_writes_last_complete, previous_version);
  ...
  logger().debug(
      "{}: after pwlc={}", __func__, info->partial_writes_last_complete);
}
