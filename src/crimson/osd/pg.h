...
public:
  ...
  void
  cancel_pull(const hobject_t& soid) override
  {
    // TODO
  }
      ...
      epoch_t from_epoch) override final
  {
    std::ignore = seastar::do_with(
        std::move(messages), [this, from_epoch](auto&& messages) {
          return seastar::do_for_each(messages, [this, from_epoch](auto&& im) {
            auto& [osd_id, msg] = im;
            return shard_services.send_to_osd(
                osd_id, MessageURef{msg}, from_epoch);
          });
        });
  }

  void send_message_osd_cluster(int osd, MOSDPGPush* msg, epoch_t from_epoch)
      override;

  std::ostream&
  gen_dbg_prefix(std::ostream& out) const override final
  {
  ...
  on_global_recover(
      ...
      bool is_delete) final
  {
    get_recovery_handler()->on_global_recover(oid, stat_diff, is_delete);
  }
  ...
  epoch_t
  get_last_peering_reset_epoch() const final
  {
    return get_last_peering_reset();
  }

  pg_shard_t
  primary_shard() const final
  {
    return get_primary();
  }

  bool
  pgb_is_primary() const final
  {
    return is_primary();
  }

  hobject_t
  get_temp_recovery_object(const hobject_t& target, eversion_t version) final
  {
    return get_recovery_handler()->get_temp_recovery_object(target, version);
  }

  void
  inc_osd_stat_repaired() final
  {
  ...
  uint64_t
  get_snap_trimq_size() const final
  {
  ...
  send_cluster_message(
      ...
      MessageURef m,
      epoch_t epoch,
      bool share_map_update = false) final
  {
    LOG_PREFIX(PG::send_cluster_message);
    SUBDEBUGDPP(
        osd, "message {} to {} share_map_update {}", *this, *m, osd,
        share_map_update);
  ...
  void
  send_pg_created(pg_t pgid) final
  {
  ...
  void
  start_flush_on_transaction(ceph::os::Transaction& t) final
  {
  ...
  template <typename T>
  void
  start_peering_event_operation(T&& evt, float delay = 0)
  {
    ...
    (void)shard_services.start_operation<LocalPeeringEvent>(
        this, pg_whoami, pgid, delay, std::forward<T>(evt));
  }

  void
  schedule_event_after(PGPeeringEventRef event, float delay) final
  {
      ...
      PGPeeringEventURef on_preempt) final
  {
    ...
    SUBDEBUGDPP(
        osd, "priority {} on_grant {} on_preempt {}", *this, priority,
        on_grant->get_desc(), on_preempt->get_desc());
    shard_services.local_request_reservation(
        orderer, pgid,
        on_grant
            ? make_lambda_context([this, on_grant = std::move(on_grant)](int) {
                start_peering_event_operation(std::move(*on_grant));
              })
            : nullptr,
        priority,
        on_preempt
            ? make_lambda_context([this,
                                   on_preempt = std::move(on_preempt)](int) {
  ...
  void
  update_local_background_io_priority(unsigned priority) final
  {
    LOG_PREFIX(PG::update_local_background_io_priority);
    SUBDEBUGDPP(osd, "priority {}", *this, priority);
    shard_services.local_update_priority(orderer, pgid, priority);
  }

  void
  cancel_local_background_io_reservation() final
  {
    LOG_PREFIX(PG::cancel_local_background_io_reservation);
    SUBDEBUGDPP(osd, "", *this);
    shard_services.local_cancel_reservation(orderer, pgid);
  }
  ...
  void
  cancel_remote_recovery_reservation() final
  {
      ...
      PGPeeringEventRef on_commit) final
  {
    ...
    t.register_on_commit(
        make_lambda_context([this, on_commit = std::move(on_commit)](int) {
          start_peering_event_operation(std::move(*on_commit));
        }));
  }

  void
  update_heartbeat_peers(std::set<int> peers) final
  {
    // Not needed yet
  }

  void
  set_probe_targets(const std::set<pg_shard_t>& probe_set) final
  {
    // Not needed yet
  }

  void
  clear_probe_targets() final
  {
    // Not needed yet
  }

  void
  queue_want_pg_temp(const std::vector<int>& wanted) final
  {
  ...
  void
  check_recovery_sources(const OSDMapRef& newmap) final
  {
    LOG_PREFIX(PG::check_recovery_sources);
    recovery_backend->for_each_recovery_waiter([newmap, FNAME,
                                                this](auto&, auto& waiter) {
      if (waiter->is_pulling() && newmap->is_down(waiter->pull_info->from.osd)) {
        SUBDEBUGDPP(
            osd, " repeating pulling for {}, due to osd {} down", *this,
            waiter->pull_info->soid, waiter->pull_info->from.osd);
        waiter->repeat_pull();
      }
    });
  }
  ...
  void
  clear_ready_to_merge() final
  {}

  void
  set_not_ready_to_merge_target(pg_t pgid, pg_t src) final
  {}

  void
  set_not_ready_to_merge_source(pg_t pgid) final
  {}

  void
  set_ready_to_merge_target(eversion_t lu, epoch_t les, epoch_t lec) final
  {}

  void
  set_ready_to_merge_source(eversion_t lu) final
  {}
  ...
  epoch_t
  cluster_osdmap_trim_lower_bound() final
  {
    return shard_services.get_osdmap_tlb();
  }

  void
  on_backfill_reserved() final
  {
    recovery_handler->on_backfill_reserved();
  }

  void
  on_backfill_suspended() final
  {
    recovery_handler->backfill_suspended();
  }

  void
  on_recovery_cancelled() final
  {
    cancel_pglog_based_recovery_op();
  }

  void
  on_recovery_reserved() final
  {
  ...
  void
  unreserve_recovery_space() final
  {}
  ...
  struct PGLogEntryHandler : public PGLog::LogEntryHandler {
    ...
    void
    try_stash(const hobject_t& hoid, version_t v) override
    {
      // TODO
    }

    void
    rollback(const pg_log_entry_t& entry) override
    {
      // TODO
    }

    void
    rollforward(const pg_log_entry_t& entry) override
    {
      // TODO
    }

    void
    trim(const pg_log_entry_t& entry) override
    {
      // TODO
    }
  ...
  get_log_handler(ceph::os::Transaction& t) final
  {
    return std::make_unique<PG::PGLogEntryHandler>(this, &t);
  }

  void
  rebuild_missing_set_with_deletes(PGLog& pglog) final
  {
  ...
  void log_state_exit(
      ...
      utime_t enter_time,
      uint64_t events,
      utime_t event_dur) final;

  void
  dump_recovery_info(Formatter* f) const final
  {}
  ...
  bool
  is_peered() const final
  {
    return peering_state.is_peered();
  }

  bool
  is_recovering() const final
  {
    return peering_state.is_recovering();
  }

  bool
  is_backfilling() const final
  {
  ...
  eversion_t
  get_last_complete() const
  {
    return peering_state.get_info().last_complete;
  }

  void
  complete_write(eversion_t v, eversion_t lc)
  {
    peering_state.complete_write(v, lc);
  }

  void
  update_peer_last_complete_ondisk(pg_shard_t fromosd, eversion_t lcod)
  {
  ...
  void
  update_snap_mapper_bits(uint32_t bits)
  {
  ...
  void
  finish_split_stats(const object_stat_sum_t& stats, ObjectStore::Transaction& t)
  {
      ...
      ObjectStore::Transaction& t)
  {
    ...
    t.split_collection(parent_coll, split_bits, seed, target);
  ...
  void
  split_into(pg_t child_pgid, Ref<PG> child, unsigned split_bits)
  {
  ...
  using load_obc_ertr = crimson::
      errorator<crimson::ct_error::enoent, crimson::ct_error::object_corrupted>;
  using load_obc_iertr = ::crimson::interruptible::interruptible_errorator<
...
public:
  ...
  void log_operation(
      ...
      bool async = false);
  void clear_repop_obc(const std::vector<pg_log_entry_t>& logv);
  ...
  interruptible_future<> complete_error_log(
      const ceph_tid_t& rep_tid,
...
private:
  ...
  using submit_executer_ret =
      std::tuple<interruptible_future<>, interruptible_future<>>;
  using submit_executer_fut = interruptible_future<submit_executer_ret>;
...
public:
  using rep_op_fut_t =
      std::tuple<interruptible_future<>, interruptible_future<>>;
  interruptible_future<rep_op_fut_t> submit_transaction(
      ObjectContextRef&& obc,
...
public:
  ...
  get_next_version()
  {
    return eversion_t(get_osdmap_epoch(), projected_last_update.version + 1);
  }
...
private:
  ...
  public:
    explicit C_PG_FinishRecovery(PG& pg) :
      pg(pg)
    {}
...
public:
  ...
  template <typename T>
  friend class RemoteScrubEventBaseT;
...
public:
  ...
  void
  set_dynamic_perf_stats_queries(const std::list<OSDPerfMetricQuery>& queries)
  {
    dp_stats.set_queries(queries);
  }

  void
  get_dynamic_perf_stats(DynamicPerfStats* stats)
  {
  ...
  const PeeringState&
  get_peering_state() const
  {
    return peering_state;
  }

  bool
  has_backfill_state() const
  {
    return (bool)(recovery_handler->backfill_state);
  }

  const BackfillState&
  get_backfill_state() const
  {
  ...
  bool
  has_reset_since(epoch_t epoch) const final
  {
  ...
  void
  begin_peer_recover(pg_shard_t peer, const hobject_t oid) final
  {
    peering_state.begin_peer_recover(peer, oid);
  }

  uint64_t
  min_peer_features() const
  {
  ...
  epoch_t
  get_interval_start_epoch() const
  {
    return get_info().history.same_interval_since;
  }

  const pg_missing_const_i*
  maybe_get_shard_missing(pg_shard_t shard) const
  {
    if (shard == pg_whoami) {
  ...
  interruptible_future<std::optional<complete_op_t>> already_complete(
      const osd_reqid_t& reqid);
  ...
  bool
  can_discard_replica_op(const MsgType& m) const
  {
...
private:
  ...
  bool
  is_missing_object(const hobject_t& soid) const final
  {
    return get_local_missing().is_missing(soid);
  }

  bool
  is_unreadable_object(const hobject_t& oid, eversion_t* v = 0) const final
  {
  ...
  bool
  is_missing_on_peer(const pg_shard_t& peer, const hobject_t& soid) const
  {
  ...
  void
  add_local_next_event(const pg_log_entry_t& e) override final
  {
...
template <>
struct fmt::formatter<crimson::osd::PG> : fmt::ostream_formatter {};
#endif
