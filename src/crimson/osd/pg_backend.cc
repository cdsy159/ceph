    ...
    ECListener& eclistener)
{
  ...
  case pg_pool_t::TYPE_REPLICATED:
    return std::make_unique<ReplicatedBackend>(
        pgid, whoami, pg, coll, shard_services, dpp);
  case pg_pool_t::TYPE_ERASURE:
    return std::make_unique<ECBackend>(
        whoami, coll, shard_services, pg.get_store_index(),
        std::move(ec_profile), pool.stripe_width, pool.fast_read,
        pool.allows_ecoverwrites(), dpp, eclistener);
  default:
    throw runtime_error(
        seastar::format("unsupported pool type '{}'", pool.type));
  }
}
    ...
    crimson::os::FuturizedStore::Shard::attrs_t attrs)
{
  ...
  if (auto oiiter = attrs.find(OI_ATTR); oiiter != attrs.end()) {
    ...
    logger().error(
        "load_metadata: object {} present but missing object info", oid);
    return tl::unexpected(ErrorHelper<load_metadata_ertr>::to_error(
  ...
  if (oid.is_head()) {
    ...
    if (auto ssiter = attrs.find(SS_ATTR); ssiter != attrs.end()) {
      ...
      if (bl.length()) {
        ...
        try {
          ...
          ret->ssc->exists = true;
          logger().debug(
              "load_metadata: object {} and snapset {} present", oid,
              ret->ssc->snapset);
        } catch (const buffer::error&) {
    ...
    if (object_corrupted) {
      logger().error(
          "load_metadata: object {} present but missing snapset", oid);
      return tl::unexpected(ErrorHelper<load_metadata_ertr>::to_error(
          crimson::ct_error::object_corrupted::make()));
    }
  }
...
PGBackend::load_metadata(const hobject_t& oid)
{
  return interruptor::make_interruptible(
             crimson::os::with_store<
                 &crimson::os::FuturizedStore::Shard::get_attrs>(
                 store, coll, ghobject_t{oid, ghobject_t::NO_GEN, get_shard()},
                 0))
      .safe_then_interruptible(
          [oid, this](auto&& attrs)
            ...
            if (auto maybe_decoded = decode_metadata2(oid, std::move(attrs));
              ...
              return load_metadata_ertr::make_ready_future<
                  loaded_object_md_t::ref>(std::move(*maybe_decoded));
            } else {
              return ErrorHelper<load_metadata_ertr>::from_error<
                  PGBackend::loaded_object_md_t::ref>(maybe_decoded.error());
            }
          },
          crimson::ct_error::enoent::handle([oid] {
            logger().debug(
                "load_metadata: object {} doesn't exist, returning empty "
                "metadata",
                oid);
            return load_metadata_ertr::make_ready_future<loaded_object_md_t::ref>(
                new loaded_object_md_t{
                    ...
                    oid.is_head() ? (new crimson::osd::SnapSetContext(oid))
                                  : nullptr});
          }));
}

static inline bool
_read_verify_data(const object_info_t& oi, const ceph::bufferlist& data)
{
  if (oi.is_data_digest() && oi.size == data.length()) {
    // whole object?  can we verify the checksum?
    if (auto crc = data.crc32c(-1); crc != oi.data_digest) {
      logger().error(
          "full-object read crc {} != expected {} on {}", crc, oi.data_digest,
          oi.soid);
    ...
    object_stat_sum_t& delta_stats)
{
  ...
  return _read(oi.soid, oi.size, offset, length, op.flags)
    ...
    object_stat_sum_t& delta_stats)
{
  ...
  logger().trace(
      "sparse_read: {} {}~{}", os.oi.soid, (uint64_t)op.extent.offset,
      (uint64_t)op.extent.length);

  return interruptor::make_interruptible(
             crimson::os::with_store<&crimson::os::FuturizedStore::Shard::fiemap>(
                 store, coll, ghobject_t{os.oi.soid},
                 static_cast<uint64_t>(offset),
                 static_cast<uint64_t>(adjusted_length),
                 static_cast<uint32_t>(0)))
      .safe_then_interruptible([&delta_stats, &os, &osd_op, this](auto&& m) {
        return seastar::do_with(
            interval_set<uint64_t>{std::move(m)},
            [&delta_stats, &os, &osd_op, this](auto&& extents) {
              return interruptor::make_interruptible(
                         crimson::os::with_store<
                             &crimson::os::FuturizedStore::Shard::readv>(
                             store, coll, ghobject_t{os.oi.soid},
                             std::ref(extents), osd_op.op.flags))
                  .safe_then_interruptible_tuple(
                      [&delta_stats, &os, &osd_op,
                       &extents](auto&& bl) -> read_errorator::future<> {
                        if (_read_verify_data(os.oi, bl)) {
                          ...
                          logger().trace(
                              "sparse_read got {} bytes from object {}",
                              (uint64_t)osd_op.op.extent.length, os.oi.soid);
                          delta_stats.num_rd++;
                          delta_stats.num_rd_kb +=
                              shift_round_up(osd_op.op.extent.length, 10);
...
PGBackend::checksum(const ObjectState& os, OSDOp& osd_op)
{
  ...
  return _read(
             os.oi.soid, os.oi.size, checksum.offset, checksum.length,
             osd_op.op.flags)
      .safe_then_interruptible(
          [&osd_op](auto&& read_bl) mutable -> checksum_errorator::future<> {
            ...
            if (read_bl.length() != checksum.length) {
              logger().warn(
                  "checksum: bytes read {} != {}", read_bl.length(),
                  (uint64_t)checksum.length);
              return crimson::ct_error::invarg::make();
            }
            // calculate its checksum and put the result in outdata
            switch (checksum.type) {
            case CEPH_OSD_CHECKSUM_OP_TYPE_XXHASH32:
              return do_checksum<Checksummer::xxhash32>(
                  osd_op.indata, checksum.chunk_size, read_bl, osd_op.outdata);
            case CEPH_OSD_CHECKSUM_OP_TYPE_XXHASH64:
              return do_checksum<Checksummer::xxhash64>(
                  osd_op.indata, checksum.chunk_size, read_bl, osd_op.outdata);
            case CEPH_OSD_CHECKSUM_OP_TYPE_CRC32C:
              return do_checksum<Checksummer::crc32c>(
                  osd_op.indata, checksum.chunk_size, read_bl, osd_op.outdata);
            default:
              logger().warn(
                  "checksum: unknown crc type ({})",
...
PGBackend::cmp_ext(const ObjectState& os, OSDOp& osd_op)
{
  ...
  return read_ext.safe_then_interruptible(
      [&osd_op](auto&& read_bl) -> cmp_ext_errorator::future<> {
    ...
    object_stat_sum_t& delta_stats)
{
  if (os.exists /* TODO: && !os.is_whiteout() */) {
    logger().debug(
        "stat os.oi.size={}, os.oi.mtime={}", os.oi.size, os.oi.mtime);
    ...
    unsigned flags)
{
  ...
  if (existing && bl.length() < os.oi.size) {
    ...
    osd_op_params.clean_regions.mark_data_region_dirty(
        bl.length(), os.oi.size - bl.length());
  }
  if (bl.length()) {
    txn.write(
        coll->get_cid(), ghobject_t{os.oi.soid}, 0, bl.length(), bl, flags);
    update_size_and_usage(
        delta_stats, osd_op_params.modified_ranges, os.oi, 0, bl.length(), true);
    osd_op_params.clean_regions.mark_data_region_dirty(
        0, std::max((uint64_t)bl.length(), os.oi.size));
  }
  return seastar::now();
}
    ...
    uint32_t truncate_seq)
{
  if (truncate_seq) {
    ...
    if (truncate_seq <= os.oi.truncate_seq) {
      logger().debug(
          "{} truncate seq {} <= current {}, no-op", __func__, truncate_seq,
          os.oi.truncate_seq);
      return write_ertr::make_ready_future<>();
    } else {
      logger().debug(
          "{} truncate seq {} > current {}, truncating", __func__, truncate_seq,
          os.oi.truncate_seq);
  ...
  if (os.oi.size != offset) {
    ...
    if (os.oi.size > offset) {
      ...
      osd_op_params.clean_regions.mark_data_region_dirty(
          offset, os.oi.size - offset);
    } else {
      // os.oi.size < offset
      osd_op_params.clean_regions.mark_data_region_dirty(
          os.oi.size, offset - os.oi.size);
    }
...
void
PGBackend::update_size_and_usage(
    object_stat_sum_t& delta_stats,
...
void
PGBackend::truncate_update_size_and_usage(
    object_stat_sum_t& delta_stats,
    object_info_t& oi,
...
static bool
is_offset_and_length_valid(const std::uint64_t offset, const std::uint64_t length)
{
  if (const std::uint64_t max = local_conf()->osd_max_object_size;
      offset >= max || length > max || offset + length > max) {
    logger().debug(
        "{} osd_max_object_size: {}, offset: {}, len: {}; "
    ...
    object_stat_sum_t& delta_stats)
{
  ...
  txn.set_alloc_hint(
      coll->get_cid(), ghobject_t{os.oi.soid}, os.oi.expected_object_size,
      os.oi.expected_write_size, os.oi.alloc_hint_flags);
  return seastar::now();
}
    ...
    object_stat_sum_t& delta_stats)
{
  ...
  if (auto seq = os.oi.truncate_seq; seq != 0 && op.extent.truncate_seq < seq) {
    ...
    if (os.exists && !os.oi.is_whiteout()) {
      txn.truncate(
          coll->get_cid(), ghobject_t{os.oi.soid}, op.extent.truncate_size);
      ...
      truncate_update_size_and_usage(
          delta_stats, os.oi, op.extent.truncate_size);
    }
  ...
  if (length == 0) {
    ...
    txn.write(
        coll->get_cid(), ghobject_t{os.oi.soid}, offset, length, std::move(buf),
        op.flags);
    update_size_and_usage(
        delta_stats, osd_op_params.modified_ranges, os.oi, offset, length);
  }
  osd_op_params.clean_regions.mark_data_region_dirty(
    ...
    object_stat_sum_t& delta_stats)
{
  ...
  maybe_create_new_object(os, txn, delta_stats);
  txn.write(
      coll->get_cid(), ghobject_t{os.oi.soid}, op.writesame.offset, len,
      std::move(repeated_indata), op.flags);
  update_size_and_usage(
      delta_stats, osd_op_params.modified_ranges, os.oi, op.writesame.offset,
      len);
    ...
    object_stat_sum_t& delta_stats)
{
  ...
  return _writefull(
      os, op.extent.truncate_size, osd_op.indata, txn, osd_op_params,
      delta_stats, op.flags);
}

PGBackend::rollback_iertr::future<>
PGBackend::rollback(
    ObjectState& os,
    const SnapSet& ss,
    const OSDOp& osd_op,
    ...
    const SnapContext& snapc)
{
  ...
  logger().debug(
      "{} deleting {} and rolling back to old snap {}", __func__, os.oi.soid,
      snapid);
  ...
  return obc_loader
     ...
     */
            logger().debug(
                "PGBackend::rollback: loaded clone_obc: {}",
                resolved_obc->obs.oi.soid);
            ...
            txn.clone(
                coll->get_cid(), ghobject_t{resolved_obc->obs.oi.soid},
            ...
            osd_op_params.clean_regions.mark_data_region_dirty(
                0, std::max(os.oi.size, resolved_obc->obs.oi.size));
            osd_op_params.clean_regions.mark_omap_dirty();

            // 3) Calculate clone_overlaps by following overlaps
            const auto& clone_overlap = head_obc->ssc->snapset.clone_overlap;
      ...
      .safe_then_interruptible(
          ...
          crimson::ct_error::enoent::handle([this, &os, snapid, &txn,
                                             &delta_stats, &snapc, &ss,
                                             &osd_op_params] {
            logger().debug(
                "PGBackend::rollback: deleting head on {}"
                " with snap_id of {}"
...
PGBackend::append_ierrorator::future<>
PGBackend::append(
    ObjectState& os,
    OSDOp& osd_op,
    ceph::os::Transaction& txn,
    ...
    object_stat_sum_t& delta_stats)
{
  ...
  if (op.extent.length) {
    txn.write(
        coll->get_cid(), ghobject_t{os.oi.soid}, os.oi.size /* offset */,
        op.extent.length, std::move(osd_op.indata), op.flags);
    update_size_and_usage(
        delta_stats, osd_op_params.modified_ranges, os.oi, os.oi.size,
        op.extent.length);
    osd_op_params.clean_regions.mark_data_region_dirty(
    ...
    object_stat_sum_t& delta_stats)
{
  ...
  return _truncate(
      os, txn, osd_op_params, delta_stats, op.extent.offset,
    ...
    object_stat_sum_t& delta_stats)
{
  ...
  txn.zero(
      coll->get_cid(), ghobject_t{os.oi.soid}, op.extent.offset,
      op.extent.length);
  interval_set<uint64_t> ch;
...
PGBackend::remove(ObjectState& os, ceph::os::Transaction& txn)
{
  // todo: snapset
  txn.remove(
      coll->get_cid(), ghobject_t{os.oi.soid, ghobject_t::NO_GEN, get_shard()});
  os.oi.size = 0;
  os.oi.new_object();
  os.exists = false;
...
{
  auto gstart = start.is_min() ? ghobject_t{}
                               : ghobject_t{start, 0, get_shard()};
  auto gend = end.is_max() ? ghobject_t::get_max()
                           : ghobject_t{end, 0, get_shard()};
...
PGBackend::setxattr_ierrorator::future<>
PGBackend::setxattr(
    ObjectState& os,
    const OSDOp& osd_op,
    ceph::os::Transaction& txn,
    ...
    ObjectContext::attr_cache_t& attr_cache)
{
  ...
  return crimson::os::with_store<
             ...
             store)
      .then([this, &os, &osd_op, &txn, &delta_stats,
             &attr_cache](unsigned store_max_name_len) {
        ...
        if (osd_op.op.xattr.name_len > max_name_len) {
          return setxattr_ierrorator::future<>(
              crimson::ct_error::enametoolong::make());
        }
    ...
    object_stat_sum_t& delta_stats) const
{
  ...
  auto get_attr_maybe_from_cache =
    ...
    if (auto cache_it = attr_cache.find(name);
        cache_it != std::end(attr_cache)) {
    ...
    object_stat_sum_t& delta_stats) const
{
  ...
  return get_attrs_maybe_from_cache().safe_then([&delta_stats,
                                                 &osd_op](auto&& attrs) {
...
static int
do_xattr_cmp_u64(int op, uint64_t lhs, bufferlist& rhs_xattr)
{
  uint64_t rhs;
...
PGBackend::cmp_xattr_ierrorator::future<>
PGBackend::cmp_xattr(
    const ObjectState& os,
    OSDOp& osd_op,
    object_stat_sum_t& delta_stats) const
{
  ...
  return getxattr(os.oi.soid, std::move(name))
      .safe_then_interruptible(
          [&delta_stats,
           &osd_op](auto&& xattr) -> cmp_xattr_ierrorator::future<> {
            delta_stats.num_rd++;
            delta_stats.num_rd_kb +=
      ...
      .handle_error_interruptible(
          crimson::ct_error::enodata::handle(
              [&delta_stats, &osd_op]() -> cmp_xattr_errorator::future<> {
                ...
                logger().debug(
                    "cmpxattr: xattr does not exist, comparison failed");
                return crimson::ct_error::ecanceled::make();
              }),
...
using get_omap_ertr = crimson::os::FuturizedStore::Shard::read_errorator::extend<
    crimson::ct_error::enodata>;
using get_omap_iertr = ::crimson::interruptible::
    interruptible_errorator<::crimson::osd::IOInterruptCondition, get_omap_ertr>;

static get_omap_iertr::future<crimson::os::FuturizedStore::Shard::omap_values_t>
...
static get_omap_iterate_ertr::future<ObjectStore::omap_iter_ret_t>
    ...
    omap_iterate_cb_t callback)
{
  if (oi.is_omap()) {
    return crimson::os::with_store<
        &crimson::os::FuturizedStore::Shard::omap_iterate>(
...
{
  return crimson::os::with_store<
             &crimson::os::FuturizedStore::Shard::omap_get_header>(
    ...
    uint32_t op_flags) const
{
  if (os.oi.is_omap()) {
    return omap_get_header(
        ...
        .safe_then_interruptible([&delta_stats,
    ...
    logger().error(
        "{} on {}: backend has no OMAP support", __func__, os.oi.soid);
    return seastar::now();
  }
}
    ...
    object_stat_sum_t& delta_stats) const
{
  ...
  max_return = std::min(max_return, max_omap_entries);

  ceph::bufferlist result;
  uint32_t num = 0;
  bool truncated = false;
  ObjectStore::omap_iter_seek_t start_from{
      start_after, ObjectStore::omap_iter_seek_t::UPPER_BOUND};
  omap_iterate_cb_t callback =
      [&result, &num, &truncated,
       max_return](std::string_view key, std::string_view value) {
        if (num >= max_return) {
          truncated = true;
...
static PGBackend::omap_cmp_ertr::future<>
do_omap_val_cmp(
    ...
    object_stat_sum_t& delta_stats) const
{
  ...
  start_from.seek_position = filter_prefix > start_after ? filter_prefix
                                                         : start_after;
  ...
  omap_iterate_cb_t callback =
      [filter_prefix, max_return, &result, &num,
       &truncated](std::string_view key, std::string_view value) {
        if (num >= max_return) {
          truncated = true;
        }
    ...
    object_stat_sum_t& delta_stats)
{
  ...
  txn.omap_rmkeyrange(
      coll->get_cid(), ghobject_t{os.oi.soid}, key_begin, key_end);
  delta_stats.num_wr++;
  os.oi.clear_omap_digest();
  return seastar::now();
}
...
PGBackend::stat(CollectionRef c, const ghobject_t& oid) const
{
    ...
    osd_op_params_t& osd_op_params)
{
  ...
  if (!ret.has_value()) {
    ...
    return _writefull(
        os, bl.length(), std::move(bl), txn, osd_op_params, delta_stats, 0);
  }
}

PGBackend::tmapup_iertr::future<>
PGBackend::tmapup(
    ObjectState& os,
    const OSDOp& osd_op,
    ceph::os::Transaction& txn,
  ...
  return PGBackend::write_iertr::now()
      ...
      .handle_error_interruptible(
          ...
          crimson::ct_error::assert_all{
              fmt::format(
                  "read error in mutate_object_contents of {}", os.oi.soid)
                  .c_str()})
      .si_then(
          [this, &os, &osd_op, &txn, &delta_stats, &osd_op_params](
              auto&& bl) mutable -> PGBackend::tmapup_iertr::future<> {
            auto result = crimson::common::do_tmap_up(
                osd_op.indata.cbegin(), std::move(bl));
            if (!result.has_value()) {
            ...
            return _writefull(
                os, result.value().length(), result.value(), txn, osd_op_params,
                delta_stats, 0);
          });
}
    ...
    object_stat_sum_t& delta_stats)
{
  ...
  return _read(oi.soid, os.oi.size, 0, oi.size, 0)
      .safe_then_interruptible_tuple(
