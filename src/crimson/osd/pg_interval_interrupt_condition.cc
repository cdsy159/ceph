...
bool
IOInterruptCondition::new_interval_created()
{
  ...
  if (ret) {
    DEBUGDPP(
        "stored epoch_started e{} < interval_start e{}", *pg, epoch_started,
        interval_start);
  }
  return ret;
}
...
bool
IOInterruptCondition::is_stopping()
{
...
bool
IOInterruptCondition::is_primary()
{
