...
seastar::future<>
PGShardMapping::dump_store_shards(Formatter* f) const
{
...
seastar::future<std::pair<core_id_t, store_index_t>>
PGShardMapping::get_or_create_pg_mapping(
    spg_t pgid,
    ...
    store_index_t store_index)
{
  ...
  if (find_iter != pg_to_core.end()) {
    ...
    if (core_expected != NULL_CORE && core_expected != core_found) {
      ERROR(
          "the mapping is inconsistent for pg {}: core {}, expected {}", pgid,
          core_found, core_expected);
      ceph_abort_msg("The pg mapping is inconsistent!");
    }
    return seastar::make_ready_future<std::pair<core_id_t, store_index_t>>(
        find_iter->second);
  } else {
    DEBUG(
        "calling primary to add mapping for pg {} to the expected core {}",
        pgid, core_expected);
    return container()
        .invoke_on(
            0,
            [pgid, core_expected, store_index, FNAME,
             this](auto& primary_mapping) {
              ...
              if (find_iter != primary_mapping.pg_to_core.end()) {
                ...
                if (core_expected != NULL_CORE) {
                  ...
                  if (core_expected != core_found ||
                      store_index != store_index_found) {
                    ERROR(
                        "the mapping is inconsistent for pg {} (primary): core "
                        "{}, expected {}",
                  ...
                  DEBUG(
                      "mapping pg {} to core {} (primary): already mapped and "
                  ...
                  DEBUG(
                      "mapping pg {} to core {} (primary): already mapped",
                      pgid, core_to_update);
                }
                ...
                std::map<core_id_t, std::map<unsigned, unsigned>>::iterator
                    core_shard_iter;
                ...
                if (core_expected == NULL_CORE) {
                  ...
                  count_iter =
                      primary_mapping.core_to_num_pgs.find(core_to_update);
                }
                ceph_assert_always(
                    primary_mapping.core_to_num_pgs.end() != count_iter);
                ++(count_iter->second);

                if (crimson::common::get_conf<bool>(
                        "seastore_require_partition_count_match_reactor_"
                        "count")) {
                  shard_index_update = 0;
                } else {
                  if (seastar::smp::count > store_shard_nums) {
                    auto alien_iter = primary_mapping.core_alien_to_num_pgs.find(
                        core_to_update);
                    auto core_iter = std::min_element(
                        alien_iter->second.begin(), alien_iter->second.end(),
                        [](const auto& left, const auto& right) {
                          return left.second < right.second;
                        });
                    core_iter->second++;
                  ...
                  if (seastar::smp::count >= store_shard_nums) {
                    shard_index_update =
                        0; // use the first store shard index on this core
                  } else {
                    core_shard_iter = primary_mapping.core_shard_to_num_pgs.find(
                        core_to_update);
                    ceph_assert_always(
                        core_shard_iter !=
                        primary_mapping.core_shard_to_num_pgs.end());
                    if (shard_index_update == NULL_STORE_INDEX) {
                      ...
                      shard_index_update =
                          shard_iter
                              ->first; //find the store shard index on this core
                    } else {
                      shard_iter =
                          core_shard_iter->second.find(shard_index_update);
                    }
                    ++(shard_iter->second);
                  }
                }
                [[maybe_unused]] auto [insert_iter, inserted] =
                    primary_mapping.pg_to_core.emplace(
                        pgid,
                        std::make_pair(core_to_update, shard_index_update));
                assert(inserted);
                DEBUG(
                    "mapping pg {} to core {} (primary): num_pgs {}, "
                    "store_index {}",
              ...
              return primary_mapping.container().invoke_on_others(
                  [pgid, core_to_update, shard_index_update,
                   FNAME](auto& other_mapping) {
                    auto find_iter = other_mapping.pg_to_core.find(pgid);
                    if (find_iter == other_mapping.pg_to_core.end()) {
                      DEBUG(
                          "mapping pg {} to core {} (others), store_index {}",
                          pgid, core_to_update, shard_index_update);
                      [[maybe_unused]] auto [insert_iter, inserted] =
                          other_mapping.pg_to_core.emplace(
                              pgid, std::make_pair(
                                        core_to_update, shard_index_update));
                      assert(inserted);
                    } else {
                      auto core_found = find_iter->second.first;
                      auto store_index_found = find_iter->second.second;
                      if (core_found != core_to_update ||
                          store_index_found != shard_index_update) {
                        ERROR(
                            "the mapping is inconsistent for pg {} (others): "
                            "core {}, expected {}, store_index {}",
                            pgid, core_found, core_to_update,
                            shard_index_update);
                        ceph_abort_msg("The pg mapping is inconsistent!");
                      }
                      DEBUG(
                          "mapping pg {} to core {} (others): already mapped",
                          pgid, core_to_update);
                    }
                  });
            })
        .then([this, pgid, core_expected, store_index, FNAME] {
          ...
          if (find_iter == pg_to_core.end()) {
            ERROR(
                "the mapping is inconsistent for pg {}: core not found, "
                "expected {}",
                pgid, core_expected);
            ceph_abort_msg("The pg mapping is inconsistent!");
          }
          ...
          if (seastar::smp::count <= store_shard_nums) {
            if ((core_expected != NULL_CORE && core_found != core_expected) ||
                (store_index != NULL_STORE_INDEX &&
                 shard_index_found != store_index)) {
              ERROR(
                  "the mapping is inconsistent for pg {}: core {}, expected {}",
                  pgid, core_found, core_expected);
              ceph_abort_msg("The pg mapping is inconsistent!");
            }
          }
          DEBUG(
              "returning pg {} mapping to core {} after broadcasted", pgid,
              core_found);
          return seastar::make_ready_future<std::pair<core_id_t, store_index_t>>(
              find_iter->second);
        });
...
seastar::future<>
PGShardMapping::remove_pg_mapping(spg_t pgid)
{
  ...
  return container().invoke_on(0, [pgid, FNAME](auto& primary_mapping) {
    ...
    auto count_iter =
        primary_mapping.core_to_num_pgs.find(find_iter->second.first);
    assert(count_iter != primary_mapping.core_to_num_pgs.end());
    assert(count_iter->second > 0);
    --(count_iter->second);

    auto core_shard_iter =
        primary_mapping.core_shard_to_num_pgs.find(find_iter->second.first);
...
void
PGMap::PGCreationState::dump_detail(Formatter* f) const
{
...
PGMap::wait_for_pg(PGCreationBlockingEvent::TriggerI&& trigger, spg_t pgid)
{
  if (auto pg = get_pg(pgid)) {
    return make_pair(
        wait_for_pg_fut(wait_for_pg_ertr::ready_future_marker{}, pg), true);
  } else {
    auto& state = pgs_creating.emplace(pgid, pgid).first->second;
    return make_pair(
        wait_for_pg_fut(trigger.maybe_record_blocking(
            state.promise.get_shared_future(), state)),
        state.creating);
  }
}

void
PGMap::remove_pg(spg_t pgid)
{
  ceph_assert(pgs.erase(pgid) == 1);
}

Ref<PG>
PGMap::get_pg(spg_t pgid)
{
...
void
PGMap::pg_created(spg_t pgid, Ref<PG> pg)
{
...
void
PGMap::pg_loaded(spg_t pgid, Ref<PG> pg)
{
  ceph_assert(!pgs.count(pgid));
  pgs.emplace(pgid, pg);
}
...
void
PGMap::pg_creation_canceled(spg_t pgid)
{
