...
PGMeta::PGMeta(crimson::os::BackendStore store, spg_t pgid) :
  store{store}, pgid{pgid}
{}

namespace {
template <typename T>
std::optional<T>
find_value(const FuturizedStore::Shard::omap_values_t& values, string_view key)
{
...
PGMeta::get_epoch()
{
  return crimson::os::with_store<
             &crimson::os::FuturizedStore::Shard::open_collection>(
             store, coll_t{pgid})
      .then([this](auto ch) {
        return crimson::os::with_store<
                   &crimson::os::FuturizedStore::Shard::omap_get_values>(
                   store, ch, pgid.make_pgmeta_oid(),
                   std::set<std::string>{string{infover_key}, string{epoch_key}},
                   0)
            .safe_then(
                [](auto&& values) {
                  {
...
seastar::future<std::tuple<pg_info_t, PastIntervals>>
PGMeta::load()
{
  return crimson::os::with_store<
             &crimson::os::FuturizedStore::Shard::open_collection>(
      ...
      .then([this](auto ch) {
        return crimson::os::with_store<
            ...
            store, ch, pgid.make_pgmeta_oid(),
            std::set<std::string>{
                string{infover_key}, string{info_key}, string{biginfo_key},
                string{fastinfo_key}},
            0);
      })
      .safe_then(
          [](auto&& values) {
            ...
            {
              using biginfo_t =
                  std::pair<PastIntervals, decltype(info.purged_snaps)>;
            ...
            return seastar::make_ready_future<
                std::tuple<pg_info_t, PastIntervals>>(
