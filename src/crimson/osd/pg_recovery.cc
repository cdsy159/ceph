...
void
PGRecovery::start_pglogbased_recovery()
{
  auto [op, fut] = pg->get_shard_services().start_operation<PglogBasedRecovery>(
      static_cast<crimson::osd::PG*>(pg), pg->get_shard_services(),
      pg->get_osdmap_epoch(), float(0.001));
...
size_t
PGRecovery::start_primary_recovery_ops(
    RecoveryBackend::RecoveryBlockingEvent::TriggerI& trigger,
    size_t max_to_start,
    std::vector<PGRecovery::interruptible_future<>>* out)
{
  ...
  DEBUGDPP(
      "recovering {} in pg {}, missing {}", *pg->get_dpp(),
      pg->get_recovery_backend()->total_recovering(),
      *static_cast<crimson::osd::PG*>(pg), missing);

  unsigned started = 0;
  int skipped = 0;

  map<eversion_t, hobject_t>::const_iterator p =
      missing.get_rmissing().lower_bound(eversion_t(
          0, pg->get_peering_state().get_pg_log().get_log().last_requested));
  while (started < max_to_start && p != missing.get_rmissing().end()) {
    ...
    auto it_objects =
        pg->get_peering_state().get_pg_log().get_log().objects.find(p->second);
    if (it_objects !=
        pg->get_peering_state().get_pg_log().get_log().objects.end()) {
      // look at log!
    ...
    DEBUGDPP(
        "{} item.need {} {} {} {} {}", *pg->get_dpp(), soid, item.need,
...
size_t
PGRecovery::start_replica_recovery_ops(
    RecoveryBackend::RecoveryBlockingEvent::TriggerI& trigger,
    size_t max_to_start,
    std::vector<PGRecovery::interruptible_future<>>* out)
{
    ...
    for (auto p = pm.get_rmissing().begin();
      ...
      if (soid > pi.last_backfill) {
        if (!pg->get_recovery_backend()->is_recovering(soid)) {
          ERRORDPP(
              ...
              *pg->get_dpp(), soid, pi.last_backfill);
      ...
      if (soid.is_snap() &&
          pg->get_peering_state().get_pg_log().get_missing().is_missing(
              soid.get_head())) {
        DEBUGDPP(
            "head {} still missing on primary", *pg->get_dpp(), soid.get_head());
        continue;
      }
...
PGRecovery::prep_object_replica_deletes(
    RecoveryBackend::RecoveryBlockingEvent::TriggerI& trigger,
    const hobject_t& soid,
    eversion_t need)
{
  ...
  if (added) {
    ...
    return recovering.wait_track_blocking(
        ...
        pg->get_recovery_backend()
            ->push_delete(soid, need)
            .then_interruptible([=, this] {
              if (pg->get_peering_state().get_pg_log().get_missing().is_missing(
                      soid)) {
...
RecoveryBackend::interruptible_future<>
PGRecovery::prep_object_replica_pushes(
    RecoveryBackend::RecoveryBlockingEvent::TriggerI& trigger,
    const hobject_t& soid,
    eversion_t need)
{
  ...
  if (added) {
    DEBUGDPP("soid {} v {}, new recovery", *pg->get_dpp(), soid, need);
    return recovering.wait_track_blocking(
        trigger, recover_object_with_throttle(soid, need)
                     .handle_exception_interruptible(
                         [=, this, soid = std::move(soid)](auto e) {
                           on_failed_recover({pg->get_pg_whoami()}, soid, need);
                           return seastar::make_ready_future<>();
    ...
    ceph::os::Transaction& t)
{
  ...
  if (const auto& log = pg->get_peering_state().get_pg_log();
      ...
      log.get_missing().get_items().find(recovery_info.soid)->second.need >
          recovery_info.version) {
    assert(pg->is_primary());
    if (const auto* latest =
            log.get_log().objects.find(recovery_info.soid)->second;
        latest->op == pg_log_entry_t::LOST_REVERT) {
      ceph_abort_msg("mark_unfound_lost (LOST_REVERT) is not implemented yet");
    }
  }

  return RecoveryBackend::interruptor::async([soid, &recovery_info, is_delete,
                                              &t, this] {
    if (soid.is_snap()) {
    ...
    pg->get_peering_state().recover_got(
        soid, recovery_info.version, is_delete, t);

    if (pg->is_primary()) {
      if (!is_delete) {
        auto& obc = pg->get_recovery_backend()
                        ->get_recovering(soid)
                        .obc; //TODO: move to pg backend?
        obc->obs.exists = true;
...
void
PGRecovery::_committed_pushed_object(epoch_t epoch, eversion_t last_complete)
{
  ...
  if (!pg->has_reset_since(epoch)) {
    ...
    DEBUGDPP(
        "pg has changed, not touching last_complete_ondisk", *pg->get_dpp());
  }
}

void
PGRecovery::request_replica_scan(
    ...
    const hobject_t& end)
{
  ...
  auto msg = crimson::make_message<MOSDPGScan>(
      MOSDPGScan::OP_SCAN_GET_DIGEST, pg->get_pg_whoami(),
      pg->get_osdmap_epoch(), pg->get_last_peering_reset(),
      spg_t(pg->get_pgid().pgid, target.shard), begin, end);
  std::ignore = pg->get_shard_services().send_to_osd(
      target.osd, std::move(msg), pg->get_osdmap_epoch());
}

void
PGRecovery::request_primary_scan(const hobject_t& begin)
{
  ...
  std::ignore = pg->get_recovery_backend()
                    ->scan_for_backfill_primary(
                        begin, local_conf()->osd_backfill_scan_min,
...
PGRecovery::interruptible_future<>
PGRecovery::recover_object_with_throttle(hobject_t soid, eversion_t need)
{
  ...
  auto releaser = co_await interruptor::make_interruptible(
      pg->get_shard_services().get_throttle(crimson::osd::scheduler::params_t{
...
void
PGRecovery::send_recovery_deletes(
    const hobject_t& obj,
    const std::vector<pg_shard_t>& peers)
{
...
void
PGRecovery::maybe_flush()
{
  for (auto& [target, req] : backfill_drop_requests) {
    std::ignore = pg->get_shard_services().send_to_osd(
        target.osd, std::move(req), pg->get_osdmap_epoch());
  }
  backfill_drop_requests.clear();
}

void
PGRecovery::update_peers_last_backfill(const hobject_t& new_last_backfill)
{
  LOG_PREFIX(PGRecovery::update_peers_last_backfill);
  ...
  for (const auto& bt : pg->get_peering_state().get_backfill_targets()) {
    if (const pg_info_t& pinfo = pg->get_peering_state().get_peer_info(bt);
      ...
      DEBUGDPP(
          "peer {} num_objects now {} / {}", *pg->get_dpp(), bt,
...
bool
PGRecovery::budget_available() const
{
...
template <typename T>
void
PGRecovery::start_peering_event_operation_listener(T&& evt, float delay)
{
  (void)pg->schedule_event_after(
      PGPeeringEventRef(std::make_shared<PGPeeringEvent>(
          pg->get_osdmap_epoch(), pg->get_osdmap_epoch(), std::forward<T>(evt))),
      delay);
}

void
PGRecovery::backfilled()
{
...
void
PGRecovery::all_replicas_recovered()
{
...
void
PGRecovery::dispatch_backfill_event(
...
void
PGRecovery::on_activate_complete()
{
...
void
PGRecovery::on_backfill_reserved()
{
...
hobject_t
PGRecovery::get_temp_recovery_object(const hobject_t& target, eversion_t version)
{
  return pg->get_recovery_backend()->get_temp_recovery_object(target, version);
}
