...
seastar::future<>
PGShardManager::load_pgs(crimson::os::FuturizedStore& store)
{
    ...
    return seastar::parallel_for_each(colls_cores, [this](auto coll_core) {
      ...
      if (coll.is_pg(&pgid)) {
        return get_pg_to_shard_mapping()
            .get_or_create_pg_mapping(pgid, shard_core, store_index)
            .then([this, pgid](auto core_store) {
              return this->with_remote_shard_state(
                  core_store.first, [pgid, core_store](
                                        PerShardState& per_shard_state,
                                        ShardServices& shard_services) {
                    return shard_services.load_pg(pgid, core_store.second)
                        .then([pgid, &per_shard_state](auto&& pg) {
                          logger().info("load_pgs: loaded {}", pgid);
                          return pg->clear_temp_objects().then(
                              [&per_shard_state, pg, pgid] {
                                per_shard_state.pg_map.pg_loaded(
                                    pgid, std::move(pg));
                              });
                        });
                  });
...
PGShardManager::get_pg_stats() const
{
  ...
  return shard_services.map_reduce0(
      [](auto& local) { return local.local_state.get_pg_stats(); },
...
seastar::future<>
PGShardManager::broadcast_map_to_pgs(epoch_t epoch)
{
  ceph_assert(seastar::this_shard_id() == PRIMARY_CORE);
  return shard_services
      ...
      .then([this, epoch] {
        logger().debug(
            "PGShardManager::broadcast_map_to_pgs "
            "broadcasted up to {}",
            epoch);
...
seastar::future<>
PGShardManager::set_up_epoch(epoch_t e)
{
  ceph_assert(seastar::this_shard_id() == PRIMARY_CORE);
  return shard_services.invoke_on_all(
      seastar::smp_submit_to_options{}, [e](auto& local_service) {
...
seastar::future<>
PGShardManager::set_superblock(OSDSuperblock superblock)
{
  ceph_assert(seastar::this_shard_id() == PRIMARY_CORE);
