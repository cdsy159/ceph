...
public:
  ...
  get_local_state()
  {
    return get_shard_services().local_state;
  }

  auto&
  get_local_state() const
  {
    return get_shard_services().local_state;
  }
  ...
  {
    return pg_to_shard_mapping.local();
  }

  auto&
  get_pg_to_shard_mapping() const
  {
    return pg_to_shard_mapping.local();
  }

  seastar::future<>
  update_map(local_cached_map_t&& map)
  {
    ...
    return seastar::do_with(
        ...
        [this, map](auto& fmaps) {
          ...
          return shard_services.invoke_on_all([&fmaps](auto& local) mutable {
            local.local_state.update_map(make_local_shared_foreign(
  ...
  FORWARD(
      is_initializing,
      is_initializing,
      get_shard_services().local_state.osd_state)
  ...
  FORWARD(
      wait_for_map,
      wait_for_map,
      get_shard_services().local_state.osdmap_gate)
  ...
  template <typename F>
  auto
  with_remote_shard_state(core_id_t core, F&& f)
  {
  ...
  auto
  process_ordered_op_remotely(
      OSDConnectionPriv::crosscore_ordering_t::seq_t cc_seq,
      ShardServices& target_shard_services,
      typename T::IRef&& op,
      F&& f)
  {
    auto& crosscore_ordering =
        get_osd_priv(&op->get_foreign_connection()).crosscore_ordering;
    if (crosscore_ordering.proceed_or_wait(cc_seq)) {
      return std::invoke(std::move(f), target_shard_services, std::move(op));
    } else {
      auto& logger = crimson::get_logger(ceph_subsys_osd);
      logger.debug(
          "{} got {} at the remote pg, wait at {}", *op, cc_seq,
          crosscore_ordering.get_in_seq());
      return crosscore_ordering.wait(cc_seq).then(
          [this, cc_seq, &target_shard_services, op = std::move(op),
  ...
  template <typename T, typename F>
  auto
  with_remote_shard_state_and_op(core_id_t core, typename T::IRef&& op, F&& f)
  {
    ...
    auto& opref = *op;
    auto& crosscore_ordering =
        get_osd_priv(&opref.get_local_connection()).crosscore_ordering;
    auto cc_seq = crosscore_ordering.prepare_submit(core);
    auto& logger = crimson::get_logger(ceph_subsys_osd);
    logger.debug("{}: send {} to the remote pg core {}", opref, cc_seq, core);
    return opref.get_handle().complete().then([this, core, cc_seq,
                                               op = std::move(op),
                                               f = std::move(f)]() mutable {
      ...
      return shard_services.invoke_on(
          core,
          [this, cc_seq, f = std::move(f), op = std::move(op),
           f_conn = std::move(f_conn)](auto& target_shard_services) mutable {
            op->finish_remote_submission(std::move(f_conn));
      ...
      store_index_t store_index)
  {
    ...
    return opref
        .template with_blocking_event<PGMap::PGCreationBlockingEvent>(
            [&target_shard_services, &opref, store_index](auto&& trigger) {
              return target_shard_services.get_or_create_pg(
                  std::move(trigger), opref.get_pgid(), store_index,
        ...
        .handle_error(crimson::ct_error::ecanceled::handle([&logger,
                                                            &opref](auto) {
    ...
    return opref
        .template with_blocking_event<PGMap::PGCreationBlockingEvent>(
            [&target_shard_services, &opref, &logger](auto&& trigger) mutable {
              ...
              auto fut =
                  ShardServices::wait_for_pg_ertr::make_ready_future<Ref<PG>>(
                      pg);
              if (!pg) {
                if (opref.requires_pg()) {
                  ...
                  if (!osdmap->is_up_acting_osd_shard(
                          opref.get_pgid(),
                          target_shard_services.local_state.whoami)) {
                    logger.debug(
              ...
              return fut
                  .safe_then([&logger, &target_shard_services,
                              &opref](Ref<PG> pgref) {
                    logger.debug("{}: have_pg", opref);
                    return opref.with_pg(target_shard_services, pgref);
                  })
                  .handle_error(crimson::ct_error::ecanceled::handle(
                      [&logger, &opref](auto) {
                        logger.debug(
                            "{}: pg creation canceled, dropping", opref);
                        return seastar::now();
                      }));
            })
        .then([op = std::move(op)] {});
  ...
  seastar::future<>
  invoke_on_each_shard_seq(F&& f) const
  {
    return invoke_on_all_seq([this, f = std::forward<F>(f)]() mutable {
  ...
  start_pg_operation(Args&&... args)
  {
    ...
    auto fut =
        opref
            .template enter_stage<>(opref.get_connection_pipeline().await_active)
            .then([this, &opref, &logger] {
              logger.debug(
                  "{}: start_pg_operation in await_active stage", opref);
              return get_shard_services().local_state.osd_state.when_active();
            })
            .then([&logger, &opref] {
              logger.debug(
                  "{}: start_pg_operation active, entering await_map", opref);
              ...
              return opref.template with_blocking_event<OSDMapBlockingEvent>(
                  [this, &opref](auto&& trigger) {
                    ...
                    return get_shard_services()
                        .local_state.osdmap_gate.wait_for_map(
                            std::move(trigger), opref.get_epoch(),
                            &get_shard_services());
                  });
            })
            .then([&logger, &opref](auto epoch) {
              logger.debug(
                  "{}: got map {}, entering get_pg_mapping", opref, epoch);
              return opref.template enter_stage<>(
            ...
            .then_wrapped([this, &logger, op = std::move(op)](auto fut) mutable {
              ...
              logger.debug(
                  "{}: can_create={}, target-core={}", *op, T::can_create(),
                  core_store.first);
                  ...
                  [this, core_store](
                    ...
                    return opref
                        .template enter_stage<>(
                            opref.get_pershard_pipeline(target_shard_services)
                                .create_or_wait_pg)
                        .then([this, &target_shard_services, op = std::move(op),
                               core_store]() mutable {
  ...
  template <typename T, typename... Args>
  auto
  start_pg_operation_active(Args&&... args)
  {
    ...
    return this->template with_remote_shard_state_and_op<T>(
        core, std::move(op),
        [FNAME](ShardServices& target_shard_services, typename T::IRef op) {
          auto& opref = *op;
          auto pg = target_shard_services.get_pg(opref.get_pgid());
          if (!pg) {
            SUBDEBUG(
                osd, "{} pg {} not present, must be from prior interval", opref,
                opref.get_pgid());
            return seastar::now();
          }
          SUBDEBUG(osd, "{}: have_pg", opref);
          return op->with_pg(target_shard_services, pg).finally([op] {});
        });
  }
