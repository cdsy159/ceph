    ...
    eversion_t version) const
{
  ...
  hobject_t hoid = target.make_temp_hobject(fmt::format(
      "temp_recovering_{}_{}_{}_{}", pg.get_info().pgid, version,
      pg.get_info().history.same_interval_since, target.snap));
  DEBUGDPP("{}", pg, hoid);
  return hoid;
}

void
RecoveryBackend::add_temp_obj(const hobject_t& oid)
{
  backend->add_temp_obj(oid);
}

void
RecoveryBackend::clear_temp_obj(const hobject_t& oid)
{
  backend->clear_temp_obj(oid);
}

void
RecoveryBackend::clean_up(ceph::os::Transaction& t, interrupt_cause_t why)
{
...
void
RecoveryBackend::WaitForObjectRecovery::interrupt(interrupt_cause_t why)
{
  switch (why) {
  case interrupt_cause_t::INTERVAL_CHANGE:
    ...
    if (pulled) {
      pulled->set_exception(crimson::common::actingset_changed(pg.is_primary()));
      pulled.reset();
    }
    for (auto& [pg_shard, pr] : pushes) {
      pr.set_exception(crimson::common::actingset_changed(pg.is_primary()));
    }
    pushes.clear();
    break;
  default:
...
void
RecoveryBackend::WaitForObjectRecovery::stop()
{
  if (readable) {
...
void
RecoveryBackend::handle_backfill_finish(
    MOSDPGBackfill& m,
    crimson::net::ConnectionXcoreRef conn)
{
  ...
  auto reply = crimson::make_message<MOSDPGBackfill>(
      MOSDPGBackfill::OP_BACKFILL_FINISH_ACK, pg.get_osdmap_epoch(),
      m.query_epoch, spg_t(pg.get_pgid().pgid, pg.get_primary().shard));
  ...
  shard_services.start_operation<crimson::osd::LocalPeeringEvent>(
      static_cast<crimson::osd::PG*>(&pg), pg.get_pg_whoami(), pg.get_pgid(),
      pg.get_osdmap_epoch(), pg.get_osdmap_epoch(), RecoveryDone{});
}

RecoveryBackend::interruptible_future<>
RecoveryBackend::handle_backfill_progress(MOSDPGBackfill& m)
{
...
RecoveryBackend::interruptible_future<>
RecoveryBackend::handle_backfill_finish_ack(MOSDPGBackfill& m)
{
...
RecoveryBackend::interruptible_future<>
RecoveryBackend::handle_backfill_remove(MOSDPGBackfillRemove& m)
{
    ...
    const std::set<pg_shard_t>& backfill_targets)
{
  ...
  auto version_map = seastar::make_lw_shared<
      std::multimap<hobject_t, std::pair<shard_id_t, eversion_t>>>();
  auto&& [objects, next] = co_await backend->list_objects(start, max);
  co_await interruptor::parallel_for_each(
      objects,
      seastar::coroutine::lambda(
          [FNAME, this, version_map, backfill_targets](
            ...
            auto found =
                co_await pg.obc_loader
                    .load_and_lock(obc_manager, RWState::RWREAD)
                    .si_then([] { return true; })
                    .handle_error_interruptible(
                        ...
                        crimson::ct_error::assert_all(
                            fmt::format(
                                "{} {} error when loading obc", pg, FNAME)
                                .c_str()));
            if (!found) {
            ...
            if (obc_manager.get_obc()->obs.exists) {
              ...
              if (shard_versions.empty()) {
                version_map->emplace(
                    object, std::make_pair(shard_id_t::NO_SHARD, version));
              } else {
                ...
                for (auto& shard : backfill_targets) {
                  if (shard_versions.contains(shard.shard)) {
                    ...
                    version_map->emplace(
                        object, std::make_pair(shard.shard, shard_version));
  ...
  DEBUGDPP(
      "{} PrimaryBackfillInterval filled, leaving, {}",
    ...
    const std::int64_t max)
{
  ...
  co_await interruptor::parallel_for_each(
      ...
      seastar::coroutine::lambda(
          [FNAME, this,
           version_map](const hobject_t& object) -> interruptible_future<> {
            DEBUGDPP("querying obj:{}", pg, object);
            auto obc_manager = pg.obc_loader.get_obc_manager(
                object, /* resolve_clone = */ false);
  ...
  bi.objects = std::move(*version_map);
  DEBUGDPP(
      "{} ReplicaBackfillInterval filled, leaving, {}",
  ...
  if (false /* FIXME: check for backfill too full */) {
    std::ignore =
        shard_services.start_operation<crimson::osd::LocalPeeringEvent>(
            ...
            static_cast<crimson::osd::PG*>(&pg), pg.get_pg_whoami(),
            pg.get_pgid(), pg.get_osdmap_epoch(), pg.get_osdmap_epoch(),
            PeeringState::BackfillTooFull());
    return seastar::now();
  }
  return scan_for_backfill_replica(
             std::move(m.begin),
             crimson::common::local_conf().get_val<std::int64_t>(
                 "osd_backfill_scan_min"),
             crimson::common::local_conf().get_val<std::int64_t>(
                 "osd_backfill_scan_max"))
      .then_interruptible([this, query_epoch = m.query_epoch,
                           conn](auto backfill_interval) {
        auto reply = crimson::make_message<MOSDPGScan>(
            MOSDPGScan::OP_SCAN_DIGEST, pg.get_pg_whoami(),
            pg.get_osdmap_epoch(), query_epoch,
            spg_t(pg.get_info().pgid.pgid, pg.get_primary().shard),
            backfill_interval.begin, backfill_interval.end);
...
RecoveryBackend::interruptible_future<>
RecoveryBackend::handle_scan_digest(MOSDPGScan& m)
{
...
RecoveryBackend::interruptible_future<>
RecoveryBackend::handle_scan(MOSDPGScan& m, crimson::net::ConnectionXcoreRef conn)
{
    ...
    crimson::net::ConnectionXcoreRef conn)
{
  ...
  case MSG_OSD_PG_BACKFILL_REMOVE:
    return handle_backfill_remove(
        *boost::static_pointer_cast<MOSDPGBackfillRemove>(m));
  case MSG_OSD_PG_SCAN:
    return handle_scan(*boost::static_pointer_cast<MOSDPGScan>(m), conn);
  default:
    return seastar::make_exception_future<>(std::invalid_argument(fmt::format(
        "invalid request type: {}", (uint16_t)m->get_header().type)));
  }
}

std::unique_ptr<RecoveryBackend>
    ...
    PGBackend* backend)
{
  ...
  default:
    ceph_abort_msg(seastar::format("unsupported pool type '{}'", pool.type));
  }
}
