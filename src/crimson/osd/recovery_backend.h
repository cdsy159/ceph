...
public:
  ...
  std::pair<WaitForObjectRecovery&, bool>
  add_recovering(const hobject_t& soid)
  {
  ...
  seastar::future<>
  add_unfound(const hobject_t& soid)
  {
  ...
  void
  found_and_remove(const hobject_t& soid)
  {
  ...
  WaitForObjectRecovery&
  get_recovering(const hobject_t& soid)
  {
  ...
  void
  remove_recovering(const hobject_t& soid)
  {
    recovering.erase(soid);
  }

  bool
  is_recovering(const hobject_t& soid) const
  {
    return recovering.count(soid) != 0;
  }

  uint64_t
  total_recovering() const
  {
  ...
  interruptible_future<> recover_delete(const hobject_t& soid, eversion_t need);
  interruptible_future<> push_delete(const hobject_t& soid, eversion_t need);
  ...
  void
  on_peering_interval_change(ceph::os::Transaction& t)
  {
  ...
  stop()
  {
    ...
    for (auto& [soid, promise] : unfound) {
      promise.set_exception(crimson::common::system_shutdown_exception());
    }
    return on_stop();
  }
  ...
  void
  for_each_recovery_waiter(Func&& f)
  {
    for (auto& [soid, recovery_waiter] : recovering) {
...
public:
  ...
  public:
    ...
    WaitForObjectRecovery(crimson::osd::PG& pg) :
      pg(pg)
    {}
    ...
    seastar::future<>
    wait_for_pushes(pg_shard_t shard)
    {
    ...
    wait_for_all_pushes()
    {
      return seastar::parallel_for_each(pushes, [](auto& entry) {
    ...
    auto
    wait_track_blocking(T& trigger, F&& fut)
    {
    ...
    void
    set_push_failed(pg_shard_t shard, std::exception_ptr e)
    {
      auto it = pushes.find(shard);
      if (it != pushes.end()) {
