...
MURef<MOSDRepOp>
ReplicatedBackend::new_repop_msg(
    ...
    ceph_tid_t tid)
{
  ...
  auto m = crimson::make_message<MOSDRepOp>(
      osd_op_p.req_id, whoami, spg_t{pgid, pg_shard.shard}, hoid,
      CEPH_OSD_FLAG_ACK | CEPH_OSD_FLAG_ONDISK, map_epoch, min_epoch, tid,
      osd_op_p.at_version);
  if (send_op) {
    ...
    std::vector<pg_log_entry_t>&& logv)
{
  ...
  auto pending_txn = pending_trans
                         .try_emplace(
                             tid, pg_shards.size(), osd_op_p.at_version,
  ...
  pg.log_operation(
      std::move(log_entries), std::nullopt, osd_op_p.pg_trim_to,
      osd_op_p.at_version, osd_op_p.pg_committed_to, true, txn, false);
          ...
          .then_interruptible([pending_txn, this, _new_clone, &hoid,
            ...
            if (_new_clone && !to_push_clone.empty()) {
              pg.enqueue_push_for_backfill(
                  _new_clone->obs.oi.soid, _new_clone->obs.oi.version,
                  to_push_clone);
            }
...
void
ReplicatedBackend::on_actingset_changed(bool same_primary)
{
...
void
ReplicatedBackend::got_rep_op_reply(const MOSDRepOpReply& reply)
{
  ...
  for (auto& peer : peers.acked_peers) {
    if (peer.shard == reply.from) {
      ...
      pg.update_peer_last_complete_ondisk(peer.shard, peer.last_complete_ondisk);
      if (--peers.pending == 0) {
        pg.complete_write(peers.at_version, peers.last_complete);
        peers.all_committed.set_value();
        peers.all_committed = {};
...
seastar::future<>
ReplicatedBackend::request_committed(
...
ReplicatedBackend::interruptible_future<>
ReplicatedBackend::send_pct_update()
{
  LOG_PREFIX(ReplicatedBackend::send_pct_update);
  DEBUGDPP("", dpp);
  ceph_assert(PG_HAVE_FEATURE(pg.peering_state.get_pg_acting_features(), PCT));
  for (const auto& i : pg.peering_state.get_actingset()) {
    if (i == pg.get_pg_whoami())
      continue;

    auto pct_update = crimson::make_message<MOSDPGPCT>(
        spg_t(pg.get_pgid().pgid, i.shard), pg.get_osdmap_epoch(),
        pg.get_same_interval_since(), pg.peering_state.get_pg_committed_to());

    co_await interruptor::make_interruptible(shard_services.send_to_osd(
        i.osd, std::move(pct_update), pg.get_osdmap_epoch()));
  }
}
...
ReplicatedBackend::maybe_kick_pct_update()
{
  ...
  if (!PG_HAVE_FEATURE(pg.peering_state.get_pg_acting_features(), PCT)) {
...
void
ReplicatedBackend::do_pct(const MOSDPGPCT& m)
{
...
ReplicatedBackend::getxattr(const hobject_t& soid, std::string&& key) const
{
