...
ReplicatedRecoveryBackend::recover_object(const hobject_t& soid, eversion_t need)
{
            ...
            .with_obc<RWState::RWREAD>(
                ...
                [FNAME, this, soid, need](auto head, auto obc) {
                  if (!obc->obs.exists) {
                    ...
                    DEBUGDPP(
                        "obj={}, v={} not found on primary, aborting backfill",
                        pg, soid, need);
            ...
            .handle_error_interruptible(
                crimson::osd::PG::load_obc_ertr::all_same_way(
                    [FNAME, this, soid](auto& code) {
                      ...
                      ERRORDPP(
                          "saw error code {}, ignoring object {}", pg, code,
                          soid);
                      return seastar::now();
                    }));
      });
  ...
  return seastar::do_with(
             ...
             [this, need, soid, head_obc](auto& shards) {
               return interruptor::parallel_for_each(
                   shards, [this, need, soid, head_obc](auto shard) {
                     return prep_push_to_replica(soid, need, shard)
                         .then_interruptible([this, soid, shard](auto push) {
                           ...
                           seastar::future<> push_future =
                               get_recovering(soid).wait_for_pushes(shard);
                           return interruptor::make_interruptible(
                                      shard_services.send_to_osd(
                                          shard.osd, std::move(msg),
                                          pg.get_osdmap_epoch()))
                               .then_interruptible([push_future = std::move(
                                                        push_future)]() mutable {
    ...
    eversion_t need)
{
  ...
  return interruptor::repeat_eagain([this, soid, need] {
           ...
           return seastar::do_with(PullOp{}, [this, soid, need](PullOp& pull_op) {
             return pg.obc_loader
                 .with_obc<RWState::RWREAD>(
                     soid.get_head(),
                 ...
                 .si_then([this, soid, &pull_op]() {
                   ...
                   return shard_services
                       .send_to_osd(
                           pull_info.from.osd, std::move(msg),
                           pg.get_osdmap_epoch())
      ...
      .handle_error_interruptible(crimson::ct_error::assert_all(
          fmt::format("{} {} error with {} need {} ", pg, FNAME, soid, need)
              .c_str()));
}

RecoveryBackend::interruptible_future<>
RecoveryBackend::push_delete(const hobject_t& soid, eversion_t need)
{
  ...
  return interruptor::parallel_for_each(
      pg.get_acting_recovery_backfill(),
      [FNAME, this, soid, need,
       min_epoch](pg_shard_t shard) -> interruptible_future<> {
        ...
        if (iter->second.is_missing(soid)) {
          ...
          seastar::future<> push_future =
              get_recovering(soid).wait_for_pushes(shard);
          return interruptor::make_interruptible(shard_services.send_to_osd(
                                                     shard.osd, std::move(msg),
                                                     pg.get_osdmap_epoch()))
              .then_interruptible(
...
RecoveryBackend::interruptible_future<>
RecoveryBackend::handle_recovery_delete(Ref<MOSDPGRecoveryDelete> m)
{
    ...
    epoch_t epoch_to_freeze)
{
  ...
  return backend->load_metadata(soid)
      .safe_then_interruptible(
          [FNAME, this](auto lomt) -> interruptible_future<> {
            if (lomt->os.exists) {
              return seastar::do_with(
                  ceph::os::Transaction(),
                  [FNAME, this, lomt = std::move(lomt)](auto& txn) mutable {
                    return interruptor::async([this, lomt = std::move(lomt),
                                               &txn] {
                             pg.remove_maybe_snapmapped_object(
                                 txn, lomt->os.oi.soid);
                           })
                        .then_interruptible([FNAME, this, &txn]() mutable {
                          DEBUGDPP("submitting transaction", pg);
                          return crimson::os::with_store_do_transaction(
          ...
          [this, soid, epoch_to_freeze, need] {
            return seastar::do_with(
                ...
                [soid, need, this, epoch_to_freeze](auto& recovery_info) {
                  ...
                  return on_local_recover_persist(
                      soid, recovery_info, true, epoch_to_freeze);
                });
          },
...
RecoveryBackend::interruptible_future<>
RecoveryBackend::recover_delete(const hobject_t& soid, eversion_t need)
{
  ...
  return seastar::do_with(
      ...
      [FNAME, this, soid, need, cur_epoch](auto& stat_diff) {
        return local_recover_delete(soid, need, cur_epoch)
            .then_interruptible(
                [FNAME, this, &stat_diff, cur_epoch, soid,
                 need]() -> interruptible_future<> {
                  if (!pg.has_reset_since(cur_epoch)) {
                    ...
                    for (const auto& shard : pg.get_acting_recovery_backfill()) {
                      ...
                      if (pg.get_shard_missing(shard).is_missing(soid)) {
                        DEBUGDPP(
                            "soid {} needs to be deleted from replica {}", pg,
                            soid, shard);
                        object_missing = true;
                        break;
                      }
            ...
            .then_interruptible([this, soid, &stat_diff] {
              const auto& missing =
                  pg.get_peering_state().get_pg_log().get_missing();
    ...
    pg_shard_t pg_shard)
{
  ...
  if (soid.snap && soid.snap < CEPH_NOSNAP) {
    ...
    if (pg.get_local_missing().is_missing(head)) {
      DEBUGDPP("missing head {}, pushing raw clone", pg, head);
      if (obc->obs.oi.size) {
        subsets.data_subset.insert(0, obc->obs.oi.size);
      }
      return prep_push(soid, need, pg_shard, subsets, push_info_ss);
    }
    auto ssc = obc->ssc;
    ceph_assert(ssc);
    ...
    subsets = crimson::osd::calc_clone_subsets(
        ssc->snapset, soid, missing,
        // get_peer_info() asserts `peer_info` existence.
        pg.get_peering_state().get_peer_info(pg_shard).last_backfill);
    ...
    subsets = crimson::osd::calc_head_subsets(
        obc->obs.oi.size, ssc->snapset, soid, missing,
        pg.get_peering_state().get_peer_info(pg_shard).last_backfill);
  }
  return prep_push(soid, need, pg_shard, subsets, push_info_ss);
}
    ...
    const SnapSet push_info_ss)
{
  ...
  return build_push_op(
             push_info.recovery_info, push_info.recovery_progress,
             &push_info.stat)
...
void
ReplicatedRecoveryBackend::prepare_pull(
    const crimson::osd::ObjectContextRef& head_obc,
    PullOp& pull_op,
    ...
    eversion_t need)
{
  ...
  pull_op.recovery_info = set_recovery_info(soid, head_obc->ssc);
  pull_op.soid = soid;
    ...
    object_stat_sum_t* stat)
{
  ...
  return seastar::do_with(
      ...
      recovery_info.version, PushOp(),
      [FNAME, this, &recovery_info, &progress,
       stat](auto& new_progress, auto& available, auto& v, auto& push_op) {
        return read_metadata_for_push_op(
                   recovery_info.soid, progress, new_progress, v, &push_op)
            .then_interruptible([&, FNAME](eversion_t local_ver) mutable {
              ...
              if (v == eversion_t()) {
                ...
                ERRORDPP(
                    "push {} v{} failed because local copy is {}", pg,
                    recovery_info.soid, recovery_info.version, local_ver);
                // TODO: bail out
              }
              return read_omap_for_push_op(
                  recovery_info.soid, progress, new_progress, available,
                  &push_op);
            })
            .then_interruptible([FNAME, this, &recovery_info, &progress,
                                 &available, &push_op]() mutable {
              DEBUGDPP(
                  "available: {}, copy_subset: {}", pg, available,
                  recovery_info.copy_subset);
              return read_object_for_push_op(
                  recovery_info.soid, recovery_info.copy_subset,
                  progress.data_recovered_to, available, &push_op);
            })
            .then_interruptible([FNAME, this, &recovery_info, &v, &progress,
                                 &new_progress, stat,
                                 &push_op](uint64_t recovered_to) mutable {
              ...
              DEBUGDPP(
                  "push_op version: {}, push_op data length: {}", pg,
             ...
             interruptor::when_all_succeed(
                 backend
                     ...
                     .handle_error_interruptible<false>(
                         crimson::os::FuturizedStore::Shard::read_errorator::
                             all_same_way([FNAME, this,
                                           oid](const std::error_code& e) {
                               DEBUGDPP(
                                   "error {} when getting omap header: {}", pg,
                                   e, oid);
                 ...
                 interruptor::make_interruptible(
                     crimson::os::with_store<
                         ...
                         store, coll, ghobject_t(oid),
                         CEPH_OSD_OP_FLAG_FADVISE_DONTNEED))
                     .handle_error_interruptible<false>(
                         crimson::os::FuturizedStore::Shard::get_attrs_ertr::
                             all_same_way([FNAME, this,
                                           oid](const std::error_code& e) {
                               DEBUGDPP(
                                   "error {} when getting attrs: {}", pg, e,
                                   oid);
                               return seastar::make_ready_future<
                                   crimson::os::FuturizedStore::Shard::attrs_t>();
                             }))))
      .then_unpack_interruptible([FNAME, this, &new_progress,
                                  push_op](auto bl, auto attrs) {
        if (bl.length() == 0) {
    ...
    PushOp* push_op)
{
  ...
  return interruptor::make_interruptible(backend->fiemap(
                                             coll, ghobject_t{oid}, 0,
      ...
      .safe_then_interruptible([=, this](auto&& fiemap_included) mutable {
        ...
        return interruptor::make_interruptible(
            crimson::os::with_store<&crimson::os::FuturizedStore::Shard::readv>(
                store, coll, ghobject_t{oid}, std::ref(push_op->data_included),
                CEPH_OSD_OP_FLAG_FADVISE_DONTNEED));
      })
      .safe_then_interruptible(
...
nullopt_if_empty(const std::string& s)
{
  return s.empty() ? std::nullopt : std::make_optional(s);
}

static bool
is_too_many_entries_per_chunk(const PushOp* push_op)
{
    ...
    PushOp* push_op)
{
  ...
  std::function<ObjectStore::omap_iter_ret_t(std::string_view, std::string_view)>
      callback = [&max_len,
                  push_op](std::string_view key, std::string_view value) {
  ...
  co_await interruptor::make_interruptible(
      crimson::os::with_store<&crimson::os::FuturizedStore::Shard::omap_iterate>(
          shard_services.get_store(pg.get_store_index()), coll, ghobject_t{oid},
          start_from, callback, 0, nullptr)
...
ReplicatedRecoveryBackend::handle_pull(Ref<MOSDPGPull> m)
{
  ...
  DEBUGDPP("{}", pg, *m);
  return seastar::do_with(
      m->take_pulls(), [FNAME, this, from = m->from](auto& pulls) {
        return interruptor::parallel_for_each(
            pulls, [FNAME, this, from](auto& pull_op) {
              ...
              return backend->stat(coll, ghobject_t(soid))
                  ...
                  .then_interruptible([this, from](auto push_op) {
                    ...
                    return shard_services.send_to_osd(
                        from.osd, std::move(msg), pg.get_osdmap_epoch());
                  });
            });
      });
}
    ...
    PullOp* response)
{
  LOG_PREFIX(ReplicatedRecoveryBackend::handle_pull);
  DEBUGDPP(
      "{} {} data.size() is {} data_included: {}", pg, push_op.recovery_info,
      push_op.after_progress, push_op.data.length(), push_op.data_included);
  ...
  if (pull_info.recovery_progress.first) {
    ...
    if (pull_info.recovery_info.soid.snap &&
      ...
      recalc_subsets(pull_info.recovery_info, ssc);
    }

    pull_info.obc = recovery_waiter.obc =
        pg.obc_loader.create_cached_obc_from_push_data(
            pull_info.recovery_info.oi, ssc);
  };

  const bool first = pull_info.recovery_progress.first;
  pull_info.recovery_progress = push_op.after_progress;
  DEBUGDPP(
      "new recovery_info {}, new progress {}", pg, pull_info.recovery_info,
      pull_info.recovery_progress);
  interval_set<uint64_t> data_zeros;
  {
    uint64_t offset = push_op.before_progress.data_recovered_to;
    uint64_t length =
        (push_op.after_progress.data_recovered_to -
         push_op.before_progress.data_recovered_to);
    if (length) {
      data_zeros.insert(offset, length);
    }
  }
  auto [usable_intervals, data] = trim_pushed_data(
      pull_info.recovery_info.copy_subset, push_op.data_included, push_op.data);
  bool complete = pull_info.is_complete();
  bool clear_omap = !push_op.before_progress.omap_complete;
  co_await submit_push_data(
      pull_info.recovery_info, first, complete, clear_omap,
      std::move(data_zeros), std::move(usable_intervals), std::move(data),
      std::move(push_op.omap_header), push_op.attrset,
  ...
  if (complete) {
    ...
    auto manager = pg.obc_loader.get_obc_manager(recovery_waiter.obc);
...
void
ReplicatedRecoveryBackend::recalc_subsets(
    ObjectRecoveryInfo& recovery_info,
...
RecoveryBackend::interruptible_future<>
ReplicatedRecoveryBackend::handle_pull_response(Ref<MOSDPGPush> m)
{
  ...
  if (push_op.version == eversion_t()) {
    ...
    throw std::runtime_error(fmt::format(
        "Error on pushing side {} when pulling obj {}", m->from, push_op.soid));
  }
  ...
  const bool complete = co_await _handle_pull_response(from, push_op, &response);
...
RecoveryBackend::interruptible_future<>
ReplicatedRecoveryBackend::handle_push(Ref<MOSDPGPush> m)
{
  ...
  bool complete =
      (push_op.after_progress.data_complete &&
  ...
  if (first) {
    ...
    replica_push_targets[push_op.recovery_info.soid] =
        pg.obc_loader.create_cached_obc_from_push_data(oi, ssc);
  }

  co_await submit_push_data(
      push_op.recovery_info, first, complete, clear_omap, std::move(data_zeros),
      std::move(push_op.data_included), std::move(push_op.data),
      std::move(push_op.omap_header), push_op.attrset,
      std::move(push_op.omap_entries), &t);
  ...
  if (recovering_iter == recovering.end() ||
    ...
    if (!push_info.recovery_progress.data_complete && !error) {
      return build_push_op(
          ...
          .handle_exception_interruptible([recovering_iter, &push_info,
                                           peer](auto e) {
    ...
    if (!error) {
      pg.get_recovery_handler()->on_peer_recover(
          peer, soid, push_info.recovery_info);
    }
...
RecoveryBackend::interruptible_future<>
ReplicatedRecoveryBackend::handle_push_reply(Ref<MOSDPGPushReply> m)
{
  ...
  return _handle_push_reply(from, push_reply)
      .then_interruptible([this, from](std::optional<PushOp> push_op) {
        if (push_op) {
          auto msg = crimson::make_message<MOSDPGPush>();
          msg->from = pg.get_pg_whoami();
          msg->pgid = pg.get_pgid();
    ...
    bufferlist omap_header)
{
  ...
  if (!first) {
    co_return get_temp_recovery_object(
        recovery_info.soid, recovery_info.version);
  }
  ...
  if (complete) {
    ...
    target_oid = ghobject_t(
        get_temp_recovery_object(recovery_info.soid, recovery_info.version));
    DEBUGDPP("Adding oid {} in the temp collection", pg, target_oid);
  ...
  if (!complete || !recovery_info.object_exist) {
    ...
    t->set_alloc_hint(
        coll->get_cid(), target_oid, oi.expected_object_size,
  ...
  if (local_size) {
    ...
    local_intervals_excluded.intersection_of(
        local_intervals_included, recovery_info.copy_subset);
    local_intervals_included.subtract(local_intervals_excluded);
  }
  for (auto [off, len] : local_intervals_included) {
    DEBUGDPP("clone_range {} {}~{}", pg, recovery_info.soid, off, len);
    t->clone_range(
        coll->get_cid(), ghobject_t(recovery_info.soid), target_oid, off, len,
        off);
  }
  co_return target_oid.hobj;
}
    ...
    ObjectStore::Transaction* t)
{
  ...
  auto target_oid = co_await prep_push_target(
      recovery_info, first, complete, clear_omap, t, attrs,
  ...
  if (!data_zeros.empty()) {
    ...
    DEBUGDPP(
        "recovering object {} copy_subset: {} "
        "intervals_included: {} data_zeros: {}",
        pg, recovery_info.soid, recovery_info.copy_subset, intervals_included,
        data_zeros);
  ...
  for (auto [start, len] : intervals_included) {
    ...
    t->write(
        coll->get_cid(), ghobject_t(target_oid), start, len, bit, fadvise_flags);
    off += len;
  }
  ...
  if (complete) {
    if (!first) {
      DEBUGDPP("Removing oid {} from the temp collection", pg, target_oid);
      clear_temp_obj(target_oid);
      t->remove(coll->get_cid(), ghobject_t(recovery_info.soid));
      t->collection_move_rename(
          coll->get_cid(), ghobject_t(target_oid), coll->get_cid(),
...
void
ReplicatedRecoveryBackend::submit_push_complete(
    const ObjectRecoveryInfo& recovery_info,
    ObjectStore::Transaction* t)
{
  ...
  for (const auto& [oid, extents] : recovery_info.clone_subset) {
    for (const auto& [off, len] : extents) {
      ...
      t->clone_range(
          coll->get_cid(), ghobject_t(oid), ghobject_t(recovery_info.soid), off,
          len, off);
    }
  }
}
...
RecoveryBackend::interruptible_future<>
RecoveryBackend::handle_recovery_delete_reply(Ref<MOSDPGRecoveryDeleteReply> m)
{
    ...
    crimson::net::ConnectionXcoreRef conn)
{
  ...
  case MSG_OSD_PG_PUSH:
    if (pg.is_primary()) {
      return handle_pull_response(boost::static_pointer_cast<MOSDPGPush>(m));
    } else {
...
ReplicatedRecoveryBackend::get_md_from_push_op(PushOp& push_op)
{
  ...
  if (auto ss_attr_iter = push_op.attrset.find(SS_ATTR);
    ...
    try {
      ssc = new crimson::osd::SnapSetContext(push_op.soid.get_snapdir());
