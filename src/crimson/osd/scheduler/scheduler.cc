...
class ClassedOpQueueScheduler final : public Scheduler {
  ...
  std::array<priority_t, static_cast<size_t>(SchedulerClass::immediate)>
      priority_map = {// Placeholder, gets replaced with configured values
                      0, 0, 0};

  static SchedulerClass
  get_io_prio_cut(ConfigProxy& conf)
  {
    if (conf.get_val<std::string>("osd_op_queue_cut_off") == "debug_random") {
      srand(time(NULL));
      return (rand() % 2 < 1) ? SchedulerClass::repop
  ...
  bool
  use_strict(SchedulerClass kl) const
  {
    return static_cast<uint8_t>(kl) >= static_cast<uint8_t>(cutoff);
  }

  priority_t
  get_priority(SchedulerClass kl) const
  {
    ceph_assert(
...
public:
  ...
  ClassedOpQueueScheduler(ConfigProxy& conf, Args&&... args) :
    cutoff(get_io_prio_cut(conf)), queue(std::forward<Args>(args)...)
  {
    priority_map[static_cast<size_t>(SchedulerClass::background_best_effort)] =
        conf.get_val<uint64_t>("osd_scrub_priority");
    priority_map[static_cast<size_t>(SchedulerClass::background_recovery)] =
        conf.get_val<uint64_t>("osd_recovery_op_priority");
    priority_map[static_cast<size_t>(SchedulerClass::client)] =
        conf.get_val<uint64_t>("osd_client_op_priority");
    priority_map[static_cast<size_t>(SchedulerClass::repop)] =
        conf.get_val<uint64_t>("osd_client_op_priority");
  }

  void
  enqueue(item_t&& item) final
  {
    if (use_strict(item.params.klass))
      ...
      queue.enqueue(
          item.params.owner, get_priority(item.params.klass), item.params.cost,
          std::move(item));
  }

  void
  enqueue_front(item_t&& item) final
  {
    if (use_strict(item.params.klass))
    ...
    bool perf_cnt)
{
  ...
  if (*type == "wpq") {
    ...
    return std::make_unique<
        ...
        conf, conf.get_val<uint64_t>("osd_op_pq_max_tokens_per_priority"),
        conf->osd_op_pq_min_cost);
  } else if (*type == "mclock_scheduler") {
    return std::make_unique<mClockScheduler>(
        cct, whoami, nshards, sid, is_rotational, perf_cnt);
  } else {
...
std::ostream&
operator<<(std::ostream& lhs, const Scheduler& rhs)
{
  rhs.print(lhs);
  return lhs;
}
