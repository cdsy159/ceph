...
void
PGScrubber::dump_detail(Formatter* f) const
{
  f->dump_stream("pgid") << pg.get_pgid();
}

PGScrubber::PGScrubber(PG& pg) :
  pg(pg), dpp(pg), machine(*this)
{}
...
void
PGScrubber::on_log_update(eversion_t v)
{
  LOG_PREFIX(PGScrubber::on_interval_change);
...
void
PGScrubber::handle_scrub_requested(bool deep)
{
...
void
PGScrubber::handle_scrub_message(Message& _m)
{
  LOG_PREFIX(PGScrubber::handle_scrub_requested);
  switch (_m.get_type()) {
  case MSG_OSD_REP_SCRUB: {
    ...
    handle_event(events::replica_scan_t{m.start, m.end, m.scrub_from, m.deep});
    break;
  }
  case MSG_OSD_REP_SCRUBMAP: {
    ...
    ::decode(map, iter);
    handle_event(scan_range_complete_t{m.from, std::move(map)});
    break;
  }
  default:
    ...
    const hobject_t& hoid)
{
  ...
  if (blocked && (hoid >= blocked->begin) && (hoid < blocked->end)) {
    ...
    return trigger.maybe_record_blocking(blocked->p.get_shared_future(), *this);
  } else {
    return seastar::now();
  }
}
...
void
PGScrubber::notify_scrub_start(bool deep)
{
...
void
PGScrubber::notify_scrub_end(bool deep)
{
...
PGScrubber::get_ids_to_scrub() const
{
  return pg.peering_state.get_actingset();
}

chunk_validation_policy_t
PGScrubber::get_policy() const
{
...
void
PGScrubber::request_range(const hobject_t& start)
{
  LOG_PREFIX(PGScrubber::request_range);
  DEBUGDPP("start: {}", pg, start);
  std::ignore = pg.shard_services
                    .start_operation_may_interrupt<interruptor, ScrubFindRange>(
                        start, &pg);
}
...
void
PGScrubber::reserve_range(const hobject_t& start, const hobject_t& end)
{
  LOG_PREFIX(PGScrubber::reserve_range);
  DEBUGDPP("start: {}, end: {}", pg, start, end);
  std::ignore =
      pg.shard_services
          .start_operation_may_interrupt<interruptor, ScrubReserveRange>(
              start, end, &pg);
}

void
PGScrubber::release_range()
{
    ...
    const hobject_t& end)
{
  ...
  DEBUGDPP(
      "target: {}, version: {}, deep: {}, start: {}, end: {}", pg, target,
      version, deep, start, end);
  if (target == pg.get_pg_whoami()) {
    std::ignore =
        pg.shard_services.start_operation_may_interrupt<interruptor, ScrubScan>(
            &pg, deep, true /* local */, start, end);
  } else {
    std::ignore = pg.shard_services.send_to_osd(
        target.osd,
        crimson::make_message<MOSDRepScrub>(
            spg_t(pg.get_pgid().pgid, target.shard), version,
            pg.get_osdmap_epoch(), pg.get_osdmap_epoch(), start, end, deep,
            false /* allow preemption -- irrelevant for replicas TODO */,
            64 /* priority, TODO */, false /* high_priority TODO */),
        pg.get_osdmap_epoch());
  }
}

bool
PGScrubber::await_update(const eversion_t& version)
{
...
void
PGScrubber::generate_and_submit_chunk_result(
...
void
PGScrubber::emit_scrub_result(bool deep, object_stat_sum_t in_stats)
{
  LOG_PREFIX(PGScrubber::emit_scrub_result);
  DEBUGDPP("", pg);
  pg.peering_state.update_stats([this, FNAME, deep,
                                 &in_stats](auto& history, auto& pg_stats) {
    ...
    foreach_scrub_checked_stat([this, FNAME, &pg_stats, &in_stats](
                                   const auto& name, auto statptr,
                                   const auto& invalid_predicate) {
