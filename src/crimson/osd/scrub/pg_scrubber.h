...
class PGScrubber : public crimson::BlockerT<PGScrubber>, ScrubContext {
  ...
  using ifut = ::crimson::interruptible::
...
public:
  ...
  void handle_op_stats(const hobject_t& on_object, object_stat_sum_t delta_stats);
...
private:
  ...
  void emit_scrub_result(bool deep, object_stat_sum_t scrub_stats) final;
};
...
struct fmt::formatter<crimson::osd::scrub::blocked_range_t> {
  ...
  auto
  format(const auto& range, FormatContext& ctx) const
  {
