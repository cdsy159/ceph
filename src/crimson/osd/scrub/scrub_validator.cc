...
object_set_t
get_object_set(const scrub_map_set_t& in)
{
  object_set_t ret;
  for (const auto& [from, map] : in) {
    std::transform(
        map.objects.begin(), map.objects.end(), std::inserter(ret, ret.end()),
...
struct shard_evaluation_t {
  ...
  std::weak_ordering
  operator<=>(const shard_evaluation_t& rhs) const
  {
...
librados::obj_err_t
compare_candidate_to_authoritative(
    const chunk_validation_policy_t& policy,
    const hobject_t& oid,
    ...
    const shard_evaluation_t& cand)
{
  ...
  for (auto aiter = auth_si.attrs.begin(); aiter != auth_si.attrs.end();
       ++aiter) {
    if (is_sys_attr(aiter->first))
      continue;
    ...
    const scrub_map_set_t& maps)
{
  ...
  std::transform(
      maps.begin(), maps.end(), std::inserter(shards, shards.end()),
      [&hoid, &policy](const auto& item) -> evaluation_vec_t::value_type {
        ...
        auto maybe_shard = miter == scrub_map.objects.end() ? nullptr
                                                            : &(miter->second);
  ...
  if (iow.errors || std::any_of(shards.begin(), shards.end(), [](auto& cand) {
        return cand.has_errors();
      })) {
    for (auto& eval : shards) {
      iow.shards.emplace(
          librados::osd_shard_t{
              eval.source.osd, static_cast<int8_t>(eval.source.shard)},
          eval.shard_info);
...
std::optional<inconsistent_snapset_wrapper>
evaluate_snapset(
    ...
    const clone_meta_list_t& clones)
{
  ...
  for (auto ss_clone_id : snapset.clones) {
    for (; clone_iter != clones.end() && clone_iter->first.snap < ss_clone_id;
         ++clone_iter) {
      ret.set_clone(clone_iter->first.snap);
    }

    if (clone_iter != clones.end() && clone_iter->first.snap == ss_clone_id) {
  ...
  if (ret.errors) {
    DEBUGDPP("snapset {}, clones {}", dpp, snapset, clones);
    return ret;
  } else {
    return std::nullopt;
  }
    ...
    const scrub_map_set_t& in)
{
  ...
  while (heads.size() || clones.size()) {
    ...
    const hobject_t& next_clone = clones.size() ? clones.front().first
                                                : max_oid;
    ...
    if (auto result = evaluate_snapset(
            dpp, head_to_process, head_meta, clones_to_process);
        result) {
