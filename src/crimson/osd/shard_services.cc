...
PerShardState::PerShardState(
  ...
  ec_extent_cache_lru(
      crimson::common::local_conf().get_val<uint64_t>("ec_extent_cache_size"))
{}

seastar::future<>
PerShardState::dump_ops_in_flight(Formatter* f) const
{
...
{
  assert_core();
  return seastar::parallel_for_each(pg_map.get_pgs(), [](auto& p) {
    return p.second->stop();
  });
}

std::map<pg_t, pg_stat_t>
PerShardState::get_pg_stats()
{
  assert_core();
...
seastar::future<>
PerShardState::broadcast_map_to_pgs(ShardServices& shard_services, epoch_t epoch)
{
  assert_core();
  auto& pgs = pg_map.get_pgs();
  return seastar::parallel_for_each(
      pgs.begin(), pgs.end(), [&shard_services, epoch](auto& pg) {
        return shard_services
            .start_operation<PGAdvanceMap>(
                pg.second, shard_services, epoch, PeeringCtx{}, false)
            .second;
      });
}
...
seastar::future<>
PerShardState::update_shard_superblock(OSDSuperblock superblock)
{
  assert_core();
...
seastar::future<>
OSDSingletonState::send_to_osd(int peer, MessageURef m, epoch_t from_epoch)
{
  ...
  if (osdmap->is_down(peer)) {
    ...
    INFO("osd.{} {} > {}", peer, osdmap->get_info(peer).up_from, from_epoch);
    return seastar::now();
  } else {
...
seastar::future<>
OSDSingletonState::osdmap_subscribe(version_t epoch, bool force_request)
{
...
void
OSDSingletonState::queue_want_pg_temp(
    pg_t pgid,
...
void
OSDSingletonState::remove_want_pg_temp(pg_t pgid)
{
...
void
OSDSingletonState::requeue_pg_temp()
{
  ...
  DEBUG("{} + {} -> {}", old_wanted, old_pending, pg_temp_wanted.size());
}
...
OSDSingletonState::send_pg_temp()
{
  ...
  return seastar::parallel_for_each(
      std::begin(ms), std::end(ms), [this](auto& m) {
        if (m) {
...
std::ostream&
operator<<(std::ostream& out, const OSDSingletonState::pg_temp_t& pg_temp)
{
  out << pg_temp.acting;
  if (pg_temp.forced) {
    out << " (forced)";
...
OSDSingletonState::send_pg_created()
{
  ...
  auto o = get_osdmap();
  ceph_assert(o->require_osd_release >= ceph_release_t::luminous);
  return seastar::parallel_for_each(pg_created, [this](auto& pgid) {
    return monc.send_message(crimson::make_message<MOSDPGCreated>(pgid));
  });
}

void
OSDSingletonState::prune_pg_created()
{
...
OSDSingletonState::get_tracked_keys() const noexcept
{
  return {
...
void
OSDSingletonState::handle_conf_change(
...
void
OSDSingletonState::store_map_bl(
...
void
OSDSingletonState::store_inc_map_bl(
    ceph::os::Transaction& t,
    epoch_t e,
    bufferlist&& bl)
{
...
OSDSingletonState::load_map_bl(epoch_t e)
{
...
OSDSingletonState::load_inc_map_bl(epoch_t e)
{
  ...
  if (std::optional<bufferlist> found = inc_map_bl_cache.find(e); found) {
    ...
    return meta_coll->load_inc_map(e).safe_then(
        [this, e](auto&& bl) {
...
OSDSingletonState::load_map_bls(epoch_t first, epoch_t last)
{
  LOG_PREFIX(OSDSingletonState::load_map_bl);
  ...
  return seastar::map_reduce(
      ...
      [this, FNAME](epoch_t e) {
        return load_inc_map_bl(e)
            .safe_then(
                [](auto&& bl) {
                  return seastar::make_ready_future<OSDMapService::bls_pair>(
                      std::make_pair(
                          OSDMapService::encoded_osdmap_type_t::INCMAP,
                          std::move(bl)));
                },
                read_errorator::all_same_way([this, FNAME, e] {
                  DEBUG(
                      "can't load inc map {}, attempting full map instread", e);
                  return load_map_bl(e).then([](auto&& bl) {
                    return seastar::make_ready_future<OSDMapService::bls_pair>(
                        std::make_pair(
                            OSDMapService::encoded_osdmap_type_t::FULLMAP,
                            std::move(bl)));
                  });
                }))
...
seastar::future<std::unique_ptr<OSDMap>>
        ...
        return seastar::do_for_each(
                   ...
                   [&t, FNAME, m, this, &added_maps](epoch_t e) {
                     if (auto p = m->maps.find(e); p != m->maps.end()) {
                       ...
                       INFO(
                           "found osdmap.{} incremental map, loading osdmap.{}",
                           e, e - 1);
                       ceph_assert(std::cmp_greater(e, 0u));
                       return load_map(e - 1).then([&added_maps, FNAME, e,
                                                    bl = p->second, &t,
                                                    this](auto o) mutable {
                         ...
                         bufferlist fbl;
                         o->encode(
                             fbl, inc.encode_features | CEPH_FEATURE_RESERVED);
            ...
            .then([&t, FNAME, this, &added_maps] {
              ...
              return last_map_fut.then([&t, FNAME, last_map_epoch, this,
                                        &added_maps](auto lastmap) {
                INFO(
                    "storing final pool info lastmap epoch {}, added maps "
                    "{}->{}",
                    last_map_epoch, added_maps.begin()->first,
...
void
OSDSingletonState::trim_maps(ceph::os::Transaction& t, OSDSuperblock& superblock)
{
  ...
  while (superblock.get_oldest_map() < min &&
         t.get_num_ops() <
             crimson::common::local_conf()->osd_target_transaction_size) {
    ...
    bool do_create)
{
  ...
  auto get_pool_info_for_pg = [create_map, pgid, this] {
    if (create_map->have_pg_pool(pgid.pool())) {
      ...
      return seastar::make_ready_future<
          std::tuple<pg_pool_t, std::string, ec_profile_t>>(std::make_tuple(
  ...
  auto get_collection = [pgid, do_create, store_index, this] {
    ...
    if (do_create) {
      return crimson::os::with_store<
          &crimson::os::FuturizedStore::Shard::create_new_collection>(
  ...
  return seastar::when_all(
      ...
      .then([pgid, create_map, store_index, this](auto&& ret) {
        ...
        return seastar::make_ready_future<Ref<PG>>(new PG{
            pgid, pg_shard_t{local_state.whoami, pgid.shard},
            std::move(store_index), std::move(coll), std::move(pool),
            std::move(name), create_map, *this, ec_profile});
      });
}
    ...
    std::unique_ptr<PGCreateInfo> info)
{
  return seastar::do_with(
      std::move(info),
      [store_index, this](auto& info) -> seastar::future<Ref<PG>> {
        return get_map(info->epoch)
            .then(
                [&info, store_index, this](cached_map_t startmap)
                  ...
                  if (!get_map()->is_up_acting_osd_shard(
                          pgid, local_state.whoami) ||
                      !startmap->is_up_acting_osd_shard(
                          pgid, local_state.whoami)) {
                  ...
                  if (info->by_mon) {
                    ...
                    if (!pool->has_flag(pg_pool_t::FLAG_CREATING)) {
                      ...
                      DEBUG(
                          "dropping {} create, pool does not have CREATING "
                          "flag set",
                          pgid);
                      local_state.pg_map.pg_creation_canceled(pgid);
                      return seastar::make_ready_future<
                  ...
                  return make_pg(startmap, pgid, store_index, true)
            ...
            .then([this, &info](auto&& ret) -> seastar::future<Ref<PG>> {
              ...
              int role = startmap->calc_pg_role(
                  pg_shard_t(local_state.whoami, info->pgid.shard), acting);

              std::unique_ptr<PeeringCtx> rctx = std::make_unique<PeeringCtx>();
              create_pg_collection(
                  rctx->transaction, info->pgid,
              ...
              return pg
                  ->init(
                      ...
                      info->history, info->past_intervals, rctx->transaction)
                  .then([this, pg = pg, rctx = std::move(rctx)] {
                    return start_operation<PGAdvanceMap>(
                               pg, *this, get_map()->get_epoch(),
                               std::move(*rctx), true)
                        .second.then([pg = pg] {
    ...
    std::unique_ptr<PGCreateInfo> info)
{
  if (info) {
    auto [fut, existed] =
        local_state.pg_map.wait_for_pg(std::move(trigger), pgid);
    if (!existed) {
      local_state.pg_map.set_creating(pgid);
      (void)handle_pg_create_info(store_index, std::move(info));
    }
    return std::move(fut);
  } else {
...
ShardServices::create_split_pg(
    ...
    spg_t pgid)
{
  auto [fut, existed] = local_state.pg_map.wait_for_pg(std::move(trigger), pgid);
  if (!existed) {
    local_state.pg_map.set_creating(pgid);
  }
  return std::move(fut);
...
seastar::future<Ref<PG>>
ShardServices::load_pg(spg_t pgid, store_index_t store_index)
{
  LOG_PREFIX(OSDSingletonState::load_pg);
  DEBUG("{}", pgid);

  return seastar::do_with(
             PGMeta(get_store(store_index), pgid),
             [](auto& pg_meta) { return pg_meta.get_epoch(); })
    ...
    store_index_t store_index)
{
  ...
  if (ctx.transaction.empty()) {
    ...
    co_await crimson::os::with_store_do_transaction(
        get_store(store_index), col, ceph::os::Transaction{});
    Context* on_commit(
  ...
  co_await crimson::os::with_store_do_transaction(
      get_store(store_index), col, ctx.transaction.claim_and_reset());
  co_return;
}

Ref<PG>
ShardServices::get_pg(spg_t pgid)
{
  return local_state.get_pg(pgid);
}

seastar::future<>
ShardServices::dispatch_context_messages(BufferedRecoveryMessages&& ctx)
{
  LOG_PREFIX(OSDSingletonState::dispatch_context_messages);
  auto ret = seastar::parallel_for_each(
      std::move(ctx.message_map), [FNAME, this](auto& osd_messages) {
        auto& [peer, messages] = osd_messages;
        DEBUG("sending messages to {}", peer);
        return seastar::parallel_for_each(
            std::move(messages), [=, peer = peer, this](auto& m) {
              return send_to_osd(
                  peer, std::move(m), local_state.osdmap->get_epoch());
            });
      });
...
ShardServices::dispatch_context(
    ...
    PeeringCtx&& pctx)
{
  return seastar::do_with(std::move(pctx), [this, col, store_index](auto& ctx) {
    ceph_assert(col || ctx.transaction.empty());
    return seastar::when_all_succeed(
               dispatch_context_messages(BufferedRecoveryMessages{ctx}),
               col ? dispatch_context_transaction(col, ctx, store_index)
                   : seastar::now())
        .then_unpack([] { return seastar::now(); });
  });
}

seastar::future<MURef<MOSDMap>>
OSDSingletonState::build_incremental_map_msg(epoch_t first, epoch_t last)
{
  LOG_PREFIX(OSDSingletonState::build_incremental_map_msg);
  return seastar::do_with(
      crimson::common::local_conf()->osd_map_message_max,
      crimson::make_message<MOSDMap>(
          monc.get_fsid(), osdmap->get_encoding_features()),
      [this, &first, FNAME, last](auto& map_message_max, auto& m) {
        ...
        if (first < superblock.cluster_osdmap_trim_lower_bound) {
          INFO(
              "cluster osdmap lower bound: {}  > first {}, starting with full "
              "map",
        ...
        return maybe_handle_mapgap.then([this, first, last, &map_message_max,
                                         &m] {
          if (first > last) {
          ...
          return load_map_bls(
                     first, ((last - first) > map_message_max)
                                ? (first + map_message_max)
                                : last)
              .then([&m](auto&& bls) {
                ssize_t map_message_max_bytes =
                    crimson::common::local_conf()->osd_map_message_max_bytes;
    ...
    epoch_t first)
{
  ...
  INFO(
      "first osdmap: {} superblock's oldest map: {}, to {}", first,
      superblock.get_oldest_map(), to);
  if (to > first &&
      (int64_t)(to - first) >
          crimson::common::local_conf()->osd_map_share_max_epochs) {
    DEBUG(
        "{} > max epochs to send of {}, only sending most recent,",
...
seastar::future<>
OSDSingletonState::send_incremental_map_to_osd(int osd, epoch_t first)
{
  LOG_PREFIX(OSDSingletonState::send_incremental_map);
...
void
OSDSingletonState::update_osd_stat(
    epoch_t up_epoch,
    ...
    const store_statfs_t& st)
{
  ...
  osd_stat.statfs = st;
  osd_stat.seq = (static_cast<uint64_t>(up_epoch) << 32) | osd_stat_seq;
}
