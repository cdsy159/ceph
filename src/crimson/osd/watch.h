...
public:
  ...
  template <class... Args>
  static seastar::shared_ptr<Watch>
  create(Args&&... args)
  {
    return seastar::make_shared<Watch>(
        private_ctag_t{}, std::forward<Args>(args)...);
  };

  uint64_t
  get_watcher_gid() const
  {
...
class Notify : public seastar::enable_shared_from_this<Notify> {
  ...
  friend bool
  operator<(const ptr_t& ptr, const uint64_t id)
  {
    assert(ptr);
  ...
  friend bool
  operator<(const uint64_t id, const ptr_t& ptr)
  {
    assert(ptr);
...
public:
  ...
  seastar::future<> complete_watcher(
      WatchRef watch,
    ...
    Args&&... args)
{
  static_assert(std::is_same_v<
                typename std::iterator_traits<WatchIteratorT>::value_type,
  ...
  if (begin == end) {
    ...
    auto notify = seastar::make_shared<Notify>(
        private_ctag_t{}, begin, end, std::forward<Args>(args)...);
...
template <>
struct fmt::formatter<crimson::osd::notify_reply_t> : fmt::ostream_formatter {};
#endif
