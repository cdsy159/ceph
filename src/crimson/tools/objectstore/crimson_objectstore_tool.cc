...
std::string
cleanbin_tmp(std::string_view str_view)
{
...
to_string(operation_type_t op)
{
  ...
  case operation_type_t::CLEAR_DATA_DIGEST:
    return "clear-data-digest";
  default:
    return "unknown";
  }
}

tl::expected<operation_type_t, std::string>
parse_pg_operation(const std::string& op_str)
{
  if (op_str == "list-pgs")
    return operation_type_t::LIST_PGS;
  if (op_str == "list")
    return operation_type_t::LIST_OBJECTS;
  if (op_str == "info")
    return operation_type_t::INFO;
  return tl::unexpected("Unsupported PG operation: " + op_str);
}

tl::expected<operation_type_t, std::string>
parse_object_operation(const std::string& objcmd)
{
  if (objcmd == "get-bytes")
    return operation_type_t::GET_BYTES;
  if (objcmd == "set-bytes")
    return operation_type_t::SET_BYTES;
  if (objcmd == "get-attr")
    return operation_type_t::GET_ATTR;
  if (objcmd == "set-attr")
    return operation_type_t::SET_ATTR;
  if (objcmd == "rm-attr")
    return operation_type_t::RM_ATTR;
  if (objcmd == "list-attrs")
    return operation_type_t::LIST_ATTRS;
  if (objcmd == "get-omap")
    return operation_type_t::GET_OMAP;
  if (objcmd == "set-omap")
    return operation_type_t::SET_OMAP;
  if (objcmd == "rm-omap")
    return operation_type_t::RM_OMAP;
  if (objcmd == "list-omap")
    return operation_type_t::LIST_OMAP;
  if (objcmd == "remove")
    return operation_type_t::REMOVE;
  if (objcmd == "removeall")
    return operation_type_t::REMOVEALL;
  if (objcmd == "dump")
    return operation_type_t::DUMP;
  if (objcmd == "set-size")
    return operation_type_t::SET_SIZE;
  if (objcmd == "clear-data-digest")
    return operation_type_t::CLEAR_DATA_DIGEST;
...
struct objectstore_config_t {
  ...
  void
  populate_options(bpo::options_description& desc)
  {
    desc.add_options()("help", "produce help message")(
        ...
        "device-type",
        bpo::value<std::string>(&device_type)->default_value("SSD"),
        ...
        "CAUTION")(
        "tty", bpo::bool_switch(&tty), "Treat stdout as a tty (no binary data)")(
        "namespace", bpo::value<std::string>(&namespace_),
        "Specify namespace when searching for objects");
  }
};

tl::expected<pg_t, std::string>
parse_pgid(const std::string& pgid_str)
{
  pg_t pgid;
...
std::string
get_pgid_str_from_coll(const coll_t& coll)
{
...
public:
  ...
  seastar::future<>
  call(const coll_t& coll, seastar::shard_id shard_id, const ghobject_t& ghobj)
      override
  {
...
static seastar::future<int>
action_on_all_objects(
    StoreTool& st,
    ObjectAction& action,
...
static seastar::future<bool>
find_shard_for_object(
    StoreTool& st,
    objectstore_config_t& config,
    const std::string& object_type)
{
  auto pgs = co_await st.list_pgs();
  auto it = std::find_if(pgs.begin(), pgs.end(), [&config](const auto& pg) {
    return pg.first == config.coll;
  });
  if (it == pgs.end()) {
    fmt::println(
        std::cerr, "PG '{}' not found for {} object", config.coll, object_type);
    co_return false;
  }
...
static seastar::future<bool>
resolve_operation_parameters(StoreTool& st, objectstore_config_t& config)
{
  ...
  if (!op.object.has_value() || op.object->empty()) {
    ...
    if (is_json) {
      ...
      if (!op.pgid.has_value()) {
        fmt::println(
            std::cerr, "PG ID is required when object is specified as JSON");
        co_return false;
      }
      ...
      if (lookup.objects.empty()) {
        fmt::println(
            std::cerr,
            "Object '{}' not found. If this object is in a non-default "
            "namespace, please specify it with --namespace.",
            *op.object);
        co_return false;
      }
      if (lookup.objects.size() > 1) {
        fmt::println(
            std::cerr,
            "Found {} objects with name '{}', please specify pgid or use JSON "
            "format",
...
void
print_usage(const bpo::options_description& desc)
{
  std::cout << std::endl;
  ...
  std::cout << std::endl;
  std::cout << "crimson-objectstore-tool ... <object> (get|set)-bytes [file]"
            << std::endl;
  std::cout
      << "crimson-objectstore-tool ... <object> set-(attr|omap) <key> [file]"
      << std::endl;
  std::cout
      << "crimson-objectstore-tool ... <object> (get|rm)-(attr|omap) <key>"
      << std::endl;
  std::cout << "crimson-objectstore-tool ... <object> list-attrs" << std::endl;
  std::cout << "crimson-objectstore-tool ... <object> list-omap" << std::endl;
  std::cout << "crimson-objectstore-tool ... <object> remove|removeall"
            << std::endl;
  std::cout << "crimson-objectstore-tool ... <object> dump" << std::endl;
  std::cout << "crimson-objectstore-tool ... <object> set-size" << std::endl;
  std::cout << "crimson-objectstore-tool ... <object> clear-data-digest"
            << std::endl;
  std::cout << std::endl;
  std::cout << "<object> can be a JSON object description as displayed"
            << std::endl;
  std::cout << "by --op list." << std::endl;
  std::cout << "<object> can be an object name which will be looked up in all"
            << std::endl;
  ...
  std::cout << std::endl;
  std::cout << "The optional [file] argument will read stdin or write stdout"
            << std::endl;
...
private:
  ...
  size_t
  get_filesystem_block_size(const std::string& path)
  {
    struct stat st;
...
public:
  ...
  tl::expected<crimson::os::seastore::block_sm_superblock_t, std::string>
  load_seastore_superblock()
  {
    try {
      ...
      if (m_device_type != "RANDOM_BLOCK_SSD") {
        ...
        constexpr const char SEASTORE_SUPERBLOCK_SIGN[] =
            "seastore block device\n";
      ...
      ceph_assert(
          ceph::encoded_sizeof<crimson::os::seastore::block_sm_superblock_t>(
      ...
      return tl::unexpected(
          "Could not read seastore superblock: " + std::string(e.what()));
    } catch (...) {
      return tl::unexpected(
          "Could not read seastore superblock: unknown error");
    }
  }

  tl::expected<unsigned int, std::string>
  get_shard_count()
  {
...
{
  auto shard_count_result = read_shard_count_from_storage(
      config.data_path, config.type, config.device_type);
...
seastar::future<int>
write_output(const std::optional<std::string>& file_path, const std::string& data)
{
...
read_input(const std::optional<std::string>& file_path)
{
  ...
  if (file_path.has_value() && *file_path != "-") {
    ...
    if (!infile.is_open()) {
      fmt::println(
          std::cerr, "failed to open input-file '{}'", file_path.value());
...
seastar::future<int>
run_tool(StoreTool& st, objectstore_config_t& config)
{
  ...
  case operation_type_t::SET_OMAP: {
    ...
    bool success = co_await st.set_omap(
        config.coll, config.ghobj, op.key.value(), *omap_value_result);
    if (success) {
      fmt::println(
          std::cout, "set omap success: key={}, value size={}", op.key.value(),
          omap_value_result->size());
    } else {
  ...
  case operation_type_t::GET_OMAP: {
    ...
    std::string omap_value =
        co_await st.get_omap(config.coll, config.ghobj, op.key.value());
  ...
  case operation_type_t::RM_OMAP: {
    ...
    bool success =
        co_await st.remove_omap(config.coll, config.ghobj, op.key.value());
    if (success) {
      fmt::println(std::cout, "remove omap success: key={}", op.key.value());
    } else {
  ...
  case operation_type_t::LIST_ATTRS: {
    ...
    if (!attrs_result) {
      fmt::println(
          std::cerr, "Error listing attributes: {}", attrs_result.error());
      co_return EXIT_FAILURE;
    }
  ...
  case operation_type_t::SET_ATTR: {
    ...
    bool success = co_await st.set_attr(
        config.coll, config.ghobj, op.key.value(), *attr_value_result);
    if (success) {
      fmt::println(
          std::cout, "set attr success: key={}, value size={}", op.key.value(),
          attr_value_result->size());
    } else {
  ...
  case operation_type_t::GET_ATTR: {
    ...
    auto attr_result =
        co_await st.get_attr(config.coll, config.ghobj, op.key.value());

    if (!attr_result) {
      fmt::println(
          std::cerr, "Error reading attribute '{}': {}", op.key.value(),
          attr_result.error());
      co_return EXIT_FAILURE;
    }
  ...
  case operation_type_t::RM_ATTR: {
    ...
    bool success =
        co_await st.remove_attr(config.coll, config.ghobj, op.key.value());
    if (success) {
      fmt::println(std::cout, "remove attr success: key={}", op.key.value());
    } else {
  ...
  case operation_type_t::SET_BYTES: {
    ...
    bool success =
        co_await st.set_bytes(config.coll, config.ghobj, *object_data_result);
    if (success) {
      fmt::println(
          std::cout, "set bytes success: data size={}",
  ...
  case operation_type_t::REMOVE: {
    bool success =
        co_await st.remove_object(config.coll, config.ghobj, false, op.force);
    if (success) {
  ...
  case operation_type_t::REMOVEALL: {
    bool success =
        co_await st.remove_object(config.coll, config.ghobj, true, op.force);
    if (success) {
  ...
  case operation_type_t::DUMP: {
    std::string dump_info =
        co_await st.dump_object_info(config.coll, config.ghobj);
  ...
  case operation_type_t::SET_SIZE: {
    ...
    bool success =
        co_await st.set_object_size(config.coll, config.ghobj, op.size.value());
    if (success) {
      fmt::println(
          std::cout, "set object size success: size={}", op.size.value());
    } else {
  ...
  default:
    fmt::println(
        std::cerr, "Operation {} not implemented yet", to_string(op.op));
    co_return EXIT_FAILURE;
  }
...
main(int argc, const char* argv[])
{
  ...
  positional.add_options()(
      ...
      "command [(get|set)-bytes, (get|set|rm)-(attr|omap), list-attrs, "
      "list-omap, remove, removeall, dump, set-size, clear-data-digest]")(
  ...
  if (vm.count("objcmd")) {
    ...
    case operation_type_t::SET_ATTR:
      ...
      if (vm.count("arg2")) {
        if (params.file.has_value()) {
          fmt::println(
              std::cerr,
              "File specified both with --file and positionally, which is "
              "ambiguous");
          return EXIT_FAILURE;
        }
    ...
    config.operation = operation_params_t{*op_result,   pgid,
                                          std::nullopt, std::nullopt,
                                          std::nullopt, std::nullopt,
                                          std::nullopt, config.force};
  } else {
  ...
  try {
      ...
      return seastar::async([&] {
        try {
          ...
          auto store = crimson::os::FuturizedStore::create(
              config.type, config.data_path, local_conf().get_config_values());
          store->start().get();
          store->mount()
              .handle_error(crimson::stateful_ec::assert_failure(
                  fmt::format(
                      "error mounting object store in {}", config.data_path)
                      .c_str()))
              .get();
          ...
          fmt::println(
              std::cerr, "startup failed: {}", std::current_exception());
          return EXIT_FAILURE;
        }
      });
    });
    ...
    fmt::println(
        std::cerr, "FATAL: Exception during startup, aborting: {}",
