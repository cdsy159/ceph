...
seastar::future<std::vector<crimson::os::coll_core_t>>
StoreTool::list_pgs()
{
...
StoreTool::list_objects(const coll_t& cid, ghobject_t next)
{
  return seastar::smp::submit_to(
      ...
      [this, cid, next]()
          -> seastar::future<std::tuple<std::vector<ghobject_t>, ghobject_t>> {
        auto coll = co_await store->get_sharded_store()
                        .open_collection(cid)
                        .handle_exception([](std::exception_ptr) {
                          return seastar::make_ready_future<
                              FuturizedStore::Shard::CollectionRef>(nullptr);
                        });
        if (!coll) {
          fmt::println(
              std::cerr,
              "Failed to open collection: collection does not exist");
...
{
  return seastar::smp::submit_to(
      shard_id, [this, cid, oid, start, callback]() -> seastar::future<> {
        auto coll = co_await store->get_sharded_store()
                        .open_collection(cid)
                        .handle_exception([](std::exception_ptr) {
                          return seastar::make_ready_future<
        ...
        if (start) {
          start_from = ObjectStore::omap_iter_seek_t{
              start.value(), ObjectStore::omap_iter_seek_t::UPPER_BOUND};
        }
        co_await store->get_sharded_store()
            .omap_iterate(coll, oid, start_from, callback)
            .safe_then([](auto ret) {
...
{
  return seastar::smp::submit_to(
      shard_id, [this, cid, oid, key]() -> seastar::future<std::string> {
        auto coll = co_await store->get_sharded_store()
                        .open_collection(cid)
                        .handle_exception([](std::exception_ptr) {
                          return seastar::make_ready_future<
        ...
        to_get.insert(key);
        auto&& vals = co_await store->get_sharded_store()
                          .omap_get_values(coll, oid, to_get)
                          .handle_error(crimson::os::FuturizedStore::Shard::
                                            read_errorator::assert_all{});
...
{
  return seastar::smp::submit_to(
      shard_id, [this, cid, oid, key, value]() mutable -> seastar::future<bool> {
        auto coll = co_await store->get_sharded_store()
                        .open_collection(cid)
                        .handle_exception([](std::exception_ptr) {
                          return seastar::make_ready_future<
...
{
  return seastar::smp::submit_to(
      shard_id, [this, cid, oid, key]() -> seastar::future<bool> {
        auto coll = co_await store->get_sharded_store()
                        .open_collection(cid)
                        .handle_exception([](std::exception_ptr) {
                          return seastar::make_ready_future<
...
seastar::future<std::string>
StoreTool::get_bytes(const coll_t& cid, const ghobject_t& oid)
{
  return seastar::smp::submit_to(
      shard_id, [this, cid, oid]() -> seastar::future<std::string> {
        auto coll = co_await store->get_sharded_store()
                        .open_collection(cid)
                        .handle_exception([](std::exception_ptr) {
                          return seastar::make_ready_future<
...
{
  return seastar::smp::submit_to(
      shard_id, [this, cid, oid, data]() -> seastar::future<bool> {
        auto coll = co_await store->get_sharded_store()
                        .open_collection(cid)
                        .handle_exception([](std::exception_ptr) {
                          return seastar::make_ready_future<
...
seastar::future<tl::expected<FuturizedStore::Shard::attrs_t, std::string>>
StoreTool::get_attrs(const coll_t& cid, const ghobject_t& oid)
{
  return seastar::smp::submit_to(
      shard_id,
      [this, cid, oid]()
          -> seastar::future<
              tl::expected<FuturizedStore::Shard::attrs_t, std::string>> {
        auto coll = co_await store->get_sharded_store()
                        .open_collection(cid)
                        .handle_exception([](std::exception_ptr) {
                          return seastar::make_ready_future<
        ...
        if (!coll) {
          co_return tl::make_unexpected(std::string(
              "Failed to open collection: collection does not exist"));
        }
        ...
        co_return co_await store->get_sharded_store()
            ...
            .safe_then(
                [](attrs_t&& a) { return ret_t{tl::in_place, std::move(a)}; },
...
seastar::future<tl::expected<std::string, std::string>>
StoreTool::get_attr(
    const coll_t& cid,
    const ghobject_t& oid,
    const std::string& key)
{
  return seastar::smp::submit_to(
      shard_id,
      [this, cid, oid,
       key]() -> seastar::future<tl::expected<std::string, std::string>> {
        auto coll = co_await store->get_sharded_store()
                        .open_collection(cid)
                        .handle_exception([](std::exception_ptr) {
                          return seastar::make_ready_future<
        ...
        co_return co_await store->get_sharded_store()
            .get_attr(coll, oid, key)
            .safe_then(
                [](auto&& bl) { return return_type{tl::in_place, bl.to_str()}; },
                FuturizedStore::Shard::get_attr_errorator::all_same_way(
                    [](const std::error_code& e) {
                      return return_type{
                          tl::unexpected<std::string>(e.message())};
                    }));
      });
}
...
{
  return seastar::smp::submit_to(
      shard_id, [this, cid, oid, key, value]() -> seastar::future<bool> {
        auto coll = co_await store->get_sharded_store()
                        .open_collection(cid)
                        .handle_exception([](std::exception_ptr) {
                          return seastar::make_ready_future<
...
{
  return seastar::smp::submit_to(
      shard_id, [this, cid, oid, all, force]() -> seastar::future<bool> {
        auto coll = co_await store->get_sharded_store()
                        .open_collection(cid)
                        .handle_exception([](std::exception_ptr) {
                          return seastar::make_ready_future<
        ...
        try {
          [[maybe_unused]] auto stat_result =
              co_await store->get_sharded_store().stat(coll, oid);
          // Object exists, proceed with removal
        } catch (const std::exception& e) {
          fmt::println(
              std::cerr, "Object {} does not exist or stat failed: {}", oid,
              e.what());
          co_return false;
        }
        ...
        if (all) {
          fmt::println(
              std::cout, "removeall: removing object {} and all clones", oid);
          txn.remove(cid, oid);

        } else {
          fmt::println(
              std::cout, "remove: removing object {}{}", oid,
              force ? " (forced)" : "");

          if (oid.hobj.has_snapset() && !force) {
            fmt::println(
                std::cerr,
                "Object {} may have snapset - use removeall or force to remove",
                oid);
          }
          txn.remove(cid, oid);
        }
...
seastar::future<std::string>
StoreTool::dump_object_info(const coll_t& cid, const ghobject_t& oid)
{
  return seastar::smp::submit_to(
      shard_id, [this, cid, oid]() -> seastar::future<std::string> {
        auto coll = co_await store->get_sharded_store()
...
seastar::future<bool>
StoreTool::set_object_size(
    const coll_t& cid,
    const ghobject_t& oid,
    uint64_t size)
{
  return seastar::smp::submit_to(
      shard_id, [this, cid, oid, size]() -> seastar::future<bool> {
        auto coll = co_await store->get_sharded_store()
                        .open_collection(cid)
                        .handle_exception([](std::exception_ptr) {
                          return seastar::make_ready_future<
...
seastar::future<bool>
StoreTool::clear_data_digest(const coll_t& cid, const ghobject_t& oid)
{
  return seastar::smp::submit_to(
      shard_id, [this, cid, oid]() -> seastar::future<bool> {
        auto coll = co_await store->get_sharded_store()
                        .open_collection(cid)
                        .handle_exception([](std::exception_ptr) {
                          return seastar::make_ready_future<
...
// PG-level operations
seastar::future<pg_info_t>
StoreTool::get_pg_info(const coll_t& cid)
{
  return seastar::smp::submit_to(
      shard_id, [this, cid]() -> seastar::future<pg_info_t> {
        auto coll = co_await store->get_sharded_store()
                        .open_collection(cid)
                        .handle_exception([](std::exception_ptr) {
                          return seastar::make_ready_future<
