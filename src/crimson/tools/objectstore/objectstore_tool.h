...
class StoreTool {
public:
  StoreTool(std::unique_ptr<crimson::os::FuturizedStore> store) :
  ...
  void
  set_shard_id(seastar::shard_id shard_id)
  {
  ...
  get_attrs(const coll_t& cid, const ghobject_t& oid);
