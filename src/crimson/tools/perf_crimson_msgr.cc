...
struct client_config {
  ...
  str() const
  {
    ...
    out << "client[>> " << server_addr << "](bs=" << block_size
        << ", ramptime=" << ramptime << ", msgtime=" << msgtime
        << ", num_clients=" << num_clients << ", num_conns=" << num_conns
        << ", depth=" << depth << ", skip_core_0=" << skip_core_0 << ")";
    return out.str();
  }

  static client_config
  load(bpo::variables_map& options)
  {
    client_config conf;
    entity_addr_t addr;
    ceph_assert(
        addr.parse(options["server-addr"].as<std::string>().c_str(), nullptr));
...
struct server_config {
  ...
  {
    std::ostringstream out;
    out << "server[" << addr << "](bs=" << block_size
        << ", is_fixed_cpu=" << is_fixed_cpu << ", core=" << core << ")";
    return out.str();
  }
  ...
  struct test_state {
    struct Server final : public crimson::net::Dispatcher,
      ...
      Server(seastar::shard_id msgr_sid, unsigned msg_len, bool needs_report) :
        msgr_sid{msgr_sid}, msg_len{msg_len}
      {
      ...
      void
      ms_handle_connect(crimson::net::ConnectionRef, seastar::shard_id) override
      {
      ...
      void
      ms_handle_reset(crimson::net::ConnectionRef, bool) override
      {
        auto& server = container().local();
        --server.conn_count;
      }
      ...
      ms_dispatch(crimson::net::ConnectionRef c, MessageRef m) override
      {
        ...
        const static hobject_t hobj(
            object_t(), oloc.key, CEPH_NOSNAP, pgid.ps(), pgid.pool(),
            oloc.nspace);
      ...
      seastar::future<>
      init(const entity_addr_t& addr, bool is_fixed_cpu)
      {
        return container().invoke_on(
            msgr_sid, [addr, is_fixed_cpu](auto& server) {
              // server msgr is always with nonce 0
              server.msgr = crimson::net::Messenger::create(
                  entity_name_t::OSD(server.msgr_sid), server.lname, 0,
                  is_fixed_cpu);
              server.msgr->set_default_policy(
              ...
              return server.msgr->bind(entity_addrvec_t{addr})
                  .safe_then(
                      [&server] { return server.msgr->start({&server}); },
                  ...
                  if (is_fixed_cpu) {
                    return seastar::smp::submit_to(
                               ...
                               [&report, this] {
                                 auto& server = container().local();
                                 server.get_report(
                                     report.reports[seastar::this_shard_id()]);
                               })
                        .then([&report, this] {
    ...
    struct Client final : public crimson::net::Dispatcher,
      ...
      struct ConnStats {
        ...
        void
        prepare_summary(const ConnStats& current)
        {
          *this = current;
      ...
      struct PeriodStats {
        ...
        void
        start_collect(unsigned received_count)
        {
          start_time = mono_clock::now();
          start_count = received_count;
          sampled_count = 0u;
      ...
      struct JobReport {
        ...
        report() const
        {
          auto str = fmt::format(
              ...
              name, depth, connect_time_s, total_msgs, messaging_time_s,
      ...
      struct ConnState {
        ...
        unsigned
        get_current_units() const
        {
          ceph_assert(depth.available_units() >= 0);
          return depth.current();
        }

        seastar::future<JobReport>
          ...
          seastar::shard_id prv_shard) override
      {
        ...
        assert(is_active());
        unsigned index =
            static_cast<ConnectionPriv&>(conn->get_user_private()).index;
        auto& conn_state = conn_states[index];
        conn_state.conn_stats.finish_connecting();
      }

      std::optional<seastar::future<>>
      ms_dispatch(crimson::net::ConnectionRef conn, MessageRef m) override
      {
        assert(is_active());
        // server replies with MOSDOp to generate server-side write workload
        ceph_assert(m->get_type() == CEPH_MSG_OSD_OP);
        ...
        if (msg_id % SAMPLE_RATE == 0) {
          ...
          ceph_assert(
              conn_state.time_msgs_sent[msg_index] !=
            ...
            return seastar::do_for_each(
                ...
                [&client](auto i) {
                  ...
                  conn_state.msgr = crimson::net::Messenger::create(
                      ...
                      true);
                  conn_state.msgr->set_default_policy(
                      crimson::net::SocketPolicy::lossy_client(0));
      ...
      seastar::future<>
      connect_wait_verify(const entity_addr_t& peer_addr)
      {
        return container().invoke_on_all([peer_addr](auto& client) {
          ...
          if (client.is_active()) {
            for (unsigned i = 0; i < client.num_conns; ++i) {
              ...
              conn_state.active_conn =
                  conn_state.msgr->connect(peer_addr, entity_name_t::TYPE_OSD);
    ...
    private:
      ...
      public:
        ...
        PeriodStats&
        get_snap(unsigned client_id, unsigned i)
        {
          return snaps[client_id * num_conns + i];
        }

        ConnStats&
        get_summary(unsigned client_id, unsigned i)
        {
          return summaries[client_id * num_conns + i];
        }

        void
        set_client_reactor_utilization(unsigned client_id, double ru)
        {
          client_reactor_utilizations[client_id] = ru;
        }

        void
        set_server_reactor_utilization(double ru)
        {
        ...
        {
          std::ostringstream sout;
          sout << std::setfill(' ') << std::setw(6) << "sec" << std::setw(7)
               << "depth" << std::setw(10) << "IOPS" << std::setw(9) << "MB/s"
               << std::setw(9) << "lat(ms)";
          std::cout << sout.str() << std::endl;
        }
        ...
        report_period()
        {
          ...
          std::ostringstream sout;
          sout << setfill(' ') << std::setw(5) << elapsed_s << " "
               << std::setw(6) << depth << " " << std::setw(9) << iops << " "
        ...
        report_summary() const
        {
          ...
          std::ostringstream sout;
          sout << "--------------" << " summary " << "--------------\n"
               << setfill(' ') << std::setw(7) << elapsed_s << std::setw(6)
               << "-" << std::setw(8) << iops << std::setw(8)
               << iops * bytes_of_block / 1048576 << std::setw(8)
      ...
      seastar::future<>
      report_period(TimerReport& report)
      {
        return container()
              ...
              if (client.is_active()) {
                for (unsigned i = 0; i < client.num_conns; ++i) {
                  ...
                  conn_state.period_stats.reset_period(
                      conn_state.conn_stats.received_count,
                      client.nr_depth - conn_state.get_current_units(), snap);
                }
                report.set_client_reactor_utilization(
                    client.id, get_reactor_utilization());
              }
      ...
      seastar::future<>
      report_summary(TimerReport& report)
      {
        return container()
            .invoke_on_all([&report](auto& client) {
              if (client.is_active()) {
    ...
    public:
      seastar::future<>
      dispatch_with_timer(unsigned ramptime, unsigned msgtime)
      {
        logger().info(
            "[all clients]: start sending MOSDOps from {} clients * {} conns",
              ...
              return container().invoke_on_all([](auto& client) {
                if (client.is_active()) {
                  for (unsigned i = 0; i < client.num_conns; ++i) {
                    ...
                    conn_state.period_stats.start_collect(
                        conn_state.conn_stats.received_count);
                  }
                }
              });
    ...
    private:
      ...
      send_msg(ConnState& conn_state)
      {
        ...
        return conn_state.depth.wait(1).then([this, &conn_state] {
          ...
          if (unlikely(conn_state.sent_count % SAMPLE_RATE == 0)) {
            auto index = conn_state.sent_count %
                         conn_state.time_msgs_sent.size();
            ceph_assert(
                conn_state.time_msgs_sent[index] ==
                lowres_clock_t::time_point::min());
      ...
      void
      do_dispatch_messages(unsigned i)
      {
        ceph_assert(seastar::this_shard_id() == sid);
        auto& conn_state = conn_states[i];
        ...
        (void)seastar::do_until(
            ...
            .then([this, &conn_state, i] {
              ...
              unsigned ops = conn_state.conn_stats.received_count -
              ...
              stats.latency_ms = conn_state.conn_stats.sampled_total_lat_s /
  ...
  return seastar::when_all(
             seastar::futurize_invoke([mode, server_conf, server_needs_report] {
               if (mode == perf_mode_t::client) {
                 ...
                 return create_sharded<test_state::Server>(
                     server_conf.core, server_conf.block_size,
                     server_needs_report);
               }
             }),
             seastar::futurize_invoke([mode, client_conf, server_sid] {
               if (mode == perf_mode_t::server) {
                 ...
                 unsigned nonce_base =
                     ceph::util::generate_random_number<unsigned>();
                 logger().info("client nonce_base={}", nonce_base);
                 return create_sharded<test_state::Client>(
                     client_conf.num_clients, client_conf.num_conns,
                     client_conf.block_size, client_conf.depth, nonce_base,
                     server_sid);
               }
             }),
      ...
      .then([=](auto&& ret) {
        ...
        if (mode == perf_mode_t::both) {
          ...
          ceph_assert(
              seastar::smp::count > server_conf.core + client_conf.num_clients);
      ...
      .finally([] {
        return seastar::smp::submit_to(
                   ...
                   [] {
                     return seastar::do_for_each(
                                sharded_objects<test_state::Server>,
                                [](auto& sharded_obj) {
...
main(int argc, char** argv)
{
  ...
  app.add_options()(
      ...
      "server is in the fixed cpu mode, non-fixed doesn't support the mode "
      "both")(
