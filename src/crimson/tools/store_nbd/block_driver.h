...
public:
  struct config_t {
    ...
    bool
    prepopulate_log_enabled() const
    {
      return prepopulate_log;
    }

    void
    populate_options(boost::program_options::options_description& desc)
    {
      ...
      desc.add_options()(
          ...
          "device-path",
          po::value<std::string>()->required()->notifier([this](auto s) {
            path = s;
          }),
          ...
          ", 0 to disable")(
          "log-entry-size", po::value<unsigned>()->notifier([this](auto s) {
            log_entry_size = s;
          }),
          ...
          "object-info-size", po::value<unsigned>()->notifier([this](auto s) {
            log_entry_size = s;
          }),
          "Size of each log entry per pg to use for futurized_store backends"
          ", 0 to disable")(
          ...
          "mkfs",
          po::value<bool>()->default_value(false)->notifier([this](auto s) {
            mkfs = s;
          }),
          "Do mkfs first");
    }
