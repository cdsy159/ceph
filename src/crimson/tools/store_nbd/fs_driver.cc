...
coll_t
get_coll(unsigned num)
{
  return coll_t(spg_t(pg_t(0, num)));
}

ghobject_t
get_log_object(unsigned coll)
{
  return ghobject_t(
      shard_id_t::NO_SHARD, 0, (coll << 16), "", "", 0, ghobject_t::NO_GEN);
}

std::string
make_log_key(unsigned i)
{
    ...
    unsigned entries)
{
  ...
  while ((pg.log_head - pg.log_tail) > entries) {
    t.omap_rmkey(
        pg.collection->get_cid(), pg.log_object, make_log_key(pg.log_tail));
    ++pg.log_tail;
  }
}

FSDriver::offset_mapping_t
FSDriver::map_offset(off_t offset)
{
  ...
  return offset_mapping_t{
      collections[collid],
      ghobject_t(
          shard_id_t::NO_SHARD, 0, (collid << 16) | (objid + 1), "", "", 0,
          ghobject_t::NO_GEN),
      offset % config.object_size};
}

seastar::future<>
FSDriver::write(off_t offset, bufferptr ptr)
{
  ...
  bl.append(ptr);
  t.write(
      mapping.pg.collection->get_cid(), mapping.object, mapping.offset,
      ptr.length(), bl, 0);
...
FSDriver::read(off_t offset, size_t size)
{
  auto mapping = map_offset(offset);
  ceph_assert((mapping.offset + size) <= config.object_size);
  return sharded_fs
      ->read(mapping.pg.collection, mapping.object, mapping.offset, size, 0)
      .handle_error(
...
FSDriver::mkfs()
{
  return init()
      .then([this] {
        ...
        return fs->mkfs(uuid).handle_error(crimson::stateful_ec::assert_failure(
            fmt::format(
      ...
      .then([this] {
        return fs->mount().handle_error(crimson::stateful_ec::assert_failure(
            fmt::format(
                "error creating empty object store in {}",
                crimson::common::local_conf().get_val<std::string>("osd_data"))
                .c_str()));
      ...
      .then([this] {
        return seastar::do_for_each(
            ...
            boost::counting_iterator<unsigned>(config.num_pgs), [this](auto i) {
                  ...
                  .then([this, i](auto ref) {
                    ...
                    if (config.log_enabled()) {
                      ...
                      if (config.prepopulate_log_enabled()) {
                        populate_log(
                            t, collections[i], config.log_entry_size,
                            config.log_size);
                      }
                      return sharded_fs->do_transaction(
                          collections[i].collection, std::move(t));
                    } else {
                      return seastar::now();
                    }
                  });
...
FSDriver::init()
{
  ...
  fs = FuturizedStore::create(
      config.get_fs_type(), *config.path,
