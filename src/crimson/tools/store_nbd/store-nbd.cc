...
struct request_context_t {
  ...
  bool
  has_input_buffer() const
  {
    return get_command() == NBD_CMD_WRITE;
  }

  seastar::future<>
  read_request(seastar::input_stream<char>& in)
  {
    return in.read_exactly(sizeof(struct nbd_request))
        .then([this, &in](auto buf) {
          ...
          logger().debug(
              "Got request, magic {}, type {}, from {}, len {}", magic, type,
              from, len);
  ...
  seastar::future<>
  write_reply(seastar::output_stream<char>& out)
  {
...
struct RequestWriter {
  ...
  seastar::future<>
  complete(request_context_t::ref&& req)
  {
    auto& request = *req;
...
public:
  struct config_t {
    ...
    void
    populate_options(po::options_description& desc)
    {
      desc.add_options()(
          "uds-path",
          po::value<std::string>()
              ->default_value("/tmp/store_nbd_socket.sock")
              ->notifier([this](auto s) { uds_path = s; }),
          "Path to domain socket for nbd");
    }
  };

  NBDHandler(BlockDriver& backend, config_t config) :
    backend(backend), uds_path(config.uds_path)
  {}
...
seastar::future<>
send_negotiation(size_t size, seastar::output_stream<char>& out)
{
    ...
    RequestWriter& out)
{
  ...
  return ([&] {
           ...
           case NBD_CMD_WRITE:
             return backend.write(request.from, *request.in_buffer);
           case NBD_CMD_READ:
    ...
    RequestWriter& out)
{
  ...
  return seastar::keep_doing([&] {
           ...
           auto& request = *request_ref;
           return request.read_request(in).then([&, request_ref = std::move(
                                                        request_ref)]() mutable {
             // keep running in background
             (void)seastar::try_with_gate(
                 out.gate, [&backend, &out,
...
NBDHandler::run()
{
  ...
  server_socket = seastar::engine().listen(seastar::socket_address{
