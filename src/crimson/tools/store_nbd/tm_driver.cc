...
seastar::future<>
TMDriver::write(off_t offset, bufferptr ptr)
{
             ...
             [this, offset](auto& ptr) {
               return repeat_eagain([this, offset, &ptr] {
                 return tm->with_transaction_intr(
                     Transaction::src_t::MUTATE, "write", CACHE_HINT_TOUCH,
                       ...
                       return tm->remove(t, laddr_t::from_byte_offset(offset))
                           ...
                           .si_then([this, offset, &t, &ptr] {
                             ...
                             return tm->alloc_data_extents<TestBlock>(
                                 t, laddr_t::from_byte_offset(offset),
                                 ptr.length());
                           })
                           .si_then([this, offset, &t,
                                     &ptr](auto extents) mutable {
...
TMDriver::read_extents_ret
TMDriver::read_extents(Transaction& t, laddr_t offset, extent_len_t length)
{
  return seastar::do_with(
      lba_mapping_list_t(), lextent_list_t<TestBlock>(),
            ...
            .si_then([this, &t, &pins, &ret](auto _pins) {
              ...
              return trans_intr::do_for_each(
                         ...
                         [this, &t, &ret](auto&& pin) {
                           ...
                           return tm->read_pin<TestBlock>(t, std::move(pin))
                               .si_then([&ret](
                                 ...
                                 ret.push_back(
                                     std::make_pair(e->get_laddr(), e));
                                 logger().debug(
                                     "read_extents: got extent {}", *e);
                                 return seastar::now();
                               });
                         })
...
TMDriver::read(off_t offset, size_t size)
{
  ...
  return repeat_eagain([=, &blret, this] {
           return tm->with_transaction_intr(
               Transaction::src_t::READ, "read", CACHE_HINT_TOUCH,
               [=, &blret, this](auto& t) {
                     ...
                     .si_then([=, &blret](auto ext_list) {
                       ...
                       for (auto& i : ext_list) {
                         if (cur != i.first) {
                           assert(cur < i.first);
                           blret.append_zero(
                               i.first.template get_byte_distance<size_t>(cur));
                           cur = i.first;
                         }
                         blret.append(i.second->get_bptr());
                         cur = (cur + i.second->get_bptr().length())
                                   .checked_to_laddr();
                       }
...
TMDriver::init()
{
  ...
  tm = make_transaction_manager(
      device.get(), sec_devices, shard_stats, 0, false);
#endif
}
...
TMDriver::mkfs()
{
  ...
  return Device::make_device(*config.path, device_type_t::SSD)
      .then([this](DeviceRef dev) {
        ...
        return device->mkfs(device_config_t{
            true, (magic_t)std::rand(), device_type_t::SSD, 0, meta,
