...
static void
print_type_name(ostream& out, int t, CrushWrapper& crush)
{
...
static void
print_item_name(ostream& out, int t, CrushWrapper& crush)
{
...
static void
print_bucket_class_ids(ostream& out, int t, CrushWrapper& crush)
{
  ...
  for (auto& i : class_to_id) {
    ...
    out << "\tid " << cid << " class " << class_name
        << "\t\t# do not change unnecessarily\n";
  }
}

static void
print_item_class(ostream& out, int t, CrushWrapper& crush)
{
...
static void
print_class(ostream& out, int t, CrushWrapper& crush)
{
...
static void
print_rule_name(ostream& out, int t, CrushWrapper& crush)
{
...
static void
print_fixedpoint(ostream& out, int i)
{
  char s[20];
...
int
CrushCompiler::decompile_bucket_impl(int i, ostream& out)
{
 ...
 */
int
CrushCompiler::decompile_bucket(
    int cur,
...
int
CrushCompiler::decompile_weight_set_weights(
...
int
CrushCompiler::decompile_weight_set(
    crush_weight_set* weight_set,
    __u32 size,
    ostream& out)
{
...
int
CrushCompiler::decompile_ids(__s32* ids, __u32 size, ostream& out)
{
  out << "    ids [ ";
  for (__u32 i = 0; i < size; i++)
    out << ids[i] << " ";
...
int
CrushCompiler::decompile_choose_arg(
...
int
CrushCompiler::decompile_choose_arg_map(
...
int
CrushCompiler::decompile_choose_args(
    const std::pair<const long unsigned int, crush_choose_arg_map>& i,
    ostream& out)
{
...
int
CrushCompiler::decompile(ostream& out)
{
  out << "# begin crush map\n";
  ...
  if (crush.get_choose_local_fallback_tries() != 5)
    out << "tunable choose_local_fallback_tries "
        << crush.get_choose_local_fallback_tries() << "\n";
  ...
  if (crush.get_chooseleaf_descend_once() != 0)
    out << "tunable chooseleaf_descend_once "
        << crush.get_chooseleaf_descend_once() << "\n";
  ...
  for (int i = 0; i < crush.get_max_rules(); i++) {
    ...
    for (int j = 0; j < crush.get_rule_len(i); j++) {
      ...
      case CRUSH_RULE_SET_CHOOSE_LOCAL_FALLBACK_TRIES:
        out << "\tstep set_choose_local_fallback_tries "
      ...
      case CRUSH_RULE_SET_MSR_COLLISION_TRIES:
        out << "\tstep set_msr_collision_tries " << crush.get_rule_arg1(i, j)
            << "\n";
        break;
      ...
      case CRUSH_RULE_CHOOSE_MSR:
        out << "\tstep choosemsr " << crush.get_rule_arg1(i, j) << " type ";
        print_type_name(out, crush.get_rule_arg2(i, j), crush);
        out << "\n";
        break;
      }
...
string
CrushCompiler::string_node(node_t& node)
{
  return boost::trim_copy(string(node.value.begin(), node.value.end()));
}

int
CrushCompiler::int_node(node_t& node)
{
...
float
CrushCompiler::float_node(node_t& node)
{
...
int
CrushCompiler::parse_device(iter_t const& i)
{
...
int
CrushCompiler::parse_tunable(iter_t const& i)
{
...
int
CrushCompiler::parse_bucket_type(iter_t const& i)
{
  int id = int_node(i->children[1]);
  string name = string_node(i->children[2]);
  if (verbose)
...
int
CrushCompiler::parse_bucket(iter_t const& i)
{
    ...
    if (tag == "id") {
      ...
      if (sub->children.size() > 2) {
        ...
        if (class_id.count(cid) != 0) {
          err << "duplicate device class " << class_name << " for bucket "
              << name << std::endl;
          return -ERANGE;
        }
      ...
      for (unsigned q = 2; q < sub->children.size(); q++) {
        ...
        if (tag == "pos") {
          ...
          if (used_items.count(pos)) {
            err << "item '" << string_node(sub->children[1]) << "' in bucket '"
                << name << "' has explicit pos " << pos << ", which is occupied"
                << std::endl;
            return -1;
          }
  ...
  for (unsigned p = 3; p < i->children.size() - 1; p++) {
    ...
    if (tag == "item") {
        ...
        if (tag == "weight") {
          ...
          if (weight > CRUSH_MAX_DEVICE_WEIGHT && itemid >= 0) {
            ...
            err << "bucket weight limited to "
                << CRUSH_MAX_BUCKET_WEIGHT / 0x10000 << " to prevent overflow"
                << std::endl;
            return -ERANGE;
          }
        } else if (tag == "pos")
      ...
      if (alg == CRUSH_BUCKET_UNIFORM) {
        if (!have_uniform_weight) {
          ...
          if (uniform_weight != weight) {
            err << "item '" << iname << "' in uniform bucket '" << name
                << "' has weight " << weight
                << " but previous item(s) have weight "
                << (float)uniform_weight / (float)0x10000
                << "; uniform bucket items must all have identical weights."
                << std::endl;
            return -1;
          }
        }
      ...
      if (pos >= size) {
        err << "item '" << iname << "' in bucket '" << name << "' has pos "
            << pos << " >= size " << size << std::endl;
        return -1;
      }
      if (pos < 0) {
      ...
      if (crush_addition_is_unsafe(bucketweight, weight)) {
        err << "oh no! our bucket weights are overflowing all over the place, "
               "better lower the item weights"
            << std::endl;
        return -ERANGE;
      }
  ...
  if (verbose)
    err << "bucket " << name << " (" << id << ") " << size
        << " items and weight " << (float)bucketweight / (float)0x10000
        << std::endl;
  id_item[id] = name;
  item_id[name] = id;
  ...
  int r = crush.add_bucket(
      id, alg, hash, type, size, items.data(), weights.data(), &idout);
  if (r < 0) {
    if (r == -EEXIST)
...
int
CrushCompiler::parse_rule(iter_t const& i)
{
  ...
  for (unsigned p = start + 3; p < i->children.size() - 1; ++p) {
    ...
    if (tag == "min_size" || tag == "max_size") {
      std::cerr << "WARNING: " << tag << " is no longer supported, ignoring"
                << std::endl;
      ++p;
      continue;
    }
  ...
  for (iter_t p = i->children.begin() + step_start; step < steps; p++) {
    ...
    case crush_grammar::_step_take: {
      ...
      if (!item_id.count(item)) {
        err << "in rule '" << rname << "' item '" << item << "' not defined"
            << std::endl;
        return -1;
      }
      int id = item_id[item];
    ...
    case crush_grammar::_step_chooseleaf: {
      ...
      if (!type_id.count(type)) {
        err << "in rule '" << rname << "' type '" << type << "' not defined"
            << std::endl;
        return -1;
      }
      ...
      if (choose == "choose") {
        if (mode == "firstn")
          crush.set_rule_step_choose_firstn(
              ruleno, step++, int_node(s->children[2]), type_id[type]);
        else if (mode == "indep")
          crush.set_rule_step_choose_indep(
              ruleno, step++, int_node(s->children[2]), type_id[type]);
        else
          ceph_abort();
      } else if (choose == "chooseleaf") {
        if (mode == "firstn")
          crush.set_rule_step_choose_leaf_firstn(
              ruleno, step++, int_node(s->children[2]), type_id[type]);
        else if (mode == "indep")
          crush.set_rule_step_choose_leaf_indep(
              ruleno, step++, int_node(s->children[2]), type_id[type]);
        else
          ceph_abort();
      } else
    ...
    case crush_grammar::_step_choose_msr: {
      ...
      crush.set_rule_step_choose_msr(
          ruleno, step++, int_node(s->children[1]), type_id[type]);
    } break;
...
int
CrushCompiler::parse_weight_set_weights(
    iter_t const& i,
    int bucket_id,
    crush_weight_set* weight_set)
{
  ...
  if (size != bucket_size) {
    err << bucket_id << " needs exactly " << bucket_size << " weights but got "
        << size << std::endl;
    return -1;
  }
...
int
CrushCompiler::parse_weight_set(
    iter_t const& i,
    int bucket_id,
    crush_choose_arg* arg)
{
...
int
CrushCompiler::parse_choose_arg_ids(
    iter_t const& i,
    int bucket_id,
    crush_choose_arg* arg)
{
  ...
  __u32 bucket_size = crush.get_bucket_size(bucket_id);
  if (size != bucket_size) {
    err << bucket_id << " needs exactly " << bucket_size << " ids but got "
        << size << std::endl;
    return -1;
  }
  arg->ids_size = size;
...
int
CrushCompiler::parse_choose_arg(iter_t const& i, crush_choose_arg* args)
{
...
int
CrushCompiler::parse_choose_args(iter_t const& i)
{
  ...
  arg_map.args =
      (crush_choose_arg*)calloc(arg_map.size, sizeof(crush_choose_arg));
...
int
CrushCompiler::parse_crush(iter_t const& i)
{
...
void
CrushCompiler::dump(iter_t const& i, int ind)
{
  err << "dump";
  for (int j = 0; j < ind; j++)
    cout << "\t";
...
**/
int
CrushCompiler::adjust_bucket_item_place(iter_t const& i)
{
    ...
    for (unsigned j = i + 1; j < buckets.size(); ++j) {
      if (bucket_items[buckets[i]].count(buckets[j])) {
        if (bucket_items[buckets[j]].count(buckets[i])) {
          err << "bucket  '" << buckets[i] << "' and bucket '" << buckets[j]
...
int
CrushCompiler::compile(istream& in, const char* infn)
{
  if (!infn)
    infn = "<input>";
  ...
  if (!info.full) {
    ...
    err << infn << ":" << line //<< ":" << (pos+1)
        << " error: parse error at '" << line_val[line].substr(pos) << "'"
        << std::endl;
    return -1;
  }
