...
class CrushCompiler {
  ...
  int decompile_weight_set_weights(
      crush_weight_set weight_set,
      std::ostream& out);
  int decompile_weight_set(
      crush_weight_set* weight_set,
      __u32 size,
      std::ostream& out);
  int decompile_choose_arg(
  ...
  std::map<int32_t, std::map<int32_t, int32_t>>
      class_bucket; // bucket id -> class id -> shadow bucket id
  ...
  int parse_weight_set_weights(
      iter_t const& i,
      int bucket_id,
      crush_weight_set* weight_set);
  int parse_weight_set(iter_t const& i, int bucket_id, crush_choose_arg* arg);
  int parse_choose_arg_ids(
      iter_t const& i,
      int bucket_id,
      crush_choose_arg* args);
...
public:
  CrushCompiler(CrushWrapper& c, std::ostream& eo, int verbosity = 0) :
    crush(c), err(eo), verbose(verbosity), unsafe_tunables(false)
  {}
