 ...
 */
   ...
   */
  bool check_valid_placement(
      int ruleno,
      std::vector<int> in,
      const std::vector<__u32>& weight);
  ...
  void
  write_to_csv(std::ofstream& csv_file, std::vector<std::string>& payload)
  {
    if (csv_file.good())
      for (std::vector<std::string>::iterator it = payload.begin();
           it != payload.end(); ++it)
        csv_file << (*it);
  }

  void
  write_to_csv(std::ofstream& csv_file, std::map<int, float>& payload)
  {
    if (csv_file.good())
      for (std::map<int, float>::iterator it = payload.begin();
           it != payload.end(); ++it)
        csv_file << (*it).first << ',' << (*it).second << std::endl;
  }

  void
  write_data_set_to_csv(std::string user_tag, tester_data_set& tester_data)
  {

    std::ofstream device_utilization_file(
        (user_tag + (std::string) "-device_utilization.csv").c_str());
    std::ofstream device_utilization_all_file(
        (user_tag + (std::string) "-device_utilization_all.csv").c_str());
    std::ofstream placement_information_file(
        (user_tag + (std::string) "-placement_information.csv").c_str());
    std::ofstream proportional_weights_file(
        (user_tag + (std::string) "-proportional_weights.csv").c_str());
    std::ofstream proportional_weights_all_file(
        (user_tag + (std::string) "-proportional_weights_all.csv").c_str());
    std::ofstream absolute_weights_file(
        (user_tag + (std::string) "-absolute_weights.csv").c_str());

    // write the headers
    device_utilization_file
        << "Device ID, Number of Objects Stored, Number of Objects Expected"
        << std::endl;
    device_utilization_all_file
        << "Device ID, Number of Objects Stored, Number of Objects Expected"
        << std::endl;
    proportional_weights_file << "Device ID, Proportional Weight" << std::endl;
    proportional_weights_all_file << "Device ID, Proportional Weight"
                                  << std::endl;
    ...
    write_to_csv(
        device_utilization_all_file, tester_data.device_utilization_all);
    ...
    write_to_csv(
        proportional_weights_all_file, tester_data.proportional_weights_all);
    ...
    if (num_batches > 1) {
      ...
      std::ofstream batch_device_expected_utilization_all_file(
          (user_tag +
           (std::string) "-batch_device_expected_utilization_all.csv")
              .c_str());
      ...
      for (unsigned i = 0; i < tester_data.device_utilization.size(); i++) {
        batch_device_expected_utilization_all_file
            << ", Objects Expected on OSD" << i;
      }
      batch_device_expected_utilization_all_file << std::endl;

      write_to_csv(
          batch_device_utilization_all_file,
          tester_data.batch_device_utilization_all);
      write_to_csv(
          batch_device_expected_utilization_all_file,
          tester_data.batch_device_expected_utilization_all);
...
public:
  ...
  void
  set_output_data_file_name(std::string name)
  {
  ...
  void
  set_output_data_file(bool b)
  {
    output_data_file = b;
  }

  bool
  get_output_data_file() const
  {
    return output_data_file;
  }

  void
  set_output_csv(bool b)
  {
    output_csv = b;
  }

  bool
  get_output_csv() const
  {
    return output_csv;
  }

  void
  set_output_utilization(bool b)
  {
    output_utilization = b;
  }

  bool
  get_output_utilization() const
  {
    return output_utilization;
  }

  void
  set_output_utilization_all(bool b)
  {
    output_utilization_all = b;
  }

  bool
  get_output_utilization_all() const
  {
    return output_utilization_all;
  }

  void
  set_output_statistics(bool b)
  {
    output_statistics = b;
  }

  bool
  get_output_statistics() const
  {
    return output_statistics;
  }

  void
  set_output_mappings(bool b)
  {
    output_mappings = b;
  }

  bool
  get_output_mappings() const
  {
    return output_mappings;
  }

  void
  set_output_bad_mappings(bool b)
  {
    output_bad_mappings = b;
  }

  bool
  get_output_bad_mappings() const
  {
    return output_bad_mappings;
  }

  void
  set_output_choose_tries(bool b)
  {
    output_choose_tries = b;
  }

  bool
  get_output_choose_tries() const
  {
  ...
  bool
  get_random_placement() const
  {
    return use_crush == false;
  }

  void
  set_bucket_down_ratio(float bucket_ratio)
  {
    mark_down_bucket_ratio = bucket_ratio;
  }

  float
  get_bucket_down_ratio() const
  {
    return mark_down_bucket_ratio;
  }

  void
  set_device_down_ratio(float device_ratio)
  {
    mark_down_device_ratio = device_ratio;
  }

  float
  set_device_down_ratio() const
  {
