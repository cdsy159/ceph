...
struct Item {
  ...
  Item() :
    id(0), parent(0), depth(0), weight(0)
  {}

  Item(int i, int p, int d, float w) :
    id(i), parent(p), depth(d), weight(w)
  {}
...
public:
  ...
  virtual bool
  should_dump_leaf(int i) const
  {
    return true;
  }

  virtual bool
  should_dump_empty_bucket() const
  {
    return true;
  }
    ...
    if (qi.is_bucket()) {
      ...
      for (int k = s - 1; k >= 0; k--) {
        ...
        if (should_dump(id)) {
          ...
          sorted[sort_by] =
              std::make_pair(id, crush->get_bucket_item_weightf(qi.id, k));
        }
      }
  ...
  bool
  is_touched(int id) const
  {
    return touched.count(id) > 0;
  }

  void
  set_root(const std::string& bucket)
  {
    roots.clear();
  ...
  if (qi.parent < 0) {
    ...
    for (auto& p : crush->choose_args) {
      ...
      if (b && bidx < (int)cmap.size && cmap.args[bidx].weight_set &&
        ...
        int bpos;
        for (bpos = 0; bpos < (int)cmap.args[bidx].weight_set[0].size &&
                       b->items[bpos] != qi.id;
             ++bpos)
          ;
        std::string name;
        ...
        for (unsigned opos = 0; opos < cmap.args[bidx].weight_set_positions;
             ++opos) {
...
protected:
  void
  dump_item(const Item& qi, ceph::Formatter* f) override
  {
  ...
  virtual void
  dump_item_fields(const Item& qi, ceph::Formatter* f)
  {
    CrushTreeDumper::dump_item_fields(crush, weight_set_names, qi, f);
  }

  virtual void
  dump_bucket_children(const Item& qi, ceph::Formatter* f)
  {
