...
inline void
encode(const crush_rule_step& s, ceph::buffer::list& bl)
{
  using ceph::encode;
  encode(s.op, bl);
  encode(s.arg1, bl);
...
inline void
decode(crush_rule_step& s, ceph::buffer::list::const_iterator& p)
{
  using ceph::decode;
  decode(s.op, p);
  decode(s.arg1, p);
...
public:
  ...
  std::map<int32_t, std::string>
      type_map; // item(bucket/device) type id ==> item type name
  ...
  std::map<int32_t, std::map<int32_t, int32_t>>
      class_bucket; /* bucket[id][class] == id */
...
private:
  ...
  void
  build_rmap(
      const std::map<int, std::string>& f,
      std::map<std::string, int>& r) const
  {
    r.clear();
...
public:
  ...
  int
  get_choose_local_tries() const
  {
    return crush->choose_local_tries;
  }

  void
  set_choose_local_tries(int n)
  {
    crush->choose_local_tries = n;
  }

  int
  get_choose_local_fallback_tries() const
  {
    return crush->choose_local_fallback_tries;
  }

  void
  set_choose_local_fallback_tries(int n)
  {
    crush->choose_local_fallback_tries = n;
  }

  int
  get_choose_total_tries() const
  {
    return crush->choose_total_tries;
  }

  void
  set_choose_total_tries(int n)
  {
    crush->choose_total_tries = n;
  }

  int
  get_chooseleaf_descend_once() const
  {
    return crush->chooseleaf_descend_once;
  }

  void
  set_chooseleaf_descend_once(int n)
  {
    crush->chooseleaf_descend_once = !!n;
  }

  int
  get_chooseleaf_vary_r() const
  {
    return crush->chooseleaf_vary_r;
  }

  void
  set_chooseleaf_vary_r(int n)
  {
    crush->chooseleaf_vary_r = n;
  }

  int
  get_chooseleaf_stable() const
  {
    return crush->chooseleaf_stable;
  }

  void
  set_chooseleaf_stable(int n)
  {
    crush->chooseleaf_stable = n;
  }

  int
  get_straw_calc_version() const
  {
    return crush->straw_calc_version;
  }

  void
  set_straw_calc_version(int n)
  {
    crush->straw_calc_version = n;
  }

  unsigned
  get_msr_descents() const
  {
    return crush->msr_descents;
  }

  void
  set_msr_descents(unsigned n)
  {
    crush->msr_descents = n;
  }

  unsigned
  get_msr_collision_tries() const
  {
    return crush->msr_collision_tries;
  }

  void
  set_msr_collision_tries(unsigned n)
  {
  ...
  unsigned
  get_allowed_bucket_algs() const
  {
    return crush->allowed_bucket_algs;
  }

  void
  set_allowed_bucket_algs(unsigned n)
  {
    crush->allowed_bucket_algs = n;
  }

  bool
  has_argonaut_tunables() const
  {
    return crush->choose_local_tries == 2 &&
           crush->choose_local_fallback_tries == 5 &&
           crush->choose_total_tries == 19 &&
           crush->chooseleaf_descend_once == 0 &&
  ...
  bool
  has_bobtail_tunables() const
  {
    return crush->choose_local_tries == 0 &&
           crush->choose_local_fallback_tries == 0 &&
           crush->choose_total_tries == 50 &&
           crush->chooseleaf_descend_once == 1 &&
           crush->chooseleaf_vary_r == 0 && crush->chooseleaf_stable == 0 &&
  ...
  bool
  has_optimal_tunables() const
  {
    return has_jewel_tunables();
  }

  bool
  has_legacy_tunables() const
  {
    return has_argonaut_tunables();
  }

  bool
  has_nondefault_tunables() const
  {
    return (
        crush->choose_local_tries != 2 ||
        crush->choose_local_fallback_tries != 5 ||
  ...
  bool
  has_nondefault_tunables2() const
  {
    return crush->chooseleaf_descend_once != 0;
  }

  bool
  has_nondefault_tunables3() const
  {
    return crush->chooseleaf_vary_r != 0;
  }

  bool
  has_nondefault_tunables5() const
  {
    return crush->chooseleaf_stable != 0;
  }

  bool
  has_nondefault_tunables_msr() const
  {
    return crush->msr_descents != 100 || crush->msr_collision_tries != 100;
  }
  ...
  // bucket types
  int
  get_num_type_names() const
  {
    return type_map.size();
  }

  int
  get_max_type_id() const
  {
    if (type_map.empty())
      return 0;
  ...
  int
  get_type_id(const std::string& name) const
  {
    build_rmaps();
  ...
  std::optional<int>
  get_validated_type_id(const std::string& name) const
  {
  ...
  void
  set_type_name(int i, const std::string& name)
  {
    type_map[i] = name;
    if (have_rmaps)
      type_rmap[name] = i;
  ...
  // item/bucket names
  bool
  name_exists(const std::string& name) const
  {
    build_rmaps();
  ...
  bool
  item_exists(int i) const
  {
    return name_map.count(i);
  }

  int
  get_item_id(const std::string& name) const
  {
    build_rmaps();
  ...
  int
  set_item_name(int i, const std::string& name)
  {
  ...
  bool
  class_exists(const std::string& name) const
  {
    return class_rname.count(name);
  }

  const char*
  get_class_name(int i) const
  {
  ...
  int
  get_class_id(const std::string& name) const
  {
  ...
  int
  remove_class_name(const std::string& name)
  {
    auto p = class_rname.find(name);
    if (p == class_rname.end())
      return -ENOENT;
  ...
  int
  get_or_create_class_id(const std::string& name)
  {
  ...
  int
  set_item_class(int i, const std::string& name)
  {
    if (!is_valid_crush_name(name))
      return -EINVAL;
  ...
  void
  get_devices_by_class(const std::string& name, std::set<int>* devices) const
  {
    ceph_assert(devices);
    devices->clear();
  ...
  bool
  rule_exists(std::string name) const
  {
    build_rmaps();
  ...
  int
  get_rule_id(std::string name) const
  {
    build_rmaps();
  ...
  void
  set_rule_name(int i, const std::string& name)
  {
  ...
  bool
  rule_valid_for_pool_type(int rule_id, int ptype) const
  {
   ...
   */
  void
  find_shadow_roots(std::set<int>* roots) const
  {
    std::set<int> all;
    find_roots(&all);
    for (auto& p : all) {
   ...
   */
  void
  find_nonshadow_roots(std::set<int>* roots) const
  {
    std::set<int> all;
    find_roots(&all);
    for (auto& p : all) {
...
public:
   ...
   */
  std::pair<std::string, std::string> get_immediate_parent(
      int id,
      int* ret = NULL) const;
...
private:
  ...
  int
  _remove_item_under(CephContext* cct, int id, int ancestor, bool unlink_only);
...
public:
   ...
   */
  int get_common_ancestor_distance(
      CephContext* cct,
      int id,
   ...
   */
  static int parse_loc_map(
      const std::vector<std::string>& args,
      std::map<std::string, std::string>* ploc);
  static int parse_loc_multimap(
      const std::vector<std::string>& args,
  ...
  float
  get_item_weightf(int id) const
  {
    return (float)get_item_weight(id) / (float)0x10000;
  }

  int get_item_weight_in_loc(
      int id,
      const std::map<std::string, std::string>& loc);

  float
  get_item_weightf_in_loc(int id, const std::map<std::string, std::string>& loc)
  {
    return (float)get_item_weight_in_loc(id, loc) / (float)0x10000;
  }

  int
  validate_weightf(float weight)
  {
      ...
      bool update_weight_sets = true)
  {
    ...
    return adjust_item_weight(
        cct, id, (int)(weight * (float)0x10000), update_weight_sets);
  }
      ...
      bool update_weight_sets = true)
  {
    ...
    return adjust_item_weight_in_loc(
        cct, id, (int)(weight * (float)0x10000), loc, update_weight_sets);
  }

  void reweight(CephContext* cct);
  void reweight_bucket(
      crush_bucket* b,
      crush_choose_arg_map& arg_map,
      std::vector<uint32_t>* weightv);

  int adjust_subtree_weight(
      CephContext* cct,
      int id,
      int weight,
      bool update_weight_sets = true);
...
private:
  crush_rule*
  get_rule(unsigned ruleno) const
  {
    if (!crush)
      return (crush_rule*)(-ENOENT);
  ...
  crush_rule_step*
  get_rule_step(unsigned ruleno, unsigned step) const
  {
    crush_rule* n = get_rule(ruleno);
    if (IS_ERR(n))
      return (crush_rule_step*)(-EINVAL);
    if (step >= n->len)
      return (crush_rule_step*)(-EINVAL);
    return &n->steps[step];
  }
...
public:
  ...
  bool
  rule_exists(unsigned ruleno) const
  {
    if (!crush)
      return false;
    if (ruleno < crush->max_rules && crush->rules[ruleno] != NULL)
      return true;
    return false;
  }

  bool
  rule_has_take(unsigned ruleno, int take) const
  {
    if (!crush)
      return false;
  ...
  int
  get_rule_len(unsigned ruleno) const
  {
  ...
  int
  get_rule_type(unsigned ruleno) const
  {
    crush_rule* r = get_rule(ruleno);
    if (IS_ERR(r))
      return -1;
  ...
  int
  get_rule_op(unsigned ruleno, unsigned step) const
  {
  ...
  int
  get_rule_arg1(unsigned ruleno, unsigned step) const
  {
    crush_rule_step* s = get_rule_step(ruleno, step);
    if (IS_ERR(s))
      return PTR_ERR(s);
  ...
  int
  get_rule_arg2(unsigned ruleno, unsigned step) const
  {
    crush_rule_step* s = get_rule_step(ruleno, step);
    if (IS_ERR(s))
      return PTR_ERR(s);
...
public:
  ...
  int
  add_rule(int ruleno, int len, int type)
  {
    if (!crush)
      return -ENOENT;
  ...
  int
  set_rule_step(unsigned ruleno, unsigned step, int op, int arg1, int arg2)
  {
    if (!crush)
      return -ENOENT;
    crush_rule* n = get_rule(ruleno);
  ...
  int
  set_rule_step_take(unsigned ruleno, unsigned step, int val)
  {
    return set_rule_step(ruleno, step, CRUSH_RULE_TAKE, val, 0);
  }

  int
  set_rule_step_set_choose_tries(unsigned ruleno, unsigned step, int val)
  {
    return set_rule_step(ruleno, step, CRUSH_RULE_SET_CHOOSE_TRIES, val, 0);
  }

  int
  set_rule_step_set_choose_local_tries(unsigned ruleno, unsigned step, int val)
  {
    return set_rule_step(
        ruleno, step, CRUSH_RULE_SET_CHOOSE_LOCAL_TRIES, val, 0);
  }

  int
  set_rule_step_set_choose_local_fallback_tries(
      unsigned ruleno,
      unsigned step,
      int val)
  {
    return set_rule_step(
        ruleno, step, CRUSH_RULE_SET_CHOOSE_LOCAL_FALLBACK_TRIES, val, 0);
  }

  int
  set_rule_step_set_chooseleaf_tries(unsigned ruleno, unsigned step, int val)
  {
    return set_rule_step(ruleno, step, CRUSH_RULE_SET_CHOOSELEAF_TRIES, val, 0);
  }

  int
  set_rule_step_set_chooseleaf_vary_r(unsigned ruleno, unsigned step, int val)
  {
    return set_rule_step(ruleno, step, CRUSH_RULE_SET_CHOOSELEAF_VARY_R, val, 0);
  }

  int
  set_rule_step_set_chooseleaf_stable(unsigned ruleno, unsigned step, int val)
  {
    return set_rule_step(ruleno, step, CRUSH_RULE_SET_CHOOSELEAF_STABLE, val, 0);
  }

  int
  set_rule_step_set_msr_descents(unsigned ruleno, unsigned step, int val)
  {
    return set_rule_step(ruleno, step, CRUSH_RULE_SET_MSR_DESCENTS, val, 0);
  }

  int
  set_rule_step_set_msr_collision_tries(unsigned ruleno, unsigned step, int val)
  {
    return set_rule_step(
        ruleno, step, CRUSH_RULE_SET_MSR_COLLISION_TRIES, val, 0);
  }

  int
  set_rule_step_choose_firstn(unsigned ruleno, unsigned step, int val, int type)
  {
    return set_rule_step(ruleno, step, CRUSH_RULE_CHOOSE_FIRSTN, val, type);
  }

  int
  set_rule_step_choose_indep(unsigned ruleno, unsigned step, int val, int type)
  {
    return set_rule_step(ruleno, step, CRUSH_RULE_CHOOSE_INDEP, val, type);
  }

  int
  set_rule_step_choose_leaf_firstn(
      unsigned ruleno,
      unsigned step,
      int val,
      int type)
  ...
  int
  set_rule_step_choose_leaf_indep(
      unsigned ruleno,
      unsigned step,
      int val,
      int type)
  ...
  int
  set_rule_step_choose_msr(unsigned ruleno, unsigned step, int val, int type)
  {
    return set_rule_step(ruleno, step, CRUSH_RULE_CHOOSE_MSR, val, type);
  }

  int
  set_rule_step_emit(unsigned ruleno, unsigned step)
  {
      ...
      std::ostream* err = 0)
  {
    return add_simple_rule(
        name, root_name, failure_domain_type, -1, device_class, mode, rule_type,
        err);
  }
...
public:
  ...
  int
  get_bucket_weight(int id) const
  {
    const crush_bucket* b = get_bucket(id);
    if (IS_ERR(b))
      return PTR_ERR(b);
  ...
  float
  get_bucket_weightf(int id) const
  {
    const crush_bucket* b = get_bucket(id);
    if (IS_ERR(b))
      return 0;
  ...
  int
  get_bucket_item(int id, int pos) const
  {
    const crush_bucket* b = get_bucket(id);
    if (IS_ERR(b))
      return PTR_ERR(b);
  ...
  int
  get_bucket_item_weight(int id, int pos) const
  {
    const crush_bucket* b = get_bucket(id);
    if (IS_ERR(b))
      return PTR_ERR(b);
  ...
  start_choose_profile()
  {
     ...
     */
    crush->choose_tries = (__u32*)calloc(
   ...
   */
  int
  find_first_rule(int type) const
  {
  ...
  bool
  have_choose_args(int64_t choose_args_index) const
  {
    return choose_args.count(choose_args_index);
  }

  crush_choose_arg_map
  choose_args_get_with_fallback(int64_t choose_args_index) const
  {
  ...
  crush_choose_arg_map
  choose_args_get(int64_t choose_args_index) const
  {
    auto i = choose_args.find(choose_args_index);
    if (i == choose_args.end()) {
      crush_choose_arg_map arg_map;
  ...
  void
  destroy_choose_args(crush_choose_arg_map arg_map)
  {
  ...
  bool
  create_choose_args(int64_t id, int positions)
  {
    ...
    cmap.args = static_cast<crush_choose_arg*>(
        calloc(sizeof(crush_choose_arg), crush->max_buckets));
    ...
    for (int bidx = 0; bidx < crush->max_buckets; ++bidx) {
      ...
      if (b && b->alg == CRUSH_BUCKET_STRAW2) {
        ...
        carg.weight_set = static_cast<crush_weight_set*>(
            calloc(sizeof(crush_weight_set), carg.weight_set_positions));
  ...
  choose_args_adjust_item_weightf(
      ...
      crush_choose_arg_map cmap,
      int id,
      const std::vector<double>& weightf,
      std::ostream* ss)
  {
  ...
  int
  get_choose_args_positions(crush_choose_arg_map cmap)
  {
      ...
      uint64_t choose_args_index) const
  {
    ...
    crush_choose_arg_map arg_map =
        choose_args_get_with_fallback(choose_args_index);
    int numrep = crush_do_rule(
        crush, rule, x, std::data(rawout), maxout, std::data(weight),
        std::size(weight), std::data(work), arg_map.args);
    if (numrep < 0)
      numrep = 0;
    out.resize(numrep);
  ...
  void
  dump_tree(std::ostream* out, ceph::Formatter* f)
  {
  ...
  static bool is_valid_crush_loc(
      CephContext* cct,
      const std::map<std::string, std::string>& loc);
};
