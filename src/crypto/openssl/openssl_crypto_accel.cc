    ...
    const int encrypt)
{
  ...
  if (EVP_CipherInit_ex(pctx.get(), type, engine, key, iv, encrypt) !=
      EVP_SUCCESS) {
  ...
  int len_final = 0;
  if (EVP_CipherFinal_ex(pctx.get(), out + len_update, &len_final) !=
      EVP_SUCCESS) {
...
bool
OpenSSLCryptoAccel::cbc_encrypt(
    unsigned char* out,
    const unsigned char* in,
    size_t size,
    ...
    optional_yield y)
{
  ...
  return evp_transform(
      out, in, size, const_cast<unsigned char*>(&iv[0]),
...
bool
OpenSSLCryptoAccel::cbc_decrypt(
    unsigned char* out,
    const unsigned char* in,
    size_t size,
    const unsigned char (&iv)[AES_256_IVSIZE],
