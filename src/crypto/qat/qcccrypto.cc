...
auto
QccCrypto::async_get_instance(CompletionToken&& token)
{
  using Signature = void(int);
  return boost::asio::async_initiate<CompletionToken, Signature>(
      [this](auto handler) {
        boost::asio::post(
            my_pool, [this, handler = std::move(handler)]() mutable {
              if (!open_instances.empty()) {
                int avail_inst = open_instances.front();
                open_instances.pop_front();
                boost::asio::post(
                    boost::asio::append(std::move(handler), avail_inst));
                ...
                boost::asio::post(
                    boost::asio::append(std::move(handler), NON_INSTANCE));
              }
            });
      },
      token);
...
void
QccCrypto::QccFreeInstance(int entry)
{
  boost::asio::post(my_pool, [this, entry]() mutable {
    if (!instance_completions.empty()) {
      boost::asio::dispatch(
          boost::asio::append(std::move(instance_completions.front()), entry));
...
QccCrypto::poll_instances(void)
{
  ...
  while (!thread_stop) {
    ...
    if (0 == poll_retry_num) {
      ...
      poll_inst_cv.wait_for(lock, std::chrono::milliseconds(1), [this]() {
        return poll_retry_num > 0;
      });
      poll_retry_num = RETRY_MAX_NUM;
    }
  }
...
*/
bool
QccCrypto::init(const size_t chunk_size, const size_t max_requests)
{
  ...
  qcc_os_mem_alloc(
      (void**)&qcc_inst->is_polled,
      ((int)qcc_inst->num_instances * sizeof(CpaBoolean)));
  CpaInstanceInfo2 info;
  for (iter = 0; iter < qcc_inst->num_instances; iter++) {
    qcc_inst->is_polled[iter] =
        cpaCyInstanceGetInfo2(qcc_inst->cy_inst_handles[iter], &info) ==
                CPA_STATUS_SUCCESS
            ? info.isPolled
            : CPA_FALSE;
  }
  ...
  qcc_os_mem_alloc(
      (void**)&qcc_sess, ((int)qcc_inst->num_instances * sizeof(QCCSESS)));
  if (qcc_sess == NULL) {
  ...
  qcc_os_mem_alloc(
      (void**)&qcc_op_mem, ((int)qcc_inst->num_instances * sizeof(QCCOPMEM)));
  if (qcc_sess == NULL) {
  ...
  for (iter = 0; iter < qcc_inst->num_instances; iter++) {
    stat = cpaCySetAddressTranslation(
        qcc_inst->cy_inst_handles[iter], qaeVirtToPhysNUMA);
    if (stat == CPA_STATUS_SUCCESS) {
      ...
      if (stat != CPA_STATUS_SUCCESS) {
        dout(1) << "QAT init error, Unable to register callback function for "
                   "instance "
                << iter << " with status = " << stat << dendl;
        if (stat == CPA_STATUS_FAIL) {
          dout(1) << "QAT init: check that 'ServicesEnabled' in "
                     "'/etc/sysconfig/qat' is configured with 'sym'"
                  << dendl;
        }
        return false;
      }
    } else {
      dout(1) << "Unable to find address translations of instance " << iter
              << dendl;
      this->cleanup();
      return false;
    }
  ...
  qat_poll_thread =
      make_named_thread("qat_poll", &QccCrypto::poll_instances, this);
...
QccCrypto::destroy()
{
  ...
  for (iter = 0; iter < qcc_inst->num_instances; iter++) {
    cpaCySymDpRemoveSession(
        qcc_inst->cy_inst_handles[iter], qcc_sess[iter].sess_ctx);
    ...
    optional_yield y)
{
  ...
  */
  if (qcc_op_mem[avail_inst].is_mem_alloc == false) {
    for (size_t i = 0; i < MAX_NUM_SYM_REQ_BATCH; i++) {
      // Allocate IV memory
      status = qcc_contig_mem_alloc(
          (void**)&(qcc_op_mem[avail_inst].iv_buff[i]), AES_256_IV_LEN, 8);
      ...
      // Allocate src memory
      status = qcc_contig_mem_alloc(
          (void**)&(qcc_op_mem[avail_inst].src_buff[i]), chunk_size, 8);
      if (status != CPA_STATUS_SUCCESS) {
      ...
      //Setup OpData
      status = qcc_contig_mem_alloc(
          (void**)&(qcc_op_mem[avail_inst].sym_op_data[i]),
    ...
    status = initSession(
        qcc_inst->cy_inst_handles[avail_inst], &(qcc_sess[avail_inst].sess_ctx),
        (Cpa8U*)key, op_type);
  } else {
    do {
      cpaCySymDpRemoveSession(
          qcc_inst->cy_inst_handles[avail_inst], qcc_sess[avail_inst].sess_ctx);
      status = initSession(
          qcc_inst->cy_inst_handles[avail_inst],
          &(qcc_sess[avail_inst].sess_ctx), (Cpa8U*)key, op_type);
  ...
  return symPerformOp(
      avail_inst, qcc_sess[avail_inst].sess_ctx, in, out, size,
      reinterpret_cast<Cpa8U*>(iv), AES_256_IV_LEN, y);
}
    ...
    CpaCySymCipherDirection cipherDirection)
{
  ...
  if (unlikely(status != CPA_STATUS_SUCCESS)) {
    dout(10) << "cpaCySymUpdateSession failed with status = " << status
             << dendl;
  }
    ...
    CpaCySymCipherDirection cipherDirection)
{
  ...
  if (nullptr == *sessionCtx) {
    ...
    if (likely(CPA_STATUS_SUCCESS == status)) {
      ...
      dout(1) << "cpaCySymDpSessionCtxGetSize failed with status = " << status
              << dendl;
    }
  }
  if (likely(CPA_STATUS_SUCCESS == status)) {
    ...
    if (unlikely(status != CPA_STATUS_SUCCESS)) {
      dout(1) << "cpaCySymDpInitSession failed with status = " << status
              << dendl;
    }
  } else {
...
template <typename CompletionToken>
auto
QatCrypto::async_perform_op(
    std::span<CpaCySymDpOpData*> pOpDataVec,
    CompletionToken&& token)
{
  ...
  return boost::asio::async_initiate<CompletionToken, Signature>(
      [this](auto handler, std::span<CpaCySymDpOpData*> pOpDataVec) {
        ...
        CpaStatus status = cpaCySymDpEnqueueOpBatch(
            pOpDataVec.size(), pOpDataVec.data(), CPA_TRUE);

        if (status != CPA_STATUS_SUCCESS) {
          boost::asio::post(bind_executor(
              ex, boost::asio::append(std::move(completion_handler), status)));
        }
      },
      token, pOpDataVec);
}
    ...
    optional_yield y)
{
  ...
  for (Cpa32U off = 0; off < size; off += one_batch_size) {
    ...
    boost::container::static_vector<CpaCySymDpOpData*, MAX_NUM_SYM_REQ_BATCH>
        pOpDataVec;
    for (Cpa32U offset = off, i = 0; offset < size && i < MAX_NUM_SYM_REQ_BATCH;
         offset += chunk_size, i++) {
      ...
      Cpa32U process_size = offset + chunk_size <= size ? chunk_size
                                                        : size - offset;
    ...
    do {
      ...
      if (y) {
        boost::asio::yield_context yield = y.get_yield_context();
        status = helper.async_perform_op(
            std::span<CpaCySymDpOpData*>(pOpDataVec), yield);
      } else {
        auto result = helper.async_perform_op(
            std::span<CpaCySymDpOpData*>(pOpDataVec), boost::asio::use_future);
        status = result.get();
      }
    ...
    if (likely(CPA_STATUS_SUCCESS == status)) {
      for (Cpa32U offset = off, i = 0;
           offset < size && i < MAX_NUM_SYM_REQ_BATCH;
           offset += chunk_size, i++) {
        Cpa8U* pSrcBuffer = qcc_op_mem[avail_inst].src_buff[i];
        Cpa32U process_size = offset + chunk_size <= size ? chunk_size
                                                          : size - offset;
  ...
  Cpa32U max_used_buffer_num = iv_index > MAX_NUM_SYM_REQ_BATCH
                                   ? MAX_NUM_SYM_REQ_BATCH
                                   : iv_index;
