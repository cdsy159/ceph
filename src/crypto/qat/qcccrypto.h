...
class QccCrypto {
  ...
  boost::circular_buffer<boost::asio::any_completion_handler<void(int)>>
...
private:
  ...
  qcc_contig_mem_alloc(void** ptr, Cpa32U size, Cpa32U alignment = 1)
  {
  ...
  static inline CpaStatus
  qcc_os_mem_alloc(void** ptr, Cpa32U size)
  {
    *ptr = malloc(size);
    if (*ptr == NULL) {
      return CPA_STATUS_RESOURCE;
  ...
  CpaStatus initSession(
      CpaInstanceHandle cyInstHandle,
      CpaCySymSessionCtx* sessionCtx,
      Cpa8U* pCipherKey,
      CpaCySymCipherDirection cipherDirection);

  CpaStatus updateSession(
      CpaCySymSessionCtx sessionCtx,
      Cpa8U* pCipherKey,
      CpaCySymCipherDirection cipherDirection);
};
...
public:
  ...
  QatCrypto(boost::asio::any_io_executor ex) :
    ex(ex), count(0)
  {}
