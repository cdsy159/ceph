...
bool
ConfLine::operator<(const ConfLine& rhs) const
{
...
std::ostream&
operator<<(std::ostream& oss, const ConfLine& l)
{
  oss << "ConfLine(key = '" << l.key << "', val='" << l.val << "', newsection='"
      << l.newsection << "')";
  return oss;
}
    ...
    std::ostream* warnings)
{
  ...
  if (fstat(fileno(fp), &st_buf)) {
    ret = -errno;
    ostringstream oss;
    oss << "read_conf: failed to fstat '" << fname
...
ConfFile::const_section_iter_t
ConfFile::sections_begin() const
{
  return sections.begin();
}

ConfFile::const_section_iter_t
ConfFile::sections_end() const
{
  return sections.end();
}

void
ConfFile::trim_whitespace(std::string& str, bool strip_internal)
{
  // strip preceding
 ...
 */
std::string
ConfFile::normalize_key_name(const std::string& key)
{
  string k(key);
      ...
      if (cur_section->second.lines.count(*cline)) {
        ...
        if (cline->key.length() && warnings)
          *warnings << "warning: line " << line_no << ": '" << cline->key
                    << "' in section '" << cur_section->first << "' redefined "
                    << std::endl;
      }
  ...
  while (true) {
    ...
    case ACCEPT_INIT:
      if (c == '\0')
        ...
        ostringstream oss;
        oss << "unexpected right bracket at char " << (l - line) << ", line "
            << line_no;
        errors->push_back(oss.str());
        return NULL;
      } else if (isspace(c)) {
    ...
    case ACCEPT_KEY:
      if ((((c == '#') || (c == ';')) && (!escaping)) || (c == '\0')) {
        ...
        if (key.empty()) {
          ostringstream oss;
          oss << "error parsing key name: no key name found? " << "at char "
              << (l - line) << ", line " << line_no;
          errors->push_back(oss.str());
          return NULL;
        }
