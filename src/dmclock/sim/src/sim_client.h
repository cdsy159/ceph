...
public:
  ...
  using SubmitFunc = std::function<
      void(const ServerId&, TestRequest&&, const ClientId&, const ReqPm&)>;
    ...
    for (auto i : instructions) {
      if (CliOp::wait == i.op) {
        ...
        for (uint64_t o = 0; o < i.args.req_params.count; ++o) {
          ...
          ReqPm rp = time_stats_w_return<
              decltype(internal_stats.get_req_params_time), ReqPm>(
              internal_stats.mtx, internal_stats.get_req_params_time,
              [&]() -> ReqPm { return service_tracker.get_req_params(server); });
          count_stats(internal_stats.mtx, internal_stats.get_req_params_count);

          submit_f(
              server, TestRequest{server, static_cast<uint32_t>(o), 12}, id, rp);
          ++outstanding_ops;
    ...
    const auto proc_resp = [this, &op, &l](const bool notify_req_cv) {
      if (!resp_queue.empty()) {
        ...
        time_stats(internal_stats.mtx, internal_stats.track_resp_time, [&]() {
          service_tracker.track_resp(
              item.server_id, item.resp_params, item.request_cost);
        });
