...
public:
  ...
  using ClientRespFunc = std::function<
      void(ClientId, const TestResponse&, const ServerId&, const RespPm&, const Cost)>;

  using ServerAccumFunc =
      std::function<void(Accum& accumulator, const RespPm& additional)>;
...
public:
  ...
  using HandleRequestFunc = std::function<
      void(const ClientId&, std::unique_ptr<TestRequest>, const RespPm&, uint64_t)>;
  using CreateQueueF =
      std::function<Q*(CanHandleRequestFunc, HandleRequestFunc)>;
  ...
  SimulatedServer(
    ...
    id(_id),
    priority_queue(_create_queue_f(
        std::bind(&SimulatedServer::has_avail_thread, this),
        std::bind(
            &SimulatedServer::inner_post,
            this,
            std::placeholders::_1,
            std::placeholders::_2,
            std::placeholders::_3,
    ...
    accum_f(_accum_f)
  {
    op_time = std::chrono::microseconds((int)(0.5 + thread_pool_size *
                                                        1000000.0 / iops));
    std::chrono::milliseconds finishing_check_period(1000);
    threads = new std::thread[thread_pool_size];
    for (size_t i = 0; i < thread_pool_size; ++i) {
      threads[i] =
          std::thread(&SimulatedServer::run, this, finishing_check_period);
    }
  }
      ...
      const Cost request_cost)
  {
    time_stats(internal_stats.mtx, internal_stats.add_request_time, [&]() {
      priority_queue->add_request(
          std::move(request), client_id, req_params, request_cost);
    });
  ...
  const Q&
  get_priority_queue() const
  {
    return *priority_queue;
  }

  const InternalStats&
  get_internal_stats() const
  {
  ...
  run(std::chrono::milliseconds check_period)
  {
    ...
    while (true) {
      ...
      if (!inner_queue.empty()) {
        ...
        client_resp_f(
            client, TestResponse{req->epoch}, id, additional, request_cost);

        time_stats(
            internal_stats.mtx, internal_stats.request_complete_time,
