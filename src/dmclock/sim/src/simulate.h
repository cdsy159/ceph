...
public:
  ...
  using ServerDataOutF = std::function<void(
      std::ostream& out,
      Simulation* sim,
      ServerFilter,
      int header_w,
  ...
  using ClientDataOutF = std::function<void(
      std::ostream& out,
      Simulation* sim,
      ClientFilter,
      int header_w,
  ...
  const ServerId&
  get_server_id(std::size_t index) const
  {
    return server_ids[index];
  }

  void
  add_servers(unsigned count, std::function<TS*(ServerId)> create_server_f)
  {
  ...
  void
  add_clients(unsigned count, std::function<TC*(ClientId)> create_client_f)
  {
  ...
  run()
  {
    ...
    std::cout << "simulation completed in "
              << std::chrono::duration_cast<std::chrono::milliseconds>(
                     clients_finished_time - servers_created_time)
                     .count()
              << " millisecs" << std::endl;
  ...
  template <typename T>
  void
  display_server_internal_stats(std::ostream& out, const std::string& time_unit)
  {
    ...
    double add_request_time_per_unit = double(add_request_time.count()) /
                                       add_request_count;
    out << "total time to add requests: " << std::fixed
        << add_request_time.count() << " " << time_unit << ";" << std::endl
        << "    count: " << add_request_count << ";" << std::endl
        << "    average: " << add_request_time_per_unit << " " << time_unit
        << " per request/response" << std::endl;

    double request_complete_time_unit = double(request_complete_time.count()) /
                                        request_complete_count;
    out << "total time to note requests complete: " << std::fixed
        << request_complete_time.count() << " " << time_unit << ";" << std::endl
        << "    count: " << request_complete_count << ";" << std::endl
        << "    average: " << request_complete_time_unit << " " << time_unit
        << " per request/response" << std::endl;
  ...
  template <typename T>
  void
  display_client_internal_stats(std::ostream& out, const std::string& time_unit)
  {
    T track_resp_time(0);
    ...
    double track_resp_time_unit = double(track_resp_time.count()) /
                                  track_resp_count;
    out << "total time to track responses: " << std::fixed
        << track_resp_time.count() << " " << time_unit << ";" << std::endl
        << "    count: " << track_resp_count << ";" << std::endl
        << "    average: " << track_resp_time_unit << " " << time_unit
        << " per request/response" << std::endl;

    double get_req_params_time_unit = double(get_req_params_time.count()) /
                                      get_req_params_count;
    out << "total time to get request parameters: " << std::fixed
        << get_req_params_time.count() << " " << time_unit << ";" << std::endl
    ...
    out << "client timing for QOS algorithm: "
        << track_resp_time_unit + get_req_params_time_unit << " " << time_unit
        << " per request/response" << std::endl;
  }
  ...
  const ServerId&
  server_select_alternate(uint64_t seed, uint16_t client_idx)
  {
  ...
  {
    return [servers_per,
            this](uint64_t seed, uint16_t client_idx) -> const ServerId& {
      double factor = double(server_count) / client_count;
      size_t offset = seed % servers_per;
      size_t index = (size_t(0.5 + client_idx * factor) + offset) %
                     server_count;
      return server_ids[index];
    };
  }
  ...
  const ServerId&
  server_select_random(uint64_t seed, uint16_t client_idx)
  {
  ...
  const ServerId&
  server_select_0(uint64_t seed, uint16_t client_idx)
  {
    return server_ids[0];
  }
}; // class Simulation
