...
main(int argc, char* argv[])
{
  ...
  test::SubmitFunc server_post_f =
      [&simulation](
          const ServerId& server_id, sim::TestRequest&& request,
          const ClientId& client_id, const ssched::ReqParams& req_params) {
  ...
  test::SimpleServer::ClientRespFunc client_response_f =
      [&simulation](
          ...
          const ServerId& server_id, const ssched::NullData& resp_params,
          Cost request_cost) {
        simulation->get_client(client_id).receive_response(
            resp, server_id, resp_params, request_cost);
      };
  ...
  auto create_server_f = [&](ServerId id) -> test::SimpleServer* {
    return new test::SimpleServer(
        id, server_iops, server_threads, client_response_f,
        test::simple_server_accumulate_f, create_queue_f);
  };

  auto create_client_f = [&](ClientId id) -> test::SimpleClient* {
    return new test::SimpleClient(
