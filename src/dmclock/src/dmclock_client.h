...
public:
  ...
  static inline OrigTracker
  create(Counter the_delta, Counter the_rho)
  {
    return OrigTracker(the_delta, the_rho);
  }

  inline ReqParams
  prepare_req(Counter& the_delta, Counter& the_rho)
  {
  ...
  inline void
  resp_update(PhaseType phase, Counter& the_delta, Counter& the_rho, Cost cost)
  {
    the_delta += cost;
    my_delta += cost;
...
public:
  ...
  static inline BorrowingTracker
  create(Counter the_delta, Counter the_rho)
  {
  ...
  inline void
  resp_update(PhaseType phase, Counter& the_delta, Counter& the_rho, Counter cost)
  {
    the_delta += cost;
    if (phase == PhaseType::reservation) {
      the_rho += cost;
...
public:
  ...
  ServiceTracker(
      std::chrono::duration<Rep, Per> _clean_every,
      std::chrono::duration<Rep, Per> _clean_age) :
    delta_counter(1),
    rho_counter(1),
    clean_age(std::chrono::duration_cast<Duration>(_clean_age))
  {
    cleaning_job = std::unique_ptr<RunEvery>(
        new RunEvery(_clean_every, std::bind(&ServiceTracker::do_clean, this)));
  }
      ...
      Counter request_cost = 1u)
  {
    ...
    if (server_map.end() == it) {
      ...
      auto i =
          server_map.emplace(server_id, T::create(delta_counter, rho_counter));
      it = i.first;
    }
