...
struct ClientInfo {
  ...
  inline void
  update(double _reservation, double _weight, double _limit)
  {
  ...
  friend std::ostream&
  operator<<(std::ostream& out, const ClientInfo& client)
  {
    out << "{ ClientInfo:: r:" << client.reservation << " w:" << std::fixed
        << client.weight << " l:" << std::fixed << client.limit
        << " 1/r:" << std::fixed << client.reservation_inv
        << " 1/w:" << std::fixed << client.weight_inv << " 1/l:" << std::fixed
        << client.limit_inv << " }";
    return out;
  }
}; // class ClientInfo
...
struct RequestTag {
  ...
  RequestTag(
    ...
    delta(_delta), rho(_rho), cost(_cost), ready(false), arrival(time)
  {
    assert(cost > 0);
    Time max_time = time;
    ...
    reservation = tag_calc(
        max_time, prev_tag.reservation, client.reservation_inv, rho, true, cost);
    proportion = tag_calc(
        max_time, prev_tag.proportion, client.weight_inv, delta, true, cost);
    limit = tag_calc(
        max_time, prev_tag.limit, client.limit_inv, delta, false, cost);
  ...
  static std::string
  format_tag_change(double before, double after)
  {
    if (before == after) {
...
private:
  ...
  friend std::ostream&
  operator<<(std::ostream& out, const RequestTag& tag)
  {
    out << "{ RequestTag:: ready:" << (tag.ready ? "true" : "false")
...
protected:
  ...
  public:
    ClientReq(const RequestTag& _tag, const C& _client_id, RequestRef&& _request) :
      tag(_tag), client_id(_client_id), request(std::move(_request))
    {
      // empty
    }

    friend std::ostream&
    operator<<(std::ostream& out, const ClientReq& c)
    {
...
public:
  ...
  public:
    ...
    static inline void
    assign_unpinned_tag(double& lhs, const double rhs)
    {
    ...
    inline void
    update_req_tag(const RequestTag& _prev, const Counter& _tick)
    {
    ...
    inline void
    add_request(const RequestTag& tag, RequestRef&& request)
    {
      requests.emplace_back(tag, client, std::move(request));
    }

    inline const ClientReq&
    next_request() const
    {
    ...
    bool
    remove_by_req_filter_fw(std::function<bool(RequestRef&&)> filter_accum)
    {
    ...
    // operation might be expensive
    bool
    remove_by_req_filter_bw(std::function<bool(RequestRef&&)> filter_accum)
    {
      bool any_removed = false;
    ...
    remove_by_req_filter(
        std::function<bool(RequestRef&&)> filter_accum,
    ...
    friend std::ostream&
    operator<<(std::ostream& out, const typename PriorityQueueBase::ClientRec& e)
    {
      out << "{ ClientRec::" << " client:" << e.client
          << " prev_tag:" << e.prev_tag << " req_count:" << e.requests.size()
          << " top_req:";
      if (e.has_request()) {
        out << e.next_request();
      } else {
  ...
  struct NextReq {
    ...
    inline NextReq(HeapId _heap_id) :
      type(NextReqType::returning), heap_id(_heap_id)
    {}

    inline NextReq(Time _when_ready) :
      type(NextReqType::future), when_ready(_when_ready)
    {}

    // calls to this are clearer than calls to the default
    // constructor
    static inline NextReq
    none()
    {
      return NextReq();
    }
      ...
      std::function<void(RequestRef&&)> accum = request_sink)
  {
    ...
    if (reverse) {
      for (auto j = i->second->requests.rbegin();
           j != i->second->requests.rend(); ++j) {
        accum(std::move(j->request));
      }
    } else {
      for (auto j = i->second->requests.begin(); j != i->second->requests.end();
           ++j) {
        accum(std::move(j->request));
      }
    }
  ...
  void
  update_client_info(const C& client_id)
  {
    DataGuard g(data_mtx);
  ...
  friend std::ostream&
  operator<<(std::ostream& out, const PriorityQueueBase& q)
  {
...
protected:
  ...
  struct ClientCompare {
    bool
    operator()(const ClientRec& n1, const ClientRec& n2) const
    {
      if (n1.has_request()) {
        if (n2.has_request()) {
  ...
  c::IndIntruHeap<
      ...
      ClientCompare<&RequestTag::proportion, ReadyOption::ignore, true>,
      B>
      prop_heap;
#endif
  c::IndIntruHeap<
  ...
  static T
  get_or_default(const Variant& param, T default_value)
  {
  ...
  inline const ClientInfo*
  get_cli_info(ClientRec& client) const
  {
      ...
      Cost cost)
  {
    ...
    if (!client.has_request()) {
      ...
      assert(client_info);
      tag = RequestTag(
          client.get_req_tag(), *client_info, params, time, cost,
          anticipation_timeout);
  ...
  // data_mtx must be held by caller
  void
  update_next_tag(DelayedTagCalc delayed, ClientRec& top, const RequestTag& tag)
  {
    if (top.has_request()) {
      ...
      assert(client_info);
      next_first.tag = RequestTag(
          tag, *client_info, top.cur_delta, top.cur_rho, next_first.tag.arrival,
  ...
  void
  update_next_tag(ImmediateTagCalc imm, ClientRec& top, const RequestTag& tag)
  {
  ...
  pop_process_request(
      ...
      std::function<void(const C& client, const Cost cost, RequestRef& request)>
          process)
  {
    // gain access to data
  ...
  // data_mtx should be held when called
  void
  reduce_reservation_tags(const C& client_id, const RequestTag& tag)
  {
    auto client_it = client_map.find(client_id);
  ...
  do_next_request(Time now)
  {
    ...
    while (limits->has_request() && !limits->next_request().tag.ready &&
    ...
    if (resv_heap.top().has_request()) {
      next_call = min_not_0_time(
          next_call, resv_heap.top().next_request().tag.reservation);
    }
  ...
  static inline const Time&
  min_not_0_time(const Time& current, const Time& possible)
  {
    ...
    if (erase_point > 0 || idle_point > 0) {
      for (auto i = client_map.begin(); i != client_map.end(); /* empty */) {
        ...
        if (!(i2->second->has_request()) && erase_point &&
            erased_num < erase_max && i2->second->last_tick <= erase_point) {
  ...
  // data_mtx must be held by caller
  void
  delete_from_heaps(ClientRecRef& client)
  {
...
class PullPriorityQueue
  : public PriorityQueueBase<C, R, IsDelayed, U1, B, BackgroundJob> {
...
public:
  ...
  struct PullReq {
    ...
    is_none() const
    {
      return type == super::NextReqType::none;
    }

    bool
    is_retn() const
    {
      return type == super::NextReqType::returning;
    }
    ...
    is_future() const
    {
      return type == super::NextReqType::future;
    }
  ...
  template <typename Rep, typename Per>
  PullPriorityQueue(
      typename super::ClientInfoFunc _client_info_f,
      std::chrono::duration<Rep, Per> _idle_age,
      std::chrono::duration<Rep, Per> _erase_age,
      std::chrono::duration<Rep, Per> _check_time,
  ...
  {
    return add_request(
        typename super::RequestRef(new R(std::move(request))), client_id,
        req_params, get_time(), cost);
  }

  int
  add_request(R&& request, const C& client_id, const Cost cost = 1u)
  {
    static const ReqParams null_req_params;
    return add_request(
        typename super::RequestRef(new R(std::move(request))), client_id,
        null_req_params, get_time(), cost);
  }
  ...
  {
    return add_request(
        std::move(request), client_id, req_params, get_time(), cost);
  }
      ...
      const Cost cost = 1u)
  {
    static const ReqParams null_req_params;
    return add_request(
        std::move(request), client_id, null_req_params, get_time(), cost);
  }
...
#endif
    int r = super::do_add_request(
        std::move(request), client_id, req_params, time, cost);
  ...
  pull_request(const Time now)
  {
    ...
    auto process_f = [&](PullReq& pull_result, PhaseType phase)
        -> std::function<void(const C&, uint64_t, typename super::RequestRef&)> {
      return [&pull_result, phase](
                 const C& client, const Cost request_cost,
                 typename super::RequestRef& request) {
        pull_result.data = typename PullReq::Retn{
            client, std::move(request), phase, request_cost};
      };
    };

    switch (next.heap_id) {
    case super::HeapId::reservation:
      (void)super::pop_process_request(
          this->resv_heap, process_f(result, PhaseType::reservation));
      ++this->reserv_sched_count;
      break;
    case super::HeapId::ready: {
      auto tag = super::pop_process_request(
          this->ready_heap, process_f(result, PhaseType::priority));
...
public:
  ...
  using HandleRequestFunc = std::function<
      void(const C&, typename super::RequestRef, PhaseType, uint64_t)>;
...
public:
  ...
  {
    return add_request(
        typename super::RequestRef(new R(request)), client_id, req_params, time,
        cost);
  }
...
protected:
  ...
  submit_top_request(
      IndIntruHeap<C1, typename super::ClientRec, C2, C3, B4>& heap,
      PhaseType phase)
  {
    C client_result;
    RequestTag tag = super::pop_process_request(
        heap, [this, phase, &client_result](
                  const C& client, const Cost request_cost,
                  typename super::RequestRef& request) {
  ...
  // data_mtx should be held when called
  void
  submit_request(typename super::HeapId heap_id)
  {
    switch (heap_id) {
    case super::HeapId::reservation:
    ...
    while (!this->finishing) {
      ...
      if (TimeZero == sched_ahead_when) {
        ...
        const auto until = typename super::TimePoint{
            duration_cast<typename super::Duration>(std::chrono::duration<Time>{
                sched_ahead_when})};
