...
class IndIntruHeap {
  ...
  public:
    Iterator(Iterator&& other) :
      heap(other.heap), index(other.index)
    {
      // empty
    }
  ...
  class ConstIterator {
    ...
    ConstIterator(
        const IndIntruHeap<I, T, heap_info, C, K>& _heap,
        HeapIndex _index) :
      heap(&_heap), index(_index)
    {
      // empty
...
public:
  ...
  friend std::ostream&
  operator<<(std::ostream& out, const IndIntruHeap& h)
  {
...
protected:
  static IndIntruHeapData&
  intru_data_of(const I& item)
  {
  ...
  static bool
  all_filter(const T& data)
  {
    return true;
  }

  // when i is negative?
  static inline HeapIndex
  parent(HeapIndex i)
  {
    assert(0 != i);
    return (i - 1) / K;
  ...
  typename std::enable_if<(K > 2) && EnableBool, void>::type
