...
TEST(dmclock_client, server_erase)
{
  ...
  dmc::ServiceTracker<ServerId> st(
  ...
  lock_st([&]() {
    EXPECT_EQ(0u, st.server_map.size()) << "server map initially has size 0";
  });
...
TEST(dmclock_client, delta_rho_values_borrowing_tracker)
{
  ...
  dmc::ServiceTracker<ServerId, dmc::BorrowingTracker> st(
      std::chrono::seconds(2), std::chrono::seconds(3));
...
TEST(dmclock_client, delta_rho_values_orig_tracker)
{
  ...
  dmc::ServiceTracker<ServerId, OrigTracker> st(
      std::chrono::seconds(2), std::chrono::seconds(3));
