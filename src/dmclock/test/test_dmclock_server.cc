...
/*
     ...
     */
static void
test_locked(std::mutex& mtx, std::function<void()> code)
{
...
TEST(dmclock_server, bad_tag_deathtest)
{
  ...
  EXPECT_DEATH_IF_SUPPORTED(
      Queue(client_info_f, AtLimit::Reject), "Assertion.*Reject.*Delayed")
...
TEST(dmclock_server, client_idle_erase)
{
  ...
  Queue pq(
      client_info_f, server_ready_f, submit_req_f, std::chrono::seconds(3),
      std::chrono::seconds(5), std::chrono::seconds(2), AtLimit::Wait);
  ...
  lock_pq([&]() {
    EXPECT_EQ(0u, pq.client_map.size()) << "client map initially has size 0";
  });
...
TEST(dmclock_server, add_req_pushprio_queue)
{
  ...
  auto client_info_f = [&](ClientId c) -> const dmc::ClientInfo* { return &ci; };
  auto server_ready_f = []() -> bool { return true; };
  auto submit_req_f = [](const ClientId& c, std::unique_ptr<MyReq> req,
                         dmc::PhaseType phase, uint64_t req_cost) {
    // empty; do nothing
  };
...
TEST(dmclock_server, remove_by_client)
{
  ...
  pq.remove_by_client(client1, true, [&removed](MyReqRef&& r) {
...
TEST(dmclock_server_pull, dynamic_cli_info_f)
{
  ...
  dmc::ClientInfo info1[] = {
      dmc::ClientInfo(0.0, 100.0, 0.0), dmc::ClientInfo(0.0, 150.0, 0.0)};
  dmc::ClientInfo info2[] = {
      dmc::ClientInfo(0.0, 200.0, 0.0), dmc::ClientInfo(0.0, 50.0, 0.0)};
  ...
  auto run_test = [&](float lower_bound, float upper_bound) {
    ...
    float prop1 = float(info1[cli_info_group].weight) /
                  (info1[cli_info_group].weight + info2[cli_info_group].weight);
    float prop2 = float(info2[cli_info_group].weight) /
                  (info1[cli_info_group].weight + info2[cli_info_group].weight);
    EXPECT_NEAR(float(c1_count) / (c1_count + c2_count), prop1, tolerance)
        << "before: " << prop1
        << " of requests should have come from first client";
    EXPECT_NEAR(float(c2_count) / (c1_count + c2_count), prop2, tolerance)
        << "before: " << prop2
        << " of requests should have come from second client";
  };
  cli_info_group = 0;
...
TEST(dmclock_server_pull, ready_and_under_limit)
{
  ...
  for (int i = 0; i < 3; ++i) {
    EXPECT_EQ(
        0, pq->add_request_time(Request{}, client1, req_params, start_time));
    EXPECT_EQ(
        0, pq->add_request_time(Request{}, client2, req_params, start_time));
  }
