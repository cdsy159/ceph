...
int
ErasureCode::init(ErasureCodeProfile& profile, std::ostream* ss)
{
  int err = 0;
  err |= to_string("crush-root", profile, &rule_root, DEFAULT_RULE_ROOT, ss);
  err |= to_string(
      "crush-failure-domain", profile, &rule_failure_domain,
      DEFAULT_RULE_FAILURE_DOMAIN, ss);
  err |= to_int(
      "crush-osds-per-failure-domain", profile, &rule_osds_per_failure_domain,
      "0", ss);
  err |= to_int(
      "crush-num-failure-domains", profile, &rule_num_failure_domains, "0", ss);
    ...
    std::ostream* ss) const
{
  if (rule_osds_per_failure_domain <= 1) {
    return crush.add_simple_rule(
        name, rule_root, rule_failure_domain, rule_num_failure_domains,
        rule_device_class, "indep", pg_pool_t::TYPE_ERASURE, ss);
  } else {
    ...
    return crush.add_indep_multi_osd_per_failure_domain_rule(
        name, rule_root, rule_failure_domain, rule_num_failure_domains,
        rule_osds_per_failure_domain, rule_device_class, ss);
  }
}

int
ErasureCode::sanity_check_k_m(int k, int m, ostream* ss)
{
  if (k < 2) {
...
shard_id_t
ErasureCode::chunk_index(raw_shard_id_t i) const
{
  return chunk_mapping.size() > uint64_t(i) ? chunk_mapping[static_cast<int>(i)]
                                            : shard_id_t(int8_t(i));
}

[[deprecated]]
unsigned int
ErasureCode::chunk_index(unsigned int i) const
{
  return static_cast<unsigned int>(
      chunk_mapping.size() > uint64_t(i) ? chunk_mapping[i] : shard_id_t(i));
}

[[deprecated]]
int
ErasureCode::_minimum_to_decode(
...
[[deprecated]]
int
ErasureCode::minimum_to_decode_with_cost(
    const set<int>& want_to_read,
    ...
    shard_id_map<bufferlist>& encoded) const
{
  ...
  if (padded_chunks) {
    ...
    raw.begin((k - padded_chunks) * blocksize).copy(remainder, buf.c_str());
    buf.zero(remainder, blocksize - remainder);
    encoded[chunk_index(raw_shard_id_t(k - padded_chunks))].push_back(
        std::move(buf));
    ...
    shard_id_map<bufferlist>* decoded)
{
  ...
  for (auto&& [shard, list] : *decoded) {
    ...
    if (erasures.find(shard) == erasures.end())
      in[shard] = bp;
    else
      out[shard] = bp;
  }
...
int
ErasureCode::parse(const ErasureCodeProfile& profile, ostream* ss)
{
  return to_mapping(profile, ss);
}

const vector<shard_id_t>&
ErasureCode::get_chunk_mapping() const
{
  return chunk_mapping;
}

int
ErasureCode::to_mapping(const ErasureCodeProfile& profile, ostream* ss)
{
  if (profile.find("mapping") != profile.end()) {
    ...
    chunk_mapping.insert(
        chunk_mapping.end(), coding_chunk_mapping.begin(),
