...
int
ErasureCodeClay::init(ErasureCodeProfile& profile, ostream* ss)
{
  int r;
  r = parse(profile, ss);
  if (r)
  ...
  r = registry.factory(
      mds.profile["plugin"], directory, mds.profile, &mds.erasure_code, ss);
  if (r)
    return r;
  r = registry.factory(
      pft.profile["plugin"], directory, pft.profile, &pft.erasure_code, ss);
  return r;
}

unsigned int
ErasureCodeClay::get_chunk_size(unsigned int stripe_width) const
{
...
size_t
ErasureCodeClay::get_minimum_granularity()
{
...
[[deprecated]]
int
ErasureCodeClay::minimum_to_decode(
...
int
ErasureCodeClay::parse(ErasureCodeProfile& profile, ostream* ss)
{
  int err = 0;
  ...
  if ((d < k + 1) || (d > k + m - 1)) {
    *ss << "value of d " << d << " must be within [" << k + 1 << ","
        << k + m - 1 << "]" << std::endl;
    err = -EINVAL;
    return err;
  }
  ...
  dout(10) << __func__ << " (q,t,nu)=(" << q << "," << t << "," << nu << ")"
           << dendl;
    ...
    const set<int>& available_chunks)
{
  ...
  if (includes(
          available_chunks.begin(), available_chunks.end(),
...
int
ErasureCodeClay::minimum_to_repair(
    const set<int>& want_to_read,
    const set<int>& available_chunks,
    map<int, vector<pair<int, int>>>* minimum)
{
...
int
ErasureCodeClay::get_repair_sub_chunk_count(const set<int>& want_to_read)
{
  int weight_vector[t];
    ...
    int chunk_size)
{
  ...
  ceph_assert(
      helper_data.size() + aloof_nodes.size() + recovered_data.size() ==
      (unsigned)q * t);

  int r = repair_one_lost_chunk(
...
int
ErasureCodeClay::repair_one_lost_chunk(
    map<int, bufferlist>& recovered_data,
    set<int>& aloof_nodes,
    map<int, bufferlist>& helper_data,
          ...
          if (erasures.count(node_xy) == 0) {
            ...
            if (aloof_nodes.count(node_sw) > 0) {
              ...
              known_subchunks[i0].substr_of(
                  helper_data[node_xy], repair_plane_to_ind[z] * sub_chunksize,
                  sub_chunksize);
              ...
              if (z_vec[y] != x) {
                ...
                known_subchunks[i0].substr_of(
                    helper_data[node_xy],
                    repair_plane_to_ind[z] * sub_chunksize, sub_chunksize);
                known_subchunks[i1].substr_of(
                    helper_data[node_sw],
                    repair_plane_to_ind[z_sw] * sub_chunksize, sub_chunksize);
                pftsubchunks[i0] = known_subchunks[i0];
      ...
      for (auto i : erasures) {
        ...
        if (aloof_nodes.count(i) == 0) {
          if (x == z_vec[y]) { // hole-dot pair (type 0)
            ...
            memcpy(
                &coupled_chunk[z * sub_chunksize],
                &uncoupled_chunk[z * sub_chunksize], sub_chunksize);
          } else {
            ...
            known_subchunks[i0].substr_of(
                helper_data[i], repair_plane_to_ind[z] * sub_chunksize,
                sub_chunksize);
...
int
ErasureCodeClay::decode_erasures(
    ...
    for (int y = 0; y < t; y++) {
      ...
      if (erased_chunks.count(node_xy) == 0) {
        if (z_vec[y] < x) {
          ...
          memcpy(
              &uncoupled_chunk[z * sc_size], &coupled_chunk[z * sc_size],
              sc_size);
        } else {
          if (erased_chunks.count(node_sw) > 0) {
            get_uncoupled_from_coupled(chunks, x, y, z, z_vec, sc_size);
...
void
ErasureCodeClay::set_planes_sequential_decoding_order(
    int* order,
    set<int>& erasures)
{
  int z_vec[t];
...
void
ErasureCodeClay::recover_type1_erasure(
    map<int, bufferlist>* chunks,
    int x,
    int y,
    int z,
...
void
ErasureCodeClay::get_coupled_from_uncoupled(
    map<int, bufferlist>* chunks,
    int x,
    int y,
    int z,
...
void
ErasureCodeClay::get_uncoupled_from_coupled(
    map<int, bufferlist>* chunks,
    int x,
    int y,
    int z,
...
int
ErasureCodeClay::get_max_iscore(set<int>& erased_chunks)
{
  int weight_vec[t];
  int iscore = 0;
...
void
ErasureCodeClay::get_plane_vector(int z, int* z_vec)
{
  for (int i = 0; i < t; i++) {
    z_vec[t - 1 - i] = z % q;
    z = (z - z_vec[t - 1 - i]) / q;
