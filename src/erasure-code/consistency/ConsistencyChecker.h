...
public:
  ...
  ConsistencyCheckResult(std::string oid, std::string error_message, bool result) :
    oid(oid), error_message(error_message), result(result)
  {}
};
...
private:
  ...
  std::pair<bufferlist, bufferlist> split_data_and_parity(
...
public:
  ...
  bool check_object_consistency(const std::string& oid, const bufferlist& inbl);
  void print_results(std::ostream& out);
  void clear_results();
  bool single_read_and_check_consistency(
      const std::string& oid,
      int block_size,
      int offset,
      int length);
