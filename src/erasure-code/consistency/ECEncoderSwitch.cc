...
ECEncoderSwitch::ECEncoderSwitch(
    ceph::ErasureCodeProfile profile,
    int chunk_size,
    bool optimizations_enabled) :
  encoder_optimized(
      ceph::consistency::ECEncoder<ECUtil::stripe_info_t>(profile, chunk_size)),
  encoder_legacy(
      ceph::consistency::ECEncoder<stripe_info_l_t>(profile, chunk_size)),
...
ECEncoderSwitch::do_encode(ceph::bufferlist inbl)
{
 ...
 */
int
ECEncoderSwitch::get_chunk_size()
{
  if (optimizations_enabled) {
