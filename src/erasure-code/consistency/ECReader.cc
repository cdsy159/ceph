...
Read::Read(
    ...
    uint64_t offset,
    uint64_t length) :
  oid(oid), block_size(block_size), offset(offset), length(length)
{}
 ...
 */
uint64_t
ECReader::get_object_size(std::string oid)
{
  uint64_t size;
 ...
 */
void
ECReader::do_read(Read read)
{
  start_io();
  librados::ObjectReadOperation op;
  op.read(
      read.get_offset() * read.get_block_size(),
      read.get_length() * read.get_block_size(), nullptr, nullptr);

  std::string oid = read.get_oid();
  auto read_cb =
      [&, oid](boost::system::error_code ec, version_t ver, bufferlist outbl) {
        results.push_back({oid, ec, outbl});
        finish_io();
      };

  librados::async_operate(
      asio.get_executor(), io, read.get_oid(), std::move(op), 0, nullptr,
      read_cb);
}
