...
void
DaemonMetricCollector::request_loop()
{
  timer.async_wait([this](const boost::system::error_code& e) {
    ...
    dump_asok_metrics(
        sort_metrics, prio_limit, true, dump_response, schema_response, true);
...
void
DaemonMetricCollector::shutdown()
{
  shutdown_flag = true;
...
void
DaemonMetricCollector::parse_asok_metrics(
  ...
  try {
    ...
    for (auto& perf_group_item : counter_schema) {
      std::string perf_group = {
          perf_group_item.key().begin(), perf_group_item.key().end()};
        ...
        try {
          ...
          for (auto& counter : counters) {
            try {
              ...
              std::string counter_name_init = {
                  counter.key().begin(), counter.key().end()};
              ...
              if (!multisite_labels_and_name.first.empty()) {
                labels.insert(
                    multisite_labels_and_name.first.begin(),
              ...
              dout(1) << "Exception in counter processing for " << daemon_name
                      << ": " << e.what() << dendl;
              continue;
            }
          }
        } catch (const std::exception& e) {
          dout(1) << "Exception in schema/dump iteration for " << daemon_name
                  << ": " << e.what() << dendl;
          continue;
        }
      }
    ...
    dout(1) << "Exception in parse_asok_metrics for " << daemon_name << ": "
            << e.what() << dendl;
    return;
  }
}
 ...
 */
void
DaemonMetricCollector::dump_asok_metric(
    ...
    labels_t labels)
{
  try {
    if (!perf_info.if_contains("type") ||
      ...
      dout(1) << "Missing required key in perf_info for metric: " << name
              << dendl;
      return;
    }
    ...
    if (!perf_info["metric_type"].is_string()) {
      dout(1) << "Missing or invalid 'metric_type' in perf_info for metric: "
              << name << dendl;
      return;
    }
    ...
    if (!perf_info["description"].is_string()) {
      dout(1) << "Missing or invalid 'description' in perf_info for metric: "
              << name << dendl;
      return;
    }
    std::string description =
    ...
    if (type & PERFCOUNTER_LONGRUNAVG) {
      ...
      if (!perf_obj.if_contains("avgcount")) {
        dout(1) << "Missing 'avgcount' in perf_values for metric: " << name
                << dendl;
        return;
      }
      ...
      add_metric(
          builder, count, name + "_count", description + " Count", "counter",
          labels);
      json_value sum_value = perf_obj["sum"];
      add_double_or_int_metric(
          builder, sum_value, name + "_sum", description + " Total",
          metric_type, labels);
    } else {
      add_double_or_int_metric(
          builder, perf_values, name, description, metric_type, labels);
    }
  } catch (const std::exception& e) {
    dout(1) << "Exception in dump_asok_metric for metric: " << name << ": "
            << e.what() << dendl;
    return;
  }
}
...
DaemonMetricCollector::dump_asok_metrics(
    bool sort_metrics,
    int64_t counter_prio,
    bool sockClientsPing,
    std::string& dump_response,
    std::string& schema_response,
    bool config_show_response)
{
  ...
  for (auto& [daemon_name, sock_client] : clients) {
    if (sockClientsPing) {
      ...
      std::string ceph_daemon_socket_up_desc(
          "Reports the health status of a Ceph daemon, as determined by "
          "whether it is able to respond via its admin socket (1 = healthy, 0 "
          "= unhealthy).");
      ...
      add_metric(
          builder, static_cast<int>(ok), "ceph_daemon_socket_up",
          ceph_daemon_socket_up_desc, "gauge", ceph_daemon_socket_up_labels);
      if (!ok) {
        failures++;
        continue;
      }
    ...
    try {
      parse_asok_metrics(
          counter_dump_response, counter_schema_response, prio_limit,
          daemon_name);

      std::string config_show =
          !config_show_response
              ? ""
  ...
  add_metric(
      builder, timer.get_ms(), "ceph_exporter_scrape_time", scrap_desc, "gauge",
      scrap_labels);
...
void
DaemonMetricCollector::get_process_metrics(
    std::vector<std::pair<std::string, int>> daemon_pids)
{
  ...
  for (auto& [daemon_name, pid] : daemon_pids) {
    ...
    add_metric(
        builder, stat.minflt, "ceph_exporter_minflt_total",
        "Number of minor page faults of daemon", "counter", labels);
    add_metric(
        builder, stat.majflt, "ceph_exporter_majflt_total",
        "Number of major page faults of daemon", "counter", labels);
    add_metric(
        builder, stat.num_threads, "ceph_exporter_num_threads",
        "Number of threads used by daemon", "gauge", labels);
    add_metric(
        builder, usage, "ceph_exporter_cpu_usage", "CPU usage of a daemon",
        "gauge", labels);
    ...
    add_metric(
        builder, kernel_time, "ceph_exporter_cpu_total", cpu_time_desc,
        "counter", cpu_total_labels);
    cpu_total_labels["mode"] = quote("user");
    add_metric(
        builder, user_time, "ceph_exporter_cpu_total", cpu_time_desc, "counter",
        cpu_total_labels);
    cpu_total_labels["mode"] = quote("idle");
    add_metric(
        builder, idle_time, "ceph_exporter_cpu_total", cpu_time_desc, "counter",
        cpu_total_labels);
    add_metric(
        builder, stat.vm_size, "ceph_exporter_vm_size",
        "Virtual memory used in a daemon", "gauge", labels);
    add_metric(
        builder, stat.resident_size, "ceph_exporter_resident_size",
        "Resident memory in a daemon", "gauge", labels);
  }
}

std::string
DaemonMetricCollector::asok_request(
    AdminSocketClient& asok,
    std::string command,
...
labels_t
DaemonMetricCollector::get_extra_labels(std::string daemon_name)
{
  labels_t labels;
...
void
DaemonMetricCollector::update_sockets()
{
...
std::string
UnorderedMetricsBuilder::dump()
{
  return out;
}

void
Metric::add(labels_t labels, std::string value)
{
  metric_entry entry;
  entry.labels = labels;
  entry.value = value;
