...
usage()
{
  std::cout
      ...
      << "options:\n"
         ...
         "  --addrs:        Host IP address on which the exporter is to "
         "listen\n"
         "  --port:         TCP Port on which the exporter is to listen. "
         "Default is 9926\n"
         "  --cert-file:    Path to the certificate file when using HTTPS\n"
         "  --key-file:     Path to the certificate key file when using HTTPS\n"
         "  --prio-limit:   Only perf counters greater than or equal to "
         "prio-limit are fetched. Default: 5\n"
         "  --stats-period: Interval between daemon scrapes (seconds). "
         "Default: 5s"
      << std::endl;
...
main(int argc, char** argv)
{
  ...
  auto cct = global_init(
      NULL, args, CEPH_ENTITY_TYPE_CLIENT, CODE_ENVIRONMENT_DAEMON, 0);
  std::string val;
  for (auto i = args.begin(); i != args.end();) {
    if (ceph_argparse_double_dash(args, i)) {
      ...
      cct->_conf.set_val("exporter_key_file", val);
        ...
        ceph_argparse_witharg(args, i, &val, "--prio-limit", (char*)NULL)) {
      cct->_conf.set_val("exporter_prio_limit", val);
    } else if (
        ceph_argparse_witharg(args, i, &val, "--stats-period", (char*)NULL)) {
