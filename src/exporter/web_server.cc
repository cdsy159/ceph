...
protected:
  ...
  void
  check_deadline(std::shared_ptr<web_connection> self)
  {
  ...
  void
  handle_bad_request(beast::error_code ec)
  {
    response_.version(request_.version());
    response_.keep_alive(request_.keep_alive());
    response_.result(http::status::method_not_allowed);
...
class http_connection : public web_connection,
                        public std::enable_shared_from_this<http_connection> {
public:
  explicit http_connection(tcp::socket socket) :
    web_connection(socket.get_executor(), std::chrono::seconds(60)),
...
private:
  ...
  void
  read_request(std::shared_ptr<http_connection> self)
  {
    http::async_read(
  ...
  void
  close_connection(beast::error_code& ec) override
  {
    socket_.close(ec);
  }
};
...
class https_connection : public web_connection,
                         public std::enable_shared_from_this<https_connection> {
public:
  ...
  start() override
  {
    ...
    socket_.async_handshake(
        ssl::stream_base::server, [self](beast::error_code ec) {
          if (!ec) {
...
private:
  ...
  void
  read_request(std::shared_ptr<https_connection> self)
  {
    http::async_read(
        socket_, buffer_, request_,
        [self](beast::error_code ec, std::size_t bytes_transferred) {
...
void
https_server(tcp::acceptor& acceptor, ssl::context& ssl_ctx)
{
  acceptor.async_accept([&](beast::error_code ec, tcp::socket socket) {
    if (!ec) {
      std::make_shared<https_connection>(
          ssl::stream<tcp::socket>(std::move(socket), ssl_ctx))
          ->start();
    }
...
void
run_http_server(const std::string& exporter_addr, short unsigned int port)
{
  tcp::acceptor acceptor{
      *global_ioc, {net::ip::make_address(exporter_addr), port}};
    ...
    const std::string& key_file)
{
  ...
  dout(1) << "HTTPS server running on " << exporter_addr << ":" << port
          << dendl;
  global_ioc->run();
}
