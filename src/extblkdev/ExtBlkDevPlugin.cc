...
int
get_required_caps(CephContext* cct, cap_t& merge_caps)
{
  ...
  if (ptype != registry->plugins.end()) {
    ...
    for (auto& it : ptype->second) {
      ...
      if (ebdplugin == nullptr) {
        derr << __func__ << " Is not an extblkdev plugin: " << it.first
             << dendl;
        return -ENOENT;
      }
...
int
trim_caps(CephContext* cct, cap_t& merge_caps)
{
...
preload(CephContext* cct)
{
  ...
  auto registry = cct->get_plugin_registry();
  {
    ...
    for (auto& plg : plugins_list) {
      ...
      if (rc) {
        derr << __func__ << " failed preloading extblkdev plugin: " << plg
             << dendl;
        preload_result = rc;
        return rc;
      } else {
    ...
    ExtBlkDevInterfaceRef& ebd_impl)
{
  ...
  if (rc == 0) {
    dout(1) << __func__ << " using plugin " << plg_name << ", volume "
            << ebd_impl->get_devname() << " maps to " << logdevname << dendl;
  } else {
...
int
release_device(ExtBlkDevInterfaceRef& ebd_impl)
{
  if (ebd_impl) {
    ebd_impl.reset();
  }
