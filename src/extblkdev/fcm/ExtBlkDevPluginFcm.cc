...
class ExtBlkDevFcm : public ceph::ExtBlkDevInterface {
  ...
  public:
    ...
    query_log()
    {
      ...
      struct nvme_passthru_cmd cmd = {
          ...
          .data_len = log_size,
          .cdw10 = 202 | ((num_dw & 0xffff)
                          << 16), // log 202 contains phys util number
          .cdw11 = num_dw >> 16,
    ...
    uint64_t
    get_device_physical_size() const
    {
      return log[0];
    }

    uint64_t
    get_device_physical_util() const
    {
      return log[1];
    }

    uint64_t
    get_device_physical_avail() const
    {
    ...
    uint64_t
    get_device_logical_size() const
    {
      return log[2];
    }

    uint64_t
    get_device_logical_util() const
    {
      return log[3];
    }

    uint64_t
    get_device_logical_avail() const
    {
  ...
  int64_t lavail =
      0; // total number of logical available bytes of logical volume
  uint64_t psize = 0; // total number of physical bytes of logical volume
  uint64_t pavail =
      0; // total number of physical available bytes of logical volume
  struct timespec last_access = {0};
  PerfCounters* perfc = nullptr;

  uint64_t
  get_partition_physical_size() const
  {
    return psize;
  }

  uint64_t
  get_partition_logical_size() const
  {
    return lsize;
  }

  uint64_t
  get_partition_physical_avail() const
  {
    return pavail;
  }

  uint64_t
  get_partition_logical_avail() const
  {
    return lavail;
  }
  ...
  static int
  fcm_get_int_property(const std::string& devpath, const std::string& attr)
  {
  ...
  get_lsize()
  {
    ...
    int fd = ::open(path.c_str(), O_RDONLY);
    if (fd < 0) {
      dout(1) << __func__ << " Can not open logical device " << logdevname
              << dendl;
      return -2000 - errno;
    }
    BlkDev bdfd(fd);
    ...
    VOID_TEMP_FAILURE_RETRY(::close(fd));
    if (rc < 0) {
      dout(1) << __func__ << " Can not get device size of " << logdevname
              << dendl;
      return -3000 - errno;
    }
    return 0;
...
public:
  explicit ExtBlkDevFcm(CephContext* cct) :
    cct(cct)
  {}

  ~ExtBlkDevFcm() { unregister_perf_counters(); }

  int
  init(const std::string& logdevname_a)
  {
    ...
    for (auto& d : raw_devices) {
      ...
      if (scan_device_and_vendor(devpath, vendor, device) == 0) {
        ...
        if (vendor == 0x1014 && device == 0x0634) {
          ...
          dout(1) << __func__ << " Found FCM vendor/device id on " << d
                  << dendl;
        }
      }
    }
    ...
    if (rc < 0) {
      derr << __func__
           << " Device detected, but FCM utilization log cannot be accessed "
              "(check capabilites!)"
           << dendl;
      return rc;
    }
  ...
  register_perf_counters()
  {
    ...
    b.add_u64(
        l_dev_phy_size, "dev_phy_size", "FCM device physical size", "", useful,
        ubytes);
    b.add_u64(
        l_dev_log_size, "dev_log_size", "FCM device logical size", "", useful,
        ubytes);
    b.add_u64(
        l_dev_phy_util, "dev_phy_util", "FCM device physical util", "", useful,
        ubytes);
    b.add_u64(
        l_dev_log_util, "dev_log_util", "FCM device logical util", "", useful,
        ubytes);
    b.add_u64(
        l_part_phy_size, "part_phy_size", "FCM partition physical size", "",
        useful, ubytes);
    b.add_u64(
        l_part_log_size, "part_log_size", "FCM partition logical size", "",
        useful, ubytes);
    b.add_u64(
        l_part_phy_avail, "part_phy_avail", "FCM partition physical avail", "",
        useful, ubytes);
    b.add_u64(
        l_part_log_avail, "part_log_avail", "FCM partition logical avail", "",
        useful, ubytes);
  ...
  */
  int
  scan_device_and_vendor(const std::string& e, uint32_t& vendor, uint32_t& device)
  {
    DIR* dir;
    struct dirent* entry;
    dir = opendir(e.c_str());
  ...
  int
  get_state(ExtBlkDevState& state)
  {
      ...
      std::map<std::string, std::string>* pm)
  {
    ...
    if (rc < 0)
      return rc;
    (*pm)[prefix + "fcm"] = "true";
    (*pm)[prefix + "fcm_partition_physical_size"] =
        stringify(get_partition_physical_size());
    (*pm)[prefix + "fcm_partition_logical_size"] =
        stringify(get_partition_logical_size());
    (*pm)[prefix + "fcm_device_physical_size"] =
        stringify(get_device_physical_size());
    (*pm)[prefix + "fcm_device_logical_size"] =
        stringify(get_device_logical_size());
    return 0;
  }

  int
  get_plugin_id(std::string& id_str) override
  {
    id_str = "fcm";
    return 0;
  };
...
public:
  explicit ExtBlkDevPluginFcm(CephContext* cct) :
    ExtBlkDevPlugin(cct)
  {}

  int
  get_required_cap_set(cap_t caps)
  {
    cap_value_t adm[1];
    adm[0] = CAP_SYS_ADMIN;
  ...
  int
  factory(const std::string& logdevname, ceph::ExtBlkDevInterfaceRef& ext_blk_dev)
  {
    ...
    if (r != 0) {
      ...
      if (!cct->_conf->bdev_enable_discard) {
        derr << "FCM device in use, but bdev_enable_discard not enabled. Free "
                "space will leak."
             << dendl;
      }
    }
