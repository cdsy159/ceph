...
pidfh::remove()
{
  ...
  if (ret < 0) {
    std::cerr << __func__ << " lseek failed " << cpp_strerror(errno)
              << std::endl;
    return -errno;
  }
  ...
  ::close(pf_fd);
  if (res < 0) {
    std::cerr << __func__ << " safe_read failed " << cpp_strerror(-res)
              << std::endl;
    return res;
  }
  ...
  int a = atoi(buf);
  if (a != getpid()) {
    std::cerr << __func__ << " the pid found in the file is " << a
              << " which is different from getpid() " << getpid() << std::endl;
    return -EDOM;
  }
...
int
pidfh::open(std::string_view pid_file)
{
  pf_path = pid_file;
  ...
  if (fd < 0) {
    int err = errno;
    derr << __func__ << ": failed to open pid file '" << pf_path
  ...
  if (fstat(fd, &st) == -1) {
    int err = errno;
    derr << __func__ << ": failed to fstat pid file '" << pf_path
         << "': " << cpp_strerror(err) << dendl;
    ::close(fd);
    reset();
    return -err;
...
// to this file.
  ...
  if (r < 0) {
    if (errno == EAGAIN || errno == EACCES) {
      derr << __func__ << ": failed to lock pidfile " << pf_path
...
pidfh::write()
{
  ...
  if (::ftruncate(pf_fd, 0) < 0) {
    int err = errno;
    derr << __func__ << ": failed to ftruncate the pid file '" << pf_path
         << "': " << cpp_strerror(err) << dendl;
    return -err;
  }
  ssize_t res = safe_write(pf_fd, buf, len);
  if (res < 0) {
    derr << __func__ << ": failed to write to pid file '" << pf_path
...
int
pidfile_write(std::string_view pid_file)
{
  if (pid_file.empty()) {
