...
void
install_sighandler(int signum, signal_handler_t handler, int flags)
{
  int ret;
...
// file.
static int
parse_from_os_release(const char* file, const char* key, char* out)
{
    ...
    std::map<std::string, std::string>* extra)
{
  if (g_ceph_context && g_ceph_context->_conf->crash_dir.size()) {
    ...
    snprintf(
        base, PATH_MAX, "%s/%s", g_ceph_context->_conf->crash_dir.c_str(),
        id.c_str());
...
static void
handle_oneshot_fatal_signal(int signum)
{
  ...
  snprintf(
      ...
      message, (unsigned long long)pthread_self(), pthread_name);
#else
  snprintf(
      buf, sizeof(buf),
      "*** Caught signal (%s) **\n "
      "in thread %llx thread_name:%s\n",
      sig_str(signum), (unsigned long long)pthread_self(), pthread_name);
#endif
  dout_emergency(buf);
  pidfile_remove();
...
void
install_standard_sighandlers(void)
{
...
struct SignalHandler : public Thread {
            ...
            if (r == 1) {
              ...
              case SI_USER:
                ...
                if (siginfo->si_pid) {
                  ...
                  message << " ( Could be generated by pthread_kill(), "
                             "raise(), abort(), alarm() )";
                }
  ...
  void
  queue_signal_info(int signum, siginfo_t* siginfo, void* content)
  {
    // If this signal handler is registered, the callback must be
    // defined.  We can do this without the lock because we will never
    // have the signal handler defined without the handlers entry also
...
static void
handler_signal_hook(int signum, siginfo_t* siginfo, void* content)
{
...
void
SignalHandler::unregister_handler(int signum, signal_handler_t handler)
{
  ceph_assert(signum >= 0 && signum < 32);
...
void
register_async_signal_handler(int signum, signal_handler_t handler)
{
  ceph_assert(g_signal_handler);
...
void
register_async_signal_handler_oneshot(int signum, signal_handler_t handler)
{
  ceph_assert(g_signal_handler);
...
void
unregister_async_signal_handler(int signum, signal_handler_t handler)
{
  ceph_assert(g_signal_handler);
