...
ceph::unique_leakable_ptr<raw> create_aligned_in_mempool(
    unsigned len,
    unsigned align,
    int mempool);
...
ceph::unique_leakable_ptr<buffer::raw> create(
...
ceph::unique_leakable_ptr<buffer::raw> create_local(
    seastar::temporary_buffer<char>&& buf);
#endif
...
private:
  ...
  class iterator_impl {
    ...
    iterator_impl(
        typename std::conditional<is_const, const ptr*, ptr*>::type p,
        size_t offset,
        bool d) :
      bp(p),
...
public:
  ...
  iterator
  begin(size_t offset = 0)
  {
    return iterator(this, offset, false);
  }

  const_iterator
  begin(size_t offset = 0) const
  {
  ...
  const_iterator
  begin_deep(size_t offset = 0) const
  {
  ...
  // misc
  bool
  is_aligned(unsigned align) const
  {
  ...
  bool
  is_n_align_sized(unsigned align) const
  {
    return (length() % align) == 0;
  }

  bool
  is_n_page_sized() const
  {
...
public:
  ...
  struct disposer {
    void
    operator()(ptr_node* const delete_this)
    {
...
public:
  ...
  public:
    ...
    class buffers_iterator {
      typename std::
          conditional<std::is_const<T>::value, const ptr_hook*, ptr_hook*>::type
              cur;
      template <class U>
    ...
    public:
      ...
      template <class U>
      buffers_iterator&
      operator=(buffers_iterator<U>& other)
      {
        cur = other.cur;
        return *this;
      }
      ...
      bool
      operator==(const buffers_iterator& rhs) const
      {
        return cur == rhs.cur;
      }
    ...
    void
    insert_after(const_iterator it, reference item)
    {
      item.next = it->next;
      it->next = &item;
    ...
    void
    clone_from(const buffers_t& other)
    {
      clear_and_dispose();
      for (auto& node : other) {
    ...
    void
    swap(buffers_t& other)
    {
      const auto copy_root = _root;
      _root.next = other._root.next == &other._root ? &this->_root
                                                    : other._root.next;
      other._root.next = copy_root.next == &_root ? &other._root
                                                  : copy_root.next;
...
private:
  ...
  protected:
    ...
    typedef
        typename std::conditional<is_const, const buffers_t, buffers_t>::type
            list_t;
  ...
  public:
    ...
    template <typename A>
    void
    copy(unsigned len, std::vector<uint8_t, A>& u8v)
    {
      u8v.resize(len);
...
public:
  ...
  class contiguous_appender {
    ...
    contiguous_appender(bufferlist& bl, size_t len, bool d) :
      bl(bl), space(bl.obtain_contiguous_space(len)), pos(space.bp_data), deep(d)
    {}
  ...
  public:
    ...
    size_t
    get_out_of_band_offset() const
    {
      return out_of_band_offset;
    }

    void
    append(const char* __restrict__ p, size_t l)
    {
  ...
  contiguous_appender
  get_contiguous_appender(size_t len, bool deep = false)
  {
  ...
  public:
    ...
    void
    advance(const unsigned len)
    {
      pos += len;
    }

    void
    copy_in(const unsigned len, const char* const src)
    {
      memcpy(pos, src, len);
      advance(len);
    }
  ...
  static_assert(
      sizeof(contiguous_filler) == sizeof(char*),
  ...
  class page_aligned_appender {
    ...
    page_aligned_appender(list* l, unsigned min_pages) :
      bl(*l), min_alloc(min_pages * CEPH_PAGE_SIZE)
    {}
    ...
    template <class Func>
    void
    _append_common(size_t len, Func&& impl_f)
    {
  ...
  public:
    ...
    void
    append(const char* buf, size_t entire_len)
    {
    ...
    void
    substr_of(const list& bl, unsigned off, unsigned len)
    {
  ...
  get_page_aligned_appender(unsigned min_pages = 1)
  {
...
public:
  // cons/des
  list() :
    _carriage(&always_empty_bptr), _len(0), _num(0)
  {}
  ...
  list(const list& other) :
    _carriage(&always_empty_bptr), _len(other._len), _num(other._num)
  {
  ...
  size_t
  get_append_buffer_unused_tail_length() const
  {
  ...
  bool is_aligned_size_and_memory(
      unsigned align_size,
  ...
  void
  push_back(std::unique_ptr<ptr_node, ptr_node::disposer> bp)
  {
    _carriage = bp.get();
    _len += bp->length();
    _num += 1;
  ...
  void
  push_back(ceph::unique_leakable_ptr<raw> r)
  {
  ...
  bool rebuild_aligned_size_and_memory(
      unsigned align_size,
  ...
  [[deprecated("in favor of operator=(list&&)")]] void
  claim(list& bl)
  {
    *this = std::move(bl);
  }
  ...
  template <typename A>
  void
  append(const std::vector<uint8_t, A>& u8v)
  {
  ...
  void splice(
      unsigned off,
      unsigned len,
      list* claim_by = 0 /*, bufferlist& replace_with */);
  ...
  ssize_t
  pread_file(const char* fn, uint64_t off, uint64_t len, std::string* error);
  ...
  void
  prepare_iov(VectorT* piov) const
  {
#ifdef __CEPH__
...
inline bool
operator==(const bufferlist& lhs, const bufferlist& rhs)
{
...
inline bool
operator!=(const bufferlist& l, const bufferlist& r)
{
  return !(l == r);
}
...
inline bufferhash&
operator<<(bufferhash& l, const bufferlist& r)
{
  l.update(r);
  return l;
}
