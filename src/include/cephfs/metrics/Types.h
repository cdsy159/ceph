...
inline std::ostream&
operator<<(std::ostream& os, const ClientMetricType& type)
{
  switch (type) {
  ...
  default:
    os << "(UNKNOWN:"
       << static_cast<std::underlying_type<ClientMetricType>::type>(type)
       << ")";
    break;
  }
...
struct ClientMetricPayloadBase {
  ...
  void
  print_type(std::ostream* out) const
  {
    *out << metric_type;
  }
...
struct CapInfoPayload : public ClientMetricPayloadBase {
  ...
  {
    *out << "cap_hits: " << cap_hits << " " << "cap_misses: " << cap_misses
         << " " << "num_caps: " << nr_caps;
  }
};
...
struct SimpleIOMetric {
  ...
  static constexpr uint64_t OP_TYPE_MASK =
      1ULL << OP_TYPE_SHIFT; // 0x8000000000000000ULL

  // latency in microseconds (32 bits)
  // directly after is_write bit
  static constexpr uint64_t LATENCY_BITS = 32;
  static constexpr uint64_t LATENCY_SHIFT = OP_TYPE_SHIFT -
                                            LATENCY_BITS; // 63 - 32 = 31
  static constexpr uint64_t LATENCY_MASK = ((1ULL << LATENCY_BITS) - 1)
                                           << LATENCY_SHIFT;
  ...
  static constexpr uint64_t PAYLOAD_SIZE_SHIFT =
      0; // Starts from the LSB (bit 0)
  static constexpr uint64_t PAYLOAD_SIZE_MASK = (1ULL << PAYLOAD_SIZE_BITS) - 1;

  static constexpr uint32_t MAX_LATENCY_US = (1ULL << LATENCY_BITS) -
                                             1; // 2^32 - 1 microseconds
  static constexpr uint32_t MAX_PAYLOAD_SIZE = (1ULL << PAYLOAD_SIZE_BITS) -
                                               1; // 2^31 - 1 bytes

  SimpleIOMetric(bool is_write, utime_t latency, uint32_t payload_size) :
    packed_data(0)
  {
    if (is_write) {
    ...
    packed_data |= (static_cast<uint64_t>(payload_size) << PAYLOAD_SIZE_SHIFT) &
                   PAYLOAD_SIZE_MASK;
  }
  ...
  get_payload_size() const
  {
    return static_cast<uint32_t>(
        (packed_data & PAYLOAD_SIZE_MASK) >> PAYLOAD_SIZE_SHIFT);
  }
...
struct AggregatedIOMetrics {
  ...
  uint64_t
  avg_read_throughput_Bps() const
  {
    return read_latency_us > 0 ? (read_bytes * 1'000'000ULL) / read_latency_us
                               : 0;
  }

  uint64_t
  avg_write_throughput_Bps() const
  {
    return write_latency_us > 0
               ? (write_bytes * 1'000'000ULL) / write_latency_us
               : 0;
  }

  uint64_t
  avg_read_latency_us() const
  {
    return read_count ? read_latency_us / read_count : 0;
  }

  uint64_t
  avg_write_latency_us() const
  {
  ...
  read_iops() const
  {
    return read_latency_us > 0
               ? static_cast<double>(read_count) * 1e6 / read_latency_us
               : 0.0;
  }
  ...
  {
    return write_latency_us > 0
               ? static_cast<double>(write_count) * 1e6 / write_latency_us
               : 0.0;
  }
  ...
  friend std::ostream&
  operator<<(std::ostream& os, const AggregatedIOMetrics& m)
  {
    os << "{subvolume_id=\"" << m.subvolume_id
       << "\", read_count=" << m.read_count << ", write_count=" << m.write_count
       << ", read_bytes=" << m.read_bytes << ", write_bytes=" << m.write_bytes
...
struct SubvolumeMetricsPayload : public ClientMetricPayloadBase {
  ...
  SubvolumeMetricsPayload(std::vector<AggregatedIOMetrics> metrics) :
    ClientMetricPayloadBase(
        ClientMetricType::CLIENT_METRIC_TYPE_SUBVOLUME_METRICS),
    subvolume_metrics(std::move(metrics))
  {}
...
public:
  ...
  public:
    ...
    inline void
    operator()(const ClientMetricPayload& payload) const
    {
      using ceph::encode;
  ...
  public:
    ...
    template <typename ClientMetricPayload>
    inline void
    operator()(ClientMetricPayload& payload) const
    {
      using ceph::decode;
  ...
  public:
    ...
    operator()(const ClientMetricPayload& payload) const
    {
      m_formatter->dump_string(
          "client_metric_type", stringify(payload.get_type()));
  ...
  static std::list<ClientMetricMessage>
