...
protected:
  ...
  public:
    ...
    bool
    operator==(const const_iterator_base& o) const
    {
    ...
    const_iterator_base&
    operator=(const const_iterator_base& o)
    {
      map = o.map;
      it = o.it;
      return *this;
  ...
  public:
    ...
    bool
    operator==(const iterator_base& o) const
    {
      return (map == o.map) && (!map->map || it == o.it);
    }
    ...
    iterator_base&
    operator=(const iterator_base& o)
    {
      map = o.map;
      it = o.it;
      return *this;
...
public:
  ...
  class const_iterator
    : public const_iterator_base<typename Map::const_iterator> {
  public:
    const_iterator() {}
    ...
    const_iterator(
        const compact_map_base* m,
        const typename Map::const_iterator& i) :
      const_iterator_base<typename Map::const_iterator>(m, i)
    {}
  };

  class reverse_iterator
    : public iterator_base<typename Map::reverse_iterator> {
  public:
    reverse_iterator() {}
    ...
    reverse_iterator(
        compact_map_base* m,
        const typename Map::reverse_iterator& i) :
      iterator_base<typename Map::reverse_iterator>(m, i)
    {}
  };

  class const_reverse_iterator
    : public const_iterator_base<typename Map::const_reverse_iterator> {
  public:
    const_reverse_iterator() {}

    const_reverse_iterator(
        const iterator_base<typename Map::const_reverse_iterator>& o) :
    ...
    const_reverse_iterator(
        const compact_map_base* m,
        const typename Map::const_reverse_iterator& i) :
  ...
  bool
  operator==(const compact_map_base& o) const
  {
  ...
  void
  swap(compact_map_base& o)
  {
    map.swap(o.map);
  }

  compact_map_base&
  operator=(const compact_map_base& o)
  {
    if (o.map) {
      alloc_internal();
      *map = *o.map;
  ...
  iterator
  insert(const std::pair<const Key, T>& val)
  {
    alloc_internal();
...
template <class Key, class T, class Map>
inline void
encode(const compact_map_base<Key, T, Map>& m, ceph::buffer::list& bl)
{
  m.encode(bl);
}
...
inline void
encode(
    const compact_map_base<Key, T, Map>& m,
    ceph::buffer::list& bl,
    uint64_t features)
{
...
template <class Key, class T, class Map>
inline void
decode(compact_map_base<Key, T, Map>& m, ceph::buffer::list::const_iterator& p)
{
  m.decode(p);
}
...
template <
    ...
    class Alloc = std::allocator<std::pair<const Key, T>>>
inline std::ostream&
operator<<(std::ostream& out, const compact_map<Key, T, Compare, Alloc>& m)
{
  out << "{";
  bool first = true;
  for (const auto& p : m) {
...
template <
    ...
    class Alloc = std::allocator<std::pair<const Key, T>>>
inline std::ostream&
operator<<(std::ostream& out, const compact_multimap<Key, T, Compare, Alloc>& m)
{
  out << "{{";
  bool first = true;
  for (const auto& p : m) {
