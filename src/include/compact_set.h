...
public:
  ...
  public:
    ...
    const_iterator(
        const compact_set_base* s,
        const typename Set::const_iterator& i) :
  ...
  public:
    ...
    const_reverse_iterator(
        const iterator_base<typename Set::const_reverse_iterator>& o) :
    ...
    const_reverse_iterator(
        const compact_set_base* s,
        const typename Set::const_reverse_iterator& i) :
      iterator_base<typename Set::const_reverse_iterator>(s, i)
    {}
  };

  class reverse_iterator
    : public iterator_base<typename Set::reverse_iterator> {
  public:
    reverse_iterator() {}
    ...
    reverse_iterator(
        compact_set_base* s,
        const typename Set::reverse_iterator& i) :
  ...
  bool
  operator==(const compact_set_base& o) const
  {
  ...
  void
  swap(compact_set_base& o)
  {
    set.swap(o.set);
  }

  compact_set_base&
  operator=(const compact_set_base& o)
  {
    if (o.set) {
      alloc_internal();
      *set = *o.set;
...
template <class T, class Set>
inline void
encode(const compact_set_base<T, Set>& m, ceph::buffer::list& bl)
{
  m.encode(bl);
}
...
template <class T, class Set>
inline void
decode(compact_set_base<T, Set>& m, ceph::buffer::list::const_iterator& p)
{
  m.decode(p);
}
...
template <class T, class Compare = std::less<T>, class Alloc = std::allocator<T>>
inline std::ostream&
operator<<(std::ostream& out, const compact_set<T, Compare, Alloc>& s)
{
  bool first = true;
  for (auto& v : s) {
    if (!first)
