...
template <typename T>
inline constexpr bool has_compare_to =
    std::experimental::is_detected_v<compare_to_t, T>;
...
struct key_compare_to_adapter<
    std::less<K>,
    std::enable_if_t<std::is_unsigned_v<K>>> {
  struct type {
    inline int
    operator()(const K& lhs, const K& rhs) const noexcept
    {
...
struct common_params {
  ...
  using node_count_type = std::conditional_t<
      (kNodeValueSpace / ValueSize > (std::numeric_limits<uint8_t>::max)()),
      uint16_t,
      uint8_t>;
};
...
union map_slot_type {
  ...
  map_slot_type&
  operator=(const map_slot_type& slot)
  {
...
template <class K, class V>
void
swap(map_slot_type<K, V>& lhs, map_slot_type<K, V>& rhs)
{
...
template <
    ...
    bool Multi>
struct map_params
  : common_params<Key, Compare, Alloc, TargetNodeSize, sizeof(Key) + sizeof(Data), Multi> {
  ...
  template <class... Args>
  static void
  construct(Alloc* alloc, slot_type* slot, Args&&... args)
  {
  ...
  static void
  construct(Alloc* alloc, slot_type* slot, slot_type* other)
  {
    emplace(slot);
    std::allocator_traits<Alloc>::construct(
  ...
  static void
  move(Alloc* alloc, slot_type* src, slot_type* dest)
  {
    dest->mutable_value = std::move(src->mutable_value);
  }

  static void
  destroy(Alloc* alloc, slot_type* slot)
  {
...
struct set_params
  : public common_params<Key, Compare, Alloc, TargetNodeSize, sizeof(Key), Multi> {
...
constexpr bool
compare_result_as_less_than(const Result r)
{
...
struct upper_bound_adapter {
  ...
  bool
  operator()(const K& a, const LK& b) const
  {
...
private:
  ...
  // padding.
  constexpr static size_type
  NodeTargetValues(const int begin, const int end)
  {
  ...
  constexpr static size_type
  LeafSize(const int max_values = kNodeValues)
  {
  ...
  void
  set_parent(btree_node* p)
  {
    GetField<&base_fields::parent>() = p;
  }

  field_type&
  mutable_count()
  {
    return GetField<&base_fields::count>();
  }

  slot_type*
  slot(int i)
  {
    return GetField<&leaf_fields::values>() + i;
  }
  ...
  void
  set_position(field_type v)
  {
    GetField<&base_fields::position>() = v;
  }

  void
  set_count(field_type v)
  {
    GetField<&base_fields::count>() = v;
  }

  // This method is only called by the node init methods.
  void
  set_max_count(field_type v)
  {
    GetField<&base_fields::max_count>() = v;
  }
...
public:
  ...
  {
    static_assert(
        alignof(leaf_fields) == alignof(internal_fields),
  ...
  leaf() const
  {
    return GetField<&base_fields::max_count>() != kInternalNodeMaxCount;
  }
  ...
  position() const
  {
    return GetField<&base_fields::position>();
  }
  ...
  parent() const
  {
    return GetField<&base_fields::parent>();
  }
  ...
  child(int i) const
  {
    return *(GetField<&internal_fields::children>() + i);
  }
  ...
  template <typename K>
  int
  upper_bound(const K& k, const key_compare& comp) const
  {
  ...
  {
    return linear_search_impl(
        k, 0, count(), comp, btree_is_key_compare_to<Compare, key_type>());
  }
  ...
  static btree_node*
  init_leaf(btree_node* n, btree_node* parent, int max_count)
  {
    n->set_parent(parent);
    n->set_position(0);
    n->set_count(0);
  ...
  static btree_node*
  init_internal(btree_node* n, btree_node* parent)
  {
...
private:
  template <typename... Args>
  void
  value_init(const size_type i, allocator_type* alloc, Args&&... args)
  {
    params_type::construct(alloc, slot(i), std::forward<Args>(args)...);
  }

  void
  value_destroy(const size_type i, allocator_type* alloc)
  {
  ...
  void
  value_destroy_n(const size_type i, const size_type n, allocator_type* alloc)
  {
    for (int j = 0; j < n; ++j) {
...
class AlignedAlloc {
  ...
  num_aligned_objects(size_t size)
  {
...
public:
  static void*
  allocate(Alloc* alloc, size_t size)
  {
  ...
  static void
  deallocate(Alloc* alloc, void* p, size_t size)
  {
    alloc_t aligned_alloc(*alloc);
    traits_t::deallocate(
        aligned_alloc, static_cast<M*>(p), num_aligned_objects(size));
  }
};
...
private:
  ...
  const value_type&
  maybe_move_from_iterator(const_iterator x)
  {
    return *x;
  }

  value_type&&
  maybe_move_from_iterator(iterator x)
  {
    return std::move(*x);
  }
...
public:
  ...
  // Requirement: `key` is never referenced after consuming `args`.
  template <typename... Args>
  std::pair<iterator, bool> insert_hint_unique(
      iterator position,
      const key_type& key,
      Args&&... args);
  ...
  template <typename K>
  size_type
  count_unique(const K& key) const
  {
  ...
  template <typename K, typename LK>
  bool
  compare_keys(const K& x, const LK& y) const
  {
...
private:
  ...
  node_type*
  new_internal_node(node_type* parent)
  {
  ...
  node_type*
  new_leaf_node(node_type* parent)
  {
  ...
  node_type*
  new_leaf_root_node(const int max_count)
  {
  ...
  void
  deallocate(const size_type size, node_type* node)
  {
    using aligned_alloc_t = AlignedAlloc<node_type::Alignment(), allocator_type>;
  ...
  void
  delete_internal_node(node_type* node)
  {
  ...
  void
  delete_leaf_node(node_type* node)
  {
    if (node != EmptyNode()) {
      node->destroy(mutable_allocator());
  ...
  iterator
  internal_end(iterator iter)
  {
    return iter.node != nullptr ? iter : end();
  }

  const_iterator
  internal_end(const_iterator iter) const
  {
    return iter.node != nullptr ? iter : end();
  }
  ...
  node_stats
  internal_stats(const node_type* node) const
  {
    ...
    Args&&... args)
{
  ...
  if (i < count()) {
    ...
    std::copy_backward(
        std::make_move_iterator(slot(i)),
        std::make_move_iterator(slot(count() - 1)), slot(count()));
    value_destroy(i, alloc);
  }
...
template <typename P>
inline void
btree_node<P>::remove_value(const int i, allocator_type* alloc)
{
  if (!leaf() && count() > i + 1) {
...
template <typename P>
inline void
btree_node<P>::remove_values_ignore_children(
    const int i,
    const int to_erase,
    allocator_type* alloc)
{
  assert(to_erase >= 0);
  std::copy(
      std::make_move_iterator(slot(i + to_erase)),
      std::make_move_iterator(slot(count())), slot(i));
...
template <typename P>
void
btree_node<P>::rebalance_right_to_left(
    const int to_move,
    btree_node* right,
    allocator_type* alloc)
{
  ...
  std::copy(
      std::make_move_iterator(right->slot(to_move)),
      std::make_move_iterator(right->slot(right->count())), right->slot(0));
...
template <typename P>
void
btree_node<P>::rebalance_left_to_right(
    const int to_move,
    btree_node* right,
    allocator_type* alloc)
{
  ...
  if (right->count() >= to_move) {
    ...
    right->uninitialized_move_n(
        to_move, right->count() - to_move, right->count(), right, alloc);
    std::copy_backward(
        std::make_move_iterator(right->slot(0)),
        std::make_move_iterator(right->slot(right->count() - to_move)),
        right->slot(right->count()));

    // 2) Move the delimiting value in the parent to the right node.
    params_type::move(
        alloc, parent()->slot(position()), right->slot(to_move - 1));

    // 3) Move the (to_move - 1) values from the left node to the right node.
    std::copy(
        std::make_move_iterator(slot(count() - (to_move - 1))),
        std::make_move_iterator(slot(count())), right->slot(0));
  } else {
    ...
    uninitialized_move_n(
        uninitialized_remaining, count() - uninitialized_remaining,
...
template <typename P>
void
btree_node<P>::merge(btree_node* src, allocator_type* alloc)
{
...
template <typename P>
void
btree_node<P>::swap(btree_node* x, allocator_type* alloc)
{
  using std::swap;
  assert(leaf() == x->leaf());
  ...
  // Swap the values.
  std::swap_ranges(
      smaller->slot(0), smaller->slot(smaller->count()), larger->slot(0));

  // Move values that can't be swapped.
  const size_type to_move = larger->count() - smaller->count();
  larger->uninitialized_move_n(
      to_move, smaller->count(), smaller->count(), smaller, alloc);
  ...
  if (!leaf()) {
    ...
    std::swap_ranges(smaller_begin, smaller_begin + count, larger_begin);
...
template <typename N, typename R, typename P>
void
btree_iterator<N, R, P>::increment_slow()
{
  if (node->leaf()) {
...
template <typename N, typename R, typename P>
void
btree_iterator<N, R, P>::decrement_slow()
{
  if (node->leaf()) {
    assert(position <= -1);
    btree_iterator save(*this);
...
template <typename Btree>
void
btree<P>::copy_or_move_values_in_order(Btree* x)
{
  static_assert(
...
template <typename P>
constexpr bool
btree<P>::static_assert_validation()
{
  static_assert(
  ...
  static_assert(
      std::is_trivially_copyable_v<iterator>,
  ...
  static_assert(
      node_type::MinimumOverhead() >= sizeof(void*) + 4,
...
template <typename... Args>
auto
btree<P>::insert_unique(const key_type& key, Args&&... args)
...
template <typename InputIterator>
void
btree<P>::insert_iterator_unique(InputIterator b, InputIterator e)
{
  for (; b != e; ++b) {
...
template <typename ValueType>
auto
btree<P>::insert_multi(const key_type& key, ValueType&& v) -> iterator
{
  if (empty()) {
    mutable_root() = rightmost_ = new_leaf_root_node(1);
  }
...
template <typename ValueType>
auto
btree<P>::insert_hint_multi(iterator position, ValueType&& v) -> iterator
{
  if (!empty()) {
...
template <typename InputIterator>
void
btree<P>::insert_iterator_multi(InputIterator b, InputIterator e)
{
  for (; b != e; ++b) {
...
template <typename P>
auto
btree<P>::operator=(const btree& x) -> btree&
{
  if (this != &x) {
    clear();
...
template <typename P>
auto
btree<P>::operator=(btree&& x) noexcept -> btree&
{
  if (this != &x) {
    clear();
...
template <typename P>
auto
btree<P>::erase(iterator iter) -> iterator
{
  ...
  if (!iter.node->leaf()) {
    ...
    params_type::move(
        mutable_allocator(), iter.node->slot(iter.position),
...
template <typename P>
auto
btree<P>::rebalance_after_delete(iterator iter) -> iterator
{
...
template <typename P>
auto
btree<P>::erase(iterator begin, iterator end) -> std::pair<size_type, iterator>
{
...
template <typename P>
void
btree<P>::erase_same_node(iterator begin, iterator end)
{
  ...
  node->remove_values_ignore_children(
...
template <typename P>
auto
btree<P>::erase_from_leaf_node(iterator begin, size_type to_erase) -> iterator
{
  node_type* node = begin.node;
  assert(node->leaf());
...
template <typename K>
auto
btree<P>::erase_unique(const K& key) -> size_type
{
...
template <typename K>
auto
btree<P>::erase_multi(const K& key) -> size_type
{
...
template <typename P>
void
btree<P>::rebalance_or_split(iterator* iter)
{
...
template <typename P>
void
btree<P>::merge_nodes(node_type* left, node_type* right)
{
...
template <typename P>
bool
btree<P>::try_merge_or_rebalance(iterator* iter)
{
...
template <typename... Args>
inline auto
btree<P>::internal_emplace(iterator iter, Args&&... args) -> iterator
{
  if (!iter.node->leaf()) {
  ...
  iter.node->emplace_value(
      iter.position, mutable_allocator(), std::forward<Args>(args)...);
  ++size_;
  return iter;
}
...
template <typename K>
inline auto
btree<P>::internal_locate(
    const K& key) const -> SearchResult<iterator, is_key_compare_to::value>
{
...
template <typename K>
auto
btree<P>::internal_lower_bound(const K& key) const -> iterator
{
  iterator iter(const_cast<node_type*>(root()), 0);
  for (;;) {
    iter.position = iter.node->lower_bound(key, key_comp()).value;
...
template <typename K>
auto
btree<P>::internal_upper_bound(const K& key) const -> iterator
{
  iterator iter(const_cast<node_type*>(root()), 0);
  for (;;) {
...
template <typename K>
auto
btree<P>::internal_find(const K& key) const -> iterator
{
  auto res = internal_locate(key);
  if constexpr (res.has_match) {
    if (res.IsEq()) {
...
template <typename P>
void
btree<P>::internal_clear(node_type* node)
{
  if (!node->leaf()) {
    for (int i = 0; i <= node->count(); ++i) {
  ...
  if (!node->leaf()) {
    for (int i = 0; i <= node->count(); ++i) {
      ...
      count += internal_verify(
          node->child(i), (i == 0) ? lo : &node->key(i - 1),
