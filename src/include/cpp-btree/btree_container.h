...
protected:
  ...
  template <class K>
  using key_arg = std::conditional_t<
...
public:
  ...
  template <typename K = key_type>
  iterator
  lower_bound(const key_arg<K>& key)
  {
  ...
  template <typename K = key_type>
  const_iterator
  lower_bound(const key_arg<K>& key) const
  {
    return tree_.lower_bound(key);
  }
  ...
  template <typename K = key_type>
  iterator
  upper_bound(const key_arg<K>& key)
  {
  ...
  template <typename K = key_type>
  const_iterator
  upper_bound(const key_arg<K>& key) const
  {
    return tree_.upper_bound(key);
  }
...
public:
  ...
  friend bool
  operator==(const btree_container& x, const btree_container& y)
  {
    if (x.size() != y.size())
      return false;
...
public:
  ...
  insert(const_iterator position, value_type&& x)
  {
    return this->tree_
        .insert_hint_unique(
            iterator(position), params_type::key(x), std::move(x))
        .first;
  }
  ...
  emplace_hint(const_iterator position, Args&&... args)
  {
    ...
    return this->tree_
        .insert_hint_unique(
            iterator(position), params_type::key(v), std::move(v))
        .first;
  }
  ...
  void
  insert(std::initializer_list<init_type> init)
  {
