...
private:
  ...
  dump() const
  {
    char fn[PATH_MAX];
    ::snprintf(
        fn, sizeof(fn), ENCODE_STRINGIFY(ENCODE_DUMP_PATH) "/%s__%d.%x", name,
        getpid(), i++);
    int fd =
    ...
    if (auto bl_delta = appender.bl.length() - bl_offset; bl_delta > 0) {
      ...
      appender.bl.begin(bl_offset + space_offset)
          .copy(bl_delta - space_offset, dump_bl);
...
template <typename T>
  requires _denc::is_any_of<
      _denc::underlying_type_t<T>,
      ceph_le64,
      ceph_le32,
      ceph_le16,
      uint8_t
...
struct denc_traits<T> {
  ...
  static void
  bound_encode(const T& o, size_t& p, uint64_t f = 0)
  {
    p += sizeof(T);
  }
  ...
  static void
  decode(T& o, ceph::buffer::list::const_iterator& p)
  {
...
template <typename T>
inline void
denc_varint(T v, ceph::buffer::list::contiguous_appender& p)
{
  uint8_t byte = v & 0x7f;
  v >>= 7;
  while (v) {
...
template <typename T>
inline void
denc_varint(T& v, ceph::buffer::ptr::const_iterator& p)
{
...
inline void
denc_signed_varint(int64_t v, size_t& p)
{
  p += sizeof(v) + 2;
}
...
inline void
denc_varint_lowz(uint64_t v, size_t& p)
{
  p += sizeof(v) + 2;
}

inline void
denc_varint_lowz(uint64_t v, ceph::buffer::list::contiguous_appender& p)
{
...
template <typename T>
inline void
denc_varint_lowz(T& v, ceph::buffer::ptr::const_iterator& p)
{
  uint64_t i = 0;
  denc_varint(i, p);
  int lowznib = (i & 3);
...
inline void
denc_signed_varint_lowz(int64_t v, size_t& p)
{
  p += sizeof(v) + 2;
}
...
denc_signed_varint_lowz(int64_t v, It& p)
{
  ...
  unsigned lowznib = v ? (std::countr_zero(std::bit_cast<uint64_t>(v)) / 4)
                       : 0u;
  if (lowznib > 3)
    lowznib = 3;
  v >>= lowznib * 4;
...
denc(const T& o, size_t& p, uint64_t f = 0)
{
...
inline void
denc(T& o, It& p, uint64_t features = 0)
{
  if constexpr (traits::featured) {
...
struct has_legacy_denc<
    ...
    decltype(std::declval<T&>().decode(
        std::declval<ceph::buffer::list::const_iterator&>()))>
  : std::true_type {
  static void
  decode(T& v, ceph::buffer::list::const_iterator& p)
  {
    v.decode(p);
  }
};
...
template <typename T>
struct has_legacy_denc<T, std::enable_if_t<!denc_traits<T>::need_contiguous>>
  : std::true_type {
  static void
  decode(T& v, ceph::buffer::list::const_iterator& p)
  {
...
inline std::enable_if_t<traits::supported && has_legacy_denc::value>
denc(T& o, ceph::buffer::list::const_iterator& p)
{
...
public:
  ...
  static void
  bound_encode(const value_type& s, size_t& p, uint64_t f = 0)
  {
  ...
  template <class It>
  static void
  encode(const value_type& s, It& p, uint64_t f = 0)
  {
  ...
  template <class It>
  static void
  decode(value_type& s, It& p, uint64_t f = 0)
  {
    uint32_t len;
    denc(len, p);
  ...
  static void
  decode(value_type& s, ceph::buffer::list::const_iterator& p)
  {
    uint32_t len;
    denc(len, p);
    decode_nohead(len, s, p);
  ...
  static void
  decode_nohead(size_t len, value_type& s, ceph::buffer::list::const_iterator& p)
  {
    if (len) {
...
struct denc_traits<ceph::buffer::ptr> {
  ...
  static void
  bound_encode(const ceph::buffer::ptr& v, size_t& p, uint64_t f = 0)
  {
  ...
  static void
  decode(ceph::buffer::ptr& v, ceph::buffer::list::const_iterator& p)
  {
    uint32_t len;
    denc(len, p);
    ceph::buffer::list s;
...
struct denc_traits<ceph::buffer::list> {
  ...
  static void
  bound_encode(const ceph::buffer::list& v, size_t& p, uint64_t f = 0)
  {
    p += sizeof(uint32_t) + v.length();
  }
  ...
  static void
  decode(ceph::buffer::list& v, ceph::buffer::list::const_iterator& p)
  {
    uint32_t len;
    denc(len, p);
    v.clear();
...
struct denc_traits<
  ...
  static constexpr bool need_contiguous =
      (a_traits::need_contiguous || b_traits::need_contiguous);
  ...
  static void
  decode(std::pair<A, B>& v, ceph::buffer::ptr::const_iterator& p, uint64_t f = 0)
  {
...
public:
  ...
  static void
  decode(container& s, ceph::buffer::ptr::const_iterator& p, uint64_t f = 0)
  {
    uint32_t num;
    denc(num, p);
  ...
  template <typename U = T>
  static std::enable_if_t<!!sizeof(U) && !need_contiguous>
  decode_nohead(size_t num, container& s, ceph::buffer::list::const_iterator& p)
  {
    s.clear();
    Details::reserve(s, num);
    while (num--) {
...
class container_has_reserve {
  ...
  static std::true_type test(
      SFINAE_match<T (*)(typename T::size_type), &U::reserve>*);
...
template <typename T>
inline constexpr bool container_has_reserve_v = container_has_reserve<T>::value;
...
struct denc_traits<std::array<T, N>, std::enable_if_t<denc_traits<T>::supported>> {
private:
...
//
template <typename T>
struct denc_traits<boost::optional<T>, std::enable_if_t<denc_traits<T>::supported>> {
  using traits = denc_traits<T>;
...
struct denc_traits<boost::none_t> {
  ...
  static void
  bound_encode(const boost::none_t& v, size_t& p)
  {
    p += sizeof(bool);
  }

  static void
  encode(const boost::none_t& v, ceph::buffer::list::contiguous_appender& p)
  {
    denc(false, p);
  }
};
...
//
template <typename T>
struct denc_traits<std::optional<T>, std::enable_if_t<denc_traits<T>::supported>> {
  using traits = denc_traits<T>;
...
struct denc_traits<std::nullopt_t> {
  ...
  static void
  bound_encode(const std::nullopt_t& v, size_t& p)
  {
    p += sizeof(bool);
  }

  static void
  encode(const std::nullopt_t& v, ceph::buffer::list::contiguous_appender& p)
  {
    denc(false, p);
  }
};
...
#define _DECLARE_CLASS_DENC(T, b)                                              \
  ...
  struct denc_traits<T> {                                                      \
    ...
    static void                                                                \
    bound_encode(const T& v, size_t& p, uint64_t f = 0)                        \
    {                                                                          \
      v.bound_encode(p);                                                       \
    }                                                                          \
    static void                                                                \
    ...
    static void                                                                \
    decode(T& v, ::ceph::buffer::ptr::const_iterator& p, uint64_t f = 0)       \
    {                                                                          \
      v.decode(p);                                                             \
    }                                                                          \
  };
...
#define WRITE_CLASS_DENC_FEATURED_BOUNDED(T) \
  _DECLARE_CLASS_DENC_FEATURED(T, true)
#define _DECLARE_CLASS_DENC_FEATURED(T, b)                                       \
  ...
  struct denc_traits<T> {                                                        \
    ...
    static constexpr bool need_contiguous = !_denc::has_legacy_denc<T>::value;   \
    static void                                                                  \
    bound_encode(const T& v, size_t& p, uint64_t f)                              \
    {                                                                            \
      v.bound_encode(p, f);                                                      \
    }                                                                            \
    static void                                                                  \
    encode(const T& v, ::ceph::buffer::list::contiguous_appender& p, uint64_t f) \
    {                                                                            \
      v.encode(p, f);                                                            \
    }                                                                            \
    static void                                                                  \
...
namespace ceph {
template <typename T, typename traits = denc_traits<T>>
inline std::enable_if_t<traits::supported && !traits::featured>
encode(const T& o, ceph::buffer::list& bl, uint64_t features_unused = 0)
{
  size_t len = 0;
...
template <typename T, typename traits = denc_traits<T>>
inline std::enable_if_t<traits::supported && traits::featured>
encode(const T& o, ::ceph::buffer::list& bl, uint64_t features)
{
  size_t len = 0;
...
template <typename T, typename traits = denc_traits<T>>
inline std::enable_if_t<traits::supported && !traits::need_contiguous>
decode(T& o, ::ceph::buffer::list::const_iterator& p)
{
  if (p.end())
...
template <typename T, typename traits = denc_traits<T>>
inline std::enable_if_t<traits::supported && !traits::featured>
encode_nohead(const T& o, ceph::buffer::list& bl)
{
  size_t len = 0;
  traits::bound_encode(o, len);
  auto a = bl.get_contiguous_appender(len);
...
template <typename T, typename traits = denc_traits<T>>
inline std::enable_if_t<traits::supported && !traits::featured>
decode_nohead(size_t num, T& o, ceph::buffer::list::const_iterator& p)
{
  if (!num)
    return;
  if (p.end())
...
denc_compat_throw(
    const char* _pretty_function_,
    uint8_t code_v,
    uint8_t struct_v,
    uint8_t struct_compat)
{
  throw ::ceph::buffer::malformed_input(
      "Decoder at '" + std::string(_pretty_function_) + "' v=" +
      std::to_string(code_v) + " cannot decode v=" + std::to_string(struct_v) +
...
struct StructVChecker {
  struct CheckingWrapper {
    consteval CheckingWrapper(__u8 c) :
      c(c)
    {
      consteval_assert(
          c <= MaxV,
...
#define DENC_HELPERS                                                     \
  /* bound_encode */                                                     \
  static void _denc_start(                                               \
      size_t& p, __u8* struct_v, __u8* struct_compat, char**, uint32_t*) \
  {                                                                      \
    p += 2 + 4;                                                          \
  }                                                                      \
  static void _denc_finish(size_t& p, char**, uint32_t*) {}              \
  /* encode */                                                           \
  static void _denc_start(                                               \
      ::ceph::buffer::list::contiguous_appender& p, __u8* struct_v,      \
      __u8* struct_compat, char** len_pos, uint32_t* start_oob_off)      \
  {                                                                      \
    denc(*struct_v, p);                                                  \
  ...
  static void _denc_finish(                                              \
      ::ceph::buffer::list::contiguous_appender& p, char** len_pos,      \
  ...
  /* decode */                                                           \
  static void _denc_start(                                               \
      ::ceph::buffer::ptr::const_iterator& p, __u8* struct_v,            \
      __u8* struct_compat, char** start_pos, uint32_t* struct_len)       \
  {                                                                      \
  ...
  static void _denc_finish(                                              \
      ::ceph::buffer::ptr::const_iterator& p, char** start_pos,          \
      uint32_t* struct_len)                                              \
  {                                                                      \
...
#define DENC_START(_v, compat, p)                                        \
  ...
  uint32_t _denc_u32;                                                    \
  static_assert(                                                         \
      CEPH_RELEASE >= (CEPH_RELEASE_SQUID /*19*/ + 2) || compat == 1);   \
...
#define DENC_START_COMPAT_2(_v, compat, p)                               \
  ...
  uint32_t _denc_u32;                                                    \
  static_assert(                                                         \
      CEPH_RELEASE >= (CEPH_RELEASE_SQUID /*19*/ + 2) || compat == 2);   \
  _denc_start(p, &struct_v.v, &struct_compat, &_denc_pchar, &_denc_u32); \
  do {
...
#define DENC(Type, v, p)                                          \
  ...
  friend std::enable_if_t<                                        \
      std::is_same_v<T, Type> || std::is_same_v<T, const Type>>   \
