...
public:
  enum UnittestType {
    test_strict = strict
  }; // Required by test_interval_set.cc
...
private:
  ...
  void
  intersection_size_asym(const interval_set& s, const interval_set& l)
  {
    auto ps = s.m.begin();
  ...
  bool
  subset_size_sym(const interval_set& b) const
  {
...
public:
  bool
  operator==(const interval_set& other) const
  {
  ...
  void
  decode(ceph::buffer::list::iterator& p)
  {
    denc(m, p);
    _size = 0;
    for (const auto& p : m) {
  ...
  void
  encode_nohead(ceph::buffer::list::contiguous_appender& p) const
  {
    denc_traits<Map>::encode_nohead(m, p);
  }

  void
  decode_nohead(int n, ceph::buffer::ptr::const_iterator& p)
  {
  ...
  bool
  contains(T i, T* pstart = 0, T* plen = 0) const
  {
    auto p = find_inc(i);
    if (p == m.end())
      return false;
  ...
  bool
  contains(interval_set const& other) const
  {
    interval_set tmp;
  ...
  bool
  intersects(T start, T len) const
  {
    interval_set a;
    a.insert(start, len);
    interval_set i;
   ...
   */
  void
  insert(T start, T len, T* pstart = 0, T* plen = 0)
    requires(strict)
  {
   ...
   */
  void
  erase(T start, T len, std::function<bool(T, T)> claim = {})
    requires(strict)
  {
  ...
  */
    ...
    requires(!strict)
  {
    ...
    for (auto ap = a.find_inc(start); ap != a.m.end() && ap->first <= end;
         ++ap) {
  ...
  void
  intersection_of(const interval_set& a, const interval_set& b)
  {
    ceph_assert(&a != this);
    ceph_assert(&b != this);
    clear();
  ...
  void
  intersection_of(const interval_set& b)
  {
    interval_set a;
    swap(a);
    intersection_of(a, b);
   ...
   */
  void
  union_of(const interval_set& a, const interval_set& b)
  {
    ceph_assert(&a != this);
    ceph_assert(&b != this);
    clear();
  ...
  bool
  subset_of(const interval_set& big) const
  {
    if (!size())
      return true;
    if (size() > big.size())
   ...
   */
  void
  span_of(const interval_set& other, T start, T len)
  {
    clear();
  ...
  */
  ...
  align(T alignment)
  {
    ...
    for (const auto& [start, len] : m) {
      ...
      T aligned_len = ((start + len + alignment - 1) / alignment) * alignment -
                      aligned_start;
...
public:
  ...
  static constexpr bool need_contiguous =
      denc_traits<T, C<T, T>>::need_contiguous;

  static void
  bound_encode(const container_t& v, size_t& p)
  {
    v.bound_encode(p);
  }

  static void
  encode(const container_t& v, ceph::buffer::list::contiguous_appender& p)
  {
    v.encode(p);
  }

  static void
  decode(container_t& v, ceph::buffer::ptr::const_iterator& p)
  {
    v.decode(p);
  }
  ...
  static void
  encode_nohead(const container_t& v, ceph::buffer::list::contiguous_appender& p)
  {
    v.encode_nohead(p);
  }

  static void
  decode_nohead(size_t n, container_t& v, ceph::buffer::ptr::const_iterator& p)
  {
    v.decode_nohead(n, p);
  }
};
