...
static inline int
ceph_fsid_compare(const struct ceph_fsid* a, const struct ceph_fsid* b)
{
 ...
 */
static inline int
ceph_stable_mod(int x, int b, int bmask)
{
  if ((x & bmask) < b)
    return x & bmask;
  else
...
#define CEPH_OSD_BACKFILLFULL \
  (1 << 6) /* osd is at or above backfillfull threshold */
...
#define CEPH_OSDMAP_NEARFULL \
  (1 << 0) /* sync writes (near ENOSPC), deprecated since mimic*/
#define CEPH_OSDMAP_FULL \
  (1 << 1) /* no data writes (ENOSPC), deprecated since mimic */
...
#define CEPH_OSDMAP_NOREBALANCE \
  (1 << 14) /* block osd backfill unless pg is degraded */
#define CEPH_OSDMAP_SORTBITWISE (1 << 15) /* use bitwise hobject_t sort */
#define CEPH_OSDMAP_REQUIRE_JEWEL (1 << 16) /* require jewel for booting osds */
#define CEPH_OSDMAP_REQUIRE_KRAKEN \
  (1 << 17) /* require kraken for booting osds */
#define CEPH_OSDMAP_REQUIRE_LUMINOUS (1 << 18) /* require l for booting osds */
#define CEPH_OSDMAP_RECOVERY_DELETES \
  (1 << 19) /* deletes performed during recovery instead of peering */
...
#define CEPH_OSDMAP_PGLOG_HARDLIMIT \
  (1 << 22) /* put a hard limit on pg log length */
...
#define CEPH_OSDMAP_SEMIHIDDEN_FLAGS                             \
  (CEPH_OSDMAP_REQUIRE_JEWEL | CEPH_OSDMAP_REQUIRE_KRAKEN |      \
   CEPH_OSDMAP_REQUIRE_LUMINOUS | CEPH_OSDMAP_RECOVERY_DELETES | \
   CEPH_OSDMAP_SORTBITWISE | CEPH_OSDMAP_PURGED_SNAPDIRS |       \
...
#define __CEPH_FORALL_OSD_OPS(f)                                                          \
  ...
  f(READ, __CEPH_OSD_OP(RD, DATA, 1), "read")                                             \
              ...
              f(TRUNCATE, __CEPH_OSD_OP(WR, DATA, 3), "truncate")                         \
                  f(ZERO, __CEPH_OSD_OP(WR, DATA, 4), "zero")                             \
                      f(DELETE, __CEPH_OSD_OP(WR, DATA, 5), "delete")                     \
                                                                                          \
      /* fancy write */                                                                   \
      f(APPEND, __CEPH_OSD_OP(WR, DATA, 6),                                               \
        "append") f(STARTSYNC, __CEPH_OSD_OP(WR, DATA, 7), "startsync")                   \
          f(SETTRUNC, __CEPH_OSD_OP(WR, DATA, 8),                                         \
            "settrunc") f(TRIMTRUNC, __CEPH_OSD_OP(WR, DATA, 9), "trimtrunc")             \
                                                                                          \
              f(TMAPUP, __CEPH_OSD_OP(RMW, DATA, 10),                                     \
                "tmapup") f(TMAPPUT, __CEPH_OSD_OP(WR, DATA, 11), "tmapput")              \
                      ...
                      f(CREATE, __CEPH_OSD_OP(WR, DATA, 13), "create")                    \
                          f(ROLLBACK, __CEPH_OSD_OP(WR, DATA, 14), "rollback")            \
                              ...
                              f(WATCH, __CEPH_OSD_OP(WR, DATA, 15), "watch")              \
                                                                                          \
      /* omap */                                                                          \
      f(OMAPGETKEYS, __CEPH_OSD_OP(RD, DATA, 17), "omap-get-keys") f(                     \
          OMAPGETVALS, __CEPH_OSD_OP(RD, DATA, 18), "omap-get-vals")                      \
          f(OMAPGETHEADER, __CEPH_OSD_OP(RD, DATA, 19), "omap-get-header") f(             \
              OMAPGETVALSBYKEYS, __CEPH_OSD_OP(RD, DATA, 20),                             \
              "omap-get-vals-by-keys")                                                    \
              f(OMAPSETVALS, __CEPH_OSD_OP(WR, DATA, 21), "omap-set-vals")                \
                  f(OMAPSETHEADER, __CEPH_OSD_OP(WR, DATA, 22),                           \
                    "omap-set-header")                                                    \
                      f(OMAPCLEAR, __CEPH_OSD_OP(WR, DATA, 23), "omap-clear")             \
                          f(OMAPRMKEYS, __CEPH_OSD_OP(WR, DATA, 24),                      \
                            "omap-rm-keys")                                               \
                              f(OMAPRMKEYRANGE, __CEPH_OSD_OP(WR, DATA, 44),              \
                                "omap-rm-key-range")                                      \
                                  f(OMAP_CMP, __CEPH_OSD_OP(RD, DATA, 25),                \
                                    "omap-cmp")                                           \
                                                                                          \
      /* tiering */                                                                       \
      f(COPY_FROM, __CEPH_OSD_OP(WR, DATA, 26), "copy-from")                              \
          f(COPY_FROM2, __CEPH_OSD_OP(WR, DATA, 45),                                      \
            "copy-from2") /* was copy-get-classic */                                      \
      f(UNDIRTY, __CEPH_OSD_OP(WR, DATA, 28),                                             \
        "undirty") f(ISDIRTY, __CEPH_OSD_OP(RD, DATA, 29), "isdirty")                     \
          f(COPY_GET, __CEPH_OSD_OP(RD, DATA, 30), "copy-get")                            \
              f(CACHE_FLUSH, __CEPH_OSD_OP(CACHE, DATA, 31), "cache-flush")               \
                  f(CACHE_EVICT, __CEPH_OSD_OP(CACHE, DATA, 32), "cache-evict")           \
                      f(CACHE_TRY_FLUSH, __CEPH_OSD_OP(CACHE, DATA, 33),                  \
                        "cache-try-flush")                                                \
                                                                                          \
      ...
      f(SET_REDIRECT, __CEPH_OSD_OP(WR, DATA, 39), "set-redirect")                        \
          f(SET_CHUNK, __CEPH_OSD_OP(CACHE, DATA, 40), "set-chunk") f(                    \
              TIER_PROMOTE, __CEPH_OSD_OP(WR, DATA, 41), "tier-promote")                  \
              f(UNSET_MANIFEST, __CEPH_OSD_OP(WR, DATA, 42), "unset-manifest")            \
                  f(TIER_FLUSH, __CEPH_OSD_OP(CACHE, DATA, 43), "tier-flush")             \
                      f(TIER_EVICT, __CEPH_OSD_OP(CACHE, DATA, 44),                       \
                        "tier-evict")                                                     \
                                                                                          \
      /** attrs **/ /* read */                                                            \
          ...
          f(SETXATTRS, __CEPH_OSD_OP(WR, ATTR, 2), "setxattrs")                           \
              f(RESETXATTRS, __CEPH_OSD_OP(WR, ATTR, 3), "resetxattrs")                   \
                  f(RMXATTR, __CEPH_OSD_OP(WR, ATTR, 4), "rmxattr")                       \
                                                                                          \
      /** subop **/                                                                       \
      f(PULL, __CEPH_OSD_OP1(SUB, 1),                                                     \
        "pull") f(PUSH, __CEPH_OSD_OP1(SUB, 2), "push")                                   \
          f(BALANCEREADS, __CEPH_OSD_OP1(SUB, 3), "balance-reads")                        \
              f(UNBALANCEREADS, __CEPH_OSD_OP1(SUB, 4),                                   \
                "unbalance-reads") f(SCRUB, __CEPH_OSD_OP1(SUB, 5), "scrub")              \
                  f(SCRUB_RESERVE, __CEPH_OSD_OP1(SUB, 6), "scrub-reserve")               \
                      f(SCRUB_UNRESERVE, __CEPH_OSD_OP1(SUB, 7),                          \
                        "scrub-unreserve") /* 8 used to be scrub-stop */                  \
              ...
              f(PG_HITSET_LS, __CEPH_OSD_OP(RD, PG, 3), "pg-hitset-ls")                   \
                  f(PG_HITSET_GET, __CEPH_OSD_OP(RD, PG, 4), "pg-hitset-get")             \
                      f(PGNLS, __CEPH_OSD_OP(RD, PG, 5), "pgnls")                         \
                          f(PGNLS_FILTER, __CEPH_OSD_OP(RD, PG, 6),                       \
                            "pgnls-filter")                                               \
...
static inline int
ceph_osd_op_type_data(int op)
{
  return (op & CEPH_OSD_OP_TYPE) == CEPH_OSD_OP_TYPE_DATA;
}

static inline int
ceph_osd_op_type_attr(int op)
{
  return (op & CEPH_OSD_OP_TYPE) == CEPH_OSD_OP_TYPE_ATTR;
}

static inline int
ceph_osd_op_type_exec(int op)
{
  return (op & CEPH_OSD_OP_TYPE) == CEPH_OSD_OP_TYPE_EXEC;
}

static inline int
ceph_osd_op_type_pg(int op)
{
  return (op & CEPH_OSD_OP_TYPE) == CEPH_OSD_OP_TYPE_PG;
}

static inline int
ceph_osd_op_mode_subop(int op)
{
  return (op & CEPH_OSD_OP_MODE) == CEPH_OSD_OP_MODE_SUB;
}

static inline int
ceph_osd_op_mode_read(int op)
{
  return (op & CEPH_OSD_OP_MODE_RD) && op != CEPH_OSD_OP_CALL;
}

static inline int
ceph_osd_op_mode_modify(int op)
{
  return op & CEPH_OSD_OP_MODE_WR;
}

static inline int
ceph_osd_op_mode_cache(int op)
{
  return op & CEPH_OSD_OP_MODE_CACHE;
}

static inline bool
ceph_osd_op_uses_extent(int op)
{
  switch (op) {
...
enum {
  ...
  CEPH_OSD_FLAG_PEERSTAT_OLD =
      0x0080, /* DEPRECATED msg includes osd_peer_stat */
  ...
  CEPH_OSD_FLAG_SUPPORTSPOOLEIO =
      0x8000000, /* client understands pool EIO flag */
  CEPH_OSD_FLAG_EC_DIRECT_READ =
      0x10000000, /* Erasure code doing a partial read direct to OSD. */
};

// Indicates an IO which is direct-to-OSD and may not be on the primary.
#define CEPH_OSD_FLAGS_DIRECT_READ                              \
...
enum {
  ...
  CEPH_OSD_OP_FLAG_FADVISE_WILLNEED =
      0x10, /* data will be accessed in the near future */
  CEPH_OSD_OP_FLAG_FADVISE_DONTNEED =
      0x20, /* data will not be accessed in the near future */
  CEPH_OSD_OP_FLAG_FADVISE_NOCACHE =
      0x40, /* data will be accessed only once by this client */
...
enum {
  ...
  CEPH_OSD_COPY_FROM_FLAG_TRUNCATE_SEQ =
      32, /* use provided truncate_{seq,size} (copy-from2 only) */
};
...
#define CEPH_OSD_COPY_FROM_FLAGS                                            \
  (CEPH_OSD_COPY_FROM_FLAG_FLUSH | CEPH_OSD_COPY_FROM_FLAG_IGNORE_OVERLAY | \
   ...
   CEPH_OSD_COPY_FROM_FLAG_RWORDERED | CEPH_OSD_COPY_FROM_FLAG_TRUNCATE_SEQ)
...
#ifdef __cplusplus
static_assert(
    sizeof(ceph_osd_op) == (2 + 4 + (2 * 8 + 8 + 4) + 4),
