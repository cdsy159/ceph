...
#define LIBRGW_FILE_VER_EXTRA \
  1 /* version number needs to advance to
				 * match change in rgw_raddir2 signature */

#define LIBRGW_FILE_VERSION(maj, min, extra) ((maj << 16) + (min << 8) + extra)
#define LIBRGW_FILE_VERSION_CODE \
  LIBRGW_FILE_VERSION(           \
      LIBRGW_FILE_VER_MAJOR, LIBRGW_FILE_VER_MINOR, LIBRGW_FILE_VER_EXTRA)
...
int rgw_rename(
    struct rgw_fs* rgw_fs,
    struct rgw_file_handle* olddir,
    const char* old_name,
    struct rgw_file_handle* newdir,
    const char* new_name,
    uint32_t flags);
