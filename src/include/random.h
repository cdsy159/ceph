...
using larger_of =
    typename std::conditional<sizeof(T0) >= sizeof(T1), T0, T1>::type;

// avoid mixing floating point and integers:
template <typename NumberT0, typename NumberT1>
using has_compatible_numeric_types = std::disjunction<
    std::conjunction<
        std::is_floating_point<NumberT0>,
        std::is_floating_point<NumberT1>>,
    std::conjunction<std::is_integral<NumberT0>, std::is_integral<NumberT1>>>;


// Select the larger of type compatible numeric types:
template <typename NumberT0, typename NumberT1>
using select_number_t = std::enable_if_t<
    detail::has_compatible_numeric_types<NumberT0, NumberT1>::value,
...
template <typename NumberT, bool IsIntegral>
...
template <typename NumberT>
using default_distribution =
    typename select_distribution<NumberT, std::is_integral<NumberT>::value>::type;
...
void
randomize_rng(const SeedT seed, MutexT& m, EngineT& e)
{
...
void
randomize_rng(MutexT& m, EngineT& e)
{
  random_device_t rd;
...
template <
    ...
    typename EngineT>
NumberT
generate_random_number(const NumberT min, const NumberT max, EngineT& e)
{
...
template <
    ...
    typename EngineT>
NumberT
generate_random_number(const NumberT min, const NumberT max)
{
...
template <
    ...
    typename DistributionT = detail::default_distribution<NumberT>>
NumberT
generate_random_number(MutexT& m, EngineT& e)
{
...
NumberT
generate_random_number(const NumberT max, MutexT& m, EngineT& e)
{
...
NumberT
generate_random_number(const NumberT0 min, const NumberT1 max)
{
  return detail::generate_random_number<
      NumberT, detail::default_distribution<NumberT>, std::default_random_engine>(
...
public:
  ...
  NumberT
  operator()(const NumberT min, const NumberT max)
  {
