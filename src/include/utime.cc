...
void
utime_t::dump(ceph::Formatter* f) const
{
...
std::list<utime_t>
utime_t::generate_test_instances()
{
  std::list<utime_t> o;
  o.push_back(utime_t());
  o.push_back(utime_t());
...
std::ostream&
utime_t::gmtime(std::ostream& out, bool legacy_form) const
{
  ...
  if (sec() < ((time_t)(60 * 60 * 24 * 365 * 10))) {
    ...
    out << std::setw(2) << bdt.tm_hour << ':' << std::setw(2) << bdt.tm_min
...
std::ostream&
utime_t::gmtime_nsec(std::ostream& out) const
{
  out.setf(std::ios::right);
  char oldfill = out.fill();
  out.fill('0');
  if (sec() < ((time_t)(60 * 60 * 24 * 365 * 10))) {
    ...
    out << std::setw(4) << (bdt.tm_year + 1900) // 2007 -> '07'
        << '-' << std::setw(2) << (bdt.tm_mon + 1) << '-' << std::setw(2)
        << bdt.tm_mday << 'T' << std::setw(2) << bdt.tm_hour << ':'
...
std::ostream&
utime_t::asctime(std::ostream& out) const
{
  out.setf(std::ios::right);
  char oldfill = out.fill();
  out.fill('0');
...
std::ostream&
utime_t::localtime(std::ostream& out, bool legacy_form) const
{
  out.setf(std::ios::right);
  char oldfill = out.fill();
  out.fill('0');
