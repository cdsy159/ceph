...
public:
  ...
  static inline constexpr utime_t
  max()
  {
  ...
  constexpr utime_t(time_t s, int n)
  {
    tv.tv_sec = s;
    tv.tv_nsec = n;
    normalize();
  ...
  constexpr utime_t(const struct ceph_timespec& v) { decode_timeval(&v); }
  ...
  template <
      typename Clock,
      typename std::enable_if_t<ceph::converts_to_timespec_v<Clock>>* = nullptr>
  ...
  explicit utime_t(const std::chrono::duration<Rep, Period>& dur)
  {
    ...
    tv.tv_sec = std::max<common_t>(
        std::chrono::duration_cast<std::chrono::seconds>(dur).count(), 0);
  ...
  explicit
  operator seastar::lowres_system_clock::time_point() const noexcept
  {
  ...
  void
  to_timespec(struct timespec* ts) const
  {
    ts->tv_sec = tv.tv_sec;
  ...
  void
  copy_to_timeval(struct timeval* v) const
  {
    v->tv_sec = tv.tv_sec;
  ...
  void
  set_from_timeval(const struct timeval* v)
  {
    tv.tv_sec = v->tv_sec;
  ...
  {
    static_assert(
        sizeof(utime_t) == sizeof(tv.tv_sec) + sizeof(tv.tv_nsec),
  ...
  void
  encode_timeval(struct ceph_timespec* t) const
  {
    t->tv_sec = tv.tv_sec;
  ...
  constexpr void
  decode_timeval(const struct ceph_timespec* t)
  {
    tv.tv_sec = t->tv_sec;
...
inline utime_t
operator+(const utime_t& l, const utime_t& r)
{
...
inline utime_t&
operator+=(utime_t& l, const utime_t& r)
{
...
// comparators
inline bool
operator>(const utime_t& a, const utime_t& b)
{
...
// ostream
inline std::ostream&
operator<<(std::ostream& out, const utime_t& t)
{
  return t.localtime(out);
}

inline std::string
utimespan_str(const utime_t& age)
{
