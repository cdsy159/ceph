...
struct C_GetTags : public Context {
  ...
  {
    librados::ObjectReadOperation op;
    client::tag_list_start(
        &op, start_after_tag_tid, MAX_RETURN, client_id, tag_class);

    librados::AioCompletion* comp = librados::Rados::aio_create_completion(
        this,
        &utils::rados_state_callback<C_GetTags, &C_GetTags::handle_tag_list>);
...
void
JournalMetadata::init(Context* on_finish)
{
  {
    std::lock_guard locker{m_lock};
    ceph_assert(!m_initialized);
  ...
  on_finish = new LambdaContext([this, on_finish](int r) {
    if (r < 0) {
      lderr(m_cct) << __func__ << ": failed to watch journal" << cpp_strerror(r)
                   << dendl;
      std::lock_guard locker{m_lock};
      m_watch_handle = 0;
      on_finish->complete(r);
...
void
JournalMetadata::shut_down(Context* on_finish)
{
    ...
    Context* on_finish)
{
  client::get_immutable_metadata(
      m_ioctx, m_oid, order, splay_width, pool_id, on_finish);
}
    ...
    Context* on_finish)
{
  client::get_mutable_metadata(
      m_ioctx, m_oid, minimum_set, active_set, clients, on_finish);
}

void
JournalMetadata::register_client(const bufferlist& data, Context* on_finish)
{
...
void
JournalMetadata::update_client(const bufferlist& data, Context* on_finish)
{
  ldout(m_cct, 10) << __func__ << ": " << m_client_id << dendl;
  librados::ObjectWriteOperation op;
...
void
JournalMetadata::unregister_client(Context* on_finish)
{
    ...
    Context* on_finish)
{
  ...
  C_AllocateTag* ctx = new C_AllocateTag(
      m_cct, m_ioctx, m_oid, m_async_op_tracker, tag_class, data, tag,
      on_finish);
  ctx->send();
}

void
JournalMetadata::get_client(
    const std::string& client_id,
    cls::journal::Client* client,
    Context* on_finish)
{
  C_GetClient* ctx = new C_GetClient(
      m_cct, m_ioctx, m_oid, m_async_op_tracker, client_id, client, on_finish);
  ctx->send();
}

void
JournalMetadata::get_tag(uint64_t tag_tid, Tag* tag, Context* on_finish)
{
  C_GetTag* ctx = new C_GetTag(
      m_cct, m_ioctx, m_oid, m_async_op_tracker, tag_tid, tag, on_finish);
  ctx->send();
}

void
JournalMetadata::get_tags(
    uint64_t start_after_tag_tid,
    const boost::optional<uint64_t>& tag_class,
    Tags* tags,
    Context* on_finish)
{
  C_GetTags* ctx = new C_GetTags(
      m_cct, m_ioctx, m_oid, m_client_id, m_async_op_tracker,
      start_after_tag_tid, tag_class, tags, on_finish);
  ctx->send();
}

void
JournalMetadata::add_listener(JournalMetadataListener* listener)
{
...
void
JournalMetadata::remove_listener(JournalMetadataListener* listener)
{
  std::unique_lock locker{m_lock};
  m_update_cond.wait(locker, [this] { return m_update_notifications <= 0; });
...
void
JournalMetadata::set_minimum_set(uint64_t object_set)
{
  std::lock_guard locker{m_lock};
...
int
JournalMetadata::set_active_set(uint64_t object_set)
{
  C_SaferCond ctx;
...
void
JournalMetadata::set_active_set(uint64_t object_set, Context* on_finish)
{
  std::lock_guard locker{m_lock};
...
void
JournalMetadata::assert_active_tag(uint64_t tag_tid, Context* on_finish)
{
  std::lock_guard locker{m_lock};

  C_AssertActiveTag* ctx = new C_AssertActiveTag(
      m_cct, m_ioctx, m_oid, m_async_op_tracker, m_client_id, tag_tid,
      on_finish);
  ctx->send();
}
...
void
JournalMetadata::reserve_entry_tid(uint64_t tag_tid, uint64_t entry_tid)
{
  std::lock_guard locker{m_lock};
...
void
JournalMetadata::handle_immutable_metadata(int r, Context* on_init)
{
  if (r < 0) {
...
void
JournalMetadata::refresh(Context* on_complete)
{
  ...
  get_mutable_metadata(
      &refresh->minimum_set, &refresh->active_set, &refresh->registered_clients,
      refresh);
}

void
JournalMetadata::handle_refresh_complete(C_Refresh* refresh, int r)
{
...
void
JournalMetadata::cancel_commit_task()
{
  ldout(m_cct, 20) << __func__ << dendl;
...
void
JournalMetadata::schedule_commit_task()
{
  ldout(m_cct, 20) << __func__ << dendl;
  ...
  if (m_commit_position_task_ctx == nullptr) {
    m_commit_position_task_ctx = m_timer->add_event_after(
        m_settings.commit_interval, new C_CommitPositionTask(this));
  }
}
...
JournalMetadata::handle_commit_position_task()
{
  ceph_assert(ceph_mutex_is_locked(*m_timer_lock));
  ceph_assert(ceph_mutex_is_locked(m_lock));
  ldout(m_cct, 20) << __func__ << ": " << "client_id=" << m_client_id << ", "
...
void
JournalMetadata::schedule_watch_reset()
{
  ceph_assert(ceph_mutex_is_locked(*m_timer_lock));
...
JournalMetadata::handle_watch_reset()
{
  ...
  if (r < 0) {
    if (r == -ENOENT) {
      ...
      lderr(m_cct) << __func__
                   << ": failed to watch journal: " << cpp_strerror(r) << dendl;
    }
...
void
JournalMetadata::handle_watch_notify(uint64_t notify_id, uint64_t cookie)
{
...
void
JournalMetadata::handle_watch_error(int err)
{
  if (err == -ENOTCONN) {
...
uint64_t
JournalMetadata::allocate_commit_tid(
    uint64_t object_num,
    uint64_t tag_tid,
    uint64_t entry_tid)
{
  ...
  m_pending_commit_tids[commit_tid] =
      CommitEntry(object_num, tag_tid, entry_tid);
...
void
JournalMetadata::overflow_commit_tid(uint64_t commit_tid, uint64_t object_num)
{
  std::lock_guard locker{m_lock};
  ...
  ldout(m_cct, 20) << __func__ << ": " << "commit_tid=" << commit_tid << ", "
...
void
JournalMetadata::get_commit_entry(
    uint64_t commit_tid,
    uint64_t* object_num,
    uint64_t* tag_tid,
    uint64_t* entry_tid)
...
void
JournalMetadata::async_notify_update(Context* on_safe)
{
...
void
JournalMetadata::handle_notified(int r)
{
  ldout(m_cct, 10) << "notified journal header update: r=" << r << dendl;
}

void
JournalMetadata::schedule_laggy_clients_disconnect(Context* on_finish)
{
  ldout(m_cct, 20) << __func__ << dendl;
...
std::ostream&
operator<<(std::ostream& os, const JournalMetadata::RegisteredClients& clients)
{
  os << "[";
...
std::ostream&
operator<<(std::ostream& os, const JournalMetadata& jm)
{
  std::lock_guard locker{jm.m_lock};
  os << "[oid=" << jm.m_oid << ", " << "initialized=" << jm.m_initialized
     << ", " << "order=" << (int)jm.m_order << ", "
     << "splay_width=" << (int)jm.m_splay_width << ", "
     << "pool_id=" << jm.m_pool_id << ", " << "minimum_set=" << jm.m_minimum_set
