...
public:
  ...
  void
  get_commit_position(ObjectSetPosition* commit_position) const
  {
    std::lock_guard locker{m_lock};
  ...
  void
  get_registered_clients(RegisteredClients* registered_clients)
  {
    std::lock_guard locker{m_lock};
  ...
  inline uint64_t
  allocate_entry_tid(uint64_t tag_tid)
  {
    std::lock_guard locker{m_lock};
...
private:
  ...
  struct CommitEntry {
    ...
    CommitEntry() :
      object_num(0), tag_tid(0), entry_tid(0), committed(false)
    {}
