...
#undef dout_prefix
#define dout_prefix \
  *_dout << "JournalRecorder: " << this << " " << __func__ << ": "
...
struct C_Flush : public Context {
  ...
  C_Flush(
      ceph::ref_t<JournalMetadata> _journal_metadata,
      Context* _on_finish,
...
JournalRecorder::JournalRecorder(
  ...
  m_object_locks{ceph::make_lock_container<ceph::mutex>(
      m_journal_metadata->get_splay_width(),
      [](const size_t splay_offset) {
      ...
      })}
{
  ...
  for (uint8_t splay_offset = 0; splay_offset < splay_width; ++splay_offset) {
    ...
    m_object_ptrs[splay_offset] =
        create_object_recorder(object_number, &m_object_locks[splay_offset]);
  }
...
void
JournalRecorder::shut_down(Context* on_safe)
{
  on_safe = new LambdaContext([this, on_safe](int r) {
    Context* ctx = nullptr;
    {
      ...
      if (m_in_flight_advance_sets != 0) {
        ...
        m_on_object_set_advanced = new LambdaContext([on_safe, r](int) {
          on_safe->complete(r);
        });
      } else {
        ctx = on_safe;
...
void
JournalRecorder::set_append_batch_options(
    int flush_interval,
    ...
    double flush_age)
{
  ...
  for (uint8_t splay_offset = 0; splay_offset < splay_width; ++splay_offset) {
    ...
    object_recorder->set_append_batch_options(
        flush_interval, flush_bytes, flush_age);
  }
}

Future
JournalRecorder::append(uint64_t tag_tid, const bufferlist& payload_bl)
{
  ...
  bufferlist entry_bl;
  encode(
      Entry(future->get_tag_tid(), future->get_entry_tid(), payload_bl),
      entry_bl);
...
void
JournalRecorder::flush(Context* on_safe)
{
...
void
JournalRecorder::close_and_advance_object_set(uint64_t object_set)
{
  ceph_assert(ceph_mutex_is_locked(m_lock));
...
JournalRecorder::advance_object_set()
{
  ...
  m_journal_metadata->set_active_set(
      m_current_set, new C_AdvanceObjectSet(this));
}

void
JournalRecorder::handle_advance_object_set(int r)
{
...
bool
JournalRecorder::close_object_set(uint64_t active_set)
{
...
ceph::ref_t<ObjectRecorder>
JournalRecorder::create_object_recorder(uint64_t object_number, ceph::mutex* lock)
{
...
bool
JournalRecorder::create_next_object_recorder(
...
void
JournalRecorder::handle_closed(ObjectRecorder* object_recorder)
{
...
void
JournalRecorder::handle_overflow(ObjectRecorder* object_recorder)
{
  ldout(m_cct, 10) << object_recorder->get_oid() << dendl;
...
JournalRecorder::Lockers
JournalRecorder::lock_object_recorders()
{
  Lockers lockers;
