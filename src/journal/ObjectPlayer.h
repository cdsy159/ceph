...
public:
  ...
  inline void
  get_invalid_ranges(InvalidRanges* invalid_ranges)
  {
    std::lock_guard locker{m_lock};
  ...
  inline void
  set_refetch_state(RefetchState refetch_state)
  {
    m_refetch_state = refetch_state;
  }

  inline void
  set_max_fetch_bytes(uint64_t max_fetch_bytes)
  {
    std::lock_guard locker{m_lock};
