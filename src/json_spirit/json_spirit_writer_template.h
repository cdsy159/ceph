...
bool
add_esc_char(Char_type c, String_type& s)
{
  switch (c) {
  case '"':
...
template <class String_type>
String_type
add_esc_chars(const String_type& s, bool raw_utf8)
{
...
void
append_double(Ostream& os, const double d, const int precision)
{
...
template <class String_type>
void
erase_and_extract_exponent(String_type& str, String_type& exp)
{
...
find_first_non_zero(const String_type& str)
{
...
template <class String_type>
void
remove_trailing(String_type& str)
{
  String_type exp;

  erase_and_extract_exponent(str, exp);

  const typename String_type::size_type first_non_zero =
  ...
  if (first_non_zero != 0) {
    const int offset =
        str[first_non_zero] == '.'
            ? 2
            : 1; // note zero digits following a decimal point is non standard
...
public:
  Generator(const Value_type& value, Ostream_type& os, unsigned int options) :
    ...
    pretty_(
        (options & pretty_print) != 0 || (options & single_line_arrays) != 0),
...
private:
  ...
  void
  output(const Object_type& obj)
  {
    output_array_or_obj(obj, '{', '}');
  }

  void
  output(const Obj_member_type& member)
  {
  ...
  void
  output_int(const Value_type& value)
  {
    if (value.is_uint64()) {
  ...
  static bool
  contains_composite_elements(const Array_type& arr)
  {
  ...
  template <class Iter>
  void
  output_composite_item(Iter i, Iter last)
  {
    output(*i);
  ...
  template <class T>
  void
  output_array_or_obj(const T& t, Char_type start_char, Char_type end_char)
  {
    os_ << start_char;
    new_line();
  ...
  Generator& operator=(
      const Generator&); // to prevent "assignment operator could not be generated" warning
...
template <class Value_type, class Ostream_type>
void
write_stream(const Value_type& value, Ostream_type& os, unsigned int options = 0)
{
  os << std::dec;
...
write_string(const Value_type& value, unsigned int options = 0)
{
