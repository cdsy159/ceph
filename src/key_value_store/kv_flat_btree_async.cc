...
bool
index_data::is_timed_out(utime_t now, utime_t timeout) const
{
...
void
IndexCache::push(const string& key, const index_data& idata)
{
  if (cache_size == 0) {
    return;
  }
...
void
IndexCache::push(const index_data& idata)
{
  if (cache_size == 0) {
    return;
  }
...
void
IndexCache::erase(key_data kdata)
{
  if (cache_size == 0) {
    return;
  }
...
int
IndexCache::get(const string& key, index_data* idata) const
{
  if (cache_size == 0) {
    return -ENODATA;
  }
...
int
IndexCache::get(const string& key, index_data* idata, index_data* next_idata) const
{
  if (cache_size == 0) {
    return -ENODATA;
  }
...
int
KvFlatBtreeAsync::nothing()
{
  return 0;
}

int
KvFlatBtreeAsync::wait()
{
  if (rand() % 10 == 0) {
    usleep(wait_ms);
  }
...
int
KvFlatBtreeAsync::next(const index_data& idata, index_data* out_data)
{
  if (verbose)
  ...
  if (err < 0) {
    if (verbose)
      cout << "\t\t\t" << client_name
           << "-next: getting index failed with error " << err << std::endl;
    return err;
  }
  if (!kvs.empty()) {
...
int
KvFlatBtreeAsync::prev(const index_data& idata, index_data* out_data)
{
  if (verbose)
    cout << "\t\t" << client_name << "-prev: finding prev of " << idata.str()
         << std::endl;
  int err = 0;
  bufferlist inbl;
  ...
  if (err < 0) {
    if (verbose)
      cout << "\t\t\t" << client_name
           << "-prev: getting index failed with error " << err << std::endl;
    if (idata.is_timed_out(ceph_clock_now(), timeout)) {
      if (verbose)
        cout << client_name << " THINKS THE OTHER CLIENT DIED." << std::endl;
      //the client died after deleting the object. clean up.
  ...
  *out_data = in_args.next_idata;
  if (verbose)
    cout << "\t\t" << client_name << "-prev: prev is " << out_data->str()
         << std::endl;
  return err;
}

int
KvFlatBtreeAsync::read_index(
    const string& key,
    index_data* idata,
    ...
    bool force_update)
{
  ...
  if (!force_update) {
    if (verbose)
      cout << "\t" << client_name << "-read_index: getting index_data for "
           << key << " from cache" << std::endl;
    icache_lock.lock();
    if (next_idata != NULL) {
    ...
    if (this_idata.is_timed_out(mytime, timeout)) {
      if (verbose)
        cout << client_name << " THINKS THE OTHER CLIENT DIED. (mytime is "
             ...
             << (mytime - this_idata.ts).usec() << ", timeout is " << timeout
             << ")" << std::endl;
      //the client died after deleting the object. clean up.
  ...
  if (verbose)
    cout << "\t" << client_name << "-read_index: kvmap_size is " << kvmap.size()
...
int
KvFlatBtreeAsync::split(const index_data& idata)
{
  int err = 0;
  opmap['l']++;
  ...
  if (err < 0) {
    if (verbose)
      cout << "\t\t" << client_name << "-split: read object " << args.odata.name
           << " got " << err << std::endl;
    return err;
  }
  ...
  if (verbose)
    cout << "\t\t" << client_name << "-split: splitting " << idata.obj
         << ", which has size " << args.odata.size << " and actual size "
         << args.odata.omap.size() << std::endl;

  ///////preparations that happen outside the critical section
  //for prefix index
  vector<object_data> to_create;
  vector<object_data> to_delete;
  to_delete.push_back(object_data(
      idata.min_kdata, args.odata.max_kdata, args.odata.name,
      args.odata.version));
  ...
  to_create.push_back(object_data(
      to_create[0].max_kdata, args.odata.max_kdata,
  ...
  ops.push_back(
      std::make_pair(std::pair<int, string>(ADD_PREFIX, index_name), &owos[0]));
  for (int i = 1; i < 6; i++) {
...
int
KvFlatBtreeAsync::rebalance(
    const index_data& idata1,
    const index_data& next_idata)
{
  ...
  if (idata1.kdata.prefix == "1") {
    ...
    if (err < 0) {
      if (verbose)
        cout << "reading " << idata1.obj << " failed with " << err << std::endl;
      if (err == -ENOENT) {
        return -ECANCELED;
      }
      return err;
    ...
    if (verbose)
      cout << "\t\t" << client_name << "-rebalance: read " << idata2.obj
           << ". size: " << args1.odata.size
    ...
    if (verbose)
      cout << "\t\t" << client_name << "-rebalance: read " << idata2.obj
           << ". size: " << args2.odata.size
           << " version: " << args2.odata.version << std::endl;
  }

  if (verbose)
    cout << "\t\t" << client_name << "-rebalance: o1 is "
         << args1.odata.max_kdata.encoded() << "," << args1.odata.name
         << " with size " << args1.odata.size << " , o2 is "
         << args2.odata.max_kdata.encoded() << "," << args2.odata.name
         << " with size " << args2.odata.size << std::endl;

  //calculations
  if ((int)args1.odata.size > k && (int)args1.odata.size <= 2 * k &&
      (int)args2.odata.size > k && (int)args2.odata.size <= 2 * k) {
    //nothing to do
    if (verbose)
  ...
  ops.push_back(std::make_pair(
      std::pair<int, string>(ADD_PREFIX, index_name), &other_ops[0]));

  if ((int)args1.odata.size + (int)args2.odata.size <= 2 * k) {
    //merge
    if (verbose)
      cout << "\t\t" << client_name << "-rebalance: merging "
           << args1.odata.name << " and " << args2.odata.name << " to get "
           << o2w << std::endl;
    ...
    to_create.push_back(object_data(
        args1.odata.min_kdata, args2.odata.max_kdata, o2w, write2_map));
    ops.push_back(std::make_pair(
        std::pair<int, std::string>(MAKE_OBJECT, o2w), &create[0]));
    ceph_assert((int)write2_map.size() <= 2 * k);
  } else {
    //rebalance
    if (verbose)
      cout << "\t\t" << client_name << "-rebalance: rebalancing "
           << args1.odata.name << " and " << args2.odata.name << std::endl;
    ...
    client_index_lock.unlock();
    int target_size_1 =
        ceil(((int)args1.odata.size + (int)args2.odata.size) / 2.0);
    if (args1.odata.max_kdata != idata1.kdata) {
      //this should be true if idata1 is the high object
      target_size_1 =
          floor(((int)args1.odata.size + (int)args2.odata.size) / 2.0);
    }
    for (it = args1.odata.omap.begin();
         it != args1.odata.omap.end() && (int)write1_map.size() < target_size_1;
         ++it) {
    ...
    if (it != args1.odata.omap.end()) {
      ...
      for (it2 = args2.odata.omap.begin();
           (it2 != args2.odata.omap.end()) &&
           ((int)write1_map.size() < target_size_1);
           ++it2) {
    ...
    if (verbose)
      cout << "\t\t" << client_name << "-rebalance: write1_map has size "
           << write1_map.size() << ", write2_map.size() is "
           << write2_map.size() << std::endl;
    //at this point, write1_map and write2_map should have the correct pairs
    to_create.push_back(object_data(
        args1.odata.min_kdata, key_data(write1_map.rbegin()->first), o1w,
        write1_map));
    to_create.push_back(object_data(
        key_data(write1_map.rbegin()->first), args2.odata.max_kdata, o2w,
        write2_map));
    ops.push_back(std::make_pair(
        std::pair<int, std::string>(MAKE_OBJECT, o1w), &create[0]));
    ops.push_back(std::make_pair(
        std::pair<int, std::string>(MAKE_OBJECT, o2w), &create[1]));
  }

  to_delete.push_back(object_data(
      args1.odata.min_kdata, args1.odata.max_kdata, args1.odata.name,
      args1.odata.version));
  to_delete.push_back(object_data(
      args2.odata.min_kdata, args2.odata.max_kdata, args2.odata.name,
      args2.odata.version));
  for (int i = 1; i < 6; i++) {
  ...
  icache_lock.unlock();
  if (verbose)
    cout << "\t\t" << client_name << "-rebalance: done rebalancing."
         << std::endl;
  /////END CRITICAL SECTION/////
  return err;
}
...
int
KvFlatBtreeAsync::read_object(const string& obj, object_data* odata)
{
...
int
KvFlatBtreeAsync::read_object(const string& obj, rebalance_args* args)
{
  bufferlist inbl;
  args->encode(inbl);
  bufferlist outbl;
  ...
  if (err < 0) {
    if (verbose)
      cout << "\t\t" << client_name << "-read_object: reading failed with "
           << err << std::endl;
    a->release();
    return err;
  }
...
void
KvFlatBtreeAsync::set_up_prefix_index(
    ...
    int* err)
{
  ...
  for (vector<object_data>::const_iterator it = to_delete.begin();
    ...
    assertions[it->max_kdata.encoded()] =
...
void
KvFlatBtreeAsync::set_up_ops(
    const vector<object_data>& create_vector,
    const vector<object_data>& delete_vector,
    vector<std::pair<std::pair<int, string>, librados::ObjectWriteOperation*>>*
        ops,
    const index_data& idata,
    int* err)
{
  vector<std::pair<std::pair<int, string>, librados::ObjectWriteOperation*>>::iterator
      it;
  ...
  for (int i = 0; i < (int)idata.to_create.size(); ++i) {
    index_data this_entry(
        idata.to_create[i].max, idata.to_create[i].min, idata.to_create[i].obj);
    ...
    if (idata.to_create.size() <= 2) {
      ...
      it->first =
          std::pair<int, string>(AIO_MAKE_OBJECT, idata.to_create[i].obj);
    }
  ...
  for (int i = 0; i < (int)idata.to_delete.size(); ++i) {
    ...
    if (verbose)
      cout << "\t\t\t" << client_name << "-setup_ops: will assert "
           << this_entry.str() << std::endl;
...
void
KvFlatBtreeAsync::set_up_make_object(
...
void
KvFlatBtreeAsync::set_up_unwrite_object(
    const int& ver,
    librados::ObjectWriteOperation* owo)
{
  if (ver > 0) {
...
void
KvFlatBtreeAsync::set_up_restore_object(librados::ObjectWriteOperation* owo)
{
...
void
KvFlatBtreeAsync::set_up_delete_object(librados::ObjectWriteOperation* owo)
{
  owo->cmpxattr("unwritable", CEPH_OSD_CMPXATTR_OP_EQ, to_bl("1"));
  owo->remove();
}
        ...
        ops)
{
  ...
  int count = 0;
  for (vector<std::pair<std::pair<int, string>, librados::ObjectWriteOperation*>>::
    ...
    case ADD_PREFIX: //prefixing
      ...
      if (err < 0) {
        if (verbose)
          cout << debug_prefix << " prefixing the index failed with " << err
               << std::endl;
        return -EPREFIX;
      }
      if (verbose)
    ...
    case MAKE_OBJECT: //creating
      if (verbose)
        cout << debug_prefix << " creating " << it->first.second << std::endl;
      err = io_ctx.operate(it->first.second, it->second);
      if (err < 0) {
    ...
    case AIO_MAKE_OBJECT:
      ...
      if ((int)idata.to_create.size() == count) {
        ...
        for (count -= 1; count >= 0; count--) {
          ...
          if (err < 0) {
            //this can happen if someone else was cleaning up after us.
            cerr << debug_prefix << " a create failed" << " with code " << err
                 << std::endl;
            if (err == -EEXIST) {
              //someone thinks we died, so die
    ...
    case REMOVE_PREFIX: //rewriting index
      ...
      if (err < 0) {
        if (verbose)
          cout << debug_prefix << " rewriting the index failed with code "
...
int
KvFlatBtreeAsync::cleanup(const index_data& idata, const int& error)
{
  if (verbose)
  ...
  case -EFIRSTOBJ: {
    ...
    for (vector<delete_data>::const_iterator it = idata.to_delete.begin();
      ...
      if (verbose)
        cout << "\t\t\t" << client_name
             << "-cleanup: will assert index contains " << this_entry.str()
             << std::endl;
      assertions[it->max.encoded()] = std::pair<bufferlist, int>(
    ...
    if (verbose)
      cout << "\t\t\t" << client_name << "-cleanup: updating index"
           << std::endl;
    if ((((KeyValueStructure*)this)->*KvFlatBtreeAsync::interrupt)() == 1) {
      return -ESUICIDE;
    }
    ...
    if (verbose)
      cout << "\t\t\t" << client_name
           << "-cleanup: updated index. cleanup done." << std::endl;
    break;
  }
  case -ERANGE: {
    ...
    if (verbose)
      cout << "\t\t\t" << client_name << "-cleanup: restoring " << it->obj
           << std::endl;
    ...
    if (err < 0) {
      ...
      if (verbose)
        cout << "\t\t\t" << client_name << "-cleanup: restored " << it->obj
             << std::endl;
    }
  ...
  case -ENOENT: {
    ...
    for (int i = 0; i <= (int)idata.to_delete.size(); ++i) {
      ops.push_back(
          std::make_pair(std::pair<int, std::string>(0, ""), &owos[i]));
    }
  ...
  default: {
    ...
    for (vector<create_data>::const_reverse_iterator it =
      ...
      if (verbose)
        cout << "\t\t\t" << client_name << "-cleanup: marking " << it->obj
             << std::endl;
      ...
      if (err < 0) {
        ...
        if (verbose)
          cout << "\t\t\t" << client_name << "-cleanup: marked " << it->obj
               << std::endl;
      }
    }
      ...
      if (err == -ENOENT) {
        ...
        if (verbose)
          cout << "\t\t\t" << client_name << "-cleanup: roll forward instead"
               << std::endl;
        for (vector<create_data>::const_iterator cit = idata.to_create.begin();
          ...
          if (verbose)
            cout << "\t\t\t" << client_name << "-cleanup: restoring "
                 << cit->obj << std::endl;
          ...
          if (verbose)
            cout << "\t\t\t" << client_name << "-cleanup: restored " << cit->obj
                 << std::endl;
        }
    ...
    for (vector<create_data>::const_reverse_iterator it =
      ...
      if (verbose)
        cout << "\t\t\t" << client_name << "-cleanup: removing " << it->obj
             << std::endl;
      err = io_ctx.operate(it->obj, &rm);
      if (err < 0) {
        if (verbose)
          cout << "\t\t\t" << client_name << "-cleanup: failed to remove "
               << it->obj << std::endl;
      } else {
        if (verbose)
          cout << "\t\t\t" << client_name << "-cleanup: removed " << it->obj
               << std::endl;
      }
    }
...
string
KvFlatBtreeAsync::to_string(string s, int i)
{
  stringstream ret;
  ret << s << i;
  return ret.str();
...
void
KvFlatBtreeAsync::set_inject(injection_t inject, int wait_time)
{
  interrupt = inject;
  wait_ms = wait_time;
}
...
int
KvFlatBtreeAsync::setup(int argc, const char** argv)
{
  ...
  if (verbose)
    cout << client_name << ": created index object" << std::endl;
  ...
  if (r < 0) {
    if (verbose)
      cout << client_name << ": Setting xattr failed with code " << r
           << std::endl;
  }
    ...
    bool update_on_existing)
{
  if (verbose)
    cout << client_name << " is "
         << (update_on_existing ? "updating " : "setting ") << key << std::endl;
  int err = 0;
  utime_t mytime;
  ...
  if (err < 0) {
    if (verbose)
      cout << "\t" << client_name << ": getting oid failed with code " << err
           << std::endl;
    return err;
  }
  if (verbose)
  ...
  if (verbose)
    cout << "\t" << client_name << ": finished set with " << err << std::endl;
  return err;
}

int
KvFlatBtreeAsync::set_op(
    const string& key,
    const bufferlist& val,
    bool update_on_existing,
    index_data& idata)
{
  ...
  if (err < 0) {
    ...
    case -EEXIST: {
      ...
      cerr << "\t" << client_name << ": writing key failed with " << err
           << std::endl;
      return err;
    }
    case -EKEYREJECTED: {
      ...
      do {
        if (verbose)
          cout << "\t" << client_name << ": running split on " << idata.obj
               << std::endl;
        ...
        if (err < 0 && err != -ENOENT && err != -EBALANCE) {
          if (verbose)
            cerr << "\t" << client_name << ": split failed with " << err
                 << std::endl;
    ...
    default:
      if (verbose)
        cerr << "\t" << client_name << ": writing obj failed with " << err
             << std::endl;
      if (err == -ENOENT || err == -EACCES) {
        if (err == -ENOENT) {
          if (verbose)
            cout << "CACHE FAILURE" << std::endl;
        }
        err = read_index(key, &idata, NULL, true);
        if (err < 0) {
          if (verbose)
...
int
KvFlatBtreeAsync::remove(const string& key)
{
  if (verbose)
  ...
  if (err < 0) {
    if (verbose)
      cout << "getting oid failed with code " << err << std::endl;
    return err;
  }
  obj = idata.obj;
  if (verbose)
    cout << "\t" << client_name << ": idata is " << idata.str()
         << ", next_idata is " << next_idata.str() << ", obj is " << obj
         << std::endl;

  err = remove_op(key, idata, next_idata);

  if (verbose)
    cout << "\t" << client_name << ": finished remove with " << err
         << " and exiting" << std::endl;
  return err;
}

int
KvFlatBtreeAsync::remove_op(
    const string& key,
    index_data& idata,
    index_data& next_idata)
{
  ...
  if (verbose)
    cout << "\t" << client_name << ": removing " << key << " from " << idata.obj
         << std::endl;
  int err = io_ctx.operate(idata.obj, &owo);
  if (err < 0) {
    if (verbose)
      cout << "\t" << client_name << ": writing obj failed with " << err
           << std::endl;
    switch (err) {
    case -ENODATA: {
    ...
    case -EKEYREJECTED: {
      ...
      do {
        if (verbose)
          cerr << "\t" << client_name << ": running rebalance on " << idata.obj
               << std::endl;
        ...
        if (err < 0 && err != -ENOENT && err != -EBALANCE) {
          if (verbose)
            cerr << "\t" << client_name << ": rebalance returned " << err
                 << std::endl;
          int ret =
              handle_set_rm_errors(err, idata.obj, key, &idata, &next_idata);
          switch (ret) {
          case -ESUICIDE:
            if (verbose)
              cout << client_name << " IS SUICIDING!" << std::endl;
          ...
          case -EUCLEAN:
            ...
            if (verbose)
              cout << "\t" << client_name << ": removing " << key << " from "
                   << idata.obj << std::endl;
            int err = io_ctx.operate(idata.obj, &owo);
            if (err == 0) {
              return 0;
...
int
KvFlatBtreeAsync::handle_set_rm_errors(
    int& err,
    string obj,
    string key,
    index_data* idata,
    index_data* next_idata)
{
  if (err == -ESUICIDE) {
    return err;
      ...
      err == -ECANCELED //if an object was unwritable or index changed
      ...
      || err == -EACCES) //possible if we were acting on old index data
  {
    ...
    if (idata->obj != obj) {
      ...
      err != -ETIMEDOUT && err != -ERANGE && err != -EACCES && err != -EUCLEAN) {
    if (verbose)
      cout << "\t" << client_name
...
int
KvFlatBtreeAsync::get(const string& key, bufferlist* val)
{
  opmap['g']++;
  if (verbose)
  ...
  if (verbose)
    cout << client_name << ": got " << key << " with " << err << std::endl;

  return err;
}

int
KvFlatBtreeAsync::get_op(const string& key, bufferlist* val, index_data& idata)
{
  int err = 0;
  std::set<std::string> key_set;
  key_set.insert(key);
  ...
  if (err < 0) {
    if (err == -ENOENT) {
      ...
      if (verbose)
        cout << client_name << ": get encountered an unexpected error: " << err
             << std::endl;
      return err;
    }
  }
...
void*
KvFlatBtreeAsync::pset(void* ptr)
{
  struct aio_set_args* args = (struct aio_set_args*)ptr;
  *args->err = args->kvba->KvFlatBtreeAsync::set(
      (string)args->key, (bufferlist)args->val, (bool)args->exc);
...
void*
KvFlatBtreeAsync::pget(void* ptr)
{
  struct aio_get_args* args = (struct aio_get_args*)ptr;
  *args->err = args->kvba->KvFlatBtreeAsync::get(
      (string)args->key, (bufferlist*)args->val);
  args->cb(args->err, args->cb_args);
  delete args;
  return NULL;
}
...
int
KvFlatBtreeAsync::set_many(const std::map<string, bufferlist>& in_map)
{
  int err = 0;
  bufferlist inbl;
  bufferlist outbl;
  ...
  if (verbose)
    cout << "created key set and big_map" << std::endl;
  ...
  if (verbose)
    cout << "finished reading index for objects. there are " << imap.size()
         << " entries that need to be changed. " << std::endl;
  ...
  if (verbose)
    cout << "setting up to_delete and to_create vectors from index " << "map"
         << std::endl;
  ...
  for (map<string, bufferlist>::iterator it = imap.begin(); it != imap.end();
    ...
    big_map.insert(
        to_delete[to_delete.size() - 1].omap.begin(),
  ...
  for (map<string, bufferlist>::iterator it = big_map.begin();
    ...
    if (to_create[to_create.size() - 1].omap.size() == 1.5 * k) {
      to_create[to_create.size() - 1].max_kdata =
          key_data(to_create[to_create.size() - 1].omap.rbegin()->first);
  ...
  if (verbose)
    cout << "finished making to_create and to_delete. " << std::endl;
...
KvFlatBtreeAsync::remove_all()
{
  ...
  if (err < 0) {
    ...
    if (verbose)
      cout << "getting keys failed with error " << err << std::endl;
    return err;
  }
  ...
  if (err < 0) {
    if (verbose)
      cout << "rm index aioc failed with " << err << std::endl;
    return err;
  }
...
int
KvFlatBtreeAsync::get_all_keys(std::set<std::string>* keys)
{
  if (verbose)
...
int
KvFlatBtreeAsync::get_all_keys_and_values(
...
bool
KvFlatBtreeAsync::is_consistent()
{
  int err;
  bool ret = true;
  if (verbose)
    ...
    for (std::set<std::string>::iterator subit = sub_objs[it->second].begin();
      ...
      if ((it->first != "1" &&
        ...
        cerr << "not last element, i.e. " << it->first << " not equal to 1? "
             ...
             << "? " << (*subit > it->first.substr(1, it->first.length()))
             << std::endl
             << "less than or equal to " << prev << "? " << (*subit <= prev)
             << std::endl;
        ret = false;
      }
    }
  ...
  if (!ret) {
    if (verbose)
      cout << "failed consistency test - see error log" << std::endl;
    cerr << str();
  } else {
    if (verbose)
      cout << "passed consistency test" << std::endl;
  }
  return ret;
}
...
KvFlatBtreeAsync::str()
{
  ...
  for (map<std::string, bufferlist>::iterator it = index.begin();
    ...
    string s = idata.str();
    ret << "|"
        << string((148 - ((*it).first.length() + s.length() + 3)) / 2, ' ');
    ret << (*it).first;
    ret << " | ";
  ...
  for (vector<std::string>::iterator it = all_names.begin();
       it != all_names.end(); ++it) {
    librados::ObjectReadOperation oro;
    librados::AioCompletion* aioc = rados.aio_create_completion();
  ...
  for (int i = 0; i < indexer; i++) {
    ret << "|"
        << string(
               (25 - (string("Bucket: ").length() + all_names[i].length())) / 2,
               ' ');
  ...
  for (int i = 0; i < indexer; i++) {
    ...
    ret << "|"
        << string(
               (25 - (string("size: ").length() +
                      to_string("", all_sizes[i]).length())) /
                   2,
               ' ');
  ...
  for (int i = 0; i < indexer; i++) {
    ...
    ret << "|"
        << string(
               (25 - (string("version: ").length() +
                      to_string("", all_versions[i]).length())) /
                   2,
               ' ');
  ...
  for (int i = 0; i < indexer; i++) {
    ...
    ret << "unwritable? "
        << string(all_unwrit[i].c_str(), all_unwrit[i].length());
    ret << string((25 - (string("unwritable? ").length() + 1)) / 2, ' ')
        << "|\t";
  }
  ret << std::endl;
    ...
    for (int i = 0; i < indexer; i++) {
      if (dones[i]) {
        ...
        if (its[i] == all_maps[i].end()) {
          ...
          ret << "|"
              << string(
                     (25 - ((*its[i]).first.length() + its[i]->second.length() +
                            3)) /
                         2,
                     ' ');
          ret << (*its[i]).first;
          ...
          ret << string(
              (25 - ((*its[i]).first.length() + its[i]->second.length() + 3)) /
                  2,
              ' ');
          ret << "|\t";
          ++(its[i]);
