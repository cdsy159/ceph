...
struct krbd_spec {
  ...
  bool
  operator==(const krbd_spec& rhs) const
  {
    return pool_name == rhs.pool_name && nspace_name == rhs.nspace_name &&
           image_name == rhs.image_name && snap_name == rhs.snap_name;
  }
};

static std::ostream&
operator<<(std::ostream& os, const krbd_spec& spec)
{
  os << spec.pool_name << "/";
...
static std::optional<krbd_spec>
spec_from_dev(udev_device* dev)
{
...
static udev_device_uptr
dev_from_list_entry(udev* udev, udev_list_entry* l)
{
...
static int
sysfs_write_rbd(const char* which, const string& buf)
{
...
static int
sysfs_write_rbd_add(const string& buf)
{
  return sysfs_write_rbd("add", buf);
}

static int
sysfs_write_rbd_remove(const string& buf)
{
  return sysfs_write_rbd("remove", buf);
}

static int
have_minor_attr(void)
{
  /*
    ...
    string* pbuf)
{
  ...
  if (oss.tellp() == 0) {
    std::cerr << "rbd: failed to get mon address (possible ms_mode mismatch)"
              << std::endl;
    return -ENOENT;
  }
...
wait_for_mapping(int sysfs_r_fd, udev_monitor* mon, F udev_device_handler)
{
  struct pollfd fds[2];
  ...
  operator()(udev_device_uptr dev)
  {
    ...
    if (!strcmp(udev_device_get_subsystem(dev.get()), "rbd")) {
      ...
      if (boost::starts_with(
              udev_device_get_devnode(dev.get()), DEVNODE_PREFIX)) {
...
static const char*
get_event_source(const krbd_ctx* ctx)
{
...
static int
do_map(krbd_ctx* ctx, const krbd_spec& spec, const string& buf, string* pname)
{
  ...
  udev_monitor_uptr mon(
      udev_monitor_new_from_netlink(ctx->udev, get_event_source(ctx)));
  if (!mon)
    return -ENOMEM;
  ...
  std::tie(r, mapped) = wait_for_mapping(
      fds[0], mon.get(), UdevMapHandler(&spec, pname, &majnum, &minnum));
  if (r < 0) {
    if (!mapped) {
...
static int
devno_to_krbd_id(struct udev* udev, dev_t devno, string* pid)
{
...
retry:
  ...
  r = udev_enumerate_add_match_sysattr(
      enm.get(), "major", stringify(major(devno)).c_str());
  if (r < 0)
    return r;

  if (have_minor_attr()) {
    r = udev_enumerate_add_match_sysattr(
        enm.get(), "minor", stringify(minor(devno)).c_str());
    if (r < 0)
      return r;
  }
...
static std::string
escape_glob(const std::string& s)
{
  std::regex glob_meta("([*?[])");
...
retry:
  ...
  r = udev_enumerate_add_match_sysattr(
      enm.get(), "pool", escape_glob(spec.pool_name).c_str());
  if (r < 0)
    return r;

  if (match_nspace) {
    r = udev_enumerate_add_match_sysattr(
        enm.get(), "pool_ns", escape_glob(spec.nspace_name).c_str());
  } else {
    /*
  ...
  r = udev_enumerate_add_match_sysattr(
      enm.get(), "name", escape_glob(spec.image_name).c_str());
  if (r < 0)
    return r;

  r = udev_enumerate_add_match_sysattr(
      enm.get(), "current_snap", escape_glob(spec.snap_name).c_str());
  if (r < 0)
    return r;
    ...
    string* pid)
{
  ...
  maj = strict_strtoll(
      udev_device_get_sysattr_value(dev.get(), "major"), 10, &err);
  if (!err.empty()) {
  ...
  if (have_minor_attr()) {
    min = strict_strtoll(
        udev_device_get_sysattr_value(dev.get(), "minor"), 10, &err);
    if (!err.empty()) {
...
static void
append_unmap_options(std::string* buf, const char* options)
{
  if (strcmp(options, "") != 0) {
    *buf += " ";
    *buf += options;
...
static int
do_unmap(krbd_ctx* ctx, dev_t devno, const string& buf)
{
  bool unmapped;
  int fds[2];
  int r;
  ...
  std::tie(r, unmapped) =
      wait_for_mapping(fds[0], mon.get(), UdevUnmapHandler(devno));
  if (r < 0) {
    if (!unmapped) {
      std::cerr << "rbd: sysfs write failed" << std::endl;
    } else {
...
static int
unmap_image(struct krbd_ctx* ctx, const char* devnode, const char* options)
{
  struct stat sb;
  dev_t wholedevno = 0;
  std::string buf;
...
static int
unmap_image(struct krbd_ctx* ctx, const krbd_spec& spec, const char* options)
{
  dev_t devno = 0;
  std::string buf;
  int r;
...
static bool
dump_one_image(Formatter* f, TextTable* tbl, struct udev_device* dev)
{
...
static int
do_dump(struct udev* udev, Formatter* f, TextTable* tbl)
{
  udev_enumerate_uptr enm;
...
static int
dump_images(struct krbd_ctx* ctx, Formatter* f)
{
  TextTable tbl;
  int r;
...
static int
is_mapped_image(struct udev* udev, const krbd_spec& spec, string* pname)
{
  udev_enumerate_uptr enm;
  struct udev_list_entry* l;
  int r;
...
extern "C" void
krbd_destroy(struct krbd_ctx* ctx)
{
  if (!ctx)
    return;
...
extern "C" int
krbd_unmap(struct krbd_ctx* ctx, const char* devnode, const char* options)
{
...
int
krbd_showmapped(struct krbd_ctx* ctx, Formatter* f)
{
