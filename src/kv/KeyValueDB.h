...
public:
  ...
  public:
    ...
    /// Set Keys
    void
    set(const std::string& prefix, ///< [in] Prefix for keys, or CF name
        const std::map<std::string, ceph::buffer::list>&
            to_set ///< [in] keys/values to set
    )
    {
    ...
    void
    set(const std::string& prefix, ///< [in] prefix, or CF name
    ...
    merge(
        const std::string&
            prefix, ///< [in] Prefix/CF ==> MUST match some established merge operator
  ...
  virtual int create_and_open(
      std::ostream& out,
      const std::string& cfs = "") = 0;

  virtual int
  open_read_only(std::ostream& out, const std::string& cfs = "")
  {
    return -ENOTSUP;
  }
  ...
  virtual int
  submit_transaction_sync(Transaction t)
  {
  ...
  virtual int
  get(const std::string& prefix, ///< [in] prefix or CF name
...
private:
  ...
  public:
    ...
    {
      return prefix.empty() ? generic_iter->seek_to_first()
    ...
    {
      return prefix.empty() ? generic_iter->seek_to_last()
                            : generic_iter->seek_to_last(prefix);
    }

    int
    upper_bound(const std::string& after) override
    {
      return generic_iter->upper_bound(prefix, after);
    }

    int
    lower_bound(const std::string& to) override
    {
...
protected:
  Iterator
  make_iterator(const std::string& prefix, WholeSpaceIterator w_iter)
  {
...
public:
  ...
  virtual int
  get_statfs(struct store_statfs_t* buf)
  {
    return -EOPNOTSUPP;
  }

  virtual int
  set_cache_size(uint64_t)
  {
    return -EOPNOTSUPP;
  }

  virtual int
  set_cache_high_pri_pool_ratio(double ratio)
  {
    return -EOPNOTSUPP;
  }

  virtual int64_t
  get_cache_usage() const
  {
    return -EOPNOTSUPP;
  }

  virtual int64_t
  get_cache_usage(std::string prefix) const
  {
    return -EOPNOTSUPP;
  }
  ...
  virtual std::shared_ptr<PriorityCache::PriCache>
  get_priority_cache(std::string prefix) const
  {
    return nullptr;
  }
  ...
  virtual int64_t
  estimate_prefix_size(const std::string& prefix, const std::string& key_prefix)
  {
    return 0;
  }
  ...
  virtual void
  compact_prefix_async(const std::string& prefix)
  {}
  ...
  virtual void
  get_statistics(ceph::Formatter* f)
  {
    return;
  }
   ...
   */
  virtual bool
  get_property(const std::string& property, uint64_t* out)
  {
    return false;
  }
