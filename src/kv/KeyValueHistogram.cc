...
int
KeyValueHistogram::get_key_slab(size_t sz)
{
  return (sz / KEY_SLAB);
}

string
KeyValueHistogram::get_key_slab_to_range(int slab)
{
...
int
KeyValueHistogram::get_value_slab(size_t sz)
{
  return (sz / VALUE_SLAB);
}

string
KeyValueHistogram::get_value_slab_to_range(int slab)
{
...
void
KeyValueHistogram::update_hist_entry(
...
void
KeyValueHistogram::dump(Formatter* f)
{
  ...
  for (auto i : key_hist) {
    ...
    for (auto k : i.second) {
      ...
      for (auto j : k.second.val_map) {
        f->dump_unsigned(
            get_value_slab_to_range(j.first).data(), j.second.count);
