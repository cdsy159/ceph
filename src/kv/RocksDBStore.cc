...
static rocksdb::SliceParts
prepare_sliceparts(const bufferlist& bl, vector<rocksdb::Slice>* slices)
{
  unsigned n = 0;
...
public:
      ...
      rocksdb::Logger* logger) const override
  {
    ...
    for (auto& p : store.merge_ops) {
      if (p.first.compare(0, p.first.length(), key.data(), p.first.length()) ==
              0 &&
          key.data()[p.first.length()] == 0) {
        if (existing_value) {
          p.second->merge(
              existing_value->data(), existing_value->size(), value.data(),
...
public:
  explicit MergeOperatorLinker(
      const std::shared_ptr<KeyValueDB::MergeOperator>& o) :
    mop(o)
  {}
...
int
RocksDBStore::set_merge_operator(
    const string& prefix,
    std::shared_ptr<KeyValueDB::MergeOperator> mop)
{
...
public:
  ...
  void
  Logv(const char* format, va_list ap) override
  {
  ...
  // printed.
  void
  Logv(const rocksdb::InfoLogLevel log_level, const char* format, va_list ap)
      override
  {
...
static int
string2bool(const string& val, bool& b_val)
{
...
int
RocksDBStore::ParseOptionsFromString(const string& opt_str, rocksdb::Options& opt)
{
  return ParseOptionsFromStringStatic(
      cct, opt_str, opt,
    ...
    function<int(const string&, const string&, rocksdb::Options&)> interp)
{
  ...
  const set<string> need_interp_keys = {
...
int
RocksDBStore::init(string _options_str)
{
  ...
  if (env) {
    ...
    if (!fs::exists(path)) {
      ...
      if (!fs::create_directory(path, ec)) {
        derr << __func__ << " failed to create " << path << ": " << ec.message()
             << dendl;
        return -ec.value();
      }
      fs::permissions(
          path, fs::perms::owner_all | fs::perms::group_read |
...
int
RocksDBStore::install_cf_mergeop(
...
int
RocksDBStore::create_and_open(ostream& out, const std::string& cfs)
{
  int r = create_db_dir();
  if (r < 0)
    return r;
    ...
    double cache_prio_high)
{
  ...
  if (cache_type == "binned_lru") {
    cache = rocksdb_cache::NewBinnedLRUCache(
        cct, name, cache_size, shard_bits, false, cache_prio_high);
...
int
RocksDBStore::load_rocksdb_options(bool create_if_missing, rocksdb::Options& opt)
{
  rocksdb::Status status;
  ...
  bbt_opts.block_cache = create_block_cache(
      rocksdb::kDefaultColumnFamilyName, cct->_conf->rocksdb_cache_type,
      block_cache_size);
  ...
  if (row_cache_size > 0)
    opt.row_cache = rocksdb::NewLRUCache(
        row_cache_size, cct->_conf->rocksdb_cache_shard_bits);
  uint64_t bloom_bits =
      cct->_conf.get_val<uint64_t>("rocksdb_bloom_bits_per_key");
  if (bloom_bits > 0) {
    dout(10) << __func__ << " set bloom filter bits per key to " << bloom_bits
             << dendl;
  ...
  if (!bbt_opts.no_block_cache) {
    ...
    bbt_opts.pin_l0_filter_and_index_blocks_in_cache = cct->_conf.get_val<bool>(
        "rocksdb_pin_l0_filter_and_index_blocks_in_cache");
  }
  bbt_opts.partition_filters =
      cct->_conf.get_val<bool>("rocksdb_partition_filters");
  if (cct->_conf.get_val<Option::size_t>("rocksdb_metadata_block_size") > 0)
    bbt_opts.metadata_block_size =
        cct->_conf.get_val<Option::size_t>("rocksdb_metadata_block_size");

  // Set Compact on Deletion Factory
  if (cct->_conf->rocksdb_cf_compact_on_deletion) {
    size_t sliding_window =
        cct->_conf->rocksdb_cf_compact_on_deletion_sliding_window;
  ...
  dout(10) << __func__ << " block size " << cct->_conf->rocksdb_block_size
           ...
           << ", row_cache size " << byte_u_t(row_cache_size) << "; shards "
           << (1 << cct->_conf->rocksdb_cache_shard_bits) << ", type "
           << cct->_conf->rocksdb_cache_type << dendl;
...
void
RocksDBStore::add_column_family(
...
{
  dout(10) << __func__ << " column_name=" << cf_name
           << " shard_idx=" << shard_idx << " hash_l=" << hash_l
...
bool
RocksDBStore::is_column_family(const std::string& prefix)
{
  return cf_handles.count(prefix);
}

std::string_view
RocksDBStore::get_key_hash_view(
    const prefix_shards& shards,
    const char* key,
    const size_t keylen)
{
...
rocksdb::ColumnFamilyHandle*
RocksDBStore::get_cf_handle(const std::string& prefix, const std::string& key)
{
 ...
 */
rocksdb::ColumnFamilyHandle*
RocksDBStore::check_cf_handle_bounds(
    const cf_handles_iterator& iter,
    const IteratorBounds& bounds)
{
  ...
  auto lower_bound_hash_str = get_key_hash_view(
      iter->second, bounds.lower_bound->data(), bounds.lower_bound->size());
  auto upper_bound_hash_str = get_key_hash_view(
      iter->second, bounds.upper_bound->data(), bounds.upper_bound->size());
...
RocksDBStore::parse_sharding_def(
    const std::string_view text_def_in,
    ...
    std::string* error_msg)
{
  ...
  *error_position = nullptr;
  ...
  while (!text_def.empty()) {
    ...
    sharding_def.emplace_back(
        std::string(name), shard_cnt, std::string(options), l_bound, h_bound);
  }
  return *error_position == nullptr;
}

void
RocksDBStore::sharding_def_to_columns(
    ...
    const std::vector<ColumnFamily>& sharding_def)
{
  for (auto& p : sharding_def) {
    ...
    for (size_t idx = 0; idx < p.shard_cnt; idx++) {
      ...
      if (!status.ok()) {
        derr << __func__
             << " Failed to create rocksdb column family: " << cf_name << dendl;
        return -EINVAL;
      }
    ...
    const std::string& sharding_text)
{
  ...
  if (!sharding_text.empty()) {
    ...
    char const* error_position;
    std::string error_msg;
    b = parse_sharding_def(
        sharding_text, sharding_def, &error_position, &error_msg);
    if (!b) {
      dout(1) << __func__ << " bad sharding: " << dendl;
      dout(1) << __func__ << sharding_text << dendl;
      dout(1) << __func__
              << std::string(error_position - &sharding_text[0], ' ') << "^"
              << error_msg << dendl;
      return -EINVAL;
    }
    ...
    status = rocksdb::WriteStringToFile(
        opt.env, sharding_text, sharding_def_file, true);
    if (!status.ok()) {
...
extern rocksdb::Status rocksdb::StringToMap(
    const std::string& opts_str,
    std::unordered_map<std::string, std::string>* opts_map);
...
int
RocksDBStore::split_column_family_options(
...
int
RocksDBStore::update_column_family_options(
    ...
    rocksdb::ColumnFamilyOptions* cf_opt)
{
  ...
  rocksdb::Status status;
  int r =
      split_column_family_options(more_options, &options_map, &block_cache_opt);
  if (r != 0) {
    dout(5) << __func__
  ...
  if (!status.ok()) {
    dout(5) << __func__
            << " invalid column family optionsp; column family=" << base_name
            << " options=" << more_options << dendl;
    dout(5) << __func__ << " RocksDB error='" << status.getState() << "'"
            << dendl;
    return -EINVAL;
  }
...
int
RocksDBStore::apply_block_cache_options(
    ...
    rocksdb::ColumnFamilyOptions* cf_opt)
{
  ...
  if (const auto it = cache_options_map.find("type");
      it != cache_options_map.end()) {
  ...
  if (auto it = cache_options_map.find("high_ratio");
      it != cache_options_map.end()) {
    std::string error;
    high_pri_pool_ratio = strict_strtod(it->second.c_str(), &error);
    if (!error.empty()) {
      dout(10) << __func__ << " invalid high_pri (float): '" << it->second
               << "'" << dendl;
      return -EINVAL;
    }
    cache_options_map.erase(it);
  ...
  status = GetBlockBasedTableOptionsFromMap(
      bbt_opts, cache_options_map, &column_bbt_opts);
  if (!status.ok()) {
    dout(5) << __func__
            << " invalid block cache options; column=" << column_name
            << " options=" << block_cache_opt << dendl;
  ...
  if (column_bbt_opts.no_block_cache) {
    ...
    if (require_new_block_cache) {
      block_cache = create_block_cache(
          column_name, cache_type, cache_size, high_pri_pool_ratio);
      if (!block_cache) {
        dout(5) << __func__ << " failed to create block cache for params: "
                << block_cache_opt << dendl;
        return -EINVAL;
      }
    } else {
  ...
  cf_opt->table_factory.reset(
      NewBlockBasedTableFactory(cf_bbt_opts[column_name]));
  return 0;
}

int
RocksDBStore::verify_sharding(
    const rocksdb::Options& opt,
    std::vector<rocksdb::ColumnFamilyDescriptor>& existing_cfs,
    std::vector<std::pair<size_t, RocksDBStore::ColumnFamily>>&
        existing_cfs_shard,
    ...
    std::vector<std::pair<size_t, RocksDBStore::ColumnFamily>>& missing_cfs_shard)
{
  ...
  if (status.ok()) {
    status = rocksdb::ReadFileToString(
        opt.env, sharding_def_file, &stored_sharding_text);
    if (!status.ok()) {
  ...
  std::sort(
      stored_sharding_def.begin(), stored_sharding_def.end(),
      [](ColumnFamily& a, ColumnFamily& b) { return a.name < b.name; });

  std::vector<string> rocksdb_cfs;
  status = rocksdb::DB::ListColumnFamilies(
      rocksdb::DBOptions(opt), path, &rocksdb_cfs);
  if (!status.ok()) {
    derr << __func__ << " unable to list column families: " << status.ToString()
         << dendl;
    return -EIO;
  }
  dout(5) << __func__ << " column families from rocksdb: " << rocksdb_cfs
          << dendl;

  auto emplace_cf = [&](const RocksDBStore::ColumnFamily& column,
                        int32_t shard_id, const std::string& shard_name,
                        const rocksdb::ColumnFamilyOptions& opt) {
    if (std::find(rocksdb_cfs.begin(), rocksdb_cfs.end(), shard_name) !=
        rocksdb_cfs.end()) {
...
std::ostream&
operator<<(std::ostream& out, const RocksDBStore::ColumnFamily& cf)
{
  out << "(";
  out << cf.name;
  out << ",";
    ...
    const std::string& sharding_text)
{
  ...
  if (create_if_missing) {
    ...
    r = verify_sharding(
        opt, existing_cfs, existing_cfs_shard, missing_cfs, missing_cfs_shard);
    if (r < 0) {
      return r;
    }
    std::string sharding_recreate_text;
    status = rocksdb::ReadFileToString(
        opt.env, sharding_recreate, &sharding_recreate_text);
    ...
    if (recreate_mode == false && missing_cfs.size() != 0) {
      ...
      if (open_readonly == false &&
          std::find_if(
              missing_cfs.begin(), missing_cfs.end(),
              [](const rocksdb::ColumnFamilyDescriptor& c) {
                return c.name == resharding_column_lock;
              }) != missing_cfs.end()) {
        derr << __func__ << " missing column families: " << missing_cfs_shard
             << dendl;
        return -EIO;
      }
    }
    ...
    if (existing_cfs.empty()) {
      ...
      if (open_readonly) {
        status = rocksdb::DB::OpenForReadOnly(
            rocksdb::DBOptions(opt), path, existing_cfs, &handles, &db);
      } else {
      ...
      for (size_t i = 0; i < existing_cfs_shard.size(); i++) {
        add_column_family(
            existing_cfs_shard[i].second.name,
            existing_cfs_shard[i].second.hash_l,
            existing_cfs_shard[i].second.hash_h, existing_cfs_shard[i].first,
            handles[i]);
      }
      ...
      if (missing_cfs.size() > 0 &&
        ...
        for (size_t i = 0; i < missing_cfs.size(); i++) {
          ...
          status = db->CreateColumnFamily(
              missing_cfs[i].options, missing_cfs[i].name, &cf);
          if (!status.ok()) {
          ...
          add_column_family(
              missing_cfs_shard[i].second.name,
              missing_cfs_shard[i].second.hash_l,
              missing_cfs_shard[i].second.hash_h, missing_cfs_shard[i].first,
              cf);
        }
  ...
  plb.add_time_avg(
      l_rocksdb_get_latency, "get_latency", "Get latency", nullptr,
      PerfCountersBuilder::PRIO_USEFUL);
  plb.add_time_avg(l_rocksdb_submit_latency, "submit_latency", "Submit Latency");
  plb.add_time_avg(
      l_rocksdb_submit_sync_latency, "submit_sync_latency",
      "Submit Sync Latency");
  plb.add_u64_counter(l_rocksdb_compact, "compact", "Compactions");
  plb.add_u64_counter(
      l_rocksdb_compact_running, "compact_running", "Running compactions",
      nullptr, PerfCountersBuilder::PRIO_USEFUL);
  plb.add_u64_counter(
      l_rocksdb_compact_completed, "compact_completed", "Completed compactions");
  plb.add_time(
      l_rocksdb_compact_lasted, "compact_lasted",
      "Last completed compaction duration");
  plb.add_u64_counter(
      l_rocksdb_compact_queue_merge, "compact_queue_merge",
      "Mergings of ranges in compaction queue");
  plb.add_u64(
      l_rocksdb_compact_queue_len, "compact_queue_len",
      "Length of compaction queue");
  plb.add_time_avg(
      l_rocksdb_write_wal_time, "rocksdb_write_wal_time",
      "Rocksdb write wal time");
  plb.add_time_avg(
      l_rocksdb_write_memtable_time, "rocksdb_write_memtable_time",
      "Rocksdb write memtable time");
  plb.add_time_avg(
      l_rocksdb_write_delay_time, "rocksdb_write_delay_time",
      "Rocksdb write delay time");
  plb.add_time_avg(
...
int
RocksDBStore::_test_init(const string& dir)
{
...
int
RocksDBStore::repair(std::ostream& out)
{
  rocksdb::Status status;
  rocksdb::Options opt;
  ...
  if (!stored_sharding_text.empty()) {
    ...
    opt.env->CreateDir(sharding_def_dir);
    status = rocksdb::WriteStringToFile(
        opt.env, stored_sharding_text, sharding_def_file, true);
    if (!status.ok()) {
      derr << __func__ << " cannot write to " << sharding_def_file << dendl;
      return -1;
    }
...
bool
RocksDBStore::get_property(const std::string& property, uint64_t* out)
{
  return db->GetIntProperty(property, out);
}

int64_t
RocksDBStore::estimate_prefix_size(const string& prefix, const string& key_prefix)
{
...
void
RocksDBStore::get_statistics(Formatter* f)
{
  if (!cct->_conf->rocksdb_perf) {
    f->write_raw_data(
        "error: RocksDB perf is disabled, can't probe for stats.\n");
    return;
  }
  if (!cct->_conf->rocksdb_collect_compaction_stats &&
    ...
    f->write_raw_data(
        "error: None of rocksdb_collect_* setting is enabled, hence no "
        "output.\n");
    return;
  }
  ...
  if (cct->_conf->rocksdb_collect_compaction_stats) {
    ...
    for (auto handle : handles) {
      ...
      if (!status) {
        derr << __func__
             << " failed to get rocksdb.stats for the cf: " << handle->GetName()
             << dendl;
        continue;
      }
...
struct RocksDBStore::RocksWBHandler : public rocksdb::WriteBatch::Handler {
  ...
  void
  Put(const rocksdb::Slice& key, const rocksdb::Slice& value) override
  {
  ...
  void
  SingleDelete(const rocksdb::Slice& key) override
  {
    dump("SingleDelete", 0, key);
  }

  rocksdb::Status
  SingleDeleteCF(uint32_t column_family_id, const rocksdb::Slice& key) override
  {
  ...
  void
  Delete(const rocksdb::Slice& key) override
  {
    dump("Delete", 0, key);
  }

  rocksdb::Status
  DeleteCF(uint32_t column_family_id, const rocksdb::Slice& key) override
  {
  ...
  void
  Merge(const rocksdb::Slice& key, const rocksdb::Slice& value) override
  {
  ...
  *_dout << " Rocksdb transaction: " << bat_txc.seen.str() << dendl;
  ...
  if (cct->_conf->rocksdb_perf) {
    ...
    write_wal_time.set_from_double(
        static_cast<double>(rocksdb::get_perf_context()->write_wal_time) /
        1000000000);
    write_memtable_time.set_from_double(
        static_cast<double>(rocksdb::get_perf_context()->write_memtable_time) /
        1000000000);
    write_delay_time.set_from_double(
        static_cast<double>(rocksdb::get_perf_context()->write_delay_time) /
        1000000000);
...
int
RocksDBStore::submit_transaction_sync(KeyValueDB::Transaction t)
{
  utime_t start = ceph_clock_now();
  rocksdb::WriteOptions woptions;
...
void
RocksDBStore::RocksDBTransactionImpl::put_bat(
    ...
    const bufferlist& to_set_bl)
{
  ...
  if (to_set_bl.is_contiguous() && to_set_bl.length() > 0) {
    ...
    bat.Put(
        cf, rocksdb::SliceParts(&key_slice, 1),
...
void
RocksDBStore::RocksDBTransactionImpl::rmkey(const string& prefix, const string& k)
{
  auto cf = db->get_cf_handle(prefix, k);
  if (cf) {
...
void
RocksDBStore::RocksDBTransactionImpl::rm_single_key(
    const string& prefix,
    const string& k)
{
  auto cf = db->get_cf_handle(prefix, k);
...
void
RocksDBStore::RocksDBTransactionImpl::rmkeys_by_prefix(const string& prefix)
{
  ...
  if (p_iter == db->cf_handles.end()) {
    ...
    if (cnt == 0) {
      ...
      bat.DeleteRange(
          db->default_cf, combine_strings(prefix, string()),
...
void
RocksDBStore::RocksDBTransactionImpl::rm_range_keys(
    const string& prefix,
    const string& start,
    const string& end)
{
  ldout(db->cct, 10) << __func__ << " enter prefix=" << prefix
  ...
  if (p_iter == db->cf_handles.end()) {
    ...
    for (it->lower_bound(start);
         it->valid() && db->comparator->Compare(it->key(), end) < 0 &&
         (--cnt) != 0;
         it->next()) {
      bat.Delete(db->default_cf, combine_strings(prefix, it->key()));
    }
    ...
    if (cnt == 0) {
      ...
      bat.RollbackToSavePoint();
      bat.DeleteRange(
          db->default_cf, rocksdb::Slice(combine_strings(prefix, start)),
...
void
RocksDBStore::RocksDBTransactionImpl::merge(
    const string& prefix,
    const string& k,
    const bufferlist& to_set_bl)
{
  ...
  if (cf) {
    ...
    if (to_set_bl.is_contiguous() && to_set_bl.length() > 0) {
      bat.Merge(
          cf, rocksdb::Slice(k),
          rocksdb::Slice(
              to_set_bl.buffers().front().c_str(), to_set_bl.length()));
    } else {
      // make a copy
    ...
    if (to_set_bl.is_contiguous() && to_set_bl.length() > 0) {
      bat.Merge(
          db->default_cf, rocksdb::Slice(key),
          rocksdb::Slice(
              to_set_bl.buffers().front().c_str(), to_set_bl.length()));
    } else {
      // make a copy
      ...
      vector<rocksdb::Slice> value_slices(to_set_bl.get_num_buffers());
      bat.Merge(
          db->default_cf, rocksdb::SliceParts(&key_slice, 1),
          prepare_sliceparts(to_set_bl, &value_slices));
    }
  }
}
    ...
    std::map<string, bufferlist>* out)
{
  ...
  if (cf_handles.count(prefix) > 0) {
    for (auto& key : keys) {
      ...
      auto status = db->Get(
          rocksdb::ReadOptions(), cf_handle, rocksdb::Slice(key), &value);
      if (status.ok()) {
    ...
    for (auto& key : keys) {
      ...
      auto status = db->Get(
          rocksdb::ReadOptions(), default_cf, rocksdb::Slice(k), &value);
      if (status.ok()) {
        (*out)[key].append(value.data(), value.size());
      } else if (status.IsIOError()) {
        ceph_abort_msg(status.getState());
...
int
RocksDBStore::get(const string& prefix, const string& key, bufferlist* out)
{
...
int
RocksDBStore::split_key(rocksdb::Slice in, string* prefix, string* key)
{
  size_t prefix_len = 0;
...
int
RocksDBStore::split_key(rocksdb::Slice in, string_view* prefix, string_view* key)
{
  size_t prefix_len = 0;
...
void
RocksDBStore::compact_thread_entry()
{
...
void
RocksDBStore::compact_range_async(const string& start, const string& end)
{
  ...
  while (p != compact_queue.end()) {
    ...
    if (start <= p->first && p->first <= end) {
      ...
      compact_queue.push_back(
          make_pair(start, end > p->second ? end : p->second));
...
bool
RocksDBStore::check_omap_dir(string& omap_dir)
{
  rocksdb::Options options;
  options.create_if_missing = true;
  rocksdb::DB* db;
...
void
RocksDBStore::compact_range(const string& start, const string& end)
{
  rocksdb::CompactRangeOptions options;
...
int
RocksDBStore::RocksDBWholeSpaceIteratorImpl::seek_to_first()
{
  dbiter->SeekToFirst();
...
int
RocksDBStore::RocksDBWholeSpaceIteratorImpl::seek_to_first(const string& prefix)
{
...
int
RocksDBStore::RocksDBWholeSpaceIteratorImpl::seek_to_last()
{
  dbiter->SeekToLast();
  ceph_assert(!dbiter->status().IsIOError());
  return dbiter->status().ok() ? 0 : -1;
...
int
RocksDBStore::RocksDBWholeSpaceIteratorImpl::seek_to_last(const string& prefix)
{
...
int
RocksDBStore::RocksDBWholeSpaceIteratorImpl::upper_bound(
    const string& prefix,
    const string& after)
{
...
int
RocksDBStore::RocksDBWholeSpaceIteratorImpl::lower_bound(
    const string& prefix,
    const string& to)
{
...
bool
RocksDBStore::RocksDBWholeSpaceIteratorImpl::valid()
{
  return dbiter->Valid();
}
...
int
RocksDBStore::RocksDBWholeSpaceIteratorImpl::prev()
{
  if (valid()) {
    dbiter->Prev();
  }
...
string_view
RocksDBStore::RocksDBWholeSpaceIteratorImpl::key_as_sv()
{
  string_view out_key;
  split_key(dbiter->key(), nullptr, &out_key);
  return out_key;
...
RocksDBStore::RocksDBWholeSpaceIteratorImpl::raw_key_as_sv()
{
...
bool
RocksDBStore::RocksDBWholeSpaceIteratorImpl::raw_key_is_prefixed(
    const string& prefix)
{
...
size_t
RocksDBStore::RocksDBWholeSpaceIteratorImpl::key_size()
{
  return dbiter->key().size();
}

size_t
RocksDBStore::RocksDBWholeSpaceIteratorImpl::value_size()
{
...
std::string_view
RocksDBStore::RocksDBWholeSpaceIteratorImpl::value_as_sv()
{
  rocksdb::Slice val = dbiter->value();
...
int
RocksDBStore::RocksDBWholeSpaceIteratorImpl::status()
{
  return dbiter->status().ok() ? 0 : -1;
}

string
RocksDBStore::past_prefix(const string& prefix)
{
  string limit = prefix;
  limit.push_back(1);
  return limit;
...
public:
  ...
  int
  upper_bound(const string& after) override
  {
    lower_bound(after);
    if (valid() && (key() == after)) {
      next();
  ...
  int
  lower_bound(const string& to) override
  {
...
public:
  WholeMergeIteratorImpl(RocksDBStore* db) :
    db(db), main(db->get_default_cf_iterator())
  {
  ...
  int
  seek_to_first(const std::string& prefix) override
  {
  ...
  int
  seek_to_last(const std::string& prefix) override
  {
  ...
  int
  upper_bound(const std::string& prefix, const std::string& after) override
  {
  ...
  int
  lower_bound(const std::string& prefix, const std::string& to) override
  {
  ...
  bool
  raw_key_is_prefixed(const std::string& prefix) override
  {
    if (smaller == on_main) {
...
private:
  ...
  public:
    ...
    bool
    operator()(rocksdb::Iterator* a, rocksdb::Iterator* b) const
    {
      if (a->Valid()) {
        if (b->Valid()) {
...
public:
  explicit ShardMergeIteratorImpl(
      const RocksDBStore* db,
      const std::string& prefix,
  ...
  if (cf_it != cf_handles.end()) {
    ...
    if (cf) {
      return std::make_shared<CFIteratorImpl>(
          this, prefix, cf, std::move(bounds));
    } else {
      return std::make_shared<ShardMergeIteratorImpl>(
          this, prefix, cf_it->second.handles, std::move(bounds));
    }
  } else {
...
RocksDBStore::WholeSpaceIterator
RocksDBStore::new_shard_iterator(rocksdb::ColumnFamilyHandle* cf)
{
  return std::make_shared<RocksDBWholeSpaceIteratorImpl>(this, cf, 0);
}

KeyValueDB::Iterator
RocksDBStore::new_shard_iterator(
    rocksdb::ColumnFamilyHandle* cf,
    const std::string& prefix,
    IteratorBounds bounds)
{
...
int
RocksDBStore::prepare_for_reshard(
    const std::string& new_sharding,
    RocksDBStore::columns_t& to_process_columns)
{
  ...
  std::string error_msg;
  b = parse_sharding_def(
      new_sharding, new_sharding_def, &error_position, &error_msg);
  if (!b) {
    dout(1) << __func__ << " bad sharding: " << dendl;
    dout(1) << __func__ << new_sharding << dendl;
    dout(1) << __func__ << std::string(error_position - &new_sharding[0], ' ')
            << "^" << error_msg << dendl;
    return -EINVAL;
  }
  ...
  if (stored_sharding_text.find(resharding_column_lock) == string::npos) {
    ...
    status = rocksdb::WriteStringToFile(
        env, stored_sharding_text, sharding_def_file, true);
    if (!status.ok()) {
      derr << __func__ << " cannot write to " << sharding_def_file << dendl;
      return -EIO;
    }
  ...
  status = rocksdb::DB::ListColumnFamilies(
      rocksdb::DBOptions(opt), path, &existing_columns);
  if (!status.ok()) {
  ...
  std::vector<rocksdb::ColumnFamilyHandle*> handles;
  status = rocksdb::DB::Open(
      rocksdb::DBOptions(opt), path, cfs_to_open, &handles, &db);
  if (!status.ok()) {
    derr << status.ToString() << dendl;
    return -EINVAL;
  }
  for (size_t i = 0; i < cfs_to_open.size(); i++) {
    dout(10) << "column " << cfs_to_open[i].name << " handle "
             << (void*)handles[i] << dendl;
  }
  ...
  for (const auto& full_name : missing_columns) {
    ...
    if (!status.ok()) {
      derr << __func__
           << " Failed to create rocksdb column family: " << full_name << dendl;
      return -EINVAL;
    }
    dout(10) << "created column " << full_name << " handle = " << (void*)cf
             << dendl;
  ...
  for (const auto& col : cf_handles) {
    for (size_t i = 0; i < col.second.handles.size(); i++) {
      if (col.second.handles[i] == nullptr) {
        derr << "missing handle for column " << col.first << " shard " << i
             << dendl;
        return -EIO;
      }
    }
...
int
RocksDBStore::reshard_cleanup(const RocksDBStore::columns_t& current_columns)
{
  std::vector<std::string> new_sharding_columns;
  ...
  for (auto& [name, handle] : current_columns) {
    auto found = std::find(
                     new_sharding_columns.begin(), new_sharding_columns.end(),
    ...
    dout(5) << "Column " << name << " not part of new sharding. Deleting."
            << dendl;
    ...
    if (rocksdb::Status status = db->DropColumnFamily(handle.get());
        !status.ok()) {
...
int
RocksDBStore::reshard(
    const std::string& new_sharding,
    const RocksDBStore::resharding_ctrl* ctrl_in)
{
  ...
  auto process_column = [&](rocksdb::ColumnFamilyHandle* handle,
    ...
    dout(5) << " column=" << (void*)handle << " prefix=" << fixed_prefix
            << dendl;
    std::unique_ptr<rocksdb::Iterator> it{
    ...
    for (it->SeekToFirst(); it->Valid(); it->Next()) {
      ...
      if ((keys_processed % 10000) == 0) {
        dout(10) << "processed " << keys_processed << " keys, moved "
                 << keys_moved << dendl;
      }
      ...
      dout(25) << "moving " << (void*)handle << "/"
               << pretty_binary_string(raw_key.ToString()) << " to "
               << (void*)new_handle << "/" << pretty_binary_string(new_raw_key)
  ...
  for (auto& [name, handle] : to_process_columns) {
    dout(5) << "Processing column=" << name << " handle=" << handle.get()
            << dendl;
  ...
  env->CreateDir(sharding_def_dir);
  if (auto status = rocksdb::WriteStringToFile(
          env, new_sharding, sharding_def_file, true);
      !status.ok()) {
    derr << __func__ << " cannot write to " << sharding_def_file << dendl;
    return -EIO;
  }
...
bool
RocksDBStore::get_sharding(std::string& sharding)
{
  rocksdb::Status status;
  std::string stored_sharding_text;
  bool result = false;
  ...
  if (status.ok()) {
    status = rocksdb::ReadFileToString(
        env, sharding_def_file, &stored_sharding_text);
    if (status.ok()) {
      result = true;
