...
make_slice(const std::optional<std::string>& bound)
{
  if (bound) {
    return {*bound};
  } else {
...
private:
  ...
  std::string_view get_key_hash_view(
      const prefix_shards& shards,
      const char* key,
      const size_t keylen);
  rocksdb::ColumnFamilyHandle* get_key_cf(
      const prefix_shards& shards,
      const char* key,
      const size_t keylen);
  rocksdb::ColumnFamilyHandle* get_cf_handle(
...
public:
  static bool parse_sharding_def(
      const std::string_view text_def,
...
private:
  static void sharding_def_to_columns(
...
public:
  ...
  uint64_t
  get_delete_range_threshold() const
  {
  ...
  static int ParseOptionsFromStringStatic(
      ...
      rocksdb::Options& opt,
      std::function<int(const std::string&, const std::string&, rocksdb::Options&)>
          interp);
  ...
  void
  compact_prefix(const std::string& prefix) override
  {
    compact_range(prefix, past_prefix(prefix));
  }

  void
  compact_prefix_async(const std::string& prefix) override
  {
  ...
  {
    compact_range_async(
        combine_strings(prefix, start), combine_strings(prefix, end));
  }
  ...
  /// Opens underlying db
  int
  open(std::ostream& out, const std::string& cfs = "") override
  {
  ...
  int
  open_read_only(std::ostream& out, const std::string& cfs = "") override
  {
  ...
  public:
    size_t
    get_count() const override
    {
      return bat.Count();
    }

    size_t
    get_size_bytes() const override
    {
  ...
  public:
    explicit RocksDBWholeSpaceIteratorImpl(
...
public:
  ...
  virtual int64_t
  get_cache_usage() const override
  {
    return static_cast<int64_t>(bbt_opts.block_cache->GetUsage());
  }

  virtual int64_t
  get_cache_usage(std::string prefix) const override
  {
  ...
  int
  set_cache_size(uint64_t s) override
  {
    cache_size = s;
    set_cache_flag = true;
    return 0;
...
private:
  ...
  using columns_t = std::map<
      std::string,
      std::unique_ptr<rocksdb::ColumnFamilyHandle, cf_deleter_t>>;
  int prepare_for_reshard(
...
public:
  ...
  int reshard(
      const std::string& new_sharding,
      const resharding_ctrl* ctrl = nullptr);
