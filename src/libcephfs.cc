...
std::shared_ptr<ceph::async::io_context_pool>
get_icp(CephContext* cct)
{
  static std::mutex m;
...
public:
  ...
  int
  select_filesystem(const std::string& fs_name_)
  {
    if (mounted) {
      return -EISCONN;
    }
  ...
  int
  mount(const std::string& mount_root, const UserPerm& perms)
  {
    int ret;
  ...
  int
  conf_read_file(const char* path_list)
  {
  ...
  int
  conf_parse_argv(int argc, const char** argv)
  {
  ...
  int
  conf_parse_env(const char* name)
  {
    auto& conf = cct->_conf;
  ...
  int
  chdir(const char* to, const UserPerm& perms)
  {
...
extern "C" void
ceph_userperm_destroy(UserPerm* perm)
{
  delete perm;
}

extern "C" const char*
ceph_version(int* pmajor, int* pminor, int* ppatch)
{
...
extern "C" int
ceph_create_with_context(struct ceph_mount_info** cmount, CephContext* cct)
{
...
extern "C" int
ceph_create_from_rados(struct ceph_mount_info** cmount, rados_t cluster)
{
...
extern "C" int
ceph_abort_conn(struct ceph_mount_info* cmount)
{
...
extern "C" void
ceph_shutdown(struct ceph_mount_info* cmount)
{
  cmount->shutdown();
  delete cmount;
  cmount = nullptr;
...
extern "C" uint64_t
ceph_get_instance_id(struct ceph_mount_info* cmount)
{
...
extern "C" int
ceph_getaddrs(struct ceph_mount_info* cmount, char** addrs)
{
  if (!cmount->is_initialized())
    return -ENOTCONN;
...
extern "C" int
ceph_conf_read_file(struct ceph_mount_info* cmount, const char* path)
{
  return cmount->conf_read_file(path);
}

extern "C" mode_t
ceph_umask(struct ceph_mount_info* cmount, mode_t mode)
{
  return cmount->set_umask(mode);
}

extern "C" int
ceph_conf_parse_argv(struct ceph_mount_info* cmount, int argc, const char** argv)
{
  return cmount->conf_parse_argv(argc, argv);
}

extern "C" int
ceph_conf_parse_env(struct ceph_mount_info* cmount, const char* name)
{
...
extern "C" int
ceph_set_mount_timeout(struct ceph_mount_info* cmount, uint32_t timeout)
{
  if (cmount->is_mounted()) {
    return -EINVAL;
  }
...
public:
  CommandCContext(libcephfs_c_completion_t c, void* ud) :
    c(c), ud(ud)
  {}
    ...
    void* ud)
{
  ...
  int r = cmount->get_client()->mds_command(
      mds_spec, cmdv, inbl, &ctx->outbl, &ctx->outs, ctx, one_shot);
    ...
    size_t* outsbuflen)
{
  ...
  C_SaferCond cond;
  int r = cmount->get_client()->mds_command(
      mds_spec, cmdv, inbl, &outbl, &outs, &cond);
...
extern "C" int
ceph_is_mounted(struct ceph_mount_info* cmount)
{
  return cmount->is_mounted() ? 1 : 0;
}

extern "C" struct UserPerm*
ceph_mount_perms(struct ceph_mount_info* cmount)
{
  return &cmount->default_perms;
}

extern "C" int64_t
ceph_get_fs_cid(struct ceph_mount_info* cmount)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
...
extern "C" int
ceph_mount_perms_set(struct ceph_mount_info* cmount, struct UserPerm* perms)
{
  if (cmount->is_mounted())
    return -EISCONN;
...
extern "C" int
ceph_get_local_osd(struct ceph_mount_info* cmount)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
...
extern "C" int
ceph_chdir(struct ceph_mount_info* cmount, const char* s)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
    ...
    struct ceph_dir_result** dirpp)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
  return cmount->get_client()->opendir(
      name, (dir_result_t**)dirpp, cmount->default_perms);
}
    ...
    struct ceph_dir_result** dirpp)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
  return cmount->get_client()->fdopendir(
      dirfd, (dir_result_t**)dirpp, cmount->default_perms);
}

extern "C" int
ceph_closedir(struct ceph_mount_info* cmount, struct ceph_dir_result* dirp)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
    ...
    struct dirent* de)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
  return cmount->get_client()->readdir_r(
      reinterpret_cast<dir_result_t*>(dirp), de);
}
    ...
    struct Inode** out)
{
  ...
  if (flags & ~CEPH_REQ_FLAG_MASK)
    return -EINVAL;
  return cmount->get_client()->readdirplus_r(
      reinterpret_cast<dir_result_t*>(dirp), de, stx, want, flags, out);
}

extern "C" int
ceph_file_blockdiff_init(
    struct ceph_mount_info* cmount,
    ...
    struct ceph_file_blockdiff_info* out_info)
{
  ...
  if (!out_info || !root_path || !rel_path || !snap1 || !*snap1 || !snap2 ||
      !*snap2) {
    return -EINVAL;
  }
  ...
  int n = snprintf(
      path1, PATH_MAX, "%s/%s/%s/%s", root_path, snapdir, snap1, rel_path);
  if (n < 0 || n == PATH_MAX) {
    errno = ENAMETOOLONG;
    return -errno;
  }
  n = snprintf(
      path2, PATH_MAX, "%s/%s/%s/%s", root_path, snapdir, snap2, rel_path);
  if (n < 0 || n == PATH_MAX) {
    return -ENAMETOOLONG;
  }

  int r = cmount->get_client()->file_blockdiff_init_state(
      path1, path2, cmount->default_perms,
    ...
    struct ceph_file_blockdiff_changedblocks* blocks)
{
  ...
  int r = info->cmount->get_client()->file_blockdiff(
      state, info->cmount->default_perms, &_blocks);
  if (r < 0) {
    return r;
  }
  ...
  if (blocks->num_blocks) {
    struct cblock* b =
        (struct cblock*)calloc(blocks->num_blocks, sizeof(struct cblock));
    if (!b) {
      return -ENOMEM;
    }
...
extern "C" void
ceph_free_file_blockdiff_buffer(struct ceph_file_blockdiff_changedblocks* blocks)
{
  if (blocks->b) {
    free(blocks->b);
  }
...
extern "C" int
ceph_file_blockdiff_finish(struct ceph_file_blockdiff_info* info)
{
  if (!info->cmount->is_mounted()) {
    return -ENOTCONN;
  }
    ...
    struct ceph_snapdiff_info* out)
{
  ...
  if (!out || !root_path || !rel_path || !snap1 || !*snap1 || !snap2 ||
      !*snap2) {
    errno = EINVAL;
    return -errno;
  }
  ...
  cmount->conf_get("client_snapdir", snapdir, sizeof(snapdir) - 1);
  int n = snprintf(
      full_path1, PATH_MAX, "%s/%s/%s/%s", root_path, snapdir, snap1, rel_path);
  if (n < 0 || n == PATH_MAX) {
    errno = ENAMETOOLONG;
    return -errno;
  }
  ...
  n = snprintf(
      full_path2, PATH_MAX, "%s/%s/%s/%s", root_path, snapdir, snap2, rel_path);
  if (n < 0 || n == PATH_MAX) {
    errno = ENAMETOOLONG;
    return -errno;
  }
    ...
    struct ceph_snapdiff_entry_t* out)
{
  ...
  int r = snapdiff->cmount->get_client()->readdir_snapdiff(
      d1, d2->inode->snapid, &(out->dir_entry), &snapid);
  if (r >= 0) {
...
extern "C" int
ceph_close_snapdiff(struct ceph_snapdiff_info* snapdiff)
{
    ...
    int buflen)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
  return cmount->get_client()->getdents(
      reinterpret_cast<dir_result_t*>(dirp), buf, buflen);
}
    ...
    int buflen)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
  return cmount->get_client()->getdnames(
      reinterpret_cast<dir_result_t*>(dirp), buf, buflen);
}
...
extern "C" int64_t
ceph_telldir(struct ceph_mount_info* cmount, struct ceph_dir_result* dirp)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
...
extern "C" int
ceph_may_delete(struct ceph_mount_info* cmount, const char* path)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
...
extern "C" int
ceph_unlink(struct ceph_mount_info* cmount, const char* path)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
    ...
    int flags)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
  return cmount->get_client()->unlinkat(
      dirfd, relpath, flags, cmount->default_perms);
}

extern "C" int
ceph_rename(struct ceph_mount_info* cmount, const char* from, const char* to)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
...
// dirs
extern "C" int
ceph_mkdir(struct ceph_mount_info* cmount, const char* path, mode_t mode)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
    ...
    mode_t mode)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
  return cmount->get_client()->mkdirat(
      dirfd, relpath, mode, cmount->default_perms);
}
    ...
    size_t nr_snap_metadata)
{
  ...
  return cmount->get_client()->mksnap(
      path, name, cmount->default_perms, mode, metadata);
}

extern "C" int
ceph_rmsnap(struct ceph_mount_info* cmount, const char* path, const char* name)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
...
extern "C" int
ceph_mkdirs(struct ceph_mount_info* cmount, const char* path, mode_t mode)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
...
extern "C" int
ceph_rmdir(struct ceph_mount_info* cmount, const char* path)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
    ...
    int64_t size)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
  return cmount->get_client()->readlinkat(
      dirfd, relpath, buf, size, cmount->default_perms);
}
    ...
    const char* newname)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
  return cmount->get_client()->symlinkat(
      existing, dirfd, newname, cmount->default_perms);
}
    ...
    unsigned int flags)
{
  ...
  return cmount->get_client()->fstatx(
      fd, stx, cmount->default_perms, want, flags);
}
...
#endif
    return -EINVAL;
  return cmount->get_client()->statxat(
      dirfd, relpath, stx, cmount->default_perms, want, flags);
}
    ...
    unsigned int flags)
{
  ...
  if (flags & ~CEPH_REQ_FLAG_MASK)
    return -EINVAL;
  return cmount->get_client()->statx(
      path, stx, cmount->default_perms, want, flags);
}
    ...
    int flags)
{
  ...
  if (flags & ~CEPH_REQ_FLAG_MASK)
    return -EINVAL;
  return cmount->get_client()->setattrx(
      relpath, stx, mask, cmount->default_perms, flags);
}
    ...
    size_t size)
{
  ...
  return cmount->get_client()->getxattr(
      path, name, value, size, cmount->default_perms);
}
    ...
    size_t size)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
  return cmount->get_client()->lgetxattr(
      path, name, value, size, cmount->default_perms);
}
    ...
    size_t size)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
  return cmount->get_client()->fgetxattr(
      fd, name, value, size, cmount->default_perms);
}
    ...
    size_t size)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
  return cmount->get_client()->listxattr(
      path, list, size, cmount->default_perms);
}
    ...
    size_t size)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
  return cmount->get_client()->llistxattr(
      path, list, size, cmount->default_perms);
}

extern "C" int
ceph_flistxattr(struct ceph_mount_info* cmount, int fd, char* list, size_t size)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
...
extern "C" int
ceph_fremovexattr(struct ceph_mount_info* cmount, int fd, const char* name)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
    ...
    int flags)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
  return cmount->get_client()->setxattr(
      path, name, value, size, flags, cmount->default_perms);
}
    ...
    int flags)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
  return cmount->get_client()->lsetxattr(
      path, name, value, size, flags, cmount->default_perms);
}
    ...
    int flags)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
  return cmount->get_client()->fsetxattr(
      fd, name, value, size, flags, cmount->default_perms);
}

/* end xattr support */

extern "C" int
ceph_stat(struct ceph_mount_info* cmount, const char* path, struct stat* stbuf)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
...
extern "C" int
ceph_fstat(struct ceph_mount_info* cmount, int fd, struct stat* stbuf)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
...
extern int
ceph_lstat(struct ceph_mount_info* cmount, const char* path, struct stat* stbuf)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
...
extern "C" int
ceph_chmod(struct ceph_mount_info* cmount, const char* path, mode_t mode)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
...
extern "C" int
ceph_lchmod(struct ceph_mount_info* cmount, const char* path, mode_t mode)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
...
extern "C" int
ceph_fchmod(struct ceph_mount_info* cmount, int fd, mode_t mode)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
    ...
    int flags)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
  return cmount->get_client()->chmodat(
      dirfd, relpath, mode, flags, cmount->default_perms);
}

extern "C" int
ceph_chown(struct ceph_mount_info* cmount, const char* path, uid_t uid, gid_t gid)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
...
extern "C" int
ceph_fchown(struct ceph_mount_info* cmount, int fd, uid_t uid, gid_t gid)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
    ...
    int flags)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
  return cmount->get_client()->chownat(
      dirfd, relpath, uid, gid, flags, cmount->default_perms);
}

extern "C" int
ceph_utime(struct ceph_mount_info* cmount, const char* path, struct utimbuf* buf)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
...
extern "C" int
ceph_futime(struct ceph_mount_info* cmount, int fd, struct utimbuf* buf)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
...
extern "C" int
ceph_futimes(struct ceph_mount_info* cmount, int fd, struct timeval times[2])
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
...
extern "C" int
ceph_futimens(struct ceph_mount_info* cmount, int fd, struct timespec times[2])
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
    ...
    int flags)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
  return cmount->get_client()->utimensat(
      dirfd, relpath, times, flags, cmount->default_perms);
}

extern "C" int
ceph_flock(struct ceph_mount_info* cmount, int fd, int operation, uint64_t owner)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
...
extern "C" int
ceph_truncate(struct ceph_mount_info* cmount, const char* path, int64_t size)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
    ...
    mode_t mode)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
  return cmount->get_client()->openat(
      dirfd, relpath, flags, cmount->default_perms, mode);
}
    ...
    const char* data_pool)
{
  ...
  return cmount->get_client()->open(
      path, flags, cmount->default_perms, mode, stripe_unit, stripe_count,
      object_size, data_pool);
}

extern "C" int
ceph_close(struct ceph_mount_info* cmount, int fd)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
...
extern "C" int64_t
ceph_lseek(struct ceph_mount_info* cmount, int fd, int64_t offset, int whence)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
...
extern "C" int
ceph_ftruncate(struct ceph_mount_info* cmount, int fd, int64_t size)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
...
extern "C" int
ceph_fsync(struct ceph_mount_info* cmount, int fd, int syncdataonly)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
...
extern "C" int
ceph_lazyio(class ceph_mount_info* cmount, int fd, int enable)
{
  return (cmount->get_client()->lazyio(fd, enable));
}

extern "C" int
ceph_lazyio_propagate(
...
extern "C" int
ceph_sync_fs(struct ceph_mount_info* cmount)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
...
extern "C" int
ceph_get_file_stripe_unit(struct ceph_mount_info* cmount, int fh)
{
  file_layout_t l;
  int r;
...
extern "C" int
ceph_get_path_stripe_unit(struct ceph_mount_info* cmount, const char* path)
{
  file_layout_t l;
  int r;
...
extern "C" int
ceph_get_file_stripe_count(struct ceph_mount_info* cmount, int fh)
{
  file_layout_t l;
  int r;
...
extern "C" int
ceph_get_path_stripe_count(struct ceph_mount_info* cmount, const char* path)
{
  file_layout_t l;
  int r;
...
extern "C" int
ceph_get_file_object_size(struct ceph_mount_info* cmount, int fh)
{
  file_layout_t l;
  int r;
...
extern "C" int
ceph_get_path_object_size(struct ceph_mount_info* cmount, const char* path)
{
  file_layout_t l;
  int r;
...
extern "C" int
ceph_get_file_pool(struct ceph_mount_info* cmount, int fh)
{
  file_layout_t l;
  int r;
...
extern "C" int
ceph_get_path_pool(struct ceph_mount_info* cmount, const char* path)
{
  file_layout_t l;
  int r;
...
extern "C" int
ceph_get_default_data_pool_name(
    struct ceph_mount_info* cmount,
    char* buf,
    size_t len)
{
...
extern "C" int
ceph_get_file_replication(struct ceph_mount_info* cmount, int fh)
{
  file_layout_t l;
  int r;
...
extern "C" int
ceph_get_path_replication(struct ceph_mount_info* cmount, const char* path)
{
  file_layout_t l;
  int r;
...
extern "C" int
ceph_set_default_file_stripe_unit(struct ceph_mount_info* cmount, int stripe)
{
...
extern "C" int
ceph_set_default_file_stripe_count(struct ceph_mount_info* cmount, int count)
{
  // this option no longer exists
  return -EOPNOTSUPP;
}
...
extern "C" int
ceph_set_default_object_size(struct ceph_mount_info* cmount, int size)
{
  // this option no longer exists
  return -EOPNOTSUPP;
}
...
extern "C" int
ceph_set_default_file_replication(struct ceph_mount_info* cmount, int replication)
{
  // this option no longer exists
  return -EOPNOTSUPP;
}
...
extern "C" int
ceph_set_default_preferred_pg(struct ceph_mount_info* cmount, int osd)
{
  // this option no longer exists
  return -EOPNOTSUPP;
}
    ...
    int nosds)
{
  ...
  vector<int> vosds;
  int ret =
      cmount->get_client()->get_file_extent_osds(fh, offset, length, vosds);
  if (ret < 0)
    return ret;
...
extern "C" int
ceph_localize_reads(struct ceph_mount_info* cmount, int val)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
  if (!val)
...
extern "C" CephContext*
ceph_get_mount_context(struct ceph_mount_info* cmount)
{
  return cmount->get_ceph_context();
}

extern "C" int
ceph_debug_get_fd_caps(struct ceph_mount_info* cmount, int fd)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
...
extern "C" int
ceph_debug_get_file_caps(struct ceph_mount_info* cmount, const char* path)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
...
extern "C" int
ceph_get_stripe_unit_granularity(struct ceph_mount_info* cmount)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
...
extern "C" int
ceph_get_pool_id(struct ceph_mount_info* cmount, const char* pool_name)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
...
extern "C" int
ceph_get_pool_replication(struct ceph_mount_info* cmount, int pool_id)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
...
extern "C" struct Inode*
ceph_ll_get_inode(class ceph_mount_info* cmount, vinodeno_t vino)
{
    ...
    Inode** inode)
{
  return (cmount->get_client())
      ->ll_lookup_vino(vino, cmount->default_perms, inode);
}
...
{
  return (cmount->get_client())
      ->ll_lookup_inode(ino, cmount->default_perms, inode);
}
    ...
    const UserPerm* perms)
{
  if (flags & ~CEPH_REQ_FLAG_MASK)
    return -EINVAL;
  return (cmount->get_client())
      ->ll_lookupx(parent, name, out, stx, want, flags, *perms);
}

extern "C" void
ceph_ll_get(class ceph_mount_info* cmount, Inode* in)
{
  cmount->get_client()->ll_get(in);
}

extern "C" int
ceph_ll_put(class ceph_mount_info* cmount, Inode* in)
{
  return (cmount->get_client()->ll_put(in));
}

extern "C" int
ceph_ll_forget(class ceph_mount_info* cmount, Inode* in, int count)
{
...
{
  file_layout_t l;
  int r = (cmount->get_client()->ll_read_block(
      in, blockid, buf, offset, length, &l));
  l.to_legacy(layout);
  return r;
}
...
{
  file_layout_t l;
  int r = (cmount->get_client()->ll_write_block(
      in, blockid, buf, offset, length, &l, snapseq, sync));
  l.to_legacy(layout);
  return r;
}

extern "C" int
ceph_ll_commit_blocks(
    class ceph_mount_info* cmount,
    Inode* in,
    uint64_t offset,
    uint64_t range)
...
extern "C" int
ceph_ll_fsync(class ceph_mount_info* cmount, Fh* fh, int syncdataonly)
{
  return (cmount->get_client()->ll_fsync(fh, syncdataonly));
}

extern "C" int
ceph_ll_sync_inode(class ceph_mount_info* cmount, Inode* in, int syncdataonly)
{
...
extern "C" off_t
ceph_ll_lseek(class ceph_mount_info* cmount, Fh* fh, off_t offset, int whence)
{
    ...
    struct ceph_ll_io_info* io_info)
{
  ...
  return (cmount->get_client()->ll_preadv_pwritev(
      io_info->fh, io_info->iov, io_info->iovcnt, io_info->off, io_info->write,
      onfinish, &onfinish->bl, io_info->fsync, io_info->syncdataonly));
}

extern "C" int
ceph_ll_close(class ceph_mount_info* cmount, Fh* fh)
{
    ...
    const UserPerm* perms)
{
  ...
  return (cmount->get_client())
      ->ll_createx(
          parent, name, mode, oflags, outp, fhp, stx, want, lflags, *perms);
}
    ...
    const UserPerm* perms)
{
  if (flags & ~CEPH_REQ_FLAG_MASK)
    return -EINVAL;
  return (cmount->get_client())
      ->ll_mknodx(parent, name, mode, rdev, out, stx, want, flags, *perms);
}
    ...
    const UserPerm* perms)
{
  if (flags & ~CEPH_REQ_FLAG_MASK)
    return -EINVAL;
  return cmount->get_client()->ll_mkdirx(
      parent, name, mode, out, stx, want, flags, *perms);
}
...
ceph_ll_opendir(
    ...
    const UserPerm* perms)
{
  return (cmount->get_client()->ll_opendir(
      in, O_RDONLY, (dir_result_t**)dirpp, *perms));
}

extern "C" int
ceph_ll_releasedir(class ceph_mount_info* cmount, ceph_dir_result* dir)
{
  return cmount->get_client()->ll_releasedir(
      reinterpret_cast<dir_result_t*>(dir));
}
...
ceph_ll_rename(
    ...
    const UserPerm* perms)
{
  return cmount->get_client()->ll_rename(
      parent, name, newparent, newname, *perms);
}
...
extern "C" int
ceph_ll_statfs(class ceph_mount_info* cmount, Inode* in, struct statvfs* stbuf)
{
    ...
    const UserPerm* perms)
{
  ...
  return (cmount->get_client()->ll_symlinkx(
      in, name, value, out, stx, want, flags, *perms));
}
...
{
  return (
      cmount->get_client()->ll_setxattr(in, name, value, size, flags, *perms));
}
...
extern "C" int
ceph_ll_lazyio(class ceph_mount_info* cmount, Fh* fh, int enable)
{
...
extern "C" uint32_t
ceph_ll_stripe_unit(class ceph_mount_info* cmount, Inode* in)
{
...
uint64_t
ceph_ll_snap_seq(class ceph_mount_info* cmount, Inode* in)
{
  return (cmount->get_client()->ll_snap_seq(in));
}

extern "C" int
ceph_ll_get_stripe_osd(
    class ceph_mount_info* cmount,
    Inode* in,
    uint64_t blockno,
    struct ceph_file_layout* layout)
...
extern "C" int
ceph_ll_num_osds(class ceph_mount_info* cmount)
{
  return (cmount->get_client()->ll_num_osds());
}

extern "C" int
ceph_ll_osdaddr(class ceph_mount_info* cmount, int osd, uint32_t* addr)
{
  return (cmount->get_client()->ll_osdaddr(osd, addr));
}

extern "C" uint64_t
ceph_ll_get_internal_offset(
    class ceph_mount_info* cmount,
    Inode* in,
    uint64_t blockno)
{
...
extern "C" uint32_t
ceph_get_cap_return_timeout(class ceph_mount_info* cmount)
{
  if (!cmount->is_mounted())
    return 0;
...
extern "C" int
ceph_set_deleg_timeout(class ceph_mount_info* cmount, uint32_t timeout)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
...
extern "C" void
ceph_set_session_timeout(class ceph_mount_info* cmount, unsigned timeout)
{
  cmount->get_client()->set_session_timeout(timeout);
}

extern "C" void
ceph_set_uuid(class ceph_mount_info* cmount, const char* uuid)
{
    ...
    unsigned flags)
{
  ...
  return cmount->get_client()->start_reclaim(
      std::string(uuid), flags, cmount->get_filesystem());
}

extern "C" void
ceph_finish_reclaim(class ceph_mount_info* cmount)
{
  cmount->get_client()->finish_reclaim();
}
#if defined(__linux__)
extern "C" int
ceph_add_fscrypt_key(
    struct ceph_mount_info* cmount,
    const char* key_data,
    int key_len,
    char* out_keyid,
    int user)
{
  ...
  return cmount->get_client()->add_fscrypt_key(
      key_data, key_len, out_keyid, user);
}

extern "C" int
ceph_remove_fscrypt_key(
    struct ceph_mount_info* cmount,
...
extern "C" int
ceph_is_encrypted(struct ceph_mount_info* cmount, int fd, char* enctag)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
...
extern "C" int
ceph_ll_is_encrypted(struct ceph_mount_info* cmount, Inode* in, char* enctag)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;

  return cmount->get_client()->ll_is_encrypted(
      in, cmount->default_perms, enctag);
}
#endif
// This is deprecated, use ceph_ll_register_callbacks2 instead.
extern "C" void
ceph_ll_register_callbacks(
    class ceph_mount_info* cmount,
    ...
    struct snap_info* snap_info)
{
  ...
  int r =
      cmount->get_client()->get_snap_info(path, cmount->default_perms, &info);
  if (r < 0) {
    return r;
  }
  ...
  if (nr_metadata) {
    snap_info->snap_metadata = (struct snap_metadata*)calloc(
        nr_metadata, sizeof(struct snap_metadata));
...
extern "C" void
ceph_free_snap_info_buffer(struct snap_info* snap_info)
{
  for (size_t i = 0; i < snap_info->nr_snap_metadata; ++i) {
    free((void*)snap_info->snap_metadata[i]
             .key); // malloc'd memory is key+value composite
  }
  free(snap_info->snap_metadata);
}

extern "C" int
ceph_get_perf_counters(struct ceph_mount_info* cmount, char** perf_dump)
{
  bufferlist outbl;
    ...
    mode_t mode)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;
  return cmount->get_client()->fcopyfile(
      spath, dpath, cmount->default_perms, mode);
}
