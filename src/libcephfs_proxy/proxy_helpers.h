...
{
  value = (value & 0xff00ff00ff00ffULL) + ((value >> 8) & 0xff00ff00ff00ffULL);
  value += value >> 16;
  value += value >> 32;
...
ptr_checksum(proxy_random_t* rnd, void* ptr)
{
  ...
  if ((value & 0xff00000000000007ULL) != 0) {
    proxy_log(LOG_ERR, EINVAL, "Unexpected pointer value");
    abort();
  }
...
proxy_condition_init(pthread_cond_t* condition)
{
  ...
  if (err != 0) {
    return proxy_log(LOG_ERR, err, "Failed to initialize a condition variable");
  }
...
static inline int32_t
proxy_thread_create(pthread_t* tid, void* (*start)(void*), void* arg)
{
  int32_t err;
...
static inline int32_t
proxy_signal_set(int32_t signum, struct sigaction* action, struct sigaction* old)
{
...
int32_t proxy_hash(
    uint8_t* hash,
    size_t size,
    int32_t (*feed)(void**, void*, int32_t),
    void* data);

int32_t proxy_hash_hex(
    char* digest,
    size_t size,
    int32_t (*feed)(void**, void*, int32_t),
    void* data);
