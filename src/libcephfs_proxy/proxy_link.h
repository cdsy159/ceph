...
#define PROXY_LINK_DISCONNECTED \
  {                             \
    NULL, -1                    \
  }
...
proxy_link_negotiate_init(
    ...
    uint32_t num_cbks)
{
  proxy_link_negotiate_init_v0(
      neg, PROXY_LINK_NEGOTIATE_VERSION, min_version, num_ops, num_cbks);
...
static inline bool
proxy_link_is_connected(proxy_link_t* link)
{
  return link->sd >= 0;
}

int32_t proxy_link_client(
    proxy_link_t* link,
    const char* path,
    proxy_link_stop_t stop);
...
int32_t proxy_link_server(
    proxy_link_t* link,
    proxy_settings_t* settings,
    proxy_link_start_t start,
    proxy_link_stop_t stop);
...
int32_t proxy_link_request(
    int32_t sd,
    int32_t op,
    struct iovec* req_iov,
    int32_t req_count,
    struct iovec* ans_iov,
    int32_t ans_count);

int32_t proxy_link_ctrl_send(
    int32_t sd,
    void* data,
    int32_t size,
    int32_t type,
...
int32_t proxy_link_handshake_server(
    proxy_link_t* link,
    int32_t sd,
    proxy_link_negotiate_t* neg,
    proxy_link_negotiate_cbk_t cbk);

int32_t proxy_link_handshake_client(
    proxy_link_t* link,
    int32_t sd,
    proxy_link_negotiate_t* neg,
    proxy_link_negotiate_cbk_t cbk);
