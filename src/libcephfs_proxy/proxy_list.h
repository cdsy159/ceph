...
static inline void
list_add_between(list_t* item, list_t* prev, list_t* next)
{
  item->next = next;
  item->prev = prev;
  prev->next = item;
...
static inline void
list_splice_between(list_t* src, list_t* prev, list_t* next)
{
  list_t *first, *last;
...
static inline void
list_splice_tail(list_t* src, list_t* dst)
{
  if (!list_empty(src)) {
...
static inline void
list_splice_init(list_t* src, list_t* dst)
{
  if (!list_empty(src)) {
    list_splice_between(src, dst, dst->next);
    list_init(src);
...
static inline void
list_splice_tail_init(list_t* src, list_t* dst)
{
  if (!list_empty(src)) {
    list_splice_between(src, dst->prev, dst);
    list_init(src);
