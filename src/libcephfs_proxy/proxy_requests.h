...
#define PROTO_VERSION(_neg, _data, _ver)                        \
  do {                                                          \
    if ((_neg)->v2.protocol >= (_ver)) {                        \
      _data##_iov[0].iov_len = PROTO_VERSION_SIZE(_data, _ver); \
    }                                                           \
  } while (false)
...
#define CEPH_DATA(_name, _data, _data_count)   \
  ...
  CEPH_BUFF_ADD(_data, &_data, PROTO_VERSION_SIZE(_data, PROXY_PROTOCOL_V0))

#define CEPH_REQ(_name, _req, _req_count, _ans, _ans_count) \
  CEPH_DATA(_name, _req, _req_count);                       \
  CEPH_DATA(_name, _ans, _ans_count)

#define CEPH_CBK(_name, _cbk, _cbk_count) CEPH_DATA(_name, _cbk, _cbk_count)

#define CEPH_CALL(_sd, _op, _req, _ans) \
  proxy_link_request(                   \
      (_sd), _op, _req##_iov, _req##_count, _ans##_iov, _ans##_count)
...
#define PROTO_BUILD_1(_fields, _name) \
  PROTO_STRUCT_COND(_name)(_name, _fields) PROTO_STRUCT(, _fields)

#define PROTO_BUILD_2(_fields, _name, _more...) \
  PROTO_STRUCT_COND(_name)(_name, _fields) PROTO_BUILD_1(_fields _more, )

#define PROTO_BUILD_3(_fields, _name, _more...) \
  PROTO_STRUCT_COND(_name)(_name, _fields) PROTO_BUILD_2(_fields _more, )

#define PROTO_BUILD_4(_fields, _name, _more...) \
  PROTO_STRUCT_COND(_name)(_name, _fields) PROTO_BUILD_3(_fields _more, )

#define PROTO_TYPE(_name, _fields...) \
  typedef union {                     \
    PROTO_BUILD_4(_fields)            \
  } _name
...
PROTO_CALL(
    ceph_version,
    PROTO_REQ(PROTO_VER(v0, )),
    PROTO_ANS(PROTO_VER(v0, int32_t major; int32_t minor; int32_t patch;
                        int16_t text;)));

PROTO_CALL(
    ceph_userperm_new,
    PROTO_REQ(PROTO_VER(v0, uint32_t uid; uint32_t gid; uint32_t groups;)),
    PROTO_ANS(PROTO_VER(v0, uint64_t userperm;)));

PROTO_CALL(
    ceph_userperm_destroy,
    PROTO_REQ(PROTO_VER(v0, uint64_t userperm;)),
    PROTO_ANS(PROTO_VER(v0, )));

PROTO_CALL(
    ceph_create,
    PROTO_REQ(PROTO_VER(v0, int16_t id;)),
    PROTO_ANS(PROTO_VER(v0, uint64_t cmount;)));

PROTO_CALL(
    ceph_release,
    PROTO_REQ(PROTO_VER(v0, uint64_t cmount;)),
    PROTO_ANS(PROTO_VER(v0, )));

PROTO_CALL(
    ceph_conf_read_file,
    PROTO_REQ(PROTO_VER(v0, uint64_t cmount; uint16_t path;)),
    PROTO_ANS(PROTO_VER(v0, )));

PROTO_CALL(
    ceph_conf_get,
    PROTO_REQ(PROTO_VER(v0, uint64_t cmount; uint32_t size; uint16_t option;)),
    PROTO_ANS(PROTO_VER(v0, uint16_t value;)));

PROTO_CALL(
    ceph_conf_set,
    PROTO_REQ(PROTO_VER(v0, uint64_t cmount; uint16_t option; uint16_t value;)),
    PROTO_ANS(PROTO_VER(v0, )));
...
PROTO_CALL(
    ...
    PROTO_REQ(PROTO_VER(v0, uint64_t cmount; uint16_t fs;)),
    PROTO_ANS(PROTO_VER(v0, )));

PROTO_CALL(
    ceph_mount,
    PROTO_REQ(PROTO_VER(v0, uint64_t cmount; uint16_t root;)),
    PROTO_ANS(PROTO_VER(v0, )));
...
PROTO_CALL(
    ceph_ll_statfs,
    PROTO_REQ(PROTO_VER(v0, uint64_t cmount; uint64_t inode;)),
    PROTO_ANS(PROTO_VER(v0, )));

PROTO_CALL(
    ceph_ll_lookup,
    PROTO_REQ(
        PROTO_VER(v0, uint64_t cmount; embedded_perms_t userperm;
                  uint64_t parent;
                  uint32_t want;
                  uint32_t flags;
                  uint16_t name;),
        PROTO_VER(v1, uint32_t ngroups;)),
    PROTO_ANS(PROTO_VER(v0, uint64_t inode;)));

PROTO_CALL(
    ceph_ll_lookup_inode,
    PROTO_REQ(PROTO_VER(v0, uint64_t cmount; struct inodeno_t ino;)),
    PROTO_ANS(PROTO_VER(v0, uint64_t inode;)));
...
PROTO_CALL(
    ceph_ll_walk,
    PROTO_REQ(
        PROTO_VER(v0, uint64_t cmount; embedded_perms_t userperm; uint32_t want;
                  uint32_t flags;
                  uint16_t path;),
        PROTO_VER(v1, uint32_t ngroups;)),
    PROTO_ANS(PROTO_VER(v0, uint64_t inode;)));
...
PROTO_CALL(
    ceph_getcwd,
    PROTO_REQ(PROTO_VER(v0, uint64_t cmount;)),
    PROTO_ANS(PROTO_VER(v0, uint16_t path;)));

PROTO_CALL(
    ceph_readdir,
    PROTO_REQ(PROTO_VER(v0, uint64_t cmount; uint64_t dir;)),
    PROTO_ANS(PROTO_VER(v0, bool eod;)));
...
PROTO_CALL(
    ceph_ll_open,
    PROTO_REQ(
        PROTO_VER(v0, uint64_t cmount; embedded_perms_t userperm; uint64_t inode;
                  int32_t flags;),
        PROTO_VER(v1, uint32_t ngroups;)),
    PROTO_ANS(PROTO_VER(v0, uint64_t fh;)));
...
PROTO_CALL(
    ...
    PROTO_REQ(
        PROTO_VER(v0, uint64_t cmount; embedded_perms_t userperm;
                  ...
                  uint16_t name;),
        PROTO_VER(v1, uint32_t ngroups;)),
    PROTO_ANS(PROTO_VER(v0, uint64_t inode; uint64_t fh;)));
...
PROTO_CALL(
    ceph_ll_close,
    PROTO_REQ(PROTO_VER(v0, uint64_t cmount; uint64_t fh;)),
    PROTO_ANS(PROTO_VER(v0, )));
...
PROTO_CALL(
    ceph_ll_lseek,
    PROTO_REQ(PROTO_VER(v0, uint64_t cmount; uint64_t fh; off_t offset;
                        int32_t whence;)),
    PROTO_ANS(PROTO_VER(v0, off_t offset;)));

PROTO_CALL(
    ceph_ll_read,
    PROTO_REQ(PROTO_VER(v0, uint64_t cmount; uint64_t fh; int64_t offset;
                        uint64_t len;)),
    PROTO_ANS(PROTO_VER(v0, )));
...
PROTO_CALL(
    ceph_ll_fsync,
    PROTO_REQ(PROTO_VER(v0, uint64_t cmount; uint64_t fh; int32_t dataonly;)),
    PROTO_ANS(PROTO_VER(v0, )));
...
PROTO_CALL(
    ...
    PROTO_REQ(
        PROTO_VER(v0, uint64_t cmount; embedded_perms_t userperm; uint64_t inode;
                  size_t size;),
        PROTO_VER(v1, uint32_t ngroups;)),
    PROTO_ANS(PROTO_VER(v0, size_t size;)));
...
PROTO_CALL(
    ...
    PROTO_REQ(
        PROTO_VER(v0, uint64_t cmount; embedded_perms_t userperm;
                  uint64_t inode;),
        PROTO_VER(v1, uint32_t ngroups;)),
    PROTO_ANS(PROTO_VER(v0, uint64_t dir;)));
...
PROTO_CALL(
    ...
    PROTO_REQ(PROTO_VER(v0, uint64_t cmount; uint64_t info; uint64_t fh;
                        int64_t off;
                        uint64_t size;
                        bool write;
                        bool fsync;
                        bool syncdataonly;)),
    PROTO_ANS(PROTO_VER(v0, int64_t res;)));

PROTO_CALL(
    ceph_add_fscrypt_key,
    PROTO_REQ(PROTO_VER(v0, uint64_t cmount; uint32_t user; uint32_t kid;
                        uint16_t key;)),
    PROTO_ANS(PROTO_VER(v0, )));

PROTO_CALL(
    ceph_remove_fscrypt_key,
    PROTO_REQ(PROTO_VER(v0, uint64_t cmount; uint32_t user; uint32_t arg;)),
    PROTO_ANS(PROTO_VER(v0, )));

PROTO_CALL(
    ceph_get_fscrypt_key_status,
    PROTO_REQ(PROTO_VER(v0, uint64_t cmount; uint32_t arg;)),
    PROTO_ANS(PROTO_VER(v0, )));

PROTO_CALL(
    ceph_ll_set_fscrypt_policy_v2,
    PROTO_REQ(PROTO_VER(v0, uint64_t cmount; uint64_t inode; uint32_t policy;)),
    PROTO_ANS(PROTO_VER(v0, )));
...
PROTO_CALL(
    ceph_ll_is_encrypted,
    PROTO_REQ(PROTO_VER(v0, uint64_t cmount; uint64_t inode; uint16_t tag;)),
    PROTO_ANS(PROTO_VER(v0, uint32_t policy;)));

PROTO_CALL(
    ceph_ll_nonblocking_fsync,
    PROTO_REQ(PROTO_VER(v0, uint64_t cmount; uint64_t info; uint64_t inode;
                        bool syncdataonly;)),
    PROTO_ANS(PROTO_VER(v0, int32_t res;)));

PROTO_CALL(
    ceph_batch_readdir,
    PROTO_REQ(PROTO_VER(v0, uint64_t cmount; uint64_t dir; uint32_t size;)),
    PROTO_ANS(PROTO_VER(v0, bool eod;)));
...
PROTO_NOTIFY(
    ceph_ll_nonblocking_readv_writev,
    PROTO_CBK(PROTO_VER(v0, uint64_t info; int64_t res;)));
