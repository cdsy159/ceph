...
void
ReadCompletionData::complete_read(int r)
{
...
void
ReadCompletionData::complete_unlock(int r)
{
...
void
WriteCompletionData::complete_unlock(int r)
{
  // call parent's completion method
  // Note that we ignore the return code of the unlock as we cannot do much about it
...
void
WriteCompletionData::complete_write(int r)
{
  // Remember return code
  m_writeRc = r;
}
...
void
WriteCompletionData::safe(int r)
{
  if (m_safe)
    m_safe->finish(r);
}
...
private:
  ...
  RadosReadCompletionData(
      MultiAioCompletionImplPtr multiAioCompl,
...
private:
  ...
  RadosRemoveCompletionData(
      MultiAioCompletionImplPtr multiAioCompl,
      CephContext* context) :
    RefCountedObject(context), m_multiAioCompl(multiAioCompl){};
};
...
libradosstriper::RadosStriperImpl::RadosStriperImpl(
...
int
libradosstriper::RadosStriperImpl::setObjectLayoutStripeUnit(
...
int
libradosstriper::RadosStriperImpl::setObjectLayoutStripeCount(
...
int
libradosstriper::RadosStriperImpl::setObjectLayoutObjectSize(
...
int
libradosstriper::RadosStriperImpl::getxattr(
    const object_t& soid,
    const char* name,
    bufferlist& bl)
{
...
int
libradosstriper::RadosStriperImpl::setxattr(
    const object_t& soid,
    const char* name,
    bufferlist& bl)
{
...
int
libradosstriper::RadosStriperImpl::getxattrs(
    const object_t& soid,
...
int
libradosstriper::RadosStriperImpl::rmxattr(const object_t& soid, const char* name)
{
  std::string firstObjOid = getObjectId(soid, 0);
...
int
libradosstriper::RadosStriperImpl::write(
    const std::string& soid,
    const bufferlist& bl,
    size_t len,
    uint64_t off)
{
  ...
  int rc =
      createAndOpenStripedObject(soid, &layout, len + off, &lockCookie, true);
  if (rc)
    return rc;
...
int
libradosstriper::RadosStriperImpl::append(
    const std::string& soid,
    const bufferlist& bl,
    size_t len)
{
...
int
libradosstriper::RadosStriperImpl::write_full(
    const std::string& soid,
    const bufferlist& bl)
{
  int rc = trunc(soid, 0);
  if (rc && rc != -ENOENT)
    return rc; // ENOENT is obviously ok
  return write(soid, bl, bl.length(), 0);
}

int
libradosstriper::RadosStriperImpl::read(
    const std::string& soid,
    bufferlist* bl,
    size_t len,
    uint64_t off)
...
int
libradosstriper::RadosStriperImpl::aio_write(
    const std::string& soid,
    librados::AioCompletionImpl* c,
    const bufferlist& bl,
    size_t len,
...
int
libradosstriper::RadosStriperImpl::aio_append(
    const std::string& soid,
    librados::AioCompletionImpl* c,
    const bufferlist& bl,
    size_t len)
...
int
libradosstriper::RadosStriperImpl::aio_write_full(
    const std::string& soid,
    librados::AioCompletionImpl* c,
    const bufferlist& bl)
{
...
static void
rados_read_aio_unlock_complete(rados_striper_multi_completion_t c, void* arg)
{
  auto cdata = ceph::ref_t<ReadCompletionData>(
      static_cast<ReadCompletionData*>(arg), false);
...
static void
striper_read_aio_req_complete(rados_striper_multi_completion_t c, void* arg)
{
  auto cdata = static_cast<ReadCompletionData*>(arg);
  // launch the async unlocking of the object
  cdata->m_striper->aio_unlockObject(
      cdata->m_soid, cdata->m_lockCookie, cdata->m_unlockCompletion);
...
static void
rados_req_read_complete(rados_completion_t c, void* arg)
{
  ...
  if (rc >= 0 && (((uint64_t)rc) < data->m_expectedBytes)) {
    ...
    unsigned int existingDataToZero =
        std::min(data->m_bl->length() - rc, lenOfZeros);
    if (existingDataToZero > 0) {
      data->m_bl->zero(rc, existingDataToZero);
    }
    if (lenOfZeros > existingDataToZero) {
      ceph::bufferptr zeros(
          ceph::buffer::create(lenOfZeros - existingDataToZero));
      zeros.zero();
    ...
    uint64_t off)
{
  ...
  if (read_len > 0) {
    ...
    Striper::file_to_extents(
        cct(), format.c_str(), &l, off, read_len, 0, *extents);
  }
  ...
  auto cdata = ceph::make_ref<ReadCompletionData>(
      this, soid, lockCookie, c, bl, extents, resultbl);
  c->is_read = true;
  c->io = m_ioCtxImpl;
  ...
  int r = 0, i = 0;
  for (vector<ObjectExtent>::iterator p = extents->begin(); p != extents->end();
       ++p) {
    ...
    // will release one
    auto data =
        ceph::make_ref<RadosReadCompletionData>(nc, p->length, oid_bl, cct());
    librados::AioCompletion* rados_completion =
        librados::Rados::aio_create_completion(
            data.detach(), rados_req_read_complete);
    r = m_ioCtx.aio_read(
        p->oid.name, rados_completion, oid_bl, p->length, p->offset);
...
int
libradosstriper::RadosStriperImpl::aio_flush()
{
  int ret;
...
int
libradosstriper::RadosStriperImpl::stat(
    const std::string& soid,
    uint64_t* psize,
    time_t* pmtime)
{
...
static void
striper_stat_aio_stat_complete(rados_completion_t c, void* arg)
{
  auto data = ceph::ref_t<BasicStatCompletionData>(
      static_cast<BasicStatCompletionData*>(arg), false);
  int rc = rados_aio_get_return_value(c);
  if (rc == -ENOENT) {
...
static void
striper_stat_aio_getxattr_complete(rados_completion_t c, void* arg)
{
  auto data = ceph::ref_t<BasicStatCompletionData>(
      static_cast<BasicStatCompletionData*>(arg), false);
  int rc = rados_aio_get_return_value(c);
...
static void
striper_stat_aio_req_complete(rados_striper_multi_completion_t c, void* arg)
{
  auto data = ceph::ref_t<BasicStatCompletionData>(
      static_cast<BasicStatCompletionData*>(arg), false);
  if (data->m_statRC) {
...
template <class TimeType>
int
libradosstriper::RadosStriperImpl::aio_generic_stat(
    const std::string& soid,
    librados::AioCompletionImpl* c,
    uint64_t* psize,
    TimeType* pmtime,
    typename libradosstriper::RadosStriperImpl::StatFunction<TimeType>::Type
        statFunction)
{
  ...
  std::string firstObjOid = getObjectId(soid, 0);
  auto cdata = ceph::make_ref<StatCompletionData<TimeType>>(
      this, firstObjOid, c, multi_completion.get(), psize, pmtime);
  multi_completion->set_complete_callback(
      cdata->get() /* create ref! */, striper_stat_aio_req_complete);
  // use a regular AioCompletion for the stat async call
  librados::AioCompletion* stat_completion =
      librados::Rados::aio_create_completion(
          cdata->get() /* create ref! */, striper_stat_aio_stat_complete);
  multi_completion->add_safe_request();
  object_t obj(firstObjOid);
  int rc = (m_ioCtxImpl->*statFunction)(
      obj, stat_completion->pc, &cdata->m_objectSize, cdata->m_pmtime);
  ...
  librados::AioCompletion* getxattr_completion =
      librados::Rados::aio_create_completion(
          cdata->get() /* create ref! */, striper_stat_aio_getxattr_complete);
  multi_completion->add_safe_request();
  // in parallel, get the pmsize from the first object asynchronously
  rc = m_ioCtxImpl->aio_getxattr(
      obj, getxattr_completion->pc, XATTR_SIZE, cdata->m_bl);
...
int
libradosstriper::RadosStriperImpl::aio_stat(
    const std::string& soid,
    librados::AioCompletionImpl* c,
    uint64_t* psize,
    time_t* pmtime)
{
  return aio_generic_stat<time_t>(
      soid, c, psize, pmtime, &librados::IoCtxImpl::aio_stat);
}

int
libradosstriper::RadosStriperImpl::stat2(
    const std::string& soid,
    uint64_t* psize,
    struct timespec* pts)
{
...
int
libradosstriper::RadosStriperImpl::aio_stat2(
    const std::string& soid,
    librados::AioCompletionImpl* c,
    uint64_t* psize,
    struct timespec* pts)
{
  return aio_generic_stat<struct timespec>(
      soid, c, psize, pts, &librados::IoCtxImpl::aio_stat2);
}

static void
rados_req_remove_complete(rados_completion_t c, void* arg)
{
...
static void
striper_remove_aio_req_complete(rados_striper_multi_completion_t c, void* arg)
{
  auto cdata = ceph::ref_t<RemoveCompletionData>(
      static_cast<RemoveCompletionData*>(arg), false);
  libradosstriper::MultiAioCompletionImpl* comp =
      reinterpret_cast<libradosstriper::MultiAioCompletionImpl*>(c);
  ...
  if (rc == 0) {
    ...
    rc = cdata->m_striper->m_ioCtx.remove(
        cdata->m_striper->getObjectId(cdata->m_soid, 0), cdata->flags);
  } else {
    lderr(cdata->m_striper->cct())
        << "RadosStriperImpl : deletion/truncation incomplete for "
        << cdata->m_soid
        << ", as errors were encountered. The file is left present but it's "
           "content "
...
int
libradosstriper::RadosStriperImpl::remove(const std::string& soid, int flags)
{
  // create a completion object
  librados::AioCompletionImpl c;
...
int
libradosstriper::RadosStriperImpl::aio_remove(
    const std::string& soid,
    librados::AioCompletionImpl* c,
    int flags)
{
  ...
  int rc = m_ioCtx.lock_exclusive(
      getObjectId(soid, 0), RADOS_LOCK_NAME, lockCookie, "", 0, 0);
  if (rc)
    return rc;
  // create CompletionData for the async remove call
  auto cdata =
      ceph::make_ref<RemoveCompletionData>(this, soid, lockCookie, c, flags);
  MultiAioCompletionImplPtr multi_completion{
      new libradosstriper::MultiAioCompletionImpl, false};
  multi_completion->set_complete_callback(
      cdata->get() /* create ref! */, striper_remove_aio_req_complete);
  // call asynchronous internal version of remove
  ldout(cct(), 10) << "RadosStriperImpl : Aio_remove starting for " << soid
                   << dendl;
...
int
libradosstriper::RadosStriperImpl::internal_aio_remove(
    const std::string& soid,
    MultiAioCompletionImplPtr multi_completion,
    int flags)
{
  ...
  try {
    ...
    if (rc < 0) {
      ...
      uint64_t remaining_objects =
          std::min(remaining_stripe_units, stripe_count);
    ...
    for (int i = nb_objects - 1; i >= 1; i--) {
      ...
      auto data =
          ceph::make_ref<RadosRemoveCompletionData>(multi_completion, cct());
      librados::AioCompletion* rados_completion =
          librados::Rados::aio_create_completion(
              data->get() /* create ref! */, rados_req_remove_complete);
      if (flags == 0) {
      ...
      if (rcr < 0 and -ENOENT != rcr) {
        lderr(cct()) << "RadosStriperImpl::remove : deletion incomplete for "
                     << soid << ", as " << getObjectId(soid, i)
                     << " could not be deleted (rc=" << rc << ")" << dendl;
        break;
      }
    }
...
int
libradosstriper::RadosStriperImpl::trunc(const std::string& soid, uint64_t size)
{
  ...
  utime_t dur = utime_t();
  rados::cls::lock::lock(
      &op, RADOS_LOCK_NAME, ClsLockType::EXCLUSIVE, lockCookie, "", "", dur, 0);
...
std::string
libradosstriper::RadosStriperImpl::getObjectId(
    const object_t& soid,
    long long unsigned objectno)
{
  std::ostringstream s;
  s << soid << '.' << std::setfill('0') << std::setw(16) << std::hex
    << objectno;
  return s.str();
}

void
libradosstriper::RadosStriperImpl::unlockObject(
    const std::string& soid,
    const std::string& lockCookie)
{
...
void
libradosstriper::RadosStriperImpl::aio_unlockObject(
    const std::string& soid,
    const std::string& lockCookie,
...
static void
rados_write_aio_unlock_complete(rados_striper_multi_completion_t c, void* arg)
{
  auto cdata = ceph::ref_t<WriteCompletionData>(
      static_cast<WriteCompletionData*>(arg), false);
  libradosstriper::MultiAioCompletionImpl* comp =
      reinterpret_cast<libradosstriper::MultiAioCompletionImpl*>(c);
  cdata->complete_unlock(comp->rval);
}
...
static void
striper_write_aio_req_complete(rados_striper_multi_completion_t c, void* arg)
{
  auto cdata = ceph::ref_t<WriteCompletionData>(
      static_cast<WriteCompletionData*>(arg), false);
  // launch the async unlocking of the object
...
static void
striper_write_aio_req_safe(rados_striper_multi_completion_t c, void* arg)
{
  auto cdata = ceph::ref_t<WriteCompletionData>(
      static_cast<WriteCompletionData*>(arg), false);
  libradosstriper::MultiAioCompletionImpl* comp =
...
int
libradosstriper::RadosStriperImpl::write_in_open_object(
    const std::string& soid,
...
int
libradosstriper::RadosStriperImpl::aio_write_in_open_object(
    const std::string& soid,
    librados::AioCompletionImpl* c,
    const ceph_file_layout& layout,
    const std::string& lockCookie,
    ...
    uint64_t off)
{
  ...
  libradosstriper::MultiAioCompletionImplPtr nc{
      new libradosstriper::MultiAioCompletionImpl, false};
  nc->set_complete_callback(
      cdata->get() /* create ref! */, striper_write_aio_req_complete);
  nc->set_safe_callback(
      cdata->get() /* create ref! */, striper_write_aio_req_safe);
...
static void
rados_req_write_complete(rados_completion_t c, void* arg)
{
...
int
libradosstriper::RadosStriperImpl::internal_aio_write(
    const std::string& soid,
...
int
libradosstriper::RadosStriperImpl::extract_uint32_attr(
...
int
libradosstriper::RadosStriperImpl::extract_sizet_attr(
    std::map<std::string, bufferlist>& attrs,
    const std::string& key,
    size_t* value)
{
...
int
libradosstriper::RadosStriperImpl::internal_get_layout_and_size(
    ...
    uint64_t* size)
{
  ...
  rc = extract_uint32_attr(
      attrs, XATTR_LAYOUT_STRIPE_UNIT, &layout->fl_stripe_unit);
  if (rc)
    return rc;
  // deal with stripe_count
  rc = extract_uint32_attr(
      attrs, XATTR_LAYOUT_STRIPE_COUNT, &layout->fl_stripe_count);
  if (rc)
    return rc;
  // deal with object_size
  rc = extract_uint32_attr(
      attrs, XATTR_LAYOUT_OBJECT_SIZE, &layout->fl_object_size);
  if (rc)
    return rc;
  // deal with size
  size_t ssize;
...
int
libradosstriper::RadosStriperImpl::openStripedObjectForRead(
    const std::string& soid,
    ceph_file_layout* layout,
    uint64_t* size,
    std::string* lockCookie)
{
  ...
  *lockCookie = getUUID();
  utime_t dur = utime_t();
  rados::cls::lock::lock(
      &op, RADOS_LOCK_NAME, ClsLockType::SHARED, *lockCookie, "Tag", "", dur, 0);
  std::string firstObjOid = getObjectId(soid, 0);
  int rc = m_ioCtx.operate(firstObjOid, &op);
  if (rc) {
  ...
  if (rc) {
    ...
    lderr(cct()) << "RadosStriperImpl::openStripedObjectForRead : "
                 << "could not load layout and size for " << soid
                 << " : rc = " << rc << dendl;
  }
  return rc;
}
...
int
libradosstriper::RadosStriperImpl::openStripedObjectForWrite(
    const std::string& soid,
    ceph_file_layout* layout,
    uint64_t* size,
    std::string* lockCookie,
    bool isFileSizeAbsolute)
{
  ...
  *lockCookie = getUUID();
  ...
  if (rc) {
    if (rc == -ENOENT) {
      ...
      int rc = createAndOpenStripedObject(
          soid, layout, *size, lockCookie, isFileSizeAbsolute);
  ...
  *size = curSize;
  ...
  if (rc) {
    unlockObject(soid, *lockCookie);
    lderr(cct()) << "RadosStriperImpl::openStripedObjectForWrite : "
                 << "could not set new size for " << soid << " : rc = " << rc
                 << dendl;
  }
  return rc;
}
...
int
libradosstriper::RadosStriperImpl::createAndOpenStripedObject(
    const std::string& soid,
    ceph_file_layout* layout,
    uint64_t size,
    std::string* lockCookie,
    bool isFileSizeAbsolute)
{
  ...
  return openStripedObjectForWrite(
      soid, layout, &fileSize, lockCookie, isFileSizeAbsolute);
}

static void
striper_truncate_aio_req_complete(rados_striper_multi_completion_t c, void* arg)
{
  auto cdata = ceph::ref_t<TruncateCompletionData>(
      static_cast<TruncateCompletionData*>(arg), false);
  libradosstriper::MultiAioCompletionImpl* comp =
      reinterpret_cast<libradosstriper::MultiAioCompletionImpl*>(c);
  if (0 == comp->rval) {
...
int
libradosstriper::RadosStriperImpl::truncate(
    const std::string& soid,
    uint64_t original_size,
    uint64_t size,
    ceph_file_layout& layout)
{
  ...
  libradosstriper::MultiAioCompletionImplPtr multi_completion{
      new libradosstriper::MultiAioCompletionImpl, false};
  multi_completion->set_complete_callback(
      cdata->get() /* create ref! */, striper_truncate_aio_req_complete);
...
int
libradosstriper::RadosStriperImpl::aio_truncate(
    const std::string& soid,
    libradosstriper::MultiAioCompletionImplPtr multi_completion,
    uint64_t original_size,
    uint64_t size,
    ceph_file_layout& layout)
{
  ...
  uint64_t trunc_objectsetno = size / layout.fl_object_size /
                               layout.fl_stripe_count;
  uint64_t last_objectsetno = original_size / layout.fl_object_size /
                              layout.fl_stripe_count;
  bool exists = false;
  for (int64_t objectno = (last_objectsetno + 1) * layout.fl_stripe_count - 1;
    ...
    if (!exists) {
      ...
      uint64_t set_start_off = nb_full_object_set * layout.fl_object_size *
                               layout.fl_stripe_count;
      uint64_t object_start_off = set_start_off +
                                  object_index_in_set * layout.fl_stripe_unit;
  ...
  for (int64_t objectno = ((trunc_objectsetno + 1) * layout.fl_stripe_count) - 1;
    ...
    // if no object existed so far, check object existence
    if (!exists) {
      uint64_t object_start_off =
          ((objectno / layout.fl_stripe_count) * layout.fl_object_size) +
    ...
    if (exists) {
      ...
      uint64_t new_object_size =
          Striper::object_truncate_size(cct(), &l, objectno, size);
      int rc;
...
int
libradosstriper::RadosStriperImpl::grow(
    const std::string& soid,
    uint64_t original_size,
    uint64_t size,
    ceph_file_layout& layout)
