...
#undef dout_prefix
#define dout_prefix \
  *_dout << "librbd::AsioEngine: " << this << " " << __func__ << ": "
  ...
  m_context_wq(std::make_unique<asio::ContextWQ>(m_cct, m_io_context))
{
  ...
  if (rbd_threads > rados_threads) {
    ...
    m_cct->_conf.set_val_or_die(
        "librados_thread_count", std::to_string(rbd_threads));
...
void
AsioEngine::dispatch(Context* ctx, int r)
{
  dispatch([ctx, r]() { ctx->complete(r); });
}

void
AsioEngine::post(Context* ctx, int r)
{
