...
#undef dout_prefix
#define dout_prefix \
  *_dout << "librbd::ExclusiveLock: " << this << " " << __func__
...
template <typename I>
bool
ExclusiveLock<I>::accept_request(OperationRequestType request_type) const
{
  int ret_val; // ignored
...
template <typename I>
bool
ExclusiveLock<I>::accept_request(
    ...
    int* ret_val) const
{
  ...
  ldout(m_image_ctx.cct, 20)
      << "=" << accept_request << " ret_val=" << *ret_val
...
template <typename I>
bool
ExclusiveLock<I>::accept_ops() const
{
  std::lock_guard locker{ML<I>::m_lock};
...
template <typename I>
bool
ExclusiveLock<I>::accept_ops(const ceph::mutex& lock) const
{
  return (
      !ML<I>::is_state_shutdown() &&
      (ML<I>::is_state_locked() || ML<I>::is_state_post_acquiring()));
}

template <typename I>
void
ExclusiveLock<I>::set_require_lock(
    bool init_shutdown,
...
template <typename I>
void
ExclusiveLock<I>::unset_require_lock(io::Direction direction)
{
...
template <typename I>
void
ExclusiveLock<I>::block_requests(int r)
{
  std::lock_guard locker{ML<I>::m_lock};
...
template <typename I>
void
ExclusiveLock<I>::unblock_requests()
{
  std::lock_guard locker{ML<I>::m_lock};
...
template <typename I>
int
ExclusiveLock<I>::get_unlocked_op_error() const
{
...
template <typename I>
void
ExclusiveLock<I>::init(uint64_t features, Context* on_init)
{
  ...
  if (m_image_ctx.clone_copy_on_read ||
      (features & RBD_FEATURE_JOURNALING) != 0 || pwl_enabled) {
...
template <typename I>
void
ExclusiveLock<I>::shut_down(Context* on_shut_down)
{
...
template <typename I>
void
ExclusiveLock<I>::handle_peer_notification(int r)
{
  std::lock_guard locker{ML<I>::m_lock};
...
template <typename I>
Context*
ExclusiveLock<I>::start_op(int* ret_val)
{
  ceph_assert(ceph_mutex_is_locked(m_image_ctx.owner_lock));
  std::lock_guard locker{ML<I>::m_lock};
...
template <typename I>
void
ExclusiveLock<I>::shutdown_handler(int r, Context* on_finish)
{
  ldout(m_image_ctx.cct, 10) << dendl;
...
template <typename I>
void
ExclusiveLock<I>::pre_acquire_lock_handler(Context* on_finish)
{
  ldout(m_image_ctx.cct, 10) << dendl;
  ...
  PreAcquireRequest<I>* req =
      PreAcquireRequest<I>::create(m_image_ctx, on_finish);
...
template <typename I>
void
ExclusiveLock<I>::post_acquire_lock_handler(int r, Context* on_finish)
{
  ...
  PostAcquireRequest<I>* req = PostAcquireRequest<I>::create(
      m_image_ctx,
...
template <typename I>
void
ExclusiveLock<I>::handle_post_acquiring_lock(int r)
{
  ldout(m_image_ctx.cct, 10) << dendl;
...
template <typename I>
void
ExclusiveLock<I>::handle_post_acquired_lock(int r)
{
  ldout(m_image_ctx.cct, 10) << ": r=" << r << dendl;
...
template <typename I>
void
ExclusiveLock<I>::pre_release_lock_handler(bool shutting_down, Context* on_finish)
{
  ldout(m_image_ctx.cct, 10) << dendl;
  std::lock_guard locker{ML<I>::m_lock};
    ...
    Context* on_finish)
{
  ldout(m_image_ctx.cct, 10)
      << ": r=" << r << " shutting_down=" << shutting_down << dendl;
  if (!shutting_down) {
    {
      std::lock_guard locker{ML<I>::m_lock};
      ceph_assert(
          ML<I>::is_state_pre_releasing() || ML<I>::is_state_releasing());
    }
...
template <typename I>
void
ExclusiveLock<I>::post_reacquire_lock_handler(int r, Context* on_finish)
{
  ldout(m_image_ctx.cct, 10) << dendl;
  if (r >= 0) {
    m_image_ctx.image_watcher->notify_acquired_lock();
