...
librados::IoCtx
duplicate_io_ctx(librados::IoCtx& io_ctx)
{
...
ImageCtx::ImageCtx(
  ...
  image_watcher(NULL),
  journal(NULL),
  owner_lock(ceph::make_shared_mutex(
      util::unique_lock_name("librbd::ImageCtx::owner_lock", this))),
  image_lock(ceph::make_shared_mutex(
      util::unique_lock_name("librbd::ImageCtx::image_lock", this))),
  timestamp_lock(ceph::make_shared_mutex(
      util::unique_lock_name("librbd::ImageCtx::timestamp_lock", this))),
  async_ops_lock(ceph::make_mutex(
      util::unique_lock_name("librbd::ImageCtx::async_ops_lock", this))),
  copyup_list_lock(ceph::make_mutex(
      util::unique_lock_name("librbd::ImageCtx::copyup_list_lock", this))),
  extra_read_flags(0),
  old_format(false),
  order(0),
  size(0),
...
ImageCtx::init()
{
  ...
  string pname = string("librbd-") + id + string("-") + md_ctx.get_pool_name() +
                 string("-") + name;
  if (!snap_name.empty()) {
    pname += "-";
    pname += snap_name;
...
void
ImageCtx::init_layout(int64_t pool_id)
{
  ...
  alignments.push_back(
      stripe_count << order); // object set (in file striping terminology)
  ...
  ldout(cct, 10) << "init_layout stripe_unit " << stripe_unit
                 ...
                 << layout.object_size << " prefix " << object_prefix
                 << " format " << format_string << dendl;
}

void
ImageCtx::perf_start(string name)
{
  ...
  plb.add_u64_counter(
      l_librbd_rd_bytes, "rd_bytes", "Data size in reads", "rb", perf_prio,
      unit_t(UNIT_BYTES));
  plb.add_time_avg(
      l_librbd_rd_latency, "rd_latency", "Latency of reads", "rl", perf_prio);
  plb.add_u64_counter(l_librbd_wr, "wr", "Writes", "w", perf_prio);
  plb.add_u64_counter(
      l_librbd_wr_bytes, "wr_bytes", "Written data", "wb", perf_prio,
      unit_t(UNIT_BYTES));
  plb.add_time_avg(
      l_librbd_wr_latency, "wr_latency", "Write latency", "wl", perf_prio);
  plb.add_u64_counter(l_librbd_discard, "discard", "Discards");
  plb.add_u64_counter(
      l_librbd_discard_bytes, "discard_bytes", "Discarded data", NULL, 0,
      unit_t(UNIT_BYTES));
  plb.add_time_avg(
      l_librbd_discard_latency, "discard_latency", "Discard latency");
  plb.add_u64_counter(l_librbd_flush, "flush", "Flushes");
  plb.add_time_avg(
      l_librbd_flush_latency, "flush_latency", "Latency of flushes");
  plb.add_u64_counter(l_librbd_ws, "ws", "WriteSames");
  plb.add_u64_counter(
      l_librbd_ws_bytes, "ws_bytes", "WriteSame data", NULL, 0,
      unit_t(UNIT_BYTES));
  plb.add_time_avg(l_librbd_ws_latency, "ws_latency", "WriteSame latency");
  plb.add_u64_counter(l_librbd_cmp, "cmp", "CompareAndWrites");
  plb.add_u64_counter(
      l_librbd_cmp_bytes, "cmp_bytes", "Data size in cmps", NULL, 0,
      unit_t(UNIT_BYTES));
  ...
  plb.add_u64_counter(
      l_librbd_readahead_bytes, "readahead_bytes", "Data size in read ahead",
      NULL, 0, unit_t(UNIT_BYTES));
  plb.add_u64_counter(
      l_librbd_invalidate_cache, "invalidate_cache", "Cache invalidates");

  plb.add_time(
      l_librbd_opened_time, "opened_time", "Opened time", "ots", perf_prio);
  plb.add_time(
      l_librbd_lock_acquired_time, "lock_acquired_time", "Lock acquired time",
      "lats", perf_prio);
...
void
ImageCtx::set_read_flag(unsigned flag)
{
  extra_read_flags |= flag;
}

int
ImageCtx::get_read_flags(snap_t snap_id)
{
...
int
ImageCtx::snap_set(uint64_t in_snap_id)
{
...
const SnapInfo*
ImageCtx::get_snap_info(snap_t in_snap_id) const
{
  ceph_assert(ceph_mutex_is_locked(image_lock));
...
int
ImageCtx::get_snap_name(snap_t in_snap_id, string* out_snap_name) const
{
  ceph_assert(ceph_mutex_is_locked(image_lock));
...
uint64_t
ImageCtx::get_current_size() const
{
  ceph_assert(ceph_mutex_is_locked(image_lock));
  return size;
}
...
uint64_t
ImageCtx::get_object_size() const
{
  return 1ull << order;
}

string
ImageCtx::get_object_name(uint64_t num) const
{
  return util::data_object_name(this, num);
}

uint64_t
ImageCtx::get_stripe_unit() const
{
  return stripe_unit;
}

uint64_t
ImageCtx::get_stripe_count() const
{
  return stripe_count;
}

uint64_t
ImageCtx::get_stripe_period() const
{
...
void
ImageCtx::set_access_timestamp(utime_t at)
{
...
void
ImageCtx::set_modify_timestamp(utime_t mt)
{
...
int
ImageCtx::is_snap_protected(snap_t in_snap_id, bool* is_protected) const
{
  ceph_assert(ceph_mutex_is_locked(image_lock));
  const SnapInfo* info = get_snap_info(in_snap_id);
  if (info) {
...
int
ImageCtx::is_snap_unprotected(snap_t in_snap_id, bool* is_unprotected) const
{
  ceph_assert(ceph_mutex_is_locked(image_lock));
  const SnapInfo* info = get_snap_info(in_snap_id);
  if (info) {
    ...
    utime_t timestamp)
{
  ...
  snaps.push_back(id);
  SnapInfo info(
      in_snap_name, in_snap_namespace, in_size, parent, protection_status,
      flags, timestamp);
...
uint64_t
ImageCtx::get_image_size(snap_t in_snap_id) const
{
  ceph_assert(ceph_mutex_is_locked(image_lock));
...
uint64_t
ImageCtx::get_area_size(io::ImageArea area) const
{
...
uint64_t
ImageCtx::get_object_count(snap_t in_snap_id) const
{
  ceph_assert(ceph_mutex_is_locked(image_lock));
...
bool
ImageCtx::test_features(uint64_t features) const
{
...
bool
ImageCtx::test_op_features(uint64_t in_op_features) const
{
  std::shared_lock l{image_lock};
...
bool
ImageCtx::test_op_features(
    uint64_t in_op_features,
    const ceph::shared_mutex& in_image_lock) const
{
  ceph_assert(ceph_mutex_is_locked(image_lock));
...
int
ImageCtx::get_flags(librados::snap_t _snap_id, uint64_t* _flags) const
{
  ceph_assert(ceph_mutex_is_locked(image_lock));
...
int
ImageCtx::update_flags(snap_t in_snap_id, uint64_t flag, bool enabled)
{
  ceph_assert(ceph_mutex_is_wlocked(image_lock));
  uint64_t* _flags;
  if (in_snap_id == CEPH_NOSNAP) {
...
ImageCtx::get_parent_info(snap_t in_snap_id) const
{
  ceph_assert(ceph_mutex_is_locked(image_lock));
  if (in_snap_id == CEPH_NOSNAP)
    return &parent_md;
...
int64_t
ImageCtx::get_parent_pool_id(snap_t in_snap_id) const
{
...
string
ImageCtx::get_parent_image_id(snap_t in_snap_id) const
{
  const auto info = get_parent_info(in_snap_id);
  if (info)
...
uint64_t
ImageCtx::get_parent_snap_id(snap_t in_snap_id) const
{
  const auto info = get_parent_info(in_snap_id);
  if (info)
...
int
ImageCtx::get_parent_overlap(snap_t in_snap_id, uint64_t* raw_overlap) const
{
  const auto info = get_parent_info(in_snap_id);
  if (info) {
...
std::pair<uint64_t, io::ImageArea>
ImageCtx::reduce_parent_overlap(uint64_t raw_overlap, bool migration_write) const
{
  ceph_assert(ceph_mutex_is_locked(image_lock));
  if (migration_write) {
    ...
    bool migration_write) const
{
  ...
  if (area == overlap.second) {
    ...
    ceph_assert(
        area == io::ImageArea::CRYPTO_HEADER &&
  ...
  ldout(cct, 10) << __func__ << ": overlap=" << overlap.first << "/"
                 << overlap.second << " got overlap_bytes=" << overlap_bytes
                 << " at " << image_extents << dendl;
  return overlap_bytes;
}

void
ImageCtx::register_watch(Context* on_finish)
{
  ceph_assert(image_watcher != NULL);
...
void
ImageCtx::cancel_async_requests()
{
  C_SaferCond ctx;
...
void
ImageCtx::cancel_async_requests(Context* on_finish)
{
  {
...
ImageCtx::apply_metadata(
    const std::map<std::string, bufferlist>& meta,
    bool thread_safe)
{
...
#undef ASSIGN_OPTION
  ...
  if (r == 0 && require_osd_release >= CEPH_RELEASE_OCTOPUS) {
    read_flags = 0;
    auto read_policy =
        config.get_val<std::string>("rbd_read_from_replica_policy");
...
uint64_t
ImageCtx::get_data_offset() const
{
...
void
ImageCtx::set_image_name(const std::string& image_name)
{
...
void
ImageCtx::set_exclusive_lock_policy(exclusive_lock::Policy* policy)
{
...
void
ImageCtx::set_journal_policy(journal::Policy* policy)
{
  ceph_assert(ceph_mutex_is_wlocked(image_lock));
  ceph_assert(policy != nullptr);
  delete journal_policy;
...
void
ImageCtx::rebuild_data_io_context()
{
