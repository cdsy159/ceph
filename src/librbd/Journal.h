...
public:
  ...
  uint64_t append_write_event(
      const io::Extents& image_extents,
      const bufferlist& bl,
      bool flush_entry);
  uint64_t append_write_same_event(
      const io::Extents& image_extents,
      const bufferlist& bl,
      bool flush_entry);
  uint64_t append_compare_and_write_event(
      uint64_t offset,
      size_t length,
      const bufferlist& cmp_bl,
      const bufferlist& write_bl,
      bool flush_entry);
  uint64_t append_discard_event(
      const io::Extents& image_extents,
      uint32_t discard_granularity_bytes,
      bool flush_entry);
  uint64_t append_io_event(
  ...
  void
  commit_io_event_extent(uint64_t tid, uint64_t offset, uint64_t length, int r);

  void append_op_event(
      uint64_t op_tid,
      journal::EventEntry&& event_entry,
      Context* on_safe);
...
private:
  ...
  struct C_OpEventSafe : public Context {
    ...
    finish(int r) override
    {
      journal->handle_op_event_safe(
          r, tid, op_start_future, op_finish_future, on_safe);
    }
  };
  ...
  void add_write_event_entries(
      uint64_t offset,
      size_t length,
      const bufferlist& bl,
      uint64_t buffer_offset,
      Bufferlists* bufferlists);
  uint64_t append_io_events(
  ...
  void handle_start_external_replay(
      int r,
      journal::Replay<ImageCtxT>** journal_replay,
      Context* on_finish);
