...
template <typename I>
MirroringWatcher<I>::MirroringWatcher(
...
template <typename I>
int
MirroringWatcher<I>::notify_mode_updated(
    librados::IoCtx& io_ctx,
    ...
    const std::string& global_image_id)
{
  ...
  notify_image_updated(
      io_ctx, mirror_image_state, image_id, global_image_id, &ctx);
  return ctx.wait();
}
    ...
    Context* on_finish)
{
  ...
  encode(
      NotifyMessage{
          ImageUpdatedPayload{mirror_image_state, image_id, global_image_id}},
      bl);
...
template <typename I>
void
MirroringWatcher<I>::handle_notify(
    uint64_t notify_id,
    uint64_t handle,
    ...
    bufferlist& bl)
{
  ...
  ldout(cct, 15) << ": notify_id=" << notify_id << ", " << "handle=" << handle
                 << dendl;
  ...
  std::visit(
      watcher::util::HandlePayloadVisitor<MirroringWatcher<I>>(
          this, notify_id, handle),
      notify_message.payload);
}

template <typename I>
bool
MirroringWatcher<I>::handle_payload(
    const ModeUpdatedPayload& payload,
...
bool
MirroringWatcher<I>::handle_payload(
    const ImageUpdatedPayload& payload,
    Context* on_notify_ack)
{
  CephContext* cct = this->m_cct;
  ldout(cct, 20) << ": image state updated" << dendl;
  handle_image_updated(
      payload.mirror_image_state, payload.image_id, payload.global_image_id);
  return true;
}
