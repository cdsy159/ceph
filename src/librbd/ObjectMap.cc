...
#undef dout_prefix
#define dout_prefix \
  *_dout << "librbd::ObjectMap: " << this << " " << __func__ << ": "
...
ObjectMap<I>::ObjectMap(I& image_ctx, uint64_t snap_id) :
  ...
  m_lock(ceph::make_shared_mutex(
      util::unique_lock_name("librbd::ObjectMap::lock", this))),
...
template <typename I>
std::string
ObjectMap<I>::object_map_name(const std::string& image_id, uint64_t snap_id)
{
...
template <typename I>
bool
ObjectMap<I>::is_compatible(const file_layout_t& layout, uint64_t size)
{
...
template <typename I>
uint8_t
ObjectMap<I>::operator[](uint64_t object_no) const
{
...
template <typename I>
bool
ObjectMap<I>::object_may_exist(uint64_t object_no) const
{
  ...
  int r = m_image_ctx.test_flags(
      m_image_ctx.snap_id, RBD_FLAG_OBJECT_MAP_INVALID, m_image_ctx.image_lock,
      &flags_set);
  if (r < 0 || flags_set) {
    return true;
  }
  ...
  ldout(m_image_ctx.cct, 20)
      << "object_no=" << object_no << " r=" << exists << dendl;
  return exists;
}

template <typename I>
bool
ObjectMap<I>::update_required(
    const ceph::BitVector<2>::Iterator& it,
    uint8_t new_state)
{
...
template <typename I>
void
ObjectMap<I>::open(Context* on_finish)
{
...
template <typename I>
void
ObjectMap<I>::close(Context* on_finish)
{
  Context* ctx = create_context_callback<Context>(on_finish, this);
...
template <typename I>
bool
ObjectMap<I>::set_object_map(ceph::BitVector<2>& target_object_map)
{
  ceph_assert(ceph_mutex_is_locked(m_image_ctx.owner_lock));
  ceph_assert(ceph_mutex_is_locked(m_image_ctx.image_lock));
  ceph_assert(m_image_ctx.test_features(
      RBD_FEATURE_OBJECT_MAP, m_image_ctx.image_lock));
...
template <typename I>
void
ObjectMap<I>::rollback(uint64_t snap_id, Context* on_finish)
{
  ceph_assert(ceph_mutex_is_locked(m_image_ctx.image_lock));
...
template <typename I>
void
ObjectMap<I>::snapshot_add(uint64_t snap_id, Context* on_finish)
{
  ceph_assert(ceph_mutex_is_locked(m_image_ctx.image_lock));
  ...
  object_map::SnapshotCreateRequest* req = new object_map::SnapshotCreateRequest(
      m_image_ctx, &m_lock, &m_object_map, snap_id, ctx);
  req->send();
}

template <typename I>
void
ObjectMap<I>::snapshot_remove(uint64_t snap_id, Context* on_finish)
{
  ...
  object_map::SnapshotRemoveRequest* req = new object_map::SnapshotRemoveRequest(
      m_image_ctx, &m_lock, &m_object_map, snap_id, ctx);
  req->send();
}
...
template <typename I>
void
ObjectMap<I>::aio_save(Context* on_finish)
{
  ceph_assert(ceph_mutex_is_locked(m_image_ctx.owner_lock));
  ceph_assert(ceph_mutex_is_locked(m_image_ctx.image_lock));
  ceph_assert(m_image_ctx.test_features(
...
template <typename I>
void
ObjectMap<I>::detained_aio_update(UpdateOperation&& op)
{
  ...
  BlockGuardCell* cell;
  int r = m_update_guard->detain(
      {op.start_object_no, op.end_object_no}, &op, &cell);
  if (r < 0) {
  ...
  aio_update(
      CEPH_NOSNAP, op.start_object_no, op.end_object_no, op.new_state,
