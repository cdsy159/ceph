...
public:
  static ObjectMap*
  create(ImageCtxT& image_ctx, uint64_t snap_id)
  {
  ...
  static int aio_remove(
      librados::IoCtx& io_ctx,
      const std::string& image_id,
      librados::AioCompletion* c);
  static std::string object_map_name(
      const std::string& image_id,
      uint64_t snap_id);
  ...
  auto
  with_object_map(F&& f, Args&&... args) const
  {
    std::shared_lock locker(m_lock);
  ...
  aio_update(
      ...
      T* callback_object)
  {
    return aio_update<T, MF>(
        snap_id, start_object_no, start_object_no + 1, new_state, current_state,
        parent_trace, ignore_enoent, callback_object);
  }
      ...
      T* callback_object)
  {
    ...
    if (snap_id == CEPH_NOSNAP) {
      ...
      UpdateOperation update_operation(
          start_object_no, end_object_no, new_state, current_state,
          parent_trace, ignore_enoent,
...
private:
  ...
  void handle_detained_aio_update(
      BlockGuardCell* cell,
      int r,
      Context* on_finish);
