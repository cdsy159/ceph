...
template <typename I>
void
TrashWatcher<I>::notify_image_removed(
    librados::IoCtx& io_ctx,
    const std::string& image_id,
    Context* on_finish)
{
    ...
    bufferlist& bl)
{
  ...
  std::visit(
      watcher::util::HandlePayloadVisitor<TrashWatcher<I>>(
...
template <typename I>
bool
TrashWatcher<I>::handle_payload(
    const ImageAddedPayload& payload,
...
bool
TrashWatcher<I>::handle_payload(
    const ImageRemovedPayload& payload,
    Context* on_notify_ack)
{
  CephContext* cct = this->m_cct;
  ldout(cct, 20) << dendl;
