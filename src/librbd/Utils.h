...
void
rados_state_callback(rados_completion_t c, void* arg)
{
  T* obj = reinterpret_cast<T*>(arg);
  int r = rados_aio_get_return_value(c);
...
public:
  C_RefCallbackAdapter(T* obj, RefCountedPtr refptr) :
    refptr(std::move(refptr)), on_finish(new C_CallbackAdapter<T, MF>(obj))
  {}
...
template <typename I>
std::string
data_object_name(I* image_ctx, uint64_t object_no)
{
  char buf[RBD_MAX_OBJ_NAME_SIZE];
  size_t length =
      snprintf(buf, RBD_MAX_OBJ_NAME_SIZE, image_ctx->format_string, object_no);
...
template <typename T, void (T::*MF)(int) = &T::complete>
Context*
create_context_callback(T* obj, RefCountedPtr refptr)
{
...
template <typename I>
Context*
create_async_context_callback(I& image_ctx, Context* on_finish)
{
...
template <typename WQ>
Context*
create_async_context_callback(WQ* work_queue, Context* on_finish)
{
  // use async callback to acquire a clean lock context
...
inline ImageCtx*
get_image_ctx(ImageCtx* image_ctx)
{
  return image_ctx;
}
...
bool is_metadata_config_override(
...
int snap_create_flags_api_to_internal(
    CephContext* cct,
    uint32_t api_flags,
