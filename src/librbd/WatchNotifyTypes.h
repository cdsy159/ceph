...
struct AsyncRequestId {
  ...
  inline bool
  operator<(const AsyncRequestId& rhs) const
  {
...
struct AcquiredLockPayload : public Payload {
  ...
  NotifyOp
  get_notify_op() const override
  {
    return NOTIFY_OP_ACQUIRED_LOCK;
  }

  bool
  check_for_refresh() const override
  {
    return false;
  }
...
protected:
  ...
  SnapPayloadBase(
    ...
    AsyncRequestPayloadBase(id), snap_namespace(snap_namespace), snap_name(name)
  {}
};
