...
#undef dout_prefix
#define dout_prefix \
  *_dout << "librbd::Watcher::C_NotifyAck " << this << " " << __func__ << ": "

Watcher::C_NotifyAck::C_NotifyAck(
    Watcher* watcher,
    uint64_t notify_id,
    uint64_t handle) :
  watcher(watcher), cct(watcher->m_cct), notify_id(notify_id), handle(handle)
{
  ldout(cct, 10) << "id=" << notify_id << ", " << "handle=" << handle << dendl;
}

void
Watcher::C_NotifyAck::finish(int r)
{
...
#undef dout_prefix
#define dout_prefix \
  *_dout << "librbd::Watcher: " << this << " " << __func__ << ": "
...
Watcher::register_watch(Context* on_finish)
{
  ...
  librados::AioCompletion* aio_comp =
      create_rados_callback(new C_RegisterWatch(this, on_finish));
...
Watcher::unregister_watch(Context* on_finish)
{
...

  {
    ...
    if (m_watch_state != WATCH_STATE_IDLE) {
      ...
      librados::AioCompletion* aio_comp =
          create_rados_callback(new C_UnwatchAndFlush(m_ioctx, on_finish));
...
bool
Watcher::notifications_blocked() const
{
...
void
Watcher::block_notifies(Context* on_finish)
{
  {
...
void
Watcher::flush(Context* on_finish)
{
...
void
Watcher::set_oid(const string& oid)
{
  std::unique_lock watch_locker{m_watch_lock};
  ceph_assert(is_unregistered(m_watch_lock));
...
void
Watcher::acknowledge_notify(uint64_t notify_id, uint64_t handle, bufferlist& out)
{
...
Watcher::rewatch()
{
  ...
  Context* unregister_watch_ctx = nullptr;
  {
    ...
    if (m_unregister_watch_ctx != nullptr) {
      ...
      m_watch_error = false;
      auto ctx =
          create_context_callback<Watcher, &Watcher::handle_rewatch>(this);
      auto req = RewatchRequest::create(
          m_ioctx, m_oid, m_watch_lock, &m_watch_ctx, &m_watch_handle, ctx);
      req->send();
      return;
    }
  }
...
Watcher::handle_rewatch(int r)
{
  ...
  auto ctx =
      create_context_callback<Watcher, &Watcher::handle_rewatch_callback>(this);
