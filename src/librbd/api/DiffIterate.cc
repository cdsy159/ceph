...
public:
  ...
  send()
  {
    ...
    auto req = io::ImageDispatchSpec::create_list_snaps(
        ...
        {{m_image_offset, m_image_length}}, io::ImageArea::DATA,
...
protected:
  ...
  finish(int r) override
  {
    ...
    Diffs diffs;
    ldout(cct, 20) << "image extent " << m_image_offset << "~" << m_image_length
                   << ": list_snaps complete" << dendl;

    compute_diffs(&diffs);
    for (Diffs::const_iterator d = diffs.begin(); d != diffs.end(); ++d) {
      r = m_diff_context.callback(
          d->get<0>(), d->get<1>(), d->get<2>(), m_diff_context.callback_arg);
      if (r < 0) {
        break;
      }
    }
...
template <typename I>
bool
should_try_acquire_lock(I* image_ctx)
{
...
int
simple_diff_cb(uint64_t off, size_t len, int exists, void* arg)
{
...
DiffIterate<I>::diff_iterate(
    ...
    void* arg)
{
  ldout(ictx->cct, 10) << "from_snap_id=" << from_snap_id << ", off=" << off
                       << ", len=" << len
  ...
  C_SaferCond flush_ctx;
  {
    ...
    auto aio_comp = io::AioCompletion::create_and_start(
        &flush_ctx, ictx, io::AIO_TYPE_FLUSH);
    auto req = io::ImageDispatchSpec::create_flush(
        *ictx, io::IMAGE_DISPATCH_LAYER_INTERNAL_START, aio_comp,
...

  {
    ...
    if (from_snap_id == 0 && whole_object && should_try_acquire_lock(ictx)) {
  ...
  DiffIterate command(
      *ictx, from_snap_id, off, len, include_parent, whole_object, cb, arg);
  r = command.execute();
  return r;
}
...
DiffIterate<I>::calc_object_diff_range()
{
  ...
  if (first_period_off != last_period_off) {
    ...
    io::util::area_to_object_extents(
        &m_image_ctx, m_offset, first_period_off + period - m_offset,
        io::ImageArea::DATA, 0, &object_extents);
    io::util::area_to_object_extents(
        &m_image_ctx, last_period_off, m_offset + m_length - last_period_off,
        io::ImageArea::DATA, 0, &object_extents);
  } else {
    io::util::area_to_object_extents(
        &m_image_ctx, m_offset, m_length, io::ImageArea::DATA, 0,
        &object_extents);
  }
...
DiffIterate<I>::execute()
{
  ...
  if (m_whole_object) {
    ...
    auto req = object_map::DiffRequest<I>::create(
        &m_image_ctx, from_snap_id, end_snap_id, start_object_no, end_object_no,
    ...
    if (r < 0) {
      ...
      if (m_include_parent && from_snap_id == 0) {
        ...
        if (m_image_ctx.prune_parent_extents(
                parent_extents, io::ImageArea::DATA, raw_overlap, false) > 0) {
          ldout(cct, 10) << " first getting parent diff" << dendl;
          DiffIterate diff_parent(
              *m_image_ctx.parent, 0, parent_extents[0].first,
              parent_extents[0].second, true, true, &simple_diff_cb,
              &parent_diff);
  ...
  ldout(cct, 5) << "diff_iterate from " << from_snap_id << " to " << end_snap_id
                << " size from " << from_size << " to " << end_size << dendl;
  DiffContext diff_context(
      m_image_ctx, m_callback, m_callback_arg, m_whole_object, m_include_parent,
      from_snap_id, end_snap_id);
  ...
  while (left > 0) {
    ...
    if (fast_diff_enabled) {
      ...
      striper::LightweightObjectExtents object_extents;
      io::util::area_to_object_extents(
          &m_image_ctx, off, read_len, io::ImageArea::DATA, 0, &object_extents);
      ...
      for (const auto& oe : object_extents) {
        ceph_assert(
            oe.object_no >= start_object_no && oe.object_no < end_object_no);
        ...
        if (diff_state == object_map::DIFF_STATE_HOLE && from_snap_id == 0 &&
            !parent_diff.empty()) {
          ...
          for (const auto& be : oe.buffer_extents) {
            ...
            for (auto e = o.begin(); e != o.end(); ++e) {
              aggregate_sparse_extents.insert(
                  e.get_start(), e.get_len(),
                  {io::SPARSE_EXTENT_STATE_DATA, e.get_len()});
            }
          }
        } else if (
            diff_state == object_map::DIFF_STATE_HOLE_UPDATED ||
          ...
          for (const auto& be : oe.buffer_extents) {
            aggregate_sparse_extents.insert(
                off + be.first, be.second, {state, be.second});
          }
        }
      }
      ...
      for (const auto& se : aggregate_sparse_extents) {
        ...
        r = m_callback(
            se.get_off(), se.get_len(),
            se.get_val().state == io::SPARSE_EXTENT_STATE_DATA, m_callback_arg);
        if (r < 0) {
          return r;
        }
      }
      ...
      auto diff_object =
          new C_DiffObject<I>(m_image_ctx, diff_context, off, read_len);
      diff_object->send();
