...
#undef dout_prefix
#define dout_prefix                                                      \
  *_dout << "librbd::cache::ObjectCacherObjectDispatch: " << this << " " \
         << __func__ << ": "
...
struct ObjectCacherObjectDispatch<I>::C_InvalidateCache : public Context {
  ...
  C_InvalidateCache(
      ...
      bool purge_on_error,
      Context* on_finish) :
    dispatcher(dispatcher), purge_on_error(purge_on_error), on_finish(on_finish)
  {}
  ...
  finish(int r) override
  {
    ...
    if (r == -EBLOCKLISTED) {
      ...
      lderr(cct) << "failed to invalidate cache (purging): " << cpp_strerror(r)
                 << dendl;
      dispatcher->m_object_cacher->purge_set(dispatcher->m_object_set);
    } else if (r != 0) {
    ...
    auto unclean =
        dispatcher->m_object_cacher->release_set(dispatcher->m_object_set);
    if (unclean == 0) {
      r = 0;
    } else {
      lderr(cct) << "could not release all objects from cache: " << unclean
                 << " bytes remain" << dendl;
      if (r == 0) {
        r = -EBUSY;
      }
...
ObjectCacherObjectDispatch<I>::ObjectCacherObjectDispatch(
  ...
  m_cache_lock(ceph::make_mutex(util::unique_lock_name(
      "librbd::cache::ObjectCacherObjectDispatch::cache_lock",
      this)))
{
...
ObjectCacherObjectDispatch<I>::init()
{
  ...
  auto target_dirty = m_image_ctx->config.template get_val<Option::size_t>(
  ...
  auto max_dirty_object = m_image_ctx->config.template get_val<uint64_t>(
      "rbd_cache_max_dirty_object");

  ldout(cct, 5) << "Initial cache settings:" << " size=" << cache_size
                << " num_objects=" << 10 << " max_dirty=" << init_max_dirty
                << " target_dirty=" << target_dirty
                << " max_dirty_age=" << max_dirty_age << dendl;

  m_object_cacher = new ObjectCacher(
      cct, m_image_ctx->perfcounter->get_name(), *m_writeback_handler,
      m_cache_lock, nullptr, nullptr, cache_size, 10, /* reset this in init */
      init_max_dirty, target_dirty, max_dirty_age, block_writes_upfront);

  // size object cache appropriately
  if (max_dirty_object == 0) {
    max_dirty_object = std::min<uint64_t>(
        2000,
        std::max<uint64_t>(10, cache_size / 100 / sizeof(ObjectCacher::Object)));
  }
  ldout(cct, 5) << " cache bytes " << cache_size << " -> about "
                << max_dirty_object << " objects" << dendl;
  m_object_cacher->set_max_objects(max_dirty_object);

  m_object_set =
      new ObjectCacher::ObjectSet(nullptr, m_image_ctx->data_ctx.get_id(), 0);
...
template <typename I>
void
ObjectCacherObjectDispatch<I>::shut_down(Context* on_finish)
{
  auto cct = m_image_ctx->cct;
  ldout(cct, 5) << dendl;
...
template <typename I>
bool
ObjectCacherObjectDispatch<I>::read(
    uint64_t object_no,
    ...
    Context* on_dispatched)
{
  ...
  on_dispatched =
      util::create_async_context_callback(*m_image_ctx, on_dispatched);

  // embed the RBD-internal read flags in the generic RADOS op_flags and
  op_flags =
      ((op_flags & ~ObjectCacherWriteback::READ_FLAGS_MASK) |
  ...
  auto rd =
      m_object_cacher->prepare_read(io_context->get_read_snap(), bl, op_flags);
  m_image_ctx->image_lock.unlock_shared();

  uint64_t off = 0;
  for (auto& read_extent : *extents) {
    ObjectExtent extent(
        data_object_name(m_image_ctx, object_no), object_no, read_extent.offset,
        read_extent.length, 0);
...
template <typename I>
bool
ObjectCacherObjectDispatch<I>::discard(
    uint64_t object_no,
    uint64_t object_off,
    uint64_t object_len,
    IOContext io_context,
    ...
    Context* on_dispatched)
{
  ...
  object_extents.emplace_back(
      data_object_name(m_image_ctx, object_no), object_no, object_off,
      object_len, 0);
  ...
  *dispatch_result = io::DISPATCH_RESULT_CONTINUE;
  ...
  // the discard request
  std::lock_guard locker{m_cache_lock};
  m_object_cacher->discard_writeback(
      m_object_set, object_extents, on_dispatched);
  return true;
}

template <typename I>
bool
ObjectCacherObjectDispatch<I>::write(
    uint64_t object_no,
    uint64_t object_off,
...
template <typename I>
bool
ObjectCacherObjectDispatch<I>::write_same(
    uint64_t object_no,
    uint64_t object_off,
    uint64_t object_len,
...
template <typename I>
bool
ObjectCacherObjectDispatch<I>::compare_and_write(
    uint64_t object_no,
    uint64_t object_off,
    ...
    Context* on_dispatched)
{
  ...
  *dispatch_result = io::DISPATCH_RESULT_CONTINUE;
  ...
  m_object_cacher->flush_set(
      m_object_set, object_extents, &trace, on_dispatched);
  return true;
}

template <typename I>
bool
ObjectCacherObjectDispatch<I>::flush(
...
template <typename I>
bool
ObjectCacherObjectDispatch<I>::invalidate_cache(Context* on_finish)
{
  auto cct = m_image_ctx->cct;
  ldout(cct, 5) << dendl;
...
template <typename I>
bool
ObjectCacherObjectDispatch<I>::reset_existence_cache(Context* on_finish)
{
  auto cct = m_image_ctx->cct;
  ldout(cct, 5) << dendl;
