...
#undef dout_prefix
#define dout_prefix                                                 \
  *_dout << "librbd::cache::WriteLogImageDispatch: " << this << " " \
         << __func__ << ": "
...
template <typename I>
void
WriteLogImageDispatch<I>::shut_down(Context* on_finish)
{
    ...
    Context* on_dispatched)
{
  ...
  *dispatch_result = io::DISPATCH_RESULT_COMPLETE;
  ...
  auto* req_comp =
      m_plugin_api.create_image_read_request(aio_comp, 0, image_extents);

  m_image_cache->read(
      std::move(image_extents), &req_comp->bl, op_flags, req_comp);
  return true;
}

template <typename I>
bool
WriteLogImageDispatch<I>::write(
    io::AioCompletion* aio_comp,
    io::Extents&& image_extents,
    bufferlist&& bl,
    int op_flags,
    ...
    Context* on_dispatched)
{
  ...
  *dispatch_result = io::DISPATCH_RESULT_COMPLETE;
  ...
  m_image_cache->write(
      std::move(image_extents), std::move(bl), op_flags, req_comp);
  return true;
}

template <typename I>
bool
WriteLogImageDispatch<I>::discard(
    io::AioCompletion* aio_comp,
    io::Extents&& image_extents,
    ...
    Context* on_dispatched)
{
  ...
  *dispatch_result = io::DISPATCH_RESULT_COMPLETE;
  ...
  for (auto& extent : image_extents) {
    io::C_AioRequest* req_comp = m_plugin_api.create_aio_request(aio_comp);
    m_image_cache->discard(
        extent.first, extent.second, discard_granularity_bytes, req_comp);
  }
  return true;
}
...
template <typename I>
bool
WriteLogImageDispatch<I>::write_same(
    io::AioCompletion* aio_comp,
    io::Extents&& image_extents,
    bufferlist&& bl,
    int op_flags,
    ...
    Context* on_dispatched)
{
  ...
  *dispatch_result = io::DISPATCH_RESULT_COMPLETE;
  ...
  for (auto& extent : image_extents) {
    io::C_AioRequest* req_comp = m_plugin_api.create_aio_request(aio_comp);
    m_image_cache->writesame(
        extent.first, extent.second, std::move(bl), op_flags, req_comp);
  }
  return true;
}
...
template <typename I>
bool
WriteLogImageDispatch<I>::compare_and_write(
    io::AioCompletion* aio_comp,
    io::Extents&& image_extents,
    bufferlist&& cmp_bl,
    bufferlist&& bl,
...
template <typename I>
bool
WriteLogImageDispatch<I>::list_snaps(
    io::AioCompletion* aio_comp,
    io::Extents&& image_extents,
    io::SnapIds&& snap_ids,
...
template <typename I>
bool
WriteLogImageDispatch<I>::preprocess_length(
    io::AioCompletion* aio_comp,
...
template <typename I>
bool
WriteLogImageDispatch<I>::invalidate_cache(Context* on_finish)
{
