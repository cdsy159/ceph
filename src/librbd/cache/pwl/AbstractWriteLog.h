...
public:
  ...
  void schedule_append(
      pwl::GenericLogOperationsVector& ops,
      C_BlockIORequestT* req = nullptr);
  void schedule_append(
      pwl::GenericLogOperationSharedPtr op,
      C_BlockIORequestT* req = nullptr);
  void flush_new_sync_point(
      C_FlushRequestT* flush_req,
      pwl::DeferredContexts& later);

  std::shared_ptr<pwl::SyncPoint>
  ...
  uint32_t
  get_free_log_entries()
  {
    return m_free_log_entries;
  }

  void add_into_log_map(
      pwl::GenericWriteLogEntries& log_entries,
...
private:
  ...
  pwl::C_FlushRequest<AbstractWriteLog<ImageCtxT>>* make_flush_req(
      Context* on_finish);
  void flush_new_sync_point_if_needed(
      C_FlushRequestT* flush_req,
      pwl::DeferredContexts& later);

  void alloc_and_dispatch_io_req(C_BlockIORequestT* write_req);
  void schedule_complete_op_log_entries(
      pwl::GenericLogOperations&& ops,
      const int r);
...
protected:
  ...
  std::atomic<uint64_t> m_bytes_allocated = {
   ...
   * used.
   */
  uint64_t m_first_free_entry =
      0; /* Entries from here to m_first_valid_entry-1 are free */
  uint64_t m_first_valid_entry =
  ...
  pwl::GenericLogOperations
      m_ops_to_flush; /* Write ops needing flush in local log */
  pwl::GenericLogOperations
  ...
  void update_entries(
      ...
      std::map<uint64_t, std::shared_ptr<pwl::SyncPointLogEntry>>&
          sync_point_entries,
  ...
  Context* construct_flush_entry(
      ...
      bool invalidating);
  void detain_flush_guard_request(
  ...
  virtual void schedule_append_ops(
      pwl::GenericLogOperations& ops,
      C_BlockIORequestT* req) = 0;
  virtual void remove_pool_file() = 0;
  virtual bool initialize_pool(
      Context* on_finish,
  ...
  virtual void
  release_ram(const std::shared_ptr<pwl::GenericLogEntry> log_entry)
  {}

  virtual void
  alloc_op_log_entries(pwl::GenericLogOperations& ops)
  {}
  ...
  virtual void
  schedule_flush_and_append(pwl::GenericLogOperationsVector& ops)
  {}
  ...
  virtual void
  reserve_cache(C_BlockIORequestT* req, bool& alloc_succeeds, bool& no_space)
  {}

  virtual void construct_flush_entries(
