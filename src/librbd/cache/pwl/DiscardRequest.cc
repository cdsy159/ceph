...
#undef dout_prefix
#define dout_prefix                                                          \
  *_dout << "librbd::cache::pwl:DiscardRequest: " << this << " " << __func__ \
         << ": "
...
template <typename I>
void
DiscardRequest<I>::delete_image_cache_file()
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << dendl;

  m_cache_state =
      ImageCacheState<I>::get_image_cache_state(&m_image_ctx, m_plugin_api);
  if (!m_cache_state) {
    remove_feature_bit();
    return;
  }
...
template <typename I>
void
DiscardRequest<I>::remove_image_cache_state()
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << dendl;
...
template <typename I>
void
DiscardRequest<I>::handle_remove_image_cache_state(int r)
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << dendl;
...
template <typename I>
void
DiscardRequest<I>::remove_feature_bit()
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << dendl;
...
template <typename I>
void
DiscardRequest<I>::handle_remove_feature_bit(int r)
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << dendl;
