...
#undef dout_prefix
#define dout_prefix \
  *_dout << "librbd::cache::pwl::ImageCacheState: " << __func__ << ": "
...
template <typename I>
void
ImageCacheState<I>::init_from_config()
{
...
template <typename I>
bool
ImageCacheState<I>::init_from_metadata(json_spirit::mValue& json_root)
{
  ldout(m_image_ctx->cct, 20) << dendl;
  ...
  try {
    ...
    lderr(m_image_ctx->cct)
        << "failed to parse cache state: " << e.what() << dendl;
    return false;
  }
...
template <typename I>
void
ImageCacheState<I>::write_image_cache_state(
    ...
    Context* on_finish)
{
  ...
  ldout(m_image_ctx->cct, 20)
      << __func__ << " Store state: " << image_state_json << dendl;
  m_plugin_api.execute_image_metadata_set(
      m_image_ctx, PERSISTENT_CACHE_STATE, image_state_json, on_finish);
}

template <typename I>
void
ImageCacheState<I>::clear_image_cache_state(Context* on_finish)
{
  std::shared_lock owner_lock{m_image_ctx->owner_lock};
...
template <typename I>
ImageCacheState<I>*
ImageCacheState<I>::create_image_cache_state(
    I* image_ctx,
    ...
    int& r)
{
  ...
  r = 0;
  bool dirty_cache =
      plugin_api.test_image_features(image_ctx, RBD_FEATURE_DIRTY_CACHE);
  if (dirty_cache) {
    cls_client::metadata_get(
        &image_ctx->md_ctx, image_ctx->header_oid, PERSISTENT_CACHE_STATE,
        &cache_state_str);
  }

  ldout(image_ctx->cct, 20)
      << "image_cache_state: " << cache_state_str << dendl;

  bool pwl_enabled = cache::util::is_pwl_enabled(*image_ctx);
  bool cache_desired = pwl_enabled;
  cache_desired &= !image_ctx->read_only;
  cache_desired &=
      !plugin_api.test_image_features(image_ctx, RBD_FEATURE_MIGRATING);
  cache_desired &=
      !plugin_api.test_image_features(image_ctx, RBD_FEATURE_JOURNALING);
...
template <typename I>
ImageCacheState<I>*
ImageCacheState<I>::get_image_cache_state(
    I* image_ctx,
    plugin::Api<I>& plugin_api)
{
  ImageCacheState<I>* cache_state = nullptr;
