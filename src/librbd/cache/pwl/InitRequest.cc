...
#undef dout_prefix
#define dout_prefix \
  *_dout << "librbd::cache::pwl:InitRequest " << this << " " << __func__ << ": "
...
template <typename I>
void
InitRequest<I>::get_image_cache_state()
{
  ...
  case cache::IMAGE_CACHE_TYPE_RWL:
    m_image_cache = new librbd::cache::pwl::rwl::WriteLog<I>(
        m_image_ctx, cache_state, m_image_writeback, m_plugin_api);
    break;
#endif
#ifdef WITH_RBD_SSD_CACHE
  case cache::IMAGE_CACHE_TYPE_SSD:
    m_image_cache = new librbd::cache::pwl::ssd::WriteLog<I>(
        m_image_ctx, cache_state, m_image_writeback, m_plugin_api);
    break;
#endif
  default:
...
template <typename I>
void
InitRequest<I>::init_image_cache()
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << dendl;
...
template <typename I>
void
InitRequest<I>::handle_init_image_cache(int r)
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << dendl;
...
InitRequest<I>::set_feature_bit()
{
  ...
  using klass = InitRequest<I>;
  Context* ctx =
      create_context_callback<klass, &klass::handle_set_feature_bit>(this);
  ctx->complete(r);
}

template <typename I>
void
InitRequest<I>::handle_set_feature_bit(int r)
{
  CephContext* cct = m_image_ctx.cct;
...
template <typename I>
void
InitRequest<I>::shutdown_image_cache()
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << dendl;
...
template <typename I>
void
InitRequest<I>::handle_shutdown_image_cache(int r)
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << dendl;
