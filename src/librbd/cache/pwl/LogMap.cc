...
#undef dout_prefix
#define dout_prefix \
  *_dout << "librbd::cache::pwl::LogMap: " << this << " " << __func__ << ": "

template <typename T>
std::ostream&
operator<<(std::ostream& os, LogMapEntry<T>& e)
{
...
LogMap<T>::LogMap(CephContext* cct) :
  ...
  m_lock(ceph::make_mutex(
      pwl::unique_lock_name("librbd::cache::pwl::LogMap::m_lock", this)))
{}
...
template <typename T>
void
LogMap<T>::add_log_entry(std::shared_ptr<T> log_entry)
{
...
template <typename T>
void
LogMap<T>::add_log_entries(std::list<std::shared_ptr<T>>& log_entries)
{
  std::lock_guard locker(m_lock);
...
template <typename T>
void
LogMap<T>::remove_log_entry(std::shared_ptr<T> log_entry)
{
  std::lock_guard locker(m_lock);
...
template <typename T>
void
LogMap<T>::remove_log_entries(std::list<std::shared_ptr<T>>& log_entries)
{
  std::lock_guard locker(m_lock);
  ldout(m_cct, 20) << dendl;
  for (auto& log_entry : log_entries) {
...
LogMap<T>::find_log_entries(BlockExtent block_extent)
{
  std::lock_guard locker(m_lock);
  ldout(m_cct, 20) << dendl;
...
template <typename T>
LogMapEntries<T>
LogMap<T>::find_map_entries(BlockExtent block_extent)
{
  std::lock_guard locker(m_lock);
  ldout(m_cct, 20) << dendl;
...
template <typename T>
void
LogMap<T>::add_log_entry_locked(std::shared_ptr<T> log_entry)
{
  LogMapEntry<T> map_entry(log_entry);
  ldout(m_cct, 20) << "block_extent=" << map_entry.block_extent << dendl;
  ceph_assert(ceph_mutex_is_locked_by_me(m_lock));
  LogMapEntries<T> overlap_entries =
      find_map_entries_locked(map_entry.block_extent);
  for (auto& entry : overlap_entries) {
    ...
    if (map_entry.block_extent.block_start <= entry.block_extent.block_start) {
      if (map_entry.block_extent.block_end >= entry.block_extent.block_end) {
        ldout(m_cct, 20) << "map entry completely occluded by new log entry"
                         << dendl;
        remove_map_entry_locked(entry);
      } else {
        ceph_assert(
            map_entry.block_extent.block_end < entry.block_extent.block_end);
        /* The new entry occludes the beginning of the old entry */
        BlockExtent adjusted_extent(
            map_entry.block_extent.block_end, entry.block_extent.block_end);
      ...
      if (map_entry.block_extent.block_end >= entry.block_extent.block_end) {
        ...
        BlockExtent adjusted_extent(
            entry.block_extent.block_start, map_entry.block_extent.block_start);
        adjust_map_entry_locked(entry, adjusted_extent);
      } else {
...
template <typename T>
void
LogMap<T>::remove_log_entry_locked(std::shared_ptr<T> log_entry)
{
  ldout(m_cct, 20) << "*log_entry=" << *log_entry << dendl;
  ceph_assert(ceph_mutex_is_locked_by_me(m_lock));

  LogMapEntries<T> possible_hits =
      find_map_entries_locked(log_entry->block_extent());
...
template <typename T>
void
LogMap<T>::add_map_entry_locked(LogMapEntry<T>& map_entry)
{
...
template <typename T>
void
LogMap<T>::remove_map_entry_locked(LogMapEntry<T>& map_entry)
{
  ...
  if (0 == erased.log_entry->get_map_ref()) {
    ldout(m_cct, 20) << "log entry has zero map entries: " << erased.log_entry
                     << dendl;
  }
}
...
template <typename T>
void
LogMap<T>::adjust_map_entry_locked(
    ...
    BlockExtent& new_extent)
{
  ...
  m_block_to_log_entry_map.insert(
      LogMapEntry<T>(new_extent, adjusted.log_entry));
}

template <typename T>
void
LogMap<T>::split_map_entry_locked(
    LogMapEntry<T>& map_entry,
    BlockExtent& removed_extent)
{
  ...
  BlockExtent left_extent(
      split.block_extent.block_start, removed_extent.block_start);
  m_block_to_log_entry_map.insert(LogMapEntry<T>(left_extent, split.log_entry));

  BlockExtent right_extent(
      removed_extent.block_end, split.block_extent.block_end);
...
template <typename T>
std::list<std::shared_ptr<T>>
LogMap<T>::find_log_entries_locked(const BlockExtent& block_extent)
{
...
template <typename T>
LogMapEntries<T>
LogMap<T>::find_map_entries_locked(const BlockExtent& block_extent)
{
...
template <typename T>
bool
LogMap<T>::LogMapEntryCompare::operator()(
    const LogMapEntry<T>& lhs,
...
template class librbd::cache::pwl::LogMap<
