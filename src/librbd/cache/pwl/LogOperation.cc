...
#undef dout_prefix
#define dout_prefix                                                         \
  *_dout << "librbd::cache::pwl::LogOperation: " << this << " " << __func__ \
         << ": "
...
std::ostream&
GenericLogOperation::format(std::ostream& os) const
{
  os << "dispatch_time=[" << dispatch_time << "], buf_persist_start_time=["
     << buf_persist_start_time << "], buf_persist_comp_time=["
     << buf_persist_comp_time << "], log_append_start_time=["
     << log_append_start_time << "], log_append_comp_time=["
     << log_append_comp_time << "]";
  return os;
}

std::ostream&
operator<<(std::ostream& os, const GenericLogOperation& op)
{
  return op.format(os);
}
...
std::ostream&
SyncPointLogOperation::format(std::ostream& os) const
{
  os << "(Sync Point) ";
...
std::ostream&
operator<<(std::ostream& os, const SyncPointLogOperation& op)
{
  return op.format(os);
}
...
void
SyncPointLogOperation::clear_earlier_sync_point()
{
  std::lock_guard locker(m_lock);
...
void
SyncPointLogOperation::complete(int result)
{
  ceph_assert(sync_point);
...
GenericWriteLogOperation::GenericWriteLogOperation(
  ...
  m_lock(ceph::make_mutex(pwl::unique_lock_name(
      "librbd::cache::pwl::GenericWriteLogOperation::m_lock",
      this))),
  m_cct(cct),
  sync_point(sync_point)
{}
...
std::ostream&
GenericWriteLogOperation::format(std::ostream& os) const
{
  GenericLogOperation::format(os);
  return os;
}
...
std::ostream&
operator<<(std::ostream& os, const GenericWriteLogOperation& op)
{
  return op.format(os);
}
...
GenericWriteLogOperation::appending()
{
  ...
  if (on_append) {
    ldout(m_cct, 20) << __func__ << " " << this << " on_append=" << on_append
                     << dendl;
...
void
GenericWriteLogOperation::complete(int result)
{
  appending();
...
void
WriteLogOperation::init(
    bool has_data,
    std::vector<WriteBufferAllocation>::iterator allocation,
    uint64_t current_sync_gen,
    uint64_t last_op_sequence_num,
    bufferlist& write_req_bl,
    uint64_t buffer_offset,
    bool persist_on_flush)
{
  log_entry->init(
      has_data, current_sync_gen, last_op_sequence_num, persist_on_flush);
  buffer_alloc = &(*allocation);
  bl.substr_of(write_req_bl, buffer_offset, log_entry->write_bytes());
  log_entry->init_cache_bl(
      write_req_bl, buffer_offset, log_entry->write_bytes());
}

std::ostream&
WriteLogOperation::format(std::ostream& os) const
{
...
std::ostream&
operator<<(std::ostream& os, const WriteLogOperation& op)
{
  return op.format(os);
}

void
WriteLogOperation::complete(int result)
{
  GenericWriteLogOperation::complete(result);
  m_perfcounter->tinc(
      l_librbd_pwl_log_op_dis_to_buf_t, buf_persist_start_time - dispatch_time);
  ...
  m_perfcounter->hinc(
      l_librbd_pwl_log_op_buf_to_bufc_t_hist, buf_persist_lat.to_nsec(),
      log_entry->ram_entry.write_bytes);
  m_perfcounter->tinc(
      l_librbd_pwl_log_op_buf_to_app_t,
  ...
  sync_point(sync_point)
{
  ...
  extent_ops_persist = new C_Gather(m_cct, new LambdaContext([this](int r) {
                                      ldout(this->m_cct, 20)
                                          << __func__ << " " << this
                                          << " m_extent_ops_persist completed"
                                          << dendl;
                                      if (on_ops_persist) {
  ...
  extent_ops_appending = new C_Gather(
      m_cct, new LambdaContext([this, appending_persist_sub](int r) {
...
std::ostream&
operator<<(std::ostream& os, const WriteLogOperationSet& s)
{
...
DiscardLogOperation::DiscardLogOperation(
  ...
  log_entry(std::make_shared<DiscardLogEntry>(
      sync_point->log_entry,
      image_offset_bytes,
      write_bytes,
...
std::ostream&
DiscardLogOperation::format(std::ostream& os) const
{
  os << "(Discard) ";
  GenericWriteLogOperation::format(os);
  if (log_entry) {
...
std::ostream&
operator<<(std::ostream& os, const DiscardLogOperation& op)
{
  return op.format(os);
}
