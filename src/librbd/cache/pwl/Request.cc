...
#undef dout_prefix
#define dout_prefix \
  *_dout << "librbd::cache::pwl::Request: " << this << " " << __func__ << ": "
...
template <typename T>
std::ostream&
operator<<(std::ostream& os, const C_BlockIORequest<T>& req)
{
  os << "image_extents=" << req.image_extents << ", image_extents_summary=["
     << req.image_extents_summary << "], bl=" << req.bl
...
template <typename T>
void
C_BlockIORequest<T>::set_cell(BlockGuardCell* cell)
{
...
template <typename T>
BlockGuardCell*
C_BlockIORequest<T>::get_cell(void)
{
...
template <typename T>
void
C_BlockIORequest<T>::release_cell()
{
  ldout(pwl.get_context(), 20) << this << " cell=" << m_cell << dendl;
  ceph_assert(m_cell);
  bool initial = false;
  if (m_cell_released.compare_exchange_strong(initial, true)) {
    ...
    ldout(pwl.get_context(), 5)
        << "cell " << m_cell << " already released for " << this << dendl;
  }
}

template <typename T>
void
C_BlockIORequest<T>::complete_user_request(int r)
{
  bool initial = false;
  if (m_user_req_completed.compare_exchange_strong(initial, true)) {
    ...
    ldout(pwl.get_context(), 20)
        << this << " user req already completed" << dendl;
  }
}

template <typename T>
void
C_BlockIORequest<T>::finish(int r)
{
...
template <typename T>
std::ostream&
operator<<(std::ostream& os, const C_WriteRequest<T>& req)
{
...
template <typename T>
void
C_WriteRequest<T>::blockguard_acquired(GuardedRequestFunctionContext& guard_ctx)
{
  ldout(pwl.get_context(), 20) << __func__ << " write_req=" << this
                               << " cell=" << guard_ctx.cell << dendl;

  ceph_assert(guard_ctx.cell);
  this->detained = guard_ctx.state.detained; /* overlapped */
  this->m_queued =
      guard_ctx.state.queued; /* queued behind at least one barrier */
  this->set_cell(guard_ctx.cell);
}

template <typename T>
void
C_WriteRequest<T>::finish_req(int r)
{
  ldout(pwl.get_context(), 15)
...
template <typename T>
std::shared_ptr<WriteLogOperation>
C_WriteRequest<T>::create_operation(uint64_t offset, uint64_t len)
{
  return pwl.m_builder->create_write_log_operation(
      *op_set, offset, len, pwl.get_context(),
      pwl.m_builder->create_write_log_entry(
          op_set->sync_point->log_entry, offset, len));
}

template <typename T>
void
C_WriteRequest<T>::setup_log_operations(DeferredContexts& on_exit)
{
  GenericWriteLogEntries log_entries;
  {
    std::lock_guard locker(m_lock);
    std::shared_ptr<SyncPoint> current_sync_point = pwl.get_current_sync_point();
    if ((!pwl.get_persist_on_flush() &&
         current_sync_point->log_entry->writes_completed) ||
    ...
    op_set = make_unique<WriteLogOperationSet>(
        this->m_dispatched_time, m_perfcounter, current_sync_point,
        pwl.get_persist_on_flush(), pwl.get_context(), this);
    ldout(pwl.get_context(), 20)
        << "write_req=[" << *this << "], op_set=" << op_set.get() << dendl;
    ceph_assert(m_resources.allocated);
    ...
    for (auto& extent : this->image_extents) {
      ...
      ldout(pwl.get_context(), 20)
          << "write_req=[" << *this << "], op_set=" << op_set.get()
      ...
      operation->init(
          true, allocation, current_sync_gen, pwl.get_last_op_sequence_num(),
          this->bl, buffer_offset, op_set->persist_on_flush);
...
template <typename T>
bool
C_WriteRequest<T>::append_write_request(std::shared_ptr<SyncPoint> sync_point)
{
  std::lock_guard locker(m_lock);
  ...
  if (sync_point->earlier_sync_point) {
    ...
    sync_point->earlier_sync_point->on_sync_point_appending.push_back(
        schedule_append_ctx);
    return true;
  }
  return false;
...
template <typename T>
bool
C_WriteRequest<T>::alloc_resources()
{
...
C_WriteRequest<T>::dispatch()
{
  ...
  ldout(cct, 15) << "write_req=" << this << " cell=" << this->get_cell()
                 << dendl;
...
template <typename T>
C_FlushRequest<T>::C_FlushRequest(
    T& pwl,
    const utime_t arrived,
    io::Extents&& image_extents,
    bufferlist&& bl,
...
template <typename T>
void
C_FlushRequest<T>::finish_req(int r)
{
  ldout(pwl.get_context(), 20)
      << "flush_req=" << this << " cell=" << this->get_cell() << dendl;
  ...
  utime_t now = ceph_clock_now();
  m_perfcounter->tinc(
      l_librbd_pwl_aio_flush_latency, now - this->m_arrived_time);
}

template <typename T>
bool
C_FlushRequest<T>::alloc_resources()
{
  ldout(pwl.get_context(), 20)
      << "req type=" << get_name() << " req=[" << *this << "]" << dendl;
  return pwl.alloc_resources(this);
}

template <typename T>
void
C_FlushRequest<T>::dispatch()
{
  utime_t now = ceph_clock_now();
  ldout(pwl.get_context(), 20)
      << "req type=" << get_name() << " req=[" << *this << "]" << dendl;
  ...
  op = std::make_shared<SyncPointLogOperation>(
      m_lock, to_append, now, m_perfcounter, pwl.get_context());
...
template <typename T>
void
C_FlushRequest<T>::setup_buffer_resources(
    uint64_t* bytes_cached,
    uint64_t* bytes_dirtied,
    uint64_t* bytes_allocated,
    uint64_t* number_lanes,
...
template <typename T>
std::ostream&
operator<<(std::ostream& os, const C_FlushRequest<T>& req)
{
  os << (C_BlockIORequest<T>&)req
     << " m_resources.allocated=" << req.m_resources.allocated;
  return os;
...
template <typename T>
void
C_DiscardRequest<T>::finish_req(int r)
{
  ldout(pwl.get_context(), 20)
      << "discard_req=" << this << " cell=" << this->get_cell() << dendl;
  ceph_assert(this->get_cell());
  this->release_cell();
}
...
template <typename T>
bool
C_DiscardRequest<T>::alloc_resources()
{
  ldout(pwl.get_context(), 20)
      << "req type=" << get_name() << " req=[" << *this << "]" << dendl;
  return pwl.alloc_resources(this);
...
template <typename T>
void
C_DiscardRequest<T>::setup_log_operations()
{
  std::lock_guard locker(m_lock);
  GenericWriteLogEntries log_entries;
  for (auto& extent : this->image_extents) {
  ...
  Context* on_write_append =
      pwl.get_current_sync_point()->prior_persisted_gather_new_sub();

  Context* on_write_persist = new LambdaContext([discard_req](int r) {
    discard_req->complete(r);
  });
  op->init_op(
      current_sync_gen, persist_on_flush, pwl.get_last_op_sequence_num(),
...
template <typename T>
void
C_DiscardRequest<T>::setup_buffer_resources(
    uint64_t* bytes_cached,
    uint64_t* bytes_dirtied,
    uint64_t* bytes_allocated,
    uint64_t* number_lanes,
...
template <typename T>
void
C_DiscardRequest<T>::blockguard_acquired(
...
template <typename T>
std::ostream&
operator<<(std::ostream& os, const C_DiscardRequest<T>& req)
{
  os << (C_BlockIORequest<T>&)req;
  if (req.op) {
    os << " op=[" << *req.op << "]";
...
template <typename T>
void
C_WriteSameRequest<T>::update_req_stats(utime_t& now)
{
...
template <typename T>
std::shared_ptr<WriteLogOperation>
C_WriteSameRequest<T>::create_operation(uint64_t offset, uint64_t len)
{
  ...
  return pwl.m_builder->create_write_log_operation(
      *this->op_set.get(), offset, len, this->bl.length(), pwl.get_context(),
      pwl.m_builder->create_writesame_log_entry(
          set.sync_point->log_entry, offset, len, this->bl.length()));
}

template <typename T>
std::ostream&
operator<<(std::ostream& os, const C_WriteSameRequest<T>& req)
{
...
template <typename T>
void
C_WriteRequest<T>::update_req_stats(utime_t& now)
{
...
template class librbd::cache::pwl::C_BlockIORequest<
    librbd::cache::pwl::AbstractWriteLog<librbd::ImageCtx>>;
template class librbd::cache::pwl::C_WriteRequest<
    librbd::cache::pwl::AbstractWriteLog<librbd::ImageCtx>>;
template class librbd::cache::pwl::C_FlushRequest<
    librbd::cache::pwl::AbstractWriteLog<librbd::ImageCtx>>;
template class librbd::cache::pwl::C_DiscardRequest<
    librbd::cache::pwl::AbstractWriteLog<librbd::ImageCtx>>;
template class librbd::cache::pwl::C_WriteSameRequest<
    librbd::cache::pwl::AbstractWriteLog<librbd::ImageCtx>>;
