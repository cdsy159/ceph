...
public:
  ...
  bool detained =
      false; /* Detained in blockguard (overlapped with a prior IO) */
  ...
  uint64_t
  get_image_extents_size()
  {
    return image_extents.size();
  }

  std::vector<WriteBufferAllocation>&
...
protected:
  ...
  std::atomic<bool> m_deferred = {
      false}; /* Deferred because this or a prior IO had to wait for write resources */
...
public:
  ...
  virtual std::shared_ptr<WriteLogOperation> create_operation(
      uint64_t offset,
      uint64_t len);
...
private:
  ...
  ceph::mutex& m_lock;
  template <typename U>
  friend std::ostream& operator<<(std::ostream& os, const C_WriteRequest<U>& req);
};
...
private:
  ...
  template <typename U>
  friend std::ostream& operator<<(std::ostream& os, const C_FlushRequest<U>& req);
};
...
public:
  ...
  std::shared_ptr<WriteLogOperation> create_operation(
      uint64_t offset,
...
struct BlockGuardReqState {
  ...
  friend std::ostream&
  operator<<(std::ostream& os, const BlockGuardReqState& r)
  {
...
public:
  ...
  GuardedRequestFunctionContext(
      boost::function<void(GuardedRequestFunctionContext&)>&& callback) :
    m_callback(std::move(callback))
  {}

  ~GuardedRequestFunctionContext(void) override{};
  GuardedRequestFunctionContext(const GuardedRequestFunctionContext&) = delete;
  GuardedRequestFunctionContext& operator=(
      const GuardedRequestFunctionContext&) = delete;
...
public:
  ...
  GuardedRequestFunctionContext*
      guard_ctx; /* Work to do when guard on range obtained */
  ...
  friend std::ostream&
  operator<<(std::ostream& os, const GuardedRequest& r)
  {
