...
#undef dout_prefix
#define dout_prefix \
  *_dout << "librbd::cache::pwl::SyncPoint: " << this << " " << __func__ << ": "
...
std::ostream&
operator<<(std::ostream& os, const SyncPoint& p)
{
  os << "log_entry=[" << *p.log_entry
     ...
     << ", m_prior_log_entries_persisted_complete="
     << p.m_prior_log_entries_persisted_complete
...
void
SyncPoint::persist_gather_set_finisher(Context* ctx)
{
...
void
SyncPoint::persist_gather_activate()
{
  m_sync_point_persist->activate();
}

Context*
SyncPoint::persist_gather_new_sub()
{
  return m_sync_point_persist->new_sub();
}

void
SyncPoint::prior_persisted_gather_activate()
{
  m_prior_log_entries_persisted->activate();
}

Context*
SyncPoint::prior_persisted_gather_new_sub()
{
  return m_prior_log_entries_persisted->new_sub();
}

void
SyncPoint::prior_persisted_gather_set_finisher()
{
  Context* sync_point_persist_ready = persist_gather_new_sub();
  std::shared_ptr<SyncPoint> sp = shared_from_this();
  m_prior_log_entries_persisted->set_finisher(
      new LambdaContext([this, sp, sync_point_persist_ready](int r) {
...
void
SyncPoint::add_in_on_persisted_ctxs(Context* ctx)
{
  on_sync_point_persisted.push_back(ctx);
}

void
SyncPoint::add_in_on_appending_ctxs(Context* ctx)
{
  on_sync_point_appending.push_back(ctx);
}

void
SyncPoint::setup_earlier_sync_point(
    ...
    uint64_t last_op_sequence_num)
{
  ...
  if (!earlier_sync_point->appending) {
    ...
    earlier_sync_point->add_in_on_appending_ctxs(
