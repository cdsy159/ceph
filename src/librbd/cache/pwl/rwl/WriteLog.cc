...
#undef dout_prefix
#define dout_prefix                                                          \
  *_dout << "librbd::cache::pwl::rwl::WriteLog: " << this << " " << __func__ \
         << ": "
...
WriteLog<I>::alloc_op_log_entries(GenericLogOperations& ops)
{
  ...
  struct WriteLogCacheEntry* pmem_log_entries =
      D_RW(D_RW(pool_root)->log_entries);
  ...
  for (auto& operation : ops) {
    ...
    this->m_first_free_entry = (this->m_first_free_entry + 1) %
...
WriteLog<I>::append_op_log_entries(GenericLogOperations& ops)
{
  ...
  for (auto& operation : ops) {
    if (!entries_to_flush.empty()) {
      ...
      if (entries_to_flush.back()->get_log_entry()->log_entry_index >
        ...
        ldout(m_image_ctx.cct, 20)
            << "entries to flush wrap around the end of the ring at "
    ...
    ldout(m_image_ctx.cct, 20)
        << "Copying entry for operation at index="
        << operation->get_log_entry()->log_entry_index << " from "
        << &operation->get_log_entry()->ram_entry << " to "
        << operation->get_log_entry()->cache_entry << " operation=["
        << *operation << "]" << dendl;
    operation->log_append_start_time = now;
    *operation->get_log_entry()->cache_entry =
        operation->get_log_entry()->ram_entry;
    ldout(m_image_ctx.cct, 20)
        << "APPENDING: index=" << operation->get_log_entry()->log_entry_index
  ...
  TX_BEGIN(m_log_pool)
  {
    ...
    for (auto& operation : ops) {
      if (operation->reserved_allocated()) {
        ...
        ldout(m_image_ctx.cct, 20)
            << "skipping non-write op: " << *operation << dendl;
      }
    }
  }
  TX_ONCOMMIT {}
  TX_ONABORT
  {
    lderr(cct) << "failed to commit " << ops.size() << " log entries ("
  ...
  m_perfcounter->hinc(
      l_librbd_pwl_append_tx_t_hist, utime_t(tx_end - tx_start).to_nsec(),
      ops.size());
  for (auto& operation : ops) {
...
template <typename I>
void
WriteLog<I>::flush_op_log_entries(GenericLogOperationsVector& ops)
{
  if (ops.empty()) {
    return;
  }
  ...
  if (ops.size() > 1) {
    ceph_assert(
        ...
        ops.back()->get_log_entry()->cache_entry);
  }

  ldout(m_image_ctx.cct, 20)
      << "entry count=" << ops.size()
      << " start address=" << ops.front()->get_log_entry()->cache_entry
      << " bytes="
      << ops.size() * sizeof(*(ops.front()->get_log_entry()->cache_entry))
      << dendl;
  pmemobj_flush(
      m_log_pool, ops.front()->get_log_entry()->cache_entry,
      ops.size() * sizeof(*(ops.front()->get_log_entry()->cache_entry)));
}
...
WriteLog<I>::initialize_pool(Context* on_finish, pwl::DeferredContexts& later)
{
  ...
  if (access(this->m_log_pool_name.c_str(), F_OK) != 0) {
    if ((m_log_pool = pmemobj_create(
             this->m_log_pool_name.c_str(), this->m_pwl_pool_layout_name,
             this->m_log_pool_size, (S_IWUSR | S_IRUSR))) == NULL) {
    ...
    uint64_t num_small_writes = (uint64_t)(effective_pool_size /
                                           small_write_size);
    ...
    TX_BEGIN(m_log_pool)
    {
      ...
      D_RW(pool_root)->log_entries = TX_ZALLOC(
    ...
    TX_ONCOMMIT
    {
      ...
      this->m_free_log_entries = D_RO(pool_root)->num_log_entries -
                                 1; // leave one free
    }
    TX_ONABORT
    {
    ...
    if (D_RO(pool_root)->header.layout_version != RWL_LAYOUT_VERSION) {
      ...
      lderr(cct) << "pool layout version is "
                 << D_RO(pool_root)->header.layout_version << " expected "
...
template <typename I>
void
WriteLog<I>::load_existing_entries(DeferredContexts& later)
{
  TOID(struct WriteLogPoolRoot) pool_root;
  pool_root = POBJ_ROOT(m_log_pool, struct WriteLogPoolRoot);
  struct WriteLogCacheEntry* pmem_log_entries =
  ...
  while (entry_index != m_first_free_entry) {
    ...
    this->update_entries(
        &log_entry, pmem_entry, missing_sync_points, sync_point_entries,
        entry_index);
    ...
    std::shared_ptr<pwl::GenericLogEntry> log_entry)
{
  if (log_entry->is_write_entry()) {
    this->m_bytes_allocated +=
        std::max(log_entry->write_bytes(), MIN_WRITE_ALLOC_SIZE);
...
template <typename I>
void
WriteLog<I>::write_data_to_buffer(
...
WriteLog<I>::retire_entries(const unsigned long int frees_per_tx)
{
  ...
  if (retiring_entries.size()) {
      ...
      TX_BEGIN(m_log_pool)
      {
        if (D_RO(pool_root)->flushed_sync_gen < flushed_sync_gen) {
          ldout(m_image_ctx.cct, 20) << "flushed_sync_gen in log updated from "
                                     << D_RO(pool_root)->flushed_sync_gen
        ...
        for (auto& entry : retiring_entries) {
          if (entry->write_bytes()) {
            ldout(cct, 20) << "Freeing "
                           << entry->ram_entry.write_data.oid.pool_uuid_lo
                           << "." << entry->ram_entry.write_data.oid.off
                           << dendl;
    ...
    m_perfcounter->hinc(
        l_librbd_pwl_retire_tx_t_hist, utime_t(tx_end - tx_start).to_nsec(),
...
template <typename I>
void
WriteLog<I>::flush_then_append_scheduled_ops(void)
{
  ...
  do {
    {
      ...
      if (m_ops_to_flush.size()) {
        ...
        ops.splice(
            ops.end(), m_ops_to_flush, m_ops_to_flush.begin(), last_in_batch);
...
template <typename I>
void
WriteLog<I>::schedule_flush_and_append(GenericLogOperationsVector& ops)
{
...
WriteLog<I>::process_work()
{
  ...
  uint64_t aggressive_high_water_entries = this->m_total_log_entries *
                                           AGGRESSIVE_RETIRE_HIGH_WATER;
    ...
    if (this->m_alloc_failed_since_retire || this->m_invalidating ||
      ...
      while (this->m_alloc_failed_since_retire || this->m_invalidating ||
             ...
             (((this->m_bytes_allocated > low_water_bytes) ||
              ...
              (utime_t(ceph_clock_now() - started).to_msec() <
...
template <typename V>
void
WriteLog<I>::flush_pmem_buffer(V& ops)
{
  utime_t now = ceph_clock_now();
  for (auto& operation : ops) {
    if (operation->reserved_allocated()) {
  ...
  for (auto& operation : ops) {
    if (operation->is_writing_op()) {
      auto log_entry =
          static_pointer_cast<WriteLogEntry>(operation->get_log_entry());
      pmemobj_flush(
          m_log_pool, log_entry->cache_buffer, log_entry->write_bytes());
    }
  }
...
template <typename I>
void
WriteLog<I>::persist_last_flushed_sync_gen()
{
  TOID(struct WriteLogPoolRoot) pool_root;
  pool_root = POBJ_ROOT(m_log_pool, struct WriteLogPoolRoot);
  uint64_t flushed_sync_gen;
    ...
    bool& no_space)
{
  ...
  for (auto& buffer : buffers) {
    ...
    buffer.buffer_oid = pmemobj_reserve(
        m_log_pool, &buffer.buffer_alloc_action, buffer.allocation_size,
        0 /* Object type */);
    buffer.allocation_lat = ceph_clock_now() - before_reserve;
    if (TOID_IS_NULL(buffer.buffer_oid)) {
      ldout(m_image_ctx.cct, 5)
          << "can't allocate all data buffers: " << pmemobj_errormsg() << ". "
          << *req << dendl;
    ...
    ldout(m_image_ctx.cct, 20)
        << "Allocated " << buffer.buffer_oid.oid.pool_uuid_lo << "."
        << buffer.buffer_oid.oid.off << ", size=" << buffer.allocation_size
        << dendl;
  }
}
