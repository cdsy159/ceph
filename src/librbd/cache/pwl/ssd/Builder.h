...
public:
  std::shared_ptr<pwl::WriteLogEntry>
      ...
      Context* user_req) override
  {
    return new C_CompAndWriteRequest<T>(
        ...
        std::move(bl), mismatch_offset, fadvise_flags, lock, perfcounter,
        user_req);
  }

  std::shared_ptr<pwl::WriteLogOperation>
  ...
  std::shared_ptr<pwl::DiscardLogOperation>
