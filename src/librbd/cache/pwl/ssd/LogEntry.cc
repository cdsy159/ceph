...
#undef dout_prefix
#define dout_prefix                                                   \
  *_dout << "librbd::cache::pwl::ssd::WriteLogEntry: " << this << " " \
         << __func__ << ": "
...
void
WriteLogEntry::init_cache_bl(bufferlist& src_bl, uint64_t off, uint64_t len)
{
  cache_bl.clear();
...
void
WriteLogEntry::copy_cache_bl(bufferlist* out)
{
...
unsigned int
WriteLogEntry::get_aligned_data_size() const
{
  if (cache_bl.length()) {
...
void
WriteLogEntry::writeback_bl(
    librbd::cache::ImageWritebackInterface& image_writeback,
    Context* ctx,
    ceph::bufferlist&& bl)
{
  image_writeback.aio_write(
      {{ram_entry.image_offset_bytes, ram_entry.write_bytes}}, std::move(bl), 0,
      ctx);
}

void
WriteSameLogEntry::writeback_bl(
    librbd::cache::ImageWritebackInterface& image_writeback,
    Context* ctx,
    ceph::bufferlist&& bl)
{
