...
#undef dout_prefix
#define dout_prefix                                                          \
  *_dout << "librbd::cache::pwl::ssd::WriteLog: " << this << " " << __func__ \
         << ": "
...
static bool
is_valid_pool_root(const WriteLogPoolRoot& root)
{
...
template <typename I>
Builder<AbstractWriteLog<I>>*
WriteLog<I>::create_builder()
{
...
template <typename I>
void
WriteLog<I>::collect_read_extents(
    uint64_t read_buffer_offset,
    LogMapEntry<GenericWriteLogEntry> map_entry,
    ...
    pwl::C_ReadRequest* read_ctx)
{
  ...
  auto write_entry =
      std::static_pointer_cast<WriteLogEntry>(map_entry.log_entry);
  buffer::list hit_bl;
...
template <typename I>
int
WriteLog<I>::create_and_open_bdev()
{
  CephContext* cct = m_image_ctx.cct;

  bdev = BlockDevice::create(
      cct, this->m_log_pool_name, aio_cache_cb, nullptr, nullptr, nullptr);
  ...
  if (bdev->get_size() != this->m_log_pool_size) {
    lderr(cct) << "size mismatch: bdev size " << bdev->get_size()
               << " (block size " << bdev->get_block_size() << ") != pool size "
               << this->m_log_pool_size << dendl;
    bdev->close();
    delete bdev;
    return -EINVAL;
  }
...
template <typename I>
bool
WriteLog<I>::initialize_pool(Context* on_finish, pwl::DeferredContexts& later)
{
  int r;
  CephContext* cct = m_image_ctx.cct;
...
WriteLog<I>::remove_pool_file()
{
  ...
  if (m_cache_state->clean) {
    ldout(m_image_ctx.cct, 5)
        << "Removing empty pool file: " << this->m_log_pool_name << dendl;
    ...
    ldout(m_image_ctx.cct, 5)
        << "Not removing pool file: " << this->m_log_pool_name << dendl;
  }
}

template <typename I>
void
WriteLog<I>::load_existing_entries(pwl::DeferredContexts& later)
{
  CephContext* cct = m_image_ctx.cct;
  ...
  for (uint64_t next_log_pos = this->m_first_valid_entry;
    ...
    bdev->read(
        next_log_pos, MIN_WRITE_ALLOC_SSD_SIZE, &bl_entries, &ioctx_entry,
        false);
    ...
    for (auto it = ssd_log_entries.begin(); it != ssd_log_entries.end(); ++it) {
      this->update_entries(
          &log_entry, &*it, missing_sync_points, sync_point_entries,
          curr_log_pos);
    ...
    if (next_log_pos >= this->m_log_pool_size) {
      next_log_pos = next_log_pos % this->m_log_pool_size +
                     DATA_RING_BUFFER_OFFSET;
    }
  }
...
template <typename I>
void
WriteLog<I>::inc_allocated_cached_bytes(
...
template <typename I>
bool
WriteLog<I>::alloc_resources(C_BlockIORequestT* req)
{
  ...
  req->setup_buffer_resources(
      &bytes_cached, &bytes_dirtied, &bytes_allocated, &num_lanes,
  ...
  alloc_succeeds = this->check_allocation(
      req, bytes_cached, bytes_dirtied, bytes_allocated, num_lanes,
      num_log_entries, num_unpublished_reserves);
...
template <typename I>
bool
WriteLog<I>::has_sync_point_logs(GenericLogOperations& ops)
{
  for (auto& op : ops) {
...
template <typename I>
void
WriteLog<I>::schedule_append_ops(
    ...
    C_BlockIORequestT* req)
{
  ...
  std::copy(std::begin(ops), std::end(ops), std::back_inserter(appending));
  {
    ...
    need_finisher = !this->m_appending && ((this->m_ops_to_append.size() >=
...
template <typename I>
void
WriteLog<I>::setup_schedule_append(
...
template <typename I>
void
WriteLog<I>::append_scheduled_ops(void)
{
  GenericLogOperations ops;
...
template <typename I>
void
WriteLog<I>::append_op_log_entries(GenericLogOperations& ops)
{
  ...
  Context* append_ctx = new LambdaContext([this, new_first_free_entry, ops,
                                           ctx](int r) {
    std::shared_ptr<WriteLogPoolRoot> new_root;
    {
      ldout(m_image_ctx.cct, 20)
          << "Finished appending at " << *new_first_free_entry << dendl;
...
template <typename I>
void
WriteLog<I>::release_ram(std::shared_ptr<GenericLogEntry> log_entry)
{
...
template <typename I>
void
WriteLog<I>::alloc_op_log_entries(GenericLogOperations& ops)
{
...
template <typename I>
void
WriteLog<I>::construct_flush_entries(
    pwl::GenericLogEntries entries_to_flush,
    ...
    for (auto& log_entry : entries_to_flush) {
      GuardedRequestFunctionContext* guarded_ctx =
          new GuardedRequestFunctionContext(
              [this, log_entry,
               invalidating](GuardedRequestFunctionContext& guard_ctx) {
                log_entry->m_cell = guard_ctx.cell;
                Context* ctx =
                    this->construct_flush_entry(log_entry, invalidating);

                if (!invalidating) {
                  ctx = new LambdaContext([this, log_entry, ctx](int r) {
                    m_image_ctx.op_work_queue->queue(
                        new LambdaContext([this, log_entry, ctx](int r) {
    ...
    Context* ctx = new LambdaContext([this, entries_to_flush, read_bls](int r) {
      ...
      for (auto& log_entry : entries_to_flush) {
        if (log_entry->is_write_entry()) {
          ...
          guarded_ctx = new GuardedRequestFunctionContext(
              [this, log_entry,
               captured_entry_bl](GuardedRequestFunctionContext& guard_ctx) {
                log_entry->m_cell = guard_ctx.cell;
                Context* ctx = this->construct_flush_entry(log_entry, false);

                m_image_ctx.op_work_queue->queue(
                    new LambdaContext([this, log_entry,
                                       entry_bl = std::move(captured_entry_bl),
                                       ctx](int r) {
                      auto captured_entry_bl = std::move(entry_bl);
                      ldout(m_image_ctx.cct, 15) << "flushing:" << log_entry
                                                 << " " << *log_entry << dendl;
                      log_entry->writeback_bl(
                          this->m_image_writeback, ctx,
          ...
          guarded_ctx = new GuardedRequestFunctionContext(
              [this, log_entry](GuardedRequestFunctionContext& guard_ctx) {
                log_entry->m_cell = guard_ctx.cell;
                Context* ctx = this->construct_flush_entry(log_entry, false);
                m_image_ctx.op_work_queue->queue(
                    new LambdaContext([this, log_entry, ctx](int r) {
...
template <typename I>
void
WriteLog<I>::process_work()
{
  CephContext* cct = m_image_ctx.cct;
  int max_iterations = 4;
  bool wake_up_requested = false;
  uint64_t aggressive_high_water_bytes = this->m_bytes_allocated_cap *
  ...
  do {
    ...
    if (this->m_alloc_failed_since_retire || (this->m_shutting_down) ||
      ...
      ldout(m_image_ctx.cct, 10)
          ...
          << (m_bytes_allocated > high_water_bytes) << dendl;
      retire_entries(
          (this->m_shutting_down || this->m_invalidating ||
...
template <typename I>
bool
WriteLog<I>::retire_entries(const unsigned long int frees_per_tx)
{
  CephContext* cct = m_image_ctx.cct;
        ...
        if (this->can_retire_entry(*it)) {
          ...
          if ((*it)->log_entry_index != control_block_pos) {
            ldout(cct, 20) << "Old log_entry_index is " << control_block_pos
                           ...
                           << (*it)->log_entry_index << ",data length is "
                           << data_length << dendl;
        ...
        for (auto it = retiring_subentries.begin();
          ...
          if ((*it)->write_bytes() > 0 || (*it)->bytes_dirty() > 0) {
            auto gen_write_entry =
                static_pointer_cast<GenericWriteLogEntry>(*it);
            if (gen_write_entry) {
  ...
  if (retiring_entries.size()) {
    ...
    Context* ctx = new LambdaContext([this, first_valid_entry,
    ...
    uint64_t* pos)
{
  ...
  ceph_assert(
      *pos >= DATA_RING_BUFFER_OFFSET && *pos < this->m_log_pool_size &&
  ...
  *pos += MIN_WRITE_ALLOC_SSD_SIZE;
  ...
  if (control_block_pos + bl.length() > this->m_log_pool_size) {
    ...
    ldout(cct, 20) << "write " << control_block_pos << "~" << size
                   << " spans boundary, split into " << control_block_pos << "~"
                   << bl1.length() << " and " << DATA_RING_BUFFER_OFFSET << "~"
                   << bl.length() << dendl;
    bdev->aio_write(
        control_block_pos, bl1, &aio->ioc, false, WRITE_LIFE_NOT_SET);
    bdev->aio_write(
        DATA_RING_BUFFER_OFFSET, bl, &aio->ioc, false, WRITE_LIFE_NOT_SET);
  } else {
    ldout(cct, 20) << "write " << control_block_pos << "~" << bl.length()
                   << dendl;
...
template <typename I>
void
WriteLog<I>::schedule_update_root(
...
template <typename I>
void
WriteLog<I>::enlist_op_update_root()
{
  Context* append_ctx = new LambdaContext([this](int r) {
...
template <typename I>
void
WriteLog<I>::update_root_scheduled_ops()
{
  ldout(m_image_ctx.cct, 20) << dendl;
  ...
  Context* ctx = nullptr;
  {
    ...
    if (m_updating_pool_root) {
      ...
      ldout(m_image_ctx.cct, 15)
          << "Another thread is updating pool root" << dendl;
      return;
    }
  ...
  ldout(m_image_ctx.cct, 15)
      << "Update root number: " << root_updates.size() << dendl;
...
template <typename I>
int
WriteLog<I>::update_pool_root_sync(std::shared_ptr<WriteLogPoolRoot> root)
{
  bufferlist bl;
  SuperBlock superblock;
  superblock.root = *root;
...
template <typename I>
void
WriteLog<I>::aio_read_data_block(
    std::shared_ptr<GenericWriteLogEntry> log_entry,
    bufferlist* bl,
    Context* ctx)
{
...
template <typename I>
void
WriteLog<I>::aio_read_data_blocks(
    std::vector<std::shared_ptr<GenericWriteLogEntry>>& log_entries,
    ...
    Context* ctx)
{
  ...
  Context* read_ctx = new LambdaContext([log_entries, bls, ctx](int r) {
    for (unsigned int i = 0; i < log_entries.size(); i++) {
      ...
      auto length = write_entry->ram_entry.is_write()
  ...
  for (unsigned int i = 0; i < log_entries.size(); i++) {
    ...
    ldout(cct, 20) << "entry i=" << i << " " << log_entry->write_data_pos << "~"
                   << len << dendl;
    ceph_assert(
        log_entry->write_data_pos >= DATA_RING_BUFFER_OFFSET &&
    ...
    if (log_entry->write_data_pos + align_len > pool_root.pool_size) {
      ...
      ldout(cct, 20) << "read " << log_entry->write_data_pos << "~" << align_len
                     << " spans boundary, split into "
                     << log_entry->write_data_pos << "~" << len1 << " and "
                     << DATA_RING_BUFFER_OFFSET << "~" << len2 << dendl;
...
template <typename I>
void
WriteLog<I>::complete_user_request(Context*& user_req, int r)
{
