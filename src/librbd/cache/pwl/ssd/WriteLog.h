...
private:
  ...
  using WriteLogPoolRootUpdateList =
      std::list<std::shared_ptr<WriteLogPoolRootUpdate>>;
  WriteLogPoolRootUpdateList
      m_poolroot_to_update; /* pool root list to update to SSD */
  ...
  void construct_flush_entries(
      pwl::GenericLogEntries entires_to_flush,
