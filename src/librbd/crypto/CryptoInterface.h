...
public:
  ...
  get_pre_and_post_align(uint64_t off, uint64_t len)
  {
    if (len == 0) {
    ...
    return std::make_pair(
        p2phase(off, block_size), p2nphase(off + len, block_size));
  }

  inline std::pair<uint64_t, uint64_t>
  align(uint64_t off, uint64_t len)
  {
    auto aligns = get_pre_and_post_align(off, len);
    return std::make_pair(
        off - aligns.first, len + aligns.first + aligns.second);
  }

  inline bool
  is_aligned(uint64_t off, uint64_t len)
  {
    auto aligns = get_pre_and_post_align(off, len);
  ...
  inline bool
  is_aligned(const io::ReadExtents& extents)
  {
  ...
  inline void
  align_extents(const io::ReadExtents& extents, io::ReadExtents* aligned_extents)
  {
    for (const auto& extent : extents) {
  ...
  inline int
  decrypt_aligned_extent(io::ReadExtent& extent, uint64_t image_offset)
  {
