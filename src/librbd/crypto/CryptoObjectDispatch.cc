...
template <typename I>
uint64_t
get_file_offset(I* image_ctx, uint64_t object_no, uint64_t object_off)
{
  auto off = io::util::raw_to_area_offset(
      *image_ctx,
      Striper::get_file_offset(
          image_ctx->cct, &image_ctx->layout, object_no, object_off));
  ...
  handle_read(int r)
  {
    ...
    if (r == -ENOENT && !disable_read_from_parent) {
      io::util::read_parent<I>(
          image_ctx, object_no, extents, io_context->get_read_snap(),
          parent_trace, this);
    } else {
      complete(r);
    }
  ...
  remove_alignment_data()
  {
    for (uint64_t i = 0; i < extents->size(); ++i) {
      ...
      if (aligned_extent.extent_map.empty()) {
        ...
        int64_t padding_count = cut_offset + extent.length -
                                aligned_extent.bl.length();
        if (padding_count > 0) {
...
struct C_UnalignedObjectWriteRequest : public Context {
  ...
  check_cmp_data()
  {
    ...
    io::util::unsparsify(
        image_ctx->cct, &cmp_extent.bl, cmp_extent.extent_map,
  ...
  build_aligned_data()
  {
    ...
    if (pre_align != 0) {
      ...
      io::util::unsparsify(
          image_ctx->cct, &extent.bl, extent.extent_map, extent.offset,
          extent.length);
  ...
  void
  restart_request(bool may_copyup)
  {
...
{
  m_data_offset_object_no =
      Striper::get_num_objects(image_ctx->layout, crypto->get_data_offset());
}

template <typename I>
void
CryptoObjectDispatch<I>::shut_down(Context* on_finish)
{
  ...
  if (m_crypto->is_aligned(object_off, data.length())) {
    ...
    *dispatch_result = io::DISPATCH_RESULT_COMPLETE;
    auto req = new C_UnalignedObjectWriteRequest<I>(
        ...
        parent_trace, object_dispatch_flags, journal_tid, on_dispatched, true);
    req->send();
  }
...
template <typename I>
bool
CryptoObjectDispatch<I>::write_same(
    uint64_t object_no,
    uint64_t object_off,
    uint64_t object_len,
    ...
    Context* on_dispatched)
{
  ...
  auto ctx = new LambdaContext([on_finish_ctx = on_dispatched](int r) {
  ...
  *dispatch_result = io::DISPATCH_RESULT_COMPLETE;
  auto req = io::ObjectDispatchSpec::create_write(
      m_image_ctx,
      io::util::get_previous_layer(io::OBJECT_DISPATCH_LAYER_CRYPTO), object_no,
      object_off, std::move(ws_data), io_context, op_flags, 0, std::nullopt, 0,
      parent_trace, ctx);
  req->send();
  return true;
}
...
template <typename I>
bool
CryptoObjectDispatch<I>::compare_and_write(
    uint64_t object_no,
    uint64_t object_off,
    ceph::bufferlist&& cmp_data,
...
template <typename I>
bool
CryptoObjectDispatch<I>::discard(
    uint64_t object_no,
    uint64_t object_off,
    uint64_t object_len,
    IOContext io_context,
    ...
    Context* on_dispatched)
{
  ...
  *dispatch_result = io::DISPATCH_RESULT_COMPLETE;
  auto req = io::ObjectDispatchSpec::create_write_same(
      m_image_ctx,
      io::util::get_previous_layer(io::OBJECT_DISPATCH_LAYER_CRYPTO), object_no,
      object_off, object_len, {{0, object_len}}, std::move(bl), io_context,
...
template <typename I>
int
CryptoObjectDispatch<I>::prepare_copyup(
    uint64_t object_no,
    io::SnapshotSparseBufferlist* snapshot_sparse_bufferlist)
{
  ...
  for (auto& [key, extent_map] : *snapshot_sparse_bufferlist) {
    ...
    for (auto& extent : extent_map) {
      auto [aligned_off, aligned_len] =
          m_crypto->align(extent.get_off(), extent.get_len());
      ...
      encrypted_sparse_bufferlist.insert(
          ...
          {io::SPARSE_EXTENT_STATE_DATA, aligned_len, std::move(encrypted_bl)});
    }
