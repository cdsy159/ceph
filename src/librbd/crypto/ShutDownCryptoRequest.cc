...
#undef dout_prefix
#define dout_prefix                                                  \
  *_dout << "librbd::crypto::ShutDownCryptoRequest: " << this << " " \
         << __func__ << ": "
...
template <typename I>
void
ShutDownCryptoRequest<I>::shut_down_object_dispatch()
{
...
template <typename I>
void
ShutDownCryptoRequest<I>::handle_shut_down_object_dispatch(int r)
{
  ldout(m_image_ctx->cct, 20) << "r=" << r << dendl;

  if (r < 0) {
    lderr(m_image_ctx->cct)
        << "failed to shut down object dispatch: " << cpp_strerror(r) << dendl;
  }
...
template <typename I>
void
ShutDownCryptoRequest<I>::finish(int r)
{
  ldout(m_image_ctx->cct, 20) << "r=" << r << dendl;
