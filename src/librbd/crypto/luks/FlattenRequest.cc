...
#undef dout_prefix
#define dout_prefix                                                 \
  *_dout << "librbd::crypto::luks::FlattenRequest: " << this << " " \
         << __func__ << ": "
...
template <typename I>
void
FlattenRequest<I>::handle_read_header(int r)
{
  ...
  if (r < 0) {
    lderr(m_image_ctx->cct)
        << "unable to determine encryption header magic: " << cpp_strerror(r)
        << dendl;
    finish(r);
    return;
  } else if (r > 0) {
    ...
    if (r < 0) {
      lderr(m_image_ctx->cct)
          << "unable to restore header magic: " << cpp_strerror(r) << dendl;
      finish(r);
      return;
    }
  }
...
FlattenRequest<I>::write_header()
{
  ...
  auto req = io::ImageDispatchSpec::create_write(
      *m_image_ctx, io::IMAGE_DISPATCH_LAYER_API_START, aio_comp,
      {{0, m_bl.length()}}, io::ImageArea::CRYPTO_HEADER, std::move(m_bl), 0,
      trace);
  req->send();
}
...
template <typename I>
void
FlattenRequest<I>::handle_write_header(int r)
{
  ldout(m_image_ctx->cct, 20) << "r=" << r << dendl;

  if (r < 0) {
    lderr(m_image_ctx->cct)
        << "error writing header to image: " << cpp_strerror(r) << dendl;
    finish(r);
    return;
  }
...
template <typename I>
void
FlattenRequest<I>::handle_flush(int r)
{
  ldout(m_image_ctx->cct, 20) << "r=" << r << dendl;
