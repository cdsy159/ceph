...
#undef dout_prefix
#define dout_prefix                                                            \
  *_dout << "librbd::crypto::luks::FormatRequest: " << this << " " << __func__ \
         << ": "
...
FormatRequest<I>::send()
{
  ...
  default:
    lderr(m_image_ctx->cct)
        << "unsupported cipher algorithm: " << m_alg << dendl;
    finish(-EINVAL);
    return;
  }
  ...
  r = m_header.format(
      type, cipher, reinterpret_cast<char*>(key), key_size, "xts-plain64",
      sector_size, stripe_period, m_insecure_fast_mode);
  if (r != 0) {
    finish(r);
    return;
  }
  ...
  r = util::build_crypto(
      m_image_ctx->cct, key, key_size, m_header.get_sector_size(),
  ...
  if (m_image_ctx->parent != nullptr) {
    ...
    if (r < 0) {
      lderr(m_image_ctx->cct)
          << "error replacing LUKS magic: " << cpp_strerror(r) << dendl;
      finish(r);
      return;
    }
  }
...
template <typename I>
void
FormatRequest<I>::handle_write_header(int r)
{
