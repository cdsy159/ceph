...
#undef dout_prefix
#define dout_prefix \
  *_dout << "librbd::crypto::luks::Magic: " << __func__ << ": "
...
int
Magic::cmp(ceph::bufferlist& bl, uint32_t bl_off, const std::string& cmp_str)
{
...
int
Magic::is_luks(ceph::bufferlist& bl)
{
  return cmp(bl, 0, LUKS_MAGIC);
}

int
Magic::is_rbd_clone(ceph::bufferlist& bl)
{
  return cmp(bl, 0, RBD_CLONE_MAGIC);
}

void
Magic::transform_secondary_header_magic(char* magic)
{
...
int
Magic::replace_magic(CephContext* cct, ceph::bufferlist& bl)
{
  ...
  case 2: {
    ...
    uint64_t hdr_size;
    r = read(
        bl, MAGIC_LENGTH + sizeof(version), sizeof(hdr_size), (char*)&hdr_size);
    if (r < 0) {
