...
#undef dout_prefix
#define dout_prefix                                                          \
  *_dout << "librbd::deep_copy::SetHeadRequest: " << this << " " << __func__ \
         << ": "
...
template <typename I>
SetHeadRequest<I>::SetHeadRequest(
    I* image_ctx,
    uint64_t size,
  ...
  m_cct(image_ctx->cct)
{
  ...
  ldout(m_cct, 20) << "image_id=" << m_image_ctx->id << ", size=" << m_size
...
template <typename I>
void
SetHeadRequest<I>::send_set_size()
{
...
template <typename I>
void
SetHeadRequest<I>::handle_set_size(int r)
{
...
template <typename I>
void
SetHeadRequest<I>::send_detach_parent()
{
  m_image_ctx->image_lock.lock_shared();
  if (m_image_ctx->parent_md.spec.pool_id == -1 ||
...
template <typename I>
void
SetHeadRequest<I>::handle_detach_parent(int r)
{
  ldout(m_cct, 20) << "r=" << r << dendl;
...
template <typename I>
void
SetHeadRequest<I>::send_attach_parent()
{
  m_image_ctx->image_lock.lock_shared();
...
template <typename I>
void
SetHeadRequest<I>::handle_attach_parent(int r)
{
  ldout(m_cct, 20) << "r=" << r << dendl;
...
template <typename I>
Context*
SetHeadRequest<I>::start_lock_op(int* r)
{
