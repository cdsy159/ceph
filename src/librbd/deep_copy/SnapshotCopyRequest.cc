...
#undef dout_prefix
#define dout_prefix                                                   \
  *_dout << "librbd::deep_copy::SnapshotCopyRequest: " << this << " " \
         << __func__ << ": "
...
template <typename I>
const std::string&
get_snapshot_name(I* image_ctx, librados::snap_t snap_id)
{
...
template <typename I>
SnapshotCopyRequest<I>::SnapshotCopyRequest(
    I* src_image_ctx,
    I* dst_image_ctx,
  ...
  m_lock(ceph::make_mutex(unique_lock_name("SnapshotCopyRequest::m_lock", this)))
{
  ...
  m_src_snap_ids.insert(
      src_image_ctx->snaps.begin(), src_image_ctx->snaps.end());
  m_src_image_ctx->image_lock.unlock_shared();

  m_dst_image_ctx->image_lock.lock_shared();
  m_dst_snap_ids.insert(
      dst_image_ctx->snaps.begin(), dst_image_ctx->snaps.end());
  m_dst_image_ctx->image_lock.unlock_shared();

  if (m_src_snap_id_end != CEPH_NOSNAP) {
    m_src_snap_ids.erase(
        m_src_snap_ids.upper_bound(m_src_snap_id_end), m_src_snap_ids.end());
  }
  ...
  ldout(m_cct, 20) << "src_image_id=" << m_src_image_ctx->id
                   ...
                   << ", dst_snap_ids=" << m_dst_snap_ids << dendl;
}

template <typename I>
void
SnapshotCopyRequest<I>::send()
{
...
template <typename I>
void
SnapshotCopyRequest<I>::send_snap_unprotect()
{
  ...
  for (; snap_id_it != m_dst_snap_ids.end(); ++snap_id_it) {
    ...
    if (snap_seq_it != m_snap_seqs.end()) {
      ...
      r = m_src_image_ctx->is_snap_unprotected(
          snap_seq_it->first, &src_unprotected);
...
template <typename I>
void
SnapshotCopyRequest<I>::handle_snap_unprotect(int r)
{
...
template <typename I>
void
SnapshotCopyRequest<I>::send_snap_remove()
{
  SnapIdSet::iterator snap_id_it = m_dst_snap_ids.begin();
  if (m_prev_snap_id != CEPH_NOSNAP) {
    snap_id_it = m_dst_snap_ids.upper_bound(m_prev_snap_id);
...
template <typename I>
void
SnapshotCopyRequest<I>::handle_snap_remove(int r)
{
  ldout(m_cct, 20) << "r=" << r << dendl;
...
template <typename I>
void
SnapshotCopyRequest<I>::send_snap_create()
{
  ...
  ldout(m_cct, 20) << "snap_name=" << m_snap_name << ", "
                   << "snap_id=" << m_prev_snap_id << ", " << "size=" << size
                   << ", " << "parent_info=["
...
template <typename I>
void
SnapshotCopyRequest<I>::handle_snap_create(int r)
{
  ldout(m_cct, 20) << "r=" << r << dendl;
...
template <typename I>
void
SnapshotCopyRequest<I>::send_snap_protect()
{
  SnapIdSet::iterator snap_id_it = m_src_snap_ids.begin();
  if (m_prev_snap_id != CEPH_NOSNAP) {
    snap_id_it = m_src_snap_ids.upper_bound(m_prev_snap_id);
...
template <typename I>
void
SnapshotCopyRequest<I>::handle_snap_protect(int r)
{
  ldout(m_cct, 20) << "r=" << r << dendl;
...
template <typename I>
void
SnapshotCopyRequest<I>::send_set_head()
{
  auto snap_seq_it = m_snap_seqs.find(m_src_snap_id_end);
  if (m_src_snap_id_end != CEPH_NOSNAP &&
      (snap_seq_it == m_snap_seqs.end() || snap_seq_it->second != CEPH_NOSNAP)) {
  ...
  auto req = SetHeadRequest<I>::create(
      m_dst_image_ctx, size, parent_spec, parent_overlap, ctx);
  req->send();
}

template <typename I>
void
SnapshotCopyRequest<I>::handle_set_head(int r)
{
  ldout(m_cct, 20) << "r=" << r << dendl;
...
template <typename I>
void
SnapshotCopyRequest<I>::send_resize_object_map()
{
  int r = 0;
  ...
  if (m_dst_image_ctx->test_features(RBD_FEATURE_OBJECT_MAP)) {
    ...
    if (m_dst_image_ctx->object_map != nullptr &&
        Striper::get_num_objects(
            m_dst_image_ctx->layout, m_dst_image_ctx->size) !=
      ...
      if (finish_op_ctx != nullptr) {
        ...
        m_dst_image_ctx->object_map->aio_resize(
            m_dst_image_ctx->size, OBJECT_NONEXISTENT, ctx);
        return;
      }
...
template <typename I>
void
SnapshotCopyRequest<I>::handle_resize_object_map(int r)
{
  ldout(m_cct, 20) << "r=" << r << dendl;
...
template <typename I>
bool
SnapshotCopyRequest<I>::handle_cancellation()
{
  {
    std::lock_guard locker{m_lock};
    if (!m_canceled) {
...
template <typename I>
void
SnapshotCopyRequest<I>::error(int r)
{
  ldout(m_cct, 20) << "r=" << r << dendl;
...
template <typename I>
int
SnapshotCopyRequest<I>::validate_parent(
    I* image_ctx,
...
template <typename I>
Context*
SnapshotCopyRequest<I>::start_lock_op(int* r)
{
  std::shared_lock owner_locker{m_dst_image_ctx->owner_lock};
...
template <typename I>
Context*
SnapshotCopyRequest<I>::start_lock_op(ceph::shared_mutex& owner_lock, int* r)
{
...
template <typename I>
void
SnapshotCopyRequest<I>::finish(int r)
{
  ldout(m_cct, 20) << "r=" << r << dendl;
