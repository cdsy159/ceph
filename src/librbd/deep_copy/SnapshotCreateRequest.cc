...
#undef dout_prefix
#define dout_prefix                                                     \
  *_dout << "librbd::deep_copy::SnapshotCreateRequest: " << this << " " \
         << __func__ << ": "
  ...
  m_cct(dst_image_ctx->cct)
{
  ...
  ldout(m_cct, 20) << "dst_image_id=" << m_dst_image_ctx->id
                   ...
                   << ", size=" << m_size << ", parent_spec=" << m_parent_spec
                   << ", parent_overlap=" << m_parent_overlap << dendl;
}

template <typename I>
void
SnapshotCreateRequest<I>::send()
{
  send_set_head();
}
...
template <typename I>
void
SnapshotCreateRequest<I>::send_set_head()
{
  ldout(m_cct, 20) << dendl;

  auto ctx = create_context_callback<
      SnapshotCreateRequest<I>, &SnapshotCreateRequest<I>::handle_set_head>(
      this);
  auto req = SetHeadRequest<I>::create(
      m_dst_image_ctx, m_size, m_parent_spec, m_parent_overlap, ctx);
  req->send();
}

template <typename I>
void
SnapshotCreateRequest<I>::handle_set_head(int r)
{
...
template <typename I>
void
SnapshotCreateRequest<I>::handle_create_snap(int r)
{
  ldout(m_cct, 20) << "r=" << r << dendl;
...
SnapshotCreateRequest<I>::send_create_object_map()
{
  ...
  std::string object_map_oid(
      librbd::ObjectMap<>::object_map_name(m_dst_image_ctx->id, local_snap_id));
  uint64_t object_count =
      Striper::get_num_objects(m_dst_image_ctx->layout, m_size);
...
template <typename I>
void
SnapshotCreateRequest<I>::handle_create_object_map(int r)
{
  ldout(m_cct, 20) << "r=" << r << dendl;
...
template <typename I>
Context*
SnapshotCreateRequest<I>::start_lock_op(int* r)
{
  std::shared_lock owner_locker{m_dst_image_ctx->owner_lock};
...
template <typename I>
void
SnapshotCreateRequest<I>::finish(int r)
{
  ldout(m_cct, 20) << "r=" << r << dendl;
