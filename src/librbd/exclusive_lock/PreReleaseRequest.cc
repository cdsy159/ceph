...
#undef dout_prefix
#define dout_prefix                                                      \
  *_dout << "librbd::exclusive_lock::PreReleaseRequest: " << this << " " \
         << __func__ << ": "
...
PreReleaseRequest<I>::create(
    ...
    Context* on_finish)
{
  return new PreReleaseRequest(
      image_ctx, image_dispatch, shutting_down, async_op_tracker, on_finish);
}
...
PreReleaseRequest<I>::send_cancel_op_requests()
{
  ...
  Context* ctx =
      create_context_callback<klass, &klass::handle_cancel_op_requests>(this);
  m_image_ctx.cancel_async_requests(ctx);
}

template <typename I>
void
PreReleaseRequest<I>::handle_cancel_op_requests(int r)
{
  CephContext* cct = m_image_ctx.cct;
...
template <typename I>
void
PreReleaseRequest<I>::send_set_require_lock()
{
  ...
  using klass = PreReleaseRequest<I>;
  Context* ctx =
      create_context_callback<klass, &klass::handle_set_require_lock>(this);
  ...
  if (m_image_ctx.clone_copy_on_read ||
    ...
    m_image_dispatch->set_require_lock(
        m_shutting_down, io::DIRECTION_WRITE, ctx);
  }
}

template <typename I>
void
PreReleaseRequest<I>::handle_set_require_lock(int r)
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << "r=" << r << dendl;
...
template <typename I>
void
PreReleaseRequest<I>::send_wait_for_ops()
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << dendl;
...
template <typename I>
void
PreReleaseRequest<I>::handle_wait_for_ops(int r)
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << dendl;
...
template <typename I>
void
PreReleaseRequest<I>::send_prepare_lock()
{
  if (m_shutting_down) {
...
template <typename I>
void
PreReleaseRequest<I>::handle_prepare_lock(int r)
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << "r=" << r << dendl;
...
template <typename I>
void
PreReleaseRequest<I>::send_process_plugin_release_lock()
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << dendl;
...
template <typename I>
void
PreReleaseRequest<I>::handle_process_plugin_release_lock(int r)
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << "r=" << r << dendl;
...
template <typename I>
void
PreReleaseRequest<I>::send_invalidate_cache()
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << dendl;

  Context* ctx = create_context_callback<
      PreReleaseRequest<I>, &PreReleaseRequest<I>::handle_invalidate_cache>(
      this);
...
template <typename I>
void
PreReleaseRequest<I>::handle_invalidate_cache(int r)
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << "r=" << r << dendl;
...
template <typename I>
void
PreReleaseRequest<I>::send_flush_io()
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << dendl;
...
template <typename I>
void
PreReleaseRequest<I>::handle_flush_io(int r)
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << "r=" << r << dendl;
...
template <typename I>
void
PreReleaseRequest<I>::send_flush_notifies()
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << dendl;
...
template <typename I>
void
PreReleaseRequest<I>::handle_flush_notifies(int r)
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << dendl;
...
template <typename I>
void
PreReleaseRequest<I>::send_close_journal()
{
  {
...
template <typename I>
void
PreReleaseRequest<I>::handle_close_journal(int r)
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << "r=" << r << dendl;
...
template <typename I>
void
PreReleaseRequest<I>::send_close_object_map()
{
  {
    std::unique_lock image_locker{m_image_ctx.image_lock};
  ...
  using klass = PreReleaseRequest<I>;
  Context* ctx = create_context_callback<klass, &klass::handle_close_object_map>(
      this, m_object_map);
...
template <typename I>
void
PreReleaseRequest<I>::handle_close_object_map(int r)
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << "r=" << r << dendl;
...
template <typename I>
void
PreReleaseRequest<I>::send_unlock()
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << dendl;
