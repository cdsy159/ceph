...
#undef dout_prefix
#define dout_prefix                                                          \
  *_dout << "librbd::image::AttachChildRequest: " << this << " " << __func__ \
         << ": "
...
template <typename I>
void
AttachChildRequest<I>::v1_add_child()
{
  ...
  cls_client::add_child(
      &op,
      {m_parent_image_ctx->md_ctx.get_id(), "", m_parent_image_ctx->id,
       m_parent_snap_id},
      m_image_ctx->id);
...
template <typename I>
void
AttachChildRequest<I>::handle_v1_add_child(int r)
{
...
template <typename I>
void
AttachChildRequest<I>::v1_refresh()
{
  ldout(m_cct, 15) << dendl;
...
template <typename I>
void
AttachChildRequest<I>::handle_v1_refresh(int r)
{
  ldout(m_cct, 15) << "r=" << r << dendl;
...
template <typename I>
void
AttachChildRequest<I>::v1_remove_child_from_old_parent()
{
...
template <typename I>
void
AttachChildRequest<I>::handle_v1_remove_child_from_old_parent(int r)
{
  ldout(m_cct, 15) << "r=" << r << dendl;
...
template <typename I>
void
AttachChildRequest<I>::v2_set_op_feature()
{
  ldout(m_cct, 15) << dendl;

  librados::ObjectWriteOperation op;
  cls_client::op_features_set(
      &op, RBD_OPERATION_FEATURE_CLONE_CHILD, RBD_OPERATION_FEATURE_CLONE_CHILD);

  using klass = AttachChildRequest<I>;
  auto aio_comp =
      create_rados_callback<klass, &klass::handle_v2_set_op_feature>(this);
  int r =
...
template <typename I>
void
AttachChildRequest<I>::handle_v2_set_op_feature(int r)
{
  ldout(m_cct, 15) << "r=" << r << dendl;
...
template <typename I>
void
AttachChildRequest<I>::v2_child_attach()
{
  ldout(m_cct, 15) << dendl;
  ...
  using klass = AttachChildRequest<I>;
  auto aio_comp =
      create_rados_callback<klass, &klass::handle_v2_child_attach>(this);
  int r = m_parent_image_ctx->md_ctx.aio_operate(
      m_parent_image_ctx->header_oid, aio_comp, &op);
  ceph_assert(r == 0);
  aio_comp->release();
}
...
template <typename I>
void
AttachChildRequest<I>::handle_v2_child_attach(int r)
{
  ldout(m_cct, 15) << "r=" << r << dendl;
...
template <typename I>
void
AttachChildRequest<I>::v2_child_detach_from_old_parent()
{
  if (m_old_parent_image_ctx == nullptr) {
    finish(0);
    return;
...
template <typename I>
void
AttachChildRequest<I>::handle_v2_child_detach_from_old_parent(int r)
{
  ldout(m_cct, 15) << "r=" << r << dendl;
...
template <typename I>
void
AttachChildRequest<I>::finish(int r)
{
