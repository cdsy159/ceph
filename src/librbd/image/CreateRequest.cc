...
#undef dout_prefix
#define dout_prefix \
  *_dout << "librbd::image::CreateRequest: " << __func__ << ": "
...
int
validate_features(CephContext* cct, uint64_t features)
{
    ...
    uint64_t stripe_count)
{
  if ((stripe_unit && !stripe_count) || (!stripe_unit && stripe_count)) {
    ...
    return -EINVAL;
      ...
      stripe_unit &&
      ((1ull << order) % stripe_unit || stripe_unit > (1ull << order))) {
...
bool
validate_layout(CephContext* cct, uint64_t size, file_layout_t& layout)
{
...
int
get_image_option(const ImageOptions& image_options, int option, uint8_t* value)
{
...
template <typename I>
int
CreateRequest<I>::validate_order(CephContext* cct, uint8_t order)
{
...
#undef dout_prefix
#define dout_prefix \
  *_dout << "librbd::image::CreateRequest: " << this << " " << __func__ << ": "
  ...
  m_on_finish(on_finish)
{
  ...
  if (get_image_option(
          image_options, RBD_IMAGE_OPTION_JOURNAL_ORDER, &m_journal_order) !=
      0) {
    m_journal_order = m_config.get_val<uint64_t>("rbd_journal_order");
  }
  if (get_image_option(
          image_options, RBD_IMAGE_OPTION_JOURNAL_SPLAY_WIDTH,
          &m_journal_splay_width) != 0) {
    m_journal_splay_width =
        m_config.get_val<uint64_t>("rbd_journal_splay_width");
  }
  ...
  ldout(m_cct, 10) << "name=" << m_image_name << ", " << "id=" << m_image_id
                   << ", " << "size=" << m_size << ", "
                   << "features=" << m_features << ", "
...
template <typename I>
void
CreateRequest<I>::validate_data_pool()
{
...
template <typename I>
void
CreateRequest<I>::handle_validate_data_pool(int r)
{
...
template <typename I>
void
CreateRequest<I>::add_image_to_directory()
{
  ldout(m_cct, 15) << dendl;
...
template <typename I>
void
CreateRequest<I>::handle_add_image_to_directory(int r)
{
  ldout(m_cct, 15) << "r=" << r << dendl;
...
template <typename I>
void
CreateRequest<I>::create_id_object()
{
  ldout(m_cct, 15) << dendl;
...
template <typename I>
void
CreateRequest<I>::handle_create_id_object(int r)
{
  ldout(m_cct, 15) << "r=" << r << dendl;
...
template <typename I>
void
CreateRequest<I>::negotiate_features()
{
...
template <typename I>
void
CreateRequest<I>::handle_negotiate_features(int r)
{
  ldout(m_cct, 15) << "r=" << r << dendl;
...
CreateRequest<I>::create_image()
{
  ...
  librados::ObjectWriteOperation op;
  op.create(true);
  cls_client::create_image(
      &op, m_size, m_order, m_features, oss.str(), m_data_pool_id);
...
template <typename I>
void
CreateRequest<I>::handle_create_image(int r)
{
  ldout(m_cct, 15) << "r=" << r << dendl;
...
template <typename I>
void
CreateRequest<I>::set_stripe_unit_count()
{
...
template <typename I>
void
CreateRequest<I>::handle_set_stripe_unit_count(int r)
{
  ldout(m_cct, 15) << "r=" << r << dendl;

  if (r < 0) {
    lderr(m_cct) << "error setting stripe unit/count: " << cpp_strerror(r)
                 << dendl;
    m_r_saved = r;
...
template <typename I>
void
CreateRequest<I>::object_map_resize()
{
  if ((m_features & RBD_FEATURE_OBJECT_MAP) == 0) {
    fetch_mirror_mode();
    return;
  ...
  cls_client::object_map_resize(
      &op, Striper::get_num_objects(m_layout, m_size), OBJECT_NONEXISTENT);
...
template <typename I>
void
CreateRequest<I>::handle_object_map_resize(int r)
{
  ldout(m_cct, 15) << "r=" << r << dendl;

  if (r < 0) {
    lderr(m_cct) << "error creating initial object map: " << cpp_strerror(r)
                 << dendl;
...
template <typename I>
void
CreateRequest<I>::fetch_mirror_mode()
{
...
template <typename I>
void
CreateRequest<I>::handle_fetch_mirror_mode(int r)
{
  ldout(m_cct, 15) << "r=" << r << dendl;
...
template <typename I>
void
CreateRequest<I>::journal_create()
{
  ldout(m_cct, 15) << dendl;

  using klass = CreateRequest<I>;
  Context* ctx =
      create_context_callback<klass, &klass::handle_journal_create>(this);
  ...
  tag_data.mirror_uuid =
      (use_primary_mirror_uuid ? m_primary_mirror_uuid
...
template <typename I>
void
CreateRequest<I>::handle_journal_create(int r)
{
  ldout(m_cct, 15) << "r=" << r << dendl;
...
template <typename I>
void
CreateRequest<I>::mirror_image_enable()
{
  ...
  auto req = mirror::EnableRequest<I>::create(
      ...
      true, m_op_work_queue, ctx);
  req->send();
}

template <typename I>
void
CreateRequest<I>::handle_mirror_image_enable(int r)
{
  ldout(m_cct, 15) << "r=" << r << dendl;
...
template <typename I>
void
CreateRequest<I>::journal_remove()
{
  if ((m_features & RBD_FEATURE_JOURNALING) == 0) {
    remove_object_map();
    return;
...
template <typename I>
void
CreateRequest<I>::handle_journal_remove(int r)
{
  ldout(m_cct, 15) << "r=" << r << dendl;
...
template <typename I>
void
CreateRequest<I>::remove_object_map()
{
  if ((m_features & RBD_FEATURE_OBJECT_MAP) == 0) {
    remove_header_object();
    return;
...
template <typename I>
void
CreateRequest<I>::handle_remove_object_map(int r)
{
  ldout(m_cct, 15) << "r=" << r << dendl;
...
template <typename I>
void
CreateRequest<I>::remove_header_object()
{
  ldout(m_cct, 15) << dendl;
...
template <typename I>
void
CreateRequest<I>::handle_remove_header_object(int r)
{
  ldout(m_cct, 15) << "r=" << r << dendl;
...
template <typename I>
void
CreateRequest<I>::remove_id_object()
{
  ldout(m_cct, 15) << dendl;
...
template <typename I>
void
CreateRequest<I>::handle_remove_id_object(int r)
{
  ldout(m_cct, 15) << "r=" << r << dendl;
...
template <typename I>
void
CreateRequest<I>::remove_from_dir()
{
  ldout(m_cct, 15) << dendl;
...
template <typename I>
void
CreateRequest<I>::handle_remove_from_dir(int r)
{
  ldout(m_cct, 15) << "r=" << r << dendl;
