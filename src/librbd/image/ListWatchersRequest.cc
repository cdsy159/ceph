...
#undef dout_prefix
#define dout_prefix                                                           \
  *_dout << "librbd::image::ListWatchersRequest: " << this << " " << __func__ \
         << ": "

static std::ostream&
operator<<(std::ostream& os, const obj_watch_t& watch)
{
  os << "{addr=" << watch.addr << ", " << "watcher_id=" << watch.watcher_id
     << ", " << "cookie=" << watch.cookie << "}";
  return os;
}
...
template <typename I>
ListWatchersRequest<I>::ListWatchersRequest(
    I& image_ctx,
    int flags,
...
template <typename I>
void
ListWatchersRequest<I>::list_image_watchers()
{
  ldout(m_cct, 20) << dendl;
...
template <typename I>
void
ListWatchersRequest<I>::handle_list_image_watchers(int r)
{
...
template <typename I>
void
ListWatchersRequest<I>::list_mirror_watchers()
{
  ...
  m_out_bl.clear();
  int r = m_image_ctx.md_ctx.aio_operate(
      RBD_MIRRORING, rados_completion, &op, &m_out_bl);
  ceph_assert(r == 0);
  rados_completion->release();
}
...
template <typename I>
void
ListWatchersRequest<I>::handle_list_mirror_watchers(int r)
{
  ldout(m_cct, 20) << "r=" << r << dendl;
...
template <typename I>
void
ListWatchersRequest<I>::finish(int r)
{
  ldout(m_cct, 20) << "r=" << r << dendl;
  ...
  if (r == 0) {
    ...
    if (m_object_watchers.size() > 0) {
      ...
      uint64_t watch_handle = m_image_ctx.image_watcher != nullptr
                                  ? m_image_ctx.image_watcher->get_watch_handle()
                                  : 0;
      ...
      for (auto& w : m_object_watchers) {
        ...
        auto it = std::find_if(
            m_mirror_watchers.begin(), m_mirror_watchers.end(),
        ...
        if ((m_flags & LIST_WATCHERS_FILTER_OUT_MIRROR_INSTANCES) != 0) {
          ...
          if (it == m_mirror_watchers.end()) {
            ldout(m_cct, 20)
                << "filtering out non-mirror instance: " << w << dendl;
            continue;
          }
        }
