...
template <typename I>
void
RefreshRequest<I>::send_get_migration_header()
{
...
template <typename I>
Context*
RefreshRequest<I>::handle_get_migration_header(int* result)
{
  CephContext* cct = m_image_ctx.cct;
  ...
  case cls::rbd::MIGRATION_HEADER_TYPE_SRC:
    ...
    ldout(cct, 1) << this << " " << __func__
                  << ": migrating to: " << m_migration_spec << dendl;
    break;
  case cls::rbd::MIGRATION_HEADER_TYPE_DST:
    ldout(cct, 1) << this << " " << __func__
                  << ": migrating from: " << m_migration_spec << dendl;
    switch (m_migration_spec.state) {
    case cls::rbd::MIGRATION_STATE_PREPARING:
      ldout(cct, 5) << this << " " << __func__
                    << ": current migration state: " << m_migration_spec.state
                    << ", retrying" << dendl;
      send();
      return nullptr;
...
template <typename I>
void
RefreshRequest<I>::send_v1_read_header()
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << this << " " << __func__ << dendl;
  ...
  using klass = RefreshRequest<I>;
  librados::AioCompletion* comp =
      create_rados_callback<klass, &klass::handle_v1_read_header>(this);
  m_out_bl.clear();
  int r = m_image_ctx.md_ctx.aio_operate(
      m_image_ctx.header_oid, comp, &op, &m_out_bl);
  ceph_assert(r == 0);
  comp->release();
}
...
template <typename I>
Context*
RefreshRequest<I>::handle_v1_read_header(int* result)
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << this << " " << __func__ << ": " << "r=" << *result << dendl;
  ...
  if (*result < 0) {
    ...
    *result = -EIO;
    return m_on_finish;
      ...
      memcmp(RBD_HEADER_TEXT, m_out_bl.c_str(), sizeof(RBD_HEADER_TEXT)) != 0) {
    if (memcmp(
            RBD_MIGRATE_HEADER_TEXT, m_out_bl.c_str(),
            sizeof(RBD_MIGRATE_HEADER_TEXT)) == 0) {
      ldout(cct, 1) << this << " " << __func__
                    << ": migration v1 header detected" << dendl;
      migrating = true;
    } else {
...
template <typename I>
void
RefreshRequest<I>::send_v1_get_snapshots()
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << this << " " << __func__ << dendl;
  ...
  using klass = RefreshRequest<I>;
  librados::AioCompletion* comp =
      create_rados_callback<klass, &klass::handle_v1_get_snapshots>(this);
  m_out_bl.clear();
  int r = m_image_ctx.md_ctx.aio_operate(
      m_image_ctx.header_oid, comp, &op, &m_out_bl);
  ceph_assert(r == 0);
...
template <typename I>
Context*
RefreshRequest<I>::handle_v1_get_snapshots(int* result)
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << this << " " << __func__ << ": " << "r=" << *result << dendl;
  ...
  if (*result >= 0) {
    ...
    *result = cls_client::old_snapshot_list_finish(
        &it, &snap_names, &snap_sizes, &m_snapc);
  }
...
template <typename I>
void
RefreshRequest<I>::send_v1_get_locks()
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << this << " " << __func__ << dendl;
  ...
  using klass = RefreshRequest<I>;
  librados::AioCompletion* comp =
      create_rados_callback<klass, &klass::handle_v1_get_locks>(this);
  m_out_bl.clear();
  int r = m_image_ctx.md_ctx.aio_operate(
      m_image_ctx.header_oid, comp, &op, &m_out_bl);
  ceph_assert(r == 0);
...
template <typename I>
Context*
RefreshRequest<I>::handle_v1_get_locks(int* result)
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << this << " " << __func__ << ": " << "r=" << *result << dendl;
  ...
  if (*result >= 0) {
    ...
    *result = rados::cls::lock::get_lock_info_finish(
        &it, &m_lockers, &lock_type, &m_lock_tag);
    if (*result >= 0) {
...
template <typename I>
void
RefreshRequest<I>::send_v1_apply()
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << this << " " << __func__ << dendl;
...
template <typename I>
Context*
RefreshRequest<I>::handle_v1_apply(int* result)
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << this << " " << __func__ << dendl;
...
template <typename I>
void
RefreshRequest<I>::send_v2_get_mutable_metadata()
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << this << " " << __func__ << dendl;
  ...
  using klass = RefreshRequest<I>;
  librados::AioCompletion* comp =
      create_rados_callback<klass, &klass::handle_v2_get_mutable_metadata>(this);
  m_out_bl.clear();
  int r = m_image_ctx.md_ctx.aio_operate(
      m_image_ctx.header_oid, comp, &op, &m_out_bl);
  ceph_assert(r == 0);
...
template <typename I>
Context*
RefreshRequest<I>::handle_v2_get_mutable_metadata(int* result)
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << this << " " << __func__ << ": " << "r=" << *result << dendl;
  ...
  if (*result >= 0) {
    *result = cls_client::get_features_finish(
        &it, &m_features, &m_incompatible_features);
  }
  ...
  if (((m_incompatible_features & RBD_FEATURE_NON_PRIMARY) != 0U) &&
    ...
    m_read_only_flags |= IMAGE_READ_ONLY_FLAG_NON_PRIMARY;
      ...
      (((m_incompatible_features & RBD_FEATURE_NON_PRIMARY) == 0U) ||
       ((m_image_ctx.read_only_mask & IMAGE_READ_ONLY_FLAG_NON_PRIMARY) == 0U)) &&
      ((m_read_only_flags & IMAGE_READ_ONLY_FLAG_NON_PRIMARY) != 0U)) {
...
template <typename I>
void
RefreshRequest<I>::send_v2_get_parent()
{
  ...
  m_out_bl.clear();
  m_image_ctx.md_ctx.aio_operate(
...
template <typename I>
Context*
RefreshRequest<I>::handle_v2_get_parent(int* result)
{
  // NOTE: remove support when Mimic is EOLed
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << this << " " << __func__ << ": r=" << *result << dendl;
  ...
  if (!m_legacy_parent) {
    ...
    *result = cls_client::get_parent_finish(
        &it, &m_parent_md.spec, &m_parent_md.overlap);
    m_head_parent_overlap = true;
  }
...
template <typename I>
Context*
RefreshRequest<I>::handle_v2_get_metadata(int* result)
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << this << " " << __func__ << ": r=" << *result << dendl;
...
template <typename I>
Context*
RefreshRequest<I>::handle_v2_get_pool_metadata(int* result)
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << this << " " << __func__ << ": r=" << *result << dendl;
...
template <typename I>
void
RefreshRequest<I>::send_v2_get_op_features()
{
...
template <typename I>
Context*
RefreshRequest<I>::handle_v2_get_op_features(int* result)
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << this << " " << __func__ << ": " << "r=" << *result << dendl;
...
template <typename I>
void
RefreshRequest<I>::send_v2_get_group()
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << this << " " << __func__ << dendl;
  ...
  using klass = RefreshRequest<I>;
  librados::AioCompletion* comp =
      create_rados_callback<klass, &klass::handle_v2_get_group>(this);
  m_out_bl.clear();
  int r = m_image_ctx.md_ctx.aio_operate(
      m_image_ctx.header_oid, comp, &op, &m_out_bl);
  ceph_assert(r == 0);
...
template <typename I>
Context*
RefreshRequest<I>::handle_v2_get_group(int* result)
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << this << " " << __func__ << ": " << "r=" << *result << dendl;
...
template <typename I>
void
RefreshRequest<I>::send_v2_get_snapshots()
{
  ...
  using klass = RefreshRequest<I>;
  librados::AioCompletion* comp =
      create_rados_callback<klass, &klass::handle_v2_get_snapshots>(this);
  m_out_bl.clear();
  int r = m_image_ctx.md_ctx.aio_operate(
      m_image_ctx.header_oid, comp, &op, &m_out_bl);
  ceph_assert(r == 0);
...
template <typename I>
Context*
RefreshRequest<I>::handle_v2_get_snapshots(int* result)
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << this << " " << __func__ << ": " << "r=" << *result << dendl;
  ...
  for (size_t i = 0; i < m_snapc.snaps.size(); ++i) {
    if (m_legacy_snapshot != LEGACY_SNAPSHOT_DISABLED) {
      ...
      if (*result >= 0 &&
        ...
        *result =
            cls_client::get_snapshot_timestamp_finish(&it, &snap_timestamp);
      }

      if (*result >= 0) {
        m_snap_infos[i] = {
            m_snapc.snaps[i], {cls::rbd::UserSnapshotNamespace{}},
            snap_name,        snap_size,
            snap_timestamp,   0};
      }
    } else if (*result >= 0) {
    ...
    if (*result >= 0) {
      if (m_legacy_parent) {
        *result = cls_client::get_parent_finish(
            &it, &m_snap_parents[i].spec, &m_snap_parents[i].overlap);
      } else {
        std::optional<uint64_t> parent_overlap;
        *result = cls_client::parent_overlap_get_finish(&it, &parent_overlap);
        if (*result >= 0) {
    ...
    if (*result >= 0) {
      *result =
          cls_client::get_protection_status_finish(&it, &m_snap_protection[i]);
    }
  ...
  if (*result == -ENOENT && m_enoent_retries++ < MAX_ENOENT_RETRIES) {
    ...
    return nullptr;
      ...
      m_legacy_snapshot == LEGACY_SNAPSHOT_DISABLED && *result == -EOPNOTSUPP) {
    ldout(cct, 10) << "retrying using legacy snapshot methods" << dendl;
    m_legacy_snapshot = LEGACY_SNAPSHOT_ENABLED;
    send_v2_get_snapshots();
    return nullptr;
  } else if (
      m_legacy_snapshot == LEGACY_SNAPSHOT_ENABLED && *result == -EOPNOTSUPP) {
...
{
  {
    ...
    if (!m_skip_open_parent_image &&
        (r < 0 || RefreshParentRequest<I>::is_refresh_required(
                      m_image_ctx, parent_md, migration_info))) {
      CephContext* cct = m_image_ctx.cct;
      ldout(cct, 10) << this << " " << __func__ << dendl;

      using klass = RefreshRequest<I>;
      Context* ctx =
          create_context_callback<klass, &klass::handle_v2_refresh_parent>(this);
...
template <typename I>
Context*
RefreshRequest<I>::handle_v2_refresh_parent(int* result)
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << this << " " << __func__ << ": r=" << *result << dendl;
...
{
  if ((m_features & RBD_FEATURE_EXCLUSIVE_LOCK) == 0 || m_read_only ||
      !m_image_ctx.snap_name.empty() || m_image_ctx.exclusive_lock != nullptr) {
  ...
  using klass = RefreshRequest<I>;
  Context* ctx =
      create_context_callback<klass, &klass::handle_v2_init_exclusive_lock>(
          this);
...
template <typename I>
Context*
RefreshRequest<I>::handle_v2_init_exclusive_lock(int* result)
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << this << " " << __func__ << ": r=" << *result << dendl;
...
RefreshRequest<I>::send_v2_open_journal()
{
  bool journal_disabled =
      ((m_features & RBD_FEATURE_JOURNALING) == 0 || m_read_only ||
  ...
  if (journal_disabled || journal_disabled_by_policy) {
    ...
    if ((m_features & RBD_FEATURE_JOURNALING) != 0 &&
        !journal_disabled_by_policy && m_image_ctx.exclusive_lock != nullptr &&
  ...
  using klass = RefreshRequest<I>;
  Context* ctx =
      create_context_callback<klass, &klass::handle_v2_open_journal>(this);
...
template <typename I>
Context*
RefreshRequest<I>::handle_v2_open_journal(int* result)
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << this << " " << __func__ << ": r=" << *result << dendl;
...
template <typename I>
Context*
RefreshRequest<I>::handle_v2_block_writes(int* result)
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << this << " " << __func__ << ": r=" << *result << dendl;
...
template <typename I>
void
RefreshRequest<I>::send_v2_open_object_map()
{
  if ((m_features & RBD_FEATURE_OBJECT_MAP) == 0 ||
      m_image_ctx.object_map != nullptr ||
      (m_image_ctx.snap_name.empty() &&
       (m_read_only || m_image_ctx.exclusive_lock == nullptr ||
        !m_image_ctx.exclusive_lock->is_lock_owner()))) {
  ...
  if (m_image_ctx.snap_name.empty()) {
    ...
    for (size_t snap_idx = 0; snap_idx < m_snap_infos.size(); ++snap_idx) {
      if (m_snap_infos[snap_idx].name == m_image_ctx.snap_name) {
        m_object_map =
            m_image_ctx.create_object_map(m_snapc.snaps[snap_idx].val);
        break;
      }
    }
  ...
  using klass = RefreshRequest<I>;
  Context* ctx =
      create_context_callback<klass, &klass::handle_v2_open_object_map>(this);
  m_object_map->open(ctx);
}

template <typename I>
Context*
RefreshRequest<I>::handle_v2_open_object_map(int* result)
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << this << " " << __func__ << ": r=" << *result << dendl;
...
template <typename I>
void
RefreshRequest<I>::send_v2_apply()
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << this << " " << __func__ << dendl;
...
template <typename I>
Context*
RefreshRequest<I>::handle_v2_apply(int* result)
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << this << " " << __func__ << dendl;
...
RefreshRequest<I>::send_v2_finalize_refresh_parent()
{
  ...
  using klass = RefreshRequest<I>;
  Context* ctx =
      create_context_callback<klass, &klass::handle_v2_finalize_refresh_parent>(
          this);
...
template <typename I>
Context*
RefreshRequest<I>::handle_v2_finalize_refresh_parent(int* result)
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << this << " " << __func__ << ": r=" << *result << dendl;
...
template <typename I>
Context*
RefreshRequest<I>::send_v2_shut_down_exclusive_lock()
{
  ...
  using klass = RefreshRequest<I>;
  Context* ctx =
      create_context_callback<klass, &klass::handle_v2_shut_down_exclusive_lock>(
          this);
...
template <typename I>
Context*
RefreshRequest<I>::handle_v2_shut_down_exclusive_lock(int* result)
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << this << " " << __func__ << ": r=" << *result << dendl;
...
template <typename I>
Context*
RefreshRequest<I>::send_v2_close_journal()
{
  ...
  using klass = RefreshRequest<I>;
  Context* ctx =
      create_context_callback<klass, &klass::handle_v2_close_journal>(this);
  m_journal->close(ctx);
  return nullptr;
}
...
template <typename I>
Context*
RefreshRequest<I>::handle_v2_close_journal(int* result)
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << this << " " << __func__ << ": r=" << *result << dendl;
...
template <typename I>
Context*
RefreshRequest<I>::send_v2_close_object_map()
{
  if (m_object_map == nullptr) {
    return send_flush_aio();
  }
  ...
  using klass = RefreshRequest<I>;
  Context* ctx =
      create_context_callback<klass, &klass::handle_v2_close_object_map>(this);
...
template <typename I>
Context*
RefreshRequest<I>::handle_v2_close_object_map(int* result)
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << this << " " << __func__ << ": r=" << *result << dendl;
...
template <typename I>
Context*
RefreshRequest<I>::send_flush_aio()
{
...
template <typename I>
Context*
RefreshRequest<I>::handle_flush_aio(int* result)
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 10) << this << " " << __func__ << ": r=" << *result << dendl;
...
template <typename I>
Context*
RefreshRequest<I>::handle_error(int* result)
{
  if (m_error_result < 0) {
...
RefreshRequest<I>::apply()
{
  ...
  for (size_t i = 0; i < m_snapc.snaps.size(); ++i) {
    ...
    m_image_ctx.add_snap(
        m_snap_infos[i].snapshot_namespace, m_snap_infos[i].name,
        m_snapc.snaps[i].val, m_snap_infos[i].image_size, parent,
        protection_status, flags, m_snap_infos[i].timestamp);
  }
  m_image_ctx.parent_md.overlap = std::min(overlap, m_image_ctx.size);
  m_image_ctx.snapc = m_snapc;

  if (m_image_ctx.snap_id != CEPH_NOSNAP &&
      m_image_ctx.get_snap_id(
          m_image_ctx.snap_namespace, m_image_ctx.snap_name) !=
          m_image_ctx.snap_id) {
  ...
  if (m_image_ctx.exclusive_lock != nullptr &&
      !m_image_ctx.test_features(
          RBD_FEATURE_EXCLUSIVE_LOCK, m_image_ctx.image_lock)) {
    ...
    if (!m_image_ctx.test_features(
            RBD_FEATURE_OBJECT_MAP, m_image_ctx.image_lock) ||
        m_object_map != nullptr) {
...
template <typename I>
int
RefreshRequest<I>::get_parent_info(
    uint64_t snap_id,
    ParentImageInfo* parent_md,
    MigrationInfo* migration_info)
{
...
template <typename I>
int
RefreshRequest<I>::get_migration_info(
    ParentImageInfo* parent_md,
    MigrationInfo* migration_info,
    bool* migration_info_valid)
{
  ...
  auto it =
      std::upper_bound(m_snapc.snaps.rbegin(), m_snapc.snaps.rend(), snap_id);
  ...
  *migration_info_valid = true;
  ...
  deep_copy::util::compute_snap_map(
      m_image_ctx.cct, 0, CEPH_NOSNAP, {}, snap_seqs, &migration_info->snap_map);
  return 0;
}
