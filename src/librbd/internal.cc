...
int
validate_pool(IoCtx& io_ctx, CephContext* cct)
{
...
int
detect_format(IoCtx& io_ctx, const string& name, bool* old_format, uint64_t* size)
{
  ...
  ldout(cct, 20) << "detect format of " << name << " : "
                 << (old_format ? (*old_format ? "old" : "new") : "don't care")
                 << dendl;
  return 0;
}

bool
has_parent(int64_t parent_pool_id, uint64_t off, uint64_t overlap)
{
    ...
    uint64_t bid)
{
  ...
  snprintf(
      ondisk.block_name, sizeof(ondisk.block_name), "rb.%x.%x.%x", hi, lo,
      extra);
...
void
image_info(ImageCtx* ictx, image_info_t& info, size_t infosize)
{
  ...
  strncpy(
      info.block_name_prefix, ictx->object_prefix.c_str(),
...
uint64_t
oid_to_object_no(const string& oid, const string& object_prefix)
{
...
#define READ_SIZE 4096
  ...
  static_assert(
      sizeof(RBD_HEADER_TEXT) == sizeof(RBD_MIGRATE_HEADER_TEXT),
  ...
  if (header.length() < sizeof(RBD_HEADER_TEXT) ||
      (memcmp(RBD_HEADER_TEXT, header.c_str(), sizeof(RBD_HEADER_TEXT)) != 0 &&
       memcmp(
           RBD_MIGRATE_HEADER_TEXT, header.c_str(),
...
int
tmap_set(IoCtx& io_ctx, const string& imgname)
{
...
void
image_options_create(rbd_image_options_t* opts)
{
...
void
image_options_create_ref(rbd_image_options_t* opts, rbd_image_options_t orig)
{
...
void
image_options_copy(rbd_image_options_t* opts, const ImageOptions& orig)
{
  image_options_ref* opts_ = new image_options_ref(new image_options_t());
...
void
image_options_destroy(rbd_image_options_t opts)
{
...
int
image_options_set(rbd_image_options_t opts, int optname, uint64_t optval)
{
  image_options_ref* opts_ = static_cast<image_options_ref*>(opts);
...
int
image_options_get(rbd_image_options_t opts, int optname, std::string* optval)
{
  image_options_ref* opts_ = static_cast<image_options_ref*>(opts);
...
int
image_options_get(rbd_image_options_t opts, int optname, uint64_t* optval)
{
  image_options_ref* opts_ = static_cast<image_options_ref*>(opts);
...
int
image_options_is_set(rbd_image_options_t opts, int optname, bool* is_set)
{
...
int
image_options_unset(rbd_image_options_t opts, int optname)
{
  image_options_ref* opts_ = static_cast<image_options_ref*>(opts);
...
void
image_options_clear(rbd_image_options_t opts)
{
  image_options_ref* opts_ = static_cast<image_options_ref*>(opts);
...
bool
image_options_is_empty(rbd_image_options_t opts)
{
  image_options_ref* opts_ = static_cast<image_options_ref*>(opts);
...
int
create_v1(IoCtx& io_ctx, const char* imgname, uint64_t size, int order)
{
  CephContext* cct = (CephContext*)io_ctx.cct();
  ...
  if (r < 0) {
    ...
    if (remove_r < 0) {
      lderr(cct) << "Could not remove image from directory after "
                 << "header creation failed: " << cpp_strerror(remove_r)
                 << dendl;
    }
    return r;
  }
...
int
create(librados::IoCtx& io_ctx, const char* imgname, uint64_t size, int* order)
{
    ...
    bool skip_mirror_enable)
{
  ...
  ldout(cct, 10) << __func__ << " name=" << image_name << ", " << "id= " << id
  ...
  if (old_format) {
    ...
    if (skip_mirror_enable) {
      create_flags = image::CREATE_FLAG_SKIP_MIRROR_ENABLE;
        ...
        opts.get(RBD_IMAGE_OPTION_MIRROR_IMAGE_MODE, &mirror_image_mode) == 0) {
    ...
    const std::string& primary_mirror_uuid)
{
  ...
  ldout(cct, 10) << __func__ << " p_id=" << (p_id ?: "")
                 << ", p_name=" << (p_name ?: "") << ", p_snap_id=" << p_snap_id
                 << ", p_snap_name=" << (p_snap_name ?: "")
                 << ", c_id=" << (c_id ?: "") << ", c_name=" << c_name
                 << ", c_opts=" << c_opts << ", non_primary_global_image_id="
                 << non_primary_global_image_id
                 << ", primary_mirror_uuid=" << primary_mirror_uuid << dendl;
  ...
  if (p_id == nullptr) {
    ...
    if (r < 0) {
      if (r != -ENOENT) {
        lderr(cct) << "failed to retrieve parent image id: " << cpp_strerror(r)
                   << dendl;
      }
      return r;
    }
  ...
  auto* req = image::CloneRequest<>::create(
      ...
      {cls::rbd::UserSnapshotNamespace{}}, p_snap_id, c_ioctx, c_name, clone_id,
      c_opts, cls::rbd::MIRROR_IMAGE_MODE_JOURNAL, non_primary_global_image_id,
      primary_mirror_uuid, asio_engine.get_work_queue(), &cond);
  req->send();
...
int
rename(IoCtx& io_ctx, const char* srcname, const char* dstname)
{
  CephContext* cct = (CephContext*)io_ctx.cct();
  ldout(cct, 20) << "rename " << &io_ctx << " " << srcname << " -> " << dstname
                 << dendl;
  ...
  BOOST_SCOPE_EXIT((ictx)) { ictx->state->close(); }
...
int
info(ImageCtx* ictx, image_info_t& info, size_t infosize)
{
...
int
get_old_format(ImageCtx* ictx, uint8_t* old)
{
  int r = ictx->state->refresh_if_required();
  if (r < 0)
    return r;
...
int
get_size(ImageCtx* ictx, uint64_t* size)
{
  int r = ictx->state->refresh_if_required();
  if (r < 0)
    return r;
...
int
get_features(ImageCtx* ictx, uint64_t* features)
{
  int r = ictx->state->refresh_if_required();
  if (r < 0)
    return r;
...
int
get_overlap(ImageCtx* ictx, uint64_t* overlap)
{
  int r = ictx->state->refresh_if_required();
  if (r < 0)
    return r;
...
int
get_flags(ImageCtx* ictx, uint64_t* flags)
{
  int r = ictx->state->refresh_if_required();
  if (r < 0) {
    return r;
...
int
set_image_notification(ImageCtx* ictx, int fd, int type)
{
  CephContext* cct = ictx->cct;
  ldout(cct, 20) << __func__ << " " << ictx << " fd " << fd << " type" << type
                 << dendl;
...
int
is_exclusive_lock_owner(ImageCtx* ictx, bool* is_owner)
{
  CephContext* cct = ictx->cct;
...
int
lock_acquire(ImageCtx* ictx, rbd_lock_mode_t lock_mode)
{
  CephContext* cct = ictx->cct;
  ...
  C_SaferCond lock_ctx;
  {
    ...
    if (lock_mode == RBD_LOCK_MODE_EXCLUSIVE) {
      ...
      ictx->set_exclusive_lock_policy(new exclusive_lock::TransientPolicy(ictx));
    } else {
      return -EOPNOTSUPP;
    }
    ...
    std::list<std::string>* lock_owners)
{
  ...
  if (r == -ENOENT) {
    ...
    lderr(cct) << "failed to determine current lock owner: " << cpp_strerror(r)
               << dendl;
    return r;
  }
    ...
    size_t sparse_size)
{
  ...
  ImageCtx* dest =
      new librbd::ImageCtx(destname, "", nullptr, dest_md_ctx, false);
  r = dest->state->open(0);
  if (r < 0) {
...
public:
  ...
  finish(int r) override
  {
    ...
    while (offset < length) {
      if (util::calc_sparse_extent(
              m_ptr, m_sparse_size, length, &write_offset, &write_length,
              &offset)) {
        ...
        api::Io<>::aio_write(
            *m_dest, comp, m_offset + write_offset, write_length,
            std::move(*write_bl), LIBRADOS_OP_FLAG_FADVISE_DONTNEED,
            std::move(read_trace));
        write_offset = offset;
        write_length = 0;
      }
...
int
copy(ImageCtx* src, ImageCtx* dest, ProgressContext& prog_ctx, size_t sparse_size)
{
  src->image_lock.lock_shared();
  uint64_t src_size = src->get_image_size(src->snap_id);
  src->image_lock.unlock_shared();
  ...
  if (dest_size < src_size) {
    lderr(cct) << " src size " << src_size << " > dest size " << dest_size
               << dendl;
    return -EINVAL;
  }
  ...
  SimpleThrottle throttle(
      src->config.get_val<uint64_t>("rbd_concurrent_management_ops"), false);
  ...
  for (uint64_t offset = 0; offset < src_size; offset += period) {
    ...
    auto req = io::ImageDispatchSpec::create_read(
        *src, io::IMAGE_DISPATCH_LAYER_NONE, comp, {{offset, len}},
        io::ImageArea::DATA, io::ReadResult{bl}, src->get_data_io_context(),
...
int
lock(ImageCtx* ictx, bool exclusive, const string& cookie, const string& tag)
{
  ...
  {
    std::shared_lock locker{ictx->image_lock};
    r = rados::cls::lock::lock(
        &ictx->md_ctx, ictx->header_oid, RBD_LOCK_NAME,
        exclusive ? ClsLockType::EXCLUSIVE : ClsLockType::SHARED, cookie, tag,
        "", utime_t(), 0);
    if (r < 0) {
      return r;
    }
...
int
unlock(ImageCtx* ictx, const string& cookie)
{
  ldout(ictx->cct, 20) << "unlock image " << ictx << " cookie='" << cookie
                       << "'" << dendl;
  ...
  {
    std::shared_lock locker{ictx->image_lock};
    r = rados::cls::lock::unlock(
        &ictx->md_ctx, ictx->header_oid, RBD_LOCK_NAME, cookie);
    if (r < 0) {
      return r;
    }
  }
...
int
break_lock(ImageCtx* ictx, const string& client, const string& cookie)
{
  ...
  if (ictx->config.get_val<bool>("rbd_blocklist_on_break_lock")) {
    typedef std::map<rados::cls::lock::locker_id_t, rados::cls::lock::locker_info_t>
        Lockers;
    Lockers lockers;
    ClsLockType lock_type;
    std::string lock_tag;
    r = rados::cls::lock::get_lock_info(
        &ictx->md_ctx, ictx->header_oid, RBD_LOCK_NAME, &lockers, &lock_type,
        &lock_tag);
    if (r < 0) {
  ...
  r = rados::cls::lock::break_lock(
      &ictx->md_ctx, ictx->header_oid, RBD_LOCK_NAME, cookie, lock_client);
  if (r < 0)
    return r;
  ictx->notify_update();
  return 0;
...
void
rbd_ctx_cb(completion_t cb, void* arg)
{
    ...
    void* arg)
{
  ...
  while (left > 0) {
    ...
    auto req = io::ImageDispatchSpec::create_read(
        *ictx, io::IMAGE_DISPATCH_LAYER_NONE, c, {{off, read_len}},
        io::ImageArea::DATA, io::ReadResult{&bl}, ictx->get_data_io_context(),
        0, 0, trace);
    req->send();
...
int
clip_io(ImageCtx* ictx, uint64_t off, uint64_t* len, io::ImageArea area)
{
...
int
invalidate_cache(ImageCtx* ictx)
{
  CephContext* cct = ictx->cct;
...
int
poll_io_events(ImageCtx* ictx, io::AioCompletion** comps, int numcomp)
{
  if (numcomp <= 0)
    return -EINVAL;
  CephContext* cct = ictx->cct;
...
int
metadata_get(ImageCtx* ictx, const string& key, string* value)
{
  CephContext* cct = ictx->cct;
...
int
list_watchers(ImageCtx* ictx, std::list<librbd::image_watcher_t>& watchers)
{
  int r;
...
std::ostream&
operator<<(std::ostream& os, const librbd::ImageOptions& opts)
{
  os << "[";
