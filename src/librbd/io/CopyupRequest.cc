...
public:
  ...
  int
  update_snapshot(uint64_t snap_id)
  {
    auto& image_ctx = this->m_image_ctx;
    ceph_assert(ceph_mutex_is_locked(image_ctx.image_lock));
...
template <typename I>
void
CopyupRequest<I>::append_request(
    AbstractObjectWriteRequest<I>* req,
...
template <typename I>
void
CopyupRequest<I>::read_from_parent()
{
  auto cct = m_image_ctx->cct;
  ...
  if (m_image_ctx->parent == nullptr) {
    ...
    m_image_ctx->asio_engine->post([this]() {
      handle_read_from_parent(-ENOENT);
    });
    return;
  ...
  auto comp = AioCompletion::create_and_start<
      CopyupRequest<I>, &CopyupRequest<I>::handle_read_from_parent>(
...
template <typename I>
void
CopyupRequest<I>::handle_read_from_parent(int r)
{
  auto cct = m_image_ctx->cct;
  ...
  if (r < 0) {
    ...
    lderr(m_image_ctx->cct)
        << "failed to prepare copyup data: " << cpp_strerror(r) << dendl;
    finish(r);
    return;
  }
  ...
  if (!m_copyup_is_zero) {
    m_snap_ids.insert(
        m_snap_ids.end(), m_image_ctx->snaps.rbegin(),
...
template <typename I>
void
CopyupRequest<I>::handle_deep_copy(int r)
{
  auto cct = m_image_ctx->cct;
  ldout(cct, 20) << "r=" << r << dendl;
...
template <typename I>
void
CopyupRequest<I>::update_object_maps()
{
  ...
  if (copy_on_read && !m_snap_ids.empty() &&
      m_image_ctx->test_features(
          RBD_FEATURE_FAST_DIFF, m_image_ctx->image_lock)) {
  ...
  typename AsyncObjectThrottle<I>::ContextFactory context_factory(
      boost::lambda::bind(
          boost::lambda::new_ptr<C_UpdateObjectMap<I>>(), boost::lambda::_1,
          m_image_ctx, m_object_no, head_object_map_state, &m_snap_ids,
          m_first_snap_is_clean, m_trace, boost::lambda::_2));
  auto ctx = librbd::util::create_context_callback<
      CopyupRequest<I>, &CopyupRequest<I>::handle_update_object_maps>(this);
  auto throttle = new AsyncObjectThrottle<I>(
      nullptr, *m_image_ctx, context_factory, ctx, nullptr, 0,
      m_snap_ids.size());
  throttle->start_ops(m_image_ctx->config.template get_val<uint64_t>(
      "rbd_concurrent_management_ops"));
}

template <typename I>
void
CopyupRequest<I>::handle_update_object_maps(int r)
{
  auto cct = m_image_ctx->cct;
  ldout(cct, 20) << "r=" << r << dendl;
  ...
  if (r < 0) {
    lderr(m_image_ctx->cct)
        << "failed to update object map: " << cpp_strerror(r) << dendl;
...
template <typename I>
void
CopyupRequest<I>::handle_copyup(int r)
{
  auto cct = m_image_ctx->cct;
...
template <typename I>
void
CopyupRequest<I>::complete_requests(bool override_restart_retval, int r)
{
  auto cct = m_image_ctx->cct;
  remove_from_list();
...
template <typename I>
void
CopyupRequest<I>::disable_append_requests()
{
...
template <typename I>
void
CopyupRequest<I>::remove_from_list()
{
...
template <typename I>
bool
CopyupRequest<I>::is_copyup_required()
{
  ceph_assert(ceph_mutex_is_locked(m_lock));
...
template <typename I>
bool
CopyupRequest<I>::is_deep_copy() const
{
  ceph_assert(ceph_mutex_is_locked(m_image_ctx->image_lock));
...
template <typename I>
bool
CopyupRequest<I>::is_update_object_map_required(int r)
{
  ceph_assert(ceph_mutex_is_locked(m_image_ctx->image_lock));
...
template <typename I>
void
CopyupRequest<I>::compute_deep_copy_snap_ids()
{
  ceph_assert(ceph_mutex_is_locked(m_image_ctx->image_lock));
  ...
  std::copy_if(
      m_image_ctx->snaps.rbegin(), m_image_ctx->snaps.rend(),
      ...
      [this, cct = m_image_ctx->cct, &deep_copied](uint64_t snap_id) {
        ...
        if (r < 0) {
          ...
          object_overlap = m_image_ctx->prune_parent_extents(
...
template <typename I>
void
CopyupRequest<I>::convert_copyup_extent_map()
{
  auto cct = m_image_ctx->cct;
  ...
  for (auto [image_offset, image_length] : image_extent_map) {
    ...
    util::area_to_object_extents(
        m_image_ctx, image_offset, image_length, m_image_area, 0,
        &object_extents);
...
template <typename I>
int
CopyupRequest<I>::prepare_copyup_data()
{
  ceph_assert(ceph_mutex_is_locked(m_image_ctx->image_lock));
  auto cct = m_image_ctx->cct;
  ...
  if (copy_on_read || maybe_deep_copyup) {
    ...
    for (auto [object_offset, object_length] : m_copyup_extent_map) {
      ...
      intersection.intersection_of(
          copyup_object_extents, m_write_object_extents);
