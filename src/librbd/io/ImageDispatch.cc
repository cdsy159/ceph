...
void
start_in_flight_io(AioCompletion* aio_comp)
{
...
ImageArea
get_area(const std::atomic<uint32_t>* image_dispatch_flags)
{
  return (
    ...
    Context* on_dispatched)
{
  ...
  ldout(cct, 20) << "image_extents=" << image_extents << " area=" << area
                 << dendl;
  ...
  *dispatch_result = DISPATCH_RESULT_COMPLETE;
  ImageRequest<I>::aio_read(
      m_image_ctx, aio_comp, std::move(image_extents), area,
      std::move(read_result), io_context, op_flags, read_flags, parent_trace);
  return true;
}
    ...
    Context* on_dispatched)
{
  ...
  *dispatch_result = DISPATCH_RESULT_COMPLETE;
  ImageRequest<I>::aio_write(
      m_image_ctx, aio_comp, std::move(image_extents), area, std::move(bl),
    ...
    Context* on_dispatched)
{
  ...
  *dispatch_result = DISPATCH_RESULT_COMPLETE;
  ImageRequest<I>::aio_compare_and_write(
      m_image_ctx, aio_comp, std::move(image_extents), area, std::move(cmp_bl),
    ...
    Context* on_dispatched)
{
  ...
  *dispatch_result = DISPATCH_RESULT_COMPLETE;
  ImageListSnapsRequest<I> req(
      *m_image_ctx, aio_comp, std::move(image_extents), area,
      std::move(snap_ids), list_snaps_flags, snapshot_delta, parent_trace);
  req.send();
  return true;
}
...
template <typename I>
bool
ImageDispatch<I>::invalidate_cache(Context* on_finish)
{
  auto cct = m_image_ctx->cct;
  ldout(cct, 20) << dendl;
