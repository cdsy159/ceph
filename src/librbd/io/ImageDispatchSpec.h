...
public:
  ...
  typedef std::
      variant<Read, Discard, Write, WriteSame, CompareAndWrite, Flush, ListSnaps>
          Request;
      ...
      const ZTracer::Trace& parent_trace)
  {
    return new ImageDispatchSpec(
        ...
        Read{std::move(read_result), read_flags}, io_context, op_flags,
        parent_trace);
  }
      ...
      const ZTracer::Trace& parent_trace)
  {
    return new ImageDispatchSpec(
        image_ctx.io_image_dispatcher, image_dispatch_layer, aio_comp,
        std::move(image_extents), area, Discard{discard_granularity_bytes}, {},
        0, parent_trace);
  }
      ...
      const ZTracer::Trace& parent_trace)
  {
    return new ImageDispatchSpec(
        image_ctx.io_image_dispatcher, image_dispatch_layer, aio_comp,
        std::move(image_extents), area, Write{std::move(bl)}, {}, op_flags,
        parent_trace);
  }
      ...
      const ZTracer::Trace& parent_trace)
  {
    return new ImageDispatchSpec(
        image_ctx.io_image_dispatcher, image_dispatch_layer, aio_comp,
        std::move(image_extents), area, WriteSame{std::move(bl)}, {}, op_flags,
        parent_trace);
  }
      ...
      const ZTracer::Trace& parent_trace)
  {
    return new ImageDispatchSpec(
        image_ctx.io_image_dispatcher, image_dispatch_layer, aio_comp, {},
        ImageArea::DATA /* dummy for {} */, Flush{flush_source}, {}, 0,
        parent_trace);
  }
