...
#undef dout_prefix
#define dout_prefix \
  *_dout << "librbd::io::ImageDispatcher: " << this << " " << __func__ << ": "
...
struct ImageDispatcher<I>::SendVisitor : public boost::static_visitor<bool> {
  ...
  SendVisitor(
      ImageDispatchInterface* image_dispatch,
      ImageDispatchSpec* image_dispatch_spec) :
    image_dispatch(image_dispatch), image_dispatch_spec(image_dispatch_spec)
  {}

  bool
  operator()(ImageDispatchSpec::Read& read) const
  {
  ...
  bool
  operator()(ImageDispatchSpec::Discard& discard) const
  {
  ...
  bool
  operator()(ImageDispatchSpec::Write& write) const
  {
  ...
  bool
  operator()(ImageDispatchSpec::WriteSame& write_same) const
  {
  ...
  bool
  operator()(ImageDispatchSpec::CompareAndWrite& compare_and_write) const
  {
  ...
  bool
  operator()(ImageDispatchSpec::Flush& flush) const
  {
  ...
  bool
  operator()(ImageDispatchSpec::ListSnaps& list_snaps) const
  {
...
struct ImageDispatcher<I>::PreprocessVisitor
  ...
  PreprocessVisitor(
      ImageDispatcher<I>* image_dispatcher,
      ImageDispatchSpec* image_dispatch_spec) :
    image_dispatcher(image_dispatcher), image_dispatch_spec(image_dispatch_spec)
  {}
  ...
  clip_request() const
  {
    auto area =
        (image_dispatch_spec->image_dispatch_flags &
                 IMAGE_DISPATCH_FLAG_CRYPTO_HEADER
             ? ImageArea::CRYPTO_HEADER
             : ImageArea::DATA);
    int r = util::clip_request(
        image_dispatcher->m_image_ctx, &image_dispatch_spec->image_extents,
        area);
    if (r < 0) {
  ...
  bool
  operator()(ImageDispatchSpec::Flush&) const
  {
    return clip_request();
  }

  bool
  operator()(ImageDispatchSpec::ListSnaps&) const
  {
    return false;
  }
...
template <typename I>
void
ImageDispatcher<I>::invalidate_cache(Context* on_finish)
{
...
template <typename I>
void
ImageDispatcher<I>::shut_down(Context* on_finish)
{
...
template <typename I>
void
ImageDispatcher<I>::apply_qos_schedule_tick_min(uint64_t tick)
{
...
template <typename I>
void
ImageDispatcher<I>::apply_qos_limit(
    uint64_t flag,
    uint64_t limit,
    uint64_t burst,
...
template <typename I>
void
ImageDispatcher<I>::apply_qos_exclude_ops(uint64_t exclude_ops)
{
...
template <typename I>
bool
ImageDispatcher<I>::writes_blocked() const
{
...
template <typename I>
int
ImageDispatcher<I>::block_writes()
{
...
template <typename I>
void
ImageDispatcher<I>::block_writes(Context* on_blocked)
{
...
template <typename I>
void
ImageDispatcher<I>::unblock_writes()
{
...
template <typename I>
void
ImageDispatcher<I>::wait_on_writes_unblocked(Context* on_unblocked)
{
...
template <typename I>
bool
ImageDispatcher<I>::send_dispatch(
    ImageDispatchInterface* image_dispatch,
    ImageDispatchSpec* image_dispatch_spec)
{
...
template <typename I>
bool
ImageDispatcher<I>::preprocess(ImageDispatchSpec* image_dispatch_spec)
{
  return std::visit(
