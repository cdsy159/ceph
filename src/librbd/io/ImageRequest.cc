...
struct C_AssembleSnapshotDeltas : public C_AioRequest {
  ...
  ceph::mutex lock =
      ceph::make_mutex("librbd::io::C_AssembleSnapshotDeltas::lock", false);
  std::map<uint64_t, SnapshotDelta> object_snapshot_delta;

  C_AssembleSnapshotDeltas(
      I* image_ctx,
      AioCompletion* aio_comp,
      SnapshotDelta* snapshot_delta) :
    C_AioRequest(aio_comp), image_ctx(image_ctx), snapshot_delta(snapshot_delta)
  {}
  ...
  finish(int r) override
  {
    ...
    *snapshot_delta = {};
    for (auto& [object_no, object_snapshot_delta] : object_snapshot_delta) {
      ...
      object_to_image_intervals(
          object_no, object_snapshot_delta, &image_snapshot_delta,
          snapshot_delta);
  ...
  object_to_image_intervals(
      uint64_t object_no,
      const SnapshotDelta& object_snapshot_delta,
      SnapshotDelta* image_snapshot_delta,
    ...
    for (auto& [key, object_extents] : object_snapshot_delta) {
      for (auto& object_extent : object_extents) {
        ...
        for (auto [image_offset, image_length] : image_extents) {
          SparseExtent sparse_extent{
              object_extent.get_val().state, image_length};
...
template <typename I>
void
readahead(I* ictx, const Extents& image_extents, IOContext io_context)
{
  ...
  if (readahead_length > 0) {
    ...
    io::util::area_to_object_extents(
        ictx, readahead_offset, readahead_length, ImageArea::DATA, 0,
        &readahead_object_extents);
    for (auto& object_extent : readahead_object_extents) {
      ldout(ictx->cct, 20) << "(readahead) "
                           << data_object_name(ictx, object_extent.object_no)
                           << " " << object_extent.offset << "~"
                           << object_extent.length << dendl;

      auto req_comp = new C_RBD_Readahead<I>(
          ictx, object_extent.object_no, object_extent.offset,
...
{
  return (
      interval && (static_cast<uint64_t>(now.sec()) >= interval + timestamp));
}
...
#undef dout_prefix
#define dout_prefix \
  *_dout << "librbd::io::ImageRequest: " << this << " " << __func__ << ": "
    ...
    const ZTracer::Trace& parent_trace)
{
  ImageReadRequest<I> req(
      *ictx, c, std::move(image_extents), area, std::move(read_result),
...
ImageRequest<I>::aio_write(
    ...
    const ZTracer::Trace& parent_trace)
{
  ImageWriteRequest<I> req(
      *ictx, c, std::move(image_extents), area, std::move(bl), op_flags,
      parent_trace);
  req.send();
}
...
ImageRequest<I>::aio_discard(
    ...
    const ZTracer::Trace& parent_trace)
{
  ImageDiscardRequest<I> req(
      *ictx, c, std::move(image_extents), area, discard_granularity_bytes,
      parent_trace);
  req.send();
}
    ...
    const ZTracer::Trace& parent_trace)
{
  ImageCompareAndWriteRequest<I> req(
      *ictx, c, std::move(image_extents), area, std::move(cmp_bl),
...
template <typename I>
void
ImageRequest<I>::update_timestamp()
{
...
template <typename I>
ImageReadRequest<I>::ImageReadRequest(
    I& image_ctx,
    AioCompletion* aio_comp,
    Extents&& image_extents,
    ImageArea area,
...
ImageReadRequest<I>::send_request()
{
  ...
  if (this->m_image_area == ImageArea::DATA && image_ctx.cache &&
      image_ctx.readahead_max_bytes > 0 &&
  ...
  for (auto& extent : image_extents) {
    ...
    util::area_to_object_extents(
        &image_ctx, extent.first, extent.second, this->m_image_area, buffer_ofs,
        &object_extents);
...
AbstractImageWriteRequest<I>::send_request()
{
  ...
  for (auto& extent : this->m_image_extents) {
    ...
    io::util::area_to_object_extents(
        &image_ctx, extent.first, extent.second, this->m_image_area, clip_len,
        &object_extents);
  ...
  if (ret == 1) {
    ...
    for (auto& object_extent : object_extents) {
      ...
      this->m_image_extents.insert(
          this->m_image_extents.end(), image_extents.begin(),
          image_extents.end());
    }
  }
  ...
  if (!object_extents.empty()) {
    ...
    send_object_requests(
        object_extents, image_ctx.get_data_io_context(), journal_tid);
  }
    ...
    uint64_t journal_tid)
{
  ...
  for (auto& oe : object_extents) {
    ...
    auto request = create_object_request(
        oe, io_context, journal_tid, single_extent, req_comp);
    request->send();
  }
}
...
template <typename I>
void
ImageWriteRequest<I>::assemble_extent(
...
template <typename I>
uint64_t
ImageWriteRequest<I>::append_journal_event()
{
  I& image_ctx = this->m_image_ctx;
...
template <typename I>
void
ImageWriteRequest<I>::update_stats(size_t length)
{
  I& image_ctx = this->m_image_ctx;
...
template <typename I>
uint64_t
ImageDiscardRequest<I>::append_journal_event()
{
  I& image_ctx = this->m_image_ctx;
...
template <typename I>
void
ImageDiscardRequest<I>::update_stats(size_t length)
{
  I& image_ctx = this->m_image_ctx;
...
template <typename I>
int
ImageDiscardRequest<I>::prune_object_extents(
    LightweightObjectExtents* object_extents) const
{
  ...
  auto discard_granularity_bytes =
      std::min(m_discard_granularity_bytes, object_size);
  auto xform_lambda = [discard_granularity_bytes, object_size, &prune_required,
                       &length_modified](LightweightObjectExtent& object_extent) {
  ...
  if (prune_required) {
    ...
    auto remove_lambda = [](const LightweightObjectExtent& object_extent) {
      return (object_extent.length == 0);
    };
    object_extents->erase(
        std::remove_if(
            object_extents->begin(), object_extents->end(), remove_lambda),
        object_extents->end());
  }
...
ImageFlushRequest<I>::send_request()
{
  ...
  bool journaling = false;
  {
    std::shared_lock image_locker{image_ctx.image_lock};
    journaling =
        (m_flush_source == FLUSH_SOURCE_USER && image_ctx.journal != nullptr &&
         image_ctx.journal->is_journal_appending());
  }
...
template <typename I>
uint64_t
ImageWriteSameRequest<I>::append_journal_event()
{
  I& image_ctx = this->m_image_ctx;
    ...
    Context* on_finish)
{
  ...
  if (util::assemble_write_same_extent(object_extent, m_data_bl, &bl, false)) {
    ...
    req = ObjectDispatchSpec::create_write_same(
        ...
        std::move(bl), io_context, m_op_flags, journal_tid, this->m_trace,
        on_finish);
    return req;
  }
...
template <typename I>
void
ImageWriteSameRequest<I>::update_stats(size_t length)
{
  I& image_ctx = this->m_image_ctx;
...
template <typename I>
uint64_t
ImageCompareAndWriteRequest<I>::append_journal_event()
{
  I& image_ctx = this->m_image_ctx;
  ...
  tid = image_ctx.journal->append_compare_and_write_event(
      extent.first, extent.second, m_cmp_bl, m_bl, false);
...
template <typename I>
void
ImageCompareAndWriteRequest<I>::assemble_extent(
    const LightweightObjectExtent& object_extent,
    bufferlist* bl,
    bufferlist* cmp_bl)
{
...
template <typename I>
ObjectDispatchSpec*
ImageCompareAndWriteRequest<I>::create_object_request(
    const LightweightObjectExtent& object_extent,
    IOContext io_context,
    uint64_t journal_tid,
    bool single_extent,
...
template <typename I>
void
ImageCompareAndWriteRequest<I>::update_stats(size_t length)
{
  I& image_ctx = this->m_image_ctx;
...
template <typename I>
int
ImageCompareAndWriteRequest<I>::prune_object_extents(
    LightweightObjectExtents* object_extents) const
{
...
template <typename I>
void
ImageListSnapsRequest<I>::send_request()
{
  I& image_ctx = this->m_image_ctx;
  CephContext* cct = image_ctx.cct;
  ...
  for (auto& image_extent : image_extents) {
    ...
    io::util::area_to_object_extents(
        &image_ctx, image_extent.first, image_extent.second, this->m_image_area,
        0, &object_extents);
    for (auto& object_extent : object_extents) {
  ...
  aio_comp->set_request_count(1);
  auto assemble_ctx =
      new C_AssembleSnapshotDeltas<I>(&image_ctx, aio_comp, m_snapshot_delta);
  auto sub_aio_comp =
      AioCompletion::create_and_start<Context, &Context::complete>(
  ...
  for (auto& oe : object_number_extents) {
    ldout(cct, 20) << data_object_name(&image_ctx, oe.first) << " " << oe.second
                   << dendl;
