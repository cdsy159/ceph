...
public:
  ...
  static void aio_discard(
      ...
      const ZTracer::Trace& parent_trace);
  static void aio_flush(
      ImageCtxT* ictx,
      AioCompletion* c,
      FlushSource flush_source,
      const ZTracer::Trace& parent_trace);
  static void aio_writesame(
      ImageCtxT* ictx,
      AioCompletion* c,
      Extents&& image_extents,
      ImageArea area,
      ...
      int op_flags,
      const ZTracer::Trace& parent_trace);
  static void aio_compare_and_write(
      ImageCtxT* ictx,
      AioCompletion* c,
      Extents&& image_extents,
      ImageArea area,
...
protected:
  ...
  const char*
  get_request_type() const override
  {
    return "aio_read";
  }
...
protected:
  ...
  virtual int
  prune_object_extents(LightweightObjectExtents* object_extents) const
  {
    return 0;
  }
...
template <typename ImageCtxT = ImageCtx>
class ImageCompareAndWriteRequest
  : public AbstractImageWriteRequest<ImageCtxT> {
public:
  using typename ImageRequest<ImageCtxT>::ObjectRequests;
