...
#undef dout_prefix
#define dout_prefix \
  *_dout << "librbd::io::ObjectDispatcher: " << this << " " << __func__ << ": "
...
struct ObjectDispatcher<I>::C_ResetExistenceCache : public C_LayerIterator {
  ...
  bool
  execute(ObjectDispatchInterface* object_dispatch, Context* on_finish) override
  {
...
struct ObjectDispatcher<I>::SendVisitor {
  ...
  SendVisitor(
      ObjectDispatchInterface* object_dispatch,
      ObjectDispatchSpec* object_dispatch_spec) :
    object_dispatch(object_dispatch), object_dispatch_spec(object_dispatch_spec)
  {}

  bool
  operator()(ObjectDispatchSpec::ReadRequest& read) const
  {
  ...
  bool
  operator()(ObjectDispatchSpec::WriteRequest& write) const
  {
  ...
  bool
  operator()(ObjectDispatchSpec::WriteSameRequest& write_same) const
  {
  ...
  bool
  operator()(ObjectDispatchSpec::CompareAndWriteRequest& compare_and_write) const
  {
    return object_dispatch->compare_and_write(
        ...
        std::move(compare_and_write.data), object_dispatch_spec->io_context,
        object_dispatch_spec->op_flags, object_dispatch_spec->parent_trace,
        compare_and_write.mismatch_offset,
        &object_dispatch_spec->object_dispatch_flags,
        &compare_and_write.journal_tid, &object_dispatch_spec->dispatch_result,
        &object_dispatch_spec->dispatcher_ctx.on_finish,
        &object_dispatch_spec->dispatcher_ctx);
  }

  bool
  operator()(ObjectDispatchSpec::FlushRequest& flush) const
  {
    return object_dispatch->flush(
        flush.flush_source, object_dispatch_spec->parent_trace,
        &flush.journal_tid, &object_dispatch_spec->dispatch_result,
        &object_dispatch_spec->dispatcher_ctx.on_finish,
        &object_dispatch_spec->dispatcher_ctx);
  }

  bool
  operator()(ObjectDispatchSpec::ListSnapsRequest& list_snaps) const
  {
...
template <typename I>
void
ObjectDispatcher<I>::invalidate_cache(Context* on_finish)
{
...
template <typename I>
void
ObjectDispatcher<I>::reset_existence_cache(Context* on_finish)
{
  auto image_ctx = this->m_image_ctx;
  auto cct = image_ctx->cct;
  ldout(cct, 5) << dendl;
...
template <typename I>
void
ObjectDispatcher<I>::extent_overwritten(
    uint64_t object_no,
    uint64_t object_off,
    uint64_t object_len,
    ...
    uint64_t new_journal_tid)
{
  ...
  for (auto it : this->m_dispatches) {
    ...
    object_dispatch->extent_overwritten(
        object_no, object_off, object_len, journal_tid, new_journal_tid);
  }
}

template <typename I>
int
ObjectDispatcher<I>::prepare_copyup(
    uint64_t object_no,
...
template <typename I>
bool
ObjectDispatcher<I>::send_dispatch(
    ObjectDispatchInterface* object_dispatch,
    ObjectDispatchSpec* object_dispatch_spec)
{
  return std::visit(
