...
public:
      ...
      Context* completion)
  {
    return new ObjectReadRequest(
        ictx, objectno, extents, io_context, op_flags, read_flags, parent_trace,
        version, completion);
  }
...
public:
  ...
  virtual bool
  is_empty_write_op() const
  {
    return false;
  }

  virtual uint8_t
  get_pre_write_object_map_state() const
  {
    return OBJECT_EXISTS;
  }

  virtual void
  add_copyup_ops(neorados::WriteOp* wr)
  {
    add_write_ops(wr);
  }
...
protected:
  ...
  virtual bool
  is_no_op_for_nonexistent_object() const
  {
    return false;
  }

  virtual bool
  is_object_map_update_enabled() const
  {
    return true;
  }

  virtual bool
  is_post_copyup_write_required() const
  {
    return false;
  }

  virtual bool
  is_non_existent_post_write_object_map_state() const
  {
    return false;
  }
  ...
  virtual int
  filter_write_result(int r) const
  {
    return r;
  }

  virtual Extents
  get_copyup_overwrite_extents() const
  {
...
public:
  ObjectWriteRequest(
      ...
      const ZTracer::Trace& parent_trace,
      Context* completion) :
    AbstractObjectWriteRequest<ImageCtxT>(
        ictx,
        object_no,
        object_off,
        data.length(),
  ...
  bool
  is_empty_write_op() const override
  {
...
public:
  ...
  uint8_t
  get_pre_write_object_map_state() const override
  {
...
protected:
  bool
  is_no_op_for_nonexistent_object() const override
  {
    return (!this->has_parent());
  }

  bool
  is_object_map_update_enabled() const override
  {
    return (
  ...
  bool
  is_non_existent_post_write_object_map_state() const override
  {
    return (m_discard_action == DISCARD_ACTION_REMOVE);
  }

  void
  add_write_hint(neorados::WriteOp* wr) override
  {
    // no hint for discard
  }
...
template <typename ImageCtxT = ImageCtx>
class ObjectCompareAndWriteRequest
  : public AbstractObjectWriteRequest<ImageCtxT> {
public:
  ...
  void
  add_copyup_ops(neorados::WriteOp* wr) override
  {
    // no-op on copyup
  }
...
protected:
  ...
  Extents
  get_copyup_overwrite_extents() const override
  {
    return {};
  }
...
public:
  ...
  create(
      ...
      Context* completion)
  {
    return new ObjectListSnapsRequest(
        ictx, objectno, std::move(object_extents), std::move(snap_ids),
        list_snaps_flags, parent_trace, snapshot_delta, completion);
  }
