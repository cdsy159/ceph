...
uint64_t
get_extent_length(const Extents& extents)
{
  uint64_t length = 0;
...
uint64_t
calculate_tokens(bool read_op, uint64_t extent_length, uint64_t flag)
{
  ...
  m_image_ctx(image_ctx), m_flush_tracker(new FlushTracker<I>(image_ctx))
{
  ...
  for (auto [flag, name] : throttle_flags) {
    m_throttles.emplace_back(
        flag, new TokenBucketThrottle(cct, name, 0, 0, timer, timer_lock));
  }
}
...
template <typename I>
void
QosImageDispatch<I>::shut_down(Context* on_finish)
{
...
template <typename I>
void
QosImageDispatch<I>::apply_qos_schedule_tick_min(uint64_t tick)
{
...
template <typename I>
void
QosImageDispatch<I>::apply_qos_limit(
    uint64_t flag,
    uint64_t limit,
    uint64_t burst,
    uint64_t burst_seconds)
{
  ...
  if (r < 0) {
    lderr(cct) << throttle->get_name()
               << ": invalid qos parameter: " << "burst(" << burst
               << ") is less than " << "limit(" << limit << ")" << dendl;
...
template <typename I>
void
QosImageDispatch<I>::apply_qos_exclude_ops(uint64_t exclude_ops)
{
    ...
    Context* on_dispatched)
{
  ...
  if (m_qos_exclude_ops & RBD_IO_OPERATION_READ || m_qos_enabled_flag == 0) {
    return false;
  }

  if (needs_throttle(
          true, image_extents, tid, image_dispatch_flags, dispatch_result,
    ...
    Context* on_dispatched)
{
  ...
  if (m_qos_exclude_ops & RBD_IO_OPERATION_WRITE || m_qos_enabled_flag == 0) {
    return false;
  }

  if (needs_throttle(
          false, image_extents, tid, image_dispatch_flags, dispatch_result,
          on_finish, on_dispatched)) {
    return true;
  }
    ...
    Context* on_dispatched)
{
  ...
  if (m_qos_exclude_ops & RBD_IO_OPERATION_DISCARD || m_qos_enabled_flag == 0) {
    return false;
  }
...
template <typename I>
bool
QosImageDispatch<I>::compare_and_write(
    AioCompletion* aio_comp,
    Extents&& image_extents,
    bufferlist&& cmp_bl,
    bufferlist&& bl,
...
template <typename I>
void
QosImageDispatch<I>::handle_finished(int r, uint64_t tid)
{
  auto cct = m_image_ctx->cct;
  ldout(cct, 20) << "tid=" << tid << dendl;
...
template <typename I>
bool
QosImageDispatch<I>::set_throttle_flags(
    std::atomic<uint32_t>* image_dispatch_flags,
    uint32_t flags)
{
  ...
  return (
      (desired & IMAGE_DISPATCH_FLAG_QOS_MASK) == IMAGE_DISPATCH_FLAG_QOS_MASK);
}

template <typename I>
bool
QosImageDispatch<I>::needs_throttle(
    bool read_op,
    ...
    Context* on_dispatched)
{
  ...
  *dispatch_result = DISPATCH_RESULT_CONTINUE;
  ...
  for (auto [flag, throttle] : m_throttles) {
    ...
    if (tokens > 0 &&
        throttle->get(
            tokens, this, &QosImageDispatch<I>::handle_throttle_ready,
...
template <typename I>
void
QosImageDispatch<I>::handle_throttle_ready(Tag&& tag, uint64_t flag)
{
  auto cct = m_image_ctx->cct;
