...
template <typename I>
void
QueueImageDispatch<I>::shut_down(Context* on_finish)
{
...
template <typename I>
bool
QueueImageDispatch<I>::write_same(
    AioCompletion* aio_comp,
    Extents&& image_extents,
    bufferlist&& bl,
    int op_flags,
...
template <typename I>
bool
QueueImageDispatch<I>::compare_and_write(
    AioCompletion* aio_comp,
    Extents&& image_extents,
    bufferlist&& cmp_bl,
    bufferlist&& bl,
...
template <typename I>
void
QueueImageDispatch<I>::handle_finished(int r, uint64_t tid)
{
  auto cct = m_image_ctx->cct;
  ldout(cct, 20) << "tid=" << tid << dendl;
