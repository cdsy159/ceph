...
#undef dout_prefix
#define dout_prefix \
  *_dout << "librbd::io::ReadResult: " << this << " " << __func__ << ": "
...
struct ReadResult::SetImageExtentsVisitor {
  ...
  void
  operator()(SparseBufferlist& sbl) const
  {
    sbl.image_extents = image_extents;
  }

  void
  operator()(ChildObject& child_object) const
  {
...
struct ReadResult::AssembleResultVisitor {
  ...
  void
  operator()(std::monostate& empty) const
  {
  ...
  void
  operator()(Bufferlist& bufferlist) const
  {
    bufferlist.bl->clear();
  ...
  void
  operator()(SparseBufferlist& sparse_bufferlist) const
  {
    ...
    for (auto [image_offset, image_length] : sparse_bufferlist.image_extents) {
      ...
      while (bem_it != buffer_extent_map.end()) {
        ...
        ceph_assert(
            buffer_offset <= buffer_extent_offset &&
            buffer_offset + image_length >=
                buffer_extent_offset + buffer_extent_length);
        found_buffer_extent = true;

        auto image_extent_offset = image_offset +
                                   (buffer_extent_offset - buffer_offset);
      ...
      if (!found_buffer_extent) {
        ldout(cct, 20) << "no buffer extents for image extent " << image_offset
                       << "~" << image_length << dendl;
      }
  ...
  operator()(ChildObject& child_object) const
  {
    bufferlist bl;
    ExtentMap buffer_extent_map;
    uint64_t buffer_extents_length =
        destriper.assemble_result(cct, &buffer_extent_map, &bl);
    ...
    for (auto& read_extent : *child_object.read_extents) {
      ...
      while (bem_it != buffer_extent_map.end()) {
        ...
        ceph_assert(
            child_buffer_offset <= buffer_extent_offset &&
        ...
        read_extent.extent_map.emplace_back(
            object_extent_offset, buffer_extent_length);
        ++bem_it;
      }
...
void
ReadResult::C_ImageReadRequest::finish(int r)
{
...
void
ReadResult::C_ObjectReadRequest::finish(int r)
{
  CephContext* cct = aio_completion->ictx->cct;
...
void
ReadResult::C_ObjectReadMergedExtents::finish(int r)
{
  if (r >= 0) {
    for (auto& extent : *extents) {
...
void
ReadResult::set_image_extents(const Extents& image_extents)
{
  std::visit(SetImageExtentsVisitor(image_extents), m_buffer);
}

void
ReadResult::assemble_result(CephContext* cct)
{
