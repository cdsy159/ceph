...
enum {
  ...
  RBD_IO_OPERATIONS_ALL =
      (RBD_IO_OPERATION_READ | RBD_IO_OPERATION_WRITE |
       RBD_IO_OPERATION_DISCARD | RBD_IO_OPERATION_WRITE_SAME |
...
struct SparseExtent {
  ...
  bool
  operator==(const SparseExtent& rhs) const
  {
...
struct SparseExtentSplitMerge {
  SparseExtent
  split(uint64_t offset, uint64_t length, const SparseExtent& se) const
  {
    return SparseExtent(se.state, length);
  }

  bool
  can_merge(const SparseExtent& left, const SparseExtent& right) const
  {
    return left.state == right.state;
  }

  SparseExtent
  merge(SparseExtent&& left, SparseExtent&& right) const
  {
  ...
  uint64_t
  length(const SparseExtent& se) const
  {
    return se.length;
  }
};

typedef interval_map<uint64_t, SparseExtent, SparseExtentSplitMerge>
    SparseExtents;
...
struct SparseBufferlistExtent : public SparseExtent {
  ...
  bool
  operator==(const SparseBufferlistExtent& rhs) const
  {
    return (
        state == rhs.state && length == rhs.length && bl.contents_equal(rhs.bl));
  }
};

struct SparseBufferlistExtentSplitMerge {
  SparseBufferlistExtent
  split(uint64_t offset, uint64_t length, const SparseBufferlistExtent& sbe) const
  {
    ceph::bufferlist bl;
  ...
  bool
  can_merge(
      const SparseBufferlistExtent& left,
  ...
  SparseBufferlistExtent
  merge(SparseBufferlistExtent&& left, SparseBufferlistExtent&& right) const
  {
    ceph::bufferlist bl;
    ...
    return SparseBufferlistExtent(
        left.state, left.length + right.length, std::move(bl));
  }

  uint64_t
  length(const SparseBufferlistExtent& sbe) const
  {
    return sbe.length;
  }
};

typedef interval_map<uint64_t, SparseBufferlistExtent, SparseBufferlistExtentSplitMerge>
    SparseBufferlist;
...
struct ReadExtent {
  ...
  ReadExtent(
      ...
      uint64_t length,
      const LightweightBufferExtents&& buffer_extents) :
    offset(offset), length(length), buffer_extents(buffer_extents)
  {}
  ...
  operator<<(std::ostream& os, const ReadExtent& extent)
  {
    os << "offset=" << extent.offset << ", " << "length=" << extent.length
