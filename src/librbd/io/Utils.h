...
inline uint64_t
get_extents_length(const Extents& extents)
{
...
template <typename ImageCtxT = librbd::ImageCtx>
uint64_t area_to_raw_offset(
    const ImageCtxT& image_ctx,
    uint64_t offset,
    ImageArea area);

template <typename ImageCtxT = librbd::ImageCtx>
std::pair<uint64_t, ImageArea> raw_to_area_offset(
    const ImageCtxT& image_ctx,
    uint64_t offset);

inline ObjectDispatchLayer
get_previous_layer(ObjectDispatchLayer layer)
{
