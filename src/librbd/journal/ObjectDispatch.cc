...
#undef dout_prefix
#define dout_prefix                                                        \
  *_dout << "librbd::journal::ObjectDispatch: " << this << " " << __func__ \
         << ": "
  ...
  finish(int r) override
  {
    ...
    if (r >= 0 || (object_dispatch_flags &
      ...
      for (const auto& extent : image_extents) {
        journal->commit_io_event_extent(
            journal_tid, extent.first, extent.second, r);
      }
    }
...
template <typename I>
void
ObjectDispatch<I>::shut_down(Context* on_finish)
{
    ...
    Context* on_dispatched)
{
  ...
  *on_finish = new C_CommitIOEvent<I>(
      m_image_ctx, m_journal, object_no, object_off, object_len, *journal_tid,
      *object_dispatch_flags, *on_finish);
  *on_finish = create_context_callback<Context, &Context::complete>(
      *on_finish, m_journal);
    ...
    Context* on_dispatched)
{
  ...
  *on_finish = new C_CommitIOEvent<I>(
      m_image_ctx, m_journal, object_no, object_off, data.length(),
      *journal_tid, *object_dispatch_flags, *on_finish);
  *on_finish = create_context_callback<Context, &Context::complete>(
      *on_finish, m_journal);
...
template <typename I>
bool
ObjectDispatch<I>::compare_and_write(
    uint64_t object_no,
    uint64_t object_off,
    ...
    Context* on_dispatched)
{
  ...
  *on_finish = new C_CommitIOEvent<I>(
      m_image_ctx, m_journal, object_no, object_off, write_data.length(),
      *journal_tid, *object_dispatch_flags, *on_finish);
  *on_finish = create_context_callback<Context, &Context::complete>(
      *on_finish, m_journal);
    ...
    Context* on_dispatched)
{
  ...
  auto ctx = *on_finish;
  *on_finish = new LambdaContext([image_ctx = m_image_ctx, ctx,
                                  journal_tid = *journal_tid](int r) {
  ...
  *dispatch_result = io::DISPATCH_RESULT_CONTINUE;
  wait_or_flush_event(
      *journal_tid, io::OBJECT_DISPATCH_FLAG_FLUSH, on_dispatched);
  return true;
}

template <typename I>
void
ObjectDispatch<I>::extent_overwritten(
    uint64_t object_no,
    uint64_t object_off,
    uint64_t object_len,
    uint64_t journal_tid,
    uint64_t new_journal_tid)
{
  ...
  Context* ctx = new C_CommitIOEvent<I>(
      m_image_ctx, m_journal, object_no, object_off, object_len, journal_tid,
      false, nullptr);
...
template <typename I>
void
ObjectDispatch<I>::wait_or_flush_event(
    uint64_t journal_tid,
    int object_dispatch_flags,
    Context* on_dispatched)
{
