...
#undef dout_prefix
#define dout_prefix \
  *_dout << "librbd::journal::OpenRequest: " << this << " " << __func__ << ": "
...
OpenRequest<I>::send_init()
{
  ...
  m_journaler->init(create_async_context_callback(
      *m_image_ctx,
      create_context_callback<OpenRequest<I>, &OpenRequest<I>::handle_init>(
          this)));
}

template <typename I>
void
OpenRequest<I>::handle_init(int r)
{
  CephContext* cct = m_image_ctx->cct;
  ...
  r = m_journaler->get_cached_client(
      Journal<ImageCtx>::IMAGE_CLIENT_ID, &client);
  if (r < 0) {
  ...
  ldout(cct, 20) << this << " " << __func__ << ": " << "client: " << client
...
template <typename I>
void
OpenRequest<I>::send_get_tags()
{
  CephContext* cct = m_image_ctx->cct;
  ldout(cct, 20) << dendl;

  C_DecodeTags* tags_ctx = new C_DecodeTags(
      cct, m_lock, m_tag_tid, m_tag_data,
...
template <typename I>
void
OpenRequest<I>::handle_get_tags(int r)
{
  CephContext* cct = m_image_ctx->cct;
  ldout(cct, 20) << "r=" << r << dendl;
