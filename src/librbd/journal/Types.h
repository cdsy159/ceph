...
protected:
  ...
  SnapEventBase(
    ...
    OpEventBase(op_tid), snap_namespace(_snap_namespace), snap_name(_snap_name)
  {}
...
struct DemotePromoteEvent {
  static const EventType TYPE =
      static_cast<EventType>(EVENT_TYPE_DEMOTE_PROMOTE);
...
struct MirrorPeerSyncPoint {
  ...
  inline bool
  operator==(const MirrorPeerSyncPoint& sync) const
  {
    return (
        snap_name == sync.snap_name && from_snap_name == sync.from_snap_name &&
...
struct MirrorPeerClientMeta {
  ...
  inline bool
  operator==(const MirrorPeerClientMeta& meta) const
  {
    return (
        image_id == meta.image_id && state == meta.state &&
...
using ClientMeta = std::
    variant<ImageClientMeta, MirrorPeerClientMeta, CliClientMeta, UnknownClientMeta>;
...
struct TagPredecessor {
  ...
  inline bool
  operator==(const TagPredecessor& rhs) const
  {
    return (
        mirror_uuid == rhs.mirror_uuid && commit_valid == rhs.commit_valid &&
