...
static int
get_iovec_length(const struct iovec* iov, int iovcnt, size_t& len)
{
  len = 0;
    ...
    librbd::io::AioCompletion* aio_completion)
{
  ...
  for (int i = 0; i < iovcnt; ++i) {
    const struct iovec& io = iov[i];
    bl.push_back(create_write_raw(
        ictx, static_cast<char*>(io.iov_base), io.iov_len, aio_completion));
  }
  return bl;
}

CephContext*
get_cct(IoCtx& io_ctx)
{
...
void
group_image_status_cpp_to_c(
    const librbd::group_image_info_t& cpp_info,
...
void
group_info_cpp_to_c(
    const librbd::group_info_t& cpp_info,
...
void
group_snap_info_cpp_to_c(
    const librbd::group_snap_info2_t& cpp_info,
    ...
    rbd_group_snap_info2_t* c_info)
{
  ...
  c_info->image_snaps = static_cast<rbd_group_image_snap_info_t*>(
      calloc(cpp_info.image_snaps.size(), sizeof(rbd_group_image_snap_info_t)));
  size_t i = 0;
  for (const auto& cpp_image_snap : cpp_info.image_snaps) {
    c_info->image_snaps[i].image_name =
        strdup(cpp_image_snap.image_name.c_str());
...
void
mirror_image_info_cpp_to_c(
    const librbd::mirror_image_info_t& cpp_info,
    ...
    librbd::mirror_image_site_status_t* local_status)
{
  auto it = std::find_if(
      ...
      [](const librbd::mirror_image_site_status_t& s) {
        return (
            s.mirror_uuid == cls::rbd::MirrorImageSiteStatus::LOCAL_MIRROR_UUID);
      });
...
int
mirror_image_global_status_cpp_to_c(
...
void
trash_image_info_cpp_to_c(
    const librbd::trash_image_info_t& cpp_info,
...
void
config_option_cpp_to_c(
    const librbd::config_option_t& cpp_option,
...
void
config_option_cleanup(rbd_config_option_t& option)
{
  free(option.name);
  free(option.value);
}
...
struct C_MirrorImageGetGlobalStatus : public Context {
  ...
  C_MirrorImageGetGlobalStatus(
      rbd_mirror_image_global_status_t* mirror_image_global_status,
      Context* on_finish) :
    mirror_image_global_status(mirror_image_global_status), on_finish(on_finish)
  {}
  ...
  finish(int r) override
  {
    ...
    mirror_image_global_status_cpp_to_c(
        cpp_mirror_image_global_status, mirror_image_global_status);
    on_finish->complete(0);
  }
};
...
struct C_MirrorImageGetStatus : public Context {
  ...
  C_MirrorImageGetStatus(
      librbd::mirror_image_status_t* mirror_image_status,
      Context* on_finish) :
  ...
  finish(int r) override
  {
    ...
    if (mirror_image_status != nullptr) {
      r = mirror_image_global_status_cpp_to_c(
          cpp_mirror_image_global_status, mirror_image_status);
    } else if (mirror_image_status_cpp != nullptr) {
      librbd::mirror_image_site_status_t local_status;
      r = get_local_mirror_image_site_status(
          cpp_mirror_image_global_status, &local_status);
      if (r >= 0) {
...
public:
  ...
  int
  update_progress(uint64_t offset, uint64_t src_size) override
  {
...
int
PoolStats::add(rbd_pool_stat_option_t option, uint64_t* opt_val)
{
...
void
RBD::version(int* major, int* minor, int* extra)
{
  rbd_version(major, minor, extra);
}

int
RBD::open(IoCtx& io_ctx, Image& image, const char* name)
{
  return open(io_ctx, image, name, NULL);
}

int
RBD::open_by_id(IoCtx& io_ctx, Image& image, const char* id)
{
  return open_by_id(io_ctx, image, id, nullptr);
}

int
RBD::open(IoCtx& io_ctx, Image& image, const char* name, const char* snap_name)
{
  ImageCtx* ictx = new ImageCtx(name, "", snap_name, io_ctx, false);
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
  tracepoint(
      librbd, open_image_enter, ictx, ictx->name.c_str(), ictx->id.c_str(),
    ...
    const char* snap_name)
{
  ...
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
  tracepoint(
      librbd, open_image_by_id_enter, ictx, ictx->id.c_str(),
      ictx->snap_name.c_str(), ictx->read_only);
    ...
    RBD::AioCompletion* c)
{
  ...
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
  tracepoint(
      librbd, aio_open_image_enter, ictx, ictx->name.c_str(), ictx->id.c_str(),
    ...
    RBD::AioCompletion* c)
{
  ...
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
  tracepoint(
      librbd, aio_open_image_by_id_enter, ictx, ictx->id.c_str(),
      ictx->snap_name.c_str(), ictx->read_only, c->pc);
...
int
RBD::features_to_string(uint64_t features, std::string* str_features)
{
...
int
RBD::features_from_string(const std::string str_features, uint64_t* features)
{
  std::stringstream err;
...
int
RBD::create(IoCtx& io_ctx, const char* name, uint64_t size, int* order)
{
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
  tracepoint(
      librbd, create_enter, io_ctx.get_pool_name().c_str(), io_ctx.get_id(),
      name, size, *order);
    ...
    int* order)
{
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
  tracepoint(
      librbd, create2_enter, io_ctx.get_pool_name().c_str(), io_ctx.get_id(),
      name, size, features, *order);
    ...
    uint64_t stripe_count)
{
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
  tracepoint(
      librbd, create3_enter, io_ctx.get_pool_name().c_str(), io_ctx.get_id(),
      name, size, features, *order, stripe_unit, stripe_count);
  int r = librbd::create(
      io_ctx, name, size, false, features, order, stripe_unit, stripe_count);
  tracepoint(librbd, create3_exit, r, *order);
  return r;
}

int
RBD::create4(IoCtx& io_ctx, const char* name, uint64_t size, ImageOptions& opts)
{
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
  tracepoint(
      librbd, create4_enter, io_ctx.get_pool_name().c_str(), io_ctx.get_id(),
      name, size, opts.opts);
    ...
    int* c_order)
{
  ...
  tracepoint(
      librbd, clone_enter, p_ioctx.get_pool_name().c_str(), p_ioctx.get_id(),
      p_name, p_snap_name, c_ioctx.get_pool_name().c_str(), c_ioctx.get_id(),
      c_name, features);
  int r = librbd::clone(
      p_ioctx, p_name, p_snap_name, c_ioctx, c_name, features, c_order, 0, 0);
    ...
    int stripe_count)
{
  TracepointProvider::initialize<tracepoint_traits>(get_cct(p_ioctx));
  tracepoint(
      librbd, clone2_enter, p_ioctx.get_pool_name().c_str(), p_ioctx.get_id(),
      p_name, p_snap_name, c_ioctx.get_pool_name().c_str(), c_ioctx.get_id(),
      c_name, features, stripe_unit, stripe_count);
  int r = librbd::clone(
    ...
    ImageOptions& c_opts)
{
  TracepointProvider::initialize<tracepoint_traits>(get_cct(p_ioctx));
  tracepoint(
      librbd, clone3_enter, p_ioctx.get_pool_name().c_str(), p_ioctx.get_id(),
      p_name, p_snap_name, c_ioctx.get_pool_name().c_str(), c_ioctx.get_id(),
      c_name, c_opts.opts);
  int r = librbd::clone(
      p_ioctx, nullptr, p_name, CEPH_NOSNAP, p_snap_name, c_ioctx, nullptr,
      c_name, c_opts, "", "");
    ...
    ImageOptions& c_opts)
{
  TracepointProvider::initialize<tracepoint_traits>(get_cct(p_ioctx));
  tracepoint(
      librbd, clone4_enter, p_ioctx.get_pool_name().c_str(), p_ioctx.get_id(),
      p_name, p_snap_id, c_ioctx.get_pool_name().c_str(), c_ioctx.get_id(),
      c_name, c_opts.opts);
  int r = librbd::clone(
      p_ioctx, nullptr, p_name, p_snap_id, nullptr, c_ioctx, nullptr, c_name,
      c_opts, "", "");
...
int
RBD::remove(IoCtx& io_ctx, const char* name)
{
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
  tracepoint(
      librbd, remove_enter, io_ctx.get_pool_name().c_str(), io_ctx.get_id(),
      name);
...
int
RBD::remove_with_progress(IoCtx& io_ctx, const char* name, ProgressContext& pctx)
{
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
  tracepoint(
      librbd, remove_enter, io_ctx.get_pool_name().c_str(), io_ctx.get_id(),
...
int
RBD::trash_move(IoCtx& io_ctx, const char* name, uint64_t delay)
{
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
  tracepoint(
      librbd, trash_move_enter, io_ctx.get_pool_name().c_str(), io_ctx.get_id(),
      name);
  int r = librbd::api::Trash<>::move(
      io_ctx, RBD_TRASH_IMAGE_SOURCE_USER, name, delay);
  tracepoint(librbd, trash_move_exit, r);
  return r;
}

int
RBD::trash_get(IoCtx& io_ctx, const char* id, trash_image_info_t* info)
{
  return librbd::api::Trash<>::get(io_ctx, id, info);
}

int
RBD::trash_list(IoCtx& io_ctx, vector<trash_image_info_t>& entries)
{
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
  tracepoint(
      librbd, trash_list_enter, io_ctx.get_pool_name().c_str(), io_ctx.get_id());
...
int
RBD::trash_remove(IoCtx& io_ctx, const char* image_id, bool force)
{
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
  tracepoint(
      librbd, trash_remove_enter, io_ctx.get_pool_name().c_str(),
...
int
RBD::trash_remove_with_progress(
    IoCtx& io_ctx,
    const char* image_id,
    bool force,
...
int
RBD::trash_restore(IoCtx& io_ctx, const char* id, const char* name)
{
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
  tracepoint(
      librbd, trash_undelete_enter, io_ctx.get_pool_name().c_str(),
...
int
RBD::trash_purge(IoCtx& io_ctx, time_t expire_ts, float threshold)
{
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
  tracepoint(
      librbd, trash_purge_enter, io_ctx.get_pool_name().c_str(),
...
int
RBD::namespace_create(IoCtx& io_ctx, const char* namespace_name)
{
  return librbd::api::Namespace<>::create(io_ctx, namespace_name);
}

int
RBD::namespace_remove(IoCtx& io_ctx, const char* namespace_name)
{
  return librbd::api::Namespace<>::remove(io_ctx, namespace_name);
}

int
RBD::namespace_list(IoCtx& io_ctx, std::vector<std::string>* namespace_names)
{
  return librbd::api::Namespace<>::list(io_ctx, namespace_names);
}

int
RBD::namespace_exists(IoCtx& io_ctx, const char* namespace_name, bool* exists)
{
  return librbd::api::Namespace<>::exists(io_ctx, namespace_name, exists);
}

int
RBD::pool_init(IoCtx& io_ctx, bool force)
{
  return librbd::api::Pool<>::init(io_ctx, force);
}

int
RBD::pool_stats_get(IoCtx& io_ctx, PoolStats* stats)
{
...
int
RBD::list(IoCtx& io_ctx, vector<string>& names)
{
...
int
RBD::list2(IoCtx& io_ctx, std::vector<image_spec_t>* images)
{
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
  tracepoint(
      librbd, list_enter, io_ctx.get_pool_name().c_str(), io_ctx.get_id());
...
int
RBD::rename(IoCtx& src_io_ctx, const char* srcname, const char* destname)
{
  TracepointProvider::initialize<tracepoint_traits>(get_cct(src_io_ctx));
  tracepoint(
      librbd, rename_enter, src_io_ctx.get_pool_name().c_str(),
    ...
    ImageOptions& opts)
{
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
  tracepoint(
      librbd, migration_prepare_enter, io_ctx.get_pool_name().c_str(),
      io_ctx.get_id(), image_name, dest_io_ctx.get_pool_name().c_str(),
      dest_io_ctx.get_id(), dest_image_name, opts.opts);
  int r = librbd::api::Migration<>::prepare(
      io_ctx, image_name, dest_io_ctx, dest_image_name, opts);
  tracepoint(librbd, migration_prepare_exit, r);
  return r;
}

int
RBD::migration_prepare_import(
    const char* source_spec,
    IoCtx& dest_io_ctx,
    const char* dest_image_name,
    ImageOptions& opts)
{
  return librbd::api::Migration<>::prepare_import(
      source_spec, dest_io_ctx, dest_image_name, opts);
}

int
RBD::migration_execute(IoCtx& io_ctx, const char* image_name)
{
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
  tracepoint(
      librbd, migration_execute_enter, io_ctx.get_pool_name().c_str(),
...
int
RBD::migration_execute_with_progress(
    IoCtx& io_ctx,
    const char* image_name,
...
int
RBD::migration_abort(IoCtx& io_ctx, const char* image_name)
{
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
  tracepoint(
      librbd, migration_abort_enter, io_ctx.get_pool_name().c_str(),
      io_ctx.get_id(), image_name);
  librbd::NoOpProgressContext prog_ctx;
...
int
RBD::migration_abort_with_progress(
    IoCtx& io_ctx,
    const char* image_name,
    librbd::ProgressContext& prog_ctx)
{
...
int
RBD::migration_commit(IoCtx& io_ctx, const char* image_name)
{
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
  tracepoint(
      librbd, migration_commit_enter, io_ctx.get_pool_name().c_str(),
      io_ctx.get_id(), image_name);
  librbd::NoOpProgressContext prog_ctx;
...
int
RBD::migration_commit_with_progress(
    IoCtx& io_ctx,
    const char* image_name,
    librbd::ProgressContext& prog_ctx)
{
    ...
    size_t status_size)
{
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
  tracepoint(
      librbd, migration_status_enter, io_ctx.get_pool_name().c_str(),
      io_ctx.get_id(), image_name);
...
int
RBD::mirror_mode_get(IoCtx& io_ctx, rbd_mirror_mode_t* mirror_mode)
{
  return librbd::api::Mirror<>::mode_get(io_ctx, mirror_mode);
}

int
RBD::mirror_mode_set(IoCtx& io_ctx, rbd_mirror_mode_t mirror_mode)
{
  return librbd::api::Mirror<>::mode_set(io_ctx, mirror_mode);
}

int
RBD::mirror_remote_namespace_get(IoCtx& io_ctx, std::string* remote_namespace)
{
  return librbd::api::Mirror<>::remote_namespace_get(io_ctx, remote_namespace);
}

int
RBD::mirror_remote_namespace_set(
    IoCtx& io_ctx,
...
int
RBD::mirror_uuid_get(IoCtx& io_ctx, std::string* mirror_uuid)
{
  return librbd::api::Mirror<>::uuid_get(io_ctx, mirror_uuid);
}

int
RBD::mirror_site_name_get(librados::Rados& rados, std::string* site_name)
{
  return librbd::api::Mirror<>::site_name_get(rados, site_name);
}

int
RBD::mirror_site_name_set(librados::Rados& rados, const std::string& site_name)
{
  return librbd::api::Mirror<>::site_name_set(rados, site_name);
}

int
RBD::mirror_peer_bootstrap_create(IoCtx& io_ctx, std::string* token)
{
  return librbd::api::Mirror<>::peer_bootstrap_create(io_ctx, token);
}

int
RBD::mirror_peer_bootstrap_import(
    IoCtx& io_ctx,
...
int
RBD::mirror_peer_site_remove(IoCtx& io_ctx, const std::string& uuid)
{
  return librbd::api::Mirror<>::peer_site_remove(io_ctx, uuid);
}

int
RBD::mirror_peer_site_list(
    IoCtx& io_ctx,
...
int
RBD::mirror_peer_site_set_client_name(
    IoCtx& io_ctx,
...
int
RBD::mirror_peer_site_set_direction(
    IoCtx& io_ctx,
    const std::string& uuid,
    mirror_peer_direction_t direction)
{
...
int
RBD::mirror_peer_site_get_attributes(
    IoCtx& io_ctx,
    const std::string& uuid,
...
int
RBD::mirror_peer_site_set_attributes(
    IoCtx& io_ctx,
    const std::string& uuid,
...
int
RBD::mirror_image_global_status_list(
    IoCtx& io_ctx,
...
int
RBD::mirror_peer_remove(IoCtx& io_ctx, const std::string& uuid)
{
  return librbd::api::Mirror<>::peer_site_remove(io_ctx, uuid);
}

int
RBD::mirror_peer_list(IoCtx& io_ctx, std::vector<mirror_peer_t>* peers)
{
  std::vector<mirror_peer_site_t> peer_sites;
...
int
RBD::mirror_peer_get_attributes(
    IoCtx& io_ctx,
    const std::string& uuid,
    std::map<std::string, std::string>* key_vals)
{
...
int
RBD::mirror_peer_set_attributes(
    IoCtx& io_ctx,
    const std::string& uuid,
    const std::map<std::string, std::string>& key_vals)
{
...
int
RBD::mirror_image_status_summary(
    IoCtx& io_ctx,
...
int
RBD::mirror_image_instance_id_list(
    IoCtx& io_ctx,
    const std::string& start_id,
    size_t max,
    std::map<std::string, std::string>* instance_ids)
{
  return librbd::api::Mirror<>::image_instance_id_list(
      io_ctx, start_id, max, instance_ids);
}
...
RBD::mirror_image_info_list(
    ...
    std::map<std::string, std::pair<mirror_image_mode_t, mirror_image_info_t>>*
        entries)
{
  return librbd::api::Mirror<>::image_info_list(
      io_ctx, mode_filter, start_id, max, entries);
}

int
RBD::group_create(IoCtx& io_ctx, const char* group_name)
{
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
  tracepoint(
      librbd, group_create_enter, io_ctx.get_pool_name().c_str(),
...
int
RBD::group_remove(IoCtx& io_ctx, const char* group_name)
{
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
  tracepoint(
      librbd, group_remove_enter, io_ctx.get_pool_name().c_str(),
      io_ctx.get_id(), group_name);
...
int
RBD::group_list(IoCtx& io_ctx, vector<string>* names)
{
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
  tracepoint(
      librbd, group_list_enter, io_ctx.get_pool_name().c_str(), io_ctx.get_id());
...
int
RBD::group_get_id(IoCtx& io_ctx, const char* group_name, std::string* group_id)
{
  return librbd::api::Group<>::get_id(io_ctx, group_name, group_id);
}

int
RBD::group_rename(IoCtx& io_ctx, const char* src_name, const char* dest_name)
{
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
  tracepoint(
      librbd, group_rename_enter, io_ctx.get_pool_name().c_str(),
    ...
    const char* image_name)
{
  ...
  tracepoint(
      librbd, group_image_add_enter, group_ioctx.get_pool_name().c_str(),
      group_ioctx.get_id(), group_name, image_ioctx.get_pool_name().c_str(),
      image_ioctx.get_id(), image_name);
  int r = librbd::api::Group<>::image_add(
      group_ioctx, group_name, image_ioctx, image_name);
  tracepoint(librbd, group_image_add_exit, r);
  return r;
}

int
RBD::group_image_remove(
    IoCtx& group_ioctx,
    const char* group_name,
    IoCtx& image_ioctx,
    const char* image_name)
{
  TracepointProvider::initialize<tracepoint_traits>(get_cct(group_ioctx));
  tracepoint(
      librbd, group_image_remove_enter, group_ioctx.get_pool_name().c_str(),
      group_ioctx.get_id(), group_name, image_ioctx.get_pool_name().c_str(),
      image_ioctx.get_id(), image_name);
  int r = librbd::api::Group<>::image_remove(
      group_ioctx, group_name, image_ioctx, image_name);
    ...
    const char* image_id)
{
  TracepointProvider::initialize<tracepoint_traits>(get_cct(group_ioctx));
  tracepoint(
      librbd, group_image_remove_by_id_enter,
      group_ioctx.get_pool_name().c_str(), group_ioctx.get_id(), group_name,
      image_ioctx.get_pool_name().c_str(), image_ioctx.get_id(), image_id);
  int r = librbd::api::Group<>::image_remove_by_id(
      group_ioctx, group_name, image_ioctx, image_id);
  tracepoint(librbd, group_image_remove_by_id_exit, r);
  return r;
}

int
RBD::group_image_list(
    IoCtx& group_ioctx,
    const char* group_name,
    ...
    size_t group_image_info_size)
{
  TracepointProvider::initialize<tracepoint_traits>(get_cct(group_ioctx));
  tracepoint(
      librbd, group_image_list_enter, group_ioctx.get_pool_name().c_str(),
    ...
    const char* snap_name)
{
  TracepointProvider::initialize<tracepoint_traits>(get_cct(group_ioctx));
  tracepoint(
      librbd, group_snap_create_enter, group_ioctx.get_pool_name().c_str(),
      group_ioctx.get_id(), group_name, snap_name);
  auto flags = librbd::util::get_default_snap_create_flags(group_ioctx);
  int r = librbd::api::Group<>::snap_create(
      group_ioctx, group_name, snap_name, flags);
    ...
    const char* snap_name)
{
  TracepointProvider::initialize<tracepoint_traits>(get_cct(group_ioctx));
  tracepoint(
      librbd, group_snap_remove_enter, group_ioctx.get_pool_name().c_str(),
      group_ioctx.get_id(), group_name, snap_name);
    ...
    size_t group_snap_info_size)
{
  TracepointProvider::initialize<tracepoint_traits>(get_cct(group_ioctx));
  tracepoint(
      librbd, group_snap_list_enter, group_ioctx.get_pool_name().c_str(),
      group_ioctx.get_id(), group_name);
  ...
  int r = librbd::api::Group<>::snap_list(
      group_ioctx, group_name, true, false, &snaps2);

  for (const auto& snap : snaps2) {
    snaps->push_back(group_snap_info_t{snap.name, snap.state});
  }
...
int
RBD::group_snap_list2(
    IoCtx& group_ioctx,
    const char* group_name,
    std::vector<group_snap_info2_t>* snaps)
{
  return librbd::api::Group<>::snap_list(
      group_ioctx, group_name, true, false, snaps);
}

int
RBD::group_snap_get_info(
    IoCtx& group_ioctx,
    const char* group_name,
    const char* snap_name,
    group_snap_info2_t* group_snap)
{
  return librbd::api::Group<>::snap_get_info(
      group_ioctx, group_name, snap_name, group_snap);
}

int
RBD::group_snap_rename(
    IoCtx& group_ioctx,
    const char* group_name,
    ...
    const char* new_snap_name)
{
  TracepointProvider::initialize<tracepoint_traits>(get_cct(group_ioctx));
  tracepoint(
      librbd, group_snap_rename_enter, group_ioctx.get_pool_name().c_str(),
      group_ioctx.get_id(), group_name, old_snap_name, new_snap_name);
  int r = librbd::api::Group<>::snap_rename(
      group_ioctx, group_name, old_snap_name, new_snap_name);
  tracepoint(librbd, group_snap_list_exit, r);
  return r;
}

int
RBD::group_snap_rollback(
    IoCtx& group_ioctx,
    const char* group_name,
    const char* snap_name)
{
  TracepointProvider::initialize<tracepoint_traits>(get_cct(group_ioctx));
  tracepoint(
      librbd, group_snap_rollback_enter, group_ioctx.get_pool_name().c_str(),
      group_ioctx.get_id(), group_name, snap_name);
  librbd::NoOpProgressContext prog_ctx;
  int r = librbd::api::Group<>::snap_rollback(
      group_ioctx, group_name, snap_name, prog_ctx);
  tracepoint(librbd, group_snap_rollback_exit, r);
  return r;
}

int
RBD::group_snap_rollback_with_progress(
    IoCtx& group_ioctx,
    const char* group_name,
    const char* snap_name,
...
int
RBD::pool_metadata_get(IoCtx& ioctx, const std::string& key, std::string* value)
{
...
int
RBD::pool_metadata_remove(IoCtx& ioctx, const std::string& key)
{
...
int
RBD::config_list(IoCtx& io_ctx, std::vector<config_option_t>* options)
{
...
int
RBD::AioCompletion::wait_for_complete()
{
  librbd::io::AioCompletion* c = (librbd::io::AioCompletion*)pc;
...
int
ImageOptions::set(int optname, const std::string& optval)
{
  return librbd::image_options_set(opts, optname, optval);
}

int
ImageOptions::set(int optname, uint64_t optval)
{
  return librbd::image_options_set(opts, optname, optval);
}

int
ImageOptions::get(int optname, std::string* optval) const
{
  return librbd::image_options_get(opts, optname, optval);
}

int
ImageOptions::get(int optname, uint64_t* optval) const
{
  return librbd::image_options_get(opts, optname, optval);
}

int
ImageOptions::is_set(int optname, bool* is_set)
{
  return librbd::image_options_is_set(opts, optname, is_set);
}

int
ImageOptions::unset(int optname)
{
...
Image::close()
{
  ...
  if (ctx) {
    ...
    tracepoint(
        librbd, close_image_enter, ictx, ictx->name.c_str(), ictx->id.c_str());
...
int
Image::aio_close(RBD::AioCompletion* c)
{
  if (!ctx) {
    return -EINVAL;
  }
  ...
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, aio_close_image_enter, ictx, ictx->name.c_str(), ictx->id.c_str(),
      c->pc);

  ctx = NULL; // before initiating close
  ictx->state->close(new C_AioCompletion(
      ictx, librbd::io::AIO_TYPE_CLOSE, get_aio_completion(c)));
...
int
Image::resize(uint64_t size)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, resize_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(),
      ictx->read_only, size);
  librbd::NoOpProgressContext prog_ctx;
...
int
Image::resize2(uint64_t size, bool allow_shrink, librbd::ProgressContext& pctx)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, resize_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(),
...
int
Image::resize_with_progress(uint64_t size, librbd::ProgressContext& pctx)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, resize_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(),
...
int
Image::stat(image_info_t& info, size_t infosize)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, stat_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(),
      ictx->read_only);
...
int
Image::old_format(uint8_t* old)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, get_old_format_enter, ictx, ictx->name.c_str(),
      ictx->snap_name.c_str(), ictx->read_only);
...
int
Image::size(uint64_t* size)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, get_size_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(),
      ictx->read_only);
...
int
Image::get_group(group_info_t* group_info, size_t group_info_size)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
...
int
Image::features(uint64_t* features)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, get_features_enter, ictx, ictx->name.c_str(),
      ictx->snap_name.c_str(), ictx->read_only);
...
int
Image::update_features(uint64_t features, bool enabled)
{
...
int
Image::get_op_features(uint64_t* op_features)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
...
uint64_t
Image::get_stripe_unit() const
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, get_stripe_unit_enter, ictx, ictx->name.c_str(),
      ictx->snap_name.c_str(), ictx->read_only);
...
uint64_t
Image::get_stripe_count() const
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, get_stripe_count_enter, ictx, ictx->name.c_str(),
      ictx->snap_name.c_str(), ictx->read_only);
...
int
Image::get_create_timestamp(struct timespec* timestamp)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, get_create_timestamp_enter, ictx, ictx->name.c_str(),
      ictx->read_only);
...
int
Image::get_access_timestamp(struct timespec* timestamp)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, get_access_timestamp_enter, ictx, ictx->name.c_str(),
      ictx->read_only);
  {
...
int
Image::get_modify_timestamp(struct timespec* timestamp)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, get_modify_timestamp_enter, ictx, ictx->name.c_str(),
      ictx->read_only);
  {
    std::shared_lock timestamp_locker{ictx->timestamp_lock};
...
int
Image::overlap(uint64_t* overlap)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, get_overlap_enter, ictx, ictx->name.c_str(),
      ictx->snap_name.c_str(), ictx->read_only);
...
int
Image::get_name(std::string* name)
{
  ImageCtx* ictx = reinterpret_cast<ImageCtx*>(ctx);
  *name = ictx->name;
  return 0;
...
int
Image::get_parent(linked_image_spec_t* parent_image, snap_spec_t* parent_snap)
{
  auto ictx = reinterpret_cast<ImageCtx*>(ctx);
  tracepoint(
      librbd, get_parent_info_enter, ictx, ictx->name.c_str(),
      ictx->snap_name.c_str(), ictx->read_only);

  int r = librbd::api::Image<>::get_parent(ictx, parent_image, parent_snap);

  tracepoint(
      librbd, get_parent_info_exit, r, parent_image->pool_name.c_str(),
...
int
Image::get_migration_source_spec(std::string* source_spec)
{
  auto ictx = reinterpret_cast<ImageCtx*>(ctx);
...
int
Image::get_flags(uint64_t* flags)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
...
int
Image::set_image_notification(int fd, int type)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
...
int
Image::is_exclusive_lock_owner(bool* is_owner)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
...
int
Image::lock_acquire(rbd_lock_mode_t lock_mode)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
...
int
Image::lock_break(rbd_lock_mode_t lock_mode, const std::string& lock_owner)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
...
int
Image::rebuild_object_map(ProgressContext& prog_ctx)
{
  ImageCtx* ictx = reinterpret_cast<ImageCtx*>(ctx);
...
int
Image::check_object_map(ProgressContext& prog_ctx)
{
  ImageCtx* ictx = reinterpret_cast<ImageCtx*>(ctx);
...
int
Image::copy(IoCtx& dest_io_ctx, const char* destname)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, copy_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(),
      ictx->read_only, dest_io_ctx.get_pool_name().c_str(),
      dest_io_ctx.get_id(), destname);
  ImageOptions opts;
  librbd::NoOpProgressContext prog_ctx;
...
int
Image::copy2(Image& dest)
{
  ImageCtx* srcctx = (ImageCtx*)ctx;
  ImageCtx* destctx = (ImageCtx*)dest.ctx;
  tracepoint(
      librbd, copy2_enter, srcctx, srcctx->name.c_str(),
      srcctx->snap_name.c_str(), srcctx->read_only, destctx,
      destctx->name.c_str(), destctx->snap_name.c_str(), destctx->read_only);
  librbd::NoOpProgressContext prog_ctx;
...
int
Image::copy3(IoCtx& dest_io_ctx, const char* destname, ImageOptions& opts)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, copy3_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(),
      ictx->read_only, dest_io_ctx.get_pool_name().c_str(),
      dest_io_ctx.get_id(), destname, opts.opts);
  librbd::NoOpProgressContext prog_ctx;
  int r = librbd::copy(ictx, dest_io_ctx, destname, opts, prog_ctx, 0);
    ...
    size_t sparse_size)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, copy4_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(),
      ictx->read_only, dest_io_ctx.get_pool_name().c_str(),
      dest_io_ctx.get_id(), destname, opts.opts, sparse_size);
  librbd::NoOpProgressContext prog_ctx;
...
int
Image::copy_with_progress2(Image& dest, librbd::ProgressContext& pctx)
{
  ImageCtx* srcctx = (ImageCtx*)ctx;
  ImageCtx* destctx = (ImageCtx*)dest.ctx;
  tracepoint(
...
int
Image::deep_copy(IoCtx& dest_io_ctx, const char* destname, ImageOptions& opts)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, deep_copy_enter, ictx, ictx->name.c_str(),
      ictx->snap_name.c_str(), ictx->read_only,
      dest_io_ctx.get_pool_name().c_str(), dest_io_ctx.get_id(), destname,
      opts.opts);
  librbd::NoOpProgressContext prog_ctx;
  int r = librbd::api::Image<>::deep_copy(
      ictx, dest_io_ctx, destname, opts, prog_ctx);
  tracepoint(librbd, deep_copy_exit, r);
  return r;
}

int
Image::deep_copy_with_progress(
    IoCtx& dest_io_ctx,
    const char* destname,
    ImageOptions& opts,
    librbd::ProgressContext& prog_ctx)
...
int
Image::encryption_load2(const encryption_spec_t* specs, size_t spec_count)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
...
int
Image::flatten_with_progress(librbd::ProgressContext& prog_ctx)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(librbd, flatten_enter, ictx, ictx->name.c_str(), ictx->id.c_str());
  int r = ictx->operations->flatten(prog_ctx);
...
int
Image::sparsify(size_t sparse_size)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, sparsify_enter, ictx, ictx->name.c_str(), sparse_size,
      ictx->id.c_str());
  librbd::NoOpProgressContext prog_ctx;
...
int
Image::list_children(set<pair<string, string>>* children)
{
...
int
Image::list_children2(vector<librbd::child_info_t>* children)
{
  std::vector<linked_image_spec_t> images;
  int r = list_children3(&images);
  if (r < 0) {
...
int
Image::list_children3(std::vector<linked_image_spec_t>* images)
{
  auto ictx = reinterpret_cast<ImageCtx*>(ctx);
  tracepoint(
      librbd, list_children_enter, ictx, ictx->name.c_str(),
      ictx->snap_name.c_str(), ictx->read_only);
  ...
  if (r >= 0) {
    for (auto& it : *images) {
      tracepoint(
          librbd, list_children_entry, it.pool_name.c_str(),
          it.image_name.c_str());
    }
  }
#endif
...
int
Image::list_descendants(std::vector<linked_image_spec_t>* images)
{
  auto ictx = reinterpret_cast<ImageCtx*>(ctx);
    ...
    string* tag)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, list_lockers_enter, ictx, ictx->name.c_str(),
      ictx->snap_name.c_str(), ictx->read_only);
  ...
  if (r >= 0) {
    for (std::list<librbd::locker_t>::const_iterator it = lockers->begin();
         it != lockers->end(); ++it) {
      tracepoint(
          librbd, list_lockers_entry, it->client.c_str(), it->cookie.c_str(),
          it->address.c_str());
    }
  }
...
int
Image::lock_exclusive(const string& cookie)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, lock_exclusive_enter, ictx, ictx->name.c_str(),
...
int
Image::lock_shared(const string& cookie, const std::string& tag)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, lock_shared_enter, ictx, ictx->name.c_str(),
      ictx->snap_name.c_str(), ictx->read_only, cookie.c_str(), tag.c_str());
...
int
Image::unlock(const string& cookie)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, unlock_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(),
      ictx->read_only, cookie.c_str());
...
int
Image::break_lock(const string& client, const string& cookie)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, break_lock_enter, ictx, ictx->name.c_str(),
      ictx->snap_name.c_str(), ictx->read_only, client.c_str(), cookie.c_str());
...
int
Image::snap_create(const char* snap_name)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, snap_create_enter, ictx, ictx->name.c_str(),
...
int
Image::snap_remove(const char* snap_name)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, snap_remove_enter, ictx, ictx->name.c_str(),
      ictx->snap_name.c_str(), ictx->read_only, snap_name);
  librbd::NoOpProgressContext prog_ctx;
...
int
Image::snap_remove2(const char* snap_name, uint32_t flags, ProgressContext& pctx)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, snap_remove2_enter, ictx, ictx->name.c_str(),
...
int
Image::snap_remove_by_id(uint64_t snap_id)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
...
int
Image::snap_rollback(const char* snap_name)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, snap_rollback_enter, ictx, ictx->name.c_str(),
      ictx->snap_name.c_str(), ictx->read_only, snap_name);
  librbd::NoOpProgressContext prog_ctx;
  int r = ictx->operations->snap_rollback(
      cls::rbd::UserSnapshotNamespace(), snap_name, prog_ctx);
  tracepoint(librbd, snap_rollback_exit, r);
  return r;
}

int
Image::snap_rename(const char* srcname, const char* dstname)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, snap_rename_enter, ictx, ictx->name.c_str(),
...
int
Image::snap_rollback_with_progress(
    const char* snap_name,
    ProgressContext& prog_ctx)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
...
int
Image::snap_protect(const char* snap_name)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, snap_protect_enter, ictx, ictx->name.c_str(),
      ictx->snap_name.c_str(), ictx->read_only, snap_name);
...
int
Image::snap_unprotect(const char* snap_name)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, snap_unprotect_enter, ictx, ictx->name.c_str(),
      ictx->snap_name.c_str(), ictx->read_only, snap_name);
...
int
Image::snap_is_protected(const char* snap_name, bool* is_protected)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, snap_is_protected_enter, ictx, ictx->name.c_str(),
      ictx->snap_name.c_str(), ictx->read_only, snap_name);
...
int
Image::snap_list(vector<librbd::snap_info_t>& snaps)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, snap_list_enter, ictx, ictx->name.c_str(),
      ictx->snap_name.c_str(), ictx->read_only, &snaps);
  int r = librbd::api::Snapshot<>::list(ictx, snaps);
  if (r >= 0) {
    for (int i = 0, n = snaps.size(); i < n; i++) {
      tracepoint(
          librbd, snap_list_entry, snaps[i].id, snaps[i].size,
          snaps[i].name.c_str());
    }
  }
...
bool
Image::snap_exists(const char* snap_name)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, snap_exists_enter, ictx, ictx->name.c_str(),
      ictx->snap_name.c_str(), ictx->read_only, snap_name);
  bool exists;
...
int
Image::snap_exists2(const char* snap_name, bool* exists)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, snap_exists_enter, ictx, ictx->name.c_str(),
...
int
Image::snap_get_timestamp(uint64_t snap_id, struct timespec* timestamp)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
...
int
Image::snap_get_limit(uint64_t* limit)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
...
int
Image::snap_get_trash_namespace(uint64_t snap_id, std::string* original_name)
{
  ImageCtx* ictx = (ImageCtx*)ctx;

  snap_trash_namespace_t trash_snap;
  int r =
      librbd::api::Snapshot<>::get_trash_namespace(ictx, snap_id, &trash_snap);
  if (r < 0) {
    return r;
  }
...
int
Image::snap_get_trash_namespace2(
    uint64_t snap_id,
    snap_trash_namespace_t* trash_snap,
    size_t trash_snap_size)
{
  ...
  return librbd::api::Snapshot<>::get_trash_namespace(ictx, snap_id, trash_snap);
}

int
Image::snap_get_mirror_namespace(
    uint64_t snap_id,
...
int
Image::snap_set_limit(uint64_t limit)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
...
int
Image::snap_set(const char* snap_name)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, snap_set_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(),
      ictx->read_only, snap_name);
...
int
Image::snap_set_by_id(uint64_t snap_id)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
...
int
Image::snap_get_name(uint64_t snap_id, std::string* snap_name)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
...
int
Image::snap_get_id(const std::string snap_name, uint64_t* snap_id)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
...
ssize_t
Image::read(uint64_t ofs, size_t len, bufferlist& bl)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, read_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(),
...
ssize_t
Image::read2(uint64_t ofs, size_t len, bufferlist& bl, int op_flags)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, read2_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(),
    ...
    void* arg)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, read_iterate_enter, ictx, ictx->name.c_str(),
    ...
    void* arg)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, read_iterate2_enter, ictx, ictx->name.c_str(),
      ictx->snap_name.c_str(), ictx->read_only, ofs, len);
    ...
    void* arg)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, diff_iterate_enter, ictx, ictx->name.c_str(),
      ictx->snap_name.c_str(), ictx->read_only, fromsnapname, ofs, len, true,
      false);
  uint64_t from_snap_id = 0;
  if (fromsnapname != nullptr) {
    std::shared_lock image_locker{ictx->image_lock};
    from_snap_id =
        ictx->get_snap_id(cls::rbd::UserSnapshotNamespace(), fromsnapname);
  }
  int r = librbd::api::DiffIterate<>::diff_iterate(
      ictx, from_snap_id, ofs, len, true, false, cb, arg);
    ...
    void* arg)
{
  ...
  int r = librbd::api::DiffIterate<>::diff_iterate(
      ictx, from_snap_id, ofs, len, include_parent, whole_object, cb, arg);
  tracepoint(librbd, diff_iterate_exit, r);
  return r;
}
    ...
    void* arg)
{
  ...
  return librbd::api::DiffIterate<>::diff_iterate(
      ictx, from_snap_id, ofs, len, include_parent, whole_object, cb, arg);
}

ssize_t
Image::write(uint64_t ofs, size_t len, bufferlist& bl)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, write_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(),
      ictx->read_only, ofs, len, bl.length() < len ? NULL : bl.c_str());
  if (bl.length() < len) {
...
ssize_t
Image::write2(uint64_t ofs, size_t len, bufferlist& bl, int op_flags)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, write2_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(),
      ictx->read_only, ofs, len, bl.length() < len ? NULL : bl.c_str(),
      op_flags);
  if (bl.length() < len) {
    tracepoint(librbd, write_exit, -EINVAL);
...
int
Image::discard(uint64_t ofs, uint64_t len)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, discard_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(),
      ictx->read_only, ofs, len);
...
ssize_t
Image::writesame(uint64_t ofs, size_t len, bufferlist& bl, int op_flags)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, writesame_enter, ictx, ictx->name.c_str(),
      ictx->snap_name.c_str(), ictx->read_only, ofs, len,
...
ssize_t
Image::write_zeroes(uint64_t ofs, size_t len, int zero_flags, int op_flags)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
    ...
    int op_flags)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, compare_and_write_enter, ictx, ictx->name.c_str(),
      ictx->snap_name.c_str(), ictx->read_only, ofs, len,
...
int
Image::aio_write(uint64_t off, size_t len, bufferlist& bl, RBD::AioCompletion* c)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, aio_write_enter, ictx, ictx->name.c_str(),
  ...
  api::Io<>::aio_write(
      *ictx, get_aio_completion(c), off, len, bufferlist{bl}, 0, true);
    ...
    int op_flags)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, aio_write2_enter, ictx, ictx->name.c_str(),
      ictx->snap_name.c_str(), ictx->read_only, off, len,
  ...
  api::Io<>::aio_write(
      *ictx, get_aio_completion(c), off, len, bufferlist{bl}, op_flags, true);
...
int
Image::aio_read(uint64_t off, size_t len, bufferlist& bl, RBD::AioCompletion* c)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, aio_read_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(),
      ictx->read_only, off, len, bl.c_str(), c->pc);
  ldout(ictx->cct, 10) << "Image::aio_read() buf=" << (void*)bl.c_str() << "~"
                       << (void*)(bl.c_str() + len - 1) << dendl;

  api::Io<>::aio_read(
      *ictx, get_aio_completion(c), off, len, io::ReadResult{&bl}, 0, true);
    ...
    int op_flags)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, aio_read2_enter, ictx, ictx->name.c_str(),
      ictx->snap_name.c_str(), ictx->read_only, off, len, bl.c_str(), c->pc,
      op_flags);
  ldout(ictx->cct, 10) << "Image::aio_read() buf=" << (void*)bl.c_str() << "~"
                       << (void*)(bl.c_str() + len - 1) << dendl;

  api::Io<>::aio_read(
      *ictx, get_aio_completion(c), off, len, io::ReadResult{&bl}, op_flags,
      true);
  tracepoint(librbd, aio_read_exit, 0);
  return 0;
}
...
Image::flush()
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, flush_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(),
      ictx->read_only);
...
int
Image::aio_flush(RBD::AioCompletion* c)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, aio_flush_enter, ictx, ictx->name.c_str(),
      ictx->snap_name.c_str(), ictx->read_only, c->pc);
...
int
Image::aio_discard(uint64_t off, uint64_t len, RBD::AioCompletion* c)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, aio_discard_enter, ictx, ictx->name.c_str(),
    ...
    int op_flags)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, aio_writesame_enter, ictx, ictx->name.c_str(),
      ictx->snap_name.c_str(), ictx->read_only, off, len,
      bl.length() <= len ? NULL : bl.c_str(), bl.length(), c->pc, op_flags);
  if (bl.length() == 0 || len % bl.length()) {
  ...
  bool discard_zero =
      ictx->config.get_val<bool>("rbd_discard_on_zeroed_write_same");
  if (discard_zero && bl.is_zero()) {
    api::Io<>::aio_write_zeroes(
        *ictx, get_aio_completion(c), off, len, 0U, op_flags, true);
    tracepoint(librbd, aio_writesame_exit, 0);
    return 0;
  }

  api::Io<>::aio_write_same(
      *ictx, get_aio_completion(c), off, len, bufferlist{bl}, op_flags, true);
  tracepoint(librbd, aio_writesame_exit, 0);
  return 0;
}
    ...
    int op_flags)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  api::Io<>::aio_write_zeroes(
      *ictx, get_aio_completion(c), off, len, zero_flags, op_flags, true);
  return 0;
}
    ...
    int op_flags)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, aio_compare_and_write_enter, ictx, ictx->name.c_str(),
      ictx->snap_name.c_str(), ictx->read_only, off, len,
      cmp_bl.length() < len ? NULL : cmp_bl.c_str(),
      bl.length() < len ? NULL : bl.c_str(), c->pc, op_flags);
  ...
  api::Io<>::aio_compare_and_write(
      *ictx, get_aio_completion(c), off, len, bufferlist{cmp_bl},
      bufferlist{bl}, mismatch_off, op_flags, false);
...
Image::invalidate_cache()
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, invalidate_cache_enter, ictx, ictx->name.c_str(),
      ictx->snap_name.c_str(), ictx->read_only);
...
int
Image::poll_io_events(RBD::AioCompletion** comps, int numcomp)
{
...
int
Image::metadata_get(const std::string& key, std::string* value)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
...
int
Image::metadata_set(const std::string& key, const std::string& value)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
...
int
Image::metadata_remove(const std::string& key)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
    ...
    map<string, bufferlist>* pairs)
{
  ...
  if (r >= 0) {
    for (map<string, bufferlist>::iterator it = pairs->begin();
         it != pairs->end(); ++it) {
      tracepoint(
          librbd, metadata_list_entry, it->first.c_str(), it->second.c_str());
    }
  }
...
int
Image::mirror_image_enable2(mirror_image_mode_t mode)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
...
int
Image::mirror_image_disable(bool force)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
...
int
Image::mirror_image_promote(bool force)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
...
int
Image::mirror_image_create_snapshot(uint64_t* snap_id)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  auto flags = librbd::util::get_default_snap_create_flags(ictx);
...
int
Image::mirror_image_create_snapshot2(uint32_t flags, uint64_t* snap_id)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  return librbd::api::Mirror<>::image_snapshot_create(ictx, flags, snap_id);
}
...
int
Image::mirror_image_get_mode(mirror_image_mode_t* mode)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
...
int
Image::mirror_image_get_global_status(
    ...
    size_t status_size)
{
  ...
  librbd::mirror_image_site_status_t local_status;
  r = get_local_mirror_image_site_status(
      mirror_image_global_status, &local_status);
  if (r < 0) {
    return r;
  }

  *mirror_image_status = mirror_image_status_t{
      mirror_image_global_status.name,
...
int
Image::mirror_image_get_instance_id(std::string* instance_id)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
...
int
Image::aio_mirror_image_promote(bool force, RBD::AioCompletion* c)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  librbd::api::Mirror<>::image_promote(
      ictx, force,
      new C_AioCompletion(
          ictx, librbd::io::AIO_TYPE_GENERIC, get_aio_completion(c)));
  return 0;
}

int
Image::aio_mirror_image_demote(RBD::AioCompletion* c)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  librbd::api::Mirror<>::image_demote(
      ictx, new C_AioCompletion(
                ictx, librbd::io::AIO_TYPE_GENERIC, get_aio_completion(c)));
  return 0;
}
...
int
Image::aio_mirror_image_get_mode(mirror_image_mode_t* mode, RBD::AioCompletion* c)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
...
int
Image::aio_mirror_image_get_status(
    mirror_image_status_t* status,
    size_t status_size,
    RBD::AioCompletion* c)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
...
int
Image::update_watch(UpdateWatchCtx* wctx, uint64_t* handle)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
...
int
Image::update_unwatch(uint64_t handle)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
...
int
Image::list_watchers(std::list<librbd::image_watcher_t>& watchers)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
  tracepoint(
      librbd, list_watchers_enter, ictx, ictx->name.c_str(),
      ictx->snap_name.c_str(), ictx->read_only);
  ...
  if (r >= 0) {
    for (auto& watcher : watchers) {
      tracepoint(
          librbd, list_watchers_entry, watcher.addr.c_str(), watcher.id,
          watcher.cookie);
    }
  }
#endif
...
int
Image::config_list(std::vector<config_option_t>* options)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
...
int
Image::quiesce_watch(QuiesceWatchCtx* wctx, uint64_t* handle)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
...
int
Image::quiesce_unwatch(uint64_t handle)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
...
void
Image::quiesce_complete(uint64_t handle, int r)
{
  ImageCtx* ictx = (ImageCtx*)ctx;
...
extern "C" void
rbd_version(int* major, int* minor, int* extra)
{
  if (major)
...
extern "C" void
rbd_image_options_create(rbd_image_options_t* opts)
{
  librbd::image_options_create(opts);
}

extern "C" void
rbd_image_options_destroy(rbd_image_options_t opts)
{
  librbd::image_options_destroy(opts);
}

extern "C" int
rbd_image_options_set_string(
...
extern "C" int
rbd_image_options_is_set(rbd_image_options_t opts, int optname, bool* is_set)
{
  return librbd::image_options_is_set(opts, optname, is_set);
}
...
/* pool mirroring */
extern "C" int
rbd_mirror_site_name_get(rados_t cluster, char* name, size_t* max_len)
{
...
extern "C" int
rbd_mirror_site_name_set(rados_t cluster, const char* name)
{
  librados::Rados rados;
  librados::Rados::from_rados_t(cluster, rados);
...
extern "C" int
rbd_mirror_mode_get(rados_ioctx_t p, rbd_mirror_mode_t* mirror_mode)
{
...
extern "C" int
rbd_mirror_remote_namespace_get(
    rados_ioctx_t p,
    ...
    size_t* max_len)
{
  ...
  int r = librbd::api::Mirror<>::remote_namespace_get(
      io_ctx, &remote_namespace_str);
  if (r < 0) {
    return r;
  }
...
extern "C" int
rbd_mirror_remote_namespace_set(rados_ioctx_t p, const char* remote_namespace)
{
  librados::IoCtx io_ctx;
  librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
  return librbd::api::Mirror<>::remote_namespace_set(io_ctx, remote_namespace);
...
extern "C" int
rbd_mirror_uuid_get(rados_ioctx_t p, char* mirror_uuid, size_t* max_len)
{
  librados::IoCtx io_ctx;
  librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
...
extern "C" int
rbd_mirror_peer_bootstrap_create(rados_ioctx_t p, char* token, size_t* max_len)
{
  librados::IoCtx io_ctx;
  librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
    ...
    const char* client_name)
{
  ...
  std::string uuid_str;
  int r = librbd::api::Mirror<>::peer_site_add(
      io_ctx, &uuid_str, direction, site_name, client_name);
  if (r >= 0) {
...
extern "C" int
rbd_mirror_peer_site_remove(rados_ioctx_t p, const char* uuid)
{
  librados::IoCtx io_ctx;
  librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
...
extern "C" void
rbd_mirror_peer_site_list_cleanup(rbd_mirror_peer_site_t* peers, int max_peers)
{
...
extern "C" int
rbd_mirror_peer_site_set_client_name(
    rados_ioctx_t p,
    const char* uuid,
    const char* client_name)
{
...
extern "C" int
rbd_mirror_peer_site_set_direction(
    rados_ioctx_t p,
    const char* uuid,
    rbd_mirror_peer_direction_t direction)
{
...
extern "C" int
rbd_mirror_peer_site_get_attributes(
    rados_ioctx_t p,
    const char* uuid,
    char* keys,
    size_t* max_key_len,
...
extern "C" int
rbd_mirror_peer_site_set_attributes(
    rados_ioctx_t p,
    const char* uuid,
    const char* keys,
    const char* values,
...
extern "C" int
rbd_mirror_image_global_status_list(
    rados_ioctx_t p,
    const char* start_id,
    size_t max,
    char** image_ids,
...
extern "C" void
rbd_mirror_image_global_status_cleanup(
...
extern "C" void
rbd_mirror_image_global_status_list_cleanup(
    char** image_ids,
    rbd_mirror_image_global_status_t* images,
    size_t len)
{
...
extern "C" int
rbd_mirror_peer_remove(rados_ioctx_t p, const char* uuid)
{
  return rbd_mirror_peer_site_remove(p, uuid);
}

extern "C" int
rbd_mirror_peer_list(rados_ioctx_t p, rbd_mirror_peer_t* peers, int* max_peers)
{
  librados::IoCtx io_ctx;
  librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
...
extern "C" void
rbd_mirror_peer_list_cleanup(rbd_mirror_peer_t* peers, int max_peers)
{
  for (int i = 0; i < max_peers; ++i) {
    free(peers[i].uuid);
...
extern "C" int
rbd_mirror_peer_get_attributes(
    rados_ioctx_t p,
    const char* uuid,
    char* keys,
    size_t* max_key_len,
...
extern "C" int
rbd_mirror_peer_set_attributes(
    rados_ioctx_t p,
    const char* uuid,
    const char* keys,
    const char* values,
...
extern "C" void
rbd_mirror_image_status_list_cleanup(
    char** image_ids,
    rbd_mirror_image_status_t* images,
    size_t len)
{
...
extern "C" int
rbd_mirror_image_status_summary(
    rados_ioctx_t p,
    ...
    size_t* len)
{
  ...
  int r = librbd::api::Mirror<>::image_instance_id_list(
      io_ctx, start_id, max, &cpp_instance_ids);
  if (r < 0) {
    return r;
  }
...
extern "C" void
rbd_mirror_image_instance_id_list_cleanup(
    char** image_ids,
    char** instance_ids,
    size_t len)
{
    ...
    size_t* num_entries)
{
  ...
  int r = librbd::api::Mirror<>::image_info_list(
      io_ctx, mode_filter, start_id, max, &cpp_entries);
  if (r < 0) {
    return r;
  }
...
extern "C" void
rbd_mirror_image_info_list_cleanup(
    char** image_ids,
    rbd_mirror_image_info_t* info_entries,
    size_t num_entries)
{
...
extern "C" void
rbd_image_spec_cleanup(rbd_image_spec_t* image)
{
  free(image->id);
  free(image->name);
}
...
extern "C" void
rbd_image_spec_list_cleanup(rbd_image_spec_t* images, size_t num_images)
{
...
extern "C" void
rbd_linked_image_spec_list_cleanup(
...
extern "C" void
rbd_snap_spec_cleanup(rbd_snap_spec_t* snap)
{
  free(snap->name);
}
...
/* images */
extern "C" int
rbd_list(rados_ioctx_t p, char* names, size_t* size)
{
  librados::IoCtx io_ctx;
  librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
...
extern "C" int
rbd_list2(rados_ioctx_t p, rbd_image_spec_t* images, size_t* size)
{
  librados::IoCtx io_ctx;
  librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
...
extern "C" int
rbd_create(rados_ioctx_t p, const char* name, uint64_t size, int* order)
{
  librados::IoCtx io_ctx;
  librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
    ...
    int* c_order)
{
  ...
  tracepoint(
      librbd, clone_enter, p_ioc.get_pool_name().c_str(), p_ioc.get_id(),
      p_name, p_snap_name, c_ioc.get_pool_name().c_str(), c_ioc.get_id(),
      c_name, features);
  int r = librbd::clone(
      p_ioc, p_name, p_snap_name, c_ioc, c_name, features, c_order, 0, 0);
  tracepoint(librbd, clone_exit, r, *c_order);
  return r;
}
    ...
    int stripe_count)
{
  ...
  TracepointProvider::initialize<tracepoint_traits>(get_cct(p_ioc));
  tracepoint(
      librbd, clone2_enter, p_ioc.get_pool_name().c_str(), p_ioc.get_id(),
      p_name, p_snap_name, c_ioc.get_pool_name().c_str(), c_ioc.get_id(),
      c_name, features, stripe_unit, stripe_count);
  int r = librbd::clone(
      p_ioc, p_name, p_snap_name, c_ioc, c_name, features, c_order, stripe_unit,
      stripe_count);
  tracepoint(librbd, clone2_exit, r, *c_order);
  return r;
}
    ...
    rbd_image_options_t c_opts)
{
  ...
  TracepointProvider::initialize<tracepoint_traits>(get_cct(p_ioc));
  tracepoint(
      librbd, clone3_enter, p_ioc.get_pool_name().c_str(), p_ioc.get_id(),
      p_name, p_snap_name, c_ioc.get_pool_name().c_str(), c_ioc.get_id(),
      c_name, c_opts);
  librbd::ImageOptions c_opts_(c_opts);
  int r = librbd::clone(
      p_ioc, nullptr, p_name, CEPH_NOSNAP, p_snap_name, c_ioc, nullptr, c_name,
      c_opts_, "", "");
  tracepoint(librbd, clone3_exit, r);
  return r;
}
    ...
    rbd_image_options_t c_opts)
{
  ...
  TracepointProvider::initialize<tracepoint_traits>(get_cct(p_ioc));
  tracepoint(
      librbd, clone4_enter, p_ioc.get_pool_name().c_str(), p_ioc.get_id(),
      p_name, p_snap_id, c_ioc.get_pool_name().c_str(), c_ioc.get_id(), c_name,
      c_opts);
  librbd::ImageOptions c_opts_(c_opts);
  int r = librbd::clone(
      p_ioc, nullptr, p_name, p_snap_id, nullptr, c_ioc, nullptr, c_name,
      c_opts_, "", "");
  tracepoint(librbd, clone4_exit, r);
  return r;
}
...
extern "C" int
rbd_remove(rados_ioctx_t p, const char* name)
{
  librados::IoCtx io_ctx;
  librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
...
extern "C" int
rbd_trash_move(rados_ioctx_t p, const char* name, uint64_t delay)
{
  librados::IoCtx io_ctx;
  librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
...
extern "C" int
rbd_trash_get(rados_ioctx_t io, const char* id, rbd_trash_image_info_t* info)
{
  librados::IoCtx io_ctx;
...
extern "C" void
rbd_trash_list_cleanup(rbd_trash_image_info_t* entries, size_t num_entries)
{
  for (size_t i = 0; i < num_entries; i++) {
...
extern "C" int
rbd_trash_purge(rados_ioctx_t io, time_t expire_ts, float threshold)
{
  librados::IoCtx io_ctx;
  librados::IoCtx::from_rados_ioctx_t(io, io_ctx);
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
...
extern "C" int
rbd_trash_remove(rados_ioctx_t p, const char* image_id, bool force)
{
  librados::IoCtx io_ctx;
  librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
...
extern "C" int
rbd_trash_remove_with_progress(
    rados_ioctx_t p,
    const char* image_id,
    bool force,
    librbd_progress_fn_t cb,
...
extern "C" int
rbd_trash_restore(rados_ioctx_t p, const char* id, const char* name)
{
  librados::IoCtx io_ctx;
  librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
...
extern "C" int
rbd_namespace_create(rados_ioctx_t io, const char* namespace_name)
{
  librados::IoCtx io_ctx;
  librados::IoCtx::from_rados_ioctx_t(io, io_ctx);
...
extern "C" int
rbd_namespace_remove(rados_ioctx_t io, const char* namespace_name)
{
  librados::IoCtx io_ctx;
  librados::IoCtx::from_rados_ioctx_t(io, io_ctx);
...
extern "C" int
rbd_namespace_list(rados_ioctx_t io, char* names, size_t* size)
{
  librados::IoCtx io_ctx;
  librados::IoCtx::from_rados_ioctx_t(io, io_ctx);
...
extern "C" int
rbd_namespace_exists(rados_ioctx_t io, const char* namespace_name, bool* exists)
{
  librados::IoCtx io_ctx;
  librados::IoCtx::from_rados_ioctx_t(io, io_ctx);
...
extern "C" int
rbd_pool_init(rados_ioctx_t io, bool force)
{
  librados::IoCtx io_ctx;
  librados::IoCtx::from_rados_ioctx_t(io, io_ctx);
...
extern "C" void
rbd_pool_stats_create(rbd_pool_stats_t* stats)
{
...
extern "C" void
rbd_pool_stats_destroy(rbd_pool_stats_t stats)
{
  auto pool_stat_options =
...
extern "C" int
rbd_pool_stats_get(rados_ioctx_t io, rbd_pool_stats_t pool_stats)
{
  librados::IoCtx io_ctx;
  librados::IoCtx::from_rados_ioctx_t(io, io_ctx);
...
extern "C" int
rbd_copy(rbd_image_t image, rados_ioctx_t dest_p, const char* destname)
{
...
extern "C" int
rbd_copy2(rbd_image_t srcp, rbd_image_t destp)
{
  librbd::ImageCtx* src = (librbd::ImageCtx*)srcp;
  librbd::ImageCtx* dest = (librbd::ImageCtx*)destp;
  tracepoint(
      librbd, copy2_enter, src, src->name.c_str(), src->snap_name.c_str(),
      src->read_only, dest, dest->name.c_str(), dest->snap_name.c_str(),
      dest->read_only);
  librbd::NoOpProgressContext prog_ctx;
    ...
    rbd_image_options_t c_opts)
{
  ...
  tracepoint(
      librbd, copy3_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(),
      ictx->read_only, dest_io_ctx.get_pool_name().c_str(),
      dest_io_ctx.get_id(), destname, c_opts);
  librbd::ImageOptions c_opts_(c_opts);
  librbd::NoOpProgressContext prog_ctx;
    ...
    size_t sparse_size)
{
  ...
  tracepoint(
      librbd, copy4_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(),
      ictx->read_only, dest_io_ctx.get_pool_name().c_str(),
      dest_io_ctx.get_id(), destname, c_opts, sparse_size);
  librbd::ImageOptions c_opts_(c_opts);
  librbd::NoOpProgressContext prog_ctx;
  int r =
    ...
    void* data)
{
  ...
  tracepoint(
      librbd, copy3_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(),
      ictx->read_only, dest_io_ctx.get_pool_name().c_str(),
      dest_io_ctx.get_id(), destname, dest_opts);
    ...
    size_t sparse_size)
{
  ...
  tracepoint(
      librbd, copy4_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(),
      ictx->read_only, dest_io_ctx.get_pool_name().c_str(),
      dest_io_ctx.get_id(), destname, dest_opts, sparse_size);
  librbd::ImageOptions dest_opts_(dest_opts);
  librbd::CProgressContext prog_ctx(fn, data);
  int ret = librbd::copy(
      ictx, dest_io_ctx, destname, dest_opts_, prog_ctx, sparse_size);
    ...
    void* data)
{
  ...
  librbd::CProgressContext prog_ctx(fn, data);
  int ret = librbd::api::Image<>::deep_copy(
      ictx, dest_io_ctx, destname, opts, prog_ctx);
...
extern "C" int
rbd_sparsify(rbd_image_t image, size_t sparse_size)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  tracepoint(
      librbd, sparsify_enter, ictx, ictx->name.c_str(), sparse_size,
...
extern "C" int
rbd_rename(rados_ioctx_t src_p, const char* srcname, const char* destname)
{
...
extern "C" int
rbd_migration_execute(rados_ioctx_t p, const char* image_name)
{
  librados::IoCtx io_ctx;
  librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
...
extern "C" int
rbd_migration_execute_with_progress(
    rados_ioctx_t p,
    const char* name,
    librbd_progress_fn_t fn,
    void* data)
...
extern "C" int
rbd_migration_abort(rados_ioctx_t p, const char* image_name)
{
  librados::IoCtx io_ctx;
  librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
...
extern "C" int
rbd_migration_abort_with_progress(
    rados_ioctx_t p,
    const char* name,
    librbd_progress_fn_t fn,
    void* data)
...
extern "C" int
rbd_migration_commit(rados_ioctx_t p, const char* image_name)
{
  librados::IoCtx io_ctx;
  librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
...
extern "C" int
rbd_migration_commit_with_progress(
    rados_ioctx_t p,
    const char* name,
    librbd_progress_fn_t fn,
    void* data)
...
extern "C" void
rbd_migration_status_cleanup(rbd_image_migration_status_t* s)
{
...
extern "C" int
rbd_pool_metadata_set(rados_ioctx_t p, const char* key, const char* value)
{
  librados::IoCtx io_ctx;
  librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
...
extern "C" int
rbd_pool_metadata_remove(rados_ioctx_t p, const char* key)
{
  librados::IoCtx io_ctx;
  librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
...
extern "C" void
rbd_config_pool_list_cleanup(rbd_config_option_t* options, int max_options)
{
    ...
    rbd_completion_t c)
{
  ...
  ictx->state->open(
      0, new C_OpenComplete(ictx, get_aio_completion(comp), image));
  tracepoint(librbd, aio_open_image_exit, 0);
  return 0;
}
...
extern "C" int
rbd_features_to_string(uint64_t features, char* str_features, size_t* size)
{
  std::stringstream err;
...
extern "C" int
rbd_features_from_string(const char* str_features, uint64_t* features)
{
  std::stringstream err;
  *features = librbd::rbd_features_from_string(str_features, &err);
  if (!err.str().empty()) {
...
rbd_aio_close(rbd_image_t image, rbd_completion_t c)
{
  ...
  tracepoint(
      ...
      comp->pc);
  ictx->state->close(new C_AioCompletion(
      ictx, librbd::io::AIO_TYPE_CLOSE, get_aio_completion(comp)));
  tracepoint(librbd, aio_close_image_exit, 0);
  return 0;
}

extern "C" int
rbd_resize(rbd_image_t image, uint64_t size)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  tracepoint(
      librbd, resize_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(),
...
extern "C" int
rbd_stat(rbd_image_t image, rbd_image_info_t* info, size_t infosize)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  tracepoint(
      librbd, stat_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(),
...
extern "C" int
rbd_get_old_format(rbd_image_t image, uint8_t* old)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  tracepoint(
      librbd, get_old_format_enter, ictx, ictx->name.c_str(),
...
extern "C" int
rbd_get_size(rbd_image_t image, uint64_t* size)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  tracepoint(
      librbd, get_size_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(),
...
extern "C" int
rbd_get_features(rbd_image_t image, uint64_t* features)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  tracepoint(
      librbd, get_features_enter, ictx, ictx->name.c_str(),
...
extern "C" int
rbd_update_features(rbd_image_t image, uint64_t features, uint8_t enabled)
{
...
extern "C" int
rbd_get_op_features(rbd_image_t image, uint64_t* op_features)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  return librbd::api::Image<>::get_op_features(ictx, op_features);
}
...
extern "C" int
rbd_get_stripe_unit(rbd_image_t image, uint64_t* stripe_unit)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  tracepoint(
      librbd, get_stripe_unit_enter, ictx, ictx->name.c_str(),
...
extern "C" int
rbd_get_stripe_count(rbd_image_t image, uint64_t* stripe_count)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  tracepoint(
      librbd, get_stripe_count_enter, ictx, ictx->name.c_str(),
...
extern "C" int
rbd_get_create_timestamp(rbd_image_t image, struct timespec* timestamp)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  tracepoint(
      librbd, get_create_timestamp_enter, ictx, ictx->name.c_str(),
...
extern "C" int
rbd_get_access_timestamp(rbd_image_t image, struct timespec* timestamp)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  tracepoint(
      librbd, get_access_timestamp_enter, ictx, ictx->name.c_str(),
...
extern "C" int
rbd_get_modify_timestamp(rbd_image_t image, struct timespec* timestamp)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  tracepoint(
      librbd, get_modify_timestamp_enter, ictx, ictx->name.c_str(),
...
extern "C" int
rbd_get_overlap(rbd_image_t image, uint64_t* overlap)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  tracepoint(
      librbd, get_overlap_enter, ictx, ictx->name.c_str(),
...
extern "C" int
rbd_get_name(rbd_image_t image, char* name, size_t* name_len)
{
  librbd::ImageCtx* ictx = reinterpret_cast<librbd::ImageCtx*>(image);
...
extern "C" int
rbd_get_id(rbd_image_t image, char* id, size_t id_len)
{
  librbd::ImageCtx* ictx = reinterpret_cast<librbd::ImageCtx*>(image);
  if (ictx->old_format) {
    return -EINVAL;
...
extern "C" int
rbd_get_block_name_prefix(rbd_image_t image, char* prefix, size_t prefix_len)
{
  librbd::ImageCtx* ictx = reinterpret_cast<librbd::ImageCtx*>(image);
...
extern "C" int64_t
rbd_get_data_pool_id(rbd_image_t image)
{
  librbd::ImageCtx* ictx = reinterpret_cast<librbd::ImageCtx*>(image);
  return librbd::api::Image<>::get_data_pool_id(ictx);
}
    ...
    rbd_snap_spec_t* parent_snap)
{
  ...
  int r = librbd::api::Image<>::get_parent(
      ictx, &cpp_parent_image, &cpp_parent_snap);
  if (r < 0) {
  ...
  tracepoint(
      librbd, get_parent_info_exit, r, parent_image->pool_name,
      parent_image->image_name, parent_image->image_id, parent_snap->name);
  return r;
}

extern "C" int
rbd_get_migration_source_spec(
    rbd_image_t image,
    char* source_spec,
    size_t* max_len)
{
...
extern "C" int
rbd_get_flags(rbd_image_t image, uint64_t* flags)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  tracepoint(librbd, get_flags_enter, ictx);
  int r = librbd::get_flags(ictx, flags);
...
extern "C" int
rbd_is_exclusive_lock_owner(rbd_image_t image, int* is_owner)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  tracepoint(librbd, is_exclusive_lock_owner_enter, ictx);
  bool owner;
...
extern "C" int
rbd_lock_acquire(rbd_image_t image, rbd_lock_mode_t lock_mode)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  tracepoint(librbd, lock_acquire_enter, ictx, lock_mode);
  int r = librbd::lock_acquire(ictx, lock_mode);
...
extern "C" int
rbd_lock_release(rbd_image_t image)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  tracepoint(librbd, lock_release_enter, ictx);
  int r = librbd::lock_release(ictx);
...
extern "C" void
rbd_lock_get_owners_cleanup(char** lock_owners, size_t lock_owner_count)
{
...
extern "C" int
rbd_rebuild_object_map(rbd_image_t image, librbd_progress_fn_t cb, void* cbdata)
{
  librbd::ImageCtx* ictx = reinterpret_cast<librbd::ImageCtx*>(image);
  librbd::CProgressContext prog_ctx(cb, cbdata);
  return ictx->operations->rebuild_object_map(prog_ctx);
...
/* snapshots */
extern "C" int
rbd_snap_create(rbd_image_t image, const char* snap_name)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  tracepoint(
      librbd, snap_create_enter, ictx, ictx->name.c_str(),
...
extern "C" int
rbd_snap_rename(rbd_image_t image, const char* srcname, const char* dstname)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  tracepoint(
      librbd, snap_rename_enter, ictx, ictx->name.c_str(),
...
extern "C" int
rbd_snap_remove(rbd_image_t image, const char* snap_name)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  tracepoint(
      librbd, snap_remove_enter, ictx, ictx->name.c_str(),
...
extern "C" int
rbd_snap_remove_by_id(rbd_image_t image, uint64_t snap_id)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  return librbd::api::Snapshot<>::remove(ictx, snap_id);
}
...
extern "C" int
rbd_snap_rollback(rbd_image_t image, const char* snap_name)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  tracepoint(
      librbd, snap_rollback_enter, ictx, ictx->name.c_str(),
...
extern "C" int
rbd_snap_rollback_with_progress(
    rbd_image_t image,
    const char* snap_name,
    librbd_progress_fn_t cb,
    void* cbdata)
...
rbd_snap_list(rbd_image_t image, rbd_snap_info_t* snaps, int* max_snaps)
{
  ...
  for (i = 0; i < (int)cpp_snaps.size(); i++) {
    ...
    tracepoint(
        librbd, snap_list_entry, snaps[i].id, snaps[i].size, snaps[i].name);
  }
  snaps[i].id = 0;
  snaps[i].size = 0;
  snaps[i].name = NULL;
...
extern "C" int
rbd_snap_protect(rbd_image_t image, const char* snap_name)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  tracepoint(
      librbd, snap_protect_enter, ictx, ictx->name.c_str(),
...
extern "C" int
rbd_snap_unprotect(rbd_image_t image, const char* snap_name)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  tracepoint(
      librbd, snap_unprotect_enter, ictx, ictx->name.c_str(),
...
extern "C" int
rbd_snap_is_protected(rbd_image_t image, const char* snap_name, int* is_protected)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  tracepoint(
      librbd, snap_is_protected_enter, ictx, ictx->name.c_str(),
...
extern "C" int
rbd_snap_exists(rbd_image_t image, const char* snapname, bool* exists)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  tracepoint(
      librbd, snap_exists_enter, ictx, ictx->name.c_str(),
...
extern "C" int
rbd_snap_set(rbd_image_t image, const char* snap_name)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  tracepoint(
      librbd, snap_set_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(),
...
extern "C" int
rbd_snap_set_by_id(rbd_image_t image, uint64_t snap_id)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  return librbd::api::Image<>::snap_set(ictx, snap_id);
}
...
extern "C" int
rbd_snap_get_id(rbd_image_t image, const char* snapname, uint64_t* snap_id)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
    ...
    int* max_children)
{
  ...
  for (i = 0; i < (int)cpp_children.size(); i++) {
    ...
    tracepoint(
        librbd, list_children_entry, children[i].pool_name,
...
extern "C" void
rbd_list_children_cleanup(rbd_child_info_t* children, size_t num_children)
{
    ...
    size_t* max_images)
{
  ...
  *max_images = cpp_children.size();
  for (size_t idx = 0; idx < cpp_children.size(); ++idx) {
    ...
    tracepoint(
        librbd, list_children_entry, images[idx].pool_name,
...
extern "C" int
rbd_lock_exclusive(rbd_image_t image, const char* cookie)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  tracepoint(
      librbd, lock_exclusive_enter, ictx, ictx->name.c_str(),
...
extern "C" int
rbd_lock_shared(rbd_image_t image, const char* cookie, const char* tag)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  tracepoint(
      librbd, lock_shared_enter, ictx, ictx->name.c_str(),
...
extern "C" int
rbd_unlock(rbd_image_t image, const char* cookie)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  tracepoint(
      librbd, unlock_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(),
      ictx->read_only, cookie);
...
extern "C" int
rbd_break_lock(rbd_image_t image, const char* client, const char* cookie)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  tracepoint(
      librbd, break_lock_enter, ictx, ictx->name.c_str(),
...
/* I/O */
extern "C" ssize_t
rbd_read(rbd_image_t image, uint64_t ofs, size_t len, char* buf)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  tracepoint(
      librbd, read_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(),
...
extern "C" ssize_t
rbd_read2(rbd_image_t image, uint64_t ofs, size_t len, char* buf, int op_flags)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  tracepoint(
      librbd, read2_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(),
...
extern "C" ssize_t
rbd_write(rbd_image_t image, uint64_t ofs, size_t len, const char* buf)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  tracepoint(
      librbd, write_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(),
      ictx->read_only, ofs, len, buf);
    ...
    int op_flags)
{
  ...
  tracepoint(
      librbd, write2_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(),
      ictx->read_only, ofs, len, buf, op_flags);
...
extern "C" int
rbd_discard(rbd_image_t image, uint64_t ofs, uint64_t len)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  tracepoint(
      librbd, discard_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(),
    ...
    int op_flags)
{
  ...
  raw->set_crc(std::make_pair(0, len), std::make_pair(-1, precomputed_crc32c));
  bufferlist bl;
    ...
    rbd_completion_t c)
{
  ...
  tracepoint(
      librbd, aio_write_enter, ictx, ictx->name.c_str(),
      ictx->snap_name.c_str(), ictx->read_only, off, len, NULL, comp->pc);
    ...
    rbd_completion_t c)
{
  ...
  tracepoint(
      librbd, aio_read_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(),
      ictx->read_only, off, len, buf, comp->pc);
    ...
    rbd_completion_t c)
{
  ...
  tracepoint(
      librbd, aio_read_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(),
      ictx->read_only, off, len, NULL, comp->pc);
  if (r == 0) {
...
extern "C" int
rbd_aio_discard(rbd_image_t image, uint64_t off, uint64_t len, rbd_completion_t c)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  librbd::RBD::AioCompletion* comp = (librbd::RBD::AioCompletion*)c;
  tracepoint(
    ...
    int op_flags)
{
  ...
  librbd::api::Io<>::aio_write_zeroes(
      *ictx, get_aio_completion(comp), off, len, zero_flags, op_flags, true);
  return 0;
}
    ...
    int op_flags)
{
  ...
  auto bl = iovec_to_bufferlist(ictx, iov, iovcnt, aio_completion);
  librbd::api::Io<>::aio_compare_and_write(
      *ictx, aio_completion, off, cmp_len, std::move(cmp_bl), std::move(bl),
      mismatch_off, op_flags, false);
...
extern "C" int
rbd_invalidate_cache(rbd_image_t image)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  tracepoint(
      librbd, invalidate_cache_enter, ictx, ictx->name.c_str(),
...
extern "C" int
rbd_metadata_get(rbd_image_t image, const char* key, char* value, size_t* vallen)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  string val_s;
...
extern "C" int
rbd_mirror_image_enable(rbd_image_t image)
{
...
extern "C" int
rbd_mirror_image_disable(rbd_image_t image, bool force)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  return librbd::api::Mirror<>::image_disable(ictx, force);
}
...
extern "C" int
rbd_mirror_image_promote(rbd_image_t image, bool force)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  return librbd::api::Mirror<>::image_promote(ictx, force);
}
...
extern "C" int
rbd_mirror_image_demote(rbd_image_t image)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  return librbd::api::Mirror<>::image_demote(ictx);
}
...
extern "C" int
rbd_mirror_image_resync(rbd_image_t image)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  return librbd::api::Mirror<>::image_resync(ictx);
}
...
extern "C" int
rbd_mirror_image_create_snapshot(rbd_image_t image, uint64_t* snap_id)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  auto flags = librbd::util::get_default_snap_create_flags(ictx);
  return librbd::api::Mirror<>::image_snapshot_create(ictx, flags, snap_id);
...
extern "C" int
rbd_mirror_image_create_snapshot2(
    rbd_image_t image,
    uint32_t flags,
    uint64_t* snap_id)
{
...
extern "C" int
rbd_mirror_image_get_mode(rbd_image_t image, rbd_mirror_image_mode_t* mode)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
...
extern "C" int
rbd_mirror_image_get_global_status(
    rbd_image_t image,
...
extern "C" int
rbd_mirror_image_get_instance_id(
    rbd_image_t image,
    char* instance_id,
...
extern "C" int
rbd_aio_mirror_image_promote(rbd_image_t image, bool force, rbd_completion_t c)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  librbd::RBD::AioCompletion* comp = (librbd::RBD::AioCompletion*)c;
  librbd::api::Mirror<>::image_promote(
      ictx, force,
      new C_AioCompletion(
          ictx, librbd::io::AIO_TYPE_GENERIC, get_aio_completion(comp)));
  return 0;
}

extern "C" int
rbd_aio_mirror_image_demote(rbd_image_t image, rbd_completion_t c)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  librbd::RBD::AioCompletion* comp = (librbd::RBD::AioCompletion*)c;
  librbd::api::Mirror<>::image_demote(
...
extern "C" int
rbd_aio_mirror_image_get_global_status(
    rbd_image_t image,
    rbd_mirror_image_global_status_t* status,
    size_t status_size,
    rbd_completion_t c)
...
extern "C" int
rbd_aio_mirror_image_get_status(
    rbd_image_t image,
    rbd_mirror_image_status_t* status,
    size_t status_size,
    rbd_completion_t c)
...
extern "C" int
rbd_aio_mirror_image_create_snapshot(
    rbd_image_t image,
    uint32_t flags,
    uint64_t* snap_id,
    rbd_completion_t c)
...
extern "C" int
rbd_group_create(rados_ioctx_t p, const char* name)
{
  librados::IoCtx io_ctx;
  librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
...
extern "C" int
rbd_group_remove(rados_ioctx_t p, const char* name)
{
  librados::IoCtx io_ctx;
  librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
...
extern "C" int
rbd_group_list(rados_ioctx_t p, char* names, size_t* size)
{
  librados::IoCtx io_ctx;
  librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
...
extern "C" int
rbd_group_rename(rados_ioctx_t p, const char* src_name, const char* dest_name)
{
  librados::IoCtx io_ctx;
  librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
...
extern "C" int
rbd_group_info_cleanup(rbd_group_info_t* group_info, size_t group_info_size)
{
  if (group_info_size != sizeof(rbd_group_info_t)) {
    return -ERANGE;
  }
    ...
    rbd_group_snap_info2_t* group_snap)
{
  ...
  int r = librbd::api::Group<>::snap_get_info(
      group_ioctx, group_name, snap_name, &cpp_group_snap);
  if (r < 0) {
    return r;
  }
...
extern "C" void
rbd_group_snap_get_info_cleanup(rbd_group_snap_info2_t* group_snap)
{
  free(group_snap->id);
    ...
    size_t* snaps_size)
{
  ...
  int r = librbd::api::Group<>::snap_list(
      group_ioctx, group_name, true, false, &cpp_snaps);
...
extern "C" void
rbd_group_snap_list2_cleanup(rbd_group_snap_info2_t* snaps, size_t len)
{
  for (size_t i = 0; i < len; ++i) {
...
extern "C" int
rbd_group_snap_rollback_with_progress(
    rados_ioctx_t group_p,
    const char* group_name,
    const char* snap_name,
    librbd_progress_fn_t cb,
    ...
    size_t snap_group_namespace_size)
{
  ...
  int r = librbd::api::Snapshot<>::get_group_namespace(
      ictx, snap_id, &group_namespace);
  if (r >= 0) {
...
extern "C" int
rbd_snap_group_namespace_cleanup(
    rbd_snap_group_namespace_t* group_snap,
    size_t snap_group_namespace_size)
{
  if (snap_group_namespace_size != sizeof(rbd_snap_group_namespace_t)) {
    ...
    size_t max_length)
{
  ...
  int r = librbd::api::Snapshot<>::get_trash_namespace(
      ictx, snap_id, &trash_namespace);
  if (r < 0) {
    return r;
  }
...
extern "C" int
rbd_snap_trash_namespace_cleanup(
    rbd_snap_trash_namespace_t* trash_snap,
    size_t trash_snap_size)
{
  if (trash_snap_size != sizeof(rbd_snap_trash_namespace_t)) {
...
extern "C" void
rbd_watchers_list_cleanup(rbd_image_watcher_t* watchers, size_t num_watchers)
{
...
extern "C" void
rbd_config_image_list_cleanup(rbd_config_option_t* options, int max_options)
{
  for (int i = 0; i < max_options; ++i) {
    config_option_cleanup(options[i]);
  }
...
extern "C" int
rbd_quiesce_unwatch(rbd_image_t image, uint64_t handle)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
...
extern "C" void
rbd_quiesce_complete(rbd_image_t image, uint64_t handle, int r)
{
  librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
  ictx->state->quiesce_complete(handle, r);
}
