...
#undef dout_prefix
#define dout_prefix                                                   \
  *_dout << "librbd::managed_lock::GetLockerRequest: " << this << " " \
         << __func__ << ": "
...
template <typename I>
GetLockerRequest<I>::GetLockerRequest(
...
template <typename I>
void
GetLockerRequest<I>::send_get_lockers()
{
...
template <typename I>
void
GetLockerRequest<I>::handle_get_lockers(int r)
{
  ...
  if (r == 0) {
    ...
    r = rados::cls::lock::get_lock_info_finish(
        &it, &lockers, &lock_type, &lock_tag);
  }
  ...
  if (lock_tag != util::get_watcher_lock_tag()) {
    ldout(m_cct, 5) << "locked by external mechanism: tag=" << lock_tag
                    << dendl;
    finish(-EBUSY);
    return;
  }
  ...
  std::map<rados::cls::lock::locker_id_t, rados::cls::lock::locker_info_t>::iterator
      iter = lockers.begin();
  if (!util::decode_lock_cookie(iter->first.cookie, &m_locker->handle)) {
    ldout(m_cct, 5) << "locked by external mechanism: " << "cookie="
  ...
  ldout(m_cct, 10) << "retrieved exclusive locker: " << m_locker->entity << "@"
                   << m_locker->address << dendl;
  finish(0);
}

template <typename I>
void
GetLockerRequest<I>::finish(int r)
{
  ldout(m_cct, 10) << "r=" << r << dendl;
