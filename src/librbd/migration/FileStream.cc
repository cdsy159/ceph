...
#undef dout_prefix
#define dout_prefix                                                      \
  *_dout << "librbd::migration::FileStream::ReadRequest " << this << " " \
         << __func__ << ": "
...
struct FileStream<I>::ReadRequest {
  ...
  read()
  {
    ...
    auto ptr = buffer::ptr_node::create(
        buffer::create_small_page_aligned(byte_extent.second));
...
#undef dout_prefix
#define dout_prefix \
  *_dout << "librbd::migration::FileStream: " << this << " " << __func__ << ": "
...
template <typename I>
void
FileStream<I>::open(Context* on_finish)
{
  ...
  m_stream_descriptor =
      std::make_optional<boost::asio::posix::stream_descriptor>(
          m_strand, m_file_no);
...
template <typename I>
void
FileStream<I>::close(Context* on_finish)
{
...
template <typename I>
void
FileStream<I>::get_size(uint64_t* size, Context* on_finish)
{
  ldout(m_cct, 10) << dendl;
...
template <typename I>
void
FileStream<I>::list_sparse_extents(
    io::Extents&& byte_extents,
