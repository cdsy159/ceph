...
#undef dout_prefix
#define dout_prefix                                                      \
  *_dout << "librbd::migration::OpenSourceImageRequest: " << this << " " \
         << __func__ << ": "
...
OpenSourceImageRequest<I>::send()
{
  ...
  int r =
      SourceSpecBuilder<I>::parse_source_spec(source_spec, &source_spec_object);
  if (r < 0) {
...
template <typename I>
void
OpenSourceImageRequest<I>::open_native(
    const json_spirit::mObject& source_spec_object,
    bool import_only)
{
  ldout(m_cct, 10) << dendl;

  int r = NativeFormat<I>::create_image_ctx(
      m_dst_io_ctx, source_spec_object, import_only, m_src_snap_id,
      m_src_image_ctx, m_src_rados);
  if (r < 0) {
    lderr(m_cct) << "failed to create native image context: " << cpp_strerror(r)
                 << dendl;
    finish(r);
    return;
  }
  ...
  if (m_dst_image_ctx != nullptr) {
    ...
    if (m_dst_image_ctx->config.template get_val<bool>(
            "rbd_balance_parent_reads")) {
      src_image_ctx->set_read_flag(librados::OPERATION_BALANCE_READS);
    } else if (m_dst_image_ctx->config.template get_val<bool>(
  ...
  auto ctx = util::create_context_callback<
      OpenSourceImageRequest<I>, &OpenSourceImageRequest<I>::handle_open_native>(
      this);
...
template <typename I>
void
OpenSourceImageRequest<I>::handle_open_native(int r)
{
  ldout(m_cct, 10) << "r=" << r << dendl;

  if (r < 0) {
    lderr(m_cct) << "failed to open native image: " << cpp_strerror(r) << dendl;
...
template <typename I>
void
OpenSourceImageRequest<I>::open_format(
    const json_spirit::mObject& source_spec_object)
{
  ldout(m_cct, 10) << dendl;
  ...
  *m_src_rados = nullptr;
  ...
  src_image_ctx->layout = file_layout_t(
      src_image_ctx->stripe_unit, src_image_ctx->stripe_count,
  ...
  auto ctx = util::create_context_callback<
      OpenSourceImageRequest<I>, &OpenSourceImageRequest<I>::handle_open_format>(
      this);
  m_format->open(ctx);
}
...
template <typename I>
void
OpenSourceImageRequest<I>::handle_open_format(int r)
{
  ldout(m_cct, 10) << "r=" << r << dendl;
...
template <typename I>
void
OpenSourceImageRequest<I>::get_image_size()
{
  ldout(m_cct, 10) << dendl;
...
template <typename I>
void
OpenSourceImageRequest<I>::handle_get_image_size(int r)
{
  ldout(m_cct, 10) << "r=" << r << ", " << "image_size=" << m_image_size
                   << dendl;
...
template <typename I>
void
OpenSourceImageRequest<I>::get_snapshots()
{
  ldout(m_cct, 10) << dendl;
...
template <typename I>
void
OpenSourceImageRequest<I>::handle_get_snapshots(int r)
{
  ldout(m_cct, 10) << "r=" << r << dendl;
  ...
  for (auto it = m_snap_infos.rbegin(); it != m_snap_infos.rend(); ++it) {
    ...
    ldout(m_cct, 10) << "adding snap: ns=" << snap_info.snap_namespace << ", "
                     << "name=" << snap_info.name << ", " << "id=" << snap_id
                     << dendl;
    src_image_ctx->add_snap(
        snap_info.snap_namespace, snap_info.name, snap_id, snap_info.size,
        snap_info.parent, snap_info.protection_status, snap_info.flags,
        snap_info.timestamp);
  }
  ...
  if (m_src_snap_id != CEPH_NOSNAP) {
    ...
    if (r < 0) {
      ...
      lderr(m_cct) << "error setting source image snap id: " << cpp_strerror(r)
                   << dendl;
      finish(r);
      return;
    }
...
template <typename I>
void
OpenSourceImageRequest<I>::close_image(int r)
{
  ldout(m_cct, 10) << "r=" << r << dendl;
...
template <typename I>
void
OpenSourceImageRequest<I>::register_image_dispatch()
{
  ldout(m_cct, 10) << dendl;

  // intercept any IO requests to the source image
  auto io_image_dispatch =
      ImageDispatch<I>::create(*m_src_image_ctx, std::move(m_format));
  (*m_src_image_ctx)->io_image_dispatcher->register_dispatch(io_image_dispatch);
}

template <typename I>
void
OpenSourceImageRequest<I>::finish(int r)
{
  ldout(m_cct, 10) << "r=" << r << dendl;
