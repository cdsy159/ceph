...
#undef dout_prefix
#define dout_prefix \
  *_dout << "librbd::migration::QCOWFormat: " << __func__ << ": "
    ...
    ClusterExtents* cluster_extents)
{
  ...
  for (auto [image_offset, image_length] : image_extents) {
    while (image_length > 0) {
      ...
      cluster_extents->emplace_back(
          0, cluster_length, intra_cluster_offset, image_offset, buffer_offset);
...
#undef dout_prefix
#define dout_prefix                                                        \
  *_dout << "librbd::migration::QCOWFormat::ClusterCache: " << this << " " \
         << __func__ << ": "
...
public:
  ClusterCache(QCOWFormat* qcow_format) :
    ...
    m_strand(boost::asio::make_strand(*qcow_format->m_image_ctx->asio_engine))
  {}
      ...
      Context* on_finish)
  {
    ...
    boost::asio::dispatch(
        m_strand, [this, cluster_offset, cluster_length, intra_cluster_offset,
                   bl, on_finish]() {
          execute_get_cluster(
              cluster_offset, cluster_length, intra_cluster_offset, bl,
              on_finish);
        });
  }
...
private:
      ...
      Context* on_finish)
  {
    ...
    if (cluster && cluster->cluster_offset == cluster_offset) {
      ...
      bl->substr_of(
          cluster->cluster_data_bl, intra_cluster_offset, cluster_length);
      boost::asio::post(*qcow_format->m_image_ctx->asio_engine, [on_finish]() {
  ...
  void
  read_cluster(std::shared_ptr<Cluster> cluster)
  {
    auto cct = qcow_format->m_image_ctx->cct;
    ...
    qcow_format->m_stream->read(
        {{stream_offset, stream_length}}, &cluster->cluster_data_bl, ctx);
  }

  void
  handle_read_cluster(int r, std::shared_ptr<Cluster> cluster)
  {
    auto cct = qcow_format->m_image_ctx->cct;
    ldout(cct, 20) << "r=" << r << ", "
          ...
          for (auto completion : completions) {
            if (r >= 0) {
              std::get<2>(completion)
                  ->substr_of(
                      cluster->cluster_data_bl, std::get<0>(completion),
...
#undef dout_prefix
#define dout_prefix                                                        \
  *_dout << "librbd::migration::QCOWFormat::L2TableCache: " << this << " " \
         << __func__ << ": "
...
public:
      ...
      Context* on_finish)
  {
    ...
    boost::asio::dispatch(m_strand, [this, request = std::move(request)]() {
      ...
      Context* on_finish)
  {
    ...
    // cache state machine runs in a single strand thread
    Request request{
        l1_table, l2_table_offset, l2_table_index, cluster_offset, on_finish};
    boost::asio::dispatch(m_strand, [this, request = std::move(request)]() {
      requests.push_back(std::move(request));
    });
    dispatch_request();
...
private:
  ...
  execute_request()
  {
    ...
    if (r < 0) {
      lderr(cct) << "failed to load L2 table: l2_table_offset="
                 << request.l2_table_offset << ": " << cpp_strerror(r) << dendl;
  ...
  int
  l2_table_lookup(uint64_t l2_offset, std::shared_ptr<LookupTable>* l2_table)
  {
    auto cct = qcow_format->m_image_ctx->cct;
    ...
    for (auto idx = 0U; idx < l2_cache_entries.size(); ++idx) {
      ...
      if (l2_cache.l2_offset == l2_offset) {
        ...
        if (l2_cache.ret_val < 0) {
          ldout(cct, 20) << "l2_offset=" << l2_offset << ", " << "index=" << idx
                         << " (error): " << cpp_strerror(l2_cache.ret_val)
                         << dendl;
    ...
    ldout(cct, 20) << "l2_offset=" << l2_offset << ", " << "index=" << min_idx
                   << " (loading)" << dendl;
  ...
  void
  handle_l2_table_lookup(int r, uint32_t index, uint64_t l2_offset)
  {
    auto cct = qcow_format->m_image_ctx->cct;
    ldout(cct, 20) << "r=" << r << ", " << "l2_offset=" << l2_offset << ", "
                   << "index=" << index << dendl;
    ...
    if (r < 0) {
      lderr(cct) << "failed to load L2 table: " << "l2_offset="
                 << l2_cache.l2_offset << ": " << cpp_strerror(r) << dendl;
      l2_cache.ret_val = r;
    } else {
...
#undef dout_prefix
#define dout_prefix                                                       \
  *_dout << "librbd::migration::QCOWFormat::ReadRequest: " << this << " " \
         << __func__ << ": "
...
private:
  ...
  get_cluster_offsets()
  {
    auto cct = qcow_format->m_image_ctx->cct;
    populate_cluster_extents(
        cct, qcow_format->m_cluster_size, image_extents, &cluster_extents);
  ...
  void
  handle_get_cluster_offset(
      int r,
      const ClusterExtent& cluster_extent,
      Context* on_finish)
  {
    auto cct = qcow_format->m_image_ctx->cct;
    ldout(cct, 20) << "r=" << r << ", "
  ...
  void
  handle_get_cluster_offsets(int r)
  {
    auto cct = qcow_format->m_image_ctx->cct;
  ...
  read_clusters()
  {
    ...
    for (auto& cluster_extent : cluster_extents) {
      ...
      auto log_ctx =
          new LambdaContext([this, cct = qcow_format->m_image_ctx->cct,
                             image_offset = cluster_extent.image_offset,
      ...
      if (cluster_extent.cluster_offset == 0) {
        ...
        boost::asio::post(*qcow_format->m_image_ctx->asio_engine, [log_ctx] {
      ...
      Context* on_finish) const
  {
    ...
    ldout(cct, 20) << "r=" << r << ", " << "image_offset=" << image_offset
...
#undef dout_prefix
#define dout_prefix                                                           \
  *_dout << "librbd::migration::QCOWFormat::" << "ListSnapsRequest: " << this \
         << " " << __func__ << ": "
...
private:
  ...
  get_l2_table()
  {
    ...
    qcow_format->m_l2_table_cache->get_l2_table(
        l1_table, l2_table_offset, &l2_table, ctx);
  }

  void
  handle_get_l2_table(int r, uint64_t snap_id)
  {
    ...
    for (auto& cluster_extent : cluster_extents) {
      uint32_t l2_table_index = (cluster_extent.image_offset >>
                                 qcow_format->m_cluster_bits) &
                                (qcow_format->m_l2_size - 1);
    ...
    boost::asio::post(*qcow_format->m_image_ctx->asio_engine, [this]() {
      get_l2_table();
    });
  }
...
#undef dout_prefix
#define dout_prefix \
  *_dout << "librbd::migration::QCOWFormat: " << this << " " << __func__ << ": "
...
template <typename I>
void
QCOWFormat<I>::open(Context* on_finish)
{
...
template <typename I>
void
QCOWFormat<I>::handle_open(int r, Context* on_finish)
{
  auto cct = m_image_ctx->cct;
...
template <typename I>
void
QCOWFormat<I>::probe(Context* on_finish)
{
  auto cct = m_image_ctx->cct;
  ldout(cct, 10) << dendl;
...
template <typename I>
void
QCOWFormat<I>::handle_probe(int r, Context* on_finish)
{
  auto cct = m_image_ctx->cct;
  ldout(cct, 10) << "r=" << r << dendl;
...
template <typename I>
void
QCOWFormat<I>::read_v1_header(Context* on_finish)
{
  auto cct = m_image_ctx->cct;
  ldout(cct, 10) << dendl;
...
template <typename I>
void
QCOWFormat<I>::handle_read_v1_header(int r, Context* on_finish)
{
  auto cct = m_image_ctx->cct;
  ldout(cct, 10) << "r=" << r << dendl;
...
template <typename I>
void
QCOWFormat<I>::read_v2_header(Context* on_finish)
{
  auto cct = m_image_ctx->cct;
  ldout(cct, 10) << dendl;
...
template <typename I>
void
QCOWFormat<I>::handle_read_v2_header(int r, Context* on_finish)
{
  auto cct = m_image_ctx->cct;
  ldout(cct, 10) << "r=" << r << dendl;
...
template <typename I>
void
QCOWFormat<I>::read_snapshot(Context* on_finish)
{
...
template <typename I>
void
QCOWFormat<I>::handle_read_snapshot(int r, Context* on_finish)
{
  auto cct = m_image_ctx->cct;
  ldout(cct, 10) << "r=" << r << ", " << "index=" << m_snapshots.size()
                 << dendl;
...
template <typename I>
void
QCOWFormat<I>::read_snapshot_extra(Context* on_finish)
{
  ceph_assert(!m_snapshots.empty());
  auto& snapshot = m_snapshots.rbegin()->second;

  uint32_t length = snapshot.extra_data_size + snapshot.id.size() +
                    snapshot.name.size();
  if (length == 0) {
    uuid_d uuid_gen;
...
template <typename I>
void
QCOWFormat<I>::handle_read_snapshot_extra(int r, Context* on_finish)
{
  ceph_assert(!m_snapshots.empty());
  auto& snapshot = m_snapshots.rbegin()->second;

  auto cct = m_image_ctx->cct;
  ldout(cct, 10) << "r=" << r << ", " << "snap_id=" << m_snapshots.size()
                 << dendl;
...
template <typename I>
void
QCOWFormat<I>::read_snapshot_l1_table(Context* on_finish)
{
  ceph_assert(!m_snapshots.empty());
  auto& snapshot = m_snapshots.rbegin()->second;
  ...
  m_stream->read(
      {{snapshot.l1_table_offset, snapshot.l1_table.size * sizeof(uint64_t)}},
      &snapshot.l1_table.bl, ctx);
}

template <typename I>
void
QCOWFormat<I>::handle_read_snapshot_l1_table(int r, Context* on_finish)
{
  ceph_assert(!m_snapshots.empty());
  auto& snapshot = m_snapshots.rbegin()->second;
...
template <typename I>
void
QCOWFormat<I>::read_l1_table(Context* on_finish)
{
  auto cct = m_image_ctx->cct;
  ldout(cct, 10) << dendl;
  ...
  m_stream->read(
      {{m_l1_table_offset, m_l1_table.size * sizeof(uint64_t)}}, &m_l1_table.bl,
      ctx);
}

template <typename I>
void
QCOWFormat<I>::handle_read_l1_table(int r, Context* on_finish)
{
  auto cct = m_image_ctx->cct;
  ldout(cct, 10) << "r=" << r << dendl;
...
template <typename I>
void
QCOWFormat<I>::read_backing_file(Context* on_finish)
{
...
template <typename I>
void
QCOWFormat<I>::close(Context* on_finish)
{
  auto cct = m_image_ctx->cct;
  ldout(cct, 10) << dendl;
...
template <typename I>
void
QCOWFormat<I>::get_snapshots(SnapInfos* snap_infos, Context* on_finish)
{
  auto cct = m_image_ctx->cct;
  ldout(cct, 10) << dendl;
  ...
  for (auto& [snap_id, snapshot] : m_snapshots) {
    SnapInfo snap_info(
        snapshot.name, cls::rbd::UserSnapshotNamespace{}, snapshot.size, {}, 0,
    ...
    const ZTracer::Trace& parent_trace)
{
  ...
  auto read_request =
      new ReadRequest(this, aio_comp, l1_table, std::move(image_extents));
  read_request->send();
}

template <typename I>
void
QCOWFormat<I>::list_snaps(
    io::Extents&& image_extents,
    io::SnapIds&& snap_ids,
    ...
    Context* on_finish)
{
  ...
  on_finish = new LambdaContext([this, image_extents,
                                 snap_ids = std::move(snap_ids), snapshot_delta,
                                 on_finish](int r) mutable {
    handle_list_snaps(
        r, std::move(image_extents), std::move(snap_ids), snapshot_delta,
        on_finish);
  });
...
template <typename I>
void
QCOWFormat<I>::handle_list_snaps(
    int r,
    io::Extents&& image_extents,
    io::SnapIds&& snap_ids,
    io::SnapshotDelta* snapshot_delta,
    Context* on_finish)
{
  ...
  for (auto& [snap_id, snapshot] : m_snapshots) {
    ...
    util::zero_shrunk_snapshot(
        cct, image_extents, snap_id, snapshot.size, &previous_size,
        sparse_extents);
  }

  auto sparse_extents = &(*snapshot_delta)[{CEPH_NOSNAP, CEPH_NOSNAP}];
  util::zero_shrunk_snapshot(
      cct, image_extents, CEPH_NOSNAP, m_size, &previous_size, sparse_extents);
