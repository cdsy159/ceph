...
struct S3Stream<I>::HttpProcessor : public HttpProcessorInterface {
  ...
  void
  process_request(EmptyRequest& request) override
  {
...
#undef dout_prefix
#define dout_prefix \
  *_dout << "librbd::migration::S3Stream: " << this << " " << __func__ << ": "
...
template <typename I>
void
S3Stream<I>::open(Context* on_finish)
{
  ...
  ldout(m_cct, 10) << "url=" << m_url << ", " << "access_key=" << m_access_key
                   << dendl;
...
template <typename I>
void
S3Stream<I>::close(Context* on_finish)
{
...
template <typename I>
void
S3Stream<I>::get_size(uint64_t* size, Context* on_finish)
{
  ldout(m_cct, 10) << dendl;
...
template <typename I>
void
S3Stream<I>::process_request(HttpRequest& http_request)
{
  ...
  std::string string_to_sign = fmt::format(
      "{}\n\n\n{}\n{}",
      std::string(boost::beast::http::to_string(http_request.method())), date,
  ...
  char buf[64];
  int r = ceph_armor(
      std::begin(buf), std::begin(buf) + sizeof(buf),
...
template <typename I>
void
S3Stream<I>::list_sparse_extents(
    io::Extents&& byte_extents,
