...
#undef dout_prefix
#define dout_prefix                                                       \
  *_dout << "librbd::mirror::snapshot::CreateNonPrimaryRequest: " << this \
         << " " << __func__ << ": "
...
template <typename I>
void
CreateNonPrimaryRequest<I>::send()
{
  refresh_image();
}
...
template <typename I>
void
CreateNonPrimaryRequest<I>::refresh_image()
{
...
template <typename I>
void
CreateNonPrimaryRequest<I>::handle_refresh_image(int r)
{
  CephContext* cct = m_image_ctx->cct;
...
template <typename I>
void
CreateNonPrimaryRequest<I>::get_mirror_image()
{
  CephContext* cct = m_image_ctx->cct;
  ldout(cct, 15) << dendl;
...
template <typename I>
void
CreateNonPrimaryRequest<I>::handle_get_mirror_image(int r)
{
  CephContext* cct = m_image_ctx->cct;
  ldout(cct, 15) << "r=" << r << dendl;
...
template <typename I>
void
CreateNonPrimaryRequest<I>::get_mirror_peers()
{
  if (!m_demoted) {
    create_snapshot();
    return;
...
template <typename I>
void
CreateNonPrimaryRequest<I>::handle_get_mirror_peers(int r)
{
  CephContext* cct = m_image_ctx->cct;
  ldout(cct, 15) << "r=" << r << dendl;
...
template <typename I>
void
CreateNonPrimaryRequest<I>::create_snapshot()
{
  CephContext* cct = m_image_ctx->cct;
...
template <typename I>
void
CreateNonPrimaryRequest<I>::handle_create_snapshot(int r)
{
  CephContext* cct = m_image_ctx->cct;
  ldout(cct, 15) << "r=" << r << dendl;
...
template <typename I>
void
CreateNonPrimaryRequest<I>::write_image_state()
{
  uint64_t snap_id;
  {
  ...
  auto req = WriteImageStateRequest<I>::create(
      m_image_ctx, snap_id, m_image_state, ctx);
  req->send();
}

template <typename I>
void
CreateNonPrimaryRequest<I>::handle_write_image_state(int r)
{
  CephContext* cct = m_image_ctx->cct;
  ldout(cct, 15) << "r=" << r << dendl;
...
template <typename I>
void
CreateNonPrimaryRequest<I>::finish(int r)
{
  CephContext* cct = m_image_ctx->cct;
  ldout(cct, 15) << "r=" << r << dendl;
