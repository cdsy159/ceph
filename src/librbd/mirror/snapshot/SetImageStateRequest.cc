...
#undef dout_prefix
#define dout_prefix                                                          \
  *_dout << "librbd::mirror_snapshot::SetImageStateRequest: " << this << " " \
         << __func__ << ": "
...
template <typename I>
void
SetImageStateRequest<I>::get_name()
{
  ...
  librados::AioCompletion* comp = create_rados_callback<
      SetImageStateRequest<I>, &SetImageStateRequest<I>::handle_get_name>(this);
  m_bl.clear();
...
template <typename I>
void
SetImageStateRequest<I>::handle_get_name(int r)
{
  CephContext* cct = m_image_ctx->cct;
...
template <typename I>
void
SetImageStateRequest<I>::get_snap_limit()
{
  CephContext* cct = m_image_ctx->cct;
  ldout(cct, 15) << dendl;
  ...
  librados::AioCompletion* comp = create_rados_callback<
      SetImageStateRequest<I>, &SetImageStateRequest<I>::handle_get_snap_limit>(
      this);
  m_bl.clear();
  int r = m_image_ctx->md_ctx.aio_operate(
      m_image_ctx->header_oid, comp, &op, &m_bl);
  ceph_assert(r == 0);
  comp->release();
}
...
template <typename I>
void
SetImageStateRequest<I>::handle_get_snap_limit(int r)
{
  CephContext* cct = m_image_ctx->cct;
  ldout(cct, 15) << "r=" << r << dendl;
...
template <typename I>
void
SetImageStateRequest<I>::get_metadata()
{
  CephContext* cct = m_image_ctx->cct;
  ldout(cct, 15) << dendl;

  auto ctx = create_context_callback<
      SetImageStateRequest<I>, &SetImageStateRequest<I>::handle_get_metadata>(
      this);
...
template <typename I>
void
SetImageStateRequest<I>::handle_get_metadata(int r)
{
  CephContext* cct = m_image_ctx->cct;
  ldout(cct, 15) << "r=" << r << dendl;
...

  {
    ...
    for (auto& [snap_id, snap_info] : m_image_ctx->snap_info) {
      ...
      m_image_state.snapshots[snap_id] = {
          snap_info.snap_namespace, snap_info.name, snap_info.protection_status};
    }
  }
...
template <typename I>
void
SetImageStateRequest<I>::write_image_state()
{
  CephContext* cct = m_image_ctx->cct;
  ldout(cct, 15) << dendl;
  ...
  auto req = WriteImageStateRequest<I>::create(
      m_image_ctx, m_snap_id, m_image_state, ctx);
  req->send();
}

template <typename I>
void
SetImageStateRequest<I>::handle_write_image_state(int r)
{
  CephContext* cct = m_image_ctx->cct;
  ldout(cct, 15) << "r=" << r << dendl;
...
template <typename I>
void
SetImageStateRequest<I>::update_primary_snapshot()
{
  CephContext* cct = m_image_ctx->cct;
  ldout(cct, 15) << dendl;
...
template <typename I>
void
SetImageStateRequest<I>::handle_update_primary_snapshot(int r)
{
  CephContext* cct = m_image_ctx->cct;
  ldout(cct, 15) << "r=" << r << dendl;
...
template <typename I>
void
SetImageStateRequest<I>::finish(int r)
{
  CephContext* cct = m_image_ctx->cct;
  ldout(cct, 15) << "r=" << r << dendl;
