...
void
ImageStateHeader::encode(bufferlist& bl) const
{
  ENCODE_START(1, 1, bl);
...
void
ImageStateHeader::decode(bufferlist::const_iterator& bl)
{
  DECODE_START(1, bl);
...
void
SnapState::encode(bufferlist& bl) const
{
  ENCODE_START(1, 1, bl);
...
void
SnapState::decode(bufferlist::const_iterator& bl)
{
  DECODE_START(1, bl);
...
void
SnapState::dump(Formatter* f) const
{
...
std::ostream&
operator<<(std::ostream& os, const SnapState& snap_state)
{
  os << "[" << "namespace=" << snap_state.snap_namespace << ", "
...
void
ImageState::encode(bufferlist& bl) const
{
  ENCODE_START(1, 1, bl);
  encode(name, bl);
  encode(features, bl);
...
void
ImageState::decode(bufferlist::const_iterator& bl)
{
  DECODE_START(1, bl);
  decode(name, bl);
  decode(features, bl);
...
void
ImageState::dump(Formatter* f) const
{
  f->dump_string("name", name);
...
std::ostream&
operator<<(std::ostream& os, const ImageState& image_state)
{
  os << "[" << "name=" << image_state.name << ", "
