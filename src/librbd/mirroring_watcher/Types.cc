...
void
ModeUpdatedPayload::encode(bufferlist& bl) const
{
  using ceph::encode;
...
void
ModeUpdatedPayload::decode(__u8 version, bufferlist::const_iterator& iter)
{
  using ceph::decode;
...
void
ModeUpdatedPayload::dump(Formatter* f) const
{
  f->dump_stream("mirror_mode") << mirror_mode;
}

void
ImageUpdatedPayload::encode(bufferlist& bl) const
{
  using ceph::encode;
...
void
ImageUpdatedPayload::decode(__u8 version, bufferlist::const_iterator& iter)
{
  using ceph::decode;
  uint32_t mirror_image_state_decode;
  decode(mirror_image_state_decode, iter);
  mirror_image_state =
      static_cast<cls::rbd::MirrorImageState>(mirror_image_state_decode);
  decode(image_id, iter);
  decode(global_image_id, iter);
}

void
ImageUpdatedPayload::dump(Formatter* f) const
{
...
NotifyMessage::generate_test_instances()
{
  ...
  o.push_back(NotifyMessage(ImageUpdatedPayload(
      cls::rbd::MIRROR_IMAGE_STATE_DISABLING, "image id", "global image id")));
  return o;
}
