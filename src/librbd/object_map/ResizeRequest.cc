...
ResizeRequest::send()
{
  ...
  ldout(cct, 5) << this
                << " resizing on-disk object map: " << "ictx=" << &m_image_ctx
                << ", " << "oid=" << oid << ", num_objs=" << m_num_objs
                << dendl;
...
void
ResizeRequest::finish_request()
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 5) << this << " resizing in-memory object map: " << m_num_objs
                << dendl;
