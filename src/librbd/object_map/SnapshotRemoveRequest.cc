...
#undef dout_prefix
#define dout_prefix                                                      \
  *_dout << "librbd::object_map::SnapshotRemoveRequest: " << this << " " \
         << __func__ << ": "
...
void
SnapshotRemoveRequest::load_map()
{
  ...
  int r = m_image_ctx.md_ctx.aio_operate(
      snap_oid, rados_completion, &op, &m_out_bl);
  ceph_assert(r == 0);
  rados_completion->release();
}

void
SnapshotRemoveRequest::handle_load_map(int r)
{
  CephContext* cct = m_image_ctx.cct;
  ...
  if (r == -ENOENT) {
    ...
    lderr(cct) << "failed to load object map " << oid << ": " << cpp_strerror(r)
               << dendl;
...
void
SnapshotRemoveRequest::remove_snapshot()
{
  ...
  auto rados_completion = librbd::util::create_rados_callback<
      SnapshotRemoveRequest, &SnapshotRemoveRequest::handle_remove_snapshot>(
      this);
...
void
SnapshotRemoveRequest::handle_remove_snapshot(int r)
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 5) << "r=" << r << dendl;
  if (r < 0 && r != -ENOENT) {
    std::string oid(
        ObjectMap<>::object_map_name(m_image_ctx.id, m_next_snap_id));
...
void
SnapshotRemoveRequest::invalidate_next_map()
{
  ceph_assert(ceph_mutex_is_locked(m_image_ctx.owner_lock));
  ceph_assert(ceph_mutex_is_wlocked(m_image_ctx.image_lock));
  ...
  auto ctx = librbd::util::create_context_callback<
      SnapshotRemoveRequest, &SnapshotRemoveRequest::handle_invalidate_next_map>(
      this);
  InvalidateRequest<>* req =
      new InvalidateRequest<>(m_image_ctx, m_next_snap_id, true, ctx);
  req->send();
}

void
SnapshotRemoveRequest::handle_invalidate_next_map(int r)
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 5) << "r=" << r << dendl;
...
void
SnapshotRemoveRequest::remove_map()
{
  CephContext* cct = m_image_ctx.cct;
  std::string oid(ObjectMap<>::object_map_name(m_image_ctx.id, m_snap_id));
  ldout(cct, 5) << "oid=" << oid << dendl;
...
void
SnapshotRemoveRequest::handle_remove_map(int r)
{
  CephContext* cct = m_image_ctx.cct;
  ldout(cct, 5) << "r=" << r << dendl;
...
void
SnapshotRemoveRequest::compute_next_snap_id()
{
...
void
SnapshotRemoveRequest::update_object_map()
{
  ...
  if (m_next_snap_id == m_image_ctx.snap_id && m_next_snap_id == CEPH_NOSNAP) {
    ...
    for (; it != end_it; ++it) {
      if (*it == OBJECT_EXISTS_CLEAN &&
          (i >= m_snap_object_map.size() || *snap_it == OBJECT_EXISTS)) {
        *it = OBJECT_EXISTS;
      }
