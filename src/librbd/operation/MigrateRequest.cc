...
#undef dout_prefix
#define dout_prefix \
  *_dout << "librbd::MigrateRequest: " << this << " " << __func__ << ": "
  ...
  start_async_op()
  {
    ...
    auto ctx = create_async_context_callback(
        ...
        create_context_callback<
            C_MigrateObject<I>, &C_MigrateObject<I>::handle_start_async_op>(
            this));
  ...
  migrate_object()
  {
    ...
    if (is_within_overlap_bounds()) {
      ...
      auto req = new io::ObjectWriteRequest<I>(
          &image_ctx, m_object_no, 0, std::move(bl), m_io_context, 0, 0,
          std::nullopt, {}, ctx);
...
template <typename I>
bool
MigrateRequest<I>::should_complete(int r)
{
  I& image_ctx = this->m_image_ctx;
  CephContext* cct = image_ctx.cct;
  ldout(cct, 10) << "r=" << r << dendl;
...
template <typename I>
void
MigrateRequest<I>::migrate_objects()
{
  I& image_ctx = this->m_image_ctx;
  CephContext* cct = image_ctx.cct;
  ceph_assert(ceph_mutex_is_locked(image_ctx.owner_lock));
  ...
  typename AsyncObjectThrottle<I>::ContextFactory context_factory(
      boost::lambda::bind(
          boost::lambda::new_ptr<C_MigrateObject<I>>(), boost::lambda::_1,
...
template <typename I>
void
MigrateRequest<I>::handle_migrate_objects(int r)
{
  I& image_ctx = this->m_image_ctx;
  CephContext* cct = image_ctx.cct;
...
template <typename I>
uint64_t
MigrateRequest<I>::get_num_overlap_objects()
{
  I& image_ctx = this->m_image_ctx;
  CephContext* cct = image_ctx.cct;
  ldout(cct, 10) << dendl;
