  ...
  complete(int r) override
  {
    ...
    if (should_complete(r)) {
      ldout(image_ctx.cct, 20)
          << m_oid << " C_VerifyObjectCallback completed " << dendl;
      m_io_ctx.close();
...
private:
  ...
  should_complete(int r)
  {
    ...
    ldout(cct, 20) << m_oid
                   << " C_VerifyObjectCallback::should_complete: " << " r=" << r
                   << dendl;
  ...
  send_list_snaps()
  {
    ...
    ldout(image_ctx.cct, 5)
        << m_oid << " C_VerifyObjectCallback::send_list_snaps" << dendl;
  ...
  get_object_state()
  {
    ...
    for (std::vector<librados::clone_info_t>::const_iterator r =
             m_snap_set.clones.begin();
         r != m_snap_set.clones.end(); ++r) {
  ...
  uint64_t
  next_valid_snap_id(uint64_t snap_id)
  {
    I& image_ctx = this->m_image_ctx;
  ...
  bool
  object_map_action(uint8_t new_state)
  {
    I& image_ctx = this->m_image_ctx;
    CephContext* cct = image_ctx.cct;
    ...
    ldout(cct, 10) << "C_VerifyObjectCallback::object_map_action" << " object "
                   << image_ctx.get_object_name(m_object_no) << " state "
                   << (int)state << " new_state " << (int)new_state << dendl;
    ...
    if (state != new_state) {
      ...
      if (r) {
        lderr(cct) << "object map error: object "
                   << image_ctx.get_object_name(m_object_no) << " marked as "
                   << (int)state << ", but should be " << (int)new_state
                   << dendl;
...
template <typename I>
bool
ObjectMapIterateRequest<I>::should_complete(int r)
{
...
ObjectMapIterateRequest<I>::send_verify_objects()
{
  ...
  typename AsyncObjectThrottle<I>::ContextFactory context_factory(
      boost::lambda::bind(
          boost::lambda::new_ptr<C_VerifyObjectCallback<I>>(),
          boost::lambda::_1, &m_image_ctx, snap_id, boost::lambda::_2,
          m_handle_mismatch, &m_invalidate));
...
template <typename I>
uint64_t
ObjectMapIterateRequest<I>::get_image_size() const
{
...
template <typename I>
void
ObjectMapIterateRequest<I>::send_invalidate_object_map()
{
  CephContext* cct = m_image_ctx.cct;
  ...
  object_map::InvalidateRequest<I>* req =
      object_map::InvalidateRequest<I>::create(
          m_image_ctx, m_image_ctx.snap_id, true,
