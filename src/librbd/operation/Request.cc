...
template <typename I>
Context*
Request<I>::create_context_finisher(int r)
{
...
template <typename I>
void
Request<I>::finish_and_destroy(int r)
{
  I& image_ctx = this->m_image_ctx;
  CephContext* cct = image_ctx.cct;
...
template <typename I>
bool
Request<I>::commit_op_event(int r)
{
  I& image_ctx = this->m_image_ctx;
  std::shared_lock image_locker{image_ctx.image_lock};
  ...
  if (image_ctx.journal != nullptr &&
    ...
    image_ctx.journal->commit_op_event(
        m_op_tid, r, new C_CommitOpEvent(this, r));
    return true;
  }
  return false;
}
...
template <typename I>
void
Request<I>::handle_commit_op_event(int r, int original_ret_val)
{
  I& image_ctx = this->m_image_ctx;
  CephContext* cct = image_ctx.cct;
  ldout(cct, 10) << this << " " << __func__ << ": r=" << r << dendl;
...
template <typename I>
void
Request<I>::replay_op_ready(Context* on_safe)
{
  I& image_ctx = this->m_image_ctx;
  ceph_assert(ceph_mutex_is_locked(image_ctx.owner_lock));
...
template <typename I>
void
Request<I>::handle_op_event_safe(int r)
{
  I& image_ctx = this->m_image_ctx;
  CephContext* cct = image_ctx.cct;
  ldout(cct, 10) << this << " " << __func__ << ": r=" << r << dendl;
