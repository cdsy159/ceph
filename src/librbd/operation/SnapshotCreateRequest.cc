...
SnapshotCreateRequest<I>::SnapshotCreateRequest(
  ...
  m_ignore_notify_quiesce_error(
      flags & SNAP_CREATE_FLAG_IGNORE_NOTIFY_QUIESCE_ERROR),
  m_prog_ctx(prog_ctx)
{}

template <typename I>
void
SnapshotCreateRequest<I>::send_op()
{
...
template <typename I>
void
SnapshotCreateRequest<I>::send_notify_quiesce()
{
...
template <typename I>
Context*
SnapshotCreateRequest<I>::handle_notify_quiesce(int* result)
{
  I& image_ctx = this->m_image_ctx;
  CephContext* cct = image_ctx.cct;
...
template <typename I>
void
SnapshotCreateRequest<I>::send_suspend_requests()
{
  I& image_ctx = this->m_image_ctx;
  CephContext* cct = image_ctx.cct;
  ldout(cct, 5) << this << " " << __func__ << dendl;
...
template <typename I>
Context*
SnapshotCreateRequest<I>::handle_suspend_requests(int* result)
{
  I& image_ctx = this->m_image_ctx;
  CephContext* cct = image_ctx.cct;
  ldout(cct, 5) << this << " " << __func__ << ": r=" << *result << dendl;
...
template <typename I>
Context*
SnapshotCreateRequest<I>::handle_suspend_aio(int* result)
{
  I& image_ctx = this->m_image_ctx;
  CephContext* cct = image_ctx.cct;
  ldout(cct, 5) << this << " " << __func__ << ": r=" << *result << dendl;
...
template <typename I>
Context*
SnapshotCreateRequest<I>::handle_append_op_event(int* result)
{
  I& image_ctx = this->m_image_ctx;
  CephContext* cct = image_ctx.cct;
  ldout(cct, 5) << this << " " << __func__ << ": r=" << *result << dendl;
...
template <typename I>
void
SnapshotCreateRequest<I>::send_allocate_snap_id()
{
  I& image_ctx = this->m_image_ctx;
  CephContext* cct = image_ctx.cct;
  ldout(cct, 5) << this << " " << __func__ << dendl;
...
template <typename I>
Context*
SnapshotCreateRequest<I>::handle_allocate_snap_id(int* result)
{
  I& image_ctx = this->m_image_ctx;
  CephContext* cct = image_ctx.cct;
...
template <typename I>
void
SnapshotCreateRequest<I>::send_create_snap()
{
  I& image_ctx = this->m_image_ctx;
  CephContext* cct = image_ctx.cct;
  ldout(cct, 5) << this << " " << __func__ << dendl;
  ...
  librados::AioCompletion* rados_completion = create_rados_callback<
      SnapshotCreateRequest<I>, &SnapshotCreateRequest<I>::handle_create_snap>(
      this);
  int r =
...
template <typename I>
Context*
SnapshotCreateRequest<I>::handle_create_snap(int* result)
{
  I& image_ctx = this->m_image_ctx;
  CephContext* cct = image_ctx.cct;
  ldout(cct, 5) << this << " " << __func__ << ": r=" << *result << dendl;
...
template <typename I>
Context*
SnapshotCreateRequest<I>::handle_create_object_map(int* result)
{
  I& image_ctx = this->m_image_ctx;
  CephContext* cct = image_ctx.cct;
  ldout(cct, 5) << this << " " << __func__ << ": r=" << *result << dendl;
  ...
  if (*result < 0) {
    lderr(cct) << this << " " << __func__
               << ": failed to snapshot object map: " << cpp_strerror(*result)
               << dendl;
...
template <typename I>
Context*
SnapshotCreateRequest<I>::send_create_image_state()
{
  I& image_ctx = this->m_image_ctx;
  auto mirror_ns =
...
template <typename I>
Context*
SnapshotCreateRequest<I>::handle_create_image_state(int* result)
{
  I& image_ctx = this->m_image_ctx;
  CephContext* cct = image_ctx.cct;
  ldout(cct, 5) << this << " " << __func__ << ": r=" << *result << dendl;
  ...
  if (*result < 0) {
    lderr(cct) << this << " " << __func__
               << ": failed to create image state: " << cpp_strerror(*result)
               << dendl;
    save_result(result);
  }
...
template <typename I>
Context*
SnapshotCreateRequest<I>::handle_release_snap_id(int* result)
{
  I& image_ctx = this->m_image_ctx;
  CephContext* cct = image_ctx.cct;
  ldout(cct, 5) << this << " " << __func__ << ": r=" << *result << dendl;
...
template <typename I>
Context*
SnapshotCreateRequest<I>::send_notify_unquiesce()
{
  I& image_ctx = this->m_image_ctx;
  CephContext* cct = image_ctx.cct;
...
template <typename I>
Context*
SnapshotCreateRequest<I>::handle_notify_unquiesce(int* result)
{
  I& image_ctx = this->m_image_ctx;
  CephContext* cct = image_ctx.cct;
  ldout(cct, 5) << this << " " << __func__ << ": r=" << *result << dendl;
...
template <typename I>
void
SnapshotCreateRequest<I>::update_snap_context()
{
  I& image_ctx = this->m_image_ctx;
  ...
  image_ctx.add_snap(
      m_snap_namespace, m_snap_name, m_snap_id, m_size, m_parent_info,
  ...
  snaps.insert(
      snaps.end(), image_ctx.snapc.snaps.begin(), image_ctx.snapc.snaps.end());
