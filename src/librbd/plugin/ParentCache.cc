...
#undef dout_prefix
#define dout_prefix \
  *_dout << "librbd::plugin::ParentCache: " << this << " " << __func__ << ": "
    ...
    Context* on_finish)
{
  bool parent_cache_enabled =
      image_ctx->config.template get_val<bool>("rbd_parent_cache_enabled");
...
template <typename I>
void
ParentCache<I>::handle_init_parent_cache(int r, Context* on_finish)
{
