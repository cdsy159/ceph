...
#undef dout_prefix
#define dout_prefix \
  *_dout << "librbd::watcher::Notifier: " << this << " " << __func__ << ": "
...
Notifier::C_AioNotify::C_AioNotify(
    Notifier* notifier,
...
void
Notifier::C_AioNotify::finish(int r)
{
  if (response != nullptr) {
    if (r == 0 || r == -ETIMEDOUT) {
      try {
...
Notifier::Notifier(
  ...
  m_aio_notify_lock(ceph::make_mutex(util::unique_lock_name(
      "librbd::object_watcher::Notifier::m_aio_notify_lock",
      this)))
{
...
void
Notifier::flush(Context* on_finish)
{
  std::lock_guard aio_notify_locker{m_aio_notify_lock};
...
void
Notifier::notify(bufferlist& bl, NotifyResponse* response, Context* on_finish)
{
  {
    std::lock_guard aio_notify_locker{m_aio_notify_lock};
...
void
Notifier::handle_notify(int r, Context* on_finish)
{
