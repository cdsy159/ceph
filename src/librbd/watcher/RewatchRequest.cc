...
#undef dout_prefix
#define dout_prefix                                                        \
  *_dout << "librbd::watcher::RewatchRequest: " << this << " " << __func__ \
         << " "
...
RewatchRequest::unwatch()
{
  ...
  librados::AioCompletion* aio_comp =
      create_rados_callback<RewatchRequest, &RewatchRequest::handle_unwatch>(
          this);
...
void
RewatchRequest::handle_unwatch(int r)
{
  CephContext* cct = reinterpret_cast<CephContext*>(m_ioctx.cct());
...
void
RewatchRequest::rewatch()
{
  CephContext* cct = reinterpret_cast<CephContext*>(m_ioctx.cct());
  ldout(cct, 10) << dendl;

  librados::AioCompletion* aio_comp =
      create_rados_callback<RewatchRequest, &RewatchRequest::handle_rewatch>(
          this);
...
void
RewatchRequest::handle_rewatch(int r)
{
  CephContext* cct = reinterpret_cast<CephContext*>(m_ioctx.cct());
  ldout(cct, 10) << "r=" << r << dendl;
  if (r < 0) {
