...
void
Beacon::init(const MDSMap& mdsmap)
{
...
Beacon::ms_dispatch2(const ref_t<Message>& m)
{
 ...
 */
void
Beacon::handle_mds_beacon(const cref_t<MMDSBeacon>& m)
{
  std::unique_lock lock(mutex);
  ...
  if (it != seq_stamp.end()) {
    ...
    dout(5) << "received beacon reply " << ceph_mds_state_name(m->get_state())
    ...
    dout(1) << "discarding unexpected beacon reply "
            << ceph_mds_state_name(m->get_state()) << " seq " << m->get_seq()
            << " dne" << dendl;
  }
}
...
void
Beacon::send_and_wait(const double duration)
{
  std::unique_lock lock(mutex);
  _send();
  auto awaiting_seq = last_seq;
  dout(20) << __func__ << ": awaiting " << awaiting_seq << " for up to "
           << duration << "s" << dendl;
...
Beacon::_send()
{
  ...
  ++last_seq;
  dout(5) << "Sending beacon " << ceph_mds_state_name(want_state) << " seq "
          << last_seq << dendl;
  ...
  auto beacon = make_message<MMDSBeacon>(
      monc->get_fsid(), mds_gid_t(monc->get_global_id()), name, epoch,
      want_state, last_seq, CEPH_FEATURES_SUPPORTED_DEFAULT);
 ...
 */
void
Beacon::notify_mdsmap(const MDSMap& mdsmap)
{
  std::unique_lock lock(mutex);
...
void
Beacon::_notify_mdsmap(const MDSMap& mdsmap)
{
...
void
Beacon::set_want_state(const MDSMap& mdsmap, MDSMap::DaemonState newstate)
{
  std::unique_lock lock(mutex);
  ...
  if (want_state != newstate) {
    dout(5) << __func__ << ": " << ceph_mds_state_name(want_state) << " -> "
 ...
 */
void
Beacon::notify_health(MDSRank const* mds)
{
  std::unique_lock lock(mutex);
  if (!mds) {
    // No MDS rank held
  ...
  if (!mds->damage_table.empty()) {
    MDSHealthMetric m(
        MDS_HEALTH_DAMAGE, HEALTH_ERR, std::string("Metadata damage detected"));
    health.metrics.push_back(m);
  }
  ...
  {
    auto&& late_clients = mds->locker->get_late_revoking_clients(
    ...
    for (const auto& client : late_clients) {
      ...
      CachedStackStringStream css;
      *css << "Client " << s->get_human_name()
           << " failing to respond to capability release";
      MDSHealthMetric m(
          MDS_HEALTH_CLIENT_LATE_RELEASE, HEALTH_WARN, css->strv());
      m.metadata["client_id"] = stringify(client.v);
      late_cap_metrics.emplace_back(std::move(m));
    }

    if (late_cap_metrics.size() <=
        (size_t)g_conf()->mds_health_summarize_threshold) {
      auto&& m = late_cap_metrics;
      health.metrics.insert(
          std::end(health.metrics), std::cbegin(m), std::cend(m));
    } else {
      CachedStackStringStream css;
      *css << "Many clients (" << late_cap_metrics.size()
           << ") failing to respond to capability release";
      MDSHealthMetric m(
          MDS_HEALTH_CLIENT_LATE_RELEASE_MANY, HEALTH_WARN, css->strv());
  ...
  // Detect clients failing to advance their old_client_tid
  {
    ...
    const auto min_caps_working_set =
        g_conf().get_val<uint64_t>("mds_min_caps_working_set");
    const auto recall_warning_threshold =
        g_conf().get_val<Option::size_t>("mds_recall_warning_threshold");
    ...
    for (auto& session : sessions) {
      ...
      if (recall_caps > recall_warning_threshold &&
          num_caps > min_caps_working_set) {
        dout(2) << "Session " << *session
                << " is not releasing caps fast enough. Recalled caps at "
                << recall_caps << " > " << recall_warning_threshold
                << " (mds_recall_warning_threshold)." << dendl;
        CachedStackStringStream css;
        *css << "Client " << session->get_human_name()
      ...
      if ((session->get_num_trim_requests_warnings() > 0 &&
        ...
        *css << "Client " << session->get_human_name()
        ...
        MDSHealthMetric m(
            MDS_HEALTH_CLIENT_OLDEST_TID, HEALTH_WARN, css->strv());
        m.metadata["client_id"] = stringify(session->get_client());
        large_completed_requests_metrics.emplace_back(std::move(m));
      }
    }

    if (late_recall_metrics.size() <=
        (size_t)g_conf()->mds_health_summarize_threshold) {
    ...
    if (large_completed_requests_metrics.size() <=
        (size_t)g_conf()->mds_health_summarize_threshold) {
      ...
      *css << "Many clients (" << large_completed_requests_metrics.size()
      ...
      MDSHealthMetric m(
          MDS_HEALTH_CLIENT_OLDEST_TID_MANY, HEALTH_WARN, css->strv());
      m.metadata["client_count"] =
          stringify(large_completed_requests_metrics.size());
      health.metrics.push_back(m);
  ...
  // Detect MDS_HEALTH_SLOW_REQUEST condition
  {
    ...
    if (slow) {
      ...
      CachedStackStringStream css;
      *css << slow << " slow requests are blocked > "
           << g_conf()->mds_op_complaint_time << " secs";
  ...
  if (mds->mdcache->is_readonly()) {
    MDSHealthMetric m(
        MDS_HEALTH_READ_ONLY, HEALTH_WARN, "MDS in read-only mode");
    health.metrics.push_back(m);
  }
  ...
  if (auto c = mds->sessionmap.num_broken_root_squash_clients(); c > 0) {
    ...
    for (auto&& session : mds->sessionmap.get_broken_root_squash_clients()) {
      ...
      *css << "Client " << session->get_human_name()
      ...
      MDSHealthMetric m(
          MDS_HEALTH_CLIENTS_BROKEN_ROOTSQUASH, HEALTH_ERR, css->strv());
      m.metadata["client_id"] = stringify(session->get_client());
    ...
    if (metrics.size() <= (size_t)g_conf()->mds_health_summarize_threshold) {
      health.metrics.insert(
          std::end(health.metrics),
          std::make_move_iterator(std::begin(metrics)),
          std::make_move_iterator(std::end(metrics)));
    } else {
      CachedStackStringStream css;
      *css << "There are " << c
           << " clients with broken root_squash implementations";
    ...
    if (defer_client_eviction && !laggy_clients.empty()) {
      if (laggy_clients.size() <=
        ...
        *css << "Many client (" << laggy_clients.size()
        ...
        MDSHealthMetric m(
            MDS_HEALTH_CLIENTS_LAGGY_MANY, HEALTH_WARN, css->strv());
  ...
  if (mds->is_replay()) {
    ...
    if (estimate.elapsed_time > std::chrono::seconds(30)) {
      *css << "replay: " << estimate.percent_complete
           << "% complete - elapsed time: " << estimate.elapsed_time
           << ", estimated time remaining: " << estimate.estimated_time;
      MDSHealthMetric m(
          MDS_HEALTH_ESTIMATED_REPLAY_TIME, HEALTH_WARN, css->strv());
      health.metrics.push_back(m);
    }
  }
}
