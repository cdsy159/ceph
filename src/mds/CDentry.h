...
struct ClientLease : public boost::intrusive::set_base_hook<> {
  ...
  ClientLease(CDentry* p, Session* s) :
    parent(p), session(s), item_session_lease(this), item_lease(this)
  {}
...
struct client_is_key {
  ...
  const type
  operator()(const ClientLease& l) const
  {
    return l.get_client();
  }
};
...
public:
  ...
  struct linkage_t {
    ...
    inodeno_t
    get_remote_ino() const
    {
      return remote_ino;
    }

    unsigned char
    get_remote_d_type() const
    {
    ...
    void
    set_remote(inodeno_t ino, unsigned char d_type)
    {
      remote_ino = ino;
  ...
  void
  set_alternate_name(mempool::mds_co::string altn)
  {
    alternate_name = std::move(altn);
  }

  void
  set_alternate_name(std::string_view altn)
  {
  ...
  bool
  use_projected(client_t client, const MutationRef& mut) const
  {
    return lock.can_read_projected(client) || lock.is_xlocked_by(mut);
  }

  linkage_t*
  get_linkage(client_t client, const MutationRef& mut)
  {
  ...
  void make_path(filepath& fp, bool projected = false, int path_comp_count = -1)
      const;
  void make_trimmed_path(
      filepath& fp,
      bool projected = false,
      int path_comp_count = 10) const;
  ...
  void
  set_projected_version(version_t v)
  {
    projected_version = v;
  }
  ...
  void
  encode_export(ceph::buffer::list& bl)
  {
    ENCODE_START(1, 1, bl);
    encode(first, bl);
    encode(state, bl);
  ...
  void
  decode_import(ceph::buffer::list::const_iterator& blp, LogSegmentRef const& ls)
  {
    DECODE_START(1, blp);
    decode(first, blp);
    __u32 nstate;
  ...
  const ClientLease*
  get_client_lease(client_t c) const
  {
  ...
  typedef boost::intrusive::
      set<ClientLease, boost::intrusive::key_of_value<client_is_key>>
          ClientLeaseMap;
