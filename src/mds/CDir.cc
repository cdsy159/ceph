...
#undef dout_prefix
#define dout_prefix                                               \
  *_dout << "mds." << mdcache->mds->get_nodeid() << ".cache.dir(" \
         << this->dirfrag() << ") "
...
ostream&
operator<<(ostream& out, const CDir& dir)
{
  ...
  if (dir.state_test(CDir::STATE_COMPLETE))
    out << "|complete";
  if (dir.state_test(CDir::STATE_FREEZINGTREE))
    out << "|freezingtree";
  if (dir.state_test(CDir::STATE_FROZENTREE))
    out << "|frozentree";
  if (dir.state_test(CDir::STATE_AUXSUBTREE))
    out << "|auxsubtree";
  if (dir.state_test(CDir::STATE_FROZENDIR))
    out << "|frozendir";
  if (dir.state_test(CDir::STATE_FREEZINGDIR))
    out << "|freezingdir";
  ...
  if (dir.state_test(CDir::STATE_EXPORTING))
    out << "|exporting";
  if (dir.state_test(CDir::STATE_IMPORTING))
    out << "|importing";
  if (dir.state_test(CDir::STATE_STICKY))
    out << "|sticky";
  if (dir.state_test(CDir::STATE_DNPINNEDFRAG))
    out << "|dnpinnedfrag";
  if (dir.state_test(CDir::STATE_ASSIMRSTAT))
    out << "|assimrstat";
...
void
CDir::print(ostream& out) const
{
  out << *this;
}

ostream&
CDir::print_db_line_prefix(ostream& out) const
{
...
CDir::check_rstats(bool scrub)
{
  ...
  if (!nest_info.same_sums(fnode->rstat)) {
    dout(1) << "mismatch between child accounted_rstats and my rstats!"
            << dendl;
    ...
    if (!scrub) {
      for (auto i = items.begin(); i != items.end(); ++i) {
        ...
        if (dn->get_linkage()->is_primary()) {
          ...
          dout(1) << *dn << " rstat " << in->get_inode()->accounted_rstat
                  << dendl;
        } else {
          dout(1) << *dn << dendl;
        }
...
void
CDir::adjust_num_inodes_with_caps(int d)
{
...
CDir::lookup(std::string_view name, snapid_t snap)
{
  ...
  auto iter = items.lower_bound(
      dentry_key_t(snap, name, inode->hash_dentry_name(name)));
  if (iter == items.end())
    return 0;
  if (iter->second->get_name() == name && iter->second->first <= snap &&
...
CDentry*
CDir::lookup_exact_snap(std::string_view name, snapid_t last)
{
...
void
CDir::adjust_dentry_lru(CDentry* dn)
{
  bool bottom_lru;
  if (dn->get_linkage()->is_primary()) {
...
CDentry*
CDir::add_null_dentry(std::string_view dname, snapid_t first, snapid_t last)
{
  // foreign
  ...
  // create dentry
  CDentry* dn =
      new CDentry(dname, inode->hash_dentry_name(dname), "", first, last);
  dn->dir = this;
    ...
    snapid_t last)
{
  ...
  CDentry* dn = new CDentry(
      dname, inode->hash_dentry_name(dname), std::move(alternate_name), first,
      last);
  dn->dir = this;
  dn->version = get_projected_version();
  dn->check_corruption(true);
    ...
    snapid_t last)
{
  ...
  // create dentry
  CDentry* dn = new CDentry(
      dname, inode->hash_dentry_name(dname), std::move(alternate_name), ino,
      d_type, first, last);
  dn->dir = this;
  dn->version = get_projected_version();
  dn->check_corruption(true);
...
void
CDir::link_remote_inode(CDentry* dn, CInode* in)
{
  link_remote_inode(dn, in->ino(), IFTODT(in->get_projected_inode()->mode));
}

void
CDir::link_remote_inode(CDentry* dn, inodeno_t ino, unsigned char d_type)
{
...
void
CDir::link_primary_inode(CDentry* dn, CInode* in)
{
...
void
CDir::link_inode_work(CDentry* dn, CInode* in)
{
...
void
CDir::unlink_inode(CDentry* dn, bool adjust_lru)
{
  if (dn->get_linkage()->is_primary()) {
    dout(12) << __func__ << " " << *dn << " " << *dn->get_linkage()->get_inode()
             << dendl;
  } else {
    dout(12) << __func__ << " " << *dn << dendl;
  }
...
void
CDir::try_remove_unlinked_dn(CDentry* dn)
{
...
void
CDir::unlink_inode_work(CDentry* dn)
{
...
bool
CDir::is_in_bloom(std::string_view name)
{
  if (!bloom)
    return false;
...
bool
CDir::try_trim_snap_dentry(CDentry* dn, const set<snapid_t>& snaps)
{
  if (dn->last == CEPH_NOSNAP) {
    return false;
  }
...
void
CDir::prepare_new_fragment(bool replay)
{
  if (!replay && is_auth()) {
    _freeze_dir();
    mark_complete();
...
void
CDir::finish_old_fragment(MDSContext::vec& waiters, bool replay)
{
    ...
    bool replay)
{
  ...
  dout(10) << " rstatdiff " << rstatdiff << " fragstatdiff " << fragstatdiff
           << dendl;
  ...
  while (!items.empty()) {
    ...
    int n = (subfrag.value() & (subfrag.mask() ^ frag.mask())) >>
            subfrag.mask_shift();
    dout(15) << " subfrag " << subfrag << " n=" << n << " for " << p->first
             << dendl;
    CDir* f = subfrags[n];
    f->steal_dentry(dn);
  }
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (i == 0) {
      ...
      dout(10) << " giving rstatdiff " << rstatdiff << " fragstatdiff"
               << fragstatdiff << " to " << *subfrags[0] << dendl;
...
void
CDir::merge(const std::vector<CDir*>& subs, MDSContext::vec& waiters, bool replay)
{
  ...
  for (const auto& dir : subs) {
    ...
    if (dir->get_fnode()->accounted_rstat.version == rstat_version)
      rstatdiff.add_delta(
          dir->get_fnode()->accounted_rstat, dir->get_fnode()->rstat);
    if (dir->get_fnode()->accounted_fragstat.version == dirstat_version)
      fragstatdiff.add_delta(
          dir->get_fnode()->accounted_fragstat, dir->get_fnode()->fragstat,
...
void
CDir::resync_accounted_fragstat()
{
  ...
  if (pf->accounted_fragstat.version != pi->dirstat.version) {
    ...
    dout(10) << __func__ << " " << pf->accounted_fragstat << " -> "
             << pf->fragstat << dendl;
...
CDir::resync_accounted_rstat()
{
  ...
  if (pf->accounted_rstat.version != pi->rstat.version) {
    ...
    dout(10) << __func__ << " " << pf->accounted_rstat << " -> " << pf->rstat
             << dendl;
...
void
CDir::assimilate_dirty_rstat_inodes(MutationRef& mut)
{
  dout(10) << __func__ << dendl;
...
void
CDir::assimilate_dirty_rstat_inodes_finish(EMetaBlob* blob)
{
...
void
CDir::add_dentry_waiter(std::string_view dname, snapid_t snapid, MDSContext* c)
{
  if (waiting_on_dentry.empty())
    get(PIN_DNWAITER);
  waiting_on_dentry[string_snap_t(dname, snapid)].push_back(c);
  dout(10) << __func__ << " dentry " << dname << " snap " << snapid << " " << c
           << " on " << *this << dendl;
}

void
CDir::take_dentry_waiting(
    std::string_view dname,
    snapid_t first,
    snapid_t last,
    MDSContext::vec& ls)
{
  ...
  while (it != waiting_on_dentry.end() && !(ub < it->first)) {
    dout(10) << __func__ << " " << dname << " [" << first << "," << last
             << "] found waiter on snap " << it->first.snapid << " on " << *this
             << dendl;
...
void
CDir::add_waiter(uint64_t tag, MDSContext* c)
{
  // hierarchical?
...
void
CDir::take_waiting(uint64_t mask, MDSContext::vec& ls)
{
  if ((mask & WAIT_DENTRY) && !waiting_on_dentry.empty()) {
    // take all dentry waiters
    for (const auto& p : waiting_on_dentry) {
      dout(10) << "take_waiting dentry " << p.first.name << " snap "
               << p.first.snapid << " on " << *this << dendl;
...
void
CDir::finish_waiting(uint64_t mask, int result)
{
  dout(11) << __func__ << " mask " << hex << mask << dec << " result " << result
           << " on " << *this << dendl;
...
CDir::fnode_ptr
CDir::project_fnode(const MutationRef& mut)
{
...
void
CDir::pop_and_dirty_projected_fnode(
...
void
CDir::mark_dirty(LogSegmentRef const& ls, version_t pv)
{
  ceph_assert(is_auth());
  ...
  if (pv) {
    ...
    ceph_assert(
        !projected_fnode.empty() && pv <= projected_fnode.front()->version);
  }
...
void
CDir::_mark_dirty(LogSegmentRef const& ls)
{
  if (!state_test(STATE_DIRTY)) {
    dout(10) << __func__ << " (was clean) " << *this << " version "
             << get_version() << dendl;
    _set_dirty_flag();
    ceph_assert(ls);
  } else {
    dout(10) << __func__ << " (already dirty) " << *this << " version "
             << get_version() << dendl;
  }
  if (ls) {
...
void
CDir::mark_new(LogSegmentRef const& ls)
{
  ls->new_dirfrags.push_back(&item_new);
...
void
CDir::set_fresh_fnode(fnode_const_ptr&& ptr)
{
    ...
    bool ignore_authpinnability)
{
  ...
  if (!dname.empty() && last == CEPH_NOSNAP &&
      !g_conf().get_val<bool>("mds_dir_prefetch")) {
...
void
CDir::fetch_keys(const std::vector<dentry_key_t>& keys, MDSContext* c)
{
  ...
  MDSContext::vec_alloc<mempool::mds_co::pool_allocator>* fallback_waiting =
      nullptr;
  ...
  for (auto& key : keys) {
    ...
    auto em = waiting_on_dentry.emplace(
        std::piecewise_construct, std::forward_as_tuple(key.name, key.snapid),
...
void
CDir::_omap_fetch(std::set<string>* keys, MDSContext* c)
{
  ...
  if (keys) {
    ...
    ceph_assert(!c);
    rd.omap_get_vals(
        "", "", g_conf()->mds_dir_keys_per_op, &fin->omap, &fin->more,
        &fin->ret2);
  }
  // check the correctness of backtrace
  ...
  mdcache->mds->objecter->read(
      oid, oloc, rd, CEPH_NOSNAP, NULL, 0,
      new C_OnFinisher(fin, mdcache->mds->finisher));
}

void
CDir::_omap_fetch_more(
    ...
    MDSContext* c)
{
  ...
  ObjectOperation rd;
  rd.omap_get_vals(
      fin->omap.rbegin()->first, "", /* filter prefix */
      g_conf()->mds_dir_keys_per_op, &fin->omap_more, &fin->more, &fin->ret);
  mdcache->mds->objecter->read(
      oid, oloc, rd, CEPH_NOSNAP, NULL, 0,
      new C_OnFinisher(fin, mdcache->mds->finisher));
}
    ...
    bool* force_dirty)
{
  ...
  dout(20) << "_fetched pos " << pos << " marker '" << type << "' dname '"
           << dname << " [" << first << "," << last << "]" << dendl;

  bool stale = false;
  if (snaps && last != CEPH_NOSNAP) {
    set<snapid_t>::const_iterator p = snaps->lower_bound(first);
    if (p == snaps->end() || *p > last) {
      dout(10) << " skipping stale dentry on [" << first << "," << last << "]"
               << dendl;
      stale = true;
    }
  }
  ...
  if (type == 'L' || type == 'l') {
    ...
    if (dn) {
      CDentry::linkage_t* dnl = dn->get_linkage();
      dout(12) << "_fetched had " << (dnl->is_null() ? "NEG" : "") << " dentry "
               << *dn << dendl;
      if (committed_version == 0 && dnl->is_remote() && dn->is_dirty() &&
          ino == dnl->get_remote_ino() && d_type == dnl->get_remote_d_type() &&
          alternate_name == dn->get_alternate_name()) {
        // see comment below
        dout(10) << "_fetched  had underwater dentry " << *dn
                 << ", marking clean" << dendl;
        dn->mark_clean();
      }
    } else {
      // (remote) link
      dn = add_remote_dentry(
          dname, ino, d_type, std::move(alternate_name), first, last);

      // link to inode?
      CInode* in = mdcache->get_inode(ino); // we may or may not have it.
      if (in) {
        dn->link_remote(dn->get_linkage(), in);
        dout(12) << "_fetched  got remote link " << ino << " which we have "
                 << *in << dendl;
      } else {
        dout(12) << "_fetched  got remote link " << ino << " (don't have it)"
                 << dendl;
      }
    }
    ...
    if (dn) {
      ...
      if (dnl->is_primary()) {
        ...
        if (in->state_test(CInode::STATE_REJOINUNDEF)) {
          undef_inode = true;
            ...
            committed_version == 0 && dn->is_dirty() &&
	   ...
	   */
          dout(10) << "_fetched  had underwater dentry " << *dn
                   << ", marking clean" << dendl;
          dn->mark_clean();
          dout(10) << "_fetched  had underwater inode " << *dnl->get_inode()
                   << ", marking clean" << dendl;
          in->mark_clean();
        }
      }
    ...
    if (!dn || undef_inode) {
      ...
      if (!in || undef_inode) {
        ...
        if (!undef_inode) {
          ...
          dn = add_primary_dentry(
              dname, in, std::move(alternate_name), first, last); // link
        }
        ...
        dn = add_primary_dentry(
            dname, in, std::move(alternate_name), first, last);
      } else {
        dout(0) << "_fetched  badness: got (but i already had) " << *in
                << " mode " << in->get_inode()->mode << " mtime "
                << in->get_inode()->mtime << dendl;
        string dirpath, inopath;
        this->inode->make_path_string(dirpath);
        in->make_path_string(inopath);
        mdcache->mds->clog->error()
            << "loaded dup inode " << inode_data.inode->ino << " [" << first
            << "," << last << "] v" << inode_data.inode->version << " at "
            << dirpath << "/" << dname << ", but inode " << in->vino() << " v"
            << in->get_version() << " already exists at " << inopath;
        return dn;
      }
    }
  } else {
    ...
    int r)
{
  ...
  dout(10) << "_fetched header " << hdrbl.length() << " bytes " << omap.size()
           << " keys for " << *this << dendl;
  ...
  fnode_t got_fnode;
  {
    ...
    try {
      ...
      derr << "Corrupt fnode in dirfrag " << dirfrag() << ": " << err.what()
           << dendl;
  ...
  if (fnode->snap_purged_thru < realm->get_last_destroyed()) {
    ...
    dout(10) << " snap_purged_thru " << fnode->snap_purged_thru << " < "
             << realm->get_last_destroyed() << ", snap purge based on "
             << *snaps << dendl;
  ...
  auto proc_waiters = [&](const string_snap_t& key) {
    ...
    while (w_it != waiting_on_dentry.rend()) {
      ...
      if (cmp == 0) {
        touch = true;
        std::copy(
            w_it->second.begin(), w_it->second.end(),
  ...
  auto proc_nulls_and_waiters = [&](const string& str_key,
                                    const string_snap_t& key) {
    bool touch = false;
    int count = 0;
  ...
  for (auto p = omap.rbegin(); p != omap.rend(); ++p, --pos) {
    ...
    try {
      dn = _load_dentry(
          p->first, key.name, key.snapid, p->second, pos, snaps, rand_threshold,
          &force_dirty);
    } catch (const buffer::error& err) {
  ...
  if (complete) {
    if (!waiting_on_dentry.empty()) {
      for (auto& p : waiting_on_dentry) {
        std::copy(
            p.second.begin(), p.second.end(), std::back_inserter(finished));
  ...
  if (!null_keys.empty()) {
    ...
    for (auto& key : null_keys) {
      ...
      if (dn) {
        dout(12) << "_fetched got null for key " << key << ", have " << *dn
                 << dendl;
      } else {
        dn = add_null_dentry(key.name, first, key.snapid);
        dout(12) << "_fetched got null for key " << key << ", added " << *dn
                 << dendl;
      }
      mdcache->touch_dentry(dn);
...
void
CDir::go_bad_dentry(snapid_t last, std::string_view dname)
{
...
void
CDir::go_bad(bool complete)
{
  dout(10) << __func__ << " " << frag << dendl;
  const bool fatal =
      mdcache->mds->damage_table.notify_dirfrag(inode->ino(), frag, get_path());
  if (fatal) {
    mdcache->mds->damaged();
    ceph_abort(); // unreachable, damaged() respawns us
  }
    ...
    int op_prio)
{
  ...
  // preconditions
  ceph_assert(
      want <= get_version() || get_version() == 0); // can't commit the future
...
public:
  ...
  finish(int r) override
  {
    dir->_omap_commit_ops(
        r, op_prio, metapool, version, is_new, to_set, dfts, to_remove,
        stale_items);
  }
...
void
CDir::_encode_primary_inode_base(
    dentry_commit_item& item,
    bufferlist& dfts,
    bufferlist& bl)
{
  ENCODE_START(6, 4, bl);
    ...
    mempool::mds_co::compact_set<mempool::mds_co::string>& stales)
{
  ...
  C_GatherBuilder gather(
      g_ceph_context,
      new C_OnFinisher(
          new C_IO_Dir_Committed(this, version), mdcache->mds->finisher));
...
CDir::_omap_commit(int op_prio)
{
  ...
  auto c = new C_IO_Dir_Commit_Ops(
      this, op_prio, std::move(to_set), std::move(dfts), std::move(to_remove),
    ...
    bufferlist& bl)
{
  ...
  if (linkage.is_remote()) {
    ...
    dout(14) << " dn '" << dn->get_name() << "' remote ino " << item.ino
             << dendl;
...
void
CDir::_commit(version_t want, int op_prio)
{
  ...
  if (committed_version >= want) {
    dout(10) << "already committed " << committed_version << " >= " << want
             << dendl;
    return;
  }
  // already committing >= want?
  if (committing_version >= want) {
    dout(10) << "already committing " << committing_version << " >= " << want
             << dendl;
    ceph_assert(state_test(STATE_COMMITTING));
    return;
  }
  ...
  if (committing_version > committed_version) {
    dout(10) << "already committing older " << committing_version
  ...
  if (mdcache->mds->logger)
    mdcache->mds->logger->inc(l_mds_dir_commit);
 ...
 */
void
CDir::_committed(int r, version_t v)
{
  if (r < 0) {
    // the directory could be partly purged during MDS failover
    if (r == -ENOENT && committed_version == 0 && !inode->is_base() &&
        get_parent_dir()->inode->is_stray()) {
      r = 0;
      if (inode->snaprealm)
    ...
    if (r < 0) {
      ...
      mdcache->mds->clog->error() << "failed to commit dir " << dirfrag()
                                  << " object," << " errno " << r;
  ...
  for (auto p = dirty_dentries.begin(); !p.end();) {
    ...
    if (dn->linkage.is_primary()) {
      ...
      if (committed_version >= in->get_version()) {
        ...
        dout(15) << " dir " << committed_version << " < inode "
                 << in->get_version() << " still dirty " << *in << dendl;
        ceph_assert(
            in->is_dirty() ||
            in->last <
                CEPH_NOSNAP); // special case for cow snap items (not predirtied)
      }
    }

    // dentry
    if (committed_version >= dn->get_version()) {
      dout(15) << " dir " << committed_version << " >= dn " << dn->get_version()
               << " now clean " << *dn << dendl;
      dn->mark_clean();

      // drop clean null stray dentries immediately
      if (stray && dn->get_num_ref() == 0 && !dn->is_projected() &&
          dn->get_linkage()->is_null())
        remove_dentry(dn);
    } else {
      dout(15) << " dir " << committed_version << " < dn " << dn->get_version()
               << " still dirty " << *dn << dendl;
  ...
  while (it != waiting_for_commit.end()) {
    if (it->first > committed_version) {
      dout(10) << " there are waiters for " << it->first << ", committing again"
               << dendl;
      _commit(it->first, -1);
      break;
    }
  ...
  if (were_waiters && waiting_for_commit.empty())
    auth_unpin(this);
}
...
mds_rank_t
CDir::get_export_pin(bool inherit) const
{
...
bool
CDir::is_exportable(mds_rank_t dest) const
{
...
void
CDir::encode_export(bufferlist& bl)
{
  ENCODE_START(1, 1, bl);
  ceph_assert(!is_projected());
  encode(first, bl);
...
void
CDir::decode_import(bufferlist::const_iterator& blp, LogSegmentRef const& ls)
{
  DECODE_START(1, blp);
  decode(first, blp);
  {
  ...
  if (is_dirty_dft()) {
    if (inode->dirfragtreelock.get_state() != LOCK_MIX &&
      ...
      ls->dirty_dirfrag_dirfragtree.push_back(
          &inode->item_dirty_dirfrag_dirfragtree);
    }
  }
  DECODE_FINISH(blp);
}
...
CDir::can_rep() const
{
  ...
  unsigned mds_num =
      mdcache->mds->get_mds_map()->get_num_mds(MDSMap::STATE_ACTIVE);
 ...
 */
void
CDir::set_dir_auth(const mds_authority_t& a)
{
  dout(10) << "setting dir_auth=" << a << " from " << dir_auth << " on "
           << *this << dendl;
...
CDir::auth_pin(void* by)
{
  ...
  dout(10) << "auth_pin by " << by << " on " << *this << " count now "
           << auth_pins << dendl;
...
CDir::auth_unpin(void* by)
{
  ...
  dout(10) << "auth_unpin by " << by << " on " << *this << " count now "
           << auth_pins << dendl;
  ceph_assert(auth_pins >= 0);
...
void
CDir::adjust_nested_auth_pins(int dirinc, void* by)
{
  ceph_assert(dirinc);
  dir_auth_pins += dirinc;

  dout(15) << __func__ << " " << dirinc << " on " << *this << " by " << by
           << " count now " << auth_pins << "/" << dir_auth_pins << dendl;
  ceph_assert(dir_auth_pins >= 0);
...
CDir::verify_fragstat()
{
  ...
  if (c.nsubdirs != fnode->fragstat.nsubdirs ||
    ...
    dout(0) << "verify_fragstat failed " << fnode->fragstat << " on " << *this
            << dendl;
    dout(0) << "               i count " << c << dendl;
    ceph_abort();
  } else {
    dout(0) << "verify_fragstat ok " << fnode->fragstat << " on " << *this
            << dendl;
  }
}
#endif
...
void
CDir::_walk_tree(std::function<bool(CDir*)> callback)
{
  deque<CDir*> dfq;
  dfq.push_back(this);
...
CDir::freeze_tree()
{
  ...
  _walk_tree([this](CDir* dir) {
    ...
    freeze_tree_state->auth_pins += dir->get_auth_pins() +
...
void
CDir::adjust_freeze_after_rename(CDir* dir)
{
...
bool
CDir::can_auth_pin(int* err_ret) const
{
  int err;
  if (!is_auth()) {
    ...
    dout(20) << __func__ << ": error - fragmenting dir ("
             << (is_freezing_dir() ? "freezing" : "frozen") << ")" << dendl;
...
CDir::maybe_finish_freeze()
{
  ...
  if (freeze_tree_state) {
    if (freeze_tree_state->frozen || freeze_tree_state->auth_pins != 1)
      return;
    ...
    if (!is_subtree_root() && inode->is_frozen()) {
      dout(10) << __func__
               << " !subtree root and frozen inode, waiting for unfreeze on "
               << inode << dendl;
      // retake an auth_pin...
      auth_pin(inode);
 ...
 */
void
CDir::dump(Formatter* f, int flags) const
{
  ceph_assert(f != NULL);
  if (flags & DUMP_PATH) {
  ...
  if (flags & DUMP_STATES) {
    ...
    if (state_test(CDir::STATE_COMPLETE))
      f->dump_string("state", "complete");
    if (state_test(CDir::STATE_FREEZINGTREE))
      f->dump_string("state", "freezingtree");
    if (state_test(CDir::STATE_FROZENTREE))
      f->dump_string("state", "frozentree");
    if (state_test(CDir::STATE_FROZENDIR))
      f->dump_string("state", "frozendir");
    if (state_test(CDir::STATE_FREEZINGDIR))
      f->dump_string("state", "freezingdir");
    if (state_test(CDir::STATE_EXPORTBOUND))
      f->dump_string("state", "exportbound");
    if (state_test(CDir::STATE_IMPORTBOUND))
      f->dump_string("state", "importbound");
    if (state_test(CDir::STATE_BADFRAG))
      f->dump_string("state", "badfrag");
    f->close_section();
  }
...
void
CDir::scrub_initialize(const ScrubHeaderRef& header)
{
  ceph_assert(header);
...
std::string
CDir::get_path(bool trim_path) const
{
  std::string path;
