...
public:
  ...
  // -- state --
  static const unsigned STATE_COMPLETE =
      (1 << 0); // the complete contents are in cache
  static const unsigned STATE_FROZENTREE =
  ...
  static const unsigned STATE_TRACKEDBYOFT =
  ...
  static const unsigned MASK_STATE_IMPORT_KEPT =
      (STATE_IMPORTING | STATE_IMPORTBOUND | STATE_EXPORTBOUND |
       STATE_FROZENTREE | STATE_STICKY | STATE_TRACKEDBYOFT);
  static const unsigned MASK_STATE_EXPORT_KEPT =
      (STATE_EXPORTING | STATE_IMPORTBOUND | STATE_EXPORTBOUND |
       STATE_FROZENTREE | STATE_FROZENDIR | STATE_STICKY | STATE_TRACKEDBYOFT);
  static const unsigned MASK_STATE_FRAGMENT_KEPT =
      (STATE_DIRTY | STATE_EXPORTBOUND | STATE_IMPORTBOUND | STATE_AUXSUBTREE |
       STATE_REJOINUNDEF);
  ...
  static const uint64_t WAIT_COMPLETE =
  ...
  void
  reset_fnode(fnode_const_ptr&& ptr)
  {
    fnode = std::move(ptr);
  }
  ...
  void pop_and_dirty_projected_fnode(
  ...
  dentry_key_map::iterator
  upper_bound(dentry_key_t key)
  {
    return items.upper_bound(key);
  }

  unsigned
  get_num_head_items() const
  {
    return num_head_items;
  }

  unsigned
  get_num_head_null() const
  {
    return num_head_null;
  }

  unsigned
  get_num_snap_items() const
  {
    return num_snap_items;
  }

  unsigned
  get_num_snap_null() const
  {
  ...
  CDentry* add_null_dentry(
      std::string_view dname,
      snapid_t first = 2,
      snapid_t last = CEPH_NOSNAP);
  CDentry* add_primary_dentry(
      std::string_view dname,
      CInode* in,
      mempool::mds_co::string alternate_name,
      snapid_t first = 2,
      snapid_t last = CEPH_NOSNAP);
  CDentry* add_remote_dentry(
      std::string_view dname,
      inodeno_t ino,
      unsigned char d_type,
      mempool::mds_co::string alternate_name,
  ...
  void
  get_dist_spec(std::set<mds_rank_t>& ls, mds_rank_t auth)
  {
    if (is_auth()) {
      list_replicas(ls);
      if (!ls.empty())
  ...
  static void encode_dirstat(
      ceph::buffer::list& bl,
      const session_info_t& info,
      const DirStat& ds);

  void
  _encode_base(ceph::buffer::list& bl)
  {
    ENCODE_START(1, 1, bl);
    encode(first, bl);
    encode(*fnode, bl);
  ...
  void
  _decode_base(ceph::buffer::list::const_iterator& p)
  {
    DECODE_START(1, p);
    decode(first, p);
    {
  ...
  void fetch(
      ...
      snapid_t last,
      MDSContext* c,
      bool ignore_authpinnability = false);

  void
  fetch(MDSContext* c, bool ignore_authpinnability = false)
  {
  ...
  // -- dirtyness --
  version_t
  get_committing_version() const
  {
    return committing_version;
  }

  version_t
  get_committed_version() const
  {
    return committed_version;
  }

  void
  set_committed_version(version_t v)
  {
    committed_version = v;
  }
  ...
  bool
  is_waiting_for_dentry(std::string_view dname, snapid_t snap)
  {
  ...
  void take_dentry_waiting(
      std::string_view dentry,
      snapid_t first,
      snapid_t last,
      std::vector<MDSContext*>& ls);

  void add_waiter(uint64_t mask, MDSContext* c) override;
  void take_waiting(
      uint64_t mask,
      std::vector<MDSContext*>& ls) override; // may include dentry waiters
  ...
  inline size_t
  count_unfreeze_dir_waiters() const
  {
    return count_waiters(WAIT_UNFREEZE);
  }

  std::pair<bool, bool>
  is_freezing_or_frozen_tree() const
  {
  ...
  bool
  is_freezing() const override
  {
    return is_freezing_dir() || is_freezing_tree();
  }

  bool
  is_freezing_tree() const
  {
  ...
  bool
  is_freezeable(bool freezing = false) const
  {
    // no nested auth pins.
  ...
  bool
  is_freezeable_dir(bool freezing = false) const
  {
  ...
  bool
  is_any_freezing_or_frozen_inode() const
  {
    return num_frozen_inodes || !freezing_inodes.empty();
  }

  bool
  is_auth_pinned_by_lock_cache() const
  {
  ...
  snapid_t first = 2;
  mempool::mds_co::compact_map<snapid_t, old_rstat_t>
      dirty_old_rstat; // [value.first,key]
...
protected:
  ...
  void _omap_commit_ops(
      int r,
      int op_prio,
      int64_t metapool,
      version_t version,
      bool _new,
      std::vector<dentry_commit_item>& to_set,
      bufferlist& dfts,
      std::vector<std::string>& to_remove,
      mempool::mds_co::compact_set<mempool::mds_co::string>& _stale);
  void _encode_primary_inode_base(
  ...
  ceph::coarse_mono_time last_popularity_sample =
  ...
  mempool::mds_co::compact_map<
      version_t,
      std::vector<MDSContext*, mempool::mds_co::pool_allocator<MDSContext*>>>
      waiting_for_commit;
...
private:
  ...
  void prepare_old_fragment(
      std::map<string_snap_t, std::vector<MDSContext*>>& dentry_waiters,
      bool replay);
