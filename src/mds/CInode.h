...
public:
  ...
  template <typename... Args>
  static old_inode_map_ptr
  allocate_old_inode_map(Args&&... args)
  {
    static mempool::mds_co::pool_allocator<mempool_old_inode_map> allocator;
    return std::allocate_shared<mempool_old_inode_map>(
        allocator, std::forward<Args>(args)...);
  }

  void
  reset_inode(inode_const_ptr&& ptr)
  {
    inode = std::move(ptr);
  }

  void
  reset_xattrs(xattr_map_const_ptr&& ptr)
  {
    xattrs = std::move(ptr);
  }

  void
  reset_old_inodes(old_inode_map_const_ptr&& ptr)
  {
  ...
  /* Helpers */
  static object_t get_object_name(
      inodeno_t ino,
      frag_t fg,
  ...
  void encode(
      ceph::buffer::list& bl,
      uint64_t features,
      const ceph::buffer::list* snap_blob = NULL) const;
  void decode(
  ...
  fragtree_t
      dirfragtree; // dir frag tree, if any.  always consistent with our dirfrag map.
...
inline void
decode_noshare(
    InodeStoreBase::mempool_xattr_map& xattrs,
...
public:
  ...
  void
  encode_bare(ceph::buffer::list& bl, uint64_t features) const
  {
    InodeStoreBase::encode_bare(bl, features, &snap_blob);
  }

  void
  decode_bare(ceph::buffer::list::const_iterator& bl)
  {
...
public:
  ...
  // misc
  static const unsigned EXPORT_NONCE =
  ...
  bool
  scrub_is_in_progress() const
  {
    return (
        scrub_infop &&
        (scrub_infop->scrub_in_progress || scrub_infop->uninline_in_progress));
  }
  ...
  bool
  has_dirty_remote_dirfrag_scrubbed()
  {
    return dirty_remote_dirfrag_scrubbed;
  }

  void
  mark_dirty_remote_dirfrag_scrubbed()
  {
    dirty_remote_dirfrag_scrubbed = true;
  }

  void
  clear_dirty_remote_dirfrag_scrubbed()
  {
    dirty_remote_dirfrag_scrubbed = false;
  }

  bool
  is_multiversion() const
  {
  ...
  sr_t*
  project_snaprealm(snapid_t snapid = 0)
  {
  ...
  void record_snaprealm_parent_dentry(
      sr_t* new_snap,
      SnapRealm* newparent,
      CDentry* dn,
      bool primary_dn);
  ...
  size_t
  get_num_dirfrags() const
  {
    return dirfrags.size();
  }
  ...
  void
  get_dirfrags(Container& ls) const
  {
    // all dirfrags
  ...
  vino() const
  {
    return vinodeno_t(ino(), last);
  }

  int
  d_type() const
  {
    return IFTODT(get_inode()->mode);
  }
  ...
  is_mdsdir() const
  {
    return MDS_INO_IS_MDSDIR(ino());
  }

  bool
  is_base() const
  {
    return MDS_INO_IS_BASE(ino());
  }

  bool
  is_system() const
  {
    return ino() < MDS_INO_SYSTEM_BASE;
  }

  bool
  is_lost_and_found() const
  {
    return ino() == CEPH_INO_LOST_AND_FOUND;
  }

  bool
  is_normal() const
  {
    return !(is_base() || is_system() || is_stray());
  }

  bool
  is_file() const
  {
    return get_inode()->is_file();
  }

  bool
  is_symlink() const
  {
    return get_inode()->is_symlink();
  }

  bool
  is_dir() const
  {
    return get_inode()->is_dir();
  }
  ...
  bool
  is_any_old_inodes() const
  {
    return old_inodes && !old_inodes->empty();
  }

  const old_inode_map_const_ptr&
  get_old_inodes() const
  {
    return old_inodes;
  }
  ...
  bool
  is_lt(const MDSCacheObject* r) const override
  {
  ...
  bool is_ancestor_of(
      const CInode* other,
      std::unordered_map<CInode const*, bool>* visited = nullptr) const;
  bool is_projected_ancestor_of(const CInode* other) const;

  void make_path_string(
      std::string& s,
      bool projected = false,
      const CDentry* use_parent = NULL,
      int path_comp_count = -1) const;
  void make_trimmed_path_string(
      std::string& s,
      bool projected = false,
      const CDentry* use_parent = NULL,
      int path_comp_count = 10) const;
  void make_path(filepath& s, bool projected = false, int path_comp_count = -1)
      const;
  ...
  void _decode_locks_state_for_replica(
      ceph::buffer::list::const_iterator& p,
      bool is_new);
  void _decode_locks_rejoin(
      ceph::buffer::list::const_iterator& p,
  ...
  bool
  is_any_nonstale_caps()
  {
    return count_nonstale_caps();
  }

  const mempool::mds_co::compact_map<int32_t, int32_t>&
  get_mds_caps_wanted() const
  {
  ...
  const mempool_cap_map&
  get_client_caps() const
  {
    return client_caps;
  }
  ...
  int
  get_client_cap_pending(client_t client) const
  {
    auto client_caps_entry = client_caps.find(client);
    if (client_caps_entry != client_caps.end()) {
  ...
  int
  get_num_caps_notable() const
  {
    return num_caps_notable;
  }

  void adjust_num_caps_notable(int d);

  Capability* add_client_cap(
      client_t client,
      Session* session,
      SnapRealm* conrealm = nullptr,
      bool new_inode = false);
  void remove_client_cap(client_t client);
  void move_to_realm(SnapRealm* realm);

  Capability* reconnect_cap(
      client_t client,
  ...
  int get_caps_allowed_for_client(
      Session* s,
      Capability* cap,
  ...
  // client writeable
  bool
  is_clientwriteable() const
  {
    return state & STATE_CLIENTWRITEABLE;
  }
  ...
  // -- freeze --
  bool
  is_freezing_inode() const
  {
    return state_test(STATE_FREEZING);
  }

  bool
  is_frozen_inode() const
  {
    return state_test(STATE_FROZEN);
  }

  bool
  is_frozen_auth_pin() const
  {
  ...
  void
  remove_primary_parent(CDentry* dn)
  {
  ...
  void
  push_projected_parent(CDentry* dn)
  {
  ...
  bool
  is_ephemeral_rand() const
  {
    return state_test(STATE_RANDEPHEMERALPIN);
  }

  bool
  has_ephemeral_policy() const
  {
  ...
  static void dump_validation_results(
      const validated_data& results,
      ceph::Formatter* f);
  ...
  mempool::mds_co::compact_map<snapid_t, mempool::mds_co::set<client_t>>
...
protected:
  ...
  void
  _encode_file_locks(ceph::buffer::list& bl) const
  {
    using ceph::encode;
  ...
  void
  _decode_file_locks(ceph::buffer::list::const_iterator& p)
  {
    using ceph::decode;
  ...
  mempool::mds_co::list<CDentry*>
      projected_parent; // for in-progress rename, (un)link, etc.
  ...
  mempool::mds_co::compact_map<int32_t, int32_t>
      mds_caps_wanted; // [auth] mds -> caps wanted
...
private:
  ...
  struct projected_const_node {
    ...
    explicit projected_const_node(
        const inode_const_ptr& i,
        const xattr_map_const_ptr& x,
        sr_t* s) :
      inode(i), xattrs(x), snapnode(s)
    {}
  };

  mempool::mds_co::list<projected_const_node>
      projected_nodes; // projected values (only defined while dirty)
  size_t num_projected_srnodes = 0;

  // -- cache infrastructure --
  mempool::mds_co::compact_map<frag_t, CDir*>
      dirfrags; // cached dir fragments under this Inode
