...
public:
  ...
  ceph_seq_t
  issue(unsigned c, bool reval = false)
  {
    if (reval)
      revalidate();
    ...
    if (_pending & ~c) {
      ...
      while (!_revokes.empty() && (_revokes.back().before & ~_pending) == 0)
        _revokes.pop_back();
    } else {
      // no change.
  ...
  ceph_seq_t
  issue_norevoke(unsigned c, bool reval = false)
  {
    if (reval)
      revalidate();
  ...
  utime_t
  get_last_issue_stamp() const
  {
    return last_issue_stamp;
  }

  utime_t
  get_last_revoke_stamp() const
  {
  ...
  void
  set_last_issue_stamp(utime_t t)
  {
    last_issue_stamp = t;
  }

  void
  set_last_revoke_stamp(utime_t t)
  {
    last_revoke_stamp = t;
  }
  ...
  unsigned
  get_num_revoke_warnings() const
  {
    return num_revoke_warnings;
  }

  void
  set_cap_id(uint64_t i)
  {
    cap_id = i;
  }
  ...
  {
    return state & STATE_NEEDSNAPFLUSH;
  }

  void
  mark_needsnapflush()
  {
    state |= STATE_NEEDSNAPFLUSH;
  }
  ...
  is_noinline() const
  {
    return state & STATE_NOINLINE;
  }

  bool
  is_nopoolns() const
  {
    return state & STATE_NOPOOLNS;
  }

  bool
  is_noquota() const
  {
    return state & STATE_NOQUOTA;
  }
  ...
  {
    return Export(
        cap_id, wanted(), issued(), pending(), client_follows, get_last_seq(),
        mseq + 1, last_issue_stamp, state);
  }

  void
  merge(const Export& other, bool auth_cap)
  {
    // issued + pending
  ...
  void
  merge(int otherwanted, int otherissued)
  {
    // issued + pending
  ...
  int
  get_lock_cache_allowed() const
  {
    return lock_cache_allowed;
  }

  void
  set_lock_cache_allowed(int c)
  {
    lock_cache_allowed |= c;
  }

  void
  clear_lock_cache_allowed(int c)
  {
    lock_cache_allowed &= ~c;
  }
