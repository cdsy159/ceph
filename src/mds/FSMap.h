...
struct ClusterInfo {
  ...
  ClusterInfo(
    ...
    client_name(client_name), cluster_name(cluster_name), fs_name(fs_name)
  {}
  ...
  bool
  operator==(const ClusterInfo& cluster_info) const
  {
...
inline std::ostream&
operator<<(std::ostream& out, const ClusterInfo& cluster_info)
{
  out << "{client_name=" << cluster_info.client_name
      << ", cluster_name=" << cluster_info.cluster_name
      << ", fs_name=" << cluster_info.fs_name << "}";
  return out;
}
...
inline std::ostream&
operator<<(std::ostream& out, const Peer& peer)
{
...
struct MirrorInfo {
  ...
  // uuid variant check
  bool
  has_peer(std::string_view uuid) const
  {
  ...
  void
  peer_remove(std::string_view uuid)
  {
    peers.erase(uuid);
  }
...
inline std::ostream&
operator<<(std::ostream& out, const MirrorInfo& mirror_info)
{
...
public:
   ...
   */
  bool
  has_standby_replay(mds_gid_t who) const
  {
  ...
  bool
  is_standby_replay(mds_gid_t who) const
  {
...
private:
  void
  set_fscid(fs_cluster_id_t new_fscid)
  {
    fscid = new_fscid;
  }
...
public:
  ...
  void
  filter(const std::vector<std::string>& allowed)
  {
    if (allowed.empty()) {
      return;
    }
    ...
    erase_if(filesystems, [&](const auto& f) {
      return std::find(
                 ...
                 f.second.mds_map.get_fs_name()) == allowed.end();
    });

    erase_if(mds_roles, [&](const auto& r) {
      return std::find(
                 allowed.begin(), allowed.end(), fs_name_from_gid(r.first)) ==
             allowed.end();
    });
  }

  void
  set_enable_multiple(const bool v)
  {
    enable_multiple = v;
    if (true == v) {
  ...
  bool
  get_enable_multiple() const
  {
    return enable_multiple;
  }

  void
  set_legacy_client_fscid(fs_cluster_id_t fscid)
  {
   ...
   */
  bool
  gid_exists(mds_gid_t gid, const std::vector<std::string>& in = {}) const
  {
    try {
   ...
   */
  bool
  gid_has_rank(mds_gid_t gid) const
  {
   ...
   */
  fs_cluster_id_t
  fscid_from_gid(mds_gid_t gid) const
  {
    if (!gid_exists(gid)) {
  ...
  template <typename T>
  void
  modify_filesystem(fs_cluster_id_t fscid, T&& fn)
  {
    auto& fs = filesystems.at(fscid);
    bool did_update = true;

    if constexpr (
        std::is_convertible_v<std::invoke_result_t<T, Filesystem&>, bool>) {
      did_update = fn(fs);
    } else {
      fn(fs);
    }
  ...
  template <typename T>
  void
  modify_daemon(mds_gid_t who, T&& fn)
  {
   ...
   */
  const mds_info_t&
  get_info_gid(mds_gid_t gid) const
  {
  ...
  std::string_view
  fs_name_from_gid(mds_gid_t gid) const
  {
    auto fscid = mds_roles.at(gid);
  ...
  mds_gid_t
  get_standby_replay(mds_gid_t who) const
  {
    return filesystems.at(mds_roles.at(who)).get_standby_replay(who);
  }

  const Filesystem*
  get_legacy_filesystem() const
  {
   ...
   */
  void
  update_export_targets(mds_gid_t who, const std::set<mds_rank_t>& targets)
  {
  ...
  size_t
  filesystem_count() const
  {
    return filesystems.size();
  }

  bool
  filesystem_exists(fs_cluster_id_t fscid) const
  {
    return filesystems.count(fscid) > 0;
  }

  const Filesystem&
  get_filesystem(fs_cluster_id_t fscid) const
  {
    return filesystems.at(fscid);
  }

  const Filesystem&
  get_filesystem(void) const
  {
  ...
  const Filesystem&
  get_filesystem(mds_gid_t gid) const
  {
  ...
  const auto&
  get_standby_daemons() const
  {
    return standby_daemons;
  }

  const auto&
  get_standby_epochs() const
  {
...
inline std::ostream&
operator<<(std::ostream& out, const FSMap& m)
{
