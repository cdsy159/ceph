...
void
Locker::dispatch(const cref_t<Message>& m)
{
...
void
Locker::send_lock_message(SimpleLock* lock, int msg)
{
...
void
Locker::send_lock_message(SimpleLock* lock, int msg, const bufferlist& data)
{
  for (const auto& it : lock->get_parent()->get_replicas()) {
    if (mds->is_cluster_degraded() &&
        mds->mdsmap->get_state(it.first) < MDSMap::STATE_REJOIN)
  ...
  while (true) {
    ...
    if (!found_locked) {
      if (!t->snaplock.can_rdlock(client)) {
        ...
        t->snaplock.add_waiter(
            SimpleLock::WAIT_RD, new C_MDS_RetryRequest(mdcache, mdr));
        goto failed;
      }
...
struct MarkEventOnDestruct {
  ...
  MarkEventOnDestruct(const MDRequestRef& _mdr, std::string_view _message) :
    mdr(_mdr), message(_message), mark_event(true)
  {}
    ...
    bool skip_quiesce)
{
  ...
  for (size_t i = 0; i < lov.size(); ++i) {
    ...
    if (p.is_xlock()) {
      if ((lock->get_type() == CEPH_LOCK_ISNAP ||
          ...
          mds->is_cluster_degraded() && mdr->is_leader() &&
      ...
      if (is_inode_lock(t)) {
        ...
        default:
          ...
          if (need_quiescelock && (lock->get_cap_shift() > 0)) {
            dout(20) << "need shared quiesce lock for " << p << " on "
                     << SimpleLock::get_lock_type_name(t) << " of " << in
                     << dendl;
      ...
      if (!lock->is_locallock()) {
        ...
        if (need_quiescelock && (lock->get_cap_shift() > 0)) {
          dout(20) << "need shared quiesce lock for " << p << " on "
                   << SimpleLock::get_lock_type_name(t) << " of " << object
                   << dendl;
          need_quiescelock = false;
          CInode* in = static_cast<CInode*>(object);
          lov.add_wrlock(&in->quiescelock, i + 1);
  ...
  for (const auto& p : mustpin) {
    ...
    if (!object->is_auth()) {
      ...
      if (object->is_ambiguous_auth()) {
        ...
        object->add_waiter(
            MDSCacheObject::WAIT_SINGLEAUTH,
    ...
    if (!object->can_auth_pin(&err)) {
      ...
      if (auth_pin_nonblocking) {
        dout(10) << " can't auth_pin (freezing?) " << *object << ", nonblocking"
                 << dendl;
        mdr->aborted = true;
        return false;
      }
      ...
      dout(10) << " can't auth_pin (freezing?), waiting to authpin " << *object
               << dendl;
      if (CDentry* dn = dynamic_cast<CDentry*>(object)) {
        dout(10) << " can't auth_pin dir: " << *dn->get_dir() << dendl;
      }
      object->add_waiter(
          MDSCacheObject::WAIT_UNFREEZE, new C_MDS_RetryRequest(mdcache, mdr));
      ...
      if (mds->is_cluster_degraded() &&
        ...
        if (mdr->more()->waiting_on_peer.empty())
          mds->wait_for_active_peer(
              p.first, new C_MDS_RetryRequest(mdcache, mdr));
        return false;
      }

      auto req = make_message<MMDSPeerRequest>(
          mdr->reqid, mdr->attempt, MMDSPeerRequest::OP_AUTHPIN);
      for (auto& o : p.second) {
  ...
  for (const auto& p : lov) {
    ...
    if (p.is_xlock()) {
      if (mdr->is_xlocked(lock)) {
        dout(10) << " already xlocked " << *lock << " " << *lock->get_parent()
                 << dendl;
        continue;
      }
      ...
      dout(10) << " got xlock on " << *lock << " " << *lock->get_parent()
               << dendl;
      ...
      if (p.is_remote_wrlock()) {
        if (it != mdr->locks.end() && it->is_remote_wrlock()) {
          dout(10) << " already remote_wrlocked " << *lock << " "
      ...
      if (p.is_wrlock()) {
        ...
        dout(10) << " got wrlock on " << *lock << " " << *lock->get_parent()
                 << dendl;
      }
    } else {
      if (mdr->is_rdlocked(lock)) {
        dout(10) << " already rdlocked " << *lock << " " << *lock->get_parent()
                 << dendl;
        continue;
      }
      ...
      if (lock->needs_recover()) {
        if (mds->is_cluster_degraded()) {
          if (!mdr->is_queued_for_replay()) {
            ...
            drop_locks(mdr.get());
            mds->wait_for_cluster_recovered(
                new C_MDS_RetryRequest(mdcache, mdr));
            dout(10) << " rejoin recovering " << *lock << " "
                     << *lock->get_parent() << ", waiting for cluster recovered"
                     << dendl;
            marker.message =
      ...
      dout(10) << " got rdlock on " << *lock << " " << *lock->get_parent()
               << dendl;
    }
  }
...
void
Locker::handle_quiesce_failure(const MDRequestRef& mdr, std::string_view& marker)
{
...
void
Locker::request_drop_remote_locks(const MDRequestRef& mdr)
{
  if (!mdr->has_more())
    return;
  ...
  for (set<mds_rank_t>::iterator p = mdr->more()->peers.begin();
       p != mdr->more()->peers.end(); ++p) {
    auto r = make_message<MMDSPeerRequest>(
        mdr->reqid, mdr->attempt, MMDSPeerRequest::OP_FINISH);

    if (mdr->killed && !mdr->committing) {
      r->mark_abort();
    } else if (
        mdr->more()->srcdn_auth_mds == *p &&
  ...
  for (auto it = mdr->locks.begin(); it != mdr->locks.end();) {
    ...
    if (!lock->is_locallock() && it->is_xlock() &&
...
void
Locker::request_drop_non_rdlocks(const MDRequestRef& mdr)
{
...
void
Locker::request_drop_locks(const MDRequestRef& mdr)
{
  // NB: request_drop_remote_locks must be called before the local drop locks
  //     Otherwise, OP_FINISH won't be called and the authpins will stay on the remote objects
  request_drop_remote_locks(mdr);
...
void
Locker::notify_freeze_waiter(MDSCacheObject* o)
{
  CDir* dir = NULL;
...
void
Locker::set_xlocks_done(MutationImpl* mut, bool skip_dentry)
{
  for (const auto& p : mut->locks) {
    ...
    if (skip_dentry && (p.lock->get_type() == CEPH_LOCK_DN ||
    ...
    bool drop_rdlocks)
{
  ...
  for (set<mds_rank_t>::iterator p = peers.begin(); p != peers.end(); ++p) {
    if (!mds->is_cluster_degraded() ||
      ...
      dout(10) << "_drop_non_rdlocks dropping remote locks on mds." << *p
               << dendl;
      auto peerreq = make_message<MMDSPeerRequest>(
          mut->reqid, mut->attempt, MMDSPeerRequest::OP_DROPLOCKS);
...
void
Locker::cancel_locking(MutationImpl* mut, set<CInode*>* pneed_issue)
{
...
void
Locker::drop_locks(MutationImpl* mut, set<CInode*>* pneed_issue)
{
  // leftover locks
...
void
Locker::drop_non_rdlocks(MutationImpl* mut, set<CInode*>* pneed_issue)
{
  set<CInode*> my_need_issue;
  if (!pneed_issue)
    pneed_issue = &my_need_issue;
...
void
Locker::handle_locks_for_early_reply(MutationImpl* mut)
{
...
void
Locker::drop_lock(MutationImpl* mut, SimpleLock* what)
{
...
void
Locker::drop_locks_for_fragment_unfreeze(MutationImpl* mut)
{
  set<CInode*> need_issue;
...
void
Locker::put_lock_cache(MDLockCache* lock_cache)
{
...
void
Locker::invalidate_lock_cache(MDLockCache* lock_cache)
{
...
void
Locker::eval_lock_caches(Capability* cap)
{
  for (auto p = cap->lock_caches.begin(); !p.end();) {
    ...
    int cap_bit = lock_cache->get_cap_bit();
    if (!(cap->issued() & cap_bit)) {
      dout(20) << __func__ << ": lost " << ccap_string(cap_bit) << " on "
               << *lock_cache << dendl;
...
void
Locker::invalidate_lock_caches(CDir* dir)
{
...
void
Locker::invalidate_lock_caches(SimpleLock* lock)
{
  dout(10) << "invalidate_lock_caches " << *lock << " on "
           << *lock->get_parent() << dendl;
  if (lock->is_cached()) {
    ...
    file_layout_t* dir_layout)
{
  ...
  dout(10) << __func__ << ": for client." << client << "/"
           << ceph_mds_op_name(opcode) << " on " << *diri << dendl;
  ...
  for (auto p = cap->lock_caches.begin(); !p.end(); ++p) {
    if ((*p)->opcode == opcode) {
      dout(10) << " lock cache already exists for " << ceph_mds_op_name(opcode)
               << ", noop" << dendl;
      return;
    }
  }
  ...
  if (!(cap->issued() & cap_bit)) {
    dout(10) << " client cap lacks rights for lock cache: "
             << ccap_string(cap_bit) << dendl;
    return;
  }
  ...
  for (auto& p : mdr->object_states) {
    ...
    if (stat.auth_pinned) {
      if (!p.first->can_auth_pin()) {
        dout(10) << " can't auth_pin(freezing?) lock parent " << *p.first
                 << ", noop" << dendl;
        return;
      }
      if (CInode* in = dynamic_cast<CInode*>(p.first);
          in->is_parent_projected()) {
        CDir* dir = in->get_projected_parent_dir();
        if (!dir->can_auth_pin()) {
          dout(10) << " can't auth_pin(!auth|freezing?) dirfrag " << *dir
                   << ", noop" << dendl;
          return;
        }
      }
  ...
  for (auto dir : dfv) {
    ...
    if (dir->is_any_freezing_or_frozen_inode()) {
      dout(10) << " there is freezing/frozen inode in " << *dir << ", noop"
               << dendl;
      return;
    }
  }
  ...
  for (auto& p : mdr->locks) {
    ...
    if (obj != diri && !ancestors.count(obj))
      continue;
    if (!p.lock->is_stable()) {
      dout(10) << " unstable " << *p.lock << " on " << *obj << ", noop"
               << dendl;
      return;
    }
  }
...
bool
Locker::find_and_attach_lock_cache(const MDRequestRef& mdr, CInode* diri)
{
  if (mdr->lock_cache)
    return true;
  ...
  for (auto p = cap->lock_caches.begin(); !p.end(); ++p) {
    ...
    if (lock_cache->opcode == opcode && lock_cache->attachable()) {
      dout(10) << "found lock cache " << *lock_cache << " on " << *diri
               << dendl;
      mdr->lock_cache = lock_cache;
    ...
    MDSContext::vec* pfinishers)
{
  ...
  if (caps && in->is_head()) {
    in->get_caps_issued(
        &loner_issued, &other_issued, &xlocker_issued, lock->get_cap_shift(),
        lock->get_cap_mask());
    dout(10) << " next state is " << lock->get_state_name(next)
             << " issued/allows loner " << gcap_string(loner_issued) << "/"
             << gcap_string(lock->gcaps_allowed(CAP_LONER, next)) << " xlocker "
             << gcap_string(xlocker_issued) << "/"
             << gcap_string(lock->gcaps_allowed(CAP_XLOCKER, next)) << " other "
             << gcap_string(other_issued) << "/"
             << gcap_string(lock->gcaps_allowed(CAP_ANY, next)) << dendl;
...
#define IS_TRUE_AND_LT_AUTH(x, auth) \
  (x && ((auth && x <= AUTH) || (!auth && x < AUTH)))
  bool auth = lock->get_parent()->is_auth();
  if (!lock->is_gathering() &&
      (IS_TRUE_AND_LT_AUTH(lock->get_sm()->states[next].can_rdlock, auth) ||
       !lock->is_rdlocked()) &&
      (IS_TRUE_AND_LT_AUTH(lock->get_sm()->states[next].can_wrlock, auth) ||
       !lock->is_wrlocked()) &&
      (IS_TRUE_AND_LT_AUTH(lock->get_sm()->states[next].can_xlock, auth) ||
       !lock->is_xlocked()) &&
      (IS_TRUE_AND_LT_AUTH(lock->get_sm()->states[next].can_lease, auth) ||
       !lock->is_leased()) &&
      !(lock->get_parent()->is_auth() &&
        lock->is_flushing()) && // i.e. wait for scatter_writebehind!
      (!caps ||
       ((~lock->gcaps_allowed(CAP_ANY, next) & other_issued) == 0 &&
        (~lock->gcaps_allowed(CAP_LONER, next) & loner_issued) == 0 &&
        (~lock->gcaps_allowed(CAP_XLOCKER, next) & xlocker_issued) == 0)) &&
      lock->get_state() !=
          LOCK_SYNC_MIX2 && // these states need an explicit trigger from the auth mds
      lock->get_state() != LOCK_MIX_SYNC2) {
    dout(7) << "eval_gather finished gather on " << *lock << " on "
            << *lock->get_parent() << dendl;
    ...
    if (!lock->get_parent()->is_auth()) {
      ...
      if (!mds->is_cluster_degraded() ||
        ...
        case LOCK_SYNC_LOCK:
          mds->send_message_mds(
              make_message<MLock>(lock, LOCK_AC_LOCKACK, mds->get_nodeid()),
              auth);
          break;

        case LOCK_MIX_SYNC: {
          auto reply =
              make_message<MLock>(lock, LOCK_AC_SYNCACK, mds->get_nodeid());
        ...
        case LOCK_SYNC_MIX: {
          auto reply =
              make_message<MLock>(lock, LOCK_AC_MIXACK, mds->get_nodeid());
          mds->send_message_mds(reply, auth);
          next = LOCK_SYNC_MIX2;
        } break;
        ...
        case LOCK_MIX_LOCK: {
          ...
          mds->send_message_mds(
              make_message<MLock>(
                  lock, LOCK_AC_LOCKACK, mds->get_nodeid(), data),
              auth);
          (static_cast<ScatterLock*>(lock))->start_flush();
      ...
      if (lock->get_state() == LOCK_MIX_LOCK &&
        ...
        dout(10) << " finished (local) gather for mix->lock, now gathering "
                    "from replicas"
                 << dendl;
      ...
      case LOCK_XLOCKSNAP:
        if (lock->get_sm() == &sm_filelock) {
          ...
          int revoke = ~pending &
                       (loner_issued | other_issued | xlocker_issued);
    ...
    if (pfinishers)
      lock->take_waiting(
          SimpleLock::WAIT_STABLE | SimpleLock::WAIT_WR | SimpleLock::WAIT_RD |
...
bool
Locker::eval(CInode* in, int mask, bool caps_imported)
{
  ...
  if (in->is_auth() && in->is_head()) {
    ...
    if (in->choose_ideal_loner()) {
      dout(10) << "eval set loner: client." << orig_loner << " -> client."
               << in->get_loner() << dendl;
      need_issue = true;
      mask = -1;
    } else if (in->get_wanted_loner() != in->get_loner()) {
      dout(10) << "eval want loner: client." << in->get_wanted_loner()
               << " but failed to set it" << dendl;
      mask = -1;
    }
  }
  ...
  if (in->is_auth() && in->is_head() &&
    ...
    if (in->try_drop_loner()) {
      ...
      if (in->get_wanted_loner() >= 0) {
        dout(10) << "eval end set loner to client." << in->get_wanted_loner()
                 << dendl;
...
void
Locker::try_eval(MDSCacheObject* p, int mask)
{
  // unstable and ambiguous auth?
  if (p->is_ambiguous_auth()) {
    dout(7) << "try_eval ambiguous auth, waiting on " << *p << dendl;
    p->add_waiter(
        MDSCacheObject::WAIT_SINGLEAUTH, new C_Locker_Eval(this, p, mask));
    return;
  }
  ...
  if (p->is_auth() && p->is_frozen()) {
    ...
    p->add_waiter(
        MDSCacheObject::WAIT_UNFREEZE, new C_Locker_Eval(this, p, mask));
    return;
  }
...
void
Locker::try_eval(SimpleLock* lock, bool* pneed_issue)
{
  MDSCacheObject* p = lock->get_parent();

  // unstable and ambiguous auth?
  if (p->is_ambiguous_auth()) {
    dout(7) << "try_eval " << *lock << " ambiguousauth, waiting on " << *p
            << dendl;
    p->add_waiter(
        MDSCacheObject::WAIT_SINGLEAUTH,
        new C_Locker_Eval(this, p, lock->get_type()));
    return;
  }
...
void
Locker::eval_cap_gather(CInode* in, set<CInode*>* issue_set)
{
  bool need_issue = false;
  MDSContext::vec finishers;
...
void
Locker::eval_scatter_gathers(CInode* in)
{
  bool need_issue = false;
  MDSContext::vec finishers;
...
void
Locker::eval(SimpleLock* lock, bool* need_issue)
{
...
bool
Locker::_rdlock_kick(SimpleLock* lock, bool as_anon)
{
  // kick the lock
  if (lock->is_stable()) {
    if (lock->get_parent()->is_auth()) {
      if (lock->get_sm() == &sm_scatterlock) {
        ...
        if (lock->get_state() == LOCK_EXCL && in->get_target_loner() >= 0 &&
      ...
      if (!mds->is_cluster_degraded() ||
        ...
        dout(10) << "requesting rdlock from auth on " << *lock << " on "
                 << *lock->get_parent() << dendl;
        mds->send_message_mds(
            make_message<MLock>(lock, LOCK_AC_REQRDLOCK, mds->get_nodeid()),
            auth);
      }
      return false;
    }
...
bool
Locker::rdlock_try(SimpleLock* lock, client_t client)
{
  dout(7) << "rdlock_try on " << *lock << " on " << *lock->get_parent()
          << dendl;
...
bool
Locker::rdlock_start(SimpleLock* lock, const MDRequestRef& mut, bool as_anon)
{
  ...
  */
  ...
  if (lock->get_parent()->is_auth() && lock->is_stable())
    ...
    wait_on = SimpleLock::
        WAIT_STABLE; // REQRDLOCK is ignored if lock is unstable, so we need to retry.
  dout(7) << "rdlock_start waiting on " << *lock << " on "
          << *lock->get_parent() << dendl;
...
bool
Locker::nudge_log(SimpleLock* lock)
{
  // as with xlockdone, or cap flush
  if (lock->get_parent()->is_auth() && lock->is_unstable_and_locked() &&
      lock->has_any_waiter()) {
    dout(10) << __func__ << " YES " << *lock << " on " << *lock->get_parent()
             << dendl;
    mds->mdlog->flush();
    return true;
  } else {
    dout(20) << __func__ << " NO " << *lock << " on " << *lock->get_parent()
             << dendl;
    return false;
  }
}
...
Locker::rdlock_finish(
    const MutationImpl::lock_iterator& it,
    MutationImpl* mut,
    bool* pneed_issue)
{
  ceph_assert(it->is_rdlock());
  ...
  dout(7) << "rdlock_finish on " << *lock << " on " << *lock->get_parent()
          << dendl;
...
bool
Locker::rdlock_try_set(MutationImpl::LockOpVec& lov, const MDRequestRef& mdr)
{
  ...
  for (const auto& p : lov) {
    ...
    if (!mdr->is_rdlocked(lock) && !rdlock_try(lock, mdr->get_client())) {
      lock->add_waiter(
          SimpleLock::WAIT_STABLE | SimpleLock::WAIT_RD,
          new C_MDS_RetryRequest(mdcache, mdr));
      goto failed;
    }
    lock->get_rdlock();
    mdr->emplace_lock(lock, MutationImpl::LockOp::RDLOCK);
    dout(20) << " got rdlock on " << *lock << " " << *lock->get_parent()
             << dendl;
  }
...
bool
Locker::rdlock_try_set(MutationImpl::LockOpVec& lov, MutationRef& mut)
{
  dout(10) << __func__ << dendl;
  for (const auto& p : lov) {
    auto lock = p.lock;
...
void
Locker::wrlock_force(SimpleLock* lock, MutationRef& mut)
{
  switch (lock->get_type()) {
  ...
  dout(7) << "wrlock_force  on " << *lock << " on " << *lock->get_parent()
          << dendl;
...
bool
Locker::wrlock_try(SimpleLock* lock, const MutationRef& mut, client_t client)
{
...
bool
Locker::wrlock_start(const MutationImpl::LockOp& op, const MDRequestRef& mut)
{
  ...
  dout(10) << "wrlock_start " << *lock << " on " << *lock->get_parent()
           << dendl;

  CInode* in = static_cast<CInode*>(lock->get_parent());
  client_t client = op.is_state_pin() ? lock->get_excl_client()
                                      : mut->get_client();
  ...
  while (1) {
    ...
    if (in->is_auth()) {
      ...
      if (!mds->is_cluster_degraded() ||
          mds->mdsmap->is_clientreplay_or_active_or_stopping(auth)) {
        dout(10) << "requesting scatter from auth on " << *lock << " on "
                 << *lock->get_parent() << dendl;
        mds->send_message_mds(
            make_message<MLock>(lock, LOCK_AC_REQSCATTER, mds->get_nodeid()),
            auth);
      }
      break;
    }
  ...
  dout(7) << "wrlock_start waiting on " << *lock << " on "
          << *lock->get_parent() << dendl;
  lock->add_waiter(
      SimpleLock::WAIT_STABLE, new C_MDS_RetryRequest(mdcache, mut));
  nudge_log(lock);
    ...
    bool* pneed_issue)
{
  ...
  dout(7) << "wrlock_finish on " << *lock << " on " << *lock->get_parent()
          << dendl;
  lock->put_wrlock();
...
void
Locker::remote_wrlock_start(
    SimpleLock* lock,
    mds_rank_t target,
    const MDRequestRef& mut)
{
  dout(7) << "remote_wrlock_start mds." << target << " on " << *lock << " on "
          << *lock->get_parent() << dendl;
  ...
  auto r = make_message<MMDSPeerRequest>(
      mut->reqid, mut->attempt, MMDSPeerRequest::OP_WRLOCK);
    ...
    MutationImpl* mut)
{
  ...
  if (!mds->is_cluster_degraded() ||
    ...
    auto peerreq = make_message<MMDSPeerRequest>(
        mut->reqid, mut->attempt, MMDSPeerRequest::OP_UNWRLOCK);
...
bool
Locker::xlock_start(SimpleLock* lock, const MDRequestRef& mut)
{
  switch (lock->get_type()) {
  case CEPH_LOCK_DVERSION:
  case CEPH_LOCK_IVERSION:
  ...
  dout(7) << "xlock_start on " << *lock << " on " << *lock->get_parent()
          << dendl;
  ...
  if (lock->get_parent()->is_auth()) {
    ...
    while (1) {
      if (mut->locking && // started xlock (not preempt other request)
          ...
          !(lock->get_state() == LOCK_LOCK_XLOCK && // client is not xlocker or
            in &&
            in->issued_caps_need_gather(
                lock))) { // xlocker does not hold shared cap
    ...
    lock->add_waiter(
        SimpleLock::WAIT_WR | SimpleLock::WAIT_STABLE,
    ...
    if (lock->get_parent()->is_ambiguous_auth()) {
      lock->get_parent()->add_waiter(
          MDSCacheObject::WAIT_SINGLEAUTH, new C_MDS_RetryRequest(mdcache, mut));
      return false;
    }
    ...
    auto r = make_message<MMDSPeerRequest>(
        mut->reqid, mut->attempt, MMDSPeerRequest::OP_XLOCK);
    r->set_lock_type(lock->get_type());
    lock->get_parent()->set_object_info(r->get_object_info());
...
void
Locker::_finish_xlock(SimpleLock* lock, client_t xlocker, bool* pneed_issue)
{
  if (lock->get_type() != CEPH_LOCK_DN && lock->get_type() != CEPH_LOCK_ISNAP &&
      lock->get_type() != CEPH_LOCK_IPOLICY && lock->get_num_rdlocks() == 0 &&
      lock->get_num_wrlocks() == 0 && !lock->is_leased() &&
    ...
    bool* pneed_issue)
{
  ...
  if (!lock->get_parent()->is_auth()) {
    ...
    // tell auth
    dout(7) << "xlock_finish releasing remote xlock on " << *lock->get_parent()
            << dendl;
    mds_rank_t auth = lock->get_parent()->authority().first;
    if (!mds->is_cluster_degraded() ||
        mds->mdsmap->get_state(auth) >= MDSMap::STATE_REJOIN) {
      auto peerreq = make_message<MMDSPeerRequest>(
          mut->reqid, mut->attempt, MMDSPeerRequest::OP_UNXLOCK);
      peerreq->set_lock_type(lock->get_type());
      lock->get_parent()->set_object_info(peerreq->get_object_info());
    ...
    // others waiting?
    lock->finish_waiters(
        SimpleLock::WAIT_STABLE | SimpleLock::WAIT_WR | SimpleLock::WAIT_RD, 0);
  } else {
...
void
Locker::xlock_export(const MutationImpl::lock_iterator& it, MutationImpl* mut)
{
  ceph_assert(it->is_xlock());
  SimpleLock* lock = it->lock;
  dout(10) << "xlock_export on " << *lock << " " << *lock->get_parent()
           << dendl;
  ...
  MDSCacheObject* p = lock->get_parent();
  ceph_assert(p->state_test(
      CInode::STATE_AMBIGUOUSAUTH)); // we are exporting this (inode)
...
void
Locker::xlock_import(SimpleLock* lock)
{
  dout(10) << "xlock_import on " << *lock << " " << *lock->get_parent()
           << dendl;
...
version_t
Locker::issue_file_data_version(CInode* in)
{
    ...
    const ref_t<MClientCaps>& ack)
{
  ...
  if (in->is_head()) {
    ...
    if ((flags & UPDATE_SHAREMAX) && in->is_auth() &&
        (in->filelock.gcaps_allowed(CAP_LONER) &
         (CEPH_CAP_GWR | CEPH_CAP_GBUFFER)))
    ...
    SnapRealm* realm)
{
  ...
  if (in->is_auth()) {
    ...
    int allowed =
        get_allowed_caps(in, cap, all_allowed, loner_allowed, xlocker_allowed);
...
void
Locker::maybe_set_subvolume_id(const CInode* inode, ref_t<MClientCaps>& ack)
{
  if (ack && inode) {
    ack->subvolume_id = inode->get_subvolume_id();
    dout(10) << "setting subvolume id " << ack->subvolume_id << " for inode "
             << inode->ino().val << dendl;
  }
}

void
Locker::issue_caps_set(set<CInode*>& inset)
{
    ...
    int& xlocker_allowed)
{
  ...
  if (loner >= 0) {
    dout(7) << "get_allowed_caps loner client." << loner
            ...
            << ", others allowed=" << ccap_string(all_allowed) << " on " << *in
            << dendl;
  } else {
    dout(7) << "get_allowed_caps allowed=" << ccap_string(all_allowed)
            << ", xlocker allowed=" << ccap_string(xlocker_allowed) << " on "
            << *in << dendl;
  }
...
int
Locker::issue_caps(CInode* in, Capability* only_cap)
{
  dout(20) << __func__ << ": " << *in;
  if (only_cap) {
  ...
  *_dout << dendl;
  ...
  for (; it != in->client_caps.end(); ++it) {
    ...
    dout(20) << " client." << it->first << " pending " << ccap_string(pending)
             << " allowed " << ccap_string(allowed) << " wanted "
             << ccap_string(wanted) << dendl;

    if (!(pending & ~allowed)) {
      // skip if suppress or new, and not revocation
      if (cap->is_new() || cap->is_suppress() || cap->is_stale()) {
        dout(20) << "  !revoke and new|suppressed|stale, skipping client."
                 << it->first << dendl;
        continue;
      }
    } else {
      ...
      if (cap->is_stale()) {
        ...
        mds->queue_waiter_front(
            new C_Locker_RevokeStaleCap(this, in, it->first));
        continue;
      }
      ...
      if (!cap->is_valid() && (pending & ~CEPH_CAP_PIN)) {
        ...
        dout(7) << "   sending MClientCaps to client." << it->first << " seq "
                << seq << " re-issue " << ccap_string(pending) << dendl;

        if (mds->logger)
          mds->logger->inc(l_mdss_ceph_cap_op_grant);

        auto m = make_message<MClientCaps>(
            CEPH_CAP_OP_GRANT, in->ino(), in->find_snaprealm()->inode->ino(),
            cap->get_cap_id(), cap->get_last_seq(), pending, wanted, 0,
            cap->get_mseq(), cap->get_last_issue(),
    ...
    if ((pending & ~allowed) || // need to revoke ~allowed caps.
      ...
      dout(7) << "   sending MClientCaps to client." << it->first << " seq "
              << seq << " new pending " << ccap_string(after) << " was "
              << ccap_string(before) << dendl;

      int op = (before & ~after) ? CEPH_CAP_OP_REVOKE : CEPH_CAP_OP_GRANT;
      if (op == CEPH_CAP_OP_REVOKE) {
        if (mds->logger)
          mds->logger->inc(l_mdss_ceph_cap_op_revoke);
        revoking_caps.push_back(&cap->item_revoking_caps);
        auto em = revoking_caps_by_client.emplace(
            cap->get_client(),
      ...
      auto m = make_message<MClientCaps>(
          op, in->ino(), in->find_snaprealm()->inode->ino(), cap->get_cap_id(),
          cap->get_last_seq(), after, wanted, 0, cap->get_mseq(),
...
void
Locker::issue_truncate(CInode* in)
{
  dout(7) << "issue_truncate on " << *in << dendl;

  for (auto& p : in->client_caps) {
    if (mds->logger)
      mds->logger->inc(l_mdss_ceph_cap_op_trunc);
    Capability* cap = &p.second;
    auto m = make_message<MClientCaps>(
        CEPH_CAP_OP_TRUNC, in->ino(), in->find_snaprealm()->inode->ino(),
        cap->get_cap_id(), cap->get_last_seq(), cap->pending(), cap->wanted(),
        0, cap->get_mseq(), cap->get_last_issue(),
        mds->get_osd_epoch_barrier());
    in->encode_cap_message(m, cap);
    mds->send_message_client_counted(m, cap->get_session());
  }
...
void
Locker::revoke_stale_cap(CInode* in, client_t client)
{
  ...
  if (cap->revoking() & CEPH_CAP_ANY_WR) {
    ...
    mds->evict_client(
        client.v, false, g_conf()->mds_session_blocklist_on_timeout, *css,
        nullptr);
    return;
  }
...
bool
Locker::revoke_stale_caps(Session* session)
{
...
void
Locker::resume_stale_caps(Session* session)
{
...
void
Locker::remove_stale_leases(Session* session)
{
...
void
Locker::request_inode_file_caps(CInode* in)
{
  ceph_assert(!in->is_auth());

  int wanted = in->get_caps_wanted() & in->get_caps_allowed_ever() &
               ~CEPH_CAP_PIN;
  if (wanted != in->replica_caps_wanted) {
    ...
    dout(7) << "request_inode_file_caps " << ccap_string(wanted) << " was "
            << ccap_string(in->replica_caps_wanted) << " on " << *in
            << " to mds." << auth << dendl;
    ...
    if (!mds->is_cluster_degraded() ||
        mds->mdsmap->is_clientreplay_or_active_or_stopping(auth))
      mds->send_message_mds(
          make_message<MInodeFileCaps>(in->ino(), in->replica_caps_wanted),
          auth);
  }
}

void
Locker::handle_inode_file_caps(const cref_t<MInodeFileCaps>& m)
{
  ...
  if (mds->logger)
    mds->logger->inc(l_mdss_handle_inode_file_caps);
...
uint64_t
Locker::calc_new_max_size(const CInode::inode_const_ptr& pi, uint64_t size)
{
...
bool
Locker::check_client_ranges(CInode* in, uint64_t size)
{
...
bool
Locker::calc_new_client_ranges(CInode* in, uint64_t size, bool* max_increased)
{
  const auto& latest = in->get_projected_inode();
  uint64_t ms;
  if (latest->has_layout()) {
    ...
    if ((p.second.issued() | p.second.wanted()) & CEPH_CAP_ANY_FILE_WR) {
      ...
      if (it != pi->client_ranges.end() && it->first == p.first) {
        ...
        it = pi->client_ranges.emplace_hint(
            it, std::piecewise_construct, std::forward_as_tuple(p.first),
    ...
    utime_t new_mtime)
{
  ...
  dout(10) << "check_inode_max_size new_ranges " << new_ranges
           << " update_size " << update_size << " on " << *in << dendl;

  if (in->is_frozen()) {
    dout(10) << "check_inode_max_size frozen, waiting on " << *in << dendl;
    in->add_waiter(
        CInode::WAIT_UNFREEZE,
    ...
    if (in->filelock.is_stable()) {
      ...
      if (in->get_target_loner() >= 0 && (wanted & CEPH_CAP_ANY_FILE_WR)) {
        dout(10) << "check_inode_max_size requesting file_excl for wanted caps "
                 << ccap_string(wanted) << " " << *in << dendl;
    ...
    if (!in->filelock.can_wrlock(in->get_loner())) {
      dout(10) << "check_inode_max_size can't wrlock, waiting on " << *in
               << dendl;
      in->filelock.add_waiter(
  ...
  if (update_size) {
    dout(10) << "check_inode_max_size size " << pi.inode->size << " -> "
             << new_size << dendl;
    pi.inode->size = new_size;
    pi.inode->rstat.rbytes = new_size;
    dout(10) << "check_inode_max_size mtime " << pi.inode->mtime << " -> "
             << new_mtime << dendl;
  ...
  mds->mdlog->submit_entry(
      le, new C_Locker_FileUpdate_finish(
              this, in, mut, UPDATE_SHAREMAX, ref_t<MClientCaps>()));
...
void
Locker::share_inode_max_size(CInode* in, Capability* only_cap)
{
  /*
...
void
Locker::adjust_cap_wanted(Capability* cap, int wanted, int issue_seq)
{
  if (ceph_seq_cmp(issue_seq, cap->get_last_issue()) == 0) {
    ...
    dout(10) << " NOT changing wanted " << ccap_string(cap->wanted()) << " -> "
             << ccap_string(wanted) << " (issue_seq " << issue_seq
             << " != last_issue " << cap->get_last_issue() << ")" << dendl;
    return;
  }
  ...
  if (cap->wanted()) {
    if (cur->state_test(CInode::STATE_RECOVERING) &&
        (cap->wanted() & (CEPH_CAP_FILE_RD | CEPH_CAP_FILE_WR))) {
...
void
Locker::snapflush_nudge(CInode* in)
{
...
void
Locker::mark_need_snapflush_inode(CInode* in)
{
  ceph_assert(in->last != CEPH_NOSNAP);
  if (!in->item_to_flush.is_on_list()) {
    ...
    dout(10) << "mark_need_snapflush_inode " << *in << " - added at " << now
             << dendl;
  }
}

bool
Locker::is_revoking_any_caps_from(client_t client)
{
...
void
Locker::_do_null_snapflush(CInode* head_in, client_t client, snapid_t last)
{
  dout(10) << "_do_null_snapflush client." << client << " on " << *head_in
           << dendl;
  for (auto p = head_in->client_need_snapflush.begin();
    ...
    if (clients.count(client)) {
      dout(10) << " doing async NULL snapflush on " << snapid << " from client."
               << client << dendl;
      CInode* sin = mdcache->pick_inode_snap(head_in, snapid - 1);
      ceph_assert(sin);
      ceph_assert(sin->first <= snapid);
      _do_snap_update(
          sin, snapid, 0, sin->first - 1, client, ref_t<MClientCaps>(),
          ref_t<MClientCaps>());
...
bool
Locker::should_defer_client_cap_frozen(CInode* in)
{
  if (in->is_frozen())
    return true;
...
void
Locker::handle_client_caps(const cref_t<MClientCaps>& m)
{
  client_t client = m->get_source().num();
  snapid_t follows = m->get_snap_follows();
  auto op = m->get_op();
  auto dirty = m->get_dirty();
  dout(7) << "handle_client_caps" << " on " << m->get_ino() << " tid "
          << m->get_client_tid() << " follows " << follows << " op "
          << ceph_cap_op_name(op) << " flags 0x" << std::hex << m->flags
          << std::dec << dendl;
  ...
  if (!mds->is_clientreplay() && !mds->is_active() && !mds->is_stopping()) {
    ...
    if ((mds->is_reconnect() ||
         mds->get_want_state() == MDSMap::STATE_RECONNECT) &&
        dirty && m->get_client_tid() > 0 &&
        !session->have_completed_flush(m->get_client_tid())) {
      mdcache->set_reconnected_dirty_caps(
          client, m->get_ino(), dirty, op == CEPH_CAP_OP_FLUSHSNAP);
    }
    mds->wait_for_replay(new C_MDS_RetryMessage(mds, m));
    return;
  }
  ...
  if (mds->logger)
    mds->logger->inc(l_mdss_handle_client_caps);
  if (dirty) {
    if (mds->logger)
      mds->logger->inc(l_mdss_handle_client_caps_dirty);
  }
  ...
  if (m->get_client_tid() > 0 && session &&
    ...
    if (op == CEPH_CAP_OP_FLUSHSNAP) {
      if (mds->logger)
        mds->logger->inc(l_mdss_ceph_cap_op_flushsnap_ack);
      ack = make_message<MClientCaps>(
          CEPH_CAP_OP_FLUSHSNAP_ACK, m->get_ino(), 0, 0, 0, 0, 0, dirty, 0, 0,
          mds->get_osd_epoch_barrier());
      maybe_set_subvolume_id(head_in, ack);
    } else {
      if (mds->logger)
        mds->logger->inc(l_mdss_ceph_cap_op_flush_ack);
      ack = make_message<MClientCaps>(
          CEPH_CAP_OP_FLUSH_ACK, m->get_ino(), 0, m->get_cap_id(), m->get_seq(),
          m->get_caps(), 0, dirty, 0, 0, mds->get_osd_epoch_barrier());
      maybe_set_subvolume_id(head_in, ack);
    }
  ...
  if (!head_in) {
    if (mds->is_clientreplay()) {
      ...
      mdcache->wait_replay_cap_reconnect(
          m->get_ino(), new C_MDS_RetryMessage(mds, m));
      return;
    }
  ...
  if (!cap) {
    dout(7) << "handle_client_caps no cap for client." << client << " on "
            << *head_in << dendl;
    return;
  }
  ceph_assert(cap);
  ...
  if (op == CEPH_CAP_OP_FLUSHSNAP) {
    ...
    dout(10) << "  flushsnap follows " << follows << " -> snap " << snap
             << dendl;
    ...
    if (dirty) {
      ack = make_message<MClientCaps>(
          CEPH_CAP_OP_FLUSHSNAP_ACK, in->ino(), 0, 0, 0, 0, 0, dirty, 0, 0,
          mds->get_osd_epoch_barrier());
      ack->set_snap_follows(follows);
      ack->set_client_tid(m->get_client_tid());
    ...
    if (in == head_in || (head_in->client_need_snapflush.count(snap) &&
                          head_in->client_need_snapflush[snap].count(client))) {
      dout(7) << " flushsnap snap " << snap << " client." << client << " on "
              << *in << dendl;

      // this cap now follows a later snap (i.e. the one initiating this flush, or later)
      if (in == head_in)
        cap->client_follows = snap < CEPH_NOSNAP ? snap
      ...
      dout(7) << " not expecting flushsnap " << snap << " from client."
              << client << " on " << *in << dendl;
      if (ack) {
        if (mds->logger)
          mds->logger->inc(l_mdss_ceph_cap_op_flushsnap_ack);
  ...
  if (cap->get_cap_id() != m->get_cap_id()) {
    ...
    dout(10) << " follows " << follows << " retains "
             << ccap_string(m->get_caps()) << " dirty " << ccap_string(dirty)
             << " on " << *in << dendl;
    ...
    if (!head_in->client_need_snapflush.empty()) {
      if (!(cap->issued() & CEPH_CAP_ANY_FILE_WR) &&
        ...
        dout(10) << " revocation in progress, not making any conclusions about "
                    "null snapflushes"
                 << dendl;
      }
    }
    ...
    if (dirty && in->is_auth()) {
      ...
      ack = make_message<MClientCaps>(
          CEPH_CAP_OP_FLUSH_ACK, in->ino(), 0, cap->get_cap_id(), m->get_seq(),
          m->get_caps(), 0, dirty, 0, cap->get_last_issue(),
          mds->get_osd_epoch_barrier());
      ack->set_client_tid(m->get_client_tid());
      ack->set_oldest_flush_tid(m->get_oldest_flush_tid());
      maybe_set_subvolume_id(head_in, ack);
...
public:
  C_Locker_RetryRequestCapRelease(
      Locker* l,
      client_t c,
      const ceph_mds_request_release& it) :
...
Locker::process_request_cap_release(
    const MDRequestRef& mdr,
    client_t client,
    const ceph_mds_request_release& item,
    std::string_view dname)
{
  ...
  dout(10) << __func__ << " client." << client << " " << ccap_string(caps)
           << " on " << *in << (mdr ? "" : " (DEFERRED, no mdr)") << dendl;

  if (ceph_seq_cmp(mseq, cap->get_mseq()) < 0) {
    dout(7) << " mseq " << mseq << " < " << cap->get_mseq() << ", dropping"
            << dendl;
    return;
  }

  if (cap->get_cap_id() != cap_id) {
    dout(7) << " cap_id " << cap_id << " != " << cap->get_cap_id()
            << ", dropping" << dendl;
    return;
  }
  ...
  if (should_defer_client_cap_frozen(in)) {
    ...
    in->add_waiter(
        CInode::WAIT_UNFREEZE,
        new C_Locker_RetryRequestCapRelease(this, client, item));
    return;
  }

  if (mds->logger)
    mds->logger->inc(l_mdss_process_request_cap_release);
...
void
Locker::kick_issue_caps(CInode* in, client_t client, ceph_seq_t seq)
{
  Capability* cap = in->get_client_cap(client);
  ...
  dout(10) << "kick_issue_caps released at current seq " << seq << ", reissuing"
           << dendl;
  issue_caps(in, cap);
}

void
Locker::kick_cap_releases(const MDRequestRef& mdr)
{
  client_t client = mdr->get_client();
  for (map<vinodeno_t, ceph_seq_t>::iterator p = mdr->cap_releases.begin();
       p != mdr->cap_releases.end(); ++p) {
...
__u32
Locker::get_xattr_total_length(CInode::mempool_xattr_map& xattr)
{
  __u32 total = 0;
...
Locker::_do_snap_update(
    ...
    const ref_t<MClientCaps>& ack)
{
  dout(10) << "_do_snap_update dirty " << ccap_string(dirty) << " follows "
           << follows << " snap " << snap << " on " << *in << dendl;
  ...
  bool xattrs = (dirty & CEPH_CAP_XATTR_EXCL) && m->xattrbl.length() &&
                m->head.xattr_version >
                    in->get_projected_inode()->xattr_version;
  ...
  // xattr
  if (xattrs) {
    dout(7) << " xattrs v" << i->xattr_version << " -> "
            << m->head.xattr_version << " len " << m->xattrbl.length() << dendl;
  ...
  mdcache->predirty_journal_parents(
      mut, &le->metablob, in, 0, PREDIRTY_PRIMARY, 0, follows);
  ...
  mds->mdlog->submit_entry(
      le, new C_Locker_FileUpdate_finish(
              this, in, mut, UPDATE_SNAPFLUSH, ack, client));
}
    ...
    bool* need_flush)
{
  ...
  mds->mdlog->submit_entry(
      le,
      new C_Locker_FileUpdate_finish(this, in, mut, update_flags, ack, client));
...
void
Locker::handle_client_cap_release(const cref_t<MClientCapRelease>& m)
{
  client_t client = m->get_source().num();
  ...
  if (mds->logger)
    mds->logger->inc(l_mdss_handle_client_cap_release);
    ...
    ceph_seq_t seq)
{
  ...
  Capability* cap = in->get_client_cap(client);
  if (!cap) {
    dout(7) << "_do_cap_release no cap for client" << client << " on " << *in
            << dendl;
    return;
  }
  ...
  if (cap->get_cap_id() != cap_id) {
    dout(7) << " capid " << cap_id << " != " << cap->get_cap_id() << ", ignore"
            << dendl;
    return;
  }
  if (ceph_seq_cmp(mseq, cap->get_mseq()) < 0) {
    dout(7) << " mseq " << mseq << " < " << cap->get_mseq() << ", ignore"
            << dendl;
    return;
  }
  if (should_defer_client_cap_frozen(in)) {
...
void
Locker::remove_client_cap(CInode* in, Capability* cap, bool kill)
{
  client_t client = cap->get_client();
  ...
  if (in->is_auth()) {
    ...
    if (in->get_projected_inode()->client_ranges.count(client) &&
        !(in->get_inode()->nlink == 0 &&
          !in->is_any_caps())) { // unless it's unlink + stray
      if (kill)
        in->state_set(CInode::STATE_NEEDSRECOVER);
      else
        check_inode_max_size(in);
...
Locker::caps_tick()
{
  ...
  for (auto p = revoking_caps.begin(); !p.end(); ++p) {
    ...
    dout(20) << __func__ << " age = " << age << " client." << cap->get_client()
             << "." << cap->get_inode()->ino() << dendl;
    if (age <= mds->mdsmap->get_session_timeout()) {
      dout(20) << __func__ << " age below timeout "
               << mds->mdsmap->get_session_timeout() << dendl;
      break;
    } else {
      ++n;
      if (n > MAX_WARN_CAPS) {
        dout(1) << __func__ << " more than " << MAX_WARN_CAPS
                << " caps are late" << "revoking, ignoring subsequent caps"
                << dendl;
        break;
      }
    }
    ...
    if (age > mds->mdsmap->get_session_timeout() *
                  (1 << cap->get_num_revoke_warnings())) {
      cap->inc_num_revoke_warnings();
      CachedStackStringStream css;
      *css << "client." << cap->get_client()
           << " isn't responding to mclientcaps(revoke), ino "
...
void
Locker::handle_client_lease(const cref_t<MClientLease>& m)
{
  ...
  if (!in) {
    dout(7) << "handle_client_lease don't have ino " << m->get_ino() << "."
            << m->get_last() << dendl;
    return;
  }
  CDentry* dn = 0;
  ...
  if (!dn) {
    dout(7) << "handle_client_lease don't have dn " << m->get_ino() << " "
            << m->dname << dendl;
    return;
  }
  dout(10) << " on " << *dn << dendl;
  ...
  ClientLease* l = dn->get_client_lease(client);
  if (!l) {
    dout(7) << "handle_client_lease didn't have lease for client." << client
            << " of " << *dn << dendl;
    return;
  }
  ...
  case CEPH_MDS_LEASE_RELEASE:
    if (l->seq != m->get_seq()) {
      dout(7) << "handle_client_lease release - seq " << l->seq
              << " != provided " << m->get_seq() << dendl;
    } else {
      dout(7) << "handle_client_lease client." << client << " on " << *dn
              << dendl;
      dn->remove_client_lease(l, this);
    }
    break;
    ...
    bufferlist& bl)
{
  ...
  if (mdr->snapid == CEPH_NOSNAP && dn->lock.can_lease(client) &&
    ...
    dout(20) << "issue_client_lease seq " << lstat.seq << " dur "
             << lstat.duration_ms << "ms " << " on " << *dn << dendl;
  } else {
    // null lease
    LeaseStat lstat;
    lstat.mask = 0;
...
void
Locker::revoke_client_leases(SimpleLock* lock)
{
  ...
  for (ClientLease& l : dn->client_leases) {
    ...
    auto lease = make_message<MClientLease>(
        CEPH_MDS_LEASE_REVOKE, l.seq, mask, diri->ino(), diri->first,
        CEPH_NOSNAP, dn->get_name());
...
SimpleLock*
Locker::get_lock(int lock_type, const MDSCacheObjectInfo& info)
{
  switch (lock_type) {
  case CEPH_LOCK_DN: {
...
void
Locker::handle_lock(const cref_t<MLock>& m)
{
  // nobody should be talking to us during recovery.
  ceph_assert(
      mds->is_rejoin() || mds->is_clientreplay() || mds->is_active() ||
      mds->is_stopping());

  SimpleLock* lock = get_lock(m->get_lock_type(), m->get_object_info());
  if (!lock) {
    dout(10) << "don't have object " << m->get_object_info()
             << ", must have trimmed, dropping" << dendl;
    return;
  }
 ...
 * not put references. */
void
Locker::handle_reqrdlock(SimpleLock* lock, const cref_t<MLock>& m)
{
  MDSCacheObject* parent = lock->get_parent();
  if (parent->is_auth() && lock->get_state() != LOCK_SYNC &&
      !parent->is_frozen()) {
    dout(7) << "handle_reqrdlock got rdlock request on " << *lock << " on "
            << *parent << dendl;
    ceph_assert(parent->is_auth()); // replica auth pinned if they're doing this!
    if (lock->is_stable()) {
      simple_sync(lock);
    } else {
      dout(7) << "handle_reqrdlock delaying request until lock is stable"
              << dendl;
      lock->add_waiter(
          SimpleLock::WAIT_STABLE | MDSCacheObject::WAIT_UNFREEZE,
          new C_MDS_RetryMessage(mds, m));
    }
  } else {
    dout(7) << "handle_reqrdlock dropping rdlock request on " << *lock << " on "
            << *parent << dendl;
...
void
Locker::handle_simple_lock(SimpleLock* lock, const cref_t<MLock>& m)
{
  int from = m->get_asker();

  dout(10) << "handle_simple_lock " << *m << " on " << *lock << " "
           << *lock->get_parent() << dendl;
  ...
  case LOCK_AC_LOCKACK:
    ...
    if (lock->is_gathering()) {
      dout(7) << "handle_simple_lock " << *lock << " on " << *lock->get_parent()
              << " from " << from << ", still gathering "
              << lock->get_gather_set() << dendl;
    } else {
      dout(7) << "handle_simple_lock " << *lock << " on " << *lock->get_parent()
              << " from " << from << ", last one" << dendl;
      eval_gather(lock);
    }
    break;
...
void
Locker::simple_eval(SimpleLock* lock, bool* need_issue)
{
  ...
  if (mdcache->is_readonly()) {
    if (lock->get_state() != LOCK_SYNC) {
      dout(10) << "simple_eval read-only FS, syncing " << *lock << " on "
               << *lock->get_parent() << dendl;
  ...
  // -> excl?
  if (lock->get_state() != LOCK_EXCL && in && in->get_target_loner() >= 0 &&
      (wanted & CEPH_CAP_GEXCL)) {
    dout(7) << "simple_eval stable, going to excl " << *lock << " on "
            << *lock->get_parent() << dendl;
  ...
  // stable -> sync?
      ...
      lock->get_state() != LOCK_SYNC && !lock->is_wrlocked() &&
      ((!(wanted & CEPH_CAP_GEXCL) &&
        !lock->is_waiter_for(SimpleLock::WAIT_WR)) ||
       (lock->get_state() == LOCK_EXCL && in && in->get_target_loner() < 0))) {
    dout(7) << "simple_eval stable, syncing " << *lock << " on "
            << *lock->get_parent() << dendl;
    simple_sync(lock, need_issue);
  }
}
...
bool
Locker::simple_sync(SimpleLock* lock, bool* need_issue)
{
  dout(7) << "simple_sync on " << *lock << " on " << *lock->get_parent()
          << dendl;
  ceph_assert(lock->get_parent()->is_auth());
  ceph_assert(lock->is_stable());
...
void
Locker::simple_excl(SimpleLock* lock, bool* need_issue)
{
  dout(7) << "simple_excl on " << *lock << " on " << *lock->get_parent()
          << dendl;
  ceph_assert(lock->get_parent()->is_auth());
  ceph_assert(lock->is_stable());
...
void
Locker::simple_lock(SimpleLock* lock, bool* need_issue)
{
  dout(7) << "simple_lock on " << *lock << " on " << *lock->get_parent()
          << dendl;
  ceph_assert(lock->get_parent()->is_auth());
  ceph_assert(lock->is_stable());
  ceph_assert(lock->get_state() != LOCK_LOCK);
  ...
  if (lock->get_parent()->is_replicated() &&
      lock->get_state() == LOCK_MIX_LOCK && gather) {
    dout(10) << " doing local stage of mix->lock gather before gathering from "
                "replicas"
             << dendl;
  } else {
    ...
    if (lock->get_parent()->is_replicated() &&
        lock->get_sm()->states[old_state].replica_state !=
            LOCK_LOCK) { // replica may already be LOCK
      gather++;
      send_lock_message(lock, LOCK_AC_LOCK);
      lock->init_gather();
    }
  ...
  if (gather) {
    ...
    lock->set_state(LOCK_LOCK);
    lock->finish_waiters(
        ScatterLock::WAIT_XLOCK | ScatterLock::WAIT_WR |
        ScatterLock::WAIT_STABLE);
  }
}

void
Locker::simple_xlock(SimpleLock* lock)
{
  dout(7) << "simple_xlock on " << *lock << " on " << *lock->get_parent()
          << dendl;
  ceph_assert(lock->get_parent()->is_auth());
...
void
Locker::scatter_writebehind(ScatterLock* lock)
{
  CInode* in = static_cast<CInode*>(lock->get_parent());
  dout(10) << "scatter_writebehind " << in->get_inode()->mtime << " on "
           << *lock << " on " << *in << dendl;
...
void
Locker::scatter_writebehind_finish(ScatterLock* lock, MutationRef& mut)
{
  CInode* in = static_cast<CInode*>(lock->get_parent());
  dout(10) << "scatter_writebehind_finish on " << *lock << " on " << *in
           << dendl;
...
void
Locker::scatter_eval(ScatterLock* lock, bool* need_issue)
{
  dout(10) << "scatter_eval " << *lock << " on " << *lock->get_parent()
           << dendl;
  ...
  if (mdcache->is_readonly()) {
    if (lock->get_state() != LOCK_SYNC) {
      dout(10) << "scatter_eval read-only FS, syncing " << *lock << " on "
               << *lock->get_parent() << dendl;
      simple_sync(lock, need_issue);
    }
    return;
  ...
  if (!lock->is_rdlocked() && lock->get_state() != LOCK_MIX &&
      lock->get_scatter_wanted()) {
    dout(10) << "scatter_eval scatter_wanted, bump to mix " << *lock << " on "
             << *lock->get_parent() << dendl;
  ...
  if (!in->has_subtree_or_exporting_dirfrag() || in->is_base()) {
    ...
    if (!lock->is_wrlocked() && lock->get_state() != LOCK_SYNC) {
      dout(10)
          << "scatter_eval no wrlocks|xlocks, not subtree root inode, syncing"
          << dendl;
      simple_sync(lock, need_issue);
    }
  }
 ...
 */
void
Locker::scatter_nudge(ScatterLock* lock, MDSContext* c, bool forcelockchange)
{
  ...
  if (p->is_auth()) {
	...
	*/
        ...
        if (mdcache->is_readonly()) {
          if (lock->get_state() != LOCK_SYNC) {
            dout(10) << "scatter_nudge auth, read-only FS, syncing " << *lock
                     << " on " << *p << dendl;
        ...
        // adjust lock state
        dout(10) << "scatter_nudge auth, scatter/unscattering " << *lock
                 << " on " << *p << dendl;
        switch (lock->get_type()) {
        case CEPH_LOCK_IFILE:
        ...
        dout(10) << "scatter_nudge auth, waiting for stable " << *lock << " on "
                 << *p << dendl;
        if (c)
          lock->add_waiter(SimpleLock::WAIT_STABLE, c);
        return;
    ...
    if (!mds->is_cluster_degraded() ||
        mds->mdsmap->is_clientreplay_or_active_or_stopping(auth)) {
      mds->send_message_mds(
          make_message<MLock>(lock, LOCK_AC_NUDGE, mds->get_nodeid()), auth);
    }
...
Locker::scatter_tick()
{
  ...
  while (!updated_scatterlocks.empty()) {
    ...
    if (!lock->is_dirty()) {
      ...
      dout(10) << " removing from updated_scatterlocks " << *lock << " "
               << *lock->get_parent() << dendl;
      continue;
    }
...
void
Locker::scatter_tempsync(ScatterLock* lock, bool* need_issue)
{
  dout(10) << "scatter_tempsync " << *lock << " on " << *lock->get_parent()
           << dendl;
  ceph_assert(lock->get_parent()->is_auth());
  ceph_assert(lock->is_stable());
...
void
Locker::local_wrlock_grab(LocalLockC* lock, MutationRef& mut)
{
  dout(7) << "local_wrlock_grab  on " << *lock << " on " << *lock->get_parent()
          << dendl;
...
bool
Locker::local_wrlock_start(LocalLockC* lock, const MDRequestRef& mut)
{
  dout(7) << "local_wrlock_start  on " << *lock << " on " << *lock->get_parent()
          << dendl;
...
void
Locker::local_wrlock_finish(
    const MutationImpl::lock_iterator& it,
    MutationImpl* mut)
{
  ceph_assert(it->is_wrlock());
  LocalLockC* lock = static_cast<LocalLockC*>(it->lock);
  dout(7) << "local_wrlock_finish  on " << *lock << " on "
          << *lock->get_parent() << dendl;
  lock->put_wrlock();
  mut->locks.erase(it);
  if (lock->get_num_wrlocks() == 0) {
    ...
    ceph_assert(
        !lock->is_waiter_for(SimpleLock::WAIT_WR) ||
        lock->is_waiter_for(SimpleLock::WAIT_XLOCK));
    lock->finish_waiters(SimpleLock::WAIT_XLOCK);
  }
}

bool
Locker::local_xlock_start(LocalLockC* lock, const MDRequestRef& mut)
{
  dout(7) << "local_xlock_start  on " << *lock << " on " << *lock->get_parent()
          << dendl;
  ...
  if (!lock->can_xlock_local()) {
    lock->add_waiter(
        SimpleLock::WAIT_XLOCK, new C_MDS_RetryRequest(mdcache, mut));
    return false;
  }
...
void
Locker::local_xlock_finish(
    const MutationImpl::lock_iterator& it,
    MutationImpl* mut)
{
  ceph_assert(it->is_xlock());
  LocalLockC* lock = static_cast<LocalLockC*>(it->lock);
  dout(7) << "local_xlock_finish  on " << *lock << " on " << *lock->get_parent()
          << dendl;
  lock->put_xlock();
  mut->locks.erase(it);

  lock->finish_waiters(
      SimpleLock::WAIT_XLOCK | SimpleLock::WAIT_WR | SimpleLock::WAIT_RD);
  ...
  if (lock->get_type() == CEPH_LOCK_IQUIESCE) {
    ...
    eval(
        in,
        CEPH_LOCK_IFILE | CEPH_LOCK_IAUTH | CEPH_LOCK_ILINK | CEPH_LOCK_IXATTR,
        true);
  }
}
...
void
Locker::file_eval(ScatterLock* lock, bool* need_issue)
{
  CInode* in = static_cast<CInode*>(lock->get_parent());
  ...
  dout(7) << "file_eval wanted=" << gcap_string(wanted)
          ...
          << "  filelock=" << *lock << " on " << *lock->get_parent() << dendl;
  ...
  if (mdcache->is_readonly()) {
    if (lock->get_state() != LOCK_SYNC) {
      dout(10) << "file_eval read-only FS, syncing " << *lock << " on "
               << *lock->get_parent() << dendl;
      simple_sync(lock, need_issue);
    }
    return;
  ...
  if (lock->get_state() == LOCK_EXCL) {
    ...
    in->get_caps_issued(
        &loner_issued, &other_issued, &xlocker_issued, CEPH_CAP_SFILE);
    dout(7) << "file_eval loner_issued=" << gcap_string(loner_issued)
            << " other_issued=" << gcap_string(other_issued)
            << " xlocker_issued=" << gcap_string(xlocker_issued) << dendl;
    if (!((loner_wanted | loner_issued) &
          (CEPH_CAP_ANY_FILE_WR >> CEPH_CAP_SFILE)) ||
  ...
  // * -> mixed?
      ...
      lock->get_state() != LOCK_MIX && !lock->is_rdlocked() &&
      //!lock->is_waiter_for(SimpleLock::WAIT_WR) &&
      (lock->get_scatter_wanted() ||
       (in->get_target_loner() < 0 && (wanted & CEPH_CAP_GWR)))) {
    dout(7) << "file_eval stable, bump to mixed " << *lock << " on "
            << *lock->get_parent() << dendl;
    scatter_mix(lock, need_issue);
  }
  ...
  // * -> sync?
    ...
    dout(7) << "file_eval stable, bump to sync " << *lock << " on "
            << *lock->get_parent() << dendl;
    simple_sync(lock, need_issue);
  } else if (in->state_test(CInode::STATE_NEEDSRECOVER)) {
    mds->mdcache->queue_file_recover(in);
...
void
Locker::scatter_mix(ScatterLock* lock, bool* need_issue)
{
  ...
  if (lock->get_state() == LOCK_LOCK) {
    ...
    if (in->is_replicated()) {
      if (lock->get_state() ==
          LOCK_SYNC_MIX) { // for the rest states, replicas are already LOCK
...
void
Locker::file_excl(ScatterLock* lock, bool* need_issue)
{
  CInode* in = static_cast<CInode*>(lock->get_parent());
  ...
  ceph_assert(
      (in->get_loner() >= 0 && in->get_mds_caps_wanted().empty()) ||
      (lock->get_state() ==
       LOCK_XSYN)); // must do xsyn -> excl -> <anything else>
  ...
  if (in->is_replicated() && lock->get_state() != LOCK_LOCK_EXCL &&
      lock->get_state() !=
          LOCK_XSYN_EXCL) { // if we were lock, replicas are already lock.
    send_lock_message(lock, LOCK_AC_LOCK);
    lock->init_gather();
    gather++;
  }
...
void
Locker::file_xsyn(SimpleLock* lock, bool* need_issue)
{
...
void
Locker::file_recover(ScatterLock* lock)
{
  CInode* in = static_cast<CInode*>(lock->get_parent());
  ...
  ceph_assert(
      lock->get_state() ==
      LOCK_PRE_SCAN); // only called from MDCache::start_files_to_recover()
...
// messenger
void
Locker::handle_file_lock(ScatterLock* lock, const cref_t<MLock>& m)
{
  CInode* in = static_cast<CInode*>(lock->get_parent());
  int from = m->get_asker();
  ...
  if (mds->is_rejoin()) {
    if (in->is_rejoining()) {
      dout(7) << "handle_file_lock still rejoining " << *in << ", dropping "
              << *m << dendl;
      return;
    }
  }

  dout(7) << "handle_file_lock a="
          << lock->get_lock_action_name(m->get_action()) << " on " << *lock
          << " from mds." << from << " " << *in << dendl;
  ...
  case LOCK_AC_SYNC:
    ceph_assert(
        lock->get_state() == LOCK_LOCK || lock->get_state() == LOCK_MIX ||
  ...
  case LOCK_AC_MIX:
    ceph_assert(
        lock->get_state() == LOCK_SYNC || lock->get_state() == LOCK_LOCK ||
  ...
  case LOCK_AC_LOCKACK:
    ...
    if (lock->is_gathering()) {
      ...
      dout(7) << "handle_file_lock " << *in << " from " << from << ", last one"
              << dendl;
      eval_gather(lock);
    }
    break;
  ...
  case LOCK_AC_REQSCATTER:
    if (lock->is_stable()) {
       ...
       */
      dout(7) << "handle_file_lock got scatter request on " << *lock << " on "
              << *lock->get_parent() << dendl;
      if (lock->get_state() !=
          LOCK_MIX) // i.e., the reqscatter didn't race with an actual mix/scatter
        scatter_mix(lock);
    } else {
      dout(7) << "handle_file_lock got scatter request, !stable, marking "
                 "scatter_wanted on "
  ...
  case LOCK_AC_REQUNSCATTER:
    if (lock->is_stable()) {
       ...
       *  starvation isn't an issue).
       */
      dout(7) << "handle_file_lock got unscatter request on " << *lock << " on "
              << *lock->get_parent() << dendl;
      if (lock->get_state() ==
          LOCK_MIX) // i.e., the reqscatter didn't race with an actual mix/scatter
  ...
  case LOCK_AC_NUDGE:
    if (!lock->get_parent()->is_auth()) {
      ...
      dout(7) << "handle_file_lock trying nudge on " << *lock << " on "
              << *lock->get_parent() << dendl;
