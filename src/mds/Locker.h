...
public:
  ...
  void eval_gather(
      ...
      bool first = false,
      bool* need_issue = 0,
      std::vector<MDSContext*>* pfinishers = 0);
  ...
  void remote_wrlock_start(
      SimpleLock* lock,
      mds_rank_t target,
      const MDRequestRef& mut);
  void remote_wrlock_finish(
      const MutationImpl::lock_iterator& it,
      MutationImpl* mut);
  ...
  void scatter_eval(
      ScatterLock* lock,
      bool* need_issue); // public for MDCache::adjust_subtree_auth()

  void scatter_tick();
  void scatter_nudge(
      ScatterLock* lock,
      MDSContext* c,
      bool forcelockchange = false);
  ...
  void process_request_cap_release(
      const MDRequestRef& mdr,
      client_t client,
      const ceph_mds_request_release& r,
  ...
  Capability* issue_new_caps(
      CInode* in,
      int mode,
      const MDRequestRef& mdr,
      SnapRealm* conrealm);
  int get_allowed_caps(
      CInode* in,
      Capability* cap,
      int& all_allowed,
      int& loner_allowed,
  ...
  bool calc_new_client_ranges(
      CInode* in,
      uint64_t size,
      bool* max_increased = nullptr);
  bool check_inode_max_size(
      CInode* in,
      bool force_wrlock = false,
      uint64_t newmax = 0,
      uint64_t newsize = 0,
...
protected:
  ...
  bool _need_flush_mdlog(
      CInode* in,
      int wanted_caps,
      bool lock_state_any = false);
  void adjust_cap_wanted(Capability* cap, int wanted, int issue_seq);
  void handle_client_caps(const cref_t<MClientCaps>& m);
  void _update_cap_fields(
      CInode* in,
      int dirty,
      const cref_t<MClientCaps>& m,
      mempool_inode* pi);
  void _do_snap_update(
      CInode* in,
      snapid_t snap,
      int dirty,
      snapid_t follows,
