...
struct MDSCapParser : qi::grammar<Iterator, MDSAuthCaps()> {
    ...
    MDSCapParser::base_type(mdscaps)
  {
    ...
    path %=
        -(spaces >> lit("path") >> lit('=') >> (quoted_path | unquoted_path));
    ...
    match =
        (fs_name >> path >> root_squash >> uid >>
         gidlist)[_val = phoenix::construct<MDSCapMatch>(_1, _2, _3, _4, _5)];
  ...
  if (uid != MDS_AUTH_UID_ANY) {
    ...
    if (!gids.empty()) {
      ...
      if (std::find(gids.begin(), gids.end(), caller_gid) != gids.end())
        ...
        for (auto i = caller_gid_list->begin(); i != caller_gid_list->end();
             ++i) {
...
bool
MDSCapMatch::match_path(string_view target_path) const
{
  string _path = path;
  // drop any tailing /
 ...
 */
bool
MDSAuthCaps::path_capable(string_view inode_path) const
{
    ...
    string_view trimmed_inode_path) const
{
  ldout(g_ceph_context, 10)
      << __func__ << " fs_name " << fs_name << " inode(path /"
      << trimmed_inode_path << " owner " << inode_uid << ":" << inode_gid
      << " mode 0" << std::oct << inode_mode << std::dec << ") by caller "
      << caller_uid << ":"
      << caller_gid
      // << "[" << caller_gid_list << "]";
      << " mask " << mask << " new " << new_uid << ":" << new_gid
      << " cap: " << *this << dendl;
  ...
  for (const auto& grant : grants) {
    ...
    if (grant.match.match(
            fs_name, inode_path, caller_uid, caller_gid, caller_gid_list) &&
      ...
      if (std::find(
              grant.match.gids.begin(), grant.match.gids.end(), caller_gid) !=
          grant.match.gids.end()) {
        gids.push_back(caller_gid);
      }
      if (caller_gid_list) {
        std::set_intersection(
            grant.match.gids.begin(), grant.match.gids.end(),
      ...
      if (mask & MAY_CHGRP) {
        ...
        if (inode_uid != caller_uid ||
            std::find(gids.begin(), gids.end(), new_gid) == gids.end()) {
          continue;
        }
      }
...
bool
MDSAuthCaps::parse(string_view str, ostream* err)
{
  // Special case for legacy caps
  if (str == "allow") {
    grants.clear();
    grants.push_back(
        MDSCapGrant(MDSCapSpec(MDSCapSpec::RWPS), MDSCapMatch(), {}));
    return true;
  }
  ...
  if (r && iter == end) {
    ...
    if (err) {
      if (string(iter, end).find("allow") != string::npos) {
        ...
        *err << "mds capability parse failed, stopped at '" << string(iter, end)
             << "' of '" << str << "'";
      }
    }
    return false;
 ...
 */
bool
MDSAuthCaps::merge_one_cap_grant(MDSCapGrant ng)
{
 ...
 */
bool
MDSAuthCaps::merge(MDSAuthCaps newcaps)
{
...
ostream&
operator<<(ostream& out, const MDSCapMatch& match)
{
...
ostream&
operator<<(ostream& out, const MDSCapSpec& spec)
{
  if (spec.allow_all()) {
    out << "*";
  } else {
...
ostream&
operator<<(ostream& out, const MDSCapGrant& grant)
{
  out << "allow ";
  out << grant.spec;
  out << grant.match;
...
ostream&
operator<<(ostream& out, const MDSAuthCaps& cap)
{
  out << "MDSAuthCaps[";
...
ostream&
operator<<(ostream& out, const MDSCapAuth& auth)
{
  out << "MDSCapAuth(" << auth.match << "readable=" << auth.readable
      << ", writeable=" << auth.writeable << ")";
  return out;
}
