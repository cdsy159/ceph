...
std::string_view
MDSCacheObject::generic_pin_name(int p) const
{
  switch (p) {
  case PIN_REPLICATED:
    return "replicated";
...
void
MDSCacheObject::finish_waiting(waitmask_t mask, int result)
{
...
void
MDSCacheObject::dump(ceph::Formatter* f) const
{
 ...
 */
void
MDSCacheObject::dump_states(ceph::Formatter* f) const
{
  if (state_test(STATE_AUTH))
    f->dump_string("state", "auth");
  if (state_test(STATE_DIRTY))
    f->dump_string("state", "dirty");
  if (state_test(STATE_NOTIFYREF))
    f->dump_string("state", "notifyref");
  if (state_test(STATE_REJOINING))
    f->dump_string("state", "rejoining");
  if (state_test(STATE_REJOINUNDEF))
    f->dump_string("state", "rejoinundef");
}

bool
MDSCacheObject::is_waiter_for(waitmask_t mask)
{
...
void
MDSCacheObject::take_waiting(waitmask_t mask, MDSContext::vec& ls)
{
  if (waiting.empty()) {
    return;
  }
