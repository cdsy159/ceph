...
public:
  ...
  struct ptr_lt {
    bool
    operator()(const MDSCacheObject* l, const MDSCacheObject* r) const
    {
      return l->is_lt(r);
    }
  };
  ...
  const static int PIN_TEMPEXPORTING =
  ...
  const static int STATE_NOTIFYREF =
      (1 << 28); // notify dropping ref drop through _put()
  const static int STATE_REJOINING =
  ...
  virtual std::ostream&
  print_db_line_prefix(std::ostream& out) const
  {
  ...
  unsigned
  state_test(unsigned mask) const
  {
    return (state & mask);
  }

  void
  state_clear(unsigned mask)
  {
    state &= ~mask;
  }

  void
  state_set(unsigned mask)
  {
    state |= mask;
  }

  void
  state_reset(unsigned s)
  {
    state = s;
  }
  ...
  void
  print_pin_set(std::ostream& out) const
  {
#ifdef MDS_REF_SET
  ...
  int
  get_num_auth_pins() const
  {
    return auth_pins;
  }
#ifdef MDS_AUTHPIN_SET
  void
  print_authpin_set(std::ostream& out) const
  {
    out << " (" << auth_pin_set << ")";
  }
#endif
  ...
  is_replicated() const
  {
    return !get_replicas().empty();
  }

  bool
  is_replica(mds_rank_t mds) const
  {
    return get_replicas().count(mds);
  }

  int
  num_replicas() const
  {
    return get_replicas().size();
  }

  unsigned
  add_replica(mds_rank_t mds)
  {
  ...
  void
  add_replica(mds_rank_t mds, unsigned nonce)
  {
    if (get_replicas().empty())
      get(PIN_REPLICATED);
  ...
  unsigned
  get_replica_nonce(mds_rank_t mds)
  {
  ...
  void
  list_replicas(std::set<mds_rank_t>& ls) const
  {
  ...
  unsigned
  get_replica_nonce() const
  {
    return replica_nonce;
  }

  void
  set_replica_nonce(unsigned n)
  {
    replica_nonce = n;
  }
  ...
  inline size_t
  count_waiters(uint64_t mask) const
  {
    return waiting.count(mask);
  }
  ...
  void
  add_waiter(waitmask_t mask, MDSContext* c)
  {
    if (waiting.empty())
      get(PIN_WAITER);
  ...
  void
  finish_waiting(uint64_t mask, int result = 0)
  {
  ...
  virtual void
  set_object_info(MDSCacheObjectInfo& info)
  {
    ceph_abort();
  }

  virtual void
  encode_lock_state(int type, ceph::buffer::list& bl)
  {
    ceph_abort();
  }

  virtual void
  decode_lock_state(int type, const ceph::buffer::list& bl)
  {
    ceph_abort();
  }

  virtual void
  finish_lock_waiters(int type, uint64_t mask, int r = 0)
  {
    ceph_abort();
  }

  virtual void
  add_lock_waiter(int type, uint64_t mask, MDSContext* c)
  {
    ceph_abort();
  }

  virtual bool
  is_lock_waiting(int type, uint64_t mask)
  {
    ceph_abort();
    return false;
  }
  ...
  virtual void
  clear_dirty_scattered(int type)
  {
    ceph_abort();
  }
...
inline std::ostream&
operator<<(std::ostream& out, const mdsco_db_line_prefix& o)
{
