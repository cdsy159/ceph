...
{
  dout(1) << "asok_command: " << command << " " << cmdmap << " (starting...)"
          << dendl;
...
void
MDSDaemon::dump_status(Formatter* f)
{
  ...
  f->dump_string(
      "state", ceph_mds_state_name(
                   mdsmap->get_state_gid(mds_gid_t(monc->get_global_id()))));
  if (mds_rank) {
    std::lock_guard l(mds_lock);
...
MDSDaemon::set_up_admin_socket()
{
  ...
  r = admin_socket->register_command(
      "status", asok_hook, "high-level status of MDS");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "lockup "
      "name=millisecs,type=CephInt,req=true,range=0",
      asok_hook, "sleep with mds_lock held (dev)");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "dump_ops_in_flight", asok_hook, "show the ops currently in flight");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "ops "
  ...
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "dump_blocked_ops", asok_hook, "show the blocked ops currently in flight");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "dump_blocked_ops_count", asok_hook,
      "show the count of blocked ops currently in flight");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "dump_historic_ops", asok_hook, "show recent ops");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "dump_historic_ops_by_duration", asok_hook,
      "show recent ops, sorted by op duration");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "dump_export_states", asok_hook, "dump export states");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      ...
      asok_hook, "scrub an inode and output results");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "scrub start "
      "name=path,type=CephString "
      "name=scrubops,type=CephChoices,strings=force|recursive|repair|scrub_"
      "mdsdir,n=N,req=false "
      "name=tag,type=CephString,req=false",
      asok_hook, "scrub and inode and output results");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "scrub abort", asok_hook, "Abort in progress scrub operations(s)");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "scrub pause", asok_hook, "Pause in progress scrub operations(s)");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "scrub resume", asok_hook, "Resume paused scrub operations(s)");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "scrub status", asok_hook, "Status of scrub operations(s)");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "scrub purge_status "
      "name=tag,type=CephString,req=true",
      asok_hook, "Purge status of scrub tag|all");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "tag path name=path,type=CephString"
      " name=tag,type=CephString",
      asok_hook, "Apply scrub tag recursively");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "flush_path name=path,type=CephString", asok_hook,
  ...
  r = admin_socket->register_command(
      ...
      asok_hook, "migrate a subtree to named MDS");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "dump cache "
      "name=path,type=CephString,req=false "
      "name=timeout,type=CephInt,range=0,req=false",
      asok_hook, "dump metadata cache (optionally to a file)");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "cache drop "
      "name=timeout,type=CephInt,range=0,req=false",
      asok_hook,
      "trim cache and optionally request client to release all caps and flush "
      "the journal");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "lock path"
      ...
      asok_hook, "lock a path");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "cache status", asok_hook, "show cache status");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "quiesce path"
      " name=path,type=CephString,req=true"
      " name=await,type=CephBool,req=false",
      asok_hook, "quiesce a subtree");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "dump tree "
  ...
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "session evict name=filters,type=CephString,n=N,req=true", asok_hook,
      "Evict client session(s) based on a filter");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "client evict name=filters,type=CephString,n=N,req=true", asok_hook,
      "Evict client session(s) based on a filter");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "session kill name=client_id,type=CephString", asok_hook,
  ...
  r = admin_socket->register_command(
      ...
      asok_hook, "Config a CephFS client session");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "client config "
      "name=client_id,type=CephInt,req=true "
  ...
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "damage ls", asok_hook, "List detected metadata damage");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "damage rm "
      "name=damage_id,type=CephInt",
      asok_hook, "Remove a damage table entry");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "osdmap barrier name=target_epoch,type=CephInt", asok_hook,
      "Wait until the MDS has this OSD map epoch");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "flush journal", asok_hook, "Flush the journal to the backing store");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "force_readonly", asok_hook, "Force MDS to read-only mode");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "get subtrees", asok_hook, "Return the subtree map");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "dirfrag split "
      "name=path,type=CephString,req=true "
      ...
      asok_hook, "Fragment directory by path");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "dirfrag merge "
      "name=path,type=CephString,req=true "
      "name=frag,type=CephString,req=true",
      asok_hook, "De-fragment directory by path");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "dirfrag ls "
      "name=path,type=CephString,req=true",
      asok_hook, "List fragments in directory");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "openfiles ls", asok_hook, "List the opening files and their caps");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "dump inode "
      "name=number,type=CephInt,req=true",
      asok_hook, "dump inode by inode number");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "dump dir "
      "name=path,type=CephString,req=true "
      "name=dentry_dump,type=CephBool,req=false",
      asok_hook, "dump directory by path");
  ceph_assert(r == 0);
  ...
  r = admin_socket->register_command(
      ...
      asok_hook, "submit queries to the local QuiesceDbManager");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "heap "
  ...
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "dump stray", asok_hook, "dump stray folder content");
  ceph_assert(r == 0);
}

void
MDSDaemon::clean_up_admin_socket()
{
...
void
MDSDaemon::handle_command(const cref_t<MCommand>& m)
{
  ...
  const bool live_session =
      session->get_state_seq() > 0 && mds_rank &&
...
void
MDSDaemon::handle_mds_map(const cref_t<MMDSMap>& m)
{
  ...
  dout(1) << "Updating MDS map to version " << epoch << " from "
          << m->get_source() << dendl;
  ...
  dout(10) << "map says I am mds." << whoami << "." << incarnation << " state "
           << ceph_mds_state_name(new_state) << dendl;
  ...
  if (whoami == MDS_RANK_NONE) {
    ...
    if (mds_rank == NULL) {
      mds_rank = new MDSRankDispatcher(
          whoami, mds_lock, clog, timer, beacon, mdsmap, messenger, monc, &mgrc,
    ...
    dout(10) << __func__ << ": handling map as rank " << mds_rank->get_nodeid()
             << dendl;
...
void
MDSDaemon::handle_signal(int signum)
{
...
MDSDaemon::ms_dispatch2(const ref_t<Message>& m)
{
...
bool
MDSDaemon::handle_core_message(const cref_t<Message>& m)
{
  switch (m->get_type()) {
...
void
MDSDaemon::ms_handle_connect(Connection* con)
{}

bool
MDSDaemon::ms_handle_reset(Connection* con)
{
...
void
MDSDaemon::ms_handle_remote_reset(Connection* con)
{
  if (con->get_peer_type() != CEPH_ENTITY_TYPE_CLIENT)
    return;
  ...
  dout(5) << "ms_handle_remote_reset on " << con->get_peer_socket_addr()
          << dendl;
  if (beacon.get_want_state() == CEPH_MDS_STATE_DNE)
    return;
...
bool
MDSDaemon::ms_handle_refused(Connection* con)
{
  // do nothing for now
  return false;
}
...
bool
MDSDaemon::parse_caps(const AuthCapsInfo& info, MDSAuthCaps& caps)
{
  caps.clear();
  if (info.allow_all) {
    caps.set_allow_all();
    ...
    try {
      ...
      dout(1) << __func__ << ": cannot decode auth caps buffer of length "
              << info.caps.length() << dendl;
      return false;
    }
    ...
    if (caps.parse(auth_cap_str, cs.get())) {
      ...
      dout(1) << __func__ << ": auth cap parse error: " << cs->strv()
...
bool
MDSDaemon::ms_handle_fast_authentication(Connection* con)
{
...
void
MDSDaemon::ms_handle_accept(Connection* con)
{
  ...
  if (!s) {
    s = new Session(con);
    dout(10) << " new session " << s << " for " << s->info.inst << " con "
             << con << dendl;
  ...
  dout(10) << "ms_handle_accept " << con->get_peer_socket_addr() << " con "
           << con << " session " << s << dendl;
  if (s) {
    if (s->get_connection() != con) {
      dout(10) << " session connection " << s->get_connection() << " -> " << con
               << dendl;
