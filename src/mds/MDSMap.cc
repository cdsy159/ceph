...
MDSMap::get_compat_set_base()
{
  ...
  return CompatSet(
      feature_compat_base, feature_ro_compat_base, feature_incompat_base);
}
...
void
MDSMap::mds_info_t::dump(Formatter* f) const
{
...
void
MDSMap::mds_info_t::dump(std::ostream& o) const
{
  o << "[mds." << name << "{" << rank << ":" << global_id << "}" << " state "
    << ceph_mds_state_name(state) << " seq " << state_seq;
  if (laggy()) {
...
auto
MDSMap::mds_info_t::generate_test_instances() -> std::list<mds_info_t>
{
...
MDSMap::dump(Formatter* f) const
{
  ...
  for (map<mds_rank_t, mds_gid_t>::const_iterator p = up.begin(); p != up.end();
       ++p) {
    char s[14];
  ...
  for (set<mds_rank_t>::const_iterator p = failed.begin(); p != failed.end();
       ++p)
    f->dump_int("mds", *p);
  f->close_section();
  f->open_array_section("damaged");
  for (set<mds_rank_t>::const_iterator p = damaged.begin(); p != damaged.end();
       ++p)
    f->dump_int("mds", *p);
  f->close_section();
  f->open_array_section("stopped");
  for (set<mds_rank_t>::const_iterator p = stopped.begin(); p != stopped.end();
       ++p)
    f->dump_int("mds", *p);
  f->close_section();
...
void
MDSMap::dump_flags_state(Formatter* f) const
{
  ...
  f->dump_bool(
      flag_display.at(CEPH_MDSMAP_ALLOW_MULTIMDS_SNAPS),
      allows_multimds_snaps());
  f->dump_bool(
      flag_display.at(CEPH_MDSMAP_ALLOW_STANDBY_REPLAY),
      allows_standby_replay());
  f->dump_bool(
      flag_display.at(CEPH_MDSMAP_REFUSE_CLIENT_SESSION),
      test_flag(CEPH_MDSMAP_REFUSE_CLIENT_SESSION));
  f->dump_bool(
      flag_display.at(CEPH_MDSMAP_REFUSE_STANDBY_FOR_ANOTHER_FS),
      test_flag(CEPH_MDSMAP_REFUSE_STANDBY_FOR_ANOTHER_FS));
  f->dump_bool(
      flag_display.at(CEPH_MDSMAP_BALANCE_AUTOMATE),
      test_flag(CEPH_MDSMAP_BALANCE_AUTOMATE));
  f->close_section();
}

std::list<MDSMap>
MDSMap::generate_test_instances()
{
  std::list<MDSMap> ls;
  MDSMap m;
  m.max_mds = 1;
...
void
MDSMap::print(ostream& out) const
{
  out << "fs_name\t" << fs_name << "\n";
  out << "epoch\t" << epoch << "\n";
  ...
  out << "required_client_features\t"
      << cephfs_stringify_features(required_client_features) << "\n";
  ...
  out << "qdb_cluster\tleader: " << qdb_cluster_leader
      << " members: " << qdb_cluster_members << std::endl;
...
void
MDSMap::print_summary(Formatter* f, ostream* out) const
{
  ...
  if (f) {
    ...
    *out << "e" << get_epoch() << ": " << up.size() << "/" << in.size() << "/"
         << max_mds << " up";
  }
  ...
  for (map<string, int>::reverse_iterator p = by_state.rbegin();
       p != by_state.rend(); ++p) {
    if (f) {
...
void
MDSMap::print_flags(std::ostream& out) const
{
  if (joinable())
    ...
    list<pair<health_status_t, string>>* detail) const
{
  if (!failed.empty()) {
    ...
    *css << "mds rank" << ((failed.size() > 1) ? "s " : " ") << failed
         << ((failed.size() > 1) ? " have" : " has") << " failed";
  ...
  if (!damaged.empty()) {
    CachedStackStringStream css;
    *css << "mds rank" << ((damaged.size() > 1) ? "s " : " ") << damaged
         << ((damaged.size() > 1) ? " are" : " is") << " damaged";
    summary.push_back(make_pair(HEALTH_ERR, css->str()));
    if (detail) {
  ...
  if (!laggy.empty()) {
    CachedStackStringStream css;
    *css << "mds " << laggy << ((laggy.size() > 1) ? " are" : " is")
         << " laggy";
    summary.push_back(make_pair(HEALTH_WARN, css->str()));
  }

  if (get_max_mds() > 1 && was_snaps_ever_allowed() &&
      !allows_multimds_snaps()) {
    CachedStackStringStream css;
    *css << "multi-active mds while there are snapshots possibly created by "
            "pre-mimic MDS";
    summary.push_back(make_pair(HEALTH_WARN, css->str()));
  }
}
...
void
MDSMap::get_health_checks(health_check_map_t* checks) const
{
  // MDS_DAMAGE
  if (!damaged.empty()) {
    health_check_t& check = checks->get_or_add(
        "MDS_DAMAGE", HEALTH_ERR, "%num% mds daemon%plurals% damaged",
        damaged.size());
  ...
  if (is_degraded()) {
    ...
    *css << "fs " << fs_name << " is degraded";
    ...
    for (mds_rank_t i = mds_rank_t(0); i < get_max_mds(); i++) {
      ...
      const auto& info = mds_info.at(gid);
      CachedStackStringStream css;
      *css << "fs " << fs_name << " mds." << info.name << " at " << info.addrs
           << " rank " << i;
      if (is_resolve(i))
        *css << " is resolving";
      if (is_replay(i))
  ...
  if ((mds_rank_t)get_num_in_mds() < get_max_mds()) {
    health_check_t& check = checks->add(
        ...
        "%num% filesystem%plurals% %isorare% online with fewer MDS than "
        "max_mds",
        1);
    CachedStackStringStream css;
  ...
  if (get_max_mds() > 1 && was_snaps_ever_allowed() &&
    ...
    health_check_t& check = checks->add(
        ...
        "%num% filesystem%plurals% %isorare% multi-active mds with old "
        "snapshots",
        1);
    CachedStackStringStream css;
    *css << "multi-active mds while there are snapshots possibly created by "
...
void
MDSMap::mds_info_t::encode_versioned(bufferlist& bl, uint64_t features) const
{
  __u8 v = 10;
...
void
MDSMap::mds_info_t::encode_unversioned(bufferlist& bl) const
{
  __u8 struct_v = 3;
  using ceph::encode;
  encode(struct_v, bl);
...
void
MDSMap::mds_info_t::decode(bufferlist::const_iterator& bl)
{
...
void
MDSMap::encode(bufferlist& bl, uint64_t features) const
{
...
void
MDSMap::sanitize(const std::function<bool(int64_t pool)>& pool_exists)
{
  ...
  for (auto it = data_pools.begin(); it != data_pools.end();) {
    if (!pool_exists(*it)) {
      dout(0) << "removed non-existant data pool " << *it << " from MDSMap"
              << dendl;
...
void
MDSMap::decode(bufferlist::const_iterator& p)
{
...
MDSMap::DaemonState
MDSMap::get_state_gid(mds_gid_t gid) const noexcept
{
...
MDSMap::DaemonState
MDSMap::get_state(mds_rank_t m) const noexcept
{
  auto it = up.find(m);
  if (it == up.end())
    return STATE_NULL;
...
mds_gid_t
MDSMap::get_gid(mds_rank_t r) const noexcept
{
  return up.at(r);
}

const MDSMap::mds_info_t&
MDSMap::get_info(mds_rank_t m) const noexcept
{
  return mds_info.at(up.at(m));
}

const MDSMap::mds_info_t&
MDSMap::get_info_gid(mds_gid_t gid) const noexcept
{
  return mds_info.at(gid);
}

bool
MDSMap::state_transition_valid(DaemonState prev, DaemonState next)
{
  if (next == prev)
    return true;
  ...
  if (prev == MDSMap::STATE_BOOT) {
    ...
    return next == MDSMap::STATE_STANDBY_REPLAY ||
           next == MDSMap::STATE_REPLAY || next == MDSMap::STATE_CREATING ||
    ...
    derr << __func__ << ": Unknown prev state " << ceph_mds_state_name(prev)
         << "(" << prev << ")" << dendl;
    return false;
  }
}
...
bool
MDSMap::is_data_pool(int64_t poolid) const noexcept
{
...
const MDSMap::mds_info_t&
MDSMap::get_mds_info_gid(mds_gid_t gid) const noexcept
{
  return mds_info.at(gid);
}

const MDSMap::mds_info_t&
MDSMap::get_mds_info(mds_rank_t m) const noexcept
{
...
mds_gid_t
MDSMap::find_mds_gid_by_name(std::string_view s) const
{
  for (const auto& [gid, info] : mds_info) {
    if (info.name == s) {
      return gid;
...
unsigned
MDSMap::get_num_mds(int state) const
{
  unsigned n = 0;
...
void
MDSMap::get_up_mds_set(std::set<mds_rank_t>& s) const
{
...
void
MDSMap::add_data_pool(int64_t poolid)
{
  data_pools.push_back(poolid);
}

int
MDSMap::remove_data_pool(int64_t poolid)
{
  std::vector<int64_t>::iterator p =
      std::find(data_pools.begin(), data_pools.end(), poolid);
  if (p == data_pools.end())
    return -ENOENT;
  data_pools.erase(p);
  return 0;
...
void
MDSMap::get_recovery_mds_set(std::set<mds_rank_t>& s) const
{
  s = failed;
  for (const auto& p : damaged)
    s.insert(p);
...
void
MDSMap::get_mds_set_lower_bound(std::set<mds_rank_t>& s, DaemonState first) const
{
  for (std::map<mds_gid_t, mds_info_t>::const_iterator p = mds_info.begin();
       p != mds_info.end(); ++p)
...
void
MDSMap::get_mds_set(std::set<mds_rank_t>& s, DaemonState state) const
{
  for (std::map<mds_gid_t, mds_info_t>::const_iterator p = mds_info.begin();
       p != mds_info.end(); ++p)
    if (p->second.state == state)
...
mds_gid_t
MDSMap::get_standby_replay(mds_rank_t r) const
{
...
bool
MDSMap::is_followable(mds_rank_t r) const
{
...
bool
MDSMap::is_laggy_gid(mds_gid_t gid) const
{
  auto it = mds_info.find(gid);
...
bool
MDSMap::have_inst(mds_rank_t m) const
{
  return up.count(m);
}

entity_addrvec_t
MDSMap::get_addrs(mds_rank_t m) const
{
  return mds_info.at(up.at(m)).get_addrs();
}

mds_rank_t
MDSMap::get_rank_gid(mds_gid_t gid) const
{
...
mds_gid_t
MDSMap::get_incarnation(mds_rank_t m) const
{
  auto it = up.find(m);
  if (it == up.end())
    return MDS_GID_NONE;
...
int
MDSMap::get_inc_gid(mds_gid_t gid) const
{
...
void
MDSMap::set_min_compat_client(ceph_release_t version)
{
  vector<size_t> bits;
...
void
MDSMap::set_bal_rank_mask(std::string val)
{
  bal_rank_mask = val;
...
{
  if ((type == BAL_RANK_MASK_TYPE_ANY || type == BAL_RANK_MASK_TYPE_ALL) &&
      (val == "-1" || val == "all")) {
    return true;
  }
  if ((type == BAL_RANK_MASK_TYPE_ANY || type == BAL_RANK_MASK_TYPE_NONE) &&
      (val == "0x0" || val == "0")) {
    return true;
  }
  return false;
...
void
MDSMap::update_num_mdss_in_rank_mask_bitset()
{
  int r = -EINVAL;

  if (bal_rank_mask.length() &&
      !check_special_bal_rank_mask(bal_rank_mask, BAL_RANK_MASK_TYPE_ANY)) {
  ...
  if (r == -EINVAL) {
    if (check_special_bal_rank_mask(bal_rank_mask, BAL_RANK_MASK_TYPE_NONE)) {
      dout(10) << "Balancer is disabled with bal_rank_mask " << bal_rank_mask
               << dendl;
      bal_rank_mask_bitset.reset();
      num_mdss_in_rank_mask_bitset = 0;
    } else {
      dout(10) << "Balancer distributes mds workloads to all ranks as "
                  "bal_rank_mask is empty or invalid"
               << dendl;
  ...
  dout(10) << "update num_mdss_in_rank_mask_bitset to "
           << num_mdss_in_rank_mask_bitset << dendl;
}
    ...
    std::ostream& ss) const
{
  ...
  static const unsigned int BITS_PER_ULLONG = sizeof(unsigned long long) *
                                              CHAR_BIT;
  static const unsigned int QUARTETS_PER_ULLONG = BITS_PER_ULLONG /
                                                  BITS_PER_QUARTET;
  unsigned int offset = 0;

  std::transform(
      hex_string.begin(), hex_string.end(), hex_string.begin(), ::tolower);

  if (hex_string.substr(0, 2) == "0x") {
    offset = 2;
  }

  for (unsigned int i = offset; i < hex_string.size();
       i += QUARTETS_PER_ULLONG) {
