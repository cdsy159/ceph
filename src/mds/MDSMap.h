...
static inline const auto MDS_FEATURE_INCOMPAT_BASE =
    CompatSet::Feature(1, "base v0.20");
static inline const auto MDS_FEATURE_INCOMPAT_CLIENTRANGES =
    CompatSet::Feature(2, "client writeable ranges");
static inline const auto MDS_FEATURE_INCOMPAT_FILELAYOUT =
    CompatSet::Feature(3, "default file layouts on dirs");
static inline const auto MDS_FEATURE_INCOMPAT_DIRINODE =
    CompatSet::Feature(4, "dir inode in separate object");
static inline const auto MDS_FEATURE_INCOMPAT_ENCODING =
    CompatSet::Feature(5, "mds uses versioned encoding");
static inline const auto MDS_FEATURE_INCOMPAT_OMAPDIRFRAG =
    CompatSet::Feature(6, "dirfrag is stored in omap");
static inline const auto MDS_FEATURE_INCOMPAT_INLINE =
    CompatSet::Feature(7, "mds uses inline data");
static inline const auto MDS_FEATURE_INCOMPAT_NOANCHOR =
    CompatSet::Feature(8, "no anchor table");
static inline const auto MDS_FEATURE_INCOMPAT_FILE_LAYOUT_V2 =
    CompatSet::Feature(9, "file layout v2");
static inline const auto MDS_FEATURE_INCOMPAT_SNAPREALM_V2 =
    CompatSet::Feature(10, "snaprealm v2");
static inline const auto MDS_FEATURE_INCOMPAT_MINORLOGSEGMENTS =
    CompatSet::Feature(11, "minor log segments");
static inline const auto MDS_FEATURE_INCOMPAT_QUIESCE_SUBVOLUMES =
    CompatSet::Feature(12, "quiesce subvolumes");
...
public:
  ...
  typedef enum {
    ...
    STATE_STANDBY =
        CEPH_MDS_STATE_STANDBY, // up, idle.  waiting for assignment by monitor.

    // States of an MDS rank, and of any MDS daemon holding that rank
    // ==============================================================
    STATE_STANDBY_REPLAY =
        CEPH_MDS_STATE_STANDBY_REPLAY, // up, replaying active node, ready to take over and not serving clients. Note: Up to two MDS hold the rank being replayed.
    STATE_STOPPED =
        CEPH_MDS_STATE_STOPPED, // down, once existed, but no subtrees. empty log.  may not be held by a daemon.

    STATE_CREATING =
        CEPH_MDS_STATE_CREATING, // up, creating MDS instance (new journal, idalloc..).
    STATE_STARTING =
        CEPH_MDS_STATE_STARTING, // up, starting prior stopped MDS instance.

    STATE_REPLAY =
        CEPH_MDS_STATE_REPLAY, // up, starting prior failed instance. scanning journal.
    STATE_RESOLVE =
        CEPH_MDS_STATE_RESOLVE, // up, disambiguating distributed operations (import, rename, etc.)
    STATE_RECONNECT = CEPH_MDS_STATE_RECONNECT, // up, reconnect to clients
    STATE_REJOIN =
        CEPH_MDS_STATE_REJOIN, // up, replayed journal, rejoining distributed cache
    STATE_CLIENTREPLAY = CEPH_MDS_STATE_CLIENTREPLAY, // up, active
    STATE_ACTIVE = CEPH_MDS_STATE_ACTIVE, // up, active
    STATE_STOPPING =
        CEPH_MDS_STATE_STOPPING, // up, exporting metadata (-> standby or out)
  ...
  bool
  get_inline_data_enabled() const
  {
    return inline_data_enabled;
  }

  void
  set_inline_data_enabled(bool enabled)
  {
  ...
  void
  set_session_timeout(uint32_t t)
  {
    session_timeout = t;
  }
  ...
  void
  set_session_autoclose(uint32_t t)
  {
    session_autoclose = t;
  }

  uint64_t
  get_max_filesize() const
  {
    return max_file_size;
  }

  void
  set_max_filesize(uint64_t m)
  {
    max_file_size = m;
  }

  uint64_t
  get_max_xattr_size() const
  {
    return max_xattr_size;
  }

  void
  set_max_xattr_size(uint64_t m)
  {
    max_xattr_size = m;
  }
  ...
  void
  add_required_client_feature(size_t bit)
  {
    required_client_features.insert(bit);
  }

  void
  remove_required_client_feature(size_t bit)
  {
    required_client_features.erase(bit);
  }

  const auto&
  get_required_client_features() const
  {
  ...
  void
  set_fs_name(std::string new_fs_name)
  {
    fs_name = std::move(new_fs_name);
  }
  ...
  bool
  was_snaps_ever_allowed() const
  {
    return ever_allowed_features & CEPH_MDSMAP_ALLOW_SNAPS;
  }
  ...
  {
    clear_flag(CEPH_MDSMAP_ALLOW_STANDBY_REPLAY);
  }

  bool
  allows_standby_replay() const
  {
    return test_flag(CEPH_MDSMAP_ALLOW_STANDBY_REPLAY);
  }

  bool
  was_standby_replay_ever_allowed() const
  {
    return ever_allowed_features & CEPH_MDSMAP_ALLOW_STANDBY_REPLAY;
  }

  void
  set_balance_automate()
  {
  ...
  {
    clear_flag(CEPH_MDSMAP_BALANCE_AUTOMATE);
  }

  bool
  allows_balance_automate() const
  {
    return test_flag(CEPH_MDSMAP_BALANCE_AUTOMATE);
  }

  bool
  was_balance_automate_ever_allowed() const
  {
    return ever_allowed_features & CEPH_MDSMAP_BALANCE_AUTOMATE;
  }
  ...
  {
    clear_flag(CEPH_MDSMAP_ALLOW_MULTIMDS_SNAPS);
  }

  bool
  allows_multimds_snaps() const
  {
    return test_flag(CEPH_MDSMAP_ALLOW_MULTIMDS_SNAPS);
  }

  bool
  joinable() const
  {
    return !test_flag(CEPH_MDSMAP_NOT_JOINABLE);
  }
  ...
  void
  set_modified(utime_t mt)
  {
    modified = mt;
  }

  epoch_t
  get_last_failure() const
  {
    return last_failure;
  }
  ...
  mds_rank_t
  get_old_max_mds() const
  {
    return old_max_mds;
  }

  mds_rank_t
  get_standby_count_wanted(mds_rank_t standby_daemon_count) const
  {
  ...
  void
  set_standby_count_wanted(mds_rank_t n)
  {
    standby_count_wanted = n;
  }
  ...
  unsigned
  get_num_mdss_in_rank_mask_bitset() const
  {
    return num_mdss_in_rank_mask_bitset;
  }
  ...
  const bool check_special_bal_rank_mask(
      std::string val,
      bal_rank_mask_type_t type) const;

  mds_rank_t
  get_tableserver() const
  {
    return tableserver;
  }
  ...
  void
  get_quiesce_db_cluster(
      mds_gid_t& leader,
      std::unordered_set<mds_gid_t>& members) const
  {
  ...
  mds_gid_t
  get_quiesce_db_cluster_leader() const
  {
    return qdb_cluster_leader;
  }

  std::unordered_set<mds_gid_t> const&
  get_quiesce_db_cluster_members() const
  {
    return qdb_cluster_members;
  }

  bool
  update_quiesce_db_cluster(
      mds_gid_t const& leader,
      std::same_as<std::unordered_set<mds_gid_t>> auto&& members)
  {
    ...
    ceph_assert(std::ranges::all_of(members, [this](auto& m) {
      return mds_info.contains(m);
    }));
  ...
  get_data_pools() const
  {
    return data_pools;
  }

  int64_t
  get_first_data_pool() const
  {
    return *data_pools.begin();
  }

  int64_t
  get_metadata_pool() const
  {
  ...
  bool
  pool_in_use(int64_t poolid) const
  {
  ...
  // counts
  unsigned
  get_num_in_mds() const
  {
    return in.size();
  }

  unsigned
  get_num_up_mds() const
  {
    return up.size();
  }
  ...
  int
  get_num_failed_mds() const
  {
    return failed.size();
  }

  unsigned
  get_num_standby_replay_mds() const
  {
    unsigned num = 0;
    for (auto& i : mds_info) {
  ...
  // sets
  void
  get_mds_set(std::set<mds_rank_t>& s) const
  {
    s = in;
  }
  ...
  void
  get_active_mds_set(std::set<mds_rank_t>& s) const
  {
    get_mds_set(s, MDSMap::STATE_ACTIVE);
  }

  void
  get_standby_replay_mds_set(std::set<mds_rank_t>& s) const
  {
    get_mds_set(s, MDSMap::STATE_STANDBY_REPLAY);
  }

  void
  get_failed_mds_set(std::set<mds_rank_t>& s) const
  {
    s = failed;
  }

  void
  get_damaged_mds_set(std::set<mds_rank_t>& s) const
  {
    s = damaged;
  }
   ...
   */
  void
  get_down_mds_set(std::set<mds_rank_t>* s) const
  {
    ceph_assert(s != NULL);
  ...
  void
  get_stopped_mds_set(std::set<mds_rank_t>& s) const
  {
    s = stopped;
  }
  ...
  void get_health(
      std::list<std::pair<health_status_t, std::string>>& summary,
  ...
  // mds states
  bool
  is_down(mds_rank_t m) const
  {
    return up.count(m) == 0;
  }

  bool
  is_up(mds_rank_t m) const
  {
    return up.count(m);
  }

  bool
  is_in(mds_rank_t m) const
  {
    return up.count(m) || failed.count(m);
  }

  bool
  is_out(mds_rank_t m) const
  {
    return !is_in(m);
  }

  bool
  is_failed(mds_rank_t m) const
  {
    return failed.count(m);
  }

  bool
  is_stopped(mds_rank_t m) const
  {
    return stopped.count(m);
  }

  bool
  is_dne(mds_rank_t m) const
  {
    return in.count(m) == 0;
  }

  bool
  is_dne_gid(mds_gid_t gid) const
  {
  ...
  bool
  is_boot(mds_rank_t m) const
  {
    return get_state(m) == STATE_BOOT;
  }

  bool
  is_bootstrapping(mds_rank_t m) const
  {
    return is_creating(m) || is_starting(m) || is_replay(m);
  }

  bool
  is_creating(mds_rank_t m) const
  {
    return get_state(m) == STATE_CREATING;
  }

  bool
  is_starting(mds_rank_t m) const
  {
    return get_state(m) == STATE_STARTING;
  }

  bool
  is_replay(mds_rank_t m) const
  {
    return get_state(m) == STATE_REPLAY;
  }

  bool
  is_resolve(mds_rank_t m) const
  {
    return get_state(m) == STATE_RESOLVE;
  }

  bool
  is_reconnect(mds_rank_t m) const
  {
    return get_state(m) == STATE_RECONNECT;
  }

  bool
  is_rejoin(mds_rank_t m) const
  {
    return get_state(m) == STATE_REJOIN;
  }

  bool
  is_clientreplay(mds_rank_t m) const
  {
    return get_state(m) == STATE_CLIENTREPLAY;
  }

  bool
  is_active(mds_rank_t m) const
  {
    return get_state(m) == STATE_ACTIVE;
  }

  bool
  is_stopping(mds_rank_t m) const
  {
    return get_state(m) == STATE_STOPPING;
  }

  bool
  is_active_or_stopping(mds_rank_t m) const
  {
    return is_active(m) || is_stopping(m);
  }

  bool
  is_clientreplay_or_active_or_stopping(mds_rank_t m) const
  {
  ...
  bool
  has_standby_replay(mds_rank_t r) const
  {
  ...
  is_resolving() const
  {
    return get_num_mds(STATE_RESOLVE) > 0 && get_num_mds(STATE_REPLAY) == 0 &&
  ...
  is_rejoining() const
  {
    ...
    return get_num_mds(STATE_REJOIN) > 0 && get_num_mds(STATE_REPLAY) == 0 &&
...
protected:
  ...
  std::vector<int64_t>
      data_pools; // file data pools available to clients (via an ioctl).  first is the default.
  ...
  mds_rank_t old_max_mds =
      0; /* Value to restore when MDS cluster is marked up */
  mds_rank_t standby_count_wanted = -1;
  std::string
      balancer; /* The name/version of the mantle balancer (i.e. the rados obj name) */
  ...
  uint8_t ever_allowed_features =
      0; //< bitmap of features the cluster has allowed
  uint8_t explicitly_allowed_features =
...
inline std::ostream&
operator<<(std::ostream& out, const MDSMap& m)
{
...
inline std::ostream&
operator<<(std::ostream& o, const MDSMap::mds_info_t& info)
{
  info.dump(o);
  return o;
}
