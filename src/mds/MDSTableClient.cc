...
#undef dout_prefix
#define dout_prefix                                        \
  *_dout << "mds." << mds->get_nodeid() << ".tableclient(" \
...
void
MDSTableClient::handle_request(const cref_t<MMDSTableRequest>& m)
{
  ...
  case TABLESERVER_OP_AGREE:
    if (pending_prepare.count(reqid)) {
      ...
      *pending_prepare[reqid].ptid = tid;
      ...
      dout(10) << "got duplicated agree on " << reqid << " atid " << tid
               << dendl;
      ...
      auto req = make_message<MMDSTableRequest>(
          table, TABLESERVER_OP_ROLLBACK, 0, tid);
  ...
  case TABLESERVER_OP_ACK:
    if (pending_commit.count(tid) &&
      ...
      mds->mdlog->submit_entry(
          new ETableClient(table, TABLESERVER_OP_ACK, tid),
...
void
MDSTableClient::_logged_ack(version_t tid)
{
    ...
    MDSContext* onfinish)
{
  ...
  if (server_ready) {
    // send message
    auto req =
        make_message<MMDSTableRequest>(table, TABLESERVER_OP_PREPARE, reqid);
    req->bl = mutation;
    mds->send_message_mds(req, mds->get_mds_map()->get_tableserver());
  } else
...
void
MDSTableClient::commit(version_t tid, LogSegmentRef const& ls)
{
  ...
  if (server_ready) {
    // send message
    auto req =
        make_message<MMDSTableRequest>(table, TABLESERVER_OP_COMMIT, 0, tid);
    mds->send_message_mds(req, mds->get_mds_map()->get_tableserver());
  } else
    dout(10) << "tableserver is not ready yet, deferring request" << dendl;
}
...
void
MDSTableClient::got_journaled_agree(version_t tid, LogSegmentRef const& ls)
{
...
void
MDSTableClient::got_journaled_ack(version_t tid)
{
...
MDSTableClient::resend_commits()
{
  for (auto p = pending_commit.begin(); p != pending_commit.end(); ++p) {
    ...
    auto req = make_message<MMDSTableRequest>(
        table, TABLESERVER_OP_COMMIT, 0, p->first);
    mds->send_message_mds(req, mds->get_mds_map()->get_tableserver());
  }
}
...
MDSTableClient::resend_prepares()
{
  ...
  for (auto p = pending_prepare.begin(); p != pending_prepare.end(); ++p) {
    ...
    auto req =
        make_message<MMDSTableRequest>(table, TABLESERVER_OP_PREPARE, p->first);
...
void
MDSTableClient::handle_mds_failure(mds_rank_t who)
{
