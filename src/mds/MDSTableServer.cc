...
#undef dout_prefix
#define dout_prefix                                                       \
  *_dout << "mds." << rank << ".tableserver(" << get_mdstable_name(table) \
         << ") "
...
void
MDSTableServer::_note_prepare(mds_rank_t mds, uint64_t reqid, bool replay)
{
  version++;
  if (replay)
...
void
MDSTableServer::_note_commit(uint64_t tid, bool replay)
{
  version++;
  if (replay)
    projected_version = version;
...
void
MDSTableServer::_note_rollback(uint64_t tid, bool replay)
{
  version++;
  if (replay)
    projected_version = version;
...
void
MDSTableServer::_note_server_update(bufferlist& bl, bool replay)
{
  version++;
  if (replay)
    projected_version = version;
...
void
MDSTableServer::handle_request(const cref_t<MMDSTableRequest>& req)
{
  ...
  case TABLESERVER_OP_NOTIFY_ACK:
    return handle_notify_ack(req);
  default:
    ceph_abort_msg("unrecognized mds_table_server request op");
  }
}
...
// prepare
void
MDSTableServer::handle_prepare(const cref_t<MMDSTableRequest>& req)
{
  ...
  ETableServer* le = new ETableServer(
      table, TABLESERVER_OP_PREPARE, req->reqid, from, projected_version,
      projected_version);
  le->mutation = req->bl;
...
void
MDSTableServer::_prepare_logged(
    const cref_t<MMDSTableRequest>& req,
    version_t tid)
{
  ...
  auto reply = make_message<MMDSTableRequest>(
      table, TABLESERVER_OP_AGREE, req->reqid, tid);
...
void
MDSTableServer::handle_notify_ack(const cref_t<MMDSTableRequest>& m)
{
  ...
  if (p != pending_notifies.end()) {
    if (p->second.notify_ack_gather.erase(from)) {
      ...
      dout(0) << "got unexpected notify ack for tid " << tid << " from mds."
              << from << dendl;
    }
  } else {
...
// commit
void
MDSTableServer::handle_commit(const cref_t<MMDSTableRequest>& req)
{
  ...
  if (pending_for_mds.count(tid)) {
    ...
    mds->mdlog->submit_entry(
        new ETableServer(
            table, TABLESERVER_OP_COMMIT, 0, MDS_RANK_NONE, tid,
            projected_version),
    ...
    auto reply = make_message<MMDSTableRequest>(
        table, TABLESERVER_OP_ACK, req->reqid, tid);
...
void
MDSTableServer::_commit_logged(const cref_t<MMDSTableRequest>& req)
{
  ...
  auto reply = make_message<MMDSTableRequest>(
      table, TABLESERVER_OP_ACK, req->reqid, req->get_tid());
...
// ROLLBACK
void
MDSTableServer::handle_rollback(const cref_t<MMDSTableRequest>& req)
{
  ...
  mds->mdlog->submit_entry(
      new ETableServer(
          table, TABLESERVER_OP_ROLLBACK, 0, MDS_RANK_NONE, tid,
          projected_version),
      new C_Rollback(this, req));
}

void
MDSTableServer::_rollback_logged(const cref_t<MMDSTableRequest>& req)
{
...
void
MDSTableServer::do_server_update(bufferlist& bl)
{
  ...
  ETableServer* le = new ETableServer(
      table, TABLESERVER_OP_SERVER_UPDATE, 0, MDS_RANK_NONE, 0,
      projected_version);
  le->mutation = bl;
...
void
MDSTableServer::_server_update_logged(bufferlist& bl)
{
...
void
MDSTableServer::_do_server_recovery()
{
  ...
  for (auto p : pending_for_mds) {
    ...
    auto reply = make_message<MMDSTableRequest>(
        table, TABLESERVER_OP_AGREE, p.second.reqid, tid);
  ...
  for (auto p : active_clients) {
    auto reply = make_message<MMDSTableRequest>(
        table, TABLESERVER_OP_SERVER_READY, next_reqids[p]);
...
void
MDSTableServer::encode_state(bufferlist& bl) const
{
...
void
MDSTableServer::decode_state(bufferlist::const_iterator& bl)
{
...
void
MDSTableServer::finish_recovery(set<mds_rank_t>& active)
{
...
void
MDSTableServer::handle_mds_recovery(mds_rank_t who)
{
  ...
  for (auto p = pending_for_mds.begin(); p != pending_for_mds.end(); ++p) {
    ...
    auto reply = make_message<MMDSTableRequest>(
        table, TABLESERVER_OP_AGREE, p->second.reqid, p->second.tid);
    _get_reply_buffer(p->second.tid, &reply->bl);
    mds->send_message_mds(reply, who);
  }

  auto reply = make_message<MMDSTableRequest>(
      table, TABLESERVER_OP_SERVER_READY, next_reqid);
  mds->send_message_mds(reply, who);
}

void
MDSTableServer::handle_mds_failure_or_stop(mds_rank_t who)
{
