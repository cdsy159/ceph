...
MetricAggregator::MetricAggregator(
    ...
    MDSRank* mds,
    MgrClient* mgrc) :
  Dispatcher(cct), m_cct(cct), mds(mds), mgrc(mgrc), mds_pinger(mds)
{}

void
MetricAggregator::ping_all_active_ranks()
{
  dout(10) << ": pinging " << active_rank_addrs.size() << " active mds(s)"
           << dendl;
...
MetricAggregator::init()
{
  ...
  std::string labels = ceph::perf_counters::key_create(
      "mds_client_metrics", {{"fs_name", mds->mdsmap->get_fs_name()},
                             {"id", stringify(mds->get_global_id())}});
  PerfCountersBuilder plb(
      m_cct, labels, l_mds_client_metrics_start, l_mds_client_metrics_last);
  plb.add_u64(
      l_mds_client_metrics_num_clients, "num_clients",
      "Number of client sessions", "mcli", PerfCountersBuilder::PRIO_CRITICAL);
  ...
  pinger = std::thread([this]() {
    ...
    while (!stopping) {
      ...
      locker.unlock();
      double timo =
          g_conf().get_val<std::chrono::seconds>("mds_ping_interval").count();
      sleep(timo);
      locker.lock();
    }
  });
  ...
  subv_window_sec =
      g_conf()
          .get_val<std::chrono::seconds>("subv_metrics_window_interval")
          .count();
  return 0;
}
...
MetricAggregator::ms_dispatch2(const ref_t<Message>& m)
{
...
void
MetricAggregator::refresh_subvolume_metrics_for_rank(
    mds_rank_t rank,
    const std::vector<SubvolumeMetric>& metrics)
{
  for (const auto& m : metrics) {
    ...
    if (!subvolume_perf_counters.contains(m.subvolume_path)) {
      ...
      PerfCountersBuilder plb(
          m_cct, labels, l_subvolume_metrics_first, l_subvolume_metrics_last);
      plb.add_u64(
          l_subvolume_metrics_read_iops, "avg_read_iops", "Average read IOPS",
          "rops", PerfCountersBuilder::PRIO_CRITICAL);
      plb.add_u64(
          l_subvolume_metrics_read_tp_Bps, "avg_read_tp_Bps",
          "Average read throughput (Bps)", "rbps",
          PerfCountersBuilder::PRIO_CRITICAL);
      plb.add_u64(
          l_subvolume_metrics_avg_read_latency, "avg_read_lat_msec",
          "Average read latency (ms)", "rlav",
          PerfCountersBuilder::PRIO_CRITICAL);
      plb.add_u64(
          l_subvolume_metrics_write_iops, "avg_write_iops",
          "Average write IOPS", "wops", PerfCountersBuilder::PRIO_CRITICAL);
      plb.add_u64(
          l_subvolume_metrics_write_tp_Bps, "avg_write_tp_Bps",
          "Average write throughput (Bps)", "wbps",
          PerfCountersBuilder::PRIO_CRITICAL);
      plb.add_u64(
          l_subvolume_metrics_avg_write_latency, "avg_write_lat_msec",
          "Average write latency (ms)", "wlav",
          PerfCountersBuilder::PRIO_CRITICAL);
      plb.add_u64(
          l_subvolume_metrics_quota_bytes, "quota_bytes",
          "Configured quota (bytes) for the subvolume", nullptr,
          PerfCountersBuilder::PRIO_USEFUL, UNIT_BYTES);
      plb.add_u64(
          l_subvolume_metrics_used_bytes, "used_bytes",
      ...
      subvolume_aggregated_metrics.try_emplace(
          m.subvolume_path, subv_window_sec);
    }
  ...
  for (auto it = subvolume_aggregated_metrics.begin();
       it != subvolume_aggregated_metrics.end();) {
    const std::string& path = it->first;
    auto& tracker = it->second;
    tracker.update();

    if (tracker.is_empty()) {
      dout(10) << "Removing stale subv_metric for path=" << path
               << ", window size:=" << subv_window_sec << dendl;
      ...
      for (auto& [query, perf_key_map] : query_metrics_map) {
        ...
        auto sub_key_func_cleanup =
            [this, &path](
                const MDSPerfMetricSubKeyDescriptor& desc,
                MDSPerfMetricSubKey* sub_key) {
              if (desc.type == MDSPerfMetricSubKeyType::SUBVOLUME_PATH) {
                std::smatch match;
                if (std::regex_search(path, match, desc.regex) &&
                    match.size() > 1) {
        ...
        if (query.get_key(sub_key_func_cleanup, &key)) {
          if (perf_key_map.erase(key)) {
            dout(15) << __func__
                     << ": Removed PerfQuery entry for subv_metric=" << path
                     << dendl;
          }
        }
      }
      ...
      aggr_metric.time_window_last_dur_sec =
          tracker.get_current_window_duration_sec();
      ...
      aggr_metric.read_iops = total_read_ops /
                              aggr_metric.time_window_last_dur_sec;
      aggr_metric.write_iops = total_write_ops /
                               aggr_metric.time_window_last_dur_sec;
      aggr_metric.read_tpBs = total_read_bytes /
                              aggr_metric.time_window_last_dur_sec;
      aggr_metric.write_tBps = total_write_bytes /
                               aggr_metric.time_window_last_dur_sec;

      aggr_metric.avg_read_latency =
          (total_read_ops > 0)
              ? (weighted_read_latency_sum / total_read_ops) / 1000
              : 0;
      aggr_metric.avg_write_latency =
          (total_write_ops > 0)
      ...
      counter->set(
          l_subvolume_metrics_avg_read_latency, aggr_metric.avg_read_latency);
      ...
      counter->set(
          l_subvolume_metrics_avg_write_latency, aggr_metric.avg_write_latency);
      ...
      for (auto& [query, perf_key_map] : query_metrics_map) {
        ...
        query.update_counters(
            [&](const MDSPerformanceCounterDescriptor& desc,
...
void
MetricAggregator::refresh_metrics_for_rank(
    const entity_inst_t& client,
    mds_rank_t rank,
    const Metrics& metrics)
{
  dout(20) << ": client=" << client << ", rank=" << rank
           << ", metrics=" << metrics << dendl;
  ...
  if (ins) {
    ...
    std::string labels = ceph::perf_counters::key_create(
        "mds_client_metrics-" + std::string(mds->mdsmap->get_fs_name()),
        {{"client", stringify(client.name)}, {"rank", stringify(rank)}});
    PerfCountersBuilder plb(
        m_cct, labels, l_mds_per_client_metrics_start,
        l_mds_per_client_metrics_last);
    plb.add_u64(
        l_mds_per_client_metrics_cap_hits, "cap_hits", "Capability hits",
        "hcap", PerfCountersBuilder::PRIO_CRITICAL);
    plb.add_u64(
        l_mds_per_client_metrics_cap_misses, "cap_miss", "Capability misses",
        "mcap", PerfCountersBuilder::PRIO_CRITICAL);
    plb.add_time(
        l_mds_per_client_metrics_avg_read_latency, "avg_read_latency",
        "Average Read Latency", "arlt", PerfCountersBuilder::PRIO_CRITICAL);
    plb.add_time(
        l_mds_per_client_metrics_avg_write_latency, "avg_write_latency",
        "Average Write Latency", "awlt", PerfCountersBuilder::PRIO_CRITICAL);
    plb.add_time(
        l_mds_per_client_metrics_avg_metadata_latency, "avg_metadata_latency",
        "Average Metadata Latency", "amlt", PerfCountersBuilder::PRIO_CRITICAL);
    plb.add_u64(
        l_mds_per_client_metrics_dentry_lease_hits, "dentry_lease_hits",
        "Dentry Lease Hits", "hden", PerfCountersBuilder::PRIO_CRITICAL);
    plb.add_u64(
        l_mds_per_client_metrics_dentry_lease_misses, "dentry_lease_miss",
        "Dentry Lease Misses", "mden", PerfCountersBuilder::PRIO_CRITICAL);
    plb.add_u64(
        l_mds_per_client_metrics_opened_files, "opened_files", "Open Files",
        "ofil", PerfCountersBuilder::PRIO_CRITICAL);
    plb.add_u64(
        l_mds_per_client_metrics_opened_inodes, "opened_inodes", "Open Inodes",
        "oino", PerfCountersBuilder::PRIO_CRITICAL);
    plb.add_u64(
        l_mds_per_client_metrics_pinned_icaps, "pinned_icaps",
        "Pinned Inode Caps", "pino", PerfCountersBuilder::PRIO_CRITICAL);
    plb.add_u64(
        l_mds_per_client_metrics_total_inodes, "total_inodes", "Total Inodes",
        "tino", PerfCountersBuilder::PRIO_CRITICAL);
    plb.add_u64(
        l_mds_per_client_metrics_total_read_ops, "total_read_ops",
        "Total Read Operations", "rops", PerfCountersBuilder::PRIO_CRITICAL);
    plb.add_u64(
        l_mds_per_client_metrics_total_read_size, "total_read_size",
        "Total Read Size", "rsiz", PerfCountersBuilder::PRIO_CRITICAL);
    plb.add_u64(
        l_mds_per_client_metrics_total_write_ops, "total_write_ops",
        "Total Write Operations", "wops", PerfCountersBuilder::PRIO_CRITICAL);
    plb.add_u64(
        l_mds_per_client_metrics_total_write_size, "total_write_size",
        "Total Write Size", "wsiz", PerfCountersBuilder::PRIO_CRITICAL);
  ...
  if (perf_counter_ptr) {
    ...
    perf_counter_ptr->set(
        l_mds_per_client_metrics_cap_hits, metrics.cap_hit_metric.hits);
    perf_counter_ptr->set(
        l_mds_per_client_metrics_cap_misses, metrics.cap_hit_metric.misses);

    // some averages
    if (metrics.read_latency_metric.updated) {
      utime_t ravg(
          metrics.read_latency_metric.mean.tv.tv_sec * 100,
    ...
    if (metrics.write_latency_metric.updated) {
      utime_t wavg(
          metrics.write_latency_metric.mean.tv.tv_sec * 100,
    ...
    if (metrics.metadata_latency_metric.updated) {
      utime_t mavg(
          metrics.metadata_latency_metric.mean.tv.tv_sec * 100,
    ...
    if (metrics.dentry_lease_metric.updated) {
      perf_counter_ptr->set(
          l_mds_per_client_metrics_dentry_lease_hits,
          metrics.dentry_lease_metric.hits);
      perf_counter_ptr->set(
          l_mds_per_client_metrics_dentry_lease_misses,
          metrics.dentry_lease_metric.misses);
    }
    ...
    if (metrics.opened_files_metric.updated) {
      perf_counter_ptr->set(
          l_mds_per_client_metrics_opened_files,
          metrics.opened_files_metric.opened_files);
      perf_counter_ptr->set(
          l_mds_per_client_metrics_total_inodes,
          metrics.opened_files_metric.total_inodes);
    }
    ...
    if (metrics.pinned_icaps_metric.updated) {
      perf_counter_ptr->set(
          l_mds_per_client_metrics_pinned_icaps,
          metrics.pinned_icaps_metric.pinned_icaps);
      perf_counter_ptr->set(
          l_mds_per_client_metrics_total_inodes,
          metrics.pinned_icaps_metric.total_inodes);
    }
    ...
    if (metrics.read_io_sizes_metric.updated) {
      perf_counter_ptr->set(
          l_mds_per_client_metrics_total_read_ops,
          metrics.read_io_sizes_metric.total_ops);
      perf_counter_ptr->set(
          l_mds_per_client_metrics_total_read_size,
          metrics.read_io_sizes_metric.total_size);
    }
    if (metrics.write_io_sizes_metric.updated) {
      perf_counter_ptr->set(
          l_mds_per_client_metrics_total_write_ops,
          metrics.write_io_sizes_metric.total_ops);
      perf_counter_ptr->set(
          l_mds_per_client_metrics_total_write_size,
          metrics.write_io_sizes_metric.total_size);
    }
  }

  auto update_counter_func = [&metrics](
                                 const MDSPerformanceCounterDescriptor& d,
  ...
  auto sub_key_func = [client, rank](
                          const MDSPerfMetricSubKeyDescriptor& d,
                          MDSPerfMetricSubKey* sub_key) {
    ceph_assert(d.is_supported());
...
void
MetricAggregator::remove_metrics_for_rank(
    const entity_inst_t& client,
    mds_rank_t rank,
    bool remove)
{
...
void
MetricAggregator::handle_mds_metrics(const cref_t<MMDSMetrics>& m)
{
  ...
  dout(20) << ": applying " << client_metrics_map.size() << "+"
           << subvolume_metrics.size() << " updates for rank=" << rank
           << " with sequence number " << seq << dendl;
...
void
MetricAggregator::update_rank_perf_metrics(
    mds_rank_t rank,
    const RankPerfMetrics& metrics)
{
  auto& perf_counter = rank_perf_counters[rank];
  if (!perf_counter) {
    std::string labels = ceph::perf_counters::key_create(
        "mds_rank_perf", {{"rank", stringify(rank)}});
    PerfCountersBuilder plb(
        m_cct, labels, l_mds_rank_perf_start, l_mds_rank_perf_last);
    plb.add_u64(
        l_mds_rank_perf_cpu_usage, "cpu_usage",
        "Sum of per-core CPU utilisation reported for this MDS (100 == one "
        "full core)",
        "cpu%", PerfCountersBuilder::PRIO_USEFUL);
    plb.add_u64(
        l_mds_rank_perf_open_requests, "open_requests",
        "Number of metadata requests currently in flight on this MDS", "req",
...
void
MetricAggregator::cull_metrics_for_rank(mds_rank_t rank)
{
...
void
MetricAggregator::remove_rank_perf_metrics_for_rank(mds_rank_t rank)
{
...
void
MetricAggregator::notify_mdsmap(const MDSMap& mdsmap)
{
  dout(10) << dendl;
  ...
  boost::copy(
      active_rank_addrs | boost::adaptors::map_keys,
      std::inserter(active_set, active_set.begin()));

  std::set<mds_rank_t> diff;
  std::set_difference(
      active_set.begin(), active_set.end(), current_active.begin(),
      current_active.end(), std::inserter(diff, diff.end()));
  ...
  diff.clear();
  std::set_difference(
      current_active.begin(), current_active.end(), active_set.begin(),
      active_set.end(), std::inserter(diff, diff.end()));

  for (auto& rank : diff) {
    auto rank_addr = mdsmap.get_addrs(rank);
    dout(10) << ": active rank=" << rank << " (mds."
             << mdsmap.get_mds_info(rank).name << ") has addr=" << rank_addr
             << dendl;
...
void
MetricAggregator::set_perf_queries(const ConfigPayload& config_payload)
{
  const MDSConfigPayload& mds_config_payload =
      std::get<MDSConfigPayload>(config_payload);
  const std::map<MDSPerfMetricQuery, MDSPerfMetricLimits>& queries =
      mds_config_payload.config;

  dout(10) << ": setting " << queries.size() << " perf_queries" << dendl;

  std::scoped_lock locker(lock);
  std::map<MDSPerfMetricQuery, std::map<MDSPerfMetricKey, PerformanceCounters>>
      new_data;
  for (auto& p : queries) {
...
MetricAggregator::get_perf_reports()
{
  ...
  for (auto& [query, counters] : query_metrics_map) {
    ...
    for (auto& p : counters) {
      dout(20) << ": packing perf_metric_key=" << p.first
               << ", perf_counter=" << p.second << dendl;
