...
private:
  ...
  std::map<std::pair<entity_inst_t, mds_rank_t>, PerfCounters*>
      client_perf_counters;
  uint64_t subv_window_sec = 0;
  std::unordered_map<std::string, SlidingWindowTracker<SubvolumeMetric>>
      subvolume_aggregated_metrics;
  ...
  void refresh_metrics_for_rank(
      const entity_inst_t& client,
      mds_rank_t rank,
      const Metrics& metrics);
  void refresh_subvolume_metrics_for_rank(
      mds_rank_t rank,
      const std::vector<SubvolumeMetric>& metrics);
  void remove_metrics_for_rank(
      const entity_inst_t& client,
      mds_rank_t rank,
      bool remove);
