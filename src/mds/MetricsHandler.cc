...
MetricsHandler::MetricsHandler(CephContext* cct, MDSRank* mds) :
  Dispatcher(cct), mds(mds)
{
  clk_tck = sysconf(_SC_CLK_TCK);
  if (clk_tck <= 0) {
    dout(1) << "failed to determine clock ticks per second, cpu usage metric "
               "disabled"
            << dendl;
    clk_tck = 0;
  }
...
MetricsHandler::ms_dispatch2(const ref_t<Message>& m)
{
...
MetricsHandler::init()
{
  ...
  if (mds->get_nodeid() != 0 && !rank_perf_counters) {
    std::string labels = ceph::perf_counters::key_create(
        "mds_rank_perf", {{"rank", stringify(mds->get_nodeid())}});

    PerfCountersBuilder plb(
        cct, labels, l_mds_rank_perf_start, l_mds_rank_perf_last);
    plb.add_u64(
    ...
    plb.add_u64(
        ...
        "Number of metadata requests currently in flight", "req",
  ...
  updater = std::thread([this]() {
    ...
    while (!stopping) {
      double after =
          g_conf()
              .get_val<std::chrono::seconds>("mds_metrics_update_interval")
              .count();
      locker.unlock();
      sleep(after);
      locker.lock();
...
void
MetricsHandler::add_session(Session* session)
{
  ...
  auto p = client_metrics_map
               .emplace(client, std::pair(last_updated_seq, Metrics()))
               .first;
...
void
MetricsHandler::remove_session(Session* session)
{
  ceph_assert(session != nullptr);
  ...
  if (lus == last_updated_seq) {
    dout(10) << ": metric lus=" << lus
             << ", last_updated_seq=" << last_updated_seq << dendl;
...
void
MetricsHandler::set_next_seq(version_t seq)
{
  dout(20) << ": current sequence number " << next_seq
           << ", setting next sequence number " << seq << dendl;
  next_seq = seq;
}

void
MetricsHandler::reset_seq()
{
...
void
MetricsHandler::handle_payload(Session* session, const CapInfoPayload& payload)
{
  dout(20) << ": type=" << payload.get_type() << ", session=" << session
           << ", hits=" << payload.cap_hits << ", misses=" << payload.cap_misses
           << dendl;
...
void
MetricsHandler::handle_payload(
    Session* session,
    const ReadLatencyPayload& payload)
{
  dout(20) << ": type=" << payload.get_type() << ", session=" << session
           << ", latency=" << payload.lat << ", avg=" << payload.mean
           << ", sq_sum=" << payload.sq_sum << ", count=" << payload.count
           << dendl;
...
MetricsHandler::handle_payload(
    Session* session,
    const WriteLatencyPayload& payload)
{
  dout(20) << ": type=" << payload.get_type() << ", session=" << session
           << ", latency=" << payload.lat << ", avg=" << payload.mean
           << ", sq_sum=" << payload.sq_sum << ", count=" << payload.count
...
MetricsHandler::handle_payload(
    Session* session,
    const DentryLeasePayload& payload)
{
  dout(20) << ": type=" << payload.get_type() << ", session=" << session
           << ", hits=" << payload.dlease_hits
...
MetricsHandler::handle_payload(
    Session* session,
    const OpenedFilesPayload& payload)
{
  dout(20) << ": type=" << payload.get_type() << ", session=" << session
...
MetricsHandler::handle_payload(
    Session* session,
    const PinnedIcapsPayload& payload)
{
  dout(20) << ": type=" << payload.get_type() << ", session=" << session
           << ", pinned_icaps=" << payload.pinned_icaps
           << ", total_inodes=" << payload.total_inodes << dendl;
...
MetricsHandler::handle_payload(
    Session* session,
    const OpenedInodesPayload& payload)
{
  dout(20) << ": type=" << payload.get_type() << ", session=" << session
           << ", opened_inodes=" << payload.opened_inodes
           << ", total_inodes=" << payload.total_inodes << dendl;
...
MetricsHandler::handle_payload(
    Session* session,
    const ReadIoSizesPayload& payload)
{
  dout(20) << ": type=" << payload.get_type() << ", session=" << session
...
MetricsHandler::handle_payload(
    Session* session,
    const WriteIoSizesPayload& payload)
{
  dout(20) << ": type=" << payload.get_type() << ", session=" << session
           << ", total_ops=" << payload.total_ops
           << ", total_size=" << payload.total_size << dendl;
...
MetricsHandler::handle_payload(
    Session* session,
    const SubvolumeMetricsPayload& payload,
    std::unique_lock<ceph::mutex>& lk)
{
  dout(20) << ": type=" << payload.get_type() << ", session=" << session
           << " , subv_metrics count=" << payload.subvolume_metrics.size()
           << dendl;
  ...
  for (size_t i = 0; i < resolved_paths.size(); ++i) {
    ...
    dout(20) << " accumulating subv_metric " << payload.subvolume_metrics[i]
             << dendl;
...
void
MetricsHandler::handle_payload(Session* session, const UnknownPayload& payload)
{
  dout(5) << ": type=Unknown, session=" << session
          << ", ignoring unknown payload" << dendl;
}

void
MetricsHandler::handle_client_metrics(const cref_t<MClientMetrics>& m)
{
  ...
  for (auto& metric : m->updates) {
    ...
    if (auto* subv_payload =
            std::get_if<SubvolumeMetricsPayload>(&metric.payload)) {
...
void
MetricsHandler::handle_mds_ping(const cref_t<MMDSPing>& m)
{
  std::scoped_lock locker(lock);
  set_next_seq(m->seq);
}
...
void
MetricsHandler::notify_mdsmap(const MDSMap& mdsmap)
{
  dout(10) << dendl;
  ...
  dout(10) << ": rank0 is mds." << mdsmap.get_mds_info((mds_rank_t)0).name
           << dendl;
...
void
MetricsHandler::update_rank0(std::unique_lock<ceph::mutex>& locker)
{
  dout(20) << dendl;
  ...
  for (inodeno_t subvol_id : subvol_ids) {
    if (subvol_used_bytes.count(subvol_id) == 0) {
      ...
      dout(20) << "resolved used_bytes for subvol " << subvol_id << " = "
               << rbytes << dendl;
    }
  }
  ...
  for (auto& [path, aggregated_metrics] : subvolume_metrics_map) {
    ...
    aggregate_subvolume_metrics(
        path, aggregated_metrics, subvol_used_bytes,
  ...
  dout(20) << ": sending " << metrics_message.subvolume_metrics.size()
           << " subv_metrics to aggregator" << dendl;

  mds->send_message_mds(
      make_message<MMDSMetrics>(std::move(metrics_message)), *addr_rank0);
}

void
MetricsHandler::aggregate_subvolume_metrics(
    const std::string& subvolume_path,
    const std::vector<AggregatedIOMetrics>& metrics_list,
    const std::unordered_map<inodeno_t, uint64_t>& subvol_used_bytes,
    SubvolumeMetric& res)
{
  dout(20) << ": aggregating " << metrics_list.size() << " subv_metrics"
           << dendl;
...
void
MetricsHandler::maybe_update_subvolume_quota(
    inodeno_t subvol_id,
    ...
    bool force_zero)
{
  ...
  if (it == subvolume_quota.end()) {
    ...
    dout(20) << __func__ << " inserted subvolume_quota for " << subvol_id
             << dendl;
  }
  ...
  dout(20) << __func__ << " subvol " << subvol_id
           ...
           << " (input: quota=" << quota_bytes << ", used=" << used_bytes << ")"
           << dendl;
}

void
MetricsHandler::sample_cpu_usage()
{
  ...
  double elapsed =
      std::chrono::duration<double>(now - rank_telemetry.last_cpu_sample_time)
          .count();
  if (elapsed <= 0.0) {
    rank_telemetry.metrics.cpu_usage_percent = 0;
    if (rank_perf_counters) {
  ...
  double cpu_seconds = static_cast<double>(delta_ticks) /
...
void
MetricsHandler::sample_open_requests()
{
  uint64_t open = 0;
