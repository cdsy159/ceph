...
public:
  ...
  void maybe_update_subvolume_quota(
      inodeno_t subvol_id,
...
private:
  struct HandlePayloadVisitor : public boost::static_visitor<void> {
    ...
    inline void
    operator()(const SubvolumeMetricsPayload&) const
    {
      ceph_abort_msg("SubvolumeMetricsPayload should be handled specially");
    }
  };

  std::unique_ptr<PerfCounters> create_subv_perf_counter(
      const std::string& subv_name);
  ...
  std::unordered_map<std::string, std::vector<AggregatedIOMetrics>>
      subvolume_metrics_map;
  ...
  void handle_payload(
      ...
      std::unique_lock<ceph::mutex>& lock_guard);
  ...
  struct UnlockGuard {
    ...
    explicit UnlockGuard(std::unique_lock<ceph::mutex>& l) :
      lk(l)
    {
      lk.unlock();
    }
  ...
  void aggregate_subvolume_metrics(
