...
public:
  ...
  const static int EXPORT_PREPPING =
  ...
  int
  get_num_exporting() const
  {
    return export_state.size();
  }

  int
  get_export_queue_size() const
  {
    return export_queue.size();
  }
  ...
  void decode_import_inode_caps(
      CInode* in,
      bool auth_cap,
      bufferlist::const_iterator& blp,
      std::map<CInode*, std::map<client_t, Capability::Export>>& cap_imports);
  void finish_import_inode_caps(
      CInode* in,
      mds_rank_t from,
      bool auth_cap,
...
protected:
  ...
  void encode_export_prep_trace(
      bufferlist& bl,
      CDir* bound,
      CDir* dir,
      export_state_t& es,
      std::set<inodeno_t>& inodes_added,
      std::set<dirfrag_t>& dirfrags_added);
  void decode_export_prep_trace(
      bufferlist::const_iterator& blp,
      mds_rank_t oldauth,
      std::vector<MDSContext*>& finished);
  ...
  // importer
  void handle_export_discover(
      const cref_t<MExportDirDiscover>& m,
      bool started = false);
  void handle_export_cancel(const cref_t<MExportDirCancel>& m);
  void handle_export_prep(
      const cref_t<MExportDirPrep>& m,
      bool did_assim = false);
