...
public:
  ...
  struct LockOp {
    ...
    bool
    is_remote_wrlock() const
    {
      return !!(flags & REMOTE_WRLOCK);
    }

    void
    clear_remote_wrlock() const
    {
      flags &= ~REMOTE_WRLOCK;
  ...
  struct LockOpVec : public std::vector<LockOp> {
    ...
    void
    add_rdlock(SimpleLock* lock, int idx = -1)
    {
    ...
    void
    add_xlock(SimpleLock* lock, int idx = -1)
    {
      add_lock(LockOp(lock, LockOp::XLOCK), idx);
    }

    void
    add_wrlock(SimpleLock* lock, int idx = -1)
    {
      add_lock(LockOp(lock, LockOp::WRLOCK), idx);
    }

    void
    add_remote_wrlock(SimpleLock* lock, mds_rank_t rank)
    {
    ...
    void
    lock_scatter_gather(SimpleLock* lock)
    {
  ...
  MutationImpl(
      OpTracker* tracker,
      utime_t initiated,
      const metareqid_t& ri,
      __u32 att = 0,
      mds_rank_t peer_to = MDS_RANK_NONE) :
    TrackedOp(tracker, initiated), reqid(ri), attempt(att), peer_to_mds(peer_to)
  {}
  ...
  const ObjectState*
  find_object_state(MDSCacheObject* obj) const
  {
  ...
  lock_iterator
  emplace_lock(SimpleLock* l, unsigned f = 0, mds_rank_t t = MDS_RANK_NONE)
  {
    last_locked = l;
  ...
  bool
  is_xlocked(SimpleLock* lock) const
  {
  ...
  bool
  is_remote_wrlocked(SimpleLock* lock) const
  {
    auto it = locks.find(lock);
  ...
  bool
  is_last_locked(SimpleLock* lock) const
  {
  ...
  void
  add_projected_node(MDSCacheObject* obj)
  {
    projected_nodes.insert(obj);
  }

  void
  remove_projected_node(MDSCacheObject* obj)
  {
    projected_nodes.erase(obj);
  }

  bool
  is_projected(MDSCacheObject* obj) const
  {
...
struct MDRequestImpl : public MutationImpl {
  ...
  struct More {
    ...
    int peer_error = 0;
    std::set<mds_rank_t>
        peers; // mds nodes that have peer requests to me (implies client_request)
    std::set<mds_rank_t>
        waiting_on_peer; // peers i'm waiting for peerreq replies from.
  ...
  bool
  get_queued_next_replay_op() const
  {
    return queued_next_replay_op;
  }

  void
  set_queued_next_replay_op()
  {
  ...
  void
  dump_with_mds_lock(ceph::Formatter* f) const
  {
    return _dump(f, true);
  }
  ...
  elist<MDRequestImpl*>::item
      item_session_request; // if not on list, op is aborted.
  ...
  // -- i am a peer request
  ceph::cref_t<MMDSPeerRequest>
      peer_request; // peer request (if one is pending; implies peer == true)
...
protected:
  void
  _dump(ceph::Formatter* f) const override
  {
    _dump(f, false);
  }
...
struct MDLockCache : public MutationImpl {
  ...
  void
  set_dir_layout(file_layout_t& layout)
  {
    dir_layout = layout;
  }
