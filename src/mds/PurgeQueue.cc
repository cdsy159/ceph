...
static ostream&
_prefix(std::ostream* _dout, mds_rank_t rank)
{
...
void
PurgeItem::encode(bufferlist& bl) const
{
  ENCODE_START(2, 1, bl);
...
void
PurgeItem::decode(bufferlist::const_iterator& p)
{
  DECODE_START(2, p);
  bool done = false;
  if (struct_v == 1) {
...
void
PurgeItem::dump(Formatter* f) const
{
...
std::list<PurgeItem>
PurgeItem::generate_test_instances()
{
...
void
PurgeQueue::create_logger()
{
  PerfCountersBuilder pcb(g_ceph_context, "purge_queue", l_pq_first, l_pq_last);

  pcb.add_u64_counter(
      l_pq_executed_ops, "pq_executed_ops", "Purge queue ops executed", "puro",
      PerfCountersBuilder::PRIO_INTERESTING);
  pcb.add_u64_counter(
      l_pq_executed, "pq_executed", "Purge queue tasks executed", "purg",
      PerfCountersBuilder::PRIO_INTERESTING);

  pcb.set_prio_default(PerfCountersBuilder::PRIO_USEFUL);
  pcb.add_u64(
      l_pq_executing_ops, "pq_executing_ops", "Purge queue ops in flight");
  pcb.add_u64(
      l_pq_executing_ops_high_water, "pq_executing_ops_high_water",
      "Maximum number of executing file purge ops");
  pcb.add_u64(l_pq_executing, "pq_executing", "Purge queue tasks in flight");
  pcb.add_u64(
      l_pq_executing_high_water, "pq_executing_high_water",
      "Maximum number of executing file purges");
  pcb.add_u64(
      l_pq_item_in_journal, "pq_item_in_journal", "Purge item left in journal");
...
PurgeQueue::activate()
{
...

  {
    ...
    ::encode(item, bl);
    purge_item_journal_size = bl.length() +
                              journaler.get_journal_envelope_size();
  }
...
void
PurgeQueue::open(Context* completion)
{
...
void
PurgeQueue::wait_for_recovery(Context* c)
{
  std::lock_guard l(lock);
  if (recovered) {
    c->complete(0);
 ...
 */
void
PurgeQueue::push(const PurgeItem& pi, Context* completion)
{
  ...
  if (!could_consume) {
    ...
    if (!delayed_flush) {
      ...
      timer.add_event_after(
          g_conf()->mds_purge_queue_busy_flush_period, delayed_flush);
    }
  }
}

uint32_t
PurgeQueue::_calculate_ops(const PurgeItem& item) const
{
  ...
  if (item.action == PurgeItem::PURGE_DIR) {
    ...
    const uint64_t num = (item.size > 0)
                             ? Striper::get_num_objects(item.layout, item.size)
                             : 1;
...
void
PurgeQueue::_go_readonly(int r)
{
  if (readonly)
    return;
  dout(1) << "going readonly because internal IO failed: " << strerror(-r)
          << dendl;
  readonly = true;
...
public:
  C_IO_PurgeItem_Commit(
      PurgeQueue* pq,
      std::vector<PurgeItemCommitOp> ops,
      uint64_t expire_to) :
    ...
    uint64_t expire_to)
{
  ...
  for (auto& op : ops_vec) {
    ...
    if (op.type == PurgeItemCommitOp::PURGE_OP_RANGE) {
      ...
      filer.purge_range(
          op.item.ino, &op.item.layout, op.item.snapc, first_obj, num_obj,
          ceph::real_clock::now(), op.flags, gather.new_sub());
    } else if (op.type == PurgeItemCommitOp::PURGE_OP_REMOVE) {
      if (op.item.action == PurgeItem::PURGE_DIR) {
        objecter->remove(
            op.oid, op.oloc, nullsnapc, ceph::real_clock::now(), op.flags,
            gather.new_sub());
      } else {
        objecter->remove(
            op.oid, op.oloc, op.item.snapc, ceph::real_clock::now(), op.flags,
            gather.new_sub());
      }
    } else if (op.type == PurgeItemCommitOp::PURGE_OP_ZERO) {
      filer.zero(
          op.item.ino, &op.item.layout, op.item.snapc, 0,
          op.item.layout.object_size, ceph::real_clock::now(), 0, true,
          gather.new_sub());
    } else {
...
void
PurgeQueue::_execute_item(const PurgeItem& item, uint64_t expire_to)
{
  ceph_assert(ceph_mutex_is_locked_by_me(lock));
  ...
  auto submit_ops = [&]() {
    finisher.queue(
        new C_IO_PurgeItem_Commit(this, std::move(ops_vec), expire_to));
  };

  if (item.action == PurgeItem::PURGE_FILE) {
    if (item.size > 0) {
      uint64_t num = Striper::get_num_objects(item.layout, item.size);
      dout(10) << " 0~" << item.size << " objects 0~" << num << " snapc "
               << item.snapc << " on " << item.ino << dendl;
    ...
    if (ops_vec.empty() || !item.layout.pool_ns.empty()) {
      ...
      dout(10) << " remove backtrace object " << oid << " pool " << oloc.pool
               << " snapc " << item.snapc << dendl;
      ops_vec.emplace_back(
          item, PurgeItemCommitOp::PURGE_OP_REMOVE, 0, oid, oloc);
    }

    // remove old backtrace objects
    for (const auto& p : item.old_pools) {
      object_locator_t oloc(p);
      dout(10) << " remove backtrace object " << oid << " old pool " << p
               << " snapc " << item.snapc << dendl;
      ops_vec.emplace_back(
          item, PurgeItemCommitOp::PURGE_OP_REMOVE, 0, oid, oloc);
    }
...
void
PurgeQueue::_execute_item_complete(uint64_t expire_to)
{
  ceph_assert(ceph_mutex_is_locked_by_me(lock));
  ...
  dout(10) << "left purge items in journal: " << item_num
           ...
           << "(" << purge_item_journal_size << "/" << write_pos << "/"
           << read_pos << "/" << expire_pos << ")" << dendl;
...
void
PurgeQueue::update_op_limit(const MDSMap& mds_map)
{
  std::lock_guard l(lock);
  ...
  max_purge_ops = uint64_t(
      ((double)pg_count / (double)mds_map.get_max_mds()) *
...
PurgeItem::get_type_str() const
{
  ...
  case PurgeItem::NONE:
    return "NONE";
  case PurgeItem::PURGE_FILE:
    return "PURGE_FILE";
  case PurgeItem::PURGE_DIR:
    return "PURGE_DIR";
  case PurgeItem::TRUNCATE_FILE:
