...
public:
  ...
  struct TrackedRoot {
    ...
    TrackedRoot() :
      TrackedRoot(QS__INVALID, QuiesceTimeInterval::zero())
    {}

    bool
    should_quiesce() const
    {
      return committed_state == QS_QUIESCING || committed_state == QS_QUIESCED;
    }

    bool
    should_release() const
    {
    ...
    void
    update_committed(QuiesceMap::RootInfo const& info)
    {
  ...
  TrackedRootRef
  get_tracked_root(QuiesceRoot root)
  {
    std::lock_guard l(agent_mutex);
...
protected:
  ...
  template <class L>
  QuiesceDbVersion
  await_phase_locked(L& lock, bool pending_armed, bool current_armed)
  {
    agent_cond.wait(lock, [=, this] {
      return (
          !upkeep_needed && current.armed == current_armed &&
  ...
  virtual void set_pending_roots(
