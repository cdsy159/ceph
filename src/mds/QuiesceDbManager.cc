...
#undef dout_prefix
#define dout_prefix \
  *_dout << "quiesce.mgr." << membership.me << " <" << __func__ << "> "
...
#define dsetroot(suffix) \
  "[" << set_id << "@" << set.version << "," << root << "] " << suffix

const QuiesceInterface::PeerId QuiesceClusterMembership::INVALID_MEMBER =
    MDS_GID_NONE;

static QuiesceTimeInterval
time_distance(QuiesceTimePoint lhs, QuiesceTimePoint rhs)
{
  if (lhs > rhs) {
    return lhs - rhs;
  } else {
...
bool
QuiesceDbManager::db_thread_has_work() const
{
  return db_thread_should_exit || pending_acks.size() > 0 ||
         pending_requests.size() > 0 || pending_db_updates.size() > 0 ||
         (agent_callback.has_value() &&
          agent_callback->if_newer < db.version()) ||
         (cluster_membership.has_value() &&
          cluster_membership->epoch != membership.epoch);
}

void*
QuiesceDbManager::quiesce_db_thread_main()
{
  ...
  while (keep_working) {
    ...
    while (!db_thread_has_work()) {
      ...
      dout(20) << "db idle, age: " << db_age
               << " next_event_at_age: " << next_event_at_age << dendl;
    ...
    if (is_member) {
      ...
      if (membership.is_leader()) {
        const QuiesceTimeInterval bootstrap_delay =
            leader_bootstrap(std::move(db_updates));
        if (bootstrap_delay == QuiesceTimeInterval::zero()) {
          // we're good to process things
          next_event_at_age =
              leader_upkeep(std::move(acks), std::move(requests));
        } else {
          ...
          if (pending_db_updates.empty()) {
            ...
            // bootstrap_delay.
            dout(5) << "bootstrap: waiting for new peers with pending acks: "
                    << pending_acks.size()
    ...
    QuiesceMap quiesce_map(db.version());
    {
      ...
      if (agent_callback) {
        if (agent_callback->if_newer < db.version()) {
          dout(20) << "notifying agent with db version " << db.version()
                   << dendl;
        ...
        dout(5) << "no agent callback registered, responding with an empty ack"
                << dendl;
      }
    }
...
void
QuiesceDbManager::update_membership(
    const QuiesceClusterMembership& new_membership,
    RequestContext* inject_request)
{
  std::unique_lock lock(submit_mutex);

  bool will_participate = new_membership.members.contains(new_membership.me);
  dout(20) << "will participate: " << std::boolalpha << will_participate
           << std::noboolalpha << dendl;

  if (will_participate && !quiesce_db_thread.is_started()) {
    // start the thread
    dout(5) << "starting the db mgr thread at epoch: " << new_membership.epoch
            << dendl;
...
QuiesceDbManager::membership_upkeep()
{
  ...
  bool is_leader = cluster_membership &&
                   cluster_membership->leader == cluster_membership->me;
  if (cluster_membership) {
    dout(10) << "epoch:" << cluster_membership->epoch
             << " leader:" << std::boolalpha << was_leader << "->" << is_leader
             << std::noboolalpha << " members:" << cluster_membership->members
             << dendl;
  } else {
...
QuiesceTimeInterval
QuiesceDbManager::replica_upkeep(decltype(pending_db_updates)&& db_updates)
{
  // as a replica, we only care about the latest update
  while (db_updates.size() > 1) {
    dout(10) << "skipping an older update from " << db_updates.front().origin
             << " version " << db_updates.front().db.db_version << dendl;
    db_updates.pop();
  }
  ...
  if (time_distance(time_zero, db.time_zero) > std::chrono::seconds(1)) {
    dout(10) << "significant db_time_zero change to " << time_zero << " from "
             << db.time_zero << dendl;
  }
  ...
  if (db.set_version > update.db_version.set_version) {
    dout(3) << "got an older version of DB from the leader: "
            << update.db_version.set_version << " < " << db.set_version
            << dendl;
...
QuiesceTimeInterval
QuiesceDbManager::leader_bootstrap(decltype(pending_db_updates)&& db_updates)
{
  ...
  for (auto&& [peer, info] : peers) {
    if (info.diff_map.db_version.epoch < membership.epoch &&
        info.diff_map.db_version.set_version == 0) {
  ...
  while (!unknown_peers.empty() && !db_updates.empty()) {
    ...
    if (update.db_version.epoch == membership.epoch &&
        unknown_peers.erase(from) > 0) {
      // see if this peer's version is newer than mine
      if (db.set_version < update.db_version.set_version) {
        dout(3) << "preferring version from peer " << from << " ("
                << update.db_version << ") over mine (" << db.version() << ")"
      ...
      peers[from] =
          PeerInfo{QuiesceMap({membership.epoch, 0}), QuiesceClock::now()};
    }
    db_updates.pop();
  }
  ...
  for (auto& peer : unknown_peers) {
    ...
    QuiesceTimePoint next_discovery = info.last_activity +
  ...
  return all_peers_known ? QuiesceTimeInterval::zero()
                         : (bootstrap_delay + a_little_more);
}

QuiesceTimeInterval
QuiesceDbManager::leader_upkeep(
    decltype(pending_acks)&& acks,
    decltype(pending_requests)&& requests)
{
  // record peer acks
  while (!acks.empty()) {
...
void
QuiesceDbManager::complete_requests()
{
...
void
QuiesceDbManager::leader_record_ack(
    QuiesceInterface::PeerId from,
    QuiesceMap&& diff_map)
{
  ...
  if (diff_map.db_version > db.version()) {
    dout(15) << "future version ack by peer " << from << " ("
             << diff_map.db_version << " > " << db.version() << ")" << dendl;
    if (diff_map.db_version.epoch > db.version().epoch &&
        diff_map.db_version.set_version <= db.version().set_version) {
      dout(15)
          << "my epoch is behind, ignoring this until my membership is updated"
          << dendl;
    } else {
...
bool
QuiesceDbManager::sanitize_roots(QuiesceDbRequest::Roots& roots)
{
  ...
  static const std::unordered_set<std::string> supported_schemes{
      file_scheme, inode_scheme};
  QuiesceDbRequest::Roots result;
  for (auto& root : roots) {
    auto parsed_uri = boost::urls::parse_uri_reference(root);
    if (!parsed_uri) {
      dout(2) << "Couldn't parse root '" << root
              << "' as URI (error: " << parsed_uri.error() << ")" << dendl;
      return false;
    }
    ...
    if (!root_url.has_scheme()) {
      ...
      dout(2) << "Unsupported root URL scheme '" << root_url.scheme() << "'"
              << dendl;
      return false;
    }
    ...
    if (root_url.has_authority()) {
      ...
      if (ok_remove) {
        ...
        dout(10) << "Removing the fs name or id '" << auth_str
                 << "' from the root url authority section" << dendl;
    ...
    if (root_url.scheme() == file_scheme) {
      ...
      if (!inodeno || fmt::format("{}", inodeno) != sanitized_path) {
        dout(2) << "Root '" << root << "' does not encode a vaild inode number"
                << dendl;
        return false;
      }
    }
    ...
    if (root_url.buffer() != root) {
      dout(10) << "Normalized root '" << root << "' to '" << root_url.buffer()
               << "'" << dendl;
    }
...
int
QuiesceDbManager::leader_process_request(RequestContext* req_ctx)
{
  ...
  if (request.set_id) {
    ...
    dout(2) << "can't expect a non-zero version (" << *request.if_version
            << ") for a new set" << dendl;
    return EINVAL;
  }
  ...
  if (set_it == db.sets.end()) {
    if (request.includes_roots() && request.if_version <= 0) {
      ...
      dout(2) << "coudn't find set with id '" << *request.set_id << "'"
              << dendl;
      return ENOENT;
    }
  }
  ...
  if (set_it != db.sets.end()) {
    ...
    if (request.await) {
      ...
      if (set.rstate.state > QS_QUIESCED && !request.is_release()) {
        dout(2) << dset("can't quiesce-await a set in the state: ")
                << set.rstate.state << dendl;
        return EPERM;
      }

      auto expire_at_age = interval_saturate_add(db_age, *request.await);
      awaits.emplace(
          std::piecewise_construct, std::forward_as_tuple(set_id),
...
int
QuiesceDbManager::leader_update_set(
    Db::Sets::value_type& set_it,
    const QuiesceDbRequest& request)
{
  auto& [set_id, set] = set_it;
  if (request.if_version && set.version != *request.if_version) {
    dout(10) << dset("is newer than requested (") << *request.if_version << ") "
             << dendl;
    return ESTALE;
  }
  ...
  if (request.is_release()) {
    ...
    case QS_QUIESCED:
      ...
      did_update = true;
      dout(15) << dset("") << "updating state to: " << set.rstate.state
               << dendl;
    case QS_RELEASING:
    case QS_RELEASED:
      break;
    default:
      dout(2) << dset("can't release in the state: ") << set.rstate.state
              << dendl;
      return EPERM;
    }
  } else {
    ...
    if (!request.is_reset()) {
      ...
      if (!set.is_active() && set.version > 0) {
        dout(2) << dset("rejecting modification in the terminal state: ")
                << set.rstate.state << dendl;
        return EPERM;
      } else if (request.includes_roots() && set.is_releasing()) {
        dout(2) << dset("rejecting new roots in the QS_RELEASING state")
                << dendl;
        return EPERM;
      }
    } else {
    ...
    for (auto& [root, info] : set.members) {
      if (request.should_exclude(root)) {
        ...
        if (reset) {
          ...
          size_t reporting_peers = check_peer_reports(
              set_id, set, root, info, min_reported_state, max_reported_state);

          if (reporting_peers == peers.size() &&
              max_reported_state < QS__FAILURE) {
            effective_member_state =
                set.get_effective_member_state(min_reported_state);
          } else {
            effective_member_state = info.rstate.state;
          }
        }
    ...
    if (request.includes_roots()) {
      for (auto const& root : request.roots) {
        auto const& [member_it, emplaced] =
            set.members.try_emplace(root, db_age);
    ...
    if (included_count == 0) {
      ...
      if (did_update |= set.rstate.update(next_state, db_age)) {
        dout(15) << dset("updated to match the min state of the remaining (")
                 << included_count << ") members: " << set.rstate.state
                 << dendl;
      }
    }
  }
  ...
  if (did_update) {
    dout(20) << dset("updating version from ") << set.version << " to "
             << db.set_version + 1 << dendl;
    set.version = db.set_version + 1;
    if (did_update_roots) {
      ...
      if (range.first != range.second) {
        dout(10) << dset("interrupting awaits with rc = ") << rc
                 << " due to a change in members" << dendl;
      }
...
QuiesceTimeInterval
QuiesceDbManager::leader_upkeep_db()
{
  QuiesceTimeInterval next_event_at_age = QuiesceTimeInterval::max();
  ...
  static const QuiesceTimeInterval PEER_REPEATED_UPDATE_INTERVAL =
      std::chrono::seconds(1);
  for (auto const& peer_update : peer_updates) {
    if (peer_update.info.last_sent_version == db.version()) {
      if (now <
          (peer_update.info.last_activity + PEER_REPEATED_UPDATE_INTERVAL)) {
        // don't spam the peer with the same version
        continue;
      }
      dout(5) << "repeated update of the peer " << peer_update.peer
              << " with version " << db.version() << dendl;
    }
    ...
    std::ranges::copy(
        peer_update.set_refs, std::inserter(listing.sets, listing.sets.end()));

    dout(20) << "updating peer " << peer_update.peer << " with "
             << peer_update.set_refs.size()
             << " sets modified in db version range ("
             << peer_update.known_set_version() << ".." << db.set_version << "]"
             << dendl;
...
QuiesceState
QuiesceSet::next_state(QuiesceState min_member_state) const
{
...
size_t
QuiesceDbManager::check_peer_reports(
    const QuiesceSetId& set_id,
    const QuiesceSet& set,
    ...
    QuiesceState& max_reported_state)
{
  ...
  std::multimap<
      QuiesceState, std::pair<QuiesceInterface::PeerId, QuiesceDbVersion>>
      reporting_peers;
  ...
  for (auto& [peer, info] : peers) {
    ...
    if (dit != info.diff_map.roots.end()) {
      ...
      if (!pr_state.is_valid()) {
        dout(5) << dsetroot("ignoring an invalid peer state ") << pr_state.state
                << dendl;
        continue;
      }
    ...
    if (info.diff_map.db_version >=
        QuiesceDbVersion{membership.epoch, set.version}) {
      up_to_date_peers++;
    }
  ...
  dout(20) << dsetroot("") << "up_to_date_peers: " << up_to_date_peers
           << " min_reported_state: " << min_reported_state
           << " max_reported_state: " << max_reported_state
           << " peer_acks: " << reporting_peers << dendl;

  return up_to_date_peers;
}

QuiesceTimeInterval
QuiesceDbManager::leader_upkeep_set(Db::Sets::value_type& set_it)
{
  auto& [set_id, set] = set_it;
  ...
  for (auto& [root, member] : set.members) {
    ...
    size_t reporting_peers = check_peer_reports(
        set_id, set, root, member, min_reported_state, max_reported_state);
    ...
    if (max_reported_state >= QS__FAILURE) {
      ...
      dout(5) << dsetroot("reported by at least one peer as: ")
              << max_reported_state << dendl;
      if (member.rstate.update(max_reported_state, db_age)) {
        dout(15) << dsetroot("updating member state to ") << member.rstate.state
                 << dendl;
        set.version = db.set_version + 1;
      }
    } else if (effective_state < member.rstate.state) {
      // someone has reported a rollback state for the root
      dout(15) << dsetroot("reported by at least one peer as ")
               << min_reported_state << " vs. the expected "
               << member.rstate.state << dendl;
      ...
      dout(20) << dsetroot("min reported state for all (") << reporting_peers
               << ") peers: " << min_reported_state
    ...
    if (member.is_quiescing()) {
      ...
      auto timeout_at_age =
          interval_saturate_add(member.rstate.at_age, set.timeout);
      ...
      dout(5) << dsetroot(
                     "propagating the terminal member state to the set level: ")
              << member.rstate.state << dendl;
  ...
  if (set.rstate.update(next_state, db_age)) {
    set.version = db.set_version + 1;
    dout(15) << dset("updated set state to match member reports: ")
             << set.rstate.state << dendl;
  }
  ...
  if (set.is_quiesced() || set.is_released()) {
    ...
    for (auto it = range.first; it != range.second; it++) {
      ...
      if (set.is_quiesced()) {
        ...
        set.version = db.set_version + 1;
        dout(20) << dset("reset quiesced state age upon successful await")
                 << dendl;
      }
    }
    awaits.erase(range.first, range.second);
  ...
  if (set.is_quiescing()) {
    ...
    auto expire_at_age =
        interval_saturate_add(set.rstate.at_age, set.expiration);
...
QuiesceTimeInterval
QuiesceDbManager::leader_upkeep_awaits()
{
  QuiesceTimeInterval next_event_at_age = QuiesceTimeInterval::max();
  for (auto it = awaits.begin(); it != awaits.end();) {
    ...
    if (set_it == db.sets.cend()) {
      ...
      case QS_FAILED:
        ...
        break;
      default:
        ceph_abort("unexpected quiesce set state");
      }
    }
...
void
QuiesceDbManager::calculate_quiesce_map(QuiesceMap& map)
{
  map.roots.clear();
  ...
  for (auto& [set_id, set] : db.sets) {
    if (set.is_active()) {
      ...
      for (auto& [root, member] : set.members) {
        ...
        auto root_it =
            map.roots.try_emplace(root, QuiesceMap::RootInfo{requested, ttl})
                .first;
