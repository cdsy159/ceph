...
struct QuiesceClusterMembership {
  ...
  is_leader() const
  {
    return leader == me && me != INVALID_MEMBER;
  }
};
...
public:
  ...
  QuiesceDbManager() :
    quiesce_db_thread(this){};

  virtual ~QuiesceDbManager() { shutdown(); }
  ...
  void
  update_membership(const QuiesceClusterMembership& new_membership)
  {
  ...
  int
  submit_request(RequestContext* ctx)
  {
  ...
  int
  submit_peer_ack(QuiesceDbPeerAck&& ack)
  {
    std::lock_guard l(submit_mutex);
  ...
  int
  submit_peer_listing(QuiesceDbPeerListing&& listing)
  {
    std::lock_guard l(submit_mutex);
  ...
  int
  submit_agent_ack(QuiesceMap&& diff_map)
  {
  ...
  struct AgentCallback {
    ...
    AgentCallback(const Notify& notify, QuiesceDbVersion if_newer = {0, 0}) :
      notify(notify), if_newer(if_newer)
    {}
  };

  std::optional<AgentCallback>
  reset_agent_callback(
      AgentCallback::Notify notify,
      QuiesceDbVersion if_newer = {0, 0})
  {
  ...
  std::optional<AgentCallback>
  reset_agent_callback(
      std::optional<AgentCallback> callback_if_newer = std::nullopt)
  {
  ...
  std::optional<AgentCallback>
  reset_agent_callback(QuiesceDbVersion if_newer)
  {
    std::lock_guard ls(submit_mutex);
    std::lock_guard lc(agent_mutex);
    if (agent_callback) {
...
protected:
  ...
  struct PeerInfo {
    ...
    PeerInfo(QuiesceMap&& diff_map, QuiesceTimePoint last_activity) :
      diff_map(diff_map), last_activity(last_activity)
    {}

    PeerInfo() { last_activity = QuiesceTimePoint::min(); }
  ...
  QuiesceTimeInterval leader_bootstrap(
      decltype(pending_db_updates)&& db_updates);
  QuiesceTimeInterval leader_upkeep(
  ...
  int leader_update_set(
      Db::Sets::value_type& set_it,
      const QuiesceDbRequest& req);
