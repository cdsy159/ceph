...
#undef dout_prefix
#define dout_prefix \
  *_dout << "mds." << mds->get_nodeid() << " RecoveryQueue::" << __func__ << " "
...
RecoveryQueue::advance()
{
  ...
  logger->set(
      ...
      file_recover_queue_size + file_recover_queue_front_size);
  logger->set(l_mdc_num_recovering_prioritized, file_recover_queue_front_size);
}

void
RecoveryQueue::_start(CInode* in)
{
  const auto& pi = in->get_projected_inode();

  // blech
  if (pi->client_ranges.size() && !pi->get_max_size()) {
    mds->clog->warn() << "bad client_range " << pi->client_ranges << " on ino "
                      << pi->ino;
  }
  ...
  if (pi->client_ranges.size() && pi->get_max_size()) {
    ...
    if (p == file_recovering.end()) {
      ...
      filer.probe(
          in->ino(), &layout, in->last, pi->get_max_size(), &fin->size,
          &fin->mtime, false, 0, fin);
    } else {
      p->second = true;
      dout(10) << "already working on " << *in << ", set need_restart flag"
               << dendl;
    }
  } else {
...
void
RecoveryQueue::prioritize(CInode* in)
{
...
static bool
_is_in_any_recover_queue(CInode* in)
{
 ...
 */
void
RecoveryQueue::enqueue(CInode* in)
{
 ...
 */
void
RecoveryQueue::_recovered(CInode* in, int r, uint64_t size, utime_t mtime)
{
