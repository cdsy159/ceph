...
public:
  ...
  {
    return SimpleLock::is_sync_and_unlocked() && !is_dirty() && !is_flushing();
  }

  bool
  can_scatter_pin(client_t loner)
  {
    /*
  ...
  void
  set_xlock_snap_sync(MDSContext* c)
  {
  ...
  bool
  get_scatter_wanted() const
  {
    return state_flags & SCATTER_WANTED;
  }

  bool
  get_unscatter_wanted() const
  {
    return state_flags & UNSCATTER_WANTED;
  }

  bool
  is_dirty() const override
  {
    return state_flags & DIRTY;
  }

  bool
  is_flushing() const override
  {
    return state_flags & FLUSHING;
  }

  bool
  is_flushed() const override
  {
    return state_flags & FLUSHED;
  }

  bool
  is_dirty_or_flushing() const
  {
  ...
  void
  infer_state_from_strong_rejoin(int rstate, bool locktoo)
  {
    if (rstate == LOCK_MIX ||
  ...
  void
  encode_state_for_rejoin(ceph::buffer::list& bl, int rep)
  {
  ...
  bool
  remove_replica(int from, bool rejoin)
  {
    if (rejoin && (state == LOCK_MIX || state == LOCK_MIX_SYNC ||
                   state == LOCK_MIX_LOCK2 || state == LOCK_MIX_TSYN ||
                   state == LOCK_MIX_EXCL))
      return false;
