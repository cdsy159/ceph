...
std::ostream&
operator<<(std::ostream& os, const ScrubStack::State& state)
{
  switch (state) {
...
void
ScrubStack::dequeue(MDSCacheObject* obj)
{
...
int
ScrubStack::_enqueue(MDSCacheObject* obj, ScrubHeaderRef& header, bool top)
{
  ...
  if (CInode* in = dynamic_cast<CInode*>(obj)) {
    ...
    dout(10) << __func__ << " with {" << *dir << "}" << ", top=" << top
             << dendl;
  ...
  dout(20) << "enqueue " << *obj << " to " << (top ? "top" : "bottom")
           << " of ScrubStack" << dendl;
...
void
ScrubStack::purge_scrub_counters(std::string_view tag)
{
...
ScrubStack::purge_old_scrub_counters()
{
  ...
  auto review_period =
      ceph::make_timespan(_mds_scrub_stats_review_period * 24 * 60 * 60);
  ...
  for (mds_rank_t rank = 0; rank < (mds_rank_t)mds_scrub_stats.size(); rank++) {
    ...
    for (auto it = counters.begin(); it != counters.end();) {
      ...
      dout(20) << __func__ << " rank(" << rank
               << ") :" << " elapsed:" << elapsed << dendl;
      ++it;
...
void
ScrubStack::init_scrub_counters(std::string_view path, std::string_view tag)
{
...
int
ScrubStack::enqueue(CInode* in, ScrubHeaderRef& header, bool top)
{
  // abort in progress
  if (clear_stack)
    return -EAGAIN;
  ...
  if (!ret.second) {
    dout(10) << __func__ << " with {" << *in << "}" << ", conflicting tag "
             << header->get_tag() << dendl;
    return -EEXIST;
  }
...
void
ScrubStack::add_to_waiting(MDSCacheObject* obj)
{
  scrubs_in_progress++;
  obj->item_scrub.remove_myself();
...
void
ScrubStack::remove_from_waiting(MDSCacheObject* obj, bool kick)
{
  scrubs_in_progress--;
  if (obj->item_scrub.is_on_list()) {
    obj->item_scrub.remove_myself();
...
bool
ScrubStack::validate_inode_auth(CInode* in)
{
  if (in->is_auth()) {
    if (!in->can_auth_pin()) {
    ...
    if (in->is_ambiguous_auth()) {
      ...
      in->add_waiter(
          MDSCacheObject::WAIT_SINGLEAUTH, new C_RetryScrub(this, in));
      ...
      auto ret = remote_scrubs.emplace(
          std::piecewise_construct, std::forward_as_tuple(in),
      ...
      auto r = make_message<MMDSScrub>(
          MMDSScrub::OP_QUEUEINO, in->ino(),
          std::move(in->scrub_queued_frags()), header->get_tag(),
          header->get_origin(), header->is_internal_tag(), header->get_force(),
...
void
ScrubStack::scrub_dir_inode(CInode* in, bool* added_children, bool* done)
{
  ...
  for (auto& fg : frags) {
    ...
    if (mds->damage_table.is_dirfrag_damaged(dir)) {
       ...
       */
      dout(5) << __func__ << ": not scrubbing damaged dirfrag: " << *dir
              << dendl;
      continue;
    } else if (!dir->is_auth()) {
  ...
  if (!scrub_remote.empty()) {
    ...
    for (auto& p : scrub_remote) {
      dout(20) << __func__ << " forward " << p.second << " to mds." << p.first
               << dendl;
      auto r = make_message<MMDSScrub>(
          MMDSScrub::OP_QUEUEDIR, in->ino(), std::move(p.second),
          header->get_tag(), header->get_origin(), header->is_internal_tag(),
          header->get_force(), header->get_recursive(), header->get_repair());
...
void
ScrubStack::scrub_dir_inode_final(CInode* in)
{
...
void
ScrubStack::identify_remote_link_damage(CDentry* dn)
{
...
void
ScrubStack::scrub_dirfrag(CDir* dir, bool* added_children, bool* done)
{
  ...
  if (header->get_recursive()) {
    ...
    for (auto it = dir->begin(); it != dir->end(); /* nop */) {
      ...
      if (dn->scrub(next_seq)) {
        clog->warn() << "Scrub error on dentry " << *dn << " see "
                     << g_conf()->name
      ...
      if (dn->get_version() <= last_scrub &&
          dnl->get_remote_d_type() != DT_DIR && !header->get_force()) {
  ...
  if (!dir->scrub_local()) {
    ...
    clog->warn() << "Scrub error on dir " << dir->ino() << " (" << path
                 << ") see " << g_conf()->name
                 << " log and `damage ls` output for details";
  }
...
void
ScrubStack::scrub_file_inode(CInode* in)
{
  C_InodeValidated* fin = new C_InodeValidated(mdcache->mds, this, in);
...
void
ScrubStack::_validate_inode_done(
    CInode* in,
    int r,
    const CInode::validated_data& result)
{
  ...
  if (result.backtrace.checked && !result.backtrace.passed &&
    ...
    mdcache->mds->damage_table.notify_remote_damaged(in->ino(), path);
      ...
      result.inode.checked && !result.inode.passed && !result.inode.repaired) {
  ...
  if (!result.passed_validation) {
    if (result.all_damage_repaired()) {
      ...
      clog->warn() << "Scrub error on inode " << in->ino() << " (" << path
                   << ") see " << g_conf()->name
                   << " log and `damage ls` output for details";
    }
...
void
ScrubStack::complete_control_contexts(int r)
{
  ceph_assert(ceph_mutex_is_locked_by_me(mdcache->mds->mds_lock));
...
void
ScrubStack::set_state(State next_state)
{
  if (state != next_state) {
    dout(20) << __func__ << ", from state=" << state
             << ", to state=" << next_state << dendl;
    state = next_state;
    clog_scrub_summary();
  }
}
...
void
ScrubStack::scrub_status(Formatter* f)
{
  ceph_assert(ceph_mutex_is_locked_by_me(mdcache->mds->mds_lock));
...
void
ScrubStack::abort_pending_scrubs()
{
  ceph_assert(ceph_mutex_is_locked_by_me(mdcache->mds->mds_lock));
...
void
ScrubStack::send_state_message(int op)
{
  MDSRank* mds = mdcache->mds;
...
void
ScrubStack::scrub_abort(Context* on_finish)
{
  ceph_assert(ceph_mutex_is_locked_by_me(mdcache->mds->mds_lock));

  dout(10) << __func__ << ": aborting with " << scrubs_in_progress
           << " scrubs in progress and " << stack_size << " in the" << " stack"
           << dendl;
...
void
ScrubStack::scrub_pause(Context* on_finish)
{
  ceph_assert(ceph_mutex_is_locked_by_me(mdcache->mds->mds_lock));
...
void
ScrubStack::clog_scrub_summary(CInode* in)
{
  if (in) {
    std::string what;
    if (clear_stack) {
...
void
ScrubStack::dispatch(const cref_t<Message>& m)
{
  switch (m->get_type()) {
  case MSG_MDS_SCRUB:
...
bool
ScrubStack::remove_inode_if_stacked(CInode* in)
{
...
void
ScrubStack::handle_scrub(const cref_t<MMDSScrub>& m)
{
  ...
  case MMDSScrub::OP_QUEUEDIR: {
    ...
    for (const auto& fg : m->get_frags()) {
      for (auto f : frags) {
        ...
        if (dir->is_dirty()) {
          dout(10) << __func__ << " found dirty remote dirfrag " << *dir
                   << dendl;
    ...
    if (!dfs.empty()) {
      ...
      if (auto it = scrubbing_map.find(m->get_tag());
          it != scrubbing_map.end()) {
        header = it->second;
      } else {
        header = std::make_shared<ScrubHeader>(
            m->get_tag(), m->is_internal_tag(), m->is_force(),
            m->is_recursive(), m->is_repair());
    ...
    auto r = make_message<MMDSScrub>(
        MMDSScrub::OP_QUEUEDIR_ACK, m->get_ino(), std::move(queued),
        m->get_tag(), inodeno_t(), false, false, false, false,
        has_dirty_dirfrag);
  ...
  case MMDSScrub::OP_QUEUEINO: {
    ...
    if (auto it = scrubbing_map.find(m->get_tag()); it != scrubbing_map.end()) {
      ...
      header = std::make_shared<ScrubHeader>(
          m->get_tag(), m->is_internal_tag(), m->is_force(), m->is_recursive(),
          m->is_repair());
      header->set_origin(m->get_origin());
      scrubbing_map.emplace(header->get_tag(), header);
    }
    ...
    fragset_t queued;
    auto r = make_message<MMDSScrub>(
        MMDSScrub::OP_QUEUEINO_ACK, m->get_ino(), std::move(queued),
        m->get_tag());
    mdcache->mds->send_message_mds(r, from);
  } break;
  ...
  case MMDSScrub::OP_RESUME:
    ...
    break;
  default:
    derr << " scrub stack unknown scrub operation " << m->get_op()
         << dendl_impl;
...
void
ScrubStack::handle_scrub_stats(const cref_t<MMDSScrubStats>& m)
{
  mds_rank_t from = mds_rank_t(m->get_source().num());
  ...
  if (from == 0) {
    ...
    std::unordered_map<
        std::string, std::unordered_map<int, std::vector<_inodeno_t>>>
    ...
    for (auto it = scrubbing_map.begin(); it != scrubbing_map.end();) {
      ...
      if (header->get_num_pending() ||
        ...
        std::vector<uint64_t> c{
            header->get_uninline_started(), header->get_uninline_passed(),
            header->get_uninline_failed(), header->get_uninline_skipped()};
    ...
    auto ack = make_message<MMDSScrubStats>(
        scrub_epoch, std::move(scrubbing_tags),
        std::move(uninline_failed_meta_info), std::move(paths),
        std::move(counters), clear_stack);
...
void
ScrubStack::move_uninline_failures_to_damage_table()
{
    ...
    for (const auto& [scrub_tag, errno_ino_vec_map] : ufmi) {
      for (const auto& [errno_, ino_vec] : errno_ino_vec_map) {
        for (auto ino : ino_vec) {
          mds->damage_table.notify_uninline_failed(
              ino, rank, errno_, scrub_tag, paths[ino]);
        }
      }
    }
    ufmi.clear();
...
ScrubStack::advance_scrub_status()
{
  ...
  if (up_max == 0) {
    ...
    for (const auto& stat : mds_scrub_stats) {
      ...
      scrubbing_tags.insert(
          stat.scrubbing_tags.begin(), stat.scrubbing_tags.end());
    }
    if (!any_aborting)
      scrub_any_peer_aborting = false;
    if (fully_acked) {
...
void
ScrubStack::handle_mds_failure(mds_rank_t mds)
{
  if (mds == 0) {
    scrub_abort(nullptr);
    return;
...
void
ScrubStack::uninline_data(CInode* in, Context* fin)
{
  dout(10) << "(uninline_data) starting data uninlining for " << *in << dendl;

  MDRequestRef mdr =
      in->mdcache->request_start_internal(CEPH_MDS_OP_UNINLINE_DATA);
...
void
ScrubStack::handle_conf_change(const std::set<std::string>& changed)
{
  if (changed.count("mds_scrub_stats_review_period")) {
    _mds_scrub_stats_review_period =
        g_conf().get_val<uint64_t>("mds_scrub_stats_review_period");
  }
}
