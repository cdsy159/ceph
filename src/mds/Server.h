...
public:
  ...
  void finish_reclaim_session(
      Session* session,
      const ref_t<MClientReclaimReply>& reply = nullptr);
  ...
  std::pair<bool, uint64_t> recall_client_state(
      MDSGatherBuilder* gather,
      RecallFlags = RecallFlags::NONE);
  void force_clients_readonly();

  // -- requests --
  void set_reply_extra_bl(
  ...
  void journal_and_reply(
      const MDRequestRef& mdr,
      CInode* tracei,
      CDentry* tracedn,
      LogEvent* le,
      MDSLogContextBase* fin);
  void submit_mdlog_entry(
      LogEvent* le,
      MDSLogContextBase* fin,
      const MDRequestRef& mdr,
  ...
  CInode* rdlock_path_pin_ref(
      ...
      bool want_auth,
      bool no_want_auth = false);
  CDentry* rdlock_path_xlock_dentry(
      const MDRequestRef& mdr,
      bool create,
      bool okexist = false,
      bool authexist = false,
      bool want_layout = false);
  std::pair<CDentry*, CDentry*> rdlock_two_paths_xlock_destdn(
      const MDRequestRef& mdr,
      bool xlock_srcdn);
  ...
  bool xlock_policylock(
      const MDRequestRef& mdr,
      CInode* in,
      bool want_layout = false,
      bool xlock_snaplock = false,
  ...
  int parse_quota_vxattr(
      std::string name,
      std::string value,
      quota_info_t* quota);
  void create_quota_realm(CInode* in);
  int parse_layout_vxattr_json(
      std::string name,
      std::string value,
      const OSDMap& osdmap,
      file_layout_t* layout);
  int parse_layout_vxattr_string(
      std::string name,
      std::string value,
      const OSDMap& osdmap,
      file_layout_t* layout);
  int parse_layout_vxattr(
      std::string name,
      std::string value,
      const OSDMap& osdmap,
      file_layout_t* layout,
      bool validate = true);
  int check_layout_vxattr(
      const MDRequestRef& mdr,
      std::string name,
      std::string value,
      file_layout_t* layout);
  ...
  void handle_peer_link_prep_ack(
      const MDRequestRef& mdr,
      const cref_t<MMDSPeerRequest>& m);
  void do_link_rollback(
      bufferlist& rbl,
      mds_rank_t leader,
      const MDRequestRef& mdr);
  void _link_rollback_finish(
      MutationRef& mut,
      const MDRequestRef& mdr,
  ...
  void _logged_peer_rmdir(
      const MDRequestRef& mdr,
      CDentry* srcdn,
      CDentry* straydn);
  void _commit_peer_rmdir(const MDRequestRef& mdr, int r, CDentry* straydn);
  void handle_peer_rmdir_prep_ack(
      const MDRequestRef& mdr,
      const cref_t<MMDSPeerRequest>& ack);
  void do_rmdir_rollback(
      bufferlist& rbl,
      mds_rank_t leader,
      const MDRequestRef& mdr);
  void _rmdir_rollback_finish(
      const MDRequestRef& mdr,
      metareqid_t reqid,
      CDentry* dn,
      CDentry* straydn);
  ...
  void handle_file_blockdiff_finish(
      const MDRequestRef& mdr,
      CInode* in,
  ...
  void handle_peer_rename_prep_ack(
      const MDRequestRef& mdr,
      const cref_t<MMDSPeerRequest>& m);
  void handle_peer_rename_notify_ack(
      const MDRequestRef& mdr,
      const cref_t<MMDSPeerRequest>& m);
  void _peer_rename_sessions_flushed(const MDRequestRef& mdr);
  void _logged_peer_rename(
      const MDRequestRef& mdr,
      CDentry* srcdn,
      CDentry* destdn,
      CDentry* straydn);
  void _commit_peer_rename(
      const MDRequestRef& mdr,
      int r,
      CDentry* srcdn,
      CDentry* destdn,
      CDentry* straydn);
  void do_rename_rollback(
      bufferlist& rbl,
      mds_rank_t leader,
      const MDRequestRef& mdr,
      bool finish_mdr = false);
  void _rename_rollback_finish(
      MutationRef& mut,
      const MDRequestRef& mdr,
      CDentry* srcdn,
      version_t srcdnpv,
      CDentry* destdn,
      CDentry* staydn,
      std::map<client_t, ref_t<MClientSnap>> splits[2],
      bool finish_mdr);
...
private:
  ...
  int default_xattr_validate(
      CInode* cur,
      const xattr_map_const_ptr xattrs,
      XattrOp* xattr_op);
  void default_setxattr_handler(
      CInode* cur,
      xattr_map_ptr xattrs,
      const XattrOp& xattr_op);
  void default_removexattr_handler(
      CInode* cur,
      xattr_map_ptr xattrs,
      const XattrOp& xattr_op);

  // mirror info xattr handler
  int parse_mirror_info_xattr(
      const std::string& name,
      const std::string& value,
      std::string& cluster_id,
      std::string& fs_id);
  int mirror_info_xattr_validate(
      CInode* cur,
      const xattr_map_const_ptr xattrs,
      XattrOp* xattr_op);
  void mirror_info_setxattr_handler(
      CInode* cur,
      xattr_map_ptr xattrs,
      const XattrOp& xattr_op);
  void mirror_info_removexattr_handler(
      CInode* cur,
      xattr_map_ptr xattrs,
      const XattrOp& xattr_op);

  static bool
  is_ceph_vxattr(std::string_view xattr_name)
  {
    return xattr_name.rfind("ceph.dir.layout", 0) == 0 ||
           ...
           xattr_name == "ceph.dir.subvolume" || xattr_name == "ceph.dir.pin" ||
  ...
  static bool
  is_ceph_dir_vxattr(std::string_view xattr_name)
  {
  ...
  static bool
  is_ceph_file_vxattr(std::string_view xattr_name)
  {
    return (
        xattr_name == "ceph.file.layout" ||
  ...
  static bool
  is_allowed_ceph_xattr(std::string_view xattr_name)
  {
  ...
  void reply_client_request(
      const MDRequestRef& mdr,
      const ref_t<MClientReply>& reply);
  void flush_session(Session* session, MDSGatherBuilder& gather);

  void _finalize_readdir(
      const MDRequestRef& mdr,
      CInode* diri,
      CDir* dir,
      bool start,
  ...
  std::set<client_t>
      client_reconnect_gather; // clients i need a reconnect msg from.
  std::set<client_t>
      client_reconnect_denied; // clients whose reconnect msg have been denied .
  ...
  size_t alternate_name_max =
      g_conf().get_val<Option::size_t>("mds_alternate_name_max");
  size_t fscrypt_last_block_max_size =
...
operator|(Server::RecallFlags a, Server::RecallFlags b)
{
...
operator<<(std::ostream& os, const Server::RecallFlags& f)
{
  using T = std::underlying_type<Server::RecallFlags>::type;
...
operator!(const Server::RecallFlags& f)
{
  using T = std::underlying_type<Server::RecallFlags>::type;
