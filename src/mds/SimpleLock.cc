...
SimpleLock::unstable_bits_t*
SimpleLock::more() const
{
  if (!_unstable)
...
void
SimpleLock::get_xlock(MutationRef who, client_t client)
{
  ceph_assert(!has_xlock_by());
  ceph_assert(
      state == LOCK_XLOCK || is_locallock() ||
...
void
SimpleLock::put_xlock()
{
  ceph_assert(
      state == LOCK_XLOCK || state == LOCK_XLOCKDONE ||
...
MutationRef
SimpleLock::get_xlock_by() const
{
  return have_more() ? more()->xlock_by : MutationRef();
}

void
SimpleLock::dump(ceph::Formatter* f) const
{
  ceph_assert(f != NULL);
...
std::list<SimpleLock>
SimpleLock::generate_test_instances()
{
...
int
SimpleLock::get_wait_shift() const
{
  switch (get_type()) {
  case CEPH_LOCK_DN:
    return 0;
  case CEPH_LOCK_DVERSION:
    return 1 * SimpleLock::WAIT_BITS;
  case CEPH_LOCK_IAUTH:
    return 2 * SimpleLock::WAIT_BITS;
  case CEPH_LOCK_ILINK:
    return 3 * SimpleLock::WAIT_BITS;
  case CEPH_LOCK_IDFT:
    return 4 * SimpleLock::WAIT_BITS;
  case CEPH_LOCK_IFILE:
    return 5 * SimpleLock::WAIT_BITS;
  case CEPH_LOCK_IVERSION:
    return 6 * SimpleLock::WAIT_BITS;
  case CEPH_LOCK_IXATTR:
    return 7 * SimpleLock::WAIT_BITS;
  case CEPH_LOCK_ISNAP:
    return 8 * SimpleLock::WAIT_BITS;
  case CEPH_LOCK_INEST:
    return 9 * SimpleLock::WAIT_BITS;
  case CEPH_LOCK_IFLOCK:
    return 10 * SimpleLock::WAIT_BITS;
  case CEPH_LOCK_IPOLICY:
    return 11 * SimpleLock::WAIT_BITS;
  case CEPH_LOCK_IQUIESCE:
    return 12 * SimpleLock::WAIT_BITS;
  default:
    ceph_abort();
  }
}
...
int
SimpleLock::get_cap_shift() const
{
  switch (get_type()) {
  case CEPH_LOCK_IAUTH:
...
void
SimpleLock::add_cache(MDLockCacheItem& item)
{
...
void
SimpleLock::remove_cache(MDLockCacheItem& item)
{
...
std::vector<MDLockCache*>
SimpleLock::get_active_caches()
{
...
void
SimpleLock::_print(std::ostream& out) const
{
