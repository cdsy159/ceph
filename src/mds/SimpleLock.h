...
public:
  ...
  get_sm() const
  {
    return (type != nullptr) ? type->sm : nullptr;
  }
  ...
  void
  decode_locked_state(const ceph::buffer::list& bl)
  {
    parent->decode_lock_state(type->type, bl);
  }

  void
  encode_locked_state(ceph::buffer::list& bl)
  {
  ...
  MDSCacheObject::waitmask_t
  getmask(uint64_t mask) const
  {
  ...
  void
  finish_waiters(uint64_t mask, int r = 0)
  {
    parent->finish_waiting(getmask(mask), r);
  }

  void
  take_waiting(uint64_t mask, std::vector<MDSContext*>& ls)
  {
    parent->take_waiting(getmask(mask), ls);
  }

  void
  add_waiter(uint64_t mask, MDSContext* c)
  {
    parent->add_waiter(getmask(mask) | WAIT_ORDERED, c);
  }

  bool
  is_waiter_for(uint64_t mask) const
  {
    return parent->is_waiter_for(getmask(mask));
  }

  bool
  has_any_waiter() const
  {
  ...
  void
  set_state_rejoin(int s, std::vector<MDSContext*>& waiters, bool survivor)
  {
  ...
  bool
  is_sync_and_unlocked() const
  {
    return get_state() == LOCK_SYNC && !is_rdlocked() && !is_leased() &&
           !is_wrlocked() && !is_xlocked();
  }
  ...
  */
  ...
  // can_*
  bool
  can_lease(client_t client) const
  {
    return get_sm()->states[state].can_lease == ANY ||
           (get_sm()->states[state].can_lease == AUTH && parent->is_auth()) ||
           (get_sm()->states[state].can_lease == XCL && client >= 0 &&
            get_xlock_by_client() == client);
  }

  bool
  can_read(client_t client) const
  {
    return get_sm()->states[state].can_read == ANY ||
           (get_sm()->states[state].can_read == AUTH && parent->is_auth()) ||
           (get_sm()->states[state].can_read == XCL && client >= 0 &&
            get_xlock_by_client() == client);
  }

  bool
  can_read_projected(client_t client) const
  {
    return get_sm()->states[state].can_read_projected == ANY ||
           (get_sm()->states[state].can_read_projected == AUTH &&
            parent->is_auth()) ||
           (get_sm()->states[state].can_read_projected == XCL && client >= 0 &&
            get_xlock_by_client() == client);
  }

  bool
  can_rdlock(client_t client) const
  {
  ...
  bool
  can_wrlock(client_t client) const
  {
  ...
  bool
  can_force_wrlock(client_t client) const
  {
    return get_sm()->states[state].can_force_wrlock == ANY ||
           (get_sm()->states[state].can_force_wrlock == AUTH &&
            parent->is_auth()) ||
  ...
  bool
  can_xlock(client_t client) const
  {
  ...
  bool
  is_xlocked_by_client(client_t c) const
  {
  ...
  bool
  has_xlock_by() const noexcept
  {
    return have_more() && more()->xlock_by;
  }

  bool
  is_xlocked_by(const MutationRef& who) const noexcept
  {
  ...
  void
  encode_state_for_replica(ceph::buffer::list& bl) const
  {
  ...
  void
  decode_state(ceph::buffer::list::const_iterator& p, bool is_new = true)
  {
    using ceph::decode;
    __s16 s;
    decode(s, p);
  ...
  int
  gcaps_allowed_ever() const
  {
    return parent->is_auth() ? get_sm()->allowed_ever_auth
                             : get_sm()->allowed_ever_replica;
  }

  int
  gcaps_allowed(int who, int s = -1) const
  {
    if (s < 0)
      s = state;
    if (parent->is_auth()) {
      if (get_xlock_by_client() >= 0 && who == CAP_XLOCKER)
        return get_sm()->states[s].xlocker_caps |
               get_sm()->states[s].caps; // xlocker always gets more
      else if (is_loner_mode() && who == CAP_ANY)
        return get_sm()->states[s].caps;
      else
        return get_sm()->states[s].loner_caps |
               get_sm()->states[s].caps; // loner always gets more
    } else
  ...
  int
  gcaps_xlocker_mask(client_t client) const
  {
    if (client == get_xlock_by_client())
      return type->type == CEPH_LOCK_IFILE
                 ? 0xf
                 : (CEPH_CAP_GSHARED | CEPH_CAP_GEXCL);
    return 0;
  }

  // simplelock specifics
  int
  get_replica_state() const
  {
...
private:
  ...
  struct unstable_bits_t {
    ...
    empty()
    {
      return gather_set.empty() && num_wrlock == 0 && num_xlock == 0 &&
             xlock_by.get() == NULL && xlock_by_client == -1 &&
             excl_client == -1 && lock_caches.empty();
    }
