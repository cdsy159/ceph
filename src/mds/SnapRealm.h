...
public:
  ...
  bool
  exists(std::string_view name) const
  {
  ...
  bool
  has_snaps_in_range(snapid_t first, snapid_t last)
  {
    check_cache();
  ...
  void
  add_cap(client_t client, Capability* cap)
  {
    auto em = client_caps.emplace(
        cap->get_client(), member_offset(Capability, item_snaprealm_caps));
    em.first->second.push_back(&cap->item_snaprealm_caps);
  }

  void
  remove_cap(client_t client, Capability* cap)
  {
  ...
  std::map<client_t, elist<Capability*>>
      client_caps; // to identify clients who need snap notifications
...
private:
  ...
  mutable snapid_t
      cached_last_created; // max last_created over all past+present parents
