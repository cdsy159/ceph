...
public:
  ...
  frag_t basefrag;
  __s32 bits{
      0}; // positive for split (from basefrag), negative for merge (to basefrag)
  ...
  EFragment(MDLog* mdlog, int o, dirfrag_t df, int b) :
    LogEvent(EVENT_FRAGMENT), op(o), ino(df.ino), basefrag(df.frag), bits(b)
  {}

  void
  print(std::ostream& out) const override
  {
    out << "EFragment " << op_name(op) << " " << ino << " " << basefrag
        << " by " << bits << " " << metablob;
  }
  ...
  void
  add_orig_frag(frag_t df, dirfrag_rollback* drb = NULL)
  {
    using ceph::encode;
