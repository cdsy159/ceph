...
public:
  ...
  struct fullbit {
    ...
    version_t dnv{0};
    CInode::inode_const_ptr
        inode; // if it's not XXX should not be part of mempool; wait for std::pmr to simplify
    ...
    fullbit(
        ...
        const CInode::xattr_map_const_ptr& xa,
        std::string_view sym,
        snapid_t os,
        const bufferlist& sbl,
        __u8 st,
    ...
    bool
    need_snapflush() const
    {
      return (state & STATE_NEED_SNAPFLUSH);
    }

    bool
    is_export_ephemeral_random() const
    {
      return (state & STATE_EPHEMERAL_RANDOM);
    }

    void
    print(std::ostream& out) const
    {
      out << " fullbit dn " << dn << " [" << dnfirst << "," << dnlast
          << "] dnv " << dnv << " inode " << inode->ino << " state=" << state;
  ...
  struct remotebit {
    ...
    print(std::ostream& out) const
    {
      out << " remotebit dn " << dn << " [" << dnfirst << "," << dnlast
          << "] dnv " << dnv << " ino " << ino << " dirty=" << dirty;
      if (!alternate_name.empty()) {
        out << " altn " << binstrprint(alternate_name, 8);
      }
      out << std::endl;
  ...
  struct nullbit {
    ...
    nullbit() :
      dnfirst(0), dnlast(0), dnv(0), dirty(false)
    {}
    ...
    print(std::ostream& out) const
    {
      out << " nullbit dn " << dn << " [" << dnfirst << "," << dnlast
          << "] dnv " << dnv << " dirty=" << dirty << std::endl;
    }
  };
...
public:
  ...
  public:
    dirlump() :
      state(0), nfull(0), nremote(0), nnull(0), dn_decoded(true)
    {}
    ...
    const std::vector<remotebit>&
    get_dremote() const
    {
      return dremote;
    }

    const std::vector<nullbit>&
    get_dnull() const
    {
      return dnull;
    }
    ...
    void
    print(dirfrag_t dirfrag, std::ostream& out) const
    {
      out << "dirlump " << dirfrag << " v " << fnode->version << " state "
          << state << " num " << nfull << "/" << nremote << "/" << nnull
          << std::endl;
      _decode_bits();
    ...
    void
    _encode_bits(uint64_t features) const
    {
      using ceph::encode;
      if (!dn_decoded)
        return;
...
private:
  ...
  std::map<inodeno_t, LogSegment::seq_t>
      truncate_finish; // finished truncate (started in segment blah)
...
public:
  ...
  void
  add_client_req(metareqid_t r, uint64_t tid = 0)
  {
    client_reqs.push_back(std::pair<metareqid_t, uint64_t>(r, tid));
  }

  void
  add_client_flush(metareqid_t r, uint64_t tid = 0)
  {
    client_flushes.push_back(std::pair<metareqid_t, uint64_t>(r, tid));
  }

  void
  add_table_transaction(int table, version_t tid)
  {
    table_tids.emplace_back(table, tid);
  }

  void
  add_opened_ino(inodeno_t ino)
  {
  ...
  void
  add_truncate_start(inodeno_t ino)
  {
    truncate_start.push_back(ino);
  }

  void
  add_truncate_finish(inodeno_t ino, uint64_t segoff)
  {
    truncate_finish[ino] = segoff;
  }

  bool rewrite_truncate_finish(
      MDSRank const* mds,
      std::map<uint64_t, uint64_t> const& old_to_new);

  void
  add_destroyed_inode(inodeno_t ino)
  {
    destroyed_inodes.push_back(ino);
  }

  void
  add_null_dentry(CDentry* dn, bool dirty)
  {
    add_null_dentry(add_dir(dn->get_dir(), false), dn, dirty);
  }

  void
  add_null_dentry(dirlump& lump, CDentry* dn, bool dirty)
  {
    // add the dir
    dn->check_corruption(false);
    lump.nnull++;
    lump.add_dnull(
        dn->get_name(), dn->first, dn->last, dn->get_projected_version(), dirty);
  }

  void
  add_remote_dentry(CDentry* dn, bool dirty)
  {
    add_remote_dentry(add_dir(dn->get_dir(), false), dn, dirty, 0, 0);
  }

  void
  add_remote_dentry(CDentry* dn, bool dirty, inodeno_t rino, int rdt)
  {
      ...
      unsigned char rdt = 0)
  {
    ...
    lump.nremote++;
    lump.add_dremote(
        dn->get_name(), dn->get_alternate_name(), dn->first, dn->last,
      ...
      bool need_snapflush = false)
  {
    __u8 state = 0;
    if (dirty)
      state |= fullbit::STATE_DIRTY;
    if (dirty_parent)
      state |= fullbit::STATE_DIRTYPARENT;
    if (dirty_pool)
      state |= fullbit::STATE_DIRTYPOOL;
    if (need_snapflush)
      state |= fullbit::STATE_NEED_SNAPFLUSH;
    add_primary_dentry(add_dir(dn->get_dir(), false), dn, in, state);
  }

  void
  add_primary_dentry(dirlump& lump, CDentry* dn, CInode* in, __u8 state)
  {
    dn->check_corruption(false);
    ...
    lump.add_dfull(
        ...
        in->get_projected_xattrs(), in->symlink, in->oldest_snap, snapbl, state,
        in->get_old_inodes());
  ...
  void
  add_dentry(CDentry* dn, bool dirty)
  {
  ...
  add_root(bool dirty, CInode* in)
  {
    ...
    std::string empty;
    roots.emplace_back(
        empty, "", in->first, in->last, 0, pi, pdft, px, in->symlink,
        in->oldest_snap, snapbl, (dirty ? fullbit::STATE_DIRTY : 0),
        in->get_old_inodes());
  }

  dirlump&
  add_dir(CDir* dir, bool dirty, bool complete = false)
  {
    return add_dir(dir->dirfrag(), dir->get_projected_fnode(), dirty, complete);
  }

  dirlump&
  add_new_dir(CDir* dir)
  {
    return add_dir(
        dir->dirfrag(), dir->get_projected_fnode(), true, true,
        true); // dirty AND complete AND new
  }

  dirlump&
  add_import_dir(CDir* dir)
  {
    // dirty=false would be okay in some cases
    return add_dir(
        dir->dirfrag(), dir->get_projected_fnode(), dir->is_dirty(),
        dir->is_complete(), false, true, dir->is_dirty_dft());
  }

  dirlump&
  add_fragmented_dir(CDir* dir, bool dirty, bool dirtydft)
  {
    return add_dir(
        dir->dirfrag(), dir->get_projected_fnode(), dirty, false, false, false,
        dirtydft);
  }
  ...
  add_dir(
      dirfrag_t df,
      const CDir::fnode_const_ptr& pf,
      bool dirty,
      bool complete = false,
      bool isnew = false,
      bool importing = false,
      bool dirty_dft = false)
  {
  ...
  empty()
  {
    return roots.empty() && lump_order.empty() && table_tids.empty() &&
           ...
           destroyed_inodes.empty() && client_reqs.empty() && opened_ino == 0 &&
