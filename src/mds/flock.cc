...
std::ostream&
operator<<(std::ostream& out, const ceph_filelock& l)
{
...
static void
remove_global_waiting(ceph_filelock& fl, ceph_lock_state_t* lock_state)
{
...
void
ceph_lock_state_t::encode(ceph::bufferlist& bl) const
{
  using ceph::encode;
...
void
ceph_lock_state_t::decode(ceph::bufferlist::const_iterator& bl)
{
  using ceph::decode;
...
void
ceph_lock_state_t::dump(ceph::Formatter* f) const
{
...
std::list<ceph_lock_state_t>
ceph_lock_state_t::generate_test_instances()
{
...
bool
ceph_lock_state_t::is_waiting(const ceph_filelock& fl) const
{
...
void
ceph_lock_state_t::remove_waiting(const ceph_filelock& fl)
{
...
bool
ceph_lock_state_t::is_deadlock(
    const ceph_filelock& fl,
    ...
    unsigned depth) const
{
  ...
  for (auto p = overlapping_locks.begin(); p != overlapping_locks.end(); ++p) {
    ...
    if (fl.type == CEPH_LOCK_SHARED && (*p)->second.type == CEPH_LOCK_SHARED)
      continue;
  ...
  for (auto p = lock_owners.begin(); p != lock_owners.end(); ++p) {
    ...
    for (auto q = global_waiting_locks.lower_bound(*p);
         q != global_waiting_locks.end(); ++q) {
      if (!ceph_filelock_owner_equal(q->first, *p))
        break;

      list<multimap<uint64_t, ceph_filelock>::iterator> _overlapping_locks,
          _self_overlapping_locks;
      ceph_lock_state_t& state = *(q->second);
      if (state.get_overlapping_locks(q->first, _overlapping_locks)) {
        state.split_by_owner(
            q->first, _overlapping_locks, _self_overlapping_locks);
      }
...
void
ceph_lock_state_t::add_waiting(const ceph_filelock& fl)
{
  waiting_locks.insert(pair<uint64_t, ceph_filelock>(fl.start, fl));
  ++client_waiting_lock_counts[(client_t)fl.client];
  if (type == CEPH_LOCK_FCNTL) {
    global_waiting_locks.insert(
        pair<ceph_filelock, ceph_lock_state_t*>(fl, this));
  }
}

bool
ceph_lock_state_t::add_lock(
    ...
    bool* deadlock)
{
  ...
  bool ret = false;
  list<multimap<uint64_t, ceph_filelock>::iterator> overlapping_locks,
      self_overlapping_locks, neighbor_locks;
  ...
  if (!overlapping_locks.empty()) { //overlapping locks owned by others :(
    if (CEPH_LOCK_EXCL == new_lock.type) {
      ...
      ldout(cct, 15)
          << "overlapping lock, and this lock is exclusive, can't set" << dendl;
      ...
      if (contains_exclusive_lock(overlapping_locks)) { //blocked :(
        ...
        held_locks.insert(
            pair<uint64_t, ceph_filelock>(new_lock.start, new_lock));
        ret = true;
      }
    }
...
void
ceph_lock_state_t::look_for_lock(ceph_filelock& testing_lock)
{
  list<multimap<uint64_t, ceph_filelock>::iterator> overlapping_locks,
...
void
ceph_lock_state_t::remove_lock(
    ceph_filelock removal_lock,
    list<ceph_filelock>& activated_locks)
{
  ...
  for (list<multimap<uint64_t, ceph_filelock>::iterator>::iterator iter =
           self_overlapping_locks.begin();
       iter != self_overlapping_locks.end(); ++iter) {
    ...
    if (remove_to_end) {
      ...
      held_locks.insert(
          pair<uint64_t, ceph_filelock>(append_lock.start, append_lock));
...
bool
ceph_lock_state_t::remove_all_from(client_t client)
{
...
void
ceph_lock_state_t::adjust_locks(
    list<multimap<uint64_t, ceph_filelock>::iterator> old_locks,
    ceph_filelock& new_lock,
    list<multimap<uint64_t, ceph_filelock>::iterator> neighbor_locks)
{
  ...
  for (list<multimap<uint64_t, ceph_filelock>::iterator>::iterator iter =
           neighbor_locks.begin();
       iter != neighbor_locks.end(); ++iter) {
    old_lock = &(*iter)->second;
    old_lock_client = old_lock->client;
    ...
    uint64_t end)
{
  ...
  ldout(cct, 15) << "share_space got start: " << start << ", end: " << end
                 << ", lock: " << iter->second << ", returning " << ret
                 << dendl;
  return ret;
}
...
bool
ceph_lock_state_t::get_waiting_overlaps(
    const ceph_filelock& lock,
    list<multimap<uint64_t, ceph_filelock>::iterator>& overlaps)
{
...
void
ceph_lock_state_t::split_by_owner(
    const ceph_filelock& owner,
    list<multimap<uint64_t, ceph_filelock>::iterator>& locks,
    list<multimap<uint64_t, ceph_filelock>::iterator>& owned_locks)
{
  list<multimap<uint64_t, ceph_filelock>::iterator>::iterator iter =
      locks.begin();
...
ceph_filelock*
ceph_lock_state_t::contains_exclusive_lock(
    list<multimap<uint64_t, ceph_filelock>::iterator>& locks)
{
  for (list<multimap<uint64_t, ceph_filelock>::iterator>::iterator iter =
           locks.begin();
       iter != locks.end(); ++iter) {
    if (CEPH_LOCK_EXCL == (*iter)->second.type)
      return &(*iter)->second;
  }
  return NULL;
}
...
std::ostream&
operator<<(std::ostream& out, const ceph_lock_state_t& l)
{
  out << "ceph_lock_state_t. held_locks.size()=" << l.held_locks.size()
      ...
      << "\n held_locks -- ";
  for (auto iter = l.held_locks.begin(); iter != l.held_locks.end(); ++iter)
    out << iter->second;
  out << "\n waiting_locks -- ";
  for (auto iter = l.waiting_locks.begin(); iter != l.waiting_locks.end();
       ++iter)
    out << iter->second << "\n";
  return out;
}
