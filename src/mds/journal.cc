...
struct BatchCommitBacktrace : public Context {
  ...
  finish(int r) override
  {
    ...
    gather.set_finisher(new C_OnFinisher(
        new BatchStoredBacktrace(mds, fin, std::move(ops_vec)), mds->finisher));
    gather.activate();
  }
};

void
LogSegment::try_to_expire(MDSRank* mds, MDSGatherBuilder& gather_bld, int op_prio)
{
  set<CDir*> commit;

  dout(6) << "LogSegment(" << seq << "/" << offset << ").try_to_expire"
          << dendl;
  ...
  for (set<metareqid_t>::iterator p = uncommitted_leaders.begin();
       p != uncommitted_leaders.end(); ++p) {
    dout(10) << "try_to_expire waiting for peers to ack commit on " << *p
             << dendl;
  ...
  for (set<metareqid_t>::iterator p = uncommitted_peers.begin();
       p != uncommitted_peers.end(); ++p) {
    dout(10) << "try_to_expire waiting for leader to ack OP_FINISH on " << *p
             << dendl;
  ...
  for (set<dirfrag_t>::iterator p = uncommitted_fragments.begin();
       p != uncommitted_fragments.end(); ++p) {
    dout(10) << "try_to_expire waiting for uncommitted fragment " << *p
             << dendl;
  ...
  for (elist<CInode*>::iterator p = dirty_dirfrag_dirfragtree.begin(); !p.end();
       ++p) {
    CInode* in = *p;
    dout(10) << "try_to_expire waiting for dirfragtreelock flush on " << *in
             << dendl;
  ...
  if (!open_files.empty()) {
    ...
    elist<CInode*>::iterator p =
        open_files.begin(member_offset(CInode, item_open_file));
    while (!p.end()) {
      CInode* in = *p;
      ++p;
      if (in->last != CEPH_NOSNAP && in->is_auth() &&
          !in->client_snap_caps.empty()) {
        // journal snap inodes that need flush. This simplify the mds failover hanlding
        dout(20) << "try_to_expire requeueing snap needflush inode " << *in
                 << dendl;
        if (!le) {
  ...
  for (auto& [pool_id, ops_vec] : ops_vec_map) {
    mds->finisher->queue(new BatchCommitBacktrace(
  ...
  if (inotablev > mds->inotable->get_committed_version()) {
    dout(10) << "try_to_expire saving inotable table, need " << inotablev
             ...
             << " (" << mds->inotable->get_committing_version() << ")" << dendl;
  ...
  if (sessionmapv > mds->sessionmap.get_committed()) {
    dout(10) << "try_to_expire saving sessionmap, need " << sessionmapv
             ...
             << mds->sessionmap.get_committing() << ")" << dendl;
  ...
  for (auto p = pending_commit_tids.begin(); p != pending_commit_tids.end();
       ++p) {
    MDSTableClient* client = mds->get_table_client(p->first);
    ceph_assert(client);
    for (auto q = p->second.begin(); q != p->second.end(); ++q) {
      dout(10) << "try_to_expire " << get_mdstable_name(p->first)
               << " transaction " << *q
  ...
  // table servers
  for (map<int, version_t>::iterator p = tablev.begin(); p != tablev.end();
       ++p) {
  ...
  if (purging_inodes.size()) {
    dout(10) << "try_to_expire waiting for purge of " << purging_inodes
             << dendl;
    set_purged_cb(gather_bld.new_sub());
  }

  if (gather_bld.has_subs()) {
    dout(6) << "LogSegment(" << seq << "/" << offset
...
void
LogSegment::purge_inodes_finish(interval_set<inodeno_t>& inos)
{
...
void
EMetaBlob::add_dir_context(CDir* dir, int mode)
{
  ...
  while (true) {
    ...
    if (lump_map.count(dir->dirfrag())) {
      dout(20) << "EMetaBlob::add_dir_context(" << dir << ") have lump "
               << dir->dirfrag() << dendl;
      break;
    }
    ...
    if (mode == TO_AUTH_SUBTREE_ROOT) {
      ...
      if (dir->is_subtree_root()) {
        ...
        if (dir->get_dir_auth().first == mds->get_nodeid()) {
          mds_authority_t parent_auth = parent ? parent->authority()
                                               : CDIR_AUTH_UNDEF;
          if (parent_auth.first == dir->get_dir_auth().first) {
            if (parent_auth.second == CDIR_AUTH_UNKNOWN &&
              ...
              dout(0) << "EMetaBlob::add_dir_context unexpected subtree "
                      << *dir << dendl;
              ceph_abort();
            }
      ...
      if (!maybenot && diri->last_journaled >= last_major_segment_seq) {
        dout(20) << "EMetaBlob::add_dir_context(" << dir
                 ...
                 << diri->last_journaled << " >= " << last_major_segment_seq
    ...
    if (maybenot) {
      dout(25) << "EMetaBlob::add_dir_context(" << dir << ")      maybe "
               << *parent << dendl;
      maybe.push_front(parent);
    } else {
      dout(25) << "EMetaBlob::add_dir_context(" << dir << ") definitely "
               << *parent << dendl;
...
void
EMetaBlob::update_segment(LogSegmentRef const& ls)
{
  // dirty inode mtimes
...
void
EMetaBlob::fullbit::encode(bufferlist& bl, uint64_t features) const
{
  ENCODE_START(9, 5, bl);
  encode(dn, bl);
  encode(dnfirst, bl);
...
void
EMetaBlob::fullbit::decode(bufferlist::const_iterator& bl)
{
  DECODE_START(9, bl);
  decode(dn, bl);
  decode(dnfirst, bl);
...
void
EMetaBlob::fullbit::dump(Formatter* f) const
{
...
std::list<EMetaBlob::fullbit>
EMetaBlob::fullbit::generate_test_instances()
{
  ...
  ls.emplace_back(
      "/testdn", "", 0, 0, 0, _inode, fragtree, _xattrs, "", 0, empty_snapbl,
      false, nullptr);
  return ls;
}

void
EMetaBlob::fullbit::update_inode(MDSRank* mds, CInode* in)
{
...
void
EMetaBlob::remotebit::encode(bufferlist& bl) const
{
  ENCODE_START(3, 2, bl);
  encode(dn, bl);
  encode(
...
void
EMetaBlob::remotebit::decode(bufferlist::const_iterator& bl)
{
...
void
EMetaBlob::remotebit::dump(Formatter* f) const
{
  f->dump_string("dentry", dn);
...
std::list<EMetaBlob::remotebit>
EMetaBlob::remotebit::generate_test_instances()
{
...
void
EMetaBlob::nullbit::encode(bufferlist& bl) const
{
  ENCODE_START(2, 2, bl);
  encode(dn, bl);
  encode(
...
void
EMetaBlob::nullbit::decode(bufferlist::const_iterator& bl)
{
...
void
EMetaBlob::nullbit::dump(Formatter* f) const
{
  f->dump_string("dentry", dn);
  f->dump_int("snapid.first", dnfirst);
  f->dump_int("snapid.last", dnlast);
...
auto
EMetaBlob::nullbit::generate_test_instances() -> std::list<nullbit>
{
  std::list<nullbit> ls;
...
void
EMetaBlob::dirlump::encode(bufferlist& bl, uint64_t features) const
{
  ENCODE_START(2, 2, bl);
  encode(*fnode, bl);
  encode(
...
void
EMetaBlob::dirlump::decode(bufferlist::const_iterator& bl)
{
  DECODE_START_LEGACY_COMPAT_LEN(2, 2, 2, bl)
  {
...
void
EMetaBlob::dirlump::dump(Formatter* f) const
{
  if (!dn_decoded) {
...
auto
EMetaBlob::dirlump::generate_test_instances() -> std::list<dirlump>
{
  std::list<dirlump> ls;
  ls.emplace_back();
  dirlump& dl = ls.back();
 ...
 */
void
EMetaBlob::encode(bufferlist& bl, uint64_t features) const
{
  ENCODE_START(8, 5, bl);
...
void
EMetaBlob::decode(bufferlist::const_iterator& bl)
{
 ...
 */
void
EMetaBlob::get_inodes(std::set<inodeno_t>& inodes) const
{
  // For all dirlumps in this metablob
  for (std::map<dirfrag_t, dirlump>::const_iterator i = lump_map.begin();
       i != lump_map.end(); ++i) {
 ...
 */
void
EMetaBlob::get_dentries(
    std::map<dirfrag_t, std::set<std::string>>& dentries) const
{
  for (std::map<dirfrag_t, dirlump>::const_iterator i = lump_map.begin();
       i != lump_map.end(); ++i) {
    dirlump const& dl = i->second;
 ...
 */
void
EMetaBlob::get_paths(std::vector<std::string>& paths) const
{
  ...
  for (std::vector<Location>::iterator i = leaf_locations.begin();
       i != leaf_locations.end(); ++i) {
...
void
EMetaBlob::dump(Formatter* f) const
{
...
std::list<EMetaBlob>
EMetaBlob::generate_test_instances()
{
...
EMetaBlob::replay(
    MDSRank* mds,
    LogSegmentRef const& logseg,
    int type,
    MDPeerUpdate* peerup)
{
  dout(10) << "EMetaBlob.replay " << lump_map.size() << " dirlumps by "
           << client_name << dendl;
  ...
  for (auto& p : roots) {
    ...
    dout(10) << "EMetaBlob.replay "
             << (isnew ? " added root " : " updated root ") << *in << dendl;
  }
  ...
  if (renamed_dirino) {
    ...
    if (renamed_diri)
      dout(10) << "EMetaBlob.replay renamed inode is " << *renamed_diri
               << dendl;
    else
      dout(10) << "EMetaBlob.replay don't have renamed ino " << renamed_dirino
               << dendl;
  ...
  for (const auto& lp : lump_order) {
    ...
    if (lump.is_dirty()) {
      ...
      if (!(dir->get_fnode()->rstat == dir->get_fnode()->accounted_rstat)) {
        ...
        logseg->dirty_dirfrag_nest.push_back(
            &dir->inode->item_dirty_dirfrag_nest);
      } else {
    ...
    if (lump.is_dirty_dft()) {
      dout(10) << "EMetaBlob.replay      dirty dirfragtree on " << *dir
               << dendl;
      dir->state_set(CDir::STATE_DIRTYDFT);
      mds->locker->mark_updated_scatterlock(&dir->inode->dirfragtreelock);
      logseg->dirty_dirfrag_dirfragtree.push_back(
          &dir->inode->item_dirty_dirfrag_dirfragtree);
    }
    if (lump.is_new())
      dir->mark_new(logseg);
    if (lump.is_complete())
    ...
    for (auto& fb : lump._get_dfull()) {
      ...
      if (!dn) {
        ...
        if (fb.is_dirty())
          dn->_mark_dirty(logseg);
        dout(10) << "EMetaBlob.replay for [" << fb.dnfirst << "," << fb.dnlast
                 << "] had " << *dn << dendl;
        dn->first = fb.dnfirst;
      ...
      if (!in) {
        ...
        if (!dn->get_linkage()->is_null()) {
          if (dn->get_linkage()->is_primary()) {
            ...
            CachedStackStringStream css;
            *css << "EMetaBlob.replay FIXME had dentry linked to wrong inode "
        ...
        if (dn->get_linkage()->get_inode() != in) {
          if (!dn->get_linkage()
                   ->is_null()) { // note: might be remote.  as with stray reintegration.
            if (dn->get_linkage()->is_primary()) {
              unlinked[dn->get_linkage()->get_inode()] = dir;
              CachedStackStringStream css;
              *css << "EMetaBlob.replay FIXME had dentry linked to wrong inode "
    ...
    for (const auto& rb : lump.get_dremote()) {
      ...
      if (!dn) {
        dn = dir->add_remote_dentry(
            rb.dn, rb.ino, rb.d_type,
            mempool::mds_co::string(rb.alternate_name), rb.dnfirst, rb.dnlast);
        ...
        if (rb.dirty)
          dn->_mark_dirty(logseg);
        dout(10) << "EMetaBlob.replay for [" << rb.dnfirst << "," << rb.dnlast
                 << "] had " << *dn << dendl;
        dn->first = rb.dnfirst;
  ...
  if (renamed_dirino) {
    ...
    if (renamed_diri->authority() != CDIR_AUTH_UNDEF) {
      for (const auto& p : renamed_dir_frags) {
        ...
        if (dir) {
          ...
          dout(10) << " already had+adjusted rename import bound " << *dir
                   << dendl;
          ceph_assert(olddir);
          continue;
        }
    ...
    for (map<CInode*, CDir*>::iterator p = unlinked.begin();
         p != unlinked.end(); ++p) {
  ...
  if (inotablev) {
    if (mds->inotable->get_version() >= inotablev ||
      ...
      dout(10) << "EMetaBlob.replay inotable v " << inotablev
               ...
               << " allocated+used " << allocated_ino << " prealloc "
               << preallocated_inos << dendl;
      if (allocated_ino)
  ...
  if (sessionmapv) {
    if (mds->sessionmap.get_version() >= sessionmapv ||
      ...
      dout(10) << "EMetaBlob.replay sessionmap v " << sessionmapv << ", table "
               << mds->sessionmap.get_version() << " prealloc "
               << preallocated_inos << " used " << used_preallocated_ino
               << dendl;
      Session* session = mds->sessionmap.get_session(client_name);
      if (session) {
        dout(20) << " (session prealloc " << session->info.prealloc_inos << ")"
                 << dendl;
        if (used_preallocated_ino) {
  ...
  if (!destroyed_inodes.empty()) {
    for (vector<inodeno_t>::iterator p = destroyed_inodes.begin();
         p != destroyed_inodes.end(); ++p) {
      CInode* in = mds->mdcache->get_inode(*p);
      if (in) {
        dout(10) << "EMetaBlob.replay destroyed " << *p << ", dropping " << *in
                 << dendl;
        CDentry* parent = in->get_parent_dn();
        mds->mdcache->remove_inode(in);
        if (parent) {
          dout(10) << "EMetaBlob.replay unlinked from dentry " << *parent
                   << dendl;
          ceph_assert(parent->get_linkage()->is_null());
        }
      } else {
        dout(10) << "EMetaBlob.replay destroyed " << *p << ", not in cache"
                 << dendl;
      }
    ...
    mds->mdcache->open_file_table.note_destroyed_inos(
        logseg->seq, destroyed_inodes);
  }

  // client requests
  for (const auto& p : client_reqs) {
    if (p.first.name.is_client()) {
      dout(10) << "EMetaBlob.replay request " << p.first << " trim_to "
               << p.second << dendl;
  ...
  for (const auto& p : client_flushes) {
    if (p.first.name.is_client()) {
      dout(10) << "EMetaBlob.replay flush " << p.first << " trim_to "
               << p.second << dendl;
      Session* session = mds->sessionmap.get_session(p.first.name);
      if (session) {
...
void
EPurged::encode(bufferlist& bl, uint64_t features) const
{
  ENCODE_START(1, 1, bl);
  encode(inos, bl);
  encode(inotablev, bl);
...
void
EPurged::decode(bufferlist::const_iterator& bl)
{
  DECODE_START(1, bl);
  decode(inos, bl);
  decode(inotablev, bl);
...
void
EPurged::dump(Formatter* f) const
{
    ...
    if (open) {
      ...
      if (session) { // there always should be a session, but there's a bug
        if (session->get_connection() == NULL) {
          ...
          dout(10) << " reset session " << session->info.inst
                   << " (they reconnected)" << dendl;
        }
      } else {
        mds->clog->error() << "replayed stray Session close event for "
                           << client_inst << " from time " << stamp
                           << ", ignoring";
      }
    }
    if (session) {
...
void
ESession::encode(bufferlist& bl, uint64_t features) const
{
  ENCODE_START(6, 5, bl);
  encode(stamp, bl);
...
void
ESession::decode(bufferlist::const_iterator& bl)
{
...
void
ESession::dump(Formatter* f) const
{
...
std::list<ESession>
ESession::generate_test_instances()
{
...
void
ESessions::encode(bufferlist& bl, uint64_t features) const
{
  ENCODE_START(2, 1, bl);
...
void
ESessions::decode_old(bufferlist::const_iterator& bl)
{
  using ceph::decode;
...
void
ESessions::decode_new(bufferlist::const_iterator& bl)
{
  DECODE_START(2, bl);
  decode(client_map, bl);
  decode(cmapv, bl);
...
void
ESessions::dump(Formatter* f) const
{
  f->dump_int("client map version", cmapv);
...
std::list<ESessions>
ESessions::generate_test_instances()
{
...
void
ESessions::update_segment()
{
  get_segment()->sessionmapv = cmapv;
}

void
ESessions::replay(MDSRank* mds)
{
  if (mds->sessionmap.get_version() >= cmapv) {
...
void
ETableServer::encode(bufferlist& bl, uint64_t features) const
{
  ENCODE_START(3, 3, bl);
  encode(stamp, bl);
  encode(table, bl);
...
void
ETableServer::decode(bufferlist::const_iterator& bl)
{
...
void
ETableServer::dump(Formatter* f) const
{
...
std::list<ETableServer>
ETableServer::generate_test_instances()
{
...
void
ETableServer::update_segment()
{
  get_segment()->tablev[table] = version;
}

void
ETableServer::replay(MDSRank* mds)
{
  ...
  if (server->get_version() >= version) {
    dout(10) << "ETableServer.replay " << get_mdstable_name(table) << " "
             << get_mdstableserver_opname(op) << " event " << version
...
void
ETableClient::encode(bufferlist& bl, uint64_t features) const
{
  ENCODE_START(3, 3, bl);
  encode(stamp, bl);
  encode(table, bl);
...
void
ETableClient::decode(bufferlist::const_iterator& bl)
{
  DECODE_START_LEGACY_COMPAT_LEN(3, 3, 3, bl);
  if (struct_v >= 2)
    decode(stamp, bl);
...
void
ETableClient::dump(Formatter* f) const
{
...
std::list<ETableClient>
ETableClient::generate_test_instances()
{
...
void
ETableClient::replay(MDSRank* mds)
{
  dout(10) << " ETableClient.replay " << get_mdstable_name(table) << " op "
           << get_mdstableserver_opname(op) << " tid " << tid << dendl;
...
void
EUpdate::encode(bufferlist& bl, uint64_t features) const
{
  ENCODE_START(4, 4, bl);
  encode(stamp, bl);
  encode(type, bl);
...
void
EUpdate::decode(bufferlist::const_iterator& bl)
{
...
void
EUpdate::dump(Formatter* f) const
{
...
std::list<EUpdate>
EUpdate::generate_test_instances()
{
  std::list<EUpdate> ls;
  ls.emplace_back();
  return ls;
...
EUpdate::replay(MDSRank* mds)
{
  ...
  if (client_map.length()) {
    if (mds->sessionmap.get_version() >= cmapv) {
      dout(10) << "EUpdate.replay sessionmap v " << cmapv << " <= table "
               << mds->sessionmap.get_version() << dendl;
    } else {
...
void
EOpen::encode(bufferlist& bl, uint64_t features) const
{
  ENCODE_START(4, 3, bl);
  encode(stamp, bl);
  encode(metablob, bl, features);
...
void
EOpen::decode(bufferlist::const_iterator& bl)
{
  DECODE_START_LEGACY_COMPAT_LEN(3, 3, 3, bl);
  if (struct_v >= 2)
    decode(stamp, bl);
...
void
EOpen::dump(Formatter* f) const
{
  f->open_object_section("metablob");
  metablob.dump(f);
  f->close_section(); // metablob
  f->open_array_section("inos involved");
  for (vector<inodeno_t>::const_iterator i = inos.begin(); i != inos.end();
       ++i) {
    f->dump_int("ino", *i);
  }
...
void
EOpen::update_segment()
{
  // ??
}

void
EOpen::replay(MDSRank* mds)
{
...
void
ECommitted::replay(MDSRank* mds)
{
  if (mds->mdcache->uncommitted_leaders.count(reqid)) {
    dout(10) << "ECommitted.replay " << reqid << dendl;
    mds->mdcache->uncommitted_leaders[reqid].ls->uncommitted_leaders.erase(
        reqid);
    mds->mdcache->uncommitted_leaders.erase(reqid);
  } else {
    dout(10) << "ECommitted.replay " << reqid << " -- didn't see original op"
             << dendl;
  }
}

void
ECommitted::encode(bufferlist& bl, uint64_t features) const
{
  ENCODE_START(3, 3, bl);
  encode(stamp, bl);
  encode(reqid, bl);
...
void
ECommitted::decode(bufferlist::const_iterator& bl)
{
  DECODE_START_LEGACY_COMPAT_LEN(3, 3, 3, bl);
  if (struct_v >= 2)
    decode(stamp, bl);
...
void
ECommitted::dump(Formatter* f) const
{
...
std::list<ECommitted>
ECommitted::generate_test_instances()
{
...
void
link_rollback::encode(bufferlist& bl) const
{
  ENCODE_START(3, 2, bl);
  encode(reqid, bl);
  encode(ino, bl);
...
void
link_rollback::decode(bufferlist::const_iterator& bl)
{
  DECODE_START_LEGACY_COMPAT_LEN(3, 2, 2, bl);
  decode(reqid, bl);
  decode(ino, bl);
...
void
link_rollback::dump(Formatter* f) const
{
...
std::list<link_rollback>
link_rollback::generate_test_instances()
{
...
void
rmdir_rollback::encode(bufferlist& bl) const
{
  ENCODE_START(3, 2, bl);
  encode(reqid, bl);
  encode(src_dir, bl);
...
void
rmdir_rollback::decode(bufferlist::const_iterator& bl)
{
  DECODE_START_LEGACY_COMPAT_LEN(3, 2, 2, bl);
  decode(reqid, bl);
  decode(src_dir, bl);
...
void
rmdir_rollback::dump(Formatter* f) const
{
  f->dump_stream("metareqid") << reqid;
...
std::list<rmdir_rollback>
rmdir_rollback::generate_test_instances()
{
...
void
rename_rollback::drec::encode(bufferlist& bl) const
{
  ENCODE_START(2, 2, bl);
  encode(dirfrag, bl);
...
void
rename_rollback::drec::decode(bufferlist::const_iterator& bl)
{
  DECODE_START_LEGACY_COMPAT_LEN(2, 2, 2, bl);
  decode(dirfrag, bl);
...
void
rename_rollback::drec::dump(Formatter* f) const
{
...
auto
rename_rollback::drec::generate_test_instances() -> std::list<drec>
{
  std::list<drec> ls;
  ls.push_back(drec());
...
void
rename_rollback::encode(bufferlist& bl) const
{
  ENCODE_START(3, 2, bl);
  encode(reqid, bl);
  encode(orig_src, bl);
...
void
rename_rollback::decode(bufferlist::const_iterator& bl)
{
  DECODE_START_LEGACY_COMPAT_LEN(3, 2, 2, bl);
  decode(reqid, bl);
  decode(orig_src, bl);
...
void
rename_rollback::dump(Formatter* f) const
{
...
std::list<rename_rollback>
rename_rollback::generate_test_instances()
{
...
void
EPeerUpdate::encode(bufferlist& bl, uint64_t features) const
{
  ENCODE_START(3, 3, bl);
  encode(stamp, bl);
  encode(type, bl);
...
void
EPeerUpdate::decode(bufferlist::const_iterator& bl)
{
  DECODE_START_LEGACY_COMPAT_LEN(3, 3, 3, bl);
  if (struct_v >= 2)
    decode(stamp, bl);
...
void
EPeerUpdate::dump(Formatter* f) const
{
  f->open_object_section("metablob");
  commit.dump(f);
  f->close_section(); // metablob
...
EPeerUpdate::generate_test_instances()
{
...
void
EPeerUpdate::replay(MDSRank* mds)
{
  MDPeerUpdate* su;
  auto&& segment = get_segment();
  switch (op) {
  ...
  case EPeerUpdate::OP_COMMIT:
    dout(10) << "EPeerUpdate.replay commit " << reqid << " for mds." << leader
             << dendl;
...
void
ESubtreeMap::encode(bufferlist& bl, uint64_t features) const
{
  ENCODE_START(6, 5, bl);
  encode(stamp, bl);
  encode(metablob, bl, features);
...
void
ESubtreeMap::decode(bufferlist::const_iterator& bl)
{
...
void
ESubtreeMap::dump(Formatter* f) const
{
  f->open_object_section("metablob");
  metablob.dump(f);
  f->close_section(); // metablob
...
std::list<ESubtreeMap>
ESubtreeMap::generate_test_instances()
{
...
void
ESubtreeMap::replay(MDSRank* mds)
{
  ...
  if (mds->mdcache->is_subtrees()) {
    dout(10) << "ESubtreeMap.replay -- i already have import map; verifying"
             << dendl;
    int errors = 0;
    ...
    for (map<dirfrag_t, vector<dirfrag_t>>::iterator p = subtrees.begin();
      ...
      if (dir->get_dir_auth().first != mds->get_nodeid()) {
        mds->clog->error() << " replayed ESubtreeMap at " << get_start_off()
                           << " subtree root " << p->first
                           << " is not mine in cache (it's "
                           << dir->get_dir_auth() << ")";
        ++errors;
        continue;
      }

      for (vector<dirfrag_t>::iterator q = p->second.begin();
           q != p->second.end(); ++q)
      ...
      for (vector<dirfrag_t>::iterator q = p->second.begin();
        ...
        if (!b) {
          mds->clog->error()
              << " replayed ESubtreeMap at " << get_start_off() << " subtree "
              << p->first << " bound " << *q << " not in cache";
          ++errors;
          continue;
        }
        if (bounds.count(b) == 0) {
          mds->clog->error()
              << " replayed ESubtreeMap at " << get_start_off() << " subtree "
              << p->first << " bound " << *q << " not a bound in cache";
          ++errors;
          continue;
        }
        bounds.erase(b);
      ...
      if (ambiguous_subtrees.count(p->first)) {
        if (!mds->mdcache->have_ambiguous_import(p->first)) {
          mds->clog->error()
              << " replayed ESubtreeMap at " << get_start_off() << " subtree "
              << p->first << " is ambiguous but is not in our cache";
          ++errors;
        }
      } else {
        if (mds->mdcache->have_ambiguous_import(p->first)) {
          mds->clog->error()
              << " replayed ESubtreeMap at " << get_start_off() << " subtree "
              << p->first << " is not ambiguous but is in our cache";
          ++errors;
        }
      }
    }
    ...
    for (const auto& dir : dirs) {
      ...
      if (subtrees.count(dir->dirfrag()) == 0) {
        mds->clog->error() << " replayed ESubtreeMap at " << get_start_off()
                           << " does not include cache subtree "
                           << dir->dirfrag();
        ++errors;
      }
    }
  ...
  dout(10)
      << "ESubtreeMap.replay -- reconstructing (auth) subtree spanning tree"
      << dendl;
...
EFragment::replay(MDSRank* mds)
{
  ...
  switch (op) {
  case OP_PREPARE:
    mds->mdcache->add_uncommitted_fragment(
        dirfrag_t(ino, basefrag), bits, orig_frags, segment, &rollback);

    if (in)
      mds->mdcache->adjust_dir_fragments(
          in, basefrag, bits, &resultfrags, waiters, true);
    break;
  ...
  case OP_ROLLBACK: {
    ...
    mds->mdcache->rollback_uncommitted_fragment(
        dirfrag_t(ino, basefrag), std::move(old_frags));
    break;
  }
...
void
EFragment::encode(bufferlist& bl, uint64_t features) const
{
  ENCODE_START(5, 4, bl);
  encode(stamp, bl);
  encode(op, bl);
...
void
EFragment::decode(bufferlist::const_iterator& bl)
{
...
void
EFragment::dump(Formatter* f) const
{
  /*f->open_object_section("Metablob");
...
std::list<EFragment>
EFragment::generate_test_instances()
{
...
void
dirfrag_rollback::encode(bufferlist& bl) const
{
  ENCODE_START(1, 1, bl);
  encode(*fnode, bl);
  ENCODE_FINISH(bl);
...
void
dirfrag_rollback::decode(bufferlist::const_iterator& bl)
{
  DECODE_START(1, bl);
  {
    auto _fnode = CDir::allocate_fnode();
...
void
EExport::encode(bufferlist& bl, uint64_t features) const
{
  ENCODE_START(4, 3, bl);
  encode(stamp, bl);
  encode(metablob, bl, features);
...
void
EExport::decode(bufferlist::const_iterator& bl)
{
  DECODE_START_LEGACY_COMPAT_LEN(3, 3, 3, bl);
  if (struct_v >= 2)
    decode(stamp, bl);
...
void
EExport::dump(Formatter* f) const
{
  ...
  for (set<dirfrag_t>::const_iterator i = bounds.begin(); i != bounds.end();
       ++i) {
    f->dump_stream("dirfrag") << *i;
  }
...
std::list<EExport>
EExport::generate_test_instances()
{
  std::list<EExport> ls;
  ls.emplace_back();
  return ls;
...
void
EImportStart::replay(MDSRank* mds)
{
  ...
  mds->mdcache->adjust_bounded_subtree_auth(
      dir, realbounds, mds_authority_t(mds->get_nodeid(), mds->get_nodeid()));

  // open client sessions?
  if (mds->sessionmap.get_version() >= cmapv) {
    dout(10) << "EImportStart.replay sessionmap "
             << mds->sessionmap.get_version() << " >= " << cmapv << ", noop"
             << dendl;
  } else {
    dout(10) << "EImportStart.replay sessionmap "
             << mds->sessionmap.get_version() << " < " << cmapv << dendl;
    map<client_t, entity_inst_t> cm;
    map<client_t, client_metadata_t> cmm;
    auto blp = client_map.cbegin();
    using ceph::decode;
...
void
EImportStart::encode(bufferlist& bl, uint64_t features) const
{
  ENCODE_START(4, 3, bl);
  encode(stamp, bl);
  encode(base, bl);
...
void
EImportStart::decode(bufferlist::const_iterator& bl)
{
  DECODE_START_LEGACY_COMPAT_LEN(3, 3, 3, bl);
  if (struct_v >= 2)
    decode(stamp, bl);
...
void
EImportStart::dump(Formatter* f) const
{
  f->dump_stream("base dirfrag") << base;
...
std::list<EImportStart>
EImportStart::generate_test_instances()
{
...
void
EImportFinish::replay(MDSRank* mds)
{
  if (mds->mdcache->have_ambiguous_import(base)) {
    ...
    dout(10) << "EImportFinish.replay " << base << " success=" << success
             << " on subtree not marked as ambiguous" << dendl;
...
void
EImportFinish::encode(bufferlist& bl, uint64_t features) const
{
  ENCODE_START(3, 3, bl);
  encode(stamp, bl);
  encode(base, bl);
...
void
EImportFinish::decode(bufferlist::const_iterator& bl)
{
  DECODE_START_LEGACY_COMPAT_LEN(3, 3, 3, bl);
  if (struct_v >= 2)
    decode(stamp, bl);
...
void
EImportFinish::dump(Formatter* f) const
{
  f->dump_stream("base dirfrag") << base;
  f->dump_string("success", success ? "true" : "false");
}

std::list<EImportFinish>
EImportFinish::generate_test_instances()
{
...
void
EResetJournal::encode(bufferlist& bl, uint64_t features) const
{
  ENCODE_START(2, 2, bl);
  encode(stamp, bl);
  ENCODE_FINISH(bl);
...
void
EResetJournal::decode(bufferlist::const_iterator& bl)
{
  DECODE_START_LEGACY_COMPAT_LEN(2, 2, 2, bl);
  decode(stamp, bl);
  DECODE_FINISH(bl);
...
void
EResetJournal::dump(Formatter* f) const
{
  f->dump_stream("timestamp") << stamp;
}

std::list<EResetJournal>
EResetJournal::generate_test_instances()
{
...
void
EResetJournal::replay(MDSRank* mds)
{
  ...
  if (mds->mdsmap->get_root() == mds->get_nodeid()) {
    CDir* rootdir =
        mds->mdcache->get_root()->get_or_open_dirfrag(mds->mdcache, frag_t());
    mds->mdcache->adjust_subtree_auth(rootdir, mds->get_nodeid());
  }

  CDir* mydir =
      mds->mdcache->get_myin()->get_or_open_dirfrag(mds->mdcache, frag_t());
...
void
ESegment::encode(bufferlist& bl, uint64_t features) const
{
  ENCODE_START(1, 1, bl);
  encode(seq, bl);
  ENCODE_FINISH(bl);
...
void
ESegment::decode(bufferlist::const_iterator& bl)
{
  DECODE_START(1, bl);
  decode(seq, bl);
  DECODE_FINISH(bl);
...
void
ESegment::replay(MDSRank* mds)
{
  dout(4) << "ESegment::replay, seq " << seq << dendl;
}

void
ESegment::dump(Formatter* f) const
{
  f->dump_int("seq", seq);
}

std::list<ESegment>
ESegment::generate_test_instances()
{
...
void
ELid::encode(bufferlist& bl, uint64_t features) const
{
  ENCODE_START(1, 1, bl);
  encode(seq, bl);
  ENCODE_FINISH(bl);
...
void
ELid::decode(bufferlist::const_iterator& bl)
{
  DECODE_START(1, bl);
  decode(seq, bl);
  DECODE_FINISH(bl);
...
void
ELid::replay(MDSRank* mds)
{
  dout(4) << "ELid::replay, seq " << seq << dendl;
}

void
ELid::dump(Formatter* f) const
{
  f->dump_int("seq", seq);
}

std::list<ELid>
ELid::generate_test_instances()
{
  std::list<ELid> ls;
  ls.emplace_back();
  return ls;
...
void
ENoOp::encode(bufferlist& bl, uint64_t features) const
{
  ENCODE_START(2, 2, bl);
  encode(pad_size, bl);
...
void
ENoOp::decode(bufferlist::const_iterator& bl)
{
  DECODE_START(2, bl);
  decode(pad_size, bl);
...
void
ENoOp::replay(MDSRank* mds)
{
  dout(4) << "ENoOp::replay, " << pad_size << " bytes skipped in journal"
          << dendl;
}
 ...
 */
bool
EMetaBlob::rewrite_truncate_finish(
    MDSRank const* mds,
    std::map<LogSegment::seq_t, LogSegment::seq_t> const& old_to_new)
{
  ...
  for (const auto& p : truncate_finish) {
    ...
    if (q != old_to_new.end()) {
      dout(20) << __func__ << " applying segment seq mapping " << p.second
               << " -> " << q->second << dendl;
      new_trunc_finish.emplace(p.first, q->second);
      modified = true;
    } else {
      dout(20) << __func__ << " no segment seq mapping found for " << p.second
               << dendl;
