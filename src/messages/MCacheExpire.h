...
public:
  ...
  */
  struct realm {
    ...
    std::map<dirfrag_t, std::map<std::pair<std::string, snapid_t>, uint32_t>>
        dentries;
...
protected:
  MCacheExpire() :
    MMDSOp{MSG_MDS_CACHEEXPIRE}, from(-1)
  {}

  MCacheExpire(int f) :
    MMDSOp{MSG_MDS_CACHEEXPIRE}, from(f)
  {}
...
public:
  ...
  void
  add_inode(dirfrag_t r, vinodeno_t vino, unsigned nonce)
  {
    realms[r].inodes[vino] = nonce;
  }

  void
  add_dir(dirfrag_t r, dirfrag_t df, unsigned nonce)
  {
  ...
  void
  add_realm(dirfrag_t df, const realm& r)
  {
    auto em = realms.emplace(
        std::piecewise_construct, std::forward_as_tuple(df),
