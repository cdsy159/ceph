...
public:
  ...
  bool
  is_fscrypt_enabled() const
  {
    return !!fscrypt_auth.size();
  }

  uint64_t
  effective_size() const
  {
  ...
  __u32
  get_truncate_seq() const
  {
    return truncate_seq;
  }

  uint64_t
  get_truncate_size() const
  {
  ...
  __u64
  get_change_attr() const
  {
    return change_attr;
  }

  __u32
  get_time_warp_seq() const
  {
  ...
  void
  set_layout(const file_layout_t& l)
  {
    layout = l;
  }

  int
  get_migrate_seq() const
  {
    return head.migrate_seq;
  }
  ...
  uint64_t
  get_client_tid() const
  {
    return get_tid();
  }

  void
  set_client_tid(uint64_t s)
  {
    set_tid(s);
  }

  snapid_t
  get_snap_follows() const
  {
    return snapid_t(head.snap_follows);
  }

  void
  set_snap_follows(snapid_t s)
  {
    head.snap_follows = s;
  }
  ...
  void
  set_max_size(uint64_t ms)
  {
    max_size = ms;
  }

  void
  set_migrate_seq(unsigned m)
  {
    head.migrate_seq = m;
  }
  ...
  void
  set_mtime(const utime_t& t)
  {
    mtime = t;
  }

  void
  set_ctime(const utime_t& t)
  {
    ctime = t;
  }

  void
  set_atime(const utime_t& t)
  {
    atime = t;
  }
  ...
  void
  set_oldest_flush_tid(ceph_tid_t tid)
  {
    oldest_flush_tid = tid;
  }

  ceph_tid_t
  get_oldest_flush_tid() const
  {
...
public:
  ...
  {
    out << "client_caps(" << ceph_cap_op_name(head.op) << " ino "
        << inodeno_t(head.ino) << " " << head.cap_id << " seq " << head.seq
    ...
    out << " mtime " << mtime << " ctime " << ctime << " change_attr "
        << change_attr;
    if (time_warp_seq)
