...
protected:
  ...
  MClientLease(const MClientLease& m) :
    SafeMessage(CEPH_MSG_CLIENT_LEASE), h(m.h), dname(m.dname)
  {}
  ...
  MClientLease(
      ...
      uint64_t sl,
      std::string_view d) :
    SafeMessage(CEPH_MSG_CLIENT_LEASE), dname(d)
  {
    h.action = ac;
    h.seq = seq;
    h.mask = m;
...
public:
  ...
  {
    out << "client_lease(a=" << ceph_lease_op_name(get_action()) << " seq "
        << get_seq() << " mask " << get_mask();
    out << " " << get_ino();
