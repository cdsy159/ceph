...
inline const char*
mds_metric_name(mds_metric_t m)
{
  switch (m) {
  ...
  case MDS_HEALTH_CLIENTS_LAGGY:
    return "MDS_CLIENTS_LAGGY";
  case MDS_HEALTH_CLIENTS_LAGGY_MANY:
    return "MDS_CLIENTS_LAGGY_MANY";
  case MDS_HEALTH_CLIENTS_BROKEN_ROOTSQUASH:
    return "MDS_CLIENTS_BROKEN_ROOTSQUASH";
  case MDS_HEALTH_ESTIMATED_REPLAY_TIME:
    return "MDS_ESTIMATED_REPLAY_TIME";
  case MDS_HEALTH_DUMMY:
    return "MDS_DUMMY";
  default:
    return "???";
  }
...
inline const char*
mds_metric_summary(mds_metric_t m)
{
  switch (m) {
  case MDS_HEALTH_TRIM:
...
struct MDSHealthMetric {
  ...
  static std::list<MDSHealthMetric>
  generate_test_instances()
  {
    ...
    ls.push_back(MDSHealthMetric(
        MDS_HEALTH_TRIM, HEALTH_WARN, "MDS is behind on trimming"));
  ...
  bool
  operator==(MDSHealthMetric const& other) const
  {
    return (type == other.type && sev == other.sev && message == other.message);
  }

  MDSHealthMetric() :
    type(MDS_HEALTH_NULL), sev(HEALTH_OK)
  {}
...
struct MDSHealth {
  ...
  static std::list<MDSHealth>
  generate_test_instances()
  {
    std::list<MDSHealth> ls;
    ls.emplace_back();
    ls.emplace_back();
    ls.back().metrics.push_back(MDSHealthMetric(
        MDS_HEALTH_TRIM, HEALTH_WARN, "MDS is behind on trimming"));
    return ls;
  }

  bool
  operator==(MDSHealth const& other) const
  {
...
public:
  ...
  void
  set_sys_info(const std::map<std::string, std::string>& i)
  {
    sys_info = i;
  }
