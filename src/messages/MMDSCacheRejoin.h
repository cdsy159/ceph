...
public:
  static constexpr int OP_WEAK =
      1; // replica -> auth, i exist, + maybe open files.
  static constexpr int OP_STRONG =
      2; // replica -> auth, i exist, + open files and lock state.
  ...
  struct inode_strong {
    ...
    inode_strong(int n, int cw, int dl, int nl, int dftl) :
      nonce(n), caps_wanted(cw), filelock(dl), nestlock(nl), dftlock(dftl)
    {}
  ...
  struct dirfrag_strong {
    ...
    static std::list<dirfrag_strong>
    generate_test_instances()
    {
  ...
  // inodes
  void
  add_weak_inode(vinodeno_t i)
  {
    weak_inodes.insert(i);
  }

  void
  add_strong_inode(vinodeno_t i, int n, int cw, int dl, int nl, int dftl)
  {
    strong_inodes[i] = inode_strong(n, cw, dl, nl, dftl);
  }

  void
  add_inode_locks(CInode* in, __u32 nonce, ceph::buffer::list& bl)
  {
    using ceph::encode;
  ...
  void
  add_inode_base(CInode* in, uint64_t features)
  {
    using ceph::encode;
  ...
  void
  add_inode_authpin(vinodeno_t ino, const metareqid_t& ri, __u32 attempt)
  {
    authpinned_inodes[ino].push_back(peer_reqid(ri, attempt));
  }

  void
  add_inode_frozen_authpin(vinodeno_t ino, const metareqid_t& ri, __u32 attempt)
  {
    frozen_authpin_inodes[ino] = peer_reqid(ri, attempt);
  }

  void
  add_inode_xlock(vinodeno_t ino, int lt, const metareqid_t& ri, __u32 attempt)
  {
    xlocked_inodes[ino][lt] = peer_reqid(ri, attempt);
  }

  void
  add_inode_wrlock(vinodeno_t ino, int lt, const metareqid_t& ri, __u32 attempt)
  {
    wrlocked_inodes[ino][lt].push_back(peer_reqid(ri, attempt));
  }

  void
  add_scatterlock_state(CInode* in)
  {
  ...
  // dirfrags
  void
  add_strong_dirfrag(dirfrag_t df, int n, int dr)
  {
    strong_dirfrags[df] = dirfrag_strong(n, dr);
  }

  void
  add_dirfrag_base(CDir* dir)
  {
      ...
      int ls)
  {
    ...
    m.insert_or_assign(
        string_snap_t(dname, last), dn_strong(first, altn, pi, ri, rdt, n, ls));
  }
  ...
  add_dentry_authpin(
      ...
      __u32 attempt)
  {
    authpinned_dentries[df][string_snap_t(dname, last)].push_back(
        peer_reqid(ri, attempt));
  }
  ...
  std::map<dirfrag_t, std::map<string_snap_t, std::list<peer_reqid>>>
      authpinned_dentries;
...
inline std::ostream&
operator<<(std::ostream& out, const MMDSCacheRejoin::peer_reqid& r)
{
