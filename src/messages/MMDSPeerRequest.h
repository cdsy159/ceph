...
public:
  ...
  const std::vector<MDSCacheObjectInfo>&
  get_authpins() const
  {
    return authpins;
  }

  std::vector<MDSCacheObjectInfo>&
  get_authpins()
  {
    return authpins;
  }
  ...
  void
  mark_interrupted() const
  {
    flags |= FLAG_INTERRUPTED;
  }

  bool
  should_notify_blocking() const
  {
  ...
  void
  clear_notify_blocking() const
  {
    flags &= ~FLAG_NOTIFYBLOCKING;
  }

  bool
  is_req_blocked() const
  {
