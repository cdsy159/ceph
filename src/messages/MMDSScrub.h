...
public:
  ...
  get_opname(int o)
  {
    ...
    case OP_QUEUEDIR:
      return "queue_dir";
    case OP_QUEUEDIR_ACK:
      return "queue_dir_ack";
    case OP_QUEUEINO:
      return "queue_ino";
    case OP_QUEUEINO_ACK:
      return "queue_ino_ack";
    case OP_ABORT:
      return "abort";
    case OP_PAUSE:
      return "pause";
  ...
  {
    out << "mds_scrub(" << get_opname(op) << " " << ino << " " << frags << " "
        << tag;
    if (is_force())
      out << " force";
    if (is_recursive())
    ...
    if (is_repair())
      out << " repair";
    if (is_remote_dirfrag_dirty())
      out << " dirty_remote_dirfrag";
    out << ")";
  }
...
protected:
  ...
  MMDSScrub(
      ...
      inodeno_t _origin = inodeno_t(),
      bool internal_tag = false,
      bool force = false,
    ...
    origin(_origin)
  {
    ...
    if (repair)
      flags |= FLAG_REPAIR;
    if (remote_dirfrag_dirty)
      flags |= FLAG_DIRTY_REMOTE_DIRFRAG;
  }
