...
public:
  ...
  static constexpr int OP_COMMIT =
      5; // proposer: notify learners of agreed value
  ...
  version_t uncommitted_pn =
      0; // previous pn, if we are a LAST with an uncommitted value
...
public:
  ...
  {
    out << "paxos(" << get_opname(op) << " lc " << last_committed << " fc "
        << first_committed << " pn " << pn << " opn " << uncommitted_pn;
    if (latest_version)
      out << " latest " << latest_version << " (" << latest_value.length()
          << " bytes)";
    out << ")";
  }
