...
public:
  ...
  get_event() override
  {
    return new PGPeeringEvent(
        notify.epoch_sent, notify.query_epoch,
        MNotifyRec(
            spgid, pg_shard_t(get_source().num(), notify.from), notify,
#ifdef WITH_CRIMSON
            features
#else
        ...
        true,
        new PGCreateInfo(
            spgid, notify.epoch_sent, notify.info.history,
            notify.past_intervals, false));
  }
