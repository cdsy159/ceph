...
PyObject*
ActivePyModules::get_server_python(const std::string& hostname)
{
...
void
ActivePyModules::update_cache_metrics()
{
...
PyObject*
ActivePyModules::cacheable_get_python(const std::string& what)
{
  uint64_t ttl_seconds =
      g_conf().get_val<uint64_t>("mgr_ttl_cache_expire_seconds");
  if (ttl_seconds > 0) {
...
PyObject*
ActivePyModules::get_python(const std::string& what)
{
  uint64_t ttl_seconds =
      g_conf().get_val<uint64_t>("mgr_ttl_cache_expire_seconds");
  ...
  if (what == "fs_map") {
    ...
    without_gil_t no_gil;
    cluster_state.with_osdmap_and_pgmap([&](const OSDMap& osd_map,
                                            const PGMap& pg_map) {
      no_gil.acquire_gil();
    ...
    cluster_state.with_pgmap([&](const PGMap& pg_map) {
      ...
      for (auto& i : pg_map.pg_stat) {
        ...
        const auto pgid = stringify(pgid_raw.m_pool) + "." +
                          stringify(pgid_raw.m_seed);
...
void
ActivePyModules::start_one(PyModuleRef py_module)
{
  std::lock_guard l(lock);
  ...
  finisher.queue(new LambdaContext([this, active_module, name](int) {
    ...
    auto delay =
        g_conf().get_val<std::chrono::milliseconds>("mgr_module_load_delay");
    std::string delayed_module =
        g_conf().get_val<std::string>("mgr_module_load_delay_name");
    if ((name == delayed_module) && (delay > std::chrono::milliseconds{0})) {
      dout(4) << "Delaying load time for module '" << name << "' by " << delay
              << "..." << dendl;
    ...
    const std::string& notify_id)
{
  ...
  for (auto& [name, module] : modules) {
    ...
    mod_finisher.queue(new LambdaContext([module = module, notify_type,
                                          notify_id](int r) {
...
void
ActivePyModules::notify_all(const LogEntry& log_entry)
{
  std::lock_guard l(lock);
    ...
    std::string* val) const
{
  ...
  const std::string global_key = PyModule::mgr_store_prefix + module_name +
                                 "/" + key;
...
PyObject*
ActivePyModules::get_store_prefix(
    const std::string& module_name,
    const std::string& prefix) const
{
  without_gil_t no_gil;
...
void
ActivePyModules::update_kv_data(
...
void
ActivePyModules::_refresh_config_map()
{
  dout(10) << dendl;
  config_map.clear();
  for (auto p = store_cache.lower_bound("config/");
       p != store_cache.end() && p->first.find("config/") == 0; ++p) {
    ...
    config_map.add_option(
        ...
        [&](const std::string& name) { return g_conf().find_option(name); });
  }
}

PyObject*
ActivePyModules::with_unlabled_perf_counters(
    std::function<void(
  ...
  f.open_array_section(path);
  {
    ...
    if (metadata) {
      ...
      if (metadata->perf_counters.instances.count(path)) {
        ...
        dout(4) << "Missing counter: '" << path << "' (" << svc_name << "."
                << svc_id << ")" << dendl;
...
typedef std::vector<pair<std::string_view, std::string_view>>
    perf_counter_label_pairs;
    ...
    const perf_counter_label_pairs& labels) const
{
  ...
  */
  ...
  if (labels.empty()) {
    resolved_path = std::string(counter_name) + "." +
...
PyObject*
ActivePyModules::get_unlabeled_counter_python(
    const std::string& svc_name,
    const std::string& svc_id,
    const std::string& path)
{
  auto extract_counters = [](PerfCounterInstance& counter_instance,
                             PerfCounterType& counter_type, PyFormatter& f) {
...
PyObject*
ActivePyModules::get_latest_unlabeled_counter_python(
    const std::string& svc_name,
    const std::string& svc_id,
    const std::string& path)
{
  ...
  return with_unlabled_perf_counters(
      extract_latest_counters, svc_name, svc_id, path);
}
    ...
    const std::string& svc_id)
{
  ...
  if (!daemons.empty()) {
    ...
    dout(4) << __func__ << ": No daemon state found for " << svc_type << "."
            << svc_id << ")" << dendl;
  }
  return f.get();
}
        ...
        for (const auto& [counter_name_with_labels, _] :
          ...
          */
          ...
          if (key_labels.empty()) {
            ...
            key_name = std::string(
                ceph::perf_counters::key_name(counter_name_with_labels));
          }
    ...
    void* wrapped)
{
  ...
  if (!module) {
    ...
    derr << handle_pyerror(
                true, module_name,
                "construct_with_capsule "s + module_name + " " + clsname)
         << dendl;
  }
  ceph_assert(module);

  PyObject* wrapper_type =
      PyObject_GetAttrString(module, (const char*)clsname.c_str());
  if (!wrapper_type) {
    ...
    const std::string& name)
{
  ...
  if (!have_local_config_map) {
    dout(20) << "mon cluster wasn't pacific when we started: falling back to "
                "'config get'"
             << dendl;
    without_gil_t no_gil;
    Command cmd;
    {
      std::lock_guard l(lock);
      cmd.run(
          &monc, "{\"prefix\": \"config get\","s + "\"who\": \""s + who +
                     "\","s + "\"key\": \""s + name + "\"}");
    }
    cmd.wait();
  ...
  cluster_state.with_osdmap([&](const OSDMap& osdmap) {
    ...
    config = config_map.generate_entity_map(
        entity, crush_location, osdmap.crush.get(), device_class);
  });
...
void
ActivePyModules::set_health_checks(
    const std::string& module_name,
...
int
ActivePyModules::handle_command(
    ...
    std::stringstream* ss)
{
  ...
  lock.unlock();
  return mod_iter->second->handle_command(
      module_command, session, cmdmap, inbuf, ds, ss);
}

void
ActivePyModules::get_health_checks(health_check_map_t* checks)
{
  std::lock_guard l(lock);
  for (auto& [name, module] : modules) {
...
void
ActivePyModules::update_progress_event(
...
void
ActivePyModules::complete_progress_event(const std::string& evid)
{
  std::lock_guard l(lock);
...
void
ActivePyModules::clear_all_progress_events()
{
  std::lock_guard l(lock);
...
void
ActivePyModules::get_progress_events(
...
void
ActivePyModules::set_uri(const std::string& module_name, const std::string& uri)
{
  std::lock_guard l(lock);
...
void
ActivePyModules::set_device_wear_level(const std::string& devid, float wear_level)
{
  // update mgr state
...
void
ActivePyModules::remove_osd_perf_query(MetricQueryID query_id)
{
  int r = server.remove_osd_perf_query(query_id);
  if (r < 0) {
    dout(0) << "remove_osd_perf_query for query_id=" << query_id
...
ActivePyModules::get_osd_perf_counters(MetricQueryID query_id)
{
  ...
  if (r < 0) {
    dout(0) << "get_osd_perf_counters for query_id=" << query_id
            << " failed: " << cpp_strerror(r) << dendl;
    Py_RETURN_NONE;
  }

  PyFormatter f;
  const std::map<OSDPerfMetricKey, PerformanceCounters>& counters =
      collector.counters;
...
void
ActivePyModules::remove_mds_perf_query(MetricQueryID query_id)
{
  int r = server.remove_mds_perf_query(query_id);
  if (r < 0) {
    dout(0) << "remove_mds_perf_query for query_id=" << query_id
            << " failed: " << cpp_strerror(r) << dendl;
  }
}

void
ActivePyModules::reregister_mds_perf_queries()
{
...
ActivePyModules::get_mds_perf_counters(MetricQueryID query_id)
{
  ...
  if (r < 0) {
    dout(0) << "get_mds_perf_counters for query_id=" << query_id
            << " failed: " << cpp_strerror(r) << dendl;
    Py_RETURN_NONE;
  }

  PyFormatter f;
  const std::map<MDSPerfMetricKey, PerformanceCounters>& counters =
      collector.counters;
...
void
ActivePyModules::register_client(
    ...
    bool replace)
{
  ...
  dout(7) << "registering msgr client handle " << addrv
          << " (replace=" << replace << ")" << dendl;
  py_module_registry.register_client(name, std::move(addrv), replace);
}

void
ActivePyModules::unregister_client(std::string_view name, std::string addrs)
{
  entity_addrvec_t addrv;
  addrv.parse(addrs.data());
...
PyObject*
ActivePyModules::get_daemon_health_metrics()
{
  without_gil_t no_gil;
  return daemon_state.with_daemons_by_server(
      [&no_gil](const std::map<std::string, DaemonStateCollection>& all) {
        no_gil.acquire_gil();
        PyFormatter f;
        for (const auto& [hostname, daemon_state] : all) {
...
void
ActivePyModules::check_all_modules_started(Context* modules_start_complete)
{
  std::lock_guard l(lock);
  if (pending_modules.empty()) {
    ...
    recheck_modules_start =
        modules_start_complete; // signal that we need to check again later
  }
}
