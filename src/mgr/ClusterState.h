...
protected:
  ...
  std::map<int64_t, unsigned>
      existing_pools; ///< pools that exist, and pg_num, as of PGMap epoch
  PGMap pg_map;
...
public:
  ...
  auto
  with_servicemap(Callback&& cb, Args&&... args) const
  {
    std::lock_guard l(lock);
  ...
  template <typename Callback, typename... Args>
  auto
  with_fsmap(Callback&& cb, Args&&... args) const
  {
    std::lock_guard l(lock);
  ...
  template <typename Callback, typename... Args>
  auto
  with_mgrmap(Callback&& cb, Args&&... args) const
  {
    std::lock_guard l(lock);
  ...
  template <typename Callback, typename... Args>
  auto
  with_pgmap(Callback&& cb, Args&&... args) const
  ...
  template <typename Callback, typename... Args>
  auto
  with_mutable_pgmap(Callback&& cb, Args&&... args)
      -> decltype(cb(pg_map, std::forward<Args>(args)...))
  {
    std::lock_guard l(lock);
    return std::forward<Callback>(cb)(pg_map, std::forward<Args>(args)...);
  ...
  template <typename... Args>
  auto
  with_osdmap(Args&&... args) const -> decltype(objecter->with_osdmap(
  ...
  template <typename Callback, typename... Args>
  auto
  with_osdmap_and_pgmap(Callback&& cb, Args&&... args) const
  {
    ceph_assert(objecter != nullptr);
    std::lock_guard l(lock);
    return objecter->with_osdmap(
        std::forward<Callback>(cb), pg_map, std::forward<Args>(args)...);
  }

  template <typename Callback, typename... Args>
  auto
  with_health(Callback&& cb, Args&&... args) const
  {
    std::lock_guard l(lock);
  ...
  template <typename Callback, typename... Args>
  auto
  with_mon_status(Callback&& cb, Args&&... args) const
  {
    std::lock_guard l(lock);
    return std::forward<Callback>(cb)(
        mon_status_json, std::forward<Args>(args)...);
  }
