...
void
DaemonHealthMetric::dump(ceph::Formatter* f) const
{
...
std::list<DaemonHealthMetric>
DaemonHealthMetric::generate_test_instances()
{
...
std::ostream&
operator<<(std::ostream& out, const DaemonHealthMetric& m)
{
  return out << daemon_metric_name(m.get_type()) << "(" << m.get_n() << "|("
             << m.get_n1() << "," << m.get_n2() << "))";
}
