...
DaemonServer::DaemonServer(
  ...
  op_tracker(
      g_ceph_context,
      g_ceph_context->_conf->mgr_enable_op_tracker,
      g_ceph_context->_conf->mgr_num_op_tracker_shard)
{
  ...
  op_tracker.set_complaint_and_threshold(
      cct->_conf->mgr_op_complaint_time, cct->_conf->mgr_op_log_threshold);
  op_tracker.set_history_size_and_duration(
      cct->_conf->mgr_op_history_size, cct->_conf->mgr_op_history_duration);
  op_tracker.set_history_slow_op_size_and_threshold(
      cct->_conf->mgr_op_history_slow_op_size,
...
public:
  explicit DaemonServerHook(DaemonServer* o) :
    daemon_server(o)
  {}
...
int
DaemonServer::init(uint64_t gid, entity_addrvec_t client_addrs)
{
  ...
  msgr = Messenger::create(
      g_ceph_context, public_msgr_type, entity_name_t::MGR(gid), "mgr",
      Messenger::get_random_nonce());
  msgr->set_default_policy(Messenger::Policy::stateless_server(0));
  // throttle policy
  msgr->set_policy(
      entity_name_t::TYPE_OSD,
      Messenger::Policy::stateless_server(CEPH_FEATURE_SERVER_LUMINOUS));
  msgr->set_policy(
  ...
  // throttle clients
  msgr->set_policy_throttlers(
      entity_name_t::TYPE_CLIENT, client_byte_throttler.get(),
      client_msg_throttler.get());

  // servers
  msgr->set_policy_throttlers(
      entity_name_t::TYPE_OSD, osd_byte_throttler.get(),
      osd_msg_throttler.get());
  msgr->set_policy_throttlers(
      entity_name_t::TYPE_MDS, mds_byte_throttler.get(),
      mds_msg_throttler.get());
  msgr->set_policy_throttlers(
      entity_name_t::TYPE_MON, mon_byte_throttler.get(),
...
bool
DaemonServer::ms_handle_fast_authentication(Connection* con)
{
  auto s = ceph::make_ref<MgrSession>(cct);
  con->set_priv(s);
  s->inst.addr = con->get_peer_addr();
  s->entity_name = con->peer_name;
  dout(10) << __func__ << " new session " << s << " con " << con << " entity "
           << con->peer_name << " addr " << con->get_peer_addrs() << dendl;

  AuthCapsInfo& caps_info = con->get_peer_caps_info();
  if (caps_info.allow_all) {
    dout(10) << " session " << s << " " << s->entity_name << " allow_all"
             << dendl;
    ...
    dout(10) << " session " << s << " " << s->entity_name << " has caps "
             << s->caps << " '" << str << "'" << dendl;
  }
  return true;
}
...
bool
DaemonServer::ms_handle_reset(Connection* con)
{
  if (con->get_peer_type() == CEPH_ENTITY_TYPE_OSD) {
    ...
    std::lock_guard l(lock);
    dout(10) << "unregistering osd." << session->osd_id << "  session "
             << session << " con " << con << dendl;
...
bool
DaemonServer::ms_handle_refused(Connection* con)
{
  // do nothing for now
  return false;
}
...
DaemonServer::ms_dispatch2(const ref_t<Message>& m)
{
...
void
DaemonServer::dump_pg_ready(ceph::Formatter* f)
{
  f->dump_bool("pg_ready", pgmap_ready.load());
}

void
DaemonServer::maybe_ready(int32_t osd_id)
{
  if (pgmap_ready.load()) {
    ...
    if (reported_osds.find(osd_id) == reported_osds.end()) {
      ...
      std::set_difference(
          up_osds.begin(), up_osds.end(), reported_osds.begin(),
          reported_osds.end(),
...
void
DaemonServer::schedule_tick_locked(double delay_sec)
{
  ...
  tick_event = timer.add_event_after(
      delay_sec, new LambdaContext([this](int r) { tick(); }));
}

void
DaemonServer::schedule_tick(double delay_sec)
{
  std::lock_guard l(lock);
...
void
DaemonServer::handle_osd_perf_metric_query_updated()
{
  dout(10) << dendl;
...
void
DaemonServer::handle_mds_perf_metric_query_updated()
{
  dout(10) << dendl;
...
void
DaemonServer::fetch_missing_metadata(
    const DaemonKey& key,
...
bool
DaemonServer::handle_open(const ref_t<MMgrOpen>& m)
{
  std::unique_lock l(lock);

  DaemonKey key = key_from_service(
      m->service_name, m->get_connection()->get_peer_type(), m->daemon_name);
  ...
  if (daemon) {
    ...
    if (m->service_daemon) {
      ...
      auto [d, added] =
          pending_service_map.get_daemon(m->service_name, m->daemon_name);
    ...
    if (p != m->config_bl.end()) {
      ...
      dout(20) << " got config " << daemon->config << " ignored "
               << daemon->ignored_mon_config << dendl;
    }
    daemon->config_defaults_bl = m->config_defaults_bl;
    daemon->config_defaults.clear();
    dout(20) << " got config_defaults_bl "
             << daemon->config_defaults_bl.length() << " bytes" << dendl;
  }
...
bool
DaemonServer::handle_update(const ref_t<MMgrUpdate>& m)
{
  DaemonKey key;
  ...
  if (m->get_connection()->get_peer_type() == entity_name_t::TYPE_CLIENT &&
    ...
    dout(10) << "rejecting update request from non-daemon client "
             << m->daemon_name << dendl;
...
bool
DaemonServer::handle_close(const ref_t<MMgrClose>& m)
{
  std::lock_guard l(lock);
...
void
DaemonServer::update_task_status(
    DaemonKey key,
...
bool
DaemonServer::handle_report(const ref_t<MMgrReport>& m)
{
  DaemonKey key;
  if (!m->service_name.empty()) {
    key.type = m->service_name;
...

  {
    ...
    if (daemon = daemon_state.get(key); daemon != nullptr) {
      ...
      dout(5) << "rejecting report from " << key
              << ", since we do not have its metadata now." << dendl;
      ...
      dout(10) << "unregistering osd." << session->osd_id << "  session "
               << session << " con " << m->get_connection() << dendl;
...
void
DaemonServer::_generate_command_map(
    cmdmap_t& cmdmap,
    ...
    const MonCommand* this_cmd)
{
  ...
  bool capable = s->caps.is_capable(
      g_ceph_context, s->entity_name, service, module, prefix, param_str_map,
      cmd_r, cmd_w, cmd_x, s->get_peer_addr());

  dout(10) << " " << s->entity_name << " " << (capable ? "" : "not ")
           << "capable" << dendl;
  return capable;
}
...
public:
  ...
  explicit CommandContext(ceph::ref_t<MCommand> m) :
    m_tell{std::move(m)}, cmd(m_tell->cmd), data(m_tell->get_data())
  {}

  explicit CommandContext(ceph::ref_t<MMgrCommand> m) :
    m_mgr{std::move(m)}, cmd(m_mgr->cmd), data(m_mgr->get_data())
  {}

  void
  reply(int r, const std::stringstream& ss)
  {
    reply(r, ss.str());
  }

  void
  reply(int r, const std::string& rs)
  {
...
public:
  ...
  explicit ReplyOnFinish(
      const std::shared_ptr<CommandContext>& cmdctx_,
      MgrOpRequestRef op_) :
    cmdctx(cmdctx_), op(op_)
  {
    if (op) {
...
bool
DaemonServer::handle_command(const ref_t<MCommand>& m)
{
  std::lock_guard l(lock);
...
bool
DaemonServer::handle_command(const ref_t<MMgrCommand>& m)
{
  std::lock_guard l(lock);
  auto cmdctx = std::make_shared<CommandContext>(m);
  try {
...
void
DaemonServer::log_access_denied(
    std::shared_ptr<CommandContext>& cmdctx,
    MgrSession* session,
    std::stringstream& ss)
{
  dout(1) << " access denied" << dendl;
  audit_clog->info() << "from='" << session->inst << "' " << "entity='"
                     << session->entity_name << "' " << "cmd=" << cmdctx->cmd
                     << ":  access denied";
...
void
DaemonServer::_check_offlines_pgs(
    ...
    offline_pg_report* report)
{
  ...
  *report = offline_pg_report();
  ...
  dout(20) << osds << " -> " << report->ok.size() << " ok, "
           << report->not_ok.size() << " not ok, " << report->unknown.size()
           << " unknown" << dendl;
}

void
DaemonServer::_maximize_ok_to_stop_set(
...
void
DaemonServer::_update_upgraded_osds(
...
bool
DaemonServer::_valid_bucket_type_for_upgrade_check(
...
int
DaemonServer::_populate_crush_bucket_osds(
    const int item_id,
    const OSDMap& osdmap,
    const PGMap& pgmap,
    ...
    std::ostream* ss)
{
  ...
  dout(20) << "bucket type of parent " << item_name << " is " << bucket_type_str
           << dendl;
  ...
  if (bucket_type_str == "rack" || bucket_type_str == "chassis") {
    ...
    if (osdmap.crush->get_children(item_id, &crush_bucket_children) <= 0) {
      ostringstream os;
      os << "crush bucket \"" << item_name << "\" of type: " << bucket_type_str
         << " has no children!";
      if (ss) {
        *ss << os.str();
      }
  ...
  for (const auto& name : bucket_names) {
    ...
    if (tmp_bucket_osds.size() == 1) {
      ...
      dout(20) << "picked osd: " << tmp_bucket_osds << " from bucket: " << name
               << dendl;
      continue;
    }
    /**
    ...
    std::sort(
        child_bucket_pgs_per_osd.begin(), child_bucket_pgs_per_osd.end(),
    ...
    dout(20) << "picked osds: " << tmp_bucket_osds << " from bucket: " << name
             << dendl;
  }
  return r;
}
...
void
DaemonServer::_maximize_ok_to_upgrade_set(
    const std::vector<int>& orig_osds,
    unsigned max,
    const OSDMap& osdmap,
    const PGMap& pgmap,
    ...
    std::ostream* ss)
{
  ...
  dout(20) << "orig_osds " << orig_osds << " new ceph_version "
  ...
  for (const auto& osd : orig_osds) {
    ...
    if (ver.has_value()) {
      ...
      derr << "couldn't determine 'ceph_version_short' for " << osd_id << dendl;
  ...
  _update_upgraded_osds(
      orig_osds, to_upgrade, upgraded, version_unknown, out_osd_report);
  ...
  while (true) {
    ...
    *out_pg_report = offline_pg_report();
    ...
    if (out_pg_report->ok_to_stop()) {
      ...
      _update_upgraded_osds(
          orig_osds, osds, upgraded, version_unknown, out_osd_report);
  ...
  if (osds.size() == max) {
    // already at max
    dout(20) << "to_upgrade(" << osds.size() << ") == " << " max(" << max << ")"
             << dendl;
    return;
  }
  ...
  if (osds.size() < max && osds.size() < to_upgrade.size()) {
    ...
    for (auto it = to_upgrade.begin() + osd_subset_count;
         it != to_upgrade.end(); ++it) {
      *_dout << *it << " ";
    }
    *_dout << "]" << dendl;

    for (size_t i = osd_subset_count;
         i < to_upgrade.size() && osds.size() < max; ++i) {
      ...
      if (_pg_report.ok_to_stop()) {
        ...
        _update_upgraded_osds(
            orig_osds, osds, upgraded, version_unknown, &_osd_report);
...
DaemonServer::get_osd_metadata(const std::string& name, const std::string& osd_id)
{
...
void
upgrade_osd_report::dump(Formatter* f) const
{
...
bool
DaemonServer::_handle_command(std::shared_ptr<CommandContext>& cmdctx)
{
  MessageRef m;
  ...
  dout(10) << "decoded-size=" << cmdctx->cmdmap.size() << " prefix=" << prefix
           << dendl;
  ...
  if (prefix == "get_command_descriptions" && !admin_socket_cmd) {
    ...
    auto dump_cmd = [&cmdnum, &f, m](const MonCommand& mc) {
      ...
      dump_cmddesc_to_json(
          &f, m->get_connection()->get_features(), secname.str(), mc.cmdstring,
          mc.helpstring, mc.module, mc.req_perms, 0);
      cmdnum++;
    };
  ...
  if (admin_socket_cmd) {
    ...
    is_allowed = _allowed_command(
        session, "py", py_command.module_name, prefix, cmdctx->cmdmap,
        param_str_map, &pyc);
  } else {
    // validate user's permissions for requested command
    is_allowed = _allowed_command(
        session, mgr_cmd->module, "", prefix, cmdctx->cmdmap, param_str_map,
        mgr_cmd);
  }
  ...
  audit_clog->debug() << "from='" << session->inst << "' " << "entity='"
                      << session->entity_name << "' " << "cmd=" << cmdctx->cmd
                      << ": dispatch";
  ...
  if (prefix == "pg scrub" || prefix == "pg repair" ||
    ...
    for (auto& con : p->second) {
      ...
      con->send_message(new MOSDScrub2(
          monc->get_fsid(), epoch, pgs, scrubop == "repair",
          scrubop == "deep-scrub"));
    }
    ss << "instructing pg " << spgid << " on osd." << acting_primary << " to "
       << scrubop;
    cmdctx->reply(0, ss);
    return true;
  } else if (
      prefix == "osd scrub" || prefix == "osd deep-scrub" ||
      prefix == "osd repair") {
    string whostr;
    ...
    for (auto osd : osds) {
      ...
      if (p == osd_cons.end()) {
        ...
        for (auto& con : p->second) {
          con->send_message(new MOSDScrub2(
              monc->get_fsid(), epoch, spgs, pvec.back() == "repair",
    ...
    return true;
      ...
      prefix == "osd pool scrub" || prefix == "osd pool deep-scrub" ||
    ...
    for (auto& it : spgs_by_primary) {
      ...
      for (auto& con : p->second) {
        con->send_message(new MOSDScrub2(
            monc->get_fsid(), epoch, it.second, prefix == "osd pool repair",
    ...
    bool by_pg = prefix == "osd reweight-by-pg" ||
                 prefix == "osd test-reweight-by-pg";
    bool dry_run = prefix == "osd test-reweight-by-pg" ||
                   prefix == "osd test-reweight-by-utilization";
    ...
    r = cluster_state.with_osdmap_and_pgmap([&](const OSDMap& osdmap,
                                                const PGMap& pgmap) {
      return reweight::by_utilization(
          osdmap, pgmap, oload, max_change, max_osds, by_pg,
          pools.empty() ? NULL : &pools, no_increasing, &new_weights, &ss,
          &out_str, f.get());
    });
    if (r >= 0) {
      dout(10) << "reweight::by_utilization: finished with " << out_str
               << dendl;
    }
    if (f) {
      f->flush(cmdctx->odata);
    ...
    if (r < 0) {
      ...
      for (const auto& osd_weight : new_weights) {
        json_spirit::Config::add(
            json_object, std::to_string(osd_weight.first),
      ...
      monc->start_mon_command(
          {cmd}, {}, &on_finish->from_mon, &on_finish->outs, on_finish);
      return true;
    }
    ...
    r = cluster_state.with_osdmap_and_pgmap([&](const OSDMap& osdmap,
      ...
      if (!filter.empty() && osdmap.lookup_pg_pool_name(filter) < 0 &&
      ...
      print_osd_utilization(
          osdmap, pgmap, ss, f.get(), method == "tree", filter);
    ...
    if (r != -EOPNOTSUPP) {
      ...
      prefix == "osd safe-to-destroy" || prefix == "osd destroy" ||
      prefix == "osd purge") {
    std::set<int> osds;
    int r = 0;
    ...
    if (!r && (!active_osds.empty() || !missing_stats.empty() ||
               !stored_pgs.empty())) {
    ...
    cluster_state.with_osdmap_and_pgmap([&](const OSDMap& osdmap,
      ...
      // get candidate additions that are beneath this point in the tree
      r = _populate_crush_bucket_osds(
          id, osdmap, pgmap, osds_in_crush_bucket, &ss);
      if (r != 0) {
        return;
      }
    });
    ...
    if (!osd_upgrade_report.ok_to_upgrade()) {
      ...
      if (osd_upgrade_report.all_osds_upgraded()) {
        ss << "all " << osds_in_crush_bucket.size()
           << " osd(s) are running the new Ceph version(" << ceph_version
           << ")";
        cmdctx->reply(0, ss);
      }
    } else {
    ...
    return true;
      ...
      prefix == "pg force-recovery" || prefix == "pg force-backfill" ||
    ...
    cluster_state.with_pgmap([&](const PGMap& pg_map) {
      for (auto& i : candidates) {
        ...
        switch (actual_op) {
        case OFR_RECOVERY:
          if ((state & (PG_STATE_DEGRADED | PG_STATE_RECOVERY_WAIT |
          ...
          break;
        case OFR_BACKFILL:
          if ((state & (PG_STATE_DEGRADED | PG_STATE_BACKFILL_WAIT |
      ...
      if (prefix == "config show") {
        ...
        for (auto& i : daemon->config) {
          ...
          if (f) {
            ...
            f->dump_string(
                "source", ceph_conf_level_name(i.second.rbegin()->first));
            if (i.second.size() > 1) {
            ...
            if (i.second.size() > 1) {
              ...
              for (--j; j != i.second.rbegin(); --j) {
                if (j->second == i.second.rbegin()->second) {
                  ov.push_front(
                      string("(") + ceph_conf_level_name(j->first) +
                      string("[") + j->second + string("]") + string(")"));
                } else {
                  ov.push_front(
                      ceph_conf_level_name(j->first) + string("[") + j->second +
                      string("]"));
                }
              }
              tbl << ov;
        ...
        for (auto& i : defaults) {
          ...
          if (j != daemon->config.end() && !j->second.empty()) {
            ...
            if (f) {
              ...
              f->dump_string(
                  "source", ceph_conf_level_name(j->second.rbegin()->first));
              if (j->second.size() > 1) {
                f->open_array_section("overrides");
              ...
              if (j->second.size() > 1) {
                ...
                for (--k; k != j->second.rbegin(); --k) {
                  if (k->second == j->second.rbegin()->second) {
                    ov.push_front(
                        string("(") + ceph_conf_level_name(k->first) +
                        string("[") + k->second + string("]") + string(")"));
                  } else {
                    ov.push_front(
                        ceph_conf_level_name(k->first) + string("[") +
                        k->second + string("]"));
                  }
                }
                tbl << ov;
    ...
    if (f) {
      ...
      daemon_state.with_devices([&tbl, now](const DeviceState& dev) {
        ...
        if (dev.wear_level >= 0) {
          snprintf(
              wear_level_str, sizeof(wear_level_str) - 1, "%d%%",
              (int)(100.1 * dev.wear_level));
        }
        tbl << dev.devid << h << d << wear_level_str
            << dev.get_life_expectancy_str(now) << TextTable::endrow;
      });
      cmdctx->odata.append(stringify(tbl));
    }
    cmdctx->reply(0, ss);
    ...
    if (auto [k, valid] = DaemonKey::parse(who); !valid) {
      ...
      if (dm) {
        if (f) {
          ...
          tbl.define_column("HOST:DEV", TextTable::LEFT, TextTable::LEFT);
          tbl.define_column(
              "EXPECTED FAILURE", TextTable::LEFT, TextTable::LEFT);
          auto now = ceph_clock_now();
          for (auto& i : dm->devices) {
            daemon_state.with_device(
                i.first, [&tbl, now](const DeviceState& dev) {
                  ...
                  tbl << dev.devid << h << dev.get_life_expectancy_str(now)
                      << TextTable::endrow;
                });
          }
          cmdctx->odata.append(stringify(tbl));
      ...
      for (auto& devid : devids) {
        daemon_state.with_device(
            devid, [&tbl, &host, now](const DeviceState& dev) {
              ...
              tbl << dev.devid << n << d << dev.get_life_expectancy_str(now)
                  << TextTable::endrow;
            });
      }
      cmdctx->odata.append(stringify(tbl));
    ...
    if (!from.parse(from_str)) {
      ...
      auto on_finish = new ReplyOnFinish(cmdctx, op);
      monc->start_mon_command(
          {cmd}, std::move(json), nullptr, nullptr, on_finish);
    }
    return true;
    ...
    if (daemon_state.with_device_write(devid, [&meta](DeviceState& dev) {
      ...
      auto on_finish = new ReplyOnFinish(cmdctx, op);
      monc->start_mon_command(
          {std::move(cmd)}, std::move(json), nullptr, nullptr, on_finish);
    } else {
      cmdctx->reply(0, ss);
    }
    return true;
    ...
    if (!pgmap_ready) {
      ss << "Warning: due to ceph-mgr restart, some PG states may not be up to "
            "date\n";
    }
    if (f) {
    ...
    r = cluster_state.with_osdmap_and_pgmap([&](const OSDMap& osdmap,
                                                const PGMap& pg_map) {
      return process_pg_map_command(
          prefix, cmdctx->cmdmap, pg_map, osdmap, f.get(), &ss, &cmdctx->odata);
    });
  ...
  if (!module->is_enabled()) {
    ss << "Module '" << py_handler_name
       << "' is not enabled (required by "
          "command '"
       << prefix << "'): use `ceph mgr module enable " << py_handler_name
       << "` to enable it";
    dout(4) << ss.str() << dendl;
  ...
  if (!py_modules.is_module_active(mod_name)) {
    ss << "Module '" << mod_name
       << "' did not initialize in time (required by "
          "command '"
       << prefix
       << "'): see "
  ...
  dout(10) << "passing through command '" << prefix << "' size "
           << cmdctx->cmdmap.size() << dendl;
  Finisher& mod_finisher = py_modules.get_active_module_finisher(mod_name);

  mod_finisher.queue(new LambdaContext([this, cmdctx, session, py_command,
                                        prefix, op, py_handler_name,
                                        module](int r_) mutable {
    std::stringstream ss;

    dout(10) << "dispatching command '" << prefix << "' size "
             << cmdctx->cmdmap.size() << dendl;
    ...
    int r = py_modules.handle_command(
        py_command, *session, cmdctx->cmdmap, inbl, &ds, &ss);
    if (r == -EACCES) {
      log_access_denied(cmdctx, session, ss);
    }
...
void
DaemonServer::_prune_pending_service_map()
{
...
{
  if (!pgmap_ready) {
    if (ceph_clock_now() - started_at >
        g_conf().get_val<int64_t>("mgr_stats_period") * 4.0) {
      pgmap_ready = true;
      reported_osds.clear();
    ...
    for (const auto& [key, state] : daemons) {
      ...
      for (const auto& metric : state->daemon_health_metrics) {
        ...
        if (acc == accumulated.end()) {
          auto collector =
              DaemonHealthMetricCollector::create(metric.get_type());
          if (!collector) {
            derr << __func__ << " " << key
                 << " sent me an unknown health metric: " << std::hex
                 << static_cast<uint8_t>(metric.get_type()) << std::dec
                 << dendl;
            continue;
          }
          tie(acc, std::ignore) =
              accumulated.emplace(metric.get_type(), std::move(collector));
        }
...
DaemonServer::adjust_pgs()
{
  dout(20) << dendl;
  uint64_t max =
      std::max<uint64_t>(1, g_conf().get_val<uint64_t>("mgr_max_pg_creating"));
  ...
  cluster_state.with_osdmap_and_pgmap([&](const OSDMap& osdmap,
    ...
    dout(10) << "creating_or_unknown " << creating_or_unknown
             << " max_creating " << max << " left " << left << dendl;
    ...
    pg_map.get_recovery_stats(
        &misplaced_ratio, &degraded_ratio, &inactive_pgs_ratio,
        &unknown_pgs_ratio);
    dout(20) << "misplaced_ratio " << misplaced_ratio << " degraded_ratio "
             << degraded_ratio << " inactive_pgs_ratio " << inactive_pgs_ratio
    ...
    for (auto& i : osdmap.get_pools()) {
      ...
      if (p.get_pg_num_target() != p.get_pg_num()) {
        dout(20) << "pool " << i.first << " pg_num " << p.get_pg_num()
                 << " target " << p.get_pg_num_target() << dendl;
        if (p.has_flag(pg_pool_t::FLAG_CREATING)) {
          dout(10) << "pool " << i.first << " pg_num_target "
                   << p.get_pg_num_target() << " pg_num " << p.get_pg_num()
                   << " - still creating initial pgs" << dendl;
          ...
          if (p.get_pg_num() != p.get_pg_num_pending()) {
            ...
            dout(10) << "pool " << i.first << " pg_num_target "
                     << p.get_pg_num_target() << " pg_num " << p.get_pg_num()
                     << " - decrease blocked by pgp_num " << p.get_pgp_num()
                     << dendl;
            ok = false;
          }
          ...
          for (auto& merge_participant : {merge_source, merge_target}) {
            ...
            if (q == pg_map.pg_stat.end()) {
              ...
              ok = false;
                ...
                (q->second.state & (PG_STATE_ACTIVE | PG_STATE_CLEAN)) !=
                (PG_STATE_ACTIVE | PG_STATE_CLEAN)) {
              dout(10) << "pool " << i.first << " pg_num_target "
                       << p.get_pg_num_target() << " pg_num " << p.get_pg_num()
                       << (is_merge_source ? " - merge source "
                                           : " - merge target ")
                       << merge_participant << " not clean ("
                       << pg_state_string(q->second.state) << ")" << dendl;
              ok = false;
            }
            if (is_merge_source) {
              ...
              dout(10) << "pool " << i.first << " pg_num_target "
                       ...
                       << (is_merge_source ? " - merge source "
                                           : " - merge target ")
                       << merge_participant << " acting does not match (source "
                       << source_acting << " != target " << q->second.acting
                       << ")" << dendl;
              ok = false;
            }
          }
          ...
          if (ok) {
            ...
            dout(10) << "pool " << i.first << " pg_num_target "
                     << p.get_pg_num_target() << " pg_num " << p.get_pg_num()
                     << " -> " << target << " (merging " << merge_source
                     << " and " << merge_target << ")" << dendl;
            ...
            for (auto& j : q->second) {
              if ((j.first & (PG_STATE_ACTIVE | PG_STATE_PEERED)) == 0) {
                dout(20) << "pool " << i.first << " has " << j.second
                         << " pgs in " << pg_state_string(j.first) << dendl;
                active = false;
                break;
              }
            }
          ...
          if (!active) {
            dout(10) << "pool " << i.first << " pg_num_target "
                     << p.get_pg_num_target() << " pg_num " << p.get_pg_num()
                     << " - not all pgs active" << dendl;
          } else if (pg_gap >= max_jump) {
            dout(10) << "pool " << i.first << " pg_num " << p.get_pg_num()
      ...
      if (target != p.get_pgp_num()) {
        dout(20) << "pool " << i.first << " pgp_num_target "
                 << p.get_pgp_num_target() << " pgp_num " << p.get_pgp_num()
                 << " -> " << target << dendl;
        if (target > p.get_pgp_num() && p.get_pgp_num() == p.get_pg_num()) {
          dout(10) << "pool " << i.first << " pgp_num_target "
                   << p.get_pgp_num_target() << " pgp_num " << p.get_pgp_num()
                   << " - increase blocked by pg_num " << p.get_pg_num()
                   << dendl;
        } else if (
            !aggro && (inactive_pgs_ratio > 0 || degraded_ratio > 0 ||
                       unknown_pgs_ratio > 0)) {
          dout(10) << "pool " << i.first << " pgp_num_target "
                   << p.get_pgp_num_target() << " pgp_num " << p.get_pgp_num()
          ...
          if (aggro ||
            ...
            double room = std::min<double>(
                max_misplaced - misplaced_ratio, max_misplaced / 2.0);
            unsigned estmax =
                std::max<unsigned>((double)p.get_pg_num() * room, 1u);
            unsigned next_min = 0;
            if (p.get_pgp_num() > estmax) {
              next_min = p.get_pgp_num() - estmax;
            }
            next = std::clamp(target, next_min, p.get_pgp_num() + estmax);
            dout(20) << " room " << room << " estmax " << estmax << " delta "
                     << (target - p.get_pgp_num()) << " next " << next << dendl;
...
DaemonServer::got_mgr_map()
{
  ...
  cluster_state.with_mgrmap([&](const MgrMap& mgrmap) {
    auto md_update = [&](DaemonKey key) {
      ...
      oss << "{\"prefix\": \"mgr metadata\", \"who\": \"" << key.name
          << "\", \"id\": \"" << key.name << "\"}";
      monc->start_mon_command({oss.str()}, {}, &c->outbl, &c->outs, c);
    };
    if (mgrmap.active_name.size()) {
      ...
      if (!daemon_state.exists(key) && !daemon_state.is_updating(key)) {
        md_update(key);
        dout(10) << "triggered addition of " << key << " via metadata update"
                 << dendl;
      }
    }
...
DaemonServer::get_tracked_keys() const noexcept
{
  return {"mgr_stats_threshold"s, "mgr_stats_period"s};
}

void
DaemonServer::handle_conf_change(
    const ConfigProxy& conf,
    const std::set<std::string>& changed)
{

  if (changed.count("mgr_stats_threshold") ||
      changed.count("mgr_stats_period")) {
...
void
DaemonServer::_send_configure(ConnectionRef c)
{
  ceph_assert(ceph_mutex_is_locked_by_me(lock));
  ...
  if (c->peer_is_osd()) {
    ...
    configure->metric_config_message = MetricConfigMessage(
        MDSConfigPayload(mds_perf_metric_collector.get_queries()));
  }
...
int
DaemonServer::remove_osd_perf_query(MetricQueryID query_id)
{
  return osd_perf_metric_collector.remove_query(query_id);
}

int
DaemonServer::get_osd_perf_counters(OSDPerfCollector* collector)
{
...
int
DaemonServer::remove_mds_perf_query(MetricQueryID query_id)
{
...
int
DaemonServer::get_mds_perf_counters(MDSPerfCollector* collector)
{
  return mds_perf_metric_collector.get_counters(collector);
}

bool
DaemonServer::asok_command(
    std::string_view admin_command,
    const cmdmap_t& cmdmap,
    Formatter* f,
    ostream& ss)
