...
struct offline_pg_report {
  ...
  std::set<pg_t> bad_no_pool, bad_already_inactive,
      bad_become_inactive; // not ok
...
struct upgrade_osd_report {
  ...
  bool
  all_osds_upgraded() const
  {
    return (
        (osds.size() == ok_upgraded.size()) && ok_upgrade.empty() &&
        bad_no_version.empty());
  }
...
protected:
  ...
  static const MonCommand* _get_mgrcommand(
...
private:
  ...
  bool
  _reply(MCommand* m, int ret, const std::string& s, const bufferlist& payload);
  ...
  void _update_upgraded_osds(
      ...
      upgrade_osd_report* osd_report);
  bool _valid_bucket_type_for_upgrade_check(std::string_view bucket_type_str);
  ...
  void
  handle_metric_payload(const OSDMetricPayload& payload)
  {
    osd_perf_metric_collector.process_reports(payload);
  }

  void
  handle_metric_payload(const MDSMetricPayload& payload)
  {
    mds_perf_metric_collector.process_reports(payload);
  }

  void
  handle_metric_payload(const UnknownMetricPayload& payload)
  {
    ceph_abort();
  }
