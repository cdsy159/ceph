...
#undef dout_prefix
#define dout_prefix \
  *_dout << "mgr.mds_perf_metric_collector " << __func__ << " "
...
void
MDSPerfMetricCollector::process_reports(const MetricPayload& payload)
{
  const MDSPerfMetricReport& metric_report =
      std::get<MDSMetricPayload>(payload).metric_report;

  std::lock_guard locker(lock);
  process_reports_generic(
      metric_report.reports,
      [](PerformanceCounter* counter, const PerformanceCounter& update) {
...
int
MDSPerfMetricCollector::get_counters(PerfCollector* collector)
{
...
void
MDSPerfMetricCollector::get_delayed_ranks(std::set<mds_rank_t>* ranks)
{
...
void
MDSPerfMetricCollector::get_last_updated(utime_t* ts)
{
  ceph_assert(ceph_mutex_is_locked(lock));
