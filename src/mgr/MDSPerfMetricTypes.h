...
struct MDSPerfMetricSubKeyDescriptor {
  ...
  MDSPerfMetricSubKeyDescriptor(
      MDSPerfMetricSubKeyType type,
      const std::string& regex_str) :
    type(type), regex_str(regex_str)
  {}

  bool
  operator<(const MDSPerfMetricSubKeyDescriptor& other) const
  {
    if (type < other.type) {
      return true;
    }
...
struct denc_traits<MDSPerfMetricKeyDescriptor> {
  ...
  static void
  bound_encode(const MDSPerfMetricKeyDescriptor& v, size_t& p)
  {
    p += sizeof(uint32_t);
  ...
  static void
  encode(
      const MDSPerfMetricKeyDescriptor& v,
  ...
  static void
  decode(MDSPerfMetricKeyDescriptor& v, ceph::buffer::ptr::const_iterator& p)
  {
    unsigned num;
    denc_varint(num, p);
    v.clear();
...
struct MDSPerformanceCounterDescriptor {
  ...
  bool
  operator<(const MDSPerformanceCounterDescriptor& other) const
  {
...
typedef std::vector<MDSPerformanceCounterDescriptor>
...
struct denc_traits<MDSPerformanceCounterDescriptors> {
  ...
  static void
  bound_encode(const MDSPerformanceCounterDescriptors& v, size_t& p)
  {
    p += sizeof(uint32_t);
    const auto size = v.size();
    if (size) {
  ...
  static void
  encode(
      const MDSPerformanceCounterDescriptors& v,
      ceph::buffer::list::contiguous_appender& p)
  {
    denc_varint(v.size(), p);
    for (auto& i : v) {
  ...
  static void
  decode(
      MDSPerformanceCounterDescriptors& v,
...
struct MDSPerfMetricLimit {
  ...
  MDSPerfMetricLimit(
      const MDSPerformanceCounterDescriptor& order_by,
      uint64_t max_count) :
    order_by(order_by), max_count(max_count)
  {}

  bool
  operator<(const MDSPerfMetricLimit& other) const
  {
...
struct MDSPerfMetricQuery {
  ...
  MDSPerfMetricQuery(
      const MDSPerfMetricKeyDescriptor& key_descriptor,
  ...
  bool
  operator<(const MDSPerfMetricQuery& other) const
  {
  ...
  template <typename L>
  bool
  get_key(L&& get_sub_key, MDSPerfMetricKey* key) const
  {
  ...
  get_performance_counter_descriptors(
      MDSPerformanceCounterDescriptors* descriptors) const
  {
  ...
  dump(ceph::Formatter* f) const
  {
    ...
    f->dump_stream("performance_counter_descriptors")
...
struct MDSPerfMetrics {
  MDSPerformanceCounterDescriptors performance_counter_descriptors;
  std::map<MDSPerfMetricKey, ceph::buffer::list>
...
struct MDSPerfMetricReport {
  ...
  static std::list<MDSPerfMetricReport>
...
public:
  ...
  uint64_t
  get_current_window_duration_sec() const
  {
    std::shared_lock lock(data_lock);
    if (data_points.size() < 2) {
      return 0;
    }
    auto duration = data_points.back().timestamp -
                    data_points.front().timestamp;
  ...
  uint64_t
  get_time_from_last_sample() const
  {
    if (data_points.empty()) {
      return std::numeric_limits<uint64_t>::max();
    }
    auto duration = std::chrono::steady_clock::now() -
                    data_points.back().timestamp;
    return std::chrono::duration_cast<std::chrono::seconds>(duration).count();
  }

private:
  void
  prune_old_data(TimePoint now)
  {
    TimePoint window_start = now - window_duration;
    while (!data_points.empty() &&
           data_points.front().timestamp < window_start) {
