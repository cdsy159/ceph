...
protected:
  ...
  typedef std::function<void(PerformanceCounter*, const PerformanceCounter&)>
      UpdateCallback;
  ...
  void process_reports_generic(
      const std::map<Query, Report>& reports,
      UpdateCallback callback);
  int get_counters_generic(
      MetricQueryID query_id,
      std::map<Key, PerformanceCounters>* counters);
...
private:
  ...
  bool
  is_limited(const std::map<MetricQueryID, OptionalLimit>& limits) const
  {
