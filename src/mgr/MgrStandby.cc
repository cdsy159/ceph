...
MgrStandby::get_tracked_keys() const noexcept
{
  return {// clog & admin clog
          "clog_to_monitors"s,
          "clog_to_syslog"s,
    ...
    const std::set<std::string>& changed)
{
  ...
  if (changed.count("mgr_standby_modules") && !active_mgr) {
    if (g_conf().get_val<bool>("mgr_standby_modules") !=
        py_module_registry.have_standby_modules()) {
...
MgrStandby::init()
{
  ...
  asok_hook.reset(new MgrHook(this));
  {
    int r = admin_socket->register_command(
        "status", asok_hook.get(), "show status");
    ceph_assert(r == 0);
  }
  ...
  r = monc.authenticate();
  if (r < 0) {
    derr << "Authentication failed, did you specify a mgr ID with a valid "
            "keyring?"
         << dendl;
    monc.shutdown();
    client_messenger->shutdown();
...
MgrStandby::send_beacon()
{
  ...
  if (active_mgr != nullptr) {
    available = active_mgr->is_initialized() ||
                active_mgr->exceeded_initialization_expiration();
  }
  ...
  auto m = ceph::make_message<MMgrBeacon>(
      monc.get_fsid(), monc.get_global_id(), g_conf()->name.get_id(), addrs,
      available, std::move(module_info), std::move(metadata),
      std::move(clients), CEPH_FEATURES_ALL);
...
MgrStandby::respawn()
{
  ...
  derr << "respawn execv " << orig_argv[0] << " failed with "
       << cpp_strerror(errno) << dendl;
  ceph_abort();
}

void
MgrStandby::_update_log_config()
{
...
void
MgrStandby::handle_mgr_map(ref_t<MMgrMap> mmap)
{
  ...
  if (active_in_map) {
    if (!active_mgr) {
      ...
      active_mgr.reset(new Mgr(
          &monc, map, &py_module_registry, client_messenger.get(), &objecter,
          clog, audit_clog));
...
MgrStandby::ms_dispatch2(const ref_t<Message>& m)
{
  std::lock_guard l(lock);
...
bool
MgrStandby::ms_handle_refused(Connection* con)
{
  // do nothing for now
  return false;
}
...
int
MgrStandby::main(vector<const char*> args)
{
  client_messenger->wait();
