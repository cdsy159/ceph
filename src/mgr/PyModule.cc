...
std::string
handle_pyerror(bool crash_dump, std::string module, std::string caller)
{
...
py_bytes_as_span(PyObject* bytes)
{
  assert(bytes);
  assert(PyBytes_CheckExact(bytes));
  Py_ssize_t length;
  char* buf;
  int r = PyBytes_AsStringAndSize(bytes, &buf, &length);
  assert(r == 0);
  return std::span<std::byte const>((const std::byte*)buf, size_t(length));
}

PyObject*
py_bytes_from_span(std::span<std::byte const> s)
{
...
std::vector<std::byte>
py_bytes_as_vec(PyObject* bytes)
{
  assert(bytes);
  assert(PyBytes_CheckExact(bytes));
  Py_ssize_t length;
...
PyObject*
py_bytes_from_vec(const std::vector<std::byte>& s)
{
  auto ret = PyBytes_FromStringAndSize(
...
namespace {
PyObject*
log_write(PyObject*, PyObject* args)
{
  char* m = nullptr;
...
static PyModuleDef ceph_logger_module = {
    PyModuleDef_HEAD_INIT, "ceph_logger", nullptr, -1, log_methods,
};
} // namespace
    ...
    const std::optional<std::string>& val)
{
  ...
  if (set_cmd.r == 0) {
    ...
    if (val) {
      dout(0) << "`config set mgr " << global_key << " " << val
              << "` failed: " << cpp_strerror(set_cmd.r) << dendl;
    } else {
      dout(0) << "`config rm mgr " << global_key
              << "` failed: " << cpp_strerror(set_cmd.r) << dendl;
    }
...
int
PyModule::load(PyThreadState* pMainThreadState)
{
  ceph_assert(pMainThreadState != nullptr);

  const auto subinterpreter_modules_opt =
      g_conf().get_val<std::string>("mgr_subinterpreter_modules");
  auto subinterpreter_modules = ceph::split(subinterpreter_modules_opt);
  use_main_interpreter = std::count(
                             subinterpreter_modules.begin(),
                             subinterpreter_modules.end(), "*") == 0 &&
                         std::count(
                             subinterpreter_modules.begin(),
                             subinterpreter_modules.end(), module_name) == 0;
  ...
  if (use_main_interpreter) {
    ...
    if (thread_state == nullptr) {
      derr << "Failed to create python sub-interpreter for '" << module_name
           << '"' << dendl;
      return -EINVAL;
    } else {
  ...
  // Environment is all good, import the external module
  {
    ...
    r = load_commands();
    if (r != 0) {
      derr << "Missing or invalid COMMANDS attribute in module '" << module_name
           << "'" << dendl;
    ...
    PyObject* pCanRunTuple = PyObject_CallMethod(
        pClass, const_cast<char*>("can_run"), const_cast<char*>("()"));
    if (pCanRunTuple != nullptr) {
      if (PyTuple_Check(pCanRunTuple) && PyTuple_Size(pCanRunTuple) == 2) {
        ...
        if (!PyBool_Check(pCanRun) || !PyUnicode_Check(can_run_str)) {
          derr << "Module " << get_name() << " returned wrong type in can_run"
               << dendl;
          ...
          if (!can_run) {
            ...
            dout(4) << "Module " << get_name()
                    << " reported that it cannot run: " << error_string
                    << dendl;
          }
        }
      } else {
    ...
    std::function<int(PyObject*)> fn)
{
  ...
  for (size_t i = 0; i < list_size; ++i) {
    ...
    if (!PyDict_Check(command)) {
      derr << "Module " << get_name() << " has non-dict entry " << "in "
           << attr_name << " list" << dendl;
      return -EINVAL;
    }
...
int
PyModule::register_options(PyObject* cls)
{
  PyObject* pRegCmd = PyObject_CallMethod(
      cls, const_cast<char*>("_register_options"), const_cast<char*>("(s)"),
      module_name.c_str());
  if (pRegCmd != nullptr) {
    Py_DECREF(pRegCmd);
  } else {
    derr << "Exception calling _register_options on " << get_name() << dendl;
    derr << handle_pyerror(true, module_name, "PyModule::register_options")
         << dendl;
  }
  return 0;
}
...
PyModule::load_notify_types()
{
  ...
  if (!PyObject_TypeCheck(ls, &PyList_Type)) {
    // Relatively easy mistake for human to make, e.g. defining COMMANDS
    // as a {} instead of a []
    derr << "Module " << get_name() << " has NOTIFY_TYPES that is not a list"
         << dendl;
    return -EINVAL;
  }
  ...
  for (size_t i = 0; i < list_size; ++i) {
    ...
    if (!PyObject_TypeCheck(notify_type, &PyUnicode_Type)) {
      derr << "Module " << get_name()
           << " has non-string entry in NOTIFY_TYPES list" << dendl;
      return -EINVAL;
    }
  ...
  Py_DECREF(ls);
  dout(10) << "Module " << get_name() << " notify_types " << notify_types
           << dendl;
...
int
PyModule::load_commands()
{
  PyObject* pRegCmd = PyObject_CallMethod(
      pClass, const_cast<char*>("_register_commands"), const_cast<char*>("(s)"),
      module_name.c_str());
  if (pRegCmd != nullptr) {
    Py_DECREF(pRegCmd);
  } else {
    derr << "Exception calling _register_commands on " << get_name() << dendl;
    derr << handle_pyerror(true, module_name, "PyModule::load_commands")
         << dendl;
  }
...
bool
PyModule::is_option(const std::string& option_name)
{
  std::lock_guard l(lock);
...
PyObject*
PyModule::get_typed_option_value(
...
int
PyModule::load_subclass_of(const char* base_class, PyObject** py_class)
{
  // load the base class
  ...
  if (!mgr_module) {
    ...
    derr << handle_pyerror(true, module_name, "PyModule::load_subclass_of")
         << dendl;
    return -EINVAL;
  }
  ...
  *py_class = nullptr;
  while (PyDict_Next(locals, &pos, &key, &value)) {
    ...
    *py_class = value;
    dout(4) << __func__ << ": found class: '" << module_name << "."
            << class_name << "'" << dendl;
  }
  Py_DECREF(mgr_module_type);
