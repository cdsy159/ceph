...
#define WCHAR(s) L## #s
  ...
  BOOST_SCOPE_EXIT_ALL(&py_config) { PyConfig_Clear(&py_config); };
  ...
  status = PyConfig_SetString(
      &py_config, &py_config.program_name, WCHAR(MGR_PYTHON_EXECUTABLE));
  ceph_assertf(
      !PyStatus_Exception(status), "PyConfig_SetString: %s:%s", status.func,
      status.err_msg);
  ...
  ceph_assertf(
      !PyStatus_Exception(status), "PyConfig_SetArgv: %s:%s", status.func,
      status.err_msg);
  // Add more modules
  ...
  status = PyConfig_SetBytesString(
      &py_config, &py_config.pythonpath_env, pythonpath_env.data());
  ceph_assertf(
      !PyStatus_Exception(status), "PyConfig_SetBytesString: %s:%s",
  ...
  ceph_assertf(
      !PyStatus_Exception(status), "Py_InitializeFromConfig: %s:%s",
      status.func, status.err_msg);
#undef WCHAR
  ...
  if (!failed_modules.empty()) {
    clog->error() << "Failed to load ceph-mgr modules: "
                  << joinify(
                         failed_modules.begin(), failed_modules.end(),
                         std::string(", "));
  }
}

bool
PyModuleRegistry::handle_mgr_map(const MgrMap& mgr_map_)
{
  ...
  if (mgr_map.epoch == 0) {
    ...
    for (const auto& [module_name, module] : modules) {
      ...
      const bool always_on =
          (mgr_map.get_always_on_modules().count(module_name) > 0);
    ...
    bool modules_changed =
        mgr_map_.modules != mgr_map.modules ||
...
void
PyModuleRegistry::standby_start(MonClient& mc, Finisher& f)
{
  std::lock_guard l(lock);
  ...
  standby_modules.reset(
      new StandbyPyModules(mgr_map, module_config, clog, mc, f));
    ...
    DaemonServer& server)
{
  ...
  active_modules.reset(new ActivePyModules(
      module_config, kv_store, mon_provides_kv_sub, ds, cs, mc, clog_,
      audit_clog_, objecter_, f, server, *this));
...
std::string
PyModuleRegistry::get_site_packages()
{
  ...
  if (site_packages_fn != nullptr) {
    ...
    auto site_packages_fn =
        PyObject_GetAttrString(site_module, "addsitepackages");
...
PyModuleRegistry::probe_modules(const std::string& path) const
{
...
int
PyModuleRegistry::handle_command(
...
{
  if (active_modules) {
    return active_modules->handle_command(
...
void
PyModuleRegistry::get_health_checks(health_check_map_t* checks)
{
  std::lock_guard l(lock);
  ...
  if (active_modules) {
    ...
    for (const auto& i : modules) {
      ...
      if (module->is_enabled() && !module->get_can_run()) {
        dependency_modules[module->get_name()] = module->get_error_string();
          ...
          (module->is_enabled() && !module->is_loaded()) ||
    ...
    if (!failed_modules.empty()) {
      ...
      auto& d = checks->add(
          "MGR_MODULE_ERROR", HEALTH_ERR, ss.str(), failed_modules.size());
...
void
PyModuleRegistry::handle_config(const std::string& k, const std::string& v)
{
...
void
PyModuleRegistry::handle_config_notify()
{
  std::lock_guard l(lock);
  if (active_modules) {
...
void
PyModuleRegistry::check_all_modules_started(Context* modules_start_complete)
{
