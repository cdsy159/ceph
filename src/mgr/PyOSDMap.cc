...
static PyObject*
osdmap_get_epoch(BasePyOSDMap* self, PyObject* obj)
{
  return PyLong_FromLong(self->osdmap->get_epoch());
}

static PyObject*
osdmap_get_crush_version(BasePyOSDMap* self, PyObject* obj)
{
  return PyLong_FromLong(self->osdmap->get_crush_version());
}

static PyObject*
osdmap_dump(BasePyOSDMap* self, PyObject* obj)
{
  PyFormatter f;
...
static PyObject*
osdmap_new_incremental(BasePyOSDMap* self, PyObject* obj)
{
...
static PyObject*
osdmap_apply_incremental(BasePyOSDMap* self, BasePyOSDMapIncremental* incobj)
{
...
static PyObject*
osdmap_get_crush(BasePyOSDMap* self, PyObject* obj)
{
  return construct_with_capsule(
      "mgr_module", "CRUSHMap", (void*)(&(self->osdmap->crush)));
}

static PyObject*
osdmap_get_pools_by_take(BasePyOSDMap* self, PyObject* args)
{
  int take;
...
static PyObject*
osdmap_calc_pg_upmaps(BasePyOSDMap* self, PyObject* args)
{
  PyObject* pool_list;
  ...
  if (!PyArg_ParseTuple(
          args, "OiiO:calc_pg_upmaps", &incobj, &max_deviation, &max_iterations,
          &pool_list)) {
    return nullptr;
  }
  ...
  for (auto i = 0; i < PyList_Size(pool_list); ++i) {
    ...
    auto pool_id =
        self->osdmap->lookup_pg_pool_name(PyUnicode_AsUTF8(pool_name));
    if (pool_id < 0) {
  ...
  dout(10) << __func__ << " osdmap " << self->osdmap << " inc " << incobj->inc
           << " max_deviation " << max_deviation << " max_iterations "
           << max_iterations << " pools " << pools << dendl;
  PyThreadState* tstate = PyEval_SaveThread();
  int r = self->osdmap->calc_pg_upmaps(
      g_ceph_context, max_deviation, max_iterations, pools, incobj->inc);
...
static PyObject*
osdmap_balance_primaries(BasePyOSDMap* self, PyObject* args)
{
  int pool_id;
  BasePyOSDMapIncremental* incobj;
  if (!PyArg_ParseTuple(args, "iO:balance_primaries", &pool_id, &incobj)) {
    return nullptr;
  }
  auto check_pool = self->osdmap->get_pg_pool(pool_id);
  if (!check_pool) {
    derr << __func__ << " pool '" << pool_id << "' does not exist" << dendl;
    return nullptr;
  }
  dout(10) << __func__ << " osdmap " << self->osdmap << " pool_id " << pool_id
           << " inc " << incobj->inc << dendl;
  PyThreadState* tstate = PyEval_SaveThread();
  OSDMap tmp_osd_map;
  tmp_osd_map.deepish_copy_from(*(self->osdmap));
  int r = self->osdmap->balance_primaries(
      g_ceph_context, pool_id, incobj->inc, tmp_osd_map);
  PyEval_RestoreThread(tstate);
  dout(10) << __func__ << " r = " << r << dendl;
  return PyLong_FromLong(r);
}
...
static PyObject*
osdmap_map_pool_pgs_up(BasePyOSDMap* self, PyObject* args)
{
  int poolid;
  ...
  for (unsigned ps = 0; ps < pi->get_pg_num(); ++ps) {
    ...
    self->osdmap->pg_to_up_acting_osds(
        pgid, &pm[pgid], nullptr, nullptr, nullptr);
  }
  PyFormatter f;
  for (auto p : pm) {
...
BasePyOSDMap_init(BasePyOSDMap* self, PyObject* args, PyObject* kwds)
{
  ...
  if (!PyArg_ParseTupleAndKeywords(
          args, kwds, "O", const_cast<char**>(kwlist), &osdmap_capsule)) {
    return -1;
  }
  if (!PyObject_TypeCheck(osdmap_capsule, &PyCapsule_Type)) {
    PyErr_Format(
        PyExc_TypeError, "Expected a PyCapsule_Type, not %s",
...
static PyObject*
osdmap_pg_to_up_acting_osds(BasePyOSDMap* self, PyObject* args)
{
  int pool_id = 0;
  int ps = 0;
  ...
  self->osdmap->pg_to_up_acting_osds(
      pg_id, &up, &up_primary, &acting, &acting_primary);
...
static PyObject*
osdmap_pool_raw_used_rate(BasePyOSDMap* self, PyObject* args)
{
  int pool_id = 0;
...
static PyObject*
osdmap_build_simple(PyObject* cls, PyObject* args, PyObject* kwargs)
{
  ...
  int num_osd = -1;
  if (!PyArg_ParseTupleAndKeywords(
          args, kwargs, "izi", const_cast<char**>(kwlist), &epoch, &uuid_str,
          &num_osd)) {
    Py_RETURN_NONE;
  }
  uuid_d uuid;
  ...
  return construct_with_capsule(
      "mgr_module", "OSDMap", reinterpret_cast<void*>(osdmap));
}

PyMethodDef BasePyOSDMap_methods[] = {
    {"_get_epoch", (PyCFunction)osdmap_get_epoch, METH_NOARGS,
     "Get OSDMap epoch"},
    ...
    {"_get_crush", (PyCFunction)osdmap_get_crush, METH_NOARGS,
     "Get CrushWrapper"},
    ...
    {"_pg_to_up_acting_osds", (PyCFunction)osdmap_pg_to_up_acting_osds,
     METH_VARARGS, "Calculate up+acting OSDs for a PG ID"},
    {"_pool_raw_used_rate", (PyCFunction)osdmap_pool_raw_used_rate,
     METH_VARARGS, "Get raw space to logical space ratio"},
    {"_build_simple", (PyCFunction)osdmap_build_simple,
     METH_VARARGS | METH_CLASS, "Create a simple OSDMap"},
    {NULL, NULL, 0, NULL}};
    ...
    PyObject* kwds)
{
  ...
  if (!PyArg_ParseTupleAndKeywords(
          args, kwds, "O", const_cast<char**>(kwlist), &inc_capsule)) {
    ceph_abort();
    return -1;
  }
...
static PyObject*
osdmap_inc_get_epoch(BasePyOSDMapIncremental* self, PyObject* obj)
{
  return PyLong_FromLong(self->inc->epoch);
}

static PyObject*
osdmap_inc_dump(BasePyOSDMapIncremental* self, PyObject* obj)
{
  PyFormatter f;
  self->inc->dump(&f);
  return f.get();
...
static int
get_int_float_map(PyObject* obj, map<int, double>* out)
{
...
static PyObject*
osdmap_inc_set_osd_reweights(BasePyOSDMapIncremental* self, PyObject* weightobj)
{
  map<int, double> wm;
  ...
  for (auto i : wm) {
    self->inc->new_weight[i.first] = std::max(0.0, std::min(1.0, i.second)) *
                                     0x10000;
  }
  Py_RETURN_NONE;
}
    ...
    PyObject* weightobj)
{
  ...
  for (auto i : wm) {
    crush.choose_args_adjust_item_weightf(
        g_ceph_context,
        crush.choose_args_get(CrushWrapper::DEFAULT_CHOOSE_ARGS), i.first,
        {i.second}, nullptr);
  }
...
PyMethodDef BasePyOSDMapIncremental_methods[] = {
    ...
    {"_set_osd_reweights", (PyCFunction)osdmap_inc_set_osd_reweights, METH_O,
...
PyTypeObject BasePyOSDMapIncrementalType = {
    PyVarObject_HEAD_INIT(
        NULL,
        0) "ceph_module.BasePyOSDMapIncremental", /* tp_name */
...
static int
BasePyCRUSH_init(BasePyCRUSH* self, PyObject* args, PyObject* kwds)
{
  PyObject* crush_capsule = nullptr;
  static const char* kwlist[] = {"crush_capsule", NULL};

  if (!PyArg_ParseTupleAndKeywords(
          args, kwds, "O", const_cast<char**>(kwlist), &crush_capsule)) {
    ceph_abort();
    return -1;
  }
  ceph_assert(PyObject_TypeCheck(crush_capsule, &PyCapsule_Type));

  auto ptr_ref = (std::shared_ptr<CrushWrapper>*)(PyCapsule_GetPointer(
      crush_capsule, nullptr));
...
static PyObject*
crush_dump(BasePyCRUSH* self, PyObject* obj)
{
  PyFormatter f;
  self->crush->dump(&f);
  return f.get();
...
static PyObject*
crush_get_item_name(BasePyCRUSH* self, PyObject* args)
{
  int item;
...
static PyObject*
crush_get_item_weight(BasePyCRUSH* self, PyObject* args)
{
  int item;
...
static PyObject*
crush_find_roots(BasePyCRUSH* self)
{
  set<int> roots;
...
static PyObject*
crush_find_takes(BasePyCRUSH* self, PyObject* obj)
{
  set<int> takes;
...
static PyObject*
crush_get_take_weight_osd_map(BasePyCRUSH* self, PyObject* args)
{
  int root;
