...
static std::ostream&
_dgraph_prefix(std::ostream* _dout, CephContext* cct)
{
  return *_dout << "DirectedGraph ";
}

static std::ostream&
_prefix(std::ostream* _dout, int rank, epoch_t epoch, uint64_t version)
{
  return *_dout << "rank: " << rank << " version: " << version
                << " ConnectionTracker(" << epoch << ") ";
}

std::ostream&
operator<<(std::ostream& o, const ConnectionReport& c)
{
  o << "rank=" << c.rank << ",epoch=" << c.epoch
    << ",version=" << c.epoch_version << ", current links: " << c.current
    << ", history: " << c.history;
  return o;
}

std::ostream&
operator<<(std::ostream& o, const ConnectionTracker& c)
{
...
ConnectionReport*
ConnectionTracker::reports(int p)
{
  auto i = peer_reports.find(p);
  if (i == peer_reports.end()) {
    ceph_assert(p != rank);
    auto [j, k] = peer_reports.insert(
        std::pair<int, ConnectionReport>(p, ConnectionReport()));
    i = j;
  }
  return &i->second;
}
...
ConnectionTracker::reports(int p) const
{
  auto i = peer_reports.find(p);
  if (i == peer_reports.end()) {
    return NULL;
...
void
ConnectionTracker::receive_peer_report(const ConnectionTracker& o)
{
...
bool
ConnectionTracker::increase_epoch(epoch_t e)
{
...
ConnectionTracker::increase_version()
{
  ...
  if (rank >= 0) {
    ...
    if ((version % persist_interval) == 0) {
      ldout(cct, 30) << version << " % " << persist_interval << " == 0"
                     << dendl;
      owner->persist_connectivity_scores();
    }
  } else {
    ldout(cct, 10) << "Got a report from a rank -1, not increasing our version!"
                   << dendl;
  }
}

void
ConnectionTracker::report_live_connection(int peer_rank, double units_alive)
{
  ldout(cct, 30) << __func__ << " peer_rank: " << peer_rank
                 << " units_alive: " << units_alive << dendl;
  ldout(cct, 30) << "my_reports before: " << my_reports << dendl;
  if (peer_rank == rank) {
    lderr(cct) << "Got a report from my own rank, hopefully this is startup "
                  "weirdness, dropping"
               << dendl;
    return;
  }
  if (peer_rank < 0) {
    ldout(cct, 10)
        << "Got a report from a rank -1, not adding that to our report!"
        << dendl;
    return;
  }
  ...
  if (i == my_reports.history.end()) {
    ldout(cct, 30) << "couldn't find: " << peer_rank << " in my_reports.history"
                   << "... inserting: " << "(" << peer_rank << ", 1" << dendl;
    auto [j, k] =
        my_reports.history.insert(std::pair<int, double>(peer_rank, 1.0));
    i = j;
  }
...
void
ConnectionTracker::report_dead_connection(int peer_rank, double units_dead)
{
  ldout(cct, 30) << __func__ << " peer_rank: " << peer_rank
                 << " units_dead: " << units_dead << dendl;
  ldout(cct, 30) << "my_reports before: " << my_reports << dendl;
  if (peer_rank == rank) {
    lderr(cct) << "Got a report from my own rank, hopefully this is startup "
                  "weirdness, dropping"
...
void
ConnectionTracker::get_total_connection_score(
    int peer_rank,
    double* rating,
    int* live_count) const
{
  ...
  *live_count = 0;
  ...
  for (const auto& i : peer_reports) { // loop through all the scores
    if (i.first ==
        peer_rank) { // ... except the ones it has for itself, of course!
      continue;
    }
...
void
ConnectionTracker::notify_rank_changed(int new_rank)
{
...
void
ConnectionTracker::notify_rank_removed(int rank_removed, int new_rank)
{
  ldout(cct, 20) << __func__ << " " << rank_removed << " new_rank: " << new_rank
                 << dendl;
  ...
  ceph_assert(
      (my_reports.current.size() == starting_size_current) ||
  ...
  while (pi != peer_reports.end()) {
    peer_reports[pi->first - 1] =
        pi->second; // copy content of next rank to ourself.
    peer_reports.erase(pi++); // destroy our next rank and move on.
  }

  ceph_assert(
      (peer_reports.size() == starting_size) ||
      (peer_reports.size() + 1 == starting_size));

  if (rank_removed <
      rank) { // if the rank removed is lower than us, we need to adjust.
    --rank;
...
void
DirectedGraph::add_outgoing_edge(unsigned from, unsigned to)
{
...
void
DirectedGraph::add_incoming_edge(unsigned to, unsigned from)
{
...
bool
DirectedGraph::has_outgoing_edge(unsigned from, unsigned to) const
{
  auto from_it = outgoing_edges.find(from);
  if (from_it == outgoing_edges.end()) {
    ldout(cct, 30) << "Node " << from << " has no outgoing edges" << dendl;
    return false;
  }
...
bool
DirectedGraph::has_incoming_edge(unsigned to, unsigned from) const
{
  auto to_it = incoming_edges.find(to);
  if (to_it == incoming_edges.end()) {
    ldout(cct, 30) << "Node " << to << " has no incoming edges" << dendl;
    return false;
  }
...
ConnectionTracker::get_netsplit(std::set<unsigned>& mons_down)
{
  ldout(cct, 30) << __func__ << dendl;
  /*
...
bool
ConnectionTracker::is_clean(int mon_rank, int monmap_size)
{
  ldout(cct, 30) << __func__ << dendl;
  ...
  if (rank != mon_rank || my_reports.rank != mon_rank) {
    ...
    if (peer_reports.rbegin()->first > monmap_size - 1)
      return false;
  }
  return true;
}
...
void
ConnectionTracker::encode(bufferlist& bl) const
{
  ENCODE_START(1, 1, bl);
  encode(rank, bl);
  encode(epoch, bl);
...
void
ConnectionTracker::decode(bufferlist::const_iterator& bl)
{
  clear_peer_reports();
  encoding.clear();
...
const bufferlist&
ConnectionTracker::get_encoded_bl()
{
  if (!encoding.length()) {
    encode(encoding);
  }
...
void
ConnectionReport::dump(ceph::Formatter* f) const
{
...
std::list<ConnectionReport>
ConnectionReport::generate_test_instances()
{
...
void
ConnectionTracker::dump(ceph::Formatter* f) const
{
  f->dump_int("rank", rank);
  f->dump_int("epoch", epoch);
...
std::list<ConnectionTracker>
ConnectionTracker::generate_test_instances()
{
