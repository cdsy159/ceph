...
static ostream&
_prefix(std::ostream* _dout, epoch_t epoch, ElectionOwner* elector)
{
  return *_dout << "paxos." << elector->get_my_rank() << ").electionLogic("
                << epoch << ") ";
}
...
void
ElectionLogic::bump_epoch(epoch_t e)
{
...
void
ElectionLogic::declare_standalone_victory()
{
...
void
ElectionLogic::clear_live_election_state()
{
  leader_acked = -1;
  electing_me = false;
...
void
ElectionLogic::reset_stable_tracker()
{
  stable_peer_tracker.reset(new ConnectionTracker(*peer_tracker));
}

void
ElectionLogic::connectivity_bump_epoch_in_election(epoch_t mepoch)
{
...
void
ElectionLogic::defer(int who)
{
  if (strategy == CLASSIC) {
    ldout(cct, 5) << "defer to " << who << dendl;
    ceph_assert(who < elector->get_my_rank());
  } else {
    ldout(cct, 5) << "defer to " << who << ", disallowed_leaders="
                  << elector->get_disallowed_leaders() << dendl;
...
bool
ElectionLogic::propose_classic_prefix(int from, epoch_t mepoch)
{
  if (mepoch > epoch) {
    bump_epoch(mepoch);
    ...
    if (epoch % 2 == 0 && // in a non-election cycle
      ...
      ldout(cct, 5) << " got propose from old epoch, " << from
    ...
    const ConnectionTracker* ct)
{
  ...
  if (from == elector->get_my_rank()) {
    lderr(cct) << "I got a propose from my own rank, hopefully this is startup "
                  "weirdness,dropping"
               << dendl;
    return;
  }
...
void
ElectionLogic::propose_disallow_handler(int from, epoch_t mepoch)
{
  ...
  bool their_win =
      ...
      (leader_acked < 0 ||
       leader_acked >=
           from); // they are a better choice than our previously-acked choice
  ...
  if (my_win) {
    ...
    if (leader_acked >= 0) { // we already acked someone
      ceph_assert(
          leader_acked < from ||
          from_disallowed); // and they still win, of course
...
void
ElectionLogic::propose_classic_handler(int from, epoch_t mepoch)
{
  if (propose_classic_prefix(from, mepoch)) {
    return;
  }
...
double
ElectionLogic::connectivity_election_score(int rank)
{
  ...
  if (stable_peer_tracker) {
    ...
    ldout(cct, 30)
        << "stable_peer_tracker does not exists, using peer_tracker ..."
        << dendl;
...
void
ElectionLogic::propose_connectivity_handler(
    int from,
    epoch_t mepoch,
    const ConnectionTracker* ct)
{
  ldout(cct, 10) << __func__ << " from " << from << " mepoch: " << mepoch
                 << " epoch: " << epoch << dendl;
  ...
  if (elector->is_tiebreaker(elector->get_my_rank()) &&
    ...
    ldout(cct, 10) << "Ignoring proposal from marked down mon " << from
                   << dendl;
    return;
  }
  if ((epoch % 2 == 0) && last_election_winner != elector->get_my_rank() &&
    ...
    ldout(cct, 30) << "Lets see if this out-of-quorum peer is worth it "
                   << dendl;
    int best_scorer = 0;
    double best_score = 0;
    double last_voted_for_score = 0;
    ldout(cct, 30) << "elector->paxos_size(): " << elector->paxos_size()
                   << dendl;
    ...
    ldout(cct, 30) << "best_scorer: " << best_scorer
                   << " best_score: " << best_score
                   << " last_voted_for: " << last_voted_for
                   << " last_voted_for_score: " << last_voted_for_score
                   << dendl;
    if (best_scorer == last_voted_for ||
      ...
      ldout(cct, 30) << "drop this message; it won't change our vote so we "
                        "defer to leader "
                     << dendl;
      return;
    }
  ...
  ldout(cct, 20) << "propose from rank=" << from << ", tracker: "
                 << (stable_peer_tracker ? *stable_peer_tracker : *peer_tracker)
                 << dendl;

  ldout(cct, 10) << "propose from rank=" << from << ",from_score=" << from_score
                 << "; my score=" << my_score << "; currently acked "
                 << leader_acked << ",leader_score=" << leader_score
                 << ",disallowed_leaders=" << elector->get_disallowed_leaders()
                 << dendl;

  bool my_win =
      (my_score >= 0) && // My score is non-zero; I am allowed to lead
      ((my_rank < from &&
        my_score >=
            from_score) || // We have same scores and I have lower rank, or
  ...
  bool their_win =
      ...
      ((from < my_rank &&
        from_score >=
            my_score) || // Either they have lower rank and same score, or
       (from_score > my_score)) && // their score is higher, AND
      ((from <= leader_acked &&
        from_score >=
            leader_score) || // same conditions compared to leader, or IS leader
  ...
  if (my_win) {
    ...
    if (leader_acked >= 0) { // we already acked someone
      ...
      // wait, i should win!
      if (!electing_me) {
        ldout(cct, 10) << " wait, i should win! triggering new election ..."
                       << dendl;
        elector->trigger_new_election();
      }
    }
    ...
    if (their_win || from == leader_acked) {
      if (leader_acked >= 0 && from != leader_acked) {
        ...
        leader_peer_tracker->get_total_connection_score(
            from, &leader_from_score, &leader_from_liveness);
        ...
        if ((from < leader_acked && leader_from_score >= leader_leader_score) ||
          ...
          peer_tracker->get_total_connection_score(
              leader_acked, &cur_leader_score, &cur_leader_live);
          peer_tracker->get_total_connection_score(
              from, &cur_from_score, &cur_from_live);
      ...
      // ignore them!
      ldout(cct, 5) << "no, we already acked " << leader_acked
                    << " with score >=" << from_score << dendl;
    }
  }
}

void
ElectionLogic::receive_ack(int from, epoch_t from_epoch)
{
  ceph_assert(from_epoch % 2 == 1); // sender in an election epoch
  if (from_epoch > epoch) {
    ldout(cct, 5) << "woah, that's a newer epoch, i must have rebooted.  "
                     "bumping and re-starting!"
                  << dendl;
...
bool
ElectionLogic::victory_makes_sense(int from)
{
  ...
  case CONNECTIVITY:
    ...
    ldout(cct, 5) << "victory from " << from
                  << " makes sense? lscore:" << leader_score
...
bool
ElectionLogic::receive_victory_claim(int from, epoch_t from_epoch)
{
  ...
  if (!election_okay) {
    ...
    ldout(cct, 1) << "I should have been elected over this leader; bumping and "
                     "restarting!"
                  << dendl;
    bump_epoch(from_epoch);
    start();
  ...
  if (from_epoch != epoch + 1) {
    ldout(cct, 5) << "woah, that's a funny epoch, i must have rebooted.  "
                     "bumping and re-starting!"
                  << dendl;
    bump_epoch(from_epoch);
    start();
