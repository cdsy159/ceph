...
private:
  ...
  struct log_channel_info {
    ...
    std::string expand_channel_meta(
    ...
    std::string
    get_facility(const std::string& channel)
    {
    ...
    std::string
    get_log_level(const std::string& channel)
    {
      return get_str_map_key(log_level, channel, &CLOG_CONFIG_DEFAULT_KEY);
    }

    bool
    do_log_to_graylog(const std::string& channel)
    {
      return (
          get_str_map_key(log_to_graylog, channel, &CLOG_CONFIG_DEFAULT_KEY) ==
          "true");
    }

    std::shared_ptr<ceph::logging::Graylog> get_graylog(
        const std::string& channel);

    bool
    do_log_to_journald(const std::string& channel)
    {
      return (
          get_str_map_key(log_to_journald, channel, &CLOG_CONFIG_DEFAULT_KEY) ==
          "true");
    }
