...
static ostream&
_prefix(ostream* _dout, Monitor& mon, const FSMap& fsmap)
{
  return *_dout << "mon." << mon.name << "@" << mon.rank << "("
                << mon.get_state_name() << ").mds e" << fsmap.get_epoch()
                << " ";
}
...
template <>
bool
cmd_getval(const cmdmap_t& cmdmap, string_view k, mds_gid_t& val)
{
...
template <>
bool
cmd_getval(const cmdmap_t& cmdmap, string_view k, mds_rank_t& val)
{
...
template <>
bool
cmd_getval(const cmdmap_t& cmdmap, string_view k, MDSMap::DaemonState& val)
{
  return cmd_getval(cmdmap, k, (int64_t&)val);
}
} // namespace TOPNSPC::common
...
template <int dblV>
void
MDSMonitor::print_map(const FSMap& m)
{
  dout(dblV) << "print_map\n";
  m.print(*_dout);
  *_dout << dendl;
...
// service methods
void
MDSMonitor::create_initial()
{
  dout(10) << "create_initial" << dendl;
}

void
MDSMonitor::get_store_prefixes(set<string>& s) const
{
...
MDSMonitor::update_from_paxos(bool* need_bootstrap)
{
  ...
  dout(10) << __func__ << " version " << version << ", my e "
           << get_fsmap().get_epoch() << dendl;
...
void
MDSMonitor::assign_quiesce_db_leader(FSMap& fsmap)
{

  // the quiesce leader is the lowest rank with the highest state up to ACTIVE
  auto less_leader = [](MDSMap::mds_info_t const* l,
                        MDSMap::mds_info_t const* r) {
  ...
  for (const auto& [fscid, fs] : std::as_const(fsmap)) {
    ...
    std::priority_queue<
        MDSMap::mds_info_t const*, std::vector<MDSMap::mds_info_t const*>,
        decltype(less_leader)>
    ...
    auto leader = member_info.empty() ? MDS_GID_NONE
                                      : member_info.top()->global_id;

    fsmap.modify_filesystem(
        fscid, [&leader, &members](auto& writable_fs) -> bool {
          return writable_fs.get_mds_map().update_quiesce_db_cluster(
              leader, std::move(members));
        });
  }
}

void
MDSMonitor::encode_pending(MonitorDBStore::TransactionRef t)
{
  ...
  for (const auto& i : info_map) {
    ...
    for (const auto& metric : health.metrics) {
      ...
      health_check_t* check = &new_checks.get_or_add(
          mds_metric_name(metric.type), metric.sev,
          mds_metric_summary(metric.type), 1);
      ostringstream ss;
  ...
  for (auto& p : new_checks.checks) {
    ...
    p.second.summary = std::regex_replace(
        p.second.summary, std::regex("%num%"),
...
MDSMonitor::get_trim_to() const
{
  ...
  if (g_conf()->mon_mds_force_trim_to > 0 &&
    ...
    dout(10) << __func__ << " explicit mon_mds_force_trim_to = " << floor
             << dendl;
  }
...
bool
MDSMonitor::preprocess_query(MonOpRequestRef op)
{
...
void
MDSMonitor::_note_beacon(MMDSBeacon* m)
{
...
bool
MDSMonitor::preprocess_beacon(MonOpRequestRef op)
{
  op->mark_mdsmon_event(__func__);
  ...
  if (!session->is_capable("mds", MON_CAP_X)) {
    dout(0) << "preprocess_beacon got MMDSBeacon from entity with insufficient "
               "privileges "
            << session->caps << dendl;
    goto ignore;
  }
  ...
  if (m->get_fsid() != mon.monmap->fsid) {
    dout(0) << "preprocess_beacon on fsid " << m->get_fsid()
            << " != " << mon.monmap->fsid << dendl;
    goto ignore;
  }

  dout(5) << "preprocess_beacon " << *m << " from " << m->get_orig_source()
          << " " << m->get_orig_source_addrs() << " " << m->get_compat()
          << dendl;
  ...
  if (!fsmap.gid_exists(gid)) {
    if (state != MDSMap::STATE_BOOT) {
       ...
       */
      auto m =
          make_message<MMDSMap>(mon.monmap->fsid, MDSMap::create_null_mdsmap());
  ...
  if (!(pending_daemon_health[gid] == m->get_health())) {
    dout(10) << __func__ << " health metrics for gid " << gid << " were updated"
             << dendl;
    _note_beacon(m);
    return false;
  }
...
reply:
  ...
  _note_beacon(m);
  {
    auto beacon = make_message<MMDSBeacon>(
        mon.monmap->fsid, m->get_global_id(), m->get_name(), effective_epoch,
...
bool
MDSMonitor::preprocess_offload_targets(MonOpRequestRef op)
{
  op->mark_mdsmon_event(__func__);
  auto m = op->get_req<MMDSLoadTargets>();
  dout(10) << "preprocess_offload_targets " << *m << " from "
  ...
  if (!session)
    goto ignore;
  if (!session->is_capable("mds", MON_CAP_X)) {
    dout(0) << "preprocess_offload_targets got MMDSLoadTargets from entity "
               "with insufficient caps "
            << session->caps << dendl;
    goto ignore;
  }
...
bool
MDSMonitor::prepare_update(MonOpRequestRef op)
{
  op->mark_mdsmon_event(__func__);
  auto m = op->get_req<PaxosServiceMessage>();
...
bool
MDSMonitor::prepare_beacon(MonOpRequestRef op)
{
  op->mark_mdsmon_event(__func__);
  auto m = op->get_req<MMDSBeacon>();
  // -- this is an update --
  ...
  dout(15) << __func__ << " got health from gid " << gid << " with "
           << m->get_health().metrics.size() << " metrics." << dendl;
  ...
  for (const auto& old_metric : old_health) {
    if (new_types.count(old_metric.type) == 0) {
      mon.clog->info() << "MDS health message cleared (" << m->get_orig_source()
                       << "): " << old_metric.message;
    }
  }
  ...
  if (state == MDSMap::STATE_BOOT) {
    ...
    if (state == MDSMap::STATE_DNE) {
      ...
      auto beacon = make_message<MMDSBeacon>(
          mon.monmap->fsid, m->get_global_id(), m->get_name(),
          get_fsmap().get_epoch(), m->get_state(), m->get_seq(),
    ...
    if ((info.state == MDSMap::STATE_STANDBY && state != info.state) ||
        (info.state == MDSMap::STATE_STANDBY_REPLAY && state != info.state &&
         state != MDSMap::STATE_DAMAGED)) {
      ...
      goto evict;
        ...
        info.state != MDSMap::STATE_STANDBY && state != info.state &&
    ...
    if (state == MDSMap::STATE_STOPPED) {
      ...
      mon.clog->info() << info.human_name() << " finished " << "stopping rank "
                       << info.rank << " in filesystem "
      ...
      const auto blocklist_epoch =
          mon.osdmon()->blocklist(rankinfo.addrs, until);
      ...
      if (info.state != MDSMap::STATE_ACTIVE && state == MDSMap::STATE_ACTIVE) {
        ...
        mon.clog->info() << info.human_name() << " is now active in "
                         << "filesystem " << fs.get_mds_map().fs_name
                         << " as rank " << info.rank;
      }
...
bool
MDSMonitor::prepare_offload_targets(MonOpRequestRef op)
{
  auto& pending = get_pending_fsmap_writeable();
  bool propose = false;
...
bool
MDSMonitor::should_propose(double& delay)
{
...
void
MDSMonitor::_updated(MonOpRequestRef op)
{
  const auto& fsmap = get_fsmap();
  op->mark_mdsmon_event(__func__);
  auto m = op->get_req<MMDSBeacon>();
  ...
  if (m->get_state() == MDSMap::STATE_STOPPED) {
    ...
    auto beacon = make_message<MMDSBeacon>(
        mon.monmap->fsid, m->get_global_id(), m->get_name(), fsmap.get_epoch(),
...
void
MDSMonitor::dump_info(Formatter* f)
{
...
bool
MDSMonitor::preprocess_command(MonOpRequestRef op)
{
  op->mark_mdsmon_event(__func__);
  ...
  if (prefix == "mds stat") {
    ...
    if (fsmap.get_num_standby() - standby.size() < active.size()) {
      ...
      ss << "insufficent standby MDS daemons to stop active gids "
         << stringify(active) << " and/or standby gids " << stringify(standby);
      ;
      goto out;
    }
    r = 0;
    ...
    if (f) {
      ...
      for (const auto& [fscid, fs] : fsmap) {
        f->open_object_section("filesystem");
        {
          ...
          f->dump_string(
              "metadata_pool",
              mon.osdmon()->osdmap.get_pool_name(mds_map.metadata_pool));
      ...
      for (const auto& [fscid, fs] : fsmap) {
        const MDSMap& mds_map = fs.get_mds_map();
        const string& md_pool_name =
            mon.osdmon()->osdmap.get_pool_name(mds_map.metadata_pool);

        ds << "name: " << mds_map.fs_name << ", metadata pool: " << md_pool_name
           << ", data pools: [";
        for (const auto& id : mds_map.data_pools) {
...
bool
MDSMonitor::fail_mds_gid(FSMap& fsmap, mds_gid_t gid)
{
  const auto& info = fsmap.get_info_gid(gid);
  dout(1) << "fail_mds_gid " << gid << " mds." << info.name << " role "
          << info.rank << dendl;
...
bool
MDSMonitor::is_leader() const
{
  return mon.is_leader();
}

mds_gid_t
MDSMonitor::gid_from_arg(const FSMap& fsmap, const string& arg, ostream& ss)
{
  ...
  if (r == 0) {
    ...
    if (fs.get_mds_map().is_up(role.rank)) {
      dout(10) << __func__ << ": validated rank/GID " << role << " as a rank"
               << dendl;
  ...
  if (!err.empty()) {
    ...
    dout(10) << __func__ << ": resolved MDS name '" << arg << "' to GID "
             << mds_info->global_id << dendl;
  ...
  dout(1) << __func__ << ": rank/GID " << arg << " not a existent rank or GID"
          << dendl;
  return MDS_GID_NONE;
}
...
bool
MDSMonitor::prepare_command(MonOpRequestRef op)
{
  op->mark_mdsmon_event(__func__);
  auto m = op->get_req<MMonCommand>();
  int r = -EINVAL;
...
bool
MDSMonitor::has_health_warnings(
    const vector<mds_metric_t>& warnings,
    const mds_gid_t& gid)
{
...
bool
MDSMonitor::has_any_health_warning()
{
  return std::any_of(
    ...
    stringstream& ss)
{
  ...
  if (prefix == "mds set_state") {
    ...
    if (!confirm && has_health_warnings(
                        {MDS_HEALTH_TRIM, MDS_HEALTH_CACHE_OVERSIZED}, gid)) {
    ...
    string_view fs_name =
        fsmap.get_filesystem(role.fscid).get_mds_map().get_fs_name();
    if (!op->get_session()->fs_name_capable(fs_name, MON_CAP_W)) {
      ss << "Permission denied.";
      return -EPERM;
    }

    fsmap.modify_filesystem(role.fscid, [role](auto&& fs) {
...
void
MDSMonitor::check_sub(Subscription* sub)
{
  ...
  if (sub->type == "fsmap") {
    ...
    if (is_mds) {
      ...
      if (!fsmap.filesystem_exists(fscid)) {
        ...
        // flag to MMDSMap?
        dout(1) << "Client subscribed to non-existent namespace '" << fscid
                << "'" << dendl;
        return;
      }
    }
    dout(10) << __func__ << ": is_mds=" << is_mds << ", fscid=" << fscid
             << dendl;
    ...
    dout(10) << __func__ << " selected MDS map epoch " << mds_map->epoch
             << " for namespace " << fscid << " for subscriber "
...
void
MDSMonitor::update_metadata(mds_gid_t gid, const map<string, string>& metadata)
{
...
void
MDSMonitor::remove_from_metadata(
    const FSMap& fsmap,
...
int
MDSMonitor::load_metadata(map<mds_gid_t, Metadata>& m)
{
  bufferlist bl;
...
void
MDSMonitor::count_metadata(const string& field, map<string, int>* out)
{
...
void
MDSMonitor::count_metadata(const string& field, Formatter* f)
{
...
void
MDSMonitor::get_versions(map<string, list<string>>& versions)
{
  map<mds_gid_t, Metadata> meta;
  load_metadata(meta);
  const auto& fsmap = get_fsmap();
...
int
MDSMonitor::print_nodes(Formatter* f)
{
  ceph_assert(f);
 ...
 */
bool
MDSMonitor::maybe_resize_cluster(FSMap& fsmap, const Filesystem& fs)
{
  ...
  if (in < max && !mds_map.test_flag(CEPH_MDSMAP_NOT_JOINABLE)) {
    ...
    mon.clog->info() << info->human_name()
                     << " assigned to "
                        "filesystem "
                     << mds_map.fs_name << " as rank " << mds << " (now has "
                     << mds_map.get_num_in_mds() + 1 << " ranks)";
    ...
    bool* osd_propose)
{
  ...
  if (info.is_frozen()) {
    return false;
      ...
      state == MDSMap::STATE_STANDBY_REPLAY || state == MDSMap::STATE_STANDBY) {
    ...
    *osd_propose |= fail_mds_gid(fsmap, gid);
    ...
    dout(1) << " replacing " << gid << " " << info.addrs << " mds." << rank
            << "." << info.inc << " " << ceph_mds_state_name(state) << " with "
            << rep_info->global_id << "/" << rep_info->name << " "
            << rep_info->addrs << dendl;

    mon.clog->warn() << "Replacing " << info.human_name() << " as rank " << rank
...
bool
MDSMonitor::check_health(FSMap& fsmap, bool* propose_osdmap)
{
  ...
  const auto mds_beacon_interval =
      g_conf().get_val<double>("mds_beacon_interval");
  ...
  for (auto it = last_beacon.begin(); it != last_beacon.end();) {
    ...
    if (since_last.count() >= g_conf()->mds_beacon_grace) {
      ...
      if ((mon_down || new_quorum) && since_last < mds_beacon_mon_down_grace) {
         ...
         */
        dout(10)
            << "deferring removal for mds_beacon_mon_down_grace during MON_DOWN"
            << dendl;
        ++it;
        continue;
      }
      ...
      if (!info.laggy()) {
        dout(1) << " marking " << gid << " " << info.addrs << " mds."
                << info.rank << "." << info.inc << " "
          ...
          if (better_affinity) {
            if (state == MDSMap::STATE_STANDBY_REPLAY) {
              ...
              *propose_osdmap |= fail_mds_gid(fsmap, gid);
              ...
              mon.clog->info()
                  << "Dropping low affinity active " << info.human_name()
                  << " in favor of higher affinity standby.";
...
bool
MDSMonitor::maybe_promote_standby(FSMap& fsmap, const Filesystem& fs)
{
  auto& mds_map = fs.get_mds_map();
  ...
  for (const auto& rank : failed) {
    ...
    if (info) {
      dout(1) << " taking over failed mds." << rank << " with "
              << info->global_id << "/" << info->name << " " << info->addrs
              << dendl;
...
{
  {
    auto _history_prune_time =
        g_conf().get_val<std::chrono::seconds>("mon_fsmap_prune_threshold");
    ...
    if (auto it = history.begin(); it != history.end()) {
      ...
      for (;;) {
        ...
        auto since =
            std::chrono::duration_cast<std::chrono::milliseconds>(now - btime);
        dout(20) << "loaded epoch " << fsmaph.get_epoch() << " which is "
                 << since << " old" << dendl;
  ...
  if (check_fsmap_struct_version) {
    ...
    if (elapsed > std::chrono::seconds(30)) {
      ...
      try {
        ...
        dout(5) << "flushing old fsmap struct because unable to decode FSMap: "
                << e.what() << dendl;
      }
      /* N.B. FSMap::is_struct_old is also true for undecoded (failed to decode) FSMap */
      if (fsmap.is_struct_old()) {
        dout(5)
            << "fsmap struct is too old; proposing to flush out old versions"
            << dendl;
        do_propose = true;
