...
class MgrStatMonitor : public PaxosService, public md_config_obs_t {
  // live version
  version_t version = 0;
  PGMapDigest digest;
...
public:
  ...
  bool enable_availability_tracking = g_conf().get_val<bool>(
      "enable_availability_tracking"); ///< tracking availability score feature
  double pool_availability_update_interval =
  ...
  const std::map<std::string, ProgressEvent>&
  get_progress_events()
  {
  ...
  // pg stat access
  const pool_stat_t*
  get_pool_stat(int64_t poolid) const
  {
  ...
  const std::map<uint64_t, PoolAvailability>&
  ...
  ceph_statfs
  get_statfs(OSDMap& osdmap, std::optional<int64_t> data_pool) const
  {
    return digest.get_statfs(osdmap, data_pool);
  }

  void
  print_summary(ceph::Formatter* f, std::ostream* out) const
  {
    digest.print_summary(f, out);
  }

  void
  dump_info(ceph::Formatter* f) const
  {
    digest.dump(f);
  ...
  void
  dump_cluster_stats(std::stringstream* ss, ceph::Formatter* f, bool verbose) const
  {
