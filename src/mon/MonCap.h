...
struct MonCapGrant {
  ...
  is_allow_all() const
  {
    return allow == MON_CAP_ANY && service.length() == 0 &&
           profile.length() == 0 && command.length() == 0 && fs_name.empty();
  }
...
struct MonCap {
  ...
  bool
  fs_name_capable(const EntityName& ename, std::string_view fs_name, __u8 mask)
  {
    for (auto& g : grants) {
      if (g.is_allow_all()) {
        return true;
