...
public:
  ...
  bool
  is_con(Connection* c) const
  {
    return con.get() == c;
  }

  void
  queue_command(Message* m)
  {
...
struct MonClientPinger : public Dispatcher, public AuthClient {
  ...
  MonClientPinger(CephContext* cct_, RotatingKeyRing* keyring, std::string* res_) :
    Dispatcher(cct_), result(res_), done(false), keyring(keyring)
  {}

  int
  wait_for_reply(double timeout = 0.0)
  {
    ...
    if (ping_recvd_cond.wait_for(locker, ceph::make_timespan(timeout), [this] {
          return done;
        })) {
      return 0;
    } else {
  ...
  {
    return mc->get_auth_request(
        auth_method, preferred_modes, bl, cct->_conf->name, 0, keyring);
  }
  ...
  {
    return mc->handle_auth_bad_method(
...
make_error_code(monc_errc e) noexcept
{
...
make_error_condition(monc_errc e) noexcept
{
  return {static_cast<int>(e), monc_category()};
}
...
private:
  ...
  std::map<entity_addrvec_t, MonConnection>::iterator
  _find_pending_con(const ConnectionRef& con)
  {
...
public:
  ...
  void
  set_entity_name(EntityName name)
  {
    entity_name = name;
  }

  void
  set_handle_authentication_dispatcher(Dispatcher* d)
  {
...
public:
  ...
  bool
  sub_want(std::string what, version_t start, unsigned flags)
  {
    std::lock_guard l(monc_lock);
  ...
  void
  sub_got(std::string what, version_t have)
  {
    std::lock_guard l(monc_lock);
    sub.got(what, have);
  }
  ...
  bool
  sub_want_increment(std::string what, version_t start, unsigned flags)
  {
    std::lock_guard l(monc_lock);
...
public:
  ...
  entity_addrvec_t
  get_mon_addrs(unsigned i) const
  {
    std::lock_guard l(monc_lock);
    if (i < monmap.size())
...
public:
  ...
  {
    return start_mon_command(
        -1, std::move(cmd), std::move(inbl),
         ...
         inbl = std::move(inbl)](auto handler) mutable {
          ...
          if (!initialized || stopping) {
            asio::dispatch(
                ...
                asio::append(
                    std::move(handler),
                    make_error_code(monc_errc::shutting_down), std::string{},
                    bufferlist{}));
          } else {
            auto r = new MonCommand(
                *this, ++last_mon_command_tid, std::move(handler));
...
public:
  ...
  template <typename CompletionToken>
  auto
  get_version(std::string&& map, CompletionToken&& token)
  {
    namespace asio = boost::asio;
    auto consigned = asio::consign(
        std::forward<CompletionToken>(token),
  ...
  auto
  with_monmap(Callback&& cb, Args&&... args) const
  ...
  void
  register_config_notify_callback(std::function<void(void)> f)
  {
    config_notify_cb = f;
  }
