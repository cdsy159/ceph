...
struct MonCommand {
  ...
  bool
  has_flag(uint64_t flag) const
  {
    return (flags & flag) == flag;
  }

  void
  set_flag(uint64_t flag)
  {
    flags |= flag;
  }

  void
  unset_flag(uint64_t flag)
  {
    flags &= ~flag;
  }
   ...
   */
  void
  encode_bare(ceph::buffer::list& bl) const
  {
    using ceph::encode;
    encode(cmdstring, bl);
  ...
  bool
  is_compat(const MonCommand* o) const
  {
    return cmdstring == o->cmdstring && module == o->module &&
           req_perms == o->req_perms;
  }
  ...
  static void
  encode_array(const MonCommand* cmds, int size, ceph::buffer::list& bl)
  {
    ENCODE_START(2, 1, bl);
    uint16_t s = size;
    encode(s, bl);
  ...
  static void
  encode_vector(const std::vector<MonCommand>& cmds, ceph::buffer::list& bl)
  {
    ENCODE_START(2, 1, bl);
