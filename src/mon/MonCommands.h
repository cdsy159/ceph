...
COMMAND(
    "osd last-stat-seq name=id,type=CephOsdName",
...
COMMAND(
    "auth export name=entity,type=CephString,req=false",
...
COMMAND(
    ...
    "rx")
COMMAND(
    "auth get-key name=entity,type=CephString",
    "display requested key",
    "auth",
    "rx")
COMMAND(
    "auth print-key name=entity,type=CephString",
    "display requested key",
    "auth",
    "rx")
COMMAND(
    "auth print_key name=entity,type=CephString",
    "display requested key",
    "auth",
    "rx")
COMMAND_WITH_FLAG(
...
COMMAND(
    ...
    "command.  If key already exists, any given caps must match the existing "
    "caps for that key.",
    "auth",
    "rwx")
COMMAND(
...
COMMAND(
    "health name=detail,type=CephChoices,strings=detail,req=false",
    "show cluster health",
    "mon",
    "r")
COMMAND(
...
COMMAND(
    "df name=detail,type=CephChoices,strings=detail,req=false",
    "show cluster free space stats",
    "mon",
    "r")
COMMAND(
    "report name=tags,type=CephString,n=N,req=false",
...
COMMAND_WITH_FLAG(
    ...
    FLAG(NONE))
COMMAND(
    "mon metadata name=id,type=CephString,req=false",
    "fetch metadata for mon <id>",
    "mon",
    "r")
COMMAND(
    "mon count-metadata name=property,type=CephString",
...
COMMAND(
    "mds last-seen name=id,type=CephString,req=true",
...
COMMAND_WITH_FLAG(
    ...
    FLAG(HIDDEN))
COMMAND(
    "mds fail name=role_or_gid,type=CephString "
    "name=yes_i_really_mean_it,type=CephBool,req=false",
    "Mark MDS failed: trigger a failover if a standby is available",
    "mds",
    "rw")
COMMAND(
    "mds repaired name=role,type=CephString",
...
COMMAND(
    ...
    "rw")
COMMAND_WITH_FLAG(
    "mds rmfailed name=role,type=CephString "
    "name=yes_i_really_mean_it,type=CephBool,req=false",
    "remove failed rank",
    "mds",
    "rw",
...
COMMAND(
    "fs get name=fs_name,type=CephString",
...
COMMAND(
    ...
    "rw")
COMMAND(
    "fs flag set name=flag_name,type=CephChoices,strings=enable_multiple "
    "name=val,type=CephString "
    "name=yes_i_really_mean_it,type=CephBool,req=false",
...
COMMAND(
    "fs lsflags name=fs_name,type=CephString",
...
COMMAND(
    "fs compat "
    "name=fs_name,type=CephString "
    "name=subop,type=CephChoices,strings=rm_compat|rm_incompat|add_compat|add_"
    "incompat "
...
COMMAND(
    "fs add_data_pool name=fs_name,type=CephString "
    "name=pool,type=CephString",
    "add data pool <pool>",
    "mds",
    "rw")
COMMAND(
    "fs rm_data_pool name=fs_name,type=CephString "
    "name=pool,type=CephString",
    "remove data pool <pool>",
    "mds",
    "rw")
COMMAND_WITH_FLAG(
    "fs set_default name=fs_name,type=CephString",
    "set the default to the named filesystem",
    "fs",
    "rw",
    FLAG(DEPRECATED))
COMMAND(
    "fs set-default name=fs_name,type=CephString",
    "set the default to the named filesystem",
    "fs",
    "rw")
COMMAND(
...
COMMAND(
    "osd tree "
    "name=epoch,type=CephInt,range=0,req=false "
    "name=states,type=CephChoices,strings=up|down|in|out|destroyed,n=N,req="
    "false",
    "print OSD tree",
    "osd",
    "r")
...
COMMAND(
    ...
    "r")
COMMAND(
    "osd count-metadata name=property,type=CephString",
...
COMMAND(
    "osd setcrushmap name=prior_version,type=CephInt,req=false",
    "set crush map from input file",
    "osd",
    "rw")
COMMAND(
    "osd crush set name=prior_version,type=CephInt,req=false",
    "set crush map from input file",
    "osd",
    "rw")
COMMAND(
...
COMMAND(
    ...
    "name=args,type=CephString,n=N,goodchars=[A-Za-z0-9-_.=]",
    "update crushmap position and weight for <name> to <weight> with location "
    "<args>",
    "osd",
    "rw")
COMMAND(
    ...
    "name=weight,type=CephFloat,range=0.0 "
    "name=args,type=CephString,n=N,goodchars=[A-Za-z0-9-_.=]",
    "add or update crushmap position and weight for <name> with <weight> and "
    "location <args>",
    "osd",
    "rw")
COMMAND(
...
COMMAND(
    "osd crush tunables "
    "name=profile,type=CephChoices,strings=legacy|argonaut|bobtail|firefly|"
...
COMMAND(
    ...
    "create crush rule <name> to start from <root>, replicate across buckets "
    "of type <type>, using a choose mode of <firstn|indep> (default firstn; "
...
COMMAND(
    ...
    "replicate across buckets of type <type>, use devices of type <class> (ssd "
    "or hdd)",
    "osd",
    "rw")
COMMAND(
...
COMMAND(
    ...
    "r")
COMMAND(
    "osd crush ls name=node,type=CephString,goodchars=[A-Za-z0-9-_.]",
...
COMMAND(
    ...
    "rw")
COMMAND(
    "osd crush weight-set rm name=pool,type=CephPoolname",
...
COMMAND(
    ...
    "name=force,type=CephBool,req=false "
    "name=yes_i_really_mean_it,type=CephBool,req=false",
    "create erasure code profile <name> with [<key[=value]> ...] pairs. Add a "
...
COMMAND(
    "osd pool get "
    "name=pool,type=CephPoolname "
    "name=var,type=CephChoices,strings=size|min_size|pg_num|pgp_num|crush_rule|"
    ...
    "fpp|use_gmt_hitset|target_max_objects|target_max_bytes|cache_target_dirty_"
    "ratio|cache_target_dirty_high_ratio|cache_target_full_ratio|cache_min_"
    "flush_age|cache_min_evict_age|erasure_code_profile|min_read_recency_for_"
    "promote|all|min_write_recency_for_promote|fast_read|hit_set_grade_decay_"
    "rate|hit_set_search_last_n|scrub_min_interval|scrub_max_interval|deep_"
    "scrub_interval|recovery_priority|recovery_op_priority|scrub_priority|"
    "compression_mode|compression_algorithm|compression_required_ratio|"
    "compression_max_blob_size|compression_min_blob_size|csum_type|csum_min_"
    "block|csum_max_block|allow_ec_overwrites|fingerprint_algorithm|pg_"
    "autoscale_mode|pg_autoscale_bias|pg_num_min|pg_num_max|target_size_bytes|"
    "target_size_ratio|dedup_tier|dedup_chunk_algorithm|dedup_cdc_chunk_size|"
    "eio|bulk|read_ratio|pct_update_delay|allow_ec_optimizations",
...
COMMAND(
    "osd pool set "
    "name=pool,type=CephPoolname "
    "name=var,type=CephChoices,strings=size|min_size|pg_num|pgp_num|pgp_num_"
    "actual|crush_rule|hashpspool|nodelete|nopgchange|nosizechange|write_"
    "fadvise_dontneed|noscrub|nodeep-scrub|hit_set_type|hit_set_period|hit_set_"
    "count|hit_set_fpp|use_gmt_hitset|target_max_bytes|target_max_objects|"
    "cache_target_dirty_ratio|cache_target_dirty_high_ratio|cache_target_full_"
    "ratio|cache_min_flush_age|cache_min_evict_age|min_read_recency_for_"
    "promote|min_write_recency_for_promote|fast_read|hit_set_grade_decay_rate|"
    "hit_set_search_last_n|scrub_min_interval|scrub_max_interval|deep_scrub_"
...
COMMAND(
    ...
    "add the tier <tierpool> (the second one) to base pool <pool> (the first "
    "one)",
    "osd",
    "rw")
COMMAND(
...
COMMAND(
    "osd tier cache-mode "
    "name=pool,type=CephPoolname "
    "name=mode,type=CephChoices,strings=writeback|proxy|readproxy|readonly|"
    "none "
    "name=yes_i_really_mean_it,type=CephBool,req=false",
...
COMMAND(
    ...
    "name=tierpool,type=CephPoolname "
    "name=size,type=CephInt,range=0",
    "add a cache <tierpool> (the second one) of size <size> to existing pool "
    "<pool> (the first one)",
    "osd",
    "rw")
...
COMMAND(
    ...
    "rw")
COMMAND(
    "mgr fail name=who,type=CephString,req=false",
...
COMMAND(
    ...
    "rw")
COMMAND(
    "mgr metadata name=who,type=CephString,req=false",
    "dump metadata for all daemons or a specific daemon",
    "mgr",
    "r")
COMMAND(
    "mgr count-metadata name=property,type=CephString",
...
COMMAND(
    ...
    " name=group,type=CephString"
    " name=location,type=CephString",
    " set location to clear Disaster state - failbacks allowed for recovered "
    "location",
    "mgr",
    "rw")
...
COMMAND_WITH_FLAG(
    ...
    FLAG(TELL))
COMMAND_WITH_FLAG(
    "smart name=devid,type=CephString,req=false",
...
COMMAND_WITH_FLAG(
    "heap "
    "name=heapcmd,type=CephChoices,strings=dump|start_profiler|stop_profiler|"
    "release|stats "
