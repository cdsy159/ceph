...
struct mon_info_t {
  ...
  mon_info_t(
      const std::string& n,
      const entity_addrvec_t& p_addrs,
      uint16_t p,
      uint16_t w) :
...
public:
  ...
  std::set<std::string>
      disallowed_leaders; // can't be leader under CONNECTIVITY/DISALLOW
  bool stretch_mode_enabled = false;
  std::string tiebreaker_mon;
  std::set<std::string>
      stretch_marked_down_mons; // can't be leader or taken proposal in CONNECTIVITY
...
public:
  ...
  unsigned
  min_quorum_size(unsigned total_mons = 0) const
  {
    if (total_mons == 0) {
      total_mons = size();
    }
   ...
   */
  void
  list_addrs(std::list<entity_addr_t>& ls) const
  {
    for (auto& i : mon_info) {
  ...
  int
  set_rank(const std::string& name, int rank)
  {
   ...
   */
  bool
  contains(const entity_addr_t& a, std::string* name = nullptr) const
  {
    for (auto& i : mon_info) {
      for (auto& j : i.second.public_addrs.v) {
        if (j == a) {
  ...
  bool
  contains(const entity_addrvec_t& av, std::string* name = nullptr) const
  {
    for (auto& i : mon_info) {
      for (auto& j : i.second.public_addrs.v) {
        for (auto& k : av.v) {
  ...
  std::string
  get_name(const entity_addr_t& a) const
  {
  ...
  std::string
  get_name(const entity_addrvec_t& av) const
  {
    for (auto& i : av.v) {
  ...
  int
  get_rank(const std::string& n) const
  {
  ...
  int
  get_rank(const entity_addr_t& a) const
  {
  ...
  int
  get_rank(const entity_addrvec_t& av) const
  {
  ...
  bool
  get_addr_name(const entity_addr_t& a, std::string& name)
  {
  ...
  get_addrs(const std::string& n) const
  {
  ...
  void
  set_addrvec(const std::string& n, const entity_addrvec_t& a)
  {
    ceph_assert(mon_info.count(n));
  ...
  uint16_t
  get_priority(const std::string& n) const
  {
  ...
  uint16_t
  get_weight(const std::string& n) const
  {
    auto it = mon_info.find(n);
    ceph_assert(it != mon_info.end());
  ...
  void
  set_weight(const std::string& n, uint16_t v)
  {
    auto it = mon_info.find(n);
    ceph_assert(it != mon_info.end());
    it->second.weight = v;
...
inline std::ostream&
operator<<(std::ostream& out, const MonMap& m)
{
  m.print_summary(out);
  return out;
}
