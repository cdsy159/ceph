...
struct MonOpRequest : public TrackedOp {
  ...
  void
  mark_svc_event(const std::string& service, const std::string& event)
  {
  ...
  void
  mark_logmon_event(const std::string& event)
  {
    mark_svc_event("logm", event);
  }

  void
  mark_osdmon_event(const std::string& event)
  {
    mark_svc_event("osdmap", event);
  }

  void
  mark_pgmon_event(const std::string& event)
  {
    mark_svc_event("pgmap", event);
  }

  void
  mark_mdsmon_event(const std::string& event)
  {
    mark_svc_event("mdsmap", event);
  }

  void
  mark_authmon_event(const std::string& event)
  {
    mark_svc_event("auth", event);
  }

  void
  mark_paxos_event(const std::string& event)
  {
...
private:
  ...
  MonOpRequest(Message* req, OpTracker* tracker) :
    TrackedOp(
        tracker,
        req->get_recv_stamp().is_zero() ? ceph_clock_now()
                                        : req->get_recv_stamp()),
    request(req),
    con(NULL),
...
protected:
  void
  _dump_op_descriptor(std::ostream& stream) const override
  {
...
struct C_MonOp : public Context {
  ...
  void
  mark_op_event(const std::string& event)
  {
    if (op)
      op->mark_event(event);
  }
