...
private:
  ...
  std::map<std::string, std::set<std::string>>
      dead_mon_buckets; // bucket->mon ranks, locations with no live mons
...
public:
  ...
  is_stretch_mode()
  {
    return stretch_mode_engaged;
  }

  bool
  is_degraded_stretch_mode()
  {
    return degraded_stretch_mode;
  }

  bool
  is_recovering_stretch_mode()
  {
    return recovering_stretch_mode;
  }
  ...
  void trigger_degraded_stretch_mode(
      const std::set<std::string>& dead_mons,
...
private:
  ...
  struct ScrubState {
    ...
    ScrubState() :
      finished(false), start(ceph_clock_now())
    {}
  ...
  boost::atomic_shared_ptr<ScrubContext>
      scrub_ctx; ///< keeps track of scrub_context
  ...
  struct SyncProvider {
    ...
    SyncProvider() :
      cookie(0), last_committed(0), full(false)
    {}

    void
    reset_timeout(CephContext* cct, int grace)
    {
  ...
  std::map<std::uint64_t, SyncProvider>
      sync_providers; ///< cookie -> SyncProvider for those syncing from us
...
private:
  ...
  void timecheck();
  health_status_t timecheck_status(
   ...
   */
  bool
  timecheck_has_skew(const double skew_bound, double* abs) const
  {
...
public:
  ...
  static void _generate_command_map(
      cmdmap_t& cmdmap,
...
protected:
  ...
  public:
    ...
    HealthCheckLogStatus(
        health_status_t severity_,
        const std::string& last_message_,
        utime_t updated_at_) :
      severity(severity_), last_message(last_message_), updated_at(updated_at_)
    {}
  };
...
public:
  ...
  struct RoutedRequest {
    ...
    RoutedRequest() :
      tid(0), session(NULL), con_features(0)
    {}
  ...
  void notify_new_monmap(
      bool can_change_external_state = false,
      bool remove_rank_elector = true);
...
private:
  ...
  static void read_features_off_disk(
      MonitorDBStore* store,
      CompatSet* read_features);
  void read_features();
...
public:
  ...
  void
  set_leader_commands(const std::vector<MonCommand>& cmds)
  {
...
#define CEPH_MON_FEATURE_INCOMPAT_BASE \
  CompatSet::Feature(1, "initial feature set (~v.18)")
#define CEPH_MON_FEATURE_INCOMPAT_GV \
  CompatSet::Feature(2, "global version sequencing (v0.52)")
#define CEPH_MON_FEATURE_INCOMPAT_SINGLE_PAXOS \
  CompatSet::Feature(3, "single paxos with k/v store (v0.\?)")
#define CEPH_MON_FEATURE_INCOMPAT_OSD_ERASURE_CODES \
  CompatSet::Feature(4, "support erasure code pools")
#define CEPH_MON_FEATURE_INCOMPAT_OSDMAP_ENC \
  CompatSet::Feature(5, "new-style osdmap encoding")
#define CEPH_MON_FEATURE_INCOMPAT_ERASURE_CODE_PLUGINS_V2 \
  CompatSet::Feature(6, "support isa/lrc erasure code")
#define CEPH_MON_FEATURE_INCOMPAT_ERASURE_CODE_PLUGINS_V3 \
  CompatSet::Feature(7, "support shec erasure code")
#define CEPH_MON_FEATURE_INCOMPAT_KRAKEN \
  CompatSet::Feature(8, "support monmap features")
#define CEPH_MON_FEATURE_INCOMPAT_LUMINOUS \
  CompatSet::Feature(9, "luminous ondisk layout")
#define CEPH_MON_FEATURE_INCOMPAT_MIMIC \
  CompatSet::Feature(10, "mimic ondisk layout")
#define CEPH_MON_FEATURE_INCOMPAT_NAUTILUS \
  CompatSet::Feature(11, "nautilus ondisk layout")
#define CEPH_MON_FEATURE_INCOMPAT_OCTOPUS \
  CompatSet::Feature(12, "octopus ondisk layout")
#define CEPH_MON_FEATURE_INCOMPAT_PACIFIC \
  CompatSet::Feature(13, "pacific ondisk layout")
#define CEPH_MON_FEATURE_INCOMPAT_QUINCY \
  CompatSet::Feature(14, "quincy ondisk layout")
#define CEPH_MON_FEATURE_INCOMPAT_REEF \
  CompatSet::Feature(15, "reef ondisk layout")
#define CEPH_MON_FEATURE_INCOMPAT_SQUID \
  CompatSet::Feature(16, "squid ondisk layout")
#define CEPH_MON_FEATURE_INCOMPAT_TENTACLE \
  CompatSet::Feature(17, "tentacle ondisk layout")
#define CEPH_MON_FEATURE_INCOMPAT_UMBRELLA \
  CompatSet::Feature(18, "umbrella ondisk layout")

// Release-independent features
#define CEPH_MON_FEATURE_INCOMPAT_NVMEOF_BEACON_DIFF \
  CompatSet::Feature(32, "nvmeof beacon diff")
...
public:
  C_MonContext(const Monitor* m, T&& f) :
    LambdaContext<T>(std::forward<T>(f)), mon(m)
  {}
