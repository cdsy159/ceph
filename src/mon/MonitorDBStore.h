...
public:
  ...
  std::shared_ptr<PriorityCache::PriCache>
  ...
  struct Op {
    ...
    void
    encode(ceph::buffer::list& encode_bl) const
    {
    ...
    void
    decode(ceph::buffer::list::const_iterator& decode_bl)
    {
    ...
    approx_size() const
    {
      return 6 + 1 + 4 + prefix.size() + 4 + key.size() + 4 + endkey.size() +
             4 + bl.length();
    }
  ...
  struct Transaction {
    ...
    void
    put(const std::string& prefix, version_t ver, const ceph::buffer::list& bl)
    {
    ...
    void
    put(const std::string& prefix, const std::string& key, version_t ver)
    {
      using ceph::encode;
      ceph::buffer::list bl;
      encode(ver, bl);
    ...
    void
    erase(const std::string& prefix, const std::string& key)
    {
    ...
    void
    erase(const std::string& prefix, version_t ver)
    {
      std::ostringstream os;
      os << ver;
      erase(prefix, os.str());
    ...
    void
    compact_prefix(const std::string& prefix)
    {
    ...
    void
    append_from_encoded(ceph::buffer::list& bl)
    {
    ...
    void
    dump(ceph::Formatter* f, bool dump_val = false) const
    {
  ...
  int
  apply_transaction(MonitorDBStore::TransactionRef t)
  {
    ...
    std::list<std::pair<std::string, std::pair<std::string, std::string>>>
        compact;
      ...
      while (!compact.empty()) {
        if (compact.front().second.first == std::string() &&
          ...
          db->compact_range_async(
              compact.front().first, compact.front().second.first,
              compact.front().second.second);
        compact.pop_front();
      }
    } else {
    ...
    finish(int r) override
    {
      ...
      if (delay_prob && (rand() % 10000 < delay_prob * 10000.0)) {
        ...
        lsubdout(g_ceph_context, mon, 1)
            << "apply_transaction will be delayed for " << delay << " seconds"
            << dendl;
        delay.sleep();
      }
  ...
  public:
    WholeStoreIteratorImpl(
        KeyValueDB::WholeSpaceIterator iter,
        std::set<std::string>& prefixes) :
      StoreIteratorImpl(), iter(iter), sync_prefixes(prefixes)
    {}
     ...
     */
    void
    get_chunk_tx(TransactionRef tx, uint64_t max_bytes, uint64_t max_keys) override
    {
      using ceph::encode;
      ...
      while (iter->valid()) {
        ...
        if (sync_prefixes.count(prefix)) {
          ...
          if (tx->empty() ||
              (tx->get_bytes() + value.length() + key.size() + prefix.size() <
                   max_bytes &&
  ...
  int
  get(const std::string& prefix, const std::string& key, ceph::buffer::list& bl)
  {
  ...
  int
  get(const std::string& prefix, const version_t ver, ceph::buffer::list& bl)
  {
    std::ostringstream os;
    os << ver;
  ...
  version_t
  get(const std::string& prefix, const std::string& key)
  {
    using ceph::decode;
    ceph::buffer::list bl;
  ...
  bool
  exists(const std::string& prefix, const std::string& key)
  {
  ...
  bool
  exists(const std::string& prefix, version_t ver)
  {
    std::ostringstream os;
    os << ver;
  ...
  std::string
  combine_strings(const std::string& prefix, const std::string& value)
  {
  ...
  std::string
  combine_strings(const std::string& prefix, const version_t ver)
  {
    std::ostringstream os;
    os << ver;
  ...
  int
  clear_key(const std::string& prefix, const std::string& key)
  {
    ceph_assert(!prefix.empty());
  ...
  void
  clear(std::set<std::string>& prefixes)
  {
    KeyValueDB::Transaction dbt = db->get_transaction();
  ...
  void
  _open(const std::string& kv_type)
  {
    int pos = 0;
  ...
  int
  create_and_open(std::ostream& out)
  {
  ...
  uint64_t
  get_estimated_size(std::map<std::string, uint64_t>& extras)
  {
   ...
   */
  int
  write_meta(const std::string& key, const std::string& value) const
  {
    std::string v = value;
    v += "\n";
    int r =
   ...
   */
  int
  read_meta(const std::string& key, std::string* value) const
  {
    char buf[4096];
