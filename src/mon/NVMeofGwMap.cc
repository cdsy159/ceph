...
void
NVMeofGwMap::to_gmap(std::map<NvmeGroupKey, NvmeGwMonClientStates>& Gmap) const
{
  Gmap.clear();
  for (const auto& created_map_pair : created_gws) {
    ...
    for (const auto& gw_created_pair : gw_created_map) {
      ...
      if (gw_created.availability == gw_availability_t::GW_DELETING) {
        dout(4) << gw_id << "Send empty unicast map in Deleting state" << dendl;
        continue;
      }

      auto gw_state = NvmeGwClientState(
          gw_created.ana_grp_id, epoch, availability,
          gw_created.beacon_sequence, gw_created.beacon_sequence_ooo,
          published_features);
      for (const auto& sub : gw_created.subsystems) {
        gw_state.subsystems.insert(
            {sub.nqn, NqnState(sub.nqn, gw_created.sm_state, gw_created)});
      }
    ...
    uint64_t features)
{
  ...
  if (HAVE_FEATURE(features, NVMEOFHAMAP)) {
    ...
    if (gw_epoch_it == gw_epoch.end()) {
      ...
      dout(10) << "Allocated first gw_epoch : group_key " << group_key
               << " epoch " << gw_epoch[group_key] << dendl;
    }
  }
  for (auto& itr : created_gws[group_key]) {
    ...
    if (itr.first == gw_id) {
      if (itr.second.availability != gw_availability_t::GW_DELETING) {
        ...
        dout(4) << "GW in Deleting state " << gw_id << " was created again"
                << dendl;
        return 0;
      }
    }
  ...
  for (auto& itr : created_gws[group_key]) {
    if (itr.second.availability == gw_availability_t::GW_DELETING) {
      ...
      created_gws[group_key][gw_id].performed_full_startup = true;
      created_gws[group_key][gw_id].availability = gw_availability_t::GW_CREATED;
      ...
      dout(4) << "Created GWS after create/delete:  " << created_gws << dendl;
      return 0;
    }
  }
  if (allocated.size() == MAX_SUPPORTED_ANA_GROUPS) {
    dout(4) << "Warning:  cannot add GW " << gw_id
            << " since number GWs in the group is " << MAX_SUPPORTED_ANA_GROUPS
            << dendl;
    return -EINVAL;
  }
...
int
NVMeofGwMap::cfg_delete_gw(const NvmeGwId& gw_id, const NvmeGroupKey& group_key)
{
    ...
    for (auto& gws_states : created_gws[group_key]) {
      if (gws_states.first == gw_id) {
        ...
        dout(4) << " Deleting  GW :" << gw_id << " in state  "
                << state.availability
                << " Resulting GW availability: " << state.availability
                << dendl;
...
void
NVMeofGwMap::check_all_gws_in_deleting_state(
    const NvmeGwId& gw_id,
    const NvmeGroupKey& group_key)
{
  for (auto& gws_states : created_gws[group_key]) {
...
int
NVMeofGwMap::do_erase_gw_id(const NvmeGwId& gw_id, const NvmeGroupKey& group_key)
{
...
int
NVMeofGwMap::do_delete_gw(const NvmeGwId& gw_id, const NvmeGroupKey& group_key)
{
  for (auto& gws_states : created_gws[group_key]) {
    ...
    if (gws_states.first == gw_id) {
      ...
      dout(10) << " Delete GW :" << gw_id << " ANA grpid: " << state.ana_grp_id
               << dendl;
...
int
NVMeofGwMap::cfg_admin_state_change(
    const NvmeGwId& gw_id,
    const NvmeGroupKey& group_key,
    gw_admin_state_t state,
  ...
  if (gw_state != gws_states.end()) {
    ...
    if (state == gw_admin_state_t::GW_ADMIN_DISABLED) {
      if (st.gw_admin_state == gw_admin_state_t::GW_ADMIN_ENABLED) {
        dout(4) << "GW-id set admin Disabled " << group_key << " " << gw_id
                << dendl;
      ...
      if (st.gw_admin_state == gw_admin_state_t::GW_ADMIN_DISABLED) {
        dout(4) << "GW-id set admin Enabled " << group_key << " " << gw_id
                << dendl;
        st.gw_admin_state = state;
        propose_pending = true;
      }
...
bool
NVMeofGwMap::validate_number_locations(int num_gws, int num_locations)
{
  return true; // TODO: add validation in the separate PR
}

int
NVMeofGwMap::cfg_set_location(
    const NvmeGwId& gw_id,
    const NvmeGroupKey& group_key,
    std::string& location,
    bool& propose_pending)
{
  ...
  if (!mon->get_quorum_mon_features().contains_all(
    ...
    dout(4) << "Command is not allowed - feature is not installed" << group_key
            << " " << gw_id << dendl;
    return -EINVAL;
  }
  ...
  if (gw_state != gws_states.end()) {
    ...
    if (st.location == location) {
      dout(4) << "GW-id same location is set " << group_key << " " << gw_id
              << " " << location << dendl;
      return -EEXIST;
      ...
      if (rc == false) {
        dout(4) << "defined invalid number of locations " << locations.size()
                << dendl;
        return -EINVAL;
      }
      if (last_gw) {
      ...
      dout(10) << "GW-id  location is set " << group_key << " " << gw_id << " "
               << location << dendl;
      propose_pending = true;
      return 0;
    }
...
bool
NVMeofGwMap::is_last_gw_in_location(
    const NvmeGwId& gw_id,
    const NvmeGroupKey& group_key,
...
bool
NVMeofGwMap::is_location_in_disaster(
    const NvmeGroupKey& group_key,
    NvmeLocation& location,
...
bool
NVMeofGwMap::get_location_in_disaster_cleanup(
    const NvmeGroupKey& group_key,
    NvmeLocation& returned_location)
{
  ...
  if (grp_it != disaster_locations.end()) {
    for (auto& entry : grp_it->second) {
      ...
      if (entry.second
              .failbacks_in_process) { // find first location in recovering state and return
...
void
NVMeofGwMap::disaster_map_remove_location(
    const NvmeGroupKey& group_key,
    NvmeLocation& location)
{
...
int
NVMeofGwMap::cfg_location_disaster_set(
    const NvmeGroupKey& group_key,
    std::string& location,
    bool& propose_pending)
{
  ...
  dout(10) << " set disaster location " << location << " in group " << group_key
           << dendl;
...
int
NVMeofGwMap::cfg_location_disaster_clear(
    const NvmeGroupKey& group_key,
    std::string& location,
    bool& propose_pending)
{
  ...
  if (!is_location_in_disaster(group_key, location, cleanup_in_process)) {
    ...
    dout(4) << "command cannot be accepted since recovering already started "
               "for group "
...
int
NVMeofGwMap::cfg_enable_disable_beacon_diff(bool enable, bool& propose_pending)
{
  int rc = 0;
...
void
NVMeofGwMap::gw_performed_startup(
    const NvmeGwId& gw_id,
    const NvmeGroupKey& group_key,
    bool& propose_pending)
{
...
void
NVMeofGwMap::increment_gw_epoch(const NvmeGroupKey& group_key)
{
...
int
NVMeofGwMap::get_num_namespaces(
    const NvmeGwId& gw_id,
    const NvmeGroupKey& group_key,
...
void
NVMeofGwMap::track_deleting_gws(
    const NvmeGroupKey& group_key,
    const BeaconSubsystems& subs,
    bool& propose_pending)
{
  ...
  for (auto& itr : created_gws[group_key]) {
    ...
    if (subs.size() &&
      ...
      int num_ns = 0;
      dout(4) << " to delete ? " << gw_id << " subsystems size " << subs.size()
              << dendl;
...
void
NVMeofGwMap::skip_failovers_for_group(
    const NvmeGroupKey& group_key,
    int interval_sec)
{
...
int
NVMeofGwMap::process_gw_map_gw_no_subsys_no_listeners(
    const NvmeGwId& gw_id,
    const NvmeGroupKey& group_key,
    bool& propose_pending)
{
  ...
  if (gw_state != gws_states.end()) {
    ...
    dout(1) << __FUNCTION__ << "ERROR GW-id was not found in the map " << gw_id
            << dendl;
    rc = -EINVAL;
  }
  return rc;
...
int
NVMeofGwMap::process_gw_map_gw_down(
    const NvmeGwId& gw_id,
    const NvmeGroupKey& group_key,
    bool& propose_pending)
{
    ...
    bool& propose_pending)
{
  ...
  auto gw_state = gws_states.find(gw_id);
  auto& st = gw_state->second;
  dout(20) << "KA beacon from the GW " << gw_id << " in state "
           << (int)st.availability << dendl;
  ...
  if (st.availability == gw_availability_t::GW_CREATED ||
    ...
    for (auto& state_itr : created_gws[group_key][gw_id].sm_state) {
      fsm_handle_gw_alive(
          gw_id, group_key, gw_state->second, state_itr.second, state_itr.first,
          last_osd_epoch, propose_pending);
    }
  }
  if (propose_pending) {
    validate_gw_map(group_key);
...
void
NVMeofGwMap::handle_abandoned_ana_groups(bool& propose)
{
  propose = false;
      ...
      if ((state.availability == gw_availability_t::GW_UNAVAILABLE ||
        ...
        for (auto& gw_state2 : gws_states) {
          ...
          if (state2.availability == gw_availability_t::GW_AVAILABLE &&
            ...
            found_gw_for_ana_group =
                true; // found GW that currently handles the ana group
            break;
          }
        }
        ...
        if (found_gw_for_ana_group == false) {
          ...
          state.availability == gw_availability_t::GW_AVAILABLE &&
          state.ana_grp_id != REDUNDANT_GW_ANA_GROUP_ID &&
...
void
NVMeofGwMap::check_relocate_ana_groups(
    const NvmeGroupKey& group_key,
    bool& propose)
{
  ...
  */
  if (!mon->get_quorum_mon_features().contains_all(
          ceph::features::mon::FEATURE_NVMEOF_BEACON_DIFF)) {
    dout(4) << "relocate is not allowed - feature is not installed" << group_key
            << dendl;
    return;
  }
  ...
  if (get_location_in_disaster_cleanup(group_key, location)) {
    ...
    if (num_gw_in_location ==
        num_active_ana_in_location) { // All ana groups of disaster location are in Active
      disaster_map_remove_location(group_key, location);
      dout(4) << "the location entry is erased " << location
              << " from disaster-locations num_ana_groups in location "
              << num_gw_in_location
              << " from the failbacks-in-progress of group " << group_key
              << dendl;
      propose = true;
      return;
    }
      ...
      for (auto& gw_state : gws_states) { // loop for GWs inside group-key
        ...
        if (state.sm_state[anagrp] == gw_states_per_group_t::GW_ACTIVE_STATE) {
          if (state.location == location) { // already relocated to the location
            ...
            dout(10) << "ana " << anagrp << " to relocate to " << location
                     << dendl;
            relocate_ana_grp(
                gw_state.first, group_key, anagrp, location, propose);
          }
        }
      }
    }
...
int
NVMeofGwMap::relocate_ana_grp(
    const NvmeGwId& src_gw_id,
    const NvmeGroupKey& group_key,
    NvmeAnaGrpId grpid,
    NvmeLocation& location,
    bool& propose)
      ...
      for (auto& state_itr : state.sm_state) {
        ...
        if ((state.sm_state[anagrp] != gw_states_per_group_t::GW_ACTIVE_STATE) &&
          ...
          dout(10) << "relocatе: found gw in intermediate state "
                   << gw_state.first << " state " << state.sm_state[anagrp]
                   << dendl;
          return 0;
        }
        if (state.sm_state[anagrp] == gw_states_per_group_t::GW_ACTIVE_STATE) {
...
void
NVMeofGwMap::set_failover_gw_for_ANA_group(
    const NvmeGwId& failed_gw_id,
    const NvmeGroupKey& group_key,
    const NvmeGwId& gw_id,
    NvmeAnaGrpId ANA_groupid)
{
  NvmeGwMonState& gw_state = created_gws[group_key][gw_id];
  NvmeGwMonState& failed_gw_state = created_gws[group_key][failed_gw_id];
  epoch_t epoch = 0;
  dout(10) << "Found failover GW " << gw_id << " for ANA group "
           << (int)ANA_groupid << dendl;
  if (failed_gw_state.availability == gw_availability_t::GW_CREATED) {
    dout(10) << "Failover GW " << gw_id
             << " takes over the group of GW in Created state " << failed_gw_id
             << dendl;
    ...
    bool& propose)
{
  ...
  for (auto& gw_state_it : gws_states) {
    ...
    if (st.sm_state[gw_state.ana_grp_id] !=
      ...
      do_failback = true;
      dout(10) << "Found some gw " << gw_state_it.first << " in state "
               << st.sm_state[gw_state.ana_grp_id] << dendl;
      break;
    }
  }
  ...
  for (auto& gw_state_it : gws_states) {
    ...
    if (st.sm_state[gw_state.ana_grp_id] ==
      ...
      dout(10) << "Found Failback GW " << failback_gw_id
               ...
               << gw_state.ana_grp_id << " of the available GW " << gw_id
               << "location " << st.location << dendl;
      if (st.location != gw_state.location && !allow_inter_location) {
        //not allowed inter-location failbacks
        dout(10) << "not allowed interlocation failbacks. GW " << gw_id
                 << dendl;
        return;
      }
...
int
NVMeofGwMap::find_failover_gw_logic(
    const NvmeGroupKey& group_key,
    NvmeGwMonStates& gws_states,
    NvmeLocation& location,
    NvmeGwId& min_loaded_gw_id,
    bool ignore_locations)
{
...
#define MIN_NUM_ANA_GROUPS 0xFFF
      ...
      for (auto& state_itr : st.sm_state) {
        ...
        if ((st.sm_state[anagrp] ==
          ...
          break; // dont take into account   GWs in the transitive state
            ...
            st.sm_state[anagrp] == gw_states_per_group_t::GW_ACTIVE_STATE) {
          // how many ANA groups are handled by this GW
  ...
  if (min_loaded_gw_id != ILLEGAL_GW_ID) { // some GW choosen
    ...
    dout(4) << "some GWs are busy " << num_busy << "num Available " << num_gws
            << dendl;
    return -EBUSY;
  } else {
...
void
NVMeofGwMap::find_failover_candidate(
    const NvmeGwId& gw_id,
    const NvmeGroupKey& group_key,
    NvmeAnaGrpId grpid,
    bool& propose_pending)
{
  ...
  if ((gw_state->second.sm_state[grpid] ==
    ...
    for (auto& found_gw_state : gws_states) {
      ...
      if (st.sm_state[grpid] == gw_states_per_group_t::GW_WAIT_BLOCKLIST_CMPL) {
        dout(4) << "Warning : Failover" << st.blocklist_data[grpid].is_failover
                << " already started for the group " << grpid << " by GW "
                << found_gw_state.first << dendl;
    ...
    int rc = find_failover_gw_logic(
        group_key, gws_states, ana_location, min_loaded_gw_id, false);
    if (rc == -ENOENT) {
      // looks at all GWs
      dout(10) << "Find Failover GW -look at all Gateways in the pool/group"
               << dendl;
      rc = find_failover_gw_logic(
          group_key, gws_states, ana_location, min_loaded_gw_id, true);
    }
...
void
NVMeofGwMap::fsm_handle_gw_alive(
    const NvmeGwId& gw_id,
    const NvmeGroupKey& group_key,
    NvmeGwMonState& gw_state,
    gw_states_per_group_t state,
    NvmeAnaGrpId grpid,
    epoch_t& last_osd_epoch,
    bool& map_modified)
{
  ...
  case gw_states_per_group_t::GW_WAIT_BLOCKLIST_CMPL: {
    ...
    if (gw_map.blocklist_data[grpid].osd_epoch <= last_osd_epoch) {
      dout(10) << "is-failover: " << gw_map.blocklist_data[grpid].is_failover
               ...
               << gw_map.blocklist_data[grpid].osd_epoch << " to "
               << last_osd_epoch << " Ana-grp: " << grpid
...
void
NVMeofGwMap::fsm_handle_gw_no_subsystems(
    const NvmeGwId& gw_id,
    const NvmeGroupKey& group_key,
    gw_states_per_group_t state,
    NvmeAnaGrpId grpid,
    bool& map_modified)
{
    ...
    for (auto& gw_st : created_gws[group_key]) {
      ...
      if (st.sm_state[grpid] ==
        ...
        dout(4) << "Warning: Outgoing Failback when GW is without subsystems"
                << " Owner GW set to standby state " << gw_st.first
                << "for ANA Group " << grpid << dendl;
  ...
  case gw_states_per_group_t::GW_ACTIVE_STATE: {
    dout(4) << "Set state to Standby for GW " << gw_id << " group " << grpid
            << dendl;
    auto& gw_st = created_gws[group_key][gw_id];
    gw_st.standby_state(grpid);
  } break;
...
void
NVMeofGwMap::fsm_handle_gw_down(
    const NvmeGwId& gw_id,
    const NvmeGroupKey& group_key,
    gw_states_per_group_t state,
    NvmeAnaGrpId grpid,
...
void
NVMeofGwMap::fsm_handle_gw_delete(
    const NvmeGwId& gw_id,
    const NvmeGroupKey& group_key,
    gw_states_per_group_t state,
    NvmeAnaGrpId grpid,
...
void
NVMeofGwMap::fsm_handle_to_expired(
    const NvmeGwId& gw_id,
    const NvmeGroupKey& group_key,
    NvmeAnaGrpId grpid,
    bool& map_modified)
{
  ...
  if (fbp_gw_state.sm_state[grpid] ==
      ...
      if (st.sm_state[grpid] ==
        ...
        if (st.availability == gw_availability_t::GW_AVAILABLE) {
          if (!(fbp_gw_state.last_gw_map_epoch_valid &&
            ...
            dout(10) << "gw " << gw_id << " or gw " << gw_state.first
                     << "map epochs are not updated " << dendl;
            return;
          }
        }
        cancel_timer(gw_id, group_key, grpid);
        ...
        if (st.availability == gw_availability_t::GW_AVAILABLE) {
          ...
          dout(10) << "Expired Failback-preparation timer from GW " << gw_id
                   << " ANA groupId " << grpid << dendl;
          break;
        } else {
          st.standby_state(grpid);
          dout(10)
              << "GW failed during failback/relocation persistency interval"
    ...
    if (grp_owner_found == false) {
      cancel_timer(gw_id, group_key, grpid);
      fbp_gw_state.standby_state(grpid);
      dout(4) << "group owner/relocation target not found " << grpid
              << " GW: " << gw_id << dendl;
...
struct CMonRequestProposal : public Context {
  ...
  finish(int r)
  {
    dout(10) << "osdmon is  writable? " << m->mon->osdmon()->is_writeable()
             << dendl;
...
void
NVMeofGwMap::get_health_checks(health_check_map_t* checks)
{
      ...
      if (gw_created.availability == gw_availability_t::GW_UNAVAILABLE) {
        ...
        if (group_it != gws_deleting_time.end()) {
          ...
          if (gw_it != gw_map.end()) {
            ...
            if ((now - delete_time) >
                g_conf()
                    .get_val<std::chrono::seconds>("mon_nvmeofgw_delete_grace")
                    .count()) {
              ostringstream ss;
  ...
  if (!singleGatewayDetail.empty()) {
    ...
    auto& d = checks->add(
        "NVMEOF_SINGLE_GATEWAY", HEALTH_WARN, ss.str(),
        singleGatewayDetail.size());
  ...
  if (!gatewayDownDetail.empty()) {
    ...
    auto& d = checks->add(
        "NVMEOF_GATEWAY_DOWN", HEALTH_WARN, ss.str(), gatewayDownDetail.size());
  ...
  if (!gatewayInDeletingDetail.empty()) {
    ...
    auto& d = checks->add(
        "NVMEOF_GATEWAY_DELETING", HEALTH_WARN, ss.str(),
        gatewayInDeletingDetail.size());
    d.detail.swap(gatewayInDeletingDetail);
  }
}

int
NVMeofGwMap::blocklist_gw(
    const NvmeGwId& gw_id,
    const NvmeGroupKey& group_key,
    NvmeAnaGrpId grpid,
    epoch_t& epoch,
    bool failover)
{
  ...
  for (const auto& sub :
       gw_map.subsystems) { // recreate nonce map from subsystems
    for (const auto& ns : sub.namespaces) {
      auto& nonce_vec =
          nonce_map[ns.anagrpid - 1]; //Converting  ana groups to offsets
      if (std::find(nonce_vec.begin(), nonce_vec.end(), ns.nonce) ==
          nonce_vec.end())
  ...
  for (auto& state_itr : gw_map.sm_state) {
    ...
    nonces.insert(
        nonces.end(), nonce_map[state_itr.first].begin(),
  ...
  if (nonces.size() > 0) {
    ...
    dout(10) << str << " parse success " << success
             << " network vector: " << addr_vect << " " << addr_vect.size()
             << dendl;
    if (!success) {
      return 1;
    }
    ...
    dout(10) << str << " mon->osdmon()->blocklist: epoch : " << epoch
             << " address vector: " << addr_vect << " " << addr_vect.size()
             << dendl;
  } else {
    dout(4) << "Error: No nonces context present for gw: " << gw_id
            << " ANA group: " << grpid << dendl;
    return 1;
  }
  return 0;
}
...
void
NVMeofGwMap::validate_gw_map(const NvmeGroupKey& group_key)
{
  for (auto& gw_created : created_gws[group_key]) {
...
bool
NVMeofGwMap::put_gw_beacon_sequence_number(
    const NvmeGwId& gw_id,
    int gw_version,
    const NvmeGroupKey& group_key,
...
void
NVMeofGwMap::update_active_timers(bool& propose_pending)
{
  ...
  for (auto& group_to : fsm_timers) {
    ...
    for (auto& gw_to : group_to.second) {
      ...
      for (auto& to_itr : to.data) {
        if (to.data[to_itr.first].timer_started == 0)
          continue;
        ...
        if (now >= to.data[to_itr.first].end_time) {
          fsm_handle_to_expired(
              gw_id, std::make_pair(pool, group), to_itr.first, propose_pending);
        }
      }
    }
  }
    ...
    uint8_t value_sec)
{
  ...
  dout(10) << "start timer for ana " << anagrpid << " gw " << gw_id
           << "value sec " << (int)value_sec << dendl;
  const auto now = std::chrono::system_clock::now();
