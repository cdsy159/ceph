...
void
NVMeofGwMon::cleanup_pending_map()
{
...
void
NVMeofGwMon::synchronize_last_beacon()
{
  dout(10) << "called, is leader : " << mon.is_leader() << " active "
           << is_active() << dendl;
...
void
NVMeofGwMon::on_shutdown()
{
  dout(10) << "called " << dendl;
}

void
NVMeofGwMon::check_beacon_timeout(
    ...
    bool& propose_pending)
{
  ...
  for (auto& itr : last_beacon) {
    ...
    if (last_beacon_time < (now - nvmegw_beacon_grace)) {
      ...
      dout(1) << "beacon timeout for GW " << lb.gw_id << " for " << seconds
              << " sec" << dendl;
      pending_map.process_gw_map_gw_down(
          lb.gw_id, lb.group_key, propose_pending);
      last_beacon.erase(lb);
    } else {
      dout(20) << "beacon live for GW " << lb.group_key << " " << lb.gw_id
               << dendl;
    }
  }
...
NVMeofGwMon::tick()
{
  ...
  const std::chrono::duration<double> mon_tick_interval(
  ...
  if (last_beacon_check != ceph::coarse_mono_clock::zero() &&
      (now - last_beacon_check >
       (2 * mon_tick_interval))) { // 1 mon tick was missed
    // This case handles either local slowness (calls being delayed
    // for whatever reason) or cluster election slowness (a long gap
    // between calls while an election happened)
    dout(4) << ": resetting beacon timeouts due to mon delay "
               "(slow election?) of "
            << now - last_beacon_check << " seconds" << dendl;
  ...
  if (mon.get_quorum_mon_features().contains_all(
    ...
    if ((pending_map.ever_enabled_features & NVMeofGwMap::FLAG_BEACONDIFF) ==
        0) {
 ...
 */
void
NVMeofGwMon::restore_pending_map_info(NVMeofGwMap& tmp_map)
{
  ...
  for (auto& created_map_pair : tmp_map.created_gws) {
    ...
    for (auto& gw_created_pair : gw_created_map) {
      ...
      if (gw_created_pair.second.allow_failovers_ts > now) {
        ...
        dout(10) << " restore skip-failovers timeout for gw  " << gw_id
                 << dendl;
...
NVMeofGwMon::recreate_gw_epoch()
{
  ...
  for (auto& created_map_pair : pending_map.created_gws) {
    auto group_key = created_map_pair.first;
    if (pending_map.gw_epoch.find(group_key) == pending_map.gw_epoch.end()) {
      pending_map.gw_epoch[group_key] = pending_map.epoch;
      dout(10) << "recreated gw epoch for group " << group_key << " set epoch "
               << pending_map.epoch << dendl;
    }
  }
}
...
void
NVMeofGwMon::encode_pending(MonitorDBStore::TransactionRef t)
{
  dout(10) << dendl;
...
void
NVMeofGwMon::update_from_paxos(bool* need_bootstrap)
{
  ...
  if (version != map.epoch) {
    dout(10) << " NVMeGW loading version " << version << " " << map.epoch
             << dendl;
    bufferlist bl;
...
bool
NVMeofGwMon::get_gw_by_addr(
    const entity_addr_t& sub_addr,
    NvmeGwId& gw_id,
 ...
 */
void
NVMeofGwMon::check_sub_unconditional(Subscription* sub)
{
  dout(10) << "sub->next , map-epoch " << sub->next << " " << map.epoch
           << dendl;
...
void
NVMeofGwMon::check_sub(Subscription* sub)
{
  NvmeGwId gw_id;
  NvmeGroupKey group_key;
  if (get_gw_by_addr(sub->session->con->get_peer_addr(), gw_id, group_key)) {
...
bool
NVMeofGwMon::preprocess_query(MonOpRequestRef op)
{
  dout(20) << dendl;
...
bool
NVMeofGwMon::prepare_update(MonOpRequestRef op)
{
  auto m = op->get_req<PaxosServiceMessage>();
  switch (m->get_type()) {
  case MSG_MNVMEOF_GW_BEACON:
...
bool
NVMeofGwMon::preprocess_command(MonOpRequestRef op)
{
  dout(10) << dendl;
  ...
  if (prefix == "nvme-gw show") {
    ...
    if (HAVE_FEATURE(mon.get_quorum_con_features(), NVMEOFHA)) {
      ...
      if (map.created_gws[group_key].size()) {
        ...
        uint32_t index =
            ((seconds_since_1970 / 60) % map.created_gws[group_key].size());
      ...
      if (subsystems && subsystems->size()) {
        for (auto& subs_it : *subsystems) {
          for (auto& ns : subs_it.namespaces) {
            if (num_ns.find(ns.anagrpid) == num_ns.end())
              num_ns[ns.anagrpid] = 0;
    ...
    cmd_getval(cmdmap, "group", group);
    auto group_key = std::make_pair(pool, group);
    dout(10) << "nvme-gw listeners pool " << pool << " group " << group
             << dendl;
...
void
NVMeofGwMon::get_gw_listeners(
    Formatter* f,
    std::pair<std::string, std::string>& group_key)
{
  std::map<std::string, std::list<std::pair<BeaconListener, std::string>>>
      subsystem_listeners;
  for (auto& gw_created_pair : map.created_gws[group_key]) {
    auto& gw_id = gw_created_pair.first;
    auto& state = gw_created_pair.second;
...
bool
NVMeofGwMon::prepare_command(MonOpRequestRef op)
{
  dout(10) << dendl;
  auto m = op->get_req<MMonCommand>();
  int rc;
  ...
  if (prefix == "nvme-gw create" || prefix == "nvme-gw delete") {
    ...
    if (prefix == "nvme-gw create") {
      rc = pending_map.cfg_add_gw(id, group_key, mon.get_quorum_con_features());
      if (rc == -EINVAL) {
        err = rc;
        dout(4) << "Error: GW cannot be created " << id << " " << pool << " "
                << group << "  rc " << rc << dendl;
        sstrm.str("");
      }
    } else {
      ...
      if (rc == 0) {
        ...
        process_gw_down(
            id, group_key, propose, gw_availability_t::GW_UNAVAILABLE);
        pending_map.check_all_gws_in_deleting_state(id, group_key);
      } else if (rc == -EINVAL) {
        dout(4) << "Error: GW not found in the database " << id << " " << pool
                << " " << group << "  rc " << rc << dendl;
        err = 0;
        sstrm.str("");
      }
    ...
    if (rc == -EINVAL) {
      err = rc;
      dout(4) << "Error: GW cannot be set to admin state " << id << " " << pool
              << " " << group << "  rc " << rc << dendl;
      sstrm.str("");
    }
    // propose pending would be generated by the PaxosService
    ...
    if (rc == -EINVAL || rc == -EEXIST) {
      err = rc;
      dout(4) << "Error: GW cannot  set location " << id << " " << pool << " "
              << group << "  rc " << rc << dendl;
      sstrm.str("");
      if (rc == -EEXIST) {
    ...
    cmd_getval(cmdmap, "location", location);
    auto group_key = std::make_pair(pool, group);
    dout(10) << id << " pool " << pool << " group " << group << " location "
             << location << dendl;
    rc = pending_map.cfg_location_disaster_set(group_key, location, propose);
    ...
    cmd_getval(cmdmap, "location", location);
    auto group_key = std::make_pair(pool, group);
    dout(10) << " pool " << pool << " group " << group << " location "
             << location << dendl;
    rc = pending_map.cfg_location_disaster_clear(group_key, location, propose);
    if (rc == -EINVAL || rc == -EEXIST || rc == -EOPNOTSUPP) {
      err = rc;
      sstrm.str("");
      if (rc == -EEXIST) {
    ...
    if (choice == "beacon-diff") {
      bool propose = false;
      dout(10) << "Command " << prefix << " " << choice << " " << value
               << dendl;
  ...
  if (response == false) {
    if (err < 0 && rs.length() == 0) {
      rs = cpp_strerror(err);
      dout(10) << "Error command  err : " << err << " rs-len: " << rs.length()
               << dendl;
    }
    mon.reply_command(op, err, rs, rdata, get_last_committed());
  } else {
    ...
    gw_availability_t avail)
{
  ...
  if (it != last_beacon.end()) {
    ...
    if (avail == gw_availability_t::GW_UNAVAILABLE) {
      ...
      pending_map.process_gw_map_gw_no_subsys_no_listeners(
          gw_id, group_key, propose_pending);
    }
  }
}

bool
NVMeofGwMon::preprocess_beacon(MonOpRequestRef op)
{
...
epoch_t
NVMeofGwMon::get_ack_map_epoch(bool gw_created, const NvmeGroupKey& group_key)
{
  epoch_t rc;
  if (!gw_created) {
    rc = 0;
...
void
NVMeofGwMon::do_send_map_ack(
    MonOpRequestRef op,
    bool gw_created,
    bool gw_propose,
    uint64_t stored_sequence,
    bool is_correct_sequence,
    const NvmeGroupKey& group_key,
    const NvmeGwId& gw_id)
{
  ...
  */
  ...
  if (!gw_created)
    dout(10) << "gw not created, ack map " << ack_map << " epoch "
             << ack_map.epoch << dendl;
 ...
 */
int
NVMeofGwMon::apply_beacon(
    const NvmeGwId& gw_id,
    int affected_version,
    const NvmeGroupKey& group_key,
    void* msg,
    const BeaconSubsystems& sub,
    gw_availability_t& avail,
    bool& propose_pending)
{
  bool found = false;
  ...
  if (affected_version == BEACON_VERSION_LEGACY) {
    if (gw_subs != sub) {
      dout(10) << "BEACON_DIFF logic not applied."
                  "subsystems of GW changed, propose pending "
               << gw_id << dendl;
      gw_subs = sub;
      //rebuild  nonce map.
    ...
    if (state.beacon_sequence_ooo) {
      dout(10) << "Good sequence after out of order detection "
                  "sequence "
               << state.beacon_sequence << " " << gw_id << dendl;
    ...
    for (auto& subs_it : sub) {
      if (subs_it.change_descriptor == subsystem_change_t::SUBSYSTEM_ADDED ||
        ...
        if (!found) {
          ...
          subs_it.change_descriptor == subsystem_change_t::SUBSYSTEM_DELETED) {
        auto it = std::find_if(
            gw_subs.begin(), gw_subs.end(), [&](const auto& gw_subs_it) {
...
bool
NVMeofGwMon::prepare_beacon(MonOpRequestRef op)
{
  auto m = op->get_req<MNVMeofGwBeacon>();
  uint64_t sequence = m->get_sequence();
  int version = m->version;
  uint16_t header_ver = m->get_header().version;
  dout(10) << "availability " << m->get_availability() << " sequence "
           << sequence << " version " << version << " header version "
           << header_ver << " GW : " << m->get_gw_id() << " osdmap_epoch "
           << m->get_last_osd_epoch() << " subsystems "
           << m->get_subsystems().size() << dendl;
  ...
  uint64_t stored_sequence;
  NVMeofGwMap ack_map;
  bool epoch_filter_enabled =
      HAVE_FEATURE(mon.get_quorum_con_features(), NVMEOFHAMAP);
  ...
  if (avail == gw_availability_t::GW_CREATED) {
    if (!gw_exists) {
      ...
      dout(10) << "Warning: GW " << gw_id << " group_key " << group_key
               << " was not found in the  map.created_gws " << map.created_gws
               << dendl;
      goto set_propose;
    } else {
      ...
      pending_map.set_gw_beacon_sequence_number(
      ...
      if (pending_map.created_gws[group_key][gw_id].availability ==
        ...
        dout(1) << " Warning :GW marked as Available in the NVmeofGwMon "
                << "database, performed full startup - Apply it but don't "
                   "allow failover!"
                << gw_id << dendl;
        process_gw_down(gw_id, group_key, gw_propose, avail);
        pending_map.skip_failovers_for_group(group_key);
        dout(4) << "fast_reboot:set skip-failovers for group " << gw_id
  ...
  if (pending_map.created_gws[group_key][gw_id].availability ==
      gw_availability_t::GW_DELETING) {
    dout(4) << "GW sends beacon in DELETING state, ignore it" << gw_id << dendl;
    mon.no_reply(op);
    goto false_return; // not sending ack to this beacon
  }
  ...
  if (apply_beacon(
          gw_id, header_ver, group_key, (void*)m, sub, avail, propose) != 0) {
    nonce_propose = true;
    dout(10)
        << "subsystem(subs/listener/nonce/NM) of GW changed, propose pending "
  ...
  if (pending_map.created_gws[group_key][gw_id].last_gw_map_epoch_valid ==
    ...
    dout(20) << "map epoch of gw is not up-to-date " << gw_id << " epoch "
             << get_ack_map_epoch(true, group_key) << " beacon_epoch "
             << m->get_last_gwmap_epoch() << dendl;
  }
...
set_propose:
  ...
  if ((apply_ack_logic &&
    ...
    if (apply_ack_logic) {
      dout(20) << "ack sent: beacon index "
               ...
               << " propose " << propose << " gw-propose " << gw_propose
               << dendl;
    }
  }
  if (send_ack && ((!gw_propose && epoch_filter_enabled) ||
                   ...
                   (avail == gw_availability_t::GW_CREATED))) {
    do_send_map_ack(
        op, gw_created, gw_propose, stored_sequence, correct_sequence,
        group_key, gw_id);
  } else {
    mon.no_reply(op);
  }
