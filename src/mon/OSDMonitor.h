...
struct failure_info_t {
  ...
  void
  add_report(int who, utime_t failed_since, MonOpRequestRef op)
  {
  ...
  void
  take_report_messages(std::list<MonOpRequestRef>& ls)
  {
    for (auto p = reporters.begin(); p != reporters.end(); ++p) {
      if (p->second.op) {
...
struct osdmap_manifest_t {
  ...
  version_t
  get_lower_closest_pinned(version_t v) const
  {
...
public:
  ...
  using osdmap_cache_t = SimpleLRU<
      osdmap_key_t,
      ceph::buffer::list,
  ...
  struct CleanUpmapJob : public ParallelPGMapper::Job {
    ...
    CleanUpmapJob(CephContext* cct, const OSDMap& om, OSDMap::Incremental& pi) :
      ParallelPGMapper::Job(&om), cct(cct), osdmap(om), pending_inc(pi)
    {}

    void
    process(const std::vector<pg_t>& to_check) override
    {
      ...
      osdmap.check_pg_upmaps(
          cct, to_check, &to_cancel, &to_cancel_upmap_primary_only,
      ...
      if (!to_cancel.empty() || !to_remap.empty() ||
        ...
        osdmap.clean_pg_upmaps(
            cct, &pending_inc, to_cancel, to_cancel_upmap_primary_only,
            affected_pools, to_remap);
      }
    }

    void
    process(int64_t poolid, unsigned ps_begin, unsigned ps_end) override
    {}
...
private:
   ...
   * full osdmaps.
   */
  void encode_trim_extra(
      MonitorDBStore::TransactionRef tx,
      version_t first) override;
  ...
  struct PrimeTempJob : public ParallelPGMapper::Job {
    ...
    void
    process(int64_t pool, unsigned ps_begin, unsigned ps_end) override
    {
    ...
    void
    process(const std::vector<pg_t>& pgs) override
    {}
  ...
  bool remove_pool_snap(std::string_view snapname, pg_pool_t& pp, int64_t pool);
...
private:
  ...
  int _check_remove_pool(
      int64_t pool_id,
      const pg_pool_t& pool,
      std::ostream* ss);
  bool _check_become_tier(
      int64_t tier_pool_id,
      const pg_pool_t* tier_pool,
      int64_t base_pool_id,
  ...
  int crush_rename_bucket(
      const std::string& srcname,
      const std::string& dstname,
      std::ostream* ss);
  void check_legacy_ec_plugin(
      const std::string& plugin,
      const std::string& profile) const;
  int normalize_profile(
      const std::string& profilename,
      ceph::ErasureCodeProfile& profile,
      bool force,
      std::ostream* ss);
  int crush_rule_create_erasure(
  ...
  int get_crush_rule(
      ...
      int* crush_rule,
      std::ostream* ss);
  int get_erasure_code(
      const std::string& erasure_code_profile,
      ceph::ErasureCodeInterfaceRef* erasure_code,
      std::ostream* ss) const;
  int prepare_pool_crush_rule(
  ...
  bool erasure_code_profile_in_use(
      ...
      const std::string& profile,
      std::ostream* ss);
  int parse_erasure_code_profile(
      const std::vector<std::string>& erasure_code_profile,
      std::map<std::string, std::string>* erasure_code_profile_map,
      std::ostream* ss);
  int prepare_pool_size(
      const unsigned pool_type,
      const std::string& erasure_code_profile,
      uint8_t repl_size,
      unsigned* size,
  ...
  std::string make_purged_snap_key_value(
      int64_t pool,
      snapid_t snap,
      snapid_t num,
      epoch_t epoch,
...
protected:
  ...
  bool is_pool_currently_all_bluestore(
      int64_t pool_id,
      const pg_pool_t& pool,
      std::ostream* err);
...
public:
  ...
  int enable_pool_ec_optimizations(
      pg_pool_t& pool,
      std::stringstream* ss,
      bool enable);
  ...
  int prepare_command_pool_application(
      const std::string& prefix,
      const cmdmap_t& cmdmap,
      std::stringstream& ss);
  int preprocess_command_pool_application(
      const std::string& prefix,
      const cmdmap_t& cmdmap,
      std::stringstream& ss,
      bool* modified);
  int prepare_command_pool_stretch_set(
      const cmdmap_t& cmdmap,
      std::stringstream& ss);
  int prepare_command_pool_stretch_unset(
      const cmdmap_t& cmdmap,
      std::stringstream& ss);
  int _command_pool_application(
      const std::string& prefix,
      const cmdmap_t& cmdmap,
      std::stringstream& ss,
      bool* modified,
  ...
  void
  send_latest_now_nodelete(MonOpRequestRef op, epoch_t start = 0)
  {
  ...
  void do_application_enable(
      int64_t pool_id,
      const std::string& app_name,
      const std::string& app_key = "",
      const std::string& app_value = "",
      bool force = false);
  void do_set_pool_opt(
      int64_t pool_id,
   ...
   */
  void try_enable_stretch_mode_pools(
      std::stringstream& ss,
      bool* okay,
      int* errcode,
  ...
  */
   ...
   */
  bool check_for_dead_crush_zones(
      const std::map<std::string, std::set<std::string>>& dead_buckets,
