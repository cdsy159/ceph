...
public:
  ...
  bool
  use_per_pool_stats() const
  {
    return osd_sum.num_osds == osd_sum.num_per_pool_osds;
  }

  bool
  use_per_pool_omap_stats() const
  {
   ...
   */
  mempool::pgmap::unordered_map<
      int64_t,
      mempool::pgmap::list<std::pair<pool_stat_t, utime_t>>>
      per_pool_sum_deltas;
  /**
   ...
   */
  mempool::pgmap::unordered_map<int64_t, std::pair<pool_stat_t, utime_t>>
      per_pool_sum_delta;
  ...
  void recovery_summary(
      ceph::Formatter* f,
      std::list<std::string>* psl,
      const pool_stat_t& pool_sum) const;
  void overall_recovery_summary(
      ceph::Formatter* f,
      std::list<std::string>* psl) const;
  void pool_recovery_summary(
      ceph::Formatter* f,
      std::list<std::string>* psl,
      uint64_t poolid) const;
  void recovery_rate_summary(
      ceph::Formatter* f,
      std::ostream* out,
      const pool_stat_t& delta_sum,
      utime_t delta_stamp) const;
  void overall_recovery_rate_summary(ceph::Formatter* f, std::ostream* out) const;
  void pool_recovery_rate_summary(
      ceph::Formatter* f,
      std::ostream* out,
      uint64_t poolid) const;
  /**
   ...
   */
  void overall_client_io_rate_summary(
      ceph::Formatter* f,
   ...
   */
  void pool_client_io_rate_summary(
      ceph::Formatter* f,
      std::ostream* out,
      uint64_t poolid) const;
  /**
   ...
   * from @p per_pool_sum_delta.
   */
  void pool_cache_io_rate_summary(
      ceph::Formatter* f,
      std::ostream* out,
      uint64_t poolid) const;
   ...
   */
  virtual void dump_pool_stats_full(
      const OSDMap& osd_map,
      std::stringstream* ss,
      ceph::Formatter* f,
      bool verbose) const;
  void dump_cluster_stats(
      std::stringstream* ss,
      ceph::Formatter* f,
      bool verbose) const;
  static void dump_object_stat_sum(
      TextTable& tbl,
      ceph::Formatter* f,
  ...
  size_t
  get_num_pg_by_osd(int osd) const
  {
  ...
  int
  get_num_primary_pg_by_osd(int osd) const
  {
    auto p = num_pg_by_osd.find(osd);
    if (p == num_pg_by_osd.end())
      return 0;
  ...
  int64_t
  get_rule_avail(int ruleno) const
  {
  ...
  bool
  definitely_converted_snapsets() const
  {
    // false negative is okay; false positive is not!
    return num_pg && num_pg_unknown == 0 &&
           pg_sum.stats.sum.num_legacy_snapsets == 0;
  }

  uint64_t
  get_last_osd_stat_seq(int osd)
  {
...
public:
  ...
  public:
    const mempool::pgmap::map<int32_t, osd_stat_t>&
    get_osd_stat_updates() const
    {
      return osd_stat_updates;
    }

    const mempool::pgmap::set<int32_t>&
    get_osd_stat_rm() const
    {
      return osd_stat_rm;
    }
    ...
    void
    update_stat(int32_t osd, OsdStat&& stat)
    {
      osd_stat_updates[osd] = std::forward<OsdStat>(stat);
    }

    void
    stat_osd_out(int32_t osd)
    {
      osd_stat_updates[osd] = osd_stat_t();
    }

    void
    stat_osd_down_up(int32_t osd, const PGMap& pg_map)
    {
    ...
    Incremental() :
      version(0), osdmap_epoch(0), pg_scan(0)
    {}
  };
  ...
  mempool::pgmap::
      unordered_map<int64_t, mempool::pgmap::unordered_map<uint64_t, int32_t>>
...
public:
  ...
  mempool::pgmap::map<int, std::map<epoch_t, std::set<pg_t>>>
  ...
  void
  set_last_osdmap_epoch(epoch_t e)
  {
    last_osdmap_epoch = e;
  }
  ...
  osd_stat_t
  get_osd_sum(const std::set<int>& osds) const
  {
    if (osds.empty()) // all
      return osd_sum;
    osd_stat_t sum;
  ...
  */
  void dump_pool_stats_and_io_rate(
      int64_t poolid,
      const OSDMap& osd_map,
      ceph::Formatter* f,
  ...
  void get_filtered_pg_stats(
      uint64_t state,
      int64_t poolid,
      int64_t osdid,
      bool primary,
      std::set<pg_t>& pgs) const;
...
inline std::ostream&
operator<<(std::ostream& out, const PGMapDigest& m)
{
