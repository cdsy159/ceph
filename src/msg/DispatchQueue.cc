...
double
DispatchQueue::get_max_age(utime_t now) const
{
...
uint64_t
DispatchQueue::pre_dispatch(const ref_t<Message>& m)
{
  ldout(cct, 1) << "<== " << m->get_source_inst() << " " << m->get_seq()
                << " ==== " << *m << " ==== " << m->get_payload().length()
                << "+" << m->get_middle().length() << "+"
                << m->get_data().length() << " ("
                << ceph_con_mode_name(m->get_connection()->get_con_mode())
                << " " << m->get_footer().front_crc << " "
                << m->get_footer().middle_crc << " " << m->get_footer().data_crc
                << ")" << " " << m << " con " << m->get_connection() << dendl;
  uint64_t msize = m->get_dispatch_throttle_size();
  m->set_dispatch_throttle_size(
      0); // clear it out, in case we requeue this message.
  return msize;
}

void
DispatchQueue::post_dispatch(const ref_t<Message>& m, uint64_t msize)
{
...
bool
DispatchQueue::can_fast_dispatch(const cref_t<Message>& m) const
{
...
void
DispatchQueue::fast_preprocess(const ref_t<Message>& m)
{
  msgr->ms_fast_preprocess(m);
}

void
DispatchQueue::enqueue(const ref_t<Message>& m, int priority, uint64_t id)
{
  std::lock_guard l{lock};
  if (stop) {
    return;
...
void
DispatchQueue::local_delivery(const ref_t<Message>& m, int priority)
{
...
void
DispatchQueue::dispatch_throttle_release(uint64_t msize)
{
  if (msize) {
  ...
  while (true) {
    while (!mqueue.empty()) {
      ...
      if (qitem.is_code()) {
        ...
        if (stop) {
          ldout(cct, 10) << " stop flag set, discarding " << m << " " << *m
                         << dendl;
        } else {
          uint64_t msize = pre_dispatch(m);
...
void
DispatchQueue::discard_queue(uint64_t id)
{
  std::lock_guard l{lock};
