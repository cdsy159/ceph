...
public:
  ...
  void
  set_connection(ConnectionRef c)
  {
#ifdef WITH_CRIMSON
    // In crimson, conn is independently maintained outside Message.
  ...
  void
  set_completion_hook(CompletionHook* hook)
  {
    completion_hook = hook;
  }

  void
  set_byte_throttler(ThrottleInterface* t)
  {
    byte_throttler = t;
  }

  void
  set_message_throttler(ThrottleInterface* t)
  {
    msg_throttler = t;
  }

  void
  set_dispatch_throttle_size(uint64_t s)
  {
    dispatch_throttle_size = s;
  }

  uint64_t
  get_dispatch_throttle_size() const
  {
    return dispatch_throttle_size;
  }

  const ceph_msg_header&
  get_header() const
  {
    return header;
  }
  ...
  void
  set_header(const ceph_msg_header& e)
  {
    header = e;
  }

  void
  set_footer(const ceph_msg_footer& e)
  {
    footer = e;
  }
  ...
  void
  set_src(const entity_name_t& src)
  {
    header.src = src;
  }
  ...
  void
  set_payload(ceph::buffer::list& bl)
  {
    if (byte_throttler)
  ...
  void
  set_middle(ceph::buffer::list& bl)
  {
    if (byte_throttler)
  ...
  void
  claim_data(ceph::buffer::list& bl)
  {
    if (byte_throttler)
      byte_throttler->put(data.length());
    bl = std::move(data);
  ...
  void
  set_recv_complete_stamp(utime_t t)
  {
  ...
  calc_header_crc()
  {
    header.crc = ceph_crc32c(
        0, (unsigned char*)&header, sizeof(header) - sizeof(header.crc));
  }
...
inline void
intrusive_ptr_add_ref(Message* m)
{
  m->get();
}

inline void
intrusive_ptr_release(Message* m)
{
  m->put();
}

extern Message* decode_message(
    CephContext* cct,
    int crcflags,
...
inline std::ostream&
operator<<(std::ostream& out, const Message& m)
{
  m.print(out);
...
{
  return {
      new T(std::forward<Args>(args)...), TOPNSPC::common::UniquePtrDeleter{}};
}
} // namespace crimson
...
struct formatter<M> {
  ...
  auto
  format(const M& m, FormatContext& ctx) const
  {
