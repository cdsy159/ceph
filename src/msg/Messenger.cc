...
Messenger*
Messenger::create_client_messenger(CephContext* cct, std::string lname)
{
  ...
  return Messenger::create(
      cct, public_msgr_type, entity_name_t::CLIENT(), std::move(lname), nonce);
}
...
void
Messenger::set_endpoint_addr(const entity_addr_t& a, const entity_name_t& name)
{
  size_t hostlen;
  ...
  if (hostlen) {
    ...
    getnameinfo(
        a.get_sockaddr(), hostlen, buf, sizeof(buf), NULL, 0, NI_NUMERICHOST);
 ...
 */
int
get_default_crc_flags(const ConfigProxy& conf)
{
  int r = 0;
  if (conf->ms_crc_data)
    r |= MSG_CRC_DATA;
