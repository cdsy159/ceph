...
private:
  struct PriorityDispatcher {
    ...
    bool
    operator<(const PriorityDispatcher& other) const
    {
  ...
  static void
  insert_head(std::vector<PriorityDispatcher>& v, PriorityDispatcher d)
  {
    v.insert(std::lower_bound(v.begin(), v.end(), d), d);
  }

  static void
  insert_tail(std::vector<PriorityDispatcher>& v, PriorityDispatcher d)
  {
...
public:
  ...
  void
  set_magic(int _magic)
  {
    magic = _magic;
  }

  void
  set_auth_client(AuthClient* ac)
  {
    auth_client = ac;
  }

  void
  set_auth_server(AuthServer* as)
  {
    auth_server = as;
  }
...
protected:
   ...
   */
  virtual void
  set_myaddrs(const entity_addrvec_t& a)
  {
    my_addrs = a;
...
public:
   ...
   */
  void
  set_myname(const entity_name_t& m)
  {
    my_name = m;
  }
   ...
   */
  virtual void set_policy_throttlers(
      int type,
      Throttle* bytes,
   ...
   */
  void
  set_default_send_priority(int p)
  {
    ceph_assert(!started);
  ...
  add_dispatcher_head(
      Dispatcher* d,
      PriorityDispatcher::priority_t priority = Dispatcher::PRIORITY_DEFAULT)
  {
   ...
   */
  virtual int
  rebind(const std::set<int>& avoid_ports)
  {
    return -EOPNOTSUPP;
  }
  ...
  int
  send_to_mon(Message* m, const entity_addrvec_t& addrs)
  {
    return send_to(m, CEPH_ENTITY_TYPE_MON, addrs);
  }

  int
  send_to_mds(Message* m, const entity_addrvec_t& addrs)
  {
  ...
  virtual void
  mark_down_addrs(const entity_addrvec_t& a)
  {
...
#define MSGR_SIGPIPE_STOPPER
   ...
   */
  bool
  ms_can_fast_dispatch(const ceph::cref_t<Message>& m)
  {
    for ([[maybe_unused]] const auto& [priority, dispatcher] :
         fast_dispatchers) {
   ...
   */
  void
  ms_fast_dispatch(const ceph::ref_t<Message>& m)
  {
   ...
   */
  void
  ms_fast_preprocess(const ceph::ref_t<Message>& m)
  {
    for ([[maybe_unused]] const auto& [priority, dispatcher] :
         fast_dispatchers) {
   ...
   */
  void
  ms_deliver_dispatch(const ceph::ref_t<Message>& m)
  {
    m->set_dispatch_stamp(ceph_clock_now());
    bool acked = false;
    ...
    lsubdout(cct, ms, 0) << "ms_deliver_dispatch: unhandled message " << m
                         << " " << *m << " from " << m->get_source_inst()
                         << dendl;
    ceph_assert(!cct->_conf->ms_die_on_unhandled_msg);
  }

  void
  ms_deliver_dispatch(Message* m)
  {
   ...
   */
  void
  ms_deliver_handle_connect(Connection* con)
  {
    for ([[maybe_unused]] const auto& [priority, dispatcher] : dispatchers) {
   ...
   */
  void
  ms_deliver_handle_fast_connect(Connection* con)
  {
    for ([[maybe_unused]] const auto& [priority, dispatcher] :
         fast_dispatchers) {
   ...
   */
  void
  ms_deliver_handle_accept(Connection* con)
  {
    for ([[maybe_unused]] const auto& [priority, dispatcher] : dispatchers) {
   ...
   */
  void
  ms_deliver_handle_fast_accept(Connection* con)
  {
    for ([[maybe_unused]] const auto& [priority, dispatcher] :
         fast_dispatchers) {
   ...
   */
  void
  ms_deliver_handle_reset(Connection* con)
  {
    for ([[maybe_unused]] const auto& [priority, dispatcher] : dispatchers) {
   ...
   */
  void
  ms_deliver_handle_remote_reset(Connection* con)
  {
    for ([[maybe_unused]] const auto& [priority, dispatcher] : dispatchers) {
   ...
   */
  void
  ms_deliver_handle_refused(Connection* con)
  {
    for ([[maybe_unused]] const auto& [priority, dispatcher] : dispatchers) {
