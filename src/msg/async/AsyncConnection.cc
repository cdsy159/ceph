...
std::ostream&
AsyncConnection::_conn_prefix(std::ostream* _dout)
{
  return *_dout << "-- " << async_msgr->get_myaddrs() << " >> " << *peer_addrs
                << " conn(" << this << (msgr2 ? " msgr2=" : " legacy=")
                << protocol.get() << " "
                << ceph_con_mode_name(protocol->auth_meta->con_mode) << " :"
                << port << " s=" << get_state_name(state)
                << " l=" << policy.lossy << ").";
}
...
public:
  ...
  void
  do_request(uint64_t fd) override
  {
    conn->handle_write();
  }
};
...
AsyncConnection::AsyncConnection(
  ...
  dispatch_queue(q),
  recv_buf(NULL),
  recv_max_prefetch(std::max<int64_t>(
...
int
AsyncConnection::get_con_mode() const
{
  return protocol->get_con_mode();
}

bool
AsyncConnection::is_msgr2() const
{
...
AsyncConnection::maybe_start_delay_thread()
{
  if (!delay_state) {
    async_msgr->cct->_conf.with_val<std::string>(
        "ms_inject_delay_type", [this](const std::string& s) {
          if (s.find(ceph_entity_type_name(peer_type)) != std::string::npos) {
            ldout(msgr->cct, 1)
                << __func__ << " setting up a delay queue" << dendl;
            delay_state = new DelayedDelivery(
                async_msgr, center, dispatch_queue, conn_id);
          }
        });
  }
}
...
ssize_t
AsyncConnection::read(
    unsigned len,
    char* buffer,
    std::function<void(char*, ssize_t)> callback)
{
  ldout(async_msgr->cct, 20)
      << __func__ << (pendingReadLen ? " continue" : " start") << " len=" << len
      << dendl;
...
ssize_t
AsyncConnection::read_until(unsigned len, char* p)
{
  ldout(async_msgr->cct, 25) << __func__ << " len is " << len
                             << " state_offset is " << state_offset << dendl;

  if (async_msgr->cct->_conf->ms_inject_socket_failures && cs) {
    if (rand() % async_msgr->cct->_conf->ms_inject_socket_failures == 0) {
      ldout(async_msgr->cct, 0)
          << __func__ << " injecting socket failure" << dendl;
      cs.shutdown();
    }
  }
  ...
  if (recv_end > recv_start) {
    ...
    recv_start += to_read;
    left -= to_read;
    ldout(async_msgr->cct, 25)
        << __func__ << " got " << to_read << " in buffer " << " left is "
        << left << " buffer still has " << recv_end - recv_start << dendl;
    if (left == 0) {
      state_offset = 0;
      return 0;
    }
  ...
  if (left > (uint64_t)recv_max_prefetch) {
    ...
    do {
      ...
      ldout(async_msgr->cct, 25)
          << __func__ << " read_bulk left is " << left << " got " << r << dendl;
      if (r < 0) {
    ...
    do {
      ...
      ldout(async_msgr->cct, 25)
          << __func__ << " read_bulk recv_end is " << recv_end << " left is "
          << left << " got " << r << dendl;
      if (r < 0) {
        ldout(async_msgr->cct, 1) << __func__ << " read failed" << dendl;
        return -1;
...
ssize_t
AsyncConnection::read_bulk(char* buf, unsigned len)
{
  ssize_t nread;
again:
  ...
  if (nread < 0) {
    if (nread == -EAGAIN) {
      ...
      ldout(async_msgr->cct, 1)
          << __func__ << " reading from fd=" << cs.fd() << " : " << nread << " "
          << strerror(nread) << dendl;
      return -1;
    }
  } else if (nread == 0) {
    ldout(async_msgr->cct, 1)
        << __func__ << " peer close file descriptor " << cs.fd() << dendl;
    return -1;
  }
  return nread;
}
...
AsyncConnection::_try_send(bool more)
{
  ...
  ldout(async_msgr->cct, 25)
      << __func__ << " cs.send " << outgoing_bl.length() << " bytes" << dendl;
  ...
  if (r < 0) {
    ldout(async_msgr->cct, 1)
        << __func__ << " send error: " << cpp_strerror(r) << dendl;
    return r;
  }

  ldout(async_msgr->cct, 10)
      << __func__ << " sent bytes " << r << " remaining bytes "
      << outgoing_bl.length() << dendl;
...
bool
AsyncConnection::inject_network_congestion() const
{
  return (
      async_msgr->cct->_conf->ms_inject_network_congestion > 0 &&
...
AsyncConnection::process()
{
  ...
  case STATE_CONNECTING_RE: {
    ...
    if (r < 0) {
      ldout(async_msgr->cct, 1)
          << __func__ << " reconnect failed to " << target_addr << dendl;
      if (r == -ECONNREFUSED) {
      ...
      logger->tinc(
          l_msgr_running_recv_time, ceph::mono_clock::now() - recv_start_time);
      return;
    }
  ...
  case STATE_ACCEPTING: {
    ...
    if (async_msgr->cct->_conf->mon_use_min_delay_socket) {
      if (async_msgr->get_mytype() == CEPH_ENTITY_TYPE_MON && peer_is_mon()) {
        cs.set_priority(
            cs.fd(), SOCKET_PRIORITY_MIN_DELAY, target_addr.get_family());
      }
    }
    break;
  }
...
void
AsyncConnection::accept(
    ConnectedSocket socket,
    const entity_addr_t& listen_addr,
    const entity_addr_t& peer_addr)
{
  ldout(async_msgr->cct, 10)
      << __func__ << " sd=" << socket.fd() << " listen_addr " << listen_addr
...
AsyncConnection::send_message(Message* m)
{
  ...
  lgeneric_subdout(async_msgr->cct, ms, 1)
      << "-- " << async_msgr->get_myaddrs() << " --> " << get_peer_addrs()
      << " -- " << *m << " -- " << m << " con " << this << dendl;

  if (is_blackhole()) {
    lgeneric_subdout(async_msgr->cct, ms, 0)
        << __func__ << ceph_entity_type_name(peer_type) << " blackhole " << *m
        << dendl;
    m->put();
    return 0;
  }
...
entity_addr_t
AsyncConnection::_infer_target_addr(const entity_addrvec_t& av)
{
...
bool
AsyncConnection::is_queued() const
{
  return outgoing_bl.length();
}
...
void
AsyncConnection::DelayedDelivery::do_request(uint64_t id)
{
  Message* m = nullptr;
  {
...
void
AsyncConnection::DelayedDelivery::discard()
{
  stop_dispatch = true;
  center->submit_to(
      center->get_id(),
...
void
AsyncConnection::send_keepalive()
{
  protocol->send_keepalive();
}

void
AsyncConnection::mark_down()
{
...
void
AsyncConnection::handle_write_callback()
{
  std::lock_guard<std::mutex> l(lock);
  last_active = ceph::coarse_mono_clock::now();
  recv_start_time = ceph::mono_clock::now();
...
void
AsyncConnection::stop(bool queue_reset)
{
  lock.lock();
...
void
AsyncConnection::wakeup_from(uint64_t id)
{
  lock.lock();
  register_time_events.erase(id);
  lock.unlock();
...
void
AsyncConnection::tick(uint64_t id)
{
  ...
  if (!is_connected()) {
    if (connect_timeout_us <=
        (uint64_t)std::chrono::duration_cast<std::chrono::microseconds>(
            now - last_connect_started)
            .count()) {
      ldout(async_msgr->cct, 1)
          << __func__ << " see no progress in more than " << connect_timeout_us
    ...
    auto idle_period =
        std::chrono::duration_cast<std::chrono::microseconds>(now - last_active)
            .count();
    if (inactive_timeout_us < (uint64_t)idle_period) {
      ldout(async_msgr->cct, 1)
          << __func__ << " idle (" << idle_period << ") for more than "
          << inactive_timeout_us << " us, fault." << dendl;
      protocol->fault();
      labeled_logger->inc(l_msgr_connection_idle_timeouts);
    } else {
      last_tick_id =
          center->create_time_event(inactive_timeout_us, tick_handler);
    }
  }
}

void
AsyncConnection::dump(Formatter* f, bool tcp_info)
{
  std::lock_guard<std::mutex> l(lock);
  ...
  f->dump_string(
      "last_connect_started_ago",
      ceph::timespan_str(ceph::coarse_mono_clock::now() - last_connect_started));
  f->dump_string(
      "last_active_ago",
      fmt::format(
          "{}",
          ceph::timespan_str(ceph::coarse_mono_clock::now() - last_active)));
  f->dump_string(
      "recv_start_time_ago",
      fmt::format(
          "{}", ceph::timespan_str(ceph::mono_clock::now() - recv_start_time)));
