...
KqueueDriver::restore_events()
{
  ...
  for (i = 0; i < size; i++) {
    ...
    if (sav_events[i].mask == 0)
      continue;
    ldout(cct, 30) << __func__ << " restore kqfd = " << kqfd << " fd = " << i
                   << " mask " << sav_events[i].mask << dendl;
    ...
    if (num) {
      if (kevent(kqfd, ke, num, NULL, 0, KEVENT_NOWAIT) == -1) {
        ldout(cct, 0) << __func__
                      << " unable to add event: " << cpp_strerror(errno)
                      << dendl;
        return -errno;
      }
    }
...
int
KqueueDriver::test_thread_change(const char* funcname)
{
  ...
  if (kqfd == -1) {
    ...
    ldout(cct, 30) << funcname << " kqueue: new kqfd = " << kqfd
                   << " (was: " << oldkqfd << ")" << dendl;
    if (kqfd < 0) {
      lderr(cct) << funcname << " unable to do kqueue: " << cpp_strerror(errno)
                 << dendl;
      return -errno;
    }
    if (restore_events() < 0) {
...
int
KqueueDriver::init(EventCenter* c, int nevent)
{
  ...
  if (!res_events) {
    lderr(cct) << __func__
               << " unable to malloc memory: " << cpp_strerror(errno) << dendl;
    return -ENOMEM;
  }
...
int
KqueueDriver::add_event(int fd, int cur_mask, int add_mask)
{
  struct kevent ke[2];
  int num = 0;
  ...
  if (num) {
    if (kevent(kqfd, ke, num, NULL, 0, KEVENT_NOWAIT) == -1) {
      lderr(cct) << __func__ << " unable to add event: " << cpp_strerror(errno)
                 << dendl;
      return -errno;
    }
  }
...
int
KqueueDriver::del_event(int fd, int cur_mask, int del_mask)
{
  struct kevent ke[2];
  int num = 0;
...
int
KqueueDriver::resize_events(int newsize)
{
  ldout(cct, 30) << __func__ << " kqfd = " << kqfd << "newsize = " << newsize
                 << dendl;
  if (newsize > sav_max) {
    sav_events = (struct SaveEvent*)realloc(
        sav_events, sizeof(struct SaveEvent) * newsize);
    if (!sav_events) {
      lderr(cct) << __func__
                 << " unable to realloc memory: " << cpp_strerror(errno)
                 << dendl;
    ...
    struct timeval* tvp)
{
  ...
  if (tvp != NULL) {
    ...
    ldout(cct, 20) << __func__ << " " << timeout.tv_sec << " sec "
                   << timeout.tv_nsec << " nsec" << dendl;
  ...
  if (retval < 0) {
    ...
    ldout(cct, 5) << __func__ << " Hit timeout(" << timeout.tv_sec << " sec "
                  << timeout.tv_nsec << " nsec" << ")." << dendl;
  } else {
    int j;
