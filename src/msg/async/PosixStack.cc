...
public:
  explicit PosixConnectedSocketImpl(
      ceph::NetHandler& h,
  ...
  ssize_t
  read(char* buf, size_t len) override
  {
#ifdef _WIN32
...
// < 0 means error occurred
#ifndef _WIN32
  static ssize_t
  do_sendmsg(int fd, struct msghdr& msg, unsigned len, bool more)
  {
    size_t sent = 0;
    while (1) {
  ...
  ssize_t
  send(ceph::buffer::list& bl, bool more) override
  {
    size_t sent_bytes = 0;
...
#else
  ssize_t
  send(bufferlist& bl, bool more) override
  {
  ...
  void
  set_priority(int sd, int prio, int domain) override
  {
...
public:
  explicit PosixServerSocketImpl(
      ceph::NetHandler& h,
      int f,
    ...
    Worker* w)
{
  ...
  std::unique_ptr<PosixConnectedSocketImpl> csi(
      new PosixConnectedSocketImpl(handler, *out, sd, true));
  *sock = ConnectedSocket(std::move(csi));
  return 0;
}

void
PosixWorker::initialize()
{}
    ...
    ConnectedSocket* socket)
{
  ...
  *socket = ConnectedSocket(std::unique_ptr<PosixConnectedSocketImpl>(
      new PosixConnectedSocketImpl(net, addr, sd, !opts.nonblock)));
  return 0;
}
