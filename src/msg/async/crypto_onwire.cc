...
struct nonce_t {
  ...
  bool
  operator==(const nonce_t& rhs) const
  {
...
public:
    ...
    new_nonce_format(new_nonce_format)
  {
    ...
    if (1 != EVP_EncryptInit_ex(
                 ectx.get(), EVP_aes_128_gcm(), nullptr, nullptr, nullptr)) {
      throw std::runtime_error("EVP_EncryptInit_ex failed");
    }

    if (1 !=
        EVP_EncryptInit_ex(ectx.get(), nullptr, nullptr, key.data(), nullptr)) {
      throw std::runtime_error("EVP_EncryptInit_ex failed");
    }
  }
  ...
  ~AES128GCM_OnWireTxHandler() override
  {
    ...
    ::TOPNSPC::crypto::zeroize_for_security(
        &initial_nonce, sizeof(initial_nonce));
  }
...
void
AES128GCM_OnWireTxHandler::reset_tx_handler(
    ...
    const uint32_t* last)
{
  ...
  if (1 != EVP_EncryptInit_ex(
               ectx.get(), nullptr, nullptr, nullptr,
...
void
AES128GCM_OnWireTxHandler::authenticated_encrypt_update(
    const ceph::bufferlist& plaintext)
{
  ceph_assert(
      buffer.get_append_buffer_unused_tail_length() >= plaintext.length());
  ...
  for (const auto& plainbuf : plaintext.buffers()) {
    ...
    if (1 != EVP_EncryptUpdate(
                 ectx.get(), reinterpret_cast<unsigned char*>(filler.c_str()),
                 &update_len,
  ...
  ldout(cct, 15) << __func__ << " plaintext.length()=" << plaintext.length()
                 << " buffer.length()=" << buffer.length() << dendl;
}
...
AES128GCM_OnWireTxHandler::authenticated_encrypt_final()
{
  ...
  if (1 !=
      EVP_CIPHER_CTX_ctrl(
          ectx.get(), EVP_CTRL_GCM_GET_TAG, AESGCM_TAG_LEN, filler.c_str())) {
    throw std::runtime_error("EVP_CIPHER_CTX_ctrl failed");
  }

  ldout(cct, 15) << __func__ << " buffer.length()=" << buffer.length()
                 << " final_len=" << final_len << dendl;
...
public:
    ...
    new_nonce_format(new_nonce_format)
  {
    ...
    if (1 !=
        EVP_DecryptInit_ex(ectx.get(), nullptr, nullptr, key.data(), nullptr)) {
      throw std::runtime_error("EVP_DecryptInit_ex failed");
    }
  }
  ...
  std::uint32_t
  get_extra_size_at_final() override
  {
...
void
AES128GCM_OnWireRxHandler::reset_rx_handler()
{
  if (1 != EVP_DecryptInit_ex(
               ectx.get(), nullptr, nullptr, nullptr,
               reinterpret_cast<const unsigned char*>(&nonce))) {
...
void
AES128GCM_OnWireRxHandler::authenticated_decrypt_update(ceph::bufferlist& bl)
{
    ...
    ceph::bufferlist& bl)
{
  ...
  if (1 !=
      EVP_CIPHER_CTX_ctrl(
          ectx.get(), EVP_CTRL_GCM_SET_TAG, AESGCM_TAG_LEN, auth_tag.c_str())) {
    throw std::runtime_error("EVP_CIPHER_CTX_ctrl failed");
  }
    ...
    bool crossed)
{
  if (auth_meta.is_mode_secure()) {
    ceph_assert_always(
        auth_meta.connection_secret.length() >=
