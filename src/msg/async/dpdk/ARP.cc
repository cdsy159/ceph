...
arp_for_protocol::~arp_for_protocol() { _arp.del(_proto_num); }
...
bool
arp::forward(forward_hash& out_hash_data, Packet& p, size_t off)
{
...
void
arp::add(uint16_t proto_num, arp_for_protocol* afp)
{
  _arp_for_protocol[proto_num] = afp;
}

void
arp::del(uint16_t proto_num)
{
