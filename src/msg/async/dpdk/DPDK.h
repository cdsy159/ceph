...
public:
  ...
  void
  register_packet_provider(packet_provider_type func)
  {
  ...
  private:
     ...
     */
    static void set_cluster_offload_info(
        const Packet& p,
     ...
     */
    static size_t
    set_one_data_buf(DPDKQueuePair& qp, rte_mbuf*& m, char* va, size_t buf_len)
    {
  ...
  public:
    ...
    void
    set_zc_info(void* va, phys_addr_t pa, size_t len)
    {
      // mbuf_put()
      _mbuf.data_len = len;
      _mbuf.pkt_len = len;
...
public:
  ...
  uint32_t
  send(circular_buffer<Packet>& pb)
  {
    // Zero-copy send
...
private:
  template <class Func>
  uint32_t
  _send(circular_buffer<Packet>& pb, Func&& packet_to_tx_buf_p)
  {
    if (_tx_burst.size() == 0) {
      for (auto&& p : pb) {
        // TODO: ceph_assert() in a fast path! Remove me ASAP!
    ...
    uint16_t sent = rte_eth_tx_burst(
        _dev_port_idx, _qid, _tx_burst.data() + _tx_burst_idx,
   ...
   */
  static bool
  refill_rx_mbuf(rte_mbuf* m, size_t size, std::vector<void*>& datas)
  {
    if (datas.empty())
      return false;
...
public:
    ...
    _enable_fc(enable_fc)
  {
    ...
    plb.add_u64_counter(
        l_dpdk_dev_rx_mcast, "dpdk_device_receive_multicast_packets",
        "DPDK received multicast packets");
    plb.add_u64_counter(
        l_dpdk_dev_rx_badcrc_errors, "dpdk_device_receive_badcrc_errors",
        "DPDK received bad crc errors");

    plb.add_u64_counter(
        l_dpdk_dev_rx_total_errors, "dpdk_device_receive_total_errors",
        "DPDK received total_errors");
    plb.add_u64_counter(
        l_dpdk_dev_tx_total_errors, "dpdk_device_send_total_errors",
        "DPDK sendd total_errors");
    plb.add_u64_counter(
        l_dpdk_dev_rx_dropped_errors, "dpdk_device_receive_dropped_errors",
        "DPDK received dropped errors");
    plb.add_u64_counter(
        l_dpdk_dev_rx_nombuf_errors, "dpdk_device_receive_nombuf_errors",
  ...
  DPDKQueuePair&
  queue_for_cpu(unsigned cpu)
  {
    return *_queues[cpu];
  }

  void
  l2receive(int qid, Packet p)
  {
    _queues[qid]->_rx_stream.produce(std::move(p));
  }

  subscription<Packet>
  receive(unsigned cpuid, std::function<int(Packet)> next_packet)
  {
  ...
  void
  set_local_queue(unsigned i, std::unique_ptr<DPDKQueuePair> qp)
  {
  ...
  unsigned
  forward_dst(unsigned src_cpuid, Func&& hashfn)
  {
  ...
  bool
  is_i40e_device() const
  {
    return _is_i40e_device;
  }

  bool
  is_vmxnet3_device() const
  {
