...
template <typename Protocol>
int
DPDKServerSocketImpl<Protocol>::accept(
    ConnectedSocket* s,
...
template <typename Protocol>
void
DPDKServerSocketImpl<Protocol>::abort_accept()
{
...
class DPDKWorker : public Worker {
  ...
  void
  set_ipv4_packet_filter(ip_packet_filter* filter)
  {
...
public:
  ...
  void
  arp_learn(ethernet_address l2, ipv4_address l3)
  {
...
class DPDKStack : public NetworkStack {
  ...
  virtual Worker*
  create_worker(CephContext* c, unsigned worker_id) override
  {
    return new DPDKWorker(c, worker_id);
  }

  virtual void
  rename_thread(unsigned id) override
  {}

public:
  explicit DPDKStack(CephContext* cct) :
    NetworkStack(cct), eal(cct)
  {
    funcs.reserve(cct->_conf->ms_async_op_threads);
  }

  virtual bool
  support_local_listen_table() const override
  {
    return true;
  }
