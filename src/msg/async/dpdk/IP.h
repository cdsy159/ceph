...
struct ipv4_traits {
  ...
  {
    csum.sum_many(
        src.ip, dst.ip, uint8_t(0), uint8_t(ip_protocol_num::tcp), len);
  }
...
public:
  ...
  virtual bool
  forward(forward_hash& out_hash_data, Packet& p, size_t off)
  {
    return true;
  }
};
...
struct l4connid {
  ...
  bool
  operator==(const l4connid& x) const
  {
    return local_ip == x.local_ip && foreign_ip == x.foreign_ip &&
           local_port == x.local_port && foreign_port == x.foreign_port;
  }

  uint32_t
  hash(const rss_key_type& rss_key)
  {
...
public:
  ipv4_icmp(CephContext* c, ipv4& inet) :
    cct(c), _inet_l4(inet), _icmp(c, _inet_l4)
  {}

  virtual void
  received(Packet p, ipv4_address from, ipv4_address to) override
  {
...
struct ip_packet_filter {
  ...
  virtual void
  handle(Packet& p, ip_hdr* iph, ethernet_address from, bool& handled) = 0;
};
...
struct ipv4_frag_id {
  ...
  bool
  operator==(const ipv4_frag_id& x) const
  {
    return src_ip == x.src_ip && dst_ip == x.dst_ip &&
           identification == x.identification && protocol == x.protocol;
  }
};
...
struct ipv4_frag_id::hash : private std::hash<ipv4_address>,
  ...
  size_t
  operator()(const ipv4_frag_id& id) const noexcept
  {
    using h1 = std::hash<ipv4_address>;
    using h2 = std::hash<uint16_t>;
    using h3 = std::hash<uint8_t>;
    return h1::operator()(id.src_ip) ^ h1::operator()(id.dst_ip) ^
           h2::operator()(id.identification) ^ h3::operator()(id.protocol);
  }
};
...
private:
  ...
  bool
  in_my_netmask(ipv4_address a) const
  {
  ...
  void
  frag_drop(ipv4_frag_id frag_id, uint32_t dropped_size)
  {
...
public:
  ...
  void
  set_host_address(ipv4_address ip)
  {
    _host_address = ip;
  ...
  void
  set_gw_address(ipv4_address ip)
  {
    _gw_address = ip;
  }
  ...
  void
  set_netmask_address(ipv4_address ip)
  {
    _netmask = ip;
  }

  ipv4_address
  netmask_address() const
  {
    return _netmask;
  }
  ...
  void
  set_packet_filter(ip_packet_filter* f)
  {
    _packet_filter = f;
  }
  ...
  static bool
  needs_frag(Packet& p, ip_protocol_num proto_num, hw_features hw_features)
  {
  ...
  void
  learn(ethernet_address l2, ipv4_address l3)
  {
    _arp.learn(l2, l3);
  }

  void
  register_packet_provider(ipv4_traits::packet_provider_type&& func)
  {
...
template <ip_protocol_num ProtoNum>
inline void
ipv4_l4<ProtoNum>::register_packet_provider(
...
template <ip_protocol_num ProtoNum>
inline void
ipv4_l4<ProtoNum>::wait_l2_dst_address(
    ipv4_address to,
    Packet p,
    resolution_cb cb)
{
...
template <typename InetTraits>
struct l4connid<InetTraits>::connid_hash : private std::hash<ipaddr>,
                                           private std::hash<uint16_t> {
  size_t
  operator()(const l4connid<InetTraits>& id) const noexcept
  {
