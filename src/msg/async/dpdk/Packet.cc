...
void
Packet::linearize(size_t at_frag, size_t desired_size)
{
  ...
  std::copy(
      _impl->frags + at_frag + nr_frags, _impl->frags + _impl->_nr_frags,
  ...
  if (at_frag == 0 && desired_size == len()) {
    ...
    auto del = std::bind(
        [new_frag](deleter& d) { delete[] new_frag; },
...
Packet
Packet::free_on_cpu(EventCenter* center, std::function<void()> cb)
{
  auto del = std::bind(
      [center, cb](deleter& del) mutable {
        center->dispatch_event_external(
            new C_free_on_cpu(std::move(del), std::move(cb)));
      },
      std::move(_impl->_deleter));
