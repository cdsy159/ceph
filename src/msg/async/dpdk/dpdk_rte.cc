...
static int
bitcount(unsigned long long n)
{
  return std::bitset<CHAR_BIT * sizeof(n)>{n}.count();
}

static int
hex2bitcount(unsigned char c)
{
  int val;
...
static int
coremask_bitcount(const char* buf)
{
  int count = 0;
...
eal::start()
{
  ...
  t = std::thread([&]() {
    ...
    for_each_pair(
        cct->_conf.get_val<std::string>("ms_dpdk_devs_allowlist"), " ",
