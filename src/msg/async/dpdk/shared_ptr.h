...
protected:
  ...
  operator=(const enable_lw_shared_from_this&) noexcept
  {
    return *this;
  }

  enable_lw_shared_from_this&
  operator=(enable_lw_shared_from_this&&) noexcept
  {
    return *this;
  }
...
struct lw_shared_ptr_accessors_esft {
  ...
  static T*
  to_value(lw_shared_ptr_counter_base* counter)
  {
    return static_cast<T*>(counter);
  }

  static void
  dispose(lw_shared_ptr_counter_base* counter)
  {
    delete static_cast<T*>(counter);
  }

  static void
  instantiate_to_value(lw_shared_ptr_counter_base* p)
  {
...
struct lw_shared_ptr_accessors<
    T,
    std::void_t<decltype(lw_shared_ptr_deleter<T>{})>> {
...
public:
  ...
  lw_shared_ptr&
  operator=(const lw_shared_ptr& x) noexcept
  {
    if (_p != x._p) {
      this->~lw_shared_ptr();
  ...
  lw_shared_ptr&
  operator=(lw_shared_ptr&& x) noexcept
  {
    if (_p != x._p) {
      this->~lw_shared_ptr();
  ...
  bool
  operator==(const lw_shared_ptr<const T>& x) const
  {
    return _p == x._p;
  }
  ...
  bool
  operator==(const lw_shared_ptr<std::remove_const_t<T>>& x) const
  {
    return _p == x._p;
  }
...
operator<<(std::ostream& out, const lw_shared_ptr<T>& p)
{
  if (!p) {
    return out << "null";
  }
...
struct hash<lw_shared_ptr<T>> : private hash<T*> {
  size_t
  operator()(const lw_shared_ptr<T>& p) const
  {
