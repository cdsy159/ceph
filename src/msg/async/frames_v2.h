...
const uint64_t msgr2_frame_assumed = msgr2_required |
...
public:
  ...
  FrameAssembler(
      const ceph::crypto::onwire::rxtx_t* crypto,
      bool is_rev1,
      bool with_data_crc,
  ...
  uint32_t
  get_segment_logical_len(size_t seg_idx) const
  {
  ...
  uint16_t
  get_segment_align(size_t seg_idx) const
  {
    ceph_assert(seg_idx < m_descs.size());
  ...
  uint32_t
  get_segment_onwire_len(size_t seg_idx) const
  {
    ceph_assert(seg_idx < m_descs.size());
    if (m_crypto->rx) {
  ...
  get_epilogue_onwire_len() const
  {
    ...
    if (m_crypto->rx) {
      return (m_is_rev1 ? sizeof(epilogue_secure_rev1_block_t)
                        : sizeof(epilogue_secure_rev0_block_t)) +
             get_auth_tag_len();
    }
...
private:
  ...
  bufferlist asm_crc_rev0(
      const preamble_block_t& preamble,
      bufferlist segment_bls[]) const;
  bufferlist asm_secure_rev0(
      const preamble_block_t& preamble,
      bufferlist segment_bls[]) const;
  bufferlist asm_crc_rev1(
      const preamble_block_t& preamble,
      bufferlist segment_bls[]) const;
  bufferlist asm_secure_rev1(
      const preamble_block_t& preamble,
      bufferlist segment_bls[]) const;
  ...
  // and must be dropped.
  void disassemble_first_segment(
      bufferlist& preamble_bl,
      bufferlist& segment_bl) const;
  bool disassemble_remaining_segments(
      bufferlist segment_bls[],
      bufferlist& epilogue_bl) const;
  void disassemble_decompress(bufferlist segment_bls[]) const;

  bool disasm_all_crc_rev0(
      bufferlist segment_bls[],
      bufferlist& epilogue_bl) const;
  bool disasm_all_secure_rev0(
      bufferlist segment_bls[],
      bufferlist& epilogue_bl) const;
  void disasm_first_crc_rev1(
      bufferlist& preamble_bl,
      bufferlist& segment_bl) const;
  bool disasm_remaining_crc_rev1(
      bufferlist segment_bls[],
      bufferlist& epilogue_bl) const;
  void disasm_first_secure_rev1(
      bufferlist& preamble_bl,
      bufferlist& segment_bl) const;
  bool disasm_remaining_secure_rev1(
      bufferlist segment_bls[],
      bufferlist& epilogue_bl) const;
...
public:
  ceph::bufferlist
  get_buffer(FrameAssembler& tx_frame_asm)
  {
    auto bl = tx_frame_asm.assemble_frame(
        T::tag, segments.data(), alignments.data(), SegmentsNumV);
...
class ControlFrame
  : public Frame<C, segment_t::DEFAULT_ALIGNMENT /* single segment */> {
protected:
  ceph::bufferlist&
  get_payload_segment()
  {
  ...
  template <typename T>
  inline void
  _decode_payload_each(T& t, bufferlist::const_iterator& ti) const
  {
  ...
  inline void
  _decode_payload(bufferlist::const_iterator& ti, std::index_sequence<Is...>) const
  {
  ...
  void
  _encode(const Args&... args)
  {
    (_encode_payload_each(args), ...);
  }

  void
  _decode(const ceph::bufferlist& bl)
  {
    auto ti = bl.cbegin();
...
public:
  ...
  static C
  Decode(const ceph::bufferlist& payload)
  {
    C c;
    c._decode(payload);
    return c;
...
struct HelloFrame : public ControlFrame<
                        HelloFrame,
                        uint8_t, // entity type
...
struct AuthRequestFrame : public ControlFrame<
                              AuthRequestFrame,
...
struct AuthBadMethodFrame : public ControlFrame<
                                AuthBadMethodFrame,
                                uint32_t, // method
                                int32_t, // result
...
struct AuthReplyMoreFrame : public ControlFrame<
                                AuthReplyMoreFrame,
                                bufferlist> { // auth payload
...
struct AuthRequestMoreFrame : public ControlFrame<
...
struct AuthDoneFrame : public ControlFrame<
                           AuthDoneFrame,
                           uint64_t, // global id
...
struct AuthSignatureFrame
  : public ControlFrame<AuthSignatureFrame, sha256_digest_t> {
...
struct ClientIdentFrame : public ControlFrame<
                              ClientIdentFrame,
...
struct ServerIdentFrame : public ControlFrame<
                              ServerIdentFrame,
                              entity_addrvec_t, // my addresses
                              int64_t, // global_id
                              uint64_t, // global seq
...
struct ReconnectFrame : public ControlFrame<
                            ReconnectFrame,
                            entity_addrvec_t, // my addresses
                            uint64_t, // client cookie
                            uint64_t, // server cookie
...
struct RetryFrame : public ControlFrame<
                        RetryFrame,
...
struct RetryGlobalFrame : public ControlFrame<
                              RetryGlobalFrame,
                              uint64_t> { // global seq
...
struct ReconnectOkFrame : public ControlFrame<
                              ReconnectOkFrame,
...
struct IdentMissingFeaturesFrame : public ControlFrame<
...
struct KeepAliveFrame : public ControlFrame<
                            KeepAliveFrame,
                            utime_t> { // timestamp
...
struct KeepAliveFrameAck : public ControlFrame<
                               KeepAliveFrameAck,
...
struct MessageFrame : public Frame<
                          MessageFrame,
                          /* four segments */
  ...
  static MessageFrame
  Decode(segment_bls_t& recv_segments)
  {
    MessageFrame f;
...
struct CompressionRequestFrame
  : public ControlFrame<
...
struct CompressionDoneFrame : public ControlFrame<
