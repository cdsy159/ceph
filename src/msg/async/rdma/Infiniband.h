...
public:
  ...
  get_lid()
  {
    return active_port->get_lid();
  }

  ibv_gid
  get_gid()
  {
    return active_port->get_gid();
  }

  int
  get_gid_idx()
  {
    return active_port->get_gid_idx();
  }
...
public:
  explicit DeviceList(CephContext* cct) :
    device_list(nullptr), device_context_list(nullptr), num(0), devices(nullptr)
  {
...
public:
  ...
  public:
    ...
    public:
      ...
      bool
      is_my_buffer(const char* c) const
      {
        return c >= base && c < end;
      }

      bool
      is_valid_chunk(const Chunk* c) const
      {
    ...
    public:
      ...
      explicit MemPoolContext(MemoryManager* m) :
        perf_logger(nullptr), manager(m), n_bufs_allocated(0)
      {}
    ...
    public:
      ...
      static std::invoke_result_t<Func>
      with_context(MemPoolContext* ctx, Func&& func)
      {
    ...
    bool
    is_tx_buffer(const char* c)
    {
      return send->is_my_buffer(c);
    }

    bool
    is_valid_chunk(const Chunk* c)
    {
      return send->is_valid_chunk(c);
    }
    ...
    void
    set_rx_stat_logger(PerfCounters* logger)
    {
...
public:
  ...
  public:
    ...
    get_local_cm_meta()
    {
      return local_cm_meta;
    }

    void
    add_rq_wr(Chunk* chunk)
    {
      if (srq)
        return;
...
public:
  ...
  void
  post_chunk_to_pool(Chunk* chunk)
  {
  ...
  CompletionQueue* create_comp_queue(
      CephContext* c,
  ...
  get_lid()
  {
    return device->get_lid();
  }

  ibv_gid
  get_gid()
  {
    return device->get_gid();
  }
  ...
  get_async_fd()
  {
    return device->ctxt->async_fd;
  }

  bool
  is_tx_buffer(const char* c)
  {
    return memory_manager->is_tx_buffer(c);
  }

  Chunk*
  get_tx_chunk_by_buffer(const char* c)
  {
    return memory_manager->get_tx_chunk_by_buffer(c);
  }
