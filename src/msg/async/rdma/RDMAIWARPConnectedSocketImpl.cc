  ...
  cm_con_handler(new C_handle_cm_connection(this))
{
  ...
  if (info) {
    ...
    worker->center.submit_to(
        ...
        [this]() {
          worker->center.create_file_event(
              cm_channel->fd, EVENT_READABLE, cm_con_handler);
    ...
    status = RDMA_ID_CREATED;
    ldout(cct, 20) << __func__ << " successfully created cm id: " << cm_id
                   << dendl;
  }
}
...
int
RDMAIWARPConnectedSocketImpl::try_connect(
    ...
    const SocketOptions& opts)
{
  ...
  if (rdma_resolve_addr(
          cm_id, NULL, const_cast<struct sockaddr*>(peer_addr.get_sockaddr()),
          TIMEOUT_MS)) {
...
void
RDMAIWARPConnectedSocketImpl::close()
{
  error = ECONNRESET;
  active = false;
  if (status >= CONNECTED) {
...
void
RDMAIWARPConnectedSocketImpl::shutdown()
{
  error = ECONNRESET;
  active = false;
}
...
void
RDMAIWARPConnectedSocketImpl::handle_cm_connection()
{
  ...
  case RDMA_CM_EVENT_ROUTE_RESOLVED:
    ...
    if (alloc_resource()) {
      lderr(cct) << __func__
                 << " failed to alloc resource while resolving the route"
                 << dendl;
...
void
RDMAIWARPConnectedSocketImpl::activate()
{
...
int
RDMAIWARPConnectedSocketImpl::alloc_resource()
{
  ldout(cct, 30) << __func__ << dendl;
  qp = ib->create_queue_pair(
      cct, dispatcher->get_tx_cq(), dispatcher->get_rx_cq(), IBV_QPT_RC, cm_id);
  if (!qp) {
    return -1;
  }
...
void
RDMAIWARPConnectedSocketImpl::close_notify()
{
  ldout(cct, 30) << __func__ << dendl;
