...
class RDMADispatcher {
   ...
   */
  std::unordered_map<uint32_t, std::pair<QueuePair*, RDMAConnectedSocketImpl*>>
      qp_conns;
...
public:
  ...
  void
  make_pending_worker(RDMAWorker* w)
  {
...
public:
  ...
  void
  remove_pending_conn(RDMAConnectedSocketImpl* o)
  {
  ...
  void
  set_dispatcher(std::shared_ptr<RDMADispatcher>& dispatcher)
  {
    this->dispatcher = dispatcher;
  }

  void
  set_ib(std::shared_ptr<Infiniband>& ib)
  {
    this->ib = ib;
  }
...
public:
  ...
  virtual int try_connect(
      const entity_addr_t&,
      const SocketOptions& opt) override;
  virtual void close() override;
  virtual void shutdown() override;
...
public:
  ...
  virtual bool
  nonblock_connect_need_writable_event() const override
  {
    return false;
  }
