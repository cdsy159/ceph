...
CompressorRegistry::~CompressorRegistry() { cct->_conf.remove_observer(this); }

std::vector<std::string>
CompressorRegistry::get_tracked_keys() const noexcept
{
  return {
      "ms_osd_compress_mode"s, "ms_osd_compression_algorithm"s,
      "ms_osd_compress_min_size"s, "ms_compress_secure"s};
}

void
CompressorRegistry::handle_conf_change(
...
void
CompressorRegistry::_refresh_config()
{
  auto c_mode = Compressor::get_comp_mode_type(
      cct->_conf.get_val<std::string>("ms_osd_compress_mode"));
  ...
  ms_osd_compression_methods = _parse_method_list(
      cct->_conf.get_val<std::string>("ms_osd_compression_algorithm"));
  ms_osd_compress_min_size =
      cct->_conf.get_val<std::uint64_t>("ms_osd_compress_min_size");

  ms_compress_secure = cct->_conf.get_val<bool>("ms_compress_secure");

  ldout(cct, 10) << __func__ << " ms_osd_compression_mode "
                 << ms_osd_compress_mode << " ms_osd_compression_methods "
                 << ms_osd_compression_methods
                 << " ms_osd_compress_above_min_size "
                 << ms_osd_compress_min_size << " ms_compress_secure "
                 << ms_compress_secure << dendl;
}

Compressor::CompressionAlgorithm
CompressorRegistry::pick_method(
    uint32_t peer_type,
    const std::vector<uint32_t>& preferred_methods)
{
  ...
  auto preferred = std::find_first_of(
      preferred_methods.begin(), preferred_methods.end(),
  ...
  if (preferred == preferred_methods.end()) {
    ldout(cct, 1) << "failed to pick compression method from client's "
                  << preferred_methods << " and our " << allowed_methods
                  << dendl;
...
{
  std::scoped_lock l(lock);
  ldout(cct, 20) << __func__ << " peer_type " << peer_type << " is_secure "
                 << is_secure << dendl;
