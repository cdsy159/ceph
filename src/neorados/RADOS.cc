...
Object::~Object() { reinterpret_cast<object_t*>(&impl)->~object_t(); }
...
{
  return (
      *reinterpret_cast<const object_t*>(&lhs.impl) <
...
std::ostream&
operator<<(std::ostream& m, const Object& o)
{
...
IOContext::IOContext(std::int64_t pool) :
  IOContext()
{
  set_pool(pool);
}
...
IOContext::IOContext(IOContext&& rhs)
{
  ...
  static_assert(alignof(decltype(impl)) >= alignof(IOContextImpl));
  new (&impl)
      IOContextImpl(std::move(*reinterpret_cast<IOContextImpl*>(&rhs.impl)));
}
  ...
  if (!_snapc) {
    ...
    if (!n.is_valid()) {
      throw bs::system_error(
          EINVAL, bs::system_category(), "Invalid snap context.");
...
IOContext&&
IOContext::set_write_snap_context(
    std::optional<std::pair<std::uint64_t, std::vector<std::uint64_t>>> snapc) &&
{
...
std::ostream&
operator<<(std::ostream& m, const IOContext& o)
{
...
Op::~Op() { reinterpret_cast<OpImpl*>(&impl)->~OpImpl(); }
...
void
Op::cmpext(uint64_t off, bufferlist cmp_bl, uint64_t* unmatch)
{
  reinterpret_cast<OpImpl*>(&impl)->op.cmpext(
      off, std::move(cmp_bl), nullptr, unmatch);
}

void
Op::cmpxattr(std::string_view name, cmp_op op, const bufferlist& val)
{
  reinterpret_cast<OpImpl*>(&impl)->op.cmpxattr(
      name, std::uint8_t(op), CEPH_OSD_CMPXATTR_MODE_STRING, val);
}

void
Op::cmpxattr(std::string_view name, cmp_op op, std::uint64_t val)
{
  bufferlist bl;
  encode(val, bl);
  reinterpret_cast<OpImpl*>(&impl)->op.cmpxattr(
      name, std::uint8_t(op), CEPH_OSD_CMPXATTR_MODE_U64, bl);
}
...
Op::assert_exists()
{
  reinterpret_cast<OpImpl*>(&impl)->op.stat(
      nullptr, static_cast<ceph::real_time*>(nullptr),
      static_cast<bs::error_code*>(nullptr));
}

void
Op::cmp_omap(const std::vector<cmp_assertion>& assertions)
{
  buffer::list bl;
...
std::ostream&
operator<<(std::ostream& m, const Op& o)
{
...
ReadOp&
ReadOp::read(size_t off, uint64_t len, cb::list* out, bs::error_code* ec) &
{
...
ReadOp&
ReadOp::get_xattr(std::string_view name, cb::list* out, bs::error_code* ec) &
{
...
ReadOp&
ReadOp::get_omap_header(cb::list* out, bs::error_code* ec) &
{
...
ReadOp&
ReadOp::stat(std::uint64_t* size, ceph::real_time* mtime, bs::error_code* ec) &
{
...
ReadOp::get_omap_keys(
    ...
    bs::error_code* ec) &
{
  reinterpret_cast<OpImpl*>(&impl)->op.omap_get_keys(
      start_after, max_return, ec, keys, done);
  return *this;
}

ReadOp&
ReadOp::get_xattrs(bc::flat_map<std::string, cb::list>* kv, bs::error_code* ec) &
{
...
ReadOp::get_omap_vals(
    ...
    bs::error_code* ec) &
{
  reinterpret_cast<OpImpl*>(&impl)->op.omap_get_vals(
      start_after, filter_prefix, max_return, ec, kv, done);
  return *this;
}
...
ReadOp&
ReadOp::list_watchers(std::vector<ObjWatcher>* watchers, bs::error_code* ec) &
{
...
ReadOp&
ReadOp::list_snaps(SnapSet* snaps, bs::error_code* ec) &
{
...
inline uint8_t
checksum_op_type(hash_alg::xxhash32_t)
{
  return CEPH_OSD_CHECKSUM_OP_TYPE_XXHASH32;
}

inline uint8_t
checksum_op_type(hash_alg::xxhash64_t)
{
  return CEPH_OSD_CHECKSUM_OP_TYPE_XXHASH64;
}

inline uint8_t
checksum_op_type(hash_alg::crc32c_t)
{
...
ReadOp::checksum(
    T t,
    const typename T::init_value& iv,
    std::uint64_t off,
    std::uint64_t len,
    ...
    boost::system::error_code* ec) &
{
  ...
  reinterpret_cast<OpImpl*>(&impl)->op.checksum(
      checksum_op_type(t), std::move(init_bl), off, len, chunk_size,
...
template ReadOp& ReadOp::checksum<hash_alg::xxhash32_t>(
    hash_alg::xxhash32_t,
    const typename hash_alg::xxhash32_t::init_value&,
    std::uint64_t off,
    std::uint64_t len,
    std::uint64_t chunk_size,
    std::vector<typename hash_alg::xxhash32_t::hash_value>* out,
    boost::system::error_code* ec) &;
template ReadOp& ReadOp::checksum<hash_alg::xxhash64_t>(
    hash_alg::xxhash64_t,
    const typename hash_alg::xxhash64_t::init_value&,
    std::uint64_t off,
    std::uint64_t len,
    std::uint64_t chunk_size,
    std::vector<typename hash_alg::xxhash64_t::hash_value>* out,
    boost::system::error_code* ec) &;
template ReadOp& ReadOp::checksum<hash_alg::crc32c_t>(
    hash_alg::crc32c_t,
    const typename hash_alg::crc32c_t::init_value&,
    std::uint64_t off,
    std::uint64_t len,
    std::uint64_t chunk_size,
...
WriteOp&
WriteOp::write(uint64_t off, bufferlist bl) &
{
...
WriteOp&
WriteOp::write_full(bufferlist bl) &
{
...
WriteOp&
WriteOp::writesame(uint64_t off, uint64_t write_len, bufferlist bl) &
{
...
WriteOp&
WriteOp::setxattr(std::string_view name, bufferlist bl) &
{
...
WriteOp&
WriteOp::set_omap(const bc::flat_map<std::string, cb::list>& map) &
{
...
WriteOp&
WriteOp::set_omap_header(bufferlist bl) &
{
...
WriteOp&
WriteOp::rm_omap_keys(const bc::flat_set<std::string>& to_rm) &
{
    ...
    alloc_hint::alloc_hint_t flags) &
{
  ...
  static_assert(
      random_write == static_cast<int>(CEPH_OSD_ALLOC_HINT_FLAG_RANDOM_WRITE));
  static_assert(
      sequential_read ==
      static_cast<int>(CEPH_OSD_ALLOC_HINT_FLAG_SEQUENTIAL_READ));
  static_assert(
      random_read == static_cast<int>(CEPH_OSD_ALLOC_HINT_FLAG_RANDOM_READ));
  static_assert(
      append_only == static_cast<int>(CEPH_OSD_ALLOC_HINT_FLAG_APPEND_ONLY));
  static_assert(
      immutable == static_cast<int>(CEPH_OSD_ALLOC_HINT_FLAG_IMMUTABLE));
  static_assert(
      shortlived == static_cast<int>(CEPH_OSD_ALLOC_HINT_FLAG_SHORTLIVED));
  static_assert(
      longlived == static_cast<int>(CEPH_OSD_ALLOC_HINT_FLAG_LONGLIVED));
  static_assert(
      compressible == static_cast<int>(CEPH_OSD_ALLOC_HINT_FLAG_COMPRESSIBLE));
  static_assert(
      incompressible ==
...
void
RADOS::Builder::build_(asio::io_context& ioctx, BuildComp c)
{
  ...
  // pass back.
  {
    ...
    auto r = cct->_conf.parse_config_files(
        conf_files ? conf_files->data() : nullptr, &ss, flags);
    if (r < 0) {
      asio::post(
          ioctx.get_executor(),
          asio::append(std::move(c), ceph::to_error_code(r), RADOS{nullptr}));
      return;
    }
  }
  ...
  for (const auto& [n, v] : configs) {
    ...
    if (r < 0) {
      asio::post(
          ioctx.get_executor(),
          asio::append(
              std::move(c), ceph::to_error_code(-EINVAL), RADOS{nullptr}));
      return;
    }
  }
  ...
  if (!no_mon_conf) {
    ...
    if (err < 0) {
      asio::post(
          ioctx.get_executor(),
          asio::append(std::move(c), ceph::to_error_code(err), RADOS{nullptr}));
      return;
    }
  }
...
RADOS
RADOS::make_with_librados(librados::Rados& rados)
{
    ...
    std::uint64_t subsystem)
{
  ...
  impl->objecter->mutate(
      *oid, ioc->oloc, std::move(op->op), ioc->snapc, mtime, flags,
...
void
RADOS::lookup_pool_(std::string name, LookupPoolComp c)
{
  ...
  int64_t ret = impl->objecter->with_osdmap(
      std::mem_fn(&OSDMap::lookup_pg_pool_name), name);
  if (ret < 0) {
    impl->objecter->wait_for_latest_osdmap(
        [name = std::move(name), c = std::move(c),
         objecter = impl->objecter](bs::error_code ec) mutable {
          int64_t ret = objecter->with_osdmap([&](const OSDMap& osdmap) {
            return osdmap.lookup_pg_pool_name(name);
          });
          if (ret < 0)
            asio::dispatch(asio::append(
                std::move(c), make_error_code(osdc_errc::pool_dne),
                std::int64_t(0)));
          else
    ...
    asio::post(
        get_executor(), asio::append(std::move(c), bs::error_code{}, ret));
  }
}

std::optional<uint64_t>
RADOS::get_pool_alignment(int64_t pool_id)
{
  return impl->objecter->with_osdmap(
      [pool_id](const OSDMap& o) -> std::optional<uint64_t> {
        if (!o.have_pg_pool(pool_id)) {
          throw bs::system_error(
              ENOENT, bs::system_category(), "Cannot find pool in OSDMap.");
...
void
RADOS::list_pools_(LSPoolsComp c)
{
  asio::dispatch(
      asio::append(std::move(c), impl->objecter->with_osdmap([&](OSDMap& o) {
...
void
RADOS::create_pool_snap_(std::int64_t pool, std::string snap_name, SimpleOpComp c)
{
  auto e =
      asio::prefer(get_executor(), asio::execution::outstanding_work.tracked);
...
void
RADOS::allocate_selfmanaged_snap_(int64_t pool, SMSnapComp c)
{
  auto e =
      asio::prefer(get_executor(), asio::execution::outstanding_work.tracked);

  impl->objecter->allocate_selfmanaged_snap(
      pool, asio::bind_executor(
                std::move(e),
...
void
RADOS::delete_pool_snap_(std::int64_t pool, std::string snap_name, SimpleOpComp c)
{
  auto e =
      asio::prefer(get_executor(), asio::execution::outstanding_work.tracked);
...
void
RADOS::delete_selfmanaged_snap_(
    std::int64_t pool,
    std::uint64_t snap,
    SimpleOpComp c)
{
...
bool
RADOS::get_self_managed_snaps_mode(std::string_view pool) const
{
  return impl->objecter->with_osdmap([pool](const OSDMap& osdmap) {
...
RADOS::list_snaps(std::int64_t pool) const
{
  return impl->objecter->with_osdmap([pool](const OSDMap& osdmap) {
    const auto pgpool = osdmap.get_pg_pool(pool);
    if (!pgpool) {
...
std::vector<std::uint64_t>
RADOS::list_snaps(std::string_view pool) const
{
  return impl->objecter->with_osdmap([pool](const OSDMap& osdmap) {
    int64_t poolid = osdmap.lookup_pg_pool_name(pool);
    if (poolid < 0) {
...
std::uint64_t
RADOS::lookup_snap(std::int64_t pool, std::string_view snap) const
{
...
std::uint64_t
RADOS::lookup_snap(std::string_view pool, std::string_view snap) const
{
  return impl->objecter->with_osdmap([pool, snap](const OSDMap& osdmap) {
    int64_t poolid = osdmap.lookup_pg_pool_name(pool);
    if (poolid < 0) {
...
std::string
RADOS::get_snap_name(std::int64_t pool, std::uint64_t snap) const
{
  return impl->objecter->with_osdmap([pool, snap](const OSDMap& osdmap) {
    const auto pgpool = osdmap.get_pg_pool(pool);
    if (!pgpool) {
...
std::string
RADOS::get_snap_name(std::string_view pool, std::uint64_t snap) const
{
  return impl->objecter->with_osdmap([pool, snap](const OSDMap& osdmap) {
    int64_t poolid = osdmap.lookup_pg_pool_name(pool);
    if (poolid < 0) {
...
ceph::real_time
RADOS::get_snap_timestamp(std::int64_t pool, std::uint64_t snap) const
{
  return impl->objecter->with_osdmap([pool, snap](const OSDMap& osdmap) {
    const auto pgpool = osdmap.get_pg_pool(pool);
    if (!pgpool) {
...
ceph::real_time
RADOS::get_snap_timestamp(std::string_view pool, std::uint64_t snap) const
{
  return impl->objecter->with_osdmap([pool, snap](const OSDMap& osdmap) {
    int64_t poolid = osdmap.lookup_pg_pool_name(pool);
    if (poolid < 0) {
...
void
RADOS::delete_pool_(std::string name, SimpleOpComp c)
{
  auto e =
      asio::prefer(get_executor(), asio::execution::outstanding_work.tracked);
  impl->objecter->delete_pool(
      name, asio::bind_executor(
                std::move(e),
                [c = std::move(c)](bs::error_code e, const bufferlist&) mutable {
                  asio::dispatch(asio::append(std::move(c), e));
                }));
...
void
RADOS::delete_pool_(std::int64_t pool, SimpleOpComp c)
{
  auto e =
      asio::prefer(get_executor(), asio::execution::outstanding_work.tracked);
  impl->objecter->delete_pool(
...
void
RADOS::stat_pools_(std::vector<std::string> pools, PoolStatComp c)
{
  impl->objecter->get_pool_stats(
      pools,
      [c = std::move(c)](
          bs::error_code ec, bc::flat_map<std::string, pool_stat_t> rawresult,
        ...
        for (auto p = rawresult.begin(); p != rawresult.end(); ++p) {
          ...
          pv.num_objects_missing_on_primary =
              sum->num_objects_missing_on_primary;
        ...
        asio::dispatch(
            asio::append(std::move(c), ec, std::move(result), per_pool));
      });
}

void
RADOS::stat_fs_(std::optional<std::int64_t> _pool, StatFSComp c)
{
...
public:
  ...
  ~Notifier()
  {
    while (!handlers.empty()) {
      ...
      asio::post(
          ex, asio::append(
                  std::move(h.comp), asio::error::operation_aborted,
                  Notification{}));
      handlers.pop_front();
    }
    svc.remove(*this);
  ...
  void
  add_handler(uint64_t id, RADOS::NextNotificationComp&& h)
  {
    std::unique_lock l(m);
  ...
  cancel(uint64_t id)
  {
    ...
    for (auto i = handlers.begin(); i != handlers.end(); ++i) {
      if (i->id == id) {
        dispatch(asio::append(
            std::move(i->comp), asio::error::operation_aborted, Notification{}));
        handlers.erase(i);
        break;
      }
    }
    ...
    WatchComp c)
{
  ...
  auto linger_op =
      impl->objecter->linger_register(*oid, ioc->oloc, ioc->extra_op_flags);
  ...
  impl->objecter->linger_watch(
      ...
      asio::bind_executor(
          std::move(e),
          [c = std::move(c), cookie,
           linger_op](bs::error_code e, cb::list) mutable {
            if (e) {
    ...
    std::uint32_t queue_size = 128u)
{
  ...
  auto& n = ceph::any_cast<std::shared_ptr<Notifier>&>(linger_op->user_data);
...
void
RADOS::next_notification_(uint64_t cookie, NextNotificationComp c)
{
  boost::intrusive_ptr linger_op = impl->objecter->linger_by_cookie(cookie);
  if (!linger_op) {
    dispatch(asio::append(
        std::move(c), bs::error_code(ENOTCONN, bs::generic_category()),
        Notification{}));
  } else
    try {
      auto n = ceph::any_cast<std::shared_ptr<Notifier>&>(linger_op->user_data);
      ...
      dispatch(asio::append(
          std::move(c), make_error_code(errc::polled_callback_watch),
          Notification{}));
    }
}
    ...
    SimpleOpComp c)
{
  ...
  impl->objecter->read(
      *oid, ioc->oloc, std::move(op), ioc->snap_seq, nullptr,
      ioc->extra_op_flags, std::move(c));
}

tl::expected<ceph::timespan, bs::error_code>
RADOS::check_watch(uint64_t cookie)
{
  boost::intrusive_ptr linger_op = impl->objecter->linger_by_cookie(cookie);
  if (linger_op) {
...
void
RADOS::unwatch_(uint64_t cookie, IOContext _ioc, SimpleOpComp c)
{
  auto ioc = reinterpret_cast<const IOContextImpl*>(&_ioc.impl);
  ...
  auto e =
      asio::prefer(get_executor(), asio::execution::outstanding_work.tracked);
  impl->objecter->mutate(
      linger_op->target.base_oid, ioc->oloc, std::move(op), ioc->snapc,
      ceph::real_clock::now(), ioc->extra_op_flags,
      asio::bind_executor(
          std::move(e), [objecter = impl->objecter, linger_op,
...
void
RADOS::flush_watch_(VoidOpComp c)
{
...
struct NotifyHandler : std::enable_shared_from_this<NotifyHandler> {
  ...
  void
  handle_ack(bs::error_code ec, bufferlist&&)
  {
    asio::post(strand, [this, ec, p = shared_from_this()]() mutable {
      acked = true;
      maybe_cleanup(ec);
    });
  }
  ...
  void
  operator()(bs::error_code ec, bufferlist&& bl)
  {
    asio::post(
        strand,
  ...
  void
  maybe_cleanup(bs::error_code ec)
  {
    if (!res && ec)
      res = ec;
    ...
    NotifyComp c)
{
  ...
  auto cb = std::make_shared<NotifyHandler>(
      impl->ioctx, impl->objecter, linger_op, std::move(c));
  auto e =
      asio::prefer(get_executor(), asio::execution::outstanding_work.tracked);
  linger_op->on_notify_finish = asio::bind_executor(
      e, [cb](bs::error_code ec, ceph::bufferlist bl) mutable {
        (*cb)(ec, std::move(bl));
      });
  ObjectOperation rd;
  bufferlist inbl;
  ...
  rd.notify(
      linger_op->get_cookie(), 1, timeout.value_or(30s).count(), bl, &inbl);
...
Cursor::~Cursor() { reinterpret_cast<hobject_t*>(&impl)->~hobject_t(); }

bool
operator==(const Cursor& lhs, const Cursor& rhs)
{
  return (
      *reinterpret_cast<const hobject_t*>(&lhs.impl) ==
    ...
    EnumerateComp c)
{
  ...
  impl->objecter->enumerate_objects<Entry>(
      ioc->oloc.pool, ioc->oloc.nspace,
      *reinterpret_cast<const hobject_t*>(&begin.impl),
      *reinterpret_cast<const hobject_t*>(&end.impl), max, filter,
      [c = std::move(c)](
          bs::error_code ec, std::vector<Entry>&& v, hobject_t&& n) mutable {
        asio::dispatch(asio::append(
            std::move(c), ec, std::move(v), Cursor(static_cast<void*>(&n))));
      });
}
    ...
    CommandComp c)
{
  impl->objecter->osd_command(
      ...
      [c = std::move(c)](
        ...
        asio::dispatch(
            asio::append(std::move(c), ec, std::move(s), std::move(b)));
      });
}
  ...
  if (!impl->get_required_monitor_features().contains_all(
    ...
    impl->monclient.start_mon_command(
        {fmt::format(
            ...
            "\"pool\": \"{}\", \"app\": \"{}\"{}}}",
            pool, app_name, force ? " ,\"yes_i_really_mean_it\": true" : "")},
        {},
        [c = std::move(c)](bs::error_code e, std::string, cb::list) mutable {
          asio::dispatch(asio::append(std::move(c), e));
        });
  }
}
...
void
RADOS::blocklist_add_(
    std::string client_address,
    std::optional<std::chrono::seconds> expire,
    SimpleOpComp c)
{
  auto expire_arg =
      (expire ? fmt::format(", \"expire\": \"{}.0\"", expire->count())
              : std::string{});
  impl->monclient.start_mon_command(
      {fmt::format(
          "{{"
      ...
      [this, client_address = std::string(client_address), expire_arg,
       c = std::move(c)](bs::error_code ec, std::string, cb::list) mutable {
        if (ec != bs::errc::invalid_argument) {
          asio::post(get_executor(), asio::append(std::move(c), ec));
          return;
        }
...
void
RADOS::wait_for_latest_osd_map_(SimpleOpComp c)
{
    ...
    SimpleOpComp c)
{
  ...
  impl->monclient.start_mon_command(
      ...
      [c = std::move(c), outs,
       outbl](bs::error_code e, std::string s, cb::list bl) mutable {
        if (outs)
          *outs = std::move(s);
        if (outbl)
          *outbl = std::move(bl);
...
uint64_t
RADOS::instance_id() const
{
  return impl->get_instance_id();
}

uint64_t
RADOS::new_subsystem() const
{
  return impl->objecter->unique_subsystem_id();
}

void
RADOS::cancel_subsystem(uint64_t subsystem) const
{
  impl->objecter->subsystem_cancel(subsystem, asio::error::operation_aborted);
}
...
const char*
category::message(int ev, char*, std::size_t) const noexcept
{
  if (ev == 0)
    return "No error";
...
std::string
category::message(int ev) const
{
  return message(ev, nullptr, 0);
}

bs::error_condition
category::default_error_condition(int ev) const noexcept
{
  switch (static_cast<errc>(ev)) {
  case errc::pool_dne:
...
bool
category::equivalent(int ev, const bs::error_condition& c) const noexcept
{
...
int
category::from_code(int ev) const noexcept
{
  switch (static_cast<errc>(ev)) {
  case errc::pool_dne:
    return -ENOENT;
...
namespace std {
size_t
hash<neorados::Object>::operator()(const neorados::Object& r) const
{
...
size_t
hash<neorados::IOContext>::operator()(const neorados::IOContext& r) const
{
