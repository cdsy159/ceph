...
void
NVMeofGwMonitorClient::init_gw_ssl_opts()
{
...
NVMeofGwMonitorClient::gw_creds()
{
...
NVMeofGwMonitorClient::init()
{
  ...
  for (std::vector<const char*>::iterator i = args.begin(); i != args.end();) {
    if (ceph_argparse_double_dash(args, i)) {
      break;
        ...
        ceph_argparse_witharg(args, i, &val, "--gateway-name", (char*)NULL)) {
      name = val;
    } else if (
        ceph_argparse_witharg(args, i, &val, "--gateway-pool", (char*)NULL)) {
      pool = val;
    } else if (
        ceph_argparse_witharg(args, i, &val, "--gateway-group", (char*)NULL)) {
      group = val;
    } else if (ceph_argparse_witharg(
                   args, i, &val, "--gateway-address", (char*)NULL)) {
      gateway_address = val;
    } else if (ceph_argparse_witharg(
                   args, i, &val, "--monitor-group-address", (char*)NULL)) {
      monitor_address = val;
    } else if (
        ceph_argparse_witharg(args, i, &val, "--server-cert", (char*)NULL)) {
      server_cert = val;
    } else if (
        ceph_argparse_witharg(args, i, &val, "--client-key", (char*)NULL)) {
      client_key = val;
    } else if (
        ceph_argparse_witharg(args, i, &val, "--client-cert", (char*)NULL)) {
      client_cert = val;
    } else {
      ++i;
    }
  ...
  dout(10) << "gateway name: " << name << " pool:" << pool << " group:" << group
           << " address: " << gateway_address << dendl;
  ceph_assert(
      name != "" && pool != "" && gateway_address != "" &&
      monitor_address != "");

  // ensures that either all are empty or all are non-empty.
  ceph_assert(
      (server_cert.empty() == client_key.empty()) &&
      (client_key.empty() == client_cert.empty()));
  init_gw_ssl_opts();
  ...
  r = monc.authenticate();
  if (r < 0) {
    derr << "Authentication failed, did you specify an ID with a valid keyring?"
         << dendl;
    monc.shutdown();
    client_messenger->shutdown();
    client_messenger->wait();
...
get_gw_state(
    const char* desc,
    const std::map<NvmeGroupKey, NvmeGwMonClientStates>& m,
    const NvmeGroupKey& group_key,
    const NvmeGwId& gw_id,
    NvmeGwClientState& out)
{
  auto gw_group = m.find(group_key);
  if (gw_group == m.end()) {
    dout(10) << "can not find group (" << group_key.first << ","
             << group_key.second << ") " << desc << " map: " << m << dendl;
    return false;
  }
  auto gw_state = gw_group->second.find(gw_id);
  if (gw_state == gw_group->second.end()) {
    dout(10) << "can not find gw id: " << gw_id << " in " << desc
             << "group: " << gw_group->second << dendl;
    return false;
  }
  out = gw_state->second;
  return true;
...
void
NVMeofGwMonitorClient::send_beacon()
{
  ...
  NVMeofGwClient gw_client(grpc::CreateChannel(gateway_address, gw_creds()));
  ...
  if (first_beacon == false &&
      get_gw_state("old map", map, group_key, name, old_gw_state))
    gw_availability = ok ? gw_availability_t::GW_AVAILABLE
                         : gw_availability_t::GW_UNAVAILABLE;
  dout(1) << "sending beacon as gid " << monc.get_global_id()
          << " availability " << (int)gw_availability << " osdmap_epoch "
          << osdmap_epoch << " gwmap_epoch " << gwmap_epoch << dendl;

  // Check if NVMEOF_BEACON_DIFF feature is supported by the cluster
  dout(10) << fmt::format(
                  "NVMEOF_BEACON_DIFF supported: {}",
                  cluster_beacon_diff_included ? "yes" : "no")
           << dendl;
  ...
  auto m = ceph::make_message<MNVMeofGwBeacon>(
      ...
      gw_availability, osdmap_epoch, gwmap_epoch, beacon_sequence,
      // Pass affected features to the constructor
      cluster_beacon_diff_included ? 1 : 0);
  dout(10) << "sending beacon with diff support: "
           << (cluster_beacon_diff_included ? "enabled" : "disabled") << dendl;
...
void
NVMeofGwMonitorClient::disconnect_panic()
{
  auto disconnect_panic_duration =
      g_conf()
          .get_val<std::chrono::seconds>("nvmeof_mon_client_disconnect_panic")
          .count();
  auto now = std::chrono::steady_clock::now();
  auto elapsed_seconds =
      std::chrono::duration_cast<std::chrono::seconds>(now - last_map_time)
          .count();
  if (elapsed_seconds > disconnect_panic_duration) {
    dout(4)
        << "Triggering a panic upon disconnection from the monitor, elapsed "
        << elapsed_seconds << ", configured disconnect panic duration "
        << disconnect_panic_duration << dendl;
    throw std::runtime_error(
        "Lost connection to the monitor (beacon timeout).");
  }
}

void
NVMeofGwMonitorClient::connect_panic()
{
  ...
  auto connect_panic_duration =
      g_conf()
          .get_val<std::chrono::seconds>("nvmeof_mon_client_connect_panic")
          .count();
  auto now = std::chrono::steady_clock::now();
  auto elapsed_seconds =
      std::chrono::duration_cast<std::chrono::seconds>(now - start_time).count();
  if (elapsed_seconds > connect_panic_duration) {
    dout(4) << "Triggering a panic: did not receive initial map from monitor, "
               "elapsed "
            << elapsed_seconds << ", configured connect panic duration "
            << connect_panic_duration << " seconds." << dendl;
    throw std::runtime_error(
        "Did not receive initial map from monitor (connect panic).");
  }
}

void
NVMeofGwMonitorClient::tick()
{
  dout(10) << dendl;
...
void
NVMeofGwMonitorClient::schedule_next_tick()
{
  ceph_assert(ceph_mutex_is_locked_by_me(beacon_lock));
  ...
  auto callback = new LambdaContext([this](int r) { tick(); });

  auto tick_time = next_tick_time;
  if (!timer.add_event_at(tick_time, callback)) {
    throw std::runtime_error(
        "Failed to schedule timer event (timer shutting down)");
  }
  ...
  dout(10) << "Scheduled next tick at " << next_tick_time
           << " (delay: " << delay_ns << " ns, " << (delay_ns / 1000000.0)
           << " ms)" << dendl;
}

void
NVMeofGwMonitorClient::shutdown()
{
  std::lock_guard l(lock);
...
void
NVMeofGwMonitorClient::handle_nvmeof_gw_map(ceph::ref_t<MNVMeofGwMap> nmap)
{
  auto now = std::chrono::steady_clock::now();
  ...
  uint64_t reset_elapsed_seconds =
      std::chrono::duration_cast<std::chrono::seconds>(now - reset_timestamp)
          .count();
  NvmeGwClientState old_gw_state;
  uint64_t ignore_wrong_map_interval_sec =
      g_conf().get_val<uint64_t>("mon_nvmeofgw_wrong_map_ignore_sec");
  auto got_old_gw_state =
      get_gw_state("old map", map, group_key, name, old_gw_state);
  NvmeGwClientState new_gw_state;
  auto got_new_gw_state =
      get_gw_state("new map", new_map, group_key, name, new_gw_state);
  ...
  */
  ...
  cluster_beacon_diff_included = (new_gw_state.map_features &
                                  NVMeofGwMap::FLAG_BEACONDIFF) != 0;
  if (old_cluster_beacon_diff_included != cluster_beacon_diff_included) {
    dout(0) << fmt::format(
                   "Updated cluster features: 0x{:x}",
                   cluster_beacon_diff_included)
            << dendl;
  }
  ...
  // beacon_sequence is incremented after sending, so we compare with (beacon_sequence - 1)
  {
    ...
    if (got_new_gw_state && new_gw_state.last_beacon_seq_ooo) {
      ...
      dout(4) << "Beacon sequence mismatch detected. Expected: "
              << (beacon_sequence - 1)
  ...
  if (!got_old_gw_state) {
    ...
    while (!set_group_id) {
      ...
      if (!set_group_id) {
        ...
        auto retry_timeout =
            g_conf().get_val<uint64_t>("mon_nvmeofgw_set_group_id_retry");
  ...
  if (got_old_gw_state && got_new_gw_state) {
    dout(10) << "got_old_gw_state: " << old_gw_state
             << "got_new_gw_state: " << new_gw_state << dendl;
    ...
    if (old_gw_state.availability == gw_availability_t::GW_AVAILABLE &&
      ...
      dout(4) << "Triggering a panic upon disconnection from the monitor, gw "
                 "state - unavailable"
              << dendl;
      throw std::runtime_error(
          "Lost connection to the monitor (gw map unavailable).");
    }
  }

  // Combined subsystems
  const auto initial_ana_state = std::make_pair(
  ...
  for (const auto& nqn_state_pair : old_gw_state.subsystems) {
    ...
    if (new_gw_state.subsystems.find(nqn) == new_gw_state.subsystems.end()) {
      ana_state_t all_disabled(
          old_nqn_state.ana_state.size(), initial_ana_state);
  ...
  for (const auto& nqn_state_pair : combined_subsystems) {
    ...
    auto found_old_nqn_state =
        (old_nqn_state_pair != old_gw_state.subsystems.end());

    // old and new ana group id ranges could be different
    auto ana_state_size = (found_old_nqn_state)
                              ? std::max(
                                    old_nqn_state_pair->second.ana_state.size(),
                                    sub.ana_state.size())
                              : sub.ana_state.size();

    for (NvmeAnaGrpId ana_grp_index = 0; ana_grp_index < ana_state_size;
         ana_grp_index++) {
      auto new_group_state = (ana_grp_index < sub.ana_state.size())
                                 ? sub.ana_state[ana_grp_index]
                                 : initial_ana_state;
      auto old_group_state =
          (got_old_gw_state && found_old_nqn_state &&
           ana_grp_index < old_nqn_state_pair->second.ana_state.size())
      ...
      if (new_agroup_state ==
          ...
          blocklist_epoch != 0) {
        if (blocklist_epoch > max_blocklist_epoch)
          max_blocklist_epoch = blocklist_epoch;
      }
      gs.set_state(
          new_agroup_state ==
                  gw_exported_states_per_group_t::GW_EXPORTED_OPTIMIZED_STATE
              ? OPTIMIZED
              : INACCESSIBLE); // Set the ANA state
      nas.mutable_states()->Add(std::move(gs));
      dout(10) << " grpid " << (ana_grp_index + 1) << " state: " << new_gw_state
               << dendl;
    }
    if (nas.states_size())
      ai.mutable_states()->Add(std::move(nas));
  }
  ...
  if (ai.states_size()) {
    ...
    if (max_blocklist_epoch > osdmap_epoch) {
      ...
      dout(10) << "Ready for blocklist osd map epoch: " << osdmap_epoch
               << dendl;
    }
  }
  map = new_map;
...
NVMeofGwMonitorClient::ms_dispatch2(const ref_t<Message>& m)
{
  std::lock_guard l(lock);
...
int
NVMeofGwMonitorClient::main(std::vector<const char*> args)
{
  client_messenger->wait();
...
void
NVMeofGwMonitorClient::log_tick_execution_duration(
    const ceph::mono_clock::time_point& start_time)
{
  ...
  if (execution_time > max_allowed_time) {
    dout(1) << "WARNING: Tick execution took "
            << execution_time.count() / 1000000.0
            << "ms, exceeding 10% of tick period ("
            << max_allowed_time.count() / 1000000.0 << "ms)" << dendl;
  }
}
