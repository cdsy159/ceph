...
int
cls_unregister(cls_handle_t handle)
{
    ...
    cls_method_handle_t* handle)
{
  ClassHandler::ClassData* cls = (ClassHandler::ClassData*)hclass;
  cls_method_handle_t hmethod =
      (cls_method_handle_t)cls->register_cxx_method(method, flags, class_call);
  if (handle)
    *handle = hmethod;
  return (hmethod != NULL);
}
...
int
cls_unregister_method(cls_method_handle_t handle)
{
...
void
cls_unregister_filter(cls_filter_handle_t handle)
{
...
int
cls_gen_random_bytes(char* buf, int size)
{
    ...
    int size) /* size should be the required string size + 1 */
{
  ...
  ret = ceph_armor(
      tmp_dest, &tmp_dest[sizeof(tmp_dest)], (const char*)buf,
...
void
cls_cxx_subop_version(cls_method_context_t hctx, std::string* s)
{
  if (!s)
    return;
