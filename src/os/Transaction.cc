...
void
decode_str_str_map_to_bl(bufferlist::const_iterator& p, bufferlist* out)
{
  auto start = p;
  __u32 n;
  decode(n, p);
...
void
decode_str_set_to_bl(bufferlist::const_iterator& p, bufferlist* out)
{
  auto start = p;
  __u32 n;
  decode(n, p);
...
void
Transaction::dump(ceph::Formatter* f)
{
  ...
  while (i.have_op() && !stop_looping) {
    ...
    case Transaction::OP_SETALLOCHINT: {
      ...
      f->dump_string(
          "alloc_hint_flags", ceph_osd_alloc_hint_flag_string(alloc_hint_flags));
    } break;
...
list<Transaction>
Transaction::generate_test_instances()
{
  list<Transaction> o;
...
ostream&
operator<<(ostream& out, const Transaction& tx)
{
