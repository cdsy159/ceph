    ...
    std::string_view name)
{
  ...
  if (type == "stupid") {
    ...
    return new HybridAvlAllocator(
        cct, size, block_size,
        cct->_conf.get_val<uint64_t>("bluestore_hybrid_alloc_mem_cap"), name);
  } else if (type == "hybrid_btree2") {
    return new HybridBtree2Allocator(
        cct, size, block_size,
...
void
Allocator::release(const PExtentVector& release_vec)
{
 ...
 */
double
Allocator::get_fragmentation_score()
{
  ...
  auto get_score = [&](size_t v) -> double {
    size_t sc = sizeof(v) * 8 - std::countl_zero(v) -
    ...
    double x = double(v - sc_shifted) /
               sc_shifted; //x is <0,1) in its scale grade
