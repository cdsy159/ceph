      ...
      if (alloc) {
        ...
        r = admin_socket->register_command(
            ("bluestore allocator fragmentation histogram " + name +
             ...
             " name=num_buckets,type=CephInt,req=false")
                .c_str(),
            this, "build allocator free regions state histogram");
        ceph_assert(r == 0);
      }
    }
  }
...
AllocatorBase::~AllocatorBase() { delete asok_hook; }

const string&
AllocatorBase::get_name() const
{
...
void
AllocatorBase::FreeStateHistogram::record_extent(
    uint64_t alloc_unit,
    uint64_t off,
    uint64_t len)
{
...
void
AllocatorBase::FreeStateHistogram::foreach (
    function<
        void(uint64_t max_len, uint64_t total, uint64_t aligned, uint64_t unit)>
        cb)
{
  size_t i = 0;
  for (const auto& b : buckets) {
    cb(myTraits._get_bucket_max(i), b.total, b.aligned, b.alloc_units);
    ++i;
  }
}
