...
protected:
  ...
  struct LenPartitionedSetTraitsLinear : public LenPartitionedSetTraits {
     ...
     */
    inline size_t
    _get_bucket(uint64_t len) const
    {
     ...
     */
    inline size_t
    _get_bucket_max(size_t bucket) const
    {
      return bucket < num_buckets - 1 ? base * factor * (1 + bucket)
  ...
  struct LenPartitionedSetTraitsPow2 : public LenPartitionedSetTraits {
    ...
    {
      size_t idx;
      const size_t len_p2_max = base << ((factor * (num_buckets - 2)));
      if (len <= base) {
        idx = 0;
    ...
    _get_bucket_max(size_t bucket) const
    {
      return bucket < num_buckets - 1 ? base << (factor * bucket)
                                      : std::numeric_limits<uint64_t>::max();
    }
  };
  ...
  public:
    ...
    bool
    try_put(uint64_t offset, uint64_t len)
    {
    ...
    bool
    try_get(uint64_t* offset, uint64_t len)
    {
      if (!lock.try_lock_shared()) {
        return false;
      }
    ...
    void foreach (std::function<void(uint64_t offset, uint64_t length)> notify)
    {
      ...
      for (uint64_t i = 0; i < buckets.size(); i++) {
        auto cb = [&](uint64_t o) { notify(o, i << myTraits.base_bits); };
...
public:
  AllocatorBase(std::string_view name, int64_t _capacity, int64_t _block_size);
