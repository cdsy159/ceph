 ...
 */
uint64_t
AvlAllocator::_pick_block_after(uint64_t* cursor, uint64_t size, uint64_t align)
{
...
uint64_t
AvlAllocator::_pick_block_fits(uint64_t size, uint64_t align)
{
...
void
AvlAllocator::_add_to_tree(uint64_t start, uint64_t size)
{
  ceph_assert(size != 0);
...
void
AvlAllocator::_process_range_removal(
    uint64_t start,
    uint64_t end,
...
void
AvlAllocator::_remove_from_tree(uint64_t start, uint64_t size)
{
  uint64_t end = start + size;
...
void
AvlAllocator::_try_remove_from_tree(
    uint64_t start,
    uint64_t size,
...
void
AvlAllocator::_release(const release_set_t& release_set)
{
...
void
AvlAllocator::_release(const PExtentVector& release_set)
{
...
void
AvlAllocator::release(const release_set_t& release_set)
{
  std::lock_guard l(lock);
...
double
AvlAllocator::get_fragmentation()
{
  std::lock_guard l(lock);
...
void
AvlAllocator::init_add_free(uint64_t offset, uint64_t length)
{
...
void
AvlAllocator::init_rm_free(uint64_t offset, uint64_t length)
{
  ldout(cct, 10) << __func__ << std::hex << " 0x" << offset << "~" << length
                 << std::dec << dendl;
  if (!length)
