...
BitmapAllocator::BitmapAllocator(
  ...
  AllocatorBase(name, capacity, alloc_unit), cct(_cct)
{
...
void
BitmapAllocator::release(const interval_set<uint64_t>& release_set)
{
  if (cct->_conf->subsys.should_gather<dout_subsys, 10>()) {
...
void
BitmapAllocator::init_add_free(uint64_t offset, uint64_t length)
{
...
void
BitmapAllocator::init_rm_free(uint64_t offset, uint64_t length)
{
  ldout(cct, 10) << __func__ << " 0x" << std::hex << offset << "~" << length
                 << std::dec << dendl;
  auto mas = get_min_alloc_size();
