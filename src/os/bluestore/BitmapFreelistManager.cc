...
void
make_offset_key(uint64_t offset, std::string* key)
{
  key->reserve(10);
...
void
BitmapFreelistManager::setup_merge_operator(KeyValueDB* db, string prefix)
{
...
int
BitmapFreelistManager::_expand(uint64_t old_size, KeyValueDB* db)
{
...
int
BitmapFreelistManager::read_size_meta_from_db(KeyValueDB* kvdb, uint64_t* res)
{
  bufferlist v;
...
void
BitmapFreelistManager::_load_from_db(KeyValueDB* kvdb)
{
...
int
BitmapFreelistManager::_read_cfg(
    std::function<int(const std::string&, std::string*)> cfg_reader)
{
  dout(1) << __func__ << dendl;
  ...
  string keys[key_count] = {
      "bfm_size", "bfm_blocks", "bfm_bytes_per_block", "bfm_blocks_per_key"};
  uint64_t* vals[key_count] = {
      &size, &blocks, &bytes_per_block, &blocks_per_key};
...
void
BitmapFreelistManager::_init_misc()
{
...
void
BitmapFreelistManager::sync(KeyValueDB* kvdb)
{
  _sync(kvdb, true);
}

void
BitmapFreelistManager::_sync(KeyValueDB* kvdb, bool read_only)
{
...
void
BitmapFreelistManager::shutdown()
{
  dout(1) << __func__ << dendl;
}

void
BitmapFreelistManager::enumerate_reset()
{
...
int
get_next_clear_bit(bufferlist& bl, int start)
{
...
int
get_next_set_bit(bufferlist& bl, int start)
{
  const char* p = bl.c_str();
  int bits = bl.length() << 3;
  while (start < bits) {
...
bool
BitmapFreelistManager::enumerate_next(
    KeyValueDB* kvdb,
    uint64_t* offset,
    uint64_t* length)
{
...
void
BitmapFreelistManager::dump(KeyValueDB* kvdb)
{
  enumerate_reset();
...
void
BitmapFreelistManager::allocate(
    uint64_t offset,
    uint64_t length,
    KeyValueDB::Transaction txn)
{
...
uint64_t
BitmapFreelistManager::size_2_block_count(uint64_t target_size) const
{
...
void
BitmapFreelistManager::get_meta(
...
bool
BitmapFreelistManager::validate(uint64_t min_alloc_size) const
{
  bool ret = true;
