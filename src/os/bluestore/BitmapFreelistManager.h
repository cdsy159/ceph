...
class BitmapFreelistManager : public FreelistManager {
  ...
  uint64_t
  _get_offset(uint64_t key_off, int bit)
  {
...
public:
  ...
  int create(uint64_t size, uint64_t granularity, KeyValueDB::Transaction txn)
      override;
  ...
  bool enumerate_next(KeyValueDB* kvdb, uint64_t* offset, uint64_t* length)
      override;

  void allocate(uint64_t offset, uint64_t length, KeyValueDB::Transaction txn)
      override;
  void release(uint64_t offset, uint64_t length, KeyValueDB::Transaction txn)
      override;
  ...
  inline uint64_t
  get_alloc_units() const override
  {
    return size / bytes_per_block;
  }

  inline uint64_t
  get_alloc_size() const override
  {
