...
static void
wal_discard_cb(void* priv, void* priv2)
{
...
static void
db_discard_cb(void* priv, void* priv2)
{
  BlueFS* bluefs = static_cast<BlueFS*>(priv);
  interval_set<uint64_t>* tmp = static_cast<interval_set<uint64_t>*>(priv2);
...
static void
slow_discard_cb(void* priv, void* priv2)
{
  BlueFS* bluefs = static_cast<BlueFS*>(priv);
  interval_set<uint64_t>* tmp = static_cast<interval_set<uint64_t>*>(priv2);
...
public:
  static BlueFS::SocketHook*
  create(BlueFS* bluefs)
  {
    ...
    if (admin_socket) {
      ...
      if (r != 0) {
        ...
        ceph_assert(r == 0);
        r = admin_socket->register_command(
            "bluefs stats reset", hook,
            "Reset maximum historical value for bluefs statistics."
            "");
        ceph_assert(r == 0);
        r = admin_socket->register_command(
            "bluefs files list", hook, "print files in bluefs");
        ceph_assert(r == 0);
        r = admin_socket->register_command(
...
BlueFS::_init_logger()
{
  ...
  b.add_u64(
      l_bluefs_db_total_bytes, "db_total_bytes", "Total bytes (main db device)",
      "b", PerfCountersBuilder::PRIO_USEFUL, unit_t(UNIT_BYTES));
  b.add_u64(
      l_bluefs_db_used_bytes, "db_used_bytes", "Used bytes (main db device)",
      "u", PerfCountersBuilder::PRIO_USEFUL, unit_t(UNIT_BYTES));
  b.add_u64(
      l_bluefs_wal_total_bytes, "wal_total_bytes", "Total bytes (wal device)",
      "walb", PerfCountersBuilder::PRIO_USEFUL, unit_t(UNIT_BYTES));
  b.add_u64(
      l_bluefs_wal_used_bytes, "wal_used_bytes", "Used bytes (wal device)",
      "walu", PerfCountersBuilder::PRIO_USEFUL, unit_t(UNIT_BYTES));
  b.add_u64(
      l_bluefs_slow_total_bytes, "slow_total_bytes",
      "Total bytes (slow device)", "slob", PerfCountersBuilder::PRIO_USEFUL,
      unit_t(UNIT_BYTES));
  b.add_u64(
      l_bluefs_slow_used_bytes, "slow_used_bytes", "Used bytes (slow device)",
      "slou", PerfCountersBuilder::PRIO_USEFUL, unit_t(UNIT_BYTES));
  b.add_u64(
      l_bluefs_meta_ratio, "meta_ratio_micros",
      "Actual metadata/userdata ratio * 1e3", "mera",
      PerfCountersBuilder::PRIO_INTERESTING);
  b.add_u64(
      l_bluefs_num_files, "num_files", "File count", "f",
      PerfCountersBuilder::PRIO_USEFUL);
  b.add_u64(
      l_bluefs_log_bytes, "log_bytes", "Size of the metadata log", "jlen",
      PerfCountersBuilder::PRIO_INTERESTING, unit_t(UNIT_BYTES));
  b.add_u64_counter(
      l_bluefs_log_compactions, "log_compactions",
      "Compactions of the metadata log");
  b.add_u64_counter(
      l_bluefs_log_write_count, "log_write_count",
      "Write op count to the metadata log");
  b.add_u64_counter(
      l_bluefs_logged_bytes, "logged_bytes",
      "Bytes written to the metadata log", "j",
      PerfCountersBuilder::PRIO_CRITICAL, unit_t(UNIT_BYTES));
  b.add_u64_counter(
      l_bluefs_files_written_wal, "files_written_wal", "Files written to WAL");
  b.add_u64_counter(
      l_bluefs_files_written_sst, "files_written_sst", "Files written to SSTs");
  b.add_u64_counter(
      l_bluefs_write_count_wal, "write_count_wal", "Write op count to WAL");
  b.add_u64_counter(
      l_bluefs_write_count_sst, "write_count_sst", "Write op count to SSTs");
  b.add_u64_counter(
      l_bluefs_bytes_written_wal, "bytes_written_wal", "Bytes written to WAL",
      "walb", PerfCountersBuilder::PRIO_CRITICAL);
  b.add_u64_counter(
      l_bluefs_bytes_written_sst, "bytes_written_sst", "Bytes written to SSTs",
      "sstb", PerfCountersBuilder::PRIO_CRITICAL, unit_t(UNIT_BYTES));
  b.add_u64_counter(
      l_bluefs_bytes_written_slow, "bytes_written_slow",
      "Bytes written to WAL/SSTs at slow device", "slwb",
      PerfCountersBuilder::PRIO_CRITICAL, unit_t(UNIT_BYTES));
  b.add_u64_counter(
      l_bluefs_max_bytes_wal, "max_bytes_wal",
      "Maximum bytes allocated from WAL", "mxwb",
      PerfCountersBuilder::PRIO_INTERESTING, unit_t(UNIT_BYTES));
  b.add_u64_counter(
      l_bluefs_max_bytes_db, "max_bytes_db", "Maximum bytes allocated from DB",
      "mxdb", PerfCountersBuilder::PRIO_INTERESTING, unit_t(UNIT_BYTES));
  b.add_u64_counter(
      l_bluefs_max_bytes_slow, "max_bytes_slow",
      "Maximum bytes allocated from SLOW", "mxwb",
      PerfCountersBuilder::PRIO_INTERESTING, unit_t(UNIT_BYTES));
  b.add_u64_counter(
      l_bluefs_slow_alloc_unit, "alloc_unit_slow",
      "Allocation unit size (in bytes) for primary/shared device", "ausb",
      PerfCountersBuilder::PRIO_CRITICAL, unit_t(UNIT_BYTES));
  b.add_u64_counter(
      l_bluefs_db_alloc_unit, "alloc_unit_db",
      "Allocation unit size (in bytes) for standalone DB device", "audb",
      PerfCountersBuilder::PRIO_CRITICAL, unit_t(UNIT_BYTES));
  b.add_u64_counter(
      l_bluefs_wal_alloc_unit, "alloc_unit_wal",
      "Allocation unit size (in bytes) for standalone WAL device", "auwb",
      PerfCountersBuilder::PRIO_CRITICAL, unit_t(UNIT_BYTES));
  b.add_time_avg(
      l_bluefs_read_random_lat, "read_random_lat",
      "Average bluefs read_random latency", "rdrt",
      PerfCountersBuilder::PRIO_INTERESTING);
  b.add_u64_counter(
      l_bluefs_read_random_count, "read_random_count",
      "random read requests processed", NULL, PerfCountersBuilder::PRIO_USEFUL);
  b.add_u64_counter(
      l_bluefs_read_random_bytes, "read_random_bytes",
      "Bytes requested in random read mode", NULL,
      PerfCountersBuilder::PRIO_USEFUL, unit_t(UNIT_BYTES));
  b.add_u64_counter(
      l_bluefs_read_random_disk_count, "read_random_disk_count",
      "random reads requests going to disk", NULL,
      PerfCountersBuilder::PRIO_USEFUL);
  b.add_u64_counter(
      l_bluefs_read_random_disk_bytes, "read_random_disk_bytes",
      "Bytes read from disk in random read mode", "rrb",
      PerfCountersBuilder::PRIO_INTERESTING, unit_t(UNIT_BYTES));
  b.add_u64_counter(
      l_bluefs_read_random_disk_bytes_wal, "read_random_disk_bytes_wal",
      "random reads requests going to WAL disk", NULL,
      PerfCountersBuilder::PRIO_USEFUL, unit_t(UNIT_BYTES));
  b.add_u64_counter(
      l_bluefs_read_random_disk_bytes_db, "read_random_disk_bytes_db",
      "random reads requests going to DB disk", NULL,
      PerfCountersBuilder::PRIO_USEFUL, unit_t(UNIT_BYTES));
  b.add_u64_counter(
      l_bluefs_read_random_disk_bytes_slow, "read_random_disk_bytes_slow",
      "random reads requests going to main disk", "rrsb",
      PerfCountersBuilder::PRIO_INTERESTING, unit_t(UNIT_BYTES));
  b.add_u64_counter(
      l_bluefs_read_random_buffer_count, "read_random_buffer_count",
      "random read requests processed using prefetch buffer", NULL,
      PerfCountersBuilder::PRIO_USEFUL);
  b.add_u64_counter(
      ...
      PerfCountersBuilder::PRIO_USEFUL, unit_t(UNIT_BYTES));
  b.add_time_avg(
      l_bluefs_read_lat, "read_lat", "Average bluefs read latency", "rd_t",
      PerfCountersBuilder::PRIO_INTERESTING);
  b.add_u64_counter(
      l_bluefs_read_count, "read_count", "buffered read requests processed",
      NULL, PerfCountersBuilder::PRIO_USEFUL);
  b.add_u64_counter(
      l_bluefs_read_bytes, "read_bytes",
      "Bytes requested in buffered read mode", NULL,
      PerfCountersBuilder::PRIO_USEFUL, unit_t(UNIT_BYTES));
  b.add_u64_counter(
      l_bluefs_read_disk_count, "read_disk_count",
      "buffered reads requests going to disk", NULL,
      PerfCountersBuilder::PRIO_USEFUL);
  b.add_u64_counter(
      l_bluefs_read_disk_bytes, "read_disk_bytes",
      "Bytes read in buffered mode from disk", "rb",
      PerfCountersBuilder::PRIO_INTERESTING, unit_t(UNIT_BYTES));
  b.add_u64_counter(
      l_bluefs_read_disk_bytes_wal, "read_disk_bytes_wal",
      "reads requests going to WAL disk", NULL,
      PerfCountersBuilder::PRIO_USEFUL, unit_t(UNIT_BYTES));
  b.add_u64_counter(
      l_bluefs_read_disk_bytes_db, "read_disk_bytes_db",
      "reads requests going to DB disk", NULL, PerfCountersBuilder::PRIO_USEFUL,
      unit_t(UNIT_BYTES));
  b.add_u64_counter(
      l_bluefs_read_disk_bytes_slow, "read_disk_bytes_slow",
      "reads requests going to main disk", "rsb",
      PerfCountersBuilder::PRIO_INTERESTING, unit_t(UNIT_BYTES));
  b.add_u64_counter(
      l_bluefs_read_prefetch_count, "read_prefetch_count",
      "prefetch read requests processed", NULL,
      PerfCountersBuilder::PRIO_USEFUL);
  b.add_u64_counter(
      l_bluefs_read_prefetch_bytes, "read_prefetch_bytes",
      "Bytes requested in prefetch read mode", NULL,
      PerfCountersBuilder::PRIO_USEFUL, unit_t(UNIT_BYTES));
  b.add_u64_counter(
      l_bluefs_write_count, "write_count", "Write requests processed");
  b.add_u64_counter(
      l_bluefs_write_disk_count, "write_disk_count",
      "Write requests sent to disk");
  b.add_u64_counter(
      l_bluefs_write_bytes, "write_bytes", "Bytes written", NULL,
      PerfCountersBuilder::PRIO_USEFUL, unit_t(UNIT_BYTES));
  b.add_time_avg(
      l_bluefs_compaction_lat, "compact_lat",
      "Average bluefs log compaction latency", "c__t",
      PerfCountersBuilder::PRIO_INTERESTING);
  b.add_time_avg(
      l_bluefs_compaction_lock_lat, "compact_lock_lat",
      "Average lock duration while compacting bluefs log", "c_lt",
      PerfCountersBuilder::PRIO_INTERESTING);
  b.add_time_avg(
      l_bluefs_fsync_lat, "fsync_lat", "Average bluefs fsync latency", "fs_t",
      PerfCountersBuilder::PRIO_INTERESTING);
  b.add_time_avg(
      l_bluefs_flush_lat, "flush_lat", "Average bluefs flush latency", "fl_t",
      PerfCountersBuilder::PRIO_INTERESTING);
  b.add_time_avg(
      l_bluefs_unlink_lat, "unlink_lat", "Average bluefs unlink latency",
      "unlt", PerfCountersBuilder::PRIO_INTERESTING);
  b.add_time_avg(
      l_bluefs_truncate_lat, "truncate_lat", "Average bluefs truncate latency",
      "trnt", PerfCountersBuilder::PRIO_INTERESTING);
  b.add_u64_counter(
      l_bluefs_alloc_shared_dev_fallbacks, "alloc_slow_fallback",
  ...
  b.add_u64(
      l_bluefs_read_zeros_candidate, "read_zeros_candidate",
      "How many times bluefs read found page with all 0s");
  b.add_u64(
      l_bluefs_read_zeros_errors, "read_zeros_errors",
      "How many times bluefs read found transient page with all 0s");
  b.add_time_avg(
      l_bluefs_wal_alloc_lat, "wal_alloc_lat",
      "Average bluefs wal allocate latency", "bwal",
      PerfCountersBuilder::PRIO_USEFUL);
  b.add_time_avg(
      l_bluefs_db_alloc_lat, "db_alloc_lat",
      "Average bluefs db allocate latency", "bdal",
      PerfCountersBuilder::PRIO_USEFUL);
  b.add_time_avg(
      l_bluefs_slow_alloc_lat, "slow_alloc_lat",
...
BlueFS::_update_logger_stats()
{
  ...
  if (in_data > 0) {
    dout(10) << __func__ << " got meta ratio parameters, "
             << "data: " << in_data << ", meta: " << in_meta << dendl;
    ...
    bluefs_shared_alloc_context_t* _shared_alloc)
{
  ...
  BlockDevice* b = BlockDevice::create(
      cct, path, NULL, NULL, discard_cb[id], static_cast<void*>(this),
      dev_name.c_str());
  if (_shared_alloc) {
...
bool
BlueFS::bdev_support_label(unsigned id)
{
  ceph_assert(id < bdev.size());
  ceph_assert(bdev[id]);
...
BlockDevice*
BlueFS::get_block_device(unsigned id) const
{
...
void
BlueFS::handle_discard(unsigned id, interval_set<uint64_t>& to_release)
{
...
int64_t
BlueFS::get_used_non_bluefs()
{
  uint64_t ret = 0;
  if (shared_alloc_id > 0 && shared_alloc && shared_alloc->a) {
    ret = (int64_t)_get_block_device_size(shared_alloc_id) -
          shared_alloc->a->get_free() - shared_alloc->bluefs_used;
  }
  return ret;
}

uint64_t
BlueFS::_get_used(unsigned id) const
{
  uint64_t used = 0;
  if (!alloc[id])
    return 0;
...
uint64_t
BlueFS::_get_block_device_size(unsigned id) const
{
  ceph_assert(id < bdev.size());
...
uint64_t
BlueFS::get_block_device_size(unsigned id)
{
  return _get_block_device_size(id);
}

uint64_t
BlueFS::get_free(unsigned id)
{
  ceph_assert(id < alloc.size());
...
uint64_t
BlueFS::_get_minimal_reserved(unsigned id) const
{
  uint64_t reserved = 0;
  switch (id) {
  case BDEV_WAL:
...
uint64_t
BlueFS::get_full_reserved(unsigned id)
{
  if (!is_shared_alloc(id)) {
...
void
BlueFS::dump_perf_counters(Formatter* f)
{
...
void
BlueFS::dump_block_extents(ostream& out)
{
  for (unsigned i = 0; i < MAX_BDEV; ++i) {
    ...
    if (i != shared_alloc_id) {
      out << i << " : device size 0x" << std::hex << total << "("
          << byte_u_t(total) << ")" << " : using 0x" << total - free << "("
          << byte_u_t(total - free) << ")" << std::dec << std::endl;
    } else {
      auto bluefs_used = get_used(i);
      auto non_bluefs_used = get_used_non_bluefs();
      out << i << " : device size 0x" << std::hex << total << "("
          << byte_u_t(total) << ")" << " : using 0x" << total - free << "("
          << byte_u_t(total - free) << ")" << " : bluefs used 0x" << bluefs_used
          << "(" << byte_u_t(bluefs_used) << ")" << " : non-bluefs used 0x"
          << non_bluefs_used << "(" << byte_u_t(non_bluefs_used) << ")"
          << std::dec << std::endl;
    }
  }
}
...
int
BlueFS::mkfs(uuid_d osd_uuid, const bluefs_layout_t& layout)
{
  ...
  for (unsigned id = 0; id < bdev.size(); ++id) {
    ...
    if (is_shared_alloc(id)) {
      ...
      dout(1) << __func__ << " new, id " << id << std::hex
              ...
              << alloc_type << ", capacity 0x" << full_size << ", reserved 0x"
              << reserved << ", maybe locked " << locked << ", block size 0x"
              << bdev[id]->get_block_size() << ", alloc unit 0x"
              << alloc_size[id] << std::dec << dendl;
      alloc[id] = Allocator::create(
          cct, alloc_type, full_size, bdev[id]->get_block_size(), name);
...
BlueFS::mount()
{
  ...
  for (uint8_t i = 0; i < MAX_BDEV; i++) {
    ...
    dout(1) << __func__ << std::hex << " final locked allocations " << (int)i
            << " " << locked_alloc[i] << " => " << res_la << dendl;
    if (res_la.head_length) {
      alloc[i]->init_rm_free(res_la.head_offset, res_la.head_length);
    }
    if (res_la.tail_length) {
...
int
BlueFS::maybe_verify_layout(const bluefs_layout_t& layout) const
{
...
void
BlueFS::umount(bool avoid_compact)
{
  dout(1) << __func__ << dendl;
...
int
BlueFS::prepare_new_device(int id, const bluefs_layout_t& layout)
{
  dout(1) << __func__ << dendl;
  ...
  if (id == BDEV_NEWDB) {
    ...
    _rewrite_log_and_layout_sync_LNF_LD(
        false, BDEV_NEWDB, new_log_dev_cur, new_log_dev_next, RENAME_DB2SLOW,
        layout);
...
void
BlueFS::collect_metadata(map<string, string>* pm, unsigned skip_bdev_id)
{
  if (skip_bdev_id != BDEV_DB && bdev[BDEV_DB]) {
    bdev[BDEV_DB]->collect_metadata("bluefs_db_", pm);
    (*pm)["bluefs_db_allocator"] = alloc[BDEV_DB] ? alloc[BDEV_DB]->get_type()
                                                  : "null";
  }
  if (bdev[BDEV_WAL]) {
    bdev[BDEV_WAL]->collect_metadata("bluefs_wal_", pm);
    (*pm)["bluefs_wal_allocator"] = alloc[BDEV_WAL]
                                        ? alloc[BDEV_WAL]->get_type()
                                        : "null";
  }
}

void
BlueFS::get_devices(set<string>* ls)
{
  for (unsigned i = 0; i < MAX_BDEV; ++i) {
    if (bdev[i]) {
...
int
BlueFS::_write_super(int dev)
{
  ++super.seq;
  super._version = log.uses_envelope_mode
                       ? bluefs_super_t::ENVELOPE_MODE_ENABLED
                       : bluefs_super_t::BASELINE;
  // build superblock
  bufferlist bl;
  encode(super, bl);
  ...
  dout(20) << __func__ << " seq " << super.seq << " crc 0x" << std::hex << crc
...
BlueFS::_open_super()
{
  ...
  r = _bdev_read(
      BDEV_DB, get_super_offset(), get_super_length(), &bl, ioc[BDEV_DB], false);
  if (r < 0)
    return r;
...
int
BlueFS::_check_allocations(
    const bluefs_fnode_t& fnode,
    ...
    const char* op_name)
{
  ...
  for (auto e : fnode_extents) {
    ...
    if (int r = _verify_alloc_granularity(
            id, e.offset, e.length, alloc_unit, op_name);
        r < 0) {
      return r;
    }
...
int
BlueFS::_verify_alloc_granularity(
    __u8 id,
    uint64_t offset,
    uint64_t length,
    uint64_t alloc_unit,
...
int
BlueFS::_replay(bool noop, bool to_stdout)
{
  ...
  if (!noop) {
    if (check_allocations) {
      ...
      int r = _check_allocations(
          log_file->fnode, used_blocks, true, "Log from super");
      if (r < 0) {
        return r;
      }
    }
  ...
  while (true) {
    ...
    bufferlist bl;
    {
      ...
      if (r != (int)super.block_size && cct->_conf->bluefs_replay_recovery) {
        r += _do_replay_recovery_read(
            log_reader, pos, read_pos + r, super.block_size - r, &bl);
      }
    ...
    if (more) {
      ...
      if (r < (int)more) {
        ...
        if (cct->_conf->bluefs_replay_recovery) {
          ...
          r += _do_replay_recovery_read(
              log_reader, pos, read_pos + r, more - r, &t);
          if (r < (int)more) {
    ...
    while (!p.end()) {
      ...
      case bluefs_transaction_t::OP_FILE_UPDATE: {
        ...
        decode(fnode, p);
        ceph_assert(
            fnode.encoding == bluefs_node_encoding::PLAIN ||
        ...
        if (!noop) {
          ...
          if (check_allocations) {
            int r = _check_allocations(
                f->fnode, used_blocks, false, "OP_FILE_UPDATE");
            if (r < 0) {
              return r;
            }
          }
          ...
          if (check_allocations) {
            int r = _check_allocations(
                f->fnode, used_blocks, true, "OP_FILE_UPDATE");
            if (r < 0) {
              return r;
            }
          }
      ...
      case bluefs_transaction_t::OP_FILE_UPDATE_INC: {
        ...
        if (!noop) {
          ...
          if (delta.offset != fnode.allocated) {
            ...
            ceph_assert(
                delta.offset == fnode.allocated || delta.extents.empty());
          }
          if (check_allocations) {
            int r = _check_allocations(
                fnode, used_blocks, false, "OP_FILE_UPDATE_INC");
            if (r < 0) {
              return r;
            }
          }
          ...
          dout(20) << __func__ << " 0x" << std::hex << pos << std::dec
                   << ":  op_file_update_inc produced " << " " << fnode << " "
                   << dendl;
          ...
          if (check_allocations) {
            int r = _check_allocations(
                f->fnode, used_blocks, true, "OP_FILE_UPDATE_INC");
            if (r < 0) {
              return r;
            }
          }
      ...
      case bluefs_transaction_t::OP_FILE_REMOVE: {
        ...
        if (!noop) {
          ...
          if (check_allocations) {
            int r = _check_allocations(
                p->second->fnode, used_blocks, false, "OP_FILE_REMOVE");
            if (r < 0) {
              return r;
            }
          }
...
int
BlueFS::device_migrate_to_existing(
    CephContext* cct,
    ...
    const bluefs_layout_t& layout)
{
  ...
  for (auto& [ino, file_ref] : nodes.file_map) {
    ...
    bool rewrite = std::any_of(
        file_ref->fnode.extents.begin(), file_ref->fnode.extents.end(),
        [=](auto& ext) {
    ...
    if (rewrite) {
      ...
      for (const auto& old_ext : old_fnode_extents) {
        ...
        int r = _bdev_read_random(
            old_ext.bdev, old_ext.offset, old_ext.length, (char*)&buf.at(0),
            buffered);
        if (r != 0) {
      ...
      // write entire file
      auto l = _allocate(
          dev_target, bl.length(), 0, &file_ref->fnode, nullptr, 0, false);
      if (l < 0) {
      ...
      for (auto& ext : file_ref->fnode.extents) {
        if (dev_target != dev_target_new && ext.bdev == dev_target) {
          dout(20) << __func__ << "  " << " ... adjusting extent 0x" << std::hex
  ...
  if (devs_source.count(new_log_dev_cur)) {
    ...
    new_log_dev_next = (flags & REMOVE_WAL) || !bdev[BDEV_WAL] ? BDEV_DB
                                                               : BDEV_WAL;
    ...
    const bluefs_layout_t& layout)
{
  ...
  _rewrite_log_and_layout_sync_LNF_LD(
      false, super_dev, new_log_dev_cur, new_log_dev_next, flags, layout);
  return 0;
}

int
BlueFS::revert_wal_to_plain(const std::string& dir, const std::string& name)
{
  int r;
...
BlueFS::revert_wal_to_plain()
{
  ...
  for (const auto& [file_name, file] : dir_copy) {
    if (file->envelope_mode()) {
      ...
      sync_metadata(true);
      dout(10) << __func__
               << fmt::format(" {} envelope mode=>plain mode", file_name)
               << dendl;
    } else {
      dout(10) << __func__ << fmt::format(" {} in plain mode", file_name)
               << dendl;
    }
  }
...
std::ostream&
operator<<(std::ostream& out, const lock_fnode_print& to_lock)
{
...
void
BlueFS::_drop_link_DF(FileRef file)
{
  ...
  while (len > 0) {
    if (off < buf->bl_off || off >= buf->get_buf_end()) {
      ...
      if (!cct->_conf->bluefs_check_for_zeros) {
        r = _bdev_read_random(
            p->bdev, p->offset + x_off, l, out, cct->_conf->bluefs_buffered_io);
      } else {
        r = _read_random_and_check(
            p->bdev, p->offset + x_off, l, out, cct->_conf->bluefs_buffered_io);
      }
      ceph_assert(r == 0);
      off += l;
...
std::ostream&
operator<<(std::ostream& out, const BlueFS::File::envelope_t& w)
{
  out << fmt::format(
      "[wal:{:#x}~{:x} -> file:{:#x}~{:x}/{:x}/{:x}]", w.content_offset,
      w.content_length, w.file_offset, w.head_len, w.content_length,
      w.tail_len);
  return out;
}

std::ostream&
operator<<(std::ostream& out, const BlueFS::File::envelope_t::stamp_t& m)
{
  out << fmt::format(
      "0x{:02x}{:02x}{:02x}{:02x}{:02x}{:02x}{:02x}{:02x}", m.v[0], m.v[1],
      m.v[2], m.v[3], m.v[4], m.v[5], m.v[6], m.v[7]);
  return out;
}

void
BlueFS::_envmode_index_file(FileRef file)
{
  file->stamp = File::envelope_t::generate_stamp(super.uuid, file->fnode.ino);
  dout(10) << file << " required stamp=#" << std::hex << file->stamp << std::dec
           << dendl;
  uint64_t scan_ofs = 0;
  uint64_t env_ofs = 0;
  ...
  while (scan_ofs < file->fnode.allocated) {
    ...
    dout(20) << "envelope " << (envelope_good ? "good " : "bad ") << flush
             << dendl;
    if (envelope_good) {
    ...
    File::envelope_t* fl)
{
  ...
  int64_t r = _read(
      h, file_ofs, File::envelope_t::head_size(), nullptr,
      (char*)&flush_length_le);
  if (r != File::envelope_t::head_size())
    goto fail;
  r = _read(
      h, file_ofs + File::envelope_t::head_size() + flush_length_le,
      File::envelope_t::tail_size(), nullptr, (char*)&stamp);
  if (r != File::envelope_t::tail_size())
    goto fail;
  if (0 != memcmp(&stamp.v, &h->file->stamp.v, sizeof(stamp.v)))
    goto fail;
  dout(20) << __func__ << " read.len=0x" << std::hex << flush_length_le
           << " read.stamp=#" << stamp << std::dec << dendl;
...
fail:
  dout(20) << __func__ << "read.len=0x" << std::hex << flush_length_le
           << " read.stamp=#" << stamp << " required stamp=#" << h->file->stamp
           << std::dec << dendl;
  ...
  dout(10) << __func__
           << " failed to find envelope, created artificial: " << *fl << dendl;
  return false;
}

int64_t
BlueFS::_read_envmode(
    FileReader* h, ///< [in] read from here
    ...
    char* out) ///< [out] optional: or copy it here
{
  ...
  dout(10) << __func__ << " h " << h << " offset: 0x" << off_req << std::hex
           << "~" << len_req << std::hex << dendl;
  if (outbl) {
    outbl->clear();
  }
  ...
  while (off < off_req + len_req) {
    ...
    dout(20) << fmt::format(
                    "{} wal:{:#x}~{:#x} -> file:{:#x}~{:#x}/{:#x}/{:#x}",
                    __func__, fl.content_offset, fl.content_length,
                    fl.file_offset, fl.head_len, fl.content_length, fl.tail_len)
             << dendl;
    ceph_assert(
        fl.content_offset <= off && off < fl.content_offset + fl.content_length);
    uint64_t file_off = fl.file_offset + fl.head_len +
                        (off - fl.content_offset);
    bufferlist res;
    r = _read(
        h, file_off, readable, outbl ? &res : nullptr,
        out ? out + (off - off_req) : nullptr);
    if (r < 0) {
      dout(10) << fmt::format("{} read failed with {:#d}", __func__, r)
               << dendl;
      break;
    }
    if (outbl) {
  ...
  while (len > 0) {
    ...
    if (off < buf->bl_off || off >= buf->get_buf_end()) {
      ...
      if (off < buf->bl_off || off >= buf->get_buf_end()) {
        ...
        bool use_buffered_io = h->file->fnode.ino == 1
                                   ? false
...
void
BlueFS::invalidate_cache(FileRef f, uint64_t offset, uint64_t length)
{
...
uint64_t
BlueFS::_estimate_transaction_size(bluefs_transaction_t* t)
{
  uint64_t max_alloc_size = std::max(
      alloc_size[BDEV_WAL],
...
uint64_t
BlueFS::_make_initial_transaction(
    uint64_t start_seq,
    bluefs_fnode_t& fnode,
...
void
BlueFS::compact_log() /*_LNF_LD_NF_D*/
{
...
bool
BlueFS::_should_start_compact_log_L_N()
{
...
void
BlueFS::_compact_log_dump_metadata_NF(
    uint64_t start_seq,
    bluefs_transaction_t* t,
...
void
BlueFS::_compact_log_sync_LNF_LD()
{
  dout(10) << __func__ << dendl;
  uint8_t prefer_bdev;
  {
  ...
  _rewrite_log_and_layout_sync_LNF_LD(
      true, BDEV_DB, prefer_bdev, prefer_bdev, 0, super.memorized_layout);
...
void
BlueFS::_rewrite_log_and_layout_sync_LNF_LD(
    ...
    std::optional<bluefs_layout_t> layout)
{
  ...
  ceph_assert(
      (permit_dev_fallback && log_dev == log_dev_new) || !permit_dev_fallback);
  ...
  int r = _allocate(
      log_dev, compacted_meta_need, 0, &fnode_tail, nullptr, 0,
      permit_dev_fallback);
  ceph_assert(r == 0);
  ...
  r = _allocate(
      log_dev, starter_need, 0, &fnode_starter, nullptr, 0, permit_dev_fallback);
  ceph_assert(r == 0);
  ...
  _make_initial_transaction(
      starter_seq, fnode_persistent, starter_need, &starter_bl);
...
void
BlueFS::_compact_log_async_LD_LNF_D() //also locks FW for new_writer
{
  dout(10) << __func__ << dendl;
  utime_t mtime = ceph_clock_now();
  uint64_t starter_seq = 1;
  ...
  r = _allocate(
      vselector->select_prefer_bdev(log_file->vselector_hint), starter_need, 0,
      &new_log->fnode);
  ceph_assert(r == 0);
  ...
  bufferlist starter_bl;
  _make_initial_transaction(
      starter_seq, new_log->fnode, starter_need, &starter_bl);
  ...
  log_file->fnode.size = log.writer->pos - old_log_jump_to + starter_need +
                         compacted_meta_need;
...
void
BlueFS::_pad_bl(bufferlist& bl, uint64_t pad_size)
{
...
void
BlueFS::_consume_dirty(uint64_t seq)
{
  ceph_assert(ceph_mutex_is_locked(dirty.lock));
  ceph_assert(ceph_mutex_is_locked(log.lock));
...
void
BlueFS::_extend_log(uint64_t amount)
{
  ceph_assert(ceph_mutex_is_locked(log.lock));
  ...
  ceph_assert(
      allocated_before_extension >= log.writer->get_effective_write_pos());
...
void
BlueFS::_flush_and_sync_log_core()
{
  ceph_assert(ceph_mutex_is_locked(log.lock));
...
void
BlueFS::_clear_dirty_set_stable_D(uint64_t seq)
{
  std::lock_guard dl(dirty.lock);
...
void
BlueFS::_release_pending_allocations(vector<interval_set<uint64_t>>& to_release)
{
...
int
BlueFS::_flush_and_sync_log_LD(uint64_t want_seq)
{
  log.lock.lock();
  dirty.lock.lock();
...
int
BlueFS::_flush_and_sync_log_jump_D(uint64_t jump_to)
{
  ceph_assert(ceph_mutex_is_locked(log.lock));
  ...
  vselector->sub_usage(
      log.writer->file->vselector_hint, log.writer->file->fnode.size);
    ...
    const bluefs_super_t& super)
{
  ...
  if (tail) {
    ...
    if (file->envelope_mode() && buffer.get_append_buffer_unused_tail_length() <
                                     tail + File::envelope_t::head_size()) {
...
int
BlueFS::_signal_dirty_to_log_D(FileWriter* h)
{
...
void
BlueFS::flush_range(FileWriter* h, uint64_t offset, uint64_t length) /*_WF*/
{
...
int
BlueFS::_flush_envelope_F(FileWriter* h)
{
  ceph_assert(ceph_mutex_is_locked(h->lock));
  ceph_assert(h->file->envelope_mode());
  uint64_t content_length = h->get_buffer_length() -
                            File::envelope_t::head_size();
  ...
  h->envelope_head_filler.copy_in(
      File::envelope_t::head_size(), (char*)&flush_length_le);
  uint64_t length = File::envelope_t::head_size() + content_length +
                    File::envelope_t::tail_size();
  return _flush_range_F(h, offset, length);
}

int
BlueFS::_flush_range_F(FileWriter* h, uint64_t offset, uint64_t length)
{
  auto t0 = mono_clock::now();
  ceph_assert(ceph_mutex_is_locked(h->lock));
  ...
  if (allocated < end) {
    ...
    int r = _allocate(
        vselector->select_prefer_bdev(h->file->vselector_hint), end - allocated,
        0, &h->file->fnode, [&](const bluefs_extent_t& e) {
...
void
BlueFS::_claim_completed_aios(FileWriter* h, list<aio_t>* ls)
{
  for (auto p : h->iocv) {
    if (p) {
...
void
BlueFS::_wait_for_aio(FileWriter* h)
{
  ...
  bool flushed_sum = false;
  {
    ...
    if (h->file->envelope_mode() && h->get_buffer_length() == 0) {
      ...
      uint32_t pos1 = h->get_effective_write_pos() -
                      File::envelope_t::head_size();
      uint32_t pos2 =
          reinterpret_cast<uintptr_t>(h->envelope_head_filler.c_str());
    ...
    while (len > 0) {
      ...
      if (l0 < max_size) {
        ...
        buf += l;
        len -= l;
        need_flush = h->get_buffer_length() >=
                     cct->_conf->bluefs_min_flush_size;
      }
      if (need_flush) {
        bool flushed = false;
...
void
BlueFS::flush(FileWriter* h, bool force) /*_WF_LNF_NF_LD_D*/
{
  bool flushed = false;
  int r;
  {
...
int
BlueFS::_flush_F(FileWriter* h, bool force, bool* flushed)
{
  ceph_assert(ceph_mutex_is_locked(h->lock));
...
uint64_t
BlueFS::_flush_special(FileWriter* h)
{
  ceph_assert(h->file->fnode.ino <= 1);
  uint64_t length = h->get_buffer_length();
  uint64_t offset = h->pos;
...
int
BlueFS::truncate(FileWriter* h, uint64_t offset) /*_WF_L*/
{
  auto t0 = mono_clock::now();
  ...
  _flush_bdev(h);
  {
    ...
    if (p != fnode.extents.end()) {
      ...
      if (0 == cut_off) {
        ...
        dirty.pending_release[p->bdev].insert(
            p->offset + cut_off, p->length - cut_off);
...
int
BlueFS::fsync(FileWriter* h) /*_WF_WD_WLD_WLNF_WNF*/
{
  std::unique_lock hl(h->lock);
...
int
BlueFS::_fsync(FileWriter* h, bool force_dirty) /*_F_D_LD_LNF_NF*/
{
  auto t0 = mono_clock::now();
  _maybe_check_vselector_LNF();
...
void
BlueFS::_flush_bdev(FileWriter* h, bool check_mutext_locked)
{
...
void
BlueFS::_flush_bdev(std::array<bool, MAX_BDEV>& dirty_bdevs)
{
...
const char*
BlueFS::get_device_name(unsigned id)
{
  if (id >= MAX_BDEV)
    return "BDEV_INV";
  const char* names[] = {
...
void
BlueFS::_update_allocate_stats(uint8_t id, const ceph::timespan& d)
{
  switch (id) {
  case BDEV_SLOW:
    ...
    bool permit_dev_fallback)
{
  ...
  if (alloc_len < 0 || alloc_len < need) {
    ...
    if (alloc[id] && shared && alloc_unit != shared_unit) {
      ...
      logger->inc(l_bluefs_alloc_shared_size_fallbacks);
      return _allocate(
          id, len, alloc_unit, node, cb, alloc_attempts, permit_dev_fallback);
...
int
BlueFS::preallocate(FileRef f, uint64_t off, uint64_t len) /*_LF*/
{
  std::lock_guard ll(log.lock);
  ...
  if (off + len > allocated) {
    ...
    int r = _allocate(
        vselector->select_prefer_bdev(f->vselector_hint), want, 0, &f->fnode,
...
void
BlueFS::sync_metadata(bool avoid_compact) /*_LNF_NF_LD_D*/
{
  bool can_skip_flush;
  {
    std::lock_guard ll(log.lock);
...
void
BlueFS::_maybe_compact_log_LNF_NF_LD_D()
{
  if (!cct->_conf->bluefs_replay_recovery_disable_compact &&
    ...
    bool overwrite) /*_LND*/
{
  ...
  mempool::bluefs::vector<bluefs_extent_t> pending_release_extents;
  {
    ...
    if (q == dir->file_map.end() || q->first != filename) {
      ...
      create = true;
      vselector->add_usage(
          file->vselector_hint, file->fnode.size, true); // update file count
      logger->set(l_bluefs_num_files, nodes.file_map.size());
    } else {
      ...
      if (overwrite) {
        ...
        file->fnode.size = 0;
        vselector->add_usage(
            file->vselector_hint, file->fnode.size, true); // restore file count
    ...
    *h = _create_writer(file);
    if (boost::algorithm::ends_with(filename, ".log")) {
      if (conf_wal_envelope_mode) {
        ...
        file->stamp =
            File::envelope_t::generate_stamp(super.uuid, file->fnode.ino);
...
void
BlueFS::_drain_writer(FileWriter* h)
{
...
void
BlueFS::_close_writer(FileWriter* h)
{
  _drain_writer(h);
  delete h;
}
...
void
BlueFS::close_writer(FileWriter* h)
{
...
uint64_t
BlueFS::debug_get_dirty_seq(FileWriter* h)
{
...
bool
BlueFS::debug_get_is_dev_dirty(FileWriter* h, uint8_t dev)
{
  std::lock_guard l(h->lock);
...
void
BlueFS::collect_alerts(osd_alert_list_t& alerts)
{
  if (bdev[BDEV_DB]) {
    ...
    bool random) /*_N*/
{
  ...
  *h = new FileReader(
      file, random ? 4096 : cct->_conf->bluefs_max_prefetch,
      file->envelope_mode());
  dout(10) << __func__ << " h " << *h << " on " << file->fnode << dendl;
  return 0;
}
...
int
BlueFS::mkdir(std::string_view dirname) /*_LN*/
{
  std::lock_guard ll(log.lock);
  std::lock_guard nl(nodes.lock);
...
int
BlueFS::rmdir(std::string_view dirname) /*_LN*/
{
  std::lock_guard ll(log.lock);
  std::lock_guard nl(nodes.lock);
  dout(10) << __func__ << " " << dirname << dendl;
...
bool
BlueFS::dir_exists(std::string_view dirname) /*_N*/
{
  std::lock_guard nl(nodes.lock);
  map<string, DirRef>::iterator p = nodes.dir_map.find(dirname);
...
int
BlueFS::unlock_file(FileLock* fl) /*_N*/
{
  std::lock_guard nl(nodes.lock);
...
int
BlueFS::readdir(std::string_view dirname, vector<string>* ls) /*_N*/
{
...
int
BlueFS::unlink(std::string_view dirname, std::string_view filename) /*_LND*/
{
  auto t0 = mono_clock::now();
  std::lock_guard ll(log.lock);
  std::lock_guard nl(nodes.lock);
 ...
 */
int
BlueFS::_do_replay_recovery_read(
    ...
    bufferlist* bl)
{
  ...
  int r = _bdev_read(
      e->bdev, e->offset + e_off, e->length - e_off, &fixed, ioc[e->bdev],
      cct->_conf->bluefs_buffered_io);
  ceph_assert(r == 0);
  ...
  while (e != log_fnode.extents.end()) {
    r = _bdev_read(
        e->bdev, e->offset, e->length, &fixed, ioc[e->bdev],
        cct->_conf->bluefs_buffered_io);
    ceph_assert(r == 0);
    last_e_dev = e->bdev;
    ++e;
  ...
  struct compare {
    bool
    operator()(const bluefs_extent_t& a, const bluefs_extent_t& b) const
    {
      if (a.bdev < b.bdev)
        return true;
  ...
  for (int dcnt = 0; dcnt < 3; dcnt++) {
    ...
    for (size_t i = 0; i < disk_regions_count; reg++, i++) {
      ...
      while (len > last_32.length()) {
        ...
        r = _bdev_read_random(
            dev, pos, chunk_len, raw_data + page_size,
            cct->_conf->bluefs_buffered_io);
        ceph_assert(r == 0);
        ...
        for (char* sp = search_b;; sp += last_32.length()) {
          sp = (char*)memmem(
              sp, search_e - sp, last_32.c_str(), last_32.length());
          if (sp == nullptr) {
            break;
          }
          ...
          r = _bdev_read(
              ne.bdev, ne.offset, ne.length, &candidate, ioc[ne.bdev],
              cct->_conf->bluefs_buffered_io);
          ceph_assert(r == 0);
...
size_t
BlueFS::probe_alloc_avail(int dev, uint64_t alloc_size)
{
  size_t total = 0;
...
void
BlueFS::trim_free_space(const string& type, std::ostream& outss)
{
  unsigned bdev_id;
  if (type == "bdev-wal") {
    bdev_id = BDEV_WAL;
...
void*
OriginalVolumeSelector::get_hint_for_log() const
{
  return reinterpret_cast<void*>(BlueFS::BDEV_WAL);
}

void*
OriginalVolumeSelector::get_hint_by_dir(std::string_view dirname) const
{
...
uint8_t
OriginalVolumeSelector::select_prefer_bdev(void* hint)
{
  return (uint8_t)(reinterpret_cast<uint64_t>(hint));
}

void
OriginalVolumeSelector::get_paths(const std::string& base, paths& res) const
{
  res.emplace_back(base, db_total);
  res.emplace_back(
      base + ".slow",
      slow_total ? slow_total
                 : db_total); // use fake non-zero value if needed to
...
void
OriginalVolumeSelector::dump(ostream& sout)
{
...
void
FitToFastVolumeSelector::get_paths(const std::string& base, paths& res) const
{
  res.emplace_back(base, 1); // size of the last db_path has no effect
}

uint8_t
RocksDBBlueFSVolumeSelector::select_prefer_bdev(void* h)
{
  ...
  case LEVEL_SLOW:
    ...
    if (db_avail4slow > 0) {
      ...
      uint64_t avail = std::min(
          db_avail4slow, max_db_use < db_total ? db_total - max_db_use : 0);

      // considering current DB dev usage for SLOW data
      if (avail > per_level_per_dev_usage.at(
                      BlueFS::BDEV_DB, LEVEL_SLOW - LEVEL_FIRST)) {
        res = BlueFS::BDEV_DB;
      }
    }
    break;
...
void
RocksDBBlueFSVolumeSelector::get_paths(const std::string& base, paths& res) const
{
...
void*
RocksDBBlueFSVolumeSelector::get_hint_by_dir(std::string_view dirname) const
{
...
void
RocksDBBlueFSVolumeSelector::reset_history(ostream& sout)
{
...
void
RocksDBBlueFSVolumeSelector::dump(ostream& sout)
{
  ...
  sout << ">>Settings<<" << " extra=" << byte_u_t(db_avail4slow)
...
bool
RocksDBBlueFSVolumeSelector::compare(BlueFSVolumeSelector* other)
{
  ...
  for (size_t x = 0; x < BlueFS::MAX_BDEV + 1; x++) {
    for (size_t y = 0; y < LEVEL_MAX - LEVEL_FIRST + 1; y++) {
      equal &=
          (per_level_per_dev_usage.at(x, y) ==
