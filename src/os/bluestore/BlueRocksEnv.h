...
public:
  ...
  rocksdb::Status
  RegisterDbPaths(const std::vector<std::string>& paths) override
  {
  ...
  rocksdb::Status
  UnregisterDbPaths(const std::vector<std::string>& paths) override
  {
    return rocksdb::Status::OK();
  }
  ...
  rocksdb::Status
  Truncate(const std::string& fname, size_t size) override
  {
    return rocksdb::Status::NotSupported(
