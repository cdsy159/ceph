...
const vector<uint64_t> bdev_label_positions = {
    BDEV_FIRST_LABEL_POSITION, _1G, 10 * _1G, 100 * _1G, 1000 * _1G};
...
template <typename S>
static void
append_escaped(const string& in, S* out)
{
...
static int
decode_escaped(const char* p, string* out)
{
  char buff[256];
  char* ptr = &buff[0];
  char* max = &buff[252];
...
template <typename T>
static void
_key_encode_shard(shard_id_t shard, T* key)
{
  key->push_back((char)((uint8_t)shard.id + (uint8_t)0x80));
}

static const char*
_key_decode_shard(const char* key, shard_id_t* pshard)
{
...
static void
get_shared_blob_key(uint64_t sbid, string* key)
{
  key->clear();
...
static int
get_key_shared_blob(const string& key, uint64_t* sbid)
{
...
template <typename S>
static void
_key_encode_prefix(const ghobject_t& oid, S* key)
{
...
static const char*
_key_decode_prefix(const char* p, ghobject_t* oid)
{
...
static int
_get_key_object(const char* p, ghobject_t* oid)
{
  int r;
...
template <typename S>
static int
get_key_object(const S& key, ghobject_t* oid)
{
...
template <typename S>
static void
_get_object_key(const ghobject_t& oid, S* key)
{
...
template <typename S>
static void
get_object_key(CephContext* cct, const ghobject_t& oid, S* key)
{
  key->clear();
...
template <typename S>
static void
get_extent_shard_key(const S& onode_key, uint32_t offset, string* key)
{
  key->clear();
...
static void
rewrite_extent_shard_key(uint32_t offset, string* key)
{
...
int
get_key_extent_shard(const string& key, string* onode_key, uint32_t* offset)
{
  ceph_assert(key.size() > sizeof(uint32_t) + 1);
  ceph_assert(*key.rbegin() == EXTENT_SHARD_KEY_SUFFIX);
...
static bool
is_extent_shard_key(const string& key)
{
  return *key.rbegin() == EXTENT_SHARD_KEY_SUFFIX;
}

static void
get_deferred_key(uint64_t seq, string* out)
{
  _key_encode_u64(seq, out);
}

static void
get_pool_stat_key(int64_t pool_id, string* key)
{
  key->clear();
...
static int
get_key_pool_stat(const string& key, uint64_t* pool_id)
{
  const char* p = key.c_str();
  if (key.length() < sizeof(uint64_t))
    return -1;
...
void
_dump_extent_map(CephContext* cct, const BlueStore::ExtentMap& em)
{
  uint64_t pos = 0;
...
template <int LogLevelV>
void
_dump_onode(CephContext* cct, const BlueStore::Onode& o)
{
...
template <int LogLevelV>
void
_dump_transaction(CephContext* cct, ObjectStore::Transaction* t)
{
...
ostream&
operator<<(ostream& out, const BlueStore::Buffer& b)
{
...
std::ostream&
operator<<(std::ostream& out, const BlueStore::pool_fsck_stats_t& s)
{
...
public:
  ...
  bool
  is_ge(const ghobject_t& oid) const
  {
    return cmp(oid) >= 0;
  }

  bool
  is_lt(const ghobject_t& oid) const
  {
    return cmp(oid) < 0;
  }
...
public:
  ...
  void
  lower_bound(const ghobject_t& oid) override
  {
    string key;
  ...
  void
  upper_bound(const ghobject_t& oid) override
  {
    string key;
    get_object_key(m_cct, oid, &key);
  ...
  int
  cmp(const ghobject_t& oid) const override
  {
    ceph_assert(valid());
...
void
BlueStore::GarbageCollector::process_protrusive_extents(
...
{
  ceph_assert(
      start_offset <= start_touch_offset && end_offset >= end_touch_offset);
  ...
  for (auto it = extent_map.seek_lextent(lookup_start_offset);
    ...
    if (it->logical_offset >= start_touch_offset &&
      ...
      BlobInfo& bi = affected_blobs
                         .emplace(b, BlobInfo(b->get_referenced_bytes()))
                         .first->second;

      int adjust = (used_alloc_unit && used_alloc_unit == alloc_unit_start) ? 0
                                                                            : 1;
    ...
    uint64_t min_alloc_size)
{
  ...
  if (gc_start_offset < start_offset || gc_end_offset > end_offset) {
    process_protrusive_extents(
        extent_map, gc_start_offset, gc_end_offset, start_offset, end_offset,
        min_alloc_size);
  }
...
struct LruOnodeCacheShard : public BlueStore::OnodeCacheShard {
  ...
  void
  _add(BlueStore::Onode* o, int level) override
  {
    o->set_cached();
    if (o->pin_nref == 1) {
  ...
  void
  _rm(BlueStore::Onode* o) override
  {
    o->clear_cached();
    ...
    --num;
    dout(20) << __func__ << " " << this << " " << " " << o->oid
             << " removed, num=" << num << dendl;
  }

  void
  maybe_unpin(BlueStore::Onode* o) override
  {
  ...
  void
  _trim_to(uint64_t new_size) override
  {
  ...
  void
  _move_pinned(OnodeCacheShard* to, BlueStore::Onode* o) override
  {
    if (to == this) {
      return;
    }
  ...
  void
  add_stats(uint64_t* onodes, uint64_t* pinned_onodes) override
  {
...
#ifdef DEBUG_CACHE
  void
  _audit(const char* when) override
  {}
#endif
};
...
struct LruBufferCacheShard : public BlueStore::BufferCacheShard {
  ...
  void
  _add(BlueStore::Buffer* b, int level, BlueStore::Buffer* near) override
  {
  ...
  void
  _rm(BlueStore::Buffer* b) override
  {
  ...
  void
  _move(BlueStore::BufferCacheShard* src, BlueStore::Buffer* b) override
  {
    src->_rm(b);
    _add(b, 0, nullptr);
  }
  ...
  void
  _adjust_size(BlueStore::Buffer* b, int64_t delta) override
  {
  ...
  void
  _touch(BlueStore::Buffer* b) override
  {
  ...
  void
  _trim_to(uint64_t max) override
  {
...
public:
  explicit TwoQBufferCacheShard(BlueStore* store) :
    BufferCacheShard(store)
  {}
  ...
  void
  _move(BlueStore::BufferCacheShard* srcc, BlueStore::Buffer* b) override
  {
...
void
BlueStore::BufferSpace::__rm_buffer(BufferCacheShard* cache, Buffer* b)
{
  ceph_assert(b);
...
void
BlueStore::BufferSpace::__erase_from_map(Buffer* b)
{
  ceph_assert(b);
...
void
BlueStore::BufferSpace::_clear(BufferCacheShard* cache)
{
    ...
    uint32_t length)
{
  ...
  while (i != buffer_map.end()) {
    ...
    if (b->offset < offset) {
      ...
      if (b->end() > end) {
        ...
        if (b->data.length()) {
          ...
          _add_buffer(
              cache, new Buffer(this, b->state, b->txc, end, bl, b->flags), 0,
              0, b);
        } else {
          _add_buffer(
              cache, new Buffer(this, b->state, b->txc, end, tail, b->flags), 0,
              0, b);
        }
        if (!b->is_writing()) {
    ...
    if (b->data.length()) {
      ...
      _add_buffer(
          cache, new Buffer(this, b->state, b->txc, end, bl, b->flags), 0, 0, b);
    } else {
      _add_buffer(
          cache, new Buffer(this, b->state, b->txc, end, keep, b->flags), 0, 0,
          b);
    }
    __rm_buffer(cache, &*i);
...
*/
void
BlueStore::BufferSpace::_dup_writing(
    TransContext* txc,
    ...
    uint32_t length)
{
  ...
  for (auto i = _data_lower_bound(offset);
    ...
    Buffer* to_b = new Buffer(
        &onode->bc, b->state, b->txc, offset_to_copy, std::move(buffer_to_copy),
        b->flags);
    ldout(cache->cct, 20) << __func__ << " offset=" << std::hex << offset
...
std::ostream&
operator<<(std::ostream& out, const BlueStore::BufferSpace& bc)
{
...
// TransContext
bool
BlueStore::TransContext::add_writing(Onode* o, uint32_t off, uint32_t len)
{
...
BlueStore::OnodeRef
BlueStore::OnodeSpace::add_onode(const ghobject_t& oid, OnodeRef& o)
{
  std::lock_guard l(cache->lock);
...
void
BlueStore::OnodeSpace::_remove(const ghobject_t& oid)
{
...
BlueStore::OnodeRef
BlueStore::OnodeSpace::lookup(const ghobject_t& oid)
{
...
bool
BlueStore::OnodeSpace::map_any(std::function<bool(Onode*)> f)
{
  std::lock_guard l(cache->lock);
  ldout(cache->cct, 20) << __func__ << dendl;
...
void
BlueStore::OnodeSpace::dump(CephContext* cct)
{
  for (auto& i : onode_map) {
...
void
BlueStore::SharedBlob::dump(Formatter* f) const
{
...
ostream&
operator<<(ostream& out, const BlueStore::SharedBlob& sb)
{
  out << "SharedBlob(" << &sb;
...
void
BlueStore::SharedBlob::get_ref(uint64_t offset, uint32_t length)
{
...
{
  ceph_assert(persistent);
  persistent->ref_map.put(
      offset, length, r, unshare && !*unshare ? unshare : nullptr);
}
...
template <int LogLevelV = 30>
void
BlueStore::SharedBlobSet::dump(CephContext* cct)
{
  std::lock_guard l(lock);
  for (auto& i : sb_map) {
...
void
BlueStore::Blob::dump(Formatter* f) const
{
  if (is_spanning()) {
...
ostream&
operator<<(ostream& out, const BlueStore::Blob& b)
{
  out << "Blob(" << &b;
  if (b.is_spanning()) {
    out << " spanning " << b.id;
...
void
BlueStore::Blob::get_ref(Collection* coll, uint32_t offset, uint32_t length)
{
    ...
    uint32_t* length0)
{
  ...
  if (new_blen > blen) {
    ...
    if (new_blen > blen) {
      ...
      used_in_blob.add_tail(
          new_blen, get_blob().get_release_size(min_alloc_size));
    }
  }
  return true;
}
...
void
BlueStore::Blob::dup(const Blob& from, bool copy_used_in_blob)
{
    ...
    uint32_t len)
{
  ...
  if (end_aligned >= start_roundup) {
    copy_extents(
        cct, from, start_aligned, start_roundup - start_aligned, /*pre_len*/
  ...
  if (bto.has_csum()) {
    ...
    memcpy(
        bto.csum_data.c_str() + csd_item_start * csd_value_size,
    ...
    uint32_t post_len)
{
  ...
  auto check_sane_need_copy = [&](const PExtentVector& from,
    ...
    if (ito == to.end())
      return true; // case 1 - obviously empty
    if (!ito->is_valid()) {
...
void
BlueStore::Blob::copy_extents_over_empty(
    CephContext* cct,
    const Blob& from,
    uint32_t start,
    uint32_t len)
...
bool
BlueStore::Blob::can_merge_blob(const Blob* other, uint32_t& blob_width) const
{
  const Blob* x = other;
  const Blob* y = this;
  // checksums
  ...
  if (xb.has_csum() != yb.has_csum())
    return false;
  if (xb.has_csum()) {
    if (xb.csum_type != yb.csum_type)
      return false;
    if (xb.csum_chunk_order != yb.csum_chunk_order)
      return false;
  }
  // trackers
...
uint32_t
BlueStore::Blob::merge_blob(CephContext* cct, Blob* blob_to_dissolve)
{
  Blob* dst = this;
  ...
  auto move_data = [&](uint32_t pos, uint32_t len) {
    if (src_blob.has_csum()) {
      ...
      ceph_assert(
          dst_blob.csum_data.length() >= (item_no + item_cnt) * csum_value_size);
      memcpy(
          dst_csum_ptr + item_no * csum_value_size,
          src_csum_ptr + item_no * csum_value_size, item_cnt * csum_value_size);
    }
...
void
BlueStore::Blob::split(Collection* coll, uint32_t blob_offset, Blob* r)
{
...
void
BlueStore::Blob::maybe_prune_tail()
{
...
void
BlueStore::Extent::dump(Formatter* f) const
{
...
ostream&
operator<<(ostream& out, const BlueStore::Extent& e)
{
...
void
BlueStore::ExtentMap::dump(Formatter* f) const
{
...
void
BlueStore::ExtentMap::scan_shared_blobs(
    uint64_t start,
    uint64_t length,
...
BlueStore::Blob*
BlueStore::ExtentMap::find_mergable_companion(
    Blob* blob_to_dissolve,
    uint32_t blob_start,
    uint32_t& blob_width,
...
void
BlueStore::ExtentMap::reblob_extents(
    uint32_t blob_start,
    uint32_t blob_end,
    BlobRef from_blob,
    BlobRef to_blob)
...
void
BlueStore::ExtentMap::make_range_shared_maybe_merge(
    TransContext* txc,
    OnodeRef& onoderef,
    uint64_t srcoff,
    uint64_t length)
    ...
    if (!blob.is_shared()) {
      ...
      Blob* b = blob.is_compressed()
                    ...
                    : find_mergable_companion(
                          e.blob.get(), e.blob_start(), blob_width, candidates);
      if (b) {
        ...
        dirty_range_end = std::max<uint32_t>(
    ...
    uint64_t& dstoff)
{
  ...
  if (src_dirty) {
    oldo->extent_map.dirty_range(
        dirty_range_begin, dirty_range_end - dirty_range_begin);
    ...
    uint64_t& dstoff)
{
  ...
  for (auto ep = seek_lextent(srcoff); ep != extent_map.end(); ++ep) {
    ...
    if (e.blob->get_blob().is_compressed()) {
      ...
      cb->get_ref(
          c.get(), e.blob_offset + skip_front,
      ...
      cb->copy_from(
          b->cct, *e.blob, min_release_size, e.blob_offset + skip_front,
          e.length - skip_front - skip_back);
    }
...
void
BlueStore::ExtentMap::update(KeyValueDB::Transaction t, bool just_after_reshard)
{
  auto cct = onode->c->store->cct; //used by dout
  bool do_check = onode->c->store->debug_extent_map_encode_check;
  dout(20) << __func__ << " " << onode->oid
           << (just_after_reshard ? " force" : "") << dendl;
  if (onode->onode.extent_map_shards.empty()) {
    if (inline_bl.length() == 0) {
      unsigned n;
      // we need to encode inline_bl to measure encoded length
      bool never_happen = encode_some(
          0, OBJECT_MAX_SIZE, inline_bl, &n, do_check,
          do_check && just_after_reshard);
    ...
    while (shard != shards.end()) {
      ceph_assert(
          shard->shard_info->offset >= previous_shard->shard_info->offset);
      ...
      if (encode_some(
              shard->shard_info->offset, endoff - shard->shard_info->offset, bl,
              &shard->extents, do_check, do_check && just_after_reshard)) {
        if (just_after_reshard) {
...
BlueStore::ExtentMap::reshard_decision(uint32_t segment_size)
{
  ...
  dout(10) << __func__ << " 0x[" << std::hex << needs_reshard_begin << ","
           ...
           << " of " << onode->onode.extent_map_shards.size() << " shards on "
           << onode->oid << dendl;
  ...
  if (!shards.empty()) {
    ...
    dout(20) << __func__ << "   shards [" << shard_index_begin << ","
             << shard_index_end << ")" << " over 0x[" << std::hex
             << needs_reshard_begin << "," << needs_reshard_end << ")"
             << std::dec << dendl;
  } else {
  ...
  if (segment_size != 0) {
    if (data_reshard_end != needs_reshard_begin) {
      encoded_segment_estimate = bytes * segment_size /
                                 (data_reshard_end - needs_reshard_begin);
    } else {
      derr << __func__ << " 0 reshard-range doing 0x" << std::hex
           << needs_reshard_begin << "-0x" << needs_reshard_end << std::dec
           << " on" << pretty_binary_string(onode->oid.hobj.to_str()) << dendl;
  ...
  for (auto extent = extent_map.lower_bound(dummy); extent != extent_map.end();
    ...
    if (segment_size != 0) { //onode data has strict boundaries
      if (extent->blob_start() >= next_boundary) {
        ...
        if (estimate + encoded_segment_estimate / 2 >=
            target /*it is better to go undersize*/) {
      ...
      bool would_span = (extent->logical_offset < max_blob_end) ||
                        (extent->blob_offset != 0);
      if ((estimate > 0) &&
...
void
BlueStore::ExtentMap::reshard_action(
    ReshardPlan& plan,
    KeyValueDB* db,
    KeyValueDB::Transaction t)
{
  ...
  if (db) {
    fault_range(
        db, needs_reshard_begin, (needs_reshard_end - needs_reshard_begin));
  }
  ...
  if (extent_map_shards.empty()) {
    ...
    extent_map_shards.erase(
        extent_map_shards.begin() + shard_index_begin,
        extent_map_shards.begin() + shard_index_end);
    shards.erase(
        shards.begin() + shard_index_begin, shards.begin() + shard_index_end);
    extent_map_shards.insert(
        extent_map_shards.begin() + shard_index_begin, new_shard_info.begin(),
        new_shard_info.end());
    shards.insert(
        shards.begin() + shard_index_begin, new_shard_info.size(), Shard());
  ...
  if (extent_map_shards.empty()) {
    ...
    if (db) {
      if (spanning_scan_begin < needs_reshard_begin) {
        fault_range(
            db, spanning_scan_begin, needs_reshard_begin - spanning_scan_begin);
      }
      if (spanning_scan_end > needs_reshard_end) {
        fault_range(
            db, needs_reshard_end, spanning_scan_end - needs_reshard_end);
      }
    }
          ...
          for (const auto& sh : shards) {
            if (bstart < sh.shard_info->offset && bend > sh.shard_info->offset) {
              ...
              if (extent->logical_offset < pos && extent->logical_end() > pos) {
                ...
                Extent* ne =
                    new Extent(pos, blob_offset, extent->length - left, b);
  ...
  for (auto p = start; p != extent_map.end() && p->logical_offset < end;
    ...
    if (complain_extent_overlap) {
      if (p->logical_offset < prev_offset_end) {
        ...
        dout(-1) << __func__ << " extents overlap: " << std::hex << offset
                 << "~" << length << " " << p->logical_offset << "~"
                 << p->length << std::dec << std::endl
    ...
    if (!p->blob->is_spanning() && p->blob_escapes_range(offset, length)) {
      ...
      dout(20) << __func__ << std::hex << offset << "~" << length << std::dec
               << " extent stands out " << *p << dendl;
      request_reshard(p->blob_start(), p->blob_end());
      return true;
    } else {
...
void
BlueStore::ExtentMap::ExtentDecoder::decode_extent(
    Extent* le,
    __u8 struct_v,
    bptr_c_it_t& p,
    Collection* c)
...
unsigned
BlueStore::ExtentMap::ExtentDecoder::decode_some(
    const bufferlist& bl,
    Collection* c)
{
  __u8 struct_v;
...
void
BlueStore::ExtentMap::ExtentDecoder::decode_spanning_blobs(
    bptr_c_it_t& p,
    Collection* c)
{
  __u8 struct_v;
...
void
BlueStore::ExtentMap::ExtentDecoderFull::consume_blobid(
    BlueStore::Extent* le,
    bool spanning,
    uint64_t blobid)
{
...
void
BlueStore::ExtentMap::ExtentDecoderFull::consume_blob(
    BlueStore::Extent* le,
    uint64_t extent_no,
    uint64_t sbid,
    BlobRef b)
...
void
BlueStore::ExtentMap::ExtentDecoderFull::consume_spanning_blob(
    uint64_t sbid,
    BlueStore::BlobRef b)
{
...
BlueStore::Extent*
BlueStore::ExtentMap::ExtentDecoderFull::get_next_extent()
{
  return new Extent();
}

void
BlueStore::ExtentMap::ExtentDecoderFull::add_extent(BlueStore::Extent* le)
{
  extent_map.extent_map.insert(*le);
}

unsigned
BlueStore::ExtentMap::decode_some(bufferlist& bl)
{
...
void
BlueStore::ExtentMap::bound_encode_spanning_blobs(size_t& p)
{
  // Version 2 differs from v1 in blob's ref_map
  // serialization only. Hence there is no specific
  // handling at ExtentMap level.
...
void
BlueStore::ExtentMap::encode_spanning_blobs(bufferlist::contiguous_appender& p)
{
  // Version 2 differs from v1 in blob's ref_map
  // serialization only. Hence there is no specific
  // handling at ExtentMap level.
...
void
BlueStore::ExtentMap::init_shards(bool loaded, bool dirty)
{
...
BlueStore::ExtentMap::fault_range_ex(
    KeyValueDB* db,
    uint32_t offset,
    uint32_t length)
{
  ...
  uint32_t right_bound = (size_t)last + 1 < shards.size()
                             ? shards[last + 1].shard_info->offset
                             : OBJECT_MAX_SIZE;
  dout(20) << __func__ << " start=" << start << " last=" << last << " -> 0x"
           << std::hex << left_bound << "~" << right_bound << std::dec << dendl;
  return {left_bound, right_bound};
}

void
BlueStore::ExtentMap::fault_range(
    KeyValueDB* db,
    uint32_t offset,
    uint32_t length)
{
...
void
BlueStore::ExtentMap::maybe_load_shard(KeyValueDB* db, int start, int last)
{
  ...
  while (start <= last) {
    ...
    if (!p->loaded) {
      ...
      bufferlist v;
      generate_extent_shard_key_and_apply(
          onode->key, p->shard_info->offset, &key, [&](const string& final_key) {
      ...
      p->loaded = true;
      uint32_t shard_end = (size_t)start + 1 < shards.size()
                               ? (p + 1)->shard_info->offset
                               : OBJECT_MAX_SIZE;
      dout(20) << __func__ << " open shard for range 0x" << std::hex
               << p->shard_info->offset << "~" << shard_end << std::dec << " ("
...
void
BlueStore::ExtentMap::dirty_range(uint32_t offset, uint32_t length)
{
  dout(20) << __func__ << " 0x" << std::hex << offset << "~" << length
           << std::dec << dendl;
  if (shards.empty()) {
...
BlueStore::ExtentMap::find(uint64_t offset)
{
  Extent dummy(offset);
...
BlueStore::extent_map_t::iterator
BlueStore::ExtentMap::split_at(
    BlueStore::extent_map_t::iterator p,
    uint32_t offset)
{
...
BlueStore::extent_map_t::iterator
BlueStore::ExtentMap::maybe_split_at(uint32_t offset)
{
...
BlueStore::extent_map_t::iterator
BlueStore::ExtentMap::seek_nextent(uint64_t offset)
{
  Extent dummy(offset);
...
bool
BlueStore::ExtentMap::has_any_lextents(uint64_t offset, uint64_t length)
{
...
int
BlueStore::ExtentMap::compress_extent_map(uint64_t offset, uint64_t length)
{
  if (extent_map.empty())
    return 0;
  int removed = 0;
  ...
  while (p != extent_map.end()) {
    ...
    if (p->logical_offset < offset) {
      if (p->logical_end() > end) {
        ...
        OldExtent* oe = OldExtent::create(
            c, offset, p->blob_offset + front, length, p->blob);
        ...
        OldExtent* oe = OldExtent::create(
            c, offset, p->blob_offset + keep, p->length - keep, p->blob);
        old_extents->push_back(*oe);
        p->length = keep;
        ++p;
        continue;
    ...
    if (p->logical_offset + p->length <= end) {
      // deref whole lextent
      OldExtent* oe = OldExtent::create(
          c, p->logical_offset, p->blob_offset, p->length, p->blob);
      old_extents->push_back(*oe);
      rm(p++);
      continue;
    }
    ...
    OldExtent* oe = OldExtent::create(
        c, p->logical_offset, p->blob_offset, p->length - keep, p->blob);
    old_extents->push_back(*oe);
...
BlueStore::BlobRef
BlueStore::ExtentMap::split_blob(BlobRef lb, uint32_t blob_offset, uint32_t pos)
{
...
BlueStore::ExtentMap::debug_list_disk_layout()
{
  ...
  for (auto ep = extent_map.begin(); ep != extent_map.end(); ++ep) {
    ...
    if (bblob.has_csum()) {
      ...
      length = p2align(ep->blob_offset + chunk_size, chunk_size) -
               ep->blob_offset;
      length = std::min<uint32_t>(length_left, length);
    }
...
std::ostream&
operator<<(std::ostream& out, const BlueStore::ExtentMap::debug_au_vector_t& auv)
{
  out << "[";
...
// '-' < '.' < '~'
void
BlueStore::Onode::calc_omap_header(
    uint8_t flags,
    const Onode* o,
    std::string* out)
{
...
void
BlueStore::Onode::calc_omap_tail(uint8_t flags, const Onode* o, std::string* out)
{
  if (!bluestore_onode_t::is_pgmeta_omap(flags)) {
    if (bluestore_onode_t::is_perpg_omap(flags)) {
      _key_encode_u64(o->c->pool(), out);
    ...
    bool use_onode_segmentation)
{
  ...
  on->onode.decode(
      p, use_onode_segmentation ? 0 : bluestore_onode_t::FLAG_DEBUG_FORCE_V2);
  ...
  if (on->c) {
    ...
    if (on->prev_spanning_cnt != 0) {
      on->c->store->logger->inc(
          l_bluestore_spanning_blobs, on->prev_spanning_cnt);
    }
  }
    ...
    bool use_onode_segmentation)
{
  ...
  Onode* on = new Onode(
      c.get(), oid, (const mempool::bluestore_cache_meta::string)(key));
  ...
  if (v.length()) {
    ...
    if (segment_size != 0 && c->comp_max_blob_size.has_value() &&
        segment_size < c->comp_max_blob_size.value()) {
      segment_size =
          c->comp_max_blob_size
              .value(); // compression larger than global segment_size, use it
    }
...
void
BlueStore::Onode::dump(Formatter* f) const
{
  onode.dump(f);
  extent_map.dump(f);
}
...
void
BlueStore::Onode::rewrite_omap_key(const string& old, string* out)
{
...
size_t
BlueStore::Onode::calc_userkey_offset_in_omap_key() const
{
...
void
BlueStore::Onode::decode_omap_key(const string& key, string* user_key)
{
...
bool
BlueStore::WriteContext::has_conflict(
    BlobRef b,
    uint64_t loffs,
    uint64_t loffs_end,
    uint64_t min_alloc_size)
...
void
BlueStore::DeferredBatch::prepare_write(
    CephContext* cct,
    uint64_t seq,
    uint64_t offset,
    uint64_t length,
...
void
BlueStore::DeferredBatch::_discard(
    CephContext* cct,
    uint64_t offset,
    uint64_t length)
{
  ...
  if (p != iomap.begin()) {
    ...
    if (end > offset) {
      ...
      if (end > offset + length) {
        bufferlist tail;
        tail.substr_of(
            p->second.bl, offset + length - p->first, end - (offset + length));
...
void
BlueStore::DeferredBatch::_audit(CephContext* cct)
{
  map<uint64_t, int> sb;
...
BlueStore::Collection::Collection(
    BlueStore* store_,
    OnodeCacheShard* oc,
    BufferCacheShard* bc,
    coll_t cid) :
...
bool
BlueStore::Collection::flush_commit(Context* c)
{
  return osr->flush_commit(c);
}

void
BlueStore::Collection::flush()
{
  osr->flush();
}

void
BlueStore::Collection::flush_all_but_last()
{
  osr->flush_all_but_last();
}

void
BlueStore::Collection::open_shared_blob(uint64_t sbid, BlobRef b)
{
...
void
BlueStore::Collection::load_shared_blob(SharedBlobRef sb)
{
  if (!sb->is_loaded()) {
...
void
BlueStore::Collection::make_blob_shared(uint64_t sbid, BlobRef b)
{
...
uint64_t
BlueStore::Collection::make_blob_unshared(SharedBlob* sb)
{
...
void
BlueStore::Collection::split_cache(Collection* dest)
{
...
BlueStore::MempoolThread::entry()
{
  ...
  while (!stop) {
    ...
    uint64_t period = store->cct->_conf.get_val<uint64_t>(
    ...
    if (period != 0 && store->alloc) {
      ...
      if (elapsed > make_timespan(period)) {
        ...
        dout(0) << std::fixed << std::setprecision(6)
                << "fragmentation_score=" << score << " took=" << seconds << "s"
                << dendl;
      }
    }
...
void
BlueStore::MempoolThread::_resize_shards(bool interval_stats)
{
...
void
BlueStore::MempoolThread::_update_cache_settings()
{
...
static void
discard_cb(void* priv, void* priv2)
{
  BlueStore* store = static_cast<BlueStore*>(priv);
...
void
BlueStore::handle_discard(interval_set<uint64_t>& to_release)
{
...
BlueStore::get_tracked_keys() const noexcept
{
  return {
    ...
    const std::set<std::string>& changed)
{
  ...
  if (changed.count("bluestore_onode_segment_size")) {
    segment_size =
        (cct->_conf.get_val<Option::size_t>("bluestore_onode_segment_size"));
  }
...
BlueStore::_set_compression()
{
  ...
  CompressorRef c = !alg_name.empty() ? Compressor::create(cct, alg_name)
                                      : CompressorRef();
  if (c) {
  ...
  dout(10) << __func__ << " mode " << Compressor::get_comp_mode_name(comp_mode)
           << " alg " << alg_name << " min_blob " << comp_min_blob_size
           << " max_blob " << comp_max_blob_size << " segment_size "
           << segment_size << dendl;
}
...
void
BlueStore::_set_throttle_params()
{
...
void
BlueStore::_update_osd_memory_options()
{
...
void
BlueStore::_update_allocator_lookup_policy()
{
  ...
  dout(5) << __func__ << " use_last_lookup_position "
          << use_last_allocator_lookup_position << dendl;
}

int
BlueStore::_set_cache_sizes()
{
  ceph_assert(bdev);
  ...
  auto _set_bin = [&](std::string conf_name, std::vector<uint64_t>* intervals) {
    ...
    std::copy(
        ...
        std::istream_iterator<uint64_t>(), std::back_inserter(*intervals));
  };
...
int
BlueStore::write_meta(const std::string& key, const std::string& value)
{
  ...
  if (bdev_label_valid_locations.empty()) {
    _read_multi_bdev_label(
        cct, bdev, p, fsid, &bdev_label, &bdev_label_valid_locations,
...
int
BlueStore::read_meta(const std::string& key, std::string* value)
{
  ...
  if (bdev_label_valid_locations.empty()) {
    _read_multi_bdev_label(
        cct, local_bdev, p, fsid, &bdev_label, &bdev_label_valid_locations,
        &bdev_label_multi, &bdev_label_epoch);
  }
  if (!bdev_label_valid_locations.empty()) {
...
int
BlueStore::read_meta_conf_check_env()
{
  int r = 0;
  std::string esb;
...
BlueStore::_init_logger()
{
  ...
  //****************************************
  b.add_u64(
      l_bluestore_allocated, "allocated", "Sum for allocated bytes", "al_b",
      PerfCountersBuilder::PRIO_CRITICAL, unit_t(UNIT_BYTES));
  b.add_u64(
      l_bluestore_stored, "stored", "Sum for stored bytes", "st_b",
      PerfCountersBuilder::PRIO_CRITICAL, unit_t(UNIT_BYTES));
  b.add_u64(
      l_bluestore_omap, "omap_bytes", "Sum of bytes in OMAPs", "omap",
      PerfCountersBuilder::PRIO_INTERESTING, unit_t(UNIT_BYTES));
  b.add_u64(
      l_bluestore_fragmentation, "fragmentation_micros",
  ...
  b.add_u64(
      l_bluestore_alloc_unit, "alloc_unit", "allocation unit size in bytes",
      "au_b", PerfCountersBuilder::PRIO_CRITICAL, unit_t(UNIT_BYTES));
  //****************************************

  // Update op processing state latencies
  //****************************************
  b.add_time_avg(
      l_bluestore_state_prepare_lat, "state_prepare_lat",
      "Average prepare state latency", "sprl",
      PerfCountersBuilder::PRIO_USEFUL);
  b.add_time_avg(
      l_bluestore_state_aio_wait_lat, "state_aio_wait_lat",
      "Average aio_wait state latency", "sawl",
      PerfCountersBuilder::PRIO_INTERESTING);
  b.add_time_avg(
      l_bluestore_state_io_done_lat, "state_io_done_lat",
  ...
  b.add_time_avg(
      ...
      PerfCountersBuilder::PRIO_USEFUL);
  b.add_time_avg(
      l_bluestore_state_kv_committing_lat, "state_kv_commiting_lat",
      "Average kv_commiting state latency", "skcl",
      PerfCountersBuilder::PRIO_USEFUL);
  b.add_time_avg(
      l_bluestore_state_kv_done_lat, "state_kv_done_lat",
      "Average kv_done state latency", "skdl",
      PerfCountersBuilder::PRIO_USEFUL);
  b.add_time_avg(
      l_bluestore_state_finishing_lat, "state_finishing_lat",
      "Average finishing state latency", "sfnl",
      PerfCountersBuilder::PRIO_USEFUL);
  b.add_time_avg(
      l_bluestore_state_done_lat, "state_done_lat",
      "Average done state latency", "sdnl", PerfCountersBuilder::PRIO_USEFUL);
  b.add_time_avg(
      l_bluestore_state_deferred_queued_lat, "state_deferred_queued_lat",
      "Average deferred_queued state latency", "sdql",
      PerfCountersBuilder::PRIO_USEFUL);
  b.add_time_avg(
      l_bluestore_state_deferred_aio_wait_lat, "state_deferred_aio_wait_lat",
      "Average aio_wait state latency", "sdal",
      PerfCountersBuilder::PRIO_USEFUL);
  b.add_time_avg(
      l_bluestore_state_deferred_cleanup_lat, "state_deferred_cleanup_lat",
  ...
  //****************************************
  b.add_time_avg(
      l_bluestore_throttle_lat, "txc_throttle_lat",
      "Average submit throttle latency", "th_l",
      PerfCountersBuilder::PRIO_CRITICAL);
  b.add_time_avg(
      l_bluestore_submit_lat, "txc_submit_lat", "Average submit latency", "s_l",
      PerfCountersBuilder::PRIO_CRITICAL);
  b.add_time_avg(
      l_bluestore_commit_lat, "txc_commit_lat", "Average commit latency", "c_l",
      PerfCountersBuilder::PRIO_CRITICAL);
  ...
  //****************************************
  b.add_time_avg(
      l_bluestore_read_onode_meta_lat, "read_onode_meta_lat",
      "Average read onode metadata latency", "roml",
      PerfCountersBuilder::PRIO_USEFUL);
  b.add_time_avg(
      l_bluestore_read_wait_aio_lat, "read_wait_aio_lat",
      "Average read I/O waiting latency", "rwal",
      PerfCountersBuilder::PRIO_USEFUL);
  b.add_time_avg(
      l_bluestore_csum_lat, "csum_lat", "Average checksum latency", "csml",
      PerfCountersBuilder::PRIO_USEFUL);
  b.add_u64_counter(
      l_bluestore_read_eio, "read_eio",
      "Read EIO errors propagated to high level callers");
  b.add_u64_counter(
      l_bluestore_reads_with_retries, "reads_with_retries",
      "Read operations that required at least one retry due to failed checksum "
      "validation",
      "rd_r", PerfCountersBuilder::PRIO_USEFUL);
  b.add_time_avg(
      l_bluestore_read_lat, "read_lat", "Average read latency", "r_l",
      PerfCountersBuilder::PRIO_CRITICAL);
  //****************************************
  ...
  //****************************************
  b.add_time_avg(
      l_bluestore_kv_flush_lat, "kv_flush_lat",
      "Average kv_thread flush latency", "kfsl",
      PerfCountersBuilder::PRIO_INTERESTING);
  b.add_time_avg(
      l_bluestore_kv_commit_lat, "kv_commit_lat",
      "Average kv_thread commit latency", "kcol",
      PerfCountersBuilder::PRIO_USEFUL);
  b.add_time_avg(
      l_bluestore_kv_sync_lat, "kv_sync_lat", "Average kv_sync thread latency",
      "kscl", PerfCountersBuilder::PRIO_INTERESTING);
  b.add_time_avg(
      l_bluestore_kv_final_lat, "kv_final_lat",
  ...
  //****************************************
  b.add_time_avg(
      l_bluestore_write_lat, "write_lat", "write_op average execution time",
      "aw", PerfCountersBuilder::PRIO_USEFUL);
  b.add_u64_counter(
      l_bluestore_write_big, "write_big",
      "Large aligned writes into fresh blobs");
  b.add_u64_counter(
      l_bluestore_write_big_bytes, "write_big_bytes",
      "Large aligned writes into fresh blobs (bytes)", NULL,
      PerfCountersBuilder::PRIO_DEBUGONLY, unit_t(UNIT_BYTES));
  b.add_u64_counter(
      l_bluestore_write_big_blobs, "write_big_blobs",
      "Large aligned writes into fresh blobs (blobs)");
  b.add_u64_counter(
      l_bluestore_write_big_deferred, "write_big_deferred",
      "Big overwrites using deferred");

  b.add_u64_counter(
      l_bluestore_write_small, "write_small",
      "Small writes into existing or sparse small blobs");
  b.add_u64_counter(
      l_bluestore_write_small_bytes, "write_small_bytes",
      "Small writes into existing or sparse small blobs (bytes)", NULL,
      PerfCountersBuilder::PRIO_DEBUGONLY, unit_t(UNIT_BYTES));
  b.add_u64_counter(
      l_bluestore_write_small_unused, "write_small_unused",
      "Small writes into unused portion of existing blob");
  b.add_u64_counter(
      l_bluestore_write_small_pre_read, "write_small_pre_read",
      "Small writes that required we read some data (possibly "
      "cached) to fill out the block");

  b.add_u64_counter(
      l_bluestore_write_pad_bytes, "write_pad_bytes",
      "Sum for write-op padded bytes", NULL,
      PerfCountersBuilder::PRIO_DEBUGONLY, unit_t(UNIT_BYTES));
  b.add_u64_counter(
      l_bluestore_write_penalty_read_ops, "write_penalty_read_ops",
      "Sum for write penalty read ops");
  b.add_u64_counter(l_bluestore_write_new, "write_new", "Write into new blob");

  b.add_u64_counter(
      l_bluestore_issued_deferred_writes, "issued_deferred_writes",
      "Total deferred writes issued");
  b.add_u64_counter(
      l_bluestore_issued_deferred_write_bytes, "issued_deferred_write_bytes",
      "Total bytes in issued deferred writes", NULL,
      PerfCountersBuilder::PRIO_DEBUGONLY, unit_t(UNIT_BYTES));
  b.add_u64_counter(
      l_bluestore_submitted_deferred_writes, "submitted_deferred_writes",
  ...
  b.add_u64_counter(
      l_bluestore_write_big_skipped_blobs, "write_big_skipped_blobs",
      "Large aligned writes into fresh blobs skipped due to zero detection "
      "(blobs)");
  b.add_u64_counter(
      l_bluestore_write_big_skipped_bytes, "write_big_skipped_bytes",
      "Large aligned writes into fresh blobs skipped due to zero detection "
      "(bytes)");
  b.add_u64_counter(
      l_bluestore_write_small_skipped, "write_small_skipped",
      "Small writes into existing or sparse small blobs skipped due to zero "
      "detection");
  b.add_u64_counter(
      l_bluestore_write_small_skipped_bytes, "write_small_skipped_bytes",
      "Small writes into existing or sparse small blobs skipped due to zero "
      "detection (bytes)");
  //****************************************
  ...
  //****************************************
  b.add_u64(
      l_bluestore_compressed, "compressed", "Sum for stored compressed bytes",
      "c", PerfCountersBuilder::PRIO_USEFUL, unit_t(UNIT_BYTES));
  b.add_u64(
      l_bluestore_compressed_allocated, "compressed_allocated",
      "Sum for bytes allocated for compressed data", "c_a",
      PerfCountersBuilder::PRIO_USEFUL, unit_t(UNIT_BYTES));
  b.add_u64(
      l_bluestore_compressed_original, "compressed_original",
      "Sum for original bytes that were compressed", "c_o",
      PerfCountersBuilder::PRIO_USEFUL, unit_t(UNIT_BYTES));
  b.add_time_avg(
      l_bluestore_compress_lat, "compress_lat", "Average compress latency",
      "_cpl", PerfCountersBuilder::PRIO_USEFUL);
  b.add_time_avg(
      l_bluestore_decompress_lat, "decompress_lat",
      "Average decompress latency", "dcpl", PerfCountersBuilder::PRIO_USEFUL);
  b.add_u64_counter(
      l_bluestore_compress_success_count, "compress_success_count",
  ...
  b.add_u64(
      l_bluestore_pinned_onodes, "onodes_pinned",
      "Number of pinned onodes in cache");
  b.add_u64_counter(
      l_bluestore_onode_hits, "onode_hits", "Count of onode cache lookup hits",
      "o_ht", PerfCountersBuilder::PRIO_USEFUL);
  b.add_u64_counter(
      l_bluestore_onode_misses, "onode_misses",
      "Count of onode cache lookup misses", "o_ms",
      PerfCountersBuilder::PRIO_USEFUL);
  b.add_u64_counter(
      l_bluestore_onode_shard_hits, "onode_shard_hits",
      "Count of onode shard cache lookups hits");
  b.add_u64_counter(
      l_bluestore_onode_shard_misses, "onode_shard_misses",
  ...
  b.add_u64(
      l_bluestore_spanning_blobs, "onode_spanning_blobs",
  ...
  b.add_u64(
      l_bluestore_buffer_bytes, "buffer_bytes",
      "Number of buffer bytes in cache", NULL,
      PerfCountersBuilder::PRIO_DEBUGONLY, unit_t(UNIT_BYTES));
  b.add_u64_counter(
      l_bluestore_buffer_hit_bytes, "buffer_hit_bytes",
      "Sum for bytes of read hit in the cache", NULL,
      PerfCountersBuilder::PRIO_DEBUGONLY, unit_t(UNIT_BYTES));
  b.add_u64_counter(
      l_bluestore_buffer_miss_bytes, "buffer_miss_bytes",
  ...
  //****************************************
  b.add_u64_counter(
      l_bluestore_onode_reshard, "onode_reshard",
      "Onode extent map reshard events");
  b.add_u64_counter(
      l_bluestore_blob_split, "blob_split",
  ...
  b.add_u64_counter(
      l_bluestore_gc_merged, "gc_merged",
  ...
  //****************************************
  b.add_u64_counter(
      l_bluestore_omap_iterator_count, "omap_iterator_count",
      "Open omap iterators count");
  b.add_u64_counter(
      l_bluestore_omap_setheader_count, "omap_setheader_count",
      "amount of omap setheader calls");
  b.add_u64_counter(
      l_bluestore_omap_setheader_bytes, "omap_setheader_bytes",
      "amount of bytes set by omap setheader calls");
  b.add_u64_counter(
      l_bluestore_omap_setkeys_count, "omap_setkeys_count",
      "amount of omap setkeys calls");
  b.add_u64_counter(
      l_bluestore_omap_setkeys_records, "omap_setkeys_records",
      "amount of keys set by omap setkeys calls");
  b.add_u64_counter(
      l_bluestore_omap_setkeys_bytes, "omap_setkeys_bytes",
      "amount of bytes set by omap setkeys calls");
  b.add_u64_counter(
      l_bluestore_omap_rmkeys_count, "omap_rmkeys_count",
      "amount of omap keys removed via rmkeys");
  b.add_u64_counter(
      l_bluestore_omap_rmkey_ranges_count, "omap_rmkey_range_count",
  ...
  //****************************************
  b.add_time_avg(
      l_bluestore_omap_upper_bound_lat, "omap_upper_bound_lat",
      "Average omap iterator upper_bound call latency", "oubl",
      PerfCountersBuilder::PRIO_USEFUL);
  b.add_time_avg(
      l_bluestore_omap_lower_bound_lat, "omap_lower_bound_lat",
      "Average omap iterator lower_bound call latency", "olbl",
      PerfCountersBuilder::PRIO_USEFUL);
  b.add_time_avg(
      l_bluestore_omap_next_lat, "omap_next_lat",
      "Average omap iterator next call latency", "onxl",
      PerfCountersBuilder::PRIO_USEFUL);
  b.add_time_avg(
      l_bluestore_omap_get_keys_lat, "omap_get_keys_lat",
      "Average omap get_keys call latency", "ogkl",
      PerfCountersBuilder::PRIO_USEFUL);
  b.add_time_avg(
      l_bluestore_omap_get_values_lat, "omap_get_values_lat",
      "Average omap get_values call latency", "ogvl",
      PerfCountersBuilder::PRIO_USEFUL);
  b.add_time_avg(
      l_bluestore_omap_clear_lat, "omap_clear_lat",
      "Average omap clear call latency");
  b.add_time_avg(
      l_bluestore_clist_lat, "clist_lat", "Average collection listing latency",
      "cl_l", PerfCountersBuilder::PRIO_USEFUL);
  b.add_time_avg(
      l_bluestore_remove_lat, "remove_lat", "Average removal latency", "rm_l",
      PerfCountersBuilder::PRIO_USEFUL);
  b.add_time_avg(
      l_bluestore_truncate_lat, "truncate_lat", "Average truncate latency",
  ...
  //****************************************
  b.add_u64_counter(
      l_bluestore_slow_aio_wait_count, "slow_aio_wait_count",
      "Slow op count for aio wait", "sawc", PerfCountersBuilder::PRIO_USEFUL);
  b.add_u64_counter(
      l_bluestore_slow_committed_kv_count, "slow_committed_kv_count",
      "Slow op count for committed kv", "sckc",
      PerfCountersBuilder::PRIO_USEFUL);
  b.add_u64_counter(
      l_bluestore_slow_read_onode_meta_count, "slow_read_onode_meta_count",
      "Slow op count for read onode meta", "sroc",
      PerfCountersBuilder::PRIO_USEFUL);
  b.add_u64_counter(
      l_bluestore_slow_read_wait_aio_count, "slow_read_wait_aio_count",
      "Slow op count for read wait aio", "srwc",
      PerfCountersBuilder::PRIO_USEFUL);
  b.add_u64_counter(
      l_bluestore_slow_op_normal_count, "slow_op_normal_count",
      "Slow Normal op count in BlueStore", "snoc",
      PerfCountersBuilder::PRIO_USEFUL);
  b.add_u64_counter(
      l_bluestore_slow_op_scrub_count, "slow_op_scrub_count",
      "Slow Scrub op count in BlueStore", "ssoc",
      PerfCountersBuilder::PRIO_USEFUL);
  ...
  b.add_time_avg(
      l_bluestore_allocator_lat, "allocator_lat",
...
BlueStore::_reload_logger()
{
  ...
  if (r >= 0) {
    ...
    logger->set(
        l_bluestore_compressed_original, store_statfs.data_compressed_original);
  }
  return r;
}

void
BlueStore::_shutdown_logger()
{
...
int
BlueStore::get_block_device_fsid(
    CephContext* cct,
    const string& path,
    uuid_d* fsid)
{
    ...
    std::vector<uint64_t> locations)
{
  ...
  bl.rebuild_aligned_size_and_memory(
      BDEV_LABEL_BLOCK_SIZE, BDEV_LABEL_BLOCK_SIZE, IOV_MAX);
  int r = 0;
    ...
    uint64_t disk_position)
{
  ...
  try {
    ...
    dout(0) << __func__ << " " << path.c_str() << " data at 0x" << std::hex
            << disk_position << std::dec << ", " << "unable to decode label "
            << dendl;
    return -ENOENT;
  }
  if (crc != expected_crc) {
...
*/
int
BlueStore::_read_multi_bdev_label(
    CephContext* cct,
    BlockDevice* bdev,
    const string& path,
    uuid_d fsid,
...
void
BlueStore::_main_bdev_label_try_reserve()
{
...
void
BlueStore::_main_bdev_label_remove(Allocator* an_alloc)
{
  ceph_assert(bdev_label_multi == true);
  uint64_t lsize = std::max(BDEV_LABEL_BLOCK_SIZE, min_alloc_size);
...
int
BlueStore::_check_or_set_bdev_label(
    BlockDevice* bdev,
    const string& path,
    const string& desc,
    bool create)
...
int
BlueStore::_set_main_bdev_label()
{
  ...
  int r = _write_bdev_label(
      cct, bdev, path + "/block", label, bdev_label_valid_locations);
  if (r < 0)
    return r;
  return 0;
}
...
int
BlueStore::_check_main_bdev_label()
{
  string block_path = path + "/block";
  int r = _read_multi_bdev_label(
      cct, bdev, block_path, fsid, &bdev_label, &bdev_label_valid_locations,
      &bdev_label_multi, &bdev_label_epoch);
  if (r < 0)
    return r;
  if (cct->_conf->bluestore_debug_permit_any_bdev_label) {
  ...
  if (bluestore_bdev_label_require_all && r != 0) {
    ...
    if (valid_locations > bdev_label_valid_locations.size()) {
      derr << __func__ << " not all labels read properly, " << valid_locations
           << "!=" << bdev_label_valid_locations.size() << dendl;
      return -EIO;
    }
  }
  return 0;
...
int
BlueStore::read_bdev_label_at_pos(
    CephContext* cct,
    ...
    int64_t* out_epoch)
{
  ...
  r = BlueStore::_read_multi_bdev_label(
      cct, bdev.get(), path, fsid, out_label, out_valid_positions, out_is_multi,
      out_epoch);
  bdev->close();
  return r;
}
...
void
BlueStore::_set_alloc_sizes(void)
{
...
int
BlueStore::_open_bdev(bool create)
{
  ceph_assert(bdev == NULL);
  string p = path + "/block";
  bdev = BlockDevice::create(
      cct, p, aio_cb, static_cast<void*>(this), discard_cb,
      static_cast<void*>(this), "bluestore");
  int r = 0;
  ...
  if (create && cct->_conf.get_val<bool>("bluestore_discard_on_mkfs")) {
  ...
  if (!create && cct->_conf->bluestore_use_ebd) {
    ...
    if (r == 0) {
      ...
      if (plugin_preload_r != 0) {
        ...
        derr << "Failed preloading extblkdev plugins, error code: "
             << plugin_preload_r << dendl;
      }
      string bdev_plugin_id;
      r = bdev->get_ebd_id(bdev_plugin_id);
      bool is_osd = cct->get_module_type() & CEPH_ENTITY_TYPE_OSD;
      if (r != 0) {
        derr << __func__ << " plugin " << meta_plugin_id << " not loaded"
             << dendl;
        if (is_osd) {
          goto fail_close;
        }
        ...
        if (meta_plugin_id != bdev_plugin_id) {
          derr << __func__ << " plugin '" << meta_plugin_id
               << "' used on mkfs, " << "but now uses plugin '"
               << bdev_plugin_id << "'" << dendl;
          if (is_osd) {
            goto fail_close;
          }
        }
    ...
    bool fm_restore)
{
  ...
  if (fm->get_size() >= bdev->get_size() + min_alloc_size) {
    ostringstream ss;
    ss << "slow device size mismatch detected, " << " fm size("
       << fm->get_size() << ") > slow device size(" << bdev->get_size()
...
int
BlueStore::_write_out_fm_meta(uint64_t target_size)
{
  int r = 0;
  string p = path + "/block";
...
BlueStore::_create_alloc()
{
  ...
  alloc = Allocator::create(
      cct, allocator_type, bdev->get_size(), alloc_size, "block");
  if (!alloc) {
...
int
BlueStore::_open_fsid(bool create)
{
  ceph_assert(fsid_fd < 0);
...
int
BlueStore::_read_fsid(uuid_d* uuid)
{
  char fsid_str[40];
...
bool
BlueStore::is_journal_rotational()
{
  if (!bluefs) {
...
bool
BlueStore::_use_rotational_settings()
{
...
bool
BlueStore::is_statfs_recoverable() const
{
  // abuse fm for now
...
int
BlueStore::_minimal_open_bluefs(bool create)
{
  int r;
  ...
  if (::stat(bfn.c_str(), &st) == 0) {
    ...
    if (bluefs->bdev_support_label(BlueFS::BDEV_DB)) {
      r = _check_or_set_bdev_label(
          bluefs->get_block_device(BlueFS::BDEV_DB), bfn, "bluefs db", create);
      if (r < 0) {
  ...
  r = bluefs->add_block_device(
      bluefs_layout.shared_bdev, bfn, false, &shared_alloc);
  if (r < 0) {
    derr << __func__ << " add block device(" << bfn
         << ") returned: " << cpp_strerror(r) << dendl;
    goto free_bluefs;
  ...
  if (::stat(bfn.c_str(), &st) == 0) {
    ...
    if (bluefs->bdev_support_label(BlueFS::BDEV_WAL)) {
      r = _check_or_set_bdev_label(
          bluefs->get_block_device(BlueFS::BDEV_WAL), bfn, "bluefs wal", create);
      if (r < 0) {
        derr << __func__ << " check block device(" << bfn
             << ") label returned: " << cpp_strerror(r) << dendl;
        goto free_bluefs;
...
int
BlueStore::_open_bluefs(bool create, bool read_only)
{
  ...
  if (bluefs_layout.shared_bdev == BlueFS::BDEV_SLOW ||
    ...
    r = RocksDBStore::ParseOptionsFromStringStatic(
        cct, options, rocks_opts, nullptr);
    if (r < 0) {
      return r;
    }
...
void
BlueStore::_minimal_close_bluefs()
{
  delete bluefs;
  bluefs = NULL;
}
...
int
BlueStore::_is_bluefs(bool create, bool* ret)
{
  if (create) {
...
*/
int
BlueStore::_open_db_and_around(bool read_only, bool to_repair)
{
...
int
BlueStore::open_db_environment(KeyValueDB** pdb, bool read_only, bool to_repair)
{
  _kv_only = true;
...
int
BlueStore::_prepare_db_environment(
    bool create,
    bool read_only,
    std::string* _fn,
    std::string* _kv_backend)
{
  ...
  if (do_bluefs) {
    ...
    if (cct->_conf->bluestore_bluefs_env_mirror) {
      ...
      if (create) {
        string cmd = "rm -rf " + path + "/db " + path + "/db.slow " + path +
                     "/db.wal";
  ...
  db = KeyValueDB::create(
      cct, kv_backend, fn, kv_options, static_cast<void*>(env));
  if (!db) {
...
int
BlueStore::_open_db(bool create, bool to_repair_db, bool read_only)
{
  int r;
...
BlueStore::_close_db()
{
  ...
  if (do_destage && fm && fm->is_null_manager()) {
    ...
    if (unlikely(ret != 0)) {
      derr << __func__ << "::NCB::store_allocator() failed (we will need to rebuild it on startup)"
           << dendl;
    }
  }
...
void
BlueStore::_fsck_collections(int64_t* errors)
{
...
int
BlueStore::_setup_block_symlink_or_file(
    string name,
    string epath,
    uint64_t size,
    bool create)
...
BlueStore::mkfs()
{
  ...
  if (cct->_conf->bluestore_bluefs) {
    r = _setup_block_symlink_or_file(
        "block.wal", cct->_conf->bluestore_block_wal_path,
        cct->_conf->bluestore_block_wal_size,
        cct->_conf->bluestore_block_wal_create);
    if (r < 0)
      goto out_close_fsid;
    r = _setup_block_symlink_or_file(
        "block.db", cct->_conf->bluestore_block_db_path,
  ...
  if (cct->_conf.get_val<bool>("bluestore_bdev_label_multi")) {
    ...
    for (size_t i = 1; i < bdev_label_positions.size(); i++) {
      ...
      if (location + size > bdev->get_size())
        continue;
...
int
BlueStore::add_new_bluefs_device(int id, const string& dev_path)
{
  ...
  if (id == BlueFS::BDEV_NEWWAL) {
    ...
    r = bluefs->add_block_device(
        BlueFS::BDEV_NEWWAL, p, cct->_conf->bdev_enable_discard);
    ceph_assert(r == 0);
    ...
    r = _setup_block_symlink_or_file(
        "block.db", dev_path, cct->_conf->bluestore_block_db_size, true);
    ceph_assert(r == 0);

    r = bluefs->add_block_device(
        BlueFS::BDEV_NEWDB, p, cct->_conf->bdev_enable_discard);
    ceph_assert(r == 0);

    if (bluefs->bdev_support_label(BlueFS::BDEV_NEWDB)) {
      r = _check_or_set_bdev_label(
          bluefs->get_block_device(BlueFS::BDEV_NEWDB), p, "bluefs db", true);
      ceph_assert(r == 0);
    }
    bluefs_layout.shared_bdev = BlueFS::BDEV_SLOW;
    bluefs_layout.dedicated_db = true;
...
int
BlueStore::migrate_to_existing_bluefs_device(const set<int>& devs_source, int id)
{
...
int
BlueStore::migrate_to_new_bluefs_device(
    ...
    const string& dev_path)
{
  ...
  if (id == BlueFS::BDEV_NEWWAL) {
    ...
    r = bluefs->add_block_device(
        BlueFS::BDEV_NEWWAL, dev_path, cct->_conf->bdev_enable_discard);
    ceph_assert(r == 0);
    ...
    r = bluefs->add_block_device(
        BlueFS::BDEV_NEWDB, dev_path, cct->_conf->bdev_enable_discard);
    ceph_assert(r == 0);

    if (bluefs->bdev_support_label(BlueFS::BDEV_NEWDB)) {
      r = _check_or_set_bdev_label(
          bluefs->get_block_device(BlueFS::BDEV_NEWDB), dev_path, "bluefs db",
          true);
      ceph_assert(r == 0);
    }
  }
...
int
BlueStore::expand_devices(ostream& out)
{
    ...
    if (my_bdev->supported_bdev_label()) {
      ...
      if (r < 0) {
        ...
        if (size == my_label.size) {
          // no need to expand
          out << devid << " : nothing to do, skipped" << std::endl;
          continue;
        } else if (size < my_label.size) {
          // something weird in bdev label
          out << devid << " : ERROR: bdev label is above device size, skipped"
              << std::endl;
          continue;
        } else {
          my_label.size = size;
          out << devid << " : Expanding to 0x" << std::hex << size << std::dec
              << "(" << byte_u_t(size) << ")" << std::endl;
          r = _write_bdev_label(cct, my_bdev, my_path, my_label);
          if (r < 0) {
            derr << "unable to write label for " << my_path << ": "
                 << cpp_strerror(r) << dendl;
          } else {
            out << devid << " : size updated to 0x" << std::hex << size
                << std::dec << "(" << byte_u_t(size) << ")" << std::endl;
          }
        }
      }
    }
  ...
  if (aligned_size == size0) {
    ...
    if (r != 0) {
      ...
      r = _write_bdev_label(
          cct, bdev, my_path, bdev_label, bdev_label_valid_locations);
      if (r != 0) {
    ...
    if (r == 0) {
      out << devid << " : size updated to 0x" << std::hex << size << std::dec
          << "(" << byte_u_t(size) << ")" << std::endl;
      _close_db_and_around();
...
bool
BlueStore::get_db_sharding(std::string& res_sharding)
{
  bool ret = false;
...
int
BlueStore::dump_bluefs_sizes(ostream& out)
{
  int r = _open_db_and_around(true);
  ceph_assert(r == 0);
  bluefs->dump_block_extents(out);
...
void
BlueStore::trim_free_space(const string& type, std::ostream& outss)
{
...
int
BlueStore::zap_device(CephContext* cct, const string& dev)
{
  string path = dev; // dummy var for dout
  uint64_t brush_size;
  dout(5) << __func__ << " " << dev << dendl;
  unique_ptr<BlockDevice> _bdev(
  ...
  for (auto off : bdev_label_positions) {
    ...
    if (end > off) {
      ...
      bufferlist bl;
      bl.append_zero(l);
      dout(10) << __func__ << " writing 0x" << std::hex << off << "~" << l
               << std::dec << " to " << dev << dendl;
      r = _bdev->write(off, bl, false);
      if (r < 0) {
        derr << __func__ << " error writing 0x" << std::hex << off << "~" << l
             << std::dec << " to " << dev << " : " << cpp_strerror(r) << dendl;
        break;
      }
    } else {
      break;
...
void
BlueStore::set_cache_shards(unsigned num)
{
...
//---------------------------------------------
bool
BlueStore::has_null_manager() const
{
  return (fm && fm->is_null_manager());
}

int
BlueStore::mount_readonly()
{
...
BlueStore::_mount()
{
  ...
  debug_extent_map_encode_check =
      cct->_conf.get_val<bool>("bluestore_debug_extent_map_encode_check");
  _kv_only = false;
...
void
BlueStore::_fsck_foreach_shared_blob(
    std::function<bool(coll_t, ghobject_t, uint64_t, const bluestore_blob_t&)> cb)
{
  ...
  if (it) {
    ...
    for (it->lower_bound(string()); it->valid(); it->next()) {
      ...
      OnodeRef o;
      o.reset(Onode::create_decode(
          c, oid, it->key(), it->value(), false, segment_size != 0));
...
void
BlueStore::_fsck_repair_shared_blobs(
    BlueStoreRepairer& repairer,
    ...
    BlueStore::FSCK_ObjectCtx& ctx)
{
  ...
  if (!o->extent_map.shards.empty()) {
    ...
    if (depth != FSCK_SHALLOW) {
      ...
      for (auto& s : o->extent_map.shards) {
        ...
        get_extent_shard_key(
            o->key, s.shard_info->offset, &expecting_shards->back());
  ...
  for (auto& l : o->extent_map.extent_map) {
    ...
    if (depth != FSCK_SHALLOW && blob.has_unused()) {
      ...
      uint64_t end = round_up_to(l.blob_offset + l.length, chunk_size) /
                     chunk_size;
  ...
  for (auto& i : ref_map) {
    ...
    if (blob.is_shared()) {
      ...
      ceph_assert(
          sbi.pool_id == sb_info_t::INVALID_POOL_ID ||
      ...
      errors += _fsck_sum_extents(
          blob.get_extents(), blob.is_compressed(), *res_statfs,
          *pool_fsck_stat);
    }
  } // for (auto& i : ref_map)

  {
    ...
    if (broken) {
      derr << "fsck error: " << oid << " - " << broken
           ...
           << dendl;
      if (repairer) {
        repairer->fix_spanning_blobs(db, [&](KeyValueDB::Transaction txn) {
          _record_onode(o, txn);
        });
      }
    }
...
public:
  ...
  void
  worker(ThreadPool::WorkThread* wt) override
  {
    int next_wq = 0;
    while (!_stop) {
      ...
      if (item) {
        processing++;
        TPHandle tp_handle(
            cct, nullptr, wq->timeout_interval.load(),
  ...
  struct FSCKWorkQueue : public ThreadPool::WorkQueue_ {
    ...
    void
    _void_process_finish(void*) override
    {
      ceph_abort();
    }
    ...
    void
    finalize(ThreadPool& tp, BlueStore::FSCK_ObjectCtx& ctx)
    {
      if (batch_acquired) {
        auto& batch = batches[last_batch_pos];
        ceph_assert(batch.running);
      ...
      for (size_t i = 0; i < batchCount; i++) {
        ...
        if (batch.entry_count) {
          TPHandle tp_handle(
              store->cct, nullptr, timeout_interval.load(),
...
void
BlueStore::_fsck_check_object_omap(
    FSCKDepth depth,
    OnodeRef& o,
...
void
BlueStore::_fsck_check_objects(FSCKDepth depth, BlueStore::FSCK_ObjectCtx& ctx)
{
  auto& errors = ctx.errors;
  auto sb_info_lock = ctx.sb_info_lock;
  auto& sb_info = ctx.sb_info;
  ...
  if (it) {
    ...
    std::unique_ptr<WQ> wq(new WQ(
        "FSCKWorkQueue", (thread_count ?: 1) * 32, this, sb_info_lock, sb_info,
        sb_ref_counts, repairer));

    ShallowFSCKThreadPool thread_pool(
        cct, "ShallowFSCKThreadPool", "ShallowFSCK", thread_count);
    ...
    for (it->lower_bound(string()); it->valid(); it->next()) {
      ...
      if (!queued) {
        ...
        o = fsck_check_objects_shallow(
            depth, pool_id, c, oid, it->key(), it->value(), &expecting_shards,
            &referenced, ctx);
      }
      ...
      if (depth != FSCK_SHALLOW) {
        ...
        for (auto& i : referenced) {
          ...
          if (blob.has_csum()) {
            ...
            for (unsigned p = 0; p < csum_count; ++p) {
              ...
              unsigned lastbit = (pos + csum_chunk_size - 1) /
                                 unused_chunk_size;
        ...
        if (depth == FSCK_DEEP) {
          ...
          do {
            ...
            int r = _do_read(
                c.get(), o, offset, l, bl, CEPH_OSD_OP_FLAG_FADVISE_NOCACHE);
            if (r < 0) {
              ++errors;
              derr << "fsck error: " << oid << std::hex
                   << " error during read: " << " " << offset << "~" << l << " "
                   << cpp_strerror(r) << std::dec << dendl;
              break;
            }
            offset += l;
    ...
    if (depth == FSCK_SHALLOW && thread_count > 0) {
      ...
      if (processed_myself) {
        ...
        dout(0) << __func__ << " partial offload" << ", done myself "
                << processed_myself << " of " << ctx.num_objects
                << "objects, threads " << thread_count << dendl;
      }
    }
  } // if (it)
}
...
*/
int
BlueStore::_fsck(BlueStore::FSCKDepth depth, bool repair)
{
...
int
BlueStore::_fsck_on_open(BlueStore::FSCKDepth depth, bool repair)
{
  ...
  if (bluefs) {
    ...
    bluefs->foreach_block_extents(
        bluefs_layout.shared_bdev, [&](uint64_t start, uint32_t len) {
          apply_for_bitset_range(
              start, len, alloc_size, used_blocks,
              [&](uint64_t pos, mempool_dynamic_bitset& bs) {
  ...
  if (bdev->supported_bdev_label() && bdev_label_multi) {
    ...
    for (uint64_t position : bdev_labels_broken) {
      ...
      apply_for_bitset_range(
          position, length, alloc_size, bluefs_used_blocks,
          [&](uint64_t pos, mempool_dynamic_bitset& bs) {
      ...
      for (it->lower_bound(string()); it->valid() && !bypass_rest; it->next()) {
        ...
        for (auto b : blobs) {
          ...
          for (auto e = pextents.begin(); e != pextents.end(); e++) {
            ...
            int64_t alloc_len =
                alloc->allocate(e->length, min_alloc_size, 0, -1, &exts);
            ...
            b->get_blob().map_bl(
                b_off_cur, bl, [&](uint64_t offset, bufferlist& t) {
  ...
  _fsck_check_statfs(
      expected_store_statfs, expected_pool_statfs, errors, warnings,
      repair ? &repairer : nullptr);
  if (depth != FSCK_SHALLOW) {
    ...
    if (!fm->is_null_manager()) {
      ...
      sum.insert(
          sum.end(), bdev_labels_in_repair.begin(), bdev_labels_in_repair.end());
            ...
            [&](uint64_t pos, mempool_dynamic_bitset& bs) {
              ...
              if (bs.test(pos) && !bluefs_used_blocks.test(pos)) {
                if (offset == SUPER_RESERVED &&
                  ...
                  intersects = true;
                  if (repair) {
                    repairer.fix_false_free(
                        db, fm, pos * min_alloc_size, min_alloc_size);
                  }
                }
              } else {
                bs.set(pos);
...
int
BlueStore::get_shared_blob(const string& key, bufferlist& bl)
{
...
void
BlueStore::inject_broken_shared_blob_key(const string& key, const bufferlist& bl)
{
...
void
BlueStore::inject_no_shared_blob_key()
{
  KeyValueDB::Transaction txn;
  txn = db->get_transaction();
...
void
BlueStore::inject_stray_shared_blob_key(uint64_t sbid)
{
  KeyValueDB::Transaction txn;
  txn = db->get_transaction();
...
void
BlueStore::inject_leaked(uint64_t len)
{
  PExtentVector exts;
  int64_t alloc_len =
      alloc->allocate(len, min_alloc_size, min_alloc_size * 256, -1, &exts);
...
void
BlueStore::inject_false_free(coll_t cid, ghobject_t oid)
{
...
void
BlueStore::inject_legacy_omap(coll_t cid, ghobject_t oid)
{
...
void
BlueStore::inject_stray_omap(uint64_t head, const string& name)
{
  dout(1) << __func__ << dendl;
  KeyValueDB::Transaction txn = db->get_transaction();
...
void
BlueStore::inject_statfs(const string& key, const store_statfs_t& new_statfs)
{
  BlueStoreRepairer repairer;
...
void
BlueStore::inject_global_statfs(const store_statfs_t& new_statfs)
{
  KeyValueDB::Transaction t = db->get_transaction();
  volatile_statfs v;
  v = new_statfs;
...
void
BlueStore::inject_zombie_spanning_blob(
    coll_t cid,
    ghobject_t oid,
    int16_t blob_id)
{
...
void
BlueStore::collect_metadata(map<string, string>* pm)
{
  dout(10) << __func__ << dendl;
  ...
  (*pm)["bluestore_onode_segmentation"] = segment_size == 0 ? "inactive"
                                                            : "active";
  std::string sharding;
...
void
BlueStore::prepare_for_fast_shutdown()
{
  m_fast_shutdown = true;
}

int
BlueStore::get_devices(set<string>* ls)
{
  if (bdev) {
    bdev->get_devices(ls);
    if (bluefs) {
...
void
BlueStore::_get_statfs_overall(struct store_statfs_t* buf)
{
  buf->reset();
  ...
  if (rc == 0) {
    ...
    buf->allocated = ebd_state.get_physical_total() -
                     ebd_state.get_physical_avail();
    ;
  } else {
...
int
BlueStore::statfs(struct store_statfs_t* buf, osd_alert_list_t* alerts)
{
  if (alerts) {
    alerts->clear();
    _log_alerts(*alerts);
...
void
BlueStore::_check_legacy_statfs_alert()
{
  string s;
...
void
BlueStore::_check_no_per_pg_or_pool_omap_alert()
{
...
BlueStore::CollectionRef
BlueStore::_get_collection(const coll_t& cid)
{
...
BlueStore::CollectionRef
BlueStore::_get_collection_by_oid(const ghobject_t& oid)
{
  std::shared_lock l(coll_lock);
...
void
BlueStore::_queue_reap_collection(CollectionRef& c)
{
...
void
BlueStore::refresh_perf_counters()
{
...
BlueStore::open_collection(const coll_t& cid)
{
...
void
BlueStore::set_collection_commit_queue(
    const coll_t& cid,
...
bool
BlueStore::exists(CollectionHandle& c_, const ghobject_t& oid)
{
...
int
BlueStore::set_collection_opts(CollectionHandle& ch, const pool_opts_t& opts)
{
  ...
  if (c->pool_opts.get(pool_opts_t::COMPRESSION_ALGORITHM, &val)) {
    ...
    if (alg.has_value() && *alg != Compressor::COMP_ALG_NONE) {
      ...
      if (!cp) {
        ...
        derr << __func__ << " unable to load compressor plugin for "
             << val.c_str() << dendl;
      } else {
  ...
  if (c->pool_opts.get(pool_opts_t::CSUM_TYPE, &ival)) {
    if (ival >= Checksummer::CSUM_MAX) {
      derr << __func__ << " unrecognized checksum type: " << ival << dendl;
    } else {
    ...
    uint32_t op_flags)
{
  ...
  int r;
  {
    ...
    OnodeRef o = c->get_onode(oid, false);
    log_latency(
        "get_onode@read", l_bluestore_read_onode_meta_lat,
...
out:
  if (r >= 0 && _debug_data_eio(oid)) {
    ...
    derr << __func__ << " " << c->cid << " " << oid << " INJECT EIO" << dendl;
      ...
      oid.hobj.pool > 0 && /* FIXME, see #23029 */
      cct->_conf->bluestore_debug_random_read_err &&
      (rand() % (int)(cct->_conf->bluestore_debug_random_read_err * 100.0)) ==
          0) {
  ...
  if (op_flags & CEPH_OSD_OP_FLAG_SCRUB) {
    log_latency_scrub(
        __func__, l_bluestore_read_lat, mono_clock::now() - start,
    ...
    blobs2read_t& blobs2read)
{
  ...
  while (left > 0 && lp != o->extent_map.extent_map.end()) {
    ...
    o->bc.read(
        o->c->cache, pos, b_len, cache_res, cache_interval, read_cache_policy);
      ...
      for (auto& req : r2r) {
        ...
        for (const auto& r : req.regs) {
          if (buffered) {
            ...
            o->bc.did_read(
                o->c->cache, r.logical_offset, std::move(region_buffer));
          }
    ...
    uint64_t retry_count)
{
  ...
  if (op_flags & CEPH_OSD_OP_FLAG_FADVISE_WILLNEED) {
    ...
    buffered = true;
      ...
      cct->_conf->bluestore_default_buffered_read &&
  ...
  log_latency(
      __func__, l_bluestore_read_onode_meta_lat, mono_clock::now() - start,
      cct->_conf->bluestore_log_op_age, "",
      l_bluestore_slow_read_onode_meta_count);
  _dump_onode<30>(cct, *o);
  ...
  r = _generate_read_result_bl(
      o, offset, length, ready_regions, compressed_blob_bls, blobs2read,
    ...
    uint64_t logical_offset)
{
  ...
  log_latency(
      __func__, l_bluestore_csum_lat, mono_clock::now() - start,
      cct->_conf->bluestore_log_op_age);
...
int
BlueStore::_decompress(bufferlist& source, bufferlist* result)
{
  int r = 0;
  auto start = mono_clock::now();
  ...
  CompressorRef cp = alg < int(compressors.size()) ? compressors[alg]
                                                   : CompressorRef();
  if (!cp.get()) {
  ...
  log_latency(
      __func__, l_bluestore_decompress_lat, mono_clock::now() - start,
      cct->_conf->bluestore_log_op_age);
  return r;
}
    ...
    uint64_t retry_count)
{
  ...
  o->extent_map.fault_range(
      db, m.range_start(), m.range_end() - m.range_start());
  log_latency(
      __func__, l_bluestore_read_onode_meta_lat, mono_clock::now() - start,
      cct->_conf->bluestore_log_op_age, "",
      l_bluestore_slow_read_onode_meta_count);
  ...
  for (auto p = m.begin(); p != m.end(); p++, i++) {
    ...
    _read_cache(
        o, p.get_start(), p.get_len(), read_cache_policy,
        std::get<0>(raw_results[i]), std::get<2>(raw_results[i]));
    r = _prepare_read_ioc(
        std::get<2>(raw_results[i]), &std::get<1>(raw_results[i]), &ioc);
    // we always issue aio for reading, so errors other than EIO are not allowed
    if (r < 0)
      return r;
  }
...
int
BlueStore::list_collections(vector<coll_t>& ls)
{
  std::shared_lock l(coll_lock);
...
bool
BlueStore::collection_exists(const coll_t& c)
{
  std::shared_lock l(coll_lock);
...
int
BlueStore::collection_empty(CollectionHandle& ch, bool* empty)
{
...
int
BlueStore::collection_bits(CollectionHandle& ch)
{
  dout(15) << __func__ << " " << ch->cid << dendl;
  Collection* c = static_cast<Collection*>(ch.get());
  std::shared_lock l(c->lock);
...
int
BlueStore::collection_list_legacy(
    CollectionHandle& c_,
    const ghobject_t& start,
    const ghobject_t& end,
    int max,
    ...
    ghobject_t* pnext)
{
  ...
  auto log_latency = make_scope_guard([&, start_time = mono_clock::now(),
                                       func_name = __func__] {
    log_latency_fn(
  ...
  get_coll_range(
      c->cid, c->cnode.bits, &coll_range_temp_start, &coll_range_temp_end,
      &coll_range_start, &coll_range_end, legacy);
  ...
  if ((start == ghobject_t() || start.hobj == hobject_t() ||
    ...
    ranges.push_back(std::tuple(
        std::move(coll_range_temp_start), std::move(coll_range_temp_end)));
  }
...
out:
  c->store->log_latency(
      __func__, l_bluestore_omap_get_values_lat, mono_clock::now() - start1,
...
int
BlueStore::omap_iterate(
    CollectionHandle& c_, ///< [in] collection
    const ghobject_t& oid, ///< [in] object
    ObjectStore::omap_iter_seek_t
        start_from, ///< [in] where the iterator should point to at the beginning
    std::function<omap_iter_ret_t(std::string_view, std::string_view)> f)
{
  ...
  // seek the iterator
  {
    ...
    if (start_from.seek_type == omap_iter_seek_t::LOWER_BOUND) {
      ...
      c->store->log_latency(
          __func__, l_bluestore_omap_lower_bound_lat,
          ceph::mono_clock::now() - start,
          c->store->cct->_conf->bluestore_log_omap_iterator_age);
    } else {
      it->upper_bound(seek_key);
      c->store->log_latency(
          __func__, l_bluestore_omap_upper_bound_lat,
          ceph::mono_clock::now() - start,
          c->store->cct->_conf->bluestore_log_omap_iterator_age);
    }
  }
  ...
  c->store->log_latency(
      __func__, l_bluestore_omap_next_lat, next_lat_acc,
      c->store->cct->_conf->bluestore_log_omap_iterator_age);
  return more;
}
...
uint64_t
BlueStore::_get_ondisk_reserved() const
{
...
void
BlueStore::_prepare_ondisk_format_super(KeyValueDB::Transaction& t)
{
...
void
BlueStore::_assign_nid(TransContext* txc, OnodeRef& o)
{
  if (o->onode.nid) {
    ceph_assert(o->exists);
    return;
...
uint64_t
BlueStore::_assign_blobid(TransContext* txc)
{
...
void
BlueStore::get_db_statistics(Formatter* f)
{
  db->get_statistics(f);
}
...
void
BlueStore::_txc_calc_cost(TransContext* txc)
{
...
void
BlueStore::_txc_update_store_statfs(TransContext* txc)
{
  ...
  logger->inc(
      l_bluestore_compressed_original, txc->statfs_delta.compressed_original());
...
BlueStore::_txc_state_proc(TransContext* txc)
{
  ...
  while (true) {
    ...
    case TransContext::STATE_DEFERRED_CLEANUP:
      throttle.log_state_latency(
          *txc, logger, l_bluestore_state_deferred_cleanup_lat);
      txc->set_state(TransContext::STATE_FINISHING);
      // ** fall-thru **
...
void
BlueStore::_txc_finish_io(TransContext* txc)
{
...
void
BlueStore::_txc_write_nodes(TransContext* txc, KeyValueDB::Transaction t)
{
  ...
  for (auto o : txc->onodes) {
    _record_onode(o, t);
    int16_t spanning_change = o->extent_map.spanning_blob_map.size() -
                              o->prev_spanning_cnt;
...
void
BlueStore::BSPerfTracker::update_from_perfcounters(PerfCounters& logger)
{
...
void
BlueStore::_txc_finalize_kv(TransContext* txc, KeyValueDB::Transaction t)
{
...
void
BlueStore::_txc_apply_kv(TransContext* txc, bool sync_submit_transaction)
{
  ceph_assert(txc->get_state() == TransContext::STATE_KV_QUEUED);
  {
    ...
    if (txc->tracing) {
      tracepoint(
          bluestore, transaction_kv_submit_latency,
          txc->osr->get_sequencer_id(), (uint64_t)txc, sync_submit_transaction,
...
void
BlueStore::_txc_committed_kv(TransContext* txc)
{
...
void
BlueStore::_txc_finish(TransContext* txc)
{
...
void
BlueStore::_osr_attach(Collection* c)
{
...
void
BlueStore::_osr_register_zombie(OpSequencer* osr)
{
  std::lock_guard l(zombie_osr_lock);
...
void
BlueStore::_osr_drain_preceding(TransContext* txc)
{
  OpSequencer* osr = txc->osr.get();
...
void
BlueStore::_osr_drain(OpSequencer* osr)
{
    ...
    if (kv_queue.empty() &&
      ...
      if (force_flush) {
        ...
        if (deferred_stable.empty()) {
          ...
          deferred_stable.insert(
              deferred_stable.end(), deferred_done.begin(), deferred_done.end());
          deferred_done.clear();
        }
      }
      ...
      for (auto txc : kv_committing) {
        throttle.log_state_latency(
            *txc, logger, l_bluestore_state_kv_queued_lat);
      ...
      int r = db_was_opened_read_only ||
                      cct->_conf->bluestore_debug_omit_kv_commit
                  ? 0
      ...
      for (auto txc : kv_committing) {
        if (txc->tracing) {
          tracepoint(
              bluestore, transaction_kv_sync_latency,
              txc->osr->get_sequencer_id(), (uint64_t)txc, kv_committing.size(),
              deferred_done.size(), deferred_stable.size(), sync_latency);
        }
      }
#endif

      {
        ...
        if (kv_committing_to_finalize.empty()) {
          ...
          kv_committing_to_finalize.insert(
              kv_committing_to_finalize.end(), kv_committing.begin(),
              kv_committing.end());
          kv_committing.clear();
        }
        if (deferred_stable_to_finalize.empty()) {
          ...
          deferred_stable_to_finalize.insert(
              deferred_stable_to_finalize.end(), deferred_stable.begin(),
              deferred_stable.end());
...

      {
        ...
        log_latency(
            "kv_flush", l_bluestore_kv_flush_lat, dur_flush,
            cct->_conf->bluestore_log_op_age);
        log_latency(
            "kv_commit", l_bluestore_kv_commit_lat, dur_kv,
            cct->_conf->bluestore_log_op_age);
        log_latency(
            "kv_sync", l_bluestore_kv_sync_lat, dur,
            cct->_conf->bluestore_log_op_age);
      }
...
BlueStore::_kv_finalize_thread()
{
  ...
  while (true) {
    ...
    if (kv_committing_to_finalize.empty() &&
      ...
      _reap_collections();
      log_latency(
          "kv_final", l_bluestore_kv_final_lat, mono_clock::now() - start,
          cct->_conf->bluestore_log_op_age);
...
BlueStore::_get_deferred_op(TransContext* txc, uint64_t len)
{
  if (!txc->deferred_txn) {
...
void
BlueStore::_deferred_queue(TransContext* txc)
{
...
void
BlueStore::_deferred_submit_unlock(OpSequencer* osr)
{
  ...
  for (auto& txc : b->txcs) {
    throttle.log_state_latency(
        txc, logger, l_bluestore_state_deferred_queued_lat);
  }
...
void
BlueStore::_deferred_aio_finish(OpSequencer* osr)
{
...

  {
    uint64_t costs = 0;
    {
      for (auto& i : b->txcs) {
        TransContext* txc = &i;
        throttle.log_state_latency(
            *txc, logger, l_bluestore_state_deferred_aio_wait_lat);
...
bool
BlueStore::_eliminate_outdated_deferred(
    bluestore_deferred_transaction_t* deferred_txn,
    interval_set<uint64_t>& bluefs_extents)
{
  bool has_some = false;
  ...
  while (it != deferred_txn->ops.end()) {
    ...
    for (auto& e : it->extents) {
      ...
      for (auto ki = region.begin(); ki != region.end(); ki++) {
        ...
        chunk.substr_of(
            it->data, data_offset + (ki.get_start() - e.offset), ki.get_len());
    ...
    ThreadPool::TPHandle* handle)
{
  ...
  log_latency(
      "submit_transact", l_bluestore_submit_lat, mono_clock::now() - start,
      cct->_conf->bluestore_log_op_age);
  log_latency(
      "throttle_transact", l_bluestore_throttle_lat, tend - tstart,
      cct->_conf->bluestore_log_op_age);
  return 0;
}

void
BlueStore::_txc_aio_submit(TransContext* txc)
{
...
BlueStore::_txc_add_transaction(TransContext* txc, Transaction* t)
{
  ...
  for (int pos = 0; i.have_op(); ++pos) {
    ...
    if (!!c ? c->cid.is_pg(&pgid) : false) {
      ceph_assert(
          txc->osd_pool_id == META_POOL_ID || txc->osd_pool_id == pgid.pool());
    ...
    case Transaction::OP_SETALLOCHINT: {
      r = _set_alloc_hint(
          txc, c, o, op->expected_object_size, op->expected_write_size,
          op->hint);
    } break;
...
int
BlueStore::_touch(TransContext* txc, CollectionRef& c, OnodeRef& o)
{
...
void
BlueStore::_pad_zeros(bufferlist* bl, uint64_t* offset, uint64_t chunk_size)
{
    ...
    WriteContext* wctx)
{
    ...
    if (ep != end && ep->logical_offset < offset + max_bsize) {
      ...
      if (bstart >= end_offs) {
        ...
        dout(20) << __func__ << " ignoring immutable " << *b << dendl;
          ...
          ep->logical_offset % min_alloc_size !=
        ...
        if ((b_off % chunk_size == 0 && b_len % chunk_size == 0) &&
          ...
          _buffer_cache_write(
              txc, o, offset, bl, wctx->buffered ? 0 : Buffer::FLAG_NOCACHE);
        ...
        if (b->get_blob().get_ondisk_length() >= b_off + b_len &&
          ...
          if (head_read) {
            bufferlist head_bl;
            int r = _do_read(
                c.get(), o, offset - head_pad - head_read, head_read, head_bl,
                0);
          ...
          if (tail_read) {
            bufferlist tail_bl;
            int r = _do_read(
                c.get(), o, offset + length + tail_pad, tail_read, tail_bl, 0);
          ...
          _buffer_cache_write(
              txc, o, offset - head_read - head_pad, bl,
        ...
        if (b->can_reuse_blob(
                min_alloc_size, max_bsize, offset0 - bstart, &alloc_len)) {
          ...
          // do_alloc_write().
          if (!wctx->has_conflict(
                  b, offset0, offset0 + alloc_len, min_alloc_size)) {
            ...
            if (!cct->_conf->bluestore_zero_block_detection || !bl.is_zero()) {
              ...
              dout(20) << __func__ << " reuse blob " << *b << std::hex << " (0x"
                       << b_off0 << "~" << bl.length() << ")" << " (0x" << b_off
                       << "~" << length << ")" << std::dec << dendl;

              wctx->write(
                  offset, b, alloc_len, b_off0, bl, b_off, length, false, false);
              logger->inc(l_bluestore_write_small_unused);
            } else { // if (bl.is_zero())
      ...
      if (b->can_reuse_blob(
        ...
        if (!wctx->has_conflict(
          ...
          if (!cct->_conf->bluestore_zero_block_detection || !bl.is_zero()) {
            ...
            dout(20) << __func__ << " skip small zero block " << std::hex
                     << " (0x" << b_off0 << "~" << bl.length() << ")" << " (0x"
                     << b_off << "~" << length << ")" << std::dec << dendl;
            logger->inc(l_bluestore_write_small_skipped);
            logger->inc(l_bluestore_write_small_skipped_bytes, length);
          }
  ...
  if (!cct->_conf->bluestore_zero_block_detection || !bl.is_zero()) {
    ...
    wctx->write(
        offset, b, alloc_len, b_off0, bl, b_off, length,
        min_alloc_size !=
...
bool
BlueStore::BigDeferredWriteContext::can_defer(
...
bool
BlueStore::BigDeferredWriteContext::apply_defer()
{
...
void
BlueStore::_do_write_big_apply_deferred(
    TransContext* txc,
    CollectionRef& c,
    OnodeRef& o,
    ...
    WriteContext* wctx)
{
  ...
  _buffer_cache_write(
      txc, o, dctx.off - dctx.head_read, bl,
      wctx->buffered ? 0 : Buffer::FLAG_NOCACHE);

  b0->dirty_blob().calc_csum(dctx.b_off, bl);

  Extent* le = o->extent_map.set_lextent(
      c, dctx.off, dctx.off - dctx.blob_start, dctx.used, b0,
      &wctx->old_extents);
  ...
  while (length > 0) {
    ...
    if (!wctx->compress) {
      ...
      if (prefer_deferred_size_snapshot &&
        ...
        bool will_defer = ep != end ? head_info.can_defer(
                                          ep, prefer_deferred_size_snapshot,
                                          block_size, offset, l)
                                    : false;
      ...
      do {
        ...
        if (prev_ep != end && prev_ep->logical_offset >= min_off) {
          ...
          if (prev_ep->blob->can_reuse_blob(
                  min_alloc_size, max_bsize, offset - prev_ep->blob_start(),
                  &l)) {
            b = prev_ep->blob;
    ...
    WriteContext* wctx)
{
  ...
  for (auto& wi : wctx->writes) {
    if (wctx->compressor && wi.blob_length > min_alloc_size) {
      ...
      if (r == 0 && result_len <= want_len && result_len < wi.blob_length) {
        ...
        if (result_len <= want_len && result_len < wi.blob_length) {
          ...
          dout(20) << __func__ << std::hex << "  compressed 0x"
                   ...
                   << result_len << " with " << wctx->compressor->get_type()
                   << std::dec << dendl;
        ...
        dout(5) << __func__ << std::hex << "  0x" << wi.blob_length
                << " bytes compressed using "
                << wctx->compressor->get_type_name() << std::dec
      ...
      log_latency(
          "compress@_do_alloc_write", l_bluestore_compress_lat,
  ...
  prealloc_left = alloc->allocate(
      need, min_alloc_size, need, use_last_allocator_lookup_position ? -1 : 0,
      &prealloc);
  log_latency(
      "allocator@_do_alloc_write", l_bluestore_allocator_lat,
      mono_clock::now() - start, cct->_conf->bluestore_log_op_age);
  ...
  for (auto& wi : wctx->writes) {
    ...
    if (wi.compressed) {
      ...
      uint32_t suggested_boff = (wi.logical_offset - (wi.b_off0 - wi.b_off)) %
                                max_bsize;
    ...
    Extent* le = o->extent_map.set_lextent(
        coll, wi.logical_offset, b_off + (wi.b_off0 - wi.b_off), wi.length0,
        wi.b, nullptr);
    ...
    _buffer_cache_write(
        txc, o, wi.logical_offset, std::move(without_pad),
        wctx->buffered ? 0 : Buffer::FLAG_NOCACHE);
  ...
  while (oep != wctx->old_extents.end()) {
    ...
    if (!r.empty()) {
      ...
      if (blob.is_shared()) {
        ...
        bool unshare = false;
        bool* unshare_ptr = !maybe_unshared_blobs || b->is_referenced()
                                ? nullptr
                                : &unshare;
        for (auto e : r) {
  ...
  if (offset / min_alloc_size == (end - 1) / min_alloc_size &&
    ...
    if (segment_size) {
      ...
      while (write_offset < middle_offset + middle_length) {
        ...
        _do_write_big(
            txc, c, o, write_offset, segment_end - write_offset, p, wctx);
...
void
BlueStore::_choose_write_options(
    CollectionRef& c,
    OnodeRef& o,
    ...
    WriteContext* wctx)
{
  if (fadvise_flags & CEPH_OSD_OP_FLAG_FADVISE_WILLNEED) {
    ...
    wctx->buffered = true;
      ...
      cct->_conf->bluestore_default_buffered_write &&
  ...
  // checksum
  wctx->csum_type = c->csum_type.has_value() ? *(c->csum_type)
                                             : csum_type.load();

  // compression parameters
  unsigned alloc_hints = o->onode.alloc_hint_flags;

  auto cm = c->compression_mode.has_value() ? *(c->compression_mode)
                                            : comp_mode.load();

  wctx->compress = (cm != Compressor::COMP_NONE) &&
                   ((cm == Compressor::COMP_FORCE) ||
                    (cm == Compressor::COMP_AGGRESSIVE &&
                     (alloc_hints & CEPH_OSD_ALLOC_HINT_FLAG_INCOMPRESSIBLE) ==
                         0) ||
    ...
    uint32_t fadvise_flags)
{
  ...
  dout(20) << __func__ << " " << o->oid << " 0x" << std::hex << offset << "~"
           ...
           << o->onode.size << ")" << " bytes" << std::hex
           << " fadvise_flags 0x" << fadvise_flags << " alloc_hint 0x"
           << o->onode.alloc_hint_flags << " expected_object_size "
           << o->onode.expected_object_size << " expected_write_size "
           << o->onode.expected_write_size << std::dec << dendl;
  _dump_onode<30>(cct, *o);
  ...
  if (wctx.extents_to_gc.empty() || wctx.extents_to_gc.range_start() > offset ||
    ...
    benefit = gc.estimate(
        offset, length, o->extent_map, wctx.old_extents, min_alloc_size);
  }
  ...
  if (wctx.compressor) {
    ...
    if (segment_size) {
      ...
      while (write_offset != end) {
        ...
        _do_write_v2_compressed(
            txc, c, o, wctx, write_offset, write_length, chunk,
      ...
      _do_write_v2_compressed(
          txc, c, o, wctx, offset, length, bl, scan_left, scan_right);
    }
  } else {
    ...
    o->extent_map.dirty_range(
        wr.left_affected_range,
...
int
BlueStore::_do_write_v2_compressed(
    TransContext* txc,
    CollectionRef& c,
    OnodeRef& o,
    WriteContext& wctx,
    ...
    uint32_t scan_right)
{
  ...
  scanner.write_lookaround(
      o.get(), offset, length, scan_left, scan_right, estimator);
  ...
  *_dout << std::dec << dendl;
  for (const auto& i : regions) {
    ...
    if (i.offset <= offset && offset < i.offset + i.length) {
      ...
      ceph_assert(
          offset + length < i.offset || offset + length >= i.offset + i.length);
    ...
    disk_for_raw = p2roundup(i.offset + i.length, au_size) -
                   p2align(i.offset, au_size);
    BlueStore::Writer wr(this, txc, &wctx, o);
...
int
BlueStore::_do_remove(TransContext* txc, CollectionRef& c, OnodeRef& o)
{
  set<SharedBlob*> maybe_unshared_blobs;
  ...
  for (auto& e : h->extent_map.extent_map) {
    ...
    SharedBlob* sb = e.blob->get_shared_blob().get();
    if (b.is_shared() &&
        std::find(unshared_blobs.begin(), unshared_blobs.end(), sb) !=
            unshared_blobs.end()) {
...
int
BlueStore::_remove(TransContext* txc, CollectionRef& c, OnodeRef& o)
{
...
int
BlueStore::_rmattrs(TransContext* txc, CollectionRef& c, OnodeRef& o)
{
  dout(15) << __func__ << " " << c->cid << " " << o->oid << dendl;
  int r = 0;
...
void
BlueStore::_do_omap_clear(TransContext* txc, OnodeRef& o)
{
...
int
BlueStore::_omap_clear(TransContext* txc, CollectionRef& c, OnodeRef& o)
{
  dout(15) << __func__ << " " << c->cid << " " << o->oid << dendl;
  auto t0 = mono_clock::now();
    ...
    CollectionRef* c)
{
  ...
  (*c)->flush_all_but_last();
  {
    ...
    r = _collection_list(
        c->get(), ghobject_t(), ghobject_t::get_max(), nonexistent_count + 1,
        false, &ls, &next);
    if (r >= 0) {
...
void
BlueStore::_do_remove_collection(TransContext* txc, CollectionRef* c)
{
    ...
    int rem)
{
  ...
  dout(10) << __func__ << " " << c->cid << " to " << d->cid << " " << " bits "
           << bits << " = " << r << dendl;
  return r;
}

int
BlueStore::_merge_collection(
    TransContext* txc,
    CollectionRef* c,
    CollectionRef& d,
    unsigned bits)
...
BlueStore::_trim_slow_op_event_queue(mono_clock::time_point cur_time)
{
  ...
  while (!slow_op_event_queue.empty() &&
         ((slow_op_event_queue.front().first < cur_time - warn_duration) ||
          ...
          (slow_scrub_op_event_count >
           cct->_conf->bluestore_slow_scrub_ops_warn_threshold))) {
...
void
BlueStore::_add_slow_scrub_op_event()
{
...
void
BlueStore::log_latency_fn_scrub(
    const char* name,
    int idx,
    const ceph::timespan& l,
    double lat_threshold,
...
#if defined(WITH_LTTNG)
void
BlueStore::BlueStoreThrottle::emit_initial_tracepoint(
    KeyValueDB& db,
    TransContext& txc,
    mono_clock::time_point start_throttle_acquire)
{
  ...
  if (should_trace(&started, &completed)) {
    ...
    db.get_property(
        "rocksdb.cur-size-all-mem-tables", &rocksdb_cur_size_all_mem_tables);
    ...
    db.get_property(
        "rocksdb.num-running-compactions", &rocksdb_num_running_compactions);
    db.get_property("rocksdb.num-running-flushes", &rocksdb_num_running_flushes);
    db.get_property(
        "rocksdb.actual-delayed-write-rate", &rocksdb_actual_delayed_write_rate);
    ...
    tracepoint(
        bluestore, transaction_initial_state_rocksdb,
    ...
    int state)
{
  ...
  if (txc.tracing && state >= l_bluestore_state_prepare_lat &&
    ...
    tracepoint(
        bluestore, transaction_state_duration, txc.osr->get_sequencer_id(),
        (uint64_t)&txc, state, ceph::to_seconds<double>(lat));
  }
#endif
  txc.last_stamp = now;
  return lat;
...
bool
BlueStore::BlueStoreThrottle::try_start_transaction(
    KeyValueDB& db,
    TransContext& txc,
    mono_clock::time_point start_throttle_acquire)
{
...
void
BlueStore::BlueStoreThrottle::finish_start_transaction(
    KeyValueDB& db,
    TransContext& txc,
    mono_clock::time_point start_throttle_acquire)
{
...
#if defined(WITH_LTTNG)
void
BlueStore::BlueStoreThrottle::complete_kv(TransContext& txc)
{
  pending_kv_ios -= 1;
  ios_completed_since_last_traced++;
  if (txc.tracing) {
    tracepoint(
        bluestore, transaction_commit_latency, txc.osr->get_sequencer_id(),
        (uint64_t)&txc,
...
#if defined(WITH_LTTNG)
void
BlueStore::BlueStoreThrottle::complete(TransContext& txc)
{
  if (txc.deferred_txn) {
  ...
  if (txc.tracing) {
    ...
    tracepoint(
        bluestore, transaction_total_duration, txc.osr->get_sequencer_id(),
        (uint64_t)&txc, ceph::to_seconds<double>(lat));
  }
}
#endif
...
void
BlueStore::generate_db_histogram(Formatter* f)
{
  //globals
  uint64_t num_onodes = 0;
  ...
  while (iter->valid()) {
    ...
    if (key.first == PREFIX_SUPER) {
      ...
      if (key.second.back() == ONODE_KEY_SUFFIX) {
        hist.update_hist_entry(
            hist.key_hist, prefix_onode, key_size, value_size);
        num_onodes++;
      } else {
        hist.update_hist_entry(
            hist.key_hist, prefix_onode_shard, key_size, value_size);
        num_shards++;
      }
      ...
      hist.update_hist_entry(
          hist.key_hist, PREFIX_PERPOOL_OMAP, key_size, value_size);
      num_omap++;
    } else if (key.first == PREFIX_PERPG_OMAP) {
      hist.update_hist_entry(
          hist.key_hist, PREFIX_PERPG_OMAP, key_size, value_size);
      num_omap++;
    } else if (key.first == PREFIX_PGMETA_OMAP) {
      hist.update_hist_entry(
          hist.key_hist, PREFIX_PGMETA_OMAP, key_size, value_size);
      num_pgmeta_omap++;
    } else if (key.first == PREFIX_DEFERRED) {
      hist.update_hist_entry(
          hist.key_hist, PREFIX_DEFERRED, key_size, value_size);
      num_deferred++;
      ...
      hist.update_hist_entry(
          hist.key_hist, PREFIX_SHARED_BLOB, key_size, value_size);
      num_shared_shards++;
    } else {
...
int
BlueStore::flush_cache(ostream* os)
{
  dout(10) << __func__ << dendl;
  for (auto i : onode_cache_shards) {
    i->flush();
...
void
BlueStore::_record_onode(OnodeRef& o, KeyValueDB::Transaction& txn)
{
  ...
  // bound encode
  size_t bound = 0;
  uint64_t flag = segment_size != 0 ? 0
                                    : bluestore_onode_t::FLAG_DEBUG_FORCE_V2;
...
void
BlueStore::_log_alerts(osd_alert_list_t& alerts)
{
  std::lock_guard l(qlock);
  ...
  if (bluefs) {
    ...
    if (main_size > 0) {
      ...
      if (ratio > warn_ratio) {
        ostringstream ss;
        ss << "BlueFS usage (" << byte_u_t(total_bluefs_usage) << ") exceeds "
           << std::fixed << std::setprecision(4) << (warn_ratio * 100.0)
           << "% of main device (" << byte_u_t(main_size) << ", " << std::fixed
...
void
BlueStore::_collect_allocation_stats(
    uint64_t need,
    uint32_t alloc_size,
    const PExtentVector& extents)
{
...
size_t
BlueStoreRepairer::StoreSpaceTracker::filter_out(
    const interval_set<uint64_t>& extents)
{
  ...
  for (auto e : extents) {
    ...
    while (pos != npos && pos < end_pos) {
      ceph_assert(
          collections_bfs[pos].element_count() ==
...
void
BlueStoreRepairer::fix_per_pool_omap(KeyValueDB* db, int val)
{
...
bool
BlueStoreRepairer::fix_shared_blob(
    KeyValueDB::Transaction txn,
    uint64_t sbid,
...
bool
BlueStoreRepairer::fix_false_free(
    KeyValueDB* db,
    FreelistManager* fm,
    uint64_t offset,
    uint64_t len)
...
bool
BlueStoreRepairer::fix_spanning_blobs(
    KeyValueDB* db,
...
bool
BlueStoreRepairer::preprocess_misreference(KeyValueDB* db)
{
...
unsigned
BlueStoreRepairer::apply(KeyValueDB* db)
{
  //NB: not for use in multithreading mode!!!
...
struct allocator_image_header {
  ...
  friend std::ostream&
  operator<<(std::ostream& out, const allocator_image_header& header)
  {
  ...
  int
  verify(CephContext* cct, const std::string& path)
  {
...
struct allocator_image_trailer {
  ...
  friend std::ostream&
  operator<<(std::ostream& out, const allocator_image_trailer& trailer)
  {
  ...
  verify(
      CephContext* cct,
      const std::string& path,
      const allocator_image_header* p_header,
      uint64_t entries_count,
      uint64_t allocation_size)
  {
    if (valid_signature == ALLOCATOR_IMAGE_VALID_SIGNATURE) {
...
int
BlueStore::invalidate_allocation_file_on_bluefs()
{
...
//-----------------------------------------------------------------------------------
static uint32_t
flush_extent_buffer_with_crc(
    BlueFS::FileWriter* p_handle,
    const char* buffer,
    const char* p_curr,
    uint32_t crc)
{
...
//-----------------------------------------------------------------------------------
int
BlueStore::store_allocator(Allocator* src_allocator)
{
  ...
  BlueFS::FileWriter* p_handle = nullptr;
  ret = bluefs->open_for_write(
      allocator_dir, allocator_file, &p_handle, overwrite_file);
  if (ret != 0) {
  ...
  auto iterated_allocation = [&](uint64_t extent_offset,
    ...
    if (p_curr == p_end) {
      crc = flush_extent_buffer_with_crc(
          p_handle, (const char*)buffer, (const char*)p_curr, crc);
...

  {
    allocator_image_trailer trailer(
        timestamp, s_format_version, s_serial, extent_count, allocation_size);
    ...
    uint64_t* bytes)
{
  ...
  int ret = bluefs->open_for_read(
      allocator_dir, allocator_file, &p_temp_handle, false);
  if (ret != 0) {
  ...
  uint64_t extent_count = 0;
  uint64_t extents_bytes_left = file_size -
                                (header_size + trailer_size + sizeof(crc));
...
//-----------------------------------------------------------------------------------
void
BlueStore::set_allocation_in_simple_bmap(
    SimpleBitmap* sbmap,
    uint64_t offset,
    uint64_t length)
{
...
void
BlueStore::ExtentDecoderPartial::_consume_new_blob(
    bool spanning,
    uint64_t extent_no,
    uint64_t sbid,
    BlobRef b)
...
void
BlueStore::ExtentDecoderPartial::consume_blobid(
    Extent* le,
    bool spanning,
    uint64_t blobid)
{
...
void
BlueStore::ExtentDecoderPartial::consume_blob(
    Extent* le,
    uint64_t extent_no,
    uint64_t sbid,
    BlobRef b)
...
void
BlueStore::ExtentDecoderPartial::consume_spanning_blob(uint64_t sbid, BlobRef b)
{
  _consume_new_blob(true, 0 /*doesn't matter*/, sbid, b);
}

void
BlueStore::ExtentDecoderPartial::reset(
...
int
BlueStore::read_allocation_from_onodes(
    SimpleBitmap* sbmap,
    read_alloc_stats_t& stats)
{
  ...
  ExtentDecoderPartial edecoder(
      *this, stats, *sbmap, sb_info, min_alloc_size_order);
  ...
  for (it->lower_bound(string()); it->valid(); it->next(), kv_count++) {
    ...
    if (!is_extent_shard_key(it->key())) {
      ...
      edecoder.reset(
          oid, &stats.actual_pool_vstatfs[oid.hobj.get_logical_pool()]);
      Onode dummy_on(cct);
...
//---------------------------------------------------------
int
BlueStore::reconstruct_allocations(SimpleBitmap* sbmap, read_alloc_stats_t& stats)
{
...
//-----------------------------------------------------------------------------------
static void
copy_simple_bitmap_to_allocator(
    SimpleBitmap* sbmap,
    Allocator* dest_alloc,
    uint64_t alloc_size)
{
  ...
  while (ext.length != 0) {
    dest_alloc->init_add_free(
        ext.offset << alloc_size_shift, ext.length << alloc_size_shift);
...
//---------------------------------------------------------
int
BlueStore::add_existing_bluefs_allocation(
    Allocator* allocator,
    read_alloc_stats_t& stats)
{
...
BlueStore::read_allocation_from_drive_for_bluestore_tool()
{
...

  {
    ...
    ret = compare_allocators(
        allocator.get(), alloc, stats.insert_count, memory_target);
    if (ret == 0) {
...
//---------------------------------------------------------
Allocator*
BlueStore::clone_allocator_without_bluefs(Allocator* src_allocator)
{
...
//---------------------------------------------------------
static void
clear_allocation_objects_from_rocksdb(
    KeyValueDB* db,
    CephContext* cct,
    const std::string& path)
{
...
//---------------------------------------------------------
void
BlueStore::copy_allocator_content_to_fm(
    Allocator* allocator,
...
//---------------------------------------------------------
Allocator*
BlueStore::initialize_allocator_from_freelist(FreelistManager* real_fm)
{
...
int
BlueStore::reset_fm_for_restore()
{
...
int
BlueStore::verify_rocksdb_allocations(Allocator* allocator)
{
  ...
  uint64_t memory_target =
      cct->_conf.get_val<Option::size_t>("osd_memory_target");
  int ret = compare_allocators(
      allocator, temp_allocator, insert_count, memory_target);
...
int
BlueStore::push_allocation_to_rocksdb()
{
  if (cct->_conf->bluestore_allocation_from_file) {
...
//-------------------------------------------------------------------------------------
int
BlueStore::commit_freelist_type()
{
...
//-------------------------------------------------------------------------------------
int
BlueStore::commit_to_null_manager()
{
...
//-------------------------------------------------------------------------------------
int
BlueStore::commit_to_real_manager()
{
