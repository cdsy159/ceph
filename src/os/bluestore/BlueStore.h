...
public:
  ...
  struct printer {
    ...
    static constexpr uint16_t BUF =
        256; // print Blob's buffers (takes cache lock)
    static constexpr uint16_t SBUF =
        512; // short print Blob's buffers (takes cache lock)
    static constexpr uint16_t ATTRS = 1024; // print attrs in onode
    static constexpr uint16_t JUSTID =
        2048; // used to suppress printing length, spanning and shared blob
  };
  ...
  struct Buffer {
    ...
    Buffer(
        ...
        uint32_t l,
        unsigned f = 0) :
      space(space), state(s), flags(f), txc(_txc), offset(o), length(l)
    {
      total++;
    }
  ...
  struct BufferSpace {
    ...
    BufferSpace(Onode& _onode) :
      onode(_onode)
    {}

    ~BufferSpace() { ceph_assert(buffer_map.empty()); }
    ...
    void
    _rm_buffer(BufferCacheShard* cache, Buffer* b)
    {
    ...
    int
    discard(BufferCacheShard* cache, uint32_t offset, uint32_t length)
    {
        ...
        unsigned flags)
    {
      ...
      _add_buffer(
          cache,
          new Buffer(
              this, Buffer::STATE_WRITING, txc, offset, std::move(bl), flags),
    ...
    void
    did_read(BufferCacheShard* cache, uint32_t offset, ceph::buffer::list&& bl)
    {
      std::lock_guard l(cache->lock);
      uint16_t cache_private = _discard(cache, offset, bl.length());
      _add_buffer(
    ...
    void
    truncate(BufferCacheShard* cache, uint32_t offset)
    {
    ...
    void
    dump(BufferCacheShard* cache, ceph::Formatter* f) const
    {
      std::lock_guard l(cache->lock);
  ...
  struct SharedBlob {
    ...
    SharedBlob(Collection* _coll) :
      collection(_coll), sbid_unloaded(0)
    {}
    ...
    friend void
    intrusive_ptr_add_ref(SharedBlob* b)
    {
      b->get();
    }

    friend void
    intrusive_ptr_release(SharedBlob* b)
    {
      b->put();
    }
    ...
    void
    put_ref(uint64_t offset, uint32_t length, PExtentVector* r, bool* unshare);

    friend bool
    operator==(const SharedBlob& l, const SharedBlob& r)
    {
      return l.get_sbid() == r.get_sbid();
    }

    inline BufferCacheShard*
    get_cache()
    {
      return collection ? collection->cache : nullptr;
    }

    inline SharedBlobSet*
    get_parent()
    {
  ...
  struct SharedBlobSet {
    ...
    void
    add(Collection* coll, SharedBlob* sb)
    {
      std::lock_guard l(lock);
    ...
    bool
    remove(SharedBlob* sb, bool verify_nref_is_zero = false)
    {
      std::lock_guard l(lock);
  ...
  struct Blob {
    ...
    void
    set_shared_blob(SharedBlobRef sb)
    {
      ceph_assert((bool)sb);
  ...
  public:
    friend void
    intrusive_ptr_add_ref(Blob* b)
    {
      b->get();
    }

    friend void
    intrusive_ptr_release(Blob* b)
    {
      b->put();
    }
    ...
    const SharedBlobRef&
    get_shared_blob() const
    {
      return shared_blob;
    }

    SharedBlobRef&
    get_dirty_shared_blob()
    {
      return shared_blob;
    }
    ...
    can_split()
    {
      ...
      return used_in_blob.can_split() && get_blob().can_split();
    }
    ...
    bool
    can_split_at(uint32_t blob_offset) const
    {
    ...
    void copy_extents(
        ...
        uint32_t main_len,
        uint32_t post_len);
    void copy_extents_over_empty(
        CephContext* cct,
        const Blob& from,
        uint32_t start,
        uint32_t len);

    inline const bluestore_blob_t&
    get_blob() const
    {
      return blob;
    }

    inline bluestore_blob_t&
    dirty_blob()
    {
      return blob;
    }
  ...
  struct Extent : public ExtentBase {
    ...
    friend bool
    operator<(const Extent& a, const Extent& b)
    {
    ...
    bool
    blob_escapes_range(uint32_t o, uint32_t l) const
    {
  ...
  struct OldExtent {
    ...
    static OldExtent*
    create(CollectionRef c, uint32_t lo, uint32_t o, uint32_t l, BlobRef& b);
  };
  ...
  struct ExtentMap {
    ...
    void reblob_extents(
        ...
        BlobRef from_blob,
        BlobRef to_blob);
    void make_range_shared_maybe_merge(
        TransContext* txc,
        OnodeRef& onode,
        uint64_t srcoff,
        uint64_t length);
    ...
    void
    request_reshard(uint32_t begin, uint32_t end)
    {
    ...
    void
    maybe_reshard(uint32_t begin, uint32_t end)
    {
    ...
    protected:
      ...
      void
      decode_extent(Extent* le, __u8 struct_v, bptr_c_it_t& p, Collection* c);
    ...
    protected:
      ...
      void consume_blob(Extent* le, uint64_t extent_no, uint64_t sbid, BlobRef b)
          override;
    ...
    bool
    spans_shard(uint32_t offset, uint32_t length)
    {
      if (shards.empty()) {
        return false;
      }
    ...
    std::pair<uint32_t, uint32_t> fault_range_ex(
        ...
        uint32_t offset,
        uint32_t length);
    void maybe_load_shard(KeyValueDB* db, int begin_shard, int end_shard);
    ...
    /// add a new Extent
    void
    add(uint32_t lo, uint32_t o, uint32_t l, BlobRef& b)
    {
  ...
  */
  ...
  private:
    ...
    CephContext* cct;
    std::map<Blob*, BlobInfo>
        affected_blobs; ///< compressed blobs and their ref_map
  ...
  protected:
    void process_protrusive_extents(
  ...
  struct Onode {
    ...
    Onode(
      ...
      extent_map(
          this,
          c->store->cct->_conf->bluestore_extent_map_inline_shard_prealloc_size),
      bc(*this)
    {}
    ...
    Onode(CephContext* cct) :
      ...
      extent_map(
          this,
          cct->_conf->bluestore_extent_map_inline_shard_prealloc_size),
      bc(*this)
    {}
    ...
    void
    get_omap_header(std::string* out)
    {
      calc_omap_header(onode.flags, this, out);
    }

    void
    get_omap_key(const std::string& key, std::string* out)
    {
      calc_omap_key(onode.flags, this, key, out);
    }

    void
    get_omap_tail(std::string* out)
    {
    ...
    printer
    print(uint16_t mode, uint32_t from, uint32_t end) const
    {
  ...
  struct CacheShard {
    ...
    CacheShard(CephContext* cct) :
      cct(cct), logger(nullptr), age_bins(1)
    {
      shift_bins();
    }
    ...
    uint64_t
    sum_bins(uint32_t start, uint32_t end)
    {
      std::lock_guard l(lock);
  ...
  struct OnodeCacheShard : public CacheShard {
    std::array<std::pair<ghobject_t, ceph::mono_clock::time_point>, 64>
        dumped_onodes;
  ...
  public:
    ...
    static BufferCacheShard* create(
        BlueStore* store,
        std::string type,
        PerfCounters* logger);
  ...
  struct volatile_statfs {
    ...
    void
    publish(store_statfs_t* buf) const
    {
    ...
    volatile_statfs&
    operator+=(const volatile_statfs& other)
    {
      for (size_t i = 0; i < STATFS_LAST; ++i) {
    ...
    int64_t
    compressed_allocated() const
    {
      return values[STATFS_COMPRESSED_ALLOCATED];
    }

    volatile_statfs&
    operator=(const store_statfs_t& st)
    {
    ...
    bool
    operator==(const volatile_statfs& rhs) const
    {
      return values[STATFS_ALLOCATED] == rhs.values[STATFS_ALLOCATED] &&
             values[STATFS_STORED] == rhs.values[STATFS_STORED] &&
             values[STATFS_COMPRESSED_ORIGINAL] ==
                 rhs.values[STATFS_COMPRESSED_ORIGINAL] &&
             values[STATFS_COMPRESSED] == rhs.values[STATFS_COMPRESSED] &&
             values[STATFS_COMPRESSED_ALLOCATED] ==
                 rhs.values[STATFS_COMPRESSED_ALLOCATED];
    }
    ...
    void
    decode(ceph::buffer::list::const_iterator& it)
    {
      using ceph::decode;
      for (size_t i = 0; i < STATFS_LAST; i++) {
        decode(values[i], it);
  ...
  struct TransContext final : public AioContext {
    ...
    const char*
    get_state_latency_name(int state)
    {
      switch (state) {
    ...
    ceph::mutex writings_lock =
        ceph::make_mutex("BlueStore::TransContextWritings::lock");
    ...
    explicit TransContext(
        CephContext* cct,
        Collection* c,
        OpSequencer* o,
        std::list<Context*>* on_commits) :
      ch(c), osr(o), ioc(cct, this), start(ceph::mono_clock::now())
    {
      last_stamp = start;
      if (on_commits) {
    ...
    void
    write_shared_blob(const SharedBlobRef& sb)
    {
      shared_blobs.insert(sb);
    }

    void
    unshare_blob(SharedBlob* sb)
    {
    ...
    void
    note_modified_object(OnodeRef& o)
    {
    ...
    void
    note_removed_object(OnodeRef& o)
    {
      modified_objects.insert(o);
      onodes.erase(o);
    }
    ...
    void
    aio_finish(BlueStore* store) override
    {
  ...
  class BlueStoreThrottle {
#if defined(WITH_LTTNG)
    const std::chrono::time_point<ceph::mono_clock> time_base =
        ceph::mono_clock::now();
    ...
    bool
    should_trace(uint64_t* started, uint64_t* completed)
    {
  ...
  public:
...
#else
    void
    complete_kv(TransContext& txc)
    {}

    void
    complete(TransContext& txc)
    {}
#endif
    ...
    void
    release_kv_throttle(uint64_t cost, uint64_t txcs)
    {
    ...
    void
    release_deferred_throttle(uint64_t cost)
    {
    ...
    void
    reset_throttle(const ConfigProxy& conf)
    {
  ...
  public:
    ...
    uint32_t
    get_sequencer_id() const
    {
      return sequencer_id;
    }

    void
    queue_new(TransContext* txc)
    {
    ...
    void
    drain_preceding(TransContext* txc)
    {
      std::unique_lock l(qlock);
      while (&q.front() != txc)
        qcond.wait(l);
...
private:
  ...
  bluefs_shared_alloc_context_t
      shared_alloc; ///< consumed by BlueFS (may be == alloc)
  ...
  ceph::shared_mutex coll_lock =
      ceph::make_shared_mutex("BlueStore::coll_lock");
  ...
  std::atomic_int deferred_queue_size = {
  ...
  bool elastic_shared_blobs =
      false; ///< use smart ExtentMap::dup to reduce shared blob count
  ...
  std::atomic<uint32_t> segment_size =
      {0}; ///< snapshot of conf value "bluestore_onode_segment_size"
  ...
  double cache_age_bin_interval =
      0; ///< time to wait between cache age bin rotations
  ...
  public:
    ...
    struct MetaCache : public MempoolCache {
      ...
      virtual uint64_t
      _sum_bins(uint32_t start, uint32_t end) const
      {
        uint64_t onodes = 0;
        for (auto i : store->onode_cache_shards) {
  ...
  */
  ...
  int _setup_block_symlink_or_file(
      std::string name,
      std::string path,
      uint64_t size,
      bool create);
...
private:
  static int _write_bdev_label(
      ...
      std::vector<uint64_t> locations = std::vector<uint64_t>(
  ...
  static int _read_bdev_label(
      ...
      const std::string& path,
      bluestore_bdev_label_t* label,
      uint64_t disk_position = BDEV_FIRST_LABEL_POSITION);
  int _check_or_set_bdev_label(
      BlockDevice* bdev,
      const std::string& path,
...
private:
  ...
  bool _eliminate_outdated_deferred(
      bluestore_deferred_transaction_t* deferred_txn,
      interval_set<uint64_t>& bluefs_extents);

public:
  using mempool_dynamic_bitset = boost::dynamic_bitset<
      uint64_t,
  ...
  struct pool_fsck_stats_t {
    ...
    void
    add(const pool_fsck_stats_t& other)
    {
...
private:
  ...
  _buffer_cache_write(
      ...
      unsigned flags)
  {
    onode->bc.write(onode->c->cache, txc, offset, std::move(bl), flags);
  }
  ...
  _buffer_cache_write(
      ...
      unsigned flags)
  {
    onode->bc.write(onode->c->cache, txc, offset, bl, flags);
  }
  ...
  T
  select_option(const std::string& opt_name, T val1, F f)
  {
...
public:
  ...
  bool
  needs_journal() override
  {
    return false;
  };

  bool
  wants_journal() override
  {
    return false;
  };

  bool
  allows_journal() override
  {
    return false;
  };
  ...
  uint64_t
  get_min_alloc_size() const override
  {
  ...
  static int get_block_device_fsid(
      CephContext* cct,
      const std::string& path,
      uuid_d* fsid);
...
public:
  ...
  int
  fsck(bool deep) override
  {
    return _fsck(deep ? FSCK_DEEP : FSCK_REGULAR, false);
  }

  int
  repair(bool deep) override
  {
  ...
  void
  dump_cache_stats(ceph::Formatter* f) override
  {
  ...
  void
  dump_cache_stats(std::ostream& ss) override
  {
    int onode_count = 0, buffers_bytes = 0;
    for (auto i : onode_cache_shards) {
      onode_count += i->_get_num();
  ...
  int
  validate_hobject_key(const hobject_t& obj) const override
  {
    return 0;
  }

  unsigned
  get_max_attr_name_length() override
  {
  ...
  void
  dump_perf_counters(ceph::Formatter* f) override
  {
  ...
  int migrate_to_new_bluefs_device(
...
private:
  ...
  struct region_t {
    ...
    region_t(uint64_t offset, uint64_t b_offs, uint64_t len, uint64_t front = 0) :
      logical_offset(offset), blob_xoffset(b_offs), length(len), front(front)
    {}
    ...
    friend std::ostream&
    operator<<(std::ostream& out, const region_t& r)
    {
  ...
  struct read_req_t {
    ...
    friend std::ostream&
    operator<<(std::ostream& out, const read_req_t& r)
    {
...
public:
  ...
  void set_collection_commit_queue(
      const coll_t& cid,
  ...
  uint64_t
  estimate_objects_overhead(uint64_t num_objects) override
  {
  ...
  // error injection
  void
  inject_data_error(const ghobject_t& o) override
  {
  ...
  void
  inject_mdata_error(const ghobject_t& o) override
  {
    std::unique_lock l(debug_read_error_lock);
  ...
  int get_shared_blob(const std::string& key, ceph::buffer::list& bl);
  void inject_broken_shared_blob_key(
      const std::string& key,
  ...
  bool
  has_builtin_csum() const override
  {
    return true;
  }
  ...
  void
  debug_punch_hole(CollectionRef& c, OnodeRef& o, uint32_t off, uint32_t len)
  {
  ...
  {
    return _write_bdev_label(
        cct, bdev, path, label, std::vector<uint64_t>({disk_position}));
  }
  ...
  {
    _punch_hole_2(
        c.get(), o, offset, length, released, pruned_blobs, shared_changed,
        statfs_delta);
  }
  ...
  void
  debug_set_block_size(uint64_t _block_size)
  {
  ...
  void
  debug_set_prefer_deferred_size(uint64_t s)
  {
    prefer_deferred_size = s;
  }

  inline void log_latency(
      const char* name,
      int idx,
  ...
  inline void log_latency_fn_scrub(
      const char* name,
      int idx,
      const ceph::timespan& lat,
      double lat_threshold,
...
private:
  bool
  _debug_data_eio(const ghobject_t& o)
  {
  ...
  bool
  _debug_mdata_eio(const ghobject_t& o)
  {
    if (!cct->_conf->bluestore_debug_inject_read_err) {
      return false;
    }
  ...
  void
  _debug_obj_on_delete(const ghobject_t& o)
  {
    if (cct->_conf->bluestore_debug_inject_read_err) {
      std::unique_lock l(debug_read_error_lock);
...
private:
  ...
  std::pair<size_t, size_t> _trim_slow_op_event_queue(
      ceph::mono_clock::time_point cur_time);
  void _add_slow_op_event();
  void _add_slow_scrub_op_event();
  void _log_alerts(osd_alert_list_t& alerts);

  bool
  _set_compression_alert(bool cmode, const char* s)
  {
    std::lock_guard l(qlock);
    if (cmode) {
  ...
  void
  _set_spurious_read_errors_alert(const std::string& s)
  {
    std::lock_guard l(qlock);
...
public:
  struct WriteContext {
    ...
    {
      writes.emplace_back(
          loffs, b, blob_len, o, bl, o0, len0, _mark_unused, _new_blob);
    }
...
private:
  ...
  void _do_truncate(
      ...
      std::set<SharedBlob*>* maybe_unshared_blobs = 0);
  int
  _truncate(TransContext* txc, CollectionRef& c, OnodeRef& o, uint64_t offset);
  ...
  int _split_collection(
      TransContext* txc,
      CollectionRef& c,
      CollectionRef& d,
      unsigned bits,
      int rem);
  int _merge_collection(
      TransContext* txc,
      CollectionRef* c,
      CollectionRef& d,
      unsigned bits);

  void _collect_allocation_stats(
      uint64_t need,
      uint32_t alloc_size,
      const PExtentVector&);
...
#endif
  void set_allocation_in_simple_bmap(
      SimpleBitmap* sbmap,
      uint64_t offset,
      uint64_t length);
...
private:
  ...
  friend std::ostream&
  operator<<(std::ostream& out, const read_alloc_stats_t& stats)
  {
    out << "=========================================================="
        << std::endl;
    out << "NCB::onode_count             = ";
    ...
    out.width(10);
    out << stats.shared_blob_count << std::endl
        << "NCB::compressed_blob_count   = ";
    out.width(10);
    out << stats.compressed_blob_count << std::endl
        << "NCB::spanning_blob_count     = ";
    out.width(10);
    out << stats.spanning_blob_count << std::endl
        << "NCB::skipped_illegal_extent  = ";
    out.width(10);
    out << stats.skipped_illegal_extent << std::endl
        << "NCB::extent_count            = ";
    out.width(10);
  ...
  void copy_allocator_content_to_fm(
      Allocator* allocator,
      FreelistManager* real_fm);


  void _fsck_check_object_omap(
      FSCKDepth depth,
      OnodeRef& o,
...
inline std::ostream&
operator<<(std::ostream& out, const BlueStore::volatile_statfs& s)
{
  return out
         << " allocated:"
...
static inline void
intrusive_ptr_add_ref(BlueStore::Onode* o)
{
  o->get();
}

static inline void
intrusive_ptr_release(BlueStore::Onode* o)
{
  o->put();
}

static inline void
intrusive_ptr_add_ref(BlueStore::OpSequencer* o)
{
  o->get();
}

static inline void
intrusive_ptr_release(BlueStore::OpSequencer* o)
{
  o->put();
}
...
public:
  ...
  struct StoreSpaceTracker {
    ...
    void
    init(uint64_t total, uint64_t min_alloc_size, uint64_t mem_cap = DEF_MEM_CAP)
    {
      ...
      collections_bfs.resize(
          entries, bloom_filter(
                       BLOOM_FILTER_SALT_COUNT, BLOOM_FILTER_TABLE_SIZE, 0,
    ...
    inline uint32_t
    get_hash(const coll_t& cid) const
    {
    ...
    inline bool
    is_used(const coll_t& cid) const
    {
      ceph_assert(was_filtered_out);
    ...
    inline bool
    is_used(const ghobject_t& oid) const
    {
      ceph_assert(was_filtered_out);
    ...
    inline bool
    is_used(const coll_t& cid, uint64_t offs) const
    {
      ceph_assert(granularity); // initialized
      ceph_assert(!was_filtered_out);
    ...
    inline bool
    is_used(const ghobject_t& oid, uint64_t offs) const
    {
      ceph_assert(granularity); // initialized
      ceph_assert(!was_filtered_out);
...
public:
  ...
  void
  note_misreference(uint64_t offs, uint64_t len, bool inc_error)
  {
    std::lock_guard l(lock);
  ...
  void
  init_space_usage_tracker(uint64_t total_space, uint64_t lres_tracking_unit_size)
  {
