...
public:
  ...
  struct range_seg_t {
    ...
    struct before_t {
      ...
      bool
      operator()(const KeyLeft& lhs, const KeyRight& rhs) const
      {
    ...
    struct shorter_t {
      ...
      int
      compare(const KeyType1& lhs, const KeyType2& rhs) const
      {
        int64_t delta = int64_t(lhs.end) - int64_t(lhs.start) -
                        int64_t(rhs.end) + int64_t(rhs.start);
        if (delta == 0) {
      ...
      int
      operator()(const range_seg_t& lhs, const KeyType& rhs) const
      {
...
public:
  ...
  ~Btree2Allocator() override { shutdown(); }
...
private:
  ...
  using range_tree_t = btree::btree_map<
      uint64_t, // start
      uint64_t, // end
      std::less<uint64_t>,
  ...
  //
  using range_size_tree_t = btree::
      btree_set<range_seg_t, range_seg_t::shorter_t, pool_allocator<range_seg_t>>;
...
protected:
  ...
  typedef std::array<const release_set_t::value_type*, pextent_array_size>
      PExtentArray;

  void
  set_weight_factor(double _rweight_factor)
  {
  ...
  range_size_tree_t*
  _get_lowest(range_size_tree_t::iterator* rs_p)
  {
  ...
  double
  _get_fragmentation() const
  {
    auto free_blocks = p2align(num_free.load(), (uint64_t)block_size) /
                       block_size;
    if (free_blocks <= 1) {
      return .0;
    }
  ...
  virtual void
  _spillover_range(uint64_t start, uint64_t end)
  {
  ...
  bool
  try_put_cache(uint64_t start, uint64_t len)
  {
  ...
  bool
  try_get_from_cache(uint64_t* res_offset, uint64_t want)
  {
...
private:
  ...
  inline range_size_tree_t::iterator _pick_block(
      int distance,
      range_size_tree_t* tree,
      uint64_t size);

  inline void _remove_from_tree(uint64_t start, uint64_t size);
  inline range_tree_iterator _remove_from_tree(
      range_tree_iterator rt_p,
      uint64_t start,
      uint64_t end);
  inline range_tree_iterator _remove_from_tree(
      range_size_tree_t* rs_tree,
  ...
  inline bool __try_insert_range(
  ...
  inline void _range_size_tree_rm(
      range_size_tree_t* rs_tree,
      range_size_tree_t::iterator rs);
};
