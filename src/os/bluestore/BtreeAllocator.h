...
class BtreeAllocator : public AllocatorBase {
  ...
  struct compare_range_value_t {
    int
    operator()(const range_value_t& lhs, const range_value_t& rhs) const noexcept
    {
...
private:
   ...
   */
  using range_size_tree_t = btree::btree_set<
...
private:
  ...
  uint64_t
  _lowest_size_available() const
  {
    auto rs = range_size_tree.begin();
    return rs != range_size_tree.end() ? rs->size
