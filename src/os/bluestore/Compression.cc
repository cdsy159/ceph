...
struct basic_ostream_formatter
  : fmt::formatter<std::basic_string_view<Char>, Char> {
  template <typename T, typename OutputIt>
  auto
  format(const T& value, fmt::basic_format_context<OutputIt, Char>& ctx) const
      -> OutputIt
  {
...
template <>
struct fmt::formatter<BlueStore::Blob::printer> : ostream_formatter {};

template <>
struct fmt::formatter<BlueStore::Extent::printer> : ostream_formatter {};
...
void
Estimator::set_wctx(const WriteContext* wctx)
{
  this->wctx = wctx;
}

inline void
Estimator::batch(const BlueStore::Extent* e, uint32_t gain)
{
  ...
  if (h_bblob.is_compressed()) {
    ...
    compressed_size += e->length * h_bblob.get_compressed_payload_length() /
                       h_bblob.get_logical_length();
  ...
  dout(20) << fmt::format(
                  "Estimator::batch {} gain {:#x}", e->print(P::NICK), gain)
           << dendl;
  dout(20) << fmt::format(
                  "Estimator::batch non-compr={:#x} compr-occup={:#x} "
                  "compr-size?={:#x}",
                  uncompressed_size, compressed_occupied, compressed_size)
           << dendl;
}
...
Estimator::is_worth()
{
  ...
  if (uncompressed_size == 0 && single_compressed_blob) {
    ...
    uint64_t padding_size =
        p2nphase<uint64_t>(compressed_size, bluestore->min_alloc_size);
    uint32_t split_tax = padding_size * compressed_area /
                         wctx->target_blob_size;
    cost += split_tax;
  }
...
inline bool
Estimator::is_worth(const BlueStore::Extent* e)
{
  const Blob* h_Blob = &(*e->blob);
  const bluestore_blob_t& h_bblob = h_Blob->get_blob();
...
inline void
Estimator::mark_recompress(const BlueStore::Extent* e)
{
...
inline void
Estimator::mark_main(uint32_t location, uint32_t length)
{
  ceph_assert(!extra_recompress.contains(location));
  dout(25) << "main data compress: " << std::hex << location << "~" << length
           << std::dec << dendl;
...
void
Estimator::get_regions(std::vector<region_t>& regions)
{
  ...
  while (i != extra_recompress.end()) {
    dout(25) << std::hex << i->first << "~" << i->second << dendl;
    if (end == unset) {
...
int32_t
Estimator::split_and_compress(ceph::buffer::list& data_bl, Writer::blob_vec& bd)
{
  ...
  for (auto& i : blob_sizes) {
    ...
    int r = wctx->compressor->compress(
        bd.back().object_data, t, compressor_message);
    ceph_assert(r == 0);
...
Estimator::finish()
{
  dout(25) << "new_size=" << new_size << " unc_size=" << total_uncompressed_size
           << " comp_cost=" << total_compressed_size << dendl;
  uint32_t sum = new_size + total_uncompressed_size + total_compressed_size;
  double expected = (new_size + total_uncompressed_size) *
                        expected_compression_factor +
                    total_compressed_size * expected_recompression_error;
  double size_misprediction = double(expected - actual_compressed) /
                              actual_compressed;
  double size_misprediction_weighted = 1.0 / sum * size_misprediction * 0.01;
  expected_compression_factor -= (new_size + total_uncompressed_size) *
                                 size_misprediction_weighted;
  expected_recompression_error -= total_compressed_size *
                                  size_misprediction_weighted;

  double expected_pad = actual_compressed * expected_pad_expansion;
  double pad_misprediction = (expected_pad - actual_compressed_plus_pad) /
                             actual_compressed;
...
void
Estimator::dump(Formatter* f) const
{
...
struct scan_blob_element_t {
  uint32_t blob_use_size =
      0; // how many bytes in object logical mapping are covered by this blob
  uint32_t visited_size =
      0; // updated during scanning, will be either commited to consumed or reverted from consumed
...
public:
  ...
  Scan(BlueStore* bluestore, BlueStore::Onode* onode, Estimator* estimator) :
    bluestore(bluestore),
    onode(onode),
...
private:
  bool
  is_end(const exmp_cit& it) const
  {
...
template <>
struct fmt::formatter<BlueStore::Scanner::Scan::exmp_logical_offset>
  : ostream_formatter {};
...
std::ostream&
operator<<(std::ostream& out, const scan_blob_element_t& b)
{
  out << fmt::format(
      "blob=[{:#x}-{:x}] extent=[{:#x}-{:x}] {:#x}/{:x}/{:x}({:x}){}",
      b.blob_left, b.blob_right, b.leftmost_extent->logical_offset,
      b.rightmost_extent->logical_offset, b.blob_use_size, b.visited_size,
      b.consumed_size, b.consumed_saved_size, b.rejected ? " rejected" : "");
  return out;
}

std::ostream&
operator<<(std::ostream& out, const object_scan_info_t& scanned_blobs)
{
  out << "[";
...
std::ostream&
operator<<(std::ostream& out, const Scan::exmp_logical_offset& lo)
{
...
void
Scan::scan_for_compressed_blobs(exmp_cit start, exmp_cit finish)
{
  ...
  for (auto it = start; it != finish; ++it) {
    ...
    if (h_bblob.is_shared()) {
      ...
      dout(20) << "scan.compr " << h_Blob->print(P::NICK | P::JUSTID)
               << " shared, rejected" << dendl;
      continue;
    }
    if (!h_bblob.is_compressed()) {
      dout(20) << "scan.compr " << h_Blob->print(P::NICK | P::JUSTID)
    ...
    if (bit == scanned_blobs.end()) {
      ...
      dout(20) << fmt::format(
                      "scan.compr {} new, range [{:#x}-{:x}]",
                      h_Blob->print(P::NICK | P::JUSTID), it->logical_offset,
                      it->logical_end())
               << dendl;
      scanned_blobs.emplace(
          h_Blob,
          scan_blob_element_t(
              h_Blob->get_blob_use_tracker().get_referenced_bytes(), it->length,
              0, 0, it->blob_start(), it->blob_end(), it, it));
      ...
      dout(20) << fmt::format(
                      "scan.compr {} updated range [{:#x}-{:x}]",
                      h_Blob->print(P::NICK | P::JUSTID),
                      info.leftmost_extent->logical_offset,
                      info.rightmost_extent->logical_end())
               << dendl;
...
bool
Scan::maybe_expand_scan_range(const exmp_cit& it, uint32_t& left, uint32_t& right)
{
  bool expanded = false;
  const Blob* h_Blob = &(*it->blob);
  const bluestore_blob_t& h_bblob = h_Blob->get_blob();
  ...
  if (h_bblob.is_shared()) {
    dout(30) << "maybe_expand rejecting shared "
             << h_Blob->print(P::NICK + P::JUSTID) << dendl;
  } else {
    ceph_assert(h_bblob.is_compressed());
    ...
    if (i == scanned_blobs.end() || i->second.rejected == false) {
      ...
      dout(20) << fmt::format(
                      "maybe_expand take {} range=[{:#x}-{:x}]",
                      h_Blob->print(P::NICK + P::SDISK + P::SUSE + P::SCHK),
                      left, right)
               << dendl;
      expanded = true;
    } else {
...
Scan::candidate(const Extent* e)
{
  ...
  if (h_bblob.is_shared()) {
    dout(30) << "candidate ignoring shared "
             << h_Blob->print(P::NICK + P::JUSTID) << dendl;
    ...
    if (bit != scanned_blobs.end()) {
      ...
      derr << "candidate blob not scanned before! "
           << h_Blob->print(P::NICK + P::SUSE) << dendl;
...
void
Scan::estimate_gain_left(exmp_cit left, uint32_t right)
{
  auto i = left;
...
void
Scan::estimate_gain_right(uint32_t left, exmp_cit right)
{
...
// 'right' is the leftmost logical offset of area already marked for recompression
void
Scan::add_compress_left(exmp_cit left, uint32_t right)
{
  ceph_assert(left != extent_map->extent_map.end());
  dout(30) << fmt::format(
                  "add_compress_left extents {:#x}(include) .. {:#x}",
                  left->logical_offset, right)
           << dendl;
  ceph_assert(!left->blob->get_blob().is_shared());
  for (auto i = left;
       i != extent_map->extent_map.end() && i->logical_offset < right; ++i) {
...
void
Scan::add_compress_right(uint32_t left, exmp_cit right)
{
  ceph_assert(right != extent_map->extent_map.end());
  dout(30) << fmt::format(
                  "add_compress_right {:#x} .. {:#x}(include)", left,
...
Scan::expand_left(exmp_cit& it)
{
  ...
  for (; it != extent_map->extent_map.begin();) {
    ...
    const Blob* h_Blob = &(*it->blob);
    const bluestore_blob_t& h_bblob = h_Blob->get_blob();
    if (it->logical_offset < limit_left || h_bblob.is_shared() ||
        h_bblob.is_compressed()) {
...
Scan::expand_right(exmp_cit& it)
{
  ...
  for (; !is_end(it); ++it) {
    const Blob* h_Blob = &(*it->blob);
    const bluestore_blob_t& h_bblob = h_Blob->get_blob();
    if (it->logical_end() > limit_right || h_bblob.is_shared() ||
        h_bblob.is_compressed()) {
...
object_scan_info_t::iterator
Scanner::Scan::find_least_expanding()
{
  auto expansion_size = [&](object_scan_info_t::const_iterator i) -> uint32_t {
    return done_left -
           std::min(i->second.leftmost_extent->logical_offset, done_left) +
           std::max(i->second.rightmost_extent->logical_end(), done_right) -
           done_right;
  };
...
Scan::on_write_start(
    uint32_t offset,
    uint32_t length,
    uint32_t _limit_left,
    uint32_t _limit_right)
{
  ...
  dout(10) << fmt::format(
                  "on_write #1 [{:#x}-{:x}] limit=({:#x}-{:x})", done_left,
                  done_right, limit_left, limit_right)
           << dendl;
  dout(20) << "on_write #1 on: "
           << onode->print(P::NICK + P::SDISK + P::SUSE, limit_left, limit_right)
           << dendl;
  ...
  // STEP 2.
  dout(15) << fmt::format(
                  "on_write #2 scan=[{:#x}-{:x}] it={}-{}", scanned_left,
                  scanned_right, lo(scanned_left_it), lo(scanned_right_it))
           << dendl;
...
step3:
  if (left < scanned_left || scanned_right < right) {
    dout(15) << fmt::format(
                    "on_write #3 expand [{:#x}-{:x}] to [{:#x}-{:x}]",
                    scanned_left, scanned_right, left, right)
             << dendl;
    ...
    while (true) {
      ...
      dout(25) << "on_write #4 test expand "
               << b_it->first->print(P::NICK | P::JUSTID) << ", "
               << b_it->second << dendl;
      save_consumed();
    ...
    dout(15) << fmt::format(
                    "on_write #3 scanned=[{:#x}-{:x}]", scanned_left,
                    scanned_right)
             << dendl;
  }
  // STEP 5.
  ...
  dout(15) << "on_write #5 left=" << lo(left_it) << " right=" << lo(right_it)
           << dendl;
  if (!is_end(right_it)) {
  ...
  dout(15) << "on_write #6 left=" << lo(left_it) << " right=" << lo(right_it)
           << dendl;
