...
public:
  ...
  void foreach (std::function<void(uint64_t, uint64_t)> notify) override
  {
    std::lock_guard l(PrimaryAllocator::get_lock());
...
private:
  ...
  uint64_t _allocate_or_rollback(
      bool primary,
      uint64_t want,
      uint64_t unit,
      uint64_t max_alloc_size,
  ...
  uint64_t
  _get_spilled_over() const override
  {
    return bmap_alloc ? bmap_alloc->get_free() : 0;
  }
  ...
  void
  _add_to_tree(uint64_t start, uint64_t size) override
  {
    if (bmap_alloc) {
...
public:
  HybridAvlAllocator(
      ...
      uint64_t max_mem,
      std::string_view name) :
    HybridAllocatorBase<AvlAllocator>(cct, device_size, _block_size, max_mem, name)
  {}
...
public:
  HybridBtree2Allocator(
      ...
      std::string_view name) :
    HybridAllocatorBase<Btree2Allocator>(
        cct,
        device_size,
        _block_size,
        max_mem,
