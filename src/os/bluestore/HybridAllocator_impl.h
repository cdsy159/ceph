...
template <typename T>
int64_t
HybridAllocatorBase<T>::allocate(
    uint64_t want,
    uint64_t unit,
...
{
  dout(10) << __func__ << std::hex << " 0x" << want << "/" << unit << ","
  ...
  bool primary_first =
      !(bmap_alloc && bmap_alloc->get_free() &&
        want < T::_lowest_size_available());

  int64_t res = _allocate_or_rollback(
      primary_first, want, unit, max_alloc_size, hint, extents);
  ceph_assert(res >= 0);
...
HybridAllocatorBase<T>::dump()
{
  ...
  dout(0) << __func__ << " avl_free: " << T::_get_free()
...
template <typename T>
void
HybridAllocatorBase<T>::init_rm_free(uint64_t offset, uint64_t length)
{
  if (!length)
    return;
  std::lock_guard l(T::get_lock());
  dout(10) << __func__ << std::hex << " offset 0x" << offset << " length 0x"
           << length << std::dec << dendl;
  T::_try_remove_from_tree(
        ...
        if (!found) {
          if (bmap_alloc) {
            ...
            derr << __func__ << " lambda " << std::hex
                 << "Uexpected extent: " << " 0x" << o << "~" << l << std::dec
                 << dendl;
            ceph_assert(false);
          }
        }
...
template <typename T>
void
HybridAllocatorBase<T>::_spillover_range(uint64_t start, uint64_t end)
{
  auto size = end - start;
  ...
  if (!bmap_alloc) {
    ...
    bmap_alloc = std::make_unique<BitmapAllocator>(
        T::get_context(), T::get_capacity(), T::get_block_size(),
        T::get_name() + ".fallback");
  }
...
template <typename T>
uint64_t
HybridAllocatorBase<T>::_spillover_allocate(
    uint64_t want,
    uint64_t unit,
    uint64_t max_alloc_size,
    int64_t hint,
    PExtentVector* extents)
{
  return _allocate_or_rollback(false, want, unit, max_alloc_size, hint, extents);
}

template <typename PrimaryAllocator>
uint64_t
HybridAllocatorBase<PrimaryAllocator>::_allocate_or_rollback(
    bool primary,
    uint64_t want,
    uint64_t unit,
    uint64_t max_alloc_size,
