...
StupidAllocator::StupidAllocator(
  ...
  AllocatorBase(name, capacity, _block_size), cct(cct), num_free(0), free(10)
{
...
unsigned
StupidAllocator::_choose_bin(uint64_t orig_len)
{
...
void
StupidAllocator::_insert_free(uint64_t off, uint64_t len)
{
...
found:
  *offset = p.get_start();
  *length = std::min(
      std::max(alloc_unit, want_size), p2align(p.get_len(), alloc_unit));

  if (cct->_conf->bluestore_debug_small_allocations) {
    uint64_t max = alloc_unit *
                   (rand() % cct->_conf->bluestore_debug_small_allocations);
    if (max && *length > max) {
    ...
    PExtentVector* extents)
{
  ...
  while (allocated_size < want_size) {
    res = allocate_int(
        std::min(max_alloc_size, (want_size - allocated_size)), alloc_unit,
        hint, &offset, &length);
    if (res != 0) {
      /*
       * Allocation failed.
...
void
StupidAllocator::release(const interval_set<uint64_t>& release_set)
{
  std::lock_guard l(lock);
...
StupidAllocator::get_fragmentation()
{
  ...
  {
    std::lock_guard l(lock);
    max_intervals = p2roundup<uint64_t>(num_free, get_block_size()) /
                    get_block_size();
...
void
StupidAllocator::init_add_free(uint64_t offset, uint64_t length)
{
  if (!length)
    return;
  std::lock_guard l(lock);
...
void
StupidAllocator::init_rm_free(uint64_t offset, uint64_t length)
{
  if (!length)
    return;
  std::lock_guard l(lock);
