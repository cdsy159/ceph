...
public:
  ...
  struct blob_data_t {
    ...
    uint32_t
        real_length; // Size of object data covered by this chunk. Same as object_data.length().
    uint32_t
        compressed_length; // Size of compressed representation. 0 or disk_data.length().
    bufferlist
        disk_data; // Bitstream to got o disk. Its either same as object_data,
    ...
    blob_data_t(
        ...
        const bufferlist& disk_data,
        const bufferlist& object_data) :
  ...
  Writer(BlueStore* bstore, TransContext* txc, WriteContext* wctx, OnodeRef o) :
    left_shard_bound(0),
    right_shard_bound(OBJECT_MAX_SIZE),
    bstore(bstore),
    txc(txc),
    wctx(wctx),
    onode(o)
...
public:
  void do_write(uint32_t location, bufferlist& data);
...
private:
  ...
  inline void _schedule_io(const PExtentVector& disk_extents, bufferlist data);

  //Take `length` space from `this.disk_allocs` and put it to `dst`.
  void _get_disk_space(uint32_t length, PExtentVector& dst);

  inline bufferlist _read_self(uint32_t offset, uint32_t length);

  inline void _maybe_expand_blob(Blob* blob, uint32_t new_blob_size);
  ...
  void _split_data(uint32_t location, bufferlist& data, blob_vec& bd);
  ...
  BlobRef _blob_create_with_data(uint32_t in_blob_offset, bufferlist& disk_data);

  BlobRef _blob_create_full(bufferlist& disk_data);
  ...
  std::pair<bool, uint32_t> _write_expand_l(uint32_t logical_offset);

  std::pair<bool, uint32_t> _write_expand_r(uint32_t end_offset);
...
std::ostream& operator<<(
    std::ostream& out,
    const BlueStore::Writer::blob_data_printer& printer);
