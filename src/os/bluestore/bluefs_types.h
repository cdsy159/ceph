...
enum bluefs_node_encoding {
  ...
  ENVELOPE =
      1, ///< Data flushed to file is wrapped in envelope - no size update needed.
      ///  Without shutdown, range [fnode.size ... fnode.allocated) may contain envelopes.
  ENVELOPE_FIN =
      2, ///< Same as envelope but file orderly closed. Fnode.size reflects actual end.
  ENCODING_MAX = 3
};
...
struct bluefs_locked_extents_t {
  ...
  head_end() const
  {
    return head_offset + head_length;
  }

  uint64_t
  gray_tail_end() const
  {
    return gray_tail_offset + gray_tail_length;
  }

  uint64_t
  tail_end() const
  {
    return tail_offset + tail_length;
  }
...
struct bluefs_fnode_delta_t {
  ...
  uint64_t
      content_size; // The size of payload in the file; size = wal_size + n * envelope_size
  ...
  friend std::enable_if_t<
      std::is_same_v<bluefs_fnode_delta_t, std::remove_const_t<T>>>
...
struct bluefs_fnode_t {
  ...
  bluefs_fnode_t() :
    ino(0), size(0), allocated(0), allocated_commited(0), content_size(0)
  {}
  ...
  void
  clone_extents(const bluefs_fnode_t& fnode)
  {
  ...
  void
  claim_extents(mempool::bluefs::vector<bluefs_extent_t>& extents)
  {
  ...
  void
  append_extent(const bluefs_extent_t& ext)
  {
  ...
  void
  swap_extents(bluefs_fnode_t& other)
  {
...
struct bluefs_layout_t {
  ...
  bool
  single_shared_device() const
  {
    return !dedicated_db && !dedicated_wal;
  }

  bool
  operator==(const bluefs_layout_t& other) const
  {
...
struct bluefs_transaction_t {
  ...
  void
  op_dir_create(std::string_view dir)
  {
    using ceph::encode;
  ...
  void
  op_dir_remove(std::string_view dir)
  {
    using ceph::encode;
  ...
  void
  op_dir_link(std::string_view dir, std::string_view file, uint64_t ino)
  {
    using ceph::encode;
  ...
  void
  op_dir_unlink(std::string_view dir, std::string_view file)
  {
    using ceph::encode;
  ...
  void
  op_file_update(bluefs_fnode_t& file)
  {
    using ceph::encode;
  ...
  void
  op_file_update_inc(bluefs_fnode_t& file)
  {
    using ceph::encode;
  ...
  void
  op_jump(uint64_t next_seq, uint64_t offset)
  {
    using ceph::encode;
  ...
  void
  claim_ops(bluefs_transaction_t& from)
  {
