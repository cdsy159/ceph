...
void
validate_path(CephContext* cct, const string& path, bool bluefs)
{
...
const char*
find_device_path(int id, CephContext* cct, const vector<string>& devs)
{
  for (auto& i : devs) {
...
void
add_devices(BlueFS* fs, CephContext* cct, const vector<string>& devs)
{
  map<string, int> got;
  ...
  for (auto e : got) {
    ...
    int r =
        fs->add_block_device(e.second, e.first, false, 0); // 'reserved' is fake
    if (r < 0) {
...
void
log_dump(CephContext* cct, const string& path, const vector<string>& devs)
{
  validate_path(cct, path, true);
  BlueFS* fs = new BlueFS(cct);
  ...
  int r = fs->log_dump();
  if (r < 0) {
    cerr << "log_dump failed" << ": " << cpp_strerror(r) << std::endl;
    exit(EXIT_FAILURE);
  }
...
void
super_dump(CephContext* cct, const string& path, const vector<string>& devs)
{
  validate_path(cct, path, true);
  BlueFS* fs = new BlueFS(cct);
  ...
  int r = fs->super_dump();
  if (r < 0) {
    cerr << "super_dump failed" << ": " << cpp_strerror(r) << std::endl;
    exit(EXIT_FAILURE);
  }
...
void
inferring_bluefs_devices(vector<string>& devs, std::string& path)
{
    ...
    const vector<string>& devs)
{
  ...
  if (!f) {
    r = -errno;
    cerr << "open " << input_file.c_str() << " failed: " << cpp_strerror(r)
         << std::endl;
    exit(EXIT_FAILURE);
  }
  BlueStore bluestore(cct, path);
...
main(int argc, char** argv)
{
  ...
  po_options.add_options()("help,h", "produce help message")(
      ",i", po::value<string>(&osd_instance),
      "OSD instance. Requires access to monitor/ceph.conf")(
      ...
      "allocator to inspect: 'block'/'bluefs-wal'/'bluefs-db'")(
      "bdev-type", po::value<vector<string>>(&bdev_type),
      "bdev type to inspect: 'bdev-block'/'bdev-wal'/'bdev-db'")(
      "yes-i-really-really-mean-it",
      "additional confirmation for dangerous commands")(
      "sharding", po::value<string>(&new_sharding), "new sharding to apply")(
      "resharding-ctrl", po::value<string>(&resharding_ctrl),
      "gives control over resharding procedure details")(
  ...
  if (!path_aux.empty()) {
    if (path.empty()) {
      ...
      std::cerr << " Ambiguous --data-path and --path options, please provide "
                   "a single one."
                << std::endl;
      exit(EXIT_FAILURE);
    }
  ...
  if (action == "fsck" || action == "repair" || action == "quick-fix" ||
      action == "allocmap" || action == "qfsck" || action == "restore_cfb" ||
  ...
  if (action == "bluefs-export" || action == "bluefs-import" ||
      action == "bluefs-super-dump" || action == "bluefs-log-dump") {
    if (path.empty()) {
      cerr << "must specify bluestore path" << std::endl;
      exit(EXIT_FAILURE);
    }
  ...
  if (action == "trim") {
    ...
    if (!vm.count("yes-i-really-really-mean-it")) {
      cerr << "Trimming a non healthy bluestore is a dangerous operation which "
              "could cause data loss, "
           << "please run fsck and confirm with --yes-i-really-really-mean-it "
              "option"
           << std::endl;
      exit(EXIT_FAILURE);
    }
    for (auto type : bdev_type) {
      if (!type.empty() && type != "bdev-block" && type != "bdev-db" &&
          type != "bdev-wal") {
  ...
  if (action == "restore_cfb") {
    ...
    cerr << action << " bluestore.restore_cfb is not supported!!! "
         << std::endl;
    exit(EXIT_FAILURE);
#else
    ...
    if (r < 0) {
      ...
      action == "fsck" || action == "repair" || action == "quick-fix" ||
      action == "revert-wal-to-plain") {
    validate_path(cct.get(), path, false);
    BlueStore bluestore(cct.get(), path);
    int r;
    ...
    if (!valid_offset) {
      cerr << "Suspicious offset: " << disk_offset
           << ", expected locations: " << bdev_label_positions << std::endl;
    }
    int r = BlueStore::read_bdev_label_at_pos(
        cct.get(), devs[0], disk_offset, &label);
    if (r < 0) {
      cerr << "unable to read label for " << devs[0] << ": " << cpp_strerror(r)
           << std::endl;
      exit(EXIT_FAILURE);
    }
    ...
    int r = BlueStore::create_bdev_labels(
        cct.get(), path, devs, &valid_positions, force);
    if (r == -EPERM && !force) {
      cerr << "device " << devs.front()
           << " already supports bdev label refusing to create a new label "
              "without --yes-i-really-really-mean-it"
           << std::endl;
      exit(EXIT_FAILURE);
    }
    ...
    if (r == -EEXIST && !force) {
      ...
      cerr << "Creating a new label on top of an existing one is a dangerous "
              "operation "
    ...
    bool is_multi = false;
    int64_t epoch = -1;
    int r = BlueStore::read_bdev_label(
        cct.get(), devs.front(), &label, &valid_positions, &is_multi, &epoch);
    if (r < 0) {
    ...
    for (uint64_t position : valid_positions) {
      ...
      if (r < 0) {
        cerr << "unable to write label for " << devs.front() << " at 0x"
    ...
    for (auto& dir : dirs) {
      ...
      for (auto& file : ls) {
        ...
        int fd = ::open(
            path.c_str(), O_CREAT | O_WRONLY | O_TRUNC | O_CLOEXEC, 0644);
        if (fd < 0) {
          r = -errno;
    ...
    if (i != cur_devs_map.end()) {
      ...
      int r = bluestore.migrate_to_new_bluefs_device(
          src_dev_ids, dev_target_id, target_path);
      if (r == 0) {
        for (auto src : src_devs) {
          if (src.second != BlueFS::BDEV_SLOW) {
            cout << " device removed:" << src.second << " " << src.first
    ...
    g_conf().set_val_or_die(
        "bluestore_volume_selection_policy", "use_some_extra_enforced");
    BlueStore bluestore(cct.get(), path);
    int r = bluestore.cold_open();
    if (r < 0) {
      cerr << "error from cold_open: " << cpp_strerror(r) << std::endl;
    ...
    ostringstream err;
    r = admin_socket->execute_command(
        {"{\"prefix\": \"bluefs files list\"}"}, in, err, &out);
    if (r != 0) {
      cerr << "failure querying bluefs stats: " << cpp_strerror(r) << std::endl;
      exit(EXIT_FAILURE);
    }
    ...
    auto get_ctrl = [&](size_t& val) {
      if (!resharding_ctrl.empty()) {
        ...
        if (*endptr != '\0') {
          cerr << "invalid --resharding-ctrl. '" << token << "' is not a number"
               << std::endl;
          exit(EXIT_FAILURE);
        }
      }
    ...
    if (r < 0) {
      cerr << "error preparing db environment: " << cpp_strerror(r)
           << std::endl;
      exit(EXIT_FAILURE);
    }
    ceph_assert(db_ptr);
