...
void
bluestore_bdev_label_t::encode(bufferlist& bl) const
{
...
void
bluestore_bdev_label_t::decode(bufferlist::const_iterator& p)
{
  p += 60u; // see above
  DECODE_START(2, p);
  decode(osd_uuid, p);
...
void
bluestore_bdev_label_t::dump(Formatter* f) const
{
...
list<bluestore_bdev_label_t>
bluestore_bdev_label_t::generate_test_instances()
{
...
ostream&
operator<<(ostream& out, const bluestore_bdev_label_t& l)
{
...
void
bluestore_cnode_t::dump(Formatter* f) const
{
  f->dump_unsigned("bits", bits);
}

std::list<bluestore_cnode_t>
bluestore_cnode_t::generate_test_instances()
{
...
ostream&
operator<<(ostream& out, const bluestore_cnode_t& l)
{
...
void
bluestore_extent_ref_map_t::_check() const
{
  uint64_t pos = 0;
  unsigned refs = 0;
...
void
bluestore_extent_ref_map_t::_maybe_merge_left(
...
void
bluestore_extent_ref_map_t::get(uint64_t offset, uint32_t length)
{
  ...
  while (length > 0) {
    ...
    if (length < p->second.length) {
      ref_map.insert(make_pair(
          offset + length, record_t(p->second.length - length, p->second.refs)));
...
void
bluestore_extent_ref_map_t::put(
    uint64_t offset,
    uint32_t length,
...
bool
bluestore_extent_ref_map_t::contains(uint64_t offset, uint32_t length) const
{
  auto p = ref_map.lower_bound(offset);
  if (p == ref_map.end() || p->first > offset) {
    if (p == ref_map.begin()) {
...
bool
bluestore_extent_ref_map_t::intersects(uint64_t offset, uint32_t length) const
{
  auto p = ref_map.lower_bound(offset);
  if (p != ref_map.begin()) {
    --p;
...
void
bluestore_extent_ref_map_t::dump(Formatter* f) const
{
...
list<bluestore_extent_ref_map_t>
bluestore_extent_ref_map_t::generate_test_instances()
{
...
ostream&
operator<<(ostream& out, const bluestore_extent_ref_map_t& m)
{
  out << "ref_map(";
...
void
bluestore_blob_use_tracker_t::allocate(uint32_t au_count)
{
...
void
bluestore_blob_use_tracker_t::release(uint32_t au_count, uint32_t* ptr)
{
  if (au_count) {
    delete[] ptr;
    mempool::get_pool(
        mempool::pool_index_t(mempool::mempool_bluestore_cache_other))
        .adjust_count(
            -(int32_t)au_count, -(int32_t)(sizeof(uint32_t) * au_count));
  }
}

void
bluestore_blob_use_tracker_t::init(uint32_t full_length, uint32_t _au_size)
{
...
void
bluestore_blob_use_tracker_t::get(uint32_t offset, uint32_t length)
{
  ceph_assert(au_size);
  if (!num_au) {
    total_bytes += length;
...
bool
bluestore_blob_use_tracker_t::put(
    uint32_t offset,
    uint32_t length,
...
bluestore_blob_use_tracker_t::put_simple(uint32_t offset, uint32_t length)
{
  if (num_au == 0) {
...
bool
bluestore_blob_use_tracker_t::can_split() const
{
  return num_au > 0;
}

bool
bluestore_blob_use_tracker_t::can_split_at(uint32_t blob_offset) const
{
  ceph_assert(au_size);
...
void
bluestore_blob_use_tracker_t::split(
...
void
bluestore_blob_use_tracker_t::dup(
    const bluestore_blob_use_tracker_t& from,
    uint32_t start,
    uint32_t len)
{
    ...
    const bluestore_blob_use_tracker_t& other) const
{
  ...
  uint32_t referenced = num_au ? other.get_referenced_bytes()
...
void
bluestore_blob_use_tracker_t::dump(Formatter* f) const
{
...
list<bluestore_blob_use_tracker_t>
bluestore_blob_use_tracker_t::generate_test_instances()
{
...
ostream&
operator<<(ostream& out, const bluestore_blob_use_tracker_t& m)
{
  out << "use_tracker(" << std::hex;
  if (!m.num_au) {
    out << "0x" << m.au_size << " " << "0x" << m.total_bytes;
  } else {
...
void
bluestore_pextent_t::dump(Formatter* f) const
{
...
ostream&
operator<<(ostream& out, const bluestore_pextent_t& o)
{
  if (o.is_valid())
...
list<bluestore_pextent_t>
bluestore_pextent_t::generate_test_instances()
{
...
void
bluestore_blob_t::adjust_to(const bluestore_blob_t& other, uint32_t target_length)
{
...
size_t
bluestore_blob_t::get_csum_value_size() const
{
  return Checksummer::get_csum_value_size(csum_type);
}

void
bluestore_blob_t::dump(Formatter* f) const
{
...
list<bluestore_blob_t>
bluestore_blob_t::generate_test_instances()
{
...
ostream&
operator<<(ostream& out, const bluestore_blob_t& o)
{
...
void
bluestore_blob_t::calc_csum(uint64_t b_off, const bufferlist& bl)
{
  switch (csum_type) {
...
struct vecbuilder {
  ...
  void
  add(uint64_t offset, uint64_t length)
  {
...
void
bluestore_blob_t::allocated_test(const bluestore_pextent_t& alloc)
{
...
bool
bluestore_blob_t::release_extents(
    bool all,
    ...
    PExtentVector* released_disk)
{
  ...
  if (/*offset >= 0 &&*/ offset + length < p->length) {
    ...
    if (offset > 0) {
      ...
      anchor_it =
          extents.insert(anchor_it, 2, bluestore_pextent_t(EMPTY, length));
      ++anchor_it;
      ...
      if (empty) {
        ...
        anchor_it =
            extents.insert(anchor_it, 1, bluestore_pextent_t(EMPTY, length));
        ++anchor_it;
      }
    ...
    if (insert_element_cnt > 0) {
      anchor_it = extents.insert(
          anchor_it, insert_element_cnt, bluestore_pextent_t(0, 0));
    }
    if (insert_element_cnt < 0) {
...
void
bluestore_blob_t::split(uint32_t blob_offset, bluestore_blob_t& rb)
{
...
void
bluestore_blob_t::dup(const bluestore_blob_t& from)
{
...
void
bluestore_shared_blob_t::dump(Formatter* f) const
{
  f->dump_int("sbid", sbid);
  f->dump_object("ref_map", ref_map);
}

list<bluestore_shared_blob_t>
bluestore_shared_blob_t::generate_test_instances()
{
...
ostream&
operator<<(ostream& out, const bluestore_shared_blob_t& sb)
{
...
void
bluestore_onode_t::shard_info::dump(Formatter* f) const
{
  f->dump_unsigned("offset", offset);
...
auto
bluestore_onode_t::shard_info::generate_test_instances() -> list<shard_info>
{
  list<shard_info> o;
  o.emplace_back();
  o.emplace_back();
...
ostream&
operator<<(ostream& out, const bluestore_onode_t::shard_info& si)
{
...
void
bluestore_onode_t::dump(Formatter* f) const
{
...
list<bluestore_onode_t>
bluestore_onode_t::generate_test_instances()
{
...
void
bluestore_deferred_op_t::dump(Formatter* f) const
{
...
list<bluestore_deferred_op_t>
bluestore_deferred_op_t::generate_test_instances()
{
...
void
bluestore_deferred_transaction_t::dump(Formatter* f) const
{
...
list<bluestore_deferred_transaction_t>
bluestore_deferred_transaction_t::generate_test_instances()
{
...
void
bluestore_compression_header_t::dump(Formatter* f) const
{
...
list<bluestore_compression_header_t>
bluestore_compression_header_t::generate_test_instances()
{
...
shared_blob_2hash_tracker_t::build_hash_input(uint64_t sbid, uint64_t offset) const
{
  hash_input_t res = {
      sbid, offset >> au_void_bits,
      ((sbid & 0xffffffff) << 32) +
...
void
shared_blob_2hash_tracker_t::inc(uint64_t sbid, uint64_t offset, int n)
{
  auto hash_input = build_hash_input(sbid, offset);
  ref_counter_2hash_tracker_t::inc(
      (char*)hash_input.data(), get_hash_input_size(), n);
}

void
shared_blob_2hash_tracker_t::inc_range(
    uint64_t sbid,
    uint64_t offset,
    uint32_t len,
    int n)
...
bool
shared_blob_2hash_tracker_t::test_hash_conflict(
    uint64_t sbid1,
    uint64_t offset1,
    uint64_t sbid2,
...
bool
shared_blob_2hash_tracker_t::test_all_zero(uint64_t sbid, uint64_t offset) const
{
  auto hash_input = build_hash_input(sbid, offset);
  return ref_counter_2hash_tracker_t::test_all_zero(
      (char*)hash_input.data(), get_hash_input_size());
}

bool
shared_blob_2hash_tracker_t::test_all_zero_range(
    uint64_t sbid,
    uint64_t offset,
    uint32_t len) const
{
