...
inline interval_t
_align2units(uint64_t offset, uint64_t len, uint64_t min_length)
{
...
interval_t
AllocatorLevel01Loose::_get_longest_from_l0(
    uint64_t pos0,
    uint64_t pos1,
    uint64_t min_length,
    ...
    if ((pos % d) == 0) {
      ...
      if (pos1 - pos >= d) {
        ...
        case all_slot_set:
          ...
          if (end_loop) {
            *tail = res_candidate;
            res_candidate = _align2units(
                res_candidate.offset, res_candidate.length, min_granules);
...
void
AllocatorLevel01Loose::_analyze_partials(
    uint64_t pos_start,
    uint64_t pos_end,
    uint64_t length,
    uint64_t min_length,
  ...
  for (auto pos = pos_start / d; pos < pos_end / d; ++pos) {
    ...
    for (auto c = 0; c < d; c++) {
      ...
      case L1_ENTRY_PARTIAL:
        ...
        longest = _get_longest_from_l0(
            l1_pos * l0_w, (l1_pos + 1) * l0_w, min_length, &prev_tail);
...
void
AllocatorLevel01Loose::_mark_l1_on_l0(int64_t l0_pos, int64_t l0_pos_end)
{
...
void
AllocatorLevel01Loose::_mark_alloc_l0(int64_t l0_pos_start, int64_t l0_pos_end)
{
...
interval_t
AllocatorLevel01Loose::_allocate_l1_contiguous(
    uint64_t length,
    uint64_t min_length,
    uint64_t max_length,
    uint64_t pos_start,
    uint64_t pos_end)
{
  ...
  if (unlikely(length <= l0_granularity)) {
    search_ctx_t ctx;
    _analyze_partials(
        pos_start, pos_end, l0_granularity, l0_granularity, STOP_ON_PARTIAL,
        &ctx);
    ...
    search_ctx_t ctx;
    _analyze_partials(
        pos_start, pos_end, length, min_length, STOP_ON_EMPTY, &ctx);
    ...
    if (ctx.min_affordable_len) {
      ...
      auto pos_end = (ctx.min_affordable_offs + ctx.min_affordable_len) /
                     l0_granularity;
      _mark_alloc_l1_l0(pos_start, pos_end);
    ...
    if (ctx.free_count) {
      ...
      if (aligned_extent.length > 0) {
        ...
        auto pos_end = (aligned_extent.offset + aligned_extent.length) /
                       l0_granularity;
...
bool
AllocatorLevel01Loose::_allocate_l1(
    uint64_t length,
    uint64_t min_length,
    uint64_t max_length,
    ...
    interval_vector_t* res)
{
  ...
  if (min_length != l0_granularity) {
    ...
    while (length > *allocated && has_space) {
      interval_t i = _allocate_l1_contiguous(
          length - *allocated, min_length, max_length, l1_pos_start, l1_pos_end);
      if (i.length == 0) {
        has_space = false;
      } else {
      ...
      if (slot_val == all_slot_clear) {
        ...
        *allocated += to_alloc;
        ...
        _fragment_and_emplace(
            max_length, idx * d1 * l1_granularity, to_alloc, res);
        _mark_alloc_l1_l0(
...
void
AllocatorLevel01Loose::collect_stats(std::map<size_t, size_t>& bins_overall)
{
...
inline ssize_t
AllocatorLevel01Loose::count_0s(slot_t slot_val, size_t start_pos)
{
#ifdef __GNUC__
...
inline ssize_t
AllocatorLevel01Loose::count_1s(slot_t slot_val, size_t start_pos)
{
  return count_0s(~slot_val, start_pos);
}

void
AllocatorLevel01Loose::foreach_internal(
  ...
  for (size_t i = 0; i < l1.size(); i++) {
    for (size_t j = 0; j < L1_ENTRIES_PER_SLOT * L1_ENTRY_WIDTH;
      ...
      case L1_ENTRY_FREE:
        ...
        break;
      case L1_ENTRY_PARTIAL:
        size_t pos = ((bits_per_slot * i + j) / L1_ENTRY_WIDTH) *
                     slots_per_slotset;
...
uint64_t
AllocatorLevel01Loose::_claim_free_to_left_l0(int64_t l0_pos_start)
{
...
uint64_t
AllocatorLevel01Loose::_claim_free_to_right_l0(int64_t l0_pos_start)
{
  auto d0 = L0_ENTRIES_PER_SLOT;
  ...
  if (idx < l0.size() && (*val_s) != all_slot_set &&
      (*val_s) != all_slot_clear) {
    int64_t pos_e = p2roundup<int64_t>(pos + 1, d0);
    slot_t bits = (slot_t)1 << (pos % d0);
    while (pos < pos_e) {
