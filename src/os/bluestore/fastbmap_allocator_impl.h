...
inline size_t
find_next_set_bit(slot_t slot_val, size_t start_pos)
{
#ifdef __GNUC__
  if (start_pos == 0) {
...
protected:
  ...
  uint64_t
  _level_granularity() const override
  {
    return l1_granularity;
  }

  inline bool
  _is_slot_fully_allocated(uint64_t idx) const
  {
...
class AllocatorLevel01Loose : public AllocatorLevel01 {
  ...
  uint64_t
  _children_per_slot() const override
  {
    ...
    for (auto idx = l0_pos0 / d0; (idx < l0_pos1 / d0) && (length > *allocated);
      ...
      if (slot_val == all_slot_clear) {
        ...
        *allocated += to_alloc * l0_granularity;
        ...
        _fragment_and_emplace(
            max_length, base * l0_granularity, to_alloc * l0_granularity, res);
      ...
      while (next_pos < bits_per_slot && (next_pos - free_pos) < need_entries) {
        ...
        if (0 == (slot_val & (slot_t(1) << next_pos))) {
          ...
          *allocated += to_alloc * l0_granularity;
          ...
          need_entries -= to_alloc;
          _fragment_and_emplace(
              max_length, (base + free_pos) * l0_granularity,
...
protected:
  ...
  void
  _init(uint64_t capacity, uint64_t _alloc_unit, bool mark_as_free = true)
  {
    ...
    size_t slot_count = aligned_capacity / l1_granularity /
                        _children_per_slot();
  ...
  void
  _mark_alloc_l1_l0(int64_t l0_pos_start, int64_t l0_pos_end)
  {
  ...
  void
  _mark_free_l0(int64_t l0_pos_start, int64_t l0_pos_end)
  {
  ...
  void
  _mark_free_l1_l0(int64_t l0_pos_start, int64_t l0_pos_end)
  {
  ...
  bool
  _is_empty_l0(uint64_t l0_pos, uint64_t l0_pos_end)
  {
    bool no_free = true;
  ...
  bool
  _is_empty_l1(uint64_t l1_pos, uint64_t l1_pos_end)
  {
    bool no_free = true;
  ...
  interval_t _allocate_l1_contiguous(
      uint64_t length,
      uint64_t min_length,
      uint64_t max_length,
      uint64_t pos_start,
  ...
  uint64_t
  _mark_alloc_l1(uint64_t offset, uint64_t length)
  {
    uint64_t l0_pos_start = offset / l0_granularity;
    uint64_t l0_pos_end = p2roundup(offset + length, l0_granularity) /
                          l0_granularity;
  ...
  uint64_t
  _free_l1(uint64_t offs, uint64_t len)
  {
    uint64_t l0_pos_start = offs / l0_granularity;
    uint64_t l0_pos_end = p2roundup(offs + len, l0_granularity) /
                          l0_granularity;
  ...
  uint64_t
  claim_free_to_left_l1(uint64_t offs)
  {
  ...
  uint64_t
  claim_free_to_right_l1(uint64_t offs)
  {
    uint64_t l0_pos_start = offs / l0_granularity;
...
public:
  uint64_t
  debug_get_allocated(uint64_t pos0 = 0, uint64_t pos1 = 0)
  {
    if (pos1 == 0) {
  ...
  uint64_t
  debug_get_free(uint64_t l1_pos0 = 0, uint64_t l1_pos1 = 0)
  {
...
public:
  void
  collect_stats(std::map<size_t, size_t>& bins_overall) override
  {
    // not implemented
  }
};
...
public:
  uint64_t
  debug_get_free(uint64_t pos0 = 0, uint64_t pos1 = 0)
  {
    std::lock_guard l(lock);
    return l1.debug_get_free(
  ...
  uint64_t
  claim_free_to_left(uint64_t offset)
  {
    std::lock_guard l(lock);
    ...
    uint64_t l2_pos_end = p2roundup(int64_t(offset), int64_t(l2_granularity)) /
                          l2_granularity;
  ...
  uint64_t
  claim_free_to_right(uint64_t offset)
  {
    std::lock_guard l(lock);
    ...
    uint64_t l2_pos_end = p2roundup(end, int64_t(l2_granularity)) /
                          l2_granularity;
    _mark_l2_on_l1(l2_pos, l2_pos_end);
    return allocated;
  }
  ...
  void
  foreach_internal(std::function<void(uint64_t offset, uint64_t length)> notify)
  {
...
protected:
  ...
  _init(uint64_t capacity, uint64_t _alloc_unit, bool mark_as_free = true)
  {
    ...
    l2_granularity = l1._level_granularity() * l1._children_per_slot() *
                     slots_per_slotset;
  ...
  void
  _mark_l2_allocated(int64_t l2_pos, int64_t l2_pos_end)
  {
  ...
  void
  _mark_l2_free(int64_t l2_pos, int64_t l2_pos_end)
  {
    auto d = L1_ENTRIES_PER_SLOT;
    ceph_assert(0 <= l2_pos_end);
    ceph_assert((int64_t)l2.size() >= (l2_pos_end / d));
  ...
  void
  _mark_l2_on_l1(int64_t l2_pos, int64_t l2_pos_end)
  {
    auto d = L1_ENTRIES_PER_SLOT;
    ceph_assert(0 <= l2_pos_end);
    ceph_assert((int64_t)l2.size() >= (l2_pos_end / d));
      ...
      interval_vector_t* res)
  {
    ...
    if (hint != -1) {
      last_pos = (hint / (d * l2_granularity)) < l2.size()
                     ? p2align(hint / l2_granularity, d)
                     : 0;
    }
  ...
  void
  _free_l2(const interval_set<uint64_t>& rr)
  {
    uint64_t released = 0;
    std::lock_guard l(lock);
    for (auto r : rr) {
  ...
  void
  _mark_allocated(uint64_t o, uint64_t len)
  {
    uint64_t l2_pos = o / l2_granularity;
    uint64_t l2_pos_end = p2roundup(int64_t(o + len), int64_t(l2_granularity)) /
                          l2_granularity;
  ...
  void
  _mark_free(uint64_t o, uint64_t len)
  {
    uint64_t l2_pos = o / l2_granularity;
    uint64_t l2_pos_end = p2roundup(int64_t(o + len), int64_t(l2_granularity)) /
                          l2_granularity;
