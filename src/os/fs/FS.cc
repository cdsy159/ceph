...
int
FS::set_alloc_hint(int fd, uint64_t hint)
{
  return 0; // no-op
}
...
int
FS::get_handle(int fd, std::string* h)
{
...
int
FS::open_handle(int mount_fd, const std::string& h, int flags)
{
...
int
FS::zero(int fd, uint64_t offset, uint64_t length)
{
  int r;
