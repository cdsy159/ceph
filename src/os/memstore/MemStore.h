...
public:
  struct Object : public RefCountedObject {
    ...
    ceph::shared_mutex omap_mutex{
        ceph::make_shared_mutex("MemStore::Object::omap_mutex")};
    ...
    virtual int
    clone(Object* src, uint64_t srcoff, uint64_t len, uint64_t dstoff) = 0;
    ...
    void
    encode_base(ceph::buffer::list& bl) const
    {
      using ceph::encode;
      encode(xattr, bl);
    ...
    void
    decode_base(ceph::buffer::list::const_iterator& p)
    {
      using ceph::decode;
      decode(xattr, p);
  ...
  struct Collection : public CollectionImpl {
    ...
    ObjectRef
    get_or_create_object(ghobject_t oid)
    {
    ...
    bool
    flush_commit(Context* c) override
    {
      return true;
    }
...
private:
  ...
  int
  _zero(const coll_t& cid, const ghobject_t& oid, uint64_t offset, size_t len);
  ...
  int
  _collection_hint_expected_num_objs(
      const coll_t& cid,
      uint32_t pg_num,
...
public:
  MemStore(CephContext* cct, const std::string& path) :
    ObjectStore(cct, path), finisher(cct), used_bytes(0)
  {}
  ...
  int
  get_devices(std::set<std::string>* ls) override
  {
    // no devices for us!
    return 0;
  }
  ...
  CollectionHandle
  open_collection(const coll_t& c) override
  {
  ...
  void
  refresh_perf_counters() override
  {}
