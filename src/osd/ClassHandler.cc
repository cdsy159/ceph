...
int
ClassHandler::open_class(const string& cname, ClassData** pcls)
{
...
ClassHandler::open_all_classes()
{
  ...
  while ((pde = ::readdir(dir))) {
    ...
    if (strlen(pde->d_name) > sizeof(CLS_PREFIX) - 1 + sizeof(CLS_SUFFIX) - 1 &&
        ...
        strcmp(
            pde->d_name + strlen(pde->d_name) - (sizeof(CLS_SUFFIX) - 1),
            CLS_SUFFIX) == 0) {
 ...
 */
bool
ClassHandler::in_class_list(const std::string& cname, const std::string& list)
{
...
ClassHandler::_get_class(const string& cname, bool check_allowed)
{
  ClassData* cls;
...
int
ClassHandler::_load_class(ClassData* cls)
{
  // already open
  if (cls->status == ClassData::CLASS_OPEN)
    return 0;
  ...
  if (cls->status == ClassData::CLASS_UNKNOWN ||
    ...
    char fname[PATH_MAX];
    snprintf(
        fname, sizeof(fname), "%s/" CLS_PREFIX "%s" CLS_SUFFIX,
...
void
ClassHandler::unregister_class(ClassHandler::ClassData* cls)
{
...
ClassHandler::ClassMethod*
ClassHandler::ClassData::register_cxx_method(
    const char* mname,
    int flags,
...
int
ClassHandler::ClassData::get_method_flags(const std::string& mname)
{
...
void
ClassHandler::ClassData::unregister_method(ClassHandler::ClassMethod* method)
{
  /* no need for locking, called under the class_init mutex */
...
void
ClassHandler::ClassMethod::unregister()
{
  cls->unregister_method(this);
}

void
ClassHandler::ClassData::unregister_filter(ClassHandler::ClassFilter* filter)
{
  /* no need for locking, called under the class_init mutex */
...
void
ClassHandler::ClassFilter::unregister()
{
  cls->unregister_filter(this);
}

int
ClassHandler::ClassMethod::exec(
