...
public:
  ...
  void
  merge(const DynamicPerfStats& dps)
  {
    for (auto& query_it : dps.data) {
      ...
      for (auto& key_it : query_it.second) {
        ...
        auto update_counter_fnc = [&counter_it](
  ...
  void
  set_queries(const std::list<OSDPerfMetricQuery>& queries)
  {
    std::map<OSDPerfMetricQuery, std::map<OSDPerfMetricKey, PerformanceCounters>>
        new_data;
    for (auto& query : queries) {
      ...
      const utime_t& latency)
  {
    ...
    auto update_counter_fnc = [&op, inb, outb, &latency](
                                  const PerformanceCounterDescriptor& d,
                                  PerformanceCounter* c) {
    ...
    auto get_subkey_fnc = [osd, &pg_info, &op](
...
private:
  static bool
  is_limited(const OSDPerfMetricLimits& limits, size_t counters_size)
  {
    if (limits.empty()) {
      return false;
    }
