...
static ostream&
_prefix(std::ostream* _dout, ECBackendL* pgb)
{
  return pgb->get_parent()->gen_dbg_prefix(*_dout) << "ECBackendL ";
}

static ostream&
_prefix(std::ostream* _dout, ECBackendL::RecoveryBackend* pgb)
{
  return pgb->get_parent()->gen_dbg_prefix(*_dout) << "ECBackendL ";
}
...
static ostream&
operator<<(ostream& lhs, const map<pg_shard_t, bufferlist>& rhs)
{
  lhs << "[";
...
static ostream&
operator<<(ostream& lhs, const map<int, bufferlist>& rhs)
{
  lhs << "[";
...
ostream&
operator<<(ostream& lhs, const ECBackendL::RecoveryBackend::RecoveryOp& rhs)
{
  return lhs << "RecoveryOp(" << "hoid=" << rhs.hoid << " v=" << rhs.v
             ...
             << " obc refcount=" << rhs.obc.use_count() << " state="
             << ECBackendL::RecoveryBackend::RecoveryOp::tostr(rhs.state)
             << " waiting_on_pushes=" << rhs.waiting_on_pushes
             << " extent_requested=" << rhs.extent_requested << ")";
}

void
ECBackendL::RecoveryBackend::RecoveryOp::dump(Formatter* f) const
{
...
PGBackend::RecoveryHandle*
ECBackendL::open_recovery_op()
{
...
PGBackend::RecoveryHandle*
ECBackendL::RecoveryBackend::open_recovery_op()
{
  return new ECRecoveryHandle;
}

void
ECBackendL::RecoveryBackend::_failed_push(
...
struct RecoveryMessages {
  map<hobject_t, ECCommonL::read_request_t> recovery_reads;
      ...
      bool attrs)
  {
    ...
    recovery_reads.insert(
        make_pair(hoid, ECCommonL::read_request_t(to_read, need, attrs)));
  }
...
void
ECBackendL::handle_recovery_push(
    const PushOp& op,
    RecoveryMessages* m,
    bool is_repair)
{
...
void
ECBackendL::RecoveryBackend::handle_recovery_push(
    const PushOp& op,
    RecoveryMessages* m,
    bool is_repair)
{
...
void
ECBackendL::RecoveryBackend::handle_recovery_push_reply(
...
void
ECBackendL::RecoveryBackend::handle_recovery_read_complete(
    const hobject_t& hoid,
    ...
    RecoveryMessages* m)
{
  ...
  if (attrs) {
    ...
    op.hinfo =
        unstable_hashinfo_registry.maybe_put_hash_info(hoid, std::move(hinfo));
  }
...
struct RecoveryReadCompleter : ECCommonL::ReadCompleter {
      ...
      set<int> wanted_to_read) override
  {
    ...
    backend.handle_recovery_read_complete(
        hoid, res.returned.back(), res.attrs, &rm);
  }
...
void
ECBackendL::ECRecoveryBackend::commit_txn_send_replies(
    ceph::os::Transaction&& txn,
    std::map<int, MOSDPGPushReply*> replies)
{
  txn.register_on_complete(get_parent()->bless_context(
      new SendPushReplies(get_parent(), get_osdmap_epoch(), replies)));
  get_parent()->queue_transaction(std::move(txn));
}

void
ECBackendL::RecoveryBackend::dispatch_recovery_messages(
    RecoveryMessages& m,
    int priority)
{
  ...
  read_pipeline.start_read_op(
      priority, m.want_to_read, m.recovery_reads, OpRequestRef(), false, true,
      std::make_unique<RecoveryReadCompleter>(*this));
}

void
ECBackendL::RecoveryBackend::continue_recovery_op(
  ...
  while (1) {
    ...
    case RecoveryOp::IDLE: {
      ...
      if (op.recovery_progress.first && op.obc) {
        if (auto [r, attrs, size] =
                ecbackend->get_attrs_n_size_from_disk(op.hoid);
            r >= 0 || r == -ENOENT) {
        ...
        if (!op.hinfo) {
          ...
          get_parent()->on_failed_pull(
              {get_parent()->whoami_shard()}, op.hoid, v);
          return;
        }
      ...
      m->recovery_read(
          op.hoid, op.recovery_progress.data_recovered_to, amount,
    ...
    case RecoveryOp::READING: {
      ...
      for (set<pg_shard_t>::iterator mi = op.missing_on.begin();
        ...
        ceph_assert(
            pop.data.length() == sinfo.aligned_logical_offset_to_chunk_offset(
                                     after_progress.data_recovered_to -
...
void
ECBackendL::run_recovery_op(PGBackend::RecoveryHandle* _h, int priority)
{
  ceph_assert(_h);
...
void
ECBackendL::RecoveryBackend::run_recovery_op(ECRecoveryHandle& h, int priority)
{
  RecoveryMessages m;
...
int
ECBackendL::RecoveryBackend::recover_object(
    const hobject_t& hoid,
    eversion_t v,
    ObjectContextRef head,
    ObjectContextRef obc,
...
bool
ECBackendL::can_handle_while_inactive(OpRequestRef _op)
{
  return false;
}

bool
ECBackendL::_handle_message(OpRequestRef _op)
{
  ...
  case MSG_OSD_EC_WRITE: {
    ...
    handle_sub_write(
        op->op.from, _op, op->op, _op->pg_trace,
...
void
ECBackendL::sub_write_committed(
    ceph_tid_t tid,
    eversion_t version,
    eversion_t last_complete,
    const ZTracer::Trace& trace)
    ...
    ECListener&)
{
  ...
  get_parent()->log_operation(
      std::move(op.log_entries), op.updated_hit_set_history, op.trim_to,
      op.pg_committed_to, op.pg_committed_to, !op.backfill_or_async_recovery,
      localt, async);
  ...
  localt.register_on_commit(get_parent()->bless_context(new SubWriteCommitted(
      this, msg, op.tid, op.at_version, get_parent()->get_info().last_complete,
      trace)));
  ...
  for (auto i = op.to_read.begin(); i != op.to_read.end(); ++i) {
    ...
    for (auto j = i->second.begin(); j != i->second.end(); ++j) {
      ...
      if ((op.subchunks.find(i->first)->second.size() == 1) &&
        ...
        dout(20) << __func__ << " case1: reading the complete chunk/shard."
                 << dendl;
        r = switcher->store->read(
            switcher->ch, ghobject_t(i->first, ghobject_t::NO_GEN, shard),
            j->get<0>(), j->get<1>(), bl, j->get<2>()); // Allow EIO return
      } else {
        int subchunk_size = sinfo.get_chunk_size() /
                            ec_impl->get_sub_chunk_count();
      ...
      if (!get_parent()->get_pool().allows_ecoverwrites()) {
        ...
        if (r >= 0) {
          dout(10) << __func__ << ": found on disk, size " << st.st_size
                   << dendl;
          r = switcher->objects_get_attrs_with_hinfo(i->first, &attrs);
        }
        if (r >= 0) {
          hinfo = unstable_hashinfo_registry.get_hash_info(
              i->first, false, attrs, st.st_size);
        } else {
          derr << __func__ << ": access (attrs) on " << i->first
        ...
        if ((bl.length() == hinfo->get_total_chunk_size()) &&
          ...
          if (h.digest() != hinfo->get_chunk_hash(s)) {
            get_parent()->clog_error()
                ...
                << h.digest() << " expected 0x" << hinfo->get_chunk_hash(s)
                << dec;
            dout(5) << __func__ << ": Bad hash for " << i->first << " digest 0x"
                    << hex << h.digest() << " expected 0x"
                    << hinfo->get_chunk_hash(s) << dec << dendl;
            r = -EIO;
            goto error;
          }
        }
...
void
ECBackendL::handle_sub_write_reply(
    pg_shard_t from,
    const ECSubWriteReply& op,
    const ZTracer::Trace& trace)
{
  map<ceph_tid_t, RMWPipeline::OpRef>::iterator i =
      rmw_pipeline.tid_to_op_map.find(op.tid);
  ...
  if (cct->_conf->bluestore_debug_inject_read_err &&
    ...
    std::string cmd = "{ \"prefix\": \"osd down\", \"ids\": [\"" +
                      std::to_string(get_parent()->whoami()) + "\"] }";
    vector<std::string> vcmd{cmd};
    dout(0) << __func__ << " Error inject - marking OSD down" << dendl;
    get_parent()->start_mon_command(
        std::move(vcmd), {}, nullptr, nullptr, nullptr);
  }
  rmw_pipeline.check_ops();
}

void
ECBackendL::handle_sub_read_reply(
    pg_shard_t from,
    ECSubReadReply& op,
    const ZTracer::Trace& trace)
{
  ...
  map<ceph_tid_t, ReadOp>::iterator iter =
      read_pipeline.tid_to_read_map.find(op.tid);
  ...
  for (auto i = op.buffers_read.begin(); i != op.buffers_read.end(); ++i) {
    ...
    list<boost::tuple<uint64_t, uint64_t, map<pg_shard_t, bufferlist>>>::iterator
        riter = rop.complete[i->first].returned.begin();
    for (list<pair<uint64_t, bufferlist>>::iterator j = i->second.begin();
      ...
      pair<uint64_t, uint64_t> aligned = sinfo.chunk_aligned_offset_len_to_chunk(
  ...
  if (rop.do_redundant_reads || rop.in_progress.empty()) {
    for (map<hobject_t, read_result_t>::const_iterator iter =
      ...
      int err;
      if ((err = ec_impl->minimum_to_decode(
               rop.want_to_read[iter->first], have, &dummy_minimum)) < 0) {
...
void
ECBackendL::check_recovery_sources(const OSDMapRef& osdmap)
{
  struct FinishReadOp : public GenContext<ThreadPool::TPHandle&> {
    ...
    void
    finish(ThreadPool::TPHandle&) override
    {
  ...
  read_pipeline.check_recovery_sources(
      osdmap,
      [this](const hobject_t& obj) { recovery_backend.recovery_ops.erase(obj); },
...
void
ECBackendL::clear_recovery_state()
{
  recovery_backend.recovery_ops.clear();
}

void
ECBackendL::dump_recovery_info(Formatter* f) const
{
  ...
  for (map<hobject_t, RecoveryBackend::RecoveryOp>::const_iterator i =
       ...
       i != recovery_backend.recovery_ops.end(); ++i) {
  ...
  for (map<ceph_tid_t, ReadOp>::const_iterator i =
           read_pipeline.tid_to_read_map.begin();
       i != read_pipeline.tid_to_read_map.end(); ++i) {
...
struct ECClassicalOp : ECCommonL::RMWPipeline::Op {
      ...
      const ceph_release_t require_osd_release) final
  {
    ...
    ECTransactionL::generate_transactions(
        t.get(), plan, ecimpl, pgid, sinfo, remote_read_result, log_entries,
        written, transactions, &temp_added, &temp_cleared, dpp,
        require_osd_release);
  }

  template <typename F>
  static ECTransactionL::WritePlan
  get_write_plan(
      const ECUtilL::stripe_info_t& sinfo,
      PGTransaction& t,
      F&& get_hinfo,
      DoutPrefixProvider* dpp)
  {
    return ECTransactionL::get_write_plan(
        sinfo, t, std::forward<F>(get_hinfo), dpp);
  }
};

std::tuple<int, map<string, bufferlist, less<>>, size_t>
    ...
    OpRequestRef client_op)
{
  ...
  op->plan = op->get_write_plan(
      sinfo, *(op->t),
      [&](const hobject_t& i) {
        dout(10) << "submit_transaction: obtaining hash info for get_write_plan"
                 << dendl;
        ECUtilL::HashInfoRef ref;
        if (auto [r, attrs, size] = get_attrs_n_size_from_disk(i);
            r >= 0 || r == -ENOENT) {
        ...
        if (!ref) {
          derr << __func__ << ": get_hash_info(" << i << ")"
               ...
               << " way to recover from such an error in this " << " context"
               << dendl;
          ceph_abort();
        }
        return ref;
    ...
    bool fast_read)
{
  ...
  struct cb {
    ECBackendL* ec;
    hobject_t hoid;
    list<pair<ec_align_t, pair<bufferlist*, Context*>>> to_read;
    ...
    cb(ECBackendL* ec,
       ...
       Context* on_complete,
       CephContext* cct) :
      ec(ec), hoid(hoid), to_read(to_read), on_complete(on_complete), cct(cct)
    {}

    void
    operator()(ECCommonL::ec_extents_t&& results)
    {
      auto dpp = ec->get_parent()->get_dpp();
      ldpp_dout(dpp, 20) << "objects_read_async_cb: got: " << results << dendl;
      ldpp_dout(dpp, 20) << "objects_read_async_cb: cache: "
                         << ec->rmw_pipeline.cache << dendl;
      ...
      for (auto&& read : to_read) {
        if (got.err < 0) {
          ...
          read.second.first->substr_of(
              range.first.get_val(), offset - range_offset, length);
          if (read.second.second) {
  ...
  objects_read_and_reconstruct(
      reads, fast_read,
      make_gen_lambda_context<ECCommonL::ec_extents_t&&, cb>(
          cb(this, hoid, to_read, on_complete, cct)));
}

void
ECBackendL::objects_read_and_reconstruct(
...
void
ECBackendL::kick_reads()
{
  read_pipeline.kick_reads();
}

int
ECBackendL::object_stat(const hobject_t& hoid, struct stat* st)
{
  int r = switcher->store->stat(
      switcher->ch,
    ...
    ScrubMap::object& o)
{
  ...
  r = switcher->store->read(
      ...
      pos.data_pos, stride, bl, ECCommonL::scrub_fadvise_flags);
  if (r < 0) {
  ...
  ECUtilL::HashInfoRef hinfo =
      unstable_hashinfo_registry.get_hash_info(poid, false, o.attrs, o.size);
  if (!hinfo) {
    ...
    if (!get_parent()->get_pool().allows_ecoverwrites()) {
      ...
      if (hinfo->get_chunk_hash(s) != pos.data_hash.digest()) {
        dout(0) << "_scan_list  " << poid << " got incorrect hash on read 0x"
                ...
                << hinfo->get_chunk_hash(s) << std::dec << dendl;
