...
public:
  ...
  void run_recovery_op(PGBackend::RecoveryHandle* h, int priority);
  ...
  void
  call_write_ordered(std::function<void(void)>&& cb)
  {
...
public:
  struct RecoveryBackend {
    ...
    PGBackend::RecoveryHandle* open_recovery_op();
    void run_recovery_op(struct ECRecoveryHandle& h, int priority);
    int recover_object(
        const hobject_t& hoid,
        eversion_t v,
        ObjectContextRef head,
...
public:
  ...
  public:
    ...
    bool
    operator()(const std::set<pg_shard_t>& _have) const override
    {
      std::set<int> have;
  ...
  bool
  get_ec_supports_crc_encode_decode() const
  {
    return false;
  }

  uint64_t
  object_size_to_shard_size(const uint64_t size) const
  {
  ...
  public:
    ECReadPred(pg_shard_t whoami, ceph::ErasureCodeInterfaceRef ec_impl) :
      whoami(whoami), rec_pred(ec_impl)
    {}
  ...
  std::tuple<int, std::map<std::string, ceph::bufferlist, std::less<>>, size_t>
...
public:
  ...
  uint64_t
  be_get_ondisk_size(uint64_t logical_size) const
  {
...
ostream& operator<<(
    ostream& lhs,
    const ECBackendL::RMWPipeline::pipeline_state_t& rhs);
} // namespace ECLegacy
