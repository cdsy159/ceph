...
static ostream&
_prefix(std::ostream* _dout, ECCommon::RMWPipeline const* rmw_pipeline)
{
  return rmw_pipeline->get_parent()->gen_dbg_prefix(*_dout);
}

static ostream&
_prefix(std::ostream* _dout, ECCommon::ReadPipeline const* read_pipeline)
{
  return read_pipeline->get_parent()->gen_dbg_prefix(*_dout);
}

static ostream&
_prefix(std::ostream* _dout, ECCommon::RecoveryBackend* pgb)
{
...
void
ECCommon::ReadOp::dump(Formatter* f) const
{
...
void
ECCommon::RecoveryBackend::RecoveryOp::dump(Formatter* f) const
{
...
void
ECCommon::ReadPipeline::complete_read_op(ReadOp&& rop)
{
  ...
  for (; req_iter != rop.to_read.end(); ++req_iter, ++resiter) {
    ...
    rop.on_complete->finish_single_request(hoid, std::move(res), req);
  }
...
void
ECCommon::ReadPipeline::on_change()
{
...
void
ECCommon::ReadPipeline::get_all_avail_shards(
...
int
ECCommon::ReadPipeline::get_min_avail_to_read_shards(
    const hobject_t& hoid,
    bool for_recovery,
    ...
    const std::optional<set<pg_shard_t>>& error_shards)
{
  ...
  if (kth_iter != want.end()) {
    ...
    r = ec_impl->minimum_to_decode(
        want_for_plugin, have, need_set, need_sub_chunks.get());
  } else {
    r = ec_impl->minimum_to_decode(want, have, need_set, need_sub_chunks.get());
  }
  ...
  dout(20) << __func__ << " for_recovery: " << for_recovery
           ...
           << " error_shards: " << error_shards << dendl;
  return 0;
}
    ...
    ECUtil::shard_extent_set_t& want_shard_reads)
{
  sinfo.ro_range_to_shard_extent_set(
      to_read.offset, to_read.size, want_shard_reads);
  dout(20) << __func__ << ": to_read " << to_read << " read_request "
           << want_shard_reads << dendl;
}

int
ECCommon::ReadPipeline::get_remaining_shards(
    const hobject_t& hoid,
    ...
    bool want_attrs)
{
  ...
  const int r = get_min_avail_to_read_shards(
      hoid, for_recovery, false, read_request, error_shards);
...
void
ECCommon::ReadPipeline::start_read_op(
    const int priority,
    ...
    std::unique_ptr<ReadCompleter> on_complete)
{
  ...
  auto& op = tid_to_read_map
                 .emplace(
                     tid, ReadOp(
                              priority, tid, do_redundant_reads, for_recovery,
                              std::move(on_complete), std::move(to_read)))
                 .first->second;
...
void
ECCommon::ReadPipeline::do_read_op(ReadOp& rop)
{
  ...
  for (auto&& [hoid, read_request] : rop.to_read) {
    ...
    for (auto& [_, shard_read] : read_request.shard_reads) {
      ...
      rop.debug_log.emplace_back(
          ECUtil::READ_REQUEST, shard_read.pg_shard, shard_read.extents);
  ...
  for (auto&& [pg_shard, read] : messages) {
    ...
    dout(10) << __func__ << ": will send msg " << *msg << " to osd." << pg_shard
             << dendl;
  }
  if (!m.empty()) {
...
void
ECCommon::ReadPipeline::get_want_to_read_shards(
...
void
ECCommon::ReadPipeline::get_want_to_read_all_shards(
    const list<ec_align_t>& to_read,
    ECUtil::shard_extent_set_t& want_shard_reads)
{
  for (const auto& single_region : to_read) {
    sinfo.ro_range_to_shard_extent_set_with_parity(
        single_region.offset, single_region.size, want_shard_reads);
  }
  dout(20) << __func__ << ": to_read " << to_read << " read_request "
           << want_shard_reads << dendl;
}
 ...
 */
void
ECCommon::ReadPipeline::create_parity_read_buffer(
...
struct ClientReadCompleter final : ECCommon::ReadCompleter {
  ClientReadCompleter(
    ...
    read_pipeline(read_pipeline), status(status)
  {}
      ...
      ECCommon::read_request_t& req) override
  {
...
#endif
    dout(20) << __func__ << " completing hoid=" << hoid << " res=" << res
             << " req=" << req << dendl;
    extent_map result;
    if (res.r == 0) {
      ceph_assert(res.errors.empty());
      dout(30) << __func__
               << ": before decode: " << res.buffers_read.debug_string(2048, 0)
               << dendl;
      /* Decode any missing buffers */
      res.buffers_read.add_zero_padding_for_decode(req.zeros_for_decode);
      int r = res.buffers_read.decode(
          read_pipeline.ec_impl, req.shard_want_to_read, req.object_size,
          read_pipeline.get_parent()->get_dpp());
      ceph_assert(r == 0);
      dout(30) << __func__
               << ": after decode: " << res.buffers_read.debug_string(2048, 0)
               << dendl;
      ...
      for (auto&& read : req.to_read) {
        ...
        if (cct->_conf->bluestore_debug_inject_read_err &&
          ...
          read_pipeline.create_parity_read_buffer(
              res.buffers_read, read, &data_and_parity);
    ...
    dout(20) << __func__ << " calling complete_object with result=" << result
             << dendl;
    status->complete_object(
        hoid, res.r, std::move(result), std::move(res.buffers_read));
...
static ostream&
_prefix(std::ostream* _dout, ClientReadCompleter const* read_completer)
{
  return _prefix(_dout, &read_completer->read_pipeline);
}

void
ECCommon::ReadPipeline::objects_read_and_reconstruct(
    ...
    GenContextURef<ec_extents_t&&>&& func)
{
  ...
  for (auto&& [hoid, to_read] : reads) {
    ...
    const int r =
        get_min_avail_to_read_shards(hoid, false, fast_read, read_request);
    ceph_assert(r == 0);

    const int subchunk_size = sinfo.get_chunk_size() /
  ...
  start_read_op(
      CEPH_MSG_PRIO_DEFAULT, for_read_op, fast_read, false,
      std::make_unique<ClientReadCompleter>(
          *this, &(in_progress_client_reads.back())));
}

void
ECCommon::ReadPipeline::objects_read_and_reconstruct_for_rmw(
    map<hobject_t, read_request_t>&& to_read,
    GenContextURef<ec_extents_t&&>&& func)
{
  ...
  for (auto&& [hoid, read_request] : to_read) {
    ...
    const int subchunk_size = sinfo.get_chunk_size() /
                              ec_impl->get_sub_chunk_count();
    dout(20) << __func__ << " read_request=" << read_request
             << " subchunk_size=" << subchunk_size
             << " chunk_size=" << sinfo.get_chunk_size() << dendl;
  ...
  start_read_op(
      CEPH_MSG_PRIO_DEFAULT, for_read_op, false, false,
      std::make_unique<ClientReadCompleter>(
          *this, &(in_progress_client_reads.back())));
}

int
ECCommon::ReadPipeline::send_all_remaining_reads(
    const hobject_t& hoid,
    ReadOp& rop)
{
  // (Note cuixf) If we need to read attrs and we read failed, try to read again.
  const bool want_attrs = rop.to_read.at(hoid).want_attrs &&
                          (!rop.complete.at(hoid).attrs ||
                           rop.complete.at(hoid).attrs->empty());
  if (want_attrs) {
  ...
  return get_remaining_shards(
      hoid, rop.complete.at(hoid), read_request, rop.for_recovery, want_attrs);
}

void
ECCommon::ReadPipeline::kick_reads()
{
...
bool
ec_align_t::operator==(const ec_align_t& other) const
{
  return offset == other.offset && size == other.size && flags == other.flags;
}

bool
ECCommon::shard_read_t::operator==(const shard_read_t& other) const
{
...
bool
ECCommon::read_request_t::operator==(const read_request_t& other) const
{
  return to_read == other.to_read && flags == other.flags &&
         shard_want_to_read == other.shard_want_to_read &&
         shard_reads == other.shard_reads && want_attrs == other.want_attrs;
}

void
ECCommon::RMWPipeline::start_rmw(OpRef op)
{
  ...
  for (auto& plan : op->plan.plans) {
    ECExtentCache::OpRef cache_op = extent_cache.prepare(
        plan.hoid, plan.to_read, plan.will_write, plan.orig_size,
        plan.projected_size, plan.invalidates_cache,
...
void
ECCommon::RMWPipeline::cache_ready(Op& op)
{
  ...
  op.generate_transactions(
      ec_impl, get_parent()->get_info().pgid.pgid, sinfo, &written, &trans,
      get_parent()->get_dpp(), get_osdmap(), first_write_in_interval);
  ...
  for (auto&& pg_shard : get_parent()->get_acting_recovery_backfill_shards()) {
    ...
    if (transaction.empty()) {
      dout(20) << __func__ << " Transaction for osd." << pg_shard.osd
               << " shard " << shard << " is empty" << dendl;
    } else {
      // NOTE: All code between dout and dendl is executed conditionally on
      //       debug level.
      dout(20) << __func__ << " Transaction for osd." << pg_shard.osd
               << " shard " << shard << " contents ";
     ...
     */
    if (!next_write_all_shards && should_send &&
        op.skip_transaction(pending_roll_forward, shard, transaction)) {
      // Must be an empty transaction
      ceph_assert(transaction.empty());
      dout(20) << __func__ << " Skipping transaction for shard " << shard
               << dendl;
      continue;
    }
    if (!should_send || transaction.empty()) {
      dout(20) << __func__ << " Sending empty transaction for shard " << shard
               << dendl;
    }
    op.pending_commits++;
    ...
    ECSubWrite sop(
        get_parent()->whoami_shard(), op.tid, op.reqid, op.hoid, stats,
        should_send ? transaction : empty, op.version, op.trim_to,
        op.pg_committed_to, op.log_entries, op.updated_hit_set_history,
        op.temp_added, op.temp_cleared, !should_send);
    ...
    if (pg_shard == get_parent()->whoami_shard()) {
      ...
      local_write_op.claim(sop);
        ...
        cct->_conf->bluestore_debug_inject_read_err &&
        ECInject::test_write_error1(
            ghobject_t(op.hoid, ghobject_t::NO_GEN, pg_shard.shard))) {
  ...
  if (should_write_local) {
    handle_sub_write(
        get_parent()->whoami_shard(), op.client_op, local_write_op, op.trace);
  }
...
struct ECDummyOp final : ECCommon::RMWPipeline::Op {
  ...
  generate_transactions(
      ...
      bool& first_write_in_interval) override
  {
...
void
ECCommon::RMWPipeline::try_finish_rmw()
{
...
void
ECCommon::RMWPipeline::finish_rmw(OpRef const& op)
{
  dout(20) << __func__ << " op=" << *op << dendl;
...
void
ECCommon::RMWPipeline::on_change()
{
...
void
ECCommon::RMWPipeline::on_change2()
{
  extent_cache.on_change2();
}

void
ECCommon::RMWPipeline::call_write_ordered(std::function<void(void)>&& cb)
{
...
void
ECCommon::RecoveryBackend::_failed_push(
    const hobject_t& hoid,
    ECCommon::read_result_t& res)
{
...
void
ECCommon::RecoveryBackend::handle_recovery_push(
    const PushOp& op,
    RecoveryMessages* m,
    bool is_repair)
{
  ...
  if (oneshot) {
    ...
    tobj = ghobject_t(
        get_parent()->get_temp_recovery_object(op.soid, op.version),
  ...
  if (op.after_progress.data_complete) {
    if ((get_parent()->pgb_is_primary())) {
      ...
      get_parent()->on_local_recover(
          op.soid, op.recovery_info, recovery_ops[op.soid].obc, false, &m->t);
    } else {
      // If primary told us this is a repair, bump osd_stat_t::num_objects_repaired
      if (is_repair)
        get_parent()->inc_osd_stat_repaired();
      get_parent()->on_local_recover(
          op.soid, op.recovery_info, ObjectContextRef(), false, &m->t);
    }
  }
...
void
ECCommon::RecoveryBackend::handle_recovery_push_reply(
...
void
ECCommon::RecoveryBackend::update_object_size_after_read(
    uint64_t size,
    read_result_t& res,
    read_request_t& req)
{
  ...
  for (auto&& [shard, eset] : zero_mask) {
    ...
    if (!eset.empty() && (res.zero_length_reads.contains(shard) ||
   ...
   */
  for (auto iter = req.shard_want_to_read.begin();
       iter != req.shard_want_to_read.end();) {
  ...
  dout(20) << "Update want and zeros from read:size=" << size << " res=" << res
           << " req=" << req << dendl;
}

void
ECCommon::RecoveryBackend::handle_recovery_read_complete(
    const hobject_t& hoid,
    read_result_t&& res,
    read_request_t& req,
    RecoveryMessages* m)
{
  ...
  dout(30) << __func__ << " before decode: oid=" << op.hoid
           << " EC_DEBUG_BUFFERS: " << op.returned_data->debug_string(2048, 0)
           << dendl;

  op.returned_data->add_zero_padding_for_decode(req.zeros_for_decode);
  int r = op.returned_data->decode(
      ec_impl, req.shard_want_to_read, aligned_size, get_parent()->get_dpp(),
      true);
  ceph_assert(r == 0);
  ...
  dout(30) << __func__ << " after decode: oid=" << op.hoid
           << " EC_DEBUG_BUFFERS: " << op.returned_data->debug_string(2048, 0)
           << dendl;
...
struct RecoveryReadCompleter : ECCommon::ReadCompleter {
      ...
      ECCommon::read_request_t& req) override
  {
    ...
    backend.handle_recovery_read_complete(hoid, std::move(res), req, &rm);
  }
...
void
ECCommon::RecoveryBackend::dispatch_recovery_messages(
    RecoveryMessages& m,
    int priority)
{
  ...
  read_pipeline.start_read_op(
      priority, m.recovery_reads, false, true,
      std::make_unique<RecoveryReadCompleter>(*this));
}

void
ECCommon::RecoveryBackend::continue_recovery_op(
    ...
    RecoveryMessages* m)
{
  ...
  while (1) {
    ...
    case RecoveryOp::IDLE: {
      ...
      for (auto shard : shard_id_set::difference(
               sinfo.get_all_shards(), op.missing_on_shards)) {
        want.erase(shard);
      }
      ...
      if (read_request.shard_reads.empty()) {
        ...
        m->recovery_read(op.hoid, read_request);
    ...
    case RecoveryOp::READING: {
      ...
      for (auto&& pg_shard : op.missing_on) {
        ...
        op.returned_data->get_sparse_buffer(
            pg_shard.shard, pop.data, pop.data_included);
        ...
        if (op.recovery_progress.first) {
          if (sinfo.is_nonprimary_shard(pg_shard.shard)) {
            if (pop.version == op.recovery_info.oi.version) {
              ...
              dout(10) << __func__ << ": partial write OI attr: oi=" << oi
                       << dendl;
              bufferlist bl;
              oi.encode(
                  bl, get_osdmap()->get_features(CEPH_ENTITY_TYPE_OSD, nullptr));
              pop.attrset[OI_ATTR] = bl;
            }
          } else {
            dout(10) << __func__ << ": push all attrs (not nonprimary)"
                     << dendl;
        ...
        if (pg_shard != get_parent()->primary_shard()) {
          ...
          get_parent()->begin_peer_recover(pg_shard, op.hoid);
        }
      }
...
std::optional<object_info_t>
ECCommon::get_object_info_from_obc(ObjectContextRef& obc)
{
...
ECTransaction::WritePlan
ECCommon::get_write_plan(
    const ECUtil::stripe_info_t& sinfo,
    PGTransaction& t,
    ECCommon::ReadPipeline& read_pipeline,
  ...
  t.safe_create_traverse(
      [&](std::pair<const hobject_t, PGTransaction::ObjectOperation>& i) {
        ...
        ECTransaction::WritePlanObj plan(
            oid, inner_op, sinfo, readable_shards, writable_shards,
            object_in_cache, old_object_size, oi, soi,
