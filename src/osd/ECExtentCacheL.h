...
private:
  struct extent {
    ...
    bool
    operator<(const extent& rhs) const
    {
  ...
  struct object_extent_set : boost::intrusive::set_base_hook<> {
    ...
    bool
    operator<(const object_extent_set& rhs) const
    {
      return oid < rhs.oid;
    }
    ...
    struct uint_cmp {
      bool
      operator()(uint64_t lhs, const extent& rhs) const
      {
        return lhs < rhs.offset;
      }

      bool
      operator()(const extent& lhs, uint64_t rhs) const
      {
    ...
    template <typename F>
    void
    traverse_update(pin_state& pin, uint64_t offset, uint64_t length, F&& f)
    {
      ...
      for (auto p = range.first; p != range.second;) {
        ...
        uint64_t extlen = std::min(
            ext->length - (extoff - ext->offset), offset + length - extoff);
        ...
        if (action.action == update_action::NONE) {
          ...
          if ((ext->offset + ext->length > offset + length) &&
            ...
            uint64_t nlen = (ext->offset + ext->get_length()) -
                            (offset + length);
            extent* tail = nullptr;
            if (ext->bl) {
              ceph::buffer::list bl;
          ...
          if (action.action == update_action::UPDATE_PIN) {
            if (ext->bl) {
              ...
              final_extent = new ECExtentCacheL::extent(extoff, bl);
            } else {
              final_extent = new ECExtentCacheL::extent(extoff, extlen);
            }
        ...
        if (extoff + extlen < next_off) {
          ...
          if (action.action == update_action::UPDATE_PIN) {
            extent* ext = action.bl ? new extent(tailoff, *action.bl)
  ...
  struct Cmp {
    bool
    operator()(const hobject_t& oid, const object_extent_set& rhs) const
    {
      return oid < rhs.oid;
    }

    bool
    operator()(const object_extent_set& lhs, const hobject_t& oid) const
    {
      return lhs.oid < oid;
    }
  };
  ...
  struct pin_state {
    ...
    void
    _open(uint64_t in_tid, pin_type_t in_type)
    {
      ceph_assert(pin_type == NONE);
...
public:
   ...
   */
  void
  release_write_pin(write_pin& pin)
  {
    release_pin(pin);
  }
