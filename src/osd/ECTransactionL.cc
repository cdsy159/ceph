    ...
    DoutPrefixProvider* dpp)
{
  ...
  int r = ECUtilL::encode(sinfo, ecimpl, bl, want, &buffers);
  ceph_assert(r == 0);
  ...
  for (auto&& i : *transactions) {
    ...
    if (offset >= before_size) {
      i.second.set_alloc_hint(
          ...
          ghobject_t(oid, ghobject_t::NO_GEN, i.first), 0, 0,
    ...
    i.second.write(
        ...
        sinfo.logical_to_prev_chunk_offset(offset), enc_bl.length(), enc_bl,
        flags);
  }
}
    ...
    const ceph_release_t require_osd_release)
{
  ...
  t.safe_create_traverse(
      [&](pair<const hobject_t, PGTransaction::ObjectOperation>& opair) {
        ...
        if (op.truncate) {
          ldpp_dout(dpp, 20) << "generate_transactions: " << " truncate is "
                             << *(op.truncate) << dendl;
        }
	     ...
	     * the right thing. */
            op.attr_updates.insert(
                obc->attr_cache.begin(), obc->attr_cache.end());
          }
        }
	   ...
	   * the keys already won't exist */
        ...
        if (!op.attr_updates.empty()) {
          ...
          for (auto&& j : op.attr_updates) {
            if (j.second) {
              ...
              for (auto&& st : *transactions) {
                st.second.rmattr(
                    coll_t(spg_t(pgid, st.first)),
                    ghobject_t(oid, ghobject_t::NO_GEN, st.first), j.first);
              }
            }
            if (obc) {
              ...
              if (entry) {
                if (citer != obc->attr_cache.end()) {
                  ...
                  xattr_rollback.insert(make_pair(
                      j.first, std::optional<bufferlist>(citer->second)));
                } else {
                  // won't overwrite anything we put in earlier
          ...
          for (auto&& st : *transactions) {
            st.second.setattrs(
                coll_t(spg_t(pgid, st.first)),
                ghobject_t(oid, ghobject_t::NO_GEN, st.first), to_set);
          }
	   ...
	   * 2) we don't track the old value */
          ...
          for (auto&& st : *transactions) {
            st.second.set_alloc_hint(
                coll_t(spg_t(pgid, st.first)),
                ghobject_t(oid, ghobject_t::NO_GEN, st.first), object_size,
                write_size, op.alloc_hint->flags);
          }
        }
        ...
        if (op.truncate && op.truncate->first < new_size) {
          ...
          to_write.erase(
              new_size, std::numeric_limits<uint64_t>::max() - new_size);
          ...
          if (entry && !op.is_fresh_object()) {
            ...
            ldpp_dout(dpp, 20)
                << "generate_transactions: truncating to " << new_size << dendl;
            ...
            for (auto&& st : *transactions) {
              ...
              st.second.clone_range(
                  ...
                  ghobject_t(oid, ghobject_t::NO_GEN, st.first),
                  ghobject_t(oid, entry->version.version, st.first),
                  restore_from, restore_len, restore_from);
            }
          } else {
        ...
        for (auto&& extent : op.buffer_updates) {
          ...
          match(
              ...
              [&](const BufferUpdate::CloneRange&) {
                ceph_assert(
                    0 ==
                    "CloneRange is not allowed, do_op should have returned "
                    "ENOTSUPP");
              });
          ...
          if (off > new_size) {
            ...
            len += off - new_size;
            ldpp_dout(dpp, 20)
                << "generate_transactions: prepending zeroes to align " << off
                << "->" << new_size << dendl;
            off = new_size;
          }
          if (!sinfo.logical_offset_is_stripe_aligned(end) &&
            ...
            bl.append_zero(tail);
            ldpp_dout(dpp, 20)
                << "generate_transactions: appending zeroes to align end "
        ...
        for (auto&& extent : to_overwrite) {
          ...
          if (entry) {
            ...
            ldpp_dout(dpp, 20) << "generate_transactions: overwriting "
                               << restore_from << "~" << restore_len << dendl;
          ...
          encode_and_write(
              pgid, oid, sinfo, ecimpl, want, extent.get_off(),
              extent.get_val(), fadvise_flags, hinfo, written, transactions,
              dpp);
        }

        auto to_append = to_write.intersect(
            append_after, std::numeric_limits<uint64_t>::max() - append_after);
        ldpp_dout(dpp, 20) << "generate_transactions: to_append: " << to_append
                           << dendl;
        for (auto&& extent : to_append) {
          ceph_assert(sinfo.logical_offset_is_stripe_aligned(extent.get_off()));
          ceph_assert(sinfo.logical_offset_is_stripe_aligned(extent.get_len()));
          ldpp_dout(dpp, 20)
              << "generate_transactions: appending " << extent.get_off() << "~"
        ...
        if (!rollback_extents.empty() && entry) {
          if (entry) {
            ldpp_dout(dpp, 20)
                << "generate_transactions: " << oid
                << " marking rollback extents " << rollback_extents << dendl;
