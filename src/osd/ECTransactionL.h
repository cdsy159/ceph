...
{
  WritePlan plan;
  t.safe_create_traverse([&](std::pair<
                             const hobject_t, PGTransaction::ObjectOperation>& i) {
    const auto& [obj, op] = i;
    ...
    if (op.truncate && op.truncate->first < projected_size) {
      if (!(sinfo.logical_offset_is_stripe_aligned(op.truncate->first))) {
      ...
      projected_size = sinfo.logical_to_next_stripe_offset(op.truncate->first);
    }
    ...
    for (auto extent = raw_write_set.begin(); extent != raw_write_set.end();
         ++extent) {
      uint64_t head_start =
      ...
      if (head_start != head_finish && head_start < orig_size) {
        ...
        ldpp_dout(dpp, 20) << __func__ << ": reading partial head stripe "
                           ...
                           << dendl;
        plan.to_read[obj].union_insert(head_start, sinfo.get_stripe_width());
      }

      uint64_t tail_start = sinfo.logical_to_prev_stripe_offset(
          extent.get_start() + extent.get_len());
      uint64_t tail_finish = sinfo.logical_to_next_stripe_offset(
          extent.get_start() + extent.get_len());
      if (tail_start != tail_finish &&
        ...
        ldpp_dout(dpp, 20) << __func__ << ": reading partial tail stripe "
                           ...
                           << dendl;
        plan.to_read[obj].union_insert(tail_start, sinfo.get_stripe_width());
      }

      if (head_start != tail_finish) {
        ceph_assert(
            sinfo.logical_offset_is_stripe_aligned(tail_finish - head_start));
    ...
    if (op.truncate && op.truncate->second > projected_size) {
      ...
      ldpp_dout(dpp, 20) << __func__ << ": truncating out to " << truncating_to
                         << dendl;
      will_write.union_insert(projected_size, truncating_to - projected_size);
      projected_size = truncating_to;
    }

    ldpp_dout(dpp, 20) << __func__ << ": " << obj << " projected size "
                       << projected_size << dendl;
	 ...
	 * cloning it */
    ceph_assert(
        plan.to_read.count(obj) == 0 ||
        (!plan.to_read.at(obj).empty() && !i.second.has_source()));
  });
  return plan;
}
