...
struct raw_shard_id_t {
  ...
  explicit constexpr raw_shard_id_t(int8_t _id) :
    id(_id)
  {}
  ...
  static std::list<raw_shard_id_t>
  ...
  operator<=>(const raw_shard_id_t& lhs, const raw_shard_id_t& rhs)
  {
    return lhs.id <=> rhs.id;
  }

  friend constexpr std::strong_ordering
  operator<=>(int lhs, const raw_shard_id_t& rhs)
  {
    return lhs <=> rhs.id;
  }

  friend constexpr std::strong_ordering
  operator<=>(const raw_shard_id_t& lhs, int rhs)
  {
    return lhs.id <=> rhs;
  }
  ...
  bool
  operator==(const raw_shard_id_t& other) const
  {
    return id == other.id;
  }
};
