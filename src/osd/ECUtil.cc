 ...
 */
void
ECUtil::stripe_info_t::ro_range_to_shards(
    uint64_t ro_offset,
    uint64_t ro_size,
    ...
    shard_extent_map_t* shard_extent_map) const
{
  ...
  for (auto i = start_shard; i < end_shard; i++) {
    ...
    if (shard_extent_map) {
      ...
      shard_extent_map->insert_in_shard(
          shard, off, shard_bl, ro_offset, ro_offset + ro_size);
    }
  }
}

void
ECUtil::stripe_info_t::trim_shard_extent_set_for_ro_offset(
    uint64_t ro_offset,
    shard_extent_set_t& shard_extent_set) const
{
  ...
  if (ro_offset_shard == 0) {
    uint64_t shard_offset =
        ro_offset_to_shard_offset(ro_offset, raw_shard_id_t(0));
    for (auto&& iter = shard_extent_set.begin();
         iter != shard_extent_set.end();) {
...
void
ECUtil::stripe_info_t::ro_size_to_stripe_aligned_read_mask(
    uint64_t ro_size,
    shard_extent_set_t& shard_extent_set) const
{
...
void
ECUtil::stripe_info_t::ro_size_to_read_mask(
    uint64_t ro_size,
    shard_extent_set_t& shard_extent_set) const
{
  ro_range_to_shard_extent_set_with_parity(
      0, align_next(ro_size), shard_extent_set);
}

void
ECUtil::stripe_info_t::ro_size_to_zero_mask(
    uint64_t ro_size,
    shard_extent_set_t& shard_extent_set) const
{
  ...
  ro_range_to_shard_extent_set(
      align_next(ro_size),
      ro_offset_to_next_stripe_ro_offset(ro_size) - align_next(ro_size),
      shard_extent_set);
  trim_shard_extent_set_for_ro_offset(ro_size, shard_extent_set);
}
...
namespace ECUtil {
void
shard_extent_map_t::erase_after_ro_offset(uint64_t ro_offset)
{
  ...
  sinfo->ro_range_to_shard_extent_set(
      ro_offset, ro_end - ro_offset, ro_to_erase);
...
shard_extent_map_t
shard_extent_map_t::intersect(optional<shard_extent_set_t> const& other) const
{
  if (!other) {
    return shard_extent_map_t(sinfo);
  }
    ...
    if (extent_maps.contains(shard)) {
      ...
      if (!tmp.empty()) {
        ...
        if (raw_shard < sinfo->get_k()) {
          out.ro_start =
              std::min(out.ro_start, calc_ro_offset(raw_shard, range_start));
...
void
shard_extent_map_t::insert(shard_extent_map_t const& other)
{
...
void
shard_extent_map_t::deep_copy(shard_extent_map_t const& other)
{
  for (auto&& [shard, emap] : other.extent_maps) {
    for (auto iter : emap) {
 ...
 */
void
shard_extent_map_t::insert_in_shard(
    shard_id_t shard,
    uint64_t off,
 ...
 */
void
shard_extent_map_t::insert_ro_zero_buffer(uint64_t ro_offset, uint64_t ro_length)
{
 ...
 */
void
shard_extent_map_t::append_zeros_to_ro_offset(uint64_t ro_offset)
{
 ...
 */
void
shard_extent_map_t::insert_ro_extent_map(const extent_map& host_extent_map)
{
  for (auto&& range = host_extent_map.begin(); range != host_extent_map.end();
       ++range) {
    buffer::list bl = range.get_val();
    sinfo->ro_range_to_shard_extent_map(
        range.get_off(), range.get_len(), bl, *this);
  }
}

extent_set
shard_extent_map_t::get_extent_superset() const
{
  extent_set eset;
...
void
shard_extent_map_t::insert_parity_buffers()
{
...
slice_iterator
shard_extent_map_t::begin_slice_iterator(
    ...
    shard_id_set* dedup_zeros)
{
  ...
  for (auto iter = begin_slice_iterator(out_set, dpp, dedup_zeros);
       !iter.is_end(); ++iter) {
 ...
 */
int
shard_extent_map_t::encode_parity_delta(
    const ErasureCodeInterfaceRef& ec_impl,
    ...
    DoutPrefixProvider* dpp)
{
  ...
  for (auto data_shard : sinfo->get_data_shards()) {
    ...
    for (auto iter = s.begin_slice_iterator(out_set, dpp); !iter.is_end();
         ++iter) {
      ...
      if (data_shards[shard_id_t(0)].length() != 0 &&
          data_shards[shard_id_t(1)].length() != 0) {
        ec_impl->encode_delta(
            data_shards[shard_id_t(0)], data_shards[shard_id_t(1)], &delta);
...
void
shard_extent_map_t::pad_on_shards(
...
void
shard_extent_map_t::pad_on_shard(const extent_set& pad_to, const shard_id_t shard)
{
 ...
 */
void
shard_extent_map_t::trim(const shard_extent_set_t& trim_to)
{
    ...
    bool dedup_zeros)
{
  ...
  shard_id_set decode_set =
      shard_id_set::intersection(need_set, sinfo->get_data_shards());
  shard_id_set encode_set =
      shard_id_set::intersection(need_set, sinfo->get_parity_shards());
  ...
  if (!encode_set.empty()) {
     ...
     * any missing...
     */
    shard_id_set decode_for_parity_shards =
        shard_id_set::difference(sinfo->get_data_shards(), have_set);
    decode_for_parity_shards = shard_id_set::intersection(
        decode_for_parity_shards, read_mask.get_shard_id_set());
...
void
shard_extent_map_t::pad_and_rebuild_to_ec_align()
{
  bool resized = false;
  for (auto&& [shard, emap] : extent_maps) {
    extent_map aligned;
...
shard_extent_map_t::slice_map(uint64_t offset, uint64_t length) const
{
  ...
  if (offset <= start_offset && offset + length >= end_offset)
    return *this;
  ...
  for (auto&& [shard, emap] : extent_maps) {
    ...
    if (!iemap.empty()) {
      ...
      slice.ro_start = min(
          slice.start_offset,
          calc_ro_offset(sinfo->get_raw_shard(shard), iemap.get_start_off()));
      slice.ro_end =
          min(slice.ro_end,
              calc_ro_end(sinfo->get_raw_shard(shard), iemap.get_end_off()));
...
void
shard_extent_map_t::get_shard_first_buffer(
    shard_id_t shard,
    buffer::list& append_to) const
{
  if (!extent_maps.contains(shard)) {
...
uint64_t
shard_extent_map_t::get_shard_first_offset(shard_id_t shard) const
{
  if (!extent_maps.contains(shard)) {
...
void
shard_extent_map_t::zero_pad(shard_extent_set_t const& pad_to)
{
...
void
shard_extent_map_t::zero_pad(shard_id_t shard, uint64_t offset, uint64_t length)
{
...
void
shard_extent_map_t::pad_with_other(
...
void
shard_extent_map_t::erase_shard(shard_id_t shard)
{
...
bufferlist
shard_extent_map_t::get_ro_buffer(uint64_t ro_offset, uint64_t ro_length) const
{
  bufferlist bl;
  ...
  for (uint64_t chunk_offset = chunk_aligned_read.first;
    ...
    uint64_t sub_chunk_shard_offset = (chunk_offset / stripe_size) *
                                          chunk_size +
                                      sub_chunk_offset - chunk_offset;
    uint64_t sub_chunk_len =
        std::min(ro_offset + ro_length, chunk_offset + chunk_size) -
        sub_chunk_offset;

    get_buffer(
        sinfo->get_shard(raw_shard), sub_chunk_shard_offset, sub_chunk_len, bl);
  }
  return bl;
}

bufferlist
shard_extent_map_t::get_ro_buffer() const
{
  return get_ro_buffer(ro_start, ro_end - ro_start);
}

bool
get_int_from_bufferlist(bufferlist bl, int offset, uint32_t* value)
{
  *value = 0;
...
std::string
shard_extent_map_t::debug_string(uint64_t interval, uint64_t offset) const
{
...
void
shard_extent_map_t::erase_stripe(uint64_t offset, uint64_t length)
{
  for (auto iter = extent_maps.begin(); iter != extent_maps.end();) {
    auto&& [shard, emap] = *iter;
...
bool
shard_extent_map_t::contains(shard_id_t shard) const
{
  return extent_maps.contains(shard);
}

bool
shard_extent_map_t::contains(optional<shard_extent_set_t> const& other) const
{
  if (!other) {
    return true;
  }
...
bool
shard_extent_map_t::contains(shard_extent_set_t const& other) const
{
...
void
shard_extent_set_t::subtract(const shard_extent_set_t& other)
{
...
void
shard_extent_set_t::intersection_of(const shard_extent_set_t& other)
{
...
void
shard_extent_set_t::insert(const shard_extent_set_t& other)
{
  for (auto&& [shard, eset] : other) {
...
std::ostream&
operator<<(std::ostream& out, const shard_extent_map_t& rhs)
{
  // sinfo not thought to be needed for debug, as it is constant.
  return out << "shard_extent_map: ({" << rhs.ro_start << "~" << rhs.ro_end
             << "}, maps=" << rhs.extent_maps << ")";
}

std::ostream&
operator<<(std::ostream& out, const log_entry_t& rhs)
{
  switch (rhs.event) {
  case READ_REQUEST:
    out << "READ_REQUEST";
...
bool
is_hinfo_key_string(const string& key)
{
