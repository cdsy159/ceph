...
struct bl_split_merge {
  ceph::buffer::list
  split(uint64_t offset, uint64_t length, ceph::buffer::list& bl) const
  {
  ...
  bool
  can_merge(const ceph::buffer::list& left, const ceph::buffer::list& right) const
  {
    return true;
  }

  ceph::buffer::list
  merge(ceph::buffer::list&& left, ceph::buffer::list&& right) const
  {
  ...
  uint64_t
  length(const ceph::buffer::list& b) const
  {
    return b.length();
  }
};
...
class slice_iterator {
  ...
  shard_id_map<std::pair<extent_map::iterator, bufferlist::iterator>> iters;
  ...
  struct zeros {
    ...
    zeros(uint64_t _off, uint64_t _len) :
      off(_off), len(_len)
    {}
    ...
    dedup(bufferptr& bp)
    {
      ...
      uint64_t analysed = p2roundup((uintptr_t)c_str, EC_ALIGN_SIZE) -
                          (uintptr_t)c_str;
    ...
    for (auto&& iter = iters.begin(); iter != iters.end();) {
      ...
      if (iter_offset == start) {
        ...
        if (out_set.contains(shard)) {
          ldpp_dout(dpp, 20) << __func__ << " out[" << shard << "]=" << start
                             << "~" << (end - start) << dendl;
          out.emplace(
              shard, bufferptr(bl_iter.get_current_ptr(), 0, end - start));
        } else {
          ldpp_dout(dpp, 20) << __func__ << " in[" << shard << "]=" << start
                             << "~" << (end - start) << dendl;
          in.emplace(
              shard, bufferptr(bl_iter.get_current_ptr(), 0, end - start));
        }
  ...
  void
  invalidate_crcs(shard_id_t shard)
  {
...
public:
  ...
  bool
  is_page_aligned() const
  {
    for (auto&& [_, ptr] : in) {
      uintptr_t p = (uintptr_t)ptr.c_str();
      if (p & ~EC_ALIGN_MASK)
        return false;
      if ((p + ptr.length()) & ~EC_ALIGN_MASK)
        return false;
    }
...
struct shard_extent_set_t {
  ...
  shard_extent_set_t(short max_shards) :
    map(max_shards)
  {}

  bool
  contains(shard_id_t shard) const
  {
    return map.contains(shard);
  }
  ...
  void
  swap(shard_extent_set_t& other) noexcept
  {
    map.swap(other.map);
  }
  ...
  auto
  erase(shard_id_t shard)
  {
    return map.erase(shard);
  }

  auto
  erase(shard_id_map<extent_set>::iterator& iter)
  {
    return map.erase(iter);
  }

  void
  erase_stripe(uint64_t offset, uint64_t length)
  {
  ...
  void
  emplace(shard_id_t shard, extent_set&& set)
  {
  ...
  const extent_set&
  at(shard_id_t shard) const
  {
    return map.at(shard);
  }

  extent_set
  get(shard_id_t shard) const
  {
  ...
  extent_set&
  operator[](shard_id_t shard)
  {
    return map[shard];
  }

  bool
  operator==(shard_extent_set_t const& other) const
  {
    return map == other.map;
  }

  friend std::ostream&
  operator<<(std::ostream& lhs, const shard_extent_set_t& rhs)
  {
    lhs << rhs.map;
    return lhs;
  }
  ...
  void
  get_extent_superset(extent_set& eset) const
  {
    for (auto&& [_, e] : map) {
      eset.union_of(e);
    }
  ...
  void
  populate_shard_id_set(shard_id_set& set) const
  {
    map.populate_bitset_set(set);
  }

  shard_id_set
  get_shard_id_set() const
  {
    shard_id_set r;
...
inline uint64_t
align_next(uint64_t val)
{
  return p2roundup(val, EC_ALIGN_SIZE);
}

inline uint64_t
align_prev(uint64_t val)
{
...
private:
  ...
  static std::vector<shard_id_t>
  complete_chunk_mapping(
  ...
  static std::vector<raw_shard_id_t>
  reverse_chunk_mapping(const std::vector<shard_id_t>& chunk_mapping)
  {
  ...
  static shard_id_set
  calc_shards(
      raw_shard_id_t start,
      int count,
      const std::vector<shard_id_t>& chunk_mapping)
  {
    shard_id_set data_shards;
    for (raw_shard_id_t raw_shard = start; raw_shard < int(start) + count;
         ++raw_shard) {
      shard_id_t shard = chunk_mapping[int(raw_shard)];
...
public:
  ...
  uint64_t
  object_size_to_shard_size(const uint64_t size, shard_id_t shard) const
  {
  ...
  bool
  supports_ec_overwrites() const
  {
    return pool->allows_ecoverwrites();
  }

  bool
  supports_ec_optimisations() const
  {
    return pool->allows_ecoptimizations();
  }

  bool
  supports_sub_chunks() const
  {
    return (plugin_flags &
  ...
  bool
  supports_parity_delta_writes() const
  {
    return (plugin_flags &
  ...
  bool
  supports_encode_decode_crcs() const
  {
    return (plugin_flags &
  ...
  bool
  supports_direct_reads() const
  {
    return (plugin_flags & ErasureCodeInterface::FLAG_EC_PLUGIN_DIRECT_READS) !=
           0;
  }
  ...
  const shard_id_t
  get_shard(const raw_shard_id_t raw_shard) const
  {
    return chunk_mapping[int(raw_shard)];
  }

  raw_shard_id_t
  get_raw_shard(shard_id_t shard) const
  {
  ...
  auto
  get_data_shards() const
  {
    return data_shards;
  }

  auto
  get_parity_shards() const
  {
    return parity_shards;
  }

  auto
  get_all_shards() const
  {
    return all_shards;
  }

  uint64_t
  ro_offset_to_prev_chunk_offset(uint64_t offset) const
  {
    return (offset / stripe_width) * chunk_size;
  }

  uint64_t
  ro_offset_to_next_chunk_offset(uint64_t offset) const
  {
    return ((offset + stripe_width - 1) / stripe_width) * chunk_size;
  }

  uint64_t
  ro_offset_to_prev_stripe_ro_offset(uint64_t offset) const
  {
    return offset - (offset % stripe_width);
  }

  uint64_t
  ro_offset_to_next_stripe_ro_offset(uint64_t offset) const
  {
    return (
        (offset % stripe_width)
  ...
  uint64_t
  aligned_ro_offset_to_chunk_offset(uint64_t offset) const
  {
  ...
  uint64_t
  chunk_aligned_ro_offset_to_chunk_offset(uint64_t offset) const
  {
  ...
  uint64_t
  chunk_aligned_ro_length_to_shard_length(uint64_t len) const
  {
    // this rounds up
  ...
  uint64_t
  shard_offset_to_ro_offset(shard_id_t shard, uint64_t offset) const
  {
    raw_shard_id_t raw_shard = get_raw_shard(shard);
    auto result = std::lldiv(offset, chunk_size);
    return result.quot * stripe_width + (int)raw_shard * chunk_size +
           result.rem;
  }
  ...
  ro_offset_len_to_stripe_ro_offset_len(uint64_t _off, uint64_t _len) const
  {
    uint64_t off = ro_offset_to_prev_stripe_ro_offset(_off);
    uint64_t len = ro_offset_to_next_stripe_ro_offset((_off - off) + _len);
    return std::make_pair(off, len);
  }

  std::pair<uint64_t, uint64_t>
  ro_range_to_chunk_ro_range(const std::pair<uint64_t, uint64_t>& in) const
  {
  ...
  void
  ro_range_to_shard_extent_set(
      uint64_t ro_offset,
      uint64_t ro_size,
      ECUtil::shard_extent_set_t& shard_extent_set) const
  {
    ro_range_to_shards(
        ro_offset, ro_size, &shard_extent_set, nullptr, nullptr, nullptr);
  }
  ...
  {
    ro_range_to_shards(
        ro_offset, ro_size, &shard_extent_set, &extent_superset, nullptr,
        nullptr);
  }
      ...
      ECUtil::shard_extent_set_t& shard_extent_set) const
  {
    extent_set parity;
    ro_range_to_shards(
        ro_offset, ro_size, &shard_extent_set, &parity, nullptr, nullptr);
  ...
  void
  ro_range_to_shard_extent_set_with_superset(
      uint64_t ro_offset,
      uint64_t ro_size,
      ECUtil::shard_extent_set_t& shard_extent_set,
  ...
  {
    ro_range_to_shards(
        ro_offset, ro_size, &shard_extent_set, &superset, nullptr, nullptr);
  }

  void
  ro_range_to_shard_extent_map(
      uint64_t ro_offset,
      uint64_t ro_size,
      buffer::list& bl,
      shard_extent_map_t& shard_extent_map) const
  {
    ro_range_to_shards(
        ro_offset, ro_size, nullptr, nullptr, &bl, &shard_extent_map);
  }

  void trim_shard_extent_set_for_ro_offset(
      uint64_t ro_offset,
      ECUtil::shard_extent_set_t& shard_extent_set) const;
...
public:
  ...
  uint64_t
  calc_ro_offset(raw_shard_id_t raw_shard, int shard_offset) const
  {
    int stripes = shard_offset / sinfo->chunk_size;
    return stripes * sinfo->stripe_width +
           uint64_t(raw_shard) * sinfo->chunk_size +
           shard_offset % sinfo->chunk_size;
  }

  uint64_t
  calc_ro_end(raw_shard_id_t raw_shard, int shard_offset) const
  {
...
public:
  ...
  shard_extent_map_t(
    ...
    sinfo(sinfo), extent_maps(std::move(_extent_maps))
  {
   ...
   */
  const extent_map&
  get_extent_map(shard_id_t shard) const
  {
    return extent_maps.at(shard);
  }

  extent_set
  get_extent_set(const shard_id_t& shard) const
  {
    extent_set ret;
  ...
  void
  to_shard_extent_set(shard_extent_set_t& set) const
  {
    for (auto&& [shard, emap] : extent_maps) {
  ...
  bool
  contains_shard(shard_id_t shard) const
  {
    return extent_maps.contains(shard);
  }

  void erase_after_ro_offset(uint64_t ro_offset);
  shard_extent_map_t intersect_ro_range(
      uint64_t ro_offset,
      uint64_t ro_length) const;
  shard_extent_map_t intersect(
      std::optional<shard_extent_set_t> const& other) const;
  ...
  int encode_parity_delta(
      const ErasureCodeInterfaceRef& ec_impl,
      shard_extent_map_t& old_sem,
      DoutPrefixProvider* dpp);

  void pad_on_shards(
      const shard_extent_set_t& pad_to,
      const shard_id_set& shards);
  void pad_on_shards(const extent_set& pad_to, const shard_id_set& shards);
  void pad_on_shard(const extent_set& pad_to, const shard_id_t shard);
  ...
  void pad_with_other(
      shard_extent_set_t const& pad_to,
  ...
  shard_count()
  {
    return extent_maps.size();
  }

  void
  assert_buffer_contents_equal(shard_extent_map_t other)
  {
    for (auto&& [shard, emap] : extent_maps) {
      for (auto&& i : emap) {
  ...
  void
  add_zero_padding_for_decode(ECUtil::shard_extent_set_t& zeros)
  {
    bool changed = false;
  ...
  template <typename IntervalSetT>
    requires is_interval_set_v<IntervalSetT>
  void
  get_sparse_buffer(shard_id_t shard, bufferlist& bl_out, IntervalSetT& iset)
  {
    ceph_assert(bl_out.length() == 0);
    if (!extent_maps.contains(shard)) {
      return;
    }
    for (auto iter = extent_maps.at(shard).begin();
         iter != extent_maps.at(shard).end(); ++iter) {
  ...
  friend bool
  operator==(const shard_extent_map_t& lhs, const shard_extent_map_t& rhs)
  {
    return lhs.sinfo == rhs.sinfo && lhs.ro_start == rhs.ro_start &&
           lhs.ro_end == rhs.ro_end && lhs.extent_maps == rhs.extent_maps;
  }
};
...
struct log_entry_t {
  ...
  log_entry_t(const log_event_t event, const pg_shard_t& shard) :
