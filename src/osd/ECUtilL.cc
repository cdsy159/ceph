...
ECUtilL::stripe_info_t::chunk_aligned_offset_len_to_chunk(
    ...
    map<int, bufferlist*>& out)
{
  ...
  for (int i = 0; i < chunks_count; i++) {
    ...
    for (auto j = to_decode.begin(); j != to_decode.end(); ++j) {
      chunks[j->first].substr_of(
          j->second, i * repair_data_per_chunk, repair_data_per_chunk);
    }
...
void
ECUtilL::HashInfo::append(uint64_t old_size, map<int, bufferlist>& to_append)
{
...
void
ECUtilL::HashInfo::encode(bufferlist& bl) const
{
  ENCODE_START(1, 1, bl);
...
void
ECUtilL::HashInfo::decode(bufferlist::const_iterator& bl)
{
  DECODE_START(1, bl);
...
void
ECUtilL::HashInfo::dump(Formatter* f) const
{
...
namespace ECUtilL {
std::ostream&
operator<<(std::ostream& out, const HashInfo& hi)
{
...
auto
ECUtilL::HashInfo::generate_test_instances() -> list<HashInfo>
{
  list<HashInfo> o;
...
bool
ECUtilL::is_hinfo_key_string(const string& key)
{
