...
class stripe_info_t {
  ...
  const unsigned int
      k; // Can be calculated with a division from above. Better to cache.
...
private:
  ...
  complete_chunk_mapping(std::vector<shard_id_t> _chunk_mapping, unsigned int n)
  {
  ...
  reverse_chunk_mapping(std::vector<int> chunk_mapping)
  {
...
public:
  ...
  bool
  logical_offset_is_stripe_aligned(uint64_t logical) const
  {
  ...
  int
  get_shard(unsigned int raw_shard) const
  {
    return chunk_mapping[raw_shard];
  }

  unsigned int
  get_raw_shard(int shard) const
  {
    return chunk_mapping_reverse[shard];
  }

  uint64_t
  logical_to_prev_chunk_offset(uint64_t offset) const
  {
    return (offset / stripe_width) * chunk_size;
  }

  uint64_t
  logical_to_next_chunk_offset(uint64_t offset) const
  {
    return ((offset + stripe_width - 1) / stripe_width) * chunk_size;
  }

  uint64_t
  logical_to_prev_stripe_offset(uint64_t offset) const
  {
    return offset - (offset % stripe_width);
  }

  uint64_t
  logical_to_next_stripe_offset(uint64_t offset) const
  {
    return (
        (offset % stripe_width)
  ...
  uint64_t
  aligned_logical_offset_to_chunk_offset(uint64_t offset) const
  {
  ...
  uint64_t
  chunk_aligned_logical_offset_to_chunk_offset(uint64_t offset) const
  {
  ...
  uint64_t
  chunk_aligned_logical_size_to_chunk_size(uint64_t len) const
  {
  ...
  uint64_t
  aligned_chunk_offset_to_logical_offset(uint64_t offset) const
  {
  ...
  offset_len_to_stripe_bounds(std::pair<uint64_t, uint64_t> in) const
  {
  ...
  std::pair<uint64_t, uint64_t>
  offset_len_to_chunk_bounds(std::pair<uint64_t, uint64_t> in) const
  {
  ...
  std::pair<uint64_t, uint64_t>
  offset_length_to_data_chunk_indices(uint64_t off, uint64_t len) const
  {
  ...
  bool
  offset_length_is_same_stripe(uint64_t off, uint64_t len) const
  {
    if (len == 0) {
      return true;
    }
...
public:
  ...
  clear()
  {
    ...
    cumulative_shard_hashes =
        std::vector<uint32_t>(cumulative_shard_hashes.size(), -1);
  }
  ...
  uint32_t
  get_chunk_hash(int shard) const
  {
  ...
  uint64_t
  get_total_chunk_size() const
  {
    return total_chunk_size;
  }

  uint64_t
  get_projected_total_chunk_size() const
  {
    return projected_total_chunk_size;
  }

  uint64_t
  get_total_logical_size(const stripe_info_t& sinfo) const
  {
  ...
  uint64_t
  get_projected_total_logical_size(const stripe_info_t& sinfo) const
  {
  ...
  void
  set_total_chunk_size_clear_hash(uint64_t new_chunk_size)
  {
  ...
  bool
  has_chunk_hash() const
  {
    return !cumulative_shard_hashes.empty();
  }

  void
  update_to(const HashInfo& rhs)
  {
