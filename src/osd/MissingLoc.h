...
public:
  ...
  struct loc_count_t {
    ...
    friend bool
    operator<(const loc_count_t& l, const loc_count_t& r)
    {
      return (l.up < r.up || (l.up == r.up && (l.other < r.other)));
    }

    friend std::ostream&
    operator<<(std::ostream& out, const loc_count_t& l)
    {
      ceph_assert(l.up >= 0);
...
private:
  loc_count_t
  _get_count(const std::set<pg_shard_t>& shards)
  {
    loc_count_t r;
    for (auto s : shards) {
  ...
  void
  _inc_count(const std::set<pg_shard_t>& s)
  {
    std::map<shard_id_t, std::set<pg_shard_t>> pgsbs;
  ...
  void
  _dec_count(const std::set<pg_shard_t>& s)
  {
    std::map<shard_id_t, std::set<pg_shard_t>> pgsbs;
    pgs_by_shard_id(s, pgsbs);
    for (auto shard : pgsbs) {
...
public:
  ...
  bool
  needs_recovery(const hobject_t& hoid, eversion_t* v = 0) const
  {
  ...
  bool
  is_deleted(const hobject_t& hoid) const
  {
  ...
  bool
  is_unfound(const hobject_t& hoid) const
  {
  ...
  void
  add_location(const hobject_t& hoid, pg_shard_t location)
  {
  ...
  void
  remove_location(const hobject_t& hoid, pg_shard_t location)
  {
    auto p = missing_loc.find(hoid);
    if (p != missing_loc.end()) {
      _dec_count(p->second);
  ...
  void
  clear_location(const hobject_t& hoid)
  {
    auto p = missing_loc.find(hoid);
    if (p != missing_loc.end()) {
      _dec_count(p->second);
  ...
  void
  add_active_missing(const pg_missing_t& missing)
  {
    for (std::map<hobject_t, pg_missing_item>::const_iterator i =
      ...
      if (j == needs_recovery_map.end()) {
        ...
        if (i->second.need != j->second.need) {
          lgeneric_dout(cct, 0)
              << this << " " << pgid << " unexpected need for " << i->first
              << " have " << j->second << " tried to add " << i->second
              << dendl;
  ...
  void
  revise_need(const hobject_t& hoid, eversion_t need)
  {
    auto it = needs_recovery_map.find(hoid);
  ...
  void add_batch_sources_info(
      const std::set<pg_shard_t>&
          sources, ///< [in] a std::set of resources which can be used for all objects
      HBHandle* handle ///< [in] ThreadPool handle
  );
  ...
  void
  recovered(const hobject_t& hoid)
  {
      ...
      const std::map<pg_shard_t, pg_info_t>& pinfo)
  {
    ...
    for (auto&& i : pmissing) {
      ...
      if (item->need <= pinfoiter->second.last_update &&
          hoid <= pinfoiter->second.last_backfill && !i.second.is_missing(hoid))
  ...
  const std::set<pg_shard_t>&
  get_locations(const hobject_t& hoid) const
  {
  ...
  const std::map<hobject_t, pg_missing_item>&
  get_needs_recovery() const
  {
    return needs_recovery_map;
  }

  const missing_by_count_t&
