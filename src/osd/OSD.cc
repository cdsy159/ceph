...
static ostream&
_prefix(std::ostream* _dout, int whoami, epoch_t epoch)
{
...
OSD::get_osd_initial_compat_set()
{
  ...
  return CompatSet(
      ceph_osd_feature_compat, ceph_osd_feature_ro_compat,
...
OSDService::OSDService(OSD* osd, ceph::async::io_context_pool& poolctx) :
  ...
  scrub_reserver(cct, &reserver_finisher, cct->_conf->osd_max_scrubs),
  ...
  bind_epoch(0)
{
  ...
  for (int i = 0; i < m_objecter_finishers; i++) {
    ...
    auto fin = make_unique<Finisher>(
        osd->client_messenger->cct, str.str(), "finisher");
...
#ifdef PG_DEBUG_REFS
void
OSDService::add_pgid(spg_t pgid, PG* pg)
{
...
void
OSDService::remove_pgid(spg_t pgid, PG* pg)
{
  std::lock_guard l(pgid_lock);
...
void
OSDService::identify_splits_and_merges(
    OSDMapRef old_map,
    OSDMapRef new_map,
    spg_t pgid,
    ...
    set<pair<spg_t, epoch_t>>* merge_pgs)
{
  ...
  dout(20) << __func__ << " " << pgid << " e" << old_map->get_epoch() << " to e"
           << new_map->get_epoch() << " pg_nums " << pool_pg_num_history_map
           << dendl;
  deque<spg_t> queue;
  ...
  while (!queue.empty()) {
    ...
    for (auto map_iter =
             pool_pg_num_history_map.lower_bound(old_map->get_epoch());
         map_iter != pool_pg_num_history_map.end(); ++map_iter) {
      ...
      if (pgnum < new_pgnum) {
        ...
        if (cur.ps() < pgnum) {
          ...
          dout(20) << __func__ << " " << cur << " e" << new_epoch << " pg_num "
                   << pgnum << " -> " << new_pgnum << " is a child" << dendl;
          ...
          dout(20) << __func__ << " " << cur << " e" << new_epoch << " pg_num "
                   << pgnum << " -> " << new_pgnum << " is post-split, skipping"
                   << dendl;
        }
      } else if (merge_pgs) {
        // merge?
...
void
OSDService::queue_renew_lease(epoch_t epoch, spg_t spgid)
{
...
void
OSDService::release_map(OSDMapRef osdmap)
{
  std::lock_guard l(pre_publish_lock);
...
void
OSDService::await_reserved_maps()
{
  ...
  pre_publish_cond.wait(l, [this] {
    ...
    return (
        i == map_reservations.cend() || i->first >= next_osdmap->get_epoch());
  });
  pre_publish_waiter--;
...
void
OSDService::request_osdmap_update(epoch_t e)
{
...
void
OSDService::promote_throttle_recalibrate()
{
...
OSDService::s_names
OSDService::recalc_full_state(float ratio, float pratio, string& inject)
{
...
void
OSDService::check_full_status(float ratio, float pratio)
{
...
bool
OSDService::_check_inject_full(DoutPrefixProvider* dpp, s_names type) const
{
...
bool
OSDService::_check_full(DoutPrefixProvider* dpp, s_names type) const
{
  std::lock_guard l(full_status_lock);
...
bool
OSDService::check_failsafe_full(DoutPrefixProvider* dpp) const
{
  return _check_full(dpp, FAILSAFE);
}

bool
OSDService::check_full(DoutPrefixProvider* dpp) const
{
...
bool
OSDService::check_backfill_full(DoutPrefixProvider* dpp) const
{
  return _check_full(dpp, BACKFILLFULL);
}

bool
OSDService::check_nearfull(DoutPrefixProvider* dpp) const
{
  return _check_full(dpp, NEARFULL);
}

bool
OSDService::is_failsafe_full() const
{
  std::lock_guard l(full_status_lock);
...
bool
OSDService::is_backfillfull() const
{
  std::lock_guard l(full_status_lock);
...
void
OSDService::set_injectfull(s_names type, int64_t count)
{
  std::lock_guard l(full_status_lock);
...
OSDService::set_statfs(
    const struct store_statfs_t& stbuf,
...
osd_stat_t
OSDService::set_osd_stat(vector<int>& hb_peers, int num_pgs)
{
  utime_t now = ceph_clock_now();
...
void
OSDService::inc_osd_stat_repaired()
{
  std::lock_guard l(stat_lock);
...
void
OSDService::set_osd_stat_repaired(int64_t count)
{
  std::lock_guard l(stat_lock);
...
float
OSDService::compute_adjusted_ratio(
    osd_stat_t new_stat,
    float* pratio,
    uint64_t adjust_used)
{
  *pratio = ((float)new_stat.statfs.get_used_raw()) /
            ((float)new_stat.statfs.total);
  ...
  return ((float)new_stat.statfs.get_used_raw()) /
         ((float)new_stat.statfs.total);
}

void
OSDService::send_message_osd_cluster(int peer, Message* m, epoch_t from_epoch)
{
...
ConnectionRef
OSDService::get_con_osd_cluster(int peer, epoch_t from_epoch)
{
  dout(20) << __func__ << " to osd." << peer << " from_epoch " << from_epoch
           << dendl;
  OSDMapRef next_map = get_nextmap_reserved();
  // service map is always newer/newest
  ceph_assert(from_epoch <= next_map->get_epoch());
...
void
OSDService::queue_want_pg_temp(pg_t pgid, const vector<int>& want, bool forced)
{
...
void
OSDService::remove_want_pg_temp(pg_t pgid)
{
  std::lock_guard l(pg_temp_lock);
...
void
OSDService::_sent_pg_temp()
{
#ifdef HAVE_STDLIB_MAP_SPLICING
  pg_temp_pending.merge(pg_temp_wanted);
#else
  pg_temp_pending.insert(
      make_move_iterator(begin(pg_temp_wanted)),
...
std::ostream&
operator<<(std::ostream& out, const OSDService::pg_temp_t& pg_temp)
{
  out << pg_temp.acting;
  if (pg_temp.forced) {
    out << " (forced)";
...
void
OSDService::send_pg_created(pg_t pgid)
{
...
void
OSDService::set_epochs(
    const epoch_t* _boot_epoch,
...
OSDService::prepare_to_stop()
{
  ...
  if (osdmap && osdmap->is_up(whoami)) {
    dout(0) << __func__ << " telling mon we are shutting down and dead "
            << dendl;
    set_state(PREPARING_TO_STOP);
    monc->send_mon_message(new MOSDMarkMeDown(
        monc->get_fsid(), whoami, osdmap->get_addrs(whoami), osdmap->get_epoch(),
        true, // request ack
        true // mark as down and dead
        ));
    const auto timeout =
        ceph::make_timespan(cct->_conf->osd_mon_shutdown_timeout);
    is_stopping_cond.wait_for(l, timeout, [this] {
...
{
  auto get_map_and_adjust_counters = [](bufferlist& bl, int& max,
                                        ssize_t& max_bytes,
  ...
  if (since < m->cluster_osdmap_trim_lower_bound) {
    ...
    dout(10) << __func__ << " cluster osdmap lower bound "
             << sblock.cluster_osdmap_trim_lower_bound << " > since " << since
             << ", starting with full map" << dendl;
    since = m->cluster_osdmap_trim_lower_bound;
    if (bufferlist bl; get_map_and_adjust_counters(bl, max, max_bytes, [&] {
          return get_map_bl(since, bl);
        })) {
...
panic:
  ...
  if (get_inc_map_bl(m->newest_map, bl)) {
    ...
    derr
        << __func__ << " unable to load latest map " << m->newest_map
    ...
    const OSDMapRef& osdmap)
{
  ...
  dout(10) << fmt::format(
                  "{} epoch range: ({}, {}] to {} {}", __func__, since, to,
                  con->get_peer_entity_name().to_str(), con->get_peer_addr())
           << dendl;
...
bool
OSDService::_get_map_bl(epoch_t e, bufferlist& bl)
{
  ...
  found = store->read(
              meta_ch, OSD::get_osdmap_pobject_name(e), 0, 0, bl,
...
bool
OSDService::get_inc_map_bl(epoch_t e, bufferlist& bl)
{
  ...
  logger->inc(l_osd_map_bl_cache_miss);
  found = store->read(
              meta_ch, OSD::get_inc_osdmap_pobject_name(e), 0, 0, bl,
              CEPH_OSD_OP_FLAG_FADVISE_WILLNEED) >= 0;
  if (found) {
    if (!bl.is_page_aligned()) {
      bl.rebuild_page_aligned();
...
void
OSDService::_add_map_bl(epoch_t e, bufferlist& bl)
{
...
void
OSDService::_add_map_inc_bl(epoch_t e, bufferlist& bl)
{
...
void
OSDService::reply_op_error(OpRequestRef op, int err)
{
  reply_op_error(op, err, eversion_t(), 0, {});
}

void
OSDService::reply_op_error(
    OpRequestRef op,
    int err,
    eversion_t v,
    version_t uv,
    vector<pg_log_op_return_item_t> op_returns)
{
  ...
  MOSDOpReply* reply = new MOSDOpReply(
      m, err, osdmap->get_epoch(), flags, !m->has_flag(CEPH_OSD_FLAG_RETURNVEC));
...
void
OSDService::handle_misdirected_op(PG* pg, OpRequestRef op)
{
...
void
OSDService::enqueue_back(OpSchedulerItem&& qi)
{
  osd->op_shardedwq.queue(std::move(qi));
}

void
OSDService::enqueue_front(OpSchedulerItem&& qi)
{
  osd->op_shardedwq.queue_front(std::move(qi));
}

double
OSDService::get_cost_per_io() const
{
    ...
    int priority)
{
  ...
  enqueue_back(OpSchedulerItem(
      ...
      cost_for_queue, cct->_conf->osd_recovery_priority, ceph_clock_now(), 0,
      e));
}

void
OSDService::queue_for_snap_trim(PG* pg, uint64_t cost_per_object)
{
  ...
  enqueue_back(OpSchedulerItem(
      ...
      cost_for_queue, cct->_conf->osd_snap_trim_priority, ceph_clock_now(), 0,
      pg->get_osdmap_epoch()));
}

template <class MSG_TYPE>
void
OSDService::queue_scrub_event_msg(
    PG* pg,
    ...
    uint64_t cost)
{
  ...
  enqueue_back(OpSchedulerItem(
      ...
      pg->scrub_requeue_priority(with_priority, qu_priority), ceph_clock_now(),
      0, epoch));
}
...
template <class MSG_TYPE>
void
OSDService::queue_scrub_event_msg_default_cost(
    PG* pg,
    Scrub::scrub_prio_t with_priority,
    unsigned int qu_priority,
    Scrub::act_token_t act_token)
{
  ...
  queue_scrub_event_msg<MSG_TYPE>(
      pg, with_priority, qu_priority, act_token, cost);
}
...
void
OSDService::queue_for_scrub(PG* pg, Scrub::scrub_prio_t with_priority)
{
  queue_scrub_event_msg_default_cost<PGScrub>(pg, with_priority);
}

void
OSDService::queue_for_rep_scrub(
    PG* pg,
    Scrub::scrub_prio_t with_priority,
    unsigned int qu_priority,
    Scrub::act_token_t act_token,
    uint64_t cost)
{
  queue_scrub_event_msg<PGRepScrub>(
      pg, with_priority, qu_priority, act_token, cost);
}

void
OSDService::queue_for_rep_scrub_resched(
    PG* pg,
    Scrub::scrub_prio_t with_priority,
    unsigned int qu_priority,
    Scrub::act_token_t act_token)
{
  ...
  queue_scrub_event_msg_default_cost<PGRepScrubResched>(
      pg, with_priority, qu_priority, act_token);
}

void
OSDService::queue_for_scrub_resched(PG* pg, Scrub::scrub_prio_t with_priority)
{
...
void
OSDService::queue_scrub_pushes_update(PG* pg, Scrub::scrub_prio_t with_priority)
{
...
void
OSDService::queue_scrub_chunk_free(
    PG* pg,
    Scrub::scrub_prio_t with_priority,
    uint64_t cost)
{
...
void
OSDService::queue_scrub_chunk_busy(PG* pg, Scrub::scrub_prio_t with_priority)
{
...
void
OSDService::queue_scrub_applied_update(PG* pg, Scrub::scrub_prio_t with_priority)
{
  queue_scrub_event_msg_default_cost<PGScrubAppliedUpdate>(pg, with_priority);
}

void
OSDService::queue_scrub_unblocking(PG* pg, Scrub::scrub_prio_t with_priority)
{
...
void
OSDService::queue_scrub_digest_update(PG* pg, Scrub::scrub_prio_t with_priority)
{
...
void
OSDService::queue_scrub_got_repl_maps(PG* pg, Scrub::scrub_prio_t with_priority)
{
...
void
OSDService::queue_scrub_replica_pushes(PG* pg, Scrub::scrub_prio_t with_priority)
{
...
void
OSDService::queue_scrub_is_finished(PG* pg)
{
  // Resulting scrub event: 'ScrubFinished'
  queue_scrub_event_msg_default_cost<PGScrubScrubFinished>(
      pg, Scrub::scrub_prio_t::high_priority);
}

void
OSDService::queue_scrub_next_chunk(PG* pg, Scrub::scrub_prio_t with_priority)
{
...
void
OSDService::queue_for_pg_delete(spg_t pgid, epoch_t e, int64_t num_objects)
{
  ...
  enqueue_back(OpSchedulerItem(
      ...
      cost_for_queue, cct->_conf->osd_pg_delete_priority, ceph_clock_now(), 0,
      e));
}

bool
OSDService::try_finish_pg_delete(PG* pg, unsigned old_pg_num)
{
...
void
OSDService::set_ready_to_merge_source(PG* pg, eversion_t version)
{
...
void
OSDService::set_ready_to_merge_target(
    PG* pg,
    eversion_t version,
    ...
    epoch_t last_epoch_clean)
{
  std::lock_guard l(merge_lock);
  dout(10) << __func__ << " " << pg->pg_id << dendl;
  ready_to_merge_target.insert(make_pair(
      pg->pg_id.pgid,
...
void
OSDService::set_not_ready_to_merge_source(pg_t source)
{
  std::lock_guard l(merge_lock);
...
void
OSDService::set_not_ready_to_merge_target(pg_t target, pg_t source)
{
  std::lock_guard l(merge_lock);
...
void
OSDService::_send_ready_to_merge()
{
  ...
  for (auto src : not_ready_to_merge_source) {
    if (sent_ready_to_merge_source.count(src) == 0) {
      monc->send_mon_message(new MOSDPGReadyToMerge(
          src, {}, {}, 0, 0, false, osdmap->get_epoch()));
...
void
OSDService::clear_ready_to_merge(PG* pg)
{
  std::lock_guard l(merge_lock);
  dout(10) << __func__ << " " << pg->pg_id << dendl;
...
void
OSDService::clear_sent_ready_to_merge()
{
  std::lock_guard l(merge_lock);
...
void
OSDService::prune_sent_ready_to_merge(const OSDMapRef& osdmap)
{
  std::lock_guard l(merge_lock);
...
void
OSDService::_queue_for_recovery(
    ...
    uint64_t reserved_pushes)
{
  ...
  enqueue_back(OpSchedulerItem(
      unique_ptr<OpSchedulerItem::OpQueueable>(new PGRecovery(
          p.pg->get_pgid(), p.epoch_queued, reserved_pushes, p.priority)),
      cost_for_queue, cct->_conf->osd_recovery_priority, ceph_clock_now(), 0,
      p.epoch_queued));
}
    ...
    string osdspec_affinity)
{
  ...
  ret = write_meta(
      cct, store.get(), sb.cluster_fsid, sb.osd_fsid, whoami, osdspec_affinity);
  if (ret) {
...
OSD::run_osd_bench(CephContext* cct, ObjectStore* store)
{
  // Bench test details:
...
OSD::OSD(
  ...
  op_tracker(
      cct,
      cct->_conf->osd_enable_op_tracker,
  ...
  service(this, poolctx)
{
  ...
  for (uint32_t i = 0; i < num_shards; i++) {
    OSDShard* one_shard = new OSDShard(i, cct, this, op_queue, op_queue_cut_off);
    ...
    asok_finisher on_finish)
{
  ...
  if (prefix == "pg" || prefix == "query" || prefix == "log" ||
      prefix == "mark_unfound_lost" || prefix == "list_unfound" ||
      prefix == "scrub" || prefix == "deep-scrub" || prefix == "scrub-abort" ||
...
will start to track new ops received afterwards.";
    ...
    ss << "kicking recovery queue. set osd_recovery_delay_start " << "to "
       << cct->_conf->osd_recovery_delay_start;
  }
    ...
    struct osd_ping_time_t {
      ...
      bool
      operator<(const osd_ping_time_t& rhs) const
      {
    ...
    for (auto& sitem : boost::adaptors::reverse(sorted)) {
      ...
      f->dump_format_unquoted(
          "1min", "%s", fixed_u_to_string(sitem.times[0], 3).c_str());
      f->dump_format_unquoted(
          "5min", "%s", fixed_u_to_string(sitem.times[1], 3).c_str());
      f->dump_format_unquoted(
          "15min", "%s", fixed_u_to_string(sitem.times[2], 3).c_str());
      f->close_section(); // average
      f->open_object_section("min");
      f->dump_format_unquoted(
          "1min", "%s", fixed_u_to_string(sitem.max[0], 3).c_str());
      f->dump_format_unquoted(
          "5min", "%s", fixed_u_to_string(sitem.max[1], 3).c_str());
      f->dump_format_unquoted(
          "15min", "%s", fixed_u_to_string(sitem.max[2], 3).c_str());
      f->close_section(); // min
      ...
      f->close_section(); // max
      f->dump_format_unquoted(
          "last", "%s", fixed_u_to_string(sitem.last, 3).c_str());
      f->close_section(); // entry
    }
    ...
    ostream& ss)
{
  ...
  OSDBenchTest osd_bench{cct,   store.get(), service.meta_ch, count, bsize,
                         osize, onum};
...
int
OSD::enable_disable_fuse(bool stop)
{
#ifdef HAVE_LIBFUSE
  int r;
...
float
OSD::get_osd_recovery_sleep_degraded()
{
...
OSD::init()
{
  ...
  for (auto m :
       {cluster_messenger, objecter_messenger, hb_front_client_messenger,
        hb_back_client_messenger, hb_front_server_messenger,
        hb_back_server_messenger}) {
    m->set_auth_client(monc);
  }
  for (auto m :
       {client_messenger, cluster_messenger, hb_front_server_messenger,
        hb_back_server_messenger}) {
  ...
  for (auto& shard : shards) {
    ...
    for (auto pg : pgs) {
      ...
      set<pair<spg_t, epoch_t>> merge_pgs;
      service.identify_splits_and_merges(
          pg->get_osdmap(), osdmap, pg->pg_id, &new_children, &merge_pgs);
...
OSD::final_init()
{
  ...
  int r = admin_socket->register_command(
      "status", asok_hook, "high-level status of OSD");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "flush_journal", asok_hook, "flush the journal to permanent store");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "dump_ops_in_flight "
  ...
  r = admin_socket->register_command(
      ...
      "name=filterstr,type=CephString,n=N,req=false",
      asok_hook, "show the count of blocked ops currently in flight");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "dump_historic_ops "
      "name=filterstr,type=CephString,n=N,req=false",
  ...
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "dump_op_pq_state", asok_hook, "dump op queue state");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "dump_blocklist", asok_hook, "dump blocklisted clients and times");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "dump_watchers", asok_hook,
      "show clients which have active watches,"
      " and on which objects");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "dump_recovery_reservations", asok_hook, "show recovery reservations");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "dump_scrub_reservations", asok_hook, "show scrub reservations");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "get_latest_osdmap", asok_hook,
  ...
  r = admin_socket->register_command(
      ...
      asok_hook, "update malloc extension heap property");
  ceph_assert(r == 0);

  r = admin_socket->register_command(
      "get_heap_property "
      "name=property,type=CephString",
      asok_hook, "get malloc extension heap property");
  ceph_assert(r == 0);
  ...
  r = admin_socket->register_command(
      "dump_scrubs", asok_hook, "print scheduled scrubs");
  ceph_assert(r == 0);
  ...
  r = admin_socket->register_command(
      "flush_store_cache", asok_hook, "Flush bluestore internal cache");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "rotate-stored-key", asok_hook, "Update the stored osd_key");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "dump_pgstate_history", asok_hook, "show recent state history");
  ceph_assert(r == 0);
  ...
  r = admin_socket->register_command(
      "list_devices", asok_hook, "list OSD devices.");
  r = admin_socket->register_command(
      "send_beacon", asok_hook, "send OSD beacon to mon immediately");
  ...
  r = admin_socket->register_command(
      "dump_osd_pg_stats ", asok_hook, "Dump OSD PGs' statistics");
  ceph_assert(r == 0);
  ...
  r = admin_socket->register_command(
      ...
      test_ops_hook, "truncate object to length");
  ceph_assert(r == 0);
  ...
  r = admin_socket->register_command(
      ...
      test_ops_hook, "inject error for read of object in an EC pool");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      ...
      "name=shardid,type=CephInt,req=true,range=0|255 "
      "name=type,type=CephInt,req=false",
      test_ops_hook, "clear read error injects for object in an EC pool");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "injectecwriteerr "
      "name=pool,type=CephString "
      ...
      "name=when,type=CephInt,req=false "
      "name=duration,type=CephInt,req=false",
      test_ops_hook, "inject error for write of object in an EC pool");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      ...
      "name=shardid,type=CephInt,req=true,range=0|255 "
      "name=type,type=CephInt,req=false",
      test_ops_hook, "clear write error inject for object in an EC pool");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "set_recovery_delay "
      "name=utime,type=CephInt,req=false",
      test_ops_hook, "Delay osd recovery by specified seconds");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "injectfull "
  ...
  r = admin_socket->register_command(
      ...
      "name=pool,type=CephString "
      "name=objname,type=CephObjectname ",
      test_ops_hook, "Clear a parity read inject");
  ceph_assert(r == 0);
  ...
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "flush_pg_stats", asok_hook, "flush pg stats");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "heap "
  ...
  r = admin_socket->register_command(
      ...
      asok_hook, "dump missing objects to a named file");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "debug kick_recovery_wq "
      "name=delay,type=CephInt,range=0",
      asok_hook, "set osd_recovery_delay_start to <val>");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "cpu_profiler "
  ...
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "dump_pg_recovery_stats", asok_hook, "dump pg recovery statistics");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "reset_pg_recovery_stats", asok_hook, "reset pg recovery statistics");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "trim stale osdmaps", asok_hook,
      "cleanup any existing osdmap from the store "
      "in the range of 0 up to the superblock's oldest_map.");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "cache drop", asok_hook, "Drop all OSD caches");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "cache status", asok_hook, "Get OSD caches statistics");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "scrub_purged_snaps", asok_hook, "Scrub purged_snaps vs snapmapper index");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "reset_purged_snaps_last", asok_hook,
  ...
  r = admin_socket->register_command(
      ...
      "name=value,type=CephString,req=false",
      asok_hook, "debug the scrubber");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "clear_shards_repaired "
      "name=count,type=CephInt,req=false,range=0",
      asok_hook, "clear num_shards_repaired to clear health warning");
  ceph_assert(r == 0);
  ...
  r = admin_socket->register_command(
      "query", asok_hook, "show details of a specific pg");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "log", asok_hook, "dump pg_log of a specific pg");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "mark_unfound_lost "
  ...
  r = admin_socket->register_command(
      ...
      "name=offset,type=CephString,req=false",
      asok_hook,
      "list unfound objects on this pg, perhaps starting at an offset given in "
      "JSON");
  ceph_assert(r == 0);
  ...
  r = admin_socket->register_command(
      "scrub "
      "name=pgid,type=CephPgid,req=false",
      asok_hook, "Trigger a scrub");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "deep-scrub "
      "name=pgid,type=CephPgid,req=false",
      asok_hook, "Trigger a deep scrub");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "scrub-abort "
      "name=pgid,type=CephPgid,req=false",
      asok_hook, "Abort an ongoing scrub. Cancel any operator-initiated scrub");
  ceph_assert(r == 0);
  ...
  r = admin_socket->register_command(
      ...
      asok_hook, "Schedule a scrub");
  ceph_assert(r == 0);
  r = admin_socket->register_command(
      "schedule-deep-scrub "
      "name=pgid,type=CephPgid,req=false "
      "name=time,type=CephInt,req=false",
      asok_hook, "Schedule a deep scrub");
  ceph_assert(r == 0);
}
...
OSD::shutdown()
{
  ...
  if (!cct->_conf->osd_fast_shutdown &&
      cct->_conf.get_val<bool>("osd_debug_shutdown")) {
  ...
  if (cct->_conf->osd_fast_shutdown) {
    ...
    dout(0) << "Fast Shutdown duration total     :"
            << end_time - start_time_func << " seconds" << dendl;
    if (cct->_conf->osd_fast_shutdown_timeout &&
        end_time - start_time_func > cct->_conf->osd_fast_shutdown_timeout) {
      dout(0) << "Fast Shutdown duration exceeded :"
              << cct->_conf->osd_fast_shutdown_timeout << " seconds" << dendl;
    }
    dout(0) << "Fast Shutdown duration osd_drain :"
            << start_time_umount - start_time_osd_drain << " seconds" << dendl;
    dout(0) << "Fast Shutdown duration umount    :"
            << end_time - start_time_umount << " seconds" << dendl;
    dout(0) << "Fast Shutdown duration timer     :"
            << start_time_osd_drain - start_time_timer << " seconds" << dendl;
    cct->_log->flush();
...
int
OSD::mon_cmd_maybe_osd_create(string&& cmd)
{
  bool created = false;
  while (true) {
    ...
    if (r < 0) {
      if (r == -ENOENT && !created) {
        string newcmd = "{\"prefix\": \"osd create\", \"id\": " +
                        stringify(whoami) + ", \"uuid\": \"" +
...
OSD::update_crush_location()
{
  ...
  if (cct->_conf->osd_crush_initial_weight >= 0) {
    snprintf(
        weight, sizeof(weight), "%.4lf", cct->_conf->osd_crush_initial_weight);
  } else {
...
OSD::read_superblock()
{
  ...
  if (r_omap >= 0 && vals.size() > 0) {
    try {
      ...
      derr << __func__ << " omap replica is corrupted." << dendl;
      r_omap = -EFAULT;
    }
  } else {
    derr << __func__ << " omap replica is missing." << dendl;
    r_omap = -ENOENT;
  }
  ...
  if (r_disk >= 0) {
    try {
      ...
      derr << __func__ << " disk replica is corrupted." << dendl;
      r_disk = -EFAULT;
    }
  } else {
    derr << __func__ << " disk replica is missing." << dendl;
    r_disk = -ENOENT;
  }
  ...
  if (r_omap >= 0 && r_disk < 0) {
    ...
    if (superblock.current_epoch != super_disk.current_epoch) {
      derr << __func__ << " got mismatching superblocks, omap:" << superblock
           << " vs. disk:" << super_disk << dendl;
      if (superblock.current_epoch < super_disk.current_epoch) {
        std::swap(superblock, super_disk);
        dout(0) << __func__ << " using disk superblock" << dendl;
      } else {
        dout(0) << __func__ << " using omap superblock" << dendl;
      }
    }
  }
  ...
  for (vector<coll_t>::iterator p = ls.begin(); p != ls.end(); ++p) {
    ...
    while (1) {
      ...
      store->collection_list(
          ch, next, ghobject_t::get_max(), store->get_ideal_list_max(),
          &objects, &next);
      if (objects.empty())
        break;
    ...
    coll_t tmp)
{
  ...
  while (true) {
    objects.clear();
    store->collection_list(
        ch, next, ghobject_t::get_max(), max, &objects, &next);
...
PG*
OSD::_make_pg(OSDMapRef createmap, spg_t pgid)
{
  ...
  if (pi.type == pg_pool_t::TYPE_REPLICATED ||
    ...
    pg = new PrimaryLogPG(
        &service, createmap, pool, ec_profile, pgid,
        lookup_ec_extent_cache_lru(pgid));
  else
    ceph_abort();
  return pg;
}
...
void
OSD::_get_pgs(vector<PGRef>* v, bool clear_too)
{
  v->clear();
...
bool
OSD::try_finish_pg_delete(PG* pg, unsigned old_pg_num)
{
...
ECExtentCache::LRU&
OSD::lookup_ec_extent_cache_lru(spg_t pgid) const
{
  uint32_t shard_index = pgid.hash_to_shard(num_shards);
  auto sdata = shards[shard_index];
...
OSD::load_pgs()
{
  ...
  for (vector<coll_t>::iterator it = ls.begin(); it != ls.end(); ++it) {
    ...
    }
    {
      ...
      store->set_collection_commit_queue(
          pg->coll, &(shards[shard_index]->context_queue));
    }
...
PGRef
OSD::handle_pg_create_info(const OSDMapRef& osdmap, const PGCreateInfo* info)
{
  ...
  pg->init(
      role, up, up_primary, acting, acting_primary, info->history,
      info->past_intervals, rctx.transaction);
...
static vector<int32_t>
twiddle(const vector<int>& acting)
{
  if (acting.size() > 1) {
    return {acting[0]};
  } else {
  ...
  bool have_pending_creates = false;
  {
    ...
    while (spare_pgs > 0 && pg != pending_creates_from_osd.cend()) {
      ...
      get_osdmap()->pg_to_up_acting_osds(
          pg->first.pgid, nullptr, nullptr, &acting, nullptr);
...
OSD::_add_heartbeat_peer(int p)
{
  ...
  if (i == heartbeat_peers.end()) {
    pair<ConnectionRef, ConnectionRef> cons =
        service.get_con_osd_hb(p, get_osdmap_epoch());
    if (!cons.first)
      return;
...
void
OSD::_remove_heartbeat_peer(int n)
{
...
void
OSD::maybe_update_heartbeat_peers()
{
  ceph_assert(ceph_mutex_is_locked(osd_lock));
...
void
OSD::reset_heartbeat_peers(bool all)
{
  ceph_assert(ceph_mutex_is_locked(osd_lock));
...
void
OSD::handle_osd_ping(MOSDPing* m)
{
  ...
  case MOSDPing::PING: {
    if (cct->_conf->osd_debug_drop_ping_probability > 0) {
      ...
      if (heartbeat_drop != debug_heartbeat_drops_remaining.end()) {
        if (heartbeat_drop->second == 0) {
          ...
          cct->_conf->osd_debug_drop_ping_probability >
    ...
    s->stamps->got_ping(
        m->up_from, mnow, m->mono_send_stamp, m->delta_ub, &sender_delta_ub);
    ...
    if (curmap->is_up(from)) {
      if (is_active()) {
        ...
        !curmap->exists(from) || curmap->get_down_at(from) > m->map_epoch) {
...
#define ROUND_S_TO_USEC(sec) (uint32_t)((sec) * 1000 * 1000 + 0.5)
          ...
          if (now - i->second.hb_interval_start >=
            ...
            uint32_t back_avg = i->second.hb_total_back /
                                i->second.hb_average_count;
            uint32_t back_min = i->second.hb_min_back;
            uint32_t back_max = i->second.hb_max_back;
            uint32_t front_avg = i->second.hb_total_front /
                                 i->second.hb_average_count;
        ...
        if (i->second.is_healthy(now)) {
          ...
          if (failure_pending_entry != failure_pending.end()) {
            dout(10) << "handle_osd_ping canceling in-flight "
                     << "failure report for osd." << from << dendl;
            send_still_alive(
                curmap->get_epoch(), from, failure_pending_entry->second.second);
...
OSD::heartbeat()
{
  ...
  for (map<int, HeartbeatInfo>::iterator i = heartbeat_peers.begin();
    ...
    i->second.con_back->send_message(new MOSDPing(
        monc->get_fsid(), service.get_osdmap_epoch(), MOSDPing::PING, now, mnow,
        mnow, service.get_up_epoch(), cct->_conf->osd_heartbeat_min_size,
        delta_ub));

    if (i->second.con_front)
      i->second.con_front->send_message(new MOSDPing(
          monc->get_fsid(), service.get_osdmap_epoch(), MOSDPing::PING, now,
          mnow, mnow, service.get_up_epoch(),
...
bool
OSD::heartbeat_reset(Connection* con)
{
  std::lock_guard l(heartbeat_lock);
  auto s = con->get_priv();
...
OSD::tick_without_osd_lock()
{
  ...
  logger->set(
      l_osd_cached_crc_adjusted, ceph::buffer::get_cached_crc_adjusted());
    ...
    ostream& ss)
{
  ...
  if (command == "setomapval" || command == "rmomapkey" ||
      ...
      command == "injectdataerr" || command == "injectecreaderr" ||
      command == "injectecclearreaderr" || command == "injectecwriteerr" ||
    ...
    hobject_t obj(object_t(objname), ""s, CEPH_NOSNAP, rawpg.ps(), pool, nspace);
  ...
  if (command == "set_recovery_delay") {
    ...
    ostringstream oss;
    oss << delay;
    int r = service->cct->_conf.set_val(
        "osd_recovery_delay_start", oss.str().c_str());
    if (r != 0) {
    ...
    ss << "set_recovery_delay: set osd_recovery_delay_start " << "to "
       << service->cct->_conf->osd_recovery_delay_start;
    return;
  }
...
void
OSD::ms_handle_connect(Connection* con)
{
...
void
OSD::ms_handle_fast_connect(Connection* con)
{
  if (con->get_peer_type() != CEPH_ENTITY_TYPE_MON &&
...
void
OSD::ms_handle_fast_accept(Connection* con)
{
  if (con->get_peer_type() != CEPH_ENTITY_TYPE_MON &&
      con->get_peer_type() != CEPH_ENTITY_TYPE_MGR) {
    if (auto s = ceph::ref_cast<Session>(con->get_priv()); !s) {
...
bool
OSD::ms_handle_reset(Connection* con)
{
...
bool
OSD::ms_handle_refused(Connection* con)
{
...
struct CB_OSD_GetVersion {
  ...
  void
  operator()(boost::system::error_code ec, version_t newest, version_t oldest)
  {
    if (!ec)
...
void
OSD::_got_mon_epochs(epoch_t oldest, epoch_t newest)
{
  std::lock_guard l(osd_lock);
  if (is_preboot()) {
...
void
OSD::_preboot(epoch_t oldest, epoch_t newest)
{
...
void
OSD::handle_get_purged_snaps_reply(MMonGetPurgedSnapsReply* m)
{
  ...
  if (!is_preboot() || m->last < superblock.purged_snaps_last) {
    ...
    SnapMapper::record_purged_snaps(
        cct, osdriver, osdriver.get_transaction(&t), m->purged_snaps);
  }
  ...
  if (is_waiting_for_healthy()) {
    ...
    if (osd_markdown_log.empty()) {
      dout(5) << __func__ << " force returning true since last markdown"
              << " was " << cct->_conf->osd_max_markdown_period << "s ago"
              << dendl;
      return true;
    }
    std::lock_guard l(heartbeat_lock);
...
OSD::_send_boot()
{
  ...
  dout(20) << " initial client_addrs " << client_addrs << ", cluster_addrs "
           << cluster_addrs << ", hb_back_addrs "
           << hb_back_server_messenger->get_myaddrs() << ", hb_front_addrs "
           << hb_front_server_messenger->get_myaddrs() << dendl;
...
void
OSD::_collect_metadata(map<string, string>* pm)
{
  // config info
...
void
OSD::request_full_map(epoch_t first, epoch_t last)
{
  ...
  ceph_assert(
      first >= requested_full_first); // we shouldn't ever ask for older maps
...
OSD::send_failures()
{
  ...
  while (!failure_queue.empty()) {
    ...
    if (!failure_pending.count(osd)) {
      ...
      monc->send_mon_message(new MOSDFailure(
          monc->get_fsid(), osd, osdmap->get_addrs(osd), failed_for,
          osdmap->get_epoch()));
      failure_pending[osd] =
...
void
OSD::send_still_alive(epoch_t epoch, int osd, const entity_addrvec_t& addrs)
{
  MOSDFailure* m = new MOSDFailure(
      monc->get_fsid(), osd, addrs, 0, epoch, MOSDFailure::FLAG_ALIVE);
  monc->send_mon_message(m);
}

void
OSD::cancel_pending_failures()
{
  std::lock_guard l(heartbeat_lock);
...
void
OSD::send_beacon(const ceph::coarse_mono_clock::time_point& now)
{
  const auto& monmap = monc->monmap;
  ...
  if (monmap.epoch > 0 && monmap.get_required_features().contains_all(
    ...
    MOSDBeacon* beacon = nullptr;
    {
      ...
      beacon = new MOSDBeacon(
          get_osdmap_epoch(), min_last_epoch_clean,
...
void
OSD::scrub_purged_snaps()
{
  dout(10) << __func__ << dendl;
  ceph_assert(ceph_mutex_is_locked(osd_lock));
  SnapMapper::Scrubber s(
      cct, store.get(), service.meta_ch, make_snapmapper_oid(),
...
void
OSD::probe_smart(const string& only_devid, ostream& ss)
{
  set<string> devnames;
  store->get_devices(&devnames);
...
bool
OSD::heartbeat_dispatch(Message* m)
{
  ...
  epoch_t send_from = 0;
  {
    ...
    if (peer_epoch_lb > session->projected_epoch) {
      dout(10) << __func__ << ": con " << con->get_peer_addr()
               ...
               << session->projected_epoch << " to ping map epoch of "
               << peer_epoch_lb << dendl;
    ...
    dout(10) << __func__ << ": con " << con->get_peer_addr() << " map epoch "
  ...
  while (i != session->waiting_on_map.end()) {
    ...
    if (m->get_type() == CEPH_MSG_OSD_OP &&
      ...
      if (!m->get_connection()->has_features(CEPH_FEATUREMASK_SERVER_TENTACLE)) {
        ...
        pgid.reset_shard(
            osdmap->pgtemp_undo_primaryfirst(*pi, pgid.pgid, pgid.shard));
      }
    }
...
OSD::ms_fast_dispatch(Message* m)
{
...
#endif
    tracepoint(
        osd, ms_fast_dispatch, reqid.name._type, reqid.name._num, reqid.tid,
        reqid.inc);
  }

  if (m->otel_trace.IsValid()) {
    op->osd_parent_span =
        tracing::osd::tracer.add_span("op-request-created", m->otel_trace);
  } else {
    op->osd_parent_span =
        tracing::osd::tracer.start_trace("op-request-created");
  }
  ...
  bool legacy =
      !m->get_connection()->has_features(CEPH_FEATUREMASK_SERVER_TENTACLE);
  spg_t spg = static_cast<MOSDFastDispatchOp*>(m)->get_spg();
  if (legacy) {
    // Optimization - replica pools and EC shard 0 are never remapped
    if ((spg.shard == shard_id_t::NO_SHARD) || (spg.shard == shard_id_t(0))) {
      legacy = false;
    }
  }
  if (!legacy &&
...
bool
OSD::ms_handle_fast_authentication(Connection* con)
{
...
void
OSD::handle_fast_scrub(MOSDScrub2* m)
{
  dout(10) << __func__ << " " << *m << dendl;
...
std::optional<PGLockWrapper>
OSDService::get_locked_pg(spg_t pgid)
{
  ...
  vector<DaemonHealthMetric> metrics;
  {
    ...
    auto count_slow_ops = [&](TrackedOp& op) {
      if (op.get_initiated() < too_old) {
        stringstream ss;
        ss << "slow request " << op.get_desc() << " initiated "
           << op.get_initiated() << " currently " << op.state_string();
...
void
OSD::osdmap_subscribe(version_t epoch, bool force_request)
{
...
void
OSD::trim_maps(epoch_t oldest)
{
  epoch_t min = std::min(oldest, service.map_cache.cached_key_lower_bound());
  dout(20) << __func__ << ": min=" << min
           << " oldest_map=" << superblock.get_oldest_map() << dendl;
  ...
  while (superblock.get_oldest_map() < min &&
    ...
    dout(20) << " removing old osdmap epoch " << superblock.get_oldest_map()
             << dendl;
    t.remove(
        coll_t::meta(), get_osdmap_pobject_name(superblock.get_oldest_map()));
    t.remove(
        coll_t::meta(),
        get_inc_osdmap_pobject_name(superblock.get_oldest_map()));
...
std::optional<epoch_t>
OSD::get_epoch_from_osdmap_object(const ghobject_t& osdmap)
{
...
OSD::trim_stale_maps()
{
  ...
  int r = store->collection_list(
      service.meta_ch, ghobject_t{}, ghobject_t::get_max(), INT_MAX, &objects,
      NULL);
  if (r != 0) {
  ...
  if (t.get_num_ops() > 0) {
    num_removed += t.get_num_ops();
    int tr =
        store->queue_transaction(service.meta_ch, t.claim_and_reset(), nullptr);
    ceph_assert(tr == 0);
  }
...
OSD::handle_osd_map(MOSDMap* m)
{
  // wait for pgs to catch up
  {
    ...
    if (osd_min > 0 && osdmap_epoch > max_lag &&
  ...
  dout(3) << "handle_osd_map epochs [" << first << "," << last << "], i have "
          << superblock.get_newest_map() << ", src has ["
  ...
  dout(10) << __func__ << ": received " << m->maps.size() << " full maps "
           << "and " << m->incremental_maps.size() << " incremental maps"
           << dendl;
  ...
  for (epoch_t e = start; e <= last; e++) {
    ...
    if (p != m->incremental_maps.end()) {
      ...
      if (o->apply_incremental(inc) < 0) {
        derr << "ERROR: bad fsid?  i have " << get_osdmap()->get_fsid()
      ...
      if ((inc.have_crc && o->get_crc() != inc.full_crc) || injected_failure) {
        ...
        *_dout << dendl;
        ...
        if (last < start) {
          dout(10) << __func__ << " bailing because last < start (" << last
                   << "<" << start << ")" << dendl;
          m->put();
          return;
        }
  ...
  if (superblock.purged_snaps_last == start - 1) {
    OSDriver osdriver{store.get(), service.meta_ch, make_purged_snaps_oid()};
    SnapMapper::record_purged_snaps(
        cct, osdriver, osdriver.get_transaction(&t), purged_snaps);
 ...
 */
void
OSD::track_pools_and_pg_num_changes(
    ...
    ObjectStore::Transaction& t)
{
  ...
  if (superblock.is_maps_empty()) {
    ...
    if (!(lastmap = service.try_get_map(superblock.get_newest_map()))) {
      // This is unexpected
      ceph_abort();
    }
  }
  ...
  for (auto& [current_added_map_epoch, current_added_map] : added_maps) {
    _track_pools_and_pg_num_changes(
        t, lastmap, current_added_map, current_added_map_epoch);
...
void
OSD::_track_pools_and_pg_num_changes(
    ObjectStore::Transaction& t,
    ...
    epoch_t current_added_map_epoch)
{
  ...
  for (auto& [pool_id, pg_pool] : lastmap->get_pools()) {
    if (!current_added_map->have_pg_pool(pool_id)) {
      ...
      dout(10) << __func__ << " recording final pg_pool_t for pool " << pool_id
               << dendl;
      ...
      pg_num_history.log_pg_num_change(
          current_added_map_epoch, pool_id, new_pg_num);
    }
  }
  ...
  for (auto& [pool_id, pg_pool] : current_added_map->get_pools()) {
    if (!lastmap->have_pg_pool(pool_id)) {
      ...
      pg_num_history.log_pg_num_change(
          current_added_map_epoch, pool_id, pg_pool.get_pg_num());
    }
  }
}

void
OSD::_committed_osd_maps(epoch_t first, epoch_t last, MOSDMap* m)
{
  ...
  if (osdmap->is_up(whoami) &&
      osdmap->get_addrs(whoami).legacy_equals(client_messenger->get_myaddrs()) &&
    ...
    if (!osdmap->exists(whoami)) {
      ...
      if (!osdmap->is_up(whoami)) {
        ...
        if (monc->monmap.min_mon_release >= ceph_release_t::octopus) {
          ...
          monc->send_mon_message(new MOSDMarkMeDead(
              monc->get_fsid(), whoami, osdmap->get_epoch()));
        }
      ...
      if (!service.is_stopping()) {
        ...
        if ((int)osd_markdown_log.size() > cct->_conf->osd_max_markdown_count) {
          derr << __func__ << " marked down " << osd_markdown_log.size()
        ...
        if (r != 0) {
          ...
          network_error = true;
          derr << __func__
               << " marked down:" << " rebind cluster_messenger failed"
               << dendl;
        }
...
OSD::check_osdmap_features()
{
  ...
  if (osdmap->require_osd_release != last_require_osd_release) {
    ...
    store->write_meta(
        "require_osd_release", stringify((int)osdmap->require_osd_release));
...
void
OSD::_finish_splits(set<PGRef>& pgs)
{
...
bool
OSD::add_merge_waiter(OSDMapRef nextmap, spg_t target, PGRef src, unsigned need)
{
  std::lock_guard l(merge_lock);
    ...
    PeeringCtx& rctx)
{
  ...
  unsigned old_pg_num = lastmap->have_pg_pool(pg->pg_id.pool())
                            ...
                            : 0;
  for (epoch_t next_epoch = first_new_epoch; next_epoch <= osd_epoch;
       ++next_epoch) {
    ...
    if (old_pg_num && new_pg_num && old_pg_num != new_pg_num) {
      ...
      if (nextmap->have_pg_pool(pg->pg_id.pool())) {
        ...
        if (pg->pg_id.is_merge_source(old_pg_num, new_pg_num, &parent)) {
          ...
          if (!new_pgs.empty()) {
            rctx.transaction.register_on_applied(
                new C_FinishSplits(this, new_pgs));
            new_pgs.clear();
          }
    ...
    if (oldpool != lastmap->get_pools().end() &&
      ...
      oldpool->second.opts.get(
          pool_opts_t::SCRUB_MIN_INTERVAL, &old_min_interval);
      newpool->second.opts.get(
          pool_opts_t::SCRUB_MIN_INTERVAL, &new_min_interval);

      double old_max_interval = 0, new_max_interval = 0;
      oldpool->second.opts.get(
          pool_opts_t::SCRUB_MAX_INTERVAL, &old_max_interval);
      newpool->second.opts.get(
          pool_opts_t::SCRUB_MAX_INTERVAL, &new_max_interval);
    }
...
OSD::consume_map()
{
  ...
  service.await_reserved_maps();
  service.publish_map(osdmap);
  dout(20) << "consume_map " << osdmap->get_epoch() << " -- publish done"
           << dendl;
...
bool
OSD::require_mon_peer(const Message* m)
{
...
bool
OSD::require_mon_or_mgr_peer(const Message* m)
{
  if (!m->get_connection()->peer_is_mon() &&
...
bool
OSD::require_osd_peer(const Message* m)
{
    ...
    PeeringCtx& rctx)
{
  ...
  for (set<spg_t>::const_iterator i = childpgids.begin(); i != childpgids.end();
...

    {
      ...
      ceph_assert(NULL != shards[shard_index]);
      store->set_collection_commit_queue(
          child->coll, &(shards[shard_index]->context_queue));
    }
    ...
    parent->split_colls(
        *i, split_bits, i->ps(), &child->get_pgpool().info, rctx.transaction);
    ...
    ThreadPool::TPHandle* handle)
{
  ...
  if ((!ctx.transaction.empty() || ctx.transaction.has_contexts()) && pg) {
    int tr = store->queue_transaction(
        pg->ch, std::move(ctx.transaction), TrackedOpRef(), handle);
    ceph_assert(tr == 0);
  }
}

void
OSD::handle_fast_pg_create(MOSDPGCreate2* m)
{
    ...
    if (q == m->pg_extra.end()) {
      ...
      if (!q->second.second.empty() &&
        ...
        enqueue_peering_evt(
            pgid,
            PGPeeringEventRef(std::make_shared<PGPeeringEvent>(
                m->epoch, m->epoch, PgCreateEvt(), true,
                new PGCreateInfo(
...
void
OSD::handle_fast_pg_notify(MOSDPGNotify* m)
{
  dout(7) << __func__ << " " << *m << " from " << m->get_source() << dendl;
  ...
  for (auto& p : m->get_pg_list()) {
    ...
    enqueue_peering_evt(
        ...
        PGPeeringEventRef(std::make_shared<PGPeeringEvent>(
            ...
            MNotifyRec(
                pgid, pg_shard_t(from, p.from), p,
                m->get_connection()->get_features()),
            true,
            new PGCreateInfo(
                pgid, p.query_epoch, p.info.history, p.past_intervals, false))));
  }
  m->put();
}

void
OSD::handle_fast_pg_info(MOSDPGInfo* m)
{
  dout(7) << __func__ << " " << *m << " from " << m->get_source() << dendl;
  if (!require_osd_peer(m)) {
    m->put();
...
void
OSD::handle_fast_pg_remove(MOSDPGRemove* m)
{
  dout(7) << __func__ << " " << *m << " from " << m->get_source() << dendl;
  if (!require_osd_peer(m)) {
    m->put();
...
void
OSD::handle_fast_force_recovery(MOSDForceRecovery* m)
{
  dout(10) << __func__ << " " << *m << dendl;
  if (!require_mon_or_mgr_peer(m)) {
    m->put();
...
void
OSD::handle_pg_query_nopg(const MQuery& q)
{
  spg_t pgid = q.pgid;
  dout(10) << __func__ << " " << pgid << dendl;
  ...
  if (con) {
    ...
    if (q.query.type == pg_query_t::LOG || q.query.type == pg_query_t::FULLLOG) {
      m = new MOSDPGLog(
          q.query.from, q.query.to, osdmap->get_epoch(), empty,
          q.query.epoch_sent);
    } else {
      pg_notify_t notify{q.query.from,          q.query.to, q.query.epoch_sent,
...
void
OSDService::queue_check_readable(spg_t spgid, epoch_t lpr, ceph::signedspan delay)
{
  if (delay == ceph::signedspan::zero()) {
    osd->enqueue_peering_evt(
        spgid, PGPeeringEventRef(std::make_shared<PGPeeringEvent>(
                   lpr, lpr, PeeringState::CheckReadable())));
  } else {
    mono_timer.add_event(delay, [this, spgid, lpr]() {
...
bool
OSDService::_recover_now(uint64_t* available_pushes)
{
  if (available_pushes)
    *available_pushes = 0;
...
unsigned
OSDService::get_target_pg_log_entries() const
{
  ...
  if (num_pgs > 0 && target > 0) {
    ...
    return std::max<unsigned>(
        std::min<unsigned>(target / num_pgs, cct->_conf->osd_max_pg_log_entries),
    ...
    ThreadPool::TPHandle& handle)
{
   ...
   */
  float recovery_sleep = pg->is_degraded() ? get_osd_recovery_sleep_degraded()
    ...
    if (recovery_sleep > 0 && service.recovery_needs_sleep) {
      ...
      auto recovery_requeue_callback = new LambdaContext(
          [this, pgref, queued, reserved_pushes, priority](int r) {
            ...
            service.queue_recovery_after_sleep(
                pgref.get(), queued, reserved_pushes, priority);
          });
      ...
      service.sleep_timer.add_event_at(
          service.recovery_schedule_time, recovery_requeue_callback);
...
void
OSDService::start_recovery_op(PG* pg, const hobject_t& soid)
{
...
void
OSDService::finish_recovery_op(PG* pg, const hobject_t& soid, bool dequeue)
{
  std::lock_guard l(recovery_lock);
...
void
OSDService::release_reserved_pushes(uint64_t pushes)
{
  std::lock_guard l(recovery_lock);
...
bool
OSD::op_is_discardable(const MOSDOp* op)
{
...
void
OSD::enqueue_op(spg_t pg, OpRequestRef&& op, epoch_t epoch)
{
  ...
  dout(15) << "enqueue_op " << *op->get_req() << " prio " << priority
           << " type " << type << " cost " << cost << " latency " << latency
           << " epoch " << epoch << dendl;
  ...
  if (PGRecoveryMsg::is_recovery_msg(op)) {
    op_shardedwq.queue(OpSchedulerItem(
        unique_ptr<OpSchedulerItem::OpQueueable>(
            new PGRecoveryMsg(pg, std::move(op))),
        cost, priority, stamp, owner, epoch));
  } else {
    op_shardedwq.queue(OpSchedulerItem(
        unique_ptr<OpSchedulerItem::OpQueueable>(
            new PGOpItem(pg, std::move(op))),
        cost, priority, stamp, owner, epoch));
  }
}

void
OSD::enqueue_peering_evt(spg_t pgid, PGPeeringEventRef evt)
{
 ...
 */
void
OSD::dequeue_op(PGRef pg, OpRequestRef op, ThreadPool::TPHandle& handle)
{
  ...
  dout(10) << "dequeue_op " << *op->get_req() << " prio " << m->get_priority()
           << " cost " << m->get_cost() << " latency " << latency << " " << *m
           << " pg " << *pg << dendl;

  logger->tinc(l_osd_op_before_dequeue_op_lat, latency);

  service.maybe_share_map(
      m->get_connection().get(), pg->get_osdmap(), op->sent_epoch);
  ...
  // finish
  dout(10) << "dequeue_op " << *op->get_req() << " finish" << " latency "
           << (ceph_clock_now() - now) << dendl;
    ...
    ThreadPool::TPHandle& handle)
{
  dequeue_peering_evt(
      sdata, pg,
      PGPeeringEventRef(
          std::make_shared<PGPeeringEvent>(e, e, PeeringState::DeleteSome())),
      handle);
}
...
OSD::get_tracked_keys() const noexcept
{
  return {
      "osd_max_backfills"s, "osd_min_recovery_priority"s,
      "osd_max_trimming_pgs"s, "osd_op_complaint_time"s,
      "osd_op_log_threshold"s, "osd_op_history_size"s,
      "osd_op_history_duration"s, "osd_op_history_slow_op_size"s,
      "osd_op_history_slow_op_threshold"s, "osd_enable_op_tracker"s,
      "osd_map_cache_size"s, "osd_pg_epoch_max_lag_factor"s,
      "osd_pg_epoch_persisted_max_stale"s, "osd_recovery_sleep"s,
      "osd_recovery_sleep_hdd"s, "osd_recovery_sleep_ssd"s,
      "osd_recovery_sleep_hybrid"s, "osd_recovery_sleep_degraded"s,
      "osd_recovery_sleep_degraded_hdd"s, "osd_recovery_sleep_degraded_ssd"s,
      "osd_recovery_sleep_degraded_hybrid"s, "osd_delete_sleep"s,
      "osd_delete_sleep_hdd"s, "osd_delete_sleep_ssd"s,
      "osd_delete_sleep_hybrid"s, "osd_snap_trim_sleep"s,
      "osd_snap_trim_sleep_hdd"s, "osd_snap_trim_sleep_ssd"s,
      "osd_snap_trim_sleep_hybrid"s, "osd_scrub_sleep"s,
      "osd_recovery_max_active"s, "osd_recovery_max_active_hdd"s,
      "osd_recovery_max_active_ssd"s,
      // clog & admin clog
      "clog_to_monitors"s, "clog_to_syslog"s, "clog_to_syslog_facility"s,
      "clog_to_syslog_level"s, "osd_objectstore_fuse"s, "clog_to_graylog"s,
      "clog_to_graylog_host"s, "clog_to_graylog_port"s, "host"s, "fsid"s,
      "osd_recovery_delay_start"s, "osd_client_message_size_cap"s,
      "osd_client_message_cap"s, "osd_heartbeat_min_size"s,
      "osd_heartbeat_interval"s, "osd_object_clean_region_max_num_intervals"s,
      "osd_scrub_min_interval"s, "osd_scrub_max_interval"s,
      "osd_deep_scrub_interval"s, "osd_deep_scrub_interval_cv"s,
      "osd_scrub_interval_randomize_ratio"s, "osd_op_thread_timeout"s,
      "osd_op_thread_suicide_timeout"s, "osd_max_scrubs"s};
}
    ...
    const std::set<std::string>& changed)
{
  ...
  if (changed.count("osd_max_scrubs")) {
    dout(0) << fmt::format(
                   "{}: scrub concurrency max changed to {}", __func__,
  ...
  if (changed.count("osd_op_thread_timeout")) {
    op_shardedwq.set_timeout(
        g_conf().get_val<int64_t>("osd_op_thread_timeout"));
  }
  if (changed.count("osd_op_thread_suicide_timeout")) {
    op_shardedwq.set_suicide_timeout(
        g_conf().get_val<int64_t>("osd_op_thread_suicide_timeout"));
  }
}

void
OSD::maybe_override_max_osd_capacity_for_qos()
{
  ...
  if ((op_queue_type_t::mClockScheduler == osd_op_queue_type()) &&
    ...
    if (!force_run_benchmark) {
      ...
      if (!force_run_benchmark && (default_iops != cur_iops)) {
        dout(1) << __func__ << std::fixed << std::setprecision(2)
                ...
                << " cur_iops: " << cur_iops << ". Skip OSD benchmark test."
                << dendl;
        return;
      }
    }
    ...
    int ret = osd_bench.run_test();
    if (ret != 0) {
      derr << __func__ << " osd bench err: " << ret
           << " osd bench errstr: " << osd_bench.get_errstr() << dendl;
      return;
    }
    ...
    if (store_is_rotational) {
      hi_threshold_iops =
          cct->_conf.get_val<double>("osd_mclock_iops_capacity_threshold_hdd");
...
bool
OSD::maybe_override_options_for_qos(const std::set<std::string>* changed)
{
  ...
  if (op_queue_type_t::mClockScheduler == osd_op_queue_type()) {
    ...
    if (changed != nullptr) {
      ...
      for (auto opt : recovery_qos_defaults) {
        ...
        dout(1) << __func__ << " " << opt.first << " set to " << opt_val
                << dendl;
...
void
OSD::maybe_override_sleep_options_for_qos()
{
  // Override options only if the scheduler enabled is mclock and the
  // underlying objectstore is supported by mclock
  if (op_queue_type_t::mClockScheduler == osd_op_queue_type()) {
...
void
OSD::mon_cmd_set_config(const std::string& key, const std::string& val)
{
  std::string cmd =
      "{"
      "\"prefix\": \"config set\", "
...
void
OSD::set_perf_queries(const ConfigPayload& config_payload)
{
...
void
OSDShard::_attach_pg(OSDShardPGSlot* slot, PG* pg)
{
...
void
OSDShard::_detach_pg(OSDShardPGSlot* slot)
{
...
void
OSDShard::update_pg_epoch(OSDShardPGSlot* slot, epoch_t e)
{
...
void
OSDShard::wait_min_pg_epoch(epoch_t need)
{
...
void
OSDShard::consume_map(const OSDMapRef& new_osdmap, unsigned* pushes_to_free)
{
  std::lock_guard l(shard_lock);
...
int
OSDShard::_wake_pg_slot(spg_t pgid, OSDShardPGSlot* slot)
{
  int count = 0;
...
void
OSDShard::identify_splits_and_merges(
    ...
    set<pair<spg_t, epoch_t>>* merge_pgs)
{
  ...
  if (shard_osdmap) {
    for (auto& i : pg_slots) {
      dout(20) << __func__ << " slot pgid:" << i.first
               << "slot:" << i.second.get() << dendl;
      const spg_t& pgid = i.first;
      auto* slot = i.second.get();
      if (slot->pg) {
        osd->service.identify_splits_and_merges(
            shard_osdmap, as_of_osdmap, pgid, split_pgs, merge_pgs);
      } else if (!slot->waiting_for_split.empty()) {
        osd->service.identify_splits_and_merges(
            shard_osdmap, as_of_osdmap, pgid, split_pgs, nullptr);
      } else {
...
void
OSDShard::_prime_splits(set<pair<spg_t, epoch_t>>* pgids)
{
  dout(10) << *pgids << dendl;
  auto p = pgids->begin();
  while (p != pgids->end()) {
...
void
OSDShard::register_and_wake_split_child(PG* pg)
{
  ...
  osd->enqueue_peering_evt(
      pg->pg_id, PGPeeringEventRef(std::make_shared<PGPeeringEvent>(
                     epoch, epoch, NullEvt())));
...
void
OSDShard::unprime_split_children(spg_t parent, unsigned old_pg_num)
{
  std::lock_guard l(shard_lock);
...
op_queue_type_t
OSDShard::get_op_queue_type() const
{
...
OSDShard::OSDShard(
  ...
  ec_extent_cache_lru(cct->_conf.get_val<uint64_t>("ec_extent_cache_size"))
{
...
void
OSD::ShardedOpWQ::_add_slot_waiter(
    spg_t pgid,
    OSDShardPGSlot* slot,
    OpSchedulerItem&& qi)
{
    ...
    heartbeat_handle_d* hb)
{
  ...
  if (sdata->scheduler->empty() &&
    ...
    if (is_smallest_thread_index && !sdata->context_queue.empty()) {
      ...
      osd->cct->get_heartbeat_map()->reset_timeout(
          hb, timeout_interval.load(), suicide_interval.load());
    } else {
  ...
  while (!std::get_if<OpSchedulerItem>(&work_item)) {
    if (sdata->scheduler->empty()) {
      if (osd->is_stopping()) {
        ...
        for (auto c : oncommits) {
          dout(10) << __func__ << " discarding in-flight oncommit " << c
                   << dendl;
          delete c;
        }
    ...
    if (auto when_ready = std::get_if<double>(&work_item)) {
      ...
      dout(10) << __func__ << " dequeue future request at " << future_time
               << dendl;
    ...
    if (!slot->waiting_for_split.empty()) {
      ...
      if (std::optional<OpRequestRef> _op = qi.maybe_get_op()) {
        osd->service.maybe_share_map(
            (*_op)->get_req()->get_connection().get(), sdata->shard_osdmap,
            (*_op)->sent_epoch);
      }
...
#endif
    tracepoint(
        osd, opwq_process_start, reqid.name._type, reqid.name._num, reqid.tid,
        reqid.inc);
  }
...
#endif
    tracepoint(
        osd, opwq_process_finish, reqid.name._type, reqid.name._num, reqid.tid,
        reqid.inc);
  }
...
void
OSD::ShardedOpWQ::_enqueue(OpSchedulerItem&& item)
{
...
void
OSD::ShardedOpWQ::_enqueue_front(OpSchedulerItem&& item)
{
  if (unlikely(m_fast_shutdown)) {
    // stop enqueing when we are in the middle of a fast shutdown
    return;
...
void
OSD::ShardedOpWQ::stop_for_fast_shutdown()
{
...
OSDBenchTest::precheck()
{
  ...
  if (bsize > (int64_t)cct->_conf->osd_bench_max_block_size) {
    ...
    int64_t max_count = bsize * duration *
                        cct->_conf->osd_bench_small_size_max_iops;
    if (count > max_count) {
      errmsg << "'count' values greater than " << max_count
             << " for a block size of " << byte_u_t(bsize) << ", assuming "
             << cct->_conf->osd_bench_small_size_max_iops << " IOPS," << " for "
             << duration << " seconds," << " can cause ill effects on osd. "
    ...
    int64_t max_count = cct->_conf->osd_bench_large_size_max_throughput *
                        duration;
    if (count > max_count) {
      errmsg << "'count' values greater than " << max_count
             << " for a block size of " << byte_u_t(bsize) << ", assuming "
  ...
  if (osize && bsize > osize) {
    dout(0) << fmt::format(
                   ...
                   " overriding bsize to {}.",
                   __func__, byte_u_t(bsize), byte_u_t(osize), byte_u_t(osize))
            << dendl;
    bsize = osize;
  }
...
OSDBenchTest::run_test()
{
  ...
  if (elapsed && bsize) {
    ...
    errmsg << "Unable to determine bench result."
           << " elapsed time: " << elapsed << " bsize: " << bsize;
  }
 ...
 */
void
OSDBenchTest::wait_for_flush_commit()
{
  C_SaferCond waiter;
  if (!ch->flush_commit(&waiter)) {
    waiter.wait();
...
OSDBenchTest::prefill_objects()
{
  if (osize && onum) {
    ...
    dout(0) << fmt::format(
                   "{}: Prefill took {:.2f} secs.", __func__, prefill_time)
            << dendl;
  } else {
 ...
 */
void
OSDBenchTest::perform_write_test()
{
  std::mt19937 random_gen(std::random_device{}());
  bufferlist bl;

  utime_t start = ceph_clock_now();
  for (int64_t bytes_written = 0; bytes_written < count;
       bytes_written += bsize) {
    std::string nm;
    unsigned offset = 0;
    bufferptr bp(bsize);
  ...
  dout(0) << fmt::format("{}: Test took {:.2f} secs.", __func__, elapsed)
          << dendl;
}
