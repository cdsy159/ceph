...
void
osd_info_t::dump(Formatter* f) const
{
...
void
osd_info_t::encode(ceph::buffer::list& bl) const
{
  using ceph::encode;
  __u8 struct_v = 1;
  encode(struct_v, bl);
...
void
osd_info_t::decode(ceph::buffer::list::const_iterator& bl)
{
  using ceph::decode;
  __u8 struct_v;
  decode(struct_v, bl);
...
list<osd_info_t>
osd_info_t::generate_test_instances()
{
  list<osd_info_t> o;
  o.emplace_back();
  o.emplace_back();
...
ostream&
operator<<(ostream& out, const osd_info_t& info)
{
...
void
osd_xinfo_t::dump(Formatter* f) const
{
...
void
osd_xinfo_t::encode(ceph::buffer::list& bl, uint64_t enc_features) const
{
  uint8_t v = 4;
...
void
osd_xinfo_t::decode(ceph::buffer::list::const_iterator& bl)
{
  DECODE_START(4, bl);
...
list<osd_xinfo_t>
osd_xinfo_t::generate_test_instances()
{
  list<osd_xinfo_t> o;
  o.emplace_back();
  o.emplace_back();
...
ostream&
operator<<(ostream& out, const osd_xinfo_t& xi)
{
...
int
OSDMap::Incremental::get_net_marked_out(const OSDMap* previous) const
{
  int n = 0;
...
int
OSDMap::Incremental::get_net_marked_down(const OSDMap* previous) const
{
  int n = 0;
...
int
OSDMap::Incremental::identify_osd(uuid_d u) const
{
...
int
OSDMap::Incremental::propagate_base_properties_to_tiers(
    CephContext* cct,
    const OSDMap& osdmap)
{
...
bool
OSDMap::subtree_is_down(int id, set<int>* down_cache) const
{
  if (id >= 0)
    return is_down(id);
    ...
    unordered_map<int, set<int>>* subtree_type_down) const
{
  ...
  for (const auto& child : children) {
    if (!subtree_type_is_down(
            cct, child, crush->get_bucket_type(child), down_in_osds, up_in_osds,
            subtree_up, subtree_type_down)) {
...
void
OSDMap::Incremental::encode_client_old(ceph::buffer::list& bl) const
{
  using ceph::encode;
  __u16 v = 5;
  encode(v, bl);
...
template <class T>
static void
encode_addrvec_map_as_addr(const T& m, ceph::buffer::list& bl, uint64_t f)
{
  uint32_t n = m.size();
  encode(n, bl);
  for (auto& i : m) {
...
template <class T>
static void
encode_addrvec_pvec_as_addr(const T& m, ceph::buffer::list& bl, uint64_t f)
{
  uint32_t n = m.size();
  encode(n, bl);
  for (auto& i : m) {
 ...
 */
void
OSDMap::Incremental::encode(ceph::buffer::list& bl, uint64_t features) const
{
  using ceph::encode;
...
void
OSDMap::Incremental::decode_classic(ceph::buffer::list::const_iterator& p)
{
  using ceph::decode;
  __u32 n, t;
  // base
 ...
 */
void
OSDMap::Incremental::decode(ceph::buffer::list::const_iterator& bl)
{
  using ceph::decode;
  /**
...
void
OSDMap::Incremental::dump(Formatter* f) const
{
  ...
  f->dump_int(
      ...
      to_integer<int>(new_require_min_compat_client));
  f->dump_int(
      "new_require_osd_release", to_integer<int>(new_require_osd_release));
  f->dump_unsigned(
      "mutate_allow_crimson", static_cast<unsigned>(mutate_allow_crimson));
...
auto
OSDMap::Incremental::generate_test_instances() -> list<Incremental>
{
  list<Incremental> o;
  o.emplace_back();
  return o;
...
void
OSDMap::range_bits::get_ipv6_bytes(
...
void
OSDMap::range_bits::parse(const entity_addr_t& addr)
{
  // parse it into meaningful data
  if (addr.is_ipv6()) {
    get_ipv6_bytes(
        addr.in6_addr().sin6_addr.s6_addr, &bits.ipv6.upper_64_bits,
        &bits.ipv6.lower_64_bits);
    int32_t lower_shift =
        std::min(128 - static_cast<int32_t>(addr.get_nonce()), 64);
    int32_t upper_shift = std::max(
        64 - //(128-b.first.get_nonce())-64
...
bool
OSDMap::range_bits::matches(const entity_addr_t& addr) const
{
  if (addr.is_ipv4() && !ipv6) {
    return (
        (ntohl(addr.in4_addr().sin_addr.s_addr) & bits.ipv4.mask) ==
...
bool
OSDMap::is_blocklisted(const entity_addr_t& orig, CephContext* cct) const
{
  if (cct)
    ldout(cct, 25) << "is_blocklisted: " << orig << dendl;
  if (blocklist.empty() && range_blocklist.empty()) {
    if (cct)
      ldout(cct, 30) << "not blocklisted: " << orig << dendl;
    return false;
  }
  ...
  if (blocklist.count(a)) {
    if (cct)
      ldout(cct, 20) << "blocklist contains " << a << dendl;
    return true;
  }
  ...
  for (const auto& i : calculated_ranges) {
    ...
    if (blocked) {
      if (cct)
        ldout(cct, 20) << "range_blocklist contains " << a << dendl;
      return true;
    }
  }

  if (cct)
    ldout(cct, 25) << "not blocklisted: " << orig << dendl;
  return false;
}

bool
OSDMap::is_blocklisted(const entity_addrvec_t& av, CephContext* cct) const
{
  if (blocklist.empty() && range_blocklist.empty())
    return false;
...
void
OSDMap::get_out_of_subnet_osd_counts(
    CephContext* cct,
...
void
OSDMap::get_all_osds(set<int32_t>& ls) const
{
  for (int i = 0; i < max_osd; i++)
    if (exists(i))
      ls.insert(i);
...
void
OSDMap::get_up_osds(set<int32_t>& ls) const
{
  for (int i = 0; i < max_osd; i++) {
    if (is_up(i))
      ls.insert(i);
...
void
OSDMap::get_out_existing_osds(set<int32_t>& ls) const
{
  for (int i = 0; i < max_osd; i++) {
...
void
OSDMap::get_flag_set(set<string>* flagset) const
{
...
void
OSDMap::calc_state_set(int state, set<string>& st)
{
  unsigned t = state;
...
void
OSDMap::adjust_osd_weights(const map<int, double>& weights, Incremental& inc) const
{
  float max = 0;
  ...
  for (const auto& weight : weights) {
    inc.new_weight[weight.first] = (unsigned)((weight.second / max) *
                                              CEPH_OSD_IN);
  }
}

int
OSDMap::identify_osd(const entity_addr_t& addr) const
{
  for (int i = 0; i < max_osd; i++)
    if (exists(i) &&
...
int
OSDMap::identify_osd(const uuid_d& u) const
{
  for (int i = 0; i < max_osd; i++)
...
int
OSDMap::identify_osd_on_all_channels(const entity_addr_t& addr) const
{
  for (int i = 0; i < max_osd; i++)
    if (exists(i) &&
        (get_addrs(i).contains(addr) || get_cluster_addrs(i).contains(addr) ||
...
int
OSDMap::find_osd_on_ip(const entity_addr_t& ip) const
{
  for (int i = 0; i < max_osd; i++)
...
uint64_t
OSDMap::get_features(int entity_type, uint64_t* pmask) const
{
  ...
  if (crush->has_nondefault_tunables_msr() || crush->has_msr_rules())
...
uint64_t
OSDMap::get_up_osd_features() const
{
  return cached_up_osd_features;
}

bool
OSDMap::any_osd_laggy() const
{
  for (int osd = 0; osd < max_osd; ++osd) {
    if (!is_up(osd)) {
      continue;
...
void
OSDMap::dedup(const OSDMap* o, OSDMap* n)
{
  using ceph::encode;
  if (o->epoch == n->epoch)
    return;
  ...
  // do uuids match?
  if (o->osd_uuid->size() == n->osd_uuid->size() && *o->osd_uuid == *n->osd_uuid)
    ...
    Incremental* pending_inc)
{
  ...
  for (auto pg : *nextmap.pg_temp) {
    ...
    auto acting_set =
        nextmap.pgtemp_undo_primaryfirst(*pool, pg.first, pg.second);
...
void
OSDMap::get_upmap_pgs(vector<pg_t>* upmap_pgs) const
{
  upmap_pgs->reserve(
      pg_upmap.size() + pg_upmap_items.size() + pg_upmap_primaries.size());
  for (auto& p : pg_upmap)
    ...
    map<pg_t, mempool::osdmap::vector<pair<int, int>>>* to_remap) const
{
  ...
  for (auto& pg : to_check) {
      ...
      for (auto& p : j->second) {
        ...
        if (std::find(raw.begin(), raw.end(), osd_from) == raw.end()) {
          ...
          ldout(cct, 20) << __func__
                         << " pg_upmap_items (source osd does not exist) "
                         << pg_upmap_items << dendl;
          continue;
        }
        if (osd_to != CRUSH_ITEM_NONE && osd_to < max_osd && osd_to >= 0 &&
            osd_weight[osd_to] == 0) {
          // cancel mapping if target osd is out
          ldout(cct, 20) << __func__ << " pg_upmap_items (target osd is out) "
                         << pg_upmap_items << dendl;
          continue;
        }
        newmap.push_back(p);
    ...
    if (k != pg_upmap_primaries.end()) {
      ...
      for (auto osd : raw) {
        if ((curr_prim == osd) && (curr_prim != raw.front())) {
          valid_prim = true;
          break;
        }
      }
      if (!valid_prim) {
        ldout(cct, 10) << __func__ << " pg_upmap_primary (PG " << pg
                       << " has an invalid or redundant primary: " << curr_prim
                       << " -> " << raw << ")" << dendl;
  ...
  any_change = any_change || !to_cancel->empty() ||
               !to_cancel_upmap_primary_only->empty();
  return any_change;
}
    ...
    const map<pg_t, mempool::osdmap::vector<pair<int, int>>>& to_remap) const
{
  for (auto& pg : to_cancel) {
    ...
    if (k != pending_inc->new_pg_upmap_primary.end()) {
      ldout(cct, 10) << __func__ << " cancel invalid pending "
                     << "pg_upmap_primaries entry " << k->first << "->"
                     << k->second << dendl;
  ...
  for (auto pid : affected_pools) {
    ldout(cct, 10) << __func__ << " cancel all pg_upmap_primaries for pool "
                   << pid << " since pg_num changed" << dendl;
    rm_all_upmap_prims(cct, pending_inc, pid);
  }
}

bool
OSDMap::clean_pg_upmaps(CephContext* cct, Incremental* pending_inc) const
{
  ldout(cct, 10) << __func__ << dendl;
  ...
  auto any_change = check_pg_upmaps(
      cct, to_check, &to_cancel, &to_cancel_upmap_primary_only, &affected_pools,
      &to_remap);
  clean_pg_upmaps(
      cct, pending_inc, to_cancel, to_cancel_upmap_primary_only, affected_pools,
      to_remap);
  return any_change;
}
...
ceph_object_layout
OSDMap::make_object_layout(object_t oid, int pg_pool, string nspace) const
{
...
void
OSDMap::_remove_nonexistent_osds(const pg_pool_t& pool, vector<int>& osds) const
{
...
int
OSDMap::_pick_primary(const vector<int>& osds) const
{
  for (auto osd : osds) {
...
void
OSDMap::_apply_upmap(const pg_pool_t& pi, pg_t raw_pg, vector<int>* raw) const
{
    ...
    for (auto& [osd_from, osd_to] : q->second) {
      ...
      for (unsigned i = 0; i < raw->size(); ++i) {
        ...
        if (osd == osd_from && pos < 0 &&
            !(osd_to != CRUSH_ITEM_NONE && osd_to < max_osd && osd_to >= 0 &&
              osd_weight[osd_to] == 0)) {
          pos = i;
        }
      }
...
void
OSDMap::_apply_primary_affinity(
    ps_t seed,
    const pg_pool_t& pool,
    vector<int>* osds,
    int* primary) const
...
{
  if ((shard == shard_id_t::NO_SHARD) || (shard == shard_id_t(0))) {
    return shard;
  }
...
void
OSDMap::pg_to_raw_osds(pg_t pg, vector<int>* raw, int* primary) const
{
...
void
OSDMap::pg_to_raw_upmap(pg_t pg, vector<int>* raw, vector<int>* raw_upmap) const
{
...
void
OSDMap::pg_to_raw_up(pg_t pg, vector<int>* up, int* primary) const
{
  const pg_pool_t* pool = get_pg_pool(pg.pool());
  if (!pool) {
    *primary = -1;
...
int
OSDMap::calc_pg_role_broken(int osd, const vector<int>& acting, int nrep)
{
...
int
OSDMap::calc_pg_role(pg_shard_t who, const vector<int>& acting)
{
...
uint64_t
OSDMap::get_encoding_features() const
{
  ...
  if (require_osd_release < ceph_release_t::jewel) {
    f &=
        ~(CEPH_FEATURE_SERVER_JEWEL | CEPH_FEATURE_NEW_OSDOP_ENCODING |
...
void
OSDMap::encode_client_old(ceph::buffer::list& bl) const
{
  using ceph::encode;
  __u16 v = 5;
  encode(v, bl);
...
void
OSDMap::encode_classic(ceph::buffer::list& bl, uint64_t features) const
{
  using ceph::encode;
  if ((features & CEPH_FEATURE_PGID64) == 0) {
    encode_client_old(bl);
 ...
 */
void
OSDMap::encode(ceph::buffer::list& bl, uint64_t features) const
{
  using ceph::encode;
  if ((features & CEPH_FEATURE_OSDMAP_ENC) == 0) {
    encode_classic(bl, features);
 ...
 */
void
OSDMap::decode(ceph::buffer::list& bl)
{
  auto p = bl.cbegin();
  decode(p);
}
...
void
OSDMap::decode_classic(ceph::buffer::list::const_iterator& p)
{
  using ceph::decode;
  __u32 n, t;
  __u16 v;
...
void
OSDMap::decode(ceph::buffer::list::const_iterator& bl)
{
  using ceph::decode;
  /**
...
void
OSDMap::dump_erasure_code_profiles(
...
void
OSDMap::dump_osds(Formatter* f) const
{
  f->open_array_section("osds");
...
void
OSDMap::dump_osd(int id, Formatter* f) const
{
...
void
OSDMap::dump_read_balance_score(
    CephContext* cct,
    int64_t pid,
    const pg_pool_t& pdata,
    ceph::Formatter* f) const
  ...
  if (pdata.is_replicated()) {
    ...
    if (rc >= 0) {
      ...
      if (rb_info.score_type == RBS_FAIR) {
        ...
        f->dump_float(
            "average_primary_affinity_weighted", rb_info.pa_weighted_avg);
...
void
OSDMap::dump(Formatter* f, CephContext* cct) const
{
  ...
  f->dump_string(
      "require_min_compat_client", to_string(require_min_compat_client));
...
list<OSDMap>
OSDMap::generate_test_instances()
{
  list<OSDMap> o;
  o.emplace_back();
...
void
OSDMap::print_pools(CephContext* cct, ostream& out) const
{
  for (const auto& [pid, pdata] : pools) {
    std::string name("<unknown>");
    const auto& pni = pool_name.find(pid);
    ...
    if (pdata.is_replicated()) {
      ...
      if (rc >= 0)
        snprintf(
            rb_score_str, sizeof(rb_score_str), " read_balance_score %.2f",
            rb_info.acting_adj_score);
    }
...
void
OSDMap::print_osds(ostream& out) const
{
  for (int i = 0; i < get_max_osd(); i++) {
    if (exists(i)) {
      print_osd(i, out);
...
void
OSDMap::print_osd(int id, ostream& out) const
{
  if (!exists(id)) {
    return;
  }
...
void
OSDMap::print(CephContext* cct, ostream& out) const
{
  out << "epoch " << get_epoch() << "\n"
      << "fsid " << get_fsid() << "\n"
      << "created " << get_created() << "\n"
...
public:
  ...
  bool
  should_dump_leaf(int i) const override
  {
    if (!filter) {
  ...
  bool
  should_dump_empty_bucket() const override
  {
    return !filter;
  }

  void
  init_table(TextTable* tbl)
  {
  ...
  void
  dump(TextTable* tbl, string& bucket)
  {
    init_table(tbl);
...
protected:
  void
  dump_item(const CrushTreeDumper::Item& qi, TextTable* tbl) override
  {
...
public:
  ...
  void
  dump(Formatter* f, string& bucket)
  {
    if (!bucket.empty()) {
      set_root(bucket);
...
protected:
  void
  dump_item_fields(const CrushTreeDumper::Item& qi, Formatter* f) override
  {
...
void
OSDMap::print_tree(Formatter* f, ostream* out, unsigned filter, string bucket)
    const
{
  if (f) {
...
void
OSDMap::print_oneline_summary(ostream& out) const
{
...
bool
OSDMap::crush_rule_in_use(int rule_id) const
{
  for (const auto& pool : pools) {
...
int
OSDMap::validate_crush_rules(CrushWrapper* newcrush, ostream* ss) const
{
  for (auto& i : pools) {
    auto& pool = i.second;
    int ruleno = pool.get_crush_rule();
    ...
    bool default_pool)
{
  ...
  const int default_replicated_rule =
      crush->get_osd_pool_default_crush_replicated_rule(cct);
...
int
OSDMap::get_erasure_code_profile_default(
    CephContext* cct,
    map<string, string>& profile_map,
    ostream* ss)
{
...
int
OSDMap::_build_crush_types(CrushWrapper& crush)
{
    ...
    ostream* ss)
{
  ...
  int rootid;
  int r = crush.add_bucket(
      0, 0, CRUSH_HASH_DEFAULT, root_type, 0, NULL, NULL, &rootid);
  ceph_assert(r == 0);
  crush.set_item_name(rootid, "default");

  map<string, string> loc{
      {"host", "localhost"}, {"rack", "localrack"}, {"root", "default"}};
...
int
OSDMap::build_simple_crush_rules(
    CephContext* cct,
    CrushWrapper& crush,
    const string& root,
    ostream* ss)
{
  ...
  r = crush.add_simple_rule_at(
      "replicated_rule", root, failure_domain, "", "firstn",
      pg_pool_t::TYPE_REPLICATED, crush_rule, ss);
  if (r < 0)
    return r;
...
int
OSDMap::summarize_mapping_stats(
    OSDMap* newmap,
    ...
    vector<int>* out) ///< resulting alternative mapping
{
  ...
  int r = crush->try_remap_rule(
      cct, rule, pool->get_size(), overfull, underfull, more_underfull, *orig,
      out);
  if (r < 0)
    return false;
  if (*out == *orig)
    ...
    const std::optional<rb_policy>& rbp) const
{
  ...
  pgs_by_osd = tmp_osd_map.get_pgs_by_osd(
      cct, pid, &prim_pgs_by_osd, &acting_prims_by_osd);
  ...
  int rc = 0;
  rc = calc_desired_primary_distribution(
      cct, pid, osds_to_check, desired_prim_dist, rbp);
  if (rc < 0) {
    ldout(cct, 10) << __func__
                   << " Error in calculating desired primary distribution"
                   << dendl;
    return -EINVAL;
  }
  ...
  for (auto osd : osds_to_check) {
    ...
    ldout(cct, 10) << __func__ << " desired distribution for osd." << osd << " "
                   << desired << dendl;
  }
  ...
  if (rb_info.err_msg.length() > 0) {
    ldout(cct, 10) << __func__ << (rc < 0 ? " ERROR: " : " Warning: ")
                   << rb_info.err_msg << dendl;
    return -EINVAL;
  }
  ...
  while (true) {
    ...
    for (const auto& [pg, mapped] : prim_pgs_to_check) {
      ...
      tmp_osd_map.pg_to_up_acting_osds(
      ...
      auto legal_swap =
          crush->verify_upmap(cct, crush_rule, pool_size, {(int)curr_best_osd});
      ...
      ldout(cct, 20) << __func__ << " curr_num_changes: " << curr_num_changes
                     << dendl;
    }
    ...
    if (curr_num_changes == 0) {
      ldout(cct, 20)
          << __func__
          << " curr_num_changes is 0; no further optimizations can be made."
          << dendl;
      break;
    }
  }
  ...
  for (auto& [_, pgs] : prim_pgs_by_osd) {
    for (auto& pg : pgs) {
      if (pending_inc->new_pg_upmap_primary.contains(pg)) {
        ldout(cct, 30) << __func__ << " Removing pending pg_upmap_prim for pg "
                       << pg << dendl;
        pending_inc->new_pg_upmap_primary.erase(pg);
      }
      if (pg_upmap_primaries.contains(pg)) {
        ldout(cct, 30) << __func__ << " Removing pg_upmap_prim for pg " << pg
                       << dendl;
        pending_inc->old_pg_upmap_primary.insert(pg);
      }
    }
    ...
    const std::optional<rb_policy>& rbp) const
{
  ...
  switch (policy) {
  case RB_SIMPLE:
    return calc_desired_primary_distribution_simple(
        cct, pid, osds, desired_primary_distribution);
  case RB_OSDSIZEOPT:
    return calc_desired_primary_distribution_osdsize_opt(
        cct, pid, osds, desired_primary_distribution);
  default:
    ldout(cct, 10) << __func__ << " invalid read balance policy" << int(policy)
                   << dendl;
    return -EINVAL;
  }
}
...
int
OSDMap::calc_desired_primary_distribution_simple(
    CephContext* cct,
    int64_t pid,
    const vector<uint64_t>& osds,
    map<uint64_t, float>& desired_primary_distribution) const
{
  ...
  if (pool->is_replicated()) {
    ldout(cct, 20) << __func__
                   << " calculating simple distribution for replicated pool "
                   << get_pool_name(pid) << dendl;
    ...
    for (const auto& osd : osds) {
      float osd_primary_count = ((float)pgs_by_osd[osd].size() /
                                 (float)replica_count) *
    ...
    if (distribution_sum <= 0) {
      ldout(cct, 10) << __func__
                     << " Unable to calculate primary distribution, likely "
                        "because primary affinity is"
...
//
float
OSDMap::calc_desired_prims_for_osdsizeopt(
    int npgs,
    int forced_primaries,
    int forced_secondaries,
    int iops_per_osd,
    int write_ratio) const
{
  int forced_iops_per_pg = forced_primaries * 100 +
                           forced_secondaries * write_ratio;
  ...
  if (pgs_left <= 0)
    ...
    return float(forced_primaries) +
           float(iops_left - pgs_left * write_ratio) / float(100 - write_ratio);
}

int
OSDMap::calc_desired_primary_distribution_osdsize_opt(
    CephContext* cct,
    int64_t pid,
    const vector<uint64_t>& osds,
    map<uint64_t, float>& desired_primary_distribution) const
  ...
  if (!pool->is_replicated()) { // read balancing works only for replicated pools
    ...
    if (sum_pgs != pg_num * replica_count) {
      ldout(cct, 10) << __func__ << " Some of the PGs for pool '"
                     << get_pool_name(pid) << "' don't have" << replica_count
                     << " replicas "
                     << "- can't perform osd-size-optimized read balancing "
                     << dendl;
      return -EINVAL;
    }
    ldout(cct, 20) << __func__
                   << " calculating OSD size optimized primary distribution "
                      "for replicated pool "
                   << get_pool_name(pid) << dendl;
    ...
    int64_t read_ratio = 0;
    {
      ...
      uint64_t def_read_ratio =
          cct->_conf.get_val<uint64_t>("osd_pool_default_read_ratio");
      read_ratio = pool->opts.value_or<int64_t>(
          pool_opts_t::key_t::READ_RATIO, std::move(def_read_ratio));
    }
    int write_ratio = 100 - read_ratio;
    ldout(cct, 30) << __func__ << " Pool: " << pid
                   << " read ratio: " << read_ratio
                   << " write ratio: " << write_ratio << dendl;
    ...
    ldout(cct, 30) << __func__
                   << " read-balancer: All primaries OSDs for pool '"
                   << get_pool_name(pid) << "': " << osds_set << dendl;
    ldout(cct, 30) << __func__ << " read-balancer: PGs with primaries fixes "
                   << pgs_used << dendl;
    //
    ...
    ldout(cct, 30)
        << __func__
        << " read-balancer: OSDs with all primaries or no primaries '"
        << get_pool_name(pid) << "': " << osds_set << dendl;
    ...
    if (osds_left > 0) {
      ...
      ldout(cct, 30) << __func__ << " read-balancer: primaries_left ["
                     << primaries_left << "] osds_left [" << osds_left
                     << "] iops_left [" << iops_left << "] iops_per_osd ["
                     << iops_per_osd << "]" << dendl;
      ...
      for (int i = 0; i < num_osds; i++) {
        ...
        float nprims = calc_desired_prims_for_osdsizeopt(
            npgs, forced_primaries, forced_secondaries, iops_per_osd,
            write_ratio);
    ...
    std::random_device::result_type* p_seed)
{
  ...
  osd_weight_total = build_pool_pgs_info(
      cct, only_pools, tmp_osd_map, total_pgs, pgs_by_osd, osd_weight);
  ...
  float cur_max_deviation = calc_deviations(
      cct, pgs_by_osd, osd_weight, pgs_per_weight, osd_deviation, deviation_osd,
      stddev);
  ...
  while (max--) {
    ...
    fill_overfull_underfull(
        cct, deviation_osd, max_deviation, overfull, more_overfull, underfull,
        more_underfull);
  ...
  retry:
    ...
    for (auto p = deviation_osd.rbegin(); p != deviation_osd.rend(); ++p) {
      ...
      if (try_drop_remap_overfull(
              cct, pgs, tmp_osd_map, osd, temp_pgs_by_osd, to_unmap, to_upmap))
        goto test_change;
      ...
      for (auto pg : pgs) {
        ...
        tmp_osd_map.pg_to_raw_upmap(
            pg, &raw, &orig); // including existing upmaps too
        if (!try_pg_upmap(
                cct, pg, overfull, underfull, more_underfull, &orig, &out)) {
          continue;
        }
        ...
        if (pos != -1) {
          ...
          // the perfect status.
          add_remap_pair(
              cct, orig[pos], out[pos], pg, (size_t)pg_pool_size, osd, existing,
              temp_pgs_by_osd, new_upmap_items, to_upmap);
          goto test_change;
        }
      }
      if (fast_aggressive) {
    ...
    for (auto& [deviation, osd] : deviation_osd) {
      if (std::find(underfull.begin(), underfull.end(), osd) == underfull.end())
        break;
      float target = osd_weight[osd] * pgs_per_weight;
      ceph_assert(target > 0);
      ...
      if (try_drop_remap_underfull(
              cct, candidates, osd, temp_pgs_by_osd, to_unmap, to_upmap)) {
        goto test_change;
      }
    }
  ...
  test_change:
    ...
    float cur_max_deviation = calc_deviations(
        cct, temp_pgs_by_osd, osd_weight, pgs_per_weight, temp_osd_deviation,
    ...
    num_changed +=
        pack_upmap_results(cct, to_unmap, to_upmap, tmp_osd_map, pending_inc);
    ...
    map<uint64_t, set<pg_t>>* p_acting_primaries_by_osd) const
{
  ...
  const pg_pool_t* pool = get_pg_pool(pid);
  if (!pool) {
    ldout(cct, 20) << __func__ << " pool " << pid << " does not exist" << dendl;
    return pgs_by_osd;
  }
  ...
  for (unsigned ps = 0; ps < pool->get_pg_num(); ++ps) {
    ...
    if (cct != nullptr)
      ldout(cct, 20) << __func__ << " " << pg << " up " << up << " primary "
                     << primary << " acting_primary " << acting_prim << dendl;
    ...
    map<int, float>& osds_weight) const
{
  ...
  ldout(cct, 20) << __func__ << " pool " << pid << " ruleno " << ruleno
    ...
    map<int, float>& osds_weight)
{
  ...
  for (auto& [oid, oweight] : osds_weight) {
    ...
    ldout(cct, 20) << " osd." << oid << " weight " << oweight << " pgs " << pgs
                   << dendl;
  }
  return osds_weight_total;
    ...
    float& stddev) // return current max deviation
{
  ...
  for (auto& [oid, opgs] : pgs_by_osd) {
    ...
    ldout(cct, 20) << " osd." << oid << "\tpgs " << opgs.size() << "\ttarget "
                   << target << "\tdeviation " << deviation << dendl;
...
void
OSDMap::fill_overfull_underfull(
    CephContext* cct,
    ...
    std::vector<int>& more_underfull)
{
  ...
  for (auto i = deviation_osd.begin(); i != deviation_osd.end(); i++) {
    auto& odev = i->first;
    auto& oid = i->second;
    ldout(cct, 30) << " check " << odev << " >= " << -(int)max_deviation
                   << dendl;
    if (odev >= 0)
      break;
    ...
    OSDMap::Incremental* pending_inc)
{
  ...
  for (auto& [pg, um_items] : to_upmap) {
    ldout(cct, 10) << " upmap pg " << pg << " new pg_upmap_items " << um_items
                   << dendl;
...
bool
OSDMap::try_drop_remap_overfull(
    CephContext* cct,
    ...
    map<pg_t, mempool::osdmap::vector<pair<int32_t, int32_t>>>& to_upmap)
{
  ...
  for (auto pg : pgs) {
    ...
    for (auto um_pair : pg_upmap_items) {
      ...
      if (um_to == osd) {
        ldout(cct, 10) << " will try dropping existing" << " remapping pair "
                       << um_from << " -> " << um_to << " which remapped " << pg
    ...
    if (new_upmap_items.empty()) {
      ...
      ldout(cct, 10) << " existing pg_upmap_items " << pg_upmap_items
                     << " remapped " << pg << " into overfull osd." << osd
                     << ", new_pg_upmap_items now " << new_upmap_items << dendl;
...
bool
OSDMap::try_drop_remap_underfull(
    CephContext* cct,
    ...
    map<pg_t, mempool::osdmap::vector<std::pair<int32_t, int32_t>>>& to_upmap)
{
  ...
  for (auto& [pg, um_pairs] : candidates) {
    ...
    if (new_upmap_items.empty()) {
      // drop whole item
      ldout(cct, 10) << " existing pg_upmap_items " << um_pairs << " remapped "
                     << pg << " out from underfull osd." << osd
                     << ", will try cancelling it entirely" << dendl;
      to_unmap.insert(pg);
      return true;
    ...
    map<pg_t, mempool::osdmap::vector<pair<int32_t, int32_t>>>& to_upmap)
{
  ...
  //
  ldout(cct, 10) << " will try adding new remapping pair " << orig << " -> "
                 << out << " for " << pg
    ...
    std::random_device::result_type* p_seed)
{
  ...
  if (aggressive) {
    ...
    std::shuffle(
        candidates.begin(), candidates.end(), get_random_engine(cct, p_seed));
  }
  return candidates;
}
...
int64_t
OSDMap::has_zero_pa_pgs(CephContext* cct, int64_t pool_id) const
{
    ...
    if (pg_zero_pa) {
      if (cct != nullptr) {
        ldout(cct, 20) << __func__ << " " << pg
                       << " - maps only to OSDs with primiary affinity 0"
                       << dendl;
      }
      return (int64_t)ps;
    }
...
void
OSDMap::zero_rbi(read_balance_info_t& rbi) const
{
    ...
    float max_osd_score) const
{
  ...
  if (total_w_pa / total_osd_weight < 1. / float(pool->get_size())) {
    ...
    rbi.err_msg = fmt::format(
        "pool {} average primary affinity is lower than {:.2f}, read balance "
  ...
  rbi.pa_weighted_avg =
      rbi_round(rbi.pa_weighted / total_osd_weight); // in [0..1]
  ...
  if (acting_on_zero_pa) {
    ...
    rbi.err_msg = fmt::format(
        "pool {} has acting primaries on OSD(s) with primary affinity 0, read "
        "balance score is not accurate",
        pool_id);
  } else {
    rbi.acting_raw_score =
        rbi_round((float)max_acting_prims_per_osd / avg_prims_per_osd);
  }
  ...
  if (rbi.pa_avg != 0.) {
    ...
    if ((zpg = has_zero_pa_pgs(cct, pool_id)) >= 0) {
      ...
      ss << pg;
      ldout(cct, 10)
          << __func__ << " pool " << pool_id
          << " has some PGs where all OSDs are with primary_affinity 0 (" << pg
          << ",...)" << dendl;
      rbi.err_msg = fmt::format(
          "pool {} has some PGs where all OSDs are with primary_affinity 0 (at "
          "least pg {}), read balance score may not be reliable",
          pool_id, ss.str());
      return -EINVAL;
    }
    ...
    rbi.adjusted_score =
        rbi_round(rbi.raw_score / rbi.optimal_score); // >= 1 if PA is not low
    rbi.acting_adj_score = rbi_round(
        rbi.acting_raw_score / rbi.optimal_score); // >= 1 if PA is not low

  } else {
    // We should never get here - this condition is checked before calling this function - this is just sanity check code.
    rbi.err_msg = fmt::format(
        "pool {} all OSDs have zero primary affinity, can't calculate a "
        "reliable read balance score",
        pool_id);
    return -EINVAL;
  }
...
int
OSDMap::calc_rbs_fair(
    CephContext* cct,
    OSDMap& tmp_osd_map,
    int64_t pool_id,
    const pg_pool_t* pgpool,
    read_balance_info_t& rbi) const
{
  ...
  pgs_by_osd = tmp_osd_map.get_pgs_by_osd(
      cct, pool_id, &prim_pgs_by_osd, &acting_prims_by_osd);

  if (cct != nullptr)
    ldout(cct, 30) << __func__ << " Primaries for pool: " << prim_pgs_by_osd
                   << dendl;
  ...
  if (cct != nullptr) {
    ldout(cct, 20) << __func__ << " pool " << pool_id << " ruleno " << ruleno
                   << " weight-map " << osds_crush_weight << dendl;
  }
  uint osd_pa_count = 0;
  ...
  for (auto [osd, oweight] : osds_crush_weight) { // loop over all OSDs
    ...
    if (cct != nullptr) {
      ...
      auto nap = acting_prims_by_osd.count(osd)
                     ? acting_prims_by_osd.at(osd).size()
                     : 0;
      auto wt = osds_crush_weight.count(osd) ? osds_crush_weight.at(osd) : 0.;
      ldout(cct, 30) << __func__ << " OSD." << osd
                     << " info: " << " num_primaries " << np
                     << " num_acting_prims " << nap << " prim_affinity "
                     << tmp_osd_map.get_primary_affinityf(osd) << " weight "
                     << wt << dendl;
    }
  }
  if (cct != nullptr) {
    ldout(cct, 30) << __func__ << " pool " << pool_id << " total_osd_weight "
                   << total_osd_weight << " total_weighted_pa "
                   << total_weighted_pa << dendl;
  }

  if (prim_affinity_sum == 0.0) {
    if (cct != nullptr) {
      ldout(cct, 10) << __func__ << " pool " << pool_id
                     << " has primary_affinity set to zero on all OSDs"
                     << dendl;
    }
    zero_rbi(rbi);
    rbi.err_msg = fmt::format(
        "pool {} has primary_affinity set to zero on all OSDs", pool_id);
  ...
  int rc = tmp_osd_map.set_rbi_fair(
      cct, rbi, pool_id, total_weighted_pa, prim_affinity_sum, num_osds,
      osd_pa_count, total_osd_weight, max_prims_per_osd,
      max_acting_prims_per_osd, avg_prims_per_osd, prim_on_zero_pa,
      acting_on_zero_pa, max_osd_score);

  if (cct != nullptr) {
    ldout(cct, 30) << __func__ << " pool " << get_pool_name(pool_id)
                   << " pa_avg " << rbi.pa_avg << " pa_weighted "
                   << rbi.pa_weighted << " pa_weighted_avg "
                   << rbi.pa_weighted_avg << " optimal_score "
                   << rbi.optimal_score << " adjusted_score "
                   << rbi.adjusted_score << " acting_adj_score "
                   << rbi.acting_adj_score << dendl;
    ldout(cct, 20) << __func__ << " pool " << get_pool_name(pool_id)
                   << " raw_score: " << rbi.raw_score
                   << " acting_raw_score: " << rbi.acting_raw_score << dendl;
    ...
    read_balance_info_t& rbi) const
{
  ...
  if (num_pgs == 0) {
    ...
    if (cct != nullptr) {
      ldout(cct, 20)
          << __func__ << " pool " << pool_id
          << " has no PGs - can't calculate size-optimal read balancer score"
          << dendl;
    }
  }
  ...
  for (uint64_t i = 0; i < num_osds; i++) {
    if (get_primary_affinity(int(i)) != CEPH_OSD_DEFAULT_PRIMARY_AFFINITY) {
      if (cct != nullptr) {
        ldout(cct, 30)
            << __func__ << " pool " << pool_id
            << " has primary_affinity set to non-default value on some OSDs"
            << dendl;
      }
      if (rbi.err_msg.empty()) {
        rbi.err_msg = fmt::format(
            "Warning: pool {} has primary_affinity set to non-default value on "
            "some OSDs, "
            "this is ignored by the size-optimal read balancer",
            pool_id);
      }
    }
  ...
  if (num_pg_osd_legs != num_pgs * pgpool->get_size()) {
    if (cct != nullptr) {
      ldout(cct, 30) << __func__ << " pool " << pool_id << " has "
                     << num_pg_osd_legs << " PG OSD legs, expected "
                     << num_pgs * pgpool->get_size()
                     << " - can't calculate size-optimal read balancer score"
                     << dendl;
    }
    rbi.err_msg = fmt::format(
        "ERROR: pool {} has {} PG OSD legs, expected {} - Can't calculate "
        "size-optimal read balancer score",
  ...
  if (rr <= 0 || rr > 100) {
    if (cct != nullptr) {
      ldout(cct, 30) << __func__ << " pool " << pool_id
                     << " has invalid read_ratio " << rr
                     << " - can't calculate size-optimal read balancer score"
                     << dendl;
    }
    rbi.err_msg = fmt::format(
        "ERROR: pool {} has invalid read_ratio {} - Can't calculate "
        "size-optimal read balancer score",
        pool_id, rr);
    return -EINVAL;
  }
  ...
  if (num_osds == 0) {
    rbi.err_msg = fmt::format(
        "ERROR: pool {} has no active OSDs, can't calculate loads and read "
        "balance score",
        pool_id);
    return -EINVAL;
  }
  ...
  for (auto& [o, pgs] : pgs_by_osd) {
    ...
    int64_t nprims = prim_pgs_by_osd.contains(o) ? prim_pgs_by_osd.at(o).size()
                                                 : 0;
    int64_t nacting_prims = acting_prims_by_osd.contains(o)
                                ? acting_prims_by_osd.at(o).size()
                                : 0;
    int64_t load = nprims * 100 + (npgs - nprims) * (100 - rr);
    int64_t acting_load = nacting_prims * 100 +
                          (npgs - nacting_prims) * (100 - rr);
...
int
OSDMap::calc_read_balance_score(
    CephContext* cct,
    int64_t pool_id,
    read_balance_info_t* p_rbi) const
{
  ...
  if (tmp_osd_map.pools.count(pool_id) == 0) {
    if (cct != nullptr)
      ldout(cct, 30) << __func__ << " pool " << pool_id << " not found."
                     << dendl;
    zero_rbi(*p_rbi);
  ...
  if (!pool->is_replicated()) {
    zero_rbi(*p_rbi);
    p_rbi->err_msg = fmt::format(
        "pool {} is not a replicated pool, read balance score is meaningless",
        pool_id);
    return -EPERM;
  }
...
int
OSDMap::get_osds_by_bucket_name(const string& name, set<int>* osds) const
{
...
void
OSDMap::get_pool_ids_by_osd(CephContext* cct, int osd, set<int64_t>* pool_ids)
    const
{
...
protected:
  ...
  void
  dump_item(const CrushTreeDumper::Item& qi, F* f) override
  {
    ...
    if (get_bucket_utilization(
            qi.id, &kb, &kb_used, &kb_used_data, &kb_used_omap, &kb_used_meta,
            &kb_avail))
      if (kb_used && kb)
    ...
    dump_item(
        qi, reweight, kb, kb_used, kb_used_data, kb_used_omap, kb_used_meta,
  ...
  virtual void dump_item(
      const CrushTreeDumper::Item& qi,
      float& reweight,
      int64_t kb,
      int64_t kb_used,
  ...
  average_utilization()
  {
    ...
    for (int i = 0; i < osdmap->get_max_osd(); i++) {
      ...
      if (get_osd_utilization(
              i, &kb_i, &kb_used_i, &kb_used_data_i, &kb_used_omap_i,
      ...
      int64_t* kb_avail) const
  {
    if (id >= 0) {
      ...
      return get_osd_utilization(
          id, kb, kb_used, kb_used_data, kb_used_omap, kb_used_meta, kb_avail);
    }
    ...
    *kb_avail = 0;
    ...
    for (int k = osdmap->crush->get_bucket_size(id) - 1; k >= 0; k--) {
      ...
      if (!get_bucket_utilization(
              item, &kb_i, &kb_used_i, &kb_used_data_i, &kb_used_omap_i,
              &kb_used_meta_i, &kb_avail_i))
        return false;
      *kb += kb_i;
      *kb_used += kb_used_i;
...
public:
  ...
  dump(TextTable* tbl)
  {
    ...
    *tbl << "" << "" << "" << "TOTAL" << byte_u_t(sum.statfs.total)
...
ostream&
operator<<(ostream& out, const OSDUtilizationPlainDumper::lowprecision_t& v)
{
  if (v.v < -0.01) {
    return out << "-";
  } else if (v.v < 0.001) {
...
void
OSDMap::check_health(CephContext* cct, health_check_map_t* checks) const
{
    ...
    for (int i = 0; i < get_max_osd(); i++) {
      ...
      if (!is_up(i)) {
        ...
        for (int type = 0; type <= max_type; type++) {
          ...
          if (!subtree_type_is_down(
                  cct, parent_id, type, &down_in_osds, &up_in_osds, &subtree_up,
                  &subtree_type_down))
            break;
          current = parent_id;
        }
    ...
    if (num_down_in_osds > 0) {
      ...
      for (int type = max_type; type > 0; type--) {
        ...
        if (subtree_type_down[type].size() > 0) {
          ...
          string err = string("OSD_") + string(crush->get_type_name(type)) +
                       "_DOWN";
          boost::to_upper(err);
          auto& d = checks->add(
              err, HEALTH_WARN, ss.str(), subtree_type_down[type].size());
  ...
  // OSD_OUT_OF_ORDER_FULL
  {
    ...
    if (!detail.empty()) {
      auto& d = checks->add(
          "OSD_OUT_OF_ORDER_FULL", HEALTH_ERR, "full ratio(s) out of order", 0);
  ...
  // OSD_FAILSAFE_FULL
  {
    ...
    if (backfillfull.size()) {
      ...
      auto& d = checks->add(
          "OSD_BACKFILLFULL", HEALTH_WARN, ss.str(), backfillfull.size());
  ...
  // OSDMAP_FLAGS
  {
    ...
    if (test_flag(warn_flags)) {
      ...
      ss << s << " flag(s) set";
      checks->add(
          "OSDMAP_FLAGS", HEALTH_WARN, ss.str(),
  ...
  {
    list<string> detail;
    const unsigned flags = CEPH_OSD_NOUP | CEPH_OSD_NOIN | CEPH_OSD_NODOWN |
                           CEPH_OSD_NOOUT;
    for (int i = 0; i < max_osd; ++i) {
  ...
  if (cct->_conf->mon_warn_on_legacy_crush_tunables) {
    ...
    if (min < cct->_conf->mon_crush_min_required_version) {
      ...
      d.detail.push_back(
          "see "
          "http://docs.ceph.com/en/latest/rados/operations/crush-map/"
          "#tunables");
    }
  }
  ...
  if (cct->_conf->mon_warn_on_cache_pools_without_hit_sets) {
    ...
    if (!detail.empty()) {
      ...
      auto& d = checks->add(
          "CACHE_POOL_NO_HIT_SET", HEALTH_WARN, ss.str(), detail.size());
      d.detail.swap(detail);
    }
  }
  ...
  // POOL_NEARFULL/BACKFILLFULL/FULL
  {
    ...
    if (!backfillfull_detail.empty()) {
      ...
      auto& d = checks->add(
          "POOL_BACKFILLFULL", HEALTH_WARN, ss.str(),
    ...
    if (!nearfull_detail.empty()) {
      ...
      auto& d = checks->add(
          "POOL_NEARFULL", HEALTH_WARN, ss.str(), nearfull_detail.size());
  ...
  if (cct->_conf.get_val<bool>("mon_warn_on_pool_pg_num_not_power_of_two")) {
    ...
    if (!detail.empty()) {
      ...
      auto& d = checks->add(
          "POOL_PG_NUM_NOT_POWER_OF_TWO", HEALTH_WARN, ss.str(), detail.size());
      d.detail.swap(detail);
    }
  }
  ...
  if (cct->_conf.get_val<bool>("mon_warn_on_pool_no_redundancy")) {
    ...
    if (!detail.empty()) {
      ...
      auto& d = checks->add(
          "POOL_NO_REDUNDANCY", HEALTH_WARN, ss.str(), detail.size());
      d.detail.swap(detail);
    }
  }
  ...
  if (cct->_conf.get_val<bool>("mon_warn_on_degraded_stretch_mode")) {
    if (recovering_stretch_mode) {
      ...
      ss << "We are recovering stretch mode buckets, only requiring "
         << degraded_stretch_mode << " of " << stretch_bucket_count
         << " buckets to peer";
  ...
  if (stretch_mode_enabled) {
    ...
    if (subtrees.size() != 2) {
      ...
      checks->add(
          "INCORRECT_NUM_BUCKETS_STRETCH_MODE", HEALTH_WARN, ss.str(), 0);
      return;
    }
    ...
    double stretch_max_weight_delta =
        cct->_conf.get_val<double>("mon_stretch_max_bucket_weight_delta");
    stringstream ss;
    if (abs(weight1 - weight2) >
        (stretch_max_weight_delta * std::min(weight1, weight2))) {
      ss << "Stretch mode buckets differ in weight by more than "
         << (stretch_max_weight_delta * 100) << "%";
      checks->add(
          "STRETCH_MODE_BUCKET_WEIGHT_IMBALANCE", HEALTH_WARN, ss.str(), 0);
    }
  }
  ...
  // PUBLIC ADDRESS IS IN SUBNET MASK
  {
    ...
    if (!public_network.empty()) {
      ...
      if (unreachable.size()) {
        ostringstream ss;
        ss << unreachable.size() << " osds(s) "
           << (unreachable.size() == 1 ? "is" : "are") << " not reachable";
        auto& d = checks->add(
            "OSD_UNREACHABLE", HEALTH_ERR, ss.str(), unreachable.size());
        for (auto& i : unreachable) {
          ostringstream ss;
          ss << "osd." << i << "'s public address is not in '" << public_network
             << "' subnet";
          d.detail.push_back(ss.str());
        }
      }
...
int
OSDMap::parse_osd_id_list(const vector<string>& ls, set<int>* out, ostream* ss)
    const
{
  out->clear();
...
float
OSDMap::pool_raw_used_rate(int64_t poolid) const
{
  const pg_pool_t* pool = get_pg_pool(poolid);
...
unsigned
OSDMap::get_osd_crush_node_flags(int osd) const
{
  unsigned flags = 0;
...
unsigned
OSDMap::get_crush_node_flags(int id) const
{
  unsigned flags = 0;
...
unsigned
OSDMap::get_device_class_flags(int id) const
{
  unsigned flags = 0;
