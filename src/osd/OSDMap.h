...
struct osd_xinfo_t {
  ...
  osd_xinfo_t() :
    laggy_probability(0), laggy_interval(0), features(0), old_weight(0)
  {}
...
struct PGTempMap {
  ...
  decode(ceph::buffer::list::const_iterator& p)
  {
    ...
    for (auto i : offsets) {
      map.insert(
          map.end(), std::make_pair(i.first, (ceph_le32*)(start + i.second)));
    }
  }
  ...
  friend bool
  operator==(const PGTempMap& l, const PGTempMap& r)
  {
  ...
  public:
    ...
    const std::pair<pg_t, std::vector<int32_t>>&
    operator*() const
    {
      return current;
    }
  ...
  void
  set(pg_t pgid, const mempool::osdmap::vector<int32_t>& v)
  {
    using ceph::encode;
  ...
  class iterator {
    mempool::osdmap::map<pg_t, mempool::osdmap::vector<int32_t>>::const_iterator
        it;
  ...
  public:
    ...
    std::pair<pg_t, const mempool::osdmap::vector<int32_t>&>
    operator*() const
    {
      return *it;
    }

    const std::pair<const pg_t, mempool::osdmap::vector<int32_t>>*
    operator->() const
    {
      return &*it;
    }
  ...
  const mempool::osdmap::vector<int32_t>&
  get(pg_t pgid)
  {
...
public:
  ...
  public:
    ...
    mempool::osdmap::map<std::string, std::map<std::string, std::string>>
    ...
    mempool::osdmap::map<pg_t, mempool::osdmap::vector<int32_t>>
    ...
    mempool::osdmap::map<int32_t, std::pair<epoch_t, epoch_t>>
    ...
    pg_pool_t*
    get_new_pool(int64_t pool, const pg_pool_t* orig)
    {
    ...
    bool
    has_erasure_code_profile(const std::string& name) const
    {
    ...
    size_t
    get_pending_state_osds(std::vector<int>* osds)
    {
      ceph_assert(osds);
      osds->clear();
    ...
    bool
    pending_osd_has_state(int osd, unsigned state)
    {
      return new_state.count(osd) && (new_state[osd] & state) != 0;
    }

    bool
    pending_osd_state_set(int osd, unsigned state)
    {
    ...
    bool
    pending_osd_state_clear(int osd, unsigned state)
    {
      if (!pending_osd_has_state(osd, state)) {
    ...
    bool
    in_new_removed_snaps(int64_t pool, snapid_t snap) const
    {
    ...
    set_allow_crimson()
    {
      mutate_allow_crimson = mutate_allow_crimson_t::SET;
    }
  };
...
private:
  ...
  mempool::osdmap::map<int32_t, uint32_t>
      crush_node_flags; // crush node -> CEPH_OSD_* flags
  mempool::osdmap::map<int32_t, uint32_t>
      device_class_flags; // device class -> CEPH_OSD_* flags
  ...
  static constexpr uint64_t SIGNIFICANT_FEATURES =
      CEPH_FEATUREMASK_PGID64 | CEPH_FEATUREMASK_PGPOOL3 |
      CEPH_FEATUREMASK_OSDENC | CEPH_FEATUREMASK_OSDMAP_ENC |
      CEPH_FEATUREMASK_OSD_POOLRESEND | CEPH_FEATUREMASK_NEW_OSDOP_ENCODING |
      CEPH_FEATUREMASK_MSG_ADDR2 | CEPH_FEATUREMASK_CRUSH_TUNABLES5 |
      CEPH_FEATUREMASK_CRUSH_CHOOSE_ARGS | CEPH_FEATUREMASK_SERVER_LUMINOUS |
      CEPH_FEATUREMASK_SERVER_MIMIC | CEPH_FEATUREMASK_SERVER_NAUTILUS |
      CEPH_FEATUREMASK_SERVER_OCTOPUS | CEPH_FEATUREMASK_SERVER_REEF |
  ...
  mempool::osdmap::vector<__u32>
      osd_weight; // 16.16 fixed point, 0x10000 = "in", 0 = "out"
  ...
  std::shared_ptr<mempool::osdmap::map<pg_t, int32_t>>
      primary_temp; // temp primary mapping (e.g. while we rebuild)
  std::shared_ptr<mempool::osdmap::vector<__u32>>
      osd_primary_affinity; ///< 16.16 fixed point, 0x10000 = baseline
...
public:
  ...
  static uint64_t
  get_significant_features(uint64_t features)
  {
  ...
  have_significant_feature(uint64_t x)
  {
  ...
  void
  deepish_copy_from(const OSDMap& o)
  {
    *this = o;
    ...
    if (o.osd_primary_affinity)
      osd_primary_affinity.reset(
          new mempool::osdmap::vector<__u32>(*o.osd_primary_affinity));
  ...
  bool is_blocklisted(
      const entity_addrvec_t& a,
      CephContext* cct = nullptr) const;
  void get_blocklist(
      std::list<std::pair<entity_addr_t, utime_t>>* bl,
  ...
  float
  get_full_ratio() const
  {
    return full_ratio;
  }

  float
  get_backfillfull_ratio() const
  {
    return backfillfull_ratio;
  }

  float
  get_nearfull_ratio() const
  {
  ...
  void get_full_osd_counts(
      std::set<int>* full,
      std::set<int>* backfill,
      std::set<int>* nearfull) const;

  void get_out_of_subnet_osd_counts(
      CephContext* cct,
  ...
  int
  get_state(int o, std::set<std::string>& st) const
  {
    ceph_assert(o < max_osd);
  ...
  unsigned
  get_primary_affinity(int o) const
  {
    ceph_assert(o < max_osd);
    if (!osd_primary_affinity)
  ...
  float
  get_primary_affinityf(int o) const
  {
    return (float)get_primary_affinity(o) /
  ...
  int get_erasure_code_profile_default(
      CephContext* cct,
  ...
  get_erasure_code_profile(const std::string& name) const
  {
  ...
  const mempool::osdmap::map<std::string, std::map<std::string, std::string>>&
  get_erasure_code_profiles() const
  {
  ...
  bool
  get_allow_crimson() const
  {
    return allow_crimson;
  }

  bool
  exists(int osd) const
  {
    //ceph_assert(osd >= 0);
  ...
  bool
  has_been_up_since(int osd, epoch_t epoch) const
  {
    return is_up(osd) && get_up_from(osd) <= epoch;
  }

  bool
  is_down(int osd) const
  {
    return !is_up(osd);
  }

  bool
  is_stop(int osd) const
  {
    return exists(osd) && is_down(osd) && (osd_state[osd] & CEPH_OSD_STOP);
  }

  bool
  is_out(int osd) const
  {
  ...
  bool containing_subtree_is_down(
      CephContext* cct,
      int osd,
      int subtree_type,
      std::set<int>* down_cache) const;

  bool subtree_type_is_down(
      CephContext* cct,
      int id,
      int subtree_type,
      std::set<int>* down_in_osds,
      std::set<int>* up_in_osds,
      std::set<int>* subtree_up,
  ...
  bool
  have_addr(const entity_addr_t& addr) const
  {
  ...
  get_addrs(int osd) const
  {
    ceph_assert(exists(osd));
    return osd_addrs->client_addrs[osd] ? *osd_addrs->client_addrs[osd]
                                        : _blank_addrvec;
  }

  const entity_addrvec_t&
  get_most_recent_addrs(int osd) const
  {
    return get_addrs(osd);
  }

  const entity_addrvec_t&
  get_cluster_addrs(int osd) const
  {
    ceph_assert(exists(osd));
    return osd_addrs->cluster_addrs[osd] ? *osd_addrs->cluster_addrs[osd]
                                         : _blank_addrvec;
  }

  const entity_addrvec_t&
  get_hb_back_addrs(int osd) const
  {
    ceph_assert(exists(osd));
    return osd_addrs->hb_back_addrs[osd] ? *osd_addrs->hb_back_addrs[osd]
                                         : _blank_addrvec;
  }

  const entity_addrvec_t&
  get_hb_front_addrs(int osd) const
  {
    ceph_assert(exists(osd));
    return osd_addrs->hb_front_addrs[osd] ? *osd_addrs->hb_front_addrs[osd]
                                          : _blank_addrvec;
  }

  const uuid_d&
  get_uuid(int osd) const
  {
    ceph_assert(exists(osd));
    return (*osd_uuid)[osd];
  }
  ...
  int
  get_next_up_osd_after(int n) const
  {
    if (get_max_osd() == 0)
      return -1;
    for (int i = n + 1; i != n; ++i) {
  ...
  int
  get_previous_up_osd_before(int n) const
  {
    if (get_max_osd() == 0)
      return -1;
    for (int i = n - 1; i != n; --i) {
  ...
  void get_random_up_osds_by_subtree(
      int n, // whoami
      std::string& subtree,
      int limit, // how many
      std::set<int> skip,
  ...
  int
  get_num_pg_upmap_primaries() const
  {
  ...
  void clean_pg_upmaps(
      ...
      const std::map<pg_t, mempool::osdmap::vector<std::pair<int, int>>>&
          to_remap) const;
...
public:
  ...
  int map_to_pg(
      ...
      pg_t* pg) const;
  int object_locator_to_pg(
      const object_t& oid,
      const object_locator_t& loc,
      pg_t& pg) const;

  pg_t
  object_locator_to_pg(const object_t& oid, const object_locator_t& loc) const
  {
    pg_t pg;
  ...
  file_to_object_locator(const file_layout_t& layout)
  {
    return object_locator_t(layout.pool_id, layout.pool_ns);
  }

  ceph_object_layout
  file_to_object_layout(object_t oid, file_layout_t& layout) const
  {
    return make_object_layout(oid, layout.pool_id, layout.pool_ns);
  }

  ceph_object_layout make_object_layout(
      object_t oid,
      int pg_pool,
      std::string nspace) const;

  int
  get_pg_num(int pg_pool) const
  {
  ...
  int
  get_pg_pool_min_size(pg_t pgid) const
  {
    if (!pg_exists(pgid)) {
      return -ENOENT;
    }
  ...
  int
  get_pg_pool_size(pg_t pgid) const
  {
    if (!pg_exists(pgid)) {
      return -ENOENT;
    }
  ...
  int
  get_pg_pool_crush_rule(pg_t pgid) const
  {
    if (!pg_exists(pgid)) {
      return -ENOENT;
    }
...
private:
  ...
  void _remove_nonexistent_osds(
      const pg_pool_t& pool,
      std::vector<int>& osds) const;

  void _apply_primary_affinity(
      ps_t seed,
      const pg_pool_t& pool,
      std::vector<int>* osds,
      int* primary) const;
...
public:
  ...
  void
  pg_to_acting_osds(pg_t pg, std::vector<int>& acting) const
  {
  ...
  void
  pg_to_up_acting_osds(pg_t pg, std::vector<int>& up, std::vector<int>& acting)
      const
  {
  ...
  bool
  get_primary_shard(const pg_t& pgid, spg_t* out) const
  {
  ...
  bool
  get_primary_shard(const pg_t& pgid, int* primary, spg_t* out) const
  {
    ...
    if (poolit->second.is_erasure()) {
      for (uint8_t i = 0; i < acting.size(); ++i) {
        if (acting[i] == *primary) {
          *out = spg_t(
              pgid,
              pgtemp_undo_primaryfirst(poolit->second, pgid, shard_id_t(i)));
          return true;
        }
      }
    } else {
  ...
  bool
  has_pgtemp(const pg_t pg) const
  {
    return (pg_temp->find(pg) != pg_temp->end());
  }

  const std::vector<int> pgtemp_primaryfirst(
      const pg_pool_t& pool,
      const std::vector<int>& pg_temp) const;
  const std::vector<int> pgtemp_undo_primaryfirst(
      const pg_pool_t& pool,
      const pg_t pg,
      const std::vector<int>& acting) const;
  const shard_id_t pgtemp_primaryfirst(
      const pg_pool_t& pool,
      const pg_t pg,
      const shard_id_t shard) const;
  shard_id_t pgtemp_undo_primaryfirst(
      const pg_pool_t& pool,
      const pg_t pg,
      const shard_id_t shard) const;

  bool
  in_removed_snaps_queue(int64_t pool, snapid_t snap) const
  {
  ...
  int64_t
  lookup_pg_pool_name(std::string_view name) const
  {
  ...
  const mempool::osdmap::map<int64_t, pg_pool_t>&
  get_pools() const
  {
    return pools;
  }
  ...
  void
  get_pool_ids_by_rule(int rule_id, std::set<int64_t>* pool_ids) const
  {
  ...
  void get_pool_ids_by_osd(
      CephContext* cct,
      int osd,
      std::set<int64_t>* pool_ids) const;

  const std::string&
  get_pool_name(int64_t p) const
  {
  ...
  const mempool::osdmap::map<int64_t, std::string>&
  ...
  bool
  have_pg_pool(int64_t p) const
  {
    return pools.count(p);
  }

  const pg_pool_t*
  get_pg_pool(int64_t p) const
  {
    auto i = pools.find(p);
    if (i != pools.end())
      return &i->second;
  ...
  int
  get_pool_crush_rule(int64_t pool_id) const
  {
  ...
  int
  get_pg_acting_primary(pg_t pg) const
  {
    int primary = -1;
   ...
   */
  bool
  is_up_acting_osd_shard(spg_t pg, int osd) const
  {
  ...
  static int calc_pg_role_broken(
      int osd,
  ...
  int
  get_pg_acting_role(spg_t pg, int osd) const
  {
  ...
  bool try_pg_upmap(
      ...
      const std::vector<int>&
          underfull, ///< osds to move to, in order of preference
      const std::vector<int>&
          more_underfull, ///< less full osds to move to, in order of preference
      std::vector<int>* orig,
  ...
  int calc_desired_primary_distribution_simple(
      ...
      std::map<uint64_t, float>&
          desired_primary_distribution /* vector of osd ids */) const;
  ...
  float calc_desired_prims_for_osdsizeopt(
      int npgs,
...
private
  : // Bunch of internal functions used only by calc_pg_upmaps (result of code refactoring)
  ...
  bool try_drop_remap_overfull(
      ...
      std::map<pg_t, mempool::osdmap::vector<std::pair<int32_t, int32_t>>>&
          to_upmap);

  typedef std::vector<
      std::pair<pg_t, mempool::osdmap::vector<std::pair<int, int>>>>
      candidates_t;
...
public:
  ...
  bool
  have_pg_upmaps(pg_t pg) const
  {
    return pg_upmap.count(pg) || pg_upmap_items.count(pg);
  }

  bool
  check_full(const std::set<pg_shard_t>& missing_on) const
  {
...
public:
  int
  build_simple(CephContext* cct, epoch_t e, uuid_d& fsid, int num_osd)
  {
  ...
  static int build_simple_crush_map(
      CephContext* cct,
      CrushWrapper& crush,
      int num_osd,
      std::ostream* ss);
  static int build_simple_crush_map_from_conf(
      CephContext* cct,
      CrushWrapper& crush,
      std::ostream* ss);
...
public:
  ...
  bool
  check_new_blocklist_entries() const
  {
    return new_blocklist_entries;
  }

  void check_health(CephContext* cct, health_check_map_t* checks) const;

  int parse_osd_id_list(
...
inline std::ostream&
operator<<(std::ostream& out, const OSDMap& m)
{
