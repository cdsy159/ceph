...
public:
  ...
  dump(ostream& out)
  {
    ...
    for (std::map<const char*, per_state_info>::iterator p = info.begin();
      ...
      out << i.enter << "\t" << i.exit << "\t" << i.events << "\t"
          << i.event_time << "\t" << i.total_time << "\t" << i.min_time << "\t"
          << i.max_time << "\t" << p->first << "\n";
    }
  }

  void
  dump_formatted(ceph::Formatter* f)
  {
    std::lock_guard l(lock);
  ...
  void
  log_exit(const char* s, utime_t dur, uint64_t events, utime_t event_dur)
  {
    std::lock_guard l(lock);
...
public:
  ...
  bool
  is_scrub_active() const
  {
    return m_scrubber->is_scrub_active();
  }

  /// set when the scrub request is queued, and reset after scrubbing fully
  /// cleaned up.
  bool
  is_scrub_queued_or_active() const
  {
...
public:
  ...
  bool
  get_is_nonprimary_shard(const pg_shard_t& shard) const final
  {
  ...
  static void
  set_last_scrub_stamp(utime_t t, pg_history_t& history, pg_stat_t& stats)
  {
  ...
  void
  set_last_scrub_stamp(utime_t t)
  {
  ...
  static void
  set_last_deep_scrub_stamp(utime_t t, pg_history_t& history, pg_stat_t& stats)
  {
  ...
  void
  set_last_deep_scrub_stamp(utime_t t)
  {
    recovery_state.update_stats([t](auto& history, auto& stats) {
  ...
  static void
  add_objects_scrubbed_count(int64_t count, pg_stat_t& stats)
  {
    stats.objects_scrubbed += count;
  }

  void
  add_objects_scrubbed_count(int64_t count)
  {
  ...
  static void
  reset_objects_scrubbed(pg_stat_t& stats)
  {
  ...
  void start_split_stats(
      ...
      std::vector<object_stat_sum_t>* v);
  ...
  void
  scrub(epoch_t queued, ThreadPool::TPHandle& handle)
  {
    // a new scrub
    forward_scrub_event(
        &ScrubPgIF::initiate_regular_scrub, queued, "StartScrub");
  }
  ...
  void
  replica_scrub_resched(
      epoch_t queued,
      Scrub::act_token_t act_token,
      ThreadPool::TPHandle& handle)
  {
    forward_scrub_event(
        &ScrubPgIF::send_sched_replica, queued, act_token, "SchedReplica");
  }

  void
  scrub_send_scrub_resched(epoch_t queued, ThreadPool::TPHandle& handle)
  {
    forward_scrub_event(
        &ScrubPgIF::send_scrub_resched, queued, "InternalSchedScrub");
  }

  void
  scrub_send_pushes_update(epoch_t queued, ThreadPool::TPHandle& handle)
  {
    forward_scrub_event(
        &ScrubPgIF::active_pushes_notification, queued, "ActivePushesUpd");
  }

  void
  scrub_send_applied_update(epoch_t queued, ThreadPool::TPHandle& handle)
  {
    forward_scrub_event(
        &ScrubPgIF::update_applied_notification, queued, "UpdatesApplied");
  }

  void
  scrub_send_unblocking(epoch_t queued, ThreadPool::TPHandle& handle)
  {
    forward_scrub_event(&ScrubPgIF::send_scrub_unblock, queued, "Unblocked");
  }

  void
  scrub_send_digest_update(epoch_t queued, ThreadPool::TPHandle& handle)
  {
    forward_scrub_event(
        &ScrubPgIF::digest_update_notification, queued, "DigestUpdate");
  }

  void
  scrub_send_replmaps_ready(epoch_t queued, ThreadPool::TPHandle& handle)
  {
    forward_scrub_event(
        &ScrubPgIF::send_replica_maps_ready, queued, "GotReplicas");
  }

  void
  scrub_send_replica_pushes(epoch_t queued, ThreadPool::TPHandle& handle)
  {
    forward_scrub_event(
        &ScrubPgIF::send_replica_pushes_upd, queued, "ReplicaPushesUpd");
  }

  void
  scrub_send_get_next_chunk(epoch_t queued, ThreadPool::TPHandle& handle)
  {
    forward_scrub_event(&ScrubPgIF::send_get_next_chunk, queued, "NextChunk");
  }

  void
  scrub_send_scrub_is_finished(epoch_t queued, ThreadPool::TPHandle& handle)
  {
    forward_scrub_event(
        &ScrubPgIF::send_scrub_is_finished, queued, "ScrubFinished");
  }

  void
  scrub_send_chunk_free(epoch_t queued, ThreadPool::TPHandle& handle)
  {
    forward_scrub_event(
        &ScrubPgIF::send_chunk_free, queued, "SelectedChunkFree");
  }

  void
  scrub_send_chunk_busy(epoch_t queued, ThreadPool::TPHandle& handle)
  {
  ...
  void scrub_requested(
      ...
      scrub_type_t scrub_type) override;
  ...
  static void
  add_objects_trimmed_count(int64_t count, pg_stat_t& stats)
  {
    stats.objects_trimmed += count;
  }

  void
  add_objects_trimmed_count(int64_t count)
  {
    recovery_state.update_stats_wo_resched([count](auto& history, auto& stats) {
      add_objects_trimmed_count(count, stats);
    });
  }

  static void
  reset_objects_trimmed(pg_stat_t& stats)
  {
  ...
  {
    recovery_state.update_stats_wo_resched([](auto& history, auto& stats) {
  ...
  void set_ready_to_merge_target(eversion_t lu, epoch_t les, epoch_t lec)
      override;
...
private:
  ...
  void forward_scrub_event(
      ScrubAPI fn,
      epoch_t epoch_queued,
      std::string_view desc);
  // and for events that carry a meaningful 'activation token'
  using ScrubSafeAPI = void (
      ScrubPgIF::*)(epoch_t epoch_queued, Scrub::act_token_t act_token);
  void forward_scrub_event(
      ScrubSafeAPI fn,
      epoch_t epoch_queued,
      Scrub::act_token_t act_token,
...
public:
  ...
  struct C_DeleteMore : public Context {
    ...
    C_DeleteMore(PG* p, epoch_t e, int64_t num) :
      pg(p), epoch(e), num_objects(num)
    {}
...
private:
  friend void
  intrusive_ptr_add_ref(PG* pg)
  {
    pg->get("intptr");
  }

  friend void
  intrusive_ptr_release(PG* pg)
  {
    pg->put("intptr");
  }
...
public:
  ...
  void send_cluster_message(
      ...
      MessageRef m,
      epoch_t epoch,
      bool share_map_update) override;
...
public:
  ...
  void
  add_local_num_bytes(int64_t num_bytes)
  {
    if (num_bytes) {
  ...
  void
  sub_local_num_bytes(int64_t num_bytes)
  {
  ...
  void
  add_num_bytes(int64_t num_bytes)
  {
  ...
  void
  sub_num_bytes(int64_t num_bytes)
  {
    ceph_assert(ceph_mutex_is_locked_by_me(_lock));
    ceph_assert(num_bytes >= 0);
    if (num_bytes) {
  ...
  get_stats_num_bytes()
  {
    ...
    if (pool.info.is_erasure()) {
      ...
      num_bytes += get_pgbackend()->get_ec_stripe_chunk_size() *
                   info.stats.stats.sum.num_objects;
    }
    int64_t lnb = local_num_bytes.load();
    if (lnb && lnb != num_bytes) {
      lgeneric_dout(cct, 0)
          << this << " " << info.pgid << " num_bytes mismatch " << lnb
          << " vs stats " << info.stats.stats.sum.num_bytes << " / chunk "
  ...
  {
    ceph_assert(ceph_mutex_is_locked_by_me(_lock));
    auto num_bytes = static_cast<uint64_t>(std::max<int64_t>(
        0, // ensure bytes is non-negative
        info.stats.stats.sum.num_bytes));
    auto num_objects = static_cast<uint64_t>(std::max<int64_t>(
...
protected:
  ...
  std::map<hobject_t, std::list<OpRequestRef>> waiting_for_unreadable_object,
      waiting_for_degraded_object, waiting_for_blocked_object;
  ...
  std::map<
      eversion_t,
      std::list<
          std::tuple<OpRequestRef, version_t, int, std::vector<pg_log_op_return_item_t>>>>
      waiting_for_ondisk;
  ...
  bool
  needs_recovery() const
  {
    return recovery_state.needs_recovery();
  }

  bool
  needs_backfill() const
  {
  ...
  struct PGLogEntryHandler : public PGLog::LogEntryHandler {
    ...
    // LogEntryHandler
    void
    remove(const hobject_t& hoid) override
    {
  ...
  void
  release_backoffs(const hobject_t& o)
  {
  ...
  void
  add_pg_backoff(const ceph::ref_t<Session>& s)
  {
...
public:
  ...
  is_degraded() const
  {
    return recovery_state.is_degraded();
  }
...
protected:
  ...
  {
    return recovery_state.state_test(m);
  }

  void
  state_set(uint64_t m)
  {
    recovery_state.state_set(m);
  }

  void
  state_clear(uint64_t m)
  {
    recovery_state.state_clear(m);
  }
  ...
  is_activating() const
  {
    return recovery_state.is_activating();
  }

  bool
  is_peering() const
  {
    return recovery_state.is_peering();
  }

  bool
  is_down() const
  {
    return recovery_state.is_down();
  }

  bool
  is_recovery_unfound() const
  {
    return recovery_state.is_recovery_unfound();
  }

  bool
  is_backfill_unfound() const
  {
    return recovery_state.is_backfill_unfound();
  }

  bool
  is_incomplete() const
  {
    return recovery_state.is_incomplete();
  }
  ...
  is_undersized() const
  {
    return recovery_state.is_undersized();
  }
  ...
  is_remapped() const
  {
    return recovery_state.is_remapped();
  }

  bool
  is_peered() const
  {
    return recovery_state.is_peered();
  }

  bool
  is_recovering() const
  {
    return recovery_state.is_recovering();
  }

  bool
  is_premerge() const
  {
    return recovery_state.is_premerge();
  }

  bool
  is_repair() const
  {
    return recovery_state.is_repair();
  }

  bool
  is_laggy() const
  {
    return state_test(PG_STATE_LAGGY);
  }

  bool
  is_wait() const
  {
    return state_test(PG_STATE_WAIT);
  }

  bool
  is_empty() const
  {
    return recovery_state.is_empty();
  }
...
public:
  ...
  void
  write_if_dirty(PeeringCtx& rctx)
  {
...
protected:
  void
  write_if_dirty(ObjectStore::Transaction& t)
  {
  ...
  void
  update_history(const pg_history_t& history)
  {
  ...
  bool
  old_peering_evt(PGPeeringEventRef evt)
  {
    return old_peering_msg(evt->get_epoch_sent(), evt->get_epoch_requested());
  }

  bool
  have_same_or_newer_map(epoch_t e)
  {
...
public:
  ...
  OSDService*
  get_pg_osd(ScrubberPasskey) const
  {
    return osd;
  }
  ...
  {
    return get_pgbackend()->be_get_ondisk_size(
        logical_size, shard_id_t(shard_id), object_is_legacy_ec);
  }
  ...
  ec_encode_acting_set(const bufferlist& in_bl) const final
  {
...
public:
  ...
  PGLockWrapper(PGLockWrapper&& rhs) noexcept :
    m_pg(std::move(rhs.m_pg))
  {
    rhs.m_pg = nullptr;
  }
