...
static ostream&
_prefix(std::ostream* _dout, PGBackend* pgb)
{
  return pgb->get_parent()->gen_dbg_prefix(*_dout);
}

void
PGBackend::recover_delete_object(
    const hobject_t& oid,
    eversion_t v,
    RecoveryHandle* h)
{
...
void
PGBackend::send_recovery_deletes(
    int prio,
  ...
  for (const auto& p : deletes) {
    ...
    while (it != objects.end()) {
      ...
      MOSDPGRecoveryDelete* msg = new MOSDPGRecoveryDelete(
          get_parent()->whoami_shard(), target_pg, get_osdmap_epoch(),
          min_epoch);
...
bool
PGBackend::handle_message(OpRequestRef op)
{
...
void
PGBackend::handle_recovery_delete(OpRequestRef op)
{
...
void
PGBackend::handle_recovery_delete_reply(OpRequestRef op)
{
...
void
PGBackend::rollback(const pg_log_entry_t& entry, ObjectStore::Transaction* t)
{
  ...
  struct RollbackVisitor : public ObjectModDesc::Visitor {
    ...
    append(uint64_t old_size) override
    {
      ...
      const uint64_t shard_size = pg->object_size_to_shard_size(
          old_size, pg->get_parent()->whoami_shard().shard);
    ...
    void
    setattrs(map<string, std::optional<bufferlist>>& attrs) override
    {
      auto dpp = pg->get_parent()->get_dpp();
      ...
      for (unsigned int i = 0; i < extents.size(); i++) {
        if (shards.empty() || shards[i].empty() ||
          ...
          ldpp_dout(dpp, 20)
              ...
              << object_size << " " << shard_size << dendl;
          pg->rollback_extents(
              gen, extents[i].first, extents[i].second, hoid, shard_size, &temp);
          donework = true;
        } else {
          // Unwritten shard - nothing to rollback
          ldpp_dout(dpp, 20) << " entry " << entry.version
...
void
PGBackend::rollforward(const pg_log_entry_t& entry, ObjectStore::Transaction* t)
{
...
void
PGBackend::trim(const pg_log_entry_t& entry, ObjectStore::Transaction* t)
{
  if (!entry.can_rollback())
    return;
  Trimmer trimmer(entry.soid, this, t);
    ...
    const pg_log_entry_t& entry)
{
  ...
  ldpp_dout(dpp, 20) << __func__ << " version=" << entry.version
                     << " written_shards=" << entry.written_shards << " pwlc=e"
    ...
    if (!entry.is_written_shard(shard)) {
      ...
      if (old_v == new_v) {
        if (old_v.version == eversion_t::max().version) {
          ...
          // invalid
          ldpp_dout(dpp, 20) << __func__ << " pwlc invalid " << shard << dendl;
          ...
          ldpp_dout(dpp, 20)
              << __func__ << " pwlc is ahead of entry " << shard << dendl;
        } else {
...
void
PGBackend::remove(const hobject_t& hoid, ObjectStore::Transaction* t)
{
...
void
PGBackend::on_change_cleanup(ObjectStore::Transaction* t)
{
    ...
    hobject_t* next)
{
  ...
  while (!_next.is_max() && ls->size() < (unsigned)min) {
    ...
    if (HAVE_FEATURE(
            parent->min_upacting_features(), OSD_FIXED_COLLECTION_LIST)) {
      r = store->collection_list(
          ch, _next, ghobject_t::get_max(), max - ls->size(), &objects, &_next);
    } else {
    ...
    vector<ghobject_t>* gen_obs)
{
  ...
  if (HAVE_FEATURE(parent->min_upacting_features(), OSD_FIXED_COLLECTION_LIST)) {
    r = store->collection_list(
        ch,
        ghobject_t(
            start, ghobject_t::NO_GEN, get_parent()->whoami_shard().shard),
    ...
    ObjectStore::Transaction* t)
{
  ...
  if (offset >= shard_size) {
    ...
    dout(20) << __func__ << " " << hoid << " " << offset << "~" << length
             << " is out of range " << shard_size << dendl;
  } else {
    ...
    dout(20) << __func__ << " " << hoid << " " << offset << "~" << length
             << dendl;
    t->clone_range(
        coll, ghobject_t(hoid, gen, shard),
        ghobject_t(hoid, ghobject_t::NO_GEN, shard), offset, length, offset);
  }
}
    ...
    ECExtentCache::LRU& ec_extent_cache_lru)
{
  ...
  case pg_pool_t::TYPE_ERASURE: {
    ...
    ceph_assert(ec_impl);
    return new ECSwitch(
        l, coll, ch, store, cct, ec_impl, pool.stripe_width,
        ec_extent_cache_lru);
  }
  default:
    ceph_abort();
