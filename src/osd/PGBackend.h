...
public:
  virtual int
  object_stat(const hobject_t& hoid, struct stat* st)
  {
    return -1;
  };
...
public:
  ...
  public:
    ...
    virtual const std::set<pg_shard_t>& get_acting_recovery_backfill_shards()
        const = 0;
    ...
    virtual const std::map<hobject_t, std::set<pg_shard_t>>&
    get_missing_loc_shards() const = 0;
    ...
    virtual const std::map<pg_shard_t, pg_missing_t>& get_shard_missing()
        const = 0;
    virtual const pg_missing_const_i& get_shard_missing(
        pg_shard_t peer) const = 0;

    virtual const std::map<pg_shard_t, pg_info_t>& get_shard_info() const = 0;

    virtual const pg_info_t&
    get_shard_info(pg_shard_t peer) const
    {
    ...
    virtual void update_pct(eversion_t pct) = 0;
    ...
    virtual hobject_t get_temp_recovery_object(
  ...
  PGBackend(
      ...
      const coll_t& coll,
      ObjectStore::CollectionHandle& ch) :
    cct(cct), store(store), coll(coll), ch(ch), parent(l)
  {}
  ...
  void recover_delete_object(
      const hobject_t& oid,
      eversion_t v,
      RecoveryHandle* h);
  void send_recovery_deletes(
      int prio,
      const std::map<pg_shard_t, std::vector<std::pair<hobject_t, eversion_t>>>&
          deletes);
  ...
  get_ec_data_chunk_count() const
  {
    return 0;
  };

  virtual int
  get_ec_stripe_chunk_size() const
  {
    return 0;
  };
  ...
  virtual bool
  get_is_nonprimary_shard(shard_id_t shard) const
  {
    return false; // Only EC has nonprimary shards.
  };

  virtual bool
  get_is_hinfo_required() const
  {
    return false; // Only EC can have hinfo.
  }

  virtual bool
  get_is_ec_optimized() const
  {
...
public:
  ...
  void
  add_temp_obj(const hobject_t& oid)
  {
    temp_contents.insert(oid);
  }

  void
  add_temp_objs(const std::set<hobject_t>& oids)
  {
  ...
  void
  clear_temp_objs(const std::set<hobject_t>& oids)
  {
    for (std::set<hobject_t>::const_iterator i = oids.begin(); i != oids.end();
         ++i) {
...
protected:
  ...
  void
  rollback_create(const hobject_t& hoid, ObjectStore::Transaction* t)
  {
    remove(hoid, t);
  }
...
public:
  ...
  extent_to_shard_extent(uint64_t off, uint64_t len)
  {
