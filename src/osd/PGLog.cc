...
static ostream&
_prefix(std::ostream* _dout, const PGLog* pglog)
{
    ...
    eversion_t* write_from_dups)
{
  ...
  auto earliest_dup_version = log.rbegin()->version.version <
  ...
  for (size_t max_dups_to_trim = cct->_conf->osd_pg_log_trim_max;
       max_dups_to_trim > 0 && dups.size() > max_dups; max_dups_to_trim--) {
  ...
  if (tail < s)
    tail = s;
  lgeneric_subdout(cct, osd, 20)
      << "IndexedLog::trim after trim" << " dups.size()=" << dups.size()
      << " tail=" << tail << " s=" << s << dendl;
}

ostream&
PGLog::IndexedLog::print(ostream& out) const
{
  out << *this << std::endl;
...
void
PGLog::clear_info_log(spg_t pgid, ObjectStore::Transaction* t)
{
  coll_t coll(pgid);
    ...
    bool ec_optimizations_enabled) const
{
  ...
  if (olog.head < log.tail) {
    dout(10) << __func__ << ": osd." << from
             << " does not overlap, not looking " << "for divergent objects"
             << dendl;
    return;
  }
  ...
  */
  ...
  _merge_divergent_entries(
      folog, divergent, oinfo, olog.get_can_rollback_to(), omissing, nullptr,
      ec_optimizations_enabled, to.shard, this);
    ...
    const pg_shard_t& shard)
{
  ...
  if (!divergent.empty()) {
    ...
    dout(10) << "rewind_divergent_log crt = " << log.get_can_rollback_to()
             << dendl;
    dirty_log = true;
  }
  ...
  _merge_divergent_entries(
      log, divergent, info, original_crt, missing, rollbacker,
      ec_optimizations_enabled, shard.shard, this);
    ...
    bool ec_optimizations_enabled)
{
  ...
  if (olog.head < log.head) {
    rewind_divergent_log(
        olog.head, info, rollbacker, dirty_info, dirty_big_info,
        ec_optimizations_enabled, toosd);
    changed = true;
  }
  ...
  if (olog.head > log.head) {
    ...
    append_log_entries_update_missing(
        info.last_backfill, new_entries, false, &log, missing, rollbacker, pool,
        toosd.shard, this);

    _merge_divergent_entries(
        log, divergent, info, original_crt, missing, rollbacker,
        ec_optimizations_enabled, toosd.shard, this);
...
bool
PGLog::merge_log_dups(const pg_log_t& olog)
{
  dout(5) << __func__ << " log.dups.size()=" << log.dups.size()
...
{
  if (needs_write()) {
    dout(6) << "write_log_and_missing with: " << "dirty_to: " << dirty_to
    ...
    _write_log_and_missing(
        ...
        (pg_log_debug ? &log_keys_debug : nullptr), this);
    undirty();
  } else {
...
{
  _write_log_and_missing(
      t, km, log, coll, log_oid, eversion_t::max(), eversion_t(), eversion_t(),
      set<eversion_t>(), set<string>(), missing, true, require_rollback, false,
...
PGLog::_write_log_and_missing_wo_missing(
    ...
    const DoutPrefixProvider* dpp)
{
  ldpp_dout(dpp, 10) << "_write_log_and_missing_wo_missing, clearing up to "
                     << dirty_to << " dirty_to_dups=" << dirty_to_dups
  ...
  if (dirty_to != eversion_t()) {
    t.omap_rmkeyrange(
        coll, log_oid, eversion_t().get_key_name(), dirty_to.get_key_name());
  ...
  if (dirty_to != eversion_t::max() && dirty_from != eversion_t::max()) {
    ...
    t.omap_rmkeyrange(
        coll, log_oid, dirty_from.get_key_name(),
        eversion_t::max().get_key_name());
    clear_after(log_keys_debug, dirty_from.get_key_name());
  }

  for (auto p = log.log.begin(); p != log.log.end() && p->version <= dirty_to;
       ++p) {
  ...
  if (dirty_to_dups != eversion_t()) {
    ...
    ldpp_dout(dpp, 10) << __func__ << " remove dups min=" << min.get_key_name()
                       << " to dirty_to_dup=" << dirty_to_dup.get_key_name()
                       << dendl;
    t.omap_rmkeyrange(
        coll, log_oid, min.get_key_name(), dirty_to_dup.get_key_name());
  }
  if (dirty_to_dups != eversion_t::max() &&
    ...
    t.omap_rmkeyrange(
        coll, log_oid, dirty_from_dup.get_key_name(), max.get_key_name());
  }

  ldpp_dout(dpp, 10) << __func__
                     << " going to encode log.dups.size()=" << log.dups.size()
                     << dendl;
  ...
  ldpp_dout(dpp, 10) << __func__
                     << " 1st round encoded log.dups.size()=" << log.dups.size()
                     << dendl;
  ...
  ldpp_dout(dpp, 10) << __func__
                     << " 2st round encoded log.dups.size()=" << log.dups.size()
                     << dendl;
  ...
  if (require_rollback) {
    ...
    encode(
        log.get_rollback_info_trimmed_to(), (*km)["rollback_info_trimmed_to"]);
  }
    ...
    const DoutPrefixProvider* dpp)
{
  ...
  if (dirty_to != eversion_t::max() && dirty_from != eversion_t::max()) {
    ldpp_dout(dpp, 10) << "write_log_and_missing, clearing from " << dirty_from
                       << dendl;
    t.omap_rmkeyrange(
        coll, log_oid, dirty_from.get_key_name(),
        eversion_t::max().get_key_name());
...
struct FuturizedShardStoreLogReader {
  ...
  void
  process_entry(const auto& key, const auto& value)
  {
    if (key[0] == '_')
      return;
  ...
  seastar::future<>
  read(crimson::os::CollectionRef ch, ghobject_t pgmeta_oid)
  {
    ...
    std::function<ObjectStore::omap_iter_ret_t()> on_conflict = [&kvs]() {
      kvs.clear();
      return ObjectStore::omap_iter_ret_t::NEXT;
    };
    ...
    co_await crimson::os::with_store<
        ...
        store, ch, pgmeta_oid, start_from, callback, 0, on_conflict)
        .safe_then([](auto ret) {
...
PGLog::read_log_and_missing_crimson(
    ...
    const DoutPrefixProvider* dpp)
{
  ldpp_dout(dpp, 20) << "read_log_and_missing coll " << ch->get_cid() << " "
                     << pgmeta_oid << dendl;
  return seastar::do_with(
      FuturizedShardStoreLogReader{
          store, info, log, log_keys_debug, missing, dpp},
