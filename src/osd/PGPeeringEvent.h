...
struct MInfoRec : boost::statechart::event<MInfoRec> {
  ...
  MInfoRec(
    ...
    from(from), info(info), msg_epoch(msg_epoch), lease(l), lease_ack(la)
  {}
...
struct MQuery : boost::statechart::event<MQuery> {
  ...
  print(std::ostream* out) const
  {
    *out << "MQuery " << pgid << " from " << from << " query_epoch "
         << query_epoch << " query: " << query;
  }
};
...
struct RequestBackfillPrio : boost::statechart::event<RequestBackfillPrio> {
  ...
  print(std::ostream* out) const
  {
    *out << "RequestBackfillPrio: priority " << priority << " primary bytes "
         << primary_num_bytes << " local bytes " << local_num_bytes;
  }
};
...
struct RequestRecoveryPrio : boost::statechart::event<RequestRecoveryPrio> {
  ...
  explicit RequestRecoveryPrio(unsigned prio) :
    boost::statechart::event<RequestRecoveryPrio>(), priority(prio)
  {}
...
TrivialEvent(NullEvt) TrivialEvent(PgCreateEvt)
                    ...
                    TrivialEvent(RemoteReservationCanceled)
                        TrivialEvent(RemoteRecoveryReserved)
                            TrivialEvent(RecoveryDone)
                                ...
                                struct DeferRecovery
  : boost::statechart::event<DeferRecovery> {
  float delay;
