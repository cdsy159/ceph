...
public:
  ...
  public:
    ...
    using InitType =
        std::variant<Init::None, Init::Create, Init::Clone, Init::Rename>;
    ...
    bool
    has_source(hobject_t* source = nullptr) const
    {
      return match(
          init_type,
    ...
    std::optional<std::pair<std::set<snapid_t>, std::set<snapid_t>>>
        updated_snaps;
    ...
    using BufferUpdateType = std::
        variant<BufferUpdate::Write, BufferUpdate::Zero, BufferUpdate::CloneRange>;

  private:
    struct SplitMerger {
      BufferUpdateType
      split(uint64_t offset, uint64_t len, const BufferUpdateType& bu) const
      {
        return match(
            bu,
      ...
      uint64_t
      length(const BufferUpdateType& left) const
      {
        return match(
            left,
      ...
      bool
      can_merge(const BufferUpdateType& left, const BufferUpdateType& right) const
      {
        return match(
            left,
      ...
      BufferUpdateType
      merge(BufferUpdateType&& left, BufferUpdateType&& right) const
      {
        return match(
            left,
            [&](const BufferUpdate::Write& w) -> BufferUpdateType {
  ...
  public:
    using buffer_update_type =
        interval_map<uint64_t, BufferUpdateType, SplitMerger>;
...
private:
  ObjectOperation&
  get_object_op_for_modify(const hobject_t& hoid)
  {
    auto& op = op_map[hoid];
  ...
  ObjectOperation&
  get_object_op(const hobject_t& hoid)
  {
    return op_map[hoid];
  }
...
public:
  ...
  void
  remove(const hobject_t& hoid ///< [in] obj to remove
  )
  {
  ...
  )
  {
    ...
    op.updated_snaps = make_pair(std::move(old_snaps), std::move(new_snaps));
  }

  /// Clears, truncates
  void
  omap_clear(const hobject_t& hoid ///< [in] object to clear omap
  )
  {
    auto& op = get_object_op_for_modify(hoid);
    op.clear_omap = true;
  ...
  )
  {
    ...
    op.buffer_updates.insert(
        off, len, ObjectOperation::BufferUpdate::Write{bl, fadvise_flags});
  }
  ...
  )
  {
    ...
    op.omap_updates.emplace_back(
        std::make_pair(ObjectOperation::OmapUpdateType::Insert, keys_bl));
  }
  ...
  omap_setkeys(
      ...
      std::map<std::string, ceph::buffer::list>&
          keys ///< [in] omap keys, may be cleared
  )
  {
    using ceph::encode;
    ceph::buffer::list bl;
  ...
  )
  {
    auto& op = get_object_op_for_modify(hoid);
    op.omap_updates.emplace_back(
        std::make_pair(ObjectOperation::OmapUpdateType::Remove, keys_bl));
  }
  ...
  )
  {
    auto& op = get_object_op_for_modify(hoid);
    op.omap_updates.emplace_back(
        std::make_pair(ObjectOperation::OmapUpdateType::RemoveRange, range_bl));
  }
  ...
  void
  nop(const hobject_t& hoid ///< [in] obj to which we are doing nothing
  )
  {
