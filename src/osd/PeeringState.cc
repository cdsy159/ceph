...
static int
get_max_prio_for_base(int base)
{
...
void
BufferedRecoveryMessages::send_notify(int to, const pg_notify_t& n)
{
...
void
BufferedRecoveryMessages::send_query(int to, spg_t to_spgid, const pg_query_t& q)
{
  send_osd_message(to, TOPNSPC::make_message<MOSDPGQuery2>(to_spgid, q));
}

void
BufferedRecoveryMessages::send_info(
    int to,
    spg_t to_spgid,
    epoch_t min_epoch,
    epoch_t cur_epoch,
...
{
  send_osd_message(
      to, TOPNSPC::make_message<MOSDPGInfo2>(
              to_spgid, info, cur_epoch, min_epoch, lease, lease_ack));
}

void
PGPool::update(OSDMapRef map)
{
...
#undef dout_prefix
#define dout_prefix \
  (dpp->gen_prefix(*_dout)) << "PeeringState::" << __func__ << " "
#undef psdout
...
void
PeeringState::start_handle(PeeringCtx* new_ctx)
{
  ceph_assert(!rctx);
...
void
PeeringState::begin_block_outgoing()
{
...
void
PeeringState::clear_blocked_outgoing()
{
  ceph_assert(orig_ctx);
  ceph_assert(rctx);
...
void
PeeringState::end_block_outgoing()
{
  ceph_assert(messages_pending_flush);
  ceph_assert(orig_ctx);
  ceph_assert(rctx);
...
void
PeeringState::check_recovery_sources(const OSDMapRef& osdmap)
{
  /*
...
void
PeeringState::update_history(const pg_history_t& new_history)
{
...
PeeringState::purge_strays()
{
  ...
  for (auto p = stray_set.begin(); p != stray_set.end(); ++p) {
    ...
    if (get_osdmap()->is_up(p->osd)) {
      ...
      auto m =
          TOPNSPC::make_message<MOSDPGRemove>(get_osdmap_epoch(), to_remove);
...
void
PeeringState::query_unfound(Formatter* f, string state)
{
  psdout(20) << "Enter PeeringState common QueryUnfound" << dendl;
  {
    ...
    for (auto p = might_have_unfound.begin(); p != might_have_unfound.end();
         ++p) {
...
void
PeeringState::apply_pwlc(
    const std::pair<eversion_t, eversion_t> pwlc,
  ...
  if (toversion > info.last_update) {
    if (fromversion <= info.last_update) {
      if (info.last_complete == info.last_update) {
        psdout(10) << "osd." << shard << " has last_complete" << "=last_update "
                   << info.last_update << " pwlc can advance both to "
                   << toversion << dendl;
        info.last_complete = toversion;
      } else {
        psdout(10) << "osd." << shard << " has last_complete "
                   << info.last_complete << " and last_update "
                   << info.last_update << " pwlc can advance last_update to "
                   << toversion << dendl;
      }
      ...
      psdout(10) << "osd." << shard << " has last_complete "
                 << info.last_complete << " and last_update "
                 << info.last_update << " cannot apply pwlc from "
...
void
PeeringState::update_peer_info(const pg_shard_t& from, const pg_info_t& oinfo)
{
    ...
    for (const auto& [shard, versionrange] :
      ...
      if (info.partial_writes_last_complete.contains(shard)) {
        ...
        bool same_epoch =
            (oinfo.partial_writes_last_complete_epoch ==
             info.partial_writes_last_complete_epoch);
        if (newer_epoch || (same_epoch && (otoversion > toversion)) ||
            (same_epoch && (otoversion == toversion) &&
             (ofromversion < fromversion))) {
          if (!updated) {
            updated = true;
            psdout(10) << "osd." << from
  ...
  if (info.partial_writes_last_complete.contains(pg_whoami.shard)) {
    apply_pwlc(
        info.partial_writes_last_complete[pg_whoami.shard], pg_whoami, info,
        &pg_log);
  }
}
...
void
PeeringState::remove_down_peer_info(const OSDMapRef& osdmap)
{
  ...
  while (it != peer_purged.end()) {
    if (!osdmap->is_up(it->osd)) {
      psdout(10) << " dropping down osd." << *it << " from peer_purged"
                 << dendl;
...
void
PeeringState::update_heartbeat_peers()
{
  if (!is_primary())
    return;
...
void
PeeringState::write_if_dirty(ObjectStore::Transaction& t)
{
  pl->prepare_write(
      info, last_written_info, past_intervals, pg_log, dirty_info,
      dirty_big_info, last_persisted_osdmap < get_osdmap_epoch(), t);
...
void
PeeringState::activate_map(PeeringCtx& rctx)
{
  psdout(10) << dendl;
  ActMap evt;
  handle_event(evt, &rctx);
  if (osdmap_ref->get_epoch() - last_persisted_osdmap >
    ...
    psdout(20) << ": Dirtying info: last_persisted is " << last_persisted_osdmap
...
void
PeeringState::set_last_peering_reset()
{
...
void
PeeringState::check_full_transition(OSDMapRef lastmap, OSDMapRef osdmap)
{
    ...
    OSDMapRef osdmap)
{
  if (PastIntervals::is_new_interval(
          primary.osd, newactingprimary, acting, newacting, up_primary.osd,
          newupprimary, up, newup, osdmap.get(), lastmap.get(),
          info.pgid.pgid)) {
  ...
  if (!lastmap->is_up(pg_whoami.osd) && osdmap->is_up(pg_whoami.osd)) {
    psdout(10) << "osd transitioned from down -> up" << dendl;
    return true;
  }
  return false;
}
    ...
    ObjectStore::Transaction& t)
{
  ...
  if (!lastmap) {
    ...
    psdout(10) << ": check_new_interval output: " << debug.str() << dendl;
    if (new_interval) {
      ...
      if (osdmap->have_pg_pool(info.pgid.pgid.pool()) &&
          info.pgid.pgid.is_split(
              lastmap->get_pg_num(info.pgid.pgid.pool()),
              osdmap->get_pg_num(info.pgid.pgid.pool()), nullptr)) {
  ...
  psdout(1) << "up " << oldup << " -> " << up << ", acting " << oldacting
            << " -> " << acting << ", acting_primary " << old_acting_primary
            << " -> " << new_acting_primary << ", up_primary " << old_up_primary
            << " -> " << new_up_primary << ", role " << oldrole << " -> "
            << role << ", features acting " << acting_features << " upacting "
            << upacting_features << dendl;
...
PeeringState::on_new_interval()
{
  ...
  psdout(20) << "upacting_features 0x" << std::hex << upacting_features
             << std::dec << " from " << acting << "+" << up << dendl;
  ...
  psdout(10) << "prior_readable_until_ub " << prior_readable_until_ub
             << " (mnow " << mnow << " + "
             << info.history.prior_readable_until_ub << ")" << dendl;
...
void
PeeringState::init_primary_up_acting(
    const vector<int>& newup,
    const vector<int>& newacting,
    int new_up_primary,
    int new_acting_primary)
{
  ...
  for (uint8_t i = 0; i < up.size(); ++i) {
    if (up[i] != CRUSH_ITEM_NONE)
      upset.insert(pg_shard_t(
          up[i], pool.info.is_erasure() ? shard_id_t(i) : shard_id_t::NO_SHARD));
  }
  if (!pool.info.is_erasure()) {
    ...
    for (uint8_t i = 0; i < acting.size(); ++i) {
      if (pg_temp[i] == new_acting_primary) {
        if (has_pgtemp) {
          primary = pg_shard_t(
              new_acting_primary, osdmap->pgtemp_undo_primaryfirst(
...
void
PeeringState::clear_recovery_state()
{
...
get_required_past_interval_bounds(const pg_info_t& info, epoch_t oldest_map)
{
  epoch_t start = std::max(
      ...
      oldest_map);
  epoch_t end = std::max(
      info.history.same_interval_since, info.history.epoch_pool_created);
...
PeeringState::check_past_interval_bounds() const
{
  ...
  if (rpib.first >= rpib.second) {
    ...
    if (!past_intervals.empty() && rpib.first > oldest_epoch) {
      ...
      derr << info.pgid << " required past_interval bounds are" << " empty ["
           << rpib << ") but past_intervals is not: " << past_intervals
           << dendl;
    }
  } else {
    ...
    if (apib.first > rpib.first) {
      ...
      derr << info.pgid << " past_intervals [" << apib
           << ") start interval does not contain the required" << " bound ["
           << rpib << ") start" << dendl;
...
int
PeeringState::clamp_recovery_priority(
    int priority,
    int pool_recovery_priority,
    int max)
{
  static_assert(
      OSD_RECOVERY_PRIORITY_MIN < OSD_RECOVERY_PRIORITY_MAX,
...
unsigned
PeeringState::get_recovery_priority()
{
  ...
  if (state & PG_STATE_FORCED_RECOVERY) {
    ...
    ret = clamp_recovery_priority(
        ret, pool_recovery_priority, get_max_prio_for_base(base));
  }
...
unsigned
PeeringState::get_backfill_priority()
{
  // a higher value -> a higher priority
...
unsigned
PeeringState::get_delete_priority()
{
...
bool
PeeringState::set_force_recovery(bool b)
{
  bool did = false;
  if (b) {
    if (!(state & PG_STATE_FORCED_RECOVERY) &&
  ...
  if (did) {
    psdout(20) << "state " << get_current_state() << dendl;
...
bool
PeeringState::set_force_backfill(bool b)
{
  bool did = false;
  if (b) {
    if (!(state & PG_STATE_FORCED_BACKFILL) &&
...
void
PeeringState::schedule_renew_lease()
{
...
PeeringState::send_lease()
{
  ...
  for (auto peer : actingset) {
    ...
    pl->send_cluster_message(
        peer.osd,
        TOPNSPC::make_message<MOSDPGLease>(
            epoch, spg_t(spgid.pgid, peer.shard), get_lease()),
        epoch);
  }
}

void
PeeringState::proc_lease(const pg_lease_t& l)
{
  ...
  if (ruub > readable_until_ub) {
    ...
    psdout(20) << "readable_until_ub now " << readable_until_ub << dendl;
  }
}

void
PeeringState::proc_lease_ack(int from, const pg_lease_ack_t& a)
{
  ceph_assert(HAVE_FEATURE(upacting_features, SERVER_OCTOPUS));
...
PeeringState::recalc_readable_until()
{
  ...
  for (unsigned i = 0; i < acting.size(); ++i) {
    ...
    dout(20) << "peer osd." << acting[i] << " ruub "
  ...
  dout(20) << "readable_until[_ub] " << readable_until << " (sent "
           << readable_until_ub_sent << ")" << dendl;
}

bool
PeeringState::check_prior_readable_down_osds(const OSDMapRef& map)
{
  ceph_assert(HAVE_FEATURE(upacting_features, SERVER_OCTOPUS));
  bool changed = false;
...
bool
PeeringState::adjust_need_up_thru(const OSDMapRef osdmap)
{
...
bool
PeeringState::needs_recovery() const
{
  ceph_assert(is_primary());
  ...
  if (missing.num_missing()) {
    psdout(10) << "primary has " << missing.num_missing() << " missing"
               << dendl;
    return true;
  }
  ...
  for (const pg_shard_t& peer : acting_recovery_backfill) {
    ...
    if (pm == peer_missing.end()) {
      psdout(10) << "osd." << peer << " doesn't have missing set" << dendl;
      continue;
    }
    if (pm->second.num_missing()) {
      std::ostringstream ss;
      ss << "osd." << peer << " has " << pm->second.num_missing() << " missing";
...
bool
PeeringState::needs_backfill() const
{
  ceph_assert(is_primary());
  ...
  for (const pg_shard_t& peer : backfill_targets) {
    ...
    if (!pi->second.last_backfill.is_max()) {
      psdout(10) << "osd." << peer << " has last_backfill "
...
*/
bool
PeeringState::can_serve_read(const hobject_t& hoid)
{
  ceph_assert(!is_primary());
  ...
  if (!pg_log.get_log().has_write_since(hoid, pg_committed_to)) {
 ...
 */
bool
PeeringState::all_unfound_are_queried_or_lost(const OSDMapRef osdmap) const
{
  ceph_assert(is_primary());
...
PeeringState::reject_reservation()
{
  ...
  pl->send_cluster_message(
      ...
      TOPNSPC::make_message<MBackfillReserve>(
          ...
          spg_t(info.pgid.pgid, primary.shard), get_osdmap_epoch()),
      get_osdmap_epoch());
}
 ...
 */
void
PeeringState::calculate_maxles_and_minlua(
    ...
    bool* history_les_bound) const
{
  epoch_t max_last_epoch_started;
  eversion_t min_last_update_acceptable;
  calculate_maxles_and_minlua(
      infos, max_last_epoch_started, min_last_update_acceptable,
      exclude_nonprimary_shards, history_les_bound);
  ...
  if (up.size() && !primary->second.is_incomplete() &&
    ...
    if ((uint64_t)approx_missing_objects > force_auth_primary_missing_objects) {
      ...
      ss << "up_primary: " << up_primary << ") has approximate "
         << approx_missing_objects << "(>" << force_auth_primary_missing_objects
 ...
 */
void
PeeringState::calc_replicated_acting(
    ...
    ostream& ss)
{
  ...
  std::sort(
      candidate_by_last_update.begin(), candidate_by_last_update.end(),
      sort_by_eversion);
...
public:
  void
  add_osd(osd_ord_t ord, osd_id_t osd)
  {
  ...
  bool
  operator<(const bucket_candidates_t& rhs) const
  {
...
std::ostream&
operator<<(std::ostream& lhs, const bucket_candidates_t& cand)
{
...
class bucket_heap_t {
  ...
  struct comp {
    bool
    operator()(const elem_t& lhs, const elem_t& rhs)
    {
 ...
 */
void
PeeringState::calc_replicated_acting_stretch(
    map<pg_shard_t, pg_info_t>::const_iterator primary,
    eversion_t oldest_auth_log_entry,
    unsigned size,
    const vector<int>& acting,
    ...
    ostream& ss)
{
  ...
  auto get_ancestor = [&](int osd) mutable {
    int ancestor = osdmap->crush->get_parent_of_type(
        osd, pool.info.peering_crush_bucket_barrier, pool.info.crush_rule);
   ...
   */
  {
    ...
    auto get_osd_ord = [&](bool is_acting, const pg_info_t& info) -> osd_ord_t {
      return std::make_tuple(
          !is_acting /* acting should sort first */, info.last_update);
    };
...
bool
PeeringState::recoverable(const vector<int>& want) const
{
...
void
PeeringState::choose_async_recovery_ec(
    const map<pg_shard_t, pg_info_t>& all_info,
    ...
    const OSDMapRef osdmap) const
{
  ...
  psdout(20) << "candidates by cost are: " << candidates_by_cost << dendl;
  ...
  psdout(20) << "result want=" << *want << " async_recovery=" << *async_recovery
             << dendl;
}

void
PeeringState::choose_async_recovery_replicated(
    const map<pg_shard_t, pg_info_t>& all_info,
    const pg_info_t& auth_info,
    vector<int>* want,
    set<pg_shard_t>* async_recovery,
    ...
    bool* repeat_getlog)
{
  ...
  if (cct->_conf->subsys.should_gather<dout_subsys, 10>()) {
    for (auto p = all_info.begin(); p != all_info.end(); ++p) {
      psdout(10) << "all_info osd." << p->first << " " << p->second << dendl;
    }
  }

  auto auth_log_shard =
      find_best_info(all_info, restrict_to_up_acting, true, history_les_bound);
  auto get_log_shard =
      find_best_info(all_info, restrict_to_up_acting, false, history_les_bound);

  if ((auth_log_shard == all_info.end()) || (get_log_shard == all_info.end())) {
    if (up != acting) {
  ...
  if (pool.info.is_replicated()) {
    auto [primary_shard, oldest_log] = select_replicated_primary(
        ...
        up, up_primary, all_info, get_osdmap(), ss);
    if (pool.info.is_stretch_pool()) {
      calc_replicated_acting_stretch(
          primary_shard, oldest_log, get_osdmap()->get_pg_size(info.pgid.pgid),
          acting, up, up_primary, all_info, restrict_to_up_acting, &want,
          &want_backfill, &want_acting_backfill, get_osdmap(), pool, ss);
    } else {
    ...
    calc_ec_acting(
        auth_log_shard, get_osdmap()->get_pg_size(info.pgid.pgid), acting, up,
        all_info, restrict_to_up_acting, &want, &want_backfill,
        &want_acting_backfill, ss);
  }
  psdout(10) << ss.str() << dendl;
  ...
  while (want.size() > pool.info.size) {
    ...
    psdout(10) << "evicting osd." << want.back() << " from oversized want "
               << want << dendl;
    want.pop_back();
  }
  if ((want != acting) || pool.info.is_nonprimary_shard(pg_whoami.shard)) {
    if (pool.info.is_nonprimary_shard(pg_whoami.shard)) {
      psdout(10) << "shard " << pg_whoami.shard << " cannot be primary, want "
                 << pg_vector_string(want) << " acting "
                 << pg_vector_string(acting) << ", requesting pg_temp change"
                 << dendl;
    } else {
      psdout(10) << "want " << pg_vector_string(want) << " != acting "
                 << pg_vector_string(acting) << ", requesting pg_temp change"
                 << dendl;
    }
    want_acting = want;
  ...
  ceph_assert(
      ...
      async_recovery_targets == want_async_recovery || !needs_recovery());
    ...
    PeeringCtxWrapper& ctx)
{
  ...
  if (found_missing && oinfo.last_update != eversion_t()) {
    ...
    tinfo.partial_writes_last_complete_epoch =
        info.partial_writes_last_complete_epoch;
    ...
    if (!tinfo.partial_writes_last_complete.empty()) {
      psdout(20) << "sending info to " << from << " pwlc=e"
                 << tinfo.partial_writes_last_complete_epoch << ":"
                 << tinfo.partial_writes_last_complete << " info=" << tinfo
                 << dendl;
    }
    ctx.send_info(
        from.osd, spg_t(info.pgid.pgid, from.shard),
...
bool
PeeringState::discover_all_missing(BufferedRecoveryMessages& rctx)
{
  auto& missing = pg_log.get_missing();
  uint64_t unfound = get_num_unfound();
  bool any = false; // did we start any queries

  psdout(10) << missing.num_missing() << " missing, " << unfound << " unfound"
             << dendl;
  ...
  for (; m != mend; ++m) {
    ...
    if (peer_missing.find(peer) != peer_missing.end()) {
      psdout(20) << ": osd." << peer << ": we already have pg_missing_t"
                 << dendl;
      continue;
    }
    if (peer_log_requested.find(peer) != peer_log_requested.end()) {
      psdout(20) << ": osd." << peer << ": in peer_log_requested" << dendl;
      continue;
    }
    if (peer_missing_requested.find(peer) != peer_missing_requested.end()) {
      psdout(20) << ": osd." << peer << ": in peer_missing_requested" << dendl;
      continue;
    }

    // Request missing
    psdout(10) << ": osd." << peer << ": requesting pg_missing_t" << dendl;
    peer_missing_requested.insert(peer);
    rctx.send_query(
        peer.osd, spg_t(info.pgid.pgid, peer.shard),
        pg_query_t(
            pg_query_t::FULLLOG, peer.shard, pg_whoami.shard, info.history,
            get_osdmap_epoch()));
    any = true;
  }
  return any;
    ...
    PeeringCtxWrapper& ctx)
{
  ...
  if (is_primary()) {
    ...
    info.history.refresh_prior_readable_until_ub(
        pl->get_mnow(), prior_readable_until_ub);
      ...
      if (pi.last_update == info.last_update) {
        ...
        if (!pi.is_empty()) {
          ...
          if (!info.partial_writes_last_complete.empty()) {
            psdout(20) << "sending info to " << peer << " pwlc=e"
                       << info.partial_writes_last_complete_epoch << ":"
                       << info.partial_writes_last_complete << " info=" << info
                       << dendl;
          }
          ctx.send_info(
              peer.osd, spg_t(info.pgid.pgid, peer.shard),
          ...
          m = TOPNSPC::make_message<MOSDPGLog>(
              i->shard, pg_whoami.shard, get_osdmap_epoch(), info,
              last_peering_reset);
        }
      } else if (
	 ...
	 */
        ...
        pi.partial_writes_last_complete_epoch =
            info.partial_writes_last_complete_epoch;
        ...
        m = TOPNSPC::make_message<MOSDPGLog>(
            i->shard, pg_whoami.shard, get_osdmap_epoch(), pi,
            last_peering_reset /* epoch to create pg at */);

        // send some recent log, so that op dup detection works well.
        m->log.copy_up_to(
            cct, pg_log.get_log(), cct->_conf->osd_max_pg_log_entries);
    ...
    for (auto i = acting_recovery_backfill.begin();
         i != acting_recovery_backfill.end(); ++i) {
      psdout(20) << "setting up missing_loc from shard " << *i << " " << dendl;
      if (*i == get_primary()) {
    ...
    if (needs_recovery()) {
      ...
      if (complete_shards.size() + 1 == acting_recovery_backfill.size()) {
        ...
        missing_loc.add_source_info(
            pg_whoami, info, pg_log.get_missing(), ctx.handle);
        for (auto i = acting_recovery_backfill.begin();
             i != acting_recovery_backfill.end(); ++i) {
          if (*i == pg_whoami)
            continue;
          ...
          ceph_assert(pm_it != peer_missing.end());
          missing_loc.add_source_info(
              *i, pi_it->second, pm_it->second, ctx.handle);
        }
      }
      for (auto i = peer_missing.begin(); i != peer_missing.end(); ++i) {
        ...
        ceph_assert(pi_it != peer_info.end());
        search_for_missing(pi_it->second, i->second, i->first, ctx);
      }
...
PeeringState::share_pg_info()
{
  ...
  for (auto pg_shard : acting_recovery_backfill) {
    ...
    auto m = TOPNSPC::make_message<MOSDPGInfo2>(
        spg_t{info.pgid.pgid, pg_shard.shard}, info, get_osdmap_epoch(),
        get_osdmap_epoch(), std::optional<pg_lease_t>{get_lease()},
        std::nullopt);
...
void
PeeringState::rewind_divergent_log(
    ObjectStore::Transaction& t,
    eversion_t newhead)
{
  PGLog::LogEntryHandlerRef rollbacker{pl->get_log_handler(t)};
...
void
PeeringState::proc_primary_info(
    ObjectStore::Transaction& t,
    const pg_info_t& oinfo)
{
  ceph_assert(!is_primary());
  ...
  if (!(info.purged_snaps == oinfo.purged_snaps)) {
    psdout(10) << "updating purged_snaps to " << oinfo.purged_snaps << dendl;
...
void
PeeringState::consider_adjusting_pwlc(eversion_t last_complete)
{
  for (const auto& [shard, versionrange] : info.partial_writes_last_complete) {
    ...
    if (last_complete > toversion) {
      ...
      info.partial_writes_last_complete[shard] =
          std::pair(last_complete, last_complete);
...
void
PeeringState::consider_rollback_pwlc(eversion_t last_complete)
{
  for (const auto& [shard, versionrange] : info.partial_writes_last_complete) {
    auto [fromversion, toversion] = versionrange;
    ...
    pg_shard_t from)
{
  ...
  if (info.partial_writes_last_complete.contains(from.shard)) {
    apply_pwlc(
        info.partial_writes_last_complete[from.shard], from, oinfo, &olog);
  }
  ...
  if (pool.info.allows_ecoptimizations() && (olog.head < pg_log.get_head())) {
    ...
    while (p != pg_log.get_log().log.end()) {
      ...
      for (auto&& [pg_shard, pi] : all_info) {
        psdout(20) << "version " << p->version << " testing osd " << pg_shard
      ...
      psdout(20) << "shards_with_update=" << shards_with_update
                 ...
                 << dendl;
      if (!shard_id_set::intersection(
               shards_without_update, shards_currently_present)
               .empty()) {
  ...
  if (info.last_backfill.is_max() && pool.info.is_nonprimary_shard(from.shard)) {
    invalidate_stats = true;
  }
    ...
    pg_shard_t from)
{
  ...
  pg_log.proc_replica_log(
      oinfo, olog, omissing, from, pg_whoami,
      pool.info.allows_ecoptimizations());
  ...
  for (auto i = omissing.get_items().begin(); i != omissing.get_items().end();
       ++i) {
...
PeeringState::calc_min_last_complete_ondisk()
{
  ...
  for (const auto& pg_shard : acting_recovery_backfill) {
    ...
    if (peer_last_complete_ondisk.count(pg_shard) == 0) {
      psdout(20) << "no complete info on: " << pg_shard << dendl;
      return;
    }
  ...
  if (min != min_last_complete_ondisk) {
    psdout(20) << "last_complete_ondisk is " << "updated to: " << min
               << " from: " << min_last_complete_ondisk << dendl;
    min_last_complete_ondisk = min;
  }
}

void
PeeringState::fulfill_info(
    pg_shard_t from,
    ...
    epoch_t query_epoch)
{
  ...
  auto mlog = TOPNSPC::make_message<MOSDPGLog>(
      from.shard, pg_whoami.shard, get_osdmap_epoch(), info, query_epoch);
...
void
PeeringState::fulfill_query(const MQuery& query, PeeringCtxWrapper& rctx)
{
  if (query.query.type == pg_query_t::INFO) {
    ...
    rctx.send_notify(
        ...
        pg_notify_t(
            notify_info.first.shard, pg_whoami.shard, query.query_epoch,
            get_osdmap_epoch(), notify_info.second, past_intervals,
      ...
      if (pool.info.is_pending_merge(info.pgid.pgid, &target)) {
        if (target) {
          ...
          pl->set_ready_to_merge_target(
              info.last_update, info.history.last_epoch_started,
    ...
    unsigned split_bits)
{
  ...
  child->info.partial_writes_last_complete_epoch =
      info.partial_writes_last_complete_epoch;
    ...
    const pg_merge_meta_t& last_pg_merge_meta)
{
  ...
  for (auto& i : sources) {
    ...
    if (source->info.last_complete != source->info.last_update ||
      ...
      psdout(10) << "source " << source->pg_whoami << " incomplete" << dendl;
      incomplete = true;
    }
    if (last_pg_merge_meta.source_pgid != pg_t()) {
  ...
  if (info.history.epoch_created == 0) {
    ...
    psdout(10) << "set les/c to " << last_pg_merge_meta.last_epoch_started
    ...
    if (info.history.last_epoch_clean < pib.first) {
      psdout(10) << "last_epoch_clean " << info.history.last_epoch_clean
                 << " < past_interval start " << pib.first
    ...
    if (info.history.last_epoch_started < info.history.same_interval_since) {
      psdout(10) << "last_epoch_started " << info.history.last_epoch_started
                 << " < same_interval_since "
                 << info.history.same_interval_since
...
void
PeeringState::start_split_stats(
    const set<spg_t>& childpgs,
...
void
PeeringState::finish_split_stats(
...
void
PeeringState::update_blocked_by()
{
...
static bool
find_shard(const set<pg_shard_t>& pgs, shard_id_t shard)
{
  for (auto& p : pgs)
    if (p.shard == shard)
      return true;
...
static pg_shard_t
get_another_shard(const set<pg_shard_t>& pgs, pg_shard_t skip, shard_id_t shard)
{
  for (auto& p : pgs) {
    if (p == skip)
      continue;
...
PeeringState::update_calc_stats()
{
  ...
  if (info.stats.stats.sum.num_objects < 0) {
    psdout(0) << "negative num_objects = " << info.stats.stats.sum.num_objects
              << " setting it to 0 " << dendl;
  ...
  if ((is_remapped() || is_undersized() || !is_clean()) &&
    ...
    psdout(20) << "actingset " << actingset << " upset " << upset
...

    {
      ...
      psdout(20) << "shard " << pg_whoami << " primary objects " << num_objects
    ...
    for (auto& peer : peer_info) {
      ...
      if (is_backfill_target(peer.first)) {
        ...
        if (pm_it != peer_missing.end()) {
          ...
          psdout(20) << "no peer_missing found for " << peer.first << dendl;
          if (is_recovering()) {
            estimate = true;
          }
          missing = std::max((int64_t)0, num_objects - peer_num_objects);
      ...
      psdout(20) << "shard " << peer.first << " objects " << peer_num_objects
                 << " missing " << missing << dendl;
    }
    ...
    for (auto& ml : missing_loc.get_missing_locs()) {
      ...
      psdout(30) << ml.first << " object_location_counts[" << ml.second
                 << "]=" << info.stats.object_location_counts[ml.second]
                 << dendl;
    }
    ...
    psdout(30) << "object_location_counts[" << upset
               << "]=" << info.stats.object_location_counts[upset] << dendl;
    psdout(20) << "object_location_counts " << info.stats.object_location_counts
               << dendl;
    ...
    if (is_recovering()) {
      for (auto& sml : missing_loc.get_missing_by_count()) {
        for (auto& ml : sml.second) {
          ...
          if (sml.first == shard_id_t::NO_SHARD) {
            psdout(20) << "ml " << ml.second << " upset size " << upset.size()
            ...
            psdout(20) << "shard " << sml.first << " ml " << ml.second
      ...
      psdout(20) << "missing based degraded " << degraded << dendl;
      psdout(20) << "missing based misplaced " << misplaced << dendl;
    ...
    if (pool.info.is_replicated()) {
      ...
      if (needed)
        missing_target_objects.emplace(
            num_objects * needed, pg_shard_t(pg_shard_t::NO_OSD));
    } else {
      for (unsigned i = 0; i < num_shards; ++i) {
        shard_id_t shard(i);
        bool found = false;
        ...
        if (!found)
          missing_target_objects.emplace(
              num_objects, pg_shard_t(pg_shard_t::NO_OSD, shard));
      }
    }
    ...
    for (const auto& a : acting_source_objects) {
      ...
      psdout(20) << "extra acting misplaced " << extra_misplaced << dendl;
      misplaced += extra_misplaced;
    }
  out:
    // NOTE: Tests use these messages to verify this code
    psdout(20) << "degraded " << degraded << (estimate ? " (est)" : "")
               << dendl;
    psdout(20) << "misplaced " << misplaced << (estimate ? " (est)" : "")
               << dendl;
    ...
    const object_stat_collection_t& unstable_stats)
{
  if (info.stats.stats.sum.num_scrub_errors) {
    psdout(10) << "inconsistent due to "
               << info.stats.stats.sum.num_scrub_errors << " scrub errors"
               << dendl;
    state_set(PG_STATE_INCONSISTENT);
  } else {
  ...
  psdout(20) << "reporting purged_snaps " << pre_publish.purged_snaps << dendl;
...
void
PeeringState::increment_stats_invalidations_counter(bool invalidation_state)
{
...
void
PeeringState::dump_peering_state(Formatter* f)
{
...
void
PeeringState::update_stats_wo_resched(
...
bool
PeeringState::append_log_entries_update_missing(
    ...
    std::optional<eversion_t> pg_committed_to)
{
  ...
  bool invalidate_stats = pg_log.append_new_log_entries(
      info.last_backfill, entries, &info, rollbacker.get(), pool.info,
      pg_whoami.shard, pool.info.allows_ecoptimizations());
...
void
PeeringState::merge_new_log_entries(
    const mempool::osd_pglog::list<pg_log_entry_t>& entries,
    ObjectStore::Transaction& t,
    std::optional<eversion_t> trim_to,
    std::optional<eversion_t> pg_committed_to)
{
  ...
  bool rebuild_missing =
      append_log_entries_update_missing(entries, t, trim_to, pg_committed_to);
  for (auto i = acting_recovery_backfill.begin();
       i != acting_recovery_backfill.end(); ++i) {
    pg_shard_t peer(*i);
    if (peer == pg_whoami)
    ...
    psdout(20) << "peer_missing for " << peer << " = " << pmissing << dendl;
    pg_info_t& pinfo = pi_it->second;
    bool invalidate_stats = PGLog::append_log_entries_update_missing(
        pinfo.last_backfill, entries, true, NULL, pmissing, NULL, pool.info,
        peer.shard, dpp);
  ...
  for (auto&& i : entries) {
    missing_loc.rebuild(
        i.soid, pg_whoami, acting_recovery_backfill, info, pg_log.get_missing(),
        peer_missing, peer_info);
  }
}

void
PeeringState::add_log_entry(
    ...
    bool applied)
{
  ...
  // log mutation
  enum PGLog::NonPrimary nonprimary{
      pool.info.is_nonprimary_shard(info.pgid.shard)};
    ...
    bool async)
{
  ...
  if (transaction_applied && roll_forward_to > pg_log.get_can_rollback_to()) {
    pg_log.roll_forward_to(roll_forward_to, &info, handler.get());
    last_rollback_info_trimmed_to_applied = roll_forward_to;
  }

  psdout(10) << "approx pg log length =  " << pg_log.get_log().approx_size()
             << dendl;
  psdout(10) << "dups pg log length =  " << pg_log.get_log().dups.size()
             << dendl;
  psdout(10) << "transaction_applied = " << transaction_applied << dendl;
  if (!transaction_applied || async)
    psdout(10) << pg_whoami << " is async_recovery or backfill target" << dendl;
  if (pool.info.allows_ecoptimizations() &&
...
void
PeeringState::update_backfill_progress(
    ...
    ObjectStore::Transaction& t)
{
  ...
  if (preserve_local_num_bytes) {
    psdout(25) << "primary " << updated_stats.stats.sum.num_bytes << " local "
...
void
PeeringState::adjust_purged_snaps(
...
void
PeeringState::begin_peer_recover(pg_shard_t peer, const hobject_t soid)
{
  peer_missing[peer].revise_have(soid, eversion_t());
}

void
PeeringState::force_object_missing(
    ...
    eversion_t version)
{
  ...
  missing_loc.rebuild(
      soid, pg_whoami, acting_recovery_backfill, info, pg_log.get_missing(),
      peer_missing, peer_info);
}
    ...
    eversion_t at_version)
{
  ...
  if (requires_missing_loc) {
    for (auto&& entry : logv) {
      ...
      missing_loc.add_missing(
          entry.soid, entry.version, eversion_t(), entry.is_delete());
...
void
PeeringState::recovery_committed_to(eversion_t version)
{
  psdout(10) << "version " << version << " now ondisk" << dendl;
    ...
    if (!is_primary()) {
      ...
      pl->send_cluster_message(
          ...
          TOPNSPC::make_message<MOSDPGTrim>(
              get_osdmap_epoch(), spg_t(info.pgid.pgid, primary.shard),
...
void
PeeringState::complete_write(eversion_t v, eversion_t lc)
{
  pg_committed_to = v;
...
PeeringState::calc_trim_to_aggressive()
{
  ...
  eversion_t limit = std::min(
      {pg_log.get_head(), pg_log.get_can_rollback_to(), pg_committed_to});
  ...
  if (limit != eversion_t() && limit != pg_trim_to &&
    ...
    psdout(10) << "approx pg log length =  " << pg_log.get_log().approx_size()
               << dendl;
    uint64_t num_to_trim = std::min<uint64_t>(
        pg_log.get_log().approx_size() - target,
        cct->_conf->osd_pg_log_trim_max);
...
void
PeeringState::update_complete_backfill_object_stats(
    const hobject_t& hoid,
...
void
PeeringState::update_peer_last_backfill(
    pg_shard_t peer,
...
void
PeeringState::set_revert_with_targets(
    const hobject_t& soid,
...
void
PeeringState::update_peer_last_complete_ondisk(
    pg_shard_t fromosd,
    eversion_t lcod)
{
  psdout(20) << "updating peer_last_complete_ondisk" << " of osd: " << fromosd
             << " to: " << lcod << dendl;
  peer_last_complete_ondisk[fromosd] = lcod;
}

void
PeeringState::update_last_complete_ondisk(eversion_t lcod)
{
  psdout(20) << "updating last_complete_ondisk" << " to: " << lcod << dendl;
  last_complete_ondisk = lcod;
}

void
PeeringState::prepare_backfill_for_missing(
    const hobject_t& soid,
    const eversion_t& version,
...
void
PeeringState::update_hset(const pg_hit_set_history_t& hset_history)
{
...
PeeringState::Crashed::Crashed(my_context ctx) :
  my_base(ctx), NamedState(context<PeeringMachine>().state_history, "Crashed")
{
...
PeeringState::Initial::Initial(my_context ctx) :
  my_base(ctx), NamedState(context<PeeringMachine>().state_history, "Initial")
{
  context<PeeringMachine>().log_enter(state_name);
}
...
boost::statechart::result
PeeringState::Initial::react(const MInfoRec& i)
{
  DECLARE_LOCALS;
...
boost::statechart::result
PeeringState::Initial::react(const MLogRec& i)
{
  DECLARE_LOCALS;
  ceph_assert(!ps->is_primary());
  post_event(i);
...
PeeringState::Started::Started(my_context ctx) :
  my_base(ctx), NamedState(context<PeeringMachine>().state_history, "Started")
{
  context<PeeringMachine>().log_enter(state_name);
}
...
boost::statechart::result
PeeringState::Started::react(const AdvMap& advmap)
{
  DECLARE_LOCALS;
  psdout(10) << "Started advmap" << dendl;
  ps->check_full_transition(advmap.lastmap, advmap.osdmap);
  if (ps->should_restart_peering(
          advmap.up_primary, advmap.acting_primary, advmap.newup,
          advmap.newacting, advmap.lastmap, advmap.osdmap)) {
...
boost::statechart::result
PeeringState::Started::react(const QueryState& q)
{
...
boost::statechart::result
PeeringState::Started::react(const QueryUnfound& q)
{
...
PeeringState::Reset::Reset(my_context ctx) :
  my_base(ctx), NamedState(context<PeeringMachine>().state_history, "Reset")
{
  context<PeeringMachine>().log_enter(state_name);
  DECLARE_LOCALS;
...
boost::statechart::result
PeeringState::Reset::react(const AdvMap& advmap)
{
  DECLARE_LOCALS;
  ...
  if (ps->should_restart_peering(
    ...
    ps->start_peering_interval(
        advmap.lastmap, advmap.newup, advmap.up_primary, advmap.newacting,
        advmap.acting_primary, context<PeeringMachine>().get_cur_transaction());
  }
  ps->remove_down_peer_info(advmap.osdmap);
  ...
  if (ps->should_send_notify() && ps->get_primary().osd >= 0) {
    ...
    context<PeeringMachine>().send_notify(
        ...
        pg_notify_t(
            ...
            ps->past_intervals, ps->local_pg_acting_features));
  }
...
boost::statechart::result
PeeringState::Reset::react(const QueryState& q)
{
  q.f->open_object_section("state");
  q.f->dump_string("name", state_name);
  q.f->dump_stream("enter_time") << enter_time;
...
boost::statechart::result
PeeringState::Reset::react(const QueryUnfound& q)
{
...
PeeringState::Start::Start(my_context ctx) :
  my_base(ctx), NamedState(context<PeeringMachine>().state_history, "Start")
{
  context<PeeringMachine>().log_enter(state_name);
...
boost::statechart::result
PeeringState::Peering::react(const AdvMap& advmap)
{
  DECLARE_LOCALS;
...
boost::statechart::result
PeeringState::Peering::react(const QueryState& q)
{
  DECLARE_LOCALS;
  ...
  for (auto p = prior_set.blocked_by.begin(); p != prior_set.blocked_by.end();
       ++p) {
...
boost::statechart::result
PeeringState::Peering::react(const QueryUnfound& q)
{
...
void
PeeringState::Backfilling::backfill_release_reservations()
{
  DECLARE_LOCALS;
  pl->cancel_local_background_io_reservation();
  for (auto it = ps->backfill_targets.begin(); it != ps->backfill_targets.end();
       ++it) {
    ceph_assert(*it != ps->pg_whoami);
    pl->send_cluster_message(
        it->osd,
        TOPNSPC::make_message<MBackfillReserve>(
            MBackfillReserve::RELEASE, spg_t(ps->info.pgid.pgid, it->shard),
            ps->get_osdmap_epoch()),
        ps->get_osdmap_epoch());
  }
}
...
void
PeeringState::Backfilling::suspend_backfill()
{
  DECLARE_LOCALS;
...
boost::statechart::result
PeeringState::WaitRemoteBackfillReserved::react(
...
void
PeeringState::WaitRemoteBackfillReserved::exit()
{
  context<PeeringMachine>().log_exit(state_name, enter_time);
  DECLARE_LOCALS;
...
void
PeeringState::WaitRemoteBackfillReserved::retry()
{
  DECLARE_LOCALS;
  pl->cancel_local_background_io_reservation();
      ...
      "Started/Primary/Active/WaitLocalBackfillReserved")
{
  ...
  pl->request_local_background_io_reservation(
      ...
      std::make_unique<PGPeeringEvent>(
          ps->get_osdmap_epoch(), ps->get_osdmap_epoch(),
...
void
PeeringState::WaitLocalBackfillReserved::exit()
{
  context<PeeringMachine>().log_exit(state_name, enter_time);
  DECLARE_LOCALS;
  utime_t dur = ceph_clock_now() - enter_time;
...
boost::statechart::result
PeeringState::NotBackfilling::react(const QueryUnfound& q)
{
  DECLARE_LOCALS;
...
void
PeeringState::NotBackfilling::exit()
{
  context<PeeringMachine>().log_exit(state_name, enter_time);
...
boost::statechart::result
PeeringState::NotRecovering::react(const QueryUnfound& q)
{
  DECLARE_LOCALS;
...
void
PeeringState::NotRecovering::exit()
{
  context<PeeringMachine>().log_exit(state_name, enter_time);
...
void
PeeringState::RepNotRecovering::exit()
{
  context<PeeringMachine>().log_exit(state_name, enter_time);
  DECLARE_LOCALS;
  utime_t dur = ceph_clock_now() - enter_time;
...
PeeringState::RepWaitRecoveryReserved::react(const RemoteRecoveryReserved& evt)
{
  ...
  pl->send_cluster_message(
      ...
      TOPNSPC::make_message<MRecoveryReserve>(
          MRecoveryReserve::GRANT, spg_t(ps->info.pgid.pgid, ps->primary.shard),
          ps->get_osdmap_epoch()),
      ps->get_osdmap_epoch());
...
void
PeeringState::RepWaitRecoveryReserved::exit()
{
  context<PeeringMachine>().log_exit(state_name, enter_time);
  DECLARE_LOCALS;
  utime_t dur = ceph_clock_now() - enter_time;
  ...
  if (!pl->try_reserve_recovery_space(
    ...
    if (HAVE_FEATURE(ps->upacting_features, RECOVERY_RESERVATION_2)) {
      ...
      preempt = std::make_unique<PGPeeringEvent>(
          pl->get_osdmap_epoch(), pl->get_osdmap_epoch(),
...
void
PeeringState::RepWaitBackfillReserved::exit()
{
  context<PeeringMachine>().log_exit(state_name, enter_time);
  DECLARE_LOCALS;
  utime_t dur = ceph_clock_now() - enter_time;
...
PeeringState::RepWaitBackfillReserved::react(const RemoteBackfillReserved& evt)
{
  ...
  pl->send_cluster_message(
      ps->primary.osd,
      TOPNSPC::make_message<MBackfillReserve>(
          MBackfillReserve::GRANT, spg_t(ps->info.pgid.pgid, ps->primary.shard),
          ps->get_osdmap_epoch()),
      ps->get_osdmap_epoch());
  return transit<RepRecovering>();
}
...
PeeringState::RepRecovering::react(const RemoteRecoveryPreempted&)
{
  ...
  pl->send_cluster_message(
      ...
      TOPNSPC::make_message<MRecoveryReserve>(
          ...
          spg_t(ps->info.pgid.pgid, ps->primary.shard), ps->get_osdmap_epoch()),
      ps->get_osdmap_epoch());
  return discard_event();
}
...
void
PeeringState::RepRecovering::exit()
{
  context<PeeringMachine>().log_exit(state_name, enter_time);
  DECLARE_LOCALS;
  pl->unreserve_recovery_space();
...
PeeringState::WaitLocalRecoveryReserved::react(const RecoveryTooFull& evt)
{
  ...
  pl->schedule_event_after(
      std::make_shared<PGPeeringEvent>(
          ps->get_osdmap_epoch(), ps->get_osdmap_epoch(), DoRecovery()),
...
boost::statechart::result
PeeringState::WaitRemoteRecoveryReserved::react(
  ...
  if (remote_recovery_reservation_it !=
    ...
    pl->send_cluster_message(
        ...
        TOPNSPC::make_message<MRecoveryReserve>(
            ...
            spg_t(
                context<PeeringMachine>().spgid.pgid,
                remote_recovery_reservation_it->shard),
            ps->get_osdmap_epoch(), ps->get_recovery_priority()),
        ps->get_osdmap_epoch());
    ++remote_recovery_reservation_it;
  } else {
...
void
PeeringState::WaitRemoteRecoveryReserved::exit()
{
  context<PeeringMachine>().log_exit(state_name, enter_time);
  DECLARE_LOCALS;
  utime_t dur = ceph_clock_now() - enter_time;
...
void
PeeringState::Recovering::release_reservations(bool cancel)
{
  DECLARE_LOCALS;
  ...
  for (auto i = context<Active>().remote_shards_to_reserve_recovery.begin();
    ...
    pl->send_cluster_message(
        i->osd,
        TOPNSPC::make_message<MRecoveryReserve>(
            MRecoveryReserve::RELEASE, spg_t(ps->info.pgid.pgid, i->shard),
            ps->get_osdmap_epoch()),
        ps->get_osdmap_epoch());
  }
}
      ...
      "Started/Primary/Active/Recovered")
{
  ...
  if (ps->acting != ps->up && !ps->choose_acting(get_log_shard, true)) {
...
template <typename T>
set<pg_shard_t>
unique_osd_shard_set(const pg_shard_t& skip, const T& in)
{
  set<int> osds_found;
  set<pg_shard_t> out;
  ...
  all_replicas_activated(false)
{
  ...
  ps->activate(
      context<PeeringMachine>().get_cur_transaction(), ps->get_osdmap_epoch(),
...
boost::statechart::result
PeeringState::Active::react(const AdvMap& advmap)
{
  DECLARE_LOCALS;
  ...
  if (ps->peer_info.count(notevt.from)) {
    ...
    if (!ps->want_acting.empty() && ps->want_acting != ps->acting) {
      psdout(10) << "Active: got notify from previous acting member "
                 << notevt.from << ", requesting pg_temp change" << dendl;
    }
  }
  return discard_event();
}
...
boost::statechart::result
PeeringState::Active::react(const MTrim& trim)
{
  DECLARE_LOCALS;
  ceph_assert(ps->is_primary());
  ...
  ldout(ps->cct, 10) << " replica osd." << trim.from << " lcod " << trim.trim_to
                     << dendl;
  ps->update_peer_last_complete_ondisk(
      pg_shard_t{trim.from, trim.shard}, trim.trim_to);
...
boost::statechart::result
PeeringState::Active::react(const MInfoRec& infoevt)
{
  DECLARE_LOCALS;
  ceph_assert(ps->is_primary());
...
boost::statechart::result
PeeringState::Active::react(const MLogRec& logevt)
{
  DECLARE_LOCALS;
  psdout(10) << "searching osd." << logevt.from << " log for unfound items"
             << dendl;
  ps->proc_replica_log(
      logevt.msg->info, logevt.msg->log, std::move(logevt.msg->missing),
      logevt.from);
...
boost::statechart::result
PeeringState::Active::react(const QueryState& q)
{
  DECLARE_LOCALS;
  ...
  }
  {
    ...
    for (auto p = ps->backfill_targets.begin(); p != ps->backfill_targets.end();
         ++p)
...
boost::statechart::result
PeeringState::Active::react(const QueryUnfound& q)
{
  DECLARE_LOCALS;
...
PeeringState::Active::react(const AllReplicasActivated& evt)
{
  ...
  if (ps->prior_readable_until_ub > mnow) {
    ...
    pl->queue_check_readable(
        ps->last_peering_reset, ps->prior_readable_until_ub - mnow);
  } else {
...
boost::statechart::result
PeeringState::Active::react(const RenewLease& rl)
{
  DECLARE_LOCALS;
  ps->proc_renew_lease();
  return discard_event();
...
boost::statechart::result
PeeringState::Active::react(const MLeaseAck& la)
{
  DECLARE_LOCALS;
...
boost::statechart::result
PeeringState::Active::react(const PgCreateEvt& evt)
{
  DECLARE_LOCALS;
  pg_t pgid = context<PeeringMachine>().spgid.pgid;
...
PeeringState::ReplicaActive::react(const Activate& actevt)
{
  ...
  ps->activate(
      context<PeeringMachine>().get_cur_transaction(), actevt.activation_epoch,
      context<PeeringMachine>().get_recovery_ctx());
  psdout(10) << "Activate Finished" << dendl;
  return discard_event();
}
...
PeeringState::ReplicaActive::react(const ActivateCommitted& evt)
{
  ...
  rctx.send_info(
      ps->get_primary().osd, spg_t(ps->info.pgid.pgid, ps->get_primary().shard),
      epoch, epoch, i, {}, /* lease */
      ps->get_lease_ack());
...
boost::statechart::result
PeeringState::ReplicaActive::react(const MLease& l)
{
  DECLARE_LOCALS;
  ...
  pl->send_cluster_message(
      ps->get_primary().osd,
      TOPNSPC::make_message<MOSDPGLeaseAck>(
          epoch, spg_t(spgid.pgid, ps->get_primary().shard),
          ps->get_lease_ack()),
      epoch);
  return discard_event();
}
...
{
  DECLARE_LOCALS;
  ps->proc_primary_info(
      context<PeeringMachine>().get_cur_transaction(), infoevt.info);
  return discard_event();
}
...
boost::statechart::result
PeeringState::ReplicaActive::react(const QueryUnfound& q)
{
...
void
PeeringState::ReplicaActive::exit()
{
  context<PeeringMachine>().log_exit(state_name, enter_time);
  DECLARE_LOCALS;
  pl->unreserve_recovery_space();
...
boost::statechart::result
PeeringState::Stray::react(const MLogRec& logevt)
{
  DECLARE_LOCALS;
  MOSDPGLog* msg = logevt.msg.get();
...
boost::statechart::result
PeeringState::Stray::react(const MInfoRec& infoevt)
{
  DECLARE_LOCALS;
  ...
  if (infoevt.info.last_update > ps->info.last_update) {
    ...
    ceph_assert(infoevt.info.partial_writes_last_complete.contains(
        ps->pg_whoami.shard));
    auto pwlc =
        infoevt.info.partial_writes_last_complete.at(ps->pg_whoami.shard);
    psdout(20) << "info from osd." << infoevt.from
               << " last_update=" << infoevt.info.last_update
               << " last_complete=" << infoevt.info.last_complete << " pwlc=e"
               << infoevt.info.partial_writes_last_complete_epoch << ":" << pwlc
...
boost::statechart::result
PeeringState::Stray::react(const MQuery& query)
{
  DECLARE_LOCALS;
  ps->fulfill_query(query, context<PeeringMachine>().get_recovery_ctx());
  return discard_event();
...
boost::statechart::result
PeeringState::ToDelete::react(const ActMap& evt)
{
  DECLARE_LOCALS;
...
void
PeeringState::WaitDeleteReserved::exit()
{
  context<PeeringMachine>().log_exit(state_name, enter_time);
}
...
boost::statechart::result
PeeringState::Deleting::react(const DeleteSome& evt)
{
  DECLARE_LOCALS;
      ...
      "Started/Primary/Peering/GetInfo")
{
  ...
  if (ps->prior_readable_down_osds.empty()) {
    psdout(10) << " no prior_set down osds, will clear prior_readable_until_ub "
                  "before activating"
               << dendl;
  }
...
void
PeeringState::GetInfo::get_infos()
{
  DECLARE_LOCALS;
  PastIntervals::PriorSet& prior_set = context<Peering>().prior_set;
  ...
  for (auto it = prior_set.probe.begin(); it != prior_set.probe.end(); ++it) {
    ...
    if (peer_info_requested.count(peer)) {
      ...
      context<PeeringMachine>().send_query(
          peer.osd, pg_query_t(
                        pg_query_t::INFO, it->shard, ps->pg_whoami.shard,
...
PeeringState::GetInfo::react(const MNotifyRec& infoevt)
{
  ...
  if (ps->proc_replica_notify(infoevt.from, infoevt.notify)) {
    ...
    if (peer_info_requested.empty() && !prior_set.pg_down) {
      ...
      psdout(20) << "Common pg_acting_features: " << hex
                 << ps->get_pg_acting_features() << dec << dendl;
      post_event(GotInfo());
    }
  }
  return discard_event();
...
boost::statechart::result
PeeringState::GetInfo::react(const QueryState& q)
{
  DECLARE_LOCALS;
  q.f->open_object_section("state");
  q.f->dump_string("name", state_name);
  ...
  for (auto p = peer_info_requested.begin(); p != peer_info_requested.end();
       ++p) {
    q.f->open_object_section("osd");
    q.f->dump_stream("osd") << *p;
...
boost::statechart::result
PeeringState::GetInfo::react(const QueryUnfound& q)
{
  ...
  msg(0)
{
  ...
  // adjust acting?
  if (!ps->choose_acting(
          auth_log_shard, false, false, &context<Peering>().history_les_bound,
          &repeat_getlog)) {
  ...
  context<PeeringMachine>().send_query(
      auth_log_shard.osd,
      pg_query_t(
          pg_query_t::LOG, auth_log_shard.shard, ps->pg_whoami.shard,
...
boost::statechart::result
PeeringState::GetLog::react(const AdvMap& advmap)
{
...
boost::statechart::result
PeeringState::GetLog::react(const MLogRec& logevt)
{
  ceph_assert(!msg);
  if (logevt.from != auth_log_shard) {
    psdout(10) << "GetLog: discarding log from " << "non-auth_log_shard osd."
               << logevt.from << dendl;
    return discard_event();
  }
...
PeeringState::GetLog::react(const GotLog&)
{
  ...
  if (msg) {
    ...
    ps->proc_master_log(
        context<PeeringMachine>().get_cur_transaction(), msg->info,
        std::move(msg->log), std::move(msg->missing), auth_log_shard);
    if (repeat_getlog) {
      // Only EC pools with ec_optimizations enabled:
...
boost::statechart::result
PeeringState::GetLog::react(const QueryState& q)
{
  q.f->open_object_section("state");
  q.f->dump_string("name", state_name);
  q.f->dump_stream("enter_time") << enter_time;
...
boost::statechart::result
PeeringState::GetLog::react(const QueryUnfound& q)
{
...
boost::statechart::result
PeeringState::WaitActingChange::react(const QueryUnfound& q)
{
...
void
PeeringState::WaitActingChange::exit()
{
  context<PeeringMachine>().log_exit(state_name, enter_time);
  DECLARE_LOCALS;
  utime_t dur = ceph_clock_now() - enter_time;
...
boost::statechart::result
PeeringState::Down::react(const QueryState& q)
{
  q.f->open_object_section("state");
  q.f->dump_string("name", state_name);
  q.f->dump_stream("enter_time") << enter_time;
...
boost::statechart::result
PeeringState::Down::react(const QueryUnfound& q)
{
...
boost::statechart::result
PeeringState::Down::react(const MNotifyRec& infoevt)
{
  DECLARE_LOCALS;
  ...
  if (!ps->peer_info.count(infoevt.from) &&
      ps->get_osdmap()->has_been_up_since(
          infoevt.from.osd, infoevt.notify.epoch_sent)) {
...
boost::statechart::result
PeeringState::Incomplete::react(const QueryState& q)
{
  q.f->open_object_section("state");
  q.f->dump_string("name", state_name);
  q.f->dump_stream("enter_time") << enter_time;
...
boost::statechart::result
PeeringState::Incomplete::react(const QueryUnfound& q)
{
      ...
      "Started/Primary/Peering/GetMissing")
{
  ...
  for (auto i = ps->acting_recovery_backfill.begin();
    ...
    if (pi.log_tail <= since) {
      ...
      context<PeeringMachine>().send_query(
          i->osd, pg_query_t(
                      pg_query_t::LOG, i->shard, ps->pg_whoami.shard, since,
                      ps->info.history, ps->get_osdmap_epoch()));
    } else {
      ...
      context<PeeringMachine>().send_query(
          i->osd, pg_query_t(
                      pg_query_t::FULLLOG, i->shard, ps->pg_whoami.shard,
                      ps->info.history, ps->get_osdmap_epoch()));
    }
...
boost::statechart::result
PeeringState::GetMissing::react(const QueryState& q)
{
  DECLARE_LOCALS;
  q.f->open_object_section("state");
  q.f->dump_string("name", state_name);
...
boost::statechart::result
PeeringState::GetMissing::react(const QueryUnfound& q)
{
...
boost::statechart::result
PeeringState::WaitUpThru::react(const ActMap& am)
{
  DECLARE_LOCALS;
  if (!ps->need_up_thru) {
    post_event(Activate(ps->get_osdmap_epoch()));
...
boost::statechart::result
PeeringState::WaitUpThru::react(const QueryState& q)
{
  q.f->open_object_section("state");
  q.f->dump_string("name", state_name);
  q.f->dump_stream("enter_time") << enter_time;
...
boost::statechart::result
PeeringState::WaitUpThru::react(const QueryUnfound& q)
{
...
void
PeeringState::PeeringMachine::log_enter(const char* state_name)
{
  DECLARE_LOCALS;
...
void
PeeringState::PeeringMachine::log_exit(const char* state_name, utime_t enter_time)
{
  DECLARE_LOCALS;
  utime_t dur = ceph_clock_now() - enter_time;
...
ostream&
operator<<(ostream& out, const PeeringState& ps)
{
