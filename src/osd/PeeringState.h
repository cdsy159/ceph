...
struct PGPool {
  ...
  PGPool(
    ...
    cached_epoch(map->get_epoch()), id(i), name(name), info(info)
  {
  ...
  ceph::timespan
  get_readable_interval(ConfigProxy& conf) const
  {
    double v = 0;
...
struct fmt::formatter<PGPool> {
  ...
  auto
  format(const PGPool& pool, FormatContext& ctx)
  {
...
struct BufferedRecoveryMessages {
  ...
  void
  accept_buffered_messages(BufferedRecoveryMessages& m)
  {
    for (auto& [target, ls] : m.message_map) {
      ...
      ls.insert(
          ls.end(), std::make_move_iterator(ovec.begin()),
          std::make_move_iterator(ovec.end()));
      ovec.clear();
      ovec.swap(ls);
    }
  }
  ...
  void
  send_osd_message(int target, MsgT&& m)
  {
...
struct HeartbeatStamps : public RefCountedObject {
  ...
  void
  sent_ping(std::optional<ceph::signedspan>* delta_ub)
  {
    std::lock_guard l(lock);
...
inline std::ostream&
operator<<(std::ostream& out, const HeartbeatStamps& hb)
{
  hb.print(out);
  return out;
}
...
struct PeeringCtxWrapper {
  ...
  PeeringCtxWrapper(PeeringCtx& wrapped) :
    msgs(wrapped), transaction(wrapped.transaction), handle(wrapped.handle)
  {}

  PeeringCtxWrapper(BufferedRecoveryMessages& buf, PeeringCtx& wrapped) :
    msgs(buf), transaction(wrapped.transaction), handle(wrapped.handle)
  {}
  ...
  void
  send_notify(int to, const pg_notify_t& n)
  {
    msgs.send_notify(to, n);
  }

  void
  send_query(int to, spg_t spgid, const pg_query_t& q)
  {
...
/// Send cluster message to osd
    ...
    virtual void set_ready_to_merge_target(
        eversion_t lu,
        epoch_t les,
        epoch_t lec) = 0;
  ...
  struct AdvMap : boost::statechart::event<AdvMap> {
    ...
    AdvMap(
        OSDMapRef osdmap,
        OSDMapRef lastmap,
        std::vector<int>& newup,
        int up_primary,
        std::vector<int>& newacting,
        int acting_primary) :
      osdmap(osdmap),
      lastmap(lastmap),
      newup(newup),
  ...
  struct Activate : boost::statechart::event<Activate> {
    ...
    explicit Activate(epoch_t q) :
      boost::statechart::event<Activate>(), activation_epoch(q)
    {}
  ...
  struct ActivateCommitted : boost::statechart::event<ActivateCommitted> {
    ...
    print(std::ostream* out) const
    {
      *out << "ActivateCommitted from " << activation_epoch << " processed at "
           << epoch;
    }
  };
...
public:
  ...
  struct RequestScrub : boost::statechart::event<RequestScrub> {
    ...
    explicit RequestScrub(bool d, bool r) :
      deep(scrub_level_t(d)), repair(scrub_type_t(r))
    {}
    ...
    {
      *out << "RequestScrub("
           << ((deep == scrub_level_t::deep) ? "deep" : "shallow")
           << ((repair == scrub_type_t::do_repair) ? " repair)" : ")");
    }
  };

  TrivialEvent(Initialize) TrivialEvent(GotInfo) TrivialEvent(
      NeedUpThru) TrivialEvent(Backfilled) TrivialEvent(LocalBackfillReserved)
      TrivialEvent(RejectTooFullRemoteReservation) TrivialEvent(
          RequestBackfill) TrivialEvent(RemoteRecoveryPreempted)
          TrivialEvent(RemoteBackfillPreempted) TrivialEvent(
              BackfillTooFull) TrivialEvent(RecoveryTooFull)

              TrivialEvent(MakePrimary) TrivialEvent(MakeStray) TrivialEvent(
                  NeedActingChange) TrivialEvent(IsIncomplete)
                  TrivialEvent(RepeatGetLog) TrivialEvent(IsDown)

                      TrivialEvent(AllReplicasRecovered) TrivialEvent(
                          DoRecovery) TrivialEvent(LocalRecoveryReserved)
                          TrivialEvent(AllRemotesReserved) TrivialEvent(
                              AllBackfillsReserved) TrivialEvent(GoClean)

                              TrivialEvent(AllReplicasActivated)

                                  TrivialEvent(IntervalFlush)

                                      TrivialEvent(DeleteStart) TrivialEvent(
                                          DeleteSome)

                                          TrivialEvent(SetForceRecovery) TrivialEvent(
                                              UnsetForceRecovery)
...
private:
  ...
  class PeeringMachine
    : public boost::statechart::state_machine<PeeringMachine, Initial> {
  public:
    PeeringState* state;
    ...
    void
    send_query(int to, const pg_query_t& query)
    {
  ...
  struct Crashed : boost::statechart::state<Crashed, PeeringMachine>,
                   NamedState {
  ...
  struct Initial : boost::statechart::state<Initial, PeeringMachine>,
                   NamedState {
  ...
  struct Started : boost::statechart::state<Started, PeeringMachine, Start>,
                   NamedState {
  ...
  struct Active : boost::statechart::state<Active, Primary, Activating>,
    ...
    boost::statechart::result
    react(const DeferRecovery& evt)
    {
      return discard_event();
    }

    boost::statechart::result
    react(const DeferBackfill& evt)
    {
      return discard_event();
    }

    boost::statechart::result
    react(const UnfoundRecovery& evt)
    {
      return discard_event();
    }

    boost::statechart::result
    react(const UnfoundBackfill& evt)
    {
      return discard_event();
    }
  ...
  struct Backfilling : boost::statechart::state<Backfilling, Active>,
                       NamedState {
    typedef boost::mpl::list<
        boost::statechart::custom_reaction<Backfilled>,
        boost::statechart::custom_reaction<DeferBackfill>,
    ...
    boost::statechart::result
    react(const RemoteReservationRejectedTooFull& evt)
    {
  ...
  struct WaitRemoteBackfillReserved
    : boost::statechart::state<WaitRemoteBackfillReserved, Active>,
      NamedState {
    typedef boost::mpl::list<
  ...
  struct WaitLocalBackfillReserved
    : boost::statechart::state<WaitLocalBackfillReserved, Active>,
      NamedState {
    typedef boost::mpl::list<
        boost::statechart::
            transition<LocalBackfillReserved, WaitRemoteBackfillReserved>,
        boost::statechart::custom_reaction<RemoteBackfillReserved>>
        reactions;
    explicit WaitLocalBackfillReserved(my_context ctx);

    boost::statechart::result
    react(const RemoteBackfillReserved& evt)
    {
      /* no-op */
      return discard_event();
    }
  ...
  struct NotBackfilling : boost::statechart::state<NotBackfilling, Active>,
                          NamedState {
    typedef boost::mpl::list<
        boost::statechart::custom_reaction<QueryUnfound>,
  ...
  struct NotRecovering : boost::statechart::state<NotRecovering, Active>,
                         NamedState {
    typedef boost::mpl::list<
        boost::statechart::custom_reaction<QueryUnfound>,
        boost::statechart::transition<DoRecovery, WaitLocalRecoveryReserved>,
  ...
  struct ReplicaActive
    : boost::statechart::state<ReplicaActive, Started, RepNotRecovering>,
      NamedState {
    ...
    boost::statechart::result
    react(const RemoteBackfillPreempted& evt)
    {
      return discard_event();
    }

    boost::statechart::result
    react(const RemoteRecoveryPreempted& evt)
    {
      return discard_event();
    }
  };

  struct RepRecovering : boost::statechart::state<RepRecovering, ReplicaActive>,
                         NamedState {
    typedef boost::mpl::list<
        boost::statechart::transition<RecoveryDone, RepNotRecovering>,
        // for compat with old peers
        boost::statechart::
            transition<RemoteReservationRejectedTooFull, RepNotRecovering>,
  ...
  struct RepWaitBackfillReserved
    : boost::statechart::state<RepWaitBackfillReserved, ReplicaActive>,
      NamedState {
    typedef boost::mpl::list<
        boost::statechart::custom_reaction<RemoteBackfillReserved>,
  ...
  struct RepWaitRecoveryReserved
    : boost::statechart::state<RepWaitRecoveryReserved, ReplicaActive>,
      NamedState {
    typedef boost::mpl::list<
  ...
  struct RepNotRecovering
    : boost::statechart::state<RepNotRecovering, ReplicaActive>,
      NamedState {
    typedef boost::mpl::list<
        ...
        boost::statechart::transition<
            RecoveryDone,
            RepNotRecovering> // for compat with pre-reservation peers
        >
        reactions;
    ...
    boost::statechart::result
    react(const RemoteRecoveryReserved& evt)
    {
      // my reservation completion raced with a RELEASE from primary
      return discard_event();
    }
  ...
  struct WaitRemoteRecoveryReserved
    : boost::statechart::state<WaitRemoteRecoveryReserved, Active>,
      NamedState {
    typedef boost::mpl::list<
        boost::statechart::custom_reaction<RemoteRecoveryReserved>,
  ...
  struct WaitLocalRecoveryReserved
    : boost::statechart::state<WaitLocalRecoveryReserved, Active>,
      NamedState {
    typedef boost::mpl::list<
        boost::statechart::
            transition<LocalRecoveryReserved, WaitRemoteRecoveryReserved>,
  ...
  struct ToDelete
    : boost::statechart::state<ToDelete, Started, WaitDeleteReserved>,
      NamedState {
    unsigned priority = 0;
    typedef boost::mpl::list<
        boost::statechart::custom_reaction<ActMap>,
  ...
  struct WaitDeleteReserved
    : boost::statechart::state<WaitDeleteReserved, ToDelete>,
      NamedState {
    typedef boost::mpl::list<
  ...
  struct Incomplete : boost::statechart::state<Incomplete, Peering>,
                      NamedState {
    typedef boost::mpl::list<
        boost::statechart::custom_reaction<AdvMap>,
        boost::statechart::custom_reaction<MNotifyRec>,
  ...
  std::set<pg_shard_t>
      peer_log_requested; ///< logs i've requested (and start stamps)
  ...
  void
  update_osdmap_ref(OSDMapRef newmap)
  {
...
public:
  ...
  int
  get_recovery_op_priority() const
  {
...
private:
  ...
  void calculate_maxles_and_minlua(
  ...
  void proc_master_log(
      ObjectStore::Transaction& t,
      pg_info_t& oinfo,
      pg_log_t&& olog,
      pg_missing_t&& omissing,
      pg_shard_t from);
  void proc_replica_log(
      pg_info_t& oinfo,
      pg_log_t& olog,
      pg_missing_t&& omissing,
      pg_shard_t from);
...
public:
  ...
  /// Process evt
  void
  handle_event(const boost::statechart::event_base& evt, PeeringCtx* rctx)
  {
    start_handle(rctx);
  ...
  /// Process evt
  void
  handle_event(PGPeeringEventRef evt, PeeringCtx* rctx)
  {
    start_handle(rctx);
  ...
  void start_split_stats(
      const std::set<spg_t>& childpgs,
      std::vector<object_stat_sum_t>* out);
  ...
  void
  add_local_next_event(const pg_log_entry_t& e)
  {
  ...
  void
  object_recovered(const hobject_t& hoid, const object_stat_sum_t& stat_diff)
  {
  ...
  void
  set_last_requested(version_t v)
  {
  ...
  void
  local_write_applied(eversion_t v)
  {
  ...
  void
  force_write_state(ObjectStore::Transaction& t)
  {
    dirty_info = true;
    dirty_big_info = true;
    write_if_dirty(t);
  ...
  const std::set<int>&
  get_prior_readable_down_osds() const
  {
  ...
  void
  renew_lease(ceph::signedspan now)
  {
  ...
  bool
  is_ec_pg() const override
  {
    return pool.info.is_erasure();
  }

  int
  get_pg_size() const override
  {
  ...
  bool
  is_acting_recovery_backfill(pg_shard_t osd) const
  {
    return acting_recovery_backfill.count(osd);
  }

  bool
  is_acting(pg_shard_t osd) const
  {
    return has_shard(pool.info.is_erasure(), acting, osd);
  }

  bool
  is_up(pg_shard_t osd) const
  {
    return has_shard(pool.info.is_erasure(), up, osd);
  }

  static bool
  has_shard(bool ec, const std::vector<int>& v, pg_shard_t osd)
  {
    if (ec) {
      return v.size() > (unsigned)osd.shard &&
             v[static_cast<int>(osd.shard)] == osd.osd;
    } else {
  ...
  bool
  pg_has_reset_since(epoch_t e) const
  {
  ...
  int
  get_up_primary() const
  {
    return up_primary.osd;
  }

  bool
  is_backfill_target(pg_shard_t osd) const
  {
  ...
  const shard_id_set&
  get_backfill_target_shard_id_set() const
  {
    return backfill_target_shard_id_set;
  }

  bool
  is_async_recovery_target(pg_shard_t peer) const
  {
  ...
  const shard_id_set&
  get_acting_recovery_backfill_shard_id_set() const
  {
  ...
  bool
  state_test(uint64_t m) const
  {
    return (state & m) != 0;
  }

  void
  state_set(uint64_t m)
  {
    state |= m;
  }

  void
  state_clear(uint64_t m)
  {
    state &= ~m;
  }
  ...
  is_incomplete() const
  {
    return state_test(PG_STATE_INCOMPLETE);
  }
  ...
  is_remapped() const
  {
    return state_test(PG_STATE_REMAPPED);
  }
  ...
  is_recovering() const
  {
    return state_test(PG_STATE_RECOVERING);
  }

  bool
  is_premerge() const
  {
    return state_test(PG_STATE_PREMERGE);
  }

  bool
  is_repair() const
  {
    return state_test(PG_STATE_REPAIR);
  }
  ...
  get_peer_missing(const pg_shard_t& peer) const
  {
    if (peer == pg_whoami) {
  ...
  const pg_info_t&
  get_peer_info(pg_shard_t peer) const
  {
  ...
  bool
  has_peer_info(pg_shard_t peer) const
  {
  ...
  bool
  perform_deletes_during_peering() const
  {
  ...
  unsigned int
  get_num_missing() const
  {
    return pg_log.get_missing().num_missing();
  }

  bool
  is_missing_any_head_or_clone_of(const hobject_t& hoid)
  {
    const auto& missing = pg_log.get_missing();
  ...
  eversion_t
  get_pg_committed_to() const
  {
    return pg_committed_to;
  }

  bool
  debug_has_dirty_state() const
  {
  ...
  void
  print_past_intervals(std::ostream& out) const
  {
    out << "[" << past_intervals.get_bounds() << ")/" << past_intervals.size();
  }

  void
  dump_history(ceph::Formatter* f) const
  {
...
private:
  ...
  void
  apply_peer_features(uint64_t f)
  {
    peer_features &= f;
  }
...
public:
  ...
  uint64_t
  get_min_upacting_features() const
  {
  ...
  pg_feature_vec_t
  get_pg_acting_features() const
  {
    return pg_acting_features;
  }
...
private:
   ...
   */
  void
  start_flush(ObjectStore::Transaction& t)
  {
    flushes_in_progress++;
