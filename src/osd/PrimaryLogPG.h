...
class PrimaryLogPG : public PG, public PGBackend::Listener, public ECListener {
  friend class OSD;
  friend class Watch;
...
public:
  ...
  struct CopyResults {
    ...
    mempool::osd_pglog::vector<std::pair<osd_reqid_t, version_t>>
        reqids; // [(reqid, user_version)]
    mempool::osd_pglog::map<uint32_t, int>
        reqid_return_codes; // std::map reqids by index to error code
  ...
  /// Listener methods
  void
  add_temp_obj(const hobject_t& oid) override
  {
    get_pgbackend()->add_temp_obj(oid);
  }

  void
  clear_temp_obj(const hobject_t& oid) override
  {
    get_pgbackend()->clear_temp_obj(oid);
  }
  ...
  void
  begin_peer_recover(pg_shard_t peer, const hobject_t oid) override
  {
  ...
  void
  send_message(int to_osd, Message* m) override
  {
    osd->send_message_osd_cluster(to_osd, m, get_osdmap_epoch());
  }

  void
  queue_transaction(ObjectStore::Transaction&& t, OpRequestRef op) override
  {
  ...
  const shard_id_set&
  get_acting_recovery_backfill_shard_id_set() const override
  {
  ...
  get_shard_missing(pg_shard_t peer) const override
  {
  ...
  const std::map<pg_shard_t, pg_info_t>&
  ...
  const pg_info_t&
  get_shard_info(pg_shard_t peer) const override
  {
  ...
  void
  add_local_next_event(const pg_log_entry_t& e) override
  {
    recovery_state.add_local_next_event(e);
  }

  bool
  pgb_is_primary() const override
  {
    return is_primary();
  }
  ...
  eversion_t
  get_pg_committed_to() const override
  {
  ...
  bool
  try_lock_for_read(const hobject_t& hoid, ObcLockManager& manager) override
  {
  ...
  void
  release_locks(ObcLockManager& manager) override
  {
    release_object_locks(manager);
  }

  void
  inc_osd_stat_repaired() override
  {
    osd->inc_osd_stat_repaired();
  }

  bool
  pg_is_remote_backfilling() override
  {
    return is_remote_backfilling();
  }

  void
  pg_add_local_num_bytes(int64_t num_bytes) override
  {
    add_local_num_bytes(num_bytes);
  }

  void
  pg_sub_local_num_bytes(int64_t num_bytes) override
  {
    sub_local_num_bytes(num_bytes);
  }

  void
  pg_add_num_bytes(int64_t num_bytes) override
  {
    add_num_bytes(num_bytes);
  }

  void
  pg_sub_num_bytes(int64_t num_bytes) override
  {
    sub_num_bytes(num_bytes);
  }

  void
  pgb_set_object_snap_mapping(
      const hobject_t& soid,
      ...
      bool async = false) override
  {
    ...
    recovery_state.append_log(
        std::move(logv), trim_to, roll_forward_to, pg_committed_to, t,
        transaction_applied, async);
  }
  ...
  bool
  pg_is_undersized() const override
  {
    return is_undersized();
  }

  bool
  pg_is_repair() const override
  {
    return is_repair();
  }

  void
  update_peer_last_complete_ondisk(pg_shard_t fromosd, eversion_t lcod) override
  {
    recovery_state.update_peer_last_complete_ondisk(fromosd, lcod);
  }

  void
  update_last_complete_ondisk(eversion_t lcod) override
  {
    recovery_state.update_last_complete_ondisk(lcod);
  }

  void
  update_pct(eversion_t pct) override
  {
    recovery_state.update_pct(pct);
  }

  void
  update_stats(const pg_stat_t& stat) override
  {
  ...
  uint64_t
  min_peer_features() const override
  {
    return recovery_state.get_min_peer_features();
  }

  uint64_t
  min_upacting_features() const override
  {
    return recovery_state.get_min_upacting_features();
  }

  pg_feature_vec_t
  get_pg_acting_features() const override
  {
    return recovery_state.get_pg_acting_features();
  }

  void
  send_message_osd_cluster(int peer, Message* m, epoch_t from_epoch) override
  {
    osd->send_message_osd_cluster(peer, m, from_epoch);
  }

  void
  send_message_osd_cluster(int osd, MOSDPGPush* msg, epoch_t from_epoch) override
  {
  ...
  void
  send_message_osd_cluster(MessageRef m, Connection* con) override
  {
    osd->send_message_osd_cluster(std::move(m), con);
  }

  void
  send_message_osd_cluster(Message* m, const ConnectionRef& con) override
  {
  ...
  {
    osd->monc->start_mon_command(
        std::move(cmd), std::move(inbl), outbl, outs, onfinish);
  }
  ...
  struct OpContext {
    ...
    // side effects
    std::list<std::pair<watch_info_t, bool>>
        watch_connects; ///< new watch + will_ping flag
    ...
    std::list<std::pair<
        boost::tuple<uint64_t, uint64_t, unsigned>,
    ...
    void
    reset_obs(ObjectContextRef obc)
    {
    ...
    ~OpContext()
    {
      ...
      for (std::list<std::pair<
           ...
           i != pending_async_reads.end(); pending_async_reads.erase(i++)) {
...
protected:
  ...
  void hit_set_in_memory_trim(
      uint32_t max_in_memory); ///< discard old in memory HitSets
  void hit_set_remove_all();

  hobject_t get_hit_set_current_object(utime_t stamp);
  hobject_t get_hit_set_archive_object(
      utime_t start,
      utime_t end,
      bool using_gmt);
  ...
  bool
  agent_work(int max) override ///< entry point to do some agent work
  {
  ...
  void get_obc_watchers(
      ObjectContextRef obc,
      std::list<obj_watch_item_t>& pg_watchers);
...
protected:
  ...
  void
  register_snapset_context(SnapSetContext* ssc)
  {
  ...
  void
  _register_snapset_context(SnapSetContext* ssc)
  {
  ...
  dump_recovery_info(ceph::Formatter* f) const override
  {
    ...
    }
    {
      ...
      for (std::map<hobject_t, ObjectContextRef>::const_iterator i =
               recovering.begin();
           i != recovering.end(); ++i) {
  ...
  void write_update_size_and_usage(
  ...
  cache_result_t maybe_handle_cache_detail(
      OpRequestRef op,
      bool write_ordered,
      ObjectContextRef obc,
      int r,
  ...
  cache_result_t maybe_handle_manifest_detail(
      OpRequestRef op,
      bool write_ordered,
      ObjectContextRef obc);

  bool
  maybe_handle_manifest(OpRequestRef op, bool write_ordered, ObjectContextRef obc)
  {
    return cache_result_t::NOOP !=
           maybe_handle_manifest_detail(op, write_ordered, obc);
  }
      ...
      bool in_hit_set = false)
  {
    return cache_result_t::NOOP != maybe_handle_cache_detail(
                                       op, write_ordered, obc, r, missing_oid,
                                       must_promote, in_hit_set, nullptr);
  }
  ...
  void cancel_cls_gather(
      std::map<hobject_t, CLSGatherOp>::iterator iter,
      bool requeue,
  ...
  int do_checksum(
      OpContext* ctx,
      OSDOp& osd_op,
      ceph::buffer::list::const_iterator* bl_it);
  int finish_checksum(
      OSDOp& osd_op,
  ...
  void do_proxy_chunked_op(
      OpRequestRef op,
      const hobject_t& missing_oid,
      ObjectContextRef obc,
      bool write_ordered);
  void do_proxy_chunked_read(
      OpRequestRef op,
      ObjectContextRef obc,
      int op_index,
  ...
  void _copy_some_manifest(
      ObjectContextRef obc,
      CopyOpRef cop,
      uint64_t start_offset);
  void process_copy_chunk_manifest(
      hobject_t oid,
      ceph_tid_t tid,
      int r,
      uint64_t offset);
  ...
  std::pair<int, hobject_t> get_fpoid_from_chunk(
      const hobject_t soid,
      bufferlist& chunk);
  int finish_set_dedup(hobject_t oid, int r, ceph_tid_t tid, uint64_t offset);
  int finish_set_manifest_refcount(
      hobject_t oid,
      int r,
      ceph_tid_t tid,
      uint64_t offset);
...
private:
  ...
  struct SnapTrimmer
    : public boost::statechart::state_machine<SnapTrimmer, NotTrimming> {
    PrimaryLogPG* pg;
  ...
  struct Trimming
    : boost::statechart::state<Trimming, SnapTrimmer, WaitReservation>,
      NamedState {
    ...
    explicit Trimming(my_context ctx) :
      my_base(ctx), NamedState(nullptr, "Trimming")
    {
  ...
  /* SnapTrimmerStates */
  struct WaitTrimTimer : boost::statechart::state<WaitTrimTimer, Trimming>,
                         NamedState {
    typedef boost::mpl::list<
    ...
    explicit WaitTrimTimer(my_context ctx) :
      my_base(ctx), NamedState(nullptr, "Trimming/WaitTrimTimer")
    {
      context<SnapTrimmer>().log_enter(state_name);
      ...
      if (osd_snap_trim_sleep > 0) {
        ...
        wakeup = pg->osd->sleep_timer.add_event_after(
            osd_snap_trim_sleep, new OnTimer{pg, pg->get_osdmap_epoch()});
      } else {
  ...
  struct WaitRWLock : boost::statechart::state<WaitRWLock, Trimming>,
                      NamedState {
    typedef boost::mpl::list<
    ...
    explicit WaitRWLock(my_context ctx) :
      my_base(ctx), NamedState(nullptr, "Trimming/WaitRWLock")
    {
      context<SnapTrimmer>().log_enter(state_name);
      ceph_assert(context<Trimming>().in_flight.empty());
    }
  ...
  struct WaitRepops : boost::statechart::state<WaitRepops, Trimming>,
                      NamedState {
    typedef boost::mpl::list<boost::statechart::custom_reaction<RepopsComplete>>
        reactions;

    explicit WaitRepops(my_context ctx) :
      my_base(ctx), NamedState(nullptr, "Trimming/WaitRepops")
    {
      context<SnapTrimmer>().log_enter(state_name);
      ceph_assert(!context<Trimming>().in_flight.empty());
    }
  ...
  struct AwaitAsyncWork : boost::statechart::state<AwaitAsyncWork, Trimming>,
                          NamedState {
    typedef boost::mpl::list<boost::statechart::custom_reaction<DoSnapWork>>
        reactions;
  ...
  struct WaitReservation : boost::statechart::state<WaitReservation, Trimming>,
                           NamedState {
    /* WaitReservation is a sub-state of trimming simply so that exiting Trimming
     * always cancels the reservation */
    typedef boost::mpl::list<boost::statechart::custom_reaction<SnapTrimReserved>>
        reactions;
    ...
    explicit WaitReservation(my_context ctx) :
      my_base(ctx), NamedState(nullptr, "Trimming/WaitReservation")
    {
      context<SnapTrimmer>().log_enter(state_name);
      ceph_assert(context<Trimming>().in_flight.empty());
      auto* pg = context<SnapTrimmer>().pg;
  ...
  struct WaitScrub : boost::statechart::state<WaitScrub, SnapTrimmer>,
                     NamedState {
    typedef boost::mpl::list<
    ...
    explicit WaitScrub(my_context ctx) :
      my_base(ctx), NamedState(nullptr, "WaitScrub")
    {
      context<SnapTrimmer>().log_enter(state_name);
    }
  ...
  struct NotTrimming : boost::statechart::state<NotTrimming, SnapTrimmer>,
                       NamedState {
    typedef boost::mpl::list<
        boost::statechart::custom_reaction<KickTrim>,
        boost::statechart::transition<Reset, NotTrimming>>
...
public:
  ...
  bool
  is_unreadable_object(const hobject_t& oid) const
  {
  ...
  void mark_all_unfound_lost(int what, asok_finisher on_finish);
  ...
  const pg_missing_const_i*
  maybe_get_shard_missing(pg_shard_t peer) const
  {
    if (peer == primary_shard()) {
...
inline ostream&
operator<<(ostream& out, const PrimaryLogPG::RepGather& repop)
{
  out << "repgather(" << &repop << " " << repop.v
      << " rep_tid=" << repop.rep_tid << " committed?=" << repop.all_committed
      << " r=" << repop.r << ")";
  return out;
}

inline ostream&
operator<<(ostream& out, const PrimaryLogPG::ProxyWriteOpRef& pwop)
{
  out << "proxywrite(" << &pwop << " " << pwop->user_version
