...
static ostream&
_prefix(std::ostream* _dout, ReplicatedBackend* pgb)
{
...
static void
log_subop_stats(PerfCounters* logger, OpRequestRef op, int subop)
{
...
ReplicatedBackend::ReplicatedBackend(
    ...
    ObjectStore* store,
    CephContext* cct) :
  PGBackend(cct, pg, store, coll, c), pct_callback(this)
{}

void
ReplicatedBackend::run_recovery_op(PGBackend::RecoveryHandle* _h, int priority)
{
...
void
ReplicatedBackend::check_recovery_sources(const OSDMapRef& osdmap)
{
...
bool
ReplicatedBackend::can_handle_while_inactive(OpRequestRef op)
{
...
bool
ReplicatedBackend::_handle_message(OpRequestRef op)
{
  dout(10) << __func__ << ": " << *op->get_req() << dendl;
  switch (op->get_req()->get_type()) {
...
void
ReplicatedBackend::clear_recovery_state()
{
...
bool
ReplicatedBackend::get_ec_supports_crc_encode_decode() const
{
...
bool
ReplicatedBackend::ec_can_decode(const shard_id_set& available_shards) const
{
...
ReplicatedBackend::ec_encode_acting_set(const bufferlist& in_bl) const
{
...
shard_id_map<bufferlist>
ReplicatedBackend::ec_decode_acting_set(
...
ReplicatedBackend::ec_get_sinfo() const
{
  ...
  pgt->safe_create_traverse(
      [&](pair<const hobject_t, PGTransaction::ObjectOperation>& obj_op) {
        ...
        if (op.alloc_hint) {
          ...
          t->set_alloc_hint(
              coll, goid, hint.expected_object_size, hint.expected_write_size,
              hint.flags);
        }
        ...
        for (auto&& extent : op.buffer_updates) {
          ...
          match(
              ...
              [&](const BufferUpdate::Write& op) {
                t->write(
                    coll, goid, extent.get_off(), extent.get_len(), op.buffer,
                    op.fadvise_flags);
              },
              ...
              [&](const BufferUpdate::CloneRange& op) {
                ...
                t->clone_range(
                    coll,
                    ghobject_t(
                        op.from, ghobject_t::NO_GEN, shard_id_t::NO_SHARD),
                    goid, op.offset, extent.get_len(), extent.get_off());
              });
        }
      });
}
...
void
ReplicatedBackend::do_pct(OpRequestRef op)
{
  const MOSDPGPCT* m = static_cast<const MOSDPGPCT*>(op->get_req());
  dout(10) << __func__ << ": received pct update to " << m->pg_committed_to
           << dendl;
  parent->update_pct(m->pg_committed_to);
}

void
ReplicatedBackend::send_pct_update()
{
  dout(10) << __func__ << ": sending pct update" << dendl;
  ceph_assert(PG_HAVE_FEATURE(parent->get_pg_acting_features(), PCT));
  for (const auto& i : parent->get_acting_shards()) {
    if (i == parent->whoami_shard())
      continue;

    auto* pct_update = new MOSDPGPCT(
        spg_t(parent->whoami_spg_t().pgid, i.shard), get_osdmap_epoch(),
        parent->get_interval_start_epoch(), parent->get_pg_committed_to());

    dout(10) << __func__ << ": sending pct update to i " << i << ", i.osd "
             << i.osd << dendl;
...
void
ReplicatedBackend::maybe_kick_pct_update()
{
  ...
  int64_t pct_delay;
  if (!parent->get_pool().opts.get(pool_opts_t::PCT_UPDATE_DELAY, &pct_delay)) {
    dout(20) << __func__ << ": not scheduling pct update, PCT_UPDATE_DELAY not"
             << " set" << dendl;
    return;
  }

  dout(10) << __func__ << ": scheduling pct update after " << pct_delay
           << " seconds" << dendl;
...
void
ReplicatedBackend::cancel_pct_update()
{
  if (pct_callback.is_scheduled()) {
    ...
    OpRequestRef orig_op)
{
  ...
  generate_transaction(
      t, coll, log_entries, &op_t, &added, &removed,
      get_osdmap()->require_osd_release);
  ceph_assert(added.size() <= 1);
  ceph_assert(removed.size() <= 1);

  auto insert_res = in_progress_ops.insert(make_pair(
      tid,
      ceph::make_ref<InProgressOp>(tid, on_all_commit, orig_op, at_version)));
  ...
  issue_op(
      soid, at_version, tid, reqid, trim_to, pg_committed_to,
  ...
  parent->log_operation(
      std::move(log_entries), hset_history, trim_to, at_version,
      pg_committed_to, true, op_t);
...
void
ReplicatedBackend::op_commit(const ceph::ref_t<InProgressOp>& op)
{
  ...
  OID_EVENT_TRACE_WITH_MSG(
      (op && op->op) ? op->op->get_req() : NULL, "OP_COMMIT_BEGIN", true);
...
void
ReplicatedBackend::do_repop_reply(OpRequestRef op)
{
  ...
  if (iter != in_progress_ops.end()) {
    ...
    parent->update_peer_last_complete_ondisk(
        from, r->get_last_complete_ondisk());
...
static uint32_t
crc32_netstring(const uint32_t orig_crc, std::string_view data)
{
...
ReplicatedBackend::be_deep_scrub_read_data(
    ...
    ScrubMap::object& smap_object)
{
  ...
  dout(20) << __func__ << " done with " << poid << " omap_digest " << std::hex
           << smap_object.omap_digest << std::dec << dendl;
...
void
ReplicatedBackend::_do_push(OpRequestRef op)
{
...
struct C_ReplicatedBackend_OnPullComplete : GenContext<ThreadPool::TPHandle&> {
  ...
  void
  finish(ThreadPool::TPHandle& handle) override
  {
    ...
    for (auto&& i : to_continue) {
      ...
      if (started < 0) {
        ...
        bc->get_parent()->on_failed_pull(
            {bc->get_parent()->whoami_shard()}, i.hoid, obc->obs.oi.version);
      } else if (!started) {
...
void
ReplicatedBackend::_do_pull_response(OpRequestRef op)
{
  auto m = op->get_req<MOSDPGPush>();
  ceph_assert(m->get_type() == MSG_OSD_PG_PUSH);
  pg_shard_t from = m->from;
  ...
  if (!to_continue.empty()) {
    C_ReplicatedBackend_OnPullComplete* c =
        new C_ReplicatedBackend_OnPullComplete(
            this, m->get_priority(), std::move(to_continue));
...
void
ReplicatedBackend::do_pull(OpRequestRef op)
{
...
void
ReplicatedBackend::do_push_reply(OpRequestRef op)
{
    ...
    ObjectStore::Transaction& op_t)
{
  if (parent->get_acting_recovery_backfill_shards().size() > 1) {
    ...
    for (const auto& shard :
      ...
      Message* wr;
      wr = generate_subop(
          soid, at_version, tid, reqid, pg_trim_to, pg_committed_to,
          new_temp_oid, discard_temp_oid, logs, hset_hist, op_t, shard, pinfo);
...
// sub op modify
void
ReplicatedBackend::do_repop(OpRequestRef op)
{
  ...
  RepModifyRef rm(
      std::make_shared<RepModify>(get_parent()->min_peer_features()));
  rm->op = op;
  ...
  parent->log_operation(
      std::move(log), m->updated_hit_set_history, m->pg_trim_to,
      m->version, /* Replicated PGs don't have rollback info */
      m->pg_committed_to, update_snaps, rm->localt, async);
...
void
ReplicatedBackend::repop_commit(RepModifyRef rm)
{
  ...
  MOSDRepOpReply* reply = new MOSDRepOpReply(
      m, get_parent()->whoami_shard(), 0, get_osdmap_epoch(),
...
void
ReplicatedBackend::calc_head_subsets(
    ObjectContextRef obc,
    SnapSet& snapset,
    ...
    ObcLockManager& manager)
{
  ...
  if (cloning.num_intervals() >
      g_conf().get_val<uint64_t>("osd_recover_clone_overlap_limit")) {
...
void
ReplicatedBackend::calc_clone_subsets(
    SnapSet& snapset,
    const hobject_t& soid,
    const pg_missing_t& missing,
    const hobject_t& last_backfill,
    ...
    RPGHandle* h)
{
  ...
  ceph_assert(
      missing_iter != get_parent()->get_local_missing().get_items().end());
  ...
  if (p == q->second.end()) {
    ...
    p = q->second.begin();
    std::advance(
        p, ceph::util::generate_random_number<int>(0, q->second.size() - 1));
  }
  ...
  if (pmissing.is_missing(soid, v)) {
    ...
    ceph_assert(
        ...
        (get_parent()->get_log().get_log().objects.find(soid)->second->reverting_to ==
         v));
  }
    ...
    bool cache_dont_need)
{
  ...
  if (soid.snap && soid.snap < CEPH_NOSNAP) {
    ...
    calc_clone_subsets(
        ssc->snapset, soid, pm->second, pi->second.last_backfill, data_subset,
        clone_subsets, lock_manager);
  ...
  return prep_push(
      obc, soid, peer, oi.version, data_subset, clone_subsets, pop,
    ...
    bool cache_dont_need)
{
  ...
  return prep_push(
      obc, soid, peer, obc->obs.oi.version, data_subset, clone_subsets, pop,
      cache_dont_need, ObcLockManager());
}
    ...
    ObcLockManager&& lock_manager)
{
  ...
  int r = build_push_op(
      push_info.recovery_info, push_info.recovery_progress, &new_progress, pop,
    ...
    ObjectStore::Transaction* t)
{
  ...
  if (first && complete) {
    ...
    target_oid = get_parent()->get_temp_recovery_object(
  ...
  if (first) {
    if (!complete) {
      ...
      t->set_alloc_hint(
          coll, ghobject_t(target_oid), oi.expected_object_size,
    ...
    if (get_parent()->pg_is_remote_backfilling()) {
      ...
      if (size != recovery_info.oi.size) {
        get_parent()->pg_add_local_num_bytes(
            (int64_t)recovery_info.oi.size - (int64_t)size);
        get_parent()->pg_add_num_bytes(
            (int64_t)recovery_info.oi.size - (int64_t)size);
    ...
    if (!complete) {
      ...
      if (recovery_info.object_exist) {
        ...
        for (interval_set<uint64_t>::const_iterator q =
          ...
          t->clone_range(
              coll, ghobject_t(recovery_info.soid), ghobject_t(target_oid),
  ...
  for (interval_set<uint64_t>::const_iterator p = intervals_included.begin();
    ...
    t->write(
        coll, ghobject_t(target_oid), p.get_start(), p.get_len(), bit,
        fadvise_flags);
    off += p.get_len();
  }
  ...
  if (complete) {
    if (!first) {
      ...
      t->collection_move_rename(
          coll, ghobject_t(target_oid), coll, ghobject_t(recovery_info.soid));
    }
...
void
ReplicatedBackend::submit_push_complete(
    const ObjectRecoveryInfo& recovery_info,
    ObjectStore::Transaction* t)
{
  for (map<hobject_t, interval_set<uint64_t>>::const_iterator p =
    ...
    for (interval_set<uint64_t>::const_iterator q = p->second.begin();
      ...
      dout(15) << " clone_range " << p->first << " " << q.get_start() << "~"
               << q.get_len() << dendl;
      t->clone_range(
          coll, ghobject_t(p->first), ghobject_t(recovery_info.soid),
          q.get_start(), q.get_len(), q.get_start());
    }
  }
}
    ...
    ObcLockManager& manager)
{
  ...
  calc_clone_subsets(
      ...
      get_info().last_backfill, new_info.copy_subset, new_info.clone_subset,
      manager);
  return new_info;
}
    ...
    ObjectStore::Transaction* t)
{
  ...
  if (first) {
    ...
    pull_info.recovery_info = recalc_subsets(
        pull_info.recovery_info, pull_info.obc->ssc, pull_info.lock_manager);
  }
  ...
  trim_pushed_data(
      pull_info.recovery_info.copy_subset, data_included, data,
  ...
  dout(10) << "new recovery_info " << pull_info.recovery_info
           << ", new progress " << pull_info.recovery_progress << dendl;
  interval_set<uint64_t> data_zeros;
  ...
  submit_push_data(
      ...
      pull_info.cache_dont_need, data_zeros, data_included, data,
  ...
  get_parent()->get_logger()->inc(
      l_osd_rbytes, pop.omap_entries.size() + data.length());
...
void
ReplicatedBackend::send_pushes(int prio, map<pg_shard_t, vector<PushOp>>& pushes)
{
...
void
ReplicatedBackend::send_pulls(int prio, map<pg_shard_t, vector<PullOp>>& pulls)
{
...
static bufferlist
to_bufferlist(std::string_view in)
{
  bufferlist bl;
  bl.append(in);
  return bl;
    ...
    bool cache_dont_need)
{
  ...
  if (progress.first) {
    int r = store->omap_get_header(
        ch, ghobject_t(recovery_info.soid), &out_op->omap_header);
    if (r < 0) {
  ...
  if (!progress.omap_complete) {
    using omap_iter_seek_t = ObjectStore::omap_iter_seek_t;
    auto result = store->omap_iterate(
        ch, ghobject_t{recovery_info.soid},
        // try to seek as many keys-at-once as possible for the sake of performance.
        // note complexity should be logarithmic, so seek(n/2) + seek(n/2) is worse
        // than just seek(n).
        ObjectStore::omap_iter_seek_t{
  ...
  if (available > 0) {
    if (!recovery_info.copy_subset.empty()) {
      ...
      map<uint64_t, uint64_t> m;
      int r = store->fiemap(
          ch, ghobject_t(recovery_info.soid), 0, copy_subset.range_end(), m);
      if (r >= 0) {
      ...
      out_op->data_included.span_of(
          copy_subset, progress.data_recovered_to, available);
  ...
  bufferlist bit;
  int r = store->readv(
      ch, ghobject_t(recovery_info.soid), out_op->data_included, bit,
  ...
  if (stat) {
    ...
    get_parent()->get_logger()->inc(
        l_osd_rbytes, out_op->omap_entries.size() + out_op->data.length());
  }
...
void
ReplicatedBackend::prep_push_op_blank(const hobject_t& soid, PushOp* op)
{
  ...
  if (pushing.count(soid) == 0) {
    ...
    if (!push_info->recovery_progress.data_complete && !error) {
      dout(10) << " pushing more from, "
               << push_info->recovery_progress.data_recovered_to << " of "
               << push_info->recovery_info.copy_subset << dendl;
      ObjectRecoveryProgress new_progress;
      int r = build_push_op(
          push_info->recovery_info, push_info->recovery_progress, &new_progress,
          reply, &(push_info->stat));
...
void
ReplicatedBackend::handle_pull(pg_shard_t peer, PullOp& op, PushOp* reply)
{
  const hobject_t& soid = op.soid;
  struct stat st;
 ...
 */
void
ReplicatedBackend::trim_pushed_data(
...
void
ReplicatedBackend::_failed_pull(pg_shard_t from, const hobject_t& soid)
{
...
void
ReplicatedBackend::clear_pull_from(map<hobject_t, pull_info_t>::iterator piter)
{
