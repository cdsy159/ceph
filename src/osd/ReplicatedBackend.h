...
private:
  ...
  void clear_pull_from(std::map<hobject_t, pull_info_t>::iterator piter);

  void _do_push(OpRequestRef op);
  void _do_pull_response(OpRequestRef op);

  void
  do_push(OpRequestRef op)
  {
    if (is_primary()) {
  ...
  private:
    ...
    InProgressOp(
        ...
        OpRequestRef op,
        eversion_t v) :
      tid(tid), on_commit(on_commit), op(op), v(v)
    {}
  };
  ...
  struct pct_callback_t final : public common::intrusive_timer::callback_t {
    ...
    pct_callback_t(ReplicatedBackend* backend) :
      backend(backend)
    {}
...
public:
  ...
  void
  call_write_ordered(std::function<void(void)>&& cb) override
  {
...
private:
  ...
  struct RepModify {
    ...
    RepModify(uint64_t features) :
      committed(false), ackerosd(-1), epoch_started(0), localt(features)
    {}
  };
  ...
  uint64_t
  be_get_ondisk_size(uint64_t logical_size, shard_id_t unused, bool unused2)
      const final
  {
    return logical_size;
  }
