    ...
    std::pair<std::string, ceph::buffer::list>* next)
{
  ...
  ObjectStore::omap_iter_seek_t start_from{
      key, ObjectStore::omap_iter_seek_t::UPPER_BOUND};
  std::function<ObjectStore::omap_iter_ret_t(std::string_view, std::string_view)>
      callback = [FNAME, key,
                  next](std::string_view _key, std::string_view _value) {
        ...
        if (!SnapMapper::is_mapping(std::string(_key))) {
          ...
          ceph_assertf(
              _key > key, "Key order violation: input_key='%s' got_key='%s'",
    ...
    std::pair<std::string, ceph::buffer::list>* next_or_current)
{
  ...
  return interruptor::green_get(
      crimson::os::with_store<
          &crimson::os::FuturizedStore::Shard::omap_get_values>(
          os, ch, hoid, FuturizedStore::Shard::omap_keys_t{key}, 0)
          .safe_then(
              [FNAME, &key,
               next_or_current](FuturizedStore::Shard::omap_values_t&& vals) {
                DEBUG("returning {}", key);
                ceph_assert(vals.size() == 1);
                *next_or_current =
                    std::make_pair(key, std::move(vals.begin()->second));
                return 0;
              },
    ...
    std::pair<std::string, ceph::buffer::list>* next)
{
  ...
  const auto result = os->omap_iterate(
      ...
      ObjectStore::omap_iter_seek_t{
          .seek_position = seek_key, .seek_type = omap_iter_seek_t::UPPER_BOUND},
    ...
    std::pair<std::string, ceph::buffer::list>* next_or_current)
{
  ...
  const auto result = os->omap_iterate(
      ch, hoid,
      ObjectStore::omap_iter_seek_t{
          .seek_position = seek_key, .seek_type = omap_iter_seek_t::LOWER_BOUND},
...
string
SnapMapper::get_prefix(int64_t pool, snapid_t snap)
{
  ...
  return fmt::sprintf(
      "%s%lld_%.16X_", MAPPING_PREFIX, pool, static_cast<uint64_t>(snap));
}

string
SnapMapper::to_raw_key(const pair<snapid_t, hobject_t>& in) const
{
...
std::string
SnapMapper::to_raw_key(snapid_t snap, const hobject_t& clone) const
{
  return get_prefix(clone.pool, snap) + shard_prefix + clone.to_str();
}

pair<string, ceph::buffer::list>
SnapMapper::to_raw(const pair<snapid_t, hobject_t>& in) const
{
  ceph::buffer::list bl;
...
SnapMapper::from_raw(const pair<std::string, ceph::buffer::list>& image)
{
  using ceph::decode;
  Mapping map;
...
std::pair<snapid_t, hobject_t>
SnapMapper::from_raw(const ceph::buffer::list& image)
{
  using ceph::decode;
  Mapping map;
...
bool
SnapMapper::is_mapping(const string& to_test)
{
  return to_test.compare(0, MAPPING_PREFIX.size(), MAPPING_PREFIX) == 0;
}

string
SnapMapper::to_object_key(const hobject_t& hoid) const
{
  return OBJECT_PREFIX + shard_prefix + hoid.to_str();
}

void
SnapMapper::object_snaps::encode(ceph::buffer::list& bl) const
{
  ENCODE_START(1, 1, bl);
  encode(oid, bl);
  encode(snaps, bl);
...
void
SnapMapper::object_snaps::decode(ceph::buffer::list::const_iterator& bl)
{
  DECODE_START(1, bl);
  decode(oid, bl);
  decode(snaps, bl);
...
void
SnapMapper::object_snaps::dump(ceph::Formatter* f) const
{
...
auto
SnapMapper::object_snaps::generate_test_instances() -> std::list<object_snaps>
{
...
bool
SnapMapper::check(const hobject_t& hoid) const
{
...
int
SnapMapper::get_snaps(const hobject_t& oid, object_snaps* out) const
{
...
void
SnapMapper::update_bits(uint32_t new_bits) ///< [in] new split bits
{
  dout(20) << *this << __func__ << " new_bits: " << new_bits << dendl;
  mask_bits = new_bits;
  std::set<std::string> _prefixes =
      hobject_t::get_prefixes(mask_bits, match, pool);
  prefixes.clear();
...
SnapMapper::update_snaps(
    ...
    MapCacher::Transaction<std::string, ceph::buffer::list>* t)
{
  dout(20) << *this << __func__ << " " << oid << " " << new_snaps << " was "
...
void
SnapMapper::reset_prefix_itr(snapid_t snap, const char* s)
{
  if (prefix_itr_snap == CEPH_NOSNAP) {
    dout(10) << *this << __func__ << "::from <CEPH_NOSNAP> to <" << snap
             << "> ::" << s << dendl;
  } else if (snap == CEPH_NOSNAP) {
    dout(10) << *this << __func__ << "::from <" << prefix_itr_snap
             << "> to <CEPH_NOSNAP> ::" << s << dendl;
  } else if (prefix_itr_snap == snap) {
    dout(10) << *this << __func__ << "::with the same snapid <" << snap
             << "> ::" << s << dendl;
  } else {
    // This is unexpected!!
    dout(10) << *this << __func__ << "::from <" << prefix_itr_snap << "> to <"
             << snap << "> ::" << s << dendl;
  }
...
vector<hobject_t>
SnapMapper::get_objects_by_prefixes(snapid_t snap, unsigned max)
{
  ...
  for (; prefix_itr != prefixes.end(); prefix_itr++) {
    ...
    while (out.size() < max) {
      ...
      if (next.first.compare(0, prefix.size(), prefix) != 0) {
        ...
        dout(20) << fmt::format(
                        "{}: breaking, prefix expected {} got key {} with a "
                        "different prefix",
    ...
    if (out.size() >= max) {
      dout(20) << *this
               << fmt::format(
                      "{}: reached max of: {} returning", __func__, out.size())
               << dendl;
      return out;
    }
  }
...
std::optional<vector<hobject_t>>
SnapMapper::get_next_objects_to_trim(snapid_t snap, unsigned max)
{
  ...
  if (unlikely(objs.size() == 0)) {
    ...
    if (unlikely(objs.size() > 0)) {
      derr << *this << __func__ << " New Clone-Objects were added to Snap "
           << snap << " after trimming was started" << dendl;
    }
...
int
SnapMapper::get_snaps(const hobject_t& oid, std::set<snapid_t>* snaps) const
{
  ceph_assert(check(oid));
  object_snaps out;
  int r = get_snaps(oid, &out);
    ...
    MapCacher::Transaction<std::string, ceph::buffer::list>* _t)
{
  ...
  if (i.is_delete()) {
    int r = remove_oid(i.soid, _t);
    if (r)
      dout(20) << *this << __func__ << " remove_oid " << i.soid
               << " failed with " << r << dendl;
    ...
    try {
      ...
      dout(20) << *this << __func__ << " decode snaps failure on " << i
               << dendl;
      snaps.clear();
    }
    ...
    if (i.is_clone() || i.is_promote()) {
      add_oid(i.soid, _snaps, _t);
    } else if (i.is_modify()) {
      int r = update_snaps(i.soid, _snaps, 0, _t);
      ceph_assert(r == 0);
    } else {
...
string
SnapMapper::make_purged_snap_key(int64_t pool, snapid_t last)
{
  ceph_assert(last != CEPH_NOSNAP && last != CEPH_SNAPDIR);
  return fmt::sprintf(
      "%s_%lld_%016llx", PURGED_SNAP_PREFIX, pool, static_cast<uint64_t>(last));
}

void
SnapMapper::make_purged_snap_key_value(
    int64_t pool,
    snapid_t begin,
    snapid_t end,
...
int
SnapMapper::_lookup_purged_snap(
    CephContext* cct,
    OSDriver& backend,
    int64_t pool,
    snapid_t snap,
    ...
    snapid_t* end)
{
  ...
  if (kv.first.find(PURGED_SNAP_PREFIX) != 0) {
    dout(20) << __func__ << " pool " << pool << " snap " << snap << " key '"
             << k << "' lower_bound got mismatched prefix '" << kv.first << "'"
             << dendl;
    return -ENOENT;
  }
  ...
  for (auto& [epoch, bypool] : purged_snaps) {
    ...
    for (auto& [pool, snaps] : bypool) {
      for (auto i = snaps.begin(); i != snaps.end(); ++i) {
        ...
        int b = _lookup_purged_snap(
            cct, backend, pool, begin - 1, &before_begin, &before_end);
        int a = _lookup_purged_snap(
            cct, backend, pool, end, &after_begin, &after_end);
        if (!b && !a) {
...
#ifndef WITH_CRIMSON
bool
SnapMapper::Scrubber::_parse_p(std::string_view key, std::string_view value)
{
...
bool
SnapMapper::Scrubber::_parse_m(std::string_view key, std::string_view value)
{
  ...
  store->omap_iterate(
      ...
      [this](std::string_view key, std::string_view value) mutable {
        ...
        if (ret < 0) {
          ...
          pool = -1;
          derr << "omap_iterate() on purged_snaps_hoid returns " << ret
               << dendl;
        ...
        if (mapping.hoid.pool < pool || mapping.snap < begin) {
          ...
          // invalid
          dout(10) << fmt::format(
                          "{} stray {} snap {} in pool {} shard {} "
                          "purged_snaps[{}, {})",
                          __func__, mapping.hoid, mapping.snap, pool, shard,
                          begin, end)
                   << dendl;
