...
class OSDriver
  : public MapCacher::StoreDriver<std::string, ceph::buffer::list> {
#ifdef WITH_CRIMSON
...
public:
  class OSTransaction
    : public MapCacher::Transaction<std::string, ceph::buffer::list> {
  ...
  public:
    void
    set_keys(const std::map<std::string, ceph::buffer::list>& to_set) override
    {
      t->omap_setkeys(cid, hoid, to_set);
    }

    void
    remove_keys(const std::set<std::string>& to_remove) override
    {
  ...
  OSTransaction
  get_transaction(ceph::os::Transaction* t) const
  {
...
private:
  ...
  std::vector<hobject_t> get_objects_by_prefixes(snapid_t snap, unsigned max);
...
inline std::ostream&
operator<<(std::ostream& os, const SnapMapper& sm)
{
  os << fmt::format(
      " [pg_id:{:x}, match:{}, mask_bits:{}, "
      "last_key_checked:{}, pool:{}, shard:{}, "
      "shard_prefix: {}, prefixes: {}] ",
      sm.match, sm.match, sm.mask_bits, sm.last_key_checked, sm.pool,
      sm.shard.id, sm.shard_prefix, sm.prefixes);
  return os;
}
