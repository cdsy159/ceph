...
struct RWState {
  ...
  bool
  get_write_lock(bool greedy = false)
  {
    if (!greedy) {
      // don't starve anybody!
...
inline std::ostream&
operator<<(std::ostream& out, const RWState& rw)
{
  return out << "rwstate(" << rw.get_state_name() << " n=" << rw.count
             << " w=" << rw.waiters << ")";
}
