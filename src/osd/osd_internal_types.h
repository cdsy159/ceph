...
inline std::ostream&
operator<<(std::ostream& out, const SnapSetContext& ssc)
{
  return out << "ssc(" << ssc.oid << " snapset: " << ssc.snapset
             << " ref: " << ssc.ref << " registered: " << ssc.registered
             << " exists: " << ssc.exists << ")";
}
...
public:
  ...
  bool
  get_write(OpRequestRef& op, bool greedy = false)
  {
  ...
  void
  wake(std::list<OpRequestRef>* requeue)
  {
  ...
  void
  put_read(std::list<OpRequestRef>* requeue)
  {
    if (rwstate.put_read()) {
      wake(requeue);
    }
  ...
  void
  put_write(std::list<OpRequestRef>* requeue)
  {
  ...
  void
  put_excl(std::list<OpRequestRef>* requeue)
  {
    if (rwstate.put_excl()) {
      wake(requeue);
    }
  ...
  bool
  get_lock_type(OpRequestRef& op, RWState::State type)
  {
    switch (type) {
  ...
  bool
  get_write_greedy(OpRequestRef& op)
  {
    return get_write(op, true);
  }

  bool
  get_snaptrimmer_write(bool mark_if_unsuccessful)
  {
  ...
  void
  drop_recovery_read(std::list<OpRequestRef>* ls)
  {
...
inline std::ostream&
operator<<(std::ostream& out, const ObjectState& obs)
{
  out << obs.oi.soid;
  if (!obs.exists)
    out << "(dne)";
...
inline std::ostream&
operator<<(std::ostream& out, const ObjectContext& obc)
{
...
public:
  ...
  bool
  take_write_lock(const hobject_t& hoid, ObjectContextRef obc)
  {
    ceph_assert(locks.find(hoid) == locks.end());
  ...
  bool
  get_write_greedy(const hobject_t& hoid, ObjectContextRef obc, OpRequestRef op)
  {
    ceph_assert(locks.find(hoid) == locks.end());
  ...
  /// try get read lock
  bool
  try_get_read_lock(const hobject_t& hoid, ObjectContextRef obc)
  {
    ceph_assert(locks.find(hoid) == locks.end());
      ...
      bool* requeue_snaptrimmer)
  {
    for (auto& p : locks) {
      ...
      p.second.obc->put_lock_type(
          p.second.type, &_to_requeue, requeue_recovery, requeue_snaptrimmer);
      if (to_requeue) {
        // We can safely std::move here as the whole `locks` is going
        // to die just after the loop.
        to_requeue->emplace_back(
            std::move(p.second.obc), std::move(_to_requeue));
      }
    }
    locks.clear();
  }
