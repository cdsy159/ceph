    ...
    unsigned op_queue_cut_off)
{
  ...
  if (op_queue_type_t::WeightedPriorityQueue == osd_scheduler ||
    ...
    return std::make_unique<
        ...
        cct, op_queue_cut_off, cct->_conf->osd_op_pq_max_tokens_per_priority,
    ...
    return std::make_unique<mClockScheduler>(
        cct, whoami, num_shards, shard_id, is_rotational, op_queue_cut_off);
  } else {
...
std::ostream&
operator<<(std::ostream& lhs, const OpScheduler& rhs)
{
  rhs.print(lhs);
  return lhs;
}
