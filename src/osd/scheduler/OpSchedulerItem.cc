...
void
PGOpItem::run(OSD* osd, OSDShard* sdata, PGRef& pg, ThreadPool::TPHandle& handle)
{
...
void
PGScrub::run(OSD* osd, OSDShard* sdata, PGRef& pg, ThreadPool::TPHandle& handle)
{
...
PGRecovery::run(
    ...
    ThreadPool::TPHandle& handle)
{
  osd->logger->tinc(l_osd_recovery_queue_lat, ceph_clock_now() - time_queued);
  osd->do_recovery(pg.get(), epoch_queued, reserved_pushes, priority, handle);
  pg->unlock();
}

void
PGRecoveryContext::run(
    OSD* osd,
    OSDShard* sdata,
    PGRef& pg,
    ThreadPool::TPHandle& handle)
{
  osd->logger->tinc(
      l_osd_recovery_context_queue_lat, ceph_clock_now() - time_queued);
  c.release()->complete(handle);
  pg->unlock();
}

void
PGDelete::run(OSD* osd, OSDShard* sdata, PGRef& pg, ThreadPool::TPHandle& handle)
{
