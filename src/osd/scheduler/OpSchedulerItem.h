...
public:
  ...
  public:
    ...
    virtual std::optional<OpRequestRef>
    maybe_get_op() const
    {
      return std::nullopt;
    }
    ...
    virtual bool
    peering_requires_pg() const
    {
      ceph_abort();
    }

    virtual const PGCreateInfo*
    creates_pg() const
    {
      return nullptr;
    }
    ...
    virtual void
    run(OSD* osd, OSDShard* sdata, PGRef& pg, ThreadPool::TPHandle& handle) = 0;
    virtual SchedulerClass get_scheduler_class() const = 0;

    virtual ~OpQueueable() {}

    friend std::ostream&
    operator<<(std::ostream& out, const OpQueueable& q)
    {
      return q.print(out);
    }
  };
...
public:
  ...
  uint32_t
  get_queue_token() const
  {
    return qitem->get_queue_token();
  }

  const spg_t&
  get_ordering_token() const
  {
  ...
  void
  run(OSD* osd, OSDShard* sdata, PGRef& pg, ThreadPool::TPHandle& handle)
  {
  ...
  SchedulerClass
  get_scheduler_class() const
  {
    return qitem->get_scheduler_class();
  }

  void
  set_qos_cost(uint32_t scaled_cost)
  {
    qos_cost = scaled_cost;
  }

  friend std::ostream&
  operator<<(std::ostream& out, const OpSchedulerItem& item)
  {
    out << "OpSchedulerItem(" << item.get_ordering_token() << " "
        << *item.qitem;
...
protected:
  ...
  static SchedulerClass
  priority_to_scheduler_class(int priority)
  {
...
public:
  ...
  uint32_t
  get_queue_token() const final
  {
    return get_pgid().ps();
  }

  const spg_t&
  get_ordering_token() const final
  {
    return get_pgid();
  }
};
...
public:
  PGOpItem(spg_t pg, OpRequestRef op) :
    PGOpQueueable(pg), op(std::move(op))
  {}

  std::ostream&
  print(std::ostream& rhs) const final
  {
...
public:
  ...
  bool
  is_peering() const override
  {
    return true;
  }

  bool
  peering_requires_pg() const override
  {
    return evt->requires_pg;
  }

  const PGCreateInfo*
  creates_pg() const override
  {
...
public:
  ...
  uint64_t
  get_reserved_pushes() const final
  {
...
public:
  ...
  print(std::ostream& rhs) const final
  {
    return rhs << "PGRecoveryContext(pgid=" << get_pgid() << " c=" << c.get()
               << " epoch=" << epoch << ")";
  }
  ...
  {
    return fmt::format(
        "PGRecoveryContext(pgid={} c={} epoch={})", get_pgid(), (void*)c.get(),
        epoch);
  }
...
public:
  ...
  static bool
  is_recovery_msg(OpRequestRef& op)
  {
...
struct fmt::formatter<ceph::osd::scheduler::OpSchedulerItem> {
      ...
      FormatContext& ctx) const
  {
    ...
    using class_t = std::underlying_type_t<SchedulerClass>;
