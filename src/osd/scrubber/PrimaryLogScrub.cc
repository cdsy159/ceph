...
bool
PrimaryLogScrub::get_store_errors(
    ...
    scrub_ls_result_t& res_inout) const
{
  ...
  if (arg.get_snapsets) {
    res_inout.vals = m_store->get_snap_errors(
        m_pg->get_pgid().pool(), arg.start_after, arg.max_return);
  } else {
    res_inout.vals = m_store->get_object_errors(
        m_pg->get_pgid().pool(), arg.start_after, arg.max_return);
  }
  return true;
}
...
void
PrimaryLogScrub::submit_digest_fixes(const digests_fixes_t& fixes)
{
  ...
  for (auto& [obj, dgs] : fixes) {
    ...
    dout(15) << fmt::format(
                    "{}: {}, pg[{}] {}/{}", __func__,
                    num_digest_updates_pending, m_pg_id, obj, dgs)
             << dendl;
    ...
    ctx->register_on_success([this]() {
      if ((num_digest_updates_pending >= 1) &&
        ...
        m_osds->queue_scrub_digest_update(
            m_pl_pg, m_pl_pg->is_scrub_blocking_ops());
      }
    });
...
void
PrimaryLogScrub::add_to_stats(const object_stat_sum_t& stat)
{
  m_scrub_cstat.add(stat);
}

void
PrimaryLogScrub::_scrub_finish()
{
...
void
PrimaryLogScrub::_scrub_clear_state()
{
  m_scrub_cstat = object_stat_collection_t();
}

void
PrimaryLogScrub::stats_of_handled_objects(
    ...
    const hobject_t& soid)
{
  ...
  if (is_primary() && is_scrub_active()) {
    if (soid < m_start) {
      ...
      dout(25) << fmt::format("{} {} >= [{},{})", __func__, soid, m_start, m_end)
               << dendl;
    }
  }
}
