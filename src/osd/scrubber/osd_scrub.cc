...
OsdScrub::~OsdScrub() { destroy_scrub_perf_counters(); }

std::ostream&
OsdScrub::gen_prefix(std::ostream& out, std::string_view fn) const
{
...
void
OsdScrub::dump_scrubs(ceph::Formatter* f) const
{
...
void
OsdScrub::dump_scrub_reservations(ceph::Formatter* f) const
{
...
void
OsdScrub::log_fwd(std::string_view text)
{
  dout(20) << text << dendl;
}

bool
OsdScrub::scrub_random_backoff() const
{
...
void
OsdScrub::debug_log_all_jobs() const
{
  m_queue.for_each_job(
      [this](const Scrub::SchedEntry& sj) {
...
void
OsdScrub::initiate_scrub(bool is_recovery_active)
{
 ...
 */
bool
OsdScrub::is_sched_target_eligible(
    ...
    utime_t time_now)
{
  ...
  if (r.random_backoff_active && ScrubJob::observes_random_backoff(e.urgency)) {
    return false;
  }
...
OsdScrub::initiate_a_scrub(
    const Scrub::SchedEntry& candidate,
    Scrub::OSDRestrictions restrictions)
{
  dout(20) << fmt::format("trying pg[{}] (target:{})", candidate.pgid, candidate)
           << dendl;
...
bool
OsdScrub::scrub_load_below_threshold() const
{
...
// closed.
static inline bool
isbetween_modulo(int64_t from, int64_t till, int p)
{
...
bool
OsdScrub::scrub_time_permit(utime_t now) const
{
  ...
  bool day_permits = isbetween_modulo(
      conf->osd_scrub_begin_week_day, conf->osd_scrub_end_week_day, bdt.tm_wday);
  if (!day_permits) {
    dout(20) << fmt::format(
...
OsdScrub::scrub_sleep_time(utime_t t_now, bool scrub_respects_ext_sleep) const
{
  const milliseconds regular_sleep_period = milliseconds{
      int64_t(std::max(0.0, 1'000 * conf->osd_scrub_sleep))};
  ...
  // forbidden hours
  const milliseconds extended_sleep = milliseconds{
      int64_t(1'000 * conf->osd_scrub_extended_sleep)};
  dout(20) << fmt::format(
...
void
OsdScrub::create_scrub_perf_counters()
{
...
void
OsdScrub::destroy_scrub_perf_counters()
{
...
PerfCounters*
OsdScrub::get_perf_counters(int pool_type, scrub_level_t level)
{
...
void
OsdScrub::enqueue_scrub_job(const Scrub::ScrubJob& sjob)
{
  m_queue.enqueue_scrub_job(sjob);
}

void
OsdScrub::enqueue_target(const Scrub::SchedTarget& trgt)
{
  m_queue.enqueue_target(trgt);
}

void
OsdScrub::dequeue_target(spg_t pgid, scrub_level_t s_or_d)
{
  m_queue.dequeue_target(pgid, s_or_d);
}

void
OsdScrub::remove_from_osd_queue(spg_t pgid)
{
  m_queue.remove_from_osd_queue(pgid);
}

std::unique_ptr<Scrub::LocalResourceWrapper>
OsdScrub::inc_scrubs_local(bool is_high_priority)
{
  return m_resource_bookkeeper.inc_scrubs_local(is_high_priority);
}

void
OsdScrub::dec_scrubs_local()
{
  m_resource_bookkeeper.dec_scrubs_local();
}

void
OsdScrub::mark_pg_scrub_blocked(spg_t blocked_pg)
{
  m_queue.mark_pg_scrub_blocked(blocked_pg);
}

void
OsdScrub::clear_pg_scrub_blocked(spg_t blocked_pg)
{
  m_queue.clear_pg_scrub_blocked(blocked_pg);
}

int
OsdScrub::get_blocked_pgs_count() const
{
