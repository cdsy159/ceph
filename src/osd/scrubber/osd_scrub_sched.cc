...
std::ostream&
ScrubQueue::gen_prefix(std::ostream& out, std::string_view fn) const
{
 ...
 */
void
ScrubQueue::remove_from_osd_queue(spg_t pgid)
{
  dout(10) << fmt::format("removing pg[{}] from OSD scrub queue", pgid)
           << dendl;
...
void
ScrubQueue::enqueue_scrub_job(const Scrub::ScrubJob& sjob)
{
  std::unique_lock lck{jobs_lock};
...
void
ScrubQueue::enqueue_target(const Scrub::SchedTarget& trgt)
{
  std::unique_lock lck{jobs_lock};
...
void
ScrubQueue::dequeue_target(spg_t pgid, scrub_level_t s_or_d)
{
  std::unique_lock lck{jobs_lock};
  remove_entry_unlocked(pgid, s_or_d);
}

std::optional<Scrub::SchedEntry>
ScrubQueue::pop_ready_entry(
    EligibilityPred eligibility_pred,
    OSDRestrictions restrictions,
    utime_t time_now)
{
  auto eligible_filtr = [&, rst = restrictions](const SchedEntry& e) -> bool {
 ...
 */
std::set<spg_t>
ScrubQueue::get_pgs(const ScrubQueue::EntryPred& pred) const
{
  std::lock_guard lck(jobs_lock);

  using acc_t = std::set<spg_t>;
  auto extract_pg = [pred](acc_t&& acc, const SchedEntry& se, bool is_eligible) {
  ...
  return to_scrub.accumulate<acc_t, decltype(extract_pg)>(std::move(extract_pg));
}

void
ScrubQueue::for_each_job(
...
bool
ScrubQueue::remove_entry_unlocked(spg_t pgid, scrub_level_t s_or_d)
{
...
void
ScrubQueue::dump_scrubs(ceph::Formatter& f) const
{
...
void
ScrubQueue::clear_pg_scrub_blocked(spg_t blocked_pg)
{
...
void
ScrubQueue::mark_pg_scrub_blocked(spg_t blocked_pg)
{
...
int
ScrubQueue::get_blocked_pgs_count() const
{
