...
struct shard_as_auth_t {
  ...
  shard_as_auth_t(
      const object_info_t& anoi,
      shard_to_scrubmap_t::iterator it,
      std::string err_msg,
      std::optional<uint32_t> data_digest,
      bool nonprimary_ec) :
    possible_auth{
        nonprimary_ec ? usable_t::not_usable_no_err : usable_t::usable},
    error_text{err_msg},
    oi{anoi},
    auth_iter{it},
    digest{data_digest}
...
struct formatter<shard_as_auth_t> {
  ...
  auto
  format(shard_as_auth_t const& as_auth, FormatContext& ctx) const
  {
    if (debug_log) {
      ...
      if (as_auth.possible_auth ==
          shard_as_auth_t::usable_t::not_usable_no_err) {
        return fmt::format_to(
            ctx.out(), "{{shard-not-usable-no-err:{}}}", as_auth.error_text);
      }
      return fmt::format_to(
          ctx.out(), "{{shard-usable: soid:{} {{txt:{}}} }}", as_auth.oi.soid,
          as_auth.error_text);

    } else {
      return fmt::format_to(
          ctx.out(), "usable:{} soid:{} {{txt:{}}}",
          (as_auth.possible_auth == shard_as_auth_t::usable_t::usable) ? "yes"
                                                                       : "no",
          as_auth.oi.soid, as_auth.error_text);
    }
  }
...
struct auth_selection_t {
  shard_to_scrubmap_t::const_iterator
      auth; ///< an iter into one of this_chunk->maps
...
struct formatter<auth_selection_t> {
  ...
  template <typename FormatContext>
  auto
  format(auth_selection_t const& aus, FormatContext& ctx) const
  {
    return fmt::format_to(
...
public:
   ...
   */
  int
  authoritative_peers_count() const
  {
    return m_auth_peer.size();
  }
...
private:
  ...
  /// shallow/deep error counters
  error_counters_t
  get_error_counts() const
  {
    return this_chunk->m_error_counts;
  }
  ...
  auth_and_obj_errs_t match_in_shards(
      const hobject_t& ho,
   ...
   * entry.
   */
  shard_as_auth_t possible_auth_shard(
      const hobject_t& ho,
      const pg_shard_t& srd,
      shard_info_map_t& shard_map);

  auth_selection_t select_auth_object(
      const hobject_t& ho,
      std::stringstream& errstream);

  void setup_ec_digest_map(auth_selection_t& auth_selection, const hobject_t& ho);
  ...
  uint64_t logical_to_ondisk_size(
  ...
  char
  retrieve_byte(uint32_t value, uint32_t index)
  {
...
struct formatter<data_omap_digests_t> {
  ...
  template <typename FormatContext>
  auto
  format(const data_omap_digests_t& dg, FormatContext& ctx) const
  {
...
struct formatter<std::pair<hobject_t, data_omap_digests_t>> {
  ...
  auto
  format(
      const std::pair<hobject_t, data_omap_digests_t>& x,
      FormatContext& ctx) const
  {
    return fmt::format_to(
        ctx.out(), "{{ {} - {} }}", std::get<0>(x), std::get<1>(x));
  }
};
} // namespace fmt
