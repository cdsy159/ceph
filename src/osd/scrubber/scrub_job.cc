...
void
SchedEntry::dump(ceph::Formatter& f) const
{
...
void
SchedTarget::up_urgency_to(urgency_t u)
{
...
namespace std {
ostream&
operator<<(ostream& out, const ScrubJob& sjob)
{
...
void
ScrubJob::set_both_targets_queued()
{
...
void
ScrubJob::adjust_shallow_schedule(
    utime_t last_scrub,
    ...
    utime_t scrub_clock_now)
{
  ...
  if (ScrubJob::requires_randomization(shallow_target.urgency())) {
    ...
    adj_target += app_conf.shallow_interval *
                  app_conf.interval_randomize_ratio * r;
...
void
ScrubJob::operator_forced(scrub_level_t s_or_d, scrub_type_t scrub_type)
{
...
std::optional<std::reference_wrapper<SchedTarget>>
ScrubJob::earliest_eligible(utime_t scrub_clock_now)
{
...
const SchedTarget&
ScrubJob::earliest_target() const
{
  std::weak_ordering compr = cmp_future_entries(
      shallow_target.queued_element(), deep_target.queued_element());
  return (compr == std::weak_ordering::less) ? shallow_target : deep_target;
...
SchedTarget&
ScrubJob::earliest_target(utime_t scrub_clock_now)
{
  std::weak_ordering compr = cmp_entries(
      scrub_clock_now, shallow_target.queued_element(),
      deep_target.queued_element());
...
const SchedTarget&
ScrubJob::earliest_target(utime_t scrub_clock_now) const
{
  std::weak_ordering compr = cmp_entries(
      scrub_clock_now, shallow_target.queued_element(),
      deep_target.queued_element());
...
utime_t
ScrubJob::get_sched_time() const
{
  return earliest_target().sched_info.schedule.not_before;
}

void
ScrubJob::adjust_deep_schedule(
    utime_t last_deep,
    const Scrub::sched_conf_t& app_conf,
    utime_t scrub_clock_now)
{
    ...
    utime_t scrub_clock_now)
{
  ...
  auto& delayed_target = (level == scrub_level_t::deep) ? deep_target
                                                        : shallow_target;
...
std::string
ScrubJob::scheduling_state(utime_t now_is) const
{
...
std::ostream&
ScrubJob::gen_prefix(std::ostream& out, std::string_view fn) const
{
...
bool
ScrubJob::observes_noscrub_flags(urgency_t urgency)
{
  return urgency < urgency_t::after_repair;
}

bool
ScrubJob::observes_allowed_hours(urgency_t urgency)
{
  return urgency < urgency_t::operator_requested;
}

bool
ScrubJob::observes_extended_sleep(urgency_t urgency)
{
  return urgency == urgency_t::periodic_regular;
}

bool
ScrubJob::observes_load_limit(urgency_t urgency)
{
  return urgency < urgency_t::after_repair;
}

bool
ScrubJob::requires_reservation(urgency_t urgency)
{
  return urgency < urgency_t::after_repair;
}

bool
ScrubJob::requires_randomization(urgency_t urgency)
{
  return urgency == urgency_t::periodic_regular;
}

bool
ScrubJob::observes_max_concurrency(urgency_t urgency)
{
  return urgency < urgency_t::operator_requested;
}

bool
ScrubJob::observes_random_backoff(urgency_t urgency)
{
  return urgency < urgency_t::after_repair;
}

bool
ScrubJob::observes_recovery(urgency_t urgency)
{
  return urgency < urgency_t::operator_requested;
}

bool
ScrubJob::has_high_queue_priority(urgency_t urgency)
{
  return urgency >= urgency_t::operator_requested;
}

bool
ScrubJob::is_repair_implied(urgency_t urgency)
{
  return urgency == urgency_t::repairing || urgency == urgency_t::must_repair;
}

bool
ScrubJob::is_autorepair_allowed(urgency_t urgency)
{
...
bool
ScrubJob::is_repairs_count_limited(urgency_t urgency)
{
  return urgency < urgency_t::operator_requested;
}
