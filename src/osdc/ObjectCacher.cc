...
ObjectCacher::BufferHead*
ObjectCacher::Object::split(BufferHead* left, loff_t off)
{
...
void
ObjectCacher::Object::merge_left(BufferHead* left, BufferHead* right)
{
  ceph_assert(ceph_mutex_is_locked(oc->lock));
  ...
  for (auto p = right->waitfor_read.begin(); p != right->waitfor_read.end(); ++p)
    left->waitfor_read[p->first].splice(
        left->waitfor_read[p->first].begin(), p->second);
...
bool
ObjectCacher::Object::can_merge_bh(BufferHead* left, BufferHead* right)
{
...
void
ObjectCacher::Object::try_merge_bh(BufferHead* bh)
{
  ceph_assert(ceph_mutex_is_locked(oc->lock));
...
void
ObjectCacher::Object::maybe_rebuild_buffer(BufferHead* bh)
{
  auto& bl = bh->bl;
 ...
 */
bool
ObjectCacher::Object::is_cached(loff_t cur, loff_t left) const
{
  ceph_assert(ceph_mutex_is_locked(oc->lock));
 ...
 */
bool
ObjectCacher::Object::include_all_cached_data(loff_t off, loff_t len)
{
  ceph_assert(ceph_mutex_is_locked(oc->lock));
  if (data.empty())
    return true;
...
void
ObjectCacher::Object::audit_buffers()
{
  loff_t offset = 0;
  for (auto it = data.begin(); it != data.end(); ++it) {
    ...
    for (auto w_it = bh->waitfor_read.begin(); w_it != bh->waitfor_read.end();
         ++w_it) {
 ...
 */
ObjectCacher::BufferHead*
ObjectCacher::Object::map_write(ObjectExtent& ex, ceph_tid_t tid)
{
  ceph_assert(ceph_mutex_is_locked(oc->lock));
  BufferHead* final = 0;
...
void
ObjectCacher::Object::replace_journal_tid(BufferHead* bh, ceph_tid_t tid)
{
  ...
  if (bh_tid != 0 && bh_tid != tid) {
    ...
    oc->writeback_handler.overwrite_extent(
        get_oid(), bh->start(), bh->length(), bh_tid, tid);
  }
  bh->set_journal_tid(tid);
}

void
ObjectCacher::Object::truncate(loff_t s)
{
  ceph_assert(ceph_mutex_is_locked(oc->lock));
...
void
ObjectCacher::perf_start()
{
  string n = "objectcacher-" + name;
  PerfCountersBuilder plb(cct, n, l_objectcacher_first, l_objectcacher_last);

  plb.add_u64_counter(
      l_objectcacher_cache_ops_hit, "cache_ops_hit", "Hit operations");
  plb.add_u64_counter(
      l_objectcacher_cache_ops_miss, "cache_ops_miss", "Miss operations");
  plb.add_u64_counter(
      l_objectcacher_cache_bytes_hit, "cache_bytes_hit", "Hit data", NULL, 0,
      unit_t(UNIT_BYTES));
  plb.add_u64_counter(
      l_objectcacher_cache_bytes_miss, "cache_bytes_miss", "Miss data", NULL, 0,
      unit_t(UNIT_BYTES));
  plb.add_u64_counter(l_objectcacher_data_read, "data_read", "Read data");
  plb.add_u64_counter(
      l_objectcacher_data_written, "data_written", "Data written to cache");
  plb.add_u64_counter(
      l_objectcacher_data_flushed, "data_flushed", "Data flushed");
  plb.add_u64_counter(
      l_objectcacher_overwritten_in_flush, "data_overwritten_while_flushing",
      "Data overwritten while flushing");
  plb.add_u64_counter(
      l_objectcacher_write_ops_blocked, "write_ops_blocked",
      "Write operations, delayed due to dirty limits");
  plb.add_u64_counter(
      l_objectcacher_write_bytes_blocked, "write_bytes_blocked",
      "Write data blocked on dirty limit", NULL, 0, unit_t(UNIT_BYTES));
  plb.add_time(
      l_objectcacher_write_time_blocked, "write_time_blocked",
...
void
ObjectCacher::close_object(Object* ob)
{
  ceph_assert(ceph_mutex_is_locked(lock));
    ...
    const ZTracer::Trace& parent_trace)
{
  ...
  // finisher
  C_ReadFinish* onfinish = new C_ReadFinish(
      this, bh->ob, bh->last_read_tid, bh->start(), bh->length(), trace);
  // go
    ...
    bool trust_enoent)
{
  ...
  if (objects[poolid].count(oid) == 0) {
    ...
    while (true) {
      ...
      // finishers?
      for (auto it = bh->waitfor_read.begin(); it != bh->waitfor_read.end();
           ++it)
...
void
ObjectCacher::bh_write_scattered(list<BufferHead*>& blist)
{
  ceph_assert(ceph_mutex_is_locked(lock));
...
void
ObjectCacher::bh_write(BufferHead* bh, const ZTracer::Trace& parent_trace)
{
  ceph_assert(ceph_mutex_is_locked(lock));
  ...
  // finishers
  C_WriteCommit* oncommit = new C_WriteCommit(
      this, bh->ob->oloc.pool, bh->ob->get_soid(), bh->start(), bh->length(),
      trace);
  // go
  ceph_tid_t tid = writeback_handler.write(
    ...
    int r)
{
  ...
  for (vector<pair<loff_t, uint64_t>>::iterator p = ranges.begin();
    ...
    if (!ob->exists) {
      ...
      if (writeback_handler.may_copy_on_write(
              ob->get_oid(), start, length, ob->get_snap())) {
...
void
ObjectCacher::flush(ZTracer::Trace* trace, loff_t amount, int max_bhs)
{
  ceph_assert(trace != nullptr);
  ceph_assert(ceph_mutex_is_locked(lock));
  ceph::real_time cutoff = ceph::real_clock::now();

  ldout(cct, 10) << "flush " << amount << " bytes, max bufferheads " << max_bhs
                 << dendl;
  ...
  while ((amount == 0 && max_bhs == 0) || left > 0 || left_bhs > 0) {
    ...
    if (scattered_write) {
      bh_write_adjacencies(
          bh, cutoff, amount > 0 ? &left : NULL, max_bhs > 0 ? &left_bhs : NULL);
    } else {
      left -= bh->length();
      left_bhs--;
      bh_write(bh, *trace);
...
ObjectCacher::trim()
{
  ...
  uint64_t nr_clean_bh = bh_lru_rest.lru_get_size() -
                         bh_lru_rest.lru_get_num_pinned();
    ...
    std::vector<ObjHole>* holes)
{
  ...
  for (vector<ObjectExtent>::iterator ex_it = rd->extents.begin();
    ...
    Object* o = get_object(
        soid, ex_it->objectno, oset, ex_it->oloc, ex_it->truncate_size,
        oset->truncate_seq);
    if (external_call)
      touch_ob(o);
    ...
    if (oset->return_enoent && !o->exists) {
      ...
      if (writeback_handler.may_copy_on_write(
              soid.oid, ex_it->offset, ex_it->length, soid.snap)) {
...
void
ObjectCacher::retry_waiting_reads()
{
  list<Context*> ls;
    ...
    bool block_writes_upfront)
{
  ...
  if (perfcounter) {
    ...
    if (bytes_written_in_flush) {
      perfcounter->inc(
          l_objectcacher_overwritten_in_flush, bytes_written_in_flush);
    }
  }

  int r = _wait_for_write(
      wr, bytes_written, oset, &trace, onfreespace, block_writes_upfront);
  delete wr;
...
void
ObjectCacher::C_WaitForWrite::finish(int r)
{
...
void
ObjectCacher::_maybe_wait_for_writeback(uint64_t len, ZTracer::Trace* trace)
{
  ceph_assert(ceph_mutex_is_locked(lock));
    ...
    bool block_writes_upfront)
{
  ...
  if (max_dirty > 0 && !(wr->fadvise_flags & LIBRADOS_OP_FLAG_FADVISE_FUA)) {
    ...
    bool done = false;
    Context* fin = block_writes_upfront ? new C_Cond(cond, &done, &ret)
                                        : onfreespace;
    ceph_assert(fin);
...
bool
ObjectCacher::set_is_empty(ObjectSet* oset)
{
  ceph_assert(ceph_mutex_is_locked(lock));
...
bool
ObjectCacher::set_is_cached(ObjectSet* oset)
{
  ceph_assert(ceph_mutex_is_locked(lock));
  if (oset->objects.empty())
    return false;
...
bool
ObjectCacher::set_is_dirty_or_committing(ObjectSet* oset)
{
  ceph_assert(ceph_mutex_is_locked(lock));
  if (oset->objects.empty())
    return false;
...
bool
ObjectCacher::_flush_set_finish(C_GatherBuilder* gather, Context* onfinish)
{
  ceph_assert(ceph_mutex_is_locked(lock));
  if (gather->has_subs()) {
...
bool
ObjectCacher::flush_set(ObjectSet* oset, Context* onfinish)
{
  ceph_assert(ceph_mutex_is_locked(lock));
...
// returns true if already flushed
bool
ObjectCacher::flush_all(Context* onfinish)
{
  ceph_assert(ceph_mutex_is_locked(lock));
  ceph_assert(onfinish != NULL);
...
void
ObjectCacher::purge_set(ObjectSet* oset)
{
  ceph_assert(ceph_mutex_is_locked(lock));
  if (oset->objects.empty()) {
...
loff_t
ObjectCacher::release(Object* ob)
{
  ceph_assert(ceph_mutex_is_locked(lock));
...
loff_t
ObjectCacher::release_set(ObjectSet* oset)
{
  ceph_assert(ceph_mutex_is_locked(lock));
...
void
ObjectCacher::clear_nonexistence(ObjectSet* oset)
{
  ceph_assert(ceph_mutex_is_locked(lock));
 ...
 */
void
ObjectCacher::discard_set(ObjectSet* oset, const vector<ObjectExtent>& exls)
{
  ceph_assert(ceph_mutex_is_locked(lock));
...
void
ObjectCacher::_discard_finish(ObjectSet* oset, bool was_dirty, Context* on_finish)
{
  ceph_assert(ceph_mutex_is_locked(lock));
...
void
ObjectCacher::verify_stats() const
{
  ceph_assert(ceph_mutex_is_locked(lock));
...
void
ObjectCacher::bh_stat_add(BufferHead* bh)
{
  ceph_assert(ceph_mutex_is_locked(lock));
  switch (bh->get_state()) {
  case BufferHead::STATE_MISSING:
...
void
ObjectCacher::bh_stat_sub(BufferHead* bh)
{
  ceph_assert(ceph_mutex_is_locked(lock));
  switch (bh->get_state()) {
  case BufferHead::STATE_MISSING:
...
void
ObjectCacher::bh_set_state(BufferHead* bh, int s)
{
  ceph_assert(ceph_mutex_is_locked(lock));
...
void
ObjectCacher::bh_add(Object* ob, BufferHead* bh)
{
  ceph_assert(ceph_mutex_is_locked(lock));
...
void
ObjectCacher::bh_remove(Object* ob, BufferHead* bh)
{
  ceph_assert(ceph_mutex_is_locked(lock));
