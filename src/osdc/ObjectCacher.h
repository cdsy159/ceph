...
public:
  ...
  OSDRead*
  prepare_read(snapid_t snap, ceph::buffer::list* b, int f) const
  {
  ...
  struct OSDWrite {
    ...
    OSDWrite(
        const SnapContext& sc,
        const ceph::buffer::list& b,
        ceph::real_time mt,
        int f,
        ceph_tid_t _journal_tid) :
      snapc(sc), bl(b), mtime(mt), fadvise_flags(f), journal_tid(_journal_tid)
    {}
  };
  ...
  public:
    ...
    inline void
    set_journal_tid(ceph_tid_t _journal_tid)
    {
    ...
    inline bool
    can_merge_journal(BufferHead* bh) const
    {
    ...
    struct ptr_lt {
      bool
      operator()(const BufferHead* l, const BufferHead* r) const
      {
  ...
  public:
    ...
    void
    set_object_locator(object_locator_t& l)
    {
      oloc = l;
    }
     ...
     */
    std::map<loff_t, BufferHead*>::const_iterator
    data_lower_bound(loff_t offset) const
    {
...
private:
  ...
  // objects
  Object*
  get_object_maybe(sobject_t oid, object_locator_t& l)
  {
    // have it?
    if (((uint32_t)l.pool < objects.size()) && (objects[l.pool].count(oid)))
  ...
  loff_t
  get_stat_dirty() const
  {
    return stat_dirty;
  }

  loff_t
  get_stat_clean() const
  {
    return stat_clean;
  }
  ...
  loff_t
  get_stat_dirty_waiting() const
  {
    return stat_dirty_waiting;
  }

  size_t
  get_stat_nr_dirty_waiters() const
  {
    return stat_nr_dirty_waiters;
  }

  void
  touch_bh(BufferHead* bh)
  {
    if (bh->is_dirty())
...
public:
  bool
  CFG_block_writes_upfront(void)
  {
    return cfg_block_writes_upfront;
  }
...
private:
  ...
  void _discard(
      ObjectSet* oset,
      const std::vector<ObjectExtent>& exls,
...
public:
  ...
  void discard_writeback(
      ObjectSet* oset,
      const std::vector<ObjectExtent>& ex,
      Context* on_finish);
  ...
  // cache sizes
  void
  set_max_dirty(uint64_t v)
  {
    max_dirty = v;
  }

  void
  set_target_dirty(int64_t v)
  {
    target_dirty = v;
  }

  void
  set_max_size(int64_t v)
  {
    max_size = v;
  }

  void
  set_max_dirty_age(double a)
  {
    max_dirty_age = ceph::make_timespan(a);
  }

  void
  set_max_objects(int64_t v)
  {
    max_objects = v;
  }
      ...
      uint64_t len)
  {
    ...
    Striper::file_to_extents(
        cct, oset->ino, layout, offset, len, oset->truncate_size, extents);
      ...
      Context* onfinish)
  {
    ...
    Striper::file_to_extents(
        cct, oset->ino, layout, offset, len, oset->truncate_size, rd->extents);
      ...
      bool block_writes_upfront)
  {
    ...
    Striper::file_to_extents(
        cct, oset->ino, layout, offset, len, oset->truncate_size, wr->extents);
...
inline std::ostream&
operator<<(std::ostream& out, const ObjectCacher::BufferHead& bh)
{
  out << "bh[ " << &bh << " " << bh.start() << "~" << bh.length() << " "
      << bh.ob << " (" << bh.bl.length() << ")" << " v " << bh.last_write_tid;
...
inline std::ostream&
operator<<(std::ostream& out, const ObjectCacher::ObjectSet& os)
{
  return out << "objectset[" << os.ino << " ts " << os.truncate_seq << "/"
             << os.truncate_size << " objects " << os.objects.size()
             << " dirty_or_tx " << os.dirty_or_tx << "]";
}

inline std::ostream&
operator<<(std::ostream& out, const ObjectCacher::Object& ob)
{
  out << "object[" << ob.get_soid() << " oset " << ob.oset << std::dec << " wr "
      << ob.last_write_tid << "/" << ob.last_commit_tid;
