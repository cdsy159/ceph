...
Objecter::get_tracked_keys() const noexcept
{
  return {"crush_location"s, "rados_mon_op_timeout"s, "rados_osd_op_timeout"s};
}

void
Objecter::handle_conf_change(
...
void
Objecter::update_crush_location()
{
...
Objecter::init()
{
  ...
  if (!logger) {
    ...
    pcb.add_u64_avg(
        l_osdc_oplen_avg, "oplen_avg", "Average length of operation vector");

    pcb.add_u64_counter(l_osdc_op, "op", "Operations");
    pcb.add_u64_counter(
        l_osdc_op_r, "op_r", "Read operations", "rd",
        PerfCountersBuilder::PRIO_CRITICAL);
    pcb.add_u64_counter(
        l_osdc_op_w, "op_w", "Write operations", "wr",
        PerfCountersBuilder::PRIO_CRITICAL);
    pcb.add_u64_counter(
        l_osdc_op_rmw, "op_rmw", "Read-modify-write operations", "rdwr",
    ...
    pcb.add_u64_counter(
        l_osdc_osdop_create, "osdop_create", "Create object operations");
    ...
    pcb.add_u64_counter(
        l_osdc_osdop_writefull, "osdop_writefull",
        "Write full object operations");
    pcb.add_u64_counter(
        l_osdc_osdop_writesame, "osdop_writesame", "Write same operations");
    pcb.add_u64_counter(l_osdc_osdop_append, "osdop_append", "Append operation");
    pcb.add_u64_counter(
        l_osdc_osdop_zero, "osdop_zero", "Set object to zero operations");
    pcb.add_u64_counter(
        l_osdc_osdop_truncate, "osdop_truncate", "Truncate object operations");
    pcb.add_u64_counter(
        l_osdc_osdop_delete, "osdop_delete", "Delete object operations");
    pcb.add_u64_counter(
        l_osdc_osdop_mapext, "osdop_mapext", "Map extent operations");
    pcb.add_u64_counter(
        l_osdc_osdop_sparse_read, "osdop_sparse_read", "Sparse read operations");
    pcb.add_u64_counter(
        l_osdc_osdop_clonerange, "osdop_clonerange", "Clone range operations");
    pcb.add_u64_counter(
        l_osdc_osdop_getxattr, "osdop_getxattr", "Get xattr operations");
    pcb.add_u64_counter(
        l_osdc_osdop_setxattr, "osdop_setxattr", "Set xattr operations");
    pcb.add_u64_counter(
        l_osdc_osdop_cmpxattr, "osdop_cmpxattr", "Xattr comparison operations");
    pcb.add_u64_counter(
        l_osdc_osdop_rmxattr, "osdop_rmxattr", "Remove xattr operations");
    pcb.add_u64_counter(
        l_osdc_osdop_resetxattrs, "osdop_resetxattrs", "Reset xattr operations");
    pcb.add_u64_counter(
        l_osdc_osdop_call, "osdop_call", "Call (execute) operations");
    pcb.add_u64_counter(
        l_osdc_osdop_watch, "osdop_watch", "Watch by object operations");
    pcb.add_u64_counter(
        l_osdc_osdop_notify, "osdop_notify", "Notify about object operations");
    pcb.add_u64_counter(
        l_osdc_osdop_src_cmpxattr, "osdop_src_cmpxattr",
    ...
    pcb.add_u64(
        l_osdc_linger_active, "linger_active", "Active lingering operations");
    pcb.add_u64_counter(
        l_osdc_linger_send, "linger_send", "Sent lingering operations");
    pcb.add_u64_counter(
        l_osdc_linger_resend, "linger_resend", "Resent lingering operations");
    pcb.add_u64_counter(
        l_osdc_linger_ping, "linger_ping", "Sent pings to lingering operations");

    pcb.add_u64(l_osdc_poolop_active, "poolop_active", "Active pool operations");
    pcb.add_u64_counter(
        l_osdc_poolop_send, "poolop_send", "Sent pool operations");
    pcb.add_u64_counter(
        l_osdc_poolop_resend, "poolop_resend", "Resent pool operations");

    pcb.add_u64(
        l_osdc_poolstat_active, "poolstat_active",
        "Active get pool stat operations");
    pcb.add_u64_counter(
        l_osdc_poolstat_send, "poolstat_send", "Pool stat operations sent");
    pcb.add_u64_counter(
        l_osdc_poolstat_resend, "poolstat_resend", "Resent pool stats");
    ...
    pcb.add_u64_counter(
        l_osdc_statfs_resend, "statfs_resend", "Resent FS stats");
    ...
    pcb.add_u64_counter(
        l_osdc_command_resend, "command_resend", "Resent commands");

    pcb.add_u64(l_osdc_map_epoch, "map_epoch", "OSD map epoch");
    pcb.add_u64_counter(l_osdc_map_full, "map_full", "Full OSD maps received");
    pcb.add_u64_counter(
        l_osdc_map_inc, "map_inc", "Incremental OSD maps received");

    pcb.add_u64(
        l_osdc_osd_sessions, "osd_sessions",
        "Open sessions"); // open sessions
    pcb.add_u64_counter(
        l_osdc_osd_session_open, "osd_session_open", "Sessions opened");
    pcb.add_u64_counter(
        l_osdc_osd_session_close, "osd_session_close", "Sessions closed");
    pcb.add_u64(l_osdc_osd_laggy, "osd_laggy", "Laggy OSD sessions");

    pcb.add_u64_counter(
        l_osdc_osdop_omap_wr, "omap_wr", "OSD OMAP write operations");
    pcb.add_u64_counter(
        l_osdc_osdop_omap_rd, "omap_rd", "OSD OMAP read operations");
    pcb.add_u64_counter(
        l_osdc_osdop_omap_del, "omap_del", "OSD OMAP delete operations");

    pcb.add_u64_counter(
        l_osdc_replica_read_sent, "replica_read_sent",
        "Operations sent to replica");
    pcb.add_u64_counter(
        l_osdc_replica_read_bounced, "replica_read_bounced",
        "Operations bounced by replica to be resent to primary");
    pcb.add_u64_counter(
        l_osdc_replica_read_completed, "replica_read_completed",
  ...
  int ret = admin_socket->register_command(
      "objecter_requests", m_request_state_hook,
    ...
    ceph::shunique_lock<ceph::shared_mutex>& sul)
{
  ...
  watchl.unlock();
  auto o = new Op(
      info->target.base_oid, info->target.base_oloc, std::move(opv),
      info->target.flags | CEPH_OSD_FLAG_READ, std::move(oncommit),
      info->pobjver);
  o->outbl = poutbl;
  o->snapid = info->snap;
  o->snapc = info->snapc;
...
void
Objecter::_linger_commit(LingerOp* info, bs::error_code ec, cb::list& outbl)
{
  ...
  if (info->on_reg_commit) {
    asio::defer(
        ...
        asio::append(std::move(info->on_reg_commit), ec, cb::list{}));
  }
  if (ec && info->on_notify_finish) {
    asio::defer(
        service.get_executor(),
        asio::append(std::move(info->on_notify_finish), ec, cb::list{}));
  }
...
void
Objecter::_linger_reconnect(LingerOp* info, bs::error_code ec)
{
...
void
Objecter::_send_linger_ping(LingerOp* info)
{
  ...
  Op* o = new Op(
      info->target.base_oid, info->target.base_oloc, std::move(opv),
      info->target.flags | CEPH_OSD_FLAG_READ, CB_Linger_Ping(this, info, now),
      nullptr, nullptr);
  o->target = info->target;
  o->should_resend = false;
  _send_op_account(o);
    ...
    uint32_t register_gen)
{
  ...
  ldout(cct, 10) << __func__ << " " << info->linger_id << " sent " << sent
                 << " gen " << register_gen << " = " << ec << " (last_error "
...
tl::expected<ceph::timespan, bs::error_code>
Objecter::linger_check(LingerOp* info)
{
...
void
Objecter::linger_cancel(LingerOp* info)
{
  unique_lock wl(rwlock);
  _linger_cancel(info);
}
...
void
Objecter::_linger_cancel(LingerOp* info)
{
  // rwlock is locked unique
...
auto
Objecter::linger_by_cookie(uint64_t cookie) -> boost::intrusive_ptr<LingerOp>
{
...
auto
Objecter::_linger_by_cookie(uint64_t cookie) -> boost::intrusive_ptr<LingerOp>
{
...
Objecter::linger_register(
    ...
    int flags) -> boost::intrusive_ptr<LingerOp>
{
    ...
    ceph::shunique_lock<ceph::shared_mutex>& sul)
{
  ...
  ceph_assert(
      info->ctx_budget != -1); // caller needs to have taken budget already!
...
void
Objecter::handle_watch_notify(MWatchNotify* m)
{
  ...
  if (m->opcode == CEPH_WATCH_EVENT_DISCONNECT) {
    ...
    if (info->notify_id && info->notify_id != m->notify_id) {
      ...
      asio::defer(
          service.get_executor(),
          asio::append(
              std::move(info->on_notify_finish), osdcode(m->return_code),
              std::move(m->get_data())));
...
Objecter::ms_dispatch2(const MessageRef& m)
{
    ...
    ceph::shunique_lock<ceph::shared_mutex>& sul)
{
  ...
  for (auto iter = unregister_lingers.begin(); iter != unregister_lingers.end();
       ++iter) {
    _linger_cancel(*iter);
    (*iter)->put();
  }
  ...
  if (m->get_last() <= osdmap->get_epoch()) {
    ...
    if (osdmap->get_epoch()) {
      ...
      for (epoch_t e = osdmap->get_epoch() + 1; e <= m->get_last(); e++) {
        ...
        _scan_requests(
            homeless_session, skipped_map, cluster_full, &pool_full_map,
            need_resend, need_resend_linger, need_resend_command, sul);
        for (auto p = osd_sessions.begin(); p != osd_sessions.end();) {
          auto s = p->second;
          _scan_requests(
              s, skipped_map, cluster_full, &pool_full_map, need_resend,
      ...
      if (m->maps.count(m->get_last())) {
        for (auto p = osd_sessions.begin(); p != osd_sessions.end(); ++p) {
          ...
          _scan_requests(
              s, false, false, NULL, need_resend, need_resend_linger,
        ...
        _scan_requests(
            homeless_session, false, false, NULL, need_resend,
            need_resend_linger, need_resend_command, sul);
      } else {
...
void
Objecter::consume_blocklist_events(std::set<entity_addr_t>* events)
{
  unique_lock wl(rwlock);
...
void
Objecter::emit_blocklist_events(const OSDMap::Incremental& inc)
{
...
void
Objecter::emit_blocklist_events(
    const OSDMap& old_osd_map,
    const OSDMap& new_osd_map)
{
  ...
  std::set_difference(
      new_range_set.begin(), new_range_set.end(), old_range_set.begin(),
      old_range_set.end(), std::inserter(delta_set, delta_set.begin()));
...
void
Objecter::CB_Op_Map_Latest::operator()(
    bs::error_code e,
    version_t latest,
    version_t)
{
  ...
  lgeneric_subdout(objecter->cct, objecter, 10)
      << "op_map_latest r=" << e << " tid=" << tid << " latest " << latest
      << dendl;
...
int
Objecter::pool_snap_list(int64_t poolid, vector<uint64_t>* snaps)
{
  shared_lock rl(rwlock);
...
// sl may be unlocked.
void
Objecter::_check_op_pool_dne(Op* op, std::unique_lock<std::shared_mutex>* sl)
{
  // rwlock is locked unique
  ...
  if (op->map_dne_bound > 0) {
    if (osdmap->get_epoch() >= op->map_dne_bound) {
      ...
      if (op->has_completion()) {
        num_in_flight--;
        op->complete(
            make_error_code(osdc_errc::pool_dne), -ENOENT,
            service.get_executor());
      }
...
// sl may be unlocked.
void
Objecter::_check_op_pool_eio(Op* op, std::unique_lock<std::shared_mutex>* sl)
{
  // rwlock is locked unique

  // we had a new enough map
  ldout(cct, 10) << "check_op_pool_eio tid " << op->tid << " concluding pool "
                 << op->target.base_pgid.pool() << " has eio" << dendl;
  if (op->has_completion()) {
    num_in_flight--;
    op->complete(
        make_error_code(osdc_errc::pool_eio), -EIO, service.get_executor());
  }
...
void
Objecter::_send_op_map_check(Op* op)
{
  // rwlock is locked unique
  // ask the monitor
...
void
Objecter::_op_cancel_map_check(Op* op)
{
  // rwlock is locked unique
...
void
Objecter::CB_Linger_Map_Latest::operator()(
    bs::error_code e,
    version_t latest,
    version_t)
{
...
void
Objecter::_check_linger_pool_dne(LingerOp* op, bool* need_unregister)
{
  // rwlock is locked unique
...
void
Objecter::_check_linger_pool_eio(LingerOp* op)
{
  // rwlock is locked unique
  ...
  if (op->on_reg_commit) {
    asio::defer(
        service.get_executor(),
        asio::append(
            std::move(op->on_reg_commit), make_error_code(osdc_errc::pool_dne),
            cb::list{}));
  }
  if (op->on_notify_finish) {
    asio::defer(
...
void
Objecter::_send_linger_map_check(LingerOp* op)
{
  // ask the monitor
...
void
Objecter::_linger_cancel_map_check(LingerOp* op)
{
  // rwlock is locked unique
...
void
Objecter::CB_Command_Map_Latest::operator()(
    bs::error_code e,
    version_t latest,
    version_t)
{
...
void
Objecter::_check_command_map_dne(CommandOp* c)
{
  // rwlock is locked unique
  ...
  if (c->map_dne_bound > 0) {
    if (osdmap->get_epoch() >= c->map_dne_bound) {
      _finish_command(
          c, osdcode(c->map_check_error), std::move(c->map_check_error_str), {});
    }
  } else {
...
void
Objecter::_send_command_map_check(CommandOp* c)
{
  // rwlock is locked unique
  // session is locked unique
...
void
Objecter::_command_cancel_map_check(CommandOp* c)
{
...
void
Objecter::put_session(Objecter::OSDSession* s)
{
  if (s && !s->is_homeless()) {
...
void
Objecter::get_session(Objecter::OSDSession* s)
{
  ceph_assert(s != NULL);
...
void
Objecter::_reopen_session(OSDSession* s)
{
  // rwlock is locked unique
  // s->lock is locked
...
void
Objecter::close_session(OSDSession* s)
{
  // rwlock is locked unique
...
void
Objecter::wait_for_osd_map(epoch_t e)
{
  unique_lock l(rwlock);
  ...
  auto ex = boost::asio::prefer(
      service.get_executor(), boost::asio::execution::outstanding_work.tracked);
  waiting_for_map[e].emplace_back(
      asio::bind_executor(service.get_executor(), w.ref()), bs::error_code{});
  l.unlock();
  w.wait();
}
    ...
    std::unique_lock<ceph::shared_mutex>&& l)
{
  ...
  if (osdmap->get_epoch() >= newest) {
    ...
    l.unlock();
    asio::defer(
        service.get_executor(), asio::append(std::move(fin), bs::error_code{}));
  } else {
...
void
Objecter::_wait_for_new_map(OpCompletion c, epoch_t epoch, bs::error_code ec)
{
  // rwlock is locked unique
 ...
 */
bool
Objecter::have_map(const epoch_t epoch)
{
  shared_lock rl(rwlock);
...
void
Objecter::_kick_requests(OSDSession* session, map<uint64_t, LingerOp*>& lresend)
{
  // rwlock is locked unique
  ...
  for (auto j = session->linger_ops.begin(); j != session->linger_ops.end();
       ++j) {
  ...
  for (auto k = session->command_ops.begin(); k != session->command_ops.end();
       ++k) {
...
Objecter::start_tick()
{
  ...
  tick_event = timer.add_event(
      ceph::make_timespan(cct->_conf->objecter_tick_interval), &Objecter::tick,
      this);
}
...
Objecter::tick()
{
  ...
  if (num_homeless_ops || !toping.empty()) {
    ...
    auto stale_window =
        ceph::make_timespan(cct->_conf->objecter_tick_interval) * 2;
    ...
    ldout(cct, 20) << __func__ << ": elapsed since last osdmap request "
                   ...
                   << ", epoch " << epoch << ", monc->get_start(osdmap) "
                   << exist_sub << dendl;
    // check that:
    ...
    if (elapsed > stale_window && ((exist_sub == 0) || (epoch > exist_sub))) {
      double elapsed_s = std::chrono::duration<double>(elapsed).count();
      double thresh_s = std::chrono::duration<double>(stale_window).count();
      ldout(cct, 20) << __func__ << ": osdmap stale: " << elapsed_s << "s > "
                     << thresh_s << "s, maybe requesting map" << dendl;
      _maybe_request_map();
    }
  }
...
Objecter::resend_mon_ops()
{
  ...
  for (auto p = check_latest_map_ops.begin(); p != check_latest_map_ops.end();
       ++p) {
  ...
  for (auto p = check_latest_map_lingers.begin();
    ...
    monc->get_version(
        "osdmap", CB_Linger_Map_Latest(this, p->second->linger_id));
  }
...
void
Objecter::op_post_submit(Op* op)
{
  boost::asio::post(service, [this, op]() { op_submit(op); });
}

void
Objecter::op_submit(Op* op, ceph_tid_t* ptid, int* ctx_budget)
{
...
void
Objecter::_send_op_account(Op* op)
{
  inflight_ops++;
...
struct op_cancellation {
  ...
  void
  operator()(asio::cancellation_type_t type)
  {
...
int
Objecter::op_cancel(OSDSession* s, ceph_tid_t tid, int r, bs::error_code ec)
{
  ceph_assert(initialized);
...
void
Objecter::subsystem_cancel(uint64_t subsystem, bs::error_code ec)
{
  unique_lock wl(rwlock);
...
int
Objecter::op_cancel(const vector<ceph_tid_t>& tids, int r)
{
  unique_lock wl(rwlock);
...
epoch_t
Objecter::op_cancel_writes(int r, int64_t pool)
{
  unique_lock wl(rwlock);
    ...
    bool any_change)
{
  if (OSDMap::primary_changed_broken( // https://tracker.ceph.com/issues/43213
          oldprimary, oldacting, newprimary, newacting))
    return true;
...
bool
Objecter::target_should_be_paused(op_target_t* t)
{
 ...
 */
bool
Objecter::osdmap_full_flag() const
{
  shared_lock rl(rwlock);
...
bool
Objecter::osdmap_pool_full(const int64_t pool_id) const
{
  shared_lock rl(rwlock);
...
bool
Objecter::_osdmap_pool_full(const int64_t pool_id) const
{
...
bool
Objecter::_osdmap_has_pool_full() const
{
  for (auto it = osdmap->get_pools().begin(); it != osdmap->get_pools().end();
       ++it) {
 ...
 */
bool
Objecter::_osdmap_full_flag() const
{
...
void
Objecter::update_pool_full_map(map<int64_t, bool>& pool_full_map)
{
...
int64_t
Objecter::get_object_hash_position(
    int64_t pool,
    const string& key,
    const string& ns)
{
...
int64_t
Objecter::get_object_pg_hash_position(
    int64_t pool,
    const string& key,
    const string& ns)
{
...
int
Objecter::_calc_target(op_target_t* t, bool any_change)
{
  // rwlock is locked
  ...
  if (!lookup_pg_mapping(
          actual_pgid, osdmap->get_epoch(), &up, &up_primary, &acting,
          &acting_primary)) {
    osdmap->pg_to_up_acting_osds(
        actual_pgid, &up, &up_primary, &acting, &acting_primary);
    pg_mapping_t pg_mapping(
        osdmap->get_epoch(), up, up_primary, acting, acting_primary);
  ...
  if (any_change &&
      PastIntervals::is_new_interval(
          ...
          sort_bitwise, t->recovery_deletes, recovery_deletes,
          ...
          t->allows_ecoptimizations, pi->allows_ecoptimizations(), prev_pgid)) {
    force_resend = true;
  }
  ...
  bool legacy_change = t->pgid != pgid ||
                       is_pg_changed(
  ...
  if (legacy_change || split_or_merge || force_resend) {
    ...
    if (t->force_shard) {
      ...
      if (pi->is_erasure()) {
        spgid.reset_shard(osdmap->pgtemp_undo_primaryfirst(
            *pi, actual_pgid, *t->force_shard));
      }
    ...
    ldout(cct, 10) << __func__ << " " << " raw pgid " << pgid << " -> actual "
                   << t->actual_pgid << " acting " << t->acting << " primary "
      ...
      if (t->flags & CEPH_OSD_FLAG_BALANCE_READS) {
        ...
        for (unsigned i = 0; i < t->acting.size(); ++i) {
          ...
          ldout(cct, 20) << __func__ << " localize: rank " << i << " osd."
                         << t->acting[i] << " locality " << locality << dendl;
          if (i == 0 ||
  ...
  if (split_or_merge &&
      (osdmap->require_osd_release >= ceph_release_t::luminous ||
       HAVE_FEATURE(
           osdmap->get_xinfo(acting_primary).features, RESEND_ON_SPLIT))) {
    return RECALC_OP_TARGET_NEED_RESEND;
  }
...
void
Objecter::_session_op_assign(OSDSession* to, Op* op)
{
  // to->lock is locked
  ceph_assert(op->session == NULL);
  ceph_assert(op->tid);
...
void
Objecter::_session_op_remove(OSDSession* from, Op* op)
{
...
void
Objecter::_session_linger_op_assign(OSDSession* to, LingerOp* op)
{
...
void
Objecter::_session_linger_op_remove(OSDSession* from, LingerOp* op)
{
...
void
Objecter::_session_command_op_remove(OSDSession* from, CommandOp* op)
{
  ceph_assert(from == op->session);
  // from->lock is locked
...
void
Objecter::_session_command_op_assign(OSDSession* to, CommandOp* op)
{
  // to->lock is locked
  ceph_assert(op->session == NULL);
  ceph_assert(op->tid);
...
void
Objecter::_cancel_linger_op(Op* op)
{
...
void
Objecter::_finish_op(Op* op, int r)
{
...
Objecter::MOSDOp*
Objecter::_prepare_osd_op(Op* op)
{
  // rwlock is locked
  ...
  auto m = new MOSDOp(
      client_inc, op->tid, hobj, op->target.actual_pgid, osdmap->get_epoch(),
      flags, op->features);
...
int
Objecter::calc_op_budget(const bc::small_vector_base<OSDOp>& ops)
{
  int op_budget = 0;
...
int
Objecter::take_linger_budget(LingerOp* info)
{
  return 1;
}

bs::error_code
Objecter::handle_osd_op_reply2(Op* op, vector<OSDOp>& out_ops)
{
  ...
  for (unsigned i = 0; p != out_ops.end() && pb != op->out_bl.end();
    ...
    if (*ph) {
      try {
        ...
        ldout(cct, 10) << "ERROR: tid " << op->tid
                       << ": handler function threw " << e.what() << dendl;
...
Objecter::handle_osd_op_reply(MOSDOpReply* m)
{
  ...
  if (m->is_redirect_reply()) {
    ...
    op->tid = 0;
    m->get_redirect().combine_with_locator(
        op->target.target_oloc, op->target.target_oid.name);
    op->target.flags |=
        (CEPH_OSD_FLAG_REDIRECTED | CEPH_OSD_FLAG_IGNORE_CACHE |
  ...
  logger->tinc(
      l_osdc_op_latency, ceph::coarse_mono_time::clock::now() - op->stamp);
  ...
  if (Op::has_completion(onfinish)) {
    if (rc == 0 && handler_error) {
      Op::complete(
          std::move(onfinish), handler_error, -EIO, service.get_executor());
    } else if (handler_error) {
      Op::complete(
          std::move(onfinish), handler_error, rc, service.get_executor());
    } else {
...
void
Objecter::handle_osd_backoff(MOSDBackoff* m)
{
  ldout(cct, 10) << __func__ << " " << *m << dendl;
  shunique_lock sul(rwlock, ceph::acquire_shared);
  if (!initialized) {
  ...
  case CEPH_OSD_BACKOFF_OP_BLOCK: {
    ...
    auto r = new MOSDBackoff(
        m->pgid, m->map_epoch, CEPH_OSD_BACKOFF_OP_ACK_BLOCK, m->id, m->begin,
        m->end);
...
uint32_t
Objecter::list_nobjects_seek(NListContext* list_context, uint32_t pos)
{
  shared_lock rl(rwlock);
  list_context->pos = hobject_t(
      object_t(), string(), CEPH_NOSNAP, pos, list_context->pool_id, string());
...
uint32_t
Objecter::list_nobjects_seek(NListContext* list_context, const hobject_t& cursor)
{
  shared_lock rl(rwlock);
...
void
Objecter::list_nobjects_get_cursor(NListContext* list_context, hobject_t* cursor)
{
  shared_lock rl(rwlock);
  if (list_context->list.empty()) {
    *cursor = list_context->pos;
    ...
    uint32_t h =
        osdmap->get_pg_pool(list_context->pool_id)->hash_key(*key, entry.nspace);
    *cursor = hobject_t(
        entry.oid, entry.locator, list_context->pool_snap_seq, h,
        list_context->pool_id, entry.nspace);
  }
}

void
Objecter::list_nobjects(NListContext* list_context, Context* onfinish)
{
  ...
  if (list_context->sort_bitwise != sort_bitwise) {
    list_context->pos = hobject_t(
        object_t(), string(), CEPH_NOSNAP, list_context->current_pg,
  ...
  ObjectOperation op;
  op.pg_nls(
      list_context->max_entries, list_context->filter, list_context->pos,
      osdmap->get_epoch());
  ...
  pg_read(
      list_context->current_pg, oloc, op, &list_context->bl, 0, onack,
...
void
Objecter::put_nlist_context_budget(NListContext* list_context)
{
    ...
    decltype(PoolOp::onfinish)&& onfinish)
{
  ...
  if (!p) {
    asio::defer(
        service.get_executor(),
        asio::append(
            std::move(onfinish), make_error_code(osdc_errc::pool_dne),
            cb::list{}));
    return;
  }
  if (p->snap_exists(snap_name)) {
    asio::defer(
        service.get_executor(),
        asio::append(
            std::move(onfinish), osdc_errc::snapshot_exists, cb::list{}));
    return;
  }
...
struct CB_SelfmanagedSnap {
  ...
  void
  operator()(bs::error_code ec, const cb::list& bl)
  {
    snapid_t snapid = 0;
    if (!ec) {
      try {
    ...
    decltype(PoolOp::onfinish)&& onfinish)
{
  ...
  if (!p->snap_exists(snap_name)) {
    asio::defer(
        service.get_executor(),
        asio::append(
            std::move(onfinish), make_error_code(osdc_errc::snapshot_dne),
            cb::list{}));
    return;
  }
    ...
    int crush_rule)
{
  ...
  if (osdmap->lookup_pg_pool_name(name) >= 0) {
    asio::defer(
        service.get_executor(),
        asio::append(
            std::move(onfinish), make_error_code(osdc_errc::pool_exists),
            cb::list{}));
    return;
  }
...
void
Objecter::delete_pool(int64_t pool, decltype(PoolOp::onfinish)&& onfinish)
{
  unique_lock wl(rwlock);
...
void
Objecter::_do_delete_pool(int64_t pool, decltype(PoolOp::onfinish)&& onfinish)
...
void
Objecter::pool_op_submit(PoolOp* op)
{
  // rwlock is locked
...
void
Objecter::_pool_op_submit(PoolOp* op)
{
  // rwlock is locked unique

  ldout(cct, 10) << "pool_op_submit " << op->tid << dendl;
  auto m = new MPoolOp(
      monc->get_fsid(), op->tid, op->pool, op->name, op->pool_op,
    ...
    if (osdmap->get_epoch() < m->epoch) {
      ...
      if (osdmap->get_epoch() < m->epoch) {
        ...
        _wait_for_new_map(
            asio::bind_executor(
                e,
                [o = std::move(op->onfinish), bl = std::move(bl),
                 e = service.get_executor()](bs::error_code ec) mutable {
        ...
        asio::defer(
            service.get_executor(),
            asio::append(std::move(op->onfinish), ec, std::move(bl)));
      }
    } else {
      ceph_assert(op->onfinish);
      asio::defer(
...
int
Objecter::pool_op_cancel(ceph_tid_t tid, int r)
{
  ceph_assert(initialized);
  ...
  if (op->onfinish)
    asio::defer(
        service.get_executor(),
        asio::append(std::move(op->onfinish), osdcode(r), cb::list{}));
...
void
Objecter::_finish_pool_op(PoolOp* op, int r)
{
  // rwlock is locked unique
...
void
Objecter::_poolstat_submit(PoolStatOp* op)
{
  ldout(cct, 10) << "_poolstat_submit " << op->tid << dendl;
  monc->send_mon_message(new MGetPoolStats(
      monc->get_fsid(), op->tid, op->pools, last_seen_pgmap_version));
  op->last_submit = ceph::coarse_mono_clock::now();
...
Objecter::handle_get_pool_stats_reply(MGetPoolStatsReply* m)
{
  ...
  if (iter != poolstat_ops.end()) {
    ...
    asio::defer(
        service.get_executor(), asio::append(
                                    std::move(op->onfinish), bs::error_code{},
...
int
Objecter::pool_stat_op_cancel(ceph_tid_t tid, int r)
{
  ceph_assert(initialized);
  ...
  if (op->onfinish)
    asio::defer(
        service.get_executor(),
        asio::append(
            std::move(op->onfinish), osdcode(r),
...
void
Objecter::_finish_pool_stat_op(PoolStatOp* op, int r)
{
  // rwlock is locked unique
...
void
Objecter::get_fs_stats_(
    std::optional<int64_t> poolid,
    decltype(StatfsOp::onfinish)&& onfinish)
{
  ...
  if (mon_timeout > timespan(0)) {
    op->ontimeout = timer.add_event(mon_timeout, [this, op]() {
      statfs_op_cancel(op->tid, -ETIMEDOUT);
    });
  } else {
    op->ontimeout = 0;
  }
...
void
Objecter::_fs_stats_submit(StatfsOp* op)
{
  // rwlock is locked unique

  ldout(cct, 10) << "fs_stats_submit" << op->tid << dendl;
  monc->send_mon_message(new MStatfs(
      monc->get_fsid(), op->tid, op->data_pool, last_seen_pgmap_version));
  op->last_submit = ceph::coarse_mono_clock::now();
...
Objecter::handle_fs_stats_reply(MStatfsReply* m)
{
  ...
  if (statfs_ops.count(tid)) {
    ...
    asio::defer(
        service.get_executor(),
        asio::append(std::move(op->onfinish), bs::error_code{}, m->h.st));
...
int
Objecter::statfs_op_cancel(ceph_tid_t tid, int r)
{
  ceph_assert(initialized);
  ...
  if (op->onfinish)
    asio::defer(
        service.get_executor(),
        asio::append(std::move(op->onfinish), osdcode(r), ceph_statfs{}));
  _finish_statfs_op(op, r);
  return 0;
}

void
Objecter::_finish_statfs_op(StatfsOp* op, int r)
{
  // rwlock is locked unique
...
void
Objecter::ms_handle_connect(Connection* con)
{
...
bool
Objecter::ms_handle_reset(Connection* con)
{
  if (!initialized)
    return false;
...
void
Objecter::ms_handle_remote_reset(Connection* con)
{
  /*
...
bool
Objecter::ms_handle_refused(Connection* con)
{
  // just log for now
...
void
Objecter::op_target_t::dump(Formatter* f) const
{
...
void
Objecter::_dump_active(OSDSession* s)
{
  for (auto p = s->ops.begin(); p != s->ops.end(); ++p) {
    Op* op = p->second;
...
void
Objecter::dump_requests(Formatter* fmt)
{
...
void
Objecter::_dump_ops(const OSDSession* s, Formatter* fmt)
{
  for (auto p = s->ops.begin(); p != s->ops.end(); ++p) {
    Op* op = p->second;
    auto age = std::chrono::duration<double>(
        ceph::coarse_mono_clock::now() - op->stamp);
...
void
Objecter::dump_ops(Formatter* fmt)
{
...
void
Objecter::_dump_linger_ops(const OSDSession* s, Formatter* fmt)
{
  for (auto p = s->linger_ops.begin(); p != s->linger_ops.end(); ++p) {
    auto op = p->second;
...
void
Objecter::dump_linger_ops(Formatter* fmt)
{
...
void
Objecter::_dump_command_ops(const OSDSession* s, Formatter* fmt)
{
  for (auto p = s->command_ops.begin(); p != s->command_ops.end(); ++p) {
    auto op = p->second;
...
void
Objecter::dump_command_ops(Formatter* fmt)
{
...
void
Objecter::dump_pool_ops(Formatter* fmt) const
{
...
void
Objecter::dump_pool_stat_ops(Formatter* fmt) const
{
...
void
Objecter::dump_statfs_ops(Formatter* fmt) const
{
...
int
Objecter::RequestStateHook::call(
    std::string_view command,
    const cmdmap_t& cmdmap,
    const bufferlist&,
    Formatter* f,
...
void
Objecter::blocklist_self(bool set)
{
...
Objecter::handle_command_reply(MCommandReply* m)
{
  ...
  _finish_command(
      c, m->r < 0 ? bs::error_code(-m->r, osd_category()) : bs::error_code(),
      std::move(m->rs), std::move(m->get_data()));
  sul.unlock();
...
void
Objecter::submit_command(CommandOp* c, ceph_tid_t* ptid)
{
  shunique_lock sul(rwlock, ceph::acquire_unique);
  ...
  if (osd_timeout > timespan(0)) {
    c->ontimeout = timer.add_event(osd_timeout, [this, c, tid]() {
      command_op_cancel(c->session, tid, osdc_errc::timed_out);
    });
  }
    ...
    shunique_lock<ceph::shared_mutex>& sul)
{
  ...
  ceph_assert(
      r != -EAGAIN); /* shouldn't happen as we're holding the write lock */
...
void
Objecter::_assign_command_session(
    CommandOp* c,
    shunique_lock<ceph::shared_mutex>& sul)
{
  ceph_assert(sul.owns_lock() && sul.mutex() == &rwlock);
...
void
Objecter::_send_command(CommandOp* c)
{
...
int
Objecter::command_op_cancel(OSDSession* s, ceph_tid_t tid, bs::error_code ec)
{
  ceph_assert(initialized);
    ...
    cb::list&& bl)
{
  ...
  ldout(cct, 10) << "_finish_command " << c->tid << " = " << ec << " " << rs
                 << dendl;

  if (c->onfinish)
    asio::defer(
        service.get_executor(),
        asio::append(std::move(c->onfinish), ec, std::move(rs), std::move(bl)));
...
{
  mon_timeout =
      cct->_conf.get_val<std::chrono::seconds>("rados_mon_op_timeout");
  osd_timeout =
      cct->_conf.get_val<std::chrono::seconds>("rados_osd_op_timeout");

  auto read_policy =
      cct->_conf.get_val<std::string>("rados_replica_read_policy");
  if (read_policy == "localize") {
 ...
 */
void
Objecter::set_epoch_barrier(epoch_t epoch)
{
  unique_lock wl(rwlock);
...
private:
  fu2::unique_function<void(bs::error_code, std::vector<T>, hobject_t) &&>
      on_finish;
...
public:
  ...
  void
  operator()(bs::error_code ec, std::vector<T> v, hobject_t h) &&
  {
    if (budget >= 0) {
        ...
        on_finish)
{
  ...
  _issue_enumerate(
      start, std::make_unique<EnumerationContext<T>>(
                 this, std::move(end), filter_bl, max,
                 object_locator_t{pool_id, ns}, std::move(on_finish)));
}
...
template void Objecter::enumerate_objects<librados::ListObjectImpl>(
    ...
    const cb::list& filter_bl,
    fu2::unique_function<
        void(bs::error_code, std::vector<librados::ListObjectImpl>, hobject_t) &&>
        on_finish);
...
template void Objecter::enumerate_objects<neorados::Entry>(
    ...
    const cb::list& filter_bl,
    fu2::unique_function<
        void(bs::error_code, std::vector<neorados::Entry>, hobject_t) &&>
        on_finish);

template <typename T>
void
Objecter::_issue_enumerate(
    hobject_t start,
    std::unique_ptr<EnumerationContext<T>> ctx)
{
  ObjectOperation op;
  ...
  auto e = boost::asio::prefer(
      service.get_executor(), boost::asio::execution::outstanding_work.tracked);
  pg_read(
      start.get_hash(), c->oloc, op, pbl, 0,
      asio::bind_executor(
          e, [c = std::move(on_ack)](bs::error_code ec) mutable { (*c)(ec); }),
      epoch, budget);
}
...
template void Objecter::_issue_enumerate<neorados::Entry>(
    hobject_t start,
    ...
    std::unique_ptr<EnumerationContext<T>>&& ctx)
{
  ...
  if ((response.handle <= ctx->end)) {
    ...
    while (!response.entries.empty()) {
      uint32_t hash = response.entries.back().locator.empty()
                          ? pool->hash_key(
                                response.entries.back().oid,
                                response.entries.back().nspace)
                          : pool->hash_key(
                                response.entries.back().locator,
                                response.entries.back().nspace);
      hobject_t last(
          response.entries.back().oid, response.entries.back().locator,
          CEPH_NOSNAP, hash, ctx->oloc.get_pool(),
          response.entries.back().nspace);
      if (last < ctx->end)
        break;
  ...
  if (response.entries.size() <= ctx->max) {
    ...
    uint32_t hash = i->locator.empty() ? pool->hash_key(i->oid, i->nspace)
                                       : pool->hash_key(i->locator, i->nspace);

    next = hobject_t{i->oid, i->locator,           CEPH_NOSNAP,
                     hash,   ctx->oloc.get_pool(), i->nspace};
  }
...
template void Objecter::_enumerate_reply<neorados::Entry>(
    cb::list&& bl,
    bs::error_code ec,
    std::unique_ptr<EnumerationContext<neorados::Entry>>&& ctx);
...
template <typename T>
void
do_decode(std::vector<T>& items, std::vector<cb::list>& bls)
{
  for (auto bl : bls) {
    auto p = bl.cbegin();
    T t;
