...
osdcode(int r)
{
  return (r < 0) ? boost::system::error_code(-r, osd_category())
...
constexpr static uint64_t kReplicaMinReadSize = kReplicaMinShardReadSize *
                                                kReplicaMinShardReads;

#undef dout_prefix
#define dout_prefix *_dout << " ECSplitOp::"

std::pair<SplitOp::extent_set, bufferlist>
ECSplitOp::assemble_buffer_sparse_read(int ops_index)
{
  bufferlist bl_out;
  ...
  const pg_pool_t* pi =
      objecter.osdmap->get_pg_pool(orig_op->target.base_oloc.pool);
  ECStripeView stripe_view(
      orig_osd_op.extent.offset, orig_osd_op.extent.length, pi);
  ldout(cct, DBG_LVL) << __func__ << " start:" << orig_osd_op.extent.offset
                      << "~" << orig_osd_op.extent.length << dendl;

  std::vector<uint64_t> buffer_offset(stripe_view.data_chunk_count);
  mini_flat_map<shard_id_t, extents_map::iterator> map_iterators(
      stripe_view.data_chunk_count);
  ...
  for (auto&& chunk_info : stripe_view) {
    ...
    uint64_t bl_len = 0;
    while (extent_iter != details.e->end() &&
           extent_iter->first < chunk_info.ro_offset + stripe_view.chunk_size) {
      auto [off, len] = *extent_iter;
      ldout(cct, DBG_LVL) << __func__ << " extent=" << off << "~" << len
                          << dendl;
...
void
ECSplitOp::assemble_buffer_read(bufferlist& bl_out, int ops_index)
{
  auto& orig_osd_op = orig_op->ops[ops_index].op;
  const pg_pool_t* pi =
      objecter.osdmap->get_pg_pool(orig_op->target.base_oloc.pool);
  ...
  std::vector<uint64_t> buffer_offset(stripe_view.data_chunk_count);
  ldout(cct, DBG_LVL) << __func__ << " " << orig_osd_op.extent.offset << "~"
                      << orig_osd_op.extent.length << dendl;
  ...
  for (auto&& chunk_info : stripe_view) {
    ...
    bufferlist bl;
    bl.substr_of(
        details.bl, buffer_offset[(int)chunk_info.shard], chunk_info.length);
    bl_out.append(bl);
...
void
ECSplitOp::init_read(OSDOp& op, bool sparse, int ops_index)
{
  ...
  for (unsigned i = first_shard; i < first_shard + count; i++) {
    ...
    if (sparse) {
      d.e.emplace();
      sub_reads.at(shard).rd.sparse_read(
          offset, length, &(*d.e), &d.bl, &d.rval);
    } else {
...
std::pair<SplitOp::extent_set, bufferlist>
ReplicaSplitOp::assemble_buffer_sparse_read(int ops_index)
{
  extent_set extents_out;
  bufferlist bl_out;
...
void
ReplicaSplitOp::assemble_buffer_read(bufferlist& bl_out, int ops_index)
{
...
void
ReplicaSplitOp::init_read(OSDOp& op, bool sparse, int ops_index)
{
  ...
  uint64_t slice_count =
      std::min(length / kReplicaMinShardReadSize, osds.size());
  uint64_t chunk_size =
      p2roundup(length / slice_count, (uint64_t)CEPH_PAGE_SIZE);
...
SplitOp::complete()
{
  ...
  if (rc >= 0) {
    ...
    ldout(cct, DBG_LVL) << __func__ << " success this=" << this << " rc=" << rc
                        << dendl;
    Objecter::Op::complete(
        std::move(orig_op->onfinish), osdcode(rc), rc,
        objecter.service.get_executor());
    objecter._finish_op(orig_op, rc);
  } else {
    ldout(cct, DBG_LVL) << __func__ << " retry this=" << this << " rc=" << rc
                        << dendl;
...
static bool
validate(Objecter::Op* op, bool is_erasure, CephContext* cct)
{

  if ((op->target.flags & CEPH_OSD_FLAG_BALANCE_READS) == 0) {
    ldout(cct, DBG_LVL) << __func__ << " REJECT: Client rejects balanced read"
                        << dendl;
    return false;
  }
...
void
SplitOp::init(OSDOp& op, int ops_index)
{
  switch (op.op.op) {
  case CEPH_OSD_OP_SPARSE_READ: {
...
namespace {
void
debug_op_summary(const std::string& str, Objecter::Op* op, CephContext* cct)
{
  auto& t = op->target;
  ldout(cct, DBG_LVL) << str << " osd=" << t.osd << " shard="
                      << (t.force_shard ? *t.force_shard : shard_id_t(-1))
                      << " balance_reads="
    ...
    CephContext* cct)
{
  ...
  ldout(cct, DBG_LVL) << __func__ << " sub_reads ready. count="
                      << split_read->sub_reads.size() << dendl;
