...
object_t
format_oid(const char* object_format, uint64_t object_no)
{
...
struct OrderByObject {
  constexpr bool
  operator()(uint64_t object_no, const striper::LightweightObjectExtent& rhs) const
  {
    return object_no < rhs.object_no;
  }

  constexpr bool
  operator()(const striper::LightweightObjectExtent& lhs, uint64_t object_no) const
  {
    ...
    uint64_t buffer_offset)
{
  ...
  file_to_extents(
      cct, layout, offset, len, trunc_size, buffer_offset,
    ...
    striper::LightweightObjectExtents* object_extents)
{
  ...
  while (left > 0) {
    ...
    auto it = std::upper_bound(
        object_extents->begin(), object_extents->end(), objectno,
        OrderByObject());
...
uint64_t
Striper::get_num_objects(const file_layout_t& layout, uint64_t size)
{
  ...
  if ((remainder_bytes > 0) &&
    ...
    remainder_objs = stripe_count -
                     ((remainder_bytes + stripe_unit - 1) / stripe_unit);
...
void
Striper::StripedReadResult::add_partial_result(
    CephContext* cct,
    bufferlist& bl,
    ...
    const striper::LightweightBufferExtents& buffer_extents)
{
  ...
  for (auto& be : buffer_extents) {
    ...
    size_t actual = std::min<uint64_t>(
        bl.length(), be.second); //NOLINT(bugprone-use-after-move)
    ...
    const std::vector<pair<uint64_t, uint64_t>>& buffer_extents)
{
  ...
  for (auto& be : buffer_extents) {
    ::add_partial_sparse_result(
        cct, &partial, &total_intended_len, bl, &s, bl_map.end(), &bl_off,
        be.first, be.second);
  }
}
    ...
    const striper::LightweightBufferExtents& buffer_extents)
{
  ...
  for (auto& be : buffer_extents) {
    ::add_partial_sparse_result(
        cct, &partial, &total_intended_len, bl, &s, bl_map.cend(), &bl_off,
        be.first, be.second);
  }
}

void
Striper::StripedReadResult::assemble_result(
    CephContext* cct,
    bufferlist& bl,
    bool zero_tail)
{
