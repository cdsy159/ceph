...
public:
  ...
  void
  dump_formatted(ceph::Formatter* f) const
  {
...
protected:
  ...
  void
  dump_formatted_values(ceph::Formatter* f) const
  {
    visit_values(
        [f](int) { f->open_array_section("values"); },
  ...
  template <typename... T>
  int64_t
  get_raw_index_for_value(T... axes) const
  {
  ...
  template <typename... T>
  int64_t
  get_raw_index_for_bucket(T... buckets) const
  {
    static_assert(sizeof...(T) == DIM, "Incorrect number of arguments");
  ...
  int64_t
  get_raw_index_internal(F, int64_t startIndex) const
  {
      ...
      int startIndex = 0) const
  {
    ...
    for (int32_t i = 0; i < ac.m_buckets; ++i, ++startIndex) {
      visit_values(
          onDimensionEnter, onValue, onDimensionLeave, level + 1, startIndex);
    }
