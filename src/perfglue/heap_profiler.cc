...
void
ceph_heap_profiler_stats(char* buf, int length)
{
...
void
ceph_heap_set_release_rate(double val)
{
  MallocExtension::instance()->SetMemoryReleaseRate(val);
}

bool
ceph_heap_get_numeric_property(const char* property, size_t* value)
{
  return MallocExtension::instance()->GetNumericProperty(property, value);
}

bool
ceph_heap_set_numeric_property(const char* property, size_t value)
{
  return MallocExtension::instance()->SetNumericProperty(property, value);
}

bool
ceph_heap_profiler_running()
{
...
static void
get_profile_name(char* profile_name, int profile_name_len)
{
...
#pragma GCC diagnostic ignored "-Wformat-truncation"
  ...
  if (last_slash == NULL) {
    snprintf(
        profile_name, profile_name_len, "./%s.profile",
        g_conf()->name.to_cstr());
  } else {
    last_slash[1] = '\0';
    snprintf(
        profile_name, profile_name_len, "%s/%s.profile", path,
        g_conf()->name.to_cstr());
  }
#if __GNUC__ && __GNUC__ >= 8
...
ceph_heap_profiler_start()
{
  ...
  generic_dout(0) << "turning on heap profiler with prefix " << profile_name
                  << dendl;
...
void
ceph_heap_profiler_dump(const char* reason)
{
#ifdef HAVE_LIBTCMALLOC
    ...
    std::ostream& out)
{
  ...
  if (cmd.size() == 1 && cmd[0] == "dump") {
    ...
    out << g_conf()->name << " release rate: " << std::setprecision(4)
        << ceph_heap_get_release_rate() << "\n";
    ...
    try {
      ...
      out << g_conf()->name
          << " release rate changed to: " << std::setprecision(4)
          << ceph_heap_get_release_rate() << "\n";
    } catch (...) {
