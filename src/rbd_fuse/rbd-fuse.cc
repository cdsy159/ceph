...
enumerate_images(struct rbd_image_data* data)
{
  ...
  while (true) {
    ...
    if (ret == -ERANGE) {
      data->image_specs = static_cast<rbd_image_spec_t*>(realloc(
          data->image_specs, sizeof(rbd_image_spec_t) * data->image_spec_count));
    } else if (ret < 0) {
...
static void
iter_images(void* cookie, void (*iter)(void* cookie, const char* image))
{
  struct rbd_image* im;
...
static void
count_images_cb(void* cookie, const char* image)
{
  (*((unsigned int*)cookie))++;
}
...
static int
rbdfs_open(const char* path, struct fuse_file_info* fi)
{
  int fd;
    ...
    struct fuse_file_info* fi)
{
  ...
  while (size > 0) {
    ...
    if ((size_t)(offset + size) > rbdsize(fi->fh)) {
      int r;
      fprintf(
          stderr, "rbdfs_write resizing %s to 0x%" PRIxMAX "\n", path,
          offset + size);
      ...
      r = rbd_stat(
          rbd->image, &(rbd->rbd_stat.rbd_info), sizeof(rbd_image_info_t));
      if (r < 0)
        return r;
    }
...
static void
rbdfs_statfs_image_cb(void* num, const char* image)
{
  int fd;
...
static int
rbdfs_statfs(const char* path, struct statvfs* buf)
{
  uint64_t num[2];
...
static int
rbdfs_fsync(const char* path, int datasync, struct fuse_file_info* fi)
{
  if (!gotrados)
    return -ENXIO;
...
static int
rbdfs_opendir(const char* path, struct fuse_file_info* fi)
{
...
static void
rbdfs_readdir_cb(void* _info, const char* name)
{
...
static int
rbdfs_releasedir(const char* path, struct fuse_file_info* fi)
{
...
)
{
  ...
  if (ret < 0)
    ...
    FUSE_VERSION < FUSE_MAKE_VERSION(3, 0)
...
)
{
  ...
  rbd = &opentbl[fd];
  fprintf(
      stderr, "truncate %s to %" PRIdMAX " (0x%" PRIxMAX ")\n", path, size,
      size);
  ...
  for (ap = attrs; ap->attrname != NULL; ap++) {
    if (strcmp(name, ap->attrname) == 0) {
      *ap->attrvalp = strtoull(value, NULL, 0);
      fprintf(
          stderr, "rbd-fuse: %s set to 0x%" PRIx64 "\n", ap->attrname,
          *ap->attrvalp);
      return 0;
    }
  }
...
usage(const char* progname)
{
  fprintf(
      ...
      "    -c   --conf            ceph configuration file "
      "[/etc/ceph/ceph.conf]\n"
...
static int
rbdfs_opt_proc(void* data, const char* arg, int key, struct fuse_args* outargs)
{
  if (key == KEY_HELP) {
...
main(int argc, const char* argv[])
{
  ...
  std::map<std::string, int> filter_options = {
      {"-r", OPT_NOT_FOUND}, {"-f", OPT_NOT_FOUND}, {"-d", OPT_NOT_FOUND}};
  ...
  auto cct = global_init(
      NULL, arg_vector, CEPH_ENTITY_TYPE_CLIENT, CODE_ENVIRONMENT_DAEMON,
  ...
  struct fuse_args args =
      FUSE_ARGS_INIT((int)arg_vector.size(), (char**)&arg_vector.front());
