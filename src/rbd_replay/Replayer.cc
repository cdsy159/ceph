...
bool
is_versioned_replay(BufferReader& buffer_reader)
{
...
void
Worker::add_pending(PendingIO::ptr io)
{
  ceph_assert(io);
...
Worker::run()
{
  ...
  }
  {
    ...
    while (!m_pending_ios.empty()) {
      if (!first_time) {
        dout(THREAD_LEVEL) << "Worker thread trying to stop, still waiting for "
                           << m_pending_ios.size()
...
void
Worker::remove_pending(PendingIO::ptr io)
{
  ceph_assert(io);
...
void
Worker::put_image(imagectx_id_t imagectx_id, librbd::Image* image)
{
  ceph_assert(image);
...
void
Worker::erase_image(imagectx_id_t imagectx_id)
{
...
void
Worker::set_action_complete(action_id_t id)
{
  m_replayer.set_action_complete(id);
}

bool
Worker::readonly() const
{
  return m_replayer.readonly();
}

rbd_loc
Worker::map_image_name(std::string image_name, std::string snap_name) const
{
...
{
  assertf(
      num_action_trackers > 0, "num_action_trackers = %d", num_action_trackers);
}

Replayer::~Replayer() { delete[] m_action_trackers; }

Replayer::action_tracker_d&
...
void
Replayer::run(const std::string& replay_file)
{
  {
    ...
    m_ioctx = new librados::IoCtx();
    {
      ...
      if (r) {
        cerr << "Failed to open pool " << m_pool_name << ": " << cpp_strerror(r)
             << std::endl;
        goto out2;
      }
      ...
      while (true) {
        ...
        try {
          ...
          std::cerr << "Failed to decode trace action: " << err.what()
                    << std::endl;
          exit(1);
        }
...
void
Replayer::put_image(imagectx_id_t imagectx_id, librbd::Image* image)
{
  ceph_assert(image);
...
void
Replayer::erase_image(imagectx_id_t imagectx_id)
{
  std::unique_lock lock{m_images_mutex};
...
void
Replayer::set_action_complete(action_id_t id)
{
...
bool
Replayer::is_action_complete(action_id_t id)
{
  action_tracker_d& tracker = tracker_for(id);
...
void
Replayer::wait_for_actions(const action::Dependencies& deps)
{
...
void
Replayer::set_latency_multiplier(float f)
{
  assertf(f >= 0, "f = %f", f);
...
bool
Replayer::readonly() const
{
  return m_readonly;
}

void
Replayer::set_readonly(bool readonly)
{
  m_readonly = readonly;
}
