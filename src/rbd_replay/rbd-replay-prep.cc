...
public:
  ...
  void
  issued_io(IO::ptr io, std::set<IO::ptr>* latest_ios)
  {
    ceph_assert(io);
...
public:
  void
  init(const string& image_name, int index)
  {
...
usage(const string& prog)
{
  ...
  str << "Usage: " << prog << " ";
  std::cout << str.str()
            << "[ --window <seconds> ] [ --anonymize ] [ --verbose ]"
            << std::endl
...
usage_exit(const string& prog, const string& msg)
{
  cerr << msg << endl;
  usage(prog);
  exit(1);
...
public:
  ...
  run(vector<string> args)
  {
    ...
    int trace_handle = bt_context_add_trace(
        ctx,
    ...
    uint64_t start_time_ns =
        bt_trace_handle_get_timestamp_begin(ctx, trace_handle, BT_CLOCK_REAL);
    ASSERT_EXIT(
        start_time_ns != -1ULL, "Error extracting creation time from trace");

    struct bt_ctf_iter* itr = bt_ctf_iter_create(
        ctx,
        NULL, // begin_pos
        NULL); // end_pos
    ceph_assert(itr);
    ...
    int fd = open(
        output_file_name.c_str(), O_WRONLY | O_CREAT | O_EXCL | O_BINARY, 0644);
    ASSERT_EXIT(
        fd >= 0, "Error opening output file " << output_file_name << ": "
                                              << cpp_strerror(errno));
    ...
    while (true) {
      ...
      ts -= trace_start;
      ts +=
          4; // This is so we have room to insert two events (thread start and open image) at unique timestamps before whatever the first event is.
...
private:
  ...
  void
  serialize_events(int fd, const IO::ptrs& ptrs)
  {
  ...
  insert_thread_stops(int fd)
  {
    ...
    for (map<thread_id_t, Thread::ptr>::const_iterator itr = m_threads.begin(),
      ...
      ios.push_back(IO::ptr(new StopThreadIO(
          next_id(), thread->max_ts(), thread->id(), m_recent_completions)));
    }
    serialize_events(fd, ios);
  }

  void
  process_event(uint64_t ts, struct bt_ctf_event* evt, IO::ptrs* ios)
  {
    const char* event_name = bt_ctf_event_name(evt);
    const struct bt_definition* scope_context =
        bt_ctf_get_top_level_scope(evt, BT_STREAM_EVENT_CONTEXT);
    ASSERT_EXIT(scope_context != NULL, "Error retrieving event context");

    const struct bt_definition* scope_fields =
        bt_ctf_get_top_level_scope(evt, BT_EVENT_FIELDS);
    ASSERT_EXIT(scope_fields != NULL, "Error retrieving event fields");

    const struct bt_definition* pthread_id_field =
        bt_ctf_get_field(evt, scope_context, "pthread_id");
    ...
    public:
      ...
      string(const char* name)
      {
        ...
        ASSERT_EXIT(
            c && err == 0,
            "Error retrieving field value '" << name << "': error=" << err);
        return c;
      }
    ...
    if (strcmp(event_name, "librbd:open_image_enter") == 0) {
      ...
      IO::ptr io(new OpenImageIO(
          ionum, ts, threadID, m_recent_completions, imagectx, aname.first,
      ...
      boost::shared_ptr<OpenImageIO> io(
          boost::dynamic_pointer_cast<OpenImageIO>(thread->latest_io()));
      ceph_assert(io);
      m_open_images.insert(io->imagectx());
    } else if (strcmp(event_name, "librbd:close_image_enter") == 0) {
      imagectx_id_t imagectx = fields.uint64("imagectx");
      action_id_t ionum = next_id();
      IO::ptr io(new CloseImageIO(
          ionum, ts, threadID, m_recent_completions, imagectx));
      thread->issued_io(io, &m_latest_ios);
      ios->push_back(thread->latest_io());
    } else if (strcmp(event_name, "librbd:close_image_exit") == 0) {
      completed(thread->latest_io());
      boost::shared_ptr<CloseImageIO> io(
          boost::dynamic_pointer_cast<CloseImageIO>(thread->latest_io()));
      ceph_assert(io);
      ...
      uint64_t completion = fields.uint64("completion");
      action_id_t ionum = next_id();
      IO::ptr io(new AioCloseImageIO(
          ionum, ts, threadID, m_recent_completions, imagectx));
      thread->issued_io(io, &m_latest_ios);
      ios->push_back(thread->latest_io());
      m_pending_ios[completion] = io;
    } else if (
        strcmp(event_name, "librbd:read_enter") == 0 ||
      ...
      action_id_t ionum = next_id();
      IO::ptr io(new ReadIO(
          ionum, ts, threadID, m_recent_completions, imagectx, offset, length));
      thread->issued_io(io, &m_latest_ios);
      ios->push_back(io);
    } else if (strcmp(event_name, "librbd:read_exit") == 0) {
      completed(thread->latest_io());
    } else if (
        strcmp(event_name, "librbd:write_enter") == 0 ||
      ...
      completed(thread->latest_io());
        ...
        strcmp(event_name, "librbd:aio_read_enter") == 0 ||
      ...
      require_image(ts, thread, imagectx, name, snap_name, readonly, ios);
      action_id_t ionum = next_id();
      IO::ptr io(new AioReadIO(
          ionum, ts, threadID, m_recent_completions, imagectx, offset, length));
      ios->push_back(io);
      thread->issued_io(io, &m_latest_ios);
      m_pending_ios[completion] = io;
    } else if (
        strcmp(event_name, "librbd:aio_write_enter") == 0 ||
  ...
  {
    uint64_t limit =
        (io->start_time() < m_window ? 0 : io->start_time() - m_window);
  ...
  pair<string, string>
  map_image_snap(string image_name, string snap_name)
  {
    if (!m_anonymize) {
      ...
      IO::ptrs* ios)
  {
    ...
    pair<string, string> aname(map_image_snap(name, snap_name));
    IO::ptr io(new OpenImageIO(
        ionum, ts - 2, thread->id(), m_recent_completions, imagectx,
        aname.first, aname.second, readonly));
    thread->issued_io(io, &m_latest_ios);
    ios->push_back(io);
    completed(io);
