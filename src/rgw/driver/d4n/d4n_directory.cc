    ...
    optional_yield y)
{
  if (!redis_pool) [[unlikely]] {
    ...
    ldpp_dout(dpp, 0)
        << "Directory::" << __func__
        << " not using connection-pool, it's using the shared connection "
        << dendl;
  } else [[likely]]
    ...
    optional_yield y)
{
  ...
  try {
    ...
    if (ec) {
      ldpp_dout(dpp, 0) << "BucketDirectory::" << __func__
                        << "() ERROR: " << ec.what() << dendl;
      return -ec.value();
    }
  ...
  try {
    ...
    if (pipeline && pipeline->is_pipeline()) {
      ...
      if (std::get<0>(resp).value() != "1") {
        ldpp_dout(dpp, 10) << "BucketDirectory::" << __func__
                           << "() Response value is: "
    ...
    optional_yield y)
{
  try {
    ...
    if (std::get<0>(resp).value() != "1") {
      ldpp_dout(dpp, 10) << "BucketDirectory::" << __func__
                         << "() Response is: " << std::get<0>(resp).value()
                         << dendl;
      return -ENOENT;
    }
    ...
    optional_yield y)
{
  try {
    ...
    if (offset == 0 && count == 0) {
      ...
      req.push(
          "ZRANGE", bucket_id, start, stop, "bylex", "LIMIT", offset, count);
    }
    ...
    optional_yield y)
{
  try {
    ...
    std::vector<boost::redis::resp3::basic_node<std::__cxx11::basic_string<char>>>
        root_array;
    if (resp.has_value()) {
      root_array = resp.value();
      ldpp_dout(dpp, 20) << "BucketDirectory::" << __func__
                         << "() aggregate size is: " << root_array.size()
                         << dendl;
      ...
      if (size >= 2) {
        ...
        for (uint64_t i = 3; i < size; i = i + 2) {
          ...
          ldpp_dout(dpp, 20)
              << "BucketDirectory::" << __func__
              << "() member is: " << root_array[i].value << dendl;
        }
      }
    } else {
      return -ENOENT;
...
std::string
ObjectDirectory::build_index(CacheObj* object)
{
  return object->bucketName + "_" + object->objName;
}

int
ObjectDirectory::exist_key(
    const DoutPrefixProvider* dpp,
    CacheObj* object,
    optional_yield y)
{
  ...
  try {
    ...
    if (ec) {
      ldpp_dout(dpp, 0) << "ObjectDirectory::" << __func__
                        << "() ERROR: " << ec.what() << dendl;
      return -ec.value();
    }
  } catch (std::exception& e) {
    ...
    optional_yield y)
{
  ...
  if ((ret = check_bool(std::to_string(object->dirty))) != -EINVAL) {
    ...
    ldpp_dout(dpp, 0) << "ObjectDirectory::" << __func__
                      << "() ERROR: Invalid bool value" << dendl;
    return -EINVAL;
  }
    ...
    optional_yield y)
{
  ...
  ldpp_dout(dpp, 10) << "ObjectDirectory::" << __func__
                     << "(): index is: " << key << dendl;
  ...
  try {
    ...
    object->bucketName =
        std::get<0>(resp).value()[std::size_t(Fields::BucketName)];
    object->creationTime =
        std::get<0>(resp).value()[std::size_t(Fields::CreationTime)];
    object->dirty =
        (std::stoi(std::get<0>(resp).value()[std::size_t(Fields::Dirty)]) != 0);
    boost::split(
        object->hostsList,
        std::get<0>(resp).value()[std::size_t(Fields::Hosts)],
        boost::is_any_of("_"));
    object->etag = std::get<0>(resp).value()[std::size_t(Fields::Etag)];
    object->size =
        std::stoull(std::get<0>(resp).value()[std::size_t(Fields::ObjSize)]);
    object->user_id = std::get<0>(resp).value()[std::size_t(Fields::UserID)];
    object->display_name =
        std::get<0>(resp).value()[std::size_t(Fields::DisplayName)];
  } catch (std::exception& e) {
    ldpp_dout(dpp, 0) << "ObjectDirectory::" << __func__
                      << "() ERROR: " << e.what() << dendl;
    return -EINVAL;
  ...
  if ((ret = exist_key(dpp, object, y))) {
    try {
      if (field == "hosts") {
        ...
        ldpp_dout(dpp, 20) << "ObjectDirectory::" << __func__
                           << "(): Appending to hosts list." << dendl;
...
int
ObjectDirectory::zremrangebyscore(
    const DoutPrefixProvider* dpp,
    CacheObj* object,
    double min,
    double max,
...
std::string
BlockDirectory::build_index(CacheBlock* block)
{
  return block->cacheObj.bucketName + "_" + block->cacheObj.objName + "_" +
         std::to_string(block->blockID) + "_" + std::to_string(block->size);
}

int
BlockDirectory::exist_key(
    const DoutPrefixProvider* dpp,
    CacheBlock* block,
    optional_yield y)
{
  ...
  try {
    ...
    if (ec) {
      ldpp_dout(dpp, 0) << "BlockDirectory::" << __func__
                        << "() ERROR: " << ec.what() << dendl;
      return -ec.value();
    }
  } catch (std::exception& e) {
...
int
BlockDirectory::set_values(
    const DoutPrefixProvider* dpp,
    CacheBlock& block,
    Container& redisValues,
    optional_yield y)
{
  std::string hosts;
  /* Creating a redisValues of the entry's properties */
    ...
    Pipeline* pipeline)
{
  /* For existing keys, call get method beforehand. 
     Sets completely overwrite existing values. */
  std::string key = build_index(block);
  ldpp_dout(dpp, 10) << "BlockDirectory::" << __func__
                     << "(): index is: " << key << dendl;
...
constexpr void
constexpr_for_each(std::integer_sequence<Integer, I...>, F&& func)
{
...
constexpr void
constexpr_for(F&& func)
{
  if constexpr (N > 0) {
    constexpr_for_each(
        std::make_integer_sequence<decltype(N), N>{}, std::forward<F>(func));
  }
}

template <typename T>
void
parse_response(T t, std::vector<std::vector<std::string>>& responses)
{
...
template int BlockDirectory::get<100>(
    const DoutPrefixProvider* dpp,
    std::vector<CacheBlock>& blocks,
    optional_yield y);
    ...
    optional_yield y)
{
  ...
  for (size_t i = 0; i < blocks.size(); i++) {
    ...
    if (vec.empty()) {
      ldpp_dout(dpp, 0) << "BlockDirectory::" << __func__
                        << "(): No values returned for key="
                        << build_index(block) << dendl;
      continue;
    }
    ...
    block->deleteMarker =
        (std::stoi(vec[std::size_t(Fields::DeleteMarker)]) != 0);
    ...
    boost::split(
        block->cacheObj.hostsList, vec[std::size_t(Fields::Hosts)],
        boost::is_any_of("_"));
    ...
    optional_yield y)
{
  ...
  try {
    ...
    if (std::get<0>(resp).value().value().empty()) {
      ldpp_dout(dpp, 10) << "BlockDirectory::" << __func__
                         << "(): No values returned for key=" << key << dendl;
      return -ENOENT;
    }
    ...
    block->cacheObj.dirty =
        (std::stoi(std::get<0>(resp).value().value()[8]) != 0);
    boost::split(
        block->cacheObj.hostsList, std::get<0>(resp).value().value()[9],
        boost::is_any_of("_"));
    ...
    optional_yield y)
{
  ...
  for (auto& element : resp.value()) {
    ldpp_dout(dpp, 10) << "BlockDirectory::" << __func__ << "(): i is: " << i
                       << dendl;
    CacheBlock* block = &blocks[i];
    std::string key = build_index(block);
    ldpp_dout(dpp, 10) << "BlockDirectory::" << __func__
                       << "(): index is: " << key << dendl;
    if (element.data_type == boost::redis::resp3::type::array ||
        element.data_type == boost::redis::resp3::type::map) {
      ...
      ldpp_dout(dpp, 10) << "BlockDirectory::" << __func__
                         << "() num_elements: " << num_elements << dendl;
      continue;
    } else {
      if (j < num_elements) {
        if (field_key && !field_val) {
          if (element.value == "blockID" || element.value == "version" ||
              element.value == "deleteMarker" || element.value == "size" ||
              element.value == "globalWeight" || element.value == "objName" ||
              element.value == "bucketName" ||
              element.value == "creationTime" || element.value == "dirty" ||
              element.value == "hosts" || element.value == "etag" ||
              element.value == "objSize" || element.value == "userId" ||
              element.value == "displayName") {
            prev_val = element.value;
            ldpp_dout(dpp, 10) << "BlockDirectory::" << __func__
                               << "() field key: " << prev_val << dendl;
            field_key = false;
            field_val = true;
          }
          continue;
          ...
          if (prev_val == "blockID") {
            ...
            boost::split(
                block->cacheObj.hostsList, element.value, boost::is_any_of("_"));
    ...
    optional_yield y)
{
  ...
  auto copyBlock = CacheBlock{
      .cacheObj = {.objName = copyName, .bucketName = copyBucketName},
      .blockID = 0};
    ...
    optional_yield y)
{
  ...
  try {
    ...
    if (ec) {
      ldpp_dout(dpp, 0) << "BlockDirectory::" << __func__
                        << "() ERROR: " << ec.what() << dendl;
      std::cout << "BlockDirectory::" << __func__ << "() ERROR: " << ec.what()
                << std::endl;
      return -ec.value();
    }
  } catch (std::exception& e) {
    ldpp_dout(dpp, 0) << "BlockDirectory::" << __func__
                      << "() ERROR: " << e.what() << dendl;
    std::cout << "BlockDirectory::" << __func__ << "() ERROR: " << e.what()
              << std::endl;
    return -EINVAL;
  }
    ...
    optional_yield y)
{
  ...
  if ((ret = exist_key(dpp, block, y))) {
    try {
      if (field == "hosts") {
        /* Append rather than overwrite */
        ldpp_dout(dpp, 20) << "BlockDirectory::" << __func__
                           << "() Appending to hosts list." << dendl;
    ...
    optional_yield y)
{
  ...
  try {
    ...
    if (std::get<0>(resp).value() != "1") {
      ldpp_dout(dpp, 0) << "BlockDirectory::" << __func__
                        << "() Response value is: " << std::get<0>(resp).value()
                        << dendl;
      return -EINVAL;
    }
    ...
    optional_yield y)
{
  ...
  try {
    ...
    if (std::get<0>(resp).value().empty()) {
      ldpp_dout(dpp, 0) << "BlockDirectory::" << __func__ << "() Empty response"
                        << dendl;
      return -EINVAL;
    }
...
int
Pipeline::execute(const DoutPrefixProvider* dpp, optional_yield y)
{
  boost::redis::generic_response resp;
  try {
    boost::system::error_code ec;
    ...
    if (ec) {
      ldpp_dout(dpp, 0) << "Directory::" << __func__
                        << "() ERROR: " << ec.what() << dendl;
      return -ec.value();
    }
  } catch (std::exception& e) {
    ldpp_dout(dpp, 0) << "Directory::" << __func__ << "() ERROR: " << e.what()
                      << dendl;
    return -EINVAL;
  }
  return 0;
