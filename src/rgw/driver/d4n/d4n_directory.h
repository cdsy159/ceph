...
public:
  ...
  ~RedisPool() { cancel_all(); }

  std::shared_ptr<connection>
  acquire(const DoutPrefixProvider* dpp = nullptr)
  {
    ...
    if (!m_is_pool_connected) {
      for (auto& it : m_pool) {
        auto conn = it;
        conn->async_run(
            m_cfg, {}, boost::asio::consign(boost::asio::detached, conn));
      }
...
struct CacheObj {
  ...
  std::unordered_set<std::string>
      hostsList; /* List of hostnames <ip:port> of object locations for multiple backends */
  ...
  std::string
      display_name; // display name of owner, needed for list object versions
};
...
public:
  ...
  void
  set_redis_pool(std::shared_ptr<RedisPool> pool)
  {
    redis_pool = pool;
  }
...
public:
  Pipeline(
    ...
    conn(conn), redis_pool(redis_pool)
  {}
...
public:
  BucketDirectory(std::shared_ptr<connection>& conn) :
    conn(conn)
  {}
...
public:
  ObjectDirectory(std::shared_ptr<connection>& conn) :
    conn(conn)
  {}
  ...
  int set(
      const DoutPrefixProvider* dpp,
      CacheObj* object,
      optional_yield y); /* If nx is true, set only if key doesn't exist */
...
public:
  BlockDirectory(std::shared_ptr<connection>& conn) :
    conn(conn)
  {}
