...
struct initiate_exec {
  ...
  void
  operator()(Handler handler, const boost::redis::request& req, Response& resp)
  {
    ...
    rgw::sal::Driver* _driver)
{
  ...
  static auto obj_callback =
      [this](
          const DoutPrefixProvider* dpp, const std::string& key,
          const std::string& version, bool deleteMarker, uint64_t size,
          ...
          std::string& restore_val) {
        update_dirty_object(
            dpp, key, version, deleteMarker, size, creationTime, user, etag,
            bucket_name, bucket_id, obj_key, RefCount::NOOP, y, restore_val);
      };

  static auto block_callback = [this](
                                   const DoutPrefixProvider* dpp,
                                   const std::string& key, uint64_t offset,
                                   uint64_t len, const std::string& version,
                                   bool dirty, optional_yield y,
                                   std::string& restore_val) {
    update(
        dpp, key, offset, len, version, dirty, RefCount::NOOP, y, restore_val);
  };
  ...
  try {
    boost::system::error_code ec;
    response<
        ignore_t, ignore_t, ignore_t,
        response<std::optional<int>, std::optional<int>>>
        resp;
    request req;
    req.push("MULTI");
    req.push(
        "HSET", "lfuda", "minLocalWeights_sum",
        std::to_string(
            weightSum), /* New cache node will always have the minimum average weight */
        "minLocalWeights_size", std::to_string(entries_map.size()),
        "minLocalWeights_address",
        dpp->get_cct()->_conf->rgw_d4n_local_rgw_address);
    req.push(
        "HSETNX", "lfuda", "age",
        age); /* Only set maximum age if it doesn't exist */
    req.push("EXEC");
    ...
    ldpp_dout(dpp, 0) << "LFUDAPolicy::" << __func__ << "() ERROR: " << e.what()
                      << dendl;
    return -EINVAL;
  }

  asio::co_spawn(io_context.get_executor(), redis_sync(dpp, y), asio::detached);

  return 0;
}

int
LFUDAPolicy::age_sync(const DoutPrefixProvider* dpp, optional_yield y)
{
  ...
  if (std::get<0>(resp).value().value().empty() ||
      age > std::stoi(
                std::get<0>(resp).value().value())) { /* Set new maximum age */
    try {
      boost::system::error_code ec;
...
int
LFUDAPolicy::local_weight_sync(const DoutPrefixProvider* dpp, optional_yield y)
{
  if (fabs(weightSum - postedSum) > (postedSum * 0.1)) {
    ...
    float minAvgWeight = std::stof(std::get<0>(resp).value()[0]) /
                         std::stof(std::get<0>(resp).value()[1]);
    float localAvgWeight = 0;
    if (entries_map.size())
      localAvgWeight = static_cast<float>(weightSum) /
                       static_cast<float>(entries_map.size());
    ...
    if (localAvgWeight < minAvgWeight) { /* Set new minimum weight */
      try {
        ...
        request req;
        req.push(
            "HSET", "lfuda", "minLocalWeights_sum", std::to_string(weightSum),
            "minLocalWeights_size", std::to_string(entries_map.size()),
            "minLocalWeights_address",
            dpp->get_cct()->_conf->rgw_d4n_local_rgw_address);
  ...
  try { /* Post update for local cache */
    ...
    request req;
    req.push(
        "HSET", dpp->get_cct()->_conf->rgw_d4n_local_rgw_address,
        "avgLocalWeight_sum", std::to_string(weightSum), "avgLocalWeight_size",
        std::to_string(entries_map.size()));
...
asio::awaitable<void>
LFUDAPolicy::redis_sync(const DoutPrefixProvider* dpp, optional_yield y)
{
  ...
  for (;;)
    try {
      ...
      if (int ret = age_sync(dpp, y) < 0) {
        ldpp_dout(dpp, 0) << "LFUDAPolicy::" << __func__ << "() ERROR: " << ret
                          << dendl;
      }
    ...
    const std::string& key)
{
  ...
  if (p == o_entries_map.end()) {
    ldpp_dout(dpp, 10) << "LFUDAPolicy::" << __func__ << "(): key=" << key
                       << " not found" << dendl;
    return false;
  }
  ...
  if (p->second.second == State::INIT) {
    ldpp_dout(dpp, 10) << "LFUDAPolicy::" << __func__
                       << "(): Setting State::INVALID for key=" << key << dendl;
    p->second.second = State::INVALID;
    int ret = cacheDriver->set_attr(dpp, key, RGW_CACHE_ATTR_INVALID, "1", y);
    if (ret < 0) {
      ldpp_dout(dpp, 0) << "LFUDAPolicy::" << __func__
                        << "(): Failed to set xattr, ret=" << ret << dendl;
      return false;
    }
    return true;
  } else if (p->second.second == State::IN_PROGRESS) {
    state_cond.wait(l, [this, &key] {
      return (o_entries_map.find(key) == o_entries_map.end());
    });
  }
...
CacheBlock*
LFUDAPolicy::get_victim_block(const DoutPrefixProvider* dpp, optional_yield y)
{
  ...
  if (block_info.size() != 5) {
    ldpp_dout(dpp, 0) << "LFUDAPolicy::" << __func__
                      << "(): Key of the top entry in the min heap has not "
  ...
  if (entry->dirty || entry->refcount > 0) {
    ldpp_dout(dpp, 0) << "LFUDAPolicy::" << __func__
                      << "(): Top entry in min heap is dirty or with positive "
                         "refcount, no entry is available for eviction!"
                      << dendl;
    return nullptr;
  }
...
int
LFUDAPolicy::exist_key(const std::string& key)
{
    ...
    optional_yield y)
{
  ...
  while (
      freeSpace <
      size) { // TODO: Think about parallel reads and writes; can this turn into an infinite loop?
    std::unique_lock<std::mutex> l(lfuda_lock);
    CacheBlock* victim = get_victim_block(dpp, y);

    if (victim == nullptr) {
      ldpp_dout(dpp, 0) << "LFUDAPolicy::" << __func__
                        << "(): Could not retrieve victim block." << dendl;
      delete victim;
      l.unlock();
      return -ENOSPC;
    }
    ...
    if ((ret = blockDir->remove_host(
             dpp, victim, dpp->get_cct()->_conf->rgw_d4n_local_rgw_address,
             y)) < 0) {
      delete victim;
      return ret;
    }
    ...
    ldpp_dout(dpp, 10) << "LFUDAPolicy::" << __func__ << "(): Block " << key
                       << " has been evicted." << dendl;
...
bool
LFUDAPolicy::update_refcount_if_key_exists(
    const DoutPrefixProvider* dpp,
    const std::string& key,
    uint8_t op,
    optional_yield y)
{
  ldpp_dout(dpp, 20) << "LFUDAPolicy::" << __func__
                     << "(): updating refcount for entry: " << key << dendl;
  const std::lock_guard l(lfuda_lock);
  ...
  ldpp_dout(dpp, 20) << "LFUDAPolicy::" << __func__
                     << "(): old refcount is: " << refcount << dendl;
  if (op == RefCount::INCR) {
    refcount += 1;
...
{
  ldpp_dout(dpp, 10) << "LFUDAPolicy::" << __func__
                     << "(): updating entry: " << key << dendl;
  using handle_type = boost::heap::fibonacci_heap<
      LFUDAEntry*, boost::heap::compare<EntryComparator<LFUDAEntry>>>::handle_type;
  const std::lock_guard l(lfuda_lock);
  int localWeight = age;
  auto entry = find_entry(key);
  ...
  if (!restore_val.empty()) {
    ...
    ldpp_dout(dpp, 10) << "LFUDAPolicy::" << __func__
                       << "(): restored localWeight is: " << localWeight
                       << dendl;
  }
  ...
  ldpp_dout(dpp, 10) << "LFUDAPolicy::" << __func__
                     << "(): updated refcount is: " << refcount << dendl;
  ...
  if (entry) {
    ...
    LFUDAEntry* e = new LFUDAEntry(
        key, offset, len, version, is_dirty, refcount, localWeight);
  ...
  if (updateLocalWeight) {
    int ret = -1;
    if ((ret = cacheDriver->set_attr(
             dpp, key, RGW_CACHE_ATTR_LOCAL_WEIGHT, std::to_string(localWeight),
             y)) < 0)
      ldpp_dout(dpp, 0) << "LFUDAPolicy::" << __func__
                        << "(): CacheDriver set_attr method failed, ret=" << ret
                        << dendl;
  }
...
void
LFUDAPolicy::update_dirty_object(
    const DoutPrefixProvider* dpp,
    const std::string& key,
    const std::string& version,
    bool deleteMarker,
    ...
    std::string& restore_val)
{
  using handle_type = boost::heap::fibonacci_heap<
      LFUDAObjEntry*,
      boost::heap::compare<ObjectComparator<LFUDAObjEntry>>>::handle_type;
  State state{State::INIT};
  ldpp_dout(dpp, 10) << "LFUDAPolicy::" << __func__
                     << "(): Before acquiring lock, adding entry: " << key
                     << dendl;

  if (!restore_val.empty() &&
      restore_val ==
          "1") { // No need to set the xattr because this case only occurs when the state has
    state = State::INVALID; // been retrieved from the xattr itself.
    ldpp_dout(dpp, 10) << "LFUDAPolicy::" << __func__
                       << "(): State restored to INVALID." << dendl;
  } else {
    state = State::INIT;
  }

  const std::lock_guard l(lfuda_cleaning_lock);
  LFUDAObjEntry* e = new LFUDAObjEntry{key,    version,      deleteMarker,
                                       size,   creationTime, user,
                                       etag,   bucket_name,  bucket_id,
                                       obj_key};
...
bool
LFUDAPolicy::erase_dirty_object(
    const DoutPrefixProvider* dpp,
    const std::string& key,
    optional_yield y)
{
...
int
LFUDAPolicy::delete_data_blocks(
    const DoutPrefixProvider* dpp,
    LFUDAObjEntry* e,
    optional_yield y)
{
  ...
  do {
    ...
    std::string oid_in_cache = rgw::sal::get_key_in_cache(
        e->key, std::to_string(fst), std::to_string(cur_len));
    ...
    if ((ret = cacheDriver->delete_data(dpp, oid_in_cache, y)) == 0) {
      if (!(ret = erase(dpp, oid_in_cache, y))) {
        ldpp_dout(dpp, 0) << "Failed to delete policy entry for: "
                          << oid_in_cache << ", ret=" << ret << dendl;
        return -EINVAL;
      }
    } else {
...
void
LFUDAPolicy::cleaning(const DoutPrefixProvider* dpp)
{
  ...
  while (!quit) {
    ...
    ldpp_dout(dpp, 20) << "LFUDAPolicy::" << __func__ << "" << __LINE__
                       << "(): Before acquiring cleaning-lock" << dendl;
    std::unique_lock<std::mutex> l(lfuda_cleaning_lock);
    LFUDAObjEntry* e;
    ...
    ldpp_dout(dpp, 10) << __LINE__ << " " << __func__ << "(): e->key=" << e->key
                       << dendl;
    ldpp_dout(dpp, 10) << __LINE__ << " " << __func__
                       << "(): e->delete_marker=" << e->delete_marker << dendl;
    ldpp_dout(dpp, 10) << __LINE__ << " " << __func__
                       << "(): e->version=" << e->version << dendl;
    ldpp_dout(dpp, 10) << __LINE__ << " " << __func__
                       << "(): e->bucket_name=" << e->bucket_name << dendl;
    ldpp_dout(dpp, 10) << __LINE__ << " " << __func__
                       << "(): e->bucket_id=" << e->bucket_id << dendl;
    ldpp_dout(dpp, 10) << __LINE__ << " " << __func__
                       << "(): e->user=" << e->user << dendl;
    ldpp_dout(dpp, 10) << __LINE__ << " " << __func__
                       << "(): e->obj_key=" << e->obj_key << dendl;
    l.unlock();

    int diff = std::difftime(time(NULL), e->creationTime);
    if (!e->key.empty() &&
        (diff >
         interval)) { // if block is dirty and written more than interval seconds ago
      l.lock();
      ...
      if (invalid) {
        ldpp_dout(dpp, 10) << __func__
                           << "(): State is INVALID; deleting object." << dendl;
        int ret = -1;
        ...
        if (it != entries_map.end()) {
          if (it->second->refcount > 0) {
            ...
            (*e->handle)->creationTime = (*e->handle)->creationTime +
                                         interval / 2;
            ldpp_dout(dpp, 20) << "LFUDAPolicy::" << __func__
                               << "(): updated creation time is: "
                               << (*e->handle)->creationTime << dendl;
          ...
          if ((ret = cacheDriver->delete_data(dpp, e->key, y)) == 0) {
            if (!(ret = erase(dpp, e->key, y))) {
              ldpp_dout(dpp, 0)
                  << "Failed to delete head policy entry for: " << e->key
                  << ", ret=" << ret
                  << dendl; // TODO: what must occur during failure?
            }
          } else {
            ldpp_dout(dpp, 0) << "Failed to delete head object for: " << e->key
                              << ", ret=" << ret << dendl;
          }
        } else {
        ...
        if (!e->delete_marker) {
          ...
          if (ret == 0) {
            ...
            ldpp_dout(dpp, 0) << "Failed to delete blocks for: " << e->key
                              << ", ret=" << ret << dendl;
          }
        } else {
          erase_dirty_object(dpp, e->key, null_yield);
        ...
        rgw_bucket c_rgw_bucket =
            rgw_bucket(c_rgw_user.tenant, e->bucket_name, e->bucket_id);
        ...
        if (ret < 0) {
          ldpp_dout(dpp, 10)
              << __func__ << "(): load_bucket() returned ret=" << ret << dendl;
        ...
        std::unique_ptr<rgw::sal::Object> c_obj =
            c_bucket->get_object(e->obj_key);
        bool null_instance = (c_obj->get_instance() == "null");
        if (null_instance) {
          //clear the instance for backend store
          c_obj->clear_instance();
        }
        ldpp_dout(dpp, 20) << __func__ << "(): c_obj oid =" << c_obj->get_oid()
                           << dendl;
        ...
        if (e->delete_marker) {
          ...
          del_op->params.versioning_status =
              c_bucket->get_info().versioning_status();
          ...
          if (op_ret >= 0) {
            ...
            ldpp_dout(dpp, 20)
                << __func__ << "delete_obj delete_marker=" << delete_marker
                << dendl;
            ldpp_dout(dpp, 20)
                << __func__ << "delete_obj version_id=" << version_id << dendl;
          } else {
            ldpp_dout(dpp, 20)
                << __func__ << "delete_obj returned ret=" << op_ret << dendl;
            erase_dirty_object(dpp, e->key, null_yield);
            continue;
          }
          ...
          std::unique_ptr<rgw::sal::Writer> processor =
              driver->get_atomic_writer(
                  dpp, null_yield, c_obj.get(), owner, NULL, 0, "");

          op_ret = processor->prepare(null_yield);
          if (op_ret < 0) {
            ldpp_dout(dpp, 20)
                << __func__ << "processor->prepare() returned ret=" << op_ret
                << dendl;
            erase_dirty_object(dpp, e->key, null_yield);
            continue;
          }
          ...
          bufferlist bl;
          op_ret = cacheDriver->get_attrs(
              dpp, e->key, obj_attrs, null_yield); //get obj attrs from head
          if (op_ret < 0) {
            ldpp_dout(dpp, 20)
                << __func__ << "cacheDriver->get_attrs returned ret=" << op_ret
                << dendl;
            erase_dirty_object(dpp, e->key, null_yield);
            continue;
          }
          ...
          do {
            ...
            std::string oid_in_cache = rgw::sal::get_key_in_cache(
                e->key, std::to_string(fst), std::to_string(cur_len));
            ldpp_dout(dpp, 10)
                << __func__ << "(): oid_in_cache=" << oid_in_cache << dendl;
            rgw::sal::Attrs attrs;
            cacheDriver->get(
                dpp, oid_in_cache, 0, cur_len, data, attrs, null_yield);
            if (op_ret < 0) {
              ldpp_dout(dpp, 20)
                  << __func__ << "cacheDriver->get returned ret=" << op_ret
                  << dendl;
              erase_dirty_object(dpp, e->key, null_yield);
              continue;
            }
            ...
            if (op_ret < 0) {
              ldpp_dout(dpp, 20)
                  << __func__ << "processor->process() returned ret=" << op_ret
                  << dendl;
              erase_dirty_object(dpp, e->key, null_yield);
              continue;
            }
          ...
          op_ret = processor->complete(
              lst, e->etag, &mtime,
              ceph::real_clock::from_time_t(e->creationTime), obj_attrs,
              std::nullopt, ceph::real_time(), nullptr, nullptr, nullptr,
              nullptr, nullptr, rctx, rgw::sal::FLAG_LOG_OP);

          if (op_ret < 0) {
            ldpp_dout(dpp, 20)
                << __func__ << "processor->complete() returned ret=" << op_ret
                << dendl;
            erase_dirty_object(dpp, e->key, null_yield);
            continue;
          }
	     ...
	     reset values */
          ...
          do {
            ...
            block.size = cur_len;
            block.blockID = fst;
            if ((op_ret = cacheDriver->set_attr(
                     dpp, oid_in_cache, RGW_CACHE_ATTR_DIRTY, "0", y)) == 0) {
              std::string dirty = "false";
              op_ret = blockDir->update_field(
                  dpp, &block, "dirty", dirty, null_yield);
              if (op_ret < 0) {
                ldpp_dout(dpp, 0)
                    << __func__
                    << "updating dirty flag in block directory failed, ret="
                    << op_ret << dendl;
              }
            } else {
              ldpp_dout(dpp, 0)
                  << __func__
                  << "(): Failed to update dirty xattr in cache, ret=" << op_ret
                  << dendl;
            }
        ...
        if ((ret = cacheDriver->set_attr(
                 dpp, e->key, RGW_CACHE_ATTR_DIRTY, "0", y)) < 0) {
          ldpp_dout(dpp, 0)
              << __func__
              << "(): Failed to update dirty attr in cache, ret=" << op_ret
              << dendl;
        }
        ...
        ldpp_dout(dpp, 20) << __func__
                           << "(): bucket name: " << block.cacheObj.bucketName
                           << dendl;
        ...
        if (!c_obj->have_instance()) {
            ...
            if (block.version == e->version) {
              ...
              if (op_ret < 0) {
                ldpp_dout(dpp, 0)
                    << __func__
                    << "(): Failed to get latest entry in block directory for: "
                    << null_block.cacheObj.objName << ", ret=" << ret << dendl;
              } else {
                if (null_block.version == e->version) {
                  ...
                  if (blk_op_ret < 0 || null_op_ret < 0) {
                    ldpp_dout(dpp, 0)
                        << __func__
                        << "(): Failed to Queue update dirty flag for latest "
                           "entry/null entry in block directory"
                        << dendl;
                  }
                }
              }
          ...
          ldpp_dout(dpp, 10)
              ...
              << "(): Removing object name: " << c_obj->get_name() << " score: "
              << std::setprecision(std::numeric_limits<double>::max_digits10)
              << e->creationTime << " from ordered set" << dendl;
	     ...
	     need not be part of a transaction as it is being removed based on its score which is its creation time. */
          ret = objDir->zremrangebyscore(
              dpp, &dir_obj, e->creationTime, e->creationTime, y);
          if (ret < 0) {
            ldpp_dout(dpp, 0)
                << __func__
                << "(): Failed to remove object from ordered set with error: "
                << ret << dendl;
          }
        }
        if (c_obj->have_instance()) { //versioned case
          ...
          instance_block.cacheObj.bucketName =
              c_obj->get_bucket()->get_bucket_id();
          instance_block.cacheObj.objName = objName;
          instance_block.size = 0;
          instance_block.blockID = 0;
          std::string dirty = "false";
          op_ret = blockDir->update_field(
              dpp, &instance_block, "dirty", dirty, null_yield);
          if (op_ret < 0) {
            ldpp_dout(dpp, 20)
                << __func__ << "updating dirty flag in block directory for instance block failed!"
                << dendl;
          }
            ...
            if (latest_block.version == e->version) {
              ...
              if (c_obj->have_instance()) {
                ...
                if (ret < 0) {
                  ldpp_dout(dpp, 0)
                      << __func__
                      << "(): Failed to queue del for latest hash entry: "
                      << latest_block.cacheObj.objName << ", ret=" << ret
                      << dendl;
                  continue;
                }
              }
              ...
              if (ret < 0) {
                ldpp_dout(dpp, 0)
                    << __func__ << "(): Failed to queue zrem for object entry: "
                    << c_obj->get_name() << ", ret=" << ret << dendl;
                continue;
              }
            }
            ldpp_dout(dpp, 10)
                << "D4NFilterObject::" << __func__
                << "(): Removing object name: " << c_obj->get_name()
                << " score: "
                << std::setprecision(std::numeric_limits<double>::max_digits10)
                << e->creationTime << " from ordered set" << dendl;
            rgw::d4n::CacheObj dir_obj = rgw::d4n::CacheObj{
        ...
        diff <
        interval) { //end-if std::difftime(time(NULL), e->creationTime) > interval
      {
        std::unique_lock<std::mutex> wait_lock(lfuda_cleaning_lock);
        cond.wait_for(wait_lock, std::chrono::seconds(interval - diff), [] {
          return quit.load();
        });
      }
      continue;
...
int
LRUPolicy::exist_key(const std::string& key)
{
    ...
    optional_yield y)
{
  ...
  while (freeSpace < size) {
    ...
    if (ret < 0) {
      ldpp_dout(dpp, 0)
          << __func__
          << "(): Failed to delete data from the cache backend, ret=" << ret
          << dendl;
      return ret;
    }
...
{
  const std::lock_guard l(lru_lock);
  ObjEntry* e = new ObjEntry(
      key, version, deleteMarker, size, creationTime, user, etag, bucket_name,
      bucket_id, obj_key);
    ...
    optional_yield y)
{
  ...
  entries_map.erase(p);
  entries_lru_list.erase_and_dispose(
      entries_lru_list.iterator_to(*(p->second)), Entry_delete_disposer());
  return true;
}
