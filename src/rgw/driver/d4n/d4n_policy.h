...
public:
  ...
  virtual int init(
      ...
      rgw::sal::Driver* _driver) = 0;
  virtual int exist_key(const std::string& key) = 0;
  virtual int eviction(
      const DoutPrefixProvider* dpp,
      uint64_t size,
      optional_yield y) = 0;
  virtual bool update_refcount_if_key_exists(
      const DoutPrefixProvider* dpp,
      const std::string& key,
      uint8_t op,
      optional_yield y) = 0;
  virtual void update(
      ...
      uint8_t op,
      optional_yield y,
      std::string& restore_val = empty) = 0;
  virtual void update_dirty_object(
      const DoutPrefixProvider* dpp,
      const std::string& key,
      const std::string& version,
      bool deleteMarker,
  ...
  virtual bool erase(
      ...
      const std::string& key,
      optional_yield y) = 0;
  virtual bool erase_dirty_object(
      const DoutPrefixProvider* dpp,
      const std::string& key,
      optional_yield y) = 0;
  virtual bool invalidate_dirty_object(
      const DoutPrefixProvider* dpp,
...
private:
  ...
  struct LFUDAEntry : public Entry {
    ...
    using handle_type = boost::heap::fibonacci_heap<
        LFUDAEntry*,
        boost::heap::compare<EntryComparator<LFUDAEntry>>>::handle_type;
    handle_type handle;
    ...
    LFUDAEntry(
        ...
        uint64_t refcount,
        int localWeight) :
      Entry(key, offset, len, version, dirty, refcount),
      localWeight(localWeight)
    {}

    void
    set_handle(handle_type handle_)
    {
      handle = handle_;
    }
  };
  ...
  using Heap = boost::heap::fibonacci_heap<
      LFUDAEntry*,
      boost::heap::compare<EntryComparator<LFUDAEntry>>>;
  using Object_Heap = boost::heap::fibonacci_heap<
      LFUDAObjEntry*,
      boost::heap::compare<ObjectComparator<LFUDAObjEntry>>>;
  Heap entries_heap;
  Object_Heap
      object_heap; //This heap contains dirty objects ordered by their creation time, used for cleaning method
  std::unordered_map<std::string, LFUDAEntry*> entries_map;
  std::unordered_map<std::string, std::pair<LFUDAObjEntry*, State>>
      o_entries_map; //Contains only dirty objects, used for look-up
  ...
  asio::awaitable<void> redis_sync(
      const DoutPrefixProvider* dpp,
      optional_yield y);
  ...
  LFUDAEntry*
  find_entry(const std::string& key)
  {
...
public:
  LFUDAPolicy(
      ...
      rgw::cache::CacheDriver* cacheDriver,
      optional_yield y) :
    CachePolicy(), y(y), conn(conn), cacheDriver(cacheDriver)
  {
  ...
  virtual void update(
      ...
      uint8_t op,
      optional_yield y,
      std::string& restore_val = empty) override;
  virtual bool erase(
      const DoutPrefixProvider* dpp,
      const std::string& key,
      optional_yield y) override;
  ...
  LFUDAObjEntry*
  find_obj_entry(const std::string& key)
  {
...
public:
  LRUPolicy(rgw::cache::CacheDriver* cacheDriver) :
  ...
  virtual void
  cleaning(const DoutPrefixProvider* dpp) override
  {}
};
...
public:
  ...
  ~PolicyDriver() { delete cachePolicy; }
