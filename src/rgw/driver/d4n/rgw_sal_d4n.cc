...
D4NFilterDriver::D4NFilterDriver(
    ...
    bool admin) :
  FilterDriver(_next), io_context(io_context), y(null_yield)
{
...
int
D4NFilterDriver::initialize(CephContext* cct, const DoutPrefixProvider* dpp)
{
  ...
  policyDriver = std::make_unique<rgw::d4n::PolicyDriver>(
      conn, cacheDriver.get(), "lfuda", this->y);
  ...
  if (!cfg.addr.host.length() || !cfg.addr.port.length()) {
    ldpp_dout(dpp, 0) << "D4NFilterDriver::" << __func__
                      << "(): Endpoint was not configured correctly." << dendl;
    return -EDESTADDRREQ;
  }
  ...
  uint64_t rgw_redis_connection_pool_size =
      dpp->get_cct()->_conf->rgw_redis_connection_pool_size;
  std::shared_ptr<rgw::d4n::RedisPool> redis_pool = nullptr;
  if (rgw_redis_connection_pool_size > 0) {
    redis_pool = std::make_shared<rgw::d4n::RedisPool>(
        &io_context, cfg, rgw_redis_connection_pool_size);
    ldpp_dout(dpp, 10) << "redis connection pool created with "
                       << rgw_redis_connection_pool_size << " connections "
                       << dendl;
  }
...
std::unique_ptr<User>
D4NFilterDriver::get_user(const rgw_user& u)
{
...
D4NFilterBucket::get_object(const rgw_obj_key& k)
{
...
D4NFilterDriver::get_bucket(const RGWBucketInfo& i)
{
  return std::make_unique<D4NFilterBucket>(next->get_bucket(i), this);
}

int
D4NFilterDriver::load_bucket(
...
D4NFilterBucket::list(
    ...
    optional_yield y)
{
  ldpp_dout(dpp, 20) << "D4NFilterBucket::" << __func__
                     << " params.marker.name: " << params.marker.name << dendl;
  ldpp_dout(dpp, 20) << "D4NFilterBucket::" << __func__
                     << " params.marker.instance: " << params.marker.instance
                     << dendl;
  ldpp_dout(dpp, 20) << "D4NFilterBucket::" << __func__
                     << " params.end_marker.key: " << params.end_marker.name
                     << dendl;
  ldpp_dout(dpp, 20) << "D4NFilterBucket::" << __func__ << " max: " << max
                     << dendl;
  ...
  if (g_conf()->d4n_writecache_enabled) {
    ...
    if (!params.prefix.empty()) {
      ...
      do {
        ...
        auto ret = bucketDir->zscan(
            dpp, this->get_bucket_id(), cursor, pattern, (max + 1),
            temp_objects, next_cursor, y);
        if (ret < 0 && ret != -ENOENT) {
          ldpp_dout(dpp, 0) << "D4NFilterBucket::" << __func__
                            << " zscan failed with ret: " << ret << dendl;
          return ret;
        }
        ...
        for (auto it = temp_objects.begin(); it != temp_objects.end(); it++) {
          ...
          if (!params.marker.name.empty() && *it <= params.marker.name) {
            if (*it != params.marker.name || !params.list_versions ||
                params.marker.instance.empty()) {
              continue;
            }
          } else {
            ...
            if (!params.delim.empty() && pos != std::string::npos) {
              ...
              ldpp_dout(dpp, 20) << "D4NFilterBucket::" << __func__
                                 << " delim_str: " << delim_str << dendl;
              if (cache_results.common_prefixes.find(delim_str) ==
                  cache_results.common_prefixes.end()) {
                cache_results.common_prefixes.emplace(
                    std::make_pair(delim_str, true));
                store_results.common_prefixes.emplace(
                    std::make_pair(delim_str, true));
              ...
              if (params.list_versions) {
                ...
                if (params.marker.instance.empty() ||
                    instance_marker_processed || params.marker.name.empty()) {
                  start = "0";
                } else {
                ...
                auto ret = objDir->zrevrange(
                    dpp, &dir_obj, start, "-1", temp_versions, y);
                if (ret < 0 && ret != -ENOENT) {
                  ldpp_dout(dpp, 0)
                      << "D4NFilterBucket::" << __func__
                      << " zrevrange failed with ret: " << ret << dendl;
                  return ret;
                }
              } //params.list_version
              if (!temp_versions.empty()) {
                for (auto version_it = temp_versions.begin();
                     version_it != temp_versions.end(); version_it++) {
          ...
          if (num_objs == max) {
            if (std::next(it) != temp_objects.end()) {
              if (cache_results.next_marker.empty()) {
                cache_results.is_truncated = true;
                cache_results.next_marker.name = last_element_processed;
                if (cache_results.next_marker.instance.empty() &&
      ...
      do {
        std::vector<std::string> temp_objects;
        ldpp_dout(dpp, 20) << "D4NFilterBucket::" << __func__
                           << " zrange start is: " << start << dendl;
        auto ret = bucketDir->zrange(
            dpp, this->get_bucket_id(), start, "+", 0, (max + 1), temp_objects,
            y);
        if (ret < 0 && ret != -ENOENT) {
          ldpp_dout(dpp, 0) << "D4NFilterBucket::" << __func__
                            << " zrange failed with ret: " << ret << dendl;
          return ret;
        }
        ldpp_dout(dpp, 20) << "D4NFilterBucket::" << __func__
                           << " params.delim: " << params.delim << dendl;
        std::string last_element_processed;
        for (auto it = temp_objects.begin(); it != temp_objects.end(); it++) {
          last_element_processed = *it;
          ...
          if (!params.delim.empty() && pos != std::string::npos) {
            ...
            if (cache_results.common_prefixes.find(delim_str) ==
              ...
              if (num_objs == max) {
                ...
                do {
                  ...
                  auto ret = bucketDir->zscan(
                      dpp, this->get_bucket_id(), cursor, delim_pattern, 10,
                      delim_objects, next_cursor, y);
                  if (ret < 0 && ret != -ENOENT) {
                    ldpp_dout(dpp, 0)
                        << "D4NFilterObject::" << __func__
                        << " zscan failed with ret: " << ret << dendl;
                    return ret;
                  }
                  if ((next_cursor == 0) || (ret == -ENOENT)) {
                    if (!delim_objects.empty()) {
                      ...
                      auto ret = bucketDir->zrange(
                          dpp, this->get_bucket_id(), start, "+", 0, 1,
                          one_object, y);
                      if (ret < 0 && ret != -ENOENT) {
                        ldpp_dout(dpp, 0)
                            << "D4NFilterObject::" << __func__
            ...
            if (params.list_versions) {
              ...
              std::string start;
              if (params.marker.instance.empty() || instance_marker_processed ||
                  params.marker.name.empty()) {
                start = "0";
              } else {
                if (!instance_marker_processed) {
                  std::string member = params.marker.instance;
        ...
        if ((num_objs == max) || (ret == -ENOENT) || temp_objects.empty()) {
          ldpp_dout(dpp, 20)
              << "D4NFilterBucket::" << __func__ << " Breaking out! " << dendl;
          ldpp_dout(dpp, 20) << "D4NFilterBucket::" << __func__ << " num_objs "
                             << num_objs << dendl;
          ldpp_dout(dpp, 20)
              << "D4NFilterBucket::" << __func__ << " ret " << ret << dendl;
          break;
        }
    ...
    while (remainder_size > 0) {
      ...
      for (auto i = 0; i < batch_size; i++) {
        ldpp_dout(dpp, 20) << "D4NFilterBucket::" << __func__
                           << " objects[j]: " << entries[j].key.name << dendl;
        ldpp_dout(dpp, 20) << "D4NFilterBucket::" << __func__
                           << " remainder_size: " << remainder_size << dendl;
        ...
        ldpp_dout(dpp, 20) << "D4NFilterBucket::" << __func__
                           << " blocks[i].cacheObj.objName: "
                           << blocks[i].cacheObj.objName << dendl;
      ...
      if (ret < 0) {
        ldpp_dout(dpp, 0) << "D4NFilterBucket::" << __func__
                          << " blockDir->get() returned error: " << ret
                          << dendl;
        return ret;
      }
      ...
      for (auto block : blocks) {
        if (return_blocks) {
          if (i >= dir_blocks_objNames.size() ||
              i >= block.cacheObj.objName.size()) {
            ldpp_dout(dpp, 0)
                << "D4NFilterBucket::" << __func__
                << " Unable to construct list of cached blocks." << dendl;
            return -EINVAL;
          }
          if (dir_blocks_objNames[i] == block.cacheObj.objName) {
            ...
            ldpp_dout(dpp, 0)
                << "D4NFilterBucket::"
                << __func__ << " Error constructing list of cached blocks while handling object "
                << dir_blocks_objNames[i] << dendl;
            return -EINVAL;
          }
        }
        i++;
        ...
        try {
          ...
          ldpp_dout(dpp, 0)
              << "D4NFilterBucket::" << __func__
              << " Invalid value of time: " << block.cacheObj.creationTime
              << dendl;
        }
  ...
  if (!cache_results.objs.empty() && !store_results.objs.empty()) {
    ...
    while (elementsAdded < max && i < cache_results.objs.size() &&
      ...
      ldpp_dout(dpp, 20) << "D4NFilterObject::" << __func__
                         << " key_name_in_cache: " << key_name_in_cache
                         << dendl;
      ldpp_dout(dpp, 20) << "D4NFilterObject::" << __func__
                         << " key_name_in_store: " << key_name_in_store
                         << dendl;
      ...
      if (key_name_in_cache == key_name_in_store) {
        ...
        if (!params.list_versions ||
            key_instance_in_cache == key_instance_in_store) {
          j++;
        }
  ...
  if (!store_results.common_prefixes.empty()) {
    ...
    if (results.next_marker.empty()) {
      results.is_truncated = store_results.is_truncated |
                             cache_results.is_truncated;
    ...
    optional_yield y)
{
  ...
  do {
    ...
    if (!results.objs.empty() && !delete_children) {
      ldpp_dout(dpp, 10) << "ERROR: could not remove non-empty bucket "
                         << this->get_name() << dendl;
      return -ENOTEMPTY;
    }

    for (const auto& obj : results.objs) {
      if (((PIPELINE_MAX - blocks.size()) <= OBJECT_LIST_VAL) ||
          (blocks.size() > (PIPELINE_MAX - 1000))) {
      ...
      ldpp_dout(dpp, 20) << "D4NFilterBucket::" << __func__
                         << "(): Obj size=" << lst << dendl;
      block.cacheObj.objName = "_:" + oid_version + "_" +
                               block.cacheObj.objName;
      auto it = dir_blocks.find(block.cacheObj.objName);
      if (it != dir_blocks.end() && it->second.cacheObj.dirty) {
        if (!this->filter->get_policy_driver()
                 ->get_cache_policy()
                 ->invalidate_dirty_object(
                     dpp,
                     get_cache_block_prefix(c_obj.get(), it->second.version))) {
          ldpp_dout(dpp, 10)
              << "D4NFilterBucket::" << __func__
              << "(): Failed to invalidate obj=" << c_obj->get_name()
              << " in cache" << dendl;
          return -EINVAL;
        }
       ...
       * later eviction calls. */
        ...
        ldpp_dout(dpp, 20) << "D4NFilterBucket::" << __func__
                           << "(): Listing retrieved from backend for object "
                           << c_obj->get_name() << dendl;
      }

      // Handle versioned head objects
      ldpp_dout(dpp, 20) << "D4NFilterBucket::" << __func__
                         << "(): versioned oid: " << block.cacheObj.objName
                         << dendl;
      blocks.push_back(block);

      // Handle data blocks
      ldpp_dout(dpp, 20) << "D4NFilterBucket::" << __func__
                         << "(): Object size=" << lst << dendl;
      off_t fst = 0;
      do {
    ...
    if ((PIPELINE_MAX - objects.size()) <= OBJECT_LIST_VAL) {
      if ((ret = objDir->del(dpp, objects, y)) < 0) {
        ldpp_dout(dpp, 10)
            << "D4NFilterBucket::" << __func__
            << "(): Failed to delete bucket in bucket directory, ret=" << ret
            << dendl;
        return ret;
      }
      objects.clear();
  ...
  if ((ret = this->filter->get_bucket_dir()->del(
           dpp, this->get_bucket_id(), y)) < 0 &&
      (ret != -ENOENT)) {
    ldpp_dout(dpp, 10)
        << "D4NFilterBucket::" << __func__
        << "(): Failed to delete bucket in bucket directory, ret=" << ret
  ...
  ldpp_dout(dpp, 20) << "D4NFilterBucket::" << __func__
                     << "(): calling next->remove" << dendl;
  return next->remove(dpp, delete_children, y);
}

int
D4NFilterBucket::check_empty(const DoutPrefixProvider* dpp, optional_yield y)
{
  // if the bucket exists in the bucket directory, then there are objects in the local cache
  int ret;
  if ((ret = this->filter->get_bucket_dir()->exist_key(
           dpp, this->get_bucket_id(), y)) < 0) {
    ldpp_dout(dpp, 10)
        << "D4NFilterBucket::" << __func__
        << "(): Failed to retrieve bucket in bucket directory, ret=" << ret
        << dendl;
    return ret;
  }
  if (ret == 0) {
    ldpp_dout(dpp, 20) << "D4NFilterBucket::" << __func__
                       << "(): calling next->check_empty" << dendl;
...
D4NFilterBucket::get_multipart_upload(
    ...
    ceph::real_time mtime)
{
  ...
  return std::make_unique<D4NFilterMultipartUpload>(
      std::move(nmu), this, this->filter);
}

int
D4NFilterObject::copy_object(
    ...
    optional_yield y)
{
  ...
  if (auto ret = read_op->prepare(y, dpp); ret < 0) {
    ldpp_dout(dpp, 10) << "D4NFilterObject::" << __func__
                       << "(): prepare method failed with ret: " << ret
                       << dendl;
  ...
  ldpp_dout(dpp, 20) << "D4NFilterObject::" << __func__
                     << "(): is_multipart: " << is_multipart() << dendl;
  //for multipart objects or for read only cache, write to backend store
  if (is_multipart() || !write_to_cache) {
    write_to_cache = false;
    auto ret = next->copy_object(
        owner, remote_user, info, source_zone, nextObject(dest_object),
        nextBucket(dest_bucket), nextBucket(src_bucket), dest_placement,
        src_mtime, mtime, mod_ptr, unmod_ptr, high_precision_time, if_match,
        if_nomatch, attrs_mod, copy_if_newer, attrs, category, olh_epoch,
        delete_at, version_id, tag, etag, progress_cb, progress_data,
        dp_factory, dpp, y);
    if (ret < 0) {
      ldpp_dout(dpp, 10) << "D4NFilterObject::" << __func__
                         << "(): next->copy_object failed with ret: " << ret
                         << dendl;
      return ret;
    }
  }
  ...
  if (attrs_mod == rgw::sal::ATTRSMOD_NONE) {
    ...
    if (version_id) {
      ...
      baseAttrs[RGW_CACHE_ATTR_VERSION_ID] =
          std::move(bl_val); //populate destination version id
    }
  }
  ...
  if (write_to_cache) {
    ...
    if (!dest_object->have_instance()) {
      if (dest_object->get_bucket()->versioned() &&
          !dest_object->get_bucket()
               ->versioning_enabled()) { //if versioning is suspended
        dest_version = "null";
      } else {
        char buf[OBJ_INSTANCE_LEN + 1];
        gen_rand_alphanumeric_no_underscore(
            dpp->get_cct(), buf, OBJ_INSTANCE_LEN);
        dest_version =
            buf; //version for non-versioned objects, using gen_rand_alphanumeric_no_underscore for the time being
        ldpp_dout(dpp, 10) << "D4NFilterObject::" << __func__
                           << "(): generating version: " << version << dendl;
      }
    } else {
      dest_version = dest_object->get_instance();
    }
    d4n_dest_object->set_object_version(dest_version);
    if (auto ret = read_op->iterate(dpp, 0, (this->get_size() - 1), nullptr, y);
        ret < 0) {
      ldpp_dout(dpp, 10) << "D4NFilterObject::" << __func__
                         << "(): iterate method failed with ret: " << ret
                         << dendl;
      return ret;
    }
    ...
    ldpp_dout(dpp, 20) << "D4NFilterObject::" << __func__
                       << " size is: " << dest_object->get_size() << dendl;
    ...
    if (dest_version.empty()) {
      ldpp_dout(dpp, 10) << "D4NFilterObject::" << __func__
                         << "(): version could not be calculated." << dendl;
    }
  }
  bufferlist bl_val;
  ...
  auto ret = driver->get_policy_driver()->get_cache_policy()->eviction(
      dpp, baseAttrs.size(), y);
  if (ret == 0) {
    ...
    if (ret == 0) {
      ldpp_dout(dpp, 20) << "D4NFilterObject::" << __func__
                         << " version stored in update method is: "
                         << dest_version << dendl;
      bufferlist bl;
      driver->get_policy_driver()->get_cache_policy()->update(
          dpp, key, 0, bl.length(), dest_version, dirty,
          rgw::d4n::RefCount::NOOP, y);
      d4n_dest_object->set_object_version(dest_version);
      ret =
          d4n_dest_object->set_head_obj_dir_entry(dpp, nullptr, y, true, dirty);
      if (ret < 0) {
        ldpp_dout(dpp, 10)
            << "D4NFilterObject::" << __func__
            << "(): BlockDirectory set method failed for head object with ret: "
            << ret << dendl;
        return ret;
      }
      if (dirty) {
        driver->get_policy_driver()->get_cache_policy()->update_dirty_object(
            dpp, key, dest_version, false, this->get_size(), creationTime,
            std::get<rgw_user>(dest_object->get_bucket()->get_owner()), *etag,
            dest_object->get_bucket()->get_name(),
            dest_object->get_bucket()->get_bucket_id(), dest_object->get_key(),
            rgw::d4n::RefCount::NOOP, y);
      }
    }
  }
...
int
D4NFilterObject::load_obj_state(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    bool follow_olh)
{
    ...
    uint32_t flags)
{
  ...
  if (check_head_exists_in_cache_get_oid(
          dpp, head_oid_in_cache, attrs, block, y)) {
    found_in_cache = true;
    if (setattrs != nullptr) {
      /* Ensure setattrs and delattrs do not overlap */
      if (delattrs != nullptr) {
        for (const auto& attr : *delattrs) {
          if (std::find(setattrs->begin(), setattrs->end(), attr) !=
              setattrs->end()) {
      ...
      if (this->driver->get_policy_driver()
              ->get_cache_policy()
              ->update_refcount_if_key_exists(
                  dpp, head_oid_in_cache, rgw::d4n::RefCount::INCR, y)) {
        auto ret = driver->get_cache_driver()->set_attrs(
            dpp, head_oid_in_cache, *setattrs, y);
        this->driver->get_policy_driver()
            ->get_cache_policy()
            ->update_refcount_if_key_exists(
                dpp, head_oid_in_cache, rgw::d4n::RefCount::DECR, y);
        if (ret < 0) {
          ldpp_dout(dpp, 0)
              << "D4NFilterObject::" << __func__
              << "(): CacheDriver set_attrs method failed with ret: " << ret
              << dendl;
          return ret;
        }
      } else {
    ...
    if (delattrs != nullptr) {
      ...
      for (const auto& attr : *delattrs) {
        if (std::find(currentattrs.begin(), currentattrs.end(), attr) ==
            currentattrs.end()) {
          delattrs->erase(std::find(delattrs->begin(), delattrs->end(), attr));
        }
      }
      if (this->driver->get_policy_driver()
              ...
              ->update_refcount_if_key_exists(
                  dpp, head_oid_in_cache, rgw::d4n::RefCount::INCR, y)) {
        auto ret = driver->get_cache_driver()->delete_attrs(
            dpp, head_oid_in_cache, *delattrs, y);
        this->driver->get_policy_driver()
            ->get_cache_policy()
            ->update_refcount_if_key_exists(
                dpp, head_oid_in_cache, rgw::d4n::RefCount::DECR, y);
        if (ret < 0) {
          ldpp_dout(dpp, 0)
              << "D4NFilterObject::" << __func__
              << "(): CacheDriver delete_attrs method failed with ret: " << ret
              << dendl;
          return ret;
        }
      } else {
    ...
    if (block.deleteMarker) {
      ldpp_dout(dpp, 10) << "D4NFilterObject::" << __func__ << "(): object "
                         << this->get_name() << " does not exist." << dendl;
      return -ENOENT;
    }
  }
  ...
  if (!found_in_cache) {
    ...
    if (ret < 0) {
      ldpp_dout(dpp, 0)
          << "D4NFilterObject::" << __func__
          << "(): set_obj_attrs method of backend store failed with ret: "
          << ret << dendl;
      return ret;
    }
  }
...
int
D4NFilterObject::get_obj_attrs_from_cache(
    const DoutPrefixProvider* dpp,
    optional_yield y)
{
  ...
  rgw::sal::Attrs attrs;
  rgw::d4n::CacheBlock block;
  bool found_in_cache = check_head_exists_in_cache_get_oid(
      dpp, head_oid_in_cache, attrs, block, y);
  ...
  if (block.deleteMarker) {
    ...
    ldpp_dout(dpp, 10) << "D4NFilterObject::" << __func__
                       << "(): obj is: " << this->get_obj().key.name << dendl;
    std::string instance;
    for (auto& attr : attrs) {
      if (attr.second.length() > 0) {
        if (attr.first == RGW_CACHE_ATTR_MTIME) {
          ...
          ldpp_dout(dpp, 10)
              << "D4NFilterObject::" << __func__
              << "(): setting version_id to: " << instance << dendl;
        } else if (attr.first == RGW_CACHE_ATTR_SOURC_ZONE) {
          ldpp_dout(dpp, 10) << "D4NFilterObject::" << __func__
                             << "(): setting source zone id." << dendl;
          auto short_zone_id =
              static_cast<uint32_t>(std::stoul(attr.second.to_str()));
          ...
          ldpp_dout(dpp, 10) << "D4NFilterObject::" << __func__
                             << "(): is_multipart: " << this->multipart
                             << " multipart: " << multipart << dendl;
        } else {
          ldpp_dout(dpp, 20)
              << "D4NFilterObject::" << __func__
              << "(): Unexpected attribute; not locally set, attr name: "
              << attr.first << dendl;
        }
      } //end-if
    } //end-for
    if (!instance.empty()) {
      this->set_instance(
          instance); //set this only after setting object state else it won't take effect
    }
    ...
    if (ret < 0) {
      ldpp_dout(dpp, 0) << "D4NFilterObject::" << __func__
                        << "(): D4NFilterObject set_attrs method failed."
                        << dendl;
    }
...
int
D4NFilterObject::set_attr_crypt_parts(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    rgw::sal::Attrs& attrs)
{
  if (attrs.count(RGW_ATTR_CRYPT_MODE)) {
    ...
    uint64_t num_parts = (obj_size % obj_max_chunk_size) == 0
                             ? obj_size / obj_max_chunk_size
                             : (obj_size / obj_max_chunk_size) + 1;
    ...
    bool dirty)
{
  ...
  attrs[RGW_CACHE_ATTR_ACCOUNTED_SIZE] =
      std::move(bl_val); // will this get updated?
...
int
D4NFilterObject::calculate_version(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    std::string& version,
    rgw::sal::Attrs& attrs)
{
  ...
  ldpp_dout(dpp, 10) << "D4NFilterObject::" << __func__
                     ...
                     << " instance: " << this->have_instance() << dendl;
  if (!this->have_instance() && version.empty()) {
    bufferlist bl = attrs[RGW_ATTR_ID_TAG];
    if (bl.length()) {
      version = bl.c_str();
      if (!version.empty()) {
        ldpp_dout(dpp, 20) << __func__ << " id tag version is: " << version
                           << dendl;
      }
    }
  }
...
int
D4NFilterObject::create_delete_marker(
    const DoutPrefixProvider* dpp,
    optional_yield y)
{
  ...
  this->version = buf;
  ldpp_dout(dpp, 20) << "D4NFilterObject::" << __func__
                     << "(): generating delete marker: " << version << dendl;
  if (this->get_bucket()->versioned() &&
      !this->get_bucket()->versioning_enabled()) { //if versioning is suspended
  ...
  bufferlist bl;
  ldpp_dout(dpp, 20) << "D4NFilterObject::" << __func__ << "(): key is: " << key
                     << dendl;
  auto ret = driver->get_policy_driver()->get_cache_policy()->eviction(
      dpp, attrs.size(), y);
  if (ret == 0) {
    ...
    if (ret == 0) {
      ldpp_dout(dpp, 20) << "D4NFilterObject::" << __func__
                         ...
                         << version << dendl;
      driver->get_policy_driver()->get_cache_policy()->update(
          dpp, key, 0, bl.length(), version, true, rgw::d4n::RefCount::NOOP, y);
      std::vector<std::string> exec_responses;
      ret = this->set_head_obj_dir_entry(dpp, &exec_responses, y, true, true);
      if (ret < 0) {
        ldpp_dout(dpp, 0)
            << "D4NFilterObject::" << __func__
            << "(): BlockDirectory set method failed for head object, ret="
            << ret << dendl;
        return ret;
      }
      auto creationTime = ceph::real_clock::to_double(this->get_mtime());
      ldpp_dout(dpp, 0) << "D4NFilterObject::" << __func__ << "(): key=" << key
                        << dendl;
      std::string objEtag = "";
      driver->get_policy_driver()->get_cache_policy()->update_dirty_object(
          dpp, key, version, true, this->get_accounted_size(), creationTime,
          std::get<rgw_user>(this->get_bucket()->get_owner()), objEtag,
          this->get_bucket()->get_name(), this->get_bucket()->get_bucket_id(),
          this->get_key(), rgw::d4n::RefCount::NOOP, y);
    } else { //if get_cache_driver()->put()
      ldpp_dout(dpp, 0) << "D4NFilterObject::" << __func__
                        << "(): put failed for key, ret=" << ret
                        << " key: " << key << dendl;
      return ret;
    }
  } else {
    ldpp_dout(dpp, 0) << "D4NFilterObject::" << __func__
                      << "(): eviction failed for key, ret=" << ret << dendl;
    return ret;
  }
...
int
D4NFilterObject::set_head_obj_dir_entry(
    const DoutPrefixProvider* dpp,
    std::vector<std::string>* exec_responses,
    optional_yield y,
    ...
    bool dirty)
{
  ldpp_dout(dpp, 10) << "D4NFilterObject::" << __func__
                     << "(): object name: " << this->get_name()
                     << " bucket name: " << this->get_bucket()->get_name()
                     << dendl;
  rgw::d4n::CacheBlock block;
  ...
  ldpp_dout(dpp, 10) << "D4NFilterObject::" << __func__ << "(): etag: " << etag
                     << dendl;
  ...
  if (acl_it != attrs.end()) {
    ...
    try {
      ...
      ldpp_dout(dpp, 0)
          << "D4NFilterObject::" << __func__
          << "(): ERROR: could not decode policy, caught buffer::error"
          << dendl;
      return -EIO;
    }
    ...
    ldpp_dout(dpp, 20) << "D4NFilterObject::" << __func__
                       << "(): INFO: user_d: " << user.to_str() << dendl;
    ldpp_dout(dpp, 20) << "D4NFilterObject::" << __func__
                       << "(): INFO: display_name: " << owner.display_name
                       << dendl;
    user_id = user.to_str();
  ...
  if (is_latest_version) {
    ...
    ldpp_dout(dpp, 10) << "D4NFilterObject::" << __func__
                       << "(): objName after special Handling: " << objName
                       << dendl;
    rgw::d4n::CacheObj object = rgw::d4n::CacheObj{
        .objName = objName,
        .bucketName = this->get_bucket()->get_bucket_id(),
        .creationTime =
            std::to_string(ceph::real_clock::to_double(this->get_mtime())),
        .dirty = dirty,
    ...
    if (dirty) {
      ...
      if (!(this->get_bucket()->versioned())) {
        ...
        if (ret < 0) {
          ldpp_dout(dpp, 10)
              << "D4NFilterObject::"
              << __func__ << "(): BlockDirectory set method failed for null head object with ret: "
              << ret << dendl;
          return ret;
        }
      }
      ...
      if (!this->get_bucket()->versioned() ||
          !this->get_bucket()->versioning_enabled()) {
        object_version = "null";
      } else {
      ...
      ldpp_dout(dpp, 10) << "D4NFilterObject::" << __func__
                         << "(): Score of object name: " << this->get_name()
                         << " version: " << object_version << " is: " << score
                         << ret << dendl;
      ...
      if (ret < 0) {
        ldpp_dout(dpp, 10)
            << "D4NFilterObject::" << __func__
            << "(): Failed to add version to ordered set with error: " << ret
            << dendl;
        return ret;
      }
      ...
      ret = bucketDir->zadd(
          dpp, this->get_bucket()->get_bucket_id(), 0, this->get_name(), y, &p);
      if (ret < 0) {
        ldpp_dout(dpp, 10)
            << "D4NFilterObject::" << __func__
            << "(): Failed to add object to ordered set with error: " << ret
            << dendl;
        return ret;
      }
      p.execute(dpp, y);
      ...
      if (ret == -ENOENT) {
        if (!(this->get_bucket()->versioned())) {
          ...
          if (ret < 0) {
            ldpp_dout(dpp, 10)
                << "D4NFilterObject::"
                << __func__ << "(): BlockDirectory set method failed for head object with ret: "
                << ret << dendl;
            return ret;
          }
          //bucket is non versioned, set a null instance
          ...
          if (ret < 0) {
            ldpp_dout(dpp, 10) << "D4NFilterObject::" << __func__
                               << "(): BlockDirectory set method failed for "
                                  "null head object with ret: "
                               << ret << dendl;
            return ret;
          }
          p.execute(dpp, y);
        ...
        ldpp_dout(dpp, 10)
            << "D4NFilterObject::" << __func__
            << "(): BlockDirectory get method failed for head object with ret: "
            << ret << dendl;
        ...
        if (this->get_bucket()->versioned()) {
          ...
          if (ret < 0 && ret != -ENOENT) {
            ldpp_dout(dpp, 10)
                << "D4NFilterObject::"
                << __func__ << "(): BlockDirectory del method failed for head object with ret: "
                << ret << dendl;
          }
        }
  ...
  if (this->get_bucket()->versioned()) {
    ...
    version_object.hostsList.insert(
    ...
    if (ret < 0) {
      ldpp_dout(dpp, 10) << "D4NFilterObject::" << __func__
                         << "(): BlockDirectory set method failed for "
                            "versioned head object with ret: "
                         << ret << dendl;
      return ret;
    }
...
int
D4NFilterObject::set_data_block_dir_entries(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    std::string& version,
    bool dirty)
{
  ...
  do {
    ...
    off_t cur_size =
        std::min<off_t>(fst + dpp->get_cct()->_conf->rgw_max_chunk_size, lst);
    off_t cur_len = cur_size - fst;
  ...
  if (ret < 0) {
    ldpp_dout(dpp, 0)
        << "D4NFilterWriter::" << __func__
        << "(): BlockDirectory pipelined get() method failed, ret=" << ret
        << dendl;
    return ret;
  }
  ...
  for (auto& block : blocks) {
    ...
    block.cacheObj.hostsList.insert(
        dpp->get_cct()->_conf->rgw_d4n_local_rgw_address);
  ...
  if ((ret = blockDir->set(dpp, blocks, y)) < 0) {
    ldpp_dout(dpp, 0)
        << "D4NFilterWriter::" << __func__
        << "(): BlockDirectory pipelined set() method failed, ret=" << ret
        << dendl;
    return ret;
  }
...
int
D4NFilterObject::delete_cache_entry(
    const DoutPrefixProvider* dpp,
    const std::string key,
    optional_yield y)
{
  int ret;
  if ((ret = driver->get_cache_driver()->delete_data(dpp, key, y)) ==
      0) { // Inline cache delete
    if (!(ret = driver->get_policy_driver()->get_cache_policy()->erase(
              dpp, key, y))) {
      ldpp_dout(dpp, 10) << "Failed to delete policy entry for: " << key
                         << ", ret=" << ret << dendl;
      return ret;
    }
  } else {
    ldpp_dout(dpp, 10) << "Failed to delete object in cache for: " << key
                       << ", ret=" << ret << dendl;
    return ret;
  }
...
int
D4NFilterObject::delete_data_block_cache_entries(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    std::string& version,
    bool dirty)
{
  ...
  do {
    ...
    std::string key = get_key_in_cache(
        get_cache_block_prefix(this, version), std::to_string(fst),
...
bool
D4NFilterObject::check_head_exists_in_cache_get_oid(
    const DoutPrefixProvider* dpp,
    std::string& head_oid_in_cache,
    rgw::sal::Attrs& attrs,
    ...
    optional_yield y)
{
  ...
  ldpp_dout(dpp, 20) << "D4NFilterObject::" << __func__
                     << "(): objName: " << objName << dendl;
  rgw::d4n::CacheObj object = rgw::d4n::CacheObj{
      .objName =
          objName, //version-enabled buckets will not have version for latest version, so this will work even when version is not provided in input
      .bucketName = this->get_bucket()->get_bucket_id(),
  };

  rgw::d4n::CacheBlock block =
      rgw::d4n::CacheBlock{.cacheObj = object, .blockID = 0, .size = 0};
  ...
  if ((ret = blockDir->get(dpp, &block, y)) == 0) {
    ...
    if (this->driver->get_policy_driver()
            ->get_cache_policy()
            ->update_refcount_if_key_exists(
                dpp, key, rgw::d4n::RefCount::INCR, y)) {
      ldpp_dout(dpp, 10) << "D4NFilterObject::" << __func__
                         << "(): Is block dirty: " << block.cacheObj.dirty
                         << dendl;
      ldpp_dout(dpp, 10) << "D4NFilterObject::" << __func__
                         << "(): version: " << block.version << dendl;
      head_oid_in_cache = get_cache_block_prefix(this, version);
      ldpp_dout(dpp, 10) << "D4NFilterObject::" << __func__
                         << "(): Fetching attrs from cache for head obj id: "
                         << head_oid_in_cache << dendl;
      auto ret = this->driver->get_cache_driver()->get_attrs(
          dpp, head_oid_in_cache, attrs, y);
      if (ret < 0) {
        found_in_cache = false;
        ldpp_dout(dpp, 10) << "D4NFilterObject::" << __func__
                           << "(): CacheDriver get_attrs method failed."
                           << dendl;
      }
      std::string key = head_oid_in_cache;
      this->driver->get_policy_driver()->get_cache_policy()->update(
          dpp, key, 0, 0, version, block.cacheObj.dirty,
          rgw::d4n::RefCount::DECR, y);
    ...
    found_in_cache = false;
    ldpp_dout(dpp, 0) << "D4NFilterObject::" << __func__
                      << "(): BlockDirectory get method failed, ret=" << ret
                      << dendl;
  }
...
int
D4NFilterObject::get_obj_attrs(optional_yield y, const DoutPrefixProvider* dpp)
{
  ...
  if ((ret = get_obj_attrs_from_cache(dpp, y)) == -ENOENT) {
    ...
    std::string version;
    ldpp_dout(dpp, 10) << "D4NFilterObject::" << __func__
                       << "(): Fetching attrs from backend store." << dendl;
    auto ret = next->get_obj_attrs(y, dpp);
    if (ret < 0) {
      ldpp_dout(dpp, 0)
          << "D4NFilterObject::" << __func__
          << "(): Failed to fetching attrs from backend store with ret: " << ret
          << dendl;
      return ret;
    }
    ...
    ldpp_dout(dpp, 10) << "D4NFilterObject::" << __func__
                       << "(): this->obj oid is: " << this->obj.key.name
                       << "instance is: " << this->obj.key.instance << dendl;
    ...
    if (this->driver->get_policy_driver()
            ...
            ->update_refcount_if_key_exists(
                dpp, head_oid_in_cache, rgw::d4n::RefCount::INCR, y)) {
      ret = this->driver->get_cache_driver()->set_attrs(
          dpp, head_oid_in_cache, attrs, y);
    } else {
      ret = this->driver->get_policy_driver()->get_cache_policy()->eviction(
          dpp, attrs.size(), y);
      if (ret == 0) {
        bufferlist bl;
        ret = this->driver->get_cache_driver()->put(
            dpp, head_oid_in_cache, bl, 0, attrs, y);
      } else {
        ldpp_dout(dpp, 0) << "D4NFilterObject::" << __func__
                          << "(): Failed to evict data, ret=" << ret << dendl;
      }
    }
    if (ret == 0) {
      ldpp_dout(dpp, 20) << "D4NFilterObject::" << __func__
                         << " version stored in update method is: "
                         << this->get_object_version() << dendl;
      this->driver->get_policy_driver()->get_cache_policy()->update(
          dpp, head_oid_in_cache, 0, 0, version, false,
          rgw::d4n::RefCount::DECR, y);
      ...
      ldpp_dout(dpp, 0)
          << "D4NFilterObject::" << __func__
          << "(): failed to cache head object in cache backend, ret=" << ret
          << dendl;
    }
  } else {
    if (perfcounter) {
...
int
D4NFilterObject::modify_obj_attrs(
    ...
    uint32_t flags)
{
  ...
  if (check_head_exists_in_cache_get_oid(
          dpp, head_oid_in_cache, attrs, block, y)) {
    if (auto ret = driver->get_cache_driver()->update_attrs(
            dpp, head_oid_in_cache, update, y);
        ret < 0) {
      ldpp_dout(dpp, 0)
          << "D4NFilterObject::" << __func__
          << "(): CacheDriver update_attrs method failed with ret: " << ret
          << dendl;
      return ret;
    }
  } else {
    ...
    if (ret < 0) {
      ldpp_dout(dpp, 0)
          << "D4NFilterObject::" << __func__
          << "(): modify_obj_attrs of backend store failed with ret: " << ret
          << dendl;
      return ret;
    }
  }
...
int
D4NFilterObject::delete_obj_attrs(
    const DoutPrefixProvider* dpp,
    const char* attr_name,
    optional_yield y)
{
  ...
  if (check_head_exists_in_cache_get_oid(
    ...
    if (std::find_if(
            currentattrs.begin(), currentattrs.end(), [&](const auto& pair) {
              return pair.first == attr->first;
            }) != currentattrs.end()) {
      if (this->driver->get_policy_driver()
              ->get_cache_policy()
              ->update_refcount_if_key_exists(
                  dpp, head_oid_in_cache, rgw::d4n::RefCount::INCR, y)) {
        auto ret = driver->get_cache_driver()->delete_attrs(
            dpp, head_oid_in_cache, delattr, y);
        this->driver->get_policy_driver()
            ->get_cache_policy()
            ->update_refcount_if_key_exists(
                dpp, head_oid_in_cache, rgw::d4n::RefCount::DECR, y);
  ...
  if (!found_in_cache) {
    ...
    if (auto ret = next->delete_obj_attrs(dpp, attr_name, y); ret < 0) {
      ldpp_dout(dpp, 0)
          << "D4NFilterObject::" << __func__
          << "(): delete_obj_attrs method of backend store failed with ret: "
          << ret << dendl;
      return ret;
    }
  }
...
std::unique_ptr<Object>
D4NFilterDriver::get_object(const rgw_obj_key& k)
{
  std::unique_ptr<Object> o = next->get_object(k);
...
D4NFilterDriver::get_atomic_writer(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    rgw::sal::Object* obj,
    const ACLOwner& owner,
...
{
  std::unique_ptr<Writer> writer = next->get_atomic_writer(
      dpp, y, nextObject(obj), owner, ptail_placement_rule, olh_epoch,
      unique_tag);

  return std::make_unique<D4NFilterWriter>(
      std::move(writer), this, obj, dpp, true, y);
}

void
D4NFilterDriver::shutdown()
{
...
int
D4NFilterObject::D4NFilterReadOp::prepare(
    optional_yield y,
    const DoutPrefixProvider* dpp)
{
  ...
  if ((ret = source->get_obj_attrs_from_cache(dpp, y)) == -ENOENT) {
    ldpp_dout(dpp, 10) << "D4NFilterObject::D4NFilterReadOp::" << __func__
    ...
    std::string version;
    ldpp_dout(dpp, 10) << "D4NFilterObject::" << __func__
                       << "(): fetching head object from backend store"
                       << dendl;
    next->params = params;
    auto ret = next->prepare(y, dpp);
    if (ret < 0) {
      ldpp_dout(dpp, 0) << "D4NFilterObject::" << __func__
                        << "(): next->prepare method failed, ret=" << ret
                        << dendl;
      return ret;
    }
    ...
    ret = source->driver->get_policy_driver()->get_cache_policy()->eviction(
        dpp, attrs.size(), y);
    if (ret == 0) {
      ret = source->driver->get_cache_driver()->put(
          dpp, head_oid_in_cache, bl, 0, attrs, y);
      if (ret == 0) {
        ldpp_dout(dpp, 20) << "D4NFilterObject::" << __func__
                           << " version stored in update method is: "
                           << this->source->get_object_version() << dendl;
        source->driver->get_policy_driver()->get_cache_policy()->update(
            dpp, head_oid_in_cache, 0, bl.length(), version, false,
            rgw::d4n::RefCount::NOOP, y);
        ...
        ldpp_dout(dpp, 0) << "D4NFilterObject::" << __func__
                          << "(): put for head object failed, ret=" << ret
                          << dendl;
      }
    } else {
      ldpp_dout(dpp, 0)
          << "D4NFilterObject::" << __func__
          << "(): failed to cache head object during eviction, ret=" << ret
          << dendl;
    }
  } else {
    /* 
    ...
    */
    if (params.part_num || (!params.part_num && source->is_multipart())) {
      ldpp_dout(dpp, 0) << "D4NFilterObject::" << __func__
                        << "(): source->is_multipart()= "
                        << source->is_multipart() << dendl;
      if (params.part_num) {
        ldpp_dout(dpp, 0) << "D4NFilterObject::" << __func__
                          << "(): *(params.part_num)= " << *(params.part_num)
                          << dendl;
      }
      if (!source->is_multipart()) {
        ...
        if (ret < 0) {
          ldpp_dout(dpp, 0) << "D4NFilterObject::" << __func__
                            << "(): next->prepare failed, ret=" << ret << dendl;
          return ret;
        }
        params.parts_count = next->params.parts_count;
        return 0;
    ...
    if (params.mod_ptr || params.unmod_ptr) {
      if (params.mod_ptr && !params.if_nomatch) {
        ldpp_dout(dpp, 10) << "If-Modified-Since: " << *params.mod_ptr
                           << " Last-Modified: " << source->get_mtime()
                           << dendl;
      ...
      if (params.unmod_ptr && !params.if_match) {
        ldpp_dout(dpp, 10) << "If-Modified-Since: " << *params.unmod_ptr
                           << " Last-Modified: " << source->get_mtime()
                           << dendl;
    ...
    if (params.if_match) {
      ...
      ldpp_dout(dpp, 10) << "If-Match: " << if_match_str
                         << " ETAG: " << etag_bl.c_str() << dendl;

      if (if_match_str.compare(
              0, etag_bl.length(), etag_bl.c_str(), etag_bl.length()) != 0) {
        return -ERR_PRECONDITION_FAILED;
      }
    }
    if (params.if_nomatch) {
      std::string if_nomatch_str = rgw_string_unquote(params.if_nomatch);
      ldpp_dout(dpp, 10) << "If-No-Match: " << if_nomatch_str
                         << " ETAG: " << etag_bl.c_str() << dendl;
...
void
D4NFilterObject::D4NFilterReadOp::cancel()
{
  aio->drain();
}

int
D4NFilterObject::D4NFilterReadOp::drain(
    const DoutPrefixProvider* dpp,
    optional_yield y)
{
  auto c = aio->drain();
  ...
  for (auto it : blocks_info) {
    ...
    std::string oid_in_cache = prefix + CACHE_DELIM + std::to_string(ofs) +
                               CACHE_DELIM + std::to_string(len);
    source->driver->get_policy_driver()
        ->get_cache_policy()
        ->update_refcount_if_key_exists(
            dpp, oid_in_cache, rgw::d4n::RefCount::DECR, y);
  }
  if (r < 0) {
    cancel();
    return r;
...
int
D4NFilterObject::D4NFilterReadOp::flush(
    const DoutPrefixProvider* dpp,
    ...
    optional_yield y)
{
  ...
  while (!completed.empty() && completed.front().id == offset) {
    ...
    ldpp_dout(dpp, 20)
        << "D4NFilterObject::flush:: calling handle_data for offset: " << offset
        << " bufferlist length: " << bl.length() << dendl;
    ...
    if (it != blocks_info.end()) {
      ...
      std::string oid_in_cache =
          get_key_in_cache(prefix, std::to_string(ofs), std::to_string(len));

      ldpp_dout(dpp, 20) << "D4NFilterObject::" << __func__
                         << " calling update for offset: " << offset
                         << " adjusted offset: " << ofs << " length: " << len
                         << " oid_in_cache: " << oid_in_cache << dendl;
      ldpp_dout(dpp, 20) << "D4NFilterObject::" << __func__
                         << " version stored in update method is: " << version
                         << " " << source->get_object_version() << dendl;
      source->driver->get_policy_driver()->get_cache_policy()->update(
          dpp, oid_in_cache, ofs, len, version, std::nullopt,
          rgw::d4n::RefCount::DECR, y);
      blocks_info.erase(it);
      if (source->dest_object && source->dest_bucket) {
        D4NFilterObject* d4n_dest_object =
            dynamic_cast<D4NFilterObject*>(source->dest_object);
        ...
        dest_block.size = len;
        dest_block.cacheObj.hostsList.insert(
            dpp->get_cct()->_conf->rgw_d4n_local_rgw_address);
        dest_block.version = dest_version;
        dest_block.cacheObj.dirty = true;
        std::string key = get_key_in_cache(
            get_cache_block_prefix(source->dest_object, dest_version),
            std::to_string(ofs), std::to_string(len));
        auto ret =
            source->driver->get_policy_driver()->get_cache_policy()->eviction(
                dpp, dest_block.size, y);
        if (ret == 0) {
          rgw::sal::Attrs attrs;
          ldpp_dout(dpp, 20)
              << "D4NFilterObject::" << __func__
              << " destination object version in update method is: "
              << dest_version << dendl;
          // destination key is the same as key
          ret = source->driver->get_cache_driver()->put(
              dpp, key, bl, bl.length(), attrs, y);
          if (ret == 0) {
            source->driver->get_policy_driver()->get_cache_policy()->update(
                dpp, key, ofs, bl.length(), dest_version, true,
                rgw::d4n::RefCount::NOOP, y);
          }
          if (ret = source->driver->get_block_dir()->set(dpp, &dest_block, y);
              ret < 0) {
            ldpp_dout(dpp, 20)
                << "D4NFilterObject::" << __func__
                << " BlockDirectory set failed with ret: " << ret << dendl;
          }
        } else {
          ldpp_dout(dpp, 20) << "D4NFilterObject::" << __func__
                             << " eviction returned ret: " << ret << dendl;
        }
      }
    } else {
      ldpp_dout(dpp, 0) << "D4NFilterObject::" << __func__
                        << " offset not found: " << offset << dendl;
    }
...
int
D4NFilterObject::D4NFilterReadOp::iterate(
    const DoutPrefixProvider* dpp,
    int64_t ofs,
    int64_t end,
    RGWGetDataCB* cb,
    optional_yield y)
{
  ...
  ldpp_dout(dpp, 20) << "D4NFilterObject::iterate:: " << "prefix: " << prefix
                     << dendl;
  ldpp_dout(dpp, 20) << "D4NFilterObject::iterate:: " << "oid: "
                     << source->get_key().get_oid() << " ofs: " << ofs
                     << " end: " << end << dendl;
  ...
  uint64_t max_chunk_size =
      std::min(g_conf()->rgw_max_chunk_size, source->get_size());
  uint64_t start_part_num = 0;
  uint64_t part_num = ofs /
                      max_chunk_size; //part num of ofs wrt start of the object
  uint64_t adjusted_start_ofs =
      part_num *
      max_chunk_size; //in case of ranged request, adjust the start offset to the beginning of a chunk/ part
  uint64_t start_diff_ofs =
      ofs -
      adjusted_start_ofs; //difference between actual start offset and adjusted start offset
  off_t len = (end - adjusted_start_ofs) + 1;
  uint64_t num_parts =
      (len % max_chunk_size) == 0
          ? len / max_chunk_size
          : (len / max_chunk_size) +
                1; //calculate num parts based on adjusted offset
  uint64_t last_part_num = end / max_chunk_size;
  uint64_t adjusted_end_ofs = std::min(
      ((last_part_num + 1) * max_chunk_size - 1),
      (source->get_size() -
       1)); //align end offset to max_chunk_size boundary in case of ranged request
  uint64_t end_diff_ofs =
      adjusted_end_ofs -
      end; //difference between actual end offset and adjusted end offset
  ...
  ldpp_dout(dpp, 20) << "D4NFilterObject::iterate:: "
                     << " adjusted_start_offset: " << adjusted_start_ofs
                     << dendl;
  ldpp_dout(dpp, 20) << "D4NFilterObject::iterate:: " << " adjusted_end_ofs: "
                     << adjusted_end_ofs << dendl;
  ldpp_dout(dpp, 20) << "D4NFilterObject::iterate:: " << " adjusted_len: "
                     << adjusted_len << dendl;
  ldpp_dout(dpp, 20) << "D4NFilterObject::iterate:: " << " len: " << len
                     << dendl;

  if ((params.part_num && !source->is_multipart()) || !params.part_num) {
    aio = rgw::make_throttle(window_size, y);

    ldpp_dout(dpp, 20) << "D4NFilterObject::iterate:: " << "max_chunk_size "
                       << max_chunk_size << " num_parts " << num_parts << dendl;
    ...
    do {
      uint64_t
          id = adjusted_start_ofs,
          read_ofs =
              0; //read_ofs is the actual offset to start reading from the current part/ chunk
      ...
      ceph::bufferlist bl;
      std::string oid_in_cache = get_key_in_cache(
          prefix, std::to_string(adjusted_start_ofs), std::to_string(part_len));

      ldpp_dout(dpp, 20) << "D4NFilterObject::iterate:: " << __func__ << " "
                         << __LINE__
                         << "(): READ FROM CACHE: oid=" << oid_in_cache
                         << " length to read is: " << len_to_read
                         << " part num: " << start_part_num
                         << " read_ofs: " << read_ofs
      ...
      if (policy->update_refcount_if_key_exists(
              dpp, oid_in_cache, rgw::d4n::RefCount::INCR, y) > 0) {
        ldpp_dout(dpp, 20) << "D4NFilterObject::iterate:: " << __func__
                           << "(): " << __LINE__
                           << ": READ FROM CACHE: oid_in_cache=" << oid_in_cache
                           << dendl;
        // Read From Cache
        auto completed = cache_driver->get_async(
            dpp, y, aio.get(), oid_in_cache, read_ofs, len_to_read, cost, id);
        this->blocks_info.insert(
            std::make_pair(id, std::make_pair(adjusted_start_ofs, part_len)));
        ldpp_dout(dpp, 20) << "D4NFilterObject::iterate:: " << __func__
                           << "(): Info: flushing data for oid: "
                           << oid_in_cache << dendl;
        auto r = flush(dpp, std::move(completed), y);
        if (r < 0) {
          drain(dpp, y);
          ldpp_dout(dpp, 0) << "D4NFilterObject::iterate:: " << __func__
                            << "(): Error: failed to flush, ret=" << r << dendl;
          return r;
        }
        ...
        if ((ret = block_dir->get(dpp, &block, y)) == 0) {
          if (block.version != version) {
            ...
            ldpp_dout(dpp, 20)
                << "D4NFilterObject::iterate:: " << __func__
                << "(): Info: Version mismatch, draining data for oid: "
                << oid_in_cache << dendl;
            auto r = drain(dpp, y);
            if (r < 0) {
              ldpp_dout(dpp, 0)
                  << "D4NFilterObject::iterate:: " << __func__
                  << "(): Error: failed to drain, ret=" << r << dendl;
              return r;
            }
            break;
          } //end if block.version != version
          auto it = block.cacheObj.hostsList.find(
              dpp->get_cct()->_conf->rgw_d4n_local_rgw_address);
          auto hostsListSize = block.cacheObj.hostsList.size();
          if (it != block.cacheObj.hostsList.end()) {
            if ((r = block_dir->remove_host(
                     dpp, &block,
                     dpp->get_cct()->_conf->rgw_d4n_local_rgw_address, y)) < 0) {
              ldpp_dout(dpp, 10)
                  << "D4NFilterObject::iterate:: "
                  << __func__ << "(): Error: failed to remove incorrect host from block with oid="
                  << oid_in_cache << ", ret=" << r << dendl;
              hostsListSize = hostsListSize - 1;
            }
          }
          if (hostsListSize > 0) { /* Remote copy */
            ldpp_dout(dpp, 20) << "D4NFilterObject::iterate:: " << __func__
                               << "(): Block with oid=" << oid_in_cache
                               << " found in remote cache." << dendl;
            // TODO: Retrieve remotely
            // Policy decision: should we cache remote blocks locally?
          } else {
            ldpp_dout(dpp, 20)
                << "D4NFilterObject::iterate:: " << __func__
                << "(): Info: draining data for oid: " << oid_in_cache << dendl;
            auto r = drain(dpp, y);
            if (r < 0) {
              ldpp_dout(dpp, 0)
                  << "D4NFilterObject::iterate:: " << __func__
          ...
          ldpp_dout(dpp, 5)
              << "Failed to fetch block for: " << block.cacheObj.objName
              << " blockID: " << block.blockID << " block size: " << block.size
              << ", ret=" << ret << dendl;
          ldpp_dout(dpp, 20)
              << "D4NFilterObject::iterate:: " << __func__
              << "(): Info: draining data for oid: " << oid_in_cache << dendl;
  ...
  ldpp_dout(dpp, 20) << "D4NFilterObject::iterate:: " << __func__
                     << "(): Fetching object from backend store" << dendl;
  ...
  ldpp_dout(dpp, 20) << "D4NFilterObject::iterate:: " << __func__
                     << "(): adjusted_start_ofs: " << adjusted_start_ofs
                     << " end: " << end << dendl;
  auto r = next->iterate(
      dpp, adjusted_start_ofs, adjusted_end_ofs, this->cb.get(), y);
  //calculate the number of blocks read from backend store, and increment the perfcounter using that
  if (perfcounter) {
    uint64_t len_to_read_from_store =
        ((adjusted_end_ofs - adjusted_start_ofs) + 1);
    uint64_t num_blocks = (len_to_read_from_store % max_chunk_size) == 0
                              ? len_to_read_from_store / max_chunk_size
                              : (len_to_read_from_store / max_chunk_size) + 1;
  ...
  if (r < 0) {
    ldpp_dout(dpp, 0)
        << "D4NFilterObject::iterate:: " << __func__
        << "(): Error: failed to fetch object from backend store, ret=" << r
        << dendl;
    return r;
  }
...
int
D4NFilterObject::D4NFilterReadOp::get_attr(
    const DoutPrefixProvider* dpp,
    const char* name,
    bufferlist& dest,
    optional_yield y)
{
  ...
  if (attrs.empty()) {
    ...
    if (ret < 0) {
      ldpp_dout(dpp, 0) << "D4NFilterObject::" << __func__
                        << "(): Error: failed to fetch attrs, ret=" << ret
                        << dendl;
      return ret;
    }
  ...
  if (it == attrs.end()) {
    ldpp_dout(dpp, 0) << "D4NFilterObject::" << __func__
                      << "(): Attribute value NOT found for attr name= " << name
                      << dendl;
...
int
D4NFilterObject::D4NFilterReadOp::D4NFilterGetCB::flush_last_part()
{
  last_part = true;
...
int
D4NFilterObject::D4NFilterReadOp::D4NFilterGetCB::handle_data(
    bufferlist& bl,
    off_t bl_ofs,
    off_t bl_len)
{
  auto rgw_max_chunk_size = g_conf()->rgw_max_chunk_size;
  ldpp_dout(dpp, 20) << __func__ << ": bl_ofs is: " << bl_ofs
                     << " bl_len is: " << bl_len << " part_num: " << part_num
                     << dendl;
  ldpp_dout(dpp, 20) << __func__ << ": start_ofs is: " << start_ofs
                     << " end_ofs is: " << end_ofs << " part_num: " << part_num
                     << dendl;
  if (!last_part && bl.length() <= rgw_max_chunk_size) {
    if (client_cb) {
      ...
      if (bl_ofs != start_ofs && part_num == 0) {
        if (start_ofs <
            bl_len) { // this can happen in case of multipart where each chunk returned is not always of size rgw_max_chunk_size
          off_t bl_part_len = bl_len - start_ofs;
          ldpp_dout(dpp, 20)
              << __func__ << ": bl_part_len is: " << bl_part_len << dendl;
          bufferlist bl_part;
          bl.begin(start_ofs).copy(bl_part_len, bl_part);
          ldpp_dout(dpp, 20)
              << __func__ << ": bl_part.length() is: " << bl_part.length()
              << dendl;
          ...
          ldpp_dout(dpp, 20)
              << __func__ << ": New value ofs is: " << start_ofs << dendl;
        }
        ...
        ldpp_dout(dpp, 20) << __func__ << ": last part bl_part.length() is: "
                           << bl_part.length() << dendl;
  ...
  if (write_to_cache) {
    ...
    block.cacheObj.bucketName = source->get_bucket()->get_bucket_id();
    std::stringstream s;
    block.cacheObj.creationTime =
        std::to_string(ceph::real_clock::to_double(source->get_mtime()));
    ...
    if (source->dest_object && source->dest_bucket) {
      ...
      dest_block.cacheObj.bucketName =
          source->dest_object->get_bucket()->get_bucket_id();
    ...
    if (bl.length() > 0 &&
        last_part) { // if bl = bl_rem has data and this is the last part, write it to cache
      std::string oid = get_key_in_cache(
          prefix, std::to_string(adjusted_start_ofs), std::to_string(bl_len));
      if (!policy->exist_key(oid)) {
        ...
        if (ret == 0) {
          ...
          if (ret == 0) {
            std::string objEtag = "";
            policy->update(
                dpp, oid, adjusted_start_ofs, bl.length(), version, dirty,
                rgw::d4n::RefCount::NOOP, *y);
            blocks.emplace_back(block);
          } else {
            ldpp_dout(dpp, 10)
                << "D4NFilterObject::D4NFilterReadOp::D4NFilterGetCB::"
                << __func__ << "(): put() to cache backend failed, ret=" << ret
                << dendl;
          }
        } //end-if ret == 0
      } //end-if exist_key
      if (source->dest_object && source->dest_bucket) {
        D4NFilterObject* d4n_dest_object =
            dynamic_cast<D4NFilterObject*>(source->dest_object);
        std::string dest_version = d4n_dest_object->get_object_version();
        std::string dest_oid = get_key_in_cache(
            dest_prefix, std::to_string(adjusted_start_ofs),
            std::to_string(bl_len));
        ...
        if (ret == 0) {
          ...
          if (ret == 0) {
            policy->update(
                dpp, dest_oid, adjusted_start_ofs, bl.length(), dest_version,
                dirty, rgw::d4n::RefCount::NOOP, *y);
            dest_blocks.emplace_back(dest_block);
          }
        }
      }
    } else if (
        bl.length() == rgw_max_chunk_size &&
        bl_rem.length() ==
            0) { // if bl is the same size as rgw_max_chunk_size, write it to cache
      std::string oid = get_key_in_cache(
          prefix, std::to_string(adjusted_start_ofs), std::to_string(bl_len));
      block.blockID = adjusted_start_ofs;
      block.size = bl.length();
      ...
      if (bl_rem.length() == rgw_max_chunk_size) {
        std::string oid = prefix + CACHE_DELIM +
                          std::to_string(adjusted_start_ofs) + CACHE_DELIM +
        ...
        if (!policy->exist_key(oid)) {
          ...
          if (ret == 0) {
            ret =
                cache_driver->put(dpp, oid, bl_rem, bl_rem.length(), attrs, *y);
            if (ret == 0) {
              policy->update(
                  dpp, oid, adjusted_start_ofs, bl_rem.length(), version, dirty,
                  rgw::d4n::RefCount::NOOP, *y);
              blocks.emplace_back(block);
            } else {
              ldpp_dout(dpp, 0)
                  << "D4NFilterObject::D4NFilterReadOp::D4NFilterGetCB::"
                  << __func__
                  << "(): put() to cache backend failed, ret=" << ret << dendl;
            }
          } else {
            ldpp_dout(dpp, 0)
                << "D4N Filter: " << __func__
                << " An error occurred during eviction, ret=" << ret << dendl;
          }
        }
        ...
        if (source->dest_object && source->dest_bucket) {
          D4NFilterObject* d4n_dest_object =
              dynamic_cast<D4NFilterObject*>(source->dest_object);
          std::string dest_version = d4n_dest_object->get_object_version();
          std::string dest_oid = dest_prefix + CACHE_DELIM +
                                 std::to_string(adjusted_start_ofs) +
                                 CACHE_DELIM + std::to_string(bl_rem.length());
          dest_block.blockID = adjusted_start_ofs;
          dest_block.size = bl_rem.length();
          auto ret = policy->eviction(dpp, dest_block.size, *y);
          if (ret == 0) {
            ret = cache_driver->put(
                dpp, dest_oid, bl_rem, bl_rem.length(), attrs, *y);
            if (ret == 0) {
              policy->update(
                  dpp, dest_oid, adjusted_start_ofs, bl_rem.length(),
                  dest_version, dirty, rgw::d4n::RefCount::NOOP, *y);
              dest_blocks.emplace_back(dest_block);
            }
          }
        }
    ...
    if (last_part) {
      ...
      if (source->dest_object && source->dest_bucket) {
        if ((ret = blockDir->set(dpp, dest_blocks, *y)) < 0) {
          ldpp_dout(dpp, 10) << "D4NFilterWriter::" << __func__
                             << "(): BlockDirectory pipelined set() method for "
                                "dest blocks failed, ret="
                             << ret << dendl;
        }
      }
    } // if last_part
...
int
D4NFilterObject::D4NFilterDeleteOp::delete_obj(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    uint32_t flags)
{
  ...
  if (!source->check_head_exists_in_cache_get_oid(
          dpp, head_oid_in_cache, attrs, block, y) &&
      !block.deleteMarker) {
    ...
    ldpp_dout(dpp, 10) << "D4NFilterObject::" << __func__
                       << "(): head object not found; calling next->delete_obj"
                       << dendl;
    next->params = params;
    ...
    if (objDirty &&
        !cache_request) { // head object dirty flag represents object dirty flag
      ...
      if (!source->get_bucket()->versioned() ||
        ...
        ldpp_dout(dpp, 10) << "D4NFilterObject::" << __func__
                           << "(): calling invalidate_dirty_object for: "
                           << head_oid_in_cache << dendl;
        if (!source->driver->get_policy_driver()
                 ->get_cache_policy()
                 ->invalidate_dirty_object(dpp, head_oid_in_cache)) {
          objDirty = false;
        }
      }
    }
      ...
      if (!objDirty) {
        if (source->have_instance()) {
          if ((ret = blockDir->del(dpp, &block, y)) < 0) {
            ldpp_dout(dpp, 0)
                << "Failed to delete head object in block directory for: "
                << block.cacheObj.objName << ", ret=" << ret << dendl;
            return ret;
          }
          if (cache_request) {
            if ((ret = source->delete_cache_entry(
                     dpp, get_cache_block_prefix(source, version), y)) < 0) {
              return ret;
            }
          }
        }
        ...
        if (!source->get_bucket()->versioning_enabled()) {
          ...
          if (cache_request) {
            if ((ret = source->delete_cache_entry(dpp, head_oid_in_cache, y)) <
                0) {
              return ret;
            }
          }
          ...
          objDirty) { //2. dirty objects - 1. add delete marker for simple request 2. delete version if given and correctly promote latest version if needed
        ...
        while (retry) {
          ...
          if (ret < 0) {
            ldpp_dout(dpp, 0)
                << "D4NFilterObject::" << __func__
                << "(): Failed to get latest entry in block directory for: "
                << latest_block.cacheObj.objName << ", ret=" << ret << dendl;
            return ret;
          }
          ...
          if (block.cacheObj.objName == objName) {
            ...
            if (!latest_block.deleteMarker) {
              ...
              if (ret < 0) {
                ldpp_dout(dpp, 0)
                    << "D4NFilterObject::" << __func__
                    << "(): Failed to create a delete marker for: "
                    << block.cacheObj.objName << ", ret=" << ret << dendl;
            ...
            if (ret == 0) {
              ...
              if (latest_block.version == block.version) {
                ...
                ret = objDir->zrevrange(
                    dpp, &dir_obj, std::to_string(0), std::to_string(1),
                    members, y);
                if (ret < 0) {
                  ldpp_dout(dpp, 0)
                      << "D4NFilterObject::" << __func__
                      << "(): Failed to get the second latest version for: "
                      << dir_obj.objName << ", ret=" << ret << dendl;
                  return ret;
                }
                //if there is a second latest version
                if (members.size() == 2) {
                  rgw::d4n::CacheBlock version_block = latest_block;
                  version_block.cacheObj.objName = "_:" + members[1] + "_" +
                                                   source->get_name();
                  //get versioned entry
                  ...
                  if (ret < 0) {
                    ldpp_dout(dpp, 0)
                        << "D4NFilterObject::" << __func__
                        << "(): Failed to get the versioned entry for: "
                        << version_block.cacheObj.objName << ", ret=" << ret
                        << dendl;
                    return 0;
                  }
                  ...
                  ldpp_dout(dpp, 20)
                      << "D4NFilterObject::" << __func__
                      << "(): INFO: promoting latest version entry to version: "
                      << version_block.version << ", ret=" << ret << dendl;
                  ret = blockDir->set(dpp, &version_block, y);
                  if (ret < 0) {
                    ldpp_dout(dpp, 0)
                        << "D4NFilterObject::" << __func__
                        << "(): Failed to set new latest entry for: "
                        << version_block.cacheObj.objName << ", ret=" << ret
                        << dendl;
                    return 0;
                  }
                  ...
                  if (ret < 0) {
                    ldpp_dout(dpp, 0)
                        << "D4NFilterObject::" << __func__
                        << "(): Failed to delete latest entry in block "
                           "directory, when it is the same as version "
                           "requested, for: "
                        << block.cacheObj.objName << ", ret=" << ret << dendl;
                    return ret;
                  }
                  ...
                  ret = bucketDir->zrem(
                      dpp, source->get_bucket()->get_bucket_id(),
                      source->get_name(), y);
                  if (ret < 0) {
                    ldpp_dout(dpp, 0)
                        << "D4NFilterObject::"
                        << __func__ << "(): Failed to Queue zrem request in bucket directory for: "
                        << source->get_name() << ", ret=" << ret << dendl;
                    return ret;
                  }
                  if (cache_request) {
                    std::string req_oid_in_cache = get_key_in_cache(
                        head_oid_in_cache + "#0#0", std::to_string(0),
                        std::to_string(0));
                    if ((ret = source->delete_cache_entry(
                             dpp, req_oid_in_cache, y)) < 0) {
                      return ret;
                    }
                  }
                }
              ...
              if ((ret = blockDir->del(dpp, &block, y)) < 0 && ret != -ENOENT) {
                ldpp_dout(dpp, 0)
                    << "D4NFilterObject::"
                    << __func__ << "(): Failed to delete head object in block directory for: "
                    << block.cacheObj.objName << ", ret=" << ret << dendl;
                return ret;
              }
              ...
              ldpp_dout(dpp, 20)
                  << "D4NFilterObject::" << __func__
                  << "(): Version to be deleted is: " << version << dendl;
              ret = objDir->zrem(dpp, &dir_obj, version, y);
              if (ret < 0) {
                ldpp_dout(dpp, 0)
                    << "D4NFilterObject::"
                    << __func__ << "(): Failed to Queue zrem request in object directory for: "
                    << source->get_name() << ", ret=" << ret << dendl;
                return ret;
              }
              if (ret < 0) {
                ldpp_dout(dpp, 0)
                    << "D4NFilterObject::" << __func__
                    << "(): Failed to execute exec in block directory: "
                    << "ret= " << ret << dendl;
                return ret;
              }
              if (cache_request) {
                std::string req_oid_in_cache = get_key_in_cache(
                    get_cache_block_prefix(source, version), std::to_string(0),
                    std::to_string(0));
                if ((ret = source->delete_cache_entry(
                         dpp, req_oid_in_cache, y)) < 0) {
                  return ret;
        ...
        if (!transaction_success) {
          ldpp_dout(dpp, 0)
              << "D4NFilterObject::" << __func__
              << "(): Redis transaction failed after retrying! " << dendl;
    ...
    if (!source->get_bucket()->versioned()) {
      ...
      if (ret < 0) {
        ldpp_dout(dpp, 0)
            << "D4NFilterObject::"
            << __func__ << "(): Failed to Queue delete head object op in block directory for: "
            << block.cacheObj.objName << ", ret=" << ret << dendl;
        return ret;
      }
      if (cache_request) {
      ...
      if (ret < 0) {
        ldpp_dout(dpp, 0)
            << "D4NFilterObject::" << __func__
            << "(): Failed to Queue delete head object in block directory for: "
            << block.cacheObj.objName << ", ret=" << ret << dendl;
        return ret;
      }
      ...
      if (objDirty) {
        ...
        if (ret < 0) {
          ldpp_dout(dpp, 0)
              << "D4NFilterObject::" << __func__
              << "(): Failed to Queue zrem request in object directory for: "
              << source->get_name() << ", ret=" << ret << dendl;
          return ret;
        }
        //delete entry from ordered set of objects
        ret = bucketDir->zrem(
            dpp, source->get_bucket()->get_bucket_id(), source->get_name(), y);
        if (ret < 0) {
          ldpp_dout(dpp, 0)
              << "D4NFilterObject::" << __func__
              << "(): Failed to Queue zrem request in bucket directory for: "
              << source->get_name() << ", ret=" << ret << dendl;
          return ret;
        }
      }
    ...
    if (objDirty) {
      ...
      if (attrs.find(RGW_CACHE_ATTR_OBJECT_SIZE) != attrs.end()) {
        ...
        ldpp_dout(dpp, 0) << "D4NFilterObject::" << __func__
                          << "(): Failed to retrieve size for for: "
                          << block.cacheObj.objName << ", ret=" << ret << dendl;
        return -EINVAL;
      }
      size = stoi(size_str);
    ...
    if ((!objDirty && source->get_bucket()->versioned() &&
      ...
      do { // loop through the data blocks
        ...
        if ((ret = blockDir->get(dpp, &block, y)) < 0) {
          if (ret == -ENOENT) {
            ldpp_dout(dpp, 10)
                << "D4NFilterObject::" << __func__
                << "(): Directory entry for: " << source->get_oid()
                << " blockid: " << fst << " block size: " << cur_len
                << " does not exist; continuing" << dendl;
            fst += cur_len;
            if (fst >= lst) {
              break;
            }
            ...
            ldpp_dout(dpp, 10)
                ...
                << source->get_oid() << " blockid: " << fst
                << " block size: " << cur_len << ", ret=" << ret << dendl;
            return ret;
          }
        }
        ...
        if ((ret = blockDir->del(dpp, &block, y)) == -ENOENT) {
          ...
          ldpp_dout(dpp, 0)
              << "D4NFilterObject::" << __func__
              << "(): Failed to delete directory entry for: "
              << source->get_name() << " blockid: " << fst
              << " block size: " << cur_len << ", ret=" << ret << dendl;
          return ret;
        }

        std::string req_oid_in_cache = get_key_in_cache(
            get_cache_block_prefix(source, version),
            std::to_string(block.blockID), std::to_string(block.size));
        if (cache_request) {
    ...
    if (!objDirty) {
      ...
      next->params = params;
      ldpp_dout(dpp, 10) << "D4NFilterObject::" << __func__
                         << "(): object is not dirty; calling next->delete_obj"
                         << dendl;
      ret = next->delete_obj(dpp, y, flags);
      result = next->result;
      return ret;
...
int
D4NFilterWriter::prepare(optional_yield y)
{
  ...
  if (!d4n_writecache) {
    ...
    ldpp_dout(dpp, 0) << "D4NFilterWriter::" << __func__
                      << "(): calling next->prepare" << dendl;
    ...
    if (!object->get_bucket()->versioned() ||
        (object->get_bucket()->versioned() &&
         !object->get_bucket()->versioning_enabled())) {
      if (object->get_bucket()->versioned() &&
          !object->get_bucket()->versioning_enabled()) {
      ...
      rgw::d4n::CacheBlock block;
      rgw::sal::Attrs attrs;
      if (object->check_head_exists_in_cache_get_oid(
              dpp, prev_oid_in_cache, attrs, block, y)) {
        ldpp_dout(dpp, 20) << "D4NFilterWriter::" << __func__
                           << "(): found in cache, prev_oid_in_cache="
                           << prev_oid_in_cache << dendl;
      }
  ...
  if (!object->have_instance()) {
    if (object->get_bucket()->versioned() &&
        !object->get_bucket()->versioning_enabled()) { //if versioning is suspended
      object->set_instance("null");
    }
    char buf[OBJ_INSTANCE_LEN + 1];
    gen_rand_alphanumeric_no_underscore(dpp->get_cct(), buf, OBJ_INSTANCE_LEN);
    version =
        buf; // using gen_rand_alphanumeric_no_underscore for the time being
    ldpp_dout(dpp, 20) << "D4NFilterWriter::" << __func__
                       << "(): generating version: " << version << dendl;
  } else {
    ldpp_dout(dpp, 20) << "D4NFilterWriter::" << __func__
                       << "(): version is: " << object->get_instance() << dendl;
...
int
D4NFilterWriter::process(bufferlist&& data, uint64_t offset)
{
  bufferlist bl = data;
  ...
  if (!d4n_writecache) {
    ...
    ret = driver->get_policy_driver()->get_cache_policy()->eviction(
        dpp, bl.length(), y);
    if (ret == 0) {
      if (bl.length() > 0) {
        ldpp_dout(dpp, 10) << "D4NFilterWriter::" << __func__
                           << "(): oid_in_cache is: " << oid_in_cache << dendl;
        ret = driver->get_cache_driver()->put(
            dpp, oid_in_cache, bl, bl.length(), attrs, y);
        if (ret == 0) {
          if (!object->is_cache_request()) {
            dirty = true;
          }
          if (!object->is_cache_request()) {
            ret = driver->get_cache_driver()->set_attr(
                dpp, oid_in_cache, RGW_CACHE_ATTR_DIRTY, "1", y);
            if (ret == 0) {
              driver->get_policy_driver()->get_cache_policy()->update(
                  dpp, oid_in_cache, ofs, bl.length(), version, dirty,
                  rgw::d4n::RefCount::NOOP, y);
            }
          } else {
            driver->get_policy_driver()->get_cache_policy()->update(
          ...
          ldpp_dout(dpp, 0)
              << "D4NFilterWriter::" << __func__
              << "(): ERROR: writing data to the cache failed, ret=" << ret
              << dendl;
          return ret;
        }
      }
    ...
    uint32_t flags)
{
  ...
  if (d4n_writecache) {
    ...
    ret = next->complete(
        accounted_size, etag, mtime, set_mtime, attrs, cksum, delete_at,
        if_match, if_nomatch, user_data, zones_trace, canceled, rctx, flags);
    if (ret < 0) {
      ldpp_dout(dpp, 0) << "D4NFilterWriter::" << __func__
                        << "(): writing to backend store failed, ret=" << ret
                        << dendl;
      return ret;
    }
  ...
  bufferlist bl;
  //same as key, as there is no len or offset attached to head oid in cache
  ldpp_dout(dpp, 20) << "D4NFilterWriter::" << __func__ << "(): key is: " << key
                     << dendl;
  ret = driver->get_policy_driver()->get_cache_policy()->eviction(
      dpp, attrs.size(), y);
  if (ret == 0) {
    ...
    if (ret == 0) {
      ldpp_dout(dpp, 20) << "D4NFilterWriter::" << __func__
                         ...
                         << version << dendl;
      driver->get_policy_driver()->get_cache_policy()->update(
          dpp, key, 0, bl.length(), version, dirty, rgw::d4n::RefCount::NOOP, y);
      ...
      if (dirty) {
        ...
        ldpp_dout(dpp, 20) << "D4NFilterWriter::" << __func__
                           << "(): key=" << key << dendl;
        ldpp_dout(dpp, 20) << "D4NFilterWriter::" << __func__
                           << "(): obj->get_key()=" << obj->get_key() << dendl;
        driver->get_policy_driver()->get_cache_policy()->update_dirty_object(
            dpp, key, version, false, accounted_size, creationTime,
            std::get<rgw_user>(obj->get_bucket()->get_owner()), objEtag,
            obj->get_bucket()->get_name(), obj->get_bucket()->get_bucket_id(),
            obj->get_key(), rgw::d4n::RefCount::NOOP, y);
        if (!prev_oid_in_cache.empty()) {
          driver->get_policy_driver()
              ->get_cache_policy()
              ->invalidate_dirty_object(dpp, prev_oid_in_cache);
        }
      }
    } else { //if get_cache_driver()->put()
      ldpp_dout(dpp, 0) << "D4NFilterWriter::" << __func__
                        << "(): put failed for head_oid_in_cache, ret=" << ret
                        << dendl;
      return ret;
    }
  } else {
    ldpp_dout(dpp, 0) << "D4NFilterWriter::" << __func__
                      << "(): eviction failed for head_oid_in_cache, ret="
                      << ret << dendl;
    return ret;
  }
  return 0;
...
int
D4NFilterMultipartUpload::complete(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    CephContext* cct,
    ...
    const char* if_nomatch)
{
  ...
  auto ret = next->complete(
      dpp, y, cct, part_etags, remove_objs, accounted_size, compressed, cs_info,
      ofs, tag, owner, olh_epoch, nextObject(target_obj), processed_prefixes);
  if (ret < 0) {
    return ret;
  }
  ...
  bufferlist bl;
  std::string head_oid_in_cache =
      get_cache_block_prefix(d4n_target_obj, version);
  ...
  if (ret == 0) {
    ret = driver->get_cache_driver()->put(
        dpp, head_oid_in_cache, bl, 0, attrs, y);
    if (ret == 0) {
      ldpp_dout(dpp, 20) << "D4NFilterMultipartUpload::" << __func__
                         << " version stored in update method is: "
                         << d4n_target_obj->get_object_version() << dendl;
      driver->get_policy_driver()->get_cache_policy()->update(
          dpp, head_oid_in_cache, 0, bl.length(), version, false,
          rgw::d4n::RefCount::NOOP, y);
      ...
      if (ret < 0) {
        ldpp_dout(dpp, 0)
            << "D4NFilterMultipartUpload::" << __func__
            << "(): BlockDirectory set method failed for head object, ret="
            << ret << dendl;
      }
    } else {
      ldpp_dout(dpp, 0) << "D4NFilterMultipartUpload::" << __func__
                        << "(): put for head object failed, ret=" << ret
                        << dendl;
      return ret;
    }
    ...
    ldpp_dout(dpp, 0) << "D4NFilterMultipartUpload::" << __func__
                      << "(): failed to cache head object during eviction, ret="
                      << ret << dendl;
    return ret;
  }
...
rgw::sal::Driver*
newD4NFilter(rgw::sal::Driver* next, void* io_context, bool admin)
{
  rgw::sal::D4NFilterDriver* driver = new rgw::sal::D4NFilterDriver(
      next, *static_cast<boost::asio::io_context*>(io_context), admin);
