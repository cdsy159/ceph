...
inline std::string
get_cache_block_prefix(rgw::sal::Object* object, const std::string& version)
{
  return fmt::format(
      "{}{}{}{}{}", url_encode(object->get_bucket()->get_bucket_id(), true),
      CACHE_DELIM, url_encode(version, true), CACHE_DELIM,
      url_encode(object->get_name(), true));
}
...
{
  return fmt::format(
      "{}{}{}{}{}", prefix, CACHE_DELIM, offset, CACHE_DELIM, len);
}
...
public:
  ...
  rgw::cache::CacheDriver*
  get_cache_driver()
  {
    return cacheDriver.get();
  }

  rgw::d4n::ObjectDirectory*
  get_obj_dir()
  {
    return objDir.get();
  }

  rgw::d4n::BlockDirectory*
  get_block_dir()
  {
    return blockDir.get();
  }

  rgw::d4n::BucketDirectory*
  get_bucket_dir()
  {
    return bucketDir.get();
  }

  rgw::d4n::PolicyDriver*
  get_policy_driver()
  {
    return policyDriver.get();
  }

  void
  save_y(optional_yield y)
  {
    this->y = y;
  }
  ...
  std::shared_ptr<rgw::d4n::RedisPool>
  get_redis_pool()
  {
    return redis_pool;
  }
...
public:
  D4NFilterUser(std::unique_ptr<User> _next, D4NFilterDriver* _driver) :
    FilterUser(std::move(_next)), driver(_driver)
  {}
...
private:
  ...
  std::unordered_map<std::string, rgw::d4n::CacheBlock>
      dir_blocks; // for use in bucket removal

public:
  D4NFilterBucket(std::unique_ptr<Bucket> _next, D4NFilterDriver* _filter) :
    FilterBucket(std::move(_next)), filter(_filter)
  {}
...
public:
  ...
  public:
    ...
    public:
      D4NFilterGetCB(D4NFilterDriver* _filter, D4NFilterObject* _source) :
        filter(_filter), source(_source)
      {}
      ...
      void
      set_start_ofs(uint64_t ofs)
      {
        this->start_ofs = ofs;
      }

      void
      set_len(uint64_t len)
      {
        this->len = len;
      }

      void
      set_adjusted_start_ofs(uint64_t adjusted_start_ofs)
      {
        this->adjusted_start_ofs = adjusted_start_ofs;
      }

      void
      set_part_num(uint64_t part_num)
      {
        this->part_num = part_num;
      }

      void
      set_num_parts(uint64_t num_parts)
      {
      ...
      bypass_cache_write()
      {
        this->write_to_cache = false;
      }
    };

    D4NFilterObject* source;

    D4NFilterReadOp(std::unique_ptr<ReadOp> _next, D4NFilterObject* _source) :
      FilterReadOp(std::move(_next)), source(_source)
    {
  ...
  struct D4NFilterDeleteOp : FilterDeleteOp {
    ...
    D4NFilterDeleteOp(std::unique_ptr<DeleteOp> _next, D4NFilterObject* _source) :
      FilterDeleteOp(std::move(_next)), source(_source)
    {}
  ...
  D4NFilterObject(std::unique_ptr<Object> _next, D4NFilterDriver* _driver) :
    FilterObject(std::move(_next)), driver(_driver)
  {}

  D4NFilterObject(
      std::unique_ptr<Object> _next,
      Bucket* _bucket,
      D4NFilterDriver* _driver) :
    FilterObject(std::move(_next), _bucket), driver(_driver)
  {}

  D4NFilterObject(D4NFilterObject& _o, D4NFilterDriver* _driver) :
    FilterObject(_o), driver(_driver)
  {}
  ...
  void
  set_object_version(const std::string& version)
  {
  ...
  int calculate_version(
      ...
      std::string& version,
      rgw::sal::Attrs& attrs);
  int set_head_obj_dir_entry(
      const DoutPrefixProvider* dpp,
      std::vector<std::string>* exec_responses,
      optional_yield y,
      bool is_latest_version = true,
      bool dirty = false);
  int set_data_block_dir_entries(
      const DoutPrefixProvider* dpp,
      optional_yield y,
      std::string& version,
      bool dirty = false);
  int delete_data_block_cache_entries(
      const DoutPrefixProvider* dpp,
      optional_yield y,
      std::string& version,
      bool dirty = false);
  bool check_head_exists_in_cache_get_oid(
      const DoutPrefixProvider* dpp,
  ...
  rgw::sal::Bucket*
  get_destination_bucket(const DoutPrefixProvider* dpp)
  {
    return dest_bucket;
  }

  rgw::sal::Object*
  get_destination_object(const DoutPrefixProvider* dpp)
  {
    return dest_object;
  }
  ...
  void
  set_load_obj_from_store(bool load_from_store)
  {
    this->load_from_store = load_from_store;
  }

  int delete_cache_entry(
      const DoutPrefixProvider* dpp,
...
public:
  D4NFilterWriter(
      std::unique_ptr<Writer> _next,
      D4NFilterDriver* _driver,
      Object* _obj,
      const DoutPrefixProvider* _dpp,
      optional_yield _y) :
    FilterWriter(std::move(_next), _obj),
    driver(_driver),
    dpp(_dpp),
    atomic(false),
    y(_y)
  {
    object = static_cast<D4NFilterObject*>(obj);
  }
...
public:
  D4NFilterMultipartUpload(
      std::unique_ptr<MultipartUpload> _next,
      Bucket* _b,
      D4NFilterDriver* driver) :
    FilterMultipartUpload(std::move(_next), _b), driver(driver)
  {}
