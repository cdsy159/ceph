...
int
DB::Initialize(string logfile, int loglevel)
{
  int ret = -1;
  ...
  ldpp_dout(dpp, 0) << "DB successfully initialized - name:" << db_name << ""
                    << dendl;
...
int
DB::createGC(const DoutPrefixProvider* dpp)
{
  int ret = 0;
  /* create gc thread */
...
int
DB::Destroy(const DoutPrefixProvider* dpp)
{
  if (db) {
    stopGC();
    closeDB(dpp);
    ...
    class ObjectOp* ptr)
{
  ...
  if (iter != DB::objectmap.end()) {
    ...
    ldpp_dout(dpp, 30) << "Objectmap entry already exists for bucket(" << bucket
                       << "). Not inserted " << dendl;
    delete ptr;
    return 0;
  }
...
int
DB::objectmapDelete(const DoutPrefixProvider* dpp, string bucket)
{
  map<string, class ObjectOp*>::iterator iter;
...
int
DB::InitializeParams(const DoutPrefixProvider* dpp, DBOpParams* params)
{
  int ret = -1;
    ...
    RGWObjVersionTracker* pobjv_tracker)
{
  ...
  if (query_str.empty() || query_str_val.empty()) {
    ldpp_dout(dpp, 0) << "In GetUser - Invalid query(" << query_str
                      << "), query_str_val(" << query_str_val << ")" << dendl;
    return -1;
  }
  ...
  if (query_str == "username") {
    ...
    ldpp_dout(dpp, 0) << "In GetUser Invalid query string :"
                      << query_str.c_str() << ") " << dendl;
    return -1;
  }
  ...
  if (params.op.user.uinfo.access_keys.empty() ||
    ...
    ldpp_dout(dpp, 0) << "In GetUser - No user with query(" << query_str.c_str()
                      << "), user_id(" << uinfo.user_id << ") found" << dendl;
    return -ENOENT;
  }
    ...
    RGWUserInfo* pold_info)
{
  ...
  ret = get_user(
      dpp, string("user_id"), uinfo.user_id.id, orig_info, nullptr,
      &objv_tracker);
  ...
  if (!ret && obj_ver.ver) {
    ...
    if (pobjv && (pobjv->read_version.ver != obj_ver.ver)) {
      ...
      ret = -ECANCELED;
      ldpp_dout(dpp, 0) << "User Read version mismatch err:(" << ret << ") "
                        << dendl;
      return ret;
    }
  ...
  ldpp_dout(dpp, 20) << "User creation successful - userid:(" << uinfo.user_id
                     << ") " << dendl;
    ...
    RGWObjVersionTracker* pobjv)
{
  ...
  if (ret) {
    ldpp_dout(dpp, 0) << "remove_user failed with err:(" << ret << ") "
                      << dendl;
    goto out;
  }
    ...
    obj_version* pbucket_version)
{
  ...
  if (query_str == "name") {
    ...
    ldpp_dout(dpp, 0) << "In GetBucket Invalid query string :"
                      << query_str.c_str() << ") " << dendl;
    return -1;
  }
    ...
    optional_yield y)
{
  ...
  ret = get_bucket_info(
      dpp, string("name"), "", orig_info, nullptr, nullptr, nullptr);
  ...
  if (ret) {
    ldpp_dout(dpp, 0) << "create_bucket failed with err:(" << ret << ") "
                      << dendl;
    goto out;
  }
...
int
DB::remove_bucket(const DoutPrefixProvider* dpp, const RGWBucketInfo info)
{
  int ret = 0;
    ...
    bool* is_truncated)
{
  ...
  if (ret) {
    ldpp_dout(dpp, 0) << "In ListUserBuckets failed err:(" << ret << ") "
                      << dendl;
    goto out;
  }
    ...
    RGWObjVersionTracker* pobjv)
{
  ...
  params.op.bucket.info.bucket.name = info.bucket.name;
  ret = get_bucket_info(
      dpp, string("name"), "", orig_info, &attrs, nullptr, &bucket_version);

  if (ret) {
    ldpp_dout(dpp, 0) << "Failed to read bucket info err:(" << ret << ") "
                      << dendl;
    goto out;
  }
  ...
  if (pobjv) {
    if (pobjv->read_version.ver != bucket_version.ver) {
      ldpp_dout(dpp, 0) << "Read version mismatch err:(" << ret << ") "
                        << dendl;
      ret = -ECANCELED;
      goto out;
    }
  ...
  if (query_str == "attrs") {
    ...
    ret = -1;
    ldpp_dout(dpp, 0) << "In UpdateBucket Invalid query_str : " << query_str
                      << dendl;
    goto out;
  }
    ...
    bool* is_truncated)
{
  ...
  if (ret) {
    ldpp_dout(dpp, 0) << "In ListBucketObjects failed err:(" << ret << ") "
                      << dendl;
    goto out;
  }
...
int
DB::raw_obj::InitializeParamsfromRawObj(
    const DoutPrefixProvider* dpp,
    DBOpParams* params)
{
  int ret = 0;
...
int
DB::Object::InitializeParamsfromObject(
    const DoutPrefixProvider* dpp,
    DBOpParams* params)
{
  int ret = 0;
...
int
DB::Object::get_object_impl(const DoutPrefixProvider* dpp, DBOpParams& params)
{
  int ret = 0;
  ...
  if (!ret && !params.op.obj.state.exists) {
    ldpp_dout(dpp, 0) << "Object(bucket:" << bucket_info.bucket.name
                      << ", Object:" << obj.key.name << ") doesn't exist"
                      << dendl;
    ret = -ENOENT;
  }
...
int
DB::Object::obj_omap_set_val_by_key(
    const DoutPrefixProvider* dpp,
...
int
DB::Object::obj_omap_get_vals_by_keys(
    const DoutPrefixProvider* dpp,
...
int
DB::Object::add_mp_part(const DoutPrefixProvider* dpp, RGWUploadPartInfo info)
{
  int ret = 0;
...
void
DB::gen_rand_obj_instance_name(rgw_obj_key* target_key)
{
...
int
DB::Object::list_versioned_objects(
    const DoutPrefixProvider* dpp,
    std::list<rgw_bucket_dir_entry>& list_entries)
{
  ...
  if (ret) {
    ldpp_dout(dpp, 0) << "In ListVersionedObjects failed err:(" << ret << ") "
                      << dendl;
  } else {
    ...
    RGWObjState** state)
{
  ...
  if (!obj.key.instance.empty()) {
    ...
    if (ret && ret != -ENOENT) {
      ldpp_dout(dpp, 0) << "get_object_impl failed err:(" << ret << ")"
                        << dendl;
      goto out;
    }
  } else {
    ...
    if (params.op.obj.list_entries.size() != 0) {
      ...
      if (ret) {
        ldpp_dout(dpp, 0) << "get_object_impl of versioned object failed err:("
                          << ret << ")" << dendl;
        goto out;
      }
    } else {
...
int
DB::Object::Read::prepare(const DoutPrefixProvider* dpp)
{
...
int
DB::Object::Read::range_to_ofs(uint64_t obj_size, int64_t& ofs, int64_t& end)
{
  if (ofs < 0) {
    ofs += obj_size;
    if (ofs < 0)
    ...
    const DoutPrefixProvider* dpp)
{
  ...
  raw_obj read_obj(
      store, source->get_bucket_info().bucket.name, astate->obj.key.name,
      astate->obj.key.instance, astate->obj.key.ns, source->obj_id, "0.0",
      part_num);

  read_len = len;

  ldpp_dout(dpp, 20) << "dbstore->read obj-ofs=" << ofs
    ...
    void* arg)
{
  ...
  return d->store->get_obj_iterate_cb(
      dpp, read_obj, obj_ofs, len, is_head_obj, astate, arg);
}

int
DB::get_obj_iterate_cb(
    const DoutPrefixProvider* dpp,
    ...
    void* arg)
{
  ...
  while (read_ofs < bl.length()) {
    unsigned chunk_len =
        std::min((uint64_t)bl.length() - read_ofs, (uint64_t)len);
    r = d->client_cb->handle_data(bl, read_ofs, chunk_len);
    if (r < 0)
      return r;
    read_ofs += chunk_len;
    read_len += chunk_len;
    ldpp_dout(dpp, 20) << "dbstore->get_obj_iterate_cb  obj-ofs=" << obj_ofs
                       << " len=" << len << " chunk_len = " << chunk_len
    ...
    RGWGetDataCB* cb)
{
  ...
  int r = source->iterate_obj(
      dpp, source->get_bucket_info(), state.obj, ofs, end, chunk_size,
      _get_obj_iterate_cb, &data);
  if (r < 0) {
    ...
    void* arg)
{
  ...
  while (ofs <= end && (uint64_t)ofs < astate->size) {
    ...
    /* XXX: Handle multipart_str */
    raw_obj read_obj(
        store, get_bucket_info().bucket.name, astate->obj.key.name,
...
int
DB::Object::Write::prepare(const DoutPrefixProvider* dpp)
{
  DB* store = target->get_store();
    ...
    uint64_t ofs)
{
  ...
  while (write_ofs < end) {
    ...
    /* XXX: Handle multipart_str */
    raw_obj write_obj(
        store, target->get_bucket_info().bucket.name, obj_state.obj.key.name,
        obj_state.obj.key.instance, obj_state.obj.key.ns, target->obj_id,
        mp_part_str, part_num);


    ldpp_dout(dpp, 20) << "dbstore->write obj-ofs=" << ofs
                       << " write_len=" << len << dendl;
...
int
DB::Object::Write::_do_write_meta(
    const DoutPrefixProvider* dpp,
    uint64_t size,
    ...
    bool modify_tail)
{
  ...
  if (target->bucket_info.obj_lock_enabled() &&
      target->bucket_info.obj_lock.has_rule()) {
    // && meta.flags == PUT_OBJ_CREATE) {
    auto iter = attrs.find(RGW_ATTR_OBJECT_RETENTION);
    if (iter == attrs.end()) {
      real_time lock_until_date =
          target->bucket_info.obj_lock.get_lock_until_date(meta.set_mtime);
  ...
  /* Check if versioned */
  bool is_versioned = !target->obj.key.instance.empty() &&
                      (target->obj.key.instance != "null");
    ...
    map<string, bufferlist>& attrs)
{
  ...
  /* handle assume_noent */
  int r = _do_write_meta(
      dpp, size, accounted_size, attrs, assume_noent, meta.modify_tail);
  return r;
}

int
DB::Object::Delete::delete_obj(const DoutPrefixProvider* dpp)
{
  int ret = 0;
  DBOpParams del_params = {};
  bool versioning_enabled =
      ((params.versioning_status & BUCKET_VERSIONED) == BUCKET_VERSIONED);
  bool versioning_suspended =
      ((params.versioning_status & BUCKET_VERSIONS_SUSPENDED) ==
  ...
  if (ret < 0 && ret != -ENOENT) {
    ldpp_dout(dpp, 0) << "GetObject during delete failed err:(" << ret << ")"
                      << dendl;
    return ret;
  }
  ...
  if (!ret) {
    ...
    if (ret) {
      ldpp_dout(dpp, 0) << "ListVersionedObjects failed err:(" << ret << ")"
                        << dendl;
      return ret;
    }
...
int
DB::Object::Delete::delete_obj_impl(
    const DoutPrefixProvider* dpp,
    DBOpParams& del_params)
{
  int ret = 0;
  ...
  if (ret) {
    ldpp_dout(dpp, 0) << "Updating tail objects mtime failed err:(" << ret
                      << ")" << dendl;
  }
  return ret;
}
    ...
    DBOpParams& del_params)
{
  ...
  if (ret) {
    ldpp_dout(dpp, 0) << "delete_olh: failed to create delete marker - err:("
                      << ret << ")" << dendl;
    return ret;
  }
...
int
DB::set_entry(const std::string& oid, const rgw::sal::LCEntry& entry)
{
  int ret = 0;
  const DoutPrefixProvider* dpp = get_def_dpp();
  ...
  if (ret) {
    ldpp_dout(dpp, 0) << "In InsertLCEntry failed err:(" << ret << ") "
                      << dendl;
    goto out;
  }
    ...
    std::vector<rgw::sal::LCEntry>& entries)
{
  ...
  if (ret) {
    ldpp_dout(dpp, 0) << "In ListLCEntries failed err:(" << ret << ") "
                      << dendl;
    goto out;
  }
...
int
DB::rm_entry(const std::string& oid, const rgw::sal::LCEntry& entry)
{
  int ret = 0;
  const DoutPrefixProvider* dpp = get_def_dpp();
  ...
  if (ret) {
    ldpp_dout(dpp, 0) << "In RemoveLCEntry failed err:(" << ret << ") "
                      << dendl;
    goto out;
  }
...
int
DB::get_head(const std::string& oid, rgw::sal::LCHead& head)
{
  int ret = 0;
  const DoutPrefixProvider* dpp = get_def_dpp();
...
int
DB::put_head(const std::string& oid, const rgw::sal::LCHead& head)
{
  int ret = 0;
  const DoutPrefixProvider* dpp = get_def_dpp();
    ...
    uint32_t min_wait)
{
  ...
  ldpp_dout(dpp, 20) << " Deleting stale_objs of bucket( " << bucket << ")"
                     << dendl;
  ...
  if (ret) {
    ldpp_dout(dpp, 0) << "In DeleteStaleObjectData failed err:(" << ret << ")"
                      << dendl;
  }
...
DB::GC::entry()
{
  do {
    ...
    do {
      ...
      int r = db->list_buckets(
          dpp, "all", owner, marker, string(), max, false, &buckets,
          &is_truncated);
      ...
      for (const auto& ent : buckets.get_buckets()) {
        ...
        if (r < 0) { //do nothing? skip to next entry?
          ldpp_dout(dpp, 2) << " delete_stale_objs failed for bucket( " << bname
                            << ")" << dendl;
        }
        bucket_marker = bname;
        user_marker = owner;
