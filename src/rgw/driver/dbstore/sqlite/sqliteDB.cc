...
#define SQL_PREPARE(dpp, params, sdb, stmt, ret, Op)                         \
  do {                                                                       \
    ...
    if (!stmt) {                                                             \
      ldpp_dout(dpp, 0) << "failed to prepare statement " << "for Op(" << Op \
                        << "); Errmsg -" << sqlite3_errmsg(*sdb) << dendl;   \
      ret = -1;                                                              \
      goto out;                                                              \
    }                                                                        \
  ...
  do {                                                                        \
    ...
    if (index <= 0) {                                                         \
      ldpp_dout(dpp, 0) << "failed to fetch bind parameter"                   \
                           " index for str("                                  \
                        << str << ") in " << "stmt(" << stmt << "); Errmsg -" \
                        << sqlite3_errmsg(*sdb) << dendl;                     \
      rc = -1;                                                                \
      goto out;                                                               \
    }                                                                         \
    ldpp_dout(dpp, 20) << "Bind parameter index for str(" << str              \
                       << ") in stmt(" << stmt << ") is " << index << dendl;  \
  } while (0);
...
#define SQL_BIND_TEXT(dpp, stmt, index, str, sdb)                           \
  do {                                                                      \
    ...
    if (rc != SQLITE_OK) {                                                  \
      ldpp_dout(dpp, 0) << "sqlite bind text failed for index(" << index    \
                        << "), str(" << str << ") in stmt(" << stmt         \
                        << "); Errmsg - " << sqlite3_errmsg(*sdb) << dendl; \
      rc = -1;                                                              \
      goto out;                                                             \
    }                                                                       \
    ldpp_dout(dpp, 20) << "Bind parameter text for index(" << index         \
                       << ") in stmt(" << stmt << ") is " << str << dendl;  \
  } while (0);
...
#define SQL_BIND_INT(dpp, stmt, index, num, sdb)                            \
  do {                                                                      \
    rc = sqlite3_bind_int(stmt, index, num);                                \
                                                                            \
    if (rc != SQLITE_OK) {                                                  \
      ldpp_dout(dpp, 0) << "sqlite bind int failed for index(" << index     \
                        << "), num(" << num << ") in stmt(" << stmt         \
                        << "); Errmsg - " << sqlite3_errmsg(*sdb) << dendl; \
      rc = -1;                                                              \
      goto out;                                                             \
    }                                                                       \
    ldpp_dout(dpp, 20) << "Bind parameter int for index(" << index          \
                       << ") in stmt(" << stmt << ") is " << num << dendl;  \
  } while (0);
...
#define SQL_BIND_BLOB(dpp, stmt, index, blob, size, sdb)                    \
  do {                                                                      \
    rc = sqlite3_bind_blob(stmt, index, blob, size, SQLITE_TRANSIENT);      \
                                                                            \
    if (rc != SQLITE_OK) {                                                  \
      ldpp_dout(dpp, 0) << "sqlite bind blob failed for index(" << index    \
                        << "), blob(" << blob << ") in stmt(" << stmt       \
                        << "); Errmsg - " << sqlite3_errmsg(*sdb) << dendl; \
      rc = -1;                                                              \
      goto out;                                                             \
    }                                                                       \
...
#define SQL_READ_BLOB(dpp, stmt, index, void_ptr, len)             \
  do {                                                             \
    ...
    len = sqlite3_column_bytes(stmt, index);                       \
                                                                   \
    if (!void_ptr || len == 0) {                                   \
      ldpp_dout(dpp, 20) << "Null value for blob index(" << index  \
                         << ") in stmt(" << stmt << ") " << dendl; \
    }                                                              \
  } while (0);
...
#define SQL_EXECUTE(dpp, params, stmt, cbk, args...)                          \
  do {                                                                        \
    ...
    ret = Bind(dpp, params);                                                  \
    if (ret) {                                                                \
      ldpp_dout(dpp, 0) << "Bind parameters failed for stmt(" << stmt << ") " \
                        << dendl;                                             \
      goto out;                                                               \
    }                                                                         \
                                                                              \
    ...
    Reset(dpp, stmt);                                                         \
                                                                              \
    if (ret) {                                                                \
      ldpp_dout(dpp, 0) << "Execution failed for stmt(" << stmt << ")"        \
                        << dendl;                                             \
      goto out;                                                               \
    }                                                                         \
  } while (0);
...
static int
list_callback(void* None, int argc, char** argv, char** aname)
{
  int i;
  for (i = 0; i < argc; i++) {
...
static int
list_user(const DoutPrefixProvider* dpp, DBOpInfo& op, sqlite3_stmt* stmt)
{
  if (!stmt)
    return -1;
  ...
  op.user.uinfo.display_name =
      (const char*)sqlite3_column_text(stmt, DisplayName); // user_name
  ...
  op.user.uinfo.default_placement.name =
      (const char*)sqlite3_column_text(stmt, PlacementName);

  op.user.uinfo.default_placement.storage_class =
      (const char*)sqlite3_column_text(stmt, PlacementStorageClass);

  SQL_DECODE_BLOB_PARAM(
      dpp, stmt, PlacementTags, op.user.uinfo.placement_tags, sdb);

  SQL_DECODE_BLOB_PARAM(
      dpp, stmt, BucketQuota, op.user.uinfo.quota.bucket_quota, sdb);
  SQL_DECODE_BLOB_PARAM(
      dpp, stmt, TempURLKeys, op.user.uinfo.temp_url_keys, sdb);
  SQL_DECODE_BLOB_PARAM(
      dpp, stmt, UserQuota, op.user.uinfo.quota.user_quota, sdb);
  ...
  op.user.user_version.tag =
      (const char*)sqlite3_column_text(stmt, UserVersionTag);

  return 0;
}

static int
list_bucket(const DoutPrefixProvider* dpp, DBOpInfo& op, sqlite3_stmt* stmt)
{
  if (!stmt)
    return -1;

  op.bucket.ent.bucket.name = (const char*)sqlite3_column_text(stmt, BucketName);
  op.bucket.ent.bucket.tenant =
      (const char*)sqlite3_column_text(stmt, Bucket_Tenant);
  op.bucket.ent.bucket.marker = (const char*)sqlite3_column_text(stmt, Marker);
  op.bucket.ent.bucket.bucket_id =
      (const char*)sqlite3_column_text(stmt, BucketID);
  op.bucket.ent.size = sqlite3_column_int(stmt, Size);
  op.bucket.ent.size_rounded = sqlite3_column_int(stmt, SizeRounded);
  SQL_DECODE_BLOB_PARAM(
      dpp, stmt, CreationTime, op.bucket.ent.creation_time, sdb);
  op.bucket.ent.count = sqlite3_column_int(stmt, Count);
  op.bucket.ent.placement_rule.name =
      (const char*)sqlite3_column_text(stmt, Bucket_PlacementName);
  op.bucket.ent.placement_rule.storage_class =
      (const char*)sqlite3_column_text(stmt, Bucket_PlacementStorageClass);
  ...
  SQL_DECODE_BLOB_PARAM(
      dpp, stmt, WebsiteConf, op.bucket.info.website_conf, sdb);
  op.bucket.info.swift_versioning = sqlite3_column_int(stmt, SwiftVersioning);
  op.bucket.info.swift_ver_location =
      (const char*)sqlite3_column_text(stmt, SwiftVerLocation);
  SQL_DECODE_BLOB_PARAM(
      dpp, stmt, MdsearchConfig, op.bucket.info.mdsearch_config, sdb);
  op.bucket.info.new_bucket_instance_id =
      (const char*)sqlite3_column_text(stmt, NewBucketInstanceID);
  SQL_DECODE_BLOB_PARAM(dpp, stmt, ObjectLock, op.bucket.info.obj_lock, sdb);
  SQL_DECODE_BLOB_PARAM(
      dpp, stmt, SyncPolicyInfoGroups, op.bucket.info.sync_policy, sdb);
  ...
  op.bucket.bucket_version.tag =
      (const char*)sqlite3_column_text(stmt, BucketVersionTag);
...
static int
list_object(const DoutPrefixProvider* dpp, DBOpInfo& op, sqlite3_stmt* stmt)
{
  if (!stmt)
    return -1;
  ...
  op.bucket.info.bucket.name =
      (const char*)sqlite3_column_text(stmt, ObjBucketName);
  op.obj.state.obj.key.instance =
      (const char*)sqlite3_column_text(stmt, ObjInstance);
  ...
  op.obj.owner_display_name =
      (const char*)sqlite3_column_text(stmt, OwnerDisplayName);
  ...
  op.obj.state.obj.index_hash_source =
      (const char*)sqlite3_column_text(stmt, IndexHashSource);
  ...
  op.obj.state.objv_tracker.read_version.ver =
      sqlite3_column_int(stmt, ObjVersion);
  op.obj.state.objv_tracker.read_version.tag =
      (const char*)sqlite3_column_text(stmt, ObjVersionTag);
  ...
  op.obj.head_placement_rule.name =
      (const char*)sqlite3_column_text(stmt, HeadPlacementRuleName);
  op.obj.head_placement_rule.storage_class =
      (const char*)sqlite3_column_text(stmt, HeadPlacementRuleStorageClass);
  op.obj.tail_placement.placement_rule.name =
      (const char*)sqlite3_column_text(stmt, TailPlacementRuleName);
...
static int
get_objectdata(const DoutPrefixProvider* dpp, DBOpInfo& op, sqlite3_stmt* stmt)
{
  if (!stmt)
    return -1;
  ...
  op.obj_data.multipart_part_str =
      (const char*)sqlite3_column_text(stmt, MultipartPartStr);
...
static int
list_lc_entry(const DoutPrefixProvider* dpp, DBOpInfo& op, sqlite3_stmt* stmt)
{
  if (!stmt)
    return -1;

  op.lc_entry.index = (const char*)sqlite3_column_text(stmt, LCEntryIndex);
  op.lc_entry.entry.bucket =
      (const char*)sqlite3_column_text(stmt, LCEntryBucketName);
...
static int
list_lc_head(const DoutPrefixProvider* dpp, DBOpInfo& op, sqlite3_stmt* stmt)
{
  if (!stmt)
    return -1;
...
int
SQLiteDB::InitializeDBOps(const DoutPrefixProvider* dpp)
{
  (void)createTables(dpp);
  dbops.InsertUser =
      make_shared<SQLInsertUser>(&this->db, this->getDBname(), cct);
  dbops.RemoveUser =
      make_shared<SQLRemoveUser>(&this->db, this->getDBname(), cct);
  dbops.GetUser = make_shared<SQLGetUser>(&this->db, this->getDBname(), cct);
  dbops.InsertBucket =
      make_shared<SQLInsertBucket>(&this->db, this->getDBname(), cct);
  dbops.UpdateBucket =
      make_shared<SQLUpdateBucket>(&this->db, this->getDBname(), cct);
  dbops.RemoveBucket =
      make_shared<SQLRemoveBucket>(&this->db, this->getDBname(), cct);
  dbops.GetBucket = make_shared<SQLGetBucket>(&this->db, this->getDBname(), cct);
  dbops.ListUserBuckets =
      make_shared<SQLListUserBuckets>(&this->db, this->getDBname(), cct);
  dbops.InsertLCEntry =
      make_shared<SQLInsertLCEntry>(&this->db, this->getDBname(), cct);
  dbops.RemoveLCEntry =
      make_shared<SQLRemoveLCEntry>(&this->db, this->getDBname(), cct);
  dbops.GetLCEntry =
      make_shared<SQLGetLCEntry>(&this->db, this->getDBname(), cct);
  dbops.ListLCEntries =
      make_shared<SQLListLCEntries>(&this->db, this->getDBname(), cct);
  dbops.InsertLCHead =
      make_shared<SQLInsertLCHead>(&this->db, this->getDBname(), cct);
  dbops.RemoveLCHead =
      make_shared<SQLRemoveLCHead>(&this->db, this->getDBname(), cct);
...
void*
SQLiteDB::openDB(const DoutPrefixProvider* dpp)
{
  string dbname;
  int rc = 0;
  ...
  if (rc) {
    ...
    ldpp_dout(dpp, 0) << "Opened database(" << dbname << ") successfully"
                      << dendl;
  }
...
int
SQLiteDB::closeDB(const DoutPrefixProvider* dpp)
{
  if (db)
...
int
SQLiteDB::Reset(const DoutPrefixProvider* dpp, sqlite3_stmt* stmt)
{
  int ret = -1;
...
again:
  ...
  ldpp_dout(dpp, 20) << "sqlite step successfully executed for stmt(" << stmt
                     << ")  ret = " << ret << dendl;
...
int
SQLiteDB::createTables(const DoutPrefixProvider* dpp)
{
  int ret = -1;
  int cu = 0, cb = 0, cq = 0;
  DBOpParams params = {};
...
int
SQLiteDB::createUserTable(const DoutPrefixProvider* dpp, DBOpParams* params)
{
  int ret = -1;
  string schema;
...
int
SQLiteDB::createBucketTable(const DoutPrefixProvider* dpp, DBOpParams* params)
{
  int ret = -1;
  string schema;
...
int
SQLiteDB::createObjectTable(const DoutPrefixProvider* dpp, DBOpParams* params)
{
  int ret = -1;
  string schema;
...
int
SQLiteDB::createObjectTableTrigger(
    const DoutPrefixProvider* dpp,
    DBOpParams* params)
{
  int ret = -1;
...
int
SQLiteDB::createObjectView(const DoutPrefixProvider* dpp, DBOpParams* params)
{
  int ret = -1;
  string schema;
...
int
SQLiteDB::createQuotaTable(const DoutPrefixProvider* dpp, DBOpParams* params)
{
  int ret = -1;
  string schema;
...
int
SQLiteDB::createObjectDataTable(const DoutPrefixProvider* dpp, DBOpParams* params)
{
  int ret = -1;
  string schema;
...
int
SQLiteDB::createLCTables(const DoutPrefixProvider* dpp)
{
  int ret = -1;
  string schema;
  DBOpParams params = {};
...
int
SQLiteDB::DeleteUserTable(const DoutPrefixProvider* dpp, DBOpParams* params)
{
  int ret = -1;
  string schema;
...
int
SQLiteDB::DeleteBucketTable(const DoutPrefixProvider* dpp, DBOpParams* params)
{
  int ret = -1;
  string schema;
...
int
SQLiteDB::DeleteObjectTable(const DoutPrefixProvider* dpp, DBOpParams* params)
{
  int ret = -1;
  string schema;
...
int
SQLiteDB::DeleteObjectDataTable(const DoutPrefixProvider* dpp, DBOpParams* params)
{
  int ret = -1;
  string schema;
...
int
SQLiteDB::DeleteQuotaTable(const DoutPrefixProvider* dpp, DBOpParams* params)
{
  int ret = -1;
  string schema;
...
int
SQLiteDB::DeleteLCEntryTable(const DoutPrefixProvider* dpp, DBOpParams* params)
{
  int ret = -1;
  string schema;
...
int
SQLiteDB::DeleteLCHeadTable(const DoutPrefixProvider* dpp, DBOpParams* params)
{
  int ret = -1;
  string schema;
...
int
SQLiteDB::ListAllUsers(const DoutPrefixProvider* dpp, DBOpParams* params)
{
  int ret = -1;
  string schema;
...
int
SQLiteDB::ListAllBuckets(const DoutPrefixProvider* dpp, DBOpParams* params)
{
  int ret = -1;
  string schema;
...
int
SQLiteDB::ListAllObjects(const DoutPrefixProvider* dpp, DBOpParams* params)
{
  int ret = -1;
  string schema;
...
int
SQLObjectOp::InitializeObjectOps(string db_name, const DoutPrefixProvider* dpp)
{
  ...
  DeleteStaleObjectData =
      make_shared<SQLDeleteStaleObjectData>(sdb, db_name, cct);

  return 0;
}

int
SQLInsertUser::Prepare(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int ret = -1;
...
int
SQLInsertUser::Bind(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int index = -1;
  int rc = 0;
  struct DBOpPrepareParams p_params = PrepareParams;
  ...
  SQL_BIND_TEXT(
      dpp, stmt, index, params->op.user.uinfo.user_id.tenant.c_str(), sdb);
  ...
  SQL_BIND_TEXT(
      dpp, stmt, index, params->op.user.uinfo.display_name.c_str(), sdb);
  ...
  SQL_ENCODE_BLOB_PARAM(
      dpp, stmt, index, params->op.user.uinfo.access_keys, sdb);
  ...
  SQL_BIND_TEXT(
      dpp, stmt, index, params->op.user.uinfo.default_placement.name.c_str(),
      sdb);

  SQL_BIND_INDEX(
      dpp, stmt, index, p_params.op.user.placement_storage_class, sdb);
  SQL_BIND_TEXT(
      dpp, stmt, index,
      params->op.user.uinfo.default_placement.storage_class.c_str(), sdb);

  SQL_BIND_INDEX(dpp, stmt, index, p_params.op.user.placement_tags, sdb);
  SQL_ENCODE_BLOB_PARAM(
      dpp, stmt, index, params->op.user.uinfo.placement_tags, sdb);

  SQL_BIND_INDEX(dpp, stmt, index, p_params.op.user.bucket_quota, sdb);
  SQL_ENCODE_BLOB_PARAM(
      dpp, stmt, index, params->op.user.uinfo.quota.bucket_quota, sdb);

  SQL_BIND_INDEX(dpp, stmt, index, p_params.op.user.temp_url_keys, sdb);
  SQL_ENCODE_BLOB_PARAM(
      dpp, stmt, index, params->op.user.uinfo.temp_url_keys, sdb);

  SQL_BIND_INDEX(dpp, stmt, index, p_params.op.user.user_quota, sdb);
  SQL_ENCODE_BLOB_PARAM(
      dpp, stmt, index, params->op.user.uinfo.quota.user_quota, sdb);
...
int
SQLInsertUser::Execute(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int ret = -1;
...
int
SQLRemoveUser::Prepare(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int ret = -1;
  struct DBOpPrepareParams p_params = PrepareParams;
...
int
SQLRemoveUser::Bind(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int index = -1;
  int rc = 0;
  struct DBOpPrepareParams p_params = PrepareParams;
...
int
SQLRemoveUser::Execute(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int ret = -1;
...
int
SQLGetUser::Prepare(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int ret = -1;
  struct DBOpPrepareParams p_params = PrepareParams;
...
int
SQLGetUser::Bind(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int index = -1;
  int rc = 0;
  struct DBOpPrepareParams p_params = PrepareParams;
  ...
  if (params->op.query_str == "email") {
    ...
    SQL_BIND_TEXT(
        dpp, email_stmt, index, params->op.user.uinfo.user_email.c_str(), sdb);
  } else if (params->op.query_str == "access_key") {
    if (!params->op.user.uinfo.access_keys.empty()) {
      string access_key;
      map<string, RGWAccessKey>::const_iterator it =
    ...
    SQL_BIND_TEXT(
        dpp, userid_stmt, index, params->op.user.uinfo.user_id.id.c_str(), sdb);
  } else { // by default by userid
    SQL_BIND_INDEX(dpp, stmt, index, p_params.op.user.user_id, sdb);
    SQL_BIND_TEXT(
        dpp, stmt, index, params->op.user.uinfo.user_id.id.c_str(), sdb);
  }
...
int
SQLGetUser::Execute(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int ret = -1;
...
int
SQLInsertBucket::Prepare(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int ret = -1;
  struct DBOpPrepareParams p_params = PrepareParams;
...
int
SQLInsertBucket::Bind(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int index = -1;
  int rc = 0;
  struct DBOpPrepareParams p_params = PrepareParams;
  ...
  SQL_BIND_TEXT(
      dpp, stmt, index, params->op.bucket.info.bucket.name.c_str(), sdb);

  SQL_BIND_INDEX(dpp, stmt, index, p_params.op.bucket.tenant, sdb);
  SQL_BIND_TEXT(
      dpp, stmt, index, params->op.bucket.info.bucket.tenant.c_str(), sdb);

  SQL_BIND_INDEX(dpp, stmt, index, p_params.op.bucket.marker, sdb);
  SQL_BIND_TEXT(
      dpp, stmt, index, params->op.bucket.info.bucket.marker.c_str(), sdb);

  SQL_BIND_INDEX(dpp, stmt, index, p_params.op.bucket.bucket_id, sdb);
  SQL_BIND_TEXT(
      dpp, stmt, index, params->op.bucket.info.bucket.bucket_id.c_str(), sdb);
  ...
  SQL_ENCODE_BLOB_PARAM(
      dpp, stmt, index, params->op.bucket.info.creation_time, sdb);
  ...
  SQL_BIND_TEXT(
      dpp, stmt, index, params->op.bucket.info.placement_rule.name.c_str(), sdb);

  SQL_BIND_INDEX(
      dpp, stmt, index, p_params.op.bucket.placement_storage_class, sdb);
  SQL_BIND_TEXT(
      dpp, stmt, index,
      params->op.bucket.info.placement_rule.storage_class.c_str(), sdb);
  ...
  SQL_ENCODE_BLOB_PARAM(
      dpp, stmt, index, params->op.bucket.info.website_conf, sdb);
  ...
  SQL_BIND_TEXT(
      dpp, stmt, index, params->op.bucket.info.swift_ver_location.c_str(), sdb);

  SQL_BIND_INDEX(dpp, stmt, index, p_params.op.bucket.mdsearch_config, sdb);
  SQL_ENCODE_BLOB_PARAM(
      dpp, stmt, index, params->op.bucket.info.mdsearch_config, sdb);

  SQL_BIND_INDEX(
      dpp, stmt, index, p_params.op.bucket.new_bucket_instance_id, sdb);
  SQL_BIND_TEXT(
      dpp, stmt, index, params->op.bucket.info.new_bucket_instance_id.c_str(),
      sdb);
  ...
  SQL_BIND_INDEX(
      dpp, stmt, index, p_params.op.bucket.sync_policy_info_groups, sdb);
  SQL_ENCODE_BLOB_PARAM(
      dpp, stmt, index, params->op.bucket.info.sync_policy, sdb);
  ...
  SQL_BIND_TEXT(
      dpp, stmt, index, params->op.bucket.bucket_version.tag.c_str(), sdb);
...
int
SQLInsertBucket::Execute(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int ret = -1;
...
int
SQLUpdateBucket::Prepare(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int ret = -1;
  struct DBOpPrepareParams p_params = PrepareParams;
...
int
SQLUpdateBucket::Bind(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int index = -1;
  int rc = 0;
  struct DBOpPrepareParams p_params = PrepareParams;
  ...
  if (params->op.query_str == "attrs") {
    SQL_BIND_INDEX(dpp, *stmt, index, p_params.op.bucket.bucket_attrs, sdb);
    SQL_ENCODE_BLOB_PARAM(
        dpp, *stmt, index, params->op.bucket.bucket_attrs, sdb);
  } else if (params->op.query_str == "owner") {
    SQL_BIND_INDEX(dpp, *stmt, index, p_params.op.bucket.creation_time, sdb);
    SQL_ENCODE_BLOB_PARAM(
        dpp, *stmt, index, params->op.bucket.info.creation_time, sdb);
    ...
    SQL_BIND_INDEX(dpp, *stmt, index, p_params.op.bucket.placement_name, sdb);
    SQL_BIND_TEXT(
        dpp, *stmt, index, params->op.bucket.info.placement_rule.name.c_str(),
        sdb);
    ...
    SQL_BIND_INDEX(dpp, *stmt, index, p_params.op.bucket.zonegroup, sdb);
    SQL_BIND_TEXT(
        dpp, *stmt, index, params->op.bucket.info.zonegroup.c_str(), sdb);

    SQL_BIND_INDEX(dpp, *stmt, index, p_params.op.bucket.has_instance_obj, sdb);
    SQL_BIND_INT(
        dpp, *stmt, index, params->op.bucket.info.has_instance_obj, sdb);
    ...
    SQL_BIND_INDEX(dpp, *stmt, index, p_params.op.bucket.swift_versioning, sdb);
    SQL_BIND_INT(
        dpp, *stmt, index, params->op.bucket.info.swift_versioning, sdb);

    SQL_BIND_INDEX(
        dpp, *stmt, index, p_params.op.bucket.swift_ver_location, sdb);
    SQL_BIND_TEXT(
        dpp, *stmt, index, params->op.bucket.info.swift_ver_location.c_str(),
        sdb);
    ...
    SQL_BIND_TEXT(
        dpp, *stmt, index,
        params->op.bucket.info.new_bucket_instance_id.c_str(), sdb);

    SQL_BIND_INDEX(dpp, *stmt, index, p_params.op.bucket.obj_lock, sdb);
    SQL_ENCODE_BLOB_PARAM(
        dpp, *stmt, index, params->op.bucket.info.obj_lock, sdb);
...
int
SQLUpdateBucket::Execute(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int ret = -1;
  sqlite3_stmt** stmt = NULL; // Prepared statement
...
int
SQLRemoveBucket::Prepare(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int ret = -1;
  struct DBOpPrepareParams p_params = PrepareParams;
...
int
SQLRemoveBucket::Bind(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int index = -1;
  int rc = 0;
  struct DBOpPrepareParams p_params = PrepareParams;
...
int
SQLRemoveBucket::Execute(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int ret = -1;
...
int
SQLGetBucket::Prepare(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int ret = -1;
  struct DBOpPrepareParams p_params = PrepareParams;
...
int
SQLGetBucket::Bind(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int index = -1;
  int rc = 0;
  struct DBOpPrepareParams p_params = PrepareParams;
...
int
SQLGetBucket::Execute(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int ret = -1;
  class SQLObjectOp* ObPtr = NULL;
...
int
SQLListUserBuckets::Bind(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int index = -1;
  int rc = 0;
  struct DBOpPrepareParams p_params = PrepareParams;
...
int
SQLPutObject::Prepare(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int ret = -1;
  struct DBOpPrepareParams p_params = PrepareParams;
...
int
SQLPutObject::Bind(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int index = -1;
  int rc = 0;
  int VersionNum = 0;
  ...
  SQL_BIND_TEXT(
      dpp, stmt, index, params->op.obj.state.obj.key.name.c_str(), sdb);
  ...
  SQL_BIND_TEXT(
      dpp, stmt, index, params->op.obj.state.obj.key.instance.c_str(), sdb);
  ...
  SQL_BIND_TEXT(
      dpp, stmt, index, params->op.obj.owner_display_name.c_str(), sdb);
  ...
  SQL_BIND_TEXT(
      dpp, stmt, index, params->op.obj.state.obj.index_hash_source.c_str(), sdb);
  ...
  SQL_BIND_INT(
      dpp, stmt, index, params->op.obj.state.objv_tracker.read_version.ver, sdb);

  SQL_BIND_INDEX(dpp, stmt, index, p_params.op.obj.obj_version_tag, sdb);
  SQL_BIND_TEXT(
      dpp, stmt, index,
      params->op.obj.state.objv_tracker.read_version.tag.c_str(), sdb);
  ...
  SQL_BIND_INDEX(
      dpp, stmt, index, p_params.op.obj.head_placement_rule_name, sdb);
  SQL_BIND_TEXT(
      dpp, stmt, index, params->op.obj.head_placement_rule.name.c_str(), sdb);

  SQL_BIND_INDEX(
      dpp, stmt, index, p_params.op.obj.head_placement_storage_class, sdb);
  SQL_BIND_TEXT(
      dpp, stmt, index,
      params->op.obj.head_placement_rule.storage_class.c_str(), sdb);

  SQL_BIND_INDEX(
      dpp, stmt, index, p_params.op.obj.tail_placement_rule_name, sdb);
  SQL_BIND_TEXT(
      dpp, stmt, index,
      params->op.obj.tail_placement.placement_rule.name.c_str(), sdb);

  SQL_BIND_INDEX(
      dpp, stmt, index, p_params.op.obj.tail_placement_storage_class, sdb);
  SQL_BIND_TEXT(
      dpp, stmt, index,
      params->op.obj.tail_placement.placement_rule.storage_class.c_str(), sdb);
...
int
SQLPutObject::Execute(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int ret = -1;
...
int
SQLDeleteObject::Prepare(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int ret = -1;
  struct DBOpPrepareParams p_params = PrepareParams;
...
int
SQLDeleteObject::Bind(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int index = -1;
  int rc = 0;
  struct DBOpPrepareParams p_params = PrepareParams;
...
int
SQLDeleteObject::Execute(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int ret = -1;
...
int
SQLGetObject::Prepare(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int ret = -1;
  struct DBOpPrepareParams p_params = PrepareParams;
...
int
SQLGetObject::Bind(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int index = -1;
  int rc = 0;
  struct DBOpPrepareParams p_params = PrepareParams;
...
int
SQLGetObject::Execute(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int ret = -1;
...
int
SQLUpdateObject::Prepare(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int ret = -1;
  struct DBOpPrepareParams p_params = PrepareParams;
...
int
SQLUpdateObject::Bind(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int index = -1;
  int rc = 0;
  struct DBOpPrepareParams p_params = PrepareParams;
  ...
  if (params->op.query_str == "meta") {
    SQL_BIND_INDEX(dpp, *stmt, index, p_params.op.obj.obj_ns, sdb);
    SQL_BIND_TEXT(
        dpp, *stmt, index, params->op.obj.state.obj.key.ns.c_str(), sdb);
    ...
    SQL_BIND_INDEX(dpp, *stmt, index, p_params.op.obj.index_hash_source, sdb);
    SQL_BIND_TEXT(
        dpp, *stmt, index, params->op.obj.state.obj.index_hash_source.c_str(),
        sdb);
    ...
    SQL_BIND_INDEX(dpp, *stmt, index, p_params.op.obj.write_tag, sdb);
    SQL_BIND_TEXT(
        dpp, *stmt, index, params->op.obj.state.write_tag.c_str(), sdb);
    ...
    SQL_BIND_INDEX(dpp, *stmt, index, p_params.op.obj.shadow_obj, sdb);
    SQL_BIND_TEXT(
        dpp, *stmt, index, params->op.obj.state.shadow_obj.c_str(), sdb);
    ...
    SQL_BIND_INDEX(dpp, *stmt, index, p_params.op.obj.obj_version, sdb);
    SQL_BIND_INT(
        dpp, *stmt, index, params->op.obj.state.objv_tracker.read_version.ver,
        sdb);
    ...
    SQL_BIND_TEXT(
        dpp, *stmt, index,
        params->op.obj.tail_placement.placement_rule.storage_class.c_str(),
        sdb);
...
int
SQLUpdateObject::Execute(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int ret = -1;
  sqlite3_stmt** stmt = NULL; // Prepared statement
...
int
SQLListVersionedObjects::Prepare(
    const DoutPrefixProvider* dpp,
    struct DBOpParams* params)
{
  int ret = -1;
...
int
SQLListVersionedObjects::Execute(
    const DoutPrefixProvider* dpp,
    struct DBOpParams* params)
{
  int ret = -1;
...
int
SQLPutObjectData::Bind(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int index = -1;
  int rc = 0;
  struct DBOpPrepareParams p_params = PrepareParams;
  ...
  SQL_BIND_TEXT(
      dpp, stmt, index, params->op.obj_data.multipart_part_str.c_str(), sdb);
...
int
SQLGetObjectData::Bind(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int index = -1;
  int rc = 0;
  struct DBOpPrepareParams p_params = PrepareParams;
...
int
SQLDeleteStaleObjectData::Prepare(
    const DoutPrefixProvider* dpp,
    struct DBOpParams* params)
{
  int ret = -1;
...
int
SQLDeleteStaleObjectData::Execute(
    const DoutPrefixProvider* dpp,
    struct DBOpParams* params)
{
  int ret = -1;
...
int
SQLInsertLCEntry::Bind(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int index = -1;
  int rc = 0;
  struct DBOpPrepareParams p_params = PrepareParams;
...
int
SQLRemoveLCEntry::Bind(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int index = -1;
  int rc = 0;
  struct DBOpPrepareParams p_params = PrepareParams;
...
int
SQLGetLCEntry::Prepare(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int ret = -1;
  sqlite3_stmt** pstmt = NULL; // Prepared statement
  struct DBOpPrepareParams p_params = PrepareParams;
...
int
SQLGetLCEntry::Bind(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int index = -1;
  int rc = 0;
  struct DBOpPrepareParams p_params = PrepareParams;
  ...
  SQL_BIND_TEXT(
      dpp, *pstmt, index, params->op.lc_entry.entry.bucket.c_str(), sdb);
...
int
SQLGetLCEntry::Execute(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int ret = -1;
  sqlite3_stmt** pstmt = NULL; // Prepared statement
...
int
SQLListLCEntries::Bind(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int index = -1;
  int rc = 0;
  struct DBOpPrepareParams p_params = PrepareParams;
...
int
SQLInsertLCHead::Prepare(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int ret = -1;
  struct DBOpPrepareParams p_params = PrepareParams;
...
int
SQLInsertLCHead::Bind(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int index = -1;
  int rc = 0;
  struct DBOpPrepareParams p_params = PrepareParams;
  ...
  SQL_ENCODE_BLOB_PARAM(
      dpp, stmt, index,
      static_cast<int64_t>(params->op.lc_head.head.start_date), sdb);
...
int
SQLInsertLCHead::Execute(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int ret = -1;
...
int
SQLRemoveLCHead::Prepare(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int ret = -1;
  struct DBOpPrepareParams p_params = PrepareParams;
...
int
SQLRemoveLCHead::Bind(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int index = -1;
  int rc = 0;
  struct DBOpPrepareParams p_params = PrepareParams;
...
int
SQLRemoveLCHead::Execute(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int ret = -1;
...
int
SQLGetLCHead::Prepare(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int ret = -1;
  struct DBOpPrepareParams p_params = PrepareParams;
...
int
SQLGetLCHead::Bind(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int index = -1;
  int rc = 0;
  struct DBOpPrepareParams p_params = PrepareParams;
...
int
SQLGetLCHead::Execute(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
  int ret = -1;
