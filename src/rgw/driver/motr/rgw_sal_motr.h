...
public:
  ...
  void
  set_category(RGWObjCategory _category)
  {
    category = _category;
  }

  int get_bucket_dir_ent(
      const DoutPrefixProvider* dpp,
...
public:
  ...
  virtual int
  try_lock(const DoutPrefixProvider* dpp, utime_t dur, optional_yield y) override
  {
    return 0;
  }
...
public:
  ...
  virtual int
  initialize(CephContext* cct, const DoutPrefixProvider* dpp)
  {
    return 0;
  }
  ...
  virtual std::unique_ptr<Restore> get_restore(
      const int n_objs,
  ...
  virtual int
  clear_usage(const DoutPrefixProvider* dpp) override
  {
    return 0;
  }
  ...
  int next_query_by_name(
      std::string idx_name,
      std::vector<std::string>& key_str_vec,
  ...
  int store_access_key(
      const DoutPrefixProvider* dpp,
      optional_yield y,
      MotrAccessKey access_key);
  int delete_access_key(
      const DoutPrefixProvider* dpp,
      optional_yield y,
      std::string access_key);
  int store_email_info(
      const DoutPrefixProvider* dpp,
      optional_yield y,
  ...
  init(RGWObjState* state)
  {
    ...
    if (state->get_attr(RGW_ATTR_INTERNAL_MTIME, bl)) {
      try {
        ...
        ldpp_dout(dpp, 0) << "ERROR: couldn't decode RGW_ATTR_INTERNAL_MTIME"
                          << dendl;
      }
    }
  }
...
inline std::ostream&
operator<<(std::ostream& out, const obj_time_weight& o)
{
  out << o.mtime;
