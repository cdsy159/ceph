...
public:
  ...
  void
  set_env(std::shared_ptr<LMDBSafe::MDBEnv>& _env, LMDBSafe::MDBDbi& _dbi)
  {
    env = _env;
    dbi = _dbi;
  }
  ...
  struct BucketCacheEntryLT {
    ...
    bool
    operator()(const BucketCacheEntry& lhs, const BucketCacheEntry& rhs) const
    {
    ...
    bool
    operator()(const std::string& k, const BucketCacheEntry& rhs) const
    {
      return k < rhs.name;
    }

    bool
    operator()(const BucketCacheEntry& lhs, const std::string& k) const
    {
      return lhs.name < k;
    }
  };
  ...
  typedef bi::
      member_hook<BucketCacheEntry, member_hook_t, &BucketCacheEntry::name_hook>
          name_hook_t;
  typedef bi::avltree<BucketCacheEntry, bi::compare<BucketCacheEntryLT>, name_hook_t>
      bucket_avl_t;
  ...
  bool
  reclaim(const cohort::lru::ObjectFactory* newobj_fac)
  {
    auto factory =
        dynamic_cast<const BucketCacheEntry<D, B>::Factory*>(newobj_fac);
    if (factory == nullptr) {
      return false;
    }
    { /* anon block */
...
using fill_cache_cb_t = const fu2::unique_function<
    int(const DoutPrefixProvider* dpp, rgw_bucket_dir_entry&) const>;
...
struct BucketCache : public Notifiable {
  ...
  public:
    Lmdbs(std::string& database_root, uint8_t lmdb_count) :
      database_root(database_root), lmdb_count(lmdb_count), dbp(database_root)
    {
      ...
      for (int ix = 0; ix < lmdb_count; ++ix) {
        ...
        auto env = LMDBSafe::getMDBEnv(
            env_path.string().c_str(), 0 /* flags? */, 0600);
        envs.push_back(env);
      }
    }

    inline std::shared_ptr<LMDBSafe::MDBEnv>&
    get_sp_env(BucketCacheEntry<D, B>* bucket)
    {
      return envs[(bucket->hk % lmdb_count)];
    }

    inline LMDBSafe::MDBEnv&
    get_env(BucketCacheEntry<D, B>* bucket)
    {
...
public:
    ...
    un(Notify::factory(this, bucket_root))
  {
    if (!(sf::exists(rp) && sf::is_directory(rp))) {
      std::cerr << fmt::format(
                       "{} bucket root {} invalid", __func__, bucket_root)
                << std::endl;
      exit(1);
    }
    ...
    if (!(sf::exists(dp) && sf::is_directory(dp))) {
      std::cerr << fmt::format(
                       "{} database root {} invalid", __func__, database_root)
                << std::endl;
      exit(1);
    }
  }
  ...
  ~BucketCache()
  {
    ...
    cache.drain(
        [this](BucketCacheEntry<D, B>* entry) {
  ...
  retry:
    b = cache.find_latch(
        fac.hk /* partition selector */, name /* key */, lat /* serializer */,
        BucketCacheEntry<D, B>::bucket_avl_cache::FLAG_LOCK);
    /* LATCHED */
    if (b) {
      b->mtx.lock();
      if (b->deleted() || !lru.ref(b, cohort::lru::FLAG_INITIAL)) {
        // lru ref failed
        lat.lock->unlock();
        b->mtx.unlock();
        goto retry;
      ...
      if (b) [[likely]] {
        ...
        if (!(iflags & cohort::lru::FLAG_RECYCLE)) [[likely]] {
          ...
          cache.insert_latched(
              b, lat, BucketCacheEntry<D, B>::bucket_avl_cache::FLAG_UNLOCK);
        } else {
          /* recycle step invalidates Latch */
          lat.lock->unlock(); /* !LATCHED */
          cache.insert(
              fac.hk, b, BucketCacheEntry<D, B>::bucket_avl_cache::FLAG_NONE);
        }
  ...
  concat_key(const rgw_obj_index_key& k)
  {
    std::string k_str;
      ...
      list_bucket_each_t each_func)
  {
    ...
    GetBucketResult gbr = get_bucket(
        dpp, sal_bucket->get_name(),
      ...
      const std::vector<Notifiable::Event>& evec) override
  {
    ...
    int rc{0};
    GetBucketResult gbr =
        get_bucket(nullptr, bname, BucketCache<D, B>::FLAG_LOCK);
    auto [b /* BucketCacheEntry */, flags] = gbr;
    if (b) {
      unique_lock ulk{b->mtx, std::adopt_lock};
      if ((b->name != bname) || (opaque && (b != opaque)) ||
  ...
  int
  invalidate_bucket(const DoutPrefixProvider* dpp, std::string bname)
  {
    using namespace LMDBSafe;
