    ...
    unsigned int flags)
{
  ...
  if (const auto rc =
          mdb_dbi_open(txn, dbname.empty() ? 0 : &dbname[0], flags, &d_dbi))
    ...
    MDB_dbi maxDBs)
{
  ...
  if (const auto rc =
          mdb_env_set_mapsize(d_env, 16ULL * 4096 * 244140ULL)) { // 4GB
  ...
  if (const auto rc = mdb_env_open(d_env, fname, flags | MDB_NOTLS, mode)) {
    ...
    mdb_env_close(d_env);
    throw LMDBError(
        "Unable to open database file " + std::string(fname) + ": ", rc);
  }
}
...
std::shared_ptr<MDBEnv>
getMDBEnv(const char* fname, unsigned int flags, mdb_mode_t mode, MDB_dbi maxDBs)
{
  struct Value {
    weak_ptr<MDBEnv> wp;
    unsigned int flags;
  ...
  if (stat(fname, &statbuf)) {
    if (errno != ENOENT)
      throw LMDBError(
          "Unable to stat prospective mdb database: " + string(strerror(errno)));
    else {
      std::lock_guard<std::mutex> l(mut);
      auto fresh = std::make_shared<MDBEnv>(fname, flags, mode, maxDBs);
      if (stat(fname, &statbuf))
        throw LMDBError(
            "Unable to stat prospective mdb database: " +
...
MDBDbi
MDBEnv::openDB(const string_view dbname, unsigned int flags)
{
...
MDB_txn*
MDBRWTransactionImpl::openRWTransaction(
    MDBEnv* env,
    MDB_txn* parent,
    unsigned int flags)
{
...
MDBRWTransactionImpl::~MDBRWTransactionImpl() { MDBRWTransactionImpl::abort(); }
...
MDBROTransactionImpl::MDBROTransactionImpl(MDBEnv* parent, MDB_txn* txn) :
  d_parent(parent), d_cursors(), d_txn(txn)
{}

MDB_txn*
MDBROTransactionImpl::openROTransaction(
    MDBEnv* env,
    MDB_txn* parent,
    unsigned int flags)
{
...
void
MDBROTransactionImpl::closeROCursors()
{
...
MDBROTransactionImpl::abort()
{
  ...
  if (d_txn) {
    d_parent->decROTX();
    mdb_txn_abort(
        d_txn); // this appears to work better than abort for r/o database opening
    d_txn = nullptr;
  }
}
...
MDBROTransactionImpl::commit()
{
  ...
  if (d_txn) {
    d_parent->decROTX();
    if (const auto rc = mdb_txn_commit(
            d_txn)) { // this appears to work better than abort for r/o database opening
...
void
MDBRWTransactionImpl::clear(MDB_dbi dbi)
{
...
MDBRWCursor
MDBRWTransactionImpl::getRWCursor(const MDBDbi& dbi)
{
  MDB_cursor* cursor;
  ;
...
MDBRWCursor
MDBRWTransactionImpl::getCursor(const MDBDbi& dbi)
{
...
void
MDBRWTransactionImpl::closeRWCursors()
{
...
MDBROCursor
MDBROTransactionImpl::getCursor(const MDBDbi& dbi)
{
  return getROCursor(dbi);
}

MDBROCursor
MDBROTransactionImpl::getROCursor(const MDBDbi& dbi)
{
  MDB_cursor* cursor;
  if (const auto rc = mdb_cursor_open(d_txn, dbi, &cursor)) {
    throw LMDBError("Error creating RO cursor: ", rc);
