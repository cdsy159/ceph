...
public:
  static std::unique_ptr<Notify> factory(
      Notifiable* n,
...
class Inotify : public Notify {
  ...
  static constexpr uint32_t aw_mask =
      IN_ALL_EVENTS &
      ~(IN_MOVE_SELF | IN_OPEN | IN_ACCESS | IN_ATTRIB | IN_CLOSE_WRITE |
        IN_CLOSE_NOWRITE | IN_MODIFY | IN_DELETE_SELF);

  static constexpr uint64_t sig_shutdown =
      std::numeric_limits<uint64_t>::max() - 0xdeadbeef;
  ...
  public:
    ...
    {
      m = static_cast<char*>(
          aligned_alloc(__alignof__(struct inotify_event), rd_size));
      if (!m) [[unlikely]] {
        std::cerr << fmt::format("{} buffer allocation failure", __func__)
                  << std::endl;
        abort();
      }
    }

    ~AlignedBuf() { std::free(m); }
      ...
      if (npoll > 0) {
        ...
        for (char* ptr = buf; ptr < buf + len;
          ...
          if (event->mask & IN_Q_OVERFLOW) [[unlikely]] {
            ...
            evec.clear();
            evec.emplace_back(Notifiable::Event(
                Notifiable::EventType::INVALIDATE, std::nullopt));
            ...
            if ((event->mask & IN_CREATE) || (event->mask & IN_MOVED_TO)) {
              /* new object in dir */
              evec.emplace_back(
                  Notifiable::Event(Notifiable::EventType::ADD, event->name));
            } else if (
                (event->mask & IN_DELETE) || (event->mask & IN_MOVED_FROM)) {
              /* object removed from dir */
              evec.emplace_back(Notifiable::Event(
                  Notifiable::EventType::REMOVE, event->name));
            }
          } /* !overflow */
          if (evec.size() > 0) {
            n->notify(watch_name, watch_opaque, evec);
  ...
  {
    if (wfd == -1) {
      std::cerr << fmt::format("{} inotify_init1 failed with {}", __func__, wfd)
                << std::endl;
      exit(1);
    }
    if (efd == -1) {
...
public:
  virtual int
  add_watch(const std::string& dname, void* opaque) override
  {
    sf::path wp{rp / dname};
    int wd = inotify_add_watch(wfd, wp.c_str(), aw_mask);
    if (wd == -1) {
      std::cerr << fmt::format(
                       "{} inotify_add_watch {} failed with {}", __func__,
                       dname, wd)
                << std::endl;
    } else {
      std::lock_guard lock(map_mutex);
      wd_callback_map.insert(
          wd_callback_map_t::value_type(wd, WatchRecord(wd, dname, opaque)));
  ...
  virtual int
  remove_watch(const std::string& dname) override
  {
    int r{0};
    std::lock_guard lock(map_mutex);
    ...
    if (elt != wd_remove_map.end()) {
      ...
      if (r == -1) {
        std::cerr << fmt::format(
                         "{} inotify_rm_watch {} failed with {}", __func__,
                         dname, wd)
                  << std::endl;
      }
