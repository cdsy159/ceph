...
struct ObjectType {
  ...
  bool
  operator==(const ObjectType& t) const
  {
    return (type == t.type);
  }

  bool
  operator==(const ObjectType::Type& t) const
  {
    return (type == t);
  }
  ...
  operator<<(std::ostream& out, const ObjectType& t)
  {
    switch (t.type) {
    case UNKNOWN:
      out << "UNKNOWN";
...
public:
  FSEnt(std::string _name, Directory* _parent, CephContext* _ctx) :
  ...
  virtual int link_temp_file(
      const DoutPrefixProvider* dpp,
      optional_yield y,
      std::string target_fname) = 0;
  virtual std::unique_ptr<FSEnt> clone_base() = 0;
  virtual int fill_cache(
      const DoutPrefixProvider* dpp,
      optional_yield y,
      fill_cache_cb_t& cb);
...
public:
  File(std::string _name, Directory* _parent, CephContext* _ctx) :
    FSEnt(_name, _parent, _ctx)
  {}
  ...
  virtual int link_temp_file(
      const DoutPrefixProvider* dpp,
      optional_yield y,
      std::string target_fname) override;

  virtual std::unique_ptr<FSEnt>
  clone_base() override
  {
...
public:
  Directory(std::string _name, Directory* _parent, CephContext* _ctx) :
    FSEnt(_name, _parent, _ctx)
  {}
  ...
  virtual std::unique_ptr<Directory>
  clone_dir()
  {
    return std::make_unique<Directory>(*this);
  }
...
public:
  MPDirectory(std::string _name, Directory* _parent, CephContext* _ctx) :
  ...
  MPDirectory(const MPDirectory& _d) :
    Directory(_d), parts(_d.parts)
  {
    if (_d.cur_read_part)
      cur_read_part = _d.cur_read_part->clone_base();
  }
...
public:
  VersionedDirectory(std::string _name, Directory* _parent, CephContext* _ctx) :
    Directory(_name, _parent, _ctx)
  {}
  ...
  VersionedDirectory(
      ...
      CephContext* _ctx) :
    Directory(_name, _parent, _ctx), instance_id(_instance_id)
  {}

  VersionedDirectory(
      std::string _name,
      Directory* _parent,
      std::unique_ptr<FSEnt>&& _cur,
      CephContext* _ctx) :
    Directory(_name, _parent, _ctx), cur_version(std::move(_cur))
  {}
  ...
  VersionedDirectory(
      ...
      struct statx& _stx,
      CephContext* _ctx) :
    Directory(_name, _parent, _stx, _ctx), instance_id(_instance_id)
  {}
  ...
  std::unique_ptr<VersionedDirectory>
  clone()
  {
    return std::make_unique<VersionedDirectory>(*this);
  }
...
public:
  POSIXZoneGroup(POSIXDriver* _store, std::unique_ptr<RGWZoneGroup> _group) :
  ...
  virtual int
  get_zone_by_id(const std::string& id, std::unique_ptr<Zone>* zone) override
  {
    return -1;
  }

  virtual int
  get_zone_by_name(const std::string& name, std::unique_ptr<Zone>* zone) override
  {
    return -1;
  }
...
protected:
  ...
  std::unique_ptr<RGWZone>
      zone_public_config; /* external zone params, e.g., entrypoints, log flags, etc. */
  std::unique_ptr<RGWZoneParams>
      zone_params; /* internal zone params, e.g., rados pools */
  std::unique_ptr<RGWPeriod> current_period;

public:
  POSIXZone(POSIXDriver* _store) :
    store(_store)
  {
...
public:
  POSIXLuaManager(POSIXDriver* _s) :
    store(_s)
  {}
...
public:
  POSIXDriver(CephContext* _cct) :
    StoreDriver(), cct(_cct), zone(this)
  {
    ...
    userDB =
        std::make_unique<rgw::store::POSIXUserDB>(db_full_path.string(), cct);
  }
  ...
  virtual std::string
  get_cluster_id(const DoutPrefixProvider* dpp, optional_yield y) override
  {
    return "PLACEHOLDER";
  };
  ...
  virtual int load_owner_by_email(
      const DoutPrefixProvider* dpp,
      optional_yield y,
      std::string_view email,
  ...
  virtual std::string
  zone_unique_id(uint64_t unique_num) override
  {
    return "";
  }
  ...
  virtual std::unique_ptr<Lifecycle>
  get_lifecycle(void) override
  {
    return nullptr;
  } // TODO: implement

  virtual std::unique_ptr<Restore>
  get_restore(void)
  {
    return nullptr;
  }
  ...
  virtual bool
  process_expired_objects(const DoutPrefixProvider* dpp, optional_yield y) override
  {
    return 0;
  }
  ...
  {
    return NULL;
  } // TODO: Lifecycle not currently supported

  virtual rgw::restore::Restore*
  get_rgwrestore(void)
  {
    return nullptr;
  }

  virtual RGWCoroutinesManagerRegistry*
  ...
  virtual int
  log_op(const DoutPrefixProvider* dpp, std::string& oid, bufferlist& bl) override
  {
    return 0;
  }

  virtual int
  register_to_service_map(
      const DoutPrefixProvider* dpp,
  ...
  virtual void
  get_quota(RGWQuota& quota) override
  {
    return;
  }
  ...
  get_data_sync_manager(const rgw_zone_id& source_zone) override
  {
    return 0;
  }

  virtual void
  wakeup_meta_sync_shards(std::set<int>& shard_ids) override
  {
    return;
  }

  virtual void
  wakeup_data_sync_shards(
      const DoutPrefixProvider* dpp,
  ...
  virtual int
  clear_usage(const DoutPrefixProvider* dpp, optional_yield y) override
  {
    return 0;
  }
  ...
  virtual int
  get_config_key_val(std::string name, bufferlist* bl) override
  {
    return -ENOTSUP;
  }
  ...
  virtual void
  meta_list_keys_complete(void* handle) override
  {
    return;
  }

  virtual std::string
  meta_get_marker(void* handle) override
  {
    return "";
  }
  ...
  virtual const RGWSyncModuleInstanceRef&
  get_sync_module() override
  {
    return sync_module;
  }
  ...
  virtual bool
  valid_placement(const rgw_placement_rule& rule) override
  {
    return true;
  }
  ...
  get_root_fd()
  {
    return root_dir->get_fd();
  }

  rgw::store::POSIXUserDB*
  get_user_db()
  {
    return userDB.get();
  }
...
public:
  POSIXBucket(
      ...
      Directory* _p_dir,
      const rgw_bucket& _b,
      std::optional<std::string> _ns = std::nullopt) :
    StoreBucket(_b),
    driver(_dr),
    acls(),
    ns(_ns),
  ...
  POSIXBucket(
      POSIXDriver* _dr,
      std::unique_ptr<Directory> _this_dir,
      const rgw_bucket& _b,
      std::optional<std::string> _ns = std::nullopt) :
    StoreBucket(_b), driver(_dr), acls(), ns(_ns), dir(std::move(_this_dir))
  {}
  ...
  POSIXBucket(const POSIXBucket& _b) :
    StoreBucket(_b), driver(_b.driver), acls(_b.acls), ns(_b.ns)
  {
  ...
  virtual int sync_owner_stats(
      const DoutPrefixProvider* dpp,
      optional_yield y,
      RGWBucketEnt* ent) override;
  virtual int check_bucket_shards(
      const DoutPrefixProvider* dpp,
      uint64_t num_objs,
      optional_yield y) override;
  virtual int chown(
  ...
  int
  get_dir_fd(const DoutPrefixProvider* dpp)
  {
    dir->open(dpp);
    return dir->get_fd();
  }
...
public:
  ...
  POSIXObject(POSIXDriver* _dr, const rgw_obj_key& _k) :
    StoreObject(_k), driver(_dr), acls()
  {}

  POSIXObject(POSIXDriver* _driver, const rgw_obj_key& _k, Bucket* _b) :
    StoreObject(_k, _b), driver(_driver), acls()
  {}

  POSIXObject(const POSIXObject& _o) :
    StoreObject(_o), driver(_o.driver)
  {}
  ...
  bool
  check_exists(const DoutPrefixProvider* dpp)
  {
    stat(dpp);
    return state.exists;
  }
...
struct POSIXMPObj {
  ...
  void
  init(const std::string& _oid, const std::string& _upload_id, ACLOwner& _owner)
  {
    if (_oid.empty()) {
      clear();
      return;
  ...
  bool
  from_meta(const std::string& meta, ACLOwner& _owner)
  {
...
public:
  POSIXMultipartUpload(
    ...
    driver(_driver),
    mp_obj(driver, _oid, _upload_id, _owner),
    mtime(_mtime)
  {}
