...
static std::string
get_buckets_key(std::string_view account_id)
{
  return string_cat_reserve(buckets_oid_prefix, account_id);
}

rgw_raw_obj
get_buckets_obj(const RGWZoneParams& zone, std::string_view account_id)
{
  return {zone.account_pool, get_buckets_key(account_id)};
}

static std::string
get_users_key(std::string_view account_id)
{
  return string_cat_reserve(users_oid_prefix, account_id);
}

rgw_raw_obj
get_users_obj(const RGWZoneParams& zone, std::string_view account_id)
{
  return {zone.account_pool, get_users_key(account_id)};
}

static std::string
get_groups_key(std::string_view account_id)
{
  return string_cat_reserve(groups_oid_prefix, account_id);
}

rgw_raw_obj
get_groups_obj(const RGWZoneParams& zone, std::string_view account_id)
{
  return {zone.account_pool, get_groups_key(account_id)};
}

static std::string
get_roles_key(std::string_view account_id)
{
  return string_cat_reserve(roles_oid_prefix, account_id);
}

rgw_raw_obj
get_roles_obj(const RGWZoneParams& zone, std::string_view account_id)
{
  return {zone.account_pool, get_roles_key(account_id)};
}

static std::string
get_topics_key(std::string_view account_id)
{
  return string_cat_reserve(topics_oid_prefix, account_id);
}

rgw_raw_obj
get_topics_obj(const RGWZoneParams& zone, std::string_view account_id)
{
  return {zone.account_pool, get_topics_key(account_id)};
}

static std::string
get_account_key(std::string_view account_id)
{
  return string_cat_reserve(account_oid_prefix, account_id);
}

static rgw_raw_obj
get_account_obj(const RGWZoneParams& zone, std::string_view account_id)
{
  return {zone.account_pool, get_account_key(account_id)};
}

static std::string
get_name_key(std::string_view tenant, std::string_view name)
{
...
static std::string
get_email_key(std::string_view email)
{
...
static rgw_raw_obj
get_email_obj(const RGWZoneParams& zone, std::string_view email)
{
    ...
    RedirectObj& redirect)
{
  ...
  int r = rgw_get_system_obj(
      &sysobj, redirect.obj.pool, redirect.obj.oid, bl, &redirect.objv, nullptr,
      y, dpp);
  if (r < 0) {
    ...
    RedirectObj& redirect)
{
  ...
  return rgw_put_system_obj(
      dpp, &sysobj, redirect.obj.pool, redirect.obj.oid, bl, exclusive,
    ...
    RGWObjVersionTracker& objv)
{
  ...
  bufferlist bl;
  int r = rgw_get_system_obj(
      &sysobj, obj.pool, obj.oid, bl, &objv, &mtime, y, dpp, &attrs);
  if (r < 0) {
  ...
  try {
    ...
    ldpp_dout(dpp, 0) << "ERROR: failed to decode account info: " << e.what()
                      << dendl;
    return -EIO;
  }
  if (info.id != account_id) {
    ldpp_dout(dpp, 0) << "ERROR: read account id mismatch " << info.id
                      << " != " << account_id << dendl;
    return -EIO;
  }
  return 0;
    ...
    RGWObjVersionTracker& objv)
{
  ...
  if (!same_name && !info.name.empty()) {
    ...
    if (r == -ENOENT) {
      ...
      ldpp_dout(dpp, 1) << "ERROR: account name obj " << redirect.obj
                        << " already taken for account id " << redirect.data.id
                        << dendl;
      return -EEXIST;
    } else if (r < 0) {
      return r;
  ...
  if (remove_name) {
    ...
    int r = rgw_delete_system_obj(
        dpp, &sysobj, redirect.obj.pool, redirect.obj.oid, &redirect.objv, y);
    if (r < 0) {
      ldpp_dout(dpp, 20) << "WARNING: failed to remove old name obj "
                         << redirect.obj.oid << ": " << cpp_strerror(r)
                         << dendl;
    } // not fatal
  }
  if (!same_name && !info.name.empty()) {
    ...
    if (r < 0) {
      ldpp_dout(dpp, 20) << "WARNING: failed to write name obj " << redirect.obj
                         << " with: " << cpp_strerror(r) << dendl;
    } // not fatal
  }
  ...
  if (!same_email && !info.email.empty()) {
    ...
    if (r < 0) {
      ldpp_dout(dpp, 20) << "WARNING: failed to write email obj "
                         << redirect.obj << " with: " << cpp_strerror(r)
                         << dendl;
    } // not fatal
  }
    ...
    RGWObjVersionTracker& objv)
{
  ...
  if (r < 0) {
    ldpp_dout(dpp, 1) << "ERROR: failed to remove account obj " << obj
                      << " with: " << cpp_strerror(r) << dendl;
    return r;
  }
  ...
  if (!info.name.empty()) {
    ...
    if (r < 0) {
      ldpp_dout(dpp, 20) << "WARNING: failed to remove name obj " << obj
                         << " with: " << cpp_strerror(r) << dendl;
    } // not fatal
  }
  if (!info.email.empty()) {
    ...
    r = rgw_delete_system_obj(dpp, &sysobj, obj.pool, obj.oid, nullptr, y);
    if (r < 0) {
      ldpp_dout(dpp, 20) << "WARNING: failed to remove email obj " << obj
                         << " with: " << cpp_strerror(r) << dendl;
    } // not fatal
  }
  {
    ...
    r = rgw_delete_system_obj(dpp, &sysobj, obj.pool, obj.oid, nullptr, y);
    if (r < 0) {
      ldpp_dout(dpp, 20) << "WARNING: failed to remove users obj " << obj
                         << " with: " << cpp_strerror(r) << dendl;
    } // not fatal
  }
...
public:
      ...
      const DoutPrefixProvider* dpp) override
  {
    ...
    int r = read(
        dpp, y, sysobj, zone, account_id, aci.info, aci.attrs, mtime, objv);
    if (r < 0) {
      return r;
    }
      ...
      bool from_remote_zone) override
  {
    ...
    int r = read(
        dpp, y, sysobj, zone, account_id, old_info, old_attrs, old_mtime, objv);
    if (r < 0 && r != -ENOENT) {
      return r;
    }
    ...
    return write(
        dpp, y, sysobj, zone, new_info, pold_info, account_obj->get().attrs,
      ...
      const DoutPrefixProvider* dpp) override
  {
    ...
    std::map<std::string, ceph::buffer::list> attrs;
    ceph::real_time mtime;
    int r = read(dpp, y, sysobj, zone, account_id, info, attrs, mtime, objv);
    if (r < 0) {
      return r;
    }
  ...
  {
    auto lister =
        std::make_unique<MetadataLister>(sysobj.get_pool(zone.account_pool));
...
auto
create_metadata_handler(RGWSI_SysObj& sysobj, const RGWZoneParams& zone)
