...
std::string
zone_info_oid(std::string_view zone_id)
{
  return string_cat_reserve(zone_info_oid_prefix, zone_id);
}

std::string
zone_name_oid(std::string_view zone_id)
{
  return string_cat_reserve(zone_names_oid_prefix, zone_id);
}

std::string
default_zone_oid(const ceph::common::ConfigProxy& conf, std::string_view realm_id)
{
  return fmt::format("{}.{}", conf->rgw_default_zone_info_oid, realm_id);
}

int
RadosConfigStore::write_default_zone_id(
...
int
RadosConfigStore::read_default_zone_id(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    std::string_view realm_id,
    std::string& zone_id)
...
int
RadosConfigStore::delete_default_zone_id(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    std::string_view realm_id)
{
...
public:
  RadosZoneWriter(
    ...
    impl(impl), objv(std::move(objv)), zone_id(zone_id), zone_name(zone_name)
  {}
...
int
RadosConfigStore::read_zone_by_id(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    std::string_view zone_id,
    RGWZoneParams& info,
...
int
RadosConfigStore::read_zone_by_name(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    std::string_view zone_name,
    RGWZoneParams& info,
...
int
RadosConfigStore::read_default_zone(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    std::string_view realm_id,
    RGWZoneParams& info,
...
int
RadosConfigStore::list_zone_names(
    const DoutPrefixProvider* dpp,
    optional_yield y,
