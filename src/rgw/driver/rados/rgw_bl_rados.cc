...
static std::string
get_commit_list_name(
    const rgw::sal::Bucket* log_bucket,
    const std::string& prefix)
{
  return fmt::format(
      "{}/{}/{}/{}", log_bucket->get_tenant(), log_bucket->get_name(),
      log_bucket->get_bucket_id(), prefix);
}

int
add_commit_target_entry(
    ...
    optional_yield y)
{
  ...
  std::string target_obj_name = get_commit_list_name(log_bucket, prefix);
  {
    ...
    auto ret = rgw_rados_operate(
        dpp, ioctx_logging, target_obj_name, std::move(op), y);
    if (ret < 0) {
      ldpp_dout(dpp, 1) << "ERROR: failed to add logging object entry "
                        << obj_name
  ...
  auto ret = rgw_rados_operate(
      dpp, ioctx_logging, COMMIT_LIST_OBJECT_NAME, std::move(op), y);
  if (ret < 0) {
    ...
    optional_yield y)
{
  ...
  while (more) {
    ...
    const auto ret = rgw_rados_operate(
        dpp, ioctx_logging, target_obj_name, std::move(op), nullptr, y);
    if (ret == -ENOENT) {
...
class BucketLoggingManager : public DoutPrefixProvider {
  ...
  static constexpr auto m_log_commits_update_period =
      std::chrono::milliseconds(10000); // 10s
  static constexpr auto m_log_commits_update_retry =
      std::chrono::milliseconds(1000); // 1s
  static constexpr auto m_log_commits_idle_sleep =
      std::chrono::milliseconds(5000); // 5s
  const utime_t m_failover_time =
      utime_t(m_log_commits_update_period * 9); // 90s
...
*/
  ...
  int
  read_global_logging_list(commit_list_t& commits, optional_yield y)
  {
    constexpr auto max_chunk = 1024U;
    std::string start_after;
    bool more = true;
    ...
    while (more) {
      ...
      const auto ret = rgw_rados_operate(
          this, m_rados_store->getRados()->get_logging_pool_ctx(),
          COMMIT_LIST_OBJECT_NAME, std::move(op), nullptr, y);
      if (ret == -ENOENT) {
      ...
      if (ret < 0) {
        // TODO: do we need to check on rval as well as ret?
        ldpp_dout(this, 5) << "ERROR: failed to read bucket logging global "
                              "commit list. error: "
                           << ret << dendl;
        return ret;
      }
      ...
      boost::asio::yield_context yield)
  {
    ...
    if (ret < 0 && ret != -ENOENT) {
      ldpp_dout(this, 1) << "ERROR: failed to load bucket : " << bucket_name
                         << " id : " << bucket_id << ", error: " << ret
                         << dendl;
      return ret;
    }
    if (ret == 0) {
      ldpp_dout(this, 20) << "INFO: loaded bucket : " << bucket_name
                          << ", id: " << bucket_id << dendl;
      ret = log_bucket->commit_logging_object(
          entry, yield, this, prefix, nullptr, false);
      if (ret < 0) {
    ...
    ldpp_dout(this, 20) << "INFO: log bucket : " << bucket_name
                        ...
                        << temp_obj_name << " from pool " << obj_pool << dendl;
    ret = rgw_delete_system_obj(
        this, m_rados_store->svc()->sysobj, obj_pool, temp_obj_name, nullptr,
        yield);
    if (ret < 0 && ret != -ENOENT) {
    ...
    ldpp_dout(this, 20) << "INFO: Deleted pending logging object "
                        ...
                        << " for deleted log bucket : " << bucket_name << dendl;
    return 0;
  }
      ...
      boost::asio::yield_context yield)
  {
    ...
    const auto ret = rgw_rados_operate(
        this, rados_ioctx, list_obj_name, std::move(op), yield);
    if (ret == -ENOENT) {
  ...
  {
    {
      ...
      rados::cls::lock::assert_locked(
          &op, m_lock_name, ClsLockType::EXCLUSIVE, m_lock_cookie,
          "" /*no tag*/);
      op.remove();
      auto ret = rgw_rados_operate(
          this, rados_ioctx, commit_list_name, std::move(op), y);
      if (ret < 0 && ret != -ENOENT) {
        ldpp_dout(this, 1)
            << "ERROR: failed to remove bucket logging commit list :"
      ...
      boost::asio::yield_context yield)
  {
    ...
    int ret = parse_list_name(
        list_obj_name, tenant_name, bucket_name, bucket_id, prefix);
    if (ret) {
    ...
    while (!m_shutdown) {
      ...
      auto total_entries = 0U;
      {
        ...
        auto ret = rgw_rados_operate(
            this, rados_ioctx, list_obj_name, std::move(op), nullptr, yield);
        if (ret == -ENOENT) {
        ...
        if (ret == -EBUSY) {
          ldpp_dout(this, 10)
              << "WARNING: commit list : " << list_obj_name
        ...
        try {
          ...
          ldpp_dout(this, 1)
              << "ERROR:  failed to get pool information from object: "
              << list_obj_name << dendl;
          return;
        }
      }
      ...
      if (total_entries == 0) {
        ...
        if (ret == -ENOENT) {
          ldpp_dout(this, 20)
              << "ERROR: failed to load bucket : " << bucket_name
              << ", id : " << bucket_id << ", error: " << ret << dendl;
          bucket_deleted = true;
        }
      } else {
        // log when not idle
        ldpp_dout(this, 20)
            << "INFO: found: " << total_entries
            << " entries in commit list: " << list_obj_name << dendl;
      }
      ...
      for (auto const& [key, val] : entries) {
        ...
        boost::asio::spawn(
            ...
            [this, &is_idle, &list_obj_name, &remove_entries, &stop_processing,
             &tenant_name, &bucket_name, &bucket_id, &prefix,
             token = std::move(token), key, temp_obj_name, temp_data_pool,
             &bucket_deleted,
             &skip_bucket_deletion](boost::asio::yield_context yield) {
              auto result = process_entry(
                  this->get_cct()->_conf, key, temp_obj_name, tenant_name,
      ...
      if (!remove_entries.empty()) {
        ...
        if (ret < 0) {
          ldpp_dout(this, 1)
              << "ERROR: failed to remove entries from commit list: "
              << list_obj_name << ". error: " << ret
              << ". This could cause some log records to be lost." << dendl;
          return;
        } else {
          ldpp_dout(this, 20)
              << "INFO: removed entries from commit list: " << list_obj_name
              << dendl;
        }
      }
    }
  ...
  void
  process_global_logging_list(boost::asio::yield_context yield)
  {
    ...
    while (!m_shutdown) {
      ...
      if (ceph::coarse_real_clock::now() > next_check_time) {
        next_check_time = ceph::coarse_real_clock::now() +
                          m_log_commits_update_period;
        const auto tp =
            ceph::coarse_real_time::clock::to_time_t(next_check_time);
        ldpp_dout(this, 20)
            << "INFO: processing global logging commit list. next "
            << "processing will happen at: " << std::ctime(&tp) << dendl;
      } else {
        // short sleep duration to prevent busy wait when refreshing list
        // or retrying after error
        ldpp_dout(this, 20)
            << "INFO: processing global logging commit list. Sleep now."
            << dendl;
      ...
      if (ret < 0) {
        has_error = true;
        ldpp_dout(this, 1)
            << "ERROR: failed to read global logging list. Retry. ret:" << ret
            << dendl;
        continue;
      }
      ...
      for (const auto& list_obj_name : commit_lists) {
        ...
        ldpp_dout(this, 20)
            << "INFO: processing commit list: " << list_obj_name << dendl;
        librados::ObjectWriteOperation op;
        op.assert_exists();
        rados::cls::lock::lock(
            &op, m_lock_name, ClsLockType::EXCLUSIVE, m_lock_cookie,
            "" /*no tag*/, "" /*no description*/, m_failover_time,
            LOCK_FLAG_MAY_RENEW);
        ...
        if (ret == -ENOENT) {
          ...
          ldpp_dout(this, 20)
              << "INFO: commit list: " << list_obj_name
        ...
        if (ret < 0) {
          ...
          ldpp_dout(this, 10)
              << "ERROR: failed to lock commit list: " << list_obj_name
              << ". error: " << ret << dendl;
          has_error = true;
          continue;
        }
        ...
        if (owned_commit_logs.insert(list_obj_name).second) {
          ldpp_dout(this, 20) << "INFO: " << list_obj_name
                              << " now owned (locked) by this daemon" << dendl;
          // start processing this list
          boost::asio::spawn(
              make_strand(m_io_context), std::allocator_arg,
              make_stack_allocator(),
              [this, &commit_list_gc, &commit_list_gc_lock, &rados_ioctx,
               list_obj_name,
               &processed_list_count](boost::asio::yield_context yield) {
                ...
                const auto ret =
                    unlock_list_object(rados_ioctx, list_obj_name, yield);
                if (ret < 0) {
                  ldpp_dout(this, 15)
                      << "WARNING: failed to unlock commit list: "
              ...
              [this, list_obj_name](std::exception_ptr eptr) {
                ldpp_dout(this, 10) << "ERROR: " << list_obj_name
                                    << " processing failed" << dendl;
                if (eptr)
                  std::rethrow_exception(eptr);
              });
        } else {
      ...
      // erase all list objects that were deleted
      {
        std::lock_guard lock_guard(commit_list_gc_lock);
        std::for_each(
            commit_list_gc.begin(), commit_list_gc.end(),
            [this, &owned_commit_logs](const std::string& list_obj_name) {
              owned_commit_logs.erase(list_obj_name);
              ldpp_dout(this, 20)
                  << "INFO: commit list object: " << list_obj_name
                  << " was removed" << dendl;
            });
    ...
    ldpp_dout(this, 5) << "INFO: BucketLogging manager stopped. Done "
                          "processing all commit lists"
                       << dendl;
  }
...
public:
  ...
  {
    ldpp_dout(this, 5)
        << "INFO: BucketLogging manager received stop signal. shutting down..."
        << dendl;
    m_shutdown = true;
    m_work_guard.reset();
    if (m_worker.joinable()) {
      ...
      if (future.wait_for(m_log_commits_update_retry * 2) ==
          std::future_status::timeout) {
        // force stop if graceful shutdown takes too long
        if (!m_io_context.stopped()) {
          ldpp_dout(this, 5)
              << "INFO: force shutdown of BucketLogging Manager" << dendl;
          m_io_context.stop();
        }
        m_worker.join();
      }
    }
    ldpp_dout(this, 5) << "INFO: BucketLogging manager shutdown complete"
                       << dendl;
  }
  ...
  {
    boost::asio::spawn(
        make_strand(m_io_context), std::allocator_arg, make_stack_allocator(),
    ...
    m_worker = std::thread([this]() {
      ...
      try {
        ...
        ldpp_dout(this, 10)
            << "ERROR: bucket logging worker failed with error: " << err.what()
            << dendl;
        throw err;
      }
    });
    ...
    const SiteConfig& site)
{
  if (s_manager) {
    ldpp_dout(dpp, 1)
        << "ERROR: failed to init BucketLogging manager: already exists"
        << dendl;
    return false;
  }
  ...
  s_manager =
      std::make_unique<BucketLoggingManager>(dpp->get_cct(), store, site);
  s_manager->init();
  return true;
}
