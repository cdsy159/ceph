...
static const string BI_OLH_ENTRY_NS_START =
    "\x80"
    "1001_";
static const string BI_INSTANCE_ENTRY_NS_START =
    "\x80"
    "1000_";
...
static void
dump_multipart_index_results(std::list<rgw_obj_index_key>& objs, Formatter* f)
{
    ...
    const DoutPrefixProvider* dpp)
{
  ...
  do {
    int ret = driver->list_buckets(
        dpp, owner, tenant, listing.next_marker, string(), max_entries, false,
        listing, y);
    if (ret < 0) {
      ldpp_dout(dpp, 0) << "failed to read user buckets: " << cpp_strerror(-ret)
                        << dendl;
      return;
    }
    ...
    for (const auto& ent : listing.buckets) {
      ...
      int r = driver->load_bucket(
          dpp, rgw_bucket(tenant, ent.bucket.name), &bucket, y);
      if (r < 0) {
        ldpp_dout(dpp, 0) << "could not get bucket info for bucket=" << bucket
                          << dendl;
        continue;
      }

      if (ent.bucket != bucket->get_key()) {
        cout << "bucket info mismatch: expected " << ent.bucket << " got "
             << bucket << std::endl;
        if (fix) {
...
bool
rgw_bucket_object_check_filter(const std::string& oid)
{
    ...
    const bool force)
{
  ...
  return object->delete_object(
      dpp, y, rgw::sal::FLAG_LOG_OP | possible_force_flag, nullptr, nullptr);
}

static void
set_err_msg(std::string* sink, std::string msg)
{
  if (sink && !msg.empty())
    *sink = msg;
}
    ...
    std::string* err_msg)
{
  ...
  int r = driver->load_bucket(
      dpp, rgw_bucket(tenant, bucket_name, bucket_id), &bucket, y);
  if (r < 0) {
    set_err_msg(
        err_msg, "failed to fetch bucket info for bucket=" + bucket_name);
    return r;
  }
...
bool
rgw_find_bucket_by_id(
    const DoutPrefixProvider* dpp,
    CephContext* cct,
    rgw::sal::Driver* driver,
    const string& marker,
    const string& bucket_id,
    rgw_bucket* bucket_out)
{
  void* handle = NULL;
...
RGWBucket::chown(
    ...
    std::string* err_msg)
{
  return rgw_chown_bucket_and_objects(
      driver, bucket.get(), user.get(), marker, err_msg, dpp, y);
}
...
static void
dump_bucket_index(const vector<rgw_bucket_dir_entry>& objs, Formatter* f)
{
    ...
    optional_yield y)
{
  ...
  int ret = bs.init(
      dpp, bucket->get_info(), bucket->get_info().layout.current_index, shard,
      y);
  if (ret < 0) {
  ...
  std::string marker =
      rgw_obj_key(std::string(), std::string(), RGW_OBJ_NS_MULTIPART)
          .get_index_key_name();
  ...
  do {
    entries_read.clear();
    ret = store->bi_list(
        bs, "", marker, -1, &entries_read, &is_truncated, false, y);
    if (ret < 0) {
    ...
    for (const auto& entry : entries_read) {
      ...
      if (entry_suffix == "meta") {
        if (entry_prefix != prev_entry_prefix) {
          entries_to_unlink.insert(
              entries_to_unlink.end(), entries_window.cbegin(),
              entries_window.cend());
        }
      ...
      if (entries_to_unlink.size() >= listing_max_entries) {
        dump_multipart_index_results(entries_to_unlink, flusher.get_formatter());
        if (fix_index) {
          store->remove_objs_from_index(
              dpp, bucket->get_info(), entries_to_unlink);
        }
  ...
  entries_to_unlink.insert(
      entries_to_unlink.end(), entries_window.cbegin(), entries_window.cend());
  entries_window.clear();

  if (!entries_to_unlink.empty()) {
    dump_multipart_index_results(entries_to_unlink, flusher.get_formatter());
    if (fix_index) {
      store->remove_objs_from_index(dpp, bucket->get_info(), entries_to_unlink);
    }
    entries_to_unlink.clear();
  }
 ...
 */
int
RGWBucket::check_bad_index_multipart(
    rgw::sal::RadosStore* const rados_store,
    RGWBucketAdminOpState& op_state,
    RGWFormatterFlusher& flusher,
    const DoutPrefixProvider* dpp,
    ...
    std::string* err_msg)
{
  ...
  if (index_layout.type != rgw::BucketIndexType::Normal) {
    ldpp_dout(dpp, 0)
        << "ERROR: cannot check bucket indices with layouts of type "
    ...
    boost::asio::spawn(
        ...
        [&](boost::asio::yield_context yield) {
          while (true) {
            ...
            int r = ::check_bad_index_multipart(
                rados_store, &*bucket, dpp, op_state, flusher, shard, yield);
            if (r < 0) {
              ldpp_dout(dpp, -1) << "WARNING: error processing shard " << shard
                                 << " check_bad_index_multipart(): " << r
                                 << "; skipping" << dendl;
              if (!any_error) {
  ...
  do {
    ...
    for (iter = entries.begin(); iter != entries.end(); ++iter) {
      ...
      try {
        ...
        ldpp_dout(dpp, -1) << "ERROR failed to decode olh entry for key: "
                           << entry.idx << dendl;
        continue;
      }
      ...
      if (op_state.will_fix_index()) {
        ...
        if (olh_entry.pending_log.empty()) {
          ret = store->clear_olh(
              dpp, obj_ctx, obj, bucket->get_info(), olh_entry.tag,
              olh_entry.epoch, y);
          if (ret < 0) {
            ldpp_dout(dpp, -1)
                << "ERROR failed to clear olh for: " << olh_entry.key.name
                << " clear_olh(): " << cpp_strerror(-ret) << dendl;
            continue;
          }
        } else {
          std::unique_ptr<rgw::sal::Object> object =
              bucket->get_object({olh_entry.key.name});
          ret = object->load_obj_state(dpp, y, false);
          if (ret < 0) {
            ldpp_dout(dpp, -1)
                << "ERROR failed to load state for: " << olh_entry.key.name
                << " load_obj_state(): " << cpp_strerror(-ret) << dendl;
            continue;
          }
          RGWObjState& state =
              static_cast<rgw::sal::RadosObject*>(object.get())->get_state();
          ret = store->update_olh(
              dpp, obj_ctx, &state, bucket->get_info(), obj, y);
          if (ret < 0) {
            ldpp_dout(dpp, -1)
                << "ERROR failed to update olh for: " << olh_entry.key.name
    ...
    RGWFormatterFlusher& flusher)
{
  ...
  if ((bucket_info.versioning_status() & BUCKET_VERSIONED) == 0) {
    ldpp_dout(dpp, 0)
        << "WARNING: this command is only applicable to versioned buckets"
        << dendl;
    return 0;
  }
  ...
  if (index_layout.type != rgw::BucketIndexType::Normal) {
    ldpp_dout(dpp, 0)
        << "ERROR: cannot check bucket indices with layouts of type "
        << current_layout_desc(bucket_info.layout) << " for bad OLH entries"
        << dendl;
    return -EINVAL;
  }
    ...
    boost::asio::spawn(
        ...
        [&](boost::asio::yield_context yield) {
          while (true) {
            ...
            int r = ::check_index_olh(
                rados_store, &*bucket, dpp, op_state, flusher, shard,
                &shard_count, y);
            if (r < 0) {
            ...
            if (!op_state.hide_progress) {
              ldpp_dout(dpp, 1)
                  << "NOTICE: finished shard " << shard << " (" << shard_count
                  << " entries " << verb << ")" << dendl;
            }
          }
        },
 ...
 */
static int
is_versioned_instance_listable(
    const DoutPrefixProvider* dpp,
    ...
    optional_yield y)
{
  ...
  do {
    ...
    cls_rgw_bucket_list_op(
        op, marker, key.name, empty_delim, 1000, true, &result);
    bufferlist ibl;
    ...
    for (iter = entries.begin(); iter != entries.end(); ++iter) {
      ...
      if (ret < 0) {
        ldpp_dout(dpp, -1) << "ERROR is_versioned_instance_listable(key='"
                           << dir_entry.key << "'): " << cpp_strerror(-ret)
                           << dendl;
        continue;
      }
      if (listable) {
      ...
      if (op_state.will_fix_index()) {
        ...
        if (ret < 0) {
          ldpp_dout(dpp, -1) << "ERROR rgw_remove_obj(key='" << dir_entry.key
                             << "'): " << cpp_strerror(-ret) << dendl;
          continue;
        }
      }
 ...
 */
int
RGWBucket::check_index_unlinked(
    rgw::sal::RadosStore* const rados_store,
    const DoutPrefixProvider* dpp,
    RGWBucketAdminOpState& op_state,
    RGWFormatterFlusher& flusher)
    ...
    std::string* err_msg)
{
  if (!driver->is_meta_master()) {
    set_err_msg(
        err_msg,
        "ERROR: failed to update bucket sync: only allowed on meta master "
        "zone");
    return -EINVAL;
  }
    ...
    const DoutPrefixProvider* dpp)
{
  ...
  if (!object_name.empty()) {
    ...
    std::unique_ptr<rgw::sal::Object> obj =
        bucket->get_object(rgw_obj_key(object_name));
    ...
    if (ret < 0) {
      ldout(driver->ctx(), 0)
          << "failed to decode RGWAccessControlPolicy" << dendl;
    }
    return ret;
  }

  map<string, bufferlist>::iterator aiter =
      bucket->get_attrs().find(RGW_ATTR_ACL);
...
int
RGWBucketAdminOp::dump_s3_policy(
    rgw::sal::Driver* driver,
    RGWBucketAdminOpState& op_state,
    ostream& os,
    const DoutPrefixProvider* dpp,
    ...
    string* err)
{
  ...
  return radosdriver->ctl()->bucket->unlink_bucket(
      *rados, owner, op_state.get_bucket()->get_info().bucket, y, dpp, true);
}

int
RGWBucketAdminOp::link(
    rgw::sal::Driver* driver,
    RGWBucketAdminOpState& op_state,
    const DoutPrefixProvider* dpp,
    optional_yield y,
    string* err)
{
  ...
  if (op_state.is_account_op()) {
    ...
    ret = driver->load_account_by_id(
        dpp, y, op_state.get_account_id(), info, attrs, objv);
    if (ret < 0) {
    ...
    set_err_msg(
        err, "account users cannot own buckets. use --account-id instead");
    return -EINVAL;
  } else {
  ...
  map<string, bufferlist>::iterator aiter =
      loc_bucket->get_attrs().find(RGW_ATTR_ACL);
  if (aiter == loc_bucket->get_attrs().end()) {
    // should never happen; only pre-argonaut buckets lacked this.
    ldpp_dout(dpp, 0) << "WARNING: can't bucket link because no acl on bucket="
                      << old_bucket << dendl;
    set_err_msg(
        err,
  ...
  auto* rados = radosdriver->getRados()->get_rados_handle();
  int r = radosdriver->ctl()->bucket->unlink_bucket(
      *rados, owner.id, old_bucket->get_info().bucket, y, dpp, false);
  if (r < 0) {
    set_err_msg(
        err, "could not unlink bucket from owner " + to_string(owner.id));
    return r;
  }

  // now update the user for the bucket...
  if (display_name.empty()) {
    ldpp_dout(dpp, 0) << "WARNING: user " << op_state.get_user_id()
  ...
  if (r < 0) {
    set_err_msg(
        err, "ERROR: failed writing bucket instance info: " + cpp_strerror(-r));
    return r;
  }
  ...
  r = radosdriver->ctl()->bucket->link_bucket(
      *rados, new_owner, loc_bucket->get_info().bucket,
      loc_bucket->get_info().creation_time, y, dpp, true, &ep_data);
  if (r < 0) {
  ...
  if (*loc_bucket != *old_bucket) {
    ...
    if (r < 0) {
      set_err_msg(
          err, "failed to unlink old bucket " + old_bucket->get_tenant() + "/" +
                   old_bucket->get_name());
      return r;
    }
    r = radosdriver->ctl()->bucket->remove_bucket_instance_info(
        old_bucket->get_key(), old_bucket->get_info(), y, dpp,
        RGWBucketCtl::BucketInstance::RemoveParams().set_objv_tracker(
            &ep_data.ep_objv));
    if (r < 0) {
      set_err_msg(
          err, "failed to unlink old bucket " + old_bucket->get_tenant() + "/" +
...
int
RGWBucketAdminOp::check_index_olh(
    ...
    const DoutPrefixProvider* dpp)
{
  ...
  if (!rados_store) {
    ldpp_dout(dpp, 0) << "WARNING: couldn't access a RadosStore, "
                         "so skipping bad incomplete multipart check"
                      << dendl;
  } else {
    ret = bucket.check_bad_index_multipart(
        rados_store, op_state, flusher, dpp, y);
    if (ret < 0) {
      return ret;
    }
  }
...
RGWBucketAdminOp::remove_bucket(
    ...
    bool forwarded_request)
{
  std::unique_ptr<rgw::sal::Bucket> bucket;

  int ret = driver->load_bucket(
      dpp, rgw_bucket(op_state.get_tenant(), op_state.get_bucket_name()),
      &bucket, y);
  if (ret < 0)
    return ret;

  // check if the bucket is owned by my zonegroup, if not, refuse to remove it
  if (!forwarded_request && bucket->get_info().zonegroup !=
                                driver->get_zone()->get_zonegroup().get_id()) {
    ldpp_dout(dpp, 0)
        << "ERROR: The bucket's zonegroup does not match. "
        << "Removal is not allowed. Please execute this operation "
        << "on the bucket's zonegroup: " << bucket->get_info().zonegroup
        << dendl;
    return -ERR_PERMANENT_REDIRECT;
  }

  if (bypass_gc)
    ret = bucket->remove_bypass_gc(
        op_state.get_max_aio(), keep_index_consistent, y, dpp);
  else
  ...
  env.set(
      "QUERY_STRING",
      fmt::format(
          "bucket={}&tenant={}", bucket->get_name(), bucket->get_tenant()));
  req_info req(dpp->get_cct(), &env);
  rgw_err err; // unused

  ret = rgw_forward_request_to_master(
      dpp, site, bucket->get_owner(), nullptr, nullptr, req, err, y);
  if (ret < 0) {
    ldpp_dout(dpp, 0)
        << "ERROR: failed to forward request to master zonegroup: " << ret
        << dendl;
    return ret;
  }
...
int
RGWBucketAdminOp::remove_object(
    rgw::sal::Driver* driver,
    RGWBucketAdminOpState& op_state,
    const DoutPrefixProvider* dpp,
    optional_yield y)
    ...
    optional_yield y)
{
  ...
  int ret = driver->load_bucket(
      dpp, rgw_bucket(tenant_name, bucket_name), &bucket, y);
  if (ret < 0) {
    return ret;
  }
  rgw::sal::Bucket::ListParams params;
  ...
  do {
    ...
    for (vector<rgw_bucket_dir_entry>::iterator iter = results.objs.begin();
      ...
      if (obj) {
        ...
        for (map<string, bufferlist>::iterator getattriter =
                 obj->get_attrs().begin();
             getattriter != obj->get_attrs().end(); ++getattriter) {
    ...
    optional_yield y)
{
  ...
  // - are not indexless
  const bool local_zonegroup =
      (site.get_zonegroup().id == bucket_info.zonegroup);
  ...
  if (has_index) {
    ret = bucket->read_stats(
        dpp, y, index, RGW_NO_SHARD, &bucket_ver, &master_ver, stats,
        &max_marker);
    if (ret < 0) {
      cerr << "error getting bucket stats bucket=" << bucket->get_name()
           << " ret=" << ret << std::endl;
      return ret;
    }
  }
  ...
  ::encode_json(
      "explicit_placement", bucket->get_key().explicit_placement, formatter);
  ...
  formatter->dump_string(
      "reshard_status", to_string(bucket_info.layout.resharding));
  ...
  if (dump_restore_stats) {
    ret = bucket_restore_stats(
        driver, tenant_name, bucket_name, formatter, dpp, y);
    if (ret < 0) {
      return ret;
    }
  }
  ...
  for (const auto& user_id : user_ids) {
    ...
    do {
      ret = driver->list_buckets(
          dpp, user, user.tenant, listing.next_marker, string(), max_entries,
          false, listing, y);
      if (ret < 0)
        return ret;
      ...
      for (const auto& ent : listing.buckets) {
        ...
        uint64_t objs_per_shard = (num_shards) ? num_objects / num_shards
                                               : num_objects;
        {
          bool warn;
          stringstream ss;
    ...
    RGWFormatterFlusher& flusher)
{
  ...
  do {
    ...
    int ret = driver->list_buckets(
        dpp, owner, tenant, listing.next_marker, empty_end_marker, max_items,
        no_need_stats, listing, y);
    if (ret < 0) {
      return ret;
    }
    ...
    for (const auto& ent : listing.buckets) {
      if (show_stats) {
        bucket_stats(
            driver, site, tenant, ent.bucket.name, false, formatter, dpp, y);
      } else {
    ...
    const DoutPrefixProvider* dpp)
{
  ...
  if (!bucket_name.empty()) {
    ret = bucket_stats(
        driver, site, user_id.tenant, bucket_name, op_state.restore_stats,
        formatter, dpp, y);
    if (ret < 0) {
      return ret;
    }
    ...
    if (!info.account_id.empty()) {
      ldpp_dout(dpp, 1) << "Listing buckets in user account " << info.account_id
                        << dendl;
      ret = list_owner_bucket_info(
          dpp, y, driver, site, info.account_id, uid.tenant, op_state.marker,
          op_state.max_entries, show_stats, flusher);
    } else {
      ret = list_owner_bucket_info(
          dpp, y, driver, site, uid, uid.tenant, op_state.marker,
          op_state.max_entries, show_stats, flusher);
    }
    if (ret < 0) {
      return ret;
    ...
    ret = list_owner_bucket_info(
        dpp, y, driver, site, account_id, info.tenant, op_state.marker,
        op_state.max_entries, show_stats, flusher);
    if (ret < 0) {
      return ret;
    }
    ...
    while (ret == 0 && truncated) {
      ...
      ret = driver->meta_list_keys_next(
          dpp, handle, max_keys, buckets, &truncated);
...
inline auto
split_tenant(const std::string& bucket_name)
{
    ...
    optional_yield y)
{
  ...
  for (const auto& bucket_instance : lst) {
    ...
    rgw_bucket_parse_bucket_key(
        driver->ctx(), bucket_instance, &rbucket, nullptr);
  ...
  int r = driver->load_bucket(dpp, rgw_bucket(tenant, bname), &cur_bucket, y);
  if (r < 0) {
    if (r == -ENOENT) {
      // bucket doesn't exist, everything is stale then
      stale_instances.insert(
          std::end(stale_instances),
      ...
      ldpp_dout(dpp, -1) << "error: reading bucket info for bucket: " << bname
                         << cpp_strerror(-r) << dendl;
    }
    return;
  }
  ...
  other_instances.erase(
      std::remove_if(
          other_instances.begin(), other_instances.end(),
          [&cur_bucket_info](const RGWBucketInfo& b) {
            return (
                b.bucket.bucket_id == cur_bucket_info.bucket.bucket_id ||
  ...
  // with these
  {
    ...
    if (r < 0) {
      ...
      ldpp_dout(dpp, 5) << __func__
                        << "failed to take reshard lock; reshard underway likey"
                        << dendl;
      return;
    }
...
static int
process_stale_instances(
    rgw::sal::Driver* driver,
    RGWBucketAdminOpState& op_state,
    RGWFormatterFlusher& flusher,
    const DoutPrefixProvider* dpp,
    std::function<void(const bucket_instance_ls&, Formatter*, rgw::sal::Driver*)>
        process_f,
    optional_yield y)
{
  std::string marker;
  ...
  do {
    ...
    ret = driver->meta_list_keys_next(
        dpp, handle, default_max_keys, keys, &truncated);
    if (ret < 0 && ret != -ENOENT) {
      cerr << "ERROR: lists_keys_next(): " << cpp_strerror(-ret) << std::endl;
      return ret;
    }
...
int
RGWBucketAdminOp::list_stale_instances(
    rgw::sal::Driver* driver,
    RGWBucketAdminOpState& op_state,
    RGWFormatterFlusher& flusher,
    const DoutPrefixProvider* dpp,
    optional_yield y)
{
  auto process_f = [](const bucket_instance_ls& lst, Formatter* formatter,
                      rgw::sal::Driver*) {
...
int
RGWBucketAdminOp::clear_stale_instances(
    rgw::sal::Driver* driver,
    RGWBucketAdminOpState& op_state,
    RGWFormatterFlusher& flusher,
    const DoutPrefixProvider* dpp,
...
{
  auto process_f = [dpp,
                    y](const bucket_instance_ls& lst, Formatter* formatter,
                       rgw::sal::Driver* driver) {
    for (const auto& binfo : lst) {
    ...
    optional_yield y)
{
  ...
  return rgw::lc::fix_lc_shard_entry(
      dpp, driver, driver->get_rgwlc()->get_lc(), bucket.get());
}
    ...
    int status)
{
  ...
  std::string entry = tenant_name.empty() ? bucket_name
                                          : tenant_name + "/" + bucket_name;
...
int
RGWBucketAdminOp::fix_lc_shards(
    rgw::sal::Driver* driver,
    RGWBucketAdminOpState& op_state,
    RGWFormatterFlusher& flusher,
    const DoutPrefixProvider* dpp,
    optional_yield y)
{
  ...
  if (const std::string& bucket_name = op_state.get_bucket_name();
    ...
    const rgw_user user_id = op_state.get_user_id();
    process_single_lc_entry(
        driver, formatter, user_id.tenant, bucket_name, dpp, y);
    formatter->flush(cout);
  } else {
...

    {
      ...
      do {
        ...
        ret = driver->meta_list_keys_next(
            dpp, handle, default_max_keys, keys, &truncated);
        if (ret < 0 && ret != -ENOENT) {
          std::cerr << "ERROR: lists_keys_next(): " << cpp_strerror(-ret)
                    << std::endl;
          return ret;
        }
        if (ret != -ENOENT) {
          for (const auto& key : keys) {
            ...
            process_single_lc_entry(
                driver, formatter, tenant_name, bucket_name, dpp, y);
          }
        }
    ...
    optional_yield y)
{
  ...
  int ret = rgw_object_get_attr(
      dpp, driver, obj.get(), RGW_ATTR_DELETE_AT, delete_at_bl, y);
  if (ret < 0) {
    ...
    optional_yield y)
{
  ...
  do {
    ...
    if (ret < 0) {
      ldpp_dout(dpp, -1) << "ERROR failed to list objects in the bucket"
                         << dendl;
      return ret;
    }
...
int
RGWBucketAdminOp::fix_obj_expiry(
    rgw::sal::Driver* driver,
    RGWBucketAdminOpState& op_state,
    RGWFormatterFlusher& flusher,
    const DoutPrefixProvider* dpp,
...
void
RGWBucketCompleteInfo::dump(Formatter* f) const
{
...
void
RGWBucketCompleteInfo::decode_json(JSONObj* obj)
{
    ...
    const DoutPrefixProvider* dpp)
{
  ...
  int ret = svc_bucket->read_bucket_entrypoint_info(
      entry, &be, &ot, &mtime, &attrs, y, dpp);
  if (ret < 0) {
    return ret;
  }

  *obj = new RGWBucketEntryMetadataObject(
      be, ot.read_version, mtime, std::move(attrs));
  return 0;
}

int
RGWBucketMetadataHandler::put(
    std::string& entry,
    RGWMetadataObject* obj,
    RGWObjVersionTracker& objv_tracker,
    optional_yield y,
    ...
    bool from_remote_zone)
{
  ...
  ret = svc_bucket->store_bucket_entrypoint_info(
      entry, new_be, false, obj->get_mtime(), obj->get_pattrs(), &objv_tracker,
      y, dpp);
  if (ret < 0) {
    return ret;
  }
  ...
  if (old_be && (old_be->owner != new_be.owner || // owner changed
    ...
    int ret = ctl_bucket->unlink_bucket(
        rados, old_be->owner, old_be->bucket, y, dpp, update_entrypoint);
    if (ret < 0) {
      return ret;
    }
  }
  ...
  if (new_be.linked && (!old_be || !old_be->linked || // linked -> true
    ...
    int ret = ctl_bucket->link_bucket(
        rados, new_be.owner, new_be.bucket, new_be.creation_time, y, dpp,
        update_entrypoint);
    if (ret < 0) {
      return ret;
    }
...
int
RGWBucketMetadataHandler::remove(
    std::string& entry,
    RGWObjVersionTracker& objv_tracker,
    optional_yield y,
    const DoutPrefixProvider* dpp)
{
  RGWBucketEntryPoint be;
  int ret = svc_bucket->read_bucket_entrypoint_info(
      entry, &be, &objv_tracker, nullptr, nullptr, y, dpp);
  if (ret < 0) {
    return ret;
  }
  ...
  if (ret < 0) {
    ldpp_dout(dpp, -1) << "could not unlink bucket=" << entry
                       << " owner=" << be.owner << dendl;
  }
...
int
RGWBucketMetadataHandler::mutate(
    const std::string& entry,
    const ceph::real_time& mtime,
    RGWObjVersionTracker* objv_tracker,
    optional_yield y,
...
int
RGWBucketMetadataHandler::list_keys_init(
    const DoutPrefixProvider* dpp,
    const std::string& marker,
    void** phandle)
{
...
int
RGWBucketMetadataHandler::list_keys_next(
    const DoutPrefixProvider* dpp,
    void* handle,
    int max,
    std::list<std::string>& keys,
...
void
RGWBucketMetadataHandler::list_keys_complete(void* handle)
{
  delete static_cast<RGWMetadataLister*>(handle);
}

std::string
RGWBucketMetadataHandler::get_marker(void* handle)
{
  auto lister = static_cast<RGWMetadataLister*>(handle);
...
static void
get_md5_digest(const RGWBucketEntryPoint* be, string& md5_digest)
{
...
struct archive_meta_info {
  ...
  bool
  from_attrs(CephContext* cct, map<string, bufferlist>& attrs)
  {
  ...
  void
  store_in_attrs(map<string, bufferlist>& attrs) const
  {
...
public:
      ...
      const DoutPrefixProvider* dpp) override
  {
    ...
    ldpp_dout(dpp, 5)
        << "SKIP: bucket removal is not allowed on archive zone: bucket:"
        << entry << " ... proceeding to rename" << dendl;
    ...
    map<string, bufferlist> attrs;
    int ret = svc_bucket->read_bucket_entrypoint_info(
        entry, &be, &objv_tracker, &mtime, &attrs, y, dpp);
    if (ret < 0) {
      return ret;
    }
    ...
    ret = ctl_bucket->read_bucket_instance_info(
        be.bucket, &old_bi, y, dpp,
    ...
    ret = ctl_bucket->store_bucket_instance_info(
        new_be.bucket, new_bi, y, dpp,
    ...
    if (ret < 0) {
      ldpp_dout(dpp, 0)
          << "ERROR: failed to put new bucket instance info for bucket="
          << new_bi.bucket << " ret=" << ret << dendl;
      return ret;
    }
    ...
    ret = svc_bucket->store_bucket_entrypoint_info(
        RGWSI_Bucket::get_entrypoint_meta_key(new_be.bucket), new_be, true,
        mtime, &attrs, nullptr, y, dpp);
    if (ret < 0) {
      ldpp_dout(dpp, 0)
          << "ERROR: failed to put new bucket entrypoint for bucket="
          << new_be.bucket << " ret=" << ret << dendl;
      return ret;
    }

    /* link new bucket */

    ret = ctl_bucket->link_bucket(
        rados, new_be.owner, new_be.bucket, new_be.creation_time, y, dpp, false);
    if (ret < 0) {
      ldpp_dout(dpp, 0) << "ERROR: failed to link new bucket for bucket="
                        << new_be.bucket << " ret=" << ret << dendl;
      return ret;
    }
    ...
    ret =
        ctl_bucket->unlink_bucket(rados, be.owner, entry_bucket, y, dpp, false);
    if (ret < 0) {
      ldpp_dout(dpp, -1) << "could not unlink bucket=" << entry
                         << " owner=" << be.owner << dendl;
    }
    ...
    ret = svc_bucket->remove_bucket_entrypoint_info(
        RGWSI_Bucket::get_entrypoint_meta_key(be.bucket), &objv_tracker, y, dpp);
    if (ret < 0) {
      ldpp_dout(dpp, 0)
          << "ERROR: failed to put new bucket entrypoint for bucket="
          << new_be.bucket << " ret=" << ret << dendl;
      ...
      bool from_remote_zone) override
  {
    ...
    return RGWBucketMetadataHandler::put(
        entry, obj, objv_tracker, y, dpp, type, from_remote_zone);
  }
};
...
public:
  RGWBucketInstanceMetadataHandler(
    ...
    driver(driver),
    svc_zone(svc_zone),
    svc_bucket(svc_bucket),
    svc_bi(svc_bi),
    svc_datalog(svc_datalog)
  {}
...
int
RGWBucketInstanceMetadataHandler::get(
    std::string& entry,
    RGWMetadataObject** obj,
    optional_yield y,
    const DoutPrefixProvider* dpp)
{
  ...
  int ret = svc_bucket->read_bucket_instance_info(
      entry, &bci.info, &mtime, &bci.attrs, y, dpp);
  if (ret < 0) {
    return ret;
  }

  *obj = new RGWBucketInstanceMetadataObject(
      bci, bci.info.objv_tracker.read_version, mtime);
  return 0;
}

int
RGWBucketInstanceMetadataHandler::put(
    std::string& entry,
    RGWMetadataObject* obj,
    RGWObjVersionTracker& objv_tracker,
    optional_yield y,
    ...
    bool from_remote_zone)
{
  ...
  int ret = svc_bucket->read_bucket_instance_info(
      entry, &old->info, nullptr, &old->attrs, y, dpp);
  if (ret == -ENOENT) {
    old = std::nullopt;
  } else if (ret < 0) {
    return ret;
  ...
  ret = svc_bucket->store_bucket_instance_info(
      entry, bci.info, old_info, false, mtime, &bci.attrs, y, dpp);
  if (ret < 0) {
    return ret;
  }
    ...
    bool from_remote_zone)
{
  if (from_remote_zone) {
    ...
    if (old_bci) {
      ...
      init_default_bucket_layout(
          dpp->get_cct(), bci.info.layout, svc_zone->get_zone(), std::nullopt,
          std::nullopt);
    }
  }
  ...
  if (!old_bci || old_bci->info.bucket.bucket_id != bci.info.bucket.bucket_id) {
    ...
    if (svc_zone->sync_module_supports_writes()) {
      int ret = svc_zone->select_bucket_location_by_rule(
          dpp, bci.info.placement_rule, &rule_info, y);
      if (ret < 0) {
        ldpp_dout(dpp, 0) << "ERROR: select_bucket_placement() returned " << ret
                          << dendl;
        return ret;
      }
    }
    ...
    if (bci.info.bucket_deleted() &&
        log.layout.type != rgw::BucketLogType::Deleted) {
      const auto index_log = bci.info.layout.logs.back();
      const int shards_num = rgw::num_shards(index_log.layout.in_index);
      bci.info.layout.logs.push_back(
          {log.gen + 1, {rgw::BucketLogType::Deleted}});
      ldpp_dout(dpp, 10) << "store log layout type: "
                         << bci.info.layout.logs.back().layout.type << dendl;
  ...
  if (driver->get_zone()->get_tier_type() == "archive") {
    bci.info.flags = (bci.info.flags & ~BUCKET_VERSIONS_SUSPENDED) |
                     BUCKET_VERSIONED;
  }
...
int
RGWBucketInstanceMetadataHandler::put_post(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    const RGWBucketCompleteInfo& bci,
    const std::optional<RGWBucketCompleteInfo>& old_bci,
    RGWObjVersionTracker& objv_tracker)
{
  ...
  /* update lc list on changes to lifecyle policy */
  {
    ...
    if (lc_it != bci.attrs.end()) {
      ldpp_dout(dpp, 20) << "set lc config for " << bci.info.bucket.name
                         << dendl;
      ...
      if (lc_it != old_bci->attrs.end()) {
        ldpp_dout(dpp, 20) << "remove lc config for "
                           << old_bci->info.bucket.name << dendl;
...
int
RGWBucketInstanceMetadataHandler::remove(
    std::string& entry,
    RGWObjVersionTracker& objv_tracker,
    optional_yield y,
    const DoutPrefixProvider* dpp)
{
  RGWBucketCompleteInfo bci;
  int ret = svc_bucket->read_bucket_instance_info(
      entry, &bci.info, nullptr, &bci.attrs, y, dpp);
  if (ret == -ENOENT) {
    return 0;
  }
  if (ret < 0) {
  ...
  std::ignore =
      update_bucket_topic_mappings(dpp, &bci, /*current_bci=*/nullptr, driver);
  return 0;
}

int
RGWBucketInstanceMetadataHandler::mutate(
    const std::string& entry,
    const ceph::real_time& mtime,
    RGWObjVersionTracker* objv_tracker,
    optional_yield y,
...
int
RGWBucketInstanceMetadataHandler::list_keys_init(
    const DoutPrefixProvider* dpp,
    const std::string& marker,
    void** phandle)
{
...
int
RGWBucketInstanceMetadataHandler::list_keys_next(
    const DoutPrefixProvider* dpp,
    void* handle,
    int max,
    std::list<std::string>& keys,
...
void
RGWBucketInstanceMetadataHandler::list_keys_complete(void* handle)
{
  delete static_cast<RGWMetadataLister*>(handle);
}

std::string
RGWBucketInstanceMetadataHandler::get_marker(void* handle)
{
  auto lister = static_cast<RGWMetadataLister*>(handle);
  return lister->get_marker();
}

class RGWArchiveBucketInstanceMetadataHandler
...
public:
  ...
  remove(
      ...
      const DoutPrefixProvider* dpp) override
  {
    ldpp_dout(dpp, 0) << "SKIP: bucket instance removal is not allowed on "
                         "archive zone: bucket.instance:"
                      << entry << dendl;
    return 0;
  }
};

RGWBucketCtl::RGWBucketCtl(
    RGWSI_Zone* zone_svc,
    ...
    const DoutPrefixProvider* dpp)
{
  ...
  datalog->set_bucket_filter([this](
...
{
  return svc.bucket->read_bucket_entrypoint_info(
      RGWSI_Bucket::get_entrypoint_meta_key(bucket), info, params.objv_tracker,
      params.mtime, params.attrs, y, dpp, params.cache_info,
      params.refresh_version);
}

int
RGWBucketCtl::store_bucket_entrypoint_info(
    const rgw_bucket& bucket,
    RGWBucketEntryPoint& info,
    optional_yield y,
    const DoutPrefixProvider* dpp,
...
{
  return svc.bucket->store_bucket_entrypoint_info(
      RGWSI_Bucket::get_entrypoint_meta_key(bucket), info, params.exclusive,
      params.mtime, params.attrs, params.objv_tracker, y, dpp);
}
    ...
    const Bucket::RemoveParams& params)
{
  return svc.bucket->remove_bucket_entrypoint_info(
      RGWSI_Bucket::get_entrypoint_meta_key(bucket), params.objv_tracker, y,
      dpp);
}
...
{
  int ret = svc.bucket->read_bucket_instance_info(
      RGWSI_Bucket::get_bi_meta_key(bucket), info, params.mtime, params.attrs,
      y, dpp, params.cache_info, params.refresh_version);
  if (ret < 0) {
    return ret;
  }
    ...
    RGWObjVersionTracker* ep_objv_tracker)
{
  ...
  if (b->bucket_id.empty()) {
    ...
    int r = read_bucket_entrypoint_info(
        *b, &(*ep), y, dpp,
        RGWBucketCtl::Bucket::GetParams().set_objv_tracker(ep_objv_tracker));
    if (r < 0) {
      return r;
    }
  ...
  int ret = svc.bucket->read_bucket_instance_info(
      RGWSI_Bucket::get_bi_meta_key(*b), info, params.mtime, params.attrs, y,
      dpp, params.cache_info, params.refresh_version);
  if (ret < 0) {
    return ret;
  }
    ...
    const BucketInstance::PutParams& params)
{
  ...
  return svc.bucket->store_bucket_instance_info(
      RGWSI_Bucket::get_bi_meta_key(bucket), info, params.orig_info,
      params.exclusive, params.mtime, params.attrs, y, dpp);
}

int
RGWBucketCtl::remove_bucket_instance_info(
    const rgw_bucket& bucket,
    RGWBucketInfo& info,
    optional_yield y,
    const DoutPrefixProvider* dpp,
    const BucketInstance::RemoveParams& params)
{
  ...
  return svc.bucket->remove_bucket_instance_info(
      RGWSI_Bucket::get_bi_meta_key(bucket), info, &info.objv_tracker, y, dpp);
}
    ...
    const DoutPrefixProvider* dpp)
{
  ...
  int ret = svc.bucket->store_bucket_instance_info(
      RGWSI_Bucket::get_bi_meta_key(info.bucket), info, orig_info, exclusive,
      mtime, pattrs, y, dpp);
  if (ret < 0) {
    return ret;
  }
  ...
  return svc.bucket->store_bucket_entrypoint_info(
      RGWSI_Bucket::get_entrypoint_meta_key(info.bucket), entry_point,
      exclusive, mtime, pattrs, &ot, y, dpp);
}
    ...
    const DoutPrefixProvider* dpp)
{
  ...
  ldpp_dout(dpp, 10) << "RGWRados::convert_old_bucket_info(): bucket=" << bucket
                     << dendl;

  int ret = svc.bucket->read_bucket_entrypoint_info(
      RGWSI_Bucket::get_entrypoint_meta_key(bucket), &entry_point, &ot,
      &ep_mtime, &attrs, y, dpp);
  if (ret < 0) {
    ldpp_dout(dpp, 0) << "ERROR: get_bucket_entrypoint_info() returned " << ret
                      << " bucket=" << bucket << dendl;
    return ret;
  }
  ...
  ret = do_store_linked_bucket_info(
      info, nullptr, false, ep_mtime, &ot.write_version, &attrs, true, y, dpp);
  if (ret < 0) {
    ldpp_dout(dpp, 0) << "ERROR: failed to put_linked_bucket_info(): " << ret
                      << dendl;
    return ret;
  }
    ...
    const DoutPrefixProvider* dpp)
{
  ...
  if (!bucket_info.has_instance_obj) {
    ...
    if (ret < 0) {
      ldpp_dout(dpp, 0) << "ERROR: failed converting old bucket info: " << ret
                        << dendl;
      return ret;
    }
  }

  return store_bucket_instance_info(
      bucket, bucket_info, y, dpp,
      BucketInstance::PutParams()
          .set_attrs(&attrs)
    ...
    const rgw_owner& owner)
{
  return std::visit(
      fu2::overload(
          [&](const rgw_user& uid) { return svc_user->get_buckets_obj(uid); },
    ...
    rgw_ep_info* pinfo)
{
  ...
  if (update_entrypoint) {
    ...
    if (pinfo) {
      ...
      ret = svc.bucket->read_bucket_entrypoint_info(
          meta_key, &ep, &rot, nullptr, &attrs, y, dpp);
      if (ret < 0 && ret != -ENOENT) {
  ...
  ret =
      rgwrados::buckets::add(dpp, y, rados, buckets_obj, bucket, creation_time);
  if (ret < 0) {
    ldpp_dout(dpp, 0) << "ERROR: error adding bucket to owner directory:"
                      << " owner=" << owner << " bucket=" << bucket
    ...
    bool update_entrypoint)
{
  ...
  ret = svc.bucket->read_bucket_entrypoint_info(
      meta_key, &ep, &ot, nullptr, &attrs, y, dpp);
  if (ret == -ENOENT)
    return 0;
  if (ret < 0)
    return ret;
  ...
  if (ep.owner != owner) {
    ldpp_dout(dpp, 0)
        << "bucket entry point owner mismatch, can't unlink bucket: "
        << ep.owner << " != " << owner << dendl;
    return -EINVAL;
  }

  ep.linked = false;
  return svc.bucket->store_bucket_entrypoint_info(
      meta_key, ep, false, real_time(), &attrs, &ot, y, dpp);
}

int
RGWBucketCtl::read_bucket_stats(
    const rgw_bucket& bucket,
    RGWBucketEnt* result,
    optional_yield y,
    const DoutPrefixProvider* dpp)
...
int
RGWBucketCtl::read_buckets_stats(
    ...
    RGWBucketEnt* pent)
{
  ...
  if (r < 0) {
    ldpp_dout(dpp, 20) << __func__ << "(): failed to read bucket stats (r=" << r
                       << ")" << dendl;
    return r;
  }

  // flush stats to the user/account owner object
  const rgw_raw_obj& obj = std::visit(
      fu2::overload(
          [&](const rgw_user& user) { return svc.user->get_buckets_obj(user); },
...
int
RGWBucketCtl::get_sync_policy_handler(
    ...
    const DoutPrefixProvider* dpp)
{
  ...
  if (r < 0) {
    ldpp_dout(dpp, 20) << __func__
                       << "(): failed to get policy handler for bucket="
                       << bucket << " (r=" << r << ")" << dendl;
    return r;
  }
  return 0;
...
int
RGWBucketCtl::bucket_exports_data(
    const rgw_bucket& bucket,
    optional_yield y,
    const DoutPrefixProvider* dpp)
{
...
int
RGWBucketCtl::bucket_imports_data(
    const rgw_bucket& bucket,
    optional_yield y,
    const DoutPrefixProvider* dpp)
{
...
auto
create_bucket_metadata_handler(
    librados::Rados& rados,
    RGWSI_Bucket* svc_bucket,
    RGWBucketCtl* ctl_bucket) -> std::unique_ptr<RGWMetadataHandler>
{
...
auto
create_bucket_instance_metadata_handler(
    rgw::sal::Driver* driver,
    RGWSI_Zone* svc_zone,
    RGWSI_Bucket* svc_bucket,
    RGWSI_BucketIndex* svc_bi,
    RGWDataChangesLog* svc_datalog) -> std::unique_ptr<RGWMetadataHandler>
{
...
auto
create_archive_bucket_metadata_handler(
    librados::Rados& rados,
    RGWSI_Bucket* svc_bucket,
    RGWBucketCtl* ctl_bucket) -> std::unique_ptr<RGWMetadataHandler>
{
...
auto
create_archive_bucket_instance_metadata_handler(
    rgw::sal::Driver* driver,
    RGWSI_Zone* svc_zone,
    RGWSI_Bucket* svc_bucket,
    RGWSI_BucketIndex* svc_bi,
...
list<RGWBucketEntryPoint>
RGWBucketEntryPoint::generate_test_instances()
{
  list<RGWBucketEntryPoint> o;
  RGWBucketEntryPoint bp;
  init_bucket(
      &bp.bucket, "tenant", "bucket", "pool", ".index.pool", "marker", "10");
  bp.owner = "owner";
  bp.creation_time =
...
void
RGWBucketEntryPoint::dump(Formatter* f) const
{
...
void
RGWBucketEntryPoint::decode_json(JSONObj* obj)
{
