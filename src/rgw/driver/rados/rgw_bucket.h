...
extern void rgw_get_buckets_obj(
...
void init_default_bucket_layout(
    CephContext* cct,
...
public:
   ...
   */
  std::map<std::string, RGWBucketEnt>&
  get_buckets()
  {
    return buckets;
  }
...
auto create_bucket_metadata_handler(
...
auto create_bucket_instance_metadata_handler(
...
auto create_archive_bucket_metadata_handler(
    librados::Rados& rados,
    RGWSI_Bucket* svc_bucket,
    RGWBucketCtl* ctl_bucket) -> std::unique_ptr<RGWMetadataHandler>;

// archive bucket instance metadata handler factory
auto create_archive_bucket_instance_metadata_handler(
    rgw::sal::Driver* driver,
    RGWSI_Zone* svc_zone,
    RGWSI_Bucket* svc_bucket,
    RGWSI_BucketIndex* svc_bi,
...
extern int rgw_object_get_attr(
    rgw::sal::Driver* driver,
    rgw::sal::Object* obj,
...
void check_bad_owner_bucket_mapping(
    rgw::sal::Driver* driver,
...
struct RGWBucketAdminOpState {
  ...
  void
  set_fetch_stats(bool value)
  {
    stat_buckets = value;
  }

  void
  set_restore_stats(bool value)
  {
    restore_stats = value;
  }

  void
  set_check_objects(bool value)
  {
    check_objects = value;
  }

  void
  set_fix_index(bool value)
  {
    fix_index = value;
  }

  void
  set_delete_children(bool value)
  {
    delete_child_objects = value;
  }

  void
  set_hide_progress(bool value)
  {
    hide_progress = value;
  }

  void
  set_dump_keys(bool value)
  {
    dump_keys = value;
  }

  void
  set_max_aio(int value)
  {
    max_aio = value;
  }

  void
  set_min_age(ceph::timespan value)
  {
    min_age = value;
  }

  void
  set_user_id(const rgw_user& user_id)
  {
    if (!user_id.empty())
      uid = user_id;
  }
  ...
  void
  set_tenant(const std::string& tenant_str)
  {
    uid.tenant = tenant_str;
  }

  void
  set_bucket_name(const std::string& bucket_str)
  {
    bucket_name = bucket_str;
  }

  void
  set_object(std::string& object_str)
  {
    object_name = object_str;
  }

  void
  set_new_bucket_name(std::string& new_bucket_str)
  {
    new_bucket_name = new_bucket_str;
  }

  void
  set_quota(RGWQuotaInfo& value)
  {
    quota = value;
  }

  void
  set_bucket_ratelimit(RGWRateLimitInfo& value)
  {
    ratelimit_info = value;
  }

  void
  set_sync_bucket(bool value)
  {
    sync_bucket = value;
  }
  ...
  void
  set_bucket(std::unique_ptr<rgw::sal::Bucket> _bucket)
  {
  ...
  void
  set_bucket_id(const std::string& bi)
  {
    bucket_id = bi;
  }
...
public:
  RGWBucket() :
    driver(NULL), handle(NULL), failure(false)
  {}

  int init(
      rgw::sal::Driver* storage,
      RGWBucketAdminOpState& op_state,
      optional_yield y,
      const DoutPrefixProvider* dpp,
      std::string* err_msg = NULL);

  int check_bad_index_multipart(
...
public:
  ...
  static int check_index(
      ...
      optional_yield y,
      const DoutPrefixProvider* dpp);
  static int check_index_olh(
      rgw::sal::RadosStore* driver,
      RGWBucketAdminOpState& op_state,
      RGWFormatterFlusher& flusher,
      const DoutPrefixProvider* dpp);
  static int check_index_unlinked(
      rgw::sal::RadosStore* driver,
      RGWBucketAdminOpState& op_state,
      RGWFormatterFlusher& flusher,
      const DoutPrefixProvider* dpp);
  ...
  static int clear_stale_instances(
      ...
      const DoutPrefixProvider* dpp,
      optional_yield y);
  static int fix_lc_shards(
      rgw::sal::Driver* driver,
      RGWBucketAdminOpState& op_state,
      RGWFormatterFlusher& flusher,
      const DoutPrefixProvider* dpp,
      optional_yield y);
  static int fix_obj_expiry(
      rgw::sal::Driver* driver,
      RGWBucketAdminOpState& op_state,
      RGWFormatterFlusher& flusher,
      const DoutPrefixProvider* dpp,
...
public:
  ...
  struct Bucket {
    struct GetParams {
      ...
      GetParams&
      set_objv_tracker(RGWObjVersionTracker* _objv_tracker)
      {
      ...
      GetParams&
      set_mtime(ceph::real_time* _mtime)
      {
        mtime = _mtime;
        return *this;
      }
      ...
      GetParams&
      set_attrs(std::map<std::string, bufferlist>* _attrs)
      {
        attrs = _attrs;
        return *this;
      }
      ...
      GetParams&
      set_cache_info(rgw_cache_entry_info* _cache_info)
      {
      ...
      GetParams&
      set_refresh_version(const obj_version& _refresh_version)
      {
    ...
    struct PutParams {
      ...
      PutParams&
      set_attrs(const std::map<std::string, bufferlist>* _attrs)
      {
        attrs = _attrs;
        return *this;
      }
  ...
  struct BucketInstance {
    ...
    struct PutParams {
      std::optional<RGWBucketInfo*>
          orig_info; /* nullopt: orig_info was not fetched,
      ...
      PutParams&
      set_orig_info(RGWBucketInfo* pinfo)
      {
        orig_info = pinfo;
        return *this;
      }
  ...
  int remove_bucket_entrypoint_info(
      const rgw_bucket& bucket,
      optional_yield y,
      const DoutPrefixProvider* dpp,
  ...
  int remove_bucket_instance_info(
      const rgw_bucket& bucket,
      RGWBucketInfo& info,
      optional_yield y,
      const DoutPrefixProvider* dpp,
  ...
  int read_buckets_stats(
      std::vector<RGWBucketEnt>& buckets,
      optional_yield y,
      const DoutPrefixProvider* dpp);

  int read_bucket_stats(
      const rgw_bucket& bucket,
      RGWBucketEnt* result,
      optional_yield y,
      const DoutPrefixProvider* dpp);
  ...
  /* bucket sync */
  int get_sync_policy_handler(
      std::optional<rgw_zone_id> zone,
      std::optional<rgw_bucket> bucket,
      RGWBucketSyncPolicyHandlerRef* phandler,
      optional_yield y,
      const DoutPrefixProvider* dpp);
  int bucket_exports_data(
      const rgw_bucket& bucket,
      optional_yield y,
      const DoutPrefixProvider* dpp);
  int bucket_imports_data(
      const rgw_bucket& bucket,
      optional_yield y,
      const DoutPrefixProvider* dpp);

private:
  int convert_old_bucket_info(
      const rgw_bucket& bucket,
      optional_yield y,
      const DoutPrefixProvider* dpp);

  int do_store_linked_bucket_info(
      RGWBucketInfo& info,
