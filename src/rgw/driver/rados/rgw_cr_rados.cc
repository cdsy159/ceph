...
bool
RGWAsyncRadosProcessor::RGWWQ::_enqueue(RGWAsyncRadosRequest* req)
{
...
bool
RGWAsyncRadosProcessor::RGWWQ::_empty()
{
...
void
RGWAsyncRadosProcessor::RGWWQ::_process(
    RGWAsyncRadosRequest* req,
...
void
RGWAsyncRadosProcessor::RGWWQ::_dump_queue()
{
  ...
  dout(20) << "RGWWQ:" << dendl;
  for (iter = processor->m_req_queue.begin();
       iter != processor->m_req_queue.end(); ++iter) {
...
void
RGWAsyncRadosProcessor::start()
{
  m_tp.start();
}

void
RGWAsyncRadosProcessor::stop()
{
  going_down = true;
  m_tp.drain(&req_wq);
  m_tp.stop();
...
void
RGWAsyncRadosProcessor::handle_request(
...
void
RGWAsyncRadosProcessor::queue(RGWAsyncRadosRequest* req)
{
  if (is_going_down()) {
...
int
RGWAsyncGetSystemObj::_send_request(const DoutPrefixProvider* dpp)
{
...
RGWAsyncGetSystemObj::RGWAsyncGetSystemObj(
...
int
RGWSimpleRadosReadAttrsCR::send_request(const DoutPrefixProvider* dpp)
{
  int r = store->getRados()->get_raw_obj_ref(dpp, obj, &ref);
  if (r < 0) {
    ldpp_dout(dpp, -1) << "ERROR: failed to get ref for (" << obj
                       << ") ret=" << r << dendl;
    return r;
  }
...
int
RGWSimpleRadosReadAttrsCR::request_complete()
{
...
int
RGWAsyncPutSystemObj::_send_request(const DoutPrefixProvider* dpp)
{
...
RGWAsyncPutSystemObj::RGWAsyncPutSystemObj(
    const DoutPrefixProvider* _dpp,
    RGWCoroutine* caller,
    RGWAioCompletionNotifier* cn,
    RGWSI_SysObj* _svc,
...
int
RGWAsyncPutSystemObjAttrs::_send_request(const DoutPrefixProvider* dpp)
{
  auto sysobj = svc->get_obj(obj);
  return sysobj.wop()
      .set_objv_tracker(&objv_tracker)
...
RGWAsyncPutSystemObjAttrs::RGWAsyncPutSystemObjAttrs(
    const DoutPrefixProvider* _dpp,
    RGWCoroutine* caller,
    RGWAioCompletionNotifier* cn,
    RGWSI_SysObj* _svc,
...
int
RGWAsyncLockSystemObj::_send_request(const DoutPrefixProvider* dpp)
{
  rgw_rados_ref ref;
  int r = store->getRados()->get_raw_obj_ref(dpp, obj, &ref);
  if (r < 0) {
...
int
RGWAsyncUnlockSystemObj::_send_request(const DoutPrefixProvider* dpp)
{
  rgw_rados_ref ref;
  int r = store->getRados()->get_raw_obj_ref(dpp, obj, &ref);
  if (r < 0) {
...
RGWAsyncUnlockSystemObj::RGWAsyncUnlockSystemObj(
    RGWCoroutine* caller,
    RGWAioCompletionNotifier* cn,
    rgw::sal::RadosStore* _store,
    RGWObjVersionTracker* _objv_tracker,
...
int
RGWRadosSetOmapKeysCR::send_request(const DoutPrefixProvider* dpp)
{
  int r = store->getRados()->get_raw_obj_ref(dpp, obj, &ref);
  if (r < 0) {
    ldpp_dout(dpp, -1) << "ERROR: failed to get ref for (" << obj
...
int
RGWRadosSetOmapKeysCR::request_complete()
{
...
int
RGWRadosGetOmapKeysCR::send_request(const DoutPrefixProvider* dpp)
{
  ...
  librados::ObjectReadOperation op;
  op.omap_get_keys2(
      marker, max_entries, &result->entries, &result->more, nullptr);

  cn = stack->create_completion_notifier(result);
  return result->ref.ioctx.aio_operate(
      result->ref.obj.oid, cn->completion(), &op, NULL);
}

int
RGWRadosGetOmapKeysCR::request_complete()
{
  int r = cn->completion()->get_return_value();
...
RGWRadosGetOmapValsCR::RGWRadosGetOmapValsCR(
    rgw::sal::RadosStore* _store,
    const rgw_raw_obj& _obj,
    const string& _marker,
    int _max_entries,
...
int
RGWRadosGetOmapValsCR::send_request(const DoutPrefixProvider* dpp)
{
  int r = store->getRados()->get_raw_obj_ref(dpp, obj, &result->ref);
  if (r < 0) {
    ldpp_dout(dpp, -1) << "ERROR: failed to get ref for (" << obj
...
int
RGWRadosGetOmapValsCR::request_complete()
{
  int r = cn->completion()->get_return_value();
...
int
RGWRadosRemoveOmapKeysCR::send_request(const DoutPrefixProvider* dpp)
{
  int r = store->getRados()->get_raw_obj_ref(dpp, obj, &ref);
  if (r < 0) {
    ldpp_dout(dpp, -1) << "ERROR: failed to get ref for (" << obj
...
int
RGWRadosRemoveOmapKeysCR::request_complete()
{
  int r = cn->completion()->get_return_value();
...
RGWRadosRemoveCR::RGWRadosRemoveCR(
...
int
RGWRadosRemoveCR::send_request(const DoutPrefixProvider* dpp)
{
  auto rados = store->getRados()->get_rados_handle();
  int r = rgw_init_ioctx(dpp, rados, obj.pool, ioctx);
  if (r < 0) {
    lderr(cct) << "ERROR: failed to open pool (" << obj.pool.name
               << ") ret=" << r << dendl;
    return r;
  }
...
int
RGWRadosRemoveCR::request_complete()
{
  int r = cn->completion()->get_return_value();
...
RGWRadosRemoveOidCR::RGWRadosRemoveOidCR(
    rgw::sal::RadosStore* store,
...
int
RGWRadosRemoveOidCR::send_request(const DoutPrefixProvider* dpp)
{
  librados::ObjectWriteOperation op;
  if (objv_tracker) {
    objv_tracker->prepare_op_for_write(&op);
...
int
RGWRadosRemoveOidCR::request_complete()
{
  int r = cn->completion()->get_return_value();
...
RGWSimpleRadosLockCR::RGWSimpleRadosLockCR(
  ...
  req(nullptr)
{
  set_description() << "rados lock dest=" << obj << " lock=" << lock_name
                    << " cookie=" << cookie << " duration=" << duration;
}

void
RGWSimpleRadosLockCR::request_cleanup()
{
  if (req) {
    req->finish();
    req = NULL;
...
int
RGWSimpleRadosLockCR::send_request(const DoutPrefixProvider* dpp)
{
  set_status() << "sending request";
  req = new RGWAsyncLockSystemObj(
      this, stack->create_completion_notifier(), store, NULL, obj, lock_name,
      cookie, duration);
...
int
RGWSimpleRadosLockCR::request_complete()
{
...
RGWSimpleRadosUnlockCR::RGWSimpleRadosUnlockCR(
  ...
  req(NULL)
{
  set_description() << "rados unlock dest=" << obj << " lock=" << lock_name
                    << " cookie=" << cookie;
}

void
RGWSimpleRadosUnlockCR::request_cleanup()
{
  if (req) {
    req->finish();
    req = NULL;
...
int
RGWSimpleRadosUnlockCR::send_request(const DoutPrefixProvider* dpp)
{
  set_status() << "sending request";

  req = new RGWAsyncUnlockSystemObj(
      this, stack->create_completion_notifier(), store, NULL, obj, lock_name,
      cookie);
  async_rados->queue(req);
  return 0;
...
int
RGWSimpleRadosUnlockCR::request_complete()
{
  set_status() << "request complete; ret=" << req->get_ret_status();
  return req->get_ret_status();
}
...
int
RGWOmapAppend::operate(const DoutPrefixProvider* dpp)
{
  reenter(this)
  {
    for (;;) {
...
bool
RGWOmapAppend::append(const string& s)
{
  if (is_done()) {
    return false;
  }
...
int
RGWAsyncGetBucketInstanceInfo::_send_request(const DoutPrefixProvider* dpp)
{
  int r;
  if (!bucket.bucket_id.empty()) {
    r = store->getRados()->get_bucket_instance_info(
        bucket, bucket_info, nullptr, &attrs, null_yield, dpp);
  } else {
    r = store->ctl()->bucket->read_bucket_info(
        bucket, &bucket_info, null_yield, dpp,
...
int
RGWAsyncPutBucketInstanceInfo::_send_request(const DoutPrefixProvider* dpp)
{
  auto r = store->getRados()->put_bucket_instance_info(
      bucket_info, exclusive, mtime, attrs, dpp, null_yield);
  if (r < 0) {
...
int
RGWAsyncRemoveBucketInstanceInfo::_send_request(const DoutPrefixProvider* dpp)
{
  auto r = store->ctl()->bucket->remove_bucket_instance_info(
      bucket, bucket_info, null_yield, dpp);
  if (r < 0) {
...
int
RGWRadosBILogTrimCR::send_request(const DoutPrefixProvider* dpp)
{
...
int
RGWRadosBILogTrimCR::request_complete()
{
  int r = cn->completion()->get_return_value();
  set_status() << "request complete; ret=" << r;
  return r;
    ...
    const rgw::notify::EventTypeList& event_types)
{
  ...
  rgw::notify::reservation_t notify_res(
      dpp, store, obj, nullptr, bucket, user_id, bucket->get_tenant(), req_id,
      null_yield);
  int ret = rgw::notify::publish_reserve(
      dpp, *store->svc()->site, event_types, notify_res, &obj_tags);
  if (ret < 0) {
    ...
    ret = rgw::notify::publish_commit(
        obj, obj_size, ceph::real_clock::now(), etag, obj->get_instance(),
        notify_res, dpp);
    if (ret < 0) {
...
int
RGWAsyncFetchRemoteObj::_send_request(const DoutPrefixProvider* dpp)
{
  RGWObjectCtx obj_ctx(store);

  char buf[16];
  snprintf(
      buf, sizeof(buf), ".%lld", (long long)store->getRados()->instance_id());
  rgw::sal::Attrs attrs;
  ...
  int r = store->getRados()->fetch_remote_obj(
      ...
      NULL, /* req_info */
      source_zone, dest_obj.get_obj(), src_obj, dest_bucket_info, /* dest */
      nullptr, /* source */
      dest_placement_rule, nullptr, /* real_time* src_mtime, */
      ...
      RGWRados::ATTRSMOD_NONE, copy_if_newer, attrs, RGWObjCategory::Main,
      versioned_epoch, real_time(), /* delete_at */
      NULL, /* string *ptag, */
      &etag, /* string *petag, */
      NULL, /* void (*progress_cb)(off_t, void *), */
      NULL, /* void *progress_data*); */
      rctx, filter.get(), stat_follow_olh, stat_dest_obj, source_trace_entry,
      &zones_trace, &bytes_transferred, keep_tags);
...
int
RGWAsyncStatRemoteObj::_send_request(const DoutPrefixProvider* dpp)
{
  RGWObjectCtx obj_ctx(store);
  ...
  int r = store->getRados()->stat_remote_obj(
      dpp, obj_ctx, nullptr, /* user_id */
      nullptr, /* req_info */
      source_zone, src_obj, nullptr, /* source */
      ...
      pattrs, pheaders, nullptr, nullptr, /* string *ptag, */
...
int
RGWAsyncRemoveObj::_send_request(const DoutPrefixProvider* dpp)
{
  ...
  if (ret < 0) {
    ldpp_dout(dpp, 20) << __func__ << "(): load_obj_state() obj=" << obj
  ...
  if (!sync_pipe.info.handler.find_obj_params(
          obj->get_key(), obj_tags.get_tags(), &params)) {
  ...
  if (params.mode == rgw_sync_pipe_params::MODE_USER) {
    ...
    if (ret < 0) {
      ldpp_dout(dpp, 0) << "ERROR: " << __func__
                        << ": failed to init user perms for uid="
                        << *params.user << " ret=" << ret << dendl;
      return ret;
    }
    ...
    if (ret < 0) {
      ldpp_dout(dpp, 0) << "ERROR: " << __func__
                        << ": failed to init bucket perms for uid="
                        << *params.user << " bucket=" << bucket->get_key()
                        << " ret=" << ret << dendl;
      return ret;
    }

    if (!dest_bucket_perms.verify_bucket_permission(
            obj->get_key(), rgw::IAM::s3ReplicateDelete)) {
      ldpp_dout(dpp, 20) << "ERROR: " << __func__
  ...
  if (ret < 0) {
    ldpp_dout(dpp, 20) << __func__ << "(): delete_obj() obj=" << obj
                       << " returned ret=" << ret << dendl;
  } else {
    send_sync_notification(
        dpp, store, bucket.get(), obj.get(), obj->get_attrs(), obj->get_size(),
...
int
RGWContinuousLeaseCR::operate(const DoutPrefixProvider* dpp)
{
  if (aborted) {
  ...
  reenter(this)
  {
    ...
    while (!going_down) {
      ...
      yield call(new RGWSimpleRadosLockCR(
          async_rados, store, obj, lock_name, cookie, interval));
      if (latency) {
      ...
      if (current_time - last_renew_try_time > interval_tolerance) {
        ...
        ldout(store->ctx(), 1)
            << *this << ": WARNING: did not renew lock " << obj << ":"
            << lock_name << ": within 90\% of interval. "
            << (current_time - last_renew_try_time) << " > "
            << interval_tolerance << dendl;
      }
      last_renew_try_time = current_time;

      caller->set_sleeping(
          false); /* will only be relevant when we return, that's why we can do it early */
      if (retcode < 0) {
        set_locked(false);
        ldout(store->ctx(), 20)
            << *this << ": couldn't lock " << obj << ":" << lock_name
            << ": retcode=" << retcode << dendl;
        return set_state(RGWCoroutine_Error, retcode);
      }
      ldout(store->ctx(), 20) << *this << ": successfully locked " << obj << ":"
                              << lock_name << dendl;
      set_locked(true);
    ...
    current_time = ceph::coarse_mono_clock::now();
    yield call(
        new RGWSimpleRadosUnlockCR(async_rados, store, obj, lock_name, cookie));
    if (latency) {
      latency->add_latency(ceph::coarse_mono_clock::now() - current_time);
    }
    return set_state(RGWCoroutine_Done);
...
{
  stringstream& s = set_description();
  s << "timelog add entry oid=" << oid << "entry={id=" << entry.id
    << ", section=" << entry.section << ", name=" << entry.name << "}";
  entries.push_back(entry);
}

int
RGWRadosTimelogAddCR::send_request(const DoutPrefixProvider* dpp)
{
  set_status() << "sending request";

  cn = stack->create_completion_notifier();
  return store->svc()->cls->timelog.add(
      dpp, oid, entries, cn->completion(), true, null_yield);
}

int
RGWRadosTimelogAddCR::request_complete()
{
  int r = cn->completion()->get_return_value();
...
int
RGWRadosTimelogTrimCR::send_request(const DoutPrefixProvider* dpp)
{
  set_status() << "sending request";

  cn = stack->create_completion_notifier();
  return store->svc()->cls->timelog.trim(
      dpp, oid, start_time, end_time, from_marker, to_marker, cn->completion(),
      null_yield);
}
...
RGWSyncLogTrimCR::RGWSyncLogTrimCR(
    const DoutPrefixProvider* dpp,
    rgw::sal::RadosStore* store,
    const std::string& oid,
    const std::string& to_marker,
...
int
RGWSyncLogTrimCR::request_complete()
{
...
int
RGWAsyncStatObj::_send_request(const DoutPrefixProvider* dpp)
{
  rgw_raw_obj raw_obj;
  store->getRados()->obj_to_raw(bucket_info.placement_rule, obj, &raw_obj);
  return store->getRados()->raw_obj_stat(
      dpp, raw_obj, psize, pmtime, pepoch, nullptr, nullptr, objv_tracker,
      null_yield);
}
...
int
RGWStatObjCR::send_request(const DoutPrefixProvider* dpp)
{
  req = new RGWAsyncStatObj(
      dpp, this, stack->create_completion_notifier(), store, bucket_info, obj,
      psize, pmtime, pepoch, objv_tracker);
  async_rados->queue(req);
  return 0;
}

int
RGWStatObjCR::request_complete()
{
  return req->get_ret_status();
}

RGWRadosNotifyCR::RGWRadosNotifyCR(
    rgw::sal::RadosStore* store,
    const rgw_raw_obj& obj,
    bufferlist& request,
    uint64_t timeout_ms,
...
int
RGWRadosNotifyCR::send_request(const DoutPrefixProvider* dpp)
{
  int r = store->getRados()->get_raw_obj_ref(dpp, obj, &ref);
  if (r < 0) {
    ldpp_dout(dpp, -1) << "ERROR: failed to get ref for (" << obj
  ...
  cn = stack->create_completion_notifier();
  return ref.ioctx.aio_notify(
      ref.obj.oid, cn->completion(), request, timeout_ms, response);
}

int
RGWRadosNotifyCR::request_complete()
{
  int r = cn->completion()->get_return_value();
...
int
RGWDataPostNotifyCR::operate(const DoutPrefixProvider* dpp)
{
  reenter(this)
  {
    using PostNotify2 = RGWPostRESTResourceCR<
        bc::flat_map<int, bc::flat_set<rgw_data_notify_entry>>, int>;
    yield
    {
      rgw_http_param_pair pairs[] = {
          ...
          {NULL, NULL}};
      call(new PostNotify2(
          store->ctx(), conn, &http_manager, "/admin/log", pairs, shards,
          nullptr));
    }
    if (retcode == -ERR_METHOD_NOT_ALLOWED) {
      ...
      yield
      {
        ...
        call(new PostNotify1(
            store->ctx(), conn, &http_manager, "/admin/log", pairs, encoder,
            nullptr));
      }
    }
    if (retcode < 0) {
...
RGWStatRemoteBucketCR::RGWStatRemoteBucketCR(
    const DoutPrefixProvider* dpp,
...
int
RGWStatRemoteBucketCR::operate(const DoutPrefixProvider* dpp)
{
  reenter(this)
  {
    yield
    {
      ...
      for (auto& zid : zids) {
        ...
        if (ziter == zone_conn_map.end()) {
          ldpp_dout(dpp, 0)
              << "WARNING: no connection to zone " << ziter->first << dendl;
          continue;
        }
        ...
        rgw_http_param_pair pairs[] = {
            {"versions", NULL},          {"format", "json"},
            {"objs-container", "true"},  {"max-keys", "1"},
            {"allow-unordered", "true"}, {"key-marker", NULL},
            {"version-id-marker", NULL}, {NULL, NULL}};
        string p = string("/") + bucket.get_key(':', 0);
        spawn(
            new RGWReadRESTResourceCR<bucket_unordered_list_result>(
                store->ctx(), &*conn, &*http, p, pairs, &*result),
            false);
        ++result;
      }
    }
