...
int
D3nCacheAioWriteRequest::d3n_libaio_prepare_write_op(
    bufferlist& bl,
    unsigned int len,
    string digest_oid,
    string cache_location)
{
  ...
  lsubdout(g_ceph_context, rgw_datacache, 20)
      ...
      << "(): Write To Cache, location=" << location << dendl;
  cb = new struct aiocb;
  ...
  if (fd < 0) {
    ldout(cct, 0)
        << "ERROR: D3nCacheAioWriteRequest::create_io: open file failed, errno="
        << errno << ", location='" << location.c_str() << "'" << dendl;
    return r;
  }
  r = 0;
  ...
  if (!data) {
    ldout(cct, 0)
        << "ERROR: D3nCacheAioWriteRequest::create_io: memory allocation failed"
        << dendl;
    return -1;
  }
  cb->aio_buf = data;
...
{
  lsubdout(g_ceph_context, rgw_datacache, 5)
      << "D3nDataCache: " << __func__ << "()" << dendl;
}

void
D3nDataCache::init(CephContext* _cct)
{
  cct = _cct;
    ...
    if (efs::exists(cache_location)) {
      ...
      if (g_conf()->rgw_d3n_l1_evict_cache_on_start) {
        lsubdout(g_ceph_context, rgw, 5)
            << "D3nDataCache: init: evicting the persistent storage directory "
               "on start"
            << dendl;
      ...
      lsubdout(g_ceph_context, rgw, 5)
          << "D3nDataCache: init: creating the persistent storage directory on "
             "start"
          << dendl;
      efs::create_directories(cache_location);
      efs::permissions(
          cache_location, efs::perms::owner_all | efs::perms::group_all |
                              efs::perms::others_read);
    }
  } catch (const efs::filesystem_error& e) {
    lderr(g_ceph_context) << "D3nDataCache: init: ERROR initializing the cache "
                             "storage directory '"
                          << cache_location << "' : " << e.what() << dendl;
  }

  auto conf_eviction_policy =
      cct->_conf.get_val<std::string>("rgw_d3n_l1_eviction_policy");
    ...
    std::string digest_oid)
{
  ...
  lsubdout(g_ceph_context, rgw_datacache, 20)
      << "D3nDataCache: " << __func__ << "(): location=" << location << dendl;

  {
    ...
    if (cache_file == nullptr) {
      ldout(cct, 0)
          << "ERROR: D3nDataCache::fopen file has return error, errno=" << errno
          << dendl;
      return -errno;
    }
    ...
    if (nbytes != len) {
      ldout(cct, 0) << "ERROR: D3nDataCache::io_write: fwrite has returned "
                       "error: nbytes!=len, nbytes="
  ...
  if (r != 0) {
    ldout(cct, 0) << "ERROR: D3nDataCache::fclose file has return error, errno="
                  << errno << dendl;
    return -errno;
  }
  ...
  { // update cache_map entries for new chunk in cache
    ...
    chunk_info->size = len;
    d3n_cache_map.insert(
        pair<string, D3nChunkDataInfo*>(digest_oid, chunk_info));
  }
...
void
d3n_libaio_write_cb(sigval sigval)
{
  lsubdout(g_ceph_context, rgw_datacache, 30)
      << "D3nDataCache: " << __func__ << "()" << dendl;
  D3nCacheAioWriteRequest* c =
      static_cast<D3nCacheAioWriteRequest*>(sigval.sival_ptr);
  c->priv_data->d3n_libaio_write_completion_cb(c);
}

void
D3nDataCache::d3n_libaio_write_completion_cb(D3nCacheAioWriteRequest* c)
{
  D3nChunkDataInfo* chunk_info{nullptr};

  ldout(cct, 5) << "D3nDataCache: " << __func__ << "(): oid=" << c->oid
                << dendl;
...
int
D3nDataCache::d3n_libaio_create_write_request(
    bufferlist& bl,
    unsigned int len,
    std::string oid)
{
  lsubdout(g_ceph_context, rgw_datacache, 30)
      << "D3nDataCache: " << __func__ << "(): Write To Cache, oid=" << oid
      << ", len=" << len << dendl;
  ...
  if ((r = wr->d3n_libaio_prepare_write_op(bl, len, oid, cache_location)) < 0) {
    ldout(cct, 0) << "ERROR: D3nDataCache: " << __func__
                  << "() prepare libaio write op r=" << r << dendl;
    return r;
  }
...
void
D3nDataCache::put(bufferlist& bl, unsigned int len, std::string& oid)
{
  size_t sr = 0;
  uint64_t freed_size = 0, _free_data_cache_size = 0,
           _outstanding_write_size = 0;
  std::string digest_oid = D3nL1CacheRequest::generate_oid_digest(oid);
  ldout(cct, 10) << "D3nDataCache::" << __func__ << "(): oid=" << oid
                 << ", digest_oid=" << digest_oid << ", len=" << len << dendl;
  {
    const std::lock_guard l(d3n_cache_lock);
    std::unordered_map<string, D3nChunkDataInfo*>::iterator iter =
        d3n_cache_map.find(digest_oid);
    if (iter != d3n_cache_map.end()) {
      ldout(cct, 10) << "D3nDataCache::" << __func__
    ...
    if (it != d3n_outstanding_write_list.end()) {
      ldout(cct, 10)
          << "D3nDataCache: NOTE: data put in cache already issued, no rewrite"
          << dendl;
      return;
    }
  ...
  ldout(cct, 20) << "D3nDataCache: Before eviction _free_data_cache_size:"
                 << _free_data_cache_size
                 << ", _outstanding_write_size:" << _outstanding_write_size
                 << ", freed_size:" << freed_size << dendl;
  while (len > (_free_data_cache_size - _outstanding_write_size + freed_size)) {
    ...
    if (eviction_policy == _eviction_policy::LRU) {
      ...
      ldout(cct, 0) << "D3nDataCache: Warning: unknown cache eviction policy, "
                       "defaulting to lru eviction"
                    << dendl;
      sr = lru_eviction();
    }
    if (sr == 0) {
      ldout(cct, 2) << "D3nDataCache: Warning: eviction was not able to free "
                       "disk space, not writing to cache"
                    << dendl;
      d3n_outstanding_write_list.erase(digest_oid);
      return;
    }
    ldout(cct, 20) << "D3nDataCache: completed eviction of " << sr << " bytes"
                   << dendl;
    freed_size += sr;
  }
  int r = 0;
  ...
  if (r < 0) {
    const std::lock_guard l(d3n_cache_lock);
    d3n_outstanding_write_list.erase(digest_oid);
    ldout(cct, 1) << "D3nDataCache: create_aio_write_request fail, r=" << r
                  << dendl;
    return;
  }
...
bool
D3nDataCache::get(const string& oid, const off_t len)
{
  const std::lock_guard l(d3n_cache_lock);
  bool exist = false;
  std::string digest_oid = D3nL1CacheRequest::generate_oid_digest(oid);
  ...
  lsubdout(g_ceph_context, rgw_datacache, 20)
      << "D3nDataCache: " << __func__ << "(): oid=" << oid
      << ", digest_oid=" << digest_oid << ", location=" << location << dendl;
  std::unordered_map<string, D3nChunkDataInfo*>::iterator iter =
      d3n_cache_map.find(digest_oid);
  if (!(iter == d3n_cache_map.end())) {
    ...
    if (r != -1 &&
        st.st_size ==
            len) { // file exists and contains required data range length
      exist = true;
      /*LRU*/
...
D3nDataCache::random_eviction()
{
  ...
  string del_oid, location;
  {
    ...
    ldout(cct, 20) << "D3nDataCache: random_eviction: index:" << random_index
                   << ", free size: " << del_entry->size << dendl;
...
D3nDataCache::lru_eviction()
{
...

  {
    ...
    if (del_entry == nullptr) {
      ldout(cct, 2) << "D3nDataCache: lru_eviction: del_entry=null_ptr"
                    << dendl;
      return 0;
    }
...

  {
    ...
    ldout(cct, 20) << "D3nDataCache: lru_eviction: oid to remove: " << del_oid
                   << dendl;
    d3n_cache_map.erase(del_oid); // oid
  }
  freed_size = del_entry->size;
