...
struct D3nCacheAioWriteRequest {
  ...
  int d3n_libaio_prepare_write_op(
      bufferlist& bl,
      unsigned int len,
      std::string oid,
...
public:
  ...
  int d3n_libaio_create_write_request(
      bufferlist& bl,
      unsigned int len,
      std::string oid);
  ...
  void
  lru_insert_head(struct D3nChunkDataInfo* o)
  {
  ...
  void
  lru_insert_tail(struct D3nChunkDataInfo* o)
  {
    lsubdout(g_ceph_context, rgw_datacache, 30)
        << "D3nDataCache: " << __func__ << "()" << dendl;
    o->lru_next = nullptr;
  ...
  void
  lru_remove(struct D3nChunkDataInfo* o)
  {
    lsubdout(g_ceph_context, rgw_datacache, 30)
        << "D3nDataCache: " << __func__ << "()" << dendl;
    if (o->lru_next)
...
template <typename T>
int
D3nRGWDataCache<T>::get_obj_iterate_cb(
    const DoutPrefixProvider* dpp,
    const rgw_raw_obj& read_obj,
    off_t obj_ofs,
    off_t read_ofs,
...
{
  lsubdout(g_ceph_context, rgw_datacache, 30)
      << "D3nDataCache::" << __func__ << "(): is head object : " << is_head_obj
      << dendl;
  ...
  if (is_head_obj) {
    ...
    ldpp_dout(dpp, 20) << "D3nDataCache::" << __func__
                       << "(): oid=" << read_obj.oid << " obj-ofs=" << obj_ofs
    ...
    auto completed = d->aio->get(
        ref.obj, rgw::Aio::librados_op(ref.ioctx, std::move(op), d->yield),
        cost, id);
    ...
    const bool is_compressed =
        (astate->attrset.find(RGW_ATTR_COMPRESSION) != astate->attrset.end());
    const bool is_encrypted =
        (astate->attrset.find(RGW_ATTR_CRYPT_MODE) != astate->attrset.end());
    if (read_ofs != 0 || astate->size != astate->accounted_size ||
      ...
      lsubdout(g_ceph_context, rgw, 5)
          ...
          << "(): Note - bypassing datacache: oid=" << read_obj.oid
          << ", read_ofs!=0 = " << read_ofs << ", size=" << astate->size
          << " != accounted_size=" << astate->accounted_size
    ...
    if (d->rgwrados->d3n_data_cache->get(oid, len)) {
      // Read From Cache
      ldpp_dout(dpp, 20) << "D3nDataCache: " << __func__
                         << "(): READ FROM CACHE: oid=" << read_obj.oid
                         << ", obj-ofs=" << obj_ofs << ", read_ofs=" << read_ofs
                         << ", len=" << len << dendl;
      auto completed = d->aio->get(
          ref.obj,
          rgw::Aio::d3n_cache_op(
              dpp, d->yield, read_ofs, len,
              d->rgwrados->d3n_data_cache->cache_location),
          cost, id);
      r = d->flush(std::move(completed));
      if (r < 0) {
        lsubdout(g_ceph_context, rgw, 0)
            << "D3nDataCache: " << __func__
            << "(): Error: failed to drain/flush, r= " << r << dendl;
      }
      return r;
    } else {
      // Write To Cache
      ldpp_dout(dpp, 20) << "D3nDataCache: " << __func__
                         << "(): WRITE TO CACHE: oid=" << read_obj.oid
                         << ", obj-ofs=" << obj_ofs << ", read_ofs=" << read_ofs
                         << " len=" << len << dendl;
      auto completed = d->aio->get(
          ref.obj, rgw::Aio::librados_op(ref.ioctx, std::move(op), d->yield),
  ...
  lsubdout(g_ceph_context, rgw, 1)
      << "D3nDataCache: " << __func__
      << "(): Warning: Check head object cache handling flow, oid="
      << read_obj.oid << dendl;
