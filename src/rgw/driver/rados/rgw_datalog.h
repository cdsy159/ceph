...
inline std::ostream&
operator<<(std::ostream& m, const DataLogEntityType& t)
{
  switch (t) {
...
inline std::ostream&
operator<<(std::ostream& m, const rgw_data_change& c)
{
  return m << "[entity_type: " << c.entity_type << ", key: " << c.key
           << ", timestamp: " << c.timestamp << ", gen: " << c.gen << "]";
}
...
inline std::ostream&
operator<<(std::ostream& m, const rgw_data_change_log_entry& e)
{
  return m << "[log_id: " << e.log_id << ", log_timestamp: " << e.log_timestamp
           << ", entry: " << e.entry << "]";
}
...
struct RGWDataChangesLogMarker {
  ...
  operator bool() const { return (shard > 0 || !marker.empty()); }
...
struct rgw_data_notify_entry {
  ...
  bool
  operator<(const rgw_data_notify_entry& d) const
  {
    if (key < d.key) {
      return true;
    }
  ...
  friend std::ostream&
  operator<<(std::ostream& m, const rgw_data_notify_entry& e)
  {
...
class DataLogBackends final
  ...
  DataLogBackends(
    ...
    logback_generations(rados, oid, loc, std::move(get_oid), shards),
    datalog(datalog)
  {}
...
public:
  ...
  list(
      ...
      std::string marker);
  asio::awaitable<void> trim_entries(
      const DoutPrefixProvider* dpp,
      int shard_id,
  ...
  asio::awaitable<void> trim_generations(
      const DoutPrefixProvider* dpp,
...
inline bool
operator==(const BucketGen& l, const BucketGen& r)
{
...
inline std::ostream&
operator<<(std::ostream& m, const BucketGen& bg)
{
...
struct hash<BucketGen> {
  std::size_t
  operator()(const BucketGen& bg) const noexcept
  {
...
public:
  ...
  asio::awaitable<bool> establish_watch(
      const DoutPrefixProvider* dpp,
      std::string_view oid);
  asio::awaitable<void> process_notification(
      const DoutPrefixProvider* dpp,
      std::string_view oid);
  asio::awaitable<void> watch_loop();
  int choose_oid(const rgw_bucket_shard& bs);
  asio::awaitable<void> add_entry(
      const DoutPrefixProvider* dpp,
  ...
  asio::awaitable<RGWDataChangesLogInfo> get_info(
      const DoutPrefixProvider* dpp,
      int shard_id);
  asio::awaitable<void> trim_entries(
      const DoutPrefixProvider* dpp,
  ...
  asio::awaitable<void> trim_generations(
      const DoutPrefixProvider* dpp,
      std::optional<uint64_t>& through);
  asio::awaitable<void> change_format(
      const DoutPrefixProvider* dpp,
      log_type type);
  ...
  void
  set_observer(rgw::BucketChangeObserver* observer)
  {
    this->observer = observer;
  }

  void
  set_bucket_filter(decltype(bucket_filter)&& f)
  {
  ...
  asio::awaitable<bool> synthesize_entries(
      const DoutPrefixProvider* dpp,
      int index,
      const bc::flat_map<std::string, uint64_t>& semcount);
  asio::awaitable<bool> gather_working_sets(
      const DoutPrefixProvider* dpp,
      int index,
      bc::flat_map<std::string, uint64_t>& semcount);
  asio::awaitable<void> decrement_sems(
      int index,
  ...
  asio::awaitable<void> admin_sem_list(
      std::optional<int> req_shard,
      std::uint64_t max_entries,
      std::string marker,
      std::ostream& m,
      ceph::Formatter& formatter);
  asio::awaitable<void> admin_sem_reset(
      std::string_view marker,
      std::uint64_t count);
};
...
public:
  using entries =
      std::variant<std::vector<cls::log::entry>, std::deque<ceph::buffer::list>>;
  ...
  virtual void prepare(
      ...
      entries& out) = 0;
  virtual asio::awaitable<void> push(
      const DoutPrefixProvider* dpp,
      int index,
      entries&& items) = 0;
  virtual void push(
  ...
  virtual asio::awaitable<
      std::tuple<std::span<rgw_data_change_log_entry>, std::string>>
  list(
      const DoutPrefixProvider* dpp,
      int shard,
      std::span<rgw_data_change_log_entry> entries,
      std::string marker) = 0;
  virtual asio::awaitable<RGWDataChangesLogInfo> get_info(
      const DoutPrefixProvider* dpp,
      int index) = 0;
  virtual asio::awaitable<void> trim(
      const DoutPrefixProvider* dpp,
      int index,
