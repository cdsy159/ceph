    ...
    const RGWObjManifest& manifest)
{
  ...
  unsigned idx = 0;
  for (auto p = manifest.obj_begin(dpp); p != manifest.obj_end(dpp);
       ++p, ++idx) {
...
//---------------------------------------------------------------------------
void
Background::DedupWatcher::handle_notify(
    uint64_t notify_id,
    uint64_t cookie,
    uint64_t notifier_id,
    bufferlist& bl)
{
  ldpp_dout(parent->dpp, 10)
      << __func__ << "::notify_id=" << notify_id << "::cookie=" << cookie
      << "::notifier_id=" << notifier_id << dendl;
  if (parent->d_watch_handle != cookie) {
    ldpp_dout(parent->dpp, 1)
        << __func__ << "::ERR: wrong cookie=" << cookie
...
//---------------------------------------------------------------------------
void
Background::DedupWatcher::handle_error(uint64_t cookie, int err)
{
  if (parent->d_watch_handle != cookie) {
    ldpp_dout(parent->dpp, 1)
        << __func__ << "::ERR: wrong cookie=" << cookie
...
//---------------------------------------------------------------------------
void
encode(const control_t& ctl, ceph::bufferlist& bl)
{
  ENCODE_START(1, 1, bl);
...
//---------------------------------------------------------------------------
void
decode(control_t& ctl, ceph::bufferlist::const_iterator& bl)
{
  DECODE_START(1, bl);
  int32_t dedup_type;
...
//---------------------------------------------------------------------------
std::ostream&
operator<<(std::ostream& out, const control_t& ctl)
{
  out << ctl.dedup_type;
  if (ctl.started) {
    out << "::started";
    ...
    int64_t expected_pool_id)
{
  ...
  if (pool_id < 0) {
    ...
    if (err == ENOENT) {
      ldpp_dout(dpp, 10)
          << __func__
          << "::pool doesn't exist (probably was removed by other RGW)::"
          << dedup_pool.name << "::expected_pool_id=" << expected_pool_id
          << dendl;
    } else {
      ldpp_dout(dpp, 5) << __func__ << "::failed pool_lookup("
                        << dedup_pool.name << ") err=" << cpp_strerror(-err)
                        << dendl;
    }
    return err;
  }
  ...
  if (pool_id != expected_pool_id) {
    ldpp_dout(dpp, 5) << __func__
                      << "::ERR: pool_id was changed from: " << expected_pool_id
                      << " to: " << pool_id << " abort pool_delete() request!"
                      << dendl;
    ...
    const std::string& pool_name)
{
  ...
  if (ret != 0 && ret != -EEXIST) {
    ldpp_dout(dpp, 1) << __func__ << "::ERR: failed to create pool "
                      << pool_name << " with: " << cpp_strerror(-ret)
                      << ", ret=" << ret << dendl;
    return ret;
  }
  const rgw_pool& dedup_pool = store->svc()->zone->get_zone_params().dedup_pool;
    ...
    librados::IoCtx& ioctx)
{
  ...
  if (unlikely(ret < 0)) {
    ldpp_dout(dpp, 1) << __func__
                      << "::ERR: failed rgw_init_ioctx() ret=" << ret
  ...
  if (ret == 0) {
    ...
    ldpp_dout(dpp, 1) << __func__ << "::ERR: failed to enable pool "
                      << dedup_pool.name << " with: " << cpp_strerror(-ret)
                      << ", ret=" << ret << dendl;
  }
  return ret;
}
...
//---------------------------------------------------------------------------
int
Background::init_rados_access_handles(bool init_pool)
{
  ...
  d_watcher_ctx(this)
{
  ...
  ldpp_dout(dpp, 10) << "Config Vals::d_head_object_size=" << d_head_object_size
                     ...
                     << d_min_obj_size_for_dedup
                     << "::d_split_head=" << d_split_head << dendl;

  int ret = init_rados_access_handles(false);
  if (ret != 0) {
    derr << __func__ << "::ERR: failed init_rados_access_handles() ret=" << ret
         << "::" << cpp_strerror(-ret) << dendl;
...
//------------------------------------------------------------------------------
uint64_t
Background::__calc_deduped_bytes(uint16_t num_parts, uint64_t size_bytes)
{
  return calc_deduped_bytes(
      d_head_object_size, d_min_obj_size_for_dedup, d_split_head, num_parts,
      size_bytes);
}

//---------------------------------------------------------------------------
int
Background::add_disk_rec_from_bucket_idx(
    disk_block_array_t& disk_arr,
...
{
  disk_record_t rec(
      p_bucket, obj_name, p_parsed_etag, instance, obj_size, storage_class);
  ...
  ldpp_dout(dpp, 20) << __func__ << "::" << p_bucket->get_name() << "/"
                     << obj_name
                     << " was written to block_idx=" << rec_info.block_id
...
//---------------------------------------------------------------------------
int
Background::add_record_to_dedup_table(
    ...
    remapper_t* remapper)
{
  ...
  storage_class_idx_t sc_idx =
      remapper->remap(p_rec->stor_class, dpp, &p_stats->failed_map_overflow);
  if (unlikely(sc_idx == remapper_t::NULL_IDX)) {
    return -EOVERFLOW;
  }
  key_t key(
      p_rec->s.md5_high, p_rec->s.md5_low, size_4k_units, p_rec->s.num_parts,
      sc_idx);
  bool has_shared_manifest = p_rec->s.flags.has_shared_manifest();
  ldpp_dout(dpp, 20) << __func__ << "::bucket=" << p_rec->bucket_name
                     << ", obj=" << p_rec->obj_name
                     << ", block_id=" << (uint32_t)block_id
                     << ", rec_id=" << (uint32_t)rec_id
  ...
  int ret = p_table->add_entry(
      &key, block_id, rec_id, has_shared_manifest, &p_stats->big_objs_stat);
  if (ret == 0) {
    ...
    ldpp_dout(dpp, level) << __func__
                          << "::Failed p_table->add_entry (overflow)"
                          << "::loaded_objects=" << p_stats->loaded_objects
                          << "::failed_table_load="
                          << p_stats->failed_table_load << dendl;
...
build_oid(const std::string& bucket_id, const std::string& obj_name)
{
  std::string oid;
...
//---------------------------------------------------------------------------
static inline bool
invalid_tail_placement(const rgw_bucket_placement& tail_placement)
{
  return (
      tail_placement.bucket.name.empty() ||
      tail_placement.placement_rule.name.empty());
}
    ...
    std::string* p_tail_oid /*OUT*/)
{
  ...
  if (ret < 0) {
    ldpp_dout(dpp, 1) << __func__
                      << "::failed rgw_get_rados_ref() for oid=" << raw_obj.oid
...
//---------------------------------------------------------------------------
void
Background::remove_created_tail_object(
    const disk_record_t* p_rec,
    const RGWObjManifest& manifest,
    md5_stats_t* p_stats /*IN-OUT*/)
{
  ...
  std::string tail_oid;
  int ret =
      get_first_tail_obj_params(dpp, rados, manifest, &tail_ioctx, &tail_oid);
  if (unlikely(ret != 0)) {
    return;
  }
...
//---------------------------------------------------------------------------
inline bool
Background::should_split_head(const RGWObjManifest& manifest)
{
  ...
  if (manifest.get_rule(0, &rule)) {
    success =
        (d_split_head && // split-head was not disabled from yaml
    ...
    if (unlikely(!success)) {
      ldpp_dout(dpp, 20) << __func__ << "::ERR::d_split_head=" << d_split_head
                         ...
                         << rule.override_prefix.empty() << dendl;
    }
...
empty_rgw_bucket(const rgw_bucket& b)
{
  return (
      b.tenant.empty() && b.name.empty() && b.marker.empty() &&
      b.bucket_id.empty() && b.explicit_placement.data_pool.empty() &&
...
show_ref_tags(const DoutPrefixProvider* dpp, std::string& oid, rgw_rados_ref& obj)
{
  unsigned idx = 0;
  ...
  if (ret < 0) {
    ldpp_dout(dpp, 0) << __func__
                      << "::ERR: manifest::failed cls_refcount_read()"
                      << " idx=" << idx << dendl;
    return;
  }
...
//---------------------------------------------------------------------------
int
Background::free_tail_objs_by_manifest(
    const string& ref_tag,
    const string& oid,
    const RGWObjManifest& manifest)
{
  unsigned idx = 0;
  std::unique_ptr<rgw::Aio> aio = rgw::make_throttle(
      cct->_conf->rgw_max_copy_obj_concurrent_io, null_yield);
  for (auto p = manifest.obj_begin(dpp); p != manifest.obj_end(dpp);
       ++p, ++idx) {
    rgw_raw_obj raw_obj = p.get_location().get_raw_obj(rados);
    if (oid == raw_obj.oid) {
      ldpp_dout(dpp, 20) << __func__ << "::[" << idx
                         << "] Skip HEAD OBJ: " << raw_obj.oid << dendl;
      continue;
    }
    ...
    completed = aio->get(
        obj.obj, rgw::Aio::librados_op(obj.ioctx, std::move(op), null_yield),
        cost, id);
  }
...
//---------------------------------------------------------------------------
int
Background::rollback_ref_by_manifest(
    const string& ref_tag,
    const string& oid,
    const RGWObjManifest& manifest)
{
  ...
  for (auto p = manifest.obj_begin(dpp); p != manifest.obj_end(dpp);
    ...
    if (local_ret < 0) {
      ret_code = local_ret;
      ldpp_dout(dpp, 1) << __func__
                        << "::ERR: manifest::failed to open context " << obj
                        << dendl;
    ...
    ldpp_dout(dpp, 20) << __func__
                       << "::dec ref-count on tail object: " << raw_obj.oid
                       << dendl;
    cls_refcount_put(op, ref_tag, true);
...
//---------------------------------------------------------------------------
int
Background::inc_ref_count_by_manifest(
    const string& ref_tag,
    const string& oid,
    const RGWObjManifest& manifest)
{
  ...
  for (auto p = manifest.obj_begin(dpp); p != manifest.obj_end(dpp);
    ...
    if (ret < 0) {
      ldpp_dout(dpp, 1) << __func__
                        << "::ERR: manifest::failed to open context "
                        << raw_obj.oid << dendl;
      break;
    }
    ...
    d_ctl.metadata_access_throttle.acquire();
    ldpp_dout(dpp, 20) << __func__
                       << "::inc ref-count on tail object: " << obj.obj.oid
                       << "::" << raw_obj.to_str() << dendl;
    rgw::AioResultList completed = aio->get(
        obj.obj, rgw::Aio::librados_op(obj.ioctx, std::move(op), null_yield),
        cost, id);
    ...
    if (ret < 0) {
      ldpp_dout(dpp, 1) << __func__
                        << "::ERR: failed refcount_get() obj=" << obj
                        << ", ret=" << ret << " err is " << cpp_strerror(-ret)
                        << dendl;
      break;
    }
  }
  ...
  if (ret == 0) {
    ...
    if (ret == 0) {
      ...
      ldpp_dout(dpp, 1) << __func__
                        << "::ERR: manifest: failed to drain ios ret=" << ret
                        << dendl;
    }
  }
  ...
  for (auto& aio_res : all_results) {
    if (aio_res.result < 0) {
      ldpp_dout(dpp, 10) << __func__
                         << "::skip failed refcount inc: " << aio_res.obj.oid
                         << dendl;
    ...
    cls_refcount_put(op, ref_tag, true);
    ldpp_dout(dpp, 10) << __func__
                       << "::rollback refcount inc on: " << aio_res.obj.oid
                       << dendl;
    rgw::AioResultList completed = aio->get(
        obj.obj, rgw::Aio::librados_op(obj.ioctx, std::move(op), null_yield),
        cost, id);
    ...
    if (ret2 < 0) {
      ldpp_dout(dpp, 1)
          << __func__
          << "::ERR: cleanup after error failed to drop reference on obj="
          << aio_res.obj << dendl;
    }
  }
  completed = aio->drain();
  ...
  if (ret2 < 0) {
    ldpp_dout(dpp, 1) << __func__
                      << "::ERR: failed to drain rollback ios, ret=" << ret2
                      << dendl;
  }
...
{
  ldpp_dout(dpp, 20) << __func__ << "::DEDUP SRC:" << p_src_rec->bucket_name
                     << "/" << p_src_rec->obj_name << "(" << src_head_size
                     << ") ::TGT:" << p_tgt_rec->bucket_name << "/"
                     << p_tgt_rec->obj_name << "(" << tgt_head_size << ")"
                     << dendl;
    ...
    const dedup_table_t::value_t* p_src_val)
{
  ...
  bufferlist etag_bl;
  etag_to_bufferlist(
      p_tgt_rec->s.md5_high, p_tgt_rec->s.md5_low, p_tgt_rec->s.num_parts,
      &etag_bl);
  bool should_print_debug =
      cct->_conf->subsys.should_gather<ceph_subsys_rgw_dedup, 20>();
  if (unlikely(should_print_debug)) {
    dedup_object_log(
        dpp, p_src_rec, p_tgt_rec, src_head_size, tgt_head_size, etag_bl);
  }
  ...
  if (!p_src_val->has_shared_manifest()) {
    ...
    librados::ObjectWriteOperation src_op;
    {
      ...
      if (p_src_rec->s.flags.hash_calculated() && !p_src_val->has_valid_hash()) {
        ...
        ldpp_dout(dpp, 20) << __func__ << "::Set SRC Strong Hash in CLS"
                           << dendl;
    ...
    if (unlikely(ret != 0)) {
      ldpp_dout(dpp, 1) << __func__ << "::ERR: failed src_ioctx.operate("
                        << src_oid << "), err is " << cpp_strerror(-ret)
                        << dendl;
    ...
    if (offset > 0 || !p_pre_calc_hmac) {
      ...
      if (ret < 0) {
        ldpp_dout(dpp, 1) << __func__ << "::failed rgw_get_rados_ref() for oid="
                          << raw_obj.oid << ", err is " << cpp_strerror(-ret)
                          << dendl;
        return ret;
      }
      ...
      for (const auto& bptr : bl.buffers()) {
        blake3_hasher_update(
            p_hmac, (const unsigned char*)bptr.c_str(), bptr.length());
      }
    }
  }
    ...
    md5_shard_t md5_shard)
{
  ldpp_dout(dpp, 20) << __func__ << "::bucket=" << p_tgt_rec->bucket_name
                     ...
                     << ", block_id=" << block_id << ", rec_id=" << (int)rec_id
                     << "\n"
...
//---------------------------------------------------------------------------
static void
set_explicit_tail_placement(
    const DoutPrefixProvider* dpp,
    RGWObjManifest* p_manifest, // IN-OUT PARAM
    md5_stats_t* p_stats)
{
  p_stats->manifest_no_tail_placement++;
  ldpp_dout(dpp, 20) << __func__ << "::invalid_tail_placement -> update"
                     << dendl;
...
//---------------------------------------------------------------------------
int
Background::add_obj_attrs_to_record(
    disk_record_t* p_rec,
    ...
    md5_stats_t* p_stats) /*IN-OUT*/
{
  ...
  if (itr != attrs.end()) {
    ...
    try {
      ...
      ldpp_dout(dpp, 1) << __func__ << "::ERROR: unable to decode manifest"
                        << dendl;
      return -EINVAL;
    }
    ...
    if (unlikely(manifest.has_explicit_objs())) {
      ...
      ldpp_dout(dpp, 20) << __func__ << "::explicit_objs can't be dedup"
                         << dendl;
      return -ENOTSUP;
    }
    ...
    if (!manifest.get_rule(0, &rule) ||
      ...
      ldpp_dout(dpp, 10) << __func__ << "::Alibaba Cloud OSS can't be dedup"
                         ...
                         << "::override_prefix=" << rule.override_prefix
                         << dendl;
      return -ENOTSUP;
    }
  ...
  const std::string& storage_class =
      rgw_placement_rule::get_canonical_storage_class(
          head_placement_rule.storage_class);

  // p_rec holds an the storage_class value taken from the bucket-index/obj-attr
  if (unlikely(storage_class != p_rec->stor_class)) {
    ldpp_dout(dpp, 5) << __func__
                      << "::ERROR::manifest storage_class=" << storage_class
                      << " != " << "::bucket-index storage_class="
  ...
  if (itr != attrs.end()) {
    ...
    ldpp_dout(dpp, 20) << __func__
                       << "::Set Shared_Manifest::OBJ_NAME=" << p_rec->obj_name
                       << "::shared_manifest=0x" << std::hex
  ...
  if (!need_to_split_head) {
    ldpp_dout(dpp, 20) << __func__
                       << "::CALC Object Strong Hash::" << p_rec->obj_name
                       << dendl;
...
int
Background::read_object_attribute(
    dedup_table_t* p_table,
    disk_record_t* p_rec,
    ...
    remapper_t* remapper)
{
  ...
  if (ret != 0) {
    if (!dedupable_object(
            p_rec->multipart_object(), d_min_obj_size_for_dedup,
            ondisk_byte_size)) {
      ...
      ldpp_dout(dpp, 20) << __func__
                         << "::skipped purged small obj::" << p_rec->obj_name
      ...
      ldpp_dout(dpp, 20) << __func__
                         << "::skipped singleton::" << p_rec->obj_name
                         << std::dec << dendl;
    }
    return 0;
  }
  ...
  if (src_val.get_count() <= MAX_COPIES_PER_OBJ) {
    ...
    ldpp_dout(dpp, 20) << __func__ << "::Obj " << p_rec->obj_name << " has "
                       << src_val.get_count() << " copies" << dendl;
  ...
  if (src_val.has_shared_manifest() &&
      (attrs.find(RGW_ATTR_SHARE_MANIFEST) != attrs.end())) {
    ...
    uint64_t dedupable_objects_bytes =
        __calc_deduped_bytes(p_rec->s.num_parts, ondisk_byte_size);
    p_stats->shared_manifest_dedup_bytes += dedupable_objects_bytes;
    ldpp_dout(dpp, 20) << __func__
                       << "::(1)skipped shared_manifest, SRC::block_id="
  ...
  if (itr != attrs.end()) {
    if (unlikely(!parse_etag_string(itr->second.to_str(), &parsed_etag))) {
      ...
      ldpp_dout(dpp, 10) << __func__
                         << "::ERROR: corrupted etag::" << p_rec->obj_name
                         << dendl;
      return -EINVAL;
    }
  } else {
    p_stats->ingress_corrupted_etag++;
    ldpp_dout(dpp, 10) << __func__ << "::ERROR: no etag" << p_rec->obj_name
                       << dendl;
    return -EINVAL;
  }
  ...
  if (unlikely(storage_class != p_rec->stor_class)) {
    ldpp_dout(dpp, 5) << __func__
                      << "::ERROR::ATTR storage_class=" << storage_class
                      << " != " << "::bucket-index storage_class="
                      << p_rec->stor_class << dendl;
    p_stats->different_storage_class++;
    return -EINVAL;
  ...
  key_t key_from_obj(
      parsed_etag.md5_high, parsed_etag.md5_low,
      byte_size_to_disk_blocks(p_obj->get_size()), parsed_etag.num_parts,
      sc_idx);
  if (unlikely(
          key_from_obj != key_from_bucket_index ||
  ...
  if (unlikely(ret != 0)) {
    ...
    ldpp_dout(dpp, 5) << __func__
                      << "::ERR: failed add_obj_attrs_to_record() ret=" << ret
                      << "::" << cpp_strerror(-ret) << dendl;
    return ret;
  }
  ...
  if (ret == 0) {
    ...
    ldpp_dout(dpp, 20) << __func__ << "::" << p_rec->bucket_name << "/"
                       << p_rec->obj_name
                       << " was written to block_idx=" << rec_info.block_id
                       << "::rec_id=" << (int)rec_info.rec_id
                       << "::shared_manifest="
                       << p_rec->s.flags.has_shared_manifest() << dendl;
    p_table->update_entry(
        &key_from_bucket_index, rec_info.block_id, rec_info.rec_id,
        p_rec->s.flags.has_shared_manifest());
  } else {
    ldpp_dout(dpp, 5) << __func__ << "::ERR: Failed p_disk->add_record()"
                      << dendl;
    if (ret == -EINVAL) {
    ...
    md5_stats_t* p_stats)
{
  ...
  etag_to_bufferlist(
      p_rec->s.md5_high, p_rec->s.md5_low, p_rec->s.num_parts, &etag_bl);
  ...
  if (unlikely(ret != 0)) {
    ldpp_dout(dpp, 5) << __func__ << "::ERR: failed ioctx.operate(" << oid
                      << "), err is " << cpp_strerror(-ret) << dendl;
  }
    ...
    md5_stats_t* p_stats)
{
  if (unlikely(
          0 != memcmp(
                   p_src_rec->s.hash, p_tgt_rec->s.hash,
                   sizeof(p_src_rec->s.hash)))) {
    ...
    disk_record_t* p_rec)
{
  ...
  if (unlikely(ret < 0)) {
    ldpp_dout(dpp, 5) << __func__ << "::ERR: failed ioctx.getxattrs(" << oid
                      << "), err is " << cpp_strerror(-ret) << dendl;
    return ret;
  }
  ...
  if (itr != attrset.end()) {
    ldpp_dout(dpp, 20) << __func__ << "::Got Manifest " << p_rec->obj_name
                       << dendl;
...
//---------------------------------------------------------------------------
int
Background::split_head_object(
    disk_record_t* p_src_rec, // IN-OUT PARAM
    ...
    md5_stats_t* p_stats /* IN-OUT */)
{
  ...
  if (unlikely(ret != (int)head_size)) {
    ldpp_dout(dpp, 1) << __func__ << "::ERR: failed to read " << head_oid
                      << ", ret=" << ret << ", error is " << cpp_strerror(-ret)
                      << dendl;
    return ret;
  }
  ...
  librados::IoCtx tail_ioctx;
  std::string tail_oid;
  ret = get_first_tail_obj_params(
      dpp, rados, src_manifest, &tail_ioctx, &tail_oid);
  if (unlikely(ret != 0)) {
    return ret;
  }
  ...
  if (unlikely(ret < 0)) {
    ...
    ldpp_dout(dpp, 20) << __func__ << "::wrote tail obj:" << tail_oid
                       << "::ret=" << ret << dendl;
  }
...
//---------------------------------------------------------------------------
bool
Background::check_and_set_strong_hash(
    disk_record_t* p_src_rec,
    disk_record_t* p_tgt_rec,
    ...
    md5_stats_t* p_stats)
{
  ...
  if (!p_tgt_rec->s.flags.has_valid_hash()) {
    ldpp_dout(dpp, 20) << __func__
                       << "::CALC TGT Strong Hash::" << p_tgt_rec->obj_name
                       << dendl;
    ret = calc_object_blake3(
        tgt_manifest, p_tgt_rec, (uint8_t*)p_tgt_rec->s.hash);
    if (unlikely(ret != 0)) {
  ...
  if (!p_src_rec->s.flags.has_valid_hash() && p_src_val->has_valid_hash()) {
    ...
    ldpp_dout(dpp, 20) << __func__
                       << "::Fetch SRC strong hash from head-object::"
                       << p_src_rec->obj_name << dendl;
  ...
  if (should_split_head(src_manifest)) {
    ...
    ldpp_dout(dpp, 5) << __func__
                      << "::ERR: object not targeted for split_head has no hash"
                      << dendl;
    ...
    RGWObjManifest* p_tgt_manifest)
{
  ...
  try {
    ...
    ldpp_dout(dpp, 1) << __func__ << "::ERROR: bad "
                      << (valid_src_manifest ? "TGT" : "SRC") << " manifest"
                      << dendl;
    return -EINVAL;
  }
    ...
    md5_stats_t* p_stats)
{
  ...
  unsigned idx = 0;
  for (auto p = src_manifest.obj_begin(dpp); p != src_manifest.obj_end(dpp);
       ++p, ++idx) {
    rgw_raw_obj raw_obj = p.get_location().get_raw_obj(rados);
    if (src_oid != raw_obj.oid) {
      ldpp_dout(dpp, 20) << __func__ << "::[" << idx << "]" << raw_obj.oid
                         << dendl;
  ...
  idx = 0;
  for (auto p = tgt_manifest.obj_begin(dpp); p != tgt_manifest.obj_end(dpp);
       ++p, ++idx) {
    rgw_raw_obj raw_obj = p.get_location().get_raw_obj(rados);
...
//
int
Background::try_deduping_record(
    dedup_table_t* p_table,
    disk_record_t* p_tgt_rec,
    disk_block_id_t block_id,
    record_id_t rec_id,
    ...
    remapper_t* remapper)
{
  ...
  storage_class_idx_t sc_idx = remapper->remap(
      p_tgt_rec->stor_class, dpp, &p_stats->failed_map_overflow);
  if (unlikely(sc_idx == remapper_t::NULL_IDX)) {
    ldpp_dout(dpp, 5) << __func__ << "::invalid_storage_class_mapping for "
                      << p_tgt_rec->stor_class << "::" << p_tgt_rec->obj_name
                      << dendl;
    p_stats->invalid_storage_class_mapping++;
    return 0;
  }
  key_t key(
      p_tgt_rec->s.md5_high, p_tgt_rec->s.md5_low, size_4k_units,
  ...
  if (unlikely(ret != 0)) {
    ...
    ldpp_dout(dpp, 5) << __func__
                      << "::skipped singleton::" << p_tgt_rec->bucket_name
                      << "/" << p_tgt_rec->obj_name
                      << "::num_parts=" << p_tgt_rec->s.num_parts
                      << "::ETAG=" << std::hex << p_tgt_rec->s.md5_high
                      << p_tgt_rec->s.md5_low << std::dec << dendl;
  ...
  if (block_id == src_block_id && rec_id == src_rec_id) {
    ...
    ldpp_dout(dpp, 20) << __func__
                       << "::(2)skipped source-record, block_id=" << block_id
                       << "::rec_id=" << (int)rec_id << dendl;
    return 0;
  }
  ...
  if (p_tgt_rec->s.flags.has_shared_manifest()) {
    ...
    ldpp_dout(dpp, 1) << __func__
                      << "::(3)skipped shared_manifest, block_id=" << block_id
                      << "::rec_id=" << (int)rec_id << dendl;
  ...
  if (unlikely(p_src_rec->s.obj_bytes_size != p_tgt_rec->s.obj_bytes_size)) {
    ...
    ldpp_dout(dpp, 10) << __func__
                       << "::WARN: different byte size for objects::"
                       << p_src_rec->obj_name
                       << "::" << p_src_rec->s.obj_bytes_size
                       << "::" << p_tgt_rec->obj_name
  ...
  if (unlikely(has_shared_tail_objects(
          dpp, rados, p_src_rec, p_tgt_rec, src_manifest, tgt_manifest,
          p_stats))) {
    return 0;
  }

  bool success = check_and_set_strong_hash(
      p_src_rec, p_tgt_rec, src_manifest, tgt_manifest, &src_val, p_stats);
  if (unlikely(!success)) {
    if (p_src_rec->s.flags.hash_calculated() && !src_val.has_valid_hash()) {
      // set hash attributes on head objects to save calc next time
      ldpp_dout(dpp, 20) << __func__ << "::failed: store valid SRC hash"
                         << dendl;
    ...
    if (p_tgt_rec->s.flags.hash_calculated()) {
      ldpp_dout(dpp, 20) << __func__ << "::failed: store valid TGT hash"
                         << dendl;
  ...
  ret = dedup_object(
      p_src_rec, p_tgt_rec, src_manifest, tgt_manifest, p_stats, &src_val);
  if (ret == 0) {
    ldpp_dout(dpp, 20) << __func__ << "::dedup success " << p_src_rec->obj_name
                       << dendl;
    p_stats->deduped_objects++;
    p_stats->deduped_objects_bytes += dedupable_objects_bytes;
    if (p_tgt_rec->s.flags.is_split_head()) {
      ldpp_dout(dpp, 20) << __func__
                         << "::TGT-Split: dedup_bytes=" << ondisk_byte_size
                         << dendl;
    ...
    if (!src_val.has_shared_manifest()) {
      ldpp_dout(dpp, 20) << __func__ << "::mark shared_manifest+valid_hash"
                         << dendl;
      ...
      ldpp_dout(dpp, 20) << __func__
                         << "::SRC object already marked as shared_manifest"
                         << dendl;
    }
  } else {
...
//---------------------------------------------------------------------------
const char*
Background::dedup_step_name(dedup_step_t step)
{
  static const char* names[] = {
      "STEP_NONE", "STEP_BUCKET_INDEX_INGRESS", "STEP_BUILD_TABLE",
      "STEP_READ_ATTRIBUTES", "STEP_REMOVE_DUPLICATES"};
    ...
    remapper_t* remapper)
{
  ...
  int failure_count = 0;
  ldpp_dout(dpp, 20) << __func__ << "::" << dedup_step_name(step)
                     << "::worker_id=" << worker_id
  ...
  *p_slab_count = 0;
  while (has_more) {
    bufferlist bl;
    int ret = load_slab(
        d_dedup_cluster_ioctx, bl, md5_shard, worker_id, seq_number, dpp);
    if (unlikely(ret < 0)) {
      ldpp_dout(dpp, 1) << __func__
                        << "::ERR::Failed loading object!! md5_shard="
                        << md5_shard << ", worker_id=" << worker_id
    ...
    for (uint32_t block_num = 0; block_num < DISK_BLOCK_COUNT;
         block_num++, seq_number++) {
      disk_block_id_t disk_block_id(worker_id, seq_number);
      const char* p =
          get_next_data_ptr(bl_itr, block_buff, sizeof(block_buff), dpp);
      ...
      if (unlikely(p_header->verify(disk_block_id, dpp) != 0)) {
        ...
        if (failure_count++ < MAX_BAD_BLOCKS) {
          ...
          ldpp_dout(dpp, 1)
              << __func__
              << "::Skipping slab with too many bad blocks::" << (int)md5_shard
      ...
      for (unsigned rec_id = 0; rec_id < p_header->rec_count; rec_id++) {
        ...
        if (step == STEP_BUILD_TABLE) {
          add_record_to_dedup_table(
              p_table, &rec, disk_block_id, rec_id, p_stats, remapper);
          slab_rec_count++;
        }
#ifdef FULL_DEDUP_SUPPORT
        else if (step == STEP_READ_ATTRIBUTES) {
          read_object_attribute(
              p_table, &rec, disk_block_id, rec_id, md5_shard, p_stats,
              p_disk_block_seq, remapper);
          slab_rec_count++;
      ...
      check_and_update_md5_heartbeat(
          md5_shard, p_stats->loaded_objects, p_stats->processed_objects);
      ...
      if (!has_more) {
        ldpp_dout(dpp, 20) << __func__
                           << "::No more blocks! block_id=" << disk_block_id
...
//---------------------------------------------------------------------------
int
Background::ingress_bucket_idx_single_object(
    disk_block_array_t& disk_arr,
    const rgw::sal::Bucket* p_bucket,
    ...
    worker_stats_t* p_worker_stats /*IN-OUT*/)
{
  ...
  if (unlikely(!parse_etag_string(entry.meta.etag, &parsed_etag))) {
    ...
    ldpp_dout(dpp, 1) << __func__
                      << "::ERROR: corrupted etag:" << entry.meta.etag
  ...
  if (unlikely((cct->_conf->subsys.should_gather<ceph_subsys_rgw_dedup, 20>()))) {
    show_ingress_bucket_idx_obj(
        dpp, parsed_etag, p_bucket->get_name(), entry.key.name);
  }
  ...
  return add_disk_rec_from_bucket_idx(
      disk_arr, p_bucket, &parsed_etag, entry.key.name, entry.key.instance,
      entry.meta.size, storage_class);
}

//---------------------------------------------------------------------------
void
Background::check_and_update_heartbeat(
    unsigned shard_id,
    uint64_t count_a,
    uint64_t count_b,
    const char* prefix)
{
  ...
  if (unlikely(time_elapsed.tv.tv_sec >= d_heart_beat_max_elapsed_sec)) {
    ...
    d_cluster.update_shard_token_heartbeat(
        store, shard_id, count_a, count_b, prefix);
  }
}

//---------------------------------------------------------------------------
void
Background::check_and_update_worker_heartbeat(
    work_shard_t worker_id,
    int64_t ingress_obj_count)
{
  check_and_update_heartbeat(
      worker_id, ingress_obj_count, 0, WORKER_SHARD_PREFIX);
}

//---------------------------------------------------------------------------
void
Background::check_and_update_md5_heartbeat(
    md5_shard_t md5_id,
    uint64_t load_count,
...
//---------------------------------------------------------------------------
static uint32_t
move_to_next_bucket_index_shard(
    const DoutPrefixProvider* dpp,
    ...
    rgw_obj_index_key* p_marker /* OUT-PARAM */)
{
  ...
  ldpp_dout(dpp, 20) << __func__ << "::" << bucket_name
                     << "::curr_shard=" << current_shard
                     << ", next shard=" << next_shard << dendl;
...
//---------------------------------------------------------------------------
int
Background::process_bucket_shards(
    disk_block_array_t& disk_arr,
    ...
    worker_stats_t* p_worker_stats /*IN-OUT*/)
{
  ...
  while (current_shard < num_shards) {
    ...
    cls_rgw_bucket_list_op(
        op, marker, null_prefix, null_delimiter, max_entries, list_versions,
        &result);
    int ret =
        rgw_rados_operate(dpp, ioctx, oid, std::move(op), nullptr, null_yield);
    if (unlikely(ret < 0)) {
      ldpp_dout(dpp, 1) << __func__
                        << "::ERR: failed rgw_rados_operate() ret=" << ret
                        << "::" << cpp_strerror(-ret) << dendl;
      current_shard = move_to_next_bucket_index_shard(
          dpp, current_shard, num_work_shards, bucket->get_name(), &marker);
      continue;
    }
    ...
    for (auto& entry : result.dir.m) {
      ...
      ldpp_dout(dpp, 20) << __func__ << "::dirent = " << bucket->get_name()
                         << "/" << marker.name
                         << "::instance=" << marker.instance << dendl;
      if (unlikely(
              (!dirent.exists && !dirent.is_delete_marker()) ||
              !dirent.pending_map.empty())) {
        // TBD: should we bailout ???
        ldpp_dout(dpp, 1) << __func__
                          << "::ERR: bad dirent::" << bucket->get_name() << "/"
                          << marker.name << "::instance=" << marker.instance
                          << dendl;
        continue;
      } else if (unlikely(dirent.is_delete_marker())) {
        ldpp_dout(dpp, 20) << __func__
                           << "::skip delete_marker::" << bucket->get_name()
                           << "/" << marker.name
                           << "::instance=" << marker.instance << dendl;
        continue;
      }
      ret = ingress_bucket_idx_single_object(
          disk_arr, bucket, dirent, p_worker_stats);
    }
    // TBD: advance marker only once here!
    if (result.is_truncated) {
      ldpp_dout(dpp, 15) << __func__ << "::[" << current_shard
                         << "]result.is_truncated::count=" << obj_count
                         << dendl;
    } else {
      // we reached the end of this shard -> move to the next shard
      current_shard = move_to_next_bucket_index_shard(
          dpp, current_shard, num_work_shards, bucket->get_name(), &marker);
      ldpp_dout(dpp, 15) << __func__
                         << "::move_to_next_bucket_index_shard::count="
                         << obj_count << "::new_shard=" << current_shard
                         << dendl;
    }
  }
...
//---------------------------------------------------------------------------
int
Background::ingress_bucket_objects_single_shard(
    disk_block_array_t& disk_arr,
    ...
    worker_stats_t* p_worker_stats /*IN-OUT*/)
{
  ...
  ret = rados->get_bucket_instance_info(
      bucket_id, bucket_info, nullptr, nullptr, null_yield, dpp);
  if (unlikely(ret < 0)) {
    if (ret == -ENOENT) {
    ...
    ldpp_dout(dpp, 5) << __func__
                      << "::ERROR: get_bucket_instance_info(), ret=" << ret
                      << "::" << cpp_strerror(-ret) << dendl;
    return ret;
  }
  ...
  ret = store->svc()->bi_rados->open_bucket_index(
      dpp, bucket_info, std::nullopt, idx_layout, &ioctx, &oids, nullptr);
  if (ret >= 0) {
    // process all the shards in this bucket owned by the worker_id
    return process_bucket_shards(
        disk_arr, bucket.get(), oids, ioctx, worker_id, num_work_shards,
        p_worker_stats);
  } else {
    ldpp_dout(dpp, 1) << __func__ << "::ERROR: open_bucket_index() ret=" << ret
                      << "::" << cpp_strerror(-ret) << dendl;
    return ret;
  }
}
...
//---------------------------------------------------------------------------
static void
display_table_stat_counters(
    const DoutPrefixProvider* dpp,
    const md5_stats_t* p_stats)
{
  uint64_t obj_count_in_shard =
      (p_stats->big_objs_stat.singleton_count +
...
//---------------------------------------------------------------------------
int
Background::objects_dedup_single_md5_shard(
    dedup_table_t* p_table,
    md5_shard_t md5_shard,
    md5_stats_t* p_stats,
    work_shard_t num_work_shards)
{
  ...
  storage_class_idx_t sc_idx = remapper.remap(
      RGW_STORAGE_CLASS_STANDARD, dpp, &p_stats->failed_map_overflow);
  ...
  for (work_shard_t worker_id = 0; worker_id < num_work_shards; worker_id++) {
    process_all_slabs(
        p_table, STEP_BUILD_TABLE, md5_shard, worker_id,
  ...
  // TBD  - find a better name than num_work_shards for the combined output
  {
    ...
    disk_block_seq_t disk_block_seq(
        dpp, arr, num_work_shards, md5_shard, &wstat);
    for (work_shard_t worker_id = 0; worker_id < num_work_shards; worker_id++) {
      process_all_slabs(
          p_table, STEP_READ_ATTRIBUTES, md5_shard, worker_id,
  ...
  ldpp_dout(dpp, 10) << __func__ << "::STEP_REMOVE_DUPLICATES::started..."
                     << dendl;
  uint32_t slab_count = 0;
  process_all_slabs(
      p_table, STEP_REMOVE_DUPLICATES, md5_shard, num_work_shards, &slab_count,
      p_stats, nullptr, &remapper);
  if (unlikely(d_ctl.should_stop())) {
    ldpp_dout(dpp, 5) << __func__ << "::STEP_REMOVE_DUPLICATES::STOPPED\n"
                      << dendl;
    return -ECANCELED;
  }
  ldpp_dout(dpp, 10) << __func__ << "::STEP_REMOVE_DUPLICATES::finished..."
                     << dendl;
    ...
    uint64_t* p_size)
{
  ...
  if (is_layout_indexless(index)) {
    ldpp_dout(dpp, 1)
        << __func__ << "::ERR, indexless buckets do not maintain stats; bucket="
  ...
  ret = bucket->read_stats(
      dpp, null_yield, index, RGW_NO_SHARD, &bucket_ver, &master_ver, stats,
      &max_marker);
  if (ret < 0) {
  ...
  for (auto itr = stats.begin(); itr != stats.end(); ++itr) {
    ...
    ldpp_dout(dpp, 20) << __func__ << "::" << bucket->get_name()
                       << "::" << to_string(itr->first)
                       << "::num_obj=" << s.num_objects << "::size=" << s.size
                       << dendl;
...
//---------------------------------------------------------------------------
int
Background::collect_all_buckets_stats()
{
  int ret = 0;
  ...
  while (has_more) {
    ...
    if (ret == 0) {
      for (auto& entry : entries) {
        ...
        if (unlikely(ret < 0)) {
          ldpp_dout(dpp, 1)
              << __func__ << "::ERR: Failed rgw_bucket_parse_bucket_key: "
              << cpp_strerror(-ret) << dendl;
          goto err;
        }
        ldpp_dout(dpp, 20) << __func__ << "::bucket=" << bucket << dendl;
        ret = read_bucket_stats(
            bucket, &d_all_buckets_obj_count, &d_all_buckets_obj_size);
        if (unlikely(ret != 0)) {
          goto err;
        }
      }
      ...
      ldpp_dout(dpp, 1) << __func__
                        << "::ERR: failed driver->meta_list_keys_next()"
                        << dendl;
      goto err;
    }
  }
...
//---------------------------------------------------------------------------
int
Background::objects_ingress_single_work_shard(
    work_shard_t worker_id,
    work_shard_t num_work_shards,
    ...
    uint64_t raw_mem_size)
{
  ...
  disk_block_array_t disk_arr(
      dpp, raw_mem, raw_mem_size, worker_id, p_worker_stats, num_md5_shards);
  bool has_more = true;
  ...
  while (ret == 0 && has_more) {
    ...
    ret = driver->meta_list_keys_next(dpp, handle, max_keys, entries, &has_more);
    if (ret == 0) {
      ldpp_dout(dpp, 20) << __func__ << "::entries.size()=" << entries.size()
                         << dendl;
      for (auto& entry : entries) {
        ldpp_dout(dpp, 20) << __func__ << "::bucket_name=" << entry << dendl;
        rgw_bucket bucket;
        ...
        ldpp_dout(dpp, 20) << __func__ << "::bucket=" << bucket << dendl;
        ret = ingress_bucket_objects_single_shard(
            disk_arr, bucket, worker_id, num_work_shards, p_worker_stats);
        if (unlikely(ret != 0)) {
          ...
          ldpp_dout(dpp, 1)
              << __func__ << "::Failed ingress_bucket_objects_single_shard()"
              << dendl;
      ...
      ldpp_dout(dpp, 1) << __func__ << "::failed driver->meta_list_keys_next()"
                        << dendl;
      driver->meta_list_keys_complete(handle);
  ...
  ldpp_dout(dpp, 20) << __func__
                     << "::flush_output_buffers() worker_id=" << worker_id
                     << dendl;
    ...
    uint32_t slab_count)
{
  ...
  for (uint32_t slab_id = 0; slab_id < slab_count; slab_id++) {
    ...
    ldpp_dout(dpp, 20) << __func__ << "::calling ioctx->remove(" << oid << ")"
                       << dendl;
    int ret = d_dedup_cluster_ioctx.remove(oid);
    if (ret != 0) {
      ldpp_dout(dpp, 0) << __func__ << "::ERR Failed ioctx->remove(" << oid
                        << ")" << dendl;
      failure_count++;
    }
  }
    ...
    md5_shard_t num_md5_shards)
{
  ...
  int ret = objects_ingress_single_work_shard(
      worker_id, num_work_shards, num_md5_shards, &worker_stats, raw_mem,
      raw_mem_size);
  if (ret == 0) {
    worker_stats.duration = ceph_clock_now() - start_time;
    worker_stats.bidx_throttle_sleep_events =
        d_ctl.bucket_index_throttle.get_sleep_events();
    worker_stats.bidx_throttle_sleep_time_usec =
        d_ctl.bucket_index_throttle.get_sleep_time_usec();
    d_cluster.mark_work_shard_token_completed(store, worker_id, &worker_stats);
    ldpp_dout(dpp, 10) << "stat counters [worker]:\n" << worker_stats << dendl;
    ldpp_dout(dpp, 10) << "Shard Process Duration   = " << worker_stats.duration
                       << dendl;
  }
    ...
    md5_shard_t num_md5_shards)
{
  ...
  //DEDUP_DYN_ALLOC
  dedup_table_t table(
      dpp, d_head_object_size, d_min_obj_size_for_dedup, d_split_head, raw_mem,
      raw_mem_size);
  int ret = objects_dedup_single_md5_shard(
      &table, md5_shard, &md5_stats, num_work_shards);
  if (ret == 0) {
    md5_stats.duration = ceph_clock_now() - start_time;
    md5_stats.md_throttle_sleep_events =
        d_ctl.metadata_access_throttle.get_sleep_events();
    md5_stats.md_throttle_sleep_time_usec =
        d_ctl.metadata_access_throttle.get_sleep_time_usec();

    d_cluster.mark_md5_shard_token_completed(store, md5_shard, &md5_stats);
    ldpp_dout(dpp, 10) << "stat counters [md5]:\n" << md5_stats << dendl;
    ldpp_dout(dpp, 10) << "Shard Process Duration   = " << md5_stats.duration
                       << dendl;
  }
  return ret;
}
...
//---------------------------------------------------------------------------
int
Background::process_all_shards(
    bool ingress_work_shards,
    int (Background::*
             func)(unsigned, uint8_t*, uint64_t, work_shard_t, md5_shard_t),
    uint8_t* raw_mem,
    uint64_t raw_mem_size,
    work_shard_t num_work_shards,
    md5_shard_t num_md5_shards)
{
  while (true) {
    ...
    if (shard_id != NULL_SHARD) {
      ...
      int ret = (this->*func)(
          shard_id, raw_mem, raw_mem_size, num_work_shards, num_md5_shards);
      if (unlikely(ret != 0)) {
        if (d_ctl.should_stop()) {
          ...
          ldpp_dout(dpp, 5)
              << __func__ << "::Skip shard #" << shard_id << dendl;
        }
      }
    } else {
      ldpp_dout(dpp, 10) << __func__ << "::finished processing all shards"
                         << dendl;
      break;
    }
  } // while loop
  ...
  *p_num_objects_bytes = 0;
  ...
  for (auto i = stats.begin(); i != stats.end(); ++i) {
    ...
    *p_num_objects_bytes = s.num_bytes / replica_level;
    ldpp_dout(dpp, 10) << __func__ << "::" << pool_name
                       << "::num_objects=" << s.num_objects
                       << "::num_copies=" << s.num_object_copies
                       << "::num_bytes=" << s.num_bytes << "/"
                       << *p_num_objects_bytes << dendl;
  }
  return 0;
}
...
//-------------||--------------||---------||-----------------------------------||
static md5_shard_t
calc_num_md5_shards(uint64_t obj_count)
{
...
//---------------------------------------------------------------------------
int
Background::setup(dedup_epoch_t* p_epoch)
{
  ...
  if (ret != 0) {
    derr << "dedup_bg::resume() failed init_rados_access_handles() ret=" << ret
         << "::" << cpp_strerror(-ret) << dendl;
    return ret;
  }
  display_ioctx_state(dpp, d_dedup_cluster_ioctx, __func__);
  ...
  if (unlikely(p_epoch->num_work_shards > MAX_WORK_SHARD)) {
    ldpp_dout(dpp, 1) << __func__ << "::ERR: p_epoch->num_work_shards="
                      ...
                      << " is larger than MAX_WORK_SHARD (" << MAX_WORK_SHARD
                      << ")" << dendl;
    return -EOVERFLOW;
  }
  if (unlikely(p_epoch->num_md5_shards > MAX_MD5_SHARD)) {
    ldpp_dout(dpp, 1) << __func__ << "::ERR: p_epoch->num_md5_shards="
                      ...
                      << " is larger than MAX_MD5_SHARD (" << MAX_MD5_SHARD
                      << ")" << dendl;
    return -EOVERFLOW;
  }
...
#ifdef FULL_DEDUP_SUPPORT
  ceph_assert(
      d_ctl.dedup_type == dedup_req_type_t::DEDUP_TYPE_EXEC ||
...
//---------------------------------------------------------------------------
int
Background::watch_reload(const DoutPrefixProvider* dpp)
{
...
//---------------------------------------------------------------------------
int
Background::unwatch_reload(const DoutPrefixProvider* dpp)
{
  if (d_watch_handle == 0) {
    // nothing to unwatch
...
//---------------------------------------------------------------------------
void
Background::handle_notify(uint64_t notify_id, uint64_t cookie, bufferlist& bl)
{
  int ret = 0;
  ...
  ldpp_dout(dpp, 5) << __func__ << "::" << get_urgent_msg_names(urgent_msg)
                    << dendl;
  ...
  if (unlikely(d_ctl.local_urgent_req())) {
    ...
    ldpp_dout(dpp, 5) << __func__
                      << "::system is paused/shutdown -> cancel notification"
                      << dendl;
  ...
  case URGENT_MSG_ABORT:
    if (d_ctl.dedup_exec) {
      ...
      d_cond.notify_all();
      d_cond.wait(cond_lock, [this] {
        return d_ctl.remote_aborted || d_ctl.local_urgent_req();
      });
      d_ctl.remote_aborted ? ret = 0 : ret = -EBUSY;
    } else {
      ldpp_dout(dpp, 5) << __func__ << "::inactive dedup->nothing to do"
                        << dendl;
    }
    break;
  ...
  case URGENT_MSG_PASUE:
    if (d_ctl.dedup_exec && !d_ctl.remote_paused) {
      ...
      d_cond.notify_all();
      d_cond.wait(cond_lock, [this] {
        return d_ctl.remote_paused || d_ctl.local_urgent_req();
      });
  ...
  case URGENT_MSG_RESUME:
    if (d_ctl.remote_pause_req || d_ctl.remote_paused) {
      ...
      ldpp_dout(dpp, 5) << __func__ << "::dedup is not paused->nothing to do"
                        << dendl;
    }
    break;
...
Background::pause()
{
  ...
  d_cond.notify_all();
  d_cond.wait(cond_lock, [this] {
    return d_ctl.local_paused || d_ctl.shutdown_done;
  });
  if (nested_call) {
...
//---------------------------------------------------------------------------
void
Background::resume(rgw::sal::Driver* _driver)
{
  ...
  if (!d_ctl.local_paused) {
    ...
    if (_driver != driver) {
      ldpp_dout(dpp, 1)
          << "dedup_bg attempt to change driver on an active system was refused"
          << dendl;
    }
    return;
  }
...
//---------------------------------------------------------------------------
void
Background::handle_pause_req(const char* caller)
{
  ldpp_dout(dpp, 5) << __func__ << "::caller=" << caller << dendl;
  ldpp_dout(dpp, 5) << __func__ << "::" << d_ctl << dendl;
  while (d_ctl.local_pause_req || d_ctl.local_paused ||
         d_ctl.remote_pause_req || d_ctl.remote_paused) {
    std::unique_lock cond_lock(d_cond_mutex);
    if (d_ctl.should_stop()) {
    ...
    if (d_ctl.local_paused) {
      ...
      d_cond.wait(cond_lock, [this] {
        return !d_ctl.local_paused || d_ctl.should_stop();
      });
    }

    if (d_ctl.remote_paused) {
      ldpp_dout(dpp, 10) << __func__ << "::wait on d_ctl.remote_paused"
                         << dendl;
      d_cond.wait(cond_lock, [this] {
        return !d_ctl.remote_paused || d_ctl.should_stop() ||
               d_ctl.local_pause_req;
      });
    }
  } // while loop
...
//---------------------------------------------------------------------------
void
Background::work_shards_barrier(work_shard_t num_work_shards)
{
  ...
  while (true) {
    ...
    ldpp_dout(dpp, 10) << __func__
                       << "::Wait for object ingress completion, ttl=" << ttl
                       << " seconds" << dendl;
    std::unique_lock cond_lock(d_cond_mutex);
    d_cond.wait_for(cond_lock, std::chrono::seconds(ttl), [this] {
      return d_ctl.should_stop() || d_ctl.should_pause();
    });
    if (unlikely(d_ctl.should_pause())) {
      handle_pause_req(__func__);
    }
...
//---------------------------------------------------------------------------
void
Background::md5_shards_barrier(md5_shard_t num_md5_shards)
{
  ...
  while (!all_md5_shards_completed(&d_cluster, store, num_md5_shards)) {
    ldpp_dout(dpp, 10) << __func__ << "::Wait for md5 completion, ttl=" << ttl
                       << " seconds" << dendl;
    std::unique_lock cond_lock(d_cond_mutex);
    d_cond.wait_for(cond_lock, std::chrono::seconds(ttl), [this] {
      return d_ctl.should_stop() || d_ctl.should_pause();
...
Background::run()
{
  ...
  // 256x8KB=2MB
  const uint64_t PER_SHARD_BUFFER_SIZE = DISK_BLOCK_COUNT *
                                         sizeof(disk_block_t);
  ...
  while (!d_ctl.shutdown_req) {
    ...
    if (d_ctl.dedup_exec) {
      ...
      const rgw_pool& dedup_pool =
          store->svc()->zone->get_zone_params().dedup_pool;
      int64_t pool_id = rados_handle->pool_lookup(dedup_pool.name.c_str());
      if (pool_id < 0) {
      ...
      if (raw_mem == nullptr) {
        ldpp_dout(dpp, 1) << "failed slab memory allocation - size="
                          << RAW_MEM_SIZE << dendl;
        return;
      }

      process_all_shards(
          true, &Background::f_ingress_work_shard, raw_mem.get(), RAW_MEM_SIZE,
      ...
      if (!d_ctl.should_stop()) {
        ...
        if (!d_ctl.should_stop()) {
          process_all_shards(
              false, &Background::f_dedup_md5_shard, raw_mem.get(),
        ...
        ldpp_dout(dpp, 5) << __func__ << "::stop req from ingress_work_shard"
                          << dendl;
      }
    } // dedup_exec
    ...
    if (d_ctl.remote_abort_req) {
      ...
      d_ctl.remote_paused = false;
      d_cond.notify_all();
      ldpp_dout(dpp, 5) << __func__ << "::Dedup was aborted on a remote req"
                        << dendl;
    }
    d_cond.wait(cond_lock, [this] {
      return d_ctl.remote_restart_req || d_ctl.should_stop() ||
             d_ctl.should_pause();
    });
