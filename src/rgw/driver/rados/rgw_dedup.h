...
class Background : public RGWRealmReloader::Pauser {
  ...
  public:
    DedupWatcher(Background* _parent) :
      parent(_parent)
    {}
...
private:
  ...
  void remove_created_tail_object(
  ...
  int read_buckets();
  void check_and_update_heartbeat(
      unsigned shard_id,
      uint64_t count_a,
      uint64_t count_b,
      const char* prefix);

  inline void check_and_update_worker_heartbeat(
      work_shard_t worker_id,
      int64_t obj_count);
  inline void check_and_update_md5_heartbeat(
      md5_shard_t md5_id,
      uint64_t load_count,
      uint64_t dedup_count);
  int ingress_bucket_idx_single_object(
  ...
  int process_bucket_shards(
      ...
      worker_stats_t* p_worker_stats /*IN-OUT*/);
  int ingress_bucket_objects_single_shard(
      disk_block_array_t& disk_arr,
      const rgw_bucket& bucket_rec,
      work_shard_t worker_id,
      work_shard_t num_work_shards,
      worker_stats_t* p_worker_stats /*IN-OUT*/);
  int objects_ingress_single_work_shard(
      work_shard_t worker_id,
      work_shard_t num_work_shards,
  ...
  int f_ingress_work_shard(
      unsigned shard_id,
      uint8_t* raw_mem,
      uint64_t raw_mem_size,
      work_shard_t num_work_shards,
      md5_shard_t num_md5_shards);
  int f_dedup_md5_shard(
      unsigned shard_id,
      uint8_t* raw_mem,
      uint64_t raw_mem_size,
      work_shard_t num_work_shards,
      md5_shard_t num_md5_shards);
  int process_all_shards(
  ...
  int objects_dedup_single_md5_shard(
      dedup_table_t* p_table,
      md5_shard_t md5_shard,
      md5_stats_t* p_stats,
      work_shard_t num_work_shards);
  int add_disk_rec_from_bucket_idx(
      disk_block_array_t& disk_arr,
  ...
  int read_object_attribute(
      ...
      remapper_t* remapper);
  bool check_and_set_strong_hash(
      disk_record_t* p_src_rec, // IN/OUT PARAM
  ...
  int try_deduping_record(
      ...
      md5_stats_t* p_stats, /* IN-OUT */
      remapper_t* remapper);
  int inc_ref_count_by_manifest(
      const std::string& ref_tag,
      const std::string& oid,
      const RGWObjManifest& manifest);
  int rollback_ref_by_manifest(
      const std::string& ref_tag,
      const std::string& oid,
      const RGWObjManifest& tgt_manifest);
  int free_tail_objs_by_manifest(
      const std::string& ref_tag,
      const std::string& oid,
      const RGWObjManifest& tgt_manifest);
  int dedup_object(
