...
static int collect_shard_stats(
    ...
    librados::IoCtx& ctl_ioctx /* OUT-PARAM */)
{
  ...
  if (unlikely(ret < 0)) {
    ldpp_dout(dpp, 1) << __func__
                      << "::ERR: failed rgw_init_ioctx() for control_pool ret="
    ...
    const char* caller)
{
  ...
  if (ret > 0) {
    ...
    if (caller) {
      ldpp_dout(dpp, 10) << __func__ << "::" << caller << "::" << *p_epoch
                         << dendl;
    }
    return 0;
  } else {
    ...
    md5_shard_t num_md5_shards)
{
  ...
  if (ret == 0) {
    ldpp_dout(dpp, 10) << __func__ << "::successfully created Epoch object!"
                       << dendl;
    // now try and take ownership
  } else if (ret == -EEXIST) {
    ldpp_dout(dpp, 10) << __func__
                       << "::Epoch object exists -> trying to take over"
                       << dendl;
  ...
  dedup_epoch_t new_epoch = {
      serial, dedup_type, ceph_clock_now(), num_work_shards, num_md5_shards};
  ...
  // probably best to read attribute from epoch!
    ...
    if (ret == 0) {
      ldpp_dout(dpp, 10) << __func__ << "::Accept existing Epoch object"
                         << dendl;
    }
    return ret;
  } else {
    ldpp_dout(dpp, 5) << __func__ << "::ERR: failed ctl_ioctx.operate(" << oid
    ...
    md5_shard_t num_md5_shards)
{
  ...
  dedup_epoch_t new_epoch = {
      p_old_epoch->serial + 1, dedup_type, ceph_clock_now(), num_work_shards,
      num_md5_shards};
...
struct shard_progress_t {
  ...
  shard_progress_t(
      uint64_t _progress_a,
      uint64_t _progress_b,
      bool _completed,
      const std::string& _owner,
      const bufferlist& _stats_bl) :
    owner(_owner), stats_bl(_stats_bl)
  {
...
//---------------------------------------------------------------------------
std::ostream&
operator<<(std::ostream& out, shard_progress_t& sp)
{
  out << (sp.completed ? " + ::" : " - ::");
...
//---------------------------------------------------------------------------
void
encode(const shard_progress_t& sp, ceph::bufferlist& bl)
{
  ENCODE_START(1, 1, bl);
...
//---------------------------------------------------------------------------
void
decode(shard_progress_t& sp, ceph::bufferlist::const_iterator& bl)
{
  DECODE_START(1, bl);
    ...
    md5_shard_t num_md5_shards)
{
  ...
  while (true) {
    ...
    if (p_epoch->num_work_shards && p_epoch->num_md5_shards) {
      ldpp_dout(dpp, 10) << __func__
                         << "::ACC num_work_shards=" << p_epoch->num_work_shards
                         << "::num_md5_shards=" << p_epoch->num_md5_shards
                         << dendl;
      break;
      ...
      ret = swap_epoch(
          store, dpp, p_epoch,
          static_cast<dedup_req_type_t>(p_epoch->dedup_type), num_work_shards,
          num_md5_shards);
    }
  }
  ...
  ret = verify_all_shard_tokens(
      store, p_epoch->num_work_shards, WORKER_SHARD_PREFIX);
  if (ret != 0) {
    return ret;
  }
  return verify_all_shard_tokens(
      store, p_epoch->num_md5_shards, MD5_SHARD_PREFIX);
}

//---------------------------------------------------------------------------
int
cluster::cleanup_prev_run(rgw::sal::RadosStore* store)
{
  librados::IoCtx ctl_ioctx;
  int ret = get_control_ioctx(store, dpp, ctl_ioctx);
  if (unlikely(ret != 0)) {
  ...
  do {
    ...
    int ret =
        rgw_list_pool(dpp, ctl_ioctx, max, filter, marker, &oids, &truncated);
    if (ret == -ENOENT) {
      ldpp_dout(dpp, 10) << __func__ << "::rgw_list_pool() ret == -ENOENT"
                         << dendl;
      break;
    } else if (ret < 0) {
    ...
    for (const std::string& oid : oids) {
      ...
      if (d_epoch_time < mtime) {
        ldpp_dout(dpp, 10) << __func__ << "::skipping new obj! " << "::EPOCH={"
                           << d_epoch_time.tv.tv_sec << ":"
                           << d_epoch_time.tv.tv_nsec << "} " << "::mtime={"
                           << mtime.tv.tv_sec << ":" << mtime.tv.tv_nsec << "}"
                           << dendl;
        skipped_count++;
        continue;
      }
    ...
    const char* prefix)
{
  ...
  for (unsigned shard = 0; shard < shards_count; shard++) {
    ...
    ret = ctl_ioctx.create(oid, exclusive);
    if (ret == 0) {
      ldpp_dout(dpp, 15) << __func__ << "::oid=" << oid << " was created!"
                         << dendl;
    } else if (ret == -EEXIST) {
      ldpp_dout(dpp, 15) << __func__ << "::failed ctl_ioctx.create(" << oid
                         << ") -EEXIST!" << dendl;
    } else {
      // TBD: can it happen legally ?
      ldpp_dout(dpp, 1) << __func__ << "::failed ctl_ioctx.create(" << oid
                        << ") with: " << ret << "::" << cpp_strerror(-ret)
                        << dendl;
    }
  }
...
//---------------------------------------------------------------------------
int
cluster::verify_all_shard_tokens(
    rgw::sal::RadosStore* store,
    unsigned shards_count,
    const char* prefix)
{
...
//---------------------------------------------------------------------------
int
cluster::update_shard_token_heartbeat(
    rgw::sal::RadosStore* store,
    unsigned shard,
    uint64_t count_a,
    uint64_t count_b,
...
//---------------------------------------------------------------------------
int
cluster::mark_shard_token_completed(
    rgw::sal::RadosStore* store,
    unsigned shard,
    uint64_t obj_count,
    const char* prefix,
    ...
    const char* prefix)
{
  ...
  for (auto shard = start_shard; shard < max_shard; shard++) {
    ...
    op.assert_exists();
    rados::cls::lock::lock(
        &op, oid, ClsLockType::EXCLUSIVE, d_lock_cookie, lock_tag,
        "dedup_shard_token", lock_duration, lock_flags);
    ...
    bufferlist empty_bl;
    shard_progress_t sp(
        SP_NO_OBJECTS, SP_NO_OBJECTS, false, d_cluster_id, empty_bl);
...
//---------------------------------------------------------------------------
work_shard_t
cluster::get_next_work_shard_token(
    rgw::sal::RadosStore* store,
    work_shard_t num_work_shards)
{
  int32_t shard = get_next_shard_token(
      store, d_curr_worker_shard, num_work_shards, WORKER_SHARD_PREFIX);
...
//---------------------------------------------------------------------------
md5_shard_t
cluster::get_next_md5_shard_token(
    rgw::sal::RadosStore* store,
    md5_shard_t num_md5_shards)
{
  int32_t shard = get_next_shard_token(
      store, d_curr_md5_shard, num_md5_shards, MD5_SHARD_PREFIX);
...
//---------------------------------------------------------------------------
int
cluster::all_shard_tokens_completed(
    rgw::sal::RadosStore* store,
    unsigned shards_count,
    const char* prefix,
    ...
    uint8_t completed_arr[])
{
  ...
  for (unsigned shard = 0; shard < shards_count; shard++) {
    ...
    if (unlikely(ret <= 0)) {
      if (ret != -ENODATA) {
        ldpp_dout(dpp, 10) << __func__
                           << "::failed ctl_ioctx.getxattr() ret=" << ret
                           << "::" << cpp_strerror(-ret) << dendl;
      }
    ...
    try {
      ...
      ldpp_dout(dpp, 1) << __func__ << "::failed shard_progress_t decode!"
                        << dendl;
      completed_arr[shard] = TOKEN_STATE_CORRUPTED;
      // all failures to get valid token state return ENODATA
      err_code = -ENODATA;
  ...
  if (count < shards_count) {
    ...
    ldpp_dout(dpp, 10) << __func__ << "::waiting for " << n << " tokens"
                       << dendl;
  }
  return err_code;
}
    ...
    shard_progress_t* sp_arr)
{
  ...
  for (unsigned shard = 0; shard < shards_count; shard++) {
    ...
    utime_t mtime(tspec);
    if (epoch_time > mtime) {
      ldpp_dout(dpp, 10) << __func__ << "::skipping old obj! " << "::EPOCH={"
                         << epoch_time.tv.tv_sec << ":" << epoch_time.tv.tv_nsec
                         << "} " << "::mtime={" << mtime.tv.tv_sec << ":"
                         << mtime.tv.tv_nsec << "}" << dendl;
      continue;
    }
    ...
    if (ret > 0) {
      try {
        ...
        ldpp_dout(dpp, 10) << __func__ << "::(1)failed shard_progress_t decode!"
                           << dendl;
        return -EINVAL;
      }
    } else if (ret != -ENODATA) {
      ldpp_dout(dpp, 10) << __func__ << "::" << oid
                         << "::failed getxattr() ret=" << ret
                         << "::" << cpp_strerror(-ret) << dendl;
      continue;
    }
    ...
    const char* name)
{
  ...
  ldpp_dout(dpp, 10) << __func__ << "::Got " << name << " stats for shard #"
                     << shard << dendl;
}

//---------------------------------------------------------------------------
static void
show_incomplete_shards_fmt(
    bool has_incomplete_shards,
    unsigned num_shards,
    const shard_progress_t sp_arr[],
    Formatter* fmt)
    ...
    Formatter* fmt)
{
  ...
  if (show_time) {
    uint32_t sec = all_members_time.end_time.tv.tv_sec -
                   all_members_time.start_time.tv.tv_sec;
    ...
    fmt->dump_unsigned(
        "aggregated time (sec)", all_members_time.aggregated_time.tv.tv_sec);
  }
...
//---------------------------------------------------------------------------
static void
show_dedup_ratio_estimate_fmt(
    const worker_stats_t& wrk_stats_sum,
...
//---------------------------------------------------------------------------
static void
show_dedup_ratio_actual_fmt(
    const worker_stats_t& wrk_stats_sum,
    const md5_stats_t& md5_stats_sum,
    Formatter* fmt)
{
  uint64_t s3_bytes_before = wrk_stats_sum.ingress_obj_bytes;
  uint64_t s3_dedup_bytes =
      (md5_stats_sum.deduped_objects_bytes +
...
int
cluster::collect_all_shard_stats(
    rgw::sal::RadosStore* store,
    Formatter* fmt,
    const DoutPrefixProvider* dpp)
{
  ...
  worker_stats_t wrk_stats_sum;
  {
    ...
    int cnt = collect_shard_stats(
        store, dpp, epoch.time, num_work_shards, WORKER_SHARD_PREFIX, bl_arr,
        sp_arr);
    ...
    for (unsigned shard = 0; shard < num_work_shards; shard++) {
      ...
      try {
        ...
        std::cerr << __func__ << "::(2)failed worker_stats_t decode #" << shard
                  << std::endl;
        continue;
      }
      collect_single_shard_stats(
          dpp, owner_map, sp_arr, shard, &show_time, "WORKER");
    }
    Formatter::ObjectSection worker_stats(*fmt, "worker_stats");
    wrk_stats_sum.dump(fmt);
    show_incomplete_shards_fmt(
        has_incomplete_shards, num_work_shards, sp_arr, fmt);
  ...
  if (completed_work_shards_count == num_work_shards) {
    ...
    int cnt = collect_shard_stats(
        store, dpp, epoch.time, num_md5_shards, MD5_SHARD_PREFIX, bl_arr,
        sp_arr);
    ...
    for (unsigned shard = 0; shard < num_md5_shards; shard++) {
      ...
      try {
        ...
        // TBD: can we use std::cerr or should we use formatter ??
        std::cerr << __func__ << "::failed md5_stats_t decode #" << shard
                  << std::endl;
        continue;
      }
      collect_single_shard_stats(
          dpp, owner_map, sp_arr, shard, &show_time, "MD5");
    }
    {
      Formatter::ObjectSection outer(*fmt, "md5_stats");
      md5_stats_sum.dump(fmt);
      show_incomplete_shards_fmt(
          has_incomplete_shards, num_md5_shards, sp_arr, fmt);
    ...
    librados::WatchCtx2* ctx)
{
  ...
  ldpp_dout(dpp, 5) << "dedup_bg::watch_reload(): Started watching " << oid
                    << "::watch_handle=" << *p_watch_handle << dendl;
  return 0;
}

//---------------------------------------------------------------------------
int
cluster::unwatch_reload(
    rgw::sal::RadosStore* store,
    const DoutPrefixProvider* dpp,
...
//---------------------------------------------------------------------------
static void
report_throttle_state(const struct rgw::dedup::control_t& ctl)
{
    ...
    bufferlist urgent_msg_bl)
{
  ...
  bufferlist reply_bl;
  ret = rgw_rados_notify(
      dpp, ctl_ioctx, DEDUP_WATCH_OBJ, urgent_msg_bl, timeout_ms, &reply_bl,
      null_yield);
  if (ret < 0) {
    ldpp_dout(dpp, 1) << __func__ << "::failed rgw_rados_notify("
                      << DEDUP_WATCH_OBJ << ")::err=" << cpp_strerror(-ret)
                      << dendl;
    return ret;
  }
  ...
  for (auto& ack : acks) {
    try {
      ...
      decode(ctl, iter);
      ldpp_dout(dpp, 10) << __func__ << "::++ACK::ctl=" << ctl
                         << "::ret=" << ret << dendl;
    ...
    urgent_msg_t urgent_msg)
{
  ...
  if (urgent_msg != URGENT_MSG_RESUME && urgent_msg != URGENT_MSG_PASUE &&
      urgent_msg != URGENT_MSG_RESTART && urgent_msg != URGENT_MSG_ABORT) {
    ldpp_dout(dpp, 1) << __func__ << "::illegal urgent_msg=" << urgent_msg
                      << dendl;
    return -EINVAL;
  }
    ...
    const DoutPrefixProvider* dpp)
{
  ...
  if (ret != 0) {
    ...
    ldpp_dout(dpp, 1) << __func__
                      << "::set empty EPOCH using cluster_id: " << cluster_id
                      << dendl;
...
//---------------------------------------------------------------------------
bool
cluster::can_start_new_scan(rgw::sal::RadosStore* store)
{
  ...
  if (new_epoch.time <= d_epoch_time) {
    if (new_epoch.time == d_epoch_time) {
      ldpp_dout(dpp, 10) << __func__
                         << "::Epoch hasn't change - > Do not restart scan!!"
                         << dendl;
    } else {
      ldpp_dout(dpp, 1) << __func__
                        << " ::Do not restart scan!\n    epoch=" << d_epoch_time
