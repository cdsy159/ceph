...
public:
  ...
  public:
    ...
    //---------------------------------------------------------------------------
    void
    set_shard(uint16_t shard)
    {
      int n = snprintf(
          this->buff + this->prefix_len, BUFF_SIZE - this->prefix_len, "%03x",
          shard);
    ...
    //---------------------------------------------------------------------------
    static bool
    legal_oid_name(const std::string& oid)
    {
      return (
          (oid.length() <= BUFF_SIZE) && (oid.starts_with(WORKER_SHARD_PREFIX) ||
  ...
  work_shard_t get_next_work_shard_token(
      rgw::sal::RadosStore* store,
      work_shard_t num_work_shards);
  md5_shard_t get_next_md5_shard_token(
      rgw::sal::RadosStore* store,
      md5_shard_t num_md5_shards);
  bool can_start_new_scan(rgw::sal::RadosStore* store);
  static int collect_all_shard_stats(
      rgw::sal::RadosStore* store,
      Formatter* p_formatter,
      const DoutPrefixProvider* dpp);
  static int watch_reload(
      rgw::sal::RadosStore* store,
      const DoutPrefixProvider* dpp,
      uint64_t* p_watch_handle,
      librados::WatchCtx2* ctx);
  static int unwatch_reload(
      rgw::sal::RadosStore* store,
      const DoutPrefixProvider* dpp,
      uint64_t watch_handle);
  static int ack_notify(
      rgw::sal::RadosStore* store,
      const DoutPrefixProvider* dpp,
      ...
      uint64_t cookie,
      int status);
  static int dedup_control_bl(
      rgw::sal::RadosStore* store,
      const DoutPrefixProvider* dpp,
      urgent_msg_t urgent_msg,
      bufferlist urgent_msg_bl);
  static int dedup_control(
      rgw::sal::RadosStore* store,
      const DoutPrefixProvider* dpp,
      urgent_msg_t urgent_msg);
  static int dedup_restart_scan(
      rgw::sal::RadosStore* store,
      ...
      const worker_stats_t* p_stats)
  {
    ...
    return mark_shard_token_completed(
        store, work_shard, p_stats->ingress_obj, WORKER_SHARD_PREFIX, bl);
  }

  //---------------------------------------------------------------------------
  int
  mark_md5_shard_token_completed(
      rgw::sal::RadosStore* store,
      ...
      const md5_stats_t* p_stats)
  {
    ...
    return mark_shard_token_completed(
        store, md5_shard, p_stats->loaded_objects, MD5_SHARD_PREFIX, bl);
  }

  int update_shard_token_heartbeat(
      rgw::sal::RadosStore* store,
      unsigned shard,
      uint64_t count_a,
      uint64_t count_b,
  ...
  //---------------------------------------------------------------------------
  int
  all_work_shard_tokens_completed(
      rgw::sal::RadosStore* store,
      work_shard_t num_work_shards)
  {
    return all_shard_tokens_completed(
        store, num_work_shards, WORKER_SHARD_PREFIX, &d_num_completed_workers,
        d_completed_workers);
  }

  //---------------------------------------------------------------------------
  int
  all_md5_shard_tokens_completed(
      rgw::sal::RadosStore* store,
      md5_shard_t num_md5_shards)
  {
    return all_shard_tokens_completed(
        store, num_md5_shards, MD5_SHARD_PREFIX, &d_num_completed_md5,
        d_completed_md5);
  }
...
private:
  ...
  void clear();
  int all_shard_tokens_completed(
      rgw::sal::RadosStore* store,
  ...
  int32_t get_next_shard_token(
      rgw::sal::RadosStore* store,
      uint16_t start_shard,
      uint16_t max_count,
      const char* prefix);
  int create_shard_tokens(
      rgw::sal::RadosStore* store,
      unsigned shards_count,
      const char* prefix);
  int verify_all_shard_tokens(
      rgw::sal::RadosStore* store,
      unsigned shards_count,
      const char* prefix);
  int mark_shard_token_completed(
      rgw::sal::RadosStore* store,
      unsigned shard,
      uint64_t obj_count,
      const char* prefix,
