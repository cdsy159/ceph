...
//---------------------------------------------------------------------------
size_t
disk_record_t::serialize(char* buff) const
{
  ...
  if (this->s.flags.is_fastlane()) {
    ...
    const char* p_manifest =
        const_cast<disk_record_t*>(this)->manifest_bl.c_str();
...
//---------------------------------------------------------------------------
size_t
disk_record_t::length() const
{
  return (
      sizeof(this->s) + this->obj_name.length() + this->bucket_name.length() +
      this->bucket_id.length() + this->tenant_name.length() +
      this->instance.length() + this->stor_class.length() +
      this->ref_tag.length() + this->manifest_bl.length());
}

//---------------------------------------------------------------------------
int
disk_record_t::validate(
    const char* caller,
    ...
    record_id_t rec_id) const
{
  ...
  if (this->s.rec_version != 0) {
    ...
    ldpp_dout(dpp, 5) << __func__ << "::" << caller
                      << "::ERR: Bad record version: " << this->s.rec_version
  ...
  ldpp_dout(dpp, 5) << __func__ << "::" << caller
                    << "::ERR: record size too big: " << this->length()
                    << "::block_id=" << block_id << "::rec_id=" << rec_id
                    << dendl;
...
//---------------------------------------------------------------------------
std::ostream&
operator<<(std::ostream& stream, const disk_record_t& rec)
{
...
//---------------------------------------------------------------------------
void
disk_block_t::init(work_shard_t worker_id, uint32_t seq_number)
{
...
//---------------------------------------------------------------------------
int
disk_block_header_t::verify(
    disk_block_id_t expected_block_id,
    const DoutPrefixProvider* dpp)
{
  if (unlikely(offset != BLOCK_MAGIC && offset != LAST_BLOCK_MAGIC)) {
    ldpp_dout(dpp, 1) << __func__ << "::ERR::bad magic number (0x" << std::hex
                      << offset << std::dec << ")" << dendl;
    return -EINVAL;
  }

  if (unlikely(rec_count > MAX_REC_IN_BLOCK)) {
    ldpp_dout(dpp, 1) << __func__ << "::ERR::rec_count=" << rec_count
                      << " > MAX_REC_IN_BLOCK" << dendl;
    return -EINVAL;
  }
...
//---------------------------------------------------------------------------
void
disk_block_t::close_block(const DoutPrefixProvider* dpp, bool has_more)
{
  disk_block_header_t* p_header = get_header();
  ldpp_dout(dpp, 20) << __func__ << "::rec_count=" << p_header->rec_count
...
//---------------------------------------------------------------------------
void
disk_block_header_t::deserialize()
{
...
//---------------------------------------------------------------------------
std::ostream&
operator<<(std::ostream& out, const disk_block_id_t& block_id)
{
  std::ios_base::fmtflags flags = out.flags();
  out << std::hex << "0x" << (uint32_t)block_id.get_work_shard_id()
...
//---------------------------------------------------------------------------
std::string
disk_block_id_t::get_slab_name(md5_shard_t md5_shard) const
{
  ...
  unsigned n = snprintf(
      name_buf, sizeof(name_buf), SLAB_NAME_FORMAT, md5_shard, work_id, slab_id);
    ...
    const DoutPrefixProvider* dpp)
{
  ...
  ldpp_dout(dpp, 20) << __func__ << "::worker_id=" << (uint32_t)worker_id
                     ...
                     << ", seq_number=" << seq_number << ":: oid=" << oid
                     << dendl;
#ifndef DEBUG_FRAGMENTED_BUFFERLIST
  int ret = ioctx.read(oid, bl_out, 0, 0);
  if (ret > 0) {
    ldpp_dout(dpp, 20) << __func__ << "::oid=" << oid
                       << ", len=" << bl_out.length() << dendl;
  }
#else
    ...
    const DoutPrefixProvider* dpp)
{
  disk_block_id_t block_id(worker_id, seq_number);
  std::string oid(block_id.get_slab_name(md5_shard));
  ldpp_dout(dpp, 20) << __func__ << "::oid=" << oid << ", len=" << bl.length()
                     << dendl;
  ceph_assert(bl.length());

  int ret = ioctx.write_full(oid, bl);
  if (ret == 0) {
    ldpp_dout(dpp, 20) << __func__ << "::SLAB was written successfully"
                       << dendl;
  } else {
    ldpp_dout(dpp, 1) << "ERROR: failed to write " << oid << "::ret=" << ret
...
//---------------------------------------------------------------------------
int
disk_block_seq_t::flush(librados::IoCtx& ioctx)
{
...
//---------------------------------------------------------------------------
int
disk_block_seq_t::flush_disk_records(librados::IoCtx& ioctx)
{
  ceph_assert(p_arr);
  ...
  if (p_curr_block == &p_arr[0] && p_curr_block->is_empty()) {
    ldpp_dout(dpp, 20) << __func__
                       << "::Empty buffers, generate terminating block"
                       << dendl;
  }
    ...
    record_info_t* p_rec_info) // OUT-PARAM
{
  ...
  if (p_rec_info->rec_id < MAX_REC_IN_BLOCK) {
    ...
    ldpp_dout(dpp, 20) << __func__ << "::Block is full-> close and move to next"
                       << dendl;
    p_stats->egress_blocks++;
...
//---------------------------------------------------------------------------
disk_block_array_t::disk_block_array_t(
    const DoutPrefixProvider* dpp,
    uint8_t* raw_mem,
    ...
    md5_shard_t num_md5_shards)
{
  ...
  for (unsigned md5_shard = 0; md5_shard < d_num_md5_shards; md5_shard++) {
    ldpp_dout(dpp, 20) << __func__ << "::p=" << p << "::p_end=" << p_end
                       << dendl;
    if (p + DISK_BLOCK_COUNT <= p_end) {
      ...
      ldpp_dout(dpp, 1) << __func__ << "::ERR: buffer overflow! "
                        << "::md5_shard=" << md5_shard << "/"
                        << d_num_md5_shards << "::raw_mem_size=" << raw_mem_size
                        << dendl;
...
//---------------------------------------------------------------------------
void
disk_block_array_t::flush_output_buffers(
    const DoutPrefixProvider* dpp,
    librados::IoCtx& ioctx)
{
  for (md5_shard_t md5_shard = 0; md5_shard < d_num_md5_shards; md5_shard++) {
    ldpp_dout(dpp, 20) << __func__
                       << "::flush buffers:: worker_id=" << d_worker_id
