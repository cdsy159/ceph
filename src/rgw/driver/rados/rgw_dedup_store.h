...
public:
  ...
  disk_block_id_t&
  operator=(const disk_block_id_t& other)
  {
    this->block_id = other.block_id;
    return *this;
  }

  inline disk_block_id_t&
  ...
  inline bool
  operator==(const disk_block_id_t& other) const
  {
  ...
  friend std::ostream& operator<<(
      std::ostream& os,
      const disk_block_id_t& block_id);

  std::string get_slab_name(md5_shard_t md5_shard) const;

  static inline slab_id_t
  seq_num_to_slab_id(uint32_t seq_number)
  {
    return (seq_number & SLAB_ID_MASK) >> SLAB_ID_SHIFT;
  }

  static inline uint32_t
  slab_id_to_seq_num(uint32_t slab_id)
  {
    return (slab_id << SLAB_ID_SHIFT);
  }

  inline block_offset_t
  get_block_offset() const
  {
    return get_block_offset(get_seq_num());
  }

  inline work_shard_t
  get_work_shard_id() const
  {
...
private:
  ...
  inline block_offset_t
  get_block_offset(uint32_t seq_number) const
  {
...
public:
  ...
  inline bool
  hash_calculated() const
  {
    return ((flags & RGW_RECORD_FLAG_HASH_CALCULATED) != 0);
  }
  ...
  inline void
  clear_hash_calculated()
  {
    flags &= ~RGW_RECORD_FLAG_HASH_CALCULATED;
  }

  inline bool
  has_valid_hash() const
  {
    return ((flags & RGW_RECORD_FLAG_HAS_VALID_HASH) != 0);
  }

  inline void
  set_has_valid_hash()
  {
    flags |= RGW_RECORD_FLAG_HAS_VALID_HASH;
  }

  inline bool
  has_shared_manifest() const
  {
    return ((flags & RGW_RECORD_FLAG_SHARED_MANIFEST) != 0);
  }

  inline void
  set_shared_manifest()
  {
    flags |= RGW_RECORD_FLAG_SHARED_MANIFEST;
  }

  inline bool
  is_fastlane() const
  {
    return ((flags & RGW_RECORD_FLAG_FASTLANE) != 0);
  }

  inline void
  set_fastlane()
  {
    flags |= RGW_RECORD_FLAG_FASTLANE;
  }

  inline bool
  is_split_head() const
  {
    return ((flags & RGW_RECORD_FLAG_SPLIT_HEAD) != 0);
  }

  inline void
  set_split_head()
  {
    flags |= RGW_RECORD_FLAG_SPLIT_HEAD;
  }

  inline bool
  is_ref_tag_from_tail() const
  {
    return ((flags & RGW_RECORD_FLAG_TAIL_REFTAG) != 0);
  }

  inline void
  set_ref_tag_from_tail()
  {
    flags |= RGW_RECORD_FLAG_TAIL_REFTAG;
  }
...
static constexpr unsigned MAX_REC_SIZE =
    (DISK_BLOCK_SIZE - sizeof(disk_block_header_t));

struct __attribute__((packed)) disk_block_t {
  const disk_block_header_t*
  get_header() const
  {
    return (disk_block_header_t*)data;
  }
...
private:
  ...
  inline const disk_block_t*
  last_block()
  {
    return &p_arr[DISK_BLOCK_COUNT - 1];
  }
...
public:
  ...
  disk_block_seq_t*
  get_shard_block_seq(uint64_t md5_low)
  {
