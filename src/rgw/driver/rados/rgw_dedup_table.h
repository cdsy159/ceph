...
struct key_t {
  ...
  bool
  operator==(const struct key_t& other) const
  {
  ...
  uint32_t
      size_4k_units; // Object size in 4KB units max out at 16TB (AWS MAX-SIZE is 5TB)
  uint16_t
      num_parts; // How many parts were used in multipart upload (AWS MAX-PART is 10,000)
...
class dedup_table_t {
  ...
  public:
    ...
    {
      return ((flags & RGW_TABLE_FLAG_HAS_VALID_HASH) != 0);
    }

    inline void
    set_has_valid_hash()
    {
      flags |= RGW_TABLE_FLAG_HAS_VALID_HASH;
    }

    inline bool
    has_shared_manifest() const
    {
      return ((flags & RGW_TABLE_FLAG_SHARED_MANIFEST) != 0);
    }

    inline void
    set_shared_manifest()
    {
      flags |= RGW_TABLE_FLAG_SHARED_MANIFEST;
    }

    inline bool
    is_occupied() const
    {
      return ((this->flags & RGW_TABLE_FLAG_OCCUPIED) != 0);
    }

    inline void
    set_occupied()
    {
      this->flags |= RGW_TABLE_FLAG_OCCUPIED;
    }
...
public:
  ...
  struct value_t {
    ...
    has_shared_manifest() const
    {
      return flags.has_shared_manifest();
    }
    ...
    has_valid_hash() const
    {
      return flags.has_valid_hash();
    }

  private:
    inline void
    set_shared_manifest_src()
    {
      this->flags.set_shared_manifest();
    }
    ...
    inline void
    set_has_valid_hash_src()
    {
      this->flags.set_has_valid_hash();
    }
  ...
  int set_shared_manifest_src_mode(
      const key_t* p_key,
      disk_block_id_t block_id,
      record_id_t rec_id);
...
private:
  ...
  void inc_counters(const key_t* p_key, dedup_stats_t* p_dedup_stats);
