...
//---------------------------------------------------------------------------
std::ostream&
operator<<(std::ostream& out, const dedup_req_type_t& dedup_type)
{
...
//---------------------------------------------------------------------------
void
encode(const Throttle& t, ceph::bufferlist& bl)
{
  ENCODE_START(1, 1, bl);
...
//---------------------------------------------------------------------------
void
decode(Throttle& t, ceph::bufferlist::const_iterator& bl)
{
  DECODE_START(1, bl);
...
//---------------------------------------------------------------------------
std::ostream&
operator<<(std::ostream& out, const throttle_action_t& msg)
{
...
//---------------------------------------------------------------------------
std::ostream&
operator<<(std::ostream& out, const throttle_msg_t& msg)
{
...
//---------------------------------------------------------------------------
void
encode(const throttle_action_t& m, ceph::bufferlist& bl)
{
  ENCODE_START(1, 1, bl);
...
//---------------------------------------------------------------------------
void
decode(throttle_action_t& m, ceph::bufferlist::const_iterator& bl)
{
  DECODE_START(1, bl);
  int tmp;
  decode(tmp, bl);
...
//---------------------------------------------------------------------------
void
encode(const throttle_msg_t& m, ceph::bufferlist& bl)
{
  ENCODE_START(1, 1, bl);
  encode(m.vec, bl);
  ENCODE_FINISH(bl);
...
//---------------------------------------------------------------------------
void
decode(throttle_msg_t& m, ceph::bufferlist::const_iterator& bl)
{
  DECODE_START(1, bl);
  decode(m.vec, bl);
  DECODE_FINISH(bl);
...
//---------------------------------------------------------------------------
std::ostream&
operator<<(std::ostream& out, const dedup_stats_t& stats)
{
...
//---------------------------------------------------------------------------
void
encode(const dedup_stats_t& ds, ceph::bufferlist& bl)
{
  ENCODE_START(1, 1, bl);
...
//---------------------------------------------------------------------------
void
decode(dedup_stats_t& ds, ceph::bufferlist::const_iterator& bl)
{
  DECODE_START(1, bl);
...
//---------------------------------------------------------------------------
bool
hex2int(const char* p, const char* p_end, uint64_t* p_val)
{
...
//---------------------------------------------------------------------------
bool
dec2int(const char* p, const char* p_end, uint16_t* p_val)
{
  uint16_t val = 0;
  while (p < p_end) {
    uint8_t byte = *p++;
...
//---------------------------------------------------------------------------
static bool
get_num_parts(const std::string& etag, uint16_t* p_num_parts)
{
...
//---------------------------------------------------------------------------
bool
parse_etag_string(const std::string& etag, parsed_etag_t* parsed_etag)
{
    ...
    if (hex2int(buff, buff + 16, &high)) {
      if (hex2int(buff + 16, buff + 32, &low)) {
        ...
        parsed_etag->num_parts =
            num_parts; // How many parts were used in multipart upload
        return true;
      }
    }
  }
...
const char*
get_next_data_ptr(
    bufferlist::const_iterator& bl_itr,
    char data_buff[],
    size_t len,
...
static const char* s_urgent_msg_names[] = {
    "URGENT_MSG_NONE",   "URGENT_MSG_ABORT",   "URGENT_MSG_PASUE",
    "URGENT_MSG_RESUME", "URGENT_MSG_RESTART", "URGENT_MSG_THROTTLE",
    "URGENT_MSG_INVALID"};

//---------------------------------------------------------------------------
const char*
get_urgent_msg_names(int msg)
{
...
//---------------------------------------------------------------------------
void
worker_stats_t::dump(Formatter* f) const
{
  // main section
  {
...

  {
    ...
    if (this->ingress_skip_too_small) {
      f->dump_unsigned(
          "Ingress skip: too small objs", this->ingress_skip_too_small);
      f->dump_unsigned(
          "Ingress skip: too small bytes", this->ingress_skip_too_small_bytes);
    }
  }
...
//---------------------------------------------------------------------------
std::ostream&
operator<<(std::ostream& out, const worker_stats_t& s)
{
...
//---------------------------------------------------------------------------
void
encode(const worker_stats_t& w, ceph::bufferlist& bl)
{
  ENCODE_START(1, 1, bl);
...
//---------------------------------------------------------------------------
void
decode(worker_stats_t& w, ceph::bufferlist::const_iterator& bl)
{
  DECODE_START(1, bl);
...
//---------------------------------------------------------------------------
std::ostream&
operator<<(std::ostream& out, const md5_stats_t& s)
{
  JSONFormatter formatter(false);
  s.dump(&formatter);
  std::stringstream sstream;
...
//---------------------------------------------------------------------------
void
md5_stats_t::dump(Formatter* f) const
{
  // main section
  {
    Formatter::ObjectSection main(*f, "main");
    ...
    f->dump_unsigned(
        "Already Deduped bytes (prev cycles)",
...

  {
    ...
    if (this->md_throttle_sleep_events) {
      f->dump_unsigned(
          "Metadata Throttle Sleep Events", this->md_throttle_sleep_events);
      f->dump_unsigned(
          "Metadata Throttle Sleep Time (sec)",
    ...
    if (this->skip_shared_tail_objs) {
      f->dump_unsigned(
          "Skip Shared Tail Objs (server-side-copy)",
          this->skip_shared_tail_objs);
    }
    ...
    if (this->manifest_no_tail_placement) {
      f->dump_unsigned(
          "Manifest has no tail placement", this->manifest_no_tail_placement);
    }
...

  {
    ...
    if (this->ingress_skip_encrypted) {
      ...
      f->dump_unsigned(
          "Skipped Encrypted Bytes", this->ingress_skip_encrypted_bytes);
    }
    if (this->ingress_skip_compressed) {
      f->dump_unsigned("Skipped Compressed objs", this->ingress_skip_compressed);
      f->dump_unsigned(
          "Skipped Compressed Bytes", this->ingress_skip_compressed_bytes);
    }
    if (this->ingress_skip_changed_objs) {
      f->dump_unsigned(
          "Skipped Changed Object", this->ingress_skip_changed_objs);
    }
    if (this->ingress_skip_explicit_objs) {
      f->dump_unsigned(
          "Skipped Explicit Objs", this->ingress_skip_explicit_objs);
    }
...

  {
    ...
    if (this->ingress_failed_get_obj_attrs) {
      f->dump_unsigned(
          "Failed get_obj_attrs", this->ingress_failed_get_obj_attrs);
    }
    if (this->ingress_corrupted_etag) {
      f->dump_unsigned("Corrupted ETAG", this->ingress_corrupted_etag);
    }
    if (this->ingress_corrupted_obj_attrs) {
      f->dump_unsigned(
          "Corrupted obj attributes", this->ingress_corrupted_obj_attrs);
    }
    ...
    if (this->missing_last_block_marker) {
      f->dump_unsigned(
          "Failed missing_last_block_marker in rec",
          this->missing_last_block_marker);
    }

    if (this->failed_split_head_creat) {
      f->dump_unsigned(
          "Failed Split-Head Create (EEXIST)", this->failed_split_head_creat);
    }
...

  {
    ...
    if (this->different_storage_class) {
      f->dump_unsigned("different_storage_class", this->different_storage_class);
    }
    if (this->invalid_hash_no_split_head) {
      f->dump_unsigned(
          "Failed rec has invalid hash w/o split-head ",
          this->invalid_hash_no_split_head);
    }
    if (this->invalid_storage_class_mapping) {
      f->dump_unsigned(
          "Failed, invalid_storage_class_mapping",
          this->invalid_storage_class_mapping);
    }
    if (this->singleton_after_purge) {
      f->dump_unsigned(
          "Failed, has singleton after purge", this->singleton_after_purge);
    }
    if (this->shared_manifest_after_purge) {
      f->dump_unsigned(
          "Failed, has shared manifest after purge",
          this->shared_manifest_after_purge);
    }
    if (this->split_head_no_tail_placement) {
      f->dump_unsigned(
          "No Tail Placement during Split-Head processing",
...
//---------------------------------------------------------------------------
void
encode(const md5_stats_t& m, ceph::bufferlist& bl)
{
  ENCODE_START(1, 1, bl);
...
//---------------------------------------------------------------------------
void
decode(md5_stats_t& m, ceph::bufferlist::const_iterator& bl)
{
  DECODE_START(1, bl);
