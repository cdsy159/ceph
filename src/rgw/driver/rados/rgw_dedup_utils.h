...
public:
  ...
  inline void
  set_max_calls_per_sec(uint32_t max_calls_per_sec)
  {
    max_calls = max_calls_per_sec;
  }

  inline size_t
  get_max_calls_per_second() const
  {
    return max_calls;
  }

  inline uint64_t
  get_sleep_events() const
  {
    return sleep_events;
  }

  inline uint64_t
  get_sleep_time_usec() const
  {
  ...
  acquire()
  {
    ...
    uint64_t elapsed_usec =
        duration_cast<microseconds>(now - last_reset).count();
...
byte_size_to_disk_blocks(uint64_t byte_size)
{
...
//---------------------------------------------------------------------------
static inline uint64_t
disk_blocks_to_byte_size(uint64_t disk_blocks)
{
...
static inline uint64_t
calc_on_disk_byte_size(uint64_t byte_size)
{
...
void etag_to_bufferlist(
    uint64_t md5_high,
    uint64_t md5_low,
    uint16_t num_parts,
    ceph::bufferlist* bl);
const char* get_next_data_ptr(
