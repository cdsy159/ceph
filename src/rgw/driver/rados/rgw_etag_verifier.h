...
public:
  ETagVerifier_MPU(
    ...
    ETagVerifier(cct, next), part_ofs(std::move(part_ofs))
  {
    // Allow use of MD5 digest in FIPS mode for non-cryptographic purposes
    hash.SetFlags(EVP_MD_CTX_FLAG_NON_FIPS_ALLOW);
  }
...
constexpr auto max_etag_verifier_size =
    std::max(sizeof(ETagVerifier_Atomic), sizeof(ETagVerifier_MPU));
using etag_verifier_ptr = ceph::static_ptr<ETagVerifier, max_etag_verifier_size>;

int create_etag_verifier(
