...
struct RGWLCCloudTierCtx {
  ...
  RGWLCCloudTierCtx(
      ...
      rgw_bucket_dir_entry& _o,
      rgw::sal::Driver* _driver,
      RGWBucketInfo& _binfo,
      rgw::sal::Object* _obj,
      RGWRESTConn& _conn,
      std::string& _bucket,
...
int rgw_cloud_tier_transfer_object(
    RGWLCCloudTierCtx& tier_ctx,
    std::set<std::string>& cloud_targets);

int rgw_cloud_tier_get_object(
    RGWLCCloudTierCtx& tier_ctx,
    bool head,
    ...
    rgw::sal::Attrs& attrs,
    void* cb);
int rgw_cloud_tier_restore_object(
    RGWLCCloudTierCtx& tier_ctx,
    std::map<std::string, std::string>& headers,
    real_time* pset_mtime,
    std::string& etag,
