...
inline std::ostream&
operator<<(std::ostream& m, const shard_check& t)
{
  switch (t) {
    ...
    bool& fifo_unsupported)
{
  ...
  if (!fifo_unsupported) {
    ...
    auto fifo = co_await fifo::FIFO::open(
        dpp, rados, obj, loc, asio::redirect_error(asio::use_awaitable, ec),
        std::nullopt, true);
    if (!ec) {
      ldpp_dout(dpp, 20) << __PRETTY_FUNCTION__ << ":" << __LINE__
                         << ": obj=" << obj << " is FIFO" << dendl;
  ...
  if (def == log_type::fifo) {
    ...
    try {
      auto fifo = co_await fifo::FIFO::create(
          dpp, rados, obj, loc, asio::use_awaitable);
    ...
    const fu2::unique_function<std::string(int) const>& get_oid)
{
  ...
  for (int i = 0; i < shards; ++i) {
    auto c = co_await probe_shard(
        dpp, rados, neorados::Object(get_oid(i)), loc, fifo_unsupported);
    if (c == shard_check::corrupt)
      throw sys::system_error(EIO, sys::generic_category());
    if (c == shard_check::dne)
      continue;
    ...
    if (check != c) {
      ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__ << ":" << __LINE__
                         << " clashing types: check=" << check << ", c=" << c
                         << dendl;
      throw sys::system_error(EIO, sys::generic_category());
    }
  }
  ...
  if (check == shard_check::dne)
    co_return co_await handle_dne(
        dpp, rados, get_oid(0), loc, def, fifo_unsupported);
    ...
    bool leave_zero)
{
  ...
  for (int i = 0; i < shards; ++i) {
    ...
    auto [info, part_header_size, part_entry_overhead] =
        co_await fifo::FIFO::get_meta(
            rados, oid, loc, std::nullopt,
    ...
    if (!ec && info.head_part_num > -1) {
      for (auto j = info.tail_part_num; j <= info.head_part_num; ++j) {
        ...
        co_await rados.execute(
            info.part_oid(j), loc, neorados::WriteOp{}.remove(),
    ...
    if (i == 0 && leave_zero) {
      ...
      op.set_omap_header({}).clear_omap().truncate(0);
    } else {
      op.remove();
    }
    co_await rados.execute(
        oid, loc, std::move(op), asio::redirect_error(asio::use_awaitable, ec));
  ...
  if (watchcookie > 0) {
    ...
    if (ec) {
      lderr(cct) << __PRETTY_FUNCTION__ << ":" << __LINE__
                 << ": failed unwatching oid=" << oid << ", " << ec.message()
                 << dendl;
    }
    watchcookie = 0;
  }
...
asio::awaitable<void>
logback_generations::setup(const DoutPrefixProvider* dpp, log_type def)
{
  ...
  if (must_create) {
    ...
    // metadata.
    auto type = co_await log_backing_type(
        dpp, rados, loc, def, shards,
        [this](int shard) { return this->get_oid(0, shard); });
    auto op = co_await async::async_dispatch(
        ...
        [this, type] {
          ...
          encode(entries, bl);
          op.create(true).exec(version::set(version)).write_full(std::move(bl));
          return op;
        },
        asio::use_awaitable);
    ...
    if (ec == sys::errc::file_exists) {
      ...
      if (l.gen_id != 0) {
        co_await log_remove(
            dpp, rados, loc, shards,
            [this](int shard) { return this->get_oid(0, shard); }, true);
      }
      co_await async::async_dispatch(
          strand,
          [&] {
  ...
  auto e = co_await async::async_dispatch(
      ...
      [&] {
        ...
        entries_t e;
        std::copy(i, entries.cend(), std::inserter(e, e.end()));
        return e;
      },
      asio::use_awaitable);
  try {
...
asio::awaitable<void>
logback_generations::update(const DoutPrefixProvider* dpp)
{
  entries_t es;
  obj_version v;
  std::tie(es, v) = co_await read(dpp);
  auto [do_nothing, highest_empty, new_entries] = co_await async::async_dispatch(
      strand,
      [&]() -> std::tuple<bool, std::optional<std::uint64_t>, entries_t> {
        ...
        if (new_lowest->first < cur_lowest->first) {
          throw sys::system_error{
              EFAULT, sys::generic_category(),
              "logback_generations::update: INCONSISTENCY! Tail moved wrong "
              "way."};
        }
        ...
        if ((es.end() - 1)->first < (entries.end() - 1)->first) {
          throw sys::system_error{
              EFAULT, sys::generic_category(),
              "logback_generations::update: INCONSISTENCY! Head moved wrong "
              "way."};
        }
        if ((es.end() - 1)->first > (entries.end() - 1)->first) {
...
auto
logback_generations::read(const DoutPrefixProvider* dpp)
    -> asio::awaitable<std::pair<entries_t, obj_version>>
{
  neorados::ReadOp op;
  op.exec(version::check(
      co_await async::async_dispatch(
          strand, [this] { return version; }, asio::use_awaitable),
      VER_COND_GE));
  obj_version v2;
  ...
  sys::error_code ec;
  co_await rados.execute(
      oid, loc, std::move(op), nullptr,
      asio::redirect_error(asio::use_awaitable, ec));
  if (ec) {
    if (ec == sys::errc::no_such_file_or_directory) {
      ldpp_dout(dpp, 5) << __PRETTY_FUNCTION__ << ":" << __LINE__
                        << ": oid=" << oid << " not found" << dendl;
    } else {
      ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__ << ":" << __LINE__
...
asio::awaitable<bool>
logback_generations::write(const DoutPrefixProvider* dpp, entries_t&& e)
{
  ...
  if (!ec) {
    ...
    ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__ << ":" << __LINE__
                       << ": failed reading oid=" << oid << ", " << ec.message()
                       << dendl;
    throw sys::system_error(ec);
  }
  co_return canceled;
...
asio::awaitable<void>
logback_generations::watch()
{
  watchcookie = co_await rados.watch(
      oid, loc, std::nullopt, std::ref(*this), asio::use_awaitable);
  co_return;
}
...
logback_generations::new_backing(const DoutPrefixProvider* dpp, log_type type)
{
  ...
  do {
    co_await update(dpp);
    canceled = co_await asio::co_spawn(
        strand,
        [](logback_generations& l, log_type type, entries_t& new_entries,
           const DoutPrefixProvider* dpp) -> asio::awaitable<bool> {
  ...
  if (tries >= max_tries) {
    ...
    throw sys::system_error(
        ECANCELED, sys::generic_category(), "Exhausted retry attempts");
  }
...
asio::awaitable<void>
logback_generations::empty_to(const DoutPrefixProvider* dpp, uint64_t gen_id)
{
  static constexpr auto max_tries = 10;
  auto tries = 0;
  uint64_t newtail = 0;
  ...
  do {
    ...
    canceled = co_await asio::co_spawn(
        strand,
        [](logback_generations& l, uint64_t gen_id, uint64_t& newtail,
           const DoutPrefixProvider* dpp) -> asio::awaitable<bool> {
          {
            auto last = l.entries.end() - 1;
...
logback_generations::remove_empty(const DoutPrefixProvider* dpp)
{
  ...
  auto tries = 0;
  entries_t new_entries;
  bool nothing_to_do = co_await async::async_dispatch(
      strand,
      [this] {
  ...
  do {
    ...
    co_await async::async_dispatch(
        ...
        [this, now, &es, &es2] {
          std::copy_if(
              entries.cbegin(), entries.cend(), std::inserter(es, es.end()),
              [now](const auto& e) {
                if (!e.second.pruned)
                  return false;
    ...
    canceled = co_await co_spawn(
        strand, write(dpp, std::move(es2)), asio::use_awaitable);
    if (canceled) {
      co_await update(dpp);
    }
    ++tries;
...
asio::awaitable<void>
logback_generations::handle_notify(
    sys::error_code ec,
    uint64_t notify_id,
    uint64_t cookie,
    uint64_t notifier_id,
...
{
  const DoutPrefix dp(
      rados.cct(), dout_subsys, "logback generations handle_notify: ");
  if (!ec) {
    if (notifier_id != my_id) {
      try {
        ...
        ldpp_dout(&dp, -1)
            << __PRETTY_FUNCTION__ << ":" << __LINE__ << ": update failed ("
            << e.what() << "), no one to report to and no safe way to continue."
            << dendl;
        std::terminate();
      }
    }
    co_await rados.notify_ack(
        oid, loc, notify_id, watchcookie, {}, asio::use_awaitable);
  } else {
    ec.clear();
    co_await rados.unwatch(
        watchcookie, loc, asio::redirect_error(asio::use_awaitable, ec));
    if (ec) {
