...
inline void
encode(const log_type& type, ceph::buffer::list& bl)
{
...
inline void
decode(log_type& type, bufferlist::const_iterator& bl)
{
  uint8_t t;
  decode(t, bl);
...
inline std::ostream&
operator<<(std::ostream& m, const log_type& t)
{
  switch (t) {
  case log_type::omap:
...
asio::awaitable<log_type> log_backing_type(
...
inline std::ostream&
operator<<(std::ostream& m, const logback_generation& g)
{
...
private:
  ...
  asio::awaitable<std::pair<entries_t, obj_version>> read(
      const DoutPrefixProvider* dpp);
  ...
  auto
  lowest_nomempty(const entries_t& es)
  {
    return std::find_if(es.begin(), es.end(), [](const auto& e) {
...
public:
  ...
  void operator()(
      ...
      bufferlist&& bl);
  asio::awaitable<void> handle_notify(
      sys::error_code ec,
      uint64_t notify_id,
      uint64_t cookie,
      uint64_t notifier_id,
  ...
  static asio::awaitable<std::unique_ptr<T>>
  init(
      const DoutPrefixProvider* dpp,
      neorados::RADOS r_,
      ...
      Args&&... args)
  {
    std::unique_ptr<T> lg{new T(
        r_, oid_, loc_, std::move(get_oid_), shards_,
  ...
  asio::awaitable<void> new_backing(const DoutPrefixProvider* dpp, log_type type);
...
inline std::string
gencursor(uint64_t gen_id, std::string_view cursor)
{
  return (
      gen_id > 0 ? fmt::format("G{:0>20}@{}", gen_id, cursor)
                 : std::string(cursor));
}
...
class LazyFIFO {
  ...
  asio::awaitable<void>
  lazy_init(const DoutPrefixProvider* dpp)
  {
    std::unique_lock l(m);
    if (fifo) {
      co_return;
      ...
      // race to create them.
      auto fifo_tmp =
          co_await fifo::FIFO::create(dpp, r, oid, loc, asio::use_awaitable);
      l.lock();
      if (!fifo) {
        // We won the race
  ...
  void
  lazy_init(const DoutPrefixProvider* dpp, asio::yield_context y)
  {
    std::unique_lock l(m);
    if (fifo) {
      return;
...
public:
  ...
  asio::awaitable<void>
  push(const DoutPrefixProvider* dpp, std::deque<ceph::buffer::list> entries)
  {
  ...
  asio::awaitable<void>
  trim(const DoutPrefixProvider* dpp, std::string markstr, bool exclusive)
  {
    co_await lazy_init(dpp);
