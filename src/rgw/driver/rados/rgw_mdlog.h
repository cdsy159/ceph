...
public:
  ...
  void
  finish(librados::completion_t cb)
  {
...
class RGWMetadataLog {
  ...
  static std::string
  make_prefix(const std::string& period)
  {
    if (period.empty())
...
public:
  RGWMetadataLog(
    ...
    cct(_cct), prefix(make_prefix(period)), lock("RGWMetaLog::lock")
  {
    svc.zone = _zone_svc;
    svc.cls = _cls_svc;
  }
  ...
  void
  get_shard_oid(int id, std::string& oid) const
  {
    char buf[16];
