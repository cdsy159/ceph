...
void
rgw_shard_name(const string& prefix, unsigned shard_id, string& name)
{
  char buf[16];
    ...
    optional_yield y)
{
  ...
  rgw_shard_name(
      prefix, cct->_conf->rgw_md_log_max_shards, hash_key, oid, &shard_id);
...
int
RGWMetadataLog::get_shard_id(const string& hash_key, int* shard_id)
{
  string oid;
...
void
RGWMetadataLog::init_list_entries(
    int shard_id,
    const real_time& from_time,
...
void
RGWMetadataLog::complete_list_entries(void* handle)
{
    ...
    optional_yield y)
{
  ...
  int ret = svc.cls->timelog.list(
      dpp, ctx->cur_oid, ctx->from_time, ctx->end_time, max_entries, entries,
      ctx->marker, &next_marker, truncated, y);
  if ((ret < 0) && (ret != -ENOENT))
    return ret;
...
static void
_mdlog_info_completion(librados::completion_t cb, void* arg)
{
    ...
    RGWMetadataLogInfoCompletion* completion)
{
  ...
  return svc.cls->timelog.info_async(
      dpp, completion->get_io_obj(), oid, &completion->get_header(),
    ...
    optional_yield y)
{
  ...
  return svc.cls->timelog.trim(
      dpp, oid, from_time, end_time, start_marker, end_marker, nullptr, y);
}

int
RGWMetadataLog::lock_exclusive(
    const DoutPrefixProvider* dpp,
    int shard_id,
    timespan duration,
    string& zone_id,
    string& owner_id)
{
  ...
  return svc.cls->lock.lock_exclusive(
      dpp, svc.zone->get_zone_params().log_pool, oid, duration, zone_id,
      owner_id);
}

int
RGWMetadataLog::unlock(
    const DoutPrefixProvider* dpp,
    int shard_id,
    string& zone_id,
    string& owner_id)
{
  ...
  return svc.cls->lock.unlock(
      dpp, svc.zone->get_zone_params().log_pool, oid, zone_id, owner_id);
}

void
RGWMetadataLog::mark_modified(int shard_id)
{
  lock.get_read();
...
void
RGWMetadataLog::read_clear_modified(set<int>& modified)
{
  std::unique_lock wl{lock};
...
void
RGWMetadataLogInfo::dump(Formatter* f) const
{
...
void
RGWMetadataLogInfo::decode_json(JSONObj* obj)
{
