...
public:
  ...
  void
  set_pattrs(std::map<std::string, bufferlist>* _pattrs)
  {
    pattrs = _pattrs;
  }
...
public:
  ...
  virtual RGWMetadataObject* get_meta_obj(
      JSONObj* jo,
      const obj_version& objv,
      const ceph::real_time& mtime) = 0;
  ...
  virtual int list_keys_init(
      const DoutPrefixProvider* dpp,
      const std::string& marker,
      void** phandle) = 0;
  virtual int list_keys_next(
      const DoutPrefixProvider* dpp,
      void* handle,
      int max,
  ...
  virtual int
  get_shard_id(const std::string& entry, int* shard_id)
  {
    *shard_id = 0;
    return 0;
  }
