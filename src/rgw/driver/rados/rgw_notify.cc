...
operator<<(std::ostream& out, const event_entry_t& e)
{
  std::string host;
  std::string user;
  std::string password;
...
using entries_persistency_tracker =
    std::unordered_map<std::string, persistency_tracker>;
using queues_persistency_tracker =
    std::unordered_map<std::string, entries_persistency_tracker>;
...
void
publish_commit_completion(rados_completion_t completion, void* arg)
{
  std::unique_ptr<PublishCommitCompleteArg> pcc_args{
      reinterpret_cast<PublishCommitCompleteArg*>(arg)};
  if (const auto rc = rados_aio_get_return_value(completion); rc < 0) {
    ldout(pcc_args->cct, 1)
        << "ERROR: failed to commit reservation to queue: "
...
class Manager : public DoutPrefixProvider {
  ...
  bool shutdown = false;
  static constexpr auto queues_update_period =
      std::chrono::milliseconds(30000); // 30s
  static constexpr auto queues_update_retry =
      std::chrono::milliseconds(1000); // 1s
  static constexpr auto queue_idle_sleep =
  ...
  std::thread worker;
  static constexpr auto stale_reservations_period =
      std::chrono::seconds(120); // 120s
  static constexpr auto reservations_cleanup_period =
...
private:
  ...
  {
    return out << "rgw notify: ";
  }

  // read the list of queues from the queue list object
  int
  read_queue_list(queues_t& queues, optional_yield y)
  {
    ...
    while (more) {
      ...
      const auto ret = rgw_rados_operate(
          this, rados_store->getRados()->get_notif_pool_ctx(),
          Q_LIST_OBJECT_NAME, std::move(op), nullptr, y);
      if (ret == -ENOENT) {
      ...
      if (ret < 0) {
        ...
        ldpp_dout(this, 1) << "ERROR: failed to read queue list. error: " << ret
                           << dendl;
        return ret;
      }
  ...
  static int
  set_min_marker(std::string& m1, const std::string m2)
  {
  ...
  public:
    ...
    tokens_waiter(DoutPrefixProvider* _dpp) :
      dpp(_dpp)
    {}
    ...
    void
    async_wait(boost::asio::yield_context yield)
    {
      if (pending_tokens == 0) {
        return;
      }
      ...
      ldpp_dout(dpp, 20)
          << "INFO: tokens waiter finished waiting for all tokens" << dendl;
    }
  };
  ...
  std::vector<std::string> entryProcessingResultString = {
      ...
      int& ret)
  {
    ...
    try {
      ...
      ldpp_dout(this, 1) << "WARNING: failed to decode entry marker: "
                         << entry.marker << ". error: " << err.what() << dendl;
    ...
    const auto topic_persistency_ttl =
        event_entry.time_to_live != DEFAULT_GLOBAL_VALUE
            ? event_entry.time_to_live
            : conf->rgw_topic_persistency_time_to_live;
    const auto topic_persistency_max_retries =
        event_entry.max_retries != DEFAULT_GLOBAL_VALUE
            ? event_entry.max_retries
            : conf->rgw_topic_persistency_max_retries;
    const auto topic_persistency_sleep_duration =
        event_entry.retry_sleep_duration != DEFAULT_GLOBAL_VALUE
            ? event_entry.retry_sleep_duration
            : conf->rgw_topic_persistency_sleep_duration;
    const auto time_now = ceph::coarse_real_clock::now();
    if ((topic_persistency_ttl != 0 &&
         event_entry.creation_time != ceph::coarse_real_clock::zero() &&
         time_now - event_entry.creation_time >
             std::chrono::seconds(topic_persistency_ttl)) ||
        (topic_persistency_max_retries != 0 &&
         entry_persistency_tracker.retries_num >
             topic_persistency_max_retries)) {
    ...
    if (time_now - entry_persistency_tracker.last_retry_time <
        std::chrono::seconds(topic_persistency_sleep_duration)) {
    ...
    if (ret < 0) {
      ldpp_dout(this, 5) << "WARNING: push entry marker: " << entry.marker
                         << " failed." << " (will retry) for event with "
                         << event_entry << " ret=" << ret << dendl;
  ...
  void
  cleanup_queue(const std::string& queue_name, boost::asio::yield_context yield)
  {
    ...
    while (!shutdown) {
      ...
      if (ceph::coarse_real_clock::now() > next_check_time) {
        next_check_time = ceph::coarse_real_clock::now() +
                          reservations_cleanup_period;
      } else {
      ...
      ldpp_dout(this, 20)
          << "INFO: performing stale reservation cleanup for queue: "
          << queue_name << ". next cleanup will happen at: " << std::ctime(&tp)
          << dendl;
      const auto stale_time = ceph::coarse_real_time::clock::now() -
                              stale_reservations_period;
      librados::ObjectWriteOperation op;
      op.assert_exists();
      rados::cls::lock::assert_locked(
          &op, queue_name + "_lock", ClsLockType::EXCLUSIVE, lock_cookie,
          "" /*no tag*/);
      ...
      auto ret = rgw_rados_operate(
          this, rados_store->getRados()->get_notif_pool_ctx(), queue_name,
          std::move(op), yield);
      if (ret == -ENOENT) {
        // queue was deleted
      ...
      if (ret < 0) {
        ldpp_dout(this, 5) << "WARNING: failed to cleanup stale reservation "
                              "from queue and/or lock queue: "
                           << queue_name << ". error: " << ret << dendl;
      }
    }
    ldpp_dout(this, 5) << "INFO: manager stopped. done cleanup for queue: "
                       << queue_name << dendl;
  }

  // unlock (lose ownership) queue
  int
  unlock_queue(const std::string& queue_name, boost::asio::yield_context yield)
  {
    librados::ObjectWriteOperation op;
    op.assert_exists();
    ...
    const auto ret =
        rgw_rados_operate(this, rados_ioctx, queue_name, std::move(op), yield);
    if (ret == -ENOENT) {
      ldpp_dout(this, 10) << "INFO: queue: " << queue_name
  ...
  void
  process_queue(const std::string& queue_name, boost::asio::yield_context yield)
  {
    ...
    boost::asio::spawn(
        make_strand(io_context), std::allocator_arg, make_stack_allocator(),
    ...
    while (!shutdown) {
      ...
      auto total_entries = 0U;
      {
        ...
        auto ret = rgw_rados_operate(
            this, rados_ioctx, queue_name, std::move(op), nullptr, yield);
        if (ret == -ENOENT) {
          // queue was deleted
        ...
        if (ret < 0) {
          ldpp_dout(this, 5)
              << "WARNING: failed to get list of entries in queue and/or lock "
                 "queue: "
              << queue_name << ". error: " << ret << " (will retry)" << dendl;
          continue;
        }
        ret = cls_2pc_queue_list_entries_result(
            obl, entries, &truncated, end_marker);
        if (ret < 0) {
          ldpp_dout(this, 5)
              << "WARNING: failed to parse list of entries in queue: "
              << queue_name << ". error: " << ret << " (will retry)" << dendl;
          continue;
        }
      }
      ...
      for (auto& entry : entries) {
        ...
        entries_persistency_tracker& notifs_persistency_tracker =
            topics_persistency_tracker[queue_name];
        tokens_waiter::token token(&tw);
        boost::asio::spawn(
            yield, std::allocator_arg, make_stack_allocator(),
            [this, &is_idle, &notifs_persistency_tracker, &queue_name,
             entry_idx, total_entries, &end_marker, &remove_entries,
             &stop_processing, token = std::move(token), &entry,
             &needs_migration_vector, push_endpoint = push_endpoint.get(),
             &topic_info](boost::asio::yield_context yield) {
              auto& persistency_tracker =
                  notifs_persistency_tracker[entry.marker];
              int result_code;
              auto result = process_entry(
                  this->get_cct()->_conf, persistency_tracker, entry,
                  push_endpoint, topic_info, yield, result_code);
              if (result == EntryProcessingResult::Successful ||
                  result == EntryProcessingResult::Expired ||
                  result == EntryProcessingResult::Migrating) {
                ldpp_dout(this, 20)
                    << "INFO: processing of entry: " << entry.marker << " ("
                    << entry_idx << "/" << total_entries
                    << ") from: " << queue_name << " "
                    << entryProcessingResultString[static_cast<unsigned int>(
                           result)]
                    << dendl;
                remove_entries = true;
                needs_migration_vector[entry_idx - 1] =
                    (result == EntryProcessingResult::Migrating);
                notifs_persistency_tracker.erase(entry.marker);
                is_idle = false;
                if (result == EntryProcessingResult::Expired && perfcounter)
                  perfcounter->inc(l_rgw_pubsub_push_failed);
                return;
              }
              if (set_min_marker(end_marker, entry.marker) < 0) {
                ldpp_dout(this, 1)
                    << "ERROR: cannot determine minimum between malformed "
                       "markers: "
                    << end_marker << ", " << entry.marker << dendl;
              } else {
                ldpp_dout(this, 20)
                    << "INFO: new end marker for removal: " << end_marker
                    << " from: " << queue_name << dendl;
              }
              if (result == EntryProcessingResult::Sleeping) {
                ldpp_dout(this, 20)
                    << "INFO: skipped processing of entry: " << entry.marker
                    << " (" << entry_idx << "/" << total_entries
                    << ") from: " << queue_name << dendl;
              } else {
                is_idle = (result_code == -EBUSY);
                ldpp_dout(this, 20)
                    << "INFO: failed processing of entry: " << entry.marker
                    << " (" << entry_idx << "/" << total_entries
                    << ") from: " << queue_name
      ...
      if (remove_entries) {
        ...
        for (const auto& entry : entries) {
          ...
          if (needs_migration_vector[index]) {
            ldpp_dout(this, 20) << "INFO: migrating entry " << entry.marker
                                << " from: " << queue_name << dendl;
        ...
        auto ret = rgw_rados_operate(
            this, rados_ioctx, queue_name, std::move(op), yield);
        if (ret == -ENOENT) {
          // queue was deleted
          ldpp_dout(this, 10) << "INFO: queue: " << queue_name
                              << ". was removed. processing will stop" << dendl;
        ...
        if (ret < 0) {
          ldpp_dout(this, 1)
              << "ERROR: failed to remove entries and/or lock queue up to: "
              << end_marker << " from queue: " << queue_name
              << ". error: " << ret << dendl;
          return;
        } else {
        ...
        if (!entries_to_migrate.empty()) {
          ...
          rgw_pubsub_topic topic;
          auto ret_of_get_topic =
              ps.get_topic(this, queue_name, topic, yield, nullptr);
          if (ret_of_get_topic < 0) {
            // we can't migrate entries without topic info
            ldpp_dout(this, 1) << "ERROR: failed to fetch topic: " << queue_name
                               << " error: " << ret_of_get_topic
                               << ". Aborting migration!" << dendl;
            return;
          }
          ...
          for (auto entry : entries_to_migrate) {
            ...
            try {
              ...
              ldpp_dout(this, 5)
                  << "WARNING: failed to decode entry. error: " << err.what()
                  << dendl;
              continue;
            }
          ...
          cls_2pc_queue_reserve(
              op, size_to_migrate, migration_vector.size(), &obl, &rval);
          ret = rgw_rados_operate(
              this, rados_ioctx, queue_name, std::move(op), yield,
              librados::OPERATION_RETURNVEC);
          if (ret < 0) {
            ldpp_dout(this, 1)
                << "ERROR: failed to reserve migration space on queue: "
                << queue_name << ". error: " << ret << dendl;
            return;
          }
          ret = cls_2pc_queue_reserve_result(obl, reservation_id);
          if (ret < 0) {
            ldpp_dout(this, 1) << "ERROR: failed to parse reservation id for "
                                  "migration. error: "
                               << ret << dendl;
            return;
          }
      ...
      const auto ret = cls_2pc_queue_get_topic_stats(
          rados_ioctx, queue_name, entries_number, entries_size);
      if (ret < 0) {
        ldpp_dout(this, 1) << "ERROR: topic stats for topic: " << queue_name
                           << ". error: " << ret << dendl;
      } else {
    ...
    ldpp_dout(this, 5) << "INFO: manager stopped. done processing for queue: "
                       << queue_name << dendl;
  }
  ...
  void
  process_queues(boost::asio::yield_context yield)
  {
    ...
    while (!shutdown) {
      ...
      if (ceph::coarse_real_clock::now() > next_check_time) {
        ...
        const auto tp =
            ceph::coarse_real_time::clock::to_time_t(next_check_time);
        ldpp_dout(this, 20) << "INFO: processing queue list. next queues "
                               "processing will happen at: "
                            << std::ctime(&tp) << dendl;
      } else {
      ...
      for (const auto& queue_name : queues) {
        ...
        rados::cls::lock::lock(
            &op, queue_name + "_lock", ClsLockType::EXCLUSIVE, lock_cookie,
            "" /*no tag*/, "" /*no description*/, failover_time,
            LOCK_FLAG_MAY_RENEW);
        ...
        if (ret == -EBUSY) {
          ...
          ldpp_dout(this, 20) << "INFO: queue: " << queue_name
        ...
        if (ret < 0) {
          ...
          ldpp_dout(this, 1) << "ERROR: failed to lock queue: " << queue_name
                             << ". error: " << ret << dendl;
          has_error = true;
          continue;
        }
        ...
        if (owned_queues.insert(queue_name).second) {
          ...
          boost::asio::spawn(
              make_strand(io_context), std::allocator_arg,
              make_stack_allocator(),
              [this, &queue_gc, &queue_gc_lock, queue_name,
               &processed_queue_count](boost::asio::yield_context yield) {
                ...
                if (ret < 0) {
                  ldpp_dout(this, 5)
                      << "WARNING: failed to unlock queue: " << queue_name
                      << " with error: " << ret
      ...
      // erase all queue that were deleted
      {
        std::lock_guard lock_guard(queue_gc_lock);
        std::for_each(
            queue_gc.begin(), queue_gc.end(),
            [this, &owned_queues](const std::string& queue_name) {
              topics_persistency_tracker.erase(queue_name);
              owned_queues.erase(queue_name);
              ldpp_dout(this, 10)
                  << "INFO: queue: " << queue_name << " was removed" << dendl;
            });
        queue_gc.clear();
      }
    }
    while (processed_queue_count > 0) {
      ldpp_dout(this, 20) << "INFO: manager stopped. " << processed_queue_count
                          << " queues are still being processed" << dendl;
      async_sleep(yield, queues_update_retry);
    }
    ldpp_dout(this, 5) << "INFO: manager stopped. done processing all queues"
                       << dendl;
  }
...
public:
  ...
  stop()
  {
    ldpp_dout(this, 5) << "INFO: manager received stop signal. shutting down..."
                       << dendl;
    shutdown = true;
    work_guard.reset();
    if (worker.joinable()) {
      ...
      if (future.wait_for(queues_update_retry * 2) ==
  ...
  init()
  {
    boost::asio::spawn(
        make_strand(io_context), std::allocator_arg, make_stack_allocator(),
        [this](boost::asio::yield_context yield) { process_queues(yield); },
        [](std::exception_ptr eptr) {
          if (eptr)
            std::rethrow_exception(eptr);
        });
    ...
    worker = std::thread([this]() {
      ...
      try {
        ...
        ldpp_dout(this, 1) << "ERROR: notification worker failed with error: "
                           << err.what() << dendl;
        throw err;
      }
    });
    ...
    const SiteConfig& site)
{
  if (s_manager) {
    ldpp_dout(dpp, 1)
        << "ERROR: failed to init notification manager: already exists"
        << dendl;
    return false;
  }
    ...
    optional_yield y)
{
  if (topic_queue == Q_LIST_OBJECT_NAME) {
    ldpp_dout(dpp, 1) << "ERROR: topic name cannot be: " << Q_LIST_OBJECT_NAME
  ...
  if (ret == -EEXIST) {
    ...
    ldpp_dout(dpp, 20) << "INFO: queue for topic: " << topic_queue
                       << " already exists. nothing to do" << dendl;
    return 0;
  }
  if (ret < 0) {
    // failed to create queue
    ldpp_dout(dpp, 1) << "ERROR: failed to create queue for topic: "
                      << topic_queue << ". error: " << ret << dendl;
    return ret;
  }
  ...
  ret =
      rgw_rados_operate(dpp, rados_ioctx, Q_LIST_OBJECT_NAME, std::move(op), y);
  if (ret < 0) {
    ldpp_dout(dpp, 1) << "ERROR: failed to add queue: " << topic_queue
                      << " to queue list. error: " << ret << dendl;
    return ret;
  }
  ldpp_dout(dpp, 20) << "INFO: queue: " << topic_queue << " added to queue list"
                     << dendl;
  return 0;
}

int
remove_persistent_topic(
    const DoutPrefixProvider* dpp,
    librados::IoCtx& rados_ioctx,
    const std::string& topic_queue,
    optional_yield y)
{
  ...
  if (ret < 0) {
    ...
    ldpp_dout(dpp, 1) << "ERROR: failed to remove queue for topic: "
                      << topic_queue << ". error: " << ret << dendl;
    return ret;
  }
  ...
  ret =
      rgw_rados_operate(dpp, rados_ioctx, Q_LIST_OBJECT_NAME, std::move(op), y);
  if (ret < 0) {
    ldpp_dout(dpp, 1) << "ERROR: failed to remove queue: " << topic_queue
                      << " from queue list. error: " << ret << dendl;
    return ret;
  }
  ldpp_dout(dpp, 20) << "INFO: queue: " << topic_queue
                     << " removed from queue list" << dendl;
  return 0;
}

rgw::sal::Object*
get_object_with_attributes(const reservation_t& res, rgw::sal::Object* obj)
{
  ...
  if (src_obj->get_attrs().empty()) {
    ...
    if (ret < 0) {
      ldpp_dout(res.dpp, 20)
          << "failed to get attributes from object: " << src_obj->get_key()
          << ". ret = " << ret << dendl;
      return nullptr;
    }
  }
...
static inline void
filter_amz_meta(meta_map_t& dest, const meta_map_t& src)
{
  std::copy_if(
      src.cbegin(), src.cend(), std::inserter(dest, dest.end()),
      [](const auto& m) {
...
static inline void
metadata_from_attributes(reservation_t& res, rgw::sal::Object* obj)
{
    ...
    rgw_pubsub_s3_event& event)
{
  ...
  event.x_amz_id_2 =
      res.store->getRados()->host_id; // RGW on which the change was made
  ...
  boost::algorithm::hex(
      (const char*)&ts, (const char*)&ts + sizeof(utime_t),
    ...
    const uint64_t num_shards)
{
  ...
  ldpp_dout(dpp, 20) << "INFO: Hash Value (hash) is:  " << hash
                     << ". Hash Key: " << bucket_name << ":" << object_key
                     << dendl;
  return hash % num_shards;
}

static inline std::string
get_shard_name(const std::string& topic_name, const uint64_t& shard_id)
{
  return (shard_id == 0) ? topic_name
                         : fmt::format("{}.{}", topic_name, shard_id);
}
    ...
    const RGWObjTags* req_tags)
{
  ...
  if (all_zonegroups_support(site, zone_features::notification_v2) &&
      res.store->stat_topics_v1(res.user_tenant, res.yield, res.dpp) ==
          -ENOENT) {
  ...
  for (auto& bucket_topic : bucket_topics.topics) {
    ...
    for (auto& event_type : event_types) {
      ...
      const std::string& topic_tenant = std::visit(
          fu2::overload(
      ...
      int ret = ps.get_topic(
          res.dpp, topic_cfg.dest.arn_topic, topic_cfg, res.yield, nullptr);
      if (ret < 0) {
        ldpp_dout(res.dpp, 1)
      ...
      if (topic_cfg.dest.persistent) {
        ...
        const std::string object_key = res.object_name ? *res.object_name
                                                       : res.object->get_name();
        const uint64_t num_shards = topic_cfg.dest.num_shards;
        target_shard =
            get_target_shard(dpp, bucket_name, object_key, num_shards);
        const auto shard_name =
            get_shard_name(topic_cfg.dest.persistent_queue, target_shard);
      ...
      res.topics.emplace_back(
          topic_filter.s3_id, topic_cfg, res_id, event_type, target_shard);
    }
  }
  return 0;
}
    ...
    const DoutPrefixProvider* dpp)
{
  for (auto& topic : res.topics) {
    ...
    event_entry_t event_entry;
    populate_event(
        res, obj, size, mtime, etag, version, topic.event_type,
        event_entry.event);
    ...
    if (topic.cfg.dest.persistent) {
      ...
      const auto shard_name =
          get_shard_name(topic.cfg.dest.persistent_queue, target_shard);
      ldpp_dout(res.dpp, 1) << "INFO: target_shard: " << shard_name << dendl;
      if (bl.length() > res.size) {
        ...
        ldpp_dout(dpp, 5) << "WARNING: committed size: " << bl.length()
                          ...
                          << " . trying to make a larger reservation on queue:"
                          << shard_name << dendl;
        // first cancel the existing reservation
        librados::ObjectWriteOperation op;
        cls_2pc_queue_abort(op, topic.res_id);
        auto ret = rgw_rados_operate(
            dpp, res.store->getRados()->get_notif_pool_ctx(), shard_name,
            std::move(op), res.yield);
        if (ret < 0) {
          ldpp_dout(dpp, 1)
              << "ERROR: failed to abort reservation: " << topic.res_id
              << " when trying to make a larger reservation on queue: "
              << shard_name << ". error: " << ret << dendl;
          return ret;
        }
        ...
        if (ret < 0) {
          ldpp_dout(dpp, 1)
              << "ERROR: failed to reserve extra space on queue: " << shard_name
              << ". error: " << ret << dendl;
          return (ret == -ENOSPC) ? -ERR_RATE_LIMITED : ret;
        }
      ...
      topic.res_id = cls_2pc_reservation::NO_ID;
      auto pcc_arg =
          make_unique<PublishCommitCompleteArg>(shard_name, dpp->get_cct());
      aio_completion_ptr completion{librados::Rados::aio_create_completion(
          pcc_arg.get(), publish_commit_completion)};
      auto& io_ctx = res.store->getRados()->get_notif_pool_ctx();
      if (const int ret = io_ctx.aio_operate(shard_name, completion.get(), &op);
          ret < 0) {
      ...
      try {
        ...
        const auto push_endpoint = RGWPubSubEndpoint::create(
            topic.cfg.dest.push_endpoint, topic.cfg.dest.arn_topic,
            RGWHTTPArgs(topic.cfg.dest.push_endpoint_args, dpp),
            dpp->get_cct());
        ldpp_dout(res.dpp, 20)
            << "INFO: push endpoint created: " << topic.cfg.dest.push_endpoint
            << dendl;
...
int
publish_abort(reservation_t& res)
{
  for (auto& topic : res.topics) {
    if (!topic.cfg.dest.persistent ||
        topic.res_id == cls_2pc_reservation::NO_ID) {
    ...
    optional_yield y)
{
  ...
  for (const auto& shard_name : shards) {
    auto ret =
        cls_2pc_queue_list_reservations(rados_ioctx, shard_name, reservations);
    if (ret < 0) {
      ldpp_dout(dpp, 1) << "ERROR: failed to read shard: " << shard_name
                        << "'s list reservation: " << ret << dendl;
      return ret;
    }
    stats.queue_reservations += reservations.size();
    shard_entries = 0;
    shard_size = 0;
    ret = cls_2pc_queue_get_topic_stats(
        rados_ioctx, shard_name, shard_entries, shard_size);
    stats.queue_size += shard_size;
    stats.queue_entries += shard_entries;
    if (ret < 0) {
      ldpp_dout(dpp, 1) << "ERROR: failed to get the size or number of entries "
                           "for queue shard: "
                        << shard_name << ret << dendl;
      return ret;
    }
  }
  return 0;
...
reservation_t::~reservation_t() { publish_abort(*this); }

void
rgw_topic_stats::dump(Formatter* f) const
{
