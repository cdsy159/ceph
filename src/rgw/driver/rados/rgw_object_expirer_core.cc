...
static int
objexp_key_shard(const rgw_obj_index_key& key, int num_shards)
{
...
static string
objexp_hint_get_keyext(
    const string& tenant_name,
    const string& bucket_name,
    const string& bucket_id,
    const rgw_obj_key& obj_key)
{
  return tenant_name + (tenant_name.empty() ? "" : ":") + bucket_name + ":" +
         bucket_id + ":" + obj_key.name + ":" + obj_key.instance;
}

static void
objexp_get_shard(int shard_num, string* shard)
{
...
int
RGWObjExpStore::objexp_hint_add(
    const DoutPrefixProvider* dpp,
...
{
  const string keyext =
      objexp_hint_get_keyext(tenant_name, bucket_name, bucket_id, obj_key);
  ...
  string shard_name = objexp_hint_get_shardname(
      objexp_key_shard(obj_key, cct->_conf->rgw_objexp_hints_num_shards));
  rgw_rados_ref obj;
  int r = rgw_get_rados_ref(
      dpp, driver->getRados()->get_rados_handle(),
      {driver->svc()->zone->get_zone_params().log_pool, shard_name}, &obj);
  if (r < 0) {
    ldpp_dout(dpp, 0) << "ERROR: " << __func__
                      << "(): failed to open obj=" << obj << " (r=" << r << ")"
                      << dendl;
    return r;
  }
...
int
RGWObjExpStore::objexp_hint_list(
    const DoutPrefixProvider* dpp,
    const string& oid,
    ...
    bool* truncated) /* out */
{
  ...
  cls_timeindex_list(
      op, utime_t(start_time), utime_t(end_time), marker, max_entries, entries,
      out_marker, truncated);

  rgw_rados_ref obj;
  int r = rgw_get_rados_ref(
      dpp, driver->getRados()->get_rados_handle(),
      {driver->svc()->zone->get_zone_params().log_pool, oid}, &obj);
  if (r < 0) {
    ldpp_dout(dpp, 0) << "ERROR: " << __func__
                      << "(): failed to open obj=" << obj << " (r=" << r << ")"
                      << dendl;
...
int
RGWObjExpStore::objexp_hint_trim(
    const DoutPrefixProvider* dpp,
    const string& oid,
    const ceph::real_time& start_time,
    const ceph::real_time& end_time,
...
{
  rgw_rados_ref ref;
  auto ret = rgw_get_rados_ref(
      dpp, driver->getRados()->get_rados_handle(),
      {driver->svc()->zone->get_zone_params().log_pool, oid}, &ref);
  if (ret < 0) {
    ldpp_dout(dpp, 0) << "ERROR: " << __func__
                      << "(): failed to open oid=" << oid << " (r=" << ret
                      << ")" << dendl;
    return ret;
  }
  ret = cls_timeindex_trim_repeat(
      dpp, ref, oid, utime_t(start_time), utime_t(end_time), from_marker,
      to_marker, y);
  if ((ret < 0) && (ret != -ENOENT)) {
    return ret;
  }
...
int
RGWObjectExpirer::garbage_single_object(
    const DoutPrefixProvider* dpp,
    objexp_hint_entry& hint)
{
  ...
  int ret = driver->load_bucket(
      dpp, rgw_bucket(hint.tenant, hint.bucket_name, hint.bucket_id), &bucket,
      null_yield);
  if (-ENOENT == ret) {
    ...
    ldpp_dout(dpp, 1) << "ERROR: could not init bucket = " << hint.bucket_name
                      << "due to ret = " << ret << dendl;
    return ret;
  }
  ...
  ret = static_cast<rgw::sal::RadosObject*>(obj.get())->handle_obj_expiry(
      dpp, null_yield);
...
void
RGWObjectExpirer::garbage_chunk(
    const DoutPrefixProvider* dpp,
    ...
    bool& need_trim) /* out */
{
  ...
  for (list<cls_timeindex_entry>::iterator iter = entries.begin();
    ...
    if (ret < 0) {
      ldpp_dout(dpp, 1) << "cannot parse removal hint for " << hint.obj_key
                        << dendl;
      continue;
    }
    ...
    if (ret == -ERR_PRECONDITION_FAILED) {
      ldpp_dout(dpp, 15) << "not actual hint for object: " << hint.obj_key
                         << dendl;
    } else if (ret < 0) {
      ldpp_dout(dpp, 1) << "cannot remove expired object: " << hint.obj_key
                        << dendl;
    }
    ...
    optional_yield y)
{
  ...
  int ret = exp_store.objexp_hint_trim(
      dpp, shard, rt_from, rt_to, from_marker, to_marker, y);
  if (ret < 0) {
    ...
    optional_yield y)
{
  ...
  int ret = l.lock_exclusive(
      &static_cast<rgw::sal::RadosStore*>(driver)->getRados()->objexp_pool_ctx,
      shard);
  if (ret == -EBUSY) { /* already locked by another processor */
    ldpp_dout(dpp, 5) << __func__ << "(): failed to acquire lock on " << shard
                      << dendl;
    return false;
  }
  ...
  do {
    ...
    ret = exp_store.objexp_hint_list(
        dpp, shard, rt_last, rt_start, num_entries, marker, entries,
        &out_marker, &truncated);
    if (ret < 0) {
...
bool
RGWObjectExpirer::inspect_all_shards(
    const DoutPrefixProvider* dpp,
    const utime_t& last_run,
    const utime_t& round_start,
    optional_yield y)
...
bool
RGWObjectExpirer::going_down()
{
  return down_flag;
}

void
RGWObjectExpirer::start_processor()
{
...
void*
RGWObjectExpirer::OEWorker::entry()
{
  utime_t last_run;
  do {
...
void
RGWObjectExpirer::OEWorker::stop()
{
...
unsigned
RGWObjectExpirer::OEWorker::get_subsys() const
{
  return dout_subsys;
}

std::ostream&
RGWObjectExpirer::OEWorker::gen_prefix(std::ostream& out) const
{
