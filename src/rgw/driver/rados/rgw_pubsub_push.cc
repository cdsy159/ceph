...
std::string
json_format_pubsub_event(const EventType& event)
{
  std::stringstream ss;
...
bool
get_bool(const RGWHTTPArgs& args, const std::string& name, bool default_value)
{
  bool value;
  bool exists;
  if (args.get_bool(name.c_str(), &value, &exists) == -EINVAL) {
    throw RGWPubSubEndpoint::configuration_error(
...
public:
    ...
    if (!exists || str_ack_level == "any") {
      ...
      if (ack_level < 100 || ack_level >= 600) {
        throw configuration_error(
            "HTTP/S: invalid http-ack-level: " + str_ack_level);
      }
    }
  }
      ...
      optional_yield y) override
  {
    ...
    if (!s_http_manager) {
      ldout(cct, 1) << "ERROR: send failed. http endpoint manager not running"
                    << dendl;
      return -ESRCH;
    }
    const auto max_inflight = cct->_conf->rgw_http_notif_max_inflight;
    if (max_inflight != 0 && s_http_manager_inflight >= max_inflight) {
      ldout(cct, 1) << "ERROR: send failed. http endpoint manager busy. "
                       "in-flight requests: "
    ...
    request.set_req_connect_timeout(
        cct->_conf->rgw_http_notif_connection_timeout);
    ...
    if (cloudevents) {
      ...
      request.append_header(
          "ce-id", event.x_amz_request_id + "." + event.x_amz_id_2);
      request.append_header(
          "ce-source",
          event.eventSource + "." + event.awsRegion + "." + event.bucket_name);
    ...
    if (perfcounter)
      perfcounter->inc(l_rgw_pubsub_push_pending);
    ...
    --s_http_manager_inflight;
    if (perfcounter)
      perfcounter->dec(l_rgw_pubsub_push_pending);
...
private:
  ...
  bool
  get_verify_ssl(const RGWHTTPArgs& args)
  {
    bool exists;
    ...
    throw configuration_error(
        "'verify-ssl' must be true/false, not: " + str_verify_ssl);
  }

  std::string
  get_exchange(const RGWHTTPArgs& args)
  {
    bool exists;
  ...
  ack_level_t
  get_ack_level(const RGWHTTPArgs& args)
  {
    bool exists;
...
public:
  ...
  {
    if (!amqp::connect(
            conn_id, endpoint, exchange, (ack_level == ack_level_t::Broker),
            get_verify_ssl(args), args.get_optional("ca-location"))) {
      throw configuration_error(
          "AMQP: failed to create connection to: " + endpoint);
    }
  }
      ...
      optional_yield y) override
  {
    if (ack_level == ack_level_t::None) {
      if (perfcounter)
        perfcounter->inc(l_rgw_pubsub_push_pending);
      const auto rc =
          amqp::publish(conn_id, topic, json_format_pubsub_event(event));
      if (perfcounter)
        perfcounter->dec(l_rgw_pubsub_push_pending);
      if (rc < 0 && perfcounter)
...
private:
  ...
  get_ack_level(const RGWHTTPArgs& args)
  {
    ...
    throw configuration_error(
        "Kafka: invalid kafka-ack-level: " + str_ack_level);
  }
...
public:
  RGWPubSubKafkaEndpoint(
      ...
      const std::string& _topic,
      const RGWHTTPArgs& args) :
    topic(_topic), ack_level(get_ack_level(args))
  {
    if (!kafka::connect(
            ...
            args.get_optional("user-name"), args.get_optional("password"),
            args.get_optional("kafka-brokers"))) {
      throw configuration_error(
          "Kafka: failed to create connection to: " + _endpoint);
    }
  }
      ...
      optional_yield y) override
  {
    if (ack_level == ack_level_t::None) {
      if (perfcounter)
        perfcounter->inc(l_rgw_pubsub_push_pending);
      const auto rc =
          kafka::publish(conn_id, topic, json_format_pubsub_event(event));
      if (perfcounter)
        perfcounter->dec(l_rgw_pubsub_push_pending);
      if (rc < 0 && perfcounter)
        perfcounter->inc(l_rgw_pubsub_push_failed);
...
const std::string&
get_schema(const std::string& endpoint)
{
  if (endpoint.empty()) {
    return NO_SCHEMA;
  }
...
bool
init_http_manager(CephContext* cct)
{
...
bool
RGWPubSubEndpoint::init_all(CephContext* cct)
{
#ifdef WITH_RADOSGW_AMQP_ENDPOINT
  if (!amqp::init(cct)) {
