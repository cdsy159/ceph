...
protected:
  ...
  virtual int process_first_chunk(
      bufferlist&& data,
...
public:
  ...
  void
  set_head_chunk_size(uint64_t size)
  {
...
public:
  ...
  void
  set_owner(const ACLOwner& _owner)
  {
    owner = _owner;
  }

  void
  set_tail_placement(const rgw_placement_rule& tpr)
  {
...
class AtomicObjectProcessor : public ManifestObjectProcessor {
  ...
  int process_first_chunk(
      bufferlist&& data,
      rgw::sal::DataProcessor** processor) override;
