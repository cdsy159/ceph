...
rgw_raw_obj
rgw_obj_select::get_raw_obj(RGWRados* store) const
{
  if (!is_raw) {
    rgw_raw_obj r;
...
void
RGWObjVersionTracker::prepare_op_for_read(ObjectReadOperation* op)
{
...
void
RGWObjVersionTracker::prepare_read(neorados::ReadOp& op)
{
...
void
RGWObjVersionTracker::prepare_write(neorados::WriteOp& op)
{
  namespace version = neorados::cls::version;
  auto check_objv = version_for_check();
...
void
RGWObjVersionTracker::prepare_op_for_write(ObjectWriteOperation* op)
{
  obj_version* check_objv = version_for_check();
...
void
RGWObjVersionTracker::apply_write()
{
...
RGWObjStateManifest*
RGWObjectCtx::get_state(const rgw_obj& obj)
{
...
void
RGWObjectCtx::set_compressed(const rgw_obj& obj)
{
...
void
RGWObjectCtx::set_atomic(const rgw_obj& obj, bool atomic)
{
  std::unique_lock wl{lock};
  assert(!obj.empty());
...
void
RGWObjectCtx::set_prefetch_data(const rgw_obj& obj)
{
  std::unique_lock wl{lock};
  assert(!obj.empty());
...
void
RGWObjectCtx::invalidate(const rgw_obj& obj)
{
  std::unique_lock wl{lock};
...
public:
      ...
      set<int>& shards)
  {
    rgw_http_param_pair pairs[] = {
        {"type", "metadata"}, {"notify", NULL}, {NULL, NULL}};
...
public:
      ...
      bc::flat_map<int, bc::flat_set<rgw_data_notify_entry>>& shards)
  {
    ...
    const char* source_zone =
        store->svc.zone->get_zone_params().get_id().c_str();
    for (auto iter = conn_map.begin(); iter != conn_map.end(); ++iter) {
      RGWRESTConn* conn = iter->second;
      RGWCoroutinesStack* stack = new RGWCoroutinesStack(store->ctx(), this);
      stack->call(new RGWDataPostNotifyCR(
          store, http_manager, shards, source_zone, conn));
      stacks.push_back(stack);
    }
...
RGWRadosThread::Worker::entry()
{
  ...
  do {
    ...
    if (r < 0) {
      ldpp_dout(this, 0) << "ERROR: processor->process() returned error r=" << r
                         << dendl;
    }
...
int
RGWMetaNotifier::process(const DoutPrefixProvider* dpp)
{
  set<int> shards;
...
int
RGWDataNotifier::process(const DoutPrefixProvider* dpp)
{
  ...
  for (const auto& [shard_id, entries] : shards) {
    ...
    for (const auto& entry : entries) {
      ldpp_dout(dpp, 20) << __func__
                         ...
                         << shard_id << ":" << entry.gen << ":" << entry.key
                         << dendl;
    }
  }

  notify_mgr.notify_all(
      dpp, store->svc.zone->get_zone_data_notify_to_map(), shards);
...
public:
  RGWSyncProcessorThread(
      RGWRados* _driver,
      const string& thread_name = "radosgw") :
...
public:
  ...
  void
  wakeup_sync_shards(set<int>& shard_ids)
  {
    for (set<int>::iterator iter = shard_ids.begin(); iter != shard_ids.end();
         ++iter) {
      sync.wakeup(*iter);
    }
  }
  ...
  int
  init(const DoutPrefixProvider* dpp) override
  {
...
public:
  RGWDataSyncProcessorThread(
    ...
    counters(sync_counters::build(
        store->ctx(),
        std::string("data-sync-from-") + source_zone->name)),
...
public:
  ...
  process(const DoutPrefixProvider* dpp) override
  {
    ...
    auto metatrimcr = create_meta_log_trim_cr(
        this, static_cast<rgw::sal::RadosStore*>(store), &http,
        cct->_conf->rgw_md_log_max_shards, trim_interval);
    if (!metatrimcr) {
    ...
    if (store->svc()->zone->sync_module_exports_data()) {
      ...
      data->call(create_data_log_trim_cr(
          dpp, static_cast<rgw::sal::RadosStore*>(store), &http,
          cct->_conf->rgw_data_log_num_shards, trim_interval));
...
void
RGWRados::wakeup_meta_sync_shards(set<int>& shard_ids)
{
...
void
RGWRados::wakeup_data_sync_shards(
    const DoutPrefixProvider* dpp,
    const rgw_zone_id& source_zone,
    bc::flat_map<int, bc::flat_set<rgw_data_notify_entry>>& entries)
{
  ldpp_dout(dpp, 20) << __func__ << ": source_zone=" << source_zone
                     << ", entries=" << entries << dendl;
  for (bc::flat_map<int, bc::flat_set<rgw_data_notify_entry>>::iterator iter =
           entries.begin();
       iter != entries.end(); ++iter) {
    ldpp_dout(dpp, 20) << __func__ << "(): updated shard=" << iter->first
                       << dendl;
    bc::flat_set<rgw_data_notify_entry>& entries = iter->second;
    for (const auto& [key, gen] : entries) {
      ldpp_dout(dpp, 20) << __func__ << ": source_zone=" << source_zone
                         << ", key=" << key << ", gen=" << gen << dendl;
    }
  }
  ...
  if (iter == data_sync_processor_threads.end()) {
    ldpp_dout(dpp, 10) << __func__ << ": couldn't find sync thread for zone "
                       << source_zone << ", skipping async data sync processing"
                       << dendl;
    return;
  }
...
RGWDataSyncStatusManager*
RGWRados::get_data_sync_manager(const rgw_zone_id& source_zone)
{
  std::lock_guard l{data_sync_thread_lock};
    ...
    uint64_t* palignment)
{
  ...
  if (!get_obj_data_pool(placement_rule, obj, &pool)) {
    ldpp_dout(dpp, 0) << "ERROR: failed to get data pool for object " << obj
                      << dendl;
    return -EIO;
  }
...
public:
  RGWIndexCompletionManager(RGWRados* _driver) :
    ...
    locks{ceph::make_lock_container<ceph::mutex>(
        num_shards,
        [](const size_t i) {
          return ceph::make_mutex(
              "RGWIndexCompletionManager::lock::" + std::to_string(i));
        })},
...
static void
obj_complete_cb(completion_t cb, void* arg)
{
...
RGWIndexCompletionManager::process()
{
  ...
  while (!_stop) {
    ...
    for (auto c : comps) {
      ...
      ldpp_dout(&dpp, 20) << __func__
                          << "(): handling completion for key=" << c->key
                          << dendl;
      ...
      if (r < 0) {
        ldpp_dout(&dpp, 0) << "ERROR: " << __func__
                           << "(): failed to initialize BucketShard, obj="
                           << c->obj << " r=" << r << dendl;
        /* not much to do */
        continue;
      }

      r = store->guard_reshard(
          &dpp, &bs, c->obj, bucket_info,
          [&](RGWRados::BucketShard* bs) -> int {
            const bool bitx =
                ctx()->_conf->rgw_bucket_index_transaction_instrumentation;
            ldout_bitx(bitx, &dpp, 10)
                << "ENTERING " << __func__ << ": bucket-shard=" << bs
                << " obj=" << c->obj << " tag=" << c->tag << " op=" << c->op
                << ", remove_objs=" << c->remove_objs << dendl_bitx;
            ldout_bitx(bitx, &dpp, 25) << "BACKTRACE: " << __func__ << ": "
                                       << ClibBackTrace(1) << dendl_bitx;

            librados::ObjectWriteOperation o;
            o.assert_exists();
            cls_rgw_guard_bucket_resharding(o, -ERR_BUSY_RESHARDING);
            cls_rgw_bucket_complete_op(
                o, c->op, c->tag, c->ver, c->key, c->dir_meta, &c->remove_objs,
      ...
      if (r < 0) {
        ldpp_dout(&dpp, 0) << "ERROR: " << __func__
                           << "(): bucket index completion failed, obj="
                           << c->obj << " r=" << r << dendl;
      ...
      if (c->log_op) {
        ...
        std::ignore = add_datalog_entry(
            &dpp, store->svc.datalog_rados, bucket_info, bs.shard_id,
            null_yield);
    ...
    complete_op_data** result)
{
  ...
  if (zones_trace) {
    ...
    entry->zones_trace.insert(
        store->svc.zone->get_zone().id, obj.bucket.get_key());
  }

  *result = entry;

  entry->rados_completion =
      librados::Rados::aio_create_completion(entry, obj_complete_cb);
...
void
RGWIndexCompletionManager::add_completion(complete_op_data* completion)
{
  {
...
bool
RGWIndexCompletionManager::handle_completion(
    completion_t cb,
    complete_op_data* arg)
{
  int shard_id = arg->manager_shard_id;
  {
    ...
    if (iter == comps.end()) {
      ldout(arg->manager->ctx(), 0)
          << __func__ << "(): cannot find completion for obj=" << arg->key
          << dendl;
      return true;
    }
  ...
  add_completion(arg);
  ldout(arg->manager->ctx(), 20)
      << __func__ << "(): async completion added for obj=" << arg->key << dendl;
  return false;
}
...
int
RGWRados::register_to_service_map(
    const DoutPrefixProvider* dpp,
    ...
    const map<string, string>& meta)
{
  ...
  int ret = rados.service_daemon_register(
      daemon_type, stringify(rados.get_instance_id()), metadata);
  if (ret < 0) {
    ldpp_dout(dpp, 0) << "ERROR: service_daemon_register() returned ret=" << ret
...
int
RGWRados::update_service_map(
    const DoutPrefixProvider* dpp,
    std::map<std::string, std::string>&& status)
{
  int ret = rados.service_daemon_update_status(std::move(status));
  if (ret < 0) {
    ldpp_dout(dpp, 0) << "ERROR: service_daemon_update_status() returned ret="
    ...
    rgw::sal::ConfigStore* cfgstore)
{
  ...
  sync_tracer =
      new RGWSyncTraceManager(cct, cct->_conf->rgw_sync_trace_history_size);
  ...
  if (run_sync_thread) {
    for (const auto& pt : zonegroup.placement_targets) {
      if (zone_params.placement_pools.find(pt.second.name) ==
        ...
        ldpp_dout(dpp, 0)
            << "WARNING: This zone does not contain the placement target "
    ...
    meta_sync_processor_thread =
        new RGWMetaSyncProcessorThread(this->driver, async_processor, cfgstore);
    ret = meta_sync_processor_thread->init(dpp);
    if (ret < 0) {
      ldpp_dout(dpp, 0) << "ERROR: failed to initialize meta sync thread"
                        << dendl;
      return ret;
    }
    ...
    if (ret < 0) {
      ldpp_dout(dpp, 0) << "ERROR: failed to start bucket trim manager"
                        << dendl;
      return ret;
    }
    ...
    for (auto source_zone : svc.zone->get_data_sync_source_zones()) {
      ldpp_dout(dpp, 5) << "starting data sync thread for zone "
                        << source_zone->name << dendl;
      auto* thread = new RGWDataSyncProcessorThread(
          this->driver, svc.async_processor, source_zone, cfgstore);
      ret = thread->init(dpp);
      if (ret < 0) {
        ldpp_dout(dpp, 0) << "ERROR: failed to initialize data sync thread"
                          << dendl;
        return ret;
      }
      thread->start();
    ...
    if (interval > 0) {
      sync_log_trimmer =
          new RGWSyncLogTrimThread(this->driver, &*bucket_trim, interval);
      ret = sync_log_trimmer->init(dpp);
      if (ret < 0) {
        ldpp_dout(dpp, 0) << "ERROR: failed to initialize sync log trim thread"
                          << dendl;
        return ret;
      }
  ...
  quota_handler =
      RGWQuotaHandler::generate_handler(dpp, this->driver, quota_threads);
  ...
  if (bucket_index_max_shards > get_max_bucket_shards()) {
    ...
    ldpp_dout(dpp, 1)
        << __func__ << " bucket index max shards is too large, reset to value: "
  ...
  bool need_tombstone_cache = !svc.zone->get_zone_data_notify_to_map()
                                   .empty(); /* have zones syncing from us */

  if (need_tombstone_cache) {
    obj_tombstone_cache =
        new tombstone_cache_t(cct->_conf->rgw_obj_tombstone_cache_size);
  }
  ...
  if (run_bucket_logging_thread) {
    if (!rgw::bucketlogging::init(dpp, this->driver, *svc.site)) {
      ldpp_dout(dpp, 0) << "ERROR: failed to initialize bucket logging manager"
                        << dendl;
    }
  ...
  if (run_notification_thread) {
    if (!rgw::notify::init(dpp, driver, *svc.site)) {
      ldpp_dout(dpp, 0) << "ERROR: failed to initialize notification manager"
                        << dendl;
    }
    ...
    if (svc.site->is_meta_master() &&
      ...
      boost::asio::spawn(
          ...
          [this](boost::asio::yield_context yield) {
            ...
            rgwrados::topic_migration::migrate(
                &dpp, driver, v1_topic_migration, yield);
          },
    ...
    rgw::sal::ConfigStore* cfgstore)
{
  ...
  return svc.init(
      cct, driver, use_cache, run_sync_thread, background_tasks, null_yield,
      dpp, site, cfgstore);
}
    ...
    rgw::sal::ConfigStore* cfgstore)
{
  ...
  auto ret = init_rados();
  if (ret < 0) {
    ldpp_dout(dpp, 0) << "ERROR: failed to initialize librados (ret="
                      << cpp_strerror(-ret) << ")" << dendl;
    return ret;
  }
  ...
  if (ret < 0) {
    ldpp_dout(dpp, 0) << "ERROR: failed to init services (ret="
                      << cpp_strerror(-ret) << ")" << dendl;
    return ret;
  }
  ...
  if (ret < 0) {
    ldpp_dout(dpp, 0) << "ERROR: failed to init ctls (ret="
                      << cpp_strerror(-ret) << ")" << dendl;
    return ret;
  }
 ...
 */
int
RGWRados::open_root_pool_ctx(const DoutPrefixProvider* dpp)
{
  return rgw_init_ioctx(
      dpp, get_rados_handle(), svc.zone->get_zone_params().domain_root,
      root_pool_ctx, true, true);
}

int
RGWRados::open_gc_pool_ctx(const DoutPrefixProvider* dpp)
{
  return rgw_init_ioctx(
      dpp, get_rados_handle(), svc.zone->get_zone_params().gc_pool, gc_pool_ctx,
      true, true);
}

int
RGWRados::open_lc_pool_ctx(const DoutPrefixProvider* dpp)
{
  return rgw_init_ioctx(
      dpp, get_rados_handle(), svc.zone->get_zone_params().lc_pool, lc_pool_ctx,
      true, true);
}

int
RGWRados::open_restore_pool_ctx(const DoutPrefixProvider* dpp)
{
  return rgw_init_ioctx(
      dpp, get_rados_handle(), svc.zone->get_zone_params().restore_pool,
      restore_pool_ctx, true, true);
}

int
RGWRados::open_restore_pool_neo_ctx(const DoutPrefixProvider* dpp)
{
  maybe_warn_about_blocking(dpp);
  try {
    restore_pool_neo_ctx = rgw::init_iocontext(
        dpp, driver->get_neorados(), svc.zone->get_zone_params().restore_pool,
...
int
RGWRados::open_objexp_pool_ctx(const DoutPrefixProvider* dpp)
{
  return rgw_init_ioctx(
      dpp, get_rados_handle(), svc.zone->get_zone_params().log_pool,
      objexp_pool_ctx, true, true);
}

int
RGWRados::open_reshard_pool_ctx(const DoutPrefixProvider* dpp)
{
  return rgw_init_ioctx(
      dpp, get_rados_handle(), svc.zone->get_zone_params().reshard_pool,
      reshard_pool_ctx, true, true);
}

int
RGWRados::open_notif_pool_ctx(const DoutPrefixProvider* dpp)
{
  return rgw_init_ioctx(
      dpp, get_rados_handle(), svc.zone->get_zone_params().notif_pool,
      notif_pool_ctx, true, true);
}

int
RGWRados::open_logging_pool_ctx(const DoutPrefixProvider* dpp)
{
  return rgw_init_ioctx(
      dpp, get_rados_handle(), svc.zone->get_zone_params().bucket_logging_pool,
      logging_pool_ctx, true, true);
}
    ...
    bool bulk)
{
  ...
  return rgw_init_ioctx(
      dpp, get_rados_handle(), pool, io_ctx, create, mostly_omap, bulk);
}
    ...
    RGWAccessHandle* handle)
{
  ...
  try {
    ...
    *handle = (RGWAccessHandle)state;
    ...
    r = -e.code().value();
    ldpp_dout(dpp, 10) << "nobjects_begin threw " << e.what() << ", returning "
                       << r << dendl;
    return r;
...
int
RGWRados::log_list_next(RGWAccessHandle handle, string* name)
{
...
int
RGWRados::log_remove(const DoutPrefixProvider* dpp, const string& name)
{
  librados::IoCtx io_ctx;
  int r = rgw_init_ioctx(
      dpp, get_rados_handle(), svc.zone->get_zone_params().log_pool, io_ctx);
  if (r < 0)
    return r;
    ...
    rgw_log_entry* entry)
{
  ...
  ldpp_dout(dpp, 10) << "log_show_next pos " << state->pos << " bl "
                     << state->bl.length() << " off " << off << " eof "
                     << (int)state->eof << dendl;
  // read some?
 ...
 */
static void
usage_log_hash(CephContext* cct, const string& name, string& hash, uint32_t index)
{
  uint32_t val = index;
  ...
  snprintf(
      buf, sizeof(buf), RGW_USAGE_OBJ_PREFIX "%u", (unsigned)(val % max_shards));
  hash = buf;
}
    ...
    optional_yield y)
{
  ...
  for (iter = usage_info.begin(); iter != usage_info.end(); ++iter) {
    ...
    if (ub.user.empty()) {
      ldpp_dout(dpp, 0)
          << "WARNING: RGWRados::log_usage(): user name empty (bucket="
          << ub.bucket << "), skipping" << dendl;
      continue;
    }
    ...
    map<rgw_user_bucket, rgw_usage_log_entry>& usage)
{
  ...
  do {
    ...
    int ret = cls_obj_usage_log_read(
        dpp, hash, user_str, bucket_name, start_epoch, end_epoch, num,
    ...
    optional_yield y)
{
  ...
  hash = first_hash;
  do {
    int ret = cls_obj_usage_log_trim(
        dpp, hash, user_str, bucket_name, start_epoch, end_epoch, y);
...
int
RGWRados::clear_usage(const DoutPrefixProvider* dpp, optional_yield y)
{
  ...
  for (unsigned i = 0; i < max_shards; i++) {
    ...
    if (ret < 0) {
      ldpp_dout(dpp, 0) << "usage clear on oid=" << oid
                        << "failed with ret=" << ret << dendl;
      return ret;
    }
  }
  return ret;
    ...
    ACLOwner* owner)
{
  ...
  try {
    ...
    ldpp_dout(dpp, 0) << "ERROR: could not decode policy, caught buffer::error"
                      << dendl;
    return -EIO;
  }
...
int
RGWRados::Bucket::update_bucket_id(
    ...
    optional_yield y)
{
  ...
  int ret = store->get_bucket_instance_info(
      bucket, bucket_info, nullptr, nullptr, y, dpp);
  if (ret < 0) {
    return ret;
  }
    ...
    optional_yield y)
{
  ...
  for (uint16_t attempt = 1; /* empty */; ++attempt) {
    ...
    if (attempt > 1 && prev_marker == cur_marker) {
      ...
      ldpp_dout(dpp, 0) << "ERROR: " << __func__
                        << " marker failed to make forward progress; attempt="
                        << attempt << ", prev_marker=" << prev_marker
    ...
    int r = store->cls_bucket_list_ordered(
        dpp, target->get_bucket_info(), current_index, shard_id, cur_marker,
        cur_prefix, params.delim, read_ahead + 1 - count, params.list_versions,
        attempt, ent_map, &truncated, &cls_filtered, &cur_marker, y,
    ...
    for (auto eiter = ent_map.begin(); eiter != ent_map.end(); ++eiter) {
      ...
      ldpp_dout(dpp, 20) << __func__ << ": considering entry " << entry.key
                         << dendl;
      ...
      if (!valid) {
        ldpp_dout(dpp, 0) << "ERROR: " << __func__
                          << " could not parse object name: " << obj.name
                          << dendl;
        continue;
      }
      ...
      if (!params.list_versions && !entry.is_visible()) {
        ldpp_dout(dpp, 10) << __func__ << ": skipping not visible entry \""
                           << entry.key << "\"" << dendl;
        continue;
      }
      ...
      if (params.enforce_ns && !matched_ns) {
        if (!params.ns.empty()) {
          ...
          if (cur_marker < potential_marker) {
            ldpp_dout(dpp, 20)
                << __func__ << ": skipping past region of namespaced entries, starting with \""
                << entry.key << "\"" << dendl;
        ...
        ldpp_dout(dpp, 20) << __func__
                           << ": skipping past individual namespaced entry \""
                           << entry.key << "\"" << dendl;
        continue;
      }
      ...
      if (cur_end_marker_valid && cur_end_marker <= index_key) {
        ...
        ldpp_dout(dpp, 10) << __func__
                           << ": finished due to getting end marker of \""
                           << cur_end_marker << "\" with \"" << entry.key
                           << "\"" << dendl;
        goto done;
      }
      ...
      if (params.access_list_filter &&
        ...
        ldpp_dout(dpp, 20) << __func__
                           << ": skipping past filtered out entry \""
                           << entry.key << "\"" << dendl;
        continue;
      }
      ...
      if (params.prefix.size() &&
        ...
        ldpp_dout(dpp, 20) << __func__ << ": skipping object \"" << entry.key
                           << "\" that doesn't match prefix \"" << params.prefix
                           << "\"" << dendl;
        continue;
      }
          ...
          if (cls_filtered) {
            ...
            if (common_prefixes) {
              if (count >= max) {
                truncated = true;
                ldpp_dout(dpp, 10)
                    << __func__ << ": stopping early with common prefix \""
                    << entry.key << "\" because requested number (" << max
            ...
            ldpp_dout(dpp, 20)
                << __func__ << ": finished entry with common prefix \""
                << entry.key << "\" so continuing loop (cls filtered)" << dendl;
            continue;
          } else {
            ...
            ldpp_dout(dpp, 20)
                << __func__ << ": finished entry with common prefix \""
                << entry.key << "\" so continuing loop (not cls filtered)"
                << dendl;
            continue;
      ...
      if (count >= max) {
        truncated = true;
        ldpp_dout(dpp, 10) << __func__ << ": stopping early with entry \""
                           << entry.key << "\" because requested number ("
                           << max << ") reached" << dendl;
        goto done;
      }

      ldpp_dout(dpp, 20) << __func__ << ": adding entry " << entry.key
                         << " to result" << dendl;
    ...
    if (!cls_filtered && !params.delim.empty()) {
      ...
      if (marker_delim_pos >= 0) {
        ...
        ldpp_dout(dpp, 20) << __func__
                           << ": skip_after_delim=" << skip_after_delim
                           << dendl;

        if (skip_after_delim > cur_marker.name) {
          cur_marker = skip_after_delim;
          ldpp_dout(dpp, 20)
              << __func__ << ": setting cur_marker=" << cur_marker.name << "["
    ...
    if (!truncated || count >= (max + 1) / 2) {
      ...
      ldpp_dout(dpp, 10) << __func__
                         << ": exiting attempt loop because we reached end ("
                         << truncated
                         << ") or we're returning half the requested entries ("
                         << count << " of " << max << ")" << dendl;
      break;
      ...
      ldpp_dout(dpp, 10) << __func__
                         ...
                         << " attempts and we're returning " << count
                         << " entries" << dendl;
      break;
    }
    ...
    optional_yield y)
{
  ...
  while (truncated && count <= max) {
    ...
    int r = store->cls_bucket_list_unordered(
        dpp, target->get_bucket_info(), current_index, shard_id, cur_marker,
        cur_prefix, read_ahead, params.list_versions, ent_list, &truncated,
        &cur_marker, y);
    if (r < 0) {
      ldpp_dout(dpp, 0) << "ERROR: " << __func__
                        << " cls_bucket_list_unordered returned " << r
                        << " for " << target->get_bucket_info().bucket << dendl;
      return r;
    }
 ...
 */
int
RGWRados::create_pool(const DoutPrefixProvider* dpp, const rgw_pool& pool)
{
  librados::IoCtx io_ctx;
...
void
RGWRados::create_bucket_id(string* bucket_id)
{
  uint64_t iid = instance_id();
  uint64_t bid = next_bucket_id();
  char buf[svc.zone->get_zone_params().get_id().size() + 48];
  snprintf(
      buf, sizeof(buf), "%s.%" PRIu64 ".%" PRIu64,
...
#define MAX_CREATE_RETRIES 20 /* need to bound retries */
  for (int i = 0; i < MAX_CREATE_RETRIES; i++) {
    ...
    if (zone_placement) {
      ...
      init_default_bucket_layout(
          cct, info.layout, svc.zone->get_zone(), index_type, index_shards);
    }
    ...
    ret = put_linked_bucket_info(
        info, exclusive, ceph::real_time(), pep_objv, &attrs, true, dpp, y);
    if (ret == -ECANCELED) {
      ret = -EEXIST;
    }
    if (ret == -EEXIST) {
      ...
      int r = get_bucket_info(
          &svc, bucket.tenant, bucket.name, orig_info, NULL, y, dpp);
      if (r < 0) {
        if (r == -ENOENT) {
          continue;
        }
      ...
      if (orig_info.bucket.bucket_id != bucket.bucket_id) {
        if (zone_placement) {
          ...
          if (r < 0) {
            ldpp_dout(dpp, 0)
                << "WARNING: could not remove bucket index (r=" << r << ")"
                << dendl;
          }
        }
  ...
  ldpp_dout(dpp, 0) << "ERROR: could not create bucket, continuously raced "
                       "with bucket creation and removal"
                    << dendl;
  return -ENOENT;
}
...
std::string
RGWRados::get_cluster_fsid(const DoutPrefixProvider* dpp, optional_yield y)
{
  std::string s;
    ...
    rgw_rados_ref* ref)
{
  ...
  if (r < 0) {
    ldpp_dout(dpp, 0) << "ERROR: failed opening data pool (pool=" << raw.pool
                      << "); r=" << r << dendl;
    return r;
  }
  return 0;
    ...
    rgw_rados_ref* ref)
{
  ...
  if (r < 0) {
    ldpp_dout(dpp, 0) << "ERROR: failed opening pool (pool=" << obj.pool
                      << "); r=" << r << dendl;
    return r;
  }
    ...
    optional_yield y)
{
  ...
  if (locator.empty()) {
    ldpp_dout(dpp, 20) << "object does not have a locator, nothing to fix"
                       << dendl;
    return 0;
  }
  ...
  ioctx.locator_set_key(
      string()); /* override locator for this object, use empty locator */
...
#define HEAD_SIZE 512 * 1024
  ...
  if (ret < 0) {
    ldpp_dout(dpp, -1) << "ERROR: rgw_rados_operate(oid=" << oid
                       << ") returned ret=" << ret << dendl;
    return ret;
  }

  if (size > HEAD_SIZE) {
    ldpp_dout(dpp, -1) << "ERROR: returned object size (" << size
                       << ") > HEAD_SIZE (" << HEAD_SIZE << ")" << dendl;
    return -EIO;
  }
  ...
  if (remove_bad) {
    ...
    if (ret < 0) {
      ldpp_dout(dpp, -1) << "ERROR: failed to remove original bad object"
                         << dendl;
      return ret;
    }
  }
...
RGWRados::move_rados_obj(
    ...
    const string& src_locator,
    librados::IoCtx& dst_ioctx,
    const string& dst_oid,
...
#define COPY_BUF_SIZE (4 * 1024 * 1024)
  ...
  if (ofs != size) {
    ldpp_dout(dpp, -1) << "ERROR: " << __func__ << ": copying " << src_oid
                       << " -> " << dst_oid << ": expected " << size
                       << " bytes to copy, ended up with " << ofs << dendl;
    ret = -EIO;
    goto done_err;
  }
...
done_err:
  ...
  ldpp_dout(dpp, -1) << "ERROR: failed to copy " << src_oid << " -> " << dst_oid
                     << dendl;
  return ret;
}
    ...
    optional_yield y)
{
  ...
  if (manifest) {
    ...
    for (miter = manifest->obj_begin(dpp); miter != manifest->obj_end(dpp);
         ++miter) {
      ...
      RGWSI_Tier_RADOS::raw_obj_to_obj(
          manifest->get_tail_placement().bucket, raw_loc, &loc);
      ...
      ldpp_dout(dpp, 20) << __func__ << ": key=" << key << " oid=" << oid
                         << " locator=" << locator << dendl;
      ...
      ldpp_dout(dpp, 20) << __func__ << ": found bad object part: " << loc
                         << dendl;
      if (need_fix) {
        *need_fix = true;
      }
      if (fix) {
        ...
        if (r < 0) {
          ldpp_dout(dpp, -1) << "ERROR: copy_rados_obj() on oid=" << oid
                             << " returned r=" << r << dendl;
        }
      }
    }
    ...
    optional_yield y)
{
  ...
  RGWBucketInfo bucket_info;
  RGWBucketInfo* bucket_info_p = bucket_info_out ? bucket_info_out
                                                 : &bucket_info;

  int ret = store->get_bucket_instance_info(
      bucket, *bucket_info_p, NULL, NULL, y, dpp);
  if (ret < 0) {
    return ret;
  }
  ...
  if (ret < 0) {
    ldpp_dout(dpp, 0) << "ERROR: open_bucket_index_shard() returned ret=" << ret
                      << dendl;
    return ret;
  }
    ...
    optional_yield y)
{
  ...
  int ret = store->svc.bi_rados->open_bucket_index_shard(
      dpp, bucket_info, obj.get_hash_object(), &bucket_obj, &shard_id);
  if (ret < 0) {
    ldpp_dout(dpp, 0) << "ERROR: open_bucket_index_shard() returned ret=" << ret
                      << dendl;
    return ret;
    ...
    optional_yield y)
{
  ...
  int ret = store->svc.bi_rados->open_bucket_index_shard(
      dpp, bucket_info, index, shard_id, &bucket_obj);
  if (ret < 0) {
    ldpp_dout(dpp, 0) << "ERROR: open_bucket_index_shard() returned ret=" << ret
                      << dendl;
    return ret;
...
int
RGWRados::on_last_entry_in_listing(
    const DoutPrefixProvider* dpp,
    RGWBucketInfo& bucket_info,
    ...
    optional_yield y)
{
  ...
  ldpp_dout(dpp, 20) << "iterating listing for bucket="
                     << bucket_info.bucket.name << ", obj_prefix=" << obj_prefix
  ...
  do {
    ...
    int ret = list_op.list_objects(
        dpp, MAX_LIST_OBJS, &entries, nullptr, &is_truncated, y);
    if (ret < 0) {
      return ret;
...
bool
RGWRados::swift_versioning_enabled(const RGWBucketInfo& bucket_info) const
{
...
int
RGWRados::swift_versioning_copy(
    ...
    optional_yield y)
{
  ...
  RGWObjManifest* manifest = nullptr;
  int r = get_obj_state(
      dpp, &obj_ctx, bucket_info, obj, &state, &manifest, false, y);
  if (r < 0) {
    return r;
  }
  ...
  snprintf(
      buf, sizeof(buf), "%03x%s/%lld.%06ld", (int)src_name.size(),
      src_name.c_str(), (long long)ts.tv_sec, ts.tv_nsec / 1000);

  RGWBucketInfo dest_bucket_info;

  r = get_bucket_info(
      &svc, bucket_info.bucket.tenant, bucket_info.swift_ver_location,
      dest_bucket_info, NULL, y, NULL);
  if (r < 0) {
  ...
  r = copy_obj(
      obj_ctx, obj_ctx, /* src and dest share an obj_ctx */
      owner, remote_user, NULL, /* req_info *info */
      no_zone, dest_obj, obj, dest_bucket_info, bucket_info,
      bucket_info.placement_rule, NULL, /* time_t *src_mtime */
      NULL, /* time_t *mtime */
      ...
      RGWRados::ATTRSMOD_NONE, true, /* bool copy_if_newer */
      state->attrset, RGWObjCategory::Main, 0, /* uint64_t olh_epoch */
...
int
RGWRados::swift_versioning_restore(
    RGWObjectCtx& obj_ctx,
    const ACLOwner& owner,
    const rgw_user& remote_user,
    RGWBucketInfo& bucket_info,
    ...
    optional_yield y)
{
  ...
  const auto handler = [&](const rgw_bucket_dir_entry& entry) -> int {
    ...
    int ret = copy_obj(
        obj_ctx, obj_ctx, /* src and dest share an obj_ctx */
        owner, remote_user, nullptr, /* req_info *info */
        no_zone, obj, /* dest obj */
        archive_obj, /* src obj */
        ...
        RGWRados::ATTRSMOD_NONE, true, /* bool copy_if_newer */
        no_attrs, RGWObjCategory::Main, 0, /* uint64_t olh_epoch */
        real_time(), /* time_t delete_at */
    ...
    ret = delete_obj(
        dpp, obj_ctx, archive_binfo, archive_obj,
  ...
  const auto prefix =
      boost::str(boost::format("%03x%s") % obj_name.size() % obj_name);

  return on_last_entry_in_listing(
      dpp, archive_binfo, prefix, std::string(), handler, y);
}
    ...
    bool log_op)
{
  RGWRados::Bucket::UpdateIndex* index_op =
      static_cast<RGWRados::Bucket::UpdateIndex*>(_index_op);
  RGWRados* store = target->get_store();
  ...
  RGWObjState* state;
  RGWObjManifest* manifest = nullptr;
  int r = target->get_state(
      rctx.dpp, &state, &manifest, false, rctx.y, assume_noent);
  if (r < 0)
    return r;
  ...
  if (obj.get_oid().empty()) {
    ldpp_dout(rctx.dpp, 0) << "ERROR: " << __func__
                           << "(): cannot write object with empty name"
                           << dendl;
    return -EIO;
  }

  rgw_rados_ref ref;
  r = store->get_obj_head_ref(
      rctx.dpp, target->get_meta_placement_rule(), obj, &ref);
  if (r < 0)
    return r;
  ...
  r = target->check_preconditions(
      rctx.dpp, std::nullopt, real_clock::zero(), false, meta.if_match,
      meta.if_nomatch, *current_state, rctx.y);
  if (r < 0) {
    return r;
  }
  bool guard = ((target->manifest) || (target->state->obj_tag.length() != 0)) &&
               (!target->state->fake_tag);
  bool set_attr_id_tag = guard && target->obj.key.instance.empty() &&
                         (meta.if_nomatch == nullptr ||
                          meta.if_nomatch != "*"sv);
  r = target->prepare_atomic_modification(
      rctx.dpp, op, reset_obj, ptag, meta.modify_tail, set_attr_id_tag, rctx.y);
  if (r < 0)
    return r;
  ...
  if (target->get_bucket_info().obj_lock_enabled() &&
      target->get_bucket_info().obj_lock.has_rule() &&
  ...
  if (meta.manifest) {
    storage_class =
        meta.manifest->get_tail_placement().placement_rule.storage_class;
  ...
  bool versioned_op =
      (target->versioning_enabled() || is_olh || versioned_target);
  ...
  r = rgw_rados_operate(
      rctx.dpp, ref.ioctx, ref.obj.oid, std::move(op), rctx.y, 0, &trace,
      &epoch);
  ...
  if (r < 0) {
    ldpp_dout(rctx.dpp, 0) << "ERROR: complete_atomic_modification returned r="
                           << r << dendl;
  }

  tracepoint(rgw_rados, complete_enter, req_id.c_str());
  r = index_op->complete(
      rctx.dpp, poolid, epoch, size, accounted_size, meta.set_mtime, etag,
      content_type, storage_class, meta.owner, meta.category, meta.remove_objs,
      rctx.y, meta.user_data, meta.appendable, log_op);
  ...
  if (versioned_op && meta.olh_epoch) {
    ...
    r = store->set_olh(
        rctx.dpp, target->get_ctx(), target->get_bucket_info(), obj, false,
        NULL, *meta.olh_epoch, real_time(), false, rctx.y, meta.zones_trace,
        add_log);
    if (r < 0) {
      return r;
    }
  ...
  if (!real_clock::is_zero(meta.delete_at)) {
    ...
    r = store->obj_expirer->hint_add(
        rctx.dpp, meta.delete_at, obj.bucket.tenant, obj.bucket.name,
        obj.bucket.bucket_id, obj_key);
    if (r < 0) {
      ldpp_dout(rctx.dpp, 0) << "ERROR: objexp_hint_add() returned r=" << r
  ...
  if (meta.completeMultipart) {
    store->quota_handler->update_stats(
        meta.bucket_owner, obj.bucket, (orig_exists ? 0 : 1), 0, orig_size);
  } else {
    store->quota_handler->update_stats(
        meta.bucket_owner, obj.bucket, (orig_exists ? 0 : 1), accounted_size,
        orig_size);
  }
  return 0;
...
done_cancel:
  ...
  if (r != -ETIMEDOUT) {
    ...
    if (ret < 0) {
      ldpp_dout(rctx.dpp, 0)
          << "ERROR: index_op.cancel() returned ret=" << ret << dendl;
    }
    ...
    bool log_op)
{
  ...
  if (assume_noent) {
    r = _do_write_meta(
        size, accounted_size, attrs, assume_noent, (void*)&index_op, rctx,
        trace, log_op);
    if (r == -EEXIST) {
      assume_noent = false;
    }
...
public:
  RGWRadosPutObj(
      ...
      const rgw_obj& src_obj,
      const std::optional<RGWUserPermHandler::Bucket>& _src_bucket_perms =
          std::nullopt) :
    dpp(dpp),
    cct(cct),
    filter(p),
  ...
  process_attrs(void)
  {
    ...
    if (extra_data_bl.length()) {
      ...
      if (!jp.parse(extra_data_bl.c_str(), extra_data_bl.length())) {
        ldpp_dout(dpp, 0) << "failed to parse response extra data. len="
                          << extra_data_bl.length()
      ...
      if (iter != src_attrs.end()) {
        ...
        if (src_attrs.count(RGW_ATTR_CRYPT_MODE) &&
          ...
          int r = RGWGetObj_BlockDecrypt::read_manifest_parts(
              dpp, manifest_bl, parts_len);
          if (r < 0) {
            ldpp_dout(dpp, 4)
                << "failed to read part lengths from the manifest" << dendl;
          } else {
    ...
    if (try_etag_verify) {
      ret = rgw::putobj::create_etag_verifier(
          dpp, cct, filter, manifest_bl, compression_info, etag_verifier);
      if (ret < 0) {
  ...
  int
  handle_data(bufferlist& bl, bool* pause) override
  {
    if (progress_cb) {
  ...
  void
  set_extra_data_len(uint64_t len) override
  {
    extra_data_left = len;
  ...
  int
  handle_headers(const map<string, string>& headers, int http_status) override
  {
    if (src_bucket_perms && http_status != 403 && http_status != 401) {
      ...
      if (iter == headers.end()) {
        ...
        if (!src_bucket_perms->verify_bucket_permission(
                src_obj.key, rgw::IAM::s3ReplicateObject)) {
          ldpp_dout(dpp, 4) << "ERROR: " << __func__
                            << "(): fallback permission check denied" << dendl;
          return -EACCES;
        }
      }
    }
    ...
    RGWRados::AttrsMod attrs_mod)
{
  ...
  case RGWRados::ATTRSMOD_MERGE:
    for (map<string, bufferlist>::iterator it = src_attrs.begin();
         it != src_attrs.end(); ++it) {
    ...
    optional_yield y)
{
  ...
  return copy_obj_data(
      octx, owner, dest_bucket_info, dest_bucket_info.placement_rule, read_op,
      obj_size - 1, obj, NULL, mtime, attrset, 0, real_time(), NULL, NULL, dpp,
      y);
}
    ...
    optional_yield y)
{
  ...
  auto link_helper = [&](const bool is_delete_marker,
    ...
    ret = get_obj_state(
        dpp, &obj_ctx, bucket_info, olh_obj, &olh_state, &olh_manifest,
        false, // don't follow olh
        y);
    if (ret < 0) {
      ldpp_dout(dpp, 0) << "ERROR: " << __func__ << ": during " << log_tag
                        << " get_obj_state on OLH object " << olh_obj.key
    ...
    ret = bucket_index_link_olh(
        dpp, bucket_info, *olh_state, head_obj, is_delete_marker, empty_op_tag,
        &meta,
    ...
    // this trims it out
    ret = bucket_index_trim_olh_log(
        dpp, bucket_info, *olh_state, head_obj, max_ver, y);
    if (ret < 0) {
      ldpp_dout(dpp, 0) << "ERROR: " << __func__ << ": during " << log_tag
  ...
  if (ret < 0) {
    ldpp_dout(dpp, 0) << "ERROR: " << __func__ << ": get_obj_head_ioctx for "
                      << p(head_obj) << " returned: " << cpp_strerror(-ret)
                      << dendl;
    return ret;
  }
  ...
  ldpp_dout(dpp, 20) << "INFO: " << __func__ << ": reindexing " << p(head_obj)
                     << dendl;
  ...
  ret = get_obj_state(
      dpp, &obj_ctx, bucket_info, head_obj, &head_state, &head_manifest,
      false, // don't follow olh
      y);
  if (ret < 0) {
    ldpp_dout(dpp, 0) << "ERROR: " << __func__ << ": get_obj_state on "
                      << p(head_obj) << " returned: " << cpp_strerror(-ret)
                      << dendl;
    return ret;
  }
  ...
  if (!head_state->exists && is_versioned && has_instance) {
    ...
    // and return
    ldpp_dout(dpp, 20) << "INFO: " << __func__ << ": indexing " << p(head_obj)
                       << " as delete marker" << dendl;
    ...
    ldpp_dout(dpp, 0) << "ERROR: " << __func__
                      << ": unable to complete stat of " << p(head_obj)
                      << "; returned: " << cpp_strerror(-ret) << dendl;
    return ret;
  }
  ...
  if (found_olh_info) {
    try {
      ...
      if (!info.target.key.instance.empty()) {
        ...
        ldpp_dout(dpp, 20) << "INFO: " << __func__ << ": " << p(head_obj)
                           << " appears to be a pure OLH object; ignoring"
                           << dendl;
        return 0;
      }
    } catch (buffer::error& err) {
      ldpp_dout(dpp, 0) << "ERROR: " << __func__
                        << ": unable to decode OLH info for " << p(head_obj)
                        << dendl;
      return -EIO;
    }
  }
  ...
  ret = svc.bi_rados->open_bucket_index_shard(
      dpp, bucket_info, head_obj.get_hash_object(), &bucket_ref, nullptr);
  if (ret < 0) {
  ...
  if (ret == -ENOENT) {
    ...
    int create_ret = svc.bi_rados->init_index(
        dpp, y, bucket_info, bucket_info.layout.current_index, false);
  ...
  ret = update_idx.complete(
      dpp, pool_id,
      0, // bucket index epoch
      head_state->size, head_state->accounted_size, head_state->mtime, etag,
      content_type, storage_class, owner,
      ...
      appendable);
  if (ret < 0) {
    ldpp_dout(dpp, 0) << "ERROR: " << __func__ << ": update index complete for "
                      << p(head_obj) << " returned: " << cpp_strerror(-ret)
                      << dendl;
    return ret;
  }
  ...
  if (bucket_info.versioned()) {
    ldpp_dout(dpp, 20) << "INFO: " << __func__ << ": since "
                       << bucket_info.bucket
                       << " appears to be versioned, setting OLH for "
                       << p(head_obj) << dendl;
...
struct obj_time_weight {
  ...
  obj_time_weight() :
    zone_short_id(0), pg_ver(0), high_precision(false)
  {}

  bool
  compare_low_precision(const obj_time_weight& rhs)
  {
  ...
  bool
  operator<(const obj_time_weight& rhs)
  {
  ...
  void
  init(const real_time& _mtime, uint32_t _short_id, uint64_t _pg_ver)
  {
    mtime = _mtime;
...
inline ostream&
operator<<(ostream& out, const obj_time_weight& o)
{
  out << o.mtime;
    ...
    optional_yield y)
{
  ...
  if (source_zone.empty()) {
    if (!src_bucket_info || src_bucket_info->zonegroup.empty()) {
      ...
      map<string, RGWRESTConn*>::iterator iter =
          zonegroup_conn_map.find(src_bucket_info->zonegroup);
      if (iter == zonegroup_conn_map.end()) {
        ldpp_dout(dpp, 0)
            << "could not find zonegroup connection to zonegroup: "
            << source_zone << dendl;
        return -ENOENT;
      }
      conn = iter->second;
    ...
    if (iter == zone_conn_map.end()) {
      ldpp_dout(dpp, 0) << "could not find zone connection to zone: "
                        << source_zone << dendl;
      return -ENOENT;
    }
    conn = iter->second;
  ...
  for (int tries = 0; tries < NUM_ENPOINT_IOERROR_RETRIES; tries++) {
    int ret = conn->get_obj(
        dpp, user_id, nullptr, info, src_obj, pmod, unmod_ptr,
        dest_mtime_weight.zone_short_id, dest_mtime_weight.pg_ver, prepend_meta,
        get_op, rgwx_stat, sync_manifest, skip_decrypt, nullptr,
        sync_cloudtiered, true, &cb, &in_stream_req);
    if (ret < 0) {
      return ret;
    }

    ret = conn->complete_request(
        dpp, in_stream_req, nullptr, &set_mtime, psize, nullptr, pheaders, y);
    if (ret < 0) {
      if (ret == -ERR_INTERNAL_ERROR &&
          tries < NUM_ENPOINT_IOERROR_RETRIES - 1) {
        ldpp_dout(dpp, 20) << __func__
                           << "(): failed to fetch object from remote. retries="
                           << tries << dendl;
        continue;
      }
      return ret;
  ...
  if (extra_data_bl.length()) {
    ...
    src_attrs.erase(
        RGW_ATTR_MANIFEST); // not interested in original object layout
  }
...
int
RGWFetchObjFilter_Default::filter(
    CephContext* cct,
    ...
    const rgw_placement_rule** prule)
{
  const rgw_placement_rule* ptail_rule =
      (dest_placement_rule ? &(*dest_placement_rule) : nullptr);
  if (!ptail_rule) {
    ...
    bool keep_tags)
{
  ...
  const string fetched_obj = fmt::format(
      "object(src={}:{}, dest={}:{})", src_obj.bucket.bucket_id,
      src_obj.key.name, dest_obj.bucket.bucket_id, dest_obj.key.name);
  ...
  AtomicObjectProcessor processor(
      &aio, this, dest_bucket_info, nullptr, owner, dest_obj_ctx, dest_obj,
      olh_epoch, tag, rctx.dpp, rctx.y, no_trace);
  RGWRESTConn* conn;
  auto& zone_conn_map = svc.zone->get_zone_conn_map();
  auto& zonegroup_conn_map = svc.zone->get_zonegroup_conn_map();
  if (source_zone.empty()) {
    ...
    if (iter == zone_conn_map.end()) {
      ldpp_dout(rctx.dpp, 0)
          << "could not find zone connection to zone: " << source_zone << dendl;
      return -ENOENT;
    }
    conn = iter->second;
  }
  ...
  if (perm_check_uid) {
    ...
    if (ret < 0) {
      ldpp_dout(rctx.dpp, 0) << "ERROR: failed to load src bucket=" << src_obj
                             << " ret=" << ret << dendl;
      return ret;
    }
    ...
    if (ret < 0) {
      ldpp_dout(rctx.dpp, 0)
          << "ERROR: failed to init user perms for: " << src_obj
          << " ret=" << ret << dendl;
      return ret;
    }

    src_bucket_perms.emplace();
    ret = user_perms.init_bucket(
        src_bucket->get_info(), src_bucket->get_attrs(), &*src_bucket_perms);
    if (ret < 0) {
      ldpp_dout(rctx.dpp, 0)
          << "ERROR: failed to init bucket perms for: " << src_obj
          << " ret=" << ret << dendl;
      return ret;
    }
  }
  ...
  RGWRadosPutObj cb(
      rctx.dpp, cct, plugin, compressor, &processor, progress_cb, progress_data,
      [&](map<string, bufferlist>& obj_attrs) {
        const rgw_placement_rule* ptail_rule;

        int ret = filter->filter(
            cct, src_obj.key, dest_bucket_info, dest_placement_rule, obj_attrs,
            &override_owner, &ptail_rule);
        if (ret < 0) {
        ...
        const auto& compression_type =
            svc.zone->get_zone_params().get_compression_type(*ptail_rule);
  ...
  rgw_zone_set_entry dst_zone_trace(
      svc.zone->get_zone().id, dest_bucket_info.bucket.get_key());

  if (copy_if_newer) {
    /* need to get mtime for destination */
    ret = get_obj_state(
        rctx.dpp, &dest_obj_ctx, dest_bucket_info, stat_dest_obj, &dest_state,
        &manifest, stat_follow_olh, rctx.y);
    if (ret < 0)
      goto set_err_state;
  ...
  for (int tries = 0; tries < NUM_ENPOINT_IOERROR_RETRIES; tries++) {
    ret = conn->get_obj(
        rctx.dpp, user_id, perm_check_uid, info, src_obj, pmod, unmod_ptr,
        dest_mtime_weight.zone_short_id, dest_mtime_weight.pg_ver, prepend_meta,
        get_op, rgwx_stat, sync_manifest, skip_decrypt, &dst_zone_trace,
        sync_cloudtiered, true, &cb, &in_stream_req);
    if (ret < 0) {
      goto set_err_state;
    }
    ...
    ret = conn->complete_request(
        rctx.dpp, in_stream_req, &etag, &set_mtime, &accounted_size, nullptr,
        nullptr, rctx.y);
    if (ret < 0) {
      if (ret == -ERR_INTERNAL_ERROR &&
          tries < NUM_ENPOINT_IOERROR_RETRIES - 1) {
        ldpp_dout(rctx.dpp, 20)
            << __func__ << "(): failed to fetch " << fetched_obj
  ...
  if (cb.get_data_len() != accounted_size) {
    ...
    ldpp_dout(rctx.dpp, 0) << "ERROR: " << fetched_obj
                           << " truncated during fetching, expected "
                           << accounted_size << " bytes but received "
                           << cb.get_data_len() << dendl;
    goto set_err_state;
  }
  ...
  if (override_owner) {
    ...
    if (ctl.user->get_info_by_uid(
            rctx.dpp, *override_owner, &owner_info, rctx.y) < 0) {
  ...
  if (source_zone
    ...
    map<string, bufferlist>::iterator iter =
        cb.get_attrs().find(RGW_ATTR_DELETE_AT);
    if (iter != cb.get_attrs().end()) {
      try {
        decode(delete_at, iter->second);
      } catch (buffer::error& err) {
        ldpp_dout(rctx.dpp, 0)
            << "ERROR: failed to decode delete_at field in intra zone copy"
            << dendl;
      }
    }
  }
  ...
  if (copy_if_newer) {
    ...
    if (i != attrs.end() && i->second.length() > 0) {
      ...
      try {
        ...
        ldpp_dout(rctx.dpp, 0)
            << "ERROR: failed to decode pg ver attribute, ignoring" << dendl;
        /* non critical error */
      }
    }
  ...
  if (const auto& verifier_etag = cb.get_verifier_etag();
    ...
    trimmed_etag.erase(
        std::remove(trimmed_etag.begin(), trimmed_etag.end(), '\"'),
        trimmed_etag.end());

    if (verifier_etag != trimmed_etag) {
      ret = -EIO;
      ldpp_dout(rctx.dpp, 0)
          << "ERROR: source and destination objects don't match: "
          << fetched_obj << ". Expected etag:" << trimmed_etag
...
#define MAX_COMPLETE_RETRY 100
  for (i = 0; i < MAX_COMPLETE_RETRY; i++) {
    bool canceled = false;
    ret = processor.complete(
        accounted_size, etag, mtime, set_mtime, attrs, rgw::cksum::no_cksum,
        delete_at, nullptr, nullptr, nullptr, zones_trace, &canceled, rctx,
        rgw::sal::FLAG_LOG_OP);
    if (ret < 0) {
      goto set_err_state;
    }
    ...
    if (copy_if_newer && canceled) {
      ldpp_dout(rctx.dpp, 20)
          << "raced with another write of obj: " << fetched_obj << dendl;
      ...
      if (ret < 0) {
        ldpp_dout(rctx.dpp, 0)
            << "ERROR: " << __func__ << ": get_err_state() returned ret=" << ret
            << " " << fetched_obj << dendl;
        goto set_err_state;
      }
      dest_mtime_weight.init(dest_state);
      ...
      if (!dest_state->exists || dest_mtime_weight < set_mtime_weight) {
        ldpp_dout(rctx.dpp, 20)
            << "retrying writing " << fetched_obj << " mtime=" << set_mtime
            << " dest_state->mtime=" << dest_state->mtime
            << " dest_state->exists=" << dest_state->exists << dendl;
        continue;
      } else {
        ldpp_dout(rctx.dpp, 20)
            << "not retrying writing " << fetched_obj << " mtime=" << set_mtime
            << " dest_state->mtime=" << dest_state->mtime
            << " dest_state->exists=" << dest_state->exists << dendl;
      }
    }
  ...
  if (i == MAX_COMPLETE_RETRY) {
    ldpp_dout(rctx.dpp, 0) << "ERROR: retried " << fetched_obj
                           << " completion too many times, something is wrong!"
                           << dendl;
    ret = -EIO;
    goto set_err_state;
  }
...
set_err_state:
  if (copy_if_newer && ret == -ERR_NOT_MODIFIED) {
    ...
    if (olh_epoch && *olh_epoch > 0) {
      ...
      ret = set_olh(
          rctx.dpp, dest_obj_ctx, dest_bucket_info, dest_obj, false, nullptr,
...
int
RGWRados::copy_obj_to_remote_dest(
    const DoutPrefixProvider* dpp,
    RGWObjState* astate,
    map<string, bufferlist>& src_attrs,
    ...
    optional_yield y)
{
  ...
  static constexpr int NUM_ENPOINT_IOERROR_RETRIES = 20;
  for (int tries = 0; tries < NUM_ENPOINT_IOERROR_RETRIES; tries++) {
    int ret = rest_master_conn->put_obj_async_init(
        dpp, user_id, dest_obj, src_attrs, &out_stream_req);
    if (ret < 0) {
      return ret;
    }
    ...
    ret = read_op.iterate(
        dpp, 0, astate->size - 1, out_stream_req->get_out_cb(), y);
    if (ret < 0) {
      delete out_stream_req;
      return ret;
    }
    ...
    ret =
        rest_master_conn->complete_request(dpp, out_stream_req, etag, mtime, y);
    if (ret < 0) {
      if (ret == -ERR_INTERNAL_ERROR &&
          tries < NUM_ENPOINT_IOERROR_RETRIES - 1) {
        ldpp_dout(dpp, 20) << __func__
                           << "(): failed to put_obj_async_init. retries="
                           << tries << dendl;
        continue;
      }
      return ret;
    ...
    jspan_context& trace)
{
  ...
  if (remote_src && remote_dest) {
    ldpp_dout(dpp, 0)
        << "ERROR: can't copy object when both src and dest buckets are remote"
        << dendl;
    return -EINVAL;
  }

  ldpp_dout(dpp, 5) << "Copy object " << src_obj.bucket << ":"
                    << src_obj.get_oid() << " => " << dest_obj.bucket << ":"
  ...
  if (remote_src || !source_zone.empty()) {
    ...
    int ret = 0;
    boost::asio::spawn(
        driver->get_io_context(),
        [&](boost::asio::yield_context yield) {
          const req_context rctx{dpp, yield, nullptr};
          const rgw_owner remote_user_owner(remote_user);
          ret = fetch_remote_obj(
              dest_obj_ctx, &remote_user_owner, &remote_user, info, source_zone,
              dest_obj, src_obj, dest_bucket_info, &src_bucket_info,
              dest_placement, src_mtime, mtime, mod_ptr, unmod_ptr,
              high_precision_time, if_match, if_nomatch, attrs_mod,
              copy_if_newer, attrs, category, olh_epoch, delete_at, ptag, petag,
              progress_cb, progress_data, rctx, nullptr /* filter */,
              stat_follow_olh, stat_dest_obj, std::nullopt);
  ...
  ret = get_obj_state(
      dpp, &src_obj_ctx, src_bucket_info, src_obj, &astate, &amanifest,
      follow_olh, y);
  if (ret < 0) {
    return ret;
  }
  ...
  if (remote_dest) {
    ...
    return copy_obj_to_remote_dest(
        dpp, astate, attrs, read_op, remote_user, dest_obj, mtime, y);
  }
  uint64_t max_chunk_size;

  ret = get_max_chunk_size(
      dest_bucket_info.placement_rule, dest_obj, &max_chunk_size, dpp);
  if (ret < 0) {
    ldpp_dout(dpp, 0) << "ERROR: failed to get max_chunk_size() for bucket "
                      << dest_obj.bucket << dendl;
    return ret;
  }
  ...
  if (amanifest) {
    ...
    ldpp_dout(dpp, 20) << __func__
                       << "(): manifest src_rule=" << src_rule->to_str()
                       << dendl;
  }
  ...
  if (!get_obj_data_pool(*src_rule, src_obj, &src_pool)) {
    ldpp_dout(dpp, 0) << "ERROR: failed to locate data pool for " << src_obj
                      << dendl;
    return -EIO;
  }

  if (!get_obj_data_pool(dest_placement, dest_obj, &dest_pool)) {
    ldpp_dout(dpp, 0) << "ERROR: failed to locate data pool for " << dest_obj
                      << dendl;
    return -EIO;
  }

  ldpp_dout(dpp, 20) << __func__ << "(): src_rule=" << src_rule->to_str()
                     << " src_pool=" << src_pool
                     << " dest_rule=" << dest_placement.to_str()
                     << " dest_pool=" << dest_pool << dendl;

  bool copy_data = (!amanifest) || (*src_rule != dest_placement) ||
                   (src_pool != dest_pool);
  ...
  if (copy_data) { /* refcounting tail wouldn't work here, just copy the data */
    ...
    return copy_obj_data(
        dest_obj_ctx, owner, dest_bucket_info, dest_placement, read_op,
        obj_size - 1, dest_obj, mtime, real_time(), attrs, olh_epoch, delete_at,
        petag, dp_factory, dpp, y);
  }
  ...
  ldpp_dout(dpp, 20) << "dest_obj=" << dest_obj << " src_obj=" << src_obj
                     << " copy_itself=" << (int)copy_itself << dendl;

  RGWRados::Object dest_op_target(
      this, dest_bucket_info, dest_obj_ctx, dest_obj);
  ...
  if (!copy_itself) {
    ...
    for (; miter != amanifest->obj_end(dpp); ++miter) {
      ...
      rgw_rados_ref obj;
      ret = rgw_get_rados_ref(
          dpp, driver->getRados()->get_rados_handle(),
          miter.get_location().get_raw_obj(this), &obj);
      if (ret < 0) {
        ldpp_dout(dpp, 0) << "failed to open rados context for " << obj
                          << dendl;
        goto done_ret;
      }
      ...
      rgw::AioResultList completed = aio->get(
          obj.obj, rgw::Aio::librados_op(obj.ioctx, std::move(op), y), cost, id);
      ret = rgw::check_for_errors(completed);
      all_results.splice(all_results.end(), completed);
      if (ret < 0) {
        ldpp_dout(dpp, 0) << "ERROR: failed to copy obj=" << obj
    ...
    all_results.splice(all_results.end(), completed);
    if (ret < 0) {
      ldpp_dout(dpp, 0) << "ERROR: failed to drain ios, the error code = "
                        << ret << dendl;
      goto done_ret;
    }
  ...
  if (copy_first) {
    ...
    pmanifest->set_head(
        dest_bucket_info.placement_rule, dest_obj, first_chunk.length());
  } else {
  ...
  ret =
      write_op.write_meta(obj_size, astate->accounted_size, attrs, rctx, trace);
  if (ret < 0) {
    goto done_ret;
  }
...
done_ret:
  if (!copy_itself) {
    ...
    for (auto& r : all_results) {
      ...
      if (ret2 < 0) {
        ldpp_dout(dpp, 0)
            << "ERROR: cleanup after error failed to drop reference on obj="
            << r.obj << dendl;
      }
    }
    completed = aio->drain();
    ...
    if (ret2 < 0) {
      ldpp_dout(dpp, 0)
          << "ERROR: failed to drain rollback ios, the error code = " << ret2
          << dendl;
    }
  }
  return ret;
    ...
    bool log_op)
{
  ...
  using namespace rgw::putobj;
  jspan_context no_trace{false, false};
  AtomicObjectProcessor aoproc(
      aio.get(), this, dest_bucket_info, &dest_placement, owner, obj_ctx,
      dest_obj, olh_epoch, tag, dpp, y, no_trace);
  ...
  if (ret < 0) {
    ldpp_dout(dpp, 0) << "ERROR: failed to read object data ret=" << ret
                      << dendl;
    return ret;
  }
  ...
  const req_context rctx{dpp, y, nullptr};
  return aoproc.complete(
      accounted_size, etag, mtime, set_mtime, attrs, rgw::cksum::no_cksum,
      delete_at, nullptr, nullptr, nullptr, nullptr, nullptr, rctx,
      log_op ? rgw::sal::FLAG_LOG_OP : 0);
}

int
fixup_manifest_to_parts_len(
    const DoutPrefixProvider* dpp,
    rgw::sal::Attrs& src_attrs)
{
  ...
  if (src_attrs.count(RGW_ATTR_CRYPT_MODE) &&
    ...
    int r = RGWGetObj_BlockDecrypt::read_manifest_parts(
        dpp, manifest_bl, parts_len);
    if (r < 0) {
      ldpp_dout(dpp, 0) << "failed to read part lengths from the manifest r="
                        << r << dendl;
      return r;
    }
    // store the encoded part lenghts in RGW_ATTR_CRYPT_PARTS
    ...
    bool log_op)
{
  ...
  if (read_mtime != mtime) {
    /* raced */
    ldpp_dout(dpp, 0) << __func__ << " ERROR: failed to transition obj("
                      << obj.key << ") read_mtime = " << read_mtime
                      << " doesn't match mtime = " << mtime << dendl;
    return -ECANCELED;
  }
  ...
  ret = copy_obj_data(
      obj_ctx, owner, bucket_info, placement_rule, read_op, obj_size - 1, obj,
      nullptr /* pmtime */, mtime, attrs, olh_epoch, real_time(),
      nullptr /* petag */, nullptr, /* dp_factory */
      dpp, y, log_op);
  if (ret < 0) {
    return ret;
  }
...
int
RGWRados::restore_obj_from_cloud(
    ...
    optional_yield y)
{
  ...
  rgw::putobj::AtomicObjectProcessor processor(
      aio.get(), this, dest_bucket_info, nullptr, owner, obj_ctx, dest_obj_bi,
      olh_epoch, tag, dpp, y, no_trace);
  ...
  rgw_placement_rule dest_placement(
      dest_bucket_info.placement_rule, tier_ctx.restore_storage_class);
  ...
  if (!tier_ctx.driver->valid_placement(dest_placement)) {
    ...
    ldpp_dout(dpp, 0) << "ERROR: non existent restore storage class:"
                      ...
                      << ". Restore of object(" << dest_obj
                      << ") , failed, ret=" << ret << dendl;
    return ret;
  }

  RGWRadosPutObj cb(
      dpp, cct, plugin, compressor, &processor, progress_cb, progress_data,
      [&](map<string, bufferlist> obj_attrs) {
  ...
  ret = read_op.prepare(y, dpp);
  if (ret < 0) {
    ldpp_dout(dpp, 0) << "Restoring object(" << dest_obj
  ...
  if (tier_config.tier_placement.tier_type == "cloud-s3-glacier") {
    ldpp_dout(dpp, 20) << "Restoring  object:" << dest_obj << " from the cloud"
                       << dendl;
    RGWZoneGroupTierS3Glacier& glacier_params =
        tier_config.tier_placement.s3_glacier;
    ret = rgw_cloud_tier_restore_object(
        tier_ctx, headers, &set_mtime, etag, accounted_size, attrs, days,
        glacier_params, in_progress, &cb);
  } else {
    ldpp_dout(dpp, 20) << "Fetching  object:" << dest_obj << "from the cloud"
                       << dendl;
    ret = rgw_cloud_tier_get_object(
        tier_ctx, false, headers, &set_mtime, etag, accounted_size, attrs, &cb);
    in_progress = false;
  }

  if (ret < 0) {
    ldpp_dout(dpp, 20) << "Fetching from cloud failed, object:" << dest_obj
                       << dendl;
    return ret;
  }

  if (in_progress) {
    ldpp_dout(tier_ctx.dpp, 20) << "Restoring object:" << dest_obj
                                << " still in progress; returning " << dendl;
    return ret;
  }

  if (!cb_processed) {
    ldpp_dout(dpp, 20) << "Callback not processed, object:" << dest_obj
                       << dendl;
    return -EIO;
  }
...

  {
    if (!olh_epoch) {
      ...
      if (aiter != std::end(attrs)) {
        std::optional<uint64_t> olh_ep =
            ceph::parse<uint64_t>(rgw_bl_str(aiter->second));
        if (olh_ep) {
          olh_epoch = *olh_ep;
  ...
  ceph::real_time restore_time = real_clock::now();
  {
    ...
    attrs[RGW_ATTR_RESTORE_TIME] = attrs[RGW_ATTR_INTERNAL_MTIME] =
        std::move(bl);
  }
  ...
  if (days) { //temp copy; do not change mtime and set expiry date
    ...
    ldpp_dout(dpp, 5) << "Setting Restore expiration time to: "
                      << expiration_date << " , restore_time: " << restore_time
                      << ", restore_interval: "
                      << cct->_conf->rgw_restore_debug_interval << dendl;
    {
      bufferlist bl;
    ...
    }
    {
      ...
      ldpp_dout(dpp, 20) << "Setting RGW_ATTR_CLOUDTIER_STORAGE_CLASS: "
                         << tier_ctx.storage_class << dendl;
    }
  ...
  ret = processor.complete(
      accounted_size, etag, &mtime, set_mtime, attrs, rgw::cksum::no_cksum,
      delete_at, nullptr, nullptr, nullptr, (rgw_zone_set*)&zone_set, &canceled,
    ...
    optional_yield y)
{
  ...
  do {
    ...
    int r = cls_bucket_list_unordered(
        dpp, bucket_info, bucket_info.layout.current_index, RGW_NO_SHARD,
        marker, prefix, NUM_ENTRIES, true, ent_list, &is_truncated, &marker, y);
    if (r < 0) {
      return r;
    }
...
int
RGWRados::store_delete_bucket_info_flag(
    RGWBucketInfo& bucket_info,
    std::map<std::string, bufferlist>& attrs,
    optional_yield y,
    const DoutPrefixProvider* dpp)
{
  ...
  do {
    ...
    bucket_info.layout.logs.push_back(
        {log.gen + 1, {rgw::BucketLogType::Deleted}});
    r = ctl.bucket->store_bucket_instance_info(
    ...
    if (r == -ECANCELED) {
      ...
      int ret = get_bucket_instance_info(
          bucket, bucket_info, nullptr, &attrs, y, dpp);
      if (ret < 0) {
        ldpp_dout(dpp, 5)
            << "ERROR: failed to get bucket instance info for bucket="
  ...
  if (r == 0) {
    for (int i = 0; i < shards_num; ++i) {
      ldpp_dout(dpp, 10) << "adding to data_log shard_id: " << i
                         << " of gen:" << index_log.gen << dendl;
      int ret = svc.datalog_rados->add_entry(
          dpp, bucket_info, index_log, i, null_yield);
      if (ret < 0) {
        ldpp_dout(dpp, 1) << "WARNING: failed writing data log for bucket="
                          << bucket_info.bucket << ", shard_id=" << i
                          << "of generation=" << index_log.gen << dendl;
    ...
    optional_yield y)
{
  ...
  int ret = driver->get_sync_policy_handler(
      dpp, source_zone, bucket, &source_handler, y);
  if (ret < 0) {
    ldpp_dout(dpp, 10) << "failed to get bucket sync policy handler (r=" << ret
                       << ")" << dendl;
    return ret;
  }
    ...
    optional_yield y)
{
  ...
  ret = crs.run(
      dpp, new RGWStatRemoteBucketCR(
               dpp, driver, source_zone, bucket, &http, zids, peer_status));
  if (ret < 0) {
    ldpp_dout(dpp, 0) << "failed to fetch remote bucket stats "
                      << cpp_strerror(ret) << dendl;
    return ret;
  }
    ...
    optional_yield y)
{
  ...
  if (ret < 0) {
    ldpp_dout(dpp, 10) << "failed to get remote zones (r=" << ret << ")"
                       << dendl;
    return ret;
  }
  ...
  if (is_zg_policy_directional || is_bucket_policy_directional ||
    ...
    return list_remote_buckets(
        dpp, driver, source_zone, zids, bucket_info.bucket, y);
  }
  ...
  if (!opt_out_zones.empty()) {
    ldpp_dout(dpp, 10) << "sync policy exists. listing remote zones" << dendl;
    return list_remote_buckets(
        dpp, driver, source_zone, opt_out_zones, bucket_info.bucket, y);
  }
  return 0;
}
    ...
    bool check_empty)
{
  ...
  int r = svc.bi_rados->open_bucket_index(
      dpp, bucket_info, std::nullopt, bucket_info.layout.current_index,
  ...
  if (svc.zone->is_syncing_bucket_meta()) {
    ...
    int r = check_remote_bucket_empty(
        dpp, driver, source_zone, bucket_info, svc.zone->get_zonegroup().zones,
        &svc.zone->get_zonegroup().sync_policy, y);
    if (r == -ENOTEMPTY) {
      ldpp_dout(dpp, 0) << "ERROR: cannot delete bucket. objects exist in the "
                           "bucket on another zone "
                        << dendl;
      return r;
    } else if (r < 0) {
  ...
  if (objv_tracker.read_version.empty()) {
    ...
    r = ctl.bucket->read_bucket_entrypoint_info(
        bucket_info.bucket, &ep, null_yield, dpp,
        RGWBucketCtl::Bucket::GetParams().set_objv_tracker(&objv_tracker));
    if (r < 0 || (!bucket_info.bucket.bucket_id.empty() &&
                  ep.bucket.bucket_id != bucket_info.bucket.bucket_id)) {
      if (r != -ENOENT) {
        ldpp_dout(dpp, 0) << "ERROR: read_bucket_entrypoint_info() bucket="
                          << bucket_info.bucket << " returned error: r=" << r
                          << dendl;
  ...
  if (remove_ep) {
    r = ctl.bucket->remove_bucket_entrypoint_info(
        bucket_info.bucket, y, dpp,
        RGWBucketCtl::Bucket::RemoveParams().set_objv_tracker(&objv_tracker));
    if (r < 0)
      return r;
  }
  ...
  if (!svc.zone->is_syncing_bucket_meta()) {
    ...
    std::ignore = svc.bi->clean_index(
        dpp, y, bucket_info, bucket_info.layout.current_index);
  } else {
    ...
    if (r < 0) {
      ...
      ldpp_dout(dpp, 0)
          << "WARNING: failed to store bucket info flag 'deleted' on bucket: "
          << bucket.name << " r=" << r << dendl;
    } else {
      ldpp_dout(dpp, 20)
          << "INFO: setting bucket info flag to deleted for bucket: "
          << bucket.name << dendl;
    }
  }
    ...
    optional_yield y)
{
  ...
  if (bucket.bucket_id.empty()) {
    r = get_bucket_info(
        &svc, bucket.tenant, bucket.name, info, NULL, y, dpp, &attrs);
  } else {
    r = get_bucket_instance_info(bucket, info, nullptr, &attrs, y, dpp);
  }
  if (r < 0) {
    ldpp_dout(dpp, 0) << "NOTICE: get_bucket_info on bucket=" << bucket.name
                      << " returned err=" << r << dendl;
    return r;
  }
  ...
  if (r < 0) {
    ldpp_dout(dpp, 0) << "NOTICE: put_bucket_info on bucket=" << bucket.name
                      << " returned err=" << r << dendl;
    return r;
  }
    ...
    optional_yield y)
{
  ...
  for (iter = buckets.begin(); iter != buckets.end(); ++iter) {
    ...
    if (r < 0) {
      ldpp_dout(dpp, 0) << "NOTICE: get_bucket_info on bucket=" << bucket.name
                        << " returned err=" << r << ", skipping bucket"
                        << dendl;
      ret = r;
      continue;
    }
    ...
    optional_yield y)
{
  RGWBucketInfo bucket_info;
  int ret = get_bucket_info(
      &svc, bucket.tenant, bucket.name, bucket_info, NULL, y, dpp);
  if (ret < 0) {
    return ret;
  }
    ...
    optional_yield y)
{
  ...
  string tag =
      (state->tail_tag.length() > 0 ? state->tail_tag.to_str()
                                    : state->obj_tag.to_str());
  if (store->gc == nullptr) {
    ldpp_dout(dpp, 0) << "deleting objects inline since gc isn't initialized"
                      << dendl;
    //Delete objects inline just in case gc hasn't been initialised, prevents crashes
    store->delete_objs_inline(dpp, chain, tag, y);
  } else {
    auto [ret, leftover_chain] =
        store->gc->send_split_chain(chain, tag, y); // do it synchronously
...
std::tuple<int, std::optional<cls_rgw_obj_chain>>
    ...
    optional_yield y)
{
  ...
  for (; obj != chain.objs.end(); ++obj) {
    ...
    auto completed = aio->get(
        std::move(raw), rgw::Aio::librados_op(ioctx, std::move(op), y), cost,
        id);
    ...
    const RGWObjState& state)
{
  ...
  return store->set_attrs(
      dpp, &obj_ctx, bucket_info, state.obj, add_attrs, nullptr, y, true,
      set_mtime);
}

static void
try_resync_encrypted_multipart(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    RGWRados* store,
    RGWBucketInfo& bucket_info,
    ...
    Formatter* f)
{
  ...
  int ret = store->get_obj_state(
      dpp, &obj_ctx, bucket_info, obj, &astate, &manifest, follow_olh, y);
  if (ret < 0) {
...
int
RGWRados::bucket_resync_encrypted_multipart(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    ...
    RGWFormatterFlusher& flusher)
{
  ...
  do {
    ...
    for (const auto& dirent : entries) {
      RGWObjectCtx obj_ctx{driver};
      try_resync_encrypted_multipart(
          dpp, y, this, bucket_info, obj_ctx, dirent, f);
    }
    ...
    optional_yield y)
{
  ...
  int r =
      get_obj_state(dpp, octx, bucket_info, obj, &state, &manifest, false, y);
  if (r < 0)
    return r;

  if (!state->is_atomic) {
    ldpp_dout(dpp, 20) << "state for obj=" << obj
  ...
  if (state->tail_tag.length() > 0) {
    ...
    ldpp_dout(dpp, 20) << "state->obj_tag is empty, not deferring gc operation"
                       << dendl;
    return -EINVAL;
  }
...
void
RGWRados::remove_rgw_head_obj(ObjectWriteOperation& op)
{
...
void
RGWRados::cls_obj_check_prefix_exist(
    ObjectOperation& op,
    const string& prefix,
    bool fail_if_exist)
{
...
struct tombstone_entry {
  ...
  explicit tombstone_entry(const RGWObjState& state) :
    mtime(state.mtime), zone_short_id(state.zone_short_id), pg_ver(state.pg_ver)
  {}
};
    ...
    const bool skip_olh_obj_update)
{
  ...
  if (params.versioning_status & BUCKET_VERSIONED || explicit_marker_version) {
    ...
    if (instance.empty() || explicit_marker_version) {
      ...
      int r =
          target->get_state(dpp, &target->state, &target->manifest, false, y);
      if (r < 0)
        return r;
      RGWObjState* current_state = target->state;
      ...
      r = target->check_preconditions(
          dpp, params.size_match, params.last_mod_time_match,
          params.high_precision_time, params.if_match, nullptr, *current_state,
          y);
      if (r < 0) {
        return r;
      }
      ...
      r = store->set_olh(
          dpp, target->get_ctx(), target->get_bucket_info(), marker, true,
          &meta, params.olh_epoch, params.unmod_since,
      ...
      int r = store->bi_get_instance(
          dpp, target->get_bucket_info(), obj, &dirent, y);
      if (r < 0) {
        return r;
      }
      ...
      if (params.if_match && params.if_match != "*"sv) {
        ...
        ldpp_dout(dpp, 10) << "checking precondtion if_match: " << if_match
                           << ", etag: " << dirent.meta.etag << dendl;
      ...
      if (params.size_match.has_value()) {
        ldpp_dout(dpp, 10) << "checking precondtion size_match: "
                           << params.size_match
                           << ", size: " << dirent.meta.size << dendl;
      ...
      if (!real_clock::is_zero(params.last_mod_time_match)) {
        ...
        struct timespec last_mod_time =
            ceph::real_clock::to_timespec(params.last_mod_time_match);
        ...
        ldpp_dout(dpp, 10)
            << "checking precondtion If-Match-Last-Modified-Time: "
            << params.last_mod_time_match << ", Last-Modified: " << ctime
            << ", with high_precision_time:" << params.high_precision_time
            << dendl;
      ...
      r = store->unlink_obj_instance(
          ...
          params.olh_epoch, y, params.bilog_flags, params.null_verid,
          params.zones_trace, add_log, force, skip_olh_obj_update);
      if (r < 0) {
        return r;
      }
    ...
    if (add_log) {
      r = add_datalog_entry(
          dpp, store->svc.datalog_rados, target->get_bucket_info(),
          bs->shard_id, y);
      if (r < 0) {
        ldpp_dout(dpp, 0) << "failed to write datalog for object: r=" << r
                          << dendl;
        return r;
      }
    }
  ...
  if (!real_clock::is_zero(params.unmod_since)) {
    ...
    ldpp_dout(dpp, 10) << "If-UnModified-Since: " << params.unmod_since
                       << " Last-Modified: " << ctime << dendl;
    if (ctime > unmod) {
      return -ERR_PRECONDITION_FAILED;
    }
    ...
    store->cls_obj_check_mtime(
        op, params.unmod_since, params.high_precision_time,
  ...
  if (!real_clock::is_zero(params.expiration_time)) {
    ...
    if (state->get_attr(RGW_ATTR_DELETE_AT, bl)) {
      try {
        ...
        ldpp_dout(dpp, 0) << "ERROR: couldn't decode RGW_ATTR_DELETE_AT"
                          << dendl;
        return -EIO;
      }
  ...
  r = target->check_preconditions(
      dpp, params.size_match, params.last_mod_time_match,
      params.high_precision_time, params.if_match, nullptr, *state, y);
  if (!real_clock::is_zero(params.last_mod_time_match)) {
    /* only delete object if mtime is equal to params.last_mod_time_match */
    store->cls_obj_check_mtime(
        op, params.last_mod_time_match, params.high_precision_time,
        CLS_RGW_CHECK_TIME_MTIME_EQ);
  }
  if (r < 0) {
    return r;
  }
  ...
  version_t epoch = 0;
  r = rgw_rados_operate(
      dpp, ioctx, ref.obj.oid, std::move(op), y, 0, nullptr, &epoch);
  ...
  if (r == -ETIMEDOUT) {
    ...
    r = index_op.complete_del(
        dpp, poolid, epoch, state->mtime, params.remove_objs, y, log_op);

    int ret = target->complete_atomic_modification(dpp, false, y);
    if (ret < 0) {
      ldpp_dout(dpp, 0) << "ERROR: complete_atomic_modification returned ret="
                        << ret << dendl;
    }
    ...
    if (ret < 0) {
      ldpp_dout(dpp, 0) << "ERROR: index_op.cancel() returned ret=" << ret
                        << dendl;
    }
  }
  ...
  /* update quota cache */
  store->quota_handler->update_stats(
      params.bucket_owner, obj.bucket, -1, 0, obj_accounted_size);
...
RGWRados::delete_obj(
    ...
    optional_yield y, // versioning flags defined in enum RGWBucketFlags
    bool null_verid,
    ...
    const bool
        skip_olh_obj_update) // true for all deletes (except the last one) initiated by a multi-object delete op
{
    ...
    optional_yield y)
{
  ...
  RGWBucketInfo bucket_info;
  int ret =
      get_bucket_instance_info(obj.bucket, bucket_info, NULL, NULL, y, dpp);
  if (ret < 0) {
    ldpp_dout(dpp, 0) << "ERROR: " << __func__
                      << "() get_bucket_instance_info(bucket=" << obj.bucket
                      << ") returned ret=" << ret << dendl;
    return ret;
  }
...
static bool
is_olh(map<string, bufferlist>& attrs)
{
...
static bool
has_olh_tag(map<string, bufferlist>& attrs)
{
...
int
RGWRados::get_olh_target_state(
    const DoutPrefixProvider* dpp,
    RGWObjectCtx& obj_ctx,
    RGWBucketInfo& bucket_info,
    const rgw_obj& obj,
    RGWObjState* olh_state,
    RGWObjStateManifest** psm,
    optional_yield y)
{
  ceph_assert(olh_state->is_olh);

  rgw_obj target;
  int r = RGWRados::follow_olh(
      dpp, bucket_info, obj_ctx, olh_state, obj, &target,
      y); /* might return -EAGAIN */
  if (r < 0) {
    return r;
  }
    ...
    bool assume_noent)
{
  ...
  ldpp_dout(dpp, 20) << "get_obj_state: octx=" << (void*)octx << " obj=" << obj
                     << " state=" << (void*)s
  ...
  *psm = sm;
  ...
  if (!assume_noent) {
    r = RGWRados::raw_obj_stat(
        dpp, raw_obj, &s->size, &s->mtime, &s->epoch, &s->attrset,
        (s->prefetch_data ? &s->data : NULL), &s->objv_tracker, y);
  }
  ...
  if (compressed) {
    ...
    try {
      ...
      ldpp_dout(dpp, 0)
          << "ERROR: could not decode compression info for object: " << obj
          << dendl;
      return -EIO;
    }
  }
  ...
  if (iter = s->attrset.find(RGW_ATTR_MANIFEST); iter != s->attrset.end()) {
    ...
    try {
      ...
      sm->manifest->set_head(
          bucket_info.placement_rule, obj,
          s->size); /* patch manifest to reflect the head we just read, some manifests might be
    ...
    ldpp_dout(dpp, 10) << "manifest: total_size = "
                       << sm->manifest->get_obj_size() << dendl;
    if (cct->_conf->subsys.should_gather<ceph_subsys_rgw, 20>() &&
        sm->manifest->has_explicit_objs()) {
      RGWObjManifest::obj_iterator mi;
      for (mi = sm->manifest->obj_begin(dpp); mi != sm->manifest->obj_end(dpp);
           ++mi) {
        ldpp_dout(dpp, 20) << "manifest: ofs=" << mi.get_ofs()
                           << " loc=" << mi.get_location().get_raw_obj(this)
                           << dendl;
      }
    }
    ...
    if (!s->obj_tag.length()) {
       ...
       */
      generate_fake_tag(
          dpp, this, s->attrset, *sm->manifest, manifest_bl, s->obj_tag);
      s->fake_tag = true;
    }
  }
  if (iter = s->attrset.find(RGW_ATTR_PG_VER); iter != s->attrset.end()) {
    ...
    if (pg_ver_bl.length()) {
      ...
      try {
        ...
        ldpp_dout(dpp, 0) << "ERROR: couldn't decode pg ver attr for object "
                          << s->obj << ", non-critical error, ignoring"
                          << dendl;
      }
    }
  }
  if (iter = s->attrset.find(RGW_ATTR_SOURCE_ZONE); iter != s->attrset.end()) {
    ...
    if (zone_short_id_bl.length()) {
      ...
      try {
        ...
        ldpp_dout(dpp, 0)
            << "ERROR: couldn't decode zone short id attr for object " << s->obj
            << ", non-critical error, ignoring" << dendl;
      }
    }
  }
  if (s->obj_tag.length()) {
    ldpp_dout(dpp, 20) << "get_obj_state: setting s->obj_tag to "
  ...
  if (is_olh(s->attrset)) {
    ...
    ldpp_dout(dpp, 20) << __func__ << ": setting s->olh_tag to "
                       << string(s->olh_tag.c_str(), s->olh_tag.length())
                       << dendl;
    ...
    bool assume_noent)
{
  ...
  do {
    ret = get_obj_state_impl(
        dpp, octx, bucket_info, obj, psm, follow_olh, y, assume_noent);
  } while (ret == -EAGAIN);
    ...
    bool assume_noent)
{
  ...
  int r = get_obj_state(
      dpp, rctx, bucket_info, obj, &sm, follow_olh, y, assume_noent);
  if (pstate) {
    *pstate = &sm->state;
  }
  if (r < 0) {
...
int
RGWRados::Object::Stat::stat_async(const DoutPrefixProvider* dpp)
{
  ...
  int r = store->get_obj_head_ioctx(
      dpp, source->get_bucket_info(), obj, &state.io_ctx);
  if (r < 0) {
    return r;
  }
  ...
  if (r < 0) {
    ldpp_dout(dpp, 5) << __func__ << ": ERROR: aio_operate() returned ret=" << r
                      << dendl;
    return r;
  }
...
int
RGWRados::Object::Stat::wait(const DoutPrefixProvider* dpp)
{
  if (!state.completion) {
    return state.ret;
  }
...
int
RGWRados::Object::Stat::finish(const DoutPrefixProvider* dpp)
{
    ...
    optional_yield y)
{
  ...
  int r =
      get_obj_state(dpp, octx, bucket_info, obj, pstate, pmanifest, false, y);
  if (r < 0)
    return r;
...
{
  if (!state->is_atomic) {
    ldpp_dout(dpp, 20) << "state for obj=" << state->obj
                       << " is not atomic, not appending atomic test" << dendl;
    return 0;
  }

  if (state->obj_tag.length() > 0 &&
      !state->fake_tag) { // check for backward compatibility
    op.cmpxattr(RGW_ATTR_ID_TAG, LIBRADOS_CMPXATTR_OP_EQ, state->obj_tag);
  } else {
    ldpp_dout(dpp, 20) << "state->obj_tag is empty, not appending atomic test"
                       << dendl;
  }
  return 0;
}
...
RGWRados::Object::get_state(
    ...
    bool assume_noent)
{
  return store->get_obj_state(
      dpp, &ctx, bucket_info, obj, pstate, pmanifest, follow_olh, y,
      assume_noent);
}

void
RGWRados::Object::invalidate_state()
{
  ctx.invalidate(obj);
}

int
RGWRados::Object::get_current_version_state(
    const DoutPrefixProvider* dpp,
    RGWObjState*& current_state,
    optional_yield y)
{
  ...
  int r = store->get_obj_state(
      dpp, &ctx, bucket_info, current_obj, &current_state, nullptr, follow_olh,
      y);
  if (r < 0) {
    ldpp_dout(dpp, 4) << "failed to load current version or no current object "
                         "version for preconditions on "
                      << current_obj << dendl;
...
int
RGWRados::Object::check_preconditions(
    const DoutPrefixProvider* dpp,
    ...
    optional_yield y)
{
  if (size_match.has_value()) {
    ldpp_dout(dpp, 10) << "RGWRados::Object::check_preconditions size_match: "
                       << size_match.value() << ", size: " << current_state.size
                       << dendl;
  ...
  if (!real_clock::is_zero(last_mod_time_match)) {
    ...
    struct timespec last_mod_time =
        ceph::real_clock::to_timespec(last_mod_time_match);
    ...
    ldpp_dout(dpp, 10)
        << "RGWRados::Object::check_preconditions If-Match-Last-Modified-Time: "
        << last_mod_time_match << ", Last-Modified: " << ctime
        << ", with high_precision_time:" << high_precision_time << dendl;
    if (ctime != last_mod_time) {
      return -ERR_PRECONDITION_FAILED;
    }
  }
  ...
  if (if_match) {
    if (if_match == "*"sv) {
      ...
      if (current_state.get_attr(RGW_ATTR_ETAG, bl)) {
        ...
        ldpp_dout(dpp, 10) << "RGWRados::Object::check_preconditions if_match: "
                           << if_match_str << ", etag: " << etag << dendl;
        if (if_match_str.compare(
                0, etag.length(), etag.c_str(), etag.length()) != 0) {
          return -ERR_PRECONDITION_FAILED;
        }
      } else {
        ldpp_dout(dpp, 10) << "RGWRados::Object::check_preconditions if_match "
  ...
  if (if_nomatch) {
    if (if_nomatch == "*"sv) {
      ...
      if (current_state.get_attr(RGW_ATTR_ETAG, bl)) {
        ...
        string etag = string(bl.c_str(), bl.length());
        ldpp_dout(dpp, 10) << "RGWRados::Object::check_preconditions if_match: "
                           << if_nomatch_str << ", etag: " << etag << dendl;
        if (if_nomatch_str.compare(
                0, etag.length(), etag.c_str(), etag.length()) == 0) {
          return -ERR_PRECONDITION_FAILED;
        }
      }
...
RGWRados::Object::prepare_atomic_modification(
    ...
    const string* ptag,
    bool modify_tail,
    bool set_attr_id_tag,
    optional_yield y)
{
  if (!state->is_atomic) {
    ldpp_dout(dpp, 20)
        << "prepare_atomic_modification: state is not atomic. state="
        << (void*)state << dendl;

    if (reset_obj) {
      op.create(false);
      store->remove_rgw_head_obj(
          op); // we're not dropping reference here, actually removing object
    }
  ...
  ldpp_dout(dpp, 10) << "setting object write_tag=" << state->write_tag
                     << dendl;
    ...
    ceph::real_time set_mtime /* = zero() */)
{
  ...
  for (iter = attrs.begin(); iter != attrs.end(); ++iter) {
    ...
    if (name.compare(RGW_ATTR_DELETE_AT) == 0) {
      ...
      try {
        ...
        obj_expirer->hint_add(
            dpp, ts, bucket.tenant, bucket.name, bucket.bucket_id, obj_key);
      } catch (buffer::error& err) {
        ldpp_dout(dpp, 0) << "ERROR: failed to decode " RGW_ATTR_DELETE_AT
                          << " attr" << dendl;
      }
    }
  }
  ...
  if (r < 0) {
    ldpp_dout(dpp, 10) << "ERROR: cannot remove attr "
                       << replication_trace.c_str() << dendl;
  } else {
  ...
  if (state) {
    if (r >= 0) {
      ...
      ldpp_dout(dpp, 20) << "Setting obj category:" << category
                         << ", storage_class:" << storage_class << dendl;
      r = index_op.complete(
          dpp, poolid, epoch, state->size, state->accounted_size, mtime, etag,
          content_type, storage_class, owner, category, nullptr, y, nullptr,
          false, log_op);
    } else {
      int ret = index_op.cancel(dpp, nullptr, y, log_op);
      if (ret < 0) {
        ldpp_dout(dpp, 0)
            << "ERROR: complete_update_index_cancel() returned ret=" << ret
            << dendl;
      }
    }
  }
    ...
    RGWObjManifest** pmanifest)
{
  ...
  int r = store->get_obj_state(
      dpp, rctx, bucket_info, *head_obj, &sm, follow_olh, y);
  if (r < 0) {
    return r;
  }
  *pstate = &sm->state;
  ...
  gen.create_begin(
      store->ctx(), &part_manifest, manifest->get_head_placement_rule(),
      &manifest->get_tail_placement().placement_rule, head_obj->bucket,
      *head_obj);
...
int
RGWRados::Object::Read::prepare(optional_yield y, const DoutPrefixProvider* dpp)
{
  RGWRados* store = source->get_store();
  CephContext* cct = store->ctx();
  ...
  if (params.part_num) {
    ...
    r = get_part_obj_state(
        dpp, y, store, bucket_info, &source->get_ctx(), manifest,
        *params.part_num, &parts_count, part_prefetch, &astate, &manifest);
    if (r == -ERR_INVALID_PART && *params.part_num == 1) {
      ...
      ldpp_dout(dpp, 4) << "part #" << *params.part_num << " does not exist"
                        << dendl;
      return -ERR_INVALID_PART;
    } else {
...
int
RGWRados::Object::Read::range_to_ofs(
    uint64_t obj_size,
    int64_t& ofs,
    int64_t& end)
{
...
#define NUM_RESHARD_RETRIES 10
  for (int i = 0; i < NUM_RESHARD_RETRIES; ++i) {
    ...
    if (r == -ENOENT) {
      ldpp_dout(dpp, 10)
          << "ENOENT in guard_reshard(), likely bucket resharding, retrying"
          << dendl;
      invalidate_bs();
      continue;
    }
    ...
    ldpp_dout(dpp, 10) << "NOTICE: resharding operation on bucket index "
                          "detected, blocking. obj="
                       << obj_instance.key << dendl;

    r = store->block_while_resharding(
        bs, obj_instance, target->bucket_info, y, dpp);
    if (r == -ERR_BUSY_RESHARDING) {
      ldpp_dout(dpp, 10) << __func__
      ...
      ldpp_dout(dpp, 0) << __func__
                        ...
                        << obj_instance.key << ". ret=" << cpp_strerror(-r)
                        << dendl;
      return r;
    }
  ...
  if (target->bucket_info.layout.resharding ==
      rgw::BucketReshardState::InLogrecord) {
    ...
    bool log_op)
{
  ...
  int ret = get_bucket_shard(&bs, dpp, y);
  if (ret < 0) {
    ldpp_dout(dpp, 5) << "failed to get BucketShard object: ret=" << ret
                      << dendl;
    return ret;
  }
  ...
  ret = store->cls_obj_complete_add(
      *bs, obj, optag, poolid, epoch, ent, category, remove_objs, bilog_flags,
      zones_trace, add_log);
  if (add_log) {
    ret = add_datalog_entry(
        dpp, store->svc.datalog_rados, target->bucket_info, bs->shard_id, y);
  }
...
int
RGWRados::Bucket::UpdateIndex::complete_del(
    const DoutPrefixProvider* dpp,
    int64_t poolid,
    uint64_t epoch,
    ...
    bool log_op)
{
  ...
  ret = store->cls_obj_complete_del(
      *bs, optag, poolid, epoch, obj, removed_mtime, remove_objs, bilog_flags,
      zones_trace, add_log);
...
int
RGWRados::Bucket::UpdateIndex::cancel(
    const DoutPrefixProvider* dpp,
    list<rgw_obj_index_key>* remove_objs,
    optional_yield y,
    bool log_op)
{
  ...
  int ret = guard_reshard(
      dpp, obj, &bs,
      [&](BucketShard* bs) -> int {
        return store->cls_obj_complete_cancel(
            *bs, optag, obj, remove_objs, bilog_flags, zones_trace, add_log);
      },
      y);
    ...
    const DoutPrefixProvider* dpp)
{
  ...
  if (r < 0) {
    ldpp_dout(dpp, 0) << "ERROR: failed to get max_chunk_size() for pool "
                      << read_obj.pool << dendl;
    return r;
  }
  ...
  if (reading_from_head) {
    ...
    r = store->append_atomic_test(
        dpp, &source->get_ctx(), source->get_bucket_info(), state.obj, op,
        &astate, &manifest, y);
    if (r < 0)
      return r;
  ...
  ldpp_dout(dpp, 20) << "rados->read obj-ofs=" << ofs
                     << " read_ofs=" << read_ofs << " read_len=" << read_len
                     << dendl;
  ...
  if (state.cur_pool != read_obj.pool) {
    ...
    if (iter == state.io_ctxs.end()) {
      ...
      r = store->open_pool_ctx(
          dpp, read_obj.pool, *state.cur_ioctx, false, true);
      if (r < 0) {
        ldpp_dout(dpp, 20) << "ERROR: failed to open pool context for pool="
                           << read_obj.pool << " r=" << r << dendl;
        return r;
      }
    } else {
  ...
  r = rgw_rados_operate(
      dpp, *state.cur_ioctx, read_obj.oid, std::move(op), nullptr, y);
  ldpp_dout(dpp, 20) << "rados->read r=" << r << " bl.length=" << bl.length()
                     << dendl;
...
int
get_obj_data::flush(rgw::AioResultList&& results)
{
    ...
    if (rgwrados->get_use_datacache()) {
      ...
      if (bl.length() <= g_conf()->rgw_get_obj_max_req_size &&
        ...
        lsubdout(g_ceph_context, rgw_datacache, 10)
            ...
            << "(): bl.length <= rgw_get_obj_max_req_size (default 4MB) - "
               "write to datacache, bl.length="
            << bl.length() << dendl;
        rgwrados->d3n_data_cache->put(bl, bl.length(), oid);
      } else {
        lsubdout(g_ceph_context, rgw_datacache, 10)
            << "D3nDataCache: " << __func__
            << "(): not writing to datacache - bl.length > "
               "rgw_get_obj_max_req_size (default 4MB), bl.length="
            << bl.length()
            << " or d3n_bypass_cache_write=" << d3n_bypass_cache_write << dendl;
      }
    }
    ...
    void* arg)
{
  ...
  return d->rgwrados->get_obj_iterate_cb(
      dpp, read_obj, obj_ofs, read_ofs, len, is_head_obj, astate, arg);
}
    ...
    void* arg)
{
  ...
  if (is_head_obj) {
    ...
    if (astate && obj_ofs < astate->data.length()) {
      unsigned chunk_len =
          std::min((uint64_t)astate->data.length() - obj_ofs, (uint64_t)len);
  ...
  rgw_rados_ref obj;
  int r =
      rgw_get_rados_ref(dpp, d->rgwrados->get_rados_handle(), read_obj, &obj);
  if (r < 0) {
    ldpp_dout(dpp, 4) << "failed to open rados context for " << read_obj
                      << dendl;
    return r;
  }
  ...
  auto completed = d->aio->get(
      obj.obj, rgw::Aio::librados_op(obj.ioctx, std::move(op), d->yield), cost,
      id);
    ...
    optional_yield y)
{
  ...
  int r = store->iterate_obj(
      dpp, source->get_ctx(), source->get_bucket_info(), state.obj, ofs, end,
      chunk_size, _get_obj_iterate_cb, &data, y);
  if (r < 0) {
    ...
    optional_yield y)
{
  ...
  int r = get_obj_state(
      dpp, &obj_ctx, bucket_info, obj, &astate, &manifest, false, y);
  if (r < 0) {
    return r;
  }
  ...
  if (manifest) {
    ...
    for (; iter != obj_end && ofs <= end; ++iter) {
      ...
      while (ofs < next_stripe_ofs && ofs <= end) {
        ...
        uint64_t read_len =
            std::min(len, iter.get_stripe_size() - (ofs - stripe_ofs));
        read_ofs = iter.location_ofs() + (ofs - stripe_ofs);
        ...
        r =
            cb(dpp, read_obj, ofs, read_ofs, read_len, reading_from_head,
               astate, arg);
        if (r < 0) {
          return r;
        }
    ...
    optional_yield y)
{
  ...
  return rgw_rados_operate(
      dpp, ref.ioctx, ref.obj.oid, std::move(op), &outbl, y);
}

void
RGWRados::olh_cancel_modification(
    const DoutPrefixProvider* dpp,
    const RGWBucketInfo& bucket_info,
    RGWObjState& state,
    ...
    optional_yield y)
{
  ...
  if (r < 0) {
    ldpp_dout(dpp, 0) << __func__ << " target_obj=" << olh_obj
                      << " get_obj_head_ref() returned " << r << dendl;
    return;
  }
  ...
  if (r < 0) {
    if (r != -ENOENT && r != -ECANCELED) {
      ldpp_dout(dpp, 0) << __func__ << " target_obj=" << olh_obj
                        << " rmxattr rgw_rados_operate() returned " << r
                        << dendl;
    }
    return;
  }

  if (auto iter = state.attrset.find(RGW_ATTR_OLH_INFO);
      iter == state.attrset.end()) {
  ...
  if (r < 0 && (r != -ENOENT && r != -ECANCELED)) {
    ldpp_dout(dpp, 0) << __func__ << " target_obj=" << olh_obj
                      << " olh rm rgw_rados_operate() returned " << r << dendl;
  }
}

int
RGWRados::olh_init_modification_impl(
    const DoutPrefixProvider* dpp,
    const RGWBucketInfo& bucket_info,
    RGWObjState& state,
    const rgw_obj& olh_obj,
...
#define OLH_PENDING_TAG_LEN 32
  ...
  *op_tag = buf;
  ...
  string s =
      gen_rand_alphanumeric_lower(cct, OLH_PENDING_TAG_LEN - op_tag->size());
...
int
RGWRados::olh_init_modification(
    const DoutPrefixProvider* dpp,
    const RGWBucketInfo& bucket_info,
    RGWObjState& state,
    const rgw_obj& obj,
...
int
RGWRados::check_reshard_logrecord_status(
    RGWBucketInfo& bucket_info,
    optional_yield y,
    const DoutPrefixProvider* dpp)
{
  ...
  double reshard_progress_judge_interval =
      cct->_conf.get_val<uint64_t>("rgw_reshard_progress_judge_interval");
  // avoid getting reshard_lock simultaneously by mass differrent operation
  reshard_progress_judge_interval +=
      reshard_progress_judge_interval *
      cct->_conf.get_val<double>("rgw_reshard_progress_judge_ratio") * r;
  if (now - bucket_info.layout.judge_reshard_lock_time >=
      make_timespan(reshard_progress_judge_interval)) {

    map<string, bufferlist> bucket_attrs;
    int ret = get_bucket_info(
        &svc, bucket_info.bucket.tenant, bucket_info.bucket.name, bucket_info,
        nullptr, y, dpp, &bucket_attrs);
    if (ret < 0) {
      ldpp_dout(dpp, 0) << __func__
                        << " ERROR: failed to refresh bucket info : "
                        << cpp_strerror(-ret) << dendl;
      return ret;
    }
    if (bucket_info.layout.resharding == rgw::BucketReshardState::InLogrecord &&
        now - bucket_info.layout.judge_reshard_lock_time >=
            make_timespan(reshard_progress_judge_interval))
...
int
RGWRados::recover_reshard_logrecord(
    RGWBucketInfo& bucket_info,
    map<string, bufferlist>& bucket_attrs,
    optional_yield y,
    const DoutPrefixProvider* dpp)
{
  ...
  if (ret < 0) {
    ...
    ret = RGWBucketReshard::clear_resharding(
        this->driver, bucket_info, bucket_attrs, dpp, y);
    reshard_lock.unlock();
    if (ret < 0) {
      ldpp_dout(dpp, 0)
          << __func__ << " ERROR: failed to clear resharding flags for bucket "
          << bucket_info.bucket.bucket_id << dendl;
    } else {
      ldpp_dout(dpp, 5)
          << __func__
    ...
    const DoutPrefixProvider* dpp)
{
  ...
  // error code
  auto fetch_new_bucket_info = [this, bs, &obj_instance, &bucket_info,
                                &bucket_attrs, &y,
                                dpp](const std::string& log_tag) -> int {
    int ret = get_bucket_info(
        &svc, bs->bucket.tenant, bs->bucket.name, bucket_info, nullptr, y, dpp,
        &bucket_attrs);
    if (ret < 0) {
      ldpp_dout(dpp, 0)
          << __func__
    ...
    if (ret < 0) {
      ldpp_dout(dpp, 0)
          << __func__ << " ERROR: failed to refresh bucket shard generation after reshard at "
          << log_tag << ": " << cpp_strerror(-ret) << dendl;
      return ret;
    }

    const auto gen = bucket_info.layout.logs.empty()
                         ? -1
                         : bucket_info.layout.logs.back().gen;
    ldpp_dout(dpp, 20) << __func__
  ...
  for (int i = 1; i <= num_retries; i++) { // nb: 1-based for loop
    ...
    ldpp_dout(dpp, 20) << __func__ << " NOTICE: reshard still in progress; "
                       << (i < num_retries ? "retrying" : "too many retries")
                       << dendl;
    ...
    // and we're free to clear the flags.
    {
      ...
      if (ret == -ENOENT) {
        ...
        ldpp_dout(dpp, 20) << __func__
                           ...
                           << bucket_id << "; expected if resharding underway"
                           << dendl;
        ...
        if (ret < 0) {
          ldpp_dout(dpp, 0)
              << __func__ << " ERROR: failed to refresh bucket_obj for bucket "
        ...
        if (ret < 0) {
          reshard_lock.unlock();
          ldpp_dout(dpp, 0) << __func__
                            << " ERROR: failed to update bucket info before "
                               "clear resharding for bucket "
                            << bucket_id << dendl;
          continue; // try again
        }
    ...
    if (ret < 0) {
      ldpp_dout(dpp, 0) << __func__
                        << " ERROR: bucket is still resharding, please retry"
                        << dendl;
      return ret;
    }
  } // for loop
...
int
RGWRados::bucket_index_link_olh(
    const DoutPrefixProvider* dpp,
    RGWBucketInfo& bucket_info,
    RGWObjState& olh_state,
    const rgw_obj& obj_instance,
    ...
    bool log_data_change)
{
  ...
  r = guard_reshard(
      dpp, &bs, obj_instance, bucket_info,
      [&](BucketShard* bs) -> int {
        cls_rgw_obj_key key(
        ...
        cls_rgw_bucket_link_olh(
            op, key, olh_state.olh_tag, delete_marker, op_tag, meta, olh_epoch,
            unmod_since, high_precision_time, log_data_change, zones_trace);
        return rgw_rados_operate(dpp, ref.ioctx, ref.obj.oid, std::move(op), y);
      },
      y);
  if (r < 0) {
    ldpp_dout(dpp, 20)
        << "rgw_rados_operate() after cls_rgw_bucket_link_olh() returned r="
        << r << dendl;
    return r;
  }
...
void
RGWRados::bucket_index_guard_olh_op(
    const DoutPrefixProvider* dpp,
    RGWObjState& olh_state,
    ObjectOperation& op)
{
  ldpp_dout(dpp, 20)
      << __func__ << "(): olh_state.olh_tag="
...
int
RGWRados::bucket_index_unlink_instance(
    const DoutPrefixProvider* dpp,
    RGWBucketInfo& bucket_info,
    const rgw_obj& obj_instance,
    const string& op_tag,
    ...
    bool log_op)
{
  ...
  r = guard_reshard(
      ...
      [&](BucketShard* bs) -> int {
        ...
        op.assert_exists(); // bucket index shard must exist
        cls_rgw_guard_bucket_resharding(op, -ERR_BUSY_RESHARDING);
        cls_rgw_bucket_unlink_instance(
            op, key, op_tag, olh_tag, olh_epoch, log_op, bilog_flags,
            zones_trace);
        return rgw_rados_operate(dpp, ref.ioctx, ref.obj.oid, std::move(op), y);
      },
      y);
  if (r < 0) {
    ldpp_dout(dpp, 20) << "rgw_rados_operate() after "
                          "cls_rgw_bucket_link_instance() returned r="
                       << r << dendl;
    return r;
  }
...
int
RGWRados::bucket_index_read_olh_log(
    const DoutPrefixProvider* dpp,
    RGWBucketInfo& bucket_info,
    RGWObjState& state,
    const rgw_obj& obj_instance,
    ...
    optional_yield y)
{
  ...
  BucketShard bs(this);
  int ret = bs.init(
      obj_instance.bucket, obj_instance, nullptr /* no RGWBucketInfo */, dpp, y);
  if (ret < 0) {
  ...
  bufferlist outbl;
  r = rgw_rados_operate(
      dpp, shard_ref.ioctx, shard_ref.obj.oid, std::move(op), &outbl, y);
  if (r < 0) {
    return r;
  }
  if (op_ret < 0) {
    ldpp_dout(dpp, 20) << "cls_rgw_get_olh_log() returned op_ret=" << op_ret
                       << dendl;
    return op_ret;
  }
    ...
    optional_yield y)
{
  ...
  if (r < 0) {
    ldpp_dout(dpp, 0) << "repair_olh failed to read olh entry for " << obj
                      << dendl;
    return r;
  }
...
int
RGWRados::bucket_index_trim_olh_log(
    const DoutPrefixProvider* dpp,
    RGWBucketInfo& bucket_info,
    RGWObjState& state,
    const rgw_obj& obj_instance,
...
int
RGWRados::bucket_index_clear_olh(
    const DoutPrefixProvider* dpp,
    RGWBucketInfo& bucket_info,
    ...
    optional_yield y)
{
  ...
  if (ret < 0) {
    ldpp_dout(dpp, 5)
        << "rgw_rados_operate() after cls_rgw_clear_olh() returned ret=" << ret
        << dendl;
    return ret;
  }
  ...
  *plast_ver = last_ver;
  ...
  for (iter = log.begin(); iter != log.end(); ++iter) {
    ...
    for (; viter != iter->second.end(); ++viter) {
      ...
      ldpp_dout(dpp, 20) << "olh_log_entry: epoch=" << iter->first
                         << " op=" << (int)entry.op << " key=" << entry.key.name
                         << "[" << entry.key.instance << "] "
                         << (entry.delete_marker ? "(delete)" : "") << dendl;

      if (link_epoch == entry.epoch)
        ldpp_dout(dpp, 1) << "apply_olh_log epoch collision detected for "
                          << entry.key << "; incoming op: " << entry.op << "("
                          << entry.op_tag << ")" << dendl;
      ...
      case CLS_RGW_OLH_OP_LINK_OLH:
        ...
        if (need_to_remove || link_epoch < entry.epoch ||
            key.instance.empty() || key.instance > entry.key.instance) {
          ldpp_dout(dpp, 20)
              << "apply_olh_log applying key=" << entry.key
              << " epoch=" << iter->first
              << " delete_marker=" << entry.delete_marker
              << " over current=" << key << " epoch=" << link_epoch
          ...
          ldpp_dout(dpp, 20)
              << "apply_olh skipping key=" << entry.key
              << " epoch=" << iter->first
              << " delete_marker=" << entry.delete_marker
              << " before current=" << key << " epoch=" << link_epoch
              << " delete_marker=" << delete_marker << dendl;
        }
        break;
      ...
      case CLS_RGW_OLH_OP_STALE:
        break;
      default:
        ldpp_dout(dpp, 0) << "ERROR: apply_olh_log: invalid op: "
                          << (int)entry.op << dendl;
        return -EIO;
      }
      string attr_name = RGW_ATTR_OLH_PENDING_PREFIX;
  ...
  for (set<cls_rgw_obj_key>::iterator liter = remove_instances.begin();
    ...
    int ret = delete_obj(
        dpp, obj_ctx, bucket_info, obj_instance, 0, y, null_verid,
        RGW_BILOG_FLAG_VERSIONED_OP, ceph::real_time(), zones_trace, log_op,
        force, true /* skip_olh_obj_update */);
    if (ret < 0 && ret != -ENOENT) {
      ldpp_dout(dpp, 0) << "ERROR: delete_obj() returned " << ret
                        << " obj_instance=" << obj_instance << dendl;
      return ret;
    }
  }
  ...
  if (r < 0) {
    if (r != -ECANCELED) {
      ldpp_dout(dpp, 0) << "ERROR: " << __func__
                        << ": could not apply olh update to oid \""
                        << ref.obj.oid << "\", r=" << r << dendl;
    }
    return r;
  }
    ...
    optional_yield y)
{
  ...
  int r =
      get_obj_state(dpp, &obj_ctx, bucket_info, obj, &s, &manifest, false, y);
  if (r < 0) {
    return r;
  }
  ...
  if (!rm_pending_entries.empty()) {
    r = remove_olh_pending_entries(
        dpp, bucket_info, *s, obj, rm_pending_entries, y);
    if (r < 0) {
      ldpp_dout(dpp, 0) << "ERROR: rm_pending_entries returned ret=" << r
                        << dendl;
      return r;
    }
  }
  ...
  cls_obj_check_prefix_exist(
      rm_op, RGW_ATTR_OLH_PENDING_PREFIX,
  ...
  if (r < 0) {
    ldpp_dout(dpp, 0) << "ERROR: could not clear bucket index olh entries r="
                      << r << dendl;
    return r;
  }
  return 0;
    ...
    const bool force)
{
  ...
  do {
    int ret = bucket_index_read_olh_log(
        dpp, bucket_info, *state, obj, ver_marker, &log, &is_truncated, y);
    if (ret < 0) {
      return ret;
    }
    ret = apply_olh_log(
        dpp, obj_ctx, *state, bucket_info, obj, state->olh_tag, log,
        &ver_marker, y, null_verid, zones_trace, log_op, force);
    if (ret < 0) {
      return ret;
    }
  } while (is_truncated);
...
#define MAX_ECANCELED_RETRY 100
  for (i = 0; i < MAX_ECANCELED_RETRY; i++) {
    ...
    ret = get_obj_state(
        dpp, &obj_ctx, bucket_info, olh_obj, &state, &manifest, false,
        y); /* don't follow olh */
    if (ret < 0) {
      return ret;
    }
    ...
    if (cct->_conf->rgw_debug_inject_set_olh_err) {
      ...
      ret = bucket_index_link_olh(
          dpp, bucket_info, *state, target_obj, delete_marker, op_tag, meta,
          olh_epoch, unmod_since, high_precision_time, y, zones_trace,
          log_data_change);
    }
    if (ret < 0) {
      ...
      if (!skip_olh_obj_update) {
        int r = update_olh(
            dpp, obj_ctx, state, bucket_info, olh_obj, y, zones_trace,
            log_data_change);
        if (r < 0 && r != -ECANCELED) {
          ldpp_dout(dpp, 20) << "update_olh() target_obj=" << olh_obj
                             << " returned " << r << dendl;
        }
      }
      return ret;
  ...
  if (i == MAX_ECANCELED_RETRY) {
    ldpp_dout(dpp, 0) << "ERROR: exceeded max ECANCELED retries, aborting (EIO)"
                      << dendl;
    return -EIO;
  }
  ...
  ret = update_olh(
      ...
      log_data_change);
  if (ret ==
      -ECANCELED) { /* already did what we needed, no need to retry, raced with another user */
    ret = 0;
  }
  if (ret < 0) {
    ldpp_dout(dpp, 20) << "update_olh() target_obj=" << target_obj
                       << " returned " << ret << dendl;
    return ret;
  }
    ...
    const bool skip_olh_obj_update)
{
  ...
  for (i = 0; i < MAX_ECANCELED_RETRY; i++) {
    ...
    if (cct->_conf->rgw_debug_inject_latency_bi_unlink) {
      ...
      std::this_thread::sleep_for(
          cct->_conf->rgw_debug_inject_latency_bi_unlink *
          std::chrono::seconds{1});
    }

    if (null_verid) {
      bilog_flags = bilog_flags | RGW_BILOG_FLAG_VERSIONED_OP |
                    RGW_BILOG_NULL_VERSION;
    } else {
      bilog_flags = bilog_flags | RGW_BILOG_FLAG_VERSIONED_OP;
    }

    ret = bucket_index_unlink_instance(
        dpp, bucket_info, target_obj, op_tag, olh_tag, olh_epoch, y,
  ...
  ret = update_olh(
      dpp, obj_ctx, state, bucket_info, olh_obj, y, zones_trace, null_verid,
      log_op, force);
  if (ret ==
      -ECANCELED) { /* already did what we needed, no need to retry, raced with another user */
    return 0;
...
void
RGWRados::gen_rand_obj_instance_name(rgw_obj_key* target_key)
{
...
void
RGWRados::gen_rand_obj_instance_name(rgw_obj* target_obj)
{
...
void
RGWRados::check_pending_olh_entries(
    const DoutPrefixProvider* dpp,
    map<string, bufferlist>& pending_entries,
    map<string, bufferlist>* rm_pending_entries)
{
  ...
  while (iter != pending_entries.end()) {
    ...
    try {
      ...
      ldpp_dout(dpp, 0) << "ERROR: failed to decode pending entry "
                        << iter->first << dendl;
      ++iter;
      continue;
    }
    ...
    ++iter;
    if (now - pending_info.time >=
        make_timespan(cct->_conf->rgw_olh_pending_timeout_sec)) {
    ...
    optional_yield y)
{
  map<string, bufferlist> pending_entries;
  rgw_filter_attrset(
      state->attrset, RGW_ATTR_OLH_PENDING_PREFIX, &pending_entries);

  map<string, bufferlist> rm_pending_entries;
  check_pending_olh_entries(dpp, pending_entries, &rm_pending_entries);

  if (!rm_pending_entries.empty()) {
    int ret = remove_olh_pending_entries(
        dpp, bucket_info, *state, olh_obj, rm_pending_entries, y);
    if (ret < 0) {
      ldpp_dout(dpp, 20) << "ERROR: rm_pending_entries returned ret=" << ret
                         << dendl;
      return ret;
    }
  }
  if (!pending_entries.empty()) {
    ldpp_dout(dpp, 20)
        << __func__
        << "(): found pending entries, need to update_olh() on bucket="
        << olh_obj.bucket << dendl;
    ...
    optional_yield y)
{
  ...
  bufferlist outbl;
  r = rgw_rados_operate(
      dpp, ref.ioctx, ref.obj.oid, std::move(op), &outbl, y, 0, nullptr, epoch);
  if (r < 0)
    return r;
    ...
    bool* syncstopped)
{
  ...
  int r = svc.bi_rados->cls_bucket_head(
      dpp, bucket_info, idx_layout, shard_id, &headers, &bucket_instance_ids, y);
  if (r < 0) {
    return r;
  }
...
public:
  ...
  void
  handle_response(int r, const rgw_bucket_dir_header& header) override
  {
    std::lock_guard l{lock};
    if (should_cb) {
      if (r >= 0) {
...
int
RGWRados::get_bucket_stats_async(
    const DoutPrefixProvider* dpp,
    RGWBucketInfo& bucket_info,
    const rgw::bucket_index_layout_generation& idx_layout,
    int shard_id,
...
{
  int num_aio = 0;
  boost::intrusive_ptr headercb = new RGWGetBucketStatsContext(
      std::move(cb),
      bucket_info.layout.current_index.layout.normal.num_shards ?: 1);
  int r = cls_bucket_head_async(
      dpp, bucket_info, idx_layout, shard_id, headercb, &num_aio);
  if (r < 0) {
    if (num_aio) {
      headercb->unset_cb();
    }
    ...
    const DoutPrefixProvider* dpp)
{
  return ctl.bucket->read_bucket_instance_info(
      bucket, &info, y, dpp,
      RGWBucketCtl::BucketInstance::GetParams().set_mtime(pmtime).set_attrs(
          pattrs));
}
...
int
RGWRados::try_refresh_bucket_info(
    RGWBucketInfo& info,
    ...
    bufferlist& bl)
{
  ...
  librados::AioCompletion* completion =
      rad->aio_create_completion(nullptr, nullptr);
...
string
RGWRados::pool_iterate_get_cursor(RGWPoolIterCtx& ctx)
{
...
int
RGWRados::list_raw_objects_init(
    const DoutPrefixProvider* dpp,
    const rgw_pool& pool,
    const string& marker,
    RGWListRawObjsCtx* ctx)
{
  if (!ctx->initialized) {
    ...
    if (r < 0) {
      ldpp_dout(dpp, 10)
          << "failed to list objects pool_iterate_begin() returned r=" << r
          << dendl;
      return r;
    }
...
int
RGWRados::list_raw_objects_next(
    const DoutPrefixProvider* dpp,
...
string
RGWRados::list_raw_objs_get_cursor(RGWListRawObjsCtx& ctx)
{
  return pool_iterate_get_cursor(ctx.iter_ctx);
}

int
RGWRados::bi_get_instance(
    const DoutPrefixProvider* dpp,
    const RGWBucketInfo& bucket_info,
    const rgw_obj& obj,
    rgw_bucket_dir_entry* dirent,
...
int
RGWRados::bi_put(BucketShard& bs, rgw_cls_bi_entry& entry, optional_yield y)
{
  auto& ref = bs.bucket_obj;
    ...
    optional_yield y)
{
  ...
  auto& ref = bs.bucket_obj;
  ret = cls_rgw_bi_list(
      ref.ioctx, ref.obj.oid, obj_name_filter, marker, max, entries,
      is_truncated, reshardlog);
  if (ret == -ENOENT) {
    *is_truncated = false;
  }
  if (ret < 0)
    ...
    optional_yield y)
{
  BucketShard bs(this);
  int ret =
      bs.init(dpp, bucket_info, bucket_info.layout.current_index, shard_id, y);
  if (ret < 0) {
    ldpp_dout(dpp, 5) << "bs.init() returned ret=" << ret << dendl;
    return ret;
  }
  ...
  return bi_list(
      bs, obj_name_filter, marker, max, entries, is_truncated, reshardlog, y);
}

int
RGWRados::bi_remove(const DoutPrefixProvider* dpp, BucketShard& bs)
{
  auto& ref = bs.bucket_obj;
  ...
  if (ret < 0) {
    ldpp_dout(dpp, 5) << "bs.index_ctx.remove(" << bs.bucket_obj
                      << ") returned ret=" << ret << dendl;
    return ret;
  }
...
{
  return gc->list(
      index, marker, max, expired_only, result, truncated, processing_queue);
}

int
RGWRados::process_gc(bool expired_only, optional_yield y)
{
  return gc->process(expired_only, y);
}

int
RGWRados::process_lc(const std::unique_ptr<rgw::sal::Bucket>& optional_bucket)
{
  RGWLC lc;
  lc.initialize(cct, this->driver);
...
bool
RGWRados::process_expired_objects(const DoutPrefixProvider* dpp, optional_yield y)
{
  return obj_expirer->inspect_all_shards(dpp, utime_t(), ceph_clock_now(), y);
}

int
RGWRados::cls_obj_prepare_op(
    const DoutPrefixProvider* dpp,
    BucketShard& bs,
    RGWModifyOp op,
    string& tag,
    ...
    optional_yield y)
{
  ...
  ldout_bitx(bitx, dpp, 10)
      << "ENTERING " << __func__ << ": bucket-shard=" << bs << " obj=" << obj
      << " tag=" << tag << " op=" << op << dendl_bitx;
  ldout_bitx(bitx, dpp, 25)
      << "BACKTRACE: " << __func__ << ": " << ClibBackTrace(0) << dendl_bitx;
    ...
    bool log_op)
{
  ...
  ldout_bitx_c(bitx, cct, 10)
      << "ENTERING " << __func__ << ": bucket-shard=" << bs << " obj=" << obj
      << " tag=" << tag << " op=" << op << ", remove_objs="
      << (remove_objs ? *remove_objs : std::list<rgw_obj_index_key>())
      << ", log_op=" << log_op << dendl_bitx;
  ldout_bitx_c(bitx, cct, 25)
      << "BACKTRACE: " << __func__ << ": " << ClibBackTrace(0) << dendl_bitx;
  ...
  cls_rgw_guard_bucket_resharding(o, -ERR_BUSY_RESHARDING);
  cls_rgw_bucket_complete_op(
      o, op, tag, ver, key, dir_meta, remove_objs, log_op, bilog_flags,
      &zones_trace, obj.key.get_loc());
  complete_op_data* arg;
  index_completion_manager->create_completion(
      obj, op, tag, ver, key, dir_meta, remove_objs, log_op, bilog_flags,
      &zones_trace, &arg);
    ...
    bool log_op)
{
  return cls_obj_complete_op(
      bs, obj, CLS_RGW_OP_ADD, tag, pool, epoch, ent, category, remove_objs,
      bilog_flags, zones_trace, log_op);
}
    ...
    bool log_op)
{
  ...
  obj.key.get_index_key(&ent.key);
  return cls_obj_complete_op(
      bs, obj, CLS_RGW_OP_DEL, tag, pool, epoch, ent, RGWObjCategory::None,
      remove_objs, bilog_flags, zones_trace, log_op);
}
    ...
    uint32_t num_shards)
{
  ...
  uint32_t calc_read =
      1 + static_cast<uint32_t>(
              (num_entries / num_shards) +
              sqrt((2 * num_entries) * log(num_shards) / num_shards));
    ...
    RGWBucketListNameFilter force_check_filter)
{
  ...
  ldout_bitx(bitx, dpp, 10)
      ...
      << " start_after=\"" << start_after.to_string() << "\", prefix=\""
      << prefix << ", delimiter=\"" << delimiter << "\", shard_id=" << shard_id
      << "\", num_entries=" << num_entries << ", shard_id=" << shard_id
  ...
  int r = svc.bi_rados->open_bucket_index(
      dpp, bucket_info, shard_id, idx_layout, &index_pool, &shard_oids, nullptr);
  if (r < 0) {
    ldpp_dout(dpp, 0) << __func__ << ": open_bucket_index for "
                      << bucket_info.bucket << " failed" << dendl;
    return r;
  }
  ...
  ldpp_dout(dpp, 10) << __func__ << ": request from each of " << shard_count
                     << " shard(s) for " << num_entries_per_shard
                     << " entries to get " << num_entries << " total entries"
                     << dendl;
  ...
  r = svc.bi_rados->list_objects(
      dpp, y, ioctx, shard_oids, start_after_key, prefix, delimiter,
      num_entries_per_shard, list_versions, shard_list_results);
  if (r < 0) {
    ldpp_dout(dpp, 0) << __func__ << ": CLSRGWIssueBucketList for "
                      << bucket_info.bucket << " failed" << dendl;
    return r;
  }
  ...
  while (count < num_entries && !candidates.empty()) {
    ...
    ldpp_dout(dpp, 20) << __func__ << ": currently processing " << dirent.key
                       << " from shard " << tracker.shard_idx << dendl;

    const bool force_check = force_check_filter &&
    ...
    if ((!dirent.exists && !dirent.is_delete_marker() &&
      ...
      r = check_disk_state(
          dpp, bucket_info, index_ver, dirent, dirent,
          updates[tracker.oid_name], y);
      if (r < 0 && r != -ENOENT) {
        ldpp_dout(dpp, 0) << __func__ << ": check_disk_state for \""
                          << dirent.key << "\" failed with r=" << r << dendl;
        return r;
      }
    } else {
      r = 0;
    ...
    if (r >= 0) { // i.e., if r != -ENOENT
      ldpp_dout(dpp, 10) << __func__ << ": got " << dirent_key << dendl;
      ...
      ldpp_dout(dpp, 10) << __func__ << ": skipping " << dirent.key.name << "["
    ...
    if (need_to_stop) {
      ...
      ldpp_dout(dpp, 10) << __func__
                         ...
                         << ", dirent=\"" << dirent_key
                         << "\", because its shard is truncated and exhausted"
                         << dendl;
      break;
    }
  ...
  for (auto& miter : updates) {
    if (miter.second.length()) {
      ...
      ldout_bitx(bitx, dpp, 10)
          << "INFO: " << __func__ << ": doing dir_suggest on " << miter.first
          << dendl_bitx;
  ...
  *is_truncated = false;
  ...
  ldpp_dout(dpp, 20) << __func__ << ": returning, count=" << count
                     << ", is_truncated=" << *is_truncated << dendl;

  if (*is_truncated && count < num_entries) {
    ldpp_dout(dpp, 10) << __func__ << ": requested " << num_entries
  ...
  if (last_entry_visited != nullptr && last_entry) {
    *last_entry = last_entry_visited->key;
    ldpp_dout(dpp, 20) << __func__ << ": returning, last_entry=" << *last_entry
                       << dendl;
  } else {
    ldpp_dout(dpp, 20) << __func__ << ": returning, last_entry NOT SET"
                       << dendl;
  }
...
// period on.
static int
parse_index_hash_source(
    const std::string& oid_wo_ns,
    ...
    RGWBucketListNameFilter force_check_filter)
{
  ...
  *is_truncated = false;
  ...
  int r = svc.bi_rados->open_bucket_index(
      dpp, bucket_info, shard_id, idx_layout, &index_pool, &oids, nullptr);
  if (r < 0) {
    return r;
  }
  ...
  if (shard_id >= 0) {
    ...
    if (!parsed) {
      ldpp_dout(dpp, 0) << "ERROR: " << __func__
                        << " received an invalid start marker: \""
                        << start_after << "\"" << dendl;
      return -EINVAL;
      ...
      if (obj_key.ns == RGW_OBJ_NS_MULTIPART) {
        ...
        if (r < 0) {
          ldpp_dout(dpp, 0) << "ERROR: " << __func__
                            << " parse_index_hash_source unable to parse \""
                            << obj_key.name << "\", r=" << r << dendl;
          return r;
        }
        current_shard =
            svc.bi_rados->bucket_shard_index(index_hash_source, num_shards);
      } else {
        current_shard =
            svc.bi_rados->bucket_shard_index(obj_key.name, num_shards);
      }
    }
  }
  ...
  while (count <= num_entries &&
    ...
    cls_rgw_bucket_list_op(
        op, marker, prefix, empty_delimiter, num_entries, list_versions,
        &result);
    r = rgw_rados_operate(
        dpp, ioctx, oid, std::move(op), nullptr, y, 0, nullptr,
        &index_ver.epoch);
    if (r == RGWBIAdvanceAndRetryError) {
      ...
      if (prev_marker == marker) {
        ldpp_dout(dpp, 0)
            ...
            << " oid=" << oid << " prev_marker=" << prev_marker
            << " marker=" << marker << dendl;
        return -EIO;
      }

      ldpp_dout(dpp, 10) << __func__ << ": RGWBIAdvanceAndRetryError"
                         << " on oid=" << oid << "; prev_marker=" << prev_marker
                         << " -> marker=" << marker
                         << "; advancing marker and retrying" << dendl;
      continue;
    }
    if (r < 0) {
      ldpp_dout(dpp, 0) << "ERROR: " << __func__
                        << ": error in rgw_rados_operate (bucket list op), r="
                        << r << dendl;
      return r;
    }
	 ...
	 * and if the tags are old we need to do cleanup as well. */
        ldout_bitx(bitx, dpp, 20)
            ...
            << " entry=" << dirent.key << dendl_bitx;
        r = check_disk_state(
            dpp, bucket_info, index_ver, dirent, dirent, updates[oid], y);
        if (r < 0 && r != -ENOENT) {
          ldpp_dout(dpp, 0) << "ERROR: " << __func__
      ...
      if (r >= 0) { // i.e., if r != -ENOENT
        ...
        if (count < num_entries) {
          ...
          *is_truncated = true;
          ldpp_dout(dpp, 10)
              << "INFO: " << __func__ << ": reached max entries ("
              << num_entries << ") to return at \"" << dirent.key << "\""
              << dendl;
          goto check_updates;
        }
      } else { // r == -ENOENT
...
int
RGWRados::cls_obj_usage_log_read(
    const DoutPrefixProvider* dpp,
    const string& oid,
    const string& user,
    const string& bucket,
    ...
    bool* is_truncated)
{
  ...
  *is_truncated = false;
  ...
  r = cls_rgw_usage_log_read(
      ref.ioctx, ref.obj.oid, user, bucket, start_epoch, end_epoch, max_entries,
...
int
RGWRados::cls_obj_usage_log_trim(
    const DoutPrefixProvider* dpp,
    const string& oid,
    const string& user,
    const string& bucket,
    ...
    optional_yield y)
{
  ...
  r = cls_rgw_usage_log_trim_repeat(
      dpp, ref, user, bucket, start_epoch, end_epoch, y);
  return r;
}

int
RGWRados::cls_obj_usage_log_clear(
    const DoutPrefixProvider* dpp,
    string& oid,
    optional_yield y)
{
...
int
RGWRados::remove_objs_from_index(
    const DoutPrefixProvider* dpp,
    RGWBucketInfo& bucket_info,
    const std::list<rgw_obj_index_key>& entry_key_list)
{
  ...
  if (r < 0) {
    ldout_bitx(bitx, dpp, 0)
        << "ERROR: " << __func__ << " open_bucket_index returned " << r
        << dendl_bitx;
    return r;
  }
  ...
  for (const auto& removals : sharded_removals) {
    ...
    ldout_bitx(bitx, dpp, 10)
        ...
        << ": removal from bucket index, bucket=" << bucket_info.bucket
        << ", shard=" << shard << ", oid=" << oid
        << ", num_keys=" << removals.second.size() << dendl_bitx;

    r = index_pool.omap_rm_keys(oid, removals.second);
    if (r < 0) {
      ldout_bitx(bitx, dpp, 0)
          << "ERROR: " << __func__ << ": omap_rm_keys returned ret=" << r
          << dendl_bitx;
      return r;
    }
  }

  ldout_bitx(bitx, dpp, 5) << "EXITING " << __func__ << " and returning " << r
                           << dendl_bitx;
    ...
    optional_yield y)
{
  ...
  uint8_t suggest_flag =
      (svc.zone->need_to_log_data() ? CEPH_RGW_DIR_SUGGEST_LOG_OP : 0);
  ...
  if (loc != list_state.locator) {
    ldpp_dout(dpp, 0) << "WARNING: generated locator (" << loc
                      << ") is different from listed locator ("
  ...
  RGWObjectCtx octx(this->driver);
  int r =
      get_obj_state(dpp, &octx, bucket_info, obj, &astate, &manifest, false, y);
  if (r < 0)
    return r;
  ...
  if (!list_state.is_delete_marker() && !astate->exists) {
    ldout_bitx(bitx, dpp, 10)
        << "INFO: " << __func__ << ": disk state exists" << dendl_bitx;
    ...
    ldout_bitx(bitx, dpp, 10)
        << "INFO: " << __func__ << ": encoding remove of " << list_state.key
        << " on suggested_updates" << dendl_bitx;
    cls_rgw_encode_suggestion(
        CEPH_RGW_REMOVE | suggest_flag, list_state, suggested_updates);
    return -ENOENT;
  }
  ...
  if (iter != astate->attrset.end()) {
    ...
    if (r < 0) {
      ldpp_dout(dpp, 0) << "WARNING: could not decode policy for object: "
                        << obj << dendl;
    }
  }
  ...
  if (manifest) {
    ...
    for (miter = manifest->obj_begin(dpp); miter != manifest->obj_end(dpp);
      ...
      rgw_obj loc;
      RGWSI_Tier_RADOS::raw_obj_to_obj(
          manifest->get_obj().bucket, raw_loc, &loc);

      if (loc.key.ns == RGW_OBJ_NS_MULTIPART) {
        ldout_bitx(bitx, dpp, 10)
            << "INFO: " << __func__
            << " removing manifest part from index loc=" << loc << dendl_bitx;
        r = delete_obj_index(loc, astate->mtime, dpp, y);
        if (r < 0) {
          ldout_bitx(bitx, dpp, 0)
              << "WARNING: " << __func__
              << ": delete_obj_index returned r=" << r << dendl_bitx;
        }
      }
    }
  }
  ...
  ldout_bitx(bitx, dpp, 10)
      << "INFO: " << __func__ << ": encoding update of " << list_state.key
      << " on suggested_updates" << dendl_bitx;
  cls_rgw_encode_suggestion(
...
int
RGWRados::cls_bucket_head_async(
    const DoutPrefixProvider* dpp,
    const RGWBucketInfo& bucket_info,
    const rgw::bucket_index_layout_generation& idx_layout,
    int shard_id,
    ...
    int* num_aio)
{
  ...
  map<int, string> bucket_objs;
  int r = svc.bi_rados->open_bucket_index(
      dpp, bucket_info, shard_id, idx_layout, &index_pool, &bucket_objs,
      nullptr);
  if (r < 0)
    return r;
...
void
RGWRados::calculate_preferred_shards(
    const DoutPrefixProvider* dpp,
    bool is_versioned,
    ...
    uint32_t* suggested_num_shards)
{
  ...
  RGWBucketReshard::calculate_preferred_shards(
      dpp, max_dynamic_shards, min_layout_shards, max_objs_per_shard,
      is_multisite, num_objs, num_source_shards, need_resharding,
    ...
    optional_yield y)
{
  ...
  calculate_preferred_shards(
      dpp, is_versioned, num_objs, num_source_shards, min_layout_shards,
  ...
  ldpp_dout(dpp, 1) << "RGWRados::" << __func__ << " bucket "
                    ...
                    << num_source_shards << "; new num shards "
...
int
RGWRados::add_bucket_to_reshard(
    const DoutPrefixProvider* dpp,
    const RGWBucketInfo& bucket_info,
    ...
    optional_yield y)
{
  ...
  if ((!may_reduce && new_num_shards < num_source_shards) ||
    ...
    ldpp_dout(dpp, 10) << "WARNING: " << __func__
                       ...
                       << bucket_info.bucket.name
                       << ", shard count=" << num_source_shards
                       << ", new shard count=" << new_num_shards
                       << ", rgw_dynamic_resharding_may_reduce=" << may_reduce
                       << dendl;
    return 0;
  }
    ...
    bool check_size_only)
{
  ...
  if (check_size_only)
    return quota_handler->check_quota(
        dpp, bucket_owner, bucket, quota, 0, obj_size, y);

  return quota_handler->check_quota(
      dpp, bucket_owner, bucket, quota, 1, obj_size, y);
}

int
RGWRados::get_target_shard_id(
    const rgw::bucket_index_normal_layout& layout,
    ...
    int* shard_id)
{
  ...
  case rgw::BucketHashType::Mod:
    if (!layout.num_shards) {
      ...
      uint32_t sid =
          svc.bi_rados->bucket_shard_index(obj_key, layout.num_shards);
      if (shard_id) {
        *shard_id = (int)sid;
      }
    }
    ...
    list<librados::AioCompletion*>& handles)
{
  ...
  if (ret < 0) {
    ldpp_dout(dpp, -1) << "ERROR: failed to get obj ref with ret=" << ret
                       << dendl;
    return ret;
  }
    ...
    optional_yield y)
{
  ...
  if (keep_index_consistent) {
    ...
    if (ret < 0) {
      ldpp_dout(dpp, -1) << "ERROR: failed to prepare index op with ret=" << ret
                         << dendl;
      return ret;
    }
  }
  ...
  if (keep_index_consistent) {
    ...
    if (ret < 0) {
      ldpp_dout(dpp, -1) << "ERROR: failed to delete obj index with ret=" << ret
                         << dendl;
      return ret;
    }
  }
...
list<objexp_hint_entry>
objexp_hint_entry::generate_test_instances()
{
...
void
objexp_hint_entry::dump(Formatter* f) const
{
...
list<RGWOLHInfo>
RGWOLHInfo::generate_test_instances()
{
  list<RGWOLHInfo> o;
  RGWOLHInfo olh;
  olh.removed = false;
...
void
RGWOLHInfo::dump(Formatter* f) const
{
...
void
RGWOLHPendingInfo::dump(Formatter* f) const
{
  utime_t ut(time);
  encode_json("time", ut, f);
}

list<RGWOLHPendingInfo>
RGWOLHPendingInfo::generate_test_instances()
{
