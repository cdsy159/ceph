...
struct RGWUsageBatch {
  ...
  void
  insert(ceph::real_time& t, rgw_usage_log_entry& entry, bool* account)
  {
...
static_assert(
    !std::is_copy_constructible<RGWObjectCtx>::value,
    "RGWObjectCtx must not be copy-constructed");
static_assert(
    !std::is_copy_assignable<RGWObjectCtx>::value,
    "RGWObjectCtx must not be copy-assigned");
static_assert(
    !std::is_move_constructible<RGWObjectCtx>::value,
    "RGWObjectCtx must not be move-constructed");
static_assert(
    !std::is_move_assignable<RGWObjectCtx>::value,
...
class RGWRados {
  ...
  void cls_obj_check_prefix_exist(
      librados::ObjectOperation& op,
      const std::string& prefix,
      bool fail_if_exist);
  void cls_obj_check_mtime(
      librados::ObjectOperation& op,
...
protected:
  ...
  using RGWChainedCacheImpl_bucket_topics_entry =
      RGWChainedCacheImpl<pubsub_bucket_topics_entry>;
...
public:
  ...
  RGWRados&
  set_run_restore_thread(bool _use_restore_thread)
  {
  ...
  RGWRados&
  set_run_notification_thread(bool _run_notification_thread)
  {
  ...
  RGWRados&
  set_run_bucket_logging_thread(bool _run_bucket_logging_thread)
  {
  ...
  librados::IoCtx*
  get_restore_pool_ctx()
  {
    return &restore_pool_ctx;
  }

  neorados::IOContext*
  get_restore_pool_neo_ctx()
  {
  ...
  void
  set_store(rgw::sal::RadosStore* _driver)
  {
    driver = _driver;
  }
  ...
  int list_raw_objects_init(
      const DoutPrefixProvider* dpp,
      const rgw_pool& pool,
      const std::string& marker,
      RGWListRawObjsCtx* ctx);
  int list_raw_objects_next(
      const DoutPrefixProvider* dpp,
  ...
  int register_to_service_map(
      const DoutPrefixProvider* dpp,
      const std::string& daemon_type,
      const std::map<std::string, std::string>& meta);
  int update_service_map(
      const DoutPrefixProvider* dpp,
  ...
  struct BucketShard {
    ...
    friend std::ostream&
    operator<<(std::ostream& out, const BucketShard& bs)
    {
      out << "BucketShard:{ bucket=" << bs.bucket
          << ", shard_id=" << bs.shard_id << ", bucket_obj=" << bs.bucket_obj
          << "}";
      return out;
    }
  };
  ...
  protected:
    ...
    int check_preconditions(
        ...
        RGWObjState& current_state,
        optional_yield y);
    int prepare_atomic_modification(
        const DoutPrefixProvider* dpp,
  ...
  public:
        ...
        optional_yield y)
    {
      if (!bs_initialized) {
        int r = bs.init(
            bucket_info.bucket, obj, nullptr /* no RGWBucketInfo */, dpp, y);
        if (r < 0) {
          return r;
        }
        bs_initialized = true;
    ...
    void
    set_versioning_disabled(bool status)
    {
    ...
    void
    set_meta_placement_rule(const rgw_placement_rule* p)
    {
    ...
    {
      return pmeta_placement_rule ? *pmeta_placement_rule
    ...
    struct Write {
      ...
      int write_meta(
          ...
          bool log_op = true);
      int
      write_data(const char* data, uint64_t ofs, uint64_t len, bool exclusive);
  ...
  public:
    ...
    int update_bucket_id(
        const std::string& new_bucket_id,
        const DoutPrefixProvider* dpp,
        optional_yield y);
    ...
    class UpdateIndex {
      ...
      int
      init_bs(const DoutPrefixProvider* dpp, optional_yield y)
      {
        int r = bs.init(target->get_bucket(), obj, &target->bucket_info, dpp, y);
        if (r < 0) {
          return r;
        }
        bs_initialized = true;
    ...
    public:
      ...
      {
        blind = target->get_bucket_info().layout.current_index.layout.type ==
      ...
      void
      set_bilog_flags(uint16_t flags)
      {
        bilog_flags = flags;
      }
      ...
      void
      set_zones_trace(rgw_zone_set* _zones_trace)
      {
    ...
    public:
          ...
          optional_yield y)
      {
        if (params.allow_unordered) {
          return list_objects_unordered(
              dpp, max, result, common_prefixes, is_truncated, y);
        } else {
          return list_objects_ordered(
              dpp, max, result, common_prefixes, is_truncated, y);
        }
      }
  ...
  int check_bucket_empty(
      const DoutPrefixProvider* dpp,
      RGWBucketInfo& bucket_info,
      optional_yield y);

  int store_delete_bucket_info_flag(
      RGWBucketInfo& bucket_info,
      std::map<std::string, bufferlist>& attrs,
      optional_yield y,
      const DoutPrefixProvider* dpp);
  ...
  int set_bucket_owner(
      rgw_bucket& bucket,
      ACLOwner& owner,
      const DoutPrefixProvider* dpp,
      optional_yield y);
  int set_buckets_enabled(
  ...
  int delete_obj(
      ...
      const bool skip_olh_obj_update =
          false); // true for all deletes (except the last one) initiated by a multi-object delete op
  ...
  int append_atomic_test(
      const DoutPrefixProvider* dpp,
      const RGWObjState* astate,
      librados::ObjectOperation& op);

  virtual int get_obj_iterate_cb(
      const DoutPrefixProvider* dpp,
  ...
  int check_reshard_logrecord_status(
      RGWBucketInfo& bucket_info,
      optional_yield y,
      const DoutPrefixProvider* dpp);
  int recover_reshard_logrecord(
      RGWBucketInfo& bucket_info,
      std::map<std::string, bufferlist>& bucket_attrs,
      optional_yield y,
      const DoutPrefixProvider* dpp);
  int block_while_resharding(
  ...
  void bucket_index_guard_olh_op(
      const DoutPrefixProvider* dpp,
      RGWObjState& olh_state,
      librados::ObjectOperation& op);
  void olh_cancel_modification(
      const DoutPrefixProvider* dpp,
      const RGWBucketInfo& bucket_info,
      RGWObjState& state,
  ...
  int olh_init_modification(
      ...
      std::string* op_tag,
      optional_yield y);
  int olh_init_modification_impl(
      const DoutPrefixProvider* dpp,
      const RGWBucketInfo& bucket_info,
      RGWObjState& state,
      const rgw_obj& olh_obj,
  ...
  int set_olh(
      ...
      bool skip_olh_obj_update =
          false); // can skip the OLH object update if, for example, repairing index
  int repair_olh(
      const DoutPrefixProvider* dpp,
      RGWObjState* state,
      const RGWBucketInfo& bucket_info,
  ...
  void check_pending_olh_entries(
      const DoutPrefixProvider* dpp,
      std::map<std::string, bufferlist>& pending_entries,
      std::map<std::string, bufferlist>* rm_pending_entries);
  int remove_olh_pending_entries(
      const DoutPrefixProvider* dpp,
      const RGWBucketInfo& bucket_info,
      RGWObjState& state,
      const rgw_obj& olh_obj,
      std::map<std::string, bufferlist>& pending_attrs,
      optional_yield y);
  int follow_olh(
      const DoutPrefixProvider* dpp,
      RGWBucketInfo& bucket_info,
...
public:
  void
  set_atomic(void* ctx, const rgw_obj& obj, bool atomic)
  {
  ...
  void
  set_prefetch_data(void* ctx, const rgw_obj& obj)
  {
    RGWObjectCtx* rctx = static_cast<RGWObjectCtx*>(ctx);
  ...
  void
  set_compressed(void* ctx, const rgw_obj& obj)
  {
    RGWObjectCtx* rctx = static_cast<RGWObjectCtx*>(ctx);
  ...
  int get_bucket_stats(
      ...
      bool* syncstopped = NULL);
  int get_bucket_stats_async(
      const DoutPrefixProvider* dpp,
      RGWBucketInfo& bucket_info,
      const rgw::bucket_index_layout_generation& idx_layout,
      int shard_id,
  ...
  static void make_bucket_entry_name(
  ...
  RGWChainedCacheImpl_bucket_topics_entry*
  get_topic_cache()
  {
    return topic_cache;
  }
  ...
  int cls_obj_usage_log_add(
      const DoutPrefixProvider* dpp,
      const std::string& oid,
      rgw_usage_log_info& info,
      optional_yield y);
  int cls_obj_usage_log_read(
      const DoutPrefixProvider* dpp,
      const std::string& oid,
  ...
  int cls_obj_usage_log_trim(
      ...
      uint64_t end_epoch,
      optional_yield y);
  int cls_obj_usage_log_clear(
      const DoutPrefixProvider* dpp,
      std::string& oid,
      optional_yield y);

  int get_target_shard_id(
  ...
  void update_gc_chain(
      const DoutPrefixProvider* dpp,
      rgw_obj head_obj,
      RGWObjManifest& manifest,
      cls_rgw_obj_chain* chain);
  std::tuple<int, std::optional<cls_rgw_obj_chain>> send_chain_to_gc(
      cls_rgw_obj_chain& chain,
      const std::string& tag,
      optional_yield y);
  void delete_objs_inline(
      const DoutPrefixProvider* dpp,
      cls_rgw_obj_chain& chain,
      const std::string& tag,
      optional_yield y);
  ...
  int move_rados_obj(
      ...
      const std::string& src_oid,
      const std::string& src_locator,
      librados::IoCtx& dst_ioctx,
      const std::string& dst_oid,
      const std::string& dst_locator,
      optional_yield y);
  int fix_head_obj_locator(
      const DoutPrefixProvider* dpp,
      const RGWBucketInfo& bucket_info,
      bool copy_obj,
      bool remove_bad,
  ...
  void calculate_preferred_shards(
      const DoutPrefixProvider* dpp,
      bool is_versioned,
...
private:
   ...
   */
  static uint32_t calc_ordered_bucket_list_per_shard(
      uint32_t num_entries,
      uint32_t num_shards);
};
