...
public:
  ...
  bool
  should_renew(const Clock::time_point& now) const
  {
...
public:
  ...
  static uint32_t
  get_prime_shards_less_or_equal(uint32_t requested_shards)
  {
    auto it = std::upper_bound(
        reshard_primes.begin(), reshard_primes.end(), requested_shards);
  ...
  static uint32_t
  get_prime_shards_greater_or_equal(uint32_t requested_shards)
  {
  ...
  static void calculate_preferred_shards(
      const DoutPrefixProvider* dpp,
  ...
  static bool should_zone_reshard_now(
...
protected:
  ...
  std::string get_logshard_key(
      const std::string& tenant,
      const std::string& bucket_name);
  void get_bucket_logshard_oid(
      const std::string& tenant,
      const std::string& bucket_name,
      std::string* oid);
...
private:
  ...
  struct Waiter : boost::intrusive::list_base_hook<> {
    ...
    using Timer = boost::asio::
        basic_waitable_timer<Clock, boost::asio::wait_traits<Clock>, Executor>;
    Timer timer;

    explicit Waiter(boost::asio::any_io_executor ex) :
      timer(ex)
    {}
  };
