...
void
RGWOp_Period_Get::execute(optional_yield y)
{
  ...
  op_ret =
      s->penv.cfgstore->read_period(this, y, period_id, std::nullopt, period);
  if (op_ret < 0)
...
void
RGWOp_Period_Post::execute(optional_yield y)
{
  auto cct = driver->ctx();
  ...
  if (period.get_realm() != static_cast<rgw::sal::RadosStore*>(driver)
                                ->svc()
                                ->zone->get_realm()
                                .get_id()) {
    error_stream << "period with realm id " << period.get_realm()
                 << " doesn't match current realm "
  ...
  op_ret = rgw::read_realm(
      this, y, s->penv.cfgstore, realm_id, realm_name, realm, &realm_writer);
  if (op_ret < 0) {
  ...
  op_ret = s->penv.cfgstore->read_period(
      this, y, realm.current_period, std::nullopt, current_period);
  if (op_ret < 0) {
  ...
  if (period.get_id().empty()) {
    op_ret = rgw::commit_period(
        this, y, s->penv.cfgstore, driver, realm, *realm_writer, current_period,
        period, error_stream, false, *s->penv.site);
    if (op_ret == -EEXIST) {
  ...
  if (period.get_master_zone() == static_cast<rgw::sal::RadosStore*>(driver)
                                      ->svc()
                                      ->zone->get_zone_params()
                                      .get_id()) {
    ldpp_dout(this, 10) << "master zone rejecting period id=" << period.get_id()
  ...
  if (op_ret < 0) {
    ldpp_dout(this, -1) << "failed to store period " << period.get_id()
                        << dendl;
    return;
  }
  // set as latest epoch
  op_ret = s->penv.cfgstore->update_latest_epoch(
      this, y, period.get_id(), period.get_epoch());
  if (op_ret == -EEXIST) {
  ...
  auto period_history = static_cast<rgw::sal::RadosStore*>(driver)
                            ->svc()
                            ->mdlog->get_period_history();
  ...
  if (period.get_id() != current_period.get_id()) {
    ...
    if (period.get_realm_epoch() < current_epoch) {
      ldpp_dout(this, 10) << "discarding period " << period.get_id()
                          ...
                          << " older than current epoch " << current_epoch
                          << dendl;
    ...
    auto cursor =
        period_history->attach(this, RGWPeriod{period}, y, s->penv.cfgstore);
    if (!cursor) {
      ...
      ldpp_dout(this, -1)
          ...
          << ") and the new period " << period.get_id() << " (realm epoch "
          << period.get_realm_epoch() << "): " << cpp_strerror(-op_ret)
          << dendl;
      return;
    }
    if (cursor.has_next()) {
      ...
      ldpp_dout(this, 4)
          << "attached period " << period.get_id()
          << " to history, but the history contains newer periods" << dendl;
      return;
    }
    // set as current period
    op_ret = rgw::realm_set_current_period(
        this, y, s->penv.cfgstore, *realm_writer, realm, period);
    if (op_ret < 0) {
      ldpp_dout(this, -1) << "failed to update realm's current period" << dendl;
      return;
    }
    ldpp_dout(this, 4) << "period " << period.get_id()
                       << " is newer than current period "
                       << current_period.get_id()
                       << ", updating realm's current period and notifying zone"
                       << dendl;
    notify_realm = std::move(realm); // trigger realm reload
    return;
  }
  ...
  ldpp_dout(this, 4) << "period epoch " << period.get_epoch()
                     << " is newer than current epoch "
                     << current_period.get_epoch()
                     << ", updating period's latest epoch and notifying zone"
                     << dendl;
  notify_realm = std::move(realm); // trigger realm reload
...
void
RGWOp_Period_Post::send_response()
{
  RGWOp_Period_Base::send_response();
...
void
RGWOp_Realm_Get::execute(optional_yield y)
{
  string id;
  ...
  op_ret = rgw::read_realm(
      this, y, s->penv.cfgstore, realm->get_id(), realm->get_name(), *realm);
  if (op_ret < 0)
    ldpp_dout(this, -1) << "failed to read realm id=" << id << " name=" << name
                        << dendl;
}

void
RGWOp_Realm_Get::send_response()
{
...
void
RGWOp_Realm_List::execute(optional_yield y)
{
  s->penv.cfgstore->read_default_realm_id(this, y, default_id);
  op_ret = static_cast<rgw::sal::RadosStore*>(driver)->svc()->zone->list_realms(
      this, realms);
  if (op_ret < 0)
...
void
RGWOp_Realm_List::send_response()
{
  set_req_state_err(s, op_ret);
  dump_errno(s);
