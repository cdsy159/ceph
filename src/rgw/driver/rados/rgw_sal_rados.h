...
public:
  ...
  allow_read_through()
  {
    return tier.allow_read_through;
  }

  virtual uint64_t
  get_read_through_restore_days()
  {
    return tier.read_through_restore_days;
  }
...
public:
  RadosZone(RadosStore* _store, std::unique_ptr<ZoneGroup> _zg) :
    store(_store), group(std::move(_zg)), local_zone(true)
  {}

  RadosZone(RadosStore* _store, std::unique_ptr<ZoneGroup> _zg, RGWZone& z) :
    store(_store), group(std::move(_zg)), rgw_zone(z)
  {}
...
public:
  ...
  int remove_persistent_topic(
      ...
      optional_yield y,
      const std::string& topic_queue) override;
  int update_bucket_topic_mapping(
      const rgw_pubsub_topic& topic,
  ...
  get_rgwrestore(void) override
  {
    return rados->get_restore();
  }

  virtual RGWCoroutinesManagerRegistry*
  get_cr_registry() override
  {
    return rados->get_cr_registry();
  }
  ...
  {
    return rados->clear_usage(dpp, y);
  }
  ...
  get_sync_module()
  {
    return rados->get_sync_module();
  }

  virtual std::string
  get_host_id()
  {
    return rados->host_id;
  }

  std::unique_ptr<LuaManager> get_lua_manager(
  ...
  void get_raw_obj(
      ...
      const rgw_obj& obj,
      rgw_raw_obj* raw_obj);
  int get_raw_chunk_size(
      const DoutPrefixProvider* dpp,
      const rgw_raw_obj& obj,
      uint64_t* chunk_size);

  void
  setRados(RGWRados* st)
  {
    rados = st;
  }
  ...
  get_io_context()
  {
    return neorados.get_io_context();
  }
...
public:
  ...
  int remove_topics(
      ...
      optional_yield y,
      const DoutPrefixProvider* dpp) override;
  int get_logging_object_name(
      std::string& obj_name,
      const std::string& prefix,
      optional_yield y,
      const DoutPrefixProvider* dpp,
      RGWObjVersionTracker* objv_tracker) override;
  int set_logging_object_name(
      ...
      bool new_obj,
      RGWObjVersionTracker* objv_tracker) override;
  int remove_logging_object_name(
      const std::string& prefix,
      optional_yield y,
      const DoutPrefixProvider* dpp,
      RGWObjVersionTracker* objv_tracker) override;
  int commit_logging_object(
      const std::string& obj_name,
      optional_yield y,
      const DoutPrefixProvider* dpp,
      const std::string& prefix,
      ...
      bool async) override;
  int remove_logging_object(
      const std::string& obj_name,
      const std::string& prefix,
      optional_yield y,
      const DoutPrefixProvider* dpp) override;
  int write_logging_object(
      const std::string& obj_name,
...
public:
  ...
  get_past_prefixes() const
  {
    return info.past_prefixes;
  }
...
public:
  ...
  ~RadosRestore() override { finalize(); }
...
public:
  ...
  rgw::notify::reservation_t&
  get_reservation(void)
  {
    return res;
  }
...
class RadosLuaManager : public StoreLuaManager {
  class PackagesWatcher : public librados::WatchCtx2,
  ...
  void ack_reload(
      ...
      uint64_t cookie,
      int reload_status);
  void handle_reload_notify(
      const DoutPrefixProvider* dpp,
      optional_yield y,
      uint64_t notify_id,
      uint64_t cookie);
  void ack_script_update(
      const DoutPrefixProvider* dpp,
      uint64_t notify_id,
      uint64_t cookie,
      int reload_status);
  void handle_script_update_notify(
      const DoutPrefixProvider* dpp,
      optional_yield y,
      uint64_t notify_id,
      uint64_t cookie);
  int notify_script_update(
      const DoutPrefixProvider* dpp,
...
public:
  ...
  std::tuple<rgw::lua::LuaCodeType, int> get_script_or_bytecode(
      const DoutPrefixProvider* dpp,
      optional_yield y,
