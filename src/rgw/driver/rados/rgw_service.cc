...
RGWServices_Def::~RGWServices_Def() { shutdown(); }

int
RGWServices_Def::init(
    CephContext* cct,
    ...
    const rgw::SiteConfig* site)
{
  ...
  bi_rados->init(
      zone.get(), driver->getRados()->get_rados_handle(), bilog_rados.get(),
      datalog_rados.get());
  bilog_rados->init(bi_rados.get());
  bucket_sobj->init(
      zone.get(), sysobj.get(), sysobj_cache.get(), bi_rados.get(), mdlog.get(),
      sync_modules.get(), bucket_sync_sobj.get());
  bucket_sync_sobj->init(
      zone.get(), sysobj.get(), sysobj_cache.get(), bucket_sobj.get());
  cls->init(zone.get(), driver->getRados()->get_rados_handle());
  config_key_rados->init(driver->getRados()->get_rados_handle());
  mdlog->init(
      driver->getRados()->get_rados_handle(), zone.get(), sysobj.get(),
      cls.get(), async_processor.get());
  if (notify) {
    notify->init(zone.get(), driver->getRados()->get_rados_handle());
  }
  zone->init(
      sysobj.get(), driver->getRados()->get_rados_handle(), sync_modules.get(),
  ...
  if (have_cache) {
    sysobj_cache->init(
        driver->getRados()->get_rados_handle(), zone.get(), notify.get());
  ...
  user_rados->init(
      driver->getRados()->get_rados_handle(), zone.get(), mdlog.get(),
  ...
  if (notify) {
    ...
    if (r < 0) {
      ldpp_dout(dpp, 0) << "ERROR: failed to start notify service ("
  ...
  if (!raw) {
    r = zone->start(y, dpp);
    if (r < 0) {
      ldpp_dout(dpp, 0) << "ERROR: failed to start zone service ("
                        << cpp_strerror(-r) << dendl;
      return r;
    }

    r = datalog_rados->start(
        dpp, &zone->get_zone(), zone->get_zone_params(), background_tasks);
    if (r < 0) {
      ldpp_dout(dpp, 0) << "ERROR: failed to start datalog_rados service ("
                        << cpp_strerror(-r) << dendl;
      return r;
    }
    ...
    r = mdlog->start(y, dpp);
    if (r < 0) {
      ldpp_dout(dpp, 0) << "ERROR: failed to start mdlog service ("
                        << cpp_strerror(-r) << dendl;
      return r;
    }
    ...
    if (r < 0) {
      ldpp_dout(dpp, 0) << "ERROR: failed to start sync modules service ("
                        << cpp_strerror(-r) << dendl;
      return r;
    }
  }
  ...
  r = cls->start(y, dpp);
  if (r < 0) {
    ldpp_dout(dpp, 0) << "ERROR: failed to start cls service ("
                      << cpp_strerror(-r) << dendl;
    return r;
  }
  ...
  if (r < 0) {
    ldpp_dout(dpp, 0) << "ERROR: failed to start config_key service ("
                      << cpp_strerror(-r) << dendl;
    return r;
  }
  ...
  if (r < 0) {
    ldpp_dout(dpp, 0) << "ERROR: failed to start zone_utils service ("
                      << cpp_strerror(-r) << dendl;
    return r;
  }
  ...
  r = quota->start(y, dpp);
  if (r < 0) {
    ldpp_dout(dpp, 0) << "ERROR: failed to start quota service ("
                      << cpp_strerror(-r) << dendl;
    return r;
  }
  ...
  if (r < 0) {
    ldpp_dout(dpp, 0) << "ERROR: failed to start sysobj_core service ("
                      << cpp_strerror(-r) << dendl;
    return r;
  }
  ...
  if (have_cache) {
    ...
    if (r < 0) {
      ldpp_dout(dpp, 0) << "ERROR: failed to start sysobj_cache service ("
                        << cpp_strerror(-r) << dendl;
      return r;
    }
  }
  ...
  if (r < 0) {
    ldpp_dout(dpp, 0) << "ERROR: failed to start sysobj service ("
                      << cpp_strerror(-r) << dendl;
    return r;
  }
  ...
  if (!raw) {
    ...
    if (r < 0) {
      ldpp_dout(dpp, 0) << "ERROR: failed to start bucket service ("
                        << cpp_strerror(-r) << dendl;
      return r;
    }
    ...
    if (r < 0) {
      ldpp_dout(dpp, 0) << "ERROR: failed to start bucket_sync service ("
                        << cpp_strerror(-r) << dendl;
      return r;
    }
    ...
    if (r < 0) {
      ldpp_dout(dpp, 0) << "ERROR: failed to start user_rados service ("
                        << cpp_strerror(-r) << dendl;
      return r;
    }
  }
    ...
    rgw::sal::ConfigStore* cfgstore)
{
  ...
  int r = _svc.init(
      cct, driver, have_cache, raw, run_sync, background_tasks, y, dpp,
      cfgstore, site);
  if (r < 0) {
    return r;
  }
...
int
RGWServiceInstance::start(optional_yield y, const DoutPrefixProvider* dpp)
{
    ...
    const DoutPrefixProvider* dpp)
{
  ...
  bucket.reset(new RGWBucketCtl(
      svc.zone, svc.bucket, svc.bucket_sync, svc.bi, svc.user,
      svc.datalog_rados));

  auto sync_module = svc.sync_modules->get_sync_module();
  if (sync_module) {
    meta.bucket =
        sync_module->alloc_bucket_meta_handler(rados, svc.bucket, bucket.get());
    meta.bucket_instance = sync_module->alloc_bucket_instance_meta_handler(
        driver, svc.zone, svc.bucket, svc.bi, svc.datalog_rados);
  } else {
    meta.bucket =
        create_bucket_metadata_handler(rados, svc.bucket, bucket.get());
  ...
  meta.topic_cache =
      std::make_unique<RGWChainedCacheImpl<rgwrados::topic::cache_entry>>();
  meta.topic_cache->init(svc.cache);

  meta.topic = rgwrados::topic::create_metadata_handler(
      *svc.sysobj, svc.cache, *svc.mdlog, rados, svc.zone->get_zone_params(),
      *meta.topic_cache);
    ...
    const DoutPrefixProvider* dpp)
{
  ...
  if (r < 0) {
    ldpp_dout(dpp, 0) << "ERROR: failed to start init ctls ("
                      << cpp_strerror(-r) << dendl;
    return r;
  }
  ...
  if (r < 0) {
    ldout(cct, 0) << "ERROR: failed to start init meta.user ctl ("
                  << cpp_strerror(-r) << dendl;
    return r;
  }
  ...
  if (r < 0) {
    ldout(cct, 0) << "ERROR: failed to start init meta.bucket ctl ("
                  << cpp_strerror(-r) << dendl;
    return r;
  }
  ...
  if (r < 0) {
    ldout(cct, 0) << "ERROR: failed to start init meta.bucket_instance ctl ("
                  << cpp_strerror(-r) << dendl;
    return r;
  }
  ...
  if (r < 0) {
    ldout(cct, 0) << "ERROR: failed to start init otp ctl (" << cpp_strerror(-r)
                  << dendl;
    return r;
  }
  ...
  if (r < 0) {
    ldout(cct, 0) << "ERROR: failed to start init meta.role ctl ("
                  << cpp_strerror(-r) << dendl;
    return r;
  }
  ...
  if (r < 0) {
    ldout(cct, 0) << "ERROR: failed to start init meta.account ctl ("
                  << cpp_strerror(-r) << dendl;
    return r;
  }
  ...
  if (r < 0) {
    ldout(cct, 0) << "ERROR: failed to start init meta.group ctl ("
                  << cpp_strerror(-r) << dendl;
    return r;
  }
  return 0;
