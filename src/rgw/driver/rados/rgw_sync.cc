...
RGWSyncErrorLogger::RGWSyncErrorLogger(
    rgw::sal::RadosStore* _store,
    const string& oid_prefix,
    int _num_shards) :
  store(_store), num_shards(_num_shards)
{
...
string
RGWSyncErrorLogger::get_shard_oid(const string& oid_prefix, int shard_id)
{
...
RGWCoroutine*
RGWSyncErrorLogger::log_error_cr(
    ...
    const string& message)
{
  ...
  bufferlist bl;
  encode(info, bl);
  store->svc()->cls->timelog.prepare_entry(
      entry, real_clock::now(), section, name, bl);
...
void
RGWSyncBackoff::update_wait_time()
{
  if (cur_wait == 0) {
    cur_wait = 1;
  } else {
...
void
RGWSyncBackoff::backoff(RGWCoroutine* op)
{
  update_wait_time();
...
int
RGWBackoffControlCR::operate(const DoutPrefixProvider* dpp)
{
  reenter(this)
  {
    ...
    while (true) {
      ...
      if (retcode != -EBUSY && retcode != -EAGAIN) {
        ldout(cct, 0) << "ERROR: RGWBackoffControlCR called coroutine returned "
                      << retcode << dendl;
        if (exit_on_error) {
    ...
    if (retcode < 0) {
      ldout(cct, 0) << "ERROR: call to finisher_cr() failed: retcode="
                    << retcode << dendl;
      return set_cr_error(retcode);
    }
    return set_cr_done();
...
void
rgw_mdlog_info::decode_json(JSONObj* obj)
{
...
void
rgw_mdlog_entry::decode_json(JSONObj* obj)
{
...
void
rgw_mdlog_shard_data::decode_json(JSONObj* obj)
{
...
int
RGWShardCollectCR::operate(const DoutPrefixProvider* dpp)
{
  reenter(this)
  {
    while (spawn_next()) {
...
#define READ_MDLOG_MAX_CONCURRENT 10
  ...
  handle_result(int r) override
  {
    ...
    if (r < 0) {
      ldout(cct, 4) << "failed to fetch mdlog status: " << cpp_strerror(r)
                    << dendl;
    }
    return r;
  }
...
public:
  RGWReadRemoteMDLogInfoCR(
      RGWMetaSyncEnv* _sync_env,
      const std::string& period,
      int _num_shards,
...
#define READ_MDLOG_MAX_CONCURRENT 10
  ...
  handle_result(int r) override
  {
    ...
    if (r < 0) {
      ldout(cct, 4) << "failed to list remote mdlog shard: " << cpp_strerror(r)
                    << dendl;
    }
    return r;
  }
...
public:
  RGWListRemoteMDLogCR(
      RGWMetaSyncEnv* _sync_env,
      const std::string& period,
      map<int, string>& _shards,
      int _max_entries_per_shard,
      map<int, rgw_mdlog_shard_data>* _result) :
    RGWShardCollectCR(_sync_env->cct, READ_MDLOG_MAX_CONCURRENT),
    sync_env(_sync_env),
    period(period),
...
RGWRemoteMetaLog::~RGWRemoteMetaLog() { delete error_logger; }

void
RGWRemoteMetaLog::finish()
{
  going_down = true;
  stop();
}
...
int
RGWRemoteMetaLog::read_log_info(
    const DoutPrefixProvider* dpp,
    rgw_mdlog_info* log_info)
{
  ...
  int ret =
      conn->get_json_resource(dpp, "/admin/log", pairs, null_yield, *log_info);
  if (ret < 0) {
    ldpp_dout(dpp, 0) << "ERROR: failed to fetch mdlog info" << dendl;
    return ret;
  }

  ldpp_dout(dpp, 20) << "remote mdlog, num_shards=" << log_info->num_shards
                     << dendl;
...
int
RGWRemoteMetaLog::read_master_log_shards_info(
    const DoutPrefixProvider* dpp,
    const string& master_period,
    map<int, RGWMetadataLogInfo>* shards_info)
{
  ...
  return run(
      dpp, new RGWReadRemoteMDLogInfoCR(
               &sync_env, master_period, log_info.num_shards, shards_info));
}

int
RGWRemoteMetaLog::read_master_log_shards_next(
    const DoutPrefixProvider* dpp,
    const string& period,
    ...
    map<int, rgw_mdlog_shard_data>* result)
{
  ...
  return run(
      dpp,
      new RGWListRemoteMDLogCR(&sync_env, period, shard_markers, 1, result));
}

int
RGWRemoteMetaLog::init()
{
  ...
  error_logger = new RGWSyncErrorLogger(
      store, RGW_SYNC_ERROR_LOG_SHARD_PREFIX, ERROR_LOGGER_SHARDS);
...
std::ostream&
RGWMetaSyncStatusManager::gen_prefix(std::ostream& out) const
{
  return out << "meta sync: ";
}

unsigned
RGWMetaSyncStatusManager::get_subsys() const
{
  return dout_subsys;
}
...
int
RGWMetaSyncStatusManager::init(const DoutPrefixProvider* dpp)
{
  if (store->svc()->zone->is_meta_master()) {
    return 0;
  }
  ...
  int r = rgw_init_ioctx(
      dpp, store->getRados()->get_rados_handle(),
      store->svc()->zone->get_zone_params().log_pool, ioctx, true);
  if (r < 0) {
    ldpp_dout(dpp, -1) << "ERROR: failed to open log pool ("
  ...
  for (int i = 0; i < num_shards; i++) {
    shard_objs[i] = rgw_raw_obj(
        store->svc()->zone->get_zone_params().log_pool,
        sync_env.shard_obj_name(i));
  }
...
RGWMetaSyncEnv::shard_obj_name(int shard_id)
{
  ...
  snprintf(
      buf, sizeof(buf), "%s.%d", mdlog_sync_status_shard_prefix.c_str(),
      shard_id);
...
protected:
  int
  _send_request(const DoutPrefixProvider* dpp) override
  {
    real_time from_time;
    real_time end_time;
    ...
    int ret = mdlog->list_entries(
        dpp, handle, max_entries, entries, &marker, &truncated, null_yield);
...
public:
  ...
  send_request(const DoutPrefixProvider* dpp) override
  {
    ...
    req = new RGWAsyncReadMDLogEntries(
        dpp, this, stack->create_completion_notifier(), sync_env->store, mdlog,
        shard_id, marker, max_entries);
  ...
  operate(const DoutPrefixProvider* dpp) override
  {
    ...
    reenter(this)
    {
      ...
      for (tries = 0; tries < NUM_ENPOINT_IOERROR_RETRIES; tries++) {
        ldpp_dout(dpp, 20) << "read remote metadata log shard info. shard_is="
        ...
        yield
        {
          ...
          http_op =
              new RGWRESTReadResource(conn, p, pairs, NULL, env->http_manager);
        ...
        if (op_ret < 0) {
          if (op_ret == -ERR_INTERNAL_ERROR &&
            ...
            ldpp_dout(dpp, 20) << "failed to read remote metadata log shard "
                                  "info. retry. shard_id="
                               << shard_id << dendl;
            continue;
          } else {
...
RGWCoroutine*
create_read_remote_mdlog_shard_info_cr(
    RGWMetaSyncEnv* env,
    const std::string& period,
    int shard_id,
...
public:
  RGWListRemoteMDLogShardCR(
    ...
    RGWSimpleCoroutine(env->store->ctx(), NUM_ENPOINT_IOERROR_RETRIES),
    sync_env(env),
    http_op(NULL),
    period(period),
    shard_id(_shard_id),
  ...
  send_request(const DoutPrefixProvider* dpp) override
  {
    ...
    rgw_http_param_pair pairs[] = {
        {"type", "metadata"},         {"id", buf},
        {"period", period.c_str()},   {"max-entries", max_entries_buf},
        {marker_key, marker.c_str()}, {NULL, NULL}};
  ...
  request_complete() override
  {
    ...
    if (ret < 0 && ret != -ENOENT) {
      ldpp_dout(sync_env->dpp, 5)
          << "ERROR: failed to list remote mdlog shard, ret=" << ret << dendl;
      return ret;
    }
    return 0;
  }
...
RGWCoroutine*
create_list_remote_mdlog_shard_cr(
    RGWMetaSyncEnv* env,
    const std::string& period,
    int shard_id,
    ...
    rgw_mdlog_shard_data* result)
{
  return new RGWListRemoteMDLogShardCR(
      env, period, shard_id, marker, max_entries, result);
}

bool
RGWReadRemoteMDLogInfoCR::spawn_next()
{
  if (shard_id >= num_shards) {
    return false;
  }
  spawn(
      new RGWReadRemoteMDLogShardInfoCR(
          sync_env, period, shard_id, &(*mdlog_info)[shard_id]),
      false);
  shard_id++;
  return true;
}
...
bool
RGWListRemoteMDLogCR::spawn_next()
{
  if (iter == shards.end()) {
    return false;
  }
  ...
  spawn(
      new RGWListRemoteMDLogShardCR(
          sync_env, period, iter->first, iter->second, max_entries_per_shard,
          &(*result)[iter->first]),
      false);
  ++iter;
  return true;
  ...
  operate(const DoutPrefixProvider* dpp) override
  {
    ...
    reenter(this)
    {
      ...
      if (retcode < 0) {
        ...
        ldpp_dout(dpp, 0) << "ERROR: failed to write sync status, retcode="
                          << retcode << dendl;
      ...
      yield
      {
        for (int i = 0; i < (int)status.num_shards; i++) {
          spawn(
              new RGWReadRemoteMDLogShardInfoCR(
                  sync_env, status.period, i, &shards_info[i]),
              false);
        }
      }

      drain_all_but_stack(
          lease_stack.get()); /* the lease cr still needs to run */
      ...
      yield
      {
        ...
        for (int i = 0; i < (int)status.num_shards; i++) {
          ...
          rgw::sal::RadosStore* store = sync_env->store;
          spawn(
              new RGWSimpleRadosWriteCR<rgw_meta_sync_marker>(
                  dpp, store,
                  rgw_raw_obj(
                      store->svc()->zone->get_zone_params().log_pool,
                      sync_env->shard_obj_name(i)),
...
bool
RGWReadSyncStatusMarkersCR::spawn_next()
{
  if (shard_id >= num_shards) {
    return false;
  }
  ...
  rgw_raw_obj obj{
      env->store->svc()->zone->get_zone_params().log_pool,
...
int
RGWReadSyncStatusCoroutine::operate(const DoutPrefixProvider* dpp)
{
  reenter(this)
  {
    // read sync info
    ...
    yield
    {
      ...
      rgw_raw_obj obj{
          ...
          sync_env->status_oid()};
      call(new ReadInfoCR(
          dpp, sync_env->store, obj, &sync_status->sync_info, empty_on_enoent));
    }
    if (retcode < 0) {
    ...
    yield call(new ReadMarkersCR(
        sync_env, sync_status->sync_info.num_shards, sync_status->sync_markers));
    if (retcode < 0) {
...
public:
  ...
  void
  append_section_from_set(set<string>& all_sections, const string& name)
  {
  ...
  rearrange_sections()
  {
    ...
    std::move(
        all_sections.begin(), all_sections.end(), std::back_inserter(sections));
  }
  ...
  operate(const DoutPrefixProvider* dpp) override
  {
    ...
    reenter(this)
    {
      yield
      {
        ...
        string lock_name = "sync_lock";
        lease_cr.reset(new RGWContinuousLeaseCR(
            sync_env->async_rados, sync_env->store,
            rgw_raw_obj(
                sync_env->store->svc()->zone->get_zone_params().log_pool,
                sync_env->status_oid()),
            lock_name, lock_duration, this, nullptr));
      ...
      entries_index.reset(new RGWShardedOmapCRManager(
          sync_env->async_rados, sync_env->store, this, num_shards,
          sync_env->store->svc()->zone->get_zone_params().log_pool,
          mdlog_sync_full_sync_index_prefix));
      yield
      {
        call(new RGWReadRESTResourceCR<list<string>>(
            cct, conn, sync_env->http_manager, "/admin/metadata", NULL,
            &sections));
      }
      if (get_ret_status() < 0) {
        ldpp_dout(dpp, 0) << "ERROR: failed to fetch metadata sections"
                          << dendl;
...
#define META_FULL_SYNC_CHUNK_SIZE "1000"
          ...
          if (ret_status == -ENOENT) {
            set_retcode(
                0); /* reset coroutine status so that we don't return it */
            ret_status = 0;
          }
          if (ret_status < 0) {
            tn->log(
                0, SSTR(
                       "ERROR: failed to fetch metadata section: "
                       << *sections_iter));
            yield entries_index->finish();
            yield lease_cr->go_down();
            drain_all();
          ...
          for (; iter != result.keys.end(); ++iter) {
            ...
            tn->log(
                20, SSTR(
                        "list metadata: section=" << *sections_iter
                                                  << " key=" << *iter));
            string s = *sections_iter + ":" + *iter;
            int shard_id;
            rgw::sal::RadosStore* store = sync_env->store;
            int ret = store->ctl()->meta.mgr->get_shard_id(
                *sections_iter, *iter, &shard_id);
            if (ret < 0) {
              tn->log(
                  0, SSTR(
                         "ERROR: could not determine shard id for "
                         << *sections_iter << ":" << *iter));
              ret_status = ret;
              break;
            }
      ...
      if (!failed) {
        for (map<uint32_t, rgw_meta_sync_marker>::iterator iter =
                 markers.begin();
...
full_sync_index_shard_oid(int shard_id)
{
  ...
  snprintf(
      buf, sizeof(buf), "%s.%d", mdlog_sync_full_sync_index_prefix.c_str(),
      shard_id);
  return string(buf);
}
...
public:
    ...
    pbl(_pbl)
  {
    tn = sync_env->sync_tracer->add_node(
        _tn_parent, "read_remote_meta", section + ":" + key);
  }
  ...
  operate(const DoutPrefixProvider* dpp) override
  {
    ...
    reenter(this)
    {
      static constexpr int NUM_ENPOINT_IOERROR_RETRIES = 20;
      for (tries = 0; tries < NUM_ENPOINT_IOERROR_RETRIES; tries++) {
        ldpp_dout(dpp, 20) << "read remote metadata.  retries=" << tries
                           << dendl;
        ...
        yield
        {
          ...
          http_op = new RGWRESTReadResource(
              conn, p, pairs, NULL, sync_env->http_manager);
        ...
        if (op_ret < 0) {
          if (op_ret == -ERR_INTERNAL_ERROR &&
              tries < NUM_ENPOINT_IOERROR_RETRIES - 1) {
            ldpp_dout(dpp, 20)
                << "failed to read remote metadata. retry. section=" << section
                << " key=" << key << dendl;
            continue;
          } else {
            return set_cr_error(op_ret);
...
protected:
  ...
  _send_request(const DoutPrefixProvider* dpp) override
  {
    int ret = store->ctl()->meta.mgr->put(
        raw_key, bl, null_yield, dpp, RGWMDLogSyncType::APPLY_ALWAYS, true);
    if (ret < 0) {
      ldpp_dout(dpp, 0) << "ERROR: can't store key: " << raw_key
                        << " ret=" << ret << dendl;
      return ret;
    }
    return 0;
...
public:
  ...
  send_request(const DoutPrefixProvider* dpp) override
  {
    req = new RGWAsyncMetaStoreEntry(
        this, stack->create_completion_notifier(), sync_env->store, raw_key, bl,
        dpp);
    sync_env->async_rados->queue(req);
    return 0;
  }
...
protected:
  ...
  _send_request(const DoutPrefixProvider* dpp) override
  {
    ...
    if (ret < 0) {
      ldpp_dout(dpp, 0) << "ERROR: can't remove key: " << raw_key
                        << " ret=" << ret << dendl;
      return ret;
    }
    return 0;
...
public:
  RGWAsyncMetaRemoveEntry(
      ...
      const string& _raw_key,
      const DoutPrefixProvider* dpp) :
    RGWAsyncRadosRequest(caller, cn), store(_store), raw_key(_raw_key), dpp(dpp)
  {}
};
...
public:
  ...
  send_request(const DoutPrefixProvider* dpp) override
  {
    req = new RGWAsyncMetaRemoveEntry(
        this, stack->create_completion_notifier(), sync_env->store, raw_key,
        dpp);
    sync_env->async_rados->queue(req);
    return 0;
  }
...
int
RGWLastCallerWinsCR::operate(const DoutPrefixProvider* dpp)
{
  ...
  reenter(this)
  {
    while (cr) {
      ...
      if (retcode < 0) {
        ldpp_dout(dpp, 0) << "ERROR: RGWLastCallerWinsCR() failed: retcode="
                          << retcode << dendl;
        return set_cr_error(retcode);
      }
    }
...
public:
      ...
      const real_time& timestamp) override
  {
    ...
    ldpp_dout(sync_env->dpp, 20)
        << __func__ << "(): updating marker marker_oid=" << marker_oid
        << " marker=" << new_marker
        << " realm_epoch=" << sync_marker.realm_epoch << dendl;
...
RGWMetaSyncSingleEntryCR::RGWMetaSyncSingleEntryCR(
    RGWMetaSyncEnv* _sync_env,
    const string& _raw_key,
    const string& _entry_marker,
...
{
  error_injection =
      (sync_env->cct->_conf->rgw_sync_meta_inject_err_probability > 0);
  tn = sync_env->sync_tracer->add_node(_tn_parent, "entry", raw_key);
}

int
RGWMetaSyncSingleEntryCR::operate(const DoutPrefixProvider* dpp)
{
  reenter(this)
  {
#define NUM_TRANSIENT_ERROR_RETRIES 10
    ...
    if (error_injection &&
        rand() % 10000 <
            cct->_conf->rgw_sync_meta_inject_err_probability * 10000.0) {
    ...
    for (tries = 0; tries < NUM_TRANSIENT_ERROR_RETRIES; tries++) {
      yield
      {
        ...
        tn->log(
            10, SSTR(
                    "fetching remote metadata entry"
                    << (tries == 0 ? "" : " (retry)")));
      ...
      if (sync_status < 0) {
        if (tries < NUM_TRANSIENT_ERROR_RETRIES - 1) {
          ldpp_dout(dpp, 20)
              << *this << ": failed to fetch remote metadata entry: " << section
              << ":" << key << ", will retry" << dendl;
          continue;
        }

        tn->log(
            10, SSTR(
                    "failed to read remote metadata entry: section="
                    << section << " key=" << key << " status=" << sync_status));
        log_error() << "failed to read remote metadata entry: section="
                    << section << " key=" << key << " status=" << sync_status
                    << std::endl;
        yield call(sync_env->error_logger->log_error_cr(
            dpp, sync_env->conn->get_remote_id(), section, key, -sync_status,
            string("failed to read remote metadata entry: ") +
    ...
    for (tries = 0; tries < NUM_TRANSIENT_ERROR_RETRIES; tries++) {
      if (sync_status != -ENOENT) {
        tn->log(
            10, SSTR("storing local metadata entry: " << section << ":" << key));
        yield call(new RGWMetaStoreEntryCR(sync_env, raw_key, md_bl));
      } else {
        tn->log(
            10, SSTR("removing local metadata entry:" << section << ":" << key));
      ...
      if ((retcode < 0) && (tries < NUM_TRANSIENT_ERROR_RETRIES - 1)) {
        ldpp_dout(dpp, 20) << *this
                           << ": failed to store metadata entry: " << section
                           << ":" << key << ", got retcode=" << retcode
                           << ", will retry" << dendl;
        continue;
      }
      break;
...
public:
  ...
  void
  set_marker_tracker(RGWMetaSyncShardMarkerTrack* mt)
  {
    delete marker_tracker;
    marker_tracker = mt;
  }
  ...
  operate(const DoutPrefixProvider* dpp) override
  {
    ...
    while (true) {
      ...
      case rgw_meta_sync_marker::FullSync:
        r = full_sync();
        if (r < 0) {
          ldpp_dout(dpp, 10) << "sync: full_sync: shard_id=" << shard_id
                             << " r=" << r << dendl;
      ...
      case rgw_meta_sync_marker::IncrementalSync:
        r = incremental_sync();
        if (r < 0) {
          ldpp_dout(dpp, 10) << "sync: incremental_sync: shard_id=" << shard_id
                             << " r=" << r << dendl;
          return set_cr_error(r);
        }
        return 0;
    ...
    while (collect_next(&child_ret, &child)) {
      ...
      if (child_ret < 0) {
        ldpp_dout(sync_env->dpp, 0)
            << *this << ": child operation stack=" << child << " entry=" << pos
      ...
      ldpp_dout(sync_env->dpp, 4)
          << *this << ": adjusting marker pos=" << sync_marker.marker << dendl;
...
#define OMAP_GET_MAX_ENTRIES 100
    ...
    reenter(&full_cr)
    {
      ...
      yield
      {
        ...
        lease_cr.reset(new RGWContinuousLeaseCR(
            sync_env->async_rados, store,
            rgw_raw_obj(pool, sync_env->shard_obj_name(shard_id)), lock_name,
            lock_duration, this, nullptr));
        lease_stack.reset(spawn(lease_cr.get(), false));
        lost_lock = false;
      }
      while (!lease_cr->is_locked()) {
      ...
      *reset_backoff = true;
      ...
      set_marker_tracker(new RGWMetaSyncShardMarkerTrack(
          sync_env, sync_env->shard_obj_name(shard_id), sync_marker, tn));
      ...
      do {
        ...
        yield call(new RGWRadosGetOmapKeysCR(
            sync_env->store, rgw_raw_obj(pool, oid), marker, max_entries,
            omapkeys));
        if (retcode < 0) {
          ldpp_dout(sync_env->dpp, 0)
              << "ERROR: " << __func__
              ...
              << dendl;
          tn->log(
              0, SSTR("ERROR: failed to list omap keys, status=" << retcode));
          yield lease_cr->go_down();
          drain_all();
          return retcode;
        }
        ...
        for (; iter != entries.end(); ++iter) {
          ...
          if (!marker_tracker->start(marker, total_entries, real_time())) {
            tn->log(
                0, SSTR(
                       "ERROR: cannot start syncing " << marker
                                                      << ". Duplicate entry?"));
          } else {
            // fetch remote and write locally
            yield
            {
              RGWCoroutinesStack* stack = spawn(
                  new RGWMetaSyncSingleEntryCR(
                      sync_env, marker, marker, MDLOG_STATUS_COMPLETE,
                      marker_tracker, tn),
                  false);
            ...
            // limit spawn window
            while (num_spawned() >
                   static_cast<size_t>(cct->_conf->rgw_meta_sync_spawn_window)) {
              yield wait_for_child();
              collect_children();
            }
          }
      ...
      if (lost_bid) {
        ...
        if (can_adjust_marker) {
          ...
          ldpp_dout(sync_env->dpp, 4)
              << *this << ": saving marker pos=" << temp_marker->marker
        ...
        if (retcode < 0) {
          ldpp_dout(sync_env->dpp, 0)
              << "ERROR: failed to set sync marker: retcode=" << retcode
              << dendl;
          yield lease_cr->go_down();
          drain_all();
          return retcode;
  ...
  incremental_sync()
  {
    reenter(&incremental_cr)
    {
      ...
      if (sync_marker.realm_epoch < realm_epoch) {
        ldpp_dout(sync_env->dpp, 4)
            << "clearing marker=" << sync_marker.marker
            << " from old realm_epoch=" << sync_marker.realm_epoch << " (now "
            << realm_epoch << ')' << dendl;
...
#define INCREMENTAL_MAX_ENTRIES 100
        ldpp_dout(sync_env->dpp, 20)
            << __func__ << ":" << __LINE__ << ": shard_id=" << shard_id
            << " mdlog_marker=" << mdlog_marker
            << " sync_marker.marker=" << sync_marker.marker
            << " period_marker=" << period_marker << " truncated=" << truncated
            << dendl;
        if (!period_marker.empty() && period_marker <= mdlog_marker) {
          tn->log(
              10, SSTR(
                      "finished syncing current period: mdlog_marker="
        ...
        if (mdlog_marker <= max_marker || !truncated) {
          ...
          yield call(new RGWCloneMetaLogCoroutine(
              sync_env, mdlog, period, shard_id, mdlog_marker, &mdlog_marker));
        }
        if (retcode < 0) {
          tn->log(
              10, SSTR(
                      *this << ": failed to fetch more log entries, retcode="
                            << retcode));
          yield lease_cr->go_down();
          drain_all();
        ...
        *reset_backoff =
        ...
        if (mdlog_marker > max_marker) {
          ...
          tn->log(
              20, SSTR(
                      "mdlog_marker=" << mdlog_marker << " sync_marker="
                                      << sync_marker.marker));
          marker = max_marker;
          yield call(new RGWReadMDLogEntriesCR(
              sync_env, mdlog, shard_id, &max_marker, INCREMENTAL_MAX_ENTRIES,
              &log_entries, &truncated));
          if (retcode < 0) {
            tn->log(
                10, SSTR("failed to list mdlog entries, retcode=" << retcode));
            yield lease_cr->go_down();
            drain_all();
            *reset_backoff = false; // back off and try again later
            return retcode;
          ...
          for (log_iter = log_entries.begin();
               log_iter != log_entries.end() && !done_with_period; ++log_iter) {
            if (!period_marker.empty() && period_marker <= log_iter->id) {
              ...
              if (period_marker < log_iter->id) {
                tn->log(
                    10,
                    SSTR(
                        "found key=" << log_iter->id << " past period_marker="
                                     << period_marker));
                break;
              }
              ldpp_dout(sync_env->dpp, 10)
                  << "found key at period_marker=" << period_marker << dendl;
              // sync this entry, then return control to RGWMetaSyncCR
            }
            if (!mdlog_entry.convert_from(*log_iter)) {
              tn->log(
                  0, SSTR(
                         "ERROR: failed to convert mdlog entry, shard_id="
                         << shard_id << " log_entry: " << log_iter->id << ":"
                         << log_iter->section << ":" << log_iter->name << ":"
                         << log_iter->timestamp << " ... skipping entry"));
              continue;
            }
            tn->log(
                20, SSTR(
                        "log_entry: "
                        << log_iter->id << ":" << log_iter->section << ":"
                        << log_iter->name << ":" << log_iter->timestamp));
            if (!marker_tracker->start(log_iter->id, 0, log_iter->timestamp)) {
              ldpp_dout(sync_env->dpp, 0)
                  << "ERROR: cannot start syncing " << log_iter->id
              ...
              yield
              {
                RGWCoroutinesStack* stack = spawn(
                    new RGWMetaSyncSingleEntryCR(
                        sync_env, raw_key, log_iter->id,
                        mdlog_entry.log_data.status, marker_tracker, tn),
                    false);
                ceph_assert(stack);
                // stack_to_pos holds a reference to the stack
        ...
        ldpp_dout(sync_env->dpp, 20)
            << __func__ << ":" << __LINE__ << ": shard_id=" << shard_id
            << " mdlog_marker=" << mdlog_marker << " max_marker=" << max_marker
            << " sync_marker.marker=" << sync_marker.marker
        ...
        if (mdlog_marker == old_mdlog_marker && can_adjust_marker) {
          tn->log(
              20, SSTR(
                      "mdlog_marker=" << mdlog_marker << " old_mdlog_marker="
                                      << old_mdlog_marker));
...
public:
  ...
  alloc_cr() override
  {
    return new RGWMetaSyncShardCR(
        sync_env, pool, period, realm_epoch, mdlog, shard_id, sync_marker,
        period_marker, backoff_ptr(), tn);
  }
  ...
  alloc_finisher_cr() override
  {
    ...
    return new RGWSimpleRadosReadCR<rgw_meta_sync_marker>(
        sync_env->dpp, store,
        rgw_raw_obj(pool, sync_env->shard_obj_name(shard_id)), &sync_marker);
  }
};
...
public:
  RGWMetaSyncCR(
      RGWMetaSyncEnv* _sync_env,
      const RGWPeriodHistory::Cursor& cursor,
  ...
  operate(const DoutPrefixProvider* dpp) override
  {
    reenter(this)
    {
      ...
      for (;;) {
        if (cursor ==
            sync_env->store->svc()->mdlog->get_period_history()->get_current()) {
          ...
          next.next();
          ldpp_dout(dpp, 10)
              << "RGWMetaSyncCR on period=" << cursor.get_period().get_id()
        ...
        yield
        {
          ...
          tn->log(
              1,
              SSTR("realm epoch=" << realm_epoch << " period id=" << period_id));
          ...
          for (const auto& m : sync_status.sync_markers) {
            ...
            if (next) {
              ...
              if (period_marker.empty()) {
                ...
                ldpp_dout(dpp, 10)
                    << "RGWMetaSyncCR: skipping shard " << shard_id
            ...
            auto cr = new ShardCR(
                sync_env, pool, period_id, realm_epoch, mdlog, shard_id, marker,
        ...
        yield call(new RGWSimpleRadosWriteCR<rgw_meta_sync_info>(
            dpp, sync_env->store, rgw_raw_obj(pool, sync_env->status_oid()),
...
void
RGWRemoteMetaLog::init_sync_env(RGWMetaSyncEnv* env)
{
  env->dpp = dpp;
  env->cct = store->ctx();
  env->store = store;
...
int
RGWRemoteMetaLog::read_sync_status(
    const DoutPrefixProvider* dpp,
    rgw_meta_sync_status* sync_status)
{
  if (store->svc()->zone->is_meta_master()) {
  ...
  ret = crs.run(
      dpp, new RGWReadSyncStatusCoroutine(&sync_env_local, sync_status));
  http_manager.stop();
  return ret;
}

int
RGWRemoteMetaLog::init_sync_status(const DoutPrefixProvider* dpp)
{
  if (store->svc()->zone->is_meta_master()) {
    return 0;
  }
  ...
  if (r < 0) {
    ldpp_dout(dpp, -1) << "ERROR: fail to fetch master log info (r=" << r << ")"
                       << dendl;
    return r;
  }
...
int
RGWRemoteMetaLog::store_sync_info(
    const DoutPrefixProvider* dpp,
    const rgw_meta_sync_info& sync_info)
{
  tn->log(20, "store sync info");
  return run(
      dpp, new RGWSimpleRadosWriteCR<rgw_meta_sync_info>(
               dpp, store,
               rgw_raw_obj(
                   store->svc()->zone->get_zone_params().log_pool,
                   sync_env.status_oid()),
...
static RGWPeriodHistory::Cursor
get_period_at(
    const DoutPrefixProvider* dpp,
    rgw::sal::RadosStore* store,
    ...
    rgw::sal::ConfigStore* cfgstore)
{
  ...
  auto cursor =
      store->svc()->mdlog->get_period_history()->lookup(info.realm_epoch);
  if (cursor) {
    ...
    if (existing != info.period) {
      ldpp_dout(dpp, -1) << "ERROR: sync status period=" << info.period
                         ...
                         << " in history at realm epoch=" << info.realm_epoch
                         << dendl;
  ...
  RGWPeriod period;
  int r =
      store->svc()->mdlog->pull_period(dpp, info.period, period, y, cfgstore);
  if (r < 0) {
    ldpp_dout(dpp, -1) << "ERROR: failed to read period id " << info.period
                       << ": " << cpp_strerror(r) << dendl;
    return RGWPeriodHistory::Cursor{r};
  }
  // attach the period to our history
  cursor = store->svc()->mdlog->get_period_history()->attach(
      dpp, std::move(period), y, cfgstore);
  if (!cursor) {
    r = cursor.get_error();
    ...
    rgw::sal::ConfigStore* cfgstore)
{
  ...
  do {
    ...
    if (r < 0 && r != -ENOENT) {
      ldpp_dout(dpp, 0) << "ERROR: failed to fetch sync status r=" << r
                        << dendl;
      return r;
    }
    ...
    if (!mdlog_info.period.empty()) {
      ...
      if (sync_status.sync_info.period.empty() ||
        ...
        if (sync_status.sync_info.period.empty()) {
          ...
          reason = SSTR(
              "sync_info realm epoch is behind: "
              << sync_status.sync_info.realm_epoch << " < "
              << mdlog_info.realm_epoch);
        }
        tn->log(1, "initialize sync (reason: " + reason + ")");
        ldpp_dout(dpp, 1)
            << "epoch=" << sync_status.sync_info.realm_epoch
    ...
    if (sync_status.sync_info.state == rgw_meta_sync_info::StateInit) {
      ...
      r =
          run(dpp,
              new RGWInitSyncStatusCoroutine(&sync_env, sync_status.sync_info));
      if (r == -EBUSY) {
        backoff.backoff_sleep();
        continue;
      }
      backoff.reset();
      if (r < 0) {
        ldpp_dout(dpp, 0) << "ERROR: failed to init sync status r=" << r
                          << dendl;
        return r;
      }
    }
  ...
  if (num_shards != mdlog_info.num_shards) {
    ldpp_dout(dpp, -1)
        << "ERROR: can't sync, mismatch between num shards, master num_shards="
  ...
  do {
    ...
    case rgw_meta_sync_info::StateBuildingFullSyncMaps:
      ...
      r =
          run(dpp, new RGWFetchAllMetaCR(
                       &sync_env, num_shards, sync_status.sync_markers, tn));
      if (r == -EBUSY || r == -EIO) {
        backoff.backoff_sleep();
        continue;
      }
      ...
      if (r < 0) {
        tn->log(
            0, SSTR("ERROR: failed to fetch all metadata keys (r=" << r << ")"));
        return r;
      }
...
void
RGWRemoteMetaLog::wakeup(int shard_id)
{
  if (!meta_sync_cr) {
    return;
  }
...
int
RGWCloneMetaLogCoroutine::operate(const DoutPrefixProvider* dpp)
{
  reenter(this)
  {
    do {
      yield
      {
        ldpp_dout(dpp, 20) << __func__ << ": shard_id=" << shard_id
                           << ": init request" << dendl;
        return state_init();
      }
      yield
      ...
      for (tries = 0; tries < NUM_ENPOINT_IOERROR_RETRIES; tries++) {
        ...
        if (op_ret == -ERR_INTERNAL_ERROR &&
            tries < NUM_ENPOINT_IOERROR_RETRIES - 1) {
          ldout(cct, 20) << __func__ << ": request IO error. retries=" << tries
                         << dendl;
          continue;
        } else if (op_ret < 0) {
          return set_cr_error(op_ret);
...
int
RGWCloneMetaLogCoroutine::state_init()
{
...
RGWCloneMetaLogCoroutine::state_read_shard_status()
{
  ...
  if (ret < 0) {
    ldpp_dout(sync_env->dpp, 0)
        << "ERROR: mdlog->get_info_async() returned ret=" << ret << dendl;
    return set_cr_error(ret);
  }
...
int
RGWCloneMetaLogCoroutine::state_read_shard_status_complete()
{
  completion.reset();

  ldpp_dout(sync_env->dpp, 20)
      << "shard_id=" << shard_id << " marker=" << shard_info.marker
...
int
RGWCloneMetaLogCoroutine::state_send_rest_request(const DoutPrefixProvider* dpp)
{
  RGWRESTConn* conn = sync_env->conn;
  ...
  http_op = new RGWRESTReadResource(
      conn, "/admin/log", pairs, NULL, sync_env->http_manager);
...
int
RGWCloneMetaLogCoroutine::state_receive_rest_response()
{
  op_ret = http_op->wait(sync_env->dpp, &data, null_yield);
  if (op_ret < 0 && op_ret != -ERR_INTERNAL_ERROR) {
    error_stream << "http operation failed: " << http_op->to_str()
                 << " status=" << http_op->get_http_status() << std::endl;
    ldpp_dout(sync_env->dpp, 5)
        << "failed to wait for op, ret=" << op_ret << dendl;
    http_op->put();
    http_op = NULL;
    return set_cr_error(op_ret);
  }
  ...
  ldpp_dout(sync_env->dpp, 20)
      << "remote mdlog, shard_id=" << shard_id
...
int
RGWCloneMetaLogCoroutine::state_store_mdlog_entries()
{
  ...
  int ret = mdlog->store_entries_in_shard(
      sync_env->dpp, dest_entries, shard_id, cn->completion());
  if (ret < 0) {
    cn->put();
    ldpp_dout(sync_env->dpp, 10)
        << "failed to store md log entries shard_id=" << shard_id
        << " ret=" << ret << dendl;
    return set_cr_error(ret);
  }
  return io_block(0);
...
int
RGWCloneMetaLogCoroutine::state_store_mdlog_entries_complete()
{
  return set_cr_done();
}

void
rgw_meta_sync_info::decode_json(JSONObj* obj)
{
  string s;
...
void
rgw_meta_sync_info::dump(Formatter* f) const
{
  string s;
...
void
rgw_meta_sync_marker::decode_json(JSONObj* obj)
{
  int s;
...
void
rgw_meta_sync_marker::dump(Formatter* f) const
{
...
void
rgw_meta_sync_status::decode_json(JSONObj* obj)
{
...
void
rgw_meta_sync_status::dump(Formatter* f) const
{
...
void
rgw_sync_error_info::dump(Formatter* f) const
{
