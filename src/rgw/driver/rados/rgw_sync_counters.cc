...
build(CephContext* cct, const std::string& name)
{
  ...
  b.add_u64_counter(
      l_fetch_not_modified, "fetch_not_modified",
      "Number of objects already replicated");
  b.add_u64_counter(
      l_fetch_err, "fetch_errors", "Number of object replication errors");

  b.add_time_avg(
      l_poll, "poll_latency", "Average latency of replication log requests");
  b.add_u64_counter(
      l_poll_err, "poll_errors", "Number of replication log request errors");
...
void
add_rgw_sync_delta_counters(PerfCountersBuilder* lpcb)
{
  lpcb->set_prio_default(PerfCountersBuilder::PRIO_USEFUL);
  lpcb->add_time(
      l_rgw_datalog_sync_delta, "sync_delta",
      "Sync delta between data log shard in seconds");
}
  ...
  cct(cct)
{
  ...
  PerfCountersBuilder pcb(
      cct, name, l_rgw_sync_delta_first, l_rgw_sync_delta_last);
  add_rgw_sync_delta_counters(&pcb);
  sync_delta_counters =
      std::unique_ptr<PerfCounters>(pcb.create_perf_counters());
  cct->get_perfcounters_collection()->add(sync_delta_counters.get());
}

void
SyncDeltaCountersManager::tset(int idx, ceph::timespan v)
{
