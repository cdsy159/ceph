...
public:
  ...
  virtual void
  init(RGWDataSyncCtx* sync_env, uint64_t instance_id)
  {}

  virtual RGWCoroutine*
  init_sync(const DoutPrefixProvider* dpp, RGWDataSyncCtx* sc)
  {
    return nullptr;
  }

  virtual RGWCoroutine*
  start_sync(const DoutPrefixProvider* dpp, RGWDataSyncCtx* sc)
  {
    return nullptr;
  }

  virtual RGWCoroutine* sync_object(
  ...
  virtual RGWCoroutine* remove_object(
      const DoutPrefixProvider* dpp,
      RGWDataSyncCtx* sc,
      rgw_bucket_sync_pipe& bucket_info,
      rgw_obj_key& key,
      real_time& mtime,
      bool versioned,
      uint64_t versioned_epoch,
      rgw_zone_set* zones_trace) = 0;
  virtual RGWCoroutine* create_delete_marker(
      const DoutPrefixProvider* dpp,
      RGWDataSyncCtx* sc,
      rgw_bucket_sync_pipe& bucket_info,
      rgw_obj_key& key,
...
public:
  ...
  virtual RGWRESTMgr*
  get_rest_filter(int dialect, RGWRESTMgr* orig)
  {
    return orig;
  }

  virtual bool
  supports_user_writes()
  {
    return false;
  }

  virtual auto alloc_bucket_meta_handler(
      librados::Rados& rados,
      RGWSI_Bucket* svc_bucket,
      RGWBucketCtl* ctl_bucket) -> std::unique_ptr<RGWMetadataHandler>;
  virtual auto alloc_bucket_instance_meta_handler(
...
public:
  ...
  bool
  get_module(const std::string& name, RGWSyncModuleRef* module)
  {
    std::lock_guard l{lock};
  ...
  bool
  supports_data_export(const std::string& name)
  {
    RGWSyncModuleRef module;
