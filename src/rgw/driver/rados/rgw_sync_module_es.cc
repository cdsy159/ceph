...
public:
  ...
  void
  init(const string& str, bool def_val)
  {
    if (str.empty()) {
      approve_all = def_val;
    } else {
...
void
ESInfo::decode_json(JSONObj* obj)
{
...
struct ElasticConfig {
  ...
  std::map<string, string> default_headers = {
      {"Content-Type", "application/json"}};
  ESInfo es_info;

  void
  init(CephContext* cct, const JSONFormattable& config)
  {
    string elastic_endpoint = config["endpoint"];
    id = string("elastic:") + elastic_endpoint;
    conn.reset(new RGWRESTConn(
        cct, (rgw::sal::Driver*)nullptr, id, {elastic_endpoint},
        nullopt /* region */));
    explicit_custom_meta = config["explicit_custom_meta"](true);
    index_buckets.init(
        config["index_buckets_list"], true); /* approve all buckets by default */
    allow_owners.init(
        config["approved_owners_list"],
        true); /* approve all bucket owners by default */
    ...
    if (string user = config["username"], pw = config["password"];
      ...
      default_headers.emplace(
          "AUTHORIZATION", "Basic " + rgw::to_base64(auth_string));
    }
  }

  void
  init_instance(const RGWRealm& realm, uint64_t instance_id)
  {
  ...
  string
  get_obj_path(const RGWBucketInfo& bucket_info, const rgw_obj_key& key)
  {
    if (es_info.version >= ES_V7) {
      return index_path + "/_doc/" +
             url_encode(
                 bucket_info.bucket.bucket_id + ":" + key.name + ":" +
  ...
  bool
  should_handle_operation(RGWBucketInfo& bucket_info)
  {
...
es_type_to_str(const ESType& t)
{
  ...
  case ESType::Float:
    return "float";
  case ESType::Half_Float:
    return "half_float";
  case ESType::Scaled_Float:
    return "scaled_float";
  case ESType::Date:
    return "date";
  case ESType::Boolean:
    return "boolean";
  case ESType::Integer_Range:
    return "integer_range";
  case ESType::Float_Range:
    return "float_range";
  case ESType::Double_Range:
    return "date_range";
  case ESType::Date_Range:
    return "date_range";
  case ESType::Geo_Point:
    return "geo_point";
  case ESType::Ip:
    return "ip";
  default:
...
struct es_type_v2 {
  ...
  dump(Formatter* f) const
  {
    ...
    if (is_analyzed) {
      encode_json(
          "index", (is_analyzed.value() ? "analyzed" : "not_analyzed"), f);
    }
  }
};
...
struct es_index_mappings {
  ...
  void
  dump_custom(const char* section, ESType type, const char* format, Formatter* f)
      const
  {
  ...
  dump(Formatter* f) const
  {
    ...
    encode_json(
        "mtime",
        est(ESType::Date).set_format("strict_date_optional_time||epoch_millis"),
        f);
    ...
    dump_custom(
        "custom-date", ESType::Date, "strict_date_optional_time||epoch_millis",
        f);
...
struct es_index_settings {
  ...
  es_index_settings(uint32_t _replicas, uint32_t _shards) :
    num_replicas(_replicas), num_shards(_shards)
  {}
...
static bool
is_sys_attr(const std::string& attr_name)
{
  static constexpr std::initializer_list<const char*> rgw_sys_attrs = {
      RGW_ATTR_PG_VER,       RGW_ATTR_SOURCE_ZONE,  RGW_ATTR_ID_TAG,
      RGW_ATTR_TEMPURL_KEY1, RGW_ATTR_TEMPURL_KEY2, RGW_ATTR_UNIX1,
      RGW_ATTR_UNIX_KEY1};

  return std::find(rgw_sys_attrs.begin(), rgw_sys_attrs.end(), attr_name) !=
         rgw_sys_attrs.end();
}

static size_t
attr_len(const bufferlist& val)
{
  ...
  dump(Formatter* f) const
  {
    ...
    for (auto i : attrs) {
      ...
      if (boost::algorithm::starts_with(attr_name, RGW_ATTR_META_PREFIX)) {
        custom_meta.emplace(
            attr_name.substr(sizeof(RGW_ATTR_META_PREFIX) - 1),
      ...
      if (attr_name == RGW_ATTR_ACL) {
        try {
          ...
          ldpp_dout(dpp, 0) << "ERROR: failed to decode acl for "
        ...
        for (const auto& acliter : acl.get_grant_map()) {
          ...
          if (user &&
              (grant.get_permission().get_permissions() & RGW_PERM_READ) != 0) {
        ...
        if (!is_sys_attr(attr_name)) {
          out_attrs.emplace(
              attr_name.substr(sizeof(RGW_ATTR_PREFIX) - 1),
      ...
      if (config == bucket_info.mdsearch_config.end()) {
        if (!es_conf->explicit_custom_meta) {
          ...
          ldpp_dout(dpp, 20) << "custom meta entry key=" << i.first
                             << " not found in bucket mdsearch config: "
                             << bucket_info.mdsearch_config << dendl;
        }
        continue;
      }
    ...
    if (!custom_date.empty()) {
      ...
      for (auto i : custom_date) {
        ...
        if (r < 0) {
          ldpp_dout(dpp, 20)
              << __func__ << "(): failed to parse time (" << i.second
...
public:
  RGWElasticGetESInfoCBCR(RGWDataSyncCtx* _sc, ElasticConfigRef _conf) :
    RGWCoroutine(_sc->cct), sc(_sc), sync_env(_sc->env), conf(_conf)
  {}
  ...
  operate(const DoutPrefixProvider* dpp) override
  {
    reenter(this)
    {
      ldpp_dout(dpp, 5) << conf->id << ": get elasticsearch info for zone: "
                        << sc->source_zone << dendl;
      yield call(new RGWReadRESTResourceCR<ESInfo>(
          sync_env->cct, conf->conn.get(), sync_env->http_manager, "/",
          nullptr /*params*/, &(conf->default_headers), &(conf->es_info)));
      if (retcode < 0) {
        ldpp_dout(dpp, 5) << conf->id
                          << ": get elasticsearch failed: " << retcode << dendl;
        return set_cr_error(retcode);
      }

      ldpp_dout(dpp, 5) << conf->id << ": got elastic version="
                        << conf->es_info.get_version_str() << dendl;
      return set_cr_done();
    }
    return 0;
  }
...
public:
  ...
  operate(const DoutPrefixProvider* dpp) override
  {
    reenter(this)
    {
      ldpp_dout(dpp, 5) << conf->id << ": put elasticsearch index for zone: "
                        << sc->source_zone << dendl;
      ...
      yield
      {
        ...
        if (conf->es_info.version >= ES_V5) {
          ldpp_dout(dpp, 0)
              << "elasticsearch: index mapping: version >= 5" << dendl;
          index_conf.reset(
              new es_index_config<es_type_v5>(settings, conf->es_info.version));
        } else {
          ldpp_dout(dpp, 0)
              << "elasticsearch: index mapping: version < 5" << dendl;
          index_conf.reset(
              new es_index_config<es_type_v2>(settings, conf->es_info.version));
        }
        call(new RGWPutRESTResourceCR<es_index_config_base, int, _err_response>(
            sc->cct, conf->conn.get(), sync_env->http_manager, path,
            nullptr /*params*/, &(conf->default_headers), *index_conf, nullptr,
            &err_response));
      }
      if (retcode < 0) {
        ...
        if (err_response.error.type != "index_already_exists_exception" &&
          ...
          ldpp_dout(dpp, 0)
              << "elasticsearch: failed to initialize index: response.type="
              << err_response.error.type
              << " response.reason=" << err_response.error.reason << dendl;
          return set_cr_error(retcode);
        }

        ldpp_dout(dpp, 0) << "elasticsearch: index already exists, assuming "
...
public:
  RGWElasticHandleRemoteObjCBCR(
      RGWDataSyncCtx* _sc,
      rgw_bucket_sync_pipe& _sync_pipe,
      rgw_obj_key& _key,
      ElasticConfigRef _conf,
      uint64_t _versioned_epoch) :
  ...
  operate(const DoutPrefixProvider* dpp) override
  {
    reenter(this)
    {
      ldpp_dout(dpp, 10) << ": stat of remote obj: z=" << sc->source_zone
                         << " b=" << sync_pipe.info.source_bs.bucket
                         << " k=" << key << " size=" << size
                         << " mtime=" << mtime << dendl;
      ...
      yield
      {
        ...
        es_obj_metadata doc(
            sync_env->cct, conf, sync_pipe.dest_bucket_info, key, mtime, size,
            attrs, versioned_epoch);

        call(new RGWPutRESTResourceCR<es_obj_metadata, int>(
            sync_env->cct, conf->conn.get(), sync_env->http_manager, path,
            nullptr /* params */, &(conf->default_headers), doc,
            nullptr /* result */));
      }
      if (retcode < 0) {
        return set_cr_error(retcode);
...
public:
  ...
  void
  init(RGWDataSyncCtx* sc, uint64_t instance_id) override
  {
    conf->init_instance(sc->env->svc->zone->get_realm(), instance_id);
  }

  RGWCoroutine*
  init_sync(const DoutPrefixProvider* dpp, RGWDataSyncCtx* sc) override
  {
  ...
  RGWCoroutine*
  start_sync(const DoutPrefixProvider* dpp, RGWDataSyncCtx* sc) override
  {
  ...
  {
    ldpp_dout(dpp, 10) << conf->id
                       << ": sync_object: b=" << sync_pipe.info.source_bs.bucket
                       << " k=" << key
                       << " versioned_epoch=" << versioned_epoch.value_or(0)
                       << dendl;
    if (!conf->should_handle_operation(sync_pipe.dest_bucket_info)) {
      ldpp_dout(dpp, 10) << conf->id
                         << ": skipping operation (bucket not approved)"
                         << dendl;
      return nullptr;
    }
    return new RGWElasticHandleRemoteObjCR(
        sc, sync_pipe, key, conf, versioned_epoch.value_or(0));
  }
      ...
      rgw_zone_set* zones_trace) override
  {
    ...
    ldpp_dout(dpp, 10) << conf->id
                       << ": rm_object: b=" << sync_pipe.info.source_bs.bucket
                       << " k=" << key << " mtime=" << mtime
                       << " versioned=" << versioned
  ...
  create_delete_marker(
      ...
      rgw_zone_set* zones_trace) override
  {
    ldpp_dout(dpp, 10) << conf->id << ": create_delete_marker: b="
                       << sync_pipe.info.source_bs.bucket << " k=" << key
                       << " mtime=" << mtime << " versioned=" << versioned
                       << " versioned_epoch=" << versioned_epoch << dendl;
    ldpp_dout(dpp, 10) << conf->id << ": skipping operation (not handled)"
                       << dendl;
    return NULL;
  }
...
RGWElasticSyncModuleInstance::RGWElasticSyncModuleInstance(
    const DoutPrefixProvider* dpp,
    CephContext* cct,
    const JSONFormattable& config)
{
  data_handler = std::unique_ptr<RGWElasticDataSyncModule>(
      new RGWElasticDataSyncModule(dpp, cct, config));
}
...
RGWRESTMgr*
RGWElasticSyncModuleInstance::get_rest_filter(int dialect, RGWRESTMgr* orig)
{
