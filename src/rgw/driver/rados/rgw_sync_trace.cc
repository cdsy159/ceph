...
RGWSyncTraceNode::RGWSyncTraceNode(
    CephContext* _cct,
    uint64_t _handle,
...
void
RGWSyncTraceNode::log(int level, const string& s)
{
  status = s;
  history.push_back(status);
  /* dump output on either rgw_sync, or rgw -- but only once */
  if (cct->_conf->subsys.should_gather(ceph_subsys_rgw_sync, level)) {
    lsubdout(cct, rgw_sync, ceph::dout::need_dynamic(level))
        << "RGW-SYNC:" << to_str() << dendl;
  } else {
    lsubdout(cct, rgw, ceph::dout::need_dynamic(level))
        << "RGW-SYNC:" << to_str() << dendl;
  }
}
...
int
RGWSyncTraceServiceMapThread::process(const DoutPrefixProvider* dpp)
{
  map<string, string> status;
  status["current_sync"] = manager->get_active_names();
  int ret = store->update_service_map(dpp, std::move(status));
  if (ret < 0) {
    ldout(store->ctx(), 0) << "ERROR: update_service_map() returned ret=" << ret
                           << dendl;
  }
  return 0;
}
...
bool
RGWSyncTraceNode::match(const string& search_term, bool search_history)
{
  try {
    ...
    ldout(cct, 5) << "NOTICE: sync trace: bad expression: bad regex search term"
                  << dendl;
  }
...
void
RGWSyncTraceManager::init(RGWRados* store)
{
...
int
RGWSyncTraceManager::hook_to_admin_command()
{
  AdminSocket* admin_socket = cct->get_admin_socket();

  admin_commands = {
      {"sync trace show name=search,type=CephString,req=false",
       "sync trace show [filter_str]: show current multisite tracing "
       "information"},
      {"sync trace history name=search,type=CephString,req=false",
       "sync trace history [filter_str]: show history of multisite tracing "
       "information"},
...
static void
dump_node(RGWSyncTraceNode* entry, bool show_history, Formatter* f)
{
...
void
RGWSyncTraceManager::finish_node(RGWSyncTraceNode* node)
{
