...
static inline int
rgw_shards_mod(unsigned hval, int max_shards)
{
  if (max_shards <= 0) {
    return -1;
...
inline int
rgw_shard_id(const std::string& key, int max_shards)
{
  return rgw_shards_mod(
      ceph_str_hash_linux(key.c_str(), key.size()), max_shards);
}
...
int rgw_rados_operate(
    ...
    optional_yield y,
    int flags = 0,
    const jspan_context* trace_info = nullptr,
...
struct rgw_rados_ref {
  ...
  int
  aio_operate(librados::AioCompletion* c, librados::ObjectWriteOperation* op)
  {
  ...
  void
  notify_ack(uint64_t notify_id, uint64_t cookie, bufferlist& bl)
  {
...
inline std::ostream&
operator<<(std::ostream& m, const rgw_rados_ref& ref)
{
  return m << ref.obj;
}

int rgw_get_rados_ref(
    const DoutPrefixProvider* dpp,
    librados::Rados* rados,
    rgw_raw_obj obj,
    rgw_rados_ref* ref);
    ...
    CompletionToken&& token)
{
  ...
  return asio::async_initiate<CompletionToken, void(sys::error_code)>(
      asio::experimental::co_composed<void(sys::error_code)>(
          [](auto state, const DoutPrefixProvider* dpp, R::RADOS& rados,
            ...
            try {
              ...
              try {
                ...
                std::vector<std::string> biascommand{{fmt::format(
                    R"({{"prefix": "osd pool set", "pool": "{}", )"
                    R"("var": "pg_autoscale_bias", "val": "{}"}})",
                    name, bias)}};
                co_await rados.mon_command(
                    std::move(biascommand), {}, nullptr, nullptr,
                    asio::deferred);
              } catch (const std::exception& e) {
                ldpp_dout(dpp, 2)
                    << __PRETTY_FUNCTION__ << ":" << __LINE__ << ": "
                    << "mon_command to set "
                    << "autoscale bias failed with error: " << e.what()
                    << dendl;
                throw;
              }
              try {
                ...
                std::vector<std::string> recoverycommand{{fmt::format(
                    R"({{"prefix": "osd pool set", "pool": "{}", )"
                    R"("var": "recovery_priority": "{}"}})",
                    name, p)}};
                co_await rados.mon_command(
                    std::move(recoverycommand), {}, nullptr, nullptr,
                    asio::deferred);
              } catch (const std::exception& e) {
                ldpp_dout(dpp, 2)
                    << __PRETTY_FUNCTION__ << ":" << __LINE__
      ...
      asio::experimental::co_composed<void(sys::error_code)>(
          [](auto state, const DoutPrefixProvider* dpp, R::RADOS& rados,
            ...
            try {
              state.throw_if_cancelled(true);
              try {
                co_await rados.create_pool(
                    std::string(name), std::nullopt, asio::deferred);
              } catch (const sys::system_error& e) {
                if (e.code() == sys::errc::result_out_of_range) {
                  ...
                  ldpp_dout(dpp, 2)
                      << __PRETTY_FUNCTION__ << ":" << __LINE__ << ": "
                      << "RADOS::create_pool failed with error: " << e.what()
                      << dendl;
                  throw;
                }
              } catch (const std::exception& e) {
              ...
              co_await rados.enable_application(
                  std::move(name), pg_pool_t::APPLICATION_NAME_RGW, false,
                  asio::deferred);
              co_return sys::error_code{};
            } catch (const std::system_error& e) {
              ldpp_dout(dpp, 2) << __PRETTY_FUNCTION__ << ":" << __LINE__
                                << ": " << "ERROR: " << e.what() << dendl;
              co_return e.code();
            } catch (const std::exception& e) {
              ldpp_dout(dpp, 2) << __PRETTY_FUNCTION__ << ":" << __LINE__
      ...
      asio::experimental::co_composed<void(sys::error_code)>(
          [](auto state, const DoutPrefixProvider* dpp, R::RADOS& rados,
            ...
            try {
              ...
              co_await set_mostly_omap(
                  dpp, rados, std::move(name), asio::deferred);
              co_return sys::error_code{};
            } catch (const std::system_error& e) {
              ldpp_dout(dpp, 2) << __PRETTY_FUNCTION__ << ":" << __LINE__
                                << ": " << "ERROR: " << e.what() << dendl;
    ...
    CompletionToken&& token)
{
  ...
  return asio::async_initiate<
      CompletionToken, void(sys::error_code, R::IOContext)>(
      asio::experimental::co_composed<void(sys::error_code, R::IOContext)>(
          [](auto state, const DoutPrefixProvider* dpp, R::RADOS& rados,
            ...
            try {
              ...
              try {
                ioc.set_pool(
                    co_await rados.lookup_pool(pool.name, asio::deferred));
              } catch (const std::exception& e) {
                ldpp_dout(dpp, 2)
                    << __PRETTY_FUNCTION__ << ":" << __LINE__ << ": "
                    << "RADOS::lookup_pool failed with error: " << e.what()
                    << dendl;
                throw;
              }
              ioc.set_ns(pool.ns);
              ...
              ldpp_dout(dpp, 2) << __PRETTY_FUNCTION__ << ":" << __LINE__
                                << ": " << "ERROR: " << e.what() << dendl;
              co_return {
                  sys::error_code{EIO, sys::generic_category()}, R::IOContext{}};
            }
          },
          rados.get_executor()),
  ...
  return asio::async_initiate<
      ...
      asio::experimental::co_composed<void(sys::error_code, R::IOContext)>(
          [](auto state, const DoutPrefixProvider* dpp, R::RADOS& rados,
            ...
            try {
              state.throw_if_cancelled(true);
              auto ioc = co_await init_iocontext(
                  dpp, rados, pool, create, asio::deferred);
...
init_iocontext(
    ...
    CompletionToken&& token)
{
  return init_iocontext(
      dpp, rados, pool, create, mostly_omap,
