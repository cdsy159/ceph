...
std::ostream&
operator<<(std::ostream& out, const TrimCounters::BucketCounter& rhs)
{
  return out << rhs.bucket << ":" << rhs.count;
}

void
TrimCounters::BucketCounter::encode(bufferlist& bl) const
{
  using ceph::encode;
...
void
TrimCounters::BucketCounter::decode(bufferlist::const_iterator& p)
{
  using ceph::decode;
  decode(bucket, p);
  decode(count, p);
...
void
TrimCounters::Request::encode(bufferlist& bl) const
{
  ENCODE_START(1, 1, bl);
...
void
TrimCounters::Request::decode(bufferlist::const_iterator& p)
{
  DECODE_START(1, p);
...
void
TrimCounters::Response::encode(bufferlist& bl) const
{
  ENCODE_START(1, 1, bl);
...
void
TrimCounters::Response::decode(bufferlist::const_iterator& p)
{
  DECODE_START(1, p);
...
void
TrimCounters::Handler::handle(
    bufferlist::const_iterator& input,
    bufferlist& output)
{
  Request request;
...
void
TrimComplete::Request::encode(bufferlist& bl) const
{
  ENCODE_START(1, 1, bl);
  ENCODE_FINISH(bl);
}
...
void
TrimComplete::Request::decode(bufferlist::const_iterator& p)
{
  DECODE_START(1, p);
  DECODE_FINISH(p);
}
...
void
TrimComplete::Response::encode(bufferlist& bl) const
{
  ENCODE_START(1, 1, bl);
  ENCODE_FINISH(bl);
}
...
void
TrimComplete::Response::decode(bufferlist::const_iterator& p)
{
  DECODE_START(1, p);
  DECODE_FINISH(p);
}
...
public:
  ...
  {
    handlers.emplace(
        NotifyTrimCounters, std::make_unique<TrimCounters::Handler>(counters));
    handlers.emplace(
        NotifyTrimComplete, std::make_unique<TrimComplete::Handler>(counters));
  }

  ~BucketTrimWatcher() { stop(); }

  int
  start(const DoutPrefixProvider* dpp)
  {
  ...
  restart()
  {
    ...
    if (r < 0) {
      lderr(store->ctx()) << "Failed to unwatch on " << ref.obj << " with "
                          << cpp_strerror(-r) << dendl;
    }
    r = ref.ioctx.watch2(ref.obj.oid, &handle, this);
    if (r < 0) {
      ...
      bufferlist& bl) override
  {
    ...
    try {
      ...
      lderr(store->ctx()) << "Failed to decode notification: " << e.what()
                          << dendl;
    }
...
class BucketTrimShardCollectCR : public RGWShardCollectCR {
  ...
  handle_result(int r) override
  {
    ...
    if (r < 0) {
      ldout(cct, 4) << "failed to trim bilog shard: " << cpp_strerror(r)
                    << dendl;
    }
    return r;
  }
...
bool
BucketTrimShardCollectCR::spawn_next()
{
  while (i < markers.size()) {
    ...
    // skip empty markers
    if (!marker.empty()) {
      ldpp_dout(dpp, 10) << "trimming bilog shard " << shard_id << " of "
                         << bucket_info.bucket << " at marker " << marker
                         << dendl;
      spawn(
          new RGWRadosBILogTrimCR(
              dpp, store, bucket_info, shard_id, generation, std::string{},
              marker),
          false);
      return true;
    }
...
public:
  ...
  spawn_next() override
  {
    if (shard < num_shards) {
      ...
      spawn(
          new RGWRadosRemoveOidCR(store, std::move(bs.bucket_obj), nullptr),
          false);
      ++shard;
      return true;
    } else {
    ...
    reenter(this)
    {
      yield
      {
        ...
        if (ziter == zone_conn_map.end()) {
          ldpp_dout(dpp, 0)
              << "WARNING: no connection to zone " << zid
              << ", can't trim bucket: " << bucket_instance << dendl;
        ...
        call(new RGWReadRESTResourceCR<StatusShards>(
            cct, ziter->second, http, "/admin/log/", params, p));
      }

      if (retcode < 0 && retcode != -ENOENT) {
        return set_cr_error(retcode);
      } else if (
          retcode == -ENOENT && bucket_info->layout.logs.front().layout.type ==
                                    rgw::BucketLogType::Deleted) {
        p->generation = UINT64_MAX;
        ldpp_dout(dpp, 10) << "INFO: could not read shard status for bucket:"
                           << bucket_instance << " from zone: " << zid.id
                           << dendl;
      }
...
class BucketTrimInstanceCR : public RGWCoroutine {
  ...
  const RGWBucketInfo*
      pbucket_info; //< pointer to bucket instance info to locate bucket indices
  int child_ret = 0;
  const DoutPrefixProvider* dpp;
  ...
  std::optional<std::pair<RGWBucketInfo, rgw::bucket_log_layout_generation>>
      clean_info;
  ...
  take_min_generation()
  {
    ...
    if (auto m = std::min_element(
            peer_status.begin(), peer_status.end(),
            [](const StatusShards& l, const StatusShards& r) {
    ...
    auto log = std::find_if(
        logs.begin(), logs.end(), rgw::matches_gen(min_generation));
    if (log == logs.end()) {
  ...
  int
  maybe_remove_generation()
  {
    if (clean_info)
      return 0;

    bool deleted_type =
        (pbucket_info->layout.logs.back().layout.type ==
         rgw::BucketLogType::Deleted);
  ...
  for (auto peer = first; peer != last; ++peer) {
    ...
    if (peer->shards.size() != status->size()) {
      ldpp_dout(dpp, 5) << __PRETTY_FUNCTION__ << ":"
                        << "ERROR: shards don't match. peer shard:"
                        << peer->shards.size()
                        << " my shards:" << status->size()
...
template <>
inline int
parse_decode_json<StatusShards>(StatusShards& s, bufferlist& bl)
{
  JSONParser p;
...
int
BucketTrimInstanceCR::operate(const DoutPrefixProvider* dpp)
{
  reenter(this)
  {
    ...
    yield call(new RGWBucketGetSyncPolicyHandlerCR(
        store->svc()->async_processor, store, get_policy_params, source_policy,
        dpp));
    if (retcode < 0) {
      if (retcode != -ENOENT) {
        ldpp_dout(dpp, 0) << "ERROR: failed to fetch policy handler for bucket="
                          << bucket << dendl;
      }
    ...
    yield
    {
      ...
      for (auto& zid : zids) {
        spawn(
            new RGWReadRemoteStatusShardsCR(
                dpp, store, cct, http, pbucket_info, bucket_instance, zid,
                zone_id, &*p),
            false);
        ++p;
      }
    ...
    if (clean_info) {
      if (clean_info->second.layout.type != rgw::BucketLogType::InIndex) {
        ldpp_dout(dpp, 0) << "Unable to convert log of unknown type "
                          ...
                          << " to rgw::bucket_index_layout_generation "
                          << dendl;
        return set_cr_error(-EINVAL);
      }

      yield call(new BucketCleanIndexCollectCR(
          dpp, store, clean_info->first, clean_info->second.layout.in_index));
      if (retcode < 0) {
      ...
      while (clean_info && retries < MAX_RETRIES) {
        yield call(new RGWPutBucketInstanceInfoCR(
            store->svc()->async_processor, store, clean_info->first, false, {},
            no_change_attrs(), dpp));

        // Raced, try again.
        if (retcode == -ECANCELED) {
          yield call(new RGWGetBucketInstanceInfoCR(
              store->svc()->async_processor, store, clean_info->first.bucket,
              &(clean_info->first), nullptr, dpp));
          if (retcode < 0) {
            ldpp_dout(dpp, 0)
                << "failed to get bucket info: " << cpp_strerror(retcode)
                << dendl;
            return set_cr_error(retcode);
          }
        ...
        if (retcode < 0) {
          ldpp_dout(dpp, 0)
              << "failed to put bucket info: " << cpp_strerror(retcode)
              << dendl;
          return set_cr_error(retcode);
        }

        //remove bucket instance metadata
        if (clean_info->first.layout.logs.front().layout.type ==
            rgw::BucketLogType::Deleted) {
      ...
      min_markers.assign(
          std::max(1u, rgw::num_shards(totrim.layout.in_index)),
          RGWSyncLogTrimCR::max_marker);

      retcode = take_min_status(
          cct, totrim.gen, peer_status.cbegin(), peer_status.cend(),
          &min_markers, dpp);
      if (retcode < 0) {
        ldpp_dout(dpp, 4) << "failed to correlate bucket sync status from peers"
                          << dendl;
        return set_cr_error(retcode);
      }

      // trim shards with a ShardCollectCR
      ldpp_dout(dpp, 10) << "trimming bilogs for bucket="
                         << pbucket_info->bucket << " markers=" << min_markers
                         << ", shards=" << min_markers.size() << dendl;
      set_status("trimming bilog shards");
      yield call(new BucketTrimShardCollectCR(
          dpp, store, *pbucket_info, totrim.layout.in_index, min_markers));
      if (retcode == -ENOENT) {
...
class BucketTrimInstanceCollectCR : public RGWShardCollectCR {
  ...
  handle_result(int r) override
  {
    ...
    if (r < 0) {
      ldout(cct, 4) << "failed to trim bucket instance: " << cpp_strerror(r)
                    << dendl;
    }
    return r;
  }
...
bool
BucketTrimInstanceCollectCR::spawn_next()
{
  if (bucket == end) {
    return false;
  }
...
int
accumulate_peer_counters(bufferlist& bl, BucketChangeCounter& counter)
{
  counter.clear();
...
int
AsyncMetadataList::_send_request(const DoutPrefixProvider* dpp)
{
  void* handle = nullptr;
  ...
  if (r == -EINVAL) {
    ...
    ldpp_dout(dpp, 10) << "failed to init metadata listing: " << cpp_strerror(r)
                       << dendl;
    return r;
  } else {
    ldpp_dout(dpp, 20) << "starting metadata listing at " << start_marker
                       << dendl;
    ...
    do {
      ...
      if (r < 0) {
        ldpp_dout(dpp, 10) << "failed to list metadata: " << cpp_strerror(r)
                           << dendl;
        return r;
      }
...
public:
  ...
  {
    req = new AsyncMetadataList(
        cct, this, stack->create_completion_notifier(), mgr, section,
...
int
BucketTrimCR::operate(const DoutPrefixProvider* dpp)
{
  reenter(this)
  {
    ...
    if (config.buckets_per_interval) {
      ...
      yield
      {
        ...
        encode(type, bl);
        encode(request, bl);
        call(new RGWRadosNotifyCR(
            store, obj, bl, config.notify_timeout_ms, &notify_replies));
      }
      if (retcode < 0) {
      ...
      counter.get_highest(
          max_count, [this](const std::string& bucket, int count) {
    ...
    if (buckets.size() < config.buckets_per_interval) {
      ...
      ldpp_dout(dpp, 10) << "listing cold buckets from marker=" << status.marker
                         << dendl;
      ...
      yield
      {
        ...
        call(new MetadataListCR(
            cct, store->svc()->async_processor, store->ctl()->meta.mgr, section,
            status.marker, cb));
      }
      if (retcode < 0) {
    ...
    ldpp_dout(dpp, 4) << "collected " << buckets.size() << " buckets for trim"
                      << dendl;
    yield call(new BucketTrimInstanceCollectCR(
        store, http, observer, buckets, config.concurrent_buckets, dpp));
    ...
    if (!last_cold_marker.empty() && status.marker != last_cold_marker) {
      ...
      ldpp_dout(dpp, 20) << "writing bucket trim marker=" << status.marker
                         << dendl;
    ...
    yield
    {
      ...
      encode(request, bl);
      call(new RGWRadosNotifyCR(
          store, obj, bl, config.notify_timeout_ms, nullptr));
    }
    if (retcode < 0) {
...
int
BucketTrimPollCR::operate(const DoutPrefixProvider* dpp)
{
  reenter(this)
  {
    for (;;) {
      ...
      yield call(new RGWSimpleRadosLockCR(
          store->svc()->async_processor, store, obj, name, cookie,
...
public:
  ...
  void
  expire_old(const time_point& now)
  {
...
void
configure_bucket_trim(CephContext* cct, BucketTrimConfig& config)
{
...
public:
  ...
  void
  get_bucket_counters(int count, TrimCounters::Vector& buckets)
  {
  ...
  void
  reset_bucket_counters() override
  {
  ...
  void
  on_bucket_trimmed(std::string&& bucket_instance) override
  {
    ldout(store->ctx(), 20)
        << "trimmed bucket instance " << bucket_instance << dendl;
    std::lock_guard<std::mutex> lock(mutex);
    trimmed.insert(std::move(bucket_instance), clock_type::now());
  }

  bool
  trimmed_recently(const std::string_view& bucket_instance) override
  {
    std::lock_guard<std::mutex> lock(mutex);
...
BucketTrimManager::BucketTrimManager(
    rgw::sal::RadosStore* store,
    const BucketTrimConfig& config) :
...
int
BucketTrimManager::init()
{
  return impl->watcher.start(this);
}

void
BucketTrimManager::on_bucket_changed(const std::string_view& bucket)
{
...
RGWCoroutine*
BucketTrimManager::create_bucket_trim_cr(RGWHTTPManager* http)
{
  return new BucketTrimPollCR(
      impl->store, http, impl->config, impl.get(), impl->status_obj, this);
}

RGWCoroutine*
BucketTrimManager::create_admin_bucket_trim_cr(RGWHTTPManager* http)
{
  // return the trim coroutine without any polling
  return new BucketTrimCR(
      impl->store, http, impl->config, impl.get(), impl->status_obj, this);
}

CephContext*
BucketTrimManager::get_cct() const
{
  return impl->store->ctx();
}

unsigned
BucketTrimManager::get_subsys() const
{
  return dout_subsys;
}

std::ostream&
BucketTrimManager::gen_prefix(std::ostream& out) const
{
    ...
    std::string_view end_marker)
{
  ...
  auto r = store->svc()->bilog_rados->log_trim(
      p, y, bucket_info, log_layout, shard_id, start_marker, end_marker);
  if (r < 0) {
    ldpp_dout(p, 5) << __PRETTY_FUNCTION__ << ":" << __LINE__
