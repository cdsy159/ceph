...
public:
  ...
  send_request(const DoutPrefixProvider* dpp) override
  {
    ...
    store->svc()->datalog_rados->trim_entries(
        dpp, shard, marker, cn->completion());
    return 0;
  }
...
const std::string&
get_stable_marker(const rgw_data_sync_marker& m)
{
...
void
take_min_markers(IterIn first, IterIn last, IterOut dest)
{
  if (first == last) {
    return;
  }
...
int
DataLogTrimCR::operate(const DoutPrefixProvider* dpp)
{
  reenter(this)
  {
    ...
    yield
    {
      ...
      for (auto& c : store->svc()->zone->get_zone_data_notify_to_map()) {
        ...
        spawn(
            new StatusCR(cct, c.second, http, "/admin/log/", params, &*p),
            false);
        ++p;
      }
    }
    ...
    yield
    {
      ...
      take_min_markers(
          peer_status.begin(), peer_status.end(), min_shard_markers.begin());
      ...
      for (int i = 0; i < num_shards; i++) {
        ...
        ldpp_dout(dpp, 10) << "trimming log shard " << i << " at marker=" << m
...
int
DataLogTrimPollCR::operate(const DoutPrefixProvider* dpp)
{
  reenter(this)
  {
    for (;;) {
      ...
      yield call(new RGWSimpleRadosLockCR(
          store->svc()->async_processor, store,
      ...
      if (retcode < 0) {
        ...
        ldpp_dout(dpp, 4) << "failed to lock " << lock_oid
                          << ", trying again in " << interval.sec() << "s"
                          << dendl;
        continue;
      }
