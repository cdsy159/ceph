...
public:
  ...
  virtual Result
  on_complete(int shard, error_code ec)
  {
...
inline auto
make_shards(const std::map<int, std::string>& objects) -> std::vector<Shard>
{
...
struct RevertibleWriteHandler {
  ...
  void
  operator()(error_code ec, version_t = {})
  {
      ...
      boost::asio::co_composed<void(error_code)>(
          [](auto state, RevertibleWriter& writer,
            ...
            for (;;) {
              while (!sending.empty() && !terminal) {
                if (outstanding.size() >= max_concurrent) {
                  ...
                  co_await detail::async_wait(
                      waiter, sending, outstanding, &completed, failure,
                      terminal, boost::asio::deferred);
                ...
                if (failure) {
                  writer.revert(
                      shard.id(), shard.object(),
                      detail::RevertHandler{
                          writer, sending, outstanding, waiter, terminal,
                          shard});
                } else {
                  writer.write(
                      shard.id(), shard.object(),
                      detail::RevertibleWriteHandler{
                          writer, sending, outstanding, completed, waiter,
                          failure, terminal, shard});
                }
              } // while (!sending.empty() && !terminal)
              ...
              co_await detail::async_wait(
                  waiter, sending, outstanding, &completed, failure, terminal,
                  boost::asio::deferred);
            ...
            for (;;) {
              while (!sending.empty() && !terminal) {
                if (outstanding.size() >= max_concurrent) {
                  // wait for the next completion before sending another
                  co_await detail::async_wait(
                      waiter, sending, outstanding, nullptr, failure, terminal,
                      boost::asio::deferred);
                  continue; // recheck loop conditions
                }
                ...
                writer.write(
                    shard.id(), shard.object(),
                    detail::WriteHandler{
                        writer, sending, outstanding, waiter, failure, terminal,
                        shard});
              } // while (!sending.empty() && !terminal)
...
struct ReadHandler {
  ...
  void
  operator()(error_code ec, version_t = {}, bufferlist = {})
  {
    outstanding.erase(outstanding.iterator_to(shard));
            ...
            for (;;) {
              while (!sending.empty() && !failure) {
                ...
                reader.read(
                    shard.id(), shard.object(),
                    detail::ReadHandler{
                        reader, sending, outstanding, waiter, failure, terminal,
                        shard});
              } // while (!sending.empty() && !failure)
...
private:
      ...
      detail::RevertibleWriteHandler&& handler) final
  {
    ...
    librados::async_operate(
        get_executor(), ioctx, object, std::move(op), flags, trace,
        std::move(handler));
  }
...
private:
  ...
  void
  write(int shard, const std::string& object, detail::WriteHandler&& handler) final
  {
    librados::ObjectWriteOperation op;
    prepare_write(shard, op);
...
private:
  ...
  void
  read(int shard, const std::string& object, detail::ReadHandler&& handler) final
  {
