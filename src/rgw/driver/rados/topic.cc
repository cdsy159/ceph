...
static rgw_raw_obj
get_topic_obj(const RGWZoneParams& zone, std::string_view metadata_key)
{
...
static rgw_raw_obj
get_buckets_obj(const RGWZoneParams& zone, std::string_view metadata_key)
{
    ...
    RGWObjVersionTracker* pobjv)
{
  ...
  int r = rgw_get_system_obj(
      &sysobj, obj.pool, obj.oid, bl, &entry.objv, &entry.mtime, y, dpp,
      nullptr, &cache_info);
  if (r < 0) {
    return r;
  }
    ...
    bool exclusive)
{
  ...
  int r = rgw_put_system_obj(
      dpp, &sysobj, obj.pool, obj.oid, bl, exclusive, &objv, mtime, y);
  if (r < 0) {
  ...
  if (const auto* id = std::get_if<rgw_account_id>(&info.owner); id) {
    ...
    r = topics::add(
        dpp, y, rados, topics, info, false,
        std::numeric_limits<uint32_t>::max());
    if (r < 0) {
      ldpp_dout(dpp, 0) << "WARNING: could not link topic to account " << *id
    ...
    RGWObjVersionTracker& objv)
{
  ...
  if (r < 0) {
    ldpp_dout(dpp, 1) << "ERROR: failed to remove topic obj " << topic.oid
                      << " with: " << cpp_strerror(r) << dendl;
    return r;
  }
  ...
  r = rgw_delete_system_obj(dpp, &sysobj, buckets.pool, buckets.oid, nullptr, y);
  if (r < 0) {
  ...
  if (rgw::account::validate_id(tenant)) {
    ...
    if (r < 0) {
      ldpp_dout(dpp, 0) << "ERROR: could not unlink from account " << tenant
                        << ": " << cpp_strerror(r) << dendl;
    } // not fatal
  }
...
public:
      ...
      const DoutPrefixProvider* dpp) override
  {
    cache_entry e;
    int ret = read(
        dpp, y, sysobj, cache_svc, zone, entry, e.info, cache, &e.mtime,
        &e.objv);
    if (ret < 0) {
      return ret;
    }
      ...
      bool from_remote_zone) override
  {
    ...
    int r = write(
        dpp, y, sysobj, &mdlog, rados, zone, info, objv_tracker, mtime,
        exclusive);
    if (r < 0) {
      return r;
    }
    if (!info.dest.push_endpoint.empty() && info.dest.persistent &&
      ...
      if (r >= 0) {
        r = rgw::notify::add_persistent_topic(
            dpp, ioctx, info.dest.persistent_queue, y);
      }
      if (r < 0) {
        ldpp_dout(dpp, 1)
            << "ERROR: failed to create queue for persistent topic "
            << info.dest.persistent_queue << " with: " << cpp_strerror(r)
            << dendl;
        return r;
      }
    }
      ...
      const DoutPrefixProvider* dpp) override
  {
    ...
    rgw_pubsub_topic info;
    int r = read(
        dpp, y, sysobj, cache_svc, zone, entry, info, cache, nullptr,
        &objv_tracker);
    if (r < 0) {
      return r;
    }
    ...
    r = topic::remove(
        dpp, y, sysobj, &mdlog, rados, zone, tenant, name, objv_tracker);
    if (r < 0) {
      return r;
    }
    ...
    if (!dest.push_endpoint.empty() && dest.persistent &&
      ...
      r = rgw_init_ioctx(dpp, &rados, zone.notif_pool, ioctx, true, false);
      if (r >= 0) {
        r = rgw::notify::remove_persistent_topic(
            dpp, ioctx, dest.persistent_queue, y);
      }
      if (r < 0 && r != -ENOENT) {
...
{
  return std::make_unique<MetadataHandler>(
      sysobj, cache_svc, mdlog, rados, zone, cache);
}
