...
namespace helper {
inline std::string
extract_pubkey_from_cert(const std::string& certstr, const std::string& pw = "")
{
    ...
    const std::string& password = "")
{
  ...
  std::shared_ptr<EVP_PKEY> pkey(
      PEM_read_bio_PUBKEY(
          pubkey_bio.get(), nullptr, nullptr, (void*)password.c_str()),
      EVP_PKEY_free);
  if (!pkey)
    throw rsa_exception(
...
struct none {
  ...
  void
  verify(const std::string&, const std::string& signature) const
  {
    if (!signature.empty())
...
struct rsa {
    ...
    md(md), alg_name(name)
  {
    if (!private_key.empty()) {
      pkey = helper::load_private_key_from_string(
  ...
  rsa(const EVP_MD* (*md)(), const std::string& name) :
    md(md), alg_name(name)
  {}

  void
  setModulusExponentCalcPublicKey(
  ...
  verify(const std::string& data, const std::string& signature) const
  {
    ...
    auto res = EVP_VerifyFinal(
        ctx.get(), (const unsigned char*)signature.data(), signature.size(),
        pkey.get());
    if (res != 1)
      throw signature_verification_exception(
...
private:
  ...
  std::string
  base64_decode_url(const std::string& str) const
  {
    std::string s = "====";
    std::string padded_str = str.length() % 4 == 0
                                 ? str
                                 : str + s.substr(0, str.length() % 4);
...
struct ecdsa {
    ...
    md(md), alg_name(name), signature_length(siglen)
  {
    if (!public_key.empty()) {
      ...
      if (keysize != signature_length * 4 &&
          (signature_length != 132 || keysize != 521))
  ...
  sign(const std::string& data) const
  {
    ...
    std::unique_ptr<ECDSA_SIG, decltype(&ECDSA_SIG_free)> sig(
        ECDSA_do_sign(
            (const unsigned char*)hash.data(), hash.size(), pkey.get()),
        ECDSA_SIG_free);
    if (!sig)
      throw signature_generation_exception();
#ifdef OPENSSL10
    ...
    while (rr.size() != signature_length / 2)
      rr = '\0' + rr;
    while (rs.size() != signature_length / 2)
      rs = '\0' + rs;
    return rr + rs;
  }
  ...
  verify(const std::string& data, const std::string& signature) const
  {
    ...
    if (ECDSA_do_verify(
            (const unsigned char*)hash.data(), hash.size(), &sig, pkey.get()) !=
        1)
    ...
    if (ECDSA_do_verify(
            (const unsigned char*)hash.data(), hash.size(), sig.get(),
            pkey.get()) != 1)
      throw signature_verification_exception("Invalid signature");
#endif
  }
...
private:
  ...
  /**
			 ...
			 */
  std::string
  generate_hash(const std::string& data) const
  {
#ifdef OPENSSL10
    std::unique_ptr<EVP_MD_CTX, decltype(&EVP_MD_CTX_destroy)> ctx(
        EVP_MD_CTX_create(), &EVP_MD_CTX_destroy);
...
struct pss {
  ...
  sign(const std::string& data) const
  {
    ...
    std::unique_ptr<RSA, decltype(&RSA_free)> key(
        EVP_PKEY_get1_RSA(pkey.get()), RSA_free);
    ...
    if (RSA_private_encrypt(
            size, (const unsigned char*)padded.data(),
            (unsigned char*)res.data(), key.get(), RSA_NO_PADDING) < 0)
      throw signature_generation_exception(
  ...
  verify(const std::string& data, const std::string& signature) const
  {
    ...
    if (!RSA_verify_PKCS1_PSS_mgf1(
            key.get(), (const unsigned char*)hash.data(), md(), md(),
...
struct rs256 : public rsa {
  ...
  rs256() :
    rsa(EVP_sha256, "RS256")
  {}

  rs256&
  setModulusAndExponent(const std::string& modulus, const std::string& exponent)
  {
...
public:
  ...
  /**
		 ...
		 */
  bool
  has_payload_claim(const std::string& name) const noexcept
  {
  ...
  /**
		 ...
		 */
  const claim&
  get_payload_claim(const std::string& name) const
  {
  ...
  /**
		 ...
		 */
  std::unordered_map<std::string, claim>
  get_payload_claims() const
  {
...
public:
  ...
  /**
		 ...
		 */
  bool
  has_content_type() const noexcept
  {
  ...
  /**
		 ...
		 */
  bool
  has_header_claim(const std::string& name) const noexcept
  {
  ...
  /**
		 ...
		 */
  const claim&
  get_header_claim(const std::string& name) const
  {
...
public:
  ...
  /**
		 ...
		 */
  const std::string&
  get_header_base64() const noexcept
  {
  ...
  /**
		 ...
		 */
  const std::string&
  get_payload_base64() const noexcept
  {
  ...
  /**
		 ...
		 */
  const std::string&
  get_signature_base64() const noexcept
  {
...
public:
  /**
		 ...
		 */
  builder&
  set_header_claim(const std::string& id, claim c)
  {
  ...
  /**
		 ...
		 */
  builder&
  set_payload_claim(const std::string& id, claim c)
  {
  ...
  /**
		 ...
		 */
  builder&
  set_algorithm(const std::string& str)
  {
  ...
  /**
		 ...
		 */
  builder&
  set_content_type(const std::string& str)
  {
  ...
  /**
		 ...
		 */
  builder&
  set_key_id(const std::string& str)
  {
  ...
  /**
		 ...
		 */
  builder&
  set_issuer(const std::string& str)
  {
  ...
  /**
		 ...
		 */
  builder&
  set_subject(const std::string& str)
  {
  ...
  /**
		 ...
		 */
  builder&
  set_audience(const std::set<std::string>& l)
  {
  ...
  /**
		 ...
		 */
  builder&
  set_audience(const std::string& aud)
  {
...
class verifier {
  ...
  struct algo : public algo_base {
    ...
    virtual void
    verify(const std::string& data, const std::string& sig) override
    {
      alg.verify(data, sig);
    }
  };
...
public:
  ...
  /**
		 ...
		 */
  verifier&
  expires_at_leeway(size_t leeway)
  {
    return with_claim(
        "exp", claim(std::chrono::system_clock::from_time_t(leeway)));
  }
  ...
  /**
		 ...
		 */
  verifier&
  not_before_leeway(size_t leeway)
  {
    return with_claim(
        "nbf", claim(std::chrono::system_clock::from_time_t(leeway)));
  }
  ...
  /**
		 ...
		 */
  verifier&
  issued_at_leeway(size_t leeway)
  {
    return with_claim(
        "iat", claim(std::chrono::system_clock::from_time_t(leeway)));
  }
  ...
  /**
		 ...
		 */
  verifier&
  with_issuer(const std::string& iss)
  {
  ...
  /**
		 ...
		 */
  verifier&
  with_subject(const std::string& sub)
  {
  ...
  /**
		 ...
		 */
  verifier&
  with_audience(const std::set<std::string>& aud)
  {
  ...
  /**
		 ...
		 */
  verifier&
  with_claim(const std::string& name, claim c)
  {
    claims[name] = c;
    return *this;
  }
  ...
  /**
		 ...
		 */
  void
  verify(const decoded_jwt& jwt) const
  {
    const std::string data = jwt.get_header_base64() + "." +
    ...
    auto assert_claim_eq = [](const decoded_jwt& jwt, const std::string& key,
      ...
      if (!jwt.has_payload_claim(key))
        throw token_verification_exception(
            "decoded_jwt is missing " + key + " claim");
      ...
      if (c.get_type() == claim::type::int64) {
        if (c.as_date() != jc.as_date())
          throw token_verification_exception(
              "claim " + key + " does not match expected");
