...
static const DoutPrefixProvider*
dpp()
{
...
usage()
{
  ...
  cout << "  user stats                       show user stats as accounted by "
          "quota subsystem\n";
  ...
  cout << "  dedup stats                      Display dedup statistics from "
          "the last run\n";
  cout << "  dedup estimate                   Runs dedup in estimate mode (no "
          "changes will be made)\n";
  cout << "  dedup exec                       Execute dedup (duplicated tail "
          "objects will be deleted); must include --yes-i-really-mean-it to "
          "activate\n";
  ...
  cout << "  bucket list                      list buckets (specify "
          "--allow-unordered for faster, unsorted listing)\n";
  cout << "  bucket limit check               show bucket sharding stats\n";
  cout << "  bucket link                      link bucket to specified user\n";
  cout << "  bucket unlink                    unlink bucket from specified "
          "user\n";
  cout << "  bucket stats                     returns bucket statistics\n";
  cout << "  bucket rm                        remove bucket\n";
  cout << "  bucket check                     check bucket index by verifying "
          "size and object count stats\n";
  cout << "  bucket check olh                 check for olh index entries and "
          "objects that are pending removal\n";
  cout << "  bucket check unlinked            check for object versions that "
          "are not visible in a bucket listing \n";
  cout << "  bucket chown                     link bucket to specified user "
          "and update its object ACLs\n";
  cout << "  bucket reshard                   reshard bucket\n";
  cout << "  bucket set-min-shards            set the minimum number of shards "
          "that dynamic resharding will consider for a bucket\n";
  cout << "  bucket rewrite                   rewrite all objects in the "
          "specified bucket\n";
  cout << "  bucket sync checkpoint           poll a bucket's sync status "
          "until it catches up to its remote\n";
  cout << "  bucket sync disable              disable bucket sync\n";
  cout << "  bucket sync enable               enable bucket sync\n";
  cout << "  bucket radoslist                 list rados objects backing "
          "bucket's objects\n";
  cout << "  bucket logging flush             flush pending log records object "
          "of source bucket to the log bucket\n";
  cout << "  bucket logging info              get info on bucket logging "
          "configuration on source bucket or list of sources in log bucket\n";
  cout << "  bucket logging list              list the log objects pending "
          "commit for the source bucket\n";
  cout << "  bi get                           retrieve bucket index object "
          "entries\n";
  cout << "  bi put                           store bucket index object "
          "entries\n";
  cout << "  bi list                          list raw bucket index entries\n";
  cout << "  bi purge                         purge bucket index entries\n";
  cout << "  object rm                        remove object; include "
          "--yes-i-really-mean-it to force removal from bucket index\n";
  ...
  cout << "  object reindex                   reindex the object(s) indicated "
          "by --bucket and either --object or --objects-file\n";
  cout << "  objects expire                   run expired objects cleanup\n";
  cout << "  objects expire-stale list        list stale expired objects "
          "(caused by reshard)\n";
  ...
  cout << "  quota set                        set quota params for a "
          "user/bucket/account\n";
  cout << "  quota enable                     enable quota for a "
          "user/bucket/account\n";
  cout << "  quota disable                    disable quota for a "
          "user/bucket/account\n";
  ...
  cout << "  zonegroup set                    set zone group info (requires "
          "infile)\n";
  cout << "  zonegroup remove                 remove a zone from a zonegroup\n";
  cout << "  zonegroup rename                 rename a zone group\n";
  cout << "  zonegroup list                   list all zone groups set on this "
          "cluster\n";
  cout << "  zonegroup placement list         list zonegroup's placement "
          "targets\n";
  cout << "  zonegroup placement get          get a placement target of a "
          "specific zonegroup\n";
  cout << "  zonegroup placement add          add a placement target id to a "
          "zonegroup\n";
  cout << "  zonegroup placement modify       modify a placement target of a "
          "specific zonegroup\n";
  cout << "  zonegroup placement rm           remove a placement target from a "
          "zonegroup\n";
  cout << "  zonegroup placement default      set a zonegroup's default "
          "placement target\n";
  cout << "  zone create                      create a new zone\n";
  cout << "  zone rm                          remove a zone\n";
  cout << "  zone get                         show zone cluster params\n";
  cout << "  zone modify                      modify an existing zone\n";
  cout << "  zone set                         set zone cluster params "
          "(requires infile)\n";
  cout << "  zone list                        list all zones set on this "
          "cluster\n";
  ...
  cout << "  data sync status                 get data sync status of the "
          "specified source zone\n";
  cout << "  data sync init                   init data sync for the specified "
          "source zone\n";
  cout << "  data sync run                    run data sync for the specified "
          "source zone\n";
  cout << "  pool add                         add an existing pool for data "
          "placement\n";
  cout << "  pool rm                          remove an existing pool from "
          "data placement set\n";
  cout << "  pools list                       list placement active set\n";
  cout << "  policy                           read bucket/object policy\n";
  cout << "  log list                         list log objects\n";
  cout << "  log show                         dump a log from specific object "
          "or (bucket + date + bucket-id)\n";
  cout << "                                   (NOTE: required to specify "
          "formatting of date to \"YYYY-MM-DD-hh\")\n";
  cout << "  log rm                           remove log object\n";
  cout << "  usage show                       show usage (by user, by bucket, "
          "date range)\n";
  cout << "  usage trim                       trim usage (by user, by bucket, "
          "date range)\n";
  cout << "  usage clear                      reset all the usage stats for "
          "the cluster\n";
  cout << "  gc list                          dump expired garbage collection "
          "objects (specify\n";
  cout << "                                   --include-all to list all "
          "entries, including unexpired)\n";
  cout << "  gc process                       manually process garbage "
          "(specify\n";
  cout << "                                   --include-all to process all "
          "entries, including unexpired)\n";
  cout << "  lc list                          list all bucket lifecycle "
          "progress\n";
  cout << "  lc get                           get a lifecycle bucket "
          "configuration\n";
  ...
  cout << "  bilog trim                       trim bucket index log (use "
  ...
  cout << "  datalog semaphore reset          Reset recovery semaphore (use "
          "marker)\n";
  cout << "  orphans find                     deprecated -- init and run "
          "search for leaked rados objects (use job-id, pool)\n";
  cout << "  orphans finish                   deprecated -- clean up search "
          "for leaked rados objects\n";
  cout << "  orphans list-jobs                deprecated -- list the current "
          "job-ids for orphans search\n";
  cout << "    * the three 'orphans' sub-commands are now deprecated; consider "
          "using the `rgw-orphan-list` tool\n";
  cout << "  role create                      create a AWS role for use with "
          "STS\n";
  cout << "  role delete                      remove a role\n";
  cout << "  role get                         get a role\n";
  cout << "  role list                        list roles with specified path "
          "prefix\n";
  cout << "  role-trust-policy modify         modify the assume role policy of "
          "an existing role\n";
  cout << "  role-policy put                  add/update permission policy to "
          "role\n";
  cout << "  role-policy list                 list policies attached to a "
          "role\n";
  cout << "  role-policy get                  get the specified inline policy "
          "document embedded with the given role\n";
  cout << "  role-policy delete               remove policy attached to a "
          "role\n";
  ...
  cout << "  role update                      update max_session_duration of a "
          "role\n";
  cout << "  reshard add                      schedule a resharding of a "
          "bucket\n";
  cout << "  reshard list                     list all bucket resharding or "
          "scheduled to be resharded\n";
  cout << "  reshard status                   read bucket resharding status\n";
  cout << "  reshard process                  process of scheduled reshard "
          "jobs\n";
  cout << "  reshard cancel                   cancel resharding a bucket\n";
  cout << "  reshard stale-instances list     list stale-instances from bucket "
          "resharding\n";
  cout << "  reshard stale-instances delete   cleanup stale-instances from "
          "bucket resharding\n";
  ...
  cout << "  topic list                       list bucket notifications "
          "topics\n";
  cout << "  topic get                        get a bucket notifications "
          "topic\n";
  cout << "  topic rm                         remove a bucket notifications "
          "topic\n";
  cout << "  topic stats                      get a bucket notifications "
          "persistent topic stats (i.e. reservations, entries & size)\n";
  cout << "  topic dump                       dump (in JSON format) all "
          "pending bucket notifications of a persistent topic\n";
  cout << "  script put                       upload a Lua script to a "
          "context\n";
  cout
      << "  script get                       get the Lua script of a context\n";
  cout << "  script rm                        remove the Lua scripts of a "
          "context\n";
  cout << "  script-package add               add a Lua package to the scripts "
          "allowlist\n";
  cout << "  script-package rm                remove a Lua package from the "
          "scripts allowlist\n";
  cout << "  script-package list              get the Lua packages allowlist\n";
  cout << "  script-package reload            install/remove Lua packages "
          "according to allowlist\n";
  cout << "  notification list                list bucket notifications "
          "configuration\n";
  cout << "  notification get                 get a bucket notifications "
          "configuration\n";
  cout << "  notification rm                  remove a bucket notifications "
          "configuration\n";
  cout << "  restore status                   shows restoration status of "
          "object in a bucket\n";
  cout << "  restore list                     list restore status of each "
          "object in the bucket\n";
  cout << "                                   can be filtered with help of "
          "--restore-status which shows objects with specified status\n";
  cout << "options:\n";
  cout << "   --tenant=<tenant>                 tenant name\n";
  cout << "   --user_ns=<namespace>             namespace of user (oidc in "
          "case of users authenticated with oidc provider)\n";
  cout << "   --uid=<id>                        user id\n";
  cout << "   --new-uid=<id>                    new user id\n";
  cout << "   --subuser=<name>                  subuser name\n";
  cout << "   --account-name=<name>             account name\n";
  cout << "   --account-id=<id>                 account id\n";
  cout << "   --max-users                       max number of users for an "
          "account\n";
  cout << "   --max-roles                       max number of roles for an "
          "account\n";
  cout << "   --max-groups                      max number of groups for an "
          "account\n";
  cout << "   --max-access-keys                 max number of keys per user "
          "for an account\n";
  cout << "   --access-key=<key>                S3 access key\n";
  cout << "   --email=<email>                   user's email address\n";
  cout << "   --secret/--secret-key=<key>       specify secret key\n";
  cout << "   --gen-access-key                  generate random access key "
          "(for S3)\n";
  cout << "   --gen-secret                      generate random secret key\n";
  cout << "   --generate-key                    create user with or without "
          "credentials\n";
  cout << "   --key-type=<type>                 key type, options are: swift, "
          "s3\n";
  cout << "   --key-active=<bool>               activate or deactivate a key\n";
  cout << "   --temp-url-key[-2]=<key>          temp url key\n";
  cout << "   --access=<access>                 Set access permissions for "
          "sub-user, should be one\n";
  cout << "                                     of read, write, readwrite, "
          "full\n";
  cout << "   --display-name=<name>             user's display name\n";
  cout << "   --max-buckets                     max number of buckets for a "
          "user\n";
  cout << "   --admin                           set the admin flag on the "
          "user\n";
  cout << "   --system                          set the system flag on the "
          "user\n";
  cout << "   --op-mask                         set the op mask on the user\n";
  cout << "   --bucket=<bucket>                 Specify the bucket name. Also "
          "used by the quota command.\n";
  cout << "   --pool=<pool>                     Specify the pool name. Also "
          "used to scan for leaked rados objects.\n";
  cout << "   --object=<object>                 object name\n";
  cout << "   --objects-file=<file>             file containing a list of "
          "object names to process\n";
  cout << "   --object-version=<version>        object version\n";
  cout
      << "   --date=<date>                     date in the format yyyy-mm-dd\n";
  cout << "   --start-date=<date>               start date in the format "
          "yyyy-mm-dd\n";
  cout << "   --end-date=<date>                 end date in the format "
          "yyyy-mm-dd\n";
  cout << "   --bucket-id=<bucket-id>           bucket id\n";
  cout << "   --bucket-new-name=<bucket>        for bucket link: optional new "
          "name\n";
  ...
  cout << "                                       bilog trim\n";
  cout << "                                       bilog status\n";
  cout << "   --max-entries=<entries>           max entries for listing "
          "operations\n";
  cout << "   --metadata-key=<key>              key to retrieve metadata from "
          "with metadata get\n";
  cout << "   --remote=<remote>                 zone or zonegroup id of remote "
          "gateway\n";
  cout << "   --period=<id>                     period id\n";
  cout << "   --url=<url>                       url for pushing/pulling "
          "period/realm\n";
  cout << "   --epoch=<number>                  period epoch\n";
  cout << "   --commit                          commit the period during "
          "'period update'\n";
  ...
  cout << "   --rgw-zone=<name>                 name of zone in which radosgw "
          "is running\n";
  cout << "   --zone-id=<id>                    zone id\n";
  cout << "   --zone-new-name=<name>            zone new name\n";
  cout << "   --source-zone                     specify the source zone (for "
          "data sync)\n";
  cout << "   --default                         set entity (realm, zonegroup, "
          "zone) as default\n";
  cout << "   --read-only                       set zone as read-only (when "
          "adding to zonegroup)\n";
  cout << "   --redirect-zone                   specify zone id to redirect "
          "when response is 404 (not found)\n";
  cout << "   --placement-id                    placement id for zonegroup "
          "placement commands\n";
  cout << "   --storage-class                   storage class for zonegroup "
          "placement commands\n";
  cout << "   --tags=<list>                     list of tags for zonegroup "
          "placement add and modify commands\n";
  cout << "   --tags-add=<list>                 list of tags to add for "
          "zonegroup placement modify command\n";
  cout << "   --tags-rm=<list>                  list of tags to remove for "
          "zonegroup placement modify command\n";
  ...
  cout << "   --data-extra-pool=<pool>          placement target data extra "
          "(non-ec) pool\n";
  cout << "   --placement-index-type=<type>     placement target index type "
          "(normal, indexless, or #id)\n";
  cout << "   --placement-inline-data=<true>    set whether the placement "
          "target is configured to store a data\n";
  cout << "                                     chunk inline in head objects\n";
  cout << "   --compression=<type>              placement target compression "
          "type (plugin name or empty/none)\n";
  cout << "   --tier-type=<type>                zone tier type\n";
  cout << "   --tier-config=<k>=<v>[,...]       set zone tier config keys, "
          "values\n";
  cout << "   --tier-config-rm=<k>[,...]        unset zone tier config keys\n";
  cout << "   --sync-from-all[=false]           set/reset whether zone syncs "
          "from all zonegroup peers\n";
  cout << "   --sync-from=[zone-name][,...]     set list of zones to sync "
          "from\n";
  cout << "   --sync-from-rm=[zone-name][,...]  remove zones from list of "
          "zones to sync from\n";
  cout << "   --bucket-index-max-shards         override a zone/zonegroup's "
          "default bucket index shard count\n";
  cout << "   --fix                             besides checking bucket index, "
          "will also fix it\n";
  cout << "   --check-objects                   bucket check: rebuilds bucket "
          "index according to actual objects state\n";
  cout << "   --format=<format>                 specify output format for "
          "certain operations: xml, json (default: json)\n";
  cout << "   --pretty-format                   enable pretty formatting for "
          "json/xml output\n";
  cout << "   --purge-data                      when specified, user removal "
          "will also purge all the\n";
  cout << "                                     user data\n";
  cout << "   --purge-keys                      when specified, subuser "
          "removal will also purge all the\n";
  cout << "                                     subuser keys\n";
  cout << "   --purge-objects                   remove a bucket's objects "
          "before deleting it\n";
  cout << "                                     (NOTE: required to delete a "
          "non-empty bucket)\n";
  cout << "   --sync-stats                      option to 'user stats', update "
          "user stats with current\n";
  cout << "                                     stats reported by user's "
          "buckets indexes\n";
  cout << "   --reset-stats                     option to 'user stats', reset "
          "stats in accordance with user buckets\n";
  cout << "   --show-config                     show configuration\n";
  cout << "   --show-log-entries=<flag>         enable/disable dump of log "
          "entries on log show\n";
  cout << "   --show-log-sum=<flag>             enable/disable dump of log "
          "summation on log show\n";
  cout << "   --skip-zero-entries               log show only dumps entries "
          "that don't have zero value\n";
  cout << "                                     in one of the numeric field\n";
  cout << "   --infile=<file>                   file to read in when setting "
          "data\n";
  cout << "   --categories=<list>               comma separated list of "
          "categories, used in usage show\n";
  cout << "   --caps=<caps>                     list of caps (e.g., "
          "\"usage=read, write; user=read\")\n";
  cout << "   --op-mask=<op-mask>               permission of user's "
          "operations (e.g., \"read, write, delete, *\")\n";
  cout << "   --yes-i-really-mean-it            required for certain "
          "operations\n";
  cout << "   --warnings-only                   when specified with bucket "
          "limit check, list\n";
  cout << "                                     only buckets nearing or over "
          "the current max\n";
  cout << "                                     objects per shard value\n";
  cout << "   --bypass-gc                       when specified with bucket "
          "deletion, triggers\n";
  cout << "                                     object deletions by not "
          "involving GC\n";
  cout << "   --inconsistent-index              when specified with bucket "
          "deletion and bypass-gc set to true,\n";
  cout << "                                     ignores bucket index "
          "consistency\n";
  cout << "   --min-rewrite-size                min object size for bucket "
          "rewrite (default 4M)\n";
  cout << "   --max-rewrite-size                max object size for bucket "
          "rewrite (default ULLONG_MAX)\n";
  cout << "   --min-rewrite-stripe-size         min stripe size for object "
          "rewrite (default 0)\n";
  cout << "   --trim-delay-ms                   time interval in msec to limit "
          "the frequency of sync error log entries trimming operations,\n";
  cout << "                                     the trimming process will "
          "sleep the specified msec for every 1000 entries trimmed\n";
  cout << "   --max-concurrent-ios              maximum concurrent ios for "
          "bucket operations (default: 32)\n";
  cout << "   --enable-feature                  enable a zone/zonegroup "
          "feature\n";
  cout << "   --disable-feature                 disable a zone/zonegroup "
          "feature\n";
  cout << "\n";
  cout << "<date> := \"YYYY-MM-DD[ hh:mm:ss]\"\n";
  cout << "\nDedup throttle options:\n";
  cout << "   --max-bucket-index-ops        specify max bucket-index requests "
          "per second allowed for an RGW during dedup, 0 means unlimited\n";
  cout << "   --max-metadata-ops            specify max metadata requests per "
          "second allowed for an RGW during dedup, 0 means unlimited\n";
  cout << "   --stat                        display dedup throttle setting\n";
  cout << "\nQuota options:\n";
  cout << "   --max-objects                 specify max objects (negative "
          "value to disable)\n";
  cout << "   --max-size                    specify max size (in B/K/M/G/T, "
          "negative value to disable)\n";
  cout << "   --quota-scope                 scope of quota (bucket, user, "
          "account)\n";
  cout << "\nRate limiting options:\n";
  cout << "   --max-read-ops                specify max requests per "
          "accumulation interval for READ ops per RGW (GET and HEAD request "
          "methods), 0 means unlimited\n";
  cout << "   --max-read-bytes              specify max bytes per accumulation "
          "interval for READ ops per RGW (GET and HEAD request methods), 0 "
          "means unlimited\n";
  cout << "   --max-write-ops               specify max requests per "
          "accumulation interval for WRITE ops per RGW (Not GET or HEAD "
          "request methods), 0 means unlimited\n";
  cout << "   --max-write-bytes             specify max bytes per accumulation "
          "interval for WRITE ops per RGW (Not GET or HEAD request methods), 0 "
          "means unlimited\n";
  cout << "   --max-list-ops                specify max requests per "
          "accumulation interval for bucket listing requests per RGW, 0 means "
          "unlimited\n";
  cout << "   --max-delete-ops              specify max requests per "
          "accumulation interval for DELETE ops per RGW (DELETE request "
          "methods), 0 means unlimited\n";
  cout << "   --ratelimit-scope             scope of rate limiting: bucket, "
          "user, anonymous\n";
  cout << "                                 anonymous can be configured only "
          "with global rate limit\n";
  cout << "\nOrphans search options:\n";
  cout << "   --num-shards                  num of shards to use for keeping "
          "the temporary scan info\n";
  cout << "   --orphan-stale-secs           num of seconds to wait before "
          "declaring an object to be an orphan (default: 86400)\n";
  cout
      << "   --job-id                      set the job id (for orphans find)\n";
  cout << "   --detail                      detailed mode, log and stat head "
          "objects as well\n";
  ...
  cout << "   --assume-role-policy-doc      the trust relationship policy "
          "document that grants an entity permission to assume the role\n";
  ...
  cout << "\nMFA options:\n";
  cout << "   --totp-serial                 a string that represents the ID of "
          "a TOTP token\n";
  cout << "   --totp-seed                   the secret seed that is used to "
          "calculate the TOTP\n";
  cout << "   --totp-seconds                the time resolution that is being "
          "used for TOTP generation\n";
  cout << "   --totp-window                 the number of TOTP tokens that are "
          "checked before and after the current token when validating token\n";
  cout << "   --totp-pin                    the valid value of a TOTP token at "
          "a certain time\n";
  cout << "\nBucket notifications options:\n";
  cout << "   --topic                       bucket notifications topic name\n";
  cout << "   --notification-id             bucket notifications id\n";
  cout << "\nScript options:\n";
  cout << "   --context                     context in which the script runs. "
          "one of: " +
              LUA_CONTEXT_LIST + "\n";
  cout << "   --package                     name of the Lua package that "
          "should be added/removed to/from the allowlist\n";
  cout << "   --allow-compilation           package is allowed to compile C "
          "code as part of its installation\n";
  cout << "\nBucket check olh/unlinked options:\n";
  cout << "   --min-age-hours               minimum age of unlinked objects to "
          "consider for bucket check unlinked (default: 1)\n";
  cout << "   --dump-keys                   when specified, all keys "
          "identified as problematic are printed to stdout\n";
  cout << "   --hide-progress               when specified, per-shard progress "
          "details are not printed to stderr\n";
  cout << "\nradoslist options:\n";
  cout << "   --rgw-obj-fs                  the field separator that will "
          "separate the rados object name from the rgw object name;\n";
  cout << "                                 additionally rados objects for "
          "incomplete multipart uploads will not be output\n";
  cout << "\nBucket list objects options:\n";
  cout << "   --max-entries                 max number of entries listed "
          "(default 1000)\n";
  cout << "   --marker                      object name marker to specify "
          "where listing begins (default: start from beginning)\n";
  cout << "                                 requires ordered listing (do not "
          "use with --allow-unordered)\n";
  cout << "   --object-version              for versioned buckets: specify the "
          "version/instance ID to start from\n";
  cout << "                                 use together with --marker to "
          "paginate through versioned buckets\n";
  cout << "                                 example: --marker=obj1 "
          "--object-version=abc123def456\n";
  cout << "   --show-restore-stats          if the flag is in present it will "
          "show restores stats in the bucket stats command\n";
  cout << "\n";
...
private:
  ...
  string
  normalize_alias(const string& s) const
  {
  ...
  void
  init_alias_map(Aliases& aliases)
  {
  ...
  bool
  gen_next_expected(Node* node, vector<string>* expected, bool ret)
  {
...
public:
  ...
  void
  add_aliases(Aliases& aliases)
  {
    init_alias_map(aliases);
  }

  void
  add_commands(std::vector<Def>& cmds)
  {
    for (auto& cmd : cmds) {
      vector<string> words;
...
BIIndexType
get_bi_index_type(const string& type_str)
{
  if (type_str == "plain")
...
log_type
get_log_type(const string& type_str)
{
...
static void
show_user_info(RGWUserInfo& info, Formatter* formatter)
{
...
static void
show_perm_policy(string perm_policy, Formatter* formatter)
{
...
static void
show_policy_names(const std::vector<string>& policy_names, Formatter* formatter)
{
...
static void
show_policy_arns(
    const boost::container::flat_set<std::string>& arns,
    Formatter* formatter)
{
...
static void
show_reshard_status(
    const list<cls_rgw_bucket_instance_entry>& status,
    Formatter* formatter)
{
...
static int
init_bucket(const rgw_bucket& b, std::unique_ptr<rgw::sal::Bucket>* bucket)
{
...
static int
read_input(const string& infile, bufferlist& bl)
{
  int fd = 0;
  if (infile.size()) {
...
template <class T>
static int
read_decode_json(const string& infile, T& t)
{
  bufferlist bl;
...
static int
read_decode_json(const string& infile, T& t, K* k)
{
  bufferlist bl;
  int ret = read_input(infile, bl);
  if (ret < 0) {
...
template <class T>
static bool
decode_dump(const char* field_name, bufferlist& bl, Formatter* f)
{
  T t;
...
static bool
dump_string(const char* field_name, bufferlist& bl, Formatter* f)
{
  string val = bl.to_str();
  f->dump_string(
      field_name, val.c_str() /* hide encoded null termination chars */);
...
set_ratelimit_info(
    ...
    bool have_max_read_ops,
    bool have_max_write_ops,
    bool have_max_list_ops,
    bool have_max_delete_ops,
...
{
  std::unique_ptr<rgw::sal::Bucket> bucket;
  int r = driver->load_bucket(
      dpp(), rgw_bucket(tenant_name, bucket_name), &bucket, null_yield);
  if (r < 0) {
  ...
  set_quota_info(
      bucket->get_info().quota, opt_cmd, max_size, max_objects, have_max_size,
      have_max_objects);

  r = bucket->put_info(dpp(), false, real_time(), null_yield);
  if (r < 0) {
    cerr << "ERROR: failed writing bucket instance info: " << cpp_strerror(-r)
         << std::endl;
    return -r;
  }
  return 0;
    ...
    bool have_max_write_bytes)
{
  ...
  bool ratelimit_configured = set_ratelimit_info(
      ratelimit_info, opt_cmd, max_read_ops, max_write_ops, max_list_ops,
      max_delete_ops, max_read_bytes, max_write_bytes, have_max_read_ops,
      have_max_write_ops, have_max_list_ops, have_max_delete_ops,
      have_max_read_bytes, have_max_write_bytes);
  if (!ratelimit_configured) {
    ldpp_dout(dpp(), 0) << "ERROR: no rate limit values have been specified"
                        << dendl;
    return -EINVAL;
  }
  bufferlist bl;
    ...
    bool have_max_write_bytes)
{
  ...
  if (r < 0) {
    cerr << "ERROR: failed writing user instance info: " << cpp_strerror(-r)
         << std::endl;
    return -r;
  }
  return 0;
...
int
show_user_ratelimit(std::unique_ptr<rgw::sal::User>& user, Formatter* formatter)
{
  RGWRateLimitInfo ratelimit_info;
  user->load_user(dpp(), null_yield);
  auto iter = user->get_attrs().find(RGW_ATTR_RATELIMIT);
    ...
    bool have_max_objects)
{
  ...
  set_quota_info(
      user_info.quota.bucket_quota, opt_cmd, max_size, max_objects,
  ...
  if (r < 0) {
    cerr << "ERROR: failed updating user info: " << cpp_strerror(-r) << ": "
         << err << std::endl;
    return -r;
  }
  return 0;
    ...
    bool have_max_objects)
{
  ...
  set_quota_info(
      user_info.quota.user_quota, opt_cmd, max_size, max_objects, have_max_size,
      have_max_objects);
    ...
    bool* need_rewrite)
{
  ...
  if (ret < 0) {
    ldpp_dout(dpp(), -1) << "ERROR: failed to stat object, returned error: "
    ...
    Formatter* f)
{
  ...
  if ((needs_fixing || remove_bad) && fix) {
    ret = static_cast<rgw::sal::RadosStore*>(driver)
              ->getRados()
              ->fix_head_obj_locator(
                  dpp(), obj->get_bucket()->get_info(), needs_fixing,
                  remove_bad, obj->get_key(), null_yield);
    if (ret < 0) {
      cerr << "ERROR: fix_head_object_locator() returned ret=" << ret
           << std::endl;
      goto done;
    }
    status = "fixed";
...
int
check_obj_tail_locator_underscore(
    ...
    Formatter* f)
{
  ...
  int ret = static_cast<rgw::sal::RadosStore*>(driver)
                ->getRados()
                ->fix_tail_obj_locator(
                    dpp(), bucket_info, key, fix, &needs_fixing, null_yield);
  if (ret < 0) {
    cerr << "ERROR: fix_tail_object_locator_underscore() returned ret=" << ret
         << std::endl;
    status = "failed";
  } else {
    ...
    Formatter* f)
{
  ...
  do {
    ...
    if (ret < 0) {
      cerr << "ERROR: driver->list_objects(): " << cpp_strerror(-ret)
           << std::endl;
      return -ret;
    }
    ...
    for (vector<rgw_bucket_dir_entry>::iterator iter = results.objs.begin();
         iter != results.objs.end(); ++iter) {
      ...
      if (obj->get_name()[0] == '_') {
        ...
        if (ret >= 0) {
          ret = check_obj_tail_locator_underscore(
              bucket->get_info(), obj->get_key(), fix, f);
          if (ret < 0) {
            cerr << "ERROR: check_obj_tail_locator_underscore(): "
...
static boost::optional<RGWRESTConn>
get_remote_conn(
    ...
    const std::string& remote)
{
  ...
  if (remote == zonegroup.get_id()) {
    conn.emplace(
        driver->ctx(), driver, remote, zonegroup.endpoints, zonegroup.api_name);
  } else {
    for (const auto& z : zonegroup.zones) {
      const auto& zone = z.second;
      if (remote == zone.id) {
        conn.emplace(
            driver->ctx(), driver, remote, zone.endpoints, zonegroup.api_name);
        break;
      }
    }
  }
    ...
    JSONParser& parser)
{
  ...
  rgw_user user;
  auto result = conn->forward(
      dpp(), user, info, MAX_REST_RESPONSE, &in_data, &response, null_yield);
  if (!result) {
    return result.error();
  }
    ...
    JSONParser& parser)
{
  if (access.empty() || secret.empty()) {
    cerr << "An --access-key and --secret must be provided with --url."
         << std::endl;
    return -EINVAL;
  }
  RGWAccessKey key;
  ...
  RGWRESTSimpleRequest req(
      g_ceph_context, info.method, url, NULL, &params, opt_region);

  bufferlist response;
  auto result = req.forward_request(
      dpp(), key, info, MAX_REST_RESPONSE, &in_data, &response, null_yield);
  if (!result) {
    return result.error();
  }
  int ret = rgw_http_error_to_errno(*result);
...
commit_period(
    ...
    RGWRealm& realm,
    rgw::sal::RealmWriter& realm_writer,
    RGWPeriod& period,
    string remote,
    const string& url,
    std::optional<string> opt_region,
    ...
    rgw::SiteConfig* site)
{
  ...
  if (master_zone.empty()) {
    cerr << "cannot commit period: period does not have a master zone of a "
            "master zonegroup"
         << std::endl;
    return -EINVAL;
  }
  ...
  if (driver->get_zone()->get_id() == master_zone) {
    ...
    int ret = cfgstore->read_period(
        dpp(), null_yield, realm.current_period, std::nullopt, current_period);
    if (ret < 0) {
      cerr << "failed to load current period: " << cpp_strerror(ret)
           << std::endl;
      return ret;
    }
    // the master zone can commit locally
    ret = rgw::commit_period(
        dpp(), null_yield, cfgstore, driver, realm, realm_writer,
        current_period, period, cerr, force, *site);
    if (ret < 0) {
  ...
  if (!remote.empty()) {
    conn = get_remote_conn(
        static_cast<rgw::sal::RadosStore*>(driver), period.get_map(), remote);
    if (!conn) {
      cerr << "failed to find a zone or zonegroup for remote " << remote
           << std::endl;
      return -ENOENT;
    }
    remote_conn = &*conn;
  ...
  JSONParser p;
  int ret = send_to_remote_or_url(
      remote_conn, url, opt_region, access, secret, info, bl, p);
  if (ret < 0) {
  ...
  ret = cfgstore->update_latest_epoch(
      dpp(), null_yield, period.get_id(), period.get_epoch());
  if (ret == -EEXIST) {
    // already have this epoch (or a more recent one)
    cerr << "already have epoch >= " << period.get_epoch() << " for period "
         << period.get_id() << std::endl;
    return 0;
  }
  if (ret < 0) {
    cerr << "Error updating latest epoch for period " << period.get_id() << ": "
         << cpp_strerror(ret) << std::endl;
    return ret;
  }
    ...
    rgw::SiteConfig* site)
{
  ...
  int ret = rgw::read_realm(
      dpp(), null_yield, cfgstore, realm_id, realm_name, realm, &realm_writer);
  if (ret < 0) {
  ...
  RGWPeriod period;
  ret = cfgstore->read_period(
      dpp(), null_yield, realm.current_period, epoch, period);
  if (ret < 0) {
    cerr << "failed to load current period: " << cpp_strerror(-ret)
         << std::endl;
    return ret;
  ...
  if (commit) {
    ret = commit_period(
        cfgstore, realm, *realm_writer, period, remote, url, opt_region, access,
        secret, force, site);
    if (ret < 0) {
      cerr << "failed to commit period: " << cpp_strerror(-ret) << std::endl;
      return ret;
    ...
    RGWPeriod* period)
{
  ...
  JSONParser p;
  int ret = send_to_remote_or_url(
      remote_conn, url, opt_region, access_key, secret_key, info, bl, p);
  if (ret < 0) {
    cerr << "request failed: " << cpp_strerror(-ret) << std::endl;
    return ret;
  }
  ...
  ret = cfgstore->create_period(dpp(), null_yield, exclusive, *period);
  if (ret < 0) {
    cerr << "Error storing period " << period->get_id() << ": "
         << cpp_strerror(ret) << std::endl;
  }
...
void
flush_ss(stringstream& ss, list<string>& l)
{
  if (!ss.str().empty()) {
    l.push_back(ss.str());
  }
...
stringstream&
push_ss(stringstream& ss, list<string>& l, int tab = 0)
{
  flush_ss(ss, l);
  if (tab > 0) {
    ss << setw(tab) << "" << setw(1);
...
static void
get_md_sync_status(list<string>& status)
{
  ...
  stringstream ss;
  push_ss(ss, status) << "full sync: " << num_full << "/" << total_shards
                      << " shards";

  if (num_full > 0) {
    push_ss(ss, status) << "full sync: " << full_total - full_complete
                        << " entries to sync";
  }

  push_ss(ss, status) << "incremental sync: " << num_inc << "/" << total_shards
                      << " shards";

  map<int, RGWMetadataLogInfo> master_shards_info;
  string master_period = static_cast<rgw::sal::RadosStore*>(driver)
                             ->svc()
                             ->zone->get_current_period_id();

  ret = sync.read_master_log_shards_info(
      dpp(), master_period, &master_shards_info);
  if (ret < 0) {
    status.push_back(
  ...
  if (sync_status.sync_info.period != master_period) {
    status.push_back(string(
        "master is on a different period: master_period=" + master_period +
    ...
    for (auto local_iter : sync_status.sync_markers) {
      ...
      if (iter == master_shards_info.end()) {
        /* huh? */
        derr << "ERROR: could not find remote sync shard status for shard_id="
             << shard_id << dendl;
        continue;
      }
      auto master_marker = iter->second.marker;
      if (local_iter.second.state ==
              rgw_meta_sync_marker::SyncState::IncrementalSync &&
  ...
  if (!shards_behind.empty()) {
    ...
    ret = sync.read_master_log_shards_next(
        dpp(), sync_status.sync_info.period, shards_behind, &master_pos);
    if (ret < 0) {
      derr << "ERROR: failed to fetch master next positions ("
           << cpp_strerror(-ret) << ")" << dendl;
    } else {
        ...
        if (shard_data.entries.empty()) {
          ...
          if (!oldest) {
            oldest.emplace(iter.first, entry.timestamp);
              ...
              !ceph::real_clock::is_zero(entry.timestamp) &&
  ...
  int total_behind = shards_behind.size() +
                     (sync_status.sync_info.num_shards - num_inc);
  if (total_behind == 0) {
    push_ss(ss, status) << "metadata is caught up with master";
  } else {
    push_ss(ss, status) << "metadata is behind on " << total_behind
                        << " shards";
    ...
    int tab)
{
  ...
  if (!(sz = static_cast<rgw::sal::RadosStore*>(driver)->svc()->zone->find_zone(
            source_zone))) {
  ...
  if (!static_cast<rgw::sal::RadosStore*>(driver)->svc()->zone->zone_syncs_from(
          *sz)) {
  ...
  ret = sync.read_recovering_shards(
      dpp(), sync_status.sync_info.num_shards, recovering_shards);
  if (ret < 0 && ret != ENOENT) {
    push_ss(ss, status, tab)
  ...
  push_ss(ss, status, tab) << "full sync: " << num_full << "/" << total_shards
                           << " shards";

  if (num_full > 0) {
    push_ss(ss, status, tab)
        << "full sync: " << full_total - full_complete << " buckets to sync";
  }

  push_ss(ss, status, tab) << "incremental sync: " << num_inc << "/"
                           << total_shards << " shards";
  ...
  if (!shards_behind.empty()) {
    ...
    if (ret < 0) {
      derr << "ERROR: failed to fetch next positions (" << cpp_strerror(-ret)
           << ")" << dendl;
    } else {
      for (auto iter : master_pos) {
  ...
  if (total_behind == 0 && total_recovering == 0) {
    ...
    push_ss(ss, status, tab)
        << "data is behind on " << total_behind << " shards";
    push_ss(ss, status, tab)
        << "behind shards: " << "[" << shards_behind_set << "]";
    if (oldest) {
      push_ss(ss, status, tab)
          << "oldest incremental change not applied: " << oldest->second << " ["
          << oldest->first << ']';
    }
  }
...
static void
tab_dump(const string& header, int width, const list<string>& entries)
{
  string s = header;
...
static auto
get_disabled_features(const rgw::zone_features::set& enabled)
{
  auto features = rgw::zone_features::set{
...
static void
sync_status(Formatter* formatter)
{
  ...
  cout << std::setw(width) << "realm" << std::setw(1) << " "
       << zone->get_realm_id() << " (" << zone->get_realm_name() << ")"
       << std::endl;
  cout << std::setw(width) << "zonegroup" << std::setw(1) << " "
       << zonegroup.get_id() << " (" << zonegroup.get_name() << ")"
       << std::endl;
  cout << std::setw(width) << "zone" << std::setw(1) << " " << zone->get_id()
       << " (" << zone->get_name() << ")" << std::endl;
  ...
  cout << std::setw(width)
       << "zonegroup features enabled: " << rzg.enabled_features << std::endl;
  if (auto d = get_disabled_features(rzg.enabled_features); !d.empty()) {
    cout << std::setw(width) << "                   disabled: " << d
         << std::endl;
  }
  ...
  auto& zone_conn_map = static_cast<rgw::sal::RadosStore*>(driver)
                            ->svc()
                            ->zone->get_zone_conn_map();
  ...
  for (auto iter : zone_conn_map) {
    ...
    if (driver->get_zone()->get_zonegroup().get_zone_by_id(source_id.id, &sz) ==
        0) {
...
struct indented {
  ...
  indented(int w, std::string header = "") :
    w(w), header(header)
  {}
};

std::ostream&
operator<<(std::ostream& out, const indented& h)
{
...
struct bucket_source_sync_info {
  ...
  bucket_source_sync_info(const RGWZone& source) :
    _source(source)
  {}

  void
  _print_plaintext(std::ostream& out, int width) const
  {
    out << indented{width, "source zone"} << _source.id << " (" << _source.name
        << ")" << std::endl;
    if (!error.empty()) {
    ...
    out << indented{width} << "incremental sync on " << total_shards
        << " shards\n";
    if (!shards_behind.empty()) {
      out << indented{width} << "bucket is behind on " << shards_behind.size()
          << " shards\n";
      set<int> shard_ids;
  ...
  void
  _print_formatter(std::ostream& out, Formatter* formatter) const
  {
...
static int
bucket_source_sync_status(
    const DoutPrefixProvider* dpp,
    rgw::sal::RadosStore* driver,
    const RGWZone& zone,
    const RGWZone& source,
    RGWRESTConn* conn,
    ...
    bucket_source_sync_info& source_sync_info)
{
  ...
  if (!pipe.source.bucket) {
    source_sync_info.error =
        fmt::format("{} (): missing source bucket", __func__);
    return -EINVAL;
  }
  ...
  if (r < 0) {
    source_sync_info.error =
        fmt::format("failed to read source bucket info: {}", cpp_strerror(r));
    return r;
  }
  ...
  r = rgw_read_bucket_full_sync_status(
      dpp, driver, pipe, &full_status, null_yield);
  if (r >= 0) {
    ...
    if (full_status.state == BucketSyncState::Full) {
      source_sync_info.status = fmt::format(
          "full sync: {} objects completed", full_status.full.count);
      return 0;
    }
    ...
    if (log.layout.type != rgw::BucketLogType::InIndex) {
      source_sync_info.error = fmt::format(
          "unrecognized log layout type {}", to_string(log.layout.type));
      return -EINVAL;
    }
    ...
    source_sync_info.error = fmt::format(
        "failed to read bucket full sync status: {}", cpp_strerror(r));
    return r;
  }
  ...
  if (r < 0) {
    source_sync_info.error = fmt::format(
        "failed to read bucket incremental sync status: {}", cpp_strerror(r));
    return r;
  }
  ...
  r = rgw_read_remote_bilog_info(
      dpp, conn, source_bucket->get_key(), remote_info, remote_markers,
      null_yield);
  if (r < 0) {
    source_sync_info.error =
        fmt::format("failed to read remote log: {}", cpp_strerror(r));
    return r;
  }
  ...
  for (const auto& r : remote_markers.get()) {
    ...
    const auto pos =
        BucketIndexShardsManager::get_shard_marker(m.inc_marker.position);
    if (pos < r.second) {
      shards_behind[shard_id] = r.second;
    }
  }
...
encode_json(
    const char* name,
    const RGWBucketSyncFlowManager::pipe_set& pset,
    Formatter* f)
{
...
convert_bucket_set_to_str_vec(const std::set<rgw_bucket>& bs)
{
    ...
    std::set<rgw_sync_bucket_entity>* hint_entities)
{
  for (auto& zone_id : zones) {
    for (auto& b : buckets) {
      ...
      if (ret < 0) {
        ldpp_dout(dpp(), 20)
            << "could not init bucket info for hint bucket=" << b
            << " ... skipping" << dendl;
        continue;
      }

      hint_entities->insert(
          rgw_sync_bucket_entity(zone_id, hint_bucket->get_key()));
    }
  }
}

static rgw_zone_id
resolve_zone_id(const string& s)
{
...
rgw_zone_id
validate_zone_id(const rgw_zone_id& zone_id)
{
    ...
    Formatter* formatter)
{
  ...
  if (eff_bucket) {
    ...
    if (ret >= 0) {
      ...
      bucket_handler.reset(
          handler->alloc_child(bucket->get_info(), std::move(attrs)));
    } else {
    ...
    if (ret < 0) {
      cerr << "ERROR: failed to init bucket sync policy handler: "
           << cpp_strerror(-ret) << " (ret=" << ret << ")" << std::endl;
      return ret;
    }
  ...
  auto source_hints_vec =
      convert_bucket_set_to_str_vec(handler->get_source_hints());
  auto target_hints_vec =
      convert_bucket_set_to_str_vec(handler->get_target_hints());
  ...
  zone_policy_handler->reflect(
      dpp(), nullptr, nullptr, nullptr, nullptr, &source_zones, &target_zones,
  ...
  for (auto& hint_entity : hint_entities) {
    ...
    int r = driver->get_sync_policy_handler(
        dpp(), zid, hint_bucket, &hint_bucket_handler, null_yield);
    if (r < 0) {
      ldpp_dout(dpp(), 20)
          << "could not get bucket sync policy handler for hint bucket="
          << hint_bucket << " ... skipping" << dendl;
      continue;
    }

    hint_bucket_handler->get_pipes(
        &resolved_dests, &resolved_sources,
...

  {
    ...
    {
      Formatter::ObjectSection resolved_hints_section(
          *formatter, "resolved-hints-1");
      encode_json("sources", resolved_sources, formatter);
      encode_json("dests", resolved_dests, formatter);
    }
    {
      Formatter::ObjectSection resolved_hints_section(
          *formatter, "resolved-hints");
    ...
    std::ostream& out)
{
  ...
  out << indented{width, "realm"} << zone->get_realm_id() << " ("
      << zone->get_realm_name() << ")\n";
  out << indented{width, "zonegroup"} << zonegroup.get_id() << " ("
      << zonegroup.get_name() << ")\n";
  out << indented{width, "zone"} << zone->get_id() << " (" << zone->get_name()
      << ")\n";
  out << indented{width, "bucket"} << info.bucket << "\n\n";

  if (!static_cast<rgw::sal::RadosStore*>(driver)
           ->ctl()
           ->bucket->bucket_imports_data(info.bucket, null_yield, dpp())) {
  ...
  int r = driver->get_sync_policy_handler(
      dpp(), std::nullopt, info.bucket, &handler, null_yield);
  if (r < 0) {
    ldpp_dout(dpp(), -1) << "ERROR: failed to get policy handler for bucket ("
                         << info.bucket << "): r=" << r << ": "
  ...
  for (auto& m : sources) {
    ...
    for (auto& pipe_handler : m.second) {
      out << indented{width, "bucket"} << *pipe_handler.source.bucket
          << std::endl;
    }
  }
...
struct bucket_sync_status_info {
  ...
  bucket_sync_status_info(const RGWBucketInfo& bucket_info) :
    _bucket_info(bucket_info)
  {}

  void
  print(std::ostream& out, bool use_formatter, Formatter* formatter)
  {
    if (use_formatter) {
  ...
  void
  _print_plaintext(std::ostream& out)
  {
    out << indented{width, "realm"} << _zone->get_realm_id() << " ("
        << _zone->get_realm_name() << ")" << std::endl;
    out << indented{width, "zonegroup"} << _zonegroup->get_id() << " ("
        << _zonegroup->get_name() << ")" << std::endl;
    out << indented{width, "zone"} << _zone->get_id() << " ("
        << _zone->get_name() << ")" << std::endl;
  ...
  void
  _print_formatter(std::ostream& out, Formatter* formatter)
  {
    ...
    bucket_sync_status_info& bucket_sync_info)
{
  ...
  if (!static_cast<rgw::sal::RadosStore*>(driver)
           ->ctl()
           ->bucket->bucket_imports_data(info.bucket, null_yield, dpp())) {
    bucket_sync_info.error = fmt::format(
  ...
  if (r < 0) {
    bucket_sync_info.error = fmt::format(
        "ERROR: failed to get policy handler for bucket ({}): r={}: {}",
        info.bucket.name, r, cpp_strerror(-r));
    return r;
  }
  ...
  if (!source_zone_id.empty()) {
    std::unique_ptr<rgw::sal::Zone> zone;
    int ret = driver->get_zone()->get_zonegroup().get_zone_by_id(
        source_zone_id.id, &zone);
    if (ret < 0) {
      bucket_sync_info.error = fmt::format(
          "Source zone not found in zonegroup {}", zonegroup.get_name());
      return -EINVAL;
    }
    auto c = zone_conn_map.find(source_zone_id);
    if (c == zone_conn_map.end()) {
      bucket_sync_info.error =
          fmt::format("No connection to zone {}", zone->get_name());
      return -EINVAL;
    }
  ...
  for (auto& zone_id : zone_ids) {
    auto z = static_cast<rgw::sal::RadosStore*>(driver)
                 ->svc()
                 ->zone->get_zonegroup()
                 .zones.find(zone_id.id);
    if (z == static_cast<rgw::sal::RadosStore*>(driver)
                 ->svc()
                 ->zone->get_zonegroup()
                 .zones.end()) { /* shouldn't happen */
      continue;
    }
    ...
    for (auto& entry : sources) {
      ...
      if (pipe.source.zone.value_or(rgw_zone_id()) == z->second.id) {
        ...
        bucket_source_sync_status(
            dpp(), static_cast<rgw::sal::RadosStore*>(driver),
            static_cast<rgw::sal::RadosStore*>(driver)->svc()->zone->get_zone(),
            z->second, c->second, info, pipe, source_sync_info);

        bucket_sync_info.source_status_info.emplace_back(
            std::move(source_sync_info));
      }
    }
  }
...
static void
parse_tier_config_param(const string& s, map<string, string, ltstr_nocase>& out)
{
  int level = 0;
  string cur_conf;
  list<string> confs;
...
static int
check_pool_support_omap(const rgw_pool& pool)
{
  librados::IoCtx io_ctx;
  int ret = static_cast<rgw::sal::RadosStore*>(driver)
                ->getRados()
                ->get_rados_handle()
                ->ioctx_create(pool.to_str().c_str(), io_ctx);
  if (ret < 0) {
    ...
    std::unique_ptr<rgw::sal::Bucket>* bucket)
{
  ...
  if (num_shards > (int)static_cast<rgw::sal::RadosStore*>(driver)
                       ->getRados()
...
#define MAX_TOTP_SKEW_HOURS (24 * 7)
  ...
  while (time_ofs_abs < max_skew) {
    ...
    int rc = oath_totp_validate2(
        totp.seed_bin.c_str(), totp.seed_bin.length(), start_time,
        // coverity[store_truncates_time_t:SUPPRESS]
        step_size, time_ofs, 1, nullptr, pins[0].c_str());
    if (rc != OATH_INVALID_OTP) {
      rc = oath_totp_validate2(
          totp.seed_bin.c_str(), totp.seed_bin.length(), start_time,
          // coverity[store_truncates_time_t:SUPPRESS]
          step_size,
          time_ofs - step_size, /* smaller time_ofs moves time forward */
          1, nullptr, pins[1].c_str());
      if (rc != OATH_INVALID_OTP) {
        *pofs = time_ofs - step_size + step_size * totp.window / 2;
        ldpp_dout(dpp(), 20) << "found at time=" << start_time - time_ofs
                             << " time_ofs=" << time_ofs << dendl;
        return 0;
      }
    }
    sign = -sign;
...
static int
trim_sync_error_log(int shard_id, const string& marker, int delay_ms)
{
  auto oid = RGWSyncErrorLogger::get_shard_oid(
      RGW_SYNC_ERROR_LOG_SHARD_PREFIX, shard_id);
  // call cls_log_trim() until it returns -ENODATA
  for (;;) {
    int ret =
        static_cast<rgw::sal::RadosStore*>(driver)->svc()->cls->timelog.trim(
...
static bool
symmetrical_flow_opt(const string& opt)
{
  return (opt == "symmetrical" || opt == "symmetric");
}

static bool
directional_flow_opt(const string& opt)
{
...
template <class T>
static bool
require_opt(std::optional<T> opt, bool extra_check = true)
{
  if (!opt || !extra_check) {
    return false;
  }
...
template <class T>
static bool
require_non_empty_opt(std::optional<T> opt, bool extra_check = true)
{
...
template <class T>
static void
show_result(T& obj, Formatter* formatter, ostream& os)
{
...
public:
  ...
  int
  init(const string& zonegroup_id, const string& zonegroup_name)
  {
    int ret = rgw::read_zonegroup(
        dpp(), null_yield, cfgstore, zonegroup_id, zonegroup_name, zonegroup,
        &zonegroup_writer);
    if (ret < 0) {
  ...
  write_policy()
  {
    if (!b) {
      ...
      if (ret < 0) {
        cerr << "failed to update zonegroup: " << cpp_strerror(-ret)
             << std::endl;
        return -ret;
      }
      return 0;
    ...
    if (ret < 0) {
      cerr << "failed to driver bucket info: " << cpp_strerror(-ret)
           << std::endl;
      return -ret;
    }
    ...
    std::optional<rgw_zone_id>& zone_id)
{
  ...
  std::unique_ptr<rgw::sal::Zone> zone;
  int ret =
      driver->get_zone()->get_zonegroup().get_zone_by_name(*zone_name, &zone);
  if (ret < 0) {
    cerr << "WARNING: cannot find source zone id for name=" << *zone_name
         << std::endl;
...
void
resolve_zone_ids_opt(
    std::optional<vector<string>>& names,
    std::optional<vector<rgw_zone_id>>& ids)
{
  if (!names || ids) {
    return;
  }
  ...
  for (auto& name : *names) {
    ...
    if (ret < 0) {
      cerr << "WARNING: cannot find source zone id for name=" << name
           << std::endl;
...
static vector<rgw_zone_id>
zone_ids_from_str(const string& val)
{
...
class JSONFormatter_PrettyZone : public JSONFormatter {
  class Handler : public JSONEncodeFilter::Handler<rgw_zone_id> {
    void
    encode_json(const char* name, const void* pval, ceph::Formatter* f)
        const override
    {
      auto zone_id = *(static_cast<const rgw_zone_id*>(pval));
      string zone_name;
      std::unique_ptr<rgw::sal::Zone> zone;
      if (driver->get_zone()->get_zonegroup().get_zone_by_id(
              zone_id.id, &zone) == 0) {
        zone_name = zone->get_name();
      } else {
        cerr << "WARNING: cannot find zone name for id=" << zone_id
             << std::endl;
...
public:
  ...
  void*
  get_external_feature_handler(const std::string& feature) override
  {
...
main(int argc, const char** argv)
{
  ...
  auto cct = rgw_global_init(
      &defaults, args, CEPH_ENTITY_TYPE_CLIENT, CODE_ENVIRONMENT_UTILITY, 0);
  ...
  std::string role_name, path, assume_role_doc, policy_name, perm_policy_doc,
      path_prefix, max_session_duration;
  ...
  init_realm_param(
      cct.get(), zonegroup_id, opt_zonegroup_id, "rgw_zonegroup_id");
  ...
  for (std::vector<const char*>::iterator i = args.begin(); i != args.end();) {
    if (ceph_argparse_double_dash(args, i)) {
      ...
      user_ns = val;
        ...
        ceph_argparse_witharg(args, i, &val, "--account-name", (char*)NULL)) {
      account_name = val;
    } else if (
        ceph_argparse_witharg(args, i, &val, "--account-id", (char*)NULL)) {
      account_id = val;
      ...
      if (!max_roles) {
        ...
        ceph_argparse_witharg(args, i, &val, "--max-groups", (char*)NULL)) {
      ...
      if (!max_groups) {
        ...
        return EINVAL;
                   ...
                   args, i, &val, "--max-access-keys", (char*)NULL)) {
      ...
      if (!max_access_keys) {
        ...
        ceph_argparse_witharg(args, i, &val, "--access-key", (char*)NULL)) {
      access_key = val;
    } else if (ceph_argparse_witharg(args, i, &val, "--subuser", (char*)NULL)) {
      subuser = val;
    } else if (ceph_argparse_witharg(
                   args, i, &val, "--secret", "--secret-key", (char*)NULL)) {
      secret_key = val;
    } else if (
        ceph_argparse_witharg(args, i, &val, "-e", "--email", (char*)NULL)) {
      user_email = val;
    } else if (ceph_argparse_witharg(
                   args, i, &val, "-n", "--display-name", (char*)NULL)) {
      display_name = val;
    } else if (
        ceph_argparse_witharg(args, i, &val, "-b", "--bucket", (char*)NULL)) {
      bucket_name = val;
      opt_bucket_name = val;
    } else if (
      ...
      pool = rgw_pool(pool_name);
        ...
        ceph_argparse_witharg(args, i, &val, "-o", "--object", (char*)NULL)) {
      object = val;
    } else if (
        ceph_argparse_witharg(args, i, &val, "--objects-file", (char*)NULL)) {
      objects_file = val;
    } else if (
        ceph_argparse_witharg(args, i, &val, "--object-version", (char*)NULL)) {
      object_version = val;
      ...
      if (key_type_str.compare("swift") == 0) {
        ...
        ceph_argparse_witharg(args, i, &val, "--generate-key", (char*)NULL)) {
      key_type_str = val;
      if (key_type_str.compare("true") == 0) {
        ...
        cerr << "wrong value for --generate-key: " << key_type_str
             << " please specify either true or false" << std::endl;
        exit(1);
      }
      // do nothing
    } else if (ceph_argparse_binary_flag(
                   args, i, &key_active, NULL, "--key-active", (char*)NULL)) {
      key_active_specified = true;
    } else if (ceph_argparse_witharg(args, i, &val, "--job-id", (char*)NULL)) {
      job_id = val;
    } else if (ceph_argparse_binary_flag(
                   args, i, &gen_access_key, NULL, "--gen-access-key",
                   (char*)NULL)) {
      // do nothing
    } else if (ceph_argparse_binary_flag(
                   args, i, &gen_secret_key, NULL, "--gen-secret",
                   (char*)NULL)) {
      // do nothing
    } else if (ceph_argparse_binary_flag(
                   args, i, &show_log_entries, NULL, "--show-log-entries",
                   (char*)NULL)) {
      // do nothing
    } else if (ceph_argparse_binary_flag(
                   args, i, &show_log_sum, NULL, "--show-log-sum",
                   (char*)NULL)) {
      // do nothing
    } else if (ceph_argparse_binary_flag(
                   args, i, &skip_zero_entries, NULL, "--skip-zero-entries",
                   (char*)NULL)) {
      // do nothing
    } else if (ceph_argparse_binary_flag(
                   args, i, &admin, NULL, "--admin", (char*)NULL)) {
      admin_specified = true;
    } else if (ceph_argparse_binary_flag(
                   args, i, &system, NULL, "--system", (char*)NULL)) {
      system_specified = true;
    } else if (ceph_argparse_binary_flag(
                   args, i, &account_root, NULL, "--account-root",
                   (char*)NULL)) {
      account_root_specified = true;
    } else if (ceph_argparse_binary_flag(
                   args, i, &verbose, NULL, "--verbose", (char*)NULL)) {
      // do nothing
    } else if (ceph_argparse_binary_flag(
                   args, i, &staging, NULL, "--staging", (char*)NULL)) {
      // do nothing
    } else if (ceph_argparse_binary_flag(
                   args, i, &commit, NULL, "--commit", (char*)NULL)) {
      // do nothing
    } else if (ceph_argparse_witharg(
                   args, i, &val, "--min-rewrite-size", (char*)NULL)) {
      min_rewrite_size = (uint64_t)atoll(val.c_str());
    } else if (ceph_argparse_witharg(
                   args, i, &val, "--max-rewrite-size", (char*)NULL)) {
      max_rewrite_size = (uint64_t)atoll(val.c_str());
    } else if (ceph_argparse_witharg(
                   args, i, &val, "--min-rewrite-stripe-size", (char*)NULL)) {
      min_rewrite_stripe_size = (uint64_t)atoll(val.c_str());
    } else if (
        ceph_argparse_witharg(args, i, &val, "--max-buckets", (char*)NULL)) {
      ...
      if (!max_buckets) {
        ...
        ceph_argparse_witharg(args, i, &val, "--max-entries", (char*)NULL)) {
      ...
      have_max_size = true;
        ...
        ceph_argparse_witharg(args, i, &val, "--max-objects", (char*)NULL)) {
      ...
      have_max_objects = true;
        ...
        ceph_argparse_witharg(args, i, &val, "--max-read-ops", (char*)NULL)) {
      max_read_ops = (int64_t)strict_strtoll(val.c_str(), 10, &err);
      if (!err.empty()) {
        cerr << "ERROR: failed to parse max read requests: " << err
             << std::endl;
        return EINVAL;
      }
      have_max_read_ops = true;
    } else if (
        ceph_argparse_witharg(args, i, &val, "--max-list-ops", (char*)NULL)) {
      max_list_ops = (int64_t)strict_strtoll(val.c_str(), 10, &err);
      if (!err.empty()) {
        cerr << "ERROR: failed to parse max list requests: " << err
             << std::endl;
        return EINVAL;
      }
      have_max_list_ops = true;
    } else if (
        ceph_argparse_witharg(args, i, &val, "--max-delete-ops", (char*)NULL)) {
      max_delete_ops = (int64_t)strict_strtoll(val.c_str(), 10, &err);
      if (!err.empty()) {
        cerr << "ERROR: failed to parse max delete requests: " << err
             << std::endl;
        return EINVAL;
      }
      have_max_delete_ops = true;
    } else if (
        ceph_argparse_witharg(args, i, &val, "--max-write-ops", (char*)NULL)) {
      max_write_ops = (int64_t)strict_strtoll(val.c_str(), 10, &err);
      if (!err.empty()) {
        cerr << "ERROR: failed to parse max write requests: " << err
             << std::endl;
        return EINVAL;
      }
      have_max_write_ops = true;
    } else if (
        ceph_argparse_witharg(args, i, &val, "--max-read-bytes", (char*)NULL)) {
      ...
      have_max_read_bytes = true;
                   ...
                   args, i, &val, "--max-write-bytes", (char*)NULL)) {
      ...
      have_max_write_bytes = true;
                   ...
                   args, i, &val, "--max-bucket-index-ops", (char*)NULL)) {
      max_bucket_index_ops = (int64_t)strict_strtoll(val.c_str(), 10, &err);
      if (!err.empty()) {
        cerr << "ERROR: failed to parse max bucket index ops: " << err
             << std::endl;
        return EINVAL;
      }
      have_max_bucket_index_ops = true;
    } else if (ceph_argparse_witharg(
                   args, i, &val, "--max-metadata-ops", (char*)NULL)) {
      ...
      have_max_metadata_ops = true;
        ...
        ceph_argparse_witharg(args, i, &val, "--date", "--time", (char*)NULL)) {
      date = val;
      if (end_date.empty())
        end_date = date;
    } else if (ceph_argparse_witharg(
                   args, i, &val, "--start-date", "--start-time", (char*)NULL)) {
      start_date = val;
    } else if (ceph_argparse_witharg(
                   args, i, &val, "--end-date", "--end-time", (char*)NULL)) {
      end_date = val;
    } else if (
        ceph_argparse_witharg(args, i, &val, "--num-shards", (char*)NULL)) {
      ...
      num_shards_specified = true;
                   ...
                   args, i, &val, "--bucket-index-max-shards", (char*)NULL)) {
      bucket_index_max_shards = (int)strict_strtol(val.c_str(), 10, &err);
      if (!err.empty()) {
        cerr << "ERROR: failed to parse bucket-index-max-shards: " << err
             << std::endl;
        return EINVAL;
      }
    } else if (ceph_argparse_witharg(
                   args, i, &val, "--max-concurrent-ios", (char*)NULL)) {
      max_concurrent_ios = (int)strict_strtol(val.c_str(), 10, &err);
      if (!err.empty()) {
        cerr << "ERROR: failed to parse max concurrent ios: " << err
             << std::endl;
        return EINVAL;
      }
    } else if (
        ceph_argparse_witharg(args, i, &val, "--min-age-hours", (char*)NULL)) {
      min_age = std::chrono::hours(atoi(val.c_str()));
    } else if (ceph_argparse_witharg(
                   args, i, &val, "--orphan-stale-secs", (char*)NULL)) {
      orphan_stale_secs = (uint64_t)strict_strtoll(val.c_str(), 10, &err);
      if (!err.empty()) {
        cerr << "ERROR: failed to parse orphan stale secs: " << err
             << std::endl;
        return EINVAL;
      }
      ...
      set_perm = true;
        ...
        ceph_argparse_witharg(args, i, &val, "--temp-url-key", (char*)NULL)) {
      temp_url_keys[0] = val;
      set_temp_url_key = true;
    } else if (ceph_argparse_witharg(
                   args, i, &val, "--temp-url-key2", "--temp-url-key-2",
                   (char*)NULL)) {
      temp_url_keys[1] = val;
      set_temp_url_key = true;
      ...
      if (bucket_id.empty()) {
        ...
        exit(1);
                   ...
                   args, i, &val, "--bucket-new-name", (char*)NULL)) {
      new_bucket_name = val;
    } else if (ceph_argparse_witharg(args, i, &val, "--format", (char*)NULL)) {
      format = val;
      format_arg_passed = true;
    } else if (
        ceph_argparse_witharg(args, i, &val, "--categories", (char*)NULL)) {
      string cat_str = val;
      ...
      for (iter = cat_list.begin(); iter != cat_list.end(); ++iter) {
        categories[*iter] = true;
                   ...
                   args, i, &delete_child_objects, NULL, "--purge-objects",
                   (char*)NULL)) {
      // do nothing
    } else if (ceph_argparse_binary_flag(
                   args, i, &pretty_format, NULL, "--pretty-format",
                   (char*)NULL)) {
      // do nothing
    } else if (ceph_argparse_binary_flag(
                   args, i, &purge_data, NULL, "--purge-data", (char*)NULL)) {
      delete_child_objects = purge_data;
    } else if (ceph_argparse_binary_flag(
                   args, i, &purge_keys, NULL, "--purge-keys", (char*)NULL)) {
      // do nothing
    } else if (ceph_argparse_binary_flag(
                   args, i, &yes_i_really_mean_it, NULL,
                   "--yes-i-really-mean-it", (char*)NULL)) {
      // do nothing
    } else if (ceph_argparse_binary_flag(
                   args, i, &throttle_stat, NULL, "--stat", (char*)NULL)) {
      // do nothing
    } else if (
        ceph_argparse_binary_flag(args, i, &fix, NULL, "--fix", (char*)NULL)) {
      // do nothing
    } else if (ceph_argparse_binary_flag(
                   args, i, &remove_bad, NULL, "--remove-bad", (char*)NULL)) {
      // do nothing
    } else if (ceph_argparse_binary_flag(
                   args, i, &check_head_obj_locator, NULL,
                   "--check-head-obj-locator", (char*)NULL)) {
      // do nothing
    } else if (ceph_argparse_binary_flag(
                   args, i, &check_objects, NULL, "--check-objects",
                   (char*)NULL)) {
      // do nothing
    } else if (ceph_argparse_binary_flag(
                   args, i, &sync_stats, NULL, "--sync-stats", (char*)NULL)) {
      // do nothing
    } else if (ceph_argparse_binary_flag(
                   args, i, &reset_stats, NULL, "--reset-stats", (char*)NULL)) {
      // do nothing
    } else if (ceph_argparse_binary_flag(
                   args, i, &include_all, NULL, "--include-all", (char*)NULL)) {
      // do nothing
    } else if (ceph_argparse_binary_flag(
                   args, i, &allow_unordered, NULL, "--allow-unordered",
                   (char*)NULL)) {
      // do nothing
    } else if (ceph_argparse_binary_flag(
                   args, i, &extra_info, NULL, "--extra-info", (char*)NULL)) {
      // do nothing
    } else if (ceph_argparse_binary_flag(
                   args, i, &bypass_gc, NULL, "--bypass-gc", (char*)NULL)) {
      // do nothing
    } else if (ceph_argparse_binary_flag(
                   args, i, &warnings_only, NULL, "--warnings-only",
                   (char*)NULL)) {
      // do nothing
    } else if (ceph_argparse_binary_flag(
                   args, i, &inconsistent_index, NULL, "--inconsistent-index",
                   (char*)NULL)) {
      // do nothing
      ...
      dump_keys = true;
                   ...
                   args, i, &placement_inline_data, NULL,
      ...
      infile = val;
        ...
        ceph_argparse_witharg(args, i, &val, "--metadata-key", (char*)NULL)) {
      metadata_key = val;
    } else if (ceph_argparse_witharg(args, i, &val, "--marker", (char*)NULL)) {
      marker = val;
    } else if (
        ceph_argparse_witharg(args, i, &val, "--start-marker", (char*)NULL)) {
      start_marker = val;
    } else if (
        ceph_argparse_witharg(args, i, &val, "--end-marker", (char*)NULL)) {
      end_marker = val;
    } else if (
        ceph_argparse_witharg(args, i, &val, "--quota-scope", (char*)NULL)) {
      quota_scope = val;
    } else if (ceph_argparse_witharg(
                   args, i, &val, "--ratelimit-scope", (char*)NULL)) {
      ratelimit_scope = val;
    } else if (
        ceph_argparse_witharg(args, i, &val, "--index-type", (char*)NULL)) {
      ...
      opt_log_type = l;
                   ...
                   args, i, &is_master_int, NULL, "--master", (char*)NULL)) {
      is_master = (bool)is_master_int;
      is_master_set = true;
    } else if (ceph_argparse_binary_flag(
                   args, i, &set_default, NULL, "--default", (char*)NULL)) {
      /* do nothing */
    } else if (
        ceph_argparse_witharg(args, i, &val, "--redirect-zone", (char*)NULL)) {
      redirect_zone = val;
      redirect_zone_set = true;
    } else if (ceph_argparse_binary_flag(
                   args, i, &read_only_int, NULL, "--read-only", (char*)NULL)) {
      read_only = (bool)read_only_int;
      is_read_only_set = true;
    } else if (
        ceph_argparse_witharg(args, i, &val, "--master-zone", (char*)NULL)) {
      master_zone = val;
      ...
      g_conf().set_val("rgw_realm_id", val);
        ...
        ceph_argparse_witharg(args, i, &val, "--realm-new-name", (char*)NULL)) {
      realm_new_name = val;
    } else if (
        ceph_argparse_witharg(args, i, &val, "--zonegroup-id", (char*)NULL)) {
      zonegroup_id = val;
      opt_zonegroup_id = val;
      g_conf().set_val("rgw_zonegroup_id", val);
    } else if (ceph_argparse_witharg(
                   args, i, &val, "--zonegroup-new-name", (char*)NULL)) {
      zonegroup_new_name = val;
    } else if (
        ceph_argparse_witharg(args, i, &val, "--placement-id", (char*)NULL)) {
      placement_id = val;
    } else if (
        ceph_argparse_witharg(args, i, &val, "--storage-class", (char*)NULL)) {
      ...
      g_conf().set_val("rgw_zone_id", val);
        ...
        ceph_argparse_witharg(args, i, &val, "--zone-new-name", (char*)NULL)) {
      zone_new_name = val;
      ...
      get_str_list(val, sync_from);
        ...
        ceph_argparse_witharg(args, i, &val, "--sync-from-rm", (char*)NULL)) {
      get_str_list(val, sync_from_rm);
    } else if (ceph_argparse_binary_flag(
                   args, i, &tmp_int, NULL, "--sync-from-all", (char*)NULL)) {
      sync_from_all = (bool)tmp_int;
      sync_from_all_specified = true;
    } else if (
        ceph_argparse_witharg(args, i, &val, "--source-zone", (char*)NULL)) {
      source_zone_name = val;
      opt_source_zone_name = val;
    } else if (
        ceph_argparse_witharg(args, i, &val, "--source-zone-id", (char*)NULL)) {
      opt_source_zone_id = val;
    } else if (ceph_argparse_witharg(args, i, &val, "--dest-zone", (char*)NULL)) {
      opt_dest_zone_name = val;
    } else if (
        ceph_argparse_witharg(args, i, &val, "--dest-zone-id", (char*)NULL)) {
      opt_dest_zone_id = val;
    } else if (ceph_argparse_witharg(args, i, &val, "--tier-type", (char*)NULL)) {
      tier_type = val;
      tier_type_specified = true;
    } else if (
        ceph_argparse_witharg(args, i, &val, "--tier-config", (char*)NULL)) {
      parse_tier_config_param(val, tier_config_add);
    } else if (
        ceph_argparse_witharg(args, i, &val, "--tier-config-rm", (char*)NULL)) {
      parse_tier_config_param(val, tier_config_rm);
    } else if (
        ceph_argparse_witharg(args, i, &val, "--index-pool", (char*)NULL)) {
      index_pool = val;
    } else if (ceph_argparse_witharg(args, i, &val, "--data-pool", (char*)NULL)) {
      data_pool = val;
    } else if (ceph_argparse_witharg(
                   args, i, &val, "--data-extra-pool", (char*)NULL)) {
      data_extra_pool = val;
    } else if (ceph_argparse_witharg(
                   args, i, &val, "--placement-index-type", (char*)NULL)) {
      if (val == "normal") {
        ...
        placement_index_type =
            (rgw::BucketIndexType)strict_strtol(val.c_str(), 10, &err);
        if (!err.empty()) {
          cerr << "ERROR: failed to parse index type index: " << err
               << std::endl;
          return EINVAL;
        }
      }
      index_type_specified = true;
        ...
        ceph_argparse_witharg(args, i, &val, "--compression", (char*)NULL)) {
      ...
      path = val;
                   ...
                   args, i, &val, "--assume-role-policy-doc", (char*)NULL)) {
      assume_role_doc = val;
    } else if (
        ceph_argparse_witharg(args, i, &val, "--policy-name", (char*)NULL)) {
      policy_name = val;
    } else if (
        ceph_argparse_witharg(args, i, &val, "--policy-doc", (char*)NULL)) {
      perm_policy_doc = val;
    } else if (
        ceph_argparse_witharg(args, i, &val, "--path-prefix", (char*)NULL)) {
      path_prefix = val;
    } else if (
        ceph_argparse_witharg(args, i, &val, "--policy-arn", (char*)NULL)) {
      policy_arn = val;
    } else if (ceph_argparse_witharg(
                   args, i, &val, "--max-session-duration", (char*)NULL)) {
      max_session_duration = val;
    } else if (
        ceph_argparse_witharg(args, i, &val, "--description", (char*)NULL)) {
      description = val;
    } else if (
        ceph_argparse_witharg(args, i, &val, "--totp-serial", (char*)NULL)) {
      totp_serial = val;
      ...
      totp_seed = val;
        ...
        ceph_argparse_witharg(args, i, &val, "--totp-seed-type", (char*)NULL)) {
      totp_seed_type = val;
    } else if (
        ceph_argparse_witharg(args, i, &val, "--totp-seconds", (char*)NULL)) {
      totp_seconds = atoi(val.c_str());
    } else if (
        ceph_argparse_witharg(args, i, &val, "--totp-window", (char*)NULL)) {
      totp_window = atoi(val.c_str());
    } else if (
        ceph_argparse_witharg(args, i, &val, "--trim-delay-ms", (char*)NULL)) {
      trim_delay_ms = atoi(val.c_str());
    } else if (ceph_argparse_witharg(args, i, &val, "--topic", (char*)NULL)) {
      topic_name = val;
    } else if (ceph_argparse_witharg(
                   args, i, &val, "--notification-id", (char*)NULL)) {
      notification_id = val;
    } else if (
        ceph_argparse_witharg(args, i, &val, "--subscription", (char*)NULL)) {
      sub_name = val;
      ...
      opt_flow_type = val;
                   ...
                   args, i, &val, "--zones", "--zone-names", (char*)NULL)) {
      vector<string> v;
      get_str_vec(val, v);
      ...
      opt_zone_ids = zone_ids_from_str(val);
                   ...
                   args, i, &val, "--source-zones", "--source-zone-names",
                   (char*)NULL)) {
      vector<string> v;
      get_str_vec(val, v);
      opt_source_zone_names = std::move(v);
    } else if (ceph_argparse_witharg(
                   args, i, &val, "--source-zone-ids", (char*)NULL)) {
      opt_source_zone_ids = zone_ids_from_str(val);
    } else if (ceph_argparse_witharg(
                   args, i, &val, "--dest-zones", "--dest-zone-names",
                   (char*)NULL)) {
      vector<string> v;
      get_str_vec(val, v);
      opt_dest_zone_names = std::move(v);
    } else if (
        ceph_argparse_witharg(args, i, &val, "--dest-zone-ids", (char*)NULL)) {
      ...
      opt_pipe_id = val;
        ...
        ceph_argparse_witharg(args, i, &val, "--source-tenant", (char*)NULL)) {
      opt_source_tenant = val;
    } else if (
        ceph_argparse_witharg(args, i, &val, "--source-bucket", (char*)NULL)) {
      opt_source_bucket_name = val;
    } else if (ceph_argparse_witharg(
                   args, i, &val, "--source-bucket-id", (char*)NULL)) {
      opt_source_bucket_id = val;
    } else if (
        ceph_argparse_witharg(args, i, &val, "--dest-tenant", (char*)NULL)) {
      opt_dest_tenant = val;
    } else if (
        ceph_argparse_witharg(args, i, &val, "--dest-bucket", (char*)NULL)) {
      opt_dest_bucket_name = val;
    } else if (
        ceph_argparse_witharg(args, i, &val, "--dest-bucket-id", (char*)NULL)) {
      opt_dest_bucket_id = val;
    } else if (ceph_argparse_witharg(
                   args, i, &val, "--effective-zone-name", "--effective-zone",
                   (char*)NULL)) {
      opt_effective_zone_name = val;
    } else if (ceph_argparse_witharg(
                   args, i, &val, "--effective-zone-id", (char*)NULL)) {
      ...
      opt_mode = val;
        ...
        ceph_argparse_witharg(args, i, &val, "--dest-owner", (char*)NULL)) {
      opt_dest_owner.emplace(val);
      opt_dest_owner = val;
    } else if (
        ceph_argparse_witharg(args, i, &val, "--retry-delay-ms", (char*)NULL)) {
      opt_retry_delay_ms = std::chrono::milliseconds(atoi(val.c_str()));
    } else if (
        ceph_argparse_witharg(args, i, &val, "--timeout-sec", (char*)NULL)) {
      opt_timeout_sec = std::chrono::seconds(atoi(val.c_str()));
    } else if (ceph_argparse_witharg(
                   args, i, &val, "--inject-error-at", (char*)NULL)) {
      inject_error_at = val;
    } else if (ceph_argparse_witharg(
                   args, i, &val, "--inject-error-code", (char*)NULL)) {
      inject_error_code = atoi(val.c_str());
    } else if (ceph_argparse_witharg(
                   args, i, &val, "--inject-abort-at", (char*)NULL)) {
      inject_abort_at = val;
    } else if (ceph_argparse_witharg(
                   args, i, &val, "--inject-delay-at", (char*)NULL)) {
      inject_delay_at = val;
    } else if (ceph_argparse_witharg(
                   args, i, &val, "--inject-delay-ms", (char*)NULL)) {
      inject_delay = std::chrono::milliseconds(atoi(val.c_str()));
    } else if (ceph_argparse_binary_flag(
                   args, i, &detail, NULL, "--detail", (char*)NULL)) {
      // do nothing
      ...
      script_package = val;
                   ...
                   args, i, &allow_compilation, NULL, "--allow-compilation",
                   (char*)NULL)) {
      // do nothing
    } else if (
        ceph_argparse_witharg(args, i, &val, "--rgw-obj-fs", (char*)NULL)) {
      rgw_obj_fs = val;
    } else if (
        ceph_argparse_witharg(args, i, &val, "--enable-feature", (char*)NULL)) {
      if (!rgw::zone_features::supports(val)) {
        std::cerr << "ERROR: Cannot enable unrecognized zone feature \"" << val
                  << "\"" << std::endl;
        return EINVAL;
      }
      enable_features.insert(val);
    } else if (ceph_argparse_witharg(
                   args, i, &val, "--disable-feature", (char*)NULL)) {
      disable_features.insert(val);
    } else if (
        ceph_argparse_witharg(args, i, &val, "--restore-status", (char*)NULL)) {
      restore_status_filter = val;
    } else if (ceph_argparse_binary_flag(
                   args, i, &show_restore_stats, NULL, "--show-restore-stats",
                   (char*)NULL)) {
      // do nothing
  ...
  if (args.empty()) {
    ...
    bool localzonegroup_op =
        ((opt_cmd == OPT::PERIOD_UPDATE && commit) ||
         (opt_cmd == OPT::PERIOD_PULL && url.empty()));

    std::set<OPT> raw_storage_ops_list = {
        OPT::ZONEGROUP_ADD,
    ...
    std::set<OPT> gc_ops_list = {
        OPT::GC_LIST,        OPT::GC_PROCESS,
        OPT::OBJECT_RM,
        OPT::BUCKET_RM, // --purge-objects
        OPT::USER_RM, // --purge-data
        OPT::OBJECTS_EXPIRE, OPT::OBJECTS_EXPIRE_STALE_RM,
        OPT::LC_PROCESS,     OPT::BUCKET_SYNC_RUN,
        OPT::DATA_SYNC_RUN,  OPT::BUCKET_REWRITE,
        OPT::OBJECT_REWRITE};

    raw_storage_op =
        (raw_storage_ops_list.find(opt_cmd) != raw_storage_ops_list.end() ||
         raw_period_update || raw_period_pull);
    bool need_cache = readonly_ops_list.find(opt_cmd) ==
                      readonly_ops_list.end();
    bool need_gc = (gc_ops_list.find(opt_cmd) != gc_ops_list.end()) &&
                   !bypass_gc;
    ...
    if (raw_storage_op) {
      ...
      driver = DriverManager::get_raw_storage(
          dpp(), g_ceph_context, cfg, context_pool, *site, cfgstore.get());
    } else {
      ...
      driver = DriverManager::get_storage(
          dpp(), g_ceph_context, cfg, context_pool, *site, false, false, false,
          false, false, false, false, false,
          false, // No background tasks!
          null_yield, cfgstore.get(), need_cache && g_conf()->rgw_cache_enabled,
          need_gc, true /* admin */);
    }
    if (!driver) {
    ...
    init_optional_bucket(
        opt_source_bucket, opt_source_tenant, opt_source_bucket_name,
        opt_source_bucket_id);
    init_optional_bucket(
        opt_dest_bucket, opt_dest_tenant, opt_dest_bucket_name,
        opt_dest_bucket_id);
    ...
    if (tenant.empty()) {
      ...
      if (rgw::sal::User::empty(user) && opt_cmd != OPT::ROLE_CREATE &&
          opt_cmd != OPT::ROLE_DELETE && opt_cmd != OPT::ROLE_GET &&
          opt_cmd != OPT::ROLE_TRUST_POLICY_MODIFY &&
          opt_cmd != OPT::ROLE_LIST && opt_cmd != OPT::ROLE_POLICY_PUT &&
          opt_cmd != OPT::ROLE_POLICY_LIST && opt_cmd != OPT::ROLE_POLICY_GET &&
          ...
          opt_cmd != OPT::ROLE_UPDATE && opt_cmd != OPT::RESHARD_ADD &&
          ...
          opt_cmd != OPT::PUBSUB_TOPIC_DUMP && opt_cmd != OPT::SCRIPT_PUT &&
          opt_cmd != OPT::SCRIPT_GET && opt_cmd != OPT::SCRIPT_RM &&
          opt_cmd != OPT::ACCOUNT_CREATE && opt_cmd != OPT::ACCOUNT_MODIFY &&
          opt_cmd != OPT::ACCOUNT_GET && opt_cmd != OPT::ACCOUNT_STATS &&
          opt_cmd != OPT::ACCOUNT_RM && opt_cmd != OPT::ACCOUNT_LIST) {
    ...
    if ((!access_key.empty()) &&
        (gen_access_key || generate_key == OPTION_SET_TRUE)) {
      cerr << "ERROR: key parameter conflict, --access-key & "
              "--gen-access-key/generate-key"
           << std::endl;
      return EINVAL;
    }
    if ((!secret_key.empty()) &&
        (gen_secret_key || generate_key == OPTION_SET_TRUE)) {
      cerr << "ERROR: key parameter conflict, --secret & "
              "--gen-secret/generate-key"
           << std::endl;
      return EINVAL;
    }
    if (generate_key == OPTION_SET_FALSE) {
      if ((!access_key.empty()) || gen_access_key || (!secret_key.empty()) ||
          gen_secret_key) {
        cerr << "ERROR: key parameter conflict, if --generate-key is not set "
                "so no other key parameters can be set"
             << std::endl;
        return EINVAL;
      }
    }
  ...
  if (!source_zone_name.empty()) {
    std::unique_ptr<rgw::sal::Zone> zone;
    if (driver->get_zone()->get_zonegroup().get_zone_by_name(
            source_zone_name, &zone) < 0) {
      cerr << "WARNING: cannot find source zone id for name="
           << source_zone_name << std::endl;
  ...
  if (raw_storage_op) {
    ...
    case OPT::PERIOD_DELETE: {
      ...
      if (ret < 0) {
        cerr << "ERROR: couldn't delete period: " << cpp_strerror(-ret)
             << std::endl;
        return -ret;
      }
    ...
    case OPT::PERIOD_GET: {
      ...
      if (staging) {
        RGWRealm realm;
        int ret = rgw::read_realm(
            dpp(), null_yield, cfgstore.get(), realm_id, realm_name, realm);
        if (ret < 0) {
          cerr << "failed to load realm: " << cpp_strerror(-ret) << std::endl;
          return -ret;
        }
      ...
      RGWPeriod period;
      int ret =
          cfgstore->read_period(dpp(), null_yield, period_id, epoch, period);
      if (ret < 0) {
    ...
    case OPT::PERIOD_LIST: {
      ...
      do {
        int ret = cfgstore->list_period_ids(
            dpp(), null_yield, listing.next, period_ids, listing);
        if (ret < 0) {
          std::cerr << "failed to list periods: " << cpp_strerror(-ret)
                    << std::endl;
          return -ret;
        }
    ...
    case OPT::PERIOD_UPDATE: {
      int ret = update_period(
          cfgstore.get(), realm_id, realm_name, period_epoch, commit, remote,
          url, opt_region, access_key, secret_key, formatter.get(),
    ...
    case OPT::PERIOD_PULL: {
      ...
      RGWPeriod period;
      ret = do_period_pull(
          cfgstore.get(), nullptr, url, opt_region, access_key, secret_key,
          realm_id, realm_name, period_id, period_epoch, &period);
      if (ret < 0) {
    ...
    case OPT::GLOBAL_RATELIMIT_DISABLE: {
      if (realm_id.empty()) {
        if (!realm_name.empty()) {
          ...
          int ret =
              cfgstore->read_realm_id(dpp(), null_yield, realm_name, realm_id);
          if (ret < 0) {
      ...
      int ret = cfgstore->read_period_config(
          dpp(), null_yield, realm_id, period_config);
      if (ret < 0 && ret != -ENOENT) {
        cerr << "ERROR: failed to read period config: " << cpp_strerror(-ret)
             << std::endl;
        return -ret;
      }
      bool ratelimit_configured = true;
      ...
      if (ratelimit_scope == "bucket") {
        ratelimit_configured = set_ratelimit_info(
            period_config.bucket_ratelimit, opt_cmd, max_read_ops,
            max_write_ops, max_list_ops, max_delete_ops, max_read_bytes,
            max_write_bytes, have_max_read_ops, have_max_write_ops,
            have_max_list_ops, have_max_delete_ops, have_max_read_bytes,
            have_max_write_bytes);
        encode_json(
            "bucket_ratelimit", period_config.bucket_ratelimit, formatter.get());
      } else if (ratelimit_scope == "user") {
        ratelimit_configured = set_ratelimit_info(
            period_config.user_ratelimit, opt_cmd, max_read_ops, max_write_ops,
            max_list_ops, max_delete_ops, max_read_bytes, max_write_bytes,
            have_max_read_ops, have_max_write_ops, have_max_list_ops,
            have_max_delete_ops, have_max_read_bytes, have_max_write_bytes);
        encode_json(
            "user_ratelimit", period_config.user_ratelimit, formatter.get());
      } else if (ratelimit_scope == "anonymous") {
        ratelimit_configured = set_ratelimit_info(
            period_config.anon_ratelimit, opt_cmd, max_read_ops, max_write_ops,
            max_list_ops, max_delete_ops, max_read_bytes, max_write_bytes,
            have_max_read_ops, have_max_write_ops, have_max_list_ops,
            have_max_delete_ops, have_max_read_bytes, have_max_write_bytes);
        encode_json(
            "anonymous_ratelimit", period_config.anon_ratelimit,
            formatter.get());
      } else if (
          ratelimit_scope.empty() && opt_cmd == OPT::GLOBAL_RATELIMIT_GET) {
        ...
        cerr << "ERROR: invalid rate limit scope specification. Please specify "
                "either --ratelimit-scope=bucket, or --ratelimit-scope=user or "
      ...
      if (opt_cmd != OPT::GLOBAL_RATELIMIT_GET) {
        ...
        constexpr bool exclusive = false;
        ret = cfgstore->write_period_config(
            dpp(), null_yield, exclusive, realm_id, period_config);
        if (ret < 0) {
          cerr << "ERROR: failed to write period config: " << cpp_strerror(-ret)
               << std::endl;
          return -ret;
        }
        if (!realm_id.empty()) {
          cout
              << "Global ratelimit changes saved. Use 'period update' to apply "
                 "them to the staging period, and 'period commit' to commit "
                 "the "
                 "new period."
              << std::endl;
        } else {
    ...
    case OPT::GLOBAL_QUOTA_DISABLE: {
      ...
      if (quota_scope == "bucket") {
        set_quota_info(
            period_config.quota.bucket_quota, opt_cmd, max_size, max_objects,
            have_max_size, have_max_objects);
        encode_json(
            "bucket quota", period_config.quota.bucket_quota, formatter.get());
      } else if (quota_scope == "user") {
        set_quota_info(
            period_config.quota.user_quota, opt_cmd, max_size, max_objects,
            have_max_size, have_max_objects);
        encode_json(
            "user quota", period_config.quota.user_quota, formatter.get());
    ...
    case OPT::REALM_CREATE: {
      ...
      int ret = rgw::create_realm(
          dpp(), null_yield, cfgstore.get(), exclusive, realm);
      if (ret < 0) {
        cerr << "ERROR: couldn't create realm " << realm_name << ": "
             << cpp_strerror(-ret) << std::endl;
        return -ret;
      }
      ...
      if (set_default) {
        ...
        if (ret < 0) {
          cerr << "failed to set realm " << realm_name
               << " as default: " << cpp_strerror(-ret) << std::endl;
        }
      }
    ...
    case OPT::REALM_GET: {
      ...
      if (ret < 0) {
        if (ret == -ENOENT && realm_name.empty() && realm_id.empty()) {
          cerr << "missing realm name or id, or default realm not found"
               << std::endl;
        } else {
          cerr << "failed to load realm: " << cpp_strerror(-ret) << std::endl;
        }
    ...
    case OPT::REALM_GET_DEFAULT: {
      ...
      if (ret == -ENOENT) {
        ...
        cerr << "Error reading default realm: " << cpp_strerror(-ret)
             << std::endl;
        return -ret;
      }
    ...
    case OPT::REALM_LIST: {
      ...
      int ret = cfgstore->read_default_realm_id(dpp(), null_yield, default_id);
      if (ret < 0 && ret != -ENOENT) {
        cerr << "could not determine default realm: " << cpp_strerror(-ret)
             << std::endl;
      }
      ...
      do {
        ret = cfgstore->list_realm_names(
            dpp(), null_yield, listing.next, names, listing);
        if (ret < 0) {
          std::cerr << "failed to list realms: " << cpp_strerror(-ret)
                    << std::endl;
          return -ret;
        }
    ...
    case OPT::REALM_LIST_PERIODS: {
      ...
      while (!period_id.empty()) {
        RGWPeriod period;
        ret = cfgstore->read_period(
            dpp(), null_yield, period_id, std::nullopt, period);
        if (ret < 0) {
    ...
    case OPT::REALM_SET: {
      ...
      if (new_realm) {
        ...
        constexpr bool exclusive = true;
        ret = rgw::create_realm(
            dpp(), null_yield, cfgstore.get(), exclusive, realm);
        if (ret < 0) {
          cerr << "ERROR: couldn't create new realm: " << cpp_strerror(-ret)
               << std::endl;
          return 1;
        }
      } else {
        ...
        if (ret < 0) {
          cerr << "ERROR: couldn't write realm info: " << cpp_strerror(-ret)
               << std::endl;
          return 1;
        }
      }
    ...
    case OPT::REALM_DEFAULT: {
      ...
      ret = rgw::set_default_realm(dpp(), null_yield, cfgstore.get(), realm);
      if (ret < 0) {
        cerr << "failed to set realm as default: " << cpp_strerror(-ret)
             << std::endl;
        return -ret;
      }
    } break;
    case OPT::REALM_DEFAULT_RM:
      if (int ret = cfgstore->delete_default_realm_id(dpp(), null_yield);
          ret < 0) {
        cerr << "failed to remove default realm: " << cpp_strerror(-ret)
             << std::endl;
        return -ret;
      }
      break;
    case OPT::REALM_PULL: {
      ...
      JSONParser p;
      int ret =
          send_to_url(url, opt_region, access_key, secret_key, info, bl, p);
      if (ret < 0) {
        cerr << "request failed: " << cpp_strerror(-ret) << std::endl;
        if (ret == -EACCES) {
      ...
      if (!current_period.empty()) {
        ...
        ret = do_period_pull(
            cfgstore.get(), nullptr, url, opt_region, access_key, secret_key,
            realm_id, realm_name, current_period, "", &period);
        if (ret < 0) {
    ...
    case OPT::ZONEGROUP_ADD: {
      ...
      std::unique_ptr<rgw::sal::ZoneGroupWriter> zonegroup_writer;
      int ret = rgw::read_zonegroup(
          dpp(), null_yield, cfgstore.get(), zonegroup_id, zonegroup_name,
      ...
      ret = rgw::read_zone(
          dpp(), null_yield, cfgstore.get(), zone_id, zone_name, zone_params,
          &zone_writer);
      if (ret < 0) {
      ...
      if (zone_params.realm_id != zonegroup.realm_id) {
        if (!zone_params.realm_id.empty()) {
          cerr << "WARNING: overwriting zone realm_id=" << zone_params.realm_id
               << " to match zonegroup realm_id=" << zonegroup.realm_id
               << std::endl;
        }
      ...
      if (need_zone_update) {
        ...
        if (ret < 0) {
          cerr << "failed to save zone info: " << cpp_strerror(-ret)
               << std::endl;
          return -ret;
        }
      }
      ...
      const bool* psync_from_all =
          (sync_from_all_specified ? &sync_from_all : nullptr);
      const string* predirect_zone =
          (redirect_zone_set ? &redirect_zone : nullptr);

      // validate --tier-type if specified
      const string* ptier_type = (tier_type_specified ? &tier_type : nullptr);
      if (ptier_type) {
        auto sync_mgr = static_cast<rgw::sal::RadosStore*>(driver)
                            ->svc()
                            ->sync_modules->get_manager();
      ...
      ret = rgw::add_zone_to_group(
          dpp(), zonegroup, zone_params, pis_master, pread_only, endpoints,
          ptier_type, psync_from_all, sync_from, sync_from_rm, predirect_zone,
          bucket_index_max_shards, enable_features, disable_features);
      if (ret < 0) {
        return -ret;
      }
    ...
    case OPT::ZONEGROUP_CREATE: {
      ...
      if (zonegroup.enabled_features.empty()) { // enable features by default
        zonegroup.enabled_features.insert(
            rgw::zone_features::enabled.begin(),
      ...
      for (const auto& feature : disable_features) {
        ...
        if (i == zonegroup.enabled_features.end()) {
          ldout(cct, 1) << "WARNING: zone feature \"" << feature
                        << "\" was not enabled in zonegroup " << zonegroup_name
                        << dendl;
          continue;
        }
      ...
      constexpr bool exclusive = true;
      ret = rgw::create_zonegroup(
          dpp(), null_yield, cfgstore.get(), exclusive, zonegroup);
      if (ret < 0) {
        cerr << "failed to create zonegroup " << zonegroup_name << ": "
             << cpp_strerror(-ret) << std::endl;
        return -ret;
      }

      if (set_default) {
        ret = rgw::set_default_zonegroup(
            dpp(), null_yield, cfgstore.get(), zonegroup);
        if (ret < 0) {
          cerr << "failed to set zonegroup " << zonegroup_name
               << " as default: " << cpp_strerror(-ret) << std::endl;
        }
      }
    ...
    case OPT::ZONEGROUP_DEFAULT: {
      ...
      ret = rgw::set_default_zonegroup(
          dpp(), null_yield, cfgstore.get(), zonegroup);
      if (ret < 0) {
        cerr << "failed to set zonegroup as default: " << cpp_strerror(-ret)
             << std::endl;
        return -ret;
      }
    } break;
    case OPT::ZONEGROUP_DELETE: {
      ...
      ret = writer->remove(dpp(), null_yield);
      if (ret < 0) {
        cerr << "ERROR: couldn't delete zonegroup: " << cpp_strerror(-ret)
             << std::endl;
        return -ret;
      }
    } break;
    ...
    case OPT::ZONEGROUP_LIST: {
      ...
      int ret = rgw::read_zonegroup(
          dpp(), null_yield, cfgstore.get(), {}, {}, default_zonegroup);
      if (ret < 0 && ret != -ENOENT) {
        cerr << "could not determine default zonegroup: " << cpp_strerror(-ret)
             << std::endl;
      }
      ...
      std::array<std::string, 1000> names; // list in pages of 1000
      do {
        ret = cfgstore->list_zonegroup_names(
            dpp(), null_yield, listing.next, names, listing);
        if (ret < 0) {
          std::cerr << "failed to list zonegroups: " << cpp_strerror(-ret)
                    << std::endl;
          return -ret;
        }
        for (const auto& name : listing.entries) {
    ...
    case OPT::ZONEGROUP_MODIFY: {
      ...
      if (!realm_id.empty()) {
        ...
        ret = cfgstore->read_realm_id(
            dpp(), null_yield, realm_name, zonegroup.realm_id);
        if (ret < 0) {
    ...
    case OPT::ZONEGROUP_SET: {
      ...
      int ret = rgw::read_realm(
          dpp(), null_yield, cfgstore.get(), realm_id, realm_name, realm);
      bool default_realm_not_exist =
          (ret == -ENOENT && realm_id.empty() && realm_name.empty());
      ...
      for (const auto& feature : zonegroup.enabled_features) {
        if (!rgw::zone_features::supports(feature)) {
          std::cerr << "ERROR: Unrecognized zonegroup feature \"" << feature
                    << "\"" << std::endl;
          return EINVAL;
        }
      }
      for (const auto& [name, zone] : zonegroup.zones) {
        ...
        for (const auto& feature : zone.supported_features) {
          if (!rgw::zone_features::supports(feature)) {
            std::cerr << "ERROR: Unrecognized zone feature \"" << feature
                      << "\" in zone " << zone.name << std::endl;
            return EINVAL;
          }
        }
      ...
      ret = rgw::create_zonegroup(
          dpp(), null_yield, cfgstore.get(), exclusive, zonegroup);
      if (ret < 0) {
        cerr << "ERROR: couldn't create zonegroup info: " << cpp_strerror(-ret)
             << std::endl;
        return 1;
      }
    ...
    case OPT::ZONEGROUP_REMOVE: {
      ...
      ret = writer->write(dpp(), null_yield, zonegroup);
      if (ret < 0) {
        cerr << "failed to write zonegroup: " << cpp_strerror(-ret)
             << std::endl;
        return -ret;
      }
    ...
    case OPT::ZONEGROUP_RENAME: {
      ...
      if (ret < 0) {
        cerr << "failed to rename zonegroup: " << cpp_strerror(-ret)
             << std::endl;
        return -ret;
      }
    } break;
    case OPT::ZONEGROUP_PLACEMENT_LIST: {
      ...
      encode_json(
          "placement_targets", zonegroup.placement_targets, formatter.get());
      formatter->flush(cout);
    } break;
    ...
    case OPT::ZONEGROUP_PLACEMENT_DEFAULT: {
      ...
      if (opt_cmd == OPT::ZONEGROUP_PLACEMENT_ADD ||
        ...
        RGWZoneGroupPlacementTarget& target =
            zonegroup.placement_targets[placement_id];
        if (!tags.empty()) {
          target.tags.clear();
          for (auto& t : tags) {
        ...
        if (tier_class) {
          if (tier_config_add.size() > 0) {
            ...
            for (auto add : tier_config_add) {
              ...
              if (r < 0) {
                cerr << "ERROR: failed to set configurable: " << add
                     << std::endl;
                return EINVAL;
              }
            }
            ...
            if (r < 0) {
              cerr << "ERROR: failed to update tier_config options"
                   << std::endl;
            }
          }
        ...
        if (zonegroup.default_placement.empty()) {
          zonegroup.default_placement.init(
              rule.name, RGW_STORAGE_CLASS_STANDARD);
        }
        ...
        if (!opt_storage_class || opt_storage_class->empty()) {
          ...
          if (iter != zonegroup.placement_targets.end()) {
            RGWZoneGroupPlacementTarget& info =
                zonegroup.placement_targets[placement_id];
    ...
    case OPT::ZONE_CREATE: {
      ...
      if (!zonegroup_id.empty() || !zonegroup_name.empty()) {
        int ret = rgw::read_zonegroup(
            ...
            zonegroup, &zonegroup_writer);
        if (ret < 0) {
          cerr << "failed to load zonegroup " << zonegroup_name << ": "
               << cpp_strerror(-ret) << std::endl;
          return -ret;
        }
        if (realm_id.empty() && realm_name.empty()) {
      ...
      constexpr bool exclusive = true;
      int ret = rgw::create_zone(
          dpp(), null_yield, cfgstore.get(), exclusive, zone_params);
      if (ret < 0) {
        cerr << "failed to create zone " << zone_name << ": "
             << cpp_strerror(-ret) << std::endl;
        return -ret;
      }
      ...
      if (zonegroup_writer) {
        ...
        ret = zonegroup_writer->write(dpp(), null_yield, zonegroup);
        if (ret < 0) {
          cerr << "failed to add zone " << zone_name << " to zonegroup "
               << zonegroup.get_name() << ": " << cpp_strerror(-ret)
               << std::endl;
          return -ret;
        }
      }
      ...
      if (set_default) {
        ret = rgw::set_default_zone(
            dpp(), null_yield, cfgstore.get(), zone_params);
        if (ret < 0) {
          cerr << "failed to set zone " << zone_name
               << " as default: " << cpp_strerror(-ret) << std::endl;
        }
      }
    ...
    case OPT::ZONE_DEFAULT: {
      ...
      ret =
          rgw::set_default_zone(dpp(), null_yield, cfgstore.get(), zone_params);
      if (ret < 0) {
        cerr << "failed to set zone as default: " << cpp_strerror(-ret)
             << std::endl;
        return -ret;
      }
    } break;
    case OPT::ZONE_DELETE: {
      ...
      ret = rgw::delete_zone(
          dpp(), null_yield, cfgstore.get(), zone_params, *writer);
      if (ret < 0) {
    ...
    case OPT::ZONE_SET: {
      ...
      std::unique_ptr<rgw::sal::ZoneWriter> writer;
      int ret = rgw::read_zone(
          dpp(), null_yield, cfgstore.get(), zone_id, zone_name, zone, &writer);
      if (ret < 0 && ret != -ENOENT) {
        cerr << "failed to load zone: " << cpp_strerror(ret) << std::endl;
        return -ret;
      }
      ...
      if (!zone_name.empty() && !zone.get_name().empty() &&
          zone.get_name() != zone_name) {
        cerr << "ERROR: zone name " << zone_name
             << " is different than the zone name " << zone.get_name()
      ...
      if (ret < 0) {
        cerr << "ERROR: couldn't create zone: " << cpp_strerror(-ret)
             << std::endl;
        return -ret;
      }
    ...
    case OPT::ZONE_LIST: {
      ...
      int ret = rgw::read_zone(
          dpp(), null_yield, cfgstore.get(), {}, {}, default_zone_params);
      if (ret < 0 && ret != -ENOENT) {
        cerr << "could not determine default zone: " << cpp_strerror(-ret)
             << std::endl;
      }
      ...
      std::array<std::string, 1000> names; // list in pages of 1000
      do {
        ret = cfgstore->list_zone_names(
            dpp(), null_yield, listing.next, names, listing);
        if (ret < 0) {
          std::cerr << "failed to list zones: " << cpp_strerror(-ret)
                    << std::endl;
          return -ret;
        }
        for (const auto& name : listing.entries) {
    ...
    case OPT::ZONE_MODIFY: {
      ...
      if (!realm_id.empty()) {
        ...
        // get realm id from name
        ret = cfgstore->read_realm_id(
            dpp(), null_yield, realm_name, zone_params.realm_id);
        if (ret < 0) {
          cerr << "failed to find realm by name " << realm_name << std::endl;
          return -ret;
        }
    ...
    case OPT::ZONE_RENAME: {
      ...
      if (ret < 0) {
        cerr << "failed to rename zone " << zone_name << " to " << zone_new_name
             << ": " << cpp_strerror(-ret) << std::endl;
        return -ret;
      }
      ...
      ret = rgw::read_zonegroup(
          ...
          zonegroup, &zonegroup_writer);
      if (ret < 0) {
        cerr << "WARNING: failed to load zonegroup " << zonegroup_name
             << std::endl;
        return EXIT_SUCCESS;
      }
      ...
      ret = zonegroup_writer->write(dpp(), null_yield, zonegroup);
      if (ret < 0) {
        cerr << "Error in zonegroup rename for " << zone_name << ": "
             << cpp_strerror(-ret) << std::endl;
        return -ret;
      }
    } break;
    ...
    case OPT::ZONE_PLACEMENT_RM: {
      ...
      if (opt_cmd == OPT::ZONE_PLACEMENT_ADD ||
        ...
        ret = rgw::read_zonegroup(
            ...
            zonegroup);
        if (ret < 0) {
          cerr << "failed to init zonegroup: " << cpp_strerror(-ret)
               << std::endl;
          return -ret;
        }
        ...
        if (ptiter == zonegroup.placement_targets.end()) {
          cerr << "ERROR: placement id '" << placement_id
               << "' is not configured in zonegroup placement targets"
               << std::endl;
          return EINVAL;
        }

        string storage_class = rgw_placement_rule::get_canonical_storage_class(
            opt_storage_class.value_or(string()));
        if (ptiter->second.storage_classes.find(storage_class) ==
            ptiter->second.storage_classes.end()) {
          cerr << "ERROR: storage class '" << storage_class
               << "' is not defined in zonegroup '" << placement_id
               << "' placement target" << std::endl;
          return EINVAL;
        }
        if (ptiter->second.tier_targets.find(storage_class) !=
            ptiter->second.tier_targets.end()) {
          cerr << "ERROR: storage class '" << storage_class
        ...
        if (info.index_pool.empty()) {
          cerr << "ERROR: index pool not configured, need to specify "
                  "--index-pool"
               << std::endl;
          return EINVAL;
        }
        ...
        if (opt_data_pool.empty()) {
          ...
          if (opt_data_pool.empty()) {
            cerr << "ERROR: data pool not configured, need to specify "
                    "--data-pool"
                 << std::endl;
            return EINVAL;
          }
        ...
        info.storage_classes.set_storage_class(
            storage_class, &dp, compression_type.get_ptr());
        ...
        if (ret < 0) {
          cerr << "ERROR: the data extra (non-ec) pool '"
               << info.get_data_extra_pool() << "' does not support omap"
               << std::endl;
          return ret;
        }
    ...
    case OPT::ZONE_PLACEMENT_LIST: {
      RGWZoneParams zone;
      int ret = rgw::read_zone(
          dpp(), null_yield, cfgstore.get(), zone_id, zone_name, zone);
      if (ret < 0) {
        cerr << "unable to initialize zone: " << cpp_strerror(-ret)
             << std::endl;
        return -ret;
      }
  ...
  std::set<OPT> non_master_ops_list = {
      ...
      OPT::ROLE_POLICY_ATTACH,
      OPT::ROLE_POLICY_DETACH,
      OPT::USER_POLICY_ATTACH,
      OPT::USER_POLICY_DETACH,
      OPT::RATELIMIT_SET,
      OPT::RATELIMIT_ENABLE,
      OPT::RATELIMIT_DISABLE};

  bool print_warning_message =
      (non_master_ops_list.find(opt_cmd) != non_master_ops_list.end() &&
       non_master_cmd);

  if (print_warning_message) {
    cerr << "Please run the command on master zone. Performing this operation "
            "on non-master zone leads to inconsistent metadata between zones"
         << std::endl;
    cerr << "Are you sure you want to go ahead? (requires "
  ...
  if (!placement_id.empty()) {
    ...
    if (!driver->valid_placement(target_rule)) {
      cerr << "NOTICE: invalid dest placement: " << target_rule.to_str()
           << std::endl;
      return EINVAL;
    }
  ...
  case OPT::PERIOD_PUSH: {
    ...
    JSONParser p;
    ret = send_to_remote_or_url(
        nullptr, url, opt_region, access_key, secret_key, info, bl, p);
    if (ret < 0) {
      cerr << "request failed: " << cpp_strerror(-ret) << std::endl;
      return -ret;
    }
  ...
  case OPT::PERIOD_UPDATE: {
    int ret = update_period(
        cfgstore.get(), realm_id, realm_name, period_epoch, commit, remote, url,
        opt_region, access_key, secret_key, formatter.get(),
        yes_i_really_mean_it, site.get());
    if (ret < 0) {
      return -ret;
    }
  ...
  case OPT::PERIOD_COMMIT: {
    ...
    ret = commit_period(
        cfgstore.get(), realm, *realm_writer, period, remote, url, opt_region,
        access_key, secret_key, yes_i_really_mean_it, site.get());
    if (ret < 0) {
      cerr << "failed to commit period: " << cpp_strerror(-ret) << std::endl;
      return -ret;
    }
  ...
  case OPT::ROLE_CREATE: {
    ...
    std::unique_ptr<rgw::sal::RGWRole> role = driver->get_role(
        role_name, tenant, account_id, path, assume_role_doc, description,
  ...
  case OPT::ROLE_DELETE: {
    ...
    std::unique_ptr<rgw::sal::RGWRole> role =
        driver->get_role(role_name, tenant, account_id);
  ...
  case OPT::ROLE_TRUST_POLICY_MODIFY: {
    ...
    cout << "Assume role policy document updated successfully for role: "
         << role_name << std::endl;
    return 0;
  }
  case OPT::ROLE_LIST: {
    ...
    do {
      ...
      if (!account_id.empty()) {
        ...
        ret = driver->list_account_roles(
            dpp(), null_yield, account_id, path_prefix, listing.next_marker,
            count, listing);
      } else {
        // list roles in the tenant
        ret = driver->list_roles(
            dpp(), null_yield, tenant, path_prefix, listing.next_marker, count,
            listing);
      }
      if (ret < 0) {
        return -ret;
  ...
  case OPT::ROLE_POLICY_PUT: {
    ...
    if (!infile.empty()) {
      ...
      int ret = read_input(infile, bl);
      if (ret < 0) {
        cerr << "ERROR: failed to read input policy document: "
             << cpp_strerror(-ret) << std::endl;
        return -ret;
      }
    ...
    try {
      const rgw::IAM::Policy p(
          g_ceph_context, nullptr, perm_policy_doc,
          g_ceph_context->_conf.get_val<bool>(
              "rgw_policy_reject_invalid_principals"));
    } catch (rgw::IAM::PolicyParseException& e) {
  ...
  case OPT::ROLE_POLICY_DELETE: {
    ...
    cout << "Policy: " << policy_name
         << " successfully deleted for role: " << role_name << std::endl;
    return 0;
  }
  case OPT::ROLE_POLICY_ATTACH: {
    ...
    if (role->get_info().account_id.empty()) {
      std::cerr << "Managed policies are only supported for account roles"
                << std::endl;
      return EINVAL;
    }
  ...
  case OPT::ROLE_UPDATE: {
    ...
    cout << "Max session duration updated successfully for role: " << role_name
         << std::endl;
    return 0;
  }
  default:
  ...
  if (opt_cmd == OPT::POLICY) {
    if (format == "xml") {
      int ret = RGWBucketAdminOp::dump_s3_policy(
          driver, bucket_op, cout, dpp(), null_yield);
      if (ret < 0) {
        cerr << "ERROR: failed to get policy: " << cpp_strerror(-ret)
             << std::endl;
        return -ret;
      }
    } else {
      int ret = RGWBucketAdminOp::get_policy(
          driver, bucket_op, stream_flusher, dpp(), null_yield);
      if (ret < 0) {
        cerr << "ERROR: failed to get policy: " << cpp_strerror(-ret)
             << std::endl;
        return -ret;
  ...
  if (opt_cmd == OPT::BUCKET_LIMIT_CHECK) {
    ...
    if (!rgw::sal::User::empty(user)) {
      ...
      ret = RGWBucketAdminOp::limit_check(
          driver, bucket_op, user_ids, stream_flusher, null_yield, dpp(),
          warnings_only);
    } else {
      ...
      do {
        ret = driver->meta_list_keys_next(
            dpp(), handle, max, user_ids, &truncated);
        if (ret < 0 && ret != -ENOENT) {
  ...
  if (opt_cmd == OPT::BUCKETS_LIST) {
    if (bucket_name.empty()) {
      ...
      RGWBucketAdminOp::info(
          driver, *site, bucket_op, stream_flusher, null_yield, dpp());
    } else {
      int ret = init_bucket(tenant, bucket_name, bucket_id, &bucket);
      if (ret < 0) {
        cerr << "ERROR: could not init bucket: " << cpp_strerror(-ret)
      ...
      do {
        ...
        ret = bucket->list(
            dpp(), params, std::min(remaining, paginate_size), results,
            null_yield);
        if (ret < 0) {
          cerr << "ERROR: driver->list_objects(): " << cpp_strerror(-ret)
               << std::endl;
  ...
  if (opt_cmd == OPT::BUCKET_RADOS_LIST) {
    RGWRadosList lister(
        static_cast<rgw::sal::RadosStore*>(driver), max_concurrent_ios,
        orphan_stale_secs, tenant);
    if (rgw_obj_fs) {
  ...
  if (opt_cmd == OPT::BUCKET_STATS) {
    if (bucket_name.empty() && !bucket_id.empty()) {
      rgw_bucket bucket;
      if (!rgw_find_bucket_by_id(
              dpp(), driver->ctx(), driver, marker, bucket_id, &bucket)) {
  ...
  if (opt_cmd == OPT::BUCKET_SHARD_OBJECTS) {
    ...
    if (specified_shard_id) {
      ...
      for (uint64_t ctr = 0, shardsleft = num_shards; shardsleft > 0; ++ctr) {
        ...
        auto shard =
            RGWSI_BucketIndex_RADOS::bucket_shard_index(key, num_shards);
        if (objs[shard].empty()) {
  ...
  if (opt_cmd == OPT::BUCKET_RESYNC_ENCRYPTED_MULTIPART) {
    ...
    if (!rados_driver->svc()->zone->need_to_log_data() &&
        !yes_i_really_mean_it) {
      cerr << "This command is only necessary for replicated buckets."
           << std::endl;
      cerr << "do you really mean it? (requires --yes-i-really-mean-it)"
           << std::endl;
      return EPERM;
    }
    ...
    ret = rados_driver->getRados()->bucket_resync_encrypted_multipart(
        dpp(), null_yield, rados_driver, bucket->get_info(), marker,
        stream_flusher);
    if (ret < 0) {
      return -ret;
    }
  ...
  if (opt_cmd == OPT::BUCKET_CHOWN) {
    ...
    int r = RGWBucketAdminOp::chown(
        driver, bucket_op, marker, dpp(), null_yield, &err);
    if (r < 0) {
      cerr << "failure: " << cpp_strerror(-r) << ": " << err << std::endl;
      return -r;
    }
  ...
  if (opt_cmd == OPT::BUCKET_LOGGING_FLUSH) {
    ...
    ret = rgw::bucketlogging::get_target_and_conf_from_source(
        dpp(), driver, bucket.get(), tenant, configuration, target_bucket,
        null_yield);
    if (ret < 0 && ret != -ENODATA) {
      cerr << "ERROR: failed to get target bucket and logging conf from source "
              "bucket '"
           << bucket_name << "': " << cpp_strerror(-ret) << std::endl;
      return -ret;
    } else if (ret == -ENODATA) {
      cerr << "ERROR: bucket '" << bucket_name
           << "' does not have logging enabled" << std::endl;
      return 0;
    }

    // make sure that the logging source attribute is up-to-date
    if (ret = rgw::bucketlogging::update_bucket_logging_sources(
            dpp(), target_bucket, bucket->get_key(), true, null_yield);
        ret < 0) {
      cerr << "WARNING: failed to update logging sources attribute '"
           << RGW_ATTR_BUCKET_LOGGING_SOURCES << "' in logging target '"
           << target_bucket->get_key() << "'. error: " << cpp_strerror(ret)
           << std::endl;
    }

    std::string obj_name;
    RGWObjVersionTracker objv_tracker;
    ret = target_bucket->get_logging_object_name(
        obj_name, configuration.target_prefix, null_yield, dpp(), &objv_tracker);
    if (ret < 0 && ret != -ENOENT) {
      cerr << "ERROR: failed to get pending logging object name from target "
              "bucket '"
           << configuration.target_bucket << "'. error: " << cpp_strerror(-ret)
           << std::endl;
      return -ret;
    }
    std::string old_obj;
    const auto region = driver->get_zone()->get_zonegroup().get_api_name();
    ret = rgw::bucketlogging::rollover_logging_object(
        configuration, target_bucket, obj_name, dpp(), region, bucket,
        null_yield, true, &objv_tracker, false, &old_obj);
    if (ret < 0) {
      cerr << "ERROR: failed to flush pending logging object '" << obj_name
           << "' to target bucket '" << configuration.target_bucket
  ...
  if (opt_cmd == OPT::BUCKET_LOGGING_INFO) {
    ...
    if (iter != bucket_attrs.end()) {
      ...
      try {
        ...
        cerr << "ERROR: failed to decode logging attribute '"
             << RGW_ATTR_BUCKET_LOGGING << "'. error: " << err.what()
             << std::endl;
        return EINVAL;
      }
    ...
    if (iter != bucket_attrs.end()) {
      ...
      try {
        ...
        cerr << "ERROR: failed to decode logging sources attribute '"
             << RGW_ATTR_BUCKET_LOGGING_SOURCES << "'. error: " << err.what()
             << std::endl;
        return EINVAL;
      }
  ...
  if (opt_cmd == OPT::BUCKET_LOGGING_LIST) {
    ...
    if (driver->get_name() != "rados") {
      cerr << "ERROR: this command is only available with the RADOS driver."
           << std::endl;
      return EINVAL;
    }
    ...
    ret = rgw::bucketlogging::list_pending_commit_objects(
        dpp(), static_cast<rgw::sal::RadosStore*>(driver), target_bucket.get(),
        target_prefix, entries, null_yield);

    if (ret < 0) {
      cerr << "ERROR: failed to get pending log entries for bucket '"
           << bucket_name << "': " << cpp_strerror(-ret) << std::endl;
      return ret;
    }
  ...
  if (opt_cmd == OPT::LOG_LIST) {
    ...
    if (date.size() && date.size() != 10) {
      cerr << "bad date format for '" << date << "', expect YYYY-MM-DD"
           << std::endl;
      return EINVAL;
    }
    ...
    RGWAccessHandle h;
    int r =
        static_cast<rgw::sal::RadosStore*>(driver)->getRados()->log_list_init(
            dpp(), date, &h);
    if (r == -ENOENT) {
      // no logs.
    } else {
      ...
      while (true) {
        string name;
        int r = static_cast<rgw::sal::RadosStore*>(driver)
                    ->getRados()
                    ->log_list_next(h, &name);
        if (r == -ENOENT)
          break;
        if (r < 0) {
          cerr << "log list: error " << r << std::endl;
  ...
  if (opt_cmd == OPT::LOG_SHOW || opt_cmd == OPT::LOG_RM) {
    ...
    if (opt_cmd == OPT::LOG_SHOW) {
      ...
      int r =
          static_cast<rgw::sal::RadosStore*>(driver)->getRados()->log_show_init(
              dpp(), oid, &h);
      if (r < 0) {
        cerr << "error opening log " << oid << ": " << cpp_strerror(-r)
             << std::endl;
        return -r;
      }
      ...
      r = static_cast<rgw::sal::RadosStore*>(driver)->getRados()->log_show_next(
          dpp(), h, &entry);
      if (r < 0) {
        cerr << "error reading log " << oid << ": " << cpp_strerror(-r)
             << std::endl;
        return -r;
      }
      ...
      do {
        ...
        uint64_t total_time =
            duration_cast<milliseconds>(entry.total_time).count();
      ...
      next:
        r = static_cast<rgw::sal::RadosStore*>(driver)
                ->getRados()
                ->log_show_next(dpp(), h, &entry);
      } while (r > 0);
    ...
    if (opt_cmd == OPT::LOG_RM) {
      int r = static_cast<rgw::sal::RadosStore*>(driver)->getRados()->log_remove(
          dpp(), oid);
      if (r < 0) {
        cerr << "error removing log " << oid << ": " << cpp_strerror(-r)
             << std::endl;
        return -r;
      }
    }
  ...
  if (opt_cmd == OPT::USAGE_SHOW) {
    ...
    ret = RGWUsage::show(
        dpp(), driver, user.get(), bucket.get(), start_epoch, end_epoch,
        show_log_entries, show_log_sum, &categories, stream_flusher);
    if (ret < 0) {
  ...
  if (opt_cmd == OPT::USAGE_TRIM) {
    if (rgw::sal::User::empty(user) && bucket_name.empty() &&
      ...
      cerr << "usage trim without user/date/bucket specified will remove *all* "
              "users data"
           << std::endl;
      cerr << "do you really mean it? (requires --yes-i-really-mean-it)"
           << std::endl;
  ...
  if (opt_cmd == OPT::USAGE_CLEAR) {
    if (!yes_i_really_mean_it) {
      cerr << "usage clear would remove *all* users usage data for all time"
           << std::endl;
      cerr << "do you really mean it? (requires --yes-i-really-mean-it)"
           << std::endl;
      return 1;
  ...
  if (opt_cmd == OPT::OLH_GET) {
    ...
    ret = static_cast<rgw::sal::RadosStore*>(driver)->getRados()->get_olh(
        dpp(), bucket->get_info(), obj, &olh, null_yield);
    if (ret < 0) {
  ...
  if (opt_cmd == OPT::OLH_READLOG) {
    ...
    RGWObjState& state =
        static_cast<rgw::sal::RadosObject*>(obj.get())->get_state();

    ret = static_cast<rgw::sal::RadosStore*>(driver)
              ->getRados()
              ->bucket_index_read_olh_log(
                  dpp(), bucket->get_info(), state, obj->get_obj(), 0, &log,
  ...
  if (opt_cmd == OPT::BI_GET) {
    ...
    ret = static_cast<rgw::sal::RadosStore*>(driver)->getRados()->bi_get(
        dpp(), bucket->get_info(), obj, bi_index_type, &entry, null_yield);
    if (ret < 0) {
  ...
  if (opt_cmd == OPT::BI_PUT) {
    ...
    ret = static_cast<rgw::sal::RadosStore*>(driver)->getRados()->bi_put(
        dpp(), bucket->get_key(), obj, entry, null_yield);
    if (ret < 0) {
  ...
  if (opt_cmd == OPT::BI_LIST) {
    ...
    ldpp_dout(dpp(), 20) << "INFO: " << __func__
                         ...
                         << ", index=" << index << ", max_shards=" << max_shards
                         << dendl;
    ...
    for (; i < max_shards; i++) {
      ldpp_dout(dpp(), 20) << "INFO: " << __func__ << ": starting shard=" << i
                           << dendl;
      marker.clear();
      ...
      if (ret < 0) {
        ldpp_dout(dpp(), 0)
            << "ERROR: bs.init(bucket=" << bucket << ", shard=" << i
            << "): " << cpp_strerror(-ret) << dendl;
        return -ret;
      }
      ...
      do {
        ...
        ret = rados->bi_list(
            bs, object, marker, max_entries, &entries, &is_truncated, false,
            null_yield);
        if (ret < 0) {
          ldpp_dout(dpp(), 0)
              << "ERROR: bi_list(): " << cpp_strerror(-ret) << dendl;
          return -ret;
        }
  ...
  if (opt_cmd == OPT::BI_PURGE) {
    ...
    if (ret == -ENOENT) {
      ...
      cerr << "ERROR: could not init current bucket info for bucket_name="
           << bucket_name << ": " << cpp_strerror(-ret) << std::endl;
      return -ret;
    } else if (
        cur_bucket->get_bucket_id() == bucket->get_bucket_id() &&
        !yes_i_really_mean_it) {
      cerr << "specified bucket instance points to a current bucket instance"
           << std::endl;
      cerr << "do you really mean it? (requires --yes-i-really-mean-it)"
           << std::endl;
      return EINVAL;
    ...
    for (int i = 0; i < max_shards; i++) {
      RGWRados::BucketShard bs(
          static_cast<rgw::sal::RadosStore*>(driver)->getRados());
      int ret = bs.init(dpp(), bucket->get_info(), index, i, null_yield);
      if (ret < 0) {
        cerr << "ERROR: bs.init(bucket=" << bucket << ", shard=" << i
             << "): " << cpp_strerror(-ret) << std::endl;
        return -ret;
      }

      ret = static_cast<rgw::sal::RadosStore*>(driver)->getRados()->bi_remove(
          dpp(), bs);
      if (ret < 0) {
  ...
  if (opt_cmd == OPT::OBJECT_PUT) {
    ...
    int ret = data_access.get_bucket(
        dpp(), tenant, bucket_name, bucket_id, &b, null_yield);
    if (ret < 0) {
      cerr << "ERROR: failed to init bucket: " << cpp_strerror(-ret)
           << std::endl;
      return -ret;
    }
    ...
    if (ret < 0) {
      cerr << "ERROR: failed to get object: " << cpp_strerror(-ret)
           << std::endl;
      return -ret;
    }
    ...
    if (ret < 0) {
      cerr << "ERROR: failed to read input: " << cpp_strerror(-ret)
           << std::endl;
    }
    ...
    if (ret < 0) {
      cerr << "ERROR: put object returned error: " << cpp_strerror(-ret)
           << std::endl;
    }
  }
  ...
  if (opt_cmd == OPT::OBJECT_RM) {
    ...
    ret = rgw_remove_object(
        dpp(), driver, bucket.get(), key, null_yield, yes_i_really_mean_it);
    if (ret < 0) {
      cerr << "ERROR: object remove returned: " << cpp_strerror(-ret)
           << std::endl;
      return -ret;
    }
  }
  ...
  if (opt_cmd == OPT::OBJECT_REWRITE) {
    ...
    if (min_rewrite_stripe_size > 0) {
      ret = check_min_obj_stripe_size(
          driver, obj.get(), min_rewrite_stripe_size, &need_rewrite);
      if (ret < 0) {
        ldpp_dout(dpp(), 0)
            << "WARNING: check_min_obj_stripe_size failed, r=" << ret << dendl;
      }
    }
    if (need_rewrite) {
      RGWRados* store = static_cast<rgw::sal::RadosStore*>(driver)->getRados();
      ret = store->rewrite_obj(
          bucket->get_info(), obj->get_obj(), dpp(), null_yield);
      if (ret < 0) {
        cerr << "ERROR: object rewrite returned: " << cpp_strerror(-ret)
             << std::endl;
        return -ret;
      }
    } else {
  ...
  if (opt_cmd == OPT::OBJECT_REINDEX) {
    ...
    if (object.empty() && objects_file.empty()) {
      cerr << "ERROR: neither --object nor --objects-file specified."
           << std::endl;
      return EINVAL;
    } else if (!object.empty() && !objects_file.empty()) {
      cerr << "ERROR: both --object and --objects-file specified and only one "
              "is allowed."
           << std::endl;
      return EINVAL;
    } else if (!objects_file.empty() && !object_version.empty()) {
      cerr << "ERROR: cannot specify --object_version when --objects-file "
              "specified."
           << std::endl;
      return EINVAL;
    }
    ...
    auto process = [&](const std::string& p_object,
                       const std::string& p_object_version) -> int {
      std::unique_ptr<rgw::sal::Object> obj = bucket->get_object(p_object);
      obj->set_instance(p_object_version);
      ret = store->reindex_obj(
          driver, bucket->get_info(), obj->get_obj(), dpp(), null_yield);
      if (ret < 0) {
        return ret;
      }
      return 0;
    ...
    if (!object.empty()) {
      ...
      if (!file.is_open()) {
        std::cerr << "ERROR: unable to open objects-file \"" << objects_file
                  << "\"." << std::endl;
        return ENOENT;
      }
      ...
      while (std::getline(file, obj_name)) {
        ...
        if (ret < 0) {
          std::cerr << "ERROR: while processing \"" << obj_name
                    << "\", received " << cpp_strerror(-ret) << "."
                    << std::endl;
          if (!yes_i_really_mean_it) {
            std::cerr
                << "NOTE: with *caution* you can use --yes-i-really-mean-it to "
                   "push through errors and continue processing."
                << std::endl;
            return -ret;
          }
        }
  ...
  if (opt_cmd == OPT::OBJECTS_EXPIRE) {
    if (!driver->process_expired_objects(dpp(), null_yield)) {
      cerr << "ERROR: process_expired_objects() processing returned error."
           << std::endl;
      return 1;
    }
  }
  ...
  if (opt_cmd == OPT::OBJECTS_EXPIRE_STALE_LIST) {
    ret = RGWBucketAdminOp::fix_obj_expiry(
        driver, bucket_op, stream_flusher, dpp(), null_yield, true);
    if (ret < 0) {
  ...
  if (opt_cmd == OPT::OBJECTS_EXPIRE_STALE_RM) {
    ret = RGWBucketAdminOp::fix_obj_expiry(
        driver, bucket_op, stream_flusher, dpp(), null_yield, false);
    if (ret < 0) {
  ...
  if (opt_cmd == OPT::BUCKET_REWRITE) {
    ...
    while (is_truncated) {
      ...
      int r = static_cast<rgw::sal::RadosStore*>(driver)
                  ->getRados()
                  ->cls_bucket_list_ordered(
                      dpp(), bucket->get_info(), current_index, RGW_NO_SHARD,
                      marker, empty_prefix, empty_delimiter, NUM_ENTRIES, true,
                      expansion_factor, result, &is_truncated, &cls_filtered,
                      &marker, null_yield, rgw_bucket_object_check_filter);
      if (r < 0 && r != -ENOENT) {
      ...
      for (auto iter = result.begin(); iter != result.end(); ++iter) {
        ...
        if ((entry.meta.size < min_rewrite_size) ||
          ...
          bool need_rewrite = true;
          if (min_rewrite_stripe_size > 0) {
            r = check_min_obj_stripe_size(
                driver, obj.get(), min_rewrite_stripe_size, &need_rewrite);
            if (r < 0) {
              ldpp_dout(dpp(), 0)
                  << "WARNING: check_min_obj_stripe_size failed, r=" << r
                  << dendl;
            }
          }
          if (!need_rewrite) {
  ...
  if (opt_cmd == OPT::BUCKET_RESHARD) {
    int ret = check_reshard_bucket_params(
        driver, bucket_name, tenant, bucket_id, num_shards_specified,
        num_shards, yes_i_really_mean_it, &bucket);
    if (ret < 0) {
      return ret;
    }
    ...
    if (!RGWBucketReshard::should_zone_reshard_now(
            bucket->get_info(), zone_svc) &&
        !yes_i_really_mean_it) {
      std::cerr
          ...
          << ") "
             ...
             "Resharding is not recommended until the old generations sync, "
             "but "
    ...
    RGWBucketReshard br(
        static_cast<rgw::sal::RadosStore*>(driver), bucket->get_info(),
        bucket->get_attrs(), nullptr /* no callback */);
...
#define DEFAULT_RESHARD_MAX_ENTRIES 1000
    ...
    ret = br.execute(
        num_shards, fault, max_entries, cls_rgw_reshard_initiator::Admin, dpp(),
        null_yield, verbose, &cout, formatter.get());
    return -ret;
  }
  ...
  if (opt_cmd == OPT::RESHARD_LIST) {
    ...
    for (int i = 0; i < num_logshards; i++) {
      ...
      do {
        ...
        ret = reshard.list(
            dpp(), i, marker, max_entries - count, entries, &is_truncated);
        if (ret < 0) {
          cerr << "Error listing resharding buckets: " << cpp_strerror(-ret)
               << std::endl;
          return ret;
        }
        for (const auto& entry : entries) {
  ...
  if (opt_cmd == OPT::RESHARD_PROCESS) {
    ...
    if (ret < 0) {
      cerr << "ERROR: failed to process reshard logs, error="
           << cpp_strerror(-ret) << std::endl;
      return -ret;
    }
  }
  ...
  if (opt_cmd == OPT::RESHARD_CANCEL) {
    ...
    if (ret < 0) {
      if (yes_i_really_mean_it) {
        ...
        cerr << "ERROR: could not init bucket: " << cpp_strerror(-ret)
             << "; if you want to cancel the reshard request nonetheless, "
                "please "
      ...
      if (ret < 0) {
        if (ret == -EBUSY) {
          cerr << "There is ongoing resharding, please retry after "
               << driver->ctx()->_conf.get_val<uint64_t>(
    ...
    if (ret == -ENOENT) {
      ...
      cerr << "Error in updating reshard log with bucket \"" << bucket_name
           << "\": " << cpp_strerror(-ret) << std::endl;
      return -ret;
    }
  } // OPT_RESHARD_CANCEL
    ...
    if (type != rgw::BucketIndexType::Normal) {
      cerr << "ERROR: the bucket's layout is type " << type
           ...
           << " and therefore does not have a "
              "minimum number of shards that can be altered"
           << std::endl;
      return EINVAL;
    }
  ...
  if (opt_cmd == OPT::OBJECT_UNLINK) {
    ...
    if (ret < 0) {
      cerr << "ERROR: remove_obj_from_index() returned error: "
           << cpp_strerror(-ret) << std::endl;
      return 1;
    }
  }
  ...
  if (opt_cmd == OPT::OBJECT_STAT) {
    ...
    if (ret < 0) {
      cerr << "ERROR: failed to stat object, returned error: "
           << cpp_strerror(-ret) << std::endl;
      return 1;
    }
    ...
    for (iter = obj->get_attrs().begin(); iter != obj->get_attrs().end();
         ++iter) {
      bufferlist& bl = iter->second;
      bool handled = false;
      if (iter->first == RGW_ATTR_MANIFEST) {
        ...
        handled =
            decode_dump<RGWAccessControlPolicy>("policy", bl, formatter.get());
        ...
        handled =
            decode_dump<RGWCompressionInfo>("compression", bl, formatter.get());
        ...
        handled = decode_dump<ceph::real_time>(
            "restore_expiry_date", bl, formatter.get());
        ...
        decode(rt, bl);
        formatter->dump_string(
            "RestoreType", rgw::sal::rgw_restore_type_dump(rt));
        handled = true;
      } else if (iter->first == RGW_ATTR_RESTORE_STATUS) {
        rgw::sal::RGWRestoreStatus rs;
        decode(rs, bl);
        formatter->dump_string(
            "RestoreStatus", rgw::sal::rgw_restore_status_dump(rs));
        handled = true;
    ...
    for (iter = other_attrs.begin(); iter != other_attrs.end(); ++iter) {
      ...
      if (iter->first == RGW_ATTR_OBJ_REPLICATION_TIMESTAMP) {
        decode_dump<ceph::real_time>(
            "user.rgw.replicated-at", bl, formatter.get());
      } else if (iter->first == RGW_ATTR_RESTORE_TIME) {
        decode_dump<ceph::real_time>("user.rgw.restore-at", bl, formatter.get());
      } else if (iter->first == RGW_ATTR_INTERNAL_MTIME) {
        decode_dump<ceph::real_time>(
            "user.rgw.rgw-internal-mtime", bl, formatter.get());
      } else {
    ...
    for (auto p = m.obj_begin(dpp()); p != m.obj_end(dpp()); ++p, ++index) {
      ...
      if (index == 0 && raw_obj != raw_head_obj) {
        ...
        formatter->open_object_section(
            "object"); // name not displayed since in array
  ...
  if (opt_cmd == OPT::BUCKET_CHECK) {
    if (check_head_obj_locator) {
      ...
      do_check_object_locator(
          tenant, bucket_name, fix, remove_bad, formatter.get());
    } else {
      RGWBucketAdminOp::check_index(
          driver, bucket_op, stream_flusher, null_yield, dpp());
    }
  }
  ...
  if (opt_cmd == OPT::BUCKET_CHECK_UNLINKED) {
    ...
    RGWBucketAdminOp::check_index_unlinked(
        store, bucket_op, stream_flusher, dpp());
  }

  if (opt_cmd == OPT::BUCKET_RM) {
    if (!inconsistent_index) {
      RGWBucketAdminOp::remove_bucket(
          driver, *site, bucket_op, null_yield, dpp(), bypass_gc, true, false);
    } else {
      if (!yes_i_really_mean_it) {
      ...
      RGWBucketAdminOp::remove_bucket(
          driver, *site, bucket_op, null_yield, dpp(), bypass_gc, false, false);
    }
  }

  if (opt_cmd == OPT::DEDUP_STATS || opt_cmd == OPT::DEDUP_ESTIMATE ||
      opt_cmd == OPT::DEDUP_ABORT || opt_cmd == OPT::DEDUP_PAUSE ||
      opt_cmd == OPT::DEDUP_RESUME || opt_cmd == OPT::DEDUP_THROTTLE ||
    ...
    if (opt_cmd == OPT::DEDUP_THROTTLE) {
      ...
      if (throttle_stat) {
        ...
        return cluster::dedup_control_bl(
            store, dpp(), urgent_msg, urgent_msg_bl);
      }

      if (unlikely(!have_max_bucket_index_ops && !have_max_metadata_ops)) {
        std::cerr << "dedup throttle must set either --max-bucket-index-ops or "
                     "--max-metadata-ops"
                  << std::endl;
        return EINVAL;
      }
      ...
      if (have_max_bucket_index_ops) {
        throttle_action_t action = {
            .op_type = BUCKET_INDEX_OP, .limit = max_bucket_index_ops};
        throttle_msg.vec.push_back(action);
      }

      if (have_max_metadata_ops) {
        throttle_action_t action = {
            .op_type = METADATA_ACCESS_OP, .limit = max_metadata_ops};
        throttle_msg.vec.push_back(action);
      }
    ...
    if (opt_cmd == OPT::DEDUP_ABORT || opt_cmd == OPT::DEDUP_PAUSE ||
    ...
    if (opt_cmd == OPT::DEDUP_EXEC || opt_cmd == OPT::DEDUP_ESTIMATE) {
      ...
      if (ret == 0) {
        ...
        std::cerr << "Error is: " << ret << "::" << cpp_strerror(ret)
                  << std::endl;
      }
      return ret;
    }
  ...
  if (opt_cmd == OPT::GC_LIST) {
    ...
    do {
      ...
      int ret =
          static_cast<rgw::sal::RadosStore*>(driver)->getRados()->list_gc_objs(
              &index, marker, 1000, !include_all, result, &truncated,
              processing_queue);
      if (ret < 0) {
  ...
  if (opt_cmd == OPT::GC_PROCESS) {
    rgw::sal::RadosStore* rados_store =
        dynamic_cast<rgw::sal::RadosStore*>(driver);
    if (!rados_store) {
      cerr << "WARNING: this command can only work when the cluster has a "
              "RADOS backing store."
           << std::endl;
      return 0;
    }
    ...
    if (ret < 0) {
      cerr << "ERROR: gc processing returned error: " << cpp_strerror(-ret)
           << std::endl;
      return 1;
    }
  }
...
#define MAX_LC_LIST_ENTRIES 100
    ...
    do {
      ...
      for (const auto& entry : bucket_lc_map) {
        ...
        if (std::strftime(
                exp_buf, sizeof(exp_buf), "%a, %d %b %Y %T %Z",
                std::gmtime(&t))) {
  ...
  if (opt_cmd == OPT::LC_PROCESS) {
    ...
    int ret = static_cast<rgw::sal::RadosStore*>(driver)->getRados()->process_lc(
        bucket);
    if (ret < 0) {
      cerr << "ERROR: lc processing returned error: " << cpp_strerror(-ret)
           << std::endl;
      return 1;
    }
  }
  ...
  if (opt_cmd == OPT::LC_RESHARD_FIX) {
    ret = RGWBucketAdminOp::fix_lc_shards(
        driver, bucket_op, stream_flusher, dpp(), null_yield);
    if (ret < 0) {
  ...
  if (opt_cmd == OPT::ORPHANS_FIND) {
    if (!yes_i_really_mean_it) {
      cerr << "this command is now deprecated; please consider using the "
      ...
      cerr << "IMPORTANT: this command is now deprecated; please consider "
              "using the rgw-orphan-list tool"
           << std::endl;
    }

    RGWOrphanSearch search(
        static_cast<rgw::sal::RadosStore*>(driver), max_concurrent_ios,
        orphan_stale_secs);
  ...
  if (opt_cmd == OPT::USER_CHECK) {
    check_bad_owner_bucket_mapping(
        driver, user->get_id(), user->get_display_name(), user->get_tenant(),
        fix, null_yield, dpp());
  }
  ...
  if (opt_cmd == OPT::USER_STATS) {
    ...
    if (sync_stats) {
      if (!bucket_name.empty()) {
        ...
        if (ret < 0) {
          cerr << "ERROR: could not sync bucket stats: " << cpp_strerror(-ret)
               << std::endl;
          return -ret;
        }
      } else {
        int ret = rgw_sync_all_stats(
            dpp(), null_yield, driver, user->get_id(), user->get_tenant());
        if (ret < 0) {
          cerr << "ERROR: could not sync user stats: " << cpp_strerror(-ret)
               << std::endl;
          return -ret;
        }
      }
    ...
    if (ret < 0) {
      cerr << "User has not been initialized or user does not exist"
           << std::endl;
      return -ret;
    }
    ...
    ret = driver->load_stats(
        dpp(), null_yield, owner, stats, last_stats_sync, last_stats_update);
    if (ret < 0) {
  ...
  if (opt_cmd == OPT::METADATA_PUT) {
    ...
    ret = static_cast<rgw::sal::RadosStore*>(driver)->ctl()->meta.mgr->put(
        metadata_key, bl, null_yield, dpp(), RGWMDLogSyncType::APPLY_ALWAYS,
        false);
    if (ret < 0) {
  ...
  if (opt_cmd == OPT::METADATA_RM) {
    int ret =
        static_cast<rgw::sal::RadosStore*>(driver)->ctl()->meta.mgr->remove(
            metadata_key, null_yield, dpp());
    if (ret < 0) {
    ...
    if (opt_cmd == OPT::USER_LIST) {
      ...
      if (!account_id.empty() || !account_name.empty()) {
        ...
        int ret = rgw::account::list_users(
            dpp(), driver, op_state, path_prefix, marker, max_entries_specified,
            max_entries, account_root, err_msg, stream_flusher, null_yield);
        if (ret < 0) {
  ...
  if (opt_cmd == OPT::MDLOG_LIST) {
    ...
    if (period_id.empty()) {
      ...
      period_id = realm.current_period;
      std::cerr << "No --period given, using current period=" << period_id
                << std::endl;
    }
    RGWMetadataLog* meta_log =
        static_cast<rgw::sal::RadosStore*>(driver)->svc()->mdlog->get_log(
            period_id);
    ...
    for (; i < g_ceph_context->_conf->rgw_md_log_max_shards; i++) {
      ...
      bool truncated;
      do {
        int ret = meta_log->list_entries(
            dpp(), handle, 1000, entries, NULL, &truncated, null_yield);
        if (ret < 0) {
          cerr << "ERROR: meta_log->list_entries(): " << cpp_strerror(-ret)
               << std::endl;
          return -ret;
        }
        ...
        for (auto iter = entries.begin(); iter != entries.end(); ++iter) {
          ...
          static_cast<rgw::sal::RadosStore*>(driver)
              ->ctl()
              ->meta.mgr->dump_log_entry(entry, formatter.get());
        }
        formatter->flush(cout);
      } while (truncated);
  ...
  if (opt_cmd == OPT::MDLOG_AUTOTRIM) {
    ...
    static_cast<rgw::sal::RadosStore*>(driver)
        ->svc()
        ->mdlog->init_oldest_log_period(null_yield, dpp(), cfgstore.get());

    RGWCoroutinesManager crs(driver->ctx(), driver->get_cr_registry());
    RGWHTTPManager http(driver->ctx(), crs.get_completion_mgr());
    int ret = http.start();
    if (ret < 0) {
      cerr << "failed to initialize http client with " << cpp_strerror(ret)
           << std::endl;
      return -ret;
    }
    ...
    if (ret < 0) {
      cerr << "automated mdlog trim failed with " << cpp_strerror(ret)
           << std::endl;
      return -ret;
    }
  }
  ...
  if (opt_cmd == OPT::MDLOG_TRIM) {
    ...
    if (!specified_shard_id) {
      cerr << "ERROR: shard-id must be specified for trim operation"
           << std::endl;
      return EINVAL;
    }
  ...
  if (opt_cmd == OPT::METADATA_SYNC_STATUS) {
    ...
    ret = sync.read_sync_status(dpp(), &sync_status);
    if (ret < 0) {
      cerr << "ERROR: sync.read_sync_status() returned ret=" << ret
           << std::endl;
      return -ret;
    }
  ...
  if (opt_cmd == OPT::METADATA_SYNC_INIT) {
    ...
    if (ret < 0) {
      cerr << "ERROR: sync.init_sync_status() returned ret=" << ret
           << std::endl;
      return -ret;
    }
  }
  ...
  if (opt_cmd == OPT::DATA_SYNC_STATUS) {
    ...
    if (specified_shard_id) {
      ...
      ret = sync.read_shard_status(
          dpp(), shard_id, pending_buckets, recovering_buckets, &sync_marker,
          max_entries_specified ? max_entries : 20);
      if (ret < 0 && ret != -ENOENT) {
        cerr << "ERROR: sync.read_shard_status() returned ret=" << ret
             << std::endl;
        return -ret;
      }
      formatter->open_object_section("summary");
  ...
  if (opt_cmd == OPT::DATA_SYNC_RUN) {
    ...
    int ret = static_cast<rgw::sal::RadosStore*>(driver)
                  ...
                  ->create_instance(
                      ...
                      &sync_module);
    if (ret < 0) {
      ldpp_dout(dpp(), -1) << "ERROR: failed to init sync module instance, ret="
                           << ret << dendl;
      return ret;
    }
    ...
    RGWDataSyncStatusManager sync(
        static_cast<rgw::sal::RadosStore*>(driver),
        static_cast<rgw::sal::RadosStore*>(driver)->svc()->async_processor,
        source_zone, nullptr, sync_module);
  ...
  if (opt_cmd == OPT::BUCKET_SYNC_CHECKPOINT) {
    ...
    RGWBucketSyncPolicyHandlerRef handler;
    ret = driver->get_sync_policy_handler(
        dpp(), std::nullopt, bucket->get_key(), &handler, null_yield);
    if (ret < 0) {
      std::cerr << "ERROR: failed to get policy handler for bucket (" << bucket
                << "): r=" << ret << ": " << cpp_strerror(-ret) << std::endl;
      return -ret;
    }

    auto timeout_at = ceph::coarse_mono_clock::now() + opt_timeout_sec;
    ret = rgw_bucket_sync_checkpoint(
        dpp(), static_cast<rgw::sal::RadosStore*>(driver), *handler,
        bucket->get_info(), opt_source_zone, opt_source_bucket,
        opt_retry_delay_ms, timeout_at);
    if (ret < 0) {
      ldpp_dout(dpp(), -1) << "bucket sync checkpoint failed: "
                           << cpp_strerror(ret) << dendl;
      return -ret;
    }
  }
  ...
  if ((opt_cmd == OPT::BUCKET_SYNC_DISABLE) ||
    ...
    string err_msg;
    ret = RGWBucketAdminOp::sync_bucket(
        driver, bucket_op, dpp(), null_yield, &err_msg);
    if (ret < 0) {
  ...
  if (opt_cmd == OPT::BUCKET_SYNC_STATUS) {
    ...
    ret = bucket_sync_status(
        driver, bucket_info, source_zone, opt_source_bucket, bucket_sync_info);

    if (ret == 0) {
      bucket_sync_info.print(std::cout, format_arg_passed, formatter.get());
    } else {
      cerr << "failed to get bucket sync status. see logs for more info"
           << std::endl;
    }
  }
  ...
  if (opt_cmd == OPT::BILOG_LIST) {
    ...
    do {
      ...
      ret = static_cast<rgw::sal::RadosStore*>(driver)
                ->svc()
                ->bilog_rados->log_list(
                    dpp(), null_yield, bucket->get_info(), log_layout, shard_id,
                    marker, max_entries - count, entries, &truncated);
      if (ret < 0) {
        cerr << "ERROR: list_bi_log_entries(): " << cpp_strerror(-ret)
             << std::endl;
        return -ret;
      }
      ...
      for (list<rgw_bi_log_entry>::iterator iter = entries.begin();
  ...
  if (opt_cmd == OPT::SYNC_ERROR_LIST) {
    ...
    for (; shard_id < ERROR_LOGGER_SHARDS; ++shard_id) {
      ...
      int count = 0;
      string oid = RGWSyncErrorLogger::get_shard_oid(
          RGW_SYNC_ERROR_LOG_SHARD_PREFIX, shard_id);

      do {
        vector<cls::log::entry> entries;
        ret =
            static_cast<rgw::sal::RadosStore*>(driver)->svc()->cls->timelog.list(
                dpp(), oid, {}, {}, max_entries - count, entries, marker,
                &marker, &truncated, null_yield);
        if (ret == -ENOENT) {
          break;
        }
        if (ret < 0) {
          cerr << "ERROR: svc.cls->timelog.list(): " << cpp_strerror(-ret)
               << std::endl;
          return -ret;
        }
  ...
  if (opt_cmd == OPT::SYNC_GROUP_CREATE || opt_cmd == OPT::SYNC_GROUP_MODIFY) {
    CHECK_TRUE(
        require_non_empty_opt(opt_group_id), "ERROR: --group-id not specified",
        EINVAL);
    CHECK_TRUE(
        require_opt(opt_status),
        "ERROR: --status is not specified (options: forbidden, allowed, "
        "enabled)",
        EINVAL);
    ...
    if (opt_cmd == OPT::SYNC_GROUP_MODIFY) {
      ...
      if (iter == sync_policy.groups.end()) {
        cerr << "ERROR: could not find group '" << *opt_group_id << "'"
             << std::endl;
        return ENOENT;
      }
    }
    ...
    if (opt_status) {
      if (!group.set_status(*opt_status)) {
        cerr << "ERROR: unrecognized status (options: forbidden, allowed, "
                "enabled)"
             << std::endl;
        return EINVAL;
      }
  ...
  if (opt_cmd == OPT::SYNC_GROUP_FLOW_CREATE) {
    CHECK_TRUE(
        ...
        EINVAL);
    CHECK_TRUE(
        require_non_empty_opt(opt_flow_id), "ERROR: --flow-id not specified",
        EINVAL);
    CHECK_TRUE(
        require_opt(opt_flow_type),
        "ERROR: --flow-type not specified (options: symmetrical, directional)",
        EINVAL);
    CHECK_TRUE(
    ...
    if (symmetrical_flow_opt(*opt_flow_type)) {
      CHECK_TRUE(
          require_non_empty_opt(opt_zone_ids),
      ...
      CHECK_TRUE(
          require_non_empty_opt(opt_source_zone_id),
          "ERROR: --source-zone not provided for directional flow rule, or is "
          "empty",
          EINVAL);
      CHECK_TRUE(
          require_non_empty_opt(opt_dest_zone_id),
          "ERROR: --dest-zone not provided for directional flow rule, or is "
          "empty",
          EINVAL);

      rgw_sync_directional_rule* flow_rule;

      group.data_flow.find_or_create_directional(
          *opt_source_zone_id, *opt_dest_zone_id, &flow_rule);
    }
  ...
  if (opt_cmd == OPT::SYNC_GROUP_PIPE_CREATE ||
    ...
    CHECK_TRUE(
        ...
        EINVAL);
    CHECK_TRUE(
        require_non_empty_opt(opt_pipe_id), "ERROR: --pipe-id not specified",
        EINVAL);
    if (opt_cmd == OPT::SYNC_GROUP_PIPE_CREATE) {
      CHECK_TRUE(
          require_non_empty_opt(opt_source_zone_ids),
          "ERROR: --source-zones not provided or is empty; should be list of "
          "zones or '*'",
          EINVAL);
      CHECK_TRUE(
          require_non_empty_opt(opt_dest_zone_ids),
          "ERROR: --dest-zones not provided or is empty; should be list of "
          "zones or '*'",
          EINVAL);
    }
    ...
    if (opt_cmd == OPT::SYNC_GROUP_PIPE_CREATE) {
      ...
      if (!group.find_pipe(*opt_pipe_id, false, &pipe)) {
        cerr << "ERROR: could not find pipe '" << *opt_pipe_id << "'"
             << std::endl;
        return ENOENT;
      }
    }
    ...
    pipe->source.set_bucket(
        opt_source_tenant, opt_source_bucket_name, opt_source_bucket_id);
    if (opt_dest_zone_ids) {
      pipe->dest.add_zones(*opt_dest_zone_ids);
    }
    pipe->dest.set_bucket(
        opt_dest_tenant, opt_dest_bucket_name, opt_dest_bucket_id);
    ...
    if (opt_mode) {
      if (*opt_mode == "system") {
        ...
        cerr << "ERROR: bad mode value: should be one of the following: "
                "system, user"
             << std::endl;
        return EINVAL;
      }
  ...
  if (opt_cmd == OPT::SYNC_GROUP_PIPE_REMOVE) {
    ...
    pipe->source.remove_bucket(
        opt_source_tenant, opt_source_bucket_name, opt_source_bucket_id);
    if (opt_dest_zone_ids) {
      pipe->dest.remove_zones(*opt_dest_zone_ids);
    }
    pipe->dest.remove_bucket(
        opt_dest_tenant, opt_dest_bucket_name, opt_dest_bucket_id);

    if (!(opt_source_zone_ids || opt_source_tenant || opt_source_bucket ||
          opt_source_bucket_id || opt_dest_zone_ids || opt_dest_tenant ||
          opt_dest_bucket || opt_dest_bucket_id)) {
  ...
  if (opt_cmd == OPT::BILOG_TRIM) {
    ...
    ret = bilog_trim(
        dpp(), null_yield, static_cast<rgw::sal::RadosStore*>(driver),
        bucket->get_info(), *gen, shard_id, start_marker, end_marker);
    if (ret < 0) {
      cerr << "ERROR: trim_bi_log_entries(): " << cpp_strerror(-ret)
           << std::endl;
      return -ret;
    }
  }
  ...
  if (opt_cmd == OPT::BILOG_STATUS) {
    ...
    ret = static_cast<rgw::sal::RadosStore*>(driver)
              ->svc()
              ->bilog_rados->get_log_status(
                  dpp(), bucket->get_info(), log_layout, shard_id, &markers,
                  null_yield);
    if (ret < 0) {
  ...
  if (opt_cmd == OPT::BILOG_AUTOTRIM) {
    ...
    rgw::BucketTrimManager trim(
        static_cast<rgw::sal::RadosStore*>(driver), config);
    ret = trim.init();
    if (ret < 0) {
      cerr << "trim manager init failed with " << cpp_strerror(ret)
           << std::endl;
      return -ret;
    }
    ret = crs.run(dpp(), trim.create_admin_bucket_trim_cr(&http));
    if (ret < 0) {
      cerr << "automated bilog trim failed with " << cpp_strerror(ret)
           << std::endl;
      return -ret;
    }
  }
  ...
  if (opt_cmd == OPT::DATALOG_SEMAPHORE_LIST) {
    auto datalog =
        static_cast<rgw::sal::RadosStore*>(driver)->svc()->datalog_rados;
    ...
    ret = run_coro(
        dpp(), context_pool,
        datalog->admin_sem_list(shard, max_entries, marker, cout, *formatter),
        &err);
    if (ret < 0) {
  ...
  if (opt_cmd == OPT::DATALOG_SEMAPHORE_RESET) {
    ...
    ret = rgw::run_coro(
        dpp(), context_pool,
        datalog->admin_sem_reset(marker, count.value_or(0)), &errstr);
    if (ret < 0) {
      ...
      if (specified_shard_id) {
        ret = run_coro(
            ...
            datalog_svc->list_entries(
                dpp(), shard_id, max_entries - count, marker),
            std::tie(entries, marker, truncated), &errstr);
      } else {
        ret = run_coro(
            dpp(), context_pool,
            datalog_svc->list_entries(dpp(), max_entries - count, log_marker),
            std::tie(entries, log_marker, truncated), &errstr);
      }
      if (ret < 0) {
  ...
  if (opt_cmd == OPT::DATALOG_AUTOTRIM) {
    ...
    ret = crs.run(
        dpp(), create_admin_data_log_trim_cr(
                   dpp(), static_cast<rgw::sal::RadosStore*>(driver), &http,
                   num_shards, markers));
    if (ret < 0) {
      cerr << "automated datalog trim failed with " << cpp_strerror(ret)
           << std::endl;
      return -ret;
    }
  }
  ...
  if (opt_cmd == OPT::DATALOG_TRIM) {
    ...
    auto datalog =
        static_cast<rgw::sal::RadosStore*>(driver)->svc()->datalog_rados;
    ret = run_coro(
        dpp(), context_pool, datalog->trim_entries(dpp(), shard_id, marker),
        &errstr);
  ...
  if (opt_cmd == OPT::DATALOG_TYPE) {
    ...
    std::string errstr;
    ret = run_coro(
        dpp(), context_pool, datalog->change_format(dpp(), *opt_log_type),
        &errstr);
    if (ret < 0) {
  ...
  if (opt_cmd == OPT::DATALOG_PRUNE) {
    ...
    std::string errstr;
    ret = run_coro(
        dpp(), context_pool, datalog->trim_generations(dpp(), through), &errstr);
  ...
  bool quota_op =
      (opt_cmd == OPT::QUOTA_SET || opt_cmd == OPT::QUOTA_ENABLE ||
  ...
  if (quota_op) {
    if (!bucket_name.empty()) {
      ...
      set_bucket_quota(
          driver, opt_cmd, tenant, bucket_name, max_size, max_objects,
          have_max_size, have_max_objects);
    } else if (!rgw::sal::User::empty(user)) {
      if (quota_scope == "bucket") {
        return set_user_bucket_quota(
            opt_cmd, ruser, user_op, max_size, max_objects, have_max_size,
            have_max_objects);
      } else if (quota_scope == "user") {
        return set_user_quota(
            opt_cmd, ruser, user_op, max_size, max_objects, have_max_size,
            have_max_objects);
      } else {
        cerr << "ERROR: invalid quota scope specification. Please specify "
      ...
      if (quota_scope != "bucket" && quota_scope != "account") {
        cerr << "ERROR: invalid quota scope specification. Please specify "
                "either --quota-scope=bucket or --quota-scope=account"
             << std::endl;
        return EINVAL;
      }
      ...
      if (have_max_size) {
        op_state.quota_max_size =
            std::max<int64_t>(-1, rgw_rounded_kb(max_size) * 1024);
      }

      std::string err_msg;
      ret = rgw::account::modify(
          dpp(), driver, op_state, err_msg, stream_flusher, null_yield);
      if (ret < 0) {
        cerr << "ERROR: failed to set account quota with " << cpp_strerror(-ret)
             << ": " << err_msg << std::endl;
        return -ret;
      }
    } else {
      cerr << "ERROR: bucket name or uid or account is required for quota "
              "operation"
           << std::endl;
      return EINVAL;
    }
  ...
  bool ratelimit_op_set =
      (opt_cmd == OPT::RATELIMIT_SET || opt_cmd == OPT::RATELIMIT_ENABLE ||
       opt_cmd == OPT::RATELIMIT_DISABLE);
  bool ratelimit_op_get = opt_cmd == OPT::RATELIMIT_GET;
  if (ratelimit_op_set) {
    if (bucket_name.empty() && rgw::sal::User::empty(user)) {
      cerr << "ERROR: bucket name or uid is required for ratelimit operation"
           << std::endl;
      return EINVAL;
    }
    ...
    if (!bucket_name.empty()) {
      if (!ratelimit_scope.empty() && ratelimit_scope != "bucket") {
        cerr << "ERROR: invalid ratelimit scope specification. (bucket scope "
                "is not bucket but bucket has been specified)"
             << std::endl;
        return EINVAL;
      }
      return set_bucket_ratelimit(
          driver, opt_cmd, tenant, bucket_name, max_read_ops, max_write_ops,
          max_list_ops, max_delete_ops, max_read_bytes, max_write_bytes,
          have_max_read_ops, have_max_write_ops, have_max_list_ops,
          have_max_delete_ops, have_max_read_bytes, have_max_write_bytes);
    } else if (!rgw::sal::User::empty(user)) {
      if (ratelimit_scope == "user") {
        return set_user_ratelimit(
            opt_cmd, user, max_read_ops, max_write_ops, max_list_ops,
            max_delete_ops, max_read_bytes, max_write_bytes, have_max_read_ops,
            have_max_write_ops, have_max_list_ops, have_max_delete_ops,
            have_max_read_bytes, have_max_write_bytes);
      } else {
        cerr << "ERROR: invalid ratelimit scope specification. Please specify "
                "either --ratelimit-scope=bucket, or --ratelimit-scope=user"
             << std::endl;
        return EINVAL;
      }
    }
  ...
  if (ratelimit_op_get) {
    ...
    if (!bucket_name.empty()) {
      ...
      if (ratelimit_scope == "user") {
        ...
        if (ret < 0) {
          std::cerr << "ERROR: failed to get a ratelimit for user id: '"
                    << user->get_id() << "', errno: " << cpp_strerror(-ret)
                    << std::endl;
        }
        return ret;
      } else {
  ...
  if (opt_cmd == OPT::MFA_CREATE) {
    ...
    if (totp_serial.empty()) {
      cerr << "ERROR: TOTP device serial number was not provided (via "
              "--totp-serial)"
           << std::endl;
      return EINVAL;
    }
    ...
    if (totp_seed.empty()) {
      cerr << "ERROR: TOTP device seed was not provided (via --totp-seed)"
           << std::endl;
      return EINVAL;
    }
    ...
    int ret =
        static_cast<rgw::sal::RadosStore*>(driver)->ctl()->meta.mgr->mutate(
            rgwrados::otp::get_meta_key(user->get_id()), mtime, &objv_tracker,
            null_yield, dpp(), MDLOG_STATUS_WRITE, [&] {
              return static_cast<rgw::sal::RadosStore*>(driver)
                  ->svc()
                  ->cls->mfa.create_mfa(
                      dpp(), user->get_id(), config, &objv_tracker, mtime,
                      null_yield);
            });
    if (ret < 0) {
        ...
        static_cast<rgw::sal::RadosStore*>(driver)->ctl()->meta.mgr->mutate(
            ...
            null_yield, dpp(), MDLOG_STATUS_WRITE, [&] {
              return static_cast<rgw::sal::RadosStore*>(driver)
                  ->svc()
                  ->cls->mfa.remove_mfa(
                      dpp(), user->get_id(), totp_serial, &objv_tracker, mtime,
                      null_yield);
            });
    if (ret < 0) {
  ...
  if (opt_cmd == OPT::MFA_GET) {
    ...
    int ret =
        static_cast<rgw::sal::RadosStore*>(driver)->svc()->cls->mfa.get_mfa(
            dpp(), user->get_id(), totp_serial, &result, null_yield);
    if (ret < 0) {
      if (ret == -ENOENT || ret == -ENODATA) {
        cerr << "MFA serial id not found" << std::endl;
      } else {
        cerr << "MFA retrieval failed, error: " << cpp_strerror(-ret)
             << std::endl;
      }
      return -ret;
    }
  ...
  if (opt_cmd == OPT::MFA_LIST) {
    ...
    int ret =
        static_cast<rgw::sal::RadosStore*>(driver)->svc()->cls->mfa.list_mfa(
  ...
  if (opt_cmd == OPT::MFA_CHECK) {
    ...
    list<rados::cls::otp::otp_info_t> result;
    int ret =
        static_cast<rgw::sal::RadosStore*>(driver)->svc()->cls->mfa.check_mfa(
  ...
  if (opt_cmd == OPT::MFA_RESYNC) {
    ...
    if (totp_pin.size() != 2) {
      cerr << "ERROR: missing two --totp-pin params (--totp-pin=<first> "
              "--totp-pin=<second>)"
           << std::endl;
      return EINVAL;
    }
    ...
    ret = static_cast<rgw::sal::RadosStore*>(driver)
              ->svc()
              ->cls->mfa.otp_get_current_time(
                  dpp(), user->get_id(), &now, null_yield);
    if (ret < 0) {
      cerr << "ERROR: failed to fetch current time from osd: "
           << cpp_strerror(-ret) << std::endl;
      return -ret;
    }
    time_t time_ofs;
    ...
    if (ret < 0) {
      if (ret == -ENOENT) {
        ...
        cerr << "ERROR: failed to scan for TOTP values: " << cpp_strerror(-ret)
             << std::endl;
      }
      return -ret;
    }
  ...
  if (opt_cmd == OPT::RESHARD_STALE_INSTANCES_LIST) {
    if (!static_cast<rgw::sal::RadosStore*>(driver)->svc()->zone->can_reshard() &&
        !yes_i_really_mean_it) {
      cerr << "Resharding disabled in a multisite env, stale instances "
              "unlikely from resharding"
           << std::endl;
      cerr << "These instances may not be safe to delete." << std::endl;
      cerr << "Use --yes-i-really-mean-it to force displaying these instances."
           << std::endl;
      return EINVAL;
    }

    ret = RGWBucketAdminOp::list_stale_instances(
        driver, bucket_op, stream_flusher, dpp(), null_yield);
    if (ret < 0) {
      cerr << "ERROR: listing stale instances" << cpp_strerror(-ret)
           << std::endl;
    }
  }
  ...
  if (opt_cmd == OPT::RESHARD_STALE_INSTANCES_DELETE) {
    if (!static_cast<rgw::sal::RadosStore*>(driver)->svc()->zone->can_reshard()) {
      cerr << "Resharding disabled in a multisite env. Stale instances are not "
              "safe to be deleted."
           << std::endl;
      return EINVAL;
    }

    ret = RGWBucketAdminOp::clear_stale_instances(
        driver, bucket_op, stream_flusher, dpp(), null_yield);
    if (ret < 0) {
      cerr << "ERROR: deleting stale instances" << cpp_strerror(-ret)
           << std::endl;
    }
  }
    ...
    for (; i < max_shards; i++) {
      ...
      do {
        ...
        ret = static_cast<rgw::sal::RadosStore*>(driver)->getRados()->bi_list(
            bs, "", marker, max_entries, &entries, &is_truncated, true,
            null_yield);
        if (ret < 0) {
  ...
  if (opt_cmd == OPT::RESHARDLOG_PURGE) {
    ...
    ret =
        static_cast<rgw::sal::RadosStore*>(driver)
            ->svc()
            ->bi_rados->trim_reshard_log(dpp(), null_yield, bucket->get_info());
    if (ret < 0) {
  ...
  if (opt_cmd == OPT::PUBSUB_TOPIC_LIST) {
    ...
    if (rgw::all_zonegroups_support(*site, rgw::zone_features::notification_v2) &&
      ...
      do {
        int ret = ps.get_topics_v2(
            dpp(), next_token, max_entries, result, next_token, null_yield);
        if (ret < 0 && ret != -ENOENT) {
          cerr << "ERROR: could not get topics: " << cpp_strerror(-ret)
               << std::endl;
          return -ret;
        ...
        for (const auto& [_, topic] : result.topics) {
          ...
          ret = driver->get_bucket_topic_mapping(
              topic, subscribed_buckets, null_yield, dpp());
          if (ret < 0) {
  ...
  if (opt_cmd == OPT::PUBSUB_NOTIFICATION_GET) {
    if (notification_id.empty()) {
      cerr << "ERROR: notification-id was not provided (via --notification-id)"
           << std::endl;
      return EINVAL;
    }
    if (bucket_name.empty()) {
  ...
  if (opt_cmd == OPT::PUBSUB_TOPIC_RM) {
    ...
    if (ret < 0) {
      cerr << "ERROR: could not remove topic: " << cpp_strerror(-ret)
           << std::endl;
      return -ret;
    }
  }
  ...
  if (opt_cmd == OPT::PUBSUB_NOTIFICATION_RM) {
    ...
    if (rgw::all_zonegroups_support(*site, rgw::zone_features::notification_v2)) {
      if (ret = driver->stat_topics_v1(tenant, null_yield, dpp());
          ret != -ENOENT) {
        cerr << "WARNING: "
             << (ret == 0 ? "topic migration in process"
                          : "cannot determine topic migration status. ret = " +
                                std::to_string(ret))
             << ". please try again later" << std::endl;
        return -ret;
      }
      ret = remove_notification_v2(
          dpp(), driver, bucket.get(), notification_id, null_yield);
    } else {
      const std::string& account = !account_id.empty() ? account_id : tenant;
      RGWPubSub ps(driver, account, *site);
  ...
  if (opt_cmd == OPT::PUBSUB_TOPIC_STATS) {
    ...
    auto ioctx = static_cast<rgw::sal::RadosStore*>(driver)
                     ->getRados()
                     ->get_notif_pool_ctx();
    rgw::notify::rgw_topic_stats stats;
    ret = rgw::notify::get_persistent_queue_stats(
        dpp(), ioctx, topic.dest.get_shard_names(), stats, null_yield);
    if (ret < 0) {
      cerr << "ERROR: could not get persistent queues: " << cpp_strerror(-ret)
           << std::endl;
      return -ret;
    }
  ...
  if (opt_cmd == OPT::PUBSUB_TOPIC_DUMP) {
    ...
    ret = ps.get_topic(dpp(), topic_name, topic, null_yield, nullptr);
    if (ret < 0) {
      cerr << "ERROR: could not get topic. error: " << cpp_strerror(-ret)
           << std::endl;
      return -ret;
    }
    ...
    for (const auto& shard_name : topic.dest.get_shard_names()) {
      ...
      while (truncated) {
        ...
        if (rc < 0) {
          cerr << "ERROR: could not list entries from queue. error: "
               << cpp_strerror(-ret) << std::endl;
          return -rc;
        }
        rc = cls_2pc_queue_list_entries_result(
            bl, queue_entries, &truncated, end_marker);
        if (rc < 0) {
          cerr << "ERROR: failed to parse list entries from queue (skipping). "
                  "error: "
               << cpp_strerror(-ret) << std::endl;
          return -rc;
        }
        std::for_each(
            queue_entries.cbegin(), queue_entries.cend(),
            [&formatter](const auto& queue_entry) {
              ...
              try {
                ...
                cerr << "ERROR: failed to decode queue entry. error: "
                     << e.what() << std::endl;
              }
            });
        formatter->flush(cout);
  ...
  if (opt_cmd == OPT::SCRIPT_PUT) {
    ...
    if (rc < 0) {
      cerr << "ERROR: failed to read script: '" << infile << "'. error: " << rc
           << std::endl;
      return -rc;
    }
    ...
    rc = rgw::lua::write_script(
        dpp(), lua_manager.get(), tenant, null_yield, script_ctx, script);
    if (rc < 0) {
  ...
  if (opt_cmd == OPT::SCRIPT_RM) {
    ...
    auto lua_manager = driver->get_lua_manager("");
    const auto rc = rgw::lua::delete_script(
        dpp(), lua_manager.get(), tenant, null_yield, script_ctx);
    if (rc < 0) {
  ...
  if (opt_cmd == OPT::SCRIPT_PACKAGE_ADD) {
    ...
    if (!script_package) {
      cerr << "ERROR: Lua package name was not provided (via --package)"
           << std::endl;
      return EINVAL;
    }
    const auto rc = rgw::lua::add_package(
        dpp(), driver, null_yield, *script_package, bool(allow_compilation));
    if (rc < 0) {
      cerr << "ERROR: failed to add Lua package: " << script_package
           << " .error: " << rc << std::endl;
      return -rc;
    }
#else
  ...
  if (opt_cmd == OPT::SCRIPT_PACKAGE_RM) {
    ...
    const auto rc =
        rgw::lua::remove_package(dpp(), driver, null_yield, *script_package);
    if (rc == -ENOENT) {
      cerr << "WARNING: package " << script_package
           << " did not exists or already removed" << std::endl;
      return 0;
    }
    if (rc < 0) {
      cerr << "ERROR: failed to remove Lua package: " << script_package
           << " .error: " << rc << std::endl;
      return -rc;
    }
#else
  ...
  if (opt_cmd == OPT::SCRIPT_PACKAGE_LIST) {
    ...
    if (rc == -ENOENT) {
      ...
      cerr << "ERROR: failed to read Lua packages allowlist. error: " << rc
           << std::endl;
      return rc;
    } else {
  ...
  if (opt_cmd == OPT::SCRIPT_PACKAGE_RELOAD) {
    ...
    if (rc < 0) {
      cerr << "ERROR: failed to reload Lua packages. error: " << rc
           << std::endl;
      return rc;
    }
#else
  ...
  if (opt_cmd == OPT::ACCOUNT_CREATE || opt_cmd == OPT::ACCOUNT_MODIFY ||
      opt_cmd == OPT::ACCOUNT_GET || opt_cmd == OPT::ACCOUNT_STATS ||
    ...
    if (opt_cmd == OPT::ACCOUNT_STATS) {
      ret = rgw::account::stats(
          dpp(), driver, op_state, sync_stats, reset_stats, err_msg,
          stream_flusher, null_yield);
      if (ret < 0) {
        cerr << "ERROR: failed to read account stats with "
  ...
  if (opt_cmd == OPT::RESTORE_STATUS || opt_cmd == OPT::RESTORE_LIST) {
    ...
    if (opt_cmd == OPT::RESTORE_STATUS) {
      ...
      ret = driver->get_rgwrestore()->status(
          dpp(), entry, err_msg, stream_flusher, null_yield);
    } else if (opt_cmd == OPT::RESTORE_LIST) {
      ret = driver->get_rgwrestore()->list(
          dpp(), entry, restore_status_filter, err_msg, stream_flusher,
          null_yield);
    }
  }
  return 0;
