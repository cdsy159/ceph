...
std::string
incremental_marker(const rgw_bucket_shard_sync_info& info)
{
...
bool
operator<(
    const std::vector<rgw_bucket_shard_sync_info>& lhs,
...
bool
empty(const BucketIndexShardsManager& markers, int size)
{
...
std::ostream&
operator<<(std::ostream& out, const std::vector<rgw_bucket_shard_sync_info>& rhs)
{
...
std::ostream&
operator<<(std::ostream& out, const BucketIndexShardsManager& rhs)
{
  out << '[';
  const char* separator = ""; // first entry has no comma
    ...
    ceph::coarse_mono_time timeout_at)
{
  ...
  int r = rgw_read_bucket_full_sync_status(
      dpp, store, pipe, &full_status, null_yield);
  ...
  while (full_status.state != BucketSyncState::Incremental) {
    ...
    if (delay_until > timeout_at) {
      lderr(store->ctx())
          << "bucket checkpoint timed out waiting to reach incremental sync"
          << dendl;
      return -ETIMEDOUT;
    }
  ...
  while (full_status.incremental_gen < latest_gen) {
    ...
    ldout(store->ctx(), 1) << "waiting to reach latest gen " << latest_gen
                           << ", on " << full_status.incremental_gen << ".."
                           << dendl;
    std::this_thread::sleep_until(delay_until);
  ...
  r = rgw_read_bucket_inc_sync_status(
      dpp, store, pipe, full_status.incremental_gen, &status);
  if (r < 0) {
    return r;
  }
  ...
  while (status < remote_markers) {
    const auto delay_until = ceph::coarse_mono_clock::now() + retry_delay;
    if (delay_until > timeout_at) {
      std::cerr << "bucket checkpoint timed out waiting for incremental sync "
                   "to catch up"
                << std::endl;
      return -ETIMEDOUT;
    }
    ...
    optional_yield y)
{
  ...
  return rgw_read_remote_bilog_info(
      dpp, conn->second, *pipe.source.bucket, info, markers, y);
}

} // anonymous namespace

int
rgw_bucket_sync_checkpoint(
    const DoutPrefixProvider* dpp,
    rgw::sal::RadosStore* store,
    ...
    ceph::coarse_mono_time timeout_at)
{
  ...
  for (const auto& [source_zone_id, pipe] : policy.get_all_sources()) {
    ...
    entry.pipe = pipe;
    entry.pipe.dest.bucket =
        info.bucket; // so it contains the bucket key (+bucket id)
    ...
    boost::asio::spawn(
        ...
        [&](boost::asio::yield_context yield) {
          ...
          rgw_bucket_index_marker_info info;
          int r = source_bilog_info(
              dpp, store->svc()->zone, entry.pipe, info, entry.remote_markers,
              y);
          if (r < 0) {
            ldpp_dout(dpp, 0)
                << "failed to fetch remote bilog markers: " << cpp_strerror(r)
                << dendl;
    ...
    boost::asio::spawn(
        ...
        [&](boost::asio::yield_context yield) {
          ...
          int r = store->getRados()->get_bucket_instance_info(
              *entry.pipe.source.bucket, entry.source_bucket_info, nullptr,
              nullptr, y, dpp);
          if (r < 0) {
            ldpp_dout(dpp, 0)
                << "failed to read source bucket info: " << cpp_strerror(r)
                << dendl;
            throw std::system_error(-r, std::system_category());
          }
        },
  ...
  for (const auto& e : sources) {
    int r = bucket_source_sync_checkpoint(
        dpp, store, info, e.source_bucket_info, e.pipe, e.latest_gen,
        e.remote_markers, retry_delay, timeout_at);
    if (r < 0) {
      std::cerr << "bucket sync checkpoint failed: " << cpp_strerror(r)
                << std::endl;
      return r;
    }
  }
