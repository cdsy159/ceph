...
bool
operator==(const ACLPermission& lhs, const ACLPermission& rhs)
{
...
bool
operator==(const ACLGranteeType& lhs, const ACLGranteeType& rhs)
{
...
bool
operator==(const ACLGrant& lhs, const ACLGrant& rhs)
{
...
bool
operator==(const ACLReferer& lhs, const ACLReferer& rhs)
{
...
bool
operator==(const RGWAccessControlList& lhs, const RGWAccessControlList& rhs)
{
...
bool
operator==(const RGWAccessControlPolicy& lhs, const RGWAccessControlPolicy& rhs)
{
...
void
RGWAccessControlList::register_grant(const ACLGrant& grant)
{
...
void
RGWAccessControlList::add_grant(const ACLGrant& grant)
{
  std::string id;
  if (const auto* user = grant.get_user(); user) {
...
void
RGWAccessControlList::remove_canon_user_grant(const rgw_owner& owner)
{
...
uint32_t
RGWAccessControlList::get_group_perm(
    const DoutPrefixProvider* dpp,
...
uint32_t
RGWAccessControlList::get_referer_perm(
    const DoutPrefixProvider* dpp,
...
uint32_t
RGWAccessControlPolicy::get_perm(
    const DoutPrefixProvider* dpp,
    const rgw::auth::Identity& auth_identity,
...
{
  ldpp_dout(dpp, 20) << "-- Getting permissions begin with perm_mask="
                     << perm_mask << dendl;
  ...
  ldpp_dout(dpp, 5) << "-- Getting permissions done for identity="
                    << auth_identity << ", owner=" << owner.id
                    << ", perm=" << perm << dendl;
    ...
    bool ignore_public_acls) const
{
  ...
  uint32_t policy_perm =
      get_perm(dpp, auth_identity, test_perm, http_referer, ignore_public_acls);
...
bool
RGWAccessControlPolicy::is_public(const DoutPrefixProvider* dpp) const
{

  static constexpr auto public_groups = {
      ACL_GROUP_ALL_USERS, ACL_GROUP_AUTHENTICATED_USERS};
  return std::any_of(
      public_groups.begin(), public_groups.end(), [&, dpp](ACLGroupTypeEnum g) {
...
list<ACLPermission>
ACLPermission::generate_test_instances()
{
...
void
ACLPermission::dump(Formatter* f) const
{
  f->dump_int("flags", flags);
}

void
ACLGranteeType::dump(Formatter* f) const
{
  f->dump_unsigned("type", type);
}

void
ACLGrant::dump(Formatter* f) const
{
  ...
  struct dump_visitor {
    ...
    void
    operator()(const ACLGranteeCanonicalUser& user)
    {
    ...
    void
    operator()(const ACLGranteeEmailUser& email)
    {
      encode_json("email", email.address, f);
    }

    void
    operator()(const ACLGranteeGroup& group)
    {
      encode_json("group", static_cast<int>(group.type), f);
    }

    void
    operator()(const ACLGranteeUnknown&)
    {}

    void
    operator()(const ACLGranteeReferer& r)
    {
...
list<ACLGrant>
ACLGrant::generate_test_instances()
{
  list<ACLGrant> o;
  ACLGrant g1;
...
list<ACLGranteeType>
ACLGranteeType::generate_test_instances()
{
...
list<ACLOwner>
ACLOwner::generate_test_instances()
{
  list<ACLOwner> o;
  ACLOwner owner;
  owner.id = "rgw";
...
list<RGWAccessControlPolicy>
RGWAccessControlPolicy::generate_test_instances()
{
...
void
RGWAccessControlList::dump(Formatter* f) const
{
...
void
ACLOwner::dump(Formatter* f) const
{
...
void
ACLOwner::decode_json(JSONObj* obj)
{
  string id_str;
...
void
RGWAccessControlPolicy::dump(Formatter* f) const
{
...
ACLGroupTypeEnum
ACLGrant::uri_to_group(std::string_view uri)
{
