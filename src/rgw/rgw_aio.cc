...
void
cb(librados::completion_t, void* arg)
{
...
Aio::OpFunc
aio_abstract(librados::IoCtx ctx, Op&& op, jspan_context* trace_ctx = nullptr)
{
  return [ctx = std::move(ctx), op = std::forward<Op>(op),
          trace_ctx](Aio* aio, AioResult& r) mutable {
    constexpr bool read =
        std::is_same_v<std::decay_t<Op>, librados::ObjectReadOperation>;
...
struct Handler {
  ...
  // write callback
  void
  operator()(boost::system::error_code ec, version_t) const
  {
    r.result = -ec.value();
    throttle->put(r);
  }
  ...
  // read callback
  void
  operator()(boost::system::error_code ec, version_t, bufferlist bl) const
  {
    r.result = -ec.value();
    r.data = std::move(bl);
    throttle->put(r);
...
{
  return [dpp, y, read_ofs, read_len,
          cache_location](Aio* aio, AioResult& r) mutable {
    ...
    lsubdout(g_ceph_context, rgw_datacache, 20)
        << "D3nDataCache: d3n_cache_aio_abstract(): libaio Read From Cache, "
           "oid="
        << r.obj.oid << dendl;
    c->file_aio_read_abstract(
        dpp, y.get_yield_context(), cache_location, read_ofs, read_len, aio, r);
  };
}
    ...
    jspan_context* trace_ctx = nullptr)
{
  ...
  if (y) {
    return aio_abstract(
        std::move(ctx), std::forward<Op>(op), y.get_yield_context(), trace_ctx);
  }
