...
connection_id_t::connection_id_t(
    const amqp_connection_info& info,
...
{
  return fmt::format(
      "{}://{}:{}{}?exchange={}", id.ssl ? "amqps" : "amqp", id.host, id.port,
      id.vhost, id.exchange);
}
...
struct connection_t {
  ...
  destroy(int s)
  {
    ...
    std::for_each(callbacks.begin(), callbacks.end(), [this](auto& cb_tag) {
      cb_tag.cb(status);
      ldout(cct, 20) << "AMQP destroy: invoking callback with tag="
                     << cb_tag.tag << dendl;
    });
    callbacks.clear();
    delivery_tag = 1;
...
std::string
to_string(const amqp_connection_info& info)
{
  std::stringstream ss;
  ss << "connection info:" << "\nHost: " << info.host << "\nPort: " << info.port
     << "\nUser: " << info.user << "\nPassword: " << info.password
...
int
reply_to_code(const amqp_rpc_reply_t& reply)
{
...
std::string
to_string(const amqp_rpc_reply_t& reply)
{
  std::stringstream ss;
  switch (reply.reply_type) {
  case AMQP_RESPONSE_NORMAL:
...
bool
new_state(connection_t* conn, const connection_id_t& conn_id)
{
  ...
  if (conn->use_ssl) {
    ...
    SSL_CTX* ssl_ctx =
        reinterpret_cast<SSL_CTX*>(amqp_ssl_socket_get_context(socket));
#else
  ...
  if (conn->use_ssl) {
    ...
    if (conn->ca_location.has_value()) {
      const auto s =
          amqp_ssl_socket_set_cacert(socket, conn->ca_location.get().c_str());
      if (s != AMQP_STATUS_OK) {
  ...
  const auto reply = amqp_login(
      state, conn_id.vhost.c_str(), AMQP_DEFAULT_MAX_CHANNELS,
  ...
  {
    const auto ok = amqp_exchange_declare(
        state, CHANNEL_ID, amqp_cstring_bytes(conn_id.exchange.c_str()),
  ...
  }
  {
    ...
    const auto queue_ok = amqp_queue_declare(
        state,
    ...
    const auto consume_ok = amqp_basic_consume(
        state, CONFIRMING_CHANNEL_ID, queue_ok->queue,
...
private:
  ...
  publish_internal(message_wrapper_t* message)
  {
    ...
    if (conn_it == connections.end()) {
      ldout(cct, 1) << "AMQP publish: connection '" << to_string(conn_id)
                    << "' not found" << dendl;
      if (message->cb) {
    ...
    if (message->cb == nullptr) {
      const auto rc = amqp_basic_publish(
          conn->state, CHANNEL_ID, amqp_cstring_bytes(conn_id.exchange.c_str()),
      ...
      ldout(cct, 1) << "AMQP publish (no callback): failed with error "
                    << status_to_string(rc) << dendl;
    ...
    const auto rc = amqp_basic_publish(
        conn->state, CONFIRMING_CHANNEL_ID,
        amqp_cstring_bytes(conn_id.exchange.c_str()),
        amqp_cstring_bytes(message->topic.c_str()), conn->mandatory,
        0, // not immediate
        &props, amqp_cstring_bytes(message->message.c_str()));

    if (rc == AMQP_STATUS_OK) {
      auto const q_len = conn->callbacks.size();
      if (q_len < max_inflight) {
        ldout(cct, 20) << "AMQP publish (with callback, tag="
                       << conn->delivery_tag << "): OK. Queue has: " << q_len
                       << " callbacks" << dendl;
        conn->callbacks.emplace_back(conn->delivery_tag++, message->cb);
      } else {
        // immediately invoke callback with error
        ldout(cct, 1) << "AMQP publish (with callback): failed with error: "
                         "callback queue full"
                      << dendl;
  ...
  run() noexcept
  {
    ...
    while (!stopped) {
      ...
      const auto count = messages.consume_all(
      ...
      for (; conn_it != end_it;) {
        ...
        if (conn->timestamp.sec() + max_idle_time < ceph_clock_now()) {
          ldout(cct, 20) << "AMQP run: Time for deleting a connection due to "
                            "idle behaviour: "
        ...
        if (!conn->is_ok()) {
          ...
          if (now >= conn->next_reconnect) {
            ...
            if (!new_state(conn.get(), conn_id)) {
              ldout(cct, 10)
                  << "AMQP run: connection '" << to_string(conn_id)
        ...
        const auto rc =
            amqp_simple_wait_frame_noblock(conn->state, &frame, &read_timeout);
        ...
        if (rc != AMQP_STATUS_OK) {
          // an error occurred, close connection
          // it will be retied by the main loop
          ldout(cct, 1) << "AMQP run: connection read error: "
                        << status_to_string(rc) << dendl;
          conn->destroy(rc);
          INCREMENT_AND_CONTINUE(conn_it);
        }
        ...
        if (frame.frame_type != AMQP_FRAME_METHOD) {
          ldout(cct, 10)
              << "AMQP run: ignoring non n/ack messages. frame type: "
        ...
        case AMQP_CHANNEL_CLOSE_METHOD: {
          ...
          ldout(cct, 10) << "AMQP run: connection was closed by broker"
                         << dendl;
          conn->destroy(rc);
          INCREMENT_AND_CONTINUE(conn_it);
        }
        ...
        if (tag_it != conn->callbacks.end()) {
          if (multiple) {
            ...
            ldout(cct, 20)
                << "AMQP run: n/ack received, invoking callback with tag="
                << tag << " and result=" << result << dendl;
            tag_it->cb(result);
            conn->callbacks.erase(tag_it);
          }
        } else {
          ldout(cct, 10) << "AMQP run: unsolicited n/ack received with tag="
                         << tag << dendl;
        }
...
public:
      ...
      boost::optional<const std::string&> ca_location)
  {
    ...
    if (AMQP_STATUS_OK != retcode) {
      ldout(cct, 1) << "AMQP connect: URL parsing failed. error: " << retcode
                    << dendl;
      return false;
    }
    ...
    ++connection_count;
    auto conn =
        std::make_unique<connection_t>(cct, info, verify_ssl, ca_location);
    if (new_state(conn.get(), tmp_id)) {
      ldout(cct, 10)
          << "AMQP connect: new connection is created. Total connections: "
          << connection_count << dendl;
    } else {
      ldout(cct, 1)
          << "AMQP connect: new connection '" << to_string(tmp_id)
          << "' is created. but state creation failed (will retry). error: "
          << status_to_string(conn->status) << " (" << conn->reply_code << ")"
          << dendl;
    }
  ...
  {
    if (stopped) {
      ldout(cct, 1) << "AMQP publish_with_confirm: manager is not running"
                    << dendl;
      return RGW_AMQP_STATUS_MANAGER_STOPPED;
    }
    auto wrapper =
  ...
  get_inflight() const
  {
    ...
    std::lock_guard lock(connections_lock);
    std::for_each(
        connections.begin(), connections.end(), [&sum](auto& conn_pair) {
...
init(CephContext* cct)
{
  ...
  s_manager = new Manager(
      MAX_CONNECTIONS_DEFAULT, MAX_INFLIGHT_DEFAULT, MAX_QUEUE_DEFAULT,
    ...
    boost::optional<const std::string&> ca_location)
{
  ...
  return s_manager->connect(
      conn_id, url, exchange, mandatory_delivery, verify_ssl, ca_location);
}
