...
namespace {
boost::optional<Partition>
to_partition(const smatch::value_type& p, bool wildcards)
{
  if (p == "aws") {
...
boost::optional<Service>
to_service(const smatch::value_type& s, bool wildcards)
{
...
boost::optional<ARN>
ARN::parse(const std::string& s, bool wildcards)
{
  static const std::regex rx_wild(
      "arn:([^:]*):([^:]*):([^:]*):([^:]*):([^:]*)",
  ...
  if ((s == "*") && wildcards) {
    return ARN(Partition::wildcard, Service::wildcard, "*", "*", "*");
      ...
      regex_match(s, match, wildcards ? rx_wild : rx_no_wild) &&
      match.size() == 6) {
...
bool
operator==(const ARN& l, const ARN& r)
{
  return (
      (l.partition == r.partition) && (l.service == r.service) &&
      (l.region == r.region) && (l.account == r.account) &&
...
bool
ARN::match(const ARN& candidate) const
{
...
boost::optional<ARNResource>
ARNResource::parse(const std::string& s)
{
