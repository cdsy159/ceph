...
struct ARN {
  ...
  ARN(const std::string& resource_name,
...
inline std::string
to_string(const ARN& a)
{
  return a.to_string();
}

inline std::ostream&
operator<<(std::ostream& m, const ARN& a)
{
...
struct ARNResource {
  ...
  ARNResource() :
    resource_type(""), resource(wildcard), qualifier("")
  {}

  ARNResource(
      const std::string& _resource_type,
...
struct hash<::rgw::Service> {
  size_t
  operator()(const ::rgw::Service& s) const noexcept
  {
