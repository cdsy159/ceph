...
void
decode_json_obj(rgw_user& val, JSONObj* obj)
{
  val.from_str(obj->get_data());
}

void
encode_json(const char* name, const rgw_user& val, Formatter* f)
{
  f->dump_string(name, val.to_str());
}

void
encode_xml(const char* name, const rgw_user& val, Formatter* f)
{
...
void
rgw_bucket::convert(cls_user_bucket* b) const
{
  b->name = name;
  b->marker = marker;
  ...
  b->explicit_placement.data_extra_pool =
      explicit_placement.data_extra_pool.to_str();
  b->explicit_placement.index_pool = explicit_placement.index_pool.to_str();
}

std::string
rgw_bucket::get_key(char tenant_delim, char id_delim, size_t reserve) const
{
  const size_t max_len = tenant.size() + sizeof(tenant_delim) + name.size() +
                         sizeof(id_delim) + bucket_id.size() + reserve;
...
list<rgw_bucket>
rgw_bucket::generate_test_instances()
{
  list<rgw_bucket> o;
  rgw_bucket b;
...
void
encode(const rgw_bucket_shard& b, bufferlist& bl, uint64_t f)
{
...
void
decode(rgw_bucket_shard& b, bufferlist::const_iterator& bl)
{
  decode(b.bucket, bl);
...
void
encode_json_impl(const char* name, const rgw_zone_id& zid, Formatter* f)
{
  encode_json(name, zid.id, f);
}

void
decode_json_obj(rgw_zone_id& zid, JSONObj* obj)
{
  decode_json_obj(zid.id, obj);
}

list<rgw_user>
rgw_user::generate_test_instances()
{
  list<rgw_user> o;
...
void
rgw_data_placement_target::dump(Formatter* f) const
{
...
void
rgw_data_placement_target::decode_json(JSONObj* obj)
{
...
void
rgw_bucket::dump(Formatter* f) const
{
...
void
rgw_bucket::decode_json(JSONObj* obj)
{
  ...
  if (explicit_placement.data_pool.empty()) {
    ...
    JSONDecoder::decode_json(
        "data_extra_pool", explicit_placement.data_extra_pool, obj);
...
namespace auth {
ostream&
operator<<(ostream& m, const Principal& p)
{
  if (p.is_wildcard()) {
    return m << "*";
  }
...
// rgw_account_id
void
encode_json_impl(const char* name, const rgw_account_id& id, Formatter* f)
{
  f->dump_string(name, id);
}

void
decode_json_obj(rgw_account_id& id, JSONObj* obj)
{
...
// rgw_owner variant
rgw_owner
parse_owner(const std::string& str)
{
...
std::ostream&
operator<<(std::ostream& out, const rgw_owner& o)
{
  struct visitor {
    std::ostream& out;
...
void
encode_json_impl(const char* name, const rgw_owner& o, ceph::Formatter* f)
{
  encode_json(name, to_string(o), f);
}

void
decode_json_obj(rgw_owner& o, JSONObj* obj)
{
  std::string str;
