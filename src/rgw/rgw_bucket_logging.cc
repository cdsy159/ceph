...
bool
configuration::decode_xml(XMLObj* obj)
{
  ...
  if (o) {
    ...
    RGWXMLDecoder::decode_xml(
        "TargetBucket", target_bucket, o, throw_if_missing);
    RGWXMLDecoder::decode_xml("TargetPrefix", target_prefix, o);
    // TODO: decode grant
    RGWXMLDecoder::decode_xml(
        "ObjectRollTime", obj_roll_time, default_obj_roll_time, o);
    ...
    if (iter = o->find("TargetObjectKeyFormat");
      ...
      if (iter = oo->find("PartitionedPrefix");
        ...
        if (type == "DeliveryTime") {
          ...
          throw RGWXMLDecoder::err(
              "invalid bucket logging partition date source: '" + type + "'");
        }
      } else if (iter = oo->find("SimplePrefix"); iter.get_next()) {
        obj_key_format = KeyFormat::Simple;
      } else {
        throw RGWXMLDecoder::err(
            "TargetObjectKeyFormat must contain a format tag");
      }
    }
  }
...
void
configuration::dump_xml(Formatter* f) const
{
  if (!enabled) {
    return;
  }
...
void
configuration::dump(Formatter* f) const
{
...
std::string
configuration::to_json_str() const
{
  JSONFormatter f;
  dump(&f);
  std::stringstream ss;
...
random_string()
{
  static const std::string possible_characters{
  ...
  std::generate_n(str.begin(), N, [&]() {
    return possible_characters[dist(engine)];
  });
  return str;
}
    ...
    std::optional<std::string> old_name)
{
  ...
  const auto str_counter =
      old_name->substr(old_name->length() - (INC + RND), INC);
  try {
    ...
    ldpp_dout(dpp, 5) << "WARNING: failed to convert string '" << str_counter
                      << "' to counter. " << e.what()
                      << ". will create random temporary logging file name"
                      << dendl;
...
time_from_name(const std::string& obj_name, const DoutPrefixProvider* dpp)
{
  static const auto time_format_length =
      std::string{"YYYY-MM-DD-hh-mm-ss"}.length();
  ...
  if (obj_name_length < time_format_length + UniqueStringLength + 1) {
    ldpp_dout(dpp, 1) << "ERROR: logging object name too short: " << obj_name
                      << dendl;
    return extracted_time;
  }
  const auto time_start_pos = obj_name_length -
                              (time_format_length + UniqueStringLength + 1);
  ...
  if (const char* ret = strptime(time_str.c_str(), "%Y-%m-%d-%H-%M-%S", &t);
      ret == nullptr || *ret != '\0') {
    ldpp_dout(dpp, 1) << "ERROR: invalid time format: '" << time_str
                      << "' in logging object name: " << obj_name << dendl;
    return extracted_time;
  }
  extracted_time = ceph::coarse_real_time::clock::from_time_t(mktime(&t));
  ldpp_dout(dpp, 20) << "INFO: time '" << extracted_time
                     << "' extracted from logging object name: " << obj_name
                     << dendl;
  return extracted_time;
}

std::string
full_bucket_name(const std::unique_ptr<rgw::sal::Bucket>& bucket)
{
...
{
  const auto tt = ceph::coarse_real_time::clock::to_time_t(
      ceph::coarse_real_time::clock::now());
  std::tm t{};
  localtime_r(&tt, &t);

  const auto unique =
      incremental_string<CounterStringLength, RandomStringLength>(dpp, old_name);

  switch (conf.obj_key_format) {
  case KeyFormat::Simple:
    // [DestinationPrefix][YYYY]-[MM]-[DD]-[hh]-[mm]-[ss]-[UniqueString]
    obj_name =
        fmt::format("{}{:%Y-%m-%d-%H-%M-%S}-{}", conf.target_prefix, t, unique);
    break;
  case KeyFormat::Partitioned: {
    // TODO: support both EventTime and DeliveryTime
    // [DestinationPrefix][SourceAccountId]/[SourceRegion]/[SourceBucket]/[YYYY]/[MM]/[DD]/[YYYY]-[MM]-[DD]-[hh]-[mm]-[ss]-[UniqueString]
    obj_name = fmt::format(
        "{}{}/{}/{}/{:%Y/%m/%d}/{:%Y-%m-%d-%H-%M-%S}-{}", conf.target_prefix,
        to_string(source_bucket->get_owner()), region,
        full_bucket_name(source_bucket), t, t, unique);
  } break;
  }
  const auto& target_bucket_id = target_bucket->get_key();
  int ret = target_bucket->set_logging_object_name(
      obj_name, conf.target_prefix, y, dpp, (old_name == std::nullopt),
      objv_tracker);
  if (ret == -EEXIST || ret == -ECANCELED) {
    if (ret = target_bucket->get_logging_object_name(
            obj_name, conf.target_prefix, y, dpp, nullptr);
        ret < 0) {
      ldpp_dout(dpp, 1)
          << "ERROR: failed to get name of logging object of logging bucket '"
          << target_bucket_id << "' and prefix '" << conf.target_prefix
          << "', ret = " << ret << dendl;
      return ret;
    }
    ldpp_dout(dpp, 20) << "INFO: name already set. got name of logging object '"
                       << obj_name << "' of logging bucket '"
                       << target_bucket_id << "' and prefix '"
                       << conf.target_prefix << "'" << dendl;
    return -ECANCELED;
  } else if (ret < 0) {
    ldpp_dout(dpp, 1) << "ERROR: failed to write name of logging object '"
                      << obj_name << "' of logging bucket '" << target_bucket_id
                      << "'. ret = " << ret << dendl;
    return ret;
  }
  ldpp_dout(dpp, 20) << "INFO: wrote name of logging object '" << obj_name
                     << "' of logging bucket '" << target_bucket_id << "'"
                     << dendl;
  return 0;
}
    ...
    std::string* last_committed)
{
  ...
  int ret = rgw_parse_url_bucket(
      conf.target_bucket, tenant_name, target_tenant_name, target_bucket_name);
  if (ret < 0) {
    ldpp_dout(dpp, 1) << "ERROR: failed to parse logging bucket '"
                      << conf.target_bucket
  ...
  ret = driver->load_bucket(dpp, target_bucket_id, &target_bucket, y);
  if (ret < 0) {
    ldpp_dout(dpp, 1) << "ERROR: failed to get logging bucket '"
                      << target_bucket_id
                      << "' when committing logging object, ret = " << ret
                      << dendl;
    return ret;
  ...
  std::string obj_name;
  if (const int ret = target_bucket->get_logging_object_name(
          obj_name, conf.target_prefix, y, dpp, nullptr);
      ret < 0) {
    ldpp_dout(dpp, 1)
        << "ERROR: failed to get name of logging object of logging bucket '"
  ...
  if (const int ret = target_bucket->commit_logging_object(
          obj_name, y, dpp, conf.target_prefix, last_committed, false);
      ret < 0) {
    ldpp_dout(dpp, 1) << "ERROR: failed to commit logging object '" << obj_name
                      << "' of logging bucket '" << target_bucket->get_key()
                      << "'. ret = " << ret << dendl;
    return ret;
  }
  return 0;
    ...
    std::string* err_message)
{
  std::string target_bucket_name;
  std::string target_tenant_name;
  std::ignore = rgw_parse_url_bucket(
      conf.target_bucket, target_bucket->get_tenant(), target_tenant_name,
      target_bucket_name);
  if (target_bucket_name != target_bucket->get_name() ||
      target_tenant_name != target_bucket->get_tenant()) {
    ldpp_dout(dpp, 1) << "ERROR: logging bucket name mismatch. conf= '"
                      << conf.target_bucket << "', bucket= '"
                      << target_bucket->get_key() << "'" << dendl;
    return -EINVAL; // this should never happen
  }

  auto old_obj = obj_name.empty() ? std::nullopt
                                  : std::optional<std::string>(obj_name);

  auto handle_error =
      [&dpp, &old_obj, &target_bucket, err_message](int ret) {
        if (ret < 0) {
          if (ret == -ECANCELED) {
            ldpp_dout(dpp, 20)
                << "INFO: rollover already performed for logging object '"
                << old_obj << "' to logging bucket '"
                << target_bucket->get_key() << "'. ret = " << ret << dendl;
            if (err_message) {
              *err_message = fmt::format(
                  "Rollover already performed on logging bucket '{}'",
                  target_bucket->get_name());
            }
          } else {
            ldpp_dout(dpp, 1)
                << "ERROR: failed to rollover logging object '" << old_obj
                << "' to logging bucket '" << target_bucket->get_key()
                << "'. ret = " << ret << dendl;
            if (err_message) {
              *err_message = fmt::format(
                  "Failed to rollover logging object of logging bucket '{}'",
                  target_bucket->get_name());
            }
          }
        }
  ...
  if (const int ret = handle_error(new_logging_object(
          conf, target_bucket, obj_name, dpp, region, source_bucket, y, old_obj,
          objv_tracker));
      ret < 0) {
    return ret;
  }
  if (!old_obj) {
    ...
    old_obj = obj_name;
    if (const int ret = handle_error(new_logging_object(
            conf, target_bucket, obj_name, dpp, region, source_bucket, y,
            old_obj, objv_tracker));
        ret < 0) {
      return ret;
    }
  }
  if (const int ret = target_bucket->commit_logging_object(
          *old_obj, y, dpp, conf.target_prefix, last_committed, async);
      ret < 0) {
    if (must_commit) {
      if (err_message) {
        *err_message = fmt::format(
            "Failed to commit logging object of logging bucket '{}'",
            target_bucket->get_name());
      }
      return ret;
    }
    ldpp_dout(dpp, 5) << "WARNING: failed to commit object '" << old_obj
                      << "' to logging bucket '" << target_bucket->get_key()
                      << "'. ret = " << ret << dendl;
...
#define dash_if_zero_or_null(P, I) \
  (((P) == nullptr) || ((I) == 0)) ? "-" : std::to_string(I)
    ...
    bool log_source_bucket)
{
  if (!s->bucket) {
    ldpp_dout(dpp, 1)
        << "ERROR: only bucket operations are logged in bucket logging"
        << dendl;
    return -EINVAL; // this should never happen
  }
  auto set_journal_err = [&conf, s](const std::string& err_message) {
    if (conf.logging_type == LoggingType::Journal)
      s->err.message = err_message;
  };
  std::string target_bucket_name;
  std::string target_tenant_name;
  int ret = rgw_parse_url_bucket(
      conf.target_bucket, s->bucket_tenant, target_tenant_name,
      target_bucket_name);
  if (ret < 0) {
    ldpp_dout(dpp, 1) << "ERROR: failed to parse logging bucket name '"
                      << conf.target_bucket << "', ret = " << ret << dendl;
    set_journal_err(fmt::format(
        "Failed to parse logging bucket name '{}'", conf.target_bucket));
    return ret;
  }
  const rgw_bucket target_bucket_id(target_tenant_name, target_bucket_name);
  std::unique_ptr<rgw::sal::Bucket> target_bucket;
  ret = driver->load_bucket(dpp, target_bucket_id, &target_bucket, y);
  if (ret < 0) {
    ldpp_dout(dpp, 1) << "ERROR: failed to load logging bucket '"
                      << target_bucket_id << "'. ret = " << ret << dendl;
    set_journal_err(fmt::format(
        "Failed to load logging bucket '{}'", target_bucket_id.bucket_id));
    return ret;
  }

  rgw::ARN target_resource_arn(target_bucket_id, conf.target_prefix);
  std::string err_message;
  if (ret = verify_target_bucket_policy(
          dpp, target_bucket.get(), target_resource_arn, s, &err_message);
      ret < 0) {
    set_journal_err(err_message);
    return ret;
  }

  if (ret = verify_target_bucket_attributes(
          dpp, target_bucket.get(), &err_message);
      ret < 0) {
    set_journal_err(err_message);
    return ret;
  }
  ...
  if (ret = update_bucket_logging_sources(
          dpp, target_bucket, s->bucket->get_key(), true, y);
      ret < 0) {
    ldpp_dout(dpp, 5) << "WARNING: could not update bucket logging source '"
                      << s->bucket->get_key() << "' in logging bucket '"
                      << target_bucket_id
                      << "' attribute, during record logging. ret = " << ret
                      << dendl;
  }
  ...
  ret = target_bucket->get_logging_object_name(
      obj_name, conf.target_prefix, y, dpp, &objv_tracker);
  if (ret == 0) {
    const auto time_to_commit = time_from_name(obj_name, dpp) +
                                std::chrono::seconds(conf.obj_roll_time);
    if (ceph::coarse_real_time::clock::now() > time_to_commit) {
      ldpp_dout(dpp, 20)
          << "INFO: logging object '" << obj_name
          << "' exceeded its time, will be committed to logging bucket '"
          << target_bucket_id << "'" << dendl;
      if (ret = rollover_logging_object(
              conf, target_bucket, obj_name, dpp, region, s->bucket, y, false,
              &objv_tracker, true, nullptr, &err_message);
          ret < 0 && ret != -ECANCELED) {
        set_journal_err(err_message);
        return ret;
      }
      ...
      ldpp_dout(dpp, 20)
          << "INFO: record will be written to current logging object '"
          << obj_name << "'. will be committed at: " << time_to_commit << dendl;
    }
  } else if (ret == -ENOENT) {
    // try to create the temporary log object for the first time
    ret = new_logging_object(
        conf, target_bucket, obj_name, dpp, region, s->bucket, y, std::nullopt,
        &objv_tracker);
    if (ret == 0) {
      ldpp_dout(dpp, 20) << "INFO: first time logging for logging bucket '"
                         << target_bucket_id << "' and prefix '"
                         << conf.target_prefix << "'" << dendl;
    } else if (ret == -ECANCELED) {
      ldpp_dout(dpp, 20) << "INFO: logging object '" << obj_name
                         << "' already exists for logging bucket '"
                         << target_bucket_id << "' and prefix"
                         << conf.target_prefix << "'" << dendl;
    } else {
      ldpp_dout(dpp, 1) << "ERROR: failed to create first time logging object "
                           "of logging bucket '"
                        << target_bucket_id << "' and prefix '"
                        << conf.target_prefix << "'. ret = " << ret << dendl;
      set_journal_err(fmt::format(
          "Failed to create first time logging object of logging bucket '{}'",
          target_bucket->get_name()));
      return ret;
    }
  } else {
    ldpp_dout(dpp, 1)
        ...
        << target_bucket_id << "'. ret = " << ret << dendl;
    set_journal_err(fmt::format(
        "Failed to get name of logging object of logging bucket '{}'",
        target_bucket->get_name()));
    return ret;
  }
  ...
  if (log_source_bucket && conf.logging_type == LoggingType::Standard) {
    ...
    if (!s->src_object || !s->src_object->get_bucket()) {
      ldpp_dout(dpp, 1) << "ERROR: source object or bucket is missing when "
                           "logging source bucket"
                        << dendl;
      return -EINVAL;
    }
  ...
  case LoggingType::Standard:
    record = fmt::format(
        "{} {} [{:%d/%b/%Y:%H:%M:%S %z}] {} {} {} {} {} \"{} {}{}{} HTTP/1.1\" "
        "{} {} {} {} {} {} {} \"{}\" {} {} {} {} {} {} {} {} {}",
        dash_if_empty(bucket_owner), dash_if_empty(bucket_name), t,
        s->info.env->get("REMOTE_ADDR", "-"), dash_if_empty(user_or_account),
        dash_if_empty(s->req_id), op_name,
        dash_if_empty_or_null(obj, obj->get_name()), s->info.method,
        s->info.request_uri, s->info.request_params.empty() ? "" : "?",
        s->info.request_params, dash_if_zero(s->err.http_ret),
        dash_if_empty(s->err.err_code), dash_if_zero(s->content_length),
        dash_if_zero(size),
        ...
        s->info.x_meta_map.contains("x-amz-id-2")
            ? s->info.x_meta_map.at("x-amz-id-2")
            : "-",
        aws_version, s->info.env->get("SSL_CIPHER", "-"),
        dash_if_empty(auth_type), dash_if_empty(fqdn),
  ...
  case LoggingType::Journal:
    record = fmt::format(
        "{} {} [{:%d/%b/%Y:%H:%M:%S %z}] {} {} {} {} {}",
        dash_if_empty(bucket_owner), dash_if_empty(bucket_name), t, op_name,
        dash_if_empty_or_null(obj, obj->get_name()), dash_if_zero(size),
        dash_if_empty_or_null(obj, obj->get_instance()), dash_if_empty(etag));
    break;
  case LoggingType::Any:
    ldpp_dout(dpp, 1)
        << "ERROR: failed to format record when writing to logging object '"
  ...
  if (ret = get_owner_quota_info(
          dpp, y, driver, target_bucket->get_owner(), user_quota);
      ret < 0) {
    ldpp_dout(dpp, 1)
        << "ERROR: failed to get quota of owner of logging bucket '"
        << target_bucket_id << "' failed. ret = " << ret << dendl;
    set_journal_err(fmt::format(
        "Failed to get quota of owner of logging bucket '{}'",
        target_bucket->get_name()));
    return ret;
  }
  ...
  if (ret = target_bucket->check_quota(dpp, quota, record.length(), y);
      ret < 0) {
    ldpp_dout(dpp, 1) << "ERROR: quota check on logging bucket '"
                      << target_bucket_id << "' failed. ret = " << ret << dendl;
    set_journal_err(fmt::format(
        "Quota check on logging bucket '{}' failed", target_bucket->get_name()));
    return ret;
  }

  if (ret = target_bucket->write_logging_object(
          obj_name, record, conf.target_prefix, y, dpp, async_completion);
      ret < 0 && ret != -EFBIG) {
    ldpp_dout(dpp, 1) << "ERROR: failed to write record to logging object '"
                      << obj_name << "'. ret = " << ret << dendl;
    set_journal_err(fmt::format(
        "Failed to write record to logging object of logging bucket {}",
        target_bucket->get_name()));
    return ret;
  }
  if (ret == -EFBIG) {
    ldpp_dout(dpp, 5) << "WARNING: logging object '" << obj_name
                      << "' is full, will be committed to logging bucket '"
                      << target_bucket->get_key() << "'" << dendl;
    if (ret = rollover_logging_object(
            conf, target_bucket, obj_name, dpp, region, s->bucket, y, true,
            &objv_tracker, true, nullptr, &err_message);
        ret < 0 && ret != -ECANCELED) {
      set_journal_err(err_message);
      return ret;
  ...
  ldpp_dout(dpp, 20) << "INFO: wrote logging record: '" << record << "' to '"
                     << obj_name << "'" << dendl;
  return 0;
}

std::string
object_name_oid(const rgw::sal::Bucket* bucket, const std::string& prefix)
{
  // TODO: do i need bucket marker in the name?
  return fmt::format(
      "logging.{}.bucket.{}/{}", bucket->get_tenant(), bucket->get_bucket_id(),
      prefix);
}
    ...
    bool log_source_bucket)
{
  ...
  try {
    ...
    ldpp_dout(dpp, 20)
        << "INFO: found matching logging configuration of bucket '"
        << s->bucket->get_key()
        << "' configuration: " << configuration.to_json_str() << dendl;
    if (const int ret = log_record(
            driver, obj, s, op_name, etag, size, configuration, dpp, y,
            async_completion, log_source_bucket);
        ret < 0) {
      ldpp_dout(dpp, 1) << "ERROR: failed to perform logging for bucket '"
                        << s->bucket->get_key() << "'. ret=" << ret << dendl;
      return ret;
    }
  } catch (buffer::error& err) {
    ldpp_dout(dpp, 1) << "ERROR: failed to decode logging attribute '"
    ...
    rgw_bucket& bucket_id)
{
  ...
  if (const int ret = rgw_parse_url_bucket(
          bucket_name, tenant_name, parsed_tenant_name, parsed_bucket_name);
      ret < 0) {
    return ret;
  }
...
int
update_bucket_logging_sources(
    const DoutPrefixProvider* dpp,
    rgw::sal::Driver* driver,
    const rgw_bucket& target_bucket_id,
    ...
    optional_yield y)
{
  ...
  if (ret < 0) {
    ldpp_dout(dpp, 5) << "WARNING: failed to get logging bucket '"
                      << target_bucket_id
                      << "' in order to update logging sources. ret = " << ret
                      << dendl;
    return ret;
  }
  return update_bucket_logging_sources(
    ...
    optional_yield y)
{
  return retry_raced_bucket_write(
      dpp, bucket.get(),
      [dpp, &bucket, &src_bucket_id, add, y] {
        auto& attrs = bucket->get_attrs();
        auto iter = attrs.find(RGW_ATTR_BUCKET_LOGGING_SOURCES);
        if (iter == attrs.end()) {
          if (!add) {
            ldpp_dout(dpp, 20)
                << "INFO: no logging sources attribute '"
                << RGW_ATTR_BUCKET_LOGGING_SOURCES << "' for bucket '"
                << bucket->get_key() << "', nothing to remove" << dendl;
            return 0;
          }
          ...
          attrs.insert(
              std::make_pair(RGW_ATTR_BUCKET_LOGGING_SOURCES, std::move(bl)));
        ...
        try {
          ...
          ldpp_dout(dpp, 5)
              << "WARNING: failed to decode logging sources attribute '"
              << RGW_ATTR_BUCKET_LOGGING_SOURCES << "' for bucket '"
              << bucket->get_key()
              << "' when updating logging sources. error: " << err.what()
              << dendl;
        }
        // no change
        return 0;
    ...
    optional_yield y)
{
  ...
  auto& attrs = bucket->get_attrs();
  if (const auto iter = attrs.find(RGW_ATTR_BUCKET_LOGGING_SOURCES);
      iter != attrs.end()) {
    source_buckets sources;
    try {
      ceph::decode(sources, iter->second);
      ...
      ldpp_dout(dpp, 5)
          << "WARNING: failed to decode logging sources attribute '"
          << RGW_ATTR_BUCKET_LOGGING_SOURCES << "' for logging bucket '"
          << bucket->get_key() << "'. error: " << err.what() << dendl;
      return -EIO;
    }
    ...
    for (const auto& source : sources) {
      ...
      if (const int ret = driver->load_bucket(dpp, source, &src_bucket, y);
          ret < 0) {
        ldpp_dout(dpp, 5) << "WARNING: failed to get logging source bucket '"
                          << source << "' for logging bucket '"
                          << bucket->get_key()
                          << "' during cleanup. ret = " << ret << dendl;
        continue;
      }
      auto& src_attrs = src_bucket->get_attrs();
      if (const auto iter = src_attrs.find(RGW_ATTR_BUCKET_LOGGING);
          iter != src_attrs.end()) {
        configuration conf;
        try {
          auto bl_iter = iter->second.cbegin();
          ...
          ldpp_dout(dpp, 5)
              << "WARNING: failed to decode logging attribute '"
              << RGW_ATTR_BUCKET_LOGGING << "' of bucket '"
              << src_bucket->get_key()
              << "' during cleanup. error: " << err.what() << dendl;
          continue;
        }
        std::string obj_name;
        RGWObjVersionTracker objv;
        if (const int ret = bucket->get_logging_object_name(
                obj_name, conf.target_prefix, y, dpp, &objv);
            ret < 0) {
          ldpp_dout(dpp, 5) << "WARNING: failed to get logging object name for "
                               "logging bucket '"
                            << bucket->get_key()
                            << "' during cleanup. ret = " << ret << dendl;
          continue;
        }
        if (const int ret = bucket->remove_logging_object_name(
                conf.target_prefix, y, dpp, &objv);
            ret < 0) {
          ldpp_dout(dpp, 5) << "WARNING: failed to delete object holding "
                               "bucket logging object name for logging bucket '"
                            << bucket->get_key()
                            << "' during cleanup. ret = " << ret << dendl;
          continue;
        }
        ldpp_dout(dpp, 20)
            << "INFO: successfully deleted object holding bucket logging "
               "object name from deleted logging bucket '"
            << bucket->get_key() << "'" << dendl;
        if (const int ret = bucket->remove_logging_object(
                obj_name, conf.target_prefix, y, dpp);
            ret < 0) {
          ldpp_dout(dpp, 5)
              << "WARNING: failed to delete pending logging object '"
              << obj_name << "' for logging bucket '" << bucket->get_key()
              << "' during cleanup. ret = " << ret << dendl;
          continue;
        }
        ldpp_dout(dpp, 20)
            << "INFO: successfully deleted pending logging object '" << obj_name
            << "' from deleted logging bucket '" << bucket->get_key() << "'"
            << dendl;
      } else {
        ldpp_dout(dpp, 5) << "WARNING: no logging attribute '"
                          << RGW_ATTR_BUCKET_LOGGING
                          << "' found in logging source bucket '"
                          << src_bucket->get_key() << "' during cleanup"
                          << dendl;
      }
    }
  }
    ...
    std::string* last_committed)
{
  ...
  if (const int ret = retry_raced_bucket_write(
          dpp, bucket,
          [dpp, bucket, &conf, remove_attr, y] {
            auto& attrs = bucket->get_attrs();
            if (auto iter = attrs.find(RGW_ATTR_BUCKET_LOGGING);
                iter != attrs.end()) {
              try {
                auto bl_iter = iter->second.cbegin();
                ...
                ldpp_dout(dpp, 5)
                    << "WARNING: failed to decode existing logging attribute '"
                    << RGW_ATTR_BUCKET_LOGGING << "' of bucket '"
                    << bucket->get_key()
                    << "' during source cleanup. error: " << err.what()
                    << dendl;
                return -EIO;
              }
              if (remove_attr) {
      ...
      ret < 0) {
    if (remove_attr) {
      ldpp_dout(dpp, 5) << "WARNING: failed to remove logging attribute '"
                        << RGW_ATTR_BUCKET_LOGGING << "' from bucket '"
                        << bucket->get_key()
  ...
  if (const int ret = get_bucket_id(
          conf->target_bucket, info.bucket.tenant, target_bucket_id);
      ret < 0) {
  ...
  if (const int ret = update_bucket_logging_sources(
          dpp, target_bucket, bucket->get_key(), false, y);
      ret < 0) {
    ldpp_dout(dpp, 5) << "WARNING: could not update bucket logging source '"
                      << bucket->get_key() << "' in logging bucket '"
                      << target_bucket_id
                      << "' attribute, during source cleanup. ret = " << ret
                      << dendl;
  }
  std::string obj_name;
  RGWObjVersionTracker objv;
  if (const int ret = target_bucket->get_logging_object_name(
      ...
      ret < 0) {
    ldpp_dout(dpp, 5)
        << "WARNING: failed to get logging object name for logging bucket '"
        << bucket->get_key() << "' during source cleanup. ret = " << ret
        << dendl;
  ...
  if (const int ret = target_bucket->remove_logging_object_name(
          conf->target_prefix, y, dpp, &objv);
      ret < 0) {
    ldpp_dout(dpp, 5) << "WARNING: failed to delete object holding bucket "
                         "logging object name for bucket '"
                      << bucket->get_key()
                      << "' during source cleanup. ret = " << ret << dendl;
  ...
  ldpp_dout(dpp, 20) << "INFO: successfully deleted object holding bucket "
                        "logging object name for bucket '"
  ...
  if (const int ret = target_bucket->commit_logging_object(
          obj_name, y, dpp, conf->target_prefix, nullptr, false);
      ret < 0) {
    ldpp_dout(dpp, 5)
        << "WARNING: could not commit pending logging object of bucket '"
        << bucket->get_key() << "' during source cleanup. ret = " << ret
        << dendl;
    return 0;
  }
  ldpp_dout(dpp, 20)
      << "INFO: successfully committed pending logging object of bucket '"
    ...
    std::string* err_message)
{
  ...
  if (policy_it == target_attrs.end()) {
    ldpp_dout(dpp, 1) << "ERROR: logging bucket '" << target_bucket_id
                      << "' must have bucket policy to allow logging" << dendl;
    if (err_message) {
      *err_message = fmt::format(
          "Logging bucket '{}' must have policy to allow logging",
          target_bucket->get_name());
    }
    return -EACCES;
  }
  try {
    const rgw::IAM::Policy policy{
        s->cct, &target_bucket_id.tenant, policy_it->second.to_str(), false};
    ...
    if (policy.eval(s->env, ident, rgw::IAM::s3PutObject, target_resource_arn) !=
        rgw::IAM::Effect::Allow) {
      ldpp_dout(dpp, 1)
          << "ERROR: logging bucket: '" << target_bucket_id
          << "' must have a bucket policy that allows logging service "
             "principal to put objects in the following resource ARN: '"
          << target_resource_arn.to_string() << "' from source bucket ARN: '"
          << source_bucket_arn << "' and source account: '" << source_account
          << "'" << dendl;
      if (err_message) {
        *err_message = fmt::format(
            "Logging bucket '{}' policy does not allow logging",
            target_bucket->get_name());
      }
      return -EACCES;
    }
    ...
    ldpp_dout(dpp, 1) << "ERROR: failed to parse logging bucket '"
                      << target_bucket_id << "' policy. error: " << err.what()
                      << dendl;
    if (err_message) {
      *err_message = fmt::format(
          "Logging bucket '{}' has invalid policy", target_bucket->get_name());
    }
    return -EACCES;
  }
  return 0;
...
int
verify_target_bucket_attributes(
    const DoutPrefixProvider* dpp,
    rgw::sal::Bucket* target_bucket,
    std::string* err_message)
{
  ...
  if (target_info.requester_pays) {
    ...
    ldpp_dout(dpp, 1) << "ERROR: logging bucket '" << target_bucket->get_key()
                      << "', is configured with requester pays" << dendl;
    if (err_message) {
      *err_message = fmt::format(
          "Logging bucket '{}' is configured with requester pays",
          target_bucket->get_name());
    }
    return -EINVAL;
  }
  ...
  if (target_attrs.find(RGW_ATTR_BUCKET_LOGGING) != target_attrs.end()) {
    ...
    ldpp_dout(dpp, 1) << "ERROR: logging bucket '" << target_bucket->get_key()
                      << "', is configured with bucket logging" << dendl;
    if (err_message) {
      *err_message = fmt::format(
          "Logging bucket '{}' is configured with bucket logging",
          target_bucket->get_name());
    }
    return -EINVAL;
  }
  if (target_attrs.find(RGW_ATTR_BUCKET_ENCRYPTION_POLICY) !=
      target_attrs.end()) {
    // verify target bucket does not have encryption
    ldpp_dout(dpp, 1) << "ERROR: logging bucket '" << target_bucket->get_key()
                      << "', is configured with encryption" << dendl;
    if (err_message) {
      *err_message = fmt::format(
          "Logging bucket '{}' is configured with encryption",
          target_bucket->get_name());
    }
    return -EINVAL;
  }
...
int
get_target_and_conf_from_source(
    const DoutPrefixProvider* dpp,
    rgw::sal::Driver* driver,
    ...
    optional_yield y)
{
  ...
  auto iter = bucket_attrs.find(RGW_ATTR_BUCKET_LOGGING);
  if (iter == bucket_attrs.end()) {
    ldpp_dout(dpp, 1) << "WARNING: no logging configured on bucket '"
                      << src_bucket_id << "'" << dendl;
    return -ENODATA;
  }
  try {
    ...
    ldpp_dout(dpp, 1) << "WARNING: failed to decode logging attribute '"
                      << RGW_ATTR_BUCKET_LOGGING << "' for bucket '"
                      << src_bucket_id << "', error: " << err.what() << dendl;
    return -EINVAL;
  }

  rgw_bucket target_bucket_id;
  if (const int ret =
          get_bucket_id(configuration.target_bucket, tenant, target_bucket_id);
      ret < 0) {
    ldpp_dout(dpp, 1) << "ERROR: failed to parse logging bucket '"
                      << configuration.target_bucket << "', ret = " << ret
                      << dendl;
    return ret;
  }

  if (const int ret =
          driver->load_bucket(dpp, target_bucket_id, &target_bucket, y);
      ret < 0) {
    ldpp_dout(dpp, 1) << "ERROR: failed to get logging bucket '"
                      << target_bucket_id << "', ret = " << ret << dendl;
    return ret;
