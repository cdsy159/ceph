...
struct configuration {
  bool
  operator==(const configuration& rhs) const
  {
    return enabled == rhs.enabled && target_bucket == rhs.target_bucket &&
           ...
           date_source == rhs.date_source && key_filter == rhs.key_filter;
  }
  ...
  uint32_t
      obj_roll_time; // time in seconds to move object to bucket and start another object
  ...
  uint32_t records_batch_size =
      0; // how many records to batch in memory before writing to the object
...
using bucket_logging_records =
    std::array<std::string, MAX_BUCKET_LOGGING_BUFFER>;

template <typename Records>
inline std::string
to_string(const Records& records)
{
...
int update_bucket_logging_sources(
    const DoutPrefixProvider* dpp,
...
// - encryption
// optional error message is returned by reference
int verify_target_bucket_attributes(
    const DoutPrefixProvider* dpp,
    rgw::sal::Bucket* target_bucket,
    std::string* err_message = nullptr);
