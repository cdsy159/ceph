...
ostream&
operator<<(ostream& os, const rgw_sync_bucket_entity& e)
{
  os << "{b=" << rgw_sync_bucket_entities::bucket_key(e.bucket)
     << ",z=" << e.zone.value_or(rgw_zone_id()) << ",az=" << (int)e.all_zones
     << "}";
  return os;
}

ostream&
operator<<(ostream& os, const rgw_sync_bucket_pipe& pipe)
{
...
ostream&
operator<<(ostream& os, const rgw_sync_bucket_entities& e)
{
  os << "{b=" << rgw_sync_bucket_entities::bucket_key(e.bucket)
     << ",z=" << e.zones.value_or(std::set<rgw_zone_id>()) << "}";
  return os;
}

ostream&
operator<<(ostream& os, const rgw_sync_bucket_pipes& pipe)
{
  os << "{id=" << pipe.id << ",s=" << pipe.source << ",d=" << pipe.dest << "}";
  return os;
}

static std::vector<rgw_sync_bucket_pipe>
filter_relevant_pipes(
...
static bool
is_wildcard_bucket(const rgw_bucket& bucket)
{
  return bucket.name.empty();
}

void
rgw_sync_group_pipe_map::dump(ceph::Formatter* f) const
{
...
void
rgw_sync_group_pipe_map::try_add_to_pipe_map(
    const rgw_zone_id& source_zone,
    const rgw_zone_id& dest_zone,
    const std::vector<rgw_sync_bucket_pipes>& pipes,
...
void
rgw_sync_group_pipe_map::try_add_source(
    const rgw_zone_id& source_zone,
    const rgw_zone_id& dest_zone,
    const std::vector<rgw_sync_bucket_pipes>& pipes,
    CB filter_cb)
{
  return try_add_to_pipe_map(
      source_zone, dest_zone, pipes, &sources, filter_cb,
      [&](const rgw_sync_bucket_pipe& pipe, rgw_sync_bucket_entity* zb) {
        *zb = rgw_sync_bucket_entity{source_zone, pipe.source.get_bucket()};
        return filter_cb(
            source_zone, zb->bucket, dest_zone, pipe.dest.get_bucket());
      });
}

template <typename CB>
void
rgw_sync_group_pipe_map::try_add_dest(
    const rgw_zone_id& source_zone,
    const rgw_zone_id& dest_zone,
    const std::vector<rgw_sync_bucket_pipes>& pipes,
    CB filter_cb)
{
  return try_add_to_pipe_map(
      source_zone, dest_zone, pipes, &dests, filter_cb,
      [&](const rgw_sync_bucket_pipe& pipe, rgw_sync_bucket_entity* zb) {
        *zb = rgw_sync_bucket_entity{dest_zone, pipe.dest.get_bucket()};
        return filter_cb(
            source_zone, pipe.source.get_bucket(), dest_zone, zb->bucket);
      });
}

using zb_pipe_map_t = rgw_sync_group_pipe_map::zb_pipe_map_t;

pair<zb_pipe_map_t::const_iterator, zb_pipe_map_t::const_iterator>
rgw_sync_group_pipe_map::find_pipes(
    const zb_pipe_map_t& m,
    ...
    CB filter_cb)
{
  ...
  for (auto& pipe : group.pipes) {
    if (pipe.contains_zone_bucket(zone, bucket)) {
      ldpp_dout(dpp, 20) << __func__ << "(): pipe_map (zone=" << zone
                         << " bucket=" << bucket_key
 ...
 */
vector<rgw_sync_bucket_pipe>
rgw_sync_group_pipe_map::find_source_pipes(
    const rgw_zone_id& source_zone,
 ...
 */
vector<rgw_sync_bucket_pipe>
rgw_sync_group_pipe_map::find_dest_pipes(
    std::optional<rgw_bucket> source_bucket,
    const rgw_zone_id& dest_zone,
    std::optional<rgw_bucket> dest_bucket) const
{
...
void
RGWBucketSyncFlowManager::pipe_rules::insert(const rgw_sync_bucket_pipe& pipe)
{
...
bool
RGWBucketSyncFlowManager::pipe_rules::find_basic_info_without_tags(
    ...
    bool* need_more_info) const
{
  ...
  *need_more_info = false;
  ...
  if (iter != prefix_refs.begin()) {
    iter = prefix_refs.find(
        iter->first); /* prefix_refs is multimap, find first element
                                             holding that key */
  }
  ...
  for (; iter != end; ++iter) {
    ...
    if (rule_params.priority > priority) {
      ...
      *need_more_info =
          filter.has_tags(); /* if highest priority filter has tags, then
...
bool
RGWBucketSyncFlowManager::pipe_rules::find_obj_params(
    const rgw_obj_key& key,
...
RGWBucketSyncFlowManager::pipe_rules::prefix_map_t::const_iterator
RGWBucketSyncFlowManager::pipe_rules::prefix_search(const std::string& s) const
{
  if (prefix_refs.empty()) {
...
void
RGWBucketSyncFlowManager::pipe_set::insert(const rgw_sync_bucket_pipe& pipe)
{
...
void
RGWBucketSyncFlowManager::pipe_set::remove_all()
{
  pipe_map.clear();
...
void
RGWBucketSyncFlowManager::pipe_set::disable(const rgw_sync_bucket_pipe& pipe)
{
...
void
RGWBucketSyncFlowManager::pipe_set::dump(ceph::Formatter* f) const
{
  encode_json("pipes", pipe_map, f);
}

bool
RGWBucketSyncFlowManager::allowed_data_flow(
    const rgw_zone_id& source_zone,
    std::optional<rgw_bucket> source_bucket,
    const rgw_zone_id& dest_zone,
    ...
    bool check_activated) const
{
  ...
  for (auto m : flow_groups) {
    auto& fm = m.second;
    auto pipes =
        fm.find_pipes(source_zone, source_bucket, dest_zone, dest_bucket);
...
void
RGWBucketSyncFlowManager::init(
    const DoutPrefixProvider* dpp,
    const rgw_sync_policy_info& sync_policy)
{
  ...
  for (auto& item : sync_policy.groups) {
    ...
    flow_group_map.init(
        dpp, cct, zone_id, bucket, group,
        (default_flow ? &(*default_flow) : nullptr), &all_zones,
        [&](const rgw_zone_id& source_zone,
            std::optional<rgw_bucket> source_bucket,
            const rgw_zone_id& dest_zone,
            std::optional<rgw_bucket> dest_bucket) {
          ...
          return parent->allowed_data_flow(
              source_zone, source_bucket, dest_zone, dest_bucket,
...
*/
void
RGWBucketSyncFlowManager::reflect(
    const DoutPrefixProvider* dpp,
...

{
  ...
  if (parent) {
    parent->reflect(
        dpp, effective_bucket, source_pipes, dest_pipes, only_enabled);
  }
  ...
  for (auto& item : flow_groups) {
    ...
    if (flow_group_map.status == rgw_sync_policy_group::Status::FORBIDDEN) {
      ...
      is_forbidden = true;
        ...
        flow_group_map.status != rgw_sync_policy_group::Status::ENABLED &&
        (only_enabled ||
         flow_group_map.status != rgw_sync_policy_group::Status::ALLOWED)) {
    ...
    for (auto& entry : flow_group_map.sources) {
      ...
      if (is_forbidden) {
        ldpp_dout(dpp, 20) << __func__ << "(): flow manager (bucket="
...
RGWBucketSyncFlowManager::RGWBucketSyncFlowManager(
    CephContext* _cct,
    const rgw_zone_id& _zone_id,
    std::optional<rgw_bucket> _bucket,
    const RGWBucketSyncFlowManager* _parent) :
  cct(_cct), zone_id(_zone_id), bucket(_bucket), parent(_parent)
{}
...
RGWBucketSyncPolicyHandler::RGWBucketSyncPolicyHandler(
    RGWSI_Zone* _zone_svc,
    RGWSI_SyncModules* sync_modules_svc,
    RGWSI_Bucket_Sync* _bucket_sync_svc,
    std::optional<rgw_zone_id> effective_zone) :
  zone_svc(_zone_svc), bucket_sync_svc(_bucket_sync_svc)
{
  zone_id = effective_zone.value_or(zone_svc->zone_id());
  flow_mgr.reset(
      new RGWBucketSyncFlowManager(zone_svc->ctx(), zone_id, nullopt, nullptr));
  sync_policy = zone_svc->get_zonegroup().sync_policy;

  if (sync_policy.empty()) {
    RGWSyncPolicyCompat::convert_old_sync_config(
        zone_svc, sync_modules_svc, &sync_policy);
    legacy_config = true;
  }
}

RGWBucketSyncPolicyHandler::RGWBucketSyncPolicyHandler(
    const RGWBucketSyncPolicyHandler* _parent,
  ...
  bucket_attrs(std::move(_bucket_attrs))
{
  ...
  flow_mgr.reset(new RGWBucketSyncFlowManager(
      zone_svc->ctx(), parent->zone_id, _bucket_info.bucket,
      parent->flow_mgr.get()));
}
  ...
  parent(_parent)
{
  ...
  bucket_sync_svc = parent->bucket_sync_svc;
  flow_mgr.reset(new RGWBucketSyncFlowManager(
      zone_svc->ctx(), parent->zone_id, _bucket, parent->flow_mgr.get()));
}

RGWBucketSyncPolicyHandler*
RGWBucketSyncPolicyHandler::alloc_child(
    const RGWBucketInfo& bucket_info,
    map<string, bufferlist>&& bucket_attrs) const
{
  return new RGWBucketSyncPolicyHandler(
      this, bucket_info, std::move(bucket_attrs));
}
...
int
RGWBucketSyncPolicyHandler::init(const DoutPrefixProvider* dpp, optional_yield y)
{
  int r = bucket_sync_svc->get_bucket_sync_hints(
      dpp, bucket.value_or(rgw_bucket()), &source_hints, &target_hints, y);
  if (r < 0) {
    ldpp_dout(dpp, 0) << "ERROR: failed to initialize bucket sync policy "
                         "handler: get_bucket_sync_hints() on bucket="
  ...
  reflect(
      dpp, &source_pipes, &target_pipes, &sources, &targets, &source_zones,
      &target_zones, true);
...
void
RGWBucketSyncPolicyHandler::reflect(
    const DoutPrefixProvider* dpp,
...
multimap<rgw_zone_id, rgw_sync_bucket_pipe>
...
multimap<rgw_zone_id, rgw_sync_bucket_pipe>
RGWBucketSyncPolicyHandler::get_all_dests_in_zone(
...
void
RGWBucketSyncPolicyHandler::get_pipes(
    std::set<rgw_sync_bucket_pipe>* _sources,
...
bool
RGWBucketSyncPolicyHandler::bucket_exports_object(
    const std::string& obj_name,
    const RGWObjTags& tags) const
{
...
bool
RGWBucketSyncPolicyHandler::bucket_exports_data() const
{
  if (!bucket) {
    return false;
  }
...
bool
RGWBucketSyncPolicyHandler::bucket_imports_data() const
{
