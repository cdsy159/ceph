...
struct rgw_sync_group_pipe_map {
  ...
  using zb_pipe_map_t =
      std::multimap<rgw_sync_bucket_entity, rgw_sync_bucket_pipe>;
  ...
  rgw_sync_data_flow_group* default_flow{
  ...
  std::pair<zb_pipe_map_t::const_iterator, zb_pipe_map_t::const_iterator>
  find_pipes(
   ...
   */
  std::vector<rgw_sync_bucket_pipe> find_source_pipes(
      const rgw_zone_id& source_zone,
   ...
   */
  std::vector<rgw_sync_bucket_pipe> find_dest_pipes(
      std::optional<rgw_bucket> source_bucket,
      const rgw_zone_id& dest_zone,
      std::optional<rgw_bucket> dest_bucket) const;
   ...
   */
  std::vector<rgw_sync_bucket_pipe> find_pipes(
      const rgw_zone_id& source_zone,
      std::optional<rgw_bucket> source_bucket,
      const rgw_zone_id& dest_zone,
      std::optional<rgw_bucket> dest_bucket) const;
...
public:
  static void convert_old_sync_config(
      RGWSI_Zone* zone_svc,
...
public:
  struct endpoints_pair {
    ...
    bool
    operator<(const endpoints_pair& e) const
    {
      if (source < e.source) {
        return true;
      }
  ...
  public:
    ...
    bool find_basic_info_without_tags(
  ...
  struct pipe_handler : public endpoints_pair {
    ...
    pipe_handler(pipe_rules_ref& _rules, const rgw_sync_bucket_pipe& _pipe) :
      endpoints_pair(_pipe), rules(_rules)
    {}
        ...
        bool* need_more_info) const
    {
      ...
      return rules->find_basic_info_without_tags(
          key, user, acl_translation, storage_class, mode, need_more_info);
    }
...
operator<<(std::ostream& os, const RGWBucketSyncFlowManager::endpoints_pair& e)
{
  os << e.dest << " -> " << e.source;
  return os;
}
...
class RGWBucketSyncPolicyHandler {
  ...
  std::map<rgw_zone_id, RGWBucketSyncFlowManager::pipe_set>
      sources; /* source pipes by source zone id */
  std::map<rgw_zone_id, RGWBucketSyncFlowManager::pipe_set>
      targets; /* target pipes by target zone id */
  ...
  bool
  bucket_is_sync_source() const
  {
    return !targets.empty() || !resolved_dests.empty();
  }

  bool
  bucket_is_sync_target() const
  {
...
public:
  ...
  void
  set_resolved_hints(
      std::set<rgw_sync_bucket_pipe>&& _resolved_sources,
  ...
  const std::set<rgw_sync_bucket_pipe>&
  ...
  const std::map<rgw_zone_id, RGWBucketSyncFlowManager::pipe_set>&
  get_sources()
  {
    return sources;
  }
  ...
  std::multimap<rgw_zone_id, rgw_sync_bucket_pipe> get_all_dests_in_zone(
  ...
  const std::optional<RGWBucketInfo>&
  ...
  get_pipes(
      ...
      RGWBucketSyncFlowManager::pipe_set** _targets)
  ...
  void get_pipes(
      ...
      std::set<rgw_sync_bucket_pipe>* targets,
      std::optional<rgw_sync_bucket_entity> filter_peer);

  const std::set<rgw_bucket>&
...
inline std::ostream&
operator<<(std::ostream& out, const rgw_bucket_sync_pair_info& p)
{
...
struct rgw_bucket_sync_pipe {
  ...
  RGWBucketSyncFlowManager::pipe_rules_ref&
  get_rules()
  {
...
inline std::ostream&
operator<<(std::ostream& out, const rgw_bucket_sync_pipe& p)
{
  return out << p.info;
}
