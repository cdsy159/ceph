...
combine_crc_cksum(const Cksum& ck1, const Cksum& ck2, uintmax_t len1)
{
  ...
  case cksum::Type::crc64nvme: {
    ...
    auto cck1 = rgw::digest::byteswap(std::get<uint64_t>(*ck1.get_crc()));
    auto cck2 = rgw::digest::byteswap(std::get<uint64_t>(*ck2.get_crc()));
    /* madler crcany */
  ...
  case cksum::Type::crc32c: {
    uint32_t cck3;
    auto cck1 = rgw::digest::byteswap(std::get<uint32_t>(*ck1.get_crc()));
    auto cck2 = rgw::digest::byteswap(std::get<uint32_t>(*ck2.get_crc()));
    /* madler crcany */
    switch (ck1.type) {
    case cksum::Type::crc32:
...
public:
  DigestCombiner(cksum::Type t) :
    Combiner(t), dv(digest_factory(t)), digest(get_digest(dv))
  {}

  virtual void
  append(const Cksum& cksum, uint64_t ignored)
  {
    auto ckr = cksum.raw();
...
public:
  ...
  virtual void
  append(const Cksum& rhs, uint64_t part_size)
  {
    if (!once) {
