...
public:
  ...
  static constexpr uint16_t COMPOSITE_MASK = (FLAG_COMBINED | FLAG_COMPOSITE);
    ...
    type(_type), flags(FLAG_V2)
  {
    ...
    case CtorStyle::from_armored:
      (void)ceph_unarmor(
          (char*)digest.begin(), (char*)digest.begin() + ckd.digest_size, _data,
  ...
  to_armor() const
  {
    ...
    ceph_armor(
        (char*)hs.data(), (char*)hs.data() + ckd.armored_size,
        (char*)digest.begin(), (char*)digest.begin() + ckd.digest_size);
    return hs;
  }
  ...
  hex() const
  {
    ...
    ba::hex_lower(
        digest.begin(), digest.begin() + ckd.digest_size,
...
static inline Type
parse_cksum_type(const char* name)
{
...
static inline Type
parse_cksum_type_hdr(const std::string_view hdr_name)
{
...
static inline bool
is_checksum_hdr(const std::string_view hdr_name)
{
  return hdr_name == "x-amz-checksum-algorithm" ||
         parse_cksum_type_hdr(hdr_name) != Type::none;
} /* is_cksum_hdr */

using PermittedCksumResult = std::tuple<bool, const std::string, const char*>;
...
permitted_cksum_algo_and_type(Type type, uint16_t cksum_flags)
{
  ...
  /* FULL_OBJECT */
  const auto& ckd = Cksum::checksums[uint16_t(type)];
  return PermittedCksumResult(
      (ckd.flags & cksum::FLAG_CRC), to_string(type), "FULL_OBJECT");
}

std::optional<Cksum> combine_crc_cksum(
    const Cksum& ck1,
    const Cksum& ck2,
    uintmax_t len2);
