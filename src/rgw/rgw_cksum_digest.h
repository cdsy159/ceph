...
public:
  ...
  void
  Update(const unsigned char* data, uint64_t len) override
  {
    d.Update(data, len);
  }

  void
  Update(const ceph::buffer::list& bl)
  {
  ...
  void
  Final(unsigned char* digest) override
  {
    d.Final(digest);
  }
};
...
typedef std::
    variant<std::monostate, Blake3, Crc32, Crc32c, XXH3, SHA1, SHA256, SHA512, Crc64Nvme>
        DigestVariant;
...
struct get_digest_ptr {
  ...
  Digest*
  operator()(const std::monostate& b) const
  {
    return nullptr;
  }

  Digest*
  operator()(Blake3& digest) const
  {
    return &digest;
  }

  Digest*
  operator()(Crc32& digest) const
  {
    return &digest;
  }

  Digest*
  operator()(Crc32c& digest) const
  {
    return &digest;
  }

  Digest*
  operator()(XXH3& digest) const
  {
    return &digest;
  }

  Digest*
  operator()(SHA1& digest) const
  {
    return &digest;
  }
  ...
  Digest*
  operator()(SHA512& digest) const
  {
    return &digest;
  }

  Digest*
  operator()(Crc64Nvme& digest) const
  {
    return &digest;
  }
};

static inline Digest*
get_digest(DigestVariant& ev)
{
  return std::visit(get_digest_ptr{}, ev);
}

static inline DigestVariant
digest_factory(const Type cksum_type)
{
  switch (cksum_type) {
  case Type::blake3:
    return Blake3();
...
static inline Cksum
finalize_digest(Digest* digest, Type type)
{
  Cksum cksum(type);
  if (digest) {
