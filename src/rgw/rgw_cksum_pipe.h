...
static inline const cksum_hdr_t
cksum_algorithm_hdr(rgw::cksum::Type t)
{
  static constexpr std::string_view hdr = "HTTP_X_AMZ_SDK_CHECKSUM_ALGORITHM";
...
static inline const cksum_hdr_t
cksum_algorithm_hdr(const RGWEnv& env)
{
  /* If the individual checksum value you provide through
    ...
    */
  for (const auto hk :
       {"HTTP_X_AMZ_CHECKSUM_ALGORITHM", "HTTP_X_AMZ_SDK_CHECKSUM_ALGORITHM"}) {
    auto hv = env.get(hk);
    if (hv) {
  ...
  if (hv) {
	 ...
	 * we need "CRC32", for known checksum types */
      ...
      if (type != cksum::Type::none) {
        return cksum_hdr_t(
            "HTTP_X_AMZ_CHECKSUM_ALGORITHM", cksum::to_uc_string(type).data());
      }
    }
  }
...
static inline GetHeaderCksumResult
get_hdr_cksum(const RGWEnv& env)
{
  ...
  if (algo_hdr.first) {
    if (algo_hdr.second) {
      ...
      if (hv) {
        return GetHeaderCksumResult(
            cksum::Cksum(cksum_type, hv, cksum::Cksum::CtorStyle::from_armored),
...
static inline GetHeaderCksumResult
find_hdr_cksum(const RGWEnv& env)
{
  cksum::Type cksum_type;
...
public:
  ...
  verify(const RGWEnv& env)
  {
    ...
    return VerifyResult(
        cksum_hdr.first && hv && !std::strcmp(hv, cv.c_str()), _cksum);
  }
