...
#define RGW_AMZ_WEBSITE_REDIRECT_LOCATION \
  RGW_AMZ_PREFIX "website-redirect-location"
...
#define RGW_ATTR_AMZ_WEBSITE_REDIRECT_LOCATION \
  RGW_ATTR_PREFIX RGW_AMZ_WEBSITE_REDIRECT_LOCATION
...
#define RGW_ATTR_CLOUDTIER_STORAGE_CLASS \
  RGW_ATTR_PREFIX "cloudtier_storage_class"
...
#define RGW_ATTR_RESTORE_VERSIONED_EPOCH \
  RGW_ATTR_PREFIX "restore-versioned-epoch"
...
#define RGW_ATTR_IAM_POLICY_REMOVE_SELF_ACCESS \
  RGW_ATTR_PREFIX "iam-policy-remove-self-access"
...
#define RGW_ATTR_BUCKET_ENCRYPTION_POLICY \
  RGW_ATTR_BUCKET_ENCRYPTION_PREFIX "policy"
#define RGW_ATTR_BUCKET_ENCRYPTION_KEY_ID \
  RGW_ATTR_BUCKET_ENCRYPTION_PREFIX "key-id"
...
to_mime_type(const RGWFormat f)
{
  switch (f) {
  case RGWFormat::XML:
...
#define RGW_OP_TYPE_ALL \
  (RGW_OP_TYPE_READ | RGW_OP_TYPE_WRITE | RGW_OP_TYPE_DELETE)
...
#define ERR_NO_SUCH_OWNERSHIP_CONTROLS \
  2229 // 404 OwnershipControlsNotFoundError
...
public:
  ...
  boost::optional<const std::string&> get_optional(
  ...
  bool
  exists(const char* name) const
  {
    return (val_map.find(name) != std::end(val_map));
  }

  bool
  sub_resource_exists(const char* name) const
  {
    return (sub_resources.find(name) != std::end(sub_resources));
  }

  bool
  exist_obj_excl_sub_resource() const
  {
    const char* const obj_sub_resource[] = {
        "append", "torrent", "uploadId", "partNumber", "versionId"};
    for (unsigned i = 0; i != std::size(obj_sub_resource); i++) {
      if (sub_resource_exists(obj_sub_resource[i]))
        return true;
    }
    return false;
  }
...
const char* rgw_conf_get(
    const std::map<std::string, std::string, ltstr_nocase>& conf_map,
    const char* name,
    const char* def_val);
boost::optional<const std::string&> rgw_conf_get_optional(
    const std::map<std::string, std::string, ltstr_nocase>& conf_map,
    const std::string& name);
int rgw_conf_get_int(
    const std::map<std::string, std::string, ltstr_nocase>& conf_map,
    const char* name,
    int def_val);
bool rgw_conf_get_bool(
    const std::map<std::string, std::string, ltstr_nocase>& conf_map,
    const char* name,
    bool def_val);
...
public:
  ...
  const std::map<std::string, std::string, ltstr_nocase>&
  get_map() const
  {
    return env_map;
  }
  ...
  int
  get_enable_ops_log() const
  {
    return conf.enable_ops_log;
  }

  int
  get_enable_usage_log() const
  {
    return conf.enable_usage_log;
  }

  int
  get_defer_to_bucket_acls() const
  {
...
void encode_json(
    const char* name,
    const rgw_placement_rule& val,
    ceph::Formatter* f);
void decode_json_obj(rgw_placement_rule& v, JSONObj* obj);

inline std::ostream&
operator<<(std::ostream& out, const rgw_placement_rule& rule)
{
...
struct RGWUserInfo {
  ...
  RGWAccessKey*
  get_key(const std::string& access_key)
  {
...
inline std::ostream&
operator<<(std::ostream& out, const RGWObjVersionTracker& ot)
{
...
struct RGWBucketInfo {
  ...
  RGWObjVersionTracker
      objv_tracker; /* we don't need to serialize this, for runtime tracking */
  RGWQuotaInfo quota;
  ...
  int
  versioning_status() const
  {
    return flags &
           (BUCKET_VERSIONED | BUCKET_VERSIONS_SUSPENDED | BUCKET_MFA_ENABLED);
  }

  bool
  versioning_enabled() const
  {
    return (versioning_status() &
            (BUCKET_VERSIONED | BUCKET_VERSIONS_SUSPENDED)) == BUCKET_VERSIONED;
  }

  bool
  mfa_enabled() const
  {
    return (versioning_status() & BUCKET_MFA_ENABLED) != 0;
  }

  bool
  datasync_flag_enabled() const
  {
    return (flags & BUCKET_DATASYNC_DISABLED) == 0;
  }

  bool
  obj_lock_enabled() const
  {
    return (flags & BUCKET_OBJ_LOCK_ENABLED) != 0;
  }

  bool
  bucket_deleted() const
  {
    return (flags & BUCKET_DELETED) != 0;
  }
  ...
  rgw::bucket_index_layout_generation&
  get_current_index()
  {
    return layout.current_index;
  }
...
struct RGWBucketEntryPoint {
  ...
  RGWBucketEntryPoint() :
    linked(false), has_bucket_info(false)
  {}
...
struct req_state : DoutPrefixProvider {
  ...
  void
  set_user(std::unique_ptr<rgw::sal::User>& u)
  {
    user.swap(u);
  }
...
struct RGWBucketEnt {
  ...
  void
  convert(cls_user_bucket_entry* b) const
  {
  ...
  encode(bufferlist& bl) const
  {
    ...
    std::string
        empty_str; // originally had the bucket name here, but we encode bucket later
    encode(empty_str, bl);
    encode(s, bl);
    encode(mt, bl);
    encode(count, bl);
...
inline std::ostream&
operator<<(std::ostream& out, const rgw_obj& o)
{
...
struct multipart_upload_info {
  ...
  static std::list<multipart_upload_info>
...
static inline int
rgw_str_to_bool(const char* s, int def_val)
{
  if (!s)
    return def_val;

  return (
      strcasecmp(s, "true") == 0 || strcasecmp(s, "on") == 0 ||
...
rgw_rounded_kb(uint64_t bytes)
{
  return (bytes + 1023) / 1024;
}

static inline uint64_t
rgw_rounded_objsize(uint64_t bytes)
{
  return ((bytes + 4095) & ~4095);
}

static inline uint64_t
rgw_rounded_objsize_kb(uint64_t bytes)
{
...
extern void parse_csv_string(
    const std::string& ival,
    std::vector<std::string>& ovals);
extern int parse_key_value(
...
extern bool parse_iso8601(
    const char* s,
    struct tm* t,
    uint32_t* pns = NULL,
    bool extended_format = true);
...
struct perm_state_base {
  ...
  perm_state_base(
      ...
      boost::optional<PublicAccessBlockConfiguration> _bucket_access_conf =
          boost::none) :
    cct(_cct),
    env(_env),
    identity(_identity),
...
bool verify_user_permission_no_policy(
    const DoutPrefixProvider* dpp,
    struct perm_state_base* const s,
    const RGWAccessControlPolicy& user_acl,
    const int perm);

bool verify_object_permission_no_policy(
    const DoutPrefixProvider* dpp,
    struct perm_state_base* const s,
    const RGWAccessControlPolicy& user_acl,
    const RGWAccessControlPolicy& bucket_acl,
...
extern std::string url_decode(
    const std::string_view& src_str,
    bool in_query = false);
...
extern std::string url_remove_prefix(
    const std::string& url); // Removes http, https and www from url
/* destination should be CEPH_CRYPTO_HMACSHA1_DIGESTSIZE bytes long */
extern void calc_hmac_sha1(
    const char* key,
    int key_len,
    const char* msg,
    int msg_len,
...
static inline sha256_digest_t
calc_hmac_sha256(const sha256_digest_t& key, const std::string_view& msg)
{
  sha256_digest_t dest;
  calc_hmac_sha256(
      reinterpret_cast<const char*>(key.v), sha256_digest_t::SIZE, msg.data(),
      msg.size(), reinterpret_cast<char*>(dest.v));
  return dest;
}
    ...
    const std::string_view& msg)
{
  sha256_digest_t dest;
  calc_hmac_sha256(
      reinterpret_cast<const char*>(key.data()), key.size(), msg.data(),
      msg.size(), reinterpret_cast<char*>(dest.v));
  return dest;
}
...
static inline sha256_digest_t
calc_hmac_sha256(
    const std::array<unsigned char, KeyLenN>& key,
    const std::string_view& msg)
{
  sha256_digest_t dest;
  calc_hmac_sha256(
...
extern void calc_hash_sha256_update_stream(
    ceph::crypto::SHA256* hash,
    const char* msg,
    int len);
...
extern bool match_policy(
    const std::string& pattern,
    const std::string& input,
    uint32_t flag);

extern std::string camelcase_dash_http_attr(
    const std::string& orig,
    bool convert2dash = true);
extern std::string lowercase_dash_http_attr(
    const std::string& orig,
...
rgw_bl_str(const ceph::buffer::list& bl)
{
...
static inline std::string
ys_header_mangle(std::string_view name)
{
  /* can we please stop doing this? */
  std::string out;
  out.reserve(name.length());
  std::transform(
      std::begin(name), std::end(name), std::back_inserter(out),
      [](const int c) {
...
extern int rgw_bucket_parse_bucket_instance(
    const std::string& bucket_instance,
    std::string* bucket_name,
    std::string* bucket_id,
    int* shard_id);

boost::intrusive_ptr<CephContext> rgw_global_init(
...
struct AioCompletionDeleter {
  void
  operator()(librados::AioCompletion* c)
  {
    c->release();
  }
};

using aio_completion_ptr =
    std::unique_ptr<librados::AioCompletion, AioCompletionDeleter>;

extern boost::optional<rgw::IAM::Policy> get_iam_policy_from_attr(
    CephContext* cct,
