...
int
rgw_compression_info_from_attr(
...
int
RGWPutObj_Compress::process(bufferlist&& in, uint64_t logical_offset)
{
  bufferlist out;
  ...
  if (in.length() > 0) {
    ...
    if ((logical_offset > 0 && compressed) || // if previous part was compressed
      ...
      ldout(cct, 10) << "Compression for rgw is enabled, compress part "
                     << in.length() << dendl;
      ...
      if (cr < 0) {
        ...
        newbl.new_ofs = bs > 0 ? blocks[bs - 1].len + blocks[bs - 1].new_ofs
                               : 0;
    ...
    size_t bs = blocks.size();
    compressed_ofs = bs > 0 ? blocks[bs - 1].len + blocks[bs - 1].new_ofs
                            : logical_offset;
  }
...
RGWGetObj_Decompress::RGWGetObj_Decompress(
    CephContext* cct_,
...
RGWGetObj_Decompress::handle_data(bufferlist& bl, off_t bl_ofs, off_t bl_len)
{
  ...
  while (first_block <= last_block) {
    ...
    while (out_bl.length() - q_ofs >=
      ...
      if (r < 0) {
        lsubdout(cct, rgw, 0)
            << "handle_data failed with exit code " << r << dendl;
        return r;
      }
...
int
RGWGetObj_Decompress::fixup_range(off_t& ofs, off_t& end)
{
  if (partial_content) {
    ...
    if (cs_info->blocks.size() > 1) {
      ...
      auto cmp_u = [](off_t ofs, const compression_block& e) {
        return (uint64_t)ofs < e.old_ofs;
      };
      auto cmp_l = [](const compression_block& e, off_t ofs) {
        return e.old_ofs <= (uint64_t)ofs;
      };
      fb = upper_bound(
          cs_info->blocks.begin() + 1, cs_info->blocks.end(), ofs, cmp_u);
      first_block = fb - 1;
...
void
compression_block::dump(Formatter* f) const
{
...
void
RGWCompressionInfo::dump(Formatter* f) const
{
...
list<RGWCompressionInfo>
RGWCompressionInfo::generate_test_instances()
{
