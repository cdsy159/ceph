...
public:
  RGWAioCompletionNotifierWith(
      RGWCompletionManager* mgr,
      const rgw_io_id& io_id,
      void* user_data,
      T value) :
...
struct RGWCoroutinesEnv {
  ...
  RGWCoroutinesEnv() :
    run_context(0), manager(NULL), scheduled_stacks(NULL), stack(NULL)
  {}
};
...
struct rgw_spawned_stacks {
  ...
  void
  add_pending(RGWCoroutinesStack* s)
  {
    entries.push_back(s);
  }

  void
  inherit(rgw_spawned_stacks* source)
  {
...
#define MAX_COROUTINE_HISTORY 10
  ...
  struct Status {
    ...
    explicit Status(CephContext* _cct) :
      cct(_cct), max_history(MAX_COROUTINE_HISTORY)
    {}
...
protected:
  ...
  virtual int
  operate_wrapper(const DoutPrefixProvider* dpp)
  {
    return operate(dpp);
  }
...
public:
  ...
  RGWCoroutinesStack* spawn(
      RGWCoroutine* op,
      bool wait); /* execute on a different stack */
  bool collect(
      int* ret,
  ...
  bool collect_next(
      ...
      RGWCoroutinesStack** collected_stack =
          NULL); /* returns true if found a stack to collect */
  ...
  bool drain_children(
      int num_cr_left,
      std::optional<std::function<int(uint64_t stack_id, int ret)>>
...
protected:
  ...
  RGWCoroutinesStack* spawn(
      RGWCoroutine* source_op,
      RGWCoroutine* next_op,
      bool wait);
  bool collect(
      RGWCoroutine* op,
      int* ret,
...
public:
  ...
  void
  set_io_blocked(bool flag)
  {
    blocked_flag = flag;
  }

  void
  set_io_blocked_id(const rgw_io_id& io_id)
  {
    io_blocked_id = io_id;
  }
  ...
  bool
  can_io_unblock(const rgw_io_id& io_id)
  {
    return ((io_blocked_id.id < 0) || io_blocked_id.intersects(io_id));
  }
  ...
  is_blocked()
  {
    return is_blocked_by_stack() || is_sleeping() || is_io_blocked() ||
           waiting_for_child();
  }
  ...
  void
  set_blocked_by(RGWCoroutinesStack* s)
  {
...
template <class T>
void
RGWConsumerCR<T>::receive(std::list<T>& l, bool wakeup)
{
...
template <class T>
void
RGWConsumerCR<T>::receive(const T& p, bool wakeup)
{
  product.push_back(p);
  if (wakeup) {
    set_sleeping(false);
...
class RGWCoroutinesManagerRegistry : public RefCountedObject,
...
class RGWCoroutinesManager {
  ...
  void handle_unblocked_stack(
      std::set<RGWCoroutinesStack*>& context_stacks,
      std::list<RGWCoroutinesStack*>& scheduled_stacks,
...
public:
  RGWCoroutinesManager(
    ...
    cct(_cct), cr_registry(_cr_registry), ops_window(RGW_ASYNC_OPS_MGR_WINDOW)
  {
...
template <typename T>
RGWAioCompletionNotifier*
RGWCoroutinesManager::create_completion_notifier(
    RGWCoroutinesStack* stack,
    T value)
{
  rgw_io_id io_id{get_next_io_id(), -1};
  RGWAioCompletionNotifier* cn = new RGWAioCompletionNotifierWith<T>(
      completion_mgr, io_id, (void*)stack, std::move(value));
...
public:
  RGWSimpleCoroutine(CephContext* _cct) :
    RGWCoroutine(_cct), called_cleanup(false), max_eio_retries(1)
  {}

  RGWSimpleCoroutine(CephContext* _cct, const int _max_eio_retries) :
    RGWCoroutine(_cct), called_cleanup(false), max_eio_retries(_max_eio_retries)
  {}
