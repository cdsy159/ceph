...
#define RGW_CORS_ALL                                             \
  (RGW_CORS_GET | RGW_CORS_PUT | RGW_CORS_HEAD | RGW_CORS_POST | \
   RGW_CORS_DELETE | RGW_CORS_COPY)
...
protected:
  ...
  std::string id;
  std::set<std::string>
      allowed_hdrs; /* If you change this, you need to discard lowercase_allowed_hdrs */
  std::set<std::string>
      lowercase_allowed_hdrs; /* Not built until needed in RGWCORSRule::is_header_allowed */
  std::set<std::string> allowed_origins;
  std::list<std::string> exposable_hdrs;

public:
  RGWCORSRule() :
    max_age(CORS_MAX_AGE_INVALID), allowed_methods(0)
  {}
...
static inline int
validate_name_string(std::string_view o)
{
  if (o.length() == 0)
    return -1;
...
get_cors_method_flags(const char* req_meth)
{
  uint8_t flags = 0;
