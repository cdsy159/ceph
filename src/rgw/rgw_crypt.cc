...
public:
    ...
    dpp(dpp), cct(cct)
  {
    ...
    if (U_FAILURE(status)) {
      ldpp_dout(this->dpp, -1)
          << "ERROR: can't get nfc instance, error = " << status << dendl;
      normalizer = 0;
    }
  }

  bool compare_helper(const M*, const M*);
  bool make_string_canonical(
      rapidjson::Value&,
...
canonical_char_sorter<M>::compare_helper(const M* a, const M* b)
{
  ...
  icu::UnicodeString aw{icu::UnicodeString::fromUTF8(as)},
      bw{icu::UnicodeString::fromUTF8(bs)};
...
template <typename M>
bool
canonical_char_sorter<M>::make_string_canonical(
    rapidjson::Value& v,
    rapidjson::Document::AllocatorType& a)
{
  ...
  if (U_FAILURE(status)) {
    ldpp_dout(this->dpp, 5)
        << "conversion error; code=" << status << " on string " << as << dendl;
    return false;
  }
  std::string ans;
  an.toUTF8String(ans);
...
typedef rapidjson::
    GenericMember<rapidjson::UTF8<>, rapidjson::MemoryPoolAllocator<>>
        MyMember;
...
sort_and_write(
    rapidjson::Value& d,
    H& writer,
    canonical_char_sorter<MyMember>& ccs)
{
  bool r;
  switch (d.GetType()) {
  case rapidjson::kObjectType: {
    struct comparer {
      ...
      bool
      operator()(const MyMember* a, const MyMember* b)
      {
...
mec_error
make_everything_canonical(
    rapidjson::Value& d,
    rapidjson::Document::AllocatorType& a,
    canonical_char_sorter<MyMember>& ccs,
    mec_option f = mec_option::empty)
{
  mec_error r;
  switch (d.GetType()) {
  case rapidjson::kObjectType:
    for (auto& m : d.GetObject()) {
      ...
      if ((r = make_everything_canonical(m.value, a, ccs, f)) !=
          mec_error::success)
        goto Error;
    }
    break;
    ...
    std::string& cooked_context)
{
  ...
  b = make_everything_canonical(d, allocator, ccs, options) ==
      mec_error::success;
  if (!b) {
    ldpp_dout(s, -1) << "ERROR: can't make canonical json <" << context << ">"
                     << dendl;
    ...
    const size_t max_requests)
{
  ...
  CryptoPlugin* factory = dynamic_cast<CryptoPlugin*>(
      reg->get_with_load("crypto", crypto_accel_type));
  if (factory == nullptr) {
    ldpp_dout(dpp, -1) << __func__ << " cannot load crypto accelerator of type "
                       << crypto_accel_type << dendl;
    return nullptr;
  }
    ...
    const bool encrypt)
{
  ...
  if (1 !=
      EVP_CipherInit_ex(pctx.get(), type, nullptr, nullptr, nullptr, encrypt)) {
...
public:
  ...
  bool
  set_key(const uint8_t* _key, size_t key_size)
  {
      ...
      optional_yield y)
  {
    ...
    if (result == false) {
      ...
      for (size_t offset = 0; result && (offset < size); offset += CHUNK_SIZE) {
        size_t process_size = offset + CHUNK_SIZE <= size ? CHUNK_SIZE
                                                          : size - offset;
        prepare_iv(iv, stream_offset + offset);
        if (crypto_accel != nullptr && accelerator != "crypto_qat") {
          if (encrypt) {
            result = crypto_accel->cbc_encrypt(
                out + offset, in + offset, process_size, iv, key, y);
          } else {
            result = crypto_accel->cbc_decrypt(
                out + offset, in + offset, process_size, iv, key, y);
          }
        } else {
          result = cbc_transform(
              out + offset, in + offset, process_size, iv, key, encrypt);
        }
      }
    }
    return result;
      ...
      optional_yield y)
  {
    ...
    result = cbc_transform(
        buf_raw, input_raw + in_ofs, aligned_size, stream_offset, key, true, y);
    if (result && (unaligned_rest_size > 0)) {
      ...
      if (aligned_size % CHUNK_SIZE > 0) {
        ...
        result = cbc_transform(
            buf_raw + aligned_size, buf_raw + aligned_size - AES_256_IVSIZE,
            AES_256_IVSIZE, iv, key, true);
      } else {
        ...
        result = cbc_transform(
            buf_raw + aligned_size, data, AES_256_IVSIZE, iv, key, true);
      }
      if (result) {
      ...
      optional_yield y)
  {
    ...
    result = cbc_transform(
        buf_raw, input_raw + in_ofs, aligned_size, stream_offset, key, false, y);
    if (result && unaligned_rest_size > 0) {
      ...
      if (aligned_size % CHUNK_SIZE > 0) {
        ...
        result = cbc_transform(
            ...
            input_raw + in_ofs + aligned_size - AES_256_IVSIZE, AES_256_IVSIZE,
            iv, key, true);
      } else {
        /* 0 full blocks in current chunk, use IV as base for unaligned part */
        unsigned char iv[AES_256_IVSIZE] = {0};
  ...
  void
  prepare_iv(unsigned char (&iv)[AES_256_IVSIZE], off_t offset)
  {
...
int
RGWGetObj_BlockDecrypt::read_manifest_parts(
    const DoutPrefixProvider* dpp,
    ...
    std::vector<size_t>& parts_len)
{
  ...
  if (manifest_bl.length()) {
    ...
    for (size_t i = 0; i < parts_len.size(); i++) {
      ldpp_dout(dpp, 20) << "Manifest part " << i << ", size=" << parts_len[i]
                         << dendl;
    }
  }
  return 0;
...
int
RGWGetObj_BlockDecrypt::fixup_range(off_t& bl_ofs, off_t& bl_end)
{
  /*
...
int
RGWGetObj_BlockDecrypt::process(bufferlist& in, size_t part_ofs, size_t size)
{
  bufferlist data;
...
int
RGWGetObj_BlockDecrypt::handle_data(bufferlist& bl, off_t bl_ofs, off_t bl_len)
{
 ...
 */
int
RGWGetObj_BlockDecrypt::flush()
{
  ldpp_dout(this->dpp, 25) << "Decrypt flushing " << cache.length() << " bytes"
                           << dendl;
  int res = 0;
  size_t part_ofs = ofs;
  for (size_t part : parts_len) {
...
int
RGWPutObj_BlockEncrypt::process(bufferlist&& data, uint64_t logical_offset)
{
...
std::string
create_random_key_selector(CephContext* const cct)
{
...
struct CryptAttributes {
  ...
  std::string_view
  get(crypt_option_e option)
  {
    static_assert(
        X_AMZ_SERVER_SIDE_ENCRYPTION_LAST ==
            sizeof(crypt_options) / sizeof(*crypt_options),
...
std::string
fetch_bucket_key_id(req_state* s)
{
...
std::string
expand_key_name(req_state* s, const std::string_view& t)
{
  std::string r;
  size_t i, j;
  for (i = 0;;) {
...
static int
get_sse_s3_bucket_key(req_state* s, optional_yield y, std::string& key_id)
{
  int res;
...

  {
    ...
    if (!req_sse_ca.empty()) {
      ...
      if (s->cct->_conf->rgw_crypt_require_ssl &&
        ...
        ldpp_dout(s, 5)
            << "ERROR: Insecure request, rgw_crypt_require_ssl is set" << dendl;
        return -ERR_INVALID_REQUEST;
      }
      ...
      try {
        ...
        ldpp_dout(s, 5)
            << "ERROR: rgw_s3_prepare_encrypt invalid encryption "
      ...
      try {
        ...
        ldpp_dout(s, 5)
            << "ERROR: rgw_s3_prepare_encrypt invalid encryption key "
      ...
      key_hash.Update(
          reinterpret_cast<const unsigned char*>(key_bin.c_str()),
          key_bin.size());
      key_hash.Final(key_hash_res);

      if (memcmp(key_hash_res, keymd5_bin.c_str(), CEPH_CRYPTO_MD5_DIGESTSIZE) !=
          0) {
      ...
      if (block_crypt) {
        ...
        aes->set_key(
            reinterpret_cast<const uint8_t*>(key_bin.c_str()), AES_256_KEYSIZE);
      ...
      if (!customer_key.empty()) {
        ldpp_dout(s, 5)
            << "ERROR: SSE-C encryption request is missing the header "
    ...
    if (!req_sse.empty()) {
      ...
      if (req_sse == "aws:kms") {
        if (s->cct->_conf->rgw_crypt_require_ssl &&
            !rgw_transport_is_secure(s->cct, *s->info.env)) {
          ldpp_dout(s, 5)
              << "ERROR: insecure request, rgw_crypt_require_ssl is set"
              << dendl;
          return -ERR_INVALID_REQUEST;
        }
        ...
        if (s->cct->_conf->rgw_crypt_s3_kms_backend ==
            RGW_SSE_KMS_BACKEND_TESTING) {
        ...
        if (res != 0) {
          ldpp_dout(s, 5)
              << "ERROR: failed to retrieve actual key from key_id: " << key_id
              << dendl;
          s->err.message = "Failed to retrieve the actual key, kms-keyid: " +
                           std::string(key_id);
          return res;
        }
        if (actual_key.size() != AES_256_KEYSIZE) {
          ldpp_dout(s, 5) << "ERROR: key obtained from key_id:" << key_id
        ...
        ::ceph::crypto::zeroize_for_security(
            actual_key.data(), actual_key.length());

        crypt_http_responses["x-amz-server-side-encryption"] = "aws:kms";
        crypt_http_responses["x-amz-server-side-encryption-aws-kms-key-id"] =
            std::string(key_id);
        crypt_http_responses["x-amz-server-side-encryption-context"] =
            std::move(cooked_context);
        return 0;
      ...
      if (s->cct->_conf->rgw_crypt_sse_s3_backend != "vault") {
        s->err.message =
            "Request specifies Server Side Encryption "
            "but server configuration does not support this.";
        return -EINVAL;
      }

      ldpp_dout(s, 5) << "RGW_ATTR_BUCKET_ENCRYPTION ALGO: " << req_sse
                      << dendl;
      std::string_view context = "";
      std::string cooked_context;
      if ((res = make_canonical_context(s, context, cooked_context)))
      ...
      if (res != 0) {
        ldpp_dout(s, 5) << "ERROR: failed to retrieve actual key from key_id: "
                        << key_id << dendl;
      ...
      try {
        ...
        ldpp_dout(s, 5)
            << "ERROR: rgw_s3_prepare_encrypt invalid default encryption key "
      ...
      if (master_encryption_key.size() != 256 / 8) {
        ldpp_dout(s, 0) << "ERROR: failed to decode 'rgw crypt default "
                           "encryption key' to 256 bit string"
                        << dendl;
      ...
      if (block_crypt) {
        auto aes = std::unique_ptr<AES_256_CBC>(new AES_256_CBC(s, s->cct));
        aes->set_key(
            reinterpret_cast<const uint8_t*>(actual_key), AES_256_KEYSIZE);
        *block_crypt = std::move(aes);
      }
      ::ceph::crypto::zeroize_for_security(actual_key, sizeof(actual_key));
      return 0;
    ...
    bool copy_source)
{
  ...
  const char* req_sse =
      s->info.env->get("HTTP_X_AMZ_SERVER_SIDE_ENCRYPTION", NULL);
  ...
  if (stored_mode == "SSE-C-AES256") {
    if (s->cct->_conf->rgw_crypt_require_ssl &&
        !rgw_transport_is_secure(s->cct, *s->info.env)) {
      ldpp_dout(s, 5) << "ERROR: Insecure request, rgw_crypt_require_ssl is set"
                      << dendl;
      return -ERR_INVALID_REQUEST;
    }

    const char* sse_c_algo_hdr =
        copy_source
            ? "HTTP_X_AMZ_COPY_SOURCE_SERVER_SIDE_ENCRYPTION_CUSTOMER_ALGORITHM"
    ...
    if (nullptr == req_cust_alg) {
      ...
      ldpp_dout(s, 5) << "ERROR: The requested encryption algorithm is not "
                         "valid, must be AES256."
                      << dendl;
      s->err.message =
          "The requested encryption algorithm is not valid, must be AES256.";
      return -ERR_INVALID_ENCRYPTION_ALGORITHM;
    ...
    const char* sse_c_key_md5_hdr =
        copy_source
            ? "HTTP_X_AMZ_COPY_SOURCE_SERVER_SIDE_ENCRYPTION_CUSTOMER_KEY_MD5"
    ...
    try {
      ...
      ldpp_dout(s, 5)
          << "ERROR: rgw_s3_prepare_decrypt invalid encryption key md5 "
          << "which contains character that is not base64 encoded." << dendl;
      s->err.message =
          "Requests specifying Server Side Encryption with Customer "
          "provided keys must provide an appropriate secret key md5.";
    ...
    key_hash.Update(
        reinterpret_cast<const unsigned char*>(key_bin.c_str()), key_bin.size());
    key_hash.Final(key_hash_res);
  ...
  if (stored_mode == "SSE-KMS") {
    ...
    if (res != 0) {
      ldpp_dout(s, 10) << "ERROR: failed to retrieve actual key from key_id: "
                       << key_id << dendl;
      s->err.message = "Failed to retrieve the actual key, kms-keyid: " +
                       key_id;
      return res;
    ...
    if (actual_key.size() != AES_256_KEYSIZE) {
      ldpp_dout(s, 0) << "ERROR: key obtained from key_id:" << key_id
                      << " is not 256 bit size" << dendl;
  ...
  if (stored_mode == "RGW-AUTO") {
    ...
    try {
      master_encryption_key = from_base64(
          std::string(s->cct->_conf->rgw_crypt_default_encryption_key));
    } catch (...) {
      ldpp_dout(s, 5)
          << "ERROR: rgw_s3_prepare_decrypt invalid default encryption key "
          << "which contains character that is not base64 encoded." << dendl;
    ...
    if (attr_key_selector.size() != AES_256_CBC::AES_256_KEYSIZE) {
      ldpp_dout(s, 0) << "ERROR: missing or invalid " RGW_ATTR_CRYPT_KEYSEL
                      << dendl;
      return -EIO;
    }
    uint8_t actual_key[AES_256_KEYSIZE];
  ...
  if (stored_mode == "AES256") {
    ...
    if (actual_key.size() != AES_256_KEYSIZE) {
      ldpp_dout(s, 0) << "ERROR: key obtained " << "is not 256 bit size"
                      << dendl;
      s->err.message = "SSE-S3 provided an invalid key for the given keyid.";
      return -EINVAL;
    }
...
int
rgw_remove_sse_s3_bucket_key(req_state* s, optional_yield y)
{
  int res;
