...
struct D3nL1CacheRequest {
  ~D3nL1CacheRequest()
  {
    lsubdout(g_ceph_context, rgw_datacache, 30)
        << "D3nDataCache: " << __func__ << "(): Read From Cache, complete"
        << dendl;
  }
  ...
  struct AsyncFileReadOp {
        ...
        void* arg)
    {
      ...
      aio_cb->aio_fildes = TEMP_FAILURE_RETRY(
          ::open(location.c_str(), O_RDONLY | O_CLOEXEC | O_BINARY));
      if (aio_cb->aio_fildes < 0) {
        int err = errno;
        ldpp_dout(dpp, 1) << "ERROR: D3nDataCache: " << __func__
                          << "(): can't open " << location << " : "
    ...
    static auto
    create(const Executor1& ex1, CompletionHandler&& handler)
    {
      ...
      CompletionToken&& token)
  {
    ...
    return boost::asio::async_initiate<CompletionToken, Signature>(
        [](auto handler, const DoutPrefixProvider* dpp, const Executor& ex,
           const std::string& location, off_t read_ofs, off_t read_len) {
          ...
          ldpp_dout(dpp, 20) << "D3nDataCache: " << __func__
                             << "(): location=" << location << dendl;
          int ret =
              op.init_async_read(dpp, location, read_ofs, read_len, p.get());
          if (0 == ret) {
  ...
  struct d3n_libaio_handler {
    ...
    // read callback
    void
    operator()(boost::system::error_code ec, bufferlist bl) const
    {
      r.result = -ec.value();
      r.data = std::move(bl);
      throttle->put(r);
  ...
  static std::string
  generate_oid_digest(const std::string& oid)
  {
      ...
      rgw::AioResult& r)
  {
    ...
    ldpp_dout(dpp, 20) << "D3nDataCache: " << __func__
                       << "(): oid=" << r.obj.oid << dendl;
    async_read(
        dpp, ex, cache_location + "/" + generate_oid_digest(r.obj.oid),
        read_ofs, read_len, bind_executor(ex, d3n_libaio_handler{aio, r}));
  }
};
