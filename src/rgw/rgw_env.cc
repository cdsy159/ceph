...
void
RGWEnv::init(CephContext* cct)
{
  conf.init(cct);
}

void
RGWEnv::set(std::string name, std::string val)
{
  env_map[std::move(name)] = std::move(val);
}

void
RGWEnv::init(CephContext* cct, char** envp)
{
  const char* p;
...
const char*
rgw_conf_get(
    const map<string, string, ltstr_nocase>& conf_map,
    const char* name,
    const char* def_val)
{
...
const char*
RGWEnv::get(const char* name, const char* def_val) const
{
...
int
RGWEnv::get_int(const char* name, int def_val) const
{
...
size_t
RGWEnv::get_size(const char* name, size_t def_val) const
{
...
bool
RGWEnv::exists(const char* name) const
{
  return env_map.find(name) != env_map.end();
}

bool
RGWEnv::exists_prefix(const char* prefix) const
{
