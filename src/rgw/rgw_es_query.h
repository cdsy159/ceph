...
public:
  ...
  void
  assign(std::list<std::string>& src)
  {
    l.swap(src);
    iter = l.begin();
  }
...
public:
  explicit ESInfixQueryParser(const std::string& _query) :
    query(_query), size(query.size()), str(query.c_str())
  {}
...
struct ESEntityTypeMap {
  ...
  bool
  find(const std::string& entity, EntityType* ptype)
  {
...
public:
  ...
  void
  set_generic_type_map(ESEntityTypeMap* entity_map)
  {
  ...
  void
  set_custom_type_map(ESEntityTypeMap* entity_map)
  {
  ...
  void
  set_field_aliases(std::map<std::string, std::string, ltstr_nocase>* fa)
  {
    field_aliases = fa;
  }

  std::string
  unalias_field(const std::string& field)
  {
    if (!field_aliases) {
      return field;
    }
  ...
  void
  set_restricted_fields(std::set<std::string>* rf)
  {
    restricted_fields = rf;
  }

  bool
  is_restricted(const std::string& f)
  {
    return (
        restricted_fields &&
        restricted_fields->find(f) != restricted_fields->end());
  }
};
