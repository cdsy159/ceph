...
inline int
valid_fs_bucket_name(const string& name)
{
...
inline int
valid_fs_object_name(const string& name)
{
...
public:
  std::size_t
  operator()(const rgw_xattrstr& att) const noexcept
  {
...
public:
  bool
  operator()(const rgw_xattrstr& lhs, const rgw_xattrstr& rhs) const
  {
...
static const std::unordered_set<rgw_xattrstr, XattrHash, XattrEqual>
    rgw_exposed_attrs = {rgw_xattrstr{
        const_cast<char*>(RGW_ATTR_ETAG), sizeof(RGW_ATTR_ETAG) - 1}};

static inline bool
is_exposed_attr(const rgw_xattrstr& k)
{
  return (rgw_exposed_attrs.find(k) != rgw_exposed_attrs.end());
}

LookupFHResult
RGWLibFS::stat_bucket(
    ...
    uint32_t flags)
{
  ...
  if ((rc == 0) && (req.get_ret() == 0) && (req.matched())) {
    fhr = lookup_fh(
        parent, path,
        (flags & RGWFileHandle::FLAG_LOCKED) | RGWFileHandle::FLAG_CREATE |
    ...
    uint32_t flags)
{
  ...
  for (auto ix : {0, 1, 2}) {
    ...
    case 0: {
      ...
      RGWStatObjRequest req(
          cct, user->clone(), parent->bucket_name(), obj_path,
	     ...
	     * not return NULL */
          if ((flags & RGWFileHandle::FLAG_EXACT_MATCH) && !req.exact_matched) {
            lsubdout(get_context(), rgw, 15)
                << __func__
                << ": stat leaf not exact match file name = " << path << dendl;
            goto done;
          }
          fhr = lookup_fh(
              parent, path,
              RGWFileHandle::FLAG_CREATE |
                  ((req.is_dir) ? RGWFileHandle::FLAG_DIRECTORY
    ...
    uint32_t flags)
{
  ...
  RGWReadRequest req(
      get_context(), user->clone(), rgw_fh, offset, length, buffer);
...
int
RGWLibFS::unlink(RGWFileHandle* rgw_fh, const char* name, uint32_t flags)
{
  int rc = 0;
  BucketStats bs;
  ...
  if (parent->is_root()) {
    ...
    fhr = stat_bucket(
        parent, name, bs,
        (rgw_fh) ? RGWFileHandle::FLAG_LOCKED : RGWFileHandle::FLAG_NONE);
    bkt_fh = get<0>(fhr);
    if (unlikely(!bkt_fh)) {
	 ...
	 * atomicity at this endpoint */
      ...
      rc = rgw_lookup(
          get_fs(), parent->get_fh(), name, &fh, nullptr /* st */, 0 /* mask */,
  ...
  if (!rc || rc == -ENOENT) {
    ...
    fh_cache.remove(
        rgw_fh->fh.fh_hk.object, rgw_fh, RGWFileHandle::FHCache::FLAG_LOCK);
  }
...
int
RGWLibFS::rename(
    RGWFileHandle* src_fh,
    RGWFileHandle* dst_fh,
...

{
  ...
  if (!rgw_fh) {
    ldout(get_context(), 0)
        << __func__ << " BUG no such src renaming path=" << src_name << dendl;
    goto out;
  }
  ...
  if (rgw_fh->is_dir()) {
    ldout(get_context(), 12)
        << __func__ << " rejecting attempt to rename directory path="
        << rgw_fh->full_object_name() << dendl;
    rc = -EPERM;
    goto unlock;
  }
  ...
  if (rgw_fh->is_open()) {
    ldout(get_context(), 12)
        << __func__ << " rejecting attempt to rename open file path="
        << rgw_fh->full_object_name() << dendl;
    rc = -EPERM;
    goto unlock;
  }
  ...
  for (int ix : {0, 1}) {
    switch (ix) {
    case 0: {
      RGWCopyObjRequest req(
          cct, user->clone(), src_fh, dst_fh, src_name, dst_name);
      int rc = g_rgwlib->get_fe()->execute_req(&req);
      if ((rc != 0) || ((rc = req.get_ret()) != 0)) {
        ldout(get_context(), 1)
            << __func__
            << " rename step 0 failed src=" << src_fh->full_object_name() << " "
            << src_name << " dst=" << dst_fh->full_object_name() << " "
            << dst_name << "rc " << rc << dendl;
        goto unlock;
      }
      ldout(get_context(), 12)
          << __func__
          << " rename step 0 success src=" << src_fh->full_object_name() << " "
          << src_name << " dst=" << dst_fh->full_object_name() << " "
          << dst_name << " rc " << rc << dendl;
    ...
    case 1: {
      rc = this->unlink(
          rgw_fh /* LOCKED */, _src_name, RGWFileHandle::FLAG_UNLINK_THIS);
      /* !LOCKED, -ref */
      if (!rc) {
        ldout(get_context(), 12)
            << __func__
            << " rename step 1 success src=" << src_fh->full_object_name()
            << " " << src_name << " dst=" << dst_fh->full_object_name() << " "
            << dst_name << " rc " << rc << dendl;
        ...
        ldout(get_context(), 1)
            << __func__
            << " rename step 1 failed src=" << src_fh->full_object_name() << " "
            << src_name << " dst=" << dst_fh->full_object_name() << " "
            << dst_name << " rc " << rc << dendl;
      }
    }
    ...
    uint32_t flags)
{
  ...
  rc = rgw_lookup(
      get_fs(), parent->get_fh(), name, &lfh, nullptr /* st */, 0 /* mask */,
      RGW_LOOKUP_FLAG_NONE);
  if (!rc) {
    /* conflict! */
  ...
  fhr = lookup_fh(
      parent, name,
      RGWFileHandle::FLAG_CREATE | RGWFileHandle::FLAG_DIRECTORY |
  ...
  if (parent->is_root()) {
    ...
    RGWPutObjRequest req(
        get_context(), user->clone(), parent->bucket_name(), dir_name, bl);
    ...
    uint32_t flags)
{
  ...
  if ((rc == 0) && (rc2 == 0)) {
    /* XXX atomicity */
    LookupFHResult fhr = lookup_fh(
        parent, name, RGWFileHandle::FLAG_CREATE | RGWFileHandle::FLAG_LOCK);
    RGWFileHandle* rgw_fh = get<0>(fhr);
    if (rgw_fh) {
    ...
    uint32_t flags)
{
  ...
  fhr = lookup_fh(
      parent, name,
      RGWFileHandle::FLAG_CREATE | RGWFileHandle::FLAG_SYMBOLIC_LINK |
          RGWFileHandle::FLAG_LOCK);
  rgw_fh = get<0>(fhr);
  if (rgw_fh) {
    rgw_fh->create_stat(st, mask);
  ...
  RGWPutObjRequest req(
      get_context(), user->clone(), parent->bucket_name(), obj_name, bl);
...
int
RGWLibFS::getattr(RGWFileHandle* rgw_fh, struct stat* st)
{
    ...
    uint32_t flags)
{
  ...
  if (rc == -ENOENT) {
    ...
    buffer::list bl;
    RGWPutObjRequest req(
        get_context(), user->clone(), rgw_fh->bucket_name(), obj_name, bl);
...
prefix_xattr_keystr(const rgw_xattrstr& key)
{
  std::string keystr;
...
unprefix_xattr_keystr(const std::string& key)
{
    ...
    uint32_t flags)
{
  ...
  if (ldlog_p1(get_context(), ceph_subsys_rgw, 15)) {
    lsubdout(get_context(), rgw, 15)
        << __func__ << " get keys for: " << rgw_fh->object_name()
        << " keys:" << dendl;
  ...
  if (rc3 == 0) {
	 ...
	 * RGWGetAttrs::attrs, I think */
      std::string_view svk =
          is_exposed_attr(
              rgw_xattrstr{const_cast<char*>(k.c_str()), uint32_t(k.length())})
              ? k
      ...
      rgw_xattrstr xattr_k = {
          const_cast<char*>(svk.data()), uint32_t(svk.length())};
    ...
    uint32_t flags)
{
  ...
  if (rc3 == 0) {
    ...
    for (const auto& k : keys) {
      ...
      /* return exposed attr keys as given, else unprefix */
      std::string_view svk = is_exposed_attr(rgw_xattrstr{
                                 const_cast<char*>(k.first.c_str()),
...
int
RGWLibFS::setxattrs(RGWFileHandle* rgw_fh, rgw_xattrlist* attrs, uint32_t flags)
{
  /* cannot store on fs_root, should not on buckets? */
  if ((rgw_fh->is_bucket()) || (rgw_fh->is_root())) {
    return -EINVAL;
...
int
RGWLibFS::rmxattrs(RGWFileHandle* rgw_fh, rgw_xattrlist* attrs, uint32_t flags)
{
  /* cannot store on fs_root, should not on buckets? */
  if ((rgw_fh->is_bucket()) || (rgw_fh->is_root())) {
    return -EINVAL;
...
void
RGWLibFS::update_fh(RGWFileHandle* rgw_fh)
{
  int rc, rc2;
  string obj_name{rgw_fh->relative_object_name()};
  buffer::list ux_key, ux_attrs;
  ...
  lsubdout(get_context(), rgw, 17)
      << __func__ << " update old versioned fh : " << obj_name << dendl;
  ...
  if ((rc != 0) || (rc2 != 0)) {
    lsubdout(get_context(), rgw, 17)
        << __func__ << " update fh failed : " << obj_name << dendl;
  }
} /* RGWLibFS::update_fh */
...
RGWLibFS::close()
{
  ...
  public:
    ...
    void
    operator()(RGWFileHandle* fh) const
    {
      lsubdout(fs->get_context(), rgw, 5)
          << __PRETTY_FUNCTION__ << fh->name
          << " before ObjUnref refs=" << fh->get_refcnt() << dendl;
      fs->unref(fh);
    }
  };
...
inline std::ostream&
operator<<(std::ostream& os, fh_key const& fhk)
{
  os << "<fh_key: bucket=";
  os << fhk.fh_hk.bucket;
  os << "; object=";
...
inline std::ostream&
operator<<(std::ostream& os, struct timespec const& ts)
{
  os << "<timespec: tv_sec=";
  os << ts.tv_sec;
  os << "; tv_nsec=";
...
std::ostream&
operator<<(std::ostream& os, RGWLibFS::event const& ev)
{
  os << "<event:";
  switch (ev.t) {
...
RGWLibFS::gc()
{
  ...
  do {
    ...
    lsubdout(get_context(), rgw, 15)
        << "GC: top of expire loop" << " now=" << now
        << " expire_s=" << expire_s << dendl;
    {
      lock_guard guard(state.mtx); /* LOCKED */
      lsubdout(get_context(), rgw, 15)
          << "GC: processing" << " count=" << events.size() << " events"
          << dendl;
        ...
        if (rgw_fh) {
          ...
          if (d) {
            ...
            if (unlikely(ev_ts < d_last_readdir)) {
              ...
              lsubdout(get_context(), rgw, 15)
                  << "GC: delay expiration for " << rgw_fh->object_name()
                  << " ev.ts=" << ev_ts << " last_readdir=" << d_last_readdir
                  << dendl;
              continue;
            } else {
              lsubdout(get_context(), rgw, 15)
                  << "GC: expiring " << rgw_fh->object_name() << dendl;
              rgw_fh->clear_state();
              rgw_fh->invalidate();
            }
          }
...
std::ostream&
operator<<(std::ostream& os, RGWFileHandle const& rgw_fh)
{
...
fh_key
RGWFileHandle::make_fhk(const std::string& name)
{
...
RGWFileHandle::decode_attrs(
    const ceph::buffer::list* ux_key1,
...
bool
RGWFileHandle::reclaim(const cohort::lru::ObjectFactory* newobj_fac)
{
  ...
  if (!fs->fh_cache.is_same_partition(
          factory->fhk.fh_hk.object, fh.fh_hk.object)) {
    return false;
  }
...
bool
RGWFileHandle::has_children() const
{
  if (unlikely(!is_dir()))
    return false;
  ...
  RGWRMdirCheck req(
      fs->get_context(),
      g_rgwlib->get_driver()->get_user(fs->get_user()->user_id), this);
  int rc = g_rgwlib->get_fe()->execute_req(&req);
  if (!rc) {
...
std::ostream&
operator<<(std::ostream& os, RGWFileHandle::readdir_offset const& offset)
{
    ...
    uint32_t flags)
{
  ...
  lsubdout(cct, rgw, 10) << __func__ << " readdir called on " << object_name()
                         << dendl;
  ...
  if (is_root()) {
    RGWListBucketsRequest req(
        cct, g_rgwlib->get_driver()->get_user(fs->get_user()->user_id), this,
        rcb, cb_arg, offset);
    rc = g_rgwlib->get_fe()->execute_req(&req);
    if (!rc) {
      (void)clock_gettime(CLOCK_MONOTONIC_COARSE, &now); /* !LOCKED */
  ...
  lsubdout(fs->get_context(), rgw, 15)
      << __func__ << " final link count=" << state.nlink << dendl;
    ...
    void* buffer)
{
  ...
  if (deleted()) {
    lsubdout(fs->get_context(), rgw, 5)
        << __func__ << " write attempted on deleted object "
        << this->object_name() << dendl;
  ...
  if (!f->write_req) {
    ...
    f->write_req = new RGWWriteRequest(
        g_rgwlib->get_driver(), penv,
        g_rgwlib->get_driver()->get_user(fs->get_user()->user_id), this,
        bucket_name(), object_name);
    rc = g_rgwlib->get_fe()->start_req(f->write_req);
    if (rc < 0) {
      lsubdout(fs->get_context(), rgw, 5)
          << __func__ << this->object_name() << " write start failed " << off
          << " (" << rc << ")" << dendl;
  ...
  if (rc == 0) {
    ...
    lsubdout(fs->get_context(), rgw, 5)
        << __func__ << object_name() << " failed write at position " << off
...
int
RGWFileHandle::write_finish(uint32_t flags)
{
  ...
  if (f && (f->write_req)) {
    lsubdout(fs->get_context(), rgw, 10)
        << __func__ << " finishing write trans on " << object_name() << dendl;
...
RGWFileHandle::file::~file() { delete write_req; }

void
RGWFileHandle::clear_state()
{
  directory* d = std::get_if<directory>(&variant_type);
  if (d) {
    state.nlink = 2;
...
void
RGWFileHandle::advance_mtime(uint32_t flags)
{
...
bool
RGWListBucketsRequest::eof()
{
  if (unlikely(cct->_conf->subsys.should_gather(ceph_subsys_rgw, 15))) {
    bool is_offset = unlikely(!std::get_if<const char*>(&offset)) ||
...
RGWWriteRequest::exec_start()
{
  ...
  processor = get_driver()->get_atomic_writer(
      this, state->yield, state->object.get(), state->bucket_owner,
...
RGWWriteRequest::exec_finish()
{
  ...
  op_ret = state->bucket->check_quota(
      this, quota, state->obj_size, null_yield, true);
  /* max_size exceed */
  if (op_ret < 0) {
    goto done;
  }
  ...
  op_ret = processor->complete(
      state->obj_size, etag, &mtime, real_time(), attrs, rgw::cksum::no_cksum,
      (delete_at ? *delete_at : real_time()), if_match, if_nomatch, nullptr,
...
void
rgwfile_version(int* major, int* minor, int* extra)
{
  if (major)
    ...
    uint32_t flags)
{
  ...
  RGWLibFS* new_fs =
      new RGWLibFS(static_cast<CephContext*>(rgw), uid, acc_key, sec_key, "/");
  ceph_assert(new_fs);
    ...
    uint32_t flags)
{
  ...
  const DoutPrefix dp(
      g_rgwlib->get_driver()->ctx(), dout_subsys, "rgw mount2: ");
  rc = new_fs->authorize(&dp, g_rgwlib->get_driver());
  if (rc != 0) {
    delete new_fs;
    return -EINVAL;
...
*/
int
rgw_umount(struct rgw_fs* rgw_fs, uint32_t flags)
{
  RGWLibFS* fs = static_cast<RGWLibFS*>(rgw_fs->fs_private);
  fs->close();
  return 0;
    ...
    uint32_t flags)
{
  ...
  RGWGetClusterStatReq req(
      fs->get_context(),
      g_rgwlib->get_driver()->get_user(fs->get_user()->user_id), stats);
  int rc = g_rgwlib->get_fe()->execute_req(&req);
  if (rc < 0) {
    lderr(fs->get_context())
        << "ERROR: getting total cluster usage" << cpp_strerror(-rc) << dendl;
    return rc;
  }
...
rgw_rename(
    struct rgw_fs* rgw_fs,
    struct rgw_file_handle* src,
    const char* src_name,
    struct rgw_file_handle* dst,
    const char* dst_name,
    uint32_t flags)
{
  RGWLibFS* fs = static_cast<RGWLibFS*>(rgw_fs->fs_private);
 ...
 */
int
rgw_fh_rele(struct rgw_fs* rgw_fs, struct rgw_file_handle* fh, uint32_t flags)
{
  RGWLibFS* fs = static_cast<RGWLibFS*>(rgw_fs->fs_private);
...
*/
int
rgw_close(struct rgw_fs* rgw_fs, struct rgw_file_handle* fh, uint32_t flags)
{
  RGWLibFS* fs = static_cast<RGWLibFS*>(rgw_fs->fs_private);
  RGWFileHandle* rgw_fh = get_rgwfh(fh);
    ...
    uint32_t flags)
{
  ...
  lsubdout(parent->get_fs()->get_context(), rgw, 15)
      << __func__ << " offset=" << ((name) ? name : "(nil)") << dendl;
...
void
rgw_readv_rele(struct rgw_uio* uio, uint32_t flags)
{
    ...
    uint32_t flags)
{
  ...
  for (unsigned int ix = 0; ix < uio->uio_cnt; ++ix) {
    rgw_vio* vio = &(uio->uio_vio[ix]);
    bl.push_back(
        buffer::create_static(vio->vio_len, static_cast<char*>(vio->vio_base)));
  }
...
*/
int
rgw_fsync(struct rgw_fs* rgw_fs, struct rgw_file_handle* handle, uint32_t flags)
{
  return 0;
}
