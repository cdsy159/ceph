...
inline bool
operator<(const fh_key& lhs, const fh_key& rhs)
{
  return (
...
class RGWFileHandle : public cohort::lru::Object {
  ...
  struct directory {
    ...
    directory() :
      flags(FLAG_NONE), last_readdir{0, 0}
    {}
  };
...
#define CREATE_FLAGS(x) \
  ...
  static constexpr uint32_t RCB_MASK = RGW_SETATTR_MTIME | RGW_SETATTR_CTIME |
                                       RGW_SETATTR_ATIME | RGW_SETATTR_SIZE;
...
private:
  ...
  uint64_t
  init_fsid(std::string& uid)
  {
    return XXH64(uid.c_str(), uid.length(), fh_key::seed);
  }

  void
  init_rootfs(std::string& fsid, const std::string& object_name, bool is_bucket)
  {
    /* fh_key */
    fh.fh_hk.bucket = XXH64(fsid.c_str(), fsid.length(), fh_key::seed);
    fh.fh_hk.object =
        XXH64(object_name.c_str(), object_name.length(), fh_key::seed);
    fhk = fh.fh_hk;
    name = object_name;
  ...
  decode(bufferlist::const_iterator& bl)
  {
    ...
    uint32_t fh_type;
    decode(fh_type, bl);
    if ((fh.fh_type != fh_type) && (fh_type == RGW_FS_TYPE_SYMBOLIC_LINK))
...
public:
  ...
  void
  create_stat(struct stat* st, uint32_t mask)
  {
  ...
  int
  stat(struct stat* st, uint32_t flags = FLAG_NONE)
  {
    /* partial Unix attrs */
  ...
  std::string
  full_object_name(bool omit_bucket = false) const
  {
    std::string path;
    ...
    for (auto& s : boost::adaptors::reverse(segments)) {
      if (pos > 1) {
        ...
        if (!omit_bucket && ((path.length() == 0) || (path.front() != '/')))
          path += "/";
      }
      path += *s;
      ++pos;
  ...
  inline std::string
  format_child_name(const std::string& cbasename, bool is_dir) const
  {
    std::string child_name{relative_object_name()};
    if ((child_name.size() > 0) && (child_name.back() != '/'))
      child_name += "/";
  ...
  inline std::string
  make_key_name(const char* name) const
  {
  ...
  void
  add_marker(uint64_t off, const rgw_obj_key& marker, uint8_t obj_type)
  {
  ...
  const rgw_obj_key*
  find_marker(uint64_t off) const
  {
    using std::get;
    if (off > 0) {
  ...
  int
  offset_of(const std::string& name, int64_t* offset, uint32_t flags)
  {
    if (unlikely(!is_dir())) {
      return -EINVAL;
    }
  ...
  is_link() const
  {
    return (fh.fh_type == RGW_FS_TYPE_SYMBOLIC_LINK);
  }
  ...
  bool
  stateless_open() const
  {
    return flags & FLAG_STATELESS_OPEN;
  }

  bool has_children() const;

  int
  open(uint32_t gsh_flags)
  {
  ...
  int
  commit(uint64_t offset, uint64_t length, uint32_t flags)
  {
  ...
  void
  inc_nlink(const uint64_t n)
  {
    state.nlink += n;
  }

  void
  set_nlink(const uint64_t n)
  {
    state.nlink = n;
  }
  ...
  void
  set_times(const struct timespec& ts)
  {
    state.ctime = ts;
  ...
  void
  set_times(real_time t)
  {
    set_times(real_clock::to_timespec(t));
  }

  void
  set_ctime(const struct timespec& ts)
  {
    state.ctime = ts;
  }

  void
  set_mtime(const struct timespec& ts)
  {
    state.mtime = ts;
  }

  void
  set_atime(const struct timespec& ts)
  {
    state.atime = ts;
  }

  void
  set_etag(const ceph::buffer::list& _etag)
  {
    etag = _etag;
  }

  void
  set_acls(const ceph::buffer::list& _acls)
  {
    acls = _acls;
  }
  ...
  DecodeAttrsResult decode_attrs(
  ...
  struct FhLT {
    ...
    bool
    operator()(const RGWFileHandle& lhs, const RGWFileHandle& rhs) const
    {
    ...
    bool
    operator()(const fh_key& k, const RGWFileHandle& fh) const
    {
      return k < fh.get_key();
    }

    bool
    operator()(const RGWFileHandle& fh, const fh_key& k) const
    {
      return fh.get_key() < k;
    }
  };
  ...
  typedef bi::member_hook<RGWFileHandle, tree_hook_type, &RGWFileHandle::fh_hook>
      FhHook;
...
#endif
  typedef cohort::lru::TreeX<RGWFileHandle, FhTree, FhLT, FhEQ, fh_key, std::mutex>
      FHCache;
  ...
  public:
    ...
    Factory(
        ...
        std::string& _name,
        uint32_t _flags) :
      fs(_fs), parent(_parent), fhk(_fhk), name(_name), flags(_flags)
    {}

    void
    recycle(cohort::lru::Object* o) override
    {
...
inline RGWFileHandle*
get_rgwfh(struct rgw_file_handle* fh)
{
  return static_cast<RGWFileHandle*>(fh->fh_private);
}

inline enum rgw_fh_type
...
public:
  ...
  RGWLibFS(
    ...
    refcnt(1),
    fh_cache(
        cct->_conf->rgw_nfs_fhcache_partitions,
  ...
  friend void
  intrusive_ptr_add_ref(const RGWLibFS* fs)
  {
    fs->refcnt.fetch_add(1, std::memory_order_relaxed);
  }

  friend void
  intrusive_ptr_release(const RGWLibFS* fs)
  {
  ...
  void
  release_evict(RGWFileHandle* fh)
  {
  ...
  int
  authorize(const DoutPrefixProvider* dpp, rgw::sal::Driver* driver)
  {
  ...
  int
  register_invalidate(rgw_fh_callback_t cb, void* arg, uint32_t flags)
  {
    invalidate_cb = cb;
    invalidate_arg = arg;
    return 0;
  ...
  LookupFHResult
  lookup_fh(const fh_key& fhk, const uint32_t flags = RGWFileHandle::FLAG_NONE)
  {
    using std::get;
  ...
  retry:
    RGWFileHandle* fh = fh_cache.find_latch(
        fhk.fh_hk.object /* partition selector*/, fhk /* key */,
        lat /* serializer */, RGWFileHandle::FHCache::FLAG_LOCK);
    /* LATCHED */
    if (fh) {
      if (likely(!fh_locked))
      ...
      const uint32_t flags = RGWFileHandle::FLAG_NONE)
  {
    ...
    lsubdout(get_context(), rgw, 10)
        << __func__ << " called on " << parent->object_name() << " for "
        << key_name << " (" << obj_name << ")" << " -> " << fhk << dendl;
  ...
  retry:
    ...
    if (fh) {
      ...
      RGWFileHandle::Factory prototype(
          this, parent, fhk, obj_name, CREATE_FLAGS(flags));
      uint32_t iflags{cohort::lru::FLAG_INITIAL};
      fh = static_cast<RGWFileHandle*>(
          fh_lru.insert(&prototype, cohort::lru::Edge::MRU, iflags));
      if (fh) {
        /* lock fh (LATCHED) */
        if (flags & RGWFileHandle::FLAG_LOCK)
          fh->mtx.lock();
  ...
  LookupFHResult stat_bucket(
      RGWFileHandle* parent,
      const char* path,
      RGWLibFS::BucketStats& bs,
      uint32_t flags);

  LookupFHResult fake_leaf(
      RGWFileHandle* parent,
      const char* path,
  ...
  int rename(
      RGWFileHandle* old_fh,
      RGWFileHandle* new_fh,
      const char* old_name,
      const char* new_name);
  ...
  retry:
    ...
    if (!fh) {
      ...
      lsubdout(get_context(), rgw, 0)
          << __func__ << " handle lookup failed " << fhk << dendl;
      goto out;
    }
    fh->mtx.lock();
    if (fh->flags & RGWFileHandle::FLAG_DELETED) {
  ...
  void
  update_user(const DoutPrefixProvider* dpp)
  {
    (void)g_rgwlib->get_driver()->get_user_by_access_key(
        dpp, key.id, null_yield, &user);
  }
...
make_uri(const std::string& bucket_name, const std::string& object_name)
{
  std::string uri("/");
...
public:
  ...
  RGWListBucketsRequest(
      ...
      RGWFileHandle* _rgw_fh,
      rgw_readdir_cb _rcb,
      void* _cb_arg,
      RGWFileHandle::readdir_offset& _offset) :
  ...
  void
  send_response_begin(bool has_buckets) override
  {
    sent_data = true;
  }

  void
  send_response_data(std::span<const RGWBucketEnt> buckets) override
  {
    if (!sent_data)
      return;
    for (const auto& ent : buckets) {
      if (!this->operator()(ent.bucket.name, ent.bucket.name)) {
        ...
        lsubdout(cct, rgw, 5)
            << "ListBuckets rcb failed" << " dirent=" << ent.bucket.name
            << " call count=" << ix << dendl;
        rcb_eof = true;
        return;
      }
  ...
  void
  send_response_end() override
  {
    // do nothing
  }

  int
  operator()(const std::string_view& name, const std::string_view& marker)
  {
    ...
    rgw_fh->add_marker(
        off, rgw_obj_key{marker.data(), ""}, RGW_FS_TYPE_DIRECTORY);
    ++d_count;
...
public:
      ...
      uint8_t type)
  {
    ...
    return rcb(
        name.data(), cb_arg, off, &st, RGWFileHandle::RCB_MASK,
        (type == RGW_FS_TYPE_DIRECTORY) ? RGW_LOOKUP_FLAG_DIR
  ...
  send_response() override
  {
    ...
    public:
      ...
      entry_is_obj()
      {
        return (is_obj() && ((!is_cp()) || (obj_sref.get() < cp_sref.get())));
      }
      ...
      std::vector<rgw_bucket_dir_entry>::iterator&
      get_obj_iter()
      {
        return obj_iter;
      }
    ...
    for (;;) {
      ...
      if (di.entry_is_obj()) {
        ...
        if (sref.empty()) {
          ...
          if (!this->operator()(
                  sref, next_marker, obj_entry.meta.mtime,
                  obj_entry.meta.accounted_size, RGW_FS_TYPE_FILE)) {
            /* caller cannot accept more */
            lsubdout(cct, rgw, 5)
                << "readdir rcb caller signalled stop"
        ...
        if (!di.skip_cp()) {
          ...
          lsubdout(cct, rgw, 15)
              << "RGWReaddirRequest " << __func__ << " "
              << "list uri=" << state->relative_uri << " "
              << " prefix=" << prefix << " " << " cpref=" << sref << dendl;
	     ...
	     * interpretation */
            ...
            if (!this->operator()(
                    sref, next_marker, cnow, 0, RGW_FS_TYPE_DIRECTORY)) {
              /* caller cannot accept more */
              lsubdout(cct, rgw, 5)
                  << "readdir rcb caller signalled stop"
                  << " dirent=" << sref.data() << " call count=" << ix << dendl;
...
public:
  ...
  send_response() override
  {
    valid = true;
    if ((objs.size() > 1) ||
        (!objs.empty() && (objs.front().key.name != prefix))) {
      has_children = true;
      return;
    }
...
public:
  ...
  int
  read_permissions(RGWOp* op_obj, optional_yield) override
  {
...
public:
  ...
  int
  get_data(buffer::list& _bl) override
  {
...
public:
  ...
  header_init() override
  {
    ...
    /* XXX derp derp derp */
    state->relative_uri =
        make_uri(rgw_fh->bucket_name(), rgw_fh->relative_object_name());
  ...
  int
  send_response_data(ceph::buffer::list& bl, off_t bl_off, off_t bl_len) override
  {
    size_t bytes;
  ...
  int
  send_response_data_error(optional_yield) override
  {
...
public:
  ...
  int
  send_response_data(ceph::buffer::list& _bl, off_t s_off, off_t e_off) override
  {
    /* NOP */
    /* XXX save attrs? */
    return 0;
  ...
  void
  execute(optional_yield y) override
  {
    RGWGetObj::execute(y);
...
public:
  ...
  send_response() override
  {
    ...
    for (const auto& iter : objs) {
      ...
      lsubdout(cct, rgw, 15)
          << "RGWStatLeafRequest " << __func__ << " "
          << "list uri=" << state->relative_uri << " " << " prefix=" << prefix
          << " " << " obj path=" << name << "" << " target = " << path << ""
          << dendl;
...
public:
  ...
  void
  put_data(off_t off, buffer::list& _bl)
  {
    if (off != real_ofs) {
      eio = true;
    }
...
public:
  ...
  RGWCopyObjRequest(
      CephContext* _cct,
      std::unique_ptr<rgw::sal::User> _user,
      RGWFileHandle* _src_parent,
      RGWFileHandle* _dst_parent,
  ...
  void
  send_partial_response(off_t ofs) override
  {}
...
public:
  ...
  const flat_map<std::string, std::optional<buffer::list>>&
  get_attrs()
  {
    return attrs;
  }
...
public:
  ...
  const std::map<std::string, buffer::list>&
  get_attrs()
  {
    return attrs;
  }
