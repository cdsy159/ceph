...
flt::Ticket
FlightKeyToTicket(const FlightKey& key)
{
  flt::Ticket result;
...
arw::Result<FlightKey>
TicketToFlightKey(const flt::Ticket& t)
{
  try {
    ...
    return arw::Status::Invalid(
        "could not convert Ticket containing \"%s\" into a Flight Key due to "
        "range",
        t.ticket);
  }
}
...
FlightKey
MemoryFlightStore::add_flight(FlightData&& flight)
{
...
arw::Result<FlightData>
MemoryFlightStore::get_flight(const FlightKey& key) const
{
  const std::lock_guard lock(mtx);
  auto i = map.find(key);
  if (i == map.cend()) {
...
std::optional<FlightData>
MemoryFlightStore::after_key(const FlightKey& key) const
{
...
int
MemoryFlightStore::remove_flight(const FlightKey& key)
{
  return 0;
}

int
MemoryFlightStore::expire_flights()
{
  return 0;
}
...
FlightServer::FlightServer(
  ...
  env(_env), driver(env.driver), dp(_dp), flight_store(_flight_store)
{}
...
FlightServer::ListFlights(
    const flt::ServerCallContext& context,
    ...
    std::unique_ptr<flt::FlightListing>* listings)
{
  ...
  public:
    RGWFlightListing(FlightStore* flight_store) :
      flight_store(flight_store), previous_key(null_flight_key)
    {}

    arrow::Result<std::unique_ptr<flt::FlightInfo>>
    Next() override
    {
      std::optional<FlightData> fd = flight_store->after_key(previous_key);
      if (fd) {
        previous_key = fd->key;
        auto descriptor = flt::FlightDescriptor::Path(
            {fd->tenant_name, fd->bucket_name, fd->object_key.name,
             fd->object_key.instance, fd->object_key.ns});
        flt::FlightEndpoint endpoint;
        endpoint.ticket = FlightKeyToTicket(fd->key);
        std::vector<flt::FlightEndpoint> endpoints{endpoint};

        ARROW_ASSIGN_OR_RAISE(
            flt::FlightInfo info_obj, flt::FlightInfo::Make(
                                          *fd->schema, descriptor, endpoints,
                                          fd->num_records, fd->obj_size));
...
protected:
  OwnedBuffer(uint8_t* _buffer, int64_t _size) :
    Buffer(_buffer, _size), buffer(_buffer)
  {}

public:
  ~OwnedBuffer() override { delete[] buffer; }

  static arw::Result<std::shared_ptr<OwnedBuffer>>
  make(int64_t size)
  {
    uint8_t* buffer = new (std::nothrow) uint8_t[size];
    if (!buffer) {
      return arw::Status::OutOfMemory(
          "could not allocated buffer of size %" PRId64, size);
    }
...
public:
  RandomAccessObject(
      const FlightData& _flight_data,
      std::unique_ptr<rgw::sal::Object>& obj,
      const DoutPrefix _dp) :
    flight_data(_flight_data), dp(_dp), position(-1), is_closed(false)
  {
    op = obj->get_read_op();
  }
  ...
  Read(int64_t nbytes, void* out) override
  {
    ...
    if (nbytes != bytes_read) {
      INFO << "partial read: nbytes=" << nbytes << ", bytes_read=" << bytes_read
           << dendl;
    }
  ...
  arw::Result<std::shared_ptr<arw::Buffer>>
  Read(int64_t nbytes) override
  {
    ...
    ARROW_ASSIGN_OR_RAISE(
        const int64_t bytes_read, Read(nbytes, buffer->writeable_data()));
    buffer->set_size(bytes_read);
  ...
  Peek(int64_t nbytes) override
  {
    ...
    ARROW_ASSIGN_OR_RAISE(
        OwningStringView buffer, OwningStringView::make(nbytes));

    ARROW_ASSIGN_OR_RAISE(
        const int64_t bytes_read, Read(nbytes, (void*)buffer.writeable_data()));
  ...
  arw::Result<std::shared_ptr<const arw::KeyValueMetadata>>
  ReadMetadata()
  {
    return flight_data.kv_metadata;
  }

  arw::Future<std::shared_ptr<const arw::KeyValueMetadata>>
  ReadMetadataAsync(const arw::io::IOContext& io_context) override
  {
    return arw::Future<std::shared_ptr<const arw::KeyValueMetadata>>::MakeFinished(
        ReadMetadata());
  }
    ...
    std::unique_ptr<flt::FlightDataStream>* stream)
{
  ...
  ret = driver->load_bucket(
      &dp, rgw_bucket(fd.tenant_name, fd.bucket_name), &bucket, null_yield);
  if (ret < 0) {
  ...
  ARROW_RETURN_NOT_OK(
      parquet::arrow::OpenFile(input, arw::default_memory_pool(), &reader));
  ...
  ARROW_ASSIGN_OR_RAISE(
      auto owning_reader,
      arw::RecordBatchReader::Make(std::move(batches), table->schema()));
