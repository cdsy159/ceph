...
class OwningStringView : public std::string_view {
  ...
  OwningStringView(OwningStringView&& from, int64_t new_size) :
    buffer(nullptr), capacity(from.capacity), consumed(new_size)
  {
...
public:
  ...
  static arw::Result<OwningStringView>
  make(int64_t size)
  {
  ...
  static arw::Result<OwningStringView>
  shrink(OwningStringView&& from, int64_t new_size)
  {
