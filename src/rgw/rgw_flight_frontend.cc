...
FlightFrontend::init()
{
  ...
  const std::string url = std::string("grpc+tcp://localhost:") +
                          std::to_string(port);
...
{
  try {
    flight_thread = make_named_thread(
        server_thread_name, &FlightServer::ServeAlt, env.flight_server);
...
void
FlightFrontend::pause_for_new_config()
{
  // ignore since config changes won't alter flight_server
}

void
FlightFrontend::unpause_with_new_config()
{
  // ignore since config changes won't alter flight_server
}
...
FlightGetObj_Filter::~FlightGetObj_Filter()
{
  ...
  if (error) {
    ...
    INFO << "parquet/arrow schema determination status: " << schema_status
         << dendl;
  }
}

int
FlightGetObj_Filter::handle_data(bufferlist& bl, off_t bl_ofs, off_t bl_len)
{
  INFO << "flight handling data from offset " << current_offset << " ("
       << bl_ofs << ") of size " << bl_len << dendl;
  ...
  if (temp_file.is_open()) {
    ...
    if (current_offset >= expected_size) {
      INFO << "data read is completed, current_offset=" << current_offset
           << ", expected_size=" << expected_size << dendl;
      temp_file.close();
      ...
      auto process_metadata = [&aw_schema, &num_rows, &kv_metadata,
                               this]() -> arrow::Status {
        ...
        ARROW_RETURN_NOT_OK(
            parquet::arrow::FromParquetSchema(pq_schema, &aw_schema));
      ...
      if (!schema_status.ok()) {
        ERROR << "reading metadata to access schema, error=" << schema_status
              << dendl;
      } else {
        // INFO << "arrow_schema=" << *aw_schema << dendl;
        FlightStore* store = penv.flight_store;
        auto key = store->add_flight(FlightData(
            uri, tenant_name, bucket_name, object_key, num_rows, expected_size,
