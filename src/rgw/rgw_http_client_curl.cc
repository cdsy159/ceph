...
void
rgw_ssl_locking_callback(int mode, int id, const char* file, int line)
{
...
bool
fe_inits_ssl(boost::optional<const fe_map_t&> m, long& curl_global_flags)
{
  if (m) {
    for (const auto& kv : *m) {
      if (kv.first == "beast") {
...
void
setup_curl(boost::optional<const fe_map_t&> m)
{
