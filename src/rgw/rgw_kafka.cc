...
inline int
rd_kafka_err2errno(rd_kafka_resp_err_t err)
{
  if (err == 0)
    return 0;
  switch (err) {
...
bool
operator==(const connection_id_t& lhs, const connection_id_t& rhs)
{
...
struct connection_id_hasher {
  std::size_t
  operator()(const connection_id_t& k) const
  {
    std::size_t h = 0;
...
std::string
to_string(const connection_id_t& id)
{
...
struct connection_t {
  ...
  const bool use_ssl;
  const bool
      verify_ssl; // TODO currently ignored, not supported in librdkafka v0.11.6
    ...
    void* opaque)
{
  ...
  if (!rkmessage->_private) {
    ldout(conn->cct, 20) << "Kafka run: n/ack received without a callback"
                         << dendl;
    return;
  }
  ...
  if (tag_it != callbacks_end) {
    ldout(conn->cct, 20)
        << "Kafka run: n/ack received, invoking callback with tag=" << *tag
        << dendl;
...
void
log_callback(const rd_kafka_t* rk, int level, const char* fac, const char* buf)
{
  ceph_assert(rd_kafka_opaque(rk));

  const auto conn = reinterpret_cast<connection_t*>(rd_kafka_opaque(rk));
  if (level <= 3)
    ldout(conn->cct, 1) << "RDKAFKA-" << level << "-" << fac << ": "
                        << rd_kafka_name(rk) << ": " << buf << dendl;
  else if (level <= 5)
    ldout(conn->cct, 2) << "RDKAFKA-" << level << "-" << fac << ": "
                        << rd_kafka_name(rk) << ": " << buf << dendl;
  else if (level <= 6)
    ldout(conn->cct, 10) << "RDKAFKA-" << level << "-" << fac << ": "
                         << rd_kafka_name(rk) << ": " << buf << dendl;
  else
    ldout(conn->cct, 20) << "RDKAFKA-" << level << "-" << fac << ": "
                         << rd_kafka_name(rk) << ": " << buf << dendl;
}

void
poll_err_callback(rd_kafka_t* rk, int err, const char* reason, void* opaque)
{
  const auto conn = reinterpret_cast<connection_t*>(rd_kafka_opaque(rk));
  ldout(conn->cct, 10) << "Kafka run: poll error(" << err << "): " << reason
                       << dendl;
}
...
bool
new_producer(connection_t* conn)
{
  ...
  if (!conf) {
    ldout(conn->cct, 1) << "Kafka connect: failed to allocate configuration"
                        << dendl;
  ...
  const auto message_timeout = std::max(
      min_message_timeout, conn->cct->_conf->rgw_kafka_message_timeout);
  if (rd_kafka_conf_set(
          conf.get(), "message.timeout.ms",
  ...
  if (rd_kafka_conf_set(
          conf.get(), "bootstrap.servers", conn->broker.c_str(), errstr,
          sizeof(errstr)) != RD_KAFKA_CONF_OK)
    goto conf_error;
  ...
  if (conn->use_ssl) {
    if (!conn->user.empty()) {
      // use SSL+SASL
      if (rd_kafka_conf_set(
              conf.get(), "security.protocol", "SASL_SSL", errstr,
              sizeof(errstr)) != RD_KAFKA_CONF_OK ||
          rd_kafka_conf_set(
              conf.get(), "sasl.username", conn->user.c_str(), errstr,
              sizeof(errstr)) != RD_KAFKA_CONF_OK ||
          rd_kafka_conf_set(
              conf.get(), "sasl.password", conn->password.c_str(), errstr,
              sizeof(errstr)) != RD_KAFKA_CONF_OK)
        goto conf_error;
      ldout(conn->cct, 20)
      ...
      if (conn->mechanism) {
        if (rd_kafka_conf_set(
                conf.get(), "sasl.mechanism", conn->mechanism->c_str(), errstr,
                sizeof(errstr)) != RD_KAFKA_CONF_OK)
          goto conf_error;
        ldout(conn->cct, 20)
        ...
        if (rd_kafka_conf_set(
                conf.get(), "sasl.mechanism", "PLAIN", errstr,
                sizeof(errstr)) != RD_KAFKA_CONF_OK)
          goto conf_error;
        ldout(conn->cct, 20)
            << "Kafka connect: using default SASL mechanism" << dendl;
      }

    } else {
      // use only SSL
      if (rd_kafka_conf_set(
              conf.get(), "security.protocol", "SSL", errstr, sizeof(errstr)) !=
          RD_KAFKA_CONF_OK)
        goto conf_error;
      ldout(conn->cct, 20)
          << "Kafka connect: successfully configured SSL security" << dendl;
    }
    if (conn->ca_location) {
      if (rd_kafka_conf_set(
              conf.get(), "ssl.ca.location", conn->ca_location->c_str(), errstr,
              sizeof(errstr)) != RD_KAFKA_CONF_OK)
        goto conf_error;
      ldout(conn->cct, 20)
          << "Kafka connect: successfully configured CA location" << dendl;
    } else {
      ldout(conn->cct, 20) << "Kafka connect: using default CA location"
                           << dendl;
    }
    ...
    ldout(conn->cct, 20) << "Kafka connect: successfully configured security"
                         << dendl;
  } else if (!conn->user.empty()) {
    // use SASL+PLAINTEXT
    if (rd_kafka_conf_set(
            conf.get(), "security.protocol", "SASL_PLAINTEXT", errstr,
            sizeof(errstr)) != RD_KAFKA_CONF_OK ||
        rd_kafka_conf_set(
            conf.get(), "sasl.username", conn->user.c_str(), errstr,
            sizeof(errstr)) != RD_KAFKA_CONF_OK ||
      ...
      goto conf_error;
    ldout(conn->cct, 20)
        << "Kafka connect: successfully configured SASL_PLAINTEXT" << dendl;
    ...
    if (conn->mechanism) {
      ...
      if (rd_kafka_conf_set(
              conf.get(), "sasl.mechanism", "PLAIN", errstr, sizeof(errstr)) !=
          RD_KAFKA_CONF_OK)
        goto conf_error;
      ldout(conn->cct, 20) << "Kafka connect: using default SASL mechanism"
                           << dendl;
    }
  }
  ...
  conn->producer =
      rd_kafka_new(RD_KAFKA_PRODUCER, conf.release(), errstr, sizeof(errstr));
  if (!conn->producer) {
    conn->status = rd_kafka_last_error();
    ldout(conn->cct, 1) << "Kafka connect: failed to create producer: "
                        << errstr << dendl;
    return false;
  }
  ldout(conn->cct, 20) << "Kafka connect: successfully created new producer"
                       << dendl;
  {
    // set log level of producer
    const auto log_level =
        conn->cct->_conf->subsys.get_log_level(ceph_subsys_rgw);
    if (log_level <= 1)
...
conf_error:
  conn->status = rd_kafka_last_error();
  ldout(conn->cct, 1) << "Kafka connect: configuration failed: " << errstr
                      << dendl;
  return false;
}
...
struct message_wrapper_t {
  ...
  message_wrapper_t(
      const connection_id_t& _conn_id,
...
typedef std::unordered_map<connection_id_t, connection_t_ptr, connection_id_hasher>
    ConnectionList;
typedef boost::lockfree::
    queue<message_wrapper_t*, boost::lockfree::fixed_sized<true>>
        MessageQueue;
...
private:
  ...
  publish_internal(message_wrapper_t* message)
  {
    ...
    if (conn->status != 0) {
      ...
      ldout(conn->cct, 1) << "Kafka publish: producer was closed while message "
                             "was in the queue. with status: "
    ...
    if (topic_it == conn->topics.end()) {
      connection_t::topic_ptr topic(
          rd_kafka_topic_new(conn->producer, message->topic.c_str(), nullptr));
      if (!topic) {
        const auto err = rd_kafka_last_error();
        ldout(conn->cct, 1)
            << "Kafka publish: failed to create topic: " << message->topic
            << " error: " << rd_kafka_err2str(err) << "(" << err << ")"
            << dendl;
        if (message->cb) {
      ...
      ldout(conn->cct, 20) << "Kafka publish: successfully created topic: "
                           << message->topic << dendl;
    } else {
      ldout(conn->cct, 20) << "Kafka publish: reused existing topic: "
                           << message->topic << dendl;
    }

    const auto tag =
        (message->cb == nullptr ? nullptr : new uint64_t(conn->delivery_tag++));
    const auto rc = rd_kafka_produce(
        ...
        RD_KAFKA_MSG_F_COPY, message->message.data(), message->message.length(),
    ...
    if (tag) {
      ...
      if (q_len < max_inflight) {
        ...
        ldout(conn->cct, 1) << "Kafka publish (with callback): failed with "
  ...
  run() noexcept
  {
    ...
    while (!stopped) {
      ...
      auto reply_count = 0U;
      const auto send_count = messages.consume_all([this](auto message) {
        this->publish_internal(message);
      });
      dequeued += send_count;
      ...
      for (; conn_it != end_it;) {
        ...
        if (conn->timestamp.sec() + conn->cct->_conf->rgw_kafka_connection_idle <
            ceph_clock_now()) {
          ldout(conn->cct, 20)
              << "kafka run: deleting a connection that was idle for: "
              << conn->cct->_conf->rgw_kafka_connection_idle
              << " seconds. last activity was at: " << conn->timestamp << dendl;
          std::lock_guard lock(connections_lock);
  ...
  static void
  delete_message(const message_wrapper_t* message)
  {
    delete message;
  }
...
public:
      ...
      boost::optional<const std::string&> brokers)
  {
    ...
    if (topic_user_name.has_value()) {
      if (!user.empty()) {
        ldout(cct, 5) << "Kafka connect: username provided via both topic "
                         "attributes and endpoint URL: using topic attributes"
                      << dendl;
      }
    ...
    if (topic_password.has_value()) {
      if (!password.empty()) {
        ldout(cct, 5) << "Kafka connect: password provided via both topic "
                         "attributes and endpoint URL: using topic attributes"
                      << dendl;
      }
    ...
    connection_id_t tmp_id(
        broker_list, user, password, ca_location, mechanism, use_ssl);
    std::lock_guard lock(connections_lock);
    ...
    auto conn = std::make_unique<connection_t>(
        cct, broker_list, use_ssl, verify_ssl, ca_location, user, password,
        mechanism);
    if (!new_producer(conn.get())) {
      ldout(cct, 10)
          << "Kafka connect: producer creation failed in new connection"
          << dendl;
      return false;
    }
    ++connection_count;
  ...
  get_inflight() const
  {
    ...
    std::for_each(
        ...
        [&sum](auto& conn_pair) { sum += conn_pair.second->callbacks.size(); });
    return sum;
  }
...
init(CephContext* cct)
{
  ...
  s_manager = new Manager(
      MAX_CONNECTIONS_DEFAULT, MAX_INFLIGHT_DEFAULT, MAX_QUEUE_DEFAULT, cct);
  return true;
}
    ...
    boost::optional<const std::string&> brokers)
{
  ...
  if (!s_manager)
    return false;
  return s_manager->connect(
      conn_id, url, use_ssl, verify_ssl, ca_location, mechanism, user_name,
      password, brokers);
}
