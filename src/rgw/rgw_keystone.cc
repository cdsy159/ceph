...
bool
rgw_is_pki_token(const string& token)
{
  return token.compare(0, sizeof(PKI_ANS1_PREFIX) - 1, PKI_ANS1_PREFIX) == 0;
}

void
rgw_get_token_id(const string& token, string& token_id)
{
...
read_secret(const std::string& file_path)
{
  using namespace std;
...
int
Service::issue_admin_token_request(
    const DoutPrefixProvider* dpp,
    const Config& config,
    optional_yield y,
    TokenEnvelope& t)
    ...
    std::string& token)
{
  ...
  ldpp_dout(dpp, 20) << "Requesting secret from barbican url=" << token_url
                     << dendl;
  int ret = token_req.process(dpp, y);
  if (ret < 0) {
    ldpp_dout(dpp, 20) << "Barbican process error:" << token_bl.c_str()
                       << dendl;
    return ret;
  }
...
bool
TokenEnvelope::has_role(const std::string& r) const
{
...
bool
TokenCache::find(const std::string& token_id, rgw::keystone::TokenEnvelope& token)
{
    ...
    std::list<std::string>& tokens_lru)
{
  ...
  if (iter == tokens.end()) {
    if (perfcounter)
      perfcounter->inc(l_rgw_keystone_token_cache_miss);
    return false;
  }
  ...
  if (entry.token.expired()) {
    tokens.erase(iter);
    if (perfcounter)
      perfcounter->inc(l_rgw_keystone_token_cache_hit);
    return false;
  }
  token = entry.token;
...
bool
TokenCache::find_admin(rgw::keystone::TokenEnvelope& token)
{
  std::lock_guard l{lock};
...
bool
TokenCache::find_barbican(rgw::keystone::TokenEnvelope& token)
{
  std::lock_guard l{lock};
...
void
TokenCache::add_admin(const rgw::keystone::TokenEnvelope& token)
{
  std::lock_guard l{lock};
...
void
TokenCache::add_barbican(const rgw::keystone::TokenEnvelope& token)
{
  std::lock_guard l{lock};
...
void
TokenCache::invalidate(const DoutPrefixProvider* dpp, const std::string& token_id)
{
  std::lock_guard l{lock};
  map<string, token_entry>::iterator iter = tokens.find(token_id);
  if (iter == tokens.end())
...
void
TokenCache::invalidate_admin(const DoutPrefixProvider* dpp)
{
...
void
rgw::keystone::TokenEnvelope::Token::decode_json(JSONObj* obj)
{
  ...
  if (parse_iso8601(expires_iso8601.c_str(), &t)) {
    ...
    throw JSONDecoder::err(
        "Failed to parse ISO8601 expiration date from Keystone response.");
  }
}

void
rgw::keystone::TokenEnvelope::Role::decode_json(JSONObj* obj)
{
...
void
rgw::keystone::TokenEnvelope::Domain::decode_json(JSONObj* obj)
{
  JSONDecoder::decode_json("id", id, obj, true);
  JSONDecoder::decode_json("name", name, obj, true);
}
...
void
rgw::keystone::TokenEnvelope::Project::decode_json(JSONObj* obj)
{
  JSONDecoder::decode_json("id", id, obj, true);
  JSONDecoder::decode_json("name", name, obj, true);
...
void
rgw::keystone::TokenEnvelope::User::decode_json(JSONObj* obj)
{
  JSONDecoder::decode_json("id", id, obj, true);
  JSONDecoder::decode_json("name", name, obj, true);
  JSONDecoder::decode_json("domain", domain, obj);
...
void
rgw::keystone::TokenEnvelope::ApplicationCredential::decode_json(JSONObj* obj)
{
  JSONDecoder::decode_json("id", id, obj, true);
  JSONDecoder::decode_json("name", name, obj, true);
...
void
rgw::keystone::TokenEnvelope::decode(JSONObj* const root_obj)
{
  ...
  if (JSONDecoder::decode_json(
          "application_credential", tmp_app_cred, root_obj, false)) {
  ...
  if (parse_iso8601(expires_iso8601.c_str(), &t)) {
    ...
    token.expires = 0;
    throw JSONDecoder::err(
        "Failed to parse ISO8601 expiration date"
...
static inline std::string
to_string(const std::string_view& s)
{
  return std::string(s.data(), s.length());
}

void
rgw::keystone::AdminTokenRequest::dump(Formatter* const f) const
{
...
void
rgw::keystone::BarbicanTokenRequest::dump(Formatter* const f) const
{
  f->open_object_section("token_request");
  f->open_object_section("auth");
  f->open_object_section("identity");
