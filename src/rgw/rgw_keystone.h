...
rgw_get_token_id(const std::string& token)
{
  std::string token_id;
...
public:
  ...
  static int get_admin_token(
      ...
      std::string& token,
      bool& token_cached);
  static int issue_admin_token_request(
      const DoutPrefixProvider* dpp,
      const Config& config,
      optional_yield y,
      TokenEnvelope& token);
  static int get_keystone_barbican_token(
      const DoutPrefixProvider* dpp,
      optional_yield y,
      std::string& token);
};
...
public:
  ...
  public:
    Role() :
      is_admin(false), is_reader(false)
    {}
...
public:
  ...
  const std::string&
  get_domain_name() const
  {
    return project.domain.name;
  };

  const std::string&
  get_project_id() const
  {
    return project.id;
  };
...
class TokenCache {
  ...
  explicit TokenCache(const rgw::keystone::Config& config) :
    cct(g_ceph_context), max(cct->_conf->rgw_keystone_token_cache_size)
  {}
...
public:
  ...
  {
    static_assert(
        std::is_base_of<rgw::keystone::Config, ConfigT>::value,
  ...
  boost::optional<TokenEnvelope>
  find(const std::string& token_id)
  {
  ...
  boost::optional<TokenEnvelope>
  find_service(const std::string& token_id)
  {
    TokenEnvelope token_envlp;
