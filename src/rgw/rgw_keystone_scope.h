...
struct ScopeInfo {
  ...
  std::vector<std::string>
      roles; // May be empty (controlled by include_roles config)
...
inline void
encode(const ScopeInfo::domain_t& v, bufferlist& bl)
{
  v.encode(bl);
}

inline void
decode(ScopeInfo::domain_t& v, bufferlist::const_iterator& p)
{
  v.decode(p);
}

inline void
encode(const ScopeInfo::project_t& v, bufferlist& bl)
{
  v.encode(bl);
}

inline void
decode(ScopeInfo::project_t& v, bufferlist::const_iterator& p)
{
  v.decode(p);
}

inline void
encode(const ScopeInfo::user_t& v, bufferlist& bl)
{
  v.encode(bl);
}

inline void
decode(ScopeInfo::user_t& v, bufferlist::const_iterator& p)
{
  v.decode(p);
}

inline void
encode(const ScopeInfo::app_cred_t& v, bufferlist& bl)
{
  v.encode(bl);
}

inline void
decode(ScopeInfo::app_cred_t& v, bufferlist::const_iterator& p)
{
  v.decode(p);
}

inline void
encode(const ScopeInfo& v, bufferlist& bl)
{
  v.encode(bl);
}

inline void
decode(ScopeInfo& v, bufferlist::const_iterator& p)
{
  v.decode(p);
}
...
inline void
ScopeInfo::encode(bufferlist& bl) const
{
  ENCODE_START(kEncV, kEncV, bl);
  encode(project, bl);
  encode(user, bl);
...
inline void
ScopeInfo::decode(bufferlist::const_iterator& p)
{
  DECODE_START(kEncV, p);
  decode(project, p);
  decode(user, p);
