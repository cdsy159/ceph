...
LCRule::valid() const
{
  if (id.length() > MAX_ID_LEN) {
    return false;
      ...
      expiration.empty() && noncur_expiration.empty() &&
      mp_expiration.empty() && !dm_expiration && transitions.empty() &&
      noncur_transitions.empty()) {
    return false;
  } else if (
      !expiration.valid() || !noncur_expiration.valid() ||
      !mp_expiration.valid()) {
    return false;
  }
...
void
RGWLifecycleConfiguration::add_rule(const LCRule& rule)
{
  auto& id =
      rule.get_id(); // note that this will return false for groups, but that's ok, we won't search groups
  rule_map.insert(pair<string, LCRule>(id, rule));
}

bool
RGWLifecycleConfiguration::_add_rule(const LCRule& rule)
{
...
int
RGWLifecycleConfiguration::check_and_add_rule(const LCRule& rule)
{
  if (!rule.valid()) {
    return -EINVAL;
  }
  ...
  if (rule.get_filter().has_tags() &&
      (rule.get_dm_expiration() || !rule.get_mp_expiration().empty())) {
...
bool
RGWLifecycleConfiguration::valid()
{
  return true;
}
...
void
RGWLC::initialize(CephContext* _cct, rgw::sal::Driver* _driver)
{
  cct = _cct;
  driver = _driver;
...
operator<<(std::ostream& os, rgw::sal::LCEntry& ent)
{
  os << "<ent: bucket=";
  os << ent.bucket;
  os << "; start_time=";
    ...
    ceph::real_time* expire_time = nullptr)
{
  ...
  ldpp_dout(dpp, 20) << __func__ << "(): mtime=" << mtime << " days=" << days
                     << " base_time=" << base_time << " timediff=" << timediff
                     << " cmp=" << cmp << " is_expired=" << (timediff >= cmp)
                     << dendl;
...
public:
    ...
    driver(_driver), bucket(_bucket)
  {
    ...
    uint64_t threshold =
        cct->_conf.get_val<uint64_t>("rgw_lc_ordered_list_threshold");

    const auto& current_index = bucket->get_info().layout.current_index;
    list_params.allow_unordered =
  ...
  int
  init(const DoutPrefixProvider* dpp, optional_yield y)
  {
    return fetch(dpp, y);
  }

  int
  fetch(const DoutPrefixProvider* dpp, optional_yield y)
  {
  ...
  void
  delay(const DoutPrefixProvider* dpp)
  {
    if (delay_ms) {
      ...
      std::function<void(void)> fetch_barrier = []() { /* nada */ })
  {
    ...
    if (obj_iter->key.name == pre_obj.key.name) {
      ...
      num_noncurrent =
          0; // XXX the first element must be current or a delete marker (?)
    }
...
struct op_env {
  ...
  op_env(
      ...
      LCObjsLister& _ol) :
    op(_op), driver(_driver), worker(_worker), bucket(_bucket), ol(_ol)
  {}
}; /* op_env */
...
struct lc_op_ctx {
  ...
  bool
  next_has_same_name(const std::string& key_name)
  {
    return (
        next_key_name &&
        key_name.compare(boost::get<std::string>(next_key_name)) == 0);
  }

}; /* lc_op_ctx */

static bool
pass_size_limit_checks(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    lc_op_ctx& oc)
{
    ...
    const rgw::notify::EventTypeList& event_types)
{
  ...
  ret = notify->publish_commit(
      dpp, size, ceph::real_clock::now(), etag, version_id);
  if (ret < 0) {
...
static bool
zonegroup_lc_check(const DoutPrefixProvider* dpp, rgw::sal::Zone* zone)
{
    ...
    const rgw::notify::EventTypeList& event_types)
{
  ...
  if (ret < 0) {
    ...
    if (!(o.is_delete_marker() && (ret == -ENOENT))) {
      ldpp_dout(oc.dpp, 0) << fmt::format(
                                  "ERROR: get_obj_state() failed in {} for "
                                  "object k={} error r={}",
                                  __func__, oc.o.key.to_string(), ret)
                           << dendl;
      return ret;
    }
  }
  ...
  uint32_t flags = (!remove_indeed ||
                    !zonegroup_lc_check(dpp, oc.driver->get_zone()))
                       ? rgw::sal::FLAG_LOG_OP
                       : 0;
  ret = del_op->delete_obj(dpp, y, flags);
  if (ret < 0) {
    ldpp_dout(dpp, 1) << fmt::format(
                             "ERROR: {} failed, with error: {}", __func__, ret)
                      << dendl;
  } else {
    if (have_notify) {
      send_notification(
          dpp, y, driver, obj.get(), oc.bucket, etag, size, version_id,
          event_types);
    }
  }
...
public:
  ...
  virtual int
  process(lc_op_ctx& oc, optional_yield y)
  {
    return 0;
  }
...
public:
  ...
  virtual bool
  check(const DoutPrefixProvider* dpp, lc_op_ctx& oc, optional_yield y)
  {
    return false;
  }
}; /* LCOpFilter */
...
public:
  ...
  std::vector<shared_ptr<LCOpAction>>&
  get_actions()
  {
    return actions;
  }
...
static inline bool
worker_should_stop(time_t stop_at, bool once)
{
    ...
    bool once)
{
  ...
  auto pf = [this, target](
                optional_yield y, const lc_op& rule,
  ...
  for (auto prefix_iter = grouped_mp_ops.begin();
       prefix_iter != grouped_mp_ops.end(); ++prefix_iter) {

    if (worker_should_stop(stop_at, once)) {
      ldpp_dout(this, 5) << __func__
                         << " interval budget EXPIRED worker=" << worker->ix
                         << " bucket=" << target->get_name() << dendl;
      return 0;
    }
    ...
    do {
      ...
      for (auto obj_iter = results.objs.begin(); obj_iter != results.objs.end();
           ++obj_iter, ++offset) {
        const auto obj = *obj_iter;
        for (auto* op : prefix_iter->second) {
          workpool.spawn([pf, op,
                          obj](boost::asio::yield_context yield) mutable {
            pf(yield, *op, obj);
          });
        }
        if (going_down()) {
      ...
      if ((offset % 100) == 0) {
        if (worker_should_stop(stop_at, once)) {
          ldpp_dout(this, 5)
              << __func__ << " interval budget EXPIRED worker=" << worker->ix
              << " bucket=" << target->get_name() << dendl;
          return 0;
        }
      }
...
static bool
is_valid_op(const lc_op& op)
{
  return (
      op.status && (op.expiration > 0 || op.expiration_date != boost::none ||
                    op.noncur_expiration > 0 || op.dm_expiration ||
                    !op.transitions.empty() || !op.noncur_transitions.empty()));
}

static bool
zone_check(const lc_op& op, rgw::sal::Zone* zone)
{
...
static inline bool
has_all_tags(const lc_op& rule_action, const RGWObjTags& object_tags)
{
    ...
    *skip = true;
    ...
    if (oc.cached_tags) {
      if (!has_all_tags(op, *oc.cached_tags)) {
        ldpp_dout(oc.dpp, 20)
            << __func__ << "() skipping obj " << oc.obj
            << " as tags do not match in rule: " << op.id << dendl;
        return 0;
      }
    ...
    if (ret < 0) {
      if (ret != -ENODATA) {
        ldpp_dout(oc.dpp, 5)
            << "ERROR: read_obj_tags returned r=" << ret << dendl;
      }
      return 0;
    }
...
public:
  bool
  check(const DoutPrefixProvider* dpp, lc_op_ctx& oc, optional_yield y) override
  {
    auto& o = oc.o;
...
public:
      ...
      optional_yield y) override
  {
    ...
    ldpp_dout(dpp, 20) << __func__ << "(): key=" << o.key
                       ...
                       << " size_check_p: " << size_check_p << dendl;

    return is_expired && size_check_p;
  }

  int
  process(lc_op_ctx& oc, optional_yield y) override
  {
    auto& o = oc.o;
    int r;
    if (o.is_delete_marker()) {
      ...
      if (r < 0) {
        ldpp_dout(oc.dpp, 0)
            << "ERROR: current is-dm remove_expired_obj " << oc.bucket << ":"
            << o.key << " " << cpp_strerror(r) << dendl;
        return r;
      }
      ldpp_dout(oc.dpp, 2) << "DELETED: current is-dm " << oc.bucket << ":"
                           << o.key << dendl;
    } else {
      /* ! o.is_delete_marker() */
      r = remove_expired_obj(
          oc.dpp, y, oc, !oc.bucket->versioning_enabled(),
          {rgw::notify::ObjectExpirationCurrent,
           rgw::notify::LifecycleExpirationDelete});
      if (r < 0) {
        ldpp_dout(oc.dpp, 0) << "ERROR: remove_expired_obj " << oc.bucket << ":"
                             << o.key << " " << cpp_strerror(r) << dendl;
        return r;
      }
      if (perfcounter) {
      ...
      ldpp_dout(oc.dpp, 2) << "DELETED:" << oc.bucket << ":" << o.key << dendl;
    }
    return 0;
  }
};
...
public:
      ...
      optional_yield y) override
  {
    ...
    bool is_expired =
        obj_has_expired(dpp, oc.cct, oc.effective_mtime, expiration, exp_time);
    auto size_check_p = pass_size_limit_checks(dpp, y, oc);
    auto newer_noncurrent_p = (oc.num_noncurrent > oc.op.newer_noncurrent);

    ldpp_dout(dpp, 20) << __func__ << "(): key=" << o.key
                       << ": is_expired=" << is_expired << " "
                       << ": num_noncurrent=" << oc.num_noncurrent
                       << " size_check_p: " << size_check_p
                       << " newer_noncurrent_p: " << newer_noncurrent_p
                       << dendl;

    return is_expired && (oc.num_noncurrent > oc.op.newer_noncurrent) &&
           size_check_p &&
  ...
  {
    auto& o = oc.o;
    int r = remove_expired_obj(
        oc.dpp, y, oc, true,
        {rgw::notify::LifecycleExpirationDelete,
         rgw::notify::ObjectExpirationNoncurrent});
    if (r < 0) {
      ldpp_dout(oc.dpp, 0)
          << "ERROR: remove_expired_obj (non-current expiration) " << oc.bucket
          << ":" << o.key << " " << cpp_strerror(r) << dendl;
      return r;
    }
    if (perfcounter) {
...
public:
  ...
  process(lc_op_ctx& oc, optional_yield y) override
  {
    ...
    if (r < 0) {
      ldpp_dout(oc.dpp, 0)
          << "ERROR: remove_expired_obj (delete marker expiration) "
          << oc.bucket << ":" << o.key << " " << cpp_strerror(r) << dendl;
      return r;
    }
    if (perfcounter) {
...
public:
      ...
      optional_yield y) override
  {
    ...
    if (transition.days < 0) {
      ...
      *exp_time = *transition.date;
      ...
      is_expired =
          obj_has_expired(dpp, oc.cct, mtime, transition.days, exp_time);
    }

    auto size_check_p = pass_size_limit_checks(dpp, y, oc);

    ldpp_dout(oc.dpp, 20) << __func__ << "(): key=" << o.key
                          << ": is_expired=" << is_expired << " "
                          << " size_check_p: " << size_check_p << dendl;
  ...
  bool
  should_process() override
  {
    return need_to_process;
  }

  int
  delete_tier_obj(lc_op_ctx& oc, optional_yield y)
  {
    int ret = 0;
    ...
    if (!oc.bucket->versioning_enabled()) {
      ret = remove_expired_obj(
          oc.dpp, y, oc, true, {/* no delete notify expected */});
      ...
      if (oc.o.is_current() && !oc.o.is_delete_marker()) {
        ret = remove_expired_obj(
            oc.dpp, y, oc, false, {/* no delete notify expected */});
        ldpp_dout(oc.dpp, 20) << "delete_tier_obj Object(key:" << oc.o.key
  ...
  int
  transition_obj_to_cloud(lc_op_ctx& oc, optional_yield y)
  {
    int ret{0};
    /* If CurrentVersion object & bucket has versioning enabled, remove it &
     * create delete marker */
    bool delete_object =
        (!oc.tier->retain_head_object() ||
    ...
    ret = oc.obj->transition_to_cloud(
        oc.bucket, oc.tier.get(), oc.o, oc.env.worker->get_cloud_targets(),
    ...
    if (ret < 0) {
      ...
      if (bucket->versioned() && oc.o.is_current() && !oc.o.is_delete_marker()) {
        event_types.insert(
            event_types.end(), {rgw::notify::ObjectTransitionCurrent,
                                rgw::notify::LifecycleTransition});
      } else {
        event_types.push_back(rgw::notify::ObjectTransitionNonCurrent);
      }
      send_notification(
          oc.dpp, y, oc.driver, obj.get(), oc.bucket, etag, size,
    ...
    if (delete_object) {
      ...
      if (ret < 0) {
        ldpp_dout(oc.dpp, 0) << "ERROR: Deleting tier object(" << oc.o.key
  ...
  process(lc_op_ctx& oc, optional_yield y) override
  {
    ...
    if (oc.o.meta.category == RGWObjCategory::CloudTiered) {
      ...
      ldpp_dout(oc.dpp, 30) << "Object(key:" << oc.o.key
                            << ") is already cloud tiered to cloud-s3 tier: "
                            << oc.o.meta.storage_class << dendl;
      return 0;
    }
    ...
    if (r < 0) {
      ldpp_dout(oc.dpp, 0) << fmt::format(
                                  "ERROR: get_obj_state() failed on transition "
                                  "of object k={} error r={}",
                                  oc.o.key.to_string(), r)
                           << dendl;
      return r;
    }
    ...
    if ((!bucket->versioned()) || (bucket->versioned() && oc.o.is_current() &&
    ...
    std::unique_ptr<rgw::sal::Notification> notify = oc.driver->get_notification(
        oc.dpp, obj.get(), nullptr, event_types, bucket, lc_id,
        const_cast<std::string&>(oc.bucket->get_tenant()), lc_req_id, y);
    ...
    if (!r && oc.tier->is_tier_type_s3()) {
      ldpp_dout(oc.dpp, 30)
          << "Found cloud s3 tier: " << target_placement.storage_class << dendl;
      if (!oc.o.is_current() &&
      ...
      if (r < 0) {
        ldpp_dout(oc.dpp, 0)
            << "ERROR: failed to transition obj(key:" << oc.o.key
            << ") to cloud (r=" << r << ")" << dendl;
        return r;
      }
    } else {
      if (!oc.driver->valid_placement(target_placement)) {
        ldpp_dout(oc.dpp, 0)
            << "ERROR: non existent dest placement: " << target_placement
            << " bucket=" << oc.bucket << " rule_id=" << oc.op.id << dendl;
        return -EINVAL;
      }
      ...
      int r = oc.obj->transition(
          oc.bucket, target_placement, o.meta.mtime, o.versioned_epoch, oc.dpp,
          y, flags);
      if (r < 0) {
        ldpp_dout(oc.dpp, 0)
            << "ERROR: failed to transition obj " << oc.bucket << ":" << o.key
            << " -> " << transition.storage_class << " " << cpp_strerror(r)
            << dendl;
        return r;
      }
    }
    ...
    int publish_ret = notify->publish_commit(
        oc.dpp, obj->get_size(), ceph::real_clock::now(), etag, version_id);
    if (publish_ret < 0) {
      ldpp_dout(oc.dpp, 5)
          << "WARNING: notify publish_commit failed, with error: "
          << publish_ret << dendl;
    }

    ldpp_dout(oc.dpp, 2) << "TRANSITIONED:" << oc.bucket << ":" << o.key
...
protected:
  bool
  check_current_state(bool is_current) override
  {
    return is_current;
  }

  ceph::real_time
  get_effective_mtime(lc_op_ctx& oc) override
  {
...
public:
  LCOpAction_NonCurrentTransition(
      op_env& env,
    ...
    const RGWObjTags* cached_tags)
{
  ...
  if (selected && (*selected)->should_process()) {
    ...
    if (r < 0) {
      ldpp_dout(dpp, 0) << "ERROR: remove_expired_obj " << env.bucket << ":"
                        << o.key << " " << cpp_strerror(r) << dendl;
      return r;
    }
    ldpp_dout(dpp, 20) << "processed:" << env.bucket << ":" << o.key << dendl;
  }
    ...
    boost::asio::yield_context yield)
{
  ...
  ldpp_dout(this, 5) << "RGWLC::bucket_lc_process ENTER bucket=" << bucket_name
                     << dendl;
  ...
  int ret = driver->load_bucket(
      this, rgw_bucket(bucket_tenant, bucket_name), &bucket, yield);
  if (ret < 0) {
    ldpp_dout(this, 0) << "LC:get_bucket for " << bucket_name << " failed"
                       << dendl;
    return ret;
  }
  ...
  auto stack_guard = make_scope_guard([&workpool] { workpool.wait(); });
  ...
  try {
    ...
    ldpp_dout(this, 0) << __func__
                       << "() decode life cycle config failed bucket="
                       << bucket_name << dendl;
    return -1;
  }
  ...
  auto pf = [&bucket_name](
                ...
                const RGWObjTags* cached_tags) {
    ldpp_dout(dpp, 20) << __func__ << "(): key=" << o.key << dendl;
    int ret = op_rule.process(o, dpp, y, cached_tags);
    if (ret < 0) {
      ldpp_dout(dpp, 20) << "ERROR: orule.process() returned ret=" << ret
                         << " bucket=" << bucket_name << dendl;
    }
  };

  multimap<string, lc_op>& prefix_map = config.get_prefix_map();
  ldpp_dout(this, 10) << __func__ << "() prefix_map size=" << prefix_map.size()
                      << dendl;
  ...
  for (auto prefix_iter = grouped_ops.begin(); prefix_iter != grouped_ops.end();
    ...
    if (prefix_iter != grouped_ops.begin() &&
        (prefix_iter->first.compare(
             0, prev(prefix_iter)->first.length(), prev(prefix_iter)->first) ==
         0)) {
    ...
    for (auto* op : prefix_iter->second) {
      ...
      if (!zone_check(*op, zone)) {
        ldpp_dout(this, 7) << "LC rule not executable in "
                           << zone->get_tier_type() << " zone, skipping"
                           << dendl;
        continue;
      }
    ...
    for (auto offset = 0; ol.get_obj(this, yield, &o /* , fetch_barrier */);
         ++offset, ol.next()) {
      const auto obj = *o;
      ...
      workpool.spawn([&pf, dpp = this, rules_copy = rules, obj,
        ...
        if (any_rule_needs_tags && !obj.is_delete_marker()) {
          ...
          std::unique_ptr<rgw::sal::Object::ReadOp> rop =
              temp_obj->get_read_op();
          ...
          if (ret == 0) {
            try {
              ...
              ldpp_dout(dpp, 5)
                  << "ERROR: decode tags for " << obj.key << dendl;
            }
          }
        }

        for (auto& rule : rules_copy) {
          if (rule.needs_tags() && !obj.is_delete_marker() && cached_tags_ptr &&
  ...
  ret = handle_multipart_expiration(
      bucket.get(), prefix_map, workpool, yield, worker, stop_at, once);
  return ret;
}
    ...
    bool once)
{
  ...
  boost::asio::spawn(
      context,
      [this, &shard_id, worker, stop_at,
       once](boost::asio::yield_context yield) {
...
public:
  ...
  bool
  wait_backoff(const fu2::unique_function<bool(void) const>& barrier)
  {
    reset();
    ...
    LCWorker* worker)
{
  ...
  do {
    ...
    if (ret == -EBUSY || ret == -EEXIST) {
      ...
      ldpp_dout(this, 0) << "RGWLC::bucket_lc_post() failed to acquire lock on "
                         << obj_names[index] << ", sleep 5, try again "
                         << dendl;
      sleep(5);
      continue;
    }
  ...
  clean:
    ...
    ldpp_dout(this, 20) << "RGWLC::bucket_lc_post() unlock " << obj_names[index]
                        << dendl;
    return 0;
  } while (true);
} /* RGWLC::bucket_lc_post */
    ...
    int& index)
{
  ...
  for (; index < max_objs; index++, marker = "") {
    ...
    int ret = sal_lc->list_entries(
        this, null_yield, obj_names[index], marker, max_entries, entries);
    if (ret < 0) {
      if (ret == -ENOENT) {
        ldpp_dout(this, 10)
            << __func__ << "() ignoring unfound lc object=" << obj_names[index]
            << dendl;
        continue;
      } else {
        return ret;
...
static inline int
get_lc_index(CephContext* cct, const std::string& shard_id)
{
  int max_objs =
      (cct->_conf->rgw_lc_max_objs > HASH_PRIME ? HASH_PRIME
                                                : cct->_conf->rgw_lc_max_objs);
  /* n.b. review hash algo */
  int index = ceph_str_hash_linux(shard_id.c_str(), shard_id.size()) %
              HASH_PRIME % max_objs;
  return index;
}

static inline void
get_lc_oid(CephContext* cct, const std::string& shard_id, string* oid)
{
  /* n.b. review hash algo */
...
static std::string
get_bucket_lc_key(const rgw_bucket& bucket)
{
...
bool
RGWLC::expired_session(time_t started, time_t lc_start_time)
{
  ...
  ldpp_dout(this, 16) << "RGWLC::expired_session" << " started: " << started
...
RGWLC::process_bucket(
    ...
    bool once = false)
{
  ldpp_dout(this, 5) << "RGWLC::process_bucket(): ENTER: " << "index: " << index
                     << " worker ix: " << worker->ix << dendl;

  int ret = 0;
  std::unique_ptr<rgw::sal::LCSerializer> serializer = sal_lc->get_serializer(
      lc_index_lock_name, obj_names[index], worker->thr_name());
  if (max_lock_secs <= 0) {
    return -EAGAIN;
  }
  ...
  rgw::sal::LCEntry entry;
  ret = sal_lc->get_entry(
      this, null_yield, obj_names[index], bucket_entry_marker, entry);
  if (ret >= 0) {
    if (entry.status == lc_processing) {
      if (expired_session(entry.start_time, worker->lc_start_time)) {
        ldpp_dout(this, 5)
            << "RGWLC::process_bucket(): STALE lc session found for: " << entry
            << " index: " << index << " worker ix: " << worker->ix
            << " (clearing)" << dendl;
      } else {
        ldpp_dout(this, 5) << "RGWLC::process_bucket(): ACTIVE entry: " << entry
                           << " index: " << index
                           << " worker ix: " << worker->ix << dendl;
        return ret;
      }
...
static inline bool
allow_shard_rollover(CephContext* cct, time_t now, time_t shard_rollover_date)
{
  /* return true iff:
...
static inline bool
already_run_today(CephContext* cct, time_t start_date)
{
  struct tm bdt;
  time_t begin_of_day;
    ...
    time_t start_date)
{
  ...
  ret = sal_lc->get_next_entry(
      this, null_yield, lc_shard, entry.bucket, next_entry);
  if (ret < 0) {
  ...
  if (ret < 0) {
    ldpp_dout(this, 0) << "RGWLC::process() failed to put head " << lc_shard
                       << dendl;
    goto exit;
  }
exit:
    ...
    int worker_ix)
{
  ...
  if (head.marker.empty()) {
    ...
    ldpp_dout(this, 5)
        ...
        << lc_shard << " worker=" << worker_ix << dendl;
    ...
    int worker_ix)
{
  ...
  if (ret < 0) {
    ldpp_dout(this, 0)
        << "RGWLC::update_head() failed to check if shard is done " << lc_shard
        << dendl;
  }
...
int
RGWLC::process(int index, int max_lock_secs, LCWorker* worker, bool once = false)
{
  int ret{0};
  ...
  ldpp_dout(this, 5) << "RGWLC::process(): ENTER: " << "index: " << index
                     << " worker ix: " << worker->ix << dendl;
  ...
  do {
	...
	allow_shard_rollover(cct, now, head.shard_rollover_date) /* prevent multiple passes by diff.
      ...
      ldpp_dout(this, 5) << "RGWLC::process() process shard rollover lc_shard="
                         << lc_shard << " head.marker=" << head.marker
                         << " head.shard_rollover_date="
                         << head.shard_rollover_date << dendl;

      vector<rgw::sal::LCEntry> entries;
      int ret = sal_lc->list_entries(
          this, null_yield, lc_shard, head.marker, 1, entries);
      if (ret < 0) {
        ldpp_dout(this, 0) << "RGWLC::process() sal_lc->list_entries(lc_shard, "
                              "head.marker, 1, "
      ...
      ldpp_dout(this, 0)
          << "RGWLC::process() head.marker !empty() at START for shard=="
          << lc_shard << " head last stored at "
          << rgw_to_asctime(utime_t(head.start_date, 0)) << dendl;

      /* fetches the entry pointed to by head.bucket */
      ret = sal_lc->get_entry(this, null_yield, lc_shard, head.marker, entry);
      if (ret == -ENOENT) {
        /* skip to next entry */
      ...
      if (ret < 0) {
        ldpp_dout(this, 0) << "RGWLC::process() sal_lc->get_entry(lc_shard, "
                              "head.marker, entry) "
    ...
    if (!entry.bucket.empty()) {
      if (entry.status == lc_processing) {
        ...
        if ((entry.status == lc_complete) &&
          ...
          ldpp_dout(this, 5) << "RGWLC::process() worker ix: " << worker->ix
                             << " SKIP processing for already-processed bucket "
                             << entry.bucket << dendl;
          /* skip to next entry */
          if (update_head(lc_shard, head, entry, now, worker->ix) != 0) {
            break;
      ...
      ldpp_dout(this, 5)
          << "RGWLC::process() entry.bucket.empty() == true at START 1"
    ...
    if (ret == -ENOENT) {
      ...
      if (ret < 0) {
        ldpp_dout(this, 0)
            << "RGWLC::process() failed to set entry on lc_shard=" << lc_shard
            << " entry=" << entry << dendl;
        /* fatal, locked */
        break;
      }
...
unsigned
RGWLC::get_subsys() const
{
  return dout_subsys;
}

std::ostream&
RGWLC::gen_prefix(std::ostream& out) const
{
...
bool
RGWLC::LCWorker::should_work(utime_t& now)
{
  int start_hour = 0;
  int start_minute = 0;
  int end_hour = 23;
  ...
  sscanf(
      worktime.c_str(), "%d:%d-%d:%d", &start_hour, &start_minute, &end_hour,
      &end_minute);
  struct tm bdt;
  time_t tt = now.sec();
  localtime_r(&tt, &bdt);
  ...
  if (end_hour < start_hour) {
    bdt.tm_hour = bdt.tm_hour > end_hour ? bdt.tm_hour
                                         : bdt.tm_hour + hours_in_a_day;
  ...
  if (cct->_conf->rgw_lc_debug_interval > 0) {
    ...
    return true;
      ...
      (bdt.tm_hour * 60 + bdt.tm_min >= start_hour * 60 + start_minute) &&
...
int
RGWLC::LCWorker::schedule_next_start_time(utime_t& start, utime_t& now)
{
  int secs;
...
RGWLC::LCWorker::~LCWorker() {} /* ~LCWorker */

list<RGWLifecycleConfiguration>
RGWLifecycleConfiguration::generate_test_instances()
{
    ...
    const F& f)
{
  ...
  do {
    ...
    if (ret < 0) {
      ldpp_dout(dpp, 0) << "RGWLC::RGWPutLC() failed to set entry on " << oid
                        << ", ret=" << ret << dendl;
    }
    break;
  } while (true);
    ...
    RGWLifecycleConfiguration* config)
{
  ...
  ret = guard_lc_modify(
      dpp, driver, sal_lc.get(), b, cookie,
    ...
    rgw::sal::Bucket* bucket)
{
  ...
  if (ret == -ENOENT) {
    ldpp_dout(dpp, 1) << "No entry for bucket=" << bucket << " creating "
                      << dendl;
    ...
    ret = guard_lc_modify(
        dpp, driver, sal_lc, bucket->get_key(), cookie,
        [dpp, &lc_oid](
            rgw::sal::Lifecycle* slc, const string& oid,
            rgw::sal::LCEntry& entry) {
    ...
    const std::map<std::string, buffer::list>& bucket_attrs)
{
  ...
  try {
    ...
    ldpp_dout(dpp, 0) << __func__ << "() decode life cycle config failed"
                      << dendl;
    return hdr;
  } /* catch */
  ...
  if (cct->_conf->subsys.should_gather(ceph_subsys_rgw, 16)) {
    for (const auto& elt : obj_tag_map) {
      ldpp_dout(dpp, 16) << __func__ << "() key=" << elt.first
                         << " val=" << elt.second << dendl;
    }
  }
  ...
  for (const auto& ri : rule_map) {
    ...
    auto& prefix = filter.has_prefix() ? filter.get_prefix()
                                       : rule.get_prefix();
    auto& expiration = rule.get_expiration();
    auto& noncur_expiration = rule.get_noncur_expiration();

    ldpp_dout(dpp, 10) << "rule: " << ri.first << " prefix: " << prefix
                       << " expiration: " << " date: " << expiration.get_date()
                       << " days: " << expiration.get_days()
                       << " noncur_expiration: " << " date: "
                       << noncur_expiration.get_date()
                       << " days: " << noncur_expiration.get_days() << dendl;
    ...
    const LCExpiration& rule_expiration = (obj_key.instance.empty())
                                              ? expiration
                                              : noncur_expiration;
    ...
    if (rule_expiration.has_date()) {
      ...
      if (rule_expiration.has_days()) {
        rule_expiration_date = boost::optional<ceph::real_time>(
            mtime + make_timespan(
                        double(rule_expiration.get_days()) * secs_in_a_day -
                        ceph::real_clock::to_time_t(mtime) % (secs_in_a_day) +
                        secs_in_a_day));
      }
    }
  ...
  if (expiration_date && rule_id) {
    ...
    hdr = fmt::format("expiry-date=\"{0}\", rule-id=\"{1}\"", exp_str, *rule_id);
  }
    ...
    std::string& rule_id)
{
  ...
  for (const auto& ri : rule_map) {
    ...
    const auto& filter = rule.get_filter();
    const auto& prefix = filter.has_prefix() ? filter.get_prefix()
                                             : rule.get_prefix();
    ...
    if (mp_expiration.has_days()) {
      rule_abort_date = std::optional<ceph::real_time>(
          mtime + make_timespan(
                      mp_expiration.get_days() * secs_in_a_day -
                      ceph::real_clock::to_time_t(mtime) % (secs_in_a_day) +
                      secs_in_a_day));
    }
...
void
LCFilter::dump(Formatter* f) const
{
...
void
LCExpiration::dump(Formatter* f) const
{
...
void
RGWLifecycleConfiguration::dump(Formatter* f) const
{
