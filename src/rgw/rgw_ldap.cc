...
std::string
parse_rgw_ldap_bindpw(CephContext* ctx)
{
  string ldap_bindpw;
...
int
LDAPHelper::auth(const std::string& uid, const std::string& pwd)
{
  int ret;
  std::string filter;
  if (msad) {
    ...
    if (searchfilter.empty()) {
      ...
      if (searchfilter.find("@USERNAME@") != std::string::npos) {
        ...
        filter = searchfilter;
        filter.replace(
            searchfilter.find("@USERNAME@"), std::string("@USERNAME@").length(),
            uid);
      } else {
  ...
  ldout(g_ceph_context, 12)
      << __func__ << " search filter: " << filter << dendl;
...
retry_bind:
  ret = ldap_search_s(
      ldap, searchdn.c_str(), LDAP_SCOPE_SUBTREE, filter.c_str(), attrs, 0,
      &answer);
  if (ret == LDAP_SUCCESS) {
    ...
    if (entry) {
      ...
      ldout(g_ceph_context, 12)
          << __func__ << " ldap_search_s no user matching uid=" << uid << dendl;
      ret = LDAP_NO_SUCH_ATTRIBUTE; // fixup result
    }
    ldap_msgfree(answer);
  } else {
    ldout(g_ceph_context, 5) << __func__ << " ldap_search_s error uid=" << uid
                             << " ldap err=" << ret << dendl;
