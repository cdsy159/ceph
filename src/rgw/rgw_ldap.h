...
public:
  ...
  LDAPHelper(
      ...
      std::string _binddn,
      std::string _bindpw,
      const std::string& _searchdn,
      const std::string& _searchfilter,
      const std::string& _dnattr) :
    uri(std::move(_uri)),
  ...
  int
  simple_bind(const char* dn, const std::string& pwd)
  {
    LDAP* tldap;
...
public:
  ...
  int
  auth(const std::string& uid, const std::string& pwd)
  {
    return -EACCES;
  }
