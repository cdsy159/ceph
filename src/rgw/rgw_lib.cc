...
RGWLibProcess::run()
{
  ...
  while (!shutdown) {
    ...
    auto delay_s =
        std::max(int64_t(1), std::min(int64_t(MIN_EXPIRE_S), expire_s / 2));
...
void
RGWLibProcess::handle_request(const DoutPrefixProvider* dpp, RGWRequest* r)
{
  /*
...
int
RGWLibProcess::process_request(RGWLibRequest* req)
{
  // XXX move RGWLibIO and timing setup into process_request
...
static inline void
abort_req(req_state* s, RGWOp* op, int err_no)
{
  if (!s)
    return;
...
int
RGWLibProcess::process_request(RGWLibRequest* req, RGWLibIO* io)
{
  int ret = 0;
  bool should_log = true; // XXX

  dout(1) << "====== " << __func__ << " starting new request req=" << hex << req
          << dec << " ======" << dendl;
  ...
  try {
    ...
    if (ret < 0) {
      if (s->auth.identity->is_admin()) {
        ldpp_dout(op, 2) << "overriding permissions due to admin operation"
                         << dendl;
      } else {
        abort_req(s, op, ret);
        goto done;
  ...
  ldpp_dout(op, 1) << "====== " << __func__ << " req done req=" << hex << req
                   << dec << " http_status=" << http_ret << " ======" << dendl;

  return (ret < 0 ? ret : s->err.ret);
} /* process_request */

int
RGWLibProcess::start_request(RGWLibContinuedReq* req)
{
...
int
RGWLibProcess::finish_request(RGWLibContinuedReq* req)
{
  RGWOp* op = (req->op) ? req->op : dynamic_cast<RGWOp*>(req);
  if (!op) {
    ldpp_dout(op, 1) << "failed to derive cognate RGWOp (invalid op?)" << dendl;
...
RGWLibFrontend::init()
{
  ...
  pprocess = new RGWLibProcess(
      g_ceph_context, env, g_conf()->rgw_thread_pool_size, uri_prefix, conf);
  return 0;
}

void
RGWLib::set_fe(rgw::RGWLibFrontend* fe)
{
  this->fe = fe;
}
...
int
RGWLib::init(vector<const char*>& args)
{
  int r{0};
...
int
RGWLibIO::set_uid(rgw::sal::Driver* driver, const rgw_user& uid)
{
...
int
RGWLibRequest::read_permissions(RGWOp* op, optional_yield y)
{
  ...
  if (ret < 0) {
    ldpp_dout(op, 10) << "read_permissions (bucket policy) on "
                      << get_state()->bucket << ":" << get_state()->object
    ...
    /* object ops */
    ret = rgw_build_object_policies(
        op, g_rgwlib->get_driver(), get_state(), op->prefetch_data(), y);
    if (ret < 0) {
...
int
RGWHandler_Lib::authorize(const DoutPrefixProvider* dpp, optional_yield y)
{
  /* TODO: handle
     *  1. subusers
     *  2. anonymous access
