...
private:
  std::unique_ptr<rgw::sal::User>
      tuser; // Don't use this.  It's empty except during init.
...
public:
    ...
    rstate(_cct, penv, &io_ctx.get_env(), id)
  {
    ...
    ldpp_dout(get_state(), 2)
        << "initializing for trans_id = " << get_state()->trans_id.c_str()
        << dendl;
  }

  inline rgw::sal::Driver*
  get_driver()
  {
    return driver;
  }
