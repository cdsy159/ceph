...
struct RGWTable : EmptyMetaTable {
  ...
  NewIndexClosure(lua_State* L)
  {
    ...
    if (strcasecmp(index, INCREMENT) == 0 || strcasecmp(index, DECREMENT) == 0) {
      return luaL_error(
          L, "increment/decrement are reserved function names for RGW");
    }
...
private:
  ...
  std::map<std::string, std::unique_ptr<std::vector<char>>>
      lua_bytecode_cache; // script-name -> bytecode
  //  bool updating = false;
...
public:
  ...
  template <typename T>
  void
  put_table_value(const std::string& key, T value)
  {
