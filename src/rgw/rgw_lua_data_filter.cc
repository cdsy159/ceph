...
void
push_bufferlist_byte(lua_State* L, bufferlist::iterator& it)
{
  char byte[1];
  it.copy(1, byte);
...
struct BufferlistMetaTable : public EmptyMetaTable {
  ...
  IndexClosure(lua_State* L)
  {
    ...
    auto bl = reinterpret_cast<bufferlist*>(
  ...
  {
    const auto bl =
        reinterpret_cast<bufferlist*>(lua_touserdata(L, lua_upvalueindex(1)));
...
int
RGWObjFilter::execute(bufferlist& bl, off_t offset, const char* op_name) const
{
...
int
RGWGetObjFilter::handle_data(bufferlist& bl, off_t bl_ofs, off_t bl_len)
{
...
int
RGWPutObjFilter::process(bufferlist&& data, uint64_t logical_offset)
{
