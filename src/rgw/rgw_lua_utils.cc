...
void
create_debug_action(lua_State* L, CephContext* cct)
{
...
void
set_package_path(lua_State* L, const std::string& install_dir)
{
  ...
  const auto cpath = install_dir + "/lib/lua/" + CEPH_LUA_VERSION + "/?.so;" +
                     install_dir + "/lib64/lua/" + CEPH_LUA_VERSION + "/?.so";
  pushstring(L, cpath);
  lua_setfield(L, -2, "cpath");
}

void
open_standard_libs(lua_State* L)
{
  luaL_openlibs(L);
  unsetglobal(L, "load");
...
void*
allocator(void* ud, void* ptr, std::size_t osize, std::size_t nsize)
{
...
lua_State*
newstate(lua_state_guard* guard)
{
...
lua_state_guard::~lua_state_guard()
{
  ...
  if (mem_in_use > 0) {
    ldpp_dout(dpp, 20) << "Lua is using: " << mem_in_use << " bytes ("
                       << std::to_string(
                              100.0 - (100.0 * mem_in_use / max_memory))
                       << "%)" << dendl;
  }
...
void
lua_state_guard::set_mem_in_use(std::size_t _mem_in_use)
{
  mem_in_use = _mem_in_use;
}

void
lua_state_guard::runtime_hook(lua_State* L, lua_Debug* ar)
{
  ...
  if (elapsed > max_runtime) {
    ...
    constexpr size_t max_digits =
        std::numeric_limits<decltype(elapsed)::rep>::digits10 + 2;
    char elapsed_str[max_digits] = {};
    auto [ptr, ec] = std::to_chars(
        std::begin(elapsed_str), std::end(elapsed_str), elapsed.count());
    ...
    *ptr = '\0';
    ...
    luaL_error(
        L, "Lua runtime limit exceeded: total elapsed time is %s ms",
        elapsed_str);
  }
}

void
lua_state_guard::set_runtime_hook()
{
  lua_pushlightuserdata(
      state, const_cast<std::chrono::milliseconds*>(&max_runtime));
  lua_setfield(state, LUA_REGISTRYINDEX, max_runtime_key);
  lua_pushlightuserdata(
      state, const_cast<ceph::real_clock::time_point*>(&start_time));
...
int
lua_execute(lua_State* L, const DoutPrefixProvider* dpp, const LuaCodeType& code)
{
      ...
      overloads{
          [L, dpp](const std::string& script) -> int {
            if (luaL_dostring(L, script.c_str()) != LUA_OK) {
              ...
              ldpp_dout(dpp, 1)
                  << "Lua ERROR: failed to execute script : " << err << dendl;
              lua_pop(L, 1);
              return -1;
            }
            return 0;
          ...
          [L, dpp](const std::vector<char>& bytecode) -> int {
            if (luaL_loadbuffer(
                    L, bytecode.data(), bytecode.size(), "bytecode_chunk") !=
                LUA_OK) {
              const std::string err(lua_tostring(L, -1));
              ldpp_dout(dpp, 1)
                  << "Lua ERROR: failed to load buffer for bytecode : " << err
                  << dendl;
              lua_pop(L, 1);
              return -1;
            }
            if (lua_pcall(L, 0, 0, 0) != LUA_OK) {
              const std::string err(lua_tostring(L, -1));
              ldpp_dout(dpp, 1)
                  << "Lua ERROR: failed to execute bytecode : " << err << dendl;
              lua_pop(L, 1);
              return -1;
            }
            return 0;
