...
const char*
pushtime(lua_State* L, const CephTime& tp)
{
...
pushstring(lua_State* L, std::string_view str)
{
  return lua_pushlstring(L, str.data(), str.size());
}

inline void
pushvalue(lua_State* L, const std::string& value)
{
  pushstring(L, value);
}

inline void
pushvalue(lua_State* L, long long value)
{
  lua_pushinteger(L, value);
}

inline void
pushvalue(lua_State* L, double value)
{
  lua_pushnumber(L, value);
}

inline void
pushvalue(lua_State* L, bool value)
{
  lua_pushboolean(L, value);
}

inline void
unsetglobal(lua_State* L, const char* name)
{
  lua_pushnil(L);
...
public:
  ...
  std::size_t
  get_max_memory() const
  {
    return max_memory;
  }

  std::size_t
  get_mem_in_use() const
  {
    return mem_in_use;
  }
    ...
    Upvalues... upvalues)
{
  const std::string qualified_name(fmt::format(
      "{}{}{}", parent_name, (parent_name.empty() ? "" : "."), field_name));
...
create_metatable(
    ...
    bool toplevel,
    std::unique_ptr<typename MetaTable::Type>& ptr)
{
  if (ptr) {
    create_metatable<MetaTable>(
        L, parent_name, field_name, toplevel,
...
struct EmptyMetaTable {
  ...
  {
    return luaL_error(
        L, "unknown field name: %s provided to: %s", index.c_str(),
        table.c_str());
  }
};
...
inline std::string
get_iterator_name(const std::string_view name)
{
...
inline const char*
table_name_upvalue(lua_State* L)
{
  const auto name = reinterpret_cast<const char*>(
      lua_tostring(L, lua_upvalueindex(FIRST_UPVAL)));
  ceph_assert(name);
  return name;
}
...
create_iterator_metadata(
    lua_State* L,
    const std::string_view name,
    const typename MapType::iterator& start_it,
    const typename MapType::iterator& end_it)
{
  ...
  const auto metatable_is_new =
      luaL_newmetatable(L, get_iterator_name(name).c_str());
  ...
  if (!metatable_is_new) {
    ...
    auto old_it =
        reinterpret_cast<typename MapType::iterator*>(lua_touserdata(L, -1));
    // verify we are not mid-iteration
    if (*old_it != end_it) {
      luaL_error(
          L, "Trying to iterate '%s' before previous iteration finished",
          name.data());
      return nullptr;
    }
  ...
  lua_pushcclosure(
      ...
      [](lua_State* L) {
        ...
        static_assert(
            std::is_constructible<typename MapType::key_type, const char*>());
...
update_erased_iterator(
    lua_State* L,
    const std::string_view name,
    const typename MapType::iterator& old_it,
    const typename MapType::iterator& new_it)
{
...
int
StringMapWriteableNewIndex(lua_State* L)
{
  static_assert(
      std::is_constructible<typename MapType::key_type, const char*>());
  static_assert(
      std::is_constructible<typename MapType::mapped_type, const char*>());
  const auto name = table_name_upvalue(L);
  const auto map = reinterpret_cast<MapType*>(
...
next(lua_State* L)
{
  using Iterator = typename MapType::iterator;
  const auto name = table_name_upvalue(L);
  auto map = reinterpret_cast<MapType*>(
      lua_touserdata(L, lua_upvalueindex(SECOND_UPVAL)));
  ceph_assert(map);
  ...
  if (lua_isnil(L, 2)) {
    ...
    // create userdata
    next_it =
        create_iterator_metadata<MapType>(L, name, map->begin(), map->end());
  } else {
  ...
  if constexpr (std::is_constructible<std::string, ValueType>()) {
    ...
    // as a metatable
    create_metatable<ValueMetaType>(L, name, (*next_it)->first, false, &(value));
  }
  // return key, value
  return TWO_RETURNVALS;
}
