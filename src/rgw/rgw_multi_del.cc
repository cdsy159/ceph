...
bool
RGWMultiDelObject::xml_end(const char* el)
{
  ...
  XMLObj* last_modified_time =
      static_cast<XMLObj*>(find_first("LastModifiedTime"));
...
bool
RGWMultiDelDelete::xml_end(const char* el)
{
  RGWMultiDelQuiet* quiet_set =
      static_cast<RGWMultiDelQuiet*>(find_first("Quiet"));
  if (quiet_set) {
