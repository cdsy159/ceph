...
public:
  ...
  RGWObjectRetention(std::string _mode, ceph::real_time _date) :
    mode(_mode), retain_until_date(_date)
  {}

  void
  set_mode(std::string _mode)
  {
    mode = _mode;
  }
  ...
  void
  set_retain_until_date(ceph::real_time _retain_until_date)
  {
    retain_until_date = _retain_until_date;
  }

  ceph::real_time
  get_retain_until_date() const
  {
