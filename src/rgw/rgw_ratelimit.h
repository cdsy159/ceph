...
class RateLimiterEntry {
  ...
  */
  ...
  bool
  should_rate_limit_read(int64_t ops_limit, int64_t bw_limit)
  {
  ...
  bool
  should_rate_limit_list(int64_t ops_limit)
  {
  ...
  bool
  should_rate_limit_delete(int64_t ops_limit)
  {
  ...
  bool
  should_rate_limit_write(int64_t ops_limit, int64_t bw_limit)
  {
    //check if tenants did not reach their bw or ops limits and that the limits are not 0 (which is unlimited)
  ...
  */
  bool
  minimum_time_reached(ceph::timespan curr_timestamp) const
  {
    using namespace std::chrono;
    const size_t accumulation_interval =
        g_ceph_context->_conf->rgw_ratelimit_interval;
    const auto min_duration =
        duration_cast<ceph::timespan>(seconds(accumulation_interval)) /
        fixed_point_rgw_ratelimit;
  ...
  void
  increase_tokens(ceph::timespan curr_timestamp, const RGWRateLimitInfo* info)
  {
    ...
    if (first_run) {
      ...
      const int64_t time_in_ms =
          std::chrono::duration_cast<std::chrono::milliseconds>(
              curr_timestamp - ts)
              .count() /
          static_cast<double>(accumulation_interval) / std::milli::den *
          fixed_point;
      ts = curr_timestamp;
      ...
      read.bytes =
          std::min(info->max_read_bytes * fixed_point, read_bw + read.bytes);
      write.ops =
          std::min(info->max_write_ops * fixed_point, write_ops + write.ops);
      write.bytes =
          std::min(info->max_write_bytes * fixed_point, write_bw + write.bytes);
      list.ops = std::min(info->max_list_ops * fixed_point, list_ops + list.ops);
      del.ops =
          std::min(info->max_delete_ops * fixed_point, delete_ops + del.ops);
    }
  }
...
public:
      ...
      ceph::timespan curr_timestamp)
  {
    ...
    switch (op_type) {
    case OpType::Read:
      return should_rate_limit_read(
          ratelimit_info->max_read_ops, ratelimit_info->max_read_bytes);
    case OpType::Write:
      return should_rate_limit_write(
          ratelimit_info->max_write_ops, ratelimit_info->max_write_bytes);
    case OpType::List:
  ...
  void
  decrease_bytes(bool is_read, int64_t amount, const RGWRateLimitInfo* info)
  {
    std::unique_lock lock(ts_lock);
    // we don't want the tenant to be with higher debt than 120 seconds(2 min) of its limit
    if (is_read) {
      read.bytes = std::max(
          read.bytes - amount * fixed_point_rgw_ratelimit,
          info->max_read_bytes * fixed_point_rgw_ratelimit * -2);
    } else {
      write.bytes = std::max(
          write.bytes - amount * fixed_point_rgw_ratelimit,
          info->max_write_bytes * fixed_point_rgw_ratelimit * -2);
    }
  }

  void
  giveback_tokens(OpType op_type)
  {
    std::unique_lock lock(ts_lock);
    switch (op_type) {
    case OpType::Read:
...
class RateLimiter : public DoutPrefix {
  ...
  static constexpr size_t map_size =
      2000000; // will create it with the closest upper prime number
  ...
  static inline constexpr std::string_view RESOURCE_PATTERN_LIST_TYPE =
      "list-type=";
  static inline constexpr std::string_view RESOURCE_PATTERN_PREFIX = "prefix=";
  static inline constexpr std::string_view RESOURCE_PATTERN_DELIMITER =
      "delimiter=";
      ...
      const RGWRateLimitInfo* ratelimit_info)
  {
    const bool ratelimit_list = ratelimit_info &&
                                (ratelimit_info->max_list_ops > 0);
    const bool ratelimit_delete = ratelimit_info &&
                                  (ratelimit_info->max_delete_ops > 0);

    auto contains_any = [](std::string_view s, auto&&... patterns) {
      return ((s.find(patterns) != std::string::npos) || ...);
    };
    if (ratelimit_list && method == "GET" && !resource.empty() &&
        contains_any(
            resource, RESOURCE_PATTERN_LIST_TYPE, RESOURCE_PATTERN_PREFIX,
            RESOURCE_PATTERN_DELIMITER)) {
      return OpType::List;
    }
  ...
  auto&
  find_or_create(const std::string& key)
  {
...
public:
  ...
  {
    ldpp_dout(this, 21) << "checking should_rate_limit: key="
                        << std::quoted(key)
...
class ActiveRateLimiter : public DoutPrefix {
  ...
  replace_active()
  {
    ...
    while (!stopped) {
      ...
      ldpp_dout(this, 20) << "replacing active ratelimit data structure"
                          << dendl;
      ...
      ldpp_dout(this, 20) << "clearing passive ratelimit data structure"
                          << dendl;
