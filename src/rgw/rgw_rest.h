...
extern void rgw_flush_formatter_and_reset(
    req_state* s,
...
rgw_sanitized_hdrval(ceph::buffer::list& raw)
{
...
rgw_rest_get_json_input_keep_data(
    CephContext* cct,
    req_state* s,
    T& out,
    uint64_t max_len)
...
public:
  ...
  RGWRESTFlusher() :
    RGWFormatterFlusher(NULL), s(NULL), op(NULL)
  {}

  void
  init(req_state* _s, RGWOp* _op)
  {
    s = _s;
    op = _op;
...
protected:
  ...
  static int parse_part_field(
      const std::string& line,
      std::string& field_name, /* out */
      post_part_field& field); /* out */

  static void parse_boundary_params(
      const std::string& params_str,
      std::string& first,
      std::map<std::string, std::string>& params);

  static bool part_str(
      parts_collection_t& parts,
      const std::string& name,
      std::string* val);

  static std::string get_part_str(
      parts_collection_t& parts,
      const std::string& name,
...
public:
  ...
  /* not actually used */
  int
  send_response_data_error(optional_yield y) override
  {
    return 0;
  };

  int
  send_response_data(bufferlist& bl, off_t ofs, off_t len) override
  {
    return 0;
  };
};
...
class RGWGetBucketOwnershipControls_ObjStore
  : public RGWGetBucketOwnershipControls {};

class RGWPutBucketOwnershipControls_ObjStore
  : public RGWPutBucketOwnershipControls {};

class RGWDeleteBucketOwnershipControls_ObjStore
  : public RGWDeleteBucketOwnershipControls {};
...
public:
  ...
  virtual int
  check_caps(const RGWUserCaps& caps)
  {
    return -EPERM;
...
protected:
  ...
  virtual RGWRESTMgr* get_resource_mgr(
      req_state* s,
...
public:
  RGWRESTMgr() :
    should_log(false), default_mgr(nullptr)
  {}
  ...
  virtual void
  put_handler(RGWHandler_REST* const handler)
  {
    delete handler;
  }

  void
  set_logging(bool _should_log)
  {
...
public:
  ...
  void
  put_handler(RGWHandler_REST* handler)
  {
  ...
  void
  register_default_mgr(RGWRESTMgr* m)
  {
  ...
  bool
  log_x_header(const std::string& header)
  {
    ...
    Args&&... args)
{
  ...
  const auto len = snprintf(
      full_name_buf, sizeof(full_name_buf), "%.*s%.*s%.*s",
    ...
    const std::string_view& val)
{
  ...
  const auto len = snprintf(
      qvalbuf, sizeof(qvalbuf), "\"%.*s\"", static_cast<int>(val.length()),
      val.data());
...
parse_content_length(const char* content_length)
{
  int64_t len = -1;
...
inline std::string
compute_domain_uri(const req_state* s)
{
  std::string uri = (!s->info.domain.empty()) ? s->info.domain
                                              : [&s]() -> std::string {
    RGWEnv const& env(*(s->info.env));
    std::string uri = rgw_transport_is_secure(s->cct, env) ? "https://"
                                                           : "http://";
...
extern void abort_early(
    ...
    optional_yield y);
extern void
dump_range(req_state* s, uint64_t ofs, uint64_t end, uint64_t total_size);
...
extern void dump_access_control(
    req_state* s,
    const char* origin,
    const char* meth,
    const char* hdr,
