...
namespace {
int
verify_bucket_logging_params(const DoutPrefixProvider* dpp, const req_state* s)
{
  bool exists;
  ...
  if (no_value.length() > 0) {
    ldpp_dout(dpp, 1) << "ERROR: param 'logging' should not have any value"
                      << dendl;
    return -EINVAL;
  }
...
void
update_mtime_attribute(const DoutPrefixProvider* dpp, rgw::sal::Attrs& attrs)
{
  bufferlist mtime_bl;
...
public:
  ...
  execute(optional_yield y) override
  {
    ...
    if (auto iter = src_bucket->get_attrs().find(RGW_ATTR_BUCKET_LOGGING);
        iter != src_bucket->get_attrs().end()) {
      try {
        configuration.enabled = true;
        decode(configuration, iter->second);
        if (auto mtime_it =
                src_bucket->get_attrs().find(RGW_ATTR_BUCKET_LOGGING_MTIME);
          ...
          try {
            ...
            ldpp_dout(this, 5)
                << "WARNING: failed to decode logging mtime attribute '"
                << RGW_ATTR_BUCKET_LOGGING_MTIME << "' for bucket '"
                << src_bucket_id << "', error: " << err.what() << dendl;
          }
        } else {
          ldpp_dout(this, 5)
              << "WARNING: no logging mtime attribute '"
              << RGW_ATTR_BUCKET_LOGGING_MTIME << "' for bucket '"
              << src_bucket_id << "'" << dendl;
        }
      } catch (buffer::error& err) {
    ...
    ldpp_dout(this, 20) << "INFO: found logging configuration on bucket '"
                        << src_bucket_id << "'"
                        << "'. configuration: " << configuration.to_json_str()
                        << dendl;
  }
  ...
  {
    return fmt::format("REST.{}.LOGGING", s->info.method);
  }
...
class RGWPutBucketLoggingOp : public RGWDefaultResponseOp {
  ...
  init_processing(optional_yield y) override
  {
    ...
    if (std::tie(ret, data) = read_all_input(s, max_size, false); ret < 0) {
      ldpp_dout(this, 1) << "ERROR: failed to read XML logging payload, ret = "
                         << ret << dendl;
      return ret;
    }
    if (data.length() == 0) {
    ...
    if (!parser.parse(data.c_str(), data.length(), 1)) {
      ldpp_dout(this, 1) << "ERROR: failed to parse XML logging payload"
                         << dendl;
      return -ERR_MALFORMED_XML;
    }
    configuration.default_obj_roll_time =
        get_cct()->_conf->rgw_bucket_logging_obj_roll_time;
    try {
      RGWXMLDecoder::decode_xml(
          "BucketLoggingStatus", configuration, &parser, true);
    } catch (RGWXMLDecoder::err& err) {
      ldpp_dout(this, 1)
          << "ERROR: failed to parse XML logging payload. error: " << err
          << dendl;
      return -ERR_MALFORMED_XML;
    }
    ...
    if (const auto ret =
            driver->load_bucket(this, target_bucket_id, &target_bucket, y);
        ret < 0) {
      ldpp_dout(this, 1) << "ERROR: failed to get logging bucket '"
  ...
  verify_permission(optional_yield y) override
  {
    ...
    if (!s->auth.identity->is_owner_of(s->bucket->get_info().owner)) {
      ldpp_dout(this, 1) << "ERROR: user '"
                         << s->auth.identity->get_aclowner().id
                         << "' is not the owner of bucket '" << s->bucket_name
                         << "'. owner is '" << s->bucket->get_info().owner
                         << "'" << dendl;
      return -EACCES;
    }
    ...
    const auto target_resource_arn =
        rgw::ARN(target_bucket->get_key(), configuration.target_prefix);
    return rgw::bucketlogging::verify_target_bucket_policy(
        this, target_bucket.get(), target_resource_arn, s);
  }

  const char*
  name() const override
  {
    return "put_bucket_logging";
  }
  ...
  get_type() override
  {
    return RGW_OP_PUT_BUCKET_LOGGING;
  }
  ...
  execute(optional_yield y) override
  {
    ...
    if (target_bucket_id == src_bucket_id) {
      ...
      ldpp_dout(this, 1) << "ERROR: logging bucket '" << target_bucket_id
                         << "' must be different from source bucket" << dendl;
      op_ret = -EINVAL;
      return;
    }
    ...
    if (target_info.zonegroup != src_bucket->get_info().zonegroup) {
      ...
      ldpp_dout(this, 1) << "ERROR: logging bucket '" << target_bucket_id
                         ...
                         << "' is different from the source bucket '"
                         << src_bucket_id << "' zonegroup '"
                         << src_bucket->get_info().zonegroup << "'" << dendl;
      op_ret = -EINVAL;
      return;
    }

    // following checks need to also be done on every write to the log bucket
    if (const auto ret = rgw::bucketlogging::verify_target_bucket_attributes(
            this, target_bucket.get());
        ret < 0) {
      op_ret = ret;
      return;
    }
    ...
    op_ret = retry_raced_bucket_write(
        this, src_bucket.get(),
        [this, &conf_bl, &src_bucket, &old_conf, y] {
          ...
          if (it != attrs.end()) {
            try {
              ...
              ldpp_dout(this, 1)
                  ...
                  << RGW_ATTR_BUCKET_LOGGING << "' for bucket '"
                  << src_bucket->get_key() << "', error: " << err.what()
                  << dendl;
            }
        ...
        y);
    if (op_ret < 0) {
      ldpp_dout(this, 1) << "ERROR: failed to set logging attribute '"
                         << RGW_ATTR_BUCKET_LOGGING << "' to bucket '"
                         << src_bucket_id << "', ret = " << op_ret << dendl;
      return;
    }
    if (!old_conf) {
      ldpp_dout(this, 20) << "INFO: new logging configuration added to bucket '"
                          << src_bucket_id
                          << "'. configuration: " << configuration.to_json_str()
                          << dendl;
      if (const auto ret = rgw::bucketlogging::update_bucket_logging_sources(
              this, target_bucket, src_bucket_id, true, y);
          ret < 0) {
        ldpp_dout(this, 1) << "WARNING: failed to add source bucket '"
                           << src_bucket_id
                           << "' to logging sources of logging bucket '"
                           << target_bucket_id << "', ret = " << ret << dendl;
      }
      ...
      std::string obj_name;
      if (int ret = target_bucket->get_logging_object_name(
        ...
        ldpp_dout(this, 1)
            ...
            << configuration.target_prefix << "', ret = " << ret << dendl;
        op_ret = ret;
        return;
      }
      const auto region = driver->get_zone()->get_zonegroup().get_api_name();
      if (const auto ret = rollover_logging_object(
              *old_conf, target_bucket, obj_name, this, region, src_bucket, y,
              false, // rollover should happen even if commit failed
              &objv_tracker, false, &old_obj);
          ret < 0) {
        ldpp_dout(this, 1)
            << "WARNING: failed to flush pending logging object '" << obj_name
            << "'" << " to target bucket '" << target_bucket_id << "'. "
            << "' when updating logging configuration of bucket '"
            << src_bucket->get_key() << ". error: " << ret << dendl;
      } else {
        ldpp_dout(this, 20)
            << "INFO: flushed pending logging object '" << old_obj
            << "' to target bucket '" << target_bucket_id
            << "' when updating logging configuration of bucket '"
      ...
      if (old_conf->target_bucket != configuration.target_bucket) {
        ...
        if (const auto ret = rgw::bucketlogging::get_bucket_id(
            ...
            ret < 0) {
          ldpp_dout(this, 1)
              << "ERROR: failed to parse logging bucket '"
        ...
        if (const auto ret = rgw::bucketlogging::update_bucket_logging_sources(
            ...
            ret < 0) {
          ldpp_dout(this, 1)
              << "WARNING: failed to remove source bucket '" << src_bucket_id
              << "' from logging sources of original logging bucket '"
        ...
        if (const auto ret = rgw::bucketlogging::update_bucket_logging_sources(
            ...
            ret < 0) {
          ldpp_dout(this, 1)
              << "WARNING: failed to add source bucket '" << src_bucket_id
              << "' to logging sources of logging bucket '" << target_bucket_id
              << "', ret = " << ret << dendl;
        }
      }
  ...
  send_response() override
  {
    ...
    if (!old_obj.empty()) {
      ...
      s->formatter->open_object_section_in_ns(
          "PutBucketLoggingOutput", XMLNS_AWS_S3);
...
class RGWPostBucketLoggingOp : public RGWDefaultResponseOp {
  ...
  init_processing(optional_yield y) override
  {
...

    {
      const rgw_bucket src_bucket_id{s->bucket_tenant, s->bucket_name};
      if (const auto ret =
              driver->load_bucket(this, src_bucket_id, &source_bucket, y);
          ret < 0) {
        ldpp_dout(this, 1) << "ERROR: failed to get bucket '" << src_bucket_id
                           << "', ret = " << ret << dendl;
        return ret;
  ...
  get_type() override
  {
    return RGW_OP_POST_BUCKET_LOGGING;
  }
  ...
  execute(optional_yield y) override
  {
    ...
    if (const auto ret = rgw::bucketlogging::update_bucket_logging_sources(
            this, target_bucket, source_bucket->get_key(), true, y);
        ret < 0) {
      ldpp_dout(this, 5)
          << "WARNING: failed to update logging sources attribute '"
          << RGW_ATTR_BUCKET_LOGGING_SOURCES << "' in logging target '"
          << target_bucket_id << "'. error: " << ret << dendl;
    }
    std::string obj_name;
    RGWObjVersionTracker objv_tracker;
    ...
    if (op_ret < 0 && op_ret != -ENOENT) {
      ldpp_dout(this, 1) << "ERROR: failed to get pending logging object name "
                            "from logging bucket '"
                         << target_bucket_id << "'. error: " << op_ret << dendl;
      return;
    }
    if (op_ret == -ENOENT) {
      // no pending object - nothing to flush
      ldpp_dout(this, 5)
          << "INFO: no pending logging object in logging bucket '"
          << target_bucket_id << "'. new object should be created" << dendl;
    }
    const auto region = driver->get_zone()->get_zonegroup().get_api_name();
    op_ret = rgw::bucketlogging::rollover_logging_object(
        configuration, target_bucket, obj_name, this, region, source_bucket, y,
        true, &objv_tracker, false, &old_obj);
    if (op_ret < 0) {
      ldpp_dout(this, 1) << "ERROR: failed to flush pending logging object '"
                         << obj_name << "'" << " to logging bucket '"
                         << target_bucket_id << "'. "
  ...
  send_response() override
  {
    ...
    dump_start(s);
    s->formatter->open_object_section_in_ns(
        "PostBucketLoggingOutput", XMLNS_AWS_S3);
    s->formatter->dump_string("FlushedLoggingObject", old_obj);
    s->formatter->close_section();
    rgw_flush_formatter_and_reset(s, s->formatter);
  }
