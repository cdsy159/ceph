...
int
RGWHTTPSimpleRequest::get_status()
{
...
int
RGWHTTPSimpleRequest::handle_header(const string& name, const string& val)
{
...
int
RGWHTTPSimpleRequest::receive_header(void* ptr, size_t len)
{
...
static void
get_new_date_str(string& date_str)
{
...
int
RGWHTTPSimpleRequest::send_data(void* ptr, size_t len, bool* pause)
{
  if (!send_iter)
    return 0;
...
int
RGWHTTPSimpleRequest::receive_data(void* ptr, size_t len, bool* pause)
{
  size_t cp_len, left_len;

  left_len = max_response > response.length()
                 ? (max_response - response.length())
                 : 0;
  if (left_len == 0)
...
static void
append_param(string& dest, const string& name, const string& val)
{
  if (dest.empty()) {
    dest.append("?");
  } else {
...
void
RGWHTTPSimpleRequest::get_params_str(
    map<string, string>& extra_args,
    string& dest)
{
...
void
RGWHTTPSimpleRequest::get_out_headers(map<string, string>* pheaders)
{
  unique_lock guard(out_headers_lock);
    ...
    const bufferlist* opt_content)
{
  ...
  if (!rgw_create_s3_canonical_header(
          dpp, RGW_OP_UNKNOWN, info, NULL, canonical_header, false)) {
    ...
    const bufferlist* opt_content)
{
  ...
  for (auto& entry : sigv4_headers) {
    ldpp_dout(dpp, 20) << __func__ << "(): sigv4 header: " << entry.first
                       << ": " << entry.second << dendl;
...
auto
RGWRESTSimpleRequest::forward_request(
    const DoutPrefixProvider* dpp,
    const RGWAccessKey& key,
    const req_info& info,
    size_t max_response,
    ...
    bufferlist* outbl,
    optional_yield y,
    std::string service) -> tl::expected<int, int>
{
  ...
  if (std::string::npos != pos)
    request_uri_encode = string("/") + bucket_encode +
                         new_info.request_uri.substr(pos + 1);
  else
    request_uri_encode = string("/") + bucket_encode;
...
int
RGWRESTStreamOutCB::handle_data(bufferlist& bl, off_t bl_ofs, off_t bl_len)
{
...
static void
add_grants_headers(map<int, string>& grants, RGWEnv& env, meta_map_t& meta_map)
{
  struct grant_type_to_header* t;
...
RGWRESTGenerateHTTPHeaders::RGWRESTGenerateHTTPHeaders(
    CephContext* _cct,
    RGWEnv* _env,
    req_info* _info) :
...
void
RGWRESTGenerateHTTPHeaders::init(
    ...
    std::optional<string> api_name)
{
  ...
  for (auto iter = params.begin(); iter != params.end(); ++iter) {
    ...
    new_info->args.append(
        url_encode(iter->first, encode_slash),
...
static bool
is_x_amz(const string& s)
{
  return boost::algorithm::starts_with(s, "x-amz-");
}

void
RGWRESTGenerateHTTPHeaders::set_extra_headers(
...
int
RGWRESTGenerateHTTPHeaders::set_obj_attrs(
    const DoutPrefixProvider* dpp,
...
void
RGWRESTGenerateHTTPHeaders::set_http_attrs(const map<string, string>& http_attrs)
{
  /* merge send headers */
...
void
RGWRESTGenerateHTTPHeaders::set_policy(const RGWAccessControlPolicy& policy)
{
  /* update acl headers */
...
int
RGWRESTGenerateHTTPHeaders::sign(
    const DoutPrefixProvider* dpp,
    RGWAccessKey& key,
    const bufferlist* opt_content)
{
...
void
RGWRESTStreamS3PutObj::send_init(const rgw_obj& obj)
{
  string resource_str;
  string resource;
  string new_url = url;
  ...
  ldpp_dout(this, 20) << __func__ << "(): host = " << host
                      << " , resource = " << resource
                      << " , new_host = " << new_host
                      << " , new_url = " << new_url << dendl;

  method = "PUT";
  headers_gen.init(
      method, new_host, resource_prefix, new_url, resource, params, api_name);

  url = headers_gen.get_url();
}

void
RGWRESTStreamS3PutObj::send_ready(
    const DoutPrefixProvider* dpp,
    RGWAccessKey& key,
    map<string, bufferlist>& rgw_attrs)
{
...
void
RGWRESTStreamS3PutObj::put_obj_init(
    const DoutPrefixProvider* dpp,
    RGWAccessKey& key,
    const rgw_obj& obj,
...
static void
send_prepare_convert(const rgw_obj& obj, string* resource)
{
...
int
RGWRESTStreamRWRequest::send_prepare(
    const DoutPrefixProvider* dpp,
    RGWAccessKey& key,
    map<string, string>& extra_headers,
    const rgw_obj& obj)
...
int
RGWRESTStreamRWRequest::do_send_prepare(
    const DoutPrefixProvider* dpp,
    RGWAccessKey* key,
    map<string, string>& extra_headers,
    const string& resource,
    bufferlist* send_data)
{
  ...
  headers_gen->init(
      method, new_host, resource_prefix, new_url, new_resource, params,
      api_name);
...
int
RGWRESTStreamRWRequest::send(RGWHTTPManager* mgr)
{
  if (!headers_gen) {
    ldpp_dout(this, 0) << "ERROR: " << __func__
                       << "(): send_prepare() was not called: likely a bug!"
                       << dendl;
    return -EINVAL;
  }
...
int
RGWHTTPStreamRWRequest::complete_request(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    string* etag,
    real_time* mtime,
    ...
    map<string, string>* pheaders)
{
  ...
  for (auto iter = out_headers.begin(); pattrs && iter != out_headers.end();
    ...
    if (attr_name.compare(
            0, sizeof(RGW_HTTP_RGWX_ATTR_PREFIX) - 1,
...
int
RGWHTTPStreamRWRequest::handle_headers(
...
int
RGWHTTPStreamRWRequest::handle_header(const string& name, const string& val)
{
...
int
RGWHTTPStreamRWRequest::receive_data(void* ptr, size_t len, bool* pause)
{
  size_t orig_len = len;
...
void
RGWHTTPStreamRWRequest::set_stream_write(bool s)
{
...
void
RGWHTTPStreamRWRequest::unpause_receive()
{
...
void
RGWHTTPStreamRWRequest::add_send_data(bufferlist& bl)
{
...
uint64_t
RGWHTTPStreamRWRequest::get_pending_send_size()
{
  std::lock_guard wl{write_lock};
  return outbl.length();
}
...
void
RGWHTTPStreamRWRequest::finish_write()
{
  std::scoped_lock locker{get_req_lock(), write_lock};
...
int
RGWHTTPStreamRWRequest::send_data(void* ptr, size_t len, bool* pause)
{
  uint64_t out_len;
  uint64_t send_size;
  {
...
int
RGWHTTPStreamRWRequest::send(RGWHTTPManager* mgr)
{
  if (!mgr) {
