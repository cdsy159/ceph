...
public:
  ...
  void
  set_headers(param_vec_t* _headers)
  {
    if (_headers)
      headers = *_headers;
  }
  ...
  void
  set_params(param_vec_t* _params)
  {
    if (_params)
      params = *_params;
  }
  ...
  void get_out_headers(
      std::map<std::string, std::string>* pheaders); /* modifies out_headers */
...
public:
  ...
  int set_obj_attrs(
      const DoutPrefixProvider* dpp,
      std::map<std::string, bufferlist>& rgw_attrs);
...
public:
  ...
  public:
    ...
    virtual void
    set_extra_data_len(uint64_t len)
    {
      extra_data_len = len;
    }
  };
  ...
  RGWHTTPStreamRWRequest(
      ...
      param_vec_t* _headers,
      param_vec_t* _params) :
    RGWHTTPSimpleRequest(_cct, _method, _url, _headers, _params), cb(_cb)
  {}

  virtual ~RGWHTTPStreamRWRequest() override {}

  void
  set_outbl(bufferlist& _outbl)
  {
    outbl.swap(_outbl);
  }

  void
  set_in_cb(ReceiveCB* _cb)
  {
    cb = _cb;
  }

  void
  set_write_drain_cb(RGWWriteDrainCB* _cb)
  {
    write_drain_cb = _cb;
  }
...
public:
  RGWRESTStreamRWRequest(
      ...
      const std::string& _method,
      const std::string& _url,
      RGWHTTPStreamRWRequest::ReceiveCB* _cb,
      param_vec_t* _headers,
      param_vec_t* _params,
      std::optional<std::string> _api_name,
...
public:
  ...
  void send_ready(
      const DoutPrefixProvider* dpp,
      RGWAccessKey& key,
      const std::map<std::string, std::string>& http_attrs,
