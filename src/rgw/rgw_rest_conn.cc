  ...
  host_style(_host_style)
{
  ...
  std::for_each(
      remote_endpoints.begin(), remote_endpoints.end(), [this](const auto& url) {
...
int
RGWRESTConn::get_url(string& endpoint)
{
  if (endpoints.empty()) {
...
void
RGWRESTConn::set_url_unconnectable(const std::string& endpoint)
{
  if (endpoint.empty() ||
...
RGWRESTConn::forward(
    ...
    bufferlist* inbl,
    bufferlist* outbl,
    optional_yield y) -> tl::expected<int, int>
{
  ...
  for (int tries = 0; tries < NUM_ENPOINT_IOERROR_RETRIES; tries++) {
    ...
    auto result =
        req.forward_request(dpp, key, info, max_response, inbl, outbl, y);
    if (result) {
      return result;
    ...
    optional_yield y) -> tl::expected<int, int>
{
  ...
  for (int tries = 0; tries < NUM_ENPOINT_IOERROR_RETRIES; tries++) {
    ...
    auto result = req.forward_request(
        dpp, key, info, max_response, inbl, outbl, y, service);
    if (result) {
      return result;
    } else if (result.error() != -EIO) {
      return result;
...
int
RGWRESTConn::put_obj_async_init(
    const DoutPrefixProvider* dpp,
    const rgw_owner& uid,
    const rgw_obj& obj,
...
template <class T>
static void
set_header(T val, map<string, string>& headers, const string& header_name)
{
  stringstream s;
  s << val;
  headers[header_name] = s.str();
...
RGWRESTConn::get_obj(
    const DoutPrefixProvider* dpp,
    const rgw_obj& obj,
    const get_obj_params& in_params,
    bool send,
    RGWRESTStreamRWRequest** req)
{
  string url;
  ...
  if (in_params.perm_check_uid) {
    params.push_back(param_pair_t(
        RGW_SYS_PARAM_PREFIX "perm-check-uid",
        to_string(*in_params.perm_check_uid)));
  }
  ...
  if (in_params.dst_zone_trace) {
    params.push_back(param_pair_t(
        RGW_SYS_PARAM_PREFIX "if-not-replicated-to",
  ...
  if (in_params.get_op) {
    *req = new RGWRESTStreamReadRequest(
        cct, url, in_params.cb, NULL, &params, api_name, host_style);
  } else {
    *req = new RGWRESTStreamHeadRequest(
        cct, url, in_params.cb, NULL, &params, api_name);
  }
  ...
  set_date_header(
      in_params.mod_ptr, extra_headers, in_params.high_precision_time,
      "HTTP_IF_MODIFIED_SINCE");
  set_date_header(
      in_params.unmod_ptr, extra_headers, in_params.high_precision_time,
  ...
  if (in_params.range_is_set) {
    char buf[64];
    snprintf(
        buf, sizeof(buf), "bytes=%lld-%lld", (long long)in_params.range_start,
        (long long)in_params.range_end);
    ...
    optional_yield y)
{
  ...
  for (int tries = 0; tries < NUM_ENPOINT_IOERROR_RETRIES; tries++) {
    ...
    RGWRESTStreamReadRequest req(
        cct, url, &cb, NULL, &params, api_name, host_style);
    ...
    optional_yield y)
{
  ...
  for (int tries = 0; tries < NUM_ENPOINT_IOERROR_RETRIES; tries++) {
    ...
    RGWRESTStreamSendRequest req(
        cct, method, url, &cb, NULL, &params, api_name, host_style);
...
RGWRESTReadResource::RGWRESTReadResource(
    RGWRESTConn* _conn,
...
void
RGWRESTReadResource::init_common(param_vec_t* extra_headers)
{
...
int
RGWRESTReadResource::read(const DoutPrefixProvider* dpp, optional_yield y)
{
...
int
RGWRESTReadResource::aio_read(const DoutPrefixProvider* dpp)
{
  int ret = req.send_request(dpp, &conn->get_key(), headers, resource, mgr);
  if (ret < 0) {
    ldpp_dout(dpp, 5) << __func__ << ": send_request() resource=" << resource
...
void
RGWRESTSendResource::init_common(param_vec_t* extra_headers)
{
  conn->populate_params(params, nullptr, conn->get_self_zonegroup());
...
int
RGWRESTSendResource::aio_send(const DoutPrefixProvider* dpp, bufferlist& outbl)
{
  req.set_send_length(outbl.length());
  req.set_outbl(outbl);
