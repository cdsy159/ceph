...
bool
verify_transport_security(CephContext* cct, const RGWEnv& env)
{
  const auto is_secure = rgw_transport_is_secure(cct, env);
  if (!is_secure &&
      g_conf().get_val<bool>("rgw_allow_notification_secrets_in_cleartext")) {
    ldout(cct, 0) << "WARNING: bypassing endpoint validation, allows sending "
                     "secrets over insecure transport"
                  << dendl;
    return true;
  }
  return is_secure;
...
bool
validate_and_update_endpoint_secret(
    ...
    std::string& message)
{
  ...
  if (topic_user_name.has_value()) {
    if (!user.empty()) {
      message =
          "Username provided via both topic attributes and endpoint URL: using "
          "topic attributes";
    }
  ...
  if (topic_password.has_value()) {
    if (!password.empty()) {
      message =
          "Password provided via both topic attributes and endpoint URL: using "
          "topic attributes";
    }
  ...
  if (!user.empty()) {
    ...
    if (!verify_transport_security(cct, *ri.env)) {
      message =
          "Topic contains secrets that must be transmitted over a secure "
          "transport";
      return false;
    }
  }
...
bool
validate_topic_name(const std::string& name, std::string& message)
{
  ...
  if (!std::regex_match(name, pattern)) {
    message =
        "Name must be made up of only uppercase and lowercase "
...
auto
validate_topic_arn(const std::string& str, std::string& message)
...
const std::string&
get_account_or_tenant(const rgw_owner& owner)
{
  return std::visit(
      fu2::overload(
...
bool
topic_has_endpoint_secret(const rgw_pubsub_topic& topic)
{
  return topic.dest.stored_secret;
}

bool
topics_has_endpoint_secret(const rgw_pubsub_topics& topics)
{
  for (const auto& topic : topics.topics) {
    if (topic_has_endpoint_secret(topic.second))
      return true;
  }
  return false;
}
...
static bool
topic_needs_queue(const rgw_pubsub_dest& dest)
{
  return !dest.push_endpoint.empty() && dest.persistent;
}

auto
get_policy_from_text(req_state* const s, const std::string& policy_text)
    ...
    uint64_t op)
{
  if (s->auth.identity->get_account()) {
    const bool account_root =
        (s->auth.identity->get_identity_type() == TYPE_ROOT);
    if (!s->auth.identity->is_owner_of(owner)) {
      ldpp_dout(dpp, 4) << "cross-account request for resource owner " << owner
                        << " != " << s->owner.id << dendl;
      ...
      const auto resource_res = evaluate_iam_policies(
          dpp, s->env, *s->auth.identity, false, op, arn, policy, {}, {});
      ...
      return Effect::Allow == evaluate_iam_policies(
                                  dpp, s->env, *s->auth.identity, account_root,
                                  op, arn, policy, s->iam_identity_policies,
                                  s->session_policies);
    }
  }
...
bool
should_forward_request_to_master(req_state* s, rgw::sal::Driver* driver)
{
  return (
      !driver->is_meta_master() &&
      rgw::all_zonegroups_support(
          *s->penv.site, rgw::zone_features::notification_v2));
}
...
private:
  ...
  get_params()
  {
    ...
    s->info.args.get_int(
        "time_to_live", reinterpret_cast<int*>(&dest.time_to_live),
        rgw::notify::DEFAULT_GLOBAL_VALUE);
    s->info.args.get_int(
        "max_retries", reinterpret_cast<int*>(&dest.max_retries),
        rgw::notify::DEFAULT_GLOBAL_VALUE);
    s->info.args.get_int(
    ...
    if (!validate_and_update_endpoint_secret(
            dest, s->cct, s->info, s->err.message)) {
      return -EINVAL;
    }
    // Store topic Policy.
    ...
    for (const auto& param : s->info.args.get_params()) {
      if (param.first == "Action" || param.first == "Name" ||
          param.first == "PayloadHash") {
        continue;
      }
    ...
    topic_arn = rgw::ARN{
        rgw::Partition::aws, rgw::Service::sns,
...
public:
  ...
  int
  init_processing(optional_yield y) override
  {
    int ret = get_params();
    if (ret < 0) {
      return ret;
    ...
    if (s->auth.identity->get_account() &&
        !rgw::all_zonegroups_support(
            *s->penv.site, rgw::zone_features::notification_v2)) {
      s->err.message =
          "The 'notification_v2' zone feature must be enabled "
    ...
    const RGWPubSub ps(
        driver, get_account_or_tenant(s->owner.id), *s->penv.site);
  ...
  verify_permission(optional_yield y) override
  {
    if (topic) {
      ...
      if (!verify_topic_permission(
              this, s, *topic, topic_arn, rgw::IAM::snsCreateTopic)) {
      ...
      if (!verify_user_permission(
              this, s, topic_arn, rgw::IAM::snsCreateTopic, mandatory_policy)) {
        return -ERR_AUTHORIZATION;
      }
    }
    return 0;
...
void
RGWPSCreateTopicOp::execute(optional_yield y)
{
  // master request will replicate the topic creation.
  if (should_forward_request_to_master(s, driver)) {
    op_ret = rgw_forward_request_to_master(
        this, *s->penv.site, s->owner.id, &bl_post_body, nullptr, s->info,
        s->err, y);
    if (op_ret < 0) {
      ldpp_dout(this, 4)
  ...
  if (!already_persistent && topic_needs_queue(dest)) {
    ...
    dest.num_shards = s->get_cct()->_conf.get_val<uint64_t>(
    ...
    for (const auto& q : dest.get_shard_names()) {
      ldpp_dout(this, 20)
          << "CreateTopic Action creating persistent topic queue: " << q
          << dendl;
      op_ret = driver->add_persistent_topic(this, y, q);
      if (op_ret < 0) {
        ldpp_dout(this, 1)
            << "CreateTopic Action failed to create queue/shards for "
               "persistent topics. error:"
            << op_ret << dendl;
        return;
      }
    }
    ldpp_dout(this, 20) << "Successfully created " << dest.num_shards
                        << " shards for topic: " << topic_name << dendl;
    ...
    if (s->get_cct()->_conf.get_val<uint64_t>(
            "rgw_bucket_persistent_notif_num_shards") != dest.num_shards) {
      ldpp_dout(this, 20)
          << "CreateTopic Action called for topic with existing shards, but "
             "num_shards in config is different. "
             "Not resharding existing topic: "
          << topic_name << dendl;
    } else {
      ldpp_dout(this, 20) << "CreateTopic Action called for topic with "
                             "existing. Not resharding existing topic: "
                          << topic_name << dendl;
    }
  }
  const RGWPubSub ps(driver, get_account_or_tenant(s->owner.id), *s->penv.site);
  op_ret = ps.create_topic(
      this, topic_name, dest, topic_arn.to_string(), opaque_data, s->owner.id,
      policy_text, y);
  if (op_ret < 0) {
    ldpp_dout(this, 4) << "failed to create topic '" << topic_name
                       << "', ret=" << op_ret << dendl;
    return;
  }
  ldpp_dout(this, 20) << "successfully created topic '" << topic_name << "'"
                      << dendl;
}
...
void
RGWPSListTopicsOp::execute(optional_yield y)
{
  const std::string start_token = s->info.args.get("NextToken");

  const RGWPubSub ps(driver, get_account_or_tenant(s->owner.id), *s->penv.site);
  if (rgw::all_zonegroups_support(
          *s->penv.site, rgw::zone_features::notification_v2) &&
      driver->stat_topics_v1(
          get_account_or_tenant(s->owner.id), null_yield, this) == -ENOENT) {
    constexpr int max_items = 100;
    op_ret =
        ps.get_topics_v2(this, start_token, max_items, result, next_token, y);
  } else {
  ...
  if (topics_has_endpoint_secret(result) &&
      !verify_transport_security(s->cct, *(s->info.env))) {
    s->err.message =
        "Topic contains secrets that must be transmitted over a secure "
        "transport";
    op_ret = -EPERM;
  ...
  for (auto it = result.topics.cbegin(); it != result.topics.cend();) {
    ...
    if (!arn ||
        !verify_topic_permission(
            this, s, it->second, *arn, rgw::IAM::snsGetTopicAttributes)) {
...
public:
  ...
  init_processing(optional_yield y) override
  {
    ...
    if (topic_has_endpoint_secret(result) &&
        !verify_transport_security(s->cct, *(s->info.env))) {
      s->err.message =
          "Topic contains secrets that must be transmitted over a secure "
          "transport";
  ...
  {
    if (!verify_topic_permission(
            this, s, result, topic_arn, rgw::IAM::snsGetTopicAttributes)) {
      return -ERR_AUTHORIZATION;
    }
    return 0;
  }
...
void
RGWPSGetTopicOp::execute(optional_yield y)
{
  ldpp_dout(this, 4) << "successfully got topic '" << topic_name << "'"
                     << dendl;
}
...
public:
  ...
  send_response() override
  {
    ...
    const auto f = s->formatter;
    f->open_object_section_in_ns(
        "GetTopicAttributesResponse", RGW_REST_SNS_XMLNS);
...
void
RGWPSGetTopicAttributesOp::execute(optional_yield y)
{
  ldpp_dout(this, 4) << "successfully got topic '" << topic_name << "'"
                     << dendl;
}
...
private:
  ...
  int
  map_attributes(const rgw_pubsub_topic& topic)
  {
    ...
    if (attribute_name == "OpaqueData") {
      ...
      s->info.args.get_int(
          "AttributeValue", reinterpret_cast<int*>(&dest.time_to_live),
          rgw::notify::DEFAULT_GLOBAL_VALUE);
    } else if (attribute_name == "max_retries") {
      s->info.args.get_int(
          "AttributeValue", reinterpret_cast<int*>(&dest.max_retries),
          rgw::notify::DEFAULT_GLOBAL_VALUE);
    } else if (attribute_name == "retry_sleep_duration") {
      s->info.args.get_int(
          "AttributeValue", reinterpret_cast<int*>(&dest.retry_sleep_duration),
          rgw::notify::DEFAULT_GLOBAL_VALUE);
      ...
      s->err.message =
          fmt::format("Invalid value for AttributeName '{}'", attribute_name);
      return -EINVAL;
    }
    return 0;
  }
...
public:
  ...
  {
    if (!verify_topic_permission(
            this, s, result, topic_arn, rgw::IAM::snsSetTopicAttributes)) {
      return -ERR_AUTHORIZATION;
    }
    return 0;
  }
  ...
  name() const override
  {
    return "pubsub_topic_set";
  }

  RGWOpType
  get_type() override
  {
    return RGW_OP_PUBSUB_TOPIC_SET;
  }
  ...
  send_response() override
  {
    ...
    const auto f = s->formatter;
    f->open_object_section_in_ns(
        "SetTopicAttributesResponse", RGW_REST_SNS_XMLNS);
    f->open_object_section("ResponseMetadata");
    encode_xml("RequestId", s->trans_id, f);
    f->close_section(); // ResponseMetadata
...
void
RGWPSSetTopicAttributesOp::execute(optional_yield y)
{
  if (should_forward_request_to_master(s, driver)) {
    op_ret = rgw_forward_request_to_master(
        this, *s->penv.site, s->owner.id, &bl_post_body, nullptr, s->info,
  ...
  if (!already_persistent && topic_needs_queue(dest)) {
    ...
    // delimiter because its inclusion in a TopicName would break ARNs
    dest.persistent_queue =
        string_cat_reserve(topic_arn.account, ":", topic_name);
    dest.num_shards = s->get_cct()->_conf.get_val<uint64_t>(
        "rgw_bucket_persistent_notif_num_shards");
    if (dest.num_shards == 0) {
      dest.num_shards = 1;
    ...
    for (const auto& q : result.dest.get_shard_names()) {
      ...
      if (op_ret != -ENOENT && op_ret < 0) {
        ldpp_dout(this, 4)
            << "SetTopicAttributes Action failed to remove queue "
    ...
    ldpp_dout(this, 20) << "Successfully removed " << result.dest.num_shards
                        << " shards for topic: " << topic_name << dendl;
  }
  const RGWPubSub ps(driver, topic_arn.account, *s->penv.site);
  op_ret = ps.create_topic(
      this, topic_name, dest, topic_arn.to_string(), opaque_data, topic_owner,
      policy_text, y);
  if (op_ret < 0) {
...
public:
  ...
  verify_permission(optional_yield y) override
  {
    if (topic) {
      ...
      if (!verify_topic_permission(
              this, s, *topic, topic_arn, rgw::IAM::snsDeleteTopic)) {
        return -ERR_AUTHORIZATION;
      }
    } else {
      ...
      const bool mandatory_policy = !!s->auth.identity->get_account();
      if (!verify_user_permission(
              this, s, topic_arn, rgw::IAM::snsDeleteTopic, mandatory_policy)) {
        return -ERR_AUTHORIZATION;
      }
    }
    return 0;
...
void
RGWPSDeleteTopicOp::execute(optional_yield y)
{
  if (should_forward_request_to_master(s, driver)) {
    op_ret = rgw_forward_request_to_master(
        this, *s->penv.site, s->owner.id, &bl_post_body, nullptr, s->info,
  ...
  if (op_ret < 0 && op_ret != -ENOENT) {
    ldpp_dout(this, 4) << "failed to remove topic '" << topic_name
                       << ", ret=" << op_ret << dendl;
    return;
  }
  ldpp_dout(this, 4) << "successfully removed topic '" << topic_name << "'"
                     << dendl;
...
static const std::unordered_map<std::string, op_generator> op_generators = {
    {"CreateTopic",
     [](bufferlist bl) -> RGWOp* {
       return new RGWPSCreateTopicOp(std::move(bl));
     }},
    {"DeleteTopic",
     [](bufferlist bl) -> RGWOp* {
       return new RGWPSDeleteTopicOp(std::move(bl));
     }},
    {"ListTopics",
     [](bufferlist bl) -> RGWOp* { return new RGWPSListTopicsOp; }},
    {"GetTopic", [](bufferlist bl) -> RGWOp* { return new RGWPSGetTopicOp; }},
    {"GetTopicAttributes",
     [](bufferlist bl) -> RGWOp* { return new RGWPSGetTopicAttributesOp; }},
    {"SetTopicAttributes", [](bufferlist bl) -> RGWOp* {
       return new RGWPSSetTopicAttributesOp(std::move(bl));
     }}};

bool
RGWHandler_REST_PSTopic_AWS::action_exists(const req_info& info)
{
...
bool
RGWHandler_REST_PSTopic_AWS::action_exists(const req_state* s)
{
...
int
RGWHandler_REST_PSTopic_AWS::authorize(
    const DoutPrefixProvider* dpp,
    optional_yield y)
{
  ...
  if (s->auth.identity->is_anonymous()) {
    ldpp_dout(dpp, 1) << "anonymous user not allowed in topic operations"
                      << dendl;
...
int
remove_notification_by_topic(
    const DoutPrefixProvider* dpp,
    const std::string& topic_name,
    const RGWPubSub::Bucket& b,
    optional_yield y,
    const RGWPubSub& ps)
{
  int op_ret = b.remove_notification(dpp, topic_name, y);
  if (op_ret < 0) {
    ldpp_dout(dpp, 1) << "failed to remove notification of topic '"
                      << topic_name << "', ret=" << op_ret << dendl;
  }
  op_ret = ps.remove_topic(dpp, topic_name, y);
  if (op_ret < 0) {
    ldpp_dout(dpp, 1) << "failed to remove auto-generated topic '" << topic_name
                      << "', ret=" << op_ret << dendl;
  }
  return op_ret;
}
...
class RGWPSCreateNotifOp : public RGWDefaultResponseOp {
  ...
  int
  get_params_from_body(rgw_pubsub_s3_notifications& configurations)
  {
    ...
    try {
      ...
      RGWXMLDecoder::decode_xml(
          "NotificationConfiguration", configurations, &parser, true);
...
public:
  ...
  {
    return fmt::format("REST.{}.NOTIFICATION", s->info.method);
  }
...
int
RGWPSCreateNotifOp::init_processing(optional_yield y)
{
  ...
  for (const auto& c : configurations.list) {
    ...
    if (std::find(c.events.begin(), c.events.end(), rgw::notify::UnknownEvent) !=
        c.events.end()) {
    ...
    auto insert = topics.emplace(
        std::piecewise_construct, std::forward_as_tuple(*arn),
...
int
RGWPSCreateNotifOp::verify_permission(optional_yield y)
{
...
void
RGWPSCreateNotifOp::execute(optional_yield y)
{
  if (should_forward_request_to_master(s, driver)) {
    op_ret = rgw_forward_request_to_master(
  ...
  for (const auto& c : configurations.list) {
    ...
    op_ret = ps.create_topic(
        this, unique_topic_name, topic_info.dest, topic_info.arn,
        topic_info.opaque_data, s->owner.id, topic_info.policy_text, y);
    if (op_ret < 0) {
      ldpp_dout(this, 1) << "failed to auto-generate unique topic '"
                         << unique_topic_name << "', ret=" << op_ret << dendl;
      return;
    }
    ldpp_dout(this, 20) << "successfully auto-generated unique topic '"
                        << unique_topic_name << "'" << dendl;
    // generate the notification
    rgw::notify::EventTypeList events;
    op_ret = b.create_notification(
        this, unique_topic_name, c.events, std::make_optional(c.filter),
        notif_name, y);
    if (op_ret < 0) {
      ldpp_dout(this, 1)
          << "failed to auto-generate notification for unique topic '"
          << unique_topic_name << "', ret=" << op_ret << dendl;
...
void
RGWPSCreateNotifOp::execute_v2(optional_yield y)
{
  if (const auto ret = driver->stat_topics_v1(s->bucket_tenant, y, this);
      ret != -ENOENT) {
    ldpp_dout(this, 1)
        << "WARNING: "
  ...
  op_ret = retry_raced_bucket_write(
      ...
      [this, y] {
        if (configurations.list.empty()) {
          return remove_notification_v2(
              this, driver, s->bucket.get(),
...
int
RGWPSDeleteNotifOp::init_processing(optional_yield y)
{
  int ret = get_params();
  if (ret < 0) {
    return ret;
...
int
RGWPSDeleteNotifOp::verify_permission(optional_yield y)
{
  if (!verify_bucket_permission(this, s, rgw::IAM::s3PutBucketNotification)) {
    return -EACCES;
  }
...
void
RGWPSDeleteNotifOp::execute(optional_yield y)
{
  if (should_forward_request_to_master(s, driver)) {
    bufferlist indata;
    op_ret = rgw_forward_request_to_master(
  ...
  if (!notif_name.empty()) {
    ...
    ldpp_dout(this, 20) << "notification '" << notif_name << "' already removed"
                        << dendl;
    return;
  }
...
void
RGWPSDeleteNotifOp::execute_v2(optional_yield y)
{
  if (const auto ret = driver->stat_topics_v1(s->bucket_tenant, y, this);
      ret != -ENOENT) {
    ldpp_dout(this, 4)
...
class RGWPSListNotifsOp : public RGWOp {
  ...
  int
  get_params(std::string& notif_name) const
  {
    bool exists;
    notif_name = s->info.args.get("notification", &exists);
    if (!exists) {
...
void
RGWPSListNotifsOp::execute(optional_yield y)
{
  std::string notif_name;
  ...
  op_ret = driver->load_bucket(
      this, rgw_bucket(s->bucket_tenant, s->bucket_name), &bucket, y);
  if (op_ret < 0) {
  ...
  if (!notif_name.empty()) {
    ...
    op_ret = -ENOENT;
    ldpp_dout(this, 4) << "failed to get notification info for '" << notif_name
                       << "', ret=" << op_ret << dendl;
    return;
  }
...
int
RGWPSListNotifsOp::verify_permission(optional_yield y)
{
