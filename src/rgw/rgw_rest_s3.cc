...
void
list_all_buckets_start(req_state* s)
{
  s->formatter->open_array_section_in_ns("ListAllMyBucketsResult", XMLNS_AWS_S3);
}

void
list_all_buckets_end(req_state* s)
{
  s->formatter->close_section();
}

void
dump_bucket(req_state* s, const RGWBucketEnt& ent)
{
...
void
rgw_get_errno_s3(rgw_http_error* e, int err_no)
{
...
int
get_encryption_defaults(req_state* s)
{
  ...
  if (iter != s->info.crypt_attribute_map.end()) {
    ...
    rgw_set_amz_meta_header(
        s->info.crypt_attribute_map, encrypt_attr, sse_algorithm, OVERWRITE);
  }
  ...
  if (iter != s->info.crypt_attribute_map.end()) {
    ...
    kms_attr_seen = true;
    rgw_set_amz_meta_header(
        s->info.crypt_attribute_map, kms_attr, kms_master_key_id, OVERWRITE);
  }
  ...
  if (iter != s->info.crypt_attribute_map.end()) {
    ...
    kms_attr_seen = true;
    rgw_set_amz_meta_header(
        s->info.crypt_attribute_map, bucket_key_attr, "true", OVERWRITE);
  }
  ...
  if (kms_attr_seen && sse_algorithm != "aws:kms") {
    s->err.message = "algorithm <" + sse_algorithm +
...
int
RGWGetObj_ObjStore_S3Website::send_response_data(
    bufferlist& bl,
    off_t bl_ofs,
    off_t bl_len)
{
...
int
RGWGetObj_ObjStore_S3Website::send_response_data_error(optional_yield y)
{
  return RGWGetObj_ObjStore_S3::send_response_data_error(y);
}

int
RGWGetObj_ObjStore_S3::get_params(optional_yield y)
{
  ...
  sync_cloudtiered = s->info.args.exists(RGW_SYS_PARAM_PREFIX
                                         "sync-cloudtiered");

  dst_zone_trace = s->info.args.get(RGW_SYS_PARAM_PREFIX
                                    "if-not-replicated-to");
...
int
RGWGetObj_ObjStore_S3::send_response_data_error(optional_yield y)
{
  bufferlist bl;
...
inline bool
str_has_cntrl(const std::string s)
{
  return std::any_of(s.begin(), s.end(), ::iscntrl);
}

inline bool
str_has_cntrl(const char* s)
{
  std::string _s(s);
...
static std::string
content_encoding_without_aws_chunked(std::string_view value)
{
  std::string result;
    ...
    off_t bl_len)
{
  ...
  if (custom_http_ret) {
    ...
    set_req_state_err(
        s, (partial_content && !op_ret) ? STATUS_PARTIAL_CONTENT : op_ret);
    dump_errno(s);
  }
  ...
  if (s->system_request &&
    ...
    auto action = s->object->get_instance().empty()
    ...
    bool keep_tags = s->auth.identity->is_admin() ||
                     verify_object_permission(this, s, action);
    ...
    if (!keep_tags) {
      ...
      if (iter != attrs.end()) {
        ldpp_dout(this, 4)
            << "removing tags from attrs due to missing permission on "
            << rgw::IAM::action_bit_string(action) << dendl;
  ...
  if (s->system_request && !real_clock::is_zero(lastmod)) {
    ...
    uint64_t pg_ver = 0;
    int r = decode_attr_bl_single_value(
        attrs, RGW_ATTR_PG_VER, &pg_ver, (uint64_t)0);
    if (r < 0) {
    ...
    r = decode_attr_bl_single_value(
        attrs, RGW_ATTR_SOURCE_ZONE, &source_zone_short_id, (uint32_t)0);
    if (r < 0) {
      ldpp_dout(this, 0) << "ERROR: failed to decode pg ver attr, ignoring"
                         << dendl;
    }
      ...
      if (auto i = attrs.find(RGW_ATTR_CKSUM); i != attrs.end()) {
        try {
          ...
          if (multipart_part_num) {
            ...
            cksum_type = rgw::cksum::get_checksum_type(
                cksum, (multipart_parts_count &&
                        multipart_parts_count > 0) /* is_multipart */);
          }
	       ...
	       update that introduced CRC combining */
            if (multipart_part_num) {
              ...
              dump_header(
                  s, cksum.header_name(),
                  fmt::format(
                      "{}-{}", cksum.to_armor(), *multipart_parts_count));
            }
          } else {
          ...
          ldpp_dout(this, 0)
              << "ERROR: failed to decode rgw::cksum::Cksum" << dendl;
          ...
          if (rt == rgw::sal::RGWRestoreType::Temporary) {
            ...
            dump_header_if_nonempty(
                ...
                "ongoing-request=\"false\", expiry-date=\"" +
                    dump_time_to_str(expiration_date) + "\"");
    ...
    for (auto iter = attrs.begin(); iter != attrs.end(); ++iter) {
      ...
      if (aiter != rgw_to_http_attrs.end()) {
        ...
        dump_header(s, "X-Object-Meta-Static-Large-Object", "True");
          ...
          strncmp(name, RGW_ATTR_META_PREFIX, sizeof(RGW_ATTR_META_PREFIX) - 1) ==
          0) {
        ...
        dump_header(s, RGW_AMZ_TAG_COUNT, obj_tags.count());
          ...
          iter->first.compare(RGW_ATTR_OBJECT_RETENTION) == 0 && get_retention) {
        ...
        try {
          ...
          iter->first.compare(RGW_ATTR_OBJECT_LEGAL_HOLD) == 0 &&
          get_legal_hold) {
        RGWObjectLegalHold legal_hold;
        try {
          decode(legal_hold, iter->second);
          dump_header(
              s, "x-amz-object-lock-legal-hold", legal_hold.get_status());
        } catch (buffer::error& err) {
          ldpp_dout(this, 0)
    ...
    bufferlist* manifest_bl)
{
  ...
  return ::get_decrypt_filter(
      filter, cb, s, attrs, manifest_bl, &crypt_http_responses, copy_source);
}
...
int
RGWGetObj_ObjStore_S3::override_range_hdr(
    const rgw::auth::StrategyRegistry& auth_registry,
    optional_yield y)
{
  int ret = -EINVAL;
...
void
RGWGetObjTags_ObjStore_S3::send_response_data(bufferlist& bl)
{
  if (op_ret)
    set_req_state_err(s, op_ret);
  dump_errno(s);
...
int
RGWPutObjTags_ObjStore_S3::get_params(optional_yield y)
{
  RGWXMLParser parser;
...
void
RGWPutObjTags_ObjStore_S3::send_response()
{
  if (op_ret)
    set_req_state_err(s, op_ret);
  dump_errno(s);
...
void
RGWDeleteObjTags_ObjStore_S3::send_response()
{
  if (op_ret == 0) {
...
void
RGWGetBucketTags_ObjStore_S3::send_response_data(bufferlist& bl)
{
  if (op_ret)
    set_req_state_err(s, op_ret);
  dump_errno(s);
...
int
RGWPutBucketTags_ObjStore_S3::get_params(
...
void
RGWPutBucketTags_ObjStore_S3::send_response()
{
  if (op_ret)
    set_req_state_err(s, op_ret);
  dump_errno(s);
...
void
RGWDeleteBucketTags_ObjStore_S3::send_response()
{
...
bool
is_valid_status(const string& s)
{
...
struct ReplicationConfiguration {
  ...
  struct Rule {
    struct DeleteMarkerReplication {
      ...
      void
      dump_xml(Formatter* f) const
      {
        encode_xml("Status", status, f);
      }

      bool
      is_valid(CephContext* cct) const
      {
    ...
    struct Destination {
      ...
      decode_xml(XMLObj* obj)
      {
        RGWXMLDecoder::decode_xml(
            "AccessControlTranslation", acl_translation, obj);
    ...
    struct Filter {
      ...
      int
      to_sync_pipe_filter(CephContext* cct, rgw_sync_pipe_filter* f) const
      {
        if (!is_valid(cct)) {
          return -EINVAL;
        }
      ...
      void
      from_sync_pipe_filter(const rgw_sync_pipe_filter& f)
      {
    ...
    {
      RGWXMLDecoder::decode_xml(
          "DeleteMarkerReplication", delete_marker_replication, obj);
        ...
        bool* enabled) const
    {
      ...
      if (!std::holds_alternative<rgw_user>(s->owner.id)) {
        ...
        ldpp_dout(s, 1) << "NOTICE: replication configuration is only "
                           "supported for rgw_user"
                        << dendl;
      ...
      rgw_bucket_key dest_bk(tenant_owner, dest_bk_arn->resource);
      ...
      vector<rgw_sync_policy_group>* result) const
  {
    ...
    for (auto& rule : rules) {
      if (!priorities.insert(rule.priority).second) {
        s->err.message =
            fmt::format("Found duplicate priority {}.", rule.priority);
        return -EINVAL;
      }
...
void
RGWGetBucketReplication_ObjStore_S3::send_response_data()
{
  if (op_ret)
    set_req_state_err(s, op_ret);
  dump_errno(s);
  ...
  if (!op_ret) {
    s->formatter->open_object_section_in_ns(
        "ReplicationConfiguration", XMLNS_AWS_S3);
...
int
RGWPutBucketReplication_ObjStore_S3::get_params(optional_yield y)
{
  RGWXMLParser parser;
...
void
RGWPutBucketReplication_ObjStore_S3::send_response()
{
  if (op_ret)
    set_req_state_err(s, op_ret);
  dump_errno(s);
...
void
RGWDeleteBucketReplication_ObjStore_S3::update_sync_policy(
...
void
RGWDeleteBucketReplication_ObjStore_S3::send_response()
{
  if (op_ret)
    set_req_state_err(s, op_ret);
  dump_errno(s);
...
int
RGWListBuckets_ObjStore_S3::get_params(optional_yield y)
{
...
void
RGWListBuckets_ObjStore_S3::send_response_begin(bool has_buckets)
{
  if (op_ret)
    set_req_state_err(s, op_ret);
  dump_errno(s);
...
void
RGWListBuckets_ObjStore_S3::send_response_data(
...
int
RGWGetUsage_ObjStore_S3::get_params(optional_yield y)
{
...
void
RGWGetUsage_ObjStore_S3::send_response()
{
  if (op_ret < 0)
    set_req_state_err(s, op_ret);
  dump_errno(s);
  ...
  for (iter = usage.begin(); iter != usage.end(); ++iter) {
    ...
    if (show_log_entries) {
      ...
      if (categories.empty() || categories.count("s3select")) {
        encode_json(
            "BytesProcessed", entry.s3select_usage.bytes_processed, formatter);
        encode_json(
            "BytesReturned", entry.s3select_usage.bytes_returned, formatter);
      }
  ...
  if (show_log_sum) {
    ...
    encode_json(
        "QuotaMaxObjCount", user_info.quota.user_quota.max_objects, formatter);
    encode_json(
        "QuotaMaxBytesPerBucket", user_info.quota.bucket_quota.max_size,
        formatter);
    encode_json(
        "QuotaMaxObjCountPerBucket", user_info.quota.bucket_quota.max_objects,
        formatter);
...
int
RGWListBucket_ObjStore_S3::get_params(optional_yield y)
{
...
int
RGWListBucket_ObjStore_S3v2::get_params(optional_yield y)
{
  int ret = get_common_params();
  if (ret < 0) {
    return ret;
...
RGWListBucket_ObjStore_S3::send_common_versioned_response()
{
  ...
  s->formatter->dump_string(
      "IsTruncated", (max && is_truncated ? "true" : "false"));
...
void
RGWListBucket_ObjStore_S3::send_versioned_response()
{
...
void
RGWListBucket_ObjStore_S3v2::send_versioned_response()
{
  s->formatter->open_object_section_in_ns("ListVersionsResult", XMLNS_AWS_S3);
  ...
  if (is_truncated && !next_marker.empty()) {
    ...
    s->formatter->dump_string(
        "NextVersionIdContinuationToken", next_marker.instance);
  }
  ...
  if (op_ret >= 0) {
    ...
    for (iter = objs.begin(); iter != objs.end(); ++iter) {
      ...
      s->formatter->open_object_section(section_name);
      if (objs_container) {
        s->formatter->dump_bool(
            "IsDeleteContinuationToken", iter->is_delete_marker());
      }
      rgw_obj_key key(iter->key);
      dump_urlsafe(s, encode_key, "Key", key.name);
      string version_id = key.instance;
...
void
RGWListBucket_ObjStore_S3v2::send_response()
{
  if (op_ret < 0) {
    set_req_state_err(s, op_ret);
  }
...
RGWGetBucketLocation_ObjStore_S3::send_response()
{
  dump_errno(s);
  dump_header(
      s, "x-rgw-bucket-placement-target",
  ...
  s->formatter->dump_format_ns(
      "LocationConstraint", XMLNS_AWS_S3, "%s", api_name.c_str());
  rgw_flush_formatter_and_reset(s, s->formatter);
}

void
RGWGetBucketVersioning_ObjStore_S3::send_response()
{
  if (op_ret)
    set_req_state_err(s, op_ret);
  dump_errno(s);
  ...
  s->formatter->open_object_section_in_ns(
      "VersioningConfiguration", XMLNS_AWS_S3);
  if (versioned) {
...
int
RGWSetBucketVersioning_ObjStore_S3::get_params(optional_yield y)
{
  int r = 0;
  bufferlist data;
  std::tie(r, data) =
  ...
  if (!RGWXMLDecoder::decode_xml(
          "VersioningConfiguration", status_conf, &parser)) {
...
void
RGWSetBucketVersioning_ObjStore_S3::send_response()
{
  if (op_ret)
    set_req_state_err(s, op_ret);
  dump_errno(s);
...
int
RGWSetBucketWebsite_ObjStore_S3::get_params(optional_yield y)
{
  const auto max_size = s->cct->_conf->rgw_max_put_param_size;
  ...
  try {
    RGWXMLDecoder::decode_xml(
        "WebsiteConfiguration", website_conf, &parser, true);
  } catch (RGWXMLDecoder::err& err) {
  ...
  if (website_conf.is_redirect_all &&
    ...
    s->err.message =
        "A host name must be provided to redirect all requests (e.g. "
        "\"example.com\").";
    ldpp_dout(this, 5) << s->err.message << dendl;
    return -EINVAL;
  } else if (!website_conf.is_redirect_all && !website_conf.is_set_index_doc) {
    s->err.message =
        "A value for IndexDocument Suffix must be provided if "
        "RedirectAllRequestsTo is empty";
    ldpp_dout(this, 5) << s->err.message << dendl;
    return -EINVAL;
  } else if (
      !website_conf.is_redirect_all && website_conf.is_set_index_doc &&
...
void
RGWSetBucketWebsite_ObjStore_S3::send_response()
{
  if (op_ret < 0)
    set_req_state_err(s, op_ret);
  dump_errno(s);
...
void
RGWDeleteBucketWebsite_ObjStore_S3::send_response()
{
  if (op_ret == 0) {
    op_ret = STATUS_NO_CONTENT;
  }
...
void
RGWGetBucketWebsite_ObjStore_S3::send_response()
{
  if (op_ret)
    set_req_state_err(s, op_ret);
  dump_errno(s);
...
static void
dump_bucket_metadata(req_state* s, RGWStorageStats& stats)
{
  dump_header(
      s, "X-RGW-Object-Count", static_cast<long long>(stats.num_objects));
  dump_header(s, "X-RGW-Bytes-Used", static_cast<long long>(stats.size));
}

int
RGWStatBucket_ObjStore_S3::get_params(optional_yield y)
{
    ...
    const ACLOwner& owner)
{
  if (s->has_acl_header) {
    ...
    return rgw::s3::create_policy_from_headers(
        s, s->yield, driver, owner, *s->info.env, policy);
  }

  return rgw::s3::create_canned_acl(
      owner, s->bucket_owner, s->bucket_object_ownership, s->canned_acl, policy,
      s->err.message);
}
...
int
RGWCreateBucket_ObjStore_S3::get_params(optional_yield y)
{
  ...
  if (data.length()) {
    ...
    if (config->index) {
      ...
      if (config->index->num_shards) {
        ...
        const auto limit =
            s->cct->_conf.get_val<uint64_t>("rgw_max_dynamic_shards");
        if (*val > limit) {
  ...
  if (pos != string::npos) {
    createparams.placement_rule.init(
        location_constraint.substr(pos + 1), s->info.storage_class);
...
RGWCreateBucket_ObjStore_S3::send_response()
{
  if (op_ret == -ERR_BUCKET_EXISTS) {
    const auto eexist_override =
        s->cct->_conf.get_val<bool>("rgw_bucket_eexist_override");
    if (!eexist_override) [[likely]] {
      op_ret = 0;
    } else {
      s->err.message =
          "The requested bucket name is not available. The bucket namespace is "
          "shared by all users of the system. Specify a different name and try "
          "again.";
    }
  }
...
static inline void
map_qs_metadata(req_state* s, bool crypto_too)
{
  ...
  for (const auto& elt : params) {
    ...
    if (crypto_too && k.find("x-amz-server-side-encryption") == /* offset */ 0) {
      rgw_set_amz_meta_header(
          s->info.crypt_attribute_map, k, elt.second, OVERWRITE);
    }
  }
}

int
RGWPutObj_ObjStore_S3::get_params(optional_yield y)
{
  if (!s->length) {
...
int
RGWPutObj_ObjStore_S3::get_data(bufferlist& bl)
{
...
static inline void
set_attr(map<string, bufferlist>& attrs, const char* key, const char* value)
{
  bufferlist bl;
  encode(value, bl);
  attrs.emplace(key, std::move(bl));
...
int
RGWPutObj_ObjStore_S3::get_decrypt_filter(
    std::unique_ptr<RGWGetObj_Filter>* filter,
    RGWGetObj_Filter* cb,
    map<string, bufferlist>& attrs,
    bufferlist* manifest_bl)
{
  ...
  return ::get_decrypt_filter(
      filter, cb, s, attrs, manifest_bl, nullptr, copy_source);
}

int
RGWPutObj_ObjStore_S3::get_encrypt_filter(
    ...
    rgw::sal::DataProcessor* cb)
{
  ...
  if (!multipart_upload_id.empty()) {
    ...
    if (res == 0) {
      ...
      static constexpr bool copy_source = false;
      res = rgw_s3_prepare_decrypt(
          s, s->yield, obj->get_attrs(), &block_crypt, &crypt_http_responses,
          copy_source);
...
void
RGWPostObj_ObjStore_S3::rebuild_key(rgw::sal::Object* obj)
{
...
int
RGWPostObj_ObjStore_S3::get_params(optional_yield y)
{
  ...
  for (auto& p : parts) {
    if (boost::istarts_with(p.first, "x-amz-server-side-encryption")) {
      ...
      rgw_set_amz_meta_header(
          s->info.crypt_attribute_map, p.first, v, OVERWRITE);
    }
    /* checksum headers */
    auto& k = p.first;
  ...
  for (; piter != parts.end(); ++piter) {
    ...
    if (strncasecmp(
            n.c_str(), RGW_AMZ_META_PREFIX, sizeof(RGW_AMZ_META_PREFIX) - 1) !=
        0)
      break;
...
int
RGWPostObj_ObjStore_S3::get_tags()
{
  string tags_str;
...
int
RGWPostObj_ObjStore_S3::get_policy(optional_yield y)
{
  if (part_bl(parts, "policy", &s->auth.s3_postobj_creds.encoded_policy)) {
    ...
    if (aws4_auth) {
      ...
      if (!part_str(
              parts, "x-amz-signature", &s->auth.s3_postobj_creds.signature)) {
      ...
      // check that the signature matches the encoded policy
      if (!part_str(
              parts, "AWSAccessKeyId", &s->auth.s3_postobj_creds.access_key)) {
  ...
  int r = rgw::s3::create_canned_acl(
      s->owner, s->bucket_owner, s->bucket_object_ownership, canned_acl, policy,
      s->err.message);
  if (r < 0) {
...
int
RGWPostObj_ObjStore_S3::complete_get_params()
{
  bool done;
  do {
    struct post_form_part part;
...
int
RGWPostObj_ObjStore_S3::get_data(ceph::bufferlist& bl, bool& again)
{
  bool boundary;
  bool done;
...
void
RGWPostObj_ObjStore_S3::send_response()
{
  ...
  if (op_ret == STATUS_CREATED) {
    ...
    if (!s->bucket_tenant.empty()) {
      s->formatter->dump_format(
          "Location", "%s/%s:%s/%s", base_uri.c_str(),
...
int
RGWPostObj_ObjStore_S3::get_encrypt_filter(
    std::unique_ptr<rgw::sal::DataProcessor>* filter,
    rgw::sal::DataProcessor* cb)
{
  std::unique_ptr<BlockCrypt> block_crypt;
...
int
RGWRestoreObj_ObjStore_S3::get_params(optional_yield y)
{
...
void
RGWRestoreObj_ObjStore_S3::send_response()
{
  if (restore_ret < 0) {
  ...
  if (restore_ret == 0) {
    ...
    rgw::sal::RGWRestoreStatus st =
        static_cast<rgw::sal::RGWRestoreStatus>(restore_ret);
...
int
RGWDeleteObj_ObjStore_S3::get_params(optional_yield y)
{
  const char* if_unmod =
      s->info.env->get("HTTP_X_AMZ_DELETE_IF_UNMODIFIED_SINCE");
  const char* if_last_mod_time_match =
      s->info.env->get("HTTP_X_AMZ_IF_MATCH_LAST_MODIFIED_TIME");
  const char* if_size_match = s->info.env->get("HTTP_X_AMZ_IF_MATCH_SIZE");
  if_match = s->info.env->get("HTTP_IF_MATCH");

  if (s->system_request) {
    s->info.args.get_bool(
        RGW_SYS_PARAM_PREFIX "no-precondition-error", &no_precondition_error,
        false);
  }
  ...
  if (if_last_mod_time_match) {
    ...
    if (r < 0) {
      ldpp_dout(this, 10) << "failed to parse time: "
                          << if_last_mod_match_decoded << dendl;
      return r;
    }
  }
...
int
RGWCopyObj_ObjStore_S3::init_dest_policy()
{
...
int
RGWCopyObj_ObjStore_S3::get_params(optional_yield y)
{
  //handle object lock
  auto obj_lock_mode_str = s->info.env->get("HTTP_X_AMZ_OBJECT_LOCK_MODE");
  auto obj_lock_date_str =
  ...
  if (obj_lock_mode_str && obj_lock_date_str) {
    ...
    s->err.message =
        "need both x-amz-object-lock-mode and "
        "x-amz-object-lock-retain-until-date ";
    ldpp_dout(this, 0) << s->err.message << dendl;
    return -EINVAL;
  }
  ...
  if (s->system_request) {
    ...
    s->info.args.get_bool(
        RGW_SYS_PARAM_PREFIX "copy-if-newer", &copy_if_newer, false);
  }
...
int
RGWCopyObj_ObjStore_S3::check_storage_class(
...
void
RGWCopyObj_ObjStore_S3::send_partial_response(off_t ofs)
{
  if (!sent_header) {
    if (op_ret)
      set_req_state_err(s, op_ret);
...
void
RGWCopyObj_ObjStore_S3::send_response()
{
  if (!sent_header)
...
void
RGWGetACLs_ObjStore_S3::send_response()
{
  if (op_ret)
    set_req_state_err(s, op_ret);
  dump_errno(s);
...
int
RGWPutACLs_ObjStore_S3::get_params(optional_yield y)
{
...
int
RGWPutACLs_ObjStore_S3::get_policy_from_state(
    const ACLOwner& owner,
    RGWAccessControlPolicy& policy)
{
...
void
RGWPutACLs_ObjStore_S3::send_response()
{
  if (op_ret)
    set_req_state_err(s, op_ret);
  dump_errno(s);
...
int
RGWGetObjAttrs_ObjStore_S3::get_params(optional_yield y)
{
  string err;
  auto& env = s->info.env;
  ...
  if (hdr) {
    ...
    if (!err.empty()) {
      ...
      ldpp_dout(s, 10) << "Invalid value for MaxParts " << *hdr << ": " << err
                       << dendl;
      return -ERR_INVALID_PART;
    }
...
int
RGWGetObjAttrs_ObjStore_S3::get_decrypt_filter(
    std::unique_ptr<RGWGetObj_Filter>* filter,
    RGWGetObj_Filter* cb,
    bufferlist* manifest_bl)
{
  ...
  static constexpr bool copy_source = false;
  return rgw_s3_prepare_decrypt(
      s, s->yield, attrs, nullptr, nullptr, copy_source);
}
  ...
  if (op_ret == 0) {
    ...
    if (requested_attributes & as_flag(ReqAttributes::Checksum)) {
      ...
      if (iter != attrs.end()) {
        try {
          ...
          cksum.decode(bliter);
          auto cksum_type = rgw::cksum::get_checksum_type(
              cksum, (multipart_parts_count &&
                      multipart_parts_count > 0) /* is_multipart */);
          if (std::get<0>(cksum_type) == rgw::cksum::Cksum::FLAG_COMPOSITE) {
            /* cksum was computed with a digest algorithm, or predates the 2025
    ...
    if (requested_attributes & as_flag(ReqAttributes::ObjectParts)) {
      if (multipart_parts_count && multipart_parts_count > 0) {
        ...
        if (op_ret < 0) {
          ldpp_dout_fmt(
              this, 0, "ERROR: {} load_obj_state() failed ret={}", __func__,
              op_ret);
        }

        ldpp_dout_fmt(
            this, 16, "{} attr flags={} parts_count={}", __func__,
        ...
        int ret = s->object->list_parts(
            this, s->cct, max_parts ? *max_parts : 1000, marker ? *marker : 0,
            ...
            [&](const Object::Part& part) -> int {
              ...
              if (part.cksum.type != rgw::cksum::Type::none) {
                ...
                s->formatter->dump_string(
                    part.cksum.element_name(), part.cksum.to_armor());
              }
        ...
        if (ret < 0) {
          ldpp_dout_fmt(
              this, 0, "ERROR: {} list-parts failed for {}", __func__,
              s->object->get_name());
        }
...
void
RGWGetLC_ObjStore_S3::execute(optional_yield y)
{
...
void
RGWGetLC_ObjStore_S3::send_response()
{
  if (op_ret) {
    if (op_ret == -ENOENT) {
...
void
RGWPutLC_ObjStore_S3::send_response()
{
  if (op_ret)
    set_req_state_err(s, op_ret);
  dump_errno(s);
...
void
RGWDeleteLC_ObjStore_S3::send_response()
{
  if (op_ret == 0)
    op_ret = STATUS_NO_CONTENT;
  if (op_ret) {
...
void
RGWGetCORS_ObjStore_S3::send_response()
{
  if (op_ret) {
    if (op_ret == -ENOENT)
...
int
RGWPutCORS_ObjStore_S3::get_params(optional_yield y)
{
...
void
RGWPutCORS_ObjStore_S3::send_response()
{
  if (op_ret)
    set_req_state_err(s, op_ret);
  dump_errno(s);
...
void
RGWDeleteCORS_ObjStore_S3::send_response()
{
  int r = op_ret;
  if (!r || r == -ENOENT)
    r = STATUS_NO_CONTENT;
...
void
RGWOptionsCORS_ObjStore_S3::send_response()
{
  ...
  dump_errno(s);
  dump_access_control(
      s, origin, req_meth, hdrs.c_str(), exp_hdrs.c_str(), max_age);
  end_header(s, NULL);
}

void
RGWPutBucketEncryption_ObjStore_S3::send_response()
{
  if (op_ret) {
    set_req_state_err(s, op_ret);
  }
...
void
RGWGetBucketEncryption_ObjStore_S3::send_response()
{
  if (op_ret) {
    if (op_ret == -ENOENT)
  ...
  if (!op_ret) {
    encode_xml(
        "ServerSideEncryptionConfiguration", XMLNS_AWS_S3,
...
void
RGWDeleteBucketEncryption_ObjStore_S3::send_response()
{
  if (op_ret == 0) {
    op_ret = STATUS_NO_CONTENT;
  }
...
int
RGWPutBucketOwnershipControls_ObjStore_S3::get_params(optional_yield y)
{
  const auto max_size = s->cct->_conf->rgw_max_put_param_size;
  int ret = 0;
...
void
RGWPutBucketOwnershipControls_ObjStore_S3::send_response()
{
  if (op_ret) {
    set_req_state_err(s, op_ret);
  }
...
void
RGWGetBucketOwnershipControls_ObjStore_S3::send_response()
{
  if (op_ret) {
    if (op_ret == -ENOENT)
...
void
RGWDeleteBucketOwnershipControls_ObjStore_S3::send_response()
{
  if (op_ret == 0) {
    op_ret = STATUS_NO_CONTENT;
  }
...
void
RGWGetRequestPayment_ObjStore_S3::send_response()
{
  dump_errno(s);
  end_header(s, this, to_mime_type(s->format));
  dump_start(s);
  ...
  s->formatter->open_object_section_in_ns(
      "RequestPaymentConfiguration", XMLNS_AWS_S3);
...
public:
  ...
  int
  get_request_payment_payer(bool* requester_pays)
  {
...
int
RGWSetRequestPayment_ObjStore_S3::get_params(optional_yield y)
{
  const auto max_size = s->cct->_conf->rgw_max_put_param_size;
...
void
RGWSetRequestPayment_ObjStore_S3::send_response()
{
  if (op_ret)
    set_req_state_err(s, op_ret);
  dump_errno(s);
...
int
RGWInitMultipart_ObjStore_S3::get_params(optional_yield y)
{
  int ret;
...
void
RGWInitMultipart_ObjStore_S3::send_response()
{
  if (op_ret)
    set_req_state_err(s, op_ret);
  dump_errno(s);
  ...
  if (op_ret == 0) {
    dump_start(s);
    s->formatter->open_object_section_in_ns(
        "InitiateMultipartUploadResult", XMLNS_AWS_S3);
    if (!s->bucket_tenant.empty())
      s->formatter->dump_string("Tenant", s->bucket_tenant);
    s->formatter->dump_string("Bucket", s->bucket_name);
    s->formatter->dump_string("Key", s->object->get_name());
...
int
RGWInitMultipart_ObjStore_S3::prepare_encryption(map<string, bufferlist>& attrs)
{
  int res = 0;
  res =
...
int
RGWCompleteMultipart_ObjStore_S3::get_params(optional_yield y)
{
  ...
  std::unique_ptr<rgw::sal::MultipartUpload> upload =
      s->bucket->get_multipart_upload(s->object->get_name(), upload_id);
  std::unique_ptr<rgw::sal::Object> obj = upload->get_meta_obj();
  obj->set_in_extra_data(true);
  int res = obj->get_obj_attrs(s->yield, this);
  if (res < 0 && res != -ENOENT) {
    ldpp_dout(this, 0) << "ERROR: " << __func__
                       << " failed to get object attrs for "
                       << s->object->get_name() << ": " << cpp_strerror(res)
                       << dendl;
    return res;
  }
  ...
  if (res == 0) {
    static constexpr bool copy_source = false;
    res = rgw_s3_prepare_decrypt(
        s, s->yield, obj->get_attrs(), nullptr, &crypt_http_responses,
        copy_source);
    if (res < 0) {
      return res;
    }
...
void
RGWCompleteMultipart_ObjStore_S3::send_response()
{
  if (op_ret)
    set_req_state_err(s, op_ret);
  dump_errno(s);
  ...
  if (op_ret == 0) {
    dump_start(s);
    s->formatter->open_object_section_in_ns(
        "CompleteMultipartUploadResult", XMLNS_AWS_S3);
    std::string base_uri = compute_domain_uri(s);
    if (!s->bucket_tenant.empty()) {
      s->formatter->dump_format(
      ...
      s->formatter->dump_format(
          "Location", "%s/%s/%s", base_uri.c_str(), s->bucket_name.c_str(),
...
void
RGWAbortMultipart_ObjStore_S3::send_response()
{
  int r = op_ret;
  if (!r)
    r = STATUS_NO_CONTENT;
...
void
RGWListMultipart_ObjStore_S3::send_response()
{
  if (op_ret)
    set_req_state_err(s, op_ret);
  dump_errno(s);
  ...
  if (op_ret == 0) {
    ...
    for (; iter != upload->get_parts().end(); ++iter) {
      ...
      if (part_cksum && part_cksum->aws()) {
        s->formatter->dump_string(
            part_cksum->element_name(), part_cksum->to_armor());
      }
      s->formatter->close_section();
    }
    s->formatter->close_section();
...
RGWListBucketMultiparts_ObjStore_S3::send_response()
{
  ...
  s->formatter->open_object_section_in_ns(
      "ListMultipartUploadsResult", XMLNS_AWS_S3);
  if (!s->bucket_tenant.empty())
    s->formatter->dump_string("Tenant", s->bucket_tenant);
  s->formatter->dump_string("Bucket", s->bucket_name);
  if (!prefix.empty())
...
int
RGWDeleteMultiObj_ObjStore_S3::get_params(optional_yield y)
{
...
void
RGWDeleteMultiObj_ObjStore_S3::send_status()
{
  if (!status_dumped) {
    if (op_ret < 0)
      set_req_state_err(s, op_ret);
...
void
RGWDeleteMultiObj_ObjStore_S3::begin_response()
{
...
void
RGWDeleteMultiObj_ObjStore_S3::send_partial_response(
...
void
RGWGetObjLayout_ObjStore_S3::send_response()
{
  if (op_ret)
    set_req_state_err(s, op_ret);
  dump_errno(s);
...
int
RGWConfigBucketMetaSearch_ObjStore_S3::get_params(optional_yield y)
{
...
void
RGWConfigBucketMetaSearch_ObjStore_S3::send_response()
{
  if (op_ret)
    set_req_state_err(s, op_ret);
  dump_errno(s);
...
void
RGWGetBucketMetaSearch_ObjStore_S3::send_response()
{
  if (op_ret)
    set_req_state_err(s, op_ret);
  dump_errno(s);
...
void
RGWDelBucketMetaSearch_ObjStore_S3::send_response()
{
  if (op_ret)
    set_req_state_err(s, op_ret);
  dump_errno(s);
...
void
RGWPutBucketObjectLock_ObjStore_S3::send_response()
{
  if (op_ret) {
    set_req_state_err(s, op_ret);
  }
...
void
RGWGetBucketObjectLock_ObjStore_S3::send_response()
{
  if (op_ret) {
    set_req_state_err(s, op_ret);
  }
  ...
  encode_xml(
      "ObjectLockConfiguration", s->bucket->get_info().obj_lock, s->formatter);
  rgw_flush_formatter_and_reset(s, s->formatter);
}

int
RGWPutObjRetention_ObjStore_S3::get_params(optional_yield y)
{
  const char* bypass_gov_header =
      s->info.env->get("HTTP_X_AMZ_BYPASS_GOVERNANCE_RETENTION");
  if (bypass_gov_header) {
...
void
RGWPutObjRetention_ObjStore_S3::send_response()
{
  if (op_ret) {
    set_req_state_err(s, op_ret);
  }
...
void
RGWGetObjRetention_ObjStore_S3::send_response()
{
  if (op_ret) {
    set_req_state_err(s, op_ret);
  }
...
void
RGWPutObjLegalHold_ObjStore_S3::send_response()
{
  if (op_ret) {
    set_req_state_err(s, op_ret);
  }
...
void
RGWGetObjLegalHold_ObjStore_S3::send_response()
{
  if (op_ret) {
    set_req_state_err(s, op_ret);
  }
...
void
RGWGetBucketPolicyStatus_ObjStore_S3::send_response()
{
  if (op_ret) {
    set_req_state_err(s, op_ret);
  }
...
void
RGWPutBucketPublicAccessBlock_ObjStore_S3::send_response()
{
  if (op_ret) {
    set_req_state_err(s, op_ret);
  }
...
void
RGWGetBucketPublicAccessBlock_ObjStore_S3::send_response()
{
  if (op_ret) {
    set_req_state_err(s, op_ret);
  }
...
RGWOp*
RGWHandler_REST_Bucket_S3::get_obj_op(bool get_data) const
{
  // Non-website mode
  if (get_data) {
    int list_type = 1;
...
RGWHandler_REST_Bucket_S3::op_put()
{
  ...
  if (is_tagging_op()) {
    ...
    int ret = driver->get_sync_policy_handler(
        s, nullopt, nullopt, &sync_policy_handler, null_yield);
...
int
RGWHandler_REST_S3::postauth_init(optional_yield y)
{
  struct req_init_state* t = &s->init_state;

  const std::string& auth_tenant = s->auth.identity->get_tenant();

  int ret = rgw_parse_url_bucket(
      t->url_bucket, auth_tenant, s->bucket_tenant, s->bucket_name);
  if (ret) {
    return ret;
  }
  ...
  if (!t->src_bucket.empty()) {
    ret = rgw_parse_url_bucket(
        t->src_bucket, auth_tenant, s->src_tenant_name, s->src_bucket_name);
    if (ret) {
      return ret;
    }
    ...
    rgw::io::BasicClient* cio)
{
  ...
  if (copy_source && (!s->info.env->get("HTTP_X_AMZ_COPY_SOURCE_RANGE")) &&
    ...
    ret = RGWCopyObj::parse_copy_location(
        copy_source, s->init_state.src_bucket, key, s);
    if (!ret) {
...
int
RGWHandler_REST_S3::authorize(const DoutPrefixProvider* dpp, optional_yield y)
{
...
int
RGWHandler_Auth_S3::init(
    rgw::sal::Driver* driver,
    req_state* state,
    rgw::io::BasicClient* cio)
{
  int ret = RGWHandler_REST_S3::init_from_header(
      driver, state, RGWFormat::JSON, true);
  if (ret < 0)
    return ret;
...
public:
  ...
  void
  set(const std::string& type, const std::string& key_or_value)
  {
    if (type == "key") {
      key = key_or_value;
    } else if (type == "value") {
...
void
update_attribute_map(const std::string& input, AttributeMap& map)
{
...
void
parse_post_action(const std::string& post_body, req_state* s)
{
  if (post_body.size() > 0) {
...
bool
RGWHandler_REST_S3Website::web_dir() const
{
...
int
RGWHandler_REST_S3Website::init(
    rgw::sal::Driver* driver,
    req_state* s,
    rgw::io::BasicClient* cio)
{
...
int
RGWHandler_REST_S3Website::retarget(RGWOp* op, RGWOp** new_op, optional_yield y)
{
  *new_op = op;
...
int
RGWHandler_REST_S3Website::serve_errordoc(
    const DoutPrefixProvider* dpp,
    int http_ret,
...
int
RGWHandler_REST_S3Website::error_handler(
    int err_no,
    ...
    optional_yield y)
{
  ...
  if (should_redirect) {
    ...
    int redirect_code = 0;
    rrule.apply_rule(
        protocol, hostname, original_object_name, &s->redirect, &redirect_code);
    // Apply a custom HTTP response code
    if (redirect_code > 0)
      s->err.http_ret = redirect_code; // Apply a custom HTTP response code
    ...
    // Do NOT fire the ErrorDoc handler
      ...
      have_bucket && !s->bucket->get_info().website_conf.error_doc.empty()) {
...
{
  return rgw::auth::s3::get_v4_canonical_headers(
      info, signedheaders, using_qs, false);
}
    ...
    const bool using_qs) const
{
  ...
  auto canonical_method =
      rgw::auth::s3::get_canonical_method(s, s->op_type, s->info);
...
{
  return rgw::auth::s3::get_v4_canonical_headers(
      info, signedheaders, using_qs, true);
}
...
AWSGeneralAbstractor::get_auth_data_v2(const req_state* const s) const
{
  ...
  utime_t header_time;
  if (!rgw_create_s3_canonical_header(
          s, s->op_type, s->info, &header_time, string_to_sign, qsr)) {
...
void
rgw::auth::s3::LDAPEngine::init(CephContext* const cct)
{
  ...
  if (!ldh) {
    ...
    if (!ldh) {
      ...
      ldh = new rgw::LDAPHelper(
          ldap_uri, ldap_binddn, ldap_bindpw, ldap_searchdn, ldap_searchfilter,
          ldap_dnattr);
...
rgw::auth::s3::LDAPEngine::get_creds_info(
    ...
    optional_yield y) const
{
  ...
  auto apl = apl_factory->create_apl_remote(
      cct, s, get_acl_strategy(), get_creds_info(base64_token));
    ...
    optional_yield y) const
{
  ...
  int ret = load_account_and_policies(
      dpp, y, driver, user->get_info(), user->get_attrs(), account, policies);
  if (ret < 0) {
  ...
  if (user->get_info().system) {
    const std::string perm_check_uid = s->info.args.get(RGW_SYS_PARAM_PREFIX
                                                        "perm-check-uid");
    if (!perm_check_uid.empty()) {
      ...
      int ret = load_account_and_policies(
          dpp, y, driver, perm_check_user->get_info(),
      ...
      apl = apl_factory->create_apl_local(
          cct, s, std::move(perm_check_user), std::move(account),
          std::move(policies), rgw::auth::LocalApplier::NO_SUBUSER,
...
rgw::auth::RemoteApplier::AuthInfo
rgw::auth::s3::STSEngine::get_creds_info(
...
int
rgw::auth::s3::STSEngine::get_session_token(
    const DoutPrefixProvider* dpp,
    const std::string_view& session_token,
    ...
    optional_yield y) const
{
  ...
  if (!token.roleId.empty()) {
    ...
    if (!account_id.empty()) {
      ...
      int ret = driver->load_account_by_id(
          dpp, y, account_id, *r.account, attrs, objv);
      if (ret < 0) {
  ...
  if (token.acct_type == TYPE_KEYSTONE || token.acct_type == TYPE_LDAP) {
    auto apl = remote_apl_factory->create_apl_remote(
        cct, s, get_acl_strategy(), get_creds_info(token));
    return result_t::grant(
        std::move(apl), completer_factory(token.secret_access_key));
  } else if (token.acct_type == TYPE_ROLE) {
    t_attrs.user_id = std::move(
        token.user); // This is mostly needed to assign the owner of a bucket during its creation
    ...
    auto apl = role_apl_factory->create_apl_role(
        cct, s, std::move(r), std::move(t_attrs), is_impersonating);
    return result_t::grant(
        std::move(apl), completer_factory(token.secret_access_key));
...
bool
rgw::auth::s3::S3AnonymousEngine::is_applicable(const req_state* s) const noexcept
{
  AwsVersion version;
  AwsRoute route;
  std::tie(version, route) = discover_aws_flavour(s->info);
