...
public:
  ...
  void
  set_custom_http_response(int http_ret)
  {
...
class RGWGetBucketReplication_ObjStore_S3
  : public RGWGetBucketReplication_ObjStore {
public:
  void send_response_data() override;
};

class RGWPutBucketReplication_ObjStore_S3
...
class RGWDeleteBucketReplication_ObjStore_S3
...
class RGWGetBucketEncryption_ObjStore_S3
...
class RGWPutBucketEncryption_ObjStore_S3
...
class RGWGetBucketOwnershipControls_ObjStore_S3
  : public RGWGetBucketOwnershipControls_ObjStore {
public:
  void send_response() override;
};

class RGWPutBucketOwnershipControls_ObjStore_S3
  : public RGWPutBucketOwnershipControls_ObjStore {
  int get_params(optional_yield y) override;
...
class RGWDeleteBucketOwnershipControls_ObjStore_S3
  : public RGWDeleteBucketOwnershipControls_ObjStore {
public:
  void send_response() override;
};
...
class RGWPutBucketObjectLock_ObjStore_S3
...
class RGWGetBucketPublicAccessBlock_ObjStore_S3
...
public:
  explicit RGWHandler_Auth_S3(const rgw::auth::StrategyRegistry& auth_registry) :
    RGWHandler_REST(), auth_registry(auth_registry)
  {}
  ...
  int
  authorize(const DoutPrefixProvider* dpp, optional_yield y) override
  {
...
protected:
  ...
  bool
  is_obj_update_op() const override
  {
  ...
  bool
  is_object_lock_op() const
  {
    return s->info.args.exists("object-lock");
  }

  bool
  is_notification_op() const
  {
    if (enable_pubsub) {
...
protected:
  ...
  bool
  is_obj_retention_op() const
  {
    return s->info.args.exists("retention");
  }

  bool
  is_obj_legal_hold_op() const
  {
  ...
  is_obj_update_op() const override
  {
    return is_acl_op() || is_tagging_op() || is_obj_retention_op() ||
           is_obj_legal_hold_op() || is_select_op();
  }
...
public:
  explicit RGWRESTMgr_S3(
      bool _enable_s3website = false,
      bool _enable_sts = false,
      bool _enable_iam = false,
...
static inline bool
looks_like_ip_address(const char* bucket)
{
  struct in6_addr a;
...
inline int
valid_s3_object_name(const std::string& name)
{
  if (name.size() > 1024) {
...
inline int
valid_s3_bucket_name(const std::string& name, bool relaxed = false)
{
...
public:
  ...
  public:
    ...
    using signature_factory_t = std::function<server_signature_t(
        CephContext* cct,
...
protected:
  ...
  AWSEngine(CephContext* const cct, const VersionAbstractor& ver_abstractor) :
    cct(cct), ver_abstractor(ver_abstractor)
  {}
...
class AWSGeneralAbstractor : public AWSEngine::VersionAbstractor {
  ...
  virtual boost::optional<std::string> get_v4_canonical_headers(
      const req_info& info,
...
class AWSBrowserUploadAbstractor : public AWSEngine::VersionAbstractor {
  ...
  {
    return std::string(
        bl.c_str(), static_cast<std::string::size_type>(bl.length()));
  }
...
public:
  ...
  static prepare_result_t prepare(
      const DoutPrefixProvider* dpp,
...
extern AWSSignerV4::signature_headers_t gen_v4_signature(
    const DoutPrefixProvider* dpp,
...
public:
  LDAPEngine(
      ...
      const VersionAbstractor& ver_abstractor,
      const rgw::auth::RemoteApplier::Factory* const apl_factory) :
    AWSEngine(cct, ver_abstractor), driver(driver), apl_factory(apl_factory)
  {
    init(cct);
  }
