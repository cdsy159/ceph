...
class FormPostSignatureT
  : public rgw::auth::swift::FormatSignature<HASHFLAVOR, SIGNATUREFLAVOR> {
  using UCHARPTR = const unsigned char*;
  using base_t = rgw::auth::swift::SignatureHelperT<HASHFLAVOR>;
  using format_signature_t =
      rgw::auth::swift::FormatSignature<HASHFLAVOR, SIGNATUREFLAVOR>;
...
public:
  ...
  virtual bool
  is_equal_to(const std::string& rhs)
  {
    return false;
  };
...
public:
  ...
  {
    return d.calc(
        key, path_info, redirect, max_file_size, max_file_count, expires);
  };
...
std::unique_ptr<RGWFormPost::SignatureHelper>
RGWFormPost::SignatureHelper::get_sig_helper(std::string_view x)
{
  size_t pos = x.find(':');
  if (pos == x.npos || pos <= 0) {
    switch (x.length()) {
    case CEPH_CRYPTO_HMACSHA1_DIGESTSIZE * 2:
      return std::make_unique<SignatureHelper_x<
          ceph::crypto::HMACSHA1, rgw::auth::swift::SignatureFlavor::BARE_HEX>>();
    case CEPH_CRYPTO_HMACSHA256_DIGESTSIZE * 2:
      return std::make_unique<SignatureHelper_x<
          ceph::crypto::HMACSHA256, rgw::auth::swift::SignatureFlavor::BARE_HEX>>();
    case CEPH_CRYPTO_HMACSHA512_DIGESTSIZE * 2:
      return std::make_unique<SignatureHelper_x<
          ceph::crypto::HMACSHA512, rgw::auth::swift::SignatureFlavor::BARE_HEX>>();
    }
  ...
  if (type == "sha1") {
    return std::make_unique<SignatureHelper_x<
        ceph::crypto::HMACSHA1, rgw::auth::swift::SignatureFlavor::NAMED_BASE64>>();
  } else if (type == "sha256") {
    return std::make_unique<SignatureHelper_x<
        ceph::crypto::HMACSHA256,
        rgw::auth::swift::SignatureFlavor::NAMED_BASE64>>();
  } else if (type == "sha512") {
    return std::make_unique<SignatureHelper_x<
        ceph::crypto::HMACSHA512,
        rgw::auth::swift::SignatureFlavor::NAMED_BASE64>>();
  }
  return std::make_unique<BadSignatureHelper>();
};
...
int
RGWListBuckets_ObjStore_SWIFT::get_params(optional_yield y)
{
    ...
    const RGWAccessControlPolicy& policy)
{
  ...
  for (const auto& kv : policies_stats) {
    ...
    dump_header_infixed(
        s, "X-Account-Storage-Policy-", policy_name, "-Container-Count",
        policy_stats.buckets_count);
    dump_header_infixed(
        s, "X-Account-Storage-Policy-", policy_name, "-Object-Count",
        policy_stats.objects_count);
    dump_header_infixed(
        s, "X-Account-Storage-Policy-", policy_name, "-Bytes-Used",
        policy_stats.bytes_used);
    dump_header_infixed(
        s, "X-Account-Storage-Policy-", policy_name, "-Bytes-Used-Actual",
        policy_stats.bytes_used_rounded);
  }

  /* Dump TempURL-related stuff */
  if (s->perm_mask == RGW_PERM_FULL_CONTROL) {
    auto iter = s->user->get_info().temp_url_keys.find(0);
    if (iter != std::end(s->user->get_info().temp_url_keys) &&
        !iter->second.empty()) {
  ...
  for (iter = attrs.lower_bound(RGW_ATTR_PREFIX); iter != attrs.end(); ++iter) {
    ...
    if (geniter != rgw_to_http_attrs.end()) {
      ...
      dump_header_prefixed(
          s, "X-Account-Meta-",
          camelcase_dash_http_attr(name + PREFIX_LEN, false), iter->second);
    }
  }
...
void
RGWListBuckets_ObjStore_SWIFT::send_response_begin(bool has_buckets)
{
  if (op_ret) {
  ...
  if (!s->cct->_conf->rgw_swift_enforce_content_length) {
    ...
    dump_account_metadata(
        s, global_stats, policies_stats, s->user->get_attrs(),
        s->user->get_info().quota.user_quota, s->user->get_max_buckets(),
        s->user_acl);
    dump_errno(s);
  ...
  if (!op_ret) {
    dump_start(s);
    s->formatter->open_array_section_with_attrs(
        "account",
...
static bool
bucket_prefix_less(const RGWBucketEnt& e, std::string_view p)
{
  return e.bucket.name < p;
}

void
RGWListBuckets_ObjStore_SWIFT::handle_listing_chunk(
    std::span<RGWBucketEnt> buckets)
{
  if (!wants_reversed) {
  ...
  if (prefix.empty()) {
    reverse_buffer.insert(
        reverse_buffer.begin(), std::make_move_iterator(buckets.rbegin()),
  ...
  auto i = std::lower_bound(
      buckets.begin(), buckets.end(), prefix, bucket_prefix_less);
  for (; i != buckets.end() &&
         boost::algorithm::starts_with(i->bucket.name, prefix);
       ++i) {
...
void
RGWListBuckets_ObjStore_SWIFT::send_response_data(
...
void
RGWListBuckets_ObjStore_SWIFT::dump_bucket_entry(const RGWBucketEnt& ent)
{
...
int
RGWListBucket_ObjStore_SWIFT::get_params(optional_yield y)
{
  prefix = s->info.args.get("prefix");
  marker = s->info.args.get("marker");
  end_marker = s->info.args.get("end_marker");
...
static void dump_container_metadata(
    req_state*,
...
RGWListBucket_ObjStore_SWIFT::send_response()
{
  ...
  dump_start(s);
  dump_container_metadata(
      s, stats, quota.bucket_quota, s->bucket->get_info().website_conf);

  s->formatter->open_array_section_with_attrs(
      "container", FormatterAttrs("name", s->bucket->get_name().c_str(), NULL));
  ...
  while (iter != objs.end() || pref_iter != common_prefixes.end()) {
    ...
    if (do_pref &&
      ...
      s->formatter->open_object_section_with_attrs(
          "subdir", FormatterAttrs("name", name.c_str(), NULL));
    ...
    const RGWBucketWebsiteConf& ws_conf)
{
  ...
  if (rgw::sal::Object::empty(s->object.get())) {
    ...
    dump_header(
        s, "X-Storage-Class",
        s->bucket->get_placement_rule().get_storage_class());
...
void
RGWStatAccount_ObjStore_SWIFT::execute(optional_yield y)
{
...
void
RGWStatAccount_ObjStore_SWIFT::send_response()
{
  if (op_ret >= 0) {
    op_ret = STATUS_NO_CONTENT;
    dump_account_metadata(
        s, global_stats, policies_stats, attrs,
        s->user->get_info().quota.user_quota, s->user->get_max_buckets(),
        s->user_acl);
  }
...
void
RGWStatBucket_ObjStore_SWIFT::send_response()
{
  if (op_ret >= 0) {
    op_ret = STATUS_NO_CONTENT;
    dump_container_metadata(
    ...
    bool* const has_cors)
{
  ...
  *has_policy = false;
  ...
  if (read_list || write_list) {
    int r = rgw::swift::create_container_policy(
        s, driver, s->owner, read_list, write_list, *rw_mask, policy);
    if (r < 0) {
      return r;
    }
  ...
  *has_cors = false;
  ...
  const char* allow_origins =
      s->info.env->get("HTTP_X_CONTAINER_META_ACCESS_CONTROL_ALLOW_ORIGIN");
  const char* allow_headers =
      s->info.env->get("HTTP_X_CONTAINER_META_ACCESS_CONTROL_ALLOW_HEADERS");
  const char* expose_headers =
      s->info.env->get("HTTP_X_CONTAINER_META_ACCESS_CONTROL_EXPOSE_HEADERS");
  const char* max_age =
      s->info.env->get("HTTP_X_CONTAINER_META_ACCESS_CONTROL_MAX_AGE");
  if (allow_origins) {
    RGWCORSConfiguration_SWIFT* swift_cors = new RGWCORSConfiguration_SWIFT;
    int r = swift_cors->create_update(
        allow_origins, allow_headers, expose_headers, max_age);
    if (r < 0) {
      ldpp_dout(s, 0) << "Error creating/updating the cors configuration"
                      << dendl;
      delete swift_cors;
      return r;
    }
  ...
  for (const auto& kv : s->info.env->get_map()) {
    ...
    if (strncasecmp(p, del_prefix, del_prefix_len) == 0) {
      ...
      prefix_len = del_prefix_len;
        ...
        (strncasecmp(p, put_prefix, put_prefix_len) == 0) && kv.second.empty()) {
...
int
RGWCreateBucket_ObjStore_SWIFT::get_params(optional_yield y)
{
  int r = get_swift_container_settings(
      s, driver, policy, &has_policy, &policy_rw_mask, &cors_config, &has_cors);
  if (r < 0) {
    return r;
  }
  ...
  get_rmattrs_from_headers(
      s, CONT_PUT_ATTR_PREFIX, CONT_REMOVE_ATTR_PREFIX, rmattr_names);
  createparams.placement_rule.init(
      s->info.env->get("HTTP_X_STORAGE_POLICY", ""), s->info.storage_class);

  return get_swift_versioning_settings(s, createparams.swift_ver_location);
}

static inline int
handle_metadata_errors(req_state* const s, const int op_ret)
{
  if (op_ret == -EFBIG) {
...
static int
get_delete_at_param(req_state* s, boost::optional<real_time>& delete_at)
{
...
int
RGWPutObj_ObjStore_SWIFT::verify_permission(optional_yield y)
{
...
int
RGWPutObj_ObjStore_SWIFT::update_slo_segment_size(rgw_slo_entry& entry)
{
  ...
  if (bucket_name.compare(s->bucket->get_name()) != 0) {
    r = driver->load_bucket(
        s, rgw_bucket(s->user->get_tenant(), bucket_name), &bucket, s->yield);
    if (r < 0) {
  ...
  std::unique_ptr<rgw::sal::Object> slo_seg =
      bucket->get_object(rgw_obj_key(obj_name));
  ...
  r = rgw_compression_info_from_attrset(
      slo_seg->get_attrs(), compressed, cs_info);
  if (r < 0) {
    return -EIO;
  }
...
int
RGWPutObj_ObjStore_SWIFT::get_params(optional_yield y)
{
  if (s->has_bad_meta) {
    return -EINVAL;
  }
  ...
  if (!s->generic_attrs.count(RGW_ATTR_CONTENT_TYPE)) {
    ldpp_dout(this, 5) << "content type wasn't provided, trying to guess"
                       << dendl;
  ...
  if (exists) {
    if (multipart_manifest != "put") {
      ldpp_dout(this, 5) << "invalid multipart-manifest http param: "
                         << multipart_manifest << dendl;
      return -EINVAL;
    }

#define MAX_SLO_ENTRY_SIZE \
  (1024 + 128) // 1024 - max obj name, 128 - enough extra for other info
    uint64_t max_len = s->cct->_conf->rgw_max_slo_entries * MAX_SLO_ENTRY_SIZE;

    slo_info = new RGWSLOInfo;

    int r = 0;
    std::tie(r, slo_info->raw_data) = rgw_rest_get_json_input_keep_data(
        s->cct, s, slo_info->entries, max_len);
    if (r < 0) {
    ...
    if ((int64_t)slo_info->entries.size() > s->cct->_conf->rgw_max_slo_entries) {
      ldpp_dout(this, 5) << "too many entries in slo request: "
                         << slo_info->entries.size() << dendl;
      return -EINVAL;
    }
    ...
    bool* const has_policy)
{
  *has_policy = false;
  ...
  const char* const acl_attr =
      s->info.env->get("HTTP_X_ACCOUNT_ACCESS_CONTROL");
  if (acl_attr) {
    int r = rgw::swift::create_account_policy(
        s, driver, s->owner, acl_attr, policy);
    if (r < 0) {
      return r;
    }
...
int
RGWPutMetadataAccount_ObjStore_SWIFT::get_params(optional_yield y)
{
  if (s->has_bad_meta) {
    return -EINVAL;
  }
  ...
  get_rmattrs_from_headers(
      s, ACCT_PUT_ATTR_PREFIX, ACCT_REMOVE_ATTR_PREFIX, rmattr_names);
  return 0;
}

void
RGWPutMetadataAccount_ObjStore_SWIFT::send_response()
{
  const auto meta_ret = handle_metadata_errors(s, op_ret);
  if (meta_ret != op_ret) {
    op_ret = meta_ret;
...
int
RGWPutMetadataBucket_ObjStore_SWIFT::get_params(optional_yield y)
{
  if (s->has_bad_meta) {
    return -EINVAL;
  }
...
void
RGWPutMetadataBucket_ObjStore_SWIFT::send_response()
{
  const auto meta_ret = handle_metadata_errors(s, op_ret);
  if (meta_ret != op_ret) {
    op_ret = meta_ret;
...
int
RGWPutMetadataObject_ObjStore_SWIFT::get_params(optional_yield y)
{
  if (s->has_bad_meta) {
    return -EINVAL;
  }
...
void
RGWPutMetadataObject_ObjStore_SWIFT::send_response()
{
  const auto meta_ret = handle_metadata_errors(s, op_ret);
  if (meta_ret != op_ret) {
    op_ret = meta_ret;
...
int
RGWDeleteObj_ObjStore_SWIFT::verify_permission(optional_yield y)
{
...
int
RGWDeleteObj_ObjStore_SWIFT::get_params(optional_yield y)
{
...
RGWDeleteObj_ObjStore_SWIFT::send_response()
{
  ...
  if (multipart_delete) {
    end_header(
        s, this /* RGWOp */, nullptr /* contype */, CHUNKED_TRANSFER_ENCODING);

    if (deleter) {
      bulkdelete_respond(
          deleter->get_num_deleted(), deleter->get_num_unfound(),
          deleter->get_failures(), s->prot_flags, *s->formatter);
...
static void
get_contype_from_attrs(map<string, bufferlist>& attrs, string& content_type)
{
    ...
    const map<string, bufferlist>& attrs)
{
  ...
  for (auto kv : attrs) {
    ...
    if (aiter != std::end(rgw_to_http_attrs)) {
      ...
      dump_header_prefixed(
          s, "X-Object-Meta-", camelcase_dash_http_attr(name, false), kv.second);
    }
  }
...
int
RGWCopyObj_ObjStore_SWIFT::init_dest_policy()
{
...
int
RGWCopyObj_ObjStore_SWIFT::get_params(optional_yield y)
{
...
void
RGWCopyObj_ObjStore_SWIFT::send_partial_response(off_t ofs)
{
  if (!sent_header) {
    if (!op_ret)
      op_ret = STATUS_CREATED;
...
void
RGWCopyObj_ObjStore_SWIFT::dump_copy_info()
{
  /* Dump X-Copied-From. */
  dump_header(
      s, "X-Copied-From",
      url_encode(src_bucket->get_name()) + "/" +
...
RGWCopyObj_ObjStore_SWIFT::send_response()
{
  if (!sent_header) {
    ...
    end_header(
        s, this,
        !content_type.empty() ? content_type.c_str() : "binary/octet-stream");
  } else {
    s->formatter->close_section();
    rgw_flush_formatter(s, s->formatter);
  }
...
int
RGWGetObj_ObjStore_SWIFT::verify_permission(optional_yield y)
{
...
int
RGWGetObj_ObjStore_SWIFT::get_params(optional_yield y)
{
  const string& mm = s->info.args.get("multipart-manifest");
...
int
RGWGetObj_ObjStore_SWIFT::send_response_data_error(optional_yield y)
{
...
void
RGWOptionsCORS_ObjStore_SWIFT::send_response()
{
  ...
  while (cioin.getline(buf, sizeof(buf))) {
    ...
    if (string::npos != start_pos) {
      ...
      if (string::npos != sep_pos) {
        path.bucket_name =
            url_decode(path_str.substr(start_pos, sep_pos - start_pos));
...
void
RGWBulkDelete_ObjStore_SWIFT::send_response()
{
  set_req_state_err(s, op_ret);
  dump_errno(s);
  end_header(
...
RGWBulkUploadOp_ObjStore_SWIFT::create_stream()
{
  ...
  public:
    SwiftStreamGetter(
        ...
        req_state* const s,
        const size_t conlen) :
      dpp(dpp), conlen(conlen), curpos(0), s(s)
    {}

    ssize_t
    get_at_most(size_t want, ceph::bufferlist& dst) override
    {
      ...
      const size_t max_to_read =
          std::min({want, conlen - curpos, max_chunk_size});

      ldpp_dout(dpp, 20) << "bulk_upload: get_at_most max_to_read="
                         << max_to_read << ", dst.c_str()="
                         << reinterpret_cast<intptr_t>(dst.c_str()) << dendl;
    ...
    ssize_t
    get_exactly(size_t want, ceph::bufferlist& dst) override
    {
  ...
  if (!s->length) {
    ...
    ldpp_dout(this, 20) << "bulk upload: create_stream for length=" << s->length
                        << dendl;

    const size_t conlen = atoll(s->length);
    return std::unique_ptr<SwiftStreamGetter>(
        new SwiftStreamGetter(this, s, conlen));
  }
}

void
RGWBulkUploadOp_ObjStore_SWIFT::send_response()
{
  set_req_state_err(s, op_ret);
  dump_errno(s);
  end_header(
...
void
RGWGetCrossDomainPolicy_ObjStore_SWIFT::send_response()
{
  set_req_state_err(s, op_ret);
  dump_errno(s);
...
void
RGWGetHealthCheck_ObjStore_SWIFT::send_response()
{
  set_req_state_err(s, op_ret);
  dump_errno(s);
  end_header(s, this, "application/xml");
...
const vector<pair<string, RGWInfo_ObjStore_SWIFT::info>>
    RGWInfo_ObjStore_SWIFT::swift_info = {
...
void
RGWInfo_ObjStore_SWIFT::execute(optional_yield y)
{
  ...
  if (!swiftinfo_sig.empty() && !swiftinfo_expires.empty() &&
...
void
RGWInfo_ObjStore_SWIFT::send_response()
{
  if (op_ret < 0) {
    op_ret = STATUS_NO_CONTENT;
  }
...
void
RGWInfo_ObjStore_SWIFT::list_bulk_delete(
    Formatter& formatter,
    ...
    rgw::sal::Driver* driver)
{
  ...
  formatter.dump_int(
      "max_deletes_per_request", config->rgw_delete_multi_obj_max_num);
  formatter.close_section();
}

void
RGWInfo_ObjStore_SWIFT::list_swift_data(
    Formatter& formatter,
    const ConfigProxy& config,
    rgw::sal::Driver* driver)
{
  ...
  if (max_attr_name_len) {
    const size_t meta_name_limit = max_attr_name_len -
                                   strlen(RGW_ATTR_PREFIX RGW_AMZ_META_PREFIX);
    formatter.dump_int("max_meta_name_length", meta_name_limit);
  }

  const size_t meta_value_limit =
      g_conf().get_val<Option::size_t>("rgw_max_attr_size");
  if (meta_value_limit) {
  ...
  formatter.dump_int(
      "max_container_name_length", RGWHandler_REST::MAX_BUCKET_NAME_LEN);
  formatter.close_section();
}

void
RGWInfo_ObjStore_SWIFT::list_tempauth_data(
    Formatter& formatter,
    const ConfigProxy& config,
    rgw::sal::Driver* driver)
{
...
void
RGWInfo_ObjStore_SWIFT::list_tempurl_data(
    Formatter& formatter,
    const ConfigProxy& config,
    rgw::sal::Driver* driver)
{
...
void
RGWInfo_ObjStore_SWIFT::list_slo_data(
    Formatter& formatter,
    const ConfigProxy& config,
    rgw::sal::Driver* driver)
{
...
bool
RGWInfo_ObjStore_SWIFT::is_expired(
    ...
    const DoutPrefixProvider* dpp)
{
  ...
  const uint64_t expiration =
      (uint64_t)strict_strtoll(expires.c_str(), 10, &err);
  if (!err.empty()) {
  ...
  if (expiration <= (uint64_t)now.sec()) {
    ldpp_dout(dpp, 5) << "siginfo expired: " << expiration
                      << " <= " << now.sec() << dendl;
    return true;
  }
...
std::size_t
RGWFormPost::get_max_file_size() /*const*/
{
...
RGWFormPost::is_non_expired()
{
  ...
  if (!err.empty()) {
    ldpp_dout(this, 5) << "failed to parse FormPost's expires: " << err
                       << dendl;
    return false;
  }
  ...
  if (std::cmp_less_equal(expires_timestamp, now.sec())) {
    ldpp_dout(this, 5) << "FormPost form expired: " << expires_timestamp
                       << " <= " << now.sec() << dendl;
    return false;
  }
...
RGWFormPost::is_integral()
{
  ...
  for (const auto& kv : s->user->get_info().temp_url_keys) {
    ...
    auto sig_helper{
        RGWFormPost::SignatureHelper::get_sig_helper(form_signature)};
    sig_helper->calc(
        temp_url_key, s->info.request_uri, get_part_str(ctrl_parts, "redirect"),
    ...
    if (!r) {
      ldpp_dout(this, 5) << "FormPost's signature mismatch: " << local_sig
                         << " != " << form_signature << dendl;
    }
    if (r) {
      break;
    }
...
auto
RGWFormPost::get_owner_info(
    const req_state* const s) const -> std::unique_ptr<rgw::sal::User>
{
  ...
  int ret = driver->load_bucket(
      s, rgw_bucket(bucket_tenant, bucket_name), &bucket, s->yield);
  if (ret < 0) {
    throw ret;
  }
  ...
  if (!uid) {
    ldpp_dout(this, 20) << "bucket " << *bucket
                        << " is not owned by a user "
                           "so has no temp url keys"
                        << dendl;
    throw -EPERM;
  }
...
int
RGWFormPost::get_params(optional_yield y)
{
  ...
  do {
    ...
    if (std::end(part.fields) != field_iter &&
        std::end(field_iter->second.params) !=
            field_iter->second.params.find("filename")) {
      ...
      bool boundary;
      ret = read_data(
          part.data, s->cct->_conf->rgw_max_chunk_size, boundary, stream_done);
      if (ret < 0) {
        return ret;
      } else if (!boundary) {
...
std::string
RGWFormPost::get_current_filename() const
{
  try {
...
std::string
RGWFormPost::get_current_content_type() const
{
  try {
...
bool
RGWFormPost::is_next_file_to_upload()
{
  if (!stream_done) {
...
int
RGWFormPost::get_data(ceph::bufferlist& bl, bool& again)
{
  bool boundary;

  int r =
      read_data(bl, s->cct->_conf->rgw_max_chunk_size, boundary, stream_done);
  if (r < 0) {
    return r;
  }
...
bool
RGWFormPost::is_formpost_req(req_state* const s)
{
  std::string content_type;
  std::map<std::string, std::string> params;

  parse_boundary_params(
      s->info.env->get("CONTENT_TYPE", ""), content_type, params);
...
RGWOp*
RGWHandler_REST_Service_SWIFT::op_get()
{
  return new RGWListBuckets_ObjStore_SWIFT;
}

RGWOp*
RGWHandler_REST_Service_SWIFT::op_head()
{
  return new RGWStatAccount_ObjStore_SWIFT;
}

RGWOp*
RGWHandler_REST_Service_SWIFT::op_put()
{
...
RGWOp*
RGWHandler_REST_Service_SWIFT::op_post()
{
...
RGWOp*
RGWHandler_REST_Service_SWIFT::op_delete()
{
  if (s->info.args.exists("bulk-delete")) {
    return new RGWBulkDelete_ObjStore_SWIFT;
  }
...
int
RGWSwiftWebsiteHandler::serve_errordoc(
    const int http_ret,
    ...
    optional_yield y)
{
  ...
  if (!rgw::sal::Bucket::empty(s->bucket.get())) {
    s->object = s->bucket->get_object(
        rgw_obj_key(std::to_string(http_ret) + error_doc));
  } else {
    s->object =
        driver->get_object(rgw_obj_key(std::to_string(http_ret) + error_doc));
  }
...
int
RGWSwiftWebsiteHandler::error_handler(
    const int err_no,
    std::string* const error_content,
    optional_yield y)
{
...
bool
RGWSwiftWebsiteHandler::is_web_mode() const
{
...
bool
RGWSwiftWebsiteHandler::can_be_website_req() const
{
...
RGWOp*
RGWSwiftWebsiteHandler::get_ws_redirect_op()
{
...
RGWOp*
RGWSwiftWebsiteHandler::get_ws_index_op()
{
...
bool
RGWSwiftWebsiteHandler::is_web_dir() const
{
  ...
  std::unique_ptr<rgw::sal::Object> obj =
      s->bucket->get_object(rgw_obj_key(std::move(subdir_name)));
...
bool
RGWSwiftWebsiteHandler::is_index_present(const std::string& index) const
{
  std::unique_ptr<rgw::sal::Object> obj =
      s->bucket->get_object(rgw_obj_key(index));
...
int
RGWSwiftWebsiteHandler::retarget_bucket(RGWOp* op, RGWOp** new_op)
{
...
int
RGWSwiftWebsiteHandler::retarget_object(RGWOp* op, RGWOp** new_op)
{
...
RGWOp*
RGWHandler_REST_Bucket_SWIFT::get_obj_op(bool get_data)
{
  if (is_acl_op()) {
...
RGWOp*
RGWHandler_REST_Bucket_SWIFT::op_get()
{
  return get_obj_op(true);
}

RGWOp*
RGWHandler_REST_Bucket_SWIFT::op_head()
{
  return get_obj_op(false);
}

RGWOp*
RGWHandler_REST_Bucket_SWIFT::op_put()
{
  if (is_acl_op()) {
...
RGWOp*
RGWHandler_REST_Bucket_SWIFT::op_delete()
{
  return new RGWDeleteBucket_ObjStore_SWIFT;
}

RGWOp*
RGWHandler_REST_Bucket_SWIFT::op_post()
{
...
RGWOp*
RGWHandler_REST_Bucket_SWIFT::op_options()
{
  return new RGWOptionsCORS_ObjStore_SWIFT;
}

RGWOp*
RGWHandler_REST_Obj_SWIFT::get_obj_op(bool get_data)
{
  if (is_acl_op()) {
    return new RGWGetACLs_ObjStore_SWIFT;
  }
...
RGWOp*
RGWHandler_REST_Obj_SWIFT::op_get()
{
  return get_obj_op(true);
}

RGWOp*
RGWHandler_REST_Obj_SWIFT::op_head()
{
  return get_obj_op(false);
}

RGWOp*
RGWHandler_REST_Obj_SWIFT::op_put()
{
  if (is_acl_op()) {
    return new RGWPutACLs_ObjStore_SWIFT;
  }
...
RGWOp*
RGWHandler_REST_Obj_SWIFT::op_delete()
{
  return new RGWDeleteObj_ObjStore_SWIFT;
}

RGWOp*
RGWHandler_REST_Obj_SWIFT::op_post()
{
  if (RGWFormPost::is_formpost_req(s)) {
    return new RGWFormPost;
  } else {
...
RGWOp*
RGWHandler_REST_Obj_SWIFT::op_copy()
{
  return new RGWCopyObj_ObjStore_SWIFT;
}

RGWOp*
RGWHandler_REST_Obj_SWIFT::op_options()
{
  return new RGWOptionsCORS_ObjStore_SWIFT;
}

int
RGWHandler_REST_SWIFT::authorize(const DoutPrefixProvider* dpp, optional_yield y)
{
  return rgw::auth::Strategy::apply(dpp, auth_strategy, s, y);
}

int
RGWHandler_REST_SWIFT::postauth_init(optional_yield y)
{
  ...
  ldpp_dout(s, 10)
      ...
      << " s->bucket="
      << rgw_make_bucket_entry_name(s->bucket_tenant, s->bucket_name) << dendl;
...
int
RGWHandler_REST_SWIFT::validate_bucket_name(const string& bucket)
{
  ...
  if (len > MAX_BUCKET_NAME_LEN) {
    ...
    const auto msg = boost::str(
        boost::format("Container name length of %lld longer than %lld") % len %
...
static void
next_tok(string& str, string& tok, char delim)
{
  if (str.size() == 0) {
    tok = "";
    return;
...
int
RGWHandler_REST_SWIFT::init_from_header(
    rgw::sal::Driver* driver,
    req_state* const s,
    const std::string& frontend_prefix)
{
  ...
  if (g_conf()->rgw_swift_url_prefix == "/") {
    ...
    blen = sprintf(
        buf, "/%s/v1%s", g_conf()->rgw_swift_url_prefix.c_str(),
        tenant_path.c_str());
  }
  ...
  ldpp_dout(s, 10) << "ver=" << ver << " first=" << first << " req=" << req
                   << dendl;
  if (first.size() == 0)
    return 0;
  ...
  if (req.size()) {
    s->object = driver->get_object(rgw_obj_key(
        req, s->info.env->get(
                 "HTTP_X_OBJECT_VERSION_ID", ""))); /* rgw swift extension */
    ...
    rgw::io::BasicClient* cio)
{
  ...
  if (!copy_source.empty()) {
    rgw_obj_key key;
    bool result =
        RGWCopyObj::parse_copy_location(copy_source, t->src_bucket, key, s);
    if (!result)
      return -ERR_BAD_URL;
  ...
  if (s->op == OP_COPY) {
    ...
    bool result = RGWCopyObj::parse_copy_location(
        req_dest, dest_bucket_name, dest_obj_key, s);
    if (!result)
      return -ERR_BAD_URL;
...
RGWHandler_REST*
RGWRESTMgr_SWIFT_Info::get_handler(
    rgw::sal::Driver* driver,
    req_state* const s,
    const rgw::auth::StrategyRegistry& auth_registry,
    const std::string& frontend_prefix)
