...
public:
  ...
  void
  set_custom_http_response(const int http_ret)
  {
    custom_http_ret = http_ret;
  }

  bool
  need_object_expiration() override
  {
    return true;
  }
};
...
class RGWListBuckets_ObjStore_SWIFT : public RGWListBuckets_ObjStore {
  ...
  uint64_t
  get_default_max() const override
  {
    return 0;
  }
...
public:
  ...
  bool
  should_get_stats() override
  {
    return need_stats;
  }

  bool
  supports_account_metadata() override
  {
    return true;
  }
};
...
public:
  ...
  int
  get_params(optional_yield y) override
  {
    return 0;
  }
...
protected:
  bool
  need_metadata_upload() const override
  {
    return true;
  }
...
class RGWPutMetadataBucket_ObjStore_SWIFT
...
class RGWPutMetadataObject_ObjStore_SWIFT
...
public:
  ...
  int get_data(
      std::list<RGWBulkDelete::acct_path_t>& items,
...
protected:
  struct info {
    bool is_admin_info;
    std::function<void(Formatter&, const ConfigProxy&, rgw::sal::Driver*)>
        list_data;
  };
...
public:
  ...
  void execute(optional_yield y) override;
  void send_response() override;
  static void list_bulk_delete(
      Formatter& formatter,
      const ConfigProxy& config,
      rgw::sal::Driver* driver);
  static void list_swift_data(
      Formatter& formatter,
      const ConfigProxy& config,
      rgw::sal::Driver* driver);
  static void list_tempauth_data(
      Formatter& formatter,
      const ConfigProxy& config,
      rgw::sal::Driver* driver);
  static void list_tempurl_data(
      Formatter& formatter,
      const ConfigProxy& config,
      rgw::sal::Driver* driver);
  static void list_slo_data(
      Formatter& formatter,
      const ConfigProxy& config,
      rgw::sal::Driver* driver);
...
public:
  ...
  int error_handler(int err_no, std::string* error_content, optional_yield y)
      override;
  void send_response() override;
...
public:
  RGWSwiftWebsiteHandler(
      ...
      req_state* const s,
      RGWHandler_REST* const handler) :
    driver(driver), s(s), handler(handler)
  {}
...
public:
  ...
  alloc_policy()
  {
    return nullptr; /* return new RGWAccessControlPolicy_SWIFT; */
  }
...
public:
  ...
  int
  error_handler(int err_no, std::string* error_content, optional_yield y) override
  {
    return website_handler->error_handler(err_no, error_content, y);
  }

  int
  retarget(RGWOp* op, RGWOp** new_op, optional_yield) override
  {
...
public:
  ...
  int
  authorize(const DoutPrefixProvider* dpp, optional_yield) override
  {
    return 0;
  }
