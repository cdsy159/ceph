...
void
RGWRestUserPolicy::send_response()
{
  if (op_ret) {
...
int
RGWRestUserPolicy::init_processing(optional_yield y)
{
  int r = get_params();
  if (r < 0) {
    return r;
  ...
  if (const auto* id = std::get_if<rgw_account_id>(&s->owner.id); id) {
    ...
    r = driver->load_account_user_by_name(
        this, y, account_id, tenant, user_name, &user);
...
int
RGWRestUserPolicy::check_caps(const RGWUserCaps& caps)
{
  return caps.check_cap("user-policy", perm);
}

int
RGWRestUserPolicy::verify_permission(optional_yield y)
{
...
int
RGWPutUserPolicy::forward_to_master(optional_yield y, const rgw::SiteConfig& site)
{
...
void
RGWPutUserPolicy::execute(optional_yield y)
{
  ...
  if (op_ret == 0) {
    s->formatter->open_object_section_in_ns(
        "PutUserPolicyResponse", RGW_REST_IAM_XMLNS);
...
void
RGWGetUserPolicy::execute(optional_yield y)
{
  std::map<std::string, std::string> policies;
  if (auto it = user->get_attrs().find(RGW_ATTR_USER_POLICY);
      it != user->get_attrs().end()) {
    try {
      decode(policies, it->second);
  ...
  s->formatter->open_object_section_in_ns(
      "GetUserPolicyResponse", RGW_REST_IAM_XMLNS);
  s->formatter->open_object_section("ResponseMetadata");
  s->formatter->dump_string("RequestId", s->trans_id);
  s->formatter->close_section();
...
int
RGWListUserPolicies::get_params()
{
...
void
RGWListUserPolicies::execute(optional_yield y)
{
  std::map<std::string, std::string> policies;
  if (auto it = user->get_attrs().find(RGW_ATTR_USER_POLICY);
      it != user->get_attrs().end()) {
  ...
  s->formatter->open_object_section_in_ns(
      "ListUserPoliciesResponse", RGW_REST_IAM_XMLNS);
  s->formatter->open_object_section("ResponseMetadata");
  s->formatter->dump_string("RequestId", s->trans_id);
  s->formatter->close_section();
...
int
RGWDeleteUserPolicy::get_params()
{
  policy_name = s->info.args.get("PolicyName");
  if (!validate_iam_policy_name(policy_name, s->err.message)) {
    return -EINVAL;
...
int
RGWDeleteUserPolicy::forward_to_master(
    optional_yield y,
...
void
RGWDeleteUserPolicy::execute(optional_yield y)
{
  const rgw::SiteConfig& site = *s->penv.site;
  if (!site.is_meta_master()) {
    op_ret = forward_to_master(y, site);
  ...
  s->formatter->open_object_section_in_ns(
      "DeleteUserPoliciesResponse", RGW_REST_IAM_XMLNS);
  s->formatter->open_object_section("ResponseMetadata");
  s->formatter->dump_string("RequestId", s->trans_id);
  s->formatter->close_section();
  s->formatter->close_section();
...
public:
  ...
  name() const override
  {
    return "attach_user_policy";
  }

  RGWOpType
  get_type() override
  {
    return RGW_OP_ATTACH_USER_POLICY;
  }
};

int
RGWAttachUserPolicy_IAM::get_params()
{
...
int
RGWAttachUserPolicy_IAM::forward_to_master(
    optional_yield y,
    const rgw::SiteConfig& site)
{
  RGWXMLDecoder::XMLParser parser;
...
void
RGWAttachUserPolicy_IAM::execute(optional_yield y)
{
  ...
  if (op_ret == 0) {
    s->formatter->open_object_section_in_ns(
        "AttachUserPolicyResponse", RGW_REST_IAM_XMLNS);
    s->formatter->open_object_section("ResponseMetadata");
    s->formatter->dump_string("RequestId", s->trans_id);
    s->formatter->close_section();
    s->formatter->close_section();
...
int
RGWRestAttachedUserPolicy::init_processing(optional_yield y)
{
...
public:
  ...
  name() const override
  {
    return "detach_user_policy";
  }

  RGWOpType
  get_type() override
  {
    return RGW_OP_DETACH_USER_POLICY;
  }
};

int
RGWDetachUserPolicy_IAM::get_params()
{
  policy_arn = s->info.args.get("PolicyArn");
  if (!validate_iam_policy_arn(policy_arn, s->err.message)) {
    return -EINVAL;
...
int
RGWDetachUserPolicy_IAM::forward_to_master(
    optional_yield y,
    const rgw::SiteConfig& site)
{
  RGWXMLDecoder::XMLParser parser;
...
void
RGWDetachUserPolicy_IAM::execute(optional_yield y)
{
  const rgw::SiteConfig& site = *s->penv.site;
  if (!site.is_meta_master()) {
    op_ret = forward_to_master(y, site);
  ...
  if (op_ret == 0) {
    s->formatter->open_object_section_in_ns(
        "DetachUserPolicyResponse", RGW_REST_IAM_XMLNS);
    s->formatter->open_object_section("ResponseMetadata");
    s->formatter->dump_string("RequestId", s->trans_id);
    s->formatter->close_section();
    s->formatter->close_section();
...
public:
  ...
  get_type() override
  {
    return RGW_OP_LIST_ATTACHED_USER_POLICIES;
  }
};

int
RGWListAttachedUserPolicies_IAM::get_params()
{
  marker = s->info.args.get("Marker");
...
void
RGWListAttachedUserPolicies_IAM::execute(optional_yield y)
{
  rgw::IAM::ManagedPolicies policies;
  ...
  s->formatter->open_object_section_in_ns(
      "ListAttachedUserPoliciesResponse", RGW_REST_IAM_XMLNS);
  s->formatter->open_object_section("ResponseMetadata");
  s->formatter->dump_string("RequestId", s->trans_id);
  s->formatter->close_section();
...
RGWOp*
make_iam_attach_user_policy_op(const ceph::bufferlist& post_body)
{
  return new RGWAttachUserPolicy_IAM(post_body);
}

RGWOp*
make_iam_detach_user_policy_op(const ceph::bufferlist& post_body)
{
  return new RGWDetachUserPolicy_IAM(post_body);
}

RGWOp*
make_iam_list_attached_user_policies_op(const ceph::bufferlist& unused)
{
