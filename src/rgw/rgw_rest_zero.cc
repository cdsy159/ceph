...
public:
  explicit ZeroOp(ZeroResource* resource) :
    resource(resource)
  {}
...
public:
  explicit ZeroDeleteOp(ZeroResource* resource) :
    ZeroOp(resource)
  {}
...
void
ZeroDeleteOp::execute(optional_yield y)
{
...
public:
  explicit ZeroGetOp(ZeroResource* resource) :
    ZeroOp(resource)
  {}
...
void
ZeroGetOp::execute(optional_yield y)
{
...
public:
  explicit ZeroHeadOp(ZeroResource* resource) :
    ZeroOp(resource)
  {}
...
void
ZeroHeadOp::execute(optional_yield y)
{
  response_content_type = "application/octet-stream";
...
public:
  explicit ZeroPutOp(ZeroResource* resource) :
    ZeroOp(resource)
  {}
...
void
ZeroPutOp::execute(optional_yield y)
{
  if (!s->length) {
  ...
  if (!content_length) {
    ldpp_dout(this, 0) << "failed to parse content length \"" << s->length
                       << '"' << dendl;
    op_ret = -EINVAL;
    return;
  }
...
public:
  explicit ZeroHandler(ZeroResource* resource) :
    resource(resource)
  {}

  int
  init_permissions(RGWOp*, optional_yield) override
  {
    return 0;
  }

  int
  read_permissions(RGWOp*, optional_yield) override
  {
    return 0;
  }
  ...
  int
  postauth_init(optional_yield y) override
  {
    return 0;
  }
  ...
  op_delete() override
  {
    return new ZeroDeleteOp(resource);
  }

  RGWOp*
  op_get() override
  {
    return new ZeroGetOp(resource);
  }

  RGWOp*
  op_head() override
  {
    return new ZeroHeadOp(resource);
  }

  RGWOp*
  op_put() override
  {
    return new ZeroPutOp(resource);
  }
};

RESTMgr_Zero::RESTMgr_Zero() :
  resource(std::make_unique<ZeroResource>())
{}

RGWHandler_REST*
RESTMgr_Zero::get_handler(
    sal::Driver* driver,
    req_state* s,
