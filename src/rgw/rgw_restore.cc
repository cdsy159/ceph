...
void
RestoreEntry::dump(Formatter* f) const
{
...
void
RestoreEntry::decode_json(JSONObj* obj)
{
...
void
RestoreEntry::generate_test_instances(std::list<RestoreEntry*>& l)
{
...
int
Restore::initialize(CephContext* _cct, rgw::sal::Driver* _driver)
{
  int ret = 0;
  cct = _cct;
  driver = _driver;
  ...
  ldpp_dout(this, 20) << __PRETTY_FUNCTION__ << ": initializing Restore handle"
                      << dendl;
  ...
  for (int i = 0; i < max_objs; i++) {
    ...
    ldpp_dout(this, 30) << __PRETTY_FUNCTION__
                        << ": obj_name_i=" << obj_names[i] << dendl;
  }
  ...
  if (!sal_restore) {
    ldpp_dout(this, -1) << __PRETTY_FUNCTION__
                        << ": failed to create sal_restore" << dendl;
    return -EINVAL;
  }
  ...
  if (ret < 0) {
    ldpp_dout(this, -1) << __PRETTY_FUNCTION__
                        << ": failed to initialize sal_restore" << dendl;
  }

  ldpp_dout(this, 20) << __PRETTY_FUNCTION__
                      << ": initializing Restore handle completed" << dendl;
...
Restore::finalize()
{
  ...
  ldpp_dout(this, 20) << __PRETTY_FUNCTION__ << ": finalize Restore handle"
                      << dendl;
}

static inline std::ostream&
operator<<(std::ostream& os, RestoreEntry& ent)
{
  os << "<ent: bucket=";
  os << ent.bucket;
  os << "; obj_key=";
...
unsigned
Restore::get_subsys() const
{
  return dout_subsys;
}

std::ostream&
Restore::gen_prefix(std::ostream& out) const
{
  return out << "restore: ";
}
...
int
Restore::choose_oid(const RestoreEntry& e)
{
  int index;
  ...
  do {
    ...
    if (r < 0) {
      ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__
                         << ": ERROR: restore process() returned error r=" << r
                         << dendl;
    }
...
int
Restore::process(RestoreWorker* worker, optional_yield y)
{
 ...
 */
int
Restore::process(int index, int max_secs, optional_yield y)
{
  ldpp_dout(this, 20) << __PRETTY_FUNCTION__
                      << ": process entered index=" << index
                      << ", max_secs=" << max_secs << dendl;
  ...
  std::unique_ptr<rgw::sal::RestoreSerializer> serializer =
      sal_restore->get_serializer(
          std::string(restore_index_lock_name), std::string(obj_names[index]),
          worker->thr_name());
  ...
  do {
    ...
    ret = sal_restore->list(
        this, y, index, marker, &next_marker, max, entries, &truncated);
    ldpp_dout(this, 20) << __PRETTY_FUNCTION__
    ...
    for (iter = entries.begin(); iter != entries.end(); ++iter) {
      ...
      if (!ret && entry.status ==
        ...
        ldpp_dout(this, 20)
            << __PRETTY_FUNCTION__ << ": re-pushing entry: '" << entry
  ...
  ldpp_dout(this, 20) << __PRETTY_FUNCTION__ << ": trimming till marker: '"
                      << marker << "' on shard:" << obj_names[index] << dendl;
  ...
  if (!r_entries.empty()) {
    ...
    if (ret < 0) {
      ldpp_dout(this, -1) << __PRETTY_FUNCTION__
                          << ": ERROR: failed to add entries on "
                          << obj_names[index] << dendl;
    }
  }
...
int
Restore::process_restore_entry(RestoreEntry& entry, optional_yield y)
{
  int ret = 0;
  ...
  bool in_progress =
      ((entry.status == rgw::sal::RGWRestoreStatus::None) ? false : true);
  ...
  if (ret < 0) {
    ldpp_dout(this, -1) << __PRETTY_FUNCTION__ << ": ERROR: get_object for "
                        << entry.obj_key << " failed" << dendl;
    return ret;
  }

  ldpp_dout(this, 10) << "Restore:: Processing restore entry of object("
                      << obj->get_key() << ") entry: " << entry << dendl;
  ...
  if (restore_status != rgw::sal::RGWRestoreStatus::RestoreAlreadyInProgress) {
    ldpp_dout(this, 5) << __PRETTY_FUNCTION__ << ": Restore of object "
                       << obj->get_key() << " not in progress state" << dendl;
  ...
  if (attr_iter != attrs.end()) {
    ...
    ldpp_dout(this, -1)
        << __PRETTY_FUNCTION__
        << ": ERROR: Attr RGW_ATTR_STORAGE_CLASS not found for object: "
        << obj->get_key() << dendl;
  }

  ret = driver->get_zone()->get_zonegroup().get_placement_tier(
      target_placement, &tier);

  if (ret < 0) {
    ldpp_dout(this, -1)
        << __PRETTY_FUNCTION__
        << ": ERROR: failed to fetch tier placement handle, target_placement = "
        << target_placement << ", for zonegroup = "
        << driver->get_zone()->get_zonegroup().get_name() << ", ret = " << ret
        << dendl;
    goto done;
  } else {
    ldpp_dout(this, 20) << __PRETTY_FUNCTION__
                        << ": getting tier placement handle"
                        << " cloud tier for " << " storage class "
  ...
  ret = obj->restore_obj_from_cloud(
      bucket.get(), tier.get(), cct, days, in_progress, size, this, y);
  if (ret < 0) {
    ldpp_dout(this, -1) << __PRETTY_FUNCTION__ << ": Restore of object("
                        << obj->get_key() << ") failed" << ret << dendl;
    auto reset_ret = set_cloud_restore_status(
        this, obj.get(), y, rgw::sal::RGWRestoreStatus::RestoreFailed);
    if (reset_ret < 0) {
      ldpp_dout(this, -1)
          << __PRETTY_FUNCTION__
          << ": Setting restore status ad RestoreFailed failed for object("
          << obj->get_key() << ") " << reset_ret << dendl;
    }
    goto done;
  }

  if (in_progress) {
    ldpp_dout(this, 15) << __PRETTY_FUNCTION__ << ": Restore of object "
                        << obj->get_key() << " is still in progress" << dendl;
    entry.status = rgw::sal::RGWRestoreStatus::RestoreAlreadyInProgress;
  } else {
    ldpp_dout(this, 15) << __PRETTY_FUNCTION__ << ": Restore of object "
                        << obj->get_key() << " succeeded" << dendl;
    ...
    send_notification(
        this, driver, obj.get(), bucket.get(), etag, size,
        obj->get_key().instance, {rgw::notify::ObjectRestoreCompleted}, y);
  }

done:
  if (ret < 0) {
    ldpp_dout(this, -1) << __PRETTY_FUNCTION__ << ": Restore of entry:'"
                        << entry << "' failed" << ret << dendl;
  ...
  if (waiter_registry && !in_progress) {
    ...
    waiter_registry->notify_completion(
        entry.bucket, entry.obj_key, success, ret);
  }
...
time_t
Restore::thread_stop_at()
{
  uint64_t interval = (cct->_conf->rgw_restore_debug_interval > 0)
                          ? cct->_conf->rgw_restore_debug_interval
                          : secs_in_a_day;
...
int
Restore::set_cloud_restore_status(
    const DoutPrefixProvider* dpp,
    rgw::sal::Object* pobj,
    optional_yield y,
    ...
    ceph::real_time& exp_date)
{
  ...
  ldpp_dout(dpp, 5) << "Calculating expiration date for days:" << expiry_days
                    << dendl;
  if (cct->_conf->rgw_restore_debug_interval > 0) {
    exp_date = cur_time + make_timespan(
                              double(expiry_days) *
                              cct->_conf->rgw_restore_debug_interval);
  } else {
    exp_date = cur_time + make_timespan(double(expiry_days) * secs_in_a_day);
  }
  ldpp_dout(dpp, 5) << "expiration date: " << exp_date
                    << " , cur_time: " << cur_time << ", restore_interval: "
 ...
 */
int
Restore::update_cloud_restore_exp_date(
    ...
    optional_yield y)
{
  ...
  if (ret < 0) {
    ldpp_dout(dpp, 0) << "ERROR: failed to read object attrs "
  ...
  if (ret < 0) {
    ldpp_dout(dpp, 0) << "ERROR: failed to update restore expiry date ret="
                      << ret << dendl;
    return ret;
  }

  ldpp_dout(dpp, 5) << "Updated Restore expiry time to: " << expiration_date
                    << " , cur_time: " << cur_time << ", restore_interval: "
                    << cct->_conf->rgw_restore_debug_interval << dendl;
...
int
Restore::restore_obj_from_cloud(
    rgw::sal::Bucket* pbucket,
    rgw::sal::Object* pobj,
    ...
    optional_yield y)
{
  ...
  if (!pbucket || !pobj) {
    ldpp_dout(this, -1) << __PRETTY_FUNCTION__
                        << ": ERROR: Invalid bucket/object. Restore failed"
                        << dendl;
    return -EINVAL;
  }

  auto notify = driver->get_notification(
      dpp, pobj, nullptr, {rgw::notify::ObjectRestoreInitiated}, pbucket,
      restore_id, const_cast<std::string&>(pbucket->get_tenant()),
      restore_req_id, y);
  ...
  if (notify) {
    ...
    if (ret < 0) {
      ldpp_dout(dpp, 1)
          << "ERROR: notify publish_reserve failed with error: " << ret
          << " for restore object: " << pobj->get_name()
  ...
  ret = set_cloud_restore_status(
      this, pobj, y, rgw::sal::RGWRestoreStatus::RestoreAlreadyInProgress);
  if (ret < 0) {
    ldpp_dout(this, 0) << __PRETTY_FUNCTION__
                       << ": Setting cloud restore status to "
                          "RESTORE_ALREADY_IN_PROGRESS for the object("
                       << pobj->get_key() << " failed, ret=" << ret << dendl;
    return ret;
  }
  ...
  ldpp_dout(this, 10) << "Restore:: Adding restore entry of object("
                      << pobj->get_key() << ") entry: " << entry << dendl;

  int index = choose_oid(entry);
  ldpp_dout(this, 10) << __PRETTY_FUNCTION__
                      << ": Adding restore entry of object(" << pobj->get_key()
                      << ") entry: " << entry
                      << ", to shard:" << obj_names[index] << dendl;
  ...
  if (ret < 0) {
    ldpp_dout(this, -1) << __PRETTY_FUNCTION__
                        << ": ERROR: Adding restore entry of object("
                        << pobj->get_key() << ") failed" << ret << dendl;

    auto reset_ret = set_cloud_restore_status(
        this, pobj, y, rgw::sal::RGWRestoreStatus::RestoreFailed);
    if (reset_ret < 0) {
      ldpp_dout(this, -1)
          << __PRETTY_FUNCTION__
          << ": Setting restore status as RestoreFailed failed for object("
          << pobj->get_key() << ") " << reset_ret << dendl;
    }
  ...
  if (tier && tier->get_tier_type() == "cloud-s3") {
    ldpp_dout(this, 10)
        << __PRETTY_FUNCTION__
        << ": Waking restore worker for immediate processing (cloud-s3 tier)"
        << dendl;
    wake_worker();
  }

  ldpp_dout(this, 10) << __PRETTY_FUNCTION__ << ": Restore of object "
                      << pobj->get_key() << " is in progress." << dendl;
    ...
    optional_yield y)
{
  ...
  do {
    ...
    if (ret < 0) {
      ldpp_dout(dpp, 0) << "ERROR: driver->list_objects(): "
                        << cpp_strerror(-ret) << dendl;
      return ret;
    }
          ...
          if (getattriter->first == RGW_ATTR_RESTORE_STATUS) {
            rgw::sal::RGWRestoreStatus rs;
            {
              ...
              try {
                ...
                ldpp_dout(dpp, 0)
                    << "failed to decode JSON input: " << e.what() << dendl;
                return EINVAL;
              }
            }
            if (restore_status_filter) {
              if (restore_status_filter ==
                  rgw::sal::rgw_restore_status_dump(rs)) {
                f->dump_string(
                    iter->key.name, rgw::sal::rgw_restore_status_dump(rs));
              }
            } else {
              f->dump_string(
                  iter->key.name, rgw::sal::rgw_restore_status_dump(rs));
