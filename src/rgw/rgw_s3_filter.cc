...
void
rgw_s3_key_filter::dump(Formatter* f) const
{
  if (!has_content()) {
    return;
  }
...
bool
rgw_s3_key_filter::decode_xml(XMLObj* obj)
{
  ...
  while ((o = iter.get_next())) {
    ...
    if (name == "prefix" && prefix_not_set) {
      ...
      throw RGWXMLDecoder::err(
          "invalid/duplicate S3Key filter rule name: '" + name + "'");
    }
  }
  return true;
}
...
void
rgw_s3_key_filter::dump_xml(Formatter* f) const
{
  if (!prefix_rule.empty()) {
...
bool
rgw_s3_key_filter::has_content() const
{
  return !(prefix_rule.empty() && suffix_rule.empty() && regex_rule.empty());
}

void
rgw_s3_key_value_filter::dump(Formatter* f) const
{
  if (!has_content()) {
    return;
  }
...
bool
rgw_s3_key_value_filter::decode_xml(XMLObj* obj)
{
  kv.clear();
  XMLObjIter iter = obj->find("FilterRule");
  XMLObj* o;
...
void
rgw_s3_key_value_filter::dump_xml(Formatter* f) const
{
  for (const auto& key_value : kv) {
    f->open_object_section("FilterRule");
...
bool
rgw_s3_key_value_filter::has_content() const
{
  return !kv.empty();
}

void
rgw_s3_filter::dump(Formatter* f) const
{
...
bool
rgw_s3_filter::decode_xml(XMLObj* obj)
{
...
void
rgw_s3_filter::dump_xml(Formatter* f) const
{
...
bool
rgw_s3_filter::has_content() const
{
  return key_filter.has_content() || metadata_filter.has_content() ||
         tag_filter.has_content();
}

bool
match(const rgw_s3_key_filter& filter, const std::string& key)
{
  ...
  if (prefix_size != 0) {
    ...
    if (!std::equal(
            filter.prefix_rule.begin(), filter.prefix_rule.end(), key.begin())) {
      return false;
    }
  }
  ...
  if (suffix_size != 0) {
    ...
    if (!std::equal(
            filter.suffix_rule.begin(), filter.suffix_rule.end(),
            (key.end() - suffix_size))) {
      return false;
    }
  }
...
bool
match(const rgw_s3_key_value_filter& filter, const KeyValueMap& kv)
{
...
bool
match(const rgw_s3_key_value_filter& filter, const KeyMultiValueMap& kv)
{
  // all filter pairs must exist with the same value in the object's metadata/tags
  // object metadata/tags may include items not in the filter
  for (auto& filter : filter.kv) {
    ...
    if (std::any_of(
            ...
            [&filter](const std::pair<std::string, std::string>& p) {
...
bool
match(const rgw_s3_filter& s3_filter, const rgw::sal::Object* obj)
{
  if (obj == nullptr) {
    return false;
  }
