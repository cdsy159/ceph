...
uint64_t
aws_response_handler::get_processed_size()
{
  return processed_size;
}

void
aws_response_handler::update_processed_size(uint64_t value)
{
...
uint64_t
aws_response_handler::get_total_bytes_returned()
{
  return total_bytes_returned;
}

void
aws_response_handler::update_total_bytes_returned(uint64_t value)
{
...
void
aws_response_handler::push_header(
...
int
aws_response_handler::create_header_records()
{
  ...
  //1
  push_header(
      header_name_str[IDX(header_name_En::EVENT_TYPE)],
      header_value_str[IDX(header_value_En::RECORDS)]);
  //2
  push_header(
      header_name_str[IDX(header_name_En::CONTENT_TYPE)],
      header_value_str[IDX(header_value_En::OCTET_STREAM)]);
  //3
  push_header(
      header_name_str[IDX(header_name_En::MESSAGE_TYPE)],
      header_value_str[IDX(header_value_En::EVENT)]);
  return get_buffer()->size();
}

int
aws_response_handler::create_header_continuation()
{
  //headers description(AWS)
  //1
  push_header(
      header_name_str[IDX(header_name_En::EVENT_TYPE)],
      header_value_str[IDX(header_value_En::CONT)]);
  //2
  push_header(
      header_name_str[IDX(header_name_En::MESSAGE_TYPE)],
      header_value_str[IDX(header_value_En::EVENT)]);
...
int
aws_response_handler::create_header_progress()
{
  //headers description(AWS)
  //1
  push_header(
      header_name_str[IDX(header_name_En::EVENT_TYPE)],
      header_value_str[IDX(header_value_En::PROGRESS)]);
  //2
  push_header(
      header_name_str[IDX(header_name_En::CONTENT_TYPE)],
      header_value_str[IDX(header_value_En::XML)]);
  //3
  push_header(
      header_name_str[IDX(header_name_En::MESSAGE_TYPE)],
      header_value_str[IDX(header_value_En::EVENT)]);
...
int
aws_response_handler::create_header_stats()
{
  //headers description(AWS)
  //1
  push_header(
      header_name_str[IDX(header_name_En::EVENT_TYPE)],
      header_value_str[IDX(header_value_En::STATS)]);
  //2
  push_header(
      header_name_str[IDX(header_name_En::CONTENT_TYPE)],
      header_value_str[IDX(header_value_En::XML)]);
...
int
aws_response_handler::create_header_end()
{
  //headers description(AWS)
  //1
  push_header(
      header_name_str[IDX(header_name_En::EVENT_TYPE)],
      header_value_str[IDX(header_value_En::END)]);
  //2
  push_header(
      header_name_str[IDX(header_name_En::MESSAGE_TYPE)],
      header_value_str[IDX(header_value_En::EVENT)]);
...
int
aws_response_handler::create_error_header_records(const char* error_message)
{
  //headers description(AWS)
  //[header-name-byte-length:1][header-name:variable-length][header-value-type:1][header-value:variable-length]
  //1
  push_header(
      header_name_str[IDX(header_name_En::ERROR_CODE)],
      header_value_str[IDX(header_value_En::ENGINE_ERROR)]);
  //2
  push_header(
      header_name_str[IDX(header_name_En::ERROR_MESSAGE)], error_message);
  //3
  push_header(
      header_name_str[IDX(header_name_En::MESSAGE_TYPE)],
      header_value_str[IDX(header_value_En::ERROR_TYPE)]);
  return get_buffer()->size();
}

int
aws_response_handler::create_message(
    u_int32_t header_len,
    std::string* msg_string = nullptr)
{
  ...
  total_byte_len = msg_string->size() +
                   4; //the total is greater in 4 bytes than current size
  push_encode_int(total_byte_len, 0);
  push_encode_int(header_len, 4);
  crc32.reset();
  crc32 = std::for_each(
      msg_string->data(), msg_string->data() + 8,
      crc32); //crc for starting 8 bytes
  preload_crc = crc32();
  push_encode_int(preload_crc, 8);
  crc32.reset();
  crc32 = std::for_each(
      msg_string->begin(), msg_string->end(),
      crc32); //crc for payload + checksum
  message_crc = crc32();
...
void
aws_response_handler::init_response()
{
...
void
aws_response_handler::init_success_response()
{
  get_buffer()->clear();
...
void
aws_response_handler::send_continuation_response()
{
  m_fp_chunk_encoding();
  set_continue_buffer();
...
void
aws_response_handler::init_progress_response()
{
  sql_result.resize(header_crc_size, '\0');
  get_buffer()->clear();
...
void
aws_response_handler::init_stats_response()
{
  sql_result.resize(header_crc_size, '\0');
  get_buffer()->clear();
...
void
aws_response_handler::init_end_response()
{
  m_fp_chunk_encoding();
  sql_result.resize(header_crc_size, '\0');
  get_buffer()->clear();
...
void
aws_response_handler::send_error_response(
    const char* error_code,
    const char* error_message)
{
  m_fp_chunk_encoding();
  std::string out_error_msg = std::string(error_code) + " :" +
                              std::string(error_message) + " :" + s->trans_id;
...
void
aws_response_handler::send_success_response()
{
#ifdef PAYLOAD_TAG
...
void
aws_response_handler::send_error_response_rgw_formatter(
    const char* error_code = empty_error,
...
void
aws_response_handler::send_progress_response()
{
  m_fp_chunk_encoding();
  std::string progress_payload = fmt::format(
      "<?xml version=\"1.0\" "
      "encoding=\"UTF-8\"?><Progress><BytesScanned>{}</"
      "BytesScanned><BytesProcessed>{}</BytesProcessed><BytesReturned>{}</"
...
void
aws_response_handler::send_stats_response()
{
  m_fp_chunk_encoding();
  std::string stats_payload = fmt::format(
      "<?xml version=\"1.0\" "
      "encoding=\"UTF-8\"?><Stats><BytesScanned>{}</"
      "BytesScanned><BytesProcessed>{}</BytesProcessed><BytesReturned>{}</"
      "BytesReturned></Stats>",
      get_processed_size(), get_processed_size(), get_total_bytes_returned());
  ...
  m_is_trino_request(false)
{
  ...
  fp_range_req = [&](int64_t start, int64_t len, void* buff, optional_yield* y) {
    ldout(s->cct, 10) << "S3select: range-request start: " << start
                      << " length: " << len << dendl;
...
int
RGWSelectObj_ObjStore_S3::get_params(optional_yield y)
{
  ...
  if (ret != 0) {
    ldpp_dout(this, 10) << "s3-select query: failed to retrieve query; ret = "
                        << ret << dendl;
    return ret;
  }
  ...
  if (m_s3select_query.length() > 0) {
    ...
    ldpp_dout(this, 10) << "s3-select query: failed to retrieve query;"
                        << dendl;
    return -1;
  }
  ...
  auto user_agent = m.find("HTTP_USER_AGENT");
  {
    if (user_agent != m.end()) {
      if (user_agent->second.find("Trino") != std::string::npos) {
        ...
        ldpp_dout(this, 10)
            << "s3-select query: request sent by Trino." << dendl;
      }
    }
  }
...
int
RGWSelectObj_ObjStore_S3::run_s3select_on_csv(
    const char* query,
    const char* input,
    size_t input_length)
{
  ...
  m_s3_csv_object.set_external_system_functions(
      fp_s3select_continue, fp_s3select_result_format, fp_result_header_format,
      fp_debug_mesg);

  if (s3select_syntax.get_error_description().empty() == false) {
    //error-flow (syntax-error)
    m_aws_response_handler.send_error_response(
        s3select_syntax_error, s3select_syntax.get_error_description().c_str());
    ldpp_dout(this, 10)
        << "s3-select query: failed to prase the following query {" << query
        << "}" << dendl;
    ldpp_dout(this, 10) << "s3-select query: syntax-error {"
                        << s3select_syntax.get_error_description() << "}"
                        << dendl;
    return -1;
  } else {
    if (input == nullptr) {
    ...
    status = m_s3_csv_object.run_s3select_on_stream(
        m_aws_response_handler.get_sql_result(), input, input_length,
        m_object_size_for_processing);
    length_post_processing = m_s3_csv_object.get_return_result_size();
    m_aws_response_handler.update_total_bytes_returned(
        m_s3_csv_object.get_return_result_size());

    if (status < 0) {
      //error flow(processing-time)
      m_aws_response_handler.send_error_response(
          s3select_processTime_error,
          m_s3_csv_object.get_error_description().data());
  ...
  if ((length_post_processing - length_before_processing) != 0) {
    ldpp_dout(this, 10) << "s3-select: sql-result-size = "
                        << m_aws_response_handler.get_sql_result().size()
                        << dendl;
  }
  ldpp_dout(this, 10)
      << "s3-select: complete chunk processing : chunk length = "
      << input_length << dendl;
...
int
RGWSelectObj_ObjStore_S3::run_s3select_on_parquet(const char* query)
{
  int status = 0;
#ifdef _ARROW_EXIST
  if (!m_s3_parquet_object.is_set()) {
    ...
    m_s3_parquet_object.set_external_system_functions(
        fp_s3select_continue, fp_s3select_result_format,
        fp_result_header_format, fp_debug_mesg);

    try {
      //setting the Parquet-reader properties. i.e. the buffer-size for the Parquet-reader
      parquet::ceph::S3select_Config::getInstance()
          .set_s3select_reader_properties(
              s->cct->_conf->rgw_parquet_buffer_size);
      //at this stage the Parquet-processing requires for the meta-data that reside on Parquet object
      m_s3_parquet_object.set_parquet_object(
          std::string("s3object"), &s3select_syntax, &m_rgw_api, parquet);
    } catch (base_s3select_exception& e) {
      ldpp_dout(this, 10)
          << "S3select: failed upon parquet-reader construction: " << e.what()
          << dendl;
      fp_result_header_format(m_aws_response_handler.get_sql_result());
  ...
  if (s3select_syntax.get_error_description().empty() == false) {
    ...
    m_aws_response_handler.send_error_response(
        s3select_syntax_error,
        m_s3_parquet_object.get_error_description().c_str());

    ldpp_dout(this, 10) << "s3-select query: failed to prase query; {"
                        << s3select_syntax.get_error_description() << "}"
                        << dendl;
    status = -1;
  } else {
    ...
    status = m_s3_parquet_object.run_s3select_on_object(
        m_aws_response_handler.get_sql_result());
    if (status < 0) {
...
int
RGWSelectObj_ObjStore_S3::run_s3select_on_json(
    const char* query,
    const char* input,
    size_t input_length)
{
  ...
  m_s3_json_object.set_external_system_functions(
      fp_s3select_continue, fp_s3select_result_format, fp_result_header_format,
      fp_debug_mesg);
  ...
  if (m_json_datatype.compare("DOCUMENT") != 0) {
    ...
    m_aws_response_handler.send_error_response_rgw_formatter(
        s3select_json_error, s3select_json_error_msg);
  ...
  m_aws_response_handler.init_success_response();
  uint32_t length_before_processing =
      m_aws_response_handler.get_sql_result().size();
  //query is correct(syntax), processing is starting.
  try {
    ...
    ldpp_dout(this, 10) << "S3select: failed to process JSON object: "
                        << e.what() << dendl;
    m_aws_response_handler.get_sql_result().append(e.what());
    m_aws_response_handler.send_error_response(
  ...
  uint32_t length_post_processing =
      m_aws_response_handler.get_sql_result().size();
  m_aws_response_handler.update_total_bytes_returned(
      length_post_processing - length_before_processing);
  if (status < 0) {
    //error flow(processing-time)
    m_aws_response_handler.send_error_response(
        s3select_processTime_error,
    ...
    ldpp_dout(this, 10) << "s3-select query: failed to process query; {"
                        << m_s3_json_object.get_error_description() << "}"
                        << dendl;
    return -EINVAL;
  }
...
int
RGWSelectObj_ObjStore_S3::handle_aws_cli_parameters(std::string& sql_query)
{
...
#define APOS "&apos;"
  ...
  if (m_s3select_query.find(input_tag + "><CSV") != std::string::npos) {
    ldpp_dout(this, 10) << "s3select: engine is set to process CSV objects"
                        << dendl;
  } else if (m_s3select_query.find(input_tag + "><JSON") != std::string::npos) {
    m_json_type = true;
    ldpp_dout(this, 10) << "s3select: engine is set to process JSON objects"
                        << dendl;
  } else if (
      m_s3select_query.find(input_tag + "><Parquet") != std::string::npos) {
    m_parquet_type = true;
    ldpp_dout(this, 10) << "s3select: engine is set to process Parquet objects"
                        << dendl;
  }
  ...
  m_s3select_input = m_s3select_query.substr(
      _qi + input_tag.size() + 2, _qe - (_qi + input_tag.size() + 2));
  ...
  m_s3select_output = m_s3select_query.substr(
      _qo + output_tag.size() + 2, _qs - (_qo + output_tag.size() + 2));
  ...
  if (m_compression_type.length() > 0 &&
...
int
RGWSelectObj_ObjStore_S3::extract_by_tag(
    std::string input,
    std::string tag_name,
    std::string& result)
{
...
size_t
RGWSelectObj_ObjStore_S3::get_obj_size()
{
  return s->obj_size;
}
    ...
    optional_yield y)
{
  ...
  range_req_str = "bytes=" + std::to_string(ofs) + "-" +
  ...
  ldout(s->cct, 10) << "S3select: done waiting, buffer is complete buffer-size:"
                    << requested_buffer.size() << dendl;
  return len;
}

void
RGWSelectObj_ObjStore_S3::execute(optional_yield y)
{
  int status = 0;
  ...
  if (s->cct->_conf->rgw_disable_s3select == true) {
    std::string error_msg =
        "s3select : is disabled by rgw_disable_s3select configuration "
        "parameter";
  ...
  if (m_parquet_type) {
    ...
    if (memcmp(parquet_magic, parquet_magic1, 4) &&
        memcmp(parquet_magic, parquet_magicE, 4)) {
      ldout(s->cct, 10) << s->object->get_name()
                        << " does not contain parquet magic" << dendl;
      op_ret = -ERR_INVALID_REQUEST;
      return;
    }
    s3select_syntax.parse_query(m_sql_query.c_str());
    ...
    if (status) {
      ldout(s->cct, 10) << "S3select: failed to process query <" << m_sql_query
...
#ifdef _ARROW_EXIST
      m_aws_response_handler.update_total_bytes_returned(
          m_s3_parquet_object.get_return_result_size());
#endif
      ...
      ldpp_dout(this, 10)
          << "s3select : reached the end of parquet query request : "
             "aws_response_handler.get_processed_size() "
          << m_aws_response_handler.get_processed_size()
          << "m_object_size_for_processing : " << m_object_size_for_processing
          << dendl;

      ldout(s->cct, 10) << "S3select: complete parquet query with success "
                        << dendl;
    }
  } else {
    ...
    if (m_scan_range_ind) {
      ...
      if (m_is_trino_request) {
        ...
        range_request(
            m_start_scan_sz, m_requested_range + m_scan_offset, nullptr, y);
      } else {
...
int
RGWSelectObj_ObjStore_S3::parquet_processing(bufferlist& bl, off_t ofs, off_t len)
{ //purpose: to process the returned buffer from range-request, and to send it to the Parquet-reader.
  ...
  for (auto& it : bl.buffers()) {
    ...
    ldout(s->cct, 10) << "S3select: part " << part_no++
                      << " it.length() = " << it.length() << dendl;
    requested_buffer.append(&(it)[0] + ofs, len);
  }
  ldout(s->cct, 10) << "S3select:append_in_callback = " << append_in_callback
                    << dendl;
  if (requested_buffer.size() < m_request_range) {
    ldout(s->cct, 10) << "S3select: need another round buffer-size: "
                      << requested_buffer.size()
                      << " request range length:" << m_request_range << dendl;
    return 0;
  } else { //buffer is complete
    ldout(s->cct, 10) << "S3select: buffer is complete "
                      << requested_buffer.size()
                      << " request range length:" << m_request_range << dendl;
    m_request_range = 0;
  }
...
void
RGWSelectObj_ObjStore_S3::shape_chunk_per_trino_requests(
    const char* it_cp,
    off_t& ofs,
    off_t& len)
{
  ...
  if (m_scan_range_ind) { //only upon range-scan
    ...
    ldpp_dout(this, 10) << "s3select query: per Trino request the first and "
                           "last chunk should modified."
                        << dendl;
    ...
    if ((m_aws_response_handler.get_processed_size() + len) >=
        m_requested_range) {
      ...
      for (sc = start; sc < len;
           sc++) //assumption : row-delimiter must exist or its end ebject
      {
        char* p = const_cast<char*>(it_cp) + ofs + sc;
        if (!strncmp(row_delimiter, p, 1)) {
          ldout(s->cct, 10)
              << "S3select: found row-delimiter on " << sc
              << " get_processed_size = "
              << m_aws_response_handler.get_processed_size() << dendl;
          len =
              sc +
              1; //+1 is for delimiter.  TODO what about m_object_size_for_processing (to update according to len)
  ...
  ldout(s->cct, 10)
      << "S3select: shape_chunk_per_trino_requests:update progress len = "
      << len << dendl;
  len -= new_offset;
}

int
RGWSelectObj_ObjStore_S3::csv_processing(bufferlist& bl, off_t ofs, off_t len)
{
  int status = 0;
  ...
  if (s->obj_size == 0 || m_object_size_for_processing == 0) {
    ...
    int buff_no = 0;
    for (auto& it : bl.buffers()) {
      ldpp_dout(this, 10) << "s3select :processing segment " << buff_no
                          << " out of " << bl_len << " off " << ofs << " len "
                          << len << " obj-size " << m_object_size_for_processing
                          << dendl;
      if (it.length() == 0 || len == 0) {
        ldpp_dout(this, 10)
            << "s3select :it->_len is zero. segment " << buff_no << " out of "
            << bl_len << " obj-size " << m_object_size_for_processing << dendl;
        continue;
      }
      ...
      ldpp_dout(this, 10) << "s3select: chunk:  ofs = " << ofs
                          << " len = " << len
                          << " it.length() = " << it.length()
                          << " m_object_size_for_processing = "
      ...
      status =
          run_s3select_on_csv(m_sql_query.c_str(), &(it)[0] + ofs, it.length());
      if (status < 0) {
        return -EINVAL;
      }
  ...
  ldpp_dout(this, 10)
      << "s3select : m_aws_response_handler.get_processed_size() "
      << m_aws_response_handler.get_processed_size()
      << " m_object_size_for_processing "
  ...
  if (m_aws_response_handler.get_processed_size() >=
    ...
    if (status >= 0) {
      ...
      m_aws_response_handler.send_stats_response();
      m_aws_response_handler.init_end_response();
      ldpp_dout(this, 10) << "s3select : reached the end of query request : "
                             "aws_response_handler.get_processed_size() "
                          << m_aws_response_handler.get_processed_size()
                          << "m_object_size_for_processing : "
                          << m_object_size_for_processing << dendl;
    ...
    if (m_s3_csv_object.is_sql_limit_reached()) {
      ...
      ldpp_dout(this, 10) << "s3select : reached the limit :"
                          << m_aws_response_handler.get_processed_size()
                          << dendl;
      status = -ENOENT;
    }
...
int
RGWSelectObj_ObjStore_S3::json_processing(bufferlist& bl, off_t ofs, off_t len)
{
  int status = 0;
  ...
  if (s->obj_size == 0 || m_object_size_for_processing == 0) {
    ...
    int i = 0;
    for (auto& it : bl.buffers()) {
      ldpp_dout(this, 10) << "processing segment " << i << " out of " << bl_len
                          << " off " << ofs << " len " << len << " obj-size "
                          << m_object_size_for_processing << dendl;
      //skipping the empty chunks
      if (len == 0) {
        ldpp_dout(this, 10)
            << "s3select:it->_len is zero. segment " << i << " out of "
            << bl_len << " obj-size " << m_object_size_for_processing << dendl;
        continue;
      }
      ...
      if ((ofs + len) > it.length()) {
        ldpp_dout(this, 10)
            ...
            << ofs << " len = " << len << " it.length() = " << it.length()
            << dendl;
        ofs = 0;
        len = it.length();
      }
  ...
  if (status >= 0 && (m_aws_response_handler.get_processed_size() ==
                          uint64_t(m_object_size_for_processing) ||
...
int
RGWSelectObj_ObjStore_S3::send_response_data(bufferlist& bl, off_t ofs, off_t len)
{
