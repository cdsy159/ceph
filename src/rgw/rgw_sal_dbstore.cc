    ...
    optional_yield y)
{
  ...
  int ret = getDB()->list_buckets(
      dpp, "", ownerstr, marker, end_marker, max, need_stats, &ulist,
      &is_truncated);
  if (ret < 0)
    return ret;
    ...
    optional_yield y)
{
  ...
  return store->getDB()->create_bucket(
      dpp, params.owner, key, params.zonegroup_id, params.placement_rule,
      params.attrs, params.swift_ver_location, params.quota,
      params.creation_time, &bucket_version, info, y);
}

int
DBUser::read_attrs(const DoutPrefixProvider* dpp, optional_yield y)
{
  int ret;
  ret = store->getDB()->get_user(
      dpp, string("user_id"), get_id().id, info, &attrs, &objv_tracker);
  return ret;
}
...
int
DBUser::load_user(const DoutPrefixProvider* dpp, optional_yield y)
{
  int ret = 0;
    ...
    RGWUserInfo* old_info)
{
  ...
  ret = store->getDB()->store_user(
      dpp, info, exclusive, &attrs, &objv_tracker, old_info);

  return ret;
}

int
DBUser::remove_user(const DoutPrefixProvider* dpp, optional_yield y)
{
  int ret = 0;
    ...
    optional_yield y)
{
  ...
  if (!delete_children) {
    ...
    if (ret < 0) {
      ldpp_dout(dpp, 20) << __func__ << ": Bucket list objects returned " << ret
                         << dendl;
      return ret;
    }
    ...
    if (!results.objs.empty()) {
      ret = -ENOTEMPTY;
      ldpp_dout(dpp, -1) << __func__ << ": Bucket Not Empty.. returning " << ret
                         << dendl;
      return ret;
    }
  }
...
int
DBBucket::load_bucket(const DoutPrefixProvider* dpp, optional_yield y)
{
  int ret = 0;

  ret = store->getDB()->get_bucket_info(
      dpp, string("name"), "", info, &attrs, &mtime, &bucket_version);
    ...
    optional_yield y)
{
  ...
  ret = store->getDB()->update_bucket(
      dpp, "owner", info, false, &new_owner, nullptr, nullptr, nullptr);
  return ret;
}
    ...
    optional_yield y)
{
  ...
  ret = store->getDB()->update_bucket(
      dpp, "info", info, exclusive, nullptr, &attrs, &_mtime,
      &info.objv_tracker);
...
int
DBBucket::check_empty(const DoutPrefixProvider* dpp, optional_yield y)
{
...
int
DBBucket::merge_and_store_attrs(
    const DoutPrefixProvider* dpp,
    Attrs& new_attrs,
    optional_yield y)
{
  ...
  ret = store->getDB()->update_bucket(
      dpp, "attrs", info, false, nullptr, &attrs, nullptr,
    ...
    optional_yield y)
{
  ...
  ret = store->getDB()->get_bucket_info(
      dpp, string("name"), "", info, &attrs, pmtime, &bucket_version);
...
int
DBBucket::remove_objs_from_index(
    const DoutPrefixProvider* dpp,
...
int
DBBucket::rebuild_index(const DoutPrefixProvider* dpp, optional_yield y)
{
...
int
DBBucket::purge_instance(const DoutPrefixProvider* dpp, optional_yield y)
{
    ...
    optional_yield y)
{
  ...
  ret = store->getDB()->update_bucket(
      dpp, "attrs", info, false, &acl.get_owner().id, &attrs, nullptr, nullptr);

  return ret;
}

std::unique_ptr<Object>
DBBucket::get_object(const rgw_obj_key& k)
{
...
DBBucket::get_multipart_upload(
    const std::string& oid,
    std::optional<std::string> upload_id,
    ACLOwner owner,
    ceph::real_time mtime)
{
  return std::make_unique<DBMultipartUpload>(
      this->store, this, oid, upload_id, std::move(owner), mtime);
}

int
DBBucket::list_multiparts(
    const DoutPrefixProvider* dpp,
    const string& prefix,
    string& marker,
    const string& delim,
...
bool
DBZoneGroup::placement_target_exists(std::string& target) const
{
  return !!group->placement_targets.count(target);
}

void
DBZoneGroup::get_placement_target_names(std::set<std::string>& names) const
{
...
ZoneGroup&
DBZone::get_zonegroup()
{
  return *zonegroup;
}

const RGWZoneParams&
DBZone::get_rgw_params()
{
  return *zone_params;
}
...
bool
DBZone::get_redirect_endpoint(std::string* endpoint)
{
  return false;
}

const std::string&
DBZone::get_current_period_id()
{
  return current_period->get_id();
}

const RGWAccessKey&
DBZone::get_system_key()
{
  return zone_params->system_key;
}

const std::string&
DBZone::get_realm_name()
{
...
DBZone::get_sync_policy_handler()
{
  return nullptr;
}

std::unique_ptr<LuaManager>
DBStore::get_lua_manager(const std::string& luarocks_path)
{
    ...
    bool follow_olh)
{
  ...
  int ret = op_target.get_obj_state(
      dpp, get_bucket()->get_info(), get_obj(), follow_olh, &astate);
  if (ret < 0) {
    return ret;
  }
...
int
DBObject::get_obj_attrs(optional_yield y, const DoutPrefixProvider* dpp)
{
  DB::Object op_target(store->getDB(), get_bucket()->get_info(), get_obj());
...
void
DBObject::gen_rand_obj_instance_name()
{
  store->getDB()->gen_rand_obj_instance_name(&state.obj.key);
}

int
DBObject::omap_get_vals_by_keys(
    const DoutPrefixProvider* dpp,
    const std::string& oid,
...
int
DBObject::chown(User& new_user, const DoutPrefixProvider* dpp, optional_yield y)
{
  return 0;
}

int
MPDBSerializer::try_lock(
    const DoutPrefixProvider* dpp,
    ceph::timespan dur,
    optional_yield y)
{
...
int
MPDBSerializer::unlock(const DoutPrefixProvider* dpp, optional_yield y)
{
  clear_locked();
  return 0;
}
...
bool
DBObject::placement_rules_match(rgw_placement_rule& r1, rgw_placement_rule& r2)
{
...
int
DBObject::DBReadOp::prepare(optional_yield y, const DoutPrefixProvider* dpp)
{
  uint64_t obj_size;
...
int
DBObject::DBDeleteOp::delete_obj(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    uint32_t flags)
{
...
int
DBObject::swift_versioning_restore(
    const ACLOwner& owner,
    const rgw_user& remote_user,
    bool& restored,
    const DoutPrefixProvider* dpp,
...
int
DBObject::swift_versioning_copy(
    const ACLOwner& owner,
    const rgw_user& remote_user,
    const DoutPrefixProvider* dpp,
    optional_yield y)
    ...
    rgw::sal::Attrs& attrs)
{
  ...
  DB::Object op_target(
      store->getDB(), obj->get_bucket()->get_info(), obj->get_obj());
    ...
    const char* if_nomatch)
{
  ...
  do {
    ...
    for (auto obj_iter = parts.begin();
      ...
      DBMultipartPart* part =
          dynamic_cast<rgw::sal::DBMultipartPart*>(obj_iter->second.get());
  ...
  DB::Object op_target(
      store->getDB(), target_obj->get_bucket()->get_info(),
...
int
DBMultipartUpload::cleanup_orphaned_parts(
    const DoutPrefixProvider* dpp,
    CephContext* cct,
    optional_yield y,
    const rgw_obj& obj,
...
DBMultipartWriter::DBMultipartWriter(
    const DoutPrefixProvider* dpp,
    optional_yield y,
...
int
DBMultipartWriter::prepare(optional_yield y)
{
...
int
DBMultipartWriter::process(bufferlist&& data, uint64_t offset)
{
    ...
    uint32_t flags)
{
  ...
  DB::Object op_target(
      store->getDB(), meta_obj->get_bucket()->get_info(), meta_obj->get_obj());
...
int
DBAtomicWriter::prepare(optional_yield y)
{
  return parent_op.prepare(NULL); /* send dpp */
}

int
DBAtomicWriter::process(bufferlist&& data, uint64_t offset)
{
  total_data_size += data.length();
...
std::unique_ptr<RGWRole>
DBStore::get_role(std::string id)
{
  RGWRole* p = nullptr;
  return std::unique_ptr<RGWRole>(p);
}
...
std::unique_ptr<RGWRole>
DBStore::get_role(const RGWRoleInfo& info)
{
  RGWRole* p = nullptr;
  return std::unique_ptr<RGWRole>(p);
}
...
DBStore::get_atomic_writer(
    ...
    const std::string& unique_tag)
{
  return std::make_unique<DBAtomicWriter>(
      dpp, y, obj, this, owner, ptail_placement_rule, olh_epoch, unique_tag);
}

const std::string&
DBStore::get_compression_type(const rgw_placement_rule& rule)
{
  return zone.get_rgw_params().get_compression_type(rule);
}

bool
DBStore::valid_placement(const rgw_placement_rule& rule)
{
...
std::unique_ptr<User>
DBStore::get_user(const rgw_user& u)
{
  return std::make_unique<DBUser>(this, u);
}

int
DBStore::get_user_by_access_key(
    const DoutPrefixProvider* dpp,
    const std::string& key,
    optional_yield y,
    std::unique_ptr<User>* user)
{
  ...
  ret = getDB()->get_user(
      dpp, string("access_key"), key, uinfo, &attrs, &objv_tracker);
    ...
    std::unique_ptr<User>* user)
{
  ...
  ret = getDB()->get_user(
      dpp, string("email"), email, uinfo, &attrs, &objv_tracker);
...
int
DBStore::load_account_user_by_name(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    std::string_view account_id,
    std::string_view tenant,
...
std::string
DBStore::get_cluster_id(const DoutPrefixProvider* dpp, optional_yield y)
{
  return "PLACEHOLDER"; // for instance unique identifier
}

std::unique_ptr<Object>
DBStore::get_object(const rgw_obj_key& k)
{
  return std::make_unique<DBObject>(this, k);
}

std::unique_ptr<Bucket>
DBStore::get_bucket(const RGWBucketInfo& i)
{
...
bool
DBStore::is_meta_master()
{
  return true;
}

std::string
DBStore::zone_unique_id(uint64_t unique_num)
{
  return "";
}

std::string
DBStore::zone_unique_trans_id(const uint64_t unique_num)
{
  return "";
}

int
DBStore::get_zonegroup(const std::string& id, std::unique_ptr<ZoneGroup>* zg)
{
...
int
DBStore::cluster_stat(RGWClusterStat& stats)
{
  return 0;
}

std::unique_ptr<Lifecycle>
DBStore::get_lifecycle(void)
{
  return std::make_unique<DBLifecycle>(this);
}

bool
DBStore::process_expired_objects(const DoutPrefixProvider* dpp, optional_yield y)
{
  return 0;
}

int
DBLifecycle::get_entry(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    const std::string& oid,
    const std::string& marker,
...
int
DBStore::remove_persistent_topic(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    const std::string& topic_queue)
{
...
int
DBStore::log_op(const DoutPrefixProvider* dpp, string& oid, bufferlist& bl)
{
  return 0;
}

int
DBStore::register_to_service_map(
    const DoutPrefixProvider* dpp,
...
void
DBStore::get_quota(RGWQuota& quota)
{
    ...
    optional_yield y)
{
  ...
  for (iter = buckets.begin(); iter != buckets.end(); ++iter) {
    ...
    int r = getDB()->get_bucket_info(
        dpp, string("name"), "", info, &attrs, nullptr, nullptr);
    if (r < 0) {
    ...
    r = getDB()->update_bucket(
        dpp, "info", info, false, nullptr, &attrs, nullptr, &info.objv_tracker);
    if (r < 0) {
...
int
DBStore::get_sync_policy_handler(
    const DoutPrefixProvider* dpp,
...
DBStore::get_data_sync_manager(const rgw_zone_id& source_zone)
{
  return 0;
}

int
DBStore::read_all_usage(
    const DoutPrefixProvider* dpp,
    uint64_t start_epoch,
    uint64_t end_epoch,
    uint32_t max_entries,
...
int
DBStore::get_config_key_val(string name, bufferlist* bl)
{
  return -ENOTSUP;
}

int
DBStore::meta_list_keys_init(
    const DoutPrefixProvider* dpp,
...
void
DBStore::meta_list_keys_complete(void* handle)
{
  return;
}

std::string
DBStore::meta_get_marker(void* handle)
{
  return "";
}
...
int
DBStore::initialize(CephContext* _cct, const DoutPrefixProvider* _dpp)
{
  int ret = 0;
  cct = _cct;
  dpp = _dpp;
...
DBLuaManager::get_script_or_bytecode(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    const std::string& key)
{
...
int
DBLuaManager::reload_packages(const DoutPrefixProvider* dpp, optional_yield y)
{
  return -ENOENT;
}
