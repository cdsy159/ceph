...
public:
  ...
  virtual int
  try_lock(const DoutPrefixProvider* dpp, ceph::timespan dur, optional_yield y)
      override
  {
    return 0;
  }
...
public:
  ...
  virtual void
  set_info(const RGWUploadPartInfo& _info)
  {
    info = _info;
  }
...
public:
  ...
  get_subsys()
  {
    return ceph_subsys_rgw_dbstore;
  };
...
public:
  DBStore() :
    dbsm(nullptr), zone(this), cct(nullptr), dpp(nullptr), use_lc_thread(false)
  {}
  ...
  /* Unique to DBStore */
  void
  setDBStoreManager(DBStoreManager* stm)
  {
    dbsm = stm;
  }

  DBStoreManager*
  getDBStoreManager(void)
  {
    return dbsm;
  }
  ...
  DB*
  getDB(std::string tenant)
  {
    return dbsm->getDB(tenant, false);
  }
};
