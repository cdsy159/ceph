...
static inline PlacementTier*
nextPlacementTier(PlacementTier* t)
{
  if (!t)
    return nullptr;
...
int
FilterZoneGroup::get_placement_tier(
...
int
FilterZoneGroup::get_zone_by_id(
    const std::string& id,
...
int
FilterZoneGroup::get_zone_by_name(
...
int
FilterDriver::initialize(CephContext* cct, const DoutPrefixProvider* dpp)
{
...
std::string
FilterDriver::get_cluster_id(const DoutPrefixProvider* dpp, optional_yield y)
{
  return next->get_cluster_id(dpp, y);
}

std::unique_ptr<User>
FilterDriver::get_user(const rgw_user& u)
{
...
int
FilterDriver::get_user_by_access_key(
...
int
FilterDriver::get_user_by_email(
    const DoutPrefixProvider* dpp,
...
int
FilterDriver::get_user_by_swift(
    const DoutPrefixProvider* dpp,
...
int
FilterDriver::load_account_by_id(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    std::string_view id,
    RGWAccountInfo& info,
...
int
FilterDriver::load_account_by_name(
    const DoutPrefixProvider* dpp,
    optional_yield y,
...
int
FilterDriver::load_account_by_email(
    const DoutPrefixProvider* dpp,
    optional_yield y,
...
int
FilterDriver::complete_flush_stats(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    const rgw_owner& owner)
{
...
int
FilterDriver::load_owner_by_email(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    std::string_view email,
    rgw_owner& owner)
...
int
FilterDriver::count_account_roles(
    const DoutPrefixProvider* dpp,
    optional_yield y,
...
int
FilterDriver::list_account_roles(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    std::string_view account_id,
    ...
    RoleList& listing)
{
  return next->list_account_roles(
      dpp, y, account_id, path_prefix, marker, max_items, listing);
}

int
FilterDriver::load_account_user_by_name(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    std::string_view account_id,
    std::string_view tenant,
...
{
  std::unique_ptr<User> nu;
  int ret = next->load_account_user_by_name(
      dpp, y, account_id, tenant, username, &nu);
  if (ret >= 0) {
...
int
FilterDriver::count_account_users(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    std::string_view account_id,
    uint32_t& count)
...
int
FilterDriver::list_account_users(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    std::string_view account_id,
    std::string_view tenant,
    ...
    UserList& listing)
{
  return next->list_account_users(
      dpp, y, account_id, tenant, path_prefix, marker, max_items, listing);
}

int
FilterDriver::load_group_by_id(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    std::string_view id,
    RGWGroupInfo& info,
...
int
FilterDriver::load_group_by_name(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    std::string_view account_id,
    std::string_view name,
...
int
FilterDriver::count_account_groups(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    std::string_view account_id,
    uint32_t& count)
...
int
FilterDriver::list_account_groups(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    std::string_view account_id,
    std::string_view path_prefix,
    ...
    GroupList& listing)
{
  return next->list_account_groups(
      dpp, y, account_id, path_prefix, marker, max_items, listing);
}

std::unique_ptr<Object>
FilterDriver::get_object(const rgw_obj_key& k)
{
...
FilterDriver::get_bucket(const RGWBucketInfo& i)
{
  return std::make_unique<FilterBucket>(next->get_bucket(i));
}

int
FilterDriver::load_bucket(
    const DoutPrefixProvider* dpp,
    const rgw_bucket& b,
...
FilterDriver::list_buckets(
    ...
    optional_yield y)
{
  return next->list_buckets(
      dpp, owner, tenant, marker, end_marker, max, need_stats, buckets, y);
}

bool
FilterDriver::is_meta_master()
{
  return next->is_meta_master();
}

std::string
FilterDriver::zone_unique_id(uint64_t unique_num)
{
  return next->zone_unique_id(unique_num);
}

std::string
FilterDriver::zone_unique_trans_id(uint64_t unique_num)
{
  return next->zone_unique_trans_id(unique_num);
}

int
FilterDriver::get_zonegroup(
    const std::string& id,
...
int
FilterDriver::cluster_stat(RGWClusterStat& stats)
{
  return next->cluster_stat(stats);
}

std::unique_ptr<Lifecycle>
FilterDriver::get_lifecycle(void)
{
...
bool
FilterDriver::process_expired_objects(
    const DoutPrefixProvider* dpp,
    optional_yield y)
{
    ...
    const std::string* object_name)
{
  std::unique_ptr<Notification> n = next->get_notification(
      nextObject(obj), nextObject(src_obj), s, event_type, y, object_name);
...
int
FilterDriver::add_persistent_topic(
    const DoutPrefixProvider* dpp,
    optional_yield y,
...
int
FilterDriver::remove_persistent_topic(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    const std::string& topic_queue)
{
...
int
FilterDriver::register_to_service_map(
    const DoutPrefixProvider* dpp,
...
void
FilterDriver::get_quota(RGWQuota& quota)
{
  return next->get_quota(quota);
}

void
FilterDriver::get_ratelimit(
...
int
FilterDriver::set_buckets_enabled(
    const DoutPrefixProvider* dpp,
...
uint64_t
FilterDriver::get_new_req_id()
{
  return next->get_new_req_id();
}

int
FilterDriver::get_sync_policy_handler(
    const DoutPrefixProvider* dpp,
...
FilterDriver::get_data_sync_manager(const rgw_zone_id& source_zone)
{
  return next->get_data_sync_manager(source_zone);
}

void
FilterDriver::wakeup_meta_sync_shards(std::set<int>& shard_ids)
{
  return next->wakeup_meta_sync_shards(shard_ids);
}

void
FilterDriver::wakeup_data_sync_shards(
    const DoutPrefixProvider* dpp,
    const rgw_zone_id& source_zone,
    boost::container::flat_map<
        int,
        boost::container::flat_set<rgw_data_notify_entry>>& shard_ids)
{
  return next->wakeup_data_sync_shards(dpp, source_zone, shard_ids);
}

int
FilterDriver::clear_usage(const DoutPrefixProvider* dpp, optional_yield y)
{
  return next->clear_usage(dpp, y);
}

int
FilterDriver::read_all_usage(
    const DoutPrefixProvider* dpp,
...
{
  return next->read_all_usage(
      dpp, start_epoch, end_epoch, max_entries, is_truncated, usage_iter, usage);
}

int
FilterDriver::trim_all_usage(
    const DoutPrefixProvider* dpp,
    uint64_t start_epoch,
    uint64_t end_epoch,
    optional_yield y)
...
int
FilterDriver::get_config_key_val(std::string name, bufferlist* bl)
{
  return next->get_config_key_val(name, bl);
}

int
FilterDriver::meta_list_keys_init(
    const DoutPrefixProvider* dpp,
...
int
FilterDriver::meta_list_keys_next(
    const DoutPrefixProvider* dpp,
    void* handle,
    int max,
...
void
FilterDriver::meta_list_keys_complete(void* handle)
{
  next->meta_list_keys_complete(handle);
}

std::string
FilterDriver::meta_get_marker(void* handle)
{
  return next->meta_get_marker(handle);
}

int
FilterDriver::meta_remove(
    const DoutPrefixProvider* dpp,
...
FilterDriver::get_sync_module()
{
  return next->get_sync_module();
}

std::unique_ptr<LuaManager>
FilterDriver::get_lua_manager(const std::string& luarocks_path)
{
...
{
  return next->get_role(
      name, tenant, std::move(account_id), path, trust_policy,
      std::move(description), max_session_duration_str, tags);
}

std::unique_ptr<RGWRole>
FilterDriver::get_role(std::string id)
{
  return next->get_role(id);
}

std::unique_ptr<RGWRole>
FilterDriver::get_role(const RGWRoleInfo& info)
{
  return next->get_role(info);
}

int
FilterDriver::list_roles(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    const std::string& tenant,
    ...
    RoleList& listing)
{
  return next->list_roles(
      dpp, y, tenant, path_prefix, marker, max_items, listing);
}

int
FilterDriver::store_oidc_provider(
    const DoutPrefixProvider* dpp,
    optional_yield y,
...
int
FilterDriver::load_oidc_provider(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    std::string_view tenant,
    std::string_view url,
...
int
FilterDriver::delete_oidc_provider(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    std::string_view tenant,
    std::string_view url)
...
FilterDriver::get_append_writer(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    rgw::sal::Object* obj,
    ...
    uint64_t* cur_accounted_size)
{
  std::unique_ptr<Writer> writer = next->get_append_writer(
      dpp, y, nextObject(obj), owner, ptail_placement_rule, unique_tag,
...
std::unique_ptr<Writer>
FilterDriver::get_atomic_writer(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    rgw::sal::Object* obj,
    const ACLOwner& owner,
...
const std::string&
FilterDriver::get_compression_type(const rgw_placement_rule& rule)
{
  return next->get_compression_type(rule);
}

bool
FilterDriver::valid_placement(const rgw_placement_rule& rule)
{
  return next->valid_placement(rule);
}

void
FilterDriver::finalize(void)
{
  next->finalize();
}

CephContext*
FilterDriver::ctx(void)
{
  return next->ctx();
}

int
FilterUser::read_attrs(const DoutPrefixProvider* dpp, optional_yield y)
{
  return next->read_attrs(dpp, y);
}

int
FilterUser::merge_and_store_attrs(
    const DoutPrefixProvider* dpp,
    Attrs& new_attrs,
    optional_yield y)
{
...
FilterUser::read_usage(
    ...
    std::map<rgw_user_bucket, rgw_usage_log_entry>& usage)
{
  return next->read_usage(
      dpp, start_epoch, end_epoch, max_entries, is_truncated, usage_iter, usage);
}

int
FilterUser::trim_usage(
    const DoutPrefixProvider* dpp,
    uint64_t start_epoch,
    uint64_t end_epoch,
    optional_yield y)
...
int
FilterUser::load_user(const DoutPrefixProvider* dpp, optional_yield y)
{
  return next->load_user(dpp, y);
}

int
FilterUser::store_user(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    bool exclusive,
...
int
FilterUser::remove_user(const DoutPrefixProvider* dpp, optional_yield y)
{
  return next->remove_user(dpp, y);
}

int
FilterUser::verify_mfa(
...
std::unique_ptr<Object>
FilterBucket::get_object(const rgw_obj_key& k)
{
  std::unique_ptr<Object> o = next->get_object(k);
...
int
FilterBucket::load_bucket(const DoutPrefixProvider* dpp, optional_yield y)
{
  return next->load_bucket(dpp, y);
}

int
FilterBucket::read_stats(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    ...
    bool* syncstopped)
{
  return next->read_stats(
      dpp, y, idx_layout, shard_id, bucket_ver, master_ver, stats, max_marker,
      syncstopped);
}

int
FilterBucket::read_stats_async(
    const DoutPrefixProvider* dpp,
    const bucket_index_layout_generation& idx_layout,
    int shard_id,
...
int
FilterBucket::check_bucket_shards(
    const DoutPrefixProvider* dpp,
    uint64_t num_objs,
    optional_yield y)
{
...
const rgw_owner&
FilterBucket::get_owner() const
{
  return next->get_owner();
}

int
FilterBucket::check_empty(const DoutPrefixProvider* dpp, optional_yield y)
{
  return next->check_empty(dpp, y);
}

int
FilterBucket::check_quota(
    const DoutPrefixProvider* dpp,
    RGWQuota& quota,
    uint64_t obj_size,
    optional_yield y,
...
int
FilterBucket::merge_and_store_attrs(
    const DoutPrefixProvider* dpp,
    Attrs& new_attrs,
    optional_yield y)
{
...
int
FilterBucket::remove_objs_from_index(
    const DoutPrefixProvider* dpp,
...
int
FilterBucket::rebuild_index(const DoutPrefixProvider* dpp, optional_yield y)
{
  return next->rebuild_index(dpp, y);
}

int
FilterBucket::set_tag_timeout(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    uint64_t timeout)
{
...
int
FilterBucket::purge_instance(const DoutPrefixProvider* dpp, optional_yield y)
{
  return next->purge_instance(dpp, y);
}

std::unique_ptr<MultipartUpload>
FilterBucket::get_multipart_upload(
    ...
    optional_yield y)
{
  ...
  ret = next->list_multiparts(
      dpp, prefix, marker, delim, max_uploads, nup, common_prefixes,
      is_truncated, y);
  if (ret < 0)
    return ret;
  ...
  for (auto& ent : nup) {
    uploads.emplace_back(
        std::make_unique<FilterMultipartUpload>(std::move(ent), this));
  }
    ...
    optional_yield y)
{
  return next->copy_object(
      ...
      delete_at, version_id, tag, etag, progress_cb, progress_data, dp_factory,
      dpp, y);
}

RGWAccessControlPolicy&
FilterObject::get_acl()
{
  return next->get_acl();
}

int
FilterObject::list_parts(
    const DoutPrefixProvider* dpp,
    CephContext* cct,
    int max_parts,
    int marker,
    ...
    optional_yield y)
{
  return next->list_parts(
      dpp, cct, max_parts, marker, next_marker, truncated, std::move(each_func),
      y);
}

int
FilterObject::load_obj_state(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    bool follow_olh)
{
...
int
FilterObject::get_obj_attrs(optional_yield y, const DoutPrefixProvider* dpp)
{
  return next->get_obj_attrs(y, dpp);
}

int
FilterObject::modify_obj_attrs(
...
bool
FilterObject::is_expired()
{
  return next->is_expired();
}

void
FilterObject::gen_rand_obj_instance_name()
{
  return next->gen_rand_obj_instance_name();
}

std::unique_ptr<MPSerializer>
FilterObject::get_serializer(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    ...
    uint32_t flags)
{
  return next->transition(
      nextBucket(bucket), placement_rule, mtime, olh_epoch, dpp, y, flags);
}

int
FilterObject::transition_to_cloud(
    Bucket* bucket,
    ...
    optional_yield y)
{
  return next->transition_to_cloud(
      nextBucket(bucket), nextPlacementTier(tier), o, cloud_targets, cct,
      update_object, dpp, y);
}

int
FilterObject::restore_obj_from_cloud(
    Bucket* bucket,
    rgw::sal::PlacementTier* tier,
    CephContext* cct,
    ...
    optional_yield y)
{
  return next->restore_obj_from_cloud(
      nextBucket(bucket), nextPlacementTier(tier), cct, days, in_progress, size,
      dpp, y);
}

bool
FilterObject::placement_rules_match(
    rgw_placement_rule& r1,
...
void
FilterObject::set_bucket(Bucket* b)
{
  bucket = b;
...
int
FilterObject::swift_versioning_restore(
    const ACLOwner& owner,
    const rgw_user& remote_user,
    bool& restored,
    const DoutPrefixProvider* dpp,
...
int
FilterObject::swift_versioning_copy(
    const ACLOwner& owner,
    const rgw_user& remote_user,
    const DoutPrefixProvider* dpp,
    optional_yield y)
...
int
FilterObject::omap_get_vals_by_keys(
    const DoutPrefixProvider* dpp,
    const std::string& oid,
...
int
FilterObject::omap_set_val_by_key(
    const DoutPrefixProvider* dpp,
    const std::string& key,
    bufferlist& val,
    bool must_exist,
...
int
FilterObject::FilterReadOp::prepare(
    optional_yield y,
    const DoutPrefixProvider* dpp)
{
...
int
FilterObject::FilterReadOp::read(
    int64_t ofs,
    int64_t end,
    bufferlist& bl,
    optional_yield y,
...
int
FilterObject::FilterReadOp::get_attr(
    const DoutPrefixProvider* dpp,
    const char* name,
    bufferlist& dest,
    optional_yield y)
...
int
FilterObject::FilterReadOp::iterate(
    const DoutPrefixProvider* dpp,
    int64_t ofs,
    int64_t end,
    RGWGetDataCB* cb,
...
int
FilterObject::FilterDeleteOp::delete_obj(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    uint32_t flags)
{
...
FilterMultipartUpload::get_meta_obj()
{
...
int
FilterMultipartUpload::list_parts(
    const DoutPrefixProvider* dpp,
    CephContext* cct,
    int num_parts,
    int marker,
    ...
    bool assume_unsorted)
{
  ...
  ret = next->list_parts(
      dpp, cct, num_parts, marker, next_marker, truncated, y, assume_unsorted);
  if (ret < 0)
    return ret;
  ...
  for (auto& ent : next->get_parts()) {
    parts.emplace(
        ent.first, std::make_unique<FilterMultipartPart>(std::move(ent.second)));
  }
...
int
FilterMultipartUpload::complete(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    CephContext* cct,
...
int
FilterMultipartUpload::cleanup_orphaned_parts(
    const DoutPrefixProvider* dpp,
    CephContext* cct,
    optional_yield y,
    const rgw_obj& obj,
    ...
    prefix_map_t& processed_prefixes)
{
  return next->cleanup_orphaned_parts(
      dpp, cct, y, obj, remove_objs, processed_prefixes);
}

int
FilterMultipartUpload::get_info(
    const DoutPrefixProvider* dpp,
    optional_yield y,
...
std::unique_ptr<Writer>
FilterMultipartUpload::get_writer(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    rgw::sal::Object* obj,
    const ACLOwner& owner,
    ...
    const std::string& part_num_str)
{
  ...
  writer = next->get_writer(
      dpp, y, nextObject(obj), owner, ptail_placement_rule, part_num,
      part_num_str);
...
int
FilterMPSerializer::unlock(const DoutPrefixProvider* dpp, optional_yield y)
{
  return next->unlock(dpp, y);
}

int
FilterLCSerializer::try_lock(
    const DoutPrefixProvider* dpp,
    ceph::timespan dur,
    optional_yield y)
{
...
int
FilterLCSerializer::unlock(const DoutPrefixProvider* dpp, optional_yield y)
{
  return next->unlock(dpp, y);
}

int
FilterLifecycle::get_entry(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    const std::string& oid,
    const std::string& marker,
...
int
FilterLifecycle::get_next_entry(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    const std::string& oid,
    const std::string& marker,
...
FilterLifecycle::get_serializer(
    const std::string& lock_name,
    const std::string& oid,
...
int
FilterRestoreSerializer::try_lock(
    const DoutPrefixProvider* dpp,
    ceph::timespan dur,
    optional_yield y)
{
...
{
  return next->list(
      dpp, y, index, marker, out_marker, max_entries, entries, truncated);
}

int
FilterRestore::trim_entries(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    int index,
...
int
FilterNotification::publish_reserve(
    const DoutPrefixProvider* dpp,
    RGWObjTags* obj_tags)
{
...
int
FilterNotification::publish_commit(
    const DoutPrefixProvider* dpp,
    uint64_t size,
...
int
FilterWriter::process(bufferlist&& data, uint64_t offset)
{
  return next->process(std::move(data), offset);
}

int
FilterWriter::complete(
...
FilterLuaManager::get_script_or_bytecode(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    const std::string& key)
{
...
int
FilterLuaManager::remove_package(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    const std::string& package_name)
{
...
int
FilterLuaManager::list_packages(
    const DoutPrefixProvider* dpp,
    optional_yield y,
...
int
FilterLuaManager::reload_packages(const DoutPrefixProvider* dpp, optional_yield y)
{
  return next->reload_packages(dpp, y);
}

const std::string&
FilterLuaManager::luarocks_path() const
{
  return next->luarocks_path();
}

void
FilterLuaManager::set_luarocks_path(const std::string& path)
{
  next->set_luarocks_path(path);
}

void
FilterLuaManager::set_lua_background(rgw::lua::Background* background)
{
...
rgw::sal::Driver*
newBaseFilter(rgw::sal::Driver* next)
{
