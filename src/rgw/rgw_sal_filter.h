...
public:
  ...
  is_tier_type_s3()
  {
    return next->is_tier_type_s3();
  }

  virtual const std::string&
  get_storage_class() override
  {
    return next->get_storage_class();
  }

  virtual bool
  retain_head_object() override
  {
    return next->retain_head_object();
  }

  virtual bool
  allow_read_through()
  {
    return next->allow_read_through();
  }

  virtual uint64_t
  get_read_through_restore_days()
  {
    return next->get_read_through_restore_days();
  }
...
public:
  ...
  virtual int
  equals(const std::string& other_zonegroup) const override
  {
    return next->equals(other_zonegroup);
  }

  virtual bool
  placement_target_exists(std::string& target) const override
  {
    return next->placement_target_exists(target);
  }

  virtual bool
  is_master_zonegroup() const override
  {
  ...
  virtual void
  get_placement_target_names(std::set<std::string>& names) const override
  {
  ...
  virtual int
  get_hostnames(std::list<std::string>& names) const override
  {
    return next->get_hostnames(names);
  }

  virtual int
  get_s3website_hostnames(std::list<std::string>& names) const override
  {
    return next->get_s3website_hostnames(names);
  }

  virtual int
  get_zone_count() const override
  {
  ...
  virtual int
  list_zones(std::list<std::string>& zone_ids) override
  {
    return next->list_zones(zone_ids);
  }

  virtual std::unique_ptr<ZoneGroup>
  clone() override
  {
...
public:
  ...
  virtual bool
  is_writeable() override
  {
    return next->is_writeable();
  }

  virtual bool
  get_redirect_endpoint(std::string* endpoint) override
  {
  ...
  virtual const RGWAccessKey&
  get_system_key() override
  {
    return next->get_system_key();
  }

  virtual const std::string&
  get_realm_name() override
  {
    return next->get_realm_name();
  }

  virtual const std::string&
  get_realm_id() override
  {
  ...
  virtual RGWBucketSyncPolicyHandlerRef
  get_sync_policy_handler() override
  {
...
public:
  ...
  int complete_flush_stats(
      ...
      optional_yield y,
      const rgw_owner& owner) override;
  int load_owner_by_email(
      const DoutPrefixProvider* dpp,
      optional_yield y,
      std::string_view email,
      rgw_owner& owner) override;
  int count_account_roles(
      const DoutPrefixProvider* dpp,
      optional_yield y,
      std::string_view account_id,
      uint32_t& count) override;
  int list_account_roles(
      const DoutPrefixProvider* dpp,
      optional_yield y,
      std::string_view account_id,
  ...
  int load_account_user_by_name(
      ...
      std::unique_ptr<User>* user) override;
  int count_account_users(
      const DoutPrefixProvider* dpp,
      optional_yield y,
      std::string_view account_id,
      uint32_t& count) override;
  int list_account_users(
      const DoutPrefixProvider* dpp,
      optional_yield y,
      std::string_view account_id,
      std::string_view tenant,
  ...
  {
    return next->list_account_topics(
        dpp, y, account_id, marker, max_items, listing);
  }

  int add_persistent_topic(
      const DoutPrefixProvider* dpp,
      optional_yield y,
      const std::string& topic_queue) override;
  int remove_persistent_topic(
      const DoutPrefixProvider* dpp,
      optional_yield y,
      const std::string& topic_queue) override;

  int
  update_bucket_topic_mapping(
      const rgw_pubsub_topic& topic,
      ...
      const DoutPrefixProvider* dpp) override
  {
    return next->update_bucket_topic_mapping(
        topic, bucket_key, add_mapping, y, dpp);
  }
  ...
  remove_bucket_mapping_from_topics(
      ...
      const DoutPrefixProvider* dpp) override
  {
    return next->remove_bucket_mapping_from_topics(
        bucket_topics, bucket_key, y, dpp);
  }

  int
  get_bucket_topic_mapping(
      const rgw_pubsub_topic& topic,
  ...
  virtual void wakeup_data_sync_shards(
      ...
      boost::container::
          flat_map<int, boost::container::flat_set<rgw_data_notify_entry>>&
              shard_ids) override;
  virtual int clear_usage(
      const DoutPrefixProvider* dpp,
      optional_yield y) override;
  ...
  virtual std::string
  get_host_id() override
  {
    return next->get_host_id();
  }

  virtual std::unique_ptr<LuaManager> get_lua_manager(
      const std::string& luarocks_path) override;
  ...
  virtual int list_roles(
      ...
      uint32_t max_items,
      RoleList& listing) override;
  int store_oidc_provider(
      const DoutPrefixProvider* dpp,
      optional_yield y,
      const RGWOIDCProviderInfo& info,
      bool exclusive) override;
  int load_oidc_provider(
      const DoutPrefixProvider* dpp,
      optional_yield y,
      std::string_view tenant,
      std::string_view url,
      RGWOIDCProviderInfo& info) override;
  int delete_oidc_provider(
      const DoutPrefixProvider* dpp,
      optional_yield y,
      std::string_view tenant,
      std::string_view url) override;
  int get_oidc_providers(
      const DoutPrefixProvider* dpp,
      optional_yield y,
      std::string_view tenant,
  ...
  virtual void
  register_admin_apis(RGWRESTMgr* mgr) override
  {
...
public:
  ...
  {
    return next->get_display_name();
  }

  virtual const std::string&
  get_tenant() override
  {
    return next->get_tenant();
  }

  virtual void
  set_tenant(std::string& _t) override
  {
    next->set_tenant(_t);
  }
  ...
  virtual void
  set_ns(std::string& _ns) override
  {
    next->set_ns(_ns);
  }
  ...
  virtual int32_t
  get_max_buckets() const override
  {
    return next->get_max_buckets();
  }

  virtual void
  set_max_buckets(int32_t _max_buckets) override
  {
    return next->set_max_buckets(_max_buckets);
  }

  virtual void
  set_info(RGWQuotaInfo& _quota) override
  {
    return next->set_info(_quota);
  }

  virtual const RGWUserCaps&
  get_caps() const override
  {
    return next->get_caps();
  }

  virtual RGWObjVersionTracker&
  get_version_tracker() override
  {
  ...
  get_attrs() override
  {
    return next->get_attrs();
  }

  virtual void
  set_attrs(Attrs& _attrs) override
  {
    next->set_attrs(_attrs);
  }
...
public:
  FilterBucket(std::unique_ptr<Bucket> _next) :
    next(std::move(_next))
  {}
  ...
  virtual Attrs&
  get_attrs(void) override
  {
    return next->get_attrs();
  }

  virtual int
  set_attrs(Attrs a) override
  {
    return next->set_attrs(a);
  }
  ...
  int sync_owner_stats(
      const DoutPrefixProvider* dpp,
      optional_yield y,
      RGWBucketEnt* ent) override;
  int check_bucket_shards(
      const DoutPrefixProvider* dpp,
      uint64_t num_objs,
      optional_yield y) override;
  virtual int chown(
  ...
  virtual const std::string&
  get_tenant() const override
  {
    return next->get_tenant();
  }

  virtual const std::string&
  get_marker() const override
  {
    return next->get_marker();
  }

  virtual const std::string&
  get_bucket_id() const override
  {
    return next->get_bucket_id();
  }

  virtual rgw_placement_rule&
  get_placement_rule() override
  {
    return next->get_placement_rule();
  }

  virtual ceph::real_time&
  get_creation_time() override
  {
    return next->get_creation_time();
  }

  virtual ceph::real_time&
  get_modification_time() override
  {
    return next->get_modification_time();
  }

  virtual obj_version&
  get_version() override
  {
    return next->get_version();
  }

  virtual void
  set_version(obj_version& ver) override
  {
    next->set_version(ver);
  }

  virtual bool
  versioned() override
  {
    return next->versioned();
  }

  virtual bool
  versioning_enabled() override
  {
    return next->versioning_enabled();
  }

  virtual std::unique_ptr<Bucket>
  clone() override
  {
  ...
  {
    return next->set_logging_object_name(
        obj_name, prefix, y, dpp, new_obj, objv_track);
  }

  int
  remove_logging_object_name(
      const std::string& prefix,
      optional_yield y,
      const DoutPrefixProvider* dpp,
      RGWObjVersionTracker* objv_tracker) override
  ...
  commit_logging_object(
      ...
      bool async) override
  {
    return next->commit_logging_object(
        obj_name, y, dpp, prefix, last_committed, async);
  }

  int
  remove_logging_object(
      const std::string& obj_name,
      const std::string& prefix,
      optional_yield y,
      const DoutPrefixProvider* dpp) override
  ...
  int
  write_logging_object(
      const std::string& obj_name,
      const std::string& record,
      const std::string& prefix,
      optional_yield y,
      const DoutPrefixProvider* dpp,
      bool async_completion) override
  {
    return next->write_logging_object(
        obj_name, record, prefix, y, dpp, async_completion);
  }
  ...
  virtual bool
  operator==(const Bucket& b) const override
  {
    return next->operator==(b);
  }
...
public:
  ...
  virtual int
  set_acl(const RGWAccessControlPolicy& acl) override
  {
    return next->set_acl(acl);
  }

  virtual void
  set_atomic(bool atomic) override
  {
    return next->set_atomic(atomic);
  }

  virtual bool
  is_atomic() override
  {
    return next->is_atomic();
  }

  virtual void
  set_prefetch_data() override
  {
    return next->set_prefetch_data();
  }

  virtual bool
  is_prefetch_data() override
  {
    return next->is_prefetch_data();
  }

  virtual void
  set_compressed() override
  {
    return next->set_compressed();
  }

  virtual bool
  is_compressed() override
  {
    return next->is_compressed();
  }

  virtual bool
  is_delete_marker() override
  {
    return next->is_delete_marker();
  }
  ...
  virtual int transition_to_cloud(
      ...
      const DoutPrefixProvider* dpp,
      optional_yield y) override;
  virtual int restore_obj_from_cloud(
      Bucket* bucket,
      rgw::sal::PlacementTier* tier,
      CephContext* cct,
  ...
  virtual bool
  has_attrs(void) override
  {
    return next->has_attrs();
  };

  virtual bool
  get_attr(const std::string& name, bufferlist& dest) override
  {
    return next->get_attr(name, dest);
  }

  virtual ceph::real_time
  get_mtime(void) const override
  {
    return next->get_mtime();
  };

  virtual void
  set_mtime(ceph::real_time& mtime) override
  {
    return next->set_mtime(mtime);
  }

  virtual uint64_t
  get_size(void) const override
  {
    return next->get_size();
  };

  virtual uint64_t
  get_accounted_size(void) const override
  {
    return next->get_accounted_size();
  };

  virtual void
  set_accounted_size(uint64_t size) override
  {
    return next->set_accounted_size(size);
  }

  virtual uint64_t
  get_epoch(void) const override
  {
    return next->get_epoch();
  }

  virtual void
  set_epoch(uint64_t epoch) override
  {
    return next->set_epoch(epoch);
  }

  virtual uint32_t
  get_short_zone_id(void) const override
  {
    return next->get_short_zone_id();
  }

  virtual void
  set_short_zone_id(uint32_t id) override
  {
    return next->set_short_zone_id(id);
  }

  virtual Bucket*
  get_bucket(void) const override
  {
    return bucket;
  };
  ...
  virtual std::string
  get_hash_source(void) override
  {
    return next->get_hash_source();
  };

  virtual void
  set_hash_source(std::string s) override
  {
    return next->set_hash_source(s);
  };

  virtual std::string
  get_oid(void) const override
  {
    return next->get_oid();
  };

  virtual bool
  get_in_extra_data(void) override
  {
    return next->get_in_extra_data();
  };

  virtual bool
  exists(void) override
  {
    return next->exists();
  };

  virtual void
  set_in_extra_data(bool i) override
  {
    return next->set_in_extra_data(i);
  };

  int
  range_to_ofs(uint64_t obj_size, int64_t& ofs, int64_t& end)
  {
    return next->range_to_ofs(obj_size, ofs, end);
  };

  virtual void
  set_obj_size(uint64_t s) override
  {
    return next->set_obj_size(s);
  };

  virtual void
  set_name(const std::string& n) override
  {
    return next->set_name(n);
  };

  virtual void
  set_key(const rgw_obj_key& k) override
  {
    return next->set_key(k);
  };

  virtual rgw_obj
  get_obj(void) const override
  {
    return next->get_obj();
  };
  ...
  virtual void
  set_instance(const std::string& i) override
  {
    return next->set_instance(i);
  }

  virtual const std::string&
  get_instance() const override
  {
    return next->get_instance();
  }

  virtual bool
  have_instance(void) override
  {
    return next->have_instance();
  }

  virtual void
  clear_instance() override
  {
    return next->clear_instance();
  }
  ...
  virtual jspan_context&
  get_trace()
  {
    return next->get_trace();
  }

  virtual void
  set_trace(jspan_context&& _trace_ctx)
  {
    next->set_trace(std::move(_trace_ctx));
  }
...
public:
  ...
  virtual const std::optional<rgw::cksum::Cksum>&
  get_cksum()
  {
...
public:
  ...
  virtual const std::string&
  get_meta() const override
  {
    return next->get_meta();
  }
  ...
  virtual const std::string&
  get_upload_id() const override
  {
    return next->get_upload_id();
  }

  virtual const ACLOwner&
  ...
  virtual std::map<uint32_t, std::unique_ptr<MultipartPart>>&
  get_parts() override
  {
    return parts;
  }
  ...
  virtual int cleanup_orphaned_parts(
      const DoutPrefixProvider* dpp,
      CephContext* cct,
      optional_yield y,
      const rgw_obj& obj,
...
public:
  ...
  virtual int get_next_entry(
      ...
      const std::string& marker,
      LCEntry& entry) override;
  virtual int set_entry(
      const DoutPrefixProvider* dpp,
      optional_yield y,
      const std::string& oid,
      const LCEntry& entry) override;
  ...
  virtual int get_head(
      ...
      optional_yield y,
      const std::string& oid,
      LCHead& head) override;
  virtual int put_head(
      const DoutPrefixProvider* dpp,
      optional_yield y,
      const std::string& oid,
      const LCHead& head) override;
...
public:
  FilterRestoreSerializer(std::unique_ptr<RestoreSerializer> _next) :
    next(std::move(_next))
  {}
  ...
  virtual int
  unlock(const DoutPrefixProvider* dpp, optional_yield y) override
  {
...
public:
  FilterRestore(std::unique_ptr<Restore> _next) :
    next(std::move(_next))
  {}
  ...
  virtual int initialize(
      ...
      optional_yield y,
      int n_objs,
      std::vector<std::string>& obj_names) override;
  virtual int add_entries(
      const DoutPrefixProvider* dpp,
      optional_yield y,
      int index,
  ...
  virtual int list(
      ...
      bool* truncated) override;
  virtual int trim_entries(
      const DoutPrefixProvider* dpp,
      optional_yield y,
      int index,
      const std::string_view& marker) override;
...
public:
  ...
  virtual std::tuple<rgw::lua::LuaCodeType, int> get_script_or_bytecode(
      const DoutPrefixProvider* dpp,
      optional_yield y,
