...
public:
  TempURLApplier(CephContext* const cct, std::unique_ptr<rgw::sal::User> user) :
    LocalApplier(
        cct,
        std::move(user),
        std::nullopt,
  ...
  void modify_request_state(
      ...
      req_state* s) const override; /* in/out */
  void write_ops_log_entry(rgw_log_entry& entry) const override;

  struct Factory {
    virtual ~Factory() {}

    virtual aplptr_t create_apl_turl(
        CephContext* cct,
        const req_state* s,
...
public:
  TempURLEngine(
      ...
      const TempURLApplier::Factory* const apl_factory) :
    cct(cct), driver(_driver), apl_factory(apl_factory)
  {}
...
public:
  SignedTokenEngine(
      ...
      const rgw::auth::TokenExtractor* const extractor,
      const rgw::auth::LocalApplier::Factory* const apl_factory) :
    cct(cct), driver(_driver), extractor(extractor), apl_factory(apl_factory)
  {}
...
public:
  ...
  bool
  is_owner_of(const rgw_owner& o) const
  {
...
class SwiftAnonymousEngine : public rgw::auth::AnonymousEngine {
  ...
  bool
  is_applicable(const req_state* s) const noexcept override
  {
    return extractor->get_token(s).empty();
  }

public:
  SwiftAnonymousEngine(
      CephContext* const cct,
      const SwiftAnonymousApplier::Factory* const apl_factory,
      const rgw::auth::TokenExtractor* const extractor) :
    AnonymousEngine(cct, apl_factory), extractor(extractor)
  {}
...
class DefaultStrategy : public rgw::auth::Strategy,
  ...
  struct AuthTokenExtractor : rgw::auth::TokenExtractor {
    std::string
    get_token(const req_state* const s) const override
    {
      ...
      const rgw::auth::RemoteApplier::AuthInfo& info) const override
  {
    auto apl = rgw::auth::add_3rdparty(
        driver, rgw_user(s->account_name),
        rgw::auth::add_sysreq(
            cct, driver, s,
            rgw::auth::RemoteApplier(
                cct, driver, std::move(extra_acl_strategy), info,
        ...
        static_cast<rgw::auth::TokenExtractor*>(&auth_token_extractor))
  {
    ...
    if (!cct->_conf->rgw_keystone_url.empty()) {
      keystone_engine.emplace(
          cct, static_cast<rgw::auth::TokenExtractor*>(&auth_token_extractor),
...
template <>
inline constexpr uint32_t signature_hash_size<ceph::crypto::HMACSHA1> =
    CEPH_CRYPTO_HMACSHA1_DIGESTSIZE;
template <>
inline constexpr uint32_t signature_hash_size<ceph::crypto::HMACSHA256> =
    CEPH_CRYPTO_HMACSHA256_DIGESTSIZE;
template <>
inline constexpr uint32_t signature_hash_size<ceph::crypto::HMACSHA512> =
    CEPH_CRYPTO_HMACSHA512_DIGESTSIZE;
...
template <>
inline constexpr const char* signature_hash_name<ceph::crypto::HMACSHA1> =
    sha1_name;
;
template <>
inline constexpr const char* signature_hash_name<ceph::crypto::HMACSHA256> =
    sha256_name;
template <>
inline constexpr const char* signature_hash_name<ceph::crypto::HMACSHA512> =
    sha512_name;

template <class HASHFLAVOR>
inline const uint32_t signature_hash_name_size = -1;
template <>
inline constexpr uint32_t signature_hash_name_size<ceph::crypto::HMACSHA1> =
    sizeof sha1_name;
;
template <>
inline constexpr uint32_t signature_hash_name_size<ceph::crypto::HMACSHA256> =
    sizeof sha256_name;
template <>
inline constexpr uint32_t signature_hash_name_size<ceph::crypto::HMACSHA512> =
    sizeof sha512_name;
...
class FormatSignature<HASHFLAVOR, SignatureFlavor::BARE_HEX>
  : public SignatureHelperT<HASHFLAVOR> {
...
public:
  ...
  {
    buf_to_hex(
        (UCHARPTR)base_t::dest, signature_hash_size<HASHFLAVOR>,
        base_t::dest_str);
...
// prefix:base64
template <typename HASHFLAVOR>
class FormatSignature<HASHFLAVOR, SignatureFlavor::NAMED_BASE64>
  : public SignatureHelperT<HASHFLAVOR> {
  using UCHARPTR = const unsigned char*;
  using base_t = SignatureHelperT<HASHFLAVOR>;
...
public:
  ...
  int
  read_permissions(RGWOp* op, optional_yield) override
  {
    return 0;
  }
  ...
  virtual void
  free_policy(RGWAccessControlPolicy* policy)
  {}
};
