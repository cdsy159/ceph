...
struct rgw_sync_symmetric_group {
  ...
  rgw_sync_symmetric_group(
      ...
      const std::set<rgw_zone_id> _zones) :
    id(_id), zones(_zones)
  {}
...
struct rgw_sync_bucket_entity {
  ...
  static bool
  match_str(const std::string& s1, const std::string& s2)
  ...
  rgw_sync_bucket_entity(
    ...
    zone(_zone), bucket(_bucket.value_or(rgw_bucket()))
  {}
  ...
  bool
  match_zone(const rgw_zone_id& z) const
  {
    if (all_zones) {
      return true;
    }
  ...
  void
  apply_zone(const rgw_zone_id& z)
  {
    all_zones = false;
    zone = z;
  }
  ...
  static bool
  match_bucket_id(const std::string& bid1, const std::string& bid2)
  {
    return (bid1.empty() || bid2.empty() || (bid1 == bid2));
  }

  bool
  match_bucket(std::optional<rgw_bucket> b) const
  {
    if (!b) {
      return true;
    }
    ...
    return (
        match_str(bucket->tenant, b->tenant) &&
        match_str(bucket->name, b->name) &&
        match_bucket_id(bucket->bucket_id, b->bucket_id));
  }

  bool
  match(const rgw_sync_bucket_entity& entity) const
  {
    if (!entity.zone) {
  ...
  const bool
  operator<(const rgw_sync_bucket_entity& e) const
  {
...
struct rgw_sync_pipe_filter_tag {
  ...
  bool
  operator<(const rgw_sync_pipe_filter_tag& t) const
  {
    if (key < t.key) {
      return true;
    }
...
struct rgw_sync_pipe_acl_translation {
  ...
  bool
  operator==(const rgw_sync_pipe_acl_translation& aclt) const
  {
...
struct rgw_sync_pipe_dest_params {
  ...
  void
  set_storage_class(const std::string& sc)
  {
    storage_class = sc;
  }

  void
  set_owner(const rgw_user& owner)
  {
    if (owner.empty()) {
  ...
  bool
  operator==(const rgw_sync_pipe_dest_params& rhs) const
  {
    return (
        acl_translation == rhs.acl_translation &&
...
struct rgw_sync_bucket_pipe {
  ...
  const bool
  operator<(const rgw_sync_bucket_pipe& p) const
  {
    if (id < p.id) {
      return true;
    }
...
struct rgw_sync_bucket_entities {
  std::optional<rgw_bucket> bucket; /* define specific bucket */
  std::optional<std::set<rgw_zone_id>>
      zones; /* define specific zones, if not set then all zones */
  ...
  match_bucket(std::optional<rgw_bucket> b) const
  {
    ...
    return (
        rgw_sync_bucket_entity::match_str(bucket->tenant, b->tenant) &&
  ...
  bool
  match_zone(const rgw_zone_id& zone) const
  {
    if (!zones) {
      if (all_zones) {
        return true;
...
struct rgw_sync_bucket_pipes {
  ...
  bool
  match_source(const rgw_zone_id& zone, std::optional<rgw_bucket> b) const
  {
    return (source.match_zone(zone) && source.match_bucket(b));
  }

  bool
  match_dest(const rgw_zone_id& zone, std::optional<rgw_bucket> b) const
  {
    return (dest.match_zone(zone) && dest.match_bucket(b));
  }

  bool
  contains_zone_bucket(const rgw_zone_id& zone, std::optional<rgw_bucket> b) const
  {
  ...
  void get_potential_related_buckets(
...
struct rgw_sync_policy_group {
  ...
  rgw_sync_data_flow_group
      data_flow; /* override data flow, however, will not be able to
