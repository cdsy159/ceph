...
public:
  ...
  static const char*
  from_type(enum token_type type)
  {
    switch (type) {
    case TOKEN_AD:
      return "ad";
  ...
  valid() const
  {
    return ((type != TOKEN_NONE) && (!id.empty()) && (!key.empty()));
  }
  ...
  RGWToken&
  operator=(const std::string& json)
  {
    JSONParser p;
    p.parse(json.c_str(), json.length());
    JSONDecoder::decode_json(RGWToken::type_name, *this, &p);
  ...
  std::string
  encode_json_base64(Formatter* f)
  {
    encode_json(f);
...
inline std::ostream&
operator<<(std::ostream& os, const RGWToken& token)
{
  os << "<<RGWToken" << " type=" << RGWToken::from_type(token.type)
     << " id=" << token.id << " key=" << token.key << ">>";
  return os;
}
