    ...
    const std::string& tenant)
{
  ...
  do {
    ret = driver->list_buckets(
        ...
        listing, y);
    if (ret < 0) {
      ldpp_dout(dpp, 0) << "failed to list buckets: " << cpp_strerror(ret)
                        << dendl;
      return ret;
    }
    ...
    for (auto& ent : listing.buckets) {
      ...
      if (ret < 0) {
        ldpp_dout(dpp, 0) << "ERROR: could not read bucket info: bucket="
                          << bucket << " ret=" << ret << dendl;
        continue;
      }
      ret = bucket->sync_owner_stats(dpp, y, &ent);
      if (ret < 0) {
        ldpp_dout(dpp, 0) << "ERROR: could not sync bucket stats: ret=" << ret
                          << dendl;
        return ret;
      }
      ret = bucket->check_bucket_shards(dpp, ent.count, y);
      if (ret < 0) {
        ldpp_dout(dpp, 0) << "ERROR in check_bucket_shards: "
  ...
  if (ret < 0) {
    ldpp_dout(dpp, 0) << "ERROR: failed to complete syncing owner stats: ret="
                      << ret << dendl;
    return ret;
  }
...
int
rgw_validate_tenant_name(const string& t)
{
  struct tench {
    static bool
    is_good(char ch)
 ...
 */
void
rgw_get_anon_user(RGWUserInfo& info)
{
...
static bool
validate_access_key(string& key)
{
...
void
rgw_generate_secret_key(CephContext* cct, std::string& secret_key)
{
