...
bool
RGWBWRoutingRuleCondition::check_key_condition(const string& key)
{
  return (
      key.size() >= key_prefix_equals.size() &&
      key.compare(0, key_prefix_equals.size(), key_prefix_equals) == 0);
}

void
RGWBWRoutingRule::apply_rule(
    const string& default_protocol,
    ...
    int* redirect_code)
{
  ...
  string protocol =
      (!redirect.protocol.empty() ? redirect.protocol : default_protocol);
  string hostname =
      (!redirect.hostname.empty() ? redirect.hostname : default_hostname);
...
bool
RGWBWRoutingRules::check_key_and_error_code_condition(
    const string& key,
    int error_code,
    RGWBWRoutingRule** rule)
{
  for (list<RGWBWRoutingRule>::iterator iter = rules.begin();
       iter != rules.end(); ++iter) {
    if (iter->check_key_condition(key) &&
        iter->check_error_code_condition(error_code)) {
      *rule = &(*iter);
      return true;
    }
  }
...
bool
RGWBWRoutingRules::check_key_condition(const string& key, RGWBWRoutingRule** rule)
{
  for (list<RGWBWRoutingRule>::iterator iter = rules.begin();
       iter != rules.end(); ++iter) {
    if (iter->check_key_condition(key)) {
...
bool
RGWBWRoutingRules::check_error_code_condition(
...
bool
RGWBucketWebsiteConf::should_redirect(
    const string& key,
    const int http_error_code,
    RGWBWRoutingRule* redirect)
{
  ...
  if (!redirect_all.hostname.empty()) {
    ...
    *redirect = redirect_all_rule;
    return true;
  } else if (!routing_rules.check_key_and_error_code_condition(
...
bool
RGWBucketWebsiteConf::get_effective_key(
    const string& key,
...
void
RGWRedirectInfo::dump(Formatter* f) const
{
...
void
RGWRedirectInfo::decode_json(JSONObj* obj)
{
...
void
RGWBWRedirectInfo::dump(Formatter* f) const
{
...
void
RGWBWRedirectInfo::decode_json(JSONObj* obj)
{
  JSONDecoder::decode_json("redirect", redirect, obj);
  JSONDecoder::decode_json(
      "replace_key_prefix_with", replace_key_prefix_with, obj);
  JSONDecoder::decode_json("replace_key_with", replace_key_with, obj);
}

void
RGWBWRoutingRuleCondition::dump(Formatter* f) const
{
  encode_json("key_prefix_equals", key_prefix_equals, f);
  encode_json(
      "http_error_code_returned_equals", (int)http_error_code_returned_equals,
      f);
}

void
RGWBWRoutingRuleCondition::decode_json(JSONObj* obj)
{
...
void
RGWBWRoutingRule::dump(Formatter* f) const
{
...
void
RGWBWRoutingRule::decode_json(JSONObj* obj)
{
...
void
RGWBWRoutingRules::dump(Formatter* f) const
{
  encode_json("rules", rules, f);
}

void
RGWBWRoutingRules::decode_json(JSONObj* obj)
{
  JSONDecoder::decode_json("rules", rules, obj);
}

void
RGWBucketWebsiteConf::dump(Formatter* f) const
{
  if (!redirect_all.hostname.empty()) {
...
void
RGWBucketWebsiteConf::decode_json(JSONObj* obj)
{
...
void
RGWBWRedirectInfo::dump_xml(Formatter* f) const
{
...
void
RGWBWRedirectInfo::decode_xml(XMLObj* obj)
{
  RGWXMLDecoder::decode_xml("Protocol", redirect.protocol, obj);
  RGWXMLDecoder::decode_xml("HostName", redirect.hostname, obj);
  int code = 0;
  bool has_http_redirect_code =
      RGWXMLDecoder::decode_xml("HttpRedirectCode", code, obj);
  if (has_http_redirect_code && !(code > WEBSITE_HTTP_REDIRECT_CODE_MIN &&
                                  code < WEBSITE_HTTP_REDIRECT_CODE_MAX)) {
    throw RGWXMLDecoder::err(
        "The provided HTTP redirect code is not valid. Valid codes are 3XX "
        "except 300.");
  }
  redirect.http_redirect_code = code;
  bool has_replace_key_prefix_with = RGWXMLDecoder::decode_xml(
      "ReplaceKeyPrefixWith", replace_key_prefix_with, obj);
  bool has_replace_key_with =
...
void
RGWBWRoutingRuleCondition::dump_xml(Formatter* f) const
{
  ...
  if (http_error_code_returned_equals > 0) {
    encode_xml(
        "HttpErrorCodeReturnedEquals", (int)http_error_code_returned_equals, f);
  }
}
...
void
RGWBWRoutingRuleCondition::decode_xml(XMLObj* obj)
{
  RGWXMLDecoder::decode_xml("KeyPrefixEquals", key_prefix_equals, obj);
  int code = 0;
  bool has_http_error_code_returned_equals =
  ...
  if (has_http_error_code_returned_equals &&
    ...
    throw RGWXMLDecoder::err(
        "The provided HTTP redirect code is not valid. Valid codes are 4XX or "
        "5XX.");
  }
  http_error_code_returned_equals = code;
}
...
void
RGWBWRoutingRule::dump_xml(Formatter* f) const
{
...
void
RGWBWRoutingRule::decode_xml(XMLObj* obj)
{
...
encode_xml(
    const char* name,
    const std::list<RGWBWRoutingRule>& l,
    ceph::Formatter* f)
{
...
void
RGWBucketWebsiteConf::dump_xml(Formatter* f) const
{
  if (!redirect_all.hostname.empty()) {
...
void
decode_xml_obj(list<RGWBWRoutingRule>& l, XMLObj* obj)
{
  do_decode_xml_obj(l, "RoutingRule", obj);
}

void
RGWBucketWebsiteConf::decode_xml(XMLObj* obj)
{
