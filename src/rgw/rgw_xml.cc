...
void
XMLObjIter::set(
    const XMLObjIter::map_iter_t& _cur,
    const XMLObjIter::map_iter_t& _end)
{
  cur = _cur;
  end = _end;
}
...
bool
XMLObjIter::get_name(std::string& name) const
{
  if (cur == end) {
    return false;
  }
...
ostream&
operator<<(ostream& out, const XMLObj& obj)
{
...
bool
XMLObj::xml_start(XMLObj* parent, const char* el, const char** attr)
{
  this->parent = parent;
  obj_type = el;
...
bool
XMLObj::xml_end(const char* el)
{
  return true;
}

void
XMLObj::xml_handle_data(const char* s, int len)
{
  data.append(s, len);
}

const std::string&
XMLObj::XMLObj::get_data() const
{
  return data;
}

const std::string&
XMLObj::XMLObj::get_obj_type() const
{
  return obj_type;
}

XMLObj*
XMLObj::XMLObj::get_parent()
{
  return parent;
}

void
XMLObj::add_child(const std::string& el, XMLObj* obj)
{
  children.insert(std::pair<std::string, XMLObj*>(el, obj));
}

bool
XMLObj::get_attr(const std::string& name, std::string& attr) const
{
...
XMLObjIter
XMLObj::find(const std::string& name)
{
  XMLObjIter iter;
...
RGWXMLParser::RGWXMLParser() :
  buf(nullptr), buf_len(0), cur_obj(nullptr), success(true), init_called(false)
{
...
void
RGWXMLParser::call_xml_start(void* user_data, const char* el, const char** attr)
{
...
void
RGWXMLParser::call_xml_end(void* user_data, const char* el)
{
  RGWXMLParser* handler = static_cast<RGWXMLParser*>(user_data);
...
void
RGWXMLParser::call_xml_handle_data(void* user_data, const char* s, int len)
{
  RGWXMLParser* handler = static_cast<RGWXMLParser*>(user_data);
...
RGWXMLParser::init()
{
  ...
  XML_SetElementHandler(
      p, RGWXMLParser::call_xml_start, RGWXMLParser::call_xml_end);
...
bool
RGWXMLParser::parse(const char* _buf, int len, int done)
{
...
void
decode_xml_obj(unsigned long& val, XMLObj* obj)
{
  auto& s = obj->get_data();
...
void
decode_xml_obj(long& val, XMLObj* obj)
{
...
void
decode_xml_obj(long long& val, XMLObj* obj)
{
  const std::string s = obj->get_data();
  const char* start = s.c_str();
  char* p;
...
void
decode_xml_obj(unsigned long long& val, XMLObj* obj)
{
  const std::string s = obj->get_data();
  const char* start = s.c_str();
  char* p;
...
void
decode_xml_obj(int& val, XMLObj* obj)
{
  long l;
...
void
decode_xml_obj(unsigned& val, XMLObj* obj)
{
  unsigned long l;
  decode_xml_obj(l, obj);
...
void
decode_xml_obj(bool& val, XMLObj* obj)
{
  const std::string s = obj->get_data();
...
void
decode_xml_obj(bufferlist& val, XMLObj* obj)
{
  const std::string s = obj->get_data();
...
void
decode_xml_obj(utime_t& val, XMLObj* obj)
{
  const std::string s = obj->get_data();
  uint64_t epoch;
  uint64_t nsec;
...
void
decode_xml_obj(ceph::real_time& val, XMLObj* obj)
{
  const std::string s = obj->get_data();
  uint64_t epoch;
  uint64_t nsec;
...
void
encode_xml(const char* name, const string& val, Formatter* f)
{
  f->dump_string(name, val);
}

void
encode_xml(const char* name, const string_view& val, Formatter* f)
{
  f->dump_string(name, val);
}

void
encode_xml(const char* name, const char* val, Formatter* f)
{
  f->dump_string(name, val);
}

void
encode_xml(const char* name, bool val, Formatter* f)
{
  std::string s;
  if (val)
    s = "True";
...
void
encode_xml(const char* name, int val, Formatter* f)
{
  f->dump_int(name, val);
}

void
encode_xml(const char* name, long val, Formatter* f)
{
  f->dump_int(name, val);
}

void
encode_xml(const char* name, unsigned val, Formatter* f)
{
  f->dump_unsigned(name, val);
}

void
encode_xml(const char* name, unsigned long val, Formatter* f)
{
  f->dump_unsigned(name, val);
}

void
encode_xml(const char* name, unsigned long long val, Formatter* f)
{
  f->dump_unsigned(name, val);
}

void
encode_xml(const char* name, long long val, Formatter* f)
{
  f->dump_int(name, val);
}

void
encode_xml(const char* name, const utime_t& val, Formatter* f)
{
  val.gmtime(f->dump_stream(name));
}

void
encode_xml(const char* name, const bufferlist& bl, Formatter* f)
{
