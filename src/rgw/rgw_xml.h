...
public:
  ...
  virtual ~XMLObjIter();
  void set(
      const XMLObjIter::const_map_iter_t& _cur,
      const XMLObjIter::const_map_iter_t& _end);
  XMLObj* get_next();
...
operator<<(std::ostream& out, RGWXMLDecoder::err& err)
{
  return out << err.what();
}
...
template <class T>
void
decode_xml_obj(T& val, XMLObj* obj)
{
  val.decode_xml(obj);
}

static inline void
decode_xml_obj(std::string& val, XMLObj* obj)
{
  val = obj->get_data();
}
...
template <class T>
void
decode_xml_obj(std::optional<T>& val, XMLObj* obj)
{
  val.emplace();
...
template <class T>
void
do_decode_xml_obj(std::list<T>& l, const std::string& name, XMLObj* obj)
{
  l.clear();
...
template <class T>
bool
RGWXMLDecoder::decode_xml(const char* name, T& val, XMLObj* obj, bool mandatory)
{
  XMLObjIter iter = obj->find(name);
...
template <class T>
void
RGWXMLDecoder::decode_xml(const char* name, T& val, T& default_val, XMLObj* obj)
{
  XMLObjIter iter = obj->find(name);
  XMLObj* o = iter.get_next();
  if (!o) {
...
template <class T>
static void
encode_xml(const char* name, const T& val, ceph::Formatter* f)
{
...
template <class T>
static void
encode_xml(const char* name, const char* ns, const T& val, ceph::Formatter* f)
{
...
template <class T>
static void
encode_xml(const char* name, const std::vector<T>& l, ceph::Formatter* f)
{
...
template <class T>
static void
encode_xml(const char* name, const std::optional<T>& o, ceph::Formatter* f)
{
  if (!o) {
    return;
  }
