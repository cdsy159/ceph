...
int
RGWSI_BucketIndex_RADOS::open_pool(
...
int
RGWSI_BucketIndex_RADOS::open_bucket_index_pool(
    const DoutPrefixProvider* dpp,
...
int
RGWSI_BucketIndex_RADOS::open_bucket_index_base(
    const DoutPrefixProvider* dpp,
    const RGWBucketInfo& bucket_info,
    librados::IoCtx* index_pool,
...
bucket_obj_with_generation(
    char* buf,
    size_t len,
    const string& bucket_oid_base,
    uint64_t gen_id,
    uint32_t shard_id)
{
  return snprintf(
      buf, len, "%s.%" PRIu64 ".%d", bucket_oid_base.c_str(), gen_id, shard_id);
}

static char
bucket_obj_without_generation(
    char* buf,
    size_t len,
    const string& bucket_oid_base,
    uint32_t shard_id)
  ...
  if (!num_shards) {
    ...
    if (shard_id < 0) {
      for (uint32_t i = 0; i < num_shards; ++i) {
        if (gen_id) {
          bucket_obj_with_generation(
              buf, sizeof(buf), bucket_oid_base, gen_id, i);
        } else {
      ...
      if (std::cmp_greater(shard_id, num_shards)) {
        ...
        if (gen_id) {
          bucket_obj_with_generation(
              buf, sizeof(buf), bucket_oid_base, gen_id, shard_id);
        } else {
          // for backward compatibility, gen_id(0) will not be added in the object name
          bucket_obj_without_generation(
              buf, sizeof(buf), bucket_oid_base, shard_id);
        }
    ...
    map<int, string>* bucket_instance_ids)
{
  ...
  string bucket_oid_base;
  int ret =
      open_bucket_index_base(dpp, bucket_info, index_pool, &bucket_oid_base);
  if (ret < 0) {
    ldpp_dout(dpp, 20) << __func__ << ": open_bucket_index_pool() returned "
                       << ret << dendl;
    return ret;
  ...
  get_bucket_index_objects(
      bucket_oid_base, idx_layout.layout.normal.num_shards, idx_layout.gen,
      bucket_objs, shard_id);
  if (bucket_instance_ids) {
    get_bucket_instance_ids(
        bucket_info, idx_layout.layout.normal.num_shards, shard_id,
        bucket_instance_ids);
  }
  return 0;
}
    ...
    int* shard_id)
{
  ...
  case rgw::BucketHashType::Mod:
    if (!normal.num_shards) {
      ...
      if (gen_id) {
        bucket_obj_with_generation(
            buf, sizeof(buf), bucket_oid_base, gen_id, sid);
      } else {
...
int
RGWSI_BucketIndex_RADOS::open_bucket_index_shard(
    const DoutPrefixProvider* dpp,
    const RGWBucketInfo& bucket_info,
    ...
    int* shard_id)
{
  ...
  int ret = open_bucket_index_base(
      dpp, bucket_info, &bucket_obj->ioctx, &bucket_oid_base);
  if (ret < 0) {
    ldpp_dout(dpp, 20) << __func__ << ": open_bucket_index_pool() returned "
                       << ret << dendl;
    return ret;
  ...
  ret = get_bucket_index_object(
      bucket_oid_base, current_index.layout.normal, current_index.gen, obj_key,
    ...
    rgw_rados_ref* bucket_obj)
{
  ...
  get_bucket_index_object(
      bucket_oid_base, index.layout.normal, index.gen, shard_id,
      &bucket_obj->obj.oid);
...
struct IndexHeadReader : rgwrados::shard_io::RadosReader {
  ...
  void
  prepare_read(int shard, librados::ObjectReadOperation& op) override
  {
  ...
  Result
  on_complete(int, boost::system::error_code ec) override
  {
    // ignore ENOENT
...
int
RGWSI_BucketIndex_RADOS::cls_bucket_head(
    const DoutPrefixProvider* dpp,
    const RGWBucketInfo& bucket_info,
    const rgw::bucket_index_layout_generation& idx_layout,
    int shard_id,
    ...
    optional_yield y)
{
  ...
  int r = open_bucket_index(
      dpp, bucket_info, shard_id, idx_layout, &index_pool, &oids,
      bucket_instance_ids);
  if (r < 0)
    return r;
  ...
  if (y) {
    ...
    rgwrados::shard_io::async_reads(
        reader, oids, max_aio, ceph::async::use_blocked[ec]);
  }
  if (ec) {
  ...
  try {
    std::transform(
        buffers.begin(), buffers.end(), std::back_inserter(*headers),
        [](const auto& kv) {
...
struct IndexInitWriter : rgwrados::shard_io::RadosRevertibleWriter {
  ...
  void
  prepare_write(int shard, librados::ObjectWriteOperation& op) override
  {
  ...
  void
  prepare_revert(int shard, librados::ObjectWriteOperation& op) override
  {
    ...
    bool judge_support_logrecord)
{
  ...
  map<int, string> bucket_objs;
  get_bucket_index_objects(
      dir_oid, idx_layout.layout.normal.num_shards, idx_layout.gen,
      &bucket_objs);
  ...
  if (y) {
    ...
    maybe_warn_about_blocking(dpp);
    rgwrados::shard_io::async_writes(
        writer, bucket_objs, max_aio, ceph::async::use_blocked[ec]);
  }
  return ceph::from_error_code(ec);
}
...
int
RGWSI_BucketIndex_RADOS::read_stats(
    const DoutPrefixProvider* dpp,
    const RGWBucketInfo& bucket_info,
    RGWBucketEnt* result,
    optional_yield y)
...
int
RGWSI_BucketIndex_RADOS::get_reshard_status(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    const RGWBucketInfo& bucket_info,
    list<cls_rgw_bucket_instance_entry>* status)
{
  ...
  if (y) {
    ...
    maybe_warn_about_blocking(dpp);
    rgwrados::shard_io::async_reads(
        reader, bucket_objs, max_aio, ceph::async::use_blocked[ec]);
  }
  if (ec) {
    return ceph::from_error_code(ec);
  }
  ...
  try {
    std::transform(
        buffers.begin(), buffers.end(), std::back_inserter(*status),
        [](const auto& kv) {
...
struct ReshardStatusWriter : rgwrados::shard_io::RadosWriter {
  ...
  void
  prepare_write(int, librados::ObjectWriteOperation& op) override
  {
...
int
RGWSI_BucketIndex_RADOS::set_reshard_status(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    const RGWBucketInfo& bucket_info,
    cls_rgw_reshard_status status)
...
int
RGWSI_BucketIndex_RADOS::trim_reshard_log(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    const RGWBucketInfo& bucket_info)
{
...
int
RGWSI_BucketIndex_RADOS::set_tag_timeout(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    const RGWBucketInfo& bucket_info,
    uint64_t timeout)
...
int
RGWSI_BucketIndex_RADOS::check_index(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    const RGWBucketInfo& bucket_info,
    std::map<int, bufferlist>& buffers)
...
int
RGWSI_BucketIndex_RADOS::rebuild_index(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    const RGWBucketInfo& bucket_info)
{
...
struct ListReader : rgwrados::shard_io::RadosReader {
  ...
  prepare_read(int shard, librados::ObjectReadOperation& op) override
  {
    ...
    const cls_rgw_obj_key& marker = result.marker.empty() ? start_obj
                                                          : result.marker;
    cls_rgw_bucket_list_op(
        op, marker, prefix, delimiter, num_entries, list_versions, &result);
  }
...
int
RGWSI_BucketIndex_RADOS::list_objects(
    const DoutPrefixProvider* dpp,
    optional_yield y,
    librados::IoCtx& index_pool,
    ...
    std::map<int, rgw_cls_list_ret>& results)
{
  ...
  if (y) {
    ...
    auto yield = y.get_yield_context();
    auto ex = yield.get_executor();
    auto reader = ListReader{*dpp,   ex,        index_pool,  start_obj,
                             prefix, delimiter, num_entries, list_versions,
                             results};
    ...
    optional_yield y)
{
  ...
  if (ret < 0) {
    ldpp_dout(dpp, -1) << "ERROR: failed writing bilog (bucket=" << info.bucket
                       << "); ret=" << ret << dendl;
    return ret;
  }
