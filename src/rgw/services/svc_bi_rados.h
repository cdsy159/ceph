...
class RGWSI_BucketIndex_RADOS : public RGWSI_BucketIndex {
  ...
  int open_bucket_index_pool(
      const DoutPrefixProvider* dpp,
      const RGWBucketInfo& bucket_info,
      librados::IoCtx* index_pool);
  int open_bucket_index_base(
      const DoutPrefixProvider* dpp,
      const RGWBucketInfo& bucket_info,
      librados::IoCtx* index_pool,
...
public:
  ...
  static int
  shard_id(const std::string& key, int max_shards)
  {
    return rgw_shard_id(key, max_shards);
  }

  static int32_t
  bucket_shard_index(const std::string& key, int num_shards)
  {
  ...
  static int32_t
  bucket_shard_index(const rgw_obj_key& obj_key, int num_shards)
  {
  ...
  int set_reshard_status(
      const DoutPrefixProvider* dpp,
      optional_yield y,
      const RGWBucketInfo& bucket_info,
      cls_rgw_reshard_status status);
  int trim_reshard_log(
      const DoutPrefixProvider* dpp,
      optional_yield,
      const RGWBucketInfo& bucket_info);
  ...
  int handle_overwrite(
      const DoutPrefixProvider* dpp,
      const RGWBucketInfo& info,
      const RGWBucketInfo& orig_info,
      optional_yield y) override;

  int open_bucket_index_shard(
      const DoutPrefixProvider* dpp,
      const RGWBucketInfo& bucket_info,
      const std::string& obj_key,
