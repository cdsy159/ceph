...
void
RGWSI_BILog_RADOS::init(RGWSI_BucketIndex_RADOS* bi_rados_svc)
{
  svc.bi = bi_rados_svc;
}
    ...
    std::string_view end_marker)
{
  ...
  int r = svc.bi->open_bucket_index(
      dpp, bucket_info, shard_id, current_index, &index_pool, &bucket_objs,
      nullptr);
  if (r < 0) {
    return r;
  }
  ...
  if (y) {
    ...
    auto writer =
        TrimWriter{*dpp, ex, index_pool, start_marker_mgr, end_marker_mgr};
...
static void
build_bucket_index_marker(
    const string& shard_id_str,
...
struct LogReader : rgwrados::shard_io::RadosReader {
  ...
  LogReader(
      ...
      uint32_t max,
      std::map<int, cls_rgw_bi_log_list_ret>& logs) :
    RadosReader(dpp, std::move(ex), ioctx), start(start), max(max), logs(logs)
  {}
    ...
    std::map<int, cls_rgw_bi_log_list_ret>& logs)
{
  ...
  map<int, string> oids;
  const auto& current_index = rgw::log_to_index_layout(log_layout);
  int r = svc_bi->open_bucket_index(
      dpp, bucket_info, shard_id, current_index, &index_pool, &oids, nullptr);
  if (r < 0)
    return r;
    ...
    bool* truncated)
{
  ...
  r = bilog_list(
      dpp, y, svc.bi, bucket_info, log_layout, marker_mgr, shard_id, max,
      bi_log_lists);
  if (r < 0)
    return r;
  ...
  if (truncated) {
    for (viter = vcurrents.begin(), eiter = vends.begin();
...
int
RGWSI_BILog_RADOS::get_log_status(
    const DoutPrefixProvider* dpp,
    const RGWBucketInfo& bucket_info,
    const rgw::bucket_log_layout_generation& log_layout,
    int shard_id,
    ...
    optional_yield y)
{
  ...
  const auto& current_index = rgw::log_to_index_layout(log_layout);
  int r = svc.bi->cls_bucket_head(
      dpp, bucket_info, current_index, shard_id, &headers, &bucket_instance_ids,
      y);
  if (r < 0)
    return r;
