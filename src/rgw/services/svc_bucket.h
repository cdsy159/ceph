...
public:
  ...
  virtual int create_entrypoint_lister(
      const DoutPrefixProvider* dpp,
      const std::string& marker,
      std::unique_ptr<RGWMetadataLister>& lister) = 0;
  virtual int create_instance_lister(
      const DoutPrefixProvider* dpp,
      const std::string& marker,
      std::unique_ptr<RGWMetadataLister>& lister) = 0;

  virtual int read_bucket_entrypoint_info(
  ...
  virtual int store_bucket_entrypoint_info(
      const std::string& key,
  ...
  virtual int remove_bucket_entrypoint_info(
      const std::string& key,
      RGWObjVersionTracker* objv_tracker,
      optional_yield y,
      const DoutPrefixProvider* dpp) = 0;

  virtual int read_bucket_instance_info(
      const std::string& key,
      RGWBucketInfo* info,
      real_time* pmtime,
      std::map<std::string, bufferlist>* pattrs,
  ...
  virtual int remove_bucket_instance_info(
      const std::string& key,
  ...
  virtual int read_bucket_stats(
      const rgw_bucket& bucket,
      RGWBucketEnt* ent,
      optional_yield y,
      const DoutPrefixProvider* dpp) = 0;

  virtual int read_buckets_stats(
