    ...
    RGWSI_Bucket_SObj* bucket_sobj_svc)
{
  ...
  hint_index_mgr.reset(
      new RGWSI_Bucket_Sync_SObj_HintIndexManager(svc.zone, svc.sysobj));
}

int
RGWSI_Bucket_Sync_SObj::do_start(optional_yield, const DoutPrefixProvider* dpp)
{
  sync_policy_cache.reset(
      new RGWChainedCacheImpl<bucket_sync_policy_cache_entry>);
...
void
RGWSI_Bucket_Sync_SObj::get_hint_entities(
    ...
    const DoutPrefixProvider* dpp)
{
  ...
  for (auto& b : buckets) {
    ...
    int ret = svc.bucket_sobj->read_bucket_info(
        b, &hint_bucket_info, nullptr, nullptr, boost::none, y, dpp);
    if (ret < 0) {
      ldpp_dout(dpp, 20) << "could not init bucket info for hint bucket=" << b
                         << " ... skipping" << dendl;
      continue;
    }
...
int
RGWSI_Bucket_Sync_SObj::resolve_policy_hints(
    ...
    const DoutPrefixProvider* dpp)
{
  ...
  get_hint_entities(
      source_zones, handler->get_source_hints(), &hint_entities, y, dpp);
  get_hint_entities(
      target_zones, handler->get_target_hints(), &hint_entities, y, dpp);
  ...
  for (auto& hint_entity : hint_entities) {
    ...
    if (iter != temp_map.end()) {
      ...
      int r = do_get_policy_handler(
          zid, hint_bucket, temp_map, &hint_bucket_handler, y, dpp);
      if (r < 0) {
        ldpp_dout(dpp, 20)
  ...
  handler->set_resolved_hints(
      std::move(resolved_sources), std::move(resolved_dests));

  return 0;
}

int
RGWSI_Bucket_Sync_SObj::do_get_policy_handler(
    ...
    const DoutPrefixProvider* dpp)
{
  ...
  if (bucket.bucket_id.empty()) {
    ...
    int ret = svc.bucket_sobj->read_bucket_entrypoint_info(
        RGWSI_Bucket::get_entrypoint_meta_key(bucket), &ep_info,
    ...
    if (ret < 0) {
      if (ret != -ENOENT) {
        ldout(cct, 0) << "ERROR: svc.bucket->read_bucket_info(bucket=" << bucket
                      << ") returned r=" << ret << dendl;
      }
      return ret;
    }
  ...
  int r = svc.bucket_sobj->read_bucket_instance_info(
      bucket_key, &bucket_info, nullptr, &attrs, y, dpp, &cache_info);
  if (r < 0) {
    if (r != -ENOENT) {
      ldpp_dout(dpp, 0) << "ERROR: svc.bucket->read_bucket_instance_info(key="
                        << bucket_key << ") returned r=" << r << dendl;
    }
    return r;
  }
  ...
  if (!zone_policy_handler) {
    ldpp_dout(dpp, 20) << "ERROR: could not find policy handler for zone="
                       << zone << dendl;
    return -ENOENT;
  }

  e.handler.reset(
      zone_policy_handler->alloc_child(bucket_info, std::move(attrs)));

  r = e.handler->init(dpp, y);
  if (r < 0) {
    ldpp_dout(dpp, 20) << "ERROR: failed to init bucket sync policy handler: r="
                       << r << dendl;
    return r;
  }
  ...
  r = resolve_policy_hints(
      self_entity, e.handler, zone_policy_handler, temp_map, y, dpp);
  if (r < 0) {
    ldpp_dout(dpp, 20) << "ERROR: failed to resolve policy hints: bucket_key="
                       << bucket_key << ", r=" << r << dendl;
    return r;
  }

  if (!sync_policy_cache->put(dpp, svc.cache, cache_key, &e, {&cache_info})) {
    ldpp_dout(dpp, 20) << "couldn't put bucket_sync_policy cache entry, might "
                          "have raced with data changes"
                       << dendl;
  }
...
int
RGWSI_Bucket_Sync_SObj::get_policy_handler(
    std::optional<rgw_zone_id> zone,
    std::optional<rgw_bucket> _bucket,
    RGWBucketSyncPolicyHandlerRef* handler,
    optional_yield y,
...
public:
  struct bi_entry {
    ...
    bool
    add(const rgw_bucket& info_source, const obj_version& info_source_ver)
    {
    ...
    bool
    remove(const rgw_bucket& info_source, const obj_version& info_source_ver)
    {
  ...
  struct single_instance_info {
    ...
    void
    get_entities(std::set<rgw_bucket>* result) const
    {
  ...
  struct info_map {
    ...
    void
    get_entities(const rgw_bucket& bucket, std::set<rgw_bucket>* result) const
    {
  ...
  RGWSI_BS_SObj_HintIndexObj(RGWSI_SysObj* _sysobj_svc, const rgw_raw_obj& _obj) :
    cct(_sysobj_svc->ctx()), obj(_obj), sysobj(_sysobj_svc->get_obj(obj))
  {
    svc.sysobj = _sysobj_svc;
  }
...
template <typename C1, typename C2>
int
RGWSI_BS_SObj_HintIndexObj::update(
    const DoutPrefixProvider* dpp,
    const rgw_bucket& entity,
    const RGWBucketInfo& info_source,
    C1* add,
...
#define MAX_RETRIES 25
  ...
  for (int i = 0; i < MAX_RETRIES; ++i) {
    if (!has_data) {
      ...
      if (r < 0) {
        ldpp_dout(dpp, 0)
            << "ERROR: cannot update hint index: failed to read: r=" << r
            << dendl;
        return r;
      }
    }
    ...
    if (r != -ECANCELED) {
      ldpp_dout(dpp, 0) << "ERROR: failed to flush hint index: obj=" << obj
                        << " r=" << r << dendl;
      return r;
    }
  ...
  ldpp_dout(dpp, 0)
      << "ERROR: failed to flush hint index: too many retries (obj=" << obj
      << "), likely a bug" << dendl;
...
template <typename C1, typename C2>
void
RGWSI_BS_SObj_HintIndexObj::update_entries(
    const rgw_bucket& info_source,
    const obj_version& info_source_ver,
    C1* add,
    C2* remove,
...
int
RGWSI_BS_SObj_HintIndexObj::read(const DoutPrefixProvider* dpp, optional_yield y)
{
  ...
  int r = sysobj.rop()
              ...
              .read(dpp, &bl, y);
  if (r < 0 && r != -ENOENT) {
    ldpp_dout(dpp, 0) << "ERROR: failed reading data (obj=" << obj
                      << "), r=" << r << dendl;
    return r;
  }
...
int
RGWSI_BS_SObj_HintIndexObj::flush(const DoutPrefixProvider* dpp, optional_yield y)
{
  int r;
...
rgw_raw_obj
RGWSI_Bucket_Sync_SObj_HintIndexManager::get_sources_obj(
...
rgw_raw_obj
RGWSI_Bucket_Sync_SObj_HintIndexManager::get_dests_obj(
    const rgw_bucket& bucket) const
{
  rgw_bucket b = bucket;
  b.bucket_id.clear();
    ...
    optional_yield y)
{
  ...
  if (!added_dests.empty() || !removed_dests.empty()) {
    /* update our dests */
    RGWSI_BS_SObj_HintIndexObj index(
        svc.sysobj, get_dests_obj(bucket_info.bucket));
    int r = index.update(
        dpp, bucket_info.bucket, bucket_info, &added_dests, &removed_dests, y);
    if (r < 0) {
      ldpp_dout(dpp, 0) << "ERROR: failed to update targets index for bucket="
                        << bucket_info.bucket << " r=" << r << dendl;
      return r;
    }

    /* update dest buckets */
    for (auto& dest_bucket : added_dests) {
      RGWSI_BS_SObj_HintIndexObj dep_index(
          svc.sysobj, get_sources_obj(dest_bucket));
      int r = dep_index.update(
          dpp, dest_bucket, bucket_info, &self_entity,
          static_cast<C2*>(nullptr), y);
      if (r < 0) {
        ldpp_dout(dpp, 0) << "ERROR: failed to update targets index for bucket="
                          << dest_bucket << " r=" << r << dendl;
        return r;
      }
    }
    ...
    for (auto& dest_bucket : removed_dests) {
      RGWSI_BS_SObj_HintIndexObj dep_index(
          svc.sysobj, get_sources_obj(dest_bucket));
      int r = dep_index.update(
          dpp, dest_bucket, bucket_info, static_cast<C1*>(nullptr),
          &self_entity, y);
      if (r < 0) {
        ldpp_dout(dpp, 0) << "ERROR: failed to update targets index for bucket="
                          << dest_bucket << " r=" << r << dendl;
  ...
  if (!added_sources.empty() || !removed_sources.empty()) {
    RGWSI_BS_SObj_HintIndexObj index(
        svc.sysobj, get_sources_obj(bucket_info.bucket));
    /* update our sources */
    int r = index.update(
        dpp, bucket_info.bucket, bucket_info, &added_sources, &removed_sources,
        y);
    if (r < 0) {
      ldpp_dout(dpp, 0) << "ERROR: failed to update targets index for bucket="
                        << bucket_info.bucket << " r=" << r << dendl;
    ...
    for (auto& source_bucket : added_sources) {
      RGWSI_BS_SObj_HintIndexObj dep_index(
          svc.sysobj, get_dests_obj(source_bucket));
      int r = dep_index.update(
          dpp, source_bucket, bucket_info, &self_entity,
          static_cast<C2*>(nullptr), y);
      if (r < 0) {
        ldpp_dout(dpp, 0) << "ERROR: failed to update targets index for bucket="
                          << source_bucket << " r=" << r << dendl;
        return r;
      }
    }
    /* update removed dest buckets */
    for (auto& source_bucket : removed_sources) {
      RGWSI_BS_SObj_HintIndexObj dep_index(
          svc.sysobj, get_dests_obj(source_bucket));
      int r = dep_index.update(
          dpp, source_bucket, bucket_info, static_cast<C1*>(nullptr),
          &self_entity, y);
      if (r < 0) {
        ldpp_dout(dpp, 0) << "ERROR: failed to update targets index for bucket="
                          << source_bucket << " r=" << r << dendl;
...
int
RGWSI_Bucket_Sync_SObj::handle_bi_removal(
    const DoutPrefixProvider* dpp,
    const RGWBucketInfo& bucket_info,
    optional_yield y)
{
  ...
  if (bucket_info.sync_policy) {
    bucket_info.sync_policy->get_potential_related_buckets(
        bucket_info.bucket, &sources_set, &dests_set);
  }
  ...
  return hint_index_mgr->update_hints(
      dpp, bucket_info, added_dests, removed_dests, added_sources,
      removed_sources, y);
}
    ...
    optional_yield y)
{
  ...
  if (orig_bucket_info && orig_bucket_info->sync_policy) {
    orig_bucket_info->sync_policy->get_potential_related_buckets(
        bucket_info.bucket, &orig_sources, &orig_dests);
  }
  ...
  if (bucket_info.sync_policy) {
    bucket_info.sync_policy->get_potential_related_buckets(
        bucket_info.bucket, &sources, &dests);
  }
  ...
  ldpp_dout(dpp, 20) << __func__ << "(): bucket=" << bucket_info.bucket
                     << ": orig_dests=" << orig_dests << " new_dests=" << dests
                     << dendl;
  ldpp_dout(dpp, 20) << __func__ << "(): bucket=" << bucket_info.bucket
...
int
RGWSI_Bucket_Sync_SObj::get_bucket_sync_hints(
    const DoutPrefixProvider* dpp,
    const rgw_bucket& bucket,
    std::set<rgw_bucket>* sources,
    std::set<rgw_bucket>* dests,
    optional_yield y)
{
  ...
  if (sources) {
    RGWSI_BS_SObj_HintIndexObj index(
        svc.sysobj, hint_index_mgr->get_sources_obj(bucket));
    int r = index.read(dpp, y);
    if (r < 0) {
      ldpp_dout(dpp, 0) << "ERROR: failed to update sources index for bucket="
                        << bucket << " r=" << r << dendl;
      return r;
    }
  ...
  if (dests) {
    RGWSI_BS_SObj_HintIndexObj index(
        svc.sysobj, hint_index_mgr->get_dests_obj(bucket));
    int r = index.read(dpp, y);
    if (r < 0) {
      ldpp_dout(dpp, 0) << "ERROR: failed to read targets index for bucket="
                        << bucket << " r=" << r << dendl;
      return r;
    }
