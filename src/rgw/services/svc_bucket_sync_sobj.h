...
class RGWSI_Bucket_Sync_SObj : public RGWSI_Bucket_Sync {
  ...
  std::unique_ptr<RGWChainedCacheImpl<bucket_sync_policy_cache_entry>>
      sync_policy_cache;
  ...
  struct optional_zone_bucket {
    ...
    optional_zone_bucket(
        const std::optional<rgw_zone_id>& _zone,
        const std::optional<rgw_bucket>& _bucket) :
      zone(_zone), bucket(_bucket)
    {}

    bool
    operator<(const optional_zone_bucket& ozb) const
    {
      if (zone < ozb.zone) {
        return true;
      }
...
public:
  ...
  int handle_bi_update(
      const DoutPrefixProvider* dpp,
      RGWBucketInfo& bucket_info,
      RGWBucketInfo* orig_bucket_info,
      optional_yield y) override;
  int handle_bi_removal(
      const DoutPrefixProvider* dpp,
      const RGWBucketInfo& bucket_info,
      optional_yield y) override;

  int get_bucket_sync_hints(
      const DoutPrefixProvider* dpp,
