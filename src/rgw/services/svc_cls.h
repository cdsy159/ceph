...
public:
  ...
  public:
    ...
    std::string
    get_mfa_oid(const rgw_user& user)
    {
  ...
  RGWSI_Cls(CephContext* cct) :
    RGWServiceInstance(cct), mfa(cct), timelog(cct), lock(cct)
  {}

  void
  init(RGWSI_Zone* _zone_svc, librados::Rados* rados_)
  {
    rados = rados_;
    zone_svc = _zone_svc;
