...
RGWSI_MDLog::RGWSI_MDLog(
  ...
  RGWServiceInstance(cct), run_sync(_run_sync), cfgstore(_cfgstore)
{}
...
int
RGWSI_MDLog::do_start(optional_yield y, const DoutPrefixProvider* dpp)
{
  ...
  period_history.reset(
      new RGWPeriodHistory(cct, period_puller.get(), current_period));
    ...
    const DoutPrefixProvider* dpp) const
{
  ...
  int ret = rgw_get_system_obj(
      svc.sysobj, pool, oid, bl, objv_tracker, nullptr, y, dpp);
  if (ret < 0) {
    return ret;
  }
  if (bl.length() == 0) {
    ...
    if (ret < 0) {
      ldpp_dout(dpp, 0)
          << "ERROR: meta history is empty, but cannot remove it ("
  ...
  try {
    ...
    ldpp_dout(dpp, 1) << "failed to decode the mdlog history: " << e.what()
                      << dendl;
    return -EIO;
  }
  return 0;
    ...
    bool exclusive)
{
  ...
  return rgw_put_system_obj(
      dpp, svc.sysobj, pool, oid, bl, exclusive, objv_tracker, real_time{}, y);
}
...
public:
  ...
  ~SysObjReadCR() override
  {
    try {
      ...
      ldpp_dout(dpp, 0) << "ERROR: " << __func__
                        << ": reference counted object mismatched, \""
                        << e.what() << "\"" << dendl;
    }
  }
  ...
  send_request(const DoutPrefixProvider* dpp)
  {
    req = new RGWAsyncGetSystemObj(
        dpp, this, stack->create_completion_notifier(), svc, objv_tracker, obj,
        false, false);
...
public:
  ...
  operate(const DoutPrefixProvider* dpp)
  {
    reenter(this)
    {
      ...
      {
        rgw_raw_obj obj{
            svc.zone->get_zone_params().log_pool, RGWMetadataLogHistory::oid};
        constexpr bool empty_on_enoent = false;

        using ReadCR = SysObjReadCR<RGWMetadataLogHistory>;
        call(new ReadCR(
            dpp, async_processor, svc.sysobj, obj, &state, empty_on_enoent,
            objv_tracker));
      }
      if (retcode < 0) {
      ...
      *cursor = svc.mdlog->period_history->lookup(state.oldest_realm_epoch);
      ...
      ldpp_dout(dpp, 10) << "read mdlog history with oldest period id="
                         << state.oldest_period_id
                         << " realm_epoch=" << state.oldest_realm_epoch
                         << dendl;
      return set_cr_done();
    }
    return 0;
    ...
    reenter(this)
    {
      ...
      if (cursor.get_epoch() < existing.get_epoch()) {
        ldpp_dout(dpp, 4) << "found oldest log epoch=" << existing.get_epoch()
                          << ", rejecting trim at epoch=" << cursor.get_epoch()
                          << dendl;
    ...
    rgw::sal::ConfigStore* cfgstore)
{
  ...
  if (ret == -ENOENT) {
    ...
    ldpp_dout(dpp, 1) << "failed to read mdlog history: " << cpp_strerror(ret)
                      << dendl;
    return Cursor{ret};
  }
  ...
  if (period.get_realm_epoch() != state.oldest_realm_epoch) {
    ldpp_dout(dpp, 1) << "inconsistent mdlog history: read period id="
                      ...
                      << ", expected realm_epoch=" << state.oldest_realm_epoch
                      << dendl;
...
Cursor
RGWSI_MDLog::read_oldest_log_period(
    optional_yield y,
    const DoutPrefixProvider* dpp) const
{
  RGWMetadataLogHistory state;
...
RGWCoroutine*
RGWSI_MDLog::read_oldest_log_period_cr(
    const DoutPrefixProvider* dpp,
    Cursor* period,
...
int
RGWSI_MDLog::get_shard_id(const string& hash_key, int* shard_id)
{
  ceph_assert(current_log); // must have called init()
