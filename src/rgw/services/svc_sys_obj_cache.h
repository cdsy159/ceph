...
class RGWSI_SysObj_Cache : public RGWSI_SysObj_Core {
  ...
  void normalize_pool_and_obj(
      const rgw_pool& src_pool,
      const std::string& src_obj,
      rgw_pool& dst_pool,
      std::string& dst_obj);

protected:
  void
  init(librados::Rados* rados_, RGWSI_Zone* _zone_svc, RGWSI_Notify* _notify_svc)
  {
...
public:
  RGWSI_SysObj_Cache(const DoutPrefixProvider* dpp, CephContext* cct) :
    RGWSI_SysObj_Core(cct), asocket(dpp, this)
  {
    cache.set_ctx(cct);
  }
...
public:
  ...
  init(RGWSI_SysObj_Cache* _svc)
  {
    ...
    expiry = std::chrono::seconds(
        svc->ctx()->_conf.get_val<uint64_t>("rgw_cache_expiry_interval"));
  }
  ...
  void
  chain_cb(const std::string& key, void* data) override
  {
  ...
  void
  invalidate(const std::string& key) override
  {
    std::unique_lock wl{lock};
    entries.erase(key);
  }
