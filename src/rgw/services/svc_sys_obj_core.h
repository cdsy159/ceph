...
protected:
  ...
  void
  core_init(librados::Rados* rados_, RGWSI_Zone* _zone_svc)
  {
    rados = rados_;
    zone_svc = _zone_svc;
  }
  ...
  virtual int pool_list_prefixed_objs(
      const DoutPrefixProvider* dpp,
      const rgw_pool& pool,
      const std::string& prefix,
      std::function<void(const std::string&)> cb);

  virtual int pool_list_objects_init(
      const DoutPrefixProvider* dpp,
      const rgw_pool& pool,
      const std::string& marker,
      const std::string& prefix,
      RGWSI_SysObj::Pool::ListCtx* ctx);
  virtual int pool_list_objects_next(
      const DoutPrefixProvider* dpp,
      RGWSI_SysObj::Pool::ListCtx& ctx,
      int max,
  ...
  virtual int pool_list_objects_get_marker(
