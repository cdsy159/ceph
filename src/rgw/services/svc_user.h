...
public:
  ...
  static rgw_user
  user_from_meta_key(const std::string& key)
  {
    return rgw_user(key);
  }
  ...
  virtual int create_lister(
      const DoutPrefixProvider* dpp,
      const std::string& marker,
      std::unique_ptr<RGWMetadataLister>& lister) = 0;

  virtual int read_user_info(
      const rgw_user& user,
      RGWUserInfo* info,
  ...
  virtual int remove_user_info(
      const RGWUserInfo& info,
      RGWObjVersionTracker* objv_tracker,
      optional_yield y,
      const DoutPrefixProvider* dpp) = 0;

  virtual int get_user_info_by_email(
      ...
      optional_yield y,
      const DoutPrefixProvider* dpp) = 0;
  virtual int get_user_info_by_swift(
      ...
      optional_yield y,
      const DoutPrefixProvider* dpp) = 0;
  virtual int get_user_info_by_access_key(
