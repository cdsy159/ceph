...
class RGWSI_User_RADOS : public RGWSI_User {
  ...
  using RGWChainedCacheImpl_user_info_cache_entry =
      RGWChainedCacheImpl<user_info_cache_entry>;
  ...
  int remove_uid_index(
      const RGWUserInfo& user_info,
      RGWObjVersionTracker* objv_tracker,
      optional_yield y,
      const DoutPrefixProvider* dpp);

  int remove_key_index(
      const DoutPrefixProvider* dpp,
      const RGWAccessKey& access_key,
      optional_yield y);
  int remove_email_index(
      const DoutPrefixProvider* dpp,
      const std::string& email,
      optional_yield y);
  int remove_swift_name_index(
      const DoutPrefixProvider* dpp,
...
public:
  ...
  int get_user_info_by_swift(
      ...
      optional_yield y,
      const DoutPrefixProvider* dpp) override;
  int get_user_info_by_access_key(
