...
RGWSI_Zone::get_sync_policy_handler(std::optional<rgw_zone_id> zone) const
{
...
bool
RGWSI_Zone::zone_syncs_from(
    const RGWZone& target_zone,
    const RGWZone& source_zone) const
{
  return target_zone.syncs_from(source_zone.name) &&
         sync_modules_svc->get_manager()->supports_data_export(
             source_zone.tier_type);
}

bool
RGWSI_Zone::zone_syncs_from(const RGWZone& source_zone) const
{
...
int
RGWSI_Zone::do_start(optional_yield y, const DoutPrefixProvider* dpp)
{
  ...
  *zone_params = site->get_zone_params();
  ...
  ldpp_dout(dpp, 4) << "Realm:     " << std::left << setw(20)
                    << realm->get_name() << " (" << realm->get_id() << ")"
                    << dendl;
  ldpp_dout(dpp, 4) << "ZoneGroup: " << std::left << setw(20)
                    << zonegroup->get_name() << " (" << zonegroup->get_id()
                    << ")" << dendl;
  ldpp_dout(dpp, 4) << "Zone:      " << std::left << setw(20)
                    << zone_params->get_name() << " (" << zone_params->get_id()
                    << ")" << dendl;

  if (site->get_period().has_value()) {
    ldpp_dout(dpp, 4) << "using period configuration: "
    ...
    ldout(cct, 10) << "cannot find current period zonegroup using local "
    ...
    ret = cfgstore->read_period_config(
        dpp, y, zonegroup->realm_id, period_config);
    if (ret < 0 && ret != -ENOENT) {
  ...
  zone_short_id =
      current_period->get_map().get_zone_short_id(zone_params->get_id());

  for (auto ziter : zonegroup->zones) {
    auto zone_handler = std::make_shared<RGWBucketSyncPolicyHandler>(
        this, sync_modules_svc, bucket_sync_svc, ziter.second.id);
    ret = zone_handler->init(dpp, y);
    if (ret < 0) {
      ldpp_dout(dpp, -1)
          << "ERROR: could not initialize zone policy handler for zone="
          << ziter.second.name << dendl;
      return ret;
    }
    sync_policy_handlers[ziter.second.id] = zone_handler;
  }

  sync_policy_handler = sync_policy_handlers
      [zone_id()]; /* we made sure earlier that zonegroup->zones has our zone */

  set<rgw_zone_id> source_zones;
  set<rgw_zone_id> target_zones;

  sync_policy_handler->reflect(
  ...
  if (!sync_modules->get_module(zone_public_config->tier_type, &sm)) {
    ldpp_dout(dpp, -1) << "ERROR: tier type not found: "
                       << zone_public_config->tier_type << dendl;
    return -EINVAL;
  }
  ...
  for (const auto& ziter : zonegroup->zones) {
    ...
    if (z.endpoints.empty()) {
      ldpp_dout(dpp, 0) << "WARNING: can't generate connection for zone "
                        << z.id << " id " << z.name << ": no endpoints defined"
                        << dendl;
      continue;
    }
    ldpp_dout(dpp, 20) << "generating connection object for zone " << z.name
                       << " id " << z.id << dendl;
    RGWRESTConn* conn = new RGWRESTConn(
        cct, z.id, z.endpoints, zone_params->system_key, zonegroup->get_id(),
        zonegroup->api_name);
    ...
    if (zone_is_source || zone_is_target) {
      ...
      ldpp_dout(dpp, 20) << "NOTICE: not syncing to/from zone " << z.name
                         << " id " << z.id << dendl;
    }
  }

  ldpp_dout(dpp, 20) << "started zone id=" << zone_params->get_id()
                     << " (name=" << zone_params->get_name()
                     << ") with tier type = " << zone_public_config->tier_type
                     << dendl;
...
int
RGWSI_Zone::list_regions(const DoutPrefixProvider* dpp, list<string>& regions)
{
    ...
    list<string>& zonegroups)
{
  ...
  return syspool.list_prefixed_objs(
      dpp, zonegroup_names_oid_prefix, &zonegroups);
}

int
RGWSI_Zone::list_zones(const DoutPrefixProvider* dpp, list<string>& zones)
{
...
int
RGWSI_Zone::list_realms(const DoutPrefixProvider* dpp, list<string>& realms)
{
  RGWRealm realm;
...
int
RGWSI_Zone::list_periods(const DoutPrefixProvider* dpp, list<string>& periods)
{
  RGWPeriod period;
  list<string> raw_periods;
  RGWSI_SysObj::Pool syspool = sysobj_svc->get_pool(period.get_pool(cct));
  int ret = syspool.list_prefixed_objs(
      dpp, period.get_info_oid_prefix(), &raw_periods);
  if (ret < 0) {
    return ret;
  }
    ...
    RGWRESTConn* new_connection)
{
  ...
  map<string, RGWRESTConn*>::iterator iterZoneGroup =
      zonegroup_conn_map.find(zonegroup.get_id());
...
int
RGWSI_Zone::init_zg_from_period(const DoutPrefixProvider* dpp, optional_yield y)
{
  ...
  if (iter != current_period->get_map().zonegroups.end()) {
    ldpp_dout(dpp, 20) << "using current period zonegroup "
  ...
  for (iter = current_period->get_map().zonegroups.begin();
    ...
    if (master == zg.zones.end()) {
      ...
      if (zg.master_zone.empty() && zg.zones.size() == 1) {
        ...
        ldpp_dout(dpp, 0) << "zonegroup " << zg.get_name()
                          << " missing master_zone, setting zone "
                          << master->second.name << " id:" << master->second.id
                          << " as master" << dendl;
        if (zonegroup->get_id() == zg.get_id()) {
          zonegroup->master_zone = master->second.id;
          int ret =
              cfgstore->create_zonegroup(dpp, y, false, *zonegroup, nullptr);
          if (ret < 0) {
            ldpp_dout(dpp, 0)
                << "error updating zonegroup : " << cpp_strerror(-ret) << dendl;
            return ret;
          }
        } else {
          ...
          int ret = rgw::read_zonegroup(
              dpp, y, cfgstore, zonegroup_id, zonegroup_name, *zonegroup);
          if (ret < 0) {
            ldpp_dout(dpp, 0)
                << "error initializing zonegroup : " << cpp_strerror(-ret)
                << dendl;
            return ret;
          }
        ...
        ldpp_dout(dpp, 0) << "zonegroup " << zg.get_name()
                          << " missing zone for master_zone=" << zg.master_zone
                          << dendl;
        return -EINVAL;
      }
    }
    ...
    add_new_connection_to_map(
        zonegroup_conn_map, zg,
        new RGWRESTConn(
            cct, zg.get_id(), endpoints, zone_params->system_key,
            zonegroup->get_id(), zg.api_name));
...
int
RGWSI_Zone::init_zg_from_local(const DoutPrefixProvider* dpp, optional_yield y)
{
  ...
  if (zonegroup->is_master_zonegroup()) {
    ...
    const auto& endpoints = master->second.endpoints;
    rest_master_conn = new RGWRESTConn(
        cct, zonegroup->get_id(), endpoints, zone_params->system_key,
        zonegroup->get_id(), zonegroup->api_name);
  }
...
int
RGWSI_Zone::get_zonegroup(const string& id, RGWZoneGroup& zg) const
{
  int ret = 0;
...
const string&
RGWSI_Zone::get_current_period_id() const
{
  return current_period->get_id();
}

bool
RGWSI_Zone::zone_is_writeable()
{
  return writeable_zone && !get_zone().is_read_only();
}

uint32_t
RGWSI_Zone::get_zone_short_id() const
{
  return zone_short_id;
}

const string&
RGWSI_Zone::zone_name() const
{
  return get_zone_params().get_name();
}

RGWZone*
RGWSI_Zone::find_zone(const rgw_zone_id& id)
{
...
RGWRESTConn*
RGWSI_Zone::get_zone_conn(const rgw_zone_id& zone_id)
{
...
RGWRESTConn*
RGWSI_Zone::get_zone_conn_by_name(const string& name)
{
...
bool
RGWSI_Zone::find_zone_id_by_name(const string& name, rgw_zone_id* id)
{
  auto i = zone_id_by_name.find(name);
  if (i == zone_id_by_name.end()) {
    return false;
...
bool
RGWSI_Zone::need_to_sync() const
{
  return !(
      zonegroup->master_zone.empty() || !rest_master_conn ||
      current_period->get_id().empty());
}

bool
RGWSI_Zone::is_meta_master() const
{
  if (!zonegroup->is_master_zonegroup()) {
    return false;
  }
...
bool
RGWSI_Zone::need_to_log_metadata() const
{
...
/**
  ...
  */
bool
RGWSI_Zone::is_syncing_bucket_meta() const
{
  /* no current period  */
  if (current_period->get_id().empty()) {
    return false;
  ...
  return zonegroup->zones.size() > 1 ||
         current_period->is_multi_zonegroups_with_zones();
}

int
RGWSI_Zone::select_new_bucket_location(
    const DoutPrefixProvider* dpp,
    ...
    optional_yield y)
{
  ...
  if (ret < 0) {
    ldpp_dout(dpp, 0) << "could not find zonegroup " << zonegroup_id
  ...
  if (!request_rule.name.empty()) {
    ...
    if (titer == zonegroup.placement_targets.end()) {
      ldpp_dout(dpp, 0) << "could not find requested placement id "
                        << request_rule << " within zonegroup " << dendl;
    ...
    if (titer == zonegroup.placement_targets.end()) {
      ldpp_dout(dpp, 0) << "could not find user default placement id "
                        << user_info.default_placement << " within zonegroup "
                        << dendl;
      return -ERR_INVALID_LOCATION_CONSTRAINT;
    }
  } else {
    if (zonegroup.default_placement.name
            .empty()) { // zonegroup default rule as fallback, it should not be empty.
      ldpp_dout(dpp, 0) << "misconfiguration, zonegroup default placement id "
                           "should not be empty."
                        << dendl;
      ...
      if (titer == zonegroup.placement_targets.end()) {
        ldpp_dout(dpp, 0) << "could not find zonegroup default placement id "
                          << zonegroup.default_placement << " within zonegroup "
                          << dendl;
        return -ERR_INVALID_LOCATION_CONSTRAINT;
      }
    }
...
int
RGWSI_Zone::select_bucket_location_by_rule(
    const DoutPrefixProvider* dpp,
    ...
    optional_yield y)
{
  ...
  if (!piter->second.storage_class_exists(storage_class)) {
    ldpp_dout(dpp, 5) << "requested storage class does not exist: "
                      << storage_class << dendl;
    return -EINVAL;
  }
    ...
    optional_yield y)
{
  ...
  return select_new_bucket_location(
      dpp, user_info, zonegroup_id, placement_rule, pselected_rule, rule_info,
      y);
}

bool
RGWSI_Zone::get_redirect_zone_endpoint(string* endpoint)
{
  ...
  if (iter == zone_conn_map.end()) {
    ldout(cct, 0) << "ERROR: cannot find entry for redirect zone: "
                  << zone_public_config->redirect_zone << dendl;
    return false;
  }
  ...
  if (ret < 0) {
    ldout(cct, 0) << "ERROR: redirect zone, conn->get_endpoint() returned ret="
                  << ret << dendl;
    return false;
  }
