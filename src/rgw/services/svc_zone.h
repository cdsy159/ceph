...
class RGWSI_Zone : public RGWServiceInstance {
  ...
  std::map<rgw_zone_id, std::shared_ptr<RGWBucketSyncPolicyHandler>>
...
public:
  ...
  bool
  sync_module_supports_writes() const
  {
    return writeable_zone;
  }

  bool
  sync_module_exports_data() const
  {
    return exports_data;
  }
  ...
  std::map<std::string, RGWRESTConn*>&
  get_zonegroup_conn_map()
  {
    return zonegroup_conn_map;
  }

  std::map<rgw_zone_id, RGWRESTConn*>&
  get_zone_conn_map()
  {
    return zone_conn_map;
  }

  std::vector<const RGWZone*>&
  ...
  int select_bucket_placement(
      ...
      optional_yield y);
  int select_new_bucket_location(
      const DoutPrefixProvider* dpp,
      const RGWUserInfo& user_info,
      const std::string& zonegroup_id,
      const rgw_placement_rule& rule,
      ...
      RGWZonePlacementInfo* rule_info,
      optional_yield y);
  int select_bucket_location_by_rule(
      const DoutPrefixProvider* dpp,
  ...
  int list_zonegroups(
      const DoutPrefixProvider* dpp,
      std::list<std::string>& zonegroups);
  int list_regions(
      const DoutPrefixProvider* dpp,
