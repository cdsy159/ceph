...
public:
  ...
  public:
    ...
    void
    rmkey(const std::string& prefix, const std::string& key) override
    {
    ...
    void
    rmkeys_by_prefix(const std::string& prefix) override
    {
    ...
    complete()
    {
      for (auto i = on_commit.begin(); i != on_commit.end();
           on_commit.erase(i++)) {
        (*i)->complete(0);
      }
      return 0;
  ...
  int
  submit_transaction(Transaction trans) override
  {
    return static_cast<TransactionImpl_*>(trans.get())->complete();
  }

  uint64_t
  get_estimated_size(std::map<std::string, uint64_t>& extras) override
  {
...
private:
  bool
  exists_prefix(const std::string& prefix)
  {
