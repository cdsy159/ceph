...
public:
  ...
  ClientScaffold(
      ...
      Objecter* objecter_,
      fscrypt_env* fse) :
    Client(m, mc, objecter_), fse(fse)
  {}

  virtual ~ClientScaffold() {}

  int
  check_dummy_op(const UserPerm& perms)
  {
  ...
  int
  send_unknown_session_op(int op)
  {
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) {
      return -ENOTCONN;
  ...
  bool
  check_unknown_reclaim_flag(uint32_t flag)
  {
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) {
      return -ENOTCONN;
  ...
  {
    return do_symlinkat(
        target, dirfd, linkpath, perms, std::move(alternate_name));
  }
  ...
  {
    return do_mkdirat(
        CEPHFS_AT_FDCWD, path, mode, perm, std::move(alternate_name));
  }
      ...
      std::string alternate_name)
  {
    return do_openat(
        ...
        data_pool, std::move(alternate_name));
  }
  ...
  encrypt(const UserPerm& myperm)
  {
    ...
    pid_t mypid = getpid();
    fse->name = std::string("ceph_test_client_fscrypt.") + stringify(mypid) +
                "." + stringify(rand());
    ...
    if (r < 0) {
      std::clog << __func__ << "(): mkdir(" << fse->name << ") r=" << r
                << std::endl;
    ...
    int key_fd =
        open(key_fname.c_str(), O_RDWR | O_CREAT | O_TRUNC, myperm, 0600);
    if (key_fd < 0) {
      ...
      off_t* offset)
  {
    ...
    int64_t rc = 0, bytes_written = 0, num_iov = iov_cnt,
            bytes_expected = block_size;
    ...
    while (to_write > 0) {
      ...
      rc = ll_preadv_pwritev(
          fh, input_buffer_ptr, num_iov, *offset, true, &writefinish, nullptr);
      ASSERT_EQ(rc, 0);
  ...
  bool
  is_data_pool_full(int64_t data_pool)
  {
  ...
  bool
  wait_for_osdmap_epoch_update(
      const epoch_t initial_osd_epoch,
      int32_t timeout = 10,
      int32_t period = 5)
  {
    ...
    while (timeout) {
      if (objecter->with_osdmap(std::mem_fn(&OSDMap::get_epoch)) >
          initial_osd_epoch) {
        return true;
      } else {
        sleep(period);
...
public:
  ...
  SetUpTestSuite()
  {
    icp.start(g_ceph_context->_conf.get_val<std::uint64_t>(
