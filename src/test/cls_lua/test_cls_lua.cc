...
protected:
  ...
  SetUp() override
  {
    ...
    ss_oid << test_info->test_case_name() << "_" << test_info->name() << "_"
           << getpid();
  ...
  __clslua_exec(
      const string& oid,
      const string& script,
      librados::bufferlist* input = NULL,
      const string& funcname = "")
  {
    ...
    return cls_lua_client::exec(
        ioctx, oid, script, funcname, inbl, reply_output);
  }
...
TEST_F(ClsLua, SyntaxError) { ASSERT_EQ(-EIO, clslua_exec("-")); }

TEST_F(ClsLua, EmptyScript) { ASSERT_EQ(0, clslua_exec("")); }
...
TEST_F(ClsLua, Log)
{
  ...
  ASSERT_EQ(
      0,
      clslua_exec("s = objclass.log('one', 'two', 'three'); objclass.log(s);"));
}
...
TEST_F(ClsLua, MapSetVals)
{
  ...
  ASSERT_STREQ(
      "a_val",
      std::string(out_vals["a"].c_str(), out_vals["a"].length()).c_str());
  ...
  ASSERT_STREQ(
      "b_val",
      std::string(out_vals["b"].c_str(), out_vals["b"].length()).c_str());
  ...
  ASSERT_STREQ(
      "c_val",
      std::string(out_vals["c"].c_str(), out_vals["c"].length()).c_str());
  ...
  ASSERT_STREQ(
      "1_val",
      std::string(out_vals["1"].c_str(), out_vals["1"].length()).c_str());
  ASSERT_STREQ(
      "2_val",
      std::string(out_vals["2"].c_str(), out_vals["2"].length()).c_str());
