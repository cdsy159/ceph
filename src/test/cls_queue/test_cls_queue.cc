...
TEST_F(TestClsQueue, List)
{
  ...
  do {
    ...
    const auto ret = cls_queue_list_entries(
...
TEST_F(TestClsQueue, ListByEndMarker)
{
  ...
  do {
    ...
    ret = cls_queue_list_entries(
        ioctx, queue_name, marker, end_marker, end_marker_entries,
...
TEST_F(TestClsQueue, Dequeue)
{
  ...
  std::vector<cls_queue_entry> entries;
  const auto ret = cls_queue_list_entries(
      ioctx, queue_name, marker, remove_elements, entries, &truncated,
      end_marker);
  ASSERT_EQ(0, ret);
...
TEST_F(TestClsQueue, ListEmpty)
{
  ...
  std::vector<cls_queue_entry> entries;
  const auto ret = cls_queue_list_entries(
      ioctx, queue_name, marker, max_elements, entries, &truncated, next_marker);
  ASSERT_EQ(0, ret);
...
TEST_F(TestClsQueue, ListAll)
{
  ...
  std::vector<cls_queue_entry> entries;
  const auto ret = cls_queue_list_entries(
      ioctx, queue_name, marker, total_elements, entries, &truncated,
      next_marker);
  ASSERT_EQ(0, ret);
...
TEST_F(TestClsQueue, DeleteAll)
{
  ...
  ret = cls_queue_list_entries(
      ioctx, queue_name, marker, 10, entries, &truncated, end_marker);
  ASSERT_EQ(0, ret);
  ASSERT_EQ(truncated, false);
  ASSERT_EQ(entries.size(), 0);
}
...
TEST_F(TestClsQueue, MultiConsumer)
{
  ...
  for (auto& c : consumers) {
    c = std::thread([this, &queue_name, &consume_count, &done,
...
TEST_F(TestClsQueue, WrapAround)
{
  ...
  for (auto i = 0U; i < number_of_entries; ++i) {
    const auto entry_size = rand() % (max_entry_size - min_entry_size + 1) +
                            min_entry_size;
    std::string entry_str(entry_size - entry_overhead, 0);
    std::generate_n(entry_str.begin(), entry_str.size(), []() {
      return (char)(rand());
    });
    bufferlist entry_bl;
