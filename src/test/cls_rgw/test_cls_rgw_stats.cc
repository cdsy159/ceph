...
std::ostream&
operator<<(std::ostream& out, const rgw_bucket_category_stats& c)
{
...
last_version(librados::IoCtx& ioctx)
{
...
int
index_init(librados::IoCtx& ioctx, const std::string& oid)
{
    ...
    std::list<cls_rgw_obj_key>* remove_objs)
{
  ...
  cls_rgw_bucket_complete_op(
      op, type, tag, ver, key, meta, remove_objs, log_op, flags, zones);
  return ioctx.operate(oid, &op);
}
...
static void
account_entry(rgw_bucket_dir_stats& stats, const rgw_bucket_dir_entry_meta& meta)
{
...
using object_map_base =
    boost::intrusive::set<object, boost::intrusive::key_of_value<object_key>>;
...
object_map::iterator
simulator::find_or_create(const cls_rgw_obj_key& key)
{
...
int
simulator::try_start(const cls_rgw_obj_key& key, const std::string& tag)
{
  // choose randomly between create and delete
  const auto type = static_cast<RGWModifyOp>(
      ceph::util::generate_random_number<size_t, size_t>(
          CLS_RGW_OP_ADD, CLS_RGW_OP_DEL));
  ...
  if (type == CLS_RGW_OP_ADD && op.meta.size > max_part_size) {
    ...
    if (r != 0) {
      derr << "> failed to prepare multipart upload key=" << key
           << " upload=" << op.upload_id << " tag=" << tag << " type=" << type
    ...
    if (r != 0) {
      derr << "> failed to prepare operation key=" << key << " tag=" << tag
           << " type=" << type << ": " << cpp_strerror(r) << dendl;
      return r;
    }

    dout(1) << "> prepared operation key=" << key << " tag=" << tag
...
void
simulator::finish(const operation& op)
{
...
void
simulator::complete(const operation& op, RGWModifyOp type)
{
  int r = index_complete(
      ioctx, oid, op.key, op.tag, type, op.ver, op.meta, nullptr);
  if (r != 0) {
    derr << "< failed to complete operation key=" << op.key << " tag=" << op.tag
         << " type=" << op.type << " size=" << op.meta.size << ": "
         << cpp_strerror(r) << dendl;
    return;
  }

  if (type == CLS_RGW_OP_CANCEL) {
    dout(1) << "< canceled operation key=" << op.key << " tag=" << op.tag
    ...
    dout(1) << "< completed write operation key=" << op.key << " tag=" << op.tag
            << " type=" << type << " size=" << op.meta.size << dendl;
  } else {
...
void
simulator::suggest(const operation& op, char suggestion)
{
  ...
  if (r != 0) {
    derr << "< no bi entry to suggest for operation key=" << op.key
         << " tag=" << op.tag << " type=" << op.type << " size=" << op.meta.size
         << ": " << cpp_strerror(r) << dendl;
    return;
  }
  ...
  if (r != 0) {
    derr << "< failed to suggest operation key=" << op.key << " tag=" << op.tag
         << " type=" << op.type << " size=" << op.meta.size << ": "
         << cpp_strerror(r) << dendl;
    return;
  }
...
int
simulator::init_multipart(const operation& op)
{
  ...
  int r = index_complete(
      ioctx, oid, meta_key, empty_tag, CLS_RGW_OP_ADD, empty_ver, meta_meta,
      nullptr);
  if (r != 0) {
  ...
  while (remaining > max_part_size) {
    ...
    const auto part_key = cls_rgw_obj_key{fmt::format(
        "_multipart_{}.2~{}.{}", op.key.name, op.upload_id, part_id)};
    part_id++;

    r = index_prepare(ioctx, oid, part_key, op.tag, op.type);
    if (r != 0) {
      // if part prepare fails, remove the meta object and remove_objs
      [[maybe_unused]] int ignored = index_complete(
          ioctx, oid, meta_key, empty_tag, CLS_RGW_OP_DEL, empty_ver, meta_meta,
          &remove_objs);
      derr << "  > failed to prepare part key=" << part_key
           << " size=" << part_size << dendl;
      return r; // return the error from prepare
    }
    dout(1) << "  > prepared part key=" << part_key << " size=" << part_size
            << dendl;
...
void
simulator::complete_multipart(const operation& op)
{
  const std::string empty_tag; // empty tag enables complete without prepare
  const rgw_bucket_entry_ver empty_ver;
  ...
  while (remaining > max_part_size) {
    ...
    if (result < cancel_percent) {
      ...
      dout(1) << "  < canceled part key=" << part_key << " size=" << part_size
              << dendl;
    } else {
      rgw_bucket_dir_entry_meta meta;
      ...
      int r = index_complete(
          ioctx, oid, part_key, op.tag, op.type, empty_ver, meta, nullptr);
      if (r != 0) {
  ...
  r = index_complete(
      ioctx, oid, op.key, empty_tag, type, empty_ver, op.meta, &remove_objs);
  if (r != 0) {
    derr << "< failed to complete multipart upload key=" << op.key
         << " upload=" << op.upload_id << " tag=" << op.tag << " type=" << type
