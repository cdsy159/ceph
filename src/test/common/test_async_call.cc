...
TEST(AsyncPost, AsyncCall)
{
  ...
  asio::co_spawn(
      ...
      [&]() -> asio::awaitable<void> {
        auto strand = asio::make_strand(c.get_executor());
        auto x =
            co_await async_post(strand, [] { return 55; }, asio::use_awaitable);
        EXPECT_EQ(x, 55);
        co_return;
      },
      [](std::exception_ptr e) {
...
TEST(AsyncDispatchVoid, AsyncCall)
{
  ...
  asio::co_spawn(
      ...
      [&]() -> asio::awaitable<void> {
        auto strand = asio::make_strand(c.get_executor());
        co_await async_dispatch(strand, [] { return; }, asio::use_awaitable);
        co_return;
      },
      [](std::exception_ptr e) {
        if (e)
...
TEST(AsyncPostVoid, AsyncCall)
{
  ...
  asio::co_spawn(
      ...
      [&]() -> asio::awaitable<void> {
        auto strand = asio::make_strand(c.get_executor());
        co_await async_post(strand, [] { return; }, asio::use_awaitable);
        co_return;
      },
      [](std::exception_ptr e) {
        if (e)
...
TEST(AsyncDeferVoid, AsyncCall)
{
  ...
  asio::co_spawn(
      ...
      [&]() -> asio::awaitable<void> {
        auto strand = asio::make_strand(c.get_executor());
        co_await async_defer(strand, [] { return; }, asio::use_awaitable);
        co_return;
      },
      [](std::exception_ptr e) {
        if (e)
  ...
  asio::co_spawn(
      ...
      [&]() -> asio::awaitable<void> {
        ...
        auto op = async_dispatch(
            ...
            asio::deferred);
        EXPECT_FALSE(ran);
        std::move(op)([](int x) { EXPECT_EQ(x, 55); });
        EXPECT_TRUE(ran);
        co_return;
      },
      [](std::exception_ptr e) {
