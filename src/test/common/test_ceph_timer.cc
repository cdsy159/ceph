...
TEST(RunSome, Steady) { run_some<std::chrono::steady_clock>(); }

TEST(RunSome, Wall) { run_some<std::chrono::system_clock>(); }

TEST(RunOrderly, Steady) { run_orderly<std::chrono::steady_clock>(); }

TEST(RunOrderly, Wall) { run_orderly<std::chrono::system_clock>(); }

TEST(CancelAll, Steady) { cancel_all<std::chrono::steady_clock>(); }

TEST(CancelAll, Wall) { cancel_all<std::chrono::system_clock>(); }
...
TEST(TimerLoopTest, TimerLoop)
{
  ...
  t.add_event(
      ceph::make_timespan(tick_interval), &tick<TC>, &t, test_deadline,
      tick_interval, &test_finished, &last_tick, &tick_count, &last_tp,
      &second_last_tp);
  ...
  ASSERT_TRUE(test_finished)
      ...
      << "Time since last tick: " << (TC::now() - last_tick)
      << ". Tick count: " << tick_count << ". Last wait tp: " << last_tp
