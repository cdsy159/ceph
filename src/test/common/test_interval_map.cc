...
template <
    typename _key,
    typename _can_merge,
    template <typename, typename, typename...> class _map = std::map,
    bool _nonconst_iterator = false>
struct bufferlist_test_type {
  ...
  struct make_splitter {
    ...
    struct apply {
      bufferlist
      split(key offset, key len, bufferlist& bu) const
      {
        bufferlist bl;
      ...
      bool
      can_merge(const bufferlist& left, const bufferlist& right) const
      {
        return merge_t::value;
      }

      bufferlist
      merge(bufferlist&& left, bufferlist&& right) const
      {
        bufferlist bl;
      ...
      uint64_t
      length(const bufferlist& r) const
      {
        return r.length();
      }
    };
