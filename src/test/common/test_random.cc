...
template <typename T>
T
type_check_ok(const T min, const T max)
{
...
void
swallow_values(const X x, const XS... xs)
{
...
TEST(util, test_random)
{
  ...
  {
    auto a =
        ceph::util::generate_random_number(1, std::numeric_limits<int>::max());
    auto b =
        ceph::util::generate_random_number(1, std::numeric_limits<int>::max());
