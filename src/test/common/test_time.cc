...
TEST(RealClock, Sanity) { system_clock_sanity<real_clock>(); }

TEST(RealClock, Conversions) { system_clock_conversions<real_clock>(); }

TEST(CoarseRealClock, Sanity) { system_clock_sanity<coarse_real_clock>(); }
...
TEST(TimePoints, stringify)
{
  ...
  ceph::coarse_real_clock::time_point ctp(
      seconds(1556122013) + nanoseconds(399000000));
  s = stringify(ctp);
...
std::string
to_string(const chrono::duration<Rep, Period>& t)
{
...
void
float_format_eq(string_view lhs, string_view rhs, unsigned precision)
{
...
TEST(TimeDurations, print)
{
  float_format_eq(
      "0.123456700s",
      to_string(std::chrono::duration_cast<ceph::timespan>(0.1234567s)), 9);
  float_format_eq(
      "-0.123456700s",
      to_string(std::chrono::duration_cast<ceph::signedspan>(-0.1234567s)), 9);
