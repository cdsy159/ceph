...
struct onode_item_t {
  ...
  void
  initialize(Transaction& t, Onode& value) const
  {
    auto& ftvalue = static_cast<FLTreeOnode&>(value);
    ftvalue.update_onode_size(t, size);
    auto oroot = omap_root_t(
        laddr_t::from_raw_uint(id), cnt_modify,
  ...
  void
  validate(Onode& value) const
  {
    auto& layout = value.get_layout();
    ceph_assert(uint64_t(layout.size) == uint64_t{size});
    ceph_assert(
        layout.omap_root.get(value.get_metadata_hint(block_size)).addr ==
        laddr_t::from_raw_uint(id));
    ceph_assert(
        layout.omap_root.get(value.get_metadata_hint(block_size)).depth ==
        cnt_modify);
  }

  void
  modify(Transaction& t, Onode& value)
  {
    validate(value);
    ++cnt_modify;
    initialize(t, value);
  ...
  static onode_item_t
  create(std::size_t size, std::size_t id, uint64_t block_size)
  {
...
struct fltree_onode_manager_test_t : public seastar_test_suite_t, TMTestState {
  ...
  virtual FuturizedStore::mkfs_ertr::future<>
  _mkfs() final
  {
        ...
        .safe_then([this] {
          return repeat_eagain([this] {
            return seastar::do_with(
                create_mutate_transaction(), [this](auto& ref_t) {
                  return with_trans_intr(*ref_t, [&](auto& t) {
                    return manager->mkfs(t).si_then([this, &t] {
  ...
  template <typename F>
  void
  with_onode_write(iterator_t& it, F&& f)
  {
  ...
  template <typename F>
  void
  with_onodes_process(const iterator_t& start, const iterator_t& end, F&& f)
  {
  ...
  template <typename F>
  void
  with_onodes_write(const iterator_t& start, const iterator_t& end, F&& f)
  {
    with_onodes_process(
        start, end, [this, f = std::move(f)](auto& t, auto& oids, auto& items) {
  ...
  void
  validate_onodes(const iterator_t& start, const iterator_t& end)
  {
  ...
  void
  validate_erased(const iterator_t& start, const iterator_t& end)
  {
    with_onodes_process(start, end, [this](auto& t, auto& oids, auto& items) {
      for (auto& oid : oids) {
        auto exist = with_trans_intr(t, [&](auto& t) {
  ...
  void
  validate_list_onodes(KVPool<onode_item_t>& pool)
  {
    with_onodes_process(
        pool.begin(), pool.end(), [this](auto& t, auto& oids, auto& items) {
...
TEST_P(fltree_onode_manager_test_t, 1_single)
{
  run_async([this] {
    ...
    auto pool =
        KVPool<onode_item_t>::create_range({0, 1}, {128, 256}, block_size);
...
TEST_P(fltree_onode_manager_test_t, 2_synthetic)
{
  run_async([this] {
    ...
    auto end = pool.end();
    with_onodes_write(start, end, [](auto& t, auto& onode, auto& item) {
      item.initialize(t, onode);
    });
    restart();
    ...
    with_onodes_write(rd_start, rd_end, [](auto& t, auto& onode, auto& item) {
      item.modify(t, onode);
    });
    restart();
    validate_onodes(start, end);
