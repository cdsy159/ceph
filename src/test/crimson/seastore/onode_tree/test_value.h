...
struct test_item_t {
  ...
  static test_item_t
  create(std::size_t _size, std::size_t _id)
  {
...
inline std::ostream&
operator<<(std::ostream& os, const test_item_t& item)
{
...
inline std::ostream&
operator<<(std::ostream& os, const delta_op_t op)
{
  switch (op) {
...
template <>
struct fmt::formatter<crimson::os::seastore::onode::delta_op_t>
...
private:
  ...
  struct Replayable {
    static void
    set_id(NodeExtentMutable& payload_mut, id_t id)
    {
    ...
    static void
    set_tail_magic(NodeExtentMutable& payload_mut, magic_t magic)
    {
  ...
  private:
    static payload_t*
    get_write(NodeExtentMutable& payload_mut)
    {
...
public:
  ...
  public:
    ...
    void
    encode_set_id(NodeExtentMutable& payload_mut, id_t id)
    {
    ...
    void
    encode_set_tail_magic(NodeExtentMutable& payload_mut, magic_t magic)
    {
      auto& encoded = get_encoded(payload_mut);
  ...
  protected:
        ...
        laddr_offset_t value_addr_offset) override
    {
      ...
      try {
        ...
        case delta_op_t::UPDATE_TAIL_MAGIC: {
          logger().debug(
              "OTree::TestValue::Replay: decoding UPDATE_TAIL_MAGIC ...");
          magic_t magic;
        ...
        default:
          logger().error(
              "OTree::TestValue::Replay: got unknown op {} when replay "
              "{}~{:#x}",
        ...
        logger().error(
            "OTree::TestValue::Replay: got decode error {} when replay "
            "{}~{:#x}",
  ...
  void
  set_id_replayable(Transaction& t, id_t id)
  {
  ...
  void
  set_tail_magic_replayable(Transaction& t, magic_t magic)
  {
    auto value_mutable = prepare_mutate_payload<payload_t, Recorder>(t);
    if (value_mutable.second) {
  ...
  void
  initialize(Transaction& t, const item_t& item)
  {
  ...
  void
  validate(const item_t& item) const
  {
    ceph_assert(get_payload_size() + sizeof(value_header_t) == item.size);
...
using UnboundedValue =
    TestValue<value_magic_t::TEST_UNBOUND, 4096, 4096, 4096, 4096, 4096, false>;
using BoundedValue =
    TestValue<value_magic_t::TEST_BOUNDED, 320, 320, 640, 4096, 4096, true>;
// should be the same configuration with FLTreeOnode
using ExtendedValue =
    TestValue<value_magic_t::TEST_EXTENDED, 256, 2048, 1200, 8192, 16384, true>;

} // namespace crimson::os::seastore::onode

#if FMT_VERSION >= 90000
template <>
struct fmt::formatter<crimson::os::seastore::onode::test_item_t>
  : fmt::ostream_formatter {};
#endif
