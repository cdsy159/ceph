...
struct test_extent_desc_t {
  ...
  {
    return (len == rhs.len && checksum == rhs.checksum);
  }
...
inline std::ostream&
operator<<(std::ostream& lhs, const test_extent_desc_t& rhs)
{
...
struct TestBlock : crimson::os::seastore::LogicalChildNode {
  ...
  void
  set_contents(char c, extent_len_t offset, extent_len_t len)
  {
...
struct TestBlockPhysical : crimson::os::seastore::CachedExtent {
  ...
  void
  on_rewrite(Transaction&, CachedExtent&, extent_len_t) final
  {}
...
struct test_block_mutator_t {
  ...
  std::uniform_int_distribution<extent_len_t> offset_distribution =
      std::uniform_int_distribution<extent_len_t>(0, TestBlock::SIZE - 1);

  std::uniform_int_distribution<extent_len_t>
  length_distribution(extent_len_t offset)
  {
  ...
  void
  mutate(TestBlock& block, generator_t& gen)
  {
    auto offset = offset_distribution(gen);
    block.set_contents(
        contents_distribution(gen), offset, length_distribution(offset)(gen));
  }
};
...
template <>
struct fmt::formatter<crimson::os::seastore::test_extent_desc_t>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::os::seastore::TestBlock>
  : fmt::ostream_formatter {};

template <>
struct fmt::formatter<crimson::os::seastore::TestBlockPhysical>
  : fmt::ostream_formatter {};
#endif
