...
struct btree_test_base : public seastar_test_suite_t,
                         SegmentProvider,
                         JournalTrimmer {
  ...
  void
  set_journal_head(journal_seq_t) final
  {}
  ...
  void
  set_journal_head_sequence(segment_seq_t) final
  {}
  ...
  void
  update_journal_tails(journal_seq_t, journal_seq_t) final
  {}

  bool
  try_reserve_inline_usage(std::size_t) final
  {
    return true;
  }

  void
  release_inline_usage(std::size_t) final
  {}
      ...
      rewrite_gen_t) final
  {
    auto ret = next;
    next = segment_id_t{
        segment_manager->get_device_id(), next.device_segment_id() + 1};
  ...
  void
  close_segment(segment_id_t) final
  {}

  void
  update_segment_avail_bytes(segment_type_t, paddr_t) final
  {}

  void
  update_modify_time(segment_id_t, sea_time_point, std::size_t) final
  {}
  ...
  submit_transaction(TransactionRef t)
  {
    ...
    return seastar::do_with(
        std::move(t), [this, record = std::move(record)](auto& _t) mutable {
          ...
          return journal
              ->submit_record(
                  std::move(record), t.get_handle(), t.get_src(),
                  [this, &t](auto submit_result) {
                    cache->complete_commit(
                        t, submit_result.record_block_base,
  ...
  set_up_fut() final
  {
    ...
    return segment_manager->init()
        ...
        .safe_then([this] {
          dummy_tail = journal_seq_t{
              0, paddr_t::make_seg_paddr(
        ...
        .safe_then([this] {
          return seastar::do_with(
              cache->create_transaction(
                  Transaction::src_t::MUTATE, "test_set_up_fut",
                  CACHE_HINT_TOUCH, false),
              [this](auto& ref_t) {
                                 ...
                                 .si_then([&t] {
                                   auto chksum_func = [](auto& extent) {
                                     ...
                                     if (!extent->is_logical() ||
                                       ...
                                       extent->update_in_extent_chksum_field(
                                           crc);
                                     }
                                     assert(
                                         extent->calc_crc32c() ==
                                   ...
                                   auto pre_allocated_extents =
                                       t.get_valid_pre_alloc_list();
                                   std::for_each(
        ...
        .handle_error(crimson::ct_error::assert_all{"error"});
  }
  ...
  tear_down_fut() final
  {
    return cache->close()
        ...
        .handle_error(crimson::ct_error::assert_all{"Unable to close"});
  }
};
...
struct lba_btree_test : btree_test_base {
  ...
  auto
  get_op_context(Transaction& t)
  {
    return op_context_t{*cache, t};
  }

  LBAManager::mkfs_ret
  test_structure_setup(Transaction& t) final
  {
  ...
  template <typename F>
  auto
  lba_btree_update(F&& f)
  {
    auto tref = cache->create_transaction(
        Transaction::src_t::MUTATE, "test_btree_update", CACHE_HINT_TOUCH,
        false);
    auto& t = *tref;
    with_trans_intr(
        t,
          ...
          return cache->get_root(t)
              ...
              .si_then([t = tref.get()]() mutable {
                auto chksum_func = [](auto& extent) {
                  ...
                  assert(
                      extent->calc_crc32c() == extent->get_last_committed_crc());
                };
  ...
  lba_btree_read(F&& f)
  {
    auto t = cache->create_transaction(
        Transaction::src_t::READ, "test_btree_read", CACHE_HINT_TOUCH, false);
    return with_trans_intr(
               *t,
               [this, f = std::forward<F>(f)](auto& t) mutable {
                 return cache->get_root(t).si_then(
                     [f = std::move(f), &t](RootBlockRef croot) mutable {
                       return seastar::do_with(
                           LBABtree(croot),
                           [f = std::move(f), &t](auto& btree) mutable {
                             return std::invoke(std::move(f), btree, t);
  ...
  static auto
  get_map_val(extent_len_t len, extent_types_t type)
  {
  ...
  void
  insert(laddr_t addr, extent_len_t len)
  {
    ...
    lba_btree_update([=, this](auto& btree, auto& t) {
      ...
      return seastar::do_with(
          std::move(extents), [this, addr, &t, len, &btree](auto& extents) {
            return trans_intr::do_for_each(
                extents, [this, addr, len, &t, &btree](auto& extent) {
                  return btree
                      .insert(
    ...
    lba_btree_update([=, this](auto& btree, auto& t) {
      return btree.lower_bound(get_op_context(t), addr)
          .si_then([this, len, addr, &btree, &t](auto iter) {
            ...
            return btree.remove(get_op_context(t), iter).discard_result();
          });
    });
  }

  void
  check_lower_bound(laddr_t addr)
  {
...
struct btree_lba_manager_test : btree_test_base {
  ...
  void
  complete_commit(Transaction& t) final
  {}
  ...
  auto
  create_transaction(bool create_fake_extent = true)
  {
    auto t = test_transaction_t{
        cache->create_transaction(
            Transaction::src_t::MUTATE, "test_mutate_lba", CACHE_HINT_TOUCH,
            false),
        test_lba_mappings};
    if (create_fake_extent) {
      cache->alloc_new_non_data_extent<TestBlockPhysical>(
          *t.t, TestBlockPhysical::SIZE, placement_hint_t::HOT, 0);
    };
    return t;
  }
  ...
  create_weak_transaction()
  {
    auto t = test_transaction_t{
        cache->create_transaction(
            Transaction::src_t::READ, "test_read_weak", CACHE_HINT_TOUCH, true),
        test_lba_mappings};
    return t;
  }

  void
  submit_test_transaction(test_transaction_t t)
  {
    with_trans_intr(*t.t, [this](auto& t) {
      return seastar::do_with(
          ...
          [this, &t](auto& lextents, auto& pextents) {
            ...
            return lba_manager->update_mappings(t, lextents).si_then([&pextents] {
  ...
  auto
  get_overlap(test_transaction_t& t, laddr_t addr, size_t len)
  {
    auto bottom = t.mappings.upper_bound(addr);
    if (bottom != t.mappings.begin())
      --bottom;
    if (bottom != t.mappings.end() && bottom->first + bottom->second.len <= addr)
      ++bottom;
  ...
  auto
  alloc_mappings(test_transaction_t& t, laddr_t hint, size_t len)
  {
    auto rets =
        with_trans_intr(*t.t, [=, this](auto& t) {
          auto extents = cache->alloc_new_data_extents<TestBlock>(
              t, TestBlock::SIZE, placement_hint_t::HOT, 0, get_paddr());
          return seastar::do_with(
              std::vector<LogicalChildNodeRef>(extents.begin(), extents.end()),
              [this, &t, hint](auto& extents) {
                return lba_manager->alloc_extents(
                    t, hint, std::move(extents), EXTENT_DEFAULT_REF_COUNT);
              });
        }).unsafe_get();
    for (auto& ret : rets) {
      ...
      EXPECT_EQ(b, e);
      t.mappings.emplace(std::make_pair(
          ret->get_laddr(),
          test_extent_t{ret->get_paddr(), ret->get_length(), 1}));
    }
    return rets;
  }

  auto
  decref_mapping(test_transaction_t& t, laddr_t addr)
  {
    return decref_mapping(t, t.mappings.find(addr));
  }

  void
  decref_mapping(test_transaction_t& t, test_lba_mapping_t::iterator target)
  {
    ...
    (void)with_trans_intr(
        *t.t,
        seastar::coroutine::lambda(
            [=, this](auto& t) -> LBAManager::ref_iertr::future<> {
              auto cursor = co_await lba_manager->get_cursor(t, target->first);
              auto refcount = cursor->get_refcount() - 1;
              auto paddr = cursor->get_paddr();
              auto length = cursor->get_length();
              co_await lba_manager->update_mapping_refcount(t, cursor, -1);
              EXPECT_EQ(refcount, target->second.refcount);
              if (refcount == 0) {
                co_await cache->retire_extent_addr(t, paddr, length);
              }
            }))
        .unsafe_get();
  ...
  std::vector<laddr_t>
  get_mapped_addresses(test_transaction_t& t)
  {
    std::vector<laddr_t> addresses;
  ...
  check_mappings(test_transaction_t& t)
  {
    ...
    for (auto&& i : t.mappings) {
      ...
      auto ret_list = with_trans_intr(*t.t, [=, this](auto& t) {
                        return lba_manager->get_cursors(t, laddr, len);
                      }).unsafe_get();
      ...
      auto ret_pin = with_trans_intr(*t.t, [=, this](auto& t) {
                       return lba_manager->get_cursor(t, laddr);
                     }).unsafe_get();
...
TEST_F(btree_lba_manager_test, split_merge_multi)
{
  run_async([this] {
    ...
    iterate([&](auto& t, auto idx) {
      if ((idx % 32) > 0) {
        alloc_mappings(
            t, laddr_t::from_byte_offset(idx * block_size), block_size);
      }
    });
    check_mappings();
    iterate([&](auto& t, auto idx) {
