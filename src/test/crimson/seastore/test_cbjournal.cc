...
decode_record(bufferlist& bl)
{
  ...
  logger().debug(
      " decode_record mdlength {} records {}", r_header.mdlength,
      r_header.records);
  device_id_t d_id = 1 << (std::numeric_limits<device_id_t>::digits - 1);

  auto del_infos =
      try_decode_deltas(r_header, bl, paddr_t::make_blk_paddr(d_id, 0));
  ...
  for (auto& iter : *ex_infos) {
    for (auto e : iter.extent_infos) {
      ...
      logger().debug(
          " exten len {} remaining {} ", e.len, bliter_ex.get_remaining());
...
struct entry_validator_t {
  ...
  void
  validate(CircularBoundedJournal& cbj)
  {
    ...
    for (int i = 0; i < entries; i++) {
      ...
      auto md =
          cbj.test_read_validate_record_metadata(cursor, magic).unsafe_get();
      assert(md);
      auto& [header, md_bl] = *md;
      auto dbuf = cbj.read(paddr.add_offset(header.mdlength), header.dlength)
                      .unsafe_get();
...
struct cbjournal_test_t : public seastar_test_suite_t, JournalTrimmer {
  ...
  void
  set_journal_head(journal_seq_t head) final
  {}
  ...
  void
  update_journal_tails(journal_seq_t dirty_tail, journal_seq_t alloc_tail) final
  {}
  ...
  tear_down_fut() final
  {
    return close().then([this] { cbj.reset(); });
  }
  ...
  replay()
  {
    return cbj->replay([this](
                           const auto& offsets, const auto& e, auto& dirty_seq,
                           auto& alloc_seq, auto last_modified) {
      bool found = false;
      for (auto& i : entries) {
        ...
        if (addr == i.get_abs_addr()) {
          logger().debug(
              " compare addr: {} and i.addr {} ", base, i.get_abs_addr());
  ...
  mkfs()
  {
    ...
    return device->mkfs(config)
        .safe_then([this]() {
          return device->mount().safe_then([this]() {
            return cbj->open_for_mkfs().safe_then([](auto q) {
              return seastar::now();
            });
          });
        })
  ...
  open()
  {
    return cbj->open_for_mount().safe_then([](auto q) {
      return seastar::now();
    });
  }
  ...
  set_up_fut() final
  {
    ...
    return mkfs()
        .safe_then([this] {
          return replay().safe_then([this] {
            return open().safe_then([this] { return replay(); });
          });
        })
...
TEST_F(cbjournal_test_t, update_header)
{
  run_async([this] {
    ...
    auto [update_header, update_buf2] =
        *(cbj->get_cjs().read_header().unsafe_get());
...
TEST_F(cbjournal_test_t, replay_after_reset)
{
  run_async([this] {
    ...
    set_written_to(journal_seq_t{
        0, convert_abs_addr_to_paddr(
               cbj->get_records_start(), cbj->get_device_id())});
    cbj->close().unsafe_get();
    replay().unsafe_get();
...
TEST_F(cbjournal_test_t, multiple_submit_at_end)
{
  run_async([this] {
    ...
    seastar::parallel_for_each(
        boost::make_counting_iterator(0u), boost::make_counting_iterator(4u),
        [&](auto) {
    ...
    auto old_written_to = get_written_to();
    cbj->close().unsafe_get();
    cbj->replay([](const auto& offsets, const auto& e, auto& dirty_seq,
                   auto& alloc_seq, auto last_modified) {
         return Journal::replay_ertr::make_ready_future<
             std::pair<bool, CachedExtentRef>>(true, nullptr);
       })
