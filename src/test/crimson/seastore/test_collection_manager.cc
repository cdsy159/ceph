    ...
    return with_trans_intr(                                            \
               ...
               [this](auto& t, auto& root, auto&&... args) {           \
                 return collection_manager->METHOD(                    \
                     root, t, std::forward<decltype(args)>(args)...);  \
               },                                                      \
...
struct collection_manager_test_t : public seastar_test_suite_t, TMTestState {
  ...
  get_root()
  {
    ...
    auto coll_root = with_trans_intr(*tref, [this](auto& t) {
  ...
  void
  checking_mappings(coll_root_t& coll_root, Transaction& t)
  {
    ...
    for (std::pair<coll_t, coll_info_t> p : test_coll_mappings) {
      EXPECT_NE(
          std::find(coll_list.begin(), coll_list.end(), p), coll_list.end());
    }
  }

  void
  checking_mappings(coll_root_t& coll_root)
  {
