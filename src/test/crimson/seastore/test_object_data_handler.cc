...
public:
  TestOnode(uint32_t ddr, uint32_t dmr) :
    Onode(ddr, dmr, hobject_t())
  {}

  const onode_layout_t&
  ...
  void
  with_mutable_layout(Transaction& t, Func&& f)
  {
    f(layout);
  }

  bool
  is_alive() const final
  {
    return true;
  }

  void
  swap_layout(Transaction& t, Onode& other) final
  {
    static_cast<TestOnode&>(other).with_mutable_layout(
        t, [this](auto& o_mlayout) {
  ...
  void
  set_need_cow(Transaction& t) final
  {
  ...
  void
  unset_need_cow(Transaction& t) final
  {
    with_mutable_layout(t, [](onode_layout_t& mlayout) {
  ...
  void
  update_onode_size(Transaction& t, uint32_t size) final
  {
  ...
  void
  update_omap_root(Transaction& t, omap_root_t& oroot) final
  {
  ...
  void
  update_xattr_root(Transaction& t, omap_root_t& xroot) final
  {
  ...
  void
  update_object_data(Transaction& t, object_data_t& odata) final
  {
  ...
  void
  update_object_info(Transaction& t, ceph::bufferlist& oi_bl) final
  {
    with_mutable_layout(t, [&oi_bl](onode_layout_t& mlayout) {
      maybe_inline_memcpy(
          &mlayout.oi[0], oi_bl.c_str(), oi_bl.length(),
  ...
  void
  clear_object_info(Transaction& t) final
  {
    with_mutable_layout(t, [](onode_layout_t& mlayout) {
  ...
  void
  update_snapset(Transaction& t, ceph::bufferlist& ss_bl) final
  {
    with_mutable_layout(t, [&ss_bl](onode_layout_t& mlayout) {
      maybe_inline_memcpy(
          &mlayout.ss[0], ss_bl.c_str(), ss_bl.length(),
          onode_layout_t::MAX_OI_LENGTH);
  ...
  void
  clear_snapset(Transaction& t) final
  {
    with_mutable_layout(t, [](onode_layout_t& mlayout) {
...
struct object_data_handler_test_t : public seastar_test_suite_t, TMTestState {
  struct test_object_t {
    ...
    {
      onode = new TestOnode(
          DEFAULT_OBJECT_DATA_RESERVATION, DEFAULT_OBJECT_METADATA_RESERVATION);
      size = 0;
    ...
    void
    clone_from(const test_object_t& other)
    {
      size = other.size;
      known_contents = bufferptr(
  ...
  void
  clone(Transaction& t, snapid_t target_snap)
  {
    with_trans_intr(
        t,
        seastar::coroutine::lambda([&](auto& t) -> ObjectDataHandler::clone_ret {
          ...
          co_await objhandler.clone(ObjectDataHandler::context_t{
      ...
      snapid_t snap = CEPH_NOSNAP)
  {
    ...
    memset(obj.known_contents.c_str() + offset, fill, len);
    bufferlist bl;
    bl.append(bufferptr(obj.known_contents, offset, len));
    with_trans_intr(t, [&](auto& t) {
      return seastar::do_with(
          std::move(bl), ObjectDataHandler(MAX_OBJECT_SIZE),
  ...
  void
  truncate(Transaction& t, objaddr_t offset, snapid_t snap = CEPH_NOSNAP)
  {
    auto& obj = get_object(snap);
    if (obj.size > offset) {
      memset(obj.known_contents.c_str() + offset, 0, obj.size - offset);
      with_trans_intr(t, [&](auto& t) {
        return seastar::do_with(
            ObjectDataHandler(MAX_OBJECT_SIZE), [=, this, &t](auto& objhandler) {
              return objhandler.truncate(
                  ObjectDataHandler::context_t{*tm, t, *obj.onode}, offset);
            });
      }).unsafe_get();
    }
    obj.size = offset;
  ...
  void
  truncate(objaddr_t offset, snapid_t snap = CEPH_NOSNAP)
  {
    auto t = create_mutate_transaction();
      ...
      snapid_t snap = CEPH_NOSNAP)
  {
    ...
    bufferlist bl = with_trans_intr(t, [&](auto& t) {
                      return ObjectDataHandler(MAX_OBJECT_SIZE)
                          .read(
                              ObjectDataHandler::context_t{*tm, t, *obj.onode},
                              offset, len);
                    }).unsafe_get();
    bufferlist known;
    known.append(bufferptr(obj.known_contents, offset, len));
    EXPECT_EQ(bl.length(), known.length());
    EXPECT_EQ(bl, known);
  }

  void
  read(objaddr_t offset, extent_len_t len, snapid_t snap = CEPH_NOSNAP)
  {
  ...
  std::list<LBAMapping>
  get_mappings(Transaction& t, objaddr_t offset, extent_len_t length)
  {
    auto ret =
        with_trans_intr(t, [&](auto& t) {
  ...
  std::list<LBAMapping>
  get_mappings(objaddr_t offset, extent_len_t length)
  {
    auto t = create_mutate_transaction();
    auto ret =
        with_trans_intr(*t, [&](auto& t) {
  ...
  ObjectDataBlockRef
  get_extent(Transaction& t, loffset_t addr, extent_len_t len)
  {
    auto& layout = head.onode->get_layout();
    auto odata = layout.object_data.get();
    auto obase = odata.get_reserved_data_base();
    auto maybe_indirect_ext = with_trans_intr(t, [&](auto& trans) {
                                return tm->read_extent<ObjectDataBlock>(
                                    trans, (obase + addr).checked_to_laddr(),
                                    len);
                              }).unsafe_get();
  ...
  enable_delta_based_overwrite()
  {
    crimson::common::local_conf()
        .set_val("seastore_data_delta_based_overwrite", "16777216")
        .get();
  }
  ...
  {
    crimson::common::local_conf()
        .set_val("seastore_data_delta_based_overwrite", "0")
        .get();
  }
  ...
  objaddr_t
  get_random_write_offset(size_t block_size, objaddr_t limit)
  {
    return block_size *
...
{
  run_async([this] { test_multi_write(); });
}
...
{
  run_async([this] { test_write_hole(); });
}
...
{
  run_async([this] { test_overwrite_single(); });
}
...
{
  run_async([this] { test_overwrite_double(); });
}
...
{
  run_async([this] { test_overwrite_partial(); });
}
...
{
  run_async([this] { test_unaligned_write(); });
}
...
{
  run_async([this] { test_unaligned_overwrite(); });
}
...
TEST_P(object_data_handler_test_t, multiple_remap)
{
  run_async([this] {
    ...
    size_t res[11] = {0,        4 << 10,   12 << 10, 20 << 10,
                      32 << 10, 36 << 10,  60 << 10, 64 << 10,
                      96 << 10, 120 << 10, 124 << 10};
    auto base = pins.front().get_key();
    int i = 0;
    for (auto& pin : pins) {
    ...
    for (unsigned i = 0; i < 4; ++i) {
      for (unsigned j = 0; j < 100; ++j) {
        ...
        for (unsigned k = 0; k < 2; ++k) {
          write(
              *t, get_random_write_offset(BSIZE, TOTAL), wsize,
...
TEST_P(object_data_handler_test_t, overwrite_then_read_within_transaction)
{
  run_async([this] {
    ...
    bufferlist committed =
        with_trans_intr(*t2, [&](auto& t) {
          return ObjectDataHandler(MAX_OBJECT_SIZE)
              .read(
                  ObjectDataHandler::context_t{*tm, t, *head.onode},
                  base + 4096, 4096);
        }).unsafe_get();
    bufferlist pending;
    pending.append(bufferptr(head.known_contents, base + 4096, 4096));
...
TEST_P(object_data_handler_test_t, parallel_partial_read)
{
  run_async([this] {
    ...
    seastar::parallel_for_each(
        boost::make_counting_iterator(0lu), boost::make_counting_iterator(8lu),
        [&](auto i) { return seastar::async([&] { read(i * 4096, 8192); }); })
        .get();
    disable_delta_based_overwrite();
    enable_max_extent_size();
  });
