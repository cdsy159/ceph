...
struct rbm_test_t : public seastar_test_suite_t {
  ...
  struct rbm_transaction {
    void
    add_rbm_allocated_blocks(alloc_delta_t& d)
    {
      allocated_blocks.push_back(d);
    }

    void
    clear_rbm_allocated_blocks()
    {
    ...
    return device->mkfs(config)
        ...
        .then([this] {
          return device->mount()
              .handle_error(crimson::ct_error::assert_all{})
              .then([this] {
                return rbm_manager->open().handle_error(
  ...
  tear_down_fut() final
  {
    co_await rbm_manager->close().handle_error(crimson::ct_error::assert_all{});
    co_await device->close().handle_error(crimson::ct_error::assert_all{});
    rbm_manager.reset();
    device.reset();
    co_return;
  }
  ...
  auto
  write(uint64_t addr, bufferptr& ptr)
  {
    paddr_t paddr =
        convert_abs_addr_to_paddr(addr, rbm_manager->get_device_id());
    return rbm_manager->write(paddr, ptr).unsafe_get();
  }

  auto
  read(uint64_t addr, bufferptr& ptr)
  {
    paddr_t paddr =
        convert_abs_addr_to_paddr(addr, rbm_manager->get_device_id());
