...
struct cache_test_t : public seastar_test_suite_t {
  ...
  submit_transaction(TransactionRef t)
  {
    auto chksum_func = [](auto& extent) {
      ...
      if (!extent->is_logical() || !extent->get_last_committed_crc()) {
    ...
    std::for_each(
        pre_allocated_extents.begin(), pre_allocated_extents.end(), chksum_func);
    ...
    ceph_assert(
        (segment_off_t)bl.length() < segment_manager->get_segment_size());
    ...
    current.as_seg_paddr().set_segment_off(
        current.as_seg_paddr().get_segment_off() + bl.length());
  ...
  get_transaction()
  {
    return cache->create_transaction(
        Transaction::src_t::MUTATE, "test_cache", CACHE_HINT_TOUCH, false);
  }

  template <typename T, typename... Args>
  auto
  get_extent(Transaction& t, Args&&... args)
  {
    return with_trans_intr(
        t,
        [this](auto&&... args) { return cache->get_caching_extent<T>(args...); },
    ...
    return segment_manager->init()
        ...
        .safe_then([this] {
          ...
          return seastar::do_with(get_transaction(), [this](auto& ref_t) {
            cache->init();
            return with_trans_intr(
                       *ref_t, [&](auto& t) { return cache->mkfs(t); })
                .safe_then([this, &ref_t] {
                  return submit_transaction(std::move(ref_t)).discard_result();
                });
          });
        })
        .handle_error(crimson::ct_error::assert_all{"failed to submit"});
  }
  ...
  run_async([this] {
    ...
    int csum = 0;
    {
      ...
      auto extent = cache->alloc_new_non_data_extent<TestBlockPhysical>(
          *t, TestBlockPhysical::SIZE, placement_hint_t::HOT, 0);
    ...
    }
    {
      auto t = get_transaction();
      auto extent =
          get_extent<TestBlockPhysical>(*t, addr, TestBlockPhysical::SIZE)
              .unsafe_get();
  ...
  run_async([this] {
    ...
    int csum2 = 0;
    {
      ...
      ASSERT_TRUE(reladdr.is_relative());
      {
        ...
        // uncommitted
        auto extent =
            get_extent<TestBlockPhysical>(*t, reladdr, TestBlockPhysical::SIZE)
                .unsafe_get();
    ...
    }
    {
      ...
      auto extent2 =
          get_extent<TestBlockPhysical>(*t2, addr, TestBlockPhysical::SIZE)
              .unsafe_get();
