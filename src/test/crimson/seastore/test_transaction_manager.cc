...
laddr_t
get_laddr_hint(uint64_t offset)
{
...
struct test_extent_record_t {
  ...
  test_extent_record_t(const test_extent_desc_t& desc, unsigned refcount) :
    desc(desc), refcount(refcount)
  {}

  void
  update(const test_extent_desc_t& to)
  {
    desc = to;
  }

  bool
  operator==(const test_extent_desc_t& rhs) const
  {
    return desc == rhs;
  }
...
struct fmt::formatter<test_extent_record_t> : fmt::formatter<std::string_view> {
  ...
  auto
  format(const test_extent_record_t& r, FormatContext& ctx) const
  {
    return fmt::format_to(
        ctx.out(), "test_extent_record_t({}, refcount={})", r.desc, r.refcount);
  }
};
...
struct transaction_manager_test_t : public seastar_test_suite_t, TMTestState {
  ...
  laddr_t
  get_random_laddr(size_t block_size, size_t limit)
  {
    auto offset = block_size * std::uniform_int_distribution<>(
                                   0, (limit / block_size) - 1)(gen);
  ...
  struct test_extents_t : std::map<laddr_t, test_extent_record_t> {
    ...
    struct delta_overlay_t {
      ...
      class iterator {
        ...
        is_valid()
        {
          return is_end() || ((get_okey() < get_bkey()) && (oiter->second)) ||
                 (get_okey() > get_bkey());
        }
        ...
        get_pair()
        {
          ...
          auto bkey = get_bkey();
          return (
              bkey < okey ? std::pair<laddr_t, test_extent_record_t>(*biter)
  ...
  private:
    void
    check_available(laddr_t addr, extent_len_t len, const delta_t& delta) const
    {
    ...
    {
      delta_overlay_t overlay(*this, delta);
      auto real_hint = intermediate_hint == L_ADDR_NULL ? hint
                                                        : intermediate_hint;
    ...
    std::optional<test_extent_record_t>&
    populate_delta(laddr_t addr, delta_t& delta, const test_extent_desc_t* desc)
        const
    {
      ...
      if (iter == end()) {
        assert(desc);
        auto ret =
            delta.emplace(std::make_pair(addr, test_extent_record_t{*desc, 0}));
        assert(ret.second);
  ...
  public:
    delta_overlay_t
    get_overlay(const delta_t& delta) const
    {
      return delta_overlay_t{*this, delta};
    }

    void
    insert(TestBlock& extent, delta_t& delta) const
    {
    ...
    {
      check_hint(
          hint, intermediate_hint, extent.get_laddr(), extent.get_length(),
          delta);
      insert(extent, delta);
    }

    bool
    contains(laddr_t addr, const delta_t& delta) const
    {
      delta_overlay_t overlay(*this, delta);
    ...
    get(laddr_t addr, const delta_t& delta) const
    {
      delta_overlay_t overlay(*this, delta);
    ...
    void
    update(laddr_t addr, const test_extent_desc_t& desc, delta_t& delta) const
    {
    ...
    int
    inc_ref(laddr_t addr, delta_t& delta) const
    {
    ...
    int
    dec_ref(laddr_t addr, delta_t& delta) const
    {
      auto& rec = populate_delta(addr, delta, nullptr);
      assert(rec);
    ...
    void
    consume(const delta_t& delta, const uint64_t write_seq = 0)
    {
  ...
  disable_max_extent_size()
  {
    ...
    crimson::common::local_conf()
        .set_val("seastore_max_data_allocation_size", "16777216")
        .get();
  }
  ...
  enable_max_extent_size()
  {
    ...
    crimson::common::local_conf()
        .set_val("seastore_max_data_allocation_size", "8192")
        .get();
  }
  ...
  TestBlockRef
  alloc_extent(test_transaction_t& t, laddr_t hint, extent_len_t len)
  {
    return alloc_extent(t, hint, len, get_random_contents());
  }

  std::vector<TestBlockRef>
  alloc_extents(test_transaction_t& t, laddr_t hint, extent_len_t len)
  {
    return alloc_extents(t, hint, len, get_random_contents());
  }
  ...
  TestBlockRef
  read_pin(test_transaction_t& t, LBAMapping pin)
  {
  ...
  TestBlockRef
  get_extent(test_transaction_t& t, laddr_t addr, extent_len_t len)
  {
    ceph_assert(test_mappings.contains(addr, t.mapping_delta));
    ceph_assert(test_mappings.get(addr, t.mapping_delta).desc.len == len);
  ...
  TestBlockRef
  try_get_extent(test_transaction_t& t, laddr_t addr)
  {
    ceph_assert(test_mappings.contains(addr, t.mapping_delta));
    ...
    auto ext =
        with_trans_intr(
            *(t.t),
            [&](auto& trans) { return tm->read_extent<TestBlock>(trans, addr); })
            .safe_then([](auto ret) {
  ...
  TestBlockRef
  try_get_extent(test_transaction_t& t, laddr_t addr, extent_len_t len)
  {
    ceph_assert(test_mappings.contains(addr, t.mapping_delta));
    ceph_assert(test_mappings.get(addr, t.mapping_delta).desc.len == len);
  ...
  TestBlockRef
  try_read_pin(test_transaction_t& t, const LBAMapping pin)
  {
    ...
    auto ext =
        with_trans_intr(
            *(t.t),
            [&](auto& trans) { return tm->read_pin<TestBlock>(trans, pin); })
            .safe_then([](auto ret) {
              return ertr::make_ready_future<TestBlockRef>(ret.extent);
            })
            .handle_error(
  ...
  TestBlockRef
  mutate_extent(test_transaction_t& t, TestBlockRef ref)
  {
  ...
  TestBlockRef
  mutate_addr(test_transaction_t& t, laddr_t offset, size_t length)
  {
  ...
  LBAMapping
  get_pin(test_transaction_t& t, laddr_t offset)
  {
    ...
    auto pin = with_trans_intr(
                   *(t.t),
                   [&](auto& trans) {
                     ...
                     return tm->clone_pin(
                         trans, std::move(pos), std::move(mapping), offset, 0,
                         len, true);
                   })
                   .unsafe_get()
                   .cloned_mapping;
  ...
  LBAMapping
  get_end(test_transaction_t& t)
  {
    return LBAMapping::create_direct(with_trans_intr(*(t.t), [&](auto& trans) {
                                       return lba_manager->get_end_mapping(
                                           trans);
                                     }).unsafe_get());
  }

  std::optional<LBAMapping>
  try_get_pin(test_transaction_t& t, laddr_t offset)
  {
    ceph_assert(test_mappings.contains(offset, t.mapping_delta));
    ...
    auto pin =
        with_trans_intr(
            *(t.t), [&](auto& trans) { return tm->get_pin(trans, offset); })
            .safe_then([](auto pin) -> ret {
              return ertr::make_ready_future<std::optional<LBAMapping>>(
                  std::move(pin));
            })
            .handle_error(
                [](const crimson::ct_error::eagain& e) {
  ...
  void
  remove(test_transaction_t& t, laddr_t offset)
  {
    ceph_assert(test_mappings.contains(offset, t.mapping_delta));
  ...
  void
  check_mappings(test_transaction_t& t)
  {
  ...
  std::optional<LBAMapping>
  refresh_lba_mapping(test_transaction_t& t, LBAMapping mapping)
  {
  ...
  bool
  try_submit_transaction(test_transaction_t t)
  {
    ...
    bool success = submit_transaction_fut_with_seq(*t.t)
                       ...
                       .then([this](auto ret) {
                         return epm->run_background_work_until_halt().then(
                             [ret] { return ret; });
                       })
                       .get();
  ...
  void
  submit_transaction(test_transaction_t&& t)
  {
  ...
  void
  submit_transaction_expect_conflict(test_transaction_t&& t)
  {
    bool success = try_submit_transaction(std::move(t));
  ...
  auto
  allocate_sequentially(const size_t size, const int num, bool run_clean = true)
  {
    return repeat_eagain([this, size, num] {
             return seastar::do_with(
                 create_transaction(), [this, size, num](auto& t) {
  ...
  test_parallel_extent_read()
  {
    ...
    run_async([this] {
      for (unsigned i = 0; i < BLOCKS; ++i) {
        ...
        auto extent = alloc_extent(t, get_laddr_hint(i * BSIZE), BSIZE);
      ...
      seastar::do_with(create_read_test_transaction(), [this](auto& t) {
        return with_trans_intr(*(t.t), [this](auto& t) {
          return trans_intr::parallel_for_each(
              ...
              boost::make_counting_iterator(BLOCKS), [this, &t](auto i) {
                return tm
                    ->read_extent<TestBlock>(t, get_laddr_hint(i * BSIZE), BSIZE)
  ...
  test_random_writes_concurrent()
  {
    ...
    run_async([this] {
      std::for_each(
          ...
          boost::make_counting_iterator(WRITE_STREAMS), [&](auto idx) {
          ...
          [&](auto) {
            return seastar::async([&] {
              while (writes < 300) {
                auto t = create_transaction();
                auto ext =
                    try_get_extent(t, get_random_laddr(BSIZE, TOTAL), BSIZE);
                if (!ext) {
                  failures++;
                  continue;
                }
      ...
      check();
      logger().info(
          "random_writes_concurrent: {} suceeded, {} failed", writes, failures);
    });
  }
  ...
  test_evict()
  {
    ...
    run_async([this] {
      ...
      epm->background_process.eviction_state.init(
          stop_ratio, default_ratio, fast_ratio, epm->hot_tier_generations);
      ...
      auto run_until = [this](size_t size) -> seastar::future<> {
        return seastar::repeat([this, size] {
          size_t current_size =
              epm->background_process.main_cleaner->get_stat().data_stored;
      ...
      for (auto i = INIT_GENERATION; i <= epm->dynamic_max_rewrite_generation;
           i++) {
      ...
      for (auto t : all_extent_types) {
        ...
        if (is_root_type(t) || is_lba_backref_node(t)) {
          ...
          for (auto i = INIT_GENERATION + 1;
               i <= epm->dynamic_max_rewrite_generation; i++) {
            expected_generations[t][i] = i;
          }
        }
      }

      auto update_data_gen_mapping =
          [&](std::function<rewrite_gen_t(rewrite_gen_t)> func) {
            for (auto t : all_extent_types) {
              if (is_root_type(t) || is_lba_backref_node(t)) {
                continue;
              }
        ...
        for (auto t : all_extent_types) {
          for (auto gen : all_generations) {
            auto epm_gen = epm->adjust_generation(
                get_extent_category(t), t, placement_hint_t::HOT, gen);
            if (expected_generations[t][gen] != epm_gen) {
              logger().error(
                  "caller: {}, extent type: {}, input generation: {}, "
      ...
      auto main_size =
          epm->background_process.main_cleaner->get_stat().data_stored;
      auto cold_size =
          epm->background_process.cold_cleaner->get_stat().data_stored;
                   ...
                   *(t.t),
                   [&](auto& trans) {
                     return tm
                         ->remap_pin<TestBlock>(
                             trans, std::move(opin),
                             std::array{remap_entry_t(new_offset, new_len)})
                         .si_then([](auto ret) {
                           return base_iertr::make_ready_future<
  ...
  using _overwrite_pin_ret =
      _overwrite_pin_iertr::future<overwrite_pin_ret_bare_t>;
      ...
      ceph::bufferlist& bl)
  {
    ...
    if (new_offset != 0 && o_len != new_offset + new_len) {
      return tm
          ->remap_pin<TestBlock, 2>(
              t, std::move(opin),
              std::array{
                  remap_entry_t(0, new_offset),
                  remap_entry_t(
                      new_offset + new_len, o_len - new_offset - new_len)})
          .si_then([this, new_offset, new_len, o_laddr, &t, &bl](auto ret) {
            return tm
                ->alloc_data_extents<TestBlock>(
                    t, (o_laddr + new_offset).checked_to_laddr(), new_len)
                .si_then([this, ret = std::move(ret), new_len, new_offset,
                  ...
                  auto r_laddr =
                      (o_laddr + new_offset + new_len).checked_to_laddr();
                  // old pins expired after alloc new extent, need to get it.
                  return tm->get_pin(t, o_laddr)
                      .si_then([this, &t, ext = std::move(ext),
                                r_laddr](auto lpin) mutable {
                        return tm->get_pin(t, r_laddr)
                            .si_then([lpin = std::move(lpin),
                                      ext = std::move(ext)](auto rpin) mutable {
                              return _overwrite_pin_iertr::make_ready_future<
                                  overwrite_pin_ret_bare_t>(
                                  std::move(lpin), std::move(ext),
                                  std::move(rpin));
                            });
                      });
                })
      ...
      return tm
          ->remap_pin<TestBlock, 1>(
              t, std::move(opin), std::array{remap_entry_t(0, new_offset)})
          .si_then([this, new_offset, new_len, o_laddr, &t, &bl](auto ret) {
            return tm
                ->alloc_data_extents<TestBlock>(
                    t, (o_laddr + new_offset).checked_to_laddr(), new_len)
                .si_then([this, ret = std::move(ret), new_len, o_laddr, &t,
                          &bl](auto extents) mutable {
                  assert(extents.size() == 1);
                  auto ext = extents.front();
                  ceph_assert(ret.size() == 1);
                  auto iter = bl.cbegin();
      ...
      ceph::bufferlist& bl)
  {
    ...
    if (rpin) {
      ...
      EXPECT_EQ(
          rpin->get_val(), o_paddr.add_offset(new_offset).add_offset(new_len));
  ...
  test_overwrite_pin()
  {
    run_async([this] {
      ...
      }
      {
        ...
        auto [mlp1, mext1, mrp1] =
            overwrite_pin(t, std::move(mpin), 8 << 10, 8 << 10, mbl1);
        auto mlp1_key = mlp1->get_key();
        auto mlp1_length = mlp1->get_length();
        auto [mlp2, mext2, mrp2] =
            overwrite_pin(t, std::move(*mrp1), 4 << 10, 16 << 10, mbl2);
        auto mlp2_key = mlp2->get_key();
        auto mlp2_length = mlp2->get_length();
        auto [mlpin3, me3, mrpin3] =
            overwrite_pin(t, std::move(*mrp2), 4 << 10, 12 << 10, mbl3);
        ...
        auto [llp1, lext1, lrp1] =
            overwrite_pin(t, std::move(lpin), 0, 32 << 10, lbl1);
        EXPECT_FALSE(llp1);
        EXPECT_TRUE(lrp1);
        EXPECT_TRUE(lext1);
        ...
        auto [rlp1, rext1, rrp1] =
            overwrite_pin(t, std::move(rpin), 32 << 10, 32 << 10, rbl1);
        EXPECT_TRUE(rlp1);
        EXPECT_TRUE(rext1);
        EXPECT_FALSE(rrp1);
          ...
          [&](auto) {
            return seastar::async([&] {
              ...
              for (auto off : split_points) {
                ...
                auto new_off =
                    get_laddr_hint(off << 10).get_byte_distance<extent_len_t>(
                        last_pin->get_key());
              ...
              if (try_submit_transaction(std::move(t))) {
                success++;
                logger().info(
                    "transaction {} submit the transction",
          ...
          [&](auto) {
            return seastar::async([&] {
              ...
              while (!split_points.empty()) {
                ...
                if ((get_laddr_hint(end_off << 10) ==
                     pin0->get_key() + pin0->get_length()) ||
                    (start_off == end_off)) {
...
struct tm_random_block_device_test_t : public transaction_manager_test_t {
...
TEST_P(tm_random_block_device_test_t, scatter_allocation)
{
  run_async([this] {
    ...
    for (int i = 0; i < 1989; i++) {
      auto extents =
          alloc_extents(t, (ADDR + i * 16384).checked_to_laddr(), 16384, 'a');
    }
    alloc_extents_deemed_fail(
        t, (ADDR + 1991 * 16384).checked_to_laddr(), 16384, 'a');
    check_mappings(t);
    check();
    submit_transaction(std::move(t));
    check();
  ...
  run_async([this] {
    {
      ...
      for (unsigned i = 0; i < 300; ++i) {
        auto extent = alloc_extent(t, get_laddr_hint(i * SIZE), SIZE);
      }
      check_mappings(t);
      submit_transaction(std::move(t));
      check();
  ...
  run_async([this] {
    ...
    }
    {
      ...
      for (unsigned i = 0; i < 240; ++i) {
        remove(t, get_laddr_hint(i * SIZE));
      }
      check_mappings(t);
      submit_transaction(std::move(t));
      check();
...
TEST_P(tm_single_device_test_t, cause_lba_split)
{
  ...
  run_async([this] {
    for (unsigned i = 0; i < 200; ++i) {
      auto t = create_transaction();
      auto extent =
          alloc_extent(t, get_laddr_hint(i * SIZE), SIZE, (char)(i & 0xFF));
      ...
      for (unsigned j = 0; j < 65; ++j) {
        ...
        for (unsigned k = 0; k < 2; ++k) {
          ...
          auto paddings = alloc_extents(
              t, get_laddr_hint(TOTAL + (k * PADDING_SIZE)), PADDING_SIZE);
...
TEST_P(tm_single_device_test_t, find_hole_assert_trigger)
{
  ...
  run([&, this] {
    return seastar::parallel_for_each(
        boost::make_counting_iterator(0u), boost::make_counting_iterator(max),
...
TEST_P(tm_single_device_intergrity_check_test_t, remap_lazy_read)
{
  ...
  run_async([this, offset] {
    ...
    {
      auto t = create_transaction();
      auto extents = alloc_extents(t, offset, length, 'a');
      for (auto& extent : extents) {
    ...
    replay();
    {
      ...
      auto [lpin, ext, rpin] =
          overwrite_pin(t, std::move(pin), 4 << 10, 64 << 10, bl);
      check_mappings(t);
      submit_transaction(std::move(t));
      check();
    }
  ...
  run_async([this] {
    using namespace crimson::os::seastore::lba;
    {
      ...
      for (unsigned i = 0; i < LEAF_NODE_CAPACITY; i++) {
        auto extent = alloc_extent(t, get_laddr_hint(i * 4096), 4096, 'a');
      }
      submit_transaction(std::move(t));
    }

    {
      ...
      auto extent =
          alloc_extent(t, get_laddr_hint(LEAF_NODE_CAPACITY * 4096), 4096, 'a');
      assert(!pin.is_viewable());
      pin = get_pin(t, get_laddr_hint(LEAF_NODE_CAPACITY * 4096));
      assert(pin.is_viewable());
      std::ignore = alloc_extent(
          t, get_laddr_hint((LEAF_NODE_CAPACITY + 1) * 4096), 4096, 'a');
      assert(!pin.is_viewable());
...
TEST_P(tm_multi_tier_device_test_t, evict) { test_evict(); }
...
TEST_P(tm_single_device_test_t, test_overwrite_pin) { test_overwrite_pin(); }
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Combine(
        ...
        ::testing::Values(integrity_check_t::NONFULL_CHECK)));
...
INSTANTIATE_TEST_SUITE_P(
    ...
    tm_random_block_device_test_t,
    ::testing::Combine(
        ::testing::Values("circularbounded"),
        ::testing::Values(integrity_check_t::NONFULL_CHECK)));
