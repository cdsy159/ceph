...
public:
  ...
  {
    auto num_device_managers = num_main_device_managers +
    ...
    return segment_manager->init()
        ...
        .safe_then([this] {
          return seastar::do_with(std::size_t(0), [this](auto& cnt) {
            return crimson::do_for_each(
                secondary_segment_managers.begin(),
                secondary_segment_managers.end(), [this, &cnt](auto& sec_sm) {
                  ++cnt;
                  return sec_sm->mkfs(
                      segment_manager::get_ephemeral_device_config(
                          cnt, num_main_device_managers,
                          num_cold_device_managers));
                });
          });
        })
...
protected:
  ...
  tm_setup()
  {
    ...
    if (j_type == "circularbounded") {
      ...
      devices.reset(new EphemeralSegmentedDevices(
...
protected:
  ...
  _teardown()
  {
    return tm->close().handle_error(crimson::ct_error::assert_all{
        "Error in teardown"});
  }

  virtual FuturizedStore::mount_ertr::future<>
  _mount()
  {
    return tm->mount()
  ...
  _mkfs()
  {
    return tm->mkfs().handle_error(crimson::ct_error::assert_all{
        "Error in mkfs"});
  }
  ...
  {
    return tm->create_transaction(Transaction::src_t::MUTATE, "test_mutate");
  }

  auto
  create_read_transaction()
  {
    return tm->create_transaction(Transaction::src_t::READ, "test_read");
  }
  ...
  auto
  submit_transaction_fut2(Transaction& t)
  {
    return tm->submit_transaction(t);
  }

  auto
  submit_transaction_fut(Transaction& t)
  {
  ...
  auto
  submit_transaction_fut_with_seq(Transaction& t)
  {
    return with_trans_intr(t, [this](auto& t) {
      return tm->submit_transaction(t).si_then([this] {
...
void
EphemeralSegmentedDevices::set_primary_device_ref(DeviceRef dev)
{
...
void
EphemeralRandomBlockDevices::set_primary_device_ref(DeviceRef dev)
{
  rb_device = random_block_device::RBMDeviceRef(
      ...
      read_meta(const std::string& key) final
      {
        ...
        if (iter != parent.md.end()) {
          return read_meta_ret(
              read_meta_ertr::ready_future_marker{}, iter->second);
        } else {
          return read_meta_ret(
              read_meta_ertr::ready_future_marker{}, std::nullopt);
        }
      }
    };
...
protected:
  ...
  _init() final
  {
    ...
    return fut
        ...
        .then([this] { sharded_seastore = &(seastore->get_sharded_store()); });
  }
