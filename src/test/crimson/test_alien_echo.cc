...
struct DummyAuthAuthorizer : public AuthAuthorizer {
  ...
  bool
  add_challenge(CephContext*, const bufferlist&) override
  {
    return true;
  }
};
...
struct Server {
  ...
  struct ServerDispatcher final : crimson::net::Dispatcher {
    ...
    ms_dispatch(crimson::net::ConnectionRef c, MessageRef m) final
    {
  ...
  Server(crimson::net::MessengerRef msgr) :
    byte_throttler(local_conf()->osd_client_message_size_cap), msgr{msgr}
  {}
};
...
public:
  SeastarContext() :
    begin_fd{eventfd(0, 0)}, on_end{seastar::file_desc::eventfd(0, 0)}
  {}

  template <class Func>
  std::thread
  with_seastar(Func&& func)
  {
    return std::thread{
        [this, on_end = on_end.get(), func = std::forward<Func>(func)] {
          // alien: are you ready?
          wait_for_seastar();
  ...
  void
  run(seastar::app_template& app, int argc, char** argv)
  {
      ...
      return crimson::common::sharded_conf()
          ...
          .then([on_end = std::move(on_end)]() mutable {
            ...
            return seastar::do_with(
                seastar::pollable_fd(std::move(on_end)),
  ...
  if (role == echo_role::as_server) {
    return seastar::do_with(
        ...
        [addr, count](auto& server) mutable {
          ...
          server.msgr->set_policy_throttler(
              entity_name_t::TYPE_OSD, &server.byte_throttler);
          server.msgr->set_auth_client(&server.dummy_auth);
          server.msgr->set_auth_server(&server.dummy_auth);
          return server.msgr->bind(entity_addrvec_t{addr})
              .safe_then(
                  [&server] { return server.msgr->start({&server.dispatcher}); },
    ...
    return seastar::do_with(
        ...
        [addr, count](auto& client) {
          ...
          client.msgr->set_default_policy(
              crimson::net::SocketPolicy::lossy_client(0));
          client.msgr->set_policy_throttler(
              entity_name_t::TYPE_OSD, &client.byte_throttler);
...
main(int argc, char** argv)
{
  ...
  std::transform(
      begin(unrecognized_options), end(unrecognized_options),
