...
test_accumulate(ThreadPool& tp)
{
  ...
  return seastar::map_reduce(std::views::iota(0, N), slow_plus, 0, std::plus{})
      .then([](int sum) {
...
seastar::future<>
test_void_return(ThreadPool& tp)
{
    ...
    return crimson::common::sharded_conf()
        ...
        .then([] {
          return seastar::do_with(
              std::make_unique<crimson::os::ThreadPool>(
                  2, 128, seastar::resource::cpuset{0}),
              [](auto& tp) {
                return tp->start()
