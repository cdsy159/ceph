...
struct Server {
  ...
  {
    msgr.reset(Messenger::create(
        cct, "async", entity.name, "pong", entity.addr.get_nonce()));
  ...
  struct ServerDispatcher : Dispatcher {
    ...
    bool
    ms_dispatch(Message*) override
    {
      ceph_abort();
    }

    bool
    ms_handle_reset(Connection*) override
    {
      return true;
    }
...
struct Client {
  ...
  {
    msgr.reset(Messenger::create(
        cct, "async", entity_name_t::CLIENT(-1), "ping", getpid()));
    dummy_auth.auth_registry.refresh_config();
    msgr->set_cluster_protocol(CEPH_OSD_PROTOCOL);
  ...
  struct ClientDispatcher : Dispatcher {
    ...
    bool
    ping(Messenger* msgr, const entity_inst_t& peer)
    {
      using namespace std::chrono_literals;
      auto conn =
          msgr->connect_to(peer.name.type(), entity_addrvec_t{peer.addr});
      replied = false;
...
static void
ceph_echo(CephContext* cct, entity_addr_t addr, echo_role role, unsigned count)
{
  std::cout << "ceph/";
  ...
  if (role == echo_role::as_server) {
    ...
    client.msgr->start();
    auto conn = client.msgr->connect_to(
        entity.name.type(), entity_addrvec_t{entity.addr});
    for (unsigned i = 0; i < count; i++) {
