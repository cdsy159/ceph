...
struct improved_hobject_t : hobject_t {
  ...
  bool
  operator==(const improved_hobject_t& rhs) const
  {
...
struct FakeStore {
  ...
  void
  push(const hobject_t& obj, eversion_t version)
  {
    objs[obj] = version;
  }

  void
  drop(const hobject_t& obj, const eversion_t version)
  {
  ...
  bool
  looks_like(const FakeStore& rhs) const
  {
  ...
  bool
  operator==(const FakeStore& rhs) const
  {
...
class BackfillFixture : public crimson::osd::BackfillState::BackfillListener {
  ...
  std::map<pg_shard_t, std::vector<std::pair<hobject_t, eversion_t>>>
      enqueued_drops;
  std::deque<boost::intrusive_ptr<const boost::statechart::event_base>>
      events_to_dispatch;
  ...
  void
  schedule_event(const EventT& event)
  {
  ...
  template <class EventT>
  void
  schedule_event_immediate(const EventT& event)
  {
...
public:
  ...
  void
  next_round(std::size_t how_many = 1)
  {
  ...
  bool
  all_stores_look_like(const FakeStore& reference) const
  {
...
struct BackfillFixture::PeeringFacade
  ...
  PeeringFacade(
      FakePrimary& backfill_source,
      std::map<pg_shard_t, FakeReplica>& backfill_targets) :
    backfill_source(backfill_source), backfill_targets(backfill_targets)
  {
    std::transform(
        std::begin(backfill_targets), std::end(backfill_targets),
        std::inserter(
            backfill_targets_as_set, std::end(backfill_targets_as_set)),
        [](auto pair) { return pair.first; });
  }

  hobject_t
  earliest_backfill() const override
  {
  ...
  get_backfill_targets() const override
  {
    return backfill_targets_as_set;
  }

  const hobject_t&
  get_peer_last_backfill(pg_shard_t peer) const override
  {
  ...
  const eversion_t&
  get_log_tail() const override
  {
    return backfill_source.log_tail;
  }

  const PGLog&
  get_pg_log() const override
  {
  ...
  void
  scan_log_after(eversion_t, scan_log_func_t) const override
  {
    /* NOP */
  }

  bool
  is_backfill_target(pg_shard_t peer) const override
  {
  ...
  bool
  is_backfilling() const override
  {
    return true;
  }
};

struct BackfillFixture::PGFacade
  : public crimson::osd::BackfillState::PGFacade {
  FakePrimary& backfill_source;
  ...
  const eversion_t&
  get_projected_last_update() const override
  {
    return backfill_source.projected_last_update;
  }

  const PGLog::IndexedLog&
  get_projected_log() const override
  {
...
BackfillFixture::BackfillFixture(
  ...
  backfill_state(
      *this,
      std::make_unique<PeeringFacade>(
          this->backfill_source,
      ...
      std::make_unique<PGFacade>(this->backfill_source))
{
  ...
  backfill_state.process_event(
      crimson::osd::BackfillState::Triggered{}.intrusive_from_this());
}

void
BackfillFixture::request_replica_scan(
    const pg_shard_t& target,
    const hobject_t& begin,
    const hobject_t& end)
{
  ...
  schedule_event(
      crimson::osd::BackfillState::ReplicaScanned{target, std::move(bi)});
}

void
BackfillFixture::request_primary_scan(const hobject_t& begin)
{
  ...
  bi.end = backfill_source.store.list(begin, [&bi](auto kv) {
    ...
    case 0:
      ...
      bi.objects.insert(
          std::make_pair(hoid, std::make_pair(shard_id_t::NO_SHARD, version)));
      break;
    case 1:
      // Optimized EC partial write - Shard 3 at an earlier version
      bi.objects.insert(
          std::make_pair(hoid, std::make_pair(shard_id_t(3), version_zero)));
      bi.objects.insert(
          std::make_pair(hoid, std::make_pair(shard_id_t::NO_SHARD, version)));
      break;
    case 2:
      // Optimized EC partial write - Shard 1 and 2 at an earlier version
      bi.objects.insert(std::make_pair(
          hoid, std::make_pair(shard_id_t::NO_SHARD, version_next)));
      bi.objects.insert(
          std::make_pair(hoid, std::make_pair(shard_id_t(1), version)));
      bi.objects.insert(
          std::make_pair(hoid, std::make_pair(shard_id_t(2), version)));
      break;
    case 3:
...
void
BackfillFixture::send_recovery_deletes(
    const hobject_t& obj,
...
void
BackfillFixture::update_peers_last_backfill(const hobject_t& new_last_backfill)
{
...
bool
BackfillFixture::budget_available() const
{
  return true;
}
...
struct BackfillFixtureBuilder {
  ...
  static BackfillFixtureBuilder
  add_source(FakeStore::objs_t objs)
  {
  ...
  BackfillFixtureBuilder&&
  add_target(FakeStore::objs_t objs) &&
  {
  ...
  get_result() &&
  {
    return BackfillFixture{
        std::move(backfill_source), std::move(backfill_targets)};
  }
};
...
TEST(backfill, same_primary_same_replica)
{
  ...
  auto cluster_fixture = BackfillFixtureBuilder::add_source(reference_store.objs)
                             .add_target(reference_store.objs)
                             .get_result();
...
random_hobject()
{
  ...
  return hobject_t(
      ...
      random_string(1 + (std::rand() % 16)), snapid_t(std::rand() % 1000),
      (std::rand() & ((~0) << bits)) | (mask & ~((~0) << bits)), 0,
...
FakeStore
mutate(const FakeStore& source_store)
{
...
TEST(backfill, one_pseudorandomized_replica)
{
  ...
  auto cluster_fixture =
      BackfillFixtureBuilder::add_source(reference_store.objs)
