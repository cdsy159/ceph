...
void
so_set_attr_len(ScrubMap::object& obj, const std::string& name, size_t len)
{
...
void
so_set_attr(ScrubMap::object& obj, const std::string& name, bufferlist bl)
{
  bl.rebuild();
  obj.attrs[name] = bl;
}
...
so_get_attr(ScrubMap::object& obj, const std::string& name)
{
...
template <typename T>
std::optional<T>
so_get_attr_type(ScrubMap::object& obj, const std::string& name)
{
...
void
so_set_oi(ScrubMap::object& obj, const std::optional<object_info_t>& oi)
{
  return so_set_attr_type<object_info_t>(obj, OI_ATTR, oi);
}

std::optional<object_info_t>
so_get_oi(ScrubMap::object& obj)
{
...
template <typename F>
void
so_mut_oi(ScrubMap::object& obj, F&& f)
{
  so_set_oi(obj, std::invoke(std::forward<F>(f), so_get_oi(obj)));
}

void
so_set_ss(ScrubMap::object& obj, const std::optional<SnapSet>& ss)
{
  return so_set_attr_type<SnapSet>(obj, SS_ATTR, ss);
}

std::optional<SnapSet>
so_get_ss(ScrubMap::object& obj)
{
...
template <typename F>
void
so_mut_ss(ScrubMap::object& obj, F&& f)
{
...
struct so_builder_t {
  ...
  static hobject_t
  make_hoid(std::string name, snapid_t cloneid = CEPH_NOSNAP)
  {
    auto oid = object_t(name);
    return hobject_t{oid,     "",
                     cloneid, static_cast<uint32_t>(std::hash<object_t>()(oid)),
                     1,       ""};
  }

  static so_builder_t
  make_head(std::string name)
  {
  ...
  static so_builder_t
  make_clone(std::string name, snapid_t cloneid = 4)
  {
  ...
  so_builder_t&
  add_attr(const std::string& name, size_t len)
  {
...
struct test_obj_t : so_builder_t {
  ...
  test_obj_t(so_builder_t _builder, std::string _desc, hobject_t _hoid) :
  ...
  static test_obj_t
  make(const std::string& desc, so_builder_t builder)
  {
    hobject_t hoid = so_get_oi(builder.so)->soid;
    return test_obj_t{std::move(builder), desc, std::move(hoid)};
  }

  template <typename... Args>
  static test_obj_t
  make_head(const std::string& desc, Args&&... args)
  {
    return make(desc, so_builder_t::make_head(std::forward<Args>(args)...));
  }

  template <typename... Args>
  static test_obj_t
  make_clone(const std::string& desc, Args&&... args)
  {
    return make(desc, so_builder_t::make_clone(std::forward<Args>(args)...));
  }
...
struct SingleErrorTestCase {
  ...
  virtual test_obj_t
  adjust_base_object(test_obj_t ret) const
  {
    return ret;
  }
...
struct SingleErrorTestCaseT : SingleErrorTestCase {
  ...
  */
  ...
  librados::err_t
  get_shard_error_sig() const final
  {
    return T::shard_error_sig;
  }

  librados::obj_err_t
  get_object_error_sig() const final
  {
  ...
  bool
  valid_for_restriction(restriction_t restriction) const final
  {
    ...
    static_assert((requires_ec() + requires_head() + requires_replica()) <= 1);
    return [] {
...
struct ECHashMismatch : SingleErrorTestCaseT<ECHashMismatch> {
  ...
  test_obj_t
  inject_error(test_obj_t obj) const
  {
...
struct DataDigestMismatch : SingleErrorTestCaseT<DataDigestMismatch> {
  ...
  test_obj_t
  adjust_base_object(test_obj_t obj) const
  {
...
struct fmt::formatter<SingleErrorTestCase> {
  ...
  auto
  format(const auto& test_case, FormatContext& ctx) const
  {
    return fmt::format_to(ctx.out(), "{}", test_case.get_description());
  }
};
...
const SingleErrorTestCase*
to_ptr(const std::unique_ptr<SingleErrorTestCase>& tc)
{
  return tc.get();
}
...
using test_case_ptr_iter_t = boost::transform_iterator<
    std::function<decltype(to_ptr)>,
    decltype(std::begin(test_cases))>;

template <SingleErrorTestCase::restriction_t restriction>
struct test_case_filter_t {
  bool
  operator()(const SingleErrorTestCase* tc) const
  {
...
using test_case_filter_iter_t =
    boost::filter_iterator<test_case_filter_t<restriction>, test_case_ptr_iter_t>;
...
struct fmt::formatter<single_error_test_param_t> {
  ...
  template <typename FormatContext>
  auto
  format(const auto& param, FormatContext& ctx) const
  {
    const auto& [obj, is_primary, test_case] = param;
    return fmt::format_to(
        ctx.out(), "{}{}{}", obj.desc, is_primary ? "Primary" : "Replica",
        test_case->get_description());
  }
};

std::ostream&
operator<<(std::ostream& out, const single_error_test_param_t& p)
{
 ...
 */
auto
compare_error_signatures(const auto& lh, const auto& rh)
{
  if (lh.errors == rh.errors) {
    return ::testing::AssertionSuccess()
           << fmt::format("Signature match: {}", lh);
  } else {
    return ::testing::AssertionFailure()
           << fmt::format("Signature mismatch: {} should be {}", lh, rh);
  }
}
...
TEST_P(TestSingleError, SingleError)
{
  ...
  crimson::osd::scrub::chunk_validation_policy_t policy{
      primary, TEST_MAX_OBJECT_SIZE, std::string{TEST_INTERNAL_NAMESPACE},
      TEST_OMAP_KEY_LIMIT, TEST_OMAP_BYTES_LIMIT};
  const pg_shard_t& target = is_primary ? primary : replica;
  const std::vector<pg_shard_t> shards = {primary, replica};
  ...
  EXPECT_EQ(
      ...
      (obj_error.has_deep_errors() || obj_error.union_shards.has_deep_errors()));
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Combine(
        ::testing::Values(
            ...
            test_obj_t::make_clone("EmptyWithAttr", "foo2")
                .add_attr("extra_attr", 64),
...
class snapset_test_case_t {
  ...
  snapset_test_case_t(uint32_t signature) :
    signature(signature)
  {}

  constexpr static uint32_t POSITION_BITS = 4;
  constexpr static uint32_t position_mask[] = {0x1, 0x2, 0x4, 0x8};
  ...
  bool
  should_inject(type_t type, unsigned position) const
  {
    ceph_assert(position < MAX_POS);
    return (signature >> (type * POSITION_BITS)) & position_mask[position];
  }

  static snapset_test_case_t
  make(type_t type, unsigned position)
  {
  ...
  static auto
  generate_single_errors(type_t type)
  {
...
public:
  ...
  bool
  should_inject_missing(unsigned position) const
  {
    return should_inject(MISSING, position);
  }

  bool
  should_inject_extra(unsigned position) const
  {
    return should_inject(EXTRA, position);
  }

  bool
  should_inject_size(unsigned position) const
  {
    return should_inject(SIZE, position);
  }

  static auto
  generate_single_missing_errors()
  {
    return generate_single_errors(MISSING);
  }

  static auto
  generate_single_extra_errors()
  {
    return generate_single_errors(EXTRA);
  }

  static auto
  generate_single_size_errors()
  {
    return generate_single_errors(SIZE);
  }

  static auto
  generate_random_errors(size_t num, int seed = 0)
  {
...
std::ostream&
operator<<(std::ostream& out, snapset_test_case_t rhs)
{
  for (auto& [s, type] :
    ...
    out << s;
    for (unsigned i = 0; i < snapset_test_case_t::MAX_POS; ++i) {
...
std::pair<hobject_t, ScrubMap::object>
make_clone(std::string name, std::pair<snapid_t, size_t> in)
{
  ScrubMap ret;
  auto [cloneid, size] = in;
  hobject_t hoid = so_builder_t::make_hoid(name, in.first);
  auto so = so_builder_t::make_clone(name, cloneid);
  so.set_size(size);
...
TEST_P(TestSnapSetCloneError, CloneError)
{
  ...
  test_clone_list_t should_exist = {{10, 32}, {25, 64}, {50, 32}, {100, 64}};
  test_clone_list_t extra = {{9, 64}, {11, 32}, {99, 64}, {101, 32}};
  ...
  for (unsigned i = 0; i < snapset_test_case_t::get_max_pos(); ++i) {
    ...
    if (!GetParam().should_inject_missing(i)) {
      ...
      if (GetParam().should_inject_size(i)) {
        ...
        to_insert.second = so_builder_t{to_insert.second}
                               .set_size(so_get_oi(to_insert.second)->size + 1)
                               .get();
      }
  ...
  ASSERT_EQ(ret.snapset_errors.size(), 1)
      << fmt::format("Got snapset_errors: {}", ret.snapset_errors);

  EXPECT_TRUE(
      compare_error_signatures(ret.snapset_errors.front(), expected_error));
}
...
TEST(TestSnapSet, MissingHead)
{
  ...
  test_clone_list_t clones = {{10, 64}, {25, 32}, {50, 64}, {100, 32}};
...
TEST(TestSnapSet, Stats)
{
  ...
  unsigned num = 0;
  auto add_simple_head = [&](size_t size, auto&& f) -> ScrubMap::object& {
  ...
  const auto ret = crimson::osd::scrub::validate_chunk(dpp, policy, maps);
  EXPECT_EQ(ret.object_errors.size(), 0);
  ASSERT_EQ(ret.snapset_errors.size(), 0)
      << fmt::format("Got snapset_errors: {}", ret.snapset_errors);
