    ...
    return ertr::parallel_for_each(
        ...
        .safe_then([sum = std::move(sum)] {
          int expected = std::accumulate(
              boost::make_counting_iterator(0),
              boost::make_counting_iterator(SIZE), 0);
...
TEST_F(errorator_test_t, non_copy_then)
{
  run_async([] {
    ...
    return create_noncopyable()
        .safe_then([](auto) { return ertr::now(); })
        .unsafe_get();
  });
}
...
TEST_F(errorator_test_t, test_futurization)
{
  run_async([] {
    ...
    return ertr::now()
        .safe_then([] { return 42; })
        .safe_then([](int life) { return ertr::make_ready_future<int>(life); })
        .unsafe_get();
  });
}
...
TEST_F(errorator_test_t, pass_further_error)
{
  ...
  run_async([&res, this] {
    return invarg_foo()
        .handle_error(ertr::pass_further{})
        .handle_error(
            crimson::ct_error::invarg::handle([&res](const auto& ec) {
              res = 1;
              return seastar::now();
