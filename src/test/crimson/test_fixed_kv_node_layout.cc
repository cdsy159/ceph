...
struct test_val_t {
  ...
  bool
  operator==(const test_val_t& rhs) const
  {
...
struct test_meta_t {
  ...
  bool
  operator==(const test_meta_t& rhs) const
  {
    return rhs.t1 == t1 && rhs.t2 == t2;
  }
  ...
  std::pair<test_meta_t, test_meta_t>
  split_into(uint32_t pivot) const
  {
    return std::make_pair(test_meta_t{t1, pivot}, test_meta_t{pivot, t2});
  }

  static test_meta_t
  merge_from(const test_meta_t& lhs, const test_meta_t& rhs)
  {
  ...
  rebalance(const test_meta_t& lhs, const test_meta_t& rhs, uint32_t pivot)
  {
    return std::make_pair(
        test_meta_t{lhs.t1, pivot}, test_meta_t{pivot, rhs.t2});
  }
};
...
void
run_balance_test(unsigned left, unsigned right)
{
  auto node = TestNode();
  auto node2 = TestNode();
  ...
  auto pivot = TestNode::balance_into_new_nodes(
      node, node2, pivot_idx, node_balanced, node_balanced2);
