...
template thread_local interrupt_cond_t<TestInterruptCondition>
  ...
  run_async([] {
    interruptor::with_interruption(
        [] {
          ...
          return interruptor::
              make_interruptible(seastar::now())
                  .then_interruptible(
                      [] {
        ...
        [] {
          ...
          return interruptor::
              make_interruptible(seastar::now())
                  ...
                  .then_interruptible([] {
                    std::cout << "test interruptible seastar future do_for_each"
                              << std::endl;
                    std::vector<int> vec = {1, 2};
                    return seastar::do_with(std::move(vec), [](auto& vec) {
                      return interruptor::do_for_each(
                  ...
                  .then_interruptible([] {
                    std::cout << "test interruptible seastar future repeat"
                              << std::endl;
                    return interruptor::repeat(
                        [] {
                          ceph_assert(interruptible::interrupt_cond<TestInterruptCondition>.interrupt_cond);
                  ...
                  .then_interruptible([] {
                    std::cout
                        << "test interruptible errorated future do_for_each"
                        << std::endl;
                    std::vector<int> vec = {1, 2};
                    return seastar::do_with(std::move(vec), [](auto& vec) {
                      using namespace std::chrono_literals;
                      return interruptor::make_interruptible(seastar::now())
                          .then_interruptible([&vec] {
                            return interruptor::do_for_each(
                                       std::begin(vec), std::end(vec),
                                       [](int) {
                                         ceph_assert(interruptible::interrupt_cond<TestInterruptCondition>.interrupt_cond);
                                         return interruptor::make_interruptible(
                                             errorator<ct_error::enoent>::
                                                 make_ready_future<>());
                                       })
                                .safe_then_interruptible(
                                    [] {
                  ...
                  .then_interruptible([] {
                    std::cout << "test errorated future do_for_each"
                              << std::endl;
                    std::vector<int> vec;
                      ...
                      return interruptor::make_interruptible(seastar::now())
                          .then_interruptible([&vec] {
                            return interruptor::do_for_each(
                                       ...
                                       [](int) {
                                         ceph_assert(interruptible::interrupt_cond<TestInterruptCondition>.interrupt_cond);
                                         return errorator<ct_error::enoent>::
                                             make_ready_future<>();
                                       })
                                .safe_then_interruptible(
                                    [] {
...
using base_iertr =
    interruptible::interruptible_errorator<TestInterruptCondition, base_ertr>;

using base2_ertr = base_ertr::extend<ct_error::input_output_error>;
using base2_iertr =
    interruptible::interruptible_errorator<TestInterruptCondition, base2_ertr>;
...
with_intr(F&& f)
{
  return base_intr::with_interruption_to_error<ct_error::eagain>(
      std::forward<F>(f), TestInterruptCondition(false));
}
...
TEST_F(seastar_test_suite_t, errorated_value)
{
  run_async([] {
    base_ertr::future<int> ret = with_intr([]() {
      return base_iertr::make_ready_future<int>(1);
    });
...
TEST_F(seastar_test_suite_t, expand_errorated_value)
{
  run_async([] {
    base2_ertr::future<> ret = with_intr([]() {
      return base_iertr::make_ready_future<int>(1).si_then([](auto) {
    ...
    interruptor::with_interruption(
	  ...
	  TestInterruptCondition>.ref_count == 1);
          ...
          ceph_assert(
              interruptible::interrupt_cond<TestInterruptCondition>.interrupt_cond);
          ceph_assert(
              interruptible::interrupt_cond<TestInterruptCondition>.ref_count ==
              1);
          return fut;
        },
        [](std::exception_ptr) {}, false)
...
TEST_F(seastar_test_suite_t, interruptible_repeat_eagain)
{
  ...
  run_async([] {
    interruptor::with_interruption(
        [] {
          return seastar::do_with(0, [](auto& i) {
...
TEST_F(seastar_test_suite_t, handle_error)
{
  run_async([] {
    base_ertr::future<> ret = with_intr([]() {
      return base2_iertr::make_ready_future<int>(1)
          .handle_error_interruptible(
              base_iertr::pass_further{}, ct_error::assert_all{"crash on eio"})
          .si_then([](auto) { return base_iertr::now(); });
    });
    ret.unsafe_get();
  });
}
