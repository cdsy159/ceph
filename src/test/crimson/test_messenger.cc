...
seastar::future<T*>
create_sharded(Args... args)
{
  // we should only construct/stop shards on #0
  return seastar::smp::submit_to(
             0,
             [=]() -> seastar::future<seastar::sharded<T>*> {
...
public:
  ...
  void
  dispatch_in_background(const char* what, Func&& f)
  {
    std::ignore =
        seastar::with_gate(container().local().gate, std::forward<Func>(f))
            .handle_exception([what](std::exception_ptr eptr) {
              try {
                std::rethrow_exception(eptr);
              } catch (std::exception& e) {
                logger().error(
                    "ShardedGates::dispatch_in_background: "
                    "{} got exxception {}",
                    what, e.what());
              }
            });
  ...
  static seastar::future<ShardedGates*>
  create()
  {
...
test_echo(unsigned rounds, double keepalive_ratio)
{
  struct test_state {
    struct Server final : public crimson::net::Dispatcher {
      ...
      ms_dispatch(crimson::net::ConnectionRef c, MessageRef m) override
      {
        if (verbose) {
          ...
          const entity_addr_t& addr)
      {
        ...
        msgr->set_default_policy(
            crimson::net::SocketPolicy::stateless_server(0));
        msgr->set_auth_client(&dummy_auth);
        msgr->set_auth_server(&dummy_auth);
        return msgr->bind(entity_addrvec_t{addr})
            .safe_then(
                [this] { return msgr->start({this}); },
    ...
    public:
      ...
      seastar::future<>
      dispatch_pingpong(const entity_addr_t& peer_addr)
      {
        assert(seastar::this_shard_id() == primary_sid);
              ...
              return container()
                  .invoke_on(
                      ...
                      [pconn = &*conn, start_time](auto& local) {
                        ...
                        logger().info(
                            "{}: handshake {}, pingpong {}", *pconn,
                            dur_handshake.count(), dur_pingpong.count());
                      })
                  .then([conn] {});
            });
      }

      static seastar::future<Client*>
      create(unsigned rounds, double keepalive_ratio, ShardedGates* gates)
      {
        return create_sharded<Client>(
            seastar::this_shard_id(), rounds, keepalive_ratio, gates);
      }

    private:
      struct PingSession
        : public seastar::enable_shared_from_this<PingSession> {
        unsigned count = 0u;
      ...
      ms_dispatch(crimson::net::ConnectionRef c, MessageRef m) override
      {
        ...
        if (session->count > rounds) {
          logger().error(
              "{}: got {} pongs, more than expected {}", *c, session->count,
              rounds);
          ceph_abort();
      ...
      PingSessionRef
      find_session(crimson::net::Connection* c)
      {
      ...
      do_dispatch_pingpong(crimson::net::ConnectionRef conn)
      {
        ...
        return seastar::do_with(
            0u, 0u, [this, conn](auto& count_ping, auto& count_keepalive) {
              return seastar::do_until(
                         [this, conn, &count_ping, &count_keepalive] {
                           bool stop = (count_ping == rounds);
                           if (stop) {
                             logger().info(
                                 "{}: finished sending {} pings with {} "
                                 "keepalives",
                         ...
                         [this, conn, &count_ping, &count_keepalive] {
                           return seastar::repeat([this, conn, &count_ping,
                                                   &count_keepalive] {
                             if (keepalive_dist(rng)) {
                               return conn->send_keepalive().then(
                                   [&count_keepalive] {
                                     count_keepalive += 1;
  ...
  logger().info(
      "test_echo(rounds={}, keepalive_ratio={}):", rounds, keepalive_ratio);
              ...
              return seastar::smp::submit_to(
                         ...
                         [] {
                           return seastar::do_for_each(
                               sharded_objects<test_state::Client>,
                               [](auto& sharded_obj) {
                                 return sharded_obj->stop();
                               });
                         })
                  .then([] { sharded_objects<test_state::Client>.clear(); });
            })
...
test_preemptive_shutdown()
{
  struct test_state {
    ...
    class Client final : public crimson::net::Dispatcher {
      ...
      std::optional<seastar::future<>>
      ms_dispatch(crimson::net::ConnectionRef, MessageRef m) override
      {
        return {seastar::now()};
      }
    ...
    public:
      ...
      void
      send_pings(const entity_addr_t& addr)
      {
  ...
  return seastar::when_all_succeed(
      ...
      .handle_exception([server, client](auto eptr) {
        logger().error(
            "test_preemptive_shutdown() failed: got exception {}", eptr);
        throw;
      });
}
...
std::ostream&
operator<<(std::ostream& out, const conn_state_t& state)
{
  switch (state) {
      ...
      T expected) const
  {
    if (actual != expected) {
      throw std::runtime_error(fmt::format(
          "[{}] {} '{}' is actually {}, not the expected '{}' {}", index, *conn,
          expr_actual, actual, expr_expected, expected));
    }
  }
...
#define ASSERT_EQUAL(actual, expected) \
  ...
  void
  assert_state_at(conn_state_t expected) const
  {
  ...
  void
  assert_connect(unsigned attempts, unsigned dispatched) const
  {
    ASSERT_EQUAL(connect_attempts, attempts);
    ASSERT_EQUAL(cnt_connect_dispatched, dispatched);
  }
  ...
  void
  assert_accept(unsigned attempts, unsigned dispatched) const
  {
    ASSERT_EQUAL(accept_attempts, attempts);
    ASSERT_EQUAL(cnt_accept_dispatched, dispatched);
  }
  ...
  void
  assert_reset(unsigned local, unsigned remote) const
  {
  ...
  dump() const
  {
    logger().info(
        ...
        static_cast<const void*>(this), index, *conn, state, connect_attempts,
        client_connect_attempts, client_reconnect_attempts,
        cnt_connect_dispatched, accept_attempts, server_connect_attempts,
        server_reconnect_attempts, cnt_accept_dispatched, cnt_reset_dispatched,
        cnt_remote_reset_dispatched);
  }
};
...
struct TestInterceptor : public Interceptor {
  ...
  void
  make_fault(Breakpoint bp, unsigned round = 1)
  {
    assert(round >= 1);
  ...
  void
  make_block(Breakpoint bp, unsigned round = 1)
  {
    assert(round >= 1);
  ...
  void
  make_stall(Breakpoint bp, unsigned round = 1)
  {
    assert(round >= 1);
...
private:
  void
  register_conn(ConnectionRef conn) override
  {
    auto result = find_result(&*conn);
    if (result != nullptr) {
      logger().error(
          "The connection [{}] {} already exists when register {}",
  ...
  void
  register_conn_closed(ConnectionRef conn) override
  {
    auto result = find_result(&*conn);
    if (result == nullptr) {
  ...
  void
  register_conn_ready(ConnectionRef conn) override
  {
    auto result = find_result(&*conn);
    if (result == nullptr) {
  ...
  void
  register_conn_replaced(ConnectionRef conn) override
  {
    auto result = find_result(&*conn);
    if (result == nullptr) {
  ...
  intercept(Connection& _conn, std::vector<Breakpoint> bps) override
  {
    ...
    return seastar::smp::submit_to(primary_sid, [conn, bps, this] {
      ...
      for (const Breakpoint& bp : bps) {
        ...
        if (result == nullptr) {
          logger().error(
              "Untracked intercepted connection: {}, at breakpoint {}({})",
        ...
        if (it_bp != breakpoints.end()) {
          ...
          if (it_cnt != it_bp->second.end()) {
            logger().info(
                "[{}] {} intercepted {}({}) => {}", result->index, *conn, bp,
...
class FailoverSuite : public Dispatcher {
  ...
  std::optional<seastar::future<>>
  ms_dispatch(ConnectionRef conn_ref, MessageRef m) override
  {
    ...
    gates.dispatch_in_background("TestSuite_ms_dispatch", [this, conn, conn_ref] {
      return seastar::
          smp::
              submit_to(primary_sid, [this, conn] {
                auto result = interceptor.find_result(&*conn);
                if (result == nullptr) {
                  logger().error(
                      "Untracked ms dispatched connection: {}", *conn);
                  ceph_abort();
                }

                if (tracked_conn != &*conn) {
                  logger().warn(
                      "[{}] {} got op, but doesn't match tracked_conn [{}] {}",
                ...
                logger().info(
                    "[Test] got op, left {} ops -- [{}] {}", pending_receive,
                    result->index, *conn);
              }).then([conn_ref] {});
    });
    return {seastar::now()};
    ...
    gates.dispatch_in_background("TestSuite_ms_dispatch", [this, conn, conn_ref] {
      return seastar::smp::submit_to(
                 ...
                 [this, conn] {
                   ...
                   if (tracked_conn && !tracked_conn->is_protocol_closed() &&
                       tracked_conn != &*conn) {
                     logger().error(
                         "[{}] {} got accepted, but there's already a valid "
                         "traced_conn [{}] {}",
                         result->index, *conn, tracked_index, *tracked_conn);
                     ceph_abort();
                   }
                   ...
                   logger().info(
                       "[Test] got accept (cnt_accept_dispatched={}), track "
                       "[{}] {}",
  ...
  void
  ms_handle_connect(ConnectionRef conn_ref, seastar::shard_id prv_shard) override
  {
    Connection* conn = &*conn_ref;
    gates.dispatch_in_background("TestSuite_ms_dispatch", [this, conn, conn_ref] {
      return seastar::smp::submit_to(
                 ...
                 [this, conn] {
                   ...
                   if (tracked_conn && !tracked_conn->is_protocol_closed() &&
                       tracked_conn != &*conn) {
                     logger().error(
                         "[{}] {} got connected, but there's already a avlid "
                         "tracked_conn [{}] {}",
                         result->index, *conn, tracked_index, *tracked_conn);
                     ceph_abort();
                   }
                   ...
                   logger().info(
                       "[Test] got connected (cnt_connect_dispatched={}) -- "
                       "[{}] {}",
  ...
  void
  ms_handle_reset(ConnectionRef conn_ref, bool is_replace) override
  {
    Connection* conn = &*conn_ref;
    gates.dispatch_in_background("TestSuite_ms_dispatch", [this, conn, conn_ref] {
      return seastar::smp::submit_to(
                 ...
                 [this, conn] {
                   ...
                   if (tracked_conn != &*conn) {
                     logger().warn(
                         "[{}] {} got reset, but doesn't match tracked_conn "
                         "[{}] {}",
                         result->index, *conn, tracked_index, *tracked_conn);
                   } else {
                     ceph_assert(result->index == tracked_index);
                   ...
                   logger().info(
                       "[Test] got reset (cnt_reset_dispatched={}), untrack "
                       "[{}] {}",
  ...
  void
  ms_handle_remote_reset(ConnectionRef conn_ref) override
  {
    Connection* conn = &*conn_ref;
    gates.dispatch_in_background("TestSuite_ms_dispatch", [this, conn, conn_ref] {
      return seastar::smp::submit_to(
                 ...
                 [this, conn] {
                   ...
                   if (result == nullptr) {
                     logger().error(
                         "Untracked remotely reset connection: {}", *conn);
                     ceph_abort();
                   }

                   if (tracked_conn != &*conn) {
                     logger().warn(
                         "[{}] {} got remotely reset, but doesn't match "
                         "tracked_conn [{}] {}",
                         result->index, *conn, tracked_index, *tracked_conn);
                   } else {
                     ceph_assert(result->index == tracked_index);
                   ...
                   logger().info(
                       "[Test] got remote reset "
                       "(cnt_remote_reset_dispatched={}) -- [{}] {}",
                       result->cnt_remote_reset_dispatched, result->index,
                       *conn);
                 })
          .then([conn_ref] {});
    });
...
private:
  seastar::future<>
  init(entity_addr_t test_addr, SocketPolicy policy)
  {
    ...
    return test_msgr->bind(entity_addrvec_t{test_addr})
        .safe_then(
            [this] { return test_msgr->start({this}); },
  ...
  seastar::future<>
  send_op(bool expect_reply = true)
  {
    ...
    hobject_t hobj(
        object_t(), oloc.key, CEPH_NOSNAP, pgid.ps(), pgid.pool(), oloc.nspace);
    spg_t spgid(pgid);
    return tracked_conn->send(
        crimson::make_message<MOSDOp>(0, 0, hobj, spgid, 0, 0, 0));
  }
  ...
  seastar::future<>
  wait_ready(unsigned num_ready_conns, unsigned num_replaced, bool wait_received)
  {
    assert(seastar::this_shard_id() == primary_sid);
    ...
    for (auto& result : interceptor.results) {
      if (result.conn->is_protocol_closed_clean()) {
        ...
        if (pending_send == 0 && pending_peer_receive == 0 &&
            pending_receive == 0) {
    ...
    if (num_ready_conns > 0) {
      if (interceptor.results.size() > num_ready_conns) {
        throw std::runtime_error(fmt::format(
          ...
          interceptor.results.size() < num_ready_conns || pending_conns > 0) {
        logger().info(
            "[Test] wait_ready(): wait for connections,"
    ...
    if (wait_received) {
      if (pending_send || pending_peer_receive || pending_receive) {
        if (pending_conns || pending_establish) {
          logger().info(
              "[Test] wait_ready(): wait for pending_send={},"
    ...
    if (num_replaced > 0) {
      if (replaced_conns > num_replaced) {
        throw std::runtime_error(fmt::format(
            "{} replaced connections, more than expected: {}", replaced_conns,
            num_replaced));
      }
      if (replaced_conns < num_replaced) {
        logger().info(
            "[Test] wait_ready(): wait for {} replaced connections,"
            " currently {} ...",
    ...
    if (do_wait) {
      return interceptor.wait().then([this, num_ready_conns, num_replaced,
                                      wait_received] {
...
public:
  ...
  notify_peer_reply()
  {
    ...
    --pending_peer_receive;
    logger().info(
        "[Test] TestPeer said got op, left {} ops", pending_peer_receive);
  ...
  post_check() const
  {
    ...
    for (auto& kv : interceptor.breakpoints) {
      ...
      if (expected > it->second.counter) {
        throw std::runtime_error(fmt::format(
            "{} only triggered {} times, not the expected {}", kv.first,
            it->second.counter, expected));
      }
    }
  }
      ...
      ShardedGates& gates)
  {
    auto suite = std::make_unique<FailoverSuite>(
        Messenger::create(
            entity_name_t::OSD(TEST_OSD), "Test", TEST_NONCE, false),
        test_peer_addr, interceptor, gates);
    return suite->init(test_addr, test_policy)
        .then([suite = std::move(suite)]() mutable { return std::move(suite); });
  }
...
class FailoverTest : public Dispatcher {
  ...
  std::optional<seastar::future<>>
  ms_dispatch(ConnectionRef c, MessageRef m) override
  {
    switch (m->get_type()) {
    case CEPH_MSG_PING:
...
private:
  ...
  prepare_cmd(
      cmd_t cmd,
      std::function<void(MCommand&)> f_prepare = [](auto& m) { return; })
  {
  ...
  {
    return prepare_cmd(cmd_t::suite_start, [peer_policy](auto& m) {
...
public:
  FailoverTest(
      ...
      entity_addr_t test_addr,
      entity_addr_t test_peer_addr) :
    cmd_msgr(cmd_msgr), test_addr(test_addr), test_peer_addr(test_peer_addr)
  {}
  ...
  shutdown()
  {
    ...
    return cmd_conn->send(std::move(m))
        .then([] { return seastar::sleep(200ms); })
        .then([this] {
          cmd_msgr->stop();
      ...
      entity_addr_t test_peer_addr)
  {
    auto test = seastar::make_lw_shared<FailoverTest>(
        Messenger::create(
            entity_name_t::OSD(CMD_CLI_OSD), "CmdCli", CMD_CLI_NONCE, true),
...
public:
      ...
      std::function<seastar::future<>(FailoverSuite&)>&& f)
  {
    ...
    return ShardedGates::create().then([this, test_policy_, peer_policy,
                                        interceptor,
  ...
  peer_connect_me()
  {
    ...
    return prepare_cmd(cmd_t::suite_connect_me, [this](auto& m) {
  ...
  markdown_peer()
  {
    ...
    // sleep to propagate potential remaining acks
    return seastar::sleep(50ms)
        .then([this] { return prepare_cmd(cmd_t::suite_markdown); })
        .then([] {
...
class FailoverSuitePeer : public Dispatcher {
  ...
  std::optional<seastar::future<>>
  ms_dispatch(ConnectionRef conn, MessageRef m) override
  {
      ...
      bool is_replace) override
  {
    ...
    if (tracked_conn && !tracked_conn->is_protocol_closed() &&
        tracked_conn != conn) {
      logger().error(
          "[TestPeer] {} got accepted, but there's already a valid traced_conn "
          "{}",
          *conn, *tracked_conn);
    }
    tracked_conn = conn;
...
private:
  seastar::future<>
  init(entity_addr_t test_peer_addr, SocketPolicy policy)
  {
    ...
    return peer_msgr->bind(entity_addrvec_t{test_peer_addr})
        .safe_then(
            [this] { return peer_msgr->start({this}); },
            Messenger::bind_ertr::assert_all_func(
  ...
  send_op()
  {
    ...
    if (tracked_conn->is_protocol_closed()) {
      logger().error(
          "[TestPeer] send op but the connection is closed -- {}",
          *tracked_conn);
    }
...
public:
  ...
  connect_peer(entity_addr_t test_addr_decoded)
  {
    ...
    if (tracked_conn) {
      if (tracked_conn->is_protocol_closed()) {
        logger().info(
            "[TestPeer] this is a new session"
            " replacing an closed one");
        ceph_assert(tracked_conn != conn);
      } else {
  ...
  create(
      entity_addr_t test_peer_addr,
      const SocketPolicy& policy,
      cb_t op_callback)
  {
    auto suite = std::make_unique<FailoverSuitePeer>(
        Messenger::create(
            entity_name_t::OSD(TEST_PEER_OSD), "TestPeer", TEST_PEER_NONCE,
            true),
        op_callback);
...
private:
  ...
  seastar::future<>
  handle_cmd(cmd_t cmd, MRef<MCommand> m_cmd)
  {
    switch (cmd) {
    ...
    default:
      logger().error(
          "TestPeer got unexpected command {} from Test", fmt::ptr(m_cmd.get()));
      ceph_abort();
      return seastar::now();
    }
  }
  ...
  init(entity_addr_t cmd_peer_addr)
  {
    ...
    return cmd_msgr->bind(entity_addrvec_t{cmd_peer_addr})
        .safe_then(
            [this] { return cmd_msgr->start({this}); },
            Messenger::bind_ertr::assert_all_func(
...
public:
  FailoverTestPeer(MessengerRef cmd_msgr, entity_addr_t test_peer_addr) :
    cmd_msgr(cmd_msgr), test_peer_addr(test_peer_addr)
  {}
  ...
  create(entity_addr_t cmd_peer_addr, entity_addr_t test_peer_addr)
  {
    auto test_peer = std::make_unique<FailoverTestPeer>(
        Messenger::create(
            entity_name_t::OSD(CMD_SRV_OSD), "CmdSrv", CMD_SRV_NONCE, true),
        test_peer_addr);
      ...
      [&test](auto& failure_cases) {
        return seastar::do_for_each(failure_cases, [&test](auto bp) {
          ...
          return test.run_suite(
              ...
              interceptor, policy_t::lossy_client, policy_t::stateless_server,
              [](FailoverSuite& suite) {
  ...
  return seastar::do_with(
      ...
      [&test](auto& failure_cases) {
        return seastar::do_for_each(failure_cases, [&test](auto bp) {
          ...
          interceptor.make_fault(bp);
          return test.run_suite(
              fmt::format("test_v2_lossy_connect_fault -- {}", bp), interceptor,
              policy_t::lossy_client, policy_t::stateless_server,
              [](FailoverSuite& suite) {
                return seastar::futurize_invoke([&suite] {
                         return suite.send_peer();
                       })
      ...
      [&test](auto& failure_cases) {
        return seastar::do_for_each(failure_cases, [&test](auto bp) {
          ...
          return test.run_suite(
              ...
              interceptor, policy_t::stateless_server, policy_t::lossy_client,
              [&test](FailoverSuite& suite) {
                return seastar::futurize_invoke([&test] {
...
test_v2_lossy_accept_fault(FailoverTest& test)
{
  ...
  interceptor.make_fault(bp);
  return test.run_suite(
      fmt::format("test_v2_lossy_accept_fault -- {}", bp), interceptor,
      policy_t::stateless_server, policy_t::lossy_client,
      [&test](FailoverSuite& suite) {
...
test_v2_lossy_establishing_fault(FailoverTest& test)
{
  ...
  interceptor.make_fault(bp);
  return test.run_suite(
      fmt::format("test_v2_lossy_establishing_fault -- {}", bp), interceptor,
      policy_t::stateless_server, policy_t::lossy_client,
      [&test](FailoverSuite& suite) {
        return seastar::futurize_invoke([&test] { return test.peer_send_me(); })
            .then([&test] { return test.peer_connect_me(); })
      ...
      [&test](auto& failure_cases) {
        return seastar::do_for_each(failure_cases, [&test](auto bp) {
          ...
          return test.run_suite(
              ...
              interceptor, policy_t::lossless_client, policy_t::stateful_server,
              [&test](FailoverSuite& suite) {
                return seastar::futurize_invoke([&test] {
                         return test.send_bidirectional();
                       })
  ...
  return seastar::do_with(
      ...
      [&test](auto& failure_cases) {
        return seastar::do_for_each(failure_cases, [&test](auto bp_pair) {
          ...
          return test.run_suite(
              fmt::format(
                  "test_v2_lossless_reconnect_fault -- {}, {}", bp_pair.first,
                  bp_pair.second),
              interceptor, policy_t::lossless_client, policy_t::stateful_server,
              [&test](FailoverSuite& suite) {
                return seastar::futurize_invoke([&test] {
...
test_v2_lossless_accept_fault(FailoverTest& test)
{
  ...
  interceptor.make_fault(bp);
  return test.run_suite(
      fmt::format("test_v2_lossless_accept_fault -- {}", bp), interceptor,
      policy_t::stateful_server, policy_t::lossless_client,
      [&test](FailoverSuite& suite) {
        return seastar::futurize_invoke([&test] {
                 return test.send_bidirectional();
               })
...
test_v2_lossless_establishing_fault(FailoverTest& test)
{
  ...
  interceptor.make_fault(bp);
  return test.run_suite(
      fmt::format("test_v2_lossless_establishing_fault -- {}", bp), interceptor,
      policy_t::stateful_server, policy_t::lossless_client,
      [&test](FailoverSuite& suite) {
        return seastar::futurize_invoke([&test] {
                 return test.send_bidirectional();
      ...
      [&test](auto& failure_cases) {
        return seastar::do_for_each(failure_cases, [&test](auto bp) {
          ...
          return test.run_suite(
              ...
              interceptor, policy_t::stateful_server, policy_t::lossless_client,
              [&test](FailoverSuite& suite) {
                return seastar::futurize_invoke([&test] {
                         return test.send_bidirectional();
                       })
  ...
  return seastar::do_with(
      ...
      [&test](auto& failure_cases) {
        return seastar::do_for_each(failure_cases, [&test](auto bp_pair) {
          ...
          return test.run_suite(
              fmt::format(
                  "test_v2_lossless_reaccept_fault -- {}, {}", bp_pair.first,
                  bp_pair.second),
              interceptor, policy_t::stateful_server, policy_t::lossless_client,
              [&test, bp = bp_pair.second](FailoverSuite& suite) {
                return seastar::futurize_invoke([&test] {
                    ...
                    .then([bp](ConnResults& results) {
                      ...
                      results[0].assert_connect(0, 0, 0, 0);
                      if (bp ==
                          Breakpoint{Tag::SESSION_RECONNECT, bp_type_t::READ}) {
                        results[0].assert_accept(1, 1, 0, 2);
                      } else {
  ...
  return seastar::do_with(
      ...
      [&test](auto& failure_cases) {
        return seastar::do_for_each(failure_cases, [&test](auto bp) {
          ...
          interceptor.make_fault(bp);
          return test.run_suite(
              fmt::format("test_v2_peer_connect_fault -- {}", bp), interceptor,
              policy_t::lossless_peer, policy_t::lossless_peer,
              [](FailoverSuite& suite) {
                return seastar::futurize_invoke([&suite] {
                         return suite.send_peer();
                       })
...
test_v2_peer_accept_fault(FailoverTest& test)
{
  ...
  interceptor.make_fault(bp);
  return test.run_suite(
      fmt::format("test_v2_peer_accept_fault -- {}", bp), interceptor,
      policy_t::lossless_peer, policy_t::lossless_peer,
      [&test](FailoverSuite& suite) {
        return seastar::futurize_invoke([&test] { return test.peer_send_me(); })
            .then([&test] { return test.peer_connect_me(); })
...
test_v2_peer_establishing_fault(FailoverTest& test)
{
  ...
  interceptor.make_fault(bp);
  return test.run_suite(
      fmt::format("test_v2_peer_establishing_fault -- {}", bp), interceptor,
      policy_t::lossless_peer, policy_t::lossless_peer,
      [&test](FailoverSuite& suite) {
        return seastar::futurize_invoke([&test] { return test.peer_send_me(); })
            .then([&test] { return test.peer_connect_me(); })
...
test_v2_peer_connected_fault_reconnect(FailoverTest& test)
{
  ...
  return test.run_suite(
      ...
      interceptor, policy_t::lossless_peer, policy_t::lossless_peer,
      [](FailoverSuite& suite) {
...
check_peer_wins(FailoverTest& test)
{
  return seastar::do_with(bool(), [&test](auto& ret) {
    return test
        .run_suite(
            "check_peer_wins", TestInterceptor(), policy_t::lossy_client,
  ...
  return seastar::do_with(
      ...
      [&test](auto& failure_cases) {
        return seastar::do_for_each(failure_cases, [&test](auto bp) {
          ...
          return test.run_suite(
              fmt::format(
                  "test_v2_racing_reconnect_acceptor_lose -- {}({})", bp.second,
                  bp.first),
              interceptor, policy_t::lossless_peer, policy_t::lossless_peer,
              [&test](FailoverSuite& suite) {
                return seastar::futurize_invoke([&test] {
  ...
  return seastar::do_with(
      ...
      [&test](auto& failure_cases) {
        return seastar::do_for_each(failure_cases, [&test](auto bp) {
          ...
          return test.run_suite(
              fmt::format(
                  "test_v2_racing_reconnect_acceptor_win -- {}({})", bp.second,
                  bp.first),
              interceptor, policy_t::lossless_peer, policy_t::lossless_peer,
              [&test](FailoverSuite& suite) {
                return seastar::futurize_invoke([&suite] {
...
test_v2_racing_connect_reconnect_lose(FailoverTest& test)
{
  ...
  return test.run_suite(
      "test_v2_racing_connect_reconnect_lose", interceptor,
      policy_t::lossless_peer, policy_t::lossless_peer,
      [&test](FailoverSuite& suite) {
        return seastar::futurize_invoke([&suite] { return suite.send_peer(); })
            .then([&suite] { return suite.connect_peer(); })
...
test_v2_racing_connect_reconnect_win(FailoverTest& test)
{
  ...
  return test.run_suite(
      "test_v2_racing_connect_reconnect_win", interceptor,
      policy_t::lossless_peer, policy_t::lossless_peer,
      [&test](FailoverSuite& suite) {
        return seastar::futurize_invoke([&test] { return test.peer_send_me(); })
            .then([&suite] { return suite.connect_peer(); })
...
test_v2_stale_connect(FailoverTest& test)
{
  ...
  return test.run_suite(
      fmt::format("test_v2_stale_connect -- {}", bp), interceptor,
      policy_t::lossless_peer, policy_t::lossless_peer,
      [&test](FailoverSuite& suite) {
        return seastar::futurize_invoke([&suite] {
                 return suite.connect_peer();
...
test_v2_stale_reconnect(FailoverTest& test)
{
  ...
  interceptor.make_stall(bp);
  return test.run_suite(
      fmt::format("test_v2_stale_reconnect -- {}", bp), interceptor,
      policy_t::lossless_peer, policy_t::lossless_peer,
      [&test](FailoverSuite& suite) {
        return seastar::futurize_invoke([&suite] { return suite.send_peer(); })
            .then([&suite] { return suite.connect_peer(); })
...
test_v2_stale_accept(FailoverTest& test)
{
  ...
  interceptor.make_stall(bp);
  return test.run_suite(
      fmt::format("test_v2_stale_accept -- {}", bp), interceptor,
      policy_t::lossless_peer, policy_t::lossless_peer,
      [&test](FailoverSuite& suite) {
        return seastar::futurize_invoke([&test] {
...
test_v2_stale_establishing(FailoverTest& test)
{
  ...
  interceptor.make_stall(bp);
  return test.run_suite(
      fmt::format("test_v2_stale_establishing -- {}", bp), interceptor,
      policy_t::lossless_peer, policy_t::lossless_peer,
      [&test](FailoverSuite& suite) {
        return seastar::futurize_invoke([&test] {
                 return test.peer_connect_me();
...
test_v2_stale_reaccept(FailoverTest& test)
{
  ...
  interceptor.make_stall(bp);
  return test.run_suite(
      fmt::format("test_v2_stale_reaccept -- {}", bp), interceptor,
      policy_t::lossless_peer, policy_t::lossless_peer,
      [&test](FailoverSuite& suite) {
        return seastar::futurize_invoke([&test] { return test.peer_send_me(); })
            .then([&test] { return test.peer_connect_me(); })
...
{
  return test.run_suite(
      "test_v2_lossy_client", TestInterceptor(), policy_t::lossy_client,
      policy_t::stateless_server, [&test](FailoverSuite& suite) {
        return seastar::futurize_invoke([&suite] {
                 logger().info("-- 0 --");
...
{
  return test.run_suite(
      "test_v2_stateless_server", TestInterceptor(), policy_t::stateless_server,
      policy_t::lossy_client, [&test](FailoverSuite& suite) {
        return seastar::futurize_invoke([&test] {
                 logger().info("-- 0 --");
                 logger().info("[Test] setup connection...");
                 return test.peer_connect_me();
...
{
  return test.run_suite(
      "test_v2_lossless_client", TestInterceptor(), policy_t::lossless_client,
      policy_t::stateful_server, [&test](FailoverSuite& suite) {
        return seastar::futurize_invoke([&suite] {
                 logger().info("-- 0 --");
                 logger().info("[Test] setup connection...");
                 return suite.connect_peer();
...
{
  return test.run_suite(
      "test_v2_stateful_server", TestInterceptor(), policy_t::stateful_server,
      policy_t::lossless_client, [&test](FailoverSuite& suite) {
        return seastar::futurize_invoke([&test] {
                 logger().info("-- 0 --");
                 logger().info("[Test] setup connection...");
                 return test.peer_connect_me();
...
{
  return test.run_suite(
      "test_v2_peer_reuse_connector", TestInterceptor(),
      policy_t::lossless_peer_reuse, policy_t::lossless_peer_reuse,
      [&test](FailoverSuite& suite) {
        return seastar::futurize_invoke([&suite] {
                 logger().info("-- 0 --");
                 logger().info("[Test] setup connection...");
...
{
  return test.run_suite(
      "test_v2_peer_reuse_acceptor", TestInterceptor(),
      policy_t::lossless_peer_reuse, policy_t::lossless_peer_reuse,
      [&test](FailoverSuite& suite) {
        return seastar::futurize_invoke([&test] {
                 logger().info("-- 0 --");
...
{
  return test.run_suite(
      "test_v2_lossless_peer_connector", TestInterceptor(),
      policy_t::lossless_peer, policy_t::lossless_peer,
      [&test](FailoverSuite& suite) {
        return seastar::futurize_invoke([&suite] {
                 logger().info("-- 0 --");
...
{
  return test.run_suite(
      "test_v2_lossless_peer_acceptor", TestInterceptor(),
      policy_t::lossless_peer, policy_t::lossless_peer,
      [&test](FailoverSuite& suite) {
        return seastar::futurize_invoke([&test] {
                 logger().info("-- 0 --");
    ...
    bool peer_wins)
{
  ...
  if (test_peer_islocal) {
    ...
    logger().info(
        "test_v2_protocol: start local TestPeer at {}...", cmd_peer_addr);
    return FailoverTestPeer::create(cmd_peer_addr, test_peer_addr)
        .then([test_addr, cmd_peer_addr, test_peer_addr, peer_wins](auto peer) {
          return test_v2_protocol(
                     test_addr, cmd_peer_addr, test_peer_addr, false, peer_wins)
...
seastar::future<int>
do_test(seastar::app_template& app)
{
  ...
  return crimson::common::sharded_conf()
      ...
      .then([&app] {
        ...
        logger().info(
            "test configuration: verbose={}, rounds={}, keepalive_ratio={}, "
            "test_addr={}, cmd_peer_addr={}, test_peer_addr={}, "
            "testpeer_islocal={}, peer_wins={}, smp={}",
            verbose, rounds, keepalive_ratio, test_addr, cmd_peer_addr,
            test_peer_addr, testpeer_islocal, peer_wins, seastar::smp::count);
        return test_echo(rounds, keepalive_ratio)
            ...
            .then([test_addr, cmd_peer_addr, test_peer_addr, testpeer_islocal,
                   peer_wins] {
              return test_v2_protocol(
                  test_addr, cmd_peer_addr, test_peer_addr, testpeer_islocal,
                  peer_wins);
            })
            .then([] {
      ...
      .then([] {
        return seastar::smp::submit_to(
                   ...
                   [] {
                     return seastar::do_for_each(
                         sharded_objects<ShardedGates>,
                         [](auto& sharded_obj) { return sharded_obj->stop(); });
                   })
            .then([] { sharded_objects<ShardedGates>.clear(); });
      })
...
main(int argc, char** argv)
{
  ...
  app.add_options()(
      "verbose,v", bpo::value<bool>()->default_value(false), "chatty if true")(
      ...
      "test-addr",
      bpo::value<std::string>()->default_value("v2:127.0.0.1:9014"),
  ...
  return app.run(argc, argv, [&app] {
    ...
    return seastar::with_timeout(
               seastar::lowres_clock::now() + 300s, do_test(app))
        .handle_exception_type([](seastar::timed_out_error&) {
          logger().error(
              "test_messenger timeout after 300s, abort! "
