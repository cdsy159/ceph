...
class FailoverSuitePeer : public Dispatcher {
  ...
  ms_fast_dispatch(Message* m) override
  {
    ...
    if (tracked_conn == nullptr) {
      ldout(cct, 0) << "[!TestPeer] got op from Test(conn " << conn
                    << "not tracked yet)" << dendl;
      tracked_conn = conn;
    } else if (tracked_conn != conn) {
      lderr(cct) << "[TestPeer] got op from Test: conn(" << conn
                 << ") != tracked_conn(" << tracked_conn << ")" << dendl;
      ceph_abort();
    } else {
  ...
  void
  ms_handle_fast_connect(Connection* conn) override
  {
  ...
  void
  ms_handle_fast_accept(Connection* conn) override
  {
    if (tracked_conn == nullptr) {
      ldout(cct, 0) << "[TestPeer] accepted: " << conn << dendl;
      tracked_conn = conn;
    } else if (tracked_conn != conn) {
      lderr(cct) << "[TestPeer] accepted: conn(" << conn << ") != tracked_conn("
                 << tracked_conn << ")" << dendl;
      ceph_abort();
    } else {
  ...
  bool
  ms_handle_reset(Connection* conn) override
  {
    if (tracked_conn == conn) {
  ...
  void
  ms_handle_remote_reset(Connection* conn) override
  {
    if (tracked_conn == conn) {
  ...
  bool
  ms_handle_refused(Connection* conn) override
  {
...
private:
  ...
  init(entity_addr_t test_peer_addr, SocketPolicy policy)
  {
    peer_msgr.reset(Messenger::create(
        cct, "async", entity_name_t::OSD(TEST_PEER_OSD), "TestPeer",
        TEST_PEER_NONCE));
  ...
  send_op()
  {
    ...
    spg_t spgid(pgid);
    tracked_conn->send_message2(
        make_message<MOSDOp>(0, 0, hobj, spgid, 0, 0, 0));
  }
  ...
  {
    if (pending_send != 0) {
      ldout(cct, 0) << "[TestPeer] flush sending " << pending_send << " ops"
                    << dendl;
    }
    ceph_assert(tracked_conn);
    while (pending_send) {
...
public:
  ...
  void
  connect_peer(entity_addr_t test_addr)
  {
    ...
    if (tracked_conn) {
      if (tracked_conn == conn.get()) {
        ldout(cct, 0) << "[TestPeer] this is not a new session " << conn.get()
                      << dendl;
      } else {
  ...
  send_peer()
  {
    if (tracked_conn) {
      ...
      ++pending_send;
      ldout(cct, 0) << "[TestPeer] send_peer() (pending " << pending_send << ")"
                    << dendl;
    }
  }
...
SocketPolicy
to_socket_policy(CephContext* cct, policy_t policy)
{
  switch (policy) {
...
class FailoverTestPeer : public Dispatcher {
  ...
  ms_dispatch(Message* m) override
  {
    auto conn = m->get_connection().get();
    if (cmd_conn == nullptr) {
      ldout(cct, 0) << "[!CmdSrv] got msg from CmdCli(conn " << conn
                    << "not tracked yet)" << dendl;
      cmd_conn = conn;
    } else if (cmd_conn != conn) {
      lderr(cct) << "[CmdSrv] got msg from CmdCli: conn(" << conn
                 << ") != cmd_conn(" << cmd_conn << ")" << dendl;
      ceph_abort();
    } else {
      // good!
    }
    ...
    default:
      lderr(cct) << "[CmdSrv] " << __func__ << " " << cmd_conn
                 << " got unexpected msg from CmdCli: " << m << dendl;
      ceph_abort();
    }
    m->put();
    return true;
  ...
  void
  ms_handle_fast_connect(Connection*) override
  {
    ceph_abort();
  }
  ...
  ms_handle_fast_accept(Connection* conn) override
  {
    if (cmd_conn == nullptr) {
      ...
      lderr(cct) << "[CmdSrv] accepted: conn(" << conn << ") != cmd_conn("
                 << cmd_conn << ")" << dendl;
      ceph_abort();
    } else {
  ...
  void
  ms_handle_remote_reset(Connection*) override
  {
    ceph_abort();
  }

  bool
  ms_handle_refused(Connection*) override
  {
    ceph_abort();
  }
...
private:
  ...
  handle_cmd(cmd_t cmd, MRef<MCommand> m_cmd)
  {
    ...
    case cmd_t::suite_start: {
      ...
      auto suite = FailoverSuitePeer::create(
          cct, test_peer_addr, policy, [this] { notify_recv_op(); });
  ...
  init(entity_addr_t cmd_peer_addr)
  {
    cmd_msgr.reset(Messenger::create(
        cct, "async", entity_name_t::OSD(CMD_SRV_OSD), "CmdSrv", CMD_SRV_NONCE));
    dummy_auth.auth_registry.refresh_config();
...
public:
  ...
  {
    auto test_peer =
        std::make_unique<FailoverTestPeer>(cct, test_peer_addr, nonstop);
...
main(int argc, char** argv)
{
  ...
  auto test_peer = FailoverTestPeer::create(
      cct.get(), cmd_peer_addr, test_peer_addr, nonstop);
  test_peer->wait();
}
