...
struct fmt::formatter<Payload> : fmt::formatter<std::string_view> {
  ...
  auto
  format(const Payload& pl, FormatContext& ctx) const
  {
...
public:
  ...
  ms_dispatch(crimson::net::ConnectionRef con, MessageRef m) final
  {
    if (verbose) {
  ...
  void
  ms_handle_remote_reset(crimson::net::ConnectionRef con) final
  {
    clear_pending(con);
  }

  std::optional<seastar::future<>>
  reply_message(const MessageRef m, crimson::net::ConnectionRef con, Payload& pl)
  {
    pl.who = Payload::PONG;
    bufferlist bl;
    encode(pl, bl);
  ...
  seastar::future<>
  send_message_wrap(crimson::net::ConnectionRef con, const bufferlist& data)
  {
  ...
  void
  clear_pending(crimson::net::ConnectionRef con)
  {
...
class SyntheticWorkload {
  ...
  get_random_connection()
  {
    return seastar::do_until(
               [this] {
                 return dispatcher.get_num_pending_msgs() <= max_in_flight;
               },
        ...
        .then([this] {
          ...
          std::map<
              crimson::net::ConnectionRef,
              std::pair<crimson::net::MessengerRef, crimson::net::MessengerRef>>::
              iterator i = available_connections.begin();
          for (; index > 0; --index, ++i)
            ;
          return seastar::make_ready_future<crimson::net::ConnectionRef>(
              i->first);
        });
  }
...
public:
  ...
  maybe_generate_connection()
  {
    ...
    {
      crimson::net::ConnectionRef conn =
          client->connect(server->get_myaddr(), entity_name_t::TYPE_OSD);
      ...
      const entity_addr_t& addr)
  {
    crimson::net::MessengerRef msgr =
        crimson::net::Messenger::create(name, lname, nonce, true);
    ...
    return get_random_connection().then([this](crimson::net::ConnectionRef conn) {
      ...
      if (!client_policy.server && client_policy.standby) {
        ...
        if (!p.first->get_default_policy().server &&
          ...
          crimson::net::ConnectionRef peer =
              p.second->connect(p.first->get_myaddr(), p.first->get_mytype());
          peer->mark_down();
  ...
  {
    logger().info(
        "available_connections: {} inflight messages: {}",
        available_connections.size(), dispatcher.get_num_pending_msgs());
  ...
  void
  handle_reset(crimson::net::ConnectionRef con)
  {
...
void
SyntheticDispatcher::ms_handle_reset(
    ...
    return seastar::do_until(
               ...
               [&test_msg] {
                 ...
                 return test_msg.init_server(
                     entity_name_t::OSD(server_num), "server", server_num,
                     bind_addr);
               })
        .then([&test_msg] {
          return seastar::do_until(
              ...
              [&test_msg] {
                return test_msg.init_client(
                    entity_name_t::CLIENT(-1), "client", get_nonce());
              });
        })
        .then([&test_msg, tp] {
