...
future<SocketRef>
socket_connect(const entity_addr_t& saddr)
{
...
future<>
test_bind_same(bool is_fixed_cpu)
{
  ...
  return ShardedServerSocket::create(is_fixed_cpu).then([is_fixed_cpu](auto pss1) {
    ...
    return pss1->listen(saddr)
        .safe_then(
              ...
              return ShardedServerSocket::create(is_fixed_cpu).then([saddr](auto pss2) {
                return pss2->listen(saddr)
                    .safe_then(
                        [] {
                          logger().error(
                              "test_bind_same() should raise address_in_use");
                          ceph_abort();
                        },
                        listen_ertr::all_same_way([](const std::error_code& e) {
                          if (e == std::errc::address_in_use) {
                            ...
                            logger().error(
                                "test_bind_same() got unexpected error {}", e);
                            ceph_abort();
                          }
            ...
            listen_ertr::assert_all_func([saddr](const std::error_code& e) {
              logger().error(
                  "test_bind_same(): there is another instance running at {}",
                  saddr);
            }))
        ...
        .safe_then(
            [pss] {
              return pss->accept([](auto socket, auto paddr) {
                logger().info(
                    "test_accept(): accepted at shard {}",
                    seastar::this_shard_id());
                // simple accept
                return seastar::sleep(100ms).then(
                    [socket = std::move(socket)]() mutable {
                      return socket->close().finally(
                          [cleanup = std::move(socket)] {});
                    });
              });
            },
            listen_ertr::assert_all_func([saddr](const std::error_code& e) {
              logger().error(
                  "test_accept(): there is another instance running at {}",
                  saddr);
            }))
        .then([saddr] {
...
public:
  ...
  static future<>
  dispatch_sockets(bool is_fixed_cpu, FuncC&& cb_client, FuncS&& cb_server)
  {
    ...
    return seastar::smp::submit_to(
                       ...
                       return pss->listen(saddr).safe_then(
                           [] {}, listen_ertr::assert_all_func(
                                      [saddr](const std::error_code& e) {
                                        logger().error(
                                            "dispatch_sockets(): there is "
                                            "another instance running at {}",
                                            saddr);
                                      }));
                     });
               })
          ...
          return seastar::when_all_succeed(
              ...
              seastar::smp::submit_to(SERVER_CPU, [psf] {
                return psf->pss->accept([psf](auto _socket, auto paddr) {
                  logger().info(
                      "dispatch_sockets(): accepted at shard {}",
                      seastar::this_shard_id());
        ...
        .then([psf, cb_client = std::move(cb_client),
          ...
          return seastar::when_all_succeed(
              ...
              seastar::smp::submit_to(
                  psf->server_socket_CPU, [socket = psf->server_socket.get(),
...
class Connection {
  ...
  void
  verify_data_read(const uint64_t read_data[])
  {
  ...
  future<>
  dispatch_write(unsigned round = 0, bool force_shut = false)
  {
    logger().debug(
        "dispatch_write(round={}, force_shut={})...", round, force_shut);
    return seastar::repeat([this, round, force_shut] {
      if (round != 0 && round <= write_count) {
        return seastar::futurize_invoke([this, force_shut] {
                 if (force_shut) {
                   logger().debug(
                       "dispatch_write() done, force shutdown output");
        ...
        bufferlist bl;
        bl.append(
            buffer::copy(reinterpret_cast<const char*>(&data), sizeof(data)));
  ...
  future<>
  dispatch_read(unsigned round = 0, bool force_shut = false)
  {
    logger().debug(
        "dispatch_read(round={}, force_shut={})...", round, force_shut);
    return seastar::repeat([this, round, force_shut] {
...
public:
  static future<>
  dispatch_rw_bounded(Socket* socket, unsigned round, bool force_shut = false)
  {
    logger().debug(
        "dispatch_rw_bounded(round={}, force_shut={})...", round, force_shut);
  ...
  static future<>
  dispatch_rw_unbounded(Socket* socket, bool preemptive_shut = false)
  {
    logger().debug(
        "dispatch_rw_unbounded(preemptive_shut={})...", preemptive_shut);
                 ...
                 seastar::futurize_invoke([&conn, preemptive_shut] {
                   if (preemptive_shut) {
                     return seastar::sleep(100ms).then([&conn] {
                       logger().debug(
                           "dispatch_rw_unbounded() shutdown socket "
                           "preemptively(100ms)");
                       conn.shutdown();
                     });
                   } else {
...
future<>
test_read_write(bool is_fixed_cpu)
{
...
future<>
test_unexpected_down(bool is_fixed_cpu)
{
  ...
  return SocketFactory::dispatch_sockets(
             ...
             [](auto cs) {
               return Connection::dispatch_rw_bounded(cs, 128, true)
                   .handle_exception_type([](const std::system_error& e) {
                     logger().error(
                         "test_unexpected_down(): client get error {}", e);
      ...
      .handle_exception([](auto eptr) {
        logger().error(
            "test_unexpected_down() got unexpeted exception {}", eptr);
        ceph_abort();
      });
}

future<>
test_shutdown_propagated(bool is_fixed_cpu)
{
  ...
  return SocketFactory::dispatch_sockets(
             ...
             [](auto cs) {
               logger().debug(
                   "test_shutdown_propagated() shutdown client socket");
               cs->shutdown();
               return seastar::now();
             },
             [](auto ss) { return Connection::dispatch_rw_unbounded(ss); })
      ...
      .handle_exception([](auto eptr) {
        logger().error(
            "test_shutdown_propagated() got unexpeted exception {}", eptr);
        ceph_abort();
      });
}

future<>
test_preemptive_down(bool is_fixed_cpu)
{
  ...
  return SocketFactory::dispatch_sockets(
      ...
      .handle_exception([](auto eptr) {
        logger().error(
            "test_preemptive_down() got unexpeted exception {}", eptr);
        ceph_abort();
      });
}

future<>
do_test_with_type(bool is_fixed_cpu)
{
