...
int
get_num_dups(const vector<int>& v)
{
  std::set<int> s;
  int dups = 0;
  for (auto n : v) {
...
std::ostream&
operator<<(std::ostream& lhs, RuleType rhs)
{
...
public:
  ...
  std::unique_ptr<CrushWrapper>
  build_indep_map(CephContext* cct, int num_rack, int num_host, int num_osd)
  {
    ...
    int rootno;
    c->add_bucket(
        0, CRUSH_BUCKET_STRAW, CRUSH_HASH_RJENKINS1, 5, 0, nullptr, nullptr,
        &rootno);
    ...
    for (int r = 0; r < num_rack; ++r) {
      ...
      for (int h = 0; h < num_host; ++h) {
        loc["host"] = string("host-") + stringify(r) + string("-") +
                      stringify(h);
...
TEST_P(IndepTest, single_out_first)
{
  ...
  for (int x = 0; x < 1000; ++x) {
    ...
    cout << "input " << x << " marked out " << out[0] << " out " << out
         << " -> out2 " << out2 << std::endl;
    ...
    for (unsigned i = 0; i < weight.size(); ++i) {
      ...
      for (unsigned j = 0; j < out.size(); ++j) {
        ...
        if (i && pos.count(out[j])) {
          ...
          if (j != pos[out[j]]) {
            cout << " " << out[j] << " moved from " << pos[out[j]] << " to "
                 << j << std::endl;
            ++moved;
          }
        }
...
public:
  ...
  std::unique_ptr<CrushWrapper>
  build_firstn_map(CephContext* cct, int num_rack, int num_host, int num_osd)
  {
    std::unique_ptr<CrushWrapper> c(new CrushWrapper);
    c->create();
    c->set_tunables_optimal();
...
TEST_F(CRUSHTest, straw_zero)
{
  ...
  EXPECT_EQ(
      0, c->add_bucket(
             0, CRUSH_BUCKET_STRAW, CRUSH_HASH_RJENKINS1, ROOT_TYPE, n, items,
             weights, &root0));
  EXPECT_EQ(0, c->set_item_name(root0, root_name0));

  string name0("rule0");
  int rule0 = c->add_simple_rule(
      name0, root_name0, "osd", "", "firstn", pg_pool_t::TYPE_REPLICATED);
  EXPECT_EQ(0, rule0);
  ...
  EXPECT_EQ(
      0, c->add_bucket(
             0, CRUSH_BUCKET_STRAW, CRUSH_HASH_RJENKINS1, ROOT_TYPE, n - 1,
             items, weights, &root1));
  EXPECT_EQ(0, c->set_item_name(root1, root_name1));

  string name1("rule1");
  int rule1 = c->add_simple_rule(
      name1, root_name1, "osd", "", "firstn", pg_pool_t::TYPE_REPLICATED);
  EXPECT_EQ(1, rule1);
...
TEST_F(CRUSHTest, straw_same)
{
  ...
  if (0) {
    crush_bucket_straw* sb0 = reinterpret_cast<crush_bucket_straw*>(
        c->get_crush_map()->buckets[-1 - root0]);
    crush_bucket_straw* sb1 = reinterpret_cast<crush_bucket_straw*>(
        c->get_crush_map()->buckets[-1 - root1]);

    for (int i = 0; i < n; ++i) {
      cout << i << "\t" << sb0->item_weights[i] << "\t" << sb1->item_weights[i]
           << "\t" << "\t" << sb0->straws[i] << "\t" << sb1->straws[i]
           << std::endl;
    }
  }
  ...
  for (int i = 0; i < n; ++i) {
    cout << i << "\t" << ((double)weights[i] / (double)weights[0]) << "\t"
         ...
         << sum1[i] << "\t" << ((double)sum1[i] / (double)sum1[0]) << std::endl;
  }
  double ratio = ((double)different / (double)max);
  cout << different << " of " << max << " = " << ratio << " different"
       << std::endl;
  ASSERT_LT(ratio, .001);
}

double
calc_straw2_stddev(int* weights, int n, bool verbose)
{
  std::unique_ptr<CrushWrapper> c(new CrushWrapper);
  ...
  string root_name0("root0");
  int root0;
  crush_bucket* b0 = crush_make_bucket(
      c->get_crush_map(), CRUSH_BUCKET_STRAW2, CRUSH_HASH_RJENKINS1, ROOT_TYPE,
      n, items, weights);
...
TEST_F(CRUSHTest, straw2_stddev)
{
  ...
  for (double step = 1.0; step < 2; step += .25) {
    ...
    cout << ((double)weights[n - 1] / (double)weights[0]) << "\t" << stddev
         << std::endl;
  }
}
...
TEST_F(CRUSHTest, straw2_reweight)
{
  ...
  // between other items.
  int weights[] = {0x10000, 0x10000, 0x20000,  0x20000, 0x30000, 0x50000,
                   0x8000,  0x20000, 0x10000,  0x10000, 0x20000, 0x10000,
                   0x10000, 0x20000, 0x300000, 0x10000, 0x20000};
  int n = 15;
  ...
  string root_name1("root1");
  int root1;
  crush_bucket* b1 = crush_make_bucket(
      c->get_crush_map(), CRUSH_BUCKET_STRAW2, CRUSH_HASH_RJENKINS1, ROOT_TYPE,
      n, items, weights);
...
struct cluster_test_spec_t {
  ...
  cluster_test_spec_t(
      int num_osds_per_host,
      int num_hosts,
      int num_hosts_mapped,
      int num_mapped_per_host,
      int num_mapped_size) :
    num_osds_per_host(num_osds_per_host),
    num_hosts(num_hosts),
  ...
  bool
  check_osd(int osd) const
  {
    return osd >= 0 && osd < num_osds;
  }

  void
  validate_host(int host) const
  {
    assert(host >= 0);
  ...
  std::pair<int, int>
  host_to_osd_range(int host) const
  {
    validate_host(host);
...
std::pair<int, std::unique_ptr<CrushWrapper>>
create_crush_heirarchy(CephContext* cct, const cluster_test_spec_t& spec)
{
  ...
  int rootno;
  c->add_bucket(
      0, CRUSH_BUCKET_STRAW2, CRUSH_HASH_RJENKINS1, ROOT_TYPE, 0, nullptr,
      nullptr, &rootno);
  c->set_item_name(rootno, "default");
...
std::vector<uint32_t>
create_weight_vector(const cluster_test_spec_t& spec)
{
  return std::vector<uint32_t>(spec.num_osds, CEPH_OSD_IN);
}

std::vector<uint32_t>
create_weight_vector_first_osd_out(
...
std::vector<uint32_t>
create_weight_vector_first_host_out(
    const cluster_test_spec_t& spec,
    const std::vector<int>& mapping)
{
  auto weights = create_weight_vector(spec);
...
{
  std::vector<int> out;
  c.do_rule(ruleno, 0 /* seed */, out, spec.num_mapped_size, weights, 0);
  EXPECT_EQ(std::size(out), spec.num_mapped_size);
  return out;
}

unsigned
count_mapped(const auto& v)
{
  unsigned ret = 0;
  for (const auto& i : v)
    ret += (i != CRUSH_ITEM_NONE);
  return ret;
}
...
TEST_F(CRUSHTest, msr_4_host_2_choose_rule)
{
  ...
  EXPECT_EQ(
      0,
      c->set_rule_step_choose_msr(ruleno, 1, spec.num_hosts_mapped, HOST_TYPE));
  EXPECT_EQ(0, c->set_rule_step_choose_msr(ruleno, 2, 1, OSD_TYPE));
...
TEST_F(CRUSHTest, msr_multi_root)
{
  ...
  auto populate_root = [&](const auto& root_name) {
    ...
    for (unsigned host_id = 0; host_id < NUM_HOSTS; ++host_id) {
      const std::string host_name = fmt::format("{}-host{}", root_name, host_id);
  ...
  auto populate_rule = [&](const auto& rule_name) {
    ...
    ceph_assert(ret == 0);
    ret =
        c.set_rule_step(ruleno, step_id++, CRUSH_RULE_CHOOSE_MSR, 2, HOST_TYPE);
    ceph_assert(ret == 0);
    ret = c.set_rule_step(ruleno, step_id++, CRUSH_RULE_CHOOSE_MSR, 2, OSD_TYPE);
    ceph_assert(ret == 0);
    ret = c.set_rule_step(ruleno, step_id++, CRUSH_RULE_EMIT, 0, 0);
    ceph_assert(ret == 0);
  };
