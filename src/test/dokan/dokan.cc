...
bool
move_eof(HANDLE handle, LARGE_INTEGER offset)
{

  // Move file pointer to FILE_BEGIN + offset
  if (!SetFilePointerEx(handle, offset, NULL, FILE_BEGIN)) {
    std::cerr << "Setting file pointer failed. err: " << GetLastError()
              << std::endl;
    return false;
  }
...
void
write_file(std::string file_path, std::string data)
{
  std::ofstream file;
  file.open(file_path);
...
void
expect_write_failure(std::string file_path)
{
  std::ofstream file;
  file.open(file_path);
...
void
check_write_file(std::string file_path, std::string data)
{
...
int
wait_for_mount(std::string mount_path)
{
...
void
map_dokan(SubProcess** mount, const char* mountpoint)
{
  SubProcess* new_mount = new SubProcess("ceph-dokan");

  new_mount->add_cmd_args(
      "map", "--win-vol-name", "TestCeph", "--win-vol-serial", TEST_VOL_SERIAL,
      "-l", mountpoint, NULL);
...
void
map_dokan_read_only(SubProcess** mount, const char* mountpoint)
{
  SubProcess* new_mount = new SubProcess("ceph-dokan");
  new_mount->add_cmd_args(
      "map", "--win-vol-name", "TestCeph", "--win-vol-serial", TEST_VOL_SERIAL,
    ...
    uint64_t max_path_len)
{
  ...
  if (expect_failure) {
    new_mount = new SubProcessTimed(
        "ceph-dokan", SubProcess::CLOSE, SubProcess::CLOSE, SubProcess::CLOSE,
  ...
  new_mount->add_cmd_args(
      "map", "--debug", "--dokan-stderr", "--win-vol-name", "TestCeph",
      "--win-vol-serial", TEST_VOL_SERIAL, "--max-path-len",
      (std::to_string(max_path_len)).c_str(), "-l", mountpoint, NULL);
...
void
unmap_dokan(SubProcess* mount, const char* mountpoint)
{
  std::string ret =
      run_cmd("ceph-dokan", "unmap", "-l", mountpoint, (char*)NULL);
...
int
get_volume_max_path(std::string mountpoint)
{
  ...
  if (GetVolumeInformation(
          mountpoint.c_str(), volume_name, sizeof(volume_name), &serial_number,
          &max_component_len, &file_system_flags, file_system_name,
          sizeof(file_system_name)) != TRUE) {
    std::cerr << "GetVolumeInformation() failed, error: " << GetLastError()
              << std::endl;
  }
...
TEST_F(DokanTests, test_mount_read_only)
{
  ...
  EXPECT_THROW(
      { fs::remove(mountpoint + success_file_path); }, fs::filesystem_error);
  unmap_dokan(mount, mountpoint.c_str());
...
TEST_F(DokanTests, test_delete_on_close)
{
  ...
  ASSERT_NE(hFile, INVALID_HANDLE_VALUE)
      << "Could not open file: " << DEFAULT_MOUNTPOINT "test_create.txt "
...
TEST_F(DokanTests, test_max_path)
{
  ...
  std::string file_path = long_dir_path + "\\" + std::string(file) + "_" +
                          get_uuid();
...
TEST_F(DokanTests, test_set_alloc_size)
{
  ...
  ASSERT_NE(
      SetFileInformationByHandle(
          hFile, FileAllocationInfo, &fai, sizeof(FILE_ALLOCATION_INFO)),
      0)
      << "Error: " << GetLastError();
...
TEST_F(DokanTests, test_volume_info)
{
  ...
  ASSERT_EQ(
      GetVolumeInformation(
          DEFAULT_MOUNTPOINT, volume_name, sizeof(volume_name), &serial_number,
          &max_component_len, &file_system_flags, file_system_name,
          sizeof(file_system_name)),
      TRUE)
      << "GetVolumeInformation() failed, error: " << GetLastError()
      << std::endl;
...
TEST_F(DokanTests, test_delete_disposition)
{
  ...
  ASSERT_NE(hFile, INVALID_HANDLE_VALUE)
      << "Could not open file: " << file_path << "err: " << GetLastError()
      << std::endl;
  ...
  ASSERT_NE(
      SetFileInformationByHandle(
          hFile, FileDispositionInfo, &fdi, sizeof(FILE_DISPOSITION_INFO)),
      0);
...
bool
check_create_disposition(std::string path, DWORD disposition)
{
  HANDLE hFile = CreateFile(
...
TEST_F(DokanTests, test_create_dispositions)
{
  ...
  std::string non_existant_file = DEFAULT_MOUNTPOINT "test_create_" +
                                  get_uuid();
...
void
test_case_insensitive(bool force_lowercase)
{
  ...
  bool found_lowercase = std::find(begin(paths), end(paths), file_name) !=
                         end(paths);
  bool found_uppercase =
      std::find(begin(paths), end(paths), to_upper(file_name)) != end(paths);
