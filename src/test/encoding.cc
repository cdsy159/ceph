...
template <typename T>
static void
test_encode_and_decode(const T& src)
{
  ...
  auto i = bl.cbegin();
  decode(dst, i);
  ASSERT_EQ(src, dst) << "Encoding roundtrip changed the string: orig=" << src
                      << ", but new=" << dst;
}
...
static void
test_encode_and_nohead_nohead(Size len, const T& src)
{
  bufferlist bl(1000000);
  encode(len, bl);
...
namespace std {
static std::ostream&
operator<<(std::ostream& oss, const multimap_t& multimap)
{
  for (multimap_t::const_iterator m = multimap.begin(); m != multimap.end();
       ++m) {
...
public:
  ...
  ConstructorCounter&
  operator=(const ConstructorCounter& rhs)
  {
    data = rhs.data;
    assigns++;
    return *this;
  ...
  static int
  get_default_ctor(void)
  {
    return default_ctor;
  }

  static int
  get_one_arg_ctor(void)
  {
    return one_arg_ctor;
  }
  ...
  bool
  operator<(const ConstructorCounter& rhs) const
  {
  ...
  friend void
  decode(ConstructorCounter& s, bufferlist::const_iterator& p)
  {
    decode(s.data, p);
  }

  friend void
  encode(const ConstructorCounter& s, bufferlist& p)
  {
    encode(s.data, p);
  }

  friend ostream&
  operator<<(ostream& oss, const ConstructorCounter& cc)
  {
    oss << cc.data;
    return oss;
  }
...
template <class T>
int ConstructorCounter<T>::default_ctor = 0;
template <class T>
int ConstructorCounter<T>::one_arg_ctor = 0;
template <class T>
int ConstructorCounter<T>::copy_ctor = 0;
template <class T>
int ConstructorCounter<T>::assigns = 0;

static std::ostream&
operator<<(std::ostream& oss, const multimap2_t& multimap)
{
  for (multimap2_t::const_iterator m = multimap.begin(); m != multimap.end();
       ++m) {
    oss << m->first << "->" << m->second << " ";
  }
  return oss;
...
template <>
void
encode<uint64_t, denc_traits<uint64_t>>(const uint64_t&, bufferlist&, uint64_t f)
{
...
template <>
void
encode<ceph_le64, denc_traits<ceph_le64>>(
    const ceph_le64&,
    bufferlist&,
    uint64_t f)
{
  ...
  const struct {
    ...
    std::string expected_what;
      ...
      {DECODE_ERR_OLDVERSION(__PRETTY_FUNCTION__, 100, 200),
       fmt::format(
           "{} no longer understands old encoding version 100 < 200: Malformed "
           "input",
           __PRETTY_FUNCTION__)},
...
TEST(small_encoding, varint)
{
  uint32_t v[][4] = {
      ...
      {31, 1, 1, 1},         {32, 1, 1, 1},         {0xff, 2, 2, 2},
      {0x100, 2, 2, 2},      {0xfff, 2, 2, 2},      {0x1000, 2, 2, 2},
      {0x2000, 2, 3, 3},     {0x3fff, 2, 3, 3},     {0x4000, 3, 3, 3},
      {0x4001, 3, 3, 3},     {0x10001, 3, 3, 3},    {0x20001, 3, 3, 3},
      {0x40001, 3, 3, 3},    {0x80001, 3, 3, 3},    {0x7f0001, 4, 4, 4},
      {0xff00001, 4, 5, 5},  {0x1ff00001, 5, 5, 5}, {0xffff0001, 5, 5, 5},
      {0xffffffff, 5, 5, 5}, {1074790401, 5, 5, 5}, {0, 0, 0, 0}};
...
TEST(small_encoding, varint_lowz)
{
  uint32_t v[][4] = {
      /* value, bytes encoded */
      {0, 1, 1, 1},          {1, 1, 1, 1},          {2, 1, 1, 1},
      {15, 1, 1, 1},         {16, 1, 1, 1},         {31, 1, 2, 2},
      {63, 2, 2, 2},         {64, 1, 1, 1},         {0xff, 2, 2, 2},
      {0x100, 1, 1, 1},      {0x7ff, 2, 2, 2},      {0xfff, 2, 3, 3},
      {0x1000, 1, 1, 1},     {0x4000, 1, 1, 1},     {0x8000, 1, 1, 1},
      {0x10000, 1, 2, 2},    {0x20000, 2, 2, 2},    {0x40000, 2, 2, 2},
      {0x80000, 2, 2, 2},    {0x7f0000, 2, 2, 2},   {0xffff0000, 4, 4, 4},
      {0xffffffff, 5, 5, 5}, {0x41000000, 3, 4, 4}, {0, 0, 0, 0}};
  for (unsigned i = 0; v[i][1]; ++i) {
    {
      bufferlist bl;
      {
