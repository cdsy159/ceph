...
#define LARGE_ENOUGH 2048
  ...
  EXPECT_EQ(
      0,
      clay.encode(set<int>(want_to_encode, want_to_encode + 4), in, &encoded));
  EXPECT_EQ(4u, encoded.size());
  unsigned length = encoded[0].length();
  EXPECT_EQ(0, memcmp(encoded[0].c_str(), in.c_str(), length));
  EXPECT_EQ(
      0, memcmp(encoded[1].c_str(), in.c_str() + length, in.length() - length));
  ...
  // all chunks are available
  {
    ...
    EXPECT_EQ(
        0,
        memcmp(decoded[1].c_str(), in.c_str() + length, in.length() - length));
  }
  ...
  for (int i = 1; i < 4; i++) {
    for (int j = 0; j < i; j++) {
      ...
      EXPECT_EQ(
          0, clay._decode(
                 set<int>(want_to_decode, want_to_decode + 2), degraded,
                 &decoded));
  ...
  for (int i = 0; i < 4; i++) {
    ...
    for (map<int, vector<pair<int, int>>>::iterator h = minimum.begin();
         h != minimum.end(); ++h) {
      for (vector<pair<int, int>>::iterator ind = h->second.begin();
           ind != h->second.end(); ++ind) {
        bufferlist temp;
        temp.substr_of(
            encoded[h->first], ind->first * sc_size, ind->second * sc_size);
...
#define LARGE_ENOUGH 2048
  ...
  EXPECT_EQ(
      0,
      clay.encode(set<int>(want_to_encode, want_to_encode + 6), in, &encoded));
  ...
  if (in.length() < length) {
    ...
    EXPECT_EQ(
        0,
        memcmp(encoded[1].c_str(), in.c_str() + length, in.length() - length));
  } else {
    ...
    EXPECT_EQ(
        0, memcmp(
               encoded[2].c_str(), in.c_str() + 2 * length,
               in.length() - 2 * length));
  }
  ...
  // all chunks are available
  {
    ...
    EXPECT_EQ(
        0, clay._decode(
               set<int>(want_to_decode, want_to_decode + 3), encoded, &decoded));
  ...
  for (int i = 2; i < 6; i++) {
    for (int j = 1; j < i; j++) {
      for (int k = 0; k < j; k++) {
        ...
        EXPECT_EQ(
            0, clay._decode(
                   set<int>(want_to_decode, want_to_decode + 3), degraded,
                   &decoded));
...
TEST(ErasureCodeClay, DISABLED_encode_decode_shortening_case)
{
  ...
  EXPECT_EQ(
      0,
      clay.encode(set<int>(want_to_encode, want_to_encode + 7), in, &encoded));
  ...
  if (in.length() < length) {
    ...
    EXPECT_EQ(
        0, memcmp(
               encoded[3].c_str(), in.c_str() + 3 * length,
               in.length() - 3 * length));
  }
  ...
  // all chunks are available
  {
    ...
    EXPECT_EQ(
        0, clay._decode(
               set<int>(want_to_decode, want_to_decode + 4), encoded, &decoded));
    EXPECT_EQ(4u, decoded.size());
    EXPECT_EQ(length, decoded[0].length());
    EXPECT_EQ(0, memcmp(decoded[0].c_str(), encoded[0].c_str(), length));
    EXPECT_EQ(0, memcmp(decoded[1].c_str(), encoded[1].c_str(), length));
...
TEST(ErasureCodeClay, minimum_to_decode)
{
  ...
  //
  {
    ...
    EXPECT_EQ(
        0, clay._minimum_to_decode(want_to_read, available_chunks, &minimum));
  ...
  //
  {
    ...
    EXPECT_EQ(
        -EIO, clay._minimum_to_decode(want_to_read, available_chunks, &minimum));
  }
  //
