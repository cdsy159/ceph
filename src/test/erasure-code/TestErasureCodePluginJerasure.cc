...
TEST(ErasureCodePlugin, factory)
{
  ...
  const char* techniques[] = {"reed_sol_van", "reed_sol_r6_op",
...
bufferptr
create_bufferptr(uint32_t value)
{
  bufferlist bl;
  bl.append_zero(4096);
...
TEST(ErasureCodePlugin, parity_delta_write)
{
  ...
  erasure_code->encode_delta(
      data[shard_id_t(3)], overwrite_bp, &delta[shard_id_t(3)]);
