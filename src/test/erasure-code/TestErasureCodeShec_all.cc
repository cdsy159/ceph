...
TEST_P(ParameterTest, parameter_all)
{
  ...
  for (int w = 1; w <= i_c; w++) {
    ...
    do {
      ...
      result = shec->_minimum_to_decode(
      ...
      minimum_chunks.clear();
        ...
        array_want_to_decode, array_want_to_decode + shec->get_chunk_count()));
  }
  ...
  result = shec->minimum_to_decode_with_cost(
      want_to_decode_with_cost, available_chunks_with_cost,
  ...
  result = shec->_decode(
      set<int>(want_to_decode2, want_to_decode2 + 2), encoded, &decoded);
  EXPECT_EQ(0, result);
...
main(int argc, char** argv)
{
  ...
  std::cout << "minimum_to_decode:cannot_recover_num = " << g_cannot_recover
            << std::endl;
  recovery_percentage = 100.0 - (float)(100.0 * g_cannot_recover /
