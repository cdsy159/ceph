...
template <size_t N>
constexpr int
getint(const std::array<int, N>& is)
{
  int a = 0;
  for (const auto i : is) {
    a |= 1 << i;
      ...
      for (unsigned avails = 0; avails < (1 << 7); ++avails) {
        ...
        if (std::popcount(avails) == 4) {
          auto a = std::to_array<std::array<int, 4>>(
              {{0, 1, 2, 3}, {0, 1, 2, 4}, {0, 1, 2, 6}, {0, 1, 3, 4},
               {0, 1, 3, 6}, {0, 1, 4, 6}, {0, 2, 3, 4}, {0, 2, 3, 5},
               {0, 2, 4, 5}, {0, 2, 4, 6}, {0, 2, 5, 6}, {0, 3, 4, 5},
               {0, 3, 4, 6}, {0, 3, 5, 6}, {0, 4, 5, 6}, {1, 2, 3, 4},
               {1, 2, 3, 5}, {1, 2, 4, 5}, {1, 2, 4, 6}, {1, 2, 5, 6},
               {1, 3, 4, 5}, {1, 3, 4, 6}, {1, 3, 5, 6}, {1, 4, 5, 6},
               {2, 3, 4, 5}, {2, 4, 5, 6}, {3, 4, 5, 6}});
          if (ranges::any_of(
                  a, [avails](int n) { return cmp_equal(avails, n); },
                  getint<4>)) {
            vec.push_back(avails);
          }
        }
...
bool
search_table_shec432(shard_id_set want_to_read, shard_id_set available_chunks)
{
  ...
  for (set<shard_id_set>::iterator itr = tmp.begin(); itr != tmp.end(); ++itr) {
    ...
    value_count = 0;
    settmp = *itr;
    for (shard_id_set::const_iterator setitr = settmp.begin();
         setitr != settmp.end(); ++setitr) {
    ...
    for (unsigned w2 = 0; w2 <= shec->get_chunk_count(); ++w2) {
      ...
      do {
        do {
          ...
          result = shec->minimum_to_decode(
              want_to_read, available_chunks, &minimum_chunks);
          int dresult = shec->decode(
              want_to_read, inchunks, &decoded,
          ...
          if (want_to_read.size() == 0) {
            ...
            if (want_to_read_without_avails.size() == 0) {
              ...
              if (search_table_shec432(
                      want_to_read_without_avails, available_chunks)) {
                EXPECT_EQ(0, result);
                EXPECT_LT(0u, minimum_chunks.size());
                ...
                for (unsigned int i = 0; i < shec->get_data_chunk_count(); ++i) {
                  if (array_want_to_read[i]) {
                    ...
                    if (cmp != 0) {
                      ++unexpected_count;
                      std::cout << "decoded[" << i
                                << "] = " << decoded[i].c_str() << std::endl;
                      std::cout << "usable = " << usable.c_str() << std::endl;
                      std::cout << "want_to_read    :" << want_to_read
                                << std::endl;
                      std::cout << "available_chunks:" << available_chunks
                                << std::endl;
                      std::cout << "minimum_chunks  :" << minimum_chunks
                                << std::endl;
                    }
                  }
                }
            ...
            array_want_to_read, array_want_to_read + shec->get_chunk_count()));
