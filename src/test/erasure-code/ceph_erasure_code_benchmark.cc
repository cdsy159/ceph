...
int
ErasureCodeBench::setup(int argc, char** argv)
{
  ...
  desc.add_options()("help,h", "produce help message")(
      ...
      "If set to 'random', pick the number of chunks to recover (as specified "
      "by "
      " --erasures) at random. If set to 'exhaustive' try all combinations of "
      "erasures "
...
ErasureCodeBench::encode()
{
  ...
  cout << (end_time - begin_time) << "\t" << (max_iterations * (in_size / 1024))
       << std::endl;
  return 0;
}

static void
display_chunks(const shard_id_map<bufferlist>& chunks, unsigned int chunk_count)
{
  cout << "chunks ";
...
int
ErasureCodeBench::decode_erasures(
    const shard_id_map<bufferlist>& all_chunks,
  ...
  if (want_erasures == 0) {
    ...
    for (shard_id_set::const_iterator chunk = want_to_read.begin();
      ...
      if (all_chunks.find(*chunk)->second.length() != decoded[*chunk].length()) {
        cerr << "chunk " << *chunk
             << " length=" << all_chunks.find(*chunk)->second.length()
             << " decoded with length=" << decoded[*chunk].length()
             << std::endl;
        return -1;
      }
...
ErasureCodeBench::decode()
{
  ...
  if (erased.size() > 0) {
    for (vector<int>::const_iterator i = erased.begin(); i != erased.end(); ++i)
  ...
  for (int i = 0; i < max_iterations; i++) {
    if (exhaustive_erasures) {
      code = decode_erasures(
          encoded, encoded, shard_id_t(0), erasures, erasure_code);
      if (code)
        return code;
