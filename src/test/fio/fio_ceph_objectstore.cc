...
struct Options {
  ...
  unsigned long long cycle_throttle_period, oi_attr_len_low, oi_attr_len_high,
      snapset_attr_len_low, snapset_attr_len_high, pglog_omap_len_low,
      pglog_omap_len_high, pglog_dup_omap_len_low, pglog_dup_omap_len_high,
...
static std::vector<fio_option> ceph_options{
    ...
    make_option([](fio_option& o) {
      ...
      o.type = FIO_OPT_STR_VAL;
      o.help =
          "set to non-zero value to periodically cycle through throttle "
          "options";
...
struct Collection {
  ...
  Collection(const spg_t& pg, ObjectStore::CollectionHandle _ch) :
    pg(pg), cid(pg), ch(_ch), lock(new std::mutex)
  {}
};
    ...
    uint64_t count)
{
...

  {
    ...
    if (!exists) {
      ...
      if (r < 0) {
        derr << "Failure to write OSD superblock: " << cpp_strerror(-r)
             << dendl;
        return r;
      }
    }
  ...
  for (uint32_t i = 0; i < count; i++) {
    ...
    bool exists = os->collection_exists(cid);
    auto ch = exists ? os->open_collection(cid)
      ...
      if (perf_output_file) {
        try {
          ...
          std::cerr << "Unable to write formatted output to "
                    << *perf_output_file << ", exception: " << e.what()
                    << std::endl;
        }
      }
    }
          ...
          : std::nullopt)
{
  ...
  os = ObjectStore::create(
      g_ceph_context, g_conf().get_val<std::string>("osd objectstore"),
  ...
  o->snapset_attr_len_high =
      max(o->snapset_attr_len_low, o->snapset_attr_len_high);
  o->pglog_omap_len_high = max(o->pglog_omap_len_low, o->pglog_omap_len_high);
  o->pglog_dup_omap_len_high =
      max(o->pglog_dup_omap_len_low, o->pglog_dup_omap_len_high);
...
Engine::~Engine() { ceph_assert(!ref_count); }
...
struct Job {
  ...
  std::vector<Collection>
      collections; //< job's private collections to spread objects over
  ...
  parse_throttle_str(const char* p)
  {
    vector<unsigned> ret;
    if (p == nullptr) {
      return ret;
...
Job::Job(Engine* engine, const thread_data* td) :
  ...
  deferred_throttle_values(parse_throttle_str(
      static_cast<Options*>(td->eo)->deferred_throttle_values)),
  cycle_throttle_period(static_cast<Options*>(td->eo)->cycle_throttle_period)
{
  engine->ref();
  auto o = static_cast<Options*>(td->eo);
  unsigned long long max_data =
      max(o->oi_attr_len_high, o->snapset_attr_len_high);
    ...
    if (o->check_files) {
      ...
      if (r || ((unsigned)st.st_size) != file_size) {
        derr << "Problem checking " << oid << ", r=" << r
             << ", st.st_size=" << st.st_size << ", file_size=" << file_size
...
Job::check_throttle()
{
  ...
  if (ceph::mono_clock::is_zero(last) ||
    ...
    unsigned dtvals = deferred_throttle_values.size()
                          ? deferred_throttle_values.size()
                          : 1;
    if (!throttle_values.empty()) {
      ...
      int r =
          engine->cct->_conf.set_val("bluestore_throttle_bytes", val, nullptr);
      ceph_assert(r == 0);
    }
    if (!deferred_throttle_values.empty()) {
      std::string val =
          std::to_string(deferred_throttle_values[(index / tvals) % dtvals]);
      std::cerr << "Setting bluestore_deferred_throttle_bytes to " << val
                << std::endl;
      int r = engine->cct->_conf.set_val(
          "bluestore_throttle_deferred_bytes", val, nullptr);
      ceph_assert(r == 0);
    }
...
int
fio_ceph_os_setup(thread_data* td)
{
...
void
fio_ceph_os_cleanup(thread_data* td)
{
...
io_u*
fio_ceph_os_event(thread_data* td, int event)
{
...
enum fio_q_status
fio_ceph_os_queue(thread_data* td, io_u* u)
{
  fio_ro_check(td, u);
  ...
  if (u->ddir == DDIR_WRITE) {
    ...
    bufferlist bl;
    bl.push_back(
        buffer::copy(reinterpret_cast<char*>(u->xfer_buf), u->xfer_buflen));
    ...
    if (o->oi_attr_len_high) {
      ...
      job->one_for_all_data.set_length(ceph::util::generate_random_number(
    ...
    if (o->simulate_pglog) {
      ...
      uint64_t pglog_ver_cnt = 0;
      {
        ...
        if (o->pglog_omap_len_high &&
            pglog_ver_cnt >= coll.pglog_ver_tail +
                                 g_conf()->osd_min_pg_log_entries +
                                 g_conf()->osd_pg_log_trim_min) {
          pglog_trim_tail = coll.pglog_ver_tail;
          coll.pglog_ver_tail = pglog_trim_head = pglog_trim_tail +
                                                  g_conf()->osd_pg_log_trim_min;

          if (o->pglog_dup_omap_len_high &&
              pglog_ver_cnt >= coll.pglog_dup_ver_tail +
      ...
      if (o->pglog_omap_len_high) {
        ...
        snprintf(
            ver_key, sizeof(ver_key), "0000000011.%020llu",
            (unsigned long long)pglog_ver_cnt);
        // fill with the garbage as we do not care of the actual content...
        job->one_for_all_data.set_length(ceph::util::generate_random_number(
      ...
      if (o->pglog_dup_omap_len_high) {
        ...
        for (auto i = pglog_trim_tail; i < pglog_trim_head; ++i) {
          snprintf(
              ver_key, sizeof(ver_key), "dup_0000000011.%020llu",
    ...
    for (auto i = pglog_trim_tail; i < pglog_trim_head; ++i) {
      snprintf(
          ver_key, sizeof(ver_key), "0000000011.%020llu", (unsigned long long)i);
...
int
fio_ceph_os_commit(thread_data* td)
{
...
int
fio_ceph_os_open(thread_data* td, fio_file* f)
{
  return 0;
}

int
fio_ceph_os_close(thread_data* td, fio_file* f)
{
  return 0;
}

int
fio_ceph_os_io_u_init(thread_data* td, io_u* u)
{
...
void
fio_ceph_os_io_u_free(thread_data* td, io_u* u)
{
  u->engine_data = nullptr;
}
...
void
get_ioengine(struct ioengine_ops** ioengine_ptr)
{
