...
struct librgw_data {
  ...
  void
  save_handle(rgw_file_handle* fh)
  {
    fh_vec.push_back(fh);
  }
...
struct save_args {
  ...
  void
  push_arg(const std::string sarg)
  {
 ...
 */
static int
fio_librgw_setup(struct thread_data* td)
{
  ...
  if (!td->io_ops_data) {
    ...
    r = rgw_mount2(
        data->rgw_h, o.userid, o.access_key, o.secret_key, "/", &data->fs,
        RGW_MOUNT_FLAG_NONE);
    if (!!r) {
    ...
    r = rgw_lookup(
        data->fs, data->fs->root_fh, o.bucket_name, &data->bucket_fh, nullptr,
        0, RGW_LOOKUP_FLAG_NONE);
    if (!data->bucket_fh) {
      dprint(
          FD_IO, "rgw_lookup on bucket %s failed, will create\n", o.bucket_name);
      ...
      r = rgw_mkdir(
          data->fs, data->fs->root_fh, o.bucket_name, &st, create_mask,
          &data->bucket_fh, RGW_MKDIR_FLAG_NONE);
      if (!data->bucket_fh) {
 ...
 */
static int
fio_librgw_init(struct thread_data* td)
{
 ...
 */
static void
fio_librgw_cleanup(struct thread_data* td)
{
 ...
 */
static int
fio_librgw_prep(struct thread_data* td, struct io_u* io_u)
{
  return 0;
}
 ...
 */
static struct io_u*
fio_librgw_event(struct thread_data* td, int event)
{
  return NULL;
}
 ...
 */
static int
fio_librgw_cancel(struct thread_data* td, struct io_u* io_u)
{
  return 0;
}
...
fio_librgw_queue(struct thread_data* td, struct io_u* io_u)
{
  librgw_data* data = static_cast<librgw_data*>(td->io_ops_data);
  ...
  if (io_u->ddir == DDIR_WRITE) {
    /* Do full write cycle */
    r = rgw_lookup(
        data->fs, data->bucket_fh, object, &object_fh, nullptr, 0,
    ...
    r = rgw_write(
        data->fs, object_fh, 0, io_u->xfer_buflen, &nbytes,
    ...
    r = rgw_read(
        data->fs, object_fh, io_u->offset, io_u->xfer_buflen, &nbytes,
        io_u->xfer_buf, RGW_READ_FLAG_NONE);
    if (!!r) {
...
int
fio_librgw_commit(thread_data* td)
{
 ...
 */
static int
fio_librgw_open(struct thread_data* td, struct fio_file* f)
{
 ...
 */
static int
fio_librgw_close(struct thread_data* td, struct fio_file* f)
{
  /* for now, let's try to avoid doing open/close in these hooks */
  return 0;
}
...
int
fio_librgw_io_u_init(thread_data* td, io_u* u)
{
...
void
fio_librgw_io_u_free(thread_data* td, io_u* u)
{
  u->engine_data = nullptr;
}
