...
public:
  ...
  CacheClient*
  create_session(uint64_t random_index)
  {
  ...
  void
  server_handle_request(CacheSession* session_id, ObjectCacheRequest* req)
  {
    ...
    case RBDSC_READ: {
      ObjectCacheRequest* reply =
          new ObjectCacheReadReplyData(RBDSC_READ_REPLY, req->seq);
      session_id->send(reply);
      break;
    }
    }
  ...
  void
  test_register_client(uint64_t random_index)
  {
      ...
      bool is_last)
  {
    ...
    for (uint64_t i = 0; i < request_num; i++) {
      ...
      m_cache_client_vec[index]->lookup_object(
          pool_nspace, 1, 2, 3, "1234", std::move(ctx));
    }
...
TEST_F(TestMultiSession, test_multi_session)
{
  ...
  for (uint64_t i = 0; i <= test_times; i++) {
    ...
    if (m_cache_client_vec[random_index] == nullptr) {
      ...
      test_lookup_object(
          string("test_nspace") + std::to_string(random_index), random_index, 4,
