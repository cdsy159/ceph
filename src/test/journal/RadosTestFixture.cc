...
void
RadosTestFixture::SetUpTestCase()
{
  ...
  _thread_pool =
      new ThreadPool(cct, "RadosTestFixture::_thread_pool", "tp_test", 1);
  _thread_pool->start();
}

void
RadosTestFixture::TearDownTestCase()
{
  _thread_pool->stop();
  delete _thread_pool;
...
RadosTestFixture::SetUp()
{
  ...
  m_work_queue = new ContextWQ(
      "RadosTestFixture::m_work_queue", ceph::make_timespan(60), _thread_pool);
...
RadosTestFixture::create_metadata(
    const std::string& oid,
...
int
RadosTestFixture::append(const std::string& oid, const bufferlist& bl)
{
...
int
RadosTestFixture::client_register(
    const std::string& oid,
    const std::string& id,
...
int
RadosTestFixture::client_commit(
    const std::string& oid,
    const std::string& id,
...
bufferlist
RadosTestFixture::create_payload(const std::string& payload)
{
  bufferlist bl;
  bl.append(payload);
  return bl;
...
int
RadosTestFixture::init_metadata(
...
bool
RadosTestFixture::wait_for_update(
    const ceph::ref_t<journal::JournalMetadata>& metadata)
{
