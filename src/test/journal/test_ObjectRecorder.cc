...
public:
  ...
  struct Handler : public journal::ObjectRecorder::Handler {
    ...
    void
    closed(journal::ObjectRecorder* object_recorder) override
    {
    ...
    void
    overflow(journal::ObjectRecorder* object_recorder) override
    {
      std::lock_guard locker{lock};
  ...
  public:
    ObjectRecorderFlusher(librados::IoCtx& ioctx, ContextWQ* work_queue) :
      m_ioctx{ioctx}, m_work_queue{work_queue}
    {}
    ...
    ObjectRecorderFlusher(
      ...
      m_max_in_flight_appends{
          max_in_flight < 0 ? std::numeric_limits<uint64_t>::max()
    ...
    auto
    create_object(std::string_view oid, uint8_t order, ceph::mutex* lock)
    {
      ...
      {
        std::lock_guard locker{*lock};
        object->set_append_batch_options(
    ...
    wait_for_closed()
    {
      ...
      return m_handler.cond.wait_for(locker, 10s, [this] {
    ...
    {
      std::unique_lock locker{m_handler.lock};
      if (m_handler.cond.wait_for(locker, 10s, [this] {
  ...
  journal::AppendBuffer
  create_append_buffer(
      uint64_t tag_tid,
      uint64_t entry_tid,
...
TEST_F(TestObjectRecorder, Append)
{
  ...
  journal::AppendBuffer append_buffer1 =
      create_append_buffer(234, 123, "payload");
  journal::AppendBuffers append_buffers;
  ...
  journal::AppendBuffer append_buffer2 =
      create_append_buffer(234, 124, "payload");
...
TEST_F(TestObjectRecorder, FlushFuture)
{
  ...
  journal::AppendBuffer append_buffer =
      create_append_buffer(234, 123, "payload");
  journal::AppendBuffers append_buffers;
  ...
  ASSERT_TRUE(
      append_buffer.first->is_flush_in_progress() ||
