...
abstime(struct timespec& ts, long ms)
{
...
TEST(LibCephFS, BasicRecordLocking)
{
  ...
  rc = ceph_ll_create(
      cmount, root, c_file, fileMode, O_RDWR | O_CREAT, &inode, &fh, &stx, 0, 0,
      perms);
  ASSERT_EQ(rc, 0);
...
// Used by ConcurrentLocking test
static void
thread_ConcurrentRecordLocking(str_ConcurrentRecordLocking& s)
{
  ...
  // Get the inode and Fh corresponding to c_file
  rc = ceph_ll_create(
      cmount, root, s.file, fileMode, O_RDWR | O_CREAT, &inode, &fh, &stx, 0, 0,
  ...
  lock1.l_pid = getpid();
  ASSERT_EQ(
      -EAGAIN, ceph_ll_setlk(cmount, fh, &lock1, ceph_pthread_self(), false));
...
static void*
thread_ConcurrentRecordLocking_(void* arg)
{
...
TEST(LibCephFS, ConcurrentRecordLocking)
{
  ...
  s.sem_init(0);
  ASSERT_EQ(
      0, pthread_create(&thread, NULL, thread_ConcurrentRecordLocking_, &s));
...
TEST(LibCephFS, ThreesomeRecordLocking)
{
  ...
  s.sem_init(0);
  ASSERT_EQ(
      0, pthread_create(&thread[0], NULL, thread_ConcurrentRecordLocking_, &s));
  ASSERT_EQ(
      0, pthread_create(&thread[1], NULL, thread_ConcurrentRecordLocking_, &s));
  // Synchronization point with thread (failure: thread is dead)
  ...
  TWICE(WAIT_WORKER(6); // (6)
        ...
        // We no longer have the lock
        lock1.l_type = F_WRLCK;
        lock1.l_whence = SEEK_SET; lock1.l_start = 0; lock1.l_len = 1024;
        lock1.l_pid = getpid(); ASSERT_EQ(
            -EAGAIN,
            ceph_ll_setlk(cmount, fh, &lock1, ceph_pthread_self(), false));
        lock1.l_type = F_RDLCK; lock1.l_whence = SEEK_SET; lock1.l_start = 0;
        lock1.l_len = 1024; lock1.l_pid = getpid(); ASSERT_EQ(
            -EAGAIN,
            ceph_ll_setlk(cmount, fh, &lock1, ceph_pthread_self(), false));
...
// Used by ConcurrentLocking test
static void
process_ConcurrentRecordLocking(str_ConcurrentRecordLocking& s)
{
  const pid_t mypid = getpid();
  PROCESS_SLOW_MS();
...
TEST(LibCephFS, DISABLED_InterProcessRecordLocking)
{
  ...
  str_ConcurrentRecordLocking* const shs =
      reinterpret_cast<str_ConcurrentRecordLocking*>(mmap(
