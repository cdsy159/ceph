...
TEST(LibCephFS, LayoutVerifyDefaultLayout)
{
...

  {
    ...
    r = ceph_getxattr(
        cmount, "/", "ceph.dir.layout.json", (void*)value, sizeof(value));
    ASSERT_GT(r, 0);
...
TEST(LibCephFS, LayoutSetAndVerifyNewAndInheritedLayout)
{
...

  {
    ...
    ASSERT_EQ(
        0, ceph_setxattr(
               cmount, "test/d0", "ceph.dir.layout.json",
               (void*)new_layout.c_str(), new_layout.length(), XATTR_CREATE));
  }

  {
    ...
    r = ceph_getxattr(
        cmount, "test/d0", "ceph.dir.layout.json", (void*)value, sizeof(value));
    ASSERT_GT(r, 0);
    ASSERT_LT(r, sizeof value);
    std::clog << "layout:" << value << std::endl;
...

  {
    ...
    r = ceph_getxattr(
        cmount, "test/d0/subdir", "ceph.dir.layout.json", (void*)value,
        sizeof(value));
    ASSERT_GT(r, 0);
    ASSERT_LT(r, sizeof value);
    std::clog << "layout:" << value << std::endl;
...
TEST(LibCephFS, LayoutSetBadJSON)
{
...

  {
    ...
    const char* new_layout =
        "" // bad json without starting brace
        "\"stripe_unit\": 65536, "
        "\"stripe_count\": 1, "
        "\"object_size\": 65536, "
    ...
    ASSERT_EQ(
        -EINVAL, ceph_setxattr(
                     cmount, "test/d0", "ceph.dir.layout.json",
                     (void*)new_layout, strlen(new_layout), XATTR_CREATE));
  }
...
TEST(LibCephFS, LayoutSetBadPoolName)
{
...

  {
    ...
    ASSERT_EQ(
        -EINVAL, ceph_setxattr(
                     cmount, "test/d0", "ceph.dir.layout.pool_name",
                     (void*)"UglyPoolName", 12, XATTR_CREATE));
  }
...
TEST(LibCephFS, LayoutSetBadPoolId)
{
...

  {
    ...
    ASSERT_EQ(
        -EINVAL, ceph_setxattr(
                     cmount, "test/d0", "ceph.dir.layout.pool_id", (void*)"300",
                     3, XATTR_CREATE));
  }
...
TEST(LibCephFS, LayoutSetInvalidFieldName)
{
...

  {
    ...
    ASSERT_EQ(
        -ENODATA, ceph_setxattr(
                      cmount, "test/d0", "ceph.dir.layout.bad_field",
                      (void*)"300", 3, XATTR_CREATE));
  }
...
TEST(LibCephFS, GetAndSetDirPin)
{
...

  {
    char value[1024] = "";
    int r = ceph_getxattr(
        cmount, "test/d1", "ceph.dir.pin", (void*)value, sizeof(value));
    ASSERT_GT(r, 0);
    ASSERT_LT(r, sizeof value);
...

  {
    ...
    ASSERT_EQ(
        0, ceph_setxattr(
               cmount, "test/d1", "ceph.dir.pin", (void*)"1", 1, XATTR_CREATE));
...
TEST(LibCephFS, GetAndSetDirDistribution)
{
...

  {
    char value[1024] = "";
    int r = ceph_getxattr(
        cmount, "test/d2", "ceph.dir.pin.distributed", (void*)value,
        sizeof(value));
    ASSERT_GT(r, 0);
    ASSERT_LT(r, sizeof value);
...

  {
    ...
    ASSERT_EQ(
        0, ceph_setxattr(
               cmount, "test/d2", "ceph.dir.pin.distributed", (void*)"1", 1,
               XATTR_CREATE));
...
TEST(LibCephFS, GetAndSetDirRandom)
{
...

  {
    char value[1024] = "";
    int r = ceph_getxattr(
        cmount, "test/d3", "ceph.dir.pin.random", (void*)value, sizeof(value));
    ASSERT_GT(r, 0);
    ASSERT_LT(r, sizeof value);
    ASSERT_STREQ("0", value);
  }

  {
    ...
    ASSERT_EQ(
        0, ceph_setxattr(
               cmount, "test/d3", "ceph.dir.pin.random",
               (void*)ss.str().c_str(), strlen(ss.str().c_str()), XATTR_CREATE));
...
TEST(LibCephFS, FsCrypt)
{
  ...
  ASSERT_EQ(
      0,
      ceph_fsetxattr(cmount, fd, "ceph.fscrypt.auth", "foo", 3, XATTR_CREATE));
  ASSERT_EQ(
      0,
      ceph_fsetxattr(
          cmount, fd, "ceph.fscrypt.file", &size, sizeof(size), XATTR_CREATE));
...
TEST(LibCephFS, Removexattr)
{
  ...
  ASSERT_EQ(
      0,
      ceph_fsetxattr(cmount, fd, "user.remove.xattr", "foo", 3, XATTR_CREATE));
  ASSERT_EQ(0, ceph_fremovexattr(cmount, fd, "user.remove.xattr"));

  // remove xattr via setxattr & XATTR_REPLACE
  ASSERT_EQ(
      -ENODATA, ceph_fsetxattr(
                    cmount, fd, "user.remove.xattr", nullptr, 0, XATTR_REPLACE));
  ASSERT_EQ(
      0,
      ceph_fsetxattr(cmount, fd, "user.remove.xattr", "foo", 3, XATTR_CREATE));
  ASSERT_EQ(
...
TEST(LibCephFS, MksnapSubvolumeSnapshotVisibility)
{
  ...
  const char* SNAPSHOT_VISIBILITY_VXATTR = "ceph.dir.subvolume.snaps.visible";
  ...
  ASSERT_EQ(
      0, ceph_setxattr(
             cmount, subvol_path, "ceph.dir.subvolume", (void*)"1", 1,
             XATTR_CREATE));
  ...
  ASSERT_EQ(
      0, ceph_setxattr(
             cmount, subvol_path, SNAPSHOT_VISIBILITY_VXATTR, (void*)"0", 1,
             XATTR_CREATE));
  ...
  ASSERT_EQ(
      0, ceph_setxattr(
             cmount, subvol_path, SNAPSHOT_VISIBILITY_VXATTR, (void*)"1", 1,
             XATTR_CREATE));
