...
TEST(LibRadosServicePP, RegisterEarly)
{
  ...
  ASSERT_EQ(
      0, cluster.service_daemon_register(
             "laundry", name, {{"foo", "bar"}, {"this", "that"}}));
  ASSERT_EQ(
      -EEXIST, cluster.service_daemon_register(
                   "laundry", name, {{"foo", "bar"}, {"this", "that"}}));
...
TEST(LibRadosServicePP, Status)
{
  ...
  ASSERT_EQ(
      -ENOTCONN,
      cluster.service_daemon_update_status({{"testing", "starting"}}));
  ASSERT_EQ(0, cluster.connect());
  ASSERT_EQ(
      0, cluster.service_daemon_register(
             "laundry", name, {{"foo", "bar"}, {"this", "that"}}));
  for (int i = 0; i < 20; ++i) {
    ASSERT_EQ(
        0, cluster.service_daemon_update_status(
               {{"testing", "running"}, {"count", stringify(i)}}));
    sleep(1);
  }
  cluster.shutdown();
}
    ...
    // make sure we are deregistered
    {
      ...
      ASSERT_EQ(
          0, cluster.mon_command(
                 "{\"prefix\": \"service dump\"}", {}, &outbl, NULL));
