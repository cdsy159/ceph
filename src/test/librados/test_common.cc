...
int
wait_for_healthy(rados_t* cluster)
{
  bool healthy = false;
  ...
  while (!healthy) {
    ...
    int ret = rados_mon_command(
        *cluster, (const char**)cmd, 1, "", 0, &outbuf, &outlen, NULL, NULL);
    if (ret) {
      return ret;
    }
    ...
    const std::string& val)
{
  ...
  cmd[0] = serialized_cmd.c_str();
  int ret = rados_mon_command(
      *cluster, (const char**)cmd, 1, "", 0, NULL, NULL, NULL, NULL);
  return ret;
}
...
std::string
with_healthy_cluster(rados_t* cluster, const std::string& pool_name, Func&& func)
{
  try {
...
std::string
set_pg_num(rados_t* cluster, const std::string& pool_name, uint32_t pg_num)
{
  return with_healthy_cluster(cluster, pool_name, [&] {
    // Adjust pg_num
    if (int r = rados_pool_set(cluster, pool_name, "pg_num", stringify(pg_num));
        r != 0) {
...
std::string
set_pgp_num(rados_t* cluster, const std::string& pool_name, uint32_t pgp_num)
{
  return with_healthy_cluster(cluster, pool_name, [&] {
    // Adjust pgp_num
    if (int r =
            rados_pool_set(cluster, pool_name, "pgp_num", stringify(pgp_num));
        r != 0) {
