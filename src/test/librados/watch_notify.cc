...
static void
watch_notify_test_cb(uint8_t opcode, uint64_t ver, void* arg)
{
...
protected:
  ...
  static void watch_notify2_test_cb(
      void* arg,
      uint64_t notify_id,
      uint64_t cookie,
  ...
  static void watch_notify2_test_errcb_reconnect(
      void* arg,
      uint64_t cookie,
      int err);
  static void watch_notify2_test_errcb_aio_reconnect(
      void* arg,
      uint64_t cookie,
      int err);
};
...
void
LibRadosWatchNotify::watch_notify2_test_cb(
    void* arg,
    uint64_t notify_id,
    uint64_t cookie,
    uint64_t notifier_gid,
    ...
    size_t data_len)
{
  std::cout << __func__ << " from " << notifier_gid << " notify_id "
  ...
  thiz->notify_err = 0;
  rados_notify_ack(
      thiz->notify_io, thiz->notify_oid, notify_id, cookie, "reply", 5);
}

void
LibRadosWatchNotify::watch_notify2_test_errcb(void* arg, uint64_t cookie, int err)
{
...
void
LibRadosWatchNotify::watch_notify2_test_errcb_reconnect(
    void* arg,
    uint64_t cookie,
    int err)
{
  ...
  thiz->notify_err = rados_watch2(
      thiz->ioctx, thiz->notify_oid, &cookie, watch_notify2_test_cb,
      watch_notify2_test_errcb_reconnect, thiz);
  if (thiz->notify_err < 0) {
    std::cout << __func__ << " reconnect watch failed with error "
              << thiz->notify_err << std::endl;
    return;
  }
  return;
}
...
void
LibRadosWatchNotify::watch_notify2_test_errcb_aio_reconnect(
    void* arg,
    uint64_t cookie,
    int err)
{
  ...
  ASSERT_EQ(
      0, rados_aio_create_completion2(nullptr, nullptr, &thiz->notify_comp));
  thiz->notify_cookies.erase(cookie); //delete old cookie
  thiz->notify_err = rados_aio_watch(
      thiz->ioctx, thiz->notify_oid, thiz->notify_comp, &cookie,
...
TEST_F(LibRadosWatchNotify, WatchNotify)
{
  ...
  uint64_t handle;
  ASSERT_EQ(
      0, rados_watch(ioctx, "foo", 0, &handle, watch_notify_test_cb, NULL));
...
TEST_F(LibRadosWatchNotify, Watch2Delete)
{
  ...
  ASSERT_EQ(
      0, rados_watch2(
             ioctx, notify_oid, &handle, watch_notify2_test_cb,
  ...
  EXPECT_TRUE(
      rados_watch_check_err == -ENOTCONN || rados_watch_check_err == -ENOENT)
...
TEST_F(LibRadosWatchNotify, AioWatchDelete)
{
  ...
  rados_aio_watch(
      ioctx, notify_oid, comp, &handle, watch_notify2_test_cb,
      watch_notify2_test_errcb, this);
...
TEST_F(LibRadosWatchNotify, WatchNotify2)
{
  ...
  ASSERT_EQ(
      0,
      rados_notify2(
          ioctx, notify_oid, "notify", 6, 300000, &reply_buf, &reply_buf_len));
  bufferlist reply;
  ...
  // should get zeroed.
  ASSERT_EQ(
      -ENOENT, rados_notify2(
                   ioctx, "doesnotexist", "notify", 6, 300000, &reply_buf,
                   &reply_buf_len));
...
TEST_F(LibRadosWatchNotify, AioWatchNotify2)
{
  ...
  rados_aio_watch(
      ioctx, notify_oid, notify_comp, &handle, watch_notify2_test_cb,
      watch_notify2_test_errcb_aio_reconnect, this);
...
TEST_F(LibRadosWatchNotify, AioNotify)
{
  ...
  ASSERT_EQ(
      0,
      rados_aio_notify(
          ioctx, "foo", comp, "notify", 6, 300000, &reply_buf, &reply_buf_len));
  ASSERT_EQ(0, rados_aio_wait_for_complete(comp));
  ASSERT_EQ(0, rados_aio_get_return_value(comp));
  rados_aio_release(comp);
  ...
  ASSERT_EQ(
      ...
      rados_decode_notify_response(
          reply_buf, reply_buf_len, &acks, &nr_acks, &timeouts, &nr_timeouts));
  ...
  ASSERT_EQ(
      0, rados_aio_notify(
             ioctx, "doesnotexist", comp, "notify", 6, 300000, &reply_buf,
             &reply_buf_len));
  ASSERT_EQ(0, rados_aio_wait_for_complete(comp));
  ASSERT_EQ(-ENOENT, rados_aio_get_return_value(comp));
  rados_aio_release(comp);
...
TEST_F(LibRadosWatchNotify, WatchNotify2Multi)
{
  ...
  ASSERT_EQ(
      0, rados_watch2(
             ioctx, notify_oid, &handle1, watch_notify2_test_cb,
             watch_notify2_test_errcb, this));
  ASSERT_EQ(
      0, rados_watch2(
             ioctx, notify_oid, &handle2, watch_notify2_test_cb,
             watch_notify2_test_errcb, this));
...
TEST_F(LibRadosWatchNotify, WatchNotify2Timeout)
{
  ...
  size_t reply_buf_len;
  ASSERT_EQ(
      -ETIMEDOUT, rados_notify2(
                      ioctx, notify_oid, "notify", 6, 1000, // 1s
  ...
  ASSERT_EQ(
      0, rados_notify2(
             ioctx, notify_oid, "notify", 6, 300000, // 300s
             &reply_buf, &reply_buf_len));
  ASSERT_EQ(1u, notify_cookies.size());
  ASSERT_GT(rados_watch_check(ioctx, handle), 0);
...
TEST_F(LibRadosWatchNotify, Watch3Timeout)
{
  ...
  const uint32_t timeout = 4;
  {
    ...
    ASSERT_EQ(
        0, rados_conf_get(
               cluster, "ms_connection_idle_timeout", conf, sizeof(conf)));
  ...
  ASSERT_EQ(
      0, rados_watch3(
             ioctx, notify_oid, &handle, watch_notify2_test_cb,
             watch_notify2_test_errcb, timeout, this));
