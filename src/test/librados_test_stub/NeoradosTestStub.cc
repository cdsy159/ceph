...
public:
  ...
  get_io_ctx(const IOContext& ioc)
  {
...
void
completion_callback_adapter(rados_completion_t c, void* arg)
{
...
create_aio_completion(Op::Completion&& c)
{
...
int
save_operation_size(int result, uint64_t* pval)
{
  int our_r = result;
  if (result <= -MAX_ERRNO) {
    if (pval != NULL) {
...
int
save_operation_ec(int result, boost::system::error_code* ec)
{
  if (ec != NULL) {
...
IOContext::IOContext(IOContext&& rhs)
{
  static_assert(impl_size >= sizeof(IOContextImpl));
  new (&impl) IOContextImpl(
      std::move(*reinterpret_cast<const IOContextImpl*>(&rhs.impl)));
}
...
operator==(const IOContext& lhs, const IOContext& rhs)
{
  ...
  return (
      l->oloc == r->oloc && l->snap_seq == r->snap_seq &&
      l->snapc.seq == r->snapc.seq && l->snapc.snaps == r->snapc.snaps);
}
...
Op::assert_exists()
{
  auto o = *reinterpret_cast<librados::TestObjectOperationImpl**>(&impl);
  o->ops.push_back(
      std::bind(&librados::TestIoCtxImpl::assert_exists, _1, _2, _4));
}
...
Op::assert_version(uint64_t ver)
{
  auto o = *reinterpret_cast<librados::TestObjectOperationImpl**>(&impl);
  o->ops.push_back(
      std::bind(&librados::TestIoCtxImpl::assert_version, _1, _2, ver));
}

void
Op::cmpext(uint64_t off, ceph::buffer::list cmp_bl, uint64_t* s)
{
  auto o = *reinterpret_cast<librados::TestObjectOperationImpl**>(&impl);
  librados::ObjectOperationTestImpl op =
      std::bind(&librados::TestIoCtxImpl::cmpext, _1, _2, off, cmp_bl, _4);
  if (s != nullptr) {
    op = std::bind(
    ...
    boost::system::error_code* ec)
{
  ...
  librados::ObjectOperationTestImpl op =
      [cls_handler, cls, method, inbl = const_cast<bufferlist&>(inbl), out](
          librados::TestIoCtxImpl* io_ctx, const std::string& oid,
          bufferlist* outbl, uint64_t snap_id, const SnapContext& snapc,
          uint64_t*) mutable -> int {
    return io_ctx->exec(
        oid, cls_handler, std::string(cls).c_str(), std::string(method).c_str(),
        inbl, (out != nullptr ? out : outbl), snap_id, snapc);
  };
  if (ec != nullptr) {
...
WriteOp::create(bool exclusive) &
{
  auto o = *reinterpret_cast<librados::TestObjectOperationImpl**>(&impl);
  o->ops.push_back(
      std::bind(&librados::TestIoCtxImpl::create, _1, _2, exclusive, _5));
  return *this;
}
...
WriteOp::write_full(ceph::buffer::list bl) &
{
  auto o = *reinterpret_cast<librados::TestObjectOperationImpl**>(&impl);
  o->ops.push_back(
      std::bind(&librados::TestIoCtxImpl::write_full, _1, _2, bl, _5));
  return *this;
}
...
WriteOp::truncate(uint64_t off) &
{
  auto o = *reinterpret_cast<librados::TestObjectOperationImpl**>(&impl);
  o->ops.push_back(
      std::bind(&librados::TestIoCtxImpl::truncate, _1, _2, off, _5));
  return *this;
}
...
WriteOp::zero(uint64_t off, uint64_t len) &
{
  auto o = *reinterpret_cast<librados::TestObjectOperationImpl**>(&impl);
  o->ops.push_back(
      std::bind(&librados::TestIoCtxImpl::zero, _1, _2, off, len, _5));
  return *this;
}
...
{
  auto test_rados_client =
      reinterpret_cast<librados::TestRadosClient*>(rados.client);
    ...
    uint64_t subsystem)
{
  ...
  auto r = io_ctx->aio_operate_read(
      std::string{o}, *ops, completion, 0U, bl, snap_id, objver);
  ceph_assert(r == 0);
}
    ...
    uint64_t subsystem)
{
  ...
  auto completion = create_aio_completion(std::move(c));
  auto r = io_ctx->aio_operate(
      std::string{o}, *ops, completion, &snapc, nullptr, 0U);
  ceph_assert(r == 0);
}
...
RADOS::mon_command_(
    ...
    Op::Completion c)
{
  auto r = impl->test_rados_client->mon_command(
      std::move(command), std::move(bl), outbl, outs);
  asio::post(
      get_executor(),
      asio::append(
          std::move(c),
...
void
RADOS::wait_for_latest_osd_map_(Op::Completion c)
{
...
get_mock_io_ctx(neorados::RADOS& rados, neorados::IOContext& io_context)
{
  auto& impl =
      *reinterpret_cast<std::unique_ptr<neorados::detail::Client>*>(&rados);
...
get_mock_rados_client(neorados::RADOS& rados)
{
  auto& impl =
      *reinterpret_cast<std::unique_ptr<neorados::detail::Client>*>(&rados);
