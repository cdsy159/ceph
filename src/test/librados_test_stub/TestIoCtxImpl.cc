...
TestIoCtxImpl::~TestIoCtxImpl() { ceph_assert(m_pending_ops == 0); }

void
TestObjectOperationImpl::get()
{
  m_refcount++;
}
...
uint64_t
TestIoCtxImpl::get_instance_id() const
{
  return m_client->get_instance_id();
}

int64_t
TestIoCtxImpl::get_id()
{
  return m_pool_id;
}

uint64_t
TestIoCtxImpl::get_last_version()
{
  return 0;
}
...
void
TestIoCtxImpl::aio_flush_async(AioCompletionImpl* c)
{
  m_client->flush_aio_operations(c);
}

void
TestIoCtxImpl::aio_notify(
    ...
    bufferlist* pbl)
{
  ...
  m_client->get_watch_notify()->aio_notify(
      m_client, m_pool_id, get_namespace(), oid, bl, timeout_ms, pbl, ctx);
}

int
TestIoCtxImpl::aio_operate(
    const std::string& oid,
    TestObjectOperationImpl& ops,
    AioCompletionImpl* c,
    SnapContext* snap_context,
    ...
    int flags)
{
  ...
  m_client->add_aio_operation(
      ...
      std::bind(
          ...
          snap_context != NULL ? *snap_context : m_snapc, nullptr),
      c);
  return 0;
}

int
TestIoCtxImpl::aio_operate_read(
    const std::string& oid,
    TestObjectOperationImpl& ops,
    AioCompletionImpl* c,
    int flags,
    ...
    librados::WatchCtx2* watch_ctx)
{
  ...
  if (m_client->is_blocklisted()) {
    ...
    m_client->get_watch_notify()->aio_watch(
        m_client, m_pool_id, get_namespace(), o, get_instance_id(), handle,
        nullptr, watch_ctx, ctx);
  }
  return 0;
}

int
TestIoCtxImpl::aio_unwatch(uint64_t handle, AioCompletionImpl* c)
{
  m_pending_ops++;
  c->get();
  C_AioNotify* ctx = new C_AioNotify(this, c);
    ...
    const SnapContext& snapc)
{
  ...
  return (*call)(
      reinterpret_cast<cls_method_context_t>(
          handler->get_method_context(this, oid, snap_id, snapc).get()),
      &inbl, outbl);
}

int
TestIoCtxImpl::list_watchers(
    const std::string& o,
    std::list<obj_watch_t>* out_watchers)
{
  ...
  return m_client->get_watch_notify()->list_watchers(
      m_pool_id, get_namespace(), o, out_watchers);
}

int
TestIoCtxImpl::notify(
    const std::string& o,
    bufferlist& bl,
    uint64_t timeout_ms,
    bufferlist* pbl)
{
  ...
  return m_client->get_watch_notify()->notify(
      m_client, m_pool_id, get_namespace(), o, bl, timeout_ms, pbl);
}

void
TestIoCtxImpl::notify_ack(
    const std::string& o,
    uint64_t notify_id,
    uint64_t handle,
    bufferlist& bl)
{
  m_client->get_watch_notify()->notify_ack(
      m_client, m_pool_id, get_namespace(), o, notify_id, handle,
      m_client->get_instance_id(), bl);
}

int
TestIoCtxImpl::operate(const std::string& oid, TestObjectOperationImpl& ops)
{
  ...
  m_client->add_aio_operation(
      ...
      std::bind(
          &TestIoCtxImpl::execute_aio_operations, this, oid, &ops,
          reinterpret_cast<bufferlist*>(0), m_snap_seq, m_snapc, nullptr),
      comp);
...
void
TestIoCtxImpl::aio_selfmanaged_snap_create(uint64_t* snapid, AioCompletionImpl* c)
{
  m_client->add_aio_operation(
      "", true,
...
void
TestIoCtxImpl::aio_selfmanaged_snap_remove(uint64_t snapid, AioCompletionImpl* c)
{
  m_client->add_aio_operation(
      "", true,
...
int
TestIoCtxImpl::selfmanaged_snap_set_write_ctx(
    snap_t seq,
...
void
TestIoCtxImpl::set_snap_read(snap_t seq)
{
  if (seq == 0) {
    seq = CEPH_NOSNAP;
  }
...
int
TestIoCtxImpl::tmap_update(const std::string& oid, bufferlist& cmdbl)
{
  if (m_client->is_blocklisted()) {
    return -EBLOCKLISTED;
  }
...
int
TestIoCtxImpl::unwatch(uint64_t handle)
{
  if (m_client->is_blocklisted()) {
    return -EBLOCKLISTED;
  }
    ...
    librados::WatchCtx2* ctx2)
{
  ...
  return m_client->get_watch_notify()->watch(
      m_client, m_pool_id, get_namespace(), o, get_instance_id(), handle, ctx,
      ctx2);
}

int
TestIoCtxImpl::execute_operation(
    const std::string& oid,
    ...
    uint64_t* objver)
{
  ...
  if (m_client->is_blocklisted()) {
    ...
    TestRadosClient::Transaction transaction(m_client, get_namespace(), oid);
    for (ObjectOperations::iterator it = ops->ops.begin(); it != ops->ops.end();
         ++it) {
...
void
TestIoCtxImpl::handle_aio_notify_complete(AioCompletionImpl* c, int r)
{
  m_pending_ops--;
