...
public:
  ...
  inline void
  set_namespace(const std::string& namespace_name)
  {
  ...
  inline void
  set_snap_context(const SnapContext& snapc)
  {
    m_snapc = snapc;
  }
  ...
  virtual void aio_notify(
      const std::string& oid,
      AioCompletionImpl* c,
      bufferlist& bl,
      uint64_t timeout_ms,
      bufferlist* pbl);
  virtual int aio_operate(
      const std::string& oid,
  ...
  virtual int aio_operate_read(
      ...
      uint64_t snap_id,
      uint64_t* objver);
  virtual int aio_remove(
      const std::string& oid,
      AioCompletionImpl* c,
      int flags = 0) = 0;
  virtual int aio_watch(
      const std::string& o,
      AioCompletionImpl* c,
      uint64_t* handle,
  ...
  virtual void aio_selfmanaged_snap_create(
      uint64_t* snapid,
      AioCompletionImpl* c);
  ...
  virtual int selfmanaged_snap_rollback(
      const std::string& oid,
      uint64_t snapid) = 0;
  virtual int selfmanaged_snap_set_write_ctx(
      snap_t seq,
      std::vector<snap_t>& snaps);
  virtual int set_alloc_hint(
      const std::string& oid,
  ...
  virtual int write(
      ...
      uint64_t off,
      const SnapContext& snapc) = 0;
  virtual int write_full(
      const std::string& oid,
      bufferlist& bl,
      const SnapContext& snapc) = 0;
  virtual int writesame(
      const std::string& oid,
      bufferlist& bl,
      size_t len,
      uint64_t off,
  ...
  virtual int cmpext(
      ...
      bufferlist& cmp_bl,
      uint64_t snap_id) = 0;
  virtual int xattr_get(
      const std::string& oid,
      std::map<std::string, bufferlist>* attrset) = 0;
  virtual int xattr_set(
      const std::string& oid,
