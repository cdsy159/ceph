...
TestRadosClient*
TestMemCluster::create_rados_client(CephContext* cct)
{
  return new TestMemRadosClient(cct, this);
}

int
TestMemCluster::register_object_handler(
    int64_t pool_id,
...
void
TestMemCluster::unregister_object_handler(
    int64_t pool_id,
    const ObjectLocator& locator,
    ObjectHandler* object_handler)
{
...
int
TestMemCluster::pool_create(const std::string& pool_name)
{
  std::lock_guard locker{m_lock};
...
int
TestMemCluster::pool_delete(const std::string& pool_name)
{
  std::lock_guard locker{m_lock};
...
int
TestMemCluster::pool_get_base_tier(int64_t pool_id, int64_t* base_tier)
{
  // TODO
  *base_tier = pool_id;
  return 0;
...
int
TestMemCluster::pool_list(std::list<std::pair<int64_t, std::string>>& v)
{
  std::lock_guard locker{m_lock};
  v.clear();
...
int64_t
TestMemCluster::pool_lookup(const std::string& pool_name)
{
  std::lock_guard locker{m_lock};
  Pools::iterator iter = m_pools.find(pool_name);
  if (iter == m_pools.end()) {
...
int
TestMemCluster::pool_reverse_lookup(int64_t id, std::string* name)
{
  std::lock_guard locker{m_lock};
  for (Pools::iterator iter = m_pools.begin(); iter != m_pools.end(); ++iter) {
...
TestMemCluster::Pool*
TestMemCluster::get_pool(const ceph::mutex& lock, int64_t pool_id)
{
  for (auto& pool_pair : m_pools) {
...
TestMemCluster::Pool*
TestMemCluster::get_pool(const std::string& pool_name)
{
  std::lock_guard locker{m_lock};
  Pools::iterator iter = m_pools.find(pool_name);
  if (iter != m_pools.end()) {
...
void
TestMemCluster::allocate_client(uint32_t* nonce, uint64_t* global_id)
{
  std::lock_guard locker{m_lock};
  *nonce = m_next_nonce++;
...
void
TestMemCluster::deallocate_client(uint32_t nonce)
{
  std::lock_guard locker{m_lock};
...
bool
TestMemCluster::is_blocklisted(uint32_t nonce) const
{
  std::lock_guard locker{m_lock};
...
void
TestMemCluster::blocklist(uint32_t nonce)
{
  {
    std::lock_guard locker{m_lock};
...
void
TestMemCluster::transaction_start(const ObjectLocator& locator)
{
...
void
TestMemCluster::transaction_finish(const ObjectLocator& locator)
{
  std::lock_guard locker{m_lock};
