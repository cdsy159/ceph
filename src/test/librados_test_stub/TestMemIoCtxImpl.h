...
public:
  ...
  int aio_remove(const std::string& oid, AioCompletionImpl* c, int flags = 0)
      override;
  ...
  int create(const std::string& oid, bool exclusive, const SnapContext& snapc)
      override;
  ...
  int omap_set(
      const std::string& oid,
      const std::map<std::string, bufferlist>& map) override;
  int read(
      const std::string& oid,
      size_t len,
      uint64_t off,
  ...
  int truncate(const std::string& oid, uint64_t size, const SnapContext& snapc)
      override;
  int write(
      const std::string& oid,
      bufferlist& bl,
  ...
  int xattr_set(const std::string& oid, const std::string& name, bufferlist& bl)
      override;
  int zero(
      const std::string& oid,
      uint64_t off,
...
private:
  ...
  TestMemCluster::SharedFile get_file(
      const std::string& oid,
      bool write,
      uint64_t snap_id,
