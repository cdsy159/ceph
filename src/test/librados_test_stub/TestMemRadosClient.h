...
public:
  ...
  uint64_t
  get_instance_id() override
  {
    return m_global_id;
  }

  int
  get_min_compatible_osd(int8_t* require_osd_release) override
  {
  ...
  service_daemon_update_status(
      std::map<std::string, std::string>&& status) override
  {
    return 0;
  }
