...
protected:
  char*
  getObjName(const std::string& soid, uint64_t nb)
  {
      ...
      size_t actual_size_if_sparse)
  {
    ...
    ASSERT_LT(
        0, ioctx.getxattr(firstOid, "striper.layout.stripe_unit", xattrbl));
    std::string s_xattr(
        xattrbl.c_str(), xattrbl.length()); // adds 0 byte at the end
    uint64_t stripe_unit = strtoll(s_xattr.c_str(), NULL, 10);
    ASSERT_LT((unsigned)0, stripe_unit);
    ASSERT_EQ(stripe_unit, exp_stripe_unit);
    xattrbl.clear();
    ASSERT_LT(
        0, ioctx.getxattr(firstOid, "striper.layout.stripe_count", xattrbl));
    s_xattr = std::string(
        xattrbl.c_str(), xattrbl.length()); // adds 0 byte at the end
    ...
    xattrbl.clear();
    ASSERT_LT(
        0, ioctx.getxattr(firstOid, "striper.layout.object_size", xattrbl));
    s_xattr = std::string(
        xattrbl.c_str(), xattrbl.length()); // adds 0 byte at the end
    ...
    for (uint64_t stripe_nb = 0; stripe_nb < nb_stripes_in_object; stripe_nb++) {
      ...
      if (actual_size_if_sparse < size and
        ...
        uint64_t object_start =
            (object_in_set + objectset * stripe_per_objectset) * stripe_unit;
    ...
    for (uint64_t object_nb = 0; object_nb < nb_objects; object_nb++) {
      ...
      uint64_t object_start_stripe = nb_full_object_set * stripe_per_objectset +
                                     object_index_in_set;
      uint64_t object_start_off = object_start_stripe * stripe_unit;
      if (actual_size_if_sparse < size and
          actual_size_if_sparse <= object_start_off) {
        ...
        for (offset = object_start_off;
             (offset < (object_start_off) + set_size) &&
             (offset < actual_size_if_sparse);
             offset += stripe_size) {
...
TEST_P(StriperTestRT, StripedRoundtrip)
{
  ...
  oss << "StripedRoundtrip_" << testData.stripe_unit << "_"
      << testData.stripe_count << "_" << testData.object_size << "_"
      << testData.size;
  ...
  bufferlist bl1;
  {
    ...
    ASSERT_NO_FATAL_FAILURE(checkObjectFromRados(
        soid, bl1, testData.stripe_unit, testData.stripe_count,
        testData.object_size, testData.size));
  }
  ...
  bufferlist bl2;
  {
    ...
    ASSERT_NO_FATAL_FAILURE(checkObjectFromRados(
        soid, bl1, testData.stripe_unit, testData.stripe_count,
        testData.object_size, testData.size * 2));
  }
  ...
  // truncating back to original size and checking again (especially for 0s)
  {
    ...
    ASSERT_NO_FATAL_FAILURE(checkObjectFromRados(
        soid, bl3, testData.stripe_unit, testData.stripe_count,
        testData.object_size, testData.size, testData.size / 2));
  }
  {
    ...
    checkObjectFromRados(
        soid, bl2, testData.stripe_unit, testData.stripe_count,
        testData.object_size, testData.size);
  }
  {
    ...
    for (uint64_t object_nb = 0;
         object_nb <
         testData.size * 2 / testData.object_size + testData.stripe_count;
         object_nb++) {
      char* oid = getObjName(soid, object_nb);
