...
struct TestMockCacheWriteAroundObjectDispatch : public TestMockFixture {
  typedef WriteAroundObjectDispatch<librbd::MockTestImageCtx>
      MockWriteAroundObjectDispatch;

  void
  expect_op_work_queue(MockTestImageCtx& mock_image_ctx)
  {
    EXPECT_CALL(*mock_image_ctx.op_work_queue, queue(_, _))
        .WillRepeatedly(Invoke([](Context* ctx, int r) { ctx->complete(r); }));
  }

  void
  expect_context_complete(MockContext& mock_context, int r)
  {
...
TEST_F(TestMockCacheWriteAroundObjectDispatch, WriteThrough)
{
  ...
  ASSERT_FALSE(object_dispatch.write(
      0, 0, std::move(data), {}, 0, 0, std::nullopt, {}, nullptr, nullptr,
      &dispatch_result, &finish_ctx_ptr, &dispatch_ctx));
...
TEST_F(TestMockCacheWriteAroundObjectDispatch, WriteThroughUntilFlushed)
{
  ...
  ASSERT_FALSE(object_dispatch.flush(
      io::FLUSH_SOURCE_USER, {}, nullptr, &dispatch_result, &finish_ctx_ptr,
      &dispatch_ctx));

  expect_context_complete(dispatch_ctx, 0);
  expect_context_complete(finish_ctx, 0);

  ASSERT_TRUE(object_dispatch.write(
      0, 0, std::move(data), {}, 0, 0, std::nullopt, {}, nullptr, nullptr,
      &dispatch_result, &finish_ctx_ptr, &dispatch_ctx));
...
TEST_F(TestMockCacheWriteAroundObjectDispatch, BlockedIO)
{
  ...
  ASSERT_TRUE(object_dispatch.write(
      0, 0, std::move(data), {}, 0, 0, std::nullopt, {}, nullptr, nullptr,
      &dispatch_result, &finish_ctx_ptr1, &dispatch_ctx1));
  ASSERT_EQ(io::DISPATCH_RESULT_CONTINUE, dispatch_result);
  ...
  ASSERT_TRUE(object_dispatch.write(
      0, 4096, std::move(data), {}, 0, 0, std::nullopt, {}, nullptr, nullptr,
      &dispatch_result, &finish_ctx_ptr2, &dispatch_ctx2));
  ASSERT_EQ(io::DISPATCH_RESULT_CONTINUE, dispatch_result);
  ...
  ASSERT_TRUE(object_dispatch.write(
      0, 1024, std::move(data), {}, 0, 0, std::nullopt, {}, nullptr, nullptr,
      &dispatch_result, &finish_ctx_ptr3, &dispatch_ctx3));
  ASSERT_EQ(io::DISPATCH_RESULT_CONTINUE, dispatch_result);
...
TEST_F(TestMockCacheWriteAroundObjectDispatch, QueuedIO)
{
  ...
  ASSERT_TRUE(object_dispatch.write(
      0, 8192, std::move(data), {}, 0, 0, std::nullopt, {}, nullptr, nullptr,
      &dispatch_result, &finish_ctx_ptr2, &dispatch_ctx2));
  ASSERT_EQ(io::DISPATCH_RESULT_CONTINUE, dispatch_result);
  ASSERT_NE(finish_ctx_ptr2, &finish_ctx2);
  ASSERT_EQ(0, dispatch_ctx1.wait());
...
TEST_F(TestMockCacheWriteAroundObjectDispatch, FlushQueuedOnInFlightIO)
{
  ...
  Context* finish_ctx_ptr2 = &finish_ctx2;
  ASSERT_FALSE(object_dispatch.flush(
      io::FLUSH_SOURCE_USER, {}, nullptr, &dispatch_result, &finish_ctx_ptr2,
      &dispatch_ctx2));
  ASSERT_EQ(io::DISPATCH_RESULT_CONTINUE, dispatch_result);
  ASSERT_NE(finish_ctx_ptr2, &finish_ctx2);
...
TEST_F(TestMockCacheWriteAroundObjectDispatch, FlushQueuedOnQueuedIO)
{
  ...
  MockContext dispatch_ctx3;
  Context* finish_ctx_ptr3 = &finish_ctx3;
  ASSERT_TRUE(object_dispatch.flush(
      io::FLUSH_SOURCE_USER, {}, nullptr, &dispatch_result, &finish_ctx_ptr3,
      &dispatch_ctx3));
  ASSERT_EQ(io::DISPATCH_RESULT_CONTINUE, dispatch_result);
  ASSERT_NE(finish_ctx_ptr3, &finish_ctx3);
...
TEST_F(TestMockCacheWriteAroundObjectDispatch, UnoptimizedIO)
{
  ...
  ASSERT_FALSE(object_dispatch.compare_and_write(
      0, 0, std::move(data), std::move(data), {}, 0, {}, nullptr, nullptr,
      nullptr, &dispatch_result, &finish_ctx_ptr, &dispatch_ctx));
  ASSERT_EQ(finish_ctx_ptr, &finish_ctx);
}
...
TEST_F(TestMockCacheWriteAroundObjectDispatch, UnoptimizedIOInFlightIO)
{
  ...
  MockContext dispatch_ctx2;
  Context* finish_ctx_ptr2 = &finish_ctx2;
  ASSERT_TRUE(object_dispatch.compare_and_write(
      0, 0, std::move(data), std::move(data), {}, 0, {}, nullptr, nullptr,
      nullptr, &dispatch_result, &finish_ctx_ptr2, &dispatch_ctx2));
  ASSERT_EQ(io::DISPATCH_RESULT_CONTINUE, dispatch_result);
...
TEST_F(TestMockCacheWriteAroundObjectDispatch, UnoptimizedIOBlockedIO)
{
  ...
  ASSERT_TRUE(object_dispatch.compare_and_write(
      0, 0, std::move(data), std::move(data), {}, 0, {}, nullptr, nullptr,
      nullptr, &dispatch_result, &finish_ctx_ptr3, &dispatch_ctx3));
  ASSERT_EQ(io::DISPATCH_RESULT_CONTINUE, dispatch_result);
...
TEST_F(TestMockCacheWriteAroundObjectDispatch, WriteFUA)
{
  ...
  Context* finish_ctx_ptr = &finish_ctx;
  ASSERT_FALSE(object_dispatch.write(
      0, 0, std::move(data), {}, LIBRADOS_OP_FLAG_FADVISE_FUA, 0, std::nullopt,
      {}, nullptr, nullptr, &dispatch_result, &finish_ctx_ptr, &dispatch_ctx));
  ASSERT_EQ(finish_ctx_ptr, &finish_ctx);
}
...
TEST_F(TestMockCacheWriteAroundObjectDispatch, WriteSameFUA)
{
  ...
  MockContext dispatch_ctx;
  Context* finish_ctx_ptr = &finish_ctx;
  ASSERT_FALSE(object_dispatch.write_same(
      0, 0, 8192, {{0, 8192}}, std::move(data), {}, LIBRADOS_OP_FLAG_FADVISE_FUA,
      {}, nullptr, nullptr, &dispatch_result, &finish_ctx_ptr, &dispatch_ctx));
  ASSERT_EQ(finish_ctx_ptr, &finish_ctx);
}
