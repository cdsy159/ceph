...
struct TestMockCryptoLuksFlattenRequest : public TestMockFixture {
      ...
      bool magic_switched)
  {
    ...
    ASSERT_EQ(
        0, header.format(
               type, alg, nullptr, key_size, cipher_mode, sector_size,
               OBJECT_SIZE, true));
  ...
  expect_get_crypto()
  {
    EXPECT_CALL(*mock_encryption_format, get_crypto())
        .WillOnce(Return(&mock_crypto));
  }

  void
  expect_image_read(uint64_t offset, uint64_t length)
  {
    EXPECT_CALL(*mock_image_ctx->io_image_dispatcher, send(_))
        .WillOnce(Invoke([this, offset, length](io::ImageDispatchSpec* spec) {
          auto* read = std::get_if<io::ImageDispatchSpec::Read>(&spec->request);
  ...
  expect_image_write()
  {
    EXPECT_CALL(*mock_image_ctx->io_image_dispatcher, send(_))
        .WillOnce(Invoke([this](io::ImageDispatchSpec* spec) {
          auto* write =
              std::get_if<io::ImageDispatchSpec::Write>(&spec->request);
  ...
  void
  verify_header(const char* expected_format)
  {
    Header header(mock_image_ctx->cct);
