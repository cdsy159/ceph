...
struct CopyupRequest<librbd::MockImageCtx> {
  ...
  MOCK_METHOD2(
      append_request,
      void(AbstractObjectWriteRequest<librbd::MockImageCtx>*, const Extents&));

  static CopyupRequest* s_instance;

  static CopyupRequest*
  create(
      librbd::MockImageCtx* ictx,
      uint64_t objectno,
...
CopyupRequest<librbd::MockImageCtx>*
    CopyupRequest<librbd::MockImageCtx>::s_instance = nullptr;
...
template <>
uint64_t
get_file_offset(MockImageCtx* image_ctx, uint64_t object_no, uint64_t offset)
{
  return Striper::get_file_offset(
      image_ctx->cct, &image_ctx->layout, object_no, offset);
}
...
struct TestMockCryptoCryptoObjectDispatch : public TestMockFixture {
  ...
  SetUp() override
  {
    ...
    mock_crypto_object_dispatch =
        new MockCryptoObjectDispatch(mock_image_ctx, &crypto);
  ...
  void
  expect_object_read(io::ReadExtents* extents, uint64_t version = 0)
  {
    EXPECT_CALL(*mock_image_ctx->io_object_dispatcher, send(_))
        .WillOnce(Invoke([this, extents, version](io::ObjectDispatchSpec* spec) {
          auto* read =
              std::get_if<io::ObjectDispatchSpec::ReadRequest>(&spec->request);
      ...
      std::optional<uint64_t> assert_version)
  {
    EXPECT_CALL(*mock_image_ctx->io_object_dispatcher, send(_))
        .WillOnce(Invoke([this, object_off, data, write_flags,
          ...
          auto* write =
              std::get_if<io::ObjectDispatchSpec::WriteRequest>(&spec->request);
  ...
  expect_object_write_same()
  {
    EXPECT_CALL(*mock_image_ctx->io_object_dispatcher, send(_))
        .WillOnce(Invoke([this](io::ObjectDispatchSpec* spec) {
          auto* write_same =
              std::get_if<io::ObjectDispatchSpec::WriteSameRequest>(
                  &spec->request);
  ...
  expect_get_object_size()
  {
    EXPECT_CALL(*mock_image_ctx, get_object_size())
        .WillOnce(Return(mock_image_ctx->layout.object_size));
  }

  void
  expect_get_parent_overlap(uint64_t overlap)
  {
  ...
  void
  expect_prune_parent_extents(uint64_t object_overlap)
  {
  ...
  void
  expect_copyup(MockAbstractObjectWriteRequest** write_request, int r)
  {
  ...
  void
  expect_encrypt(int count = 1)
  {
    EXPECT_CALL(crypto, encrypt(_, _)).Times(count);
  }

  void
  expect_decrypt(int count = 1)
  {
...
TEST_F(TestMockCryptoCryptoObjectDispatch, AlignedReadFail)
{
  ...
  ASSERT_TRUE(mock_crypto_object_dispatch->read(
      11, &extents, mock_image_ctx->get_data_io_context(), 0, 0, {}, nullptr,
...
TEST_F(TestMockCryptoCryptoObjectDispatch, AlignedRead)
{
  ...
  extents[0].bl.append(
      std::string(1024, '1') + std::string(1024, '2') + std::string(1024, '3') +
      std::string(1024, '4'));
...
TEST_F(TestMockCryptoCryptoObjectDispatch, ReadFromParentDisabled)
{
  ...
  ASSERT_TRUE(mock_crypto_object_dispatch->read(
      ...
      io::READ_FLAG_DISABLE_READ_FROM_PARENT, {}, nullptr,
      &object_dispatch_flags, &dispatch_result, &on_finish, on_dispatched));
  ASSERT_EQ(dispatch_result, io::DISPATCH_RESULT_COMPLETE);
  ASSERT_EQ(on_finish, &finished_cond);
  ASSERT_EQ(ETIMEDOUT, dispatched_cond.wait_for(0));
...
{
  io::ReadExtents extents = {
      {0, 1}, {8191, 1}, {8193, 1}, {16384 + 1, 4096 * 5 - 2}};
  io::ReadExtents aligned_extents = {
      {0, 4096}, {4096, 4096}, {8192, 4096}, {16384, 4096 * 5}};
  ...
  aligned_extents[3].bl.append(
      std::string("0") + std::string(4095, '4') + std::string(4096, '5') +
      std::string(4095, '6') + std::string("0"));
  aligned_extents[3].extent_map = {
      {16384, 4096}, {16384 + 2 * 4096, 4096}, {16384 + 4 * 4096, 4096}};
  ...
  auto expected_bl_data =
      (std::string(4095, '4') + std::string(4096, '5') + std::string(4095, '6'));
  ASSERT_TRUE(extents[3].bl.to_str() == expected_bl_data);
  ASSERT_THAT(
      extents[3].extent_map,
      ElementsAre(
          Pair(16384 + 1, 4095), Pair(16384 + 2 * 4096, 4096),
...
TEST_F(TestMockCryptoCryptoObjectDispatch, UnalignedWrite)
{
  ...
  ASSERT_TRUE(mock_crypto_object_dispatch->write(
      11, 1, std::move(write_data), mock_image_ctx->get_data_io_context(), 0, 0,
      std::nullopt, {}, nullptr, nullptr, &dispatch_result, &on_finish,
      on_dispatched));
  ASSERT_EQ(dispatch_result, io::DISPATCH_RESULT_COMPLETE);
  ASSERT_EQ(on_finish, &finished_cond);

  auto expected_data = std::string("2") + std::string(8192, '1') +
                       std::string(4095, '3');
...
TEST_F(TestMockCryptoCryptoObjectDispatch, UnalignedWriteWithNoObject)
{
  ...
  auto expected_data =
      (std::string(1, '\0') + std::string(8192, '1') + std::string(4095, '\0'));
  expect_object_write(
      0, expected_data, io::OBJECT_WRITE_FLAG_CREATE_EXCLUSIVE, std::nullopt);
...
TEST_F(TestMockCryptoCryptoObjectDispatch, UnalignedWriteFailCreate)
{
  ...
  auto expected_data2 = std::string("2") + std::string(8192, '1') +
                        std::string(4095, '3');
...
TEST_F(TestMockCryptoCryptoObjectDispatch, UnalignedWriteCopyup)
{
  ...
  EXPECT_CALL(*mock_image_ctx->object_map, object_may_exist(11))
...
TEST_F(TestMockCryptoCryptoObjectDispatch, UnalignedWriteWithAssertVersion)
{
  ...
  ASSERT_TRUE(mock_crypto_object_dispatch->write(
      11, 1, std::move(write_data), mock_image_ctx->get_data_io_context(), 0, 0,
      std::make_optional(assert_version), {}, nullptr, nullptr,
...
TEST_F(TestMockCryptoCryptoObjectDispatch, UnalignedWriteWithExclusiveCreate)
{
  ...
  expect_object_read(&extents);
  ASSERT_TRUE(mock_crypto_object_dispatch->write(
      11, 1, std::move(write_data), mock_image_ctx->get_data_io_context(), 0,
      io::OBJECT_WRITE_FLAG_CREATE_EXCLUSIVE, std::nullopt, {}, nullptr,
...
TEST_F(TestMockCryptoCryptoObjectDispatch, CompareAndWriteFail)
{
  ...
  ASSERT_TRUE(mock_crypto_object_dispatch->compare_and_write(
      11, 1, std::move(cmp_data), std::move(write_data),
      mock_image_ctx->get_data_io_context(), 0, {}, &mismatch_offset, nullptr,
      nullptr, &dispatch_result, &on_finish, on_dispatched));
  ASSERT_EQ(dispatch_result, io::DISPATCH_RESULT_COMPLETE);
  ASSERT_EQ(on_finish, &finished_cond);
...
TEST_F(TestMockCryptoCryptoObjectDispatch, PrepareCopyup)
{
  ...
  InSequence seq;
  ASSERT_EQ(
      0, mock_crypto_object_dispatch->prepare_copyup(
  ...
  ASSERT_TRUE(
      it.get_val().bl.to_str() ==
      std::string("1") + std::string(4095, '\0') + std::string(4095, '\0') +
  ...
  ASSERT_TRUE(
      it.get_val().bl.to_str() ==
      std::string(4096, '\0') + std::string(4095, '\0') + std::string("6") +
  ...
  ASSERT_TRUE(
      it.get_val().bl.to_str() == std::string("9") + std::string(4095, '\0'));
