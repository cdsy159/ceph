...
struct TestMockCryptoLoadRequest : public TestMockFixture {
  ...
  SetUp() override
  {
    ...
    mock_load_request =
        MockLoadRequest::create(mock_image_ctx, std::move(formats), on_finish);
  }
  ...
  void
  expect_test_journal_feature(MockTestImageCtx* ictx, bool has_journal = false)
  {
    EXPECT_CALL(*ictx, test_features(RBD_FEATURE_JOURNALING))
        .WillOnce(Return(has_journal));
  }

  void
  expect_encryption_load(
      MockEncryptionFormat* encryption_format,
      ...
      std::string detected_format = "SOMEFORMAT")
  {
    EXPECT_CALL(*encryption_format, load(ictx, _, _))
        .WillOnce(WithArgs<1, 2>(Invoke(
            [this,
             detected_format](std::string* detected_format_name, Context* ctx) {
  ...
  void
  expect_encryption_format_clone(MockEncryptionFormat* encryption_format)
  {
  ...
  void
  expect_invalidate_cache(int r = 0)
  {
...
TEST_F(TestMockCryptoLoadRequest, LoadClonedEncryptedParent)
{
  ...
  ASSERT_EQ(mock_encryption_format, mock_image_ctx->encryption_format.get());
  ASSERT_EQ(
      cloned_encryption_format, mock_parent_image_ctx->encryption_format.get());
}
