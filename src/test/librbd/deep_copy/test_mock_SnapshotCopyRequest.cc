...
SetHeadRequest<librbd::MockTestImageCtx>*
    SetHeadRequest<librbd::MockTestImageCtx>::s_instance = nullptr;
SnapshotCreateRequest<librbd::MockTestImageCtx>*
    SnapshotCreateRequest<librbd::MockTestImageCtx>::s_instance = nullptr;
...
public:
  ...
  typedef SnapshotCreateRequest<librbd::MockTestImageCtx>
      ...
      int r)
  {
    EXPECT_CALL(mock_snapshot_create_request, send())
        .WillOnce(DoAll(
            Invoke([&mock_image_ctx, snap_id, snap_name]() {
  ...
  void
  expect_set_head(MockSetHeadRequest& mock_set_head_request, int r)
  {
  ...
  inject_snap(
      ...
      const std::string& snap_name)
  {
    mock_image_ctx.snap_ids[{cls::rbd::UserSnapshotNamespace(), snap_name}] =
        snap_id;
  }
  ...
  create_request(
      ...
      Context* on_finish)
  {
    return new MockSnapshotCopyRequest(
        &mock_src_image_ctx, &mock_dst_image_ctx, src_snap_id_start,
        src_snap_id_end, dst_snap_id_start, false, m_work_queue, &m_snap_seqs,
        on_finish);
  }
      ...
      bool protect)
  {
    ...
    int r = image_ctx->operations->snap_create(
        snap_ns, snap_name.c_str(), 0, prog_ctx);
    if (r < 0) {
      return r;
    }
    ...
    if (protect) {
      EXPECT_TRUE(
          std::holds_alternative<cls::rbd::UserSnapshotNamespace>(snap_ns));
  ...
  {
    return create_snap(
        image_ctx, cls::rbd::UserSnapshotNamespace{}, snap_name, protect);
  }

  void
  validate_snap_seqs(const librbd::SnapSeqs& snap_seqs)
  {
...
TEST_F(TestMockDeepCopySnapshotCopyRequest, Empty)
{
  ...
  C_SaferCond ctx;
  MockSnapshotCopyRequest* request = create_request(
      mock_src_image_ctx, mock_dst_image_ctx, 0, CEPH_NOSNAP, 0, &ctx);
  request->send();
...
TEST_F(TestMockDeepCopySnapshotCopyRequest, SnapCreate)
{
  ...
  uint64_t src_snap_id1 =
      m_src_image_ctx->snap_ids[{cls::rbd::UserSnapshotNamespace(), "snap1"}];
  uint64_t src_snap_id2 =
      m_src_image_ctx->snap_ids[{cls::rbd::UserSnapshotNamespace(), "snap2"}];
  ...
  expect_snap_create(
      mock_dst_image_ctx, mock_snapshot_create_request, "snap1", 12, 0);
  expect_get_snap_namespace(mock_src_image_ctx, src_snap_id2);
  expect_start_op(mock_exclusive_lock);
  expect_snap_create(
      mock_dst_image_ctx, mock_snapshot_create_request, "snap2", 14, 0);
...
TEST_F(TestMockDeepCopySnapshotCopyRequest, SnapCreateError)
{
  ...
  uint64_t src_snap_id1 =
      mock_src_image_ctx.snap_ids[{cls::rbd::UserSnapshotNamespace(), "snap1"}];
  ...
  expect_start_op(mock_exclusive_lock);
  expect_snap_create(
      mock_dst_image_ctx, mock_snapshot_create_request, "snap1", 12, -EINVAL);
...
TEST_F(TestMockDeepCopySnapshotCopyRequest, SnapRemoveAndCreate)
{
  ...
  uint64_t src_snap_id1 =
      m_src_image_ctx->snap_ids[{cls::rbd::UserSnapshotNamespace(), "snap1"}];
  uint64_t dst_snap_id1 =
      m_dst_image_ctx->snap_ids[{cls::rbd::UserSnapshotNamespace(), "snap1"}];
...
TEST_F(TestMockDeepCopySnapshotCopyRequest, SnapProtectCancel)
{
  ...
  expect_start_op(mock_exclusive_lock);
  EXPECT_CALL(
      *mock_dst_image_ctx.operations, execute_snap_protect(_, StrEq("snap1"), _))
      .WillOnce(DoAll(
          InvokeWithoutArgs([request]() { request->cancel(); }),
          WithArg<2>(Invoke([this](Context* ctx) {
            m_work_queue->queue(ctx, 0);
...
TEST_F(TestMockDeepCopySnapshotCopyRequest, NoSetHead)
{
  ...
  C_SaferCond ctx;
  MockSnapshotCopyRequest* request = create_request(
      mock_src_image_ctx, mock_dst_image_ctx, 0, src_snap_id1, 0, &ctx);
  request->send();
  ASSERT_EQ(0, ctx.wait());
...
TEST_F(TestMockDeepCopySnapshotCopyRequest, StartEndLimit)
{
  ...
  expect_start_op(mock_exclusive_lock);
  expect_snap_create(
      mock_dst_image_ctx, mock_snapshot_create_request, "snap2", 12, 0);
  ...
  C_SaferCond ctx;
  MockSnapshotCopyRequest* request = create_request(
      mock_src_image_ctx, mock_dst_image_ctx, src_snap_id1, src_snap_id3,
      dst_snap_id1, &ctx);
  request->send();
  ASSERT_EQ(0, ctx.wait());
