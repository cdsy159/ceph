...
public:
      ...
      int r)
  {
    ...
    EXPECT_CALL(
        *mock_image_ctx.operations,
        ...
        .WillOnce(DoAll(
            InvokeWithoutArgs([&mock_image_ctx, snap_id, snap_name]() {
  ...
  {
    std::string oid(
        librbd::ObjectMap<>::object_map_name(mock_image_ctx.id, snap_id));
    EXPECT_CALL(
      ...
      Context* on_finish)
  {
    return new MockSnapshotCreateRequest(
        &mock_local_image_ctx, snap_name, snap_namespace, size, spec,
...
TEST_F(TestMockDeepCopySnapshotCreateRequest, SnapCreate)
{
  ...
  C_SaferCond ctx;
  MockSnapshotCreateRequest* request = create_request(
      mock_image_ctx, "snap1", cls::rbd::UserSnapshotNamespace(),
      m_image_ctx->size, {}, 0, &ctx);
  request->send();
...
TEST_F(TestMockDeepCopySnapshotCreateRequest, SetHeadError)
{
  ...
  C_SaferCond ctx;
  MockSnapshotCreateRequest* request = create_request(
      mock_image_ctx, "snap1", cls::rbd::UserSnapshotNamespace(), 123, {}, 0,
      &ctx);
  request->send();
