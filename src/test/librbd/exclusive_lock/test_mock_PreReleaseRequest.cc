...
struct ImageDispatch<MockTestImageCtx> {
  MOCK_METHOD3(
      set_require_lock,
      void(bool init_shutdown, io::Direction, Context*));
...
public:
  ...
  void
  expect_complete_context(MockContext& mock_context, int r)
  {
      ...
      int r)
  {
    ...
    if (mock_image_ctx.clone_copy_on_read ||
        ...
        (mock_image_ctx.features & RBD_FEATURE_DIRTY_CACHE) != 0) {
      expect_set_require_lock(
          mock_image_dispatch, init_shutdown, librbd::io::DIRECTION_BOTH, r);
    } else {
      expect_set_require_lock(
          mock_image_dispatch, init_shutdown, librbd::io::DIRECTION_WRITE, r);
    }
  }
  ...
  void
  expect_cancel_op_requests(MockTestImageCtx& mock_image_ctx, int r)
  {
  ...
  void
  expect_prerelease_exclusive_lock(MockTestImageCtx& mock_image_ctx, int r)
  {
  ...
  void
  expect_invalidate_cache(MockTestImageCtx& mock_image_ctx, int r)
  {
  ...
  void
  expect_flush_notifies(MockTestImageCtx& mock_image_ctx)
  {
  ...
  void
  expect_prepare_lock(MockTestImageCtx& mock_image_ctx)
  {
  ...
  void
  expect_handle_prepare_lock_complete(MockTestImageCtx& mock_image_ctx)
  {
    EXPECT_CALL(*mock_image_ctx.state, handle_prepare_lock_complete());
  }

  void
  expect_flush_io(MockTestImageCtx& mock_image_ctx, int r)
  {
...
TEST_F(TestMockExclusiveLockPreReleaseRequest, Blocklisted)
{
  ...
  MockImageDispatch mock_image_dispatch;
  expect_set_require_lock(
      mock_image_ctx, mock_image_dispatch, false, -EBLOCKLISTED);
  expect_prepare_lock(mock_image_ctx);
