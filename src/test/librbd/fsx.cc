...
int
register_journal(rados_ioctx_t ioctx, const char* image_name)
{
  librados::IoCtx io_ctx;
...
int
unregister_journal(rados_ioctx_t ioctx, const char* image_name)
{
  librados::IoCtx io_ctx;
  librados::IoCtx::from_rados_ioctx_t(ioctx, io_ctx);
    ...
    const char* last_replay_image_name)
{
  ...
  if (last_replay_image_name == nullptr) {
    ...
    r = rbd.clone2(
        io_ctx, last_replay_image_name, "snap", io_ctx, replay_image_name,
    ...
    const char* replay_image_name)
{
  ...
  BOOST_SCOPE_EXIT_ALL((&replay_journaler)) { replay_journaler.shut_down(); };
    ...
    int stripe_count)
{
  ...
  if (clones > 0) {
    replay_imagename(
        lastreplayimagename, sizeof(lastreplayimagename), clones - 1);
  }

  int ret = create_replay_image(
      ioctx, order, stripe_unit, stripe_count, replayimagename,
      clones > 0 ? lastreplayimagename : nullptr);
  if (ret < 0) {
    exit(EXIT_FAILURE);
  }
...
#define RBD_CTX_INIT \
  (struct rbd_ctx) { NULL, NULL, NULL, -1 }
...
struct rbd_operations {
  ...
  ssize_t (
      *write)(struct rbd_ctx* ctx, uint64_t off, size_t len, const char* buf);
    ...
    const char* buf)
{
  ...
  n = rbd_compare_and_write(
      ctx->image, off, len, cmp_buf, buf, &mismatch_off, 0);
  if (n == -EINVAL) {
    return n;
  } else if (n < 0) {
    ...
    int stripe_count)
{
  ...
  ceph_assert(ret == 0);
  ret = rbd_image_options_set_uint64(
    ...
    int stripe_count)
{
  ...
  if (deep_copy) {
    ret = __librbd_deep_copy(
        ctx, src_snapname, dst_imagename, features, order, stripe_unit,
        stripe_count);
    if (ret < 0) {
    ...
    ret = rbd_clone2(
        ioctx, ctx->name, src_snapname, ioctx, dst_imagename, features, order,
...
librbd_clone(
    ...
    int stripe_count)
{
  return __librbd_clone(
      ctx, src_snapname, dst_imagename, order, stripe_unit, stripe_count);
}
...
const struct rbd_operations librbd_operations = {
    librbd_open,  librbd_close,   librbd_read,      librbd_write,
    librbd_flush, librbd_discard, librbd_get_size,  librbd_resize,
    librbd_clone, librbd_flatten, librbd_writesame, librbd_compare_and_write,
};
...
const struct rbd_operations krbd_operations = {
    krbd_open,  krbd_close,   generic_pread, generic_pwrite,
    krbd_flush, krbd_discard, krbd_get_size, krbd_resize,
    krbd_clone, krbd_flatten, NULL,
};
#endif // WITH_KRBD
...
nbd_open(const char* name, struct rbd_ctx* ctx)
{
  ...
  SubProcess process(
      "rbd-nbd", SubProcess::KEEP, SubProcess::PIPE, SubProcess::KEEP);
...
const struct rbd_operations nbd_operations = {
    nbd_open,   nbd_close,    generic_pread, generic_pwrite,
    krbd_flush, krbd_discard, krbd_get_size, krbd_resize,
    nbd_clone,  krbd_flatten, NULL,
};
#endif // __linux__
...
wnbd_set_writable(struct rbd_ctx* ctx)
{
  ...
  BOOL succeeded = DeviceIoControl(
      (HANDLE)_get_osfhandle(ctx->krbd_fd), IOCTL_DISK_SET_DISK_ATTRIBUTES,
      (LPVOID)&attributes, (DWORD)sizeof(attributes), NULL, 0, &bytesReturned,
      NULL);
  if (!succeeded) {
...
wnbd_open(const char* name, struct rbd_ctx* ctx)
{
  ...
  SubProcess process(
      "rbd-wnbd", SubProcess::KEEP, SubProcess::PIPE, SubProcess::KEEP);
  process.add_cmd_arg("map");
  std::string img;
  img.append(pool);
  img.append("/");
...
wnbd_get_size(struct rbd_ctx* ctx, uint64_t* size)
{
  ...
  BOOL succeeded = DeviceIoControl(
      (HANDLE)_get_osfhandle(ctx->krbd_fd), IOCTL_DISK_GET_LENGTH_INFO, NULL, 0,
      (LPVOID)&length, (DWORD)sizeof(length), &bytesReturned, NULL);
  if (!succeeded) {
    DWORD err = GetLastError();
...
const struct rbd_operations wnbd_operations = {
    wnbd_open,  wnbd_close,   generic_pread, generic_pwrite,
    wnbd_flush, wnbd_discard, wnbd_get_size, wnbd_resize,
    wnbd_clone, wnbd_flatten, NULL,
};
#endif // _WIN32
...
ggate_open(const char* name, struct rbd_ctx* ctx)
{
  ...
  SubProcess process(
      "rbd-ggate", SubProcess::KEEP, SubProcess::PIPE, SubProcess::KEEP);
  process.add_cmd_arg("map");
  std::string img;
  img.append(pool);
  img.append("/");
...
const struct rbd_operations ggate_operations = {
    ggate_open,  ggate_close,   generic_pread,  generic_pwrite,
    ggate_flush, ggate_discard, ggate_get_size, ggate_resize,
    ggate_clone, ggate_flatten, NULL,
};
#endif // __FreeBSD__
...
static bool
rbd_image_has_parent(struct rbd_ctx* ctx)
{
  int ret;
...
logdump(void)
{
  ...
  for (; count > 0; count--) {
    ...
    case OP_TRUNCATE:
      ...
      prt("TRUNCATE %s\tfrom 0x%x to 0x%x", down ? "DOWN" : "UP", lp->args[1],
          lp->args[0]);
    ...
    case OP_FALLOCATE:
      ...
      prt("FALLOC   0x%x thru 0x%x\t(0x%x bytes) %s", lp->args[0],
          lp->args[0] + lp->args[1], lp->args[1], falloc_type[lp->args[2]]);
      if (badoff >= lp->args[0] && badoff < lp->args[0] + lp->args[1])
        prt("\t******FFFF");
      break;
    case OP_PUNCH_HOLE:
      prt("PUNCH    0x%x thru 0x%x\t(0x%x bytes)", lp->args[0],
          lp->args[0] + lp->args[1] - 1, lp->args[1]);
      if (badoff >= lp->args[0] && badoff < lp->args[0] + lp->args[1])
        prt("\t******PPPP");
      break;
    case OP_WRITESAME:
      prt("WRITESAME    0x%x thru 0x%x\t(0x%x bytes) data_size 0x%x",
          lp->args[0], lp->args[0] + lp->args[1] - 1, lp->args[1], lp->args[2]);
      if (badoff >= lp->args[0] && badoff < lp->args[0] + lp->args[1])
        prt("\t***WSWSWSWS");
      break;
    case OP_COMPARE_AND_WRITE:
      prt("COMPARE_AND_WRITE    0x%x thru 0x%x\t(0x%x bytes)", lp->args[0],
          lp->args[0] + lp->args[1] - 1, lp->args[1]);
      if (lp->args[0] > lp->args[2])
        prt(" HOLE");
      else if (lp->args[0] + lp->args[1] > lp->args[2])
...
#define short_at(cp)                                 \
  ((unsigned short)((*((unsigned char*)(cp)) << 8) | \
                    *(((unsigned char*)(cp)) + 1)))
...
check_eofpage(char* s, unsigned offset, char* p, int size)
{
  ...
  for (should_be_zero = last_page + (file_size & page_mask);
       should_be_zero < last_page + page_size; should_be_zero++)
    if (*(char*)should_be_zero) {
      prt("Mapped %s: non-zero data past EOF (0x%llx) page offset 0x%x is "
          "0x%04x\n",
...
dotruncate(unsigned size)
{
  ...
  if ((progressinterval && testcalls % progressinterval == 0) ||
      (debug &&
       (monitorstart == -1 || monitorend == -1 || (long)size <= monitorend)))
...
do_punch_hole(unsigned offset, unsigned length)
{
  ...
  ret = ops->discard(
      &ctx, (unsigned long long)offset, (unsigned long long)length);
  if (ret < 0) {
...
dowritesame(unsigned offset, unsigned size)
{
  ...
  if (!quiet && ((progressinterval && testcalls % progressinterval == 0) ||
    ...
    prt("%lu writesame\t0x%x thru\t0x%x\tdata_size\t0x%x(0x%x bytes)\n",
        testcalls, offset, offset + size - 1, data_size, size);
...
docompareandwrite(unsigned offset, unsigned size)
{
  ...
  ret = ops->compare_and_write(
      &ctx, offset, size, temp_buf + offset, good_buf + offset);
  if (ret != (ssize_t)size) {
    if (ret == -EINVAL) {
...
void
clone_filename(char* buf, size_t len, int clones)
{
...
void
clone_imagename(char* buf, size_t len, int clones)
{
  if (clones > 0) {
...
void
replay_imagename(char* buf, size_t len, int clones)
{
...
do_clone()
{
  ...
  if (journal_replay) {
    ret = finalize_journal(
        ioctx, lastimagename, num_clones - 1, order, stripe_unit, stripe_count);
    if (ret < 0) {
      exit(EXIT_FAILURE);
    }
...
check_clone(int clonenum, bool replay_image)
{
  ...
  if (size != (uint64_t)file_info.st_size) {
    prt("check_clone: image size 0x%llx != file size 0x%llx\n",
        (unsigned long long)size, (unsigned long long)file_info.st_size);
    exit(175);
  }
  ...
  if (file_info.st_size > 0) {
    ret = posix_memalign(
        (void**)&good_buf, std::max(writebdy, (int)sizeof(void*)),
        file_info.st_size);
    if (ret > 0) {
    ...
    ret = posix_memalign(
        (void**)&temp_buf, std::max(readbdy, (int)sizeof(void*)),
        file_info.st_size);
    if (ret > 0) {
...
main(int argc, char** argv)
{
  ...
  const struct option longopts[] = {
      {"cluster", 1, NULL, LONG_OPT_CLUSTER}, {"id", 1, NULL, LONG_OPT_ID}};
  ...
  ret = posix_memalign(
      (void**)&good_buf, std::max(writebdy, (int)sizeof(void*)), maxfilelen);
  if (ret > 0) {
    if (ret == EINVAL)
  ...
  ret = posix_memalign(
      (void**)&temp_buf, std::max(readbdy, (int)sizeof(void*)), maxfilelen);
  if (ret > 0) {
    if (ret == EINVAL)
  ...
  if (lite) { /* zero entire existing file */
    ...
    if (written != (ssize_t)maxfilelen) {
      if (written < 0) {
        ...
        warn(
            "main: short write, 0x%x bytes instead "
            "of 0x%lx\n",
