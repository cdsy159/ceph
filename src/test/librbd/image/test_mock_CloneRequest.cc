...
MetadataCopyRequest<MockTestImageCtx>*
    MetadataCopyRequest<MockTestImageCtx>::s_instance = nullptr;
...
AttachChildRequest<MockTestImageCtx>*
    AttachChildRequest<MockTestImageCtx>::s_instance = nullptr;
...
EnableRequest<MockTestImageCtx>* EnableRequest<MockTestImageCtx>::s_instance =
    nullptr;
...
public:
  ...
  typedef deep_copy::MetadataCopyRequest<MockTestImageCtx>
  ...
  void
  expect_get_min_compat_client(int8_t min_compat_client, int r)
  {
    ...
    EXPECT_CALL(*mock_rados_client, get_min_compatible_client(_, _))
        .WillOnce(Invoke([min_compat_client,
                          r](int8_t* min, int8_t* required_min) {
  ...
  void
  expect_open(MockImageCtx& mock_image_ctx, int r)
  {
  ...
  void
  expect_attach_parent(MockAttachParentRequest& mock_request, int r)
  {
  ...
  void
  expect_attach_child(
      MockAttachChildRequest& mock_request,
      uint32_t clone_format,
      int r)
  {
  ...
  void
  expect_metadata_copy(MockMetadataCopyRequest& mock_request, int r)
  {
    EXPECT_CALL(mock_request, send()).WillOnce(Invoke([this, &mock_request, r]() {
      image_ctx->op_work_queue->queue(mock_request.on_finish, r);
    }));
      ...
      int r)
  {
    ...
    EXPECT_CALL(
        ...
        exec(
            RBD_MIRRORING, _, StrEq("rbd"), StrEq("mirror_mode_get"), _, _, _,
            _))
  ...
  void
  expect_mirror_enable(MockMirrorEnableRequest& mock_mirror_enable_request, int r)
  {
    EXPECT_CALL(mock_mirror_enable_request, send())
        .WillOnce(Invoke([this, &mock_mirror_enable_request, r]() {
          image_ctx->op_work_queue->queue(
              mock_mirror_enable_request.on_finish, r);
        }));
  }

  void
  expect_close(MockImageCtx& mock_image_ctx, int r)
  {
...
TEST_F(TestMockImageCloneRequest, SuccessV1)
{
  ...
  auto req = new MockCloneRequest(
      m_cct->_conf, m_ioctx, "parent id", "", {}, 123, m_ioctx, "clone name",
...
TEST_F(TestMockImageCloneRequest, SnapshotMirrorEnableNonPrimary)
{
  ...
  auto req = new MockCloneRequest(
      m_cct->_conf, m_ioctx, "parent id", "", {}, 123, m_ioctx, "clone name",
      "clone id", clone_opts, cls::rbd::MIRROR_IMAGE_MODE_JOURNAL,
      "global image id", "primary mirror uuid", image_ctx->op_work_queue, &ctx);
  req->send();
  ASSERT_EQ(0, ctx.wait());
}
