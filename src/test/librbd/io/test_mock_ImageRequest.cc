...
struct MockTestJournal : public MockJournal {
  MOCK_METHOD3(
      append_write_event,
      uint64_t(const io::Extents&, const bufferlist&, bool));
  MOCK_METHOD3(
      append_write_same_event,
      uint64_t(const io::Extents&, const bufferlist&, bool));
  MOCK_METHOD5(
      append_compare_and_write_event,
      uint64_t(uint64_t, size_t, const bufferlist&, const bufferlist&, bool));
  MOCK_METHOD3(
      append_discard_event,
      uint64_t(const io::Extents&, uint32_t, bool));
  MOCK_METHOD5(
      append_io_event_mock,
      uint64_t(const journal::EventEntry&, uint64_t, size_t, bool, int));
      ...
      int filter_ret_val)
  {
    ...
    return append_io_event_mock(
        event_entry, offset, length, flush_entry, filter_ret_val);
  }
...
struct TestMockIoImageRequest : public TestMockFixture {
  ...
  typedef ImageWriteSameRequest<librbd::MockTestImageCtx>
      MockImageWriteSameRequest;
  typedef ImageCompareAndWriteRequest<librbd::MockTestImageCtx>
      MockImageCompareAndWriteRequest;
  typedef ImageListSnapsRequest<librbd::MockTestImageCtx>
      MockImageListSnapsRequest;

  void
  expect_is_journal_appending(MockTestJournal& mock_journal, bool appending)
  {
    EXPECT_CALL(mock_journal, is_journal_appending()).WillOnce(Return(appending));
  }

  void
  expect_get_modify_timestamp(MockTestImageCtx& mock_image_ctx, bool needs_update)
  {
    if (needs_update) {
      ...
      int r)
  {
    EXPECT_CALL(*mock_image_ctx.io_object_dispatcher, send(_))
        .WillOnce(Invoke([&mock_image_ctx, object_no, offset, length,
                          r](ObjectDispatchSpec* spec) {
          auto* discard_spec =
              std::get_if<ObjectDispatchSpec::DiscardRequest>(&spec->request);
  ...
  void
  expect_object_request_send(MockTestImageCtx& mock_image_ctx, int r)
  {
    EXPECT_CALL(*mock_image_ctx.io_object_dispatcher, send(_))
  ...
  void
  expect_object_list_snaps_request(
      MockTestImageCtx& mock_image_ctx,
      uint64_t object_no,
  ...
  {
    EXPECT_CALL(*mock_image_ctx.io_object_dispatcher, send(_))
        .WillOnce(Invoke([&mock_image_ctx, object_no, snap_delta,
                          r](ObjectDispatchSpec* spec) {
          auto request =
              std::get_if<librbd::io::ObjectDispatchSpec::ListSnapsRequest>(
                  &spec->request);
...
TEST_F(TestMockIoImageRequest, AioWriteModifyTimestamp)
{
  ...
  AioCompletion* aio_comp_1 =
      AioCompletion::create_and_start(&aio_comp_ctx_1, ictx, AIO_TYPE_WRITE);

  AioCompletion* aio_comp_2 =
      AioCompletion::create_and_start(&aio_comp_ctx_2, ictx, AIO_TYPE_WRITE);
...
TEST_F(TestMockIoImageRequest, AioReadAccessTimestamp)
{
  ...
  C_SaferCond aio_comp_ctx_1, aio_comp_ctx_2;
  AioCompletion* aio_comp_1 =
      AioCompletion::create_and_start(&aio_comp_ctx_1, ictx, AIO_TYPE_READ);
  ...
  AioCompletion* aio_comp_2 =
      AioCompletion::create_and_start(&aio_comp_ctx_2, ictx, AIO_TYPE_READ);
  expect_object_request_send(mock_image_ctx, 0);
...
TEST_F(TestMockIoImageRequest, PartialDiscard)
{
  ...
  AioCompletion* aio_comp =
      AioCompletion::create_and_start(&aio_comp_ctx, ictx, AIO_TYPE_DISCARD);
...
TEST_F(TestMockIoImageRequest, AioWriteJournalAppendDisabled)
{
  ...
  C_SaferCond aio_comp_ctx;
  AioCompletion* aio_comp =
      AioCompletion::create_and_start(&aio_comp_ctx, ictx, AIO_TYPE_WRITE);
...
TEST_F(TestMockIoImageRequest, AioFlushJournalAppendDisabled)
{
  ...
  C_SaferCond aio_comp_ctx;
  AioCompletion* aio_comp =
      AioCompletion::create_and_start(&aio_comp_ctx, ictx, AIO_TYPE_FLUSH);
  MockImageFlushRequest mock_aio_image_flush(
      mock_image_ctx, aio_comp, FLUSH_SOURCE_USER, {});
  {
    std::shared_lock owner_locker{mock_image_ctx.owner_lock};
...
TEST_F(TestMockIoImageRequest, AioWriteSameJournalAppendDisabled)
{
  ...
  C_SaferCond aio_comp_ctx;
  AioCompletion* aio_comp =
      AioCompletion::create_and_start(&aio_comp_ctx, ictx, AIO_TYPE_WRITESAME);
...
TEST_F(TestMockIoImageRequest, AioCompareAndWriteJournalAppendDisabled)
{
  ...
  MockImageCompareAndWriteRequest mock_aio_image_write(
      mock_image_ctx, aio_comp, {{0, 1}}, ImageArea::DATA, std::move(cmp_bl),
      std::move(write_bl), &mismatch_offset, 0, {});
  {
    std::shared_lock owner_locker{mock_image_ctx.owner_lock};
    mock_aio_image_write.send();
  }
...
TEST_F(TestMockIoImageRequest, ListSnaps)
{
  ...
  C_SaferCond aio_comp_ctx;
  AioCompletion* aio_comp =
      AioCompletion::create_and_start(&aio_comp_ctx, ictx, AIO_TYPE_GENERIC);
