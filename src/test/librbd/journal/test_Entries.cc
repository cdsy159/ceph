...
public:
  ...
  ::journal::Journaler*
  create_journaler(librbd::ImageCtx* ictx)
  {
  ...
  bool
  wait_for_entries_available(librbd::ImageCtx* ictx)
  {
    using namespace std::chrono_literals;
    std::unique_lock locker{m_replay_handler.lock};
    while (!m_replay_handler.entries_available) {
      if (m_replay_handler.cond.wait_for(locker, 10s) ==
  ...
  bool
  get_event_entry(
      const ::journal::ReplayEntry& replay_entry,
...
TEST_F(TestJournalEntries, AioWrite)
{
  ...
  c->get();
  api::Io<>::aio_write(
      *ictx, c, 123, buffer.size(), std::move(write_bl), 0, true);
  ...
  ASSERT_EQ(
      librbd::journal::AioWriteEvent::get_fixed_size() +
          aio_write_event.data.length(),
...
TEST_F(TestJournalEntries, AioDiscard)
{
  ...
  c->get();
  api::Io<>::aio_discard(
      *ictx, c, 123, 234, ictx->discard_granularity_bytes, true);
  ASSERT_EQ(0, c->wait_for_complete());
  c->put();
  ...
  ASSERT_EQ(
      librbd::journal::EVENT_TYPE_AIO_DISCARD, event_entry.get_event_type());
...
TEST_F(TestJournalEntries, AioDiscardWithPrune)
{
  ...
  api::Io<>::aio_discard(
      *ictx, c, object_size - 4096, 2 * object_size, 8192, true);
  ASSERT_EQ(0, c->wait_for_complete());
  c->put();
