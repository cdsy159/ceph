...
public:
  int
  when_acquired_lock(librbd::ImageCtx* ictx)
  {
  ...
  template <typename T>
  void
  inject_into_journal(librbd::ImageCtx* ictx, T event)
  {
    C_SaferCond ctx;
    librbd::journal::EventEntry event_entry(event);
    {
      ...
      uint64_t tid =
          ictx->journal->append_io_event(std::move(event_entry), 0, 0, true, 0);
      ...
      int64_t* entry)
  {
    ...
    cls::journal::client::get_mutable_metadata(
        ictx->md_ctx, oid, &minimum_set, &active_set, &registered_clients,
        &cond);
...
TEST_F(TestJournalReplay, AioDiscardEvent)
{
  ...
  api::Io<>::aio_write(
      *ictx, aio_comp, 0, payload.size(), std::move(payload_bl), 0, true);
  ...
  aio_comp = new librbd::io::AioCompletion();
  api::Io<>::aio_read(
      *ictx, aio_comp, 0, read_payload.size(),
  ...
  inject_into_journal(
      ictx, librbd::journal::AioDiscardEvent(
  ...
  aio_comp = new librbd::io::AioCompletion();
  api::Io<>::aio_discard(
      *ictx, aio_comp, 0, read_payload.size(), ictx->discard_granularity_bytes,
      true);
  ASSERT_EQ(0, aio_comp->wait_for_complete());
  aio_comp->release();
}
...
TEST_F(TestJournalReplay, AioWriteEvent)
{
  ...
  payload_bl.append(payload);
  inject_into_journal(
      ictx, librbd::journal::AioWriteEvent(0, payload.size(), payload_bl));
  close_image(ictx);
  ...
  auto aio_comp = new librbd::io::AioCompletion();
  api::Io<>::aio_read(
      *ictx, aio_comp, 0, read_payload.size(), std::move(read_result), 0, true);
  ASSERT_EQ(0, aio_comp->wait_for_complete());
  aio_comp->release();
  ASSERT_EQ(payload, read_payload);
  ...
  aio_comp = new librbd::io::AioCompletion();
  api::Io<>::aio_write(
      *ictx, aio_comp, 0, payload.size(), bufferlist{payload_bl}, 0, true);
  ASSERT_EQ(0, aio_comp->wait_for_complete());
  aio_comp->release();
}
...
TEST_F(TestJournalReplay, SnapCreate)
{
  ...
  inject_into_journal(
      ictx, librbd::journal::SnapCreateEvent(
...

  {
    ...
    ASSERT_NE(
        CEPH_NOSNAP,
        ictx->get_snap_id(cls::rbd::UserSnapshotNamespace(), "snap"));
  }
  ...
  ASSERT_EQ(
      0, ictx->operations->snap_create(
             cls::rbd::UserSnapshotNamespace(), "snap2", 0, no_op_progress));
}
...
TEST_F(TestJournalReplay, SnapProtect)
{
  ...
  ASSERT_EQ(
      0, ictx->operations->snap_create(
             cls::rbd::UserSnapshotNamespace(), "snap", 0, no_op_progress));
  ...
  // inject snapshot ops into journal
  inject_into_journal(
      ictx, librbd::journal::SnapProtectEvent(
                1, cls::rbd::UserSnapshotNamespace(), "snap"));
  inject_into_journal(ictx, librbd::journal::OpFinishEvent(1, 0));
  close_image(ictx);
  ...
  bool is_protected;
  ASSERT_EQ(
      0, librbd::api::Snapshot<>::is_protected(ictx, "snap", &is_protected));
...
TEST_F(TestJournalReplay, SnapUnprotect)
{
  ...
  ASSERT_EQ(
      0,
      ictx->operations->snap_protect(cls::rbd::UserSnapshotNamespace(), "snap"));
  ...
  // inject snapshot ops into journal
  inject_into_journal(
      ictx, librbd::journal::SnapUnprotectEvent(
                1, cls::rbd::UserSnapshotNamespace(), "snap"));
  inject_into_journal(ictx, librbd::journal::OpFinishEvent(1, 0));
  close_image(ictx);
...
TEST_F(TestJournalReplay, SnapRename)
{
  ...
  // inject snapshot ops into journal
  inject_into_journal(
      ictx, librbd::journal::SnapRenameEvent(1, snap_id, "snap", "snap2"));
  inject_into_journal(ictx, librbd::journal::OpFinishEvent(1, 0));
  close_image(ictx);
...
TEST_F(TestJournalReplay, SnapRollback)
{
  ...
  // inject snapshot ops into journal
  inject_into_journal(
      ictx, librbd::journal::SnapRollbackEvent(
                1, cls::rbd::UserSnapshotNamespace(), "snap"));
  inject_into_journal(ictx, librbd::journal::OpFinishEvent(1, 0));
  close_image(ictx);
  ...
  ASSERT_EQ(
      0, ictx->operations->snap_rollback(
             cls::rbd::UserSnapshotNamespace(), "snap", no_op_progress));
}
...
TEST_F(TestJournalReplay, SnapRemove)
{
  ...
  // inject snapshot ops into journal
  inject_into_journal(
      ictx, librbd::journal::SnapRemoveEvent(
                1, cls::rbd::UserSnapshotNamespace(), "snap"));
  inject_into_journal(ictx, librbd::journal::OpFinishEvent(1, 0));
  close_image(ictx);
  ...
  ASSERT_EQ(
      0,
      ictx->operations->snap_remove(cls::rbd::UserSnapshotNamespace(), "snap"));
}
...
TEST_F(TestJournalReplay, Rename)
{
  ...
  librbd::RBD rbd;
  ASSERT_EQ(
      0, rbd.rename(m_ioctx, new_image_name.c_str(), m_image_name.c_str()));
}
...
TEST_F(TestJournalReplay, UpdateFeatures)
{
  ...
  inject_into_journal(
      ictx, librbd::journal::UpdateFeaturesEvent(1, features, enabled));
  close_image(ictx);
...
TEST_F(TestJournalReplay, MetadataSet)
{
  ...
  inject_into_journal(
      ictx, librbd::journal::MetadataSetEvent(
  ...
  std::string value;
  ASSERT_EQ(
      0, librbd::metadata_get(ictx, "conf_rbd_mirroring_replay_delay", &value));
...
TEST_F(TestJournalReplay, MetadataRemove)
{
  ...
  ASSERT_EQ(
      0,
      ictx->operations->metadata_set("conf_rbd_mirroring_replay_delay", "9876"));
  ...
  inject_into_journal(
      ictx, librbd::journal::MetadataRemoveEvent(
  ...
  ASSERT_EQ(
      -ENOENT,
      librbd::metadata_get(ictx, "conf_rbd_mirroring_replay_delay", &value));
