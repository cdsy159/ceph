...
struct ImageRequest<MockReplayImageCtx> {
  ...
  {
    ceph_assert(s_instance != nullptr);
    s_instance->aio_compare_and_write(
        c, image_extents, cmp_bl, bl, mismatch_offset, op_flags);
  }

  ImageRequest() { s_instance = this; }
};

ImageRequest<MockReplayImageCtx>* ImageRequest<MockReplayImageCtx>::s_instance =
    nullptr;
...
inline ImageCtx*
get_image_ctx(librbd::MockReplayImageCtx* image_ctx)
{
...
MATCHER_P(CStrEq, str, "") { return (strncmp(arg, str, strlen(str)) == 0); }
...
{
  image_ctx->op_work_queue->queue(
      new LambdaContext([this, arg0](int r) {
        arg0->get();
...
public:
  ...
  void
  expect_accept_ops(MockExclusiveLock& mock_exclusive_lock, bool accept)
  {
      ...
      uint32_t discard_granularity_bytes)
  {
    EXPECT_CALL(
        ...
        aio_discard(_, io::Extents{{off, len}}, discard_granularity_bytes))
      ...
      const char* data)
  {
    EXPECT_CALL(
        mock_io_image_request,
        aio_writesame(_, io::Extents{{off, len}}, BufferlistEqual(data), _))
        .WillOnce(SaveArg<0>(aio_comp));
  }

  void
  expect_aio_compare_and_write(
      MockIoImageRequest& mock_io_image_request,
      io::AioCompletion** aio_comp,
      uint64_t off,
      uint64_t len,
      ...
      uint64_t* mismatch_offset)
  {
    EXPECT_CALL(
        mock_io_image_request,
        aio_compare_and_write(
            _, io::Extents{{off, len}}, BufferlistEqual(cmp_data),
            BufferlistEqual(data), mismatch_offset, _))
        .WillOnce(SaveArg<0>(aio_comp));
  }

  void
  expect_flatten(MockReplayImageCtx& mock_image_ctx, Context** on_finish)
  {
  ...
  {
    EXPECT_CALL(
        *mock_image_ctx.operations, execute_resize(size, _, _, _, op_tid))
        .WillOnce(DoAll(
            SaveArg<3>(on_finish),
            NotifyInvoke(&m_invoke_lock, &m_invoke_cond)));
  }
  ...
  {
    EXPECT_CALL(
        *mock_image_ctx.operations, execute_snap_create(
                                        _, StrEq(snap_name), _, op_tid,
  ...
  {
    EXPECT_CALL(
        *mock_image_ctx.operations, execute_snap_remove(_, StrEq(snap_name), _))
        .WillOnce(DoAll(
            SaveArg<2>(on_finish),
            NotifyInvoke(&m_invoke_lock, &m_invoke_cond)));
  }
  ...
  {
    EXPECT_CALL(
        *mock_image_ctx.operations,
        execute_snap_rename(snap_id, StrEq(snap_name), _))
        .WillOnce(DoAll(
            SaveArg<2>(on_finish),
            NotifyInvoke(&m_invoke_lock, &m_invoke_cond)));
  }
  ...
  {
    EXPECT_CALL(
        *mock_image_ctx.operations, execute_snap_protect(_, StrEq(snap_name), _))
        .WillOnce(DoAll(
            SaveArg<2>(on_finish),
            NotifyInvoke(&m_invoke_lock, &m_invoke_cond)));
  }
      ...
      const char* snap_name)
  {
    EXPECT_CALL(
        *mock_image_ctx.operations,
        execute_snap_unprotect(_, StrEq(snap_name), _))
        .WillOnce(DoAll(
            SaveArg<2>(on_finish),
            NotifyInvoke(&m_invoke_lock, &m_invoke_cond)));
  }
      ...
      const char* snap_name)
  {
    EXPECT_CALL(
        *mock_image_ctx.operations,
        execute_snap_rollback(_, StrEq(snap_name), _, _))
        .WillOnce(DoAll(
            SaveArg<3>(on_finish),
            NotifyInvoke(&m_invoke_lock, &m_invoke_cond)));
  }
      ...
      uint64_t op_tid)
  {
    EXPECT_CALL(
        *mock_image_ctx.operations,
        execute_update_features(features, enabled, _, op_tid))
        .WillOnce(DoAll(
            SaveArg<2>(on_finish),
            NotifyInvoke(&m_invoke_lock, &m_invoke_cond)));
  }
      ...
      const char* value)
  {
    EXPECT_CALL(
        *mock_image_ctx.operations,
        execute_metadata_set(StrEq(key), StrEq(value), _))
        .WillOnce(DoAll(
            SaveArg<2>(on_finish),
            NotifyInvoke(&m_invoke_lock, &m_invoke_cond)));
  }
  ...
  {
    EXPECT_CALL(
        *mock_image_ctx.operations, execute_metadata_remove(StrEq(key), _))
        .WillOnce(DoAll(
            SaveArg<1>(on_finish),
            NotifyInvoke(&m_invoke_lock, &m_invoke_cond)));
  }
  ...
  void
  expect_refresh_image(MockReplayImageCtx& mock_image_ctx, bool required, int r)
  {
  ...
  int
  when_flush(MockJournalReplay& mock_journal_replay)
  {
    C_SaferCond ctx;
  ...
  int
  when_shut_down(MockJournalReplay& mock_journal_replay, bool cancel_ops)
  {
    C_SaferCond ctx;
  ...
  void
  wait_for_op_invoked(Context** on_finish, int r)
  {
    {
...
TEST_F(TestMockJournalReplay, AioDiscard)
{
  ...
  C_SaferCond on_safe;
  expect_aio_discard(
      mock_io_image_request, &aio_comp, 123, 456,
...
TEST_F(TestMockJournalReplay, AioWrite)
{
  ...
  when_process(
      mock_journal_replay, EventEntry{AioWriteEvent(123, 456, to_bl("test"))},
      &on_ready, &on_safe);
...
TEST_F(TestMockJournalReplay, AioFlush)
{
  ...
  when_process(
      mock_journal_replay, EventEntry{AioFlushEvent()}, &on_ready, &on_safe);
...
TEST_F(TestMockJournalReplay, AioWriteSame)
{
  ...
  when_process(
      mock_journal_replay,
      EventEntry{AioWriteSameEvent(123, 456, to_bl("333"))}, &on_ready,
      &on_safe);
...
TEST_F(TestMockJournalReplay, AioCompareAndWrite)
{
  ...
  when_process(
      mock_write_journal_replay,
      EventEntry{AioWriteEvent(512, 512, to_bl("test"))}, &on_ready, &on_safe);
  ...
  expect_aio_compare_and_write(
      mock_io_image_request, &aio_comp, 512, 512, "test", "test", nullptr);
  when_process(
      mock_compare_and_write_journal_replay,
      EventEntry{
          AioCompareAndWriteEvent(512, 512, to_bl("test"), to_bl("test"))},
      &on_ready, &on_safe);
  ...
  expect_aio_compare_and_write(
      mock_io_image_request, &aio_comp, 512, 512, "111", "test", nullptr);
  when_process(
      mock_mis_compare_and_write_journal_replay,
...
TEST_F(TestMockJournalReplay, OpFinishError)
{
  ...
  when_process(
      mock_journal_replay,
      EventEntry{
          SnapRemoveEvent(123, cls::rbd::UserSnapshotNamespace(), "snap")},
  ...
  when_process(
      mock_journal_replay, EventEntry{OpFinishEvent(123, -EIO)},
...
TEST_F(TestMockJournalReplay, BlockedOpFinishError)
{
  ...
  when_process(
      mock_journal_replay,
      EventEntry{
          SnapCreateEvent(123, cls::rbd::UserSnapshotNamespace(), "snap")},
      &on_start_ready, &on_start_safe);
  ...
  C_SaferCond on_finish_safe;
  when_process(
      mock_journal_replay, EventEntry{OpFinishEvent(123, -EBADMSG)},
      &on_finish_ready, &on_finish_safe);
...
TEST_F(TestMockJournalReplay, MissingOpFinishEvent)
{
  ...
  when_process(
      mock_journal_replay,
      EventEntry{
          SnapRemoveEvent(122, cls::rbd::UserSnapshotNamespace(), "snap")},
...
TEST_F(TestMockJournalReplay, UnknownOpFinishEvent)
{
  ...
  C_SaferCond on_safe;
  when_process(
      mock_journal_replay, EventEntry{OpFinishEvent(123, 0)}, &on_ready,
      &on_safe);
...
TEST_F(TestMockJournalReplay, OpEventError)
{
  ...
  C_SaferCond on_finish_safe;
  when_process(
      mock_journal_replay, EventEntry{OpFinishEvent(123, 0)}, &on_finish_ready,
      &on_finish_safe);
...
TEST_F(TestMockJournalReplay, SnapRenameEvent)
{
  ...
  when_process(
      mock_journal_replay,
      EventEntry{SnapRenameEvent(123, 234, "snap1", "snap")}, &on_start_ready,
      &on_start_safe);
  ASSERT_EQ(0, on_start_ready.wait());
...
TEST_F(TestMockJournalReplay, SnapProtectEvent)
{
  ...
  when_process(
      mock_journal_replay,
      EventEntry{
          SnapProtectEvent(123, cls::rbd::UserSnapshotNamespace(), "snap")},
      &on_start_ready, &on_start_safe);
  ASSERT_EQ(0, on_start_ready.wait());
...
TEST_F(TestMockJournalReplay, SnapUnprotectEvent)
{
  ...
  when_process(
      mock_journal_replay,
      EventEntry{
          SnapUnprotectEvent(123, cls::rbd::UserSnapshotNamespace(), "snap")},
      &on_start_ready, &on_start_safe);
  ASSERT_EQ(0, on_start_ready.wait());
...
TEST_F(TestMockJournalReplay, SnapUnprotectOpFinishBusy)
{
  ...
  C_SaferCond on_finish_safe;
  when_process(
      mock_journal_replay, EventEntry{OpFinishEvent(123, -EBUSY)},
      &on_finish_ready, &on_finish_safe);
...
TEST_F(TestMockJournalReplay, SnapRollbackEvent)
{
  ...
  when_process(
      mock_journal_replay,
      EventEntry{
          SnapRollbackEvent(123, cls::rbd::UserSnapshotNamespace(), "snap")},
      &on_start_ready, &on_start_safe);
  ASSERT_EQ(0, on_start_ready.wait());
...
TEST_F(TestMockJournalReplay, RenameEvent)
{
  ...
  C_SaferCond on_start_safe;
  when_process(
      mock_journal_replay, EventEntry{RenameEvent(123, "image")},
      &on_start_ready, &on_start_safe);
  ASSERT_EQ(0, on_start_ready.wait());
...
TEST_F(TestMockJournalReplay, ResizeEvent)
{
  ...
  C_SaferCond on_start_safe;
  when_process(
      mock_journal_replay, EventEntry{ResizeEvent(123, 234)}, &on_start_ready,
      &on_start_safe);
...
TEST_F(TestMockJournalReplay, FlattenEvent)
{
  ...
  C_SaferCond on_start_safe;
  when_process(
      mock_journal_replay, EventEntry{FlattenEvent(123)}, &on_start_ready,
      &on_start_safe);
  ASSERT_EQ(0, on_start_ready.wait());
...
TEST_F(TestMockJournalReplay, UpdateFeaturesEvent)
{
  ...
  when_process(
      mock_journal_replay,
      EventEntry{UpdateFeaturesEvent(123, features, enabled)}, &on_start_ready,
      &on_start_safe);
...
TEST_F(TestMockJournalReplay, MetadataSetEvent)
{
  ...
  C_SaferCond on_start_safe;
  when_process(
      mock_journal_replay, EventEntry{MetadataSetEvent(123, "key", "value")},
      &on_start_ready, &on_start_safe);
  ASSERT_EQ(0, on_start_ready.wait());
...
TEST_F(TestMockJournalReplay, MetadataRemoveEvent)
{
  ...
  C_SaferCond on_start_safe;
  when_process(
      mock_journal_replay, EventEntry{MetadataRemoveEvent(123, "key")},
      &on_start_ready, &on_start_safe);
  ASSERT_EQ(0, on_start_ready.wait());
...
TEST_F(TestMockJournalReplay, OpEventAfterShutDown)
{
  ...
  C_SaferCond on_safe;
  when_process(
      mock_journal_replay, EventEntry{RenameEvent(123, "image")}, &on_ready,
      &on_safe);
  ASSERT_EQ(0, on_ready.wait());
  ASSERT_EQ(-ESHUTDOWN, on_safe.wait());
}
