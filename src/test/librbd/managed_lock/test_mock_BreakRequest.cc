...
GetLockerRequest<librbd::MockTestImageCtx>*
    GetLockerRequest<librbd::MockTestImageCtx>::s_instance = nullptr;
...
public:
      ...
      uint64_t watch_handle)
  {
    ...
    if (r < 0) {
      ...
      obj_watch_t watcher;
      strncpy(
          watcher.addr, (address + ":0/0").c_str(), sizeof(watcher.addr) - 1);
  ...
  void
  expect_blocklist_add(MockTestImageCtx& mock_image_ctx, int r)
  {
    auto& mock_rados_client =
        librados::get_mock_rados_client(mock_image_ctx.rados_api);
    EXPECT_CALL(mock_rados_client, blocklist_add(_, _)).WillOnce(Return(r));
  }

  void
  expect_wait_for_latest_osd_map(MockTestImageCtx& mock_image_ctx, int r)
  {
    auto& mock_rados_client =
        librados::get_mock_rados_client(mock_image_ctx.rados_api);
  ...
  void
  expect_break_lock(MockTestImageCtx& mock_image_ctx, int r)
  {
    EXPECT_CALL(
        get_mock_io_ctx(mock_image_ctx.md_ctx),
        exec(
            mock_image_ctx.header_oid, _, StrEq("lock"), StrEq("break_lock"), _,
            _, _, _))
        .WillOnce(Return(r));
  }

  void
  expect_get_instance_id(MockTestImageCtx& mock_image_ctx, uint64_t id)
  {
...
TEST_F(TestMockManagedLockBreakRequest, GetLockerUpdated)
{
  ...
  expect_get_locker(
      mock_image_ctx, mock_get_locker_request,
      {entity_name_t::CLIENT(2), "auto 123", "1.2.3.4:0/0", 123}, 0);
...
TEST_F(TestMockManagedLockBreakRequest, GetLockerBusy)
{
  ...
  expect_get_locker(
      mock_image_ctx, mock_get_locker_request,
      {entity_name_t::CLIENT(1), "auto 123", "1.2.3.4:0/0", 123}, -EBUSY);
...
TEST_F(TestMockManagedLockBreakRequest, GetLockerMissing)
{
  ...
  expect_get_locker(
      mock_image_ctx, mock_get_locker_request,
      {entity_name_t::CLIENT(1), "auto 123", "1.2.3.4:0/0", 123}, -ENOENT);
...
TEST_F(TestMockManagedLockBreakRequest, BlocklistSelf)
{
  ...
  expect_get_locker(
      mock_image_ctx, mock_get_locker_request,
      {entity_name_t::CLIENT(456), "auto 123", "1.2.3.4:0/0", 123}, 0);
