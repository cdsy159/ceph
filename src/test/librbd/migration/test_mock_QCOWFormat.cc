...
struct SourceSpecBuilder<librbd::MockTestImageCtx> {
  ...
  MOCK_CONST_METHOD2(
      build_stream,
      int(const json_spirit::mObject&, std::shared_ptr<StreamInterface>*));
};

} // namespace migration

bool
operator==(const SnapInfo& lhs, const SnapInfo& rhs)
{
...
public:
      ...
      int r)
  {
    EXPECT_CALL(mock_source_spec_builder, build_stream(_, _))
        .WillOnce(WithArgs<1>(Invoke(
            [mock_stream_interface, r](std::shared_ptr<StreamInterface>* ptr) {
  ...
  void
  expect_stream_open(MockStreamInterface& mock_stream_interface, int r)
  {
  ...
  void
  expect_stream_close(MockStreamInterface& mock_stream_interface, int r)
  {
      ...
      int r)
  {
    EXPECT_CALL(mock_stream_interface, read(byte_extents, _, _))
        .WillOnce(
            WithArgs<1, 2>(Invoke([bl, r](bufferlist* out_bl, Context* ctx) {
              *out_bl = bl;
              ctx->complete(r);
            })));
  }
      ...
      int r)
  {
    ...
    header_bl.append(std::string_view(
        reinterpret_cast<char*>(&qcow_header), sizeof(qcow_header)));
    expect_stream_read(
        mock_stream_interface, {{0, sizeof(qcow_header)}}, header_bl, r);
  }

  void
  expect_read_l1_table(
      MockStreamInterface& mock_stream_interface,
      ...
      int r)
  {
    ...
    l2_table_bl.append(std::string_view(
        reinterpret_cast<char*>(l2_table->data()),
        l2_table->size() * sizeof(uint64_t)));
    expect_stream_read(
        mock_stream_interface,
        {{l2_table_offset, l2_table->size() * sizeof(uint64_t)}}, l2_table_bl,
        r);
  }
      ...
      int r)
  {
    ...
    expect_stream_read(
        mock_stream_interface, {{cluster_offset, cluster_bl.length()}},
        cluster_bl, r);
  }
      ...
      int r)
  {
    ...
    snapshot_header_bl.append(std::string_view(
        reinterpret_cast<char*>(&snapshot_header), sizeof(snapshot_header)));
    expect_stream_read(
        mock_stream_interface, {{*snapshot_offset, sizeof(snapshot_header)}},
        snapshot_header_bl, r);
    *snapshot_offset += sizeof(snapshot_header);
  }

  void
  expect_read_snapshot_header_extra(
      MockStreamInterface& mock_stream_interface,
      const std::string& id,
      const std::string& name,
      uint64_t size,
      ...
      int r)
  {
    ...
    expect_stream_read(
        mock_stream_interface, {{l1_table_offset, 8}}, snapshot_l1_table_bl, r);
  }
  ...
  {
    expect_read_snapshot_header(
        mock_stream_interface, id, name, l1_table_offset, snapshot_offset, 0);
    expect_read_snapshot_header_extra(
        mock_stream_interface, id, name, size, snapshot_offset, 0);
...
TEST_F(TestMockMigrationQCOWFormat, OpenCloseV1)
{
  ...
  header_bl.append(std::string_view(
      reinterpret_cast<char*>(&qcow_header), sizeof(qcow_header)));
  expect_stream_read(
      *mock_stream_interface, {{0, sizeof(qcow_header)}}, header_bl, 0);
  ...
  MockQCOWFormat mock_qcow_format(
...
TEST_F(TestMockMigrationQCOWFormat, ReadHeaderV1Error)
{
  ...
  header_bl.append(std::string_view(
      reinterpret_cast<char*>(&qcow_header), sizeof(qcow_header)));
  expect_stream_read(
      *mock_stream_interface, {{0, sizeof(qcow_header)}}, header_bl, -EPERM);
...
TEST_F(TestMockMigrationQCOWFormat, Snapshots)
{
  ...
  expect_read_snapshot(
      *mock_stream_interface, "1", "snap1", 1 << 29, 1 << 22, &snapshot_offset);
  expect_read_snapshot(
      *mock_stream_interface, "2", "snap2", 1 << 29, 1 << 22, &snapshot_offset);
...
TEST_F(TestMockMigrationQCOWFormat, SnapshotHeaderError)
{
  ...
  uint64_t snapshot_offset = 1 << 21;
  expect_read_snapshot_header(
      *mock_stream_interface, "1", "snap1", 1 << 22, &snapshot_offset, -EINVAL);
...
TEST_F(TestMockMigrationQCOWFormat, SnapshotExtraError)
{
  ...
  uint64_t snapshot_offset = 1 << 21;
  expect_read_snapshot_header(
      *mock_stream_interface, "1", "snap1", 1 << 22, &snapshot_offset, 0);
  expect_read_snapshot_header_extra(
      *mock_stream_interface, "1", "snap1", 1 << 29, &snapshot_offset, -EINVAL);
...
TEST_F(TestMockMigrationQCOWFormat, SnapshotL1TableError)
{
  ...
  expect_read_snapshot_header(
      *mock_stream_interface, "1", "snap1", 1 << 22, &snapshot_offset, 0);
  expect_read_snapshot_header_extra(
      *mock_stream_interface, "1", "snap1", 1 << 29, &snapshot_offset, 0);
...
TEST_F(TestMockMigrationQCOWFormat, Read)
{
  ...
  mock_qcow_format.read(
      aio_comp, CEPH_NOSNAP, {{65659, 123}}, std::move(read_result), 0, 0, {});
...
TEST_F(TestMockMigrationQCOWFormat, ReadL1DNE)
{
  ...
  io::ReadResult read_result{&bl};
  mock_qcow_format.read(
      aio_comp, CEPH_NOSNAP, {{234, 123}}, std::move(read_result), 0, 0, {});
  ASSERT_EQ(123, ctx2.wait());
  bufferlist expect_bl;
...
TEST_F(TestMockMigrationQCOWFormat, ReadZero)
{
  ...
  expect_read_l2_table(
      *mock_stream_interface, 1ULL << 40, {{0, QCOW_OFLAG_ZERO}}, 0);
...
TEST_F(TestMockMigrationQCOWFormat, ReadSnap)
{
  ...
  io::ReadResult read_result{&bl};
  mock_qcow_format.read(
      aio_comp, 1, {{65659, 123}}, std::move(read_result), 0, 0, {});
  ASSERT_EQ(123, ctx2.wait());
  ASSERT_EQ(expect_bl, bl);
...
TEST_F(TestMockMigrationQCOWFormat, ReadClusterCacheHit)
{
  ...
  mock_qcow_format.read(
      aio_comp1, CEPH_NOSNAP, {{65659, 123}}, std::move(read_result1), 0, 0, {});
  ASSERT_EQ(123, ctx2.wait());
  ...
  mock_qcow_format.read(
      aio_comp2, CEPH_NOSNAP, {{66016, 234}}, std::move(read_result2), 0, 0, {});
...
TEST_F(TestMockMigrationQCOWFormat, ReadL2TableError)
{
  ...
  expect_read_l2_table(
      *mock_stream_interface, 1ULL << 40, {{0, 1ULL << 32}}, -EIO);
...
TEST_F(TestMockMigrationQCOWFormat, ListSnaps)
{
  ...
  expect_read_snapshot(
      *mock_stream_interface, "1", "snap1", 1 << 29, 1 << 22, &snapshot_offset);
  expect_read_snapshot(
      *mock_stream_interface, "2", "snap2", 1 << 29, 1 << 23, &snapshot_offset);
  ...
  expect_read_l2_table(
      *mock_stream_interface, 1ULL << 23,
      {{1ULL << 26, QCOW_OFLAG_ZERO, 1ULL << 25}}, 0);

  io::SparseExtents sparse_extents_head;
  sparse_extents_head.insert(
      131072, 65536, {io::SPARSE_EXTENT_STATE_DATA, 65536});
  expect_read_l2_table(
      *mock_stream_interface, 1ULL << 28,
  ...
  mock_qcow_format.list_snaps(
      {{0, 196608}}, {1, CEPH_NOSNAP}, 0, &snapshot_delta, {}, &ctx2);
  ASSERT_EQ(0, ctx2.wait());
