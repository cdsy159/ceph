...
public:
  ...
  void
  expect_stream_list_sparse_extents(
      MockStreamInterface& mock_stream_interface,
      const io::Extents& byte_extents,
...
TEST_F(TestMockMigrationRawSnapshot, OpenClose)
{
  ...
  MockRawSnapshot mock_raw_snapshot(
      &mock_image_ctx, json_object, &mock_source_spec_builder, 1);
...
TEST_F(TestMockMigrationRawSnapshot, OpenError)
{
  ...
  MockRawSnapshot mock_raw_snapshot(
      &mock_image_ctx, json_object, &mock_source_spec_builder, 0);
...
TEST_F(TestMockMigrationRawSnapshot, Read)
{
  ...
  mock_raw_snapshot.read(
      aio_comp, {{123, 123}}, std::move(read_result), 0, 0, {});
...
TEST_F(TestMockMigrationRawSnapshot, ListSnap)
{
  ...
  expect_stream_list_sparse_extents(
      *mock_stream_interface, {{0, 123}}, expected_sparse_extents, 0);
