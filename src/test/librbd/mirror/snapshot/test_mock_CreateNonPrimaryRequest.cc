...
template <>
bool
can_create_non_primary_snapshot(librbd::MockTestImageCtx* image_ctx)
{
...
WriteImageStateRequest<MockTestImageCtx>*
    WriteImageStateRequest<MockTestImageCtx>::s_instance = nullptr;
...
public:
      ...
      int r)
  {
    ...
    EXPECT_CALL(
        ...
        exec(
            RBD_MIRRORING, _, StrEq("rbd"), StrEq("mirror_image_get"), _, _, _,
            _))
        .WillOnce(DoAll(WithArg<5>(CopyInBufferlist(bl)), Return(r)));
  }

  void
  expect_can_create_non_primary_snapshot(MockUtils& mock_utils, bool result)
  {
      ...
      int r)
  {
    ...
    EXPECT_CALL(mock_write_image_state_request, send())
        .WillOnce(Invoke([&mock_image_ctx, &mock_write_image_state_request,
                          r]() {
...
TEST_F(TestMockMirrorSnapshotCreateNonPrimaryRequest, Success)
{
  ...
  C_SaferCond ctx;
  auto req = new MockCreateNonPrimaryRequest(
      &mock_image_ctx, false, "mirror_uuid", 123, {{1, 2}}, {}, nullptr, &ctx);
  req->send();
...
TEST_F(TestMockMirrorSnapshotCreateNonPrimaryRequest, SuccessDemoted)
{
  ...
  C_SaferCond ctx;
  auto req = new MockCreateNonPrimaryRequest(
      &mock_image_ctx, true, "mirror_uuid", 123, {{1, 2}}, {}, nullptr, &ctx);
  req->send();
  ASSERT_EQ(0, ctx.wait());
}
...
TEST_F(TestMockMirrorSnapshotCreateNonPrimaryRequest, WriteImageStateError)
{
  ...
  MockWriteImageStateRequest mock_write_image_state_request;
  expect_write_image_state(
      mock_image_ctx, mock_write_image_state_request, -EINVAL);
