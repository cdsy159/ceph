...
ListWatchersRequest<MockTestImageCtx>*
    ListWatchersRequest<MockTestImageCtx>::s_instance = nullptr;
    ...
    uint64_t* rollback_snap_id)
{
  return Mock::s_instance->can_create_primary_snapshot(
      image_ctx, demoted, force, requires_orphan, rollback_snap_id);
}
...
CreatePrimaryRequest<MockTestImageCtx>*
    CreatePrimaryRequest<MockTestImageCtx>::s_instance = nullptr;
...
public:
  typedef librbd::image::ListWatchersRequest<MockTestImageCtx>
      ...
      bool result)
  {
    EXPECT_CALL(mock_utils, can_create_primary_snapshot(_, false, force, _, _))
        .WillOnce(DoAll(
            WithArgs<3, 4>(Invoke([requires_orphan, rollback_snap_id](
      ...
      int r)
  {
    EXPECT_CALL(mock_create_non_primary_request, send())
        .WillOnce(Invoke([&mock_image_ctx, &mock_create_non_primary_request,
                          r]() {
      ...
      int r)
  {
    EXPECT_CALL(mock_list_watchers_request, send())
        .WillOnce(Invoke([&mock_image_ctx, &mock_list_watchers_request,
                          watchers, r]() {
  ...
  void
  expect_acquire_lock(MockTestImageCtx& mock_image_ctx, int r)
  {
      ...
      int r)
  {
    ...
    EXPECT_CALL(
        *mock_image_ctx.operations,
        execute_snap_rollback(snap_info->snap_namespace, snap_info->name, _, _))
        .WillOnce(WithArg<3>(
  ...
  void
  expect_release_lock(MockTestImageCtx& mock_image_ctx, int r)
  {
    if (mock_image_ctx.exclusive_lock == nullptr) {
      return;
    }
...
TEST_F(TestMockMirrorSnapshotPromoteRequest, ErrorCannotRollback)
{
  ...
  MockUtils mock_utils;
  expect_can_create_primary_snapshot(
      mock_utils, true, false, CEPH_NOSNAP, false);
