...
template class librbd::mirror::snapshot::UnlinkPeerRequest<
...
public:
  ...
  void
  expect_get_snap_info(MockTestImageCtx& mock_image_ctx, librados::snap_t snap_id)
  {
    EXPECT_CALL(mock_image_ctx, get_snap_info(snap_id))
        .WillRepeatedly(Invoke(
            [&mock_image_ctx](librados::snap_t snap_id) -> librbd::SnapInfo* {
      ...
      int r)
  {
    ...
    EXPECT_CALL(
        ...
        exec(
            ...
            StrEq("mirror_image_snapshot_unlink_peer"), ContentsEqual(bl), _, _,
            _))
        .WillOnce(Invoke(
            [&mock_image_ctx, snap_id, peer_uuid, r](auto&&... args) -> int {
              if (r == 0) {
                auto it = mock_image_ctx.snap_info.find(snap_id);
  ...
  void
  expect_notify_update(MockTestImageCtx& mock_image_ctx, int r)
  {
      ...
      int r)
  {
    EXPECT_CALL(*mock_image_ctx.operations, snap_remove(_, _, _))
        .WillOnce(Invoke([&mock_image_ctx, snap_id,
                          r](const cls::rbd::SnapshotNamespace& snap_namespace,
...
TEST_F(TestMockMirrorSnapshotUnlinkPeerRequest, Success)
{
  ...
  C_SaferCond ctx;
  auto req = new MockUnlinkPeerRequest(
      &mock_image_ctx, snap_id, "peer1_uuid", true, &ctx);
  req->send();
...
TEST_F(TestMockMirrorSnapshotUnlinkPeerRequest, RemoveSnapshot)
{
  ...
  MockTestImageCtx mock_image_ctx(*ictx);
  cls::rbd::MirrorSnapshotNamespace ns{
      cls::rbd::MIRROR_SNAPSHOT_STATE_PRIMARY, {"peer_uuid"}, "", CEPH_NOSNAP};
  auto snap_id = snap_create(mock_image_ctx, ns, "mirror_snap");
  ...
  C_SaferCond ctx;
  auto req = new MockUnlinkPeerRequest(
      &mock_image_ctx, snap_id, "peer_uuid", true, &ctx);
  req->send();
  ASSERT_EQ(0, ctx.wait());
}
...
TEST_F(TestMockMirrorSnapshotUnlinkPeerRequest, RemoveSnapshotNotAllowed)
{
  ...
  C_SaferCond ctx;
  auto req = new MockUnlinkPeerRequest(
      &mock_image_ctx, snap_id, "peer_uuid", false, &ctx);
  req->send();
  ASSERT_EQ(0, ctx.wait());
}
...
TEST_F(
    TestMockMirrorSnapshotUnlinkPeerRequest,
...
TEST_F(TestMockMirrorSnapshotUnlinkPeerRequest, SnapshotDNE)
{
  ...
  C_SaferCond ctx;
  auto req =
      new MockUnlinkPeerRequest(&mock_image_ctx, 123, "peer_uuid", true, &ctx);
  req->send();
...
TEST_F(TestMockMirrorSnapshotUnlinkPeerRequest, PeerDNE)
{
  ...
  C_SaferCond ctx;
  auto req = new MockUnlinkPeerRequest(
      &mock_image_ctx, snap_id, "unknown_peer", true, &ctx);
  req->send();
  ASSERT_EQ(0, ctx.wait());
}
