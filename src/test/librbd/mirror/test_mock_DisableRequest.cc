...
struct PromoteRequest<librbd::MockTestImageCtx> {
  ...
  create(librbd::MockTestImageCtx*, bool force, Context* on_finish)
  {
...
PromoteRequest<librbd::MockTestImageCtx>*
    PromoteRequest<librbd::MockTestImageCtx>::s_instance = nullptr;
...
ImageStateUpdateRequest<librbd::MockTestImageCtx>*
    ImageStateUpdateRequest<librbd::MockTestImageCtx>::s_instance = nullptr;
...
public:
  ...
  typedef mirror::ImageStateUpdateRequest<MockTestImageCtx>
      MockImageStateUpdateRequest;
  typedef mirror::snapshot::PromoteRequest<MockTestImageCtx>
      MockSnapshotPromoteRequest;
      ...
      int r)
  {
    ...
    EXPECT_CALL(mock_get_info_request, send())
        .WillOnce(Invoke([&mock_image_ctx, &mock_get_info_request, mirror_image,
                          promotion_state, r]() {
          if (r == 0) {
  ...
  expect_mirror_image_state_update(
      MockTestImageCtx& mock_image_ctx,
      MockImageStateUpdateRequest& mock_request,
      int r)
  {
  ...
  expect_mirror_image_remove(
      MockTestImageCtx& mock_image_ctx,
      MockImageRemoveRequest& mock_request,
      int r)
  {
    EXPECT_CALL(mock_request, send())
        .WillOnce(Invoke([&mock_image_ctx, &mock_request, r]() {
      ...
      int r)
  {
    ...
    EXPECT_CALL(
        ...
        exec(
            ::journal::Journaler::header_oid(mock_image_ctx.id), _,
            StrEq("journal"), StrEq("client_list"), _, _, _, _))
        .WillOnce(DoAll(WithArg<5>(CopyInBufferlist(bl)), Return(r)));
  }

  void
  expect_journal_client_unregister(
      MockTestImageCtx& mock_image_ctx,
      ...
      int r)
  {
    ...
    EXPECT_CALL(
        ...
        exec(
            ::journal::Journaler::header_oid(mock_image_ctx.id), _,
            StrEq("journal"), StrEq("client_unregister"), ContentsEqual(bl), _,
            _, _))
        .WillOnce(Return(r));
  }

  void
  expect_journal_promote(
      MockTestImageCtx& mock_image_ctx,
  ...
  expect_journal_client_list(
      ...
      {{"", encode(journal::ClientData{journal::ImageClientMeta{}})},
       ...
       {"peer 2", encode(journal::ClientData{journal::MirrorPeerClientMeta{
                      "remote image id",
                      {{cls::rbd::UserSnapshotNamespace(), "snap 1",
                        boost::optional<uint64_t>(0)},
                       {cls::rbd::UserSnapshotNamespace(), "snap 2",
                        boost::optional<uint64_t>(0)}}}})}},
      0);
...
TEST_F(TestMockMirrorDisableRequest, GetMirrorInfoError)
{
  ...
  expect_get_mirror_info(
      ...
      {cls::rbd::MIRROR_IMAGE_MODE_JOURNAL, "global id",
       cls::rbd::MIRROR_IMAGE_STATE_ENABLED},
      PROMOTION_STATE_PRIMARY, -EINVAL);
