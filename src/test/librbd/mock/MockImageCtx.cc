...
void
MockImageCtx::set_timer_instance(MockSafeTimer* timer, ceph::mutex* timer_lock)
{
  s_timer = timer;
...
void
MockImageCtx::get_timer_instance(
    CephContext* cct,
    MockSafeTimer** timer,
...
void
MockImageCtx::wait_for_async_ops()
{
...
IOContext
MockImageCtx::get_data_io_context()
{
...
IOContext
MockImageCtx::duplicate_data_io_context()
{
  return std::make_shared<neorados::IOContext>(*get_data_io_context());
}

uint64_t
MockImageCtx::get_data_offset() const
{
