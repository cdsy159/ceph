...
struct MockJournal {
  ...
  static int
  is_tag_owner(ImageCtxT* image_ctx, bool* is_tag_owner)
  {
  ...
  get_tag_owner(
      ...
      Context* on_finish)
  {
    get_instance()->get_tag_owner(
        global_image_id, tag_owner, work_queue, on_finish);
  }
  ...
  MOCK_METHOD4(
      get_tag_owner,
      void(const std::string&, std::string*, ContextWQ*, Context*));
  ...
  MOCK_METHOD3(
      append_op_event_mock,
      void(uint64_t, const journal::EventEntry&, Context*));
