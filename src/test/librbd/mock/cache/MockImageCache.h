...
struct MockImageCache {
  ...
  MOCK_METHOD4(
      aio_read_mock,
      void(const Extents&, ceph::bufferlist*, int, Context*));
  ...
  MOCK_METHOD4(
      aio_write_mock,
      void(const Extents&, const ceph::bufferlist&, int, Context*));
  ...
  MOCK_METHOD5(
      aio_writesame_mock,
      void(uint64_t, uint64_t, ceph::bufferlist& bl, int, Context*));
  ...
  {
    aio_compare_and_write_mock(
        image_extents, cmp_bl, bl, mismatch_offset, fadvise_flags, on_finish);
  }
};
