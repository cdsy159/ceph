...
public:
  ...
  void
  expect_lock(MockImageCtx& mock_image_ctx, int r)
  {
    std::string oid(
        ObjectMap<>::object_map_name(mock_image_ctx.id, CEPH_NOSNAP));
    EXPECT_CALL(
  ...
  void
  expect_get_lock_info(MockImageCtx& mock_image_ctx, int r)
  {
    std::string oid(
        ObjectMap<>::object_map_name(mock_image_ctx.id, CEPH_NOSNAP));
  ...
  void
  expect_break_lock(MockImageCtx& mock_image_ctx, int r)
  {
    std::string oid(
        ObjectMap<>::object_map_name(mock_image_ctx.id, CEPH_NOSNAP));
    auto& expect = EXPECT_CALL(
