...
public:
  void
  inject_snap_info(librbd::ImageCtx* ictx, uint64_t snap_id)
  {
    std::unique_lock image_locker{ictx->image_lock};
    ictx->add_snap(
        cls::rbd::UserSnapshotNamespace(), "snap name", snap_id, ictx->size,
        ictx->parent_md, RBD_PROTECTION_STATUS_UNPROTECTED, 0, utime_t());
  }

  void
  expect_read_map(librbd::ImageCtx* ictx, int r)
  {
    if (r < 0) {
      EXPECT_CALL(
          get_mock_io_ctx(ictx->md_ctx),
          read(
              ObjectMap<>::object_map_name(ictx->id, CEPH_NOSNAP), 0, 0, _, _,
              _))
          .WillOnce(Return(r));
    } else {
      EXPECT_CALL(
  ...
  void
  expect_write_map(librbd::ImageCtx* ictx, uint64_t snap_id, int r)
  {
    if (r < 0) {
      EXPECT_CALL(
          get_mock_io_ctx(ictx->md_ctx),
  ...
  void
  expect_add_snapshot(librbd::ImageCtx* ictx, int r)
  {
...
TEST_F(TestMockObjectMapSnapshotCreateRequest, FlagCleanObjects)
{
  ...
  for (uint64_t i = 0; i < object_map.size(); ++i) {
    ASSERT_EQ(
        i % 2 == 0 ? OBJECT_EXISTS_CLEAN : OBJECT_NONEXISTENT, object_map[i]);
  }
}
