      ...
      int r)
  {
    ...
    if (r < 0) {
      EXPECT_CALL(
          get_mock_io_ctx(ictx->md_ctx),
          exec(
              oid, _, StrEq("rbd"), StrEq("object_map_update"),
...
TEST_F(TestMockObjectMapUpdateRequest, UpdateOnDiskError)
{
  ...
  expect_update(
      ictx, CEPH_NOSNAP, 0, 1, OBJECT_NONEXISTENT, OBJECT_EXISTS, -EINVAL);
...
TEST_F(TestMockObjectMapUpdateRequest, RebuildSnapOnDisk)
{
  ...
  expect_update(
      ictx, snap_id, 0, 1, OBJECT_EXISTS_CLEAN, boost::optional<uint8_t>(), 0);
  expect_unlock_exclusive_lock(*ictx);
...
TEST_F(TestMockObjectMapUpdateRequest, BatchUpdate)
{
  ...
  librbd::NoOpProgressContext no_progress;
  ASSERT_EQ(
      0, ictx->operations->resize(
             712312 * ictx->get_object_size(), false, no_progress));
  ASSERT_EQ(0, acquire_exclusive_lock(*ictx));

  expect_unlock_exclusive_lock(*ictx);
  InSequence seq;
  expect_update(
      ictx, CEPH_NOSNAP, 0, 262144, OBJECT_NONEXISTENT, OBJECT_EXISTS, 0);
  expect_update(
      ictx, CEPH_NOSNAP, 262144, 524288, OBJECT_NONEXISTENT, OBJECT_EXISTS, 0);
  expect_update(
      ictx, CEPH_NOSNAP, 524288, 712312, OBJECT_NONEXISTENT, OBJECT_EXISTS, 0);
...
TEST_F(TestMockObjectMapUpdateRequest, IgnoreMissingObjectMap)
{
  ...
  expect_update(
      ictx, CEPH_NOSNAP, 0, 1, OBJECT_NONEXISTENT, OBJECT_EXISTS, -ENOENT);
