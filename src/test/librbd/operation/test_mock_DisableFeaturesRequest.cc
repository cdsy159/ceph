...
RemoveRequest<MockOperationImageCtx>*
    RemoveRequest<MockOperationImageCtx>::s_instance;
...
DisableRequest<MockOperationImageCtx>*
    DisableRequest<MockOperationImageCtx>::s_instance;
...
public:
  ...
  void
  expect_disable_mirror_request_send(
      MockOperationImageCtx& mock_image_ctx,
      ...
      int r)
  {
    EXPECT_CALL(mock_disable_mirror_request, send())
        .WillOnce(
            FinishRequest(&mock_disable_mirror_request, r, &mock_image_ctx));
  }

  void
  expect_close_journal(MockOperationImageCtx& mock_image_ctx, int r)
  {
  ...
  void
  expect_remove_journal_request_send(
      MockOperationImageCtx& mock_image_ctx,
      ...
      int r)
  {
    EXPECT_CALL(mock_remove_journal_request, send())
        .WillOnce(
            FinishRequest(&mock_remove_journal_request, r, &mock_image_ctx));
  }

  void
  expect_remove_object_map_request_send(
      MockOperationImageCtx& mock_image_ctx,
      ...
      int r)
  {
    EXPECT_CALL(mock_remove_object_map_request, send())
        .WillOnce(
            FinishRequest(&mock_remove_object_map_request, r, &mock_image_ctx));
  }
...
TEST_F(TestMockOperationDisableFeaturesRequest, All)
{
  ...
  if (features_to_disable & RBD_FEATURE_JOURNALING) {
    expect_disable_mirror_request_send(
        mock_image_ctx, mock_disable_mirror_request, 0);
    expect_close_journal(mock_image_ctx, 0);
    expect_remove_journal_request_send(
        mock_image_ctx, mock_remove_journal_request, 0);
  }
  if (features_to_disable & RBD_FEATURE_OBJECT_MAP) {
    expect_remove_object_map_request_send(
        mock_image_ctx, mock_remove_object_map_request, 0);
  }
...
TEST_F(TestMockOperationDisableFeaturesRequest, ObjectMapError)
{
  ...
  expect_append_op_event(mock_image_ctx, true, 0);
  expect_remove_object_map_request_send(
      mock_image_ctx, mock_remove_object_map_request, -EINVAL);
  expect_unblock_requests(mock_image_ctx);
  expect_unblock_writes(mock_image_ctx);
  expect_handle_prepare_lock_complete(mock_image_ctx);
...
TEST_F(TestMockOperationDisableFeaturesRequest, MirroringError)
{
  ...
  expect_block_requests(mock_image_ctx);
  expect_disable_mirror_request_send(
      mock_image_ctx, mock_disable_mirror_request, -EINVAL);
  expect_close_journal(mock_image_ctx, 0);
  expect_remove_journal_request_send(
      mock_image_ctx, mock_remove_journal_request, 0);
  expect_notify_update(mock_image_ctx);
