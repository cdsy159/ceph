...
SetFlagsRequest<MockOperationImageCtx>*
    SetFlagsRequest<MockOperationImageCtx>::s_instance;
...
CreateRequest<MockOperationImageCtx>*
    CreateRequest<MockOperationImageCtx>::s_instance = nullptr;
...
EnableRequest<MockOperationImageCtx>*
    EnableRequest<MockOperationImageCtx>::s_instance = nullptr;
...
public:
  ...
  static CreateRequest*
  create(MockOperationImageCtx* image_ctx, Context* on_finish)
  {
    ceph_assert(s_instance != nullptr);
    s_instance->on_finish = on_finish;
    return s_instance;
...
public:
  typedef librbd::image::SetFlagsRequest<MockOperationImageCtx>
      MockSetFlagsRequest;
  typedef librbd::journal::CreateRequest<MockOperationImageCtx>
      MockCreateJournalRequest;
  typedef librbd::mirror::EnableRequest<MockOperationImageCtx>
      MockEnableMirrorRequest;
  typedef librbd::object_map::CreateRequest<MockOperationImageCtx>
      MockCreateObjectMapRequest;
  ...
  public:
    ...
    {
      EXPECT_EQ(
          0, librbd::cls_client::mirror_mode_set(
  ...
  void
  ensure_features_disabled(librbd::ImageCtx* ictx, uint64_t features_to_disable)
  {
    uint64_t features;
  ...
  void
  expect_prepare_lock(MockOperationImageCtx& mock_image_ctx)
  {
  ...
  void
  expect_handle_prepare_lock_complete(MockOperationImageCtx& mock_image_ctx)
  {
    EXPECT_CALL(*mock_image_ctx.state, handle_prepare_lock_complete());
  }

  void
  expect_block_writes(MockOperationImageCtx& mock_image_ctx)
  {
  ...
  void
  expect_verify_lock_ownership(MockOperationImageCtx& mock_image_ctx)
  {
  ...
  void
  expect_block_requests(MockOperationImageCtx& mock_image_ctx)
  {
    if (mock_image_ctx.exclusive_lock != nullptr) {
  ...
  void
  expect_unblock_requests(MockOperationImageCtx& mock_image_ctx)
  {
    if (mock_image_ctx.exclusive_lock != nullptr) {
  ...
  void
  expect_create_journal_request_send(
      MockOperationImageCtx& mock_image_ctx,
      ...
      int r)
  {
    EXPECT_CALL(mock_create_journal_request, send())
        .WillOnce(
            FinishRequest(&mock_create_journal_request, r, &mock_image_ctx));
  }

  void
  expect_enable_mirror_request_send(
      MockOperationImageCtx& mock_image_ctx,
      ...
      int r)
  {
    EXPECT_CALL(mock_enable_mirror_request, send())
        .WillOnce(
            FinishRequest(&mock_enable_mirror_request, r, &mock_image_ctx));
  }
      ...
      int r)
  {
    EXPECT_CALL(mock_create_object_map_request, send())
        .WillOnce(
            FinishRequest(&mock_create_object_map_request, r, &mock_image_ctx));
  }

  void
  expect_notify_update(MockOperationImageCtx& mock_image_ctx)
  {
...
TEST_F(TestMockOperationEnableFeaturesRequest, All)
{
  ...
  if (features_to_enable & RBD_FEATURE_JOURNALING) {
    expect_create_journal_request_send(
        mock_image_ctx, mock_create_journal_request, 0);
  }
  ...
  if (features_to_enable & RBD_FEATURE_OBJECT_MAP) {
    expect_create_object_map_request_send(
        mock_image_ctx, mock_create_object_map_request, 0);
  }
...
TEST_F(TestMockOperationEnableFeaturesRequest, Mirroring)
{
  ...
  expect_create_journal_request_send(
      mock_image_ctx, mock_create_journal_request, 0);
  expect_enable_mirror_request_send(
      mock_image_ctx, mock_enable_mirror_request, 0);
  expect_notify_update(mock_image_ctx);
  expect_unblock_requests(mock_image_ctx);
  expect_unblock_writes(mock_image_ctx);
  expect_handle_prepare_lock_complete(mock_image_ctx);
...
TEST_F(TestMockOperationEnableFeaturesRequest, JournalingError)
{
  ...
  expect_block_requests(mock_image_ctx);
  expect_create_journal_request_send(
      mock_image_ctx, mock_create_journal_request, -EINVAL);
  expect_unblock_requests(mock_image_ctx);
  expect_unblock_writes(mock_image_ctx);
  expect_handle_prepare_lock_complete(mock_image_ctx);
...
TEST_F(TestMockOperationEnableFeaturesRequest, MirroringError)
{
  ...
  expect_create_journal_request_send(
      mock_image_ctx, mock_create_journal_request, 0);
  expect_enable_mirror_request_send(
      mock_image_ctx, mock_enable_mirror_request, -EINVAL);
  expect_notify_update(mock_image_ctx);
  expect_unblock_requests(mock_image_ctx);
  expect_unblock_writes(mock_image_ctx);
  expect_handle_prepare_lock_complete(mock_image_ctx);
