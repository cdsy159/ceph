...
public:
  ...
  void
  expect_is_lock_owner(MockImageCtx& mock_image_ctx)
  {
  ...
  void
  expect_grow_object_map(MockImageCtx& mock_image_ctx)
  {
  ...
  void
  expect_shrink_object_map(MockImageCtx& mock_image_ctx)
  {
    if (mock_image_ctx.object_map != nullptr) {
      expect_is_lock_owner(mock_image_ctx);
      EXPECT_CALL(*mock_image_ctx.object_map, aio_resize(_, _, _))
  ...
  void
  expect_update_header(MockImageCtx& mock_image_ctx, int r)
  {
    if (mock_image_ctx.old_format) {
      ...
      EXPECT_CALL(
          get_mock_io_ctx(mock_image_ctx.md_ctx),
          exec(
              mock_image_ctx.header_oid, _, StrEq("rbd"), StrEq("set_size"), _,
              _, _, _))
          .WillOnce(Return(r));
    }
  }
  ...
  void
  expect_flush_cache(MockImageCtx& mock_image_ctx, int r)
  {
  ...
  void
  expect_invalidate_cache(MockImageCtx& mock_image_ctx, int r)
  {
  ...
  void
  expect_resize_object_map(MockImageCtx& mock_image_ctx, uint64_t new_size)
  {
...
TEST_F(TestMockOperationResizeRequest, ShrinkError)
{
  ...
  expect_unblock_writes(mock_image_ctx);
  ASSERT_EQ(
      -EINVAL, when_resize(mock_image_ctx, ictx->size / 2, false, 0, false));
}
...
TEST_F(TestMockOperationResizeRequest, TrimError)
{
  ...
  ASSERT_EQ(
      -EINVAL, when_resize(mock_image_ctx, ictx->size / 2, true, 0, false));
}
