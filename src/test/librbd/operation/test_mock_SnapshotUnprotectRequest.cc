...
public:
      ...
      int r)
  {
    ...
    auto& expect = EXPECT_CALL(
        ...
        exec(
            ...
            StrEq("set_protection_status"), ContentsEqual(bl), _, _, _));
    if (r < 0) {
      expect.WillOnce(Return(r));
    } else {
  ...
  size_t
  expect_create_pool_io_contexts(MockImageCtx& mock_image_ctx)
  {
  ...
  void
  expect_get_children(MockImageCtx& mock_image_ctx, size_t pools, int r)
  {
    bufferlist bl;
...
TEST_F(TestMockOperationSnapshotUnprotectRequest, Success)
{
  ...
  expect_set_protection_status(
      mock_image_ctx, snap_id, RBD_PROTECTION_STATUS_UNPROTECTING, 0);
  size_t pools = expect_create_pool_io_contexts(mock_image_ctx);
  expect_get_children(mock_image_ctx, pools, -ENOENT);
  expect_set_protection_status(
      mock_image_ctx, snap_id, RBD_PROTECTION_STATUS_UNPROTECTED, 0);
...
TEST_F(TestMockOperationSnapshotUnprotectRequest, SetProtectionStatusError)
{
  ...
  expect_is_snap_unprotected(mock_image_ctx, false, 0);
  expect_set_protection_status(
      mock_image_ctx, snap_id, RBD_PROTECTION_STATUS_UNPROTECTING, -EINVAL);
...
TEST_F(TestMockOperationSnapshotUnprotectRequest, ChildrenExist)
{
  ...
  expect_set_protection_status(
      mock_image_ctx, snap_id, RBD_PROTECTION_STATUS_PROTECTED, 0);
