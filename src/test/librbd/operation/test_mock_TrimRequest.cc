...
struct AsyncRequest<librbd::MockTestImageCtx> {
  ...
  create_async_callback_context()
  {
    return util::create_context_callback<
        AsyncRequest, &AsyncRequest::async_complete>(this);
  }
...
public:
    ...
    if (mock_image_ctx.object_map != nullptr) {
      EXPECT_CALL(
          *mock_image_ctx.object_map,
          aio_update(
              CEPH_NOSNAP, start_object, end_object, state,
  ...
  void
  expect_get_parent_overlap(MockTestImageCtx& mock_image_ctx, uint64_t overlap)
  {
  ...
  void
  expect_reduce_parent_overlap(MockTestImageCtx& mock_image_ctx, uint64_t overlap)
  {
  ...
  void
  expect_get_area_size(MockTestImageCtx& mock_image_ctx)
  {
      ...
      int r)
  {
    EXPECT_CALL(*mock_image_ctx.io_object_dispatcher, send(_))
        .WillOnce(Invoke([&mock_image_ctx, offset, length, update_object_map,
                          r](io::ObjectDispatchSpec* spec) {
...
TEST_F(TestMockOperationTrimRequest, SuccessRemove)
{
  ...
  InSequence seq;
  EXPECT_CALL(mock_image_ctx, get_stripe_period())
      .WillOnce(Return(ictx->get_object_size()));
  EXPECT_CALL(mock_image_ctx, get_stripe_count())
      .WillOnce(Return(ictx->get_stripe_count()));

  // pre
  expect_object_map_update(
      mock_image_ctx, 0, 1, OBJECT_PENDING, OBJECT_EXISTS, true, 0);
  ...
  // post
  expect_object_map_update(
      mock_image_ctx, 0, 1, OBJECT_NONEXISTENT, OBJECT_PENDING, true, 0);
...
TEST_F(TestMockOperationTrimRequest, SuccessCopyUp)
{
  ...
  // pre
  expect_object_map_update(
      mock_image_ctx, 0, 2, OBJECT_PENDING, OBJECT_EXISTS, true, 0);
  ...
  expect_get_object_name(mock_image_ctx, 0, "object0");
  expect_object_discard(
      mock_image_ctx, mock_io_object_dispatch, 0, ictx->get_object_size(),
      false, 0);
  ...
  // post
  expect_object_map_update(
      mock_image_ctx, 0, 2, OBJECT_NONEXISTENT, OBJECT_PENDING, true, 0);
...
TEST_F(TestMockOperationTrimRequest, SuccessBoundary)
{
  ...
  io::MockObjectDispatch mock_io_object_dispatch;
  expect_object_discard(
      mock_image_ctx, mock_io_object_dispatch, 1, ictx->get_object_size() - 1,
      true, 0);
...
TEST_F(TestMockOperationTrimRequest, RemoveError)
{
  ...
  // pre
  expect_object_map_update(
      mock_image_ctx, 0, 1, OBJECT_PENDING, OBJECT_EXISTS, false, 0);
...
TEST_F(TestMockOperationTrimRequest, CopyUpError)
{
  ...
  // pre
  expect_object_map_update(
      mock_image_ctx, 0, 2, OBJECT_PENDING, OBJECT_EXISTS, false, 0);
