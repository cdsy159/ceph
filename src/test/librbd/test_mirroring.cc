...
static bool
operator==(const mirror_peer_site_t& lhs, const mirror_peer_site_t& rhs)
{
  return (
      lhs.uuid == rhs.uuid && lhs.direction == rhs.direction &&
      lhs.site_name == rhs.site_name && lhs.client_name == rhs.client_name &&
      lhs.last_seen == rhs.last_seen);
}

static std::ostream&
operator<<(std::ostream& os, const mirror_peer_site_t& rhs)
{
  os << "uuid=" << rhs.uuid << ", " << "direction=" << rhs.direction << ", "
...
public:
      ...
      rbd_mirror_image_mode_t mirror_image_mode = RBD_MIRROR_IMAGE_MODE_JOURNAL)
  {
    ...
    int order = 20;
    ASSERT_EQ(
        0, m_rbd.create2(m_ioctx, image_name.c_str(), 4096, features, &order));
    librbd::Image image;
    ...
    ASSERT_EQ(
        0, image.mirror_image_get_info(&mirror_image, sizeof(mirror_image)));
    ...
    ASSERT_EQ(
        mirror_state == RBD_MIRROR_IMAGE_ENABLED ? -ENOENT : -EINVAL,
      ...
      rbd_mirror_image_state_t mirror_state)
  {
    ...
    ASSERT_EQ(
        mirror_state != RBD_MIRROR_IMAGE_DISABLED ? 0 : -EINVAL,
  ...
  void
  check_mirroring_status(size_t* images_count)
  {
    std::map<std::string, librbd::mirror_image_global_status_t> images;
    ASSERT_EQ(
        0, m_rbd.mirror_image_global_status_list(m_ioctx, "", 4096, &images));
    ...
    *images_count = images.size();
    ...
    ASSERT_EQ(
        0,
        m_rbd.mirror_image_instance_id_list(m_ioctx, "", 4096, &instance_ids));
    ASSERT_TRUE(instance_ids.empty());
  }

  void
  check_mirroring_on_create(
      uint64_t features,
      rbd_mirror_mode_t mirror_mode,
      rbd_mirror_image_state_t mirror_state)
  {
      ...
      rbd_mirror_image_mode_t mirror_image_mode = RBD_MIRROR_IMAGE_MODE_JOURNAL)
  {
    ...
    ASSERT_EQ(
        0,
        m_rbd.create2(m_ioctx, image_name.c_str(), 4096, init_features, &order));
    librbd::Image image;
    ASSERT_EQ(0, m_rbd.open(m_ioctx, image, image_name.c_str()));
      ...
      std::vector<uint64_t>& features_vec)
  {
    ...
    for (const auto& features : features_vec) {
      ...
      ASSERT_EQ(
          0,
          m_rbd.create2(m_ioctx, img_name_str.c_str(), 2048, features, &order));
    }
  }

  void
  check_mirroring_on_mirror_mode_set(
      rbd_mirror_mode_t mirror_mode,
      std::vector<rbd_mirror_image_state_t>& states_vec)
  {
    ...
    for (const auto& mirror_state : states_vec) {
      ...
      librbd::mirror_image_global_status_t status;
      ASSERT_EQ(
          0, image.mirror_image_get_global_status(&status, sizeof(status)));
      librbd::mirror_image_site_status_t local_status;
      ASSERT_EQ(0, get_local_mirror_image_site_status(status, &local_status));
      ASSERT_EQ(MIRROR_IMAGE_STATUS_STATE_UNKNOWN, local_status.state);
  ...
  void
  check_trash_move_restore(rbd_mirror_mode_t mirror_mode, bool enable_mirroring)
  {
  ...
  void
  setup_mirror_peer(librados::IoCtx& io_ctx, librbd::Image& image)
  {
...
TEST_F(TestMirroring, EnableImageMirror_In_MirrorModeImage)
{
  ...
  check_mirror_image_enable(
      RBD_MIRROR_MODE_IMAGE, features, 0, RBD_MIRROR_IMAGE_ENABLED,
...
TEST_F(TestMirroring, EnableImageMirror_In_MirrorModePool)
{
  ...
  features |= RBD_FEATURE_JOURNALING;
  check_mirror_image_enable(
      RBD_MIRROR_MODE_POOL, features, -EINVAL, RBD_MIRROR_IMAGE_ENABLED,
      RBD_MIRROR_IMAGE_MODE_JOURNAL);
}
...
TEST_F(TestMirroring, EnableImageMirror_In_MirrorModeDisabled)
{
  ...
  features |= RBD_FEATURE_JOURNALING;
  check_mirror_image_enable(
      RBD_MIRROR_MODE_DISABLED, features, -EINVAL, RBD_MIRROR_IMAGE_DISABLED);
}
...
TEST_F(TestMirroring, DisableImageMirror_In_MirrorModeImage)
{
  ...
  features |= RBD_FEATURE_JOURNALING;
  check_mirror_image_disable(
      RBD_MIRROR_MODE_IMAGE, features, 0, RBD_MIRROR_IMAGE_DISABLED);
}
...
TEST_F(TestMirroring, EnableImageMirror_In_MirrorModePool_WithoutJournaling)
{
  ...
  features |= RBD_FEATURE_EXCLUSIVE_LOCK;
  check_mirror_image_enable(
      RBD_MIRROR_MODE_POOL, features, -EINVAL, RBD_MIRROR_IMAGE_DISABLED);
}
...
TEST_F(TestMirroring, CreateImage_In_MirrorModeDisabled)
{
  ...
  features |= RBD_FEATURE_JOURNALING;
  check_mirroring_on_create(
      features, RBD_MIRROR_MODE_DISABLED, RBD_MIRROR_IMAGE_DISABLED);
}
...
TEST_F(TestMirroring, CreateImage_In_MirrorModePool)
{
  ...
  features |= RBD_FEATURE_JOURNALING;
  check_mirroring_on_create(
      features, RBD_MIRROR_MODE_POOL, RBD_MIRROR_IMAGE_ENABLED);
}
...
TEST_F(TestMirroring, CreateImage_In_MirrorModePool_WithoutJournaling)
{
  ...
  features |= RBD_FEATURE_EXCLUSIVE_LOCK;
  check_mirroring_on_create(
      features, RBD_MIRROR_MODE_POOL, RBD_MIRROR_IMAGE_DISABLED);
}
...
TEST_F(TestMirroring, EnableJournaling_In_MirrorModeDisabled)
{
  ...
  check_mirroring_on_update_features(
      init_features, true, false, features, 0, RBD_MIRROR_MODE_DISABLED,
...
TEST_F(TestMirroring, EnableJournaling_In_MirrorModeImage)
{
  ...
  uint64_t features = RBD_FEATURE_JOURNALING;
  check_mirroring_on_update_features(
      init_features, true, false, features, 0, RBD_MIRROR_MODE_IMAGE,
      RBD_MIRROR_IMAGE_DISABLED);
}
    ...
    EnableJournaling_In_MirrorModeImage_SnapshotMirroringEnabled)
{
  ...
  uint64_t features = RBD_FEATURE_JOURNALING;
  check_mirroring_on_update_features(
      init_features, true, true, features, 0, RBD_MIRROR_MODE_IMAGE,
...
TEST_F(TestMirroring, EnableJournaling_In_MirrorModePool)
{
  ...
  uint64_t features = RBD_FEATURE_JOURNALING;
  check_mirroring_on_update_features(
      init_features, true, false, features, 0, RBD_MIRROR_MODE_POOL,
...
TEST_F(TestMirroring, DisableJournaling_In_MirrorModePool)
{
  ...
  uint64_t features = RBD_FEATURE_JOURNALING;
  check_mirroring_on_update_features(
      init_features, false, false, features, 0, RBD_MIRROR_MODE_POOL,
      RBD_MIRROR_IMAGE_DISABLED);
}
...
TEST_F(TestMirroring, DisableJournaling_In_MirrorModeImage)
{
  ...
  uint64_t features = RBD_FEATURE_JOURNALING;
  check_mirroring_on_update_features(
      init_features, false, true, features, -EINVAL, RBD_MIRROR_MODE_IMAGE,
      RBD_MIRROR_IMAGE_ENABLED, RBD_MIRROR_IMAGE_MODE_JOURNAL);
}
...
TEST_F(TestMirroring, RemoveImage_With_MirrorImageEnabled)
{
  check_remove_image(
      ...
      RBD_FEATURE_EXCLUSIVE_LOCK | RBD_FEATURE_JOURNALING, true);
}
...
TEST_F(TestMirroring, RemoveImage_With_MirrorImageDisabled)
{
  check_remove_image(
      RBD_MIRROR_MODE_IMAGE,
      RBD_FEATURE_EXCLUSIVE_LOCK | RBD_FEATURE_JOURNALING, false);
}
...
TEST_F(TestMirroring, RemoveImage_With_MirrorImageDemoted)
{
  check_remove_image(
      RBD_MIRROR_MODE_IMAGE,
      RBD_FEATURE_EXCLUSIVE_LOCK | RBD_FEATURE_JOURNALING, true, true);
}
...
TEST_F(TestMirroring, MirrorStatusList)
{
  ...
  std::map<std::string, librbd::mirror_image_global_status_t> images;
  ASSERT_EQ(
      0, m_rbd.mirror_image_global_status_list(m_ioctx, last_read, 2, &images));
  ...
  ASSERT_EQ(
      0,
      m_rbd.mirror_image_global_status_list(m_ioctx, last_read, 4096, &images));
...
TEST_F(TestMirroring, AioPromoteDemote)
{
  ...
  for (auto& image_name : image_names) {
    ASSERT_EQ(
        0, m_rbd.create2(
               m_ioctx, image_name.c_str(), 2048,
               RBD_FEATURE_EXCLUSIVE_LOCK | RBD_FEATURE_JOURNALING, &order));

    images.emplace_back();
    ASSERT_EQ(0, m_rbd.open(m_ioctx, images.back(), image_name.c_str()));
    ASSERT_EQ(
        0, images.back().mirror_image_enable2(RBD_MIRROR_IMAGE_MODE_JOURNAL));
  }
...
TEST_F(TestMirroring, AioGetInfo)
{
  ...
  for (auto& image : images) {
    ...
    infos.emplace_back();
    ASSERT_EQ(
        0, image.aio_mirror_image_get_info(
               &infos.back(), sizeof(infos.back()), aio_comps.back()));
  }
  for (auto aio_comp : aio_comps) {
    ASSERT_EQ(0, aio_comp->wait_for_complete());
    ASSERT_EQ(1, aio_comp->is_complete());
...
TEST_F(TestMirroring, AioGetStatus)
{
  ...
  for (auto& image : images) {
    ...
    ASSERT_EQ(
        0, image.aio_mirror_image_get_global_status(
               &statuses.back(), sizeof(statuses.back()), aio_comps.back()));
  }
  for (auto aio_comp : aio_comps) {
    ASSERT_EQ(0, aio_comp->wait_for_complete());
    ASSERT_EQ(1, aio_comp->is_complete());
...
TEST_F(TestMirroring, Bootstrap)
{
  ...
  ASSERT_EQ(
      -EINVAL, m_rbd.mirror_peer_bootstrap_import(
...
TEST_F(TestMirroring, PeerDirection)
{
  ...
  std::string uuid;
  ASSERT_EQ(
      -EINVAL, m_rbd.mirror_peer_site_add(
                   m_ioctx, &uuid, RBD_MIRROR_PEER_DIRECTION_TX, "siteA",
                   "client.admin"));
  ASSERT_EQ(
      0, m_rbd.mirror_peer_site_add(
             m_ioctx, &uuid, RBD_MIRROR_PEER_DIRECTION_RX_TX, "siteA",
             "client.admin"));
  ...
  ASSERT_EQ(
      0, m_rbd.mirror_peer_site_set_direction(
...
TEST_F(TestMirroring, Snapshot)
{
  ...
  ASSERT_EQ(
      0, image.metadata_set("conf_rbd_mirroring_max_mirroring_snapshots", "5"));
  ...
  ASSERT_EQ(
      0, m_rbd.mirror_peer_site_add(
             m_ioctx, &peer_uuid, RBD_MIRROR_PEER_DIRECTION_RX_TX, "cluster",
             "client"));
  ...
  ASSERT_EQ(
      0, image.snap_get_mirror_namespace(
             snap_id, &mirror_snap, sizeof(mirror_snap)));
...
TEST_F(TestMirroring, SnapshotUnlinkPeer)
{
  ...
  ASSERT_EQ(
      0, m_rbd.mirror_peer_site_add(
             m_ioctx, &peer1_uuid, RBD_MIRROR_PEER_DIRECTION_RX_TX, "cluster1",
             "client"));
  std::string peer2_uuid;
  ASSERT_EQ(
      0, m_rbd.mirror_peer_site_add(
             m_ioctx, &peer2_uuid, RBD_MIRROR_PEER_DIRECTION_RX_TX, "cluster2",
             "client"));
  std::string peer3_uuid;
  ASSERT_EQ(
      0, m_rbd.mirror_peer_site_add(
             m_ioctx, &peer3_uuid, RBD_MIRROR_PEER_DIRECTION_RX_TX, "cluster3",
             "client"));
  uint64_t features;
  ASSERT_TRUE(get_features(&features));
  features &= ~RBD_FEATURE_JOURNALING;
  ...
  ASSERT_EQ(
      0, m_rbd.create2(ns_ioctx, image_name.c_str(), 4096, features, &order));
  ...
  ASSERT_EQ(
      0, ns_image.snap_get_mirror_namespace(
  ...
  ASSERT_EQ(
      -ENOENT, image.snap_get_mirror_namespace(
                   snap_id, &mirror_snap, sizeof(mirror_snap)));
  ictx->state->close();
  ictx = nullptr;
  ASSERT_EQ(0, image.close());
...
TEST_F(TestMirroring, SnapshotImageState)
{
  ...
  for (int i = 0; i < 10; i++) {
    ...
    ASSERT_EQ(
        0, strncmp(std::string(1024, 'A' + i).c_str(), bl.c_str(), bl.length()));
  }
...
TEST_F(TestMirroring, AioSnapshotCreate)
{
  ...
  for (auto& image_name : image_names) {
    ASSERT_EQ(
        0, m_rbd.create2(m_ioctx, image_name.c_str(), 2048, features, &order));
    images.emplace_back();
    ASSERT_EQ(0, m_rbd.open(m_ioctx, images.back(), image_name.c_str()));
    ASSERT_EQ(
        0, images.back().mirror_image_enable2(RBD_MIRROR_IMAGE_MODE_SNAPSHOT));
  }
  ...
  for (auto& image : images) {
    ...
    aio_comps.push_back(new librbd::RBD::AioCompletion(nullptr, nullptr));
    ASSERT_EQ(
        0, image.aio_mirror_image_create_snapshot(
               0, &snap_ids.back(), aio_comps.back()));
  }
  for (auto aio_comp : aio_comps) {
    ASSERT_EQ(0, aio_comp->wait_for_complete());
    ASSERT_EQ(1, aio_comp->is_complete());
