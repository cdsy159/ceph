...
struct ManagedLock<MockExclusiveLockImageCtx> {
  ManagedLock(
      ...
      librbd::MockImageWatcher* watcher,
...
ImageDispatch<MockExclusiveLockImageCtx>*
    ImageDispatch<MockExclusiveLockImageCtx>::s_instance = nullptr;

template <>
struct PreAcquireRequest<MockExclusiveLockImageCtx>
  : public BaseRequest<PreAcquireRequest<MockExclusiveLockImageCtx>> {
  static PreAcquireRequest<MockExclusiveLockImageCtx>*
  create(MockExclusiveLockImageCtx& image_ctx, Context* on_finish)
  {
...
template <>
struct PreReleaseRequest<MockExclusiveLockImageCtx>
  : public BaseRequest<PreReleaseRequest<MockExclusiveLockImageCtx>> {
  static PreReleaseRequest<MockExclusiveLockImageCtx>*
  create(
      MockExclusiveLockImageCtx& image_ctx,
...
ACTION_P2(CompleteRequest, request, ret) { request->on_finish->complete(ret); }
...
public:
  ...
  typedef exclusive_lock::ImageDispatch<MockExclusiveLockImageCtx>
      MockImageDispatch;
  typedef exclusive_lock::PreAcquireRequest<MockExclusiveLockImageCtx>
      MockPreAcquireRequest;
  typedef exclusive_lock::PostAcquireRequest<MockExclusiveLockImageCtx>
      MockPostAcquireRequest;
  typedef exclusive_lock::PreReleaseRequest<MockExclusiveLockImageCtx>
      MockPreReleaseRequest;

  void
  expect_set_state_initializing(MockManagedLock* managed_lock)
  {
    EXPECT_CALL(*managed_lock, set_state_initializing());
  }

  void
  expect_set_state_unlocked(MockManagedLock* managed_lock)
  {
    EXPECT_CALL(*managed_lock, set_state_unlocked());
  }

  void
  expect_set_state_waiting_for_lock(MockManagedLock* managed_lock)
  {
    EXPECT_CALL(*managed_lock, set_state_waiting_for_lock());
  }

  void
  expect_set_state_post_acquiring(MockManagedLock* managed_lock)
  {
    EXPECT_CALL(*managed_lock, set_state_post_acquiring());
  }

  void
  expect_is_state_acquiring(MockManagedLock* managed_lock, bool ret_val)
  {
    EXPECT_CALL(*managed_lock, is_state_acquiring()).WillOnce(Return(ret_val));
  }

  void
  expect_is_state_waiting_for_lock(MockManagedLock* managed_lock, bool ret_val)
  {
  ...
  void
  expect_is_state_pre_releasing(MockManagedLock* managed_lock, bool ret_val)
  {
  ...
  void
  expect_is_state_releasing(MockManagedLock* managed_lock, bool ret_val)
  {
    EXPECT_CALL(*managed_lock, is_state_releasing()).WillOnce(Return(ret_val));
  }

  void
  expect_is_state_locked(MockManagedLock* managed_lock, bool ret_val)
  {
    EXPECT_CALL(*managed_lock, is_state_locked()).WillOnce(Return(ret_val));
  }

  void
  expect_is_state_shutdown(MockManagedLock* managed_lock, bool ret_val)
  {
    EXPECT_CALL(*managed_lock, is_state_shutdown()).WillOnce(Return(ret_val));
  }

  void
  expect_is_action_acquire_lock(MockManagedLock* managed_lock, bool ret_val)
  {
  ...
  {
    EXPECT_CALL(
        mock_image_dispatch, set_require_lock(init_shutdown, direction, _))
      ...
      bool init_shutdown)
  {
    if (mock_image_ctx.clone_copy_on_read ||
      ...
      expect_set_require_lock(
          mock_image_dispatch, init_shutdown, io::DIRECTION_BOTH);
    } else {
      expect_set_require_lock(
          mock_image_dispatch, init_shutdown, io::DIRECTION_WRITE);
    }
  }

  void
  expect_unset_require_lock(MockImageDispatch& mock_image_dispatch)
  {
    EXPECT_CALL(mock_image_dispatch, unset_require_lock(io::DIRECTION_BOTH));
  }

  void
  expect_register_dispatch(MockExclusiveLockImageCtx& mock_image_ctx)
  {
    EXPECT_CALL(*mock_image_ctx.io_image_dispatcher, register_dispatch(_));
  }

  void
  expect_shut_down_dispatch(MockExclusiveLockImageCtx& mock_image_ctx)
  {
  ...
  void
  expect_prepare_lock_complete(MockExclusiveLockImageCtx& mock_image_ctx)
  {
    EXPECT_CALL(*mock_image_ctx.state, handle_prepare_lock_complete());
  }

  void
  expect_pre_acquire_request(MockPreAcquireRequest& pre_acquire_request, int r)
  {
      ...
      int r)
  {
    EXPECT_CALL(post_acquire_request, send())
        .WillOnce(DoAll(
            FinishLockUnlock(&post_acquire_request),
            CompleteRequest(&post_acquire_request, r)));
    expect_set_state_post_acquiring(mock_exclusive_lock);
  }

  void
  expect_pre_release_request(MockPreReleaseRequest& pre_release_request, int r)
  {
  ...
  void
  expect_notify_acquired_lock(MockExclusiveLockImageCtx& mock_image_ctx)
  {
    EXPECT_CALL(*mock_image_ctx.image_watcher, notify_acquired_lock()).Times(1);
  }

  void
  expect_notify_released_lock(MockExclusiveLockImageCtx& mock_image_ctx)
  {
    EXPECT_CALL(*mock_image_ctx.image_watcher, notify_released_lock()).Times(1);
  }

  void
  expect_flush_notifies(MockExclusiveLockImageCtx& mock_image_ctx)
  {
  ...
  void
  expect_shut_down(MockManagedLock* managed_lock)
  {
  ...
  int
  when_pre_acquire_lock_handler(MockManagedLock* managed_lock)
  {
    C_SaferCond ctx;
  ...
  int
  when_post_acquire_lock_handler(MockManagedLock* managed_lock, int r)
  {
    C_SaferCond ctx;
  ...
  int
  when_pre_release_lock_handler(MockManagedLock* managed_lock, bool shutting_down)
  {
    C_SaferCond ctx;
  ...
  int
  when_post_release_lock_handler(
      MockManagedLock* managed_lock,
      bool shutting_down,
      int r)
  {
    C_SaferCond ctx;
  ...
  int
  when_post_reacquire_lock_handler(MockManagedLock* managed_lock, int r)
  {
    C_SaferCond ctx;
...
TEST_F(TestMockExclusiveLock, StateTransitions)
{
  ...
  BOOST_SCOPE_EXIT(&exclusive_lock) { exclusive_lock->put(); }
...
TEST_F(TestMockExclusiveLock, AcquireLockError)
{
  ...
  ASSERT_EQ(
      -EBLOCKLISTED,
      when_post_acquire_lock_handler(exclusive_lock, -EBLOCKLISTED));
}
...
TEST_F(TestMockExclusiveLock, PreReleaseLockError)
{
  ...
  ASSERT_EQ(
      -EINVAL, when_post_release_lock_handler(exclusive_lock, false, -EINVAL));
}
...
TEST_F(TestMockExclusiveLock, BlockRequests)
{
  ...
  ASSERT_TRUE(exclusive_lock->accept_request(
      exclusive_lock::OPERATION_REQUEST_TYPE_TRASH_SNAP_REMOVE, &ret_val));
  ASSERT_EQ(0, ret_val);
