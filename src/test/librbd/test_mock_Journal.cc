...
public:
  ...
  void
  shut_down(bool cancel_ops, Context* on_finish)
  {
    MockReplay::get_instance().shut_down(cancel_ops, on_finish);
  }

  int
  decode(bufferlist::const_iterator* it, EventEntry* event_entry)
  {
    return MockReplay::get_instance().decode(it, event_entry);
  }

  void
  process(const EventEntry& event_entry, Context* on_ready, Context* on_commit)
  {
    MockReplay::get_instance().process(event_entry, on_ready, on_commit);
  }

  void
  replay_op_ready(uint64_t op_tid, Context* on_resume)
  {
...
OpenRequest<MockJournalImageCtx>* OpenRequest<MockJournalImageCtx>::s_instance =
    nullptr;
...
public:
  ...
  create(MockJournalImageCtx* image_ctx, bool force, Context* on_finish)
  {
    return &s_instance;
  }
...
PromoteRequest<MockJournalImageCtx>
    PromoteRequest<MockJournalImageCtx>::s_instance;

template <>
struct ObjectDispatch<MockJournalImageCtx> : public io::MockObjectDispatch {
  static ObjectDispatch* s_instance;

  static ObjectDispatch*
  create(MockJournalImageCtx* image_ctx, Journal<MockJournalImageCtx>* journal)
  {
    ceph_assert(s_instance != nullptr);
    return s_instance;
  }
...
ObjectDispatch<MockJournalImageCtx>*
    ObjectDispatch<MockJournalImageCtx>::s_instance = nullptr;
...
ACTION_P2(StartReplay, wq, ctx) { wq->queue(ctx, 0); }
...
public:
      ...
      int r)
  {
    ...
    EXPECT_CALL(mock_open_request, send())
        .WillOnce(DoAll(
            Invoke([&mock_open_request, primary]() {
              if (!primary) {
  ...
  void
  expect_shut_down_journaler(::journal::MockJournaler& mock_journaler)
  {
  ...
  void
  expect_register_dispatch(
      MockImageCtx& mock_image_ctx,
      MockObjectDispatch& mock_object_dispatch)
  {
    EXPECT_CALL(
        *mock_image_ctx.io_object_dispatcher,
        register_dispatch(&mock_object_dispatch));
  }

  void
  expect_shut_down_dispatch(MockImageCtx& mock_image_ctx)
  {
    EXPECT_CALL(
        *mock_image_ctx.io_object_dispatcher,
  ...
  void
  expect_start_replay(
      MockJournalImageCtx& mock_image_ctx,
      ::journal::MockJournaler& mock_journaler,
    ...
    EXPECT_CALL(mock_journaler, start_replay(_))
        .WillOnce(DoAll(
            ...
            StartReplay(
                ...
                new C_ReplayAction(&m_replay_handler, action))));
  }

  void
  expect_stop_replay(::journal::MockJournaler& mock_journaler)
  {
      ...
      bool cancel_ops = false)
  {
    EXPECT_CALL(mock_journal_replay, shut_down(cancel_ops, _))
        .WillOnce(
            WithArg<1>(Invoke([this, &mock_image_ctx, r](Context* on_flush) {
              this->commit_replay(mock_image_ctx, on_flush, r);
            })));
  }

  void
  expect_get_data(::journal::MockReplayEntry& mock_replay_entry)
  {
  ...
  void
  expect_replay_process(MockJournalReplay& mock_journal_replay)
  {
    EXPECT_CALL(mock_journal_replay, decode(_, _)).WillOnce(Return(0));
    EXPECT_CALL(mock_journal_replay, process(_, _, _))
        .WillOnce(DoAll(
            WithArg<1>(CompleteContext(0, static_cast<asio::ContextWQ*>(NULL))),
            WithArg<2>(Invoke(this, &TestMockJournal::save_commit_context))));
  }

  void
  expect_start_append(::journal::MockJournaler& mock_journaler)
  {
  ...
  expect_set_append_batch_options(
      ...
      bool user_flushed)
  {
    if (mock_image_ctx.image_ctx->config.get_val<bool>(
            "rbd_journal_object_writethrough_until_flush") == user_flushed) {
      EXPECT_CALL(mock_journaler, set_append_batch_options(_, _, _));
    }
  }

  void
  expect_stop_append(::journal::MockJournaler& mock_journaler, int r)
  {
  ...
  void
  expect_committed(::journal::MockJournaler& mock_journaler, size_t events)
  {
    EXPECT_CALL(
        mock_journaler,
        committed(MatcherCast<const ::journal::MockReplayEntryProxy&>(_)))
        .Times(events);
  }

  void
  expect_append_journaler(::journal::MockJournaler& mock_journaler)
  {
  ...
  void
  expect_wait_future(::journal::MockFuture& mock_future, Context** on_safe)
  {
    EXPECT_CALL(mock_future, wait(_)).WillOnce(SaveArg<0>(on_safe));
  }

  void
  expect_future_committed(::journal::MockJournaler& mock_journaler)
  {
    EXPECT_CALL(
        mock_journaler,
        committed(MatcherCast<const ::journal::MockFutureProxy&>(_)));
  }

  void
  expect_future_is_valid(::journal::MockFuture& mock_future)
  {
    EXPECT_CALL(mock_future, is_valid()).WillOnce(Return(false));
  }

  void
  expect_flush_commit_position(::journal::MockJournaler& mock_journaler)
  {
  ...
  int
  when_open(MockJournal* mock_journal)
  {
    C_SaferCond ctx;
  ...
  int
  when_close(MockJournal* mock_journal)
  {
    C_SaferCond ctx;
  ...
  uint64_t
  when_append_compare_and_write_event(
      MockJournalImageCtx& mock_image_ctx,
      MockJournal* mock_journal,
      uint64_t length)
  {
    ...
    std::shared_lock owner_locker{mock_image_ctx.owner_lock};
    return mock_journal->append_compare_and_write_event(
        0, length, cmp_bl, write_bl, false);
  }

  uint64_t
  when_append_io_event(
      MockJournalImageCtx& mock_image_ctx,
      MockJournal* mock_journal,
      int filter_ret_val)
  {
  ...
  void
  save_commit_context(Context* ctx)
  {
  ...
  void
  commit_replay(MockJournalImageCtx& mock_image_ctx, Context* on_flush, int r)
  {
      ...
      bool primary = true)
  {
    ...
    expect_open_journaler(
        mock_image_ctx, mock_journaler, mock_open_request, primary, 0);
  ...
  static void
  invoke_replay_ready(::journal::ReplayHandler* handler)
  {
    handler->handle_entries_available();
  }

  static void
  invoke_replay_complete(::journal::ReplayHandler* handler, int r)
  {
...
TEST_F(TestMockJournal, StateTransitions)
{
  ...
  BOOST_SCOPE_EXIT(&mock_journal) { mock_journal->put(); }
  ...
  expect_construct_journaler(mock_journaler);
  expect_open_journaler(
      mock_image_ctx, mock_journaler, mock_open_request, true, 0);
  expect_get_max_append_size(mock_journaler, 1 << 16);
  expect_start_replay(
      mock_image_ctx, mock_journaler, std::bind(&invoke_replay_ready, _1));
  ...
  expect_replay_process(mock_journal_replay);
  expect_try_pop_front(
      mock_image_ctx, mock_journaler, false, mock_replay_entry,
...
TEST_F(TestMockJournal, InitError)
{
  ...
  expect_construct_journaler(mock_journaler);
  expect_open_journaler(
      mock_image_ctx, mock_journaler, mock_open_request, true, -EINVAL);
  expect_shut_down_journaler(mock_journaler);
...
TEST_F(TestMockJournal, ReplayCompleteError)
{
  ...
  expect_get_max_append_size(mock_journaler, 1 << 16);
  expect_start_replay(
      mock_image_ctx, mock_journaler, std::bind(&invoke_replay_complete, _1, 0));
...
TEST_F(TestMockJournal, EventAndIOCommitOrder)
{
  ...
  MockJournalOpenRequest mock_open_request;
  open_journal(
      mock_image_ctx, mock_journal, mock_object_dispatch, mock_journaler,
      mock_open_request);
...
TEST_F(TestMockJournal, AppendCompareAndWriteEvent)
{
  ...
  expect_wait_future(mock_future, &on_journal_safe);
  ASSERT_EQ(
      1U, when_append_compare_and_write_event(
              mock_image_ctx, mock_journal, 1 << 16));
  mock_journal->get_work_queue()->drain();
...
TEST_F(TestMockJournal, AppendDisabled)
{
  ...
  EXPECT_CALL(mock_image_ctx, get_journal_policy())
      .WillOnce(Return(ictx->get_journal_policy()));
...
TEST_F(TestMockJournal, ResyncRequested)
{
  ...
  expect_get_journaler_tags(
      mock_image_ctx, mock_journaler, 0, {{0, 0, tag_data_bl}}, 0);
...
TEST_F(TestMockJournal, ForcePromoted)
{
  ...
  encode(tag_data, tag_data_bl);
  expect_get_journaler_tags(
      mock_image_ctx, mock_journaler, 0, {{100, 0, tag_data_bl}}, 0);
