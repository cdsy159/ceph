...
struct MockMockManagedLock : public ManagedLock<MockManagedLockImageCtx> {
  MockMockManagedLock(
    ...
    ManagedLock<MockManagedLockImageCtx>(
        ioctx,
        asio_engine,
        oid,
        watcher,
...
template <>
struct ReacquireRequest<MockManagedLockImageCtx>
  : public BaseRequest<ReacquireRequest<MockManagedLockImageCtx>> {
  static ReacquireRequest*
  create(
      librados::IoCtx& ioctx,
      const std::string& oid,
...
template <>
struct ReleaseRequest<MockManagedLockImageCtx>
  : public BaseRequest<ReleaseRequest<MockManagedLockImageCtx>> {
...
ACTION_P2(QueueContext, r, wq) { wq->queue(arg0, r); }

ACTION_P(Notify, ctx) { ctx->complete(0); }
...
public:
  ...
  typedef managed_lock::AcquireRequest<MockManagedLockImageCtx>
      MockAcquireRequest;
  typedef managed_lock::ReacquireRequest<MockManagedLockImageCtx>
      MockReacquireRequest;
  typedef managed_lock::ReleaseRequest<MockManagedLockImageCtx>
      MockReleaseRequest;

  void
  expect_get_watch_handle(
  ...
  expect_acquire_lock(
      MockImageWatcher& watcher,
      asio::ContextWQ* work_queue,
      MockAcquireRequest& acquire_request,
      int r)
  {
  ...
  void
  expect_is_blocklisted(MockImageWatcher& watcher, bool blocklisted)
  {
  ...
  expect_release_lock(
      asio::ContextWQ* work_queue,
      MockReleaseRequest& release_request,
      int r)
  {
  ...
  void
  expect_flush_notifies(MockImageWatcher* mock_watcher)
  {
  ...
  void
  expect_post_reacquired_lock_handler(
      MockImageWatcher& watcher,
  ...
  void
  expect_pre_release_lock_handler(
      MockMockManagedLock& managed_lock,
      bool shutting_down,
      int r)
  {
  ...
  void
  expect_post_release_lock_handler(
      MockMockManagedLock& managed_lock,
      bool shutting_down,
      int expect_r,
      int r)
  {
    EXPECT_CALL(
        managed_lock, post_release_lock_handler(shutting_down, expect_r, _))
  ...
  int
  when_acquire_lock(MockManagedLock& managed_lock)
  {
    C_SaferCond ctx;
    {
  ...
  int
  when_release_lock(MockManagedLock& managed_lock)
  {
    C_SaferCond ctx;
    {
  ...
  int
  when_shut_down(MockManagedLock& managed_lock)
  {
    C_SaferCond ctx;
    {
  ...
  bool
  is_lock_owner(MockManagedLock& managed_lock)
  {
...
TEST_F(TestMockManagedLock, StateTransitions)
{
  ...
  MockManagedLock managed_lock(
      ictx->md_ctx, *ictx->asio_engine, ictx->header_oid,
      mock_image_ctx.image_watcher, librbd::managed_lock::EXCLUSIVE, true, 0);
  InSequence seq;

  MockAcquireRequest request_lock_acquire1;
  expect_acquire_lock(
      *mock_image_ctx.image_watcher, ictx->op_work_queue, request_lock_acquire1,
      0);
  ...
  expect_acquire_lock(
      *mock_image_ctx.image_watcher, ictx->op_work_queue, request_lock_acquire2,
      0);
  ASSERT_EQ(0, when_acquire_lock(managed_lock));
  ASSERT_TRUE(is_lock_owner(managed_lock));
...
TEST_F(TestMockManagedLock, AcquireLockLockedState)
{
  ...
  expect_acquire_lock(
      *mock_image_ctx.image_watcher, ictx->op_work_queue, try_lock_acquire, 0);
  ASSERT_EQ(0, when_acquire_lock(managed_lock));
  ASSERT_EQ(0, when_acquire_lock(managed_lock));
...
TEST_F(TestMockManagedLock, AcquireLockAlreadyLocked)
{
  ...
  MockAcquireRequest try_lock_acquire;
  expect_acquire_lock(
      *mock_image_ctx.image_watcher, ictx->op_work_queue, try_lock_acquire,
      -EAGAIN);
...
TEST_F(TestMockManagedLock, AcquireLockBlocklist)
{
  ...
  expect_acquire_lock(
      *mock_image_ctx.image_watcher, ictx->op_work_queue, request_lock_acquire,
      -EBLOCKLISTED);
...
TEST_F(TestMockManagedLock, ReleaseLockBlocklist)
{
  ...
  expect_post_release_lock_handler(
      managed_lock, false, -EBLOCKLISTED, -EBLOCKLISTED);
...
TEST_F(TestMockManagedLock, ConcurrentRequests)
{
  ...
  expect_acquire_lock(
      *mock_image_ctx.image_watcher, ictx->op_work_queue, request_acquire, 0);
...
TEST_F(TestMockManagedLock, ReacquireLock)
{
  ...
  expect_reacquire_lock(
      *mock_image_ctx.image_watcher, ictx->op_work_queue,
...
TEST_F(TestMockManagedLock, ReacquireWithSameCookie)
{
  ...
  C_SaferCond reacquire_ctx;
  expect_post_reacquired_lock_handler(
      *mock_image_ctx.image_watcher, managed_lock, client_id);
  managed_lock.reacquire_lock(&reacquire_ctx);
