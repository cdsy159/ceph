...
struct UpdateRequest<MockTestImageCtx> {
      ...
      Context* on_finish)
  {
    ...
    s_instance->construct(
        snap_id, start_object_no, end_object_no, new_state, current_state,
        ignore_enoent);
    return s_instance;
  }
...
RefreshRequest<MockTestImageCtx>* RefreshRequest<MockTestImageCtx>::s_instance =
    nullptr;
UnlockRequest<MockTestImageCtx>* UnlockRequest<MockTestImageCtx>::s_instance =
    nullptr;
UpdateRequest<MockTestImageCtx>* UpdateRequest<MockTestImageCtx>::s_instance =
    nullptr;
...
public:
  ...
  expect_refresh(
      ...
      const ceph::BitVector<2u>& object_map,
      int r)
  {
    EXPECT_CALL(mock_refresh_request, send())
        .WillOnce(Invoke([&mock_image_ctx, &mock_refresh_request, &object_map,
                          r]() {
          *mock_refresh_request.object_map = object_map;
          mock_image_ctx.image_ctx->op_work_queue->queue(
              mock_refresh_request.on_finish, r);
        }));
  }
      ...
      int r)
  {
    EXPECT_CALL(mock_unlock_request, send())
        .WillOnce(Invoke([&mock_image_ctx, &mock_unlock_request, r]() {
          mock_image_ctx.image_ctx->op_work_queue->queue(
              mock_unlock_request.on_finish, r);
        }));
  }
  ...
  {
    EXPECT_CALL(
        mock_update_request, construct(
                                 snap_id, start_object_no, end_object_no,
                                 new_state, current_state, ignore_enoent))
        .Times(1);
...
TEST_F(TestMockObjectMap, NonDetainedUpdate)
{
  ...
  expect_update(
      mock_image_ctx, mock_update_request, CEPH_NOSNAP, 0, 1, 1, {}, false,
      &finish_update_1);
  Context* finish_update_2;
  expect_update(
      mock_image_ctx, mock_update_request, CEPH_NOSNAP, 1, 2, 1, {}, false,
      &finish_update_2);
  ...
  MockObjectMap* mock_object_map =
      new MockObjectMap(mock_image_ctx, CEPH_NOSNAP);
  BOOST_SCOPE_EXIT(&mock_object_map) { mock_object_map->put(); }
...
TEST_F(TestMockObjectMap, DetainedUpdate)
{
  ...
  Context* finish_update_1;
  expect_update(
      mock_image_ctx, mock_update_request, CEPH_NOSNAP, 1, 4, 1, {}, false,
      &finish_update_1);
  Context* finish_update_2 = nullptr;
  expect_update(
      mock_image_ctx, mock_update_request, CEPH_NOSNAP, 1, 3, 1, {}, false,
      &finish_update_2);
  Context* finish_update_3 = nullptr;
  expect_update(
      mock_image_ctx, mock_update_request, CEPH_NOSNAP, 2, 3, 1, {}, false,
      &finish_update_3);
  Context* finish_update_4 = nullptr;
  expect_update(
      mock_image_ctx, mock_update_request, CEPH_NOSNAP, 0, 2, 1, {}, false,
      &finish_update_4);
  ...
  {
    std::shared_lock image_locker{mock_image_ctx.image_lock};
    mock_object_map->aio_update(
        CEPH_NOSNAP, 1, 4, 1, {}, {}, false, &update_ctx1);
    mock_object_map->aio_update(
        CEPH_NOSNAP, 1, 3, 1, {}, {}, false, &update_ctx2);
    mock_object_map->aio_update(
        CEPH_NOSNAP, 2, 3, 1, {}, {}, false, &update_ctx3);
    mock_object_map->aio_update(
        CEPH_NOSNAP, 0, 2, 1, {}, {}, false, &update_ctx4);
  }
