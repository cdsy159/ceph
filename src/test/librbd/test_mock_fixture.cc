...
void
TestMockFixture::TearDownTestCase()
{
...
TestMockFixture::TearDown()
{
  ...
  dynamic_cast<librados::MockTestMemCluster*>(
      librados_test_stub::get_cluster().get())
      ->default_to_dispatch();
...
void
TestMockFixture::expect_unlock_exclusive_lock(librbd::ImageCtx& ictx)
{
  EXPECT_CALL(
  ...
  if (ictx.test_features(RBD_FEATURE_DIRTY_CACHE)) {
    EXPECT_CALL(
        get_mock_io_ctx(ictx.md_ctx), exec(
                                          ictx.header_oid, _, StrEq("rbd"),
                                          StrEq("set_features"), _, _, _, _))
        .WillOnce(DoDefault());
    EXPECT_CALL(
        get_mock_io_ctx(ictx.md_ctx), exec(
                                          ictx.header_oid, _, StrEq("rbd"),
                                          StrEq("metadata_set"), _, _, _, _))
        .WillOnce(DoDefault());
    EXPECT_CALL(
        get_mock_io_ctx(ictx.md_ctx), exec(
                                          ictx.header_oid, _, StrEq("rbd"),
                                          StrEq("metadata_remove"), _, _, _, _))
        .WillOnce(DoDefault());
  }
}

void
TestMockFixture::expect_op_work_queue(librbd::MockImageCtx& mock_image_ctx)
{
...
void
TestMockFixture::initialize_features(
...
void
TestMockFixture::expect_is_journal_appending(
    librbd::MockJournal& mock_journal,
    bool appending)
{
...
void
TestMockFixture::expect_is_journal_replaying(librbd::MockJournal& mock_journal)
{
  EXPECT_CALL(mock_journal, is_journal_replaying()).WillOnce(Return(false));
}

void
TestMockFixture::expect_is_journal_ready(librbd::MockJournal& mock_journal)
{
  EXPECT_CALL(mock_journal, is_journal_ready()).WillOnce(Return(true));
}

void
TestMockFixture::expect_allocate_op_tid(librbd::MockImageCtx& mock_image_ctx)
{
...
void
TestMockFixture::expect_append_op_event(
    librbd::MockImageCtx& mock_image_ctx,
    bool can_affect_io,
    int r)
{
...
void
TestMockFixture::expect_commit_op_event(
    librbd::MockImageCtx& mock_image_ctx,
    int r)
{
  if (mock_image_ctx.journal != nullptr) {
