...
struct TestMockTrashMoveRequest : public TestMockFixture {
      ...
      int r)
  {
    ...
    EXPECT_CALL(
        get_mock_io_ctx(mock_image_ctx.md_ctx),
        exec(
            StrEq("rbd_directory"), _, StrEq("rbd"), StrEq("dir_remove_image"),
...
TEST_F(TestMockTrashMoveRequest, Success)
{
  ...
  expect_trash_add(
      mock_image_ctx, "image id", cls::rbd::TRASH_IMAGE_SOURCE_USER,
      "image name", delete_time, 0);
  ...
  auto req = MockMoveRequest::create(
      mock_image_ctx.md_ctx, "image id", trash_image_spec, &ctx);
  req->send();
