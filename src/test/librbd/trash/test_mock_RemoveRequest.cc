...
struct TestMockTrashRemoveRequest : public TestMockFixture {
  ...
  void
  expect_set_deleting_state(MockTestImageCtx& mock_image_ctx, int r)
  {
    expect_set_state(
  ...
  void
  expect_restore_normal_state(MockTestImageCtx& mock_image_ctx, int r)
  {
    expect_set_state(
        mock_image_ctx, cls::rbd::TRASH_IMAGE_STATE_NORMAL,
        cls::rbd::TRASH_IMAGE_STATE_REMOVING, r);
  }

  void
  expect_close_image(MockTestImageCtx& mock_image_ctx, int r)
  {
    EXPECT_CALL(*mock_image_ctx.state, close(_))
        .WillOnce(Invoke([r](Context* on_finish) { on_finish->complete(r); }));
  }

  void
  expect_remove_image(MockImageRemoveRequest& mock_image_remove_request, int r)
  {
  ...
  void
  expect_remove_trash_entry(MockTestImageCtx& mock_image_ctx, int r)
  {
    bufferlist in_bl;
    encode(mock_image_ctx.id, in_bl);
    ...
    EXPECT_CALL(
        get_mock_io_ctx(mock_image_ctx.md_ctx),
        exec(
            StrEq("rbd_trash"), _, StrEq("rbd"), StrEq("trash_remove"),
            ContentsEqual(in_bl), _, _, _))
        .WillOnce(Return(r));
  }
};
...
TEST_F(TestMockTrashRemoveRequest, Success)
{
  ...
  C_SaferCond ctx;
  auto req = MockRemoveRequest::create(
      mock_image_ctx.md_ctx, &mock_image_ctx, nullptr, false, m_prog_ctx, &ctx);
  req->send();
