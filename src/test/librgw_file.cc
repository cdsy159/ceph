...
typedef std::tuple<string, uint64_t, struct rgw_file_handle*>
    fid_type; //in c++2014 can alias...
...
{
  int ret = rgw_mount2(
      h_rgw, uid.c_str(), access_key.c_str(), secret_key.c_str(), "/", &fs,
      RGW_MOUNT_FLAG_NONE);
  ASSERT_EQ(ret, 0);
...
TEST(LibRGW, LIST_BUCKETS)
{
  ...
  uint64_t offset = 0;
  int ret = rgw_readdir(
      fs, fs->root_fh, &offset, r1_cb, &fids1, &eof, RGW_READDIR_FLAG_NONE);
  for (auto& fid : fids1) {
...
TEST(LibRGW, LOOKUP_BUCKETS)
{
  ...
  for (auto& fid_row : bucket_matrix) {
    ...
    ASSERT_EQ(
        0, rgw_lookup(
               fs, fs->root_fh, get<0>(fid).c_str(), &rgw_fh,
...
r2_cb(
    ...
    uint32_t flags)
{
  std::vector<fid_type>& obj_vector =
      *(static_cast<std::vector<fid_type>*>(arg));
...
TEST(LibRGW, LIST_OBJECTS)
{
  ...
  for (auto& fid_row : bucket_matrix) {
    ...
    ldout(g_ceph_context, 0)
        << __func__ << " readdir on bucket " << get<0>(fid) << dendl;

    bool eof = false;
    uint64_t offset = 0;
    int ret = rgw_readdir(
        fs, bucket_fh, &offset, r2_cb, &obj_vector, &eof, RGW_READDIR_FLAG_NONE);
  ...
  if (do_getattr) {
    ...
    for (auto& fid_row : bucket_matrix) {
      ...
      for (auto& obj : obj_vector) {
        ...
        ret = rgw_lookup(
            fs, bucket_fh, get<0>(obj).c_str(), &obj_fh, nullptr /* stat */,
            0 /* mask */, 0 /* flags */);
        ASSERT_EQ(ret, 0);
...
TEST(LibRGW, SHUTDOWN) { librgw_shutdown(h_rgw); }
