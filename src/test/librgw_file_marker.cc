    ...
    uint32_t flags)
{
  ...
  printf(
      "%s bucket=%s dir=%s iv count=%d called back name=%s flags=%d\n",
      __func__, bucket_name.c_str(), marker_dir.c_str(), dvec.count, name,
      flags);
...
TEST(LibRGW, MARKER2_READDIR)
{
  if (do_marker2) {
    ...
    do {
      ASSERT_TRUE(dvec.count <= max_iterations);
      int ret = rgw_readdir2(
          fs, marker_fh, (marker.length() > 0) ? marker.c_str() : nullptr,
          r2_cb, &dvec, &eof, RGW_READDIR_FLAG_NONE);
      ASSERT_EQ(ret, 0);
      ASSERT_GE(dvec.obj_names.size(), 0);
  ...
  for (auto arg_iter = args.begin(); arg_iter != args.end();) {
    if (ceph_argparse_witharg(args, arg_iter, &val, "--access", (char*)nullptr)) {
      ...
      owner_gid = std::stoi(val);
                   ...
                   args, arg_iter, &val, "--nobjs", (char*)nullptr)) {
