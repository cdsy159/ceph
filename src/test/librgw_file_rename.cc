...
{
  int ret = rgw_mount2(
      rgw, userid.c_str(), access_key.c_str(), secret_key.c_str(), "/", &fs,
      RGW_MOUNT_FLAG_NONE);
  ASSERT_EQ(ret, 0);
...
TEST(LibRGW, CREATE_BUCKETS)
{
  if (do_create) {
    ...
    ret = rgw_mkdir(
        fs, fs->root_fh, bucket1_name.c_str(), &st, create_mask, &bucket1_fh,
        RGW_MKDIR_FLAG_NONE);
    ASSERT_EQ(ret, 0);
    ...
    ret = rgw_mkdir(
        fs, fs->root_fh, bucket2_name.c_str(), &st, create_mask, &bucket2_fh,
        RGW_MKDIR_FLAG_NONE);
    ASSERT_EQ(ret, 0);
...
TEST(LibRGW, LOOKUP_BUCKETS)
{
  int ret{0};
  ret = rgw_lookup(
      fs, fs->root_fh, bucket1_name.c_str(), &bucket1_fh, nullptr, 0,
      RGW_LOOKUP_FLAG_NONE);
  ASSERT_EQ(ret, 0);

  ret = rgw_lookup(
      fs, fs->root_fh, bucket2_name.c_str(), &bucket2_fh, nullptr, 0,
      RGW_LOOKUP_FLAG_NONE);
  ASSERT_EQ(ret, 0);
}

static inline int
make_object(struct rgw_file_handle* parent_fh, const string& name)
{
  int ret{0};
  ret = rgw_lookup(
      fs, parent_fh, name.c_str(), &object_fh, nullptr, 0,
...
TEST(LibRGW, TOPDIR_RENAME)
{
  ...
  /* now move it */
  ret = rgw_rename(
      fs, bucket1_fh, obj_name1.c_str(), bucket1_fh, obj_name2.c_str(),
      0 /* flags */);
  ASSERT_EQ(ret, 0);
  ...
  ret = rgw_lookup(
      fs, bucket1_fh, obj_name2.c_str(), &name2_fh, nullptr, 0,
      RGW_LOOKUP_FLAG_NONE);
  ASSERT_EQ(ret, 0);
  /* release file handle */
...
TEST(LibRGW, SUBDIR_RENAME)
{
  ...
  if (do_create) {
    ...
    ret = rgw_mkdir(
        fs, bucket1_fh, subdir1_name.c_str(), &st, create_mask, &subdir1_fh,
        RGW_MKDIR_FLAG_NONE);
    ASSERT_EQ(ret, 0);
    ...
    ret = rgw_lookup(
        fs, bucket1_fh, subdir1_name.c_str(), &subdir1_fh, nullptr, 0,
        RGW_LOOKUP_FLAG_NONE);
    ASSERT_EQ(ret, 0);
  }
  ...
  /* now move it */
  ret = rgw_rename(
      fs, subdir1_fh, obj_name1.c_str(), subdir1_fh, obj_name2.c_str(),
      0 /* flags */);
  ASSERT_EQ(ret, 0);
  /* now check the result */
  struct rgw_file_handle* name2_fh;
  ret = rgw_lookup(
      fs, subdir1_fh, obj_name2.c_str(), &name2_fh, nullptr, 0,
      RGW_LOOKUP_FLAG_NONE);
  ASSERT_EQ(ret, 0);
  /* release file handle */
  ret = rgw_fh_rele(fs, name2_fh, 0 /* flags */);
...
TEST(LibRGW, CROSS_BUCKET_RENAME)
{
  ...
  if (do_create) {
    ...
    ret = rgw_mkdir(
        fs, bucket2_fh, subdir2_name.c_str(), &st, create_mask, &subdir2_fh,
        RGW_MKDIR_FLAG_NONE); // galahad/mork
    ASSERT_EQ(ret, 0);
    ...
    ret = rgw_lookup(
        fs, bucket2_fh, subdir2_name.c_str(), &subdir2_fh, nullptr, 0,
        RGW_LOOKUP_FLAG_NONE);
    ASSERT_EQ(ret, 0);
  }
  ...
  ret = rgw_rename(
      fs, subdir1_fh, obj_name1.c_str(), subdir2_fh, obj_name3.c_str(),
      0 /* flags */);
  ASSERT_EQ(ret, 0);
  /* now check the result */
  struct rgw_file_handle* name3_fh; // galahad/mork/zoot
  ret = rgw_lookup(
      fs, subdir2_fh, obj_name3.c_str(), &name3_fh, nullptr, 0,
      RGW_LOOKUP_FLAG_NONE);
  ASSERT_EQ(ret, 0);
  /* release file handle */
