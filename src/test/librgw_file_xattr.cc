...
{
  int ret = rgw_mount(
      rgw_h, uid.c_str(), access_key.c_str(), secret_key.c_str(), &fs,
      RGW_MOUNT_FLAG_NONE);
  ASSERT_EQ(ret, 0);
  ...
  for (ix = 0; ix < sz; ++ix) {
    ...
    if (verbose) {
      std::cout << "name: " << dir.name
                << " refcnt: " << dir.rgw_fh->get_refcnt() << std::endl;
    }
    if (ix == 0) {
...
TEST(LibRGW, SETXATTR1)
{
  ...
  rgw_xattrstr xattr_v = {
      const_cast<char*>(val1.c_str()), uint32_t(val1.length())};
...
extern "C" {
static int
getattr_cb(rgw_xattrlist* attrs, void* arg, uint32_t flags)
{
...
TEST(LibRGW, GETXATTR1)
{
  ...
  int ret = rgw_getxattrs(
      fs, dir.fh, &xattrlist, getattr_cb, &out_attrmap, RGW_GETXATTR_FLAG_NONE);
  ASSERT_EQ(ret, 0);
  /* check exposed attrs */
...
TEST(LibRGW, LSXATTR1)
{
  ...
  int ret = rgw_lsxattrs(
      fs, dir.fh, &filter_prefix, getattr_cb, &out_attrmap,
