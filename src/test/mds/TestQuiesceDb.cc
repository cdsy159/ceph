...
struct GenericVerboseErrorCode {
  ...
  GenericVerboseErrorCode(int error_code) :
...
operator<<(
    ...
    const GenericVerboseErrorCode& ec)
{
  if (0 == ec.error_code) {
    return os << "Success(0)";
  } else {
    return os << std::generic_category().message(ec.error_code) << "("
              << ec.error_code << ")";
  }
};
...
protected:
  ...
  timed_run(D timeout, Function&& f, Args&&... args)
  {
    ...
    auto tt = std::thread(
        [job = std::move(job)](std::promise<void> done) {
          job();
          done.set_value();
        },
        std::move(done));
  ...
  struct TestQuiesceDbManager : public QuiesceDbManager {
    ...
    decltype(pending_requests)&
    internal_pending_requests()
    {
    ...
    bool
    db_thread_has_work() const override
    {
  ...
  epoch_t epoch = 0;
  std::map<QuiesceInterface::PeerId, std::unique_ptr<TestQuiesceDbManager>>
      managers;
  ...
  std::future<void>
  add_ack_hook(AckHook&& predicate)
  {
    std::lock_guard l(comms_mutex);
    auto&& [_, promise] =
        ack_hooks.emplace_back(predicate, std::promise<void>{});
    return promise.get_future();
  }

  using ListingHook =
      std::function<bool(QuiesceInterface::PeerId, QuiesceDbListing&)>;
  std::list<std::pair<ListingHook, std::promise<void>>> listing_hooks;

  std::future<void>
  add_listing_hook(ListingHook&& predicate)
  {
    std::lock_guard l(comms_mutex);
    auto&& [_, promise] =
        listing_hooks.emplace_back(predicate, std::promise<void>{});
    return promise.get_future();
  }
  ...
  void
  configure_cluster(
      std::vector<QuiesceInterface::PeerId> leader_and_replicas = {
          mds_gid_t(1), mds_gid_t(2), mds_gid_t(3)})
  {
    ++epoch;
    ASSERT_GE(leader_and_replicas.size(), 1);
    std::unordered_set<QuiesceInterface::PeerId> members(
        leader_and_replicas.begin(), leader_and_replicas.end());
    ...
    for (const auto& [this_peer, mgr] : managers) {
      QuiesceClusterMembership mem = {
          ...
          leader,
          members,
          [epoch = this->epoch, this, leader,
           me = this_peer](auto recipient, auto listing) {
            std::unique_lock l(comms_mutex);
            if (epoch == this->epoch) {
              if (this->managers.contains(recipient)) {
                std::queue<std::promise<void>> done_hooks;
                dout(10) << "listing from " << me << " (leader=" << leader
                         << ") to " << recipient << " for version "
                         << listing.db_version << " with "
                         << listing.sets.size() << " sets" << dendl;

                for (auto it = listing_hooks.begin();
                     it != listing_hooks.end();) {
                ...
                this->managers[recipient]->submit_peer_listing(
                    {me, std::move(listing)});
          ...
          [epoch = this->epoch, this, leader, me = this_peer](auto diff_map) {
            ...
            if (epoch == this->epoch) {
              if (this->managers.contains(leader)) {
                std::queue<std::promise<void>> done_hooks;
                dout(10) << "ack from " << me << " to the leader (" << leader
                         << ") for version " << diff_map.db_version << " with "
                         << diff_map.roots.size() << " roots" << dendl;
                ...
                if (!inserted) {
                  if (it->second.db_version == diff_map.db_version) {
                    if (it->second.roots == diff_map.roots) {
                      dout(1) << "WARNING: detected a potentialy redundant ack"
                              << dendl;
                    }
                  }
                  it->second = diff_map;
                ...
                this->managers[leader]->submit_peer_ack(
                    {me, std::move(diff_map)});
                comms_cond.notify_all();
                l.unlock();
                while (!done_hooks.empty()) {
                  done_hooks.front().set_value();
    ...
    dout(6) << "\n === configured cluster with the following members, starting "
               "with the leader: "
            << leader_and_replicas << dendl;
  }

  struct TestRequestContext : public QuiesceDbManager::RequestContext,
                              public C_SaferCond {
    void
    finish(int r) override
    {
      C_SaferCond::finish(r);
    }
    ...
    TestRequestContext(const QuiesceDbTest& parent) :
      parent(parent)
    {}

    ~TestRequestContext() { wait(); }

    bool
    start(std::invocable<QuiesceDbRequest&> auto const& c)
    {
      done = false;
      response.clear();
      request.reset(c);
    ...
    GenericVerboseErrorCode
    wait_result_for(double seconds)
    {
  ...
  const QuiesceDbManager::AgentCallback::Notify QUIESCING_AGENT_CB =
      [](QuiesceMap& quiesce_map) {
        dout(15) << "QUIESCING_AGENT_CB: notified with "
                 << quiesce_map.roots.size() << " roots for version "
                 << quiesce_map.db_version << dendl;
        for (auto it = quiesce_map.roots.begin();
             it != quiesce_map.roots.end();) {
          switch (it->second.state) {
          case QS_QUIESCING:
            it->second.state = QS_QUIESCED;
            dout(10) << "QUIESCING_AGENT_CB: reporting '" << it->first
                     << "' as " << it->second.state << dendl;
            it++;
            break;
          default:
  ...
  const QuiesceDbManager::AgentCallback::Notify FAILING_AGENT_CB =
      [](QuiesceMap& quiesce_map) {
        dout(15) << "FAILING_AGENT_CB: notified with "
                 << quiesce_map.roots.size() << " roots for version "
                 << quiesce_map.db_version << dendl;
        for (auto it = quiesce_map.roots.begin();
             it != quiesce_map.roots.end();) {
          ...
          case QS_QUIESCING:
            ...
            dout(10) << "FAILING_AGENT_CB: reporting '" << it->first << "' as "
                     << it->second.state << dendl;
            it++;
            break;
          default:
            it = quiesce_map.roots.erase(it);
  ...
  const QuiesceDbManager::AgentCallback::Notify SILENT_AGENT_CB =
      [](QuiesceMap& quiesce_map) {
        dout(15) << "SILENT_AGENT_CB: nacking quiesce map version "
                 << quiesce_map.db_version << " with "
                 << quiesce_map.roots.size() << " roots" << dendl;
        return false;
      };
  ...
  TestRequestContext&
  start_request(std::invocable<QuiesceDbRequest&> auto const& c)
  {
  ...
  TestQuiesceDbManager::Db&
  db(QuiesceInterface::PeerId peer)
  {
    return managers[peer]->internal_db();
  }

  static GenericVerboseErrorCode
  ERR(int val)
  {
  ...
  static QuiesceTimeInterval
  sec(double val)
  {
    return std::chrono::duration_cast<QuiesceTimeInterval>(
...
TEST_F(QuiesceDbTest, SetCreation)
{
  ...
  EXPECT_EQ(OK(), run_request([](auto& r) { r.include_roots({"root2"}); }));
  ...
  EXPECT_EQ(OK(), run_request([](auto& r) { r.reset_roots({"root2"}); }));
...
template <class T>
constexpr std::array<std::optional<T>, 2>
nullopt_and_default()
{
...
void
cartesian_apply(F func, std::array<V, S> const&... array_args)
{
...
TEST_F(QuiesceDbTest, QuiesceRequestValidation)
{
  ...
  auto checkRequest =
      [](decltype(std::declval<QuiesceDbRequest>().control.roots_op) const& op,
         ...
         decltype(std::declval<QuiesceDbRequest>().if_version)
             const& if_version,
         decltype(std::declval<QuiesceDbRequest>().timeout) const& timeout,
         decltype(std::declval<QuiesceDbRequest>().expiration)
             const& expiration,
  ...
  const auto ops = std::array{
      QuiesceDbRequest::RootsOp::INCLUDE_OR_QUERY,
      QuiesceDbRequest::RootsOp::EXCLUDE_OR_CANCEL,
      QuiesceDbRequest::RootsOp::RESET_OR_RELEASE,
      QuiesceDbRequest::RootsOp::__INVALID};
  ...
  const auto roots =
      std::array{QuiesceDbRequest::Roots{}, QuiesceDbRequest::Roots{"root1"}};

  cartesian_apply(
      checkRequest, ops, strings, versions, intervals, intervals, intervals,
      roots);
}
...
TEST_F(QuiesceDbTest, RootSanitization)
{
  ...
  ASSERT_EQ(OK(), run_request([this](auto& r) {
              r.set_id = "set1";
              r.include_roots(
                  {"file:root1", fmt::format("file://{}/root2", fs_id),
                   fmt::format("//{}/root3", fs_name),
                   fmt::format("inode://{}/4", fs_id),
                   fmt::format("inode://{}/5", fs_name),
                   "inode:18446744073709551615", "inode:/18446744073709551614",
                   "inode:/18446744073709551613/", "root6/.///./..////root6//"});
            }));
  ...
  EXPECT_TRUE(db(mds_gid_t(1))
                  .sets.at("set1")
                  .members.contains("inode:18446744073709551615"));
  EXPECT_TRUE(db(mds_gid_t(1))
                  .sets.at("set1")
                  .members.contains("inode:18446744073709551614"));
  EXPECT_TRUE(db(mds_gid_t(1))
                  .sets.at("set1")
                  .members.contains("inode:18446744073709551613"));
  EXPECT_TRUE(
      db(mds_gid_t(1)).sets.at("set1").members.contains("file:/root6/root6"));

  EXPECT_EQ(ERR(EINVAL), run_request([](auto& r) {
              r.include_roots({"//10/root1"});
            }));
  EXPECT_EQ(0, last_request->response.sets.size());

  EXPECT_EQ(ERR(EINVAL), run_request([](auto& r) {
              r.set_id = "badset";
              r.include_roots({"//badfsname/root1"});
            }));
  EXPECT_EQ(0, last_request->response.sets.size());

  EXPECT_EQ(ERR(EINVAL), run_request([](auto& r) {
              r.set_id = "badset";
              r.include_roots({"inode://badfsname/1"});
            }));
  EXPECT_EQ(0, last_request->response.sets.size());

  EXPECT_EQ(ERR(EINVAL), run_request([](auto& r) {
              r.set_id = "badset";
              r.include_roots({"inode:-4"});
            }));
  EXPECT_EQ(0, last_request->response.sets.size());
  ...
  EXPECT_EQ(ERR(EINVAL), run_request([](auto& r) {
              r.set_id = "badset";
              r.include_roots({"inode:1/2/3/4"});
            }));
  EXPECT_EQ(0, last_request->response.sets.size());

  EXPECT_EQ(ERR(EINVAL), run_request([](auto& r) {
              r.set_id = "badset";
              r.include_roots({"inode:abcd"});
            }));
  EXPECT_EQ(0, last_request->response.sets.size());

  EXPECT_EQ(ERR(EINVAL), run_request([](auto& r) {
              r.set_id = "badset";
              r.include_roots({"inode:123-456"});
            }));
  EXPECT_EQ(0, last_request->response.sets.size());

  EXPECT_EQ(ERR(EINVAL), run_request([](auto& r) {
              r.set_id = "badset";
              r.include_roots({"inode:"});
            }));
  EXPECT_EQ(0, last_request->response.sets.size());
...
TEST_F(QuiesceDbTest, SetModification)
{
  ...
  ASSERT_EQ(db(mds_gid_t(1)).sets.at("set1").members.size(), 3);
  ASSERT_TRUE(
      db(mds_gid_t(1)).sets.at("set1").members.at("file:/root1").excluded);
  ASSERT_FALSE(
      db(mds_gid_t(1)).sets.at("set1").members.at("file:/root2").excluded);
  ASSERT_FALSE(
      db(mds_gid_t(1)).sets.at("set1").members.at("file:/root3").excluded);
  ASSERT_FALSE(
      db(mds_gid_t(1)).sets.at("set1").members.contains("file:/root4"));
  ...
  ASSERT_FALSE(
      db(mds_gid_t(1)).sets.at("set1").members.at("file:/root4").excluded);
  ...
  ASSERT_FALSE(
      db(mds_gid_t(1)).sets.at("set2").members.at("file:/root5").excluded);

  // cancel with no set_id should cancel all active sets
  ASSERT_EQ(OK(), run_request([](auto& r) { r.cancel(); }));
  ...
  ASSERT_EQ(
      QuiesceState::QS_CANCELED, db(mds_gid_t(1)).sets.at("set1").rstate.state);
  ASSERT_EQ(
      QuiesceState::QS_CANCELED, db(mds_gid_t(1)).sets.at("set2").rstate.state);
  ...
  ASSERT_EQ(
      QuiesceState::QS_QUIESCING, db(mds_gid_t(1)).sets.at("set1").rstate.state);
}
...
TEST_F(QuiesceDbTest, Timeouts)
{
  ...
  ASSERT_EQ(
      QuiesceState::QS_QUIESCED,
      last_request->response.sets.at("set1").rstate.state);

  std::this_thread::sleep_for(sec(0.15));

  ASSERT_EQ(
      QuiesceState::QS_EXPIRED, db(mds_gid_t(1)).sets.at("set1").rstate.state);
  ...
  ASSERT_EQ(
      QuiesceState::QS_RELEASING,
      last_request->response.sets.at("set2").rstate.state);

  std::this_thread::sleep_for(sec(0.15));

  ASSERT_EQ(
      QuiesceState::QS_EXPIRED, db(mds_gid_t(1)).sets.at("set2").rstate.state);
  ...
  ASSERT_EQ(
      QuiesceState::QS_QUIESCING, db(mds_gid_t(1)).sets.at("set3").rstate.state);

  std::this_thread::sleep_for(sec(0.15));

  ASSERT_EQ(
      QuiesceState::QS_TIMEDOUT,
      db(mds_gid_t(1))
          .sets.at("set3")
          .rstate
          .state); // reset can be used to resurrect a set from a terminal state
  ASSERT_EQ(OK(), run_request([](auto& r) {
              r.set_id = "set3";
              r.reset_roots({"root1"});
            }));
  ASSERT_EQ(
      QuiesceState::QS_QUIESCING,
      last_request->response.sets.at("set3").rstate.state);
}
...
TEST_F(QuiesceDbTest, Failures)
{
...

  {
    ...
    auto did_ack = add_ack_hook([](auto rank, auto const& ack) {
      return ack.roots.contains("file:/root1") &&
             ack.roots.at("file:/root1").state == QS_FAILED;
    });
    ...
    EXPECT_EQ(
        ...
        did_ack.wait_for(std::chrono::milliseconds(100)));
  }

  ASSERT_EQ(OK(), run_request([](auto& r) { r.set_id = "set1"; }));

  EXPECT_EQ(
      QuiesceState::QS_FAILED, db(mds_gid_t(1)).sets.at("set1").rstate.state);
  EXPECT_EQ(
      QuiesceState::QS_FAILED,
      last_request->response.sets.at("set1").rstate.state);
...
TEST_F(QuiesceDbTest, InterruptedQuiesceAwait)
{
  ...
  ASSERT_LE(
      QuiesceClock::now() - then - epsilon,
      last_request->response.sets.at("set2").timeout);
...
TEST_F(QuiesceDbTest, ReleaseAwait)
{
  ...
  EXPECT_EQ(ERR(EINVAL), run_request([](auto& r) { r.release(); }));
}
...
TEST_F(QuiesceDbTest, LeaderShutdown)
{
  ...
  ASSERT_EQ(
      outstanding_awaits.size(),
      managers.at(mds_gid_t(1))->internal_awaits().size());
  ...
  ASSERT_EQ(
      managers.at(mds_gid_t(1))->internal_pending_requests().size(),
...
TEST_F(QuiesceDbTest, MultiRankBootstrap)
{
  ...
  const QuiesceTimeInterval PEER_DISCOVERY_INTERVAL =
  ...
  for (int i = 0; i < 5; i++) {
    ...
    auto saw_discovery = add_listing_hook([](auto recipient,
                                             auto const& listing) {
    ...
    EXPECT_EQ(
        std::future_status::ready,
        saw_discovery.wait_for(
            PEER_DISCOVERY_INTERVAL + std::chrono::milliseconds(100)));
  }
}
...
TEST_F(QuiesceDbTest, MultiRankQuiesce)
{
  ASSERT_NO_FATAL_FAILURE(
      configure_cluster({mds_gid_t(1), mds_gid_t(2), mds_gid_t(3)}));
...

  {
    ...
    auto did_ack = add_ack_hook([&](auto p, auto const& m) {
      if (m.roots.contains("file:/root1") &&
          (m.roots.at("file:/root1").state == QS_QUIESCED)) {
...

  {
    ...
    auto did_ack = add_ack_hook([](auto gid, auto const& ack) {
      return gid == mds_gid_t(3) && ack.roots.contains("file:/root1") &&
             ack.roots.at("file:/root1").state == QS_QUIESCED;
    });
...
TEST_F(QuiesceDbTest, MultiRankRelease)
{
  ...
  // wait for all peers to sync version
  {
    ...
    auto result = comms_cond.wait_for(l, std::chrono::milliseconds(100), [&] {
      auto min_v = std::min(
          {db(mds_gid_t(1)).set_version, db(mds_gid_t(2)).set_version,
           db(mds_gid_t(3)).set_version});
  ...
  for (auto&& gid : {mds_gid_t(1), mds_gid_t(2), mds_gid_t(3)}) {
    EXPECT_EQ(QS_RELEASING, db(gid).sets.at("set1").rstate.state)
        << "db of gid " << gid;
    EXPECT_EQ(QS_RELEASING, db(gid).sets.at("set2").rstate.state)
        << "db of gid " << gid;
  }
...
TEST_F(QuiesceDbTest, MultiRankRecovery)
{
  ...
  ASSERT_NO_FATAL_FAILURE(
      configure_cluster({mds_gid_t(4), mds_gid_t(2), mds_gid_t(3)}));

  EXPECT_EQ(
      std::future_status::ready,
      did_ack41.wait_for(std::chrono::milliseconds(2000)));
  ...
  EXPECT_EQ(
      std::future_status::ready,
      did_ack3.wait_for(std::chrono::milliseconds(2000)));
  EXPECT_EQ(OK(), run_request([](auto& r) {}));
  ASSERT_EQ(2, last_request->response.sets.size());
  EXPECT_EQ(QS_QUIESCED, last_request->response.sets.at("set1").rstate.state);
  EXPECT_EQ(QS_QUIESCED, last_request->response.sets.at("set2").rstate.state);
  ...
  ASSERT_NO_FATAL_FAILURE(configure_cluster(
      {mds_gid_t(1), mds_gid_t(2), mds_gid_t(3), mds_gid_t(4)}));

  EXPECT_EQ(
      std::future_status::ready,
      did_ack42.wait_for(std::chrono::milliseconds(2000)));
  EXPECT_EQ(OK(), run_request([](auto& r) {}));
  ASSERT_EQ(2, last_request->response.sets.size());
  EXPECT_EQ(QS_QUIESCING, last_request->response.sets.at("set1").rstate.state);
  EXPECT_EQ(QS_QUIESCING, last_request->response.sets.at("set2").rstate.state);
...
TEST_F(QuiesceDbTest, AckDuringEpochMismatch)
{
  ...
  // do the acking while our epoch is higher
  {
    // wait for the agent to ack root1 as failed
    auto did_ack = add_ack_hook([](auto gid, auto const& ack) {
      return gid == mds_gid_t(2) && ack.roots.contains("file:/root1") &&
             ack.roots.at("file:/root1").state == QS_QUIESCED;
    });
...
TEST_F(QuiesceDbTest, QuiesceRootMerge)
{
  ...
  auto agent_map = [this]() -> std::optional<QuiesceMap> {
    ...
    managers.at(mds_gid_t(1))
        ->reset_agent_callback([&agent_map_promise](QuiesceMap& map) -> bool {
          try {
    ...
    if (std::future_status::ready ==
        agent_map_future.wait_for(std::chrono::seconds(10))) {
