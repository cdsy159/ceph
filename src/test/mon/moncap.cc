...
TEST(MonCap, ProfileOSD)
{
  ...
  ASSERT_FALSE(cap.is_capable(
      NULL, name, "", "config-key get", ca, true, true, true, {}));
  ...
  ASSERT_TRUE(cap.is_capable(
      NULL, name, "", "config-key get", ca, true, true, true, {}));
  ASSERT_TRUE(cap.is_capable(
      NULL, name, "", "config-key put", ca, true, true, true, {}));
  ASSERT_TRUE(cap.is_capable(
      NULL, name, "", "config-key set", ca, true, true, true, {}));
  ASSERT_TRUE(cap.is_capable(
      NULL, name, "", "config-key exists", ca, true, true, true, {}));
  ASSERT_TRUE(cap.is_capable(
      NULL, name, "", "config-key delete", ca, true, true, true, {}));
}
...
TEST(MonCap, CommandRegEx)
{
  ...
  ASSERT_FALSE(cap.is_allow_all());
  ASSERT_TRUE(
      cap.parse("allow command abc with arg regex \"^[0-9a-z.]*$\"", NULL));
...
TEST(MonCap, ProfileBootstrapRBD)
{
  ...
  name.from_str("mon.a");
  ASSERT_TRUE(cap.is_capable(
      nullptr, name, "", "auth get-or-create",
      {
          {"entity", "client.rbd"},
...
TEST(MonCap, ProfileRBD)
{
  ...
  name.from_str("mon.a");
  ASSERT_FALSE(cap.is_capable(
      nullptr, name, "config-key", "config-key get",
      {
