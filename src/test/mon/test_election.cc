...
static ostream&
_prefix(std::ostream* _dout, const char* prefix, int timesteps)
{
...
struct Election {
  ...
  void
  add_disallowed_leader(int disallowed)
  {
    disallowed_leaders.insert(disallowed);
  }
  ...
  int
  timestep_count() const
  {
    return timesteps_run;
  }
};
...
struct Owner : public ElectionOwner, RankProvider {
  ...
  void
  notify_rank_removed(int removed_rank)
  {
  ...
  void
  persist_connectivity_scores()
  {}

  void
  propose_to_peers(epoch_t e, bufferlist& bl)
  {
    ceph_assert(!rank_deleted);
  ...
  void
  message_victory(const std::set<int>& members)
  {
    ceph_assert(!rank_deleted);
    for (auto i : members) {
      if (i == rank)
  ...
  bool
  is_stretch_marked_down_mons(int rank) const
  {
  ...
  bool
  is_tiebreaker(int rank) const
  {
    return tiebreaker_mon_rank == rank;
  }

  bool
  is_current_member(int r) const
  {
    return quorum.count(r) != 0;
  }

  void
  receive_propose(int from, epoch_t e, std::shared_ptr<ConnectionTracker> oct)
  {
    if (rank_deleted)
      return;
  ...
  void
  receive_ack(int from, epoch_t e)
  {
    if (rank_deleted)
      return;
  ...
  void
  receive_victory_claim(int from, epoch_t e, const set<int>& members)
  {
    if (rank_deleted)
      return;
    if (e < logic.get_epoch())
  ...
  void
  receive_victory_ack(int from, epoch_t e)
  {
    if (rank_deleted)
      return;
    if (e < logic.get_epoch())
  ...
  void
  receive_ping(int from_rank, bufferlist bl)
  {
  ...
  void
  receive_ping_timeout(int from_rank)
  {
  ...
  election_timeout()
  {
    ldout(g_ceph_context, 2)
        << "election epoch " << logic.get_epoch() << " timed out for " << rank
  ...
  {
    ldout(g_ceph_context, 2)
        << "victory epoch " << logic.get_epoch() << " timed out for " << rank
        << ", electing me:" << logic.electing_me
        << ", acked_me:" << logic.acked_me << dendl;
    ceph_assert(!rank_deleted);
    reset_election();
  ...
  send_pings()
  {
    ...
    for (int i = 0; i < parent->get_paxos_size(); ++i) {
      ...
      parent->queue_stable_or_timeout(
          rank, i, [o, r = rank, bl] { o->receive_ping(r, bl); },
  ...
  last_quorum_formed(-1)
{
  for (int i = 0; i < count; ++i) {
    electors[i] =
        std::make_shared<Owner>(i, election_strategy, tracker_halflife, this);
  }
}

void
Election::queue_stable_message(int from, int to, function<void()> m)
{
...
void
Election::queue_election_message(int from, int to, function<void(bool)> m)
{
...
void
Election::queue_timeout_message(int from, int to, function<void()> m)
{
...
void
Election::queue_stable_or_timeout(
    int from,
    int to,
    function<void()> m,
    function<void()> t)
...
void
Election::defer_to(int from, int to, epoch_t e)
{
...
void
Election::propose_to(int from, int to, epoch_t e, bufferlist& cbl)
{
  ...
  queue_election_message(
      from, to,
      [o = electors.at(to), from, e,
       oct = std::move(oct)](bool blocked) mutable {
        if (!blocked) {
...
void
Election::claim_victory(int from, int to, epoch_t e, const set<int>& members)
{
  queue_election_message(
      from, to, [o = electors.at(to), from, e, members](bool blocked) {
        if (!blocked) {
...
void
Election::accept_victory(int from, int to, epoch_t e)
{
  queue_election_message(from, to, [o = electors.at(to), from, e](bool blocked) {
    if (!blocked) {
...
void
Election::report_quorum(const set<int>& quorum)
{
  for (int i : quorum) {
...
int
Election::run_timesteps(int max)
{
...
bool
Election::election_stable() const
{
  // see if anybody has a timer running
  for (auto& i : electors) {
    if (i.second->timer_steps != -1) {
      ldout(g_ceph_context, 30) << "rank " << i.first << " has timer value "
                                << i.second->timer_steps << dendl;
      return false;
    }
  }
...
bool
Election::quorum_stable(int timesteps_stable) const
{
  ldout(g_ceph_context, 1) << "quorum_stable? last formed:"
                           << last_quorum_formed << ", last changed "
                           << last_quorum_change << ", last reported members "
                           << last_quorum_reported << dendl;
...
bool
Election::all_agree_on_leader() const
{
  ...
  for (auto& i : electors) {
    if (leader != i.second->logic.get_election_winner()) {
      ldout(g_ceph_context, 10)
          << "rank " << i.first << " has different leader "
...
bool
Election::check_epoch_agreement() const
{
...
void
Election::block_messages(int from, int to)
{
  blocked_messages[from].insert(to);
}

void
Election::block_bidirectional_messages(int a, int b)
{
  block_messages(a, b);
  block_messages(b, a);
}
...
void
Election::unblock_messages(int from, int to)
{
  blocked_messages[from].erase(to);
}

void
Election::unblock_bidirectional_messages(int a, int b)
{
...
void
Election::remove_elector(int rank)
{
...
void
single_startup_election_completes(ElectionLogic::election_strategy strategy)
{
...
void
everybody_starts_completes(ElectionLogic::election_strategy strategy)
{
  Election election(5, strategy);
  election.start_all();
  int steps = election.run_timesteps(0);
...
void
blocked_connection_continues_election(ElectionLogic::election_strategy strategy)
{
  Election election(5, strategy);
...
void
blocked_connection_converges_election(ElectionLogic::election_strategy strategy)
{
  Election election(5, strategy);
  election.block_bidirectional_messages(0, 1);
  election.start_all();
...
void
disallowed_doesnt_win(ElectionLogic::election_strategy strategy)
{
  int MON_COUNT = 5;
...
void
converges_after_flapping(ElectionLogic::election_strategy strategy)
{
  Election election(5, strategy);
  auto block_cons = [&] {
    auto& e = election;
...
void
converges_while_flapping(ElectionLogic::election_strategy strategy)
{
  Election election(5, strategy);
  auto block_cons = [&] {
    auto& e = election;
...
void
netsplit_with_disallowed_tiebreaker_converges(
    ElectionLogic::election_strategy strategy)
{
  Election election(5, strategy);
...
void
handles_singly_connected_peon(ElectionLogic::election_strategy strategy)
{
  Election election(5, strategy);
  election.block_bidirectional_messages(0, 1);
...
ConnectionReport*
get_connection_reports(ConnectionTracker& ct)
{
  return &ct.my_reports;
}

map<int, ConnectionReport>*
get_peer_reports(ConnectionTracker& ct)
{
  return &ct.peer_reports;
}

void
handles_outdated_scoring(ElectionLogic::election_strategy strategy)
{
  Election election(
      3, strategy,
...
void
handles_disagreeing_connectivity(ElectionLogic::election_strategy strategy)
{
  Election election(
      5, strategy,
      5); // ping every 5 timesteps so they start elections before settling scores!
...
void
handles_removing_ranks(ElectionLogic::election_strategy strategy)
{
  ...
  for (int deletee = 0; deletee < 5; ++deletee) {
    ...
    ldout(g_ceph_context, 1)
        << "removed rank " << deletee << " from set" << dendl;
    election.start_all();
...
#define test_classic(utest) \
  TEST(classic, utest) { utest(ElectionLogic::CLASSIC); }

#define test_disallowed(utest) \
  TEST(disallowed, utest) { utest(ElectionLogic::DISALLOW); }

#define test_connectivity(utest) \
  TEST(connectivity, utest) { utest(ElectionLogic::CONNECTIVITY); }
 ...
 quite following the rules anyway. */
    test_connectivity(everybody_starts_completes)
        test_connectivity(blocked_connection_converges_election)
            test_connectivity(disallowed_doesnt_win) test_connectivity(
                converges_after_flapping)
                test_connectivity(converges_while_flapping) test_connectivity(
                    netsplit_with_disallowed_tiebreaker_converges)
