...
class MessengerClient {
  ...
  public:
    ClientDispatcher(uint64_t delay, ClientThread* t) :
      Dispatcher(g_ceph_context), think_time(delay), thread(t)
    {}
...
public:
  MessengerClient(const string& t, const string& addr, int delay) :
    type(t), serveraddr(addr), think_time_us(delay), dummy_auth(g_ceph_context)
  {}
  ...
  void
  ready(int c, int jobs, int ops, int msg_len)
  {
    entity_addr_t addr;
    ...
    for (int i = 0; i < jobs; ++i) {
      Messenger* msgr = Messenger::create(
          g_ceph_context, type, entity_name_t::CLIENT(0), "client",
          getpid() + i);
      ...
      ClientThread* t =
          new ClientThread(msgr, c, conn, msg_len, ops, think_time_us);
...
void
MessengerClient::ClientDispatcher::ms_fast_dispatch(Message* m)
{
  usleep(think_time);
  m->put();
...
{
  cout << "Usage: "
       << name << " [server ip:port] [numjobs] [concurrency] [ios] [thinktime us] [msg length]"
       << std::endl;
  cout << "       [server ip:port]: connect to the ip:port pair" << std::endl;
  cout << "       [numjobs]: how much client threads spawned and do benchmark"
       << std::endl;
  cout << "       [concurrency]: the max inflight messages(like iodepth in fio)"
       << std::endl;
  cout << "       [ios]: how much messages sent for each client" << std::endl;
  cout << "       [thinktime]: sleep time when do fast dispatching(match "
          "client logic)"
       << std::endl;
...
main(int argc, char** argv)
{
  ...
  cout << " Total op " << (ios * numjobs) << " run time "
