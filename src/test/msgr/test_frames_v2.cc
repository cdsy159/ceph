...
struct TestFrame : Frame<
  ...
  static TestFrame
  Decode(segment_bls_t& segment_bls)
  {
    TestFrame f;
...
static std::ostream&
operator<<(std::ostream& os, const mode_t& m)
{
...
static const mode_t modes[] = {{false, false, false}, {false, true, false},
...
static std::ostream&
operator<<(std::ostream& os, const round_trip_instance_t& rti)
{
  os << rti.header_len << "+" << rti.front_len << "+" << rti.middle_len << "+"
     << rti.data_len;
  return os;
}

static bufferlist
make_bufferlist(size_t len, char c)
{
  bufferlist bl;
  if (len > 0) {
    bl.reserve(len);
...
bool
disassemble_frame(
    FrameAssembler& frame_asm,
    bufferlist& frame_bl,
    Tag& tag,
    segment_bls_t& segment_bls)
{
  ...
  return frame_asm.disassemble_segments(
      preamble_bl, segment_bls.data(), epilogue_bl);
}

class RoundTripTestBase
  : public ::testing::TestWithParam<std::tuple<round_trip_instance_t, mode_t>> {
protected:
  RoundTripTestBase() :
    m_tx_frame_asm(
        &m_tx_crypto,
        std::get<1>(GetParam()).is_rev1,
        true,
        &m_tx_comp),
    m_rx_frame_asm(
        &m_rx_crypto,
    ...
    m_data(make_bufferlist(std::get<0>(GetParam()).data_len, 'D'))
  {
    ...
    if (m.is_secure) {
      ...
      g_ceph_context->random()->get_bytes(
          auth_meta.connection_secret.data(),
  ...
  void
  check_frame_assembler(const FrameAssembler& frame_asm)
  {
    ...
    EXPECT_COMPRESSED(
        m.is_compress, onwire_lens[0], frame_asm.get_preamble_onwire_len());
    for (size_t i = 0; i < rti.num_segments; i++) {
      EXPECT_COMPRESSED(
          m.is_compress, onwire_lens[i + 1],
          frame_asm.get_segment_onwire_len(i));
    }
    EXPECT_COMPRESSED(
        m.is_compress, onwire_lens[rti.num_segments + 1],
        frame_asm.get_epilogue_onwire_len());
    EXPECT_COMPRESSED(
        m.is_compress,
        std::accumulate(
            std::begin(onwire_lens), std::end(onwire_lens), uint64_t(0)),
  ...
  test_round_trip()
  {
    ...
    EXPECT_TRUE(
        disassemble_frame(m_rx_frame_asm, onwire_bl, rx_tag, rx_segment_bls));
...
TEST_P(RoundTripTest, Basic) { test_round_trip(); }
