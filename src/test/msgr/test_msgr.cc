...
public:
  ...
  MessengerTest() :
    dummy_auth(g_ceph_context), server_msgr(NULL), client_msgr(NULL)
  {
  ...
  SetUp() override
  {
    lderr(g_ceph_context) << __func__ << " start set up " << GetParam()
                          << dendl;
    server_msgr = Messenger::create(
        g_ceph_context, string(GetParam()), entity_name_t::OSD(0), "server",
        getpid());
    client_msgr = Messenger::create(
        g_ceph_context, string(GetParam()), entity_name_t::CLIENT(-1), "client",
        getpid());
...
public:
  struct Session : public RefCountedObject {
    ...
    explicit Session(ConnectionRef c) :
      RefCountedObject(g_ceph_context), count(0), con(c)
    {}
  ...
  ms_dispatch(Message* m) override
  {
    ...
    s->count++;
    lderr(g_ceph_context) << __func__ << " conn: " << m->get_connection()
                          << " session " << s << " count: " << s->count
                          << dendl;
    if (is_server) {
      reply_message(m);
    }
...
struct TestInterceptor : public Interceptor {
  ...
  uint32_t
  count_step(Connection* conn, uint32_t step)
  {
    uint32_t count = 0;
  ...
  void
  breakpoint(uint32_t step)
  {
    breakpoints.insert(step);
  }

  void
  remove_bp(uint32_t step)
  {
    breakpoints.erase(step);
  }

  Connection*
  wait(uint32_t step, Connection* conn = nullptr)
  {
  ...
  ACTION
  wait_for_decision(uint32_t step, std::unique_lock<std::mutex>& l)
  {
    if (decisions[step]) {
  ...
  void
  proceed(uint32_t step, ACTION decision)
  {
    std::unique_lock<std::mutex> l(lock);
  ...
  ACTION
  intercept(Connection* conn, uint32_t step) override
  {
...
TEST_P(MessengerTest, ConnectionRaceTest)
{
  ...
  server_msgr->set_policy(
      entity_name_t::TYPE_CLIENT, Messenger::Policy::lossless_peer_reuse(0));
  server_msgr->interceptor = srv_interceptor;

  client_msgr->set_policy(
      entity_name_t::TYPE_OSD, Messenger::Policy::lossless_peer_reuse(0));
  ...
  ConnectionRef c2s = client_msgr->connect_to(
      server_msgr->get_mytype(), server_msgr->get_myaddrs());
  MPing* m1 = new MPing();
  ASSERT_EQ(c2s->send_message(m1), 0);

  ConnectionRef s2c = server_msgr->connect_to(
  ...
  cli_interceptor->proceed(
      Interceptor::STEP::SEND_CLIENT_IDENTITY, Interceptor::ACTION::CONTINUE);
  srv_interceptor->proceed(
      Interceptor::STEP::SEND_CLIENT_IDENTITY, Interceptor::ACTION::CONTINUE);
...
TEST_P(MessengerTest, ConnectionRaceReuseBannerTest)
{
  ...
  cli_interceptor->breakpoint(
      Interceptor::STEP::BANNER_EXCHANGE_BANNER_CONNECTING);
  ...
  Connection* s2c_accepter =
      srv_interceptor->wait(Interceptor::STEP::BANNER_EXCHANGE);
  srv_interceptor->remove_bp(Interceptor::STEP::BANNER_EXCHANGE);

  srv_interceptor->proceed(
      Interceptor::STEP::BANNER_EXCHANGE, Interceptor::ACTION::CONTINUE);
  cli_interceptor->proceed(
      Interceptor::STEP::READY, Interceptor::ACTION::CONTINUE);
  ...
  EXPECT_EQ(
      cli_interceptor->count_step(
          c2s.get(), Interceptor::STEP::START_CLIENT_BANNER_EXCHANGE),
      1u);
  EXPECT_EQ(
      cli_interceptor->count_step(c2s.get(), Interceptor::STEP::SEND_RECONNECT),
      0u);
  EXPECT_EQ(
      cli_interceptor->count_step(c2s.get(), Interceptor::STEP::READY), 1u);
...
TEST_P(MessengerTest, MissingServerIdenTest)
{
  ...
  client_msgr->set_policy(
      entity_name_t::TYPE_OSD, Messenger::Policy::lossy_client(0));
  client_msgr->interceptor = cli_interceptor;
  ...
  srv_interceptor->proceed(
      Interceptor::STEP::SEND_SERVER_IDENTITY, Interceptor::ACTION::FAIL);
  ...
  ASSERT_EQ(
      1u, static_cast<Session*>(c2s_accepter->get_priv().get())->get_count());
...
TEST_P(MessengerTest, MissingServerIdenTest2)
{
  ...
  server_msgr->set_policy(
      entity_name_t::TYPE_CLIENT, Messenger::Policy::lossless_peer(0));
  server_msgr->interceptor = srv_interceptor;

  client_msgr->set_policy(
      entity_name_t::TYPE_OSD, Messenger::Policy::lossless_peer(0));
  client_msgr->interceptor = cli_interceptor;
  ...
  ASSERT_TRUE(c2s_accepter->is_connected());
  ASSERT_EQ(
      0u, static_cast<Session*>(c2s_accepter->get_priv().get())->get_count());
  ASSERT_TRUE(c2s_accepter->peer_is_client());
...
TEST_P(MessengerTest, ReconnectTest)
{
  ...
  // failing client
  cli_interceptor->proceed(
      Interceptor::STEP::HANDLE_MESSAGE, Interceptor::ACTION::FAIL);
  ...
  ASSERT_EQ(
      srv_interceptor->count_step(
          c2s_accepter, Interceptor::STEP::SEND_RECONNECT),
      0u);
  // c2s_accepter sent 1 reconnect_ok messages
  ASSERT_EQ(
      srv_interceptor->count_step(
          c2s_accepter, Interceptor::STEP::SEND_RECONNECT_OK),
      1u);
  ...
  ASSERT_EQ(
      cli_interceptor->count_step(
          c2s.get(), Interceptor::STEP::SEND_RECONNECT_OK),
      0u);
...
TEST_P(MessengerTest, ReconnectRaceTest)
{
  ...
  cli_interceptor->proceed(
      Interceptor::STEP::SEND_RECONNECT, Interceptor::ACTION::CONTINUE);
  srv_interceptor->proceed(
      Interceptor::STEP::SEND_RECONNECT, Interceptor::ACTION::CONTINUE);
  ...
  if (srv_interceptor->count_step(
          c2s_accepter, Interceptor::STEP::SEND_RECONNECT) == 2) {
    // if the server send the reconnect message two times then
    // the client must have sent a session retry message to the server
    ASSERT_EQ(
        cli_interceptor->count_step(c2s.get(), Interceptor::STEP::SESSION_RETRY),
        1u);
  } else {
    ASSERT_EQ(
        cli_interceptor->count_step(c2s.get(), Interceptor::STEP::SESSION_RETRY),
...
TEST_P(MessengerTest, DumpBasics)
{
  ...
  if (std::strstr(GetParam(), "async") == nullptr) {
    GTEST_SKIP()
        << "skipping as only async messengers have a messenger dump hook";
  }
...
TEST_P(MessengerTest, StatefulTest)
{
  ...
  // don't lose state
  ASSERT_EQ(
      1U, static_cast<Session*>(server_conn->get_priv().get())->get_count());
  ...
  ASSERT_EQ(1U, static_cast<Session*>(conn->get_priv().get())->get_count());
  server_conn = server_msgr->connect_to(
      client_msgr->get_mytype(), srv_dispatcher.last_accept);
  {
    std::unique_lock l{srv_dispatcher.lock};
...
TEST_P(MessengerTest, AnonTest)
{
  ...
  ConnectionRef con_a = client_msgr->connect_to(
      server_msgr->get_mytype(), server_msgr->get_myaddrs(), true);
  {
    m = new MPing();
  ...
  ConnectionRef con_b = client_msgr->connect_to(
      server_msgr->get_mytype(), server_msgr->get_myaddrs(), true);
  {
    m = new MPing();
...
TEST_P(MessengerTest, MessageTest)
{
      ...
      server_msgr->get_mytype(), server_msgr->get_myaddrs());
  {
    ...
    conn->send_message(m);
    std::unique_lock l{cli_dispatcher.lock};
    cli_dispatcher.cond.wait_for(l, 500s, [&] {
...
ostream&
operator<<(ostream& out, const Payload& pl)
{
...
public:
  ...
  ms_fast_dispatch(Message* m) override
  {
    ...
    if (pl.who == Payload::PING) {
      lderr(g_ceph_context)
          << __func__ << " conn=" << m->get_connection() << pl << dendl;
      reply_message(m, pl);
      m->put();
      std::lock_guard l{lock};
      got_new = true;
  ...
  void
  reply_message(const Message* m, Payload& pl)
  {
    pl.who = Payload::PONG;
    bufferlist bl;
    encode(pl, bl);
    ...
    m->get_connection()->send_message(rm);
    lderr(g_ceph_context) << __func__ << " conn=" << m->get_connection()
                          << " reply m=" << m << " i=" << pl.seq << dendl;
  }

  void
  send_message_wrap(ConnectionRef con, const bufferlist& data)
  {
    Message* m = new MPing();
    ...
    lderr(g_ceph_context) << __func__ << " conn=" << con.get()
                          << " send m=" << m << " i=" << pl.seq << dendl;
  ...
  void
  clear_pending(ConnectionRef con)
  {
    std::lock_guard l{lock};
  ...
  print()
  {
    for (auto&& p : conn_sent) {
      if (!p.second.empty()) {
        lderr(g_ceph_context) << __func__ << " " << p.first << " wait "
                              << p.second.size() << dendl;
      }
    }
  }
...
public:
  ...
  SyntheticWorkload(
      ...
      int _max_in_flight = 64,
      int _max_connections = 128) :
    ...
    max_connections(_max_connections)
  {
    ...
    for (int i = 0; i < servers; ++i) {
      msgr = Messenger::create(
          g_ceph_context, type, entity_name_t::OSD(0), "server", getpid() + i);
    ...
    for (int i = 0; i < clients; ++i) {
      msgr = Messenger::create(
          g_ceph_context, type, entity_name_t::CLIENT(-1), "client",
          getpid() + i + servers);
  ...
  _get_random_connection()
  {
    ...
    map<ConnectionRef, pair<Messenger*, Messenger*>>::iterator i =
  ...
  generate_connection()
  {
    ...
    pair<Messenger*, Messenger*> p;
    {
      ...
      if (server->get_default_policy().server) {
        ...
        ConnectionRef conn =
            client->connect_to(server->get_mytype(), server->get_myaddrs());
  ...
  void
  send_large_message(bool inject_network_congestion = false)
  {
    std::lock_guard l{lock};
    ConnectionRef conn = _get_random_connection();
    uuid_d uuid;
  ...
  drop_connection()
  {
    ...
    if (!client_policy.server && !client_policy.lossy && client_policy.standby) {
      ...
      if (!p.first->get_default_policy().server &&
        ...
        ConnectionRef peer =
            p.second->connect_to(p.first->get_mytype(), p.first->get_myaddrs());
        peer->mark_down();
  ...
  void
  print_internal_state(bool detail = false)
  {
    std::lock_guard l{lock};
    lderr(g_ceph_context) << "available_connections: "
                          << available_connections.size()
                          << " inflight messages: " << dispatcher.get_pending()
                          << dendl;
...
bool
SyntheticDispatcher::ms_handle_reset(Connection* con)
{
...
TEST_P(MessengerTest, SyntheticStressTest)
{
  SyntheticWorkload test_msg(
      8, 32, GetParam(), 100, Messenger::Policy::stateful_server(0),
...
{
  SyntheticWorkload test_msg(
      16, 32, GetParam(), 100, Messenger::Policy::lossless_peer_reuse(0),
...
TEST_P(MessengerTest, SyntheticInjectTest)
{
  uint64_t dispatch_throttle_bytes =
      g_ceph_context->_conf->ms_dispatch_throttle_bytes;
...
TEST_P(MessengerTest, SyntheticInjectTest2)
{
  ...
  g_ceph_context->_conf.set_val("ms_inject_internal_delays", "0.1");
  SyntheticWorkload test_msg(
      8, 16, GetParam(), 100, Messenger::Policy::lossless_peer_reuse(0),
      Messenger::Policy::lossless_peer_reuse(0));
  for (int i = 0; i < 100; ++i) {
    if (!(i % 10))
      lderr(g_ceph_context) << "seeding connection " << i << dendl;
...
TEST_P(MessengerTest, SyntheticInjectTest3)
{
  ...
  g_ceph_context->_conf.set_val("ms_inject_internal_delays", "0.1");
  SyntheticWorkload test_msg(
      8, 16, GetParam(), 100, Messenger::Policy::stateless_server(0),
...
TEST_P(MessengerTest, SyntheticInjectTest4)
{
  ...
  SyntheticWorkload test_msg(
      16, 32, GetParam(), 100, Messenger::Policy::lossless_peer(0),
...
{
  SyntheticWorkload test_msg(
      1, 8, GetParam(), 100, Messenger::Policy::stateful_server(0),
...
public:
  ...
  explicit MarkdownDispatcher(bool s) :
    Dispatcher(g_ceph_context), last_mark(false)
  {}
  ...
  ms_dispatch(Message* m) override
  {
    ...
    last_mark = true;
    usleep(rand() % 500);
    for (set<ConnectionRef>::iterator it = conns.begin(); it != conns.end();
         ++it) {
...
{
  Messenger* server_msgr2 = Messenger::create(
      g_ceph_context, string(GetParam()), entity_name_t::OSD(0), "server",
      getpid());
  ...
  while (i--) {
    ConnectionRef conn1 = client_msgr->connect_to(
        server_msgr->get_mytype(), server_msgr->get_myaddrs());
