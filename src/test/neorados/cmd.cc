...
CORO_TEST_F(NeoRadosCmd, MonDescribe, NeoRadosTest)
{
  ...
  co_await rados().mon_command(
      std::move(arg), {}, &outs, &outbl, asio::use_awaitable);
...
{
  {
    ...
    co_await expect_error_code(
        rados().osd_command(0, std::move(arg), {}, asio::use_awaitable),
...

  {
    ...
    auto [ec, outs, outbl] = co_await rados().osd_command(
        0, std::move(arg), {}, asio::as_tuple(asio::use_awaitable));

    EXPECT_TRUE(
        (!ec && outbl.length() > 0) ||
        (ec == sys::errc::no_such_device_or_address && outbl.length() == 0));
  }
  co_return;
}
...
CORO_TEST_F(NeoRadosCmd, PGCmd, NeoRadosTest)
{
...

  {
    ...
    co_await expect_error_code(
        rados().pg_command(pgid, std::move(arg), {}, asio::use_awaitable),
        sys::errc::invalid_argument, sys::errc::no_such_device_or_address);
  }
  ...
  for (auto i = 0; i < 100; ++i) {
    co_await expect_error_code(
        rados().execute(
            fmt::format("obj{}", i), pool(), ReadOp{}.assert_exists(), nullptr,
            asio::use_awaitable),
        sys::errc::no_such_file_or_directory);
  }

  {
    std::vector arg({fmt::format(
        R"({{"prefix":"pg", "cmd":"query", "pgid":"{}.{}"}})", pgid.pool,
        pgid.seed)});
    // Working around a bug in GCC.
    auto coro = rados().pg_command(
        pgid, std::move(arg), {}, asio::as_tuple(asio::use_awaitable));
    auto [ec, outs, outbl] = co_await std::move(coro);

    EXPECT_TRUE(
        !ec || ec == sys::errc::no_such_file_or_directory ||
