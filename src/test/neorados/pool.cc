...
private:
  ...
  const std::string prefix_{fmt::format(
      "Test framework: {}: ",
...
protected:
  ...
  auto
  delete_pool(std::string pname) -> boost::asio::awaitable<void>
  {
...
public:
  ...
  CoTearDown() override
  {
    ...
    for (const auto& name : pools_to_delete)
      try {
...
CORO_TEST_F(NeoRadosPools, PoolList, NeoRadosPool)
{
  ...
  EXPECT_TRUE(std::find_if(pools.begin(), pools.end(), [&pname](const auto& kv) {
                return kv.second == pname;
              }) != pools.end());
  co_return;
}
...
CORO_TEST_F(NeoRadosPools, PoolLookupOtherInstance, NeoRadosPool)
{
  auto rados2 = co_await neorados::RADOS::Builder{}.build(
      asio_context, asio::use_awaitable);
  const auto pname = get_temp_pool_name();
  const auto refpid = co_await create_pool(pname);
...
CORO_TEST_F(NeoRadosPools, PoolDelete, NeoRadosPool)
{
  ...
  co_await expect_error_code(
      lookup_pool(pname), sys::errc::no_such_file_or_directory);
  co_return;
}
