...
protected:
  ...
  auto
  write_object(std::string_view data, uint64_t* objver = nullptr)
  {
  ...
  auto
  assert_version(uint64_t v)
  {
    return execute(oid, ReadOp{}.assert_version(v));
  }

  auto
  setxattr(std::string_view xattr, buffer::list bl)
  {
    return execute(oid, WriteOp{}.setxattr(xattr, std::move(bl)));
  }

  auto
  cmpxattr(std::string_view xattr, cmp_op op, buffer::list bl)
  {
...
CORO_TEST_F(NeoRadosReadOps, SetOpFlags, ReadOpTest)
{
  sys::error_code ec;
  co_await execute(
      oid, ReadOp{}.exec("rbd"sv, "get_id"sv, {}, nullptr, &ec).set_failok());
...
CORO_TEST_F(NeoRadosReadOps, AssertExists, ReadOpTest)
{
  co_await expect_error_code(
      execute("nonexistent"sv, ReadOp{}.assert_exists()),
...
CORO_TEST_F(NeoRadosReadOps, AssertVersion, ReadOpTest)
{
  ...
  co_await expect_error_code(assert_version(v + 1), sys::errc::value_too_large);
...
CORO_TEST_F(NeoRadosReadOps, CmpXattr, ReadOpTest)
{
  ...
  co_await expect_error_code(
      cmpxattr(xattr, ne, filled_buffer_list(0xcc, datalen)),
      operation_canceled);
  co_await expect_error_code(
      cmpxattr(xattr, gt, filled_buffer_list(0xcc, datalen)),
      operation_canceled);
  co_await cmpxattr(xattr, gte, filled_buffer_list(0xcc, datalen));
  co_await expect_error_code(
      cmpxattr(xattr, lt, filled_buffer_list(0xcc, datalen)),
      operation_canceled);
  co_await cmpxattr(xattr, lte, filled_buffer_list(0xcc, datalen));

  // < value
  co_await expect_error_code(
      cmpxattr(xattr, eq, filled_buffer_list(0xcb, datalen)),
      operation_canceled);
  co_await cmpxattr(xattr, ne, filled_buffer_list(0xcb, datalen));
  co_await expect_error_code(
      cmpxattr(xattr, gt, filled_buffer_list(0xcb, datalen)),
      operation_canceled);
  co_await expect_error_code(
      cmpxattr(xattr, gte, filled_buffer_list(0xcb, datalen)),
      operation_canceled);
  ...
  // > value
  co_await expect_error_code(
      cmpxattr(xattr, eq, filled_buffer_list(0xcd, datalen)),
      operation_canceled);
  ...
  co_await expect_error_code(
      cmpxattr(xattr, lt, filled_buffer_list(0xcd, datalen)),
      operation_canceled);
  co_await expect_error_code(
      cmpxattr(xattr, lte, filled_buffer_list(0xcd, datalen)),
      operation_canceled);
  ...
  co_await expect_error_code(
      cmpxattr(xattr, eq, to_buffer_list("\x00\xcc"sv)), operation_canceled);
  ...
  co_await expect_error_code(
      cmpxattr(xattr, lt, to_buffer_list("\x00\xcc"sv)), operation_canceled);
  co_await expect_error_code(
      cmpxattr(xattr, lte, to_buffer_list("\x00\xcc"sv)), operation_canceled);

  co_await cmpxattr(xattr, eq, to_buffer_list("\x00\x00"sv));
  co_await expect_error_code(
      cmpxattr(xattr, ne, to_buffer_list("\x00\x00"sv)), operation_canceled);
  co_await expect_error_code(
      cmpxattr(xattr, gt, to_buffer_list("\x00\x00"sv)), operation_canceled);
  co_await cmpxattr(xattr, gte, to_buffer_list("\x00\x00"sv));
  co_await expect_error_code(
      cmpxattr(xattr, lt, to_buffer_list("\x00\x00"sv)), operation_canceled);
...
CORO_TEST_F(NeoRadosReadOps, Read, ReadOpTest)
{
...

  {
    buffer::list bl;
    sys::error_code ec;
    co_await execute(oid, ReadOp{}.read(0, 0, &bl, &ec).set_fadvise_dontneed());
    EXPECT_TRUE(
        (data.length() == bl.length()) &&
        (0 == std::memcmp(data.data(), bl.c_str(), data.length())));
    EXPECT_FALSE(ec);
...
inline std::uint32_t
crc32c(uint32_t seed, std::string_view v)
{
  return ceph_crc32c(
      seed, reinterpret_cast<const uint8_t*>(v.data()), uint32_t(v.size()));
}
...
{
  {
    ...
    co_await execute(
        oid, ReadOp{}.checksum(hash_alg::xxhash64, seed, 0, 0, 0, &hash));
  ...
  }
  {
    ...
    co_await execute(
        oid, ReadOp{}.checksum(hash_alg::crc32c, seed, 0, 0, 0, &crc));
  ...
  }
  {
    ...
    co_await execute(
        oid, ReadOp{}.checksum(hash_alg::xxhash32, seed, 0, 0, 0, &hash));
...

  {
    ...
    co_await execute(
        oid,
        ReadOp{}.checksum(hash_alg::crc32c, seed, 0, data.length(), 4, &crc));
...
CORO_TEST_F(NeoRadosReadOps, Exec, ReadOpTest)
{
  ...
  sys::error_code ec;
  co_await execute(
      oid, ReadOp{}.exec("rbd"sv, "get_all_features"sv, {}, &bl, &ec));
  EXPECT_FALSE(ec);
...
CORO_TEST_F(NeoRadosReadOps, Stat, ReadOpTest)
{
  ...
  sys::error_code ec;
  co_await expect_error_code(
      execute("nonexistent"sv, ReadOp{}.stat(&size, nullptr, &ec)),
      sys::errc::no_such_file_or_directory);
  EXPECT_EQ(sys::errc::io_error, ec);
  EXPECT_EQ(1u, size);
  ...
  co_await expect_error_code(
      execute("nonexistent"sv, ReadOp{}.stat(nullptr, nullptr)),
      sys::errc::no_such_file_or_directory);
...
CORO_TEST_F(NeoRadosReadOps, Omap, ReadOpTest)
{
  ...
  co_await expect_error_code(
      execute(
          "nonexistent"sv, ReadOp{}.get_omap_vals({}, {}, 10, nullptr, nullptr)),
      sys::errc::no_such_file_or_directory);

  {
    ...
    sys::error_code ec;
    co_await execute(
        oid, ReadOp{}.get_omap_vals({}, {}, 10, &omap2, &truncated, &ec));
    EXPECT_FALSE(ec);
  ...
  // Check for readability
  {
    ...
    EXPECT_TRUE(std::all_of(keys.begin(), keys.end(), [&](const auto& s) {
      return omap.contains(s);
    }));
    EXPECT_TRUE(std::all_of(omap.begin(), omap.end(), [&](const auto& kv) {
  ...
  // try to remove keys with a guard that should fail
  {
    ...
    co_await expect_error_code(
        execute(oid, std::move(op)), sys::errc::operation_canceled);
  }
  // Verify the keys are still there, and then remove them
  {
    ...
    op.cmp_omap(
        {{(omap.begin() + 1)->first, cmp_op::eq, {(omap.begin() + 1)->second}}});
    op.rm_omap_keys({omap.begin()->first, (omap.begin() + 1)->first});
    co_await execute(oid, std::move(op));
...
CORO_TEST_F(NeoRadosReadOps, CmpExt, ReadOpTest)
{
  ...
  }
  {
    ...
    op.read(0, 0, &bl);
    co_await expect_error_code(
        execute(oid, std::move(op)), osd_errc::cmpext_mismatch);
    EXPECT_EQ(0, unmatch);
  ...
  // Make sure other error codes work properly
  {
    ...
    op.read(0, 0, &bl);
    co_await expect_error_code(
        execute("nonexistent"sv, std::move(op)),
        sys::errc::no_such_file_or_directory);
    EXPECT_EQ(-1, unmatch);
    EXPECT_EQ(0, bl.length());
  }
