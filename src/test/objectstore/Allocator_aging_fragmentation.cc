...
void
PrintTo(const Scenario& s, ::std::ostream* os)
{
    ...
    uint64_t min_alloc_size)
{
  ...
  rng.seed(0);
  alloc.reset(
      Allocator::create(cct.get(), allocator_name, size, min_alloc_size));
  at.reset(new AllocTracker());
}

void
AllocTest::init_close()
{
  alloc.reset(0);
  at.reset(nullptr);
}
...
void
AllocTest::do_free(uint64_t low_mark)
{
    ...
    double leak_factor)
{
  ...
  if (verbose)
    std::cout << "    fragmented allocs=" << 100.0 * fragmented / allocs << "%"
              << " #frags="
              << (fragmented != 0 ? double(fragments) / fragmented : 0)
  ...
  if (verbose)
    std::cout << "free chunks sum=" << sum << " free chunks count=" << cnt
              << std::endl;
...
void
AllocTest::TearDownTestSuite()
{
  cct.reset();

  std::cout << "Summary: " << std::endl;
  for (auto& r : results_per_allocator) {
    std::cout << r.first << "    fragmented allocs="
              << r.second.fragmented_percent / r.second.tests_cnt << "%"
...
TEST_P(AllocTest, test_alloc_triangle_0_8M_16M)
{
  ...
  boost::triangle_distribution<double> D(
  ...
  for (auto& s : scenarios) {
    ...
    doAgingTest(
        size_generator, allocator_name, s.capacity * _1G, s.alloc_unit,
        s.high_mark * s.capacity * _1G, s.low_mark * s.capacity * _1G,
