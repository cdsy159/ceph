...
private:
  ...
  struct Object : public RefCountedObjectSafe {
    ...
    void punch_hole(uint64_t offset, uint64_t length, PExtentVector& old_extents);
    void verify_extents();
  ...
  struct Collection : public CollectionImpl {
    ...
    ceph::shared_mutex lock = ceph::make_shared_mutex(
        "ObjectStoreImitator::Collection::lock",
        true,
        false);
    ...
    ObjectRef
    get_obj(const ghobject_t& oid, bool create)
    {
      ceph_assert(
          create ? ceph_mutex_is_wlocked(lock) : ceph_mutex_is_locked(lock));
  ...
  int
  _split_collection(CollectionRef& c, CollectionRef& d, unsigned bits, int rem);
...
public:
  ObjectStoreImitator(
    ...
    ObjectStore(cct, path_), alloc(nullptr), min_alloc_size(min_alloc_size_)
  {}
  ...
  int
  statfs(struct store_statfs_t* buf, osd_alert_list_t* alerts = nullptr) override
  {
    return 0;
  }

  int
  pool_statfs(uint64_t pool_id, struct store_statfs_t* buf, bool* per_pool_omap)
      override
  {
    return 0;
  }
