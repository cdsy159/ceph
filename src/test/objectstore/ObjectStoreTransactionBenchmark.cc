...
public:
  ...
  void
  setattr(coll_t cid, const ghobject_t& oid, const string& name, bufferlist& val)
  {
    uint64_t start_time = Cycles::rdtsc();
  ...
  void
  omap_rmkey(coll_t cid, const ghobject_t& oid, const string& key)
  {
    uint64_t start_time = Cycles::rdtsc();
  ...
  void
  apply_encode_decode(bool new_format)
  {
    bufferlist p_bl;
    bufferlist d_bl;
  ...
  dump_stat()
  {
    cerr << " write op: " << Cycles::to_microseconds(write_ticks.ticks)
         << "us count: " << write_ticks.count << std::endl;
    cerr << " setattr op: " << Cycles::to_microseconds(setattr_ticks.ticks)
         << "us count: " << setattr_ticks.count << std::endl;
    cerr << " omap_setkeys op: "
         << Cycles::to_microseconds(Transaction::omap_setkeys_ticks.ticks)
         << "us count: " << Transaction::omap_setkeys_ticks.count << std::endl;
    cerr << " omap_rmkey op: "
         << Cycles::to_microseconds(Transaction::omap_rmkey_ticks.ticks)
         << "us count: " << Transaction::omap_rmkey_ticks.count << std::endl;
    cerr << " encode op: "
         << Cycles::to_microseconds(Transaction::encode_ticks.ticks)
         << "us count: " << Transaction::encode_ticks.count << std::endl;
    cerr << " decode op: "
         << Cycles::to_microseconds(Transaction::decode_ticks.ticks)
         << "us count: " << Transaction::decode_ticks.count << std::endl;
    cerr << " iterate op: "
         << Cycles::to_microseconds(Transaction::iterate_ticks.ticks)
...
class PerfCase {
  ...
  bufferlist
  generate_random(uint64_t len, int frag)
  {
...
public:
  ...
  uint64_t
  rados_write_4k(int times, bool new_format)
  {
    uint64_t ticks = 0;
    uint64_t len = Kib * 4;
...
const ghobject_t PerfCase::pglog_oid(
    hobject_t(sobject_t(object_t("cid_pglog"), 0)));
...
main(int argc, char** argv)
{
  ...
  cerr << " Old format total rados op " << times << " run time "
       << Cycles::to_microseconds(ticks1) << "us." << std::endl;

  Transaction::reset_stat();

  uint64_t ticks2 = c.rados_write_4k(times, true);
  Transaction::dump_stat();
  cerr << " New format total rados op " << times << " run time "
       << Cycles::to_microseconds(ticks2) << "us." << std::endl;
