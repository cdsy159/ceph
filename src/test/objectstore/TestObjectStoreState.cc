...
void
TestObjectStoreState::init(int colls, int objs)
{
  ...
  for (int i = 0; i < colls; i++) {
    ...
    dout(5) << "init create collection " << entry->m_cid << " meta "
            << entry->m_meta_obj << dendl;
    ...
    t->collection_hint(
        entry->m_cid, ObjectStore::Transaction::COLL_HINT_EXPECTED_NUM_OBJECTS,
        hint);
  ...
  dout(5) << "init has " << m_in_flight.load() << "in-flight transactions"
          << dendl;
  wait_for_done();
...
TestObjectStoreState::coll_create(spg_t pgid, ObjectStore::CollectionHandle ch)
{
  char meta_buf[100];
...
TestObjectStoreState::get_coll_at(int pos, bool erase)
{
  ...
  dout(5) << "get_coll_at pos " << pos << ": " << entry->m_cid
          << "(removed: " << erase << ")" << dendl;
...
bool
TestObjectStoreState::coll_entry_t::check_for_obj(int id)
{
...
TestObjectStoreState::coll_entry_t::touch_obj(int id)
{
  ...
  if (it != m_objects.end()) {
    dout(5) << "touch_obj coll id " << m_cid << " name " << it->second->oid.name
            << dendl;
    return it->second;
  }
...
hobject_t*
TestObjectStoreState::coll_entry_t::get_obj(int id, bool remove)
{
  map<int, hobject_t*>::iterator it = m_objects.find(id);
  if (it == m_objects.end()) {
    dout(5) << "get_obj coll " << m_cid << " obj #" << id << " non-existent"
            << dendl;
    return NULL;
  }
  ...
  dout(5) << "get_obj coll " << m_cid << " id " << id << ": " << obj->oid.name
          << "(removed: " << remove << ")" << dendl;

  return obj;
}

hobject_t*
TestObjectStoreState::coll_entry_t::get_obj_at(int pos, int* key)
{
 ...
 */
hobject_t*
TestObjectStoreState::coll_entry_t::remove_obj_at(int pos, int* key)
{
  return get_obj_at(pos, true, key);
}

hobject_t*
TestObjectStoreState::coll_entry_t::get_obj_at(int pos, bool remove, int* key)
{
  if (m_objects.empty()) {
  ...
  if (ret == NULL) {
    dout(5) << "get_obj_at coll " << m_cid << " pos " << pos << " non-existent"
            << dendl;
    return NULL;
  }
...
int
TestObjectStoreState::coll_entry_t::get_random_obj_id(rngen_t& gen)
{
