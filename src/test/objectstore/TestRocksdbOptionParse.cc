...
TEST(RocksDBOption, simple)
{
  ...
  auto db =
      std::make_unique<RocksDBStore>(g_ceph_context, dir, kvoptions, nullptr);
  string options_string =
      ...
      "{min_merge_width=4;size_ratio=2;max_size_amplification_percent=500}";
  ...
  ASSERT_EQ(
      500, options.compaction_options_universal.max_size_amplification_percent);
}
...
TEST(RocksDBOption, interpret)
{
  ...
  auto db =
      std::make_unique<RocksDBStore>(g_ceph_context, dir, kvoptions, nullptr);
  string options_string =
      "compact_on_mount = true; compaction_threads=10;flusher_threads=5;";
