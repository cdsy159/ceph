...
void
usage_replay_alloc(const string& name)
{
  cerr << "Detailed replay_alloc usage: "
       << name << " <allocator_dump_JSON> replay_alloc <alloc_list_file> [number of replays]"
       << std::endl;
  cerr << "The number of replays defaults to 1." << std::endl;
  cerr << "The \"alloc_list_file\" parameter should be a file with allocation "
          "requests, one per line."
       << std::endl;
  cerr << "Allocation request format (space separated, optional parameters are "
          "0 if not given): want unit [max] [hint]"
       << std::endl;
}
...
int
replay_and_check_for_duplicate(char* fname)
{
  ...
  while (true) {
    ...
    if (sp) {
      ...
      if (owned_by_app.intersects(offs, len)) {
        std::cerr << "error: unexpected takeover from allocator, already owned "
                     "by app: "
                  << s << std::endl;
        return -1;
      } else {
    ...
    if (sp) {
      ...
      for (auto& ee : tmp) {
        std::cerr << "dump extent: " << std::hex << ee.offset << "~"
                  << ee.length << std::dec << std::endl;
      }
    ...
    if (sp) {
      ...
      alloc.reset(
          Allocator::create(g_ceph_context, alloc_type, total, alloc_unit));
...
int
replay_free_dump_and_apply_raw(
    char* fname,
    std::function<void(std::string_view, int64_t, int64_t, std::string_view)>
        create,
...
*/
int
replay_free_dump_and_apply(
    char* fname,
    std::function<int(Allocator*, const string& aname)> fn)
{
  unique_ptr<Allocator> alloc;
  auto create_fn = [&](std::string_view alloc_type, int64_t capacity,
                       int64_t alloc_unit, std::string_view alloc_name) {
    alloc.reset(Allocator::create(
...
void
dump_alloc(Allocator* alloc, const string& aname)
{
...
int
export_as_binary(char* fname, char* target_fname)
{
  ...
  binary_alloc_map_t amap;
  auto dummy_create_fn = [&](std::string_view alloc_type, int64_t capacity,
                             int64_t alloc_unit,
...
check_duplicates(char* fname)
{
  ...
  while (itA != invalid_extentsA.end()) {
    ...
    while (itB != invalid_extentsB.end() &&
      ...
      std::cerr << " " << itB.get_start() << "~"
                << itB.get_end() - itB.get_start();
      ++itB;
    }
...
main(int argc, char** argv)
{
  ...
  if (strcmp(argv[2], "raw_duplicates") == 0) {
    ...
    return replay_free_dump_and_apply(
        argv[1], [&](Allocator* a, const string& aname) {
          ceph_assert(a);
    ...
    if (argc < 4) {
      std::cerr << "Error: insufficient arguments for \"assess_free_extents\" "
                   "operation."
                << std::endl;
      usage(argv[0]);
      return 1;
                    ...
                    << std::endl;
          {
            ...
            for (size_t i = 0; i < count; i++) {
              ...
              std::cout << "Duration (ns): "
                        << (ceph::mono_clock::now() - t0).count() << std::endl;
              if (r < 0) {
                std::cerr << "Error: allocation failure at step:" << i + 1
                          << ", ret = " << r << " elapsed (ns): "
                          << (ceph::mono_clock::now() - t00).count()
                          << std::endl;
                return -1;
            ...
            std::cout << "Successfully allocated: " << count << " * " << want
                      << ", unit:" << alloc_unit << " elapsed (ns): "
                      << (ceph::mono_clock::now() - t00).count() << std::endl;
          }
          return 0;
        });
           ...
           */
            ...
            for (auto i = 0; i < replay_count; ++i) {
              while (fgets(s, sizeof(s), f_alloc_list) != nullptr) {
                ...
                if (std::sscanf(
                        s, "%ji %ji %ji %ji", &want, &unit, &max, &hint) < 2) {
                ...
                if (r < 0) {
                  ...
                  std::cerr
                      << "Error: allocation failure code: " << r
                  ...
                  std::cerr << "Fragmentation:" << a->get_fragmentation()
                            << std::endl;
                  std::cerr
                      << "Fragmentation score:" << a->get_fragmentation_score()
                      << std::endl;
                  std::cerr << "Free: 0x" << std::hex << a->get_free()
                            << std::dec << std::endl;
                ...
                std::cout << "Duration (ns): "
                          << (ceph::mono_clock::now() - t0).count()
                          << " want/unit/max/hint (hex): " << std::hex << want
                          << "/" << unit << "/" << max << "/" << hint
    ...
    return replay_free_dump_and_apply(
        argv[1], [&](Allocator* a, const string& aname) {
          ...
          std::cout << "Free: 0x" << std::hex << a->get_free() << std::dec
                    << std::endl;
          std::cout << "Allocation unit:" << alloc_unit << std::endl;

          AllocatorBase::FreeStateHistogram hist(num_buckets);
          a->foreach ([&](size_t off, size_t len) {
            hist.record_extent(uint64_t(alloc_unit), off, len);
          });

          uint64_t s = 0;
          hist.foreach ([&](uint64_t max_len, uint64_t total, uint64_t aligned,
                            uint64_t units) {
            uint64_t e = max_len;
            std::cout << "(" << s << ".." << e << "]" << " -> " << total
                      << " chunks, " << aligned << " aligned with " << units
                      << " alloc_units." << std::endl;
            s = e;
          });
          return 0;
        });
