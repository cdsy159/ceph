...
auto
clamp_or_throw(auto min, auto max)
{
  return [=](auto& x) {
    if (std::less<>{}(x, min) or std::greater<>{}(x, max)) {
      throw std::out_of_range(fmt::format(
          "value expected between {} and {}, but got {}", min, max, x));
    }
...
struct StringPtrCompare {
  int
  operator()(const shared_ptr<string>& lhs, const shared_ptr<string>& rhs) const
  {
    if (lhs && rhs) {
...
public:
  ...
  {
    this->setg(
        const_cast<char*>(start), const_cast<char*>(start),
        const_cast<char*>(end));
  }
};
...
void
gen_buffer(bufferlist& bl, uint64_t size)
{
...
void
completion_cb(librados::completion_t cb, void* arg)
{
...
uint64_t
timestamp_parser(std::string& date)
{
  ...
  timestamp +=
      (month * 30 * 24 * 60 *
       60); // Yes, 30 day month is the best format ever and you cannot complain
...
void
parse_entry_point(shared_ptr<ParserContext> context)
{
  cout << fmt::format(
              "Starting parser thread start={:p} end={:p}", context->start,
              context->end)
       << endl;
  ...
  while (fstream >> date) {
    // cout << date << endl;
    if (!(date.size() > 4 && isdigit(date[0]) && isdigit(date[1]) &&
    ...
    context->ops.push_back(
        Op(at, ot, offset, length, object_ptr, collection_ptr, who_ptr));
  }
}
    ...
    librados::IoCtx* io)
{
  ...
  cout << fmt::format(
              "Starting thread {} with io_depth={} max_buffer_size={}", id,
              io_depth, max_buffer_size)
       << endl;
  for (auto& op : ops) {
    {
    ...
    op.completion = librados::Rados::aio_create_completion(
        static_cast<void*>(&op), completion_cb);
    switch (op.type) {
    case Write: {
      bufferlist trimmed;
      trimmed.substr_of(bl, 0, op.length);
      int ret = io->aio_write(
          *op.object, op.completion, trimmed, op.length, op.offset);
      if (ret != 0) {
    ...
    case Read: {
      bufferlist read;
      int ret = io->aio_read(
          *op.object, op.completion, &op.read_bl, op.length, op.offset);
      if (ret != 0) {
...
int
op_comparison_by_date(const Op& lhs, const Op& rhs)
{
...
main(int argc, char** argv)
{
  ...
  librados::IoCtx io;
  uint64_t max_buffer_size =
      0; // We can use a single buffer for writes and trim it at will. The buffer will be the size of the maximum length op.
  ...
  po_options.add_options()("help,h", "produce help message")(
      ...
      "List of input files (output of op_scraper.py). Multiple files will be "
      "merged and sorted by time order")(
      "ceph-conf",
      po::value<string>(&ceph_conf_path)->default_value("ceph.conf"),
      "Path to ceph conf")(
      "io-depth", po::value<uint64_t>(&io_depth)->default_value(64),
      "I/O depth")(
      "parser-threads",
      po::value<int>(&nparser_threads)
          ->default_value(16)
          ->notifier(settings::clamp_or_throw(1, 256)),
      "Number of parser threads")(
      "worker-threads",
      po::value<int>(&nworker_threads)
          ->default_value(16)
          ->notifier(settings::clamp_or_throw(1, 256)),
      "Number of I/O worker threads")(
      "pool", po::value<string>(&pool)->default_value("test_pool"),
      "Pool to use for I/O")(
      "skip-do-ops", po::bool_switch(&skip_do_ops)->default_value(false),
      "Skip doing operations");
  ...
  vector<shared_ptr<ParserContext>>
      complete_parser_contexts; // list of ALL parser contexts so that shared_ptrs do not die.
  for (auto& file : input_files) {
    ...
    fstat(fd, &file_stat);
    char* mapped_buffer =
        (char*)mmap(NULL, file_stat.st_size, PROT_READ, MAP_SHARED, fd, 0);
    ...
    complete_parser_contexts.insert(
        complete_parser_contexts.end(), parser_contexts.begin(),
        parser_contexts.end());
  }
  ...
  if (ret < 0) {
    std::cerr << "Couldn't read the Ceph configuration file! error " << ret
              << std::endl;
    return EXIT_FAILURE;
  }
  ...
  for (int i = 0; std::cmp_less(i, nworker_threads); i++) {
    worker_threads.push_back(thread(
        worker_thread_entry, i, nworker_threads, std::ref(ops), max_buffer_size,
        io_depth, &io));
  }
