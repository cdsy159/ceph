...
public:
  ...
  init()
  {
    ...
    db.reset(KeyValueDB::create(
        g_ceph_context, string(GetParam()), "kv_test_temp_dir"));
  }
  ...
  SetUp() override
  {
    ...
    if (r < 0 && errno != EEXIST) {
      r = -errno;
      cerr << __func__
           << ": unable to create kv_test_temp_dir: " << cpp_strerror(r)
           << std::endl;
      return;
    }
    init();
...
TEST_P(KVTest, RocksDB_parse_sharding_def)
{
  ...
  result = RocksDBStore::parse_sharding_def(
      text_def, sharding_def, &error_position, &error_msg);
...
public:
  ...
  */
  ...
  std::vector<std::string> sharding_defs = {
      "Betelgeuse D", "Betelgeuse(3) D", "Betelgeuse D(3)",
      "Betelgeuse(3) D(3)"};
  std::vector<std::string> prefixes = {"Ad", "Betelgeuse", "C", "D", "Evade"};
  std::vector<std::string> randoms = {
      "0",          "1",          "2",           "3",      "4",
      "5",          "found",      "brain",       "fully",  "pen",
      "worth",      "race",       "stand",       "nodded", "whenever",
      "surrounded", "industrial", "skin",        "this",   "direction",
      "family",     "beginning",  "whenever",    "held",   "metal",
      "year",       "like",       "valuable",    "softly", "whistle",
      "perfectly",  "broken",     "idea",        "also",   "coffee",
      "branch",     "tongue",     "immediately", "bent",   "partly",
      "burn",       "include",    "certain",     "burst",  "final",
      "smoke",      "positive",   "perfectly"};
  int R = randoms.size();
  ...
  generate_data(const test_id& x)
  {
    ...
    for (int i = 0; i < 5; i++) {
      ...
      case 0:
        break;
      case 1:
        data[RocksDBStore::combine_strings(prefixes[i], randoms[k++ % R])] =
            randoms[v++ % R];
        break;
      case 2:
        std::string base = randoms[k++ % R];
        for (int j = 0; j < 10; j++) {
          data[RocksDBStore::combine_strings(
...
TEST_P(RocksDBShardingTest, wholespace_upper_bound)
{
  ...
  do {
    ...
    while (dit != data.end()) {
      ...
      key_minus_1 = key.substr(0, key.length() - 1) +
                    std::string(1, key[key.length() - 1] - 1);
      KeyValueDB::WholeSpaceIterator it1 = db->get_wholespace_iterator();
      ...
      ASSERT_EQ(raw_key.second, key);
      if (verbose)
        std::cout << "upper_bound " << prefix << " " << key_minus_1
                  << std::endl;
      ASSERT_EQ(it->next(), 0);
      ++dit;
    }
...
public:
  ...
  SetUp() override
  {
    ...
    KeyValueDB* db_kv =
        KeyValueDB::create(g_ceph_context, "rocksdb", "kv_test_temp_dir");
