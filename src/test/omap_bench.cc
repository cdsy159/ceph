...
int
OmapBench::setup(int argc, const char** argv)
{
  ...
  for (unsigned i = 0; i < args.size(); i++) {
    if (i < args.size() - 1) {
      ...
      cout << ")\n"
           ...
           << entries_per_omap;
      cout << ")\n"
           << "	--keysize       number of characters per key " << "(default "
           << key_size;
      cout << ")\n"
           << "	--valsize       number of characters per value " << "(default "
           << value_size;
      cout << ")\n"
...
void
AioWriter::set_aioc(librados::callback_t complete)
{
...
//Helper methods
void
OmapBench::aio_is_complete(rados_completion_t c, void* arg)
{
...
OmapBench::print_written_omap()
{
  for (int i = 1; i <= objects; i++) {
    ...
    for (set<string>::iterator iter = out_keys.begin(); iter != out_keys.end();
         ++iter) {
...
OmapBench::print_results()
{
  ...
  for (int i = floor(data.min_latency / increment);
       i < ceil(data.max_latency / increment); i++) {
    cout << ">= " << i * increment;
    cout << "ms";
    int spaces;
    if (i == 0)
...
int
OmapBench::write_omap_asynchronously(
    AioWriter* aiow,
...
int
OmapBench::generate_small_non_random_omap(
    const int omap_entries,
    const int key_size,
    const int value_size,
    std::map<std::string, bufferlist>* out_omap)
...
//tests
int
OmapBench::test_write_objects_in_parallel(omap_generator_t omap_gen)
{
  ...
  for (int i = 0; i < objects; i++) {
    ...
    busythreads_count++;
    int err = omap_gen(
        entries_per_omap, key_size, value_size, &this_aio_writer->get_omap());
    if (err < 0) {
      return err;
    }
    err = OmapBench::write_omap_asynchronously(
        this_aio_writer, (this_aio_writer->get_omap()));
