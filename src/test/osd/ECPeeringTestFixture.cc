...
std::ostream&
ECPeeringTestFixture::ShardDpp::gen_prefix(std::ostream& out) const
{
  out << "shard " << shard << ": ";
...
ECPeeringTestFixture::get_is_recoverable_predicate()
{
  return new MockECRecPred(k, m);
}

IsPGReadablePredicate*
ECPeeringTestFixture::get_is_readable_predicate()
{
...
ECPeeringTestFixture::SetUp()
{
  ...
  auto peering_handler =
      [this](int from_osd, int to_osd, MOSDPeeringOp* op) -> bool {
  ...
  messenger->register_typed_handler<MOSDPeeringOp>(
      MSG_OSD_PG_QUERY2, peering_handler);
  messenger->register_typed_handler<MOSDPeeringOp>(
      MSG_OSD_PG_NOTIFY2, peering_handler);
  messenger->register_typed_handler<MOSDPeeringOp>(
      MSG_OSD_PG_INFO2, peering_handler);
  messenger->register_typed_handler<MOSDPeeringOp>(
      MSG_OSD_PG_LOG, peering_handler);
  messenger->register_typed_handler<MOSDPeeringOp>(
      MSG_OSD_PG_LEASE, peering_handler);
  messenger->register_typed_handler<MOSDPeeringOp>(
      MSG_OSD_PG_LEASE_ACK, peering_handler);
...
PeeringState*
ECPeeringTestFixture::get_peering_state(int shard)
{
...
PeeringCtx*
ECPeeringTestFixture::get_peering_ctx(int shard)
{
  ceph_assert(shard >= 0 && shard < k + m);
...
MockPeeringListener*
ECPeeringTestFixture::get_peering_listener(int shard)
{
  ceph_assert(shard >= 0 && shard < k + m);
...
int
ECPeeringTestFixture::get_primary_shard_from_osdmap() const
{
...
ECPeeringTestFixture::get_primary_listener()
{
...
PGBackend*
ECPeeringTestFixture::get_primary_backend()
{
  int primary_shard = get_primary_shard_from_osdmap();
  if (primary_shard < 0) {
    return nullptr;
...
void
ECPeeringTestFixture::event_initialize()
{
...
void
ECPeeringTestFixture::event_advance_map()
{
  ...
  for (auto& [shard, ctx] : shard_peering_ctxs) {
    ...
    event_loop->schedule_peering_event(
        shard, [ps, current_osdmap, lastmap, current_pgid, peering_ctx]() {
          // Get up/acting sets from OSDMap inside the lambda
          std::vector<int> up_osds, acting_osds;
          int up_primary = -1, acting_primary = -1;
          current_osdmap->pg_to_up_acting_osds(
              current_pgid, &up_osds, &up_primary, &acting_osds,
              &acting_primary);

          ps->advance_map(
              current_osdmap, lastmap, up_osds, up_primary, acting_osds,
              acting_primary, *peering_ctx);
        });
  }
  event_loop->run_until_idle();
}
...
void
ECPeeringTestFixture::event_activate_map()
{
...
void
ECPeeringTestFixture::dispatch_buffered_messages(int from_shard, PeeringCtx* ctx)
{
...
bool
ECPeeringTestFixture::all_shards_active()
{
  // Get acting set from OSDMap
  std::vector<int> acting_osds;
  int acting_primary = -1;
...
bool
ECPeeringTestFixture::all_shards_clean()
{
...
std::string
ECPeeringTestFixture::get_state_name(int shard)
{
  return get_peering_state(shard)->get_current_state();
}

void
ECPeeringTestFixture::suspend_osd(int osd)
{
  if (event_loop) {
...
void
ECPeeringTestFixture::unsuspend_osd(int osd)
{
  if (event_loop) {
...
bool
ECPeeringTestFixture::is_osd_suspended(int osd)
{
  return event_loop && event_loop->is_to_osd_suspended(osd);
}

void
ECPeeringTestFixture::suspend_primary_to_osd(int to_osd)
{
  if (event_loop) {
...
void
ECPeeringTestFixture::unsuspend_primary_to_osd(int to_osd)
{
  if (event_loop) {
    int primary = get_primary_shard_from_osdmap();
    if (primary >= 0) {
...
PeeringState*
ECPeeringTestFixture::create_peering_state(int shard)
{
  ...
  shard_peering_listeners[shard] = std::make_unique<MockPeeringListener>(
      ...
      std::move(listeners[shard]), store.get(), colls[shard], chs[shard]);
...
void
ECPeeringTestFixture::init_peering(bool dne)
{
  ...
  osdmap->pg_to_up_acting_osds(
      this->pgid, &up_osds, &up_primary, &acting_osds, &acting_primary);

  for (int shard : acting_osds) {
    ObjectStore::Transaction t;
    get_peering_state(shard)->init(
        (shard == acting_primary) ? 0 : 1, // role
        up_osds, up_primary, acting_osds, acting_primary, history,
        past_intervals, t);
...
void
ECPeeringTestFixture::update_osdmap_with_peering(
...
void
ECPeeringTestFixture::new_epoch_loop()
{
  int max = 10;
  do {
    ceph_assert(--max);
...
bool
ECPeeringTestFixture::new_epoch(bool if_required)
{
  bool did_work = false;
  ...
  if (acting_primary >= 0) {
    ...
    if (listener->pg_temp_wanted) {
      ...
      int up_primary = -1;
      osdmap->pg_to_up_acting_osds(
          this->pgid, &up_osds, &up_primary, nullptr, nullptr);
...
void
ECPeeringTestFixture::run_first_peering()
{
  init_peering();
  event_initialize();
  new_epoch_loop();
...
int
ECPeeringTestFixture::queue_transaction_helper(
    int shard,
    ObjectStore::Transaction&& t)
{
  if (t.empty()) {
...
void
ECPeeringTestFixture::mark_osd_down(int osd_id)
{
...
void
ECPeeringTestFixture::mark_osd_up(int osd_id)
{
...
void
ECPeeringTestFixture::mark_osds_down(const std::vector<int>& osd_ids)
{
...
void
ECPeeringTestFixture::advance_epoch()
{
  auto new_osdmap = std::make_shared<OSDMap>();
  new_osdmap->deepish_copy_from(*osdmap);
