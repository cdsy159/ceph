...
private:
  ...
  event_type_name(EventType type)
  {
    switch (type) {
    case EventType::GENERIC:
      return "GENERIC";
    case EventType::OSD_MESSAGE:
      return "OSD_MESSAGE";
    case EventType::TRANSACTION:
      return "TRANSACTION";
    case EventType::PEERING_MESSAGE:
      return "PEERING_MESSAGE";
    case EventType::CLUSTER_MESSAGE:
      return "CLUSTER_MESSAGE";
    case EventType::PEERING_EVENT:
...
public:
  EventLoop(bool verbose = false) :
    verbose(verbose)
  {}

  void
  schedule_generic(GenericEvent event)
  {
    events.emplace_back(EventType::GENERIC, -1, -1, std::move(event));
  }

  void
  schedule_osd_message(int from_osd, int to_osd, GenericEvent callback)
  {
    ceph_assert(from_osd >= 0);
    ceph_assert(to_osd >= 0);
    events.emplace_back(
        EventType::OSD_MESSAGE, from_osd, to_osd, std::move(callback));
  }

  void
  schedule_transaction(int osd, GenericEvent callback)
  {
    ceph_assert(osd >= 0);
  ...
  void
  schedule_peering_message(int from_osd, int to_osd, GenericEvent callback)
  {
    ceph_assert(from_osd >= 0);
    ceph_assert(to_osd >= 0);
    events.emplace_back(
        EventType::PEERING_MESSAGE, from_osd, to_osd, std::move(callback));
  }

  void
  schedule_cluster_message(int from_osd, int to_osd, GenericEvent callback)
  {
    ceph_assert(from_osd >= 0);
    ceph_assert(to_osd >= 0);
    events.emplace_back(
        EventType::CLUSTER_MESSAGE, from_osd, to_osd, std::move(callback));
  }

  void
  schedule_peering_event(int osd, GenericEvent callback)
  {
    ceph_assert(osd >= 0);
  ...
  void
  register_idle_callback(std::function<bool()> callback)
  {
    idle_callbacks.emplace_back(std::move(callback));
  }

  size_t
  queued_event_count() const
  {
    return events.size();
  }

  int
  get_events_executed() const
  {
    return events_executed;
  }

  const std::map<EventType, int>&
  get_stats_by_type() const
  {
  ...
  run_one()
  {
    ...
    if (event.from_osd >= 0 && is_from_osd_suspended(event.from_osd)) {
      ...
      suspend_osd = event.to_osd;
        ...
        event.from_osd >= 0 && event.to_osd >= 0 &&
    ...
    if (should_suspend) {
      ...
      if (verbose) {
        ...
        std::cout << " *** SUSPENDED - moving to suspended queue ***"
                  << std::endl;
      }
  ...
  run_many(int count)
  {
    ...
    if (verbose) {
      std::cout << "=== Executed " << executed << " events, " << events.size()
                << " remaining ===" << std::endl;
    }
   ...
   */
  void
  run_until_idle(int max_events = 10000)
  {
    if (verbose) {
      std::cout << "\n=== Running until idle";
      if (max_events > 0) {
   ...
   */
  void
  suspend_from_osd(int osd)
  {
    suspended_from_osds.insert(osd);
    if (verbose) {
      std::cout << "*** Events FROM OSD." << osd << " marked as SUSPENDED ***"
                << std::endl;
    }
  }
  ...
  unsuspend_from_osd(int osd)
  {
    ...
    if (it != suspended_events.end()) {
      if (verbose) {
        std::cout << "*** Events FROM OSD." << osd
                  << " marked as UNSUSPENDED - restoring " << it->second.size()
                  << " suspended events ***" << std::endl;
      }
   ...
   */
  void
  suspend_to_osd(int osd)
  {
    suspended_to_osds.insert(osd);
    if (verbose) {
      std::cout << "*** Events TO OSD." << osd << " marked as SUSPENDED ***"
                << std::endl;
    }
  }
  ...
  unsuspend_to_osd(int osd)
  {
    ...
    if (it != suspended_events.end()) {
      ...
      if (verbose) {
        std::cout << "*** Events TO OSD." << osd << " marked as UNSUSPENDED ***"
                  << std::endl;
      }
    }
  }
   ...
   */
  void
  suspend_from_to_osd(int from_osd, int to_osd)
  {
   ...
   */
  void
  unsuspend_from_to_osd(int from_osd, int to_osd)
  {
  ...
  bool
  is_from_osd_suspended(int osd) const
  {
    return suspended_from_osds.find(osd) != suspended_from_osds.end();
  }

  bool
  is_to_osd_suspended(int osd) const
  {
    return suspended_to_osds.find(osd) != suspended_to_osds.end();
  }

  bool
  is_from_to_osd_suspended(int from_osd, int to_osd) const
  {
    return suspended_from_to_osds.find({from_osd, to_osd}) !=
           suspended_from_to_osds.end();
  }

  size_t
  get_suspended_event_count(int osd) const
  {
    auto it = suspended_events.find(osd);
  ...
  size_t
  get_total_suspended_events() const
  {
    size_t total = 0;
