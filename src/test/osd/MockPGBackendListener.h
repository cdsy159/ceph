...
public:
  ...
  void
  set_store(ObjectStore* s, ObjectStore::CollectionHandle c)
  {
    store = s;
    ch = c;
  }
  ...
  void
  set_op_tracker(OpTracker* tracker)
  {
    op_tracker = tracker;
  }

  void
  set_peering_state(PeeringState* ps)
  {
    peering_state = ps;
  }
  ...
  void
  apply_stats(const hobject_t& soid, const object_stat_sum_t& delta_stats) override
  {}
  ...
  // Context wrapping
  Context*
  bless_context(Context* c) override
  {
    return c;
  }

  GenContext<ThreadPool::TPHandle&>*
  bless_gencontext(GenContext<ThreadPool::TPHandle&>* c) override
  {
    return c;
  }

  GenContext<ThreadPool::TPHandle&>*
  bless_unlocked_gencontext(GenContext<ThreadPool::TPHandle&>* c) override
  {
    return c;
  }
  ...
  epoch_t
  pgb_get_osdmap_epoch() const override
  {
  ...
  void
  op_applied(const eversion_t& applied_version) override
  {}

  bool
  should_send_op(pg_shard_t peer, const hobject_t& hoid) override
  {
    return true;
  }
...
#if POOL_MIGRATION
  void
  update_migration_watermark(const hobject_t& watermark) override
  {}
#endif

#if POOL_MIGRATION
  std::optional<hobject_t>
  consider_updating_migration_watermark(std::set<hobject_t>& deleted) override
  {
    return std::nullopt;
  }
#endif
  ...
  hobject_t
  get_temp_recovery_object(const hobject_t& target, eversion_t version) override
  {
    return hobject_t();
  }
  ...
  ConnectionRef
  get_con_osd_cluster(int peer, epoch_t from_epoch) override
  {
    return nullptr;
  }
  ...
  bool
  maybe_preempt_replica_scrub(const hobject_t& oid) override
  {
    return false;
  }
  ...
  const pg_missing_const_i*
  maybe_get_shard_missing(pg_shard_t peer) const override
  {
    if (peering_state) {
  ...
  bool
  is_missing_object(const hobject_t& oid) const override
  {
    return false;
  }
