...
public:
      ...
      const std::string& pool_name = "")
  {
    ...
    std::string name = pool_name.empty() ? ("pool_" + std::to_string(pool_id))
                                         : pool_name;
  ...
  static const pg_pool_t*
  get_pool(const OSDMap& osdmap, int64_t pool_id)
  {
    return osdmap.get_pg_pool(pool_id);
  }

  static const pg_pool_t*
  get_pool(const std::shared_ptr<OSDMap>& osdmap, int64_t pool_id)
  {
  ...
  static void
  set_pg_acting(OSDMap& osdmap, pg_t pgid, const std::vector<int>& acting)
  {
    OSDMap::Incremental inc(osdmap.get_epoch() + 1);
    inc.fsid = osdmap.get_fsid();
  ...
  static bool
  get_pg_acting(const OSDMap& osdmap, pg_t pgid, std::vector<int>& acting)
  {
    acting.clear();
    int primary;
  ...
  static void
  set_pg_acting_primary(OSDMap& osdmap, pg_t pgid, int primary)
  {
    OSDMap::Incremental inc(osdmap.get_epoch() + 1);
    inc.fsid = osdmap.get_fsid();
  ...
  static void
  set_pg_acting_primary(std::shared_ptr<OSDMap> osdmap, pg_t pgid, int primary)
  {
    set_pg_acting_primary(*osdmap, pgid, primary);
  }

  static bool
  get_pg_acting_primary(const OSDMap& osdmap, pg_t pgid, int& primary)
  {
    std::vector<int> acting;
    osdmap.pg_to_acting_osds(pgid, &acting, &primary);
    return primary >= 0;
  ...
  static pg_pool_t
  create_ec_pool(int k, int m, uint64_t stripe_width, uint64_t flags)
  {
    pg_pool_t pool;
  ...
  static pg_pool_t
  create_replicated_pool(int size, int min_size, int64_t pool_id = 0)
  {
    pg_pool_t pool;
  ...
  static void
  setup_ec_pg(OSDMap& osdmap, pg_t pgid, int k, int m, int primary_shard = 0)
  {
    std::vector<int> acting;
    for (int i = 0; i < k + m; i++) {
      acting.push_back(i);
  ...
  static void
  clear_pool_flag(OSDMap& osdmap, int64_t pool_id, uint64_t flag)
  {
  ...
  static void
  clear_pool_flag(std::shared_ptr<OSDMap> osdmap, int64_t pool_id, uint64_t flag)
  {
   ...
   */
  static void
  mark_osd_down(OSDMap& osdmap, int osd_id)
  {
    OSDMap::Incremental inc(osdmap.get_epoch() + 1);
    inc.fsid = osdmap.get_fsid();
  ...
  static void
  mark_osd_down(std::shared_ptr<OSDMap> osdmap, int osd_id)
  {
   ...
   */
  static void
  mark_osd_up(OSDMap& osdmap, int osd_id)
  {
    OSDMap::Incremental inc(osdmap.get_epoch() + 1);
    inc.fsid = osdmap.get_fsid();
  ...
  static void
  mark_osd_up(std::shared_ptr<OSDMap> osdmap, int osd_id)
  {
   ...
   */
  static void
  mark_osds_down(OSDMap& osdmap, const std::vector<int>& osd_ids)
  {
    OSDMap::Incremental inc(osdmap.get_epoch() + 1);
    inc.fsid = osdmap.get_fsid();
    for (int osd_id : osd_ids) {
      inc.new_state[osd_id] =
  ...
  static void
  mark_osds_down(std::shared_ptr<OSDMap> osdmap, const std::vector<int>& osd_ids)
  {
  ...
  static void
  advance_epoch(std::shared_ptr<OSDMap> osdmap)
  {
