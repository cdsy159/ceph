...
protected:
  ...
  uint64_t pool_flags = pg_pool_t::FLAG_EC_OVERWRITES |
  ...
  public:
    TestDpp(CephContext* cct) :
      NoDoutPrefix(cct, ceph_subsys_osd)
    {}
...
public:
  explicit PGBackendTestFixture(PoolType type = EC) :
    pool_type(type)
  {
    ...
    oss << "memstore_test_" << std::hex << std::setfill('0') << std::setw(16)
        << random_num;
    data_dir = oss.str();
  ...
  SetUp() override
  {
    ...
    if (r < 0) {
      r = -errno;
      std::cerr << __func__ << ": unable to create " << data_dir << ": "
...
public:
  ...
  hobject_t
  make_test_object(const std::string& name) const
  {
  ...
  /// in EC pools.
  ObjectContextRef
  get_or_create_obc(const hobject_t& hoid, bool exists = false, uint64_t size = 0)
  {
    ...
    if (exists && pool_type == EC && store && !chs.empty()) {
      ...
      bool has_outstanding_writes =
          (writes_it != outstanding_writes.end() && writes_it->second > 0);
   ...
   */
  void
  set_next_version(uint64_t version)
  {
   ...
   */
  ObjectContextRef get_object_context(const hobject_t& hoid);
