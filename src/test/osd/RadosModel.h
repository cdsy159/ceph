...
public:
  ...
  TestOp(int n, RadosTestContext* context, TestOpStat* stat = 0) :
    num(n), context(context), stat(stat)
  {}
...
public:
    ...
    while (next || !inflight.empty()) {
      ...
      while (1) {
        for (auto i = inflight.begin(); i != inflight.end();) {
          if ((*i)->finished()) {
            cout_prefix() << (*i)->num << ": done (" << (inflight.size() - 1)
                          << " left)" << std::endl;
            delete *i;
            inflight.erase(i++);
          } else {
  ...
  TestWatchContext*
  get_watch_context(const std::string& oid)
  {
    return watches.count(oid) ? watches[oid] : 0;
  }

  TestWatchContext*
  watch(const std::string& oid)
  {
  ...
  ObjectDesc
  get_most_recent(const std::string& oid)
  {
    ObjectDesc new_obj;
  ...
  void
  rm_object_attrs(const std::string& oid, const std::set<std::string>& attrs)
  {
  ...
  void
  remove_object_header(const std::string& oid)
  {
    ObjectDesc new_obj = get_most_recent(oid);
  ...
  void
  update_object_header(const std::string& oid, const bufferlist& bl)
  {
    ObjectDesc new_obj = get_most_recent(oid);
    new_obj.header = bl;
    new_obj.exists = true;
  ...
  update_object_attrs(
      ...
      const std::map<std::string, ContDesc>& attrs)
  {
    ObjectDesc new_obj = get_most_recent(oid);
  ...
  void
  update_object_full(const std::string& oid, const ObjectDesc& contents)
  {
  ...
  void
  update_object_undirty(const std::string& oid)
  {
    ObjectDesc new_obj = get_most_recent(oid);
    new_obj.dirty = false;
    pool_obj_cont[current_snap].insert_or_assign(oid, new_obj);
  ...
  void
  update_object_version(const std::string& oid, uint64_t version, int snap = -1)
  {
    for (auto i = pool_obj_cont.rbegin(); i != pool_obj_cont.rend(); ++i) {
      ...
      if (j != i->second.end()) {
        ...
        cout_prefix() << __func__ << " oid " << oid << " v " << version << " "
                      << j->second.most_recent() << " "
                      << (j->second.dirty ? "dirty" : "clean") << " "
                      << (j->second.exists ? "exists" : "dne") << std::endl;
        break;
      }
    }
  }
  ...
  void
  remove_object(const std::string& oid)
  {
  ...
  bool
  find_object(const std::string& oid, ObjectDesc* contents, int snap = -1) const
  {
  ...
  void
  update_object_redirect_target(const std::string& oid, const std::string& target)
  {
    redirect_objs[oid] = target;
  }

  void
  update_object_chunk_target(
      const std::string& oid,
      uint64_t offset,
      const ChunkDesc& info)
  {
    for (auto i = pool_obj_cont.crbegin(); i != pool_obj_cont.crend(); ++i) {
      if (i->second.count(oid) != 0) {
  ...
  bool
  object_existed_at(const std::string& oid, int snap = -1) const
  {
    ObjectDesc contents;
  ...
  void
  remove_snap(int snap)
  {
    std::map<int, std::map<std::string, ObjectDesc>>::iterator next_iter =
        pool_obj_cont.find(snap);
    ceph_assert(next_iter != pool_obj_cont.end());
    std::map<int, std::map<std::string, ObjectDesc>>::iterator current_iter =
        next_iter++;
  ...
  void
  roll_back(const std::string& oid, int snap)
  {
    ceph_assert(!get_watch_context(oid));
    ObjectDesc contents;
  ...
  void
  update_object_tier_flushed(const std::string& oid, int snap)
  {
    for (auto i = pool_obj_cont.rbegin(); i != pool_obj_cont.rend(); ++i) {
      if (snap != -1 && snap < i->first)
        continue;
  ...
  bool
  check_oldest_snap_flushed(const std::string& oid, int snap)
  {
    for (auto i = pool_obj_cont.rbegin(); i != pool_obj_cont.rend(); ++i) {
      if (snap != -1 && snap < i->first)
        continue;
      ...
      if (j != i->second.end() && !j->second.flushed) {
        cout_prefix() << __func__ << " oid " << oid << " v "
                      << j->second.version << " " << j->second.most_recent()
  ...
  bool
  check_chunks_refcount(
      librados::IoCtx& chunk_pool_ctx,
      librados::IoCtx& manifest_pool_ctx)
  {
      ...
      for (const auto& i : result) {
        ...
        for (auto& pp : byo->by_object) {
          ...
          if (src_refcount > dst_refcount) {
            std::cerr << " src_object " << pp << ": src_refcount "
                      << src_refcount << ", dst_object " << oid
...
public:
  ...
  _begin() override
  {
    ...
    std::set<std::string> to_remove;
    {
      ...
      cont = ContDesc(
          context->seq_num, context->current_snap, context->seq_num, "");
  ...
  void
  _finish(CallbackInfo* info) override
  {
    std::lock_guard l{context->state_lock};
    done = true;
...
public:
  ...
  WriteOp(
      ...
      bool do_excl,
      TestOpStat* stat = 0) :
    TestOp(n, context, stat), oid(oid), do_append(do_append), do_excl(do_excl)
  {}
  ...
  _begin() override
  {
    ...
    cont = ContDesc(
        context->seq_num, context->current_snap, context->seq_num, prefix);
    ...
    if (do_append) {
      ...
      cont_gen = new AppendGenerator(
          prev_length, alignment, context->min_stride_size,
    ...
    context->cout_prefix() << num << ":  seq_num " << context->seq_num
    ...
    cb_arg = new std::pair<TestOp*, TestOp::CallbackInfo*>(
        this, new TestOp::CallbackInfo(tid++));
    rcompletion =
        context->rados.aio_create_completion((void*)cb_arg, &write_callback);
    waiting_on++;
    ...
    context->io_ctx.aio_operate(
        context->prefix + oid, rcompletion, &read_op,
  ...
  _finish(CallbackInfo* info) override
  {
    ...
    context->cout_prefix() << num << ":  finishing write tid " << tid << " to "
                           << context->prefix + oid << std::endl;

    if (tid <= last_acked_tid) {
      std::cerr << "Error: finished tid " << tid << " when last_acked_tid was "
                << last_acked_tid << std::endl;
      ceph_abort();
    }
    last_acked_tid = tid;
    ...
    if (waiting_on == 0) {
      ...
      for (auto i = waiting.begin(); i != waiting.end();) {
        ...
        if ((*i)->get_version64() > version) {
          context->cout_prefix()
              << num << ":  oid " << oid << " updating version " << version
              << " to " << (*i)->get_version64() << std::endl;
          version = (*i)->get_version64();
        } else {
          context->cout_prefix() << num << ":  oid " << oid << " version "
                                 << version << " is already newer than "
...

      {
        ...
        if (old_value.deleted())
          context->cout_prefix()
              << num << ":  left oid " << oid << " deleted" << std::endl;
        else
...
public:
  ...
  _finish(CallbackInfo* info) override
  {
    ...
    context->cout_prefix() << num << ":  finishing writesame tid " << tid
    ...
    if (waiting_on == 0) {
      ...
      for (auto i = waiting.begin(); i != waiting.end();) {
        ...
        if ((*i)->get_version64() > version) {
          context->cout_prefix()
              << "oid " << oid << "updating version " << version << "to "
              << (*i)->get_version64() << std::endl;
          version = (*i)->get_version64();
        } else {
          context->cout_prefix()
...
public:
  ...
  _begin() override
  {
    ...
    if (r && !(r == -ENOENT && !present)) {
      std::cerr << "r is " << r << " while deleting " << oid
                << " and present is " << present << std::endl;
      ceph_abort();
    }
...
public:
  ...
  maybe_randomize_offlen(uint8_t offlen_randomization_ratio, uint64_t max_len)
  {
  ...
  void
  _do_read(librados::ObjectReadOperation& read_op, int index)
  {
    uint64_t max_len = 0;
    if (old_value.has_contents()) {
      max_len = old_value.most_recent_gen()->get_length(old_value.most_recent());
    }
    offlens[index] = maybe_randomize_offlen(offlen_randomization_ratio, max_len);
    ...
    if (context->no_sparse || rand() % 2) {
      ...
      read_op.checksum(
          LIBRADOS_CHECKSUM_TYPE_CRC32C, init_value_bl, offset, length, 0,
          &checksums[index], &checksum_retvals[index]);
    } else {
      is_sparse_read[index] = true;
      read_op.sparse_read(
          offset, length, &extent_results[index], &results[index],
          &retvals[index]);
    }
  }
  ...
  _begin() override
  {
    ...
    context->cout_prefix() << num << ": read oid " << oid << " snap " << snap
                           << std::endl;
    done = 0;
    for (uint32_t i = 0; i < 3; i++) {
      completions[i] =
          context->rados.aio_create_completion((void*)this, &read_callback);
    }
    ...
    if (old_value.deleted())
      ...
      context->cout_prefix()
          << num << ":  expect " << old_value.most_recent() << std::endl;
    ...
    ceph_assert(!context->io_ctx.aio_operate(
        context->prefix + oid, completions[0], &op, flags, NULL));
    waiting_on++;
    ...
    for (uint32_t i = 1; i < 3; ++i) {
      ...
      ceph_assert(!context->io_ctx.aio_operate(
          context->prefix + oid, completions[i], &pipeline_op, 0));
      waiting_on++;
    }
    ...
    for (auto it = completions.begin(); it != completions.end(); ++it) {
      ...
      if (err != retval) {
        std::cerr << num << ": Error: oid " << oid
                  << " read returned different error codes: " << retval
                  << " and " << err << std::endl;
        ceph_abort();
      }
      if (err) {
        if (!(err == -ENOENT && old_value.deleted())) {
          std::cerr << num << ": Error: oid " << oid
                    << " read returned error code " << err << std::endl;
          ceph_abort();
        }
    ...
    if (!retval) {
      ...
      if (iter == xattrs.end()) {
        if (old_value.has_contents()) {
          std::cerr << num
                    ...
                    << old_value.has_contents() << std::endl;
      ...
      if (old_value.has_contents()) {
        ...
        for (unsigned i = 0; i < results.size(); i++) {
          if (is_sparse_read[i]) {
            if (!old_value.check_sparse(
                    extent_results[i], results[i], offlens[i])) {
              std::cerr << num << ": oid " << oid << " contents " << to_check
                        << " corrupt" << std::endl;
              context->errors++;
            }
          } else {
            ...
            if (checksum_retvals[i] != 0 || checksum != results[i].crc32c(-1)) {
              std::cerr << num << ": oid " << oid << " checksum "
                        << checksums[i] << " incorrect, expecting "
                        << results[i].crc32c(-1) << std::endl;
              context->errors++;
            }
          }
        }
...
public:
  ...
  _begin() override
  {
    ...
    if (!context->pool_snaps) {
      ...
      int j = 0;
      for (auto i = context->snaps.rbegin(); i != context->snaps.rend();
           ++i, ++j) {
        snapset[j] = i->second;
      }
      ...
      int r =
          context->io_ctx.selfmanaged_snap_set_write_ctx(context->seq, snapset);
      if (r) {
        std::cerr << "r is " << r << " snapset is " << snapset << " seq is "
                  << context->seq << std::endl;
        ceph_abort();
      }
    }
  }
  ...
  bool
  must_quiesce_other_ops() override
  {
...
public:
  ...
  SnapRemoveOp(int n, RadosTestContext* context, int snap, TestOpStat* stat = 0) :
    TestOp(n, context, stat), to_remove(snap)
  {}
...
public:
  WatchOp(
      ...
      TestOpStat* stat = 0) :
    TestOp(n, context, stat), oid(_oid)
  {}
...
public:
  ...
  _begin() override
  {
    ...
    context->cout_prefix() << "rollback oid " << oid << " to " << roll_back_to
                           << std::endl;
    ...
    }
    {
      std::pair<TestOp*, TestOp::CallbackInfo*>* cb_arg =
          new std::pair<TestOp*, TestOp::CallbackInfo*>(
              this, new TestOp::CallbackInfo(1));
      comps[1] =
          context->rados.aio_create_completion((void*)cb_arg, &write_callback);
    ...
    if (existed_after) {
      std::pair<TestOp*, TestOp::CallbackInfo*>* cb_arg =
          new std::pair<TestOp*, TestOp::CallbackInfo*>(
              this, new TestOp::CallbackInfo(2));
      comps[2] =
          context->rados.aio_create_completion((void*)cb_arg, &write_callback);
      context->io_ctx.aio_operate(
...
public:
  ...
  _begin() override
  {
    ...
    std::pair<TestOp*, TestOp::CallbackInfo*>* read_cb_arg =
        new std::pair<TestOp*, TestOp::CallbackInfo*>(
            this, new TestOp::CallbackInfo(1));
    comp_racing_read = context->rados.aio_create_completion(
  ...
  _finish(CallbackInfo* info) override
  {
    ...
    if (info->id == 0) {
      ...
      context->cout_prefix() << num << ":  finishing copy_from to "
                             << context->prefix + oid << std::endl;
      if ((r = comp->get_return_value())) {
        if (r == -ENOENT && src_value.deleted()) {
          context->cout_prefix()
              << num << ":  got expected ENOENT (src dne)" << std::endl;
        } else {
          std::cerr << "Error: oid " << oid << " copy_from " << oid_src
                    << " returned error code " << r << std::endl;
          ceph_abort();
        }
      } else {
      ...
      context->cout_prefix() << num << ":  finishing copy_from racing read to "
                             << context->prefix + oid << std::endl;
...
public:
  ...
  {
    context->state_lock.lock();
    context->cout_prefix() << num << ": chunk read oid " << oid << " snap "
                           << snap << std::endl;
    done = 0;
    ...
    context->cout_prefix() << num << ": ori offset " << iter->first
                           << " req offset " << offset << " ori length "
                           << iter->second.length << " req length " << length
                           << " ori tgt_offset " << iter->second.offset
                           << " req tgt_offset " << tgt_offset << " tgt_oid "
                           << tgt_oid << std::endl;
    ...
    ceph_assert(!context->io_ctx.aio_operate(
        context->prefix + tgt_oid, completions[1], &op, flags, NULL));
  ...
  _finish(CallbackInfo* info) override
  {
    ...
    if (!retval) {
      ...
      if (tgt_value.has_contents()) {
        ...
        if (checksum_retvals[1] != 0) {
          std::cerr << num << ": oid " << oid << " checksum retvals "
                    << checksums[0] << " error " << std::endl;
          context->errors++;
        }
        ...
        if (checksum[0] != checksum[1]) {
          std::cerr << num << ": oid " << oid << " checksum src " << checksum[0]
                    << " chunksum tgt " << checksum[1]
                    << " incorrect, expecting " << results[0].crc32c(-1)
                    << std::endl;
          context->errors++;
        }
        if (context->errors)
...
public:
  ...
  _begin() override
  {
    ...
    context->cout_prefix() << "copy op oid " << oid_src << " to " << oid
                           << " tgt_pool_name " << tgt_pool_name << std::endl;
  ...
  _finish(CallbackInfo* info) override
  {
    ...
    if (info->id == 0) {
      ceph_assert(comp->is_complete());
      context->cout_prefix()
          << num << ":  finishing copy op to oid " << oid << std::endl;
      if ((r = comp->get_return_value())) {
        std::cerr << "Error: oid " << oid << " write returned error code " << r
                  << std::endl;
        ceph_abort();
      }
    }
...
public:
  ...
  get_rand_off_len(uint32_t max_len)
  {
  ...
  _begin() override
  {
    ...
    if (!src_value.deleted() && oid_tgt.empty()) {
      ...
      ceph_assert(r > 0);
      std::string fp_oid =
          ceph::crypto::digest<ceph::crypto::SHA256>(bl).to_str();
    ...
    context->cout_prefix() << num << ": " << "set_chunk oid " << oid
                           << " offset: " << offset << " length: " << length
                           << " target oid " << target_oid
                           << " offset: " << tgt_offset << " snap " << snap
                           << std::endl;

    op.set_chunk(
        offset, length, context->low_tier_io_ctx, target_oid, tgt_offset,
  ...
  _finish(CallbackInfo* info) override
  {
    ...
    if (info->id == 0) {
      ceph_assert(comp->is_complete());
      context->cout_prefix()
          << num << ":  finishing set_chunk to oid " << oid << std::endl;
      if ((r = comp->get_return_value())) {
        if (r == -ENOENT && src_value.deleted()) {
          context->cout_prefix()
              << num << ":  got expected ENOENT (src dne)" << std::endl;
        } else if (
            r == -ENOENT && context->oid_set_chunk_tgt_pool.find(oid_tgt) !=
                                context->oid_set_chunk_tgt_pool.end()) {
          context->cout_prefix() << num << ": get expected ENOENT tgt oid "
                                 << oid_tgt << std::endl;
        } else if (r == -ERANGE && src_value.deleted()) {
          context->cout_prefix()
              << num << ":  got expected ERANGE (src dne)" << std::endl;
        } else if (r == -EOPNOTSUPP) {
          context->cout_prefix()
              << "Range is overlapped: oid " << oid << " set_chunk " << oid_tgt
              << " returned error code " << r << " offset: " << offset
              << " length: " << length << std::endl;
          context->update_object_version(oid, comp->get_version64());
        } else {
          std::cerr << "Error: oid " << oid << " set_chunk " << oid_tgt
                    << " returned error code " << r << std::endl;
          ceph_abort();
        }
      } else {
...
public:
  ...
  _begin() override
  {
    ...
    if (!context->redirect_objs[oid].empty()) {
      ...
      context->low_tier_io_ctx.aio_operate(
          context->prefix + oid_tgt, comp, &op,
          librados::OPERATION_ORDER_READS_WRITES);
      comp->wait_for_complete();
      if ((r = comp->get_return_value())) {
        std::cerr << "Error: oid " << oid << " copy_from " << oid_tgt
                  << " returned error code " << r << std::endl;
        ceph_abort();
      }
      comp->release();
    ...
    rd_op.stat(NULL, NULL, NULL);
    context->io_ctx.aio_operate(
        context->prefix + oid, comp, &rd_op,
        librados::OPERATION_ORDER_READS_WRITES |
            librados::OPERATION_IGNORE_REDIRECT,
        NULL);
    comp->wait_for_complete();
    if ((r = comp->get_return_value()) && !src_value.deleted()) {
      std::cerr << "Error: oid " << oid << " stat returned error code " << r
                << std::endl;
      ceph_abort();
    }
    context->update_object_version(oid, comp->get_version64());
    ...
    rd_op.stat(NULL, NULL, NULL);
    context->low_tier_io_ctx.aio_operate(
        context->prefix + oid_tgt, comp, &rd_op,
        librados::OPERATION_ORDER_READS_WRITES |
            librados::OPERATION_IGNORE_REDIRECT,
        NULL);
    comp->wait_for_complete();
    if ((r = comp->get_return_value())) {
      std::cerr << "Error: oid " << oid_tgt << " stat returned error code " << r
                << std::endl;
      ceph_abort();
    }
    ...
    if (src_value.version != 0 && !src_value.deleted())
      op.assert_version(src_value.version);
    op.set_redirect(
        context->prefix + oid_tgt, context->low_tier_io_ctx, tgt_version);
  ...
  _finish(CallbackInfo* info) override
  {
    ...
    if (info->id == 0) {
      ceph_assert(comp->is_complete());
      context->cout_prefix()
          << num << ":  finishing set_redirect to oid " << oid << std::endl;
      if ((r = comp->get_return_value())) {
        if (r == -ENOENT && src_value.deleted()) {
          context->cout_prefix()
              << num << ":  got expected ENOENT (src dne)" << std::endl;
        } else {
          std::cerr << "Error: oid " << oid << " set_redirect " << oid_tgt
                    << " returned error code " << r << std::endl;
          ceph_abort();
        }
      } else {
...
public:
  ...
  TierPromoteOp(
      ...
      const std::string& oid,
      TestOpStat* stat) :
    TestOp(n, context, stat), completion(NULL), oid(oid)
  {}
...
public:
  ...
  TierFlushOp(
      ...
      const std::string& oid,
      TestOpStat* stat) :
    TestOp(n, context, stat), completion(NULL), oid(oid), snap(-1)
  {}
  ...
  _begin() override
  {
    ...
    context->cout_prefix() << num << ": tier_flush oid " << oid << " snap "
                           << snap << std::endl;
  ...
  _finish(CallbackInfo* info) override
  {
    ...
    if (r == 0) {
      ...
      if (src_value.deleted()) {
	    ...
	    -- adjacent snapshot might not be recovered yet"
                                 << std::endl;
        } else {
...
public:
  ...
  _begin() override
  {
    ...
    context->cout_prefix() << num << ": tier_evict oid " << oid << " snap "
                           << snap << std::endl;
    ...
    op.cache_evict();
    int r = context->io_ctx.aio_operate(
        context->prefix + oid, completion, &op,
...
public:
  HitSetListOp(
      ...
      uint32_t hash,
      TestOpStat* stat = 0) :
    TestOp(n, context, stat), comp1(NULL), comp2(NULL), hash(hash)
  {}
  ...
  _finish(CallbackInfo* info) override
  {
    ...
    if (!comp2) {
      if (ls.empty()) {
        ...
        auto cb_arg = new std::pair<TestOp*, TestOp::CallbackInfo*>(
            this, new TestOp::CallbackInfo(0));
        comp2 = context->rados.aio_create_completion(
      ...
      if (r == 0) {
        ...
        decode(hitset, p);
        context->cout_prefix()
            << num << ": got hitset of type " << hitset.get_type_name()
            << " size " << bl.length() << std::endl;
      } else {
...
public:
  ...
  _begin() override
  {
    ...
    op.undirty();
    int r =
        context->io_ctx.aio_operate(context->prefix + oid, completion, &op, 0);
    ceph_assert(!r);
  }
...
public:
  ...
  IsDirtyOp(
      ...
      const std::string& oid,
      TestOpStat* stat = 0) :
    TestOp(n, context, stat), completion(NULL), oid(oid), dirty(false)
  {}
  ...
  _begin() override
  {
    ...
    context->cout_prefix() << num << ": is_dirty oid " << oid << " snap "
                           << snap << std::endl;
  ...
  _finish(CallbackInfo* info) override
  {
    ...
    if (r == 0) {
      context->cout_prefix()
          << num << ":  " << (dirty ? "dirty" : "clean") << std::endl;
...
public:
  ...
  _begin() override
  {
    ...
    can_fail = true;
    context->cout_prefix() << num << ": "
                           << (blocking ? "cache_flush" : "cache_try_flush")
...
public:
  ...
  _begin() override
  {
    ...
    context->cout_prefix() << num << ": cache_evict oid " << oid << " snap "
                           << snap << std::endl;
