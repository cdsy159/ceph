...
public:
  ...
  get_obc(
      ...
      const map<std::string, ceph::buffer::list, std::less<>>& attrs) override
  {
  ...
  const map<hobject_t, std::set<pg_shard_t>>&
  get_missing_loc_shards() const override
  {
    return missing_loc_shards;
  }

  const map<pg_shard_t, pg_missing_t>&
  ...
  send_message_osd_cluster(
      vector<std::pair<int, Message*>>& messages,
  ...
  ostream&
  gen_dbg_prefix(ostream& out) const override
  {
    out << dbg_prefix;
    return out;
  }
  ...
  const map<pg_shard_t, pg_info_t>&
  ...
  log_operation(
      ...
      const optional<pg_hit_set_history_t>& hset_history,
...
TEST(ECCommon, get_min_avail_to_read_shards)
{
  ...
  /* Read to every data shard, missing shard. */
  {
    ...
    int parity_shard = k;
    listenerStub.acting_shards.erase(
        pg_shard_t(int(missing_shard), shard_id_t(missing_shard)));
    ...
    for (shard_id_t i; i < k; ++i) {
      if (i != missing_shard) {
        ...
        ref.shard_reads[shard_id_t(parity_shard)].pg_shard =
            pg_shard_t(parity_shard, shard_id_t(parity_shard));
      }
    }
  ...
  /* Read to every data shard, missing shard, missing shard is adjacent. */
  {
    ...
    listenerStub.acting_shards.erase(
        pg_shard_t(missing_shard, shard_id_t(missing_shard)));
  ...
  /* Read to every data shard, missing shard. */
  {
    ...
    pipeline.get_min_avail_to_read_shards(
        hoid, false, false, read_request, error_shards);
...
TEST(ECCommon, get_min_want_to_read_shards_bug67087)
{
  ...
  {
    pipeline.get_min_want_to_read_shards(to_read1, want_to_read);
    ASSERT_EQ(want_to_read, ref);

    pipeline.get_min_want_to_read_shards(to_read2, want_to_read);
...
TEST(ECCommon, get_remaining_shards)
{
...

  {
    ...
    for (unsigned int i = 0; i < k; i++) {
      ...
      unsigned int shard_id = std::cmp_equal(i, missing_shard) ? parity_shard
                                                               : i;
  ...
  // Request re-read. There is a page of overlap in what is already read.
  {
    ...
    ECUtil::shard_extent_set_t to_read(s.get_k_plus_m());
    s.ro_range_to_shard_extent_set(
        chunk_size / 2, chunk_size + align_size, to_read);
    ECCommon::read_request_t read_request(to_read, false, object_size);
    unsigned int missing_shard = 1;
    ...
    read_result.processed_read_requests[shard_id_t(0)].insert(
        chunk_size / 2, bl.length());
    ...
    ref.shard_reads[shard_id_t(2)].extents.insert(
        0, chunk_size / 2 + align_size);
    ref.shard_reads[shard_id_t(2)].pg_shard = pg_shards[2];
    ref.shard_reads[shard_id_t(3)].extents.insert(
        0, chunk_size / 2 + align_size);
    ref.shard_reads[shard_id_t(3)].pg_shard = pg_shards[3];
    ref.shard_reads[shard_id_t(4)].extents.insert(
        0, chunk_size / 2 + align_size);
...
test_decode(
    ...
    const ECUtil::shard_extent_set_t& want,
    const shard_id_set& acting_set)
{
