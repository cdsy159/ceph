...
TEST(ECUtil, stripe_info_t)
{
  ...
  ASSERT_EQ(
      s.logical_to_prev_chunk_offset((swidth * 2) - 1), s.get_chunk_size());
  ...
  ASSERT_EQ(s.logical_to_prev_stripe_offset(swidth), s.get_stripe_width());
  ASSERT_EQ(
      s.logical_to_prev_stripe_offset((swidth * 2) - 1), s.get_stripe_width());

  ASSERT_EQ(
      s.aligned_logical_offset_to_chunk_offset(2 * swidth),
      2 * s.get_chunk_size());
  ASSERT_EQ(
      s.aligned_chunk_offset_to_logical_offset(2 * s.get_chunk_size()),
  ...
  ASSERT_EQ(
      s.chunk_aligned_offset_len_to_chunk(make_pair(swidth, 10 * swidth)),
  ...
  ASSERT_EQ(
      s.chunk_aligned_offset_len_to_chunk(
          make_pair(s.get_chunk_size(), 10 * swidth + s.get_chunk_size())),
      make_pair<uint64_t>(0, 11 * s.get_chunk_size()));
  ...
  ASSERT_EQ(
      s.chunk_aligned_offset_len_to_chunk(make_pair(
          2 * s.get_chunk_size(), 10 * swidth + 2 * s.get_chunk_size())),
      make_pair<uint64_t>(0, 11 * s.get_chunk_size()));

  ASSERT_EQ(
      s.offset_len_to_stripe_bounds(make_pair(swidth - 10, (uint64_t)20)),
...
TEST(ECCommon, get_min_want_to_read_shards)
{
  ...
  // read nothing at the very beginning
  {
    ...
    ECCommonL::ReadPipeline::get_min_want_to_read_shards(0, 0, s, &want_to_read);
