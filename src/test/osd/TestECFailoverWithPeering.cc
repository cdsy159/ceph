 ...
 */
class TestECFailoverWithPeering
  : public ECPeeringTestFixture,
...
TEST_P(TestECFailoverWithPeering, OSDFailureWithPeering)
{
  ...
  EXPECT_EQ(
      4, event_loop->get_stats_by_type().at(EventLoop::EventType::OSD_MESSAGE));
...
TEST_P(TestECFailoverWithPeering, PrimaryFailoverWithPeering)
{
  ...
  ASSERT_GE(new_primary_shard, 0)
      << "Should have a valid new primary after failover";
  ...
  EXPECT_TRUE(get_peering_listener(new_primary_shard)
                  ->backend_listener->pgb_is_primary())
...
TEST_P(TestECFailoverWithPeering, MultipleOSDFailuresWithPeering)
{
  ...
  for (int failed_osd : failed_osds) {
    EXPECT_TRUE(
        primary_ps->get_acting_recovery_backfill().count(
...
TEST_P(TestECFailoverWithPeering, RecoveryWithPeering)
{
  ...
  EXPECT_TRUE(all_shards_clean())
      << "All shards should be clean before recovery test";
  ...
  int read_result = read_object(
      obj1_name, 0, obj1_data.length(), obj1_read, obj1_data.length());
  EXPECT_GE(read_result, 0)
      << "First object should be readable after OSD failure";
      ...
      << "First object read length should match after failure";
  {
    ...
    EXPECT_EQ(read_str, obj1_data) << "First object data should be correct "
                                      "after OSD failure (EC reconstruction)";
  }

  bufferlist obj2_read;
  read_result = read_object(
      obj2_name, 0, obj2_data.length(), obj2_read, obj2_data.length());
  EXPECT_GE(read_result, 0)
      << "Second object should be readable after OSD failure";
      ...
      << "Second object read length should match after failure";
  {
    ...
    EXPECT_EQ(read_str, obj2_data) << "Second object data should be correct "
                                      "after OSD failure (EC reconstruction)";
  }

  const std::string post_recovery_obj = "test_post_recovery";
  const std::string post_recovery_data =
      "Data written after OSD failure and recovery";

  result = create_and_write(post_recovery_obj, post_recovery_data);
  EXPECT_EQ(result, 0)
      << "Write after OSD failure should complete successfully";

  bufferlist post_recovery_read;
  read_result = read_object(
      post_recovery_obj, 0, post_recovery_data.length(), post_recovery_read,
      post_recovery_data.length());
  ...
  {
    std::string read_str(
        post_recovery_read.c_str(), post_recovery_read.length());
  ...
  EXPECT_TRUE(failed_ps != nullptr)
      << "Failed OSD's PeeringState should still exist";
 ...
 * teardown.
 */
TEST_P(TestECFailoverWithPeering, RollbackAfterOSDFailure)
{
