...
using SelectObjectSize = ProgramOptionSelector<
    std::pair<int, int>,
...
inline constexpr std::array<uint64_t, block_size_array_size_stable>
    block_size_choices_stable = {
        {2048, // Default - test boundaries for EC 4K chunk size
         32768}};


using SelectBlockSize = StableOptionSelector<
    uint64_t,
...
using SelectErasurePlugin = StableOptionSelector<
    std::string_view,
...
public:
  ...
  inline bool
  get_allow_pool_autoscaling()
  {
    return allow_pool_autoscaling;
  }

  inline bool
  get_allow_pool_balancer()
  {
    return allow_pool_balancer;
  }

  inline bool
  get_allow_pool_deep_scrubbing()
  {
    return allow_pool_deep_scrubbing;
  }

  inline bool
  get_allow_pool_scrubbing()
  {
    return allow_pool_scrubbing;
  }

  inline bool
  get_allow_pool_ec_optimizations()
  {
    return allow_pool_ec_optimizations;
  }

  inline bool
  is_replicated_pool() const
  {
    return pool_type == pg_pool_t::TYPE_REPLICATED;
  }

  inline std::optional<Profile>
  getProfile()
  {
    return profile;
  }
...
protected:
  ...
  std::pair<ceph::io_exerciser::Sequence, ceph::io_exerciser::Sequence> seq_range;
  ...
  std::optional<std::pair<std::string_view, std::string_view>> pool_mappinglayers;
  bool testrecovery;
  bool checkconsistency;
  bool delete_objects;
  GenerationType data_generation_type;
