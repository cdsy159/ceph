...
shard_extent_map_t
imap_from_vector(
    vector<vector<pair<uint64_t, uint64_t>>>&& in,
    stripe_info_t const* sinfo)
{
...
shard_extent_map_t
imap_from_iset(const shard_extent_set_t& sset, stripe_info_t* sinfo)
{
  shard_extent_map_t out(sinfo);
...
struct Client : public ECExtentCache::BackendReadListener {
  ...
  Client(uint64_t chunk_size, int k, int m, uint64_t cache_size) :
    ...
    lru(cache_size),
    cache(*this, lru, sinfo, g_ceph_context){};
  ...
  void
  cache_ready(const hobject_t& _oid, const shard_extent_map_t& _result)
  {
    ceph_assert(oid == _oid);
  ...
  void
  complete_write(ECExtentCache::OpRef& op)
  {
  ...
  void
  cache_execute(ECExtentCache::OpRef& op)
  {
...
TEST(ECExtentCache, double_write_done)
{
  ...
  optional op = cl.cache.prepare(
      cl.oid, nullopt, to_write, 10, 10, false, [&cl](ECExtentCache::OpRef& op) {
...
TEST(ECExtentCache, simple_write)
{
  Client cl(32, 2, 1, 64);
  {
    ...
    auto to_write =
        iset_from_vector({{{0, 10}}, {{0, 10}}}, cl.get_stripe_info());
    ...
    optional op = cl.cache.prepare(
        cl.oid, to_read, to_write, 10, 10, false,
...
TEST(ECExtentCache, sequential_appends)
{
  ...
  // The first write...
  optional op1 = cl.cache.prepare(
      cl.oid, nullopt, to_write1, 0, 10, false, [&cl](ECExtentCache::OpRef& op) {
        cl.cache_ready(op->get_hoid(), op->get_result());
      });
  ...
  // The first write...
  optional op2 = cl.cache.prepare(
      cl.oid, nullopt, to_write1, 10, 20, false,
      [&cl](ECExtentCache::OpRef& op) {
        cl.cache_ready(op->get_hoid(), op->get_result());
      });
...
TEST(ECExtentCache, multiple_writes)
{
  ...
  optional op1 = cl.cache.prepare(
      cl.oid, to_read1, to_write1, 10, 10, false,
      [&cl](ECExtentCache::OpRef& op) {
        cl.cache_ready(op->get_hoid(), op->get_result());
      });
  cl.cache_execute(*op1);
  ...
  auto to_write2 = iset_from_vector({{{10, 10}}}, cl.get_stripe_info());
  optional op2 = cl.cache.prepare(
      cl.oid, to_read2, to_write2, 10, 10, false,
      [&cl](ECExtentCache::OpRef& op) {
        cl.cache_ready(op->get_hoid(), op->get_result());
      });
  cl.cache_execute(*op2);
  ...
  optional op3 = cl.cache.prepare(
      cl.oid, to_read3, to_write3, 10, 10, false,
      [&cl](ECExtentCache::OpRef& op) {
        cl.cache_ready(op->get_hoid(), op->get_result());
      });
  ...
  optional op4 = cl.cache.prepare(
      cl.oid, nullopt, to_write4, 10, 10, false,
      [&cl](ECExtentCache::OpRef& op) {
        cl.cache_ready(op->get_hoid(), op->get_result());
      });
  ...
  cl.complete_read();
  auto expected_read =
      iset_from_vector({{{10, 2}, {32, 6}}}, cl.get_stripe_info());
...
TEST(ECExtentCache, multiple_misaligned_writes)
{
  ...
  auto to_read1 =
      iset_from_vector({{{4 * 1024, 4 * 1024}}}, cl.get_stripe_info());
  ...
  auto to_read2 = iset_from_vector(
      {{{4 * 1024, 4 * 1024}, {12 * 4096, 4 * 4096}}}, cl.get_stripe_info());
  auto to_read2_exec =
      iset_from_vector({{{12 * 4096, 4 * 4096}}}, cl.get_stripe_info());
  auto to_write2 =
      iset_from_vector({{{4 * 1024, 12 * 1024}}}, cl.get_stripe_info());

  // IO 3 is the next misaligned 4k, very similar to IO 3.
  auto to_read3 = iset_from_vector(
      {{{12 * 1024, 4 * 1024}, {20 * 4096, 4 * 4096}}}, cl.get_stripe_info());
  auto to_read3_exec =
      iset_from_vector({{{20 * 4096, 4 * 4096}}}, cl.get_stripe_info());
  auto to_write3 =
      iset_from_vector({{{12 * 1024, 12 * 1024}}}, cl.get_stripe_info());

  //Perform the first write, which should result in a read.
  optional op1 = cl.cache.prepare(
      cl.oid, to_read1, to_write1, 22 * 1024, 22 * 1024, false,
      [&cl](ECExtentCache::OpRef& op) {
        cl.cache_ready(op->get_hoid(), op->get_result());
      });
  cl.cache_execute(*op1);
  ...
  optional op2 = cl.cache.prepare(
      cl.oid, to_read2, to_write2, 22 * 1024, 22 * 1024, false,
      [&cl](ECExtentCache::OpRef& op) {
        cl.cache_ready(op->get_hoid(), op->get_result());
      });
  cl.cache_execute(*op2);
  ...
  // And move on to op3
  optional op3 = cl.cache.prepare(
      cl.oid, to_read3, to_write3, 22 * 1024, 22 * 1024, false,
      [&cl](ECExtentCache::OpRef& op) {
        cl.cache_ready(op->get_hoid(), op->get_result());
      });
  cl.cache_execute(*op3);
...
TEST(ECExtentCache, multiple_misaligned_writes2)
{
  ...
  // first and last pages. The first part of this read should be in the cache.
  auto to_read2 = iset_from_vector(
      {{{4 * 1024, 4 * 1024}, {12 * 1024, 4 * 1024}}}, cl.get_stripe_info());
  auto to_read2_exec =
      iset_from_vector({{{12 * 1024, 4 * 1024}}}, cl.get_stripe_info());
  auto to_write2 =
      iset_from_vector({{{4 * 1024, 12 * 1024}}}, cl.get_stripe_info());

  // IO 3 is the next misaligned 4k, very similar to IO 3.
  auto to_read3 = iset_from_vector(
      {{{12 * 1024, 4 * 1024}, {20 * 1024, 4 * 1024}}}, cl.get_stripe_info());
  auto to_read3_exec =
      iset_from_vector({{{20 * 1024, 4 * 1024}}}, cl.get_stripe_info());
  auto to_write3 =
      iset_from_vector({{{12 * 1024, 12 * 1024}}}, cl.get_stripe_info());
...
TEST(ECExtentCache, test_invalidate)
{
  ...
  /* First attempt a write which does not do any reads */
  {
    ...
    optional op1 = cl.cache.prepare(
        cl.oid, to_read1, to_write1, 4096, 4096, false,
        [&cl](ECExtentCache::OpRef& op) {
          cl.cache_ready(op->get_hoid(), op->get_result());
        });
    cl.cache_execute(*op1);
    ...
    optional op2 = cl.cache.prepare(
        cl.oid, nullopt, shard_extent_set_t(cl.sinfo.get_k_plus_m()), 4 * 1024,
        0, false, [&cl](ECExtentCache::OpRef& op) {
          cl.cache_ready(op->get_hoid(), op->get_result());
        });
    cl.cache_execute(*op2);
  ...
  /* Second test, modifies, deletes, creates, then modifies.  */
  {
    ...
    optional op1 = cl.cache.prepare(
        cl.oid, to_read1, to_write1, 8192, 8192, false,
        [&cl](ECExtentCache::OpRef& op) {
          cl.cache_ready(op->get_hoid(), op->get_result());
        });
    optional op2 = cl.cache.prepare(
    ...
    optional op3 = cl.cache.prepare(
        cl.oid, nullopt, to_write2, 0, 8192, false,
        [&cl](ECExtentCache::OpRef& op) {
          cl.cache_ready(op->get_hoid(), op->get_result());
        });
    optional op4 = cl.cache.prepare(
        cl.oid, to_read3, to_write3, 8192, 8192, false,
        [&cl](ECExtentCache::OpRef& op) {
          cl.cache_ready(op->get_hoid(), op->get_result());
        });
    cl.cache_execute(*op1);
...
TEST(ECExtentCache, test_invalidate_lru)
{
  ...
  {
    uint64_t bs = 3767;
    auto io1 = iset_from_vector(
        {{{align_prev(35 * bs), align_next(36 * bs) - align_prev(35 * bs)}}},
        cl.get_stripe_info());
    io1[shard_id_t(k)].insert(io1.get_extent_superset());
    io1[shard_id_t(k + 1)].insert(io1.get_extent_superset());
    auto io2 = iset_from_vector(
        {{{align_prev(18 * bs), align_next(19 * bs) - align_prev(18 * bs)}}},
        cl.get_stripe_info());
    ...
    auto io4 = iset_from_vector(
        {{{align_prev(30 * bs), align_next(31 * bs) - align_prev(30 * bs)}}},
        cl.get_stripe_info());
    io3[shard_id_t(k)].insert(io1.get_extent_superset());
    io3[shard_id_t(k + 1)].insert(io1.get_extent_superset());
    auto io5 = iset_from_vector(
        {{{align_prev(18 * bs), align_next(19 * bs) - align_prev(18 * bs)}}},
        cl.get_stripe_info());
    ...
    optional op1 = cl.cache.prepare(
        cl.oid, nullopt, io1, 0, align_next(36 * bs), false,
        [&cl](ECExtentCache::OpRef& op) {
          cl.cache_ready(op->get_hoid(), op->get_result());
        });
    ...
    optional op2 = cl.cache.prepare(
        cl.oid, io2, io2, align_next(36 * bs), align_next(36 * bs), false,
        [&cl](ECExtentCache::OpRef& op) {
          cl.cache_ready(op->get_hoid(), op->get_result());
        });
    cl.cache_execute(*op2);
    ...
    optional op3 = cl.cache.prepare(
        cl.oid, nullopt, io3, align_next(36 * bs), 0, true,
        [&cl](ECExtentCache::OpRef& op) {
          cl.cache_ready(op->get_hoid(), op->get_result());
        });
    cl.cache_execute(*op3);
    ...
    optional op4 = cl.cache.prepare(
        cl.oid, nullopt, io4, 0, align_next(30 * bs), false,
        [&cl](ECExtentCache::OpRef& op) {
          cl.cache_ready(op->get_hoid(), op->get_result());
        });
    cl.cache_execute(*op4);
    ...
    optional op5 = cl.cache.prepare(
        cl.oid, io5, io5, align_next(30 * bs), align_next(30 * bs), false,
        [&cl](ECExtentCache::OpRef& op) {
          cl.cache_ready(op->get_hoid(), op->get_result());
        });
