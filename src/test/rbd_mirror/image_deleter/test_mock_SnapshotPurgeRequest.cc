...
public:
      ...
      int r)
  {
    EXPECT_CALL(mock_image_ctx, get_snap_namespace(snap_id, _))
        .WillOnce(WithArg<1>(Invoke([snap_namespace,
                                     r](cls::rbd::SnapshotNamespace* ns) {
          *ns = snap_namespace;
          return r;
        })));
  }
  ...
  void
  expect_start_op(librbd::MockTestImageCtx& mock_image_ctx, bool success)
  {
...
{
  {
    ...
    m_local_image_ctx->add_snap(
        cls::rbd::UserSnapshotNamespace{}, "snap1", 1, 0, {},
        RBD_PROTECTION_STATUS_PROTECTED, 0, {});
    m_local_image_ctx->add_snap(
        cls::rbd::UserSnapshotNamespace{}, "snap2", 2, 0, {},
  ...
  expect_get_snap_namespace(
      mock_image_ctx, 2, cls::rbd::UserSnapshotNamespace{}, 0);
  ...
  expect_snap_remove(
      mock_image_ctx, cls::rbd::UserSnapshotNamespace{}, "snap2", 0);

  expect_get_snap_namespace(
      mock_image_ctx, 1, cls::rbd::UserSnapshotNamespace{}, 0);
  expect_get_snap_name(mock_image_ctx, 1, "snap1", 0);
  expect_is_snap_protected(mock_image_ctx, 1, true, 0);
  expect_start_op(mock_image_ctx, true);
  expect_snap_unprotect(
      mock_image_ctx, cls::rbd::UserSnapshotNamespace{}, "snap1", 0);
  expect_start_op(mock_image_ctx, true);
  expect_snap_remove(
      mock_image_ctx, cls::rbd::UserSnapshotNamespace{}, "snap1", 0);
  ...
  C_SaferCond ctx;
  auto req =
      MockSnapshotPurgeRequest::create(m_local_io_ctx, mock_image_ctx.id, &ctx);
  req->send();
...
TEST_F(TestMockImageDeleterSnapshotPurgeRequest, SnapUnprotectBusy)
{
  ...
  expect_start_op(mock_image_ctx, true);
  expect_snap_unprotect(
      mock_image_ctx, cls::rbd::UserSnapshotNamespace{}, "snap1", -EBUSY);
...
TEST_F(TestMockImageDeleterSnapshotPurgeRequest, SnapUnprotectError)
{
  ...
  expect_start_op(mock_image_ctx, true);
  expect_snap_unprotect(
      mock_image_ctx, cls::rbd::UserSnapshotNamespace{}, "snap1", -EPERM);
...
TEST_F(TestMockImageDeleterSnapshotPurgeRequest, SnapRemoveError)
{
  ...
  expect_start_op(mock_image_ctx, true);
  expect_snap_remove(
      mock_image_ctx, cls::rbd::UserSnapshotNamespace{}, "snap1", -EINVAL);
