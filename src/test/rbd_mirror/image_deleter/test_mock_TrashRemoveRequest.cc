...
SnapshotPurgeRequest<librbd::MockTestImageCtx>*
    SnapshotPurgeRequest<librbd::MockTestImageCtx>::s_instance = nullptr;
...
public:
  ...
  typedef librbd::trash::RemoveRequest<librbd::MockTestImageCtx>
      MockLibrbdTrashRemoveRequest;

  void
  expect_trash_get(const cls::rbd::TrashImageSpec& trash_spec, int r)
  {
    using ceph::encode;
    EXPECT_CALL(
  ...
  void
  expect_trash_state_set(const std::string& image_id, int r)
  {
    bufferlist in_bl;
    ...
    EXPECT_CALL(
        get_mock_io_ctx(m_local_io_ctx),
        exec(
            StrEq(RBD_TRASH), _, StrEq("rbd"), StrEq("trash_state_set"),
            ContentsEqual(in_bl), _, _, _))
        .WillOnce(Return(r));
  }

  void
  expect_get_snapcontext(
      const std::string& image_id,
      ...
      int r)
  {
    ...
    EXPECT_CALL(
        get_mock_io_ctx(m_local_io_ctx),
        exec(
            librbd::util::header_name(image_id), _, StrEq("rbd"),
  ...
  void
  expect_snapshot_purge(
      MockSnapshotPurgeRequest& snapshot_purge_request,
      const std::string& image_id,
      int r)
  {
  ...
  void
  expect_image_remove(
      MockLibrbdTrashRemoveRequest& image_remove_request,
      const std::string& image_id,
      int r)
  {
...
TEST_F(TestMockImageDeleterTrashRemoveRequest, Success)
{
  ...
  auto req = MockTrashRemoveRequest::create(
      m_local_io_ctx, "image id", &error_result, m_threads->work_queue, &ctx);
  req->send();
