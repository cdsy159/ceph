...
struct TrashWatcher<MockTestImageCtx> {
  ...
  static TrashWatcher<MockTestImageCtx>&
  get_instance()
  {
    ceph_assert(s_instance != nullptr);
    return *s_instance;
  ...
  virtual void handle_image_added(
...
public:
  ...
  struct MockListener : TrashListener {
    MOCK_METHOD2(
        handle_trash_image,
        void(const std::string&, const ceph::real_clock::time_point&));
  };
  ...
  void
  expect_trash_watcher_is_unregistered(
      MockLibrbdTrashWatcher& mock_trash_watcher,
      bool unregistered)
  {
  ...
  void
  expect_trash_watcher_register(MockLibrbdTrashWatcher& mock_trash_watcher, int r)
  {
  ...
  void
  expect_trash_watcher_unregister(
      MockLibrbdTrashWatcher& mock_trash_watcher,
      int r)
  {
  ...
  void
  expect_create_trash(librados::IoCtx& io_ctx, int r)
  {
      ...
      int r)
  {
    ...
    EXPECT_CALL(
        ...
        exec(
            RBD_TRASH, _, StrEq("rbd"), StrEq("trash_list"), ContentsEqual(bl),
            _, _, _))
        .WillOnce(DoAll(
  ...
  int
  when_shut_down(MockTrashWatcher& mock_trash_watcher)
  {
    C_SaferCond ctx;
...
TEST_F(TestMockImageDeleterTrashWatcher, EmptyPool)
{
  ...
  MockTrashWatcher mock_trash_watcher(
      m_local_io_ctx, &mock_threads, mock_listener);
  C_SaferCond ctx;
