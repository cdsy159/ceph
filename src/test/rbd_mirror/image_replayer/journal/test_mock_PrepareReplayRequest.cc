...
class TestMockImageReplayerJournalPrepareReplayRequest
...
public:
  ...
  void
  expect_journal_get_tag_tid(librbd::MockJournal& mock_journal, uint64_t tag_tid)
  {
      ...
      int r)
  {
    EXPECT_CALL(mock_journal, is_resync_requested(_))
        .WillOnce(DoAll(SetArgPointee<0>(do_resync), Return(r)));
  }
...
TEST_F(TestMockImageReplayerJournalPrepareReplayRequest, Success)
{
  ...
  expect_journaler_get_client(
      mock_remote_journaler, librbd::Journal<>::IMAGE_CLIENT_ID, client, 0);
      ...
      {2, 123,
       encode_tag_data(
           {librbd::Journal<>::LOCAL_MIRROR_UUID,
            librbd::Journal<>::LOCAL_MIRROR_UUID, true, 344, 99})},
  };
  ...
  MockStateBuilder mock_state_builder(
      mock_local_image_ctx, mock_remote_journaler, mirror_peer_client_meta);
  bool resync_requested;
  bool syncing;
  auto request = create_request(
      mock_state_builder, "local mirror uuid", &resync_requested, &syncing,
      &ctx);
  request->send();
...
TEST_F(TestMockImageReplayerJournalPrepareReplayRequest, GetRemoteTagClassError)
{
  ...
  ::journal::MockJournaler mock_remote_journaler;
  expect_journaler_get_client(
      mock_remote_journaler, librbd::Journal<>::IMAGE_CLIENT_ID, client,
      -EINVAL);
    ...
    LocalDemotedRemoteSyncingState)
{
  ...
  expect_journal_get_tag_tid(mock_journal, 345);
  expect_journal_get_tag_data(
      mock_journal, {librbd::Journal<>::ORPHAN_MIRROR_UUID,
  ...
  Tags tags = {
      {2, 123,
       encode_tag_data(
           {librbd::Journal<>::LOCAL_MIRROR_UUID,
            librbd::Journal<>::LOCAL_MIRROR_UUID, true, 1, 99})},
      {3, 123,
       encode_tag_data(
           {librbd::Journal<>::ORPHAN_MIRROR_UUID,
            librbd::Journal<>::LOCAL_MIRROR_UUID, true, 2, 1})},
      {4, 123,
       encode_tag_data(
           {librbd::Journal<>::LOCAL_MIRROR_UUID,
            librbd::Journal<>::ORPHAN_MIRROR_UUID, true, 3, 1})},
      {5, 123,
       encode_tag_data(
           {librbd::Journal<>::ORPHAN_MIRROR_UUID,
            librbd::Journal<>::LOCAL_MIRROR_UUID, true, 4, 1})},
      {6, 123,
       encode_tag_data(
           {librbd::Journal<>::LOCAL_MIRROR_UUID,
            librbd::Journal<>::ORPHAN_MIRROR_UUID, true, 5, 1})},
      {7, 123,
       encode_tag_data(
           {librbd::Journal<>::ORPHAN_MIRROR_UUID,
            librbd::Journal<>::LOCAL_MIRROR_UUID, true, 6, 1})},
      {8, 123,
       encode_tag_data(
           {librbd::Journal<>::LOCAL_MIRROR_UUID,
            librbd::Journal<>::ORPHAN_MIRROR_UUID, true, 7, 1})}};
  expect_journaler_get_tags(mock_remote_journaler, 123, tags, 0);
...
TEST_F(TestMockImageReplayerJournalPrepareReplayRequest, RemoteDemotePromote)
{
  ...
  Tags tags = {
      ...
      {4, 123,
       encode_tag_data(
           {librbd::Journal<>::LOCAL_MIRROR_UUID,
            librbd::Journal<>::ORPHAN_MIRROR_UUID, true, 2, 1})},
      {5, 123,
       encode_tag_data(
           {librbd::Journal<>::LOCAL_MIRROR_UUID,
            librbd::Journal<>::ORPHAN_MIRROR_UUID, true, 4, 369})}};
  expect_journaler_get_tags(mock_remote_journaler, 123, tags, 0);
...
TEST_F(TestMockImageReplayerJournalPrepareReplayRequest, LocalDemoteRemotePromote)
{
  ...
  expect_journal_get_tag_data(
      mock_journal, {librbd::Journal<>::ORPHAN_MIRROR_UUID,
                     librbd::Journal<>::LOCAL_MIRROR_UUID, true, 345, 1});
  ...
  Tags tags = {
      {2, 123,
       encode_tag_data(
           {"local mirror uuid", "local mirror uuid", true, 344, 99})},
      {3, 123,
       encode_tag_data(
           {librbd::Journal<>::ORPHAN_MIRROR_UUID, "local mirror uuid", true,
            345, 1})},
      {4, 123,
       encode_tag_data(
           {librbd::Journal<>::LOCAL_MIRROR_UUID,
...
TEST_F(TestMockImageReplayerJournalPrepareReplayRequest, SplitBrainForcePromote)
{
  ...
  expect_journal_get_tag_tid(mock_journal, 345);
  expect_journal_get_tag_data(
      mock_journal, {librbd::Journal<>::LOCAL_MIRROR_UUID,
                     librbd::Journal<>::ORPHAN_MIRROR_UUID, true, 344, 0});
