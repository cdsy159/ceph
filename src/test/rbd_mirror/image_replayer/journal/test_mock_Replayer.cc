...
struct MockTestJournal : public MockJournal {
  MOCK_METHOD2(
      ...
      void(journal::Replay<MockTestImageCtx>**, Context* on_start));
...
struct EventPreprocessor<librbd::MockTestImageCtx> {
  ...
  static void
  destroy(EventPreprocessor* processor)
  {}
...
struct ReplayStatusFormatter<librbd::MockTestImageCtx> {
  ...
  static ReplayStatusFormatter*
  create(::journal::MockJournaler* journaler, const std::string& mirror_uuid)
  {
    ceph_assert(s_instance != nullptr);
    return s_instance;
  }
  ...
  static void
  destroy(ReplayStatusFormatter* formatter)
  {}
  ...
  MOCK_METHOD2(
      get_or_send_update,
      bool(std::string* description, Context* on_finish));
};
...
EventPreprocessor<librbd::MockTestImageCtx>*
    EventPreprocessor<librbd::MockTestImageCtx>::s_instance = nullptr;
ReplayStatusFormatter<librbd::MockTestImageCtx>*
    ReplayStatusFormatter<librbd::MockTestImageCtx>::s_instance = nullptr;
...
public:
  ...
  typedef ReplayStatusFormatter<librbd::MockTestImageCtx>
  ...
  bufferlist
  encode_tag_data(const librbd::journal::TagData& tag_data)
  {
    bufferlist bl;
    encode(tag_data, bl);
    return bl;
  ...
  void
  expect_init(::journal::MockJournaler& mock_journaler, int r)
  {
  ...
  void
  expect_stop_replay(::journal::MockJournaler& mock_journaler, int r)
  {
    EXPECT_CALL(mock_journaler, stop_replay(_)).WillOnce(CompleteContext(r));
  }

  void
  expect_shut_down(MockReplay& mock_replay, bool cancel_ops, int r)
  {
  ...
  expect_start_external_replay(
      librbd::MockTestJournal& mock_journal,
      MockReplay* mock_replay,
      int r)
  {
    EXPECT_CALL(mock_journal, start_external_replay(_, _))
        .WillOnce(DoAll(
            SetArgPointee<0>(mock_replay),
            WithArg<1>(CompleteContext(m_threads->work_queue, r))));
  }

  void
  expect_is_tag_owner(librbd::MockTestJournal& mock_journal, bool is_owner)
  {
    EXPECT_CALL(mock_journal, is_tag_owner()).WillOnce(Return(is_owner));
  }

  void
  expect_is_resync_requested(
      librbd::MockTestJournal& mock_journal,
      int r,
  ...
  void
  expect_get_commit_tid_in_debug(::journal::MockReplayEntry& mock_replay_entry)
  {
  ...
  void
  expect_get_tag_tid_in_debug(librbd::MockTestJournal& mock_journal)
  {
    // It is used in debug messages and depends on debug level
    EXPECT_CALL(mock_journal, get_tag_tid())
        .Times(AtLeast(0))
        .WillRepeatedly(Return(0));
  }
  ...
  expect_try_pop_front(
      ::journal::MockJournaler& mock_journaler,
      uint64_t replay_tag_tid,
      bool entries_available)
  {
    EXPECT_CALL(mock_journaler, try_pop_front(_, _))
        .WillOnce(DoAll(
            SetArgPointee<0>(::journal::MockReplayEntryProxy()),
            SetArgPointee<1>(replay_tag_tid), Return(entries_available)));
  }

  void
  expect_try_pop_front_return_no_entries(
      ::journal::MockJournaler& mock_journaler,
      Context* on_finish)
  {
    EXPECT_CALL(mock_journaler, try_pop_front(_, _))
        .WillOnce(DoAll(
            Invoke([on_finish](::journal::MockReplayEntryProxy* e, uint64_t* t) {
  ...
  void
  expect_allocate_tag(librbd::MockTestJournal& mock_journal, int r)
  {
  ...
  void
  expect_process(MockReplay& mock_replay, int on_ready_r, int on_commit_r)
  {
    EXPECT_CALL(mock_replay, process(_, _, _))
        .WillOnce(DoAll(
            WithArg<1>(CompleteContext(on_ready_r)),
            WithArg<2>(CompleteContext(on_commit_r))));
  }

  void
  expect_flush(MockReplay& mock_replay, int r)
  {
  ...
  void
  expect_flush_commit_position(::journal::MockJournaler& mock_journal, int r)
  {
  ...
  void
  expect_local_journal_add_listener(
      librbd::MockTestJournal& mock_local_journal,
      ...
      ::journal::JournalMetadataListener** remote_journal_listener)
  {
    ...
    expect_get_cached_client(
        mock_remote_journaler, "local mirror uuid", {},
        {librbd::journal::MirrorPeerClientMeta{}}, 0);
    expect_start_external_replay(
        mock_local_journal, &mock_local_journal_replay, 0);
    expect_local_journal_add_listener(
        mock_local_journal, local_journal_listener);
...
TEST_F(TestMockImageReplayerJournalReplayer, InitShutDown)
{
  ...
  librbd::MockTestImageCtx mock_local_image_ctx{
      *m_local_image_ctx, mock_local_journal};
  ::journal::MockJournaler mock_remote_journaler;
  MockReplayerListener mock_replayer_listener;
  ...
  ASSERT_EQ(
      0, init_entry_replayer(
             mock_replayer, mock_threads, mock_replayer_listener,
             mock_local_journal, mock_remote_journaler,
             mock_local_journal_replay, &local_journal_listener,
             &remote_replay_handler, &remote_journaler_listener));

  ASSERT_EQ(
      0, shut_down_entry_replayer(
             mock_replayer, mock_threads, mock_local_journal,
             mock_remote_journaler, mock_local_journal_replay));
}
...
TEST_F(TestMockImageReplayerJournalReplayer, InitDisconnected)
{
  ...
  librbd::MockTestImageCtx mock_local_image_ctx{
      *m_local_image_ctx, mock_local_journal};
  mock_local_image_ctx.config.set_val(
  ...
  expect_get_cached_client(
      ...
      {librbd::journal::MirrorPeerClientMeta{mock_local_image_ctx.id}}, 0);
  MockCloseImageRequest mock_close_image_request;
  expect_send(mock_close_image_request, 0);
  EXPECT_CALL(mock_remote_journaler, remove_listener(_));
  ...
  cls::journal::Tag tag = {
      ...
      encode_tag_data(
          {librbd::Journal<>::LOCAL_MIRROR_UUID,
           librbd::Journal<>::LOCAL_MIRROR_UUID, true, 0, 0})};
...
TEST_F(TestMockImageReplayerJournalReplayer, DelayedReplay)
{
  ...
  EXPECT_CALL(mock_local_journal_replay, decode(_, _))
      .WillOnce(DoAll(SetArgPointee<1>(event_entry), Return(0)));

  Context* delayed_task_ctx = nullptr;
  EXPECT_CALL(*mock_threads.timer, add_event_after(_, _))
      .WillOnce(DoAll(
          Invoke([this, &delayed_task_ctx](double seconds, Context* ctx) {
            std::unique_lock locker{m_lock};
