...
class TestMockImageReplayerSnapshotApplyImageStateRequest
  : public TestMockFixture {
public:
  typedef ApplyImageStateRequest<librbd::MockTestImageCtx>
      MockApplyImageStateRequest;
  typedef librbd::image::GetMetadataRequest<librbd::MockTestImageCtx>
  ...
  void
  expect_rename_image(const std::string& name, int r)
  {
  ...
  void
  expect_update_features(uint64_t features, bool enable, int r)
  {
    EXPECT_CALL(
        *m_mock_local_image_ctx->operations,
      ...
      int r)
  {
    for (auto& key : remove) {
      ...
      ceph::encode(key, bl);
      EXPECT_CALL(
          get_mock_io_ctx(m_mock_local_image_ctx->md_ctx),
          exec(
              m_mock_local_image_ctx->header_oid, _, StrEq("rbd"),
  ...
  void
  expect_unprotect_snapshot(const std::string& name, int r)
  {
    EXPECT_CALL(
        *m_mock_local_image_ctx->operations,
  ...
  void
  expect_remove_snapshot(const std::string& name, int r)
  {
    EXPECT_CALL(
        *m_mock_local_image_ctx->operations,
  ...
  void
  expect_protect_snapshot(const std::string& name, int r)
  {
    EXPECT_CALL(
        *m_mock_local_image_ctx->operations,
  ...
  void
  expect_rename_snapshot(uint64_t snap_id, const std::string& name, int r)
  {
    EXPECT_CALL(
        *m_mock_local_image_ctx->operations,
  ...
  void
  expect_set_snap_limit(uint64_t limit, int r)
  {
    EXPECT_CALL(
        *m_mock_local_image_ctx->operations, execute_snap_set_limit(limit, _))
        .WillOnce(WithArg<1>(Invoke([this, r](Context* ctx) {
          m_threads->work_queue->queue(ctx, r);
        })));
  }
...
TEST_F(TestMockImageReplayerSnapshotApplyImageStateRequest, UpdateImageMeta)
{
  ...
  MockGetMetadataRequest mock_get_metadata_request;
  expect_get_metadata(
      mock_get_metadata_request, {{"key1", {}}, {"key2", {}}}, 0);
...
TEST_F(TestMockImageReplayerSnapshotApplyImageStateRequest, GetImageMetaError)
{
  ...
  MockGetMetadataRequest mock_get_metadata_request;
  expect_get_metadata(
      mock_get_metadata_request, {{"key1", {}}, {"key2", {}}}, -EINVAL);
...
TEST_F(
    TestMockImageReplayerSnapshotApplyImageStateRequest,
    UnprotectSnapshotError)
{
  InSequence seq;
...
TEST_F(TestMockImageReplayerSnapshotApplyImageStateRequest, RenameSnapshot)
{
  ...
  image_state.snapshots = {
      {1U,
       {cls::rbd::UserSnapshotNamespace{}, "snap1-renamed",
        RBD_PROTECTION_STATUS_PROTECTED}}};
  m_mock_local_image_ctx->snap_info = {
      {11U,
       librbd::SnapInfo{
