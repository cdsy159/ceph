...
class TestMockImageReplayerSnapshotCreateLocalImageRequest
...
public:
  ...
  void
  expect_mirror_image_remove(const std::string& image_id, int r)
  {
    bufferlist bl;
    encode(image_id, bl);
    ...
    EXPECT_CALL(
        ...
        exec(
            ...
            StrEq("mirror_image_remove"), ContentsEqual(bl), _, _, _))
        .WillOnce(Return(r));
  }
      ...
      Context* on_finish)
  {
    return new MockCreateLocalImageRequest(
        ...
        &m_pool_meta_cache, nullptr, &mock_state_builder, on_finish);
  }
...
TEST_F(
    TestMockImageReplayerSnapshotCreateLocalImageRequest,
    DisableMirrorImageError)
{
  InSequence seq;
