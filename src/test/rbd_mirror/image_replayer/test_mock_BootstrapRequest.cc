...
struct OpenImageRequest<librbd::MockTestImageCtx> {
  ...
  MOCK_METHOD2(
      construct,
      void(librados::IoCtx& io_ctx, const std::string& image_id));
  MOCK_METHOD0(send, void());
};
...
struct PrepareLocalImageRequest<librbd::MockTestImageCtx> {
  ...
  static PrepareLocalImageRequest*
  create(
      librados::IoCtx&,
...
struct PrepareRemoteImageRequest<librbd::MockTestImageCtx> {
  ...
  static PrepareRemoteImageRequest*
  create(
      Threads<librbd::MockTestImageCtx>* threads,
      librados::IoCtx&,
      librados::IoCtx&,
...
struct StateBuilder<librbd::MockTestImageCtx> {
  ...
  image_sync::MockSyncPointHandler*
  ...
  MOCK_METHOD6(
      ...
      BaseRequest*(
          Threads<librbd::MockTestImageCtx>*,
          librados::IoCtx&,
          const std::string&,
          PoolMetaCache*,
          ProgressContext*,
...
public:
  ...
  typedef OpenLocalImageRequest<librbd::MockTestImageCtx>
      MockOpenLocalImageRequest;
  typedef PrepareLocalImageRequest<librbd::MockTestImageCtx>
      MockPrepareLocalImageRequest;
  typedef PrepareRemoteImageRequest<librbd::MockTestImageCtx>
  ...
  void
  expect_send(
      MockPrepareLocalImageRequest& mock_request,
  ...
  void
  expect_send(
      MockPrepareRemoteImageRequest& mock_request,
      MockStateBuilder& mock_state_builder,
  ...
  {
    EXPECT_CALL(mock_request, send())
        .WillOnce(Invoke([&mock_request, &mock_state_builder,
                          remote_mirror_uuid, remote_image_id, r]() {
          if (r >= 0) {
            *mock_request.state_builder = &mock_state_builder;
  ...
  void
  expect_is_local_primary(MockStateBuilder& mock_state_builder, bool is_primary)
  {
  ...
  void
  expect_is_remote_primary(MockStateBuilder& mock_state_builder, bool is_primary)
  {
  ...
  void
  expect_is_linked(MockStateBuilder& mock_state_builder, bool is_linked)
  {
  ...
  void
  expect_replay_requires_remote_image(
      MockStateBuilder& mock_state_builder,
      bool requires_image)
  {
  ...
  {
    EXPECT_CALL(
        mock_open_image_request, construct(IsSameIoCtx(&io_ctx), image_id));
      ...
      int r)
  {
    ...
    EXPECT_CALL(mock_open_local_image_request, send())
        .WillOnce(Invoke([this, &mock_open_local_image_request, mock_image_ctx,
                          r]() {
          if (r >= 0) {
            *mock_open_local_image_request.image_ctx = mock_image_ctx;
          }
          m_threads->work_queue->queue(
              mock_open_local_image_request.on_finish, r);
        }));
  }

  void
  expect_close_remote_image(MockStateBuilder& mock_state_builder, int r)
  {
  ...
  void
  expect_create_local_image(
      MockStateBuilder& mock_state_builder,
      const std::string& local_image_id,
      int r)
  {
    EXPECT_CALL(mock_state_builder, create_local_image_request(_, _, _, _, _, _))
        .WillOnce(WithArg<5>(Invoke([&mock_state_builder, local_image_id,
                                     r](Context* ctx) {
          if (r >= 0) {
            mock_state_builder.local_image_id = local_image_id;
          }
      ...
      int r)
  {
    EXPECT_CALL(mock_state_builder, create_prepare_replay_request(_, _, _, _, _))
        .WillOnce(WithArgs<2, 3, 4>(
            Invoke([&mock_state_builder, resync_requested, syncing,
                    r](bool* resync, bool* sync, Context* ctx) {
              if (r >= 0) {
  ...
  void
  expect_image_sync(MockImageSync& mock_image_sync, int r)
  {
  ...
  create_request(
      ...
      Context* on_finish)
  {
    return new MockBootstrapRequest(
        mock_threads, m_local_io_ctx, m_remote_io_ctx, mock_instance_watcher,
        global_image_id, local_mirror_uuid,
        {"remote mirror uuid", "remote mirror peer uuid"}, nullptr, nullptr,
        nullptr, &m_mock_state_builder, &m_do_resync, on_finish);
  }
...
TEST_F(TestMockImageReplayerBootstrapRequest, Success)
{
  ...
  expect_send(
      mock_prepare_local_image_request, mock_state_builder,
  ...
  MockOpenImageRequest mock_open_image_request;
  expect_open_image(
      mock_open_image_request, m_remote_io_ctx, mock_remote_image_ctx.id,
      mock_remote_image_ctx, 0);

  // open the local image
  librbd::MockTestImageCtx mock_local_image_ctx(*m_local_image_ctx);
  MockOpenLocalImageRequest mock_open_local_image_request;
  expect_open_local_image(
      mock_open_local_image_request, m_local_io_ctx, mock_local_image_ctx.id,
      &mock_local_image_ctx, 0);
...
TEST_F(
    TestMockImageReplayerBootstrapRequest,
    PrepareRemoteImageDNELocalLinkedCanceled)
{
  InSequence seq;
...
TEST_F(TestMockImageReplayerBootstrapRequest, OpenLocalImageDNE)
{
  ...
  expect_open_local_image(
      mock_open_local_image_request, m_local_io_ctx, "local image id",
      &mock_local_image_ctx, 0);
...
TEST_F(TestMockImageReplayerBootstrapRequest, OpenLocalImagePrimary)
{
  ...
  expect_open_local_image(
      mock_open_local_image_request, m_local_io_ctx, mock_local_image_ctx.id,
      &mock_local_image_ctx, -EREMOTEIO);
...
TEST_F(TestMockImageReplayerBootstrapRequest, CreateLocalImageError)
{
  ...
  MockStateBuilder mock_state_builder;
  expect_send(
      mock_prepare_local_image_request, mock_state_builder, "", "", -ENOENT);
