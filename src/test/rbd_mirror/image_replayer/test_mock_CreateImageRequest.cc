...
template class rbd::mirror::image_replayer::CreateImageRequest<
...
public:
  ...
  typedef librbd::image::CreateRequest<librbd::MockTestImageCtx>
      MockCreateRequest;
  ...
  void
  snap_create(librbd::ImageCtx* image_ctx, const std::string& snap_name)
  {
  ...
  int
  clone_image(
      librbd::ImageCtx* parent_image_ctx,
      ...
      const std::string& clone_name)
  {
    ...
    int order = 0;
    return librbd::clone(
        m_remote_io_ctx, parent_image_ctx->name.c_str(), snap_name.c_str(),
        m_remote_io_ctx, clone_name.c_str(), parent_image_ctx->features, &order,
        0, 0);
  }
  ...
  void
  expect_ioctx_create(librados::IoCtx& io_ctx)
  {
    librados::MockTestMemIoCtxImpl& io_ctx_impl = get_mock_io_ctx(io_ctx);
    EXPECT_CALL(
        *get_mock_io_ctx(io_ctx).get_mock_rados_client(), create_ioctx(_, _))
        .WillOnce(DoAll(
            GetReference(&io_ctx_impl), Return(&get_mock_io_ctx(io_ctx))));
  }

  void
  expect_get_parent_global_image_id(
      librados::IoCtx& io_ctx,
      ...
      int r)
  {
    ...
    EXPECT_CALL(
        get_mock_io_ctx(io_ctx), exec(
                                     ...
                                     StrEq("mirror_image_get"), _, _, _, _))
        .WillOnce(DoAll(
  ...
  void
  expect_mirror_image_get_image_id(
      librados::IoCtx& io_ctx,
      const std::string& image_id,
      int r)
  {
  ...
  void
  expect_test_op_features(librbd::MockTestImageCtx& mock_image_ctx, bool enabled)
  {
    EXPECT_CALL(
        mock_image_ctx, test_op_features(RBD_OPERATION_FEATURE_CLONE_CHILD))
        .WillOnce(Return(enabled));
  }

  void
  expect_clone_image(MockCloneRequest& mock_clone_request, int r)
  {
  ...
  create_request(
      ...
      Context* on_finish)
  {
    return new MockCreateImageRequest(
        mock_threads, m_local_io_ctx, global_image_id, remote_mirror_uuid,
        local_image_name, local_image_id, &mock_remote_image_ctx,
        &m_pool_meta_cache, cls::rbd::MIRROR_IMAGE_MODE_JOURNAL, on_finish);
  }
...
TEST_F(TestMockImageReplayerCreateImageRequest, Create)
{
  ...
  MockCreateImageRequest* request = create_request(
      &mock_threads, "global uuid", "remote uuid", "image name", "101241a7c4c9",
...
TEST_F(TestMockImageReplayerCreateImageRequest, CloneGetGlobalImageIdError)
{
  ...
  ASSERT_EQ(
      0, open_image(m_remote_io_ctx, clone_image_name, &remote_clone_image_ctx));
  ...
  MockCreateImageRequest* request = create_request(
      &mock_threads, "global uuid", "remote uuid", "image name", "101241a7c4c9",
      mock_remote_clone_image_ctx, &ctx);
  request->send();
...
TEST_F(TestMockImageReplayerCreateImageRequest, CloneOpenRemoteParentError)
{
  ...
  expect_open_image(
      mock_open_image_request, m_remote_io_ctx, m_remote_image_ctx->id,
      mock_remote_parent_image_ctx, -ENOENT);
...
TEST_F(TestMockImageReplayerCreateImageRequest, CloneParentImageSyncing)
{
  ...
  expect_open_image(
      mock_open_image_request, m_remote_io_ctx, m_remote_image_ctx->id,
      mock_remote_parent_image_ctx, 0);
...
TEST_F(TestMockImageReplayerCreateImageRequest, CloneRemoteParentCloseError)
{
  ...
  expect_close_image(
      mock_close_image_request, mock_remote_parent_image_ctx, -EINVAL);
