...
public:
      ...
      int r)
  {
    ...
    EXPECT_CALL(
        ...
        exec(RBD_DIRECTORY, _, StrEq("rbd"), StrEq("dir_get_name"), _, _, _, _))
        .WillOnce(DoAll(
...
TEST_F(TestMockImageReplayerPrepareLocalImageRequest, SuccessJournal)
{
  ...
  MockGetMirrorImageIdRequest mock_get_mirror_image_id_request;
  expect_get_mirror_image_id(
      mock_get_mirror_image_id_request, "local image id", 0);
  ...
  auto req = MockPrepareLocalImageRequest::create(
      m_local_io_ctx, "global image id", &local_image_name, &mock_state_builder,
  ...
  ASSERT_EQ(
      std::string("local image id"), mock_journal_state_builder.local_image_id);
  ASSERT_EQ(
      cls::rbd::MIRROR_IMAGE_MODE_JOURNAL,
...
TEST_F(TestMockImageReplayerPrepareLocalImageRequest, MirrorImageInfoError)
{
  ...
  expect_get_mirror_info(
      ...
      {cls::rbd::MIRROR_IMAGE_MODE_JOURNAL, "global image id",
       cls::rbd::MIRROR_IMAGE_STATE_ENABLED},
      librbd::mirror::PROMOTION_STATE_NON_PRIMARY, "remote mirror uuid",
      -EINVAL);
