...
GetMirrorImageIdRequest<librbd::MockTestImageCtx>*
    GetMirrorImageIdRequest<librbd::MockTestImageCtx>::s_instance = nullptr;
...
public:
  ...
  typedef GetMirrorImageIdRequest<librbd::MockTestImageCtx>
      ...
      int r)
  {
    EXPECT_CALL(mock_journaler, get_client(StrEq(client_id), _, _))
        .WillOnce(DoAll(
            WithArg<1>(Invoke([client](cls::journal::Client* out_client) {
              *out_client = client;
            })),
...
TEST_F(TestMockImageReplayerPrepareRemoteImageRequest, SuccessJournal)
{
  ...
  MockGetMirrorImageIdRequest mock_get_mirror_image_id_request;
  expect_get_mirror_image_id(
      mock_get_mirror_image_id_request, "remote image id", 0);
  ...
  expect_journaler_get_client(
      mock_remote_journaler, "local mirror uuid", client, 0);

  MockJournalStateBuilder mock_journal_state_builder;
  MockStateBuilder* mock_state_builder = nullptr;
  C_SaferCond ctx;
  auto req = MockPrepareRemoteImageRequest::create(
      &mock_threads, m_local_io_ctx, m_remote_io_ctx, "global image id",
      "local mirror uuid", {"remote mirror uuid", ""}, nullptr,
      &mock_state_builder, &ctx);
  req->send();
  ...
  ASSERT_EQ(
      std::string("remote mirror uuid"),
      mock_journal_state_builder.remote_mirror_uuid);
  ASSERT_EQ(
      std::string("remote image id"),
...
TEST_F(TestMockImageReplayerPrepareRemoteImageRequest, SuccessSnapshot)
{
  ...
  auto req = MockPrepareRemoteImageRequest::create(
      &mock_threads, m_local_io_ctx, m_remote_io_ctx, "global image id",
      "local mirror uuid", {"remote mirror uuid", "remote mirror peer uuid"},
      nullptr, &mock_state_builder, &ctx);
  req->send();
  ...
  ASSERT_EQ(
      std::string("remote mirror peer uuid"),
...
TEST_F(TestMockImageReplayerPrepareRemoteImageRequest, SuccessNotRegistered)
{
  ...
  MockJournalStateBuilder mock_journal_state_builder;
  expect_get_mirror_image_mode(
      mock_journal_state_builder, cls::rbd::MIRROR_IMAGE_MODE_JOURNAL);

  EXPECT_CALL(mock_remote_journaler, construct());

  cls::journal::Client client;
  expect_journaler_get_client(
      mock_remote_journaler, "local mirror uuid", client, -ENOENT);
...
TEST_F(TestMockImageReplayerPrepareRemoteImageRequest, GetMirrorInfoError)
{
  ...
  expect_get_mirror_info(
      ...
      {cls::rbd::MIRROR_IMAGE_MODE_JOURNAL, "global image id",
       cls::rbd::MIRROR_IMAGE_STATE_ENABLED},
      librbd::mirror::PROMOTION_STATE_PRIMARY, "remote mirror uuid", -EINVAL);
...
TEST_F(TestMockImageReplayerPrepareRemoteImageRequest, GetClientError)
{
  ...
  cls::journal::Client client;
  expect_journaler_get_client(
      mock_remote_journaler, "local mirror uuid", client, -EINVAL);
...
TEST_F(TestMockImageReplayerPrepareRemoteImageRequest, MirrorImageIdDNESnapshot)
{
  ...
  ASSERT_EQ(
      ...
      mock_snapshot_state_builder.mirror_image_mode);
  ASSERT_EQ(
      "remote mirror uuid", mock_snapshot_state_builder.remote_mirror_uuid);
  ASSERT_EQ(
...
TEST_F(TestMockImageReplayerPrepareRemoteImageRequest, MirrorInfoDNEJournal)
{
  ...
  expect_get_mirror_info(
      ...
      {cls::rbd::MIRROR_IMAGE_MODE_JOURNAL, "global image id",
       cls::rbd::MIRROR_IMAGE_STATE_ENABLED},
      librbd::mirror::PROMOTION_STATE_PRIMARY, "remote mirror uuid", -ENOENT);
...
TEST_F(
    TestMockImageReplayerPrepareRemoteImageRequest,
    MirrorInfoDisablingSnapshot)
{
  ...
  MockSnapshotStateBuilder mock_snapshot_state_builder;
  expect_get_mirror_image_mode(
      mock_snapshot_state_builder, cls::rbd::MIRROR_IMAGE_MODE_SNAPSHOT);
  MockStateBuilder* mock_state_builder = &mock_snapshot_state_builder;
  C_SaferCond ctx;
  auto req = MockPrepareRemoteImageRequest::create(
      &mock_threads, m_local_io_ctx, m_remote_io_ctx, "global image id",
