...
template class rbd::mirror::image_sync::SyncPointPruneRequest<
...
public:
  typedef SyncPointPruneRequest<librbd::MockTestImageCtx>
      ...
      int r)
  {
    EXPECT_CALL(mock_sync_point_handler, update_sync_points(_, _, complete, _))
        .WillOnce(DoAll(
            WithArg<1>(Invoke([this, r](const SyncPoints& sync_points) {
              if (r >= 0) {
  ...
  {
    EXPECT_CALL(
        *mock_remote_image_ctx.operations, snap_remove(_, StrEq(snap_name), _))
  ...
  create_request(
      ...
      Context* ctx)
  {
    return new MockSyncPointPruneRequest(
        &mock_remote_image_ctx, sync_complete, &mock_sync_point_handler, ctx);
  }
...
TEST_F(TestMockImageSyncSyncPointPruneRequest, SyncInProgressSuccess)
{
  ...
  C_SaferCond ctx;
  MockSyncPointPruneRequest* req = create_request(
      mock_remote_image_ctx, mock_sync_point_handler, false, &ctx);
  req->send();
...
TEST_F(
    TestMockImageSyncSyncPointPruneRequest,
    SyncInProgressUnexpectedFromSnapSuccess)
{
  m_sync_points.emplace_front(
      cls::rbd::UserSnapshotNamespace(), "snap2", "snap1", boost::none);
...
TEST_F(TestMockImageSyncSyncPointPruneRequest, SyncCompleteSuccess)
{
  ...
  C_SaferCond ctx;
  MockSyncPointPruneRequest* req = create_request(
      mock_remote_image_ctx, mock_sync_point_handler, true, &ctx);
  req->send();
  ASSERT_EQ(0, ctx.wait());
...
TEST_F(
    TestMockImageSyncSyncPointPruneRequest,
    RestartedCatchUpSyncCompleteSuccess)
{
  m_sync_points.emplace_front(
      cls::rbd::UserSnapshotNamespace(), "snap3", "snap2", boost::none);
  m_sync_points.emplace_front(
      cls::rbd::UserSnapshotNamespace(), "snap2", "snap1", boost::none);
  auto sync_points = m_sync_points;
