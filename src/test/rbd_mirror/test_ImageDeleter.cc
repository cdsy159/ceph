...
public:
  ...
  SetUp() override
  {
    ...
    m_image_deletion_throttler.reset(new rbd::mirror::Throttler<>(
        g_ceph_context, "rbd_mirror_concurrent_image_deletions"));

    m_service_daemon.reset(
        new rbd::mirror::ServiceDaemon<>(g_ceph_context, _rados, m_threads));
    ...
    EXPECT_EQ(
        0, librbd::create(
               m_local_io_ctx, m_image_name, m_local_image_id, 1 << 20,
               image_opts, GLOBAL_IMAGE_ID, m_remote_mirror_uuid, true));

    cls::rbd::MirrorImage mirror_image(
        MirrorImageMode::MIRROR_IMAGE_MODE_JOURNAL, GLOBAL_IMAGE_ID,
        MirrorImageState::MIRROR_IMAGE_STATE_ENABLED);
    EXPECT_EQ(
        0, cls_client::mirror_image_set(
               &m_local_io_ctx, m_local_image_id, mirror_image));
  }
  ...
  void
  promote_image(ImageCtx* ictx = nullptr)
  {
    bool close = false;
    int r = 0;
    if (!ictx) {
  ...
  void
  demote_image(ImageCtx* ictx = nullptr)
  {
    bool close = false;
    if (!ictx) {
      ictx = new ImageCtx("", m_local_image_id, "", m_local_io_ctx, false);
  ...
  void
  create_snapshot(std::string snap_name = "snap1", bool protect = false)
  {
    ImageCtx* ictx =
        new ImageCtx("", m_local_image_id, "", m_local_io_ctx, false);
    EXPECT_EQ(0, ictx->state->open(0));
    {
  ...
  create_clone()
  {
    ...
    EXPECT_EQ(
        0, librbd::clone(
               m_local_io_ctx, m_local_image_id.c_str(), nullptr, CEPH_NOSNAP,
               "snap1", m_local_io_ctx, clone_id.c_str(), "clone1", clone_opts,
    ...
    EXPECT_EQ(
        0,
        cls_client::mirror_image_set(&m_local_io_ctx, clone_id, mirror_image));
    EXPECT_EQ(0, ictx->state->close());
    return clone_id;
  }
  ...
  check_image_deleted()
  {
    ...
    cls::rbd::MirrorImage mirror_image;
    EXPECT_EQ(
        -ENOENT, cls_client::mirror_image_get(
                     &m_local_io_ctx, m_local_image_id, &mirror_image));
  }

  int
  trash_move(const std::string& global_image_id)
  {
    C_SaferCond ctx;
    rbd::mirror::ImageDeleter<>::trash_move(
        m_local_io_ctx, global_image_id, true, m_threads->work_queue, &ctx);
    return ctx.wait();
  }
