...
int
flush(librbd::ImageCtx* image_ctx)
{
  C_SaferCond ctx;
...
void
scribble(librbd::ImageCtx* image_ctx, int num_ops, uint64_t max_size)
{
  ...
  for (int i = 0; i < num_ops; i++) {
    ...
    if (rand() % 4 == 0) {
      ...
      ASSERT_EQ(
          (int)len,
          librbd::api::Io<>::write(*image_ctx, off, len, std::move(bl), 0));
    }
  }
...
public:
  ...
  SetUp() override
  {
    ...
    m_instance_watcher = rbd::mirror::InstanceWatcher<>::create(
        m_local_io_ctx, *m_threads->asio_engine, nullptr,
    ...
    m_remote_journaler = new ::journal::Journaler(
        ...
        m_remote_image_ctx->id, "mirror-uuid", {}, nullptr);
  ...
  void
  create_and_open(librados::IoCtx& io_ctx, librbd::ImageCtx** image_ctx)
  {
    librbd::RBD rbd;
  ...
  ImageSync<>*
  create_request(Context* ctx)
  {
    return new ImageSync<>(
        m_threads, m_local_image_ctx, m_remote_image_ctx, "mirror-uuid",
        m_sync_point_handler, m_instance_watcher, nullptr, ctx);
  }
  ...
  rbd::mirror::image_replayer::journal::StateBuilder<librbd::ImageCtx>*
...
TEST_F(TestImageSync, Resize)
{
  ...
  ASSERT_EQ(
      (int)len,
      librbd::api::Io<>::write(*m_remote_image_ctx, off, len, std::move(bl), 0));
  {
  ...
  ASSERT_EQ(
      0, m_remote_image_ctx->operations->resize(size, true, no_op_progress_ctx));
...
TEST_F(TestImageSync, Discard)
{
  ...
  ASSERT_EQ(
      (int)len - 2, librbd::api::Io<>::discard(
...
TEST_F(TestImageSync, SnapshotStress)
{
  ...
  for (int idx = 0; idx <= num_snaps; ++idx) {
    ...
    ASSERT_EQ(
        0,
        m_remote_image_ctx->operations->resize(size, true, no_op_progress_ctx));
    ASSERT_EQ(0, m_remote_image_ctx->state->refresh());
    ...
    if (idx < num_snaps) {
      ...
      ASSERT_EQ(
          0,
          create_snap(m_remote_image_ctx, snap_names.back().c_str(), nullptr));
    } else {
  ...
  for (auto& snap_name : snap_names) {
    ...
    uint64_t remote_size;
    {
      ...
      std::shared_lock remote_image_locker{m_remote_image_ctx->image_lock};
      remote_size =
          m_remote_image_ctx->get_image_size(m_remote_image_ctx->snap_id);
    }
    ...
    uint64_t local_size;
    {
      ...
      bool flags_set;
      ASSERT_EQ(
          0, m_local_image_ctx->test_flags(
                 m_local_image_ctx->snap_id, RBD_FLAG_OBJECT_MAP_INVALID,
                 m_local_image_ctx->image_lock, &flags_set));
