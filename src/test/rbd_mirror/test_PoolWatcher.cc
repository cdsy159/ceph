...
public:
  TestPoolWatcher() :
    m_pool_watcher_listener(this), m_image_number(0), m_snap_number(0)
  {
  ...
  void
  create_pool(bool enable_mirroring, const PeerSpec& peer, string* name = nullptr)
  {
    ...
    m_pool_watcher.reset(new PoolWatcher<>(
        m_threads, ioctx, "mirror uuid", m_pool_watcher_listener));
    ...
    if (enable_mirroring) {
      ...
      ASSERT_EQ(
          0, librbd::api::Mirror<>::peer_site_add(
                 ioctx, &uuid, RBD_MIRROR_PEER_DIRECTION_RX_TX,
  ...
  string
  get_image_id(librados::IoCtx* ioctx, const string& image_name)
  {
      ...
      string* image_name = nullptr)
  {
    ...
    ASSERT_EQ(
        0, librbd::create(
               ioctx, name.c_str(), 1 << 22, false, features, &order, 0, 0));
    if (mirrored) {
      librbd::Image image;
      librbd::RBD rbd;
      ...
      ASSERT_EQ(
          0, image.mirror_image_get_info(
                 &mirror_image_info, sizeof(mirror_image_info)));
      image.close();

      m_mirrored_images.insert(
          ImageId(mirror_image_info.global_id, get_image_id(&ioctx, name)));
    }
      ...
      string* image_name = nullptr)
  {
    ...
    string snap_name = "snap" + stringify(++m_snap_number);
    {
      librbd::ImageCtx* ictx = new librbd::ImageCtx(
          parent_image_name.c_str(), "", "", pioctx, false);
      ictx->state->open(0);
    ...
    int order = 0;
    librbd::clone(
        pioctx, parent_image_name.c_str(), snap_name.c_str(), cioctx,
        name.c_str(), features, &order, 0, 0);
    if (mirrored) {
      librbd::Image image;
      librbd::RBD rbd;
      ...
      m_mirrored_images.insert(
          ImageId(mirror_image_info.global_id, get_image_id(&cioctx, name)));
    }
    if (image_name != nullptr)
      *image_name = name;
  }
  ...
  check_images()
  {
    ...
    while (m_mirrored_images != m_pool_watcher_listener.image_ids) {
      if (m_pool_watcher_listener.cond.wait_for(l, 10s) ==
