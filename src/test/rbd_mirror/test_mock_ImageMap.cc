...
public:
  ...
  struct MockListener : Listener {
    ...
    MOCK_METHOD3(
        mock_remove_image,
        void(const std::string&, const std::string&, Context*));
  ...
  SetUp() override
  {
    ...
    EXPECT_EQ(
        0, _rados->conf_set("rbd_mirror_image_policy_migration_throttle", "0"));
  ...
  void
  expect_work_queue(MockThreads& mock_threads)
  {
  ...
  void
  expect_add_event(MockThreads& mock_threads)
  {
  ...
  void
  expect_rebalance_event(MockThreads& mock_threads)
  {
    EXPECT_CALL(*mock_threads.timer, add_event_after(_, _))
        .WillOnce(DoAll(
            WithArg<1>(Invoke([this](Context* ctx) {
              ...
              CephContext* cct =
                  reinterpret_cast<CephContext*>(m_local_io_ctx.cct());
              cct->_conf.set_val(
                  "rbd_mirror_image_policy_rebalance_timeout", "0");
  ...
  void
  expect_load_request(MockLoadRequest& request, int r)
  {
    EXPECT_CALL(request, send()).WillOnce(Invoke([&request, r]() {
      request.on_finish->complete(r);
    }));
  }

  void
  expect_update_request(MockUpdateRequest& request, int r)
  {
      ...
      std::map<std::string, Context*>* peer_ack_ctxs)
  {
    EXPECT_CALL(mock_listener, mock_acquire_image(global_image_id, _))
        .WillOnce(WithArg<1>(Invoke([this, global_image_id,
  ...
  {
    EXPECT_CALL(
        mock_listener, mock_remove_image(mirror_uuid, global_image_id, _))
        .WillOnce(WithArg<2>(Invoke([this, global_image_id,
                                     peer_ack_ctxs](Context* ctx) {
          std::lock_guard locker{m_lock};
          peer_ack_ctxs->insert({global_image_id, ctx});
          ++m_notify_update_count;
  ...
  void
  expect_listener_images_unmapped(
      MockListener& mock_listener,
      size_t count,
      std::set<std::string>* global_image_ids,
      std::map<std::string, Context*>* peer_ack_ctxs)
  {
    EXPECT_CALL(mock_listener, mock_release_image(_, _))
        .Times(count)
        .WillRepeatedly(Invoke([this, global_image_ids, peer_ack_ctxs](
                                   std::string global_image_id, Context* ctx) {
          std::lock_guard locker{m_lock};
  ...
  void
  remote_peer_ack_nowait(
      MockImageMap* image_map,
      ...
      std::map<std::string, Context*>* peer_ack_ctxs)
  {
    for (auto const& global_image_id : global_image_ids) {
      ...
      expect_add_event(mock_threads);
      expect_listener_acquire_image(
          mock_listener, global_image_id, peer_ack_ctxs);
    }
  }
  ...
  bool
  wait_for_listener_notify(uint32_t count)
  {
  ...
  bool
  wait_for_map_update(uint32_t count)
  {
    std::unique_lock locker{m_lock};
  ...
  int
  when_shut_down(MockImageMap* image_map)
  {
    C_SaferCond ctx;
      ...
      std::map<std::string, Context*>* peer_ack_ctxs)
  {
    for (auto const& global_image_id : global_image_ids) {
      expect_listener_remove_image(
          mock_listener, mirror_uuid, global_image_id, peer_ack_ctxs);
    }
  }
...
TEST_F(TestMockImageMap, SetLocalImages)
{
  ...
  std::unique_ptr<MockImageMap> mock_image_map{MockImageMap::create(
      m_local_io_ctx, &mock_threads, m_local_instance_id, mock_listener)};
...
TEST_F(TestMockImageMap, AddRemoveLocalImage)
{
  ...
  std::map<std::string, Context*> peer_ack_ctxs;
  listener_acquire_images(
      mock_listener, initial_global_image_ids, &peer_ack_ctxs);
  ...
  expect_add_event(mock_threads);
  listener_release_images(
      mock_listener, remove_global_image_ids, &peer_ack_ctxs);
  update_map_request(
      mock_threads, mock_update_request, remove_global_image_ids, 0);
...
TEST_F(TestMockImageMap, AddRemoveRemoteImage)
{
  ...
  // initial image list
  mock_image_map->update_images(
      "uuid1", std::move(initial_global_image_ids), {});
  ...
  listener_remove_images(
      mock_listener, "uuid1", remove_global_image_ids, &peer_remove_ack_ctxs);
  expect_add_event(mock_threads);
  listener_release_images(
      mock_listener, remove_global_image_ids, &peer_ack_ctxs);
  update_map_request(
  ...
  remote_peer_ack_nowait(
      mock_image_map.get(), remove_global_image_ids_ack, 0,
...
TEST_F(TestMockImageMap, AddRemoveRemoteImageDuplicateNotification)
{
  ...
  wait_for_scheduled_task();
  mock_image_map->update_images(
      "uuid1", std::move(initial_global_image_ids_dup), {});
  ...
  mock_image_map->update_images(
      "uuid1", {}, std::move(remove_global_image_ids_dup));
...
TEST_F(TestMockImageMap, RemoveRemoteAndLocalImage)
{
  ...
  std::set<std::string> initial_remote_global_image_ids_ack(
  ...
  std::set<std::string> remote_remove_global_image_ids_ack(
      remote_remove_global_image_ids);

  std::set<std::string> local_remove_global_image_ids{"global id 1"};
  std::set<std::string> local_remove_global_image_ids_ack(
  ...
  std::map<std::string, Context*> peer_ack_ctxs;
  listener_acquire_images(
      mock_listener, initial_remote_global_image_ids, &peer_ack_ctxs);

  // initial remote image list
  mock_image_map->update_images(
      "uuid1", std::move(initial_remote_global_image_ids), {});

  ASSERT_TRUE(wait_for_map_update(1));
  ASSERT_TRUE(
      wait_for_listener_notify(initial_remote_global_image_ids_ack.size()));

  // remote peer ACKs image acquire request
  remote_peer_ack_nowait(
      mock_image_map.get(), initial_remote_global_image_ids_ack, 0,
      &peer_ack_ctxs);

  // set initial local image list -- this is a no-op from policy pov
  mock_image_map->update_images(
      "", std::move(initial_local_global_image_ids), {});
  ...
  mock_image_map->update_images(
      "uuid1", {}, std::move(remote_remove_global_image_ids));
  ASSERT_TRUE(
      wait_for_listener_notify(remote_remove_global_image_ids_ack.size()));

  // RELEASE+REMOVE_MAPPING
  expect_add_event(mock_threads);
  listener_release_images(
      mock_listener, local_remove_global_image_ids, &peer_ack_ctxs);
  update_map_request(
      mock_threads, mock_update_request, local_remove_global_image_ids, 0);

  // remove local images
  mock_image_map->update_images(
      "", {}, std::move(local_remove_global_image_ids));
  ASSERT_TRUE(
      wait_for_listener_notify(local_remove_global_image_ids_ack.size()));

  remote_peer_ack_nowait(
      mock_image_map.get(), local_remove_global_image_ids_ack, 0,
      &peer_ack_remove_ctxs);
  remote_peer_ack_wait(
      mock_image_map.get(), local_remove_global_image_ids_ack, 0,
      &peer_ack_ctxs);
...
TEST_F(TestMockImageMap, AddInstance)
{
  ...
  expect_add_event(mock_threads);
  expect_listener_images_unmapped(
      mock_listener, 3, &shuffled_global_image_ids, &peer_ack_ctxs);
  ...
  update_map_and_acquire(
      mock_threads, mock_update_request, mock_listener,
...
TEST_F(TestMockImageMap, RemoveInstance)
{
  ...
  expect_add_event(mock_threads);
  expect_listener_images_unmapped(
      mock_listener, 2, &shuffled_global_image_ids, &peer_ack_ctxs);
...
{
  EXPECT_EQ(
      0, _rados->conf_set("rbd_mirror_image_policy_migration_throttle", "600"));
  ...
  std::set<std::string> global_image_ids{
      "global id 1",  "global id 2",  "global id 3",  "global id 4",
      "global id 5",  "global id 6",  "global id 7",  "global id 8",
      "global id 9",  "global id 10", "global id 11", "global id 12",
      "global id 13", "global id 14"};
  ...
  // ACQUIRE
  MockLoadRequest mock_load_request;
  EXPECT_CALL(mock_load_request, send())
      .WillOnce(Invoke([&mock_load_request, &image_mapping]() {
  ...
  expect_add_event(mock_threads);
  expect_listener_images_unmapped(
      mock_listener, 7, &shuffled_global_image_ids, &peer_ack_ctxs);
  ...
  std::set_intersection(
      migrated_global_image_ids.begin(), migrated_global_image_ids.end(),
...
TEST_F(TestMockImageMap, RemoveInstanceWithRemoveImage)
{
  ...
  std::map<std::string, Context*> peer_ack_remove_ctxs;
  listener_remove_images(
      mock_listener, "uuid1", shuffled_global_image_ids, &peer_ack_remove_ctxs);
  expect_add_event(mock_threads);
  listener_release_images(
      mock_listener, shuffled_global_image_ids, &peer_ack_ctxs);
  expect_add_event(mock_threads);
  expect_update_request(mock_update_request, 0);
  expect_add_event(mock_threads);
  expect_update_request(mock_update_request, 0);
  ...
  mock_image_map->update_images(
      "uuid1", {}, std::move(shuffled_global_image_ids));
  ASSERT_TRUE(
      wait_for_listener_notify(shuffled_global_image_ids_ack.size() * 2));

  // instance failed -- update policy for instance removal
  mock_image_map->update_instances_removed({"9876"});

  remote_peer_ack_nowait(
      mock_image_map.get(), shuffled_global_image_ids, -ENOENT,
      &peer_ack_remove_ctxs);
  remote_peer_ack_wait(
      mock_image_map.get(), shuffled_global_image_ids, -EBLOCKLISTED,
      &peer_ack_ctxs);
...
TEST_F(TestMockImageMap, AddErrorAndRemoveImage)
{
  ...
  update_map_and_acquire(
      mock_threads, mock_update_request, mock_listener,
      shuffled_global_image_ids, 0, &remap_peer_ack_ctxs);
  ...
  remote_peer_ack_nowait(
      mock_image_map.get(), shuffled_global_image_ids, 0, &remap_peer_ack_ctxs);
  wait_for_scheduled_task();
  ...
  listener_release_images(
      mock_listener, shuffled_global_image_ids, &peer_ack_ctxs);
  update_map_request(
      mock_threads, mock_update_request, shuffled_global_image_ids, 0);
  ...
  remote_peer_ack_nowait(
      mock_image_map.get(), shuffled_global_image_ids_ack, 0,
      &peer_ack_remove_ctxs);
  remote_peer_ack_wait(
...
TEST_F(TestMockImageMap, MirrorUUIDUpdated)
{
  ...
  std::set<std::string> remote_removed_global_image_ids{
      "global id 1", "global id 2", "global id 3"};
  std::set<std::string> remote_removed_global_image_ids_ack(
      remote_removed_global_image_ids);

  std::set<std::string> remote_added_global_image_ids{
      "global id 1", "global id 2", "global id 3"};
  std::set<std::string> remote_added_global_image_ids_ack(
  ...
  std::map<std::string, Context*> peer_remove_ack_ctxs;
  listener_remove_images(
      mock_listener, "uuid1", remote_removed_global_image_ids,
      &peer_remove_ack_ctxs);
  expect_add_event(mock_threads);
  listener_release_images(
      mock_listener, remote_removed_global_image_ids, &peer_ack_ctxs);
  update_map_request(
      mock_threads, mock_update_request, remote_removed_global_image_ids, 0);

  mock_image_map->update_images(
      "uuid1", {}, std::move(remote_removed_global_image_ids));
  ASSERT_TRUE(
      wait_for_listener_notify(remote_removed_global_image_ids_ack.size() * 2));

  remote_peer_ack_nowait(
      mock_image_map.get(), remote_removed_global_image_ids_ack, 0,
      &peer_remove_ack_ctxs);
  remote_peer_ack_wait(
      mock_image_map.get(), remote_removed_global_image_ids_ack, 0,
      &peer_ack_ctxs);
  ...
  expect_add_event(mock_threads);
  listener_acquire_images(
      mock_listener, remote_added_global_image_ids, &peer_ack_ctxs);

  mock_image_map->update_images(
      "uuid2", std::move(remote_added_global_image_ids), {});

  ASSERT_TRUE(wait_for_map_update(1));
  ASSERT_TRUE(
      wait_for_listener_notify(remote_added_global_image_ids_ack.size()));

  // remote peer ACKs image acquire request
  remote_peer_ack_nowait(
      mock_image_map.get(), remote_added_global_image_ids_ack, 0,
      &peer_ack_ctxs);
...
TEST_F(TestMockImageMap, RebalanceImageMap)
{
  ...
  expect_add_event(mock_threads);
  expect_listener_images_unmapped(
      mock_listener, 5, &shuffled_global_image_ids, &peer_ack_ctxs);
  ...
  remote_peer_ack_nowait(
      mock_image_map.get(), new_global_image_ids_ack, 0, &peer_ack_ctxs);
