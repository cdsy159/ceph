...
DeepCopyRequest<librbd::MockTestImageCtx>*
    DeepCopyRequest<librbd::MockTestImageCtx>::s_instance = nullptr;
...
SyncPointCreateRequest<librbd::MockTestImageCtx>*
    SyncPointCreateRequest<librbd::MockTestImageCtx>::s_instance = nullptr;
SyncPointPruneRequest<librbd::MockTestImageCtx>*
    SyncPointPruneRequest<librbd::MockTestImageCtx>::s_instance = nullptr;
...
public:
  ...
  typedef image_sync::SyncPointCreateRequest<librbd::MockTestImageCtx>
      MockSyncPointCreateRequest;
  typedef image_sync::SyncPointPruneRequest<librbd::MockTestImageCtx>
  ...
  void
  expect_get_snap_id(librbd::MockTestImageCtx& mock_image_ctx)
  {
  ...
  void
  expect_create_sync_point(
      librbd::MockTestImageCtx& mock_local_image_ctx,
      MockSyncPointCreateRequest& mock_sync_point_create_request,
      int r)
  {
    EXPECT_CALL(mock_sync_point_create_request, send())
        .WillOnce(Invoke([this, &mock_local_image_ctx,
                          &mock_sync_point_create_request, r]() {
          if (r == 0) {
            mock_local_image_ctx
                .snap_ids[{cls::rbd::UserSnapshotNamespace(), "snap1"}] = 123;
            m_sync_points.emplace_back(
                cls::rbd::UserSnapshotNamespace(), "snap1", "", boost::none);
          }
          m_threads->work_queue->queue(
              mock_sync_point_create_request.on_finish, r);
        }));
  }

  void
  expect_copy_image(MockImageCopyRequest& mock_image_copy_request, int r)
  {
  ...
  void
  expect_flush_sync_point(MockSyncPointHandler& mock_sync_point_handler, int r)
  {
      ...
      int r)
  {
    EXPECT_CALL(mock_sync_point_prune_request, send())
        .WillOnce(Invoke([this, &mock_sync_point_prune_request, sync_complete,
          ...
          m_threads->work_queue->queue(
              mock_sync_point_prune_request.on_finish, r);
        }));
  }

  void
  expect_get_snap_seqs(MockSyncPointHandler& mock_sync_point_handler)
  {
  ...
  void
  expect_get_sync_points(MockSyncPointHandler& mock_sync_point_handler)
  {
    EXPECT_CALL(mock_sync_point_handler, get_sync_points())
        .WillRepeatedly(Invoke([this]() { return m_sync_points; }));
  }
  ...
  create_request(
      ...
      Context* ctx)
  {
    return new MockImageSync(
        &mock_threads, &mock_local_image_ctx, &mock_remote_image_ctx,
        "mirror-uuid", &mock_sync_point_handler, &mock_instance_watcher,
        nullptr, ctx);
  }
...
TEST_F(TestMockImageSync, SimpleSync)
{
  ...
  expect_create_sync_point(
      mock_local_image_ctx, mock_sync_point_create_request, 0);
  ...
  C_SaferCond ctx;
  MockImageSync* request = create_request(
      mock_threads, mock_remote_image_ctx, mock_local_image_ctx,
      mock_sync_point_handler, mock_instance_watcher, &ctx);
  request->send();
...
TEST_F(TestMockImageSync, RestartSync)
{
  ...
  mock_local_image_ctx.snap_ids[{cls::rbd::UserSnapshotNamespace(), "snap1"}] =
      123;
  mock_local_image_ctx.snap_ids[{cls::rbd::UserSnapshotNamespace(), "snap2"}] =
      234;
...
TEST_F(TestMockImageSync, CancelNotifySyncRequest)
{
  ...
  EXPECT_CALL(
      mock_instance_watcher, notify_sync_request(mock_local_image_ctx.id, _))
      .WillOnce(Invoke([&on_sync_start,
                        &notify_sync_ctx](const std::string&, Context* ctx) {
        on_sync_start = ctx;
        notify_sync_ctx.complete(0);
      }));
  EXPECT_CALL(
      mock_instance_watcher, cancel_sync_request(mock_local_image_ctx.id))
...
TEST_F(TestMockImageSync, CancelImageCopy)
{
  ...
  EXPECT_CALL(mock_image_copy_request, send())
      .WillOnce(Invoke([&image_copy_ctx]() { image_copy_ctx.complete(0); }));
  expect_cancel_sync_request(
      mock_instance_watcher, mock_local_image_ctx.id, false);
...
TEST_F(TestMockImageSync, CancelAfterCopyImage)
{
  ...
  EXPECT_CALL(mock_image_copy_request, send())
      .WillOnce((DoAll(
          InvokeWithoutArgs([request]() { request->cancel(); }),
