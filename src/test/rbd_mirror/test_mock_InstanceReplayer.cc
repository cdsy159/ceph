...
ImageReplayer<librbd::MockTestImageCtx>*
    ImageReplayer<librbd::MockTestImageCtx>::s_instance = nullptr;
...
public:
  ...
  void
  expect_add_event_after(MockThreads& mock_threads, Context** timer_ctx = nullptr)
  {
  ...
  void
  expect_cancel_event(MockThreads& mock_threads, bool canceled)
  {
...
TEST_F(TestMockInstanceReplayer, AcquireReleaseImage)
{
  ...
  MockInstanceReplayer instance_replayer(
      m_local_io_ctx, "local_mirror_uuid", &mock_threads, &mock_service_daemon,
      &mock_status_updater, nullptr, nullptr);
  ...
  instance_replayer.acquire_image(
      &mock_instance_watcher, global_image_id, &on_acquire);
...
TEST_F(TestMockInstanceReplayer, RemoveFinishedImage)
{
  ...
  EXPECT_CALL(mock_image_replayer, get_health_state())
      .WillOnce(Return(image_replayer::HEALTH_STATE_OK));
  EXPECT_CALL(mock_image_replayer, is_stopped()).WillOnce(Return(true));
  EXPECT_CALL(mock_image_replayer, is_blocklisted()).WillOnce(Return(false));
  EXPECT_CALL(mock_image_replayer, is_finished()).WillOnce(Return(true));
  EXPECT_CALL(mock_image_replayer, destroy());
  EXPECT_CALL(mock_service_daemon, add_or_update_namespace_attribute(_, _, _, _))
      .Times(3);
...
TEST_F(TestMockInstanceReplayer, Reacquire)
{
  ...
  instance_replayer.acquire_image(
      &mock_instance_watcher, global_image_id, &on_acquire1);
  ...
  instance_replayer.acquire_image(
      &mock_instance_watcher, global_image_id, &on_acquire2);
