...
public:
  static void
  apply_pool_overrides(librados::IoCtx& io_ctx, ConfigProxy* config_proxy)
  {}
};
...
public:
  ...
  static int
  list(librados::IoCtx& io_ctx, std::vector<std::string>* names)
  {
    if (s_instance) {
...
Namespace<librbd::MockTestImageCtx>*
    Namespace<librbd::MockTestImageCtx>::s_instance = nullptr;
...
Throttler<librbd::MockTestImageCtx>*
    Throttler<librbd::MockTestImageCtx>::s_instance = nullptr;

template <>
struct NamespaceReplayer<librbd::MockTestImageCtx> {
  static std::map<std::string, NamespaceReplayer*> s_instances;

  static NamespaceReplayer*
  create(
...
LeaderWatcher<librbd::MockTestImageCtx>*
    LeaderWatcher<librbd::MockTestImageCtx>::s_instance = nullptr;
...
RemotePoolPoller<librbd::MockTestImageCtx>*
    RemotePoolPoller<librbd::MockTestImageCtx>::s_instance = nullptr;
...
struct ServiceDaemon<librbd::MockTestImageCtx> {
  ...
  MOCK_METHOD3(
      ...
      void(int64_t, const std::string&, const service_daemon::AttributeValue&));
...
public:
      ...
      CephContext** cct_ref)
  {
    EXPECT_CALL(mock_cluster, create_rados_client(_))
        .WillOnce(Invoke([cluster_name, mock_rados_client,
                          cct_ref](CephContext* cct) {
      ...
      int r)
  {
    ...
    EXPECT_CALL(
        *io_ctx_impl, exec(
                          ...
                          StrEq("mirror_uuid_get"), _, _, _, _))
        .WillOnce(DoAll(
            WithArg<5>(Invoke([out_bl](bufferlist* bl) { *bl = out_bl; })),
            Return(r)));
  }

  void
  expect_mirror_mode_get(
      librados::MockTestMemIoCtxImpl* io_ctx_impl,
      ...
      int r)
  {
    EXPECT_CALL(
        *io_ctx_impl, exec(
                          RBD_MIRRORING, _, StrEq("rbd"),
                          StrEq("mirror_mode_get"), _, _, _, _))
        .WillRepeatedly(DoAll(
            WithArg<5>(Invoke([io_ctx_impl,
                               default_namespace_enabled](bufferlist* bl) {
  ...
  void
  expect_mirror_remote_namespace_get(
      librados::MockTestMemIoCtxImpl* io_ctx_impl,
      ...
      int r)
  {
    EXPECT_CALL(
        *io_ctx_impl, exec(
        ...
        .WillRepeatedly(DoAll(
            WithArg<5>(Invoke([remote_namespace](bufferlist* bl) {
  ...
  {
    EXPECT_CALL(*mock_io_ctx, clone()).WillRepeatedly(Invoke([mock_io_ctx]() {
      mock_io_ctx->get();
      return mock_io_ctx;
    }));
  }
  ...
  void
  expect_leader_watcher_init(MockLeaderWatcher& mock_leader_watcher, int r)
  {
    EXPECT_CALL(mock_leader_watcher, init()).WillOnce(Return(r));
  }

  void
  expect_leader_watcher_shut_down(MockLeaderWatcher& mock_leader_watcher)
  {
    EXPECT_CALL(mock_leader_watcher, shut_down());
  }

  void
  expect_leader_watcher_get_leader_instance_id(
  ...
  void
  expect_leader_watcher_list_instances(MockLeaderWatcher& mock_leader_watcher)
  {
    EXPECT_CALL(mock_leader_watcher, list_instances(_)).Times(AtLeast(0));
  }

  void
  expect_remote_pool_poller_init(
      MockRemotePoolPoller& mock_remote_pool_poller,
      const RemotePoolMeta& remote_pool_meta,
      int r)
  {
    EXPECT_CALL(mock_remote_pool_poller, init(_))
        .WillOnce(Invoke([this, &mock_remote_pool_poller, remote_pool_meta,
                          r](Context* ctx) {
          if (r >= 0) {
  ...
  void
  expect_remote_pool_poller_shut_down(
      MockRemotePoolPoller& mock_remote_pool_poller,
      int r)
  {
  ...
  void
  expect_leader_watcher_is_blocklisted(
      MockLeaderWatcher& mock_leader_watcher,
      bool blocklisted)
  {
  ...
  void
  expect_namespace_replayer_is_blocklisted(
  ...
  void
  expect_namespace_replayer_get_instance_id(
      MockNamespaceReplayer& mock_namespace_replayer,
  ...
  void
  expect_namespace_replayer_init(
      MockNamespaceReplayer& mock_namespace_replayer,
      int r,
  ...
  void
  expect_namespace_replayer_shut_down(
      MockNamespaceReplayer& mock_namespace_replayer,
  ...
  {
    EXPECT_CALL(
        mock_namespace_replayer, handle_update_leader(leader_instance_id))
  ...
  void
  expect_namespace_replayer_handle_instances_removed(
      MockNamespaceReplayer& mock_namespace_replayer)
  {
  ...
  void
  expect_namespace_replayer_get_remote_namespace(
      MockNamespaceReplayer& mock_namespace_replayer,
      const std::string& remote_namespace)
  {
  ...
  void
  expect_service_daemon_add_namespace(
  ...
  void
  expect_service_daemon_remove_namespace(
      MockServiceDaemon& mock_service_daemon,
      const std::string& namespace_name)
  {
    EXPECT_CALL(
  ...
  void
  expect_service_daemon_remove_attribute(
      MockServiceDaemon& mock_service_daemon,
      const std::string& key)
  {
      ...
      const std::string& text)
  {
    EXPECT_CALL(
        mock_service_daemon,
        add_or_update_callout(
            m_local_io_ctx.get_id(), callout_id, callout_level, text))
        .WillOnce(Return(out_callout_id));
  }

  void
  expect_service_daemon_remove_callout(
      MockServiceDaemon& mock_service_daemon,
      uint64_t callout_id)
  {
    EXPECT_CALL(
...
TEST_F(TestMockPoolReplayer, ConfigKeyOverride)
{
  ...
  auto mock_local_rados_client =
      mock_cluster.do_create_rados_client(g_ceph_context);
  expect_connect(mock_cluster, mock_local_rados_client, "ceph", nullptr);

  auto mock_remote_rados_client =
      mock_cluster.do_create_rados_client(g_ceph_context);
  CephContext* remote_cct = nullptr;
  expect_connect(
      mock_cluster, mock_remote_rados_client, "cluster name", &remote_cct);
  ...
  expect_remote_pool_poller_init(
      *mock_remote_pool_poller, {"remote mirror uuid", ""}, 0);
  ...
  MockPoolReplayer pool_replayer(
      &mock_threads, &mock_service_daemon, nullptr, &m_pool_meta_cache,
...
TEST_F(TestMockPoolReplayer, AcquireReleaseLeader)
{
  ...
  auto mock_remote_rados_client =
      mock_cluster.do_create_rados_client(g_ceph_context);
  expect_connect(
      mock_cluster, mock_remote_rados_client, "cluster name", nullptr);
  ...
  expect_service_daemon_remove_attribute(
      mock_service_daemon, SERVICE_DAEMON_LEADER_KEY);
...
TEST_F(TestMockPoolReplayer, DefaultNamespaceEnabled)
{
  ...
  expect_namespace_replayer_is_blocklisted(
      *mock_default_namespace_replayer, false);
  expect_namespace_replayer_get_remote_namespace(
  ...
  expect_namespace_replayer_handle_update_leader(
      *mock_default_namespace_replayer, "", &on_default_namespace_init);
...
TEST_F(TestMockPoolReplayer, DefaultNamespaceEnableDisable)
{
  ...
  expect_service_daemon_remove_namespace(mock_service_daemon, "");
  expect_namespace_replayer_shut_down(
      *mock_default_namespace_replayer, &on_default_namespace_shut_down);
...
TEST_F(TestMockPoolReplayer, Namespaces)
{
  ...
  expect_namespace_replayer_get_remote_namespace(
      *mock_ns1_namespace_replayer, "ns1");
  ...
  expect_namespace_replayer_get_remote_namespace(
      *mock_ns2_namespace_replayer, "ns2");
  ...
  expect_namespace_replayer_handle_update_leader(
      *mock_ns1_namespace_replayer, "", &on_ns1_init);
  ...
  expect_namespace_replayer_shut_down(
      *mock_ns2_namespace_replayer, &on_ns2_shut_down);
...
TEST_F(TestMockPoolReplayer, NamespacesError)
{
  ...
  expect_service_daemon_add_namespace(mock_service_daemon, "ns2");
  expect_namespace_replayer_handle_update_leader(
      *mock_ns2_namespace_replayer, "", &on_ns2_init);
  ...
  expect_namespace_replayer_handle_acquire_leader(
      *mock_ns2_namespace_replayer, -EINVAL);
  ctx = new LambdaContext([&mock_namespace, &default_namespace_enabled](int) {
    default_namespace_enabled = false;
    mock_namespace.remove("ns2");
  });
  expect_service_daemon_remove_namespace(mock_service_daemon, "");
  ...
  expect_namespace_replayer_handle_acquire_leader(
      *mock_ns3_namespace_replayer, -EINVAL);
...
TEST_F(TestMockPoolReplayer, RemoveCalloutOnInit)
{
  ...
  mock_local_rados_client = mock_cluster.do_create_rados_client(g_ceph_context);
  expect_connect(mock_cluster, mock_local_rados_client, "ceph", nullptr);

  mock_remote_rados_client = mock_cluster.do_create_rados_client(g_ceph_context);
  expect_connect(
      mock_cluster, mock_remote_rados_client, "cluster name", nullptr);
