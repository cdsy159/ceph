    ...
    if (req_size <= backend_bandwidth) {
      while (req_size > 0) {
        if (req_size > 4 * 1024 * 1024) {
          ratelimit->decrease_bytes(
              methodop.c_str(), it.tenant, 4 * 1024 * 1024, info);
    ...
    std::shared_ptr<RateLimiter> ratelimit)
{
  ...
  std::string methodop = method[rw];
  auto dout = DoutPrefix(g_ceph_context, ceph_subsys_rgw, "rate limiter: ");
  bool to_fail = ratelimit->should_rate_limit(
      methodop.c_str(), it.tenant, time, &info, "");
  if (to_fail) {
    it.rejected++;
    it.ops++;
    return true;
  ...
  for (;;) {
    ...
    while (to_retry && to_run) {
      if (params.wait_between_retries_ms) {
        boost::asio::steady_timer timer(ioctx);
        timer.expires_after(
            std::chrono::milliseconds(params.wait_between_retries_ms));
  ...
  for (int i = 0; i < params.num_clients; i++) {
    ...
    boost::asio::spawn(
        ...
        [&to_run, x, ratelimit, info, params,
         &context](boost::asio::yield_context ctx) {
...
main(int argc, char** argv)
{
  ...
  try {
    ...
    desc.add_options()("help,h", "Help screen")(
        ...
        "how many ratelimit tenants")(
        "request_size", value<int64_t>()->default_value(1),
        "what is the request size we are testing if 0, it will be randomized")(
        "backend_bandwidth", value<int64_t>()->default_value(1),
        "what is the backend bandwidth, so there will be wait between "
        "decrease_bytes")(
        "wait_between_retries_ms", value<size_t>()->default_value(1),
        "time in seconds to wait between retries")(
        "ops_limit", value<int64_t>()->default_value(1),
        "ops limit for the tenants")(
        "bw_limit", value<int64_t>()->default_value(1),
        "bytes per second limit")(
        "threads", value<int>()->default_value(512), "server's threads count")(
        "runtime", value<int>()->default_value(60),
        "For how many seconds the test will run")(
        "num_clients", value<int>()->default_value(1),
        "number of clients per tenant to run");
    variables_map vm;
  ...
  std::unique_ptr<CephContext> cct =
      std::make_unique<CephContext>(CEPH_ENTITY_TYPE_ANY);
  if (!g_ceph_context) {
    g_ceph_context = cct.get();
  }
  std::shared_ptr<ActiveRateLimiter> ratelimit(
      new ActiveRateLimiter(g_ceph_context));
  ratelimit->start();
  ...
  for (int i = 0; i < num_ratelimit_classes; i++) {
    ...
    simulate_clients(
        context, tenantuser, info, ratelimit->get_active(), params, to_run);
  }
  ...
  for (auto& i : metrics_by_tenant) {
    ...
    std::cout << "Simulator finished accepted  sum : " << i.second.accepted
              << std::endl;
    std::cout << "Simulator finished rejected  sum : " << i.second.rejected
              << std::endl;
  }
