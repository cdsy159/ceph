...
struct TempPool {
  ...
  operator librados::IoCtx()
  {
    ...
    [[maybe_unused]] auto r =
        store->getRados()->get_rados_handle()->ioctx_create(name.c_str(), ioctx);
    assert(r == 0);
    return ioctx;
  }
};
...
TEST(ReadAttrs, Unfiltered)
{
  ...
  auto r = run(
      new RGWSimpleRadosReadAttrsCR(dpp(), store, {pool, oid}, &attrs, true));
  ASSERT_EQ(0, r);
...
TEST(ReadAttrs, Filtered)
{
  ...
  std::map<std::string, ceph::bufferlist> attrs;
  auto r = run(
      new RGWSimpleRadosReadAttrsCR(dpp(), store, {pool, oid}, &attrs, false));
  ASSERT_EQ(0, r);
  ASSERT_EQ(ref_attrs, attrs);
}
...
TEST(Read, Dne)
{
  TempPool pool;
  std::string result;
  auto r = run(new RGWSimpleRadosReadCR(
      dpp(), store, {pool, "doesnotexist"}, &result, false));
  ASSERT_EQ(-ENOENT, r);
}
...
TEST(Read, Read)
{
  ...
  std::string result;
  auto r =
      run(new RGWSimpleRadosReadCR(dpp(), store, {pool, oid}, &result, false));
  ASSERT_EQ(0, r);
...
TEST(Read, ReadVersion)
{
  ...
  std::string result;
  auto r = run(new RGWSimpleRadosReadCR(
      dpp(), store, {pool, oid}, &result, false, &robjv));
  ASSERT_EQ(0, r);
  ASSERT_EQ(result, data);
  ...
  result.clear();
  r = run(new RGWSimpleRadosReadCR(
      dpp(), store, {pool, oid}, &result, false, &robjv));
...
TEST(Write, Exclusive)
{
  ...
  auto r = run(new RGWSimpleRadosWriteCR(
      dpp(), store, {pool, oid}, "I am some DIFFERENT data!"s, nullptr, true));
  ASSERT_EQ(-EEXIST, r);
}
...
TEST(Write, Write)
{
  ...
  auto r = run(new RGWSimpleRadosWriteCR(
      dpp(), store, {pool, oid}, data, nullptr, true));
  ASSERT_EQ(0, r);
  bufferlist bl;
  librados::IoCtx ioctx(pool);
...
TEST(Write, ObjV)
{
  ...
  auto r = run(new RGWSimpleRadosWriteCR(
      dpp(), store, {pool, oid}, "I'm some data!"s, &objv, true));
  RGWObjVersionTracker interfering_objv(objv);
  r = run(new RGWSimpleRadosWriteCR(
      dpp(), store, {pool, oid}, "I'm some newer, better data!"s,
      &interfering_objv, false));
  ASSERT_EQ(0, r);
  r = run(new RGWSimpleRadosWriteCR(
      dpp(), store, {pool, oid}, "I'm some treacherous, obsolete data!"s, &objv,
      false));
  ASSERT_EQ(-ECANCELED, r);
}
...
TEST(WriteAttrs, Attrs)
{
  ...
  auto r = run(new RGWSimpleRadosWriteAttrsCR(
      dpp(), store, {pool, oid}, wrattrs, nullptr, true));
  ASSERT_EQ(0, r);
...
main(int argc, const char** argv)
{
  ...
  store = static_cast<rgw::sal::RadosStore*>(DriverManager::get_storage(
      dpp(), g_ceph_context, cfg, context_pool, site, false, false, false,
      false, false, false, true, true, true, null_yield, cfgstore.get(), false));
  if (!store) {
