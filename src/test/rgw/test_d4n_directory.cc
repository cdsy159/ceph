...
protected:
  ...
  std::vector<std::string> vals{"0",           "",  "0",
                                "0",           "0", "testName",
                                "testBucket",  "",  "0",
                                env->redisHost};
  std::vector<std::string> fields{"blockID",    "version",      "deleteMarker",
                                  "size",       "globalWeight", "objName",
                                  "bucketName", "creationTime", "dirty",
                                  "hosts"};
};
...
TEST_F(BlockDirectoryFixture, UpdateFieldYield)
{
  boost::asio::spawn(
      ...
      [this](boost::asio::yield_context yield) {
        ...
        ASSERT_EQ(
            0, dir->update_field(
                   env->dpp, block, "objName", oid, optional_yield{yield}));
        ASSERT_EQ(
            0, dir->update_field(
                   env->dpp, block, "hosts", host, optional_yield{yield}));
  ...
  boost::asio::spawn(
      ...
      [this](boost::asio::yield_context yield) {
        ...
        ASSERT_EQ(0, dir->set(env->dpp, block, optional_yield{yield}));
        {
          ...
          ASSERT_EQ(
              0, dir->remove_host(env->dpp, block, host, optional_yield{yield}));
        }
        ...
        for (int i = 0; i < 10; i++) {
          {
            ...
            std::cout << "thread id: " << std::this_thread::get_id()
                      << std::endl;
            std::cout << "INCR value: " << std::get<0>(resp).value()
                      << std::endl;
          }
        }
  ...
  boost::asio::spawn(
      ...
      [this](boost::asio::yield_context yield) {
        ...
        }
        {
          ...
          std::vector<
              boost::redis::resp3::basic_node<std::__cxx11::basic_string<char>>>
              root_array;
          if (resp.has_value()) {
            root_array = resp.value();
            std::cout << "ZADD aggregate size is: " << root_array.size()
                      << std::endl;
...
template <typename T>
void
foo(T t, std::vector<std::vector<std::string>>& responses)
{
  ...
  boost::asio::spawn(
      ...
      [this](boost::asio::yield_context yield) {
        ...
        }
        {
          ...
          for (auto& element : resp.value()) {
            if (element.data_type == boost::redis::resp3::type::array ||
              ...
              if (i < num_elements) {
                ...
                key_val_map.emplace(
                    key, std::unordered_map<std::string, std::string>{
                             {fieldkey, fieldval}});
                key.clear();
                fieldkey.clear();
                fieldval.clear();
          ...
          std::cout << "HGETALL response size is: " << key_val_map.size()
                    << std::endl;
          for (auto& it : key_val_map) {
            std::cout << "key: " << it.first << std::endl;
            std::unordered_map<std::string, std::string> field_key_val_map =
                it.second;
