    ...
    std::string& version)
{
  if (version.empty()) {
    ...
    return fmt::format(
        "{}{}{}{}{}", bucketName, CACHE_DELIM, version, CACHE_DELIM, oid);
  }
}
...
protected:
  ...
  SetUp()
  {
    ...
    rgw::cache::Partition partition_info{
        .location = "RedisCache", .reserve_size = 1073741824};
    cacheDriver = new rgw::cache::RedisDriver{io, partition_info};
    policyDriver =
        new rgw::d4n::PolicyDriver(conn, cacheDriver, "lfuda", null_yield);
    ...
    cfg.addr.port = env->redisHost.substr(
      ...
      optional_yield y)
  {
    int age = 1;
    std::string version;
    std::string oid = rgw::sal::get_key_in_cache(
        get_prefix(block->cacheObj.bucketName, block->cacheObj.objName, version),
        std::to_string(block->blockID), std::to_string(block->size));

    if (this->policyDriver->get_cache_policy()->exist_key(oid)) { /* Local copy */
      policyDriver->get_cache_policy()->update(
          env->dpp, oid, 0, TEST_DATA_LENGTH, "", false,
          rgw::d4n::RefCount::NOOP, y);
      return 0;
    } else {
      if (this->policyDriver->get_cache_policy()->eviction(dpp, block->size, y) <
          0)
        return -1;
      ...
      if (exists > 0) { /* Remote copy */
        if (dir->get(env->dpp, block, y) < 0) {
          ...
          if (!block->cacheObj.hostsList.empty()) {
            ...
            if (dir->update_field(
                    env->dpp, block, "globalWeight", globalWeight, y) < 0) {
              return -1;
            } else {
              return 0;
            }
        ...
        block->cacheObj.hostsList.insert(
            env->dpp->get_cct()->_conf->rgw_d4n_l1_datacache_address);
        if (dir->set(env->dpp, block, y) < 0)
          return -1;

        this->policyDriver->get_cache_policy()->update(
            dpp, oid, 0, TEST_DATA_LENGTH, "", false, rgw::d4n::RefCount::NOOP,
            y);
        if (cacheDriver->put(dpp, oid, bl, TEST_DATA_LENGTH, attrs, y) < 0)
          return -1;
        return cacheDriver->set_attr(
            dpp, oid, "localWeight", std::to_string(age), y);
      } else {
        return -1;
      }
    }
...
public:
  TestRedisDriver(
      ...
      rgw::cache::Partition& _partition_info) :
    RedisDriver(io_context, _partition_info), partition_info(_partition_info)
  {
    conn = std::make_shared<connection>(boost::asio::make_strand(io_context));
  }

  virtual rgw::cache::Partition
  get_current_partition_info(const DoutPrefixProvider* dpp) override
  {
    return partition_info;
  }

  virtual uint64_t
  get_free_space(const DoutPrefixProvider* dpp, optional_yield y) override
  {
    eviction_call_count++;
  ...
  virtual int
  initialize(const DoutPrefixProvider* dpp) override
  {
    ...
    if (!cfg.addr.host.length() || !cfg.addr.port.length()) {
      ldpp_dout(dpp, 0) << "RedisDriver::" << __func__
                        << "(): Endpoint was not configured correctly."
                        << dendl;
      return -EDESTADDRREQ;
    }
  ...
  virtual int
  put(const DoutPrefixProvider* dpp,
  ...
  append_data(
      ...
      optional_yield y) override
  {
    return RedisDriver::append_data(dpp, key, bl_data, y);
  }
  ...
  delete_data(
      ...
      optional_yield y) override
  {
    return RedisDriver::delete_data(dpp, key, y);
  }
  ...
  rename(
      ...
      optional_yield y) override
  {
    return RedisDriver::rename(dpp, oldKey, newKey, y);
  }
  ...
  set_attrs(
      ...
      optional_yield y) override
  {
    return RedisDriver::set_attrs(dpp, key, attrs, y);
  }
  ...
  get_attrs(
      ...
      optional_yield y) override
  {
    return RedisDriver::get_attrs(dpp, key, attrs, y);
  }
  ...
  update_attrs(
      ...
      optional_yield y) override
  {
    return RedisDriver::update_attrs(dpp, key, attrs, y);
  }
  ...
  delete_attrs(
      ...
      optional_yield y) override
  {
    return RedisDriver::delete_attrs(dpp, key, del_attrs, y);
  }
  ...
  virtual int
  restore_blocks_objects(
      const DoutPrefixProvider* dpp,
      rgw::cache::ObjectDataCallback obj_func,
      rgw::cache::BlockDataCallback block_func) override
  {
    return 0;
  }
...
TEST_F(LFUDAPolicyFixture, LocalGetBlockYield)
{
  boost::asio::spawn(
      ...
      [this](boost::asio::yield_context yield) {
        ...
        dynamic_cast<rgw::d4n::LFUDAPolicy*>(policyDriver->get_cache_policy())
            ->save_y(optional_yield{yield});
        policyDriver->get_cache_policy()->init(env->cct, env->dpp, io, driver);

        std::string version;
        std::string key = rgw::sal::get_key_in_cache(
            get_prefix(
                block->cacheObj.bucketName, block->cacheObj.objName, version),
            std::to_string(block->blockID), std::to_string(block->size));
        ASSERT_EQ(
            0, cacheDriver->put(
                   env->dpp, key, bl, TEST_DATA_LENGTH, attrs,
                   optional_yield{yield}));
        policyDriver->get_cache_policy()->update(
            env->dpp, key, 0, TEST_DATA_LENGTH, "", false,
            rgw::d4n::RefCount::NOOP, optional_yield{yield});
        ...
        request req;
        req.push(
            "HGET", "RedisCache/testBucket#testName#0#9",
            RGW_CACHE_ATTR_LOCAL_WEIGHT);
        req.push("FLUSHALL");
...
TEST_F(LFUDAPolicyFixture, EvictionYield)
{
  boost::asio::spawn(
      ...
      [this](boost::asio::yield_context yield) {
        ...
        rgw::d4n::PolicyDriver policyDriver(
            conn, &testDriver, "lfuda", optional_yield{yield});
        ...
        /* Set head blocks */
        std::string victimHeadObj = get_prefix(
            victim.cacheObj.bucketName, victim.cacheObj.objName, victim.version);
        ASSERT_EQ(
            0, testDriver.put(
                   env->dpp, victimHeadObj, bl, TEST_DATA_LENGTH, attrs,
                   optional_yield{yield}));

        std::string victimKeyInCache = rgw::sal::get_key_in_cache(
            get_prefix(
                victim.cacheObj.bucketName, victim.cacheObj.objName,
                victim.version),
            std::to_string(victim.blockID), std::to_string(TEST_DATA_LENGTH));
        ASSERT_EQ(
            0, testDriver.put(
                   env->dpp, victimKeyInCache, bl, TEST_DATA_LENGTH, attrs,
                   optional_yield{yield}));
        policyDriver.get_cache_policy()->update(
            env->dpp, victimKeyInCache, 0, TEST_DATA_LENGTH, victim.version,
            false, rgw::d4n::RefCount::NOOP, optional_yield{yield});
        ...
        { /* Avoid sending victim block to remote cache since no network is available */
          ...
          request req;
          req.push(
              "HSET", "lfuda", "minLocalWeights_sum", "10",
              "minLocalWeights_size", "1");
        ...
        EXPECT_EQ(
            policyDriver.get_cache_policy()->eviction(
                env->dpp, block->size, optional_yield{yield}),
            0);

        std::string victimKey = victim.cacheObj.bucketName + "_version_" +
                                victim.cacheObj.objName + "_" +
                                std::to_string(victim.blockID) + "_" +
                                std::to_string(victim.size);
        std::string key = block->cacheObj.bucketName + "_" +
                          block->cacheObj.objName + "_" +
                          std::to_string(block->blockID) + "_" +
                          std::to_string(block->size);
        boost::system::error_code ec;
        request req;
        req.push("EXISTS", "RedisCache/" + victimKeyInCache);
        req.push(
            "EXISTS", "RedisCache/" + victimKeyInCache + "_" +
                          std::to_string(victim.blockID) + "_" +
                          std::to_string(TEST_DATA_LENGTH));
        req.push(
            "EXISTS", victim.cacheObj.bucketName + "_" +
                          victim.cacheObj.objName + "_" +
                          std::to_string(victim.blockID) + "_" +
                          std::to_string(TEST_DATA_LENGTH));
        ...
        std::string victimKeyInPolicy = victim.cacheObj.bucketName +
                                        "#version#" + victim.cacheObj.objName +
                                        "#" + std::to_string(victim.blockID) +
                                        "#" + std::to_string(victim.size);
        EXPECT_EQ(
            policyDriver.get_cache_policy()->exist_key(victimKeyInPolicy), 0);
  ...
  boost::asio::spawn(
      ...
      [this](boost::asio::yield_context yield) {
        ...
        response<
            ...
            std::string, boost::redis::ignore_t>
            resp;
