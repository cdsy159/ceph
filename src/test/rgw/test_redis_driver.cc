...
TEST_F(RedisDriverFixture, PutYield)
{
  boost::asio::spawn(
      ...
      [this](boost::asio::yield_context yield) {
        ASSERT_EQ(
            0, cacheDriver->put(
                   env->dpp, "testName", bl, bl.length(), attrs, yield));
  ...
  boost::asio::spawn(
      ...
      [this](boost::asio::yield_context yield) {
...

        {
          ...
          request req;
          req.push(
              "HSET", "RedisCache/testName", "data", "new data", "attr",
              "newVal");
          response<int> resp;
        ...
        ASSERT_EQ(
            0, cacheDriver->get(
                   env->dpp, "testName", 0, bl.length(), ret, retAttrs, yield));
  ...
  boost::asio::spawn(
      ...
      [this](boost::asio::yield_context yield) {
        std::unique_ptr<rgw::Aio> aio =
            rgw::make_throttle(env->cct->_conf->rgw_get_obj_window_size, yield);
        auto completed = cacheDriver->put_async(
            env->dpp, yield, aio.get(), "testName", bl, bl.length(), attrs, 0,
            0);
...
TEST_F(RedisDriverFixture, GetAsyncYield)
{
  boost::asio::spawn(
      ...
      [this](boost::asio::yield_context yield) {
        ...
        std::unique_ptr<rgw::Aio> aio =
            rgw::make_throttle(env->cct->_conf->rgw_get_obj_window_size, yield);
        auto completed = cacheDriver->get_async(
            env->dpp, yield, aio.get(), "testName", 0, bl.length(), 0, 0);
        drain(env->dpp, aio.get());
...
TEST_F(RedisDriverFixture, RenameYield)
{
  boost::asio::spawn(
      ...
      [this](boost::asio::yield_context yield) {
        ASSERT_EQ(
            0, cacheDriver->put(
                   env->dpp, "testName", bl, bl.length(), attrs,
                   optional_yield{yield}));
        ASSERT_EQ(
            0, cacheDriver->rename(
                   env->dpp, "testName", "newTestName", optional_yield{yield}));
        cacheDriver->shutdown();
...
TEST_F(RedisDriverFixture, SetAttrsYield)
{
  boost::asio::spawn(
      ...
      [this](boost::asio::yield_context yield) {
        ...
        ASSERT_EQ(
            0, cacheDriver->set_attrs(env->dpp, "testName", newAttrs, yield));
        cacheDriver->shutdown();
...
TEST_F(RedisDriverFixture, GetAttrsYield)
{
  boost::asio::spawn(
      ...
      [this](boost::asio::yield_context yield) {
        ...
        ASSERT_EQ(
            0, cacheDriver->put(
                   env->dpp, "testName", bl, bl.length(), nextAttrs, yield));

        {
          boost::system::error_code ec;
          request req;
          req.push(
              "HSET", "RedisCache/testName", "attr", "newVal1", "nextAttr",
              "newVal2");
          response<int> resp;
        ...
        ASSERT_EQ(
            0, cacheDriver->get_attrs(env->dpp, "testName", retAttrs, yield));
...
TEST_F(RedisDriverFixture, UpdateAttrsYield)
{
  boost::asio::spawn(
      ...
      [this](boost::asio::yield_context yield) {
        ...
        ASSERT_EQ(
            0, cacheDriver->update_attrs(env->dpp, "testName", newAttrs, yield));
        cacheDriver->shutdown();
...
TEST_F(RedisDriverFixture, DeleteAttrsYield)
{
  boost::asio::spawn(
      ...
      [this](boost::asio::yield_context yield) {
        ...
        ASSERT_GE(
            cacheDriver->delete_attrs(env->dpp, "testName", delAttrs, yield), 0);
        cacheDriver->shutdown();
      ...
      [this](boost::asio::yield_context yield) {
        ASSERT_EQ(
            0, cacheDriver->put(
                   env->dpp, "testName", bl, bl.length(), attrs, yield));
        ASSERT_GE(
            cacheDriver->set_attr(
                env->dpp, "testName", "newAttr", "newVal", yield),
            0);
        cacheDriver->shutdown();
      ...
      [this](boost::asio::yield_context yield) {
        ...
        ASSERT_EQ(
            0, cacheDriver->get_attr(
                   env->dpp, "testName", "attr", attr_val, yield));
