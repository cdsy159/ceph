...
TEST(RGWCksum, DigestSha1)
{
  ...
  for (const auto input_str : {&lorem, &dolor}) {
    ...
    digest->Update(
        (const unsigned char*)input_str->c_str(), input_str->length());
    ...
    ASSERT_TRUE(memcmp(buf, cksum.hex().c_str(), cksum.hex().length()) == 0);
...
TEST(RGWCksum, DigestSha256)
{
  ...
  for (const auto input_str : {&lorem, &dolor}) {
    ...
    if (input_str == &lorem) {
      ...
      ASSERT_EQ(
          cksum.to_base64(),
          "MTZhYmE1MzkzYWQ3MmMwMDQxZjU2MDBhZDNjMmM1MmVjNDM3YTJmMGM3ZmMwOGZhZGZj"
          "M2MwZmU5NjQxZDdhMw==");
    } else { // &dolor
      /* compare w/known value, openssl sha1 */
      ASSERT_EQ(
      ...
      ASSERT_EQ(
          cksum.to_base64(),
          "MmQ4YzJmNmQ5NzhjYTIxNzEyYjVmNmRlMzZjOWQzMWZhOGU5NmE0ZmE1ZDhmZjhiMDE4"
          "OGRmYjllN2MxNzFiYg==");
      /* compare with aws-sdk-cpp encoded value */
      ASSERT_EQ(
          cksum.to_armor(), "LYwvbZeMohcStfbeNsnTH6jpak+l2P+LAYjfuefBcbs=");
    }
  }
}
...
TEST(RGWCksum, DigestSha512)
{
  ...
  for (const auto input_str : {&lorem, &dolor}) {
    ...
    if (input_str == &lorem) {
      ...
      ASSERT_EQ(
          cksum.hex(),
          "b1f4aaa6b51c19ffbe4b1b6fa107be09c8acafd7c768106a3faf475b1e27a940d3c0"
          "75fda671eadf46c68f93d7eabcf604bcbf7055da0dc4eae6743607a2fc3f");
      /* compare w/known value https://www.base64encode.org/ */
      ASSERT_EQ(
          cksum.to_base64(),
          "YjFmNGFhYTZiNTFjMTlmZmJlNGIxYjZmYTEwN2JlMDljOGFjYWZkN2M3NjgxMDZhM2Zh"
          "ZjQ3NWIxZTI3YTk0MGQzYzA3NWZkYTY3MWVhZGY0NmM2OGY5M2Q3ZWFiY2Y2MDRiY2Jm"
          "NzA1NWRhMGRjNGVhZTY3NDM2MDdhMmZjM2Y=");
    } else { // &dolor
      /* compare w/known value, openssl sha1 */
      ASSERT_EQ(
          cksum.hex(),
          "8ba760cac29cb2b2ce66858ead169174057aa1298ccd581514e6db6dee3285280ee6"
          "e3a54c9319071dc8165ff061d77783100d449c937ff1fb4cd1bb516a69b9");
      /* compare w/known value https://www.base64encode.org/ */
      ASSERT_EQ(
          cksum.to_base64(),
          "OGJhNzYwY2FjMjljYjJiMmNlNjY4NThlYWQxNjkxNzQwNTdhYTEyOThjY2Q1ODE1MTRl"
          "NmRiNmRlZTMyODUyODBlZTZlM2E1NGM5MzE5MDcxZGM4MTY1ZmYwNjFkNzc3ODMxMDBk"
          "NDQ5YzkzN2ZmMWZiNGNkMWJiNTE2YTY5Yjk=");
    }
  }
}
...
TEST(RGWCksum, DigestBlake3)
{
  ...
  for (const auto input_str : {&lorem, &dolor}) {
    ...
    if (input_str == &lorem) {
      ...
      ASSERT_EQ(
          cksum.hex(),
          "f1da5f4e2bd5669307bcdb2e223dad05af7425207cbee59e73526235f50f76ad");
      /* compare w/known value https://www.base64encode.org/ */
      ASSERT_EQ(
          cksum.to_base64(),
          "ZjFkYTVmNGUyYmQ1NjY5MzA3YmNkYjJlMjIzZGFkMDVhZjc0MjUyMDdjYmVlNTllNzM1"
          "MjYyMzVmNTBmNzZhZA==");
    } else { // &dolor
      /* compare w/known value, b3sum */
      ASSERT_EQ(
          cksum.hex(),
          "71fe44583a6268b56139599c293aeb854e5c5a9908eca00105d81ad5e22b7bb6");
      /* compare w/known value https://www.base64encode.org/ */
      ASSERT_EQ(
          cksum.to_base64(),
          "NzFmZTQ0NTgzYTYyNjhiNTYxMzk1OTljMjkzYWViODU0ZTVjNWE5OTA4ZWNhMDAxMDVk"
          "ODFhZDVlMjJiN2JiNg==");
    }
  }
} /* blake3 */
...
TEST(RGWCksum, DigestSTR)
{
  for (auto t : {t1, t2, t3, t4, t5, t6, t7}) {
    ...
    auto cksum = rgw::cksum::finalize_digest(digest, t);
    if (verbose) {
      std::cout << "type: " << to_string(t) << " digest: " << cksum.to_string()
                << std::endl;
    }
  }
}
...
TEST(RGWCksum, DigestBL)
{
  ...
  for ([[maybe_unused]] const auto& ix : {1, 2}) {
    dolor_bl.push_back(
        buffer::create_static(dolor.length(), const_cast<char*>(dolor.data())));
  }
...
TEST(RGWCksum, CRC64NVME_UNDIGEST)
{
  ...
  uint64_t crc2 = spdk_crc64_nvme(
      (const unsigned char*)lacrimae.c_str(), lacrimae.length(), 0ULL);
  ASSERT_EQ(crc1, crc2);
}
...
TEST(RGWCksum, CRC64NVME_COMBINE1)
{
  ...
  uint64_t crc1 = spdk_crc64_nvme(
      (const unsigned char*)dolor.c_str(), dolor.length(), 0ULL);
...
TEST(RGWCksum, CRC64NVME_COMBINE2)
{
  ...
  uint64_t crc2 = spdk_crc64_nvme(
      (const unsigned char*)lorem.c_str(), lorem.length(), 0ULL);

  uint64_t crc3 = spdk_crc64_nvme(
      (const unsigned char*)dolorem.c_str(), dolorem.length(), 0ULL);
...
TEST(RGWCksum, CRC64NVME_COMBINE3)
{
  ...
  uint64_t spdk_crc1 = spdk_crc64_nvme(
      (const unsigned char*)dolor.c_str(), dolor.length(), 0ULL);
  auto cksum_crc1 = rgw::digest::byteswap(std::get<uint64_t>(*cksum1.get_crc()));
  ...
  uint64_t spdk_crc2 = spdk_crc64_nvme(
      (const unsigned char*)lorem.c_str(), lorem.length(), 0ULL);
  auto cksum_crc2 = rgw::digest::byteswap(std::get<uint64_t>(*cksum2.get_crc()));
  ...
  uint64_t spdk_crc3 = spdk_crc64_nvme(
      (const unsigned char*)dolorem.c_str(), dolorem.length(), 0ULL);
  auto cksum_crc3 = rgw::digest::byteswap(std::get<uint64_t>(*cksum3.get_crc()));
...
TEST(RGWCksum, CRC32_COMBINE3)
{
  ...
  uint32_t madler_crc1 = crc32iso_hdlc_word(
      0U, (const unsigned char*)dolor.c_str(), dolor.length());

  auto cksum_crc1 = rgw::digest::byteswap(std::get<uint32_t>(*cksum1.get_crc()));
  ...
  uint32_t madler_crc2 = crc32iso_hdlc_word(
      0U, (const unsigned char*)lorem.c_str(), lorem.length());
  auto cksum_crc2 = rgw::digest::byteswap(std::get<uint32_t>(*cksum2.get_crc()));
  ...
  uint32_t madler_crc3 = crc32iso_hdlc_word(
      0U, (const unsigned char*)dolorem.c_str(), dolorem.length());
  auto cksum_crc3 = rgw::digest::byteswap(std::get<uint32_t>(*cksum3.get_crc()));
  ...
  if (verbose) {
    std::cout << "\ncrc1/dolor spdk: " << madler_crc1
              ...
              << " cksum_crc3: " << cksum_crc3
              << "\ncrc4/crc1+crc2: " << cksum_crc4 << std::endl;
  }
...
TEST(RGWCksum, CRC32C_COMBINE3)
{
  ...
  uint32_t madler_crc1 =
      crc32iscsi_word(0U, (const unsigned char*)dolor.c_str(), dolor.length());
  ...
  uint32_t madler_crc2 =
      crc32iscsi_word(0U, (const unsigned char*)lorem.c_str(), lorem.length());
  auto cksum_crc2 = rgw::digest::byteswap(std::get<uint32_t>(*cksum2.get_crc()));
  ...
  uint32_t madler_crc3 = crc32iscsi_word(
      0U, (const unsigned char*)dolorem.c_str(), dolorem.length());
  auto cksum_crc3 = rgw::digest::byteswap(std::get<uint32_t>(*cksum3.get_crc()));
...
TEST(RGWCksum, CtorUnarmor)
{
  ...
  digest->Update((const unsigned char*)lorem.c_str(), lorem.length());

  auto cksum1 = rgw::cksum::finalize_digest(digest, t);
  auto armored_text1 = cksum1.to_armor();
  auto cksum2 = rgw::cksum::Cksum(
      cksum1.type, armored_text1.c_str(),
...
TEST(RGWCksum, Combiner1)
{
  ...
  if (verbose) {
    ...
    std::cout << "\ncomposite cksum (delorem) " << "\n\tcksum-type "
              << to_string(t) << "\n\tflags " << cksum_flags
              << "\n\tarmored cksum " << cksum4.to_armor() << std::endl;
  }
} /* Combiner1 */

using cksum_4tuple = std::tuple<
    rgw::cksum::Cksum,
    rgw::cksum::Cksum,
    rgw::cksum::Cksum,
    rgw::cksum::Cksum>;
...
public:
  ...
  SetUpTestSuite()
  {
    ...
    for (const auto& elt : {ST(a, long_a), ST(b, long_b), ST(c, long_c)}) {
...
TEST_F(CksumCombinerFixture, Test1)
{
  ...
  for (const auto t : CksumCombinerFixture::cksum_types) {
    ...
    std::cout << "\ncomposite cksum (long_a+long_b+long_c) "
              ...
              << get<0>(cksums).to_armor() << "\n\tarmored cksum2 "
              << get<1>(cksums).to_armor() << "\n\tarmored cksum3 "
              << get<2>(cksums).to_armor() << "\n\tflags " << cksum_flags
  ...
  for (auto arg_iter = args.begin(); arg_iter != args.end();) {
    if (ceph_argparse_flag(args, arg_iter, "--verbose", (char*)nullptr)) {
      verbose = true;
        ...
        ceph_argparse_flag(args, arg_iter, "--gen_test_data", (char*)nullptr)) {
      gen_test_data = true;
    } else {
      ++arg_iter;
    }
