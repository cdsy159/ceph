    ...
    bool is_default)
{
  ...
  pinfo.storage_classes.set_storage_class(
      RGW_STORAGE_CLASS_STANDARD, &data_pool, nullptr);
...
void
test_rgw_init_env(RGWZoneGroup* zonegroup, RGWZoneParams* zone_params)
{
  test_rgw_add_placement(zonegroup, zone_params, "default-placement", true);
}

void
test_rgw_populate_explicit_placement_bucket(
    rgw_bucket* b,
    const char* t,
    const char* n,
    const char* dp,
...
std::string
test_rgw_get_obj_oid(const rgw_obj& obj)
{
  std::string oid;
  std::string loc;
...
void
test_rgw_init_explicit_placement_bucket(rgw_bucket* bucket, const char* name)
{
  test_rgw_populate_explicit_placement_bucket(
      bucket, "", name, ".data-pool", ".index-pool", "marker", "bucket-id");
}

void
test_rgw_init_old_bucket(old_rgw_bucket* bucket, const char* name)
{
  test_rgw_populate_old_bucket(
      bucket, "", name, ".data-pool", ".index-pool", "marker", "bucket-id");
}
...
void
test_rgw_init_bucket(rgw_bucket* bucket, const char* name)
{
