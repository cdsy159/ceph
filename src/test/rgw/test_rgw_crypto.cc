...
public:
  int
  process(bufferlist&& bl, uint64_t ofs) override
  {
...
TEST(TestRGWCrypto, verify_AES_256_CBC_identity)
{
  ...
  for (unsigned int step : {1, 2, 3, 5, 7, 11, 13, 17}) {
    ...
    for (size_t r = 97; r < 123; r++) {
      ...
      ASSERT_TRUE(aes->encrypt(
          input, begin, end - begin, encrypted, offset, null_yield));
      bufferlist decrypted;
      ASSERT_TRUE(aes->decrypt(
          encrypted, 0, end - begin, decrypted, offset, null_yield));

      ASSERT_EQ(decrypted.length(), end - begin);
      ASSERT_EQ(
          std::string_view(input.c_str() + begin, end - begin),
...
TEST(TestRGWCrypto, verify_AES_256_CBC_identity_2)
{
  ...
  for (unsigned int step : {1, 2, 3, 5, 7, 11, 13, 17}) {
    ...
    for (off_t end = 1; end < 6096; end += 3) {
      ...
      bufferlist encrypted;
      ASSERT_TRUE(
          aes->encrypt(input, begin, end, encrypted, offset, null_yield));
      bufferlist decrypted;
      ASSERT_TRUE(
          aes->decrypt(encrypted, 0, end, decrypted, offset, null_yield));

      ASSERT_EQ(decrypted.length(), end);
      ASSERT_EQ(
          std::string_view(input.c_str(), end),
...
TEST(TestRGWCrypto, verify_AES_256_CBC_identity_3)
{
  ...
  for (unsigned int step : {1, 2, 3, 5, 7, 11, 13, 17}) {
    ...
    for (size_t r = 97; r < 123; r++) {
      ...
      off_t chunk;
      while (pos < end) {
        chunk = block_size + (rr / 3) * (rr + 17) * (rr + 71) * (rr + 123) *
                                 (rr + 131) % 50000;
        ...
        bufferlist tmp;
        ASSERT_TRUE(
            aes->encrypt(input, pos, chunk, tmp, offset + pos, null_yield));
      ...
      pos = begin;
      while (pos < end) {
        chunk = block_size + (rr / 3) * (rr + 97) * (rr + 151) * (rr + 213) *
                                 (rr + 251) % 50000;
        chunk = chunk - chunk % block_size;
        if (pos + chunk > end)
          chunk = end - pos;
      ...
      ASSERT_EQ(
          std::string_view(encrypted1.c_str(), end),
...
TEST(TestRGWCrypto, verify_AES_256_CBC_size_0_15)
{
  ...
  for (unsigned int step : {1, 2, 3, 5, 7, 11, 13, 17}) {
    ...
    for (size_t r = 97; r < 123; r++) {
      ...
      ASSERT_TRUE(
          aes->encrypt(encrypted, 0, end, decrypted, offset, null_yield));
...
TEST(TestRGWCrypto, verify_RGWGetObj_BlockDecrypt_ranges)
{
  ...
  for (off_t r = 93; r < 150; r++) {
    ...
    RGWGetObj_BlockDecrypt decrypt(
        &no_dpp, g_ceph_context, &get_sink, std::move(cbc), {}, null_yield);

    //random ranges
    off_t begin = (r / 3) * r * (r + 13) * (r + 23) * (r + 53) * (r + 71) %
                  test_range;
    off_t end = begin +
                (r / 5) * (r + 7) * (r + 13) * (r + 101) * (r * 103) %
                    (test_range - begin) -
                1;
...
range_t
fixup_range(RGWGetObj_BlockDecrypt* decrypt, off_t ofs, off_t end)
{
...
TEST(TestRGWCrypto, check_RGWGetObj_BlockDecrypt_fixup)
{
  ...
  RGWGetObj_BlockDecrypt decrypt(
      &no_dpp, g_ceph_context, &get_sink, std::move(nonecrypt), {}, null_yield);
...
std::vector<size_t>
create_mp_parts(size_t obj_size, size_t mp_part_len)
{
...
TEST(TestRGWCrypto, check_RGWGetObj_BlockDecrypt_fixup_simple)
{
  ...
  RGWGetObj_BlockDecrypt decrypt(
      &no_dpp, g_ceph_context, &get_sink, std::move(nonecrypt),
      create_mp_parts(obj_size, part_size), null_yield);
  ...
  ASSERT_EQ(
      fixup_range(&decrypt, 0, part_size + 1), range_t(0, part_size + 4095));

  // request bytes spanning 2 parts
  ASSERT_EQ(
      fixup_range(&decrypt, part_size - 2, part_size + 2),
      range_t(part_size - 4096, part_size + 4095));

  // request last byte
  ASSERT_EQ(
      fixup_range(&decrypt, obj_size - 1, obj_size - 1),
...
TEST(TestRGWCrypto, check_RGWGetObj_BlockDecrypt_fixup_non_aligned_obj_size)
{
  ...
  RGWGetObj_BlockDecrypt decrypt(
      &no_dpp, g_ceph_context, &get_sink, std::move(nonecrypt),
      create_mp_parts(na_obj_size, part_size), null_yield);
  ...
  ASSERT_EQ(
      fixup_range(&decrypt, na_obj_size - 2, na_obj_size - 1),
      range_t(na_obj_size - 1 - 4096, na_obj_size - 1));

  // request last byte, spans last 1B part only
  ASSERT_EQ(
      fixup_range(&decrypt, na_obj_size - 1, na_obj_size - 1),
...
TEST(TestRGWCrypto, check_RGWGetObj_BlockDecrypt_fixup_non_aligned_part_size)
{
  ...
  RGWGetObj_BlockDecrypt decrypt(
      &no_dpp, g_ceph_context, &get_sink, std::move(nonecrypt),
      create_mp_parts(obj_size, na_part_size), null_yield);

  // na_part_size -2, ie. part_size -1  is aligned to 4095 boundary
  ASSERT_EQ(
      fixup_range(&decrypt, 0, na_part_size - 2), range_t(0, na_part_size - 2));
  // even though na_part_size -1 should not align to a 4095 boundary, the range
  // should not span the next part
  ASSERT_EQ(
      fixup_range(&decrypt, 0, na_part_size - 1), range_t(0, na_part_size - 1));

  ASSERT_EQ(
      fixup_range(&decrypt, 0, na_part_size), range_t(0, na_part_size + 4095));
  ASSERT_EQ(
      fixup_range(&decrypt, 0, na_part_size + 1),
      range_t(0, na_part_size + 4095));

  // request spanning 2 parts
  ASSERT_EQ(
      fixup_range(&decrypt, na_part_size - 2, na_part_size + 2),
...
TEST(TestRGWCrypto, check_RGWGetObj_BlockDecrypt_fixup_non_aligned)
{
  ...
  const size_t na_part_size = part_size + 1;
  const size_t na_obj_size = obj_size +
                             7; // (6*(5MiB + 1) + 1) for the last 1B overflow

  ut_get_sink get_sink;
  auto nonecrypt = std::make_unique<BlockCryptNone>(4096);
  RGWGetObj_BlockDecrypt decrypt(
      &no_dpp, g_ceph_context, &get_sink, std::move(nonecrypt),
      create_mp_parts(na_obj_size, na_part_size), null_yield);
...
TEST(TestRGWCrypto, check_RGWGetObj_BlockDecrypt_fixup_invalid_ranges)
{
  ...
  ASSERT_EQ(
      fixup_range(&decrypt, obj_size - 1, obj_size + 100),
      range_t(obj_size - 4096, obj_size - 1));
  ASSERT_EQ(
      fixup_range(&decrypt, obj_size, obj_size + 1),
...
TEST(TestRGWCrypto, verify_RGWPutObj_BlockEncrypt_chunks)
{
  ...
  for (off_t r = 93; r < 150; r++) {
    ...
    RGWPutObj_BlockEncrypt encrypt(
        &no_dpp, g_ceph_context, &put_sink, std::move(cbc), null_yield);

    off_t test_size = (r / 5) * (r + 7) * (r + 13) * (r + 101) * (r * 103) %
                          (test_range - 1) +
                      1;
    off_t pos = 0;
    do {
    ...
    ASSERT_EQ(
        std::string_view(decrypted.c_str(), test_size),
...
TEST(TestRGWCrypto, verify_Encrypt_Decrypt)
{
  ...
  do {
    ...
    ut_put_sink put_sink;
    RGWPutObj_BlockEncrypt encrypt(
        &no_dpp, g_ceph_context, &put_sink,
        AES_256_CBC_create(&no_dpp, g_ceph_context, &key[0], 32), null_yield);
    bufferlist bl;
    ...
    ut_get_sink get_sink;
    RGWGetObj_BlockDecrypt decrypt(
        &no_dpp, g_ceph_context, &get_sink,
