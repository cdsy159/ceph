...
TEST_CASE("buf_to_hex", "[rgw]")
{
  ...
  SECTION("C character array overload")
  {
    SECTION("empty input")
    {
      ...
      buf_to_hex(
          (const unsigned char* const)(in_empty), std::strlen(in_empty),
          out_data);
  ...
  SECTION("std::array<> overload")
  {
    ...
    constexpr array<unsigned char, 1 + out_expected.length()>
        out_array_expected = {"41616263646566676841"};
    ...
    CHECK(std::lexicographical_compare(
        begin(out_array_expected), end(out_array_expected), begin(out_data),
        end(out_data)));
  }
}
...
TEST_CASE("hex_to_buf", "[rgw]")
{
  ...
  SECTION("symmetry")
  {
    ...
    CHECK(std::equal(
        begin(in_array), end(in_array), begin(out_data1), i + begin(out_data1)));
  }
}
