...
public:
  ...
  bool
  is_owner_of(const rgw_owner& owner) const override
  {
    ceph_abort();
    return false;
  }
...
public:
  PolicyTest() { cct.reset(new CephContext(CEPH_ENTITY_TYPE_CLIENT), false); }
};
...
TEST_F(PolicyTest, Eval1)
{
  ...
  ARN arn3(
      Partition::aws, Service::s3, "", arbitrary_tenant, "erroneous_bucket");
...
TEST_F(PolicyTest, Eval2)
{
  ...
  auto trueacct =
      FakeIdentity(Principal::account("ACCOUNT-ID-WITHOUT-HYPHENS"));

  auto notacct = FakeIdentity(Principal::account("some-other-account"));
  for (auto i = 0ULL; i < s3All; ++i) {
    ARN arn1(Partition::aws, Service::s3, "", arbitrary_tenant, "mybucket");
    EXPECT_EQ(p.eval(e, trueacct, i, arn1), Effect::Allow);
    ARN arn2(
        Partition::aws, Service::s3, "", arbitrary_tenant, "mybucket/myobject");
...
TEST_F(PolicyTest, Parse3)
{
  ...
  EXPECT_EQ(
      (p->statements[2].resource.begin() + 1)->resource, "confidential-data/*");
...
TEST_F(PolicyTest, Eval3)
{
  ...
  for (auto op = 0ULL; op < s3All; ++op) {
    ...
    ARN arn3(
        Partition::aws, Service::s3, "", arbitrary_tenant, "confidential-data");
    EXPECT_EQ(p.eval(em, none, op, arn3), Effect::Pass);
    ARN arn4(
        Partition::aws, Service::s3, "", arbitrary_tenant, "confidential-data");
    EXPECT_EQ(
        p.eval(tr, none, op, arn4), s3allow[op] ? Effect::Allow : Effect::Pass);
    ARN arn5(
        Partition::aws, Service::s3, "", arbitrary_tenant, "confidential-data");
    ...
    ARN arn7(
        ...
        "confidential-data/moo");
    EXPECT_EQ(
        p.eval(tr, none, op, arn7), s3allow[op] ? Effect::Allow : Effect::Pass);
    ARN arn8(
        Partition::aws, Service::s3, "", arbitrary_tenant,
        "confidential-data/moo");
...
TEST_F(PolicyTest, Eval4)
{
  ...
  ARN arn1(
      Partition::aws, Service::iam, "", arbitrary_tenant, "role/example_role");
...
{
  auto p =
      get_managed_policy(cct.get(), "arn:aws:iam::aws:policy/IAMFullAccess");
  ASSERT_TRUE(p);
...
{
  auto p = get_managed_policy(
      cct.get(), "arn:aws:iam::aws:policy/IAMReadOnlyAccess");
  ASSERT_TRUE(p);
...
{
  auto p = get_managed_policy(
      cct.get(), "arn:aws:iam::aws:policy/AmazonSNSFullAccess");
  ASSERT_TRUE(p);
...
{
  auto p = get_managed_policy(
      cct.get(), "arn:aws:iam::aws:policy/AmazonSNSReadOnlyAccess");
  ASSERT_TRUE(p);
...
{
  auto p = get_managed_policy(
      cct.get(), "arn:aws:iam::aws:policy/AmazonS3FullAccess");
  ASSERT_TRUE(p);
...
{
  auto p = get_managed_policy(
      cct.get(), "arn:aws:iam::aws:policy/AmazonS3ReadOnlyAccess");
  ASSERT_TRUE(p);
...
protected:
  ...
  // 192.168.1.0/24
  const rgw::IAM::MaskedIP allowedIPv4Range = {
      false, rgw::IAM::Address("11000000101010000000000100000000"), 24};
  // 192.168.1.1/32
  const rgw::IAM::MaskedIP blocklistedIPv4 = {
      false, rgw::IAM::Address("11000000101010000000000100000001"), 32};
  // 2001:db8:85a3:0:0:8a2e:370:7334/128
  const rgw::IAM::MaskedIP allowedIPv6 = {
      true,
      rgw::IAM::Address(
          "00100000000000010000110110111000100001011010001100000000000000000000"
          "000000000000100010100010111000000011011100000111001100110100"),
      128};
  // ::1
  ...
  const rgw::IAM::MaskedIP allowedIPv6Range = {
      ...
      rgw::IAM::Address(
          "00100000000000010000110110111000100001011010001100000000000000000000"
          "000000000000100010100010111000000011011100000111001100110000"),
      124};

public:
  IPPolicyTest() { cct.reset(new CephContext(CEPH_ENTITY_TYPE_CLIENT), false); }
};
...
{
  auto actualIPv6Range =
      rgw::IAM::Condition::as_network("2001:db8:85a3:0:0:8a2e:370:7330/124");
...
{
  auto actualIPv6 =
      rgw::IAM::Condition::as_network("2001:db8:85a3:0:0:8a2e:370:7334");
...
TEST_F(IPPolicyTest, asNetworkInvalid)
{
  ...
  EXPECT_FALSE(
      rgw::IAM::Condition::as_network("2001:db8:85a3:0:0:8a2e:370:7334/129"));
...
class DumbUser : public rgw::sal::StoreUser {
  ...
  int
  read_attrs(const DoutPrefixProvider*, optional_yield)
  {
    return -ENOTSUP;
  }
  ...
  virtual int
  trim_usage(const DoutPrefixProvider*, uint64_t, uint64_t, optional_yield)
  {
    return -ENOTSUP;
  }
  ...
  int
  store_user(const DoutPrefixProvider*, optional_yield, bool, RGWUserInfo*)
  {
    return -ENOTSUP;
  }

  int
  remove_user(const DoutPrefixProvider*, optional_yield)
  {
    return -ENOTSUP;
  }

  int
  verify_mfa(const std::string&, bool*, const DoutPrefixProvider*, optional_yield)
  {
    return -ENOTSUP;
  }
...
TEST_F(IPPolicyTest, IPEnvironment)
{
  ...
  ASSERT_EQ(
      cct.get()->_conf.set_val("rgw_remote_addr_param", "HTTP_X_FORWARDED_FOR"),
      0);
  ...
  rgw_env.set(
      ...
      "192.168.1.4, 4.3.2.1, 2001:db8:85a3:8d3:1319:8a2e:370:7348");
  rgw_req_state.env.clear();
  rgw_build_iam_environment(&rgw_req_state);
  ip = rgw_req_state.env.find("aws:SourceIp");
  ASSERT_NE(ip, rgw_req_state.env.end());
...
TEST_F(IPPolicyTest, ParseIPAddress)
{
  ...
  EXPECT_EQ(p->statements[0].resource.begin()->resource, "example_bucket");
  EXPECT_EQ(
      (p->statements[0].resource.begin() + 1)->resource, "example_bucket/*");
  EXPECT_TRUE(p->statements[0].notresource.empty());
...
TEST_F(IPPolicyTest, EvalIPAddress)
{
  ...
  EXPECT_EQ(
      allowp.eval(blocklistedIPv6, trueacct, s3ListBucket, arn4), Effect::Pass);
  ...
  EXPECT_EQ(
      denyp.eval(blocklistedIP, trueacct, s3ListBucket, arn7), Effect::Pass);
  ARN arn8(
      Partition::aws, Service::s3, "", arbitrary_tenant,
      "example_bucket/myobject");
  EXPECT_EQ(
      denyp.eval(blocklistedIP, trueacct, s3ListBucket, arn8), Effect::Pass);

  ARN arn9(Partition::aws, Service::s3, "", arbitrary_tenant, "example_bucket");
  EXPECT_EQ(
      denyp.eval(blocklistedIPv6, trueacct, s3ListBucket, arn9), Effect::Pass);
  ARN arn10(
      Partition::aws, Service::s3, "", arbitrary_tenant,
      "example_bucket/myobject");
  EXPECT_EQ(
      denyp.eval(blocklistedIPv6, trueacct, s3ListBucket, arn10), Effect::Pass);
  ARN arn11(Partition::aws, Service::s3, "", arbitrary_tenant, "example_bucket");
  EXPECT_EQ(
      denyp.eval(allowedIPv6, trueacct, s3ListBucket, arn11), Effect::Deny);
  ARN arn12(
      Partition::aws, Service::s3, "", arbitrary_tenant,
      "example_bucket/myobject");
  EXPECT_EQ(
      denyp.eval(allowedIPv6, trueacct, s3ListBucket, arn12), Effect::Deny);
  ...
  EXPECT_EQ(
      fullp.eval(blocklistedIP, trueacct, s3ListBucket, arn15), Effect::Pass);
  ARN arn16(
      Partition::aws, Service::s3, "", arbitrary_tenant,
      "example_bucket/myobject");
  EXPECT_EQ(
      fullp.eval(blocklistedIP, trueacct, s3ListBucket, arn16), Effect::Pass);

  ARN arn17(Partition::aws, Service::s3, "", arbitrary_tenant, "example_bucket");
  EXPECT_EQ(
      fullp.eval(allowedIPv6, trueacct, s3ListBucket, arn17), Effect::Allow);
  ARN arn18(
      Partition::aws, Service::s3, "", arbitrary_tenant,
      "example_bucket/myobject");
  EXPECT_EQ(
      fullp.eval(allowedIPv6, trueacct, s3ListBucket, arn18), Effect::Allow);

  ARN arn19(Partition::aws, Service::s3, "", arbitrary_tenant, "example_bucket");
  EXPECT_EQ(
      fullp.eval(blocklistedIPv6, trueacct, s3ListBucket, arn19), Effect::Pass);
  ARN arn20(
      Partition::aws, Service::s3, "", arbitrary_tenant,
      "example_bucket/myobject");
  EXPECT_EQ(
      fullp.eval(blocklistedIPv6, trueacct, s3ListBucket, arn20), Effect::Pass);
}
...
TEST(MatchWildcards, Asterisk)
{
  ...
  EXPECT_TRUE(match_wildcards(
      "http://*.example.com", "http://www.example.com", MATCH_CASE_INSENSITIVE));
  EXPECT_FALSE(
      match_wildcards("http://*.example.com", "http://www.Example.com"));
  EXPECT_TRUE(match_wildcards(
      "http://*.example.com", "http://www.Example.com", MATCH_CASE_INSENSITIVE));
  EXPECT_TRUE(
      match_wildcards("http://example.com/*", "http://example.com/index.html"));
  EXPECT_TRUE(match_wildcards(
      "http://example.com/*/*.jpg", "http://example.com/fun/smiley.jpg"));
...
Action_t
set_range_bits(std::uint64_t start, std::uint64_t end)
{
  Action_t result;
...
TEST(set_cont_bits, iamconsts)
{
  ...
  EXPECT_EQ(
      s3objectlambdaAllValue, set_range_bits(s3All + 1, s3objectlambdaAll));
...
TEST_F(ConditionTest, StringNotEqualsLogic)
{
  ...
  // Should return false because value equals at least one condition value
  {
    Condition stringNotEquals{
        TokenID::StringNotEquals, key.data(), key.size(), false};
...
TEST_F(ConditionTest, NumericNotEqualsLogic)
{
  ...
  // Should return false because value equals at least one condition value
  {
    Condition numericNotEquals{
        TokenID::NumericNotEquals, key.data(), key.size(), false};
...
TEST_F(ConditionTest, DateNotEqualsLogic)
{
  ...
  // Should return false because value equals at least one condition value
  {
    Condition dateNotEquals{
        TokenID::DateNotEquals, key.data(), key.size(), false};
...
TEST_F(ConditionTest, StringNotLikeLogic)
{
  ...
  // Should return false because value matches at least one condition pattern
  {
    Condition stringNotLike{
        TokenID::StringNotLike, key.data(), key.size(), false};
