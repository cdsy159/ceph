...
TEST_F(TestSSEKMS, test_transit_key_version_extraction)
{
  const NoDoutPrefix no_dpp(cct, 1);
  string json =
      R"({"data": {"keys": {"6": "8qgPWvdtf6zrriS5+nkOzDJ14IGVR6Bgkub5dJn6qeg="}}})";
  EXPECT_CALL(
      *old_engine, send_request(
                       &no_dpp, StrEq("GET"), StrEq(""), StrEq("1/2/3/4/5/6"),
                       StrEq(""), _, _))
      .WillOnce(SetPointedValue(json));

  std::string actual_key;
  std::string tests[11]{"/",          "my_key/",   "my_key",    "",
                        "my_key/a",   "my_key/1a", "my_key/a1", "my_key/1a1",
                        "my_key/1/a", "1",         "my_key/1/"};

  int res;
  for (const auto& test : tests) {
    res = old_engine->get_key(
        &no_dpp, std::string_view(test), null_yield, actual_key);
    ASSERT_EQ(res, -EINVAL);
  }

  res = old_engine->get_key(
      &no_dpp, std::string_view("1/2/3/4/5/6"), null_yield, actual_key);
  ASSERT_EQ(res, 0);
  ASSERT_EQ(
      actual_key, from_base64("8qgPWvdtf6zrriS5+nkOzDJ14IGVR6Bgkub5dJn6qeg="));
}
...
TEST_F(TestSSEKMS, test_transit_backend)
{
  ...
  string json =
      R"({"data": {"keys": {"1": "8qgPWvdtf6zrriS5+nkOzDJ14IGVR6Bgkub5dJn6qeg="}}})";
  const NoDoutPrefix no_dpp(cct, 1);
  EXPECT_CALL(
      *old_engine,
      send_request(
          &no_dpp, StrEq("GET"), StrEq(""), StrEq("my_key/1"), StrEq(""), _, _))
      .WillOnce(SetPointedValue(json));
...
TEST_F(TestSSEKMS, test_transit_makekey)
{
  ...
  EXPECT_CALL(
      *transit_engine, send_request(
                           &no_dpp, StrEq("POST"), StrEq("/datakey/plaintext/"),
                           StrEq("my_key"), _, _, _))
  ...
  int res =
      transit_engine->make_actual_key(&no_dpp, attrs, null_yield, actual_key);
  std::string cipher_text{get_str_attribute(attrs, RGW_ATTR_CRYPT_DATAKEY)};

  ASSERT_EQ(res, 0);
  ASSERT_EQ(
      actual_key, from_base64("3xfTra/dsIf3TMa3mAT2IxPpM7YWm/NvUb4gDfSDX4g="));
  ASSERT_EQ(
      cipher_text,
      "vault:v2:HbdxLnUztGVo+RseCIaYVn/4wEUiJNT6GQfw57KXQmhXVe7i1/"
...
TEST_F(TestSSEKMS, test_transit_reconstitutekey)
{
  ...
  // Mocks the expected return Value from Vault Server using custom Argument Action
  set_attr(
      attrs, RGW_ATTR_CRYPT_DATAKEY,
      "vault:v2:HbdxLnUztGVo+RseCIaYVn/4wEUiJNT6GQfw57KXQmhXVe7i1/"
      "kgLWegEPg1I6lexhIuXAM6Q2YvY0aZ");
  string post_json =
      R"({"data": {"key_version": 1,"plaintext": "3xfTra/dsIf3TMa3mAT2IxPpM7YWm/NvUb4gDfSDX4g="}})";
  EXPECT_CALL(
      *transit_engine,
      send_request(
          &no_dpp, StrEq("POST"), StrEq("/decrypt/"), StrEq("my_key"), _, _, _))
      .WillOnce(SetPointedValue(post_json));

  set_attr(attrs, RGW_ATTR_CRYPT_CONTEXT, R"({"aws:s3:arn": "fred"})");
  set_attr(attrs, RGW_ATTR_CRYPT_KEYID, my_key);

  int res = transit_engine->reconstitute_actual_key(
...
TEST_F(TestSSEKMS, test_kv_backend)
{
  ...
  // Mocks the expected return value from Vault Server using custom Argument Action
  string json =
      R"({"data": {"data": {"key": "8qgPWvdtf6zrriS5+nkOzDJ14IGVR6Bgkub5dJn6qeg="}}})";
  EXPECT_CALL(
      *kv_engine,
      send_request(
          &no_dpp, StrEq("GET"), StrEq(""), StrEq("my_key"), StrEq(""), _, _))
      .WillOnce(SetPointedValue(json));
...
TEST_F(TestSSEKMS, string_ends_maybe_slash)
{
  struct {
    ...
    bool expected;
      ...
      {"jack here", "fred", false},    {"here is a fred", "fred", true},
      {"and a fred/", "fred", true},   {"no fred here", "fred", false},
...
TEST_F(TestSSEKMS, test_transit_backend_empty_response)
{
  ...
  EXPECT_CALL(
      *old_engine, send_request(
                       &no_dpp, StrEq("GET"), StrEq(""),
                       StrEq("/key/nonexistent/1"), StrEq(""), _, _))
      .WillOnce(SetPointedValue(json));
