...
TEST(TestRGWReshard, calculate_preferred_shards)
{
  ...
  RGWBucketReshard::calculate_preferred_shards(
      nullptr, 1999, 101, 100000, false, 10000000, 200, needs_resharding,
      &suggested_shard_count);

  ASSERT_EQ(false, needs_resharding)
      << "no need to reshard when shards are half-used";


  RGWBucketReshard::calculate_preferred_shards(
      nullptr, 1999, 101, 100000, false, 20200000, 200, needs_resharding,
      &suggested_shard_count, false);
  ASSERT_EQ(true, needs_resharding);
  ASSERT_EQ(404, suggested_shard_count)
      << "number of shards when primes are not preferred";

  RGWBucketReshard::calculate_preferred_shards(
      nullptr, 1999, 101, 100000, false, 20200000, 200, needs_resharding,
      &suggested_shard_count, true);
  ASSERT_EQ(true, needs_resharding);
  ASSERT_EQ(409, suggested_shard_count)
      << "number of shards when primes are preferred";

  RGWBucketReshard::calculate_preferred_shards(
      nullptr, 1999, 101, 100000, true, 20200000, 200, needs_resharding,
      &suggested_shard_count, true);
  ASSERT_EQ(true, needs_resharding);
  ...
  RGWBucketReshard::calculate_preferred_shards(
      nullptr, 1999, 3, 100000, false, 650000, 700, needs_resharding,
      &suggested_shard_count, true);
  // 650,000 objs across 700 shards -> <1000 objs per shard; 650000 /
  // 50000 = 13
  ASSERT_EQ(true, needs_resharding);
  ASSERT_EQ(13, suggested_shard_count)
      << "shard reduction without hitting min_layout_shards";

  RGWBucketReshard::calculate_preferred_shards(
      nullptr, 1999, 3, 100000, false, 350000, 400, needs_resharding,
      &suggested_shard_count, true);
  // 350,000 objs across 400 shards -> <1000 objs per shard; 350000 /
  // 50000 = 7, but hard-coded minimum of 11
  ASSERT_EQ(true, needs_resharding);
  ASSERT_EQ(11, suggested_shard_count)
      << "shard reduction and hitting hard-coded minimum of 11";

  RGWBucketReshard::calculate_preferred_shards(
      nullptr, 1999, 51, 100000, false, 650000, 700, needs_resharding,
      &suggested_shard_count, true);
  // 650,000 objs across 700 shards -> <1000 objs per shard; 650000 /
  // 50000 = 13, but bucket min of 51
  ASSERT_EQ(true, needs_resharding);
  ASSERT_EQ(51, suggested_shard_count)
      << "shard reduction and hitting min_layout_shards";
}
