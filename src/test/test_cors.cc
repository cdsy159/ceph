...
#define BUCKET_URL                         \
  ((g_test->get_key_type() == KEY_TYPE_S3) \
...
extern "C" int
ceph_armor(char* dst, const char* dst_end, const char* src, const char* end);
...
print_usage(char* exec)
{
  ...
  cout << "Options:\n"
          ...
          "-s3 <AWSAccessKeyId:SecretAccessKeyID> - Only, if the key type is "
          "S3, gives S3 credentials\n"
          "-swift <Auth-Token> - Only if the key type is SWIFT, and gives the "
          "SWIFT credentials\n";
}
...
public:
  test_cors_helper() :
    curl_inst(NULL), resp_data(NULL), resp_code(0), kt(KEY_TYPE_UNDEFINED)
  {
...
int
test_cors_helper::extract_input(unsigned argc, char* argv[])
{
...
void
test_cors_helper::set_response(char* r)
{
  string sr(r), h, v;
  size_t off = sr.find(": ");
  if (off != string::npos) {
...
delete_bucket(void)
{
  if (g_test->get_key_type() == KEY_TYPE_S3) {
    ...
    g_test->send_request(
        string("DELETE"), string("/swift/v1/" SWIFT_BUCKET_NAME));
    if (g_test->get_resp_code() != 204U) {
      cout << "Error deleting bucket, http code " << g_test->get_resp_code();
      return -1;
    }
...
xml_to_cors_rule(string s)
{
  ...
  cors_config =
      (RGWCORSConfiguration_S3*)parser.find_first("CORSConfiguration");
  if (!cors_config) {
    return NULL;
  }
...
size_t
cors_read_xml(void* ptr, size_t s, size_t n, void* ud)
{
    ...
    unsigned max_age)
{
  if (g_test->get_key_type() == KEY_TYPE_S3) {
    ...
    g_test->send_request(
        string("PUT"), string("/" S3_BUCKET_NAME "?cors"), cors_read_xml,
    ...
    g_test->set_extra_header(
        string("X-Container-Meta-Access-Control-Allow-Origin: ") + a_o);
    ...
    if (!h.empty()) {
      ...
      g_test->set_extra_header(
          string("X-Container-Meta-Access-Control-Allow-Headers: ") + a_h);
    }
    if (!e.empty()) {
      string e_h;
      ...
      g_test->set_extra_header(
          string("X-Container-Meta-Access-Control-Expose-Headers: ") + e_h);
    }
    if (max_age != CORS_MAX_AGE_INVALID) {
      char age[32];
      sprintf(age, "%u", max_age);
      g_test->set_extra_header(
          string("X-Container-Meta-Access-Control-Max-Age: ") + string(age));
    }
    //const char *data = "1";
    stringstream ss;
    ss << "1";
    g_test->send_request(
        string("POST"), string("/swift/v1/" SWIFT_BUCKET_NAME), cors_read_xml,
        (void*)&ss, 1);
  }
}
...
TEST(TestCORS, putcors_firsttime)
{
  ...
  EXPECT_EQ(
      ((g_test->get_key_type() == KEY_TYPE_SWIFT) ? 202U : 200U),
  ...
  if (g_test->get_key_type() == KEY_TYPE_S3) {
    ...
    EXPECT_TRUE(
        (r->get_allowed_methods() & (RGW_CORS_GET | RGW_CORS_PUT)) ==
...
TEST(TestCORS, optionscors_test_options_4)
{
  ...
  g_test->set_extra_header(string("Access-Control-Request-Method: GET"));
  g_test->set_extra_header(
      string("Access-Control-Allow-Headers: Header2, Header1"));
  g_test->send_request(string("OPTIONS"), BUCKET_URL);
  EXPECT_EQ(200U, g_test->get_resp_code());
  if (g_test->get_resp_code() == 200) {
    string s = g_test->get_response(string("Access-Control-Allow-Origin"));
  ...
  g_test->set_extra_header(string("Access-Control-Request-Method: GET"));
  g_test->set_extra_header(
      string("Access-Control-Allow-Headers: Header1, Header2"));
  g_test->send_request(string("OPTIONS"), BUCKET_URL);
  EXPECT_EQ(200U, g_test->get_resp_code());
  if (g_test->get_resp_code() == 200) {
    string s = g_test->get_response(string("Access-Control-Allow-Origin"));
  ...
  g_test->set_extra_header(string("Access-Control-Request-Method: GET"));
  g_test->set_extra_header(
      string("Access-Control-Allow-Headers: Header1, Header2, Header3"));
  g_test->send_request(string("OPTIONS"), BUCKET_URL);
  EXPECT_EQ(200U, g_test->get_resp_code());
  if (g_test->get_resp_code() == 200) {
    string s = g_test->get_response(string("Access-Control-Allow-Origin"));
...
TEST(TestCORS, optionscors_test_options_7)
{
  ...
  g_test->set_extra_header(string("Origin: example.com"));
  g_test->set_extra_header(string("Access-Control-Request-Method: GET"));
  g_test->set_extra_header(string(
