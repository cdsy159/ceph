...
struct shard_id_t {
  ...
  static void
  generate_test_instances(std::list<shard_id_t*>& ls)
  {
...
struct pg_shard_t {
  ...
  static void
  generate_test_instances(std::list<pg_shard_t*>& o)
  {
...
struct ModeCollectorTestB : public ::testing::Test {
  ...
  void
  expect_wrong_ID(const DT& test_case)
  {
    const auto r = mc.find_mode();
    EXPECT_EQ(r.tag, test_case.expected.tag);
    if (r.tag == mode_status_t::authorative_value) {
