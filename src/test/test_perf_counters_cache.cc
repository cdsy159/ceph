...
void
add_test_counters(PerfCountersBuilder* pcb)
{
...
create_test_counters(const std::string& name, CephContext* cct)
{
  PerfCountersBuilder pcb(
      cct, name, TEST_PERFCOUNTERS1_ELEMENT_FIRST,
      TEST_PERFCOUNTERS1_ELEMENT_LAST);
...
static PerfCountersCache*
setup_test_perf_counters_cache(CephContext* cct, uint64_t target_size = 100)
{
  return new PerfCountersCache(cct, target_size, create_test_counters);
}

void
cleanup_test(PerfCountersCache* pcc)
{
  delete pcc;
}
...
TEST(PerfCountersCache, NoCacheTest)
{
  ...
  ASSERT_EQ("{}\n", message);
  ASSERT_EQ(
      "", client.do_request(R"({ "prefix": "counter schema" })", &message));
  ASSERT_EQ("{}\n", message);
}
...
TEST(PerfCountersCache, TestLabelStrings)
{
  ...
  std::string label1 =
      key_create("good_ctrs", {{"label3", "val3"}, {"label2", "val4"}});
  ...
  std::string label2 =
      key_create("bad_ctrs1", {{"label3", "val4"}, {"label1", ""}});
  pcc->set_counter(label2, TEST_PERFCOUNTERS_COUNTER, 2);

  std::string label3 =
      key_create("bad_ctrs2", {{"", "val4"}, {"label1", "val1"}});
