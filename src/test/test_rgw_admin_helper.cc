...
test_helper::test_helper() :
  curl_inst(0), resp_data(NULL), resp_code(0)
{
  curl_global_init(CURL_GLOBAL_ALL);
}

test_helper::~test_helper() { curl_global_cleanup(); }
    ...
    size_t length)
{
  ...
  if (curl_inst) {
    ...
    curl_easy_setopt(
        curl_inst, CURLOPT_HEADERFUNCTION, admin_helper::write_header);
...
string&
test_helper::get_response(string hdr)
{
  return response[hdr];
}

void
test_helper::set_extra_header(string hdr)
{
...
void
test_helper::set_response_data(char* data, size_t len)
{
  if (resp_data)
    delete resp_data;
...
string&
test_helper::get_rgw_admin_path()
{
  return rgw_admin_path;
}

string&
test_helper::get_ceph_conf_path()
{
  return conf_path;
}

void
test_helper::set_creds(string& c)
{
  creds = c;
}

const string*
test_helper::get_response_data()
{
  return resp_data;
}

unsigned
test_helper::get_resp_code()
{
  return resp_code;
}
...
int
account_create(string& account_id, string& account_name)
{
  stringstream ss;
...
int
account_info(string& account_id, RGWAccountInfo& a_info)
{
  stringstream ss;
  ss << "-c " << g_test->get_ceph_conf_path()
...
int
account_rm(string& account_id)
{
  stringstream ss;
  ss << "-c " << g_test->get_ceph_conf_path()
     << " account rm --account-id=" << account_id;
  string cmd = ss.str();
  string out;
  if (run_rgw_admin(cmd, out) != 0) {
...
int
caps_add(const string caps_name, const string uid, const char* perm)
{
  stringstream ss;

  ss << "-c " << g_test->get_ceph_conf_path()
     << " caps add --caps=" << caps_name << "=" << perm << " --uid=" << uid;
  string out;
  string cmd = ss.str();
  if (run_rgw_admin(cmd, out) != 0) {
...
int
caps_rm(const string caps_name, const string uid, const char* perm)
{
  stringstream ss;

  ss << "-c " << g_test->get_ceph_conf_path() << " caps rm --caps=" << caps_name
     << "=" << perm << " --uid=" << uid;
  string out;
  string cmd = ss.str();
  if (run_rgw_admin(cmd, out) != 0) {
...
int
compare_access_keys(RGWAccessKey& k1, RGWAccessKey& k2)
{
...
int
compare_user_info(RGWUserInfo& i1, RGWUserInfo& i2, const string meta_caps)
{
  int rv;
...
size_t
meta_read_json(void* ptr, size_t s, size_t n, void* ud)
{
