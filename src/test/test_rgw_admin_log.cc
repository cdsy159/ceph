...
public:
  ...
  void
  set_response_data(char* data, size_t len)
  {
    if (resp_data)
      delete resp_data;
...
int
test_helper::extract_input(int argc, char* argv[])
{
...
void
test_helper::set_response(char* r)
{
  string sr(r), h, v;
  size_t off = sr.find(": ");
  if (off != string::npos) {
...
size_t
write_header(void* ptr, size_t size, size_t nmemb, void* ud)
{
...
size_t
write_data(void* ptr, size_t size, size_t nmemb, void* ud)
{
  test_helper* h = static_cast<test_helper*>(ud);
...
/* destination should be CEPH_CRYPTO_HMACSHA1_DIGESTSIZE bytes long */
{
  ...
  admin_log::buf_to_hex(
      (unsigned char*)dest, CEPH_CRYPTO_HMACSHA1_DIGESTSIZE, hex_str);
}
    ...
    string& out)
{
  ...
  if (off != string::npos) {
    ...
    admin_log::calc_hmac_sha1(
        secret.c_str(), secret.length(), auth_hdr.c_str(), auth_hdr.length(),
        hmac_sha1);
    int ret = ceph_armor(
        b64, b64 + 64, hmac_sha1, hmac_sha1 + CEPH_CRYPTO_HMACSHA1_DIGESTSIZE);
    if (ret < 0) {
...
get_date(string& d)
{
  ...
  sprintf(
      date, "%s, %d %s %d %d:%d:%d GMT", days[tm.tm_wday], tm.tm_mday,
      months[tm.tm_mon], tm.tm_year + 1900, tm.tm_hour, tm.tm_min, tm.tm_sec);
  d = date;
}

int
test_helper::send_request(
    string method,
    string res,
    ...
    size_t length)
{
  ...
  if (curl_inst) {
    ...
    for (list<string>::iterator it = extra_hdrs.begin(); it != extra_hdrs.end();
         ++it) {
    ...
    if (res != CURLE_OK) {
      cout << "Curl perform failed for " << url
...
int
run_rgw_admin(string& cmd, string& resp)
{
  pid_t pid;
  pid = fork();
  if (pid == 0) {
    ...
    if (stat(RGW_ADMIN_RESP_PATH, &st) < 0) {
      cout << "Error stating the admin response file, errno " << errno
           << std::endl;
      return -1;
    } else {
...
int
get_creds(string& json, string& creds)
{
  JSONParser parser;
...
int
user_create(string& uid, string& display_name, bool set_creds = true)
{
  stringstream ss;
  string creds;
...
int
user_info(string& uid, string& display_name, RGWUserInfo& uinfo)
{
  stringstream ss;
...
int
user_rm(string& uid, string& display_name)
{
  stringstream ss;
...
int
caps_add(const char* name, const char* perm)
{
  stringstream ss;

  ss << "-c " << g_test->get_ceph_conf_path() << " caps add --caps=" << name
     << "=" << perm << " --uid=" << uid;
  string out;
  string cmd = ss.str();
  if (run_rgw_admin(cmd, out) != 0) {
...
int
caps_rm(const char* name, const char* perm)
{
  stringstream ss;

  ss << "-c " << g_test->get_ceph_conf_path() << " caps rm --caps=" << name
     << "=" << perm << " --uid=" << uid;
  string out;
  string cmd = ss.str();
  if (run_rgw_admin(cmd, out) != 0) {
...
size_t
read_dummy_post(void* ptr, size_t s, size_t n, void* ud)
{
  int dummy = 0;
...
size_t
read_bucket_object(void* ptr, size_t s, size_t n, void* ud)
{
...
static int
put_bucket_obj(const char* obj_name, char* data, unsigned len)
{
  string req = "/" TEST_BUCKET_NAME "/";
  req.append(obj_name);
  g_test->send_request(
      string("PUT"), req, read_bucket_object, (void*)data, (size_t)len);
  if (g_test->get_resp_code() != 200U) {
    cout << "Error sending object to the bucket, http_code "
         << g_test->get_resp_code();
    return -1;
  }
  return 0;
}
...
static int
read_bucket_obj(const char* obj_name)
{
  string req = "/" TEST_BUCKET_NAME "/";
  req.append(obj_name);
...
static int
delete_obj(const char* obj_name)
{
  string req = "/" TEST_BUCKET_NAME "/";
  req.append(obj_name);
  g_test->send_request(string("DELETE"), req);
  if (g_test->get_resp_code() != 204U) {
    cout << "Error deleting object from bucket, http_code "
         << g_test->get_resp_code();
    return -1;
  }
  return 0;
...
int
parse_json_resp(JSONParser& parser)
{
  string* resp;
...
static int
decode_json(JSONObj* obj, RGWMetadataLogData& data)
{
  JSONObj* jo;
...
static int
decode_json(JSONObj* obj, cls_log_entry_json& ret)
{
...
static int
get_log_list(list<cls_log_entry_json>& entries)
{
  JSONParser parser;
...
static int
decode_json(JSONObj* obj, cls_bilog_entry& ret)
{
...
static int
get_bilog_list(list<cls_bilog_entry>& entries)
{
  JSONParser parser;
  if (parse_json_resp(parser) != 0)
    return -1;
...
static int
decode_json(JSONObj* obj, rgw_data_change& ret)
{
  string entity;
...
static int
get_datalog_list(list<rgw_data_change>& entries)
{
  JSONParser parser;
...
unsigned
get_mdlog_shard_id(string& key, int max_shards)
{
  string section = "user";
...
unsigned
get_datalog_shard_id(const char* bucket_name, int max_shards)
{
  uint32_t r = ceph_str_hash_linux(bucket_name, strlen(bucket_name)) %
               max_shards;
  return (int)r;
}
...
TEST(TestRGWAdmin, datalog_list)
{
  ...
  string rest_req;
  unsigned shard_id = get_datalog_shard_id(
      TEST_BUCKET_NAME, g_ceph_context->_conf->rgw_data_log_num_shards);
  stringstream ss;
  ...
  EXPECT_EQ(
      put_bucket_obj(TEST_BUCKET_OBJECT, bucket_obj, TEST_BUCKET_OBJECT_SIZE),
      0);
  sleep(1);
...
TEST(TestRGWAdmin, datalog_lock_unlock)
{
  ...
  g_test->send_request(
      string("POST"), rest_req, read_dummy_post, NULL, sizeof(int));
  ...
  rest_req =
      "/admin/log?type=data&lock&id=1&length=3&locker-id=ceph1&zone-id=1";
  g_test->send_request(
      string("POST"), rest_req, read_dummy_post, NULL, sizeof(int));
  EXPECT_EQ(200U, g_test->get_resp_code());
  ...
  rest_req =
      "/admin/log?type=data&lock&id=1&length=3&locker-id=ceph1&zone-id=2";
  g_test->send_request(
      string("POST"), rest_req, read_dummy_post, NULL, sizeof(int));
  EXPECT_EQ(500U, g_test->get_resp_code());
...
TEST(TestRGWAdmin, mdlog_list)
{
  ...
  string rest_req;
  unsigned shard_id =
      get_mdlog_shard_id(uid, g_ceph_context->_conf->rgw_md_log_max_shards);
  stringstream ss;
  ...
  ss.str("");
  ss << "/admin/log?type=metadata&id=" << shard_id
  ...
  ss.str("");
  ss << "/admin/log?type=metadata&id=" << shard_id
     << "&start-time=" << start_time << "&max-entries=" << 1;
  rest_req = ss.str();
  g_test->send_request(string("GET"), rest_req);
  EXPECT_EQ(200U, g_test->get_resp_code());
  entries.clear();
  ...
  ss.str("");
  ss << "/admin/log?type=metadata&id=" << shard_id
     << "&start-time=" << start_time << "&max-entries=" << 6;
  rest_req = ss.str();
  g_test->send_request(string("GET"), rest_req);
  EXPECT_EQ(200U, g_test->get_resp_code());
  entries.clear();
...
TEST(TestRGWAdmin, mdlog_lock_unlock)
{
  ...
  rest_req =
      "/admin/log?type=metadata&lock&id=1&length=3&locker-id=ceph&zone-id=1";
  g_test->send_request(
      string("POST"), rest_req, read_dummy_post, NULL, sizeof(int));
  EXPECT_EQ(200U, g_test->get_resp_code());
  ...
  rest_req =
      "/admin/log?type=metadata&lock&id=1&length=3&locker-id=ceph1&zone-id=1";
  g_test->send_request(
      string("POST"), rest_req, read_dummy_post, NULL, sizeof(int));
  EXPECT_EQ(200U, g_test->get_resp_code());
  ...
  rest_req =
      "/admin/log?type=metadata&lock&id=1&length=3&locker-id=ceph&zone-id=2";
  g_test->send_request(
      string("POST"), rest_req, read_dummy_post, NULL, sizeof(int));
  EXPECT_EQ(500U, g_test->get_resp_code());
...
TEST(TestRGWAdmin, bilog_list)
{
  ...
  ASSERT_TRUE(bucket_obj != NULL);
  EXPECT_EQ(
      put_bucket_obj(TEST_BUCKET_OBJECT_1, bucket_obj, TEST_BUCKET_OBJECT_SIZE),
      0);
  free(bucket_obj);
