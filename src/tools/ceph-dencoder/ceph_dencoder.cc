...
usage(ostream& out)
{
  ...
  out << "  add_crc32c          calculate and encode crc32c for in-memory "
         "object\n";
  ...
  out << "  get_struct_compat   print the oldest version of decoder that can "
         "decode the encoded object\n";
  out << "\n";
  out << "  copy                copy object (via operator=)\n";
  out << "  copy_ctor           copy object (via copy ctor)\n";
  out << "\n";
  out << "  count_tests         print number of generated test objects (to "
         "stdout)\n";
  out << "  select_test <n>     select generated test object as in-memory "
         "object\n";
  out << "  is_deterministic    exit w/ success if type encodes "
         "deterministically\n";
}

vector<DencoderPlugin>
load_plugins()
{
  ...
  for (auto& entry : fs::directory_iterator(mod_dir)) {
    ...
    if (entry.path().stem().string().compare(
            0, DENC_MOD_PREFIX.size(), DENC_MOD_PREFIX) != 0) {
      continue;
    }
...
main(int argc, const char** argv)
{
  ...
  for (std::vector<const char*>::iterator i = args.begin(); i != args.end();
    ...
    if (*i == string("help") || *i == string("-h") || *i == string("--help")) {
      ...
      if (fd < 0) {
        cerr << "error opening " << *i << " for write: " << cpp_strerror(errno)
             << std::endl;
        return 1;
      }
