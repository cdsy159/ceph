...
public:
  DencoderPlugin(const fs::path& path)
  {
    ...
    if (mod == nullptr) {
      std::cerr << "failed to dlopen(" << path << "): " << dlerror()
                << std::endl;
    }
  }

  DencoderPlugin(DencoderPlugin&& other) :
    mod{other.mod}, dencoders{std::move(other.dencoders)}
  {
    other.mod = nullptr;
  ...
  register_dencoders()
  {
    ...
    const auto do_register = reinterpret_cast<register_dencoders_t>(
  ...
  void
  emplace(const char* name, Args&&... args)
  {
...
#define TYPE_VARARGS(t, ...) \
  plugin->emplace<DencoderImplNoFeature<t>>(#t, false, false, ##__VA_ARGS__);
#define TYPE_STRAYDATA(t) \
  plugin->emplace<DencoderImplNoFeature<t>>(#t, true, false);
#define TYPE_NONDETERMINISTIC(t) \
  plugin->emplace<DencoderImplNoFeature<t>>(#t, false, true);
#define TYPE_FEATUREFUL(t) \
  plugin->emplace<DencoderImplFeatureful<t>>(#t, false, false);
#define TYPE_FEATUREFUL_STRAYDATA(t) \
  plugin->emplace<DencoderImplFeatureful<t>>(#t, true, false);
#define TYPE_FEATUREFUL_NONDETERMINISTIC(t) \
  plugin->emplace<DencoderImplFeatureful<t>>(#t, false, true);
#define TYPE_FEATUREFUL_NOCOPY(t) \
  plugin->emplace<DencoderImplFeaturefulNoCopy<t>>(#t, false, false);
#define TYPE_NOCOPY(t) \
  plugin->emplace<DencoderImplNoFeatureNoCopy<t>>(#t, false, false);
