...
struct Dencoder {
  ...
  unsigned
  get_struct_v(bufferlist bl, uint64_t seek) const
  {
...
protected:
  ...
  static ptr_type
  make_ptr(T* ptr, do_delete del)
  {
    auto deleter =
        (del == do_delete::yes ? deleter_type{std::default_delete<T>{}}
                               : deleter_type{[](T*) {}});
...
public:
  DencoderBase(bool stray_okay, bool nondeterministic) :
    stray_okay(stray_okay), nondeterministic(nondeterministic)
  {}
  ...
  void
  dump(ceph::Formatter* f) override
  {
    assert(m_object);
    m_object->dump(f);
  }
  ...
  int
  num_generated() override
  {
    return m_list.size();
  }

  std::string
  select_generated(unsigned i) override
  {
    ...
    m_object = make_ptr(
        std::addressof(*std::next(m_list.begin(), i - 1)), do_delete::no);
    return {};
  }

  bool
  is_deterministic() override
  {
...
public:
  ...
  void
  encode(bufferlist& out, uint64_t features) override
  {
    out.clear();
  ...
  decode(bufferlist bl, uint64_t seek) override
  {
    ...
    try {
      ...
      if (n->get_type() != m_object->get_type()) {
        std::stringstream ss;
        ss << "decoded type " << n->get_type() << " instead of expected "
           << m_object->get_type();
...
public:
  ...
  void
  register_dencoder(std::string_view name, Dencoder* denc)
  {
