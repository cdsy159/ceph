...
public:
  ...
  explicit NVMeofGwMapDencoder(const NVMeofGwMap& m) :
    m(m)
  {}
  ...
  static std::list<NVMeofGwMapDencoder>
...
public:
  ...
  explicit MNVMeofGwMapDencoder(const MNVMeofGwMap& m) :
    m(m)
  {}
  ...
  static std::list<MNVMeofGwMapDencoder>
  generate_test_instances()
  {
    ...
    for (int i = 0; i < MAX_SUPPORTED_ANA_GROUPS; i++) {
      pending_map.created_gws[group_key]["GW1"].blocklist_data[i].osd_epoch =
          i * 2;
      pending_map.created_gws[group_key]["GW1"].blocklist_data[i].is_failover =
          false;
    }
...
public:
  ...
  explicit MNVMeofGwBeaconDencoder(const MNVMeofGwBeacon& m) :
    m(m)
  {}
  ...
  static std::list<MNVMeofGwBeaconDencoder>
  generate_test_instances()
  {
