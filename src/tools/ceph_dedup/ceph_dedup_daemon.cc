...
#undef dout_prefix
#define dout_prefix *_dout << "ceph_dedup_daemon: " << __func__ << ": "
...
make_usage()
{
  ...
  op_desc.add_options()("chunk-size", po::value<int>(), ": chunk size (byte)")(
      ...
      ": set pool name")("snap", ": deduplciate snapshotted object")(
      "chunk-dedup-threshold", po::value<int>(),
      ": set the threshold for chunk dedup (number of duplication) ")(
      "sampling-ratio", po::value<int>(),
      ": set the sampling ratio (percentile)")(
      "wakeup-period", po::value<int>(),
      ": set the wakeup period of crawler thread (sec)")(
      "fpstore-threshold", po::value<size_t>()->default_value(100_M),
      ": set max size of in-memory fingerprint store (bytes)")(
...
public:
  ...
  public:
    ...
    entry_t
    increment_reference(entry_t entry)
    {
      ...
      if (entry.entry_into == entry_t::OVER ||
        ...
        auto [over_iter, inserted] = over_threshold_fp_map.insert(*entry);
    ...
    void
    drop_entries_below_threshold()
    {
      under_threshold_fp_map.clear();
    }

    FpMap(size_t dedup_threshold) :
  ...
  public:
    ...
    maybe_print_status()
    {
      ...
      if (next_report != utime_t() && now > next_report) {
        dout(5) << (int)(now - start) << "s : read " << total_bytes
                << " bytes so far..." << dendl;
        next_report = now;
  ...
  private:
    ...
    size_t total_bytes =
        0; // Accessed in the worker threads under fingerprint_lock
    const size_t memory_threshold;
    FpMap<std::string, dup_count_t>
        fp_map; // Accessed in the worker threads under fingerprint_lock
  };
  ...
  public:
    ...
    handle_signal(int signum)
    {
      ...
      case SIGTERM:
        set_all_stop();
        dout(0) << "got a signal(" << signum << "), daemon wil be terminiated"
                << dendl;
        break;
  ...
  private:
    ...
    inline static bool all_stop =
        false; // Accessed in the main thread and in other worker threads under glock
  };
  ...
  size_t
  get_total_duplicated_size() const
  {
    return total_duplicated_size;
  }

  size_t
  get_total_object_size() const
  {
...
AioCompRef
SampleDedupWorkerThread::do_async_evict(const std::string& oid)
{
  Rados rados;
...
std::tuple<std::vector<ObjectItem>, ObjectCursor>
...
std::vector<size_t>
SampleDedupWorkerThread::sample_object(size_t count)
{
...
void
SampleDedupWorkerThread::try_dedup_and_accumulate_result(
    const ObjectItem& object,
    snap_t snap)
{
  bufferlist data = read_object(object);
  if (data.length() == 0) {
    derr << __func__ << " skip object " << object.oid << " read returned size 0"
         << dendl;
    return;
  }
  ...
  for (auto& chunk : chunks) {
    ...
    dout(20) << "generate a chunk (chunk oid: " << chunk_info.oid
             << ", offset: " << chunk_info.start
             << ", length: " << chunk_info.size
             << ", fingerprint: " << chunk_info.fingerprint << ")" << dendl;

    if (sample_dedup_global.fp_store.add(chunk_info)) {
      redundant_chunks.push_back(chunk_info);
      dout(20) << chunk_info.fingerprint
               << "is duplicated, try to perform dedup" << dendl;
    }
  }
...
bufferlist
SampleDedupWorkerThread::read_object(const ObjectItem& object)
{
  ...
  dout(20) << " got object: " << object.oid << " size: " << whole_data.length()
           << dendl;
  return whole_data;
}

std::vector<std::tuple<bufferlist, std::pair<uint64_t, uint64_t>>>
SampleDedupWorkerThread::do_cdc(
    const ObjectItem& object,
    const bufferlist& data) const
{
...
std::string
SampleDedupWorkerThread::generate_fingerprint(const bufferlist& chunk_data) const
{
  std::string ret;
...
int
SampleDedupWorkerThread::do_chunk_dedup(chunk_t& chunk, snap_t snap)
{
  uint64_t size;
  time_t mtime;
  ...
  ObjectReadOperation op;
  op.set_chunk(
      chunk.start, chunk.size, chunk_io_ctx, chunk.fingerprint, 0,
...
int
run_crawling_daemon(const po::variables_map& opts)
{
  ...
  if (ret < 0) {
    derr << "error opening base pool " << base_pool_name << ": "
  ...
  if (ret < 0) {
    derr << "error opening chunk pool " << chunk_pool_name << ": "
         << cpp_strerror(ret) << dendl;
    return -EINVAL;
  }
  ...
  dout(0) << "ceph-dedup-daemon starts ( " << "SampleRatio : " << sampling_ratio
          << ", Chunk Dedup Threshold : " << chunk_dedup_threshold
          << ", Chunk Size : " << chunk_size
          << ", Fingperint Argorithm : " << fp_algo
          << ", Chunk Argorithm : " << chunk_algo
          << ", Chunk Dedup Threshold : " << chunk_dedup_threshold
          << ", Fingerprint Store Threshold : " << fp_threshold << ")" << dendl;
  ...
  while (!state.is_all_stop()) {
    ...
    for (unsigned i = 0; i < max_thread; i++) {
      ...
      io_ctx.object_list_slice(
          begin, end, i, max_thread, &shard_start, &shard_end);

      threads.emplace_back(
          io_ctx, chunk_io_ctx, shard_start, shard_end, chunk_size, fp_algo,
          chunk_algo, state, snap);
    ...
    dout(5) << "Summary: read " << total_size
            << " bytes so far and found saveable space ("
            << total_duplicate_size << " bytes)." << dendl;
...
main(int argc, const char** argv)
{
  ...
  register_async_signal_handler_oneshot(
      SIGINT, SampleDedupWorkerThread::SampleDedupGlobal::handle_signal);
  register_async_signal_handler_oneshot(
      SIGTERM, SampleDedupWorkerThread::SampleDedupGlobal::handle_signal);

  int ret = run_crawling_daemon(opts);

  unregister_async_signal_handler(
      SIGINT, SampleDedupWorkerThread::SampleDedupGlobal::handle_signal);
  unregister_async_signal_handler(
      SIGTERM, SampleDedupWorkerThread::SampleDedupGlobal::handle_signal);
