...
std::string
get_opts_pool_name(const po::variables_map& opts)
{
  if (opts.count("pool")) {
...
std::string
get_opts_chunk_algo(const po::variables_map& opts)
{
...
std::string
get_opts_fp_algo(const po::variables_map& opts)
{
  if (opts.count("fingerprint-algorithm")) {
    ...
    if (fp_algo != "sha1" && fp_algo != "sha256" && fp_algo != "sha512") {
      std::cerr << "unrecognized fingerprint-algorithm " << fp_algo
                << std::endl;
      exit(1);
    }
    return fp_algo;
...
std::string
get_opts_op_name(const po::variables_map& opts)
{
  if (opts.count("op")) {
...
std::string
get_opts_chunk_pool(const po::variables_map& opts)
{
...
std::string
get_opts_object_name(const po::variables_map& opts)
{
...
int
get_opts_max_thread(const po::variables_map& opts)
{
...
int
get_opts_report_period(const po::variables_map& opts)
{
...
std::string
make_pool_str(std::string pool, std::string var, std::string val)
{
...
std::string
make_pool_str(std::string pool, std::string var, int val)
{
