...
)
{
  ...
  try {
    po::command_line_parser parser =
        po::command_line_parser(cmd_args).options(desc_all);
 ...
 */
void
usage(const char* n, po::options_description& d)
{
  std::cerr
      ...
      << "  get monmap [-- options]         get monmap (version VER if "
         "specified)\n"
      << "                                  (default: last committed)\n"
      << "  get osdmap [-- options]         get osdmap (version VER if "
         "specified)\n"
      << "                                  (default: last committed)\n"
      << "  get mdsmap [-- options]         get mdsmap (version VER if "
         "specified)\n"
      << "                                  (default: last committed)\n"
      << "  get mgr [-- options]            get mgr map (version VER if "
         "specified)\n"
      << "                                  (default: last committed)\n"
      << "  get crushmap [-- options]       get crushmap (version VER if "
         "specified)\n"
      << "                                  (default: last committed)\n"
      << "  get-key PREFIX KEY [-- options] get key\n"
      << "  remove-key PREFIX KEY           remove key\n"
      << "  show-versions [-- options]      show the first&last committed "
         "version of map\n"
      << "                                  (show-versions -- --help for more "
         "info)\n"
      << "  dump-keys                       dumps store keys to FILE\n"
      << "                                  (default: stdout)\n"
      << "  dump-paxos [-- options]         dump paxos transactions\n"
      << "                                  (dump-paxos -- --help for more "
         "info)\n"
      << "  dump-trace FILE [-- options]    dump contents of trace file FILE\n"
      << "                                  (dump-trace -- --help for more "
         "info)\n"
      << "  replay-trace FILE [-- options]  replay trace from FILE\n"
      << "                                  (replay-trace -- --help for more "
         "info)\n"
      << "  random-gen [-- options]         add randomly generated ops to the "
         "store\n"
      << "                                  (random-gen -- --help for more "
         "info)\n"
      << "  rewrite-crush [-- options]      add a rewrite commit to the store\n"
      << "                                  (rewrite-crush -- --help for more "
         "info)\n"
    ...
    MonitorDBStore::Transaction* t)
{
  ...
  if (version <= 0) {
    if (last_committed >= (unsigned)-version) {
      ...
      std::cerr << "osdmap-version is less than: -" << last_committed
                << std::endl;
      return EINVAL;
    }
  } else {
    ...
    MonitorDBStore& store)
{
  ...
  cout << "adding pending commit " << pending_v << " " << bl.length()
       << " bytes" << std::endl;
...
static int
update_auth(MonitorDBStore& st, const string& keyring_path)
{
  ...
  for (const auto& k : keyring.get_keys()) {
    ...
    std::transform(
        begin(auth_inc.auth.caps), end(auth_inc.auth.caps),
    ...
    cout << "adding auth for '" << auth_inc.name << "': " << auth_inc.auth
    ...
    if (!mon_ids.empty()) {
      if (mon_ids.size() != monmap.size()) {
        cerr << "Please pass the same number of <mon-ids> to name the hosts "
             << "listed in 'mon_host'. " << mon_ids.size()
             << " mon-id(s) specified, " << "while you have " << monmap.size()
             << " mon hosts." << std::endl;
        return -EINVAL;
      }
      new_names = mon_ids;
...
static int
update_monitor(MonitorDBStore& st)
{
...
//  - creating_pgs
static int
update_creating_pgs(MonitorDBStore& st)
{
  bufferlist bl;
...
//  - mgr_command_desc
static int
update_mgrmap(MonitorDBStore& st)
{
  auto t = make_shared<MonitorDBStore::Transaction>();

  {
    ...
    // mgr expects epoch > 1
    map.epoch++;
    auto initial_modules = get_str_vec(
        g_ceph_context->_conf.get_val<string>("mgr_initial_modules"));
    copy(
        begin(initial_modules), end(initial_modules),
...
static int
update_paxos(MonitorDBStore& st)
{
  const string prefix("paxos");
...
main(int argc, char** argv)
{
  ...
  positional_desc.add_options()(
      ...
      "path to monitor's store")("command", po::value<string>(&cmd), "Command")(
  ...
  if (vm.empty() || vm.count("help") || store_path.empty() || cmd.empty() ||
      *cmd.begin() == '-') {
    usage(argv[0], desc);
    return 1;
  }
  ...
  auto cct = global_init(
      NULL, ceph_options, CEPH_ENTITY_TYPE_MON, CODE_ENVIRONMENT_UTILITY,
  ...
  if (cmd == "dump-keys") {
    ...
    po::variables_map op_vm;
    int r = parse_cmd_args(
        &op_desc, &hidden_op_desc, &op_positional, subcmds, &op_vm);
    if (r < 0) {
      return -r;
    }
    ...
    if (!outpath.empty()) {
      fd = ::open(
          outpath.c_str(), O_WRONLY | O_CREAT | O_TRUNC | O_BINARY, 0666);
      if (fd < 0) {
        std::cerr << "error opening output file: " << cpp_strerror(errno)
                  << std::endl;
        return EINVAL;
      }
    }
    ...
    if (!outpath.empty()) {
      std::cout << "wrote " << map_type << " version " << v << " to " << outpath
                << std::endl;
    }
  } else if (cmd == "get-key") {
    string outpath;
    ...
    hidden_op_desc.add_options()("prefix", po::value<string>(&prefix), "prefix")(
        ...
        if (prefix == "osd_snap") {
          ...
          if (key.starts_with("purged_epoch_")) {
            ...
            std::cout << "first_snap:" << first_snap
                      << " end_snap: " << end_snap << " epoch: " << epoch
                      << std::endl;
          }
        } else {
          std::cerr << "This type of readable key does not exist: " << prefix
                    << std::endl
                    << "You can only specify[osd_snap]" << std::endl;
        }
      } catch (const buffer::error& err) {
        std::cerr << "Could not decode for human readable output (you may still"
                     " use non-readable mode).  Detail: "
    ...
    if (!outpath.empty()) {
      std::cout << "wrote " << prefix << " " << key << " to " << outpath
                << std::endl;
    }
    ...
    hidden_op_desc.add_options()(
        "out,o", po::value<string>(&outpath), "file to write the dump to");
    po::positional_options_description op_positional;
    ...
    hidden_op_desc.add_options()(
        "in,i", po::value<string>(&inpath), "file to write the dump to");
    po::positional_options_description op_positional;
    ...
    out_store.close();
    std::cout << "summary: copied " << total_keys << " keys, using " << total_tx
              << " transactions, totalling " << stringify(byte_u_t(total_size))
              << std::endl;
    ...
    if (r < 0) {
      std::cerr << "error removing (" << prefix << "," << key << ")"
                << std::endl;
      return r;
    }
  } else {
