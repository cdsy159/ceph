...
EventOutput::binary() const
{
  ...
  if (r != 0) {
    r = -errno;
    if (r != -EEXIST) {
      std::cerr << "Error creating output directory: " << cpp_strerror(r)
                << std::endl;
      return r;
    }
  }
  ...
  for (JournalScanner::EventMap::const_iterator i = scan.events.begin();
    ...
    if (auto& le = i->second.log_event; le) {
      ...
      filename << "0x" << std::hex << i->first << std::dec << "_"
               << le->get_type_str() << ".bin";
    ...
    std::ofstream bin_file(
        file_path.c_str(), std::ofstream::out | std::ofstream::binary);
  ...
  std::cerr << "Wrote output to binary files in directory '" << path << "'"
            << std::endl;
...
EventOutput::list() const
{
  for (JournalScanner::EventMap::const_iterator i = scan.events.begin();
    ...
    if (auto& le = i->second.log_event; le) {
      ...
      std::cout << le->get_stamp() << " 0x" << std::hex << i->first << std::dec
                << " " << le->get_type_str() << ": " << " (" << detail << ")"
                << std::endl;
      ...
      std::cout << pi->stamp << " 0x" << std::hex << i->first << std::dec << " "
...
EventOutput::summary() const
{
  ...
  for (std::map<std::string, int>::iterator i = type_count.begin();
       i != type_count.end(); ++i) {
  ...
  if (!scan.errors.empty()) {
    for (JournalScanner::ErrorMap::const_iterator i = scan.errors.begin();
      ...
      std::cout << "  0x" << std::hex << i->first << std::dec << ": "
                << i->second.r << " " << i->second.description << std::endl;
    }
  }
}
