 ...
 */
int
JournalScanner::scan(bool const full)
{
  int r = 0;
...
int
JournalScanner::set_journal_ino()
{
  int r = 0;
  if (type == "purge_queue") {
...
int
JournalScanner::scan_header()
{
  int r;

  bufferlist header_bl;
  std::string header_name = obj_name(0);
  dout(4) << "JournalScanner::scan: reading header object '" << header_name
          << "'" << dendl;
  ...
  if (!((header->trimmed_pos <= header->expire_pos) &&
...
JournalScanner::scan_events()
{
  ...
  dout(10) << std::hex << "Header 0x" << header->trimmed_pos << " 0x"
           << header->expire_pos << " 0x" << header->write_pos << std::dec
           << dendl;
  dout(10) << "Starting journal scan from offset 0x" << std::hex << read_offset
           << std::dec << dendl;
  ...
  for (uint64_t obj_offset = (read_offset / object_size);; obj_offset++) {
    ...
    if (gap) {
      ...
      dout(4) << "Searching for sentinel from 0x" << std::hex << read_offset
              << ", 0x" << read_buf.length() << std::dec << " bytes available"
              << dendl;
      ...
      do {
        ...
        dout(4) << "Data at 0x" << std::hex << read_offset << " = 0x"
                << candidate_sentinel << std::dec << dendl;

        if (candidate_sentinel == JournalStream::sentinel) {
          dout(4) << "Found sentinel at 0x" << std::hex << read_offset
                  << std::dec << dendl;
    ...
    {
      dout(10) << "Parsing data, 0x" << std::hex << read_buf.length()
      ...
      while (true) {
        ...
        try {
          ...
          readable = false;
          dout(4) << "Invalid container encoding at 0x" << std::hex
        ...
        dout(10) << "Attempting decode at 0x" << std::hex << read_offset
                 << std::dec << dendl;
        ...
        dout(10) << "Consumed 0x" << std::hex << consumed << std::dec
                 << " bytes" << dendl;
        if (start_ptr != read_offset) {
          derr << "Bad entry start ptr (0x" << std::hex << start_ptr
               << ") at 0x" << read_offset << std::dec << dendl;
          gap = true;
          gap_start = read_offset;
        ...
        if (is_mdlog) {
          ...
          if (le) {
            dout(10) << "Valid entry at 0x" << std::hex << read_offset
                     << std::dec << dendl;
              ...
              if (sle.expire_pos > read_offset) {
                errors.insert(std::make_pair(
                    read_offset, EventError(
                                     -ERANGE,
                                     "ESubtreeMap has expire_pos ahead of its "
                                     "own position")));
              }
            }

            if (filter.apply(read_offset, *le)) {
              events.insert_or_assign(
                  read_offset, EventRecord(std::move(le), consumed));
            }
          } else {
            valid_entry = false;
          }
          ...
          try {
            ...
            if (filter.apply(read_offset, *pi)) {
              events.insert_or_assign(
                  read_offset, EventRecord(std::move(pi), consumed));
            }
        ...
        if (!valid_entry) {
          dout(10) << "Invalid entry at 0x" << std::hex << read_offset
                   << std::dec << dendl;
          gap = true;
          gap_start = read_offset;
          read_offset += consumed;
  ...
  dout(4) << "Scanned objects, " << objects_missing.size() << " missing, "
          << objects_valid.size() << " valid" << dendl;
  dout(4) << "Events scanned, " << ranges_invalid.size() << " gaps" << dendl;
  dout(4) << "Found " << events_valid.size() << " valid events" << dendl;
  dout(4) << "Selected " << events.size() << " events events for processing"
          << dendl;
 ...
 */
bool
JournalScanner::is_healthy() const
{
  return (
      (!is_mdlog || (pointer_present && pointer_valid)) && header_present &&
      header_valid && ranges_invalid.empty() && objects_missing.empty());
}
 ...
 */
bool
JournalScanner::is_readable() const
{
 ...
 */
std::string
JournalScanner::obj_name(inodeno_t ino, uint64_t offset) const
{
  char name[60];
  snprintf(
      name, sizeof(name), "%llx.%08llx", (unsigned long long)(ino),
      (unsigned long long)offset);
  return std::string(name);
}

std::string
JournalScanner::obj_name(uint64_t offset) const
{
 ...
 */
void
JournalScanner::report(std::ostream& out) const
{
  out << "Overall journal integrity: " << (is_healthy() ? "OK" : "DAMAGED")
      << std::endl;
  ...
  if (ranges_invalid.size()) {
    ...
    for (std::vector<Range>::const_iterator r = ranges_invalid.begin();
      ...
      out << "  0x" << std::hex << r->first << "-" << r->second << std::dec
          << std::endl;
    }
  }
}
