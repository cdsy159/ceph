...
main(int argc, const char** argv)
{
  ...
  general.add_options()("help,h", "produce help message")(
      ...
      "specify metapool, datapools and rank (e.g. "
      "cephfs_metadata_a:cephfs_data_a:0)")(
      "ino,i", po::value<string>(&ino),
      "specify inode. e.g. 1099511627776 or 0x10000000000, you can find it "
      "with cmd, 'ls -i'")("out,o", po::value<string>(&out), "output file")(
      "in", po::value<string>(&in),
      "input file")("yes-i-really-really-mean-it", "need by amend info");
  ...
  if (vm.count("help")) {
    ...
    std::cout << "usage : \n"
              << "  cephfs-meta-injection <listc|showm|showfn|amend|amendfn> "
                 "-r <fsname:rank> -i <ino>"
              << std::endl;
    std::cout << "example : \n"
              << "  amend info of inode(1099531628828)\n"
              << "    cephfs-meta-injection showm -r cephfs_a:0 -i "
                 "1099531628828 -o out\n"
              << "    alter file\n"
              << "    cephfs-meta-injection amend -r cephfs_a:0 -i "
                 "1099531628828 --in out --yes-i-really-mean-it"
              << std::endl;
  ...
  rc = mt.main(
      mode, rank_str, minfo, ino, out, in,
