  ...
  m_asok_hook(new MirrorAdminSocketHook(cct, filesystem, this))
{
  m_service_daemon->add_or_update_fs_attribute(
      m_filesystem.fscid, SERVICE_DAEMON_DIR_COUNT_KEY, (uint64_t)0);
  ...
  PerfCountersBuilder plb(
      m_cct, labels, l_cephfs_mirror_fs_mirror_first,
      l_cephfs_mirror_fs_mirror_last);
  auto prio = m_cct->_conf.get_val<int64_t>("cephfs_mirror_perf_stats_prio");
  plb.add_u64(
      l_cephfs_mirror_fs_mirror_peers, "mirroring_peers", "Mirroring Peers",
      "mpee", prio);
  plb.add_u64(
      l_cephfs_mirror_fs_mirror_dir_count, "directory_count", "Directory Count",
      "dirc", prio);
...
int
FSMirror::init_replayer(PeerReplayer* peer_replayer)
{
...
void
FSMirror::shutdown_replayer(PeerReplayer* peer_replayer)
{
...
void
FSMirror::init(Context* on_finish)
{
  dout(20) << dendl;

  std::scoped_lock locker(m_lock);
  int r = connect(
      g_ceph_context->_conf->name.to_str(), g_ceph_context->_conf->cluster,
      &m_cluster, "", "", m_args);
  if (r < 0) {
    m_init_failed = true;
...
void
FSMirror::shutdown(Context* on_finish)
{
  dout(20) << dendl;
...
void
FSMirror::shutdown_peer_replayers()
{
  dout(20) << dendl;
...
void
FSMirror::init_instance_watcher(Context* on_finish)
{
  dout(20) << dendl;
  ...
  Context* ctx =
      new C_CallbackAdapter<FSMirror, &FSMirror::handle_init_instance_watcher>(
          this);
  m_instance_watcher = InstanceWatcher::create(
      m_ioctx, m_snap_listener, m_ts_listener, m_work_queue);
  m_instance_watcher->init(ctx);
}

void
FSMirror::handle_init_instance_watcher(int r)
{
  dout(20) << ": r=" << r << dendl;
...
void
FSMirror::init_mirror_watcher()
{
  dout(20) << dendl;

  std::scoped_lock locker(m_lock);
  Context* ctx =
      new C_CallbackAdapter<FSMirror, &FSMirror::handle_init_mirror_watcher>(
          this);
  m_mirror_watcher =
      MirrorWatcher::create(m_ioctx, this, m_ts_listener, m_work_queue);
  m_mirror_watcher->init(ctx);
}

void
FSMirror::handle_init_mirror_watcher(int r)
{
  dout(20) << ": r=" << r << dendl;
...
void
FSMirror::shutdown_mirror_watcher()
{
  dout(20) << dendl;

  std::scoped_lock locker(m_lock);
  Context* ctx =
      new C_CallbackAdapter<FSMirror, &FSMirror::handle_shutdown_mirror_watcher>(
          this);
  m_mirror_watcher->shutdown(ctx);
}

void
FSMirror::handle_shutdown_mirror_watcher(int r)
{
  dout(20) << ": r=" << r << dendl;
...
void
FSMirror::shutdown_instance_watcher()
{
  dout(20) << dendl;
  ...
  m_instance_watcher->shutdown(
      new C_AsyncCallback<ContextWQ>(m_work_queue, ctx));
}

void
FSMirror::handle_shutdown_instance_watcher(int r)
{
  dout(20) << ": r=" << r << dendl;
...
void
FSMirror::handle_acquire_directory(string_view dir_path)
{
  ...
  if (m_perf_counters) {
    m_perf_counters->set(
        l_cephfs_mirror_fs_mirror_dir_count, m_directories.size());
  }
}

void
FSMirror::handle_release_directory(string_view dir_path)
{
  dout(5) << ": dir_path=" << dir_path << dendl;

  {
    ...
    if (it != m_directories.end()) {
      ...
      m_service_daemon->add_or_update_fs_attribute(
          m_filesystem.fscid, SERVICE_DAEMON_DIR_COUNT_KEY,
          m_directories.size());
      for (auto& [peer, peer_replayer] : m_peer_replayers) {
        dout(10) << ": peer=" << peer << dendl;
...
void
FSMirror::add_peer(const Peer& peer)
{
  dout(10) << ": peer=" << peer << dendl;
...
void
FSMirror::remove_peer(const Peer& peer)
{
  dout(10) << ": peer=" << peer << dendl;
...
void
FSMirror::mirror_status(Formatter* f)
{
  std::scoped_lock locker(m_lock);
