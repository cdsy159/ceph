...
private:
  ...
  struct ClusterListener : ClusterWatcher::Listener {
    ...
    void
    handle_mirroring_enabled(const FilesystemSpec& spec) override
    {
      mirror->mirroring_enabled(spec.filesystem, spec.pool_id);
    }

    void
    handle_mirroring_disabled(const Filesystem& filesystem) override
    {
      mirror->mirroring_disabled(filesystem);
    }

    void
    handle_peers_added(const Filesystem& filesystem, const Peer& peer) override
    {
      mirror->peer_added(filesystem, peer);
    }

    void
    handle_peers_removed(const Filesystem& filesystem, const Peer& peer) override
    {
  ...
  void
  _set_restarting(const Filesystem& filesystem)
  {
  ...
  void
  _unset_restarting(const Filesystem& filesystem)
  {
    auto& mirror_action = m_mirror_actions.at(filesystem);
  ...
  bool
  _is_restarting(const Filesystem& filesystem)
  {
    auto& mirror_action = m_mirror_actions.at(filesystem);
