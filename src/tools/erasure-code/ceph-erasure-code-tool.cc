...
void
usage(const std::string message, ostream& out)
{
  if (!message.empty()) {
    out << message << std::endl;
    out << "" << std::endl;
  }
  out << "usage: ceph-erasure-code-tool test-plugin-exists <plugin>"
      << std::endl;
  out << "       ceph-erasure-code-tool validate-profile <profile> "
         "[<display-param> ...]"
      << std::endl;
  out << "       ceph-erasure-code-tool calc-chunk-size <profile> <object_size>"
      << std::endl;
  out << "       ceph-erasure-code-tool encode <profile> <stripe_unit> "
         "<want_to_encode> <fname>"
      << std::endl;
  out << "       ceph-erasure-code-tool decode <profile> <stripe_unit> "
         "<want_to_decode> <fname>"
      << std::endl;
  out << "" << std::endl;
  out << "  plugin          - plugin name" << std::endl;
  out << "  profile         - comma separated list of erasure-code profile "
         "settings"
      << std::endl;
  out << "                    example: "
         "plugin=jerasure,technique=reed_sol_van,k=3,m=2"
      << std::endl;
  out << "  display-param   - parameter to display (display all if empty)"
      << std::endl;
  out << "                    may be: " << display_params << std::endl;
  out << "  object_size     - object size" << std::endl;
  out << "  stripe_unit     - stripe unit" << std::endl;
  out << "  want_to_encode  - comma separated list of shards to encode"
      << std::endl;
  out << "  want_to_decode  - comma separated list of shards to decode"
      << std::endl;
  out << "  fname           - name for input/output files" << std::endl;
  out << "                    when encoding input is read form {fname} file,"
      << std::endl;
  out << "                                  result is stored in "
         "{fname}.{shard} files"
      << std::endl;
  out << "                    when decoding input is read form {fname}.{shard} "
         "files,"
      << std::endl;
  out << "                                  result is stored in {fname} file"
      << std::endl;
}
...
int
do_test_plugin_exists(const std::vector<const char*>& args)
{
  if (args.size() < 1) {
...
int
do_validate_profile(const std::vector<const char*>& args)
{
  if (args.size() < 1) {
    usage("not enought arguments", std::cerr);
    return 1;
  ...
  if (args.size() > 1) {
    std::set<std::string> valid_params(
        display_params.begin(), display_params.end());
...
int
do_calc_chunk_size(const std::vector<const char*>& args)
{
  if (args.size() < 2) {
    usage("not enought arguments", std::cerr);
    return 1;
...
int
do_encode(const std::vector<const char*>& args)
{
  if (args.size() < 4) {
    usage("not enought arguments", std::cerr);
    return 1;
  ...
  sinfo->ro_range_to_shard_extent_map(
      0, input_data.length(), input_data, encoded_data);
...
int
do_decode(const std::vector<const char*>& args)
{
  if (args.size() < 4) {
    usage("not enought arguments", std::cerr);
    return 1;
