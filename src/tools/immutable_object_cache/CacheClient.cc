...
#undef dout_prefix
#define dout_prefix \
  *_dout << "ceph::cache::CacheClient: " << this << " " << __func__ << ": "
...
CacheClient::CacheClient(const std::string& file, CephContext* ceph_ctx) :
  m_cct(ceph_ctx),
  m_io_service_work(m_io_service.get_executor()),
  ...
  m_sequence_id(0)
{
  m_worker_thread_num = m_cct->_conf.get_val<uint64_t>(
      "immutable_object_cache_client_dedicated_thread_num");

  if (m_worker_thread_num != 0) {
    m_worker = new boost::asio::io_context();
    m_worker_io_service_work = new boost::asio::executor_work_guard<
        boost::asio::io_context::executor_type>(m_worker->get_executor());
...
// async connect
void
CacheClient::connect(Context* on_finish)
{
  m_dm_socket.async_connect(
      m_ep, boost::bind(
                &CacheClient::handle_connect, this, on_finish,
                boost::asio::placeholders::error));
}

void
CacheClient::handle_connect(
    Context* on_finish,
    ...
    CacheGenContextURef&& on_finish)
{
  ...
  ObjectCacheRequest* req = new ObjectCacheReadData(
      RBDSC_READ, ++m_sequence_id, 0, 0, pool_id, snap_id, object_size, oid,
      pool_nspace);
...
CacheClient::send_message()
{
  ...
  boost::asio::async_write(
      m_dm_socket, boost::asio::buffer(bl.c_str(), bl.length()),
...
CacheClient::read_reply_header()
{
  ...
  boost::asio::async_read(
      m_dm_socket, boost::asio::buffer(raw_ptr, get_header_size()),
      boost::asio::transfer_exactly(get_header_size()),
      boost::bind(
          &CacheClient::handle_reply_header, this, bp_head,
          boost::asio::placeholders::error,
          boost::asio::placeholders::bytes_transferred));
}

void
CacheClient::handle_reply_header(
    bufferptr bp_head,
    ...
    const uint64_t data_len)
{
  ...
  boost::asio::async_read(
      m_dm_socket, boost::asio::buffer(raw_ptr, data_len),
      boost::asio::transfer_exactly(data_len),
      boost::bind(
          &CacheClient::handle_reply_data, this, std::move(bp_head),
          std::move(bp_data), data_len, boost::asio::placeholders::error,
          boost::asio::placeholders::bytes_transferred));
}

void
CacheClient::handle_reply_data(
    bufferptr bp_head,
    bufferptr bp_data,
    const uint64_t data_len,
    const boost::system::error_code& ec,
...
void
CacheClient::process(ObjectCacheRequest* reply, uint64_t seq_id)
{
  ldout(m_cct, 20) << dendl;
  ...
  auto process_reply = new LambdaContext([current_request,
                                          reply](bool dedicated) {
    if (dedicated) {
...
void
CacheClient::fault(const int err_type, const boost::system::error_code& ec)
{
...
int
CacheClient::register_client(Context* on_finish)
{
  ObjectCacheRequest* reg_req = new ObjectCacheRegData(
      RBDSC_REGISTER, m_sequence_id++, ceph_version_to_str());
  reg_req->encode();
  ...
  ret = boost::asio::write(
      m_dm_socket, boost::asio::buffer(bl.c_str(), bl.length()), ec);
  ...
  ret = boost::asio::read(
      m_dm_socket, boost::asio::buffer(m_bp_header.c_str(), get_header_size()),
      ec);
  ...
  ret = boost::asio::read(
      m_dm_socket, boost::asio::buffer(bp_data.c_str(), data_len), ec);
  if (ec || ret != data_len) {
    fault(ASIO_ERROR_READ, ec);
    return -1;
  }
