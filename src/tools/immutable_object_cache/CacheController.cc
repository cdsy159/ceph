...
#undef dout_prefix
#define dout_prefix \
  *_dout << "ceph::cache::CacheController: " << this << " " << __func__ << ": "
...
void
CacheController::handle_signal(int signum)
{
  shutdown();
}
...
CacheController::run()
{
  try {
    ...
    m_cache_server = new CacheServer(
        m_cct, controller_path,
        std::bind(
            &CacheController::handle_request, this, std::placeholders::_1,
            std::placeholders::_2));
...
void
CacheController::handle_request(CacheSession* session, ObjectCacheRequest* req)
{
  ldout(m_cct, 20) << dendl;
  ...
  case RBDSC_READ: {
    ...
    int ret = m_object_cache_store->lookup_object(
        ...
        req_read_data->snap_id, req_read_data->object_size, req_read_data->oid,
        return_dne_path, cache_path);
    ...
    if (ret != OBJ_CACHE_PROMOTED && ret != OBJ_CACHE_DNE) {
      ...
      reply =
          new ObjectCacheReadReplyData(RBDSC_READ_REPLY, req->seq, cache_path);
    }
    session->send(reply);
    break;
  }
