...
#undef dout_prefix
#define dout_prefix \
  *_dout << "ceph::cache::ObjectCacheStore: " << this << " " << __func__ << ": "
  ...
  m_cct(cct), m_rados(new librados::Rados())
{
  ...
  m_policy = new SimplePolicy(
      m_cct, cache_max_size, max_inflight_ops, cache_watermark);
}
...
int
ObjectCacheStore::init(bool reset)
{
  ...
  if (reset) {
    try {
      ...
      lderr(m_cct) << "failed to initialize cache store directory: " << e.what()
                   << dendl;
      return -e.code().value();
    }
  }
    ...
    std::string object_name)
{
  ldout(m_cct, 20) << "to promote object: " << object_name
                   ...
                   << " namespace: " << pool_nspace << " snapshot: " << snap_id
                   << dendl;
...
int
ObjectCacheStore::handle_promote_callback(
    int ret,
    bufferlist* read_buf,
    std::string cache_file_name)
{
...
int
ObjectCacheStore::lookup_object(
    std::string pool_nspace,
    uint64_t pool_id,
    uint64_t snap_id,
...
int
ObjectCacheStore::promote_object(
    librados::IoCtx* ioctx,
    std::string object_name,
...
int
ObjectCacheStore::evict_objects()
{
  ldout(m_cct, 20) << dendl;
...
int
ObjectCacheStore::do_evict(std::string cache_file)
{
...
std::string
ObjectCacheStore::get_cache_file_name(
    std::string pool_nspace,
    uint64_t pool_id,
    uint64_t snap_id,
    std::string oid)
...
std::string
ObjectCacheStore::get_cache_file_path(std::string cache_file_name, bool mkdir)
{
  ldout(m_cct, 20) << cache_file_name << dendl;

  uint32_t crc = 0;
  crc = ceph_crc32c(
      0, (unsigned char*)cache_file_name.c_str(), cache_file_name.length());
...
void
ObjectCacheStore::handle_throttle_ready(uint64_t tokens, uint64_t type)
{
...
bool
ObjectCacheStore::take_token_from_throttle(
    uint64_t object_size,
    uint64_t object_num)
{
  ...
  if (!wait && (m_qos_enabled_flag & ROC_QOS_IOPS_THROTTLE)) {
    ...
    if (object_num > m_iops_tokens) {
      wait = m_throttles[ROC_QOS_IOPS_THROTTLE]->get(
          object_num, this, &ObjectCacheStore::handle_throttle_ready,
          object_num, ROC_QOS_IOPS_THROTTLE);
    } else {
  ...
  if (!wait && (m_qos_enabled_flag & ROC_QOS_BPS_THROTTLE)) {
    ...
    if (object_size > m_bps_tokens) {
      wait = m_throttles[ROC_QOS_BPS_THROTTLE]->get(
          object_size, this, &ObjectCacheStore::handle_throttle_ready,
          object_size, ROC_QOS_BPS_THROTTLE);
    } else {
...
void
ObjectCacheStore::apply_qos_tick_and_limit(
    const uint64_t flag,
    ...
    std::chrono::seconds burst_seconds)
{
  ...
  throttle = new TokenBucketThrottle(
      m_cct, throttle_flags_it->second, 0, 0, timer, timer_lock);
  throttle->set_schedule_tick_min(min_tick.count());
  int ret = throttle->set_limit(limit, burst, burst_seconds.count());
  if (ret < 0) {
    lderr(m_cct) << throttle->get_name()
