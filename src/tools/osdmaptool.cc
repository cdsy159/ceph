...
usage()
{
  ...
  cout << "   --create-from-conf      creates an osd map with default "
          "configurations"
       << std::endl;
  cout << "   --createsimple <numosd> [--clobber] [--pg-bits <bitsperosd>] "
          "[--pgp-bits <bits>] creates a relatively generic OSD map with "
          "<numosd> devices"
       << std::endl;
  cout << "   --pgp-bits <bits>       pgp_num map attribute will be shifted by "
          "<bits>"
       << std::endl;
  cout << "   --pg-bits <bits>        pg_num map attribute will be shifted by "
          "<bits>"
       << std::endl;
  cout << "   --clobber               allows osdmaptool to overwrite "
          "<mapfilename> if it already exists"
       << std::endl;
  ...
  cout << "   --test-map-pgs [--pool <poolid>] [--pg_num <pg_num>] "
          "[--range-first <first> --range-last <last>] map all pgs"
       << std::endl;
  cout << "   --test-map-pgs-dump [--pool <poolid>] [--range-first <first> "
          "--range-last <last>] map all pgs"
       << std::endl;
  cout << "   --test-map-pgs-dump-all [--pool <poolid>] [--range-first <first> "
          "--range-last <last>] map all pgs to osds"
       << std::endl;
  ...
  cout << "   --test-map-object <objectname> [--pool <poolid>] map an object "
          "to osds"
       << std::endl;
  cout
  ...
  cout
      ...
      << std::endl;
  cout << "   --upmap <file>          calculate pg upmap entries to balance pg "
          "layout"
       << std::endl;
  cout << "                           writing commands to <file> [default: - "
          "for stdout]"
       << std::endl;
  cout << "   --upmap-max <max-count> set max upmap entries to calculate "
          "[default: 10]"
       << std::endl;
  cout << "   --upmap-deviation <max-deviation>" << std::endl;
  cout << "                           max deviation from target [default: 5]"
       << std::endl;
  cout << "   --upmap-pool <poolname> restrict upmap balancing to 1 or more "
          "pools"
       << std::endl;
  cout << "   --upmap-active          Act like an active balancer, keep "
          "applying changes until balanced"
       << std::endl;
  cout << "   --dump <format>         displays the map in plain text when "
          "<format> is 'plain', 'json' if specified format is not supported"
       << std::endl;
  cout << "   --tree                  displays a tree of the map" << std::endl;
  cout << "   --test-crush [--range-first <first> --range-last <last>] map pgs "
          "to acting osds"
       << std::endl;
  cout << "   --adjust-crush-weight <osdid:weight>[,<osdid:weight>,<...>] "
          "change <osdid> CRUSH <weight> (but do not persist)"
       << std::endl;
  cout << "   --save                  write modified osdmap with upmap or "
          "crush-adjust changes"
       << std::endl;
  cout << "   --read <file>           calculate pg upmap entries to balance pg "
          "primaries"
       << std::endl;
  cout << "   --read-pool <poolname>  specify which pool the read balancer "
          "should adjust"
       << std::endl;
  cout << "   --osd-size-aware        account for devices of different sizes, "
          "applicable to read mode only"
       << std::endl;
  cout
    ...
    std::string cmd = "ceph")
{
  ...
  for (auto& i : pending_inc.new_pg_upmap_primary) {
    if (vstart)
      ss << prefix;
    ss << cmd + " osd pg-upmap-primary " << i.first << " " << i.second
       << std::endl;
  }
  string s = ss.str();
...
main(int argc, const char** argv)
{
    ...
    if (ceph_argparse_double_dash(args, i)) {
      ...
      if (!val.empty() && val != "plain") {
        tree_formatter.reset(Formatter::create(val, "", "json"));
                   ...
                   args, i, &pg_bits, err, "--osd-pg-bits", (char*)NULL)) {
    } else if (ceph_argparse_witharg(
                   args, i, &pgp_bits, err, "--osd-pgp-bits", (char*)NULL)) {
    } else if (ceph_argparse_witharg(
                   args, i, &upmap_file, "--upmap-cleanup", (char*)NULL)) {
      upmap_cleanup = true;
    } else if (
        ceph_argparse_witharg(args, i, &upmap_file, "--upmap", (char*)NULL)) {
      upmap_cleanup = true;
      upmap = true;
    } else if (
        ceph_argparse_witharg(args, i, &upmap_file, "--read", (char*)NULL)) {
      read = true;
    } else if (ceph_argparse_witharg(
                   args, i, &upmap_max, err, "--upmap-max", (char*)NULL)) {
    } else if (ceph_argparse_witharg(
                   args, i, &upmap_deviation, err, "--upmap-deviation",
                   (char*)NULL)) {
    } else if (ceph_argparse_witharg(
                   args, i, (int*)&upmap_seed, err, "--upmap-seed",
                   (char*)NULL)) {
      upmap_p_seed = &upmap_seed;
    } else if (
        ceph_argparse_witharg(args, i, &val, "--upmap-pool", (char*)NULL)) {
      upmap_pools.insert(val);
    } else if (ceph_argparse_witharg(args, i, &val, "--read-pool", (char*)NULL)) {
      read_pool = val;
    } else if (ceph_argparse_witharg(
                   args, i, &num_osd, err, "--createsimple", (char*)NULL)) {
      if (!err.str().empty()) {
      ...
      test_map_pgs_dump = true;
        ...
        ceph_argparse_flag(args, i, "--test-map-pgs-dump-all", (char*)NULL)) {
      ...
      clobber = true;
                   ...
                   args, i, &pg_bits, err, "--pg_bits", (char*)NULL)) {
      if (!err.str().empty()) {
        cerr << err.str() << std::endl;
        exit(EXIT_FAILURE);
      }
    } else if (ceph_argparse_witharg(
                   args, i, &pgp_bits, err, "--pgp_bits", (char*)NULL)) {
      if (!err.str().empty()) {
        cerr << err.str() << std::endl;
        exit(EXIT_FAILURE);
      }
    } else if (
        ceph_argparse_witharg(args, i, &val, "--export_crush", (char*)NULL)) {
      export_crush = val;
    } else if (
        ceph_argparse_witharg(args, i, &val, "--import_crush", (char*)NULL)) {
      import_crush = val;
    } else if (
        ceph_argparse_witharg(args, i, &val, "--test_map_pg", (char*)NULL)) {
      test_map_pg = val;
    } else if (ceph_argparse_witharg(
                   args, i, &val, "--test_map_object", (char*)NULL)) {
      test_map_object = val;
    } else if (ceph_argparse_flag(args, i, "--test_crush", (char*)NULL)) {
      test_crush = true;
    } else if (
        ceph_argparse_witharg(args, i, &val, err, "--pg_num", (char*)NULL)) {
      string interr;
      ...
      if (interr.length() > 0) {
        ...
        exit(EXIT_FAILURE);
                   ...
                   args, i, &range_first, err, "--range_first", (char*)NULL)) {
    } else if (ceph_argparse_witharg(
                   args, i, &range_last, err, "--range_last", (char*)NULL)) {
    } else if (
        ceph_argparse_witharg(args, i, &pool, err, "--pool", (char*)NULL)) {
      if (!err.str().empty()) {
        cerr << err.str() << std::endl;
        exit(EXIT_FAILURE);
      }
    } else if (ceph_argparse_witharg(
                   args, i, &val, err, "--adjust-crush-weight", (char*)NULL)) {
  ...
  if (!read && osd_size_aware) {
    cerr << me << ": osd-size-aware is only applicable to read mode"
         << std::endl;
    usage();
  }
  fn = args[0];
  ...
  if (!createsimple && !create_from_conf && !clobber) {
    ...
    if (r == 0) {
      ...
      (createsimple || create_from_conf) && !clobber &&
      ::stat(fn.c_str(), &st) == 0) {
  ...
  if (read) {
    ...
    if (osd_size_aware) {
      ...
      if (read_ratio <= 0 || read_ratio > 100) {
        cerr << "The read ratio for pool " << read_pool
             << " is unset or invalid."
             << " To set read ratio, please run 'ceph osd pool set <pool name> "
                "read_ratio <value>'."
             << std::endl;
        exit(1);
      } else {
        cout << "Accounting for devices of different sizes on pool "
             << read_pool << " with a read ratio of " << read_ratio << "."
             << std::endl;
      }
    }
    ...
    pgs_by_osd = tmp_osd_map.get_pgs_by_osd(
        g_ceph_context, pid, &prim_pgs_by_osd, &acting_prims_by_osd);
    ...
    if (osd_size_aware) { // account for different device sizes
      num_changes = osdmap.balance_primaries(
          g_ceph_context, pid, &pending_inc, tmp_osd_map, OSDMap::RB_OSDSIZEOPT);
    } else { // default
      num_changes = osdmap.balance_primaries(
          g_ceph_context, pid, &pending_inc, tmp_osd_map);
    }

    if (num_changes < 0) {
      cerr << "Error balancing primaries. Rerun with at least --debug-osd=10 "
              "for more details."
           << std::endl;
      exit(1);
    }
    ...
    pgs_by_osd_2 = tmp_osd_map.get_pgs_by_osd(
        g_ceph_context, pid, &prim_pgs_by_osd_2, &acting_prims_by_osd_2);
    tmp_osd_map.calc_read_balance_score(g_ceph_context, pid, &rb_info);
    ...
    if (num_changes > 0) {
      ...
      for (auto& [osd, pgs] : prim_pgs_by_osd) {
        cout << " osd." << osd << " | primary affinity: "
             << tmp_osd_map.get_primary_affinityf(osd)
             << " | number of prims: " << pgs.size() << "\n";
      }
      cout << " \n";
      cout << "read_balance_score of '" << read_pool
      ...
      cout << " Unable to find further optimization, or distribution is "
              "already perfect\n";
    }
  }
  if (upmap) {
    cout << "upmap, max-count " << upmap_max << ", max deviation "
         << upmap_deviation << std::endl;
    ...
    do {
      ...
      for (auto& i : pools) {
        ...
        int did = osdmap.calc_pg_upmaps(
            g_ceph_context, upmap_deviation, left, one_pool, &pending_inc,
            upmap_p_seed);
        total_did += did;
        left -= did;
        if (left <= 0)
      ...
      assert(r == 0);
      cout << "prepared " << total_did << "/" << upmap_max << " changes"
           << std::endl;
      float elapsed_time = (end.tv_sec - begin.tv_sec) +
                           1.0e-9 * (end.tv_nsec - begin.tv_nsec);
      if (upmap_active)
      ...
      if (total_did > 0) {
        ...
        if (upmap_active) {
          ...
          for (auto& i : pgs_by_osd)
            cout << "osd." << i.first << " pgs " << i.second.size()
                 << std::endl;
          float elapsed_time = (end.tv_sec - round_start.tv_sec) +
                               1.0e-9 * (end.tv_nsec - round_start.tv_nsec);
          cout << "Total time elapsed " << elapsed_time << " secs, " << rounds
               << " rounds" << std::endl;
        }
        break;
      }
...
skip_upmap:
  ...
  if (!import_crush.empty()) {
    ...
    inc.crush = cbl;
    osdmap.apply_incremental(inc);
    cout << me << ": imported " << cbl.length() << " byte crush map from "
         << import_crush << std::endl;
    modified = true;
  }
  ...
  if (!export_crush.empty()) {
    ...
    if (r < 0) {
      cerr << me << ": error writing crush map to " << import_crush
           << std::endl;
      exit(1);
    }
  ...
  if (!test_map_pg.empty()) {
    ...
    cout << pgid << " raw (" << raw << ", p" << raw_primary << ") up (" << up
         << ", p" << up_primary << ") acting (" << acting << ", p"
         << acting_primary << ")" << std::endl;
  }
  if (test_map_pgs || test_map_pgs_dump || test_map_pgs_dump_all) {
    ...
    for (auto p = pools.begin(); p != pools.end(); ++p) {
      ...
      cout << "pool " << p->first << " pg_num " << p->second.get_pg_num()
           << std::endl;
      for (unsigned i = 0; i < p->second.get_pg_num(); ++i) {
        ...
        if (test_map_pgs_dump) {
          ...
          cout << pgid << " raw (" << raw << ", p" << calced_primary << ") up ("
               << up << ", p" << up_primary << ") acting (" << acting << ", p"
               << acting_primary << ")" << std::endl;
        }
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (osdmap.crush->get_item_weight(i) <= 0)
        continue;
      in++;
      cout << "osd." << i << "\t" << count[i] << "\t" << first_count[i] << "\t"
           << primary_count[i] << "\t" << osdmap.crush->get_item_weightf(i)
           << "\t" << osdmap.get_weightf(i) << std::endl;
      total += count[i];
      if (count[i] && (min_osd < 0 || count[i] < count[min_osd]))
        min_osd = i;
      if (count[i] && (max_osd < 0 || count[i] > count[max_osd]))
        max_osd = i;
    }
  ...
  if (!print && !health && !tree && !modified && export_crush.empty() &&
      import_crush.empty() && test_map_pg.empty() && test_map_object.empty() &&
  ...
  if (modified) {
    ...
    // write it out
    cout << me << ": writing epoch " << osdmap.get_epoch() << " to " << fn
         << std::endl;
    int r = bl.write_file(fn.c_str());
    if (r) {
      cerr << "osdmaptool: error writing to '" << fn << "': " << cpp_strerror(r)
           << std::endl;
      return 1;
    }
  }
