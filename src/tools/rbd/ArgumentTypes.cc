...
Format::Formatter
Format::create_formatter(bool pretty) const
{
  if (value == "json") {
...
void
add_image_id_option(po::options_description* opt, const std::string& desc_suffix)
{
...
void
add_snap_option(po::options_description* opt, ArgumentModifier modifier)
{
...
void
add_snap_id_option(po::options_description* opt, ArgumentModifier modifier)
{
  ...
  opt->add_options()(name.c_str(), po::value<uint64_t>(), description.c_str());
}
...
void
add_image_or_snap_spec_options(
    po::options_description* pos,
    po::options_description* opt,
    ArgumentModifier modifier)
{
...
void
add_create_image_options(po::options_description* opt, bool include_format)
{
  ...
  if (include_format) {
    opt->add_options()(
        ...
        IMAGE_NEW_FORMAT.c_str(), po::value<ImageNewFormat>()->zero_tokens(),
        "deprecated[:image-format 2]");
  }

  opt->add_options()(
      IMAGE_ORDER.c_str(), po::value<ImageOrder>(), "deprecated[:object-size]")(
      ...
      IMAGE_STRIPE_UNIT.c_str(), po::value<ImageObjectSize>(),
...
void
add_create_journal_options(po::options_description* opt)
{
  opt->add_options()(
...
void
add_size_option(boost::program_options::options_description* opt)
{
  opt->add_options()(
...
void
add_estimated_size_option(boost::program_options::options_description* opt)
{
  opt->add_options()(
      IMAGE_ESTIMATED_SIZE.c_str(), po::value<ImageSize>(),
      "estimated image size (valid only for raw import from stdin, in M/G/T) "
      "[default: M]");
}

void
add_sparse_size_option(boost::program_options::options_description* opt)
{
  opt->add_options()(
...
void
add_limit_option(po::options_description* opt)
{
...
void
add_no_progress_option(boost::program_options::options_description* opt)
{
  opt->add_options()(
...
void
add_format_options(boost::program_options::options_description* opt)
{
  opt->add_options()(
      FORMAT.c_str(), po::value<Format>(),
...
void
add_verbose_option(boost::program_options::options_description* opt)
{
  opt->add_options()(VERBOSE.c_str(), po::bool_switch(), "be verbose");
}

void
add_no_error_option(boost::program_options::options_description* opt)
{
  opt->add_options()(NO_ERR.c_str(), po::bool_switch(), "continue after error");
}

void
add_export_format_option(boost::program_options::options_description* opt)
{
  opt->add_options()(
...
void
add_flatten_option(boost::program_options::options_description* opt)
{
  opt->add_options()(
...
void
add_snap_create_options(po::options_description* opt)
{
  opt->add_options()(
...
void
add_encryption_options(boost::program_options::options_description* opt)
{
  opt->add_options()(
      ENCRYPTION_FORMAT.c_str(), po::value<std::vector<EncryptionFormat>>(),
      "encryption format (luks, luks1, luks2) [default: luks]");

  opt->add_options()(
      ENCRYPTION_PASSPHRASE_FILE.c_str(), po::value<std::vector<std::string>>(),
...
get_short_features_help(bool append_suffix)
{
  ...
  for (auto& pair : ImageFeatures::FEATURE_MAPPING) {
    ...
    std::string suffix;
    if (append_suffix) {
      if ((pair.first & rbd::utils::get_rbd_default_features(g_ceph_context)) !=
          0) {
        suffix += "+";
      }
