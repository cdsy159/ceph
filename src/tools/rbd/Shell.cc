...
std::string
format_command_spec(const Shell::CommandSpec& spec)
{
...
std::string
format_option_suffix(const boost::shared_ptr<po::option_description>& option)
{
  std::string suffix;
...
void
print_deprecated_warning(po::option_description option, std::string description)
{
  auto pos = description.find_first_of(":");
  if (pos != std::string::npos) {
    std::string param =
        description.substr(pos + 1, description.size() - pos - 2);
...
int
Shell::execute(int argc, const char** argv)
{
  ...
  try {
    ...
    positional_options.add(
        at::POSITIONAL_COMMAND_SPEC.c_str(), matching_spec->size());
    ...
    for (; it != ceph_global_init_args.end(); ++it) {
      ...
      for (size_t i = 0; i < global_opts.options().size(); ++i) {
        std::string param_name =
            global_opts.options()[i]->canonical_display_name(prefix_style);
    ...
    if (arg == "-h" || arg == "--help") {
      *command_spec = {HELP_SPEC};
      ...
      if (i + 1 < arguments.size()) {
        command_spec->insert(
            command_spec->end(), arguments.data() + i + 1,
      ...
      if (arg.size() >= 2 &&
          ...
          (arg[1] != '-' || get_switch_arguments().count(
                                arg.substr(2, std::string::npos)) == 0) &&
    ...
    bool* is_alias)
{
  ...
  for (Action* action : actions) {
    if (action->command_spec.size() <= command_spec.size()) {
      if (std::equal(
              action->command_spec.begin(), action->command_spec.end(),
              command_spec.begin())) {
    ...
    if (!action->alias_command_spec.empty() &&
      ...
      if (std::equal(
              action->alias_command_spec.begin(),
              action->alias_command_spec.end(), command_spec.begin())) {
        if (matching_spec != NULL) {
...
void
Shell::get_global_options(po::options_description* opts)
{
  opts->add_options()(
      (at::CONFIG_PATH + ",c").c_str(), po::value<std::string>(),
      ...
      "deprecated[:id]")("name,n", po::value<std::string>(), "client name")(
...
Shell::print_help()
{
  ...
  std::vector<Action*> actions(get_actions());
  std::sort(actions.begin(), actions.end(), [](Action* lhs, Action* rhs) {
    return lhs->command_spec < rhs->command_spec;
  });
  ...
  for (size_t i = 0; i < actions.size(); ++i) {
    ...
    if (!action->description.empty()) {
      IndentStream indent_stream(
          name_width, ss.str().size(), OptionPrinter::LINE_WIDTH, std::cout);
  ...
  std::cout << std::endl
            << OptionPrinter::OPTIONAL_ARGUMENTS << ":" << std::endl;
...
void
Shell::print_action_help(Action* action, bool is_alias)
{
  std::stringstream ss;
  ss << "usage: " << APP_NAME << " "
     << format_command_spec(
            is_alias ? action->alias_command_spec : action->command_spec);
  std::cout << ss.str();
...
void
Shell::print_unknown_action(const std::vector<std::string>& command_spec)
{
  std::cerr << "error: unknown option '"
            << joinify<std::string>(
                   command_spec.begin(), command_spec.end(), " ")
            << "'" << std::endl
            << std::endl;
  print_help();
}
...
void
Shell::print_bash_completion(const CommandSpec& command_spec)
{
...
void
Shell::print_bash_completion_options(const po::options_description& ops)
{
