...
const DeviceOperations ggate_operations = {
    ggate::execute_list,   ggate::execute_map,    ggate::execute_unmap,
    ggate::execute_attach, ggate::execute_detach,
};

const DeviceOperations krbd_operations = {
    kernel::execute_list,   kernel::execute_map,    kernel::execute_unmap,
    kernel::execute_attach, kernel::execute_detach,
};

const DeviceOperations nbd_operations = {
    nbd::execute_list,   nbd::execute_map,    nbd::execute_unmap,
    nbd::execute_attach, nbd::execute_detach,
};

const DeviceOperations wnbd_operations = {
    wnbd::execute_list,   wnbd::execute_map,    wnbd::execute_unmap,
    wnbd::execute_attach, wnbd::execute_detach,
};

const DeviceOperations ubbd_operations = {
    ubbd::execute_list,   ubbd::execute_map,    ubbd::execute_unmap,
    ubbd::execute_attach, ubbd::execute_detach,
};
...
void
add_device_type_option(po::options_description* options)
{
  options->add_options()(
...
void
add_device_specific_options(po::options_description* options)
{
  options->add_options()(
...
device_type_t
get_device_type(const po::variables_map& vm)
{
...
get_device_operations(const po::variables_map& vm)
{
    ...
    po::options_description* options)
{
  ...
  options->add_options()("show-cookie", po::bool_switch(), "show device cookie")(
      "cookie", po::value<std::string>(),
      "specify device cookie")("read-only", po::bool_switch(), "map read-only")(
      "exclusive", po::bool_switch(),
      "disable automatic exclusive lock transitions")(
    ...
    po::options_description* options)
{
  ...
  positional->add_options()(
      ...
      "[<pool-name>/[<namespace>/]]<image-name>[@<snap-name>] or "
      "<device-path>");
    ...
    po::options_description* options)
{
  ...
  options->add_options()(
      ...
      "attach read-only")("force", po::bool_switch(), "force attach")(
      "exclusive", po::bool_switch(),
      "disable automatic exclusive lock transitions")(
      "quiesce", po::bool_switch(), "use quiesce hooks")(
      "quiesce-hook", po::value<std::string>(), "quiesce hook path");
...
Shell::SwitchArguments switched_arguments(
    {"exclusive", "force", "quiesce", "read-only", "show-cookie"});
