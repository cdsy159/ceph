...
static int
disk_usage_callback(uint64_t offset, size_t len, int exists, void* arg)
{
    ...
    uint64_t* used_size)
{
  ...
  *used_size = 0;
  r = image.diff_iterate2(
      from, 0, size, false, !exact, &disk_usage_callback, used_size);
  if (r < 0) {
    ...
    Formatter* f)
{
  if (f) {
    ...
    tbl << full_name << stringify(byte_u_t(size))
        << stringify(byte_u_t(used_size)) << TextTable::endrow;
  }
}
    ...
    bool merge_snap)
{
  ...
  for (auto& image_spec : images) {
    ...
    if (r < 0) {
      std::cerr << "rbd: error opening " << image_spec.name
                << " snapshots: " << cpp_strerror(r) << std::endl;
      continue;
    }

    snap_list.erase(
        remove_if(
            snap_list.begin(), snap_list.end(),
    ...
    for (std::vector<librbd::snap_info_t>::const_iterator snap =
             snap_list.begin();
         snap != snap_list.end(); ++snap) {
      librbd::Image snap_image;
      r = rbd.open_read_only(
          io_ctx, snap_image, image_spec.name.c_str(), snap->name.c_str());
      if (r < 0) {
      ...
      if (imgname == nullptr || found_from_snap ||
        ...
        r = get_image_disk_usage(
            image_spec.name, snap->name, last_snap_name, snap_image, exact,
            snap->size, &used_size);
        if (r < 0) {
          goto out;
        }
        if (!merge_snap) {
          format_image_disk_usage(
              image_spec.name, image_spec.id, snap->name, snap->id, snap->size,
              used_size, tbl, f);
        }
    ...
    if (snapname == NULL) {
      r = get_image_disk_usage(
          image_spec.name, "", last_snap_name, image, exact, info.size,
          &used_size);
      if (r < 0) {
        goto out;
      }
      ...
      if (!merge_snap) {
        format_image_disk_usage(
            image_spec.name, image_spec.id, "", CEPH_NOSNAP, info.size,
            used_size, tbl, f);
      } else {
        format_image_disk_usage(
            image_spec.name, image_spec.id, "", CEPH_NOSNAP, info.size,
            image_full_used_size, tbl, f);
      }
...
out:
  if (f) {
    ...
    if (count > 1) {
      tbl << "<TOTAL>" << stringify(byte_u_t(total_prov))
          << stringify(byte_u_t(total_used)) << TextTable::endrow;
    }
    std::cout << tbl;
  }
    ...
    const std::vector<std::string>& ceph_global_init_args)
{
  ...
  librbd::RBD rbd;
  r = do_disk_usage(
      rbd, io_ctx, image_name.empty() ? nullptr : image_name.c_str(),
