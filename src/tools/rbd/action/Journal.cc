...
void
add_journal_option(po::options_description* opt, at::ArgumentModifier modifier)
{
    ...
    std::string* journal_name)
{
  ...
  std::string journal_key =
      (mod == at::ARGUMENT_MODIFIER_DEST ? DEST_JOURNAL_NAME : JOURNAL_NAME);
  ...
  if (journal_name != nullptr && !journal_name->empty()) {
    ...
    r = extract_spec(
        journal_name_copy, pool_name, namespace_name, journal_name, nullptr,
  ...
  if (!image_name.empty()) {
    ...
    r = extract_spec(
        image_name_copy, pool_name, namespace_name, &image_name, nullptr,
  ...
  if (journal_name != nullptr && spec_arg_index != nullptr &&
    ...
    if (!spec.empty()) {
      r = extract_spec(
          spec, pool_name, namespace_name, journal_name, nullptr,
          utils::SPEC_VALIDATION_FULL);
      if (r < 0) {
        return r;
      }
  ...
  if (pool_name != nullptr && namespace_name != nullptr &&
    ...
    librbd::Image image;
    int r = utils::init_and_open_image(
    ...
    Formatter* f)
{
  ...
  std::string object_oid_prefix =
      ::journal::Journaler::object_oid_prefix(io_ctx.get_id(), journal_id);
  uint8_t order;
  uint8_t splay_width;
  int64_t pool_id;

  cls::journal::client::get_immutable_metadata(
      io_ctx, header_oid, &order, &splay_width, &pool_id, &cond);
  r = cond.wait();
  if (r < 0) {
  ...
  if (f) {
    ...
    std::cout << "\tsplay_width: " << static_cast<int>(splay_width)
              << std::endl;
    if (!object_pool_name.empty()) {
    ...
    Formatter* f)
{
  ...
  cls::journal::client::get_mutable_metadata(
      io_ctx, oid, &minimum_set, &active_set, &registered_clients, &cond);
  r = cond.wait();
  if (r < 0) {
  ...
  if (f) {
    ...
    for (std::set<cls::journal::Client>::iterator c = registered_clients.begin();
         c != registered_clients.end(); ++c) {
...
static int
do_reset_journal(librados::IoCtx& io_ctx, const std::string& journal_id)
{
  // disable/re-enable journaling to delete/re-create the journal
  // to properly handle mirroring constraints
  std::string image_name;
  int r = librbd::cls_client::dir_get_name(
      &io_ctx, RBD_DIRECTORY, journal_id, &image_name);
  if (r < 0) {
    ...
    const std::string& client_id)
{
  ...
  for (auto& c : registered_clients) {
    ...
    r = cls::journal::client::client_update_state(
        io_ctx, oid, c.id, cls::journal::CLIENT_STATE_DISCONNECTED);
    if (r < 0) {
...
public:
  JournalPlayer(
      ...
      const std::string& journal_id,
      const std::string& client_id) :
    m_journaler(io_ctx, journal_id, client_id), m_cond(), m_r(0)
  {}
...
public:
  JournalInspector(
      ...
      const std::string& journal_id,
      bool verbose) :
    JournalPlayer(io_ctx, journal_id, "INSPECT"), m_verbose(verbose), m_s()
  {}
...
private:
  ...
  int
  process_entry(::journal::ReplayEntry replay_entry, uint64_t tag_id) override
  {
    m_s.total++;
    if (m_verbose) {
      std::cout << "Entry: tag_id=" << tag_id
                << ", commit_tid=" << replay_entry.get_commit_tid()
                << std::endl;
    }
...
struct ExportEntry {
  ...
  ExportEntry() :
    tag_id(0), commit_tid(0), type(0), entry()
  {}
...
public:
  ...
  bool
  read_entry(bufferlist& bl, int& r)
  {
  ...
  exec()
  {
    ...
    std::cout << n << " entries processed, " << error_count << " errors"
              << std::endl;
...
void
get_client_disconnect_arguments(
    po::options_description* positional,
    po::options_description* options)
{
  add_journal_spec_options(positional, options, at::ARGUMENT_MODIFIER_NONE);
    ...
    const std::vector<std::string>& ceph_global_init_args)
{
  ...
  r = do_export_journal(
      io_ctx, journal_name, path, vm[at::NO_ERR].as<bool>(),
