    ...
    TextTable& tbl)
{
  ...
  if (f) {
    ...
    tbl << w->name << stringify(byte_u_t(info.size)) << parent
        << ((old_format) ? '1' : '2') << "" // protect doesn't apply to images
  ...
  if (w->img.snap_list(snaplist) >= 0 && !snaplist.empty()) {
    ...
    for (std::vector<librbd::snap_info_t>::iterator s = snaplist.begin();
      ...
      if (f) {
        ...
        tbl << w->name + "@" + s->name << stringify(byte_u_t(s->size)) << parent
            << ((old_format) ? '1' : '2') << (is_protected ? "yes" : "")
  ...
  while (true) {
    ...
    for (auto comp : workers) {
      ...
      case STATE_IDLE:
        ...
        r = rbd.aio_open_read_only(
            ioctx, comp->img, i->name.c_str(), nullptr, comp->completion);
        i++;
...
Shell::Action
    action({"list"}, {"ls"}, "List rbd images.", "", &get_arguments, &execute);
