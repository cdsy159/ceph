...
int
validate_mirroring_enabled(librbd::Image& image, bool snapshot = false)
{
  ...
  if (snapshot) {
    ...
    if (r < 0) {
      std::cerr << "rbd: failed to get mirror mode for image '" << image_name
                << "': " << cpp_strerror(r) << std::endl;
      return r;
    }
    ...
    if (mode != RBD_MIRROR_IMAGE_MODE_SNAPSHOT) {
      std::cerr << "rbd: snapshot based mirroring not enabled on image '"
                << image_name
                << "' (mirroring mode: " << utils::mirror_image_mode(mode)
                << ")" << std::endl;
      return -EINVAL;
    }
  }
...
int
execute_enable_disable(const po::variables_map& vm, bool enable, bool force)
{
  size_t arg_index = 0;
    ...
    const std::vector<std::string>& ceph_global_init_args)
{
  ...
  status.site_statuses.erase(
      std::remove_if(
          ...
          [](auto& status) {
            return (
                status.mirror_uuid == RBD_MIRROR_IMAGE_STATUS_LOCAL_MIRROR_UUID);
          }),
  ...
  if (local_site_r >= 0 && local_status.up) {
    ...
    if (r == -EOPNOTSUPP) {
      ...
      std::cerr << "rbd: failed to get service id for image " << image_name
                << ": " << cpp_strerror(r) << std::endl;
      // not fatal
  ...
  if (formatter != nullptr) {
    ...
    if (!snaps.empty()) {
      ...
      for (auto& snap : snaps) {
        ...
        formatter->dump_bool(
            "demoted", info.state == RBD_SNAP_MIRROR_STATE_PRIMARY_DEMOTED);
    ...
    if (local_site_r >= 0) {
      ...
      std::cout << "  last_update: " << utils::timestr(local_status.last_update)
                << std::endl;
    }
    if (!status.site_statuses.empty()) {
