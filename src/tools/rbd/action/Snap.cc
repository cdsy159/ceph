...
std::string
get_snap_namespace_name(librbd::snap_namespace_type_t type)
{
  switch (type) {
    ...
    librados::Rados& rados)
{
  ...
  for (std::vector<librbd::snap_info_t>::iterator s = snaps.begin();
    ...
    if (snap_namespace == RBD_SNAP_NAMESPACE_TYPE_GROUP) {
      get_group_res = image.snap_get_group_namespace(
          s->id, &group_snap, sizeof(group_snap));
    ...
    if (f) {
      ...
      if (all_snaps) {
        ...
        if (get_group_res == 0) {
          ...
          if (mirror_snap.state == RBD_SNAP_MIRROR_STATE_NON_PRIMARY ||
            ...
            f->dump_string(
                "primary_mirror_uuid", mirror_snap.primary_mirror_uuid);
      ...
      t << s->id << s->name << stringify(byte_u_t(s->size)) << protected_str
        << tt_str;
      ...
      if (all_snaps) {
        ...
        if (get_group_res == 0) {
          std::string pool_name = pool_map[group_snap.group_pool];
          oss << " (" << pool_name << "/" << group_snap.group_name << "@"
          ...
          oss << " (" << mirror_snap_state << " " << "peer_uuids:["
              << mirror_snap.mirror_peer_uuids << "]";
          if (mirror_snap.state == RBD_SNAP_MIRROR_STATE_NON_PRIMARY ||
              mirror_snap.state == RBD_SNAP_MIRROR_STATE_NON_PRIMARY_DEMOTED) {
...
int
do_rollback_snap(librbd::Image& image, const char* snapname, bool no_progress)
{
...
int
do_purge_snaps(librbd::Image& image, bool no_progress)
{
...
int
do_protect_snap(librbd::Image& image, const char* snapname)
{
...
int
do_unprotect_snap(librbd::Image& image, const char* snapname)
{
...
int
do_set_limit(librbd::Image& image, uint64_t limit)
{
    ...
    const std::vector<std::string>& ceph_global_init_args)
{
  ...
  int r = utils::get_pool_image_snapshot_names(
      ...
      &image_name, &snap_name, image_id.empty(), utils::SNAPSHOT_PRESENCE_NONE,
    ...
    const std::vector<std::string>& ceph_global_init_args)
{
  ...
  r = do_add_snap(
      image, snap_name.c_str(), flags, vm[at::NO_PROGRESS].as<bool>());
  if (r < 0) {
    ...
    po::options_description* options)
{
  ...
  options->add_options()(
      ...
      "flatten children and unprotect snapshot if needed.");
}
