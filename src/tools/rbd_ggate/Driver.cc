...
#undef dout_prefix
#define dout_prefix \
  *_dout << "rbd::ggate::Driver: " << this << " " << __func__ << ": "
...
int
Driver::kill(const std::string& devname)
{
...
int
Driver::list(std::map<std::string, DevInfo>* devices)
{
  size_t size = 1024;
...
Driver::init()
{
  ...
  int r = ggate_drv_create(
      name, namelen, m_sectorsize, m_mediasize, m_readonly, m_info.c_str(),
      &m_drv);
  if (r < 0) {
    return r;
  }
