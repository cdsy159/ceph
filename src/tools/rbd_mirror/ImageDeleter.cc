...
public:
    ...
    admin_socket(cct->get_admin_socket())
  {
    ...
    r = admin_socket->register_command(
        command, this, "get status for image deleter");
    if (r == 0) {
...
private:
  typedef std::map<std::string, ImageDeleterAdminSocketCommand*, std::less<>>
      Commands;
...
ImageDeleter<I>::ImageDeleter(
  ...
  m_lock(ceph::make_mutex(librbd::util::unique_lock_name(
      "rbd::mirror::ImageDeleter::m_lock",
      this))){}

#undef dout_prefix
#define dout_prefix \
  *_dout << "rbd::mirror::ImageDeleter: " << " " << __func__ << ": "

  template <typename I>
  void ImageDeleter<I>::trash_move(
      librados::IoCtx & local_io_ctx,
...
#undef dout_prefix
#define dout_prefix \
  *_dout << "rbd::mirror::ImageDeleter: " << this << " " << __func__ << ": "

template <typename I>
void
ImageDeleter<I>::init(Context* on_finish)
{
  dout(10) << dendl;
  ...
  m_trash_watcher = image_deleter::TrashWatcher<I>::create(
      m_local_io_ctx, m_threads, m_trash_listener);
  m_trash_watcher->init(on_finish);
}

template <typename I>
void
ImageDeleter<I>::shut_down(Context* on_finish)
{
  dout(10) << dendl;
...
template <typename I>
void
ImageDeleter<I>::shut_down_trash_watcher(Context* on_finish)
{
  dout(10) << dendl;
...
template <typename I>
void
ImageDeleter<I>::wait_for_ops(Context* on_finish)
{
  {
...
template <typename I>
void
ImageDeleter<I>::cancel_all_deletions(Context* on_finish)
{
...
template <typename I>
void
ImageDeleter<I>::wait_for_deletion(
...
template <typename I>
void
ImageDeleter<I>::complete_active_delete(DeleteInfoRef* delete_info, int r)
{
...
template <typename I>
void
ImageDeleter<I>::enqueue_failed_delete(
    DeleteInfoRef* delete_info,
    int error_code,
    double retry_delay)
{
  ...
  delete_info_ref->retry_time =
      (clock_t::now() + ceph::make_timespan(retry_delay));
...
ImageDeleter<I>::find_delete_info(const std::string& image_id)
{
  ...
  DeleteQueue delete_queues[] = {
      m_in_flight_delete_queue, m_retry_delete_queue, m_delete_queue};

  DeleteInfo delete_info{image_id};
  for (auto& queue : delete_queues) {
    auto it = std::find_if(
        queue.begin(), queue.end(), [&delete_info](const DeleteInfoRef& ref) {
...
template <typename I>
void
ImageDeleter<I>::print_status(Formatter* f)
{
  dout(20) << dendl;
...
template <typename I>
vector<string>
ImageDeleter<I>::get_delete_queue_items()
{
  vector<string> items;
...
ImageDeleter<I>::get_failed_queue_items()
{
...
ImageDeleter<I>::remove_images()
{
  ...
  while (m_running && !m_delete_queue.empty()) {
    ...
    auto on_start = create_async_context_callback(
        m_threads->work_queue, new LambdaContext([this, delete_info](int r) {
          if (r < 0) {
            notify_on_delete(delete_info->image_id, r);
            return;
          }
    ...
    m_image_deletion_throttler->start_op(
        m_local_io_ctx.get_namespace(), delete_info->image_id, on_start);
  }
}

template <typename I>
void
ImageDeleter<I>::remove_image(DeleteInfoRef delete_info)
{
...
template <typename I>
void
ImageDeleter<I>::handle_remove_image(DeleteInfoRef delete_info, int r)
{
  dout(10) << "info=" << *delete_info << ", r=" << r << dendl;

  m_image_deletion_throttler->finish_op(
      m_local_io_ctx.get_namespace(), delete_info->image_id);
  {
    std::lock_guard locker{m_lock};
    ceph_assert(ceph_mutex_is_locked(m_lock));
    auto it = std::find(
        m_in_flight_delete_queue.begin(), m_in_flight_delete_queue.end(),
        delete_info);
  ...
  if (r < 0) {
    if (delete_info->error_result == image_deleter::ERROR_RESULT_COMPLETE) {
      ...
      double failed_interval =
          cct->_conf.get_val<double>("rbd_mirror_delete_retry_interval");
...
template <typename I>
void
ImageDeleter<I>::schedule_retry_timer()
{
...
template <typename I>
void
ImageDeleter<I>::cancel_retry_timer()
{
  dout(10) << dendl;
  ceph_assert(ceph_mutex_is_locked(m_threads->timer_lock));
...
template <typename I>
void
ImageDeleter<I>::handle_retry_timer()
{
  dout(10) << dendl;
  ceph_assert(ceph_mutex_is_locked(m_threads->timer_lock));
  std::lock_guard locker{m_lock};
...
template <typename I>
void
ImageDeleter<I>::handle_trash_image(
    const std::string& image_id,
...
template <typename I>
void
ImageDeleter<I>::notify_on_delete(const std::string& image_id, int r)
{
...
template <typename I>
void
ImageDeleter<I>::DeleteInfo::print_status(Formatter* f, bool print_failure_info)
{
