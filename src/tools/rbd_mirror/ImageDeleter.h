...
public:
  ...
  {
    return new ImageDeleter(
        local_io_ctx, threads, image_deletion_throttler, service_daemon);
  }
  ...
  inline void
  set_busy_timer_interval(double interval)
  {
...
private:
  ...
  struct DeleteInfo {
    ...
    inline bool
    operator==(const DeleteInfo& delete_info) const
    {
      return (image_id == delete_info.image_id);
    }

    friend std::ostream&
    operator<<(std::ostream& os, DeleteInfo& delete_info)
    {
