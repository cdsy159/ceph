...
private:
  ...
  struct C_LoadMap : Context {
    ...
    C_LoadMap(ImageMap* image_map, Context* on_finish) :
      image_map(image_map), on_finish(on_finish)
    {}
  ...
  void
  wait_for_async_ops(Context* on_finish)
  {
  ...
  void notify_listener_acquire_release_images(
      const Updates& acquire,
      const Updates& release);
  void notify_listener_remove_images(
      const std::string& mirror_uuid,
      const Updates& remove);

  void update_images_added(
      const std::string& mirror_uuid,
      const std::set<std::string>& global_image_ids);
  void update_images_removed(
      const std::string& mirror_uuid,
      const std::set<std::string>& global_image_ids);

  void filter_instance_ids(
