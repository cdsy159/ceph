...
#undef dout_prefix
#define dout_prefix \
  *_dout << "rbd::mirror::ImageSync: " << this << " " << __func__
...
public:
  ...
  int
  update_progress(uint64_t object_no, uint64_t object_count) override
  {
...
template <typename I>
void
ImageSync<I>::send_notify_sync_request()
{
...
template <typename I>
void
ImageSync<I>::handle_notify_sync_request(int r)
{
  dout(10) << ": r=" << r << dendl;
...
template <typename I>
void
ImageSync<I>::send_prune_catch_up_sync_point()
{
...
template <typename I>
void
ImageSync<I>::handle_prune_catch_up_sync_point(int r)
{
  dout(10) << ": r=" << r << dendl;

  if (r < 0) {
    derr << ": failed to prune catch-up sync point: " << cpp_strerror(r)
         << dendl;
    finish(r);
    return;
  }
...
template <typename I>
void
ImageSync<I>::send_create_sync_point()
{
...
template <typename I>
void
ImageSync<I>::handle_create_sync_point(int r)
{
  dout(10) << ": r=" << r << dendl;
...
ImageSync<I>::send_copy_image()
{
  ...
  Context* ctx =
      create_context_callback<ImageSync<I>, &ImageSync<I>::handle_copy_image>(
          this);
  m_image_copy_prog_handler = new ImageCopyProgressHandler(this);
  m_image_copy_request = librbd::DeepCopyRequest<I>::create(
      m_remote_image_ctx, m_local_image_ctx, snap_id_start, snap_id_end, 0,
...
template <typename I>
void
ImageSync<I>::handle_copy_image(int r)
{
  dout(10) << ": r=" << r << dendl;
...
template <typename I>
void
ImageSync<I>::handle_copy_image_update_progress(
    uint64_t object_no,
...
template <typename I>
void
ImageSync<I>::send_update_sync_point()
{
  ...
  m_sync_point_handler->update_sync_points(
      m_snap_seqs_copy, m_sync_points_copy, false, ctx);
}

template <typename I>
void
ImageSync<I>::handle_update_sync_point(int r)
{
...
template <typename I>
void
ImageSync<I>::send_flush_sync_point()
{
  if (m_ret_val < 0) {
    finish(m_ret_val);
    return;
...
template <typename I>
void
ImageSync<I>::handle_flush_sync_point(int r)
{
  dout(10) << ": r=" << r << dendl;
...
template <typename I>
void
ImageSync<I>::send_prune_sync_points()
{
  dout(10) << dendl;
...
template <typename I>
void
ImageSync<I>::handle_prune_sync_points(int r)
{
  dout(10) << ": r=" << r << dendl;
...
template <typename I>
void
ImageSync<I>::update_progress(const std::string& description)
{
