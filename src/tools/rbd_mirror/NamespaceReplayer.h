...
public:
  ...
  {
    return new NamespaceReplayer(
        local_name, remote_name, local_ioctx, remote_ioctx, local_mirror_uuid,
        local_mirror_peer_uuid, remote_pool_meta, threads, image_sync_throttler,
        image_deletion_throttler, service_daemon, cache_manager_handler,
        pool_meta_cache);
  }
...
private:
  ...
  struct PoolWatcherListener : public pool_watcher::Listener {
        ...
        ImageIds&& removed_image_ids) override
    {
      namespace_replayer->handle_update(
          (local ? "" : mirror_uuid), std::move(added_image_ids),
  ...
  struct ImageMapListener : public image_map::Listener {
    ...
    {
      namespace_replayer->handle_acquire_image(
          global_image_id, instance_id, on_finish);
    }
    ...
    release_image(
        ...
        Context* on_finish) override
    {
      namespace_replayer->handle_release_image(
          global_image_id, instance_id, on_finish);
    }
    ...
    remove_image(
        ...
        Context* on_finish) override
    {
      namespace_replayer->handle_remove_image(
          mirror_uuid, global_image_id, instance_id, on_finish);
    }
  };
  ...
  void handle_init_image_map(
      int r,
      ImageMap<ImageCtxT>* image_map,
      Context* on_finish);
  ...
  void handle_acquire_image(
      const std::string& global_image_id,
      const std::string& instance_id,
      Context* on_finish);
  void handle_release_image(
      const std::string& global_image_id,
      const std::string& instance_id,
      Context* on_finish);
  void handle_remove_image(
      const std::string& mirror_uuid,
      const std::string& global_image_id,
      const std::string& instance_id,
      Context* on_finish);
