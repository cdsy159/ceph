...
#undef dout_prefix
#define dout_prefix                                                       \
  *_dout << "rbd::mirror::image_map::Policy: " << this << " " << __func__ \
         << ": "
...
bool
is_instance_action(ActionType action_type)
{
  switch (action_type) {
    ...
    const std::map<std::string, cls::rbd::MirrorImageMap>& image_mapping)
{
  ...
  for (auto& it : image_mapping) {
    ...
    auto start_action =
        set_state(&image_state, StateTransition::STATE_INITIALIZING, false);
    ceph_assert(start_action);
  }
}

LookupInfo
Policy::lookup(const std::string& global_image_id)
{
...
bool
Policy::add_image(const std::string& global_image_id)
{
  dout(5) << "global_image_id=" << global_image_id << dendl;

  std::unique_lock map_lock{m_map_lock};
  auto image_state_result =
      m_image_states.emplace(global_image_id, ImageState{});
  auto& image_state = image_state_result.first->second;
...
bool
Policy::remove_image(const std::string& global_image_id)
{
  dout(5) << "global_image_id=" << global_image_id << dendl;
    ...
    GlobalImageIds* global_image_ids)
{
  ...
  if (m_initial_update) {
    ...
    std::set<std::string> alive_instances(
  ...
  do_shuffle_add_instances(
      m_map, m_image_states.size(), &shuffle_global_image_ids);
    ...
    GlobalImageIds* global_image_ids)
{
  ...
  for (auto& instance_id : instance_ids) {
    ...
    dout(5) << "force shuffling: instance_id=" << instance_id << ", "
            << "global_image_ids=[" << instance_global_image_ids << "]"
            << dendl;
...
ActionType
Policy::start_action(const std::string& global_image_id)
{
  std::unique_lock map_lock{m_map_lock};
  ...
  if (transition.start_policy_action) {
    execute_policy_action(
        global_image_id, &image_state, *transition.start_policy_action);
...
bool
Policy::finish_action(const std::string& global_image_id, int r)
{
  std::unique_lock map_lock{m_map_lock};
  ...
  dout(5) << "global_image_id=" << global_image_id << ", "
          << "state=" << image_state.state << ", "
          << "action_type=" << transition.action_type << ", " << "r=" << r
          << dendl;

  // retry on failure unless it's an RPC message to an instance that is dead
  if (r < 0 && (!is_instance_action(image_state.transition.action_type) ||
...
void
Policy::map(const std::string& global_image_id, ImageState* image_state)
{
  ceph_assert(ceph_mutex_is_wlocked(m_map_lock));
...
void
Policy::unmap(const std::string& global_image_id, ImageState* image_state)
{
  ceph_assert(ceph_mutex_is_wlocked(m_map_lock));
...
bool
Policy::is_image_shuffling(const std::string& global_image_id)
{
  ...
  auto result =
      is_state_scheduled(image_state, StateTransition::STATE_SHUFFLING);
...
bool
Policy::can_shuffle_image(const std::string& global_image_id)
{
  ceph_assert(ceph_mutex_is_locked(m_map_lock));
  ...
  auto result =
      (StateTransition::is_idle(image_state.state) &&
