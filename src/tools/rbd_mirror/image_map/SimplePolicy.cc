...
#undef dout_prefix
#define dout_prefix                                                 \
  *_dout << "rbd::mirror::image_map::SimplePolicy: " << this << " " \
         << __func__ << ": "
...
size_t
SimplePolicy::calc_images_per_instance(
...
void
SimplePolicy::do_shuffle_add_instances(
    const InstanceToImageMap& map,
    size_t image_count,
    ...
    const std::string& global_image_id)
{
  ...
  dout(20) << "global_image_id=" << global_image_id
           << " maps to instance_id=" << min_it->first << dendl;
  return min_it->first;
}
