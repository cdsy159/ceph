...
#undef dout_prefix
#define dout_prefix                                                          \
  *_dout << "rbd::mirror::image_replayer::journal::" << "SyncPointHandler: " \
         << this << " " << __func__ << ": "
...
SyncPointHandler<I>::get_sync_points() const
{
  ...
  for (auto& sync_point : m_client_meta_copy.sync_points) {
    sync_points.emplace_back(
        sync_point.snap_namespace, sync_point.snap_name,
        sync_point.from_snap_name, sync_point.object_number);
  }
  return sync_points;
}
...
template <typename I>
librbd::SnapSeqs
SyncPointHandler<I>::get_snap_seqs() const
{
...
template <typename I>
void
SyncPointHandler<I>::update_sync_points(
...
template <typename I>
void
SyncPointHandler<I>::handle_update_sync_points(int r, Context* on_finish)
{
  dout(10) << "r=" << r << dendl;
...
template class rbd::mirror::image_replayer::journal::SyncPointHandler<
    librbd::ImageCtx>;
