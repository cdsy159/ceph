...
#undef dout_prefix
#define dout_prefix                                                           \
  *_dout << "rbd::mirror::image_sync::SyncPointPruneRequest: " << this << " " \
         << __func__
...
SyncPointPruneRequest<I>::send()
{
  ...
  if (m_sync_complete) {
    ...
    for (auto it = m_sync_points_copy.rbegin(); it != m_sync_points_copy.rend();
         ++it) {
      auto& sync_point = *it;
...
template <typename I>
void
SyncPointPruneRequest<I>::send_remove_snap()
{
  ...
  m_remote_image_ctx->operations->snap_remove(
      cls::rbd::UserSnapshotNamespace(), snap_name.c_str(), ctx);
}

template <typename I>
void
SyncPointPruneRequest<I>::handle_remove_snap(int r)
{
  dout(20) << ": r=" << r << dendl;
...
template <typename I>
void
SyncPointPruneRequest<I>::send_refresh_image()
{
  dout(20) << dendl;
...
template <typename I>
void
SyncPointPruneRequest<I>::handle_refresh_image(int r)
{
  dout(20) << ": r=" << r << dendl;
...
template <typename I>
void
SyncPointPruneRequest<I>::send_update_sync_points()
{
  dout(20) << dendl;
...
template <typename I>
void
SyncPointPruneRequest<I>::handle_update_sync_points(int r)
{
  dout(20) << ": r=" << r << dendl;
...
template <typename I>
void
SyncPointPruneRequest<I>::finish(int r)
{
  dout(20) << ": r=" << r << dendl;
