...
#define ceph_ctf_stringp(field, val)                                   \
  ctf_string(field, ((val) == NULL || *(val) == NULL) ? "" : *(val))   \
      ctf_integer(uint8_t, field##_isnull, (val) == NULL) ctf_integer( \
          uint8_t, field##_data_isnull, (val) == NULL || *(val) == NULL)
...
#define ceph_ctf_sequence(type, field, val, lentype, lenval)            \
  ctf_integer_hex(void*, field, val) ctf_sequence(                      \
      type, field##_data, (val) == NULL ? "" : (val), lentype,          \
...
#define ceph_ctf_sequencep(type, field, val, lentype, lenval)                 \
  ctf_integer_hex(void*, field, val) ctf_sequence(                            \
      type, field##_data, ((val) == NULL || *(val) == NULL) ? "" : *(val),    \
      lentype,                                                                \
      ((val) == NULL || *(val) == NULL || (lenval) == NULL)                   \
          ? 0                                                                 \
          : CEPH_MIN(*(lenval), CEPH_TRACE_BUF_TRUNC_LEN))                    \
      ctf_integer(uint8_t, field##_isnull, (val) == NULL) ctf_integer(        \
          uint8_t, field##_data_isnull, ((val) == NULL || *(val) == NULL))    \
